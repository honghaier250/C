...
#endif
    parser = (ns ? XML_ParserCreateNS (encodingName, namespaceSeparator) : XML_ParserCreate (encodingName));
    if (!parser)
        return 0;
...
void XML_SetExternalParsedEntityDeclHandler (XML_Parser parser, XML_ExternalParsedEntityDeclHandler handler)
{
    externalParsedEntityDeclHandler = handler;
}

void XML_SetInternalParsedEntityDeclHandler (XML_Parser parser, XML_InternalParsedEntityDeclHandler handler)
{
...
static enum XML_Error processXmlDecl (XML_Parser parser, int isGeneralTextEntity, const char *s, const char *next)
{
    ...
    const char *version;
    int standalone = -1;
    if (!(ns ? XmlParseXmlDeclNS : XmlParseXmlDecl) (isGeneralTextEntity, encoding, s, next, &eventPtr, &version, &encodingName, &newEncoding, &standalone))
        ...
        *eventEndPP = next;
        ...
        switch (role)
        {
            ...
            case XML_ROLE_REQUIRED_ATTRIBUTE_VALUE:
                if (dtd.complete && !defineAttribute (declElementType, declAttributeId, declAttributeIsCdata, declAttributeIsId, 0))
                    return XML_ERROR_NO_MEMORY;
                break;
            ...
            case XML_ROLE_ENTITY_VALUE:
                {
                    ...
                    if (declEntity)
                    {
                        ...
                        if (internalParsedEntityDeclHandler
                            ...
                            && ((ENTITY *) lookup (&dtd.generalEntities, declEntity->name, 0) == declEntity))
                        {
                            *eventEndPP = s;
                            internalParsedEntityDeclHandler (handlerArg, declEntity->name, declEntity->textPtr, declEntity->textLen);
                        }
                    }
                    else
                        poolDiscard (&dtd.pool);
            ...
            case XML_ROLE_EXTERNAL_GENERAL_ENTITY_NO_NOTATION:
                ...
                {
                    *eventEndPP = s;
                    externalParsedEntityDeclHandler (handlerArg, declEntity->name, declEntity->base, declEntity->systemId, declEntity->publicId);
                }
                break;
...
static int copyEntityTable (HASH_TABLE * newTable, STRING_POOL * newPool, const HASH_TABLE * oldTable)
{
    HASH_TABLE_ITER iter;
