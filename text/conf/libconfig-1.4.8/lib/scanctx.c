...
static const char *__scanctx_add_filename (struct scan_context *ctx, const char *filename)
{
	...
	{
	    ctx->filenames = (const char **) realloc ((void *) ctx->filenames, (ctx->num_filenames + CHUNK_SIZE) * sizeof (const char *));
	}
...
FILE *scanctx_push_include (struct scan_context * ctx, void *buffer, const char **error)
{
    FILE *fp = NULL;
    const char *file;
    char *full_file = NULL;
