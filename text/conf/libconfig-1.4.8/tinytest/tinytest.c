...
static tt_bool_t __tt_compare_files (const char *file1, const char *file2, tt_bool_t verbose)
{
    FILE *fp1, *fp2;
...
void tt_suite_add_test (tt_testsuite_t * suite, const char *name, void (*function) (void))
{
...
void tt_suite_run (tt_testsuite_t * suite)
{
    ...
    for (suite->current_test = suite->first_test; suite->current_test; suite->current_test = suite->current_test->next)
	{
    ...
    printf ("%d tests; %d passed, %d failed\n", suite->num_tests, suite->num_tests - suite->num_failures, suite->num_failures);
...
void tt_expect (const char *file, int line, const char *aexpr, tt_op_t op, const char *bexpr, const tt_val_t a, const tt_val_t b, tt_bool_t fatal)
{
    ...
    if (!result)
	{
	    ...
	    printf ("%s:%d: failed %s: %s [", file, line, (fatal ? "assert" : "expect"), aexpr);
...
void tt_expect_bool (const char *file, int line, const char *expr, tt_op_t op, int val, tt_bool_t fatal)
{
    tt_bool_t result = TT_FALSE;
    ...
    if (!result)
	{
	    ...
	    printf ("%s:%d: failed %s: %s(%s)\n", file, line, (fatal ? "assert" : "expect"), __tt_op_strings[op], expr);
