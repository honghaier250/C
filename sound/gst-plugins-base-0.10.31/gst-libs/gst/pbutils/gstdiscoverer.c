...
G_DEFINE_TYPE_EXTENDED(GstDiscoverer, gst_discoverer, G_TYPE_OBJECT, 0, _do_init());
...
static void gst_discoverer_set_timeout(GstDiscoverer * dc, GstClockTime timeout);
static gboolean async_timeout_cb(GstDiscoverer * dc);

static void discoverer_bus_cb(GstBus * bus, GstMessage * msg, GstDiscoverer * dc);
static void uridecodebin_pad_added_cb(GstElement * uridecodebin, GstPad * pad, GstDiscoverer * dc);
static void uridecodebin_pad_removed_cb(GstElement * uridecodebin, GstPad * pad, GstDiscoverer * dc);

static void gst_discoverer_dispose(GObject * dc);
static void gst_discoverer_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_discoverer_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

static void gst_discoverer_class_init(GstDiscovererClass * klass)
{
   ...
   */
    g_object_class_install_property(gobject_class, PROP_TIMEOUT, g_param_spec_uint64("timeout", "timeout", "Timeout", GST_SECOND, 3600 * GST_SECOND, DEFAULT_PROP_TIMEOUT, G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS));
   ...
   */
    gst_discoverer_signals[SIGNAL_FINISHED] = g_signal_new("finished", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstDiscovererClass, finished), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);
   ...
   */
    gst_discoverer_signals[SIGNAL_STARTING] = g_signal_new("starting", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstDiscovererClass, starting), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);
   ...
   */
    gst_discoverer_signals[SIGNAL_DISCOVERED] =
        g_signal_new("discovered", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstDiscovererClass, discovered), NULL, NULL, pbutils_marshal_VOID__POINTER_BOXED, G_TYPE_NONE, 2, GST_TYPE_DISCOVERER_INFO, GST_TYPE_G_ERROR);
}

static void uridecodebin_element_added_cb(GstElement * uridecodebin, GstElement * child, GstDiscoverer * dc)
{
    GST_DEBUG("New element added to uridecodebin : %s", GST_ELEMENT_NAME(child));
...
static void gst_discoverer_init(GstDiscoverer * dc)
{
    GstElement *tmp;

    dc->priv = G_TYPE_INSTANCE_GET_PRIVATE(dc, GST_TYPE_DISCOVERER, GstDiscovererPrivate);
    ...
    dc->priv->uridecodebin = gst_element_factory_make("uridecodebin", "discoverer-uri");
    ...
    g_signal_connect(dc->priv->uridecodebin, "pad-added", G_CALLBACK(uridecodebin_pad_added_cb), dc);
    g_signal_connect(dc->priv->uridecodebin, "pad-removed", G_CALLBACK(uridecodebin_pad_removed_cb), dc);

    GST_LOG_OBJECT(dc, "Getting pipeline bus");
    dc->priv->bus = gst_pipeline_get_bus((GstPipeline *) dc->priv->pipeline);

    g_signal_connect(dc->priv->bus, "message", G_CALLBACK(discoverer_bus_cb), dc);
    ...
    g_signal_connect(dc->priv->uridecodebin, "element-added", G_CALLBACK(uridecodebin_element_added_cb), dc);
    tmp = gst_element_factory_make("decodebin2", NULL);
    dc->priv->decodebin2_type = G_OBJECT_TYPE(tmp);
    gst_object_unref(tmp);
}

static void discoverer_reset(GstDiscoverer * dc)
{
...
static void gst_discoverer_dispose(GObject * obj)
{
...
static gboolean _event_probe(GstPad * pad, GstEvent * event, PrivateStream * ps)
{
...
static void uridecodebin_pad_added_cb(GstElement * uridecodebin, GstPad * pad, GstDiscoverer * dc)
{
    ...
    if (!gst_element_link_pads_full(ps->queue, "src", ps->sink, "sink", GST_PAD_LINK_CHECK_NOTHING))
        goto error;
...
static GstStructure *collect_stream_information(GstDiscoverer * dc, PrivateStream * ps, guint idx)
{
    GstCaps *caps;
    GstStructure *st;
    gchar *stname;
    ...
    if (!caps)
    {
        GST_WARNING("Couldn't get negotiated caps from %s:%s", GST_DEBUG_PAD_NAME(ps->pad));
 ...
 */
static GstDiscovererStreamInfo *collect_information(GstDiscoverer * dc, const GstStructure * st, GstDiscovererStreamInfo * parent)
{
    GstCaps *caps;
    ...
    if (!st || !gst_structure_id_has_field(st, _CAPS_QUARK))
    {
        ...
        if (parent)
            ...
            return (GstDiscovererStreamInfo *) gst_mini_object_new(GST_TYPE_DISCOVERER_STREAM_INFO);
    }
    ...
    if (g_str_has_prefix(name, "audio/"))
    {
        ...
        {
            info = (GstDiscovererAudioInfo *) gst_mini_object_new(GST_TYPE_DISCOVERER_AUDIO_INFO);
        ...
        {
            gst_structure_id_get(st, _TAGS_QUARK, GST_TYPE_STRUCTURE, &tags_st, NULL);
            ...
            info->parent.tags = gst_tag_list_merge(info->parent.tags, (GstTagList *) tags_st, GST_TAG_MERGE_REPLACE);
    ...
    else if (g_str_has_prefix(name, "video/") || g_str_has_prefix(name, "image/"))
    {
        ...
        {
            info = (GstDiscovererVideoInfo *) gst_mini_object_new(GST_TYPE_DISCOVERER_VIDEO_INFO);
            info->parent.caps = caps;
        }
    ...
    else
    {
        ...
        {
            info = (GstDiscovererStreamInfo *) gst_mini_object_new(GST_TYPE_DISCOVERER_STREAM_INFO);
            info->caps = caps;
        }

        if (gst_structure_id_get(st, _TAGS_QUARK, GST_TYPE_STRUCTURE, &tags_st, NULL))
        {
            info->tags = gst_tag_list_merge(info->tags, (GstTagList *) tags_st, GST_TAG_MERGE_REPLACE);
            gst_structure_free(tags_st);
        }
...
static GstStructure *find_stream_for_node(GstDiscoverer * dc, const GstStructure * topology)
{
    GstPad *pad;
    ...
    gst_structure_id_get(topology, _TOPOLOGY_PAD_QUARK, GST_TYPE_PAD, &pad, NULL);
...
static gboolean child_is_raw_stream(GstCaps * parent, GstCaps * child)
{
    ...
    if ((g_str_has_prefix(name1, "audio/") && g_str_has_prefix(name2, "audio/x-raw")) || ((g_str_has_prefix(name1, "video/") || g_str_has_prefix(name1, "image/")) && g_str_has_prefix(name2, "video/x-raw")))
    {
 ...
 */
static GstDiscovererStreamInfo *parse_stream_topology(GstDiscoverer * dc, const GstStructure * topology, GstDiscovererStreamInfo * parent)
{
    ...
    if (nval == NULL || GST_VALUE_HOLDS_STRUCTURE(nval))
    {
        ...
        if (add_to_list)
        {
            dc->priv->current_info->stream_list = g_list_append(dc->priv->current_info->stream_list, res);
        }
    ...
    else if (GST_VALUE_HOLDS_LIST(nval))
    {
        ...
        if (!gst_structure_id_get(topology, _CAPS_QUARK, GST_TYPE_CAPS, &caps, NULL))
            GST_WARNING("Couldn't find caps !");

        len = gst_value_list_get_size(nval);
        GST_DEBUG("next is a list of %d entries", len);

        cont = (GstDiscovererContainerInfo *) gst_mini_object_new(GST_TYPE_DISCOVERER_CONTAINER_INFO);
        cont->parent.caps = caps;
        res = (GstDiscovererStreamInfo *) cont;

        if (gst_structure_id_has_field(topology, _TAGS_QUARK))
        {
            gst_structure_id_get(topology, _TAGS_QUARK, GST_TYPE_STRUCTURE, &tags, NULL);
            cont->parent.tags = gst_tag_list_merge(cont->parent.tags, (GstTagList *) tags, GST_TAG_MERGE_APPEND);
        ...
        for (i = 0; i < len; i++)
        {
            ...
            cont->streams = g_list_append(cont->streams, gst_discoverer_stream_info_ref(substream));
        }
    }
...
static void discoverer_collect(GstDiscoverer * dc)
{
    ...
    if (dc->priv->streams)
    {
        ...
        if (TRUE)
        {
            ...
            if (gst_element_query_duration((GstElement *) dc->priv->pipeline, &format, &dur))
            {
        ...
        if (dc->priv->current_topology)
            dc->priv->current_info->stream_info = parse_stream_topology(dc, dc->priv->current_topology, NULL);
         ...
         */
        if (dc->priv->current_info->duration == 0 && dc->priv->current_info->stream_info != NULL && dc->priv->current_info->stream_info->next == NULL)
        {
            GstStructure *st = gst_caps_get_structure(dc->priv->current_info->stream_info->caps, 0);

            if (g_str_has_prefix(gst_structure_get_name(st), "image/"))
                ((GstDiscovererVideoInfo *) dc->priv->current_info->stream_info)->is_image = TRUE;
        }
    }
    ...
    if (dc->priv->async)
    {
        ...
        g_signal_emit(dc, gst_discoverer_signals[SIGNAL_DISCOVERED], 0, dc->priv->current_info, dc->priv->current_error);
...
static gboolean handle_message(GstDiscoverer * dc, GstMessage * msg)
{
    gboolean done = FALSE;
        ...
        case GST_MESSAGE_ELEMENT:
            {
                ...
                GST_DEBUG_OBJECT(GST_MESSAGE_SRC(msg), "structure %" GST_PTR_FORMAT, msg->structure);
        ...
        case GST_MESSAGE_TAG:
            {
                ...
                dc->priv->current_info->tags = gst_tag_list_merge(dc->priv->current_info->tags, tl, GST_TAG_MERGE_APPEND);
...
static void handle_current_sync(GstDiscoverer * dc)
{
    GTimer *timer;
...
static void _setup_locked(GstDiscoverer * dc)
{
    ...
    dc->priv->current_info = (GstDiscovererInfo *) gst_mini_object_new(GST_TYPE_DISCOVERER_INFO);
    dc->priv->current_info->uri = (gchar *) dc->priv->pending_uris->data;
    dc->priv->pending_uris = g_list_delete_link(dc->priv->pending_uris, dc->priv->pending_uris);

    /* set uri on uridecodebin */
    g_object_set(dc->priv->uridecodebin, "uri", dc->priv->current_info->uri, NULL);
    ...
    ret = gst_element_set_state((GstElement *) dc->priv->pipeline, GST_STATE_PAUSED);
    DISCO_LOCK(dc);

    GST_DEBUG_OBJECT(dc, "Pipeline going to PAUSED : %s", gst_element_state_change_return_get_name(ret));
}

static void discoverer_cleanup(GstDiscoverer * dc)
{
 ...
 * else a error flag.
 */
static GstDiscovererResult start_discovering(GstDiscoverer * dc)
{
...
void gst_discoverer_stop(GstDiscoverer * discoverer)
{
    ...
    if (!discoverer->priv->async)
    {
        GST_DEBUG_OBJECT(discoverer, "We were already stopped, or running synchronously");
        return;
    }
    ...
    if (discoverer->priv->processing)
    {
        ...
        gst_element_set_state((GstElement *) discoverer->priv->pipeline, GST_STATE_READY);
    }
...
gboolean gst_discoverer_discover_uri_async(GstDiscoverer * discoverer, const gchar * uri)
{
    ...
    discoverer->priv->pending_uris = g_list_append(discoverer->priv->pending_uris, g_strdup(uri));
    DISCO_UNLOCK(discoverer);
...
GstDiscovererInfo *gst_discoverer_discover_uri(GstDiscoverer * discoverer, const gchar * uri, GError ** err)
{
    ...
    {
        GST_DEBUG("Setting result to %d (was %d)", res, discoverer->priv->current_info->result);
...
GstDiscoverer *gst_discoverer_new(GstClockTime timeout, GError ** err)
{
    ...
    {
        if (err)
            *err = g_error_new(GST_CORE_ERROR, GST_CORE_ERROR_MISSING_PLUGIN, "Couldn't create 'uridecodebin' element");
