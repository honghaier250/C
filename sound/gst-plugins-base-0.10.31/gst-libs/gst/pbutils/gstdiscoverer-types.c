...
static GstDiscovererStreamInfo * gst_discoverer_info_copy_int(GstDiscovererStreamInfo * info, GHashTable * stream_map);

static GstDiscovererContainerInfo * gst_stream_container_info_copy_int(GstDiscovererContainerInfo * ptr, GHashTable * stream_map);

static GstDiscovererAudioInfo * gst_discoverer_audio_info_copy_int(GstDiscovererAudioInfo * ptr);

static GstDiscovererVideoInfo * gst_discoverer_video_info_copy_int(GstDiscovererVideoInfo * ptr);

/* Per-stream information */

G_DEFINE_TYPE(GstDiscovererStreamInfo, gst_discoverer_stream_info, GST_TYPE_MINI_OBJECT);

static void gst_discoverer_stream_info_init(GstDiscovererStreamInfo * info)
{
    /* Nothing needs initialization */
}

static void gst_discoverer_stream_info_finalize(GstDiscovererStreamInfo * info)
{
    if (info->next)
...
static GstDiscovererStreamInfo *gst_discoverer_stream_info_copy(GstDiscovererStreamInfo * info)
{
    return gst_discoverer_info_copy_int(info, NULL);
}

static void gst_discoverer_stream_info_class_init(GstMiniObjectClass * klass)
{
    klass->finalize = (GstMiniObjectFinalizeFunction) gst_discoverer_stream_info_finalize;
    klass->copy = (GstMiniObjectCopyFunction) gst_discoverer_stream_info_copy;
}

static GstDiscovererStreamInfo *gst_discoverer_stream_info_new(void)
{
...
static GstDiscovererStreamInfo *gst_discoverer_info_copy_int(GstDiscovererStreamInfo * info, GHashTable * stream_map)
{
    ...
    {
        ret = (GstDiscovererStreamInfo *) gst_stream_container_info_copy_int((GstDiscovererContainerInfo *) info, stream_map);
    }
    else if (ltyp == GST_TYPE_DISCOVERER_AUDIO_INFO)
    {
        ret = (GstDiscovererStreamInfo *) gst_discoverer_audio_info_copy_int((GstDiscovererAudioInfo *) info);

    }
    else if (ltyp == GST_TYPE_DISCOVERER_VIDEO_INFO)
    {
        ret = (GstDiscovererStreamInfo *) gst_discoverer_video_info_copy_int((GstDiscovererVideoInfo *) info);
...
G_DEFINE_TYPE(GstDiscovererContainerInfo, gst_discoverer_container_info, GST_TYPE_DISCOVERER_STREAM_INFO);

static void gst_discoverer_container_info_init(GstDiscovererContainerInfo * info)
{
    /* Nothing to initialize */
}

static GstDiscovererContainerInfo *gst_discoverer_container_info_new(void)
{
    return (GstDiscovererContainerInfo *) gst_mini_object_new(GST_TYPE_DISCOVERER_CONTAINER_INFO);
}

static void gst_discoverer_container_info_finalize(GstDiscovererContainerInfo * info)
{
    GList *tmp;

    for (tmp = ((GstDiscovererContainerInfo *) info)->streams; tmp; tmp = tmp->next)
...
static void gst_discoverer_container_info_class_init(GstMiniObjectClass * klass)
{
    klass->finalize = (GstMiniObjectFinalizeFunction) gst_discoverer_container_info_finalize;
}
...
static GstDiscovererContainerInfo *gst_stream_container_info_copy_int(GstDiscovererContainerInfo * ptr, GHashTable * stream_map)
{
    ...
    for (tmp = ((GstDiscovererContainerInfo *) ptr)->streams; tmp; tmp = tmp->next)
    {
...
/* Audio information */
G_DEFINE_TYPE(GstDiscovererAudioInfo, gst_discoverer_audio_info, GST_TYPE_DISCOVERER_STREAM_INFO);

static void gst_discoverer_audio_info_class_init(GstDiscovererAudioInfoClass * klass)
{
    /* Nothing to initialize */
}

static void gst_discoverer_audio_info_init(GstDiscovererAudioInfo * info)
{
    /* Nothing to initialize */
}

static GstDiscovererAudioInfo *gst_discoverer_audio_info_new(void)
{
    return (GstDiscovererAudioInfo *) gst_mini_object_new(GST_TYPE_DISCOVERER_AUDIO_INFO);
}
...
/* Video information */
G_DEFINE_TYPE(GstDiscovererVideoInfo, gst_discoverer_video_info, GST_TYPE_DISCOVERER_STREAM_INFO);

static void gst_discoverer_video_info_class_init(GstMiniObjectClass * klass)
{
    /* Nothing to initialize */
}

static void gst_discoverer_video_info_init(GstDiscovererVideoInfo * info)
{
    /* Nothing to initialize */
}

static GstDiscovererVideoInfo *gst_discoverer_video_info_new(void)
{
    return (GstDiscovererVideoInfo *) gst_mini_object_new(GST_TYPE_DISCOVERER_VIDEO_INFO);
}
...
static void gst_discoverer_info_init(GstDiscovererInfo * info)
{
    /* Nothing needs initialization */
}

static void gst_discoverer_info_finalize(GstDiscovererInfo * info)
{
    g_free(info->uri);
...
static GstDiscovererInfo *gst_discoverer_info_new(void)
{
...
GstDiscovererInfo *gst_discoverer_info_copy(GstDiscovererInfo * ptr)
{
    ...
    if (ptr->stream_info)
    {
        ret->stream_info = gst_discoverer_info_copy_int(ptr->stream_info, stream_map);
    }
...
static void gst_discoverer_info_class_init(GstMiniObjectClass * klass)
{
    klass->finalize = (GstMiniObjectFinalizeFunction) gst_discoverer_info_finalize;
...
{
    return gst_discoverer_info_get_streams(info, GST_TYPE_DISCOVERER_CONTAINER_INFO);
}
...
const gchar *gst_discoverer_stream_info_get_stream_type_nick(GstDiscovererStreamInfo * info)
{
    ...
    {
        if (gst_discoverer_video_info_is_image((GstDiscovererVideoInfo *) info))
            return "video(image)";
        else
            return "video";
    }
...
GList *gst_discoverer_container_info_get_streams(GstDiscovererContainerInfo * info)
{
    ...
    for (tmp = info->streams; tmp; tmp = tmp->next)
        res = g_list_append(res, gst_discoverer_stream_info_ref((GstDiscovererStreamInfo *) tmp->data));
