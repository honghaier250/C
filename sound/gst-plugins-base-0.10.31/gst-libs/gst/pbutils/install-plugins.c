 ...
 * Since: 0.10.12
 */
void gst_install_plugins_context_set_xid(GstInstallPluginsContext * ctx, guint xid)
{
...
static GstInstallPluginsContext *gst_install_plugins_context_copy(GstInstallPluginsContext * ctx)
{
...
GType gst_install_plugins_context_get_type(void)
{
    ...
    {
        gst_ipc_type = g_boxed_type_register_static("GstInstallPluginsContext", (GBoxedCopyFunc) gst_install_plugins_context_copy, (GBoxedFreeFunc) gst_install_plugins_context_free);
    }
    return gst_ipc_type;
}

static const gchar *gst_install_plugins_get_helper(void)
{
    const gchar *helper;
...
static gboolean ptr_array_contains_string(GPtrArray * arr, const gchar * s)
{
    gint i;
...
static gboolean gst_install_plugins_spawn_child(gchar ** details, GstInstallPluginsContext * ctx, GPid * child_pid, gint * exit_status)
{
    GPtrArray *arr;
    gboolean ret;
    GError *err = NULL;
    ...
    if (child_pid == NULL && exit_status != NULL)
    {
        ...
        ret = g_spawn_sync(NULL, argv, NULL, (GSpawnFlags) 0, NULL, NULL, NULL, NULL, exit_status, &err);
        install_in_progress = FALSE;
    }
    else if (child_pid != NULL && exit_status == NULL)
    {
        install_in_progress = TRUE;
        ret = g_spawn_async(NULL, argv, NULL, G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL, child_pid, &err);
    }
    else
    {
...
static GstInstallPluginsReturn gst_install_plugins_return_from_status(gint status)
{
    ...
    else
    {
        ...
        if ((ret < 0 || ret >= GST_INSTALL_PLUGINS_STARTED_OK) && ret != GST_INSTALL_PLUGINS_INTERNAL_FAILURE)
        {
            ret = GST_INSTALL_PLUGINS_INVALID;
        }
    }

    GST_LOG("plugin installer exited with status 0x%04x = %s", status, gst_install_plugins_return_get_name(ret));
...
static void gst_install_plugins_installer_exited(GPid pid, gint status, gpointer data)
{
...
GstInstallPluginsReturn gst_install_plugins_async(gchar ** details, GstInstallPluginsContext * ctx, GstInstallPluginsResultFunc func, gpointer user_data)
{
    GstInstallPluginsAsyncHelper *helper;
    GPid pid;
    ...
    if (!g_file_test(gst_install_plugins_get_helper(), G_FILE_TEST_IS_EXECUTABLE))
 ...
 * Since: 0.10.12
 */
GstInstallPluginsReturn gst_install_plugins_sync(gchar ** details, GstInstallPluginsContext * ctx)
{
    gint status;
...
{
    return g_file_test(gst_install_plugins_get_helper(), G_FILE_TEST_IS_EXECUTABLE);
}
