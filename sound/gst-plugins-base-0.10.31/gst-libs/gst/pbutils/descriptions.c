...
static gchar *format_info_get_desc(const FormatInfo * info, const GstCaps * caps)
{
    ...
    else if (strcmp(info->type, "video/x-raw-rgb") == 0)
    {
        ...
        {
            return g_strdup_printf(_("Uncompressed palettized %d-bit %s"), depth, rgb_str);
        }
        else
        {
    ...
    else if (strcmp(info->type, "video/x-msmpeg") == 0)
    {
        ...
        if (!gst_structure_get_int(s, "msmpegversion", &ver) || ver < 40 || ver > 49)
        {
    ...
    else if (strcmp(info->type, "video/mpeg") == 0)
    {
        ...
        if (!gst_structure_get_boolean(s, "systemstream", &sysstream) || !gst_structure_get_int(s, "mpegversion", &ver) || ver < 1 || ver > 4)
        {
...
static const FormatInfo *find_format_info(const GstCaps * caps)
{
    const GstStructure *s;
...
static gboolean caps_are_rtp_caps(const GstCaps * caps, const gchar * media, gchar ** format)
{
    const GstStructure *s;
    const gchar *str;
...
#if 0
void gst_pb_utils_list_all(void)
{
    gint i;
