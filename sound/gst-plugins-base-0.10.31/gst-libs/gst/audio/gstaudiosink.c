...
static void gst_audioringbuffer_init(GstAudioRingBuffer * ringbuffer, GstAudioRingBufferClass * klass);
...
static gboolean gst_audioringbuffer_acquire(GstRingBuffer * buf, GstRingBufferSpec * spec);
...
static gboolean gst_audioringbuffer_activate(GstRingBuffer * buf, gboolean active);

/* ringbuffer abstract base class */
static GType gst_audioringbuffer_get_type(void)
{
    ...
    if (!ringbuffer_type)
    {
        ...
        ringbuffer_type = g_type_register_static(GST_TYPE_RING_BUFFER, "GstAudioSinkRingBuffer", &ringbuffer_info, 0);
    }
...
static void gst_audioringbuffer_class_init(GstAudioRingBufferClass * klass)
{
    ...
    gstringbuffer_class->open_device = GST_DEBUG_FUNCPTR(gst_audioringbuffer_open_device);
    gstringbuffer_class->close_device = GST_DEBUG_FUNCPTR(gst_audioringbuffer_close_device);
    gstringbuffer_class->acquire = GST_DEBUG_FUNCPTR(gst_audioringbuffer_acquire);
    gstringbuffer_class->release = GST_DEBUG_FUNCPTR(gst_audioringbuffer_release);
    ...
    gstringbuffer_class->activate = GST_DEBUG_FUNCPTR(gst_audioringbuffer_activate);
}
 ...
 */
static void audioringbuffer_thread_func(GstRingBuffer * buf)
{
    GstAudioSink *sink;
    ...
    message = gst_message_new_stream_status(GST_OBJECT_CAST(buf), GST_STREAM_STATUS_TYPE_ENTER, GST_ELEMENT_CAST(sink));
    ...
    while (TRUE)
    {
        ...
        if (gst_ring_buffer_prepare_read(buf, &readseg, &readptr, &len))
        {
            ...
            do
            {
                ...
                GST_LOG_OBJECT(sink, "transfered %d bytes of %d from segment %d", written, left, readseg);
                if (written < 0 || written > left)
                {
                    /* might not be critical, it e.g. happens when aborting playback */
                    GST_WARNING_OBJECT(sink, "error writing data in %s (reason: %s), skipping segment (left: %d, written: %d)", GST_DEBUG_FUNCPTR_NAME(writefunc), (errno > 1 ? g_strerror(errno) : "unknown"), left, written);
                    break;
                }
                left -= written;
                readptr += written;
...
stop_running:
    {
        ...
        message = gst_message_new_stream_status(GST_OBJECT_CAST(buf), GST_STREAM_STATUS_TYPE_LEAVE, GST_ELEMENT_CAST(sink));
        gst_message_set_stream_status_object(message, &val);
...
static void gst_audioringbuffer_init(GstAudioRingBuffer * ringbuffer, GstAudioRingBufferClass * g_class)
{
...
static gboolean gst_audioringbuffer_activate(GstRingBuffer * buf, gboolean active)
{
    ...
    if (active)
    {
        ...
        sink->thread = g_thread_create((GThreadFunc) audioringbuffer_thread_func, buf, TRUE, &error);
...
GST_BOILERPLATE_FULL(GstAudioSink, gst_audio_sink, GstBaseAudioSink, GST_TYPE_BASE_AUDIO_SINK, _do_init);

static GstRingBuffer *gst_audio_sink_create_ringbuffer(GstBaseAudioSink * sink);

static void gst_audio_sink_base_init(gpointer g_class)
{
}

static void gst_audio_sink_class_init(GstAudioSinkClass * klass)
{
    GstBaseAudioSinkClass *gstbaseaudiosink_class;

    gstbaseaudiosink_class = (GstBaseAudioSinkClass *) klass;

    gstbaseaudiosink_class->create_ringbuffer = GST_DEBUG_FUNCPTR(gst_audio_sink_create_ringbuffer);

    g_type_class_ref(GST_TYPE_AUDIORING_BUFFER);
}

static void gst_audio_sink_init(GstAudioSink * audiosink, GstAudioSinkClass * g_class)
{
}
