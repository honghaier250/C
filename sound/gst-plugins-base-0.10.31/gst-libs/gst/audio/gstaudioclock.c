...
static void gst_audio_clock_class_init(GstAudioClockClass * klass)
{
    ...
    GST_DEBUG_CATEGORY_INIT(gst_audio_clock_debug, "audioclock", 0, "audioclock");
}
...
{
    GstAudioClock *aclock = GST_AUDIO_CLOCK(g_object_new(GST_TYPE_AUDIO_CLOCK, "name", name, NULL));
...
void gst_audio_clock_reset(GstAudioClock * clock, GstClockTime time)
{
    ...
    GST_DEBUG_OBJECT(clock, "reset clock to %" GST_TIME_FORMAT ", last %" GST_TIME_FORMAT ", offset %" GST_TIME_FORMAT, GST_TIME_ARGS(time), GST_TIME_ARGS(clock->last_time), GST_TIME_ARGS(time_offset));
}

static GstClockTime gst_audio_clock_func_invalid(GstClock * clock, gpointer user_data)
{
...
static GstClockTime gst_audio_clock_get_internal_time(GstClock * clock)
{
    ...
    GST_DEBUG_OBJECT(clock, "result %" GST_TIME_FORMAT ", last_time %" GST_TIME_FORMAT, GST_TIME_ARGS(result), GST_TIME_ARGS(aclock->last_time));
