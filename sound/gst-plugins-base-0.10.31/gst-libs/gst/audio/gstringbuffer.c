...
static guint default_commit(GstRingBuffer * buf, guint64 * sample, guchar * data, gint in_samples, gint out_samples, gint * accum);
...
static void gst_ring_buffer_class_init(GstRingBufferClass * klass)
{
    ...
    GST_DEBUG_CATEGORY_INIT(gst_ring_buffer_debug, "ringbuffer", 0, "ringbuffer class");
...
static void gst_ring_buffer_dispose(GObject * object)
{
    ...
    G_OBJECT_CLASS(gst_ring_buffer_parent_class)->dispose(G_OBJECT(ringbuffer));
}
...
static void gst_ring_buffer_finalize(GObject * object)
{
    ...
    G_OBJECT_CLASS(gst_ring_buffer_parent_class)->finalize(G_OBJECT(ringbuffer));
}
...
static const FormatDef *build_linear_format(int depth, int width, int unsignd, int big_endian)
{
...
void gst_ring_buffer_debug_spec_caps(GstRingBufferSpec * spec)
{
    ...
    GST_DEBUG("parsed caps: type:         %d, '%s'", spec->type, format_type_names[spec->type]);
    GST_DEBUG("parsed caps: format:       %d, '%s'", spec->format, format_names[spec->format]);
...
{
    GST_DEBUG("acquire ringbuffer: buffer time: %" G_GINT64_FORMAT " usec", spec->buffer_time);
    GST_DEBUG("acquire ringbuffer: latency time: %" G_GINT64_FORMAT " usec", spec->latency_time);
    ...
    GST_DEBUG("acquire ringbuffer: segment size: %d bytes = %d samples", spec->segsize, spec->segsize / spec->bytes_per_sample);
    GST_DEBUG("acquire ringbuffer: buffer size: %d bytes = %d samples", spec->segsize * spec->segtotal, spec->segsize * spec->segtotal / spec->bytes_per_sample);
}
...
gboolean gst_ring_buffer_parse_caps(GstRingBufferSpec * spec, GstCaps * caps)
{
    ...
    if (!strncmp(mimetype, "audio/x-raw-int", 15))
    {
        ...
        if (!(gst_structure_get_int(structure, "rate", &spec->rate) &&
              ...
              gst_structure_get_int(structure, "width", &spec->width) && gst_structure_get_int(structure, "depth", &spec->depth) && gst_structure_get_boolean(structure, "signed", &spec->sign)))
            goto parse_error;
        ...
        def = build_linear_format(spec->depth, spec->width, spec->sign ? 0 : 1, spec->bigend ? 1 : 0);
    ...
    else if (!strncmp(mimetype, "audio/x-raw-float", 17))
    {
        ...
        /* extract the needed information from the cap */
        if (!(gst_structure_get_int(structure, "rate", &spec->rate) && gst_structure_get_int(structure, "channels", &spec->channels) && gst_structure_get_int(structure, "width", &spec->width)))
            goto parse_error;
        ...
        {
            case 32:
                spec->format = G_BYTE_ORDER == G_LITTLE_ENDIAN ? GST_FLOAT32_LE : GST_FLOAT32_BE;
                break;
            case 64:
                spec->format = G_BYTE_ORDER == G_LITTLE_ENDIAN ? GST_FLOAT64_LE : GST_FLOAT64_BE;
                break;
            default:
                goto parse_error;
        }
    ...
    {
        /* extract the needed information from the cap */
        if (!(gst_structure_get_int(structure, "rate", &spec->rate) && gst_structure_get_int(structure, "channels", &spec->channels)))
            goto parse_error;
    ...
    spec->segsize = gst_util_uint64_scale(spec->rate * spec->bytes_per_sample, spec->latency_time, GST_SECOND / GST_USECOND);
...
gboolean gst_ring_buffer_convert(GstRingBuffer * buf, GstFormat src_fmt, gint64 src_val, GstFormat dest_fmt, gint64 * dest_val)
{
    ...
    GST_DEBUG("converting value %" G_GINT64_FORMAT " from %s (%d) to %s (%d)", src_val, gst_format_get_name(src_fmt), src_fmt, gst_format_get_name(dest_fmt), dest_fmt);
        ...
        case GST_FORMAT_BYTES:
            switch (dest_fmt)
            {
                case GST_FORMAT_TIME:
                    *dest_val = gst_util_uint64_scale_int(src_val / bps, GST_SECOND, rate);
                    break;
...
invalid_bps:
    {
        g_warning("invalid bytes_per_sample from acquire ringbuffer %p, fix the element", buf);
        buf->acquired = FALSE;
        res = FALSE;
        goto done;
    }
...
gboolean gst_ring_buffer_start(GstRingBuffer * buf)
{
    ...
    res = g_atomic_int_compare_and_exchange(&buf->state, GST_RING_BUFFER_STATE_STOPPED, GST_RING_BUFFER_STATE_STARTED);

    if (!res)
    {
        GST_DEBUG_OBJECT(buf, "was not stopped, try paused");
        /* was not stopped, try from paused */
        res = g_atomic_int_compare_and_exchange(&buf->state, GST_RING_BUFFER_STATE_PAUSED, GST_RING_BUFFER_STATE_STARTED);
        if (!res)
        {
...
static gboolean gst_ring_buffer_pause_unlocked(GstRingBuffer * buf)
{
    ...
    res = g_atomic_int_compare_and_exchange(&buf->state, GST_RING_BUFFER_STATE_STARTED, GST_RING_BUFFER_STATE_PAUSED);
...
gboolean gst_ring_buffer_stop(GstRingBuffer * buf)
{
    ...
    res = g_atomic_int_compare_and_exchange(&buf->state, GST_RING_BUFFER_STATE_STARTED, GST_RING_BUFFER_STATE_STOPPED);

    if (!res)
    {
        GST_DEBUG_OBJECT(buf, "was not started, try paused");
        /* was not started, try from paused */
        res = g_atomic_int_compare_and_exchange(&buf->state, GST_RING_BUFFER_STATE_PAUSED, GST_RING_BUFFER_STATE_STOPPED);
        if (!res)
        {
...
static gboolean wait_segment(GstRingBuffer * buf)
{
    ...
    if (G_UNLIKELY(g_atomic_int_get(&buf->state) != GST_RING_BUFFER_STATE_STARTED))
    {
...
static guint default_commit(GstRingBuffer * buf, guint64 * sample, guchar * data, gint in_samples, gint out_samples, gint * accum)
{
    ...
    while (*toprocess > 0)
    {
        ...
        while (TRUE)
        {
            ...
            GST_DEBUG("pointer at %d, write to %d-%d, diff %d, segtotal %d, segsize %d, base %d", segdone, writeseg, sampleoff, diff, segtotal, segsize, buf->segbase);
        ...
        *sample += avail / bps;
        ...
        GST_DEBUG_OBJECT(buf, "write @%p seg %d, sps %d, off %d, avail %d", dest + ws * segsize, ws, sps, sampleoff, avail);
...
guint gst_ring_buffer_read(GstRingBuffer * buf, guint64 sample, guchar * data, guint len)
{
    ...
    while (to_read > 0)
    {
        ...
        while (TRUE)
        {
            ...
            GST_DEBUG("pointer at %d, sample %" G_GUINT64_FORMAT ", read from %d-%d, to_read %d, diff %d, segtotal %d, segsize %d", segdone, sample, readseg, sampleoff, to_read, diff, segtotal, segsize);
        ...
        GST_DEBUG_OBJECT(buf, "read @%p seg %d, off %d, sampleslen %d", dest + readseg * segsize, readseg, sampleoff, sampleslen);

        memcpy(data, dest + (readseg * segsize) + (sampleoff * bps), (sampleslen * bps));
...
gboolean gst_ring_buffer_prepare_read(GstRingBuffer * buf, gint * segment, guint8 ** readptr, gint * len)
{
    ...
    *readptr = data + *segment * *len;
    ...
    GST_LOG("prepare read from segment %d (real %d) @%p", *segment, segdone, *readptr);
