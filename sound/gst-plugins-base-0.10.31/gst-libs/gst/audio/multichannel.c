...
gboolean gst_audio_check_channel_positions(const GstAudioChannelPosition * pos, guint channels)
{
    ...
    {
        if (pos[n] <= GST_AUDIO_CHANNEL_POSITION_INVALID || pos[n] >= GST_AUDIO_CHANNEL_POSITION_NUM)
        {
    ...
    if (pos[0] == GST_AUDIO_CHANNEL_POSITION_NONE)
    {
        for (n = 1; n < channels; ++n)
        {
            ...
            {
                GST_WARNING("Either all channel positions must be defined, or all " "be set to NONE, having only some defined is not allowed");
                return FALSE;
            }
        }
    ...
    for (i = GST_AUDIO_CHANNEL_POSITION_INVALID + 1; i < GST_AUDIO_CHANNEL_POSITION_NUM; i++)
    {
        gint count = 0;
        ...
        if (count > 1)
        {
            GST_WARNING("Channel position %d occurred %d times, not allowed", i, count);
            return FALSE;
        }
    }
    ...
    for (i = 0; conf[i].pos1[0] != GST_AUDIO_CHANNEL_POSITION_INVALID; i++)
    {
        ...
        if (found1 && found2)
        {
            GST_WARNING("Found conflicting channel positions %d/%d and %d", conf[i].pos1[0], conf[i].pos1[1], conf[i].pos2[0]);
            return FALSE;
        }
    }
...
static const GstAudioChannelPosition default_positions[NUM_DEF_CHANS][NUM_DEF_CHANS] = {
    /* 1 channel */
    {
     GST_AUDIO_CHANNEL_POSITION_FRONT_MONO,
     },
...
GstAudioChannelPosition *gst_audio_get_channel_positions(GstStructure * str)
{
    ...
    pos_val_arr = gst_structure_get_value(str, GST_AUDIO_CHANNEL_POSITIONS_FIELD_NAME);
    ...
    if (pos_val_arr == NULL)
    {
        ...
        if (channels > 2)
        {
            g_warning("Failed to retrieve channel layout from caps. This usually " "means there is a GStreamer element that does not implement " "multichannel audio correctly. Please file a bug.");
        }
    ...
    g_return_val_if_fail(gst_value_array_get_size(pos_val_arr) == channels, NULL);
    for (n = 0; n < channels; n++)
    {
...
void gst_audio_set_channel_positions(GstStructure * str, const GstAudioChannelPosition * pos)
{
    ...
    /* add to structure */
    gst_structure_set_value(str, GST_AUDIO_CHANNEL_POSITIONS_FIELD_NAME, &pos_val_arr);
...
void gst_audio_set_structure_channel_positions_list(GstStructure * str, const GstAudioChannelPosition * pos, gint num_positions)
{
    gint channels, n, c;
...
static GstCaps *add_list_to_struct(GstStructure * str, const GstAudioChannelPosition * pos, gint num_positions)
{
    ...
    {
        g_warning("Unexpected value type '%s' for channels field", GST_STR_NULL(g_type_name(G_VALUE_TYPE(chan_val))));
    }
...
void gst_audio_set_caps_channel_positions_list(GstCaps * caps, const GstAudioChannelPosition * pos, gint num_positions)
{
    gint size, n;
    ...
    for (n = 0; n < size; n++)
    {
        gst_caps_append(caps, add_list_to_struct(gst_caps_get_structure(caps, n), pos, num_positions));
    }
}
...
GstAudioChannelPosition *gst_audio_fixate_channel_positions(GstStructure * str)
{
    GstAudioChannelPosition *pos;
    ...
    {
        t = G_VALUE_TYPE(gst_value_array_get_value(pos_val_arr, n));
        g_return_val_if_fail(t == GST_TYPE_LIST || t == GST_TYPE_AUDIO_CHANNEL_POSITION, NULL);
    }
    ...
    for (i = 0; conf[i].pos1[0] != GST_AUDIO_CHANNEL_POSITION_INVALID || conf[i].pos2[0] != GST_AUDIO_CHANNEL_POSITION_INVALID; i++)
    {
        /* front/center only important if not mono (obviously) */
        if (conf[i].pos1[0] == GST_AUDIO_CHANNEL_POSITION_FRONT_LEFT_OF_CENTER && !is_stereo)
            continue;
        ...
        for (n = 0; n < channels; n++)
        {
            ...
            if (t == GST_TYPE_LIST)
            {
                ...
                for (c = 0; c < gst_value_list_get_size(pos_val_entry); c++)
                {
                    ...
                    if (g_value_get_enum(pos_val) == conf[i].pos1[0] && opt.num_opts[0] > gst_value_list_get_size(pos_val_entry) && !opt.is_fixed[0])
                    {
                        ...
                        if (opt.num_opt[0] != -1)
                        {
                            ...
                            pos_val_entry = gst_value_array_get_value(pos_val_arr, opt.num_opt[0]);
                            if (G_VALUE_TYPE(pos_val_entry) == GST_TYPE_LIST)
                            {
                                for (c1 = 0; c1 < gst_value_list_get_size(pos_val_entry); c1++)
                                {
                                    ...
                                    if (g_value_get_enum(pos_val) == conf[i].pos1[1] && opt.num_opts[1] > opt.num_opts[0] && !opt.is_fixed[1])
                                    {
                    ...
                    else if (g_value_get_enum(pos_val) == conf[i].pos1[1] && opt.num_opts[1] > gst_value_list_get_size(pos_val_entry) && !opt.is_fixed[1] && n != opt.num_opt[0])
                    {
                    ...
                    if (g_value_get_enum(pos_val) == conf[i].pos2[0] && opt.num_opts[2] > gst_value_list_get_size(pos_val_entry) && !opt.is_fixed[2])
                    {
        ...
        {
            g_warning("Pre-fixated on both %d/%d and %d - conflict!", conf[i].pos1[0], conf[i].pos1[1], conf[i].pos2[0]);
            g_free(pos);
            return NULL;
        }
        else if ((opt.is_fixed[0] && opt.num_opt[1] == -1) || (opt.is_fixed[1] && opt.num_opt[0] == -1))
        {
            g_warning("Pre-fixated one side, but other side n/a of %d/%d", conf[i].pos1[0], conf[i].pos1[1]);
            g_free(pos);
            return NULL;
        }
        ...
        if (conf[i].pos2[0] == GST_AUDIO_CHANNEL_POSITION_FRONT_MONO && opt.choice == 1)
        {
            is_stereo = FALSE;
        }
        ...
        if (opt.choice == 0)
        {
            g_assert(conf[i].pos1[0] != GST_AUDIO_CHANNEL_POSITION_INVALID && conf[i].pos1[1] != GST_AUDIO_CHANNEL_POSITION_INVALID);
    ...
    if (num_unfixed > 0)
    {
        g_warning("%d unfixed channel positions left after fixation!", num_unfixed);
        g_free(pos);
        return NULL;
    }
