...
static void gst_audio_mixer_filter_do_filter(GstAudioMixerFilterFunc filter_func, GstElementFactory * factory, GstElement ** p_element, GList ** p_collection, gpointer user_data)
{
...
static gboolean gst_audio_mixer_filter_check_element(GstElement * element)
{
...
static void gst_audio_mixer_filter_probe_feature(GstAudioMixerFilterFunc filter_func, GstElementFactory * factory, GList ** p_collection, gboolean first, gpointer user_data)
{
    GstElement *element;
        ...
        if ((devspec = gst_property_probe_get_property(probe, "device")))
        {
            ...
            if ((array = gst_property_probe_probe_and_get_values(probe, devspec)))
            {
                ...
                for (n = 0; n < array->n_values; n++)
                {
                    ...
                    {
                        gst_audio_mixer_filter_do_filter(filter_func, factory, &element, p_collection, user_data);
...
static gint element_factory_rank_compare_func(gconstpointer a, gconstpointer b)
{
 ...
 * Since: 0.10.2
 */
GList *gst_audio_default_registry_mixer_filter(GstAudioMixerFilterFunc filter_func, gboolean first, gpointer data)
{
    ...
    for (walk = feature_list; walk != NULL; walk = walk->next)
    {
        ...
        {
            gst_audio_mixer_filter_probe_feature(filter_func, factory, &mixer_list, first, data);
        }
