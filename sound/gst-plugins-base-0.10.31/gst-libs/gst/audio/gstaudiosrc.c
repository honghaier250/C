...
static GType gst_audioringbuffer_get_type(void)
{
    ...
    if (!ringbuffer_type)
    {
        ...
        ringbuffer_type = g_type_register_static(GST_TYPE_RING_BUFFER, "GstAudioSrcRingBuffer", &ringbuffer_info, 0);
    }
...
static void audioringbuffer_thread_func(GstRingBuffer * buf)
{
    ...
    message = gst_message_new_stream_status(GST_OBJECT_CAST(buf), GST_STREAM_STATUS_TYPE_ENTER, GST_ELEMENT_CAST(src));
    ...
    while (TRUE)
    {
        ...
        if (gst_ring_buffer_prepare_read(buf, &readseg, &readptr, &len))
        {
            ...
            do
            {
                ...
                GST_LOG_OBJECT(src, "transfered %d bytes of %d to segment %d", read, left, readseg);
                if (read < 0 || read > left)
                {
                    GST_WARNING_OBJECT(src, "error reading data %d (reason: %s), skipping segment", read, g_strerror(errno));
                    break;
                }
                left -= read;
                readptr += read;
...
stop_running:
    {
        ...
        message = gst_message_new_stream_status(GST_OBJECT_CAST(buf), GST_STREAM_STATUS_TYPE_LEAVE, GST_ELEMENT_CAST(src));
        gst_message_set_stream_status_object(message, &val);
...
static gboolean gst_audioringbuffer_acquire(GstRingBuffer * buf, GstRingBufferSpec * spec)
{
    ...
    src->thread = g_thread_create((GThreadFunc) audioringbuffer_thread_func, buf, TRUE, NULL);
    GST_AUDIORING_BUFFER_WAIT(buf);
...
GST_BOILERPLATE_FULL(GstAudioSrc, gst_audio_src, GstBaseAudioSrc, GST_TYPE_BASE_AUDIO_SRC, _do_init);

static GstRingBuffer *gst_audio_src_create_ringbuffer(GstBaseAudioSrc * src);

static void gst_audio_src_base_init(gpointer g_class)
{
}

static void gst_audio_src_class_init(GstAudioSrcClass * klass)
{
    GstBaseAudioSrcClass *gstbaseaudiosrc_class;

    gstbaseaudiosrc_class = (GstBaseAudioSrcClass *) klass;

    gstbaseaudiosrc_class->create_ringbuffer = GST_DEBUG_FUNCPTR(gst_audio_src_create_ringbuffer);

    g_type_class_ref(GST_TYPE_AUDIORING_BUFFER);
}

static void gst_audio_src_init(GstAudioSrc * audiosrc, GstAudioSrcClass * g_class)
{
}
