...
GstFlowReturn gst_riff_read_chunk(GstElement * element, GstPad * pad, guint64 * _offset, guint32 * tag, GstBuffer ** _chunk_data)
{
    GstBuffer *buf;
    GstFlowReturn res;
    guint size;
...
skip_junk:
    ...
    *tag = GST_READ_UINT32_LE(GST_BUFFER_DATA(buf));
    ...
    GST_DEBUG_OBJECT(element, "fourcc=%" GST_FOURCC_FORMAT ", size=%u", GST_FOURCC_ARGS(*tag), size);
...
too_small:
    {
        ...
        GST_DEBUG_OBJECT(element, "not enough data (available=%u, needed=%u)", GST_BUFFER_SIZE(buf), size);
        gst_buffer_unref(buf);
 ...
 */
gboolean gst_riff_parse_chunk(GstElement * element, GstBuffer * buf, guint * _offset, guint32 * _fourcc, GstBuffer ** chunk_data)
{
    guint size, bufsize;
    guint32 fourcc;
    guint8 *data;
    ...
    *_fourcc = 0;
    ...
    GST_DEBUG_OBJECT(element, "fourcc=%" GST_FOURCC_FORMAT ", size=%u", GST_FOURCC_ARGS(fourcc), size);
    ...
    {
        GST_DEBUG_OBJECT(element, "Needed chunk data (%d) is more than available (%d), shortcutting", size, bufsize - 8 - offset);
...
too_small:
    {
        GST_DEBUG_OBJECT(element, "Failed to parse chunk header (offset %d, %d available, %d needed)", offset, bufsize, 8);
        return FALSE;
    }
bogus_size:
    {
...
too_small:
    {
        GST_ELEMENT_ERROR(element, STREAM, WRONG_TYPE, (NULL), ("Not enough data to parse RIFF header (%d available, %d needed)", GST_BUFFER_SIZE(buf), 12));
        gst_buffer_unref(buf);
        return FALSE;
    }
not_riff:
    {
        GST_ELEMENT_ERROR(element, STREAM, WRONG_TYPE, (NULL), ("Stream is no RIFF stream: %" GST_FOURCC_FORMAT, GST_FOURCC_ARGS(tag)));
        gst_buffer_unref(buf);
        return FALSE;
    }
}
...
gboolean gst_riff_parse_strh(GstElement * element, GstBuffer * buf, gst_riff_strh ** _strh)
{
    gst_riff_strh *strh;
    ...
    GST_INFO_OBJECT(element, " type        %" GST_FOURCC_FORMAT, GST_FOURCC_ARGS(strh->type));
    GST_INFO_OBJECT(element, " fcc_handler %" GST_FOURCC_FORMAT, GST_FOURCC_ARGS(strh->fcc_handler));
...
too_small:
    {
        GST_ERROR_OBJECT(element, "Too small strh (%d available, %d needed)", GST_BUFFER_SIZE(buf), (int) sizeof(gst_riff_strh));
        gst_buffer_unref(buf);
        return FALSE;
    }
}
...
gboolean gst_riff_parse_strf_vids(GstElement * element, GstBuffer * buf, gst_riff_strf_vids ** _strf, GstBuffer ** data)
{
    ...
    *data = NULL;
    ...
    {
        GST_WARNING_OBJECT(element, "strf_vids header gave %d bytes data, only %d available", strf->size, GST_BUFFER_SIZE(buf));
        strf->size = GST_BUFFER_SIZE(buf);
    }
    if (sizeof(gst_riff_strf_vids) < GST_BUFFER_SIZE(buf))
    {
        *data = gst_buffer_create_sub(buf, sizeof(gst_riff_strf_vids), GST_BUFFER_SIZE(buf) - sizeof(gst_riff_strf_vids));
    }
    ...
    GST_INFO_OBJECT(element, " compression %" GST_FOURCC_FORMAT, GST_FOURCC_ARGS(strf->compression));
...
too_small:
    {
        GST_ERROR_OBJECT(element, "Too small strf_vids (%d available, %d needed)", GST_BUFFER_SIZE(buf), (int) sizeof(gst_riff_strf_vids));
        gst_buffer_unref(buf);
        return FALSE;
    }
}
 ...
 *          should be skipped on error, but it is not fatal.
 */
gboolean gst_riff_parse_strf_auds(GstElement * element, GstBuffer * buf, gst_riff_strf_auds ** _strf, GstBuffer ** data)
{
    ...
    *data = NULL;
    if (bufsize > sizeof(gst_riff_strf_auds) + 2)
    {
        ...
        {
            GST_WARNING_OBJECT(element, "Extradata indicated %d bytes, but only %" G_GSSIZE_FORMAT " available", len, bufsize - 2 - sizeof(gst_riff_strf_auds));
...
too_small:
    {
        GST_ERROR_OBJECT(element, "Too small strf_auds (%d available, %" G_GSSIZE_FORMAT " needed)", bufsize, sizeof(gst_riff_strf_auds));
        gst_buffer_unref(buf);
        return FALSE;
    }
}
...
gboolean gst_riff_parse_strf_iavs(GstElement * element, GstBuffer * buf, gst_riff_strf_iavs ** _strf, GstBuffer ** data)
{
...
too_small:
    {
        GST_ERROR_OBJECT(element, "Too small strf_iavs (%d available, %" G_GSSIZE_FORMAT " needed)", GST_BUFFER_SIZE(buf), sizeof(gst_riff_strf_iavs));
        gst_buffer_unref(buf);
        return FALSE;
    }
}
 ...
 */
void gst_riff_parse_info(GstElement * element, GstBuffer * buf, GstTagList ** _taglist)
{
    guint8 *data;
    guint size, tsize;
    guint32 tag;
    ...
    while (size > 8)
    {
        ...
        GST_DEBUG("tag %" GST_FOURCC_FORMAT ", size %u", GST_FOURCC_ARGS(tag), tsize);

        if (tsize > size)
        {
            GST_WARNING_OBJECT(element, "Tagsize %d is larger than available data %d", tsize, size);
            tsize = size;
        }
        ...
        switch (tag)
        {
            ...
            default:
                type = NULL;
                GST_WARNING_OBJECT(element, "Unknown INFO (metadata) tag entry %" GST_FOURCC_FORMAT, GST_FOURCC_ARGS(tag));
                break;
        }
