...
GType gst_app_buffer_get_type(void)
{
    ...
    if (G_UNLIKELY(_gst_app_buffer_type == 0))
    {
        ...
        _gst_app_buffer_type = g_type_register_static(GST_TYPE_BUFFER, "GstAppBuffer", &app_buffer_info, 0);
    }
...
static void gst_app_buffer_class_init(gpointer g_class, gpointer class_data)
{
    ...
    mini_object_class->finalize = (GstMiniObjectFinalizeFunction) gst_app_buffer_finalize;
...
GstBuffer *gst_app_buffer_new(void *data, int length, GstAppBufferFinalizeFunc finalize, void *priv)
{
    GstAppBuffer *buffer;
