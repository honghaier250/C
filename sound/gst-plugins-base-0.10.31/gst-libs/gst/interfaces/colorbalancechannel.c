...
GType gst_color_balance_channel_get_type(void)
{
    ...
    if (!gst_color_balance_channel_type)
    {
        ...
        gst_color_balance_channel_type = g_type_register_static(G_TYPE_OBJECT, "GstColorBalanceChannel", &color_balance_channel_info, 0);
    }
...
static void gst_color_balance_channel_class_init(GstColorBalanceChannelClass * klass)
{
   ...
   */
    signals[SIGNAL_VALUE_CHANGED] = g_signal_new("value-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstColorBalanceChannelClass, value_changed), NULL, NULL, g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);

    object_klass->dispose = gst_color_balance_channel_dispose;
}

static void gst_color_balance_channel_init(GstColorBalanceChannel * channel)
{
    channel->label = NULL;
