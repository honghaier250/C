...
GType gst_color_balance_get_type(void)
{
    ...
    if (!gst_color_balance_type)
    {
        ...
        gst_color_balance_type = g_type_register_static(G_TYPE_INTERFACE, "GstColorBalance", &gst_color_balance_info, 0);
        g_type_interface_add_prerequisite(gst_color_balance_type, GST_TYPE_IMPLEMENTS_INTERFACE);
    }
     ...
     */
        gst_color_balance_signals[VALUE_CHANGED] =
            g_signal_new("value-changed",
                         GST_TYPE_COLOR_BALANCE, G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstColorBalanceClass, value_changed), NULL, NULL, gst_interfaces_marshal_VOID__OBJECT_INT, G_TYPE_NONE, 2, GST_TYPE_COLOR_BALANCE_CHANNEL, G_TYPE_INT);
...
{
    g_signal_emit(G_OBJECT(balance), gst_color_balance_signals[VALUE_CHANGED], 0, channel, value);
