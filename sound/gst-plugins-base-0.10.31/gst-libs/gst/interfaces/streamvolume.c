...
static void gst_stream_volume_class_init(GstStreamVolumeInterface * iface)
{
    g_object_interface_install_property(iface, g_param_spec_double("volume", "Volume", "Linear volume factor, 1.0=100%", 0.0, G_MAXDOUBLE, 1.0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_interface_install_property(iface, g_param_spec_boolean("mute", "Mute", "Mute the audio channel without changing the volume", FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
GType gst_stream_volume_get_type(void)
{
    ...
    if (g_once_init_enter(&type))
    {
        ...
        tmp = g_type_register_static(G_TYPE_INTERFACE, "GstStreamVolume", &info, 0);
...
gdouble gst_stream_volume_get_volume(GstStreamVolume * volume, GstStreamVolumeFormat format)
{
    ...
    if (format != GST_STREAM_VOLUME_FORMAT_LINEAR)
        val = gst_stream_volume_convert_volume(GST_STREAM_VOLUME_FORMAT_LINEAR, format, val);
    return val;
}
 ...
 * Since: 0.10.25
 */
void gst_stream_volume_set_volume(GstStreamVolume * volume, GstStreamVolumeFormat format, gdouble val)
{
    g_return_if_fail(GST_IS_STREAM_VOLUME(volume));

    if (format != GST_STREAM_VOLUME_FORMAT_LINEAR)
        val = gst_stream_volume_convert_volume(format, GST_STREAM_VOLUME_FORMAT_LINEAR, val);
 ...
 * Since: 0.10.25
 */
void gst_stream_volume_set_mute(GstStreamVolume * volume, gboolean mute)
{
    g_return_if_fail(GST_IS_STREAM_VOLUME(volume));
