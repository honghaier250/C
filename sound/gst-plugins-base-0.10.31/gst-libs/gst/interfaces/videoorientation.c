...
static void gst_video_orientation_iface_init(GstVideoOrientationInterface * iface);
...
GType gst_video_orientation_get_type(void)
{
    ...
    if (!gst_video_orientation_type)
    {
        ...
        gst_video_orientation_type = g_type_register_static(G_TYPE_INTERFACE, "GstVideoOrientation", &gst_video_orientation_info, 0);
        g_type_interface_add_prerequisite(gst_video_orientation_type, GST_TYPE_IMPLEMENTS_INTERFACE);
    }
...
{
    GstVideoOrientationInterface *iface = GST_VIDEO_ORIENTATION_GET_IFACE(video_orientation);
