...
GType gst_navigation_get_type(void)
{
    ...
    if (!gst_navigation_type)
    {
        ...
        gst_navigation_type = g_type_register_static(G_TYPE_INTERFACE, "GstNavigation", &gst_navigation_info, 0);
    }
...
{
    gst_navigation_send_event(navigation, gst_structure_new(GST_NAVIGATION_EVENT_NAME, "event", G_TYPE_STRING, event, "key", G_TYPE_STRING, key, NULL));
}
...
{
    gst_navigation_send_event(navigation, gst_structure_new(GST_NAVIGATION_EVENT_NAME, "event", G_TYPE_STRING, event, "button", G_TYPE_INT, button, "pointer_x", G_TYPE_DOUBLE, x, "pointer_y", G_TYPE_DOUBLE, y, NULL));
}
...
{
    gst_navigation_send_event(navigation, gst_structure_new(GST_NAVIGATION_EVENT_NAME, "event", G_TYPE_STRING, "command", "command-code", G_TYPE_UINT, (guint) command, NULL));
}
...
GstQuery *gst_navigation_query_new_commands(void)
{
    ...
    structure = gst_structure_new(GST_NAVIGATION_QUERY_NAME, "type", G_TYPE_STRING, "commands", NULL);
    query = gst_query_new_application(GST_QUERY_CUSTOM, structure);

    return query;
}

static void gst_query_list_add_command(GValue * list, GstNavigationCommand val)
{
    GValue item = { 0, };
...
gboolean gst_navigation_query_parse_commands_nth(GstQuery * query, guint nth, GstNavigationCommand * cmd)
{
    ...
    else
    {
        ...
        {
            *cmd = (GstNavigationCommand) g_value_get_enum(gst_value_list_get_value(list, nth));
        }
        else
            *cmd = GST_NAVIGATION_COMMAND_INVALID;
    }
...
GstQuery *gst_navigation_query_new_angles(void)
{
    ...
    structure = gst_structure_new(GST_NAVIGATION_QUERY_NAME, "type", G_TYPE_STRING, "angles", NULL);
    query = gst_query_new_application(GST_QUERY_CUSTOM, structure);
...
void gst_navigation_query_set_angles(GstQuery * query, guint cur_angle, guint n_angles)
{
    ...
    structure = gst_query_get_structure(query);
    gst_structure_set(structure, "angle", G_TYPE_UINT, cur_angle, "angles", G_TYPE_UINT, n_angles, NULL);
}
...
GstMessage *gst_navigation_message_new_mouse_over(GstObject * src, gboolean active)
{
    ...
    s = gst_structure_new(GST_NAVIGATION_MESSAGE_NAME, "type", G_TYPE_STRING, "mouse-over", "active", G_TYPE_BOOLEAN, active, NULL);
...
GstMessage *gst_navigation_message_new_commands_changed(GstObject * src)
{
    ...
    s = gst_structure_new(GST_NAVIGATION_MESSAGE_NAME, "type", G_TYPE_STRING, "commands-changed", NULL);
...
GstMessage *gst_navigation_message_new_angles_changed(GstObject * src, guint cur_angle, guint n_angles)
{
    ...
    s = gst_structure_new(GST_NAVIGATION_MESSAGE_NAME, "type", G_TYPE_STRING, "angles-changed", "angle", G_TYPE_UINT, cur_angle, "angles", G_TYPE_UINT, n_angles, NULL);
...
gboolean gst_navigation_message_parse_angles_changed(GstMessage * message, guint * cur_angle, guint * n_angles)
{
    ...
    g_return_val_if_fail(GST_NAVIGATION_MESSAGE_HAS_TYPE(message, ANGLES_CHANGED), FALSE);
...
gboolean gst_navigation_event_parse_key_event(GstEvent * event, const gchar ** key)
{
    ...
    g_return_val_if_fail(e_type == GST_NAVIGATION_EVENT_KEY_PRESS || e_type == GST_NAVIGATION_EVENT_KEY_RELEASE, FALSE);
...
gboolean gst_navigation_event_parse_mouse_button_event(GstEvent * event, gint * button, gdouble * x, gdouble * y)
{
    ...
    e_type = gst_navigation_event_get_type(event);
    g_return_val_if_fail(e_type == GST_NAVIGATION_EVENT_MOUSE_BUTTON_PRESS || e_type == GST_NAVIGATION_EVENT_MOUSE_BUTTON_RELEASE, FALSE);
...
gboolean gst_navigation_event_parse_mouse_move_event(GstEvent * event, gdouble * x, gdouble * y)
{
    ...
    g_return_val_if_fail(GST_NAVIGATION_EVENT_HAS_TYPE(event, MOUSE_MOVE), FALSE);
