...
GType gst_tuner_get_type(void)
{
    ...
    if (!gst_tuner_type)
    {
        ...
        gst_tuner_type = g_type_register_static(G_TYPE_INTERFACE, "GstTuner", &gst_tuner_info, 0);
        g_type_interface_add_prerequisite(gst_tuner_type, GST_TYPE_IMPLEMENTS_INTERFACE);
    }
...
static void gst_tuner_class_init(GstTunerClass * klass)
{
     ...
     */
        gst_tuner_signals[NORM_CHANGED] = g_signal_new("norm-changed", GST_TYPE_TUNER, G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstTunerClass, norm_changed), NULL, NULL, g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, GST_TYPE_TUNER_NORM);
    /**
     ...
     */
        gst_tuner_signals[CHANNEL_CHANGED] = g_signal_new("channel-changed", GST_TYPE_TUNER, G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstTunerClass, channel_changed), NULL, NULL, g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, GST_TYPE_TUNER_CHANNEL);
    /**
     ...
     */
        gst_tuner_signals[FREQUENCY_CHANGED] =
            g_signal_new("frequency-changed", GST_TYPE_TUNER, G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstTunerClass, frequency_changed), NULL, NULL, gst_interfaces_marshal_VOID__OBJECT_ULONG, G_TYPE_NONE, 2, GST_TYPE_TUNER_CHANNEL, G_TYPE_ULONG);
    /**
     ...
     */
        gst_tuner_signals[SIGNAL_CHANGED] =
            g_signal_new("signal-changed", GST_TYPE_TUNER, G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstTunerClass, signal_changed), NULL, NULL, gst_interfaces_marshal_VOID__OBJECT_INT, G_TYPE_NONE, 2, GST_TYPE_TUNER_CHANNEL, G_TYPE_INT);
...
void gst_tuner_set_frequency(GstTuner * tuner, GstTunerChannel * channel, gulong frequency)
{
    ...
    g_return_if_fail(GST_TUNER_CHANNEL_HAS_FLAG(channel, GST_TUNER_CHANNEL_FREQUENCY));
...
gulong gst_tuner_get_frequency(GstTuner * tuner, GstTunerChannel * channel)
{
    ...
    g_return_val_if_fail(GST_TUNER_CHANNEL_HAS_FLAG(channel, GST_TUNER_CHANNEL_FREQUENCY), 0);
...
void gst_tuner_channel_changed(GstTuner * tuner, GstTunerChannel * channel)
{
    ...
    g_signal_emit(G_OBJECT(tuner), gst_tuner_signals[CHANNEL_CHANGED], 0, channel);
}
...
void gst_tuner_frequency_changed(GstTuner * tuner, GstTunerChannel * channel, gulong frequency)
{
    ...
    g_signal_emit(G_OBJECT(tuner), gst_tuner_signals[FREQUENCY_CHANGED], 0, channel, frequency);
...
void gst_tuner_signal_changed(GstTuner * tuner, GstTunerChannel * channel, gint signal)
{
    ...
    g_signal_emit(G_OBJECT(tuner), gst_tuner_signals[SIGNAL_CHANGED], 0, channel, signal);
