...
GType gst_x_overlay_get_type(void)
{
    ...
    if (!gst_x_overlay_type)
    {
        ...
        gst_x_overlay_type = g_type_register_static(G_TYPE_INTERFACE, "GstXOverlay", &gst_x_overlay_info, 0);
        g_type_interface_add_prerequisite(gst_x_overlay_type, GST_TYPE_IMPLEMENTS_INTERFACE);
    }
...
void gst_x_overlay_set_xwindow_id(GstXOverlay * overlay, gulong xwindow_id)
{
    GST_WARNING_OBJECT(overlay, "Using deprecated gst_x_overlay_set_xwindow_id()");
...
#    endif
        ...
        {
            GST_WARNING_OBJECT(overlay, "Calling deprecated set_xwindow_id() method");
...
void gst_x_overlay_got_xwindow_id(GstXOverlay * overlay, gulong xwindow_id)
{
    GST_WARNING_OBJECT(overlay, "Using deprecated gst_x_overlay_got_xwindow_id()");
...
void gst_x_overlay_got_window_handle(GstXOverlay * overlay, guintptr handle)
{
    ...
    GST_LOG_OBJECT(GST_OBJECT(overlay), "xwindow_id = %p", (gpointer) handle);
    s = gst_structure_new("have-xwindow-id", "xwindow-id", G_TYPE_ULONG, (unsigned long) handle, "window-handle", G_TYPE_UINT64, (guint64) handle, NULL);
...
gboolean gst_x_overlay_set_render_rectangle(GstXOverlay * overlay, gint x, gint y, gint width, gint height)
{
    ...
    g_return_val_if_fail((width == -1 && height == -1) || (width > 0 && height > 0), FALSE);
