...
static void gst_rtsp_extension_iface_init(GstRTSPExtension * iface)
{
    ...
    if (!initialized)
    {
        gst_rtsp_extension_signals[SIGNAL_SEND] =
            g_signal_new("send", G_TYPE_FROM_CLASS(iface), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstRTSPExtensionInterface, send), NULL, NULL, __gst_rtsp_marshal_ENUM__POINTER_POINTER, GST_TYPE_RTSP_RESULT, 2, G_TYPE_POINTER, G_TYPE_POINTER);
        initialized = TRUE;
    }
}
...
GstRTSPResult gst_rtsp_extension_receive_request(GstRTSPExtension * ext, GstRTSPMessage * msg)
{
    GstRTSPExtensionInterface *iface;
...
GstRTSPResult gst_rtsp_extension_send(GstRTSPExtension * ext, GstRTSPMessage * req, GstRTSPMessage * resp)
{
    ...
    g_signal_emit(ext, gst_rtsp_extension_signals[SIGNAL_SEND], 0, req, resp, &res);
