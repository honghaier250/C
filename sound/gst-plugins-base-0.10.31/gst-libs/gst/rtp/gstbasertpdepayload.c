...
static void gst_base_rtp_depayload_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_base_rtp_depayload_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

static gboolean gst_base_rtp_depayload_setcaps(GstPad * pad, GstCaps * caps);
static GstFlowReturn gst_base_rtp_depayload_chain(GstPad * pad, GstBuffer * in);
static gboolean gst_base_rtp_depayload_handle_sink_event(GstPad * pad, GstEvent * event);

static GstStateChangeReturn gst_base_rtp_depayload_change_state(GstElement * element, GstStateChange transition);

static void gst_base_rtp_depayload_set_gst_timestamp(GstBaseRTPDepayload * filter, guint32 rtptime, GstBuffer * buf);
static gboolean gst_base_rtp_depayload_packet_lost(GstBaseRTPDepayload * filter, GstEvent * event);

GST_BOILERPLATE(GstBaseRTPDepayload, gst_base_rtp_depayload, GstElement, GST_TYPE_ELEMENT);

static void gst_base_rtp_depayload_base_init(gpointer klass)
{
    /*GstElementClass *element_class = GST_ELEMENT_CLASS (klass); */
}

static void gst_base_rtp_depayload_class_init(GstBaseRTPDepayloadClass * klass)
{
   ...
   */
    ...
    g_object_class_install_property(gobject_class, PROP_QUEUE_DELAY, g_param_spec_uint("queue-delay", "Queue Delay", "Amount of ms to queue/buffer, deprecated", 0, G_MAXUINT, DEFAULT_QUEUE_DELAY, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
#endif
    ...
    GST_DEBUG_CATEGORY_INIT(basertpdepayload_debug, "basertpdepayload", 0, "Base class for RTP Depayloaders");
}

static void gst_base_rtp_depayload_init(GstBaseRTPDepayload * filter, GstBaseRTPDepayloadClass * klass)
{
    ...
    gst_pad_set_setcaps_function(filter->sinkpad, gst_base_rtp_depayload_setcaps);
    gst_pad_set_chain_function(filter->sinkpad, gst_base_rtp_depayload_chain);
    gst_pad_set_event_function(filter->sinkpad, gst_base_rtp_depayload_handle_sink_event);
...
static gboolean gst_base_rtp_depayload_setcaps(GstPad * pad, GstCaps * caps)
{
    ...
    if (bclass->set_caps)
    {
        ...
        if (!res)
        {
            GST_WARNING_OBJECT(filter, "Subclass rejected caps %" GST_PTR_FORMAT, caps);
        }
    }
    else
    {
...
static GstFlowReturn gst_base_rtp_depayload_chain(GstPad * pad, GstBuffer * in)
{
    ...
    timestamp = gst_segment_to_running_time(&filter->segment, GST_FORMAT_TIME, timestamp);
    ...
    GST_LOG_OBJECT(filter, "discont %d, seqnum %u, rtptime %u, timestamp %" GST_TIME_FORMAT, priv->discont, seqnum, rtptime, GST_TIME_ARGS(timestamp));
    ...
    if (G_LIKELY(priv->next_seqnum != -1))
    {
        ...
        if (G_UNLIKELY(gap != 0))
        {
            GST_LOG_OBJECT(filter, "got packet %u, expected %u, gap %d", seqnum, priv->next_seqnum, gap);
            if (gap < 0)
            {
            ...
            else
            {
                ...
                GST_LOG_OBJECT(filter, "%d > 100, packet too old, sender likely restarted", gap);
                discont = TRUE;
            }
        }
    }
        ...
        if (GST_BUFFER_CAPS(in) == NULL)
        {
            GST_ELEMENT_ERROR(filter, CORE, NEGOTIATION,
                              ...
                              ("Input buffers need to have RTP caps set on them. This is usually "
                               ...
                               "element before the depayloader and setting the 'caps' property " "on that. Also see http://cgit.freedesktop.org/gstreamer/" "gst-plugins-good/tree/gst/rtp/README"));
        }
        else
        {
            GST_ELEMENT_ERROR(filter, CORE, NEGOTIATION,
                              ("No RTP format was negotiated."), ("RTP caps on input buffer were rejected, most likely because they " "were incomplete or contained wrong values. Check the debug log " "for more information."));
        }
        gst_buffer_unref(in);
...
invalid_buffer:
    {
        /* this is not fatal but should be filtered earlier */
        GST_ELEMENT_WARNING(filter, STREAM, DECODE, (NULL), ("Received invalid RTP payload, dropping"));
        gst_buffer_unref(in);
        return GST_FLOW_OK;
    }
dropping:
...
no_process:
    {
        /* this is not fatal but should be filtered earlier */
        GST_ELEMENT_ERROR(filter, STREAM, NOT_IMPLEMENTED, (NULL), ("The subclass does not have a process method"));
        gst_buffer_unref(in);
        ...
        case GST_EVENT_NEWSEGMENT:
            {
                ...
                gst_segment_set_newsegment(&filter->segment, update, rate, fmt, start, stop, position);
...
static GstEvent *create_segment_event(GstBaseRTPDepayload * filter, gboolean update, GstClockTime position)
{
    GstEvent *event;
    GstClockTime stop;
    GstBaseRTPDepayloadPrivate *priv;
    ...
    event = gst_event_new_new_segment_full(update, priv->play_speed, priv->play_scale, GST_FORMAT_TIME, position, stop, position + priv->npt_start);

    return event;
}

static GstFlowReturn gst_base_rtp_depayload_push_full(GstBaseRTPDepayload * filter, gboolean do_ts, guint32 rtptime, GstBuffer * out_buf)
{
    GstFlowReturn ret;
    GstCaps *srccaps;
    GstBaseRTPDepayloadClass *bclass;
    ...
    /* push it */
    GST_LOG_OBJECT(filter, "Pushing buffer size %d, timestamp %" GST_TIME_FORMAT, GST_BUFFER_SIZE(out_buf), GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(out_buf)));
