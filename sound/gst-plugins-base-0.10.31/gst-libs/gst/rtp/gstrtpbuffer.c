...
void gst_rtp_buffer_allocate_data(GstBuffer * buffer, guint payload_len, guint8 pad_len, guint8 csrc_count)
{
    ...
    len = GST_RTP_HEADER_LEN + csrc_count * sizeof(guint32) + payload_len + pad_len;
    ...
    memset(GST_RTP_HEADER_CSRC_LIST_OFFSET(data, 0), 0, csrc_count * sizeof(guint32));
...
guint gst_rtp_buffer_calc_packet_len(guint payload_len, guint8 pad_len, guint8 csrc_count)
{
    ...
    return payload_len + GST_RTP_HEADER_LEN + (csrc_count * sizeof(guint32)) + pad_len;
}
...
guint gst_rtp_buffer_calc_payload_len(guint packet_len, guint8 pad_len, guint8 csrc_count)
{
    ...
    return packet_len - GST_RTP_HEADER_LEN - (csrc_count * sizeof(guint32)) - pad_len;
}
 ...
 */
static gboolean validate_data(guint8 * data, guint len, guint8 * payload, guint payload_len)
{
    guint8 padding;
    guint8 csrc_count;
    guint header_len;
...
gboolean gst_rtp_buffer_list_validate(GstBufferList * list)
{
    ...
    while (gst_buffer_list_iterator_next_group(it))
    {
        ...
        /* validate packet */
        if (!validate_data(packet_header, packet_size, packet_payload, payload_size))
        {
            goto invalid_list;
        }
    }
...
gboolean gst_rtp_buffer_set_extension_data(GstBuffer * buffer, guint16 bits, guint16 length)
{
    ...
    min_size = GST_RTP_HEADER_LEN + GST_RTP_HEADER_CSRC_SIZE(data) + 4 + length * sizeof(guint32);
...
too_small:
    {
        g_warning("rtp buffer too small: need more than %d bytes but only have %d bytes", min_size, GST_BUFFER_SIZE(buffer));
        return FALSE;
    }
}
...
static GstBufferListItem set_ssrc_header(GstBuffer ** buffer, guint group, guint idx, guint32 * ssrc)
{
    GST_RTP_HEADER_SSRC(GST_BUFFER_DATA(*buffer)) = g_htonl(*ssrc);
...
static GstBufferListItem set_pt_header(GstBuffer ** buffer, guint group, guint idx, guint8 * pt)
{
...
void gst_rtp_buffer_list_set_payload_type(GstBufferList * list, guint8 payload_type)
{
    ...
    gst_buffer_list_foreach(list, (GstBufferListFunc) set_pt_header, &payload_type);
}
...
static GstBufferListItem set_seq_header(GstBuffer ** buffer, guint group, guint idx, guint16 * seq)
{
    GST_RTP_HEADER_SEQ(GST_BUFFER_DATA(*buffer)) = g_htons(*seq);
    (*seq)++;
    return GST_BUFFER_LIST_SKIP_GROUP;
...
static GstBufferListItem set_timestamp_header(GstBuffer ** buffer, guint group, guint idx, guint32 * timestamp)
{
    GST_RTP_HEADER_TIMESTAMP(GST_BUFFER_DATA(*buffer)) = g_htonl(*timestamp);
    return GST_BUFFER_LIST_SKIP_GROUP;
}
...
{
    gst_buffer_list_foreach(list, (GstBufferListFunc) set_timestamp_header, &timestamp);
}
...
gboolean gst_rtp_buffer_get_extension_onebyte_header(GstBuffer * buffer, guint8 id, guint nth, gpointer * data, guint * size)
{
    ...
    if (!gst_rtp_buffer_get_extension_data(buffer, &bits, (gpointer) & pdata, &wordlen))
        return FALSE;
...
static guint get_onebyte_header_end_offset(guint8 * pdata, guint wordlen)
{
    guint offset = 0;
...
gboolean gst_rtp_buffer_add_extension_onebyte_header(GstBuffer * buffer, guint8 id, gpointer data, guint size)
{
    ...
    has_bit = gst_rtp_buffer_get_extension_data(buffer, &bits, (gpointer) & pdata, &wordlen);
    ...
    else
    {
        ...
        gst_rtp_buffer_get_extension_data(buffer, &bits, (gpointer) & pdata, &wordlen);
...
static guint get_twobytes_header_end_offset(guint8 * pdata, guint wordlen)
{
    guint offset = 0;
    guint bytelen = wordlen * 4;
    guint paddingcount = 0;
...
gboolean gst_rtp_buffer_add_extension_twobytes_header(GstBuffer * buffer, guint8 appbits, guint8 id, gpointer data, guint size)
{
    ...
    if (has_bit)
    {
        ...
        gst_rtp_buffer_set_extension_data(buffer, (0x100 << 4) | (appbits & 0x0F), wordlen);
    }
    else
    {
        wordlen = (size + 1) / 4 + (((size + 1) % 4) ? 1 : 0);
...
gboolean gst_rtp_buffer_list_get_extension_onebyte_header(GstBufferList * bufferlist, guint group_idx, guint8 id, guint nth, gpointer * data, guint * size)
{
    ...
    return gst_rtp_buffer_get_extension_onebyte_header(buffer, id, nth, data, size);
}
...
gboolean gst_rtp_buffer_list_get_extension_twobytes_header(GstBufferList * bufferlist, guint group_idx, guint8 * appbits, guint8 id, guint nth, gpointer * data, guint * size)
{
    ...
    return gst_rtp_buffer_get_extension_twobytes_header(buffer, appbits, id, nth, data, size);
}
...
gboolean gst_rtp_buffer_list_add_extension_onebyte_header(GstBufferListIterator * it, guint8 id, gpointer data, guint size)
{
    ...
    if (endoffset + size + 1 > GST_BUFFER_SIZE(buffer))
    {
        ...
        memcpy(GST_BUFFER_DATA(newbuffer), GST_BUFFER_DATA(buffer), GST_BUFFER_SIZE(buffer));
...
gboolean gst_rtp_buffer_list_add_extension_twobytes_header(GstBufferListIterator * it, guint8 appbits, guint8 id, gpointer data, guint size)
{
    ...
    retval = gst_rtp_buffer_add_extension_twobytes_header(buffer, appbits, id, data, size);
