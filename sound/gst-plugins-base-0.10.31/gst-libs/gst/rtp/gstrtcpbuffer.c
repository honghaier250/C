...
wrong_mask:
    {
        GST_DEBUG("mask check failed (%04x != %04x)", header_mask, GST_RTCP_VALID_VALUE);
        return FALSE;
    }
wrong_version:
    {
 ...
 */
static gboolean read_packet_header(GstRTCPPacket * packet)
{
    guint8 *data;
    guint size;
    guint offset;
...
gboolean gst_rtcp_packet_remove(GstRTCPPacket * packet)
{
    ...
    memmove(GST_BUFFER_DATA(packet->buffer) + packet->offset, GST_BUFFER_DATA(packet->buffer) + offset, GST_BUFFER_SIZE(packet->buffer) - offset);
...
{
    g_return_val_if_fail(packet != NULL, 0);
    g_return_val_if_fail(packet->type == GST_RTCP_TYPE_RR || packet->type == GST_RTCP_TYPE_SR, 0);
    g_return_val_if_fail(GST_IS_BUFFER(packet->buffer), 0);
...
void gst_rtcp_packet_get_rb(GstRTCPPacket * packet, guint nth, guint32 * ssrc, guint8 * fractionlost, gint32 * packetslost, guint32 * exthighestseq, guint32 * jitter, guint32 * lsr, guint32 * dlsr)
{
    ...
    g_return_if_fail(packet != NULL);
    g_return_if_fail(packet->type == GST_RTCP_TYPE_RR || packet->type == GST_RTCP_TYPE_SR);
    g_return_if_fail(GST_IS_BUFFER(packet->buffer));
...
gboolean gst_rtcp_packet_add_rb(GstRTCPPacket * packet, guint32 ssrc, guint8 fractionlost, gint32 packetslost, guint32 exthighestseq, guint32 jitter, guint32 lsr, guint32 dlsr)
{
    ...
    g_return_val_if_fail(packet != NULL, FALSE);
    g_return_val_if_fail(packet->type == GST_RTCP_TYPE_RR || packet->type == GST_RTCP_TYPE_SR, FALSE);
    g_return_val_if_fail(GST_IS_BUFFER(packet->buffer), FALSE);
...
static guint get_reason_offset(GstRTCPPacket * packet)
{
    guint offset;
...
guint32 gst_rtcp_packet_fb_get_sender_ssrc(GstRTCPPacket * packet)
{
    ...
    g_return_val_if_fail(packet != NULL, 0);
    g_return_val_if_fail((packet->type == GST_RTCP_TYPE_RTPFB || packet->type == GST_RTCP_TYPE_PSFB), 0);
    g_return_val_if_fail(GST_IS_BUFFER(packet->buffer), 0);
...
void gst_rtcp_packet_fb_set_sender_ssrc(GstRTCPPacket * packet, guint32 ssrc)
{
    ...
    g_return_if_fail(packet != NULL);
    g_return_if_fail(packet->type == GST_RTCP_TYPE_RTPFB || packet->type == GST_RTCP_TYPE_PSFB);
    g_return_if_fail(GST_IS_BUFFER(packet->buffer));
...
guint64 gst_rtcp_ntp_to_unix(guint64 ntptime)
{
    ...
    unixtime = gst_util_uint64_scale(unixtime, GST_SECOND, (G_GINT64_CONSTANT(1) << 32));
...
guint64 gst_rtcp_unix_to_ntp(guint64 unixtime)
{
    ...
    ntptime = gst_util_uint64_scale(unixtime, (G_GINT64_CONSTANT(1) << 32), GST_SECOND);
...
gboolean gst_rtcp_packet_fb_set_fci_length(GstRTCPPacket * packet, guint16 wordlen)
{
    ...
    g_return_val_if_fail(packet != NULL, FALSE);
    g_return_val_if_fail(packet->type == GST_RTCP_TYPE_RTPFB || packet->type == GST_RTCP_TYPE_PSFB, FALSE);
    g_return_val_if_fail(GST_IS_BUFFER(packet->buffer), FALSE);
...
guint8 *gst_rtcp_packet_fb_get_fci(GstRTCPPacket * packet)
{
    ...
    g_return_val_if_fail(packet != NULL, NULL);
    g_return_val_if_fail(packet->type == GST_RTCP_TYPE_RTPFB || packet->type == GST_RTCP_TYPE_PSFB, NULL);
    g_return_val_if_fail(GST_IS_BUFFER(packet->buffer), NULL);
