...
typedef GstClockTime(*GetBytesToTimeFunc) (GstBaseRTPAudioPayload * payload, guint64 bytes);
/* function to convert bytes to a RTP time */
typedef guint32(*GetBytesToRTPTimeFunc) (GstBaseRTPAudioPayload * payload, guint64 bytes);
/* function to convert time to bytes */
typedef guint64(*GetTimeToBytesFunc) (GstBaseRTPAudioPayload * payload, GstClockTime time);
...
static void gst_base_rtp_audio_payload_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_base_rtp_audio_payload_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

/* bytes to time functions */
static GstClockTime gst_base_rtp_audio_payload_frame_bytes_to_time(GstBaseRTPAudioPayload * payload, guint64 bytes);
static GstClockTime gst_base_rtp_audio_payload_sample_bytes_to_time(GstBaseRTPAudioPayload * payload, guint64 bytes);

/* bytes to RTP time functions */
static guint32 gst_base_rtp_audio_payload_frame_bytes_to_rtptime(GstBaseRTPAudioPayload * payload, guint64 bytes);
static guint32 gst_base_rtp_audio_payload_sample_bytes_to_rtptime(GstBaseRTPAudioPayload * payload, guint64 bytes);

/* time to bytes functions */
static guint64 gst_base_rtp_audio_payload_frame_time_to_bytes(GstBaseRTPAudioPayload * payload, GstClockTime time);
static guint64 gst_base_rtp_audio_payload_sample_time_to_bytes(GstBaseRTPAudioPayload * payload, GstClockTime time);

static GstFlowReturn gst_base_rtp_audio_payload_handle_buffer(GstBaseRTPPayload * payload, GstBuffer * buffer);

static GstStateChangeReturn gst_base_rtp_payload_audio_change_state(GstElement * element, GstStateChange transition);

static gboolean gst_base_rtp_payload_audio_handle_event(GstPad * pad, GstEvent * event);

GST_BOILERPLATE(GstBaseRTPAudioPayload, gst_base_rtp_audio_payload, GstBaseRTPPayload, GST_TYPE_BASE_RTP_PAYLOAD);

static void gst_base_rtp_audio_payload_base_init(gpointer klass)
{
}

static void gst_base_rtp_audio_payload_class_init(GstBaseRTPAudioPayloadClass * klass)
{
    ...
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_BUFFER_LIST, g_param_spec_boolean("buffer-list", "Buffer List", "Use Buffer Lists", DEFAULT_BUFFER_LIST, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    gstelement_class->change_state = GST_DEBUG_FUNCPTR(gst_base_rtp_payload_audio_change_state);

    gstbasertppayload_class->handle_buffer = GST_DEBUG_FUNCPTR(gst_base_rtp_audio_payload_handle_buffer);
    gstbasertppayload_class->handle_event = GST_DEBUG_FUNCPTR(gst_base_rtp_payload_audio_handle_event);

    GST_DEBUG_CATEGORY_INIT(basertpaudiopayload_debug, "basertpaudiopayload", 0, "base audio RTP payloader");
}

static void gst_base_rtp_audio_payload_init(GstBaseRTPAudioPayload * payload, GstBaseRTPAudioPayloadClass * klass)
{
...
void gst_base_rtp_audio_payload_set_frame_based(GstBaseRTPAudioPayload * basertpaudiopayload)
{
    ...
    basertpaudiopayload->priv->bytes_to_time = gst_base_rtp_audio_payload_frame_bytes_to_time;
    basertpaudiopayload->priv->bytes_to_rtptime = gst_base_rtp_audio_payload_frame_bytes_to_rtptime;
    basertpaudiopayload->priv->time_to_bytes = gst_base_rtp_audio_payload_frame_time_to_bytes;
}
...
void gst_base_rtp_audio_payload_set_sample_based(GstBaseRTPAudioPayload * basertpaudiopayload)
{
    ...
    basertpaudiopayload->priv->bytes_to_time = gst_base_rtp_audio_payload_sample_bytes_to_time;
    basertpaudiopayload->priv->bytes_to_rtptime = gst_base_rtp_audio_payload_sample_bytes_to_rtptime;
    basertpaudiopayload->priv->time_to_bytes = gst_base_rtp_audio_payload_sample_time_to_bytes;
}
...
void gst_base_rtp_audio_payload_set_frame_options(GstBaseRTPAudioPayload * basertpaudiopayload, gint frame_duration, gint frame_size)
{
    ...
    GST_DEBUG_OBJECT(basertpaudiopayload, "frame set to %d ms and size %d", frame_duration, frame_size);
}
...
void gst_base_rtp_audio_payload_set_sample_options(GstBaseRTPAudioPayload * basertpaudiopayload, gint sample_size)
{
    ...
    gst_base_rtp_audio_payload_set_samplebits_options(basertpaudiopayload, sample_size * 8);
}
 ...
 * Since: 0.10.18
 */
void gst_base_rtp_audio_payload_set_samplebits_options(GstBaseRTPAudioPayload * basertpaudiopayload, gint sample_size)
{
    guint fragment_size;
    GstBaseRTPAudioPayloadPrivate *priv;
    ...
    GST_DEBUG_OBJECT(basertpaudiopayload, "Samplebits set to sample size %d bits", sample_size);
}

static void gst_base_rtp_audio_payload_set_meta(GstBaseRTPAudioPayload * payload, GstBuffer * buffer, guint payload_len, GstClockTime timestamp)
{
 ...
 * Since: 0.10.13
 */
GstFlowReturn gst_base_rtp_audio_payload_push(GstBaseRTPAudioPayload * baseaudiopayload, const guint8 * data, guint payload_len, GstClockTime timestamp)
{
    GstBaseRTPPayload *basepayload;
    GstBuffer *outbuf;
    guint8 *payload;
    ...
    GST_DEBUG_OBJECT(baseaudiopayload, "Pushing %d bytes ts %" GST_TIME_FORMAT, payload_len, GST_TIME_ARGS(timestamp));
    ...
    /* set metadata */
    gst_base_rtp_audio_payload_set_meta(baseaudiopayload, outbuf, payload_len, timestamp);
...
static GstFlowReturn gst_base_rtp_audio_payload_push_buffer(GstBaseRTPAudioPayload * baseaudiopayload, GstBuffer * buffer)
{
    GstBaseRTPPayload *basepayload;
    GstBaseRTPAudioPayloadPrivate *priv;
    GstBuffer *outbuf;
 ...
 * Since: 0.10.25
 */
GstFlowReturn gst_base_rtp_audio_payload_flush(GstBaseRTPAudioPayload * baseaudiopayload, guint payload_len, GstClockTime timestamp)
{
    GstBaseRTPPayload *basepayload;
    GstBaseRTPAudioPayloadPrivate *priv;
    GstBuffer *outbuf;
    ...
    if (timestamp == -1)
    {
        ...
        GST_LOG_OBJECT(baseaudiopayload, "last timestamp %" GST_TIME_FORMAT ", distance %" G_GUINT64_FORMAT, GST_TIME_ARGS(timestamp), distance);
...
static gboolean gst_base_rtp_audio_payload_get_lengths(GstBaseRTPPayload * basepayload, guint * min_payload_len, guint * max_payload_len, guint * align)
{
    GstBaseRTPAudioPayload *payload;
    GstBaseRTPAudioPayloadPrivate *priv;
    guint max_mtu, mtu;
    ...
    if (G_LIKELY(priv->cached_mtu == mtu
                 ...
                 basepayload->abidata.ABI.ptime_multiple && priv->cached_ptime == basepayload->abidata.ABI.ptime && priv->cached_max_ptime == basepayload->max_ptime && priv->cached_min_ptime == basepayload->min_ptime))
    {
    ...
    ptime_mult_octets = priv->time_to_bytes(payload, basepayload->abidata.ABI.ptime_multiple);
...
static guint32 gst_base_rtp_audio_payload_frame_bytes_to_rtptime(GstBaseRTPAudioPayload * payload, guint64 bytes)
{
    ...
    return gst_util_uint64_scale_int(time, GST_BASE_RTP_PAYLOAD(payload)->clock_rate, GST_SECOND);
}

static guint64 gst_base_rtp_audio_payload_frame_time_to_bytes(GstBaseRTPAudioPayload * payload, GstClockTime time)
{
    return gst_util_uint64_scale(time, payload->frame_size, payload->priv->frame_duration_ns);
}
...
static GstClockTime gst_base_rtp_audio_payload_sample_bytes_to_time(GstBaseRTPAudioPayload * payload, guint64 bytes)
{
    ...
    return gst_util_uint64_scale_int(rtptime, GST_SECOND, GST_BASE_RTP_PAYLOAD(payload)->clock_rate);
}
...
static guint64 gst_base_rtp_audio_payload_sample_time_to_bytes(GstBaseRTPAudioPayload * payload, guint64 time)
{
    guint64 samples;

    samples = gst_util_uint64_scale_int(time, GST_BASE_RTP_PAYLOAD(payload)->clock_rate, GST_SECOND);
...
static GstFlowReturn gst_base_rtp_audio_payload_handle_buffer(GstBaseRTPPayload * basepayload, GstBuffer * buffer)
{
    GstBaseRTPAudioPayload *payload;
    GstBaseRTPAudioPayloadPrivate *priv;
    guint payload_len;
    ...
    if (discont)
    {
        ...
        if (priv->last_timestamp != -1 && timestamp != -1)
        {
            ...
            if (timestamp > priv->last_timestamp)
            {
                ...
                GST_DEBUG_OBJECT(payload, "elapsed time %" GST_TIME_FORMAT ", bytes %" G_GUINT64_FORMAT ", new offset %" G_GUINT64_FORMAT, GST_TIME_ARGS(diff), bytes, priv->offset);
            }
        }
    }

    if (!gst_base_rtp_audio_payload_get_lengths(basepayload, &min_payload_len, &max_payload_len, &align))
        goto config_error;

    GST_DEBUG_OBJECT(payload, "Calculated min_payload_len %u and max_payload_len %u", min_payload_len, max_payload_len);
    ...
    GST_DEBUG_OBJECT(payload, "got buffer size %u, available %u", size, available);

    if (available == 0 && (size >= min_payload_len && size <= max_payload_len) && (size % align == 0))
    {
...
config_error:
    {
        GST_ELEMENT_ERROR(payload, STREAM, NOT_IMPLEMENTED, (NULL), ("subclass did not configure us properly"));
        gst_buffer_unref(buffer);
