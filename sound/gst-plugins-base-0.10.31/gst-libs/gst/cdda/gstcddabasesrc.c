...
static void gst_cdda_base_src_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
static void gst_cdda_base_src_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
...
static gboolean gst_cdda_base_src_handle_event(GstBaseSrc * basesrc, GstEvent * event);
static gboolean gst_cdda_base_src_do_seek(GstBaseSrc * basesrc, GstSegment * segment);
...
static GstFlowReturn gst_cdda_base_src_create(GstPushSrc * pushsrc, GstBuffer ** buf);
...
GST_BOILERPLATE_FULL(GstCddaBaseSrc, gst_cdda_base_src, GstPushSrc, GST_TYPE_PUSH_SRC, gst_cdda_base_src_setup_interfaces);
...
static GstStaticPadTemplate gst_cdda_base_src_src_template = GST_STATIC_PAD_TEMPLATE("src",
                                                                                     GST_PAD_SRC,
                                                                                     GST_PAD_ALWAYS,
...
static void gst_cdda_base_src_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gst_cdda_base_src_src_template));
    ...
    GST_DEBUG_CATEGORY_INIT(gst_cdda_base_src_debug, "cddabasesrc", 0, "CDDA Base Source");
}

static void gst_cdda_base_src_class_init(GstCddaBaseSrcClass * klass)
{
    ...
    g_object_class_install_property(G_OBJECT_CLASS(klass), ARG_DEVICE, g_param_spec_string("device", "Device", "CD device location", NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), ARG_MODE, g_param_spec_enum("mode", "Mode", "Mode", GST_TYPE_CDDA_BASE_SRC_MODE, GST_CDDA_BASE_SRC_MODE_NORMAL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(G_OBJECT_CLASS(klass), ARG_TRACK, g_param_spec_uint("track", "Track", "Track", 1, 99, 1, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    g_object_class_install_property(G_OBJECT_CLASS(klass), ARG_TOC_OFFSET, g_param_spec_int("toc-offset", "Table of contents offset", "Add <n> sectors to the values reported", G_MININT, G_MAXINT, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), ARG_TOC_BIAS,
                                    g_param_spec_boolean("toc-bias", "Table of contents bias",
                                                         ...
                                                         "will be addressed as LBA 0.  Necessary for some Toshiba drives to " "get track boundaries", FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
#endif
    ...
    basesrc_class->is_seekable = GST_DEBUG_FUNCPTR(gst_cdda_base_src_is_seekable);

    pushsrc_class->create = GST_DEBUG_FUNCPTR(gst_cdda_base_src_create);
}

static void gst_cdda_base_src_init(GstCddaBaseSrc * src, GstCddaBaseSrcClass * klass)
{
    gst_pad_set_query_type_function(GST_BASE_SRC_PAD(src), GST_DEBUG_FUNCPTR(gst_cdda_base_src_get_query_types));
...
static void gst_cdda_base_src_set_device(GstCddaBaseSrc * src, const gchar * device)
{
    if (src->device)
        g_free(src->device);
    src->device = NULL;
...
static gint gst_cdda_base_src_get_track_from_sector(GstCddaBaseSrc * src, gint sector)
{
    gint i;
...
static gboolean gst_cdda_base_src_convert(GstCddaBaseSrc * src, GstFormat src_format, gint64 src_val, GstFormat dest_format, gint64 * dest_val)
{
    gboolean started;

    GST_LOG_OBJECT(src, "converting value %" G_GINT64_FORMAT " from %s into %s", src_val, gst_format_get_name(src_format), gst_format_get_name(dest_format));
...
wrong_value:
    {
        GST_DEBUG_OBJECT(src, "conversion failed: %s", "source value not within allowed range");
        return FALSE;
    }

not_started:
    {
        GST_DEBUG_OBJECT(src, "conversion failed: %s", "cannot do this conversion, device not open");
        return FALSE;
    }
}

static gboolean gst_cdda_base_src_query(GstBaseSrc * basesrc, GstQuery * query)
{
    ...
    GST_LOG_OBJECT(src, "handling %s query", gst_query_type_get_name(GST_QUERY_TYPE(query)));
        ...
        case GST_QUERY_DURATION:
            {
                ...
                {
                    sectors = src->tracks[src->cur_track].end - src->tracks[src->cur_track].start + 1;
                }
                else
                {
                    sectors = src->tracks[src->num_tracks - 1].end - src->tracks[0].start + 1;
                }

                /* ... and convert into final format */
                if (!gst_cdda_base_src_convert(src, sector_format, sectors, dest_format, &dest_val))
                {
                    return FALSE;
                }
                ...
                GST_LOG("duration: %u sectors, %" G_GINT64_FORMAT " in format %s", sectors, dest_val, gst_format_get_name(dest_format));
                break;
            }
        case GST_QUERY_POSITION:
            {
                ...
                if (!gst_cdda_base_src_convert(src, sector_format, pos_sector, dest_format, &dest_val))
                {
                    return FALSE;
                }
                ...
                GST_LOG("position: sector %u, %" G_GINT64_FORMAT " in format %s", (guint) pos_sector, dest_val, gst_format_get_name(dest_format));
                break;
            }
        case GST_QUERY_CONVERT:
            {
                ...
                if (!gst_cdda_base_src_convert(src, src_format, src_val, dest_format, &dest_val))
                {
                    return FALSE;
                }
...
static gboolean gst_cdda_base_src_do_seek(GstBaseSrc * basesrc, GstSegment * segment)
{
    ...
    GST_DEBUG_OBJECT(src, "segment %" GST_TIME_FORMAT "-%" GST_TIME_FORMAT, GST_TIME_ARGS(segment->start), GST_TIME_ARGS(segment->stop));

    if (!gst_cdda_base_src_convert(src, GST_FORMAT_TIME, segment->start, sector_format, &seek_sector))
    {
...
static gboolean gst_cdda_base_src_handle_track_seek(GstCddaBaseSrc * src, gdouble rate, GstSeekFlags flags, GstSeekType start_type, gint64 start, GstSeekType stop_type, gint64 stop)
{
    ...
    if ((flags & GST_SEEK_FLAG_SEGMENT) == GST_SEEK_FLAG_SEGMENT)
    {
        ...
        {
            GST_DEBUG_OBJECT(src, "segment seek in track format is only " "supported in CONTINUOUS mode, not in mode %d", src->mode);
            return FALSE;
        }
        ...
        switch (start_type)
        {
            case GST_SEEK_TYPE_SET:
                if (!gst_cdda_base_src_convert(src, track_format, start, GST_FORMAT_TIME, &start_time))
                {
                    GST_DEBUG_OBJECT(src, "cannot convert track %d to time", (gint) start);
                    return FALSE;
                }
                break;
            case GST_SEEK_TYPE_END:
                if (!gst_cdda_base_src_convert(src, track_format, src->num_tracks - start - 1, GST_FORMAT_TIME, &start_time))
                {
                    GST_DEBUG_OBJECT(src, "cannot convert track %d to time", (gint) start);
                    return FALSE;
                }
        ...
        {
            case GST_SEEK_TYPE_SET:
                if (!gst_cdda_base_src_convert(src, track_format, stop, GST_FORMAT_TIME, &stop_time))
                {
                    GST_DEBUG_OBJECT(src, "cannot convert track %d to time", (gint) stop);
                    return FALSE;
                }
                break;
            case GST_SEEK_TYPE_END:
                if (!gst_cdda_base_src_convert(src, track_format, src->num_tracks - stop - 1, GST_FORMAT_TIME, &stop_time))
                {
                    GST_DEBUG_OBJECT(src, "cannot convert track %d to time", (gint) stop);
                    return FALSE;
                }
        ...
        GST_LOG_OBJECT(src, "seek segment %" GST_TIME_FORMAT "-%" GST_TIME_FORMAT, GST_TIME_ARGS(start_time), GST_TIME_ARGS(stop_time));

        /* send fake segment seek event in TIME format to
         * base class, which will hopefully handle the rest */

        event = gst_event_new_seek(rate, GST_FORMAT_TIME, flags, start_type, start_time, stop_type, stop_time);
    ...
    event = gst_event_new_seek(rate, GST_FORMAT_TIME, flags, GST_SEEK_TYPE_SET, 0, GST_SEEK_TYPE_NONE, -1);
        ...
        case GST_EVENT_SEEK:
            {
                ...
                {
                    ret = gst_cdda_base_src_handle_track_seek(src, rate, flags, start_type, start, stop_type, stop);
                }
                else
                {
                    GST_LOG_OBJECT(src, "let base class handle seek in %s format", gst_format_get_name(format));
...
static GstURIType gst_cdda_base_src_uri_get_type(void)
{
    return GST_URI_SRC;
}

static gchar **gst_cdda_base_src_uri_get_protocols(void)
{
...
static const gchar *gst_cdda_base_src_uri_get_uri(GstURIHandler * handler)
{
    ...
    {
        src->uri = g_strdup_printf("cdda://%s#%d", src->device, (src->uri_track > 0) ? src->uri_track : 1);
    }
    else
    {
...
static gboolean gst_cdda_base_src_uri_set_uri(GstURIHandler * handler, const gchar * uri)
{
    GstCddaBaseSrc *src = GST_CDDA_BASE_SRC(handler);
    gchar *protocol;
    ...
    if (src->num_tracks > 0 && src->tracks != NULL && src->uri_track > src->num_tracks)
        goto failed;
    ...
    if (src->uri_track > 0 && src->tracks != NULL)
    {
        ...
        gst_pad_send_event(GST_BASE_SRC_PAD(src), gst_event_new_seek(1.0, track_format, GST_SEEK_FLAG_FLUSH, GST_SEEK_TYPE_SET, src->uri_track - 1, GST_SEEK_TYPE_NONE, -1));
    }
    else
    {
...
static void gst_cdda_base_src_uri_handler_init(gpointer g_iface, gpointer iface_data)
{
...
gboolean gst_cdda_base_src_add_track(GstCddaBaseSrc * src, GstCddaBaseSrcTrack * track)
{
    ...
    GST_DEBUG_OBJECT(src, "adding track %2u (%2u) [%6u-%6u] [%5s], tags: %" GST_PTR_FORMAT, src->num_tracks + 1, track->num, track->start, track->end, (track->is_audio) ? "AUDIO" : "DATA ", track->tags);
...
static void gst_cdda_base_src_update_duration(GstCddaBaseSrc * src)
{
    ...
    gst_element_post_message(GST_ELEMENT(src), gst_message_new_duration(GST_OBJECT(src), GST_FORMAT_TIME, -1));

    GST_LOG_OBJECT(src, "duration updated to %" GST_TIME_FORMAT, GST_TIME_ARGS(duration));
}

#define CD_MSF_OFFSET 150

/* the cddb hash function */
static guint cddb_sum(gint n)
{
    guint ret;
...
static void gst_cddabasesrc_calculate_musicbrainz_discid(GstCddaBaseSrc * src)
{
    GString *s;
    GChecksum *sha;
    guchar digest[20];
    ...
    gst_tag_list_add(src->tags, GST_TAG_MERGE_REPLACE, GST_TAG_CDDA_MUSICBRAINZ_DISCID, src->mb_discid, GST_TAG_CDDA_MUSICBRAINZ_DISCID_FULL, s->str, NULL);

    g_string_free(s, TRUE);
}

static void lba_to_msf(guint sector, guint * p_m, guint * p_s, guint * p_f, guint * p_secs)
{
    guint m, s, f;
...
static void gst_cdda_base_src_calculate_cddb_id(GstCddaBaseSrc * src)
{
    GString *s;
    ...
    for (i = 0; i < src->num_tracks; ++i)
    {
        ...
        {                        /* src->tracks[i].is_audio) { */
            ...
            lba_to_msf(src->tracks[i].start + CD_MSF_OFFSET, NULL, NULL, NULL, &secs);

            len_secs = (src->tracks[i].end - src->tracks[i].start + 1) / 75;

            GST_DEBUG_OBJECT(src, "track %02u: lsn %6u (%02u:%02u), " "length: %u seconds (%02u:%02u)", num_audio_tracks, src->tracks[i].start + CD_MSF_OFFSET, secs / 60, secs % 60, len_secs, len_secs / 60, len_secs % 60);
    ...
    GST_DEBUG_OBJECT(src, "first_sector = %u = %u secs (%02u:%02u)", first_sector, start_secs, start_secs / 60, start_secs % 60);
    GST_DEBUG_OBJECT(src, "last_sector  = %u = %u secs (%02u:%02u)", last_sector, end_secs, end_secs / 60, end_secs % 60);

    t = end_secs - start_secs;

    GST_DEBUG_OBJECT(src, "total length = %u secs (%02u:%02u), added title " "lengths = %u seconds (%02u:%02u)", t, t / 60, t % 60, total_secs, total_secs / 60, total_secs % 60);
    ...
    gst_tag_list_add(src->tags, GST_TAG_MERGE_REPLACE, GST_TAG_CDDA_CDDB_DISCID, s->str, NULL);
    ...
    gst_tag_list_add(src->tags, GST_TAG_MERGE_REPLACE, GST_TAG_CDDA_CDDB_DISCID_FULL, s->str, NULL);
...
static void gst_cdda_base_src_add_tags(GstCddaBaseSrc * src)
{
    gint i;
    ...
    for (i = 0; i < src->num_tracks; ++i)
    {
        ...
        gst_cdda_base_src_convert(src, sector_format, num_sectors, GST_FORMAT_TIME, &duration);

        gst_tag_list_add(src->tracks[i].tags, GST_TAG_MERGE_REPLACE, GST_TAG_TRACK_NUMBER, i + 1, GST_TAG_TRACK_COUNT, src->num_tracks, GST_TAG_DURATION, duration, NULL);
    }
    ...
    gst_tag_list_add(src->tags, GST_TAG_MERGE_REPLACE, GST_TAG_TRACK_COUNT, src->num_tracks, NULL);
#if 0
    for (i = 0; i < src->num_tracks; ++i)
    {
        gst_tag_list_add(src->tags, GST_TAG_MERGE_APPEND, GST_TAG_CDDA_TRACK_TAGS, src->tracks[i].tags, NULL);
    }
#endif
...
static void gst_cdda_base_src_add_index_associations(GstCddaBaseSrc * src)
{
    gint i;
    ...
    for (i = 0; i < src->num_tracks; i++)
    {
        ...
        gst_index_add_association(src->index, src->index_id, GST_ASSOCIATION_FLAG_KEY_UNIT, track_format, i,    /* here we count from 0 */
                                  ...
                                  GST_FORMAT_TIME, (gint64) (((CD_FRAMESIZE_RAW >> 2) * sector * GST_SECOND) / 44100), GST_FORMAT_BYTES, (gint64) (sector << 2), GST_FORMAT_DEFAULT, (gint64) ((CD_FRAMESIZE_RAW >> 2) * sector), NULL);
    }
}

static void gst_cdda_base_src_set_index(GstElement * element, GstIndex * index)
{
...
static GstIndex *gst_cdda_base_src_get_index(GstElement * element)
{
    GstCddaBaseSrc *src = GST_CDDA_BASE_SRC(element);
    GstIndex *index;
...
static gint gst_cdda_base_src_track_sort_func(gconstpointer a, gconstpointer b, gpointer foo)
{
...
static gboolean gst_cdda_base_src_start(GstBaseSrc * basesrc)
{
    ...
    if (src->num_tracks == 0 || src->tracks == NULL)
    {
        ...
        GST_ELEMENT_ERROR(src, RESOURCE, OPEN_READ, (_("This CD has no audio tracks")), (NULL));
        gst_cdda_base_src_stop(basesrc);
        return FALSE;
    }
    ...
    g_qsort_with_data(src->tracks, src->num_tracks, sizeof(GstCddaBaseSrcTrack), gst_cdda_base_src_track_sort_func, NULL);
...
static void gst_cdda_base_src_clear_tracks(GstCddaBaseSrc * src)
{
    if (src->tracks != NULL)
    {
        gint i;
...
static GstFlowReturn gst_cdda_base_src_create(GstPushSrc * pushsrc, GstBuffer ** buffer)
{
    ...
    switch (src->mode)
    {
        ...
        case GST_CDDA_BASE_SRC_MODE_CONTINUOUS:
            ...
            src->cur_track = gst_cdda_base_src_get_track_from_sector(src, src->cur_sector);
            break;
        default:
    ...
    {
        src->prev_track = -1;
        GST_DEBUG_OBJECT(src, "EOS at sector %d, cur_track=%d, mode=%d", src->cur_sector, src->cur_track, src->mode);
    ...
    if (src->prev_track != src->cur_track)
    {
        ...
        tags = gst_tag_list_merge(src->tags, src->tracks[src->cur_track].tags, GST_TAG_MERGE_REPLACE);
    ...
    {
        duration = gst_util_uint64_scale_int(GST_BUFFER_SIZE(buf) >> 2, GST_SECOND, 44100);
    }

    GST_BUFFER_TIMESTAMP(buf) = position;
    GST_BUFFER_DURATION(buf) = duration;

    GST_LOG_OBJECT(src, "pushing sector %d with timestamp %" GST_TIME_FORMAT, src->cur_sector, GST_TIME_ARGS(position));
