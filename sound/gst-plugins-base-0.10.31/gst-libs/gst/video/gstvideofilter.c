...
GType gst_video_filter_get_type(void)
{
    ...
    if (!video_filter_type)
    {
        ...
        video_filter_type = g_type_register_static(GST_TYPE_BASE_TRANSFORM, "GstVideoFilter", &video_filter_info, G_TYPE_FLAG_ABSTRACT);
    }
    return video_filter_type;
}

static gboolean gst_video_filter_get_unit_size(GstBaseTransform * btrans, GstCaps * caps, guint * size)
{
    GstVideoFormat fmt;
    gint width, height;
    ...
    *size = gst_video_format_get_size(fmt, width, height);
    ...
    GST_DEBUG_OBJECT(btrans, "Returning size %u bytes for caps %" GST_PTR_FORMAT, *size, caps);
...
static void gst_video_filter_class_init(gpointer g_class, gpointer class_data)
{
    ...
    trans_class->get_unit_size = GST_DEBUG_FUNCPTR(gst_video_filter_get_unit_size);

    parent_class = g_type_class_peek_parent(klass);

    GST_DEBUG_CATEGORY_INIT(gst_video_filter_debug, "videofilter", 0, "videofilter");
}
