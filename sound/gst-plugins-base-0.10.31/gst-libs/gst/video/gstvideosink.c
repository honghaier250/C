...
static void gst_video_sink_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_video_sink_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

static GstFlowReturn gst_video_sink_show_preroll_frame(GstBaseSink * bsink, GstBuffer * buf);
static GstFlowReturn gst_video_sink_show_frame(GstBaseSink * bsink, GstBuffer * buf);
...
void gst_video_sink_center_rect(GstVideoRectangle src, GstVideoRectangle dst, GstVideoRectangle * result, gboolean scaling)
{
    ...
    GST_DEBUG("source is %dx%d dest is %dx%d, result is %dx%d with x,y %dx%d", src.w, src.h, dst.w, dst.h, result->w, result->h, result->x, result->y);
}

/* Initing stuff */

static void gst_video_sink_init(GstVideoSink * videosink)
{
    videosink->width = 0;
    videosink->height = 0;
    ...
    videosink->priv = G_TYPE_INSTANCE_GET_PRIVATE(videosink, GST_TYPE_VIDEO_SINK, GstVideoSinkPrivate);
}
...
static void gst_video_sink_class_init(GstVideoSinkClass * klass)
{
   ...
   */
    g_object_class_install_property(gobject_class, PROP_SHOW_PREROLL_FRAME,
                                    g_param_spec_boolean("show-preroll-frame", "Show preroll frame", "Whether to render video frames during preroll", DEFAULT_SHOW_PREROLL_FRAME, G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS));

    basesink_class->render = GST_DEBUG_FUNCPTR(gst_video_sink_show_frame);
    basesink_class->preroll = GST_DEBUG_FUNCPTR(gst_video_sink_show_preroll_frame);

    g_type_class_add_private(klass, sizeof(GstVideoSinkPrivate));
}

static void gst_video_sink_base_init(gpointer g_class)
{
...
static GstFlowReturn gst_video_sink_show_preroll_frame(GstBaseSink * bsink, GstBuffer * buf)
{
    ...
    {
        GST_DEBUG_OBJECT(bsink, "not rendering frame with ts=%" GST_TIME_FORMAT ", preroll rendering disabled", GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buf)));
    }
    ...
    GST_LOG_OBJECT(bsink, "rendering frame, ts=%" GST_TIME_FORMAT, GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buf)));
...
static void gst_video_sink_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    ...
    switch (prop_id)
    {
        case PROP_SHOW_PREROLL_FRAME:
            g_atomic_int_set(&vsink->priv->show_preroll_frame, g_value_get_boolean(value));
            break;
        default:
...
static void gst_video_sink_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
    ...
    {
        case PROP_SHOW_PREROLL_FRAME:
            g_value_set_boolean(value, g_atomic_int_get(&vsink->priv->show_preroll_frame));
            break;
        default:
            G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
            break;
...
GType gst_video_sink_get_type(void)
{
    ...
    if (!videosink_type)
    {
        ...
        videosink_type = g_type_register_static(GST_TYPE_BASE_SINK, "GstVideoSink", &videosink_info, 0);
    }
