...
static gboolean caps_are_raw(const GstCaps * caps)
{
    guint i, len;
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if (gst_structure_has_name(st, "video/x-raw-yuv") || gst_structure_has_name(st, "video/x-raw-rgb"))
            return TRUE;
    }
...
static gboolean create_element(const gchar * factory_name, GstElement ** element, GError ** err)
{
...
static GstElement *get_encoder(const GstCaps * caps, GError ** err)
{
    GList *encoders = NULL;
    GList *filtered = NULL;
    ...
    encoders = gst_element_factory_list_get_elements(GST_ELEMENT_FACTORY_TYPE_ENCODER | GST_ELEMENT_FACTORY_TYPE_MEDIA_IMAGE, GST_RANK_NONE);

    if (encoders == NULL)
    {
        *err = g_error_new(GST_CORE_ERROR, GST_CORE_ERROR_MISSING_PLUGIN, "Cannot find any image encoder");
        goto fail;
    }
    ...
    filtered = gst_element_factory_list_filter(encoders, caps, GST_PAD_SRC, FALSE);
    GST_INFO("got filtered list %p", filtered);

    if (filtered == NULL)
    {
        gchar *tmp = gst_caps_to_string(caps);
        *err = g_error_new(GST_CORE_ERROR, GST_CORE_ERROR_MISSING_PLUGIN, "Cannot find any image encoder for caps %s", tmp);
        g_free(tmp);
        goto fail;
    }
    ...
    GST_INFO("created encoder element %p, %s", encoder, gst_element_get_name(encoder));
...
static GstElement *build_convert_frame_pipeline(GstElement ** src_element, GstElement ** sink_element, const GstCaps * from_caps, const GstCaps * to_caps, GError ** err)
{
    ...
    if (!create_element("appsrc", &src, &error) || !create_element("ffmpegcolorspace", &csp, &error) || !create_element("videoscale", &vscale, &error) || !create_element("appsink", &sink, &error))
        goto no_elements;
 ...
 *
 */
GstBuffer *gst_video_convert_frame(GstBuffer * buf, const GstCaps * to_caps, GstClockTime timeout, GError ** err)
{
    GstMessage *msg;
    ...
    pipeline = build_convert_frame_pipeline(&src, &sink, from_caps, to_caps_copy, &error);
    if (!pipeline)
        goto no_pipeline;
    ...
    GST_DEBUG("running conversion pipeline to caps %" GST_PTR_FORMAT, to_caps_copy);
    gst_element_set_state(pipeline, GST_STATE_PAUSED);

    /* feed buffer in appsrc */
    GST_DEBUG("feeding buffer %p, size %u, caps %" GST_PTR_FORMAT, buf, GST_BUFFER_SIZE(buf), from_caps);
    g_signal_emit_by_name(src, "push-buffer", buf, &ret);

    /* now see what happens. We either got an error somewhere or the pipeline
     * prerolled */
    bus = gst_element_get_bus(pipeline);
    msg = gst_bus_timed_pop_filtered(bus, timeout, GST_MESSAGE_ERROR | GST_MESSAGE_ASYNC_DONE);
    ...
    else
    {
        ...
        if (err)
            *err = g_error_new(GST_CORE_ERROR, GST_CORE_ERROR_FAILED, "Could not convert video frame: timeout during conversion");
    }
...
static void gst_video_convert_frame_context_free(GstVideoConvertFrameContext * ctx)
{
...
static void gst_video_convert_frame_callback_context_free(GstVideoConvertFrameCallbackContext * ctx)
{
    if (ctx->context)
...
static gboolean convert_frame_dispatch_callback(GstVideoConvertFrameCallbackContext * ctx)
{
...
static void convert_frame_finish(GstVideoConvertFrameContext * context, GstBuffer * buffer, GError * error)
{
    GSource *source;
    ...
    g_source_set_callback(source, (GSourceFunc) convert_frame_dispatch_callback, ctx, (GDestroyNotify) gst_video_convert_frame_callback_context_free);
...
static gboolean convert_frame_timeout_callback(GstVideoConvertFrameContext * context)
{
    GError *error;
    ...
    error = g_error_new(GST_CORE_ERROR, GST_CORE_ERROR_FAILED, "Could not convert video frame: timeout");
...
static gboolean convert_frame_bus_callback(GstBus * bus, GstMessage * message, GstVideoConvertFrameContext * context)
{
    g_mutex_lock(context->mutex);
...
static void convert_frame_need_data_callback(GstElement * src, guint size, GstVideoConvertFrameContext * context)
{
    ...
    if (ret != GST_FLOW_OK)
    {
        ...
        error = g_error_new(GST_CORE_ERROR, GST_CORE_ERROR_FAILED, "Could not push video frame: %s", gst_flow_get_name(ret));

        convert_frame_finish(context, NULL, error);
    }

    g_signal_handlers_disconnect_by_func(src, convert_frame_need_data_callback, context);

done:
    g_mutex_unlock(context->mutex);
}

static void convert_frame_new_buffer_callback(GstElement * sink, GstVideoConvertFrameContext * context)
{
    GstBuffer *buf = NULL;
    GError *error = NULL;
    ...
    if (!buf)
    {
        error = g_error_new(GST_CORE_ERROR, GST_CORE_ERROR_FAILED, "Could not get converted video frame");
    }

    convert_frame_finish(context, buf, error);

    g_signal_handlers_disconnect_by_func(sink, convert_frame_need_data_callback, context);
...
void gst_video_convert_frame_async(GstBuffer * buf, const GstCaps * to_caps, GstClockTime timeout, GstVideoConvertFrameCallback callback, gpointer user_data, GDestroyNotify destroy_notify)
{
    ...
    if (timeout != GST_CLOCK_TIME_NONE)
    {
        ...
        g_source_set_callback(source, (GSourceFunc) convert_frame_timeout_callback, ctx, NULL);
        ctx->timeout_id = g_source_attach(source, context);
        g_source_unref(source);
    }

    g_signal_connect(src, "need-data", G_CALLBACK(convert_frame_need_data_callback), ctx);
    g_signal_connect(sink, "new-preroll", G_CALLBACK(convert_frame_new_buffer_callback), ctx);

    source = gst_bus_create_watch(bus);
    g_source_set_callback(source, (GSourceFunc) convert_frame_bus_callback, ctx, NULL);
