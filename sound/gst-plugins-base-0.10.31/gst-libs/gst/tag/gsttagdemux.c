...
static GstFlowReturn gst_tag_demux_read_range(GstTagDemux * tagdemux, guint64 offset, guint length, GstBuffer ** buffer);

static gboolean gst_tag_demux_src_checkgetrange(GstPad * srcpad);
static GstFlowReturn gst_tag_demux_src_getrange(GstPad * srcpad, guint64 offset, guint length, GstBuffer ** buffer);

static gboolean gst_tag_demux_add_srcpad(GstTagDemux * tagdemux, GstCaps * new_caps);
...
static GstStateChangeReturn gst_tag_demux_change_state(GstElement * element, GstStateChange transition);
...
GType gst_tag_demux_get_type(void)
{
    ...
    if (object_type == 0)
    {
        ...
        object_type = g_type_register_static(GST_TYPE_ELEMENT, "GstTagDemux", &object_info, G_TYPE_FLAG_ABSTRACT);
    }

    return object_type;
}

static void gst_tag_demux_base_init(gpointer klass)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(klass);

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&src_factory));

    GST_DEBUG_CATEGORY_INIT(tagdemux_debug, "tagdemux", 0, "tag demux base class");
}

static void gst_tag_demux_class_init(gpointer klass, gpointer d)
{
...
static void gst_tag_demux_reset(GstTagDemux * tagdemux)
{
    ...
    g_list_foreach(tagdemux->priv->pending_events, (GFunc) gst_mini_object_unref, NULL);
    g_list_free(tagdemux->priv->pending_events);
    tagdemux->priv->pending_events = NULL;
}

static void gst_tag_demux_init(GstTagDemux * demux, GstTagDemuxClass * gclass)
{
    GstElementClass *element_klass = GST_ELEMENT_CLASS(gclass);
    GstPadTemplate *tmpl;

    demux->priv = g_type_instance_get_private((GTypeInstance *) demux, GST_TYPE_TAG_DEMUX);
    ...
    if (tmpl)
    {
        ...
        gst_pad_set_activate_function(demux->priv->sinkpad, GST_DEBUG_FUNCPTR(gst_tag_demux_sink_activate));
        gst_pad_set_event_function(demux->priv->sinkpad, GST_DEBUG_FUNCPTR(gst_tag_demux_sink_event));
        gst_pad_set_chain_function(demux->priv->sinkpad, GST_DEBUG_FUNCPTR(gst_tag_demux_chain));
...
static gboolean gst_tag_demux_add_srcpad(GstTagDemux * tagdemux, GstCaps * new_caps)
{
    if (tagdemux->priv->src_caps == NULL || !gst_caps_is_equal(new_caps, tagdemux->priv->src_caps))
    {
        ...
        {
            GST_DEBUG_OBJECT(tagdemux, "Changing src pad caps to %" GST_PTR_FORMAT, tagdemux->priv->src_caps);
    ...
    if (tagdemux->priv->srcpad == NULL)
    {
        tagdemux->priv->srcpad = gst_pad_new_from_template(gst_element_class_get_pad_template(GST_ELEMENT_GET_CLASS(tagdemux), "src"), "src");
        g_return_val_if_fail(tagdemux->priv->srcpad != NULL, FALSE);

        gst_pad_set_query_type_function(tagdemux->priv->srcpad, GST_DEBUG_FUNCPTR(gst_tag_demux_get_query_types));
        gst_pad_set_query_function(tagdemux->priv->srcpad, GST_DEBUG_FUNCPTR(gst_tag_demux_pad_query));
        gst_pad_set_event_function(tagdemux->priv->srcpad, GST_DEBUG_FUNCPTR(gst_tag_demux_srcpad_event));
        gst_pad_set_activatepull_function(tagdemux->priv->srcpad, GST_DEBUG_FUNCPTR(gst_tag_demux_src_activate_pull));
        gst_pad_set_checkgetrange_function(tagdemux->priv->srcpad, GST_DEBUG_FUNCPTR(gst_tag_demux_src_checkgetrange));
        gst_pad_set_getrange_function(tagdemux->priv->srcpad, GST_DEBUG_FUNCPTR(gst_tag_demux_src_getrange));
        ...
        GST_DEBUG_OBJECT(tagdemux, "Adding src pad with caps %" GST_PTR_FORMAT, tagdemux->priv->src_caps);
...
static gboolean gst_tag_demux_remove_srcpad(GstTagDemux * demux)
{
    gboolean res = TRUE;
...
static gboolean gst_tag_demux_trim_buffer(GstTagDemux * tagdemux, GstBuffer ** buf_ref)
{
    ...
    if (tagdemux->priv->strip_end > 0)
    {
        ...
        {
            guint64 v1tag_offset = tagdemux->priv->upstream_size - tagdemux->priv->strip_end;
    ...
    if (need_sub == TRUE)
    {
        if (out_size != GST_BUFFER_SIZE(buf) || !gst_buffer_is_writable(buf))
        {
            ...
            GST_DEBUG_OBJECT(tagdemux, "Sub-buffering to trim size %d offset %" G_GINT64_FORMAT " to %d offset %" G_GINT64_FORMAT, GST_BUFFER_SIZE(buf), GST_BUFFER_OFFSET(buf), out_size, out_offset);
        ...
        {
            GST_DEBUG_OBJECT(tagdemux, "Adjusting buffer from size %d offset %" G_GINT64_FORMAT " to %d offset %" G_GINT64_FORMAT, GST_BUFFER_SIZE(buf), GST_BUFFER_OFFSET(buf), out_size, out_offset);
        }
...
static void gst_tag_demux_chain_parse_tag(GstTagDemux * demux, GstBuffer * collect)
{
    ...
    {
        GST_DEBUG_OBJECT(demux, "Received buffer from non-zero offset %" G_GINT64_FORMAT ". Can't read tags", GST_BUFFER_OFFSET(collect));
    ...
    {
        GST_DEBUG_OBJECT(demux, "Only %u bytes available, but %u needed " "to identify tag", available, klass->min_start_size);
    ...
    do
    {
        ...
        {
            GST_DEBUG_OBJECT(demux, "Only %u bytes available, but %u needed " "to parse tag", available, tagsize);
            return;                /* wait for more data */
        }
        ...
        switch (parse_ret)
        {
            ...
            case GST_TAG_DEMUX_RESULT_BROKEN_TAG:
                demux->priv->strip_start = newsize;
                demux->priv->parsed_tags = tags;
                GST_WARNING_OBJECT(demux, "Ignoring broken start tag of size %d", demux->priv->strip_start);
                break;
...
static GstFlowReturn gst_tag_demux_chain(GstPad * pad, GstBuffer * buf)
{
    ...
    switch (demux->priv->state)
    {
        case GST_TAG_DEMUX_READ_START_TAG:
            demux->priv->collect = gst_buffer_make_metadata_writable(demux->priv->collect);
        ...
        case GST_TAG_DEMUX_TYPEFINDING:
            {
                ...
                if (GST_BUFFER_SIZE(demux->priv->collect) < TYPE_FIND_MIN_SIZE + demux->priv->strip_start)
                    break;        /* Go get more data first */

                GST_DEBUG_OBJECT(demux, "Typefinding with size %d", GST_BUFFER_SIZE(demux->priv->collect));
                ...
                caps = gst_type_find_helper_for_buffer(GST_OBJECT(demux), typefind_buf, &probability);
                ...
                if (caps == NULL)
                {
                    ...
                    /* We failed typefind */
                    GST_ELEMENT_ERROR(demux, STREAM, TYPE_NOT_FOUND, (NULL), ("Could not detect type for contents within tag"));
                    gst_buffer_unref(typefind_buf);
                ...
                GST_DEBUG_OBJECT(demux, "Found type %" GST_PTR_FORMAT " with a " "probability of %u", caps, probability);
        ...
        case GST_TAG_DEMUX_STREAMING:
            {
                ...
                if (outbuf)
                {
                    ...
                    if (demux->priv->need_newseg)
                    {
                        ...
                        {
                            GST_WARNING_OBJECT(demux, "Downstream did not handle newsegment " "event as it should");
                        }
                    ...
                    while (events != NULL)
                    {
                        GST_DEBUG_OBJECT(demux->priv->srcpad, "sending cached %s event: %" GST_PTR_FORMAT, GST_EVENT_TYPE_NAME(events->data), events->data);
    ...
    switch (GST_EVENT_TYPE(event))
    {
        ...
        case GST_EVENT_NEWSEGMENT:
            {
                ...
                gst_event_parse_new_segment_full(event, &update, &rate, &arate, &format, &start, &stop, &position);

                gst_segment_set_newsegment_full(&demux->priv->segment, update, rate, arate, format, start, stop, position);
        ...
        default:
            if (demux->priv->need_newseg && GST_EVENT_IS_SERIALIZED(event))
            {
                ...
                GST_OBJECT_LOCK(demux);
                demux->priv->pending_events = g_list_append(demux->priv->pending_events, event);
                GST_OBJECT_UNLOCK(demux);
                ret = TRUE;
            }
            else
...
static gboolean gst_tag_demux_get_upstream_size(GstTagDemux * tagdemux)
{
    ...
    if (!gst_pad_query_peer_duration(tagdemux->priv->sinkpad, &format, &len) || len <= 0)
    {
        return FALSE;
    }
        ...
        case GST_EVENT_SEEK:
            {
                ...
                if (format == GST_FORMAT_BYTES && tagdemux->priv->state == GST_TAG_DEMUX_STREAMING && gst_pad_is_linked(tagdemux->priv->sinkpad))
                {
                    GstEvent *upstream;
                    ...
                    upstream = gst_event_new_seek(rate, format, flags, cur_type, cur, stop_type, stop);
...
static gboolean gst_tag_demux_pull_end_tag(GstTagDemux * demux, GstTagList ** tags)
{
    GstTagDemuxResult parse_ret;
    GstTagDemuxClass *klass;
    ...
    flow_ret = gst_pad_pull_range(demux->priv->sinkpad, offset, klass->min_end_size, &buffer);

    if (flow_ret != GST_FLOW_OK)
    {
        GST_DEBUG_OBJECT(demux, "Could not read tag header from end of file, " "ret = %s", gst_flow_get_name(flow_ret));
        goto done;
    }

    if (GST_BUFFER_SIZE(buffer) < klass->min_end_size)
    {
        GST_DEBUG_OBJECT(demux, "Only managed to read %u bytes from file " "(required: %u bytes)", GST_BUFFER_SIZE(buffer), klass->min_end_size);
        goto done;
    }
    ...
    do
    {
        ...
        if (GST_BUFFER_SIZE(buffer) != tagsize)
        {
            ...
            flow_ret = gst_pad_pull_range(demux->priv->sinkpad, offset, tagsize, &buffer);

            if (flow_ret != GST_FLOW_OK)
            {
                GST_DEBUG_OBJECT(demux, "Could not read data from end of file at " "offset %" G_GUINT64_FORMAT ". ret = %s", offset, gst_flow_get_name(flow_ret));
                goto done;
            }

            if (GST_BUFFER_SIZE(buffer) < tagsize)
            {
                GST_DEBUG_OBJECT(demux, "Only managed to read %u bytes from file", GST_BUFFER_SIZE(buffer));
                goto done;
            }
        }
        ...
        switch (parse_ret)
        {
            case GST_TAG_DEMUX_RESULT_OK:
                ...
                GST_DEBUG_OBJECT(demux, "Read tag at end, size %d", demux->priv->strip_end);
                break;
            case GST_TAG_DEMUX_RESULT_BROKEN_TAG:
                res = TRUE;
                demux->priv->strip_end = newsize;
                GST_WARNING_OBJECT(demux, "Ignoring broken tag at end, size %d", demux->priv->strip_end);
                break;
            case GST_TAG_DEMUX_RESULT_AGAIN:
...
static gboolean gst_tag_demux_pull_start_tag(GstTagDemux * demux, GstTagList ** tags)
{
    GstTagDemuxResult parse_ret;
    GstTagDemuxClass *klass;
    GstFlowReturn flow_ret;
    ...
    if (flow_ret != GST_FLOW_OK)
    {
        GST_DEBUG_OBJECT(demux, "Could not read data from start of file ret=%s", gst_flow_get_name(flow_ret));
        goto done;
    }

    if (GST_BUFFER_SIZE(buffer) < klass->min_start_size)
    {
        GST_DEBUG_OBJECT(demux, "Only managed to read %u bytes from file - " "no tag in this file", GST_BUFFER_SIZE(buffer));
        goto done;
    }
    ...
    do
    {
        ...
        if (GST_BUFFER_SIZE(buffer) < tagsize)
        {
            ...
            {
                GST_DEBUG_OBJECT(demux, "Only managed to read %u bytes from file", GST_BUFFER_SIZE(buffer));
                GST_ELEMENT_ERROR(demux, STREAM, DECODE, (_("Failed to read tag: not enough data")), (NULL));
                goto done;
            }
        }
...
static gboolean gst_tag_demux_sink_activate(GstPad * sinkpad)
{
     ...
     * preroll.
     */
    if (!gst_pad_check_pull_range(sinkpad) || !gst_pad_activate_pull(sinkpad, TRUE))
    {
        GST_DEBUG_OBJECT(demux, "No pull mode. Changing to push, but won't be " "able to read end tags");
    ...
    else
    {
        ...
        demux->priv->parsed_tags = gst_tag_list_merge(end_tags, start_tags, GST_TAG_MERGE_REPLACE);
    }
    ...
    if (demux->priv->upstream_size <= demux->priv->strip_start + demux->priv->strip_end)
    {
    ...
    caps = gst_type_find_helper_get_range(GST_OBJECT(demux), (GstTypeFindHelperGetRangeFunction) gst_tag_demux_read_range, demux->priv->upstream_size - (demux->priv->strip_start + demux->priv->strip_end), &probability);
...
static GstFlowReturn gst_tag_demux_read_range(GstTagDemux * demux, guint64 offset, guint length, GstBuffer ** buffer)
{
    GstFlowReturn ret;
    guint64 in_offset;
    guint in_length;
...
static void gst_tag_demux_send_tag_event(GstTagDemux * demux)
{
    ...
    if (demux->priv->parsed_tags)
        gst_element_post_message(GST_ELEMENT(demux), gst_message_new_tag(GST_OBJECT(demux), gst_tag_list_copy(demux->priv->parsed_tags)));
...
static gboolean gst_tag_demux_send_new_segment(GstTagDemux * tagdemux)
{
    ...
    {
        GST_LOG_OBJECT(tagdemux, "No new segment received before first buffer. Using default");
        gst_segment_set_newsegment(seg, FALSE, 1.0, GST_FORMAT_BYTES, tagdemux->priv->strip_start, -1, tagdemux->priv->strip_start);
    }

    /* Can't adjust segments in non-BYTES formats */
    if (tagdemux->priv->segment.format != GST_FORMAT_BYTES)
    {
        event = gst_event_new_new_segment_full(tagdemux->priv->newseg_update, seg->rate, seg->applied_rate, seg->format, seg->start, seg->stop, seg->time);
    ...
    if (tagdemux->priv->strip_end > 0)
    {
        if (gst_tag_demux_get_upstream_size(tagdemux))
        {
            ...
            {
                GST_DEBUG_OBJECT(tagdemux, "Segment crosses the end tag. Trimming end");
                stop = v1tag_offset;
            }
        }
    }
    ...
    GST_DEBUG_OBJECT(tagdemux, "Sending new segment update %d, rate %g, format %d, " "start %" G_GINT64_FORMAT ", stop %" G_GINT64_FORMAT ", position %" G_GINT64_FORMAT, tagdemux->priv->newseg_update, seg->rate, seg->format, start, stop, position);

    event = gst_event_new_new_segment_full(tagdemux->priv->newseg_update, seg->rate, seg->applied_rate, seg->format, start, stop, position);
    return gst_pad_push_event(tagdemux->priv->srcpad, event);
}
