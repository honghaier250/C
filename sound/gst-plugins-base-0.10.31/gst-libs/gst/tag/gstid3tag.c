...
G_CONST_RETURN gchar *gst_tag_from_id3_tag(const gchar * id3_tag)
{
    ...
    GST_INFO("Cannot map ID3v2 tag '%c%c%c%c' to GStreamer tag", id3_tag[0], id3_tag[1], id3_tag[2], id3_tag[3]);
 ...
 * Returns: The corresponding GStreamer tag or NULL if none exists.
 */
G_CONST_RETURN gchar *gst_tag_from_id3_user_tag(const gchar * type, const gchar * id3_user_tag)
{
    int i = 0;
    ...
    {
        if (strncmp(type, user_tag_matches[i].original_tag, 4) == 0 && g_ascii_strcasecmp(id3_user_tag, user_tag_matches[i].original_tag + 5) == 0)
        {
            GST_LOG("Mapped ID3v2 user tag '%s' to GStreamer tag '%s'", user_tag_matches[i].original_tag, user_tag_matches[i].gstreamer_tag);
            return user_tag_matches[i].gstreamer_tag;
        }
    }

    GST_INFO("Cannot map ID3v2 user tag '%s' of type '%s' to GStreamer tag", id3_user_tag, type);
...
static void gst_tag_extract_id3v1_string(GstTagList * list, const gchar * tag, const gchar * start, const guint size)
{
...
GstTagList *gst_tag_list_new_from_id3v1(const guint8 * data)
{
    ...
    {
        gst_tag_extract_id3v1_string(list, GST_TAG_COMMENT, (gchar *) & data[97], 28);
        gst_tag_list_add(list, GST_TAG_MERGE_REPLACE, GST_TAG_TRACK_NUMBER, (guint) data[126], NULL);
    }
    else
    {
        gst_tag_extract_id3v1_string(list, GST_TAG_COMMENT, (gchar *) & data[97], 30);
    }
...
gboolean gst_tag_list_add_id3_image(GstTagList * tag_list, const guint8 * image_data, guint image_data_len, guint id3_picture_type)
{
    ...
    image = gst_tag_image_data_to_image_buffer(image_data, image_data_len, tag_image_type);
