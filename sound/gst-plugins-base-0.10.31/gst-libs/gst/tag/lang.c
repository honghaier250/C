...
static GstDebugCategory *ensure_debug_category(void)
{
    ...
    if (g_once_init_enter(&cat_gonce))
    {
        ...
        cat_done = (gsize) _gst_debug_category_new("tag-langcodes", 0, "GstTag language codes and names");
...
static const gchar *get_val(const gchar ** names, const gchar ** vals, const gchar * name)
{
...
static void parse_start_element(GMarkupParseContext * ctx, const gchar * element_name, const gchar ** attr_names, const gchar ** attr_vals, gpointer user_data, GError ** error)
{
    ...
    {
        GST_WARNING("broken iso_639.xml entry: c2t=%p, c2b=%p, name=%p", c2t, c2b, name);
        return;
    }
    ...
    if (strcmp(c2t, c2b) != 0)
    {
        g_hash_table_replace(ht, (gpointer) g_intern_string(c2t), (gpointer) tname);
    }

    GST_LOG("%s %s %s : %s - %s", c1, c2t, c2b, name, tname);
}

static void gst_tag_load_iso_639_xml(GHashTable * ht)
{
    GMappedFile *f;
    GError *err = NULL;
    gchar *xml_data;
...
#    ifdef ENABLE_NLS
    GST_DEBUG("binding text domain %s to locale dir %s", GETTEXT_PACKAGE, ISO_CODES_LOCALEDIR);
...
static GHashTable *gst_tag_get_iso_639_ht(void)
{
    ...
    if (g_once_init_enter(&once_val))
    {
        ...
        GST_MEMDUMP("iso 639 language names (internal default/fallback)", (guint8 *) iso_639_names, sizeof(iso_639_names));
        ...
        for (i = 0; i < G_N_ELEMENTS(iso_639_codes); ++i)
        {
            GST_LOG("%3d %s %s %c%c 0x%04x  %s", i, iso_639_codes[i].iso_639_1,
                    iso_639_codes[i].iso_639_2, ((iso_639_codes[i].flags & ISO_639_FLAG_2B)) ? 'B' : '.', ((iso_639_codes[i].flags & ISO_639_FLAG_2T)) ? 'T' : '.', iso_639_codes[i].name_offset, iso_639_names + iso_639_codes[i].name_offset);
            ...
            g_hash_table_insert(ht, (gpointer) iso_639_codes[i].iso_639_1, (gpointer) (iso_639_names + iso_639_codes[i].name_offset));
            g_hash_table_insert(ht, (gpointer) iso_639_codes[i].iso_639_2, (gpointer) (iso_639_names + iso_639_codes[i].name_offset));
        }
...
static int qsort_strcmp_func(const void *p1, const void *p2)
{
...
const gchar *gst_tag_get_language_code_iso_639_1(const gchar * lang_code)
{
    ...
    for (i = 0; i < G_N_ELEMENTS(iso_639_codes); ++i)
    {
        ...
        if (strcmp(lang_code, iso_639_codes[i].iso_639_1) == 0 || strcmp(lang_code, iso_639_codes[i].iso_639_2) == 0)
        {
...
static const gchar *gst_tag_get_language_code_iso_639_2X(const gchar * lang_code, guint8 flags)
{
    int i;
    ...
    for (i = 0; i < G_N_ELEMENTS(iso_639_codes); ++i)
    {
        ...
        if (strcmp(lang_code, iso_639_codes[i].iso_639_1) == 0 || strcmp(lang_code, iso_639_codes[i].iso_639_2) == 0)
        {
            ...
            else if (i > 0 && (iso_639_codes[i - 1].flags & flags) == flags && iso_639_codes[i].name_offset == iso_639_codes[i - 1].name_offset)
            {
                return iso_639_codes[i - 1].iso_639_2;
            }
            else if (i < G_N_ELEMENTS(iso_639_codes) && (iso_639_codes[i + 1].flags & flags) == flags && iso_639_codes[i].name_offset == iso_639_codes[i + 1].name_offset)
            {
                return iso_639_codes[i + 1].iso_639_2;
            }
        }
