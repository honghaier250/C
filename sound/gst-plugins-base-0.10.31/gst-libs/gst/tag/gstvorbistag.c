...
void gst_vorbis_tag_add(GstTagList * list, const gchar * tag, const gchar * value)
{
    ...
    if (gst_tag == NULL)
    {
        ...
        gst_tag_list_add(list, GST_TAG_MERGE_APPEND, GST_TAG_EXTENDED_COMMENT, ext_comment, NULL);
        g_free(ext_comment);
        return;
    }
        ...
        case G_TYPE_DOUBLE:
            {
                ...
                c = g_strdup(value);
                g_strdelimit(c, ",", '.');
                gst_tag_list_add(list, GST_TAG_MERGE_APPEND, gst_tag, g_strtod(c, NULL), NULL);
                g_free(c);
                break;
            }
        default:
            {
                if (tag_type == GST_TYPE_DATE)
                {
                    ...
                    if ((*check == '\0' || g_ascii_isspace(*check)) && y != 0 && g_date_valid_dmy(d, m, y))
                    {
                        GDate *date;
                ...
                {
                    GST_WARNING("Unhandled tag of type '%s' (%d)", g_type_name(tag_type), (gint) tag_type);
                }
                break;
            }
    }
...
static void gst_vorbis_tag_add_coverart(GstTagList * tags, gchar * img_data_base64, gint base64_len)
{
    GstBuffer *img;
    gsize img_len;
    guchar *out;
    ...
    img = gst_tag_image_data_to_image_buffer(out, img_len, GST_TAG_IMAGE_TYPE_NONE);

    if (img == NULL)
        goto convert_failed;

    gst_tag_list_add(tags, GST_TAG_MERGE_APPEND, GST_TAG_PREVIEW_IMAGE, img, NULL);
...
static GList *gst_tag_to_coverart(const GValue * image_value)
{
...
GList *gst_tag_to_vorbis_comments(const GstTagList * list, const gchar * tag)
{
     ...
     * image tag.
     */
    if ((strcmp(tag, GST_TAG_PREVIEW_IMAGE) == 0 && gst_tag_list_get_tag_size(list, GST_TAG_IMAGE) == 0) || strcmp(tag, GST_TAG_IMAGE) == 0)
    {
            ...
            default:
                {
                    if (tag_type == GST_TYPE_DATE)
                    {
                        ...
                        result = g_strdup_printf("%s=%04d-%02d-%02d", vorbis_tag, (gint) g_date_get_year(date), (gint) g_date_get_month(date), (gint) g_date_get_day(date));
                        g_date_free(date);
                    }
                    else
                    {
