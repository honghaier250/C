...
typedef void (*GstExifSerializationFunc) (GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag);
 ...
 */
typedef gint(*GstExifDeserializationFunc) (GstExifReader * exif_reader, GstByteReader * reader, const GstExifTagMatch * exiftag, GstExifTagData * tagdata);
...
static void gst_exif_reader_init(GstExifReader * reader, gint byte_order, const GstBuffer * buf, guint32 base_offset)
{
    ...
    if (reader->byte_order != G_LITTLE_ENDIAN && reader->byte_order != G_BIG_ENDIAN)
    {
        GST_WARNING("Unexpected byte order %d, using system default: %d", reader->byte_order, G_BYTE_ORDER);
        reader->byte_order = G_BYTE_ORDER;
    }
}

static void gst_exif_reader_add_pending_tag(GstExifReader * reader, GstExifTagData * data)
{
    GstExifTagData *copy;
...
static GstExifTagData *gst_exif_reader_get_pending_tag(GstExifReader * reader, gint tagid)
{
    GSList *walker;
...
static GstTagList *gst_exif_reader_reset(GstExifReader * reader, gboolean return_taglist)
{
    GstTagList *ret = NULL;
    GSList *walker;
...
static void gst_exif_writer_init(GstExifWriter * writer, gint byte_order)
{
    ...
    writer->tags_total = 0;
    if (writer->byte_order != G_LITTLE_ENDIAN && writer->byte_order != G_BIG_ENDIAN)
    {
        GST_WARNING("Unexpected byte order %d, using system default: %d", writer->byte_order, G_BYTE_ORDER);
        writer->byte_order = G_BYTE_ORDER;
    }
}

static GstBuffer *gst_exif_writer_reset_and_get_buffer(GstExifWriter * writer)
{
    GstBuffer *header;
    GstBuffer *data;
 ...
 */
static gint exif_tag_map_find_reverse(guint16 exif_tag, const GstExifTagMatch * tag_map, gboolean use_complementary)
{
    gint i;

    for (i = 0; tag_map[i].exif_tag != 0; i++)
    {
        if (exif_tag == tag_map[i].exif_tag || (use_complementary && exif_tag == tag_map[i].complementary_tag))
        {
            return i;
        }
    }
...
static gboolean gst_tag_list_has_ifd_tags(const GstTagList * taglist, const GstExifTagMatch * tag_map)
{
    gint i;
    ...
    for (i = 0; tag_map[i].exif_tag != 0; i++)
    {
        ...
        {
            if (tag_map[i].exif_tag == EXIF_GPS_IFD_TAG && gst_tag_list_has_ifd_tags(taglist, tag_map_gps))
                return TRUE;
            if (tag_map[i].exif_tag == EXIF_IFD_TAG && gst_tag_list_has_ifd_tags(taglist, tag_map_exif))
                return TRUE;
            continue;
        }
 ...
 */
static void gst_exif_writer_write_tag_header(GstExifWriter * writer, guint16 exif_tag, guint16 exif_type, guint32 count, guint32 offset, gboolean is_data)
{
    GST_DEBUG("Writing tag entry: id %x, type %u, count %u, offset %u", exif_tag, exif_type, count, offset);
...
static void gst_exif_writer_write_rational_data(GstExifWriter * writer, guint32 frac_n, guint32 frac_d)
{
    if (writer->byte_order == G_LITTLE_ENDIAN)
    {
...
static void gst_exif_writer_write_signed_rational_data(GstExifWriter * writer, gint32 frac_n, gint32 frac_d)
{
    if (writer->byte_order == G_LITTLE_ENDIAN)
    {
...
static void gst_exif_writer_write_rational_tag(GstExifWriter * writer, guint16 tag, guint32 frac_n, guint32 frac_d)
{
    guint32 offset = gst_byte_writer_get_size(&writer->datawriter);

    gst_exif_writer_write_tag_header(writer, tag, EXIF_TYPE_RATIONAL, 1, offset, FALSE);

    gst_exif_writer_write_rational_data(writer, frac_n, frac_d);
}

static void gst_exif_writer_write_signed_rational_tag(GstExifWriter * writer, guint16 tag, gint32 frac_n, gint32 frac_d)
{
    guint32 offset = gst_byte_writer_get_size(&writer->datawriter);

    gst_exif_writer_write_tag_header(writer, tag, EXIF_TYPE_SRATIONAL, 1, offset, FALSE);

    gst_exif_writer_write_signed_rational_data(writer, frac_n, frac_d);
}

static void gst_exif_writer_write_rational_tag_from_double(GstExifWriter * writer, guint16 tag, gdouble value)
{
    gint frac_n;
    gint frac_d;
...
static void gst_exif_writer_write_signed_rational_tag_from_double(GstExifWriter * writer, guint16 tag, gdouble value)
{
    gint frac_n;
    gint frac_d;
...
static void gst_exif_writer_write_byte_tag(GstExifWriter * writer, guint16 tag, guint8 value)
{
    guint32 offset = 0;

    GST_WRITE_UINT8((guint8 *) & offset, value);
    gst_exif_writer_write_tag_header(writer, tag, EXIF_TYPE_BYTE, 1, offset, TRUE);
}

static void gst_exif_writer_write_short_tag(GstExifWriter * writer, guint16 tag, guint16 value)
{
    guint32 offset = 0;
    ...
    gst_exif_writer_write_tag_header(writer, tag, EXIF_TYPE_SHORT, 1, offset, TRUE);
}

static void gst_exif_writer_write_long_tag(GstExifWriter * writer, guint16 tag, guint32 value)
{
    guint32 offset = 0;
    if (writer->byte_order == G_LITTLE_ENDIAN)
    {
    ...
    gst_exif_writer_write_tag_header(writer, tag, EXIF_TYPE_LONG, 1, offset, TRUE);
}

static void write_exif_undefined_tag(GstExifWriter * writer, guint16 tag, const guint8 * data, gint size)
{
    guint32 offset;
    ...
    if (size > 4)
    {
        ...
        gst_exif_writer_write_tag_header(writer, tag, EXIF_TYPE_UNDEFINED, size, offset, FALSE);
    ...
    else
    {
        ...
        gst_exif_writer_write_tag_header(writer, tag, EXIF_TYPE_UNDEFINED, size, offset, TRUE);
    }
}

static void write_exif_ascii_tag(GstExifWriter * writer, guint16 tag, const gchar * str)
{
    gint size;
    guint32 offset;
    ...
    if (size > 4)
    {
         ...
         * resulting tag headers offset and the base offset */
        offset = gst_byte_writer_get_size(&writer->datawriter);
        gst_exif_writer_write_tag_header(writer, tag, EXIF_TYPE_ASCII, size, offset, FALSE);
    ...
    else
    {
        ...
        gst_exif_writer_write_tag_header(writer, tag, EXIF_TYPE_ASCII, size, offset, TRUE);
    }
}

static void write_exif_ascii_tag_from_taglist(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
    gchar *str = NULL;
    ...
    switch (G_VALUE_TYPE(value))
    {
        ...
        default:
            if (G_VALUE_TYPE(value) == GST_TYPE_DATE_TIME)
            {
                ...
                str = g_strdup_printf("%04d:%02d:%02d %02d:%02d:%02d", gst_date_time_get_year(dt), gst_date_time_get_month(dt), gst_date_time_get_day(dt), gst_date_time_get_hour(dt), gst_date_time_get_minute(dt), gst_date_time_get_second(dt));

                cleanup = TRUE;
            }
            else
            {
                GST_WARNING("Conversion from %s to ascii string not supported", G_VALUE_TYPE_NAME(value));
            }
            break;
    }
...
static void write_exif_undefined_tag_from_taglist(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
    const GValue *value;
    ...
    switch (G_VALUE_TYPE(value))
    {
        ...
        default:
            ...
            {
                GST_WARNING("Conversion from %s to raw data not supported", G_VALUE_TYPE_NAME(value));
            }
            break;
    }
...
static void write_exif_rational_tag_from_taglist(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
    const GValue *value;
    gdouble num = 0;
    gint tag_size = gst_tag_list_get_tag_size(taglist, exiftag->gst_tag);
    ...
    switch (G_VALUE_TYPE(value))
    {
        case G_TYPE_DOUBLE:
            ...
            gst_exif_writer_write_rational_tag_from_double(writer, exiftag->exif_tag, num);
            break;
        default:
            if (G_VALUE_TYPE(value) == GST_TYPE_FRACTION)
            {
                gst_exif_writer_write_rational_tag(writer, exiftag->exif_tag, gst_value_get_fraction_numerator(value), gst_value_get_fraction_denominator(value));
            }
            else
            {
                GST_WARNING("Conversion from %s to rational not supported", G_VALUE_TYPE_NAME(value));
            }
            break;
    }
}

static void write_exif_integer_tag_from_taglist(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
    const GValue *value;
    guint32 num = 0;
    gint tag_size = gst_tag_list_get_tag_size(taglist, exiftag->gst_tag);
    ...
    switch (G_VALUE_TYPE(value))
    {
        case G_TYPE_INT:
            ...
            break;
        default:
            GST_WARNING("Conversion from %s to int not supported", G_VALUE_TYPE_NAME(value));
            break;
    }
...
static void write_exif_tag_from_taglist(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
...
static void tagdata_copy(GstExifTagData * to, const GstExifTagData * from)
{
    to->tag = from->tag;
...
static void gst_exif_tag_rewrite_offsets(GstByteWriter * writer, gint byte_order, guint32 offset, gint num_tags, GstByteWriter * inner_ifds_data)
{
    ...
    for (i = 0; i < num_tags; i++)
    {
        ...
        g_assert(gst_byte_writer_get_pos(writer) < gst_byte_writer_get_size(writer));
        ...
        if (byte_size > 4 || tag_id == EXIF_GPS_IFD_TAG || tag_id == EXIF_IFD_TAG)
        {
            ...
            GST_DEBUG("Rewriting tag offset from %u to (%u + %u) %u", cur_offset, cur_offset, offset, cur_offset + offset);

            if ((tag_id == EXIF_GPS_IFD_TAG || tag_id == EXIF_IFD_TAG) && inner_ifds_data != NULL)
            {
                /* needs special handling */
                if (!gst_byte_writer_set_pos(inner_ifds_data, cur_offset))
                {
                    GST_WARNING("Failed to position writer to rewrite inner ifd " "offsets");
                    continue;
                }

                gst_exif_tag_rewrite_offsets(inner_ifds_data, byte_order, offset, -1, NULL);
            }
        }
        else
        {
...
static void parse_exif_ascii_tag(GstExifReader * reader, const GstExifTagMatch * tag, guint32 count, guint32 offset, const guint8 * offset_as_data)
{
    GType tagtype;
    gchar *str;
    guint32 real_offset;
    ...
    if (count > 4)
    {
        ...
        {
            GST_WARNING("Offset is smaller (%u) than base offset (%u)", offset, reader->base_offset);
            return;
        }
        ...
        {
            GST_WARNING("Invalid offset %u for buffer of size %u, not adding tag %s", real_offset, GST_BUFFER_SIZE(reader->buffer), tag->gst_tag);
            return;
        }

        str = g_strndup((gchar *) (GST_BUFFER_DATA(reader->buffer) + real_offset), count);
    }
    else
    {
    ...
    if (tagtype == GST_TYPE_DATE_TIME)
    {
        ...
        if (sscanf(str, "%04d:%02d:%02d %02d:%02d:%02d", &year, &month, &day, &hour, &minute, &second) > 0)
        {
            GstDateTime *d;

            d = gst_date_time_new_local_time(year, month, day, hour, minute, second);
            gst_tag_list_add(reader->taglist, GST_TAG_MERGE_REPLACE, tag->gst_tag, d, NULL);
    ...
    {
        gst_tag_list_add(reader->taglist, GST_TAG_MERGE_REPLACE, tag->gst_tag, str, NULL);
    }
    else
    {
        GST_WARNING("No parsing function associated to %x(%s)", tag->exif_tag, tag->gst_tag);
    }
    g_free(str);
}

static void parse_exif_long_tag(GstExifReader * reader, const GstExifTagMatch * tag, guint32 count, guint32 offset, const guint8 * offset_as_data)
{
    GType tagtype;
    ...
    {
        gst_tag_list_add(reader->taglist, GST_TAG_MERGE_REPLACE, tag->gst_tag, offset, NULL);
    }
    else
    {
        GST_WARNING("No parsing function associated to %x(%s)", tag->exif_tag, tag->gst_tag);
...
static void parse_exif_undefined_tag(GstExifReader * reader, const GstExifTagMatch * tag, guint32 count, guint32 offset, const guint8 * offset_as_data)
{
    GType tagtype;
    guint8 *data;
    guint32 real_offset;
    ...
    if (tagtype == GST_TYPE_BUFFER)
    {
        ...
        gst_tag_list_add(reader->taglist, GST_TAG_MERGE_APPEND, tag->gst_tag, buf, NULL);

        gst_buffer_unref(buf);
    }
    else if (tagtype == G_TYPE_STRING)
    {
        gst_tag_list_add(reader->taglist, GST_TAG_MERGE_REPLACE, tag->gst_tag, data, NULL);
    }
    else
    {
        GST_WARNING("No parsing function associated to %x(%s)", tag->exif_tag, tag->gst_tag);
...
static gboolean exif_reader_read_rational_tag(GstExifReader * exif_reader, guint32 count, guint32 offset, gboolean is_signed, gint32 * _frac_n, gint32 * _frac_d)
{
    ...
    {
        GST_WARNING("Offset is smaller (%u) than base offset (%u)", offset, exif_reader->base_offset);
        return FALSE;
    }
    ...
    {
        GST_WARNING("Invalid offset %u for buffer of size %u", real_offset, GST_BUFFER_SIZE(exif_reader->buffer));
        return FALSE;
    }
    ...
    if (!is_signed)
    {
        ...
        if (exif_reader->byte_order == G_LITTLE_ENDIAN)
        {
            if (!gst_byte_reader_get_uint32_le(&data_reader, &aux_n) || !gst_byte_reader_get_uint32_le(&data_reader, &aux_d))
                goto reader_fail;
        }
        else
        {
            if (!gst_byte_reader_get_uint32_be(&data_reader, &aux_n) || !gst_byte_reader_get_uint32_be(&data_reader, &aux_d))
                goto reader_fail;
        }
        frac_n = (gint32) aux_n;
        frac_d = (gint32) aux_d;
    ...
    else
    {
        if (exif_reader->byte_order == G_LITTLE_ENDIAN)
        {
            if (!gst_byte_reader_get_int32_le(&data_reader, &frac_n) || !gst_byte_reader_get_int32_le(&data_reader, &frac_d))
                goto reader_fail;
        }
        else
        {
            if (!gst_byte_reader_get_int32_be(&data_reader, &frac_n) || !gst_byte_reader_get_int32_be(&data_reader, &frac_d))
                goto reader_fail;
        }
    }
...
static void parse_exif_rational_tag(GstExifReader * exif_reader, const gchar * gst_tag, guint32 count, guint32 offset, gdouble multiplier, gboolean is_signed)
{
    GType type;
    gint32 frac_n = 0;
    gint32 frac_d = 1;
    ...
    if (!exif_reader_read_rational_tag(exif_reader, count, offset, is_signed, &frac_n, &frac_d))
        return;
    ...
    switch (type)
    {
        case G_TYPE_DOUBLE:
            ...
            gst_tag_list_add(exif_reader->taglist, GST_TAG_MERGE_REPLACE, gst_tag, value, NULL);
            break;
        default:
            if (type == GST_TYPE_FRACTION)
            {
                ...
                gst_tag_list_add_value(exif_reader->taglist, GST_TAG_MERGE_REPLACE, gst_tag, &fraction);
...
static GstBuffer *write_exif_ifd(const GstTagList * taglist, gboolean byte_order, guint32 base_offset, const GstExifTagMatch * tag_map)
{
    GstExifWriter writer;
    gint i;

    GST_DEBUG("Formatting taglist %p as exif buffer. Byte order: %d, " "base_offset: %u", taglist, byte_order, base_offset);
    ...
    for (i = 0; tag_map[i].exif_tag != 0; i++)
    {
        ...
        if (tag_map[i].gst_tag == NULL)
        {
            ...
            else if (tag_map[i].exif_tag == EXIF_VERSION_TAG)
            {
                ...
                write_exif_undefined_tag(&writer, EXIF_VERSION_TAG, (guint8 *) "0230", 4);
            }
            else if (tag_map[i].exif_tag == EXIF_FLASHPIX_VERSION_TAG)
            {
                /* special case where we write the flashpix version */
                write_exif_undefined_tag(&writer, EXIF_FLASHPIX_VERSION_TAG, (guint8 *) "0100", 4);
            }

            if (inner_tag_map)
            {
                /* base offset and tagheader size are added when rewriting offset */
                inner_ifd = write_exif_ifd(taglist, byte_order, gst_byte_writer_get_size(&writer.datawriter), inner_tag_map);
            }

            if (inner_ifd)
            {
                GST_DEBUG("Adding inner ifd: %x", tag_map[i].exif_tag);
                gst_exif_writer_write_tag_header(&writer, tag_map[i].exif_tag, EXIF_TYPE_LONG, 1, gst_byte_writer_get_size(&writer.datawriter), FALSE);
                gst_byte_writer_put_data(&writer.datawriter, GST_BUFFER_DATA(inner_ifd), GST_BUFFER_SIZE(inner_ifd));
    ...
    gst_exif_tag_rewrite_offsets(&writer.tagwriter, writer.byte_order, base_offset + gst_byte_writer_get_size(&writer.tagwriter), writer.tags_total, &writer.datawriter);

    return gst_exif_writer_reset_and_get_buffer(&writer);
}

static gboolean parse_exif_tag_header(GstByteReader * reader, gint byte_order, GstExifTagData * _tagdata)
{
    g_assert(_tagdata);
    ...
    if (byte_order == G_LITTLE_ENDIAN)
    {
        if (!gst_byte_reader_get_uint16_le(reader, &_tagdata->tag) ||
            !gst_byte_reader_get_uint16_le(reader, &_tagdata->tag_type) || !gst_byte_reader_get_uint32_le(reader, &_tagdata->count) || !gst_byte_reader_get_data(reader, 4, &_tagdata->offset_as_data))
        {
            return FALSE;
        }
    ...
    else
    {
        if (!gst_byte_reader_get_uint16_be(reader, &_tagdata->tag) ||
            !gst_byte_reader_get_uint16_be(reader, &_tagdata->tag_type) || !gst_byte_reader_get_uint32_be(reader, &_tagdata->count) || !gst_byte_reader_get_data(reader, 4, &_tagdata->offset_as_data))
        {
            return FALSE;
        }
...
static gboolean parse_exif_ifd(GstExifReader * exif_reader, gint buf_offset, const GstExifTagMatch * tag_map)
{
    GstByteReader reader;
    guint16 entries = 0;
    guint16 i;

    g_return_val_if_fail(exif_reader->byte_order == G_LITTLE_ENDIAN || exif_reader->byte_order == G_BIG_ENDIAN, FALSE);
    ...
    for (i = 0; i < entries; i++)
    {
        ...
        GST_DEBUG("Parsed tag: id 0x%x, type %u, count %u, offset %u (0x%x)" ", buf size: %u", tagdata.tag, tagdata.tag_type, tagdata.count, tagdata.offset, tagdata.offset, gst_byte_reader_get_size(&reader));
        ...
        {
            parse_exif_ifd(exif_reader, tagdata.offset - exif_reader->base_offset, tag_map_gps);

            continue;
        }
        if (tagdata.tag == EXIF_IFD_TAG)
        {
            parse_exif_ifd(exif_reader, tagdata.offset - exif_reader->base_offset, tag_map_exif);

            continue;
        }
        if (tagdata.tag == EXIF_VERSION_TAG || tagdata.tag == EXIF_FLASHPIX_VERSION_TAG)
        {
            /* skip */
            continue;
        }
        ...
        {
            i += tag_map[map_index].deserialize(exif_reader, &reader, &tag_map[map_index], &tagdata);
            continue;
        }
        ...
        {
            case EXIF_TYPE_ASCII:
                parse_exif_ascii_tag(exif_reader, &tag_map[map_index], tagdata.count, tagdata.offset, tagdata.offset_as_data);
                break;
            case EXIF_TYPE_RATIONAL:
                parse_exif_rational_tag(exif_reader, tag_map[map_index].gst_tag, tagdata.count, tagdata.offset, 1, FALSE);
                break;
            case EXIF_TYPE_UNDEFINED:
                parse_exif_undefined_tag(exif_reader, &tag_map[map_index], tagdata.count, tagdata.offset, tagdata.offset_as_data);
            case EXIF_TYPE_LONG:
                parse_exif_long_tag(exif_reader, &tag_map[map_index], tagdata.count, tagdata.offset, tagdata.offset_as_data);
                break;
            default:
    ...
    /* check if the pending tags have something that can still be added */
    {
        ...
        for (walker = exif_reader->pending_tags; walker; walker = g_slist_next(walker))
        {
            data = (GstExifTagData *) walker->data;
            switch (data->tag)
            {
                case EXIF_TAG_XRESOLUTION:
                    parse_exif_rational_tag(exif_reader, GST_TAG_IMAGE_HORIZONTAL_PPI, data->count, data->offset, 1, FALSE);
                    break;
                case EXIF_TAG_YRESOLUTION:
                    parse_exif_rational_tag(exif_reader, GST_TAG_IMAGE_VERTICAL_PPI, data->count, data->offset, 1, FALSE);
                    break;
                default:
                    /* NOP */
                    break;
 ...
 * Since: 0.10.30
 */
GstBuffer *gst_tag_list_to_exif_buffer(const GstTagList * taglist, gint byte_order, guint32 base_offset)
{
...
GstBuffer *gst_tag_list_to_exif_buffer_with_tiff_header(const GstTagList * taglist)
{
    ...
    if (!gst_byte_writer_put_data(&writer, GST_BUFFER_DATA(ifd), GST_BUFFER_SIZE(ifd)))
    {
 ...
 * Since: 0.10.30
 */
GstTagList *gst_tag_list_from_exif_buffer(const GstBuffer * buffer, gint byte_order, guint32 base_offset)
{
 ...
 * Since: 0.10.30
 */
GstTagList *gst_tag_list_from_exif_buffer_with_tiff_header(const GstBuffer * buffer)
{
    GstByteReader reader;
    guint16 fortytwo = 42;
    ...
    GST_LOG("Parsing exif tags with tiff header of size %u", GST_BUFFER_SIZE(buffer));
    ...
    {
        if (!gst_byte_reader_get_uint16_le(&reader, &fortytwo) || !gst_byte_reader_get_uint32_le(&reader, &offset))
            goto byte_reader_fail;
    }
    else if (endianness == TIFF_BIG_ENDIAN)
    {
        if (!gst_byte_reader_get_uint16_be(&reader, &fortytwo) || !gst_byte_reader_get_uint32_be(&reader, &offset))
            goto byte_reader_fail;
    }
    else
    {
    ...
    subbuffer = gst_buffer_new_and_alloc(GST_BUFFER_SIZE(buffer) - (TIFF_HEADER_SIZE - 2));
    memcpy(GST_BUFFER_DATA(subbuffer), GST_BUFFER_DATA(buffer) + TIFF_HEADER_SIZE, GST_BUFFER_SIZE(buffer) - TIFF_HEADER_SIZE);

    taglist = gst_tag_list_from_exif_buffer(subbuffer, endianness == TIFF_LITTLE_ENDIAN ? G_LITTLE_ENDIAN : G_BIG_ENDIAN, 8);
...
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC(contrast, capturing_contrast);
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC(exposure_mode, capturing_exposure_mode);
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC(exposure_program, capturing_exposure_program);
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC(gain_control, capturing_gain_adjustment);
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC(metering_mode, capturing_metering_mode);
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC(orientation, image_orientation);
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC(saturation, capturing_saturation);
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC(scene_capture_type, capturing_scene_capture_type);
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC(sharpness, capturing_sharpness);
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC(source, capturing_source);
EXIF_SERIALIZATION_DESERIALIZATION_MAP_STRING_TO_INT_FUNC(white_balance, capturing_white_balance);

static void serialize_geo_coordinate(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
    gboolean latitude;
    gdouble value;
    gint degrees;
    ...
    {
        GST_WARNING("Failed to get double from tag list for tag: %s", exiftag->gst_tag);
        return;
    }
    ...
    GST_LOG("Converted geo location to %d.%d'%d'' degrees", degrees, minutes, seconds);

    offset = gst_byte_writer_get_size(&writer->datawriter);
    gst_exif_writer_write_tag_header(writer, exiftag->exif_tag, EXIF_TYPE_RATIONAL, 3, offset, FALSE);
...
static gint deserialize_geo_coordinate(GstExifReader * exif_reader, GstByteReader * reader, const GstExifTagMatch * exiftag, GstExifTagData * tagdata)
{
    ...
    GST_LOG("Starting to parse %s tag in exif 0x%x", exiftag->gst_tag, exiftag->exif_tag);

    if (exiftag->complementary_tag != tagdata->tag)
    {
        /* First should come the 'Ref' tags */
        GST_WARNING("Tag %d is not the 'Ref' tag for latitude nor longitude", tagdata->tag);
        return ret;
    }
    ...
    else if (tagdata->offset_as_data[0] == 'S' || tagdata->offset_as_data[0] == 'W')
    {
        multiplier = -1;
    }
    else
    {
        GST_WARNING("Invalid LatitudeRef or LongitudeRef %c", tagdata->offset_as_data[0]);
        return ret;
    }
    ...
    {
        GST_WARNING("Invalid type %d for geo coordinate (latitude/longitude)", next_tagdata.tag_type);
        return ret;
    }
    if (next_tagdata.count != 3)
    {
        GST_WARNING("Geo coordinate should use 3 fractions, we have %u", next_tagdata.count);
        return ret;
    }

    /* now parse the fractions */
    gst_byte_reader_init_from_buffer(&fractions_reader, exif_reader->buffer);
    if (!gst_byte_reader_set_pos(&fractions_reader, next_tagdata.offset - exif_reader->base_offset))
        goto reader_fail;
    ...
    if (exif_reader->byte_order == G_LITTLE_ENDIAN)
    {
        if (!gst_byte_reader_get_uint32_le(&fractions_reader, &degrees_n) ||
            ...
            !gst_byte_reader_get_uint32_le(&fractions_reader, &minutes_d) || !gst_byte_reader_get_uint32_le(&fractions_reader, &seconds_n) || !gst_byte_reader_get_uint32_le(&fractions_reader, &seconds_d))
            goto reader_fail;
    }
    else
    {
        if (!gst_byte_reader_get_uint32_be(&fractions_reader, &degrees_n) ||
            ...
            !gst_byte_reader_get_uint32_be(&fractions_reader, &minutes_d) || !gst_byte_reader_get_uint32_be(&fractions_reader, &seconds_n) || !gst_byte_reader_get_uint32_be(&fractions_reader, &seconds_d))
            goto reader_fail;
    }

    GST_DEBUG("Read degrees fraction for tag %s: %u/%u %u/%u %u/%u", exiftag->gst_tag, degrees_n, degrees_d, minutes_n, minutes_d, seconds_n, seconds_d);
    ...
    gst_tag_list_add(exif_reader->taglist, GST_TAG_MERGE_REPLACE, exiftag->gst_tag, degrees, NULL);
...
static void serialize_geo_direction(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
    gdouble value;
    ...
    gst_exif_writer_write_rational_tag_from_double(writer, exiftag->exif_tag, value);
}

static gint deserialize_geo_direction(GstExifReader * exif_reader, GstByteReader * reader, const GstExifTagMatch * exiftag, GstExifTagData * tagdata)
{
    ...
    if (exiftag->complementary_tag == tagdata->tag)
    {
        ...
        {
            GST_WARNING("Invalid Ref for direction or track %c", tagdata->offset_as_data[0]);
            return ret;
        }
    }
    else
    ...
    if (next_tagdata.tag_type != EXIF_TYPE_RATIONAL)
    {
        GST_WARNING("Invalid type %d for 0x%x", next_tagdata.tag_type, next_tagdata.tag);
        return ret;
    }
    if (next_tagdata.count != 1)
    {
        GST_WARNING("0x%x tag must have a single fraction, we have %u", next_tagdata.tag_type, next_tagdata.count);
        return ret;
    }

    parse_exif_rational_tag(exif_reader, exiftag->gst_tag, next_tagdata.count, next_tagdata.offset, 1, FALSE);
...
static void serialize_geo_elevation(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
    gdouble value;
    ...
    /* first write the Ref */
    gst_exif_writer_write_byte_tag(writer, exiftag->complementary_tag, value >= 0 ? 0 : 1);
...
static gint deserialize_geo_elevation(GstExifReader * exif_reader, GstByteReader * reader, const GstExifTagMatch * exiftag, GstExifTagData * tagdata)
{
    GstExifTagData next_tagdata = { 0, };
    gint multiplier = 1;
    gint ret = 0;
    ...
    parse_exif_rational_tag(exif_reader, exiftag->gst_tag, next_tagdata.count, next_tagdata.offset, multiplier, FALSE);
...
static void serialize_speed(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
    gdouble value;
    ...
    /* now the value */
    gst_exif_writer_write_rational_tag_from_double(writer, exiftag->exif_tag, value * METERS_PER_SECOND_TO_KILOMETERS_PER_HOUR);
}

static gint deserialize_speed(GstExifReader * exif_reader, GstByteReader * reader, const GstExifTagMatch * exiftag, GstExifTagData * tagdata)
{
    GstExifTagData next_tagdata = { 0, };
...
static void serialize_shutter_speed(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
    ...
    gst_util_fraction_to_double(gst_value_get_fraction_numerator(value), gst_value_get_fraction_denominator(value), &num);
    ...
    /* now the value */
    gst_exif_writer_write_signed_rational_tag_from_double(writer, exiftag->exif_tag, num);
}

static gint deserialize_shutter_speed(GstExifReader * exif_reader, GstByteReader * reader, const GstExifTagMatch * exiftag, GstExifTagData * tagdata)
{
    ...
    if (!exif_reader_read_rational_tag(exif_reader, tagdata->count, tagdata->offset, TRUE, &frac_n, &frac_d))
        return 0;
    ...
    gst_tag_list_add_value(exif_reader->taglist, GST_TAG_MERGE_KEEP, exiftag->gst_tag, &value);
    g_value_unset(&value);

    return 0;
}

static void serialize_aperture_value(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
    gdouble num;
...
static gint deserialize_aperture_value(GstExifReader * exif_reader, GstByteReader * reader, const GstExifTagMatch * exiftag, GstExifTagData * tagdata)
{
    gint32 frac_n, frac_d;
    gdouble d;
    ...
    if (!exif_reader_read_rational_tag(exif_reader, tagdata->count, tagdata->offset, FALSE, &frac_n, &frac_d))
        return 0;

    gst_util_fraction_to_double(frac_n, frac_d, &d);
    d = pow(2, d / 2);

    gst_tag_list_add(exif_reader->taglist, GST_TAG_MERGE_KEEP, exiftag->gst_tag, d, NULL);

    return 0;
}

static void serialize_sensitivity_type(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
    /* we only support ISOSpeed as the sensitivity type (3) */
    gst_exif_writer_write_short_tag(writer, exiftag->exif_tag, 3);
}

static gint deserialize_sensitivity_type(GstExifReader * exif_reader, GstByteReader * reader, const GstExifTagMatch * exiftag, GstExifTagData * tagdata)
{
    ...
    sensitivity = gst_exif_reader_get_pending_tag(exif_reader, EXIF_TAG_PHOTOGRAPHIC_SENSITIVITY);
    if (sensitivity == NULL)
    {
    ...
    gst_tag_list_add(exif_reader->taglist, GST_TAG_MERGE_KEEP, GST_TAG_CAPTURING_ISO_SPEED, sensitivity->offset_as_data, NULL);

    return 0;
}

static void serialize_flash(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
    gboolean flash_fired;
    const gchar *flash_mode;
    guint16 tagvalue = 0;

    if (!gst_tag_list_get_boolean_index(taglist, exiftag->gst_tag, 0, &flash_fired))
    {
    ...
    if (gst_tag_list_peek_string_index(taglist, GST_TAG_CAPTURING_FLASH_MODE, 0, &flash_mode))
    {
        guint16 mode = 0;
...
static gint deserialize_flash(GstExifReader * exif_reader, GstByteReader * reader, const GstExifTagMatch * exiftag, GstExifTagData * tagdata)
{
    guint16 value = 0;
    guint mode = 0;
    ...
    if (value & 0x1)
    {
        gst_tag_list_add(exif_reader->taglist, GST_TAG_MERGE_REPLACE, GST_TAG_CAPTURING_FLASH_FIRED, TRUE, NULL);
    }
    else
    {
        gst_tag_list_add(exif_reader->taglist, GST_TAG_MERGE_REPLACE, GST_TAG_CAPTURING_FLASH_FIRED, FALSE, NULL);
    }
    ...
    if (mode_str)
        gst_tag_list_add(exif_reader->taglist, GST_TAG_MERGE_REPLACE, GST_TAG_CAPTURING_FLASH_MODE, mode_str, NULL);

    return 0;
}

static gint deserialize_resolution(GstExifReader * exif_reader, GstByteReader * reader, const GstExifTagMatch * exiftag, GstExifTagData * tagdata)
{
    ...
    if (xres)
    {
        parse_exif_rational_tag(exif_reader, GST_TAG_IMAGE_HORIZONTAL_PPI, xres->count, xres->offset, multiplier, FALSE);
    }
    if (yres)
    {
        parse_exif_rational_tag(exif_reader, GST_TAG_IMAGE_VERTICAL_PPI, yres->count, yres->offset, multiplier, FALSE);
    }

    return 0;
}

static void serialize_scene_type(GstExifWriter * writer, const GstTagList * taglist, const GstExifTagMatch * exiftag)
{
    const gchar *str;
    guint8 value = 0;

    if (gst_tag_list_peek_string_index(taglist, GST_TAG_CAPTURING_SOURCE, 0, &str))
    {
        if (strcmp(str, "dsc") == 0)
        {
            value = 0;
...
static gint deserialize_scene_type(GstExifReader * exif_reader, GstByteReader * reader, const GstExifTagMatch * exiftag, GstExifTagData * tagdata)
{
    guint8 value = 0;
    ...
    if (value == 1)
    {
        gst_tag_list_add(exif_reader->taglist, GST_TAG_MERGE_KEEP, GST_TAG_CAPTURING_SOURCE, "dsc", NULL);
    }

    return 0;
}

static gint deserialize_add_to_pending_tags(GstExifReader * exif_reader, GstByteReader * reader, const GstExifTagMatch * exiftag, GstExifTagData * tagdata)
{
    GST_LOG("Adding %s tag in exif 0x%x to pending tags", exiftag->gst_tag, exiftag->exif_tag);
