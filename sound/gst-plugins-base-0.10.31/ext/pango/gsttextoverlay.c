...
static GstStaticPadTemplate src_template_factory = GST_STATIC_PAD_TEMPLATE("src",
                                                                           GST_PAD_SRC,
                                                                           GST_PAD_ALWAYS,
                                                                           GST_STATIC_CAPS(GST_VIDEO_CAPS_BGRx ";" GST_VIDEO_CAPS_xRGB ";" GST_VIDEO_CAPS_YUV("{AYUV, I420, UYVY, NV12, NV21}")));

static GstStaticPadTemplate video_sink_template_factory = GST_STATIC_PAD_TEMPLATE("video_sink",
                                                                                  GST_PAD_SINK,
                                                                                  GST_PAD_ALWAYS,
                                                                                  GST_STATIC_CAPS(GST_VIDEO_CAPS_BGRx ";" GST_VIDEO_CAPS_xRGB ";" GST_VIDEO_CAPS_YUV("{AYUV, I420, UYVY, NV12, NV21}")));

static GstStaticPadTemplate text_sink_template_factory = GST_STATIC_PAD_TEMPLATE("text_sink",
                                                                                 GST_PAD_SINK,
                                                                                 GST_PAD_ALWAYS,
                                                                                 GST_STATIC_CAPS("text/x-pango-markup; text/plain"));

#define GST_TYPE_TEXT_OVERLAY_VALIGN (gst_text_overlay_valign_get_type())
static GType gst_text_overlay_valign_get_type(void)
{
    ...
    {
        text_overlay_valign_type = g_enum_register_static("GstTextOverlayVAlign", text_overlay_valign);
    }
    return text_overlay_valign_type;
}

#define GST_TYPE_TEXT_OVERLAY_HALIGN (gst_text_overlay_halign_get_type())
static GType gst_text_overlay_halign_get_type(void)
{
    ...
    {
        text_overlay_halign_type = g_enum_register_static("GstTextOverlayHAlign", text_overlay_halign);
    }
    return text_overlay_halign_type;
}

#define GST_TYPE_TEXT_OVERLAY_WRAP_MODE (gst_text_overlay_wrap_mode_get_type())
static GType gst_text_overlay_wrap_mode_get_type(void)
{
    ...
    {
        text_overlay_wrap_mode_type = g_enum_register_static("GstTextOverlayWrapMode", text_overlay_wrap_mode);
    }
    return text_overlay_wrap_mode_type;
}

#define GST_TYPE_TEXT_OVERLAY_LINE_ALIGN (gst_text_overlay_line_align_get_type())
static GType gst_text_overlay_line_align_get_type(void)
{
    ...
    {
        text_overlay_line_align_type = g_enum_register_static("GstTextOverlayLineAlign", text_overlay_line_align);
    }
...
static GstFlowReturn gst_text_overlay_video_chain(GstPad * pad, GstBuffer * buffer);
static GstFlowReturn gst_text_overlay_video_bufferalloc(GstPad * pad, guint64 offset, guint size, GstCaps * caps, GstBuffer ** buffer);

static gboolean gst_text_overlay_text_event(GstPad * pad, GstEvent * event);
static GstFlowReturn gst_text_overlay_text_chain(GstPad * pad, GstBuffer * buffer);
static GstPadLinkReturn gst_text_overlay_text_pad_link(GstPad * pad, GstPad * peer);
...
static void gst_text_overlay_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_text_overlay_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
static void gst_text_overlay_adjust_values_with_fontdesc(GstTextOverlay * overlay, PangoFontDescription * desc);
...
static void gst_text_overlay_base_init(gpointer g_class)
{
    ...
    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&video_sink_template_factory));

    /* ugh */
    if (!GST_IS_TIME_OVERLAY_CLASS(g_class) && !GST_IS_CLOCK_OVERLAY_CLASS(g_class))
    {
        gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&text_sink_template_factory));
    }

    gst_element_class_set_details_simple(element_class, "Text overlay", "Filter/Editor/Video", "Adds text strings on top of a video buffer", "David Schleef <ds@schleef.org>, " "Zeeshan Ali <zeeshan.ali@nokia.com>");
}

static gchar *gst_text_overlay_get_text(GstTextOverlay * overlay, GstBuffer * video_frame)
{
    return g_strdup(overlay->default_text);
}

static void gst_text_overlay_class_init(GstTextOverlayClass * klass)
{
    ...
    klass->pango_context = pango_cairo_font_map_create_context(PANGO_CAIRO_FONT_MAP(fontmap));

    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_TEXT, g_param_spec_string("text", "text", "Text to be display.", DEFAULT_PROP_TEXT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_SHADING,
                                    g_param_spec_boolean("shaded-background", "shaded background", "Whether to shade the background under the text area", DEFAULT_PROP_SHADING, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_VALIGNMENT,
                                    g_param_spec_enum("valignment", "vertical alignment", "Vertical alignment of the text", GST_TYPE_TEXT_OVERLAY_VALIGN, DEFAULT_PROP_VALIGNMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_HALIGNMENT,
                                    g_param_spec_enum("halignment", "horizontal alignment", "Horizontal alignment of the text", GST_TYPE_TEXT_OVERLAY_HALIGN, DEFAULT_PROP_HALIGNMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_VALIGN,
                                    g_param_spec_string("valign", "vertical alignment", "Vertical alignment of the text (deprecated; use valignment)", DEFAULT_PROP_VALIGN, G_PARAM_WRITABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_HALIGN,
                                    g_param_spec_string("halign", "horizontal alignment", "Horizontal alignment of the text (deprecated; use halignment)", DEFAULT_PROP_HALIGN, G_PARAM_WRITABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_XPAD, g_param_spec_int("xpad", "horizontal paddding", "Horizontal paddding when using left/right alignment", 0, G_MAXINT, DEFAULT_PROP_XPAD, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_YPAD, g_param_spec_int("ypad", "vertical padding", "Vertical padding when using top/bottom alignment", 0, G_MAXINT, DEFAULT_PROP_YPAD, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_DELTAX,
                                    g_param_spec_int("deltax", "X position modifier", "Shift X position to the left or to the right. Unit is pixels.", G_MININT, G_MAXINT, DEFAULT_PROP_DELTAX, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_DELTAY,
                                    g_param_spec_int("deltay", "Y position modifier", "Shift Y position up or down. Unit is pixels.", G_MININT, G_MAXINT, DEFAULT_PROP_DELTAY, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   * GstTextOverlay:xpos
   *
   ...
   * Since: 0.10.31
   **/
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_XPOS, g_param_spec_double("xpos", "horizontal position", "Horizontal position when using position alignment", 0, 1.0, DEFAULT_PROP_XPOS, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   * GstTextOverlay:ypos
   *
   ...
   * Since: 0.10.31
   **/
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_YPOS, g_param_spec_double("ypos", "vertical position", "Vertical position when using position alignment", 0, 1.0, DEFAULT_PROP_YPOS, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_WRAP_MODE,
                                    g_param_spec_enum("wrap-mode", "wrap mode", "Whether to wrap the text and if so how.", GST_TYPE_TEXT_OVERLAY_WRAP_MODE, DEFAULT_PROP_WRAP_MODE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_FONT_DESC,
                                    g_param_spec_string("font-desc", "font description",
                                                        "Pango font description of font to be used for rendering. "
                                                        "See documentation of pango_font_description_from_string " "for syntax.", DEFAULT_PROP_FONT_DESC, G_PARAM_WRITABLE | G_PARAM_STATIC_STRINGS));
  /**
   ...
   * Since: 0.10.31
   **/
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_COLOR, g_param_spec_uint("color", "Color", "Color to use for text (big-endian ARGB).", 0, G_MAXUINT32, DEFAULT_PROP_COLOR, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   **/
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_LINE_ALIGNMENT,
                                    g_param_spec_enum("line-alignment", "line alignment", "Alignment of text lines relative to each other.", GST_TYPE_TEXT_OVERLAY_LINE_ALIGN, DEFAULT_PROP_LINE_ALIGNMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   ...
   **/
    ...
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_SILENT, g_param_spec_boolean("silent", "silent", "Whether to render the text string", DEFAULT_PROP_SILENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   ...
   * Since: 0.10.20
   **/
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_WAIT_TEXT, g_param_spec_boolean("wait-text", "Wait Text", "Whether to wait for subtitles", DEFAULT_PROP_WAIT_TEXT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(G_OBJECT_CLASS(klass),
                                    PROP_AUTO_ADJUST_SIZE, g_param_spec_boolean("auto-resize", "auto resize", "Automatically adjust font size to screen-size.", DEFAULT_PROP_AUTO_ADJUST_SIZE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_VERTICAL_RENDER, g_param_spec_boolean("vertical-render", "vertical render", "Vertical Render.", DEFAULT_PROP_VERTICAL_RENDER, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
static void gst_text_overlay_init(GstTextOverlay * overlay, GstTextOverlayClass * klass)
{
    ...
    gst_pad_set_getcaps_function(overlay->video_sinkpad, GST_DEBUG_FUNCPTR(gst_text_overlay_getcaps));
    gst_pad_set_setcaps_function(overlay->video_sinkpad, GST_DEBUG_FUNCPTR(gst_text_overlay_setcaps));
    gst_pad_set_event_function(overlay->video_sinkpad, GST_DEBUG_FUNCPTR(gst_text_overlay_video_event));
    gst_pad_set_chain_function(overlay->video_sinkpad, GST_DEBUG_FUNCPTR(gst_text_overlay_video_chain));
    gst_pad_set_bufferalloc_function(overlay->video_sinkpad, GST_DEBUG_FUNCPTR(gst_text_overlay_video_bufferalloc));
    ...
    if (!GST_IS_TIME_OVERLAY_CLASS(klass) && !GST_IS_CLOCK_OVERLAY_CLASS(klass))
    {
        ...
        gst_object_unref(template);
        gst_pad_set_setcaps_function(overlay->text_sinkpad, GST_DEBUG_FUNCPTR(gst_text_overlay_setcaps_txt));
        gst_pad_set_event_function(overlay->text_sinkpad, GST_DEBUG_FUNCPTR(gst_text_overlay_text_event));
        gst_pad_set_chain_function(overlay->text_sinkpad, GST_DEBUG_FUNCPTR(gst_text_overlay_text_chain));
        gst_pad_set_link_function(overlay->text_sinkpad, GST_DEBUG_FUNCPTR(gst_text_overlay_text_pad_link));
        gst_pad_set_unlink_function(overlay->text_sinkpad, GST_DEBUG_FUNCPTR(gst_text_overlay_text_pad_unlink));
    ...
    gst_object_unref(template);
    gst_pad_set_getcaps_function(overlay->srcpad, GST_DEBUG_FUNCPTR(gst_text_overlay_getcaps));
    gst_pad_set_event_function(overlay->srcpad, GST_DEBUG_FUNCPTR(gst_text_overlay_src_event));
    gst_pad_set_query_function(overlay->srcpad, GST_DEBUG_FUNCPTR(gst_text_overlay_src_query));
    gst_element_add_pad(GST_ELEMENT(overlay), overlay->srcpad);

    overlay->line_align = DEFAULT_PROP_LINE_ALIGNMENT;
    overlay->layout = pango_layout_new(GST_TEXT_OVERLAY_GET_CLASS(overlay)->pango_context);
    desc = pango_context_get_font_description(GST_TEXT_OVERLAY_GET_CLASS(overlay)->pango_context);
...
static void gst_text_overlay_update_wrap_mode(GstTextOverlay * overlay)
{
    ...
    else
    {
        ...
        {
            width = (overlay->use_vertical_render ? overlay->height : overlay->width) * PANGO_SCALE;
        }
...
static gboolean gst_text_overlay_setcaps_txt(GstPad * pad, GstCaps * caps)
{
    ...
    overlay->have_pango_markup = gst_structure_has_name(structure, "text/x-pango-markup");
...
static gboolean gst_text_overlay_setcaps(GstPad * pad, GstCaps * caps)
{
    ...
    if (fps && gst_video_format_parse_caps(caps, &overlay->format, &overlay->width, &overlay->height))
    {
...
static void gst_text_overlay_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_HALIGN:
            {
                ...
                if (s && g_ascii_strcasecmp(s, "left") == 0)
                    ...
                    g_warning("Invalid value '%s' for textoverlay property 'halign'", GST_STR_NULL(s));
                break;
            }
        case PROP_VALIGN:
            {
                ...
                if (s && g_ascii_strcasecmp(s, "baseline") == 0)
                    ...
                    g_warning("Invalid value '%s' for textoverlay property 'valign'", GST_STR_NULL(s));
                break;
            }
        ...
        case PROP_FONT_DESC:
            {
                ...
                {
                    GST_WARNING_OBJECT(overlay, "font description parse failed: %s", fontdesc_str);
                }
                break;
            }
        case PROP_COLOR:
        ...
        case PROP_LINE_ALIGNMENT:
            ...
            pango_layout_set_alignment(overlay->layout, (PangoAlignment) overlay->line_align);
            break;
        case PROP_WAIT_TEXT:
...
static inline void gst_text_overlay_blit_1(GstTextOverlay * overlay, guchar * dest, gint xpos, gint ypos, guchar * text_image, guint dest_stride)
{
    gint i, j = 0;
    gint x, y;
    guchar r, g, b, a;
...
static inline void gst_text_overlay_blit_sub2x2cbcr(GstTextOverlay * overlay, guchar * destcb, guchar * destcr, gint xpos, gint ypos, guchar * text_image, guint destcb_stride, guint destcr_stride, guint pix_stride)
{
    gint i, j;
    gint x, cb, cr;
    gushort r, g, b, a;
...
static void gst_text_overlay_render_pangocairo(GstTextOverlay * overlay, const gchar * string, gint textlen)
{
    cairo_t *cr;
    ...
    if (width + overlay->deltax > (overlay->use_vertical_render ? overlay->height : overlay->width))
    {
        /* 
    ...
    height = (logical_rect.height + logical_rect.y + overlay->shadow_offset) * scalef;
    ...
    surface = cairo_image_surface_create_for_data(overlay->text_image, CAIRO_FORMAT_ARGB32, width, height, width * 4);
...
static inline void gst_text_overlay_shade_planar_Y(GstTextOverlay * overlay, guchar * dest, gint x0, gint x1, gint y0, gint y1)
{
    gint i, j, dest_stride;

    dest_stride = gst_video_format_get_row_stride(overlay->format, 0, overlay->width);
...
static inline void gst_text_overlay_shade_packed_Y(GstTextOverlay * overlay, guchar * dest, gint x0, gint x1, gint y0, gint y1)
{
    gint i, j;
    ...
    component_offset = gst_video_format_get_component_offset(overlay->format, 0, overlay->width, overlay->height);
...
static inline void gst_text_overlay_shade_xRGB(GstTextOverlay * overlay, guchar * dest, gint x0, gint x1, gint y0, gint y1)
{
    gint i, j;
...
static inline void gst_text_overlay_blit_NV12_NV21(GstTextOverlay * overlay, guint8 * yuv_pixels, gint xpos, gint ypos)
{
    ...
    gst_text_overlay_blit_1(overlay, yuv_pixels, xpos, ypos, overlay->text_image, y_stride);
    gst_text_overlay_blit_sub2x2cbcr(overlay, yuv_pixels + u_offset, yuv_pixels + v_offset, xpos, ypos, overlay->text_image, uv_stride, uv_stride, 2);
}

static inline void gst_text_overlay_blit_I420(GstTextOverlay * overlay, guint8 * yuv_pixels, gint xpos, gint ypos)
{
    ...
    u_offset = gst_video_format_get_component_offset(GST_VIDEO_FORMAT_I420, 1, w, h);
    v_offset = gst_video_format_get_component_offset(GST_VIDEO_FORMAT_I420, 2, w, h);

    gst_text_overlay_blit_1(overlay, yuv_pixels, xpos, ypos, overlay->text_image, y_stride);
    gst_text_overlay_blit_sub2x2cbcr(overlay, yuv_pixels + u_offset, yuv_pixels + v_offset, xpos, ypos, overlay->text_image, u_stride, v_stride, 1);
}

static inline void gst_text_overlay_blit_UYVY(GstTextOverlay * overlay, guint8 * yuv_pixels, gint xpos, gint ypos)
{
    int a0, r0, g0, b0;
    int a1, r1, g1, b1;
    int y0, y1, u, v;
...
static inline void gst_text_overlay_blit_AYUV(GstTextOverlay * overlay, guint8 * rgb_pixels, gint xpos, gint ypos)
{
    int a, r, g, b;
    int y, u, v;
    int i, j;
...
static void gst_text_overlay_render_text(GstTextOverlay * overlay, const gchar * text, gint textlen)
{
    gchar *string;
...
static GstFlowReturn gst_text_overlay_push_frame(GstTextOverlay * overlay, GstBuffer * video_frame)
{
    gint xpos, ypos;
    gint width, height;
    ...
    if (overlay->want_shading)
    {
        switch (overlay->format)
        {
            ...
            case GST_VIDEO_FORMAT_NV21:
                gst_text_overlay_shade_planar_Y(overlay, GST_BUFFER_DATA(video_frame), xpos, xpos + overlay->image_width, ypos, ypos + overlay->image_height);
                break;
            case GST_VIDEO_FORMAT_AYUV:
            case GST_VIDEO_FORMAT_UYVY:
                gst_text_overlay_shade_packed_Y(overlay, GST_BUFFER_DATA(video_frame), xpos, xpos + overlay->image_width, ypos, ypos + overlay->image_height);
                break;
            case GST_VIDEO_FORMAT_xRGB:
                gst_text_overlay_shade_xRGB(overlay, GST_BUFFER_DATA(video_frame), xpos, xpos + overlay->image_width, ypos, ypos + overlay->image_height);
                break;
    ...
    if (overlay->text_image)
    {
        ...
        {
            case GST_VIDEO_FORMAT_I420:
                gst_text_overlay_blit_I420(overlay, GST_BUFFER_DATA(video_frame), xpos, ypos);
                break;
            case GST_VIDEO_FORMAT_NV12:
            case GST_VIDEO_FORMAT_NV21:
                gst_text_overlay_blit_NV12_NV21(overlay, GST_BUFFER_DATA(video_frame), xpos, ypos);
                break;
            case GST_VIDEO_FORMAT_UYVY:
                gst_text_overlay_blit_UYVY(overlay, GST_BUFFER_DATA(video_frame), xpos, ypos);
                break;
            case GST_VIDEO_FORMAT_AYUV:
                gst_text_overlay_blit_AYUV(overlay, GST_BUFFER_DATA(video_frame), xpos, ypos);
                break;
            case GST_VIDEO_FORMAT_BGRx:
                gst_text_overlay_blit_BGRx(overlay, GST_BUFFER_DATA(video_frame), xpos, ypos);
                break;
            case GST_VIDEO_FORMAT_xRGB:
                gst_text_overlay_blit_xRGB(overlay, GST_BUFFER_DATA(video_frame), xpos, ypos);
                break;
            default:
                g_assert_not_reached();
        }
        ...
        case GST_EVENT_NEWSEGMENT:
            {
                ...
                {
                    GST_OBJECT_LOCK(overlay);
                    gst_segment_set_newsegment_full(&overlay->text_segment, update, rate, applied_rate, GST_FORMAT_TIME, cur, stop, time);
                    GST_DEBUG_OBJECT(overlay, "TEXT SEGMENT now: %" GST_SEGMENT_FORMAT, &overlay->text_segment);
                    GST_OBJECT_UNLOCK(overlay);
                }
                else
                {
                    GST_ELEMENT_WARNING(overlay, STREAM, MUX, (NULL), ("received non-TIME newsegment event on text input"));
                }
        ...
        case GST_EVENT_NEWSEGMENT:
            {
                ...
                {
                    GST_DEBUG_OBJECT(overlay, "VIDEO SEGMENT now: %" GST_SEGMENT_FORMAT, &overlay->segment);

                    gst_segment_set_newsegment(&overlay->segment, update, rate, format, start, stop, time);
                }
                else
                {
                    GST_ELEMENT_WARNING(overlay, STREAM, MUX, (NULL), ("received non-TIME newsegment event on video input"));
                }
...
static void gst_text_overlay_pop_text(GstTextOverlay * overlay)
{
    ...
    if (overlay->text_buffer)
    {
        GST_DEBUG_OBJECT(overlay, "releasing text buffer %p", overlay->text_buffer);
        gst_buffer_unref(overlay->text_buffer);
        overlay->text_buffer = NULL;
    }
...
static GstFlowReturn gst_text_overlay_text_chain(GstPad * pad, GstBuffer * buffer)
{
    ...
    GST_LOG_OBJECT(overlay, "%" GST_SEGMENT_FORMAT "  BUFFER: ts=%" GST_TIME_FORMAT ", end=%" GST_TIME_FORMAT, &overlay->segment, GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buffer)), GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buffer) + GST_BUFFER_DURATION(buffer)));
    ...
    if (G_LIKELY(GST_BUFFER_TIMESTAMP_IS_VALID(buffer)))
    {
        ...
        in_seg = gst_segment_clip(&overlay->text_segment, GST_FORMAT_TIME, GST_BUFFER_TIMESTAMP(buffer), stop, &clip_start, &clip_stop);
    }
    else
    {
        in_seg = TRUE;
    ...
    if (in_seg)
    {
        ...
        {
            GST_DEBUG("Pad %s:%s has a buffer queued, waiting", GST_DEBUG_PAD_NAME(pad));
        ...
        if (GST_BUFFER_TIMESTAMP_IS_VALID(buffer))
            gst_segment_set_last_stop(&overlay->text_segment, GST_FORMAT_TIME, clip_start);
...
static GstFlowReturn gst_text_overlay_video_chain(GstPad * pad, GstBuffer * buffer)
{
    ...
    GST_LOG_OBJECT(overlay, "%" GST_SEGMENT_FORMAT "  BUFFER: ts=%" GST_TIME_FORMAT ", end=%" GST_TIME_FORMAT, &overlay->segment, GST_TIME_ARGS(start), GST_TIME_ARGS(stop));
    ...
    in_seg = gst_segment_clip(&overlay->segment, GST_FORMAT_TIME, start, stop, &clip_start, &clip_stop);
    ...
    if (stop == -1)
    {
        ...
        if (gst_structure_get_fraction(s, "framerate", &fps_num, &fps_denom) && fps_num && fps_denom)
        {
...
wait_for_text_buf:
    ...
    if (!overlay->text_linked)
    {
        ...
        GST_LOG_OBJECT(overlay, "Text pad not linked, rendering default " "text: '%s'", GST_STR_NULL(text));
    ...
    else
    {
        ...
        if (overlay->text_buffer)
        {
            ...
            if (!GST_BUFFER_TIMESTAMP_IS_VALID(overlay->text_buffer) || !GST_BUFFER_DURATION_IS_VALID(overlay->text_buffer))
            {
                GST_WARNING_OBJECT(overlay, "Got text buffer with invalid timestamp or duration");
                pop_text = TRUE;
            ...
            vid_running_time = gst_segment_to_running_time(&overlay->segment, GST_FORMAT_TIME, start);
            vid_running_time_end = gst_segment_to_running_time(&overlay->segment, GST_FORMAT_TIME, stop);

            /* If timestamp and duration are valid */
            if (valid_text_time)
            {
                text_running_time = gst_segment_to_running_time(&overlay->segment, GST_FORMAT_TIME, text_start);
                text_running_time_end = gst_segment_to_running_time(&overlay->segment, GST_FORMAT_TIME, text_end);
            }

            GST_LOG_OBJECT(overlay, "T: %" GST_TIME_FORMAT " - %" GST_TIME_FORMAT, GST_TIME_ARGS(text_running_time), GST_TIME_ARGS(text_running_time_end));
            GST_LOG_OBJECT(overlay, "V: %" GST_TIME_FORMAT " - %" GST_TIME_FORMAT, GST_TIME_ARGS(vid_running_time), GST_TIME_ARGS(vid_running_time_end));
        ...
        else
        {
            ...
            if (overlay->text_segment.format == GST_FORMAT_TIME)
            {
                ...
                vid_running_time = gst_segment_to_running_time(&overlay->segment, GST_FORMAT_TIME, GST_BUFFER_TIMESTAMP(buffer));
                text_start_running_time = gst_segment_to_running_time(&overlay->text_segment, GST_FORMAT_TIME, overlay->text_segment.start);
                text_last_stop_running_time = gst_segment_to_running_time(&overlay->text_segment, GST_FORMAT_TIME, overlay->text_segment.last_stop);

                if ((GST_CLOCK_TIME_IS_VALID(text_start_running_time) && vid_running_time < text_start_running_time) || (GST_CLOCK_TIME_IS_VALID(text_last_stop_running_time) && vid_running_time < text_last_stop_running_time))
                {
                    wait_for_text_buf = FALSE;
                }
            }
...
static gboolean plugin_init(GstPlugin * plugin)
{
    if (!gst_element_register(plugin, "textoverlay", GST_RANK_NONE,
        ...
        !gst_element_register(plugin, "timeoverlay", GST_RANK_NONE,
                              GST_TYPE_TIME_OVERLAY) || !gst_element_register(plugin, "clockoverlay", GST_RANK_NONE, GST_TYPE_CLOCK_OVERLAY) || !gst_element_register(plugin, "textrender", GST_RANK_NONE, GST_TYPE_TEXT_RENDER))
    {
        return FALSE;
    }
...
GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, "pango", "Pango-based text rendering and overlay", plugin_init, VERSION, GST_LICENSE, GST_PACKAGE_NAME, GST_PACKAGE_ORIGIN)
