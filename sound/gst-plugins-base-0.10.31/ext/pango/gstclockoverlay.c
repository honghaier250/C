...
GST_BOILERPLATE(GstClockOverlay, gst_clock_overlay, GstTextOverlay, GST_TYPE_TEXT_OVERLAY);

static void gst_clock_overlay_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_set_details_simple(element_class, "Clock overlay", "Filter/Editor/Video", "Overlays the current clock time on a video stream", "Tim-Philipp Müller <tim@centricular.net>");
}

static void gst_clock_overlay_finalize(GObject * object);
static void gst_clock_overlay_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_clock_overlay_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

static gchar *gst_clock_overlay_render_time(GstClockOverlay * overlay)
{
    struct tm *t;
    time_t now;
    gchar buf[256];
...
static gchar *gst_clock_overlay_get_text(GstTextOverlay * overlay, GstBuffer * video_frame)
{
...
static void gst_clock_overlay_class_init(GstClockOverlayClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_TIMEFORMAT,
                                    g_param_spec_string("time-format", "Date/Time Format", "Format to use for time and date value, as in strftime.", DEFAULT_PROP_TIMEFORMAT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
static void gst_clock_overlay_init(GstClockOverlay * overlay, GstClockOverlayClass * klass)
{
