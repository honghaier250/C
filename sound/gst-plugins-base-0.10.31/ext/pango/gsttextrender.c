...
static GstStaticPadTemplate sink_template_factory = GST_STATIC_PAD_TEMPLATE("sink",
                                                                            GST_PAD_SINK,
                                                                            GST_PAD_ALWAYS,
                                                                            GST_STATIC_CAPS("text/x-pango-markup; text/plain"));

#define GST_TYPE_TEXT_RENDER_VALIGN (gst_text_render_valign_get_type())
static GType gst_text_render_valign_get_type(void)
{
    ...
    {
        text_render_valign_type = g_enum_register_static("GstTextRenderVAlign", text_render_valign);
    }
    return text_render_valign_type;
}

#define GST_TYPE_TEXT_RENDER_HALIGN (gst_text_render_halign_get_type())
static GType gst_text_render_halign_get_type(void)
{
    ...
    {
        text_render_halign_type = g_enum_register_static("GstTextRenderHAlign", text_render_halign);
    }
    return text_render_halign_type;
}

#define GST_TYPE_TEXT_RENDER_LINE_ALIGN (gst_text_render_line_align_get_type())
static GType gst_text_render_line_align_get_type(void)
{
    ...
    {
        text_render_line_align_type = g_enum_register_static("GstTextRenderLineAlign", text_render_line_align);
    }
    return text_render_line_align_type;
}

static void gst_text_render_adjust_values_with_fontdesc(GstTextRender * render, PangoFontDescription * desc);
...
static void gst_text_render_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_text_render_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
...
static void gst_text_render_base_init(gpointer g_class)
{
    ...
    gst_element_class_set_details_simple(element_class, "Text renderer", "Filter/Editor/Video", "Renders a text string to an image bitmap", "David Schleef <ds@schleef.org>, " "GStreamer maintainers <gstreamer-devel@lists.sourceforge.net>");
}
...
static void gst_text_render_class_init(GstTextRenderClass * klass)
{
    ...
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_FONT_DESC,
                                    g_param_spec_string("font-desc", "font description",
                                                        "Pango font description of font " "to be used for rendering. " "See documentation of " "pango_font_description_from_string" " for syntax.", "", G_PARAM_WRITABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_VALIGNMENT,
                                    g_param_spec_enum("valignment", "vertical alignment", "Vertical alignment of the text", GST_TYPE_TEXT_RENDER_VALIGN, DEFAULT_PROP_VALIGNMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_HALIGNMENT,
                                    g_param_spec_enum("halignment", "horizontal alignment", "Horizontal alignment of the text", GST_TYPE_TEXT_RENDER_HALIGN, DEFAULT_PROP_HALIGNMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_LINE_ALIGNMENT,
                                    g_param_spec_enum("line-alignment", "line alignment", "Alignment of text lines relative to each other.", GST_TYPE_TEXT_RENDER_LINE_ALIGN, DEFAULT_PROP_LINE_ALIGNMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
static void gst_text_render_render_pangocairo(GstTextRender * render)
{
    cairo_t *cr;
    ...
    surface = cairo_image_surface_create_for_data(render->text_image, CAIRO_FORMAT_ARGB32, width, height, width * 4);
...
static void gst_text_render_check_argb(GstTextRender * render)
{
    ...
    if (G_LIKELY(peer_caps))
    {
        ...
        GST_DEBUG_OBJECT(render, "peer allowed caps (%u structure(s)) are %" GST_PTR_FORMAT, n, peer_caps);

        /* Check if AYUV or ARGB is first */
        for (i = 0; i < n; i++)
        {
            GstStructure *s = gst_caps_get_structure(peer_caps, i);
            if (gst_structure_has_name(s, "video/x-raw-rgb") && gst_structure_has_field(s, "alpha_mask"))
            {
            ...
            {
                guint fourcc;
                if (gst_structure_get_fourcc(s, "format", &fourcc) && fourcc == GST_MAKE_FOURCC('A', 'Y', 'U', 'V'))
                {
...
static gboolean gst_text_render_setcaps(GstPad * pad, GstCaps * caps)
{
...
static void gst_text_render_fixate_caps(GstPad * pad, GstCaps * caps)
{
    GstTextRender *render = GST_TEXT_RENDER(gst_pad_get_parent(pad));
...
static void gst_text_renderer_image_to_ayuv(GstTextRender * render, guchar * pixbuf, int xpos, int ypos, int stride)
{
    ...
    for (y = 0; y < height; y++)
    {
        ...
        for (n = 0; n < width; n++)
        {
            ...
            *p++ = a;
            *p++ = CLAMP((int) (((19595 * r) >> 16) + ((38470 * g) >> 16) + ((7471 * b) >> 16)), 0, 255);
            *p++ = CLAMP((int) (-((11059 * r) >> 16) - ((21709 * g) >> 16) + ((32768 * b) >> 16) + 128), 0, 255);
            *p++ = CLAMP((int) (((32768 * r) >> 16) - ((27439 * g) >> 16) - ((5329 * b) >> 16) + 128), 0, 255);
        }
    }
}

static void gst_text_renderer_image_to_argb(GstTextRender * render, guchar * pixbuf, int xpos, int ypos, int stride)
{
    int i, j;
    guchar *p, *bitp;
    int width, height;
...
static GstFlowReturn gst_text_render_chain(GstPad * pad, GstBuffer * inbuf)
{
    ...
    if (!render->use_ARGB)
    {
        caps = gst_video_format_new_caps(GST_VIDEO_FORMAT_AYUV, render->width, render->height, 1, 1, 1, 1);
    }
    else
    {
        caps = gst_video_format_new_caps(GST_VIDEO_FORMAT_ARGB, render->width, render->height, 1, 1, 1, 1);
    }
    ...
    ret = gst_pad_alloc_buffer_and_set_caps(render->srcpad, GST_BUFFER_OFFSET_NONE, render->width * render->height * 4, caps, &outbuf);
    ...
    if (render->text_image)
    {
        if (render->use_ARGB)
        {
            gst_text_renderer_image_to_argb(render, data, xpos, ypos, render->width * 4);
        }
        else
        {
            gst_text_renderer_image_to_ayuv(render, data, xpos, ypos, render->width * 4);
        }
    }
...
static void gst_text_render_init(GstTextRender * render, GstTextRenderClass * klass)
{
    ...
    gst_pad_set_chain_function(render->sinkpad, GST_DEBUG_FUNCPTR(gst_text_render_chain));
    ...
    gst_object_unref(template);
    gst_pad_set_fixatecaps_function(render->srcpad, GST_DEBUG_FUNCPTR(gst_text_render_fixate_caps));
    gst_pad_set_setcaps_function(render->srcpad, GST_DEBUG_FUNCPTR(gst_text_render_setcaps));

    gst_element_add_pad(GST_ELEMENT(render), render->srcpad);

    render->line_align = DEFAULT_PROP_LINE_ALIGNMENT;
    render->layout = pango_layout_new(GST_TEXT_RENDER_GET_CLASS(render)->pango_context);
    pango_layout_set_alignment(render->layout, (PangoAlignment) render->line_align);
        ...
        case PROP_FONT_DESC:
            {
                ...
                {
                    GST_WARNING("font description parse failed: %s", g_value_get_string(value));
                }
                break;
            }
