...
static GType gst_ogg_parse_get_type(void)
{
    ...
    if (!ogg_parse_type)
    {
        ...
        ogg_parse_type = g_type_register_static(GST_TYPE_ELEMENT, "GstOggParse", &ogg_parse_info, 0);
    }
    return ogg_parse_type;
}

static void free_stream(GstOggStream * stream)
{
...
static void gst_ogg_parse_delete_all_streams(GstOggParse * ogg)
{
...
static GstOggStream *gst_ogg_parse_new_stream(GstOggParse * parser, ogg_page * page)
{
    GstOggStream *stream;
    ogg_packet packet;
    int ret;
    ...
    {
        GST_ERROR("Could not initialize ogg_stream struct for serial %08x.", serialno);
        return NULL;
    }
...
static GstOggStream *gst_ogg_parse_find_stream(GstOggParse * parser, guint32 serialno)
{
    GSList *l;
...
static GstStaticPadTemplate ogg_parse_src_template_factory = GST_STATIC_PAD_TEMPLATE("src",
                                                                                     GST_PAD_SRC,
                                                                                     GST_PAD_ALWAYS,
                                                                                     GST_STATIC_CAPS("application/ogg"));

static GstStaticPadTemplate ogg_parse_sink_template_factory = GST_STATIC_PAD_TEMPLATE("sink",
                                                                                      GST_PAD_SINK,
                                                                                      GST_PAD_ALWAYS,
                                                                                      GST_STATIC_CAPS("application/ogg"));

static void gst_ogg_parse_dispose(GObject * object);
static GstStateChangeReturn gst_ogg_parse_change_state(GstElement * element, GstStateChange transition);
...
static void gst_ogg_parse_base_init(gpointer g_class)
{
    ...
    gst_element_class_set_details_simple(element_class, "Ogg parser", "Codec/Parser", "parse ogg streams into pages (info about ogg: http://xiph.org)", "Michael Smith <msmith@fluendo.com>");

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&ogg_parse_sink_template_factory));
    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&ogg_parse_src_template_factory));
}
...
static void gst_ogg_parse_init(GstOggParse * ogg)
{
    ...
    ogg->sinkpad = gst_pad_new_from_static_template(&ogg_parse_sink_template_factory, "sink");
    ogg->srcpad = gst_pad_new_from_static_template(&ogg_parse_src_template_factory, "src");
 ...
 */
static gint gst_ogg_parse_submit_buffer(GstOggParse * ogg, GstBuffer * buffer)
{
    guint size;
    guint8 *data;
    gchar *oggbuffer;
...
static void gst_ogg_parse_append_header(GValue * array, GstBuffer * buf)
{
    GValue value = { 0 };
...
static page_type gst_ogg_parse_is_header(GstOggParse * ogg, GstOggStream * stream, ogg_page * page)
{
...
static GstBuffer *gst_ogg_parse_buffer_from_page(ogg_page * page, guint64 offset, gboolean delta, GstClockTime timestamp)
{
...
static GstFlowReturn gst_ogg_parse_chain(GstPad * pad, GstBuffer * buffer)
{
    ...
    GST_LOG_OBJECT(ogg, "Chain function received buffer of size %d", GST_BUFFER_SIZE(buffer));
    ...
    while (ret != 0 && result == GST_FLOW_OK)
    {
        ...
        else
        {
            ...
            GST_LOG_OBJECT(ogg, "Timestamping outgoing buffer as %" GST_TIME_FORMAT, GST_TIME_ARGS(buffertimestamp));

            buffertimestamp = gst_ogg_stream_get_end_time_for_granulepos(stream, granule);
            pagebuffer = gst_ogg_parse_buffer_from_page(&page, startoffset, FALSE, buffertimestamp);

            /* We read out 'ret' bytes, so we set the next offset appropriately */
            ogg->offset += ret;

            GST_LOG_OBJECT(ogg,
                           "processing ogg page (serial %08x, pageno %ld, " "granule pos %" G_GUINT64_FORMAT ", bos %d, offset %" G_GUINT64_FORMAT "-%" G_GUINT64_FORMAT ")", serialno, ogg_page_pageno(&page), granule, bos, startoffset, ogg->offset);
            ...
            if (ogg_page_bos(&page))
            {
                ...
                if (stream != NULL)
                {
                    GST_LOG_OBJECT(ogg, "Incorrect stream; repeats serial number %u " "at offset %" G_GINT64_FORMAT, serialno, ogg->offset);
                }

                if (ogg->last_page_not_bos)
                {
                    GST_LOG_OBJECT(ogg, "Deleting all referenced streams, found a new " "chain starting with serial %u", serialno);
                    gst_ogg_parse_delete_all_streams(ogg);
                }
                ...
                if (!ogg->in_headers)
                {
                    GST_LOG_OBJECT(ogg, "Found start of new chain at offset %" G_GUINT64_FORMAT, startoffset);
                    ogg->in_headers = 1;
                }
            ...
            else
            {
                ...
                if (!stream)
                {
                    GST_LOG_OBJECT(ogg, "Non-BOS page unexpectedly found at %" G_GINT64_FORMAT, ogg->offset);
                    goto failure;
                }
                ...
                if (type == PAGE_PENDING && ogg->in_headers)
                {
                    ...
                    stream->unknown_pages = g_list_append(stream->unknown_pages, pagebuffer);
                }
                else if (type == PAGE_HEADER)
                {
                    if (!ogg->in_headers)
                    {
                        GST_LOG_OBJECT(ogg, "Header page unexpectedly found outside " "headers at offset %" G_GINT64_FORMAT, ogg->offset);
                        goto failure;
                    }
                    else
                    {
                         ...
                         * pages
                         */
                        stream->headers = g_list_concat(stream->headers, stream->unknown_pages);
                ...
                {                /* PAGE_DATA, or PAGE_PENDING but outside headers */
                    if (ogg->in_headers)
                    {
                        ...
                        for (l = ogg->oggstreams; l != NULL; l = l->next)
                        {
                            ...
                            {
                                GST_LOG_OBJECT(ogg, "No primary header found for stream %08lx", stream->serialno);
                                goto failure;
                            }

                            gst_ogg_parse_append_header(&array, GST_BUFFER(stream->headers->data));
                            count++;
                        }
                        ...
                        for (l = ogg->oggstreams; l != NULL; l = l->next)
                        {
                            ...
                            {
                                gst_ogg_parse_append_header(&array, GST_BUFFER(g_list_nth_data(stream->headers, j)));
                                count++;
                            }
                        }
                        ...
                        GST_LOG_OBJECT(ogg, "Set \"streamheader\" caps with %d buffers " "(one per page)", count);
                        ...
                        for (l = ogg->oggstreams; l != NULL; l = l->next)
                        {
                            ...
                            for (j = 1; j < g_list_length(stream->headers); j++)
                            {
                                GstBuffer *buf = GST_BUFFER(g_list_nth_data(stream->headers, j));
                                gst_buffer_set_caps(buf, caps);
                        ...
                        for (l = ogg->oggstreams; l != NULL; l = l->next)
                        {
                            ...
                            {
                                GST_WARNING_OBJECT(ogg, "Incorrectly muxed headers found at " "approximate offset %" G_GINT64_FORMAT, ogg->offset);
                            }
                            found_pending_headers = TRUE;

                            GST_LOG_OBJECT(ogg, "Pushing %d pending pages after headers", g_list_length(stream->unknown_pages) + 1);
...
gboolean gst_ogg_parse_plugin_init(GstPlugin * plugin)
{
    ...
    return gst_element_register(plugin, "oggparse", GST_RANK_NONE, GST_TYPE_OGG_PARSE);
}
