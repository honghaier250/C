...
static GstStaticPadTemplate sink_factory_video = GST_STATIC_PAD_TEMPLATE("sink", GST_PAD_SINK, GST_PAD_ALWAYS,
                                                                         GST_STATIC_CAPS("application/x-ogm-video"));
static GstStaticPadTemplate sink_factory_audio = GST_STATIC_PAD_TEMPLATE("sink", GST_PAD_SINK, GST_PAD_ALWAYS,
                                                                         GST_STATIC_CAPS("application/x-ogm-audio"));
static GstStaticPadTemplate sink_factory_text = GST_STATIC_PAD_TEMPLATE("sink", GST_PAD_SINK, GST_PAD_ALWAYS,
...
static gboolean gst_ogm_parse_sink_convert(GstPad * pad, GstFormat src_format, gint64 src_value, GstFormat * dest_format, gint64 * dest_value);

static GstFlowReturn gst_ogm_parse_chain(GstPad * pad, GstBuffer * buffer);

static GstStateChangeReturn gst_ogm_parse_change_state(GstElement * element, GstStateChange transition);
...
static GType gst_ogm_parse_get_type(void)
{
    ...
    if (!ogm_parse_type)
    {
        ...
        ogm_parse_type = g_type_register_static(GST_TYPE_ELEMENT, "GstOgmParse", &ogm_parse_info, 0);
    }
...
static GType gst_ogm_audio_parse_get_type(void)
{
    ...
    if (!ogm_audio_parse_type)
    {
        ...
        ogm_audio_parse_type = g_type_register_static(GST_TYPE_OGM_PARSE, "GstOgmAudioParse", &ogm_audio_parse_info, 0);
    }
...
static GType gst_ogm_video_parse_get_type(void)
{
    ...
    if (!ogm_video_parse_type)
    {
        ...
        ogm_video_parse_type = g_type_register_static(GST_TYPE_OGM_PARSE, "GstOgmVideoParse", &ogm_video_parse_info, 0);
    }
...
static GType gst_ogm_text_parse_get_type(void)
{
    ...
    if (!ogm_text_parse_type)
    {
        ...
        ogm_text_parse_type = g_type_register_static(GST_TYPE_OGM_PARSE, "GstOgmTextParse", &ogm_text_parse_info, 0);
    }
...
static void gst_ogm_audio_parse_base_init(GstOgmParseClass * klass)
{
    ...
    gst_element_class_set_details_simple(element_class, "OGM audio stream parser", "Codec/Decoder/Audio", "parse an OGM audio header and stream", "GStreamer maintainers <gstreamer-devel@lists.sourceforge.net>");

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&sink_factory_audio));
    audio_src_templ = gst_pad_template_new("src", GST_PAD_SRC, GST_PAD_SOMETIMES, caps);
...
static void gst_ogm_video_parse_base_init(GstOgmParseClass * klass)
{
    ...
    gst_element_class_set_details_simple(element_class, "OGM video stream parser", "Codec/Decoder/Video", "parse an OGM video header and stream", "GStreamer maintainers <gstreamer-devel@lists.sourceforge.net>");

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&sink_factory_video));
    video_src_templ = gst_pad_template_new("src", GST_PAD_SRC, GST_PAD_SOMETIMES, caps);
...
static void gst_ogm_text_parse_base_init(GstOgmParseClass * klass)
{
    ...
    gst_element_class_set_details_simple(element_class, "OGM text stream parser", "Codec/Decoder/Subtitle", "parse an OGM text header and stream", "GStreamer maintainers <gstreamer-devel@lists.sourceforge.net>");

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&sink_factory_text));
    text_src_templ = gst_pad_template_new("src", GST_PAD_SRC, GST_PAD_SOMETIMES, caps);
...
static void gst_ogm_parse_class_init(GstOgmParseClass * klass)
{
    ...
    gstelement_class->change_state = GST_DEBUG_FUNCPTR(gst_ogm_parse_change_state);
}
...
static void gst_ogm_audio_parse_init(GstOgmParse * ogm)
{
    ...
    gst_pad_set_query_function(ogm->sinkpad, GST_DEBUG_FUNCPTR(gst_ogm_parse_sink_query));
    gst_pad_set_chain_function(ogm->sinkpad, GST_DEBUG_FUNCPTR(gst_ogm_parse_chain));
    gst_pad_set_event_function(ogm->sinkpad, GST_DEBUG_FUNCPTR(gst_ogm_parse_sink_event));
...
static void gst_ogm_text_parse_init(GstOgmParse * ogm)
{
    ...
    gst_pad_set_query_type_function(ogm->sinkpad, gst_ogm_parse_get_sink_querytypes);
    gst_pad_set_query_function(ogm->sinkpad, GST_DEBUG_FUNCPTR(gst_ogm_parse_sink_query));
    gst_pad_set_chain_function(ogm->sinkpad, GST_DEBUG_FUNCPTR(gst_ogm_parse_chain));
    gst_pad_set_event_function(ogm->sinkpad, GST_DEBUG_FUNCPTR(gst_ogm_parse_sink_event));
    gst_element_add_pad(GST_ELEMENT(ogm), ogm->sinkpad);
            ...
            switch (*dest_format)
            {
                case GST_FORMAT_TIME:
                    switch (ogm->hdr.streamtype[0])
                    {
                        ...
                        case 'v':
                        case 't':
                            *dest_value = (GST_SECOND / 10000000) * ogm->hdr.time_unit * src_value;
                            res = TRUE;
                            break;
                        default:
                            break;
            ...
            switch (*dest_format)
            {
                case GST_FORMAT_DEFAULT:
                    switch (ogm->hdr.streamtype[0])
                    {
                        ...
                        case 'v':
                        case 't':
                            *dest_value = src_value / ((GST_SECOND / 10000000) * ogm->hdr.time_unit);
                            res = TRUE;
                            break;
                        default:
                            break;
        ...
        case GST_QUERY_POSITION:
            {
                ...
                if ((res = gst_ogm_parse_sink_convert(pad, GST_FORMAT_DEFAULT, ogm->next_granulepos, &format, &val)))
                {
        ...
        case GST_QUERY_CONVERT:
            {
                ...
                if ((res = gst_ogm_parse_sink_convert(pad, src_fmt, src_val, &dest_fmt, &dest_val)))
                {
...
static GstFlowReturn gst_ogm_parse_stream_header(GstOgmParse * ogm, const guint8 * data, guint size)
{
    GstCaps *caps = NULL;
        ...
        case 'a':
            {
                ...
                caps = gst_riff_create_audio_caps(codec_id, NULL, NULL, NULL, NULL, NULL);

                if (caps == NULL)
                {
                    GST_WARNING_OBJECT(ogm, "no audio caps for codec %u found", codec_id);
                    caps = gst_caps_new_simple("audio/x-ogm-unknown", "codec_id", G_TYPE_INT, (gint) codec_id, NULL);
                }

                gst_caps_set_simple(caps, "channels", G_TYPE_INT, ogm->hdr.s.audio.channels, "rate", G_TYPE_INT, ogm->hdr.samples_per_unit, NULL);

                GST_LOG_OBJECT(ogm, "Type: %s, subtype: 0x%04x, channels: %d, "
                               "samplerate: %d, blockalign: %d, bps: %d, caps = %" GST_PTR_FORMAT,
                               ogm->hdr.streamtype, codec_id, ogm->hdr.s.audio.channels, (gint) ogm->hdr.samples_per_unit, ogm->hdr.s.audio.blockalign, ogm->hdr.s.audio.avgbytespersec, caps);
                break;
            }
        case 'v':
            {
                ...
                fourcc = GST_MAKE_FOURCC(ogm->hdr.subtype[0], ogm->hdr.subtype[1], ogm->hdr.subtype[2], ogm->hdr.subtype[3]);

                caps = gst_riff_create_video_caps(fourcc, NULL, NULL, NULL, NULL, NULL);

                if (caps == NULL)
                {
                    GST_WARNING_OBJECT(ogm, "could not find video caps for fourcc %" GST_FOURCC_FORMAT, GST_FOURCC_ARGS(fourcc));
                    caps = gst_caps_new_simple("video/x-ogm-unknown", "fourcc", GST_TYPE_FOURCC, fourcc, NULL);
                    break;
                }
                ...
                GST_LOG_OBJECT(ogm, "Type: %s, subtype: %" GST_FOURCC_FORMAT
                               ...
                               ogm->hdr.s.video.width, ogm->hdr.s.video.height, ogm->hdr.time_unit, 10000000. / ogm->hdr.time_unit, ogm->hdr.samples_per_unit, ogm->hdr.default_len, ogm->hdr.buffersize, ogm->hdr.bits_per_sample, caps);
                ...
                gst_caps_set_simple(caps, "width", G_TYPE_INT, ogm->hdr.s.video.width, "height", G_TYPE_INT, ogm->hdr.s.video.height, "framerate", GST_TYPE_FRACTION, 10000000, time_unit, NULL);
                break;
            }
        case 't':
            {
                GST_LOG_OBJECT(ogm, "Type: %s, s/u: %" G_GINT64_FORMAT ", timeunit=%" G_GINT64_FORMAT, ogm->hdr.streamtype, ogm->hdr.samples_per_unit, ogm->hdr.time_unit);
    ...
    if (ogm->srcpad)
    {
        ...
        {
            GST_WARNING_OBJECT(ogm, "Already an existing pad %s:%s", GST_DEBUG_PAD_NAME(ogm->srcpad));
    ...
    if (ogm->srcpad == NULL)
    {
        ...
        GST_INFO_OBJECT(ogm, "Added pad %s:%s with caps %" GST_PTR_FORMAT, GST_DEBUG_PAD_NAME(ogm->srcpad), caps);
...

        {
            ...
            gst_tag_list_add(tags, GST_TAG_MERGE_APPEND, GST_TAG_SUBTITLE_CODEC, "Ogm", NULL);
...
static GstFlowReturn gst_ogm_parse_comment_packet(GstOgmParse * ogm, GstBuffer * buf)
{
    GstFlowReturn ret;
    ...
    if (ogm->hdr.streamtype[0] == 't')
    {
        ...
        tags = gst_tag_list_from_vorbiscomment_buffer(buf, (guint8 *) "\003vorbis", 7, NULL);
...
static void gst_ogm_text_parse_strip_trailing_zeroes(GstOgmParse * ogm, GstBuffer * buf)
{
    const guint8 *data;
    guint size;
...
static GstFlowReturn gst_ogm_parse_data_packet(GstOgmParse * ogm, GstBuffer * buf)
{
    GstFlowReturn ret;
    const guint8 *data;
    GstBuffer *sbuf;
    ...
    GST_LOG_OBJECT(ogm, "[0x%02x] samples: %d, hdrbytes: %d, datasize: %d", data[0], xsize, len, size - len - 1);
        ...
        case 'v':
            {
                ...
                ts = gst_util_uint64_scale(ogm->next_granulepos, ogm->hdr.time_unit * GST_SECOND, 10000000);
                next_ts = gst_util_uint64_scale(ogm->next_granulepos + samples, ogm->hdr.time_unit * GST_SECOND, 10000000);
        ...
        case 'a':
            {
                ...
                /* shouldn't this be granulepos - samples? (tpm) */
                ts = gst_util_uint64_scale_int(ogm->next_granulepos, GST_SECOND, ogm->hdr.samples_per_unit);
                next_ts = gst_util_uint64_scale_int(ogm->next_granulepos + xsize, GST_SECOND, ogm->hdr.samples_per_unit);
    ...
    if (ogm->srcpad)
    {
        ...
        GST_LOG_OBJECT(ogm, "Pushing buffer with ts=%" GST_TIME_FORMAT, GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(sbuf)));
        ret = gst_pad_push(ogm->srcpad, sbuf);
        if (ret != GST_FLOW_OK)
        {
            GST_DEBUG_OBJECT(ogm, "Flow on %s:%s = %s", GST_DEBUG_PAD_NAME(ogm->srcpad), gst_flow_get_name(ret));
        }
    }
    else
    {
...
invalid_startcode:
    {
        GST_ELEMENT_ERROR(ogm, STREAM, DECODE, (NULL), ("unexpected packet startcode 0x%02x", data[0]));
        return GST_FLOW_ERROR;
    }
buffer_too_small:
    {
        GST_ELEMENT_ERROR(ogm, STREAM, DECODE, (NULL), ("buffer too small, len+1=%u, size=%u", len + 1, size));
        return GST_FLOW_ERROR;
    }
}

static GstFlowReturn gst_ogm_parse_chain(GstPad * pad, GstBuffer * buf)
{
...
gboolean gst_ogm_parse_plugin_init(GstPlugin * plugin)
{
    ...
    return gst_element_register(plugin, "ogmaudioparse", GST_RANK_PRIMARY,
                                GST_TYPE_OGM_AUDIO_PARSE) && gst_element_register(plugin, "ogmvideoparse", GST_RANK_PRIMARY, GST_TYPE_OGM_VIDEO_PARSE) && gst_element_register(plugin, "ogmtextparse", GST_RANK_PRIMARY, GST_TYPE_OGM_TEXT_PARSE);
}
