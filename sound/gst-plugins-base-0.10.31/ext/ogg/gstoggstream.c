...
typedef gboolean(*GstOggMapSetupFunc) (GstOggStream * pad, ogg_packet * packet);
typedef GstClockTime(*GstOggMapToTimeFunc) (GstOggStream * pad, gint64 granulepos);
typedef gint64(*GstOggMapToGranuleFunc) (GstOggStream * pad, gint64 granulepos);
typedef gint64(*GstOggMapToGranuleposFunc) (GstOggStream * pad, gint64 granule, gint64 keyframe_granule);

/* returns TRUE if the granulepos denotes a key frame */
typedef gboolean(*GstOggMapIsKeyFrameFunc) (GstOggStream * pad, gint64 granulepos);

/* returns TRUE if the given packet is a stream header packet */
typedef gboolean(*GstOggMapIsHeaderPacketFunc) (GstOggStream * pad, ogg_packet * packet);
typedef gint64(*GstOggMapPacketDurationFunc) (GstOggStream * pad, ogg_packet * packet);

typedef gint64(*GstOggMapGranuleposToKeyGranuleFunc) (GstOggStream * pad, gint64 granulepos);
...
GstClockTime gst_ogg_stream_get_packet_start_time(GstOggStream * pad, ogg_packet * packet)
{
    int duration;
    ...
    return gst_ogg_stream_granule_to_time(pad, gst_ogg_stream_granulepos_to_granule(pad, packet->granulepos) - duration);
}

GstClockTime gst_ogg_stream_get_start_time_for_granulepos(GstOggStream * pad, gint64 granulepos)
{
    if (pad->frame_size == 0)
        return GST_CLOCK_TIME_NONE;

    return gst_ogg_stream_granule_to_time(pad, gst_ogg_stream_granulepos_to_granule(pad, granulepos));
}

GstClockTime gst_ogg_stream_get_end_time_for_granulepos(GstOggStream * pad, gint64 granulepos)
{
    return gst_ogg_stream_granule_to_time(pad, gst_ogg_stream_granulepos_to_granule(pad, granulepos));
}
...
GstClockTime gst_ogg_stream_granule_to_time(GstOggStream * pad, gint64 granule)
{
    ...
    return gst_util_uint64_scale(granule, GST_SECOND * pad->granulerate_d, pad->granulerate_n);
}
...
gint64 gst_ogg_stream_granule_to_granulepos(GstOggStream * pad, gint64 granule, gint64 keyframe_granule)
{
    ...
    return mappers[pad->map].granule_to_granulepos_func(pad, granule, keyframe_granule);
}

#if 0
gboolean gst_ogg_stream_packet_granulepos_is_key_frame(GstOggStream * pad, gint64 granulepos)
{
    if (granulepos == -1)
    {
        return FALSE;
...
#ifdef unused
static gboolean is_header_unknown(GstOggStream * pad, ogg_packet * packet)
{
...
static gboolean setup_theora_mapper(GstOggStream * pad, ogg_packet * packet)
{
    ...
    GST_LOG("fps = %d/%d, PAR = %u/%u, width = %u, height = %u", pad->granulerate_n, pad->granulerate_d, par_n, par_d, w, h);

    /* 2 bits + 3 bits = 5 bits KFGSHIFT */
    pad->granuleshift = ((GST_READ_UINT8(data + 40) & 0x03) << 3) + (GST_READ_UINT8(data + 41) >> 5);
    ...
    if (w > 0 && h > 0)
    {
        gst_caps_set_simple(pad->caps, "width", G_TYPE_INT, w, "height", G_TYPE_INT, h, NULL);
    }
    ...
    gst_caps_set_simple(pad->caps, "framerate", GST_TYPE_FRACTION, pad->granulerate_n, pad->granulerate_d, "pixel-aspect-ratio", GST_TYPE_FRACTION, par_n, par_d, NULL);
...
static gboolean setup_dirac_mapper(GstOggStream * pad, ogg_packet * packet)
{
    ...
    ret = dirac_sequence_header_parse(&header, packet->packet + 13, packet->bytes - 13);
    if (ret == 0)
    {
    ...
    pad->caps = gst_caps_new_simple("video/x-dirac",
                                    ...
                                    "pixel-aspect-ratio", GST_TYPE_FRACTION, header.aspect_ratio_numerator, header.aspect_ratio_denominator, "framerate", GST_TYPE_FRACTION, header.frame_rate_numerator, header.frame_rate_denominator, NULL);
...
static gint64 granulepos_to_key_granule_dirac(GstOggStream * pad, gint64 gp)
{
    gint64 pt;
    int dist_h;
    int dist_l;
...
static gboolean setup_vp8_mapper(GstOggStream * pad, ogg_packet * packet)
{
    ...
    pad->caps = gst_caps_new_simple("video/x-vp8", "width", G_TYPE_INT, width, "height", G_TYPE_INT, height, "pixel-aspect-ratio", GST_TYPE_FRACTION, par_n, par_d, "framerate", GST_TYPE_FRACTION, fps_n, fps_d, NULL);

    return TRUE;
}

static gboolean is_keyframe_vp8(GstOggStream * pad, gint64 granulepos)
{
...
static gint64 granulepos_to_granule_vp8(GstOggStream * pad, gint64 gpos)
{
...
static gint64 granule_to_granulepos_vp8(GstOggStream * pad, gint64 granule, gint64 keyframe_granule)
{
...
static gint64 packet_duration_vp8(GstOggStream * pad, ogg_packet * packet)
{
    guint32 hdr;
...
static gint64 granulepos_to_key_granule_vp8(GstOggStream * pad, gint64 granulepos)
{
...
static gboolean is_header_vp8(GstOggStream * pad, ogg_packet * packet)
{
    if (packet->bytes >= 5 && packet->packet[0] == 0x4F && packet->packet[1] == 0x56 && packet->packet[2] == 0x50 && packet->packet[3] == 0x38 && packet->packet[4] == 0x30)
        return TRUE;
    return FALSE;
}
...
static gboolean setup_vorbis_mapper(GstOggStream * pad, ogg_packet * packet)
{
    ...
    pad->caps = gst_caps_new_simple("audio/x-vorbis", "rate", G_TYPE_INT, pad->granulerate_n, "channels", G_TYPE_INT, chans, NULL);
...
static gboolean setup_speex_mapper(GstOggStream * pad, ogg_packet * packet)
{
    ...
    pad->frame_size = GST_READ_UINT32_LE(packet->packet + 64) * GST_READ_UINT32_LE(packet->packet + 56);

    if (pad->granulerate_n == 0)
        return FALSE;

    pad->caps = gst_caps_new_simple("audio/x-speex", "rate", G_TYPE_INT, pad->granulerate_n, "channels", G_TYPE_INT, chans, NULL);
...
static gboolean is_header_fLaC(GstOggStream * pad, ogg_packet * packet)
{
    ...
    {
        pad->granulerate_n = (packet->packet[14] << 12) | (packet->packet[15] << 4) | ((packet->packet[16] >> 4) & 0xf);
    }
...
static gboolean setup_flac_mapper(GstOggStream * pad, ogg_packet * packet)
{
    ...
    pad->caps = gst_caps_new_simple("audio/x-flac", "rate", G_TYPE_INT, pad->granulerate_n, "channels", G_TYPE_INT, chans, NULL);
...
static gboolean setup_fishead_mapper(GstOggStream * pad, ogg_packet * packet)
{
    ...
    if (packet->bytes >= SKELETON_FISHEAD_3_3_MIN_SIZE && pad->skeleton_major == 3 && pad->skeleton_minor > 0)
    {
        ...
        GST_INFO("firstsampletime %" G_GUINT64_FORMAT "/%" G_GUINT64_FORMAT, firstsampletime_n, firstsampletime_d);
        GST_INFO("lastsampletime %" G_GUINT64_FORMAT "/%" G_GUINT64_FORMAT, lastsampletime_n, lastsampletime_d);
        ...
        if (firstsampletime_d > 0)
            firstsampletime = gst_util_uint64_scale(GST_SECOND, firstsampletime_n, firstsampletime_d);
        else
            firstsampletime = 0;

        if (lastsampletime_d > 0)
            lastsampletime = gst_util_uint64_scale(GST_SECOND, lastsampletime_n, lastsampletime_d);
        else
            lastsampletime = 0;
        ...
        GST_INFO("skeleton fishead parsed total: %" GST_TIME_FORMAT, GST_TIME_ARGS(pad->total_time));
    }
    else if (packet->bytes >= SKELETON_FISHEAD_4_0_MIN_SIZE && pad->skeleton_major == 4)
    {
        guint64 segment_length, content_offset;
    ...
    GST_INFO("skeleton fishead %u.%u parsed (basetime: %" GST_TIME_FORMAT ", prestime: %" GST_TIME_FORMAT ")", pad->skeleton_major, pad->skeleton_minor, GST_TIME_ARGS(pad->basetime), GST_TIME_ARGS(pad->prestime));
...
gboolean gst_ogg_map_parse_fisbone(GstOggStream * pad, const guint8 * data, guint size, guint32 * serialno, GstOggSkeleton * type)
{
...
gboolean gst_ogg_map_add_fisbone(GstOggStream * pad, GstOggStream * skel_pad, const guint8 * data, guint size, GstClockTime * p_start_time)
{
    ...
    GST_INFO("skeleton fisbone parsed "
             "(start time: %" GST_TIME_FORMAT " granulerate_n: %d granulerate_d: %d " " preroll: %" G_GUINT32_FORMAT " granuleshift: %d)", GST_TIME_ARGS(start_time), pad->granulerate_n, pad->granulerate_d, pad->preroll, pad->granuleshift);
...
static gboolean read_vlc(const guint8 ** data, guint * size, guint64 * result)
{
    gint shift = 0;
    guint8 byte;
...
gboolean gst_ogg_map_add_index(GstOggStream * pad, GstOggStream * skel_pad, const guint8 * data, guint size)
{
    ...
    if ((skel_pad->skeleton_major == 3 && size < 26) || (skel_pad->skeleton_major == 4 && size < 62))
    {
    ...
    if (skel_pad->skeleton_major == 4)
    {
        ...
        GST_INFO("firstsampletime %" G_GUINT64_FORMAT "/%" G_GUINT64_FORMAT, firstsampletime_n, pad->kp_denom);
        GST_INFO("lastsampletime %" G_GUINT64_FORMAT "/%" G_GUINT64_FORMAT, lastsampletime_n, pad->kp_denom);

        firstsampletime = gst_util_uint64_scale(GST_SECOND, firstsampletime_n, pad->kp_denom);
        lastsampletime = gst_util_uint64_scale(GST_SECOND, lastsampletime_n, pad->kp_denom);
        ...
        GST_INFO("skeleton index parsed total: %" GST_TIME_FORMAT, GST_TIME_ARGS(pad->total_time));

        data += 16;
        size -= 16;
    }

    GST_INFO("skeleton index has %" G_GUINT64_FORMAT " keypoints, denom: %" G_GINT64_FORMAT, n_keypoints, pad->kp_denom);
    ...
    for (i = 0; i < n_keypoints; i++)
    {
        ...
        GST_INFO("offset %" G_GUINT64_FORMAT " time %" G_GUINT64_FORMAT, offset, timestamp);
    }
    if (isize != n_keypoints)
    {
        GST_WARNING("truncated index, expected %" G_GUINT64_FORMAT ", found %" G_GUINT64_FORMAT, n_keypoints, isize);
    }
    pad->n_index = isize;
...
static gint gst_ogg_index_compare(const GstOggIndex * index, const guint64 * ts, gpointer user_data)
{
...
gboolean gst_ogg_map_search_index(GstOggStream * pad, gboolean before, guint64 * timestamp, guint64 * offset)
{
    ...
    best = gst_util_array_binary_search(pad->index, n_index, sizeof(GstOggIndex), (GCompareDataFunc) gst_ogg_index_compare, GST_SEARCH_MODE_BEFORE, &ts, NULL);
    ...
    if (offset)
        *offset = best->offset;
    if (timestamp)
        *timestamp = gst_util_uint64_scale(best->timestamp, GST_SECOND, pad->kp_denom);
...
static gboolean setup_ogmaudio_mapper(GstOggStream * pad, ogg_packet * packet)
{
    ...
    if (pad->caps)
    {
        gst_caps_set_simple(pad->caps, "rate", G_TYPE_INT, pad->granulerate_n, NULL);
    }
    else
    {
        pad->caps = gst_caps_new_simple("audio/x-ogm-unknown", "fourcc", GST_TYPE_FOURCC, fourcc, "rate", G_TYPE_INT, pad->granulerate_n, NULL);
    }
...
static gboolean setup_ogmvideo_mapper(GstOggStream * pad, ogg_packet * packet)
{
    ...
    GST_LOG("fps = %d/%d = %.3f", pad->granulerate_n, pad->granulerate_d, (double) pad->granulerate_n / pad->granulerate_d);
    ...
    if (pad->caps == NULL)
    {
        pad->caps = gst_caps_new_simple("video/x-ogm-unknown", "fourcc", GST_TYPE_FOURCC, fourcc, "framerate", GST_TYPE_FRACTION, pad->granulerate_n, pad->granulerate_d, NULL);
    }
    else
    {
        gst_caps_set_simple(pad->caps, "framerate", GST_TYPE_FRACTION, pad->granulerate_n, pad->granulerate_d, "width", G_TYPE_INT, width, "height", G_TYPE_INT, height, NULL);
    }
...
static gboolean setup_pcm_mapper(GstOggStream * pad, ogg_packet * packet)
{
    ...
    {
        case OGGPCM_FMT_S8:
            caps = gst_caps_new_simple("audio/x-raw-int", "depth", G_TYPE_INT, 8, "width", G_TYPE_INT, 8, "signed", G_TYPE_BOOLEAN, TRUE, NULL);
            break;
        case OGGPCM_FMT_U8:
            caps = gst_caps_new_simple("audio/x-raw-int", "depth", G_TYPE_INT, 8, "width", G_TYPE_INT, 8, "signed", G_TYPE_BOOLEAN, FALSE, NULL);
            break;
        case OGGPCM_FMT_S16_LE:
            caps = gst_caps_new_simple("audio/x-raw-int", "depth", G_TYPE_INT, 16, "width", G_TYPE_INT, 16, "endianness", G_TYPE_INT, G_LITTLE_ENDIAN, "signed", G_TYPE_BOOLEAN, TRUE, NULL);
            break;
        case OGGPCM_FMT_S16_BE:
            caps = gst_caps_new_simple("audio/x-raw-int", "depth", G_TYPE_INT, 16, "width", G_TYPE_INT, 16, "endianness", G_TYPE_INT, G_BIG_ENDIAN, "signed", G_TYPE_BOOLEAN, TRUE, NULL);
            break;
        case OGGPCM_FMT_S24_LE:
            caps = gst_caps_new_simple("audio/x-raw-int", "depth", G_TYPE_INT, 24, "width", G_TYPE_INT, 24, "endianness", G_TYPE_INT, G_LITTLE_ENDIAN, "signed", G_TYPE_BOOLEAN, TRUE, NULL);
            break;
        case OGGPCM_FMT_S24_BE:
            caps = gst_caps_new_simple("audio/x-raw-int", "depth", G_TYPE_INT, 24, "width", G_TYPE_INT, 24, "endianness", G_TYPE_INT, G_BIG_ENDIAN, "signed", G_TYPE_BOOLEAN, TRUE, NULL);
            break;
        case OGGPCM_FMT_S32_LE:
            caps = gst_caps_new_simple("audio/x-raw-int", "depth", G_TYPE_INT, 32, "width", G_TYPE_INT, 32, "endianness", G_TYPE_INT, G_LITTLE_ENDIAN, "signed", G_TYPE_BOOLEAN, TRUE, NULL);
            break;
        case OGGPCM_FMT_S32_BE:
            caps = gst_caps_new_simple("audio/x-raw-int", "depth", G_TYPE_INT, 32, "width", G_TYPE_INT, 32, "endianness", G_TYPE_INT, G_BIG_ENDIAN, "signed", G_TYPE_BOOLEAN, TRUE, NULL);
            break;
        ...
        case OGGPCM_FMT_FLT32_LE:
            caps = gst_caps_new_simple("audio/x-raw-float", "width", G_TYPE_INT, 32, "endianness", G_TYPE_INT, G_LITTLE_ENDIAN, NULL);
            break;
        case OGGPCM_FMT_FLT32_BE:
            caps = gst_caps_new_simple("audio/x-raw-float", "width", G_TYPE_INT, 32, "endianness", G_TYPE_INT, G_BIG_ENDIAN, NULL);
            break;
        case OGGPCM_FMT_FLT64_LE:
            caps = gst_caps_new_simple("audio/x-raw-float", "width", G_TYPE_INT, 64, "endianness", G_TYPE_INT, G_LITTLE_ENDIAN, NULL);
            break;
        case OGGPCM_FMT_FLT64_BE:
            caps = gst_caps_new_simple("audio/x-raw-float", "width", G_TYPE_INT, 64, "endianness", G_TYPE_INT, G_BIG_ENDIAN, NULL);
            break;
        default:
            return FALSE;
    }

    gst_caps_set_simple(caps, "audio/x-raw-int", "rate", G_TYPE_INT, pad->granulerate_n, "channels", G_TYPE_INT, channels, NULL);
    pad->caps = caps;
...
static gboolean setup_celt_mapper(GstOggStream * pad, ogg_packet * packet)
{
    ...
    pad->caps = gst_caps_new_simple("audio/x-celt", "rate", G_TYPE_INT, pad->granulerate_n, NULL);
...
gboolean gst_ogg_stream_setup_map(GstOggStream * pad, ogg_packet * packet)
{
    ...
    {
        if (packet->bytes >= mappers[i].min_packet_size && packet->bytes >= mappers[i].id_length && memcmp(packet->packet, mappers[i].id, mappers[i].id_length) == 0)
        {
            ...
            {
                GST_WARNING("mapper '%s' did not accept setup header", mappers[i].media_type);
            }
        }
    }
