...
static GType gst_ogg_avi_parse_get_type(void)
{
    ...
    if (!ogg_avi_parse_type)
    {
        ...
        ogg_avi_parse_type = g_type_register_static(GST_TYPE_ELEMENT, "GstOggAviParse", &ogg_avi_parse_info, 0);
    }
...
static GstStaticPadTemplate ogg_avi_parse_src_template_factory = GST_STATIC_PAD_TEMPLATE("src",
                                                                                         GST_PAD_SRC,
                                                                                         GST_PAD_ALWAYS,
                                                                                         GST_STATIC_CAPS("audio/x-vorbis"));

static GstStaticPadTemplate ogg_avi_parse_sink_template_factory = GST_STATIC_PAD_TEMPLATE("sink",
                                                                                          GST_PAD_SINK,
                                                                                          GST_PAD_ALWAYS,
                                                                                          GST_STATIC_CAPS("application/x-ogg-avi"));

static void gst_ogg_avi_parse_finalize(GObject * object);
static GstStateChangeReturn gst_ogg_avi_parse_change_state(GstElement * element, GstStateChange transition);
...
static void gst_ogg_avi_parse_base_init(gpointer g_class)
{
    ...
    gst_element_class_set_details_simple(element_class, "Ogg AVI parser", "Codec/Parser", "parse an ogg avi stream into pages (info about ogg: http://xiph.org)", "Wim Taymans <wim@fluendo.com>");

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&ogg_avi_parse_sink_template_factory));
    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&ogg_avi_parse_src_template_factory));
}
...
static void gst_ogg_avi_parse_init(GstOggAviParse * ogg)
{
    ...
    ogg->sinkpad = gst_pad_new_from_static_template(&ogg_avi_parse_sink_template_factory, "sink");
    ...
    ogg->srcpad = gst_pad_new_from_static_template(&ogg_avi_parse_src_template_factory, "src");
...
static gboolean gst_ogg_avi_parse_setcaps(GstPad * pad, GstCaps * caps)
{
    ...
    GST_DEBUG_OBJECT(ogg, "header sizes: %u %u %u", sizes[0], sizes[1], sizes[2]);
...
static GstFlowReturn gst_ogg_avi_parse_push_packet(GstOggAviParse * ogg, ogg_packet * packet)
{
    GstBuffer *buffer;
...
gboolean gst_ogg_avi_parse_plugin_init(GstPlugin * plugin)
{
    ...
    return gst_element_register(plugin, "oggaviparse", GST_RANK_PRIMARY, GST_TYPE_OGG_AVI_PARSE);
}
