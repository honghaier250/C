...
static ogg_packet *_ogg_packet_copy(const ogg_packet * packet)
{
...
static void _ogg_packet_free(ogg_packet * packet)
{
    g_free(packet->packet);
    g_free(packet);
}

static ogg_page *gst_ogg_page_copy(ogg_page * page)
{
...
static void gst_ogg_page_free(ogg_page * page)
{
    g_free(page->header);
    g_free(page->body);
    g_free(page);
...
static gboolean gst_ogg_demux_collect_chain_info(GstOggDemux * ogg, GstOggChain * chain);
static gboolean gst_ogg_demux_activate_chain(GstOggDemux * ogg, GstOggChain * chain, GstEvent * event);
static void gst_ogg_chain_mark_discont(GstOggChain * chain);

static gboolean gst_ogg_demux_perform_seek(GstOggDemux * ogg, GstEvent * event);
static gboolean gst_ogg_demux_receive_event(GstElement * element, GstEvent * event);
...
static GstOggPad *gst_ogg_chain_get_stream(GstOggChain * chain, glong serialno);

static GstFlowReturn gst_ogg_demux_combine_flows(GstOggDemux * ogg, GstOggPad * pad, GstFlowReturn ret);
...
static void gst_ogg_pad_init(GstOggPad * pad)
{
    ...
    gst_pad_set_query_type_function(GST_PAD(pad), GST_DEBUG_FUNCPTR(gst_ogg_pad_query_types));
                ...
                else
                {
                    ...
                    if (bitrate > 0)
                    {
                        ...
                        if (gst_pad_peer_query(ogg->sinkpad, uquery))
                        {
                            ...
                            total_time = gst_util_uint64_scale(length, 8 * GST_SECOND, bitrate);

                            GST_LOG_OBJECT(ogg, "length: %" G_GINT64_FORMAT ", bitrate %d, total_time %" GST_TIME_FORMAT, length, bitrate, GST_TIME_ARGS(total_time));
                        }
        ...
        case GST_QUERY_SEEKING:
            {
                ...
                if (format == GST_FORMAT_TIME)
                {
                    ...
                    else if (ogg->current_chain->streams->len)
                    {
                        ...
                        {
                            GstOggPad *pad = g_array_index(ogg->current_chain->streams, GstOggPad *, i);
                            ...
                            if (pad->map.index != NULL && pad->map.n_index != 0)
                            {
                                ...
                                idx_time = gst_util_uint64_scale(idx->timestamp, GST_SECOND, pad->map.kp_denom);
                                if (stop == -1)
                                    stop = idx_time;
                                else
...
static void gst_ogg_pad_reset(GstOggPad * pad)
{
    ogg_stream_reset(&pad->map.stream);
...
static GstFlowReturn gst_ogg_demux_queue_data(GstOggPad * pad, ogg_packet * packet)
{
#ifndef GST_DISABLE_GST_DEBUG
    GstOggDemux *ogg = pad->ogg;
#endif

    GST_DEBUG_OBJECT(ogg, "%p queueing data serial %08lx", pad, pad->map.serialno);
...
static GstFlowReturn gst_ogg_demux_chain_peer(GstOggPad * pad, ogg_packet * packet, gboolean push_headers)
{
    GstBuffer *buf = NULL;
    ...
    GST_DEBUG_OBJECT(ogg, "%p streaming to peer serial %08lx", pad, pad->map.serialno);
    ...
    if (pad->map.is_ogm)
    {
        ...
        else if (data[0] & 3 && pad->map.is_ogm_text)
        {
            ...
            tags = gst_tag_list_from_vorbiscomment_buffer(buf, (const guint8 *) "\003vorbis", 7, NULL);
            ...
            if (tags)
            {
                ...
                gst_element_found_tags_for_pad(GST_ELEMENT(ogg), GST_PAD_CAST(pad), tags);
            }
            else
            {
    ...
    else if (pad->map.is_vp8)
    {
        if (packet->bytes >= 7 && memcmp(packet->packet, "OVP80\2 ", 7) == 0)
        {
            ...
            tags = gst_tag_list_from_vorbiscomment_buffer(buf, (const guint8 *) "OVP80\2 ", 7, NULL);
            gst_buffer_unref(buf);
            buf = NULL;
            ...
            {
                GST_DEBUG_OBJECT(ogg, "failed to extract VP8 tags from vorbis comment");
            }
            /* We don't push header packets for VP8 */
            cret = gst_ogg_demux_combine_flows(ogg, pad, GST_FLOW_OK);
            goto done;
        }
        else if (packet->b_o_s || (packet->bytes >= 5 && memcmp(packet->packet, "OVP80", 5) == 0))
        {
            /* We don't push header packets for VP8 */
            cret = gst_ogg_demux_combine_flows(ogg, pad, GST_FLOW_OK);
            goto done;
    ...
    else
    {
        ...
        {
            pad->current_granule = gst_ogg_stream_granulepos_to_granule(&pad->map, packet->granulepos);
            pad->keyframe_granule = gst_ogg_stream_granulepos_to_key_granule(&pad->map, packet->granulepos);
            GST_DEBUG_OBJECT(ogg, "new granule %" G_GUINT64_FORMAT, pad->current_granule);
        }
        else if (ogg->segment.rate > 0.0 && pad->current_granule != -1)
        {
            pad->current_granule += duration;
            GST_DEBUG_OBJECT(ogg, "interpollating granule %" G_GUINT64_FORMAT, pad->current_granule);
        }
        ...
        else
        {
            ...
            if (pad->map.is_ogm)
            {
                out_timestamp = gst_ogg_stream_granule_to_time(&pad->map, pad->current_granule);
                out_duration = gst_util_uint64_scale(duration, GST_SECOND * pad->map.granulerate_d, pad->map.granulerate_n);
            }
            ...
            {
                out_timestamp = gst_ogg_stream_granule_to_time(&pad->map, pad->current_granule - duration);
                out_duration = gst_ogg_stream_granule_to_time(&pad->map, pad->current_granule) - out_timestamp;
            }
            out_offset_end = gst_ogg_stream_granule_to_granulepos(&pad->map, pad->current_granule, pad->keyframe_granule);
            out_offset = gst_ogg_stream_granule_to_time(&pad->map, pad->current_granule);
        }
    }
    ...
    ret = gst_pad_alloc_buffer_and_set_caps(GST_PAD_CAST(pad), GST_BUFFER_OFFSET_NONE, packet->bytes - offset - trim, GST_PAD_CAPS(pad), &buf);
    ...
    /* convert to time */
    current_time = gst_ogg_stream_get_end_time_for_granulepos(&pad->map, packet->granulepos);
    ...
    GST_DEBUG_OBJECT(ogg, "ogg current time %" GST_TIME_FORMAT, GST_TIME_ARGS(current_time));

    /* check stream eos */
    if ((ogg->segment.rate > 0.0 && ogg->segment.stop != GST_CLOCK_TIME_NONE && current_time > ogg->segment.stop) || (ogg->segment.rate < 0.0 && ogg->segment.start != GST_CLOCK_TIME_NONE && current_time < ogg->segment.start))
    {
...
no_buffer:
    {
        GST_DEBUG_OBJECT(ogg, "%p could not get buffer from peer %08lx, %d (%s), combined %d (%s)", pad, pad->map.serialno, ret, gst_flow_get_name(ret), cret, gst_flow_get_name(cret));
        goto done;
    }
}

static guint64 gst_ogg_demux_collect_start_time(GstOggDemux * ogg, GstOggChain * chain)
{
    gint i;
 ...
 * stream 
 */
static GstFlowReturn gst_ogg_pad_submit_packet(GstOggPad * pad, ogg_packet * packet)
{
    gint64 granule;
    ...
    GST_DEBUG_OBJECT(ogg, "%p submit packet serial %08lx", pad, pad->map.serialno);
    ...
    if (pad->map.is_skeleton)
    {
        ...
        if (gst_ogg_map_parse_fisbone(&pad->map, packet->packet, packet->bytes, &serialno, &type))
        {

            GST_WARNING_OBJECT(pad->ogg, "got skeleton packet for stream 0x%08x", serialno);
            ...
            if (skel_pad)
            {
                switch (type)
                {
                    case GST_OGG_SKELETON_FISBONE:
                         ...
                         * live streams */
                        gst_ogg_map_add_fisbone(&skel_pad->map, &pad->map, packet->packet, packet->bytes, NULL);
                        break;
                    case GST_OGG_SKELETON_INDEX:
                        gst_ogg_map_add_index(&skel_pad->map, &pad->map, packet->packet, packet->bytes);
            ...
            {
                GST_WARNING_OBJECT(pad->ogg, "found skeleton fisbone for an unknown stream 0x%08x", serialno);
            }
        }
    }
    ...
    if (!packet->b_o_s)
    {
        ...
        if (pad->start_time == GST_CLOCK_TIME_NONE)
        {
            ...
            if (duration != -1)
            {
                ...
                GST_DEBUG("accumulated granule %" G_GINT64_FORMAT, pad->map.accumulated_granule);
            }
            ...
            if (packet->granulepos != -1)
            {
                ...
                pad->start_time = gst_ogg_stream_granule_to_time(&pad->map, start_granule);
                GST_DEBUG("start time %" G_GINT64_FORMAT, pad->start_time);
            }
            else
            {
                packet->granulepos = gst_ogg_stream_granule_to_granulepos(&pad->map, pad->map.accumulated_granule, pad->keyframe_granule);
            }
        }
    }
    else
    {
        ...
        if (!pad->map.have_headers)
        {
            pad->map.headers = g_list_append(pad->map.headers, _ogg_packet_copy(packet));
        ...
        if (chain == ogg->building_chain)
        {
            ...
            if (ogg->resync)
            {
                ...
                if (start_time != G_MAXUINT64)
                {
                    ...
                    GST_DEBUG_OBJECT(ogg, "start_time:  %" GST_TIME_FORMAT, GST_TIME_ARGS(start_time));

                    if (chain->segment_start < start_time)
                        segment_time = (start_time - chain->segment_start) + chain->begin_time;
                    else
                        segment_time = chain->begin_time;

                    /* create the newsegment event we are going to send out */
                    event = gst_event_new_new_segment(FALSE, ogg->segment.rate, GST_FORMAT_TIME, start_time, chain->segment_stop, segment_time);
            ...
            else
            {
                ...
                if (gst_ogg_demux_collect_chain_info(ogg, chain))
                {
                    ...
                    GST_DEBUG_OBJECT(ogg, "segment_start: %" GST_TIME_FORMAT, GST_TIME_ARGS(chain->segment_start));
                    GST_DEBUG_OBJECT(ogg, "segment_stop:  %" GST_TIME_FORMAT, GST_TIME_ARGS(chain->segment_stop));
                    GST_DEBUG_OBJECT(ogg, "segment_time:  %" GST_TIME_FORMAT, GST_TIME_ARGS(chain->begin_time));

                    /* create the newsegment event we are going to send out */
                    event = gst_event_new_new_segment(FALSE, ogg->segment.rate, GST_FORMAT_TIME, chain->segment_start, chain->segment_stop, chain->begin_time);
                }
            }
 ...
 * @npackets is 0;
 */
static GstFlowReturn gst_ogg_pad_stream_out(GstOggPad * pad, gint npackets)
{
    ...
    while (!done)
    {
        ...
        switch (ret)
        {
            ...
            case 1:
                ...
                break;
            default:
                GST_WARNING_OBJECT(ogg, "invalid return value %d for ogg_stream_packetout, resetting stream", ret);
...
could_not_submit:
    {
        GST_WARNING_OBJECT(ogg, "could not submit packet for stream %08lx, error: %d", pad->map.serialno, result);
        gst_ogg_pad_reset(pad);
        return result;
    }
}
 ...
 */
static GstFlowReturn gst_ogg_pad_submit_page(GstOggPad * pad, ogg_page * page)
{
    GstFlowReturn result = GST_FLOW_OK;
    GstOggDemux *ogg;
...
choked:
    {
        GST_WARNING_OBJECT(ogg, "ogg stream choked on page (serial %08lx), resetting stream", pad->map.serialno);
        gst_ogg_pad_reset(pad);
...
static GstOggChain *gst_ogg_chain_new(GstOggDemux * ogg)
{
...
static void gst_ogg_chain_free(GstOggChain * chain)
{
    gint i;
...
static void gst_ogg_chain_reset(GstOggChain * chain)
{
    gint i;
...
static GstOggPad *gst_ogg_chain_new_stream(GstOggChain * chain, glong serialno)
{
    GstOggPad *ret;
    GstTagList *list;
    gchar *name;

    GST_DEBUG_OBJECT(chain->ogg, "creating new stream %08lx in chain %p", serialno, chain);
    ...
    gst_tag_list_add(list, GST_TAG_MERGE_REPLACE, GST_TAG_SERIAL, serialno, NULL);
    gst_tag_list_free(list);

    GST_DEBUG_OBJECT(chain->ogg, "created new ogg src %p for stream with serial %08lx", ret, serialno);
...
init_failed:
    {
        GST_ERROR("Could not initialize ogg_stream struct for serial %08lx.", serialno);
...
static gboolean gst_ogg_chain_has_stream(GstOggChain * chain, glong serialno)
{
...
static GstStaticPadTemplate ogg_demux_src_template_factory = GST_STATIC_PAD_TEMPLATE("src_%d",
                                                                                     GST_PAD_SRC,
                                                                                     GST_PAD_SOMETIMES,
                                                                                     GST_STATIC_CAPS_ANY);

static GstStaticPadTemplate ogg_demux_sink_template_factory = GST_STATIC_PAD_TEMPLATE("sink",
                                                                                      GST_PAD_SINK,
                                                                                      GST_PAD_ALWAYS,
...
static GstFlowReturn gst_ogg_demux_read_chain(GstOggDemux * ogg, GstOggChain ** chain);
static GstFlowReturn gst_ogg_demux_read_end_chain(GstOggDemux * ogg, GstOggChain * chain);
...
static gboolean gst_ogg_demux_sink_activate_pull(GstPad * sinkpad, gboolean active);
static gboolean gst_ogg_demux_sink_activate_push(GstPad * sinkpad, gboolean active);
static GstStateChangeReturn gst_ogg_demux_change_state(GstElement * element, GstStateChange transition);
...
static void gst_ogg_demux_base_init(gpointer g_class)
{
    ...
    gst_element_class_set_details_simple(element_class, "Ogg demuxer", "Codec/Demuxer", "demux ogg streams (info about ogg: http://xiph.org)", "Wim Taymans <wim@fluendo.com>");

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&ogg_demux_sink_template_factory));
    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&ogg_demux_src_template_factory));
}

static void gst_ogg_demux_class_init(GstOggDemuxClass * klass)
{
...
static void gst_ogg_demux_init(GstOggDemux * ogg, GstOggDemuxClass * g_class)
{
    /* create the sink pad */
    ogg->sinkpad = gst_pad_new_from_static_template(&ogg_demux_sink_template_factory, "sink");
    ...
    gst_pad_set_activatepull_function(ogg->sinkpad, gst_ogg_demux_sink_activate_pull);
    gst_pad_set_activatepush_function(ogg->sinkpad, gst_ogg_demux_sink_activate_push);
...
static void gst_ogg_demux_reset_streams(GstOggDemux * ogg)
{
    GstOggChain *chain;
    guint i;
 ...
 */
static GstFlowReturn gst_ogg_demux_submit_buffer(GstOggDemux * ogg, GstBuffer * buffer)
{
    gint size;
    guint8 *data;
    gchar *oggbuffer;
...
no_buffer:
    {
        GST_ELEMENT_ERROR(ogg, STREAM, DECODE, (NULL), ("failed to get ogg sync buffer"));
        ret = GST_FLOW_ERROR;
        goto done;
    }
write_failed:
    {
        GST_ELEMENT_ERROR(ogg, STREAM, DECODE, (NULL), ("failed to write %d bytes to the sync buffer", size));
        ret = GST_FLOW_ERROR;
        goto done;
    }
}
 ...
 */
static void gst_ogg_demux_seek(GstOggDemux * ogg, gint64 offset)
{
 ...
 * the ogg sync layer.
 */
static GstFlowReturn gst_ogg_demux_get_data(GstOggDemux * ogg, gint64 end_offset)
{
    GstFlowReturn ret;
    GstBuffer *buffer;

    GST_LOG_OBJECT(ogg, "get data %" G_GINT64_FORMAT " %" G_GINT64_FORMAT " %" G_GINT64_FORMAT, ogg->read_offset, ogg->length, end_offset);
...
error:
    {
        GST_WARNING_OBJECT(ogg, "got %d (%s) from pull range", ret, gst_flow_get_name(ret));
        return ret;
    }
}
 ...
 * is.
 */
static GstFlowReturn gst_ogg_demux_get_next_page(GstOggDemux * ogg, ogg_page * og, gint64 boundary, gint64 * offset)
{
    gint64 end_offset = -1;
    GstFlowReturn ret;

    GST_LOG_OBJECT(ogg, "get next page, current offset %" G_GINT64_FORMAT ", bytes boundary %" G_GINT64_FORMAT, ogg->offset, boundary);
    ...
    while (TRUE)
    {
        ...
        if (more < 0)
        {
            /* skipped n bytes */
            ogg->offset -= more;
            GST_LOG_OBJECT(ogg, "skipped %ld bytes, offset %" G_GINT64_FORMAT, more, ogg->offset);
        }
        else if (more == 0)
        {
            /* we need more data */
        ...
        else
        {
            ...
            GST_LOG_OBJECT(ogg, "got page at %" G_GINT64_FORMAT ", serial %08x, end at %" G_GINT64_FORMAT ", granule %" G_GINT64_FORMAT, res_offset, ogg_page_serialno(og), ogg->offset, (gint64) ogg_page_granulepos(og));
            break;
        }
    }
...
boundary_reached:
    {
        GST_LOG_OBJECT(ogg, "offset %" G_GINT64_FORMAT " >= end_offset %" G_GINT64_FORMAT, ogg->offset, end_offset);
        return GST_FLOW_LIMIT;
    }
}
 ...
 */
static GstFlowReturn gst_ogg_demux_get_prev_page(GstOggDemux * ogg, ogg_page * og, gint64 * offset)
{
    GstFlowReturn ret;
    ...
    while (cur_offset == -1)
    {
        ...
        while (ogg->offset < end)
        {
            ...
            ret = gst_ogg_demux_get_next_page(ogg, og, end - ogg->offset, &new_offset);
    ...
    if (ret != GST_FLOW_OK)
    {
        GST_WARNING_OBJECT(ogg, "can't get last page at %" G_GINT64_FORMAT, cur_offset);
...
static gboolean gst_ogg_demux_deactivate_current_chain(GstOggDemux * ogg)
{
    gint i;
...
static gboolean gst_ogg_demux_activate_chain(GstOggDemux * ogg, GstOggChain * chain, GstEvent * event)
{
    ...
    if (event)
    {
        ...
        gst_element_found_tags(GST_ELEMENT_CAST(ogg), gst_tag_list_new_full(GST_TAG_CONTAINER_FORMAT, "Ogg", NULL));
    }
...
static gboolean do_binary_search(GstOggDemux * ogg, GstOggChain * chain, gint64 begin, gint64 end, gint64 begintime, gint64 endtime, gint64 target, gint64 * offset)
{
    gint64 best;
    GstFlowReturn ret;
    gint64 result = 0;
    ...
    GST_DEBUG_OBJECT(ogg, "chain offset %" G_GINT64_FORMAT ", end offset %" G_GINT64_FORMAT, begin, end);
    GST_DEBUG_OBJECT(ogg, "chain begin time %" GST_TIME_FORMAT ", end time %" GST_TIME_FORMAT, GST_TIME_ARGS(begintime), GST_TIME_ARGS(endtime));
    ...
    while (begin < end)
    {
        ...
        while (begin < end)
        {
            ...
            GST_DEBUG_OBJECT(ogg, "after seek, bisect %" G_GINT64_FORMAT ", begin %" G_GINT64_FORMAT ", end %" G_GINT64_FORMAT, bisect, begin, end);

            ret = gst_ogg_demux_get_next_page(ogg, &og, end - ogg->offset, &result);
            GST_LOG_OBJECT(ogg, "looking for next page returned %" G_GINT64_FORMAT, result);
            ...
            else if (ret == GST_FLOW_OK)
            {
                ...
                /* get the granulepos */
                GST_LOG_OBJECT(ogg, "found next ogg page at %" G_GINT64_FORMAT, result);
                ...
                granuletime = gst_ogg_stream_get_end_time_for_granulepos(&pad->map, granulepos);
                if (granuletime < pad->start_time)
                    continue;

                GST_LOG_OBJECT(ogg, "granulepos %" G_GINT64_FORMAT " maps to time %" GST_TIME_FORMAT, granulepos, GST_TIME_ARGS(granuletime));

                granuletime -= pad->start_time;
                granuletime += chain->begin_time;

                GST_DEBUG_OBJECT(ogg, "found page with granule %" G_GINT64_FORMAT " and time %" GST_TIME_FORMAT, granulepos, GST_TIME_ARGS(granuletime));
...
static gboolean do_index_search(GstOggDemux * ogg, GstOggChain * chain, gint64 begin, gint64 end, gint64 begintime, gint64 endtime, gint64 target, gint64 * p_offset, gint64 * p_timestamp)
{
    guint i;
    ...
    for (i = 0; i < chain->streams->len; i++)
    {
        ...
        {
            GST_INFO("found %" G_GUINT64_FORMAT " at offset %" G_GUINT64_FORMAT, timestamp, offset);
 ...
 */
static gboolean gst_ogg_demux_do_seek(GstOggDemux * ogg, GstSegment * segment, gboolean accurate, gboolean keyframe, GstOggChain ** rchain)
{
    guint64 position;
    ...
    if (!do_binary_search(ogg, chain, begin, end, begintime, endtime, target, &best))
        goto seek_error;
    ...
    while (TRUE)
    {
        ...
        if (first_parsed_page)
        {
            ...
            granule_time = gst_ogg_stream_get_end_time_for_granulepos(&pad->map, granulepos);
            for (i = 0; i < len; i++)
            {
                GstOggPad *stream = g_array_index(chain->streams, GstOggPad *, i);
        ...
        if (segment->rate < 0.0)
        {
            ...
            GST_LOG_OBJECT(ogg, "looking at page with ts %" GST_TIME_FORMAT ", target %" GST_TIME_FORMAT, GST_TIME_ARGS(granule_time), GST_TIME_ARGS(target));
        ...
        pad->keyframe_granule = gst_ogg_stream_granulepos_to_key_granule(&pad->map, granulepos);
        GST_LOG_OBJECT(ogg, "marking stream granule %" G_GINT64_FORMAT, pad->keyframe_granule);

        /* get time of the keyframe */
        keyframe_time = gst_ogg_stream_granule_to_time(&pad->map, pad->keyframe_granule);
        GST_LOG_OBJECT(ogg, "stream %08lx granule time %" GST_TIME_FORMAT, pad->map.serialno, GST_TIME_ARGS(keyframe_time));
...
next:
    ...
    if (keytarget != target)
    {
        GST_LOG_OBJECT(ogg, "final seek to target %" GST_TIME_FORMAT, GST_TIME_ARGS(keytarget));

        /* last step, seek to the location of the keyframe */
        if (!do_binary_search(ogg, chain, begin, end, begintime, endtime, keytarget, &best))
            goto seek_error;
    }
    else
    {
...
static gboolean gst_ogg_demux_perform_seek_pull(GstOggDemux * ogg, GstEvent * event)
{
    GstOggChain *chain = NULL;
    gboolean res;
    ...
    if (ogg->segment_running && !flush)
    {
        ...
        if ((chain = ogg->current_chain))
        {
            ...
            newseg = gst_event_new_new_segment(TRUE, ogg->segment.rate, GST_FORMAT_TIME, ogg->segment.start + chain_start, ogg->segment.last_stop + chain_start, ogg->segment.time);
    ...
    if (event)
    {
        gst_segment_set_seek(&ogg->segment, rate, format, flags, cur_type, cur, stop_type, stop, &update);
    }

    GST_DEBUG_OBJECT(ogg, "segment positions set to %" GST_TIME_FORMAT "-%" GST_TIME_FORMAT, GST_TIME_ARGS(ogg->segment.start), GST_TIME_ARGS(ogg->segment.stop));
    ...
    /* now we have a new position, prepare for streaming again */
    {
        ...
        if (ogg->segment.rate >= 0.0)
            event = gst_event_new_new_segment(FALSE, ogg->segment.rate, ogg->segment.format, last_stop, stop, ogg->segment.time);
        else
            event = gst_event_new_new_segment(FALSE, ogg->segment.rate, ogg->segment.format, start, last_stop, ogg->segment.time);
        ...
        if (ogg->segment.flags & GST_SEEK_FLAG_SEGMENT)
        {
            ...
            message = gst_message_new_segment_start(GST_OBJECT(ogg), GST_FORMAT_TIME, ogg->segment.last_stop);
         ...
         * flush. */
        gst_pad_start_task(ogg->sinkpad, (GstTaskFunction) gst_ogg_demux_loop, ogg->sinkpad);
    }
...
static gboolean gst_ogg_demux_perform_seek_push(GstOggDemux * ogg, GstEvent * event)
{
    gint bitrate;
    gboolean res = TRUE;
    GstFormat format;
    ...
    if (do_index_search(ogg, chain, 0, -1, 0, -1, start, &best, &best_time))
    {
        ...
        GST_DEBUG_OBJECT(ogg, "found offset %" G_GINT64_FORMAT " with time %" G_GUINT64_FORMAT, best, best_time);
        start = best;
    }
    ...
    if (res)
    {
        GST_DEBUG_OBJECT(ogg, "seeking to %" G_GINT64_FORMAT " - %" G_GINT64_FORMAT, start, stop);
        /* do seek */
        sevent = gst_event_new_seek(rate, GST_FORMAT_BYTES, flags, start_type, start, stop_type, stop);
 ...
 */
static GstFlowReturn gst_ogg_demux_bisect_forward_serialno(GstOggDemux * ogg, gint64 begin, gint64 searched, gint64 end, GstOggChain * chain, glong m)
{
    ...
    GST_LOG_OBJECT(ogg, "bisect begin: %" G_GINT64_FORMAT ", searched: %" G_GINT64_FORMAT ", end %" G_GINT64_FORMAT ", chain: %p", begin, searched, end, chain);
    ...
    {
        ret = gst_ogg_demux_bisect_forward_serialno(ogg, next, ogg->offset, end, nextchain, m + 1);
        if (ret != GST_FLOW_OK)
            goto done;
    }
 ...
 * page in the chain.
 */
static GstFlowReturn gst_ogg_demux_read_chain(GstOggDemux * ogg, GstOggChain ** res_chain)
{
    GstFlowReturn ret;
    GstOggChain *chain = NULL;
    ...
    while (TRUE)
    {
        ...
        {
            GST_WARNING_OBJECT(ogg, "found serial %08lx BOS page twice, ignoring", serial);
            continue;
        }
    ...
    while (!done)
    {
        ...
        for (i = 0; i < chain->streams->len; i++)
        {
            ...
            GST_LOG_OBJECT(ogg, "serial %08lx time %" GST_TIME_FORMAT, pad->map.serialno, GST_TIME_ARGS(pad->start_time));
            ...
            if (pad->map.serialno == serial)
            {
                ...
                if (!pad->map.is_skeleton && pad->start_time == -1 && ogg_page_eos(&op))
                {
...
static GstFlowReturn gst_ogg_demux_read_end_chain(GstOggDemux * ogg, GstOggChain * chain)
{
    ...
    if (last_pad)
    {
        chain->segment_stop = gst_ogg_stream_get_end_time_for_granulepos(&last_pad->map, last_granule);
    }
    else
    {
        chain->segment_stop = GST_CLOCK_TIME_NONE;
 ...
 */
static GstOggPad *gst_ogg_demux_find_pad(GstOggDemux * ogg, glong serialno)
{
    GstOggPad *pad;
    gint i;
 ...
 */
static GstOggChain *gst_ogg_demux_find_chain(GstOggDemux * ogg, glong serialno)
{
    GstOggPad *pad;
...
static void gst_ogg_demux_collect_info(GstOggDemux * ogg)
{
    gint i;
 ...
 */
static GstFlowReturn gst_ogg_demux_find_chains(GstOggDemux * ogg)
{
    ogg_page og;
    GstPad *peer;
    GstFormat format;
    ...
    if (!gst_ogg_chain_has_stream(chain, serialno))
    {
        ...
        ret = gst_ogg_demux_bisect_forward_serialno(ogg, 0, 0, ogg->length, chain, 0);
    }
    else
    {
        /* we still call this function here but with an empty range so that
         * we can reuse the setup code in this routine. */
        ret = gst_ogg_demux_bisect_forward_serialno(ogg, 0, ogg->length, ogg->length, chain, 0);
    }
    if (ret != GST_FLOW_OK)
        goto done;
...
static GstFlowReturn gst_ogg_demux_handle_page(GstOggDemux * ogg, ogg_page * page)
{
    GstOggPad *pad;
    gint64 granule;
    glong serialno;
    ...
    GST_LOG_OBJECT(ogg, "processing ogg page (serial %08lx, pageno %ld, granulepos %" G_GINT64_FORMAT ", bos %d)", serialno, ogg_page_pageno(page), granule, ogg_page_bos(page));
    ...
    if (ogg_page_bos(page))
    {
        ...
        if (chain)
        {
            ...
            /* create the newsegment event we are going to send out */
            event = gst_event_new_new_segment(FALSE, ogg->segment.rate, GST_FORMAT_TIME, start, chain->segment_stop, chain->begin_time);
            gst_event_set_seqnum(event, ogg->seqnum);

            GST_DEBUG_OBJECT(ogg, "segment: start %" GST_TIME_FORMAT ", stop %" GST_TIME_FORMAT ", time %" GST_TIME_FORMAT, GST_TIME_ARGS(start), GST_TIME_ARGS(chain->segment_stop), GST_TIME_ARGS(chain->begin_time));
        ...
        else
        {
            ...
            if (ogg->building_chain == NULL)
            {
                ...
                GST_DEBUG_OBJECT(ogg, "new chain, begin time %" GST_TIME_FORMAT, GST_TIME_ARGS(chain_time));
    ...
    else
    {
        ...
        GST_ELEMENT_WARNING(ogg, STREAM, DECODE, (NULL), ("unknown ogg pad for serial %08lx detected", serialno));
        return GST_FLOW_OK;
    }
    return result;
...
unknown_chain:
    {
        GST_ELEMENT_ERROR(ogg, STREAM, DECODE, (NULL), ("unknown ogg chain for serial %08lx detected", serialno));
        return GST_FLOW_ERROR;
    }
}
...
static GstFlowReturn gst_ogg_demux_loop_forward(GstOggDemux * ogg)
{
    GstFlowReturn ret;
    GstBuffer *buffer;
    ...
    {
        GST_LOG_OBJECT(ogg, "no more data to pull %" G_GINT64_FORMAT " == %" G_GINT64_FORMAT, ogg->offset, ogg->length);
        ret = GST_FLOW_UNEXPECTED;
        goto done;
    }
 ...
 */
static GstFlowReturn gst_ogg_demux_loop_reverse(GstOggDemux * ogg)
{
    GstFlowReturn ret;
    ogg_page page;
    gint64 offset;
    ...
    if (ogg->offset == 0)
    {
        GST_LOG_OBJECT(ogg, "no more data to pull %" G_GINT64_FORMAT " == 0", ogg->offset);
        ret = GST_FLOW_UNEXPECTED;
        goto done;
    }
...
static void gst_ogg_demux_sync_streams(GstOggDemux * ogg)
{
    ...
    for (i = 0; i < chain->streams->len; i++)
    {
        ...
        if (stream->map.is_sparse && stream->last_stop != GST_CLOCK_TIME_NONE)
        {
            ...
            {
                GST_DEBUG_OBJECT(stream, "synchronizing stream with others by " "advancing time from %" GST_TIME_FORMAT " to %" GST_TIME_FORMAT, GST_TIME_ARGS(stream->last_stop), GST_TIME_ARGS(cur));
                stream->last_stop = cur;
                /* advance stream time (FIXME: is this right, esp. time_pos?) */
                gst_pad_push_event(GST_PAD_CAST(stream), gst_event_new_new_segment(TRUE, ogg->segment.rate, GST_FORMAT_TIME, stream->last_stop, -1, stream->last_stop));
            }
        }
    }
}
...
seek_failed:
    {
        GST_ELEMENT_ERROR(ogg, STREAM, DEMUX, (NULL), ("failed to start demuxing ogg"));
        ret = GST_FLOW_ERROR;
        goto pause;
    }
pause:
    {
        ...
        if (ret == GST_FLOW_UNEXPECTED)
        {
            ...
            if (ogg->segment.flags & GST_SEEK_FLAG_SEGMENT)
            {
                ...
                message = gst_message_new_segment_done(GST_OBJECT(ogg), GST_FORMAT_TIME, stop);
        ...
        {
            GST_ELEMENT_ERROR(ogg, STREAM, FAILED, (_("Internal data stream error.")), ("stream stopped, reason %s", reason));
            event = gst_event_new_eos();
        }
...
static void gst_ogg_demux_clear_chains(GstOggDemux * ogg)
{
    gint i;
...
static gboolean gst_ogg_demux_sink_activate_pull(GstPad * sinkpad, gboolean active)
{
    ...
    if (active)
    {
        ...
        return gst_pad_start_task(sinkpad, (GstTaskFunction) gst_ogg_demux_loop, sinkpad);
    }
    else
    {
...
gboolean gst_ogg_demux_plugin_init(GstPlugin * plugin)
{
    ...
    GST_DEBUG_CATEGORY_INIT(gst_ogg_demux_setup_debug, "oggdemux_setup", 0, "ogg demuxer setup stage when parsing pipeline");
...
static void gst_ogg_print(GstOggDemux * ogg)
{
    ...
    GST_INFO_OBJECT(ogg, " total time: %" GST_TIME_FORMAT, GST_TIME_ARGS(ogg->total_time));
    ...
    for (i = 0; i < ogg->chains->len; i++)
    {
        ...
        GST_INFO_OBJECT(ogg, "  offset: %" G_GINT64_FORMAT " - %" G_GINT64_FORMAT, chain->offset, chain->end_offset);
        GST_INFO_OBJECT(ogg, "  begin time: %" GST_TIME_FORMAT, GST_TIME_ARGS(chain->begin_time));
        GST_INFO_OBJECT(ogg, "  total time: %" GST_TIME_FORMAT, GST_TIME_ARGS(chain->total_time));
        GST_INFO_OBJECT(ogg, "  segment start: %" GST_TIME_FORMAT, GST_TIME_ARGS(chain->segment_start));
        GST_INFO_OBJECT(ogg, "  segment stop:  %" GST_TIME_FORMAT, GST_TIME_ARGS(chain->segment_stop));
        ...
        for (j = 0; j < chain->streams->len; j++)
        {
            ...
            GST_INFO_OBJECT(ogg, "   start time:       %" GST_TIME_FORMAT, GST_TIME_ARGS(stream->start_time));
        }
    }
}
