...
static GstStaticPadTemplate sink_factory = GST_STATIC_PAD_TEMPLATE("sink_%d",
                                                                   ...
                                                                   GST_STATIC_CAPS("video/x-theora; "
                                                                                   ...
                                                                                   "video/x-smoke; video/x-vp8; text/x-cmml, encoded = (boolean) TRUE; " "subtitle/x-kate; application/x-kate"));
...
static GstFlowReturn gst_ogg_mux_collected(GstCollectPads * pads, GstOggMux * ogg_mux);
static gboolean gst_ogg_mux_handle_src_event(GstPad * pad, GstEvent * event);
static GstPad *gst_ogg_mux_request_new_pad(GstElement * element, GstPadTemplate * templ, const gchar * name);
static void gst_ogg_mux_release_pad(GstElement * element, GstPad * pad);

static void gst_ogg_mux_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_ogg_mux_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
static GstStateChangeReturn gst_ogg_mux_change_state(GstElement * element, GstStateChange transition);
...
GType gst_ogg_mux_get_type(void)
{
    ...
    if (G_UNLIKELY(ogg_mux_type == 0))
    {
        ...
        ogg_mux_type = g_type_register_static(GST_TYPE_ELEMENT, "GstOggMux", &ogg_mux_info, 0);
...
static void gst_ogg_mux_base_init(gpointer g_class)
{
    ...
    gst_element_class_set_details_simple(element_class, "Ogg muxer", "Codec/Muxer", "mux ogg streams (info about ogg: http://xiph.org)", "Wim Taymans <wim@fluendo.com>");
}
...
static void gst_ogg_mux_class_init(GstOggMuxClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, ARG_MAX_DELAY, g_param_spec_uint64("max-delay", "Max delay", "Maximum delay in multiplexing streams", 0, G_MAXUINT64, DEFAULT_MAX_DELAY, (GParamFlags) G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, ARG_MAX_PAGE_DELAY,
                                    g_param_spec_uint64("max-page-delay", "Max page delay", "Maximum delay for sending out a page", 0, G_MAXUINT64, DEFAULT_MAX_PAGE_DELAY, (GParamFlags) G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
#if 0
static const GstEventMask *gst_ogg_mux_get_sink_event_masks(GstPad * pad)
{
...
static void gst_ogg_mux_clear(GstOggMux * ogg_mux)
{
...
static void gst_ogg_mux_init(GstOggMux * ogg_mux)
{
    ...
    ogg_mux->srcpad = gst_pad_new_from_template(gst_element_class_get_pad_template(klass, "src"), "src");
    ...
    gst_collect_pads_set_function(ogg_mux->collect, (GstCollectPadsFunction) GST_DEBUG_FUNCPTR(gst_ogg_mux_collected), ogg_mux);
...
static void gst_ogg_mux_ogg_pad_destroy_notify(GstCollectData * data)
{
...
static GstPadLinkReturn gst_ogg_mux_sinkconnect(GstPad * pad, GstPad * peer)
{
    GstOggMux *ogg_mux;
...
static gboolean gst_ogg_mux_sink_event(GstPad * pad, GstEvent * event)
{
    GstOggMux *ogg_mux = GST_OGG_MUX(gst_pad_get_parent(pad));
    GstOggPadData *ogg_pad = (GstOggPadData *) gst_pad_get_element_private(pad);
    gboolean ret;

    GST_DEBUG("Got %s event on pad %s:%s", GST_EVENT_TYPE_NAME(event), GST_DEBUG_PAD_NAME(pad));
...
static GstPad *gst_ogg_mux_request_new_pad(GstElement * element, GstPadTemplate * templ, const gchar * req_name)
{
    GstOggMux *ogg_mux;
    GstPad *newpad;
...

    {
         ...
         * keep track of its status */
        {
            ...
            oggpad = (GstOggPadData *) gst_collect_pads_add_pad_full(ogg_mux->collect, newpad, sizeof(GstOggPadData), gst_ogg_mux_ogg_pad_destroy_notify);
            ogg_mux->active_pads++;
            ...
            gst_pad_set_event_function(newpad, GST_DEBUG_FUNCPTR(gst_ogg_mux_sink_event));
        }
    }
...
static GstBuffer *gst_ogg_mux_buffer_from_page(GstOggMux * mux, ogg_page * page, gboolean delta)
{
    GstBuffer *buffer;
    ...
    memcpy(GST_BUFFER_DATA(buffer) + page->header_len, page->body, page->body_len);
    ...
    GST_LOG_OBJECT(mux, GST_GP_FORMAT " created buffer %p from ogg page", GST_GP_CAST(ogg_page_granulepos(page)), buffer);

    return buffer;
}

static GstFlowReturn gst_ogg_mux_push_buffer(GstOggMux * mux, GstBuffer * buffer)
{
    GstCaps *caps;
    ...
    {
        if (mux->last_ts != GST_CLOCK_TIME_NONE && GST_BUFFER_TIMESTAMP(buffer) < mux->last_ts)
 ...
 * timestamp */
static gboolean gst_ogg_mux_dequeue_page(GstOggMux * mux, GstFlowReturn * flowret)
{
    GSList *walk;
    ...
    *flowret = GST_FLOW_OK;
    ...
    while (walk)
    {
        ...
        if (pad->pagebuffers->length == 0)
        {
            if (pad->eos)
            {
                GST_LOG_OBJECT(pad->collect.pad, "pad is EOS, skipping for dequeue decision");
            }
            else
            {
                GST_LOG_OBJECT(pad->collect.pad, "no pages in this queue, can't dequeue");
                return FALSE;
            }
        }
        else
        {
            ...
            if (!valid)
            {
                GST_LOG_OBJECT(pad->collect.pad, "No page timestamps in queue, can't dequeue");
                return FALSE;
            }
        }
    ...
    while (walk)
    {
        ...
        if (buf)
        {
            ...
            {
                GST_LOG_OBJECT(mux, "no oldest yet, taking buffer %p from pad %" GST_PTR_FORMAT " with gp time %" GST_TIME_FORMAT, buf, pad->collect.pad, GST_TIME_ARGS(GST_BUFFER_OFFSET(buf)));
            ...
            else
            {
                ...
                {
                    GST_LOG_OBJECT(mux, "older buffer %p, taking from pad %" GST_PTR_FORMAT " with gp time %" GST_TIME_FORMAT, buf, pad->collect.pad, GST_TIME_ARGS(GST_BUFFER_OFFSET(buf)));
                    oldest = GST_BUFFER_OFFSET(buf);
                    opad = pad;
                }
            }
    ...
    if (oldest != GST_CLOCK_TIME_NONE)
    {
        ...
        GST_LOG_OBJECT(opad->collect.pad, GST_GP_FORMAT " pushing oldest page buffer %p (granulepos time %" GST_TIME_FORMAT ")", GST_BUFFER_OFFSET_END(buf), buf, GST_TIME_ARGS(GST_BUFFER_OFFSET(buf)));
        *flowret = gst_ogg_mux_push_buffer(mux, buf);
        ret = TRUE;
    }
 ...
 * counting.
 */
static GstFlowReturn gst_ogg_mux_pad_queue_page(GstOggMux * mux, GstOggPadData * pad, ogg_page * page, gboolean delta)
{
    GstFlowReturn ret;
    ...
    GST_LOG_OBJECT(pad->collect.pad, GST_GP_FORMAT
                   ...
                   "), %d page buffers queued", GST_GP_CAST(ogg_page_granulepos(page)), buffer, GST_TIME_ARGS(GST_BUFFER_OFFSET(buffer)), GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buffer)), g_queue_get_length(pad->pagebuffers));
 ...
 * of muxed pages
 */
static gint gst_ogg_mux_compare_pads(GstOggMux * ogg_mux, GstOggPadData * first, GstOggPadData * second)
{
...
static GstOggPadData *gst_ogg_mux_queue_pads(GstOggMux * ogg_mux)
{
    ...
    while (walk)
    {
        ...
        if (pad->buffer == NULL)
        {
            ...
            if (pad->buffer == NULL)
            {
                GST_LOG_OBJECT(data->pad, "shifting buffer %" GST_PTR_FORMAT, pad->next_buffer);
            ...
            if (buf != NULL)
            {
                if (ogg_mux->delta_pad == NULL && GST_BUFFER_FLAG_IS_SET(buf, GST_BUFFER_FLAG_DELTA_UNIT))
                ...
                if (pad->state == GST_OGG_PAD_STATE_CONTROL)
                {
                    ...
                    if (incaps)
                    {
                        GST_DEBUG_OBJECT(ogg_mux, "got incaps buffer in control state, ignoring");
                        /* just ignore */
                        gst_buffer_unref(buf);
                        buf = NULL;
                    }
                    ...
                    {
                        GST_DEBUG_OBJECT(ogg_mux, "got data buffer in control state, switching " "to data mode");
        ...
        if (pad->buffer || pad->next_buffer)
        {
            ...
            {
                GST_LOG_OBJECT(data->pad, "new best pad, with buffers %" GST_PTR_FORMAT " and %" GST_PTR_FORMAT, pad->buffer, pad->next_buffer);
...
static GList *gst_ogg_mux_get_headers(GstOggPadData * pad)
{
    GList *res = NULL;
...
static GstCaps *gst_ogg_mux_set_header_on_caps(GstCaps * caps, GList * buffers)
{
    GstStructure *structure;
    GValue array = { 0 };
    GList *walk = buffers;
 ...
 */
static GstFlowReturn gst_ogg_mux_send_headers(GstOggMux * mux)
{
    GSList *walk;
    GList *hbufs, *hwalk;
    GstCaps *caps;
    ...
    while (walk)
    {
        ...
        else
        {
            ...
            GST_OBJECT_LOCK(pad);
            g_critical("No headers or buffers on pad %s:%s", GST_DEBUG_PAD_NAME(pad));
        ...
        GST_LOG_OBJECT(mux, "swapped out page with mime type %s", gst_structure_get_name(structure));
    ...
    while (walk)
    {
        ...
        GST_LOG_OBJECT(mux, "looping over headers for pad %s:%s", GST_DEBUG_PAD_NAME(thepad));
        ...
        while (hwalk)
        {
            ...
            if (hwalk == NULL)
            {
                GST_LOG_OBJECT(mux, "flushing page as packet %" G_GUINT64_FORMAT " is first or " "last packet", pad->packetno);
                while (ogg_stream_flush(&pad->stream, &page))
                {
 ...
 *    pads are at EOS)
 */
static GstFlowReturn gst_ogg_mux_process_best_pad(GstOggMux * ogg_mux, GstOggPadData * best)
{
    ...
    GST_LOG_OBJECT(ogg_mux, "best pad %" GST_PTR_FORMAT ", currently pulling from %" GST_PTR_FORMAT, best->collect.pad, ogg_mux->pulling);
    ...
    if (ogg_mux->pulling && best && ogg_mux->pulling != best && ogg_mux->pulling->buffer)
    {
        ...
        if (last_ts > ogg_mux->next_ts + ogg_mux->max_delay)
        {
            ...
            GST_LOG_OBJECT(pad->collect.pad, GST_GP_FORMAT " stored packet %" G_GINT64_FORMAT " will make page too long, flushing", GST_BUFFER_OFFSET_END(pad->buffer), (gint64) pad->stream.packetno);
    ...
    if (ogg_mux->pulling == NULL)
    {
        ...
        if (ogg_mux->pulling != NULL)
        {
            ...
            GST_LOG_OBJECT(ogg_mux->pulling->collect.pad, "updated times, next ts %" GST_TIME_FORMAT, GST_TIME_ARGS(ogg_mux->next_ts));
        }
        else
        {
    ...
    if (ogg_mux->pulling != NULL)
    {
        ...
        if (ogg_mux->pulling->timestamp == GST_CLOCK_TIME_NONE)
        {
            ...
            GST_LOG_OBJECT(ogg_mux->pulling->collect.pad, "updated pad timestamp to %" GST_TIME_FORMAT, GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buf)));
        }
        /* create a packet from the buffer */
        packet.packet = GST_BUFFER_DATA(buf);
        packet.bytes = GST_BUFFER_SIZE(buf);
        ...
        packet.b_o_s = (pad->packetno == 0);
        packet.packetno = pad->packetno++;
        GST_LOG_OBJECT(pad->collect.pad, GST_GP_FORMAT " packet %" G_GINT64_FORMAT " (%ld bytes) created from buffer", GST_GP_CAST(packet.granulepos), (gint64) packet.packetno, packet.bytes);
        ...
        if (force_flush)
        {
            GST_LOG_OBJECT(pad->collect.pad, GST_GP_FORMAT " forced flush of page before this packet", GST_BUFFER_OFFSET_END(pad->buffer));
            while (ogg_stream_flush(&pad->stream, &page))
            {
                /* end time of this page is the timestamp of the next buffer */
                ogg_mux->pulling->timestamp_end = GST_BUFFER_TIMESTAMP(pad->buffer);
        ...
        GST_LOG_OBJECT(pad->collect.pad, GST_GP_FORMAT " packet %" G_GINT64_FORMAT ", gp time %" GST_TIME_FORMAT ", timestamp %" GST_TIME_FORMAT " packetin'd", granulepos, (gint64) packet.packetno, GST_TIME_ARGS(gp_time), GST_TIME_ARGS(timestamp));
        ...
        if (ogg_stream_pageout(&pad->stream, &page) > 0)
        {
            ...
            GST_LOG_OBJECT(pad->collect.pad, GST_GP_FORMAT " packet %" G_GINT64_FORMAT ", time %" GST_TIME_FORMAT ") caused new page", granulepos, (gint64) packet.packetno, GST_TIME_ARGS(timestamp));
            GST_LOG_OBJECT(pad->collect.pad, GST_GP_FORMAT " new page %ld", GST_GP_CAST(ogg_page_granulepos(&page)), pad->stream.pageno);
            ...
            if (ogg_page_granulepos(&page) == granulepos)
            {
                ...
                GST_LOG_OBJECT(pad->collect.pad, GST_GP_FORMAT " packet finishes on current page, updating gp time to %" GST_TIME_FORMAT, granulepos, GST_TIME_ARGS(gp_time));
                pad->gp_time = gp_time;
            }
            else
            {
                GST_LOG_OBJECT(pad->collect.pad, GST_GP_FORMAT " packet spans beyond current page, keeping old gp time %" GST_TIME_FORMAT, granulepos, GST_TIME_ARGS(pad->gp_time));
            }
        ...
        {
            pad->gp_time = gp_time;
            GST_LOG_OBJECT(pad->collect.pad, "Updated running gp time of pad %" GST_PTR_FORMAT " to %" GST_TIME_FORMAT, pad->collect.pad, GST_TIME_ARGS(gp_time));
        }
    }
 ...
 */
static gboolean all_pads_eos(GstCollectPads * pads)
{
    GSList *walk;
...
static void gst_ogg_mux_init_collectpads(GstCollectPads * collect)
{
    GSList *walk;
...
static void gst_ogg_mux_clear_collectpads(GstCollectPads * collect)
{
    GSList *walk;
...
gboolean gst_ogg_mux_plugin_init(GstPlugin * plugin)
{
    ...
    return gst_element_register(plugin, "oggmux", GST_RANK_PRIMARY, GST_TYPE_OGG_MUX);
}
