...
static void gst_gnome_vfs_src_uri_handler_init(gpointer g_iface, gpointer iface_data);

static void gst_gnome_vfs_src_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_gnome_vfs_src_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
...
static GstFlowReturn gst_gnome_vfs_src_create(GstBaseSrc * basesrc, guint64 offset, guint size, GstBuffer ** buffer);
...
GType gst_gnome_vfs_src_get_type(void)
{
    ...
    if (!gnomevfssrc_type)
    {
        ...
        gnomevfssrc_type = g_type_register_static(GST_TYPE_BASE_SRC, "GstGnomeVFSSrc", &gnomevfssrc_info, 0);
        g_type_add_interface_static(gnomevfssrc_type, GST_TYPE_URI_HANDLER, &urihandler_info);
    }
...
static void gst_gnome_vfs_src_base_init(gpointer g_class)
{
    ...
    gst_element_class_set_details_simple(element_class, "GnomeVFS Source", "Source/File", "Read from any GnomeVFS-supported file", "Bastien Nocera <hadess@hadess.net>, " "GStreamer maintainers <gstreamer-devel@lists.sourceforge.net>");
...
static void gst_gnome_vfs_src_class_init(GstGnomeVFSSrcClass * klass)
{
    ...
    /* properties */
    gst_element_class_install_std_props(GST_ELEMENT_CLASS(klass), "location", ARG_LOCATION, G_PARAM_READWRITE, NULL);
    g_object_class_install_property(gobject_class, ARG_HANDLE, g_param_spec_boxed("handle", "GnomeVFSHandle", "Handle for GnomeVFS", GST_TYPE_GNOME_VFS_HANDLE, GST_PARAM_MUTABLE_READY | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    /* icecast stuff */
    g_object_class_install_property(gobject_class, ARG_IRADIO_MODE, g_param_spec_boolean("iradio-mode", "iradio-mode", "Enable internet radio mode (extraction of shoutcast/icecast metadata)", FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, ARG_IRADIO_NAME, g_param_spec_string("iradio-name", "iradio-name", "Name of the stream", NULL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, ARG_IRADIO_GENRE, g_param_spec_string("iradio-genre", "iradio-genre", "Genre of the stream", NULL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, ARG_IRADIO_URL, g_param_spec_string("iradio-url", "iradio-url", "Homepage URL for radio stream", NULL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, ARG_IRADIO_TITLE, g_param_spec_string("iradio-title", "iradio-title", "Name of currently playing song", NULL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    ...
    gstbasesrc_class->is_seekable = GST_DEBUG_FUNCPTR(gst_gnome_vfs_src_is_seekable);
    gstbasesrc_class->check_get_range = GST_DEBUG_FUNCPTR(gst_gnome_vfs_src_check_get_range);
...
static GstURIType gst_gnome_vfs_src_uri_get_type(void)
{
    return GST_URI_SRC;
}

static gchar **gst_gnome_vfs_src_uri_get_protocols(void)
{
    return gst_gnomevfs_get_supported_uris();
}

static const gchar *gst_gnome_vfs_src_uri_get_uri(GstURIHandler * handler)
{
...
static gboolean gst_gnome_vfs_src_uri_set_uri(GstURIHandler * handler, const gchar * uri)
{
    GstGnomeVFSSrc *src = GST_GNOME_VFS_SRC(handler);
...
static void gst_gnome_vfs_src_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case ARG_HANDLE:
            if (GST_STATE(src) == GST_STATE_NULL || GST_STATE(src) == GST_STATE_READY)
            {
                if (src->uri)
                {
                    gnome_vfs_uri_unref(src->uri);
...
static char *gst_gnome_vfs_src_unicodify(const char *str)
{
...
static void gst_gnome_vfs_src_send_additional_headers_callback(gconstpointer in, gsize in_size, gpointer out, gsize out_size, gpointer callback_data)
{
...
static void gst_gnome_vfs_src_received_headers_callback(gconstpointer in, gsize in_size, gpointer out, gsize out_size, gpointer callback_data)
{
    GList *i;
    gint icy_metaint;
    GstGnomeVFSSrc *src = GST_GNOME_VFS_SRC(callback_data);
    GnomeVFSModuleCallbackReceivedHeadersIn *in_args = (GnomeVFSModuleCallbackReceivedHeadersIn *) in;
        ...
        {                        /* ugh */
            if (sscanf(data + 12, "%d", &icy_metaint) == 1)
            {
                if (icy_metaint > 0)
                {
                    ...
                    icy_caps = gst_caps_new_simple("application/x-icy", "metadata-interval", G_TYPE_INT, icy_metaint, NULL);
...
static void gst_gnome_vfs_src_push_callbacks(GstGnomeVFSSrc * src)
{
    if (src->http_callbacks_pushed)
        return;

    GST_DEBUG_OBJECT(src, "pushing callbacks");
    gnome_vfs_module_callback_push(GNOME_VFS_MODULE_CALLBACK_HTTP_SEND_ADDITIONAL_HEADERS, gst_gnome_vfs_src_send_additional_headers_callback, src, NULL);
    gnome_vfs_module_callback_push(GNOME_VFS_MODULE_CALLBACK_HTTP_RECEIVED_HEADERS, gst_gnome_vfs_src_received_headers_callback, src, NULL);

    src->http_callbacks_pushed = TRUE;
}

static void gst_gnome_vfs_src_pop_callbacks(GstGnomeVFSSrc * src)
{
    if (!src->http_callbacks_pushed)
        return;
...
static GstFlowReturn gst_gnome_vfs_src_create(GstBaseSrc * basesrc, guint64 offset, guint size, GstBuffer ** buffer)
{
    ...
    GST_DEBUG("now at %" G_GINT64_FORMAT ", reading from %" G_GUINT64_FORMAT ", size %u", src->curoffset, offset, size);
    ...
    while (todo > 0)
    {
        ...
        if (G_UNLIKELY(res == GNOME_VFS_ERROR_EOF || (res == GNOME_VFS_OK && readbytes == 0)))
            goto eos;
...
seek_failed:
    {
        GST_ELEMENT_ERROR(src, RESOURCE, SEEK, (NULL), ("Failed to seek to requested position %" G_GINT64_FORMAT ": %s", offset, gnome_vfs_result_to_string(res)));
        return GST_FLOW_ERROR;
    }
cannot_seek:
    {
        GST_ELEMENT_ERROR(src, RESOURCE, SEEK, (NULL), ("Requested seek from %" G_GINT64_FORMAT " to %" G_GINT64_FORMAT " on non-seekable stream", src->curoffset, offset));
        return GST_FLOW_ERROR;
    }
read_failed:
    {
        gst_buffer_unref(buf);
        GST_ELEMENT_ERROR(src, RESOURCE, READ, (NULL), ("Failed to read data: %s", gnome_vfs_result_to_string(res)));
        return GST_FLOW_ERROR;
    }
eos:
    {
...
static gboolean gst_gnome_vfs_src_query(GstBaseSrc * basesrc, GstQuery * query)
{
    gboolean ret = FALSE;
...
static gboolean gst_gnome_vfs_src_is_seekable(GstBaseSrc * basesrc)
{
    GstGnomeVFSSrc *src;
...
static gboolean gst_gnome_vfs_src_check_get_range(GstBaseSrc * basesrc)
{
    GstGnomeVFSSrc *src;
    ...
    {
        GST_LOG_OBJECT(src, "local URI (%s), assuming random access is possible", GST_STR_NULL(src->uri_name));
        return TRUE;
    }
    ...
    {
        GST_LOG_OBJECT(src, "blacklisted protocol '%s', no random access possible" " (URI=%s)", protocol, GST_STR_NULL(src->uri_name));
        return FALSE;
    }
...
undecided:
    {
        ...
        GST_LOG_OBJECT(src, "undecided about URI '%s', let base class handle it", GST_STR_NULL(src->uri_name));
...
static gboolean gst_gnome_vfs_src_get_size(GstBaseSrc * basesrc, guint64 * size)
{
    GstGnomeVFSSrc *src;
    ...
    *size = -1;
    ...
    if (res == GNOME_VFS_OK)
    {
        ...
        else if (src->own_handle && gnome_vfs_uri_is_local(src->uri))
        {
            ...
            if (res == GNOME_VFS_OK && (info->valid_fields & GNOME_VFS_FILE_INFO_FIELDS_SIZE) != 0)
            {
                *size = info->size;
...
static gboolean gst_gnome_vfs_src_start(GstBaseSrc * basesrc)
{
    GnomeVFSResult res;
    GstGnomeVFSSrc *src;
...
open_failed:
    {
        ...
        if (res == GNOME_VFS_ERROR_NOT_FOUND || res == GNOME_VFS_ERROR_HOST_NOT_FOUND || res == GNOME_VFS_ERROR_SERVICE_NOT_AVAILABLE)
        {
            GST_ELEMENT_ERROR(src, RESOURCE, NOT_FOUND, (NULL), ("Could not open vfs file \"%s\" for reading: %s (%d)", filename, gnome_vfs_result_to_string(res), res));
        }
        else
        {
            GST_ELEMENT_ERROR(src, RESOURCE, OPEN_READ, (NULL), ("Could not open vfs file \"%s\" for reading: %s (%d)", filename, gnome_vfs_result_to_string(res), res));
        }
        g_free(filename);
        return FALSE;
    }
...
static gboolean gst_gnome_vfs_src_stop(GstBaseSrc * basesrc)
{
    GstGnomeVFSSrc *src;
    ...
    if (src->own_handle)
    {
        ...
        if (res != GNOME_VFS_OK)
        {
            GST_ELEMENT_ERROR(src, RESOURCE, CLOSE, (NULL), ("Could not close vfs handle: %s", gnome_vfs_result_to_string(res)));
        }
        src->handle = NULL;
    }
    src->curoffset = 0;
