...
static void gst_alsa_mixer_track_class_init(gpointer g_class, gpointer class_data);
...
static void gst_alsa_mixer_track_update_alsa_capabilities(GstAlsaMixerTrack * alsa_track)
{
    ...
    if ((GST_MIXER_TRACK(alsa_track)->flags & GST_MIXER_TRACK_INPUT))
    {
        ...
        if (snd_mixer_selem_has_capture_switch(alsa_track->element))
        {
            ...
            if (snd_mixer_selem_has_capture_switch_exclusive(alsa_track->element))
            {
                ...
                alsa_track->capture_group = snd_mixer_selem_get_capture_group(alsa_track->element);
            }
        }
    }

    GST_LOG("[%s] alsa_flags=0x%08x, capture_group=%d", snd_mixer_selem_get_name(alsa_track->element), alsa_track->alsa_flags, alsa_track->capture_group);
}

inline static gboolean alsa_track_has_cap(GstAlsaMixerTrack * alsa_track, guint32 flag)
{
    return ((alsa_track->alsa_flags & flag) != 0);
}

GstMixerTrack *gst_alsa_mixer_track_new(snd_mixer_elem_t * element, gint num, gint track_num, gint flags, gboolean sw, GstAlsaMixerTrack * shared_mute_track, gboolean append_capture)
{
    ...
    GST_LOG("[%s,%u] num=%d,track_num=%d,flags=0x%08x,sw=%s,shared_mute_track=%p", name, index, num, track_num, flags, (sw) ? "true" : "false", shared_mute_track);

    track = (GstMixerTrack *) g_object_new(GST_ALSA_MIXER_TRACK_TYPE, "untranslated-label", name, "index", index, NULL);
    ...
    {
        while (alsa_track->alsa_channels < GST_ALSA_MAX_CHANNELS && snd_mixer_selem_has_playback_channel(element, alsa_track->alsa_channels))
        {
    ...
    {
        while (alsa_track->alsa_channels < GST_ALSA_MAX_CHANNELS && snd_mixer_selem_has_capture_channel(element, alsa_track->alsa_channels))
        {
            alsa_track->alsa_channels++;
        }
    ...
    if (num == 0)
    {
        track->label = g_strdup_printf("%s%s%s", label, append_capture ? " " : "", append_capture ? _("Capture") : "");
    }
    else
    {
        track->label = g_strdup_printf("%s%s%s %d", label, append_capture ? " " : "", append_capture ? _("Capture") : "", num);
    }
...
void gst_alsa_mixer_track_update(GstAlsaMixerTrack * alsa_track)
{
    ...
    else if (alsa_track_has_cap(alsa_track, GST_ALSA_MIXER_TRACK_PVOLUME) && track->flags & GST_MIXER_TRACK_MUTE)
    {
    ...
    if (track->flags & GST_MIXER_TRACK_INPUT)
    {
        ...
        else if (alsa_track_has_cap(alsa_track, GST_ALSA_MIXER_TRACK_CVOLUME) && !(track->flags & GST_MIXER_TRACK_RECORD))
        {
            /* check if user has raised volume with a parallel running application */
