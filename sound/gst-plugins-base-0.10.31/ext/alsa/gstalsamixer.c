...
static int gst_alsa_mixer_handle_callback(snd_mixer_t * handle, unsigned int mask, snd_mixer_elem_t * elem);

/* First some utils, then the mixer implementation */
static gboolean gst_alsa_mixer_open(GstAlsaMixer * mixer)
{
    gint err;
    snd_ctl_t *ctl;
    ...
    {
        GST_WARNING("Cannot open mixer for sound device '%s': %s", mixer->device, snd_strerror(err));
        goto error;
    }
...
static snd_mixer_elem_t *gst_alsa_mixer_find_master_mixer(GstAlsaMixer * mixer, snd_mixer_t * handle)
{
    ...
    for (i = 0; i < count; i++)
    {
        if (snd_mixer_selem_has_playback_volume(element) && strcmp(snd_mixer_selem_get_name(element), "Master") == 0)
        {
    ...
    for (i = 0; i < count; i++)
    {
        if (snd_mixer_selem_has_playback_volume(element) && strcmp(snd_mixer_selem_get_name(element), "Front") == 0)
        {
            g_static_rec_mutex_unlock(mixer->rec_mutex);
            return element;
        }
    ...
    for (i = 0; i < count; i++)
    {
        if (snd_mixer_selem_has_playback_volume(element) && strcmp(snd_mixer_selem_get_name(element), "PCM") == 0)
        {
            g_static_rec_mutex_unlock(mixer->rec_mutex);
            return element;
        }
    ...
    for (i = 0; i < count; i++)
    {
        if (snd_mixer_selem_has_playback_volume(element) && strcmp(snd_mixer_selem_get_name(element), "Speaker") == 0)
        {
            g_static_rec_mutex_unlock(mixer->rec_mutex);
            return element;
        }
    ...
    for (i = 0; i < count; i++)
    {
        if (snd_mixer_selem_has_playback_volume(element) && snd_mixer_selem_has_playback_switch(element) && !snd_mixer_selem_is_playback_mono(element))
        {
            g_static_rec_mutex_unlock(mixer->rec_mutex);
            return element;
        }
    ...
    for (i = 0; i < count; i++)
    {
        if (snd_mixer_selem_has_playback_volume(element) && snd_mixer_selem_has_playback_switch(element))
        {
            g_static_rec_mutex_unlock(mixer->rec_mutex);
            return element;
        }
...
static void gst_alsa_mixer_update(GstAlsaMixer * mixer, snd_mixer_elem_t * elem)
{
    GList *item;
...
static int gst_alsa_mixer_elem_handle_callback(snd_mixer_elem_t * elem, unsigned int mask)
{
...
static void gst_alsa_mixer_ensure_track_list(GstAlsaMixer * mixer)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (mixer->dir & GST_ALSA_MIXER_PLAYBACK)
        {
            ...
            GST_LOG("[%s] PLAYBACK: has_playback_volume=%d, has_playback_switch=%d" "%s", name, has_playback_volume, has_playback_switch, (element == master) ? " MASTER" : "");
            ...
            if (has_playback_volume)
            {
                ...
                play_track = gst_alsa_mixer_track_new(element, samename, i, flags, FALSE, NULL, FALSE);

            }
            else if (has_playback_switch)
            {
                /* simple mute switch */
                play_track = gst_alsa_mixer_track_new(element, samename, i, GST_MIXER_TRACK_OUTPUT, TRUE, NULL, FALSE);
            }
        ...
        if (mixer->dir & GST_ALSA_MIXER_CAPTURE)
        {
            ...
            GST_LOG("[%s] CAPTURE: has_capture_volume=%d, has_common_volume=%d, " "has_capture_switch=%d, has_common_switch=%d, play_track=%p", name, has_capture_volume, has_common_volume, has_capture_switch, has_common_switch, play_track);

            if (has_capture_volume && !(play_track && has_common_volume))
            {
                cap_track = gst_alsa_mixer_track_new(element, samename, i, GST_MIXER_TRACK_INPUT, FALSE, NULL, play_track != NULL);
            }
            else if (has_capture_switch && !(play_track && has_common_switch))
            {
                cap_track = gst_alsa_mixer_track_new(element, samename, i, GST_MIXER_TRACK_INPUT, TRUE, NULL, play_track != NULL);
            }
        }

        if (play_track && cap_track)
        {
            GST_ALSA_MIXER_TRACK(play_track)->shared_mute = GST_ALSA_MIXER_TRACK(cap_track);
            GST_ALSA_MIXER_TRACK(cap_track)->shared_mute = GST_ALSA_MIXER_TRACK(play_track);
        }
...
static void task_monitor_alsa(gpointer data)
{
    struct pollfd *pfds;
    ...
    if (rnfds < 0)
    {
        GST_ELEMENT_ERROR(mixer, RESOURCE, READ, (NULL), ("alsa error: %s", snd_strerror(rnfds)));
    ...
    ret = snd_mixer_poll_descriptors_revents(mixer->handle, pfds, nfds, &revents);
    if (ret < 0)
    {
        GST_ELEMENT_ERROR(mixer, RESOURCE, READ, (NULL), ("alsa error: %s", snd_strerror(ret)));
        gst_task_pause(mixer->task);
    }
...
GstAlsaMixer *gst_alsa_mixer_new(const char *device, GstAlsaMixerDirection dir)
{
...
void gst_alsa_mixer_get_volume(GstAlsaMixer * mixer, GstMixerTrack * track, gint * volumes)
{
    gint i;
    ...
    {                            /* return playback volume */
        ...
        if (track->flags & GST_MIXER_TRACK_MUTE && !(alsa_track->alsa_flags & GST_ALSA_MIXER_TRACK_PSWITCH))
        {
    ...
    {                            /* return capture volume */

        /* Is emulated record flag activated? */
        if (alsa_track->alsa_flags & GST_ALSA_MIXER_TRACK_CSWITCH || track->flags & GST_MIXER_TRACK_RECORD)
        {
            for (i = 0; i < track->num_channels; i++)
            {
                long tmp = 0;
...
static gboolean check_if_volumes_are_the_same(guint num_channels, gint * volumes)
{
    guint i;
...
void gst_alsa_mixer_set_volume(GstAlsaMixer * mixer, GstMixerTrack * track, gint * volumes)
{
    GstAlsaMixerTrack *alsa_track = GST_ALSA_MIXER_TRACK(track);
    gint i;
    ...
    if (track->flags & GST_MIXER_TRACK_OUTPUT)
    {
        ...
        else
        {
            ...
            {
                snd_mixer_selem_set_playback_volume_all(alsa_track->element, volumes[0]);
                for (i = 0; i < track->num_channels; i++)
            ...
            else
            {
                ...
                {
                    alsa_track->volumes[i] = volumes[i];
                    snd_mixer_selem_set_playback_volume(alsa_track->element, i, volumes[i]);
                }
            }
        }
    ...
    else if (track->flags & GST_MIXER_TRACK_INPUT)
    {
        ...
        /* Is emulated record flag activated? */
        if (track->flags & GST_MIXER_TRACK_RECORD || alsa_track->alsa_flags & GST_ALSA_MIXER_TRACK_CSWITCH)
        {
            if (check_if_volumes_are_the_same(track->num_channels, volumes))
            {
                snd_mixer_selem_set_capture_volume_all(alsa_track->element, volumes[0]);
                for (i = 0; i < track->num_channels; i++)
                    alsa_track->volumes[i] = volumes[0];
            }
            else
...
void gst_alsa_mixer_set_mute(GstAlsaMixer * mixer, GstMixerTrack * track, gboolean mute)
{
    GstAlsaMixerTrack *alsa_track = GST_ALSA_MIXER_TRACK(track);
    ...
    if (mute)
    {
        ...
        if (alsa_track->shared_mute)
            ((GstMixerTrack *) (alsa_track->shared_mute))->flags |= GST_MIXER_TRACK_MUTE;
    }
    else
    {
        track->flags &= ~GST_MIXER_TRACK_MUTE;
    ...
    else
    {
        ...
        if ((track->flags & GST_MIXER_TRACK_INPUT) && alsa_track->shared_mute != NULL)
        ...
        {
            long vol = mute ? ((GstMixerTrack *) ctrl_track)->min_volume : ctrl_track->volumes[i];
...
void gst_alsa_mixer_set_record(GstAlsaMixer * mixer, GstMixerTrack * track, gboolean record)
{
    GstAlsaMixerTrack *alsa_track = GST_ALSA_MIXER_TRACK(track);
        ...
        if (alsa_track->alsa_flags & GST_ALSA_MIXER_TRACK_CSWITCH_EXCL)
        {
            ...
            for (item = mixer->tracklist; item != NULL; item = item->next)
            {
                ...
                if (GST_IS_ALSA_MIXER_TRACK(item->data))
                {
                    ...
                    if (item_alsa_track->alsa_flags & GST_ALSA_MIXER_TRACK_CSWITCH_EXCL && item_alsa_track->capture_group == alsa_track->capture_group)
                    {
...
void gst_alsa_mixer_set_option(GstAlsaMixer * mixer, GstMixerOptions * opts, gchar * value)
{
    gint idx = -1, n = 0;
    GList *item;
...
const gchar *gst_alsa_mixer_get_option(GstAlsaMixer * mixer, GstMixerOptions * opts)
{
    gint ret;
    guint idx;
    GstAlsaMixerOptions *alsa_opts = GST_ALSA_MIXER_OPTIONS(opts);
...
static void gst_alsa_mixer_update_option(GstAlsaMixer * mixer, GstAlsaMixerOptions * alsa_opts)
{
    ...
    if (ret == 0)
    {
        ...
        gst_mixer_option_changed(mixer->interface, GST_MIXER_OPTIONS(alsa_opts), option);
    }
}
...
static void gst_alsa_mixer_update_track(GstAlsaMixer * mixer, GstAlsaMixerTrack * alsa_track)
{
    ...
    memcpy(old_volumes, alsa_track->volumes, sizeof(gint) * track->num_channels);

    gst_alsa_mixer_track_update(alsa_track);

    if (old_record != !!(GST_MIXER_TRACK_HAS_FLAG(track, GST_MIXER_TRACK_RECORD)))
    {
        gst_mixer_record_toggled(mixer->interface, track, !!GST_MIXER_TRACK_HAS_FLAG(track, GST_MIXER_TRACK_RECORD));
    }
    if (old_mute != !!(GST_MIXER_TRACK_HAS_FLAG(track, GST_MIXER_TRACK_MUTE)))
    {
        gst_mixer_mute_toggled(mixer->interface, track, !!GST_MIXER_TRACK_HAS_FLAG(track, GST_MIXER_TRACK_MUTE));
    }
...
void _gst_alsa_mixer_set_interface(GstAlsaMixer * mixer, GstMixer * interface)
{
