...
static GstCaps *gst_alsa_detect_rates(GstObject * obj, snd_pcm_hw_params_t * hw_params, GstCaps * in_caps)
{
    GstCaps *caps;
    guint min, max;
    ...
    for (i = 0; i < gst_caps_get_size(caps); ++i)
    {
        ...
        {
            gst_structure_set(s, "rate", GST_TYPE_INT_RANGE, min_rate, max_rate, NULL);
        }
    }
...
min_rate_err:
    {
        GST_ERROR_OBJECT(obj, "failed to query minimum sample rate: %s", snd_strerror(err));
        gst_caps_unref(in_caps);
        return NULL;
    }
max_rate_err:
    {
        GST_ERROR_OBJECT(obj, "failed to query maximum sample rate: %s", snd_strerror(err));
        gst_caps_unref(in_caps);
        return NULL;
    }
}
...
static GstCaps *gst_alsa_detect_formats(GstObject * obj, snd_pcm_hw_params_t * hw_params, GstCaps * in_caps)
{
    ...
    for (i = 0; i < gst_caps_get_size(in_caps); ++i)
    {
        ...
        if (!gst_structure_get_int(s, "width", &width) || !gst_structure_get_int(s, "depth", &depth))
            continue;
        ...
        if (snd_pcm_format_mask_test(mask, pcmformats[w].sformat) && snd_pcm_format_mask_test(mask, pcmformats[w].uformat))
        {
...
static GstStructure *get_channel_free_structure(const GstStructure * in_structure)
{
...
static void caps_add_channel_configuration(GstCaps * caps, const GstStructure * in_structure, gint min_chans, gint max_chans)
{
...
static GstCaps *gst_alsa_detect_channels(GstObject * obj, snd_pcm_hw_params_t * hw_params, GstCaps * in_caps)
{
    GstCaps *caps;
    guint min, max;
    ...
    if (min_chans > max_chans)
    {
        ...
        GST_WARNING_OBJECT(obj, "minimum channels > maximum channels (%d > %d), " "please fix your soundcard drivers", min, max);
        temp = min_chans;
        min_chans = max_chans;
        max_chans = temp;
    }
...
max_chan_error:
    {
        GST_ERROR_OBJECT(obj, "failed to query maximum channel count: %s", snd_strerror(err));
        return NULL;
    }
}
...
snd_pcm_t *gst_alsa_open_iec958_pcm(GstObject * obj)
{
     ...
     */
    sprintf(devstr, "iec958:{AES0 0x%02x AES1 0x%02x AES2 0x%02x AES3 0x%02x}", IEC958_AES0_CON_EMPHASIS_NONE | IEC958_AES0_NONAUDIO, IEC958_AES1_CON_ORIGINAL | IEC958_AES1_CON_PCM_CODER, 0, IEC958_AES3_CON_FS_48000);
...
static gchar *gst_alsa_find_device_name_no_handle(GstObject * obj, const gchar * devcard, gint device_num, snd_pcm_stream_t stream)
{
...
gchar *gst_alsa_find_card_name(GstObject * obj, const gchar * devcard, snd_pcm_stream_t stream)
{
    return gst_alsa_find_device_name_no_handle(obj, devcard, -1, stream);
}

gchar *gst_alsa_find_device_name(GstObject * obj, const gchar * device, snd_pcm_t * handle, snd_pcm_stream_t stream)
{
    gchar *ret = NULL;
    ...
    GST_LOG_OBJECT(obj, "Device name for device '%s': %s", GST_STR_NULL(device), GST_STR_NULL(ret));
