...
GST_BOILERPLATE(GstGioBaseSrc, gst_gio_base_src, GstBaseSrc, GST_TYPE_BASE_SRC);
...
static gboolean gst_gio_base_src_get_size(GstBaseSrc * base_src, guint64 * size);
...
static GstFlowReturn gst_gio_base_src_create(GstBaseSrc * base_src, guint64 offset, guint size, GstBuffer ** buf);
static gboolean gst_gio_base_src_query(GstBaseSrc * base_src, GstQuery * query);

static void gst_gio_base_src_base_init(gpointer gclass)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(gclass);

    GST_DEBUG_CATEGORY_INIT(gst_gio_base_src_debug, "gio_base_src", 0, "GIO base source");

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&src_factory));
}

static void gst_gio_base_src_class_init(GstGioBaseSrcClass * klass)
{
    ...
    gstbasesrc_class->is_seekable = GST_DEBUG_FUNCPTR(gst_gio_base_src_is_seekable);
    gstbasesrc_class->unlock = GST_DEBUG_FUNCPTR(gst_gio_base_src_unlock);
    gstbasesrc_class->unlock_stop = GST_DEBUG_FUNCPTR(gst_gio_base_src_unlock_stop);
    gstbasesrc_class->check_get_range = GST_DEBUG_FUNCPTR(gst_gio_base_src_check_get_range);
...
static void gst_gio_base_src_init(GstGioBaseSrc * src, GstGioBaseSrcClass * gclass)
{
...
static gboolean gst_gio_base_src_start(GstBaseSrc * base_src)
{
    ...
    {
        GST_ELEMENT_ERROR(src, RESOURCE, OPEN_READ, (NULL), ("No input stream provided by subclass"));
        return FALSE;
    }
    else if (G_UNLIKELY(g_input_stream_is_closed(src->stream)))
    {
        GST_ELEMENT_ERROR(src, LIBRARY, FAILED, (NULL), ("Input stream is already closed"));
        return FALSE;
    }
...
static gboolean gst_gio_base_src_stop(GstBaseSrc * base_src)
{
    ...
    if (klass->close_on_stop && G_IS_INPUT_STREAM(src->stream))
    {
        ...
        {
            GST_ELEMENT_WARNING(src, RESOURCE, CLOSE, (NULL), ("g_input_stream_close failed: %s", err->message));
            g_clear_error(&err);
        }
        else if (!success)
        {
            GST_ELEMENT_WARNING(src, RESOURCE, CLOSE, (NULL), ("g_input_stream_close failed"));
        }
        else
        {
...
static gboolean gst_gio_base_src_get_size(GstBaseSrc * base_src, guint64 * size)
{
    ...
    if (G_IS_FILE_INPUT_STREAM(src->stream))
    {
        ...
        info = g_file_input_stream_query_info(G_FILE_INPUT_STREAM(src->stream), G_FILE_ATTRIBUTE_STANDARD_SIZE, src->cancel, &err);
        ...
        if (!gst_gio_error(src, "g_file_input_stream_query_info", &err, NULL))
        {
            ...
            if (GST_GIO_ERROR_MATCHES(err, NOT_SUPPORTED))
                ...
                GST_WARNING_OBJECT(src, "size information retrieval failed: %s", err->message);
    ...
    if (GST_GIO_STREAM_IS_SEEKABLE(src->stream))
    {
        ...
        if (!ret)
        {
            ...
            {
                if (GST_GIO_ERROR_MATCHES(err, NOT_SUPPORTED))
                    GST_DEBUG_OBJECT(src, "Seeking to the end of stream is not supported");
                else
                    GST_WARNING_OBJECT(src, "Seeking to end of stream failed: %s", err->message);
                g_clear_error(&err);
            }
            else
            {
        ...
        if (!ret)
        {
            if (!gst_gio_error(src, "g_seekable_seek", &err, NULL))
            {
                if (GST_GIO_ERROR_MATCHES(err, NOT_SUPPORTED))
                    ...
                    GST_ERROR_OBJECT(src, "Seeking to the old position failed: %s", err->message);
                g_clear_error(&err);
            }
            else
            {
...
static gboolean gst_gio_base_src_check_get_range(GstBaseSrc * base_src)
{
    return GST_CALL_PARENT_WITH_DEFAULT(GST_BASE_SRC_CLASS, check_get_range, (base_src), FALSE);
}

static GstFlowReturn gst_gio_base_src_create(GstBaseSrc * base_src, guint64 offset, guint size, GstBuffer ** buf_return)
{
    GstGioBaseSrc *src = GST_GIO_BASE_SRC(base_src);
    GstBuffer *buf;
    ...
    if (src->cache && offset >= GST_BUFFER_OFFSET(src->cache) && offset + size <= GST_BUFFER_OFFSET_END(src->cache))
    {
        GST_DEBUG_OBJECT(src, "Creating subbuffer from cached buffer: offset %" G_GUINT64_FORMAT " length %u", offset, size);

        buf = gst_buffer_create_sub(src->cache, offset - GST_BUFFER_OFFSET(src->cache), size);
    ...
    else
    {
        ...
        GST_LOG_OBJECT(src, "Reading %u bytes from offset %" G_GUINT64_FORMAT, cachesize, offset);
         ...
         * nothing. */
        read = 0;
        while (size - read > 0 && (res = g_input_stream_read(G_INPUT_STREAM(src->stream), GST_BUFFER_DATA(src->cache) + read, cachesize - read, src->cancel, &err)) > 0)
        {
            read += res;
        }
        ...
        {
            GST_ELEMENT_ERROR(src, RESOURCE, READ, (NULL), ("Could not read from stream: %s", err->message));
            g_clear_error(&err);
        }
        ...
        if (success && !eos)
        {
            ...
            GST_DEBUG_OBJECT(src, "Creating subbuffer from new " "cached buffer: offset %" G_GUINT64_FORMAT " length %u", offset, size);
