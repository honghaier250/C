 ...
 */
gboolean gst_gio_error(gpointer element, const gchar * func_name, GError ** err, GstFlowReturn * ret)
{
    ...
    {
        GST_ELEMENT_ERROR(element, LIBRARY, FAILED, (NULL), ("%s call failed without error set", func_name));
    }
...
GstFlowReturn gst_gio_seek(gpointer element, GSeekable * stream, guint64 offset, GCancellable * cancel)
{
    gboolean success;
    GstFlowReturn ret;
    GError *err = NULL;
    ...
    {
        GST_ELEMENT_ERROR(element, RESOURCE, SEEK, (NULL), ("Could not seek: %s", err->message));
        g_clear_error(&err);
    }
...
static gpointer _internal_get_supported_protocols(gpointer data)
{
    ...
    for (i = 0, j = 0; i < num; i++)
    {
        if (strcmp(schemes[i], "http") == 0 || strcmp(schemes[i], "https") == 0 || strcmp(schemes[i], "cdda") == 0)
            continue;
...
static gchar **gst_gio_get_supported_protocols(void)
{
...
static GstURIType gst_gio_uri_handler_get_type_src(void)
{
    return GST_URI_SRC;
}

static gchar **gst_gio_uri_handler_get_protocols(void)
{
...
static const gchar *gst_gio_uri_handler_get_uri(GstURIHandler * handler)
{
...
static gboolean gst_gio_uri_handler_set_uri(GstURIHandler * handler, const gchar * uri)
{
    GstElement *element = GST_ELEMENT(handler);
...
static void gst_gio_uri_handler_init(gpointer g_iface, gpointer iface_data)
{
...
void gst_gio_uri_handler_do_init(GType type)
{
     ...
     * classes. */
    uri_handler_info.interface_data = GINT_TO_POINTER(g_type_is_a(type, GST_TYPE_BASE_SINK));
...
static gboolean plugin_init(GstPlugin * plugin)
{
    ...
    gst_plugin_add_dependency_simple(plugin, NULL, GIO_MODULE_DIR, NULL, GST_PLUGIN_DEPENDENCY_FLAG_NONE);
    gst_plugin_add_dependency_simple(plugin, "LD_LIBRARY_PATH", GIO_LIBDIR, "gvfsd", GST_PLUGIN_DEPENDENCY_FLAG_NONE);
    ...
    ret &= gst_element_register(plugin, "giosink", GST_RANK_SECONDARY, GST_TYPE_GIO_SINK);

    ret &= gst_element_register(plugin, "giosrc", GST_RANK_SECONDARY, GST_TYPE_GIO_SRC);

    ret &= gst_element_register(plugin, "giostreamsink", GST_RANK_NONE, GST_TYPE_GIO_STREAM_SINK);

    ret &= gst_element_register(plugin, "giostreamsrc", GST_RANK_NONE, GST_TYPE_GIO_STREAM_SRC);

    return ret;
}

GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, "gio", "GIO elements", plugin_init, VERSION, "LGPL", GST_PACKAGE_NAME, GST_PACKAGE_ORIGIN)
