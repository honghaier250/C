...
GST_BOILERPLATE(GstGioBaseSink, gst_gio_base_sink, GstBaseSink, GST_TYPE_BASE_SINK);
...
static gboolean gst_gio_base_sink_event(GstBaseSink * base_sink, GstEvent * event);
static GstFlowReturn gst_gio_base_sink_render(GstBaseSink * base_sink, GstBuffer * buffer);
static gboolean gst_gio_base_sink_query(GstPad * pad, GstQuery * query);

static void gst_gio_base_sink_base_init(gpointer gclass)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(gclass);

    GST_DEBUG_CATEGORY_INIT(gst_gio_base_sink_debug, "gio_base_sink", 0, "GIO base sink");

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&sink_factory));
}

static void gst_gio_base_sink_class_init(GstGioBaseSinkClass * klass)
{
    ...
    gstbasesink_class->unlock_stop = GST_DEBUG_FUNCPTR(gst_gio_base_sink_unlock_stop);
...
static void gst_gio_base_sink_init(GstGioBaseSink * sink, GstGioBaseSinkClass * gclass)
{
    gst_pad_set_query_function(GST_BASE_SINK_PAD(sink), GST_DEBUG_FUNCPTR(gst_gio_base_sink_query));
...
static gboolean gst_gio_base_sink_start(GstBaseSink * base_sink)
{
    ...
    {
        GST_ELEMENT_ERROR(sink, RESOURCE, OPEN_WRITE, (NULL), ("No output stream provided by subclass"));
        return FALSE;
    }
    else if (G_UNLIKELY(g_output_stream_is_closed(sink->stream)))
    {
        GST_ELEMENT_ERROR(sink, LIBRARY, FAILED, (NULL), ("Output stream is already closed"));
        return FALSE;
    }
...
static gboolean gst_gio_base_sink_stop(GstBaseSink * base_sink)
{
    ...
    if (klass->close_on_stop && G_IS_OUTPUT_STREAM(sink->stream))
    {
        ...
        {
            GST_ELEMENT_WARNING(sink, RESOURCE, CLOSE, (NULL), ("gio_output_stream_close failed: %s", err->message));
            g_clear_error(&err);
        }
        else if (!success)
        {
            GST_ELEMENT_WARNING(sink, RESOURCE, CLOSE, (NULL), ("g_output_stream_close failed"));
        }
        else
        {
    ...
    else
    {
        ...
        {
            GST_ELEMENT_WARNING(sink, RESOURCE, CLOSE, (NULL), ("gio_output_stream_flush failed: %s", err->message));
            g_clear_error(&err);
        }
        else if (!success)
        {
            GST_ELEMENT_WARNING(sink, RESOURCE, CLOSE, (NULL), ("g_output_stream_flush failed"));
        }
        else
        {
    ...
    switch (GST_EVENT_TYPE(event))
    {
        case GST_EVENT_NEWSEGMENT:
            if (G_IS_OUTPUT_STREAM(sink->stream))
            {
                ...
                {
                    ret = gst_gio_seek(sink, G_SEEKABLE(sink->stream), offset, sink->cancel);
                    if (ret == GST_FLOW_OK)
        ...
        case GST_EVENT_FLUSH_START:
            if (G_IS_OUTPUT_STREAM(sink->stream))
            {
                ...
                if (!success && !gst_gio_error(sink, "g_output_stream_flush", &err, &ret))
                {
                    GST_ELEMENT_ERROR(sink, RESOURCE, WRITE, (NULL), ("flush failed: %s", err->message));
                    g_clear_error(&err);
                }
            }
            break;
...
static GstFlowReturn gst_gio_base_sink_render(GstBaseSink * base_sink, GstBuffer * buffer)
{
    ...
    GST_LOG_OBJECT(sink, "writing %u bytes to offset %" G_GUINT64_FORMAT, GST_BUFFER_SIZE(buffer), sink->position);

    written = g_output_stream_write(sink->stream, GST_BUFFER_DATA(buffer), GST_BUFFER_SIZE(buffer), sink->cancel, &err);
    ...
    if (G_UNLIKELY(success && written < GST_BUFFER_SIZE(buffer)))
    {
         ...
         * doesn't... */
        GST_ELEMENT_ERROR(sink, RESOURCE, WRITE, (NULL), ("Could not write to stream: (short write, only %" G_GSSIZE_FORMAT " bytes of %d bytes written)", written, GST_BUFFER_SIZE(buffer)));
    ...
    else
    {
        ...
        if (!gst_gio_error(sink, "g_output_stream_write", &err, &ret))
        {
            ...
            {
                GST_ELEMENT_ERROR(sink, RESOURCE, NO_SPACE_LEFT, (NULL), ("Could not write to stream: %s", err->message));
            }
            else
            {
                GST_ELEMENT_ERROR(sink, RESOURCE, WRITE, (NULL), ("Could not write to stream: %s", err->message));
            }
            g_clear_error(&err);
        }
