...
static GstStaticPadTemplate vorbis_parse_sink_factory = GST_STATIC_PAD_TEMPLATE("sink",
                                                                                GST_PAD_SINK,
                                                                                GST_PAD_ALWAYS,
                                                                                GST_STATIC_CAPS("audio/x-vorbis"));

static GstStaticPadTemplate vorbis_parse_src_factory = GST_STATIC_PAD_TEMPLATE("src",
                                                                               GST_PAD_SRC,
                                                                               GST_PAD_ALWAYS,
                                                                               GST_STATIC_CAPS("audio/x-vorbis"));

GST_BOILERPLATE(GstVorbisParse, gst_vorbis_parse, GstElement, GST_TYPE_ELEMENT);
...
static gboolean vorbis_parse_convert(GstPad * pad, GstFormat src_format, gint64 src_value, GstFormat * dest_format, gint64 * dest_value);
static GstFlowReturn vorbis_parse_parse_packet(GstVorbisParse * parse, GstBuffer * buf);

static void gst_vorbis_parse_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&vorbis_parse_src_factory));
    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&vorbis_parse_sink_factory));
    gst_element_class_set_details_simple(element_class, "VorbisParse", "Codec/Parser/Audio", "parse raw vorbis streams", "Thomas Vander Stichele <thomas at apestaart dot org>");
}
...
static void gst_vorbis_parse_init(GstVorbisParse * parse, GstVorbisParseClass * g_class)
{
    parse->sinkpad = gst_pad_new_from_static_template(&vorbis_parse_sink_factory, "sink");
    gst_pad_set_chain_function(parse->sinkpad, GST_DEBUG_FUNCPTR(vorbis_parse_chain));
    gst_pad_set_event_function(parse->sinkpad, GST_DEBUG_FUNCPTR(vorbis_parse_sink_event));
    gst_element_add_pad(GST_ELEMENT(parse), parse->sinkpad);

    parse->srcpad = gst_pad_new_from_static_template(&vorbis_parse_src_factory, "src");
    gst_pad_set_query_function(parse->srcpad, GST_DEBUG_FUNCPTR(vorbis_parse_src_query));
    gst_element_add_pad(GST_ELEMENT(parse), parse->srcpad);
}

static void vorbis_parse_set_header_on_caps(GstVorbisParse * parse, GstCaps * caps)
{
...
static void vorbis_parse_drain_event_queue(GstVorbisParse * parse)
{
...
static void vorbis_parse_push_headers(GstVorbisParse * parse)
{
...
static void vorbis_parse_clear_queue(GstVorbisParse * parse)
{
...
static GstFlowReturn vorbis_parse_push_buffer(GstVorbisParse * parse, GstBuffer * buf, gint64 granulepos)
{
    guint64 samples;
    ...
    GST_BUFFER_TIMESTAMP(buf) = GST_BUFFER_OFFSET(buf) - GST_BUFFER_DURATION(buf);
...
static GstFlowReturn vorbis_parse_drain_queue_prematurely(GstVorbisParse * parse)
{
...
static GstFlowReturn vorbis_parse_drain_queue(GstVorbisParse * parse, gint64 granulepos)
{
    GstFlowReturn ret = GST_FLOW_OK;
    GList *walk;
...
static GstFlowReturn vorbis_parse_queue_buffer(GstVorbisParse * parse, GstBuffer * buf)
{
    GstFlowReturn ret = GST_FLOW_OK;
    long blocksize;
    ogg_packet packet;
...
static gboolean vorbis_parse_queue_event(GstVorbisParse * parse, GstEvent * event)
{
...
static gboolean vorbis_parse_convert(GstPad * pad, GstFormat src_format, gint64 src_value, GstFormat * dest_format, gint64 * dest_value)
{
    ...
    if (parse->sinkpad == pad && (src_format == GST_FORMAT_BYTES || *dest_format == GST_FORMAT_BYTES))
        return FALSE;
        ...
        case GST_FORMAT_TIME:
            switch (*dest_format)
            {
                ...
                case GST_FORMAT_DEFAULT:
                    *dest_value = scale * gst_util_uint64_scale_int(src_value, parse->vi.rate, GST_SECOND);
                    break;
                default:
                    res = FALSE;
            }
            ...
            switch (*dest_format)
            {
                case GST_FORMAT_BYTES:
                    *dest_value = src_value * sizeof(float) * parse->vi.channels;
                    break;
                case GST_FORMAT_TIME:
                    *dest_value = gst_util_uint64_scale_int(src_value, GST_SECOND, parse->vi.rate);
                    break;
                default:
                    res = FALSE;
            }
            ...
            switch (*dest_format)
            {
                case GST_FORMAT_DEFAULT:
                    *dest_value = src_value / (sizeof(float) * parse->vi.channels);
                    break;
                case GST_FORMAT_TIME:
                    *dest_value = gst_util_uint64_scale_int(src_value, GST_SECOND, parse->vi.rate * sizeof(float) * parse->vi.channels);
                    break;
                default:
                    res = FALSE;
            }
        ...
        case GST_QUERY_POSITION:
            {
                ...
                if (!(res = vorbis_parse_convert(pad, GST_FORMAT_DEFAULT, granulepos, &format, &value)))
                    goto error;
                ...
                GST_LOG_OBJECT(parse, "query %p: peer returned granulepos: %" G_GUINT64_FORMAT " - we return %" G_GUINT64_FORMAT " (format %u)", query, granulepos, value, format);
        ...
        case GST_QUERY_DURATION:
            {
                ...
                {
                    GST_WARNING_OBJECT(parse, "sink pad %" GST_PTR_FORMAT " is not linked", parse->sinkpad);
                    goto error;
                }
        ...
        case GST_QUERY_CONVERT:
            {
                ...
                if (!(res = vorbis_parse_convert(pad, src_fmt, src_val, &dest_fmt, &dest_val)))
                    goto error;
