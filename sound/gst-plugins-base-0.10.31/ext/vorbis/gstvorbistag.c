...
static GstFlowReturn gst_vorbis_tag_parse_packet(GstVorbisParse * parse, GstBuffer * buffer);
...
GST_BOILERPLATE_FULL(GstVorbisTag, gst_vorbis_tag, GstVorbisParse, GST_TYPE_VORBIS_PARSE, _do_init);

static void gst_vorbis_tag_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_set_details_simple(element_class, "VorbisTag", "Formatter/Metadata", "Retags vorbis streams", "James Livingston <doclivingston@gmail.com>");
}
...
static void gst_vorbis_tag_init(GstVorbisTag * tagger, GstVorbisTagClass * g_class)
{
    /* nothing to do */
}
...
static GstFlowReturn gst_vorbis_tag_parse_packet(GstVorbisParse * parse, GstBuffer * buffer)
{
    ...
    old_tags = gst_tag_list_from_vorbiscomment_buffer(buffer, (guint8 *) "\003vorbis", 7, &encoder);
    user_tags = gst_tag_setter_get_tag_list(GST_TAG_SETTER(tagger));

    /* build new tag list */
    new_tags = gst_tag_list_merge(user_tags, old_tags, gst_tag_setter_get_tag_merge_mode(GST_TAG_SETTER(tagger)));
    gst_tag_list_free(old_tags);

    new_buf = gst_tag_list_to_vorbiscomment_buffer(new_tags, (guint8 *) "\003vorbis", 7, encoder);
