...
GST_BOILERPLATE(GstCdParanoiaSrc, gst_cd_paranoia_src, GstCddaBaseSrc, GST_TYPE_CDDA_BASE_SRC);

static void gst_cd_paranoia_src_finalize(GObject * obj);
static void gst_cd_paranoia_src_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
static void gst_cd_paranoia_src_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static GstBuffer *gst_cd_paranoia_src_read_sector(GstCddaBaseSrc * src, gint sector);
static gboolean gst_cd_paranoia_src_open(GstCddaBaseSrc * src, const gchar * device);
...
static GType gst_cd_paranoia_mode_get_type(void)
{
...
static void gst_cd_paranoia_src_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_set_details_simple(element_class, "CD Audio (cdda) Source, Paranoia IV", "Source/File", "Read audio from CD in paranoid mode", "Erik Walthinsen <omega@cse.ogi.edu>, Wim Taymans <wim@fluendo.com>");
}

static void gst_cd_paranoia_src_init(GstCdParanoiaSrc * src, GstCdParanoiaSrcClass * klass)
{
    src->d = NULL;
    src->p = NULL;
    src->next_sector = -1;
...
static void gst_cd_paranoia_src_class_init(GstCdParanoiaSrcClass * klass)
{
    ...
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_GENERIC_DEVICE, g_param_spec_string("generic-device", "Generic device", "Use specified generic scsi device", DEFAULT_GENERIC_DEVICE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_READ_SPEED, g_param_spec_int("read-speed", "Read speed", "Read from device at specified speed", -1, G_MAXINT, DEFAULT_READ_SPEED, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_PARANOIA_MODE,
                                    g_param_spec_flags("paranoia-mode", "Paranoia mode", "Type of checking to perform", GST_TYPE_CD_PARANOIA_MODE, DEFAULT_PARANOIA_MODE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_SEARCH_OVERLAP,
                                    g_param_spec_int("search-overlap", "Search overlap", "Force minimum overlap search during verification to n sectors", -1, 75, DEFAULT_SEARCH_OVERLAP, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   ...
   * Since: 0.10.24
   */
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_CACHE_SIZE, g_param_spec_int("cache-size", "Cache size", "Set CD cache size to n sectors (-1 = auto)", -1, G_MAXINT, DEFAULT_CACHE_SIZE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   */
    cdpsrc_signals[TRANSPORT_ERROR] = g_signal_new("transport-error", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstCdParanoiaSrcClass, transport_error), NULL, NULL, g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
  /**
   ...
   */
    cdpsrc_signals[UNCORRECTED_ERROR] = g_signal_new("uncorrected-error", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstCdParanoiaSrcClass, uncorrected_error), NULL, NULL, g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
}

static gboolean gst_cd_paranoia_src_open(GstCddaBaseSrc * cddabasesrc, const gchar * device)
{
    GstCdParanoiaSrc *src = GST_CD_PARANOIA_SRC(cddabasesrc);
    gint i, cache_size;

    GST_DEBUG_OBJECT(src, "trying to open device %s (generic-device=%s) ...", device, GST_STR_NULL(src->generic_device));
...
no_device:
    {
        GST_ELEMENT_ERROR(src, RESOURCE, OPEN_READ, (_("Could not open CD device for reading.")), ("cdda_identify failed"));
        return FALSE;
    }
open_failed:
    {
        GST_ELEMENT_ERROR(src, RESOURCE, OPEN_READ, (_("Could not open CD device for reading.")), ("cdda_open failed"));
        cdda_close(src->d);
        src->d = NULL;
        return FALSE;
    }
init_failed:
    {
        GST_ELEMENT_ERROR(src, LIBRARY, INIT, ("failed to initialize paranoia"), ("failed to initialize paranoia"));
        return FALSE;
    }
}

static void gst_cd_paranoia_src_close(GstCddaBaseSrc * cddabasesrc)
{
    GstCdParanoiaSrc *src = GST_CD_PARANOIA_SRC(cddabasesrc);
...
static void gst_cd_paranoia_dummy_callback(long inpos, int function)
{
    /* Used by instanced where no one is interested what's happening here */
}

static void gst_cd_paranoia_paranoia_callback(long inpos, int function)
{
...
static gboolean gst_cd_paranoia_src_signal_is_being_watched(GstCdParanoiaSrc * src, gint sig)
{
    return g_signal_has_handler_pending(src, cdpsrc_signals[sig], 0, FALSE);
}

static GstBuffer *gst_cd_paranoia_src_read_sector(GstCddaBaseSrc * cddabasesrc, gint sector)
{
    GstCdParanoiaSrc *src = GST_CD_PARANOIA_SRC(cddabasesrc);
    GstBuffer *buf;
    ...
    {
        g_message("DiscID on multisession discs might be broken. Use at own risk.");
    }
#endif
    ...
    do_serialize = gst_cd_paranoia_src_signal_is_being_watched(src, TRANSPORT_ERROR) || gst_cd_paranoia_src_signal_is_being_watched(src, UNCORRECTED_ERROR);
...
seek_failed:
    {
        ...
        GST_ELEMENT_ERROR(src, RESOURCE, SEEK, (_("Could not seek CD.")), ("paranoia_seek to %d failed: %s", sector, g_strerror(errno)));
        return NULL;
    }
read_failed:
    {
        GST_WARNING_OBJECT(src, "read at sector %d failed!", sector);
        GST_ELEMENT_ERROR(src, RESOURCE, READ, (_("Could not read CD.")), ("paranoia_read at %d failed: %s", sector, g_strerror(errno)));
        return NULL;
    }
}
...
{
    GST_DEBUG_CATEGORY_INIT(gst_cd_paranoia_src_debug, "cdparanoiasrc", 0, "CD Paranoia Source");

    if (!gst_element_register(plugin, "cdparanoiasrc", GST_RANK_SECONDARY, GST_TYPE_CD_PARANOIA_SRC))
        return FALSE;
...
GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, "cdparanoia", "Read audio from CD in paranoid mode", plugin_init, VERSION, "LGPL", GST_PACKAGE_NAME, GST_PACKAGE_ORIGIN)
