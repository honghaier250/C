...
static GstStaticPadTemplate gst_audio_resample_sink_template = GST_STATIC_PAD_TEMPLATE("sink",
                                                                                       GST_PAD_SINK, GST_PAD_ALWAYS, SUPPORTED_CAPS);

static GstStaticPadTemplate gst_audio_resample_src_template = GST_STATIC_PAD_TEMPLATE("src",
                                                                                      GST_PAD_SRC, GST_PAD_ALWAYS, SUPPORTED_CAPS);

static void gst_audio_resample_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_audio_resample_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

/* vmethods */
static gboolean gst_audio_resample_get_unit_size(GstBaseTransform * base, GstCaps * caps, guint * size);
static GstCaps *gst_audio_resample_transform_caps(GstBaseTransform * base, GstPadDirection direction, GstCaps * caps);
static void gst_audio_resample_fixate_caps(GstBaseTransform * base, GstPadDirection direction, GstCaps * caps, GstCaps * othercaps);
static gboolean gst_audio_resample_transform_size(GstBaseTransform * trans, GstPadDirection direction, GstCaps * incaps, guint insize, GstCaps * outcaps, guint * outsize);
static gboolean gst_audio_resample_set_caps(GstBaseTransform * base, GstCaps * incaps, GstCaps * outcaps);
static GstFlowReturn gst_audio_resample_transform(GstBaseTransform * base, GstBuffer * inbuf, GstBuffer * outbuf);
static gboolean gst_audio_resample_event(GstBaseTransform * base, GstEvent * event);
...
GST_BOILERPLATE(GstAudioResample, gst_audio_resample, GstBaseTransform, GST_TYPE_BASE_TRANSFORM);

static void gst_audio_resample_base_init(gpointer g_class)
{
    GstElementClass *gstelement_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&gst_audio_resample_src_template));
    gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&gst_audio_resample_sink_template));

    gst_element_class_set_details_simple(gstelement_class, "Audio resampler", "Filter/Converter/Audio", "Resamples audio", "Sebastian Dröge <sebastian.droege@collabora.co.uk>");
}

static void gst_audio_resample_class_init(GstAudioResampleClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_QUALITY,
                                    g_param_spec_int("quality", "Quality", "Resample quality with 0 being "
                                                     "the lowest and 10 being the best", SPEEX_RESAMPLER_QUALITY_MIN, SPEEX_RESAMPLER_QUALITY_MAX, SPEEX_RESAMPLER_QUALITY_DEFAULT, G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_object_class_install_property(gobject_class, PROP_FILTER_LENGTH, g_param_spec_int("filter-length", "Filter length", "Length of the resample filter", 0, G_MAXINT, 64, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    GST_BASE_TRANSFORM_CLASS(klass)->start = GST_DEBUG_FUNCPTR(gst_audio_resample_start);
    GST_BASE_TRANSFORM_CLASS(klass)->stop = GST_DEBUG_FUNCPTR(gst_audio_resample_stop);
    GST_BASE_TRANSFORM_CLASS(klass)->transform_size = GST_DEBUG_FUNCPTR(gst_audio_resample_transform_size);
    GST_BASE_TRANSFORM_CLASS(klass)->get_unit_size = GST_DEBUG_FUNCPTR(gst_audio_resample_get_unit_size);
    GST_BASE_TRANSFORM_CLASS(klass)->transform_caps = GST_DEBUG_FUNCPTR(gst_audio_resample_transform_caps);
    GST_BASE_TRANSFORM_CLASS(klass)->fixate_caps = GST_DEBUG_FUNCPTR(gst_audio_resample_fixate_caps);
    GST_BASE_TRANSFORM_CLASS(klass)->set_caps = GST_DEBUG_FUNCPTR(gst_audio_resample_set_caps);
    GST_BASE_TRANSFORM_CLASS(klass)->transform = GST_DEBUG_FUNCPTR(gst_audio_resample_transform);
    GST_BASE_TRANSFORM_CLASS(klass)->event = GST_DEBUG_FUNCPTR(gst_audio_resample_event);

    GST_BASE_TRANSFORM_CLASS(klass)->passthrough_on_same_caps = TRUE;
}

static void gst_audio_resample_init(GstAudioResample * resample, GstAudioResampleClass * klass)
{
    ...
    gst_pad_set_query_type_function(trans->srcpad, gst_audio_resample_query_type);
}
...
static const SpeexResampleFuncs *gst_audio_resample_get_funcs(gint width, gboolean fp)
{
    const SpeexResampleFuncs *funcs = NULL;

    if (gst_audio_resample_use_int && (width == 8 || width == 16) && !fp)
        funcs = &int_funcs;
    else if ((!gst_audio_resample_use_int && (width == 8 || width == 16) && !fp) || (width == 32 && fp))
        funcs = &float_funcs;
...
static SpeexResamplerState *gst_audio_resample_init_state(GstAudioResample * resample, gint width, gint channels, gint inrate, gint outrate, gint quality, gboolean fp)
{
    ...
    {
        GST_ERROR_OBJECT(resample, "Failed to create resampler state: %s", funcs->strerror(err));
        return NULL;
    }
...
static gboolean gst_audio_resample_update_state(GstAudioResample * resample, gint width, gint channels, gint inrate, gint outrate, gint quality, gboolean fp)
{
    gboolean ret = TRUE;
    gboolean updated_latency = FALSE;

    updated_latency = (resample->inrate != inrate || quality != resample->quality) && resample->state != NULL;

    if (resample->state == NULL)
    {
        ret = TRUE;
    }
    else if (resample->channels != channels || fp != resample->fp || width != resample->width)
    {
        resample->funcs->destroy(resample->state);
        resample->state = gst_audio_resample_init_state(resample, width, channels, inrate, outrate, quality, fp);
    ...
    else if (resample->inrate != inrate || resample->outrate != outrate)
    {
        ...
        if (G_UNLIKELY(err != RESAMPLER_ERR_SUCCESS))
            GST_ERROR_OBJECT(resample, "Failed to update rate: %s", resample->funcs->strerror(err));
    ...
    else if (quality != resample->quality)
    {
        ...
        if (G_UNLIKELY(err != RESAMPLER_ERR_SUCCESS))
            GST_ERROR_OBJECT(resample, "Failed to update quality: %s", resample->funcs->strerror(err));
    ...
    if (updated_latency)
        gst_element_post_message(GST_ELEMENT(resample), gst_message_new_latency(GST_OBJECT(resample)));

    return ret;
}

static void gst_audio_resample_reset_state(GstAudioResample * resample)
{
    if (resample->state)
        resample->funcs->reset_mem(resample->state);
}

static gboolean gst_audio_resample_parse_caps(GstCaps * incaps, GstCaps * outcaps, gint * width, gint * channels, gint * inrate, gint * outrate, gboolean * fp)
{
    GstStructure *structure;
    gboolean ret;
...
static gboolean gst_audio_resample_transform_size(GstBaseTransform * base, GstPadDirection direction, GstCaps * caps, guint size, GstCaps * othercaps, guint * othersize)
{
    gboolean ret = TRUE;
    ...
    GST_LOG_OBJECT(base, "asked to transform size %d in direction %s", size, direction == GST_PAD_SINK ? "SINK" : "SRC");

    /* Get sample width -> bytes_per_samp, channels, inrate, outrate */
    ret = gst_audio_resample_parse_caps(caps, othercaps, &bytes_per_samp, &channels, &inrate, &outrate, NULL);
    if (G_UNLIKELY(!ret))
    {
    ...
    GST_LOG_OBJECT(base, "transformed size %d to %d", size * bytes_per_samp, *othersize);
...
static gboolean gst_audio_resample_set_caps(GstBaseTransform * base, GstCaps * incaps, GstCaps * outcaps)
{
    ...
    GST_LOG("incaps %" GST_PTR_FORMAT ", outcaps %" GST_PTR_FORMAT, incaps, outcaps);

    ret = gst_audio_resample_parse_caps(incaps, outcaps, &width, &channels, &inrate, &outrate, &fp);

    if (G_UNLIKELY(!ret))
        return FALSE;

    ret = gst_audio_resample_update_state(resample, width, channels, inrate, outrate, resample->quality, fp);
...
static void gst_audio_resample_convert_buffer(GstAudioResample * resample, const guint8 * in, guint8 * out, guint len, gboolean inverse)
{
    ...
    if (inverse)
    {
        ...
        else if (!gst_audio_resample_use_int && resample->width == 8 && !resample->fp)
        {
            gint8 *o = (gint8 *) out;
            gfloat *i = (gfloat *) in;
            gfloat tmp;
        ...
        else if (!gst_audio_resample_use_int && resample->width == 16 && !resample->fp)
        {
            gint16 *o = (gint16 *) out;
            gfloat *i = (gfloat *) in;
            gfloat tmp;
        ...
        else if (resample->width == 24 && !resample->fp)
        {
            ...
            {
                tmp = *i;
                GST_WRITE_UINT24(o, (gint32) CLAMP(tmp * GST_MAXINT24 + 0.5, GST_MININT24, GST_MAXINT24));
                o += 3;
                i++;
                len--;
            }
...
static guint8 *gst_audio_resample_workspace_realloc(guint8 ** workspace, guint * size, guint new_size)
{
    guint8 *new;
    if (new_size <= *size)
        /* no need to resize */
...
static void gst_audio_resample_push_drain(GstAudioResample * resample)
{
    ...
    out_len = out_processed = gst_util_uint64_scale_int_ceil(history_len, den, num);
    outsize = out_len * resample->channels * (resample->width / 8);

    res = gst_pad_alloc_buffer_and_set_caps(GST_BASE_TRANSFORM_SRC_PAD(resample), GST_BUFFER_OFFSET_NONE, outsize, GST_PAD_CAPS(GST_BASE_TRANSFORM_SRC_PAD(resample)), &outbuf);
    if (G_UNLIKELY(res != GST_FLOW_OK))
    {
        GST_WARNING_OBJECT(resample, "failed allocating buffer of %d bytes", outsize);
        return;
    }
    ...
    if (resample->funcs->width != resample->width)
    {
        ...
        if (!gst_audio_resample_workspace_realloc(&resample->tmp_out, &resample->tmp_out_size, (resample->funcs->width / 8) * out_len * resample->channels))
        {
        ...
        /* process */
        err = resample->funcs->process(resample->state, NULL, &history_len, resample->tmp_out, &out_processed);

        /* convert output format */
        gst_audio_resample_convert_buffer(resample, resample->tmp_out, GST_BUFFER_DATA(outbuf), out_processed, TRUE);
    }
    else
    {
        /* don't need to convert data format;  process */
        err = resample->funcs->process(resample->state, NULL, &history_len, GST_BUFFER_DATA(outbuf), &out_processed);
    }
    ...
    if (G_UNLIKELY(err != RESAMPLER_ERR_SUCCESS))
    {
        GST_WARNING_OBJECT(resample, "Failed to process drain: %s", resample->funcs->strerror(err));
    ...
    if (GST_CLOCK_TIME_IS_VALID(resample->t0))
    {
        ...
        GST_BUFFER_TIMESTAMP(outbuf) = resample->t0 + gst_util_uint64_scale_int_round(GST_BUFFER_OFFSET(outbuf) - resample->out_offset0, GST_SECOND, resample->outrate);
        GST_BUFFER_DURATION(outbuf) = resample->t0 + gst_util_uint64_scale_int_round(GST_BUFFER_OFFSET_END(outbuf) - resample->out_offset0, GST_SECOND, resample->outrate) - GST_BUFFER_TIMESTAMP(outbuf);
    ...
    GST_BUFFER_SIZE(outbuf) = out_processed * resample->channels * (resample->width / 8);
    ...
    GST_LOG_OBJECT(resample,
                   ...
                   G_GUINT64_FORMAT, GST_BUFFER_SIZE(outbuf), GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(outbuf)), GST_TIME_ARGS(GST_BUFFER_DURATION(outbuf)), GST_BUFFER_OFFSET(outbuf), GST_BUFFER_OFFSET_END(outbuf));
...
static gboolean gst_audio_resample_check_discont(GstAudioResample * resample, GstBuffer * buf)
{
    guint64 offset;
    guint64 delta;
    ...
    if (G_UNLIKELY(!(GST_BUFFER_TIMESTAMP_IS_VALID(buf) && GST_CLOCK_TIME_IS_VALID(resample->t0) && resample->in_offset0 != GST_BUFFER_OFFSET_NONE && resample->next_in_offset != GST_BUFFER_OFFSET_NONE)))
        return FALSE;

    /* convert the inbound timestamp to an offset. */
    offset = resample->in_offset0 + gst_util_uint64_scale_int_round(GST_BUFFER_TIMESTAMP(buf) - resample->t0, resample->inrate, GST_SECOND);
    ...
    GST_WARNING_OBJECT(resample, "encountered timestamp discontinuity of %" G_GUINT64_FORMAT " samples = %" GST_TIME_FORMAT, delta, GST_TIME_ARGS(gst_util_uint64_scale_int_round(delta, GST_SECOND, resample->inrate)));
    return TRUE;
}

static GstFlowReturn gst_audio_resample_process(GstAudioResample * resample, GstBuffer * inbuf, GstBuffer * outbuf)
{
    ...
    if (resample->funcs->width != resample->width)
    {
        ...
        if (!gst_audio_resample_workspace_realloc(&resample->tmp_in,
                                                  ...
                                                  (resample->funcs->width / 8)) || !gst_audio_resample_workspace_realloc(&resample->tmp_out, &resample->tmp_out_size, out_len * resample->channels * (resample->funcs->width / 8)))
        {
            GST_ERROR_OBJECT(resample, "failed to allocate workspace");
        ...
        /* convert input */
        gst_audio_resample_convert_buffer(resample, GST_BUFFER_DATA(inbuf), resample->tmp_in, in_len, FALSE);

        /* process */
        err = resample->funcs->process(resample->state, resample->tmp_in, &in_processed, resample->tmp_out, &out_processed);
    ...
    else
    {
        ...
        err = resample->funcs->process(resample->state, GST_BUFFER_DATA(inbuf), &in_processed, GST_BUFFER_DATA(outbuf), &out_processed);
    }
    ...
    if (G_UNLIKELY(err != RESAMPLER_ERR_SUCCESS))
    {
        GST_ERROR_OBJECT(resample, "Failed to convert data: %s", resample->funcs->strerror(err));
        return GST_FLOW_ERROR;
    }

    if (G_UNLIKELY(in_len != in_processed))
    {
        GST_WARNING_OBJECT(resample, "converted %d of %d input samples", in_processed, in_len);
    }
    ...
    GST_LOG_OBJECT(resample,
                   ...
                   G_GUINT64_FORMAT, out_processed, GST_BUFFER_SIZE(outbuf), GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(outbuf)), GST_TIME_ARGS(GST_BUFFER_DURATION(outbuf)), GST_BUFFER_OFFSET(outbuf), GST_BUFFER_OFFSET_END(outbuf));
...
static GstFlowReturn gst_audio_resample_transform(GstBaseTransform * base, GstBuffer * inbuf, GstBuffer * outbuf)
{
    ...
    if (resample->state == NULL)
    {
        if (G_UNLIKELY(!(resample->state = gst_audio_resample_init_state(resample, resample->width, resample->channels, resample->inrate, resample->outrate, resample->quality, resample->fp))))
            return GST_FLOW_ERROR;

        resample->funcs = gst_audio_resample_get_funcs(resample->width, resample->fp);
    }
    ...
    GST_LOG_OBJECT(resample, "transforming buffer of %ld bytes, ts %"
                   ...
                   G_GINT64_FORMAT ", offset_end %" G_GINT64_FORMAT, size, GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(inbuf)), GST_TIME_ARGS(GST_BUFFER_DURATION(inbuf)), GST_BUFFER_OFFSET(inbuf), GST_BUFFER_OFFSET_END(inbuf));
    ...
    if (G_UNLIKELY(resample->need_discont))
    {
        ...
        if (GST_BUFFER_TIMESTAMP_IS_VALID(inbuf) && GST_BUFFER_OFFSET_IS_VALID(inbuf))
        {
            resample->t0 = GST_BUFFER_TIMESTAMP(inbuf);
            resample->in_offset0 = GST_BUFFER_OFFSET(inbuf);
            resample->out_offset0 = gst_util_uint64_scale_int_round(resample->in_offset0, resample->outrate, resample->inrate);
            resample->next_in_offset = resample->in_offset0;
            resample->next_out_offset = resample->out_offset0;
        }
        else
        {
            GST_DEBUG_OBJECT(resample, "found discontinuity but timestamp and/or " "offset is invalid, cannot sync output timestamp and offset counter");
            resample->t0 = GST_CLOCK_TIME_NONE;
            resample->in_offset0 = GST_BUFFER_OFFSET_NONE;
            resample->out_offset0 = GST_BUFFER_OFFSET_NONE;
            resample->next_in_offset = GST_BUFFER_OFFSET_NONE;
    ...
    GST_DEBUG_OBJECT(resample, "input = samples [%" G_GUINT64_FORMAT ", %"
                     ...
                     GST_BUFFER_DURATION(inbuf), GST_BUFFER_OFFSET(outbuf), GST_BUFFER_OFFSET_END(outbuf), GST_BUFFER_TIMESTAMP(outbuf), GST_BUFFER_TIMESTAMP(outbuf) + GST_BUFFER_DURATION(outbuf));
        ...
        case GST_QUERY_LATENCY:
            {
                ...
                if (resample->state)
                    resampler_latency = resample->funcs->get_input_latency(resample->state);
                else
                    resampler_latency = 0;
                ...
                if ((peer = gst_pad_get_peer(GST_BASE_TRANSFORM_SINK_PAD(trans))))
                {
                    if ((res = gst_pad_query(peer, query)))
                    {
                        ...
                        GST_DEBUG_OBJECT(resample, "Peer latency: min %" GST_TIME_FORMAT " max %" GST_TIME_FORMAT, GST_TIME_ARGS(min), GST_TIME_ARGS(max));

                        /* add our own latency */
                        if (rate != 0 && resampler_latency != 0)
                            latency = gst_util_uint64_scale_round(resampler_latency, GST_SECOND, rate);
                        else
                            latency = 0;

                        GST_DEBUG_OBJECT(resample, "Our latency: %" GST_TIME_FORMAT, GST_TIME_ARGS(latency));

                        min += latency;
                        if (GST_CLOCK_TIME_IS_VALID(max))
                            max += latency;

                        GST_DEBUG_OBJECT(resample, "Calculated total latency : min %" GST_TIME_FORMAT " max %" GST_TIME_FORMAT, GST_TIME_ARGS(min), GST_TIME_ARGS(max));
...
static void gst_audio_resample_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    ...
    switch (prop_id)
    {
        case PROP_QUALITY:
            ...
            gst_audio_resample_update_state(resample, resample->width, resample->channels, resample->inrate, resample->outrate, resample->quality, resample->fp);
...
static gboolean _benchmark_int_float(SpeexResamplerState * st)
{
    ...
    resample_float_resampler_process_interleaved_float(st, (const guint8 *) in_tmp, &inlen, (guint8 *) out_tmp, &outlen);
...
static gboolean _benchmark_int_int(SpeexResamplerState * st)
{
    gint16 in[BENCHMARK_SIZE] = { 0, }, out[BENCHMARK_SIZE / 2];
    guint32 inlen = BENCHMARK_SIZE, outlen = BENCHMARK_SIZE / 2;

    resample_int_resampler_process_interleaved_int(st, (const guint8 *) in, &inlen, (guint8 *) out, &outlen);
...
static gboolean _benchmark_integer_resampling(void)
{
    OrcProfile a, b;
    gdouble av, bv;
...
{
    GST_DEBUG_CATEGORY_INIT(audio_resample_debug, "audioresample", 0, "audio resampling element");
    ...
    if (!gst_element_register(plugin, "audioresample", GST_RANK_PRIMARY, GST_TYPE_AUDIO_RESAMPLE))
    {
        return FALSE;
    }
...
GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, "audioresample", "Resamples audio", plugin_init, VERSION, "LGPL", GST_PACKAGE_NAME, GST_PACKAGE_ORIGIN);
