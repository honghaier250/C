...
static GstStaticPadTemplate gdp_depay_sink_template = GST_STATIC_PAD_TEMPLATE("sink",
                                                                              GST_PAD_SINK,
                                                                              GST_PAD_ALWAYS,
                                                                              GST_STATIC_CAPS("application/x-gdp"));

static GstStaticPadTemplate gdp_depay_src_template = GST_STATIC_PAD_TEMPLATE("src",
                                                                             GST_PAD_SRC,
                                                                             GST_PAD_ALWAYS,
                                                                             GST_STATIC_CAPS_ANY);
...
GST_BOILERPLATE_FULL(GstGDPDepay, gst_gdp_depay, GstElement, GST_TYPE_ELEMENT, _do_init);
...
static GstStateChangeReturn gst_gdp_depay_change_state(GstElement * element, GstStateChange transition);

static void gst_gdp_depay_finalize(GObject * object);

static void gst_gdp_depay_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_set_details_simple(element_class, "GDP Depayloader", "GDP/Depayloader", "Depayloads GStreamer Data Protocol buffers", "Thomas Vander Stichele <thomas at apestaart dot org>");

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gdp_depay_sink_template));
    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gdp_depay_src_template));
}

static void gst_gdp_depay_class_init(GstGDPDepayClass * klass)
{
    ...
    gstelement_class->change_state = GST_DEBUG_FUNCPTR(gst_gdp_depay_change_state);
    gobject_class->finalize = gst_gdp_depay_finalize;
}

static void gst_gdp_depay_init(GstGDPDepay * gdpdepay, GstGDPDepayClass * g_class)
{
    gdpdepay->sinkpad = gst_pad_new_from_static_template(&gdp_depay_sink_template, "sink");
    gst_pad_set_chain_function(gdpdepay->sinkpad, GST_DEBUG_FUNCPTR(gst_gdp_depay_chain));
    gst_pad_set_event_function(gdpdepay->sinkpad, GST_DEBUG_FUNCPTR(gst_gdp_depay_sink_event));
    gst_element_add_pad(GST_ELEMENT(gdpdepay), gdpdepay->sinkpad);

    gdpdepay->srcpad = gst_pad_new_from_static_template(&gdp_depay_src_template, "src");
    gst_pad_set_event_function(gdpdepay->srcpad, GST_DEBUG_FUNCPTR(gst_gdp_depay_src_event));
...
static void gst_gdp_depay_finalize(GObject * gobject)
{
    GstGDPDepay *this;
            ...
            case GST_GDP_DEPAY_STATE_HEADER:
                {
                    ...
                    GST_LOG_OBJECT(this, "read GDP header, payload size %d, payload type %d, switching to state PAYLOAD", this->payload_length, this->payload_type);
            ...
            case GST_GDP_DEPAY_STATE_PAYLOAD:
                {
                    ...
                    if (this->payload_length && (!gst_dp_validate_payload(GST_DP_HEADER_LENGTH, this->header, gst_adapter_peek(this->adapter, this->payload_length))))
                    {
            ...
            case GST_GDP_DEPAY_STATE_BUFFER:
                {
                    ...
                    GST_LOG_OBJECT(this, "deserialized buffer %p, pushing, timestamp %"
                                   ...
                                   ", size %d, flags 0x%x", buf, GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buf)), GST_TIME_ARGS(GST_BUFFER_DURATION(buf)), GST_BUFFER_OFFSET(buf), GST_BUFFER_OFFSET_END(buf), GST_BUFFER_SIZE(buf), GST_BUFFER_FLAGS(buf));
            ...
            case GST_GDP_DEPAY_STATE_CAPS:
                {
                    ...
                    payload = gst_adapter_take(this->adapter, this->payload_length);
                    caps = gst_dp_caps_from_packet(GST_DP_HEADER_LENGTH, this->header, payload);
                    g_free(payload);
                    if (!caps)
                        goto caps_failed;
            ...
            case GST_GDP_DEPAY_STATE_EVENT:
                {
                    ...
                    if (this->payload_length > 0)
                        ...
                        payload = NULL;
                    event = gst_dp_event_from_packet(GST_DP_HEADER_LENGTH, this->header, payload);
                    g_free(payload);
                    if (!event)
                        goto event_failed;

                    GST_DEBUG_OBJECT(this, "deserialized event %p of type %s, pushing", event, gst_event_type_get_name(event->type));
...
header_validate_error:
    {
        GST_ELEMENT_ERROR(this, STREAM, DECODE, (NULL), ("GDP packet header does not validate"));
        ret = GST_FLOW_ERROR;
        goto done;
    }
payload_validate_error:
    {
        GST_ELEMENT_ERROR(this, STREAM, DECODE, (NULL), ("GDP packet payload does not validate"));
        ret = GST_FLOW_ERROR;
        goto done;
    }
wrong_type:
    {
        GST_ELEMENT_ERROR(this, STREAM, DECODE, (NULL), ("GDP packet header is of wrong type"));
        ret = GST_FLOW_ERROR;
        goto done;
    }
no_caps:
    {
        GST_ELEMENT_ERROR(this, STREAM, DECODE, (NULL), ("Received a buffer without first receiving caps"));
        ret = GST_FLOW_NOT_NEGOTIATED;
        goto done;
    }
buffer_failed:
    {
        GST_ELEMENT_ERROR(this, STREAM, DECODE, (NULL), ("could not create buffer from GDP packet"));
        ret = GST_FLOW_ERROR;
        goto done;
    }
push_error:
...
caps_failed:
    {
        GST_ELEMENT_ERROR(this, STREAM, DECODE, (NULL), ("could not create caps from GDP packet"));
        ret = GST_FLOW_ERROR;
        goto done;
    }
event_failed:
    {
        GST_ELEMENT_ERROR(this, STREAM, DECODE, (NULL), ("could not create event from GDP packet"));
        ret = GST_FLOW_ERROR;
        goto done;
    }
}
...
{
    if (!gst_element_register(plugin, "gdpdepay", GST_RANK_NONE, GST_TYPE_GDP_DEPAY))
        return FALSE;
