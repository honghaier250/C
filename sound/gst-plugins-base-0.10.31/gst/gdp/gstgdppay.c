...
static GstStaticPadTemplate gdp_pay_sink_template = GST_STATIC_PAD_TEMPLATE("sink",
                                                                            GST_PAD_SINK,
                                                                            GST_PAD_ALWAYS,
                                                                            GST_STATIC_CAPS_ANY);

static GstStaticPadTemplate gdp_pay_src_template = GST_STATIC_PAD_TEMPLATE("src",
                                                                           GST_PAD_SRC,
                                                                           GST_PAD_ALWAYS,
...
GST_BOILERPLATE_FULL(GstGDPPay, gst_gdp_pay, GstElement, GST_TYPE_ELEMENT, _do_init);
...
static GstStateChangeReturn gst_gdp_pay_change_state(GstElement * element, GstStateChange transition);

static void gst_gdp_pay_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_gdp_pay_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

static void gst_gdp_pay_finalize(GObject * gobject);

static void gst_gdp_pay_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_set_details_simple(element_class, "GDP Payloader", "GDP/Payloader", "Payloads GStreamer Data Protocol buffers", "Thomas Vander Stichele <thomas at apestaart dot org>");

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gdp_pay_sink_template));
    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gdp_pay_src_template));
}

static void gst_gdp_pay_class_init(GstGDPPayClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_CRC_HEADER, g_param_spec_boolean("crc-header", "CRC Header", "Calculate and store a CRC checksum on the header", DEFAULT_CRC_HEADER, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_CRC_PAYLOAD, g_param_spec_boolean("crc-payload", "CRC Payload", "Calculate and store a CRC checksum on the payload", DEFAULT_CRC_PAYLOAD, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_VERSION, g_param_spec_enum("version", "Version", "Version of the GStreamer Data Protocol", GST_TYPE_DP_VERSION, DEFAULT_VERSION, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    gstelement_class->change_state = GST_DEBUG_FUNCPTR(gst_gdp_pay_change_state);
}

static void gst_gdp_pay_init(GstGDPPay * gdppay, GstGDPPayClass * g_class)
{
    gdppay->sinkpad = gst_pad_new_from_static_template(&gdp_pay_sink_template, "sink");
    gst_pad_set_chain_function(gdppay->sinkpad, GST_DEBUG_FUNCPTR(gst_gdp_pay_chain));
    gst_pad_set_event_function(gdppay->sinkpad, GST_DEBUG_FUNCPTR(gst_gdp_pay_sink_event));
    gst_element_add_pad(GST_ELEMENT(gdppay), gdppay->sinkpad);

    gdppay->srcpad = gst_pad_new_from_static_template(&gdp_pay_src_template, "src");
    gst_pad_set_event_function(gdppay->srcpad, GST_DEBUG_FUNCPTR(gst_gdp_pay_src_event));
...
static void gst_gdp_stamp_buffer(GstGDPPay * this, GstBuffer * buffer)
{
...
static GstBuffer *gst_gdp_buffer_from_caps(GstGDPPay * this, GstCaps * caps)
{
    GstBuffer *headerbuf;
    ...
    if (!this->packetizer->packet_from_caps(caps, this->header_flag, &len, &header, &payload))
        goto packet_failed;
    ...
    gst_buffer_set_data(payloadbuf, payload, gst_dp_header_payload_length(header));
...
static GstBuffer *gst_gdp_pay_buffer_from_buffer(GstGDPPay * this, GstBuffer * buffer)
{
    GstBuffer *headerbuf;
    ...
    if (!this->packetizer->header_from_buffer(buffer, this->header_flag, &len, &header))
        goto no_buffer;
...
static GstBuffer *gst_gdp_buffer_from_event(GstGDPPay * this, GstEvent * event)
{
    GstBuffer *headerbuf;
    ...
    ret = this->packetizer->packet_from_event(event, this->header_flag, &len, &header, &payload);
    if (!ret)
        goto no_event;
...
no_event:
    {
        GST_WARNING_OBJECT(this, "could not create GDP header from event %s (%d)", gst_event_type_get_name(event->type), event->type);
        return NULL;
    }
}
...
static GstFlowReturn gst_gdp_pay_reset_streamheader(GstGDPPay * this)
{
    GstCaps *caps;
    ...
    if (version_one_zero)
    {
        ...
        GST_DEBUG_OBJECT(this, "1.0, appending copy of new segment buffer %p", this->new_segment_buf);
        ...
        if (this->tag_buf)
        {
            ...
            GST_DEBUG_OBJECT(this, "1.0, appending current tags buffer %p", this->tag_buf);
    ...
    if (gst_structure_has_field(structure, "streamheader"))
    {
        ...
        GST_DEBUG_OBJECT(this, "Need to serialize %d incoming streamheader buffers on ours", buffers->len);
        for (i = 0; i < buffers->len; ++i)
        {
            ...
            GST_DEBUG_OBJECT(this, "Setting IN_CAPS flag on outgoing buffer %" GST_PTR_FORMAT, outbuffer);
    ...
    GST_DEBUG_OBJECT(this, "%d serialized buffers on streamheaders", gst_value_array_get_size(&array));
    ...
    {
        GstEvent *event = gst_event_new_new_segment(TRUE, 1.0, GST_FORMAT_BYTES, 0, -1, 0);
    ...
    GST_DEBUG_OBJECT(this, "Pushing GDP new_segment buffer %p with offset %" G_GINT64_FORMAT ", offset_end %" G_GINT64_FORMAT, this->new_segment_buf, GST_BUFFER_OFFSET(this->new_segment_buf), GST_BUFFER_OFFSET_END(this->new_segment_buf));
    ...
    GST_DEBUG_OBJECT(this, "need to push %d queued buffers", g_list_length(this->queue));
    while (this->queue)
    {
        GstBuffer *buffer;
...
no_buffer:
    {
        GST_ELEMENT_ERROR(this, STREAM, FORMAT, (NULL), ("failed to create GDP buffer from streamheader"));
...
static GstFlowReturn gst_gdp_queue_buffer(GstGDPPay * this, GstBuffer * buffer)
{
    if (this->sent_streamheader)
    {
        GST_LOG_OBJECT(this, "Pushing GDP buffer %p, caps %" GST_PTR_FORMAT, buffer, this->caps);
    ...
    GST_DEBUG_OBJECT(this, "streamheader not sent yet, " "queued buffer %p, now %d buffers queued", buffer, g_list_length(this->queue));
...
static GstFlowReturn gst_gdp_pay_chain(GstPad * pad, GstBuffer * buffer)
{
    ...
    if (!this->new_segment_buf)
    {
        ...
        GST_WARNING_OBJECT(this, "did not receive new-segment before first buffer");
        ...
        if (!outbuffer)
        {
            GST_ELEMENT_WARNING(this, STREAM, ENCODE, (NULL), ("Could not create GDP buffer from new segment event"));
        }
        else
        {
    ...
    {
        GST_DEBUG_OBJECT(this, "Setting IN_CAPS flag on outgoing buffer %p", outbuffer);
        GST_BUFFER_FLAG_SET(outbuffer, GST_BUFFER_FLAG_IN_CAPS);
    }
...
no_caps:
    {
         ...
         * message */
        GST_ELEMENT_ERROR(this, STREAM, FORMAT, (NULL), ("first received buffer does not have caps set"));
        if (caps)
            gst_caps_unref(caps);
...
no_caps_buffer:
    {
        GST_ELEMENT_ERROR(this, STREAM, ENCODE, (NULL), ("Could not create GDP buffer from caps %" GST_PTR_FORMAT, caps));
        gst_caps_unref(caps);
        ret = GST_FLOW_ERROR;
        goto done;
    }
no_buffer:
    {
        GST_ELEMENT_ERROR(this, STREAM, ENCODE, (NULL), ("Could not create GDP buffer from buffer"));
        ret = GST_FLOW_ERROR;
        goto done;
    }
}
...
static gboolean gst_gdp_pay_sink_event(GstPad * pad, GstEvent * event)
{
    ...
    GST_DEBUG_OBJECT(this, "received event %p of type %s (%d)", event, gst_event_type_get_name(event->type), event->type);
    ...
    switch (GST_EVENT_TYPE(event))
    {
        case GST_EVENT_NEWSEGMENT:
            GST_DEBUG_OBJECT(this, "Storing in caps buffer %p as new_segment_buf", outbuffer);
            ...
            break;
        case GST_EVENT_TAG:
            GST_DEBUG_OBJECT(this, "Storing in caps buffer %p as tag_buf", outbuffer);
            ...
            break;
        default:
            GST_DEBUG_OBJECT(this, "queuing GDP buffer %p of event %p", outbuffer, event);
...
no_outbuffer:
    {
        GST_ELEMENT_WARNING(this, STREAM, ENCODE, (NULL), ("Could not create GDP buffer from received event (type %s)", gst_event_type_get_name(event->type)));
        ret = FALSE;
        goto done;
    }
push_error:
...
static void gst_gdp_pay_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    ...
    switch (prop_id)
    {
        case PROP_CRC_HEADER:
            this->crc_header = g_value_get_boolean(value) ? GST_DP_HEADER_FLAG_CRC_HEADER : 0;
            this->header_flag = this->crc_header | this->crc_payload;
            break;
        case PROP_CRC_PAYLOAD:
            this->crc_payload = g_value_get_boolean(value) ? GST_DP_HEADER_FLAG_CRC_PAYLOAD : 0;
            this->header_flag = this->crc_header | this->crc_payload;
            break;
        case PROP_VERSION:
