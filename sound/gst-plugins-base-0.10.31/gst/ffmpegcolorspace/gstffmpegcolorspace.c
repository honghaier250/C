...
static GstStaticPadTemplate gst_ffmpegcsp_src_template = GST_STATIC_PAD_TEMPLATE("src",
                                                                                 GST_PAD_SRC,
                                                                                 GST_PAD_ALWAYS,
                                                                                 GST_STATIC_CAPS(FFMPEGCSP_VIDEO_CAPS));

static GstStaticPadTemplate gst_ffmpegcsp_sink_template = GST_STATIC_PAD_TEMPLATE("sink",
                                                                                  GST_PAD_SINK,
                                                                                  GST_PAD_ALWAYS,
                                                                                  GST_STATIC_CAPS(FFMPEGCSP_VIDEO_CAPS));
...
static gboolean gst_ffmpegcsp_set_caps(GstBaseTransform * btrans, GstCaps * incaps, GstCaps * outcaps);
static gboolean gst_ffmpegcsp_get_unit_size(GstBaseTransform * btrans, GstCaps * caps, guint * size);
static GstFlowReturn gst_ffmpegcsp_transform(GstBaseTransform * btrans, GstBuffer * inbuf, GstBuffer * outbuf);
...
static GstCaps *gst_ffmpegcsp_caps_remove_format_info(GstCaps * caps)
{
    ...
    gst_structure_remove_fields(yuvst, "format", "endianness", "depth", "bpp", "red_mask", "green_mask", "blue_mask", "alpha_mask", "palette_data", NULL);
...
static gboolean gst_ffmpegcsp_structure_is_alpha(GstStructure * s)
{
    GQuark name;
...
static GstCaps *gst_ffmpegcsp_transform_caps(GstBaseTransform * btrans, GstPadDirection direction, GstCaps * caps)
{
    GstCaps *template;
    GstCaps *tmp, *tmp2;
    GstCaps *result;
    ...
    GST_DEBUG_OBJECT(btrans, "transformed %" GST_PTR_FORMAT " into %" GST_PTR_FORMAT, caps, result);
...
static gboolean gst_ffmpegcsp_set_caps(GstBaseTransform * btrans, GstCaps * incaps, GstCaps * outcaps)
{
    ...
    /* these must match */
    if (in_width != out_width || in_height != out_height || gst_value_compare(in_framerate, out_framerate) != GST_VALUE_EQUAL)
...
GST_BOILERPLATE(GstFFMpegCsp, gst_ffmpegcsp, GstVideoFilter, GST_TYPE_VIDEO_FILTER);

static void gst_ffmpegcsp_base_init(gpointer klass)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(klass);

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gst_ffmpegcsp_src_template));
    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gst_ffmpegcsp_sink_template));

    gst_element_class_set_details_simple(element_class, "FFMPEG Colorspace converter", "Filter/Converter/Video", "Converts video from one colorspace to another", "GStreamer maintainers <gstreamer-devel@lists.sourceforge.net>");
...
static void gst_ffmpegcsp_finalize(GObject * obj)
{
...
static void gst_ffmpegcsp_class_init(GstFFMpegCspClass * klass)
{
    ...
    gstbasetransform_class->transform_caps = GST_DEBUG_FUNCPTR(gst_ffmpegcsp_transform_caps);
    gstbasetransform_class->set_caps = GST_DEBUG_FUNCPTR(gst_ffmpegcsp_set_caps);
    gstbasetransform_class->get_unit_size = GST_DEBUG_FUNCPTR(gst_ffmpegcsp_get_unit_size);
    gstbasetransform_class->transform = GST_DEBUG_FUNCPTR(gst_ffmpegcsp_transform);

    gstbasetransform_class->passthrough_on_same_caps = TRUE;
}

static void gst_ffmpegcsp_init(GstFFMpegCsp * space, GstFFMpegCspClass * klass)
{
...
static GstFlowReturn gst_ffmpegcsp_transform(GstBaseTransform * btrans, GstBuffer * inbuf, GstBuffer * outbuf)
{
    ...
    if (G_UNLIKELY(space->from_pixfmt == PIX_FMT_NB || space->to_pixfmt == PIX_FMT_NB))
        goto unknown_format;

    /* fill from with source data */
    gst_ffmpegcsp_avpicture_fill(&space->from_frame, GST_BUFFER_DATA(inbuf), space->from_pixfmt, space->width, space->height, space->interlaced);
    ...
    /* fill target frame */
    gst_ffmpegcsp_avpicture_fill(&space->to_frame, GST_BUFFER_DATA(outbuf), space->to_pixfmt, space->width, space->height, space->interlaced);

    /* and convert */
    result = img_convert(&space->to_frame, space->to_pixfmt, &space->from_frame, space->from_pixfmt, space->width, space->height);
    if (result == -1)
        goto not_supported;
...
unknown_format:
    {
        GST_ELEMENT_ERROR(space, CORE, NOT_IMPLEMENTED, (NULL), ("attempting to convert colorspaces between unknown formats"));
        return GST_FLOW_NOT_NEGOTIATED;
    }
not_supported:
    {
        GST_ELEMENT_ERROR(space, CORE, NOT_IMPLEMENTED, (NULL), ("cannot convert between formats"));
...
{
    GST_DEBUG_CATEGORY_INIT(ffmpegcolorspace_debug, "ffmpegcolorspace", 0, "FFMPEG-based colorspace converter");
    GST_DEBUG_CATEGORY_GET(ffmpegcolorspace_performance, "GST_PERFORMANCE");

    avcodec_init();

    return gst_element_register(plugin, "ffmpegcolorspace", GST_RANK_NONE, GST_TYPE_FFMPEGCSP);
}

GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, "ffmpegcolorspace", "colorspace conversion copied from FFMpeg " FFMPEG_VERSION, plugin_init, VERSION, "LGPL", "FFMpeg", "http://ffmpeg.sourceforge.net/")
