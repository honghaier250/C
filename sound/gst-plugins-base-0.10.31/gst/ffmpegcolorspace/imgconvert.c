...
PixFmtInfo *get_pix_fmt_info(enum PixelFormat format)
{
    ...
    g_warning("Could not find info for pixel format %d out of %d known pixel formats. One segfault coming up", format, PIX_FMT_NB);
    return NULL;
}
...
#if 0
static int avpicture_layout(const AVPicture * src, int pix_fmt, int width, int height, unsigned char *dest, int dest_size)
{
    ...
    {
        if (pix_fmt == PIX_FMT_YUV422 || pix_fmt == PIX_FMT_UYVY422 || pix_fmt == PIX_FMT_RGB565 || pix_fmt == PIX_FMT_RGB555)
            w = width * 2;
...
int avpicture_get_size(int pix_fmt, int width, int height)
{
    ...
    return gst_ffmpegcsp_avpicture_fill(&dummy_pict, NULL, pix_fmt, width, height, FALSE);
}
...
static void img_copy_plane(uint8_t * dst, int dst_wrap, const uint8_t * src, int src_wrap, int width, int height)
{
    for (; height > 0; height--)
    {
 ...
 */
static void img_copy(AVPicture * dst, const AVPicture * src, int pix_fmt, int width, int height)
{
    int bwidth, bits, i;
    const PixFmtInfo *pf;
    ...
    switch (pf->pixel_type)
    {
        case FF_PIXEL_PACKED:
            ...
            img_copy_plane(dst->data[0], dst->linesize[0], src->data[0], src->linesize[0], bwidth, height);
            break;
        case FF_PIXEL_PLANAR:
            for (i = 0; i < pf->nb_channels; i++)
            {
                ...
                img_copy_plane(dst->data[i], dst->linesize[i], src->data[i], src->linesize[i], bwidth, h);
            }
            break;
        case FF_PIXEL_PALETTE:
            img_copy_plane(dst->data[0], dst->linesize[0], src->data[0], src->linesize[0], width, height);
            /* copy the palette */
            img_copy_plane(dst->data[1], dst->linesize[1], src->data[1], src->linesize[1], 4, 256);
            break;
    }
}
...
static void uyvy422_to_gray(AVPicture * dst, const AVPicture * src, int width, int height)
{
    const uint8_t *p, *p1;
    uint8_t *lum, *lum1;
    int w;
...
static void yvyu422_to_gray(AVPicture * dst, const AVPicture * src, int width, int height)
{
    const uint8_t *p, *p1;
    uint8_t *lum, *lum1;
    int w;
...
static void yvyu422_to_yuv420p(AVPicture * dst, const AVPicture * src, int width, int height)
{
    const uint8_t *p, *p1;
    uint8_t *lum, *cr, *cb, *lum1, *cr1, *cb1;
    int w;
...
static void yvyu422_to_yuv422p(AVPicture * dst, const AVPicture * src, int width, int height)
{
    const uint8_t *p, *p1;
    uint8_t *lum, *cr, *cb, *lum1, *cr1, *cb1;
    int w;
...
static void yuv422p_to_yvyu422(AVPicture * dst, const AVPicture * src, int width, int height)
{
    uint8_t *p, *p1;
    const uint8_t *lum, *cr, *cb, *lum1, *cr1, *cb1;
    int w;
...
static void yuv411p_to_uyvy411(AVPicture * dst, const AVPicture * src, int width, int height)
{
    uint8_t *p, *p1;
    const uint8_t *lum, *cr, *cb, *lum1, *cr1, *cb1;
    int w;
...
static void nv12_to_nv21(AVPicture * dst, const AVPicture * src, int width, int height)
{
...
static void nv12_to_yuv444p(AVPicture * dst, const AVPicture * src, int width, int height)
{
    int w, h;
...
static void nv21_to_yuv444p(AVPicture * dst, const AVPicture * src, int width, int height)
{
    int w, h;
    uint8_t *dst_lum1, *dst_lum2, *dst_line = dst->data[0];
    uint8_t *dst_cb1, *dst_cb2, *dst_cb_line = dst->data[1];
...
static void yuva420p_to_yuv420p(AVPicture * dst, const AVPicture * src, int width, int height)
{
...
static void yuva420p_to_yuv422(AVPicture * dst, const AVPicture * src, int width, int height)
{
    int w, h;
    uint8_t *line1, *line2, *linesrc = dst->data[0];
    uint8_t *lum1, *lum2, *lumsrc = src->data[0];
...
static void img_copy_plane_resize(uint8_t * dst, int dst_wrap, int dst_width, int dst_height, const uint8_t * src, int src_wrap, int src_width, int src_height)
{
    img_copy_plane(dst, dst_wrap, src, src_wrap, dst_width, dst_height);
}

/* 4x1 -> 1x1 */
static void shrink41(uint8_t * dst, int dst_wrap, int dst_width, int dst_height, const uint8_t * src, int src_wrap, int src_width, int src_height)
{
    int w, s_w;
    const uint8_t *s;
    uint8_t *d;
...
/* 2x1 -> 1x1 */
static void shrink21(uint8_t * dst, int dst_wrap, int dst_width, int dst_height, const uint8_t * src, int src_wrap, int src_width, int src_height)
{
    int w, s_w;
    const uint8_t *s;
    uint8_t *d;
...
/* 1x2 -> 1x1 */
static void shrink12(uint8_t * dst, int dst_wrap, int dst_width, int dst_height, const uint8_t * src, int src_wrap, int src_width, int src_height)
{
    int w;
    uint8_t *d;
    const uint8_t *s1, *s2;
...
/* 2x2 -> 1x1 */
static void shrink22(uint8_t * dst, int dst_wrap, int dst_width, int dst_height, const uint8_t * src, int src_wrap, int src_width, int src_height)
{
    int w, s_w;
    const uint8_t *s1, *s2;
    uint8_t *d;
...
/* 4x4 -> 1x1 */
static void shrink44(uint8_t * dst, int dst_wrap, int dst_width, int dst_height, const uint8_t * src, int src_wrap, int src_width, int src_height)
{
    int s_w, w;
    ...
    for (; dst_height > 0; dst_height--, src_height -= 4)
    {
        ...
        {
            if (s_w == 3)
                d[0] = (s1[0] + s1[1] + s1[2] + s2[0] + s2[1] + s2[2] + s3[0] + s3[1] + s3[2] + s4[0] + s4[1] + s4[2]) / 12;
            else if (s_w == 2)
                d[0] = (s1[0] + s1[1] + s2[0] + s2[1] + s3[0] + s3[1] + s4[0] + s4[1]) / 8;
            else                /* s_w == 1 */
...
/* 1x1 -> 2x1 */
static void grow21(uint8_t * dst, int dst_wrap, int dst_width, int dst_height, const uint8_t * src, int src_wrap, int src_width, int src_height)
{
    for (; dst_height > 0; dst_height--)
    {
...
/* 1x1 -> 2x2 */
static void grow22(uint8_t * dst, int dst_wrap, int dst_width, int dst_height, const uint8_t * src, int src_wrap, int src_width, int src_height)
{
    for (; dst_height > 0; dst_height--)
    {
        grow21_line(dst, src, dst_width);
...
/* 1x1 -> 4x1 */
static void grow41(uint8_t * dst, int dst_wrap, int dst_width, int dst_height, const uint8_t * src, int src_wrap, int src_width, int src_height)
{
    for (; dst_height > 0; dst_height--)
    {
...
/* 1x1 -> 4x4 */
static void grow44(uint8_t * dst, int dst_wrap, int dst_width, int dst_height, const uint8_t * src, int src_wrap, int src_width, int src_height)
{
    for (; dst_height > 0; dst_height--)
    {
        grow41_line(dst, src, dst_width);
...
/* 1x2 -> 2x1 */
static void conv411(uint8_t * dst, int dst_wrap, int dst_width, int dst_height, const uint8_t * src, int src_wrap, int src_width, int src_height)
{
    int w, c;
    const uint8_t *s1, *s2;
    uint8_t *d;
...
static void build_rgb_palette(uint8_t * palette, int has_alpha)
{
    ...
    for (r = 0; r < 6; r++)
    {
        for (g = 0; g < 6; g++)
        {
            for (b = 0; b < 6; b++)
            {
                pal[i++] = (0xffU << 24) | (pal_value[r] << 16) | (pal_value[g] << 8) | pal_value[b];
            }
        }
    }
    if (has_alpha)
...
static void gray_to_gray16_l(AVPicture * dst, const AVPicture * src, int width, int height)
{
...
static void gray_to_gray16_b(AVPicture * dst, const AVPicture * src, int width, int height)
{
    const unsigned char *p;
    unsigned char *q;
    int dst_wrap, src_wrap;
...
static void gray16_l_to_gray(AVPicture * dst, const AVPicture * src, int width, int height)
{
    const unsigned char *p;
    unsigned char *q;
    int dst_wrap, src_wrap;
...
static void gray16_b_to_gray(AVPicture * dst, const AVPicture * src, int width, int height)
{
    const unsigned char *p;
    unsigned char *q;
    int dst_wrap, src_wrap;
...
static void gray16_b_to_gray16_l(AVPicture * dst, const AVPicture * src, int width, int height)
{
    const unsigned char *p;
    unsigned char *q;
    int dst_wrap, src_wrap;
...
static void y800_to_y16(AVPicture * dst, const AVPicture * src, int width, int height)
{
    const unsigned char *p;
    unsigned char *q;
    int dst_wrap, src_wrap;
...
static void y16_to_y800(AVPicture * dst, const AVPicture * src, int width, int height)
{
    const unsigned char *p;
    unsigned char *q;
    int dst_wrap, src_wrap;
...
static void yuva420p_to_ayuv4444(AVPicture * dst, const AVPicture * src, int width, int height)
{
...
static void ayuv4444_to_yuva420p(AVPicture * dst, const AVPicture * src, int width, int height)
{
...
static ConvertEntry *get_convert_table_entry(int src_pix_fmt, int dst_pix_fmt)
{
    int i;

    for (i = 0; i < sizeof(convert_table) / sizeof(convert_table[0]); i++)
    {
        if (convert_table[i].src == src_pix_fmt && convert_table[i].dest == dst_pix_fmt)
        {
...
static int avpicture_alloc(AVPicture * picture, int pix_fmt, int width, int height, int interlaced)
{
    unsigned int size;
    void *ptr;
    ...
    if (!ptr)
        goto fail;
    gst_ffmpegcsp_avpicture_fill(picture, ptr, pix_fmt, width, height, interlaced);
    return 0;
fail:
...
int img_convert(AVPicture * dst, int dst_pix_fmt, const AVPicture * src, int src_pix_fmt, int src_width, int src_height)
{
    ...
    /* gray to YUV */
    if (is_yuv_planar(dst_pix) && dst_pix_fmt != PIX_FMT_Y16 && src_pix_fmt == PIX_FMT_GRAY8)
    {
        int w, h, y;
        uint8_t *d;
        ...
        {
            img_copy_plane(dst->data[0], dst->linesize[0], src->data[0], src->linesize[0], dst_width, dst_height);
        }
        else
        {
    ...
    /* YUV to gray */
    if (is_yuv_planar(src_pix) && src_pix_fmt != PIX_FMT_Y16 && dst_pix_fmt == PIX_FMT_GRAY8)
    {
    ...
    /* YUV to YUV planar */
    if (is_yuv_planar(dst_pix) && is_yuv_planar(src_pix) && dst_pix->depth == src_pix->depth)
    {
        int x_shift, y_shift, xy_shift;
        void (*resize_func) (uint8_t * dst, int dst_wrap, int dst_width, int dst_height, const uint8_t * src, int src_wrap, int src_width, int src_height);
...
#define DIV_ROUND_UP_X(v,x) (((v) + GEN_MASK(x)) >> (x))
        ...
        for (i = 1; i <= 2; i++)
        {
            ...
            {
                resize_func(dst->data[i], dst->linesize[i], w, h, src->data[i], src->linesize[i], s_w, s_h);
            }
...
no_chroma_filter:
    GST_CAT_INFO(ffmpegcolorspace_performance, "no direct path to convert colorspace from %s -> %s", src_pix->name, dst_pix->name);
    ...
    else if (src_pix_fmt == PIX_FMT_UYVY422 || dst_pix_fmt == PIX_FMT_UYVY422 || src_pix_fmt == PIX_FMT_YVYU422 || dst_pix_fmt == PIX_FMT_YVYU422)
    {
        /* specific case: convert to YUV422P first */
        int_pix_fmt = PIX_FMT_YUV422P;
    }
    ...
    if (avpicture_alloc(tmp, int_pix_fmt, dst_width, dst_height, dst->interlaced) < 0)
        return -1;
    ret = -1;
...
#if 0
static int avpicture_deinterlace(AVPicture * dst, const AVPicture * src, int pix_fmt, int width, int height)
{
    int i;

    if (pix_fmt != PIX_FMT_YUV420P && pix_fmt != PIX_FMT_YUV422P && pix_fmt != PIX_FMT_YUV444P && pix_fmt != PIX_FMT_YUV411P)
        return -1;
