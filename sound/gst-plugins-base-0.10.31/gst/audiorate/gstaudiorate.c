...
static GstStaticPadTemplate gst_audio_rate_src_template = GST_STATIC_PAD_TEMPLATE("src",
                                                                                  GST_PAD_SRC,
                                                                                  GST_PAD_ALWAYS,
                                                                                  GST_STATIC_CAPS(GST_AUDIO_INT_PAD_TEMPLATE_CAPS ";" GST_AUDIO_FLOAT_PAD_TEMPLATE_CAPS));

static GstStaticPadTemplate gst_audio_rate_sink_template = GST_STATIC_PAD_TEMPLATE("sink",
                                                                                   GST_PAD_SINK,
                                                                                   GST_PAD_ALWAYS,
                                                                                   GST_STATIC_CAPS(GST_AUDIO_INT_PAD_TEMPLATE_CAPS ";" GST_AUDIO_FLOAT_PAD_TEMPLATE_CAPS));
...
static void gst_audio_rate_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_audio_rate_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

static GstStateChangeReturn gst_audio_rate_change_state(GstElement * element, GstStateChange transition);
...
static GType gst_audio_rate_get_type(void)
{
    ...
    if (!audio_rate_type)
    {
        ...
        audio_rate_type = g_type_register_static(GST_TYPE_ELEMENT, "GstAudioRate", &audio_rate_info, 0);
    }
...
static void gst_audio_rate_base_init(gpointer g_class)
{
    ...
    gst_element_class_set_details_simple(element_class, "Audio rate adjuster", "Filter/Effect/Audio", "Drops/duplicates/adjusts timestamps on audio samples to make a perfect stream", "Wim Taymans <wim@fluendo.com>");

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gst_audio_rate_sink_template));
    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gst_audio_rate_src_template));
}
...
static void gst_audio_rate_class_init(GstAudioRateClass * klass)
{
    ...
    g_object_class_install_property(object_class, ARG_IN, g_param_spec_uint64("in", "In", "Number of input samples", 0, G_MAXUINT64, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(object_class, ARG_OUT, g_param_spec_uint64("out", "Out", "Number of output samples", 0, G_MAXUINT64, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    pspec_add = g_param_spec_uint64("add", "Add", "Number of added samples", 0, G_MAXUINT64, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
    g_object_class_install_property(object_class, ARG_ADD, pspec_add);
    pspec_drop = g_param_spec_uint64("drop", "Drop", "Number of dropped samples", 0, G_MAXUINT64, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
   ...
   **/
    g_object_class_install_property(object_class, ARG_TOLERANCE,
                                    g_param_spec_uint64("tolerance", "tolerance", "Only act if timestamp jitter/imperfection exceeds indicated tolerance (ns)", 0, G_MAXUINT64, DEFAULT_TOLERANCE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    element_class->change_state = gst_audio_rate_change_state;
}

static void gst_audio_rate_reset(GstAudioRate * audiorate)
{
...
static gboolean gst_audio_rate_setcaps(GstPad * pad, GstCaps * caps)
{
    ...
    otherpad = (pad == audiorate->srcpad) ? audiorate->sinkpad : audiorate->srcpad;
...
static void gst_audio_rate_init(GstAudioRate * audiorate)
{
    audiorate->sinkpad = gst_pad_new_from_static_template(&gst_audio_rate_sink_template, "sink");
    ...
    audiorate->srcpad = gst_pad_new_from_static_template(&gst_audio_rate_src_template, "src");
...
static void gst_audio_rate_fill_to_time(GstAudioRate * audiorate, GstClockTime time)
{
    GstBuffer *buf;

    GST_DEBUG_OBJECT(audiorate, "next_ts: %" GST_TIME_FORMAT ", filling to %" GST_TIME_FORMAT, GST_TIME_ARGS(audiorate->next_ts), GST_TIME_ARGS(time));

    if (!GST_CLOCK_TIME_IS_VALID(time) || !GST_CLOCK_TIME_IS_VALID(audiorate->next_ts))
        return;
        ...
        case GST_EVENT_NEWSEGMENT:
            {
                ...
                gst_segment_set_newsegment_full(&audiorate->sink_segment, update, rate, arate, format, start, stop, time);

                GST_DEBUG_OBJECT(audiorate, "updated segment: %" GST_SEGMENT_FORMAT, &audiorate->sink_segment);

                if (format == GST_FORMAT_TIME)
                {
                    /* TIME formats can be copied to src and forwarded */
                    res = gst_pad_push_event(audiorate->srcpad, event);
                    memcpy(&audiorate->src_segment, &audiorate->sink_segment, sizeof(GstSegment));
                }
                else
                {
...
static gboolean gst_audio_rate_convert(GstAudioRate * audiorate, GstFormat src_fmt, gint64 src_val, GstFormat dest_fmt, gint64 * dest_val)
{
    if (src_fmt == dest_fmt)
    {
        *dest_val = src_val;
    ...
    switch (src_fmt)
    {
        case GST_FORMAT_DEFAULT:
            switch (dest_fmt)
            {
                ...
                case GST_FORMAT_TIME:
                    *dest_val = gst_util_uint64_scale_int(src_val, GST_SECOND, audiorate->rate);
                    break;
                default:
                    return FALSE;;
            }
            ...
            switch (dest_fmt)
            {
                case GST_FORMAT_DEFAULT:
                    *dest_val = src_val / audiorate->bytes_per_sample;
                    break;
                case GST_FORMAT_TIME:
                    *dest_val = gst_util_uint64_scale_int(src_val, GST_SECOND, audiorate->rate * audiorate->bytes_per_sample);
                    break;
                default:
                    return FALSE;;
            }
        ...
        case GST_FORMAT_TIME:
            ...
            {
                case GST_FORMAT_BYTES:
                    *dest_val = gst_util_uint64_scale_int(src_val, audiorate->rate * audiorate->bytes_per_sample, GST_SECOND);
                    break;
                case GST_FORMAT_DEFAULT:
                    *dest_val = gst_util_uint64_scale_int(src_val, audiorate->rate, GST_SECOND);
                    break;
                default:
                    return FALSE;;
            }
...
static void gst_audio_rate_notify_drop(GstAudioRate * audiorate)
{
...
static void gst_audio_rate_notify_add(GstAudioRate * audiorate)
{
#if !GLIB_CHECK_VERSION(2,26,0)
...
static GstFlowReturn gst_audio_rate_chain(GstPad * pad, GstBuffer * buf)
{
    ...
    if (audiorate->next_offset == -1)
    {
        ...
        pos = gst_util_uint64_scale_int(audiorate->src_segment.start, audiorate->rate, GST_SECOND);
        ...
        audiorate->next_ts = gst_util_uint64_scale_int(audiorate->next_offset, GST_SECOND, audiorate->rate);
    }
    ...
    in_offset = gst_util_uint64_scale_int_round(in_time, audiorate->rate, GST_SECOND);
    ...
    GST_LOG_OBJECT(audiorate,
                   ...
                   GST_TIME_FORMAT, GST_TIME_ARGS(in_time), GST_TIME_ARGS(GST_FRAMES_TO_CLOCK_TIME(in_samples, audiorate->rate)), in_size, in_offset, in_offset_end, audiorate->next_offset, GST_TIME_ARGS(audiorate->next_ts));

    diff = in_time - audiorate->next_ts;
    if (diff <= (GstClockTimeDiff) audiorate->tolerance && diff >= (GstClockTimeDiff) - audiorate->tolerance)
    {
        ...
        GST_LOG_OBJECT(audiorate, "within tolerance %" GST_TIME_FORMAT, GST_TIME_ARGS(audiorate->tolerance));
    ...
    if (in_offset > audiorate->next_offset)
    {
        ...
        while (fillsamples > 0)
        {
            ...
            GST_DEBUG_OBJECT(audiorate, "inserting %" G_GUINT64_FORMAT " samples", cursamples);
            ...
            audiorate->next_ts = gst_util_uint64_scale_int(audiorate->next_offset, GST_SECOND, audiorate->rate);
            GST_BUFFER_DURATION(fill) = audiorate->next_ts - GST_BUFFER_TIMESTAMP(fill);
    ...
    else if (in_offset < audiorate->next_offset)
    {
        ...
        if (in_offset_end <= audiorate->next_offset)
        {
            ...
            GST_DEBUG_OBJECT(audiorate, "dropping %" G_GUINT64_FORMAT " samples", drop);
        ...
        else
        {
            ...
            GST_DEBUG_OBJECT(audiorate, "truncating %" G_GUINT64_FORMAT " samples", truncsamples);
...
send:
    ...
    audiorate->next_ts = gst_util_uint64_scale_int(in_offset_end, GST_SECOND, audiorate->rate);
    ...
    gst_segment_set_last_stop(&audiorate->src_segment, GST_FORMAT_TIME, GST_BUFFER_TIMESTAMP(buf) + GST_BUFFER_DURATION(buf));
...
not_negotiated:
    {
        ...
        GST_ELEMENT_ERROR(audiorate, STREAM, FORMAT, (NULL), ("pipeline error, format was not negotiated"));
...
{
    GST_DEBUG_CATEGORY_INIT(audio_rate_debug, "audiorate", 0, "AudioRate stream fixer");

    return gst_element_register(plugin, "audiorate", GST_RANK_NONE, GST_TYPE_AUDIO_RATE);
}

GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, "audiorate", "Adjusts audio frames", plugin_init, VERSION, GST_LICENSE, GST_PACKAGE_NAME, GST_PACKAGE_ORIGIN)
