...
static GstStateChangeReturn gst_ssa_parse_change_state(GstElement * element, GstStateChange transition);
...
static void gst_ssa_parse_base_init(gpointer klass)
{
    ...
    gst_element_class_set_details_simple(element_class, "SSA Subtitle Parser", "Codec/Parser/Subtitle", "Parses SSA subtitle streams", "Tim-Philipp Müller <tim centricular net>");

    GST_DEBUG_CATEGORY_INIT(ssa_parse_debug, "ssaparse", 0, "SSA subtitle parser");
}

static void gst_ssa_parse_dispose(GObject * object)
{
...
static void gst_ssa_parse_init(GstSsaParse * parse, GstSsaParseClass * klass)
{
    parse->sinkpad = gst_pad_new_from_static_template(&sink_templ, "sink");
    gst_pad_set_setcaps_function(parse->sinkpad, GST_DEBUG_FUNCPTR(gst_ssa_parse_setcaps));
    gst_pad_set_chain_function(parse->sinkpad, GST_DEBUG_FUNCPTR(gst_ssa_parse_chain));
    gst_pad_set_event_function(parse->sinkpad, GST_DEBUG_FUNCPTR(gst_ssa_parse_sink_event));
    gst_element_add_pad(GST_ELEMENT(parse), parse->sinkpad);

    parse->srcpad = gst_pad_new_from_static_template(&src_templ, "src");
    gst_pad_set_event_function(parse->srcpad, GST_DEBUG_FUNCPTR(gst_ssa_parse_src_event));
    gst_element_add_pad(GST_ELEMENT(parse), parse->srcpad);
    gst_pad_use_fixed_caps(parse->srcpad);
    gst_pad_set_caps(parse->srcpad, gst_static_pad_template_get_caps(&src_templ));
...
static void gst_ssa_parse_class_init(GstSsaParseClass * klass)
{
 ...
 */
static GstFlowReturn gst_ssa_parse_push_line(GstSsaParse * parse, gchar * txt, GstClockTime start, GstClockTime duration)
{
    GstFlowReturn ret;
    GstBuffer *buf;
    gchar *t, *escaped;
    ...
    num = atoi(txt);
    GST_LOG_OBJECT(parse, "Parsing line #%d at %" GST_TIME_FORMAT, num, GST_TIME_ARGS(start));
    ...
    GST_LOG_OBJECT(parse, "Pushing buffer with timestamp %" GST_TIME_FORMAT " and duration %" GST_TIME_FORMAT, GST_TIME_ARGS(start), GST_TIME_ARGS(duration));

    ret = gst_pad_push(parse->srcpad, buf);

    if (ret != GST_FLOW_OK)
    {
        GST_DEBUG_OBJECT(parse, "Push of text '%s' returned flow %s", txt, gst_flow_get_name(ret));
    }
...
static GstFlowReturn gst_ssa_parse_chain(GstPad * sinkpad, GstBuffer * buf)
{
    ...
    if (G_UNLIKELY(parse->send_tags))
    {
        ...
        gst_tag_list_add(tags, GST_TAG_MERGE_APPEND, GST_TAG_SUBTITLE_CODEC, "SubStation Alpha", NULL);
...
not_framed:
    {
        GST_ELEMENT_ERROR(parse, STREAM, FORMAT, (NULL), ("Only SSA subtitles embedded in containers are supported"));
        gst_buffer_unref(buf);
        return GST_FLOW_NOT_NEGOTIATED;
    }
empty_text:
    {
        GST_ELEMENT_WARNING(parse, STREAM, FORMAT, (NULL), ("Received empty subtitle"));
        gst_buffer_unref(buf);
        return GST_FLOW_OK;
    }
}
