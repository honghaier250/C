 ...
 */
static gchar *read_str(const gchar * line, const gchar * end)
{
    gint index = 0;
...
static gint read_int(const gchar * line)
{
    gint index = 0;
    while (line[index] != ':' && line[index] != '}')
    {
...
static gboolean string_match(const gchar * line, const gchar * match, const gchar * upto)
{
 ...
 */
static gboolean read_color(const gchar * line, gint * r, gint * g, gint * b)
{
    gint index = 0;
    while (line[index] != ':' && line[index] != '}')
    {
...
static gchar *make_color(gint r, gint g, gint b)
{
...
static gboolean qttext_parse_tag(ParserState * state, const gchar * line, gint * index)
{
    gchar *next;
    gint next_index;
    gint aux;
...
static guint64 qttext_parse_timestamp(ParserState * state, const gchar * line, gint index)
{
    int ret;
...
static void qttext_open_markup(ParserState * state)
{
    GstQTTextContext *context = GST_QTTEXT_CONTEXT(state);
    ...
    if (context->font)
        g_string_append_printf(state->buf, " font='%s %d'", context->font, context->font_size);
    else
...
static void qttext_prepare_text(ParserState * state)
{
    GstQTTextContext *context = GST_QTTEXT_CONTEXT(state);
    if (state->buf == NULL)
    {
...
static void qttext_parse_text(ParserState * state, const gchar * line, gint index)
{
    qttext_prepare_text(state);
    g_string_append(state->buf, line + index);
}

static gchar *qttext_get_text(ParserState * state)
{
    gchar *ret;
    GstQTTextContext *context = GST_QTTEXT_CONTEXT(state);
    if (state->buf == NULL)
