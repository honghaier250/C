...
static void gst_sub_parse_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_sub_parse_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
...
static GstStaticPadTemplate sink_templ = GST_STATIC_PAD_TEMPLATE("sink",
                                                                 ...
                                                                 GST_STATIC_CAPS("application/x-subtitle; application/x-subtitle-sami; "
                                                                                 "application/x-subtitle-tmplayer; application/x-subtitle-mpl2; " "application/x-subtitle-dks; application/x-subtitle-qttext"));
#else
static GstStaticPadTemplate sink_templ = GST_STATIC_PAD_TEMPLATE("sink",
                                                                 GST_PAD_SINK,
                                                                 GST_PAD_ALWAYS,
                                                                 GST_STATIC_CAPS("application/x-subtitle; application/x-subtitle-dks; " "application/x-subtitle-tmplayer; application/x-subtitle-mpl2; " "application/x-subtitle-qttext"));
#endif
...
static GstStateChangeReturn gst_sub_parse_change_state(GstElement * element, GstStateChange transition);
...
GType gst_sub_parse_get_type(void)
{
    ...
    if (!sub_parse_type)
    {
        ...
        sub_parse_type = g_type_register_static(GST_TYPE_ELEMENT, "GstSubParse", &sub_parse_info, 0);
    }
...
static void gst_sub_parse_base_init(GstSubParseClass * klass)
{
    ...
    gst_element_class_set_details_simple(element_class,
                                         "Subtitle parser", "Codec/Parser/Subtitle", "Parses subtitle (.sub) files into text streams", "Gustavo J. A. M. Carneiro <gjc@inescporto.pt>, " "GStreamer maintainers <gstreamer-devel@lists.sourceforge.net>");
}
...
static void gst_sub_parse_class_init(GstSubParseClass * klass)
{
    ...
    g_object_class_install_property(object_class, PROP_ENCODING,
                                    g_param_spec_string("subtitle-encoding", "subtitle charset encoding",
                                                        ...
                                                        "variable will be checked for an encoding to use. If that is not set " "either, ISO-8859-15 will be assumed.", DEFAULT_ENCODING, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    g_object_class_install_property(object_class, PROP_VIDEOFPS,
                                    gst_param_spec_fraction("video-fps", "Video framerate",
                                                            ...
                                                            "and the subtitle format requires it subtitles may be out of sync.", 0, 1, G_MAXINT, 1, 24000, 1001, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}

static void gst_sub_parse_init(GstSubParse * subparse)
{
    subparse->sinkpad = gst_pad_new_from_static_template(&sink_templ, "sink");
    gst_pad_set_chain_function(subparse->sinkpad, GST_DEBUG_FUNCPTR(gst_sub_parse_chain));
    gst_pad_set_event_function(subparse->sinkpad, GST_DEBUG_FUNCPTR(gst_sub_parse_sink_event));
    gst_element_add_pad(GST_ELEMENT(subparse), subparse->sinkpad);

    subparse->srcpad = gst_pad_new_from_static_template(&src_templ, "src");
    gst_pad_set_event_function(subparse->srcpad, GST_DEBUG_FUNCPTR(gst_sub_parse_src_event));
    gst_pad_set_query_function(subparse->srcpad, GST_DEBUG_FUNCPTR(gst_sub_parse_src_query));
        ...
        case GST_QUERY_POSITION:
            {
                ...
                {
                    ret = TRUE;
                    gst_query_set_position(query, GST_FORMAT_TIME, self->segment.last_stop);
                }
            }
        ...
        case GST_EVENT_SEEK:
            {
                ...
                gst_event_parse_seek(event, &rate, &format, &self->segment_flags, &start_type, &start, &stop_type, &stop);
                ...
                ret = gst_pad_push_event(self->sinkpad, gst_event_new_seek(rate, GST_FORMAT_BYTES, self->segment_flags, GST_SEEK_TYPE_SET, 0, GST_SEEK_TYPE_NONE, 0));

                if (ret)
                {
                    /* Apply the seek to our segment */
                    gst_segment_set_seek(&self->segment, rate, format, self->segment_flags, start_type, start, stop_type, stop, &update);

                    GST_DEBUG_OBJECT(self, "segment after seek: %" GST_SEGMENT_FORMAT, &self->segment);
...
static void gst_sub_parse_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    ...
    switch (prop_id)
    {
        case PROP_ENCODING:
            ...
            GST_LOG_OBJECT(object, "subtitle encoding set to %s", GST_STR_NULL(subparse->encoding));
            break;
        case PROP_VIDEOFPS:
            {
                subparse->fps_n = gst_value_get_fraction_numerator(value);
                subparse->fps_d = gst_value_get_fraction_denominator(value);
                GST_DEBUG_OBJECT(object, "video framerate set to %d/%d", subparse->fps_n, subparse->fps_d);
...
static const gchar *gst_sub_parse_get_format_description(GstSubParseFormat format)
{
    switch (format)
    {
...
static gchar *gst_convert_to_utf8(const gchar * str, gsize len, const gchar * encoding, gsize * consumed, GError ** err)
{
    gchar *ret = NULL;

    *consumed = 0;
    /* The char cast is necessary in glib < 2.24 */
    ret = g_convert_with_fallback(str, len, "UTF-8", encoding, (char *) "*", consumed, NULL, err);
    if (ret == NULL)
        return ret;

    /* + 3 to skip UTF-8 BOM if it was added */
    len = strlen(ret);
    if (len >= 3 && (guint8) ret[0] == 0xEF && (guint8) ret[1] == 0xBB && (guint8) ret[2] == 0xBF)
...
{
    if (len >= 3 && (guint8) str[0] == 0xEF && (guint8) str[1] == 0xBB && (guint8) str[2] == 0xBF)
    ...
    if (len >= 4 && (guint8) str[0] == 0x00 && (guint8) str[1] == 0x00 && (guint8) str[2] == 0xFE && (guint8) str[3] == 0xFF)
        return g_strdup("UTF-32BE");

    if (len >= 4 && (guint8) str[0] == 0xFF && (guint8) str[1] == 0xFE && (guint8) str[2] == 0x00 && (guint8) str[3] == 0x00)
        return g_strdup("UTF-32LE");

    return NULL;
}

static gchar *convert_encoding(GstSubParse * self, const gchar * str, gsize len, gsize * consumed)
{
    ...
    *consumed = 0;
    ...
    GST_LOG_OBJECT(self, "successfully converted %" G_GSIZE_FORMAT " characters from %s to UTF-8" "%s", len, encoding, (err) ? " , using ISO-8859-15 as fallback" : "");

    return ret;
}

static gchar *get_next_line(GstSubParse * self)
{
    char *line = NULL;
    const char *line_end;
    int line_len;
    ...
    self->textbuf = g_string_erase(self->textbuf, 0, line_len + (have_r ? 2 : 1));
    return line;
}

static gchar *parse_mdvdsub(ParserState * state, const gchar * line)
{
    ...
    {
        g_warning("Parse of the following line, assumed to be in microdvd .sub" " format, failed:\n%s", line);
        return NULL;
    }
    ...
    if (start_frame == 1 && end_frame == 1)
    {
        ...
        if (end != rest)
        {
            ...
            GST_INFO("framerate from file: %d/%d ('%s')", state->fps_n, state->fps_d, rest);
        }
        g_free(rest);
        return NULL;
    }

    state->start_time = gst_util_uint64_scale(start_frame, GST_SECOND * state->fps_d, state->fps_n);
    state->duration = gst_util_uint64_scale(end_frame - start_frame, GST_SECOND * state->fps_d, state->fps_n);

    /* Check our segment start/stop */
    in_seg = gst_segment_clip(state->segment, GST_FORMAT_TIME, state->start_time, state->start_time + state->duration, &clip_start, &clip_stop);
    ...
    GST_DEBUG("parse_mdvdsub returning (%f+%f): %s", state->start_time / (double) GST_SECOND, state->duration / (double) GST_SECOND, ret);
    return ret;
}

static void strip_trailing_newlines(gchar * txt)
{
    if (txt)
    {
        guint len;
...
static void subrip_unescape_formatting(gchar * txt)
{
    ...
    {
        if (g_ascii_strncasecmp(pos, "&lt;u&gt;", 9) == 0 || g_ascii_strncasecmp(pos, "&lt;i&gt;", 9) == 0 || g_ascii_strncasecmp(pos, "&lt;b&gt;", 9) == 0)
        {
            pos[0] = '<';
    ...
    for (pos = txt; pos != NULL && *pos != '\0'; ++pos)
    {
        if (g_ascii_strncasecmp(pos, "&lt;/u&gt;", 10) == 0 || g_ascii_strncasecmp(pos, "&lt;/i&gt;", 10) == 0 || g_ascii_strncasecmp(pos, "&lt;/b&gt;", 10) == 0)
        {
            pos[0] = '<';
            pos[1] = '/';
...
static gboolean subrip_remove_unhandled_tag(gchar * start, gchar * stop)
{
    gchar *tag, saved;
 ...
 * for example */
static void subrip_remove_unhandled_tags(gchar * txt)
{
    gchar *pos, *gt;
...
static void subrip_fix_up_markup(gchar ** p_txt)
{
    gchar *cur, *next_tag;
    gchar open_tags[32];
...
static gboolean parse_subrip_time(const gchar * ts_string, GstClockTime * t)
{
    gchar s[128] = { '\0', };
    gchar *end, *p;
...
static gchar *parse_subrip(ParserState * state, const gchar * line)
{
    int subnum;
    gchar *ret;
        ...
        case 1:
            {
                ...
                if ((end_time = strstr(line, " --> ")) && parse_subrip_time(line, &ts_start) && parse_subrip_time(end_time + strlen(" --> "), &ts_end) && state->start_time <= ts_end)
                {
                    state->state = 2;
...
static void unescape_newlines_br(gchar * read)
{
    gchar *write = read;
...
static gchar *parse_subviewer(ParserState * state, const gchar * line)
{
    guint h1, m1, s1, ms1;
    guint h2, m2, s2, ms2;
    gchar *ret;
    ...
    switch (state->state)
    {
        case 0:
            /* looking for start_time,end_time */
            if (sscanf(line, "%u:%u:%u.%u,%u:%u:%u.%u", &h1, &m1, &s1, &ms1, &h2, &m2, &s2, &ms2) == 8)
            {
                state->state = 1;
                state->start_time = (((guint64) h1) * 3600 + m1 * 60 + s1) * GST_SECOND + ms1 * GST_MSECOND;
                state->duration = (((guint64) h2) * 3600 + m2 * 60 + s2) * GST_SECOND + ms2 * GST_MSECOND - state->start_time;
            }
            return NULL;
        case 1:
            {
...
static gchar *parse_mpsub(ParserState * state, const gchar * line)
{
    gchar *ret;
    float t1, t2;
...
static const gchar *dks_skip_timestamp(const gchar * line)
{
    while (*line && *line != ']')
        line++;
    if (*line == ']')
...
static gchar *parse_dks(ParserState * state, const gchar * line)
{
    guint h, m, s;
        ...
        case 1:
            {
                ...
                {
                    state->state = 0;
                    state->duration = (((guint64) h) * 3600 + m * 60 + s) * GST_SECOND - state->start_time;
                }
                else
                {
...
static void parser_state_init(ParserState * state)
{
...
static void parser_state_dispose(GstSubParse * self, ParserState * state)
{
    if (state->buf)
    {
...
static gpointer gst_sub_parse_data_format_autodetect_regex_once(GstSubParseRegex regtype)
{
    ...
    switch (regtype)
    {
        case GST_SUB_PARSE_REGEX_MDVDSUB:
            result = (gpointer) g_regex_new("^\\{[0-9]+\\}\\{[0-9]+\\}", 0, 0, &gerr);
            if (result == NULL)
            {
        ...
        case GST_SUB_PARSE_REGEX_SUBRIP:
            result = (gpointer) g_regex_new("^([ 0-9]){0,3}[0-9]\\s*(\x0d)?\x0a" "[ 0-9][0-9]:[ 0-9][0-9]:[ 0-9][0-9][,.][ 0-9]{0,2}[0-9]" " +--> +([ 0-9])?[0-9]:[ 0-9][0-9]:[ 0-9][0-9][,.][ 0-9]{0,2}[0-9]", 0, 0, &gerr);
            if (result == NULL)
            {
        ...
        case GST_SUB_PARSE_REGEX_DKS:
            result = (gpointer) g_regex_new("^\\[[0-9]+:[0-9]+:[0-9]+\\].*", 0, 0, &gerr);
            if (result == NULL)
            {
...
static GstSubParseFormat gst_sub_parse_data_format_autodetect(gchar * match_str)
{
    guint n1, n2, n3;
    ...
    g_once(&mdvd_rx_once, (GThreadFunc) gst_sub_parse_data_format_autodetect_regex_once, (gpointer) GST_SUB_PARSE_REGEX_MDVDSUB);
    g_once(&subrip_rx_once, (GThreadFunc) gst_sub_parse_data_format_autodetect_regex_once, (gpointer) GST_SUB_PARSE_REGEX_SUBRIP);
    g_once(&dks_rx_once, (GThreadFunc) gst_sub_parse_data_format_autodetect_regex_once, (gpointer) GST_SUB_PARSE_REGEX_DKS);
...
#ifndef GST_DISABLE_XML
    if (strstr(match_str, "<SAMI>") != NULL || strstr(match_str, "<sami>") != NULL)
    {
    ...
    if (sscanf(match_str, "0:%02u:%02u:", &n1, &n2) == 2 ||
        sscanf(match_str, "0:%02u:%02u=", &n1, &n2) == 2 || sscanf(match_str, "00:%02u:%02u:", &n1, &n2) == 2 || sscanf(match_str, "00:%02u:%02u=", &n1, &n2) == 2 || sscanf(match_str, "00:%02u:%02u,%u=", &n1, &n2, &n3) == 3)
    {
...
static GstCaps *gst_sub_parse_format_autodetect(GstSubParse * self)
{
    gchar *data;
    ...
    switch (format)
    {
        ...
        default:
            GST_DEBUG("no subtitle format detected");
            GST_ELEMENT_ERROR(self, STREAM, WRONG_TYPE, ("The input is not a valid/supported subtitle file"), (NULL));
            return NULL;
    }
}

static void feed_textbuf(GstSubParse * self, GstBuffer * buf)
{
    gboolean discont;
    gsize consumed;
    gchar *input = NULL;
    ...
    if (GST_BUFFER_OFFSET_IS_VALID(buf) && GST_BUFFER_OFFSET(buf) != self->offset)
    {
    ...
    input = convert_encoding(self, (const gchar *) gst_adapter_peek(self->adapter, gst_adapter_available(self->adapter)), (gsize) gst_adapter_available(self->adapter), &consumed);
...
static GstFlowReturn handle_buffer(GstSubParse * self, GstBuffer * buf)
{
    ...
    {
        self->detected_encoding = detect_encoding((gchar *) GST_BUFFER_DATA(buf), GST_BUFFER_SIZE(buf));
    ...
    if (G_UNLIKELY(self->parser_type == GST_SUB_PARSE_FORMAT_UNKNOWN))
    {
        ...
        if (self->subtitle_codec != NULL)
        {
            ...
            gst_tag_list_add(tags, GST_TAG_MERGE_APPEND, GST_TAG_SUBTITLE_CODEC, self->subtitle_codec, NULL);
    ...
    while (!self->flushing && (line = get_next_line(self)))
    {
        ...
        if (subtitle)
        {
            ...
            ret = gst_pad_alloc_buffer_and_set_caps(self->srcpad, GST_BUFFER_OFFSET_NONE, subtitle_len + 1, GST_PAD_CAPS(self->srcpad), &buf);
            ...
            if (ret == GST_FLOW_OK)
            {
                ...
                GST_DEBUG_OBJECT(self, "Sending text '%s', %" GST_TIME_FORMAT " + %" GST_TIME_FORMAT, subtitle, GST_TIME_ARGS(self->state.start_time), GST_TIME_ARGS(self->state.duration));
...
static GstFlowReturn gst_sub_parse_chain(GstPad * sinkpad, GstBuffer * buf)
{
    ...
    {
        GST_LOG_OBJECT(self, "pushing newsegment event with %" GST_SEGMENT_FORMAT, &self->segment);

        gst_pad_push_event(self->srcpad, gst_event_new_new_segment(FALSE, self->segment.rate, self->segment.format, self->segment.last_stop, self->segment.stop, self->segment.time));
        ...
        case GST_EVENT_EOS:
            {
                ...
                if (self->parser_type == GST_SUB_PARSE_FORMAT_SUBRIP || self->parser_type == GST_SUB_PARSE_FORMAT_TMPLAYER || self->parser_type == GST_SUB_PARSE_FORMAT_MPL2 || self->parser_type == GST_SUB_PARSE_FORMAT_QTTEXT)
                {
        ...
        case GST_EVENT_NEWSEGMENT:
            {
                ...
                if (format == GST_FORMAT_TIME)
                {
                    gst_segment_set_newsegment(&self->segment, update, rate, format, start, stop, time);
                }
                else
                {
...
static GstStaticCaps mpl2_caps = GST_STATIC_CAPS("application/x-subtitle-mpl2");
#define SUB_CAPS (gst_static_caps_get (&sub_caps))

static GstStaticCaps tmp_caps = GST_STATIC_CAPS("application/x-subtitle-tmplayer");
...
static GstStaticCaps qttext_caps = GST_STATIC_CAPS("application/x-subtitle-qttext");
#define QTTEXT_CAPS (gst_static_caps_get (&qttext_caps))

static void gst_subparse_type_find(GstTypeFind * tf, gpointer private)
{
    GstSubParseFormat format;
    const guint8 *data;
    GstCaps *caps;
    ...
    if ((encoding = detect_encoding(str, 128)) != NULL)
    {
        ...
        {
            GST_DEBUG("Encoding '%s' detected but conversion failed: %s", encoding, err->message);
            g_error_free(err);
            g_free(encoding);
        }
        else
...
{
    static const gchar *sub_exts[] = { "srt", "sub", "mpsub", "mdvd", "smi", "txt", "dks", NULL };

    GST_DEBUG_CATEGORY_INIT(sub_parse_debug, "subparse", 0, ".sub parser");

    if (!gst_type_find_register(plugin, "subparse_typefind", GST_RANK_MARGINAL, gst_subparse_type_find, (gchar **) sub_exts, SUB_CAPS, NULL, NULL))
        return FALSE;

    if (!gst_element_register(plugin, "subparse", GST_RANK_PRIMARY, GST_TYPE_SUBPARSE) || !gst_element_register(plugin, "ssaparse", GST_RANK_PRIMARY, GST_TYPE_SSA_PARSE))
    {
        return FALSE;
    }
...
GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, "subparse", "Subtitle parsing", plugin_init, VERSION, "LGPL", GST_PACKAGE_NAME, GST_PACKAGE_ORIGIN)
