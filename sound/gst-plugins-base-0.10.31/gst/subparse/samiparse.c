...
static gchar *has_tag(GString * str, const gchar tag)
{
    return strrchr(str->str, tag);
}

static void sami_context_push_state(GstSamiContext * sctx, char state)
{
    GST_LOG("state %c", state);
    g_string_append_c(sctx->state, state);
}

static void sami_context_pop_state(GstSamiContext * sctx, char state)
{
...
static void handle_start_sync(GstSamiContext * sctx, const xmlChar ** atts)
{
    int i;
...
static void handle_start_font(GstSamiContext * sctx, const xmlChar ** atts)
{
    int i;
        ...
        for (i = 0; (atts[i] != NULL); i += 2)
        {
            ...
            if (!xmlStrncmp((const xmlChar *) "color", key, 5))
            {
                ...
                if (!(*value == '#' && len == 7))
                {
                    ...
                    if (strtol((const char *) value, &r, 16) >= 0 && ((xmlChar *) r == (value + 6) && len == 6))
                    {
                        sharp = "#";
                    }
                }
                ...
                g_string_append_printf(sctx->buf, " foreground=\"%s%s\"", sharp, value);
            }
...
static void start_sami_element(void *ctx, const xmlChar * name, const xmlChar ** atts)
{
...
static void end_sami_element(void *ctx, const xmlChar * name)
{
    GstSamiContext *sctx = (GstSamiContext *) ctx;
    ...
    else if ((!xmlStrncmp((const xmlChar *) "body", name, 4)) || (!xmlStrncmp((const xmlChar *) "sami", name, 4)))
    {
...
static void characters_sami(void *ctx, const xmlChar * ch, int len)
{
    GstSamiContext *sctx = (GstSamiContext *) ctx;
    gchar *escaped;
    gchar *tmp;
...
void sami_context_init(ParserState * state)
{
    ...
    context->htmlctxt = htmlCreatePushParserCtxt(samiSAXHandler, context, "", 0, NULL, XML_CHAR_ENCODING_UTF8);
...
static gchar *fix_invalid_entities(const gchar * line)
{
    ...
    while (cp)
    {
        ...
        cp++;
        if (g_ascii_strncasecmp(cp, "nbsp;", 5) && (!g_ascii_strncasecmp(cp, "nbsp", 4)))
        {
            /* translate "&nbsp" to "&nbsp;" */
            ...
            cp += 4;
                 ...
                 && g_ascii_strncasecmp(cp, "amp;", 4) && g_ascii_strncasecmp(cp, "apos;", 5) && g_ascii_strncasecmp(cp, "lt;", 3) && g_ascii_strncasecmp(cp, "gt;", 3) && g_ascii_strncasecmp(cp, "nbsp;", 5) && cp[0] != '#')
        {
            /* translate "&" to "&amp;" */
