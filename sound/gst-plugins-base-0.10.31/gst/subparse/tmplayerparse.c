...
static gchar *tmplayer_process_buffer(ParserState * state)
{
    gchar *ret;
...
static gchar *tmplayer_parse_line(ParserState * state, const gchar * line, guint line_num)
{
    ...
    if (sscanf(line, "%u:%02u:%02u,%u%c", &h, &m, &s, &l, &divc) == 5 && (divc == '='))
    {
    ...
    else if (sscanf(line, "%u:%02u:%02u%c", &h, &m, &s, &divc) == 4 && (divc == '=' || divc == ':'))
    {
        GST_LOG("single line format %u %u %u %u %c", h, m, s, l, divc);
        ts = GST_SECOND * ((((h * 60) + m) * 60) + s);
        text_start = strchr(line + 6, divc);
    }
    else if (line[0] == '\0' && state->buf->len > 0 && GST_CLOCK_TIME_IS_VALID(state->start_time))
    {
    ...
    if (text_start == NULL || text_start[1] == '\0' || (l == 1 && state->buf->len > 0))
    {

        if (GST_CLOCK_TIME_IS_VALID(state->start_time) && state->start_time < ts && line_num > 0)
        {
            ret = tmplayer_process_buffer(state);
