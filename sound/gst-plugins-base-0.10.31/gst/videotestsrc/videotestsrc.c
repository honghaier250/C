...
struct fourcc_list_struct *paintinfo_find_by_structure(const GstStructure * structure)
{
    ...
    if (strcmp(media_type, "video/x-raw-gray") == 0)
    {
        ...
        ret = gst_structure_get_int(structure, "bpp", &bpp) && gst_structure_get_int(structure, "depth", &depth);
    ...
    else if (strcmp(media_type, "video/x-raw-rgb") == 0)
    {
        ...
        for (i = 0; i < n_fourccs; i++)
        {
            if (fourcc_list[i].type == VTS_RGB &&
                ...
                fourcc_list[i].green_mask == green_mask && fourcc_list[i].blue_mask == blue_mask && (alpha_mask == 0 || fourcc_list[i].alpha_mask == alpha_mask) && fourcc_list[i].depth == depth && fourcc_list[i].bitspp == bpp)
            {
                return fourcc_list + i;
            }
        }
    ...
    else if (strcmp(media_type, "video/x-raw-bayer") == 0)
    {
        ...
        for (i = 0; i < n_fourccs; i++)
        {
            if (fourcc_list[i].type == VTS_BAYER && g_str_equal(format, fourcc_list[i].fourcc))
            {
                return fourcc_list + i;
            }
        }
    ...
    switch (format->type)
    {
        case VTS_RGB:
            ...
            structure = gst_structure_new("video/x-raw-rgb",
                                          ...
                                          "depth", G_TYPE_INT, format->depth, "red_mask", G_TYPE_INT, format->red_mask, "green_mask", G_TYPE_INT, format->green_mask, "blue_mask", G_TYPE_INT, format->blue_mask, NULL);
            if (format->depth == 32 && format->alpha_mask > 0)
            {
                gst_structure_set(structure, "alpha_mask", G_TYPE_INT, format->alpha_mask, NULL);
            }
            break;
        case VTS_GRAY:
            structure = gst_structure_new("video/x-raw-gray", "bpp", G_TYPE_INT, format->bitspp, "depth", G_TYPE_INT, format->bitspp, NULL);
            if (format->bitspp == 16)
        ...
        case VTS_YUV:
            {
                ...
                structure = gst_structure_new("video/x-raw-yuv", "format", GST_TYPE_FOURCC, fourcc, NULL);
                ...
                if (fourcc != GST_STR_FOURCC("Y800"))
                {
                    ...
                    if (fourcc != GST_STR_FOURCC("AYUV") && fourcc != GST_STR_FOURCC("v308") && fourcc != GST_STR_FOURCC("v410") && fourcc != GST_STR_FOURCC("Y444"))
                    {
            ...
            break;
        case VTS_BAYER:
            structure = gst_structure_new("video/x-raw-bayer", "format", G_TYPE_STRING, format->fourcc, NULL);
            break;
        default:
...
static void videotestsrc_setup_paintinfo(GstVideoTestSrc * v, paintinfo * p, int w, int h)
{
    int a, r, g, b;
...
static void videotestsrc_convert_tmpline(paintinfo * p, int j)
{
    int x = p->x_offset;
    int i;
...
#ifdef unused
static void videotestsrc_blend_color(struct vts_color_struct *dest, struct vts_color_struct *a, struct vts_color_struct *b, int x)
{
...
static void videotestsrc_blend_line(GstVideoTestSrc * v, guint8 * dest, guint8 * src, struct vts_color_struct *a, struct vts_color_struct *b, int n)
{
    int i;
    if (v->fourcc->type == VTS_RGB || v->fourcc->type == VTS_BAYER)
    {
...
void gst_video_test_src_smpte(GstVideoTestSrc * v, unsigned char *dest, int w, int h)
{
    int i;
    int y1, y2;
    int j;
    ...
    for (j = y2; j < h; j++)
    {
...

        {
            ...
            videotestsrc_blend_line(v, p->tmpline + x1 * 4, p->tmpline_u8 + x1, &p->foreground_color, &p->background_color, w - x1);
...
void gst_video_test_src_smpte75(GstVideoTestSrc * v, unsigned char *dest, int w, int h)
{
    int i;
    int j;
    paintinfo pi = { NULL, };
...
void gst_video_test_src_smpte100(GstVideoTestSrc * v, unsigned char *dest, int w, int h)
{
    int i;
    int j;
    paintinfo pi = { NULL, };
...
void gst_video_test_src_bar(GstVideoTestSrc * v, unsigned char *dest, int w, int h)
{
    int j;
    paintinfo pi = { NULL, };
    paintinfo *p = &pi;
...
void gst_video_test_src_snow(GstVideoTestSrc * v, unsigned char *dest, int w, int h)
{
    int i;
    int j;
    paintinfo pi = { NULL, };
    ...
    for (j = 0; j < h; j++)
    {
        ...
        videotestsrc_blend_line(v, p->tmpline, p->tmpline_u8, &p->foreground_color, &p->background_color, p->width);
        videotestsrc_convert_tmpline(p, j);
    }
}

static void gst_video_test_src_unicolor(GstVideoTestSrc * v, unsigned char *dest, int w, int h, int color_index)
{
    int i;
    paintinfo pi = { NULL, };
    paintinfo *p = &pi;
...
void gst_video_test_src_black(GstVideoTestSrc * v, guchar * dest, int w, int h)
{
    gst_video_test_src_unicolor(v, dest, w, h, COLOR_BLACK);
}

void gst_video_test_src_white(GstVideoTestSrc * v, guchar * dest, int w, int h)
{
    gst_video_test_src_unicolor(v, dest, w, h, COLOR_WHITE);
}

void gst_video_test_src_red(GstVideoTestSrc * v, guchar * dest, int w, int h)
{
    gst_video_test_src_unicolor(v, dest, w, h, COLOR_RED);
}

void gst_video_test_src_green(GstVideoTestSrc * v, guchar * dest, int w, int h)
{
    gst_video_test_src_unicolor(v, dest, w, h, COLOR_GREEN);
}

void gst_video_test_src_blue(GstVideoTestSrc * v, guchar * dest, int w, int h)
{
    gst_video_test_src_unicolor(v, dest, w, h, COLOR_BLUE);
}

void gst_video_test_src_blink(GstVideoTestSrc * v, unsigned char *dest, int w, int h)
{
    int i;
    paintinfo pi = { NULL, };
    paintinfo *p = &pi;
...
void gst_video_test_src_solid(GstVideoTestSrc * v, unsigned char *dest, int w, int h)
{
    int i;
    paintinfo pi = { NULL, };
    paintinfo *p = &pi;
...
void gst_video_test_src_checkers1(GstVideoTestSrc * v, guchar * dest, int w, int h)
{
    int x, y;
    paintinfo pi = { NULL, };
    paintinfo *p = &pi;
...
void gst_video_test_src_checkers2(GstVideoTestSrc * v, guchar * dest, int w, int h)
{
    int x, y;
    paintinfo pi = { NULL, };
    paintinfo *p = &pi;
...
void gst_video_test_src_checkers4(GstVideoTestSrc * v, guchar * dest, int w, int h)
{
    int x, y;
    paintinfo pi = { NULL, };
    paintinfo *p = &pi;
...
void gst_video_test_src_checkers8(GstVideoTestSrc * v, guchar * dest, int w, int h)
{
    int x, y;
    paintinfo pi = { NULL, };
    paintinfo *p = &pi;
...
void gst_video_test_src_zoneplate(GstVideoTestSrc * v, unsigned char *dest, int w, int h)
{
    int i;
    int j;
    paintinfo pi = { NULL, };
...
void gst_video_test_src_chromazoneplate(GstVideoTestSrc * v, unsigned char *dest, int w, int h)
{
    int i;
    int j;
    paintinfo pi = { NULL, };
...
void gst_video_test_src_circular(GstVideoTestSrc * v, unsigned char *dest, int w, int h)
{
    int i;
    int j;
    paintinfo pi = { NULL, };
    ...
    for (j = 0; j < h; j++)
    {
        for (i = 0; i < w; i++)
        {
            ...
            dist = sqrt((2 * i - w) * (2 * i - w) + (2 * j - h) * (2 * j - h)) / (2 * w);
            seg = floor(dist * 16);
            if (seg == 0 || seg >= 8)
            {
                p->tmpline_u8[i] = 0;
...
void gst_video_test_src_gamut(GstVideoTestSrc * v, guchar * dest, int w, int h)
{
    int x, y;
    paintinfo pi = { NULL, };
    paintinfo *p = &pi;
...
void gst_video_test_src_ball(GstVideoTestSrc * v, unsigned char *dest, int w, int h)
{
    int i;
    paintinfo pi = { NULL, };
    paintinfo *p = &pi;
    ...
    y = radius + (0.5 + 0.5 * sin(2 * M_PI * sqrt(2) * t / 200)) * (h - 2 * radius);
...
static void paint_tmpline_ARGB(paintinfo * p, int x, int w)
{
    ...
    value = (p->color->A << 0) | (p->color->R << 8) | (p->color->G << 16) | (p->color->B << 24);
#else
    value = (p->color->A << 24) | (p->color->R << 16) | (p->color->G << 8) | (p->color->B << 0);
#endif
...
#if G_BYTE_ORDER == G_LITTLE_ENDIAN
    value = (p->color->A << 0) | (p->color->Y << 8) | (p->color->U << 16) | (p->color->V << 24);
#else
    value = (p->color->A << 24) | (p->color->Y << 16) | (p->color->U << 8) | (p->color->V << 0);
#endif
...
static void convert_hline_v410(paintinfo * p, int y)
{
    ...
    for (i = 0; i < p->width; i++)
    {
        ...
        a = (TO_10(ayuv[4 * i + 2]) << 22) | (TO_10(ayuv[4 * i + 1]) << 12) | (TO_10(ayuv[4 * i + 3]) << 2);
...
#if 0
        a0 = TO_10(ayuv[4 * (i + 0) + 2]) | (TO_10(ayuv[4 * (i + 0) + 1]) << 10) | (TO_10(ayuv[4 * (i + 0) + 3]) << 20);
        a1 = TO_10(ayuv[4 * (i + 1) + 1]) | (TO_10(ayuv[4 * (i + 2) + 2]) << 10) | (TO_10(ayuv[4 * (i + 2) + 1]) << 20);
        a2 = TO_10(ayuv[4 * (i + 2) + 3]) | (TO_10(ayuv[4 * (i + 3) + 1]) << 10) | (TO_10(ayuv[4 * (i + 4) + 2]) << 20);
        a3 = TO_10(ayuv[4 * (i + 4) + 1]) | (TO_10(ayuv[4 * (i + 4) + 3]) << 10) | (TO_10(ayuv[4 * (i + 5) + 1]) << 20);
#endif
...
static void convert_hline_Y41B(paintinfo * p, int y)
{
    ...
    {
        U[i] = (ayuv[4 * (i * 4) + 2] + ayuv[4 * (i * 4 + 1) + 2] + ayuv[4 * (i * 4 + 2) + 2] + ayuv[4 * (i * 4 + 3) + 2] + 2) >> 2;
        V[i] = (ayuv[4 * (i * 4) + 3] + ayuv[4 * (i * 4 + 1) + 3] + ayuv[4 * (i * 4 + 2) + 3] + ayuv[4 * (i * 4 + 3) + 3] + 2) >> 2;
    }
}
...
static void convert_hline_RGB565(paintinfo * p, int y)
{
    ...
    for (i = 0; i < p->width; i++)
    {
        guint16 value = ((argb[4 * i + 1] & 0xf8) << 8) | ((argb[4 * i + 2] & 0xfc) << 3) | ((argb[4 * i + 3] & 0xf8) >> 3);
#if G_BYTE_ORDER == G_LITTLE_ENDIAN
...
static void convert_hline_xRGB1555(paintinfo * p, int y)
{
    ...
    for (i = 0; i < p->width; i++)
    {
        guint16 value = ((argb[4 * i + 1] & 0xf8) << 7) | ((argb[4 * i + 2] & 0xf8) << 2) | ((argb[4 * i + 3] & 0xf8) >> 3);
#if G_BYTE_ORDER == G_LITTLE_ENDIAN
        GST_WRITE_UINT16_LE(R + 2 * i, value);
#else
        GST_WRITE_UINT16_BE(R + 2 * i, value);
