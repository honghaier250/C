...
static GstStaticPadTemplate gst_adder_src_template = GST_STATIC_PAD_TEMPLATE("src",
                                                                             GST_PAD_SRC,
                                                                             GST_PAD_ALWAYS,
                                                                             GST_STATIC_CAPS(CAPS));

static GstStaticPadTemplate gst_adder_sink_template = GST_STATIC_PAD_TEMPLATE("sink%d",
                                                                              GST_PAD_SINK,
                                                                              GST_PAD_REQUEST,
                                                                              GST_STATIC_CAPS(CAPS));
...
static GstPad *gst_adder_request_new_pad(GstElement * element, GstPadTemplate * temp, const gchar * unused);
...
static GstBuffer *gst_adder_do_clip(GstCollectPads * pads, GstCollectData * data, GstBuffer * buffer, gpointer user_data);
...
static GstCaps *gst_adder_sink_getcaps(GstPad * pad)
{
    ...
    GST_LOG_OBJECT(adder, "getting caps on pad %p,%s to %" GST_PTR_FORMAT, pad, GST_PAD_NAME(pad), result);
...
static gboolean gst_adder_setcaps(GstPad * pad, GstCaps * caps)
{
    ...
    GST_LOG_OBJECT(adder, "setting caps on pad %p,%s to %" GST_PTR_FORMAT, pad, GST_PAD_NAME(pad), caps);
    ...
    if (strcmp(media_type, "audio/x-raw-int") == 0)
    {
        ...
        GST_INFO_OBJECT(pad, "parse_caps sets adder to format int, %d bit", adder->width);
        ...
        {
            case 8:
                adder->func = (adder->is_signed ? (GstAdderFunction) add_int8 : (GstAdderFunction) add_uint8);
                adder->sample_size = 1;
                break;
            case 16:
                adder->func = (adder->is_signed ? (GstAdderFunction) add_int16 : (GstAdderFunction) add_uint16);
                adder->sample_size = 2;
                break;
            case 32:
                adder->func = (adder->is_signed ? (GstAdderFunction) add_int32 : (GstAdderFunction) add_uint32);
                adder->sample_size = 4;
                break;
            default:
                goto not_supported;
    ...
    else if (strcmp(media_type, "audio/x-raw-float") == 0)
    {
        ...
        GST_INFO_OBJECT(pad, "parse_caps sets adder to format float, %d bit", adder->width);
...
static gboolean forward_event_func(GstPad * pad, GValue * ret, EventData * data)
{
 ...
 * sinkpads.
 */
static gboolean forward_event(GstAdder * adder, GstEvent * event, gboolean flush)
{
    gboolean ret;
    GstIterator *it;
    GstIteratorResult ires;
    ...
    gst_iterator_free(it);
    GST_LOG_OBJECT(adder, "Forwarded event %p (%s), ret=%d", event, GST_EVENT_TYPE_NAME(event), ret);
...
static gboolean gst_adder_src_event(GstPad * pad, GstEvent * event)
{
    ...
    switch (GST_EVENT_TYPE(event))
    {
        case GST_EVENT_SEEK:
            {
                ...
                gst_event_parse_seek(event, &adder->segment_rate, NULL, &flags, &curtype, &cur, &endtype, &end);

                if ((curtype != GST_SEEK_TYPE_NONE) && (curtype != GST_SEEK_TYPE_SET))
                {
                    result = FALSE;
                    GST_DEBUG_OBJECT(adder, "seeking failed, unhandled seek type for start: %d", curtype);
                    goto done;
                }
                if ((endtype != GST_SEEK_TYPE_NONE) && (endtype != GST_SEEK_TYPE_SET))
                {
                    result = FALSE;
                    GST_DEBUG_OBJECT(adder, "seeking failed, unhandled seek type for end: %d", endtype);
                    goto done;
                }
                ...
                GST_DEBUG_OBJECT(adder, "forwarding seek event: %" GST_PTR_FORMAT, event);
                ...
                if (g_atomic_int_compare_and_exchange(&adder->flush_stop_pending, TRUE, FALSE))
                {
        ...
        default:
            ...
            GST_DEBUG_OBJECT(adder, "forward unhandled event: %s", GST_EVENT_TYPE_NAME(event));
...
static void gst_adder_base_init(gpointer g_class)
{
    ...
    gst_element_class_set_details_simple(gstelement_class, "Adder", "Generic/Audio", "Add N audio channels together", "Thomas Vander Stichele <thomas at apestaart dot org>");
}
   ...
   */
    g_object_class_install_property(gobject_class, PROP_FILTER_CAPS,
                                    g_param_spec_boxed("caps", "Target caps",
                                                       "Set target format for mixing (NULL means ANY). " "Setting this property takes a reference to the supplied GstCaps " "object.", GST_TYPE_CAPS, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
static void gst_adder_init(GstAdder * adder, GstAdderClass * klass)
{
    ...
    gst_collect_pads_set_clip_function(adder->collect, GST_DEBUG_FUNCPTR(gst_adder_do_clip), adder);
}
...
static GstBuffer *gst_adder_do_clip(GstCollectPads * pads, GstCollectData * data, GstBuffer * buffer, gpointer user_data)
{
    ...
    buffer = gst_audio_buffer_clip(buffer, &data->segment, adder->rate, adder->bps);
...
static GstFlowReturn gst_adder_collected(GstCollectPads * pads, gpointer user_data)
{
    ...
    GST_LOG_OBJECT(adder, "starting to cycle through channels, %d bytes available (bps = %d)", outsize, adder->bps);
    ...
    for (collected = pads->data; collected; collected = next)
    {
        ...
        if (outbuf == NULL)
        {
            ...
            GST_LOG_OBJECT(adder, "channel %p: preparing output buffer of %d bytes", collect_data, outsize);
        ...
        else
        {
            if (!is_gap)
            {
                ...
                GST_LOG_OBJECT(adder, "channel %p: mixing %d bytes from data %p", collect_data, insize, indata);

                /* further buffers, need to add them */
                adder->func((gpointer) outdata, (gpointer) indata, insize / adder->sample_size);
            }
            else
            {
                /* skip gap buffer */
    ...
    if (adder->segment_pending)
    {
         ...
         */
        event = gst_event_new_new_segment_full(FALSE, adder->segment_rate, 1.0, GST_FORMAT_TIME, adder->segment_start, adder->segment_end, adder->segment_start);
        ...
        adder->offset = gst_util_uint64_scale(adder->timestamp, adder->rate, GST_SECOND);
        GST_INFO_OBJECT(adder, "seg_start %" G_GUINT64_FORMAT ", seg_end %" G_GUINT64_FORMAT, adder->segment_start, adder->segment_end);
        GST_INFO_OBJECT(adder, "timestamp %" G_GINT64_FORMAT ",new offset %" G_GINT64_FORMAT, adder->timestamp, adder->offset);

        if (event)
        {
            if (!gst_pad_push_event(adder->srcpad, event))
            {
                GST_WARNING_OBJECT(adder->srcpad, "Sending event  %p (%s) failed.", event, GST_EVENT_TYPE_NAME(event));
            }
            adder->segment_pending = FALSE;
        }
        else
        {
            GST_WARNING_OBJECT(adder->srcpad, "Creating new segment event for " "start:%" G_GINT64_FORMAT "  end:%" G_GINT64_FORMAT " failed", adder->segment_start, adder->segment_end);
        }
    }
    ...
    /* send it out */
    GST_LOG_OBJECT(adder, "pushing outbuf %p, timestamp %" GST_TIME_FORMAT " offset %" G_GINT64_FORMAT, outbuf, GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(outbuf)), GST_BUFFER_OFFSET(outbuf));
