...
static inline void data_scan_ctx_advance(GstTypeFind * tf, DataScanCtx * c, guint bytes_to_skip)
{
...
static inline gboolean data_scan_ctx_ensure_data(GstTypeFind * tf, DataScanCtx * c, gint min_len)
{
    const guint8 *data;
    guint64 len;
...
static inline gboolean data_scan_ctx_memcmp(GstTypeFind * tf, DataScanCtx * c, guint offset, const gchar * data, guint len)
{
...
/*** text/plain ***/
static gboolean xml_check_first_element(GstTypeFind * tf, const gchar * element, guint elen, gboolean strict);
...
static gboolean utf8_type_find_have_valid_utf8_at_offset(GstTypeFind * tf, guint64 offset, GstTypeFindProbability * prob)
{
    guint8 *data;
    ...
    while (probability > step && size > min_size)
    {
        ...
        if (data)
        {
            ...
            if (g_utf8_validate(start, size, (const gchar **) &end) || (end - start + 4 > size))
            {                    /* allow last char to be cut off */
                *prob = probability;
                return TRUE;
            }
            *prob = 0;
...
static void utf8_type_find(GstTypeFind * tf, gpointer unused)
{
    ...
    {
        gst_type_find_suggest(tf, MIN(start_prob, GST_TYPE_FIND_POSSIBLE), UTF8_CAPS);
        return;
    }
...
static void uri_type_find(GstTypeFind * tf, gpointer unused)
{
...
static void xml_type_find(GstTypeFind * tf, gpointer unused)
{
    if (xml_check_first_element(tf, "", 0, TRUE))
    {
...
static void sdp_type_find(GstTypeFind * tf, gpointer unused)
{
    if (sdp_check_header(tf))
...
static void smil_type_find(GstTypeFind * tf, gpointer unused)
{
...
static void html_type_find(GstTypeFind * tf, gpointer unused)
{
    gchar *d, *data;
...
static void mid_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 4);

    /* http://jedi.ks.uiuc.edu/~johns/links/music/midifile.html */
    if (data && data[0] == 'M' && data[1] == 'T' && data[2] == 'h' && data[3] == 'd')
...
static void mxmf_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = NULL;

    /* Search FileId "XMF_" 4 bytes */
    data = gst_type_find_peek(tf, 0, 4);
    if (data && data[0] == 'X' && data[1] == 'M' && data[2] == 'F' && data[3] == '_')
    {
        /* Search Format version "2.00" 4 bytes */
        data = gst_type_find_peek(tf, 4, 4);
        if (data && data[0] == '2' && data[1] == '.' && data[2] == '0' && data[3] == '0')
        {
...
static void flx_type_find(GstTypeFind * tf, gpointer unused)
{
    ...
    if (data)
    {
        ...
        if ((data[4] == 0x11 || data[4] == 0x12 || data[4] == 0x30 || data[4] == 0x44) && data[5] == 0xaf && ((data[132] == 0x00 || data[132] == 0xfa) && data[133] == 0xf1))
        {
    ...
    {
        /* check magic only */
        if ((data[4] == 0x11 || data[4] == 0x12 || data[4] == 0x30 || data[4] == 0x44) && data[5] == 0xaf)
        {
...
static void id3v2_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 10);

    if (data && memcmp(data, "ID3", 3) == 0 && data[3] != 0xFF && data[4] != 0xFF && (data[6] & 0x80) == 0 && (data[7] & 0x80) == 0 && (data[8] & 0x80) == 0 && (data[9] & 0x80) == 0)
    {
        gst_type_find_suggest(tf, GST_TYPE_FIND_MAXIMUM, ID3_CAPS);
    }
}

static void id3v1_type_find(GstTypeFind * tf, gpointer unused)
{
...
static void apetag_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data;
...
static void tta_type_find(GstTypeFind * tf, gpointer unused)
{
...
static void flac_type_find(GstTypeFind * tf, gpointer unused)
{
...
static void aac_type_find(GstTypeFind * tf, gpointer unused)
{
    ...
    while (c.offset < AAC_AMOUNT)
    {
        ...
        if (G_UNLIKELY((snc & 0xfff6) == 0xfff0))
        {
            ...
            GST_DEBUG("Found one ADTS syncpoint at offset 0x%" G_GINT64_MODIFIER "x, tracing next...", c.offset);
            ...
            if ((snc & 0xfff6) == 0xfff0)
            {
                ...
                GST_DEBUG("Found second ADTS-%d syncpoint at offset 0x%" G_GINT64_MODIFIER "x, framelen %u", mpegversion, c.offset, len);
                ...
                {
                    GST_DEBUG("Unexpected sample frequency index %d or wrong sync", sample_freq_idx);
                    goto next;
                }
                ...
                caps = gst_caps_new_simple("audio/mpeg", "framed", G_TYPE_BOOLEAN, FALSE, "mpegversion", G_TYPE_INT, mpegversion, "stream-format", G_TYPE_STRING, "adts", NULL);
                ...
                if (channel_config != 0 && channel_config <= 7)
                {
                    ...
                    gst_caps_set_simple(caps, "channels", G_TYPE_INT, channels_map[channel_config], "rate", G_TYPE_INT, rate, NULL);
                }
        ...
        else if (G_UNLIKELY(((snc & 0xffe0) == 0x56e0) || (snc == 0x4de1)))
        {
            ...
            GST_DEBUG("Found one LOAS syncword at offset 0x%" G_GINT64_MODIFIER "x, tracing next...", c.offset);

            /* check length of frame for each type of detectable LOAS streams */
            if (snc == 0x4de1)
            {
                /* EPAudioSyncStream */
                len = ((c.data[2] & 0x0f) << 9) | (c.data[3] << 1) | ((c.data[4] & 0x80) >> 7);
            ...
            {
                GST_DEBUG("Found second LOAS syncword at offset 0x%" G_GINT64_MODIFIER "x, framelen %u", c.offset, len);

                gst_type_find_suggest_simple(tf, GST_TYPE_FIND_LIKELY, "audio/mpeg", "framed", G_TYPE_BOOLEAN, FALSE, "mpegversion", G_TYPE_INT, 4, "stream-format", G_TYPE_STRING, "loas", NULL);
                break;
            }
        ...
        {
            /* ADIF header */
            gst_type_find_suggest_simple(tf, GST_TYPE_FIND_LIKELY, "audio/mpeg", "framed", G_TYPE_BOOLEAN, FALSE, "mpegversion", G_TYPE_INT, 4, "stream-format", G_TYPE_STRING, "adif", NULL);
            break;
        }
...
static const guint mp3types_bitrates[2][3][16] = { {{0, 32, 64, 96, 128, 160, 192, 224, 256, 288, 320, 352, 384, 416, 448,},
...
static inline guint mp3_type_frame_length_from_header(guint32 header, guint * put_layer, guint * put_channels, guint * put_bitrate, guint * put_samplerate, gboolean * may_be_free_format, gint possible_free_framelen)
{
    ...
    if (bitrate == 0)
    {
        ...
        {
            length += possible_free_framelen;
            bitrate = length * samplerate / ((layer == 3 && version != 3) ? 72000 : 144000);
        }
    }
    else
    {
    ...
    GST_LOG("mp3typefind: samplerate = %u - bitrate = %u - layer = %u - version = %u" " - channels = %u", samplerate, bitrate, layer, version, channels);
...
static void mp3_type_find_at_offset(GstTypeFind * tf, guint64 start_off, guint * found_layer, GstTypeFindProbability * found_prob)
{
    guint8 *data = NULL;
    ...
    while (skipped < GST_MP3_TYPEFIND_TRY_SYNC)
    {
        ...
        if (*data == 0xFF)
        {
            ...
            while (found < GST_MP3_TYPEFIND_TRY_HEADERS)
            {
                ...
                if ((gint64) (offset - skipped + 4) >= 0 && data + offset - skipped + 4 < data_end)
                {
                ...
                if (!(length = mp3_type_frame_length_from_header(head, &layer, &channels, &bitrate, &samplerate, &free, last_free_framelen)))
                {
                    if (free)
                    {
                    ...
                    GST_LOG("%d. header at offset %" G_GUINT64_FORMAT " (0x%" G_GINT64_MODIFIER "x) was not an mp3 header " "(possibly-free: %s)", found + 1, start_off + offset, start_off + offset, free ? "yes" : "no");
                    break;
                }
                if ((prev_layer && prev_layer != layer) ||
                    /* (prev_bitrate && prev_bitrate != bitrate) || <-- VBR */
                    (prev_samplerate && prev_samplerate != samplerate) || (prev_channels && prev_channels != channels))
                {
                ...
                {
                    found++;
                    GST_LOG("found %d. header at offset %" G_GUINT64_FORMAT " (0x%" G_GINT64_MODIFIER "X)", found, start_off + offset, start_off + offset);
                }
                offset += length;
            }
            g_assert(found <= GST_MP3_TYPEFIND_TRY_HEADERS);
            if (head_data == NULL && gst_type_find_peek(tf, offset + start_off - 1, 1) == NULL)
                /* Incomplete last frame - don't count it. */
                found--;
            if (found == GST_MP3_TYPEFIND_TRY_HEADERS || (found >= GST_MP3_TYPEFIND_MIN_HEADERS && head_data == NULL))
            {
                /* we can make a valid guess */
                guint probability = found * GST_TYPE_FIND_MAXIMUM * (GST_MP3_TYPEFIND_TRY_SYNC - skipped) / GST_MP3_TYPEFIND_TRY_HEADERS / GST_MP3_TYPEFIND_TRY_SYNC;

                if (!headerstart && probability > (GST_TYPE_FIND_MINIMUM + GST_MP3_WRONG_HEADER))
                ...
                GST_INFO
                    ("audio/mpeg calculated %u  =  %u  *  %u / %u  *  (%u - %"
                     G_GUINT64_FORMAT ") / %u", probability, GST_TYPE_FIND_MAXIMUM, found, GST_MP3_TYPEFIND_TRY_HEADERS, GST_MP3_TYPEFIND_TRY_SYNC, (guint64) skipped, GST_MP3_TYPEFIND_TRY_SYNC);
...
static void mp3_type_find(GstTypeFind * tf, gpointer unused)
{
    ...
    if (data && mp3_type_frame_length_from_header(GST_READ_UINT32_BE(data), &layer, NULL, NULL, NULL, NULL, 0) != 0)
    {
        if (prob == 0)
...
suggest:
    {
        ...
        gst_type_find_suggest_simple(tf, prob, "audio/mpeg", "mpegversion", G_TYPE_INT, 1, "layer", G_TYPE_INT, layer, NULL);
    }
}

/*** audio/x-musepack ***/

static GstStaticCaps musepack_caps = GST_STATIC_CAPS("audio/x-musepack, streamversion= (int) { 7, 8 }");

#define MUSEPACK_CAPS (gst_static_caps_get(&musepack_caps))
static void musepack_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 4);
    ...
    if (streamversion != -1)
    {
        gst_type_find_suggest_simple(tf, prop, "audio/x-musepack", "streamversion", G_TYPE_INT, streamversion, NULL);
    }
}
...
static void ac3_type_find(GstTypeFind * tf, gpointer unused)
{
    DataScanCtx c = { 0, NULL, 0 };
        ...
        if (c.data[0] == 0x0b && c.data[1] == 0x77)
        {
            ...
            if (bsid <= 8)
            {
                ...
                if (fscod < 3 && frmsizecod < 38)
                {
                    ...
                    GST_LOG("possible AC3 frame sync at offset %" G_GUINT64_FORMAT ", size=%u", c.offset, frame_size);
            ...
            else
            {
                ...
                frame_size = ((((c.data[2] & 0x07) << 8) + (c.data[3] & 0xff)) + 1) << 1;
                GST_LOG("possible E-AC3 frame sync at offset %" G_GUINT64_FORMAT ", size=%u", c.offset, frame_size);
                if (data_scan_ctx_ensure_data(tf, &c_next, (frame_size * 2) + 5))
                {
                    data_scan_ctx_advance(tf, &c_next, frame_size * 2);
...
static gboolean dts_parse_frame_header(DataScanCtx * c, guint * frame_size, guint * sample_rate, guint * channels)
{
...
static void dts_type_find(GstTypeFind * tf, gpointer unused)
{
    DataScanCtx c = { 0, NULL, 0 };
    ...
    while (c.offset <= DTS_MAX_FRAMESIZE)
    {
        ...
        if (G_UNLIKELY(dts_parse_frame_header(&c, &frame_size, &rate, &chans)))
        {
            ...
            if (chans > 0)
            {
                gst_type_find_suggest_simple(tf, prob, "audio/x-dts", "rate", G_TYPE_INT, rate, "channels", G_TYPE_INT, chans, NULL);
            }
            else
            {
                gst_type_find_suggest_simple(tf, prob, "audio/x-dts", "rate", G_TYPE_INT, rate, NULL);
            }
...
static GstStaticCaps wavpack_caps = GST_STATIC_CAPS("audio/x-wavpack, framed = (boolean) false");

#define WAVPACK_CAPS (gst_static_caps_get(&wavpack_caps))

static GstStaticCaps wavpack_correction_caps = GST_STATIC_CAPS("audio/x-wavpack-correction, framed = (boolean) false");

#define WAVPACK_CORRECTION_CAPS (gst_static_caps_get(&wavpack_correction_caps))

static void wavpack_type_find(GstTypeFind * tf, gpointer unused)
{
    guint64 offset;
    guint32 blocksize;
    guint8 *data;
    ...
    while (offset < 32 + blocksize)
    {
        ...
        {
            GST_LOG("chunk length too big (%u > %" G_GUINT64_FORMAT ")", sublen, blocksize - offset);
            break;
        }
        if ((data[0] & 0x20) == 0)
        {
            switch (data[0] & 0x0f)
            {
                ...
                case 0xb:        /* ID_WVC_BITSTREAM */
                    gst_type_find_suggest(tf, GST_TYPE_FIND_LIKELY, WAVPACK_CORRECTION_CAPS);
                    return;
                default:
                    break;
            }
...
static GstStaticCaps postscript_caps = GST_STATIC_CAPS("application/postscript");

#define POSTSCRIPT_CAPS (gst_static_caps_get(&postscript_caps))

static void postscript_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 3);
    if (!data)
        return;
...
static void svg_type_find(GstTypeFind * tf, gpointer unused)
{
...
static GstStaticCaps multipart_caps = GST_STATIC_CAPS("multipart/x-mixed-replace");
...
static void multipart_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data;
    guint8 *x;
...
#define MULTIPART_MAX_BOUNDARY_OFFSET 16
    ...
    for (x = data; x - data < MULTIPART_MAX_BOUNDARY_OFFSET - 2 && g_ascii_isspace(*x); x++) ;
    if (x[0] != '-' || x[1] != '-')
        return;
...
static GstStaticCaps mpeg_sys_caps = GST_STATIC_CAPS("video/mpeg, " "systemstream = (boolean) true, mpegversion = (int) [ 1, 2 ]");
...
static gboolean mpeg_sys_is_valid_pack(GstTypeFind * tf, const guint8 * data, guint len, guint * pack_size)
{
    ...
    if ((data[4] & 0xC4) == 0x44)
    {
        ...
        if ((data[6] & 0x04) != 0x04 || (data[8] & 0x04) != 0x04 || (data[9] & 0x01) != 0x01 || (data[12] & 0x03) != 0x03)
            return FALSE;
    ...
    {
        /* MPEG-1 PACK */
        if ((data[6] & 0x01) != 0x01 || (data[8] & 0x01) != 0x01 || (data[9] & 0x80) != 0x80 || (data[11] & 0x01) != 0x01)
            return FALSE;
...
static gboolean mpeg_sys_is_valid_pes(GstTypeFind * tf, guint8 * data, guint len, guint * pack_size)
{
...
static gboolean mpeg_sys_is_valid_sys(GstTypeFind * tf, guint8 * data, guint len, guint * pack_size)
{
    guint sys_hdr_len;
 ...
 */
static void mpeg_sys_type_find(GstTypeFind * tf, gpointer unused)
{
    ...
    while (data < end)
    {
        ...
        if (sync_word == 0x00000100)
        {
            ...
            if (IS_MPEG_PACK_CODE(data[0]))
            {
                ...
                if (mpegversion != 0 && mpeg_sys_is_valid_pack(tf, data - 3, end - data + 3, &pack_size))
                {
                    pack_headers++;
                }
            }
...
suggest:
    {
        ...
        gst_type_find_suggest_simple(tf, prob, "video/mpeg", "systemstream", G_TYPE_BOOLEAN, TRUE, "mpegversion", G_TYPE_INT, mpegversion, NULL);
    }
};

/*** video/mpegts Transport Stream ***/
static GstStaticCaps mpegts_caps = GST_STATIC_CAPS("video/mpegts, " "systemstream = (boolean) true, packetsize = (int) [ 188, 208 ]");
 ...
 * headers */
static gint mpeg_ts_probe_headers(GstTypeFind * tf, guint64 offset, gint packet_size)
{
...
static void mpeg_ts_type_find(GstTypeFind * tf, gpointer unused)
{
        ...
        if (IS_MPEGTS_HEADER(data))
        {
            ...
            for (p = 0; p < G_N_ELEMENTS(pack_sizes); p++)
            {
                ...
                if (found >= GST_MPEGTS_TYPEFIND_MIN_HEADERS)
                {
                    ...
                    gst_type_find_suggest_simple(tf, probability, "video/mpegts", "systemstream", G_TYPE_BOOLEAN, TRUE, "packetsize", G_TYPE_INT, pack_sizes[p], NULL);
                    return;
                }
            }
        }
 ...
 */
static gboolean mpeg_find_next_header(GstTypeFind * tf, DataScanCtx * c, guint64 max_extra_offset)
{
...
static GstStaticCaps mpeg4_video_caps = GST_STATIC_CAPS("video/mpeg, " "systemstream=(boolean)false, mpegversion=4, parsed=(boolean)false");
 ...
 */
static void mpeg4_video_type_find(GstTypeFind * tf, gpointer unused)
{
    DataScanCtx c = { 0, NULL, 0 };
    ...
    while (c.offset < GST_MPEGVID_TYPEFIND_TRY_SYNC)
    {
        ...
        if (!mpeg_find_next_header(tf, &c, GST_MPEGVID_TYPEFIND_TRY_SYNC - c.offset))
            break;
    ...
    if (num_vop_headers > 0 || seen_vol)
    {
        ...
        GST_LOG("Found %d pictures, vios: %d, vos:%d, vol:%d", num_vop_headers, seen_vios, seen_vos, seen_vol);

        if (num_vop_headers >= GST_MPEGVID_TYPEFIND_TRY_PICTURES && seen_vios_at_0 && seen_vos && seen_vol)
            probability = GST_TYPE_FIND_MAXIMUM - 1;
        else if (num_vop_headers >= GST_MPEGVID_TYPEFIND_TRY_PICTURES && seen_vios && seen_vos && seen_vol)
        ...
        else if (num_vop_headers >= GST_MPEGVID_TYPEFIND_TRY_PICTURES && seen_vos && seen_vol)
            probability = GST_TYPE_FIND_NEARLY_CERTAIN - 6;
...
static void h264_video_type_find(GstTypeFind * tf, gpointer unused)
{
    DataScanCtx c = { 0, NULL, 0 };
...
static GstStaticCaps mpeg_video_caps = GST_STATIC_CAPS("video/mpeg, " "systemstream = (boolean) false");
...
static void mpeg_video_stream_type_find(GstTypeFind * tf, gpointer unused)
{
    DataScanCtx c = { 0, NULL, 0 };
    ...
    while (c.offset < GST_MPEGVID_TYPEFIND_TRY_SYNC)
    {
        ...
        if (c.data[3] == 0x01 && num_pic_headers > found && (c.offset - last_pic_offset) >= 4 && (c.offset - last_pic_offset) <= 64)
        {
            data_scan_ctx_advance(tf, &c, 4);
            found += 1;
            continue;
...
next:
    ...
    if (found > 0 || seen_seq)
    {
        ...
        gst_type_find_suggest_simple(tf, probability, "video/mpeg", "systemstream", G_TYPE_BOOLEAN, FALSE, "mpegversion", G_TYPE_INT, 1, NULL);
    }
}
...
static void aiff_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 4);
...
static void svx_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 4);
...
static void shn_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 4);
...
static void ape_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 4);
...
static void m4a_type_find(GstTypeFind * tf, gpointer unused)
{
...
static const gchar *q3gp_type_find_get_profile(const guint8 * data)
{
...
static void q3gp_type_find(GstTypeFind * tf, gpointer unused)
{
    const gchar *profile;
    guint32 ftyp_size = 0;
    gint offset = 0;
    ...
    {
        gst_type_find_suggest_simple(tf, GST_TYPE_FIND_MAXIMUM, "application/x-3gp", "profile", G_TYPE_STRING, profile, NULL);
        return;
    }
...
static void jp2_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data;
...
static void qt_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data;
    guint tip = 0;
    guint64 offset = 0;
    ...
    while ((data = gst_type_find_peek(tf, offset, 12)) != NULL)
    {
        ...
        if (STRNCMP(&data[4], "ftypisom", 8) == 0 || STRNCMP(&data[4], "ftypavc1", 8) == 0 || STRNCMP(&data[4], "ftypmp42", 8) == 0)
        {
            tip = GST_TYPE_FIND_MAXIMUM;
            variant = "iso";
            break;
        ...
        if (STRNCMP(&data[4], "moov", 4) == 0 || STRNCMP(&data[4], "mdat", 4) == 0 || STRNCMP(&data[4], "ftyp", 4) == 0 || STRNCMP(&data[4], "free", 4) == 0 || STRNCMP(&data[4], "uuid", 4) == 0 || STRNCMP(&data[4], "skip", 4) == 0)
        {
            if (tip == 0)
            {
        ...
        else if (STRNCMP(&data[4], "pnot", 4) == 0 || STRNCMP(&data[4], "PICT", 4) == 0 || STRNCMP(&data[4], "wide", 4) == 0 || STRNCMP(&data[4], "prfl", 4) == 0)
        {
            tip = GST_TYPE_FIND_MAXIMUM;
            break;
        }
...
static void qtif_type_find(GstTypeFind * tf, gpointer unused)
{
    const guint8 *data;
...
static void mod_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data;
    ...
    if ((data = gst_type_find_peek(tf, 1080, 4)) != NULL)
    {
        ...
        if ((memcmp(data, "M.K.", 4) == 0) || (memcmp(data, "M!K!", 4) == 0) ||
            /* Star Tracker */
            (memcmp(data, "FLT", 3) == 0 && isdigit(data[3])) || (memcmp(data, "EXO", 3) == 0 && isdigit(data[3])) ||
            /* Oktalyzer (Amiga) */
            ...
            (memcmp(data + 2, "CH", 2) == 0 && isdigit(data[0]) && isdigit(data[1])) || (memcmp(data + 2, "CN", 2) == 0 && isdigit(data[0]) && isdigit(data[1])))
        {
    ...
    {
        if (g_ascii_strncasecmp((gchar *) data, "!Scream!", 8) == 0 || g_ascii_strncasecmp((gchar *) data, "BMOD2STM", 8) == 0)
        {
            guint8 *id, *stmtype;
...
static void swf_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 4);

    if (data && (data[0] == 'F' || data[0] == 'C') && data[1] == 'W' && data[2] == 'S')
    {
...
static void jpeg_type_find(GstTypeFind * tf, gpointer unused)
{
    ...
    while (data_scan_ctx_ensure_data(tf, &c, 4) && c.offset < (200 * 1024))
    {
        ...
        GST_LOG("possible JPEG marker 0x%02x (@0x%04x), segment length %u", marker, (guint) c.offset, len);
        ...
        else if (JPEG_MARKER_IS_START_OF_FRAME(marker) && len >= (2 + 8))
        {
            ...
            GST_LOG("SOF at offset %" G_GUINT64_FORMAT ", num_markers=%d, " "WxH=%dx%d", c.offset - 2, num_markers, w, h);
            ...
            gst_caps_set_simple(caps, "width", G_TYPE_INT, w, "height", G_TYPE_INT, h, NULL);
            break;
        }
        else
        {
...
static void bmp_type_find(GstTypeFind * tf, gpointer unused)
{
    DataScanCtx c = { 0, NULL, 0 };
    ...
    else if (struct_size == 40 || struct_size == 64 || struct_size == 108 || struct_size == 124 || struct_size == 0xF0)
    {
    ...
    if (w == 0 || w > 0xfffff || h == 0 || h > 0xfffff || planes != 1 || (bpp != 1 && bpp != 4 && bpp != 8 && bpp != 16 && bpp != 24 && bpp != 32))
        return;

    gst_type_find_suggest_simple(tf, GST_TYPE_FIND_MAXIMUM, "image/bmp", "width", G_TYPE_INT, w, "height", G_TYPE_INT, h, "bpp", G_TYPE_INT, bpp, NULL);
}

/*** image/tiff ***/
static GstStaticCaps tiff_caps = GST_STATIC_CAPS("image/tiff, " "endianness = (int) { BIG_ENDIAN, LITTLE_ENDIAN }");
#define TIFF_CAPS (gst_static_caps_get(&tiff_caps))
static GstStaticCaps tiff_be_caps = GST_STATIC_CAPS("image/tiff, " "endianness = (int) BIG_ENDIAN");
#define TIFF_BE_CAPS (gst_static_caps_get(&tiff_be_caps))
static GstStaticCaps tiff_le_caps = GST_STATIC_CAPS("image/tiff, " "endianness = (int) LITTLE_ENDIAN");
#define TIFF_LE_CAPS (gst_static_caps_get(&tiff_le_caps))
static void tiff_type_find(GstTypeFind * tf, gpointer ununsed)
{
...
static GstStaticCaps pnm_caps = GST_STATIC_CAPS("image/x-portable-bitmap; " "image/x-portable-graymap; image/x-portable-pixmap; " "image/x-portable-anymap");
...
static void pnm_type_find(GstTypeFind * tf, gpointer ununsed)
{
    ...
    if (c.data[0] != 'P' || c.data[1] < '1' || c.data[1] > '7' || !IS_PNM_WHITESPACE(c.data[2]) || (c.data[3] != '#' && c.data[3] < '0' && c.data[3] > '9'))
        return;
    ...
    if (c.data[1] != '7')
    {
        ...
        data_scan_ctx_advance(tf, &c, 3);
        while (c.data[0] == '#')
        {                        /* we know there's still data left */
            data_scan_ctx_advance(tf, &c, 1);
            while (c.data[0] != '\n' && c.data[0] != '\r')
            {
                if (!data_scan_ctx_ensure_data(tf, &c, 4))
        ...
        if (!data_scan_ctx_ensure_data(tf, &c, 32) && !data_scan_ctx_ensure_data(tf, &c, 4))
        {
            return;
        }
        ...
        if (sscanf(s, "%u%c%u%c", &w, &sep1, &h, &sep2) == 4 && IS_PNM_WHITESPACE(sep1) && IS_PNM_WHITESPACE(sep2) && w > 0 && w < G_MAXINT && h > 0 && h < G_MAXINT)
        {
    ...
    if (w > 0 && h > 0)
    {
        gst_type_find_suggest_simple(tf, GST_TYPE_FIND_MAXIMUM, media_type, "width", G_TYPE_INT, w, "height", G_TYPE_INT, h, NULL);
    }
    else
    {
...
static void sds_type_find(GstTypeFind * tf, gpointer ununsed)
{
    guint8 *data = gst_type_find_peek(tf, 0, 4);
...
static void ircam_type_find(GstTypeFind * tf, gpointer ununsed)
{
    guint8 *data = gst_type_find_peek(tf, 0, 4);
...
static gboolean ebml_check_header(GstTypeFind * tf, const gchar * doctype, int doctype_len)
{
...
static void matroska_type_find(GstTypeFind * tf, gpointer ununsed)
{
 ...
 */
static void mxf_type_find(GstTypeFind * tf, gpointer ununsed)
{
    static const guint8 partition_pack_key[] = { 0x06, 0x0e, 0x2b, 0x34, 0x02, 0x05, 0x01, 0x01, 0x0d, 0x01, 0x02, 0x01,
        0x01
    };
    DataScanCtx c = { 0, NULL, 0 };
    ...
    while (c.offset <= MXF_MAX_PROBE_LENGTH)
    {
        ...
        for (i = 0; i < 1024 - 16; i++)
        {
            ...
            if (G_UNLIKELY(c.data[i] == 0x06 && memcmp(c.data + i, partition_pack_key, 13) == 0))
            {
...
static GstStaticCaps dv_caps = GST_STATIC_CAPS("video/x-dv, " "systemstream = (boolean) true");
#define DV_CAPS (gst_static_caps_get(&dv_caps))
static void dv_type_find(GstTypeFind * tf, gpointer private)
{
    guint8 *data;
    ...
    if (data && (data[0] == 0x1f) && (data[1] == 0x07) && (data[2] == 0x00))
    {
        ...
        gst_type_find_suggest_simple(tf, GST_TYPE_FIND_MAXIMUM, "video/x-dv", "systemstream", G_TYPE_BOOLEAN, TRUE, "format", G_TYPE_STRING, format, NULL);
    }
}
...
static GstStaticCaps ogg_annodex_caps = GST_STATIC_CAPS("application/ogg;application/x-annodex");

#define OGGANX_CAPS (gst_static_caps_get(&ogg_annodex_caps))

static void ogganx_type_find(GstTypeFind * tf, gpointer private)
{
    guint8 *data = gst_type_find_peek(tf, 0, 4);
    ...
    if ((data != NULL) && (memcmp(data, "OggS", 4) == 0))
    {
        ...
        if (data && memcmp(data, "fishead\0", 8) == 0)
            gst_type_find_suggest(tf, GST_TYPE_FIND_MAXIMUM, gst_static_caps_get(&annodex_caps));

        gst_type_find_suggest(tf, GST_TYPE_FIND_MAXIMUM, gst_static_caps_get(&ogg_caps));
    }
}
...
static void vorbis_type_find(GstTypeFind * tf, gpointer private)
{
...
static void theora_type_find(GstTypeFind * tf, gpointer private)
{
...
/*** kate ***/
static void kate_type_find(GstTypeFind * tf, gpointer private)
{
    ...
    if (strcmp(category, "subtitles") == 0 || strcmp(category, "SUB") == 0 || strcmp(category, "spu-subtitles") == 0 || strcmp(category, "K-SPU") == 0)
    {
        gst_type_find_suggest_simple(tf, GST_TYPE_FIND_MAXIMUM, "subtitle/x-kate", NULL);
    }
    else
    {
        gst_type_find_suggest_simple(tf, GST_TYPE_FIND_MAXIMUM, "application/x-kate", NULL);
    }
}

/*** application/x-ogm-video or audio***/

static GstStaticCaps ogmvideo_caps = GST_STATIC_CAPS("application/x-ogm-video");
#define OGMVIDEO_CAPS (gst_static_caps_get(&ogmvideo_caps))
static void ogmvideo_type_find(GstTypeFind * tf, gpointer private)
{
...
static GstStaticCaps ogmaudio_caps = GST_STATIC_CAPS("application/x-ogm-audio");
#define OGMAUDIO_CAPS (gst_static_caps_get(&ogmaudio_caps))
static void ogmaudio_type_find(GstTypeFind * tf, gpointer private)
{
    guint8 *data = gst_type_find_peek(tf, 0, 9);
...
static void ogmtext_type_find(GstTypeFind * tf, gpointer private)
{
    guint8 *data = gst_type_find_peek(tf, 0, 9);
...
static void speex_type_find(GstTypeFind * tf, gpointer private)
{
...
static void celt_type_find(GstTypeFind * tf, gpointer private)
{
    guint8 *data = gst_type_find_peek(tf, 0, 8);
...
static GstStaticCaps ogg_skeleton_caps = GST_STATIC_CAPS("application/x-ogg-skeleton, parsed=(boolean)FALSE");
#define OGG_SKELETON_CAPS (gst_static_caps_get(&ogg_skeleton_caps))
static void oggskel_type_find(GstTypeFind * tf, gpointer private)
{
...
static void cmml_type_find(GstTypeFind * tf, gpointer private)
{
...
static void tar_type_find(GstTypeFind * tf, gpointer unused)
{
    ...
    if (data)
    {
        if (memcmp(data, OLDGNU_MAGIC, 8) == 0)
        {                        /* sic */
...
static void ar_type_find(GstTypeFind * tf, gpointer unused)
{
...
static void au_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 4);
...
static void nuv_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 11);

    if (data)
    {
        if (memcmp(data, "MythTVVideo", 11) == 0 || memcmp(data, "NuppelVideo", 11) == 0)
        {
...
static void paris_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 4);
...
static void ilbc_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 8);
...
static GstStaticCaps msdos_caps = GST_STATIC_CAPS("application/x-ms-dos-executable");
#define MSDOS_CAPS (gst_static_caps_get(&msdos_caps))
/* see http://www.madchat.org/vxdevl/papers/winsys/pefile/pefile.htm */
static void msdos_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 64);

    if (data && data[0] == 'M' && data[1] == 'Z' && GST_READ_UINT16_LE(data + 8) == 4)
    {
...
static void mmsh_type_find(GstTypeFind * tf, gpointer unused)
{
    ...
    if (data && data[0] == 0x24 && data[1] == 0x48 && GST_READ_UINT16_LE(data + 2) > 2 + 2 + 4 + 2 + 2 + 16 && memcmp(data + 2 + 2 + 4 + 2 + 2, asf_marker, 16) == 0)
    {
...
static void dirac_type_find(GstTypeFind * tf, gpointer unused)
{
    guint8 *data = gst_type_find_peek(tf, 0, 8);
...
static void vivo_type_find(GstTypeFind * tf, gpointer unused)
{
...
#ifdef USE_GIO
static void xdgmime_typefind(GstTypeFind * find, gpointer user_data)
{
    gchar *mimetype;
    gsize length = 16384;
    guint64 tf_length;
    ...
    if (g_str_has_prefix(mimetype, "audio/") || g_str_has_prefix(mimetype, "video/"))
    {
...
static void start_with_type_find(GstTypeFind * tf, gpointer private)
{
    GstTypeFindData *start_with = (GstTypeFindData *) private;
    guint8 *data;

    GST_LOG("trying to find mime type %s with the first %u bytes of data", gst_structure_get_name(gst_caps_get_structure(start_with->caps, 0)), start_with->size);
...
static void sw_data_destroy(GstTypeFindData * sw_data)
{
...
static void riff_type_find(GstTypeFind * tf, gpointer private)
{
...
static gboolean plugin_init(GstPlugin * plugin)
{
    ...
    static const gchar *id3_exts[] = { "mp3", "mp2", "mp1", "mpga", "ogg", "flac", "tta", NULL };
    ...
    TYPE_FIND_REGISTER_START_WITH(plugin, "video/x-ms-asf", GST_RANK_SECONDARY, asf_exts, "\060\046\262\165\216\146\317\021\246\331\000\252\000\142\316\154", 16, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER(plugin, "audio/x-musepack", GST_RANK_PRIMARY, musepack_type_find, musepack_exts, MUSEPACK_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-au", GST_RANK_MARGINAL, au_type_find, au_exts, AU_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_RIFF(plugin, "video/x-msvideo", GST_RANK_PRIMARY, avi_exts, "AVI ");
    TYPE_FIND_REGISTER_RIFF(plugin, "audio/qcelp", GST_RANK_PRIMARY, qcp_exts, "QLCM");
    TYPE_FIND_REGISTER_RIFF(plugin, "video/x-cdxa", GST_RANK_PRIMARY, cdxa_exts, "CDXA");
    TYPE_FIND_REGISTER_START_WITH(plugin, "video/x-vcd", GST_RANK_PRIMARY, cdxa_exts, "\000\377\377\377\377\377\377\377\377\377\377\000", 12, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-imelody", GST_RANK_PRIMARY, imelody_exts, "BEGIN:IMELODY", 13, GST_TYPE_FIND_MAXIMUM);
#if 0
    TYPE_FIND_REGISTER_START_WITH(plugin, "video/x-smoke", GST_RANK_PRIMARY, NULL, "\x80smoke\x00\x01\x00", 6, GST_TYPE_FIND_MAXIMUM);
#endif
    TYPE_FIND_REGISTER(plugin, "audio/midi", GST_RANK_PRIMARY, mid_type_find, mid_exts, MID_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_RIFF(plugin, "audio/riff-midi", GST_RANK_PRIMARY, mid_exts, "RMID");
    TYPE_FIND_REGISTER(plugin, "audio/mobile-xmf", GST_RANK_PRIMARY, mxmf_type_find, mxmf_exts, MXMF_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/x-fli", GST_RANK_MARGINAL, flx_type_find, flx_exts, FLX_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/x-id3v2", GST_RANK_PRIMARY + 103, id3v2_type_find, id3_exts, ID3_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/x-id3v1", GST_RANK_PRIMARY + 101, id3v1_type_find, id3_exts, ID3_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/x-apetag", GST_RANK_PRIMARY + 102, apetag_type_find, apetag_exts, APETAG_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-ttafile", GST_RANK_PRIMARY, tta_type_find, tta_exts, TTA_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-mod", GST_RANK_SECONDARY, mod_type_find, mod_exts, MOD_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/mpeg", GST_RANK_PRIMARY, mp3_type_find, mp3_exts, MP3_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-ac3", GST_RANK_PRIMARY, ac3_type_find, ac3_exts, AC3_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-dts", GST_RANK_SECONDARY, dts_type_find, dts_exts, DTS_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-gsm", GST_RANK_PRIMARY, NULL, gsm_exts, GSM_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/mpeg-sys", GST_RANK_PRIMARY, mpeg_sys_type_find, mpeg_sys_exts, MPEG_SYS_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/mpegts", GST_RANK_PRIMARY, mpeg_ts_type_find, mpeg_ts_exts, MPEGTS_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/ogg", GST_RANK_PRIMARY, ogganx_type_find, ogg_exts, OGGANX_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/mpeg-elementary", GST_RANK_MARGINAL, mpeg_video_stream_type_find, mpeg_video_exts, MPEG_VIDEO_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/mpeg4", GST_RANK_PRIMARY, mpeg4_video_type_find, m4v_exts, MPEG_VIDEO_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/x-h264", GST_RANK_PRIMARY, h264_video_type_find, h264_exts, MPEG_VIDEO_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/x-nuv", GST_RANK_SECONDARY, nuv_type_find, nuv_exts, NUV_CAPS, NULL, NULL);

    /* ISO formats */
    TYPE_FIND_REGISTER(plugin, "audio/x-m4a", GST_RANK_PRIMARY, m4a_type_find, m4a_exts, M4A_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/x-3gp", GST_RANK_PRIMARY, q3gp_type_find, q3gp_exts, Q3GP_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/quicktime", GST_RANK_SECONDARY, qt_type_find, qt_exts, QT_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "image/x-quicktime", GST_RANK_SECONDARY, qtif_type_find, qtif_exts, QTIF_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "image/jp2", GST_RANK_PRIMARY, jp2_type_find, jp2_exts, JP2_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/mj2", GST_RANK_PRIMARY, jp2_type_find, mj2_exts, MJ2_CAPS, NULL, NULL);

    TYPE_FIND_REGISTER(plugin, "text/html", GST_RANK_SECONDARY, html_type_find, html_exts, HTML_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_START_WITH(plugin, "application/vnd.rn-realmedia", GST_RANK_SECONDARY, rm_exts, ".RMF", 4, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "application/x-pn-realaudio", GST_RANK_SECONDARY, rm_exts, ".ra\375", 4, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER(plugin, "application/x-shockwave-flash", GST_RANK_SECONDARY, swf_type_find, swf_exts, SWF_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_START_WITH(plugin, "video/x-flv", GST_RANK_SECONDARY, flv_exts, "FLV", 3, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER(plugin, "text/plain", GST_RANK_MARGINAL, utf8_type_find, utf8_exts, UTF8_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "text/uri-list", GST_RANK_MARGINAL, uri_type_find, uri_exts, URI_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/sdp", GST_RANK_SECONDARY, sdp_type_find, sdp_exts, SDP_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/smil", GST_RANK_SECONDARY, smil_type_find, smil_exts, SMIL_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/xml", GST_RANK_MARGINAL, xml_type_find, xml_exts, GENERIC_XML_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_RIFF(plugin, "audio/x-wav", GST_RANK_PRIMARY, wav_exts, "WAVE");
    TYPE_FIND_REGISTER(plugin, "audio/x-aiff", GST_RANK_SECONDARY, aiff_type_find, aiff_exts, AIFF_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-svx", GST_RANK_SECONDARY, svx_type_find, svx_exts, SVX_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-paris", GST_RANK_SECONDARY, paris_type_find, paris_exts, PARIS_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-nist", GST_RANK_SECONDARY, nist_exts, "NIST", 4, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-voc", GST_RANK_SECONDARY, voc_exts, "Creative", 8, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER(plugin, "audio/x-sds", GST_RANK_SECONDARY, sds_type_find, sds_exts, SDS_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-ircam", GST_RANK_SECONDARY, ircam_type_find, ircam_exts, IRCAM_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-w64", GST_RANK_SECONDARY, w64_exts, "riff", 4, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER(plugin, "audio/x-shorten", GST_RANK_SECONDARY, shn_type_find, shn_exts, SHN_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/x-ape", GST_RANK_SECONDARY, ape_type_find, ape_exts, APE_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "image/jpeg", GST_RANK_PRIMARY + 15, jpeg_type_find, jpeg_exts, JPEG_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_START_WITH(plugin, "image/gif", GST_RANK_PRIMARY, gif_exts, "GIF8", 4, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "image/png", GST_RANK_PRIMARY + 14, png_exts, "\211PNG\015\012\032\012", 8, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER(plugin, "image/bmp", GST_RANK_PRIMARY, bmp_type_find, bmp_exts, BMP_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "image/tiff", GST_RANK_PRIMARY, tiff_type_find, tiff_exts, TIFF_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "image/x-portable-pixmap", GST_RANK_SECONDARY, pnm_type_find, pnm_exts, PNM_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/x-matroska", GST_RANK_PRIMARY, matroska_type_find, matroska_exts, MATROSKA_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/webm", GST_RANK_PRIMARY, webm_type_find, webm_exts, WEBM_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/mxf", GST_RANK_PRIMARY, mxf_type_find, mxf_exts, MXF_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_START_WITH(plugin, "video/x-mve", GST_RANK_SECONDARY, mve_exts, "Interplay MVE File\032\000\032\000\000\001\063\021", 26, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER(plugin, "video/x-dv", GST_RANK_SECONDARY, dv_type_find, dv_exts, DV_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-amr-nb-sh", GST_RANK_PRIMARY, amr_exts, "#!AMR", 5, GST_TYPE_FIND_LIKELY);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-amr-wb-sh", GST_RANK_PRIMARY, amr_exts, "#!AMR-WB", 7, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER(plugin, "audio/iLBC-sh", GST_RANK_PRIMARY, ilbc_type_find, ilbc_exts, ILBC_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-sid", GST_RANK_MARGINAL, sid_exts, "PSID", 4, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "image/x-xcf", GST_RANK_SECONDARY, xcf_exts, "gimp xcf", 8, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "video/x-mng", GST_RANK_SECONDARY, mng_exts, "\212MNG\015\012\032\012", 8, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "image/x-jng", GST_RANK_SECONDARY, jng_exts, "\213JNG\015\012\032\012", 8, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "image/x-xpixmap", GST_RANK_SECONDARY, xpm_exts, "/* XPM */", 9, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "image/x-sun-raster", GST_RANK_SECONDARY, ras_exts, "\131\246\152\225", 4, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "application/x-bzip", GST_RANK_SECONDARY, bz2_exts, "BZh", 3, GST_TYPE_FIND_LIKELY);
    TYPE_FIND_REGISTER_START_WITH(plugin, "application/x-gzip", GST_RANK_SECONDARY, gz_exts, "\037\213", 2, GST_TYPE_FIND_LIKELY);
    TYPE_FIND_REGISTER_START_WITH(plugin, "application/zip", GST_RANK_SECONDARY, zip_exts, "PK\003\004", 4, GST_TYPE_FIND_LIKELY);
    TYPE_FIND_REGISTER_START_WITH(plugin, "application/x-compress", GST_RANK_SECONDARY, compress_exts, "\037\235", 2, GST_TYPE_FIND_LIKELY);
    TYPE_FIND_REGISTER(plugin, "subtitle/x-kate", GST_RANK_MARGINAL, kate_type_find, NULL, NULL, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-flac", GST_RANK_PRIMARY, flac_type_find, flac_exts, FLAC_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-vorbis", GST_RANK_PRIMARY, vorbis_type_find, NULL, VORBIS_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/x-theora", GST_RANK_PRIMARY, theora_type_find, NULL, THEORA_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/x-ogm-video", GST_RANK_PRIMARY, ogmvideo_type_find, NULL, OGMVIDEO_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/x-ogm-audio", GST_RANK_PRIMARY, ogmaudio_type_find, NULL, OGMAUDIO_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/x-ogm-text", GST_RANK_PRIMARY, ogmtext_type_find, NULL, OGMTEXT_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-speex", GST_RANK_PRIMARY, speex_type_find, NULL, SPEEX_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-celt", GST_RANK_PRIMARY, celt_type_find, NULL, CELT_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/x-ogg-skeleton", GST_RANK_PRIMARY, oggskel_type_find, NULL, OGG_SKELETON_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "text/x-cmml", GST_RANK_PRIMARY, cmml_type_find, NULL, CMML_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_START_WITH(plugin, "application/x-executable", GST_RANK_MARGINAL, NULL, "\177ELF", 4, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER(plugin, "aac", GST_RANK_SECONDARY, aac_type_find, aac_exts, AAC_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-spc", GST_RANK_SECONDARY, spc_exts, "SNES-SPC700 Sound File Data", 27, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER(plugin, "audio/x-wavpack", GST_RANK_SECONDARY, wavpack_type_find, wavpack_exts, WAVPACK_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "audio/x-wavpack-correction", GST_RANK_SECONDARY, wavpack_type_find, wavpack_correction_exts, WAVPACK_CORRECTION_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/postscript", GST_RANK_SECONDARY, postscript_type_find, ps_exts, POSTSCRIPT_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "image/svg+xml", GST_RANK_SECONDARY, svg_type_find, svg_exts, SVG_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_START_WITH(plugin, "application/x-rar", GST_RANK_SECONDARY, rar_exts, "Rar!", 4, GST_TYPE_FIND_LIKELY);
    TYPE_FIND_REGISTER(plugin, "application/x-tar", GST_RANK_SECONDARY, tar_type_find, tar_exts, TAR_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/x-ar", GST_RANK_SECONDARY, ar_type_find, ar_exts, AR_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/x-ms-dos-executable", GST_RANK_SECONDARY, msdos_type_find, msdos_exts, MSDOS_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/x-dirac", GST_RANK_PRIMARY, dirac_type_find, NULL, DIRAC_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "multipart/x-mixed-replace", GST_RANK_SECONDARY, multipart_type_find, NULL, MULTIPART_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "application/x-mmsh", GST_RANK_SECONDARY, mmsh_type_find, NULL, MMSH_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER(plugin, "video/vivo", GST_RANK_SECONDARY, vivo_type_find, vivo_exts, VIVO_CAPS, NULL, NULL);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-nsf", GST_RANK_SECONDARY, nsf_exts, "NESM\x1a", 5, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-gym", GST_RANK_SECONDARY, gym_exts, "GYMX", 4, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-ay", GST_RANK_SECONDARY, ay_exts, "ZXAYEMUL", 8, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-gbs", GST_RANK_SECONDARY, gbs_exts, "GBS\x01", 4, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-vgm", GST_RANK_SECONDARY, vgm_exts, "Vgm\x20", 4, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-sap", GST_RANK_SECONDARY, sap_exts, "SAP\x0d\x0aAUTHOR\x20", 12, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "video/x-ivf", GST_RANK_SECONDARY, ivf_exts, "DKIF", 4, GST_TYPE_FIND_NEARLY_CERTAIN);
    TYPE_FIND_REGISTER_START_WITH(plugin, "audio/x-kss", GST_RANK_SECONDARY, kss_exts, "KSSX\0", 5, GST_TYPE_FIND_MAXIMUM);
    TYPE_FIND_REGISTER_START_WITH(plugin, "application/pdf", GST_RANK_SECONDARY, pdf_exts, "%PDF-", 5, GST_TYPE_FIND_LIKELY);
    TYPE_FIND_REGISTER_START_WITH(plugin, "application/msword", GST_RANK_SECONDARY, msword_exts, "\320\317\021\340\241\261\032\341", 8, GST_TYPE_FIND_LIKELY);
    /* Mac OS X .DS_Store files tend to be taken for video/mpeg */
    TYPE_FIND_REGISTER_START_WITH(plugin, "application/octet-stream", GST_RANK_SECONDARY, dsstore_exts, "\000\000\000\001Bud1", 8, GST_TYPE_FIND_LIKELY);
    TYPE_FIND_REGISTER_START_WITH(plugin, "image/vnd.adobe.photoshop", GST_RANK_SECONDARY, psd_exts, "8BPS\000\001\000\000\000\000", 10, GST_TYPE_FIND_LIKELY);

#ifdef USE_GIO
    TYPE_FIND_REGISTER(plugin, "xdgmime-base", GST_RANK_MARGINAL, xdgmime_typefind, NULL, NULL, NULL, NULL);
#endif

    return TRUE;
}

GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, "typefindfunctions", "default typefind functions", plugin_init, VERSION, GST_LICENSE, GST_PACKAGE_NAME, GST_PACKAGE_ORIGIN)
