...
GST_BOILERPLATE_FULL(GstVolume, gst_volume, GstAudioFilter, GST_TYPE_AUDIO_FILTER, _init_interfaces);
...
static void volume_before_transform(GstBaseTransform * base, GstBuffer * buffer);
static GstFlowReturn volume_transform_ip(GstBaseTransform * base, GstBuffer * outbuf);
static gboolean volume_stop(GstBaseTransform * base);
static gboolean volume_setup(GstAudioFilter * filter, GstRingBufferSpec * format);

static void volume_process_double(GstVolume * self, gpointer bytes, guint n_bytes);
static void volume_process_controlled_double(GstVolume * self, gpointer bytes, gdouble * volume, guint channels, guint n_bytes);
static void volume_process_float(GstVolume * self, gpointer bytes, guint n_bytes);
static void volume_process_controlled_float(GstVolume * self, gpointer bytes, gdouble * volume, guint channels, guint n_bytes);
static void volume_process_int32(GstVolume * self, gpointer bytes, guint n_bytes);
static void volume_process_int32_clamp(GstVolume * self, gpointer bytes, guint n_bytes);
static void volume_process_controlled_int32_clamp(GstVolume * self, gpointer bytes, gdouble * volume, guint channels, guint n_bytes);
static void volume_process_int24(GstVolume * self, gpointer bytes, guint n_bytes);
static void volume_process_int24_clamp(GstVolume * self, gpointer bytes, guint n_bytes);
static void volume_process_controlled_int24_clamp(GstVolume * self, gpointer bytes, gdouble * volume, guint channels, guint n_bytes);
static void volume_process_int16(GstVolume * self, gpointer bytes, guint n_bytes);
static void volume_process_int16_clamp(GstVolume * self, gpointer bytes, guint n_bytes);
static void volume_process_controlled_int16_clamp(GstVolume * self, gpointer bytes, gdouble * volume, guint channels, guint n_bytes);
static void volume_process_int8(GstVolume * self, gpointer bytes, guint n_bytes);
static void volume_process_int8_clamp(GstVolume * self, gpointer bytes, guint n_bytes);
static void volume_process_controlled_int8_clamp(GstVolume * self, gpointer bytes, gdouble * volume, guint channels, guint n_bytes);

/* helper functions */

static gboolean volume_choose_func(GstVolume * self)
{
    self->process = NULL;
...
static gboolean volume_update_volume(GstVolume * self, gfloat volume, gboolean mute)
{
...
static gboolean gst_volume_interface_supported(GstImplementsInterface * iface, GType type)
{
...
static const GList *gst_volume_list_tracks(GstMixer * mixer)
{
...
static void gst_volume_set_volume(GstMixer * mixer, GstMixerTrack * track, gint * volumes)
{
    GstVolume *self = GST_VOLUME(mixer);
...
static void gst_volume_get_volume(GstMixer * mixer, GstMixerTrack * track, gint * volumes)
{
    GstVolume *self = GST_VOLUME(mixer);
...
static void gst_volume_set_mute(GstMixer * mixer, GstMixerTrack * track, gboolean mute)
{
    GstVolume *self = GST_VOLUME(mixer);
...
static void gst_volume_mixer_init(GstMixerClass * klass)
{
...
static void gst_volume_dispose(GObject * object)
{
...
static void gst_volume_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);
    GstAudioFilterClass *filter_class = GST_AUDIO_FILTER_CLASS(g_class);
    GstCaps *caps;

    gst_element_class_set_details_simple(element_class, "Volume", "Filter/Effect/Audio", "Set volume on audio/raw streams", "Andy Wingo <wingo@pobox.com>");
...
static void gst_volume_class_init(GstVolumeClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_MUTE, g_param_spec_boolean("mute", "Mute", "mute channel", DEFAULT_PROP_MUTE, G_PARAM_READWRITE | GST_PARAM_CONTROLLABLE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_VOLUME, g_param_spec_double("volume", "Volume", "volume factor, 1.0=100%", 0.0, VOLUME_MAX_DOUBLE, DEFAULT_PROP_VOLUME, G_PARAM_READWRITE | GST_PARAM_CONTROLLABLE | G_PARAM_STATIC_STRINGS));
...
static void gst_volume_init(GstVolume * self, GstVolumeClass * g_class)
{
...
static void volume_process_int24(GstVolume * self, gpointer bytes, guint n_bytes)
{
    ...
    for (i = 0; i < num_samples; i++)
    {
        ...
        val = (gint32) samp;
        val = (((gint64) self->current_vol_i24 * val) >> VOLUME_UNITY_INT24_BIT_SHIFT);
        samp = (guint32) val;
...
static gboolean volume_setup(GstAudioFilter * filter, GstRingBufferSpec * format)
{
    ...
    if (!res)
    {
        GST_ELEMENT_ERROR(self, CORE, NEGOTIATION, ("Invalid incoming format"), (NULL));
    }
    self->negotiated = res;
...
static gboolean volume_stop(GstBaseTransform * base)
{
    ...
    return GST_CALL_PARENT_WITH_DEFAULT(GST_BASE_TRANSFORM_CLASS, stop, (base), TRUE);
}
...
static void volume_before_transform(GstBaseTransform * base, GstBuffer * buffer)
{
    ...
    timestamp = gst_segment_to_stream_time(&base->segment, GST_FORMAT_TIME, timestamp);

    GST_DEBUG_OBJECT(base, "sync to %" GST_TIME_FORMAT, GST_TIME_ARGS(timestamp));
...
static GstFlowReturn volume_transform_ip(GstBaseTransform * base, GstBuffer * outbuf)
{
    ...
    if (gst_base_transform_is_passthrough(base) || GST_BUFFER_FLAG_IS_SET(outbuf, GST_BUFFER_FLAG_GAP))
        return GST_FLOW_OK;
    ...
    if (mute_csource || (volume_csource && !self->current_mute))
    {
        ...
        if (volume_csource)
        {
            GstValueArray va = { "volume", nsamples, interval, (gpointer) self->volumes };
...
not_negotiated:
    {
        GST_ELEMENT_ERROR(self, CORE, NEGOTIATION, ("No format was negotiated"), (NULL));
...
controller_failure:
    {
        ...
        GST_ELEMENT_ERROR(self, CORE, FAILED, ("Failed to get values from controller"), (NULL));
...
GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, "volume", "plugin for controlling audio volume", plugin_init, VERSION, GST_LICENSE, GST_PACKAGE_NAME, GST_PACKAGE_ORIGIN);
