...
static GstElement *try_to_link_1(GstDecodeBin * decode_bin, GstElement * origelement, GstPad * pad, GList * factories);
static void close_link(GstElement * element, GstDecodeBin * decode_bin);
static void close_pad_link(GstElement * element, GstPad * pad, GstCaps * caps, GstDecodeBin * decode_bin, gboolean more);
static void unlinked(GstPad * pad, GstPad * peerpad, GstDecodeBin * decode_bin);
...
static GType gst_decode_bin_get_type(void)
{
    ...
    if (!gst_decode_bin_type)
    {
        ...
        gst_decode_bin_type = g_type_register_static(GST_TYPE_BIN, "GstDecodeBin", &gst_decode_bin_info, 0);
    }
...
static void gst_decode_bin_class_init(GstDecodeBinClass * klass)
{
   ...
   */
    ...
    gst_element_class_set_details_simple(gstelement_klass, "Decoder Bin", "Generic/Bin/Decoder", "Autoplug and decode to raw media", "Wim Taymans <wim.taymans@gmail.com>");
 ...
 */
static gboolean gst_decode_bin_is_dynamic(GstDecodeBin * decode_bin)
{
 ...
 * autoplugging */
static gboolean gst_decode_bin_factory_filter(GstPluginFeature * feature, GstDecodeBin * decode_bin)
{
    guint rank;
    const gchar *klass;
    ...
    if (strstr(klass, "Demux") == NULL && strstr(klass, "Decoder") == NULL && strstr(klass, "Parse") == NULL && strstr(klass, "Depayloader") == NULL)
    {
        return FALSE;
    }
...
static void print_feature(GstPluginFeature * feature)
{
    const gchar *rname;
...
static void gst_decode_bin_init(GstDecodeBin * decode_bin)
{
    ...
    factories = gst_default_registry_feature_filter((GstPluginFeatureFilter) gst_decode_bin_factory_filter, FALSE, decode_bin);
    ...
    else
    {
         ...
         * a type */
        decode_bin->have_type_id = g_signal_connect(G_OBJECT(decode_bin->typefind), "have_type", G_CALLBACK(type_found), decode_bin);
    }
...
static gint find_dynamic(GstDynamic * dyn, struct DynFind *info)
{
...
static void dynamic_add(GstElement * element, GstPad * pad, GstDecodeBin * decode_bin)
{
    GstDynamic *dyn;
    ...
    find_info.pad = pad;
    found = g_list_find_custom(decode_bin->dynamics, &find_info, (GCompareFunc) find_dynamic);
    if (found != NULL)
        goto exit;
    ...
    if (pad)
    {
        ...
        GST_DEBUG_OBJECT(decode_bin, "dynamic create for pad %" GST_PTR_FORMAT, pad);
        dyn->caps_sig_id = g_signal_connect(G_OBJECT(pad), "notify::caps", G_CALLBACK(new_caps), dyn);
    }
    else
    {
        GST_DEBUG_OBJECT(decode_bin, "dynamic create for element %" GST_PTR_FORMAT, element);
        dyn->np_sig_id = g_signal_connect(G_OBJECT(element), "pad-added", G_CALLBACK(new_pad), dyn);
        dyn->nmp_sig_id = g_signal_connect(G_OBJECT(element), "no-more-pads", G_CALLBACK(no_more_pads), dyn);
    }
...
exit:
    if (element)
    {
        GST_DEBUG_OBJECT(decode_bin, "Dynamic element already added: %" GST_PTR_FORMAT, element);
    }
    else
    {
        GST_DEBUG_OBJECT(decode_bin, "Dynamic pad already added: %" GST_PTR_FORMAT, pad);
    }
}
 ...
 */
static GList *find_compatibles(GstDecodeBin * decode_bin, const GstCaps * caps)
{
    GList *factories;
    GList *to_try = NULL;

    /* loop over all the factories */
    for (factories = decode_bin->factories; factories; factories = g_list_next(factories))
    {
...
static gboolean mimetype_is_raw(const gchar * mimetype)
{
    return g_str_has_prefix(mimetype, "video/x-raw") || g_str_has_prefix(mimetype, "audio/x-raw") || g_str_has_prefix(mimetype, "text/plain") || g_str_has_prefix(mimetype, "text/x-pango-markup");
}

static void free_pad_probes(GstDecodeBin * decode_bin)
{
    GList *tmp;
 ...
 * to activate */
static void free_pad_probe_for_element(GstDecodeBin * decode_bin, GstElement * element)
{
    GList *l;
...
static gboolean pad_probe(GstPad * pad, GstMiniObject * data, GstDecodeBin * decode_bin)
{
    GList *tmp;
    ...
    for (tmp = decode_bin->probes; tmp; tmp = g_list_next(tmp))
    {
        ...
        if (pdata->pad == pad)
        {
            ...
            else if (GST_IS_EVENT(data) && ((GST_EVENT_TYPE(data) == GST_EVENT_EOS) || (GST_EVENT_TYPE(data) == GST_EVENT_TAG) || (GST_EVENT_TYPE(data) == GST_EVENT_FLUSH_START)))
            {
        ...
        if (!(pdata->done))
        {
            GST_LOG_OBJECT(decode_bin, "Pad probe on pad %" GST_PTR_FORMAT " but pad %" GST_PTR_FORMAT " still needs data.", pad, pdata->pad);
            alldone = FALSE;
        }
    }
    if (alldone)
...
static GstPad *add_raw_queue(GstDecodeBin * decode_bin, GstPad * pad)
{
    ...
    {
        GST_WARNING_OBJECT(decode_bin, "Linking queue failed, trying without queue");
    ...
    g_signal_connect(G_OBJECT(queue), "overrun", G_CALLBACK(queue_filled_cb), decode_bin);
    g_signal_connect(G_OBJECT(queue), "underrun", G_CALLBACK(queue_underrun_cb), decode_bin);
...
static void close_pad_link(GstElement * element, GstPad * pad, GstCaps * caps, GstDecodeBin * decode_bin, gboolean more)
{
    ...
    if (mimetype_is_raw(mimetype))
    {
        ...
        {
            GST_DEBUG_OBJECT(decode_bin, "Element %s is a demuxer, inserting a queue", GST_OBJECT_NAME(element));
        ...
        data->sigid = gst_pad_add_data_probe(pad, G_CALLBACK(pad_probe), decode_bin);
        decode_bin->numwaiting++;
        ...
        g_signal_emit(G_OBJECT(decode_bin), gst_decode_bin_signals[SIGNAL_NEW_DECODED_PAD], 0, ghost, !more);
...
unknown_type:
    {
        ...
        g_signal_emit(G_OBJECT(decode_bin), gst_decode_bin_signals[SIGNAL_UNKNOWN_TYPE], 0, pad, caps);

        gst_element_post_message(GST_ELEMENT_CAST(decode_bin), gst_missing_decoder_message_new(GST_ELEMENT_CAST(decode_bin), caps));
        ...
        if (element == decode_bin->typefind)
        {
            ...
            GST_ELEMENT_ERROR(decode_bin, STREAM, CODEC_NOT_FOUND, (_("A %s plugin is required to play this stream, but not installed."), desc), ("No decoder to handle media type '%s'", gst_structure_get_name(gst_caps_get_structure(caps, 0))));
            g_free(desc);
        }
 ...
 * pad.
 */
static GstElement *try_to_link_1(GstDecodeBin * decode_bin, GstElement * srcelement, GstPad * pad, GList * factories)
{
    GList *walk;
    ...
    if (isdemux && factories != NULL)
    {
        ...
        GST_DEBUG_OBJECT(decode_bin, "Element %s is a demuxer, inserting a queue", GST_OBJECT_NAME(srcelement));
        queue = gst_element_factory_make("queue", NULL);
        decode_bin->queue_type = G_OBJECT_TYPE(queue);
    ...
    for (walk = factories; walk; walk = g_list_next(walk))
    {
        ...
        GST_DEBUG_OBJECT(decode_bin, "trying to link %s", gst_plugin_feature_get_name(GST_PLUGIN_FEATURE(factory)));

        /* make an element from the factory first */
        if ((element = gst_element_factory_create(factory, NULL)) == NULL)
        {
            /* hmm, strange. Like with all things in life, let's move on.. */
            GST_WARNING_OBJECT(decode_bin, "could not create an element from %s", gst_plugin_feature_get_name(GST_PLUGIN_FEATURE(factory)));
            continue;
        }
        ...
        {
            GST_WARNING_OBJECT(decode_bin, "Couldn't set %s to READY", GST_ELEMENT_NAME(element));
        ...
        {
            GST_DEBUG_OBJECT(decode_bin, "link failed on pad %s:%s, reason %d", GST_DEBUG_PAD_NAME(pad), ret);
            /* get rid of the sinkpad */
            gst_object_unref(sinkpad);
            /* this element did not work, remove it again and continue trying
             * other elements, the element will be disposed. */
        ...
        {
            GST_DEBUG_OBJECT(decode_bin, "linked on pad %s:%s", GST_DEBUG_PAD_NAME(usedsrcpad));
            ...
            g_signal_connect(G_OBJECT(pad), "unlinked", G_CALLBACK(unlinked), decode_bin);
            ...
            {
                GST_WARNING_OBJECT(decode_bin, "Couldn't set %s to PAUSED", GST_ELEMENT_NAME(element));
...
static GstPad *get_our_ghost_pad(GstDecodeBin * decode_bin, GstPad * pad)
{
    ...
    while (!done)
    {
        ...
        switch (gst_iterator_next(pad_it, (gpointer) & db_pad))
        {
            case GST_ITERATOR_OK:
                GST_DEBUG_OBJECT(decode_bin, "looking at pad %s:%s", GST_DEBUG_PAD_NAME(db_pad));
                if (GST_IS_GHOST_PAD(db_pad))
                {
                    ...
                    {
                        /* Found our ghost pad */
                        GST_DEBUG_OBJECT(decode_bin, "found ghostpad %s:%s for pad %s:%s", GST_DEBUG_PAD_NAME(db_pad), GST_DEBUG_PAD_NAME(pad));
                        break;
                    }
                }
                /* Not the right one */
 ...
 * decoded stream.
 */
static void remove_element_chain(GstDecodeBin * decode_bin, GstPad * pad)
{
    GstIterator *iter;
    gboolean done = FALSE;
    gpointer item;
    ...
    while (GST_OBJECT_PARENT(elem) && GST_OBJECT_PARENT(elem) != GST_OBJECT(decode_bin))
        elem = GST_ELEMENT(GST_OBJECT_PARENT(elem));

    if (G_OBJECT_TYPE(elem) == decode_bin->queue_type)
    {
        GST_DEBUG_OBJECT(decode_bin, "Encountered demuxer output queue while removing element chain");
            ...
            case GST_ITERATOR_OK:
                {
                    ...
                    pad = GST_PAD(item);
                    GST_DEBUG_OBJECT(decode_bin, "inspecting internal pad %s:%s", GST_DEBUG_PAD_NAME(pad));

                    ghostpad = get_our_ghost_pad(decode_bin, pad);
                    if (ghostpad)
                    {
                        GST_DEBUG_OBJECT(decode_bin, "found our ghost pad %s:%s for %s:%s", GST_DEBUG_PAD_NAME(ghostpad), GST_DEBUG_PAD_NAME(pad));

                        g_signal_emit(G_OBJECT(decode_bin), gst_decode_bin_signals[SIGNAL_REMOVED_DECODED_PAD], 0, ghostpad);
                    ...
                    if (peer)
                    {
                        ...
                        GST_DEBUG_OBJECT(decode_bin, "internal pad %s:%s linked to pad %s:%s", GST_DEBUG_PAD_NAME(pad), GST_DEBUG_PAD_NAME(peer));
                        ...
                        if (parent)
                        {
                            ...
                            if (grandparent != NULL)
                            {
                                ...
                                {
                                    GST_DEBUG_OBJECT(decode_bin, "dead end pad %s:%s parent %s", GST_DEBUG_PAD_NAME(peer), GST_OBJECT_NAME(grandparent));
                                }
                                else
                                {
                                    GST_DEBUG_OBJECT(decode_bin, "recursing element %s on pad %s:%s", GST_ELEMENT_NAME(elem), GST_DEBUG_PAD_NAME(pad));
 ...
 */
static guint queue_enlarge(GstElement * queue, guint bytes, GstDecodeBin * decode_bin)
{
    ...
    GST_DEBUG_OBJECT(decode_bin, "increasing queue %s max-size-bytes to %d", GST_ELEMENT_NAME(queue), bytes);
...
static void queue_filled_cb(GstElement * queue, GstDecodeBin * decode_bin)
{
    ...
    for (tmp = decode_bin->queues; tmp; tmp = g_list_next(tmp))
    {
        ...
        if (aqueue != queue)
        {
            g_object_get(G_OBJECT(aqueue), "current-level-bytes", &levelbytes, NULL);
            if (levelbytes == 0)
            {
    ...
    {
        GST_DEBUG_OBJECT(decode_bin, "Queue is full but other queues are not empty, not doing anything");
    }
    return;
...
too_large:
    {
        GST_WARNING_OBJECT(decode_bin, "Queue is bigger than 20Mbytes, something else is going wrong");
        return;
    }
}
...
static void dynamic_remove(GstDynamic * dynamic)
{
    GstDecodeBin *decode_bin = dynamic->decode_bin;
    ...
    if (decode_bin->dynamics == NULL)
    {
        ...
        {
            GST_DEBUG_OBJECT(decode_bin, "no more dynamic elements, removing fakesink");
            remove_fakesink(decode_bin);
        }
        GST_DEBUG_OBJECT(decode_bin, "no more dynamic elements, signaling no_more_pads");
...
static void no_more_pads(GstElement * element, GstDynamic * dynamic)
{
    GST_DEBUG_OBJECT(dynamic->decode_bin, "no more pads on element %s", GST_ELEMENT_NAME(element));
...
static gboolean is_our_kid(GstElement * e, GstDecodeBin * decode_bin)
{
    gboolean ret;
    GstElement *parent;
...
static gboolean elem_is_dynamic(GstElement * element, GstDecodeBin * decode_bin)
{
    GList *pads;
            ...
            case GST_PAD_SOMETIMES:
                {
                    ...
                    if (pad)
                    {
                        GST_DEBUG_OBJECT(decode_bin, "got the pad for sometimes template %s", templ_name);
                        gst_object_unref(pad);
                    }
                    else
                    {
                        GST_DEBUG_OBJECT(decode_bin, "did not get the sometimes pad of template %s", templ_name);
...
static void unlinked(GstPad * pad, GstPad * peerpad, GstDecodeBin * decode_bin)
{
    ...
    GST_DEBUG_OBJECT(decode_bin, "pad %s:%s removal while alive - chained?", GST_DEBUG_PAD_NAME(pad));
...
static void close_link(GstElement * element, GstDecodeBin * decode_bin)
{
    ...
    GST_DEBUG_OBJECT(decode_bin, "closing links with element %s", GST_ELEMENT_NAME(element));
    ...
    for (pads = GST_ELEMENT_GET_CLASS(element)->padtemplates; pads; pads = g_list_next(pads))
    {
        ...
        switch (GST_PAD_TEMPLATE_PRESENCE(templ))
        {
            case GST_PAD_ALWAYS:
                {
                    ...
                    if (pad)
                    {
                        GST_DEBUG_OBJECT(decode_bin, "got the pad for always template %s", templ_name);
                    ...
                    else
                    {
                        ...
                        GST_WARNING_OBJECT(decode_bin, "could not get the pad for always template %s", templ_name);
                    }
                    break;
                }
            case GST_PAD_SOMETIMES:
            ...
            case GST_PAD_REQUEST:
                /* ignore request pads */
                GST_DEBUG_OBJECT(decode_bin, "ignoring request padtemplate %s", templ_name);
                break;
        }
    }
    if (dynamic)
    ...
    for (pads = to_connect; pads; pads = g_list_next(pads))
    {
        ...
        GST_DEBUG_OBJECT(decode_bin, "closing pad link for %s", GST_OBJECT_NAME(pad));
...
static void type_found(GstElement * typefind, guint probability, GstCaps * caps, GstDecodeBin * decode_bin)
{
    ...
    if (typefind == decode_bin->typefind && gst_structure_has_name(gst_caps_get_structure(caps, 0), "text/plain"))
    {
        gst_element_no_more_pads(GST_ELEMENT(decode_bin));
        /* we can't handle this type of stream */
        GST_ELEMENT_ERROR(decode_bin, STREAM, WRONG_TYPE, (_("This appears to be a text file")), ("decodebin cannot decode plain text files"));
        goto exit;
    }
...
static void disconnect_unlinked_signals(GstDecodeBin * decode_bin, GstElement * element)
{
    GstIterator *pad_it = NULL;
    gboolean done = FALSE;
    ...
    while (!done)
    {
        ...
        {
            case GST_ITERATOR_OK:
                g_signal_handlers_disconnect_by_func(pad, (gpointer) unlinked, decode_bin);
                gst_object_unref(pad);
                break;
            case GST_ITERATOR_RESYNC:
                gst_iterator_resync(pad_it);
...
static void cleanup_decodebin(GstDecodeBin * decode_bin)
{
    ...
    {
        g_signal_handlers_block_by_func(typefind_pad, (gpointer) unlinked, decode_bin);
    }
    ...
    while (!done)
    {
        ...
        {
            case GST_ITERATOR_OK:
                GST_DEBUG_OBJECT(pad, "inspecting pad %s:%s", GST_DEBUG_PAD_NAME(pad));
    ...
    if (GST_IS_PAD(typefind_pad))
    {
        g_signal_handlers_unblock_by_func(typefind_pad, (gpointer) unlinked, decode_bin);
        g_signal_handlers_disconnect_by_func(typefind_pad, (gpointer) unlinked, decode_bin);
...
missing_fakesink:
    {
        gst_element_post_message(element, gst_missing_element_message_new(element, "fakesink"));
...
static gboolean plugin_init(GstPlugin * plugin)
{
    ...
    return gst_element_register(plugin, "decodebin", GST_RANK_NONE, GST_TYPE_DECODE_BIN);
}

GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, "decodebin", "decoder bin", plugin_init, VERSION, GST_LICENSE, GST_PACKAGE_NAME, GST_PACKAGE_ORIGIN)
