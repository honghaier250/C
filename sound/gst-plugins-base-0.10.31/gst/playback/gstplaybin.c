...
static gboolean setup_sinks(GstPlayBaseBin * play_base_bin, GstPlayBaseGroup * group);
static void remove_sinks(GstPlayBin * play_bin);
static void playbin_set_subtitles_visible(GstPlayBaseBin * play_base_bin, gboolean visible);
static void playbin_set_audio_mute(GstPlayBaseBin * play_base_bin, gboolean mute);
...
static gboolean gst_play_bin_send_event(GstElement * element, GstEvent * event);
...
static GType gst_play_bin_get_type(void)
{
    ...
    if (!gst_play_bin_type)
    {
        ...
        gst_play_bin_type = g_type_register_static(GST_TYPE_PLAY_BASE_BIN, "GstPlayBin", &gst_play_bin_info, 0);
    }
...
static void gst_play_bin_class_init(GstPlayBinClass * klass)
{
    ...
    g_object_class_install_property(gobject_klass, ARG_VIDEO_SINK, g_param_spec_object("video-sink", "Video Sink", "the video output element to use (NULL = default sink)", GST_TYPE_ELEMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_klass, ARG_AUDIO_SINK, g_param_spec_object("audio-sink", "Audio Sink", "the audio output element to use (NULL = default sink)", GST_TYPE_ELEMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_klass, ARG_VIS_PLUGIN, g_param_spec_object("vis-plugin", "Vis plugin", "the visualization element to use (NULL = none)", GST_TYPE_ELEMENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   * GstPlayBin:volume:
   *
   ...
   *
   */
    g_object_class_install_property(gobject_klass, ARG_VOLUME, g_param_spec_double("volume", "volume", "volume", 0.0, VOLUME_MAX_DOUBLE, 1.0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_klass, ARG_FRAME, gst_param_spec_mini_object("frame", "Frame", "The last frame (NULL = no video available)", GST_TYPE_BUFFER, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    ...
    gst_element_class_set_details_simple(gstelement_klass, "Player Bin", "Generic/Bin/Player", "Autoplug and play media from an uri", "Wim Taymans <wim.taymans@gmail.com>");
...
static void gst_play_bin_init(GstPlayBin * play_bin)
{
    ...
    play_bin->cache = g_hash_table_new_full(g_str_hash, g_str_equal, NULL, (GDestroyNotify) gst_object_unref);
}
...
static void gst_play_bin_vis_unblocked(GstPad * tee_pad, gboolean blocked, gpointer user_data)
{
    GstPlayBin *play_bin = GST_PLAY_BIN(user_data);

    if (play_bin->pending_visualisation)
        gst_pad_set_blocked_async(tee_pad, FALSE, gst_play_bin_vis_unblocked, play_bin);
}

static void gst_play_bin_vis_blocked(GstPad * tee_pad, gboolean blocked, gpointer user_data)
{
    GstPlayBin *play_bin = GST_PLAY_BIN(user_data);
    GstBin *vis_bin = NULL;
    ...
    vis_bin = GST_BIN_CAST(gst_object_get_parent(GST_OBJECT_CAST(play_bin->visualisation)));
...
static void gst_play_bin_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    ...
    switch (prop_id)
    {
        ...
        case ARG_VIS_PLUGIN:
            {
                GstElement *pending_visualisation = GST_ELEMENT_CAST(g_value_get_object(value));
                ...
                else
                {
                    ...
                    if (play_bin->visualisation != NULL)
                    {
                        ...
                        if (GST_IS_BIN(vis_bin))
                        {
                            ...
                            vis_sink_pad = gst_element_get_static_pad(play_bin->visualisation, "sink");
                            if (!GST_IS_PAD(vis_sink_pad))
                            {
                                goto beach;
                            }
                            ...
                            play_bin->pending_visualisation = pending_visualisation;
                            /* Block with callback */
                            gst_pad_set_blocked_async(tee_pad, TRUE, gst_play_bin_vis_blocked, play_bin);
beach:
                            if (vis_sink_pad)
                            {
                                gst_object_unref(vis_sink_pad);
        ...
        case ARG_VOLUME:
            ...
            if (play_bin->volume_element)
            {
                g_object_set(G_OBJECT(play_bin->volume_element), "volume", play_bin->volume, NULL);
            }
            break;
        case ARG_FONT_DESC:
            g_free(play_bin->font_desc);
            ...
            {
                g_object_set(G_OBJECT(play_bin->textoverlay_element), "font-desc", g_value_get_string(value), NULL);
            }
            break;
        default:
...
static void post_missing_element_message(GstPlayBin * playbin, const gchar * name)
{
    GstMessage *msg;
 ...
 *           handoff
 */
static GstElement *gen_video_element(GstPlayBin * play_bin)
{
    GstElement *element;
    GstElement *conv;
...
no_sinks:
    {
        ...
        GST_ELEMENT_ERROR(play_bin, CORE, MISSING_PLUGIN, (_("Both autovideosink and xvimagesink elements are missing.")), (NULL));
        return NULL;
    }
no_colorspace:
    {
        post_missing_element_message(play_bin, "ffmpegcolorspace");
        GST_ELEMENT_ERROR(play_bin, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "ffmpegcolorspace"), (NULL));
...
no_videoscale:
    {
        ...
        GST_ELEMENT_ERROR(play_bin, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "videoscale"), ("possibly a liboil version mismatch?"));
        gst_object_unref(element);
        return NULL;
    }
link_failed:
    {
        GST_ELEMENT_ERROR(play_bin, CORE, PAD, (NULL), ("Failed to configure the video sink."));
        gst_object_unref(element);
        return NULL;
    }
}
 ...
 */
static GstElement *add_text_element(GstPlayBin * play_bin, GstElement * vbin)
{
    ...
    /* Set some parameters */
    g_object_set(G_OBJECT(overlay), "halign", "center", "valign", "bottom", NULL);
...
no_overlay:
    {
        ...
        GST_WARNING_OBJECT(play_bin, "No overlay (pango) element, subtitles disabled");
        return vbin;
    }
}
 ...
 *
 */
static GstElement *add_spu_element(GstPlayBin * play_bin, GstElement * vbin)
{
    GstElement *element, *csp, *overlay;
    GstPad *pad;
...
no_overlay:
    {
        ...
        GST_WARNING_OBJECT(play_bin, "No DVD overlay (dvdspu) element. " "menu highlight/subtitles unavailable");
        return vbin;
    }
}
 ...
 *  +-------------------------------------------------------------+
 */
static GstElement *gen_audio_element(GstPlayBin * play_bin)
{
    gboolean res;
    GstElement *element;
    GstElement *conv;
...
no_sinks:
    {
        ...
        GST_ELEMENT_ERROR(play_bin, CORE, MISSING_PLUGIN, (_("Both autoaudiosink and alsasink elements are missing.")), (NULL));
        return NULL;
    }
no_audioconvert:
    {
        post_missing_element_message(play_bin, "audioconvert");
        GST_ELEMENT_ERROR(play_bin, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "audioconvert"), ("possibly a liboil version mismatch?"));
        gst_object_unref(element);
        return NULL;
    }
no_audioresample:
    {
        ...
        GST_ELEMENT_ERROR(play_bin, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "audioresample"), ("possibly a liboil version mismatch?"));
        gst_object_unref(element);
        return NULL;
    }
no_volume:
    {
        ...
        GST_ELEMENT_ERROR(play_bin, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "volume"), ("possibly a liboil version mismatch?"));
        gst_object_unref(element);
        return NULL;
    }
link_failed:
    {
        GST_ELEMENT_ERROR(play_bin, CORE, PAD, (NULL), ("Failed to configure the audio sink."));
        gst_object_unref(element);
        return NULL;
    }
}
 ...
 *  +-----------------------------------------------------------------------+
 */
static GstElement *gen_vis_element(GstPlayBin * play_bin)
{
    gboolean res;
    GstElement *element;
    GstElement *tee;
...
no_audioresample:
    {
        post_missing_element_message(play_bin, "audioresample");
        GST_ELEMENT_ERROR(play_bin, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "audioresample"), (NULL));
        gst_object_unref(element);
        return NULL;
    }
no_goom:
    {
        ...
        GST_ELEMENT_ERROR(play_bin, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "goom"), (NULL));
        gst_object_unref(element);
        return NULL;
    }
link_failed:
    {
        GST_ELEMENT_ERROR(play_bin, CORE, PAD, (NULL), ("Failed to configure the visualisation element."));
        gst_object_unref(element);
        return NULL;
    }
}
...
static void remove_sinks(GstPlayBin * play_bin)
{
    ...
    if (play_bin->visualisation)
    {
        ...
        vis_bin = GST_ELEMENT_CAST(gst_element_get_parent(play_bin->visualisation));
 ...
 */
static gboolean add_sink(GstPlayBin * play_bin, GstElement * sink, GstPad * srcpad, GstPad * subtitle_pad)
{
    GstPad *sinkpad;
    ...
    if (parent)
    {
        GST_DEBUG("Adding sink %" GST_PTR_FORMAT " with state %d (parent: %d, peer: %d)", sink, GST_STATE(sink), GST_STATE(play_bin), GST_STATE(parent));
        gst_object_unref(parent);
    }
...
link_failed:
    {
        ...
        GST_DEBUG_OBJECT(play_bin, "link failed when adding sink, caps %s, reason %d", capsstr, linkres);
        g_free(capsstr);
        gst_caps_unref(caps);
...
subtitle_failed:
    {
        ...
        GST_WARNING_OBJECT(play_bin, "subtitle link failed when adding sink, " "caps = %" GST_PTR_FORMAT ", reason %d", caps, linkres);
        gst_caps_unref(caps);
...
static void dummy_blocked_cb(GstPad * pad, gboolean blocked, gpointer user_data)
{
}
...
static gboolean setup_sinks(GstPlayBaseBin * play_base_bin, GstPlayBaseGroup * group)
{
    ...
    else if (!have_video && group->type[GST_STREAM_TYPE_AUDIO - 1].npads > 0 && play_bin->visualisation != NULL)
    {
        need_vis = TRUE;
    }
    ...
    if (group->type[GST_STREAM_TYPE_AUDIO - 1].npads > 0)
    {
        ...
        pad = gst_element_get_static_pad(group->type[GST_STREAM_TYPE_AUDIO - 1].preroll, "src");
    ...
    if (have_video)
    {
        ...
        if (need_text)
        {
            ...
            textsrcpad = gst_element_get_static_pad(group->type[GST_STREAM_TYPE_TEXT - 1].preroll, "src");
            ...
            {
                GST_DEBUG_OBJECT(textsrcpad, "this subtitle pad is from a subtitle " "file, ghosting to a suitable hierarchy");
                ...
                if (!GST_IS_PAD(ghost))
                {
                    GST_WARNING_OBJECT(textsrcpad, "failed creating ghost pad for " "subtitle-bin");
                    gst_object_unref(parent);
            ...
            {
                GST_DEBUG_OBJECT(textsrcpad, "this subtitle pad is from the demuxer " "no changes to hierarchy needed");
            }
...
beach:
        if (!sink)
            return FALSE;
        pad = gst_element_get_static_pad(group->type[GST_STREAM_TYPE_VIDEO - 1].preroll, "src");
        ...
        if (need_spu)
        {
            ...
            subpic_pad = gst_element_get_static_pad(group->type[GST_STREAM_TYPE_SUBPICTURE - 1].preroll, "src");
            ...
            if (subpic_pad && spu_sink_pad)
            {
                ...
                {
                    GST_WARNING_OBJECT(play_bin, "Failed to link DVD subpicture stream onto SPU");
                }
 ...
 */
static gboolean gst_play_bin_send_event_to_sink(GstPlayBin * play_bin, GstEvent * event)
{
    ...
    while (sinks)
    {
        ...
        {
            GST_DEBUG_OBJECT(play_bin, "Sent event succesfully to sink %" GST_PTR_FORMAT, sink);
            break;
        }
        GST_DEBUG_OBJECT(play_bin, "Event failed when sent to sink %" GST_PTR_FORMAT, sink);
...
static GstMessage *gst_play_bin_handle_redirect_message(GstPlayBin * playbin, GstMessage * msg)
{
...
gboolean gst_play_bin_plugin_init(GstPlugin * plugin)
{
    ...
    return gst_element_register(plugin, "playbin", GST_RANK_NONE, GST_TYPE_PLAY_BIN);
}
