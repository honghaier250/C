...
struct _GstURIDecodeBinClass
{
    ...
    /* signal fired to select from the proposed list of factories */
      GstAutoplugSelectResult(*autoplug_select) (GstElement * element, GstPad * pad, GstCaps * caps, GValueArray * factories);
...
static void gst_uri_decode_bin_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_uri_decode_bin_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
static void gst_uri_decode_bin_finalize(GObject * obj);

static void handle_message(GstBin * bin, GstMessage * msg);

static gboolean gst_uri_decode_bin_query(GstElement * element, GstQuery * query);
static GstStateChangeReturn gst_uri_decode_bin_change_state(GstElement * element, GstStateChange transition);

static void gst_uri_decode_bin_base_init(gpointer g_class)
{
    GstElementClass *gstelement_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&srctemplate));
    gst_element_class_set_details_simple(gstelement_class, "URI Decoder", "Generic/Bin/Decoder", "Autoplug and decode an URI to raw media", "Wim Taymans <wim.taymans@gmail.com>");
}
...
static gboolean gst_uri_decode_bin_autoplug_continue(GstElement * element, GstPad * pad, GstCaps * caps)
{
...
static void gst_uri_decode_bin_update_factories_list(GstURIDecodeBin * dec)
{
    if (!dec->factories || dec->factories_cookie != gst_default_registry_get_feature_list_cookie())
    {
        if (dec->factories)
            gst_plugin_feature_list_free(dec->factories);
        dec->factories = gst_element_factory_list_get_elements(GST_ELEMENT_FACTORY_TYPE_DECODABLE, GST_RANK_MARGINAL);
        dec->factories_cookie = gst_default_registry_get_feature_list_cookie();
    }
}

static GValueArray *gst_uri_decode_bin_autoplug_factories(GstElement * element, GstPad * pad, GstCaps * caps)
{
    GList *list, *tmp;
    GValueArray *result;
    ...
    list = gst_element_factory_list_filter(dec->factories, caps, GST_PAD_SINK, FALSE);
...
static void gst_uri_decode_bin_class_init(GstURIDecodeBinClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_URI, g_param_spec_string("uri", "URI", "URI to decode", DEFAULT_PROP_URI, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_SOURCE, g_param_spec_object("source", "Source", "Source object used", GST_TYPE_ELEMENT, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    ...
    g_object_class_install_property(gobject_class, PROP_CAPS, g_param_spec_boxed("caps", "Caps", "The caps on which to stop decoding. (NULL = default)", GST_TYPE_CAPS, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    g_object_class_install_property(gobject_class, PROP_BUFFER_SIZE,
                                    g_param_spec_int("buffer-size", "Buffer size (bytes)", "Buffer size when buffering streams (-1 default value)", -1, G_MAXINT, DEFAULT_BUFFER_SIZE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_BUFFER_DURATION,
                                    g_param_spec_int64("buffer-duration", "Buffer duration (ns)", "Buffer duration when buffering streams (-1 default value)", -1, G_MAXINT64, DEFAULT_BUFFER_DURATION, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_object_class_install_property(gobject_class, PROP_DOWNLOAD, g_param_spec_boolean("download", "Download", "Attempt download buffering when buffering network streams", DEFAULT_DOWNLOAD, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   ...
   * Since: 0.10.26
   */
    g_object_class_install_property(gobject_class, PROP_USE_BUFFERING, g_param_spec_boolean("use-buffering", "Use Buffering", "Perform buffering on demuxed/parsed media", DEFAULT_USE_BUFFERING, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   */
    gst_uri_decode_bin_signals[SIGNAL_UNKNOWN_TYPE] =
        g_signal_new("unknown-type", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstURIDecodeBinClass, unknown_type), NULL, NULL, gst_marshal_VOID__OBJECT_BOXED, G_TYPE_NONE, 2, GST_TYPE_PAD, GST_TYPE_CAPS);
   ...
   */
    gst_uri_decode_bin_signals[SIGNAL_AUTOPLUG_CONTINUE] =
        g_signal_new("autoplug-continue", G_TYPE_FROM_CLASS(klass),
                     G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstURIDecodeBinClass, autoplug_continue), _gst_boolean_accumulator, NULL, gst_play_marshal_BOOLEAN__OBJECT_BOXED, G_TYPE_BOOLEAN, 2, GST_TYPE_PAD, GST_TYPE_CAPS);
   ...
   */
    gst_uri_decode_bin_signals[SIGNAL_AUTOPLUG_FACTORIES] =
        g_signal_new("autoplug-factories", G_TYPE_FROM_CLASS(klass),
                     G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstURIDecodeBinClass, autoplug_factories), _gst_array_accumulator, NULL, gst_play_marshal_BOXED__OBJECT_BOXED, G_TYPE_VALUE_ARRAY, 2, GST_TYPE_PAD, GST_TYPE_CAPS);
    ...
    gst_uri_decode_bin_signals[SIGNAL_AUTOPLUG_SELECT] =
        g_signal_new("autoplug-select", G_TYPE_FROM_CLASS(klass),
                     G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstURIDecodeBinClass,
                                                        autoplug_select), _gst_select_accumulator, NULL, gst_play_marshal_ENUM__OBJECT_BOXED_OBJECT, GST_TYPE_AUTOPLUG_SELECT_RESULT, 3, GST_TYPE_PAD, GST_TYPE_CAPS, GST_TYPE_ELEMENT_FACTORY);
   ...
   */
    gst_uri_decode_bin_signals[SIGNAL_DRAINED] = g_signal_new("drained", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstURIDecodeBinClass, drained), NULL, NULL, gst_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);

    gstelement_class->query = GST_DEBUG_FUNCPTR(gst_uri_decode_bin_query);
    gstelement_class->change_state = GST_DEBUG_FUNCPTR(gst_uri_decode_bin_change_state);

    gstbin_class->handle_message = GST_DEBUG_FUNCPTR(handle_message);

    klass->autoplug_continue = GST_DEBUG_FUNCPTR(gst_uri_decode_bin_autoplug_continue);
    klass->autoplug_factories = GST_DEBUG_FUNCPTR(gst_uri_decode_bin_autoplug_factories);
}

static void gst_uri_decode_bin_init(GstURIDecodeBin * dec, GstURIDecodeBinClass * klass)
{
...
static void gst_uri_decode_bin_set_encoding(GstURIDecodeBin * dec, const gchar * encoding)
{
    GSList *walk;
...
static void do_async_start(GstURIDecodeBin * dbin)
{
    GstMessage *message;
...
static void do_async_done(GstURIDecodeBin * dbin)
{
    GstMessage *message;
...
static void unknown_type_cb(GstElement * element, GstPad * pad, GstCaps * caps, GstURIDecodeBin * decoder)
{
    gchar *capsstr;

    capsstr = gst_caps_to_string(caps);
    GST_ELEMENT_WARNING(decoder, STREAM, CODEC_NOT_FOUND, (_("No decoder available for type \'%s\'."), capsstr), (NULL));
    g_free(capsstr);
}
 ...
 */
static void add_element_stream(GstElement * element, GstURIDecodeBin * decoder)
{
 ...
 */
static void no_more_pads_full(GstElement * element, gboolean subs, GstURIDecodeBin * decoder)
{
    gboolean final;
...
static void no_more_pads(GstElement * element, GstURIDecodeBin * decoder)
{
    no_more_pads_full(element, FALSE, decoder);
}

static void source_no_more_pads(GstElement * element, GstURIDecodeBin * bin)
{
 ...
 * found a new raw pad.  
 */
static void new_decoded_pad_cb(GstElement * element, GstPad * pad, gboolean last, GstURIDecodeBin * decoder)
{
    GstPad *newpad;
    GstPadTemplate *pad_tmpl;
    gchar *padname;

    GST_DEBUG_OBJECT(element, "new decoded pad, name: <%s>. Last: %d", GST_PAD_NAME(pad), last);
...
static gboolean source_pad_event_probe(GstPad * pad, GstEvent * event, GstURIDecodeBin * decoder)
{
    ...
    if (GST_EVENT_TYPE(event) == GST_EVENT_EOS)
    {
        ...
        g_signal_emit(decoder, gst_uri_decode_bin_signals[SIGNAL_DRAINED], 0, NULL);
    }
    /* never drop events */
    return TRUE;
}
...
static void expose_decoded_pad(GstElement * element, GstPad * pad, GstURIDecodeBin * decoder)
{
...
static void pad_removed_cb(GstElement * element, GstPad * pad, GstURIDecodeBin * decoder)
{
    GstPad *ghost;

    GST_DEBUG_OBJECT(element, "pad removed name: <%s:%s>", GST_DEBUG_PAD_NAME(pad));
...
static gboolean array_has_uri_value(const gchar * values[], const gchar * value)
{
    gint i;
 ...
 */
static GstElement *gen_source_element(GstURIDecodeBin * decoder)
{
    GstElement *source;
    ...
    if (!strncmp(decoder->uri, "http://", 7) && g_object_class_find_property(G_OBJECT_GET_CLASS(source), "iradio-mode"))
    {
    ...
    {
        GST_DEBUG_OBJECT(decoder, "setting connection-speed=%d to source element", decoder->connection_speed / 1000);
        g_object_set(source, "connection-speed", decoder->connection_speed / 1000, NULL);
    }
    if (g_object_class_find_property(G_OBJECT_GET_CLASS(source), "subtitle-encoding"))
    {
        GST_DEBUG_OBJECT(decoder, "setting subtitle-encoding=%s to source element", decoder->encoding);
...
no_uri:
    {
        GST_ELEMENT_ERROR(decoder, RESOURCE, NOT_FOUND, (_("No URI specified to play from.")), (NULL));
        return NULL;
    }
invalid_uri:
    {
        GST_ELEMENT_ERROR(decoder, RESOURCE, NOT_FOUND, (_("Invalid URI \"%s\"."), decoder->uri), (NULL));
        return NULL;
    }
uri_blacklisted:
    {
        GST_ELEMENT_ERROR(decoder, RESOURCE, FAILED, (_("This stream type cannot be played yet.")), (NULL));
        return NULL;
    }
no_source:
    {
        ...
        if (prot)
        {
            ...
            msg = gst_missing_uri_source_message_new(GST_ELEMENT_CAST(decoder), prot);
            gst_element_post_message(GST_ELEMENT_CAST(decoder), msg);

            GST_ELEMENT_ERROR(decoder, CORE, MISSING_PLUGIN, (_("No URI handler implemented for \"%s\"."), prot), (NULL));
            g_free(prot);
        }
        else
            goto invalid_uri;
 ...
 */
static gboolean has_all_raw_caps(GstPad * pad, GstCaps * rawcaps, gboolean * all_raw)
{
    ...
    *all_raw = !gst_caps_is_empty(intersection) && (gst_caps_get_size(intersection) == capssize);
...
static void post_missing_plugin_error(GstElement * dec, const gchar * element_name)
{
    GstMessage *msg;
    ...
    GST_ELEMENT_ERROR(dec, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), element_name), (NULL));
}
 ...
 */
static gboolean analyse_source(GstURIDecodeBin * decoder, gboolean * is_raw, gboolean * have_out, gboolean * is_dynamic, gboolean use_queue)
{
...
static void remove_decoders(GstURIDecodeBin * bin, gboolean force)
{
    ...
    for (walk = bin->decodebins; walk; walk = g_slist_next(walk))
    {
        ...
        else
        {
            ...
            bin->pending_decodebins = g_slist_prepend(bin->pending_decodebins, decoder);
        }
    }
...
static void remove_pads(GstURIDecodeBin * bin)
{
    GSList *walk;
...
static void proxy_unknown_type_signal(GstElement * element, GstPad * pad, GstCaps * caps, GstURIDecodeBin * dec)
{
    GST_DEBUG_OBJECT(dec, "unknown-type signaled");

    g_signal_emit(dec, gst_uri_decode_bin_signals[SIGNAL_UNKNOWN_TYPE], 0, pad, caps);
}

static gboolean proxy_autoplug_continue_signal(GstElement * element, GstPad * pad, GstCaps * caps, GstURIDecodeBin * dec)
{
    gboolean result;

    g_signal_emit(dec, gst_uri_decode_bin_signals[SIGNAL_AUTOPLUG_CONTINUE], 0, pad, caps, &result);
...
static GValueArray *proxy_autoplug_factories_signal(GstElement * element, GstPad * pad, GstCaps * caps, GstURIDecodeBin * dec)
{
    GValueArray *result;

    g_signal_emit(dec, gst_uri_decode_bin_signals[SIGNAL_AUTOPLUG_FACTORIES], 0, pad, caps, &result);
...
static GstAutoplugSelectResult proxy_autoplug_select_signal(GstElement * element, GstPad * pad, GstCaps * caps, GstElementFactory * factory, GstURIDecodeBin * dec)
{
    GstAutoplugSelectResult result;

    g_signal_emit(dec, gst_uri_decode_bin_signals[SIGNAL_AUTOPLUG_SELECT], 0, pad, caps, factory, &result);
...
static void proxy_drained_signal(GstElement * element, GstURIDecodeBin * dec)
{
...
static GstElement *make_decoder(GstURIDecodeBin * decoder)
{
    ...
    if (decoder->pending_decodebins)
    {
        ...
        decoder->pending_decodebins = g_slist_delete_link(decoder->pending_decodebins, first);
    }
    else
    {
        ...
        g_signal_connect(decodebin, "unknown-type", G_CALLBACK(proxy_unknown_type_signal), decoder);
        g_signal_connect(decodebin, "autoplug-continue", G_CALLBACK(proxy_autoplug_continue_signal), decoder);
        g_signal_connect(decodebin, "autoplug-factories", G_CALLBACK(proxy_autoplug_factories_signal), decoder);
        g_signal_connect(decodebin, "autoplug-select", G_CALLBACK(proxy_autoplug_select_signal), decoder);
        g_signal_connect(decodebin, "drained", G_CALLBACK(proxy_drained_signal), decoder);

        /* set up callbacks to create the links between decoded data
         * and video/audio/subtitle rendering/output. */
        g_signal_connect(decodebin, "new-decoded-pad", G_CALLBACK(new_decoded_pad_cb), decoder);
        g_signal_connect(decodebin, "pad-removed", G_CALLBACK(pad_removed_cb), decoder);
        g_signal_connect(decodebin, "no-more-pads", G_CALLBACK(no_more_pads), decoder);
        g_signal_connect(decodebin, "unknown-type", G_CALLBACK(unknown_type_cb), decoder);
    }
    ...
    g_object_set(decodebin, "expose-all-streams", decoder->expose_allstreams, NULL);
    ...
    if (!decoder->is_stream)
    {
        ...
        if (decoder->use_buffering)
        {
            ...
            g_object_set(decodebin, "max-size-bytes", max_bytes, "max-size-buffers", (guint) 0, "max-size-time", max_time, NULL);
        }
    }
...
static void type_found(GstElement * typefind, guint probability, GstCaps * caps, GstURIDecodeBin * decoder)
{
    ...
    if (decoder->is_download)
    {
        ...
        decoder->is_download = (gst_element_query_duration(typefind, &fmt, &dur) && fmt == GST_FORMAT_BYTES && dur != -1);
    }
    ...
    g_object_set(queue, "ring-buffer-max-size", decoder->ring_buffer_max_size, NULL);

    GST_DEBUG_OBJECT(decoder, "check media-type %s, %d", media_type, decoder->download);
    ...
    if (decoder->is_download)
    {
        ...
        GST_DEBUG_OBJECT(decoder, "enable download buffering in %s (%s, %s, %s)", temp_template, tmp_dir, prgname, filename);
...
could_not_link:
    {
        GST_ELEMENT_ERROR(decoder, CORE, NEGOTIATION, (NULL), ("Can't link typefind to decodebin2 element"));
        return;
    }
no_queue2:
    {
...
static gboolean setup_streaming(GstURIDecodeBin * decoder)
{
    GstElement *typefind;
     ...
     * a type */
    decoder->have_type_id = g_signal_connect(decoder->typefind, "have-type", G_CALLBACK(type_found), decoder);
...
could_not_link:
    {
        GST_ELEMENT_ERROR(decoder, CORE, NEGOTIATION, (NULL), ("Can't link source to typefind element"));
...
static void remove_source(GstURIDecodeBin * bin)
{
...
static void source_new_pad(GstElement * element, GstPad * pad, GstURIDecodeBin * bin)
{
    GstElement *decoder;
    gboolean is_raw;
    GstCaps *rawcaps;
...
*/
static gboolean setup_source(GstURIDecodeBin * decoder)
{
    ...
    if (!analyse_source(decoder, &is_raw, &have_out, &is_dynamic, decoder->need_queue))
        goto invalid_source;
    ...
    if (is_dynamic)
    {
        ...
        decoder->src_np_sig_id = g_signal_connect(decoder->source, "pad-added", G_CALLBACK(source_new_pad), decoder);
        decoder->src_nmp_sig_id = g_signal_connect(decoder->source, "no-more-pads", G_CALLBACK(source_no_more_pads), decoder);
        g_object_set_data(G_OBJECT(decoder->source), "pending", GINT_TO_POINTER(1));
        decoder->pending++;
    }
    else
    {
...
invalid_source:
    {
        GST_ELEMENT_ERROR(decoder, CORE, FAILED, (_("Source element is invalid.")), (NULL));
        return FALSE;
    }
no_decoder:
    {
...
could_not_link:
    {
        GST_ELEMENT_ERROR(decoder, CORE, NEGOTIATION, (NULL), ("Can't link source to decoder element"));
        return FALSE;
    }
}

static void value_list_append_structure_list(GValue * list_val, GstStructure ** first, GList * structure_list)
{
    GList *l;
...
static GstMessage *handle_redirect_message(GstURIDecodeBin * dec, GstMessage * msg)
{
...
static void handle_message(GstBin * bin, GstMessage * msg)
{
    if (GST_MESSAGE_TYPE(msg) == GST_MESSAGE_ELEMENT && msg->structure != NULL && gst_structure_has_name(msg->structure, "redirect"))
    {
...
static void decoder_query_init(GstURIDecodeBin * dec, QueryFold * fold)
{
    fold->min = 0;
    fold->max = -1;
    fold->seekable = TRUE;
...
static gboolean decoder_query_duration_fold(GstPad * item, GValue * ret, QueryFold * fold)
{
...
static void decoder_query_duration_done(GstURIDecodeBin * dec, QueryFold * fold)
{
    GstFormat format;
...
static gboolean decoder_query_position_fold(GstPad * item, GValue * ret, QueryFold * fold)
{
    if (gst_pad_query(item, fold->query))
    {
        gint64 position;
...
static void decoder_query_position_done(GstURIDecodeBin * dec, QueryFold * fold)
{
    GstFormat format;
...
static gboolean decoder_query_latency_fold(GstPad * item, GValue * ret, QueryFold * fold)
{
    if (gst_pad_query(item, fold->query))
    {
        ...
        GST_DEBUG_OBJECT(item, "got latency min %" GST_TIME_FORMAT ", max %" GST_TIME_FORMAT ", live %d", GST_TIME_ARGS(min), GST_TIME_ARGS(max), live);
...
static void decoder_query_latency_done(GstURIDecodeBin * dec, QueryFold * fold)
{
    /* store max in query result */
    gst_query_set_latency(fold->query, fold->live, fold->min, fold->max);

    GST_DEBUG_OBJECT(dec, "latency min %" GST_TIME_FORMAT ", max %" GST_TIME_FORMAT ", live %d", GST_TIME_ARGS(fold->min), GST_TIME_ARGS(fold->max), fold->live);
}

/* we are seekable if all srcpads are seekable */
static gboolean decoder_query_seeking_fold(GstPad * item, GValue * ret, QueryFold * fold)
{
    if (gst_pad_query(item, fold->query))
    {
        gboolean seekable;
...
static void decoder_query_seeking_done(GstURIDecodeBin * dec, QueryFold * fold)
{
    GstFormat format;
...
static gboolean decoder_query_generic_fold(GstPad * item, GValue * ret, QueryFold * fold)
{
    gboolean res;
...
static gboolean gst_uri_decode_bin_query(GstElement * element, GstQuery * query)
{
    ...
    GST_DEBUG_OBJECT(element, "Sending query %p (type %d) to src pads", query, GST_QUERY_TYPE(query));
...
static gboolean gst_uri_decode_bin_plugin_init(GstPlugin * plugin)
{
    ...
    return gst_element_register(plugin, "uridecodebin", GST_RANK_NONE, GST_TYPE_URI_DECODE_BIN);
}
...
GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, "uridecodebin", "URI Decoder bin", plugin_init, VERSION, GST_LICENSE, GST_PACKAGE_NAME, GST_PACKAGE_ORIGIN)
