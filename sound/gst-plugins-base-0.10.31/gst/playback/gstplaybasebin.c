...
static void gst_play_base_bin_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * spec);
static void gst_play_base_bin_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * spec);
static void gst_play_base_bin_handle_message_func(GstBin * bin, GstMessage * msg);

static GstStateChangeReturn gst_play_base_bin_change_state(GstElement * element, GstStateChange transition);

static const GList *gst_play_base_bin_get_streaminfo(GstPlayBaseBin * bin);
static GValueArray *gst_play_base_bin_get_streaminfo_value_array(GstPlayBaseBin * play_base_bin);
static void preroll_remove_overrun(GstElement * element, GstPlayBaseBin * play_base_bin);
static void queue_remove_probe(GstElement * queue, GstPlayBaseBin * play_base_bin);
...
static void set_active_source(GstPlayBaseBin * play_base_bin, GstStreamType type, gint source_num);
static gboolean probe_triggered(GstPad * pad, GstEvent * event, gpointer user_data);
...
GType gst_play_base_bin_get_type(void)
{
    ...
    if (!gst_play_base_bin_type)
    {
        ...
        gst_play_base_bin_type = g_type_register_static(GST_TYPE_PIPELINE, "GstPlayBaseBin", &gst_play_base_bin_info, 0);
    }
...
static void gst_play_base_bin_class_init(GstPlayBaseBinClass * klass)
{
    ...
    g_object_class_install_property(gobject_klass, ARG_URI, g_param_spec_string("uri", "URI", "URI of the media to play", NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_klass, ARG_SUBURI, g_param_spec_string("suburi", ".sub-URI", "Optional URI of a subtitle", NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_klass, ARG_QUEUE_SIZE, g_param_spec_uint64("queue-size", "Queue size", "Size of internal queues in nanoseconds", 0, G_MAXINT64, DEFAULT_QUEUE_SIZE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_klass, ARG_QUEUE_THRESHOLD,
                                    g_param_spec_uint64("queue-threshold", "Queue threshold", "Buffering threshold of internal queues in nanoseconds", 0, G_MAXINT64, DEFAULT_QUEUE_THRESHOLD, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_klass, ARG_QUEUE_MIN_THRESHOLD,
                                    g_param_spec_uint64("queue-min-threshold", "Queue min threshold",
                                                        "Buffering low threshold of internal queues in nanoseconds", 0, G_MAXINT64, DEFAULT_QUEUE_MIN_THRESHOLD, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_klass, ARG_NSTREAMS, g_param_spec_int("nstreams", "NStreams", "number of streams", 0, G_MAXINT, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_klass, ARG_STREAMINFO, g_param_spec_pointer("stream-info", "Stream info", "List of streaminfo", G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_klass, ARG_STREAMINFO_VALUES,
                                    g_param_spec_value_array("stream-info-value-array",
                                                             ...
                                                             g_param_spec_object("streaminfo", "StreamInfo", "Streaminfo object", GST_TYPE_STREAM_INFO, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS), G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_klass, ARG_SOURCE, g_param_spec_object("source", "Source", "Source element", GST_TYPE_ELEMENT, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_klass, ARG_VIDEO, g_param_spec_int("current-video", "Current video", "Currently playing video stream (-1 = none)", -1, G_MAXINT, -1, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_klass, ARG_AUDIO, g_param_spec_int("current-audio", "Current audio", "Currently playing audio stream (-1 = none)", -1, G_MAXINT, -1, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_klass, ARG_TEXT, g_param_spec_int("current-text", "Current text", "Currently playing text stream (-1 = none)", -1, G_MAXINT, -1, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_object_class_install_property(gobject_klass, ARG_CONNECTION_SPEED,
                                    g_param_spec_uint("connection-speed", "Connection Speed", "Network connection speed in kbps (0 = unknown)", 0, G_MAXUINT, DEFAULT_CONNECTION_SPEED, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    GST_DEBUG_CATEGORY_INIT(gst_play_base_bin_debug, "playbasebin", 0, "playbasebin");

    gobject_klass->dispose = gst_play_base_bin_dispose;
    gobject_klass->finalize = gst_play_base_bin_finalize;

    gstbin_klass->handle_message = GST_DEBUG_FUNCPTR(gst_play_base_bin_handle_message_func);

    gstelement_klass->change_state = GST_DEBUG_FUNCPTR(gst_play_base_bin_change_state);
}
...
static GstPlayBaseGroup *group_create(GstPlayBaseBin * play_base_bin)
{
...
static GstPlayBaseGroup *get_active_group(GstPlayBaseBin * play_base_bin)
{
 ...
 * Callers must have group-lock held when calling this.
 */
static GstPlayBaseGroup *get_building_group(GstPlayBaseBin * play_base_bin)
{
    GstPlayBaseGroup *group;
...
static void group_destroy(GstPlayBaseGroup * group)
{
    ...
    for (n = 0; n < NUM_TYPES; n++)
    {
        ...
        for (item = sel->pads; item != NULL; item = item->next)
        {
            ...
            if (fakesrc != NULL)
            {
                GST_LOG("removing fakesrc from %s:%s", GST_PAD_NAME(pad), GST_ELEMENT_NAME(GST_PAD_PARENT(pad)));
 ...
 */
static void group_commit(GstPlayBaseBin * play_base_bin, gboolean fatal, gboolean subtitle)
{
    GstPlayBaseGroup *group;
    ...
    GST_DEBUG_OBJECT(play_base_bin, "commit group %p, had active %d", group, had_active_group);
    ...
    if (!subtitle && !had_active_group)
    {
        ...
        res = GST_PLAY_BASE_BIN_GET_CLASS(play_base_bin)->setup_output_pads(play_base_bin, group);
 ...
 * Callers must have group-lock held when calling this.
 */
static gboolean group_is_muted(GstPlayBaseGroup * group)
{
    gint n;
...
static inline void fill_buffer(GstPlayBaseBin * play_base_bin, gint percent)
{
    GST_DEBUG_OBJECT(play_base_bin, "buffering %d", percent);
    gst_element_post_message(GST_ELEMENT_CAST(play_base_bin), gst_message_new_buffering(GST_OBJECT_CAST(play_base_bin), percent));
}

static gboolean check_queue_event(GstPad * pad, GstEvent * event, gpointer user_data)
{
...
static gboolean check_queue(GstPad * pad, GstBuffer * data, gpointer user_data)
{
    GstElement *queue = GST_ELEMENT_CAST(user_data);
    ...
    GST_DEBUG_OBJECT(play_base_bin, "Queue size: %" GST_TIME_FORMAT, GST_TIME_ARGS(level));
 ...
 * done).
 */
static void queue_deadlock_check(GstElement * queue, GstPlayBaseBin * play_base_bin)
{
    guint64 time, min_time;
    guint bytes;

    GST_DEBUG_OBJECT(play_base_bin, "overrun signal received from queue %s", GST_ELEMENT_NAME(queue));

    /* figure out where we are */
    g_object_get(G_OBJECT(queue), "current-level-time", &time, "current-level-bytes", &bytes, "min-threshold-time", &min_time, NULL);

    GST_DEBUG_OBJECT(play_base_bin, "streaming mode, queue %s current %" GST_TIME_FORMAT ", min %" GST_TIME_FORMAT ", bytes %d", GST_ELEMENT_NAME(queue), GST_TIME_ARGS(time), GST_TIME_ARGS(min_time), bytes);
    ...
    {
        GST_DEBUG_OBJECT(play_base_bin, "possible deadlock found, removing byte limit");
        ...
        g_signal_handlers_disconnect_by_func(queue, (gpointer) queue_deadlock_check, play_base_bin);
    }
    else
    {
...
static void queue_remove_probe(GstElement * queue, GstPlayBaseBin * play_base_bin)
{
    ...
    if (data)
    {
        GST_DEBUG_OBJECT(play_base_bin, "Removing buffer probe from pad %s:%s (%p)", GST_DEBUG_PAD_NAME(sinkpad), sinkpad);
    ...
    {
        GST_DEBUG_OBJECT(play_base_bin, "No buffer probe to remove from %s:%s (%p)", GST_DEBUG_PAD_NAME(sinkpad), sinkpad);
    }
...
static void queue_threshold_reached(GstElement * queue, GstPlayBaseBin * play_base_bin)
{
    GstPlayBaseGroup *group;
    gint n;

    GST_DEBUG_OBJECT(play_base_bin, "running signal received from queue %s", GST_ELEMENT_NAME(queue));

    /* we disconnect the signal so that we don't get called for every buffer. */
    g_signal_handlers_disconnect_by_func(queue, (gpointer) queue_threshold_reached, play_base_bin);
    ...
    else
    {
        ...
        GST_DEBUG_OBJECT(play_base_bin, "setting min threshold time to %" G_GUINT64_FORMAT, play_base_bin->queue_min_threshold);
        g_object_set(queue, "min-threshold-time", play_base_bin->queue_min_threshold, NULL);
    }
 ...
 * signal is fired.
 */
static void queue_overrun(GstElement * queue, GstPlayBaseBin * play_base_bin)
{
    GST_DEBUG_OBJECT(play_base_bin, "queue %s overrun", GST_ELEMENT_NAME(queue));

    preroll_remove_overrun(queue, play_base_bin);

    group_commit(play_base_bin, FALSE, GST_OBJECT_PARENT(GST_OBJECT_CAST(queue)) == GST_OBJECT_CAST(play_base_bin->subtitle));
 ...
 */
static void queue_out_of_data(GstElement * queue, GstPlayBaseBin * play_base_bin)
{
    GST_DEBUG_OBJECT(play_base_bin, "underrun signal received from queue %s", GST_ELEMENT_NAME(queue));
     ...
     * handler. */
    g_signal_connect(G_OBJECT(queue), "pushing", G_CALLBACK(queue_threshold_reached), play_base_bin);
    GST_DEBUG_OBJECT(play_base_bin, "setting min threshold time to %" G_GUINT64_FORMAT, (guint64) play_base_bin->queue_threshold);
    g_object_set(queue, "min-threshold-time", (guint64) play_base_bin->queue_threshold, NULL);
    ...
    if (!g_object_get_data(G_OBJECT(queue), "probe"))
    {
        ...
        GST_DEBUG_OBJECT(play_base_bin, "Re-attaching buffering probe to pad %s:%s %p", GST_DEBUG_PAD_NAME(sinkpad), sinkpad);
        gst_object_unref(sinkpad);
 ...
 */
static void gen_preroll_element(GstPlayBaseBin * play_base_bin, GstPlayBaseGroup * group, GstStreamType type, GstPad * pad, GstStreamInfo * info)
{
    ...
    if (play_base_bin->raw_decoding_mode)
    {
        if (type == GST_STREAM_TYPE_VIDEO)
        {
            g_object_set(G_OBJECT(preroll), "max-size-buffers", 2, "max-size-bytes", 0, "max-size-time", (guint64) 0, NULL);
        }
        else
        {
            g_object_set(G_OBJECT(preroll), "max-size-buffers", 0, "max-size-bytes", 2 * 1024 * 1024, "max-size-time", play_base_bin->queue_size, NULL);
        }
    }
    else
    {
        g_object_set(G_OBJECT(preroll), "max-size-buffers", 0, "max-size-bytes", ((type == GST_STREAM_TYPE_VIDEO) ? 25 : 2) * 1024 * 1024, "max-size-time", play_base_bin->queue_size, NULL);
    }
     ...
     */
    overrun_sig = g_signal_connect(G_OBJECT(preroll), "overrun", G_CALLBACK(queue_overrun), play_base_bin);

    /* keep a ref to the signal id so that we can disconnect the signal callback
     * when we are done with the preroll */
    g_object_set_data(G_OBJECT(preroll), "overrun_signal_id", GINT_TO_POINTER(overrun_sig));

    if (play_base_bin->is_stream && ((type == GST_STREAM_TYPE_VIDEO && group->type[GST_STREAM_TYPE_AUDIO - 1].npads == 0) || (type == GST_STREAM_TYPE_AUDIO && group->type[GST_STREAM_TYPE_VIDEO - 1].npads == 0)))
    {
        GstPad *sinkpad;
        guint id;
         ...
         * in bytes is hit. */
        g_signal_connect(G_OBJECT(preroll), "overrun", G_CALLBACK(queue_deadlock_check), play_base_bin);
        ...
        GST_DEBUG_OBJECT(play_base_bin, "Attaching probe to pad %s:%s (%p)", GST_DEBUG_PAD_NAME(sinkpad), sinkpad);
        g_object_set_data(G_OBJECT(preroll), "probe", GINT_TO_POINTER(id));

        /* catch eos and flush events so that we can ignore underruns */
        id = gst_pad_add_event_probe(sinkpad, G_CALLBACK(check_queue_event), preroll);
         ...
         * fire an underrun. */
        g_signal_connect(G_OBJECT(preroll), "underrun", G_CALLBACK(queue_out_of_data), play_base_bin);
    ...
    state = (GST_STATE(play_base_bin) == GST_STATE_PLAYING ? GST_STATE_PLAYING : GST_STATE_PAUSED);
...
static void preroll_remove_overrun(GstElement * element, GstPlayBaseBin * play_base_bin)
{
    ...
    if (overrun_sig)
    {
        GST_LOG_OBJECT(play_base_bin, "removing preroll signal %s", GST_ELEMENT_NAME(element));
...
static void remove_groups(GstPlayBaseBin * play_base_bin)
{
    GROUP_LOCK(play_base_bin);
 ...
 */
static void add_stream(GstPlayBaseGroup * group, GstStreamInfo * info)
{
    GValue v = { 0, };
...
static gboolean string_arr_has_str(const gchar * values[], const gchar * value)
{
    if (values && value)
    {
        while (*values != NULL)
 ...
 */
static void unknown_type(GstElement * element, GstPad * pad, GstCaps * caps, GstPlayBaseBin * play_base_bin)
{
    ...
    else
    {
        ...
        GST_DEBUG_OBJECT(play_base_bin, "media type %s not handled on purpose, " "not posting a missing-plugin message on the bus", type_name);
    }
    ...
    info = gst_stream_info_new(GST_OBJECT_CAST(pad), GST_STREAM_TYPE_UNKNOWN, NULL, caps);
 ...
 */
static void add_element_stream(GstElement * element, GstPlayBaseBin * play_base_bin)
{
    GstStreamInfo *info;
    GstPlayBaseGroup *group;
    ...
    /* add the stream to the list */
    info = gst_stream_info_new(GST_OBJECT_CAST(element), GST_STREAM_TYPE_ELEMENT, NULL, NULL);
 ...
 */
static void no_more_pads_full(GstElement * element, gboolean subs, GstPlayBaseBin * play_base_bin)
{
    /* setup phase */
    GST_DEBUG_OBJECT(element, "no more pads, %d pending", play_base_bin->pending);
    ...
    GST_DEBUG_OBJECT(element, "remove pending, now %d pending", play_base_bin->pending);
...
static void no_more_pads(GstElement * element, GstPlayBaseBin * play_base_bin)
{
    no_more_pads_full(element, FALSE, play_base_bin);
}

static void sub_no_more_pads(GstElement * element, GstPlayBaseBin * play_base_bin)
{
    no_more_pads_full(element, TRUE, play_base_bin);
}

static void source_no_more_pads(GstElement * element, GstPlayBaseBin * bin)
{
    GST_DEBUG_OBJECT(bin, "No more pads in source element %s.", GST_ELEMENT_NAME(element));
...
static gboolean probe_triggered(GstPad * pad, GstEvent * event, gpointer user_data)
{
    ...
    if (type == GST_EVENT_EOS)
    {
        ...
        if (have_left)
        {
             ...
             * active */
            play_base_bin->queued_groups = g_list_remove(play_base_bin->queued_groups, group);
 ...
 */
static void preroll_unlinked(GstPad * pad, GstPad * peerpad, GstPlayBaseBin * play_base_bin)
{
    GstElement *fakesrc;
    guint sig_id;
    GstPad *srcpad;
 ...
 * (vorbis, ogmtext). */
static gboolean mute_stream(GstPad * pad, GstBuffer * buf, gpointer data)
{
...
/* Eat data. */
static gboolean silence_stream(GstPad * pad, GstMiniObject * data, gpointer user_data)
{
 ...
 * from decodebin */
static void new_decoded_pad_full(GstElement * element, GstPad * pad, gboolean last, GstPlayBaseBin * play_base_bin, gboolean is_subs)
{
    ...
    if (g_str_has_prefix(mimetype, "audio/") && parent != GST_OBJECT_CAST(play_base_bin->subtitle))
    {
        type = GST_STREAM_TYPE_AUDIO;
    }
    else if (g_str_has_prefix(mimetype, "video/x-dvd-subpicture") && parent != GST_OBJECT_CAST(play_base_bin->subtitle))
    {
        type = GST_STREAM_TYPE_SUBPICTURE;
    }
    else if (g_str_has_prefix(mimetype, "video/") && parent != GST_OBJECT_CAST(play_base_bin->subtitle))
    {
    ...
    sinkpad = gst_element_get_request_pad(group->type[type - 1].selector, "sink%d");

    /* make sure we catch unlink signals */
    sig = g_signal_connect(G_OBJECT(sinkpad), "unlinked", G_CALLBACK(preroll_unlinked), play_base_bin);
    /* keep a ref to the signal id so that we can disconnect the signal callback */
    ...
    if (!first_pad)
    {
        ...
        GST_DEBUG("Adding silence_stream data probe on type %d (npads %d)", type, group->type[type - 1].npads);

        id = gst_pad_add_data_probe(GST_PAD_CAST(pad), G_CALLBACK(silence_stream), info);
...
static void new_decoded_pad(GstElement * element, GstPad * pad, gboolean last, GstPlayBaseBin * play_base_bin)
{
    new_decoded_pad_full(element, pad, last, play_base_bin, FALSE);
}

static void subs_new_decoded_pad(GstElement * element, GstPad * pad, gboolean last, GstPlayBaseBin * play_base_bin)
{
    new_decoded_pad_full(element, pad, last, play_base_bin, TRUE);
}

static void set_encoding_element(GstElement * element, gchar * encoding)
{
...
static void decodebin_element_added_cb(GstBin * decodebin, GstElement * element, gpointer data)
{
    GstPlayBaseBin *play_base_bin = GST_PLAY_BASE_BIN(data);
    gchar *encoding;

    if (!g_object_class_find_property(G_OBJECT_GET_CLASS(element), "subtitle-encoding"))
    {
        return;
    }

    g_mutex_lock(play_base_bin->sub_lock);
    play_base_bin->subtitle_elements = g_slist_append(play_base_bin->subtitle_elements, element);
...
static void decodebin_element_removed_cb(GstBin * decodebin, GstElement * element, gpointer data)
{
    GstPlayBaseBin *play_base_bin = GST_PLAY_BASE_BIN(data);

    g_mutex_lock(play_base_bin->sub_lock);
    play_base_bin->subtitle_elements = g_slist_remove(play_base_bin->subtitle_elements, element);
    g_mutex_unlock(play_base_bin->sub_lock);
}
...
static GstElement *setup_subtitle(GstPlayBaseBin * play_base_bin, gchar * sub_uri)
{
    ...
    g_signal_connect(subdecodebin, "element-added", G_CALLBACK(decodebin_element_added_cb), play_base_bin);
    g_signal_connect(subdecodebin, "element-removed", G_CALLBACK(decodebin_element_removed_cb), play_base_bin);
...
invalid_uri:
    {
        GST_ELEMENT_WARNING(play_base_bin, RESOURCE, NOT_FOUND, (_("Invalid subtitle URI \"%s\", subtitles disabled."), sub_uri), (NULL));
        return NULL;
    }
unknown_uri:
    {
        ...
        if (prot)
        {
            ...
            gst_element_post_message(GST_ELEMENT(play_base_bin), gst_missing_uri_source_message_new(GST_ELEMENT(play_base_bin), prot));

            desc = gst_pb_utils_get_source_description(prot);
            GST_ELEMENT_ERROR(play_base_bin, CORE, MISSING_PLUGIN, (_("A %s plugin is required to play this stream, but not installed."), desc), ("No URI handler to handle sub_uri: %s", sub_uri));
            g_free(desc);
            g_free(prot);
        }
        else
 ...
 */
static GstElement *gen_source_element(GstPlayBaseBin * play_base_bin, GstElement ** subbin)
{
    GstElement *source;
    ...
    {
        GST_LOG_OBJECT(play_base_bin, "Creating decoder for subtitles URI %s", play_base_bin->suburi);
        /* subtitle specified */
    ...
    source = gst_element_make_from_uri(GST_URI_SRC, play_base_bin->uri, "source");
    if (!source)
        goto no_source;
    ...
    if (!strncmp(play_base_bin->uri, "http://", 7) && g_object_class_find_property(G_OBJECT_GET_CLASS(source), "iradio-mode"))
    {
        g_object_set(source, "iradio-mode", TRUE, NULL);
    }

    if (g_object_class_find_property(G_OBJECT_GET_CLASS(source), "connection-speed"))
    {
        GST_DEBUG_OBJECT(play_base_bin, "setting connection-speed=%d to source element", play_base_bin->connection_speed / 1000);
        g_object_set(source, "connection-speed", play_base_bin->connection_speed / 1000, NULL);
    }
...
no_uri:
    {
        GST_ELEMENT_ERROR(play_base_bin, RESOURCE, NOT_FOUND, (_("No URI specified to play from.")), (NULL));
        return NULL;
    }
invalid_uri:
    {
        GST_ELEMENT_ERROR(play_base_bin, RESOURCE, NOT_FOUND, (_("Invalid URI \"%s\"."), play_base_bin->uri), (NULL));
        return NULL;
    }
uri_blacklisted:
    {
        GST_ELEMENT_ERROR(play_base_bin, RESOURCE, FAILED, (_("RTSP streams cannot be played yet.")), (NULL));
        return NULL;
    }
no_source:
    {
        ...
        if (prot)
        {
            ...
            desc = gst_pb_utils_get_source_description(prot);
            GST_ELEMENT_ERROR(play_base_bin, CORE, MISSING_PLUGIN, (_("A %s plugin is required to play this stream, but not installed."), desc), ("No URI handler for %s", prot));
            g_free(desc);
            g_free(prot);
        }
        else
...
static void source_new_pad(GstElement * element, GstPad * pad, GstPlayBaseBin * bin)
{
    GstElement *decoder;
    gboolean is_raw;

    GST_DEBUG_OBJECT(bin, "Found new pad %s.%s in source element %s", GST_DEBUG_PAD_NAME(pad), GST_ELEMENT_NAME(element));
...
could_not_link:
    {
        GST_ELEMENT_ERROR(bin, CORE, NEGOTIATION, (NULL), ("Can't link source to decoder element"));
        return;
    }
}
...
static void setup_substreams(GstPlayBaseBin * play_base_bin)
{
    ...
    for (item = group->streaminfo; item; item = item->next)
    {
        ...
        if (data)
        {
            gst_pad_remove_data_probe(GST_PAD_CAST(info->object), GPOINTER_TO_INT(data));
        ...
        if (info->type == GST_STREAM_TYPE_UNKNOWN)
        {
            ...
            if (id == 0)
            {
                id = gst_pad_add_buffer_probe(GST_PAD_CAST(info->object), G_CALLBACK(mute_stream), info);
    ...
    for (n = 0; n < NUM_TYPES; n++)
    {
        GST_DEBUG_OBJECT(play_base_bin, "setting type %d to current %d", n, play_base_bin->current[n]);
 ...
 */
static gboolean analyse_source(GstPlayBaseBin * play_base_bin, gboolean * is_raw, gboolean * have_out, gboolean * is_dynamic)
{
        ...
        switch (gst_iterator_next(pads_iter, (gpointer) & pad))
        {
            ...
            case GST_ITERATOR_OK:
                ...
                *have_out = TRUE;
                ...
                if (*is_raw)
                {
                    new_decoded_pad_full(play_base_bin->source, pad, FALSE, play_base_bin, FALSE);
                }
...
static void remove_decoders(GstPlayBaseBin * bin)
{
    GSList *walk;
    ...
    for (walk = bin->decoders; walk; walk = g_slist_next(walk))
    {
         ...
         * we dispose of it */
        g_signal_handlers_disconnect_by_func(decoder, (gpointer) (decodebin_element_added_cb), bin);
        g_signal_handlers_disconnect_by_func(decoder, (gpointer) (decodebin_element_removed_cb), bin);
        g_signal_handlers_disconnect_by_func(decoder, (gpointer) (new_decoded_pad), bin);
        g_signal_handlers_disconnect_by_func(decoder, (gpointer) (no_more_pads), bin);
        g_signal_handlers_disconnect_by_func(decoder, (gpointer) (unknown_type), bin);
...
static GstElement *make_decoder(GstPlayBaseBin * play_base_bin)
{
    ...
    g_signal_connect(decoder, "element-added", G_CALLBACK(decodebin_element_added_cb), play_base_bin);
    g_signal_connect(decoder, "element-removed", G_CALLBACK(decodebin_element_removed_cb), play_base_bin);
    ...
    g_signal_connect(G_OBJECT(decoder), "new-decoded-pad", G_CALLBACK(new_decoded_pad), play_base_bin);
    g_signal_connect(G_OBJECT(decoder), "no-more-pads", G_CALLBACK(no_more_pads), play_base_bin);
    g_signal_connect(G_OBJECT(decoder), "unknown-type", G_CALLBACK(unknown_type), play_base_bin);
    g_object_set_data(G_OBJECT(decoder), "pending", GINT_TO_POINTER(1));
    play_base_bin->pending++;

    GST_DEBUG_OBJECT(play_base_bin, "created decodebin, %d pending", play_base_bin->pending);
...
no_decodebin:
    {
        GST_ELEMENT_ERROR(play_base_bin, CORE, MISSING_PLUGIN, (_("Could not create \"decodebin\" element.")), (NULL));
        return NULL;
    }
}

static void remove_source(GstPlayBaseBin * bin)
{
...
static GstBusSyncReply subbin_startup_sync_msg(GstBus * bus, GstMessage * msg, gpointer user_data)
{
    if (GST_MESSAGE_TYPE(msg) == GST_MESSAGE_ERROR)
    {
        ...
        {
            GST_WARNING_OBJECT(play_base_bin, "error starting up subtitle bin: %" GST_PTR_FORMAT, msg);
            play_base_bin->subtitle_done = TRUE;
            GST_DEBUG_OBJECT(play_base_bin, "signal group done");
            GROUP_SIGNAL(play_base_bin);
            GST_DEBUG_OBJECT(play_base_bin, "signaled group done");
...
*/
static gboolean setup_source(GstPlayBaseBin * play_base_bin)
{
    ...
    if (subbin)
    {
        ...
        g_signal_connect(G_OBJECT(db), "new-decoded-pad", G_CALLBACK(subs_new_decoded_pad), play_base_bin);
        g_signal_connect(G_OBJECT(db), "no-more-pads", G_CALLBACK(sub_no_more_pads), play_base_bin);
        g_signal_connect(G_OBJECT(db), "unknown-type", G_CALLBACK(unknown_type), play_base_bin);
        g_object_set_data(G_OBJECT(db), "pending", GINT_TO_POINTER(1));
        play_base_bin->pending++;

        GST_DEBUG_OBJECT(play_base_bin, "we have subtitles, %d pending", play_base_bin->pending);
        ...
        if (!play_base_bin->is_stream)
        {
            ...
            if (sret != GST_STATE_CHANGE_FAILURE)
            {
                ...
                if (!play_base_bin->building_group || play_base_bin->building_group->type[GST_STREAM_TYPE_TEXT - 1].npads == 0)
                {
    ...
    if (is_dynamic)
    {
        ...
        play_base_bin->src_np_sig_id = g_signal_connect(G_OBJECT(play_base_bin->source), "pad-added", G_CALLBACK(source_new_pad), play_base_bin);
        play_base_bin->src_nmp_sig_id = g_signal_connect(G_OBJECT(play_base_bin->source), "no-more-pads", G_CALLBACK(source_no_more_pads), play_base_bin);
        g_object_set_data(G_OBJECT(play_base_bin->source), "pending", GINT_TO_POINTER(1));
        play_base_bin->pending++;
        GST_DEBUG_OBJECT(play_base_bin, "Source has dynamic output pads, %d pending", play_base_bin->pending);
    }
    else
    {
        GstElement *decoder;
...
invalid_source:
    {
        GST_ELEMENT_ERROR(play_base_bin, CORE, FAILED, (_("Source element is invalid.")), (NULL));
        return FALSE;
    }
no_decodebin:
    {
...
could_not_link:
    {
        GST_ELEMENT_ERROR(play_base_bin, CORE, NEGOTIATION, (NULL), ("Can't link source to decoder element"));
        return FALSE;
    }
}

static void finish_source(GstPlayBaseBin * play_base_bin)
{
...
static gboolean prepare_output(GstPlayBaseBin * play_base_bin)
{
    ...
    for (item = group ? group->streaminfo : NULL; item != NULL; item = item->next)
    {
        ...
        else if (info->type == GST_STREAM_TYPE_TEXT || info->type == GST_STREAM_TYPE_SUBPICTURE)
        {
            got_subtitle = TRUE;
        }
        else if (!item->prev && !item->next)
        {
            ...
            {
                const gchar *mime = gst_structure_get_name(gst_caps_get_structure(info->caps, 0));
    ...
    if (!stream_found)
    {
        if (got_subtitle)
        {
            GST_ELEMENT_ERROR(play_base_bin, STREAM, WRONG_TYPE, (_("Only a subtitle stream was detected. " "Either you are loading a subtitle file or some other type of " "text file, or the media file was not recognized.")), (NULL));
        }
        else if (!no_media)
        {
            GST_ELEMENT_ERROR(play_base_bin, STREAM, CODEC_NOT_FOUND, (_("You do not have a decoder installed to handle this file. " "You might need to install the necessary plugins.")), (NULL));
        }
        else
        {
            GST_ELEMENT_ERROR(play_base_bin, STREAM, WRONG_TYPE, (_("This is not a media file")), (NULL));
        }
        return FALSE;
    }
    else if (got_subtitle && !got_video)
    {
        GST_ELEMENT_ERROR(play_base_bin, STREAM, WRONG_TYPE, (_("A subtitle stream was detected, but no video stream.")), (NULL));
        return FALSE;
    }
 ...
 */
static gint get_active_source(GstPlayBaseBin * play_base_bin, GstStreamType type)
{
    GstPlayBaseGroup *group;
    GList *s;
    gint num = 0;
...
#if 0
static void muted_group_change_state(GstElement * element, gint old_state, gint new_state, gpointer data);
#endif

static void mute_group_type(GstPlayBaseGroup * group, GstStreamType type, gboolean mute)
{
    ...
    if (mute)
    {
        g_signal_connect(group->type[type - 1].preroll, "state-changed", G_CALLBACK(muted_group_change_state), group);
    }
    else
    {
        g_signal_handlers_disconnect_by_func(group->type[type - 1].preroll, G_CALLBACK(muted_group_change_state), group);
    }
#endif
}
...
static void set_subtitles_visible(GstPlayBaseBin * play_base_bin, gboolean visible)
{
...
static void set_audio_mute(GstPlayBaseBin * play_base_bin, gboolean mute)
{
    GstPlayBaseBinClass *klass = GST_PLAY_BASE_BIN_GET_CLASS(play_base_bin);
    ...
    for (s = group->streaminfo; s; s = s->next)
    {
        ...
        if (info->type == type)
        {
            if (num == source_num)
            {
                ...
                GST_LOG("Unmuting (if already muted) source %d of type %d", source_num, type);
                g_object_set(info, "mute", FALSE, NULL);

                /* Tell the stream selector which pad to accept */
                sel_pad = GST_PAD_CAST(g_object_get_data(G_OBJECT(info->object), "pb_sel_pad"));
...
static void gst_play_base_bin_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    GstPlayBaseBin *play_base_bin;
    ...
    switch (prop_id)
    {
        ...
        case ARG_SUBURI:
            {
                ...
                if ((!suburi && !play_base_bin->suburi) || (suburi && play_base_bin->suburi && !strcmp(play_base_bin->suburi, suburi)))
                    return;
                g_free(play_base_bin->suburi);
        ...
        case ARG_VIDEO:
            GROUP_LOCK(play_base_bin);
            set_active_source(play_base_bin, GST_STREAM_TYPE_VIDEO, g_value_get_int(value));
            GROUP_UNLOCK(play_base_bin);
            break;
        case ARG_AUDIO:
            GROUP_LOCK(play_base_bin);
            set_active_source(play_base_bin, GST_STREAM_TYPE_AUDIO, g_value_get_int(value));
            GROUP_UNLOCK(play_base_bin);
            break;
        case ARG_TEXT:
            GROUP_LOCK(play_base_bin);
            set_active_source(play_base_bin, GST_STREAM_TYPE_TEXT, g_value_get_int(value));
            GROUP_UNLOCK(play_base_bin);
            break;
        case ARG_SUBTITLE_ENCODING:
            {
                ...
                if (encoding && play_base_bin->subencoding && !strcmp(play_base_bin->subencoding, encoding))
                {
                    return;
                }
...
static void gst_play_base_bin_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
    GstPlayBaseBin *play_base_bin;
    ...
    switch (prop_id)
    {
        ...
        case ARG_STREAMINFO:
            ...
            g_value_set_pointer(value, (gpointer) gst_play_base_bin_get_streaminfo(play_base_bin));
            break;
        ...
        case ARG_VIDEO:
            GROUP_LOCK(play_base_bin);
            g_value_set_int(value, get_active_source(play_base_bin, GST_STREAM_TYPE_VIDEO));
            GROUP_UNLOCK(play_base_bin);
            break;
        case ARG_AUDIO:
            GROUP_LOCK(play_base_bin);
            g_value_set_int(value, get_active_source(play_base_bin, GST_STREAM_TYPE_AUDIO));
            GROUP_UNLOCK(play_base_bin);
            break;
        case ARG_TEXT:
            GROUP_LOCK(play_base_bin);
            g_value_set_int(value, get_active_source(play_base_bin, GST_STREAM_TYPE_TEXT));
            GROUP_UNLOCK(play_base_bin);
            break;
        case ARG_SUBTITLE_ENCODING:
...
static const GList *gst_play_base_bin_get_streaminfo(GstPlayBaseBin * play_base_bin)
{
