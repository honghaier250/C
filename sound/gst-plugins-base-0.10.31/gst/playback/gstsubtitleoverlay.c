...
static GstStaticPadTemplate video_sinktemplate = GST_STATIC_PAD_TEMPLATE("video_sink",
                                                                         GST_PAD_SINK,
                                                                         GST_PAD_ALWAYS,
                                                                         GST_STATIC_CAPS_ANY);

static GstStaticPadTemplate subtitle_sinktemplate = GST_STATIC_PAD_TEMPLATE("subtitle_sink",
                                                                            GST_PAD_SINK,
                                                                            GST_PAD_ALWAYS,
                                                                            GST_STATIC_CAPS_ANY);
...
GST_BOILERPLATE(GstSubtitleOverlay, gst_subtitle_overlay, GstBin, GST_TYPE_BIN);

static void _pad_blocked_cb(GstPad * pad, gboolean blocked, gpointer user_data);

static GQuark _subtitle_overlay_event_marker_id = 0;

static void do_async_start(GstSubtitleOverlay * self)
{
    if (!self->do_async)
    {
        GstMessage *msg = gst_message_new_async_start(GST_OBJECT_CAST(self), FALSE);
...
static void do_async_done(GstSubtitleOverlay * self)
{
    if (self->do_async)
    {
...
static void gst_subtitle_overlay_finalize(GObject * object)
{
...
static gboolean _is_renderer(GstElementFactory * factory)
{
    ...
    if (strstr(klass, "Overlay/Subtitle") != NULL || strstr(klass, "Overlay/SubPicture") != NULL)
        return TRUE;
...
static gboolean _is_parser(GstElementFactory * factory)
{
    const gchar *klass;
...
static GstCaps *_get_sub_caps(GstElementFactory * factory)
{
...
static gboolean _factory_filter(GstPluginFeature * feature, GstCaps ** subcaps)
{
    ...
    if (is_renderer)
    {
        ...
        for (walk = (GList *) templates; walk; walk = g_list_next(walk))
        {
            ...
            if (templ->direction == GST_PAD_SINK && templ->presence == GST_PAD_ALWAYS)
            {
                if (strcmp(templ->name_template, "video") == 0 || strcmp(templ->name_template, "video_sink") == 0)
                {
    ...
    if (is_renderer && have_video_sink && templ_caps)
    {
        ...
        GST_DEBUG("Found renderer element %s (%s) with caps %" GST_PTR_FORMAT, gst_element_factory_get_longname(factory), gst_plugin_feature_get_name(feature), templ_caps);
    ...
    else if (!is_renderer && !have_video_sink && templ_caps)
    {
        ...
        GST_DEBUG("Found parser element %s (%s) with caps %" GST_PTR_FORMAT, gst_element_factory_get_longname(factory), gst_plugin_feature_get_name(feature), templ_caps);
        tmp = gst_caps_union(*subcaps, templ_caps);
        gst_caps_unref(templ_caps);
        gst_caps_replace(subcaps, tmp);
        gst_caps_unref(tmp);
...
static gboolean gst_subtitle_overlay_update_factory_list(GstSubtitleOverlay * self)
{
    if (!self->factories || self->factories_cookie != gst_default_registry_get_feature_list_cookie())
    {
        GstCaps *subcaps;
        GList *factories;
        ...
        factories = gst_default_registry_feature_filter((GstPluginFeatureFilter) _factory_filter, FALSE, &subcaps);
...
GstCaps *gst_subtitle_overlay_create_factory_caps(void)
{
    ...
    if (!_factory_caps || _factory_caps_cookie != gst_default_registry_get_feature_list_cookie())
    {
        if (_factory_caps)
            gst_caps_unref(_factory_caps);
        _factory_caps = gst_caps_new_empty();

        factories = gst_default_registry_feature_filter((GstPluginFeatureFilter) _factory_filter, FALSE, &_factory_caps);
...
static gboolean _filter_factories_for_caps(GstElementFactory * factory, const GstCaps * caps)
{
...
static gint _sort_by_ranks(GstPluginFeature * f1, GstPluginFeature * f2)
{
    gint diff;
...
static GstPad *_get_sub_pad(GstElement * element)
{
    GstPad *pad;
    guint i;
...
static GstPad *_get_video_pad(GstElement * element)
{
...
static gboolean _create_element(GstSubtitleOverlay * self, GstElement ** element, const gchar * factory_name, GstElementFactory * factory, const gchar * element_name, gboolean mandatory)
{
    GstElement *elt;
    ...
    {
        factory_name = gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(factory));
    ...
    if (G_UNLIKELY(!elt))
    {
        if (!factory)
        {
            ...
            msg = gst_missing_element_message_new(GST_ELEMENT_CAST(self), factory_name);
            gst_element_post_message(GST_ELEMENT_CAST(self), msg);

            if (mandatory)
                GST_ELEMENT_ERROR(self, CORE, MISSING_PLUGIN, (NULL), ("no '%s' plugin found", factory_name));
            else
                GST_ELEMENT_WARNING(self, CORE, MISSING_PLUGIN, (NULL), ("no '%s' plugin found", factory_name));
        }
        else
        {
            if (mandatory)
            {
                GST_ELEMENT_ERROR(self, CORE, FAILED, (NULL), ("can't instantiate '%s'", factory_name));
            }
            else
            {
                GST_ELEMENT_WARNING(self, CORE, FAILED, (NULL), ("can't instantiate '%s'", factory_name));
            }
        }
    ...
    if (G_UNLIKELY(gst_element_set_state(elt, GST_STATE_READY) != GST_STATE_CHANGE_SUCCESS))
    {
        gst_object_unref(elt);
        if (mandatory)
        {
            GST_ELEMENT_ERROR(self, CORE, STATE_CHANGE, (NULL), ("failed to set '%s' to READY", factory_name));
        }
        else
        {
    ...
    if (G_UNLIKELY(!gst_bin_add(GST_BIN_CAST(self), gst_object_ref(elt))))
    {
        ...
        if (mandatory)
        {
            GST_ELEMENT_ERROR(self, CORE, FAILED, (NULL), ("failed to add '%s' to subtitleoverlay", factory_name));
        }
        else
        {
            GST_WARNING_OBJECT(self, "Failed to add '%s' to subtitleoverlay", factory_name);
        }
        return FALSE;
    }
...
static void _remove_element(GstSubtitleOverlay * self, GstElement ** element)
{
    if (*element)
    {
...
static void _generate_update_newsegment_event(GstSegment * segment, GstEvent ** event1, GstEvent ** event2)
{
    GstEvent *event;
    ...
    *event2 = NULL;
    ...
    event = gst_event_new_new_segment_full(FALSE, segment->rate, segment->applied_rate, segment->format, 0, segment->accum, 0);
    gst_structure_id_set(event->structure, _subtitle_overlay_event_marker_id, G_TYPE_BOOLEAN, TRUE, NULL);
    *event1 = event;

    event = gst_event_new_new_segment_full(FALSE, segment->rate, segment->applied_rate, segment->format, segment->start, segment->stop, segment->time);
    gst_structure_id_set(event->structure, _subtitle_overlay_event_marker_id, G_TYPE_BOOLEAN, TRUE, NULL);
    *event2 = event;
}

static gboolean _setup_passthrough(GstSubtitleOverlay * self)
{
    GstPad *src, *sink;
    GstElement *identity;
    ...
    if (G_UNLIKELY(!_create_element(self, &self->passthrough_identity, "identity", NULL, "passthrough-identity", TRUE)))
    {
        return FALSE;
    }

    identity = self->passthrough_identity;
    g_object_set(G_OBJECT(identity), "silent", TRUE, "signal-handoffs", FALSE, NULL);

    /* Set src ghostpad target */
    src = gst_element_get_static_pad(self->passthrough_identity, "src");
    if (G_UNLIKELY(!src))
    {
        GST_ELEMENT_ERROR(self, CORE, PAD, (NULL), ("Failed to get srcpad from identity"));
        return FALSE;
    }

    if (G_UNLIKELY(!gst_ghost_pad_set_target(GST_GHOST_PAD_CAST(self->srcpad), src)))
    {
        GST_ELEMENT_ERROR(self, CORE, PAD, (NULL), ("Failed to set srcpad target"));
    ...
    if (G_UNLIKELY(!sink))
    {
        GST_ELEMENT_ERROR(self, CORE, PAD, (NULL), ("Failed to get sinkpad from identity"));
        return FALSE;
    }
    ...
    if (self->video_segment.format != GST_FORMAT_UNDEFINED)
    {
        ...
        GST_DEBUG_OBJECT(self, "Pushing video accumulate newsegment event: %" GST_PTR_FORMAT, event1->structure);
        GST_DEBUG_OBJECT(self, "Pushing video update newsegment event: %" GST_PTR_FORMAT, event2->structure);
        gst_pad_send_event(sink, event1);
        gst_pad_send_event(sink, event2);
    }

    /* Link sink ghostpads to identity */
    if (G_UNLIKELY(!gst_ghost_pad_set_target(GST_GHOST_PAD_CAST(self->video_sinkpad), sink)))
    {
        GST_ELEMENT_ERROR(self, CORE, PAD, (NULL), ("Failed to set video sinkpad target"));
...
out:
    /* Unblock pads */
    gst_pad_set_blocked_async_full(self->video_block_pad, FALSE, _pad_blocked_cb, gst_object_ref(self), (GDestroyNotify) gst_object_unref);

    if (self->subtitle_sink_blocked)
        gst_pad_set_blocked_async_full(self->subtitle_block_pad, FALSE, _pad_blocked_cb, gst_object_ref(self), (GDestroyNotify) gst_object_unref);

    return TRUE;
}

/* Must be called with subtitleoverlay lock! */
static void gst_subtitle_overlay_set_fps(GstSubtitleOverlay * self)
{
...
static const gchar *_get_silent_property(GstElement * element, gboolean * invert)
{
    static const struct
    {
        const gchar *name;
...
static gboolean _has_subtitle_encoding_property(GstElement * element)
{
    GParamSpec *pspec;

    pspec = g_object_class_find_property(G_OBJECT_GET_CLASS(element), "subtitle-encoding");
    return (pspec && pspec->value_type == G_TYPE_STRING);
}

static gboolean _has_font_desc_property(GstElement * element)
{
    GParamSpec *pspec;

    pspec = g_object_class_find_property(G_OBJECT_GET_CLASS(element), "font-desc");
    return (pspec && pspec->value_type == G_TYPE_STRING);
}
...
static void _pad_blocked_cb(GstPad * pad, gboolean blocked, gpointer user_data)
{
    ...
    {
        GST_ELEMENT_WARNING(self, CORE, NEGOTIATION, (NULL), ("Subtitle sink is blocked but we have no subtitle caps"));
        subcaps = NULL;
    }
    ...
    {
        GstPad *target = gst_ghost_pad_get_target(GST_GHOST_PAD_CAST(self->subtitle_sinkpad));
    ...
    if (self->subtitle_sink_blocked && !self->video_sink_blocked)
    {
        ...
        gst_pad_set_blocked_async_full(self->video_block_pad, TRUE, _pad_blocked_cb, gst_object_ref(self), (GDestroyNotify) gst_object_unref);
        goto out;
    }
    ...
    if (subcaps)
    {
        factories = gst_filter_run(self->factories, (GstFilterFunc) _filter_factories_for_caps, FALSE, subcaps);
        if (!factories)
        {
            GstMessage *msg;
            ...
            gst_element_post_message(GST_ELEMENT_CAST(self), msg);
            GST_ELEMENT_WARNING(self, CORE, MISSING_PLUGIN, (NULL), ("no suitable subtitle plugin found"));
            subcaps = NULL;
    ...
    for (l = factories; l; l = l->next)
    {
        ...
        GST_DEBUG_OBJECT(self, "Trying factory '%s'", GST_STR_NULL(gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(factory))));

        if (G_UNLIKELY((is_renderer && !_create_element(self, &self->renderer, NULL, factory, "renderer", FALSE)) || (!is_renderer && !_create_element(self, &self->parser, NULL, factory, "parser", FALSE))))
            continue;
        ...
        if (!is_renderer)
        {
            ...
            if (video_peer)
            {
                ...
                if (video_caps && gst_video_parse_caps_framerate(video_caps, &fps_n, &fps_d))
                {
            ...
            if (G_UNLIKELY(!_create_element(self, &self->overlay, "textoverlay", NULL, "overlay", FALSE)))
            {
                continue;
            }
            ...
            /* Set some properties */
            g_object_set(G_OBJECT(overlay), "halign", "center", "valign", "bottom", "wait-text", FALSE, NULL);
            if (self->font_desc)
            ...
            if (G_UNLIKELY(!_create_element(self, &self->post_colorspace, "ffmpegcolorspace", NULL, "post-colorspace", FALSE)))
            {
                continue;
            }
            ...
            if (G_UNLIKELY(!_create_element(self, &self->pre_colorspace, "ffmpegcolorspace", NULL, "pre-colorspace", FALSE)))
            {
                continue;
            }
            ...
            if (self->subtitle_segment.format != GST_FORMAT_UNDEFINED)
            {
                ...
                _generate_update_newsegment_event(&self->subtitle_segment, &event1, &event2);
                GST_DEBUG_OBJECT(self, "Pushing subtitle accumulate newsegment event: %" GST_PTR_FORMAT, event1->structure);
                GST_DEBUG_OBJECT(self, "Pushing subtitle update newsegment event: %" GST_PTR_FORMAT, event2->structure);
                gst_pad_send_event(sink, event1);
                gst_pad_send_event(sink, event2);
        ...
        {
            const gchar *name = gst_plugin_feature_get_name(GST_PLUGIN_FEATURE_CAST(factory));

            if (strcmp(name, "textoverlay") == 0)
            {
                /* Set some textoverlay specific properties */
                g_object_set(G_OBJECT(element), "halign", "center", "valign", "bottom", "wait-text", FALSE, NULL);
                if (self->font_desc)
            ...
            {
                self->silent_property = _get_silent_property(element, &self->silent_property_invert);
                if (_has_subtitle_encoding_property(self->renderer))
                    g_object_set(self->renderer, "subtitle-encoding", self->encoding, NULL);
    ...
    {
        GST_ELEMENT_WARNING(self, CORE, FAILED, (NULL), ("Failed to find any usable factories"));
        self->subtitle_error = TRUE;
        _setup_passthrough(self);
        do_async_done(self);
    }
...
static GstStateChangeReturn gst_subtitle_overlay_change_state(GstElement * element, GstStateChange transition)
{
    ...
    switch (transition)
    {
        case GST_STATE_CHANGE_NULL_TO_READY:
            ...
            gst_pad_set_blocked_async_full(self->video_block_pad, TRUE, _pad_blocked_cb, gst_object_ref(self), (GDestroyNotify) gst_object_unref);
            gst_pad_set_blocked_async_full(self->subtitle_block_pad, TRUE, _pad_blocked_cb, gst_object_ref(self), (GDestroyNotify) gst_object_unref);
            GST_SUBTITLE_OVERLAY_UNLOCK(self);
            break;
        ...
        case GST_STATE_CHANGE_READY_TO_NULL:
            {
                ...
                if (self->video_block_pad)
                {
                    ...
                    gst_pad_set_blocked_async_full(pad, FALSE, _pad_blocked_cb, gst_object_ref(self), (GDestroyNotify) gst_object_unref);
                }
...
static void gst_subtitle_overlay_handle_message(GstBin * bin, GstMessage * message)
{
    ...
    if (GST_MESSAGE_TYPE(message) == GST_MESSAGE_ERROR)
    {
        ...
        if (src && ((self->overlay
                     && gst_object_has_ancestor(src,
                                                GST_OBJECT_CAST(self->overlay))) || (self->parser && gst_object_has_ancestor(src, GST_OBJECT_CAST(self->parser))) || (self->renderer && gst_object_has_ancestor(src, GST_OBJECT_CAST(self->renderer)))))
        {
            GError *err = NULL;
            gchar *debug = NULL;
            GstMessage *wmsg;
            ...
            GST_DEBUG_OBJECT(self, "Got error message from subtitle element %s: %s (%s)", GST_MESSAGE_SRC_NAME(message), GST_STR_NULL(err->message), GST_STR_NULL(debug));
...
static void gst_subtitle_overlay_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
...
static void gst_subtitle_overlay_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    GstSubtitleOverlay *self = GST_SUBTITLE_OVERLAY_CAST(object);
...
static void gst_subtitle_overlay_base_init(gpointer g_class)
{
    GstElementClass *gstelement_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&srctemplate));

    gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&video_sinktemplate));
    gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&subtitle_sinktemplate));

    gst_element_class_set_details_simple(gstelement_class, "Subtitle Overlay", "Video/Overlay/Subtitle", "Overlays a video stream with subtitles", "Sebastian Dröge <sebastian.droege@collabora.co.uk>");
}

static void gst_subtitle_overlay_class_init(GstSubtitleOverlayClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_SILENT, g_param_spec_boolean("silent", "Silent", "Whether to show subtitles", FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_FONT_DESC,
                                    g_param_spec_string("font-desc", "Subtitle font description", "Pango font description of font " "to be used for subtitle rendering", NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    element_class->change_state = GST_DEBUG_FUNCPTR(gst_subtitle_overlay_change_state);

    bin_class->handle_message = GST_DEBUG_FUNCPTR(gst_subtitle_overlay_handle_message);
}

static GstFlowReturn gst_subtitle_overlay_src_proxy_chain(GstPad * proxypad, GstBuffer * buffer)
{
    GstPad *ghostpad;
    ...
    {
        GST_ERROR_OBJECT(self, "Downstream chain error: %s", gst_flow_get_name(ret));
...
static gboolean gst_subtitle_overlay_src_proxy_event(GstPad * proxypad, GstEvent * event)
{
    ...
    {
        GST_DEBUG_OBJECT(ghostpad, "Dropping event with marker: %" GST_PTR_FORMAT, event->structure);
...
static gboolean gst_subtitle_overlay_video_sink_setcaps(GstPad * pad, GstCaps * caps)
{
...
static gboolean gst_subtitle_overlay_video_sink_event(GstPad * pad, GstEvent * event)
{
    GstSubtitleOverlay *self = GST_SUBTITLE_OVERLAY(gst_pad_get_parent(pad));
    gboolean ret;
    ...
    {
        GST_DEBUG_OBJECT(pad, "Resetting video segment because of flush-stop event");
        gst_segment_init(&self->video_segment, GST_FORMAT_UNDEFINED);
        self->fps_n = self->fps_d = 0;
    }
    ...
    if (GST_EVENT_TYPE(event) == GST_EVENT_NEWSEGMENT)
    {
        ...
        GST_DEBUG_OBJECT(pad, "Newsegment event: %" GST_PTR_FORMAT, event->structure);
        gst_event_parse_new_segment_full(event, &update, &rate, &applied_rate, &format, &start, &stop, &position);

        if (format != GST_FORMAT_TIME)
        {
            GST_ERROR_OBJECT(pad, "Newsegment event in non-time format: %s", gst_format_get_name(format));
            gst_object_unref(event);
            gst_object_unref(self);
            return FALSE;
        }

        GST_DEBUG_OBJECT(pad, "Old video segment: %" GST_SEGMENT_FORMAT, &self->video_segment);
        gst_segment_set_newsegment_full(&self->video_segment, update, rate, applied_rate, format, start, stop, position);
        GST_DEBUG_OBJECT(pad, "New video segment: %" GST_SEGMENT_FORMAT, &self->video_segment);
    }
...
static GstFlowReturn gst_subtitle_overlay_video_sink_chain(GstPad * pad, GstBuffer * buffer)
{
    ...
    {
        GST_DEBUG_OBJECT(self, "Subtitle renderer produced chain error: %s", gst_flow_get_name(ret));
        GST_SUBTITLE_OVERLAY_LOCK(self);
        self->subtitle_error = TRUE;
        gst_pad_set_blocked_async_full(self->subtitle_block_pad, TRUE, _pad_blocked_cb, gst_object_ref(self), (GDestroyNotify) gst_object_unref);
...
static GstFlowReturn gst_subtitle_overlay_subtitle_sink_chain(GstPad * pad, GstBuffer * buffer)
{
    GstSubtitleOverlay *self = GST_SUBTITLE_OVERLAY(GST_PAD_PARENT(pad));
    ...
    else
    {
        ...
        {
            GST_DEBUG_OBJECT(self, "Subtitle chain error: %s", gst_flow_get_name(ret));
            GST_SUBTITLE_OVERLAY_LOCK(self);
            self->subtitle_error = TRUE;
            gst_pad_set_blocked_async_full(self->subtitle_block_pad, TRUE, _pad_blocked_cb, gst_object_ref(self), (GDestroyNotify) gst_object_unref);
...
static GstCaps *gst_subtitle_overlay_subtitle_sink_getcaps(GstPad * pad)
{
    GstSubtitleOverlay *self = GST_SUBTITLE_OVERLAY(gst_pad_get_parent(pad));
    GstCaps *ret;
...
static gboolean gst_subtitle_overlay_subtitle_sink_acceptcaps(GstPad * pad, GstCaps * caps)
{
...
static gboolean gst_subtitle_overlay_subtitle_sink_setcaps(GstPad * pad, GstCaps * caps)
{
    GstSubtitleOverlay *self = GST_SUBTITLE_OVERLAY(gst_pad_get_parent(pad));
    gboolean ret = TRUE;
    GstPad *target = NULL;;
    ...
    target = gst_ghost_pad_get_target(GST_GHOST_PAD_CAST(self->subtitle_sinkpad));
...
static GstPadLinkReturn gst_subtitle_overlay_subtitle_sink_link(GstPad * pad, GstPad * peer)
{
    GstSubtitleOverlay *self = GST_SUBTITLE_OVERLAY(gst_pad_get_parent(pad));
...
static void gst_subtitle_overlay_subtitle_sink_unlink(GstPad * pad)
{
    GstSubtitleOverlay *self = GST_SUBTITLE_OVERLAY(gst_object_ref(GST_PAD_PARENT(pad)));
...
static gboolean gst_subtitle_overlay_subtitle_sink_event(GstPad * pad, GstEvent * event)
{
    GstSubtitleOverlay *self = GST_SUBTITLE_OVERLAY(gst_pad_get_parent(pad));
    gboolean ret;
    GstFormat format;
    ...
    else if (GST_EVENT_TYPE(event) == GST_EVENT_NEWSEGMENT)
    {
        ...
        if (self->subtitle_segment.format != GST_FORMAT_UNDEFINED && self->subtitle_segment.format != format)
        {
            GST_DEBUG_OBJECT(pad, "Subtitle segment format changed: %s -> %s", gst_format_get_name(self->subtitle_segment.format), gst_format_get_name(format));
            gst_segment_init(&self->subtitle_segment, GST_FORMAT_UNDEFINED);
        }
    }
    ...
    switch (GST_EVENT_TYPE(event))
    {
        case GST_EVENT_FLUSH_STOP:
            GST_DEBUG_OBJECT(pad, "Resetting subtitle segment because of flush-stop");
            gst_segment_init(&self->subtitle_segment, GST_FORMAT_UNDEFINED);
            /* fall through */
        ...
        case GST_EVENT_EOS:
            ...
            event = GST_EVENT_CAST(gst_mini_object_make_writable(GST_MINI_OBJECT_CAST(event)));
            if (!event->structure)
            {
                event->structure = gst_structure_id_empty_new(_subtitle_overlay_event_marker_id);
                gst_structure_set_parent_refcount(event->structure, &event->mini_object.refcount);
            }
            gst_structure_id_set(event->structure, _subtitle_overlay_event_marker_id, G_TYPE_BOOLEAN, TRUE, NULL);
            break;
        default:
    ...
    if (GST_EVENT_TYPE(event) == GST_EVENT_NEWSEGMENT)
    {
        ...
        GST_DEBUG_OBJECT(pad, "Old subtitle segment: %" GST_SEGMENT_FORMAT, &self->subtitle_segment);
        ...
        gst_segment_set_newsegment_full(&self->subtitle_segment, update, rate, applied_rate, format, start, stop, position);
        GST_DEBUG_OBJECT(pad, "New subtitle segment: %" GST_SEGMENT_FORMAT, &self->subtitle_segment);
    }
    gst_event_unref(event);
...
static void gst_subtitle_overlay_init(GstSubtitleOverlay * self, GstSubtitleOverlayClass * klass)
{
    ...
    if (G_UNLIKELY(!it || gst_iterator_next(it, (gpointer) & proxypad) != GST_ITERATOR_OK || proxypad == NULL))
    {
    ...
    else
    {
        ...
        gst_pad_set_event_function(proxypad, GST_DEBUG_FUNCPTR(gst_subtitle_overlay_src_proxy_event));
        self->src_proxy_chain = GST_PAD_CHAINFUNC(proxypad);
        gst_pad_set_chain_function(proxypad, GST_DEBUG_FUNCPTR(gst_subtitle_overlay_src_proxy_chain));
    ...
    self->video_sinkpad = gst_ghost_pad_new_no_target_from_template("video_sink", templ);
    self->video_sink_event = GST_PAD_EVENTFUNC(self->video_sinkpad);
    gst_pad_set_event_function(self->video_sinkpad, GST_DEBUG_FUNCPTR(gst_subtitle_overlay_video_sink_event));
    self->video_sink_setcaps = GST_PAD_SETCAPSFUNC(self->video_sinkpad);
    gst_pad_set_setcaps_function(self->video_sinkpad, GST_DEBUG_FUNCPTR(gst_subtitle_overlay_video_sink_setcaps));
    self->video_sink_chain = GST_PAD_CHAINFUNC(self->video_sinkpad);
    gst_pad_set_chain_function(self->video_sinkpad, GST_DEBUG_FUNCPTR(gst_subtitle_overlay_video_sink_chain));

    proxypad = NULL;
    it = gst_pad_iterate_internal_links(self->video_sinkpad);
    if (G_UNLIKELY(!it || gst_iterator_next(it, (gpointer) & proxypad) != GST_ITERATOR_OK || proxypad == NULL))
    {
        GST_ERROR_OBJECT(self, "Failed to get internally linked pad from video sinkpad");
    }
    if (it)
        gst_iterator_free(it);
    ...
    self->subtitle_sinkpad = gst_ghost_pad_new_no_target_from_template("subtitle_sink", templ);
    self->subtitle_sink_link = GST_PAD_LINKFUNC(self->subtitle_sinkpad);
    gst_pad_set_link_function(self->subtitle_sinkpad, GST_DEBUG_FUNCPTR(gst_subtitle_overlay_subtitle_sink_link));
    self->subtitle_sink_unlink = GST_PAD_UNLINKFUNC(self->subtitle_sinkpad);
    gst_pad_set_unlink_function(self->subtitle_sinkpad, GST_DEBUG_FUNCPTR(gst_subtitle_overlay_subtitle_sink_unlink));
    self->subtitle_sink_event = GST_PAD_EVENTFUNC(self->subtitle_sinkpad);
    gst_pad_set_event_function(self->subtitle_sinkpad, GST_DEBUG_FUNCPTR(gst_subtitle_overlay_subtitle_sink_event));
    self->subtitle_sink_setcaps = GST_PAD_SETCAPSFUNC(self->subtitle_sinkpad);
    gst_pad_set_setcaps_function(self->subtitle_sinkpad, GST_DEBUG_FUNCPTR(gst_subtitle_overlay_subtitle_sink_setcaps));
    self->subtitle_sink_chain = GST_PAD_CHAINFUNC(self->subtitle_sinkpad);
    gst_pad_set_chain_function(self->subtitle_sinkpad, GST_DEBUG_FUNCPTR(gst_subtitle_overlay_subtitle_sink_chain));
    gst_pad_set_getcaps_function(self->subtitle_sinkpad, GST_DEBUG_FUNCPTR(gst_subtitle_overlay_subtitle_sink_getcaps));
    gst_pad_set_acceptcaps_function(self->subtitle_sinkpad, GST_DEBUG_FUNCPTR(gst_subtitle_overlay_subtitle_sink_acceptcaps));
    ...
    if (G_UNLIKELY(!it || gst_iterator_next(it, (gpointer) & proxypad) != GST_ITERATOR_OK || proxypad == NULL))
    {
        GST_ERROR_OBJECT(self, "Failed to get internally linked pad from subtitle sinkpad");
    }
    if (it)
        gst_iterator_free(it);
...
gboolean gst_subtitle_overlay_plugin_init(GstPlugin * plugin)
{
    ...
    return gst_element_register(plugin, "subtitleoverlay", GST_RANK_NONE, GST_TYPE_SUBTITLE_OVERLAY);
}
