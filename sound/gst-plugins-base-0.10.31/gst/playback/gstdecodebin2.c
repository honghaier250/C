...
/* generic templates */
static GstStaticPadTemplate decoder_bin_sink_template = GST_STATIC_PAD_TEMPLATE("sink",
                                                                                GST_PAD_SINK,
                                                                                GST_PAD_ALWAYS,
                                                                                GST_STATIC_CAPS_ANY);

static GstStaticPadTemplate decoder_bin_src_template = GST_STATIC_PAD_TEMPLATE("src%d",
                                                                               GST_PAD_SRC,
                                                                               GST_PAD_SOMETIMES,
                                                                               GST_STATIC_CAPS_ANY);
...
struct _GstDecodeBinClass
{
    ...
    GValueArray *(*autoplug_factories) (GstElement * element, GstPad * pad, GstCaps * caps);
    /* signal fired to sort the factories */
    GValueArray *(*autoplug_sort) (GstElement * element, GstPad * pad, GstCaps * caps, GValueArray * factories);
    /* signal fired to select from the proposed list of factories */
      GstAutoplugSelectResult(*autoplug_select) (GstElement * element, GstPad * pad, GstCaps * caps, GstElementFactory * factory);
...
static void type_found(GstElement * typefind, guint probability, GstCaps * caps, GstDecodeBin * decode_bin);

static void decodebin_set_queue_size(GstDecodeBin * dbin, GstElement * multiqueue, gboolean preroll);

static gboolean gst_decode_bin_autoplug_continue(GstElement * element, GstPad * pad, GstCaps * caps);
static GValueArray *gst_decode_bin_autoplug_factories(GstElement * element, GstPad * pad, GstCaps * caps);
static GValueArray *gst_decode_bin_autoplug_sort(GstElement * element, GstPad * pad, GstCaps * caps, GValueArray * factories);
static GstAutoplugSelectResult gst_decode_bin_autoplug_select(GstElement * element, GstPad * pad, GstCaps * caps, GstElementFactory * factory);

static void gst_decode_bin_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_decode_bin_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
...
static void caps_notify_cb(GstPad * pad, GParamSpec * unused, GstDecodeChain * chain);

static GstPad *find_sink_pad(GstElement * element);
static GstStateChangeReturn gst_decode_bin_change_state(GstElement * element, GstStateChange transition);
...
static GstDecodeChain *gst_decode_chain_new(GstDecodeBin * dbin, GstDecodeGroup * group, GstPad * pad);
static void gst_decode_group_hide(GstDecodeGroup * group);
static void gst_decode_group_free(GstDecodeGroup * group);
static GstDecodeGroup *gst_decode_group_new(GstDecodeBin * dbin, GstDecodeChain * chain);
...
static gboolean gst_decode_chain_expose(GstDecodeChain * chain, GList ** endpads, gboolean * missing_plugin);
...
static GstPad *gst_decode_group_control_demuxer_pad(GstDecodeGroup * group, GstPad * pad);
...
static GstDecodePad *gst_decode_pad_new(GstDecodeBin * dbin, GstPad * pad, GstDecodeChain * chain);
static void gst_decode_pad_activate(GstDecodePad * dpad, GstDecodeChain * chain);
...
static GType gst_decode_bin_get_type(void)
{
    ...
    if (!gst_decode_bin_type)
    {
        ...
        gst_decode_bin_type = g_type_register_static(GST_TYPE_BIN, "GstDecodeBin2", &gst_decode_bin_info, 0);
    }

    return gst_decode_bin_type;
}

static gboolean _gst_boolean_accumulator(GSignalInvocationHint * ihint, GValue * return_accu, const GValue * handler_return, gpointer dummy)
{
    gboolean myboolean;
...
static gboolean _gst_array_accumulator(GSignalInvocationHint * ihint, GValue * return_accu, const GValue * handler_return, gpointer dummy)
{
    gpointer array;
...
static gboolean _gst_select_accumulator(GSignalInvocationHint * ihint, GValue * return_accu, const GValue * handler_return, gpointer dummy)
{
...
static gboolean _gst_array_hasvalue_accumulator(GSignalInvocationHint * ihint, GValue * return_accu, const GValue * handler_return, gpointer dummy)
{
    gpointer array;
...
static void gst_decode_bin_class_init(GstDecodeBinClass * klass)
{
   ...
   */
    gst_decode_bin_signals[SIGNAL_NEW_DECODED_PAD] =
        g_signal_new("new-decoded-pad", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstDecodeBinClass, new_decoded_pad), NULL, NULL, gst_play_marshal_VOID__OBJECT_BOOLEAN, G_TYPE_NONE, 2, GST_TYPE_PAD, G_TYPE_BOOLEAN);
   ...
   */
    gst_decode_bin_signals[SIGNAL_REMOVED_DECODED_PAD] =
        g_signal_new("removed-decoded-pad", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstDecodeBinClass, removed_decoded_pad), NULL, NULL, gst_marshal_VOID__OBJECT, G_TYPE_NONE, 1, GST_TYPE_PAD);
   ...
   */
    gst_decode_bin_signals[SIGNAL_UNKNOWN_TYPE] =
        g_signal_new("unknown-type", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstDecodeBinClass, unknown_type), NULL, NULL, gst_marshal_VOID__OBJECT_BOXED, G_TYPE_NONE, 2, GST_TYPE_PAD, GST_TYPE_CAPS);
   ...
   */
    gst_decode_bin_signals[SIGNAL_AUTOPLUG_CONTINUE] =
        g_signal_new("autoplug-continue", G_TYPE_FROM_CLASS(klass),
                     G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstDecodeBinClass, autoplug_continue), _gst_boolean_accumulator, NULL, gst_play_marshal_BOOLEAN__OBJECT_BOXED, G_TYPE_BOOLEAN, 2, GST_TYPE_PAD, GST_TYPE_CAPS);
   ...
   */
    gst_decode_bin_signals[SIGNAL_AUTOPLUG_FACTORIES] =
        g_signal_new("autoplug-factories", G_TYPE_FROM_CLASS(klass),
                     G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstDecodeBinClass, autoplug_factories), _gst_array_accumulator, NULL, gst_play_marshal_BOXED__OBJECT_BOXED, G_TYPE_VALUE_ARRAY, 2, GST_TYPE_PAD, GST_TYPE_CAPS);
   ...
   */
    gst_decode_bin_signals[SIGNAL_AUTOPLUG_SORT] =
        g_signal_new("autoplug-sort", G_TYPE_FROM_CLASS(klass),
                     G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstDecodeBinClass, autoplug_sort), _gst_array_hasvalue_accumulator, NULL, gst_play_marshal_BOXED__OBJECT_BOXED_BOXED, G_TYPE_VALUE_ARRAY, 3, GST_TYPE_PAD, GST_TYPE_CAPS, G_TYPE_VALUE_ARRAY);
   ...
   */
    gst_decode_bin_signals[SIGNAL_AUTOPLUG_SELECT] =
        g_signal_new("autoplug-select", G_TYPE_FROM_CLASS(klass),
                     ...
                     _gst_select_accumulator, NULL, gst_play_marshal_ENUM__OBJECT_BOXED_OBJECT, GST_TYPE_AUTOPLUG_SELECT_RESULT, 3, GST_TYPE_PAD, GST_TYPE_CAPS, GST_TYPE_ELEMENT_FACTORY);
   ...
   * Since: 0.10.16
   */
    gst_decode_bin_signals[SIGNAL_DRAINED] = g_signal_new("drained", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstDecodeBinClass, drained), NULL, NULL, gst_marshal_VOID__VOID, G_TYPE_NONE, 0, G_TYPE_NONE);

    g_object_class_install_property(gobject_klass, PROP_CAPS, g_param_spec_boxed("caps", "Caps", "The caps on which to stop decoding.", GST_TYPE_CAPS, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    g_object_class_install_property(gobject_klass, PROP_SINK_CAPS, g_param_spec_boxed("sink-caps", "Sink Caps", "The caps of the input data. (NULL = use typefind element)", GST_TYPE_CAPS, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_object_class_install_property(gobject_klass, PROP_USE_BUFFERING,
                                    g_param_spec_boolean("use-buffering", "Use Buffering", "Emit GST_MESSAGE_BUFFERING based on low-/high-percent thresholds", DEFAULT_USE_BUFFERING, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 0.10.26
   */
    g_object_class_install_property(gobject_klass, PROP_LOW_PERCENT, g_param_spec_int("low-percent", "Low percent", "Low threshold for buffering to start", 0, 100, DEFAULT_LOW_PERCENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   ...
   * Since: 0.10.26
   */
    g_object_class_install_property(gobject_klass, PROP_HIGH_PERCENT, g_param_spec_int("high-percent", "High percent", "High threshold for buffering to finish", 0, 100, DEFAULT_HIGH_PERCENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_object_class_install_property(gobject_klass, PROP_MAX_SIZE_BYTES,
                                    g_param_spec_uint("max-size-bytes", "Max. size (bytes)", "Max. amount of bytes in the queue (0=automatic)", 0, G_MAXUINT, DEFAULT_MAX_SIZE_BYTES, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   ...
   */
    g_object_class_install_property(gobject_klass, PROP_MAX_SIZE_BUFFERS,
                                    g_param_spec_uint("max-size-buffers", "Max. size (buffers)", "Max. number of buffers in the queue (0=automatic)", 0, G_MAXUINT, DEFAULT_MAX_SIZE_BUFFERS, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   ...
   */
    g_object_class_install_property(gobject_klass, PROP_MAX_SIZE_TIME,
                                    g_param_spec_uint64("max-size-time", "Max. size (ns)", "Max. amount of data in the queue (in ns, 0=automatic)", 0, G_MAXUINT64, DEFAULT_MAX_SIZE_TIME, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_object_class_install_property(gobject_klass, PROP_POST_STREAM_TOPOLOGY,
                                    g_param_spec_boolean("post-stream-topology", "Post Stream Topology", "Post stream-topology messages", DEFAULT_POST_STREAM_TOPOLOGY, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_object_class_install_property(gobject_klass, PROP_EXPOSE_ALL_STREAMS,
                                    g_param_spec_boolean("expose-all-streams", "Expose All Streams",
                                                         "Expose all streams, including those of unknown type or that don't match the 'caps' property", DEFAULT_EXPOSE_ALL_STREAMS, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    klass->autoplug_continue = GST_DEBUG_FUNCPTR(gst_decode_bin_autoplug_continue);
    ...
    gst_element_class_add_pad_template(gstelement_klass, gst_static_pad_template_get(&decoder_bin_sink_template));
    gst_element_class_add_pad_template(gstelement_klass, gst_static_pad_template_get(&decoder_bin_src_template));

    gst_element_class_set_details_simple(gstelement_klass, "Decoder Bin", "Generic/Bin/Decoder", "Autoplug and decode to raw media", "Edward Hervey <edward.hervey@collabora.co.uk>, " "Sebastian Dröge <sebastian.droege@collabora.co.uk>");

    gstelement_klass->change_state = GST_DEBUG_FUNCPTR(gst_decode_bin_change_state);
}

/* Must be called with factories lock! */
static void gst_decode_bin_update_factories_list(GstDecodeBin * dbin)
{
    if (!dbin->factories || dbin->factories_cookie != gst_default_registry_get_feature_list_cookie())
    {
        if (dbin->factories)
            gst_plugin_feature_list_free(dbin->factories);
        dbin->factories = gst_element_factory_list_get_elements(GST_ELEMENT_FACTORY_TYPE_DECODABLE, GST_RANK_MARGINAL);
...
static void gst_decode_bin_init(GstDecodeBin * decode_bin)
{
    ...
    else
    {
         ...
         * a type */
        decode_bin->have_type_id = g_signal_connect(G_OBJECT(decode_bin->typefind), "have-type", G_CALLBACK(type_found), decode_bin);
    }
...
static void gst_decode_bin_set_sink_caps(GstDecodeBin * dbin, GstCaps * caps)
{
    GST_DEBUG_OBJECT(dbin, "Setting new caps: %" GST_PTR_FORMAT, caps);
...
static GstCaps *gst_decode_bin_get_sink_caps(GstDecodeBin * dbin)
{
    GstCaps *caps;
...
static void gst_decode_bin_set_subs_encoding(GstDecodeBin * dbin, const gchar * encoding)
{
    GList *walk;
    ...
    {
        g_object_set(G_OBJECT(walk->data), "subtitle-encoding", dbin->encoding, NULL);
    }
    SUBTITLE_UNLOCK(dbin);
}

static gchar *gst_decode_bin_get_subs_encoding(GstDecodeBin * dbin)
{
    gchar *encoding;
...
static GValueArray *gst_decode_bin_autoplug_factories(GstElement * element, GstPad * pad, GstCaps * caps)
{
    ...
    list = gst_element_factory_list_filter(dbin->factories, caps, GST_PAD_SINK, FALSE);
...
static gboolean connect_pad(GstDecodeBin * dbin, GstElement * src, GstDecodePad * dpad, GstPad * pad, GstCaps * caps, GValueArray * factories, GstDecodeChain * chain);
static gboolean connect_element(GstDecodeBin * dbin, GstElement * element, GstDecodeChain * chain);
static void expose_pad(GstDecodeBin * dbin, GstElement * src, GstDecodePad * dpad, GstPad * pad, GstCaps * caps, GstDecodeChain * chain);

static void pad_added_cb(GstElement * element, GstPad * pad, GstDecodeChain * chain);
static void pad_removed_cb(GstElement * element, GstPad * pad, GstDecodeChain * chain);
static void no_more_pads_cb(GstElement * element, GstDecodeChain * chain);

static GstDecodeGroup *gst_decode_chain_get_current_group(GstDecodeChain * chain);
 ...
 */
static void analyze_new_pad(GstDecodeBin * dbin, GstElement * src, GstPad * pad, GstCaps * caps, GstDecodeChain * chain)
{
    gboolean apcontinue = TRUE;
    GValueArray *factories = NULL, *result = NULL;
    GstDecodePad *dpad;

    GST_DEBUG_OBJECT(dbin, "Pad %s:%s caps:%" GST_PTR_FORMAT, GST_DEBUG_PAD_NAME(pad), caps);
    ...
    g_signal_emit(G_OBJECT(dbin), gst_decode_bin_signals[SIGNAL_AUTOPLUG_CONTINUE], 0, dpad, caps, &apcontinue);
     ...
     * unknown_type */
    g_signal_emit(G_OBJECT(dbin), gst_decode_bin_signals[SIGNAL_AUTOPLUG_FACTORIES], 0, dpad, caps, &factories);
    ...
    /* 1.d sort some more. */
    g_signal_emit(G_OBJECT(dbin), gst_decode_bin_signals[SIGNAL_AUTOPLUG_SORT], 0, dpad, caps, factories, &result);
    g_value_array_free(factories);
    factories = result;
    ...
    if (!dbin->expose_allstreams)
    {
        ...
        {
            GstElementFactory *factory = g_value_get_object(g_value_array_get_nth(factories, 0));
            GstCaps *tcaps;
            ...
            if (strstr(gst_element_factory_get_klass(factory), "Decoder"))
            {
                 ...
                 * our final caps*/
                for (tmps = gst_element_factory_get_static_pad_templates(factory); tmps && !dontuse; tmps = tmps->next)
                {
...
unknown_type:
    {
        ...
        gst_element_post_message(GST_ELEMENT_CAST(dbin), gst_missing_decoder_message_new(GST_ELEMENT_CAST(dbin), caps));

        g_signal_emit(G_OBJECT(dbin), gst_decode_bin_signals[SIGNAL_UNKNOWN_TYPE], 0, pad, caps);
        ...
        if (src == dbin->typefind)
        {
            ...
            if (caps && !gst_caps_is_empty(caps))
            {
                ...
                GST_ELEMENT_ERROR(dbin, STREAM, CODEC_NOT_FOUND, (_("A %s plugin is required to play this stream, " "but not installed."), desc), ("No decoder to handle media type '%s'", gst_structure_get_name(gst_caps_get_structure(caps, 0))));
                g_free(desc);
            }
            else
            {
                GST_ELEMENT_ERROR(dbin, STREAM, TYPE_NOT_FOUND, (_("Could not determine type of stream")), ("Stream caps %" GST_PTR_FORMAT, caps));
            }
            do_async_done(dbin);
        }
        return;
...
any_caps:
    {
        GST_WARNING_OBJECT(pad, "pad has ANY caps, not able to autoplug to anything");
        goto setup_caps_delay;
    }
setup_caps_delay:
    {
        ...
        GST_LOG_OBJECT(dbin, "Chain %p has now %d dynamic pads", chain, g_list_length(chain->pending_pads));
        ppad = g_slice_new0(GstPendingPad);
        ppad->pad = gst_object_ref(pad);
        ppad->chain = chain;
        ppad->event_probe_id = gst_pad_add_event_probe(pad, (GCallback) pad_event_cb, ppad);
        chain->pending_pads = g_list_prepend(chain->pending_pads, ppad);
        CHAIN_MUTEX_UNLOCK(chain);
        g_signal_connect(G_OBJECT(pad), "notify::caps", G_CALLBACK(caps_notify_cb), chain);
        return;
    }
}
...
static gboolean connect_pad(GstDecodeBin * dbin, GstElement * src, GstDecodePad * dpad, GstPad * pad, GstCaps * caps, GValueArray * factories, GstDecodeChain * chain)
{
    ...
    GST_DEBUG_OBJECT(dbin, "pad %s:%s , chain:%p", GST_DEBUG_PAD_NAME(pad), chain);

    /* 1. is element demuxer or parser */
    if (is_demuxer)
    {
        GST_LOG_OBJECT(src, "is a demuxer, connecting the pad through multiqueue '%s'", GST_OBJECT_NAME(chain->parent->multiqueue));
    ...
    while (factories->n_values > 0)
    {
        ...
        g_signal_emit(G_OBJECT(dbin), gst_decode_bin_signals[SIGNAL_AUTOPLUG_SELECT], 0, dpad, caps, factory, &ret);
        ...
        {
            GST_WARNING_OBJECT(dbin, "Could not create an element from %s", gst_plugin_feature_get_name(GST_PLUGIN_FEATURE(factory)));
            continue;
        }
        ...
        {
            GST_WARNING_OBJECT(dbin, "Couldn't set %s to READY", GST_ELEMENT_NAME(element));
        ...
        {
            GST_WARNING_OBJECT(dbin, "Element %s doesn't have a sink pad", GST_ELEMENT_NAME(element));
        ...
        {
            GST_WARNING_OBJECT(dbin, "Couldn't add %s to the bin", GST_ELEMENT_NAME(element));
        ...
        {
            GST_WARNING_OBJECT(dbin, "Link failed on pad %s:%s", GST_DEBUG_PAD_NAME(sinkpad));
            gst_element_set_state(element, GST_STATE_NULL);
            gst_object_unref(sinkpad);
        ...
        CHAIN_MUTEX_LOCK(chain);
        chain->elements = g_list_prepend(chain->elements, gst_object_ref(element));
        ...
        {
            SUBTITLE_LOCK(dbin);
            GST_DEBUG_OBJECT(dbin, "setting subtitle-encoding=%s to element", dbin->encoding);
            g_object_set(G_OBJECT(element), "subtitle-encoding", dbin->encoding, NULL);
            SUBTITLE_UNLOCK(dbin);
            subtitle = TRUE;
        }
        else
        ...
        if ((gst_element_set_state(element, GST_STATE_PAUSED)) == GST_STATE_CHANGE_FAILURE)
        {
            ...
            GST_WARNING_OBJECT(dbin, "Couldn't set %s to PAUSED", GST_ELEMENT_NAME(element));
...
static gboolean connect_element(GstDecodeBin * dbin, GstElement * element, GstDecodeChain * chain)
{
    ...
    GST_DEBUG_OBJECT(dbin, "Attempting to connect element %s [chain:%p] further", GST_ELEMENT_NAME(element), chain);

    /* 1. Loop over pad templates, grabbing existing pads along the way */
    for (pads = GST_ELEMENT_GET_CLASS(element)->padtemplates; pads; pads = g_list_next(pads))
    {
            ...
            case GST_PAD_ALWAYS:
                {
                    ...
                    if (pad)
                    {
                        GST_DEBUG_OBJECT(dbin, "got the pad for always template %s", templ_name);
                    ...
                    else
                    {
                        ...
                        GST_WARNING_OBJECT(dbin, "could not get the pad for always template %s", templ_name);
                    }
                    break;
                }
            case GST_PAD_SOMETIMES:
                {
                    ...
                    if (pad)
                    {
                        GST_DEBUG_OBJECT(dbin, "got the pad for sometimes template %s", templ_name);
                        /* the pad is created, we need to autoplug it */
                        to_connect = g_list_prepend(to_connect, pad);
                    }
                    else
                    {
                        GST_DEBUG_OBJECT(dbin, "did not get the sometimes pad of template %s", templ_name);
    ...
    if (dynamic)
    {
        GST_LOG_OBJECT(dbin, "Adding signals to element %s in chain %p", GST_ELEMENT_NAME(element), chain);
        g_signal_connect(G_OBJECT(element), "pad-added", G_CALLBACK(pad_added_cb), chain);
        g_signal_connect(G_OBJECT(element), "pad-removed", G_CALLBACK(pad_removed_cb), chain);
        g_signal_connect(G_OBJECT(element), "no-more-pads", G_CALLBACK(no_more_pads_cb), chain);
    }
...
static void type_found(GstElement * typefind, guint probability, GstCaps * caps, GstDecodeBin * decode_bin)
{
    ...
    {
        GST_ELEMENT_ERROR(decode_bin, STREAM, WRONG_TYPE, (_("This appears to be a text file")), ("decodebin2 cannot decode plain text files"));
        goto exit;
    }
...
static gboolean pad_event_cb(GstPad * pad, GstEvent * event, gpointer data)
{
    ...
    {
        case GST_EVENT_EOS:
            GST_DEBUG_OBJECT(dbin, "Received EOS on a non final pad, this stream " "ended too early");
            chain->deadend = TRUE;
...
static void pad_added_cb(GstElement * element, GstPad * pad, GstDecodeChain * chain)
{
    ...
    if (gst_decode_chain_is_complete(dbin->decode_chain))
    {
        GST_LOG_OBJECT(dbin, "That was the last dynamic object, now attempting to expose the group");
...
static void no_more_pads_cb(GstElement * element, GstDecodeChain * chain)
{
    ...
    {
        GST_LOG_OBJECT(chain->dbin, "no-more-pads from old chain element '%s'", GST_OBJECT_NAME(element));
        CHAIN_MUTEX_UNLOCK(chain);
        return;
    }
    else if (!chain->demuxer)
    {
        GST_LOG_OBJECT(chain->dbin, "no-more-pads from a non-demuxer element '%s'", GST_OBJECT_NAME(element));
        CHAIN_MUTEX_UNLOCK(chain);
        return;
    }
    ...
    GST_DEBUG_OBJECT(group->dbin, "Setting group %p multiqueue to " "'playing' buffering mode", group);
...
static GstDecodeGroup *gst_decode_chain_get_current_group(GstDecodeChain * chain)
{
    ...
    if (!chain->next_groups && chain->active_group && chain->active_group->overrun && !chain->active_group->no_more_pads)
    {
        GST_WARNING_OBJECT(chain->dbin, "Currently active group %p is exposed" " and wants to add a new pad without having signaled no-more-pads", chain->active_group);
        return NULL;
    }

    if (chain->next_groups && (group = chain->next_groups->data) && group->overrun && !group->no_more_pads)
    {
        GST_WARNING_OBJECT(chain->dbin, "Currently newest pending group %p " "had overflow but didn't signal no-more-pads", group);
        return NULL;
    }
    ...
    else if (!chain->active_group->overrun && !chain->active_group->no_more_pads)
    {
        group = chain->active_group;
    }
    else if (chain->next_groups && (group = chain->next_groups->data) && !group->overrun && !group->no_more_pads)
    {
        /* group = chain->next_groups->data */
    }
    else
...
static void gst_decode_group_free_internal(GstDecodeGroup * group, gboolean hide);

static void gst_decode_chain_free_internal(GstDecodeChain * chain, gboolean hide)
{
    GList *l;

    CHAIN_MUTEX_LOCK(chain);

    GST_DEBUG_OBJECT(chain->dbin, "%s chain %p", (hide ? "Hiding" : "Freeing"), chain);
    ...
    if (chain->endpad)
    {
        if (chain->endpad->exposed)
            gst_element_remove_pad(GST_ELEMENT_CAST(chain->dbin), GST_PAD_CAST(chain->endpad));
    ...
    GST_DEBUG_OBJECT(chain->dbin, "%s chain %p", (hide ? "Hidden" : "Freed"), chain);
    CHAIN_MUTEX_UNLOCK(chain);
    if (!hide)
    {
 ...
 * a group!
 */
static GstDecodeChain *gst_decode_chain_new(GstDecodeBin * dbin, GstDecodeGroup * parent, GstPad * pad)
{
    GstDecodeChain *chain = g_slice_new0(GstDecodeChain);

    GST_DEBUG_OBJECT(dbin, "Creating new chain %p with parent group %p", chain, parent);
...
static void multi_queue_overrun_cb(GstElement * queue, GstDecodeGroup * group)
{
    ...
    GST_LOG_OBJECT(dbin, "multiqueue '%s' (%p) is full", GST_OBJECT_NAME(queue), queue);
...
static void gst_decode_group_free_internal(GstDecodeGroup * group, gboolean hide)
{
    ...
    GST_DEBUG_OBJECT(group->dbin, "%s group %p", (hide ? "Hiding" : "Freeing"), group);
    ...
    GST_DEBUG_OBJECT(group->dbin, "%s group %p", (hide ? "Hided" : "Freed"), group);
    if (!hide)
...
static void decodebin_set_queue_size(GstDecodeBin * dbin, GstElement * multiqueue, gboolean preroll)
{
    ...
    g_object_set(multiqueue, "max-size-bytes", max_bytes, "max-size-time", max_time, "max-size-buffers", max_buffers, NULL);
}
 ...
 * of groups.
 */
static GstDecodeGroup *gst_decode_group_new(GstDecodeBin * dbin, GstDecodeChain * parent)
{
    GstDecodeGroup *group = g_slice_new0(GstDecodeGroup);
    GstElement *mq;

    GST_DEBUG_OBJECT(dbin, "Creating new group %p with parent chain %p", group, parent);
    ...
    group->overrunsig = g_signal_connect(G_OBJECT(mq), "overrun", G_CALLBACK(multi_queue_overrun_cb), group);
...
missing_multiqueue:
    {
        gst_element_post_message(GST_ELEMENT_CAST(dbin), gst_missing_element_message_new(GST_ELEMENT_CAST(dbin), "multiqueue"));
...
static GstPad *gst_decode_group_control_demuxer_pad(GstDecodeGroup * group, GstPad * pad)
{
    ...
    if (!it || (gst_iterator_next(it, (gpointer) & srcpad)) != GST_ITERATOR_OK || srcpad == NULL)
    {
        GST_ERROR_OBJECT(dbin, "Couldn't get srcpad from multiqueue for sinkpad %" GST_PTR_FORMAT, sinkpad);
        goto error;
    }
...
static gboolean gst_decode_chain_is_complete(GstDecodeChain * chain)
{
    ...
    if (chain->demuxer)
    {
        if (chain->active_group && gst_decode_group_is_complete(chain->active_group))
        {
            complete = TRUE;
            goto out;
        }
 ...
 * event.  */
static void gst_decode_pad_handle_eos(GstDecodePad * pad)
{
 ...
 */
static void gst_decode_chain_handle_eos(GstDecodeChain * eos_chain)
{
    ...
    drained = chain->active_group ? gst_decode_group_is_drained(chain->active_group) : TRUE;
    ...
    if (chain->active_group && drained && chain->next_groups)
    {
        ...
        GST_DEBUG_OBJECT(dbin, "Switching to next group %p", chain->next_groups->data);
        chain->active_group = chain->next_groups->data;
        chain->next_groups = g_list_delete_link(chain->next_groups, chain->next_groups);
        CHAIN_MUTEX_UNLOCK(chain);
        EXPOSE_LOCK(dbin);
        if (gst_decode_chain_is_complete(dbin->decode_chain))
            gst_decode_bin_expose(dbin);
    ...
    else if (!chain->active_group || drained)
    {
        ...
        g_signal_emit(G_OBJECT(dbin), gst_decode_bin_signals[SIGNAL_DRAINED], 0, NULL);
    }
    else
    {
        CHAIN_MUTEX_UNLOCK(chain);
        GST_DEBUG_OBJECT(dbin, "Current active group in chain %p is not drained yet", chain);
    }
}
...
static gboolean gst_decode_chain_is_drained(GstDecodeChain * chain)
{
    ...
    if (chain->active_group && gst_decode_group_is_drained(chain->active_group) && !chain->next_groups)
    {
        drained = TRUE;
        goto out;
    }
...
static GstCaps *_gst_element_get_linked_caps(GstElement * src, GstElement * sink)
{
    GstIterator *it;
    GstElement *parent;
    GstPad *pad, *peer;
 ...
 */
static GstStructure *gst_decode_chain_get_topology(GstDecodeChain * chain)
{
    GstStructure *s, *u;
    GList *l;
    GstCaps *caps;

    if (G_UNLIKELY((chain->endpad || chain->deadend) && (chain->endcaps == NULL)))
    {
    ...
    if (chain->elements && (chain->endpad || chain->deadend))
    {
        ...
        gst_structure_id_set(u, topology_caps, GST_TYPE_CAPS, chain->endcaps, NULL);
    ...
    if (!caps)
    {
        ...
        {
            GST_ERROR_OBJECT(chain->pad, "Couldn't get fixed caps, got %" GST_PTR_FORMAT, caps);
            gst_caps_unref(caps);
            caps = NULL;
        }
    }
...
static void gst_decode_bin_post_topology_message(GstDecodeBin * dbin)
{
    GstStructure *s;
    GstMessage *msg;
...
static gboolean gst_decode_bin_expose(GstDecodeBin * dbin)
{
    ...
    if (endpads == NULL)
    {
        if (missing_plugin)
        {
            ...
            GST_ELEMENT_ERROR(dbin, CORE, MISSING_PLUGIN, (NULL), ("no suitable plugins found"));
        }
        else
        {
            ...
            GST_ELEMENT_ERROR(dbin, STREAM, FAILED, (NULL), ("all streams without buffers"));
        }
        return FALSE;
    }
    ...
    for (tmp = endpads; tmp; tmp = tmp->next)
    {
        ...
        dbin->nbpads++;
        GST_DEBUG_OBJECT(dbin, "About to expose dpad %s as %s", GST_OBJECT_NAME(dpad), padname);
        gst_object_set_name(GST_OBJECT(dpad), padname);
        g_free(padname);

        /* 2. activate and add */
        if (!dpad->exposed && !gst_element_add_pad(GST_ELEMENT(dbin), GST_PAD_CAST(dpad)))
        {
        ...
        g_signal_emit(G_OBJECT(dbin), gst_decode_bin_signals[SIGNAL_NEW_DECODED_PAD], 0, dpad, (tmp->next == NULL));
...
static void source_pad_blocked_cb(GstPad * pad, gboolean blocked, GstDecodePad * dpad)
{
    GstDecodeChain *chain;
    GstDecodeBin *dbin;
...
static gboolean source_pad_event_probe(GstPad * pad, GstEvent * event, GstDecodePad * dpad)
{
...
static void gst_decode_pad_set_blocked(GstDecodePad * dpad, gboolean blocked)
{
    ...
    if (!blocked || !dbin->shutdown)
        gst_pad_set_blocked_async_full(opad, blocked, (GstPadBlockCallback) source_pad_blocked_cb, gst_object_ref(dpad), (GDestroyNotify) gst_object_unref);
...
{
    gst_pad_add_event_probe(GST_PAD_CAST(dpad), G_CALLBACK(source_pad_event_probe), dpad);
}
...
static GstDecodePad *gst_decode_pad_new(GstDecodeBin * dbin, GstPad * pad, GstDecodeChain * chain)
{
    ...
    dpad = g_object_new(GST_TYPE_DECODE_PAD, "direction", GST_PAD_DIRECTION(pad), "template", pad_tmpl, NULL);
...
static void unblock_pads(GstDecodeBin * dbin)
{
    ...
    for (tmp = dbin->blocked_pads; tmp; tmp = tmp->next)
    {
        ...
        GST_DEBUG_OBJECT(dpad, "unblocking");
        gst_pad_set_blocked_async_full(opad, FALSE, (GstPadBlockCallback) source_pad_blocked_cb, gst_object_ref(dpad), (GDestroyNotify) gst_object_unref);
...
{
    GST_DEBUG_CATEGORY_INIT(gst_decode_bin_debug, "decodebin2", 0, "decoder bin");
    ...
    return gst_element_register(plugin, "decodebin2", GST_RANK_NONE, GST_TYPE_DECODE_BIN);
}
