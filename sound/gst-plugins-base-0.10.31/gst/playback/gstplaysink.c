...
static GstStaticPadTemplate audiorawtemplate = GST_STATIC_PAD_TEMPLATE("audio_raw_sink",
                                                                       GST_PAD_SINK,
                                                                       GST_PAD_REQUEST,
                                                                       GST_STATIC_CAPS_ANY);
static GstStaticPadTemplate audiotemplate = GST_STATIC_PAD_TEMPLATE("audio_sink",
                                                                    GST_PAD_SINK,
                                                                    GST_PAD_REQUEST,
                                                                    GST_STATIC_CAPS_ANY);
static GstStaticPadTemplate videorawtemplate = GST_STATIC_PAD_TEMPLATE("video_raw_sink",
                                                                       GST_PAD_SINK,
                                                                       GST_PAD_REQUEST,
                                                                       GST_STATIC_CAPS_ANY);
static GstStaticPadTemplate videotemplate = GST_STATIC_PAD_TEMPLATE("video_sink",
                                                                    GST_PAD_SINK,
                                                                    GST_PAD_REQUEST,
                                                                    GST_STATIC_CAPS_ANY);
...
static void gst_play_sink_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * spec);
static void gst_play_sink_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * spec);

static GstPad *gst_play_sink_request_new_pad(GstElement * element, GstPadTemplate * templ, const gchar * name);
static void gst_play_sink_release_request_pad(GstElement * element, GstPad * pad);
static gboolean gst_play_sink_send_event(GstElement * element, GstEvent * event);
static GstStateChangeReturn gst_play_sink_change_state(GstElement * element, GstStateChange transition);

static void gst_play_sink_handle_message(GstBin * bin, GstMessage * message);

static void notify_volume_cb(GObject * object, GParamSpec * pspec, GstPlaySink * playsink);
static void notify_mute_cb(GObject * object, GParamSpec * pspec, GstPlaySink * playsink);

static void update_av_offset(GstPlaySink * playsink);

void gst_play_marshal_BUFFER__BOXED(GClosure * closure, GValue * return_value G_GNUC_UNUSED, guint n_param_values, const GValue * param_values, gpointer invocation_hint G_GNUC_UNUSED, gpointer marshal_data)
{
    typedef GstBuffer *(*GMarshalFunc_OBJECT__BOXED) (gpointer data1, gpointer arg_1, gpointer data2);
...
static void gst_play_sink_class_init(GstPlaySinkClass * klass)
{
   ...
   */
    ...
    g_signal_new("reconfigure", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, G_STRUCT_OFFSET(GstPlaySinkClass, reconfigure), NULL, NULL, gst_marshal_BOOLEAN__VOID, G_TYPE_BOOLEAN, 0, G_TYPE_NONE);
  /**
   ...
   * Since: 0.10.30
   */
    g_signal_new("convert-frame", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, G_STRUCT_OFFSET(GstPlaySinkClass, convert_frame), NULL, NULL, gst_play_marshal_BUFFER__BOXED, GST_TYPE_BUFFER, 1, GST_TYPE_CAPS);

    gst_element_class_add_pad_template(gstelement_klass, gst_static_pad_template_get(&audiorawtemplate));
    gst_element_class_add_pad_template(gstelement_klass, gst_static_pad_template_get(&audiotemplate));
    gst_element_class_add_pad_template(gstelement_klass, gst_static_pad_template_get(&videorawtemplate));
    gst_element_class_add_pad_template(gstelement_klass, gst_static_pad_template_get(&videotemplate));
    gst_element_class_add_pad_template(gstelement_klass, gst_static_pad_template_get(&texttemplate));
    gst_element_class_set_details_simple(gstelement_klass, "Player Sink", "Generic/Bin/Sink", "Convenience sink for multiple streams", "Wim Taymans <wim.taymans@gmail.com>");

    gstelement_klass->change_state = GST_DEBUG_FUNCPTR(gst_play_sink_change_state);
    gstelement_klass->send_event = GST_DEBUG_FUNCPTR(gst_play_sink_send_event);
    gstelement_klass->request_new_pad = GST_DEBUG_FUNCPTR(gst_play_sink_request_new_pad);
    gstelement_klass->release_pad = GST_DEBUG_FUNCPTR(gst_play_sink_release_request_pad);

    gstbin_klass->handle_message = GST_DEBUG_FUNCPTR(gst_play_sink_handle_message);
...
static void gst_play_sink_init(GstPlaySink * playsink)
{
    ...
    playsink->stream_synchronizer = g_object_new(GST_TYPE_STREAM_SYNCHRONIZER, NULL);
    gst_bin_add(GST_BIN_CAST(playsink), GST_ELEMENT_CAST(playsink->stream_synchronizer));

    g_static_rec_mutex_init(&playsink->lock);
    GST_OBJECT_FLAG_SET(playsink, GST_ELEMENT_IS_SINK);
}

static void disconnect_chain(GstPlayAudioChain * chain, GstPlaySink * playsink)
{
    if (chain)
    {
        if (chain->volume)
            g_signal_handlers_disconnect_by_func(chain->volume, notify_volume_cb, playsink);
        if (chain->mute)
            g_signal_handlers_disconnect_by_func(chain->mute, notify_mute_cb, playsink);
    }
}

static void free_chain(GstPlayChain * chain)
{
    if (chain)
    {
        if (chain->bin)
...
static void gst_play_sink_dispose(GObject * object)
{
    ...
    {
        gst_element_release_request_pad(playsink->audio_tee, playsink->audio_tee_vissrc);
    ...
    {
        gst_element_release_request_pad(playsink->audio_tee, playsink->audio_tee_asrc);
...
static void gst_play_sink_vis_unblocked(GstPad * tee_pad, gboolean blocked, gpointer user_data)
{
    GstPlaySink *playsink;
...
static void gst_play_sink_vis_blocked(GstPad * tee_pad, gboolean blocked, gpointer user_data)
{
    GstPlaySink *playsink;
    ...
    /* link pads */
    gst_pad_link_full(chain->blockpad, chain->vissinkpad, GST_PAD_LINK_CHECK_NOTHING);
    gst_ghost_pad_set_target(GST_GHOST_PAD_CAST(chain->srcpad), chain->vissrcpad);

done:
    /* Unblock the pad */
    gst_pad_set_blocked_async(tee_pad, FALSE, gst_play_sink_vis_unblocked, playsink);
    GST_PLAY_SINK_UNLOCK(playsink);
}
...
void gst_play_sink_set_vis_plugin(GstPlaySink * playsink, GstElement * vis)
{
    ...
    gst_pad_set_blocked_async(chain->blockpad, TRUE, gst_play_sink_vis_blocked, playsink);
done:
    GST_PLAY_SINK_UNLOCK(playsink);
...
void gst_play_sink_set_volume(GstPlaySink * playsink, gdouble volume)
{
    ...
    {
        GST_LOG_OBJECT(playsink, "elements: volume=%" GST_PTR_FORMAT ", mute=%" GST_PTR_FORMAT "; new volume=%.03f, mute=%d", chain->volume, chain->mute, volume, playsink->mute);
    ...
    if (chain)
    {
        ...
        else if (chain->volume)
        {
            if (mute)
                ...
                g_object_set(chain->volume, "volume", (gdouble) playsink->volume, NULL);
        }
    }
    else
    {
...
static void post_missing_element_message(GstPlaySink * playsink, const gchar * name)
{
    GstMessage *msg;
...
static gboolean add_chain(GstPlayChain * chain, gboolean add)
{
    if (chain->added == add)
        return TRUE;
...
static gboolean activate_chain(GstPlayChain * chain, gboolean activate)
{
    GstState state;
...
static gboolean element_is_sink(GstElement * element)
{
    gboolean is_sink;
...
static gboolean element_has_property(GstElement * element, const gchar * pname, GType type)
{
    GParamSpec *pspec;
    ...
    if (type == G_TYPE_INVALID || type == pspec->value_type || g_type_is_a(pspec->value_type, type))
    {
        GST_DEBUG_OBJECT(element, "has %s property of type %s", pname, (type == G_TYPE_INVALID) ? "any type" : g_type_name(type));
        return TRUE;
    }

    GST_WARNING_OBJECT(element, "has %s property, but property is of type %s " "and we expected it to be of type %s", pname, g_type_name(pspec->value_type), g_type_name(type));
...
static gint find_property(GstElement * element, FindPropertyHelper * helper)
{
    ...
    GST_INFO_OBJECT(element, "found %s with %s property", helper->prop_name, (helper->need_sink) ? "sink" : "element");
    return 0;                    /* keep it */
}
...
static GstElement *gst_play_sink_find_property_sinks(GstPlaySink * playsink, GstElement * obj, const gchar * name, GType expected_type)
{
    GstElement *result = NULL;
    GstIterator *it;
    ...
    else if (GST_IS_BIN(obj))
    {
        ...
        result = gst_iterator_find_custom(it, (GCompareFunc) find_property, &helper);
...
static GstElement *gst_play_sink_find_property(GstPlaySink * playsink, GstElement * obj, const gchar * name, GType expected_type)
{
    GstElement *result = NULL;
    GstIterator *it;
...
static void do_async_start(GstPlaySink * playsink)
{
    GstMessage *message;
    ...
    GST_BIN_CLASS(gst_play_sink_parent_class)->handle_message(GST_BIN_CAST(playsink), message);
}

static void do_async_done(GstPlaySink * playsink)
{
    GstMessage *message;
...
static GstElement *try_element(GstPlaySink * playsink, GstElement * element, gboolean unref)
{
 ...
 *
 */
static GstPlayVideoDeinterlaceChain *gen_video_deinterlace_chain(GstPlaySink * playsink)
{
    ...
    if (chain->conv == NULL)
    {
        ...
        GST_ELEMENT_WARNING(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "ffmpegcolorspace"), ("video rendering might fail"));
    }
    else
    {
    ...
    if (chain->deinterlace == NULL)
    {
        ...
        GST_ELEMENT_WARNING(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "deinterlace"), ("deinterlacing won't work"));
    }
    else
    {
        ...
        if (prev)
        {
            if (!gst_element_link_pads_full(prev, "src", chain->deinterlace, "sink", GST_PAD_LINK_CHECK_TEMPLATE_CAPS))
                goto link_failed;
        }
        else
        {
...
link_failed:
    {
        GST_ELEMENT_ERROR(playsink, CORE, PAD, (NULL), ("Failed to configure the video deinterlace chain."));
 ...
 *
 */
static GstPlayVideoChain *gen_video_chain(GstPlaySink * playsink, gboolean raw, gboolean async)
{
    GstPlayVideoChain *chain;
    GstBin *bin;
    GstPad *pad;
    ...
    elem = gst_play_sink_find_property_sinks(playsink, chain->sink, "async", G_TYPE_BOOLEAN);
    if (elem)
    {
        GST_DEBUG_OBJECT(playsink, "setting async property to %d on element %s", async, GST_ELEMENT_NAME(elem));
    ...
    chain->ts_offset = gst_play_sink_find_property_sinks(playsink, chain->sink, "ts-offset", G_TYPE_INT64);
    ...
    if (chain->queue == NULL)
    {
        ...
        GST_ELEMENT_WARNING(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "queue"), ("video rendering might be suboptimal"));
        head = chain->sink;
        prev = NULL;
    }
    else
    {
        g_object_set(G_OBJECT(chain->queue), "max-size-buffers", 3, "max-size-bytes", 0, "max-size-time", (gint64) 0, "silent", TRUE, NULL);
    ...
    if (raw && !(playsink->flags & GST_PLAY_FLAG_NATIVE_VIDEO))
    {
        ...
        else
        {
            ...
            if (prev)
            {
                if (!gst_element_link_pads_full(prev, "src", chain->conv, "sink", GST_PAD_LINK_CHECK_TEMPLATE_CAPS))
                    goto link_failed;
            }
            else
            {
        ...
        if (chain->scale == NULL)
        {
            ...
            GST_ELEMENT_WARNING(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "videoscale"), ("possibly a liboil version mismatch?"));
        }
        else
        {
            ...
            if (prev)
            {
                if (!gst_element_link_pads_full(prev, "src", chain->scale, "sink", GST_PAD_LINK_CHECK_TEMPLATE_CAPS))
                    goto link_failed;
            }
            else
            {
    ...
    if (prev)
    {
        ...
        if (!gst_element_link_pads_full(prev, "src", chain->sink, NULL, GST_PAD_LINK_CHECK_TEMPLATE_CAPS))
            goto link_failed;
    }
...
no_sinks:
    {
        if (!elem && !playsink->video_sink)
        {
            ...
            if (strcmp(DEFAULT_VIDEOSINK, "autovideosink"))
            {
                ...
                GST_ELEMENT_ERROR(playsink, CORE, MISSING_PLUGIN, (_("Both autovideosink and %s elements are missing."), DEFAULT_VIDEOSINK), (NULL));
            }
            else
            {
                GST_ELEMENT_ERROR(playsink, CORE, MISSING_PLUGIN, (_("The autovideosink element is missing.")), (NULL));
            }
        }
        else
        {
            if (playsink->video_sink)
            {
                GST_ELEMENT_ERROR(playsink, CORE, STATE_CHANGE, (_("Configured videosink %s is not working."), GST_ELEMENT_NAME(playsink->video_sink)), (NULL));
            }
            else if (strcmp(DEFAULT_VIDEOSINK, "autovideosink"))
            {
                GST_ELEMENT_ERROR(playsink, CORE, STATE_CHANGE, (_("Both autovideosink and %s elements are not working."), DEFAULT_VIDEOSINK), (NULL));
            }
            else
            {
                GST_ELEMENT_ERROR(playsink, CORE, STATE_CHANGE, (_("The autovideosink element is not working.")), (NULL));
            }
        }
        free_chain((GstPlayChain *) chain);
        return NULL;
...
link_failed:
    {
        GST_ELEMENT_ERROR(playsink, CORE, PAD, (NULL), ("Failed to configure the video sink."));
...
static gboolean setup_video_chain(GstPlaySink * playsink, gboolean raw, gboolean async)
{
    GstElement *elem;
    GstPlayVideoChain *chain;
    GstStateChangeReturn ret;
 ...
 *
 */
static GstPlayTextChain *gen_text_chain(GstPlaySink * playsink)
{
    GstPlayTextChain *chain;
    GstBin *bin;
    GstElement *elem;
    ...
    if (playsink->text_sink)
    {
        ...
        if (chain->sink)
        {
            ...
            if (elem)
            {
                ...
                {
                    post_missing_element_message(playsink, "queue");
                    GST_ELEMENT_WARNING(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "queue"), ("rendering might be suboptimal"));
                }
                else
                {
                    g_object_set(G_OBJECT(chain->queue), "max-size-buffers", 3, "max-size-bytes", 0, "max-size-time", (gint64) 0, "silent", TRUE, NULL);
                ...
                if (gst_element_link_pads_full(chain->queue, "src", chain->sink, "sink", GST_PAD_LINK_CHECK_TEMPLATE_CAPS))
                {
                ...
                {
                    GST_WARNING_OBJECT(playsink, "can't find a sink pad on custom text sink");
                ...
                if ((elem = gst_play_sink_find_property_sinks(playsink, chain->sink, "sync", G_TYPE_BOOLEAN)))
                    g_object_set(elem, "sync", TRUE, NULL);
            }
            else
            {
                GST_WARNING_OBJECT(playsink, "can't find async property in custom text sink");
            }
        }
        if (textsinkpad == NULL)
        {
            GST_ELEMENT_WARNING(playsink, CORE, MISSING_PLUGIN, (_("Custom text sink element is not usable.")), ("fallback to default textoverlay"));
        }
    }
    ...
    if (textsinkpad == NULL)
    {
        if (!(playsink->flags & GST_PLAY_FLAG_NATIVE_VIDEO))
        {
            ...
            chain->overlay = gst_element_factory_make("subtitleoverlay", "suboverlay");
            if (chain->overlay == NULL)
            {
                post_missing_element_message(playsink, "subtitleoverlay");
                GST_ELEMENT_WARNING(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "subtitleoverlay"), ("subtitle rendering disabled"));
            }
            else
            {
                GstElement *element;
                ...
                {
                    g_object_set(G_OBJECT(chain->overlay), "font-desc", playsink->font_desc, NULL);
                }
                if (playsink->subtitle_encoding)
                {
                    g_object_set(G_OBJECT(chain->overlay), "subtitle-encoding", playsink->subtitle_encoding, NULL);
                }

                gst_element_link_pads_full(chain->queue, "src", chain->overlay, "video_sink", GST_PAD_LINK_CHECK_TEMPLATE_CAPS);
                ...
                {
                    g_object_set(G_OBJECT(element), "max-size-buffers", 3, "max-size-bytes", 0, "max-size-time", (gint64) 0, "silent", TRUE, NULL);
                    gst_bin_add(bin, element);
                    gst_element_link_pads_full(element, "src", chain->overlay, "subtitle_sink", GST_PAD_LINK_CHECK_TEMPLATE_CAPS);
    ...
    if (videosinkpad == NULL)
    {
        ...
        if (chain->identity == NULL)
        {
            ...
            GST_ELEMENT_ERROR(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "identity"), (NULL));
        }
        else
        {
 ...
 *  +-------------------------------------------------------------+
 */
static GstPlayAudioChain *gen_audio_chain(GstPlaySink * playsink, gboolean raw)
{
    GstPlayAudioChain *chain;
    GstBin *bin;
    ...
    {
        GST_DEBUG_OBJECT(playsink, "trying configured audiosink %" GST_PTR_FORMAT, playsink->audio_sink);
    ...
    {
        post_missing_element_message(playsink, "queue");
        GST_ELEMENT_WARNING(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "queue"), ("audio playback and visualizations might not work"));
        head = chain->sink;
        prev = NULL;
    }
    else
    ...
    elem = gst_play_sink_find_property_sinks(playsink, chain->sink, "volume", G_TYPE_DOUBLE);
    if (elem)
    {
        chain->volume = elem;

        g_signal_connect(chain->volume, "notify::volume", G_CALLBACK(notify_volume_cb), playsink);
        ...
        chain->mute = gst_play_sink_find_property_sinks(playsink, chain->sink, "mute", G_TYPE_BOOLEAN);
        if (chain->mute)
        {
            GST_DEBUG_OBJECT(playsink, "the sink has a mute property");
            g_signal_connect(chain->mute, "notify::mute", G_CALLBACK(notify_mute_cb), playsink);
        }
    ...
    if (raw && !(playsink->flags & GST_PLAY_FLAG_NATIVE_AUDIO))
    {
        ...
        if (chain->conv == NULL)
        {
            ...
            GST_ELEMENT_WARNING(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "audioconvert"), ("possibly a liboil version mismatch?"));
        }
        else
        {
            gst_bin_add(bin, chain->conv);
        ...
        if (chain->resample == NULL)
        {
            ...
            GST_ELEMENT_WARNING(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "audioresample"), ("possibly a liboil version mismatch?"));
        }
        else
        {
            ...
            if (prev)
            {
                if (!gst_element_link_pads_full(prev, "src", chain->resample, "sink", GST_PAD_LINK_CHECK_TEMPLATE_CAPS))
                    goto link_failed;
            }
            else
            {
        ...
        if (!have_volume && playsink->flags & GST_PLAY_FLAG_SOFT_VOLUME)
        {
            ...
            if (chain->volume == NULL)
            {
                ...
                GST_ELEMENT_WARNING(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "volume"), ("possibly a liboil version mismatch?"));
            }
            else
            {
                have_volume = TRUE;
                ...
                if (prev)
                {
                    if (!gst_element_link_pads_full(prev, "src", chain->volume, "sink", GST_PAD_LINK_CHECK_TEMPLATE_CAPS))
                        goto link_failed;
                }
                else
                {
    ...
    if (!have_volume)
    {
        GST_ELEMENT_WARNING(playsink, STREAM, NOT_IMPLEMENTED, (_("No volume control found")), ("Volume/mute is not available"));
    }
...
no_sinks:
    {
        if (!elem && !playsink->audio_sink)
        {
            ...
            if (strcmp(DEFAULT_AUDIOSINK, "autoaudiosink"))
            {
                ...
                GST_ELEMENT_ERROR(playsink, CORE, MISSING_PLUGIN, (_("Both autoaudiosink and %s elements are missing."), DEFAULT_AUDIOSINK), (NULL));
            }
            else
            {
                GST_ELEMENT_ERROR(playsink, CORE, MISSING_PLUGIN, (_("The autoaudiosink element is missing.")), (NULL));
            }
        }
        else
        {
            if (playsink->audio_sink)
            {
                GST_ELEMENT_ERROR(playsink, CORE, STATE_CHANGE, (_("Configured audiosink %s is not working."), GST_ELEMENT_NAME(playsink->audio_sink)), (NULL));
            }
            else if (strcmp(DEFAULT_AUDIOSINK, "autoaudiosink"))
            {
                GST_ELEMENT_ERROR(playsink, CORE, STATE_CHANGE, (_("Both autoaudiosink and %s elements are not working."), DEFAULT_AUDIOSINK), (NULL));
            }
            else
            {
                GST_ELEMENT_ERROR(playsink, CORE, STATE_CHANGE, (_("The autoaudiosink element is not working.")), (NULL));
            }
        }
        free_chain((GstPlayChain *) chain);
        return NULL;
...
link_failed:
    {
        GST_ELEMENT_ERROR(playsink, CORE, PAD, (NULL), ("Failed to configure the audio sink."));
        /* checking sink made it READY */
        gst_element_set_state(chain->sink, GST_STATE_NULL);
        free_chain((GstPlayChain *) chain);
        return NULL;
...
static gboolean setup_audio_chain(GstPlaySink * playsink, gboolean raw)
{
    GstElement *elem;
    GstPlayAudioChain *chain;
    GstStateChangeReturn ret;
    ...
    if (elem)
    {
        ...
        if (playsink->volume_changed)
        {
            GST_DEBUG_OBJECT(playsink, "the sink has a volume property, setting %f", playsink->volume);
 ...
 *
 */
static GstPlayVisChain *gen_vis_chain(GstPlaySink * playsink)
{
    GstPlayVisChain *chain;
    GstBin *bin;
    gboolean res;
    ...
    res = gst_element_link_pads_full(chain->queue, "src", chain->conv, "sink", GST_PAD_LINK_CHECK_NOTHING);
    res &= gst_element_link_pads_full(chain->conv, "src", chain->resample, "sink", GST_PAD_LINK_CHECK_NOTHING);
    res &= gst_element_link_pads_full(chain->resample, "src", chain->vis, "sink", GST_PAD_LINK_CHECK_NOTHING);
    if (!res)
        goto link_failed;
...
no_queue:
    {
        post_missing_element_message(playsink, "queue");
        GST_ELEMENT_ERROR(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "queue"), (NULL));
        free_chain((GstPlayChain *) chain);
        return NULL;
    }
no_audioconvert:
    {
        post_missing_element_message(playsink, "audioconvert");
        GST_ELEMENT_ERROR(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "audioconvert"), ("possibly a liboil version mismatch?"));
        free_chain((GstPlayChain *) chain);
        return NULL;
    }
no_audioresample:
    {
        post_missing_element_message(playsink, "audioresample");
        GST_ELEMENT_ERROR(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "audioresample"), (NULL));
        free_chain((GstPlayChain *) chain);
        return NULL;
    }
no_goom:
    {
        ...
        GST_ELEMENT_ERROR(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "goom"), (NULL));
        free_chain((GstPlayChain *) chain);
        return NULL;
    }
link_failed:
    {
        GST_ELEMENT_ERROR(playsink, CORE, PAD, (NULL), ("Failed to configure the visualisation element."));
...
gboolean gst_play_sink_reconfigure(GstPlaySink * playsink)
{
    ...
    if (((flags & GST_PLAY_FLAG_VIDEO) || (flags & GST_PLAY_FLAG_NATIVE_VIDEO)) && playsink->video_pad)
    {
         ...
         * we have raw video */
        if ((flags & GST_PLAY_FLAG_DEINTERLACE) && !(flags & GST_PLAY_FLAG_NATIVE_VIDEO) && playsink->video_pad_raw)
    ...
    if (need_text && !need_video)
    {
        ...
        {
            GST_ELEMENT_WARNING(playsink, STREAM, FORMAT, (_("Can't play a text file without video or visualizations.")), ("Have text pad but no video pad or visualizations"));
            need_text = FALSE;
        }
        else
        {
            GST_ELEMENT_ERROR(playsink, STREAM, FORMAT, (_("Can't play a text file without video or visualizations.")), ("Have text pad but no video pad or visualizations"));
            GST_PLAY_SINK_UNLOCK(playsink);
            return FALSE;
        }
    }

    GST_DEBUG_OBJECT(playsink, "audio:%d, video:%d, vis:%d, text:%d", need_audio, need_video, need_vis, need_text);
    ...
    if (need_video)
    {
        ...
        GST_DEBUG_OBJECT(playsink, "adding video, raw %d", playsink->video_pad_raw);
        ...
        if (playsink->videochain)
        {
            ...
            if (!setup_video_chain(playsink, raw, async))
            {
                ...
                {
                    gst_element_release_request_pad(GST_ELEMENT_CAST(playsink->stream_synchronizer), playsink->video_sinkpad_stream_synchronizer);
        ...
        if (!playsink->video_sinkpad_stream_synchronizer)
        {
            ...
            playsink->video_sinkpad_stream_synchronizer = gst_element_get_request_pad(GST_ELEMENT_CAST(playsink->stream_synchronizer), "sink_%d");
            it = gst_pad_iterate_internal_links(playsink->video_sinkpad_stream_synchronizer);
            g_assert(it);
            gst_iterator_next(it, (gpointer *) & playsink->video_srcpad_stream_synchronizer);
            g_assert(playsink->video_srcpad_stream_synchronizer);
            gst_iterator_free(it);
        }

        if (playsink->video_pad)
            gst_ghost_pad_set_target(GST_GHOST_PAD_CAST(playsink->video_pad), playsink->video_sinkpad_stream_synchronizer);

        if (playsink->videochain && need_deinterlace)
        {
            if (!playsink->videodeinterlacechain)
                playsink->videodeinterlacechain = gen_video_deinterlace_chain(playsink);
            ...
            if (playsink->videodeinterlacechain)
            {
                ...
                gst_pad_link_full(playsink->video_srcpad_stream_synchronizer, playsink->videodeinterlacechain->sinkpad, GST_PAD_LINK_CHECK_NOTHING);
            }
        }
        else
        {
        ...
        if (playsink->videochain)
        {
            ...
            if (!need_vis && !need_text && (!playsink->textchain || !playsink->text_pad))
            {
                GST_DEBUG_OBJECT(playsink, "ghosting video sinkpad");
                if (need_deinterlace)
                    gst_pad_link_full(playsink->videodeinterlacechain->srcpad, playsink->videochain->sinkpad, GST_PAD_LINK_CHECK_NOTHING);
                else
                    gst_pad_link_full(playsink->video_srcpad_stream_synchronizer, playsink->videochain->sinkpad, GST_PAD_LINK_CHECK_NOTHING);
            }
        }
    }
    else
    {
        ...
        if (playsink->videochain)
        {
            ...
            if (playsink->vischain)
            {
                ...
                srcpad = gst_element_get_static_pad(playsink->vischain->chain.bin, "src");
    ...
    if (need_audio)
    {
        ...
        if (playsink->audiochain)
        {
            ...
            if (!setup_audio_chain(playsink, raw))
            {
                ...
                {
                    gst_element_release_request_pad(GST_ELEMENT_CAST(playsink->stream_synchronizer), playsink->audio_sinkpad_stream_synchronizer);
        ...
        if (!playsink->audio_sinkpad_stream_synchronizer)
        {
            ...
            playsink->audio_sinkpad_stream_synchronizer = gst_element_get_request_pad(GST_ELEMENT_CAST(playsink->stream_synchronizer), "sink_%d");
            it = gst_pad_iterate_internal_links(playsink->audio_sinkpad_stream_synchronizer);
            g_assert(it);
            gst_iterator_next(it, (gpointer *) & playsink->audio_srcpad_stream_synchronizer);
        ...
        if (playsink->audiochain)
        {
            ...
            {
                playsink->audio_tee_asrc = gst_element_get_request_pad(playsink->audio_tee, "src%d");
            }
            add_chain(GST_PLAY_CHAIN(playsink->audiochain), TRUE);
            activate_chain(GST_PLAY_CHAIN(playsink->audiochain), TRUE);
            gst_pad_link_full(playsink->audio_tee_asrc, playsink->audio_sinkpad_stream_synchronizer, GST_PAD_LINK_CHECK_NOTHING);
            gst_pad_link_full(playsink->audio_srcpad_stream_synchronizer, playsink->audiochain->sinkpad, GST_PAD_LINK_CHECK_NOTHING);
        }
    }
    else
    {
    ...
    if (need_vis)
    {
        ...
        if (playsink->vischain)
        {
            ...
            {
                playsink->audio_tee_vissrc = gst_element_get_request_pad(playsink->audio_tee, "src%d");
            }
            gst_pad_link_full(playsink->audio_tee_vissrc, playsink->vischain->sinkpad, GST_PAD_LINK_CHECK_NOTHING);
            gst_pad_link_full(srcpad, playsink->video_sinkpad_stream_synchronizer, GST_PAD_LINK_CHECK_NOTHING);
            gst_pad_link_full(playsink->video_srcpad_stream_synchronizer, playsink->videochain->sinkpad, GST_PAD_LINK_CHECK_NOTHING);
            gst_object_unref(srcpad);
        }
    }
    ...
    if (need_text)
    {
        ...
        if (playsink->textchain)
        {
            ...
            {
                playsink->text_sinkpad_stream_synchronizer = gst_element_get_request_pad(GST_ELEMENT_CAST(playsink->stream_synchronizer), "sink_%d");
                it = gst_pad_iterate_internal_links(playsink->text_sinkpad_stream_synchronizer);
                g_assert(it);
                gst_iterator_next(it, (gpointer *) & playsink->text_srcpad_stream_synchronizer);
                g_assert(playsink->text_srcpad_stream_synchronizer);
                gst_iterator_free(it);

                gst_ghost_pad_set_target(GST_GHOST_PAD_CAST(playsink->text_pad), playsink->text_sinkpad_stream_synchronizer);
                gst_pad_link_full(playsink->text_srcpad_stream_synchronizer, playsink->textchain->textsinkpad, GST_PAD_LINK_CHECK_NOTHING);
            }
            ...
            if (need_vis)
            {
                ...
                srcpad = gst_element_get_static_pad(playsink->vischain->chain.bin, "src");
                gst_pad_unlink(srcpad, playsink->videochain->sinkpad);
                gst_pad_link_full(srcpad, playsink->textchain->videosinkpad, GST_PAD_LINK_CHECK_NOTHING);
                gst_object_unref(srcpad);
            }
            else
            {
                if (need_deinterlace)
                    gst_pad_link_full(playsink->videodeinterlacechain->srcpad, playsink->textchain->videosinkpad, GST_PAD_LINK_CHECK_NOTHING);
                else
                    gst_pad_link_full(playsink->video_srcpad_stream_synchronizer, playsink->textchain->videosinkpad, GST_PAD_LINK_CHECK_NOTHING);
            }
            gst_pad_link_full(playsink->textchain->srcpad, playsink->videochain->sinkpad, GST_PAD_LINK_CHECK_NOTHING);
    ...
    else
    {
        ...
        if (playsink->text_sinkpad_stream_synchronizer)
        {
            gst_element_release_request_pad(GST_ELEMENT_CAST(playsink->stream_synchronizer), playsink->text_sinkpad_stream_synchronizer);
        ...
        if (playsink->textchain)
        {
            ...
            else
            {
                ...
                if (playsink->textchain->overlay)
                    g_object_set(G_OBJECT(playsink->textchain->overlay), "silent", TRUE, NULL);
            }
        }
...
GstBuffer *gst_play_sink_get_last_frame(GstPlaySink * playsink)
{
    ...
    if ((chain = (GstPlayVideoChain *) playsink->videochain))
    {
        ...
        if (chain->chain.activated && chain->sink)
        {
            ...
            if ((elem = gst_play_sink_find_property(playsink, chain->sink, "last-buffer", GST_TYPE_BUFFER)))
            {
    ...
    switch (type)
    {
        ...
        case GST_PLAY_SINK_TYPE_AUDIO:
            ...
            if (!playsink->audio_tee)
            {
                ...
                if (playsink->audio_tee == NULL)
                {
                    ...
                    GST_ELEMENT_ERROR(playsink, CORE, MISSING_PLUGIN, (_("Missing element '%s' - check your GStreamer installation."), "tee"), (NULL));
                    res = NULL;
                    break;
                }
                else
                {
                    playsink->audio_tee_sink = gst_element_get_static_pad(playsink->audio_tee, "sink");
            ...
            if (!playsink->audio_pad)
            {
                ...
                playsink->audio_pad = gst_ghost_pad_new(pad_name, playsink->audio_tee_sink);
                created = TRUE;
            }
        ...
        case GST_PLAY_SINK_TYPE_VIDEO:
            ...
            if (!playsink->video_pad)
            {
                ...
                playsink->video_pad = gst_ghost_pad_new_no_target(pad_name, GST_PAD_SINK);
                created = TRUE;
            }
        ...
        case GST_PLAY_SINK_TYPE_TEXT:
            ...
            {
                playsink->text_pad = gst_ghost_pad_new_no_target("text_sink", GST_PAD_SINK);
                created = TRUE;
            }
        ...
        case GST_MESSAGE_STEP_DONE:
            {
                ...
                if (format == GST_FORMAT_BUFFERS)
                {
                    ...
                    if (playsink->audiochain)
                    {
                        ...
                        event = gst_event_new_step(GST_FORMAT_TIME, duration, rate, flush, intermediate);
 ...
 */
static gboolean gst_play_sink_send_event_to_sink(GstPlaySink * playsink, GstEvent * event)
{
    gboolean res = TRUE;
        ...
        case GST_EVENT_STEP:
            {
                ...
                {
                    res = GST_ELEMENT_CLASS(gst_play_sink_parent_class)->send_event(element, event);
                }
                break;
            }
        default:
...
static GstStateChangeReturn gst_play_sink_change_state(GstElement * element, GstStateChange transition)
{
    ...
    bret = GST_ELEMENT_CLASS(gst_play_sink_parent_class)->change_state(element, transition);
...
activate_failed:
    {
        GST_DEBUG_OBJECT(element, "element failed to change states -- activation problem?");
...
static void gst_play_sink_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * spec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_SUBTITLE_ENCODING:
            gst_play_sink_set_subtitle_encoding(playsink, g_value_get_string(value));
            break;
...
static void gst_play_sink_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * spec)
{
    ...
    switch (prop_id)
    {
        ...
        case PROP_FONT_DESC:
            ...
            break;
        case PROP_SUBTITLE_ENCODING:
            g_value_take_string(value, gst_play_sink_get_subtitle_encoding(playsink));
            break;
        case PROP_VIS_PLUGIN:
...
gboolean gst_play_sink_plugin_init(GstPlugin * plugin)
{
    ...
    return gst_element_register(plugin, "playsink", GST_RANK_NONE, GST_TYPE_PLAY_SINK);
}
