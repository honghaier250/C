...
static gboolean gst_tcp_client_sink_setcaps(GstBaseSink * bsink, GstCaps * caps);
static GstFlowReturn gst_tcp_client_sink_render(GstBaseSink * bsink, GstBuffer * buf);
static GstStateChangeReturn gst_tcp_client_sink_change_state(GstElement * element, GstStateChange transition);

static void gst_tcp_client_sink_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_tcp_client_sink_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
...
GType gst_tcp_client_sink_get_type(void)
{
    ...
    if (!tcpclientsink_type)
    {
        ...
        tcpclientsink_type = g_type_register_static(GST_TYPE_BASE_SINK, "GstTCPClientSink", &tcpclientsink_info, 0);
    }
...
static void gst_tcp_client_sink_base_init(gpointer g_class)
{
    ...
    gst_element_class_set_details_simple(element_class, "TCP client sink", "Sink/Network", "Send data as a client over the network via TCP", "Thomas Vander Stichele <thomas at apestaart dot org>");
}
...
static void gst_tcp_client_sink_class_init(GstTCPClientSink * klass)
{
    ...
    g_object_class_install_property(gobject_class, ARG_PROTOCOL, g_param_spec_enum("protocol", "Protocol", "The protocol to wrap data in", GST_TYPE_TCP_PROTOCOL, GST_TCP_PROTOCOL_NONE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
static void gst_tcp_client_sink_init(GstTCPClientSink * this)
{
    ...
    switch (sink->protocol)
    {
        ...
        case GST_TCP_PROTOCOL_GDP:
            ...
            if (!sink->caps_sent)
            {
                ...
                if (!gst_tcp_gdp_write_caps(GST_ELEMENT(sink), sink->sock_fd.fd, caps, TRUE, sink->host, sink->port))
...
static GstFlowReturn gst_tcp_client_sink_render(GstBaseSink * bsink, GstBuffer * buf)
{
    ...
    g_return_val_if_fail(GST_OBJECT_FLAG_IS_SET(sink, GST_TCP_CLIENT_SINK_OPEN), GST_FLOW_WRONG_STATE);
    ...
    switch (sink->protocol)
    {
        ...
        case GST_TCP_PROTOCOL_GDP:
            ...
            if (!gst_tcp_gdp_write_buffer(GST_ELEMENT(sink), sink->sock_fd.fd, buf, TRUE, sink->host, sink->port))
                goto gdp_write_error;
            break;
        default:
            break;
...
write_error:
    {
        GST_ELEMENT_ERROR(sink, RESOURCE, WRITE, (_("Error while sending data to \"%s:%d\"."), sink->host, sink->port), ("Only %" G_GSIZE_FORMAT " of %u bytes written: %s", wrote, GST_BUFFER_SIZE(buf), g_strerror(errno)));
...
static gboolean gst_tcp_client_sink_start(GstTCPClientSink * this)
{
    int ret;
    gchar *ip;
    ...
    GST_DEBUG_OBJECT(this, "opening sending client socket to %s:%d", this->host, this->port);
    if ((this->sock_fd.fd = socket(AF_INET, SOCK_STREAM, 0)) == -1)
    {
        GST_ELEMENT_ERROR(this, RESOURCE, OPEN_WRITE, (NULL), GST_ERROR_SYSTEM);
        return FALSE;
    }
    GST_DEBUG_OBJECT(this, "opened sending client socket with fd %d", this->sock_fd.fd);
    ...
    ret = connect(this->sock_fd.fd, (struct sockaddr *) &this->server_sin, sizeof(this->server_sin));
    ...
    if (ret)
    {
        ...
        {
            case ECONNREFUSED:
                GST_ELEMENT_ERROR(this, RESOURCE, OPEN_WRITE, (_("Connection to %s:%d refused."), this->host, this->port), (NULL));
                return FALSE;
                break;
            default:
                GST_ELEMENT_ERROR(this, RESOURCE, OPEN_READ, (NULL), ("connect to %s:%d failed: %s", this->host, this->port, g_strerror(errno)));
                return FALSE;
                break;
        }
    }
...
static gboolean gst_tcp_client_sink_stop(GstTCPClientSink * this)
{
    if (!GST_OBJECT_FLAG_IS_SET(this, GST_TCP_CLIENT_SINK_OPEN))
        return TRUE;
