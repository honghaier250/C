...
static gboolean gst_tcp_server_sink_handle_wait(GstMultiFdSink * sink, GstPoll * set);
...
static void gst_tcp_server_sink_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_tcp_server_sink_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

GST_BOILERPLATE(GstTCPServerSink, gst_tcp_server_sink, GstMultiFdSink, GST_TYPE_MULTI_FD_SINK);

static void gst_tcp_server_sink_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_set_details_simple(element_class, "TCP server sink", "Sink/Network", "Send data as a server over the network via TCP", "Thomas Vander Stichele <thomas at apestaart dot org>");
}

static void gst_tcp_server_sink_class_init(GstTCPServerSinkClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, ARG_HOST, g_param_spec_string("host", "host", "The host/IP to send the packets to", TCP_DEFAULT_HOST, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, ARG_PORT, g_param_spec_int("port", "port", "The port to send the packets to", 0, TCP_HIGHEST_PORT, TCP_DEFAULT_PORT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
static void gst_tcp_server_sink_init(GstTCPServerSink * this, GstTCPServerSinkClass * klass)
{
...
static gboolean gst_tcp_server_sink_handle_server_read(GstTCPServerSink * sink)
{
    /* new client */
    int client_sock_fd;
    ...
    client_sock_fd = accept(sink->server_sock.fd, (struct sockaddr *) &client_address, &client_address_len);
    if (client_sock_fd == -1)
        goto accept_failed;

    gst_multi_fd_sink_add(GST_MULTI_FD_SINK(sink), client_sock_fd);

    GST_DEBUG_OBJECT(sink, "added new client ip %s with fd %d", inet_ntoa(client_address.sin_addr), client_sock_fd);
...
accept_failed:
    {
        GST_ELEMENT_ERROR(sink, RESOURCE, OPEN_WRITE, (NULL), ("Could not accept client on server socket %d: %s (%d)", sink->server_sock.fd, g_strerror(errno), errno));
        return FALSE;
    }
}
...
static void gst_tcp_server_sink_removed(GstMultiFdSink * sink, int fd)
{
    ...
    if (close(fd) < 0)
    {
        GST_WARNING_OBJECT(this, "error closing fd %d: %s", fd, g_strerror(errno));
    }
}
...
connection_failed:
    {
        GST_ELEMENT_ERROR(this, RESOURCE, READ, (NULL), ("client connection failed: %s", g_strerror(errno)));
        return FALSE;
    }
}
...
static gboolean gst_tcp_server_sink_init_send(GstMultiFdSink * parent)
{
    int ret;
    ...
    GST_DEBUG_OBJECT(this, "opened sending server socket with fd %d", this->server_sock.fd);

    /* make address reusable */
    ret = 1;
    if (setsockopt(this->server_sock.fd, SOL_SOCKET, SO_REUSEADDR, (void *) &ret, sizeof(ret)) < 0)
        goto reuse_failed;

    /* keep connection alive; avoids SIGPIPE during write */
    ret = 1;
    if (setsockopt(this->server_sock.fd, SOL_SOCKET, SO_KEEPALIVE, (void *) &ret, sizeof(ret)) < 0)
    ...
    ret = bind(this->server_sock.fd, (struct sockaddr *) &this->server_sin, sizeof(this->server_sin));
    if (ret)
        goto bind_failed;
    ...
    GST_DEBUG_OBJECT(this, "listening on server socket %d with queue of %d", this->server_sock.fd, TCP_BACKLOG);
    if (listen(this->server_sock.fd, TCP_BACKLOG) == -1)
        goto listen_failed;

    GST_DEBUG_OBJECT(this, "listened on server socket %d, returning from connection setup", this->server_sock.fd);
...
reuse_failed:
    {
        ...
        GST_ELEMENT_ERROR(this, RESOURCE, SETTINGS, (NULL), ("Could not setsockopt: %s", g_strerror(errno)));
        return FALSE;
    }
keepalive_failed:
    {
...
listen_failed:
    {
        gst_tcp_socket_close(&this->server_sock);
        GST_ELEMENT_ERROR(this, RESOURCE, OPEN_READ, (NULL), ("Could not listen on server socket: %s", g_strerror(errno)));
        return FALSE;
    }
bind_failed:
    {
        ...
        {
            default:
                GST_ELEMENT_ERROR(this, RESOURCE, OPEN_READ, (NULL), ("bind on port %d failed: %s", this->server_port, g_strerror(errno)));
                break;
        }
        return FALSE;
    }
...
static gboolean gst_tcp_server_sink_close(GstMultiFdSink * parent)
{
    GstTCPServerSink *this = GST_TCP_SERVER_SINK(parent);
