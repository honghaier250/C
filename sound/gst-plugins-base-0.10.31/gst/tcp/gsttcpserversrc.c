...
GST_BOILERPLATE(GstTCPServerSrc, gst_tcp_server_src, GstPushSrc, GST_TYPE_PUSH_SRC);
...
static GstFlowReturn gst_tcp_server_src_create(GstPushSrc * psrc, GstBuffer ** buf);

static void gst_tcp_server_src_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_tcp_server_src_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

static void gst_tcp_server_src_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&srctemplate));

    gst_element_class_set_details_simple(element_class, "TCP server source", "Source/Network", "Receive data as a server over the network via TCP", "Thomas Vander Stichele <thomas at apestaart dot org>");
}

static void gst_tcp_server_src_class_init(GstTCPServerSrcClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_HOST, g_param_spec_string("host", "Host", "The hostname to listen as", TCP_DEFAULT_LISTEN_HOST, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_PORT, g_param_spec_int("port", "Port", "The port to listen to", 0, TCP_HIGHEST_PORT, TCP_DEFAULT_PORT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_PROTOCOL, g_param_spec_enum("protocol", "Protocol", "The protocol to wrap data in", GST_TYPE_TCP_PROTOCOL, GST_TCP_PROTOCOL_NONE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    GST_DEBUG_CATEGORY_INIT(tcpserversrc_debug, "tcpserversrc", 0, "TCP Server Source");
}

static void gst_tcp_server_src_init(GstTCPServerSrc * src, GstTCPServerSrcClass * g_class)
{
...
static GstFlowReturn gst_tcp_server_src_create(GstPushSrc * psrc, GstBuffer ** outbuf)
{
    GstTCPServerSrc *src;
...
restart:
    ...
    if (src->client_sock_fd.fd < 0)
    {
        ...
        {
            if ((src->client_sock_fd.fd = accept(src->server_sock_fd.fd, (struct sockaddr *) &src->client_sin, &src->client_sin_len)) == -1)
                goto accept_error;
    ...
    switch (src->protocol)
    {
        case GST_TCP_PROTOCOL_NONE:
            ret = gst_tcp_read_buffer(GST_ELEMENT(src), src->client_sock_fd.fd, src->fdset, outbuf);
            break;
        ...
        case GST_TCP_PROTOCOL_GDP:
            if (!src->caps_received)
            {
                ...
                ret = gst_tcp_gdp_read_caps(GST_ELEMENT(src), src->client_sock_fd.fd, src->fdset, &caps);
            ...
            ret = gst_tcp_gdp_read_buffer(GST_ELEMENT(src), src->client_sock_fd.fd, src->fdset, outbuf);
    ...
    if (ret == GST_FLOW_OK)
    {
        GST_LOG_OBJECT(src,
                       ...
                       GST_BUFFER_SIZE(*outbuf), GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(*outbuf)), GST_TIME_ARGS(GST_BUFFER_DURATION(*outbuf)), GST_BUFFER_OFFSET(*outbuf), GST_BUFFER_OFFSET_END(*outbuf));
    }
...
select_error:
    {
        GST_ELEMENT_ERROR(src, RESOURCE, READ, (NULL), ("Select error: %s", g_strerror(errno)));
...
accept_error:
    {
        GST_ELEMENT_ERROR(src, RESOURCE, OPEN_READ, (NULL), ("Could not accept client on server socket: %s", g_strerror(errno)));
        return GST_FLOW_ERROR;
    }
gdp_cancelled:
    {
...
gdp_caps_read_error:
    {
        ...
        if (ret != GST_FLOW_WRONG_STATE)
        {
            GST_ELEMENT_ERROR(src, RESOURCE, READ, (NULL), ("Could not read caps through GDP"));
        }
        return ret;
    }
}
...
static gboolean gst_tcp_server_src_start(GstBaseSrc * bsrc)
{
    ...
    GST_DEBUG_OBJECT(src, "opened receiving server socket with fd %d", src->server_sock_fd.fd);

    /* make address reusable */
    ret = 1;
    if (setsockopt(src->server_sock_fd.fd, SOL_SOCKET, SO_REUSEADDR, &ret, sizeof(int)) < 0)
        goto sock_opt;
    ...
    if ((ret = bind(src->server_sock_fd.fd, (struct sockaddr *) &src->server_sin, sizeof(src->server_sin))) < 0)
        goto bind_error;

    GST_DEBUG_OBJECT(src, "listening on server socket %d with queue of %d", src->server_sock_fd.fd, TCP_BACKLOG);
...
sock_opt:
    {
        GST_ELEMENT_ERROR(src, RESOURCE, SETTINGS, (NULL), ("Could not setsockopt: %s", g_strerror(errno)));
...
bind_error:
    {
        ...
        {
            default:
                GST_ELEMENT_ERROR(src, RESOURCE, OPEN_READ, (NULL), ("bind failed: %s", g_strerror(errno)));
                break;
        }
        return FALSE;
    }
listen_error:
    {
        gst_tcp_socket_close(&src->server_sock_fd);
        GST_ELEMENT_ERROR(src, RESOURCE, OPEN_READ, (NULL), ("Could not listen on server socket: %s", g_strerror(errno)));
        return FALSE;
    }
socket_pair:
    {
        GST_ELEMENT_ERROR(src, RESOURCE, OPEN_READ_WRITE, (NULL), GST_ERROR_SYSTEM);
        gst_tcp_socket_close(&src->server_sock_fd);
        return FALSE;
    }
}
