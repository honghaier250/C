...
GST_BOILERPLATE(GstTCPClientSrc, gst_tcp_client_src, GstPushSrc, GST_TYPE_PUSH_SRC);

static void gst_tcp_client_src_finalize(GObject * gobject);

static GstCaps *gst_tcp_client_src_getcaps(GstBaseSrc * psrc);

static GstFlowReturn gst_tcp_client_src_create(GstPushSrc * psrc, GstBuffer ** outbuf);
...
static void gst_tcp_client_src_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_tcp_client_src_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

static void gst_tcp_client_src_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&srctemplate));

    gst_element_class_set_details_simple(element_class, "TCP client source", "Source/Network", "Receive data as a client over the network via TCP", "Thomas Vander Stichele <thomas at apestaart dot org>");
}

static void gst_tcp_client_src_class_init(GstTCPClientSrcClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_HOST, g_param_spec_string("host", "Host", "The host IP address to receive packets from", TCP_DEFAULT_HOST, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_PORT, g_param_spec_int("port", "Port", "The port to receive packets from", 0, TCP_HIGHEST_PORT, TCP_DEFAULT_PORT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    GST_DEBUG_CATEGORY_INIT(tcpclientsrc_debug, "tcpclientsrc", 0, "TCP Client Source");
}

static void gst_tcp_client_src_init(GstTCPClientSrc * this, GstTCPClientSrcClass * g_class)
{
...
static GstCaps *gst_tcp_client_src_getcaps(GstBaseSrc * bsrc)
{
    GstTCPClientSrc *src;
    GstCaps *caps = NULL;
...
static GstFlowReturn gst_tcp_client_src_create(GstPushSrc * psrc, GstBuffer ** outbuf)
{
    ...
    switch (src->protocol)
    {
        case GST_TCP_PROTOCOL_NONE:
            ret = gst_tcp_read_buffer(GST_ELEMENT(src), src->sock_fd.fd, src->fdset, outbuf);
            break;
        ...
        case GST_TCP_PROTOCOL_GDP:
            ...
            if (!src->caps_received)
            {
                ...
                ret = gst_tcp_gdp_read_caps(GST_ELEMENT(src), src->sock_fd.fd, src->fdset, &caps);
            ...
            ret = gst_tcp_gdp_read_buffer(GST_ELEMENT(src), src->sock_fd.fd, src->fdset, outbuf);
            break;
        default:
...
static gboolean gst_tcp_client_src_start(GstBaseSrc * bsrc)
{
    ...
    GST_DEBUG_OBJECT(src, "opening receiving client socket to %s:%d", src->host, src->port);

    if ((src->sock_fd.fd = socket(AF_INET, SOCK_STREAM, 0)) == -1)
        goto no_socket;

    GST_DEBUG_OBJECT(src, "opened receiving client socket with fd %d", src->sock_fd.fd);
    ...
    ret = connect(src->sock_fd.fd, (struct sockaddr *) &src->server_sin, sizeof(src->server_sin));
    if (ret)
        goto connect_failed;
...
connect_failed:
    {
        ...
        {
            case ECONNREFUSED:
                GST_ELEMENT_ERROR(src, RESOURCE, OPEN_READ, (_("Connection to %s:%d refused."), src->host, src->port), (NULL));
                break;
            default:
                GST_ELEMENT_ERROR(src, RESOURCE, OPEN_READ, (NULL), ("connect to %s:%d failed: %s", src->host, src->port, g_strerror(errno)));
                break;
        }
        return FALSE;
    }
