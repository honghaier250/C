...
resolve_error:
    {
        GST_ELEMENT_ERROR(element, RESOURCE, NOT_FOUND, (NULL), ("Could not find IP address for host \"%s\".", host));
        return NULL;
    }
not_ip:
    {
        GST_ELEMENT_ERROR(element, RESOURCE, NOT_FOUND, (NULL), ("host \"%s\" is not an IP host", host));
        return NULL;
    }
}
 ...
 */
static GstFlowReturn gst_tcp_socket_read(GstElement * this, int socket, void *buf, size_t count, GstPoll * fdset)
{
    ssize_t n;
    size_t bytes_read;
    int num_to_read;
...
select_error:
    {
        GST_ELEMENT_ERROR(this, RESOURCE, READ, (NULL), ("select failed: %s", g_strerror(errno)));
...
ioctl_error:
    {
        GST_ELEMENT_ERROR(this, RESOURCE, READ, (NULL), ("ioctl failed: %s", g_strerror(errno)));
        return GST_FLOW_ERROR;
    }
got_eos:
    {
...
read_error:
    {
        GST_ELEMENT_ERROR(this, RESOURCE, READ, (NULL), ("read failed: %s", g_strerror(errno)));
        return GST_FLOW_ERROR;
    }
short_read:
    {
        GST_ELEMENT_ERROR(this, RESOURCE, READ, (NULL), ("short read: wanted %d bytes, got %" G_GSSIZE_FORMAT, num_to_read, n));
        return GST_FLOW_ERROR;
    }
}
...
short_read:
    {
        GST_ELEMENT_ERROR(this, RESOURCE, READ, (NULL), ("short read: wanted %d bytes, got %" G_GSSIZE_FORMAT, readsize, bytes_read));
        gst_buffer_unref(*buf);
        *buf = NULL;
        return GST_FLOW_ERROR;
    }
...
GstFlowReturn gst_tcp_gdp_read_buffer(GstElement * this, int socket, GstPoll * fdset, GstBuffer ** buf)
{
    ...
    GST_LOG_OBJECT(this, "Reading %d bytes for buffer packet header", GST_DP_HEADER_LENGTH);
    ...
    *buf = gst_dp_buffer_from_header(GST_DP_HEADER_LENGTH, header);
    ...
    ret = gst_tcp_socket_read(this, socket, GST_BUFFER_DATA(*buf), GST_BUFFER_SIZE(*buf), fdset);
...
validate_error:
    {
        GST_ELEMENT_ERROR(this, RESOURCE, READ, (NULL), ("GDP buffer packet header does not validate"));
        g_free(header);
        return GST_FLOW_ERROR;
    }
is_not_buffer:
    {
        GST_ELEMENT_ERROR(this, RESOURCE, READ, (NULL), ("GDP packet contains something that is not a buffer (type %d)", gst_dp_header_payload_type(header)));
        g_free(header);
        return GST_FLOW_ERROR;
    }
data_read_error:
...
GstFlowReturn gst_tcp_gdp_read_caps(GstElement * this, int socket, GstPoll * fdset, GstCaps ** caps)
{
    ...
    GST_LOG_OBJECT(this, "Reading %d bytes for caps packet header", GST_DP_HEADER_LENGTH);
    ...
    *caps = NULL;
    ...
    GST_LOG_OBJECT(this, "Reading %" G_GSIZE_FORMAT " bytes for caps packet payload", payload_length);
...
header_validate_error:
    {
        GST_ELEMENT_ERROR(this, RESOURCE, READ, (NULL), ("GDP caps packet header does not validate"));
        g_free(header);
        return GST_FLOW_ERROR;
    }
is_not_caps:
    {
        GST_ELEMENT_ERROR(this, RESOURCE, READ, (NULL), ("GDP packet contains something that is not a caps (type %d)", gst_dp_header_payload_type(header)));
        g_free(header);
        return GST_FLOW_ERROR;
    }
payload_read_error:
...
payload_validate_error:
    {
        GST_ELEMENT_ERROR(this, RESOURCE, READ, (NULL), ("GDP caps packet payload does not validate"));
        g_free(header);
        g_free(payload);
        return GST_FLOW_ERROR;
    }
...
gboolean gst_tcp_gdp_write_buffer(GstElement * this, int socket, GstBuffer * buffer, gboolean fatal, const gchar * host, int port)
{
    guint length;
    guint8 *header;
    size_t wrote;
...
create_error:
    {
        if (fatal)
            GST_ELEMENT_ERROR(this, CORE, TOO_LAZY, (NULL), ("Could not create GDP header from buffer"));
        return FALSE;
    }
write_error:
    {
        if (fatal)
            GST_ELEMENT_ERROR(this, RESOURCE, WRITE, (_("Error while sending data to \"%s:%d\"."), host, port), ("Only %" G_GSIZE_FORMAT " of %u bytes written: %s", wrote, GST_BUFFER_SIZE(buffer), g_strerror(errno)));
        return FALSE;
    }
}
...
gboolean gst_tcp_gdp_write_caps(GstElement * this, int socket, const GstCaps * caps, gboolean fatal, const char *host, int port)
{
    guint length;
    guint8 *header;
    guint8 *payload;
...
create_error:
    {
        if (fatal)
            GST_ELEMENT_ERROR(this, CORE, TOO_LAZY, (NULL), ("Could not create GDP packet from caps"));
        return FALSE;
    }
write_header_error:
    {
        ...
        g_free(payload);
        if (fatal)
            GST_ELEMENT_ERROR(this, RESOURCE, WRITE, (_("Error while sending gdp header data to \"%s:%d\"."), host, port), ("Only %" G_GSIZE_FORMAT " of %u bytes written: %s", wrote, length, g_strerror(errno)));
        return FALSE;
    }
write_payload_error:
    {
        if (fatal)
            GST_ELEMENT_ERROR(this, RESOURCE, WRITE, (_("Error while sending gdp payload data to \"%s:%d\"."), host, port), ("Only %" G_GSIZE_FORMAT " of %u bytes written: %s", wrote, length, g_strerror(errno)));
        return FALSE;
    }
}
