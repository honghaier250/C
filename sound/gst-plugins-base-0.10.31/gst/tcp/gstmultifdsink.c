...
static GType gst_fdset_mode_get_type(void)
{
...
static GType gst_recover_policy_get_type(void)
{
    ...
    {
        recover_policy_type = g_enum_register_static("GstRecoverPolicy", recover_policy);
    }
    return recover_policy_type;
}

#define GST_TYPE_SYNC_METHOD (gst_sync_method_get_type())
static GType gst_sync_method_get_type(void)
{
...
static GType gst_unit_type_get_type(void)
{
...
static GType gst_client_status_get_type(void)
{
    ...
    {
        client_status_type = g_enum_register_static("GstClientStatus", client_status);
    }
...
static void gst_multi_fd_sink_remove_client_link(GstMultiFdSink * sink, GList * link);

static GstFlowReturn gst_multi_fd_sink_render(GstBaseSink * bsink, GstBuffer * buf);
static GstStateChangeReturn gst_multi_fd_sink_change_state(GstElement * element, GstStateChange transition);

static void gst_multi_fd_sink_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_multi_fd_sink_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

GST_BOILERPLATE(GstMultiFdSink, gst_multi_fd_sink, GstBaseSink, GST_TYPE_BASE_SINK);

static guint gst_multi_fd_sink_signals[LAST_SIGNAL] = { 0 };

static void gst_multi_fd_sink_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&sinktemplate));

    gst_element_class_set_details_simple(element_class, "Multi filedescriptor sink", "Sink/Network", "Send data to multiple filedescriptors", "Thomas Vander Stichele <thomas at apestaart dot org>, " "Wim Taymans <wim@fluendo.com>");
}

static void gst_multi_fd_sink_class_init(GstMultiFdSinkClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_PROTOCOL,
                                    g_param_spec_enum("protocol", "Protocol", "The protocol to wrap data in"
                                                      ". GDP protocol here is deprecated. Please use gdppay element.", GST_TYPE_TCP_PROTOCOL, DEFAULT_PROTOCOL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_object_class_install_property(gobject_class, PROP_MODE, g_param_spec_enum("mode", "Mode", "The mode for selecting activity on the fds (deprecated)", GST_TYPE_FDSET_MODE, DEFAULT_MODE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_BUFFERS_MAX,
                                    g_param_spec_int("buffers-max", "Buffers max", "max number of buffers to queue for a client (-1 = no limit)", -1, G_MAXINT, DEFAULT_BUFFERS_MAX, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_BUFFERS_SOFT_MAX,
                                    g_param_spec_int("buffers-soft-max", "Buffers soft max", "Recover client when going over this limit (-1 = no limit)", -1, G_MAXINT, DEFAULT_BUFFERS_SOFT_MAX, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_BYTES_MIN, g_param_spec_int("bytes-min", "Bytes min", "min number of bytes to queue (-1 = as little as possible)", -1, G_MAXINT, DEFAULT_BYTES_MIN, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_TIME_MIN, g_param_spec_int64("time-min", "Time min", "min number of time to queue (-1 = as little as possible)", -1, G_MAXINT64, DEFAULT_TIME_MIN, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_BUFFERS_MIN,
                                    g_param_spec_int("buffers-min", "Buffers min", "min number of buffers to queue (-1 = as few as possible)", -1, G_MAXINT, DEFAULT_BUFFERS_MIN, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_UNIT_TYPE,
                                    g_param_spec_enum("unit-type", "Units type", "The unit to measure the max/soft-max/queued properties", GST_TYPE_UNIT_TYPE, DEFAULT_UNIT_TYPE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_UNITS_MAX, g_param_spec_int64("units-max", "Units max", "max number of units to queue (-1 = no limit)", -1, G_MAXINT64, DEFAULT_UNITS_MAX, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_UNITS_SOFT_MAX,
                                    g_param_spec_int64("units-soft-max", "Units soft max", "Recover client when going over this limit (-1 = no limit)", -1, G_MAXINT64, DEFAULT_UNITS_SOFT_MAX, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_BUFFERS_QUEUED, g_param_spec_uint("buffers-queued", "Buffers queued", "Number of buffers currently queued", 0, G_MAXUINT, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
#if NOT_IMPLEMENTED
    g_object_class_install_property(gobject_class, PROP_BYTES_QUEUED, g_param_spec_uint("bytes-queued", "Bytes queued", "Number of bytes currently queued", 0, G_MAXUINT, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_TIME_QUEUED, g_param_spec_uint64("time-queued", "Time queued", "Number of time currently queued", 0, G_MAXUINT64, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
#endif

    g_object_class_install_property(gobject_class, PROP_RECOVER_POLICY,
                                    g_param_spec_enum("recover-policy", "Recover Policy", "How to recover when client reaches the soft max", GST_TYPE_RECOVER_POLICY, DEFAULT_RECOVER_POLICY, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_TIMEOUT,
                                    g_param_spec_uint64("timeout", "Timeout", "Maximum inactivity timeout in nanoseconds for a client (0 = no limit)", 0, G_MAXUINT64, DEFAULT_TIMEOUT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_SYNC_METHOD, g_param_spec_enum("sync-method", "Sync Method", "How to sync new clients to the stream", GST_TYPE_SYNC_METHOD, DEFAULT_SYNC_METHOD, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_BYTES_TO_SERVE, g_param_spec_uint64("bytes-to-serve", "Bytes to serve", "Number of bytes received to serve to clients", 0, G_MAXUINT64, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_BYTES_SERVED, g_param_spec_uint64("bytes-served", "Bytes served", "Total number of bytes send to all clients", 0, G_MAXUINT64, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_BURST_UNIT,
                                    g_param_spec_enum("burst-unit", "Burst unit", "The format of the burst units (when sync-method is burst[[-with]-keyframe])", GST_TYPE_UNIT_TYPE, DEFAULT_BURST_UNIT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_BURST_VALUE, g_param_spec_uint64("burst-value", "Burst value", "The amount of burst expressed in burst-unit", 0, G_MAXUINT64, DEFAULT_BURST_VALUE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_QOS_DSCP,
                                    g_param_spec_int("qos-dscp", "QoS diff srv code point", "Quality of Service, differentiated services code point (-1 default)", -1, 63, DEFAULT_QOS_DSCP, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   ...
   * Since: 0.10.23
   */
    g_object_class_install_property(gobject_class, PROP_HANDLE_READ, g_param_spec_boolean("handle-read", "Handle Read", "Handle client reads and discard the data", DEFAULT_HANDLE_READ, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   ...
   */
    g_object_class_install_property(gobject_class, PROP_RESEND_STREAMHEADER,
                                    g_param_spec_boolean("resend-streamheader", "Resend streamheader", "Resend the streamheader if it changes in the caps", DEFAULT_RESEND_STREAMHEADER, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_NUM_FDS, g_param_spec_uint("num-fds", "Number of fds", "The current number of client file descriptors.", 0, G_MAXUINT, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
   ...
   */
    gst_multi_fd_sink_signals[SIGNAL_ADD] = g_signal_new("add", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, G_STRUCT_OFFSET(GstMultiFdSinkClass, add), NULL, NULL, g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
  /**
    ...
    gst_multi_fd_sink_signals[SIGNAL_ADD_BURST] =
        g_signal_new("add-full", G_TYPE_FROM_CLASS(klass),
                     G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, G_STRUCT_OFFSET(GstMultiFdSinkClass,
                                                                          add_full), NULL, NULL,
                     gst_tcp_marshal_VOID__INT_ENUM_INT_UINT64_INT_UINT64, G_TYPE_NONE, 6, G_TYPE_INT, GST_TYPE_SYNC_METHOD, GST_TYPE_UNIT_TYPE, G_TYPE_UINT64, GST_TYPE_UNIT_TYPE, G_TYPE_UINT64);
  /**
   ...
   */
    gst_multi_fd_sink_signals[SIGNAL_REMOVE] = g_signal_new("remove", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, G_STRUCT_OFFSET(GstMultiFdSinkClass, remove), NULL, NULL, gst_tcp_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
  /**
   ...
   */
    gst_multi_fd_sink_signals[SIGNAL_REMOVE_FLUSH] =
        g_signal_new("remove-flush", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, G_STRUCT_OFFSET(GstMultiFdSinkClass, remove_flush), NULL, NULL, gst_tcp_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
  /**
   ...
   */
    gst_multi_fd_sink_signals[SIGNAL_CLEAR] = g_signal_new("clear", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, G_STRUCT_OFFSET(GstMultiFdSinkClass, clear), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
   ...
   */
    gst_multi_fd_sink_signals[SIGNAL_GET_STATS] =
        g_signal_new("get-stats", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, G_STRUCT_OFFSET(GstMultiFdSinkClass, get_stats), NULL, NULL, gst_tcp_marshal_BOXED__INT, G_TYPE_VALUE_ARRAY, 1, G_TYPE_INT);
   ...
   * for that.
   */
    gst_multi_fd_sink_signals[SIGNAL_CLIENT_ADDED] = g_signal_new("client-added", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstMultiFdSinkClass, client_added), NULL, NULL, gst_tcp_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
  /**
   ...
   */
    gst_multi_fd_sink_signals[SIGNAL_CLIENT_REMOVED] =
        g_signal_new("client-removed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstMultiFdSinkClass, client_removed), NULL, NULL, gst_tcp_marshal_VOID__INT_BOXED, G_TYPE_NONE, 2, G_TYPE_INT, GST_TYPE_CLIENT_STATUS);
  /**
   ...
   */
    gst_multi_fd_sink_signals[SIGNAL_CLIENT_FD_REMOVED] =
        g_signal_new("client-fd-removed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GstMultiFdSinkClass, client_fd_removed), NULL, NULL, gst_tcp_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);

    gstelement_class->change_state = GST_DEBUG_FUNCPTR(gst_multi_fd_sink_change_state);
...
static void gst_multi_fd_sink_init(GstMultiFdSink * this, GstMultiFdSinkClass * klass)
{
...
static gint setup_dscp_client(GstMultiFdSink * sink, GstTCPClient * client)
{
    gint tos;
    gint ret;
    union gst_sockaddr
    ...
    switch (af)
    {
        ...
        case AF_INET6:
#ifdef IPV6_TCLASS
            ret = setsockopt(client->fd.fd, IPPROTO_IPV6, IPV6_TCLASS, &tos, sizeof(tos));
            break;
#endif
        default:
            ret = 0;
...
static void setup_dscp(GstMultiFdSink * sink)
{
    GList *clients, *next;
...
void gst_multi_fd_sink_add_full(GstMultiFdSink * sink, int fd, GstSyncMethod sync_method, GstTCPUnitType min_unit, guint64 min_value, GstTCPUnitType max_unit, guint64 max_value)
{
    GstTCPClient *client;
    GList *clink;
    GTimeVal now;
    ...
    GST_DEBUG_OBJECT(sink, "[fd %5d] adding client, sync_method %d, " "min_unit %d, min_value %" G_GUINT64_FORMAT ", max_unit %d, max_value %" G_GUINT64_FORMAT, fd, sync_method, min_unit, min_value, max_unit, max_value);
    ...
    g_signal_emit(G_OBJECT(sink), gst_multi_fd_sink_signals[SIGNAL_CLIENT_ADDED], 0, fd);
...
wrong_limits:
    {
        GST_WARNING_OBJECT(sink, "[fd %5d] wrong values min =%" G_GUINT64_FORMAT ", max=%" G_GUINT64_FORMAT ", unit %d specified when adding client", fd, min_value, max_value, min_unit);
        return;
    }
duplicate:
    {
        ...
        g_signal_emit(G_OBJECT(sink), gst_multi_fd_sink_signals[SIGNAL_CLIENT_REMOVED], 0, fd, client->status);
        g_free(client);
        return;
    }
}
...
{
    gst_multi_fd_sink_add_full(sink, fd, sink->def_sync_method, sink->def_burst_unit, sink->def_burst_value, sink->def_burst_unit, -1);
}
...
void gst_multi_fd_sink_remove(GstMultiFdSink * sink, int fd)
{
    ...
    if (clink != NULL)
    {
        ...
        {
            GST_INFO_OBJECT(sink, "[fd %5d] Client already disconnecting with status %d", fd, client->status);
            goto done;
        }
...
static void gst_multi_fd_sink_remove_client_link(GstMultiFdSink * sink, GList * link)
{
    ...
    switch (client->status)
    {
        case GST_CLIENT_STATUS_OK:
            GST_WARNING_OBJECT(sink, "[fd %5d] removing client %p for no reason", fd, client);
            break;
        case GST_CLIENT_STATUS_CLOSED:
            GST_DEBUG_OBJECT(sink, "[fd %5d] removing client %p because of close", fd, client);
            break;
        case GST_CLIENT_STATUS_REMOVED:
            GST_DEBUG_OBJECT(sink, "[fd %5d] removing client %p because the app removed it", fd, client);
            break;
        case GST_CLIENT_STATUS_SLOW:
            GST_INFO_OBJECT(sink, "[fd %5d] removing client %p because it was too slow", fd, client);
            break;
        case GST_CLIENT_STATUS_ERROR:
            GST_WARNING_OBJECT(sink, "[fd %5d] removing client %p because of error", fd, client);
            break;
        case GST_CLIENT_STATUS_FLUSHING:
        default:
            GST_WARNING_OBJECT(sink, "[fd %5d] removing client %p with invalid reason %d", fd, client, client->status);
            break;
    }
    ...
    {
        GST_WARNING_OBJECT(sink, "[fd %5d] error removing client %p from hash", client->fd.fd, client);
    }
    ...
    g_signal_emit(G_OBJECT(sink), gst_multi_fd_sink_signals[SIGNAL_CLIENT_FD_REMOVED], 0, fd);
...
static gboolean gst_multi_fd_sink_handle_client_read(GstMultiFdSink * sink, GstTCPClient * client)
{
    int avail, fd;
    gboolean ret;
    ...
    GST_DEBUG_OBJECT(sink, "[fd %5d] select reports client read of %d bytes", fd, avail);
    ...
    else
    {
        ...
        do
        {
            ...
            GST_DEBUG_OBJECT(sink, "[fd %5d] client wants us to read %d bytes", fd, to_read);

            nread = read(fd, dummy, to_read);
            if (nread < -1)
            {
                GST_WARNING_OBJECT(sink, "[fd %5d] could not read %d bytes: %s (%d)", fd, to_read, g_strerror(errno), errno);
                client->status = GST_CLIENT_STATUS_ERROR;
                ret = FALSE;
                break;
            }
...
ioctl_failed:
    {
        GST_WARNING_OBJECT(sink, "[fd %5d] ioctl failed: %s (%d)", fd, g_strerror(errno), errno);
        client->status = GST_CLIENT_STATUS_ERROR;
        return FALSE;
    }
}
 ...
 */
static gboolean gst_multi_fd_sink_client_queue_data(GstMultiFdSink * sink, GstTCPClient * client, gchar * data, gint len)
{
    GstBuffer *buf;
    ...
    GST_LOG_OBJECT(sink, "[fd %5d] queueing data of length %d", client->fd.fd, len);
...
static gboolean gst_multi_fd_sink_client_queue_caps(GstMultiFdSink * sink, GstTCPClient * client, const GstCaps * caps)
{
    guint8 *header;
    guint8 *payload;
    guint length;
    ...
    GST_DEBUG_OBJECT(sink, "[fd %5d] Queueing caps %s through GDP", client->fd.fd, string);
    g_free(string);

    if (!gst_dp_packet_from_caps(caps, sink->header_flags, &length, &header, &payload))
    {
...
static gboolean is_sync_frame(GstMultiFdSink * sink, GstBuffer * buffer)
{
...
static gboolean gst_multi_fd_sink_client_queue_buffer(GstMultiFdSink * sink, GstTCPClient * client, GstBuffer * buffer)
{
    GstCaps *caps;
    ...
    if (!client->caps)
    {
        GST_DEBUG_OBJECT(sink, "[fd %5d] no previous caps for this client, send streamheader", client->fd.fd);
    ...
    else
    {
        ...
        if (!gst_caps_is_equal(caps, client->caps))
        {
            ...
            if (!gst_structure_has_field(s, "streamheader"))
            {
                ...
                GST_DEBUG_OBJECT(sink, "[fd %5d] new caps do not have streamheader, not sending", client->fd.fd);
            }
            else
            {
                ...
                if (!gst_structure_has_field(s, "streamheader"))
                {
                    ...
                    GST_DEBUG_OBJECT(sink, "[fd %5d] previous caps did not have streamheader, sending", client->fd.fd);
                    send_streamheader = TRUE;
                }
                else
                {
                    ...
                    {
                        GST_DEBUG_OBJECT(sink, "[fd %5d] asked to not resend the streamheader, not sending", client->fd.fd);
                    ...
                    else
                    {
                        ...
                        {
                            GST_DEBUG_OBJECT(sink, "[fd %5d] new streamheader different from old, sending", client->fd.fd);
                            send_streamheader = TRUE;
                        }
                    }
                }
    ...
    if (G_UNLIKELY(send_streamheader))
    {
        ...
        GST_LOG_OBJECT(sink, "[fd %5d] sending streamheader from caps %" GST_PTR_FORMAT, client->fd.fd, caps);
        s = gst_caps_get_structure(caps, 0);
        if (!gst_structure_has_field(s, "streamheader"))
        {
            GST_DEBUG_OBJECT(sink, "[fd %5d] no new streamheader, so nothing to send", client->fd.fd);
        }
        else
        {
            GST_LOG_OBJECT(sink, "[fd %5d] sending streamheader from caps %" GST_PTR_FORMAT, client->fd.fd, caps);
            ...
            for (i = 0; i < buffers->len; ++i)
            {
                ...
                GST_DEBUG_OBJECT(sink, "[fd %5d] queueing streamheader buffer of length %d", client->fd.fd, GST_BUFFER_SIZE(buffer));
                ...
                if (sink->protocol == GST_TCP_PROTOCOL_GDP)
                {
                    ...
                    {
                        GST_DEBUG_OBJECT(sink, "[fd %5d] could not create header, removing client", client->fd.fd);
                        return FALSE;
                    }
    ...
    GST_LOG_OBJECT(sink, "[fd %5d] queueing buffer of length %d", client->fd.fd, GST_BUFFER_SIZE(buffer));
 ...
 */
static gint find_syncframe(GstMultiFdSink * sink, gint idx, gint direction)
{
    gint i, len, result;
    ...
    for (i = idx; i >= 0 && i < len; i += direction)
    {
        ...
        {
            GST_LOG_OBJECT(sink, "found keyframe at %d from %d, direction %d", i, idx, direction);
            result = i;
            break;
        }
    }
...
static gint get_buffers_max(GstMultiFdSink * sink, gint64 max)
{
 ...
 */
static gboolean find_limits(GstMultiFdSink * sink, gint * min_idx, gint bytes_min, gint buffers_min, gint64 time_min, gint * max_idx, gint bytes_max, gint buffers_max, gint64 time_max)
{
    ...
    GST_LOG_OBJECT(sink, "bytes_min %d, buffers_min %d, time_min %" GST_TIME_FORMAT ", bytes_max %d, buffers_max %d, time_max %" GST_TIME_FORMAT, bytes_min, buffers_min, GST_TIME_ARGS(time_min), bytes_max, buffers_max, GST_TIME_ARGS(time_max));
    ...
    *max_idx = -1;
    ...
    do
    {
        ...
        {
            GST_LOG_OBJECT(sink, "Ts %" GST_TIME_FORMAT " on buffer", GST_TIME_ARGS(time));
            if (first == -1)
                first = time;
 ...
 */
static gboolean assign_value(GstTCPUnitType unit, guint64 value, gint * bytes, gint * buffers, GstClockTime * time)
{
    gboolean res = TRUE;
 ...
 */
static gboolean count_burst_unit(GstMultiFdSink * sink, gint * min_idx, GstTCPUnitType min_unit, guint64 min_value, gint * max_idx, GstTCPUnitType max_unit, guint64 max_value)
{
    ...
    return find_limits(sink, min_idx, bytes_min, buffers_min, time_min, max_idx, bytes_max, buffers_max, time_max);
}
 ...
 */
static gint gst_multi_fd_sink_new_client(GstMultiFdSink * sink, GstTCPClient * client)
{
    gint result;

    GST_DEBUG_OBJECT(sink, "[fd %5d] new client, deciding where to start in queue", client->fd.fd);
    GST_DEBUG_OBJECT(sink, "queue is currently %d buffers long", sink->bufqueue->len);
    switch (client->sync_method)
    {
        case GST_SYNC_METHOD_LATEST:
            ...
            GST_DEBUG_OBJECT(sink, "[fd %5d] SYNC_METHOD_LATEST, position %d", client->fd.fd, result);
            break;
        case GST_SYNC_METHOD_NEXT_KEYFRAME:
            {
                ...
                GST_LOG_OBJECT(sink, "[fd %5d] new client, bufpos %d, waiting for keyframe", client->fd.fd, client->bufpos);

                result = find_prev_syncframe(sink, client->bufpos);
                if (result != -1)
                {
                    GST_DEBUG_OBJECT(sink, "[fd %5d] SYNC_METHOD_NEXT_KEYFRAME: result %d", client->fd.fd, result);
                    break;
                }

                /* client is not on a syncbuffer, need to skip these buffers and
                 * wait some more */
                GST_LOG_OBJECT(sink, "[fd %5d] new client, skipping buffer(s), no syncpoint found", client->fd.fd);
                client->bufpos = -1;
                break;
            }
        case GST_SYNC_METHOD_LATEST_KEYFRAME:
            {
                GST_DEBUG_OBJECT(sink, "[fd %5d] SYNC_METHOD_LATEST_KEYFRAME", client->fd.fd);
                ...
                if (result != -1)
                {
                    GST_DEBUG_OBJECT(sink, "[fd %5d] SYNC_METHOD_LATEST_KEYFRAME: result %d", client->fd.fd, result);
                    break;
                }

                GST_DEBUG_OBJECT(sink, "[fd %5d] SYNC_METHOD_LATEST_KEYFRAME: no keyframe found, " "switching to SYNC_METHOD_NEXT_KEYFRAME", client->fd.fd);
        ...
        case GST_SYNC_METHOD_BURST:
            {
                 ...
                 */
                ok = count_burst_unit(sink, &result, client->burst_min_unit, client->burst_min_value, &max, client->burst_max_unit, client->burst_max_value);
                GST_DEBUG_OBJECT(sink, "[fd %5d] SYNC_METHOD_BURST: burst_unit returned %d, result %d", client->fd.fd, ok, result);
                ...
                {
                    result = MAX(max - 1, 0);
                    GST_DEBUG_OBJECT(sink, "[fd %5d] SYNC_METHOD_BURST: result above max, taken down to %d", client->fd.fd, result);
                }
                break;
            }
        case GST_SYNC_METHOD_BURST_KEYFRAME:
            {
                 ...
                 */
                /* gather burst limits */
                ok = count_burst_unit(sink, &min_idx, client->burst_min_unit, client->burst_min_value, &max_idx, client->burst_max_unit, client->burst_max_value);
                ...
                if (prev_syncframe != -1)
                {
                    GST_WARNING_OBJECT(sink, "using keyframe below min in BURST_KEYFRAME sync mode");
                    result = prev_syncframe;
                    break;
                }

                /* no prev keyframe or not enough data  */
                GST_WARNING_OBJECT(sink, "no prev keyframe found in BURST_KEYFRAME sync mode, waiting for next");
 ...
 */
static gboolean gst_multi_fd_sink_handle_client_write(GstMultiFdSink * sink, GstTCPClient * client)
{
    int fd = client->fd.fd;
    gboolean more;
    gboolean res;
    ...
    do
    {
        ...
        if (!client->sending)
        {
            ...
            else
            {
                ...
                GST_LOG_OBJECT(sink, "[fd %5d] client %p at position %d", fd, client, client->bufpos);
...
#    define FLAGS 0
            ...
            if (client->is_socket)
            {
                wrote = send(fd, GST_BUFFER_DATA(head) + client->bufoffset, maxsize, FLAGS);
            }
            else
            {
            ...
            else
            {
                if (wrote < maxsize)
                {
                     ...
                     * stop sending more */
                    GST_LOG_OBJECT(sink, "partial write on %d of %" G_GSSIZE_FORMAT " bytes", fd, wrote);
...
write_error:
    {
        GST_WARNING_OBJECT(sink, "[fd %5d] could not write, removing client: %s (%d)", fd, g_strerror(errno), errno);
        client->status = GST_CLIENT_STATUS_ERROR;
        return FALSE;
    }
}
 ...
 * position.
 */
static gint gst_multi_fd_sink_recover_client(GstMultiFdSink * sink, GstTCPClient * client)
{
    gint newbufpos;

    GST_WARNING_OBJECT(sink, "[fd %5d] client %p is lagging at %d, recover using policy %d", client->fd.fd, client, client->bufpos, sink->recover_policy);
    ...
    switch (sink->recover_policy)
    {
        ...
        case GST_RECOVER_POLICY_RESYNC_KEYFRAME:
            ...
            newbufpos = MIN(sink->bufqueue->len - 1, get_buffers_max(sink, sink->units_soft_max) - 1);
 ...
 */
static void gst_multi_fd_sink_queue_buffer(GstMultiFdSink * sink, GstBuffer * buf)
{
    GList *clients, *next;
    gint queuelen;
    ...
    if (sink->units_soft_max > 0)
        ...
        soft_max_buffers = -1;
    GST_LOG_OBJECT(sink, "Using max %d, softmax %d", max_buffers, soft_max_buffers);
...
restart:
    ...
    for (clients = sink->clients; clients; clients = next)
    {
        ...
        client->bufpos++;
        GST_LOG_OBJECT(sink, "[fd %5d] client %p at position %d", client->fd.fd, client, client->bufpos);
        ...
        if (soft_max_buffers > 0 && client->bufpos >= soft_max_buffers)
        {
            ...
            if (newpos != client->bufpos)
            {
                ...
                client->discont = TRUE;
                GST_INFO_OBJECT(sink, "[fd %5d] client %p position reset to %d", client->fd.fd, client, client->bufpos);
            }
            else
            {
                GST_INFO_OBJECT(sink, "[fd %5d] client %p not recovering position", client->fd.fd, client);
            }
        }
        /* check hard max and timeout, remove client */
        if ((max_buffers > 0 && client->bufpos >= max_buffers) || (sink->timeout > 0 && now - client->last_activity_time > sink->timeout))
        {
            /* remove client */
            GST_WARNING_OBJECT(sink, "[fd %5d] client %p is too slow, removing", client->fd.fd, client);
    ...
    /* make sure we respect bytes-min, buffers-min and time-min when they are set */
    {
        ...
        GST_LOG_OBJECT(sink, "extending queue %d to respect time_min %" GST_TIME_FORMAT ", bytes_min %d, buffers_min %d", max_buffer_usage, GST_TIME_ARGS(sink->time_min), sink->bytes_min, sink->buffers_min);
         ...
         * the max limits. */
        find_limits(sink, &usage, sink->bytes_min, sink->buffers_min, sink->time_min, &max, -1, -1, -1);
    ...
    if (sink->def_sync_method == GST_SYNC_METHOD_LATEST_KEYFRAME || sink->def_sync_method == GST_SYNC_METHOD_BURST_KEYFRAME)
    {
        ...
        GST_LOG_OBJECT(sink, "extending queue to include sync point, now at %d, limit is %d", max_buffer_usage, limit);
        for (i = 0; i < limit; i++)
        {
            buf = g_array_index(sink->bufqueue, GstBuffer *, i);
            if (is_sync_frame(sink, buf))
 ...
 */
static void gst_multi_fd_sink_handle_clients(GstMultiFdSink * sink)
{
    int result;
    GList *clients, *next;
    gboolean try_again;
    ...
    do
    {
        ...
        if (result < 0)
        {
            GST_WARNING_OBJECT(sink, "wait failed: %s (%d)", g_strerror(errno), errno);
            if (errno == EBADF)
            {
...
restart:
                ...
                for (clients = sink->clients; clients; clients = next)
                {
                    ...
                    if (res == -1)
                    {
                        GST_WARNING_OBJECT(sink, "fnctl failed for %d, removing: %s (%d)", fd, g_strerror(errno), errno);
                        if (errno == EBADF)
                        {
                            client->status = GST_CLIENT_STATUS_ERROR;
            ...
            {
                /* this is quite bad... */
                GST_ELEMENT_ERROR(sink, RESOURCE, READ, (NULL), ("select failed: %s (%d)", g_strerror(errno), errno));
                return;
            }
        }
        else
...
restart2:
    ...
    for (clients = sink->clients; clients; clients = next)
    {
        ...
        if (client->status != GST_CLIENT_STATUS_FLUSHING && client->status != GST_CLIENT_STATUS_OK)
        {
            gst_multi_fd_sink_remove_client_link(sink, clients);
            continue;
        }
...
static gpointer gst_multi_fd_sink_thread(GstMultiFdSink * sink)
{
    while (sink->running)
    {
...
static GstFlowReturn gst_multi_fd_sink_render(GstBaseSink * bsink, GstBuffer * buf)
{
    ...
    g_return_val_if_fail(GST_OBJECT_FLAG_IS_SET(sink, GST_MULTI_FD_SINK_OPEN), GST_FLOW_WRONG_STATE);
    ...
    GST_LOG_OBJECT(sink, "received buffer %p, in_caps: %s, offset %"
                   ...
                   ", timestamp %" GST_TIME_FORMAT ", duration %" GST_TIME_FORMAT, buf, in_caps ? "yes" : "no", GST_BUFFER_OFFSET(buf), GST_BUFFER_OFFSET_END(buf), GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buf)), GST_TIME_ARGS(GST_BUFFER_DURATION(buf)));
    ...
    {
        GST_DEBUG_OBJECT(sink, "receiving new IN_CAPS buffers, clearing old streamheader");
    ...
    if (in_caps)
    {
        GST_DEBUG_OBJECT(sink, "appending IN_CAPS buffer with length %d to streamheader", GST_BUFFER_SIZE(buf));
...
no_caps:
    {
        GST_ELEMENT_ERROR(sink, CORE, NEGOTIATION, (NULL), ("Received first buffer without caps set"));
...
static gboolean gst_multi_fd_sink_start(GstBaseSink * bsink)
{
    GstMultiFdSinkClass *fclass;
    GstMultiFdSink *this;
    ...
    this->running = TRUE;
    this->thread = g_thread_create((GThreadFunc) gst_multi_fd_sink_thread, this, TRUE, NULL);
...
socket_pair:
    {
        GST_ELEMENT_ERROR(this, RESOURCE, OPEN_READ_WRITE, (NULL), GST_ERROR_SYSTEM);
        return FALSE;
    }
}

static gboolean multifdsink_hash_remove(gpointer key, gpointer value, gpointer data)
{
    return TRUE;
}

static gboolean gst_multi_fd_sink_stop(GstBaseSink * bsink)
{
    GstMultiFdSinkClass *fclass;
    GstMultiFdSink *this;
    GstBuffer *buf;
    ...
    if (this->bufqueue)
    {
        GST_DEBUG_OBJECT(this, "Emptying bufqueue with %d buffers", this->bufqueue->len);
        for (i = this->bufqueue->len - 1; i >= 0; --i)
        {
            buf = g_array_index(this->bufqueue, GstBuffer *, i);
            GST_LOG_OBJECT(this, "Removing buffer %p (%d) with refcount %d", buf, i, GST_MINI_OBJECT_REFCOUNT(buf));
