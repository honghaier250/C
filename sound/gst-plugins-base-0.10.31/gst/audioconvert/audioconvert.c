...
static gint audio_convert_get_func_index(AudioConvertCtx * ctx, AudioConvertFmt * fmt)
{
    gint index = 0;
...
static inline gboolean check_default(AudioConvertCtx * ctx, AudioConvertFmt * fmt)
{
    if (!DOUBLE_INTERMEDIATE_FORMAT(ctx))
    {
        return (fmt->width == 32 && fmt->depth == 32 && fmt->endianness == G_BYTE_ORDER && fmt->sign == TRUE);
    }
    else
    {
...
gboolean audio_convert_prepare_context(AudioConvertCtx * ctx, AudioConvertFmt * in, AudioConvertFmt * out, GstAudioConvertDithering dither, GstAudioConvertNoiseShaping ns)
{
    gint idx_in, idx_out;
    ...
    g_return_val_if_fail(in->unpositioned_layout == out->unpositioned_layout, FALSE);
     ...
     * source depth. */
    if (ctx->out.depth <= 20 && (!ctx->in.is_int || ctx->in.depth >= ctx->out.depth))
    {
        ctx->dither = dither;
        ctx->ns = ns;
    }
    ...
    GST_INFO("in default %d, mix passthrough %d, out default %d", ctx->in_default, ctx->mix_passthrough, ctx->out_default);
...
gboolean audio_convert_get_sizes(AudioConvertCtx * ctx, gint samples, gint * srcsize, gint * dstsize)
{
    g_return_val_if_fail(ctx != NULL, FALSE);
...
gboolean audio_convert_convert(AudioConvertCtx * ctx, gpointer src, gpointer dst, gint samples, gboolean src_writable)
{
    ...
    size = (DOUBLE_INTERMEDIATE_FORMAT(ctx)) ? sizeof(gdouble) : sizeof(gint32);
