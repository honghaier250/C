...
/* gstreamer functions */
static gboolean gst_audio_convert_get_unit_size(GstBaseTransform * base, GstCaps * caps, guint * size);
static GstCaps *gst_audio_convert_transform_caps(GstBaseTransform * base, GstPadDirection direction, GstCaps * caps);
static void gst_audio_convert_fixate_caps(GstBaseTransform * base, GstPadDirection direction, GstCaps * caps, GstCaps * othercaps);
static gboolean gst_audio_convert_set_caps(GstBaseTransform * base, GstCaps * incaps, GstCaps * outcaps);
static GstFlowReturn gst_audio_convert_transform(GstBaseTransform * base, GstBuffer * inbuf, GstBuffer * outbuf);
static GstFlowReturn gst_audio_convert_transform_ip(GstBaseTransform * base, GstBuffer * buf);
...
static gboolean structure_has_fixed_channel_positions(GstStructure * s, gboolean * unpositioned_layout);
...
GST_BOILERPLATE_FULL(GstAudioConvert, gst_audio_convert, GstBaseTransform, GST_TYPE_BASE_TRANSFORM, DEBUG_INIT);
...
static GstStaticPadTemplate gst_audio_convert_src_template = GST_STATIC_PAD_TEMPLATE("src",
                                                                                     GST_PAD_SRC,
                                                                                     GST_PAD_ALWAYS,
                                                                                     STATIC_CAPS);

static GstStaticPadTemplate gst_audio_convert_sink_template = GST_STATIC_PAD_TEMPLATE("sink",
                                                                                      GST_PAD_SINK,
                                                                                      GST_PAD_ALWAYS,
                                                                                      STATIC_CAPS);

#define GST_TYPE_AUDIO_CONVERT_DITHERING (gst_audio_convert_dithering_get_type ())
static GType gst_audio_convert_dithering_get_type(void)
{
...
static GType gst_audio_convert_ns_get_type(void)
{
    static GType gtype = 0;
...
static void gst_audio_convert_base_init(gpointer g_class)
{
    ...
    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gst_audio_convert_src_template));
    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gst_audio_convert_sink_template));
    gst_element_class_set_details_simple(element_class, "Audio converter", "Filter/Converter/Audio", "Convert audio to different formats", "Benjamin Otte <otte@gnome.org>");
}
...
static void gst_audio_convert_class_init(GstAudioConvertClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, ARG_DITHERING, g_param_spec_enum("dithering", "Dithering", "Selects between different dithering methods.", GST_TYPE_AUDIO_CONVERT_DITHERING, DITHER_TPDF, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, ARG_NOISE_SHAPING,
                                    g_param_spec_enum("noise-shaping", "Noise shaping", "Selects between different noise shaping methods.", GST_TYPE_AUDIO_CONVERT_NOISE_SHAPING, NOISE_SHAPING_NONE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    basetransform_class->get_unit_size = GST_DEBUG_FUNCPTR(gst_audio_convert_get_unit_size);
    basetransform_class->transform_caps = GST_DEBUG_FUNCPTR(gst_audio_convert_transform_caps);
    basetransform_class->fixate_caps = GST_DEBUG_FUNCPTR(gst_audio_convert_fixate_caps);
    basetransform_class->set_caps = GST_DEBUG_FUNCPTR(gst_audio_convert_set_caps);
    basetransform_class->transform_ip = GST_DEBUG_FUNCPTR(gst_audio_convert_transform_ip);
    basetransform_class->transform = GST_DEBUG_FUNCPTR(gst_audio_convert_transform);

    basetransform_class->passthrough_on_same_caps = TRUE;
}

static void gst_audio_convert_init(GstAudioConvert * this, GstAudioConvertClass * g_class)
{
...
static gboolean gst_audio_convert_parse_caps(const GstCaps * caps, AudioConvertFmt * fmt)
{
    ...
    fmt->is_int = (strcmp(gst_structure_get_name(structure), "audio/x-raw-int") == 0);
...
static void set_structure_widths(GstStructure * s, int min, int max)
{
    GValue list = { 0 };
    GValue val = { 0 };
    int width;
...
static void set_structure_widths_32_and_64(GstStructure * s)
{
    GValue list = { 0 };
    GValue val = { 0 };
 ...
 * appropriate values.
 */
static GstStructure *make_lossless_changes(GstStructure * s, gboolean isfloat)
{
    GValue list = { 0 };
    GValue val = { 0 };
    int i;
...
static void strip_width_64(GstStructure * s)
{
...
static void append_with_other_format(GstCaps * caps, GstStructure * s, gboolean isfloat)
{
    GstStructure *s2;
...
static GstCaps *gst_audio_convert_transform_caps(GstBaseTransform * base, GstPadDirection direction, GstCaps * caps)
{
    ...
    for (i = 0; i < sizeof(fields_used) / sizeof(*fields_used); i++)
    {
        if (gst_structure_has_field(structure, fields_used[i]))
            gst_structure_set_value(s, fields_used[i], gst_structure_get_value(structure, fields_used[i]));
    }
    ...
    if (!isfloat)
    {
         ...
         * width, if so */
        if (!gst_structure_has_field(s, "depth") && gst_structure_get_int(s, "width", &width))
    ...
    GST_DEBUG_OBJECT(base, "  step1: (%d) %" GST_PTR_FORMAT, gst_caps_get_size(ret), ret);
    ...
    if (!allow_mixing)
    {
        ...
        if (gst_structure_has_field(structure, "channel-positions"))
            gst_structure_set_value(s, "channel-positions", gst_structure_get_value(structure, "channel-positions"));
    }
    else
    {
        if (channels == 0)
...
static const GValue *find_suitable_channel_layout(const GValue * val, guint chans)
{
...
static void gst_audio_convert_fixate_channels(GstBaseTransform * base, GstStructure * ins, GstStructure * outs)
{
    ...
    if (in_chans == out_chans && in_layout != NULL)
    {
        ...
        /* if output layout is fixed already and looks sane, we're done */
        if (GST_VALUE_HOLDS_ARRAY(out_layout) && gst_value_array_get_size(out_layout) == out_chans)
        {
            return;
        }
...
static gboolean gst_audio_convert_set_caps(GstBaseTransform * base, GstCaps * incaps, GstCaps * outcaps)
{
    ...
    if (!audio_convert_prepare_context(&this->ctx, &in_ac_caps, &out_ac_caps, this->dither, this->ns))
        goto no_converter;
...
static void gst_audio_convert_create_silence_buffer(GstAudioConvert * this, gpointer dst, gint size)
{
...
static GstFlowReturn gst_audio_convert_transform(GstBaseTransform * base, GstBuffer * inbuf, GstBuffer * outbuf)
{
    ...
    GST_CAT_LOG_OBJECT(GST_CAT_PERFORMANCE, base, "converting audio from %" GST_PTR_FORMAT " to %" GST_PTR_FORMAT, GST_BUFFER_CAPS(inbuf), GST_BUFFER_CAPS(outbuf));
    ...
    {
        if (!audio_convert_convert(&this->ctx, src, dst, samples, gst_buffer_is_writable(inbuf)))
            goto convert_error;
    }
    else
    {
...
error:
    {
        GST_ELEMENT_ERROR(this, STREAM, FORMAT, (NULL), ("cannot get input/output sizes for %d samples", samples));
        return GST_FLOW_ERROR;
    }
wrong_size:
    {
        GST_ELEMENT_ERROR(this, STREAM, FORMAT, (NULL), ("input/output buffers are of wrong size in: %d < %d or out: %d < %d", GST_BUFFER_SIZE(inbuf), insize, GST_BUFFER_SIZE(outbuf), outsize));
        return GST_FLOW_ERROR;
    }
convert_error:
    {
        GST_ELEMENT_ERROR(this, STREAM, FORMAT, (NULL), ("error while converting"));
        return GST_FLOW_ERROR;
    }
}
