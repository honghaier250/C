...
static GstStaticPadTemplate gst_video_rate_src_template = GST_STATIC_PAD_TEMPLATE("src",
                                                                                  GST_PAD_SRC,
                                                                                  GST_PAD_ALWAYS,
                                                                                  GST_STATIC_CAPS("video/x-raw-yuv;" "video/x-raw-rgb;" "video/x-raw-gray;" "image/jpeg;" "image/png"));

static GstStaticPadTemplate gst_video_rate_sink_template = GST_STATIC_PAD_TEMPLATE("sink",
                                                                                   GST_PAD_SINK,
                                                                                   GST_PAD_ALWAYS,
                                                                                   GST_STATIC_CAPS("video/x-raw-yuv;" "video/x-raw-rgb;" "video/x-raw-gray;" "image/jpeg;" "image/png"));

static void gst_video_rate_swap_prev(GstVideoRate * videorate, GstBuffer * buffer, gint64 time);
...
static void gst_video_rate_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_video_rate_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

static GstStateChangeReturn gst_video_rate_change_state(GstElement * element, GstStateChange transition);
...
static void gst_video_rate_base_init(gpointer g_class)
{
    ...
    gst_element_class_set_details_simple(element_class, "Video rate adjuster", "Filter/Effect/Video", "Drops/duplicates/adjusts timestamps on video frames to make a perfect stream", "Wim Taymans <wim@fluendo.com>");

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gst_video_rate_sink_template));
    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gst_video_rate_src_template));
}
...
static void gst_video_rate_class_init(GstVideoRateClass * klass)
{
    ...
    g_object_class_install_property(object_class, ARG_IN, g_param_spec_uint64("in", "In", "Number of input frames", 0, G_MAXUINT64, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(object_class, ARG_OUT, g_param_spec_uint64("out", "Out", "Number of output frames", 0, G_MAXUINT64, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    pspec_duplicate = g_param_spec_uint64("duplicate", "Duplicate", "Number of duplicated frames", 0, G_MAXUINT64, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
    g_object_class_install_property(object_class, ARG_DUP, pspec_duplicate);
    pspec_drop = g_param_spec_uint64("drop", "Drop", "Number of dropped frames", 0, G_MAXUINT64, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
    g_object_class_install_property(object_class, ARG_DROP, pspec_drop);
    g_object_class_install_property(object_class, ARG_SILENT, g_param_spec_boolean("silent", "silent", "Don't emit notify for dropped and duplicated frames", DEFAULT_SILENT, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(object_class, ARG_NEW_PREF, g_param_spec_double("new-pref", "New Pref", "Value indicating how much to prefer new frames (unused)", 0.0, 1.0, DEFAULT_NEW_PREF, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_object_class_install_property(object_class, ARG_SKIP_TO_FIRST,
                                    g_param_spec_boolean("skip-to-first", "Skip to first buffer", "Don't produce buffers before the first one we receive", DEFAULT_SKIP_TO_FIRST, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
static gboolean gst_video_rate_transformcaps(GstPad * in_pad, GstCaps * in_caps, GstPad * out_pad, GstCaps ** out_caps)
{
    GstCaps *intersect;
    ...
    for (i = 0; i < gst_caps_get_size(intersect); i++)
    {
        ...
        if (gst_structure_has_field(structure, "framerate"))
        {
            ...
            gst_structure_set(copy_structure, "framerate", GST_TYPE_FRACTION_RANGE, 0, 1, G_MAXINT, 1, NULL);
...
static GstCaps *gst_video_rate_getcaps(GstPad * pad)
{
...
static gboolean gst_video_rate_setcaps(GstPad * pad, GstCaps * caps)
{
    GstVideoRate *videorate;
    GstStructure *structure;
    gboolean ret = TRUE;
...
static void gst_video_rate_init(GstVideoRate * videorate, GstVideoRateClass * klass)
{
    videorate->sinkpad = gst_pad_new_from_static_template(&gst_video_rate_sink_template, "sink");
    gst_pad_set_event_function(videorate->sinkpad, GST_DEBUG_FUNCPTR(gst_video_rate_event));
    gst_pad_set_chain_function(videorate->sinkpad, GST_DEBUG_FUNCPTR(gst_video_rate_chain));
    gst_pad_set_getcaps_function(videorate->sinkpad, GST_DEBUG_FUNCPTR(gst_video_rate_getcaps));
    gst_pad_set_setcaps_function(videorate->sinkpad, GST_DEBUG_FUNCPTR(gst_video_rate_setcaps));
    gst_element_add_pad(GST_ELEMENT(videorate), videorate->sinkpad);

    videorate->srcpad = gst_pad_new_from_static_template(&gst_video_rate_src_template, "src");
    gst_pad_set_query_function(videorate->srcpad, GST_DEBUG_FUNCPTR(gst_video_rate_query));
    gst_pad_set_getcaps_function(videorate->srcpad, GST_DEBUG_FUNCPTR(gst_video_rate_getcaps));
    gst_pad_set_setcaps_function(videorate->srcpad, GST_DEBUG_FUNCPTR(gst_video_rate_setcaps));
...
static GstFlowReturn gst_video_rate_flush_prev(GstVideoRate * videorate, gboolean duplicate)
{
    GstFlowReturn res;
    GstBuffer *outbuf;
    ...
    outbuf = gst_buffer_make_metadata_writable(gst_buffer_ref(videorate->prevbuf));
    ...
    if (videorate->to_rate_numerator)
    {
        ...
        videorate->next_ts = videorate->segment.accum + videorate->segment.start + gst_util_uint64_scale(videorate->segment_out, videorate->to_rate_denominator * GST_SECOND, videorate->to_rate_numerator);
    ...
    GST_LOG_OBJECT(videorate, "old is best, dup, pushing buffer outgoing ts %" GST_TIME_FORMAT, GST_TIME_ARGS(push_ts));
...
static void gst_video_rate_notify_drop(GstVideoRate * videorate)
{
...
static void gst_video_rate_notify_duplicate(GstVideoRate * videorate)
{
#if !GLIB_CHECK_VERSION(2,26,0)
        ...
        case GST_EVENT_NEWSEGMENT:
            {
                ...
                if (!update && videorate->prevbuf)
                {
                    ...
                    while (res == GST_FLOW_OK && count <= MAGIC_LIMIT && ((GST_CLOCK_TIME_IS_VALID(videorate->segment.stop) && videorate->next_ts - videorate->segment.accum < videorate->segment.stop) || count < 1))
                    {
                ...
                gst_segment_set_newsegment_full(&videorate->segment, update, rate, arate, format, start, stop, time);

                GST_DEBUG_OBJECT(videorate, "updated segment: %" GST_SEGMENT_FORMAT, &videorate->segment);
                break;
            }
        case GST_EVENT_EOS:
            {
                ...
                if (GST_CLOCK_TIME_IS_VALID(videorate->segment.stop))
                {
                     ...
                     * or only send out the stored buffer if there is no specific stop.
                     * regardless, prevent going loopy in strange cases */
                    while (res == GST_FLOW_OK && count <= MAGIC_LIMIT && ((videorate->next_ts - videorate->segment.accum < videorate->segment.stop) || count < 1))
                    {
                        res = gst_video_rate_flush_prev(videorate, count > 0);
                        count++;
                    }
                ...
                else if (videorate->prevbuf)
                {
                    ...
                    {
                        GstClockTime end_ts = videorate->next_ts + GST_BUFFER_DURATION(videorate->prevbuf);

                        while (res == GST_FLOW_OK && count <= MAGIC_LIMIT && ((videorate->next_ts - videorate->segment.accum < end_ts) || count < 1))
                        {
                            res = gst_video_rate_flush_prev(videorate, count > 0);
                            count++;
                        }
...
format_error:
    {
        GST_WARNING_OBJECT(videorate, "Got segment but doesn't have GST_FORMAT_TIME value");
        ...
        case GST_QUERY_LATENCY:
            {
                ...
                if ((peer = gst_pad_get_peer(videorate->sinkpad)))
                {
                    if ((res = gst_pad_query(peer, query)))
                    {
                        ...
                        GST_DEBUG_OBJECT(videorate, "Peer latency: min %" GST_TIME_FORMAT " max %" GST_TIME_FORMAT, GST_TIME_ARGS(min), GST_TIME_ARGS(max));
                        ...
                        if (videorate->from_rate_numerator != 0)
                        {
                            ...
                            latency = gst_util_uint64_scale(GST_SECOND, videorate->from_rate_denominator, videorate->from_rate_numerator);
                        }
                        else
                        {
                        ...
                        GST_DEBUG_OBJECT(videorate, "Our latency: %" GST_TIME_FORMAT, GST_TIME_ARGS(latency));

                        min += latency;
                        if (max != -1)
                            max += latency;

                        GST_DEBUG_OBJECT(videorate, "Calculated total latency : min %" GST_TIME_FORMAT " max %" GST_TIME_FORMAT, GST_TIME_ARGS(min), GST_TIME_ARGS(max));
...
static GstFlowReturn gst_video_rate_chain(GstPad * pad, GstBuffer * buffer)
{
    ...
    if (videorate->from_rate_denominator == 0 || videorate->to_rate_denominator == 0)
        goto not_negotiated;
    ...
    GST_DEBUG_OBJECT(videorate, "got buffer with timestamp %" GST_TIME_FORMAT, GST_TIME_ARGS(in_ts));
    ...
    if (videorate->prevbuf == NULL)
    {
        ...
        if (!GST_CLOCK_TIME_IS_VALID(videorate->next_ts))
        {
            ...
            if (videorate->skip_to_first)
            {
                ...
                videorate->segment_out = gst_util_uint64_scale(in_ts, videorate->to_rate_numerator, videorate->to_rate_denominator * GST_SECOND) - (videorate->segment.accum + videorate->segment.start);
            }
            else
            {
    ...
    else
    {
        ...
        GST_LOG_OBJECT(videorate, "BEGINNING prev buf %" GST_TIME_FORMAT " new buf %" GST_TIME_FORMAT " outgoing ts %" GST_TIME_FORMAT, GST_TIME_ARGS(prevtime), GST_TIME_ARGS(intime), GST_TIME_ARGS(videorate->next_ts));
        ...
        if (intime < prevtime)
        {
            GST_DEBUG_OBJECT(videorate, "The new buffer (%" GST_TIME_FORMAT ") is before the previous buffer (%" GST_TIME_FORMAT "). Dropping new buffer.", GST_TIME_ARGS(intime), GST_TIME_ARGS(prevtime));
            videorate->drop++;
            if (!videorate->silent)
                gst_video_rate_notify_drop(videorate);
        ...
        do
        {
            ...
            GST_LOG_OBJECT(videorate, "diff with prev %" GST_TIME_FORMAT " diff with new %" GST_TIME_FORMAT " outgoing ts %" GST_TIME_FORMAT, GST_TIME_ARGS(diff1), GST_TIME_ARGS(diff2), GST_TIME_ARGS(videorate->next_ts));
            ...
            if (diff1 <= diff2)
            {
                ...
                if ((res = gst_video_rate_flush_prev(videorate, count > 1)) != GST_FLOW_OK)
                {
                    gst_buffer_unref(buffer);
                    goto done;
                }
        ...
        else if (count == 0)
        {
            ...
            GST_LOG_OBJECT(videorate, "new is best, old never used, drop, outgoing ts %" GST_TIME_FORMAT, GST_TIME_ARGS(videorate->next_ts));
        }
        GST_LOG_OBJECT(videorate,
                       ...
                       G_GUINT64_FORMAT ", dup %" G_GUINT64_FORMAT, GST_TIME_ARGS(diff1), GST_TIME_ARGS(diff2), GST_TIME_ARGS(videorate->next_ts), videorate->in, videorate->out, videorate->drop, videorate->dup);
...
invalid_buffer:
    {
        GST_WARNING_OBJECT(videorate, "Got buffer with GST_CLOCK_TIME_NONE timestamp, discarding it");
        gst_buffer_unref(buffer);
        goto done;
    }
}
...
{
    GST_DEBUG_CATEGORY_INIT(video_rate_debug, "videorate", 0, "VideoRate stream fixer");

    return gst_element_register(plugin, "videorate", GST_RANK_NONE, GST_TYPE_VIDEO_RATE);
}

GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, "videorate", "Adjusts video frames", plugin_init, VERSION, GST_LICENSE, GST_PACKAGE_NAME, GST_PACKAGE_ORIGIN)
