...
static void gst_v4l_color_balance_channel_class_init(GstV4lColorBalanceChannelClass * klass);
static void gst_v4l_color_balance_channel_init(GstV4lColorBalanceChannel * channel);

static const GList *gst_v4l_color_balance_list_channels(GstColorBalance * balance);
static void gst_v4l_color_balance_set_value(GstColorBalance * balance, GstColorBalanceChannel * channel, gint value);
static gint gst_v4l_color_balance_get_value(GstColorBalance * balance, GstColorBalanceChannel * channel);
...
GType gst_v4l_color_balance_channel_get_type(void)
{
    ...
    if (!gst_v4l_color_balance_channel_type)
    {
        ...
        gst_v4l_color_balance_channel_type = g_type_register_static(GST_TYPE_COLOR_BALANCE_CHANNEL, "GstV4lColorBalanceChannel", &v4l_tuner_channel_info, 0);
    }
...
static G_GNUC_UNUSED gboolean gst_v4l_color_balance_contains_channel(GstV4lElement * v4lelement, GstV4lColorBalanceChannel * v4lchannel)
{
    const GList *item;
...
static void gst_v4l_color_balance_set_value(GstColorBalance * balance, GstColorBalanceChannel * channel, gint value)
{
    ...
    GstV4lColorBalanceChannel *v4lchannel = GST_V4L_COLOR_BALANCE_CHANNEL(channel);

    /* assert that we're opened and that we're using a known item */
    g_return_if_fail(GST_V4L_IS_OPEN(v4lelement));
    g_return_if_fail(gst_v4l_color_balance_contains_channel(v4lelement, v4lchannel));
...
static gint gst_v4l_color_balance_get_value(GstColorBalance * balance, GstColorBalanceChannel * channel)
{
    ...
    g_return_val_if_fail(gst_v4l_color_balance_contains_channel(v4lelement, v4lchannel), 0);
