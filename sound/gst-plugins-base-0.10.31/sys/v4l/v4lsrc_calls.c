...
static gboolean gst_v4lsrc_queue_frame(GstV4lSrc * v4lsrc, gint num)
{
    ...
    {
        GST_ELEMENT_ERROR(v4lsrc, RESOURCE, WRITE, (NULL), ("Error queueing a buffer (%d): %s", num, g_strerror(errno)));
        return FALSE;
    }
...
static gboolean gst_v4lsrc_sync_frame(GstV4lSrc * v4lsrc, gint num)
{
...
{
    GST_DEBUG_OBJECT(v4lsrc, "capture properties set to %dx%d, palette %d", width, height, palette);
...
gboolean gst_v4lsrc_capture_init(GstV4lSrc * v4lsrc)
{
    ...
    {
        GST_ELEMENT_ERROR(v4lsrc, RESOURCE, READ, (NULL), ("Error getting buffer information: %s", g_strerror(errno)));
        return FALSE;
    }

    if (v4lsrc->mbuf.frames < MIN_BUFFERS_QUEUED)
    {
        GST_ELEMENT_ERROR(v4lsrc, RESOURCE, READ, (NULL), ("Not enough buffers. We got %d, we want at least %d", v4lsrc->mbuf.frames, MIN_BUFFERS_QUEUED));
        return FALSE;
    }

    GST_INFO_OBJECT(v4lsrc, "Got %d buffers (\'%s\') with total size %d KB", v4lsrc->mbuf.frames, v4l_palette_name[v4lsrc->mmap.format], v4lsrc->mbuf.size / (v4lsrc->mbuf.frames * 1024));

    /* keep track of queued buffers */
    v4lsrc->frame_queue_state = (gint8 *) g_malloc(sizeof(gint8) * v4lsrc->mbuf.frames);
    ...
    /* Map the buffers */
    GST_V4LELEMENT(v4lsrc)->buffer = mmap(NULL, v4lsrc->mbuf.size, PROT_READ | PROT_WRITE, MAP_SHARED, GST_V4LELEMENT(v4lsrc)->video_fd, 0);
    if (GST_V4LELEMENT(v4lsrc)->buffer == MAP_FAILED)
    {
        GST_ELEMENT_ERROR(v4lsrc, RESOURCE, OPEN_READ_WRITE, (NULL), ("Error mapping video buffers: %s", g_strerror(errno)));
...
gboolean gst_v4lsrc_grab_frame(GstV4lSrc * v4lsrc, gint * num)
{
    ...
    while (v4lsrc->num_queued < MIN_BUFFERS_QUEUED || v4lsrc->frame_queue_state[v4lsrc->queue_frame] == QUEUE_STATE_READY_FOR_QUEUE)
    {
        while (v4lsrc->frame_queue_state[v4lsrc->queue_frame] != QUEUE_STATE_READY_FOR_QUEUE && !v4lsrc->quit)
        {
            GST_DEBUG_OBJECT(v4lsrc, "Waiting for frames to become available (queued %d < minimum %d)", v4lsrc->num_queued, MIN_BUFFERS_QUEUED);
...
{
    if (!GST_V4L_IS_ACTIVE(GST_V4LELEMENT(v4lsrc)) || !GST_V4L_IS_OPEN(GST_V4LELEMENT(v4lsrc)))
        return NULL;
...
gboolean gst_v4lsrc_requeue_frame(GstV4lSrc * v4lsrc, gint num)
{
    ...
    {
        GST_ELEMENT_ERROR(v4lsrc, RESOURCE, TOO_LAZY, (NULL), ("Invalid state %d (expected %d), can't requeue", v4lsrc->frame_queue_state[num], QUEUE_STATE_SYNCED));
        return FALSE;
    }
...
gboolean gst_v4lsrc_capture_deinit(GstV4lSrc * v4lsrc)
{
    ...
    {
        GST_ELEMENT_ERROR(v4lsrc, RESOURCE, CLOSE, (NULL), ("error munmap'ing capture buffer: %s", g_strerror(errno)));
        return FALSE;
    }
    GST_V4LELEMENT(v4lsrc)->buffer = NULL;
...
gboolean gst_v4lsrc_try_capture(GstV4lSrc * v4lsrc, gint width, gint height, gint palette)
{
    ...
    GST_DEBUG_OBJECT(v4lsrc, "try out %dx%d, palette format %d (%s)", width, height, palette, v4l_palette_name[palette]);
    GST_V4L_CHECK_OPEN(GST_V4LELEMENT(v4lsrc));
    GST_V4L_CHECK_NOT_ACTIVE(GST_V4LELEMENT(v4lsrc));
    ...
    /* Map the buffers */
    buffer = mmap(NULL, vmbuf.size, PROT_READ | PROT_WRITE, MAP_SHARED, GST_V4LELEMENT(v4lsrc)->video_fd, 0);
    if (buffer == MAP_FAILED)
    {
        GST_ELEMENT_ERROR(v4lsrc, RESOURCE, OPEN_READ_WRITE, (NULL), ("Error mapping our try-out buffer: %s", g_strerror(errno)));
        return FALSE;
    }
    ...
    if (ioctl(GST_V4LELEMENT(v4lsrc)->video_fd, VIDIOCMCAPTURE, &vmmap) < 0)
    {
        if (errno != EINVAL)    /* our format failed! */
            GST_ERROR_OBJECT(v4lsrc, "Error queueing our try-out buffer: %s", g_strerror(errno));
...
gboolean gst_v4lsrc_get_fps(GstV4lSrc * v4lsrc, gint * fps_n, gint * fps_d)
{
    ...
    if (fps_index != 0)
    {
        ...
        GST_DEBUG_OBJECT(v4lsrc, "device reports fps of %d/%d (%.4f)", fps_index * 15, 16, fps_index * 15.0 / 16);
...
static void gst_v4lsrc_buffer_class_init(gpointer g_class, gpointer class_data);
...
static GType gst_v4lsrc_buffer_get_type(void)
{
    ...
    if (G_UNLIKELY(_gst_v4lsrc_buffer_type == 0))
    {
        ...
        _gst_v4lsrc_buffer_type = g_type_register_static(GST_TYPE_BUFFER, "GstV4lSrcBuffer", &v4lsrc_buffer_info, 0);
    }
...
static void gst_v4lsrc_buffer_class_init(gpointer g_class, gpointer class_data)
{
    ...
    mini_object_class->finalize = (GstMiniObjectFinalizeFunction) gst_v4lsrc_buffer_finalize;
}
...
GstBuffer *gst_v4lsrc_buffer_new(GstV4lSrc * v4lsrc, gint num)
{
    ...
    duration = gst_util_uint64_scale_int(GST_SECOND, fps_d * v4lsrc->offset, fps_n) - gst_util_uint64_scale_int(GST_SECOND, fps_d * (v4lsrc->offset - 1), fps_n);
