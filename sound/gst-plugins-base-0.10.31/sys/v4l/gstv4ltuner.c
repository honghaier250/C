...
static void gst_v4l_tuner_set_channel(GstTuner * tuner, GstTunerChannel * channel);
...
static void gst_v4l_tuner_set_frequency(GstTuner * tuner, GstTunerChannel * channel, gulong frequency);
static gulong gst_v4l_tuner_get_frequency(GstTuner * tuner, GstTunerChannel * channel);
static gint gst_v4l_tuner_signal_strength(GstTuner * tuner, GstTunerChannel * channel);
...
GType gst_v4l_tuner_channel_get_type(void)
{
    ...
    if (!gst_v4l_tuner_channel_type)
    {
        ...
        gst_v4l_tuner_channel_type = g_type_register_static(GST_TYPE_TUNER_CHANNEL, "GstV4lTunerChannel", &v4l_tuner_channel_info, 0);
    }
...
GType gst_v4l_tuner_norm_get_type(void)
{
    ...
    if (!gst_v4l_tuner_norm_type)
    {
        ...
        gst_v4l_tuner_norm_type = g_type_register_static(GST_TYPE_TUNER_NORM, "GstV4lTunerNorm", &v4l_tuner_norm_info, 0);
    }
...
static G_GNUC_UNUSED gboolean gst_v4l_tuner_contains_channel(GstV4lElement * v4lelement, GstV4lTunerChannel * v4lchannel)
{
    const GList *item;
...
static G_GNUC_UNUSED gboolean gst_v4l_tuner_contains_norm(GstV4lElement * v4lelement, GstV4lTunerNorm * v4lnorm)
{
    const GList *item;
...
static gulong gst_v4l_tuner_get_frequency(GstTuner * tuner, GstTunerChannel * channel)
{
    ...
    g_return_val_if_fail(gst_v4l_tuner_contains_channel(v4lelement, v4lchannel), 0);
...
static gint gst_v4l_tuner_signal_strength(GstTuner * tuner, GstTunerChannel * channel)
{
    ...
    gst_v4l_get_chan_norm(v4lelement, &chan, NULL);
    if (chan == GST_V4L_TUNER_CHANNEL(channel)->index && GST_TUNER_CHANNEL_HAS_FLAG(channel, GST_TUNER_CHANNEL_FREQUENCY))
    {
