...
static void gst_v4lsrc_base_init(gpointer g_class)
{
    ...
    gst_element_class_set_details_simple(gstelement_class, "Video (video4linux/raw) Source", "Source/Video", "Reads raw frames from a video4linux device", "GStreamer maintainers <gstreamer-devel@lists.sourceforge.net>");

    gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&v4l_src_template));
}
...
static void gst_v4lsrc_class_init(GstV4lSrcClass * klass)
{
    ...
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_AUTOPROBE, g_param_spec_boolean("autoprobe", "Autoprobe", "Whether the device should be probed for all possible features", TRUE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_AUTOPROBE_FPS, g_param_spec_boolean("autoprobe-fps", "Autoprobe FPS", "Whether the device should be probed for framerates", TRUE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_COPY_MODE,
                                    g_param_spec_boolean("copy-mode", "Copy mode", "Whether to send out copies of buffers, or direct pointers to the mmap region", TRUE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass),
                                    PROP_TIMESTAMP_OFFSET, g_param_spec_int64("timestamp-offset",
                                                                              "Timestamp offset", "A time offset subtracted from timestamps set on buffers (in ns)", G_MININT64, G_MAXINT64, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
static void gst_v4lsrc_init(GstV4lSrc * v4lsrc, GstV4lSrcClass * klass)
{
...
static void gst_v4lsrc_fixate(GstBaseSrc * bsrc, GstCaps * caps)
{
    ...
    {
        GST_DEBUG_OBJECT(v4lsrc, "device reported w: %d-%d, h: %d-%d", vcap->minwidth, vcap->maxwidth, vcap->minheight, vcap->maxheight);
...
static GstCaps *gst_v4lsrc_palette_to_caps(int palette)
{
    ...
    if (fourcc == GST_MAKE_FOURCC('R', 'G', 'B', ' '))
    {
        ...
        {
            case VIDEO_PALETTE_RGB555:
                caps = gst_caps_from_string("video/x-raw-rgb, " "bpp = (int) 16, " "depth = (int) 15, " "endianness = (int) BYTE_ORDER, " "red_mask = 0x7c00, " "green_mask = 0x03e0, " "blue_mask = 0x001f");
                break;
            case VIDEO_PALETTE_RGB565:
                caps = gst_caps_from_string("video/x-raw-rgb, " "bpp = (int) 16, " "depth = (int) 16, " "endianness = (int) BYTE_ORDER, " "red_mask = 0xf800, " "green_mask = 0x07f0, " "blue_mask = 0x001f");
                break;
            case VIDEO_PALETTE_RGB24:
                caps = gst_caps_from_string("video/x-raw-rgb, " "bpp = (int) 24, " "depth = (int) 24, " "endianness = (int) BIG_ENDIAN, " "red_mask = 0xFF0000, " "green_mask = 0x00FF00, " "blue_mask = 0x0000FF");
                break;
            case VIDEO_PALETTE_RGB32:
                caps = gst_caps_from_string("video/x-raw-rgb, " "bpp = (int) 32, " "depth = (int) 24, " "endianness = (int) BIG_ENDIAN, " "red_mask = 0xFF000000, " "green_mask = 0x00FF0000, " "blue_mask = 0x0000FF00");
                break;
            default:
...
static GstCaps *gst_v4lsrc_get_any_caps(void)
{
    gint i;
...
static GstCaps *gst_v4lsrc_get_caps(GstBaseSrc * src)
{
    ...
    if (!v4lsrc->colorspaces)
    {
        ...
        for (i = 0; all_palettes[i] != -1; i++)
        {
            ...
            if (!gst_v4lsrc_try_capture(v4lsrc, width, height, all_palettes[i]))
                continue;
            GST_DEBUG_OBJECT(v4lsrc, "Added palette %d (%s) to supported list", all_palettes[i], gst_v4lsrc_palette_name(all_palettes[i]));
            v4lsrc->colorspaces = g_list_append(v4lsrc->colorspaces, GINT_TO_POINTER(all_palettes[i]));
        }
        GST_DEBUG_OBJECT(v4lsrc, "%d palette(s) supported", g_list_length(v4lsrc->colorspaces));
    ...
    for (item = v4lsrc->colorspaces; item != NULL; item = item->next)
    {
        ...
        GST_DEBUG_OBJECT(v4lsrc, "Device reports w: %d-%d, h: %d-%d, fps: %d/%d for palette %d", vcap->minwidth, vcap->maxwidth, vcap->minheight, vcap->maxheight, fps_n, fps_d, GPOINTER_TO_INT(item->data));

        if (vcap->minwidth < vcap->maxwidth)
        {
            gst_caps_set_simple(one, "width", GST_TYPE_INT_RANGE, vcap->minwidth, vcap->maxwidth, NULL);
        }
        else
        {
        ...
        {
            gst_caps_set_simple(one, "height", GST_TYPE_INT_RANGE, vcap->minheight, vcap->maxheight, NULL);
        }
        else
        {
        ...
        {
            gst_caps_set_simple(one, "framerate", GST_TYPE_FRACTION_RANGE, 1, 1, 100, 1, NULL);
        }
...
static gboolean gst_v4lsrc_set_caps(GstBaseSrc * src, GstCaps * caps)
{
    ...
    if (new_fps)
    {
        GST_DEBUG_OBJECT(v4lsrc, "linking with %dx%d at %d/%d fps", w, h, gst_value_get_fraction_numerator(new_fps), gst_value_get_fraction_denominator(new_fps));

        if (gst_value_get_fraction_numerator(new_fps) != cur_fps_n || gst_value_get_fraction_denominator(new_fps) != cur_fps_d)
        {
            int fps_index = (gst_value_get_fraction_numerator(new_fps) * 16) / (gst_value_get_fraction_denominator(new_fps) * 15);
    ...
    if (palette == -1)
    {
        GST_WARNING_OBJECT(v4lsrc, "palette for fourcc %" GST_FOURCC_FORMAT " is -1, refusing link", GST_FOURCC_ARGS(fourcc));
        return FALSE;
    }

    GST_DEBUG_OBJECT(v4lsrc, "trying to set_capture %dx%d, palette %d", w, h, palette);
    /* this only fills in v4lsrc->mmap values */
    if (!gst_v4lsrc_set_capture(v4lsrc, w, h, palette))
    {
        GST_WARNING_OBJECT(v4lsrc, "could not set_capture %dx%d, palette %d", w, h, palette);
        return FALSE;
    }
    ...
    {
        GST_DEBUG_OBJECT(v4lsrc, "failed trying palette %d for %dx%d", palette, w, h);
        return FALSE;
    }
        ...
        case GST_QUERY_LATENCY:
            {
                ...
                GST_DEBUG_OBJECT(bsrc, "report latency min %" GST_TIME_FORMAT " max %" GST_TIME_FORMAT, GST_TIME_ARGS(min_latency), GST_TIME_ARGS(max_latency));
...
static GstFlowReturn gst_v4lsrc_create(GstPushSrc * src, GstBuffer ** buf)
{
    GstV4lSrc *v4lsrc;
    gint num;
