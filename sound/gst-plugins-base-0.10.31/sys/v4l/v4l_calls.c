...
gboolean gst_v4l_get_capabilities(GstV4lElement * v4lelement)
{
    ...
    {
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("error getting capabilities %s of from device %s", g_strerror(errno), v4lelement->videodev));
        return FALSE;
    }

    if (ioctl(v4lelement->video_fd, VIDIOCGWIN, &(v4lelement->vwin)) < 0)
    {
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("error getting window properties %s of from device %s", g_strerror(errno), v4lelement->videodev));
        return FALSE;
    }
...
gboolean gst_v4l_set_window_properties(GstV4lElement * v4lelement)
{
    ...
    GST_DEBUG_OBJECT(v4lelement, "setting window flags 0x%x to device %s", v4lelement->vwin.flags, v4lelement->videodev);
    GST_V4L_CHECK_OPEN(v4lelement);

    if (ioctl(v4lelement->video_fd, VIDIOCSWIN, &(v4lelement->vwin)) < 0)
    {
        GST_DEBUG_OBJECT(v4lelement, "could not ioctl window properties 0x%x to device %s", v4lelement->vwin.flags, v4lelement->videodev);
        return FALSE;
    }
    ...
    {
        GST_DEBUG_OBJECT(v4lelement, "set 0x%x but got 0x%x back", v4lelement->vwin.flags, vwin.flags);
        return FALSE;
    }
...
gboolean gst_v4l_open(GstV4lElement * v4lelement)
{
    ...
    {
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, NOT_FOUND, (_("No device specified.")), (NULL));
        return FALSE;
    }
    ...
    if (!GST_V4L_IS_OPEN(v4lelement))
    {
        ...
        {
            GST_ELEMENT_ERROR(v4lelement, RESOURCE, NOT_FOUND, (_("Device \"%s\" does not exist."), v4lelement->videodev), (NULL));
            return FALSE;
        }
        if (errno == EBUSY)
        {
            GST_ELEMENT_ERROR(v4lelement, RESOURCE, BUSY, (_("Device \"%s\" is already being used."), v4lelement->videodev), (NULL));
            return FALSE;
        }
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, OPEN_READ_WRITE, (_("Could not open device \"%s\" for reading and writing."), v4lelement->videodev), GST_ERROR_SYSTEM);
        return FALSE;
    }
    ...
    /* device type check */
    if ((GST_IS_V4LSRC(v4lelement) && !(v4lelement->vcap.type & VID_TYPE_CAPTURE)))
    {
...
/*           !(v4lelement->vcap.type & VID_TYPE_MJPEG_DECODER))) { */
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Device opened, but wrong type (0x%x)", v4lelement->vcap.type));
        close(v4lelement->video_fd);
        v4lelement->video_fd = -1;
        return FALSE;
    }

    GST_INFO_OBJECT(v4lelement, "Opened device \'%s\' (\'%s\') successfully", v4lelement->vcap.name, v4lelement->videodev);
    ...
    {
        GstV4lColorBalanceChannel *v4lchannel = g_object_new(GST_TYPE_V4L_COLOR_BALANCE_CHANNEL, NULL);
...
static gint gst_v4l_get_num_chans(GstV4lElement * v4lelement)
{
    ...
    for (i = 0; i < gst_v4l_get_num_chans(v4lelement); i++)
    {
        ...
        if (vchan.flags & VIDEO_VC_TUNER)
        {
            ...
            for (n = 0;; n++)
            {
                ...
                channel->freq_multiplicator = 62.5 * ((vtun.flags & VIDEO_TUNER_LOW) ? 1 : 1000);
...
{
    GST_DEBUG_OBJECT(v4lelement, "setting channel = %d, norm = %d (%s)", channel, norm, norm_name[norm]);
    GST_V4L_CHECK_OPEN(v4lelement);
    //GST_V4L_CHECK_NOT_ACTIVE (v4lelement);
    ...
    {
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Error setting the channel/norm settings: %s", g_strerror(errno)));
        return FALSE;
    }

    if (ioctl(v4lelement->video_fd, VIDIOCGCHAN, &(v4lelement->vchan)) < 0)
    {
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Error getting the channel/norm settings: %s", g_strerror(errno)));
        return FALSE;
    }
...
gboolean gst_v4l_get_signal(GstV4lElement * v4lelement, gint tunernum, guint * signal)
{
    ...
    {
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Error getting tuner signal: %s", g_strerror(errno)));
        return FALSE;
    }
...
gboolean gst_v4l_get_frequency(GstV4lElement * v4lelement, gint tunernum, gulong * frequency)
{
    struct video_tuner vtun;
    GstTunerChannel *channel;
    ...
    {
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Error getting tuner frequency: %s", g_strerror(errno)));
        return FALSE;
    }
...
gboolean gst_v4l_set_frequency(GstV4lElement * v4lelement, gint tunernum, gulong frequency)
{
    struct video_tuner vtun;
    GstTunerChannel *channel;
    ...
    {
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Error setting tuner frequency: %s", g_strerror(errno)));
        return FALSE;
    }
...
gboolean gst_v4l_get_picture(GstV4lElement * v4lelement, GstV4lPictureType type, gint * value)
{
    ...
    GST_DEBUG_OBJECT(v4lelement, "getting picture property type %d (%s)", type, picture_name[type]);
    GST_V4L_CHECK_OPEN(v4lelement);

    if (ioctl(v4lelement->video_fd, VIDIOCGPICT, &vpic) < 0)
    {
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Error getting picture parameters: %s", g_strerror(errno)));
        return FALSE;
    }
    ...
    switch (type)
    {
        ...
        case V4L_PICTURE_SATURATION:
            *value = vpic.colour;
            break;
        default:
            GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Error getting picture parameters: unknown type %d", type));
            return FALSE;
    }
...
gboolean gst_v4l_set_picture(GstV4lElement * v4lelement, GstV4lPictureType type, gint value)
{
    ...
    GST_DEBUG_OBJECT(v4lelement, "setting picture type %d (%s) to value %d", type, picture_name[type], value);
    GST_V4L_CHECK_OPEN(v4lelement);
    ...
    switch (type)
    {
        ...
        case V4L_PICTURE_SATURATION:
            ...
            break;
        default:
            GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Error setting picture parameters: unknown type %d", type));
            return FALSE;
    }

    if (ioctl(v4lelement->video_fd, VIDIOCSPICT, &vpic) < 0)
    {
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Error setting picture parameters: %s", g_strerror(errno)));
        return FALSE;
    }
...
gboolean gst_v4l_get_audio(GstV4lElement * v4lelement, gint audionum, GstV4lAudioType type, gint * value)
{
    struct video_audio vau;

    GST_DEBUG_OBJECT(v4lelement, "getting audio parameter type %d (%s)", type, audio_name[type]);
    GST_V4L_CHECK_OPEN(v4lelement);

    vau.audio = audionum;
    if (ioctl(v4lelement->video_fd, VIDIOCGAUDIO, &vau) < 0)
    {
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Error getting audio parameters: %s", g_strerror(errno)));
        return FALSE;
    }
    ...
    switch (type)
    {
        ...
        case V4L_AUDIO_MODE:
            *value = vau.mode;
            break;
        default:
            GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Error getting audio parameters: unknown type %d", type));
            return FALSE;
    }
...
gboolean gst_v4l_set_audio(GstV4lElement * v4lelement, gint audionum, GstV4lAudioType type, gint value)
{
    struct video_audio vau;

    GST_DEBUG_OBJECT(v4lelement, "setting audio parameter type %d (%s) to value %d", type, audio_name[type], value);
    GST_V4L_CHECK_OPEN(v4lelement);
    ...
    switch (type)
    {
        case V4L_AUDIO_MUTE:
            ...
            {
                GST_ELEMENT_ERROR(v4lelement, CORE, NOT_IMPLEMENTED, (NULL), ("Error setting audio mute: (un)setting mute is not supported"));
                return FALSE;
            }
            if (value)
        ...
        case V4L_AUDIO_VOLUME:
            ...
            {
                GST_ELEMENT_ERROR(v4lelement, CORE, NOT_IMPLEMENTED, (NULL), ("Error setting audio volume: setting volume is not supported"));
                return FALSE;
            }
            vau.volume = value;
            break;
        case V4L_AUDIO_MODE:
            ...
            break;
        default:
            GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Error setting audio parameters: unknown type %d", type));
            return FALSE;
    }

    if (ioctl(v4lelement->video_fd, VIDIOCSAUDIO, &vau) < 0)
    {
        GST_ELEMENT_ERROR(v4lelement, RESOURCE, SETTINGS, (NULL), ("Error setting audio parameters: %s", g_strerror(errno)));
        return FALSE;
    }
