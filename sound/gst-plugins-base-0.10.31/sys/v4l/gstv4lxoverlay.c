...
static void gst_v4l_xoverlay_set_window_handle(GstXOverlay * overlay, guintptr xwindow_id);
...
void gst_v4l_xoverlay_interface_init(GstXOverlayClass * klass)
{
    ...
    GST_DEBUG_CATEGORY_INIT(v4lxv_debug, "v4lxv", 0, "V4L XOverlay interface debugging");
}

static void gst_v4l_xoverlay_open(GstV4lElement * v4lelement)
{
    struct stat s;
    GstV4lXv *v4lxv;
    ...
    {
        gst_v4l_xoverlay_set_window_handle(GST_X_OVERLAY(v4lelement), v4lelement->xwindow_id);
    }
}

static void gst_v4l_xoverlay_close(GstV4lElement * v4lelement)
{
...
static gboolean idle_refresh(gpointer data)
{
    ...
    if (v4lxv)
    {
        ...
        XvPutVideo(v4lxv->dpy, v4lxv->port, v4lelement->xwindow_id, DefaultGC(v4lxv->dpy, DefaultScreen(v4lxv->dpy)), 0, 0, attr.width, attr.height, 0, 0, attr.width, attr.height);
...
static void gst_v4l_xoverlay_set_window_handle(GstXOverlay * overlay, guintptr id)
{
    XID xwindow_id = id;
    ...
    if (change)
    {
        ...
        {
            GST_DEBUG_OBJECT(v4lelement, "Disabling port %lx", v4lelement->xwindow_id);
