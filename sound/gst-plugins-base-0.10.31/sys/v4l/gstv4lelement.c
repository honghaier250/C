...
GST_BOILERPLATE_FULL(GstV4lElement, gst_v4lelement, GstPushSrc, GST_TYPE_PUSH_SRC, gst_v4lelement_init_interfaces);

static void gst_v4lelement_dispose(GObject * object);
static void gst_v4lelement_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_v4lelement_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);

/* element methods */
static GstStateChangeReturn gst_v4lelement_change_state(GstElement * element, GstStateChange transition);

static gboolean gst_v4l_iface_supported(GstImplementsInterface * iface, GType iface_type)
{
    GstV4lElement *v4lelement = GST_V4LELEMENT(iface);

#ifdef HAVE_XVIDEO
    g_assert(iface_type == GST_TYPE_TUNER || iface_type == GST_TYPE_X_OVERLAY || iface_type == GST_TYPE_COLOR_BALANCE);
#else
    g_assert(iface_type == GST_TYPE_TUNER || iface_type == GST_TYPE_COLOR_BALANCE);
#endif
...
static void gst_v4l_interface_init(GstImplementsInterfaceClass * klass)
{
    /* default virtual functions */
    klass->supported = gst_v4l_iface_supported;
}

static const GList *gst_v4l_probe_get_properties(GstPropertyProbe * probe)
{
...
#ifdef HAVE_GUDEV
static gboolean gst_v4l_class_probe_devices_with_udev(GstV4lElementClass * klass, gboolean check)
{
    ...
    if (!check)
    {
        ...
        while (item)
        {
            ...
            if (api == 0)
            {
                GST_WARNING("Couldn't retrieve ID_V4L_VERSION, silly udev installation?");
            }
            if ((api == 1 || api == 0))
            {
...
static gboolean gst_v4l_class_probe_devices(GstV4lElementClass * klass, gboolean check)
{
    if (!check)
    {
...
static void gst_v4l_probe_probe_property(GstPropertyProbe * probe, guint prop_id, const GParamSpec * pspec)
{
...
static gboolean gst_v4l_probe_needs_probe(GstPropertyProbe * probe, guint prop_id, const GParamSpec * pspec)
{
    GstV4lElementClass *klass = GST_V4LELEMENT_GET_CLASS(probe);
    gboolean ret = FALSE;
...
static GValueArray *gst_v4l_class_list_devices(GstV4lElementClass * klass)
{
    GValueArray *array;
    GValue value = { 0 };
    GList *item;
...
static GValueArray *gst_v4l_probe_get_values(GstPropertyProbe * probe, guint prop_id, const GParamSpec * pspec)
{
    GstV4lElementClass *klass = GST_V4LELEMENT_GET_CLASS(probe);
...
static void gst_v4l_property_probe_interface_init(GstPropertyProbeInterface * iface)
{
...
static GType gst_v4l_device_get_type(void)
{
...
static void gst_v4lelement_init_interfaces(GType type)
{
    ...
    g_type_add_interface_static(type, GST_TYPE_IMPLEMENTS_INTERFACE, &v4liface_info);
...
#endif
    g_type_add_interface_static(type, GST_TYPE_COLOR_BALANCE, &v4l_colorbalance_info);
    g_type_add_interface_static(type, GST_TYPE_PROPERTY_PROBE, &v4l_propertyprobe_info);
}
...
static void gst_v4lelement_base_init(gpointer g_class)
{
    ...
    GST_DEBUG_CATEGORY_INIT(v4lelement_debug, "v4lelement", 0, "V4L Base Class debug");
}
...
static void gst_v4lelement_class_init(GstV4lElementClass * klass)
{
    ...
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_DEVICE, g_param_spec_string("device", "Device", "Device location", NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_DEVICE_NAME, g_param_spec_string("device-name", "Device name", "Name of the device", NULL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(G_OBJECT_CLASS(klass), PROP_FLAGS, g_param_spec_flags("flags", "Flags", "Device type flags", GST_TYPE_V4L_DEVICE_FLAGS, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

}

static void gst_v4lelement_init(GstV4lElement * v4lelement, GstV4lElementClass * klass)
{
    /* some default values */
