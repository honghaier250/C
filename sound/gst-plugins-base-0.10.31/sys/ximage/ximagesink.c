...
static void gst_ximagesink_ximage_destroy(GstXImageSink * ximagesink, GstXImageBuffer * ximage);
...
static GstStaticPadTemplate gst_ximagesink_sink_template_factory = GST_STATIC_PAD_TEMPLATE("sink",
                                                                                           GST_PAD_SINK,
                                                                                           GST_PAD_ALWAYS,
                                                                                           GST_STATIC_CAPS("video/x-raw-rgb, " "framerate = (fraction) [ 0, MAX ], " "width = (int) [ 1, MAX ], " "height = (int) [ 1, MAX ]"));
...
static void gst_ximage_buffer_finalize(GstXImageBuffer * ximage)
{
    ...
    if (running == FALSE)
    {
        ...
        GST_DEBUG_OBJECT(ximagesink, "destroy image %p because the sink is shutting down", ximage);
        gst_ximagesink_ximage_destroy(ximagesink, ximage);
    }
    else if ((ximage->width != GST_VIDEO_SINK_WIDTH(ximagesink)) || (ximage->height != GST_VIDEO_SINK_HEIGHT(ximagesink)))
    {
        /* If our geometry changed we can't reuse that image. */
        GST_DEBUG_OBJECT(ximagesink, "destroy image %p as its size changed %dx%d vs current %dx%d", ximage, ximage->width, ximage->height, GST_VIDEO_SINK_WIDTH(ximagesink), GST_VIDEO_SINK_HEIGHT(ximagesink));
        gst_ximagesink_ximage_destroy(ximagesink, ximage);
    }
    else
    {
    ...
    if (!recycled)
        GST_MINI_OBJECT_CLASS(ximage_buffer_parent_class)->finalize(GST_MINI_OBJECT(ximage));

beach:
    return;
}

static void gst_ximage_buffer_free(GstXImageBuffer * ximage)
{
...
static void gst_ximage_buffer_init(GstXImageBuffer * ximage_buffer, gpointer g_class)
{
#ifdef HAVE_XSHM
...
static void gst_ximage_buffer_class_init(gpointer g_class, gpointer class_data)
{
    GstMiniObjectClass *mini_object_class = GST_MINI_OBJECT_CLASS(g_class);

    ximage_buffer_parent_class = g_type_class_peek_parent(g_class);

    mini_object_class->finalize = (GstMiniObjectFinalizeFunction) gst_ximage_buffer_finalize;
}

static GType gst_ximage_buffer_get_type(void)
{
    ...
    if (G_UNLIKELY(_gst_ximage_buffer_type == 0))
    {
        ...
        _gst_ximage_buffer_type = g_type_register_static(GST_TYPE_BUFFER, "GstXImageBuffer", &ximage_buffer_info, 0);
    }
...
static int gst_ximagesink_handle_xerror(Display * display, XErrorEvent * xevent)
{
    char error_msg[1024];
...
static gboolean gst_ximagesink_check_xshm_calls(GstXImageSink * ximagesink, GstXContext * xcontext)
{
    XImage *ximage;
    ...
    ximage = XShmCreateImage(xcontext->disp, xcontext->visual, xcontext->depth, ZPixmap, NULL, &SHMInfo, 1, 1);
    ...
    if (SHMInfo.shmid == -1)
    {
        GST_WARNING("could not get shared memory of %" G_GSIZE_FORMAT " bytes", size);
        goto beach;
    }
...
static GstXImageBuffer *gst_ximagesink_ximage_new(GstXImageSink * ximagesink, GstCaps * caps)
{
    ...
    if (!gst_structure_get_int(structure, "width", &ximage->width) || !gst_structure_get_int(structure, "height", &ximage->height))
    {
        GST_WARNING("failed getting geometry from caps %" GST_PTR_FORMAT, caps);
    }

    GST_DEBUG_OBJECT(ximagesink, "creating image %p (%dx%d)", ximage, ximage->width, ximage->height);
    ...
    {
        ximage->ximage = XShmCreateImage(ximagesink->xcontext->disp, ximagesink->xcontext->visual, ximagesink->xcontext->depth, ZPixmap, NULL, &ximage->SHMInfo, ximage->width, ximage->height);
        if (!ximage->ximage || error_caught)
        {
            ...
            XSetErrorHandler(handler);
            /* Push an error */
            GST_ELEMENT_ERROR(ximagesink, RESOURCE, WRITE, ("Failed to create output image buffer of %dx%d pixels", ximage->width, ximage->height), ("could not XShmCreateImage a %dx%d image", ximage->width, ximage->height));
            goto beach;
        }

        /* we have to use the returned bytes_per_line for our shm size */
        ximage->size = ximage->ximage->bytes_per_line * ximage->ximage->height;
        GST_LOG_OBJECT(ximagesink, "XShm image size is %" G_GSIZE_FORMAT ", width %d, stride %d", ximage->size, ximage->width, ximage->ximage->bytes_per_line);

        ximage->SHMInfo.shmid = shmget(IPC_PRIVATE, ximage->size, IPC_CREAT | 0777);
        if (ximage->SHMInfo.shmid == -1)
        {
            g_mutex_unlock(ximagesink->x_lock);
            GST_ELEMENT_ERROR(ximagesink, RESOURCE, WRITE, ("Failed to create output image buffer of %dx%d pixels", ximage->width, ximage->height), ("could not get shared memory of %" G_GSIZE_FORMAT " bytes", ximage->size));
            goto beach;
        }
        ...
        {
            g_mutex_unlock(ximagesink->x_lock);
            GST_ELEMENT_ERROR(ximagesink, RESOURCE, WRITE, ("Failed to create output image buffer of %dx%d pixels", ximage->width, ximage->height), ("Failed to shmat: %s", g_strerror(errno)));
        ...
        if (XShmAttach(ximagesink->xcontext->disp, &ximage->SHMInfo) == 0)
        {
            ...
            g_mutex_unlock(ximagesink->x_lock);
            GST_ELEMENT_ERROR(ximagesink, RESOURCE, WRITE, ("Failed to create output image buffer of %dx%d pixels", ximage->width, ximage->height), ("Failed to XShmAttach"));
            goto beach;
        }
...
#endif                            /* HAVE_XSHM */
    {
        ...
        ximage->ximage = XCreateImage(ximagesink->xcontext->disp, ximagesink->xcontext->visual, ximagesink->xcontext->depth, ZPixmap, 0, NULL, ximage->width, ximage->height, ximagesink->xcontext->bpp, 0);
        if (!ximage->ximage || error_caught)
        {
            g_mutex_unlock(ximagesink->x_lock);
            /* Reset error handler */
            ...
            XSetErrorHandler(handler);
            /* Push an error */
            GST_ELEMENT_ERROR(ximagesink, RESOURCE, WRITE, ("Failed to create output image buffer of %dx%d pixels", ximage->width, ximage->height), ("could not XCreateImage a %dx%d image", ximage->width, ximage->height));
            goto beach;
        }
        ...
        {
            GST_WARNING_OBJECT(ximagesink, "returned stride not a multiple of 4 as " "usually assumed");
        }
        ...
        allocsize = GST_ROUND_UP_4(ximage->ximage->bytes_per_line) * ximage->ximage->height;
        ximage->ximage->data = g_malloc(allocsize);
        GST_LOG_OBJECT(ximagesink, "non-XShm image size is %" G_GSIZE_FORMAT " (alloced: %u), width %d, " "stride %d", ximage->size, allocsize, ximage->width, ximage->ximage->bytes_per_line);
...
static void gst_ximagesink_xwindow_draw_borders(GstXImageSink * ximagesink, GstXWindow * xwindow, GstVideoRectangle rect)
{
    g_return_if_fail(GST_IS_XIMAGESINK(ximagesink));
    g_return_if_fail(xwindow != NULL);

    XSetForeground(ximagesink->xcontext->disp, xwindow->gc, ximagesink->xcontext->black);

    /* Left border */
    if (rect.x > 0)
    {
        XFillRectangle(ximagesink->xcontext->disp, xwindow->win, xwindow->gc, 0, 0, rect.x, xwindow->height);
    }

    /* Right border */
    if ((rect.x + rect.w) < xwindow->width)
    {
        XFillRectangle(ximagesink->xcontext->disp, xwindow->win, xwindow->gc, rect.x + rect.w, 0, xwindow->width, xwindow->height);
    }

    /* Top border */
    if (rect.y > 0)
    {
        XFillRectangle(ximagesink->xcontext->disp, xwindow->win, xwindow->gc, 0, 0, xwindow->width, rect.y);
    }

    /* Bottom border */
    if ((rect.y + rect.h) < xwindow->height)
    {
        XFillRectangle(ximagesink->xcontext->disp, xwindow->win, xwindow->gc, 0, rect.y + rect.h, xwindow->width, xwindow->height);
    }
}

/* This function puts a GstXImageBuffer on a GstXImageSink's window */
static gboolean gst_ximagesink_ximage_put(GstXImageSink * ximagesink, GstXImageBuffer * ximage)
{
    ...
    if (ximage && ximagesink->cur_image != ximage)
    {
        ...
        ximagesink->cur_image = GST_XIMAGE_BUFFER(gst_buffer_ref(GST_BUFFER_CAST(ximage)));
    }
    ...
    if (ximagesink->xcontext->use_xshm)
    {
        GST_LOG_OBJECT(ximagesink, "XShmPutImage on %p, src: %d, %d - dest: %d, %d, dim: %dx%d, win %dx%d", ximage, 0, 0, result.x, result.y, result.w, result.h, ximagesink->xwindow->width, ximagesink->xwindow->height);
        XShmPutImage(ximagesink->xcontext->disp, ximagesink->xwindow->win, ximagesink->xwindow->gc, ximage->ximage, 0, 0, result.x, result.y, result.w, result.h, FALSE);
    }
    else
#endif                            /* HAVE_XSHM */
    {
        GST_LOG_OBJECT(ximagesink, "XPutImage on %p, src: %d, %d - dest: %d, %d, dim: %dx%d, win %dx%d", ximage, 0, 0, result.x, result.y, result.w, result.h, ximagesink->xwindow->width, ximagesink->xwindow->height);
        XPutImage(ximagesink->xcontext->disp, ximagesink->xwindow->win, ximagesink->xwindow->gc, ximage->ximage, 0, 0, result.x, result.y, result.w, result.h);
    }
...
static gboolean gst_ximagesink_xwindow_decorate(GstXImageSink * ximagesink, GstXWindow * window)
{
    ...
    XChangeProperty(ximagesink->xcontext->disp, window->win, hints_atom, hints_atom, 32, PropModeReplace, (guchar *) hints, sizeof(MotifWmHints) / sizeof(long));
...
static void gst_ximagesink_xwindow_set_title(GstXImageSink * ximagesink, GstXWindow * xwindow, const gchar * media_title)
{
    if (media_title)
    {
    ...
    if (xwindow)
    {
        ...
        if (xwindow->internal)
        {
            ...
            {
                title = title_mem = g_strconcat(ximagesink->media_title, " : ", app_name, NULL);
            }
            else if (app_name)
            {
                title = app_name;
...
static GstXWindow *gst_ximagesink_xwindow_new(GstXImageSink * ximagesink, gint width, gint height)
{
    ...
    xwindow->win = XCreateSimpleWindow(ximagesink->xcontext->disp, ximagesink->xcontext->root, 0, 0, xwindow->width, xwindow->height, 0, 0, ximagesink->xcontext->black);
    ...
    if (ximagesink->handle_events)
    {
        ...
        XSelectInput(ximagesink->xcontext->disp, xwindow->win, ExposureMask | StructureNotifyMask | PointerMotionMask | KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask);

        /* Tell the window manager we'd like delete client messages instead of
         * being killed */
        wm_delete = XInternAtom(ximagesink->xcontext->disp, "WM_DELETE_WINDOW", False);
        (void) XSetWMProtocols(ximagesink->xcontext->disp, xwindow->win, &wm_delete, 1);
    }

    xwindow->gc = XCreateGC(ximagesink->xcontext->disp, xwindow->win, 0, &values);
...
static void gst_ximagesink_xwindow_destroy(GstXImageSink * ximagesink, GstXWindow * xwindow)
{
    g_return_if_fail(xwindow != NULL);
    g_return_if_fail(GST_IS_XIMAGESINK(ximagesink));
...
static void gst_ximagesink_xwindow_update_geometry(GstXImageSink * ximagesink)
{
    ...
    XGetWindowAttributes(ximagesink->xcontext->disp, ximagesink->xwindow->win, &attr);
...
static void gst_ximagesink_xwindow_clear(GstXImageSink * ximagesink, GstXWindow * xwindow)
{
    g_return_if_fail(xwindow != NULL);
    g_return_if_fail(GST_IS_XIMAGESINK(ximagesink));
    ...
    XFillRectangle(ximagesink->xcontext->disp, xwindow->win, xwindow->gc, 0, 0, xwindow->width, xwindow->height);
...
/* This function handles XEvents that might be in the queue. It generates
   ...
   and navigation.*/
static void gst_ximagesink_handle_xevents(GstXImageSink * ximagesink)
{
    XEvent e;
    ...
    g_mutex_lock(ximagesink->flow_lock);
    g_mutex_lock(ximagesink->x_lock);
    while (XCheckWindowEvent(ximagesink->xcontext->disp, ximagesink->xwindow->win, PointerMotionMask, &e))
    {
        g_mutex_unlock(ximagesink->x_lock);
        g_mutex_unlock(ximagesink->flow_lock);
    ...
    if (pointer_moved)
    {
        ...
        GST_DEBUG("ximagesink pointer moved over window at %d,%d", pointer_x, pointer_y);
        gst_navigation_send_mouse_event(GST_NAVIGATION(ximagesink), "mouse-move", 0, pointer_x, pointer_y);
    ...
    while (XCheckWindowEvent(ximagesink->xcontext->disp, ximagesink->xwindow->win, KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask, &e))
    {
        KeySym keysym;
        ...
        switch (e.type)
        {
            case ButtonPress:
                ...
                GST_DEBUG("ximagesink button %d pressed over window at %d,%d", e.xbutton.button, e.xbutton.x, e.xbutton.x);
                gst_navigation_send_mouse_event(GST_NAVIGATION(ximagesink), "mouse-button-press", e.xbutton.button, e.xbutton.x, e.xbutton.y);
                break;
            case ButtonRelease:
                GST_DEBUG("ximagesink button %d release over window at %d,%d", e.xbutton.button, e.xbutton.x, e.xbutton.x);
                gst_navigation_send_mouse_event(GST_NAVIGATION(ximagesink), "mouse-button-release", e.xbutton.button, e.xbutton.x, e.xbutton.y);
                break;
            case KeyPress:
            case KeyRelease:
                /* Key pressed/released over our window. We send upstream
                   events for interactivity/navigation */
                GST_DEBUG("ximagesink key %d pressed over window at %d,%d", e.xkey.keycode, e.xkey.x, e.xkey.x);
                g_mutex_lock(ximagesink->x_lock);
                keysym = XKeycodeToKeysym(ximagesink->xcontext->disp, e.xkey.keycode, 0);
                g_mutex_unlock(ximagesink->x_lock);
                if (keysym != NoSymbol)
                {
                    char *key_str = NULL;
                    ...
                    g_mutex_unlock(ximagesink->x_lock);
                    gst_navigation_send_key_event(GST_NAVIGATION(ximagesink), e.type == KeyPress ? "key-press" : "key-release", key_str);

                }
                else
                {
                    gst_navigation_send_key_event(GST_NAVIGATION(ximagesink), e.type == KeyPress ? "key-press" : "key-release", "unknown");
                }
                break;
            default:
                GST_DEBUG_OBJECT(ximagesink, "ximagesink unhandled X event (%d)", e.type);
        }
        g_mutex_lock(ximagesink->flow_lock);
        g_mutex_lock(ximagesink->x_lock);
    }

    while (XCheckWindowEvent(ximagesink->xcontext->disp, ximagesink->xwindow->win, ExposureMask | StructureNotifyMask, &e))
    {
        switch (e.type)
        {
            case Expose:
            ...
            case ClientMessage:
                {
                    ...
                    if (wm_delete == (Atom) e.xclient.data.l[0])
                    {
                        ...
                        GST_ELEMENT_ERROR(ximagesink, RESOURCE, NOT_FOUND, ("Output window was closed"), (NULL));
...
static gpointer gst_ximagesink_event_thread(GstXImageSink * ximagesink)
{
    g_return_val_if_fail(GST_IS_XIMAGESINK(ximagesink), NULL);
...
static void gst_ximagesink_manage_event_thread(GstXImageSink * ximagesink)
{
    GThread *thread = NULL;
    ...
    if (ximagesink->handle_expose || ximagesink->handle_events)
    {
        if (!ximagesink->event_thread)
        {
            ...
            GST_DEBUG_OBJECT(ximagesink, "run xevent thread, expose %d, events %d", ximagesink->handle_expose, ximagesink->handle_events);
            ximagesink->running = TRUE;
            ximagesink->event_thread = g_thread_create((GThreadFunc) gst_ximagesink_event_thread, ximagesink, TRUE, NULL);
        }
    }
    else
    {
        if (ximagesink->event_thread)
        {
            GST_DEBUG_OBJECT(ximagesink, "stop xevent thread, expose %d, events %d", ximagesink->handle_expose, ximagesink->handle_events);
...
static void gst_ximagesink_calculate_pixel_aspect_ratio(GstXContext * xcontext)
{
...
/* This function gets the X Display and global info about it. Everything is
   ...
   image creation */
static GstXContext *gst_ximagesink_xcontext_get(GstXImageSink * ximagesink)
{
    ...
    if (!xcontext->disp)
    {
        g_mutex_unlock(ximagesink->x_lock);
        g_free(xcontext);
        GST_ELEMENT_ERROR(ximagesink, RESOURCE, WRITE, ("Could not initialise X output"), ("Could not open display"));
        return NULL;
    }
    ...
    GST_DEBUG_OBJECT(ximagesink, "X reports %dx%d pixels and %d mm x %d mm", xcontext->width, xcontext->height, xcontext->widthmm, xcontext->heightmm);
    ...
    if (!px_formats)
    {
        ...
        g_free(xcontext->par);
        g_free(xcontext);
        GST_ELEMENT_ERROR(ximagesink, RESOURCE, WRITE, ("Could not get supported pixmap formats"), (NULL));
        return NULL;
    }
...
#ifdef HAVE_XSHM
    if (XShmQueryExtension(xcontext->disp) && gst_ximagesink_check_xshm_calls(ximagesink, xcontext))
    {
    ...
    xcontext->caps = gst_caps_new_simple("video/x-raw-rgb",
                                         ...
                                         "blue_mask", G_TYPE_INT, xcontext->visual->blue_mask, "width", GST_TYPE_INT_RANGE, 1, G_MAXINT, "height", GST_TYPE_INT_RANGE, 1, G_MAXINT, "framerate", GST_TYPE_FRACTION_RANGE, 0, 1, G_MAXINT, 1, NULL);
    if (ximagesink->par)
    {
        int nom, den;
        ...
        gst_caps_set_simple(xcontext->caps, "pixel-aspect-ratio", GST_TYPE_FRACTION, nom, den, NULL);
    }
...
static void gst_ximagesink_xcontext_clear(GstXImageSink * ximagesink)
{
...
static void gst_ximagesink_bufferpool_clear(GstXImageSink * ximagesink)
{
    ...
    while (ximagesink->buffer_pool)
    {
        ...
        ximagesink->buffer_pool = g_slist_delete_link(ximagesink->buffer_pool, ximagesink->buffer_pool);
        gst_ximage_buffer_free(ximage);
    }
...
static GstCaps *gst_ximagesink_getcaps(GstBaseSink * bsink)
{
    ...
    caps = gst_caps_copy(gst_pad_get_pad_template_caps(GST_BASE_SINK(ximagesink)->sinkpad));
    for (i = 0; i < gst_caps_get_size(caps); ++i)
    {
        ...
        if (ximagesink->par)
        {
            ...
            nom = gst_value_get_fraction_numerator(ximagesink->par);
            den = gst_value_get_fraction_denominator(ximagesink->par);
            gst_structure_set(structure, "pixel-aspect-ratio", GST_TYPE_FRACTION, nom, den, NULL);
        }
    }
...
static gboolean gst_ximagesink_setcaps(GstBaseSink * bsink, GstCaps * caps)
{
    GstXImageSink *ximagesink;
    gboolean ret = TRUE;
    ...
    GST_DEBUG_OBJECT(ximagesink, "sinkconnect possible caps %" GST_PTR_FORMAT " with given caps %" GST_PTR_FORMAT, ximagesink->xcontext->caps, caps);
    ...
    if (GST_VIDEO_SINK_WIDTH(ximagesink) <= 0 || GST_VIDEO_SINK_HEIGHT(ximagesink) <= 0)
    {
        GST_ELEMENT_ERROR(ximagesink, CORE, NEGOTIATION, (NULL), ("Invalid image size."));
        return FALSE;
    }
    ...
    if (!ximagesink->xwindow)
    {
        ximagesink->xwindow = gst_ximagesink_xwindow_new(ximagesink, GST_VIDEO_SINK_WIDTH(ximagesink), GST_VIDEO_SINK_HEIGHT(ximagesink));
    }
    ...
    /* If our ximage has changed we destroy it, next chain iteration will create
       a new one */
    if ((ximagesink->ximage) && ((GST_VIDEO_SINK_WIDTH(ximagesink) != ximagesink->ximage->width) || (GST_VIDEO_SINK_HEIGHT(ximagesink) != ximagesink->ximage->height)))
    {
        GST_DEBUG_OBJECT(ximagesink, "our image is not usable anymore, unref %p", ximagesink->ximage);
...
static GstStateChangeReturn gst_ximagesink_change_state(GstElement * element, GstStateChange transition)
{
    GstXImageSink *ximagesink;
    ...
    switch (transition)
    {
        case GST_STATE_CHANGE_NULL_TO_READY:
            ...
            GST_DEBUG_OBJECT(ximagesink, "XSynchronize called with %s", ximagesink->synchronous ? "TRUE" : "FALSE");
            g_mutex_lock(ximagesink->x_lock);
...
static void gst_ximagesink_get_times(GstBaseSink * bsink, GstBuffer * buf, GstClockTime * start, GstClockTime * end)
{
    GstXImageSink *ximagesink;
    ...
    if (GST_BUFFER_TIMESTAMP_IS_VALID(buf))
    {
        *start = GST_BUFFER_TIMESTAMP(buf);
        ...
        else
        {
            ...
            {
                *end = *start + gst_util_uint64_scale_int(GST_SECOND, ximagesink->fps_d, ximagesink->fps_n);
            }
        }
    }
}

static GstFlowReturn gst_ximagesink_show_frame(GstVideoSink * vsink, GstBuffer * buf)
{
    GstXImageSink *ximagesink;
    ...
    else
    {
        ...
        if (!ximagesink->ximage)
        {
            ...
            ximagesink->ximage = gst_ximagesink_ximage_new(ximagesink, GST_BUFFER_CAPS(buf));
            if (!ximagesink->ximage)
            ...
            {
                GST_ELEMENT_ERROR(ximagesink, RESOURCE, WRITE, ("Failed to create output image buffer of %dx%d pixels", ximagesink->ximage->width, ximagesink->ximage->height), ("XServer allocated buffer size did not match input buffer"));
        ...
        memcpy(GST_BUFFER_DATA(ximagesink->ximage), GST_BUFFER_DATA(buf), MIN(GST_BUFFER_SIZE(buf), ximagesink->ximage->size));
...
static gboolean gst_ximagesink_event(GstBaseSink * sink, GstEvent * event)
{
        ...
        case GST_EVENT_TAG:
            {
                ...
                if (title)
                {
                    ...
                    gst_ximagesink_xwindow_set_title(ximagesink, ximagesink->xwindow, title);
 ...
 * resized buffer.
 */
static GstFlowReturn gst_ximagesink_buffer_alloc(GstBaseSink * bsink, guint64 offset, guint size, GstCaps * caps, GstBuffer ** buf)
{
    GstXImageSink *ximagesink;
    GstXImageBuffer *ximage = NULL;
    GstStructure *structure = NULL;
    ...
    GST_LOG_OBJECT(ximagesink, "a buffer of %d bytes was requested with caps %" GST_PTR_FORMAT " and offset %" G_GUINT64_FORMAT, size, caps, offset);
    ...
    {
        GST_WARNING_OBJECT(ximagesink, "invalid caps for buffer allocation %" GST_PTR_FORMAT, caps);
    ...
    {
        GST_LOG_OBJECT(ximagesink, "enforcing aspect ratio in reverse caps " "negotiation");
        gst_video_sink_center_rect(src, dst, &result, TRUE);
    }
    else
    {
        GST_LOG_OBJECT(ximagesink, "trying to resize to window geometry " "ignoring aspect ratio");
        result.x = result.y = 0;
        result.w = dst.w;
        result.h = dst.h;
    }
    ...
    if (width != result.w || height != result.h)
    {
        ...
        if (ximagesink->par)
        {
            nom = gst_value_get_fraction_numerator(ximagesink->par);
            den = gst_value_get_fraction_denominator(ximagesink->par);
            gst_structure_set(desired_struct, "pixel-aspect-ratio", GST_TYPE_FRACTION, nom, den, NULL);
        }
        else if (ximagesink->xcontext->par)
        {
        ...
        if (!ximagesink->xcontext->last_caps || !gst_caps_is_equal(desired_caps, ximagesink->xcontext->last_caps))
        {
            caps_accepted = gst_pad_peer_accept_caps(GST_VIDEO_SINK_PAD(ximagesink), desired_caps);
        ...
        {
            GST_DEBUG("peer pad does not accept our desired caps %" GST_PTR_FORMAT, desired_caps);
...
static gboolean gst_ximagesink_interface_supported(GstImplementsInterface * iface, GType type)
{
...
static void gst_ximagesink_navigation_send_event(GstNavigation * navigation, GstStructure * structure)
{
...
static void gst_ximagesink_set_window_handle(GstXOverlay * overlay, guintptr id)
{
    XID xwindow_id = id;
    ...
    if (!ximagesink->xcontext && !(ximagesink->xcontext = gst_ximagesink_xcontext_get(ximagesink)))
    {
        g_mutex_unlock(ximagesink->flow_lock);
    ...
    if (xwindow_id == 0)
    {
        ...
        {
            xwindow = gst_ximagesink_xwindow_new(ximagesink, GST_VIDEO_SINK_WIDTH(ximagesink), GST_VIDEO_SINK_HEIGHT(ximagesink));
        }
    }
    else
    {
        ...
        if (ximagesink->handle_events)
        {
            XSelectInput(ximagesink->xcontext->disp, xwindow->win, ExposureMask | StructureNotifyMask | PointerMotionMask | KeyPressMask | KeyReleaseMask);
        }
...
static void gst_ximagesink_set_event_handling(GstXOverlay * overlay, gboolean handle_events)
{
    GstXImageSink *ximagesink = GST_XIMAGESINK(overlay);
    ...
    if (handle_events)
    {
        ...
        {
            XSelectInput(ximagesink->xcontext->disp, ximagesink->xwindow->win, ExposureMask | StructureNotifyMask | PointerMotionMask | KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask);
        }
        else
        {
            XSelectInput(ximagesink->xcontext->disp, ximagesink->xwindow->win, ExposureMask | StructureNotifyMask | PointerMotionMask | KeyPressMask | KeyReleaseMask);
        }
    }
    else
    {
...
static void gst_ximagesink_xoverlay_init(GstXOverlayClass * iface)
{
...
static void gst_ximagesink_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    GstXImageSink *ximagesink;
    ...
    switch (prop_id)
    {
        ...
        case PROP_PIXEL_ASPECT_RATIO:
            {
                ...
                {
                    GST_WARNING_OBJECT(ximagesink, "Could not transform string to aspect ratio");
                    g_free(tmp);
                }
                else
                {
                    GST_DEBUG_OBJECT(ximagesink, "set PAR to %d/%d", gst_value_get_fraction_numerator(tmp), gst_value_get_fraction_denominator(tmp));
                    g_free(ximagesink->par);
                    ximagesink->par = tmp;
                }
            }
        ...
        case PROP_HANDLE_EVENTS:
            gst_ximagesink_set_event_handling(GST_X_OVERLAY(ximagesink), g_value_get_boolean(value));
            gst_ximagesink_manage_event_thread(ximagesink);
            break;
...
static void gst_ximagesink_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
    GstXImageSink *ximagesink;
...
static void gst_ximagesink_finalize(GObject * object)
{
    GstXImageSink *ximagesink;
...
static void gst_ximagesink_init(GstXImageSink * ximagesink)
{
    ximagesink->display_name = NULL;
    ximagesink->xcontext = NULL;
    ximagesink->xwindow = NULL;
...
static void gst_ximagesink_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_set_details_simple(element_class, "Video sink", "Sink/Video", "A standard X based videosink", "Julien Moutte <julien@moutte.net>");

    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&gst_ximagesink_sink_template_factory));
}

static void gst_ximagesink_class_init(GstXImageSinkClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_DISPLAY, g_param_spec_string("display", "Display", "X Display name", NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    g_object_class_install_property(gobject_class, PROP_FORCE_ASPECT_RATIO,
                                    g_param_spec_boolean("force-aspect-ratio", "Force aspect ratio", "When enabled, reverse caps negotiation (scaling) will respect " "original aspect ratio", FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_PIXEL_ASPECT_RATIO, g_param_spec_string("pixel-aspect-ratio", "Pixel Aspect Ratio", "The pixel aspect ratio of the device", "1/1", G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_HANDLE_EVENTS, g_param_spec_boolean("handle-events", "Handle XEvents", "When enabled, XEvents will be selected and handled", TRUE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    gstbasesink_class->buffer_alloc = GST_DEBUG_FUNCPTR(gst_ximagesink_buffer_alloc);
...
GType gst_ximagesink_get_type(void)
{
    ...
    if (!ximagesink_type)
    {
        ...
        ximagesink_type = g_type_register_static(GST_TYPE_VIDEO_SINK, "GstXImageSink", &ximagesink_info, 0);

        g_type_add_interface_static(ximagesink_type, GST_TYPE_IMPLEMENTS_INTERFACE, &iface_info);
        g_type_add_interface_static(ximagesink_type, GST_TYPE_NAVIGATION, &navigation_info);
        g_type_add_interface_static(ximagesink_type, GST_TYPE_X_OVERLAY, &overlay_info);
