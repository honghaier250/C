...
static ChainCodec get_page_codec(ogg_page * page)
{
    ...
    if (ogg_stream_pagein(&state, page) == 0)
    {
        ...
        {
            if (strncmp((char *) &packet.packet[1], "vorbis", strlen("vorbis")) == 0)
                codec = CODEC_VORBIS;
            else if (strncmp((char *) &packet.packet[1], "theora", strlen("theora")) == 0)
                codec = CODEC_THEORA;
            else if (strncmp((char *) &packet.packet[0], "Speex   ", strlen("Speex   ")) == 0)
                codec = CODEC_SPEEX;
        }
    }
...
static void fail_if_audio(gpointer key, ChainState * state, gpointer data)
{
    fail_if(state->codec == CODEC_VORBIS, "vorbis BOS occurred before theora BOS");
    fail_if(state->codec == CODEC_SPEEX, "speex BOS occurred before theora BOS");
}

static ChainState *validate_ogg_page(ogg_page * page)
{
    gulong serialno;
    gint64 granule;
    ChainState *state;
    ...
    fail_if(ogg_page_packets(page) == 0 && granule != -1, "Must have granulepos -1 when page has no packets, has %" G_GINT64_FORMAT, granule);
    ...
    if (granule != -1)
    {
        fail_unless(granule >= state->last_granule, "Granulepos out-of-order for chain %u: old=%" G_GINT64_FORMAT ", new=" G_GINT64_FORMAT, serialno, state->last_granule, granule);
        state->last_granule = granule;
    }
...
static void is_video(gpointer key, ChainState * state, gpointer data)
{
    if (state->codec == CODEC_THEORA)
        *((gboolean *) data) = TRUE;
}

static gboolean check_chain_final_state(gpointer key, ChainState * state, gpointer data)
{
...
static gboolean eos_buffer_probe(GstPad * pad, GstBuffer * buffer, gpointer unused)
{
    gint ret;
    gint size;
    guint8 *data;
    ...
    if (state)
    {
        ...
        if (has_video && state->codec != CODEC_THEORA)
        {
            if (!GST_BUFFER_FLAG_IS_SET(buffer, GST_BUFFER_FLAG_IN_CAPS))
                fail_unless(GST_BUFFER_FLAG_IS_SET(buffer, GST_BUFFER_FLAG_DELTA_UNIT), "Non-video buffer doesn't have DELTA_UNIT in stream with video");
        }
    }
...
static void start_pipeline(GstElement * bin, GstPad * pad)
{
    GstStateChangeReturn ret;

    ogg_sync_init(&oggsync);

    eos_chain_states = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, g_free);
    probe_id = gst_pad_add_buffer_probe(pad, G_CALLBACK(eos_buffer_probe), NULL);
...
static void stop_pipeline(GstElement * bin, GstPad * pad)
{
    GstStateChangeReturn ret;
    ...
    g_hash_table_foreach_remove(eos_chain_states, (GHRFunc) check_chain_final_state, NULL);
}

static gboolean eos_watch(GstBus * bus, GstMessage * message, GMainLoop * loop)
{
...
static void test_pipeline(const char *pipeline)
{
    GstElement *bin, *sink;
    GstPad *pad, *sinkpad;
    GstBus *bus;
    ...
    fail_unless(GST_PAD_LINK_SUCCESSFUL(linkret), "Could not link to fake sink");
    ...
    /* we're EOS now; make sure oggmux out caps have stream headers on them */
    {
        ...
        fail_unless(gst_structure_has_field_typed(s, "streamheader", GST_TYPE_ARRAY));
...
{
    test_pipeline("audiotestsrc num-buffers=5 ! audioconvert ! vorbisenc ! oggmux");
}
...
GST_START_TEST(test_vorbis_oggmux_unlinked)
{
    ...
    fail_unless_equals_int(gst_element_set_state(pipe, GST_STATE_PAUSED), GST_STATE_CHANGE_SUCCESS);
    /* we expect an error (without any criticals/warnings) */
    msg = gst_bus_timed_pop_filtered(GST_ELEMENT_BUS(pipe), -1, GST_MESSAGE_ERROR);
    gst_message_unref(msg);
    fail_unless_equals_int(gst_element_set_state(pipe, GST_STATE_NULL), GST_STATE_CHANGE_SUCCESS);
...
{
    test_pipeline("videotestsrc num-buffers=5 ! ffmpegcolorspace ! theoraenc ! oggmux");
}
...
{
    test_pipeline("videotestsrc num-buffers=10 ! ffmpegcolorspace ! theoraenc ! queue ! oggmux name=mux " "audiotestsrc num-buffers=2 ! audioconvert ! vorbisenc ! queue ! mux.");
}
...
{
    test_pipeline("videotestsrc num-buffers=2 ! ffmpegcolorspace ! theoraenc ! queue ! oggmux name=mux " "audiotestsrc num-buffers=10 ! audioconvert ! vorbisenc ! queue ! mux.");
}
...
static Suite *oggmux_suite(void)
{
