...
static void check_libtheora(void)
{
...
static void check_buffer_granulepos_from_starttime(GstBuffer * buffer, GstClockTime starttime)
{
    ...
    fail_unless(framecount == expected || framecount == expected + 1, "expected frame count %" G_GUINT64_FORMAT " or %" G_GUINT64_FORMAT ", but got frame count %" G_GUINT64_FORMAT, expected, expected + 1, framecount);
}
...
GST_START_TEST(test_granulepos_offset)
{
    ...
    pipe_str = g_strdup_printf("videotestsrc timestamp-offset=%" G_GUINT64_FORMAT " num-buffers=10 ! video/x-raw-yuv,format=(fourcc)I420,framerate=10/1" " ! theoraenc ! fakesink name=fs0", TIMESTAMP_OFFSET);
...

    {
        ...
        check_buffer_duration(buffer, gst_util_uint64_scale(GST_BUFFER_OFFSET_END(buffer), GST_SECOND, FRAMERATE) - gst_util_uint64_scale(last_granulepos, GST_SECOND, FRAMERATE));
...
GST_START_TEST(test_continuity)
{
    ...
    pipe_str = g_strdup_printf("videotestsrc num-buffers=10" " ! video/x-raw-yuv,format=(fourcc)I420,framerate=10/1" " ! theoraenc ! fakesink name=fs0");
...
GST_START_TEST(test_discontinuity)
{
...

    {
        ...
        check_buffer_is_header(buffer, FALSE);
        fail_unless(GST_BUFFER_IS_DISCONT(buffer), "expected discontinuous buffer yo");
        gst_buffer_unref(buffer);
...
static Suite *theoraenc_suite(void)
{
