...
static void check_buffer_timestamp(GstBuffer * buffer, GstClockTime timestamp)
{
    fail_unless(GST_BUFFER_TIMESTAMP(buffer) == timestamp, "expected timestamp %" GST_TIME_FORMAT ", but got timestamp %" GST_TIME_FORMAT, GST_TIME_ARGS(timestamp), GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buffer)));
}

static void check_buffer_duration(GstBuffer * buffer, GstClockTime duration)
{
    fail_unless(GST_BUFFER_DURATION(buffer) == duration, "expected duration %" GST_TIME_FORMAT ", but got duration %" GST_TIME_FORMAT, GST_TIME_ARGS(duration), GST_TIME_ARGS(GST_BUFFER_DURATION(buffer)));
}

static void check_buffer_granulepos(GstBuffer * buffer, gint64 granulepos)
{
    GstClockTime clocktime;

    fail_unless(GST_BUFFER_OFFSET_END(buffer) == granulepos, "expected granulepos %" G_GUINT64_FORMAT ", but got granulepos %" G_GUINT64_FORMAT, granulepos, GST_BUFFER_OFFSET_END(buffer));

    /* contrary to what we record as TIMESTAMP, we can use OFFSET to check
     * the granulepos correctly here */
    clocktime = gst_util_uint64_scale(GST_BUFFER_OFFSET_END(buffer), 44100, GST_SECOND);

    fail_unless(clocktime == GST_BUFFER_OFFSET(buffer), "expected OFFSET set to clocktime %" GST_TIME_FORMAT ", but got %" GST_TIME_FORMAT, GST_TIME_ARGS(clocktime), GST_TIME_ARGS(GST_BUFFER_OFFSET(buffer)));
}
...
/* this check is here to check that the granulepos we derive from the timestamp
   ...
   the number plus one. */
static void check_buffer_granulepos_from_endtime(GstBuffer * buffer, GstClockTime endtime)
{
    ...
    fail_unless(granulepos == expected || granulepos == expected + 1, "expected granulepos %" G_GUINT64_FORMAT " or %" G_GUINT64_FORMAT ", but got granulepos %" G_GUINT64_FORMAT, expected, expected + 1, granulepos);
}
...
GST_START_TEST(test_granulepos_offset)
{
    ...
    pipe_str = g_strdup_printf("audiotestsrc timestamp-offset=%" G_GUINT64_FORMAT " ! audio/x-raw-int,rate=44100" " ! audioconvert ! vorbisenc ! fakesink", TIMESTAMP_OFFSET);

    bin = gst_parse_launch(pipe_str, &error);
    fail_unless(bin != NULL, "Error parsing pipeline: %s", error ? error->message : "(invalid error)");
    g_free(pipe_str);
...

    {
        ...
        check_buffer_granulepos_from_endtime(buffer, TIMESTAMP_OFFSET + GST_BUFFER_DURATION(buffer));
        ...
        check_buffer_duration(buffer, gst_util_uint64_scale(GST_BUFFER_OFFSET_END(buffer), GST_SECOND, 44100) - gst_util_uint64_scale(last_granulepos, GST_SECOND, 44100));
        check_buffer_granulepos_from_endtime(buffer, next_timestamp + GST_BUFFER_DURATION(buffer));
...
GST_START_TEST(test_timestamps)
{
    ...
    pipe_str = g_strdup_printf("audiotestsrc" " ! audio/x-raw-int,rate=44100 ! audioconvert ! vorbisenc ! fakesink");
...
static gboolean drop_second_data_buffer(GstPad * droppad, GstBuffer * buffer, gpointer unused)
{
...
GST_START_TEST(test_discontinuity)
{
    ...
    pipe_str = g_strdup_printf("audiotestsrc samplesperbuffer=1024" " ! audio/x-raw-int,rate=44100" " ! audioconvert ! vorbisenc ! fakesink");
    ...
    drop_id = gst_pad_add_buffer_probe(droppad, G_CALLBACK(drop_second_data_buffer), NULL);
    gst_buffer_straw_start_pipeline(bin, pad);
       ...
       2048. */
    {
        ...
        fail_unless(last_granulepos == 1024, "unexpected granulepos: %" G_GUINT64_FORMAT, last_granulepos);
    }

    {
         ...
         */
        fail_unless(GST_BUFFER_OFFSET_END(buffer) == 2048, "expected granulepos after gap: %" G_GUINT64_FORMAT, GST_BUFFER_OFFSET_END(buffer));
        fail_unless(GST_BUFFER_IS_DISCONT(buffer), "expected discontinuous buffer");
        gst_buffer_unref(buffer);
    }
...
static Suite *vorbisenc_suite(void)
{
