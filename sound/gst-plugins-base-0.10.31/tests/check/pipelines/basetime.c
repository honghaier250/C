...
static gboolean break_mainloop(gpointer data)
{
    GMainLoop *loop;
...
static gboolean buffer_probe_cb(GstPad * pad, GstBuffer * buffer)
{
    ...
    {
        fail_unless(GST_BUFFER_TIMESTAMP(buffer) != old_ts, "Two buffers had same timestamp");
    }
...
GST_START_TEST(test_basetime_calculation)
{
    ...
    if (asrc == NULL)
    {
        GST_WARNING("Cannot run test. test audio source %s not available", DEFAULT_AUDIOSRC);
    ...
    gst_element_post_message(asrc, gst_message_new_clock_provide(GST_OBJECT(asrc), NULL, TRUE));
...
static Suite *baseaudiosrc_suite(void)
{
