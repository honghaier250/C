...
static gboolean buffer_probe(GstPad * pad, GstMiniObject * obj, gpointer data)
{
    GstBuffer *buffer;
    GstCaps *buffer_caps;
...
GST_START_TEST(test_capsfilter_renegotiation)
{
    ...
    pipeline = gst_parse_launch("videotestsrc num-buffers=200 ! capsfilter " "caps=\"" FIRST_CAPS "\" name=cf ! fakesink name=sink", NULL);
    g_assert(pipeline);
...
static Suite *capsfilter_renegotiation_suite(void)
{
