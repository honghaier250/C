...
GST_START_TEST(test_memory_stream)
{
    ...
    input = G_MEMORY_INPUT_STREAM(g_memory_input_stream_new_from_data(in_data, 512, (GDestroyNotify) g_free));

    output = G_MEMORY_OUTPUT_STREAM(g_memory_output_stream_new(out_data, 512, (GReallocFunc) g_realloc, (GDestroyNotify) g_free));
    out_data = NULL;
...
static Suite *gio_testsuite(void)
{
