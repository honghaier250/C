...
static gboolean tag_event_probe_cb(GstPad * pad, GstEvent * event, GMainLoop * loop)
{
...
GST_START_TEST(test_multifdsink_gdp_tag)
{
    ...
    p1 = gst_parse_launch("audiotestsrc num-buffers=10 ! gdppay" " ! multifdsink name=p1sink", NULL);
    fail_if(p1 == NULL);
    p2 = gst_parse_launch("fdsrc name=p2src ! gdpdepay name=depay" " ! fakesink name=p2sink signal-handoffs=True", NULL);
    fail_if(p2 == NULL);
...
static gboolean buffer_probe_cb(GstPad * pad, GstBuffer * buffer)
{
    if (GST_BUFFER_FLAG_IS_SET(buffer, GST_BUFFER_FLAG_IN_CAPS))
    {
        ...
        for (i = 0; i < 3; ++i)
        {
            ...
            {
                if (memcmp(GST_BUFFER_DATA(buf), GST_BUFFER_DATA(buffer), GST_BUFFER_SIZE(buffer)) == 0)
                {
                    found = TRUE;
                }
            }
...
GST_START_TEST(test_multifdsink_gdp_vorbisenc)
{
    ...
    p1 = gst_parse_launch("audiotestsrc num-buffers=10 ! audioconvert " " ! vorbisenc ! gdppay ! multifdsink name=p1sink", NULL);
    fail_if(p1 == NULL);
    p2 = gst_parse_launch("fdsrc name=p2src ! gdpdepay name=depay" " ! fakesink name=p2sink signal-handoffs=True", NULL);
    fail_if(p2 == NULL);
...
static Suite *streamheader_suite(void)
{
