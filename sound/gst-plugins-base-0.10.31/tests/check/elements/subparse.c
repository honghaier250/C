...
static GstBuffer *buffer_from_static_string(const gchar * s)
{
    GstBuffer *buf;
...
static void setup_subparse(void)
{
    ...
    fail_unless_equals_int(gst_element_set_state(subparse, GST_STATE_PLAYING), GST_STATE_CHANGE_SUCCESS);
}

static void teardown_subparse(void)
{
...
static void test_srt_do_test(SubParseInputChunk * input, guint start_idx, guint num)
{
    guint n;
    ...
    for (n = start_idx; n < start_idx + num; ++n)
    {
        ...
        fail_unless_equals_uint64(GST_BUFFER_DURATION(buf), input[n].to_ts - input[n].from_ts);
        ...
        fail_unless_equals_string(gst_structure_get_name(buffer_caps_struct), "text/x-pango-markup");
    }
...
static void do_test(SubParseInputChunk * input, guint num, const gchar * media_type)
{
    guint n;
    ...
    for (n = 0; n < num; ++n)
    {
        ...
        fail_unless(GST_BUFFER_CAPS(buf) != NULL);
        buffer_caps_struct = gst_caps_get_structure(GST_BUFFER_CAPS(buf), 0);
        fail_unless_equals_string(gst_structure_get_name(buffer_caps_struct), media_type);
    }

    teardown_subparse();
}

static void test_tmplayer_do_test(SubParseInputChunk * input, guint num)
{
    do_test(input, num, "text/plain");
}

static void test_microdvd_do_test(SubParseInputChunk * input, guint num)
{
...
GST_START_TEST(test_tmplayer_multiline)
{
    ...
    test_tmplayer_do_test(tmplayer_multiline_input, G_N_ELEMENTS(tmplayer_multiline_input));
}
...
GST_START_TEST(test_tmplayer_multiline_with_bogus_lines)
{
    ...
    test_tmplayer_do_test(tmplayer_multiline_b_input, G_N_ELEMENTS(tmplayer_multiline_b_input));
}
...
GST_START_TEST(test_tmplayer_style1)
{
    ...
    test_tmplayer_do_test(tmplayer_style1_input, G_N_ELEMENTS(tmplayer_style1_input));
}
...
GST_START_TEST(test_tmplayer_style2)
{
    ...
    test_tmplayer_do_test(tmplayer_style2_input, G_N_ELEMENTS(tmplayer_style2_input));
}
...
GST_START_TEST(test_tmplayer_style3)
{
    ...
    test_tmplayer_do_test(tmplayer_style3_input, G_N_ELEMENTS(tmplayer_style3_input));
}
...
GST_START_TEST(test_tmplayer_style3b)
{
    ...
    test_tmplayer_do_test(tmplayer_style3b_input, G_N_ELEMENTS(tmplayer_style3b_input));
}
...
GST_START_TEST(test_tmplayer_style4)
{
    ...
    test_tmplayer_do_test(tmplayer_style4_input, G_N_ELEMENTS(tmplayer_style4_input));
}
...
GST_START_TEST(test_tmplayer_style4_with_bogus_lines)
{
    ...
    test_tmplayer_do_test(tmplayer_style4b_input, G_N_ELEMENTS(tmplayer_style4b_input));
}
...
GST_START_TEST(test_microdvd_with_fps)
{
    ...
    microdvd_input[0].in = "{1}{1}12,500\n{100}{200}- Hi, Eddie.|- Hiya, Scotty.\n";
    test_microdvd_do_test(microdvd_input, G_N_ELEMENTS(microdvd_input));
}
...
static Suite *subparse_suite(void)
{
