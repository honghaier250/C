...
static GstElement *setup_appsink(void)
{
    GstElement *appsink;
...
static void cleanup_appsink(GstElement * appsink)
{
...
static gint operate_on_data(gint indata)
{
    return indata * 2;
}

static GstFlowReturn callback_function(GstAppSink * appsink, gpointer callback_data)
{
...
static void notify_function(gpointer callback_data)
{
    global_testdata = operate_on_data(*((gint *) callback_data));
}
...
GST_START_TEST(test_notify0)
{
    ...
    gst_app_sink_set_callbacks(GST_APP_SINK(sink), &callbacks, &testdata, (*notify_function));
...
static GstBufferList *create_buffer_list(void)
{
...
static void check_buffer_list(GstBufferList * list)
{
    GstBufferListIterator *it;
    GstBuffer *buf;
    GstCaps *caps;
...
static GstFlowReturn callback_function_buffer_list(GstAppSink * appsink, gpointer callback_data)
{
    GstBufferList *list;
...
static Suite *appsink_suite(void)
{
