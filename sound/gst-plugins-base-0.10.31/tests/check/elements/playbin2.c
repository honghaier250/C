...
GST_START_TEST(test_sink_usage_video_only_stream)
{
    ...
    fail_unless(gst_element_register(NULL, "redvideosrc", GST_RANK_PRIMARY, gst_red_video_src_get_type()));
    ...
    fail_unless_equals_int(gst_element_set_state(playbin, GST_STATE_READY), GST_STATE_CHANGE_SUCCESS);
    fail_unless_equals_int(gst_element_set_state(playbin, GST_STATE_PAUSED), GST_STATE_CHANGE_ASYNC);
    fail_unless_equals_int(gst_element_get_state(playbin, NULL, NULL, -1), GST_STATE_CHANGE_SUCCESS);

    fail_unless_equals_int(gst_element_get_state(fakeaudiosink, &cur_state, &pending_state, 0), GST_STATE_CHANGE_SUCCESS);
...
static GstElement *create_playbin(const gchar * uri)
{
...
GST_START_TEST(test_missing_urisource_handler)
{
    ...
    fail_unless_equals_int(gst_element_set_state(playbin, GST_STATE_READY), GST_STATE_CHANGE_SUCCESS);
    fail_unless_equals_int(gst_element_set_state(playbin, GST_STATE_PAUSED), GST_STATE_CHANGE_FAILURE);
    ...
    fail_unless_equals_string(gst_structure_get_string(s, "detail"), "chocchipcookie");
    ...
    fail_unless(err->domain == GST_CORE_ERROR, "error has wrong error domain " "%s instead of core-error-quark", g_quark_to_string(err->domain));
    fail_unless(err->code == GST_CORE_ERROR_MISSING_PLUGIN, "error has wrong " "code %u instead of GST_CORE_ERROR_MISSING_PLUGIN", err->code);
    g_error_free(err);
    gst_message_unref(msg);
...
GST_START_TEST(test_missing_suburisource_handler)
{
    ...
    gst_message_parse_error(msg, &err, NULL);
    fail_unless(err != NULL);
    fail_unless(err->domain == GST_RESOURCE_ERROR, "error has wrong error domain " "%s instead of resource-error-quark", g_quark_to_string(err->domain));
    fail_unless(err->code == GST_RESOURCE_ERROR_NOT_FOUND, "error has wrong " "code %u instead of GST_RESOURCE_ERROR_NOT_FOUND", err->code);
    g_error_free(err);
    gst_message_unref(msg);
...
GST_START_TEST(test_missing_primary_decoder)
{
    ...
    fail_unless(gst_element_register(NULL, "codecsrc", GST_RANK_PRIMARY, gst_codec_src_get_type()));
    ...
    gst_message_parse_warning(msg, &err, NULL);
    fail_unless(err != NULL);
    fail_unless(err->domain == GST_STREAM_ERROR, "error has wrong error domain " "%s instead of stream-error-quark", g_quark_to_string(err->domain));
    fail_unless(err->code == GST_STREAM_ERROR_CODEC_NOT_FOUND, "error has wrong " "code %u instead of GST_STREAM_ERROR_CODEC_NOT_FOUND", err->code);
    g_error_free(err);
    gst_message_unref(msg);
...
GST_START_TEST(test_refcount)
{
    ...
    g_object_set(playbin, "audio-sink", audiosink, "video-sink", videosink, "vis-plugin", vis, "flags", 0x01 | 0x02 | 0x08, NULL);
    ...
    fail_unless_equals_int(gst_element_set_state(playbin, GST_STATE_PAUSED), GST_STATE_CHANGE_ASYNC);
    fail_unless_equals_int(gst_element_get_state(playbin, NULL, NULL, GST_CLOCK_TIME_NONE), GST_STATE_CHANGE_SUCCESS);
    fail_unless_equals_int(gst_element_set_state(playbin, GST_STATE_NULL), GST_STATE_CHANGE_SUCCESS);
...
static GstURIType gst_red_video_src_uri_get_type(void)
{
    return GST_URI_SRC;
}

static gchar **gst_red_video_src_uri_get_protocols(void)
{
...
static const gchar *gst_red_video_src_uri_get_uri(GstURIHandler * handler)
{
    return "redvideo://";
}

static gboolean gst_red_video_src_uri_set_uri(GstURIHandler * handler, const gchar * uri)
{
    return (uri != NULL && g_str_has_prefix(uri, "redvideo:"));
}

static void gst_red_video_src_uri_handler_init(gpointer g_iface, gpointer iface_data)
{
...
static void gst_red_video_src_init_type(GType type)
{
...
GST_BOILERPLATE_FULL(GstRedVideoSrc, gst_red_video_src, GstPushSrc, GST_TYPE_PUSH_SRC, gst_red_video_src_init_type);

static void gst_red_video_src_base_init(gpointer klass)
{
    ...
    gst_element_class_set_details_simple(element_class, "Red Video Src", "Source/Video", "yep", "me");
}

static GstFlowReturn gst_red_video_src_create(GstPushSrc * src, GstBuffer ** p_buf)
{
    GstBuffer *buf;
    GstCaps *caps;
    guint8 *data;
    ...
    caps = gst_caps_new_simple("video/x-raw-yuv", "format", GST_TYPE_FOURCC, GST_MAKE_FOURCC('I', '4', '2', '0'), "width", G_TYPE_INT, w, "height", G_TYPE_INT, h, "framerate", GST_TYPE_FRACTION, 1, 1, NULL);
...
static void gst_red_video_src_class_init(GstRedVideoSrcClass * klass)
{
...
static void gst_red_video_src_init(GstRedVideoSrc * src, GstRedVideoSrcClass * klass)
{
}

/*** codec:// source ***/

static GstURIType gst_codec_src_uri_get_type(void)
{
    return GST_URI_SRC;
}

static gchar **gst_codec_src_uri_get_protocols(void)
{
...
static const gchar *gst_codec_src_uri_get_uri(GstURIHandler * handler)
{
    return "codec://";
}

static gboolean gst_codec_src_uri_set_uri(GstURIHandler * handler, const gchar * uri)
{
    return (uri != NULL && g_str_has_prefix(uri, "codec:"));
}

static void gst_codec_src_uri_handler_init(gpointer g_iface, gpointer iface_data)
{
    GstURIHandlerInterface *iface = (GstURIHandlerInterface *) g_iface;
...
static void gst_codec_src_init_type(GType type)
{
    static const GInterfaceInfo uri_hdlr_info = {
...
GST_BOILERPLATE_FULL(GstCodecSrc, gst_codec_src, GstPushSrc, GST_TYPE_PUSH_SRC, gst_codec_src_init_type);

static void gst_codec_src_base_init(gpointer klass)
{
    static GstStaticPadTemplate src_templ = GST_STATIC_PAD_TEMPLATE("src",
                                                                    GST_PAD_SRC, GST_PAD_ALWAYS,
    ...
    gst_element_class_add_pad_template(element_class, gst_static_pad_template_get(&src_templ));
    gst_element_class_set_details_simple(element_class, "Codec Src", "Source/Video", "yep", "me");
}

static GstFlowReturn gst_codec_src_create(GstPushSrc * src, GstBuffer ** p_buf)
{
    GstBuffer *buf;
    GstCaps *caps;
...
static void gst_codec_src_class_init(GstCodecSrcClass * klass)
{
    GstPushSrcClass *pushsrc_class = GST_PUSH_SRC_CLASS(klass);
...
static void gst_codec_src_init(GstCodecSrc * src, GstCodecSrcClass * klass)
{
}

#endif                            /* GST_DISABLE_REGISTRY */

static Suite *playbin2_suite(void)
{
