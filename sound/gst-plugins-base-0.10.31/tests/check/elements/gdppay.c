...
static GstElement *setup_gdppay(void)
{
    GstElement *gdppay;
...
static void cleanup_gdppay(GstElement * gdppay)
{
...
GST_START_TEST(test_audio)
{
    ...
    fail_unless(gst_element_set_state(gdppay, GST_STATE_PLAYING) == GST_STATE_CHANGE_SUCCESS, "could not set to playing");

    GST_DEBUG("new segment");
    event = gst_event_new_new_segment(FALSE, 1.0, GST_FORMAT_TIME, 0, GST_SECOND, 0);
    ...
    fail_unless(gst_element_set_state(gdppay, GST_STATE_NULL) == GST_STATE_CHANGE_SUCCESS, "could not set to null");
...
static GstElement *setup_gdppay_streamheader(void)
{
    GstElement *gdppay;
...
GST_START_TEST(test_streamheader)
{
    ...
    fail_unless_equals_string((gchar *) gst_structure_get_name(structure), "application/x-gdp");
...
GST_START_TEST(test_crc)
{
    ...
    crc_calculated = gst_dp_crc(GST_BUFFER_DATA(outbuffer), 58);
    fail_if(crc_calculated == crc_read, "Introducing a byte error in the header should make the checksum fail");
...
static Suite *gdppay_suite(void)
{
