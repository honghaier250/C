...
static GstElement *setup_gdpdepay(void)
{
    GstElement *gdpdepay;
...
static void cleanup_gdpdepay(GstElement * gdpdepay)
{
...
static void gdpdepay_push_per_byte(const gchar * reason, guint8 * bytes, guint length)
{
    int i;
    GstBuffer *inbuffer;
    ...
    for (i = 0; i < length; ++i)
    {
        ...
        fail_unless(gst_pad_push(mysrcpad, inbuffer) == GST_FLOW_OK, "%s: failed pushing byte buffer", reason);
    }
}
...
GST_START_TEST(test_audio_per_byte)
{
    ...
    fail_unless(gst_element_set_state(gdpdepay, GST_STATE_PLAYING) == GST_STATE_CHANGE_SUCCESS, "could not set to playing");
    ...
    gdpdepay_push_per_byte("caps payload", payload, gst_dp_header_payload_length(header));
    fail_unless_equals_int(g_list_length(buffers), 0);
    caps = gst_pad_get_caps(srcpad);
    ...
    fail_unless_equals_int(g_list_length(buffers), 0);
    gdpdepay_push_per_byte("buffer payload", GST_BUFFER_DATA(buffer), gst_dp_header_payload_length(header));
    g_free(header);
    ...
    fail_unless(gst_element_set_state(gdpdepay, GST_STATE_NULL) == GST_STATE_CHANGE_SUCCESS, "could not set to null");
...
GST_START_TEST(test_audio_in_one_buffer)
{
    ...
    caps = gst_caps_from_string(AUDIO_CAPS_STRING);
    fail_unless(pk->packet_from_caps(caps, 0, &header_len, &caps_header, &caps_payload));
    ...
    inbuffer = gst_buffer_new_and_alloc(2 * GST_DP_HEADER_LENGTH + payload_len + GST_BUFFER_SIZE(buffer));
    ...
    i += GST_DP_HEADER_LENGTH;
    memcpy(GST_BUFFER_DATA(inbuffer) + i, GST_BUFFER_DATA(buffer), GST_BUFFER_SIZE(buffer));
...
static GstElement *setup_gdpdepay_streamheader(void)
{
    GstElement *gdpdepay;
...
GST_START_TEST(test_streamheader)
{
    ...
    GST_DEBUG("caps: %" GST_PTR_FORMAT ", padcaps: %" GST_PTR_FORMAT, caps, padcaps);
...
static Suite *gdpdepay_suite(void)
{
