...
GST_START_TEST(test_alsa_mixer_track)
{
    ...
    for (l = tracks; l != NULL; l = l->next)
    {
        ...
        if (ulabel != NULL)
        {
            ...
            {
                fail_unless(g_ascii_isprint(*p), "untranslated label '%s' not printable ASCII", ulabel);
            }
        }
    ...
    fail_unless_equals_int(gst_element_set_state(mixer, GST_STATE_NULL), GST_STATE_CHANGE_SUCCESS);
