...
static GstElement *setup_vorbistag(void)
{
...
static void cleanup_vorbistag(GstElement * vorbistag)
{
...
static gboolean buffer_probe(GstPad * pad, GstBuffer * buffer, gpointer unused)
{
    g_async_queue_push(pending_buffers, gst_buffer_ref(buffer));
    return TRUE;
}

static void start_pipeline(GstElement * element)
{
...
static GstBuffer *get_buffer(void)
{
    return GST_BUFFER(g_async_queue_pop(pending_buffers));
}

static void stop_pipeline(GstElement * element)
{
    GstBuffer *buf;
...
static GstBuffer *_create_codebook_header_buffer(void)
{
    GstBuffer *buffer;
    ogg_packet header;
...
static GstBuffer *_create_audio_buffer(void)
{
    GstBuffer *buffer;
    ogg_packet packet;
    float **vorbis_buffer;
...
GST_START_TEST(test_empty_tags_set)
{
    ...
    gst_tag_setter_merge_tags(GST_TAG_SETTER(vorbistag), tags, GST_TAG_MERGE_REPLACE);
    gst_tag_setter_set_tag_merge_mode(GST_TAG_SETTER(vorbistag), GST_TAG_MERGE_KEEP_ALL);
    ...
    memcpy(GST_BUFFER_DATA(inbuffer), identification_header, sizeof(identification_header));
    fail_unless_equals_int(gst_pad_push(mysrcpad, inbuffer), GST_FLOW_OK);

    /* send empty comment buffer */
    inbuffer = gst_buffer_new_and_alloc(sizeof(empty_comment_header));
    memcpy(GST_BUFFER_DATA(inbuffer), empty_comment_header, sizeof(empty_comment_header));
    fail_unless_equals_int(gst_pad_push(mysrcpad, inbuffer), GST_FLOW_OK);
    ...
    fail_unless_equals_int(GST_BUFFER_SIZE(outbuffer), sizeof(identification_header));
    fail_unless_equals_int(memcmp(GST_BUFFER_DATA(outbuffer), identification_header, sizeof(identification_header)), 0);
    gst_buffer_unref(outbuffer);

    /* check comment header is correct */
    outbuffer = get_buffer();
    fail_unless_equals_int(GST_BUFFER_SIZE(outbuffer), sizeof(title_comment_header));
    fail_unless_equals_int(memcmp(GST_BUFFER_DATA(outbuffer), title_comment_header, sizeof(title_comment_header)), 0);
    gst_buffer_unref(outbuffer);
...
GST_START_TEST(test_filled_tags_unset)
{
    ...
    memcpy(GST_BUFFER_DATA(inbuffer), title_comment_header, sizeof(title_comment_header));
    fail_unless_equals_int(gst_pad_push(mysrcpad, inbuffer), GST_FLOW_OK);
    ...
    /* check comment header is correct */
    outbuffer = get_buffer();
    fail_unless_equals_int(GST_BUFFER_SIZE(outbuffer), sizeof(empty_comment_header));
    fail_unless_equals_int(memcmp(GST_BUFFER_DATA(outbuffer), empty_comment_header, sizeof(empty_comment_header)), 0);
    gst_buffer_unref(outbuffer);
...
GST_START_TEST(test_filled_tags_change)
{
    ...
    memcpy(GST_BUFFER_DATA(inbuffer), artist_comment_header, sizeof(artist_comment_header));
    fail_unless_equals_int(gst_pad_push(mysrcpad, inbuffer), GST_FLOW_OK);
...
static Suite *vorbistag_suite(void)
{
