...
static GstCaps **videoscale_get_allowed_caps(void)
{
    ...
    templ = gst_element_class_get_pad_template(GST_ELEMENT_GET_CLASS(scale), "sink");
...
static void on_sink_handoff(GstElement * element, GstBuffer * buffer, GstPad * pad, gpointer user_data)
{
...
static void run_test(const GstCaps * caps, gint src_width, gint src_height, gint dest_width, gint dest_height, gint method, GCallback src_handoff, gpointer src_handoff_user_data, GCallback sink_handoff, gpointer sink_handoff_user_data)
{
    GstElement *pipeline;
    GstElement *src, *ffmpegcolorspace, *capsfilter1, *identity, *scale, *capsfilter2, *sink;
    GstMessage *msg;
    GstBus *bus;
    GstCaps *copy;
    guint n_buffers = 0;
    ...
    gst_caps_set_simple(copy, "width", G_TYPE_INT, src_width, "height", G_TYPE_INT, src_height, "framerate", GST_TYPE_FRACTION, 30, 1, NULL);
    ...
    if (src_handoff)
    {
        ...
        g_signal_connect(identity, "handoff", G_CALLBACK(src_handoff), src_handoff_user_data);
    }
    ...
    copy = gst_caps_copy(caps);
    gst_caps_set_simple(copy, "width", G_TYPE_INT, dest_width, "height", G_TYPE_INT, dest_height, NULL);
    ...
    if (sink_handoff)
    {
        g_signal_connect(sink, "handoff", G_CALLBACK(sink_handoff), sink_handoff_user_data);
    }

    gst_bin_add_many(GST_BIN(pipeline), src, ffmpegcolorspace, capsfilter1, identity, scale, capsfilter2, sink, NULL);

    fail_unless(gst_element_link_pads_full(src, "src", ffmpegcolorspace, "sink", LINK_CHECK_FLAGS));
    fail_unless(gst_element_link_pads_full(ffmpegcolorspace, "src", capsfilter1, "sink", LINK_CHECK_FLAGS));
    fail_unless(gst_element_link_pads_full(capsfilter1, "src", identity, "sink", LINK_CHECK_FLAGS));
    fail_unless(gst_element_link_pads_full(identity, "src", scale, "sink", LINK_CHECK_FLAGS));
    fail_unless(gst_element_link_pads_full(scale, "src", capsfilter2, "sink", LINK_CHECK_FLAGS));
    fail_unless(gst_element_link_pads_full(capsfilter2, "src", sink, "sink", LINK_CHECK_FLAGS));

    bus = gst_element_get_bus(pipeline);
    fail_unless(bus != NULL);

    fail_unless(gst_element_set_state(pipeline, GST_STATE_PLAYING) == GST_STATE_CHANGE_SUCCESS);

    msg = gst_bus_timed_pop_filtered(bus, GST_CLOCK_TIME_NONE, GST_MESSAGE_EOS | GST_MESSAGE_ERROR | GST_MESSAGE_WARNING);

    fail_unless_equals_int(GST_MESSAGE_TYPE(msg), GST_MESSAGE_EOS);

    fail_unless(gst_element_set_state(pipeline, GST_STATE_NULL) == GST_STATE_CHANGE_SUCCESS);
...
static void on_sink_handoff_passthrough(GstElement * element, GstBuffer * buffer, GstPad * pad, gpointer user_data)
{
...
static void on_src_handoff_passthrough(GstElement * element, GstBuffer * buffer, gpointer user_data)
{
    GList **list = user_data;
...
GST_START_TEST(test_passthrough)
{
    ...
    while (*p)
    {
        ...
        {
            GST_DEBUG("Running test for caps '%" GST_PTR_FORMAT "'" " from %dx%u to %dx%d with method %d", caps, src_width, src_height, dest_width, dest_height, method);
            run_test(caps, src_width, src_height, dest_width, dest_height, method, G_CALLBACK(on_src_handoff_passthrough), &src_buffers, G_CALLBACK(on_sink_handoff_passthrough), &sink_buffers);

            fail_unless(src_buffers && sink_buffers);
            fail_unless_equals_int(g_list_length(src_buffers), g_list_length(sink_buffers));

            for (l1 = src_buffers, l2 = sink_buffers; l1 && l2; l1 = l1->next, l2 = l2->next)
            {
                GstBuffer *a = l1->data;
                GstBuffer *b = l2->data;
...
static void _test_negotiation_message(GstBus * bus, GstMessage * message, TestNegotiationData * data)
{
    GError *err = NULL;
    gchar *debug;
...
static void _test_negotiation_notify_caps(GObject * src, GParamSpec * pspec, TestNegotiationData * data)
{
    GstCaps *caps;
    GstStructure *s;
    gint width, height;
    ...
    fail_unless(gst_structure_get_fraction(s, "pixel-aspect-ratio", &par_n, &par_d) || (data->par_n == 1 && data->par_d == 1));
    ...
    data->ok = (width == data->width) && (height == data->height) && (par_n == data->par_n) && (par_d == data->par_d);

    g_main_loop_quit(data->loop);
}

static void _test_negotiation(const gchar * src_templ, const gchar * sink_templ, gint width, gint height, gint par_n, gint par_d)
{
    GstElement *pipeline;
    ...
    GST_DEBUG("Running test for src templ caps '%s' and sink templ caps '%s'", src_templ, sink_templ);
    ...
    g_signal_connect(pad, "notify::caps", G_CALLBACK(_test_negotiation_notify_caps), &data);
    ...
    gst_bin_add_many(GST_BIN(pipeline), src, capsfilter1, scale, capsfilter2, sink, NULL);

    fail_unless(gst_element_link_pads_full(src, "src", capsfilter1, "sink", LINK_CHECK_FLAGS));
    fail_unless(gst_element_link_pads_full(capsfilter1, "src", scale, "sink", LINK_CHECK_FLAGS));
    fail_unless(gst_element_link_pads_full(scale, "src", capsfilter2, "sink", LINK_CHECK_FLAGS));
    fail_unless(gst_element_link_pads_full(capsfilter2, "src", sink, "sink", LINK_CHECK_FLAGS));
    ...
    g_signal_connect(bus, "message", G_CALLBACK(_test_negotiation_message), &data);
...
{
    _test_negotiation("video/x-raw-yuv,format=(fourcc)AYUV,width=720,height=576,pixel-aspect-ratio=16/15", "video/x-raw-yuv,format=(fourcc)AYUV,width=768,height=576", 768, 576, 1, 1);

    _test_negotiation("video/x-raw-yuv,format=(fourcc)AYUV,width=320,height=240", "video/x-raw-yuv,format=(fourcc)AYUV,width=640,height=320", 640, 320, 2, 3);

    _test_negotiation("video/x-raw-yuv,format=(fourcc)AYUV,width=320,height=240", "video/x-raw-yuv,format=(fourcc)AYUV,width=640,height=320,pixel-aspect-ratio=[0/1, 1/1]", 640, 320, 2, 3);

    _test_negotiation("video/x-raw-yuv,format=(fourcc)AYUV,width=1920,height=2560,pixel-aspect-ratio=1/1", "video/x-raw-yuv,format=(fourcc)AYUV,width=[1, 2048],height=[1, 2048],pixel-aspect-ratio=1/1", 1536, 2048, 1, 1);

    _test_negotiation("video/x-raw-yuv,format=(fourcc)AYUV,width=1920,height=2560,pixel-aspect-ratio=1/1", "video/x-raw-yuv,format=(fourcc)AYUV,width=[1, 2048],height=[1, 2048]", 1920, 2048, 4, 5);

    _test_negotiation("video/x-raw-yuv,format=(fourcc)AYUV,width=1920,height=2560", "video/x-raw-yuv,format=(fourcc)AYUV,width=[1, 2048],height=[1, 2048]", 1920, 2048, 4, 5);

    _test_negotiation("video/x-raw-yuv,format=(fourcc)AYUV,width=1920,height=2560", "video/x-raw-yuv,format=(fourcc)AYUV,width=1200,height=[1, 2048],pixel-aspect-ratio=1/1", 1200, 1600, 1, 1);

    /* Doesn't keep DAR but must be possible! */
    _test_negotiation("video/x-raw-yuv,format=(fourcc)AYUV,width=320,height=240,pixel-aspect-ratio=1/1", "video/x-raw-yuv,format=(fourcc)AYUV,width=200,height=200,pixel-aspect-ratio=1/2", 200, 200, 1, 2);
}
...
typedef struct _GstTestReverseNegotiationSinkClass GstTestReverseNegotiationSinkClass;
...
GST_BOILERPLATE(GstTestReverseNegotiationSink, gst_test_reverse_negotiation_sink, GstBaseSink, GST_TYPE_BASE_SINK);
...
static GstFlowReturn gst_test_reverse_negotiation_sink_buffer_alloc(GstBaseSink * bsink, guint64 offset, guint size, GstCaps * caps, GstBuffer ** buf)
{
    GstTestReverseNegotiationSink *sink = GST_TEST_REVERSE_NEGOTIATION_SINK(bsink);
    GstVideoFormat fmt;
    gint width, height;
    ...
    if (sink->nbuffers < 2)
    {
        *buf = gst_buffer_new_and_alloc(gst_video_format_get_size(fmt, width, height));
...
static GstFlowReturn gst_test_reverse_negotiation_sink_render(GstBaseSink * bsink, GstBuffer * buffer)
{
    GstTestReverseNegotiationSink *sink = GST_TEST_REVERSE_NEGOTIATION_SINK(bsink);
...
static void gst_test_reverse_negotiation_sink_base_init(gpointer g_class)
{
    GstElementClass *gstelement_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_set_details_simple(gstelement_class, "Test Reverse Negotiation Sink", "Sink", "Some test sink", "Sebastian Dröge <sebastian.droege@collabora.co.uk>");
    gst_element_class_add_pad_template(gstelement_class, gst_static_pad_template_get(&sinktemplate));
}

static void gst_test_reverse_negotiation_sink_class_init(GstTestReverseNegotiationSinkClass * klass)
{
    ...
    gstbase_sink_class->buffer_alloc = GST_DEBUG_FUNCPTR(gst_test_reverse_negotiation_sink_buffer_alloc);
    gstbase_sink_class->render = GST_DEBUG_FUNCPTR(gst_test_reverse_negotiation_sink_render);
}

static void gst_test_reverse_negotiation_sink_init(GstTestReverseNegotiationSink * sink, GstTestReverseNegotiationSinkClass * g_class)
{
    sink->nbuffers = 0;
}

static void _test_reverse_negotiation_message(GstBus * bus, GstMessage * message, GMainLoop * loop)
{
    GError *err = NULL;
    gchar *debug;
...
GST_START_TEST(test_reverse_negotiation)
{
    ...
    fail_unless(gst_element_link_pads_full(src, "src", csp1, "sink", LINK_CHECK_FLAGS));
    fail_unless(gst_element_link_pads_full(csp1, "src", scale, "sink", LINK_CHECK_FLAGS));
    fail_unless(gst_element_link_pads_full(scale, "src", csp2, "sink", LINK_CHECK_FLAGS));
    fail_unless(gst_element_link_pads_full(csp2, "src", sink, "sink", LINK_CHECK_FLAGS));
    ...
    g_signal_connect(bus, "message", G_CALLBACK(_test_reverse_negotiation_message), loop);
...
static Suite *videoscale_suite(void)
{
