...
static GstElement *setup_gnomevfssink(void)
{
...
static void cleanup_gnomevfssink(GstElement * gnomevfssink)
{
...
GST_START_TEST(test_seeking)
{
    ...
    tmp_fn = g_build_filename(tmpdir, "gstreamer-gnomevfssink-test-XXXXXX", NULL);
    fd = g_mkstemp(tmp_fn);
    if (fd < 0)
    {
    ...
    fail_unless_equals_int(gst_element_set_state(gnomevfssink, GST_STATE_PLAYING), GST_STATE_CHANGE_ASYNC);
    ...
    fail_unless((seeking_query = gst_query_new_seeking(GST_FORMAT_BYTES)) != NULL);
    ...
    fail_unless(gst_pad_push_event(mysrcpad, gst_event_new_new_segment(FALSE, 1.0, GST_FORMAT_BYTES, 0, -1, 0)));
    ...
    if (gst_pad_push_event(mysrcpad, gst_event_new_new_segment(TRUE, 1.0, GST_FORMAT_BYTES, 8800, -1, 0)))
    {
        GST_LOG("seek ok");
    ...
    fail_unless_equals_int(gst_element_set_state(gnomevfssink, GST_STATE_NULL), GST_STATE_CHANGE_SUCCESS);
    ...
    /* check that we wrote data to the right position after the seek */
    {
        ...
        fail_unless(g_file_get_contents(tmp_fn, &data, &len, NULL), "Failed to read in newly-created file '%s'", tmp_fn);
...
GST_START_TEST(test_coverage)
{
    ...
    fail_unless_equals_int(gst_element_set_state(gnomevfssink, GST_STATE_PLAYING), GST_STATE_CHANGE_FAILURE);
...
GST_START_TEST(test_uri_interface)
{
    ...
    fail_unless(gst_uri_handler_set_uri(GST_URI_HANDLER(gnomevfssink), "file:///foo/bar"));
    location = (gchar *) gst_uri_handler_get_uri(GST_URI_HANDLER(gnomevfssink));
    ...
    fail_unless(gst_uri_handler_set_uri(GST_URI_HANDLER(gnomevfssink), "file://localhost/foo/baz"));
    location = (gchar *) gst_uri_handler_get_uri(GST_URI_HANDLER(gnomevfssink));
    ...
    fail_if(gst_uri_handler_set_uri(GST_URI_HANDLER(gnomevfssink), "file://hostname/foo/foo"));
...
static Suite *gnomevfssink_suite(void)
{
