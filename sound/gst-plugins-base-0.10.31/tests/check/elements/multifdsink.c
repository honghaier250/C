...
static GstElement *setup_multifdsink(void)
{
...
static void cleanup_multifdsink(GstElement * multifdsink)
{
...
static void wait_bytes_served(GstElement * sink, guint64 bytes)
{
...
static void gst_multifdsink_create_streamheader(const gchar * data1, const gchar * data2, GstBuffer ** hbuf1, GstBuffer ** hbuf2, GstCaps ** caps)
{
    GstBuffer *buf;
    GValue array = { 0 };
    GValue value = { 0 };
...
GST_START_TEST(test_streamheader)
{
    ...
    /* create caps with streamheader, set the caps, and push the IN_CAPS
     * buffers */
    gst_multifdsink_create_streamheader("babe", "deadbeef", &hbuf1, &hbuf2, &caps);
...
GST_START_TEST(test_change_streamheader)
{
    ...
    /* create caps with streamheader, set the caps, and push the IN_CAPS
     * buffers */
    gst_multifdsink_create_streamheader("first", "header", &hbuf1, &hbuf2, &caps);
    fail_unless(gst_pad_set_caps(mysrcpad, caps));
    /* one is ours, two on the buffers, and one now on the pad */
    ASSERT_CAPS_REFCOUNT(caps, "caps", 4);
    ...
    gst_multifdsink_create_streamheader("second", "header", &hbuf1, &hbuf2, &caps);
    fail_unless(gst_pad_set_caps(mysrcpad, caps));
    /* one to hold for the test and one to give away */
    ASSERT_BUFFER_REFCOUNT(hbuf1, "hbuf1", 2);
    ASSERT_BUFFER_REFCOUNT(hbuf2, "hbuf2", 2);
...
GST_START_TEST(test_burst_client_bytes)
{
    ...
    /* now add the clients */
    g_signal_emit_by_name(sink, "add", pfd1[1]);
    g_signal_emit_by_name(sink, "add_full", pfd2[1], 3, 3, (guint64) 50, 3, (guint64) 200);
    g_signal_emit_by_name(sink, "add_full", pfd3[1], 3, 3, (guint64) 50, 3, (guint64) 50);
...
GST_START_TEST(test_burst_client_bytes_keyframe)
{
    ...
    /* now add the clients */
    g_signal_emit_by_name(sink, "add", pfd1[1]);
    g_signal_emit_by_name(sink, "add_full", pfd2[1], 4, 3, (guint64) 50, 3, (guint64) 90);
    g_signal_emit_by_name(sink, "add_full", pfd3[1], 4, 3, (guint64) 50, 3, (guint64) 50);
...
GST_START_TEST(test_burst_client_bytes_with_keyframe)
{
    ...
    /* now add the clients */
    g_signal_emit_by_name(sink, "add", pfd1[1]);
    g_signal_emit_by_name(sink, "add_full", pfd2[1], 5, 3, (guint64) 50, 3, (guint64) 90);
    g_signal_emit_by_name(sink, "add_full", pfd3[1], 5, 3, (guint64) 50, 3, (guint64) 50);
 ...
 */
static Suite *multifdsink_suite(void)
{
