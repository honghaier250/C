...
static GstCaps *rgb_format_to_caps(RGBFormat * fmt)
{
    GstCaps *caps;
    ...
    caps = gst_caps_new_simple("video/x-raw-rgb",
                               ...
                               "blue_mask", G_TYPE_INT, fmt->blue_mask, "width", G_TYPE_INT, 16, "height", G_TYPE_INT, 16, "endianness", G_TYPE_INT, fmt->endianness, "framerate", GST_TYPE_FRACTION, 1, 1, NULL);

    fail_unless(fmt->alpha_mask == 0 || fmt->bpp == 32);

    if (fmt->alpha_mask != 0)
    {
        gst_structure_set(gst_caps_get_structure(caps, 0), "alpha_mask", G_TYPE_INT, fmt->alpha_mask, NULL);
    }

    return caps;
}

static GList *create_rgb_conversions(void)
{
...
static void rgb_conversion_free(RGBConversion * conv)
{
...
static guint32 right_shift_colour(guint32 mask, guint32 pixel)
{
    if (mask == 0)
        return 0;
...
static guint8 fix_expected_colour(guint32 col_mask, guint8 col_expected)
{
    guint32 mask;
...
static void check_rgb_buf(const guint8 * pixels, guint32 r_mask, guint32 g_mask, guint32 b_mask, guint32 a_mask, guint8 r_expected, guint8 g_expected, guint8 b_expected, guint endianness, guint bpp, guint depth)
{
        ...
        case 24:
            {
                ...
                {
                    pixel = (GST_READ_UINT8(pixels) << 16) | (GST_READ_UINT8(pixels + 1) << 8) | (GST_READ_UINT8(pixels + 2) << 0);
                }
                else
                {
                    pixel = (GST_READ_UINT8(pixels + 2) << 16) | (GST_READ_UINT8(pixels + 1) << 8) | (GST_READ_UINT8(pixels + 0) << 0);
                }
                break;
            }
        case 16:
    ...
    fail_unless(red == r_expected, "RED: expected 0x%02x, found 0x%02x    " "Bytes: 0x%02x 0x%02x 0x%02x 0x%02x    Pixel: 0x%08x", r_expected, red, pixels[0], pixels[1], pixels[2], pixels[3], pixel);
    fail_unless(green == g_expected, "GREEN: expected 0x%02x, found 0x%02x    " "Bytes: 0x%02x 0x%02x 0x%02x 0x%02x    Pixel: 0x%08x", g_expected, green, pixels[0], pixels[1], pixels[2], pixels[3], pixel);
    fail_unless(blue == b_expected, "BLUE: expected 0x%02x, found 0x%02x    " "Bytes: 0x%02x 0x%02x 0x%02x 0x%02x    Pixel: 0x%08x", b_expected, blue, pixels[0], pixels[1], pixels[2], pixels[3], pixel);

//  FIXME: fix alpha check
//  fail_unless (a_mask == 0 || alpha != 0);      /* better than nothing */
}

static void got_buf_cb(GstElement * sink, GstBuffer * new_buf, GstPad * pad, GstBuffer ** p_old_buf)
{
...
GST_START_TEST(test_rgb_to_rgb)
{
    ...
    for (l = conversions; l != NULL; l = l->next)
    {
        ...
        {
            GST_DEBUG("videotestsrc doesn't support from_caps %" GST_PTR_FORMAT, conv->from_caps);
            continue;
        }
        ...
        for (p = 0; p < G_N_ELEMENTS(test_patterns); ++p)
        {
            ...
            GST_INFO("%5s %u/%u %08x %08x %08x %08x %u => "
                     ...
                     from->green_mask, from->blue_mask, from->alpha_mask, from->endianness, to->nick, to->bpp, to->depth, to->red_mask, to->green_mask, to->blue_mask, to->alpha_mask, to->endianness, test_patterns[p].pattern_name);
            ...
            if (state_ret == GST_STATE_CHANGE_FAILURE)
            {
                ...
                if (msg->src == GST_OBJECT_CAST(src) && err->code == GST_STREAM_ERROR_FORMAT)
                {
                ...
                fail_unless(state_ret != GST_STATE_CHANGE_FAILURE, "pipeline _set_state() to PAUSED failed: %s", err->message);
            }

            state_ret = gst_element_get_state(pipeline, NULL, NULL, -1);
            fail_unless(state_ret == GST_STATE_CHANGE_SUCCESS, "pipeline failed going to PAUSED state");
            ...
            check_rgb_buf(GST_BUFFER_DATA(buf), to->red_mask, to->green_mask, to->blue_mask, to->alpha_mask, test_patterns[p].r_expected, test_patterns[p].g_expected, test_patterns[p].b_expected, to->endianness, to->bpp, to->depth);
...
static Suite *ffmpegcolorspace_suite(void)
{
