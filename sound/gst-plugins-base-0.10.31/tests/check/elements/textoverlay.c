...
static GstPad *notgst_check_setup_src_pad2(GstElement * element, GstStaticPadTemplate * template, GstCaps * caps, const gchar * sink_template_name)
{
    ...
    fail_if(sinkpad == NULL, "Could not get sink pad from %s", GST_ELEMENT_NAME(element));
    ASSERT_OBJECT_REFCOUNT(sinkpad, "sinkpad", 2);
    if (caps)
        fail_unless(gst_pad_set_caps(srcpad, caps));
    fail_unless(gst_pad_link(srcpad, sinkpad) == GST_PAD_LINK_OK, "Could not link source and %s sink pads", GST_ELEMENT_NAME(element));
...
static void notgst_check_teardown_src_pad2(GstElement * element, const gchar * sink_template_name)
{
    GstPad *srcpad, *sinkpad;
...
static GstElement *setup_textoverlay(gboolean video_only_no_text)
{
    ...
    myvideosrcpad = notgst_check_setup_src_pad2(textoverlay, &video_srctemplate, NULL, "video_sink");

    if (!video_only_no_text)
    {
        mytextsrcpad = notgst_check_setup_src_pad2(textoverlay, &text_srctemplate, NULL, "text_sink");
...
static gboolean buffer_is_all_black(GstBuffer * buf)
{
    GstStructure *s;
    gint x, y, w, h;
...
static GstBuffer *create_black_buffer(const gchar * caps_string)
{
    GstStructure *s;
    GstBuffer *buffer;
    GstCaps *caps;
...
static GstBuffer *create_text_buffer(const gchar * txt, GstClockTime ts, GstClockTime duration)
{
    GstBuffer *buffer;
    GstCaps *caps;
    guint txt_len;
...
static void cleanup_textoverlay(GstElement * textoverlay)
{
...
GST_START_TEST(test_video_passthrough)
{
    ...
    fail_unless(gst_element_set_state(textoverlay, GST_STATE_PLAYING) == GST_STATE_CHANGE_SUCCESS, "could not set to playing");
    ...
    gst_pad_push_event(myvideosrcpad, gst_event_new_new_segment(FALSE, 1.0, GST_FORMAT_TIME, 1 * GST_SECOND, -1, 0));
    ...
    gst_pad_push_event(myvideosrcpad, gst_event_new_new_segment(FALSE, 1.0, 1 * GST_FORMAT_TIME, GST_SECOND, -1, 0));
    ...
    fail_unless_equals_int(g_list_length(buffers), 1);
    fail_unless(GST_BUFFER_CAST(buffers->data) != inbuffer);
    fail_unless(GST_BUFFER_TIMESTAMP(GST_BUFFER_CAST(buffers->data)) == GST_SECOND);
    fail_unless(GST_BUFFER_DURATION(GST_BUFFER_CAST(buffers->data)) == (GST_SECOND / 4));
...
GST_START_TEST(test_video_render_static_text)
{
    ...
    fail_unless(GST_BUFFER_DURATION(GST_BUFFER_CAST(buffers->data)) == (GST_SECOND / 10));
...
static gpointer test_video_waits_for_text_send_text_newsegment_thread(gpointer data)
{
    ...
    gst_pad_push_event(mytextsrcpad, gst_event_new_new_segment(TRUE, 1.0, GST_FORMAT_TIME, 35 * GST_SECOND, -1, 35 * GST_SECOND));

    return NULL;
}

static gpointer test_video_waits_for_text_shutdown_element(gpointer data)
{
    g_usleep(1 * G_USEC_PER_SEC);
...
GST_START_TEST(test_video_waits_for_text)
{
    ...
    /* there should be text rendered */
    fail_unless(buffer_is_all_black(GST_BUFFER_CAST(buffers->next->data)) == FALSE);
    ...
    thread = g_thread_create(test_video_waits_for_text_send_text_newsegment_thread, NULL, FALSE, NULL);
    ...
    fail_unless(buffer_is_all_black(GST_BUFFER_CAST(buffers->next->next->data)));
     ...
     * works ok */
    thread = g_thread_create(test_video_waits_for_text_shutdown_element, textoverlay, FALSE, NULL);
    fail_unless(thread != NULL);
...
static gpointer test_render_continuity_push_video_buffers_thread(gpointer data)
{
    ...
    do
    {
        ...
        /* pushing gives away one of the two references we have ... */
        GST_LOG("pushing video buffer %u @ %" GST_TIME_FORMAT, frame_count, GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(vbuf)));
...
GST_START_TEST(test_render_continuity)
{
    ...
    thread = g_thread_create(test_render_continuity_push_video_buffers_thread, NULL, FALSE, NULL);
    fail_unless(thread != NULL);

    tbuf = create_text_buffer("XLX", 2 * GST_SECOND, GST_SECOND);
    GST_LOG("pushing text buffer @ %" GST_TIME_FORMAT, GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(tbuf)));
    fail_unless(gst_pad_push(mytextsrcpad, tbuf) == GST_FLOW_OK);
    ...
    GST_LOG("give the other thread some time to push through the remaining" "video buffers");
    ...
    fail_unless(buffer_is_all_black(GST_BUFFER(g_list_nth_data(buffers, 2))) == FALSE);
    fail_unless(buffer_is_all_black(GST_BUFFER(g_list_nth_data(buffers, 3))) == FALSE);
    fail_unless(buffer_is_all_black(GST_BUFFER(g_list_nth_data(buffers, 4))) == FALSE);
    ...
    fail_unless(buffer_is_all_black(GST_BUFFER(g_list_nth_data(buffers, 7))) == FALSE);
    fail_unless(buffer_is_all_black(GST_BUFFER(g_list_nth_data(buffers, 8))) == FALSE);
    fail_unless(buffer_is_all_black(GST_BUFFER(g_list_nth_data(buffers, 9))) == FALSE);
    fail_unless(buffer_is_all_black(GST_BUFFER(g_list_nth_data(buffers, 10))) == FALSE);
    fail_unless(buffer_is_all_black(GST_BUFFER(g_list_nth_data(buffers, 11))) == FALSE);
    fail_unless(buffer_is_all_black(GST_BUFFER(g_list_nth_data(buffers, 12))) == FALSE);
    fail_unless(buffer_is_all_black(GST_BUFFER(g_list_nth_data(buffers, 13))) == FALSE);
    fail_unless(buffer_is_all_black(GST_BUFFER(g_list_nth_data(buffers, 14))) == FALSE);
...
static Suite *textoverlay_suite(void)
{
