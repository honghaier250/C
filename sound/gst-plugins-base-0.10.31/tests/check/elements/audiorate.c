...
static void test_injector_base_init(gpointer g_class)
{
...
static void test_injector_class_init(TestInjectorClass * klass)
{
    /* nothing to do here */
}

static GstFlowReturn test_injector_chain(GstPad * pad, GstBuffer * buf)
{
    GstFlowReturn ret;
    GstPad *srcpad;

    srcpad = gst_element_get_static_pad(GST_ELEMENT(GST_PAD_PARENT(pad)), "src");

    /* since we're increasing timestamp/offsets, push this one first */
    GST_LOG(" passing buffer   [t=%" GST_TIME_FORMAT "-%" GST_TIME_FORMAT
            "], offset=%" G_GINT64_FORMAT ", offset_end=%" G_GINT64_FORMAT, GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buf)), GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buf) + GST_BUFFER_DURATION(buf)), GST_BUFFER_OFFSET(buf), GST_BUFFER_OFFSET_END(buf));
    ...
    if (g_random_double() < injector_inject_probability)
    {
        ...
        if (GST_BUFFER_TIMESTAMP_IS_VALID(ibuf) || GST_BUFFER_OFFSET_IS_VALID(ibuf))
        {
            GST_LOG("injecting buffer [t=%" GST_TIME_FORMAT "-%" GST_TIME_FORMAT
                    "], offset=%" G_GINT64_FORMAT ", offset_end=%" G_GINT64_FORMAT,
                    GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(ibuf)), GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(ibuf) + GST_BUFFER_DURATION(ibuf)), GST_BUFFER_OFFSET(ibuf), GST_BUFFER_OFFSET_END(ibuf));
...
static void test_injector_init(TestInjector * injector, TestInjectorClass * klass)
{
    GstPad *pad;
...
static gboolean probe_cb(GstPad * pad, GstBuffer * buf, gdouble * drop_probability)
{
...
static void got_buf(GstElement * fakesink, GstBuffer * buf, GstPad * pad, GList ** p_bufs)
{
    *p_bufs = g_list_append(*p_bufs, gst_buffer_ref(buf));
}

static void do_perfect_stream_test(guint rate, guint width, gdouble drop_probability, gdouble inject_probability)
{
    ...
    caps = gst_caps_new_simple("audio/x-raw-int", "rate", G_TYPE_INT, rate, "width", G_TYPE_INT, width, NULL);

    GST_INFO("-------- drop=%.0f%% caps = %" GST_PTR_FORMAT " ---------- ", drop_probability * 100.0, caps);
    ...
    gst_bin_add_many(GST_BIN(pipe), src, conv, filter, injector, audiorate, sink, NULL);
    gst_element_link_many(src, conv, filter, injector, audiorate, sink, NULL);

    fail_unless_equals_int(gst_element_set_state(pipe, GST_STATE_PLAYING), GST_STATE_CHANGE_ASYNC);

    fail_unless_equals_int(gst_element_get_state(pipe, NULL, NULL, -1), GST_STATE_CHANGE_SUCCESS);

    msg = gst_bus_poll(GST_ELEMENT_BUS(pipe), GST_MESSAGE_EOS | GST_MESSAGE_ERROR, -1);
    ...
    for (l = bufs; l != NULL; l = l->next)
    {
        ...
        GST_LOG("buffer: ts=%" GST_TIME_FORMAT ", end_ts=%" GST_TIME_FORMAT
                " off=%" G_GINT64_FORMAT ", end_off=%" G_GINT64_FORMAT, GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buf)), GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buf) + GST_BUFFER_DURATION(buf)), GST_BUFFER_OFFSET(buf), GST_BUFFER_OFFSET_END(buf));
...
GST_START_TEST(test_large_discont)
{
    ...
    caps = gst_caps_new_simple("audio/x-raw-float", "channels", G_TYPE_INT, 1, "rate", G_TYPE_INT, 44100, "width", G_TYPE_INT, 32, NULL);
    ...
    fail_unless(gst_element_set_state(audiorate, GST_STATE_PLAYING) == GST_STATE_CHANGE_SUCCESS, "failed to set audiorate playing");
...
static Suite *audiorate_suite(void)
{
