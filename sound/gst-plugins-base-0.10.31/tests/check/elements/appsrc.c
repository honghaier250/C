...
static GstElement *setup_appsrc(void)
{
    GstElement *appsrc;
...
static void cleanup_appsrc(GstElement * appsrc)
{
...
GST_START_TEST(test_appsrc_null_caps)
{
    ...
    fail_unless(gst_app_src_push_buffer(GST_APP_SRC(src), buffer) == GST_FLOW_OK);
...
static Suite *appsrc_suite(void)
{
