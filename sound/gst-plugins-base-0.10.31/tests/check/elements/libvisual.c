...
static gboolean filter_func(GstPluginFeature * feature, gpointer user_data)
{
    return (g_str_has_prefix(GST_PLUGIN_FEATURE_NAME(feature), "libvisual_"));
}

static void test_shutdown_for_factory(const gchar * factory_name)
{
    ...
    caps = gst_caps_new_simple("video/x-raw-rgb", "width", G_TYPE_INT, 320, "height", G_TYPE_INT, 240, "framerate", GST_TYPE_FRACTION, 15, 1, NULL);
...
static Suite *libvisual_suite(void)
{
