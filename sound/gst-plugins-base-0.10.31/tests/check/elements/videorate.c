...
static GstStaticPadTemplate downstreamsinktemplate = GST_STATIC_PAD_TEMPLATE("sink",
                                                                             GST_PAD_SINK,
                                                                             GST_PAD_ALWAYS,
...
static void assert_videorate_stats(GstElement * videorate, const gchar * reason, guint64 xin, guint64 xout, guint64 xdropped, guint64 xduplicated)
{
    guint64 in, out, dropped, duplicated;

    g_object_get(videorate, "in", &in, "out", &out, "drop", &dropped, "duplicate", &duplicated, NULL);
...
static GstElement *setup_videorate_full(GstStaticPadTemplate * srctemplate, GstStaticPadTemplate * sinktemplate)
{
...
static GstElement *setup_videorate(void)
{
    return setup_videorate_full(&srctemplate, &sinktemplate);
}

static void cleanup_videorate(GstElement * videorate)
{
...
GST_START_TEST(test_one)
{
    ...
    fail_unless(gst_element_set_state(videorate, GST_STATE_PLAYING) == GST_STATE_CHANGE_SUCCESS, "could not set to playing");
...
GST_START_TEST(test_more)
{
    ...
    l = g_list_next(l);
    fail_unless_equals_uint64(GST_BUFFER_TIMESTAMP(l->data), GST_SECOND * 2 / 25);
    fail_unless_equals_int(GST_BUFFER_DATA(l->data)[0], 2);
    fail_unless_equals_uint64(GST_BUFFER_OFFSET(l->data), 2);
    fail_unless_equals_uint64(GST_BUFFER_OFFSET_END(l->data), 3);

    l = g_list_next(l);
    fail_unless_equals_uint64(GST_BUFFER_TIMESTAMP(l->data), GST_SECOND * 3 / 25);
    fail_unless_equals_int(GST_BUFFER_DATA(l->data)[0], 2);
...
GST_START_TEST(test_wrong_order_from_zero)
{
    ...
    fail_unless_equals_uint64(GST_BUFFER_TIMESTAMP(outbuffer), GST_SECOND * 37 / 25);
...
GST_START_TEST(test_changing_size)
{
    ...
    newsegment = gst_event_new_new_segment(FALSE, 1.0, GST_FORMAT_TIME, 0, -1, 0);
...
GST_START_TEST(test_selected_caps)
{
    ...
    pipeline = gst_parse_launch("videotestsrc num-buffers=1" " ! identity ! videorate name=videorate0 ! " VIDEO_CAPS_UNUSUAL_FRAMERATE " ! fakesink", NULL);
    fail_if(pipeline == NULL);
    videorate = gst_bin_get_by_name(GST_BIN(pipeline), "videorate0");
    fail_if(videorate == NULL);
    bus = gst_pipeline_get_bus(GST_PIPELINE(pipeline));

    fail_if(gst_element_set_state(pipeline, GST_STATE_PLAYING) == GST_STATE_CHANGE_FAILURE, "could not set to playing");
...
static Suite *videorate_suite(void)
{
