...
static GstElement *setup_videotestsrc(void)
{
...
static void cleanup_videotestsrc(GstElement * videotestsrc)
{
...
GST_START_TEST(test_all_patterns)
{
    ...
    while (values[j].value_name)
    {
        ...
        fail_unless(gst_element_set_state(videotestsrc, GST_STATE_PLAYING) == GST_STATE_CHANGE_SUCCESS, "could not set to playing");
...
static void check_rgb_buf(const guint8 * pixels, guint32 r_mask, guint32 g_mask, guint32 b_mask, guint32 a_mask, guint8 r_expected, guint8 g_expected, guint8 b_expected, guint endianness, guint bpp, guint depth)
{
    ...
    fail_unless(red == r_expected, "RED: expected 0x%02x, found 0x%02x", r_expected, red);
    fail_unless(green == g_expected, "GREEN: expected 0x%02x, found 0x%02x", g_expected, green);
    fail_unless(blue == b_expected, "BLUE: expected 0x%02x, found 0x%02x", b_expected, blue);
...
GST_START_TEST(test_rgb_formats)
{
    ...
    for (i = 0; i < G_N_ELEMENTS(rgb_formats); ++i)
    {
        for (e = 0; e < 2; ++e)
        {
            ...
            {
                endianness = (G_BYTE_ORDER == G_BIG_ENDIAN) ? G_LITTLE_ENDIAN : G_BIG_ENDIAN;
            }
            ...
            caps = gst_caps_new_simple("video/x-raw-rgb",
                                       ...
                                       "blue_mask", G_TYPE_INT, rgb_formats[i].blue_mask, "width", G_TYPE_INT, 16, "height", G_TYPE_INT, 16, "endianness", G_TYPE_INT, endianness, "framerate", GST_TYPE_FRACTION, 1, 1, NULL);

            fail_unless(rgb_formats[i].alpha_mask == 0 || rgb_formats[i].bpp == 32);

            if (rgb_formats[i].alpha_mask != 0)
            {
                gst_structure_set(gst_caps_get_structure(caps, 0), "alpha_mask", G_TYPE_INT, rgb_formats[i].alpha_mask, NULL);
            }
            ...
            if (gst_caps_is_subset(caps, template_caps))
            {
                ...
                for (p = 0; p < G_N_ELEMENTS(test_patterns); ++p)
                {
                    ...
                    GST_INFO("%5s %u/%u %08x %08x %08x %08x %u, pattern=%s",
                             rgb_formats[i].nick, rgb_formats[i].bpp, rgb_formats[i].depth, rgb_formats[i].red_mask, rgb_formats[i].green_mask, rgb_formats[i].blue_mask, rgb_formats[i].alpha_mask, endianness, test_patterns[p].pattern_name);

                    /* now get videotestsrc to produce a buffer with the given caps */
                    g_object_set(filter, "caps", caps, NULL);

                    state_ret = gst_element_set_state(pipeline, GST_STATE_PAUSED);
                    fail_unless(state_ret != GST_STATE_CHANGE_FAILURE, "pipeline _set_state() to PAUSED failed");
                    ...
                    check_rgb_buf(GST_BUFFER_DATA(buf), rgb_formats[i].red_mask,
                                  ...
                                  rgb_formats[i].alpha_mask, test_patterns[p].r_expected, test_patterns[p].g_expected, test_patterns[p].b_expected, endianness, rgb_formats[i].bpp, rgb_formats[i].depth);
...
static Suite *videotestsrc_suite(void)
{
