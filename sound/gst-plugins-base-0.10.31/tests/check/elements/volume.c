...
static GstElement *setup_volume(void)
{
    GstElement *volume;
...
static void cleanup_volume(GstElement * volume)
{
...
GST_START_TEST(test_get_set)
{
    ...
    fail_unless(val == 1.0);
    fail_unless(val == gst_stream_volume_get_volume(GST_STREAM_VOLUME(volume), GST_STREAM_VOLUME_FORMAT_LINEAR));
    ...
    gst_stream_volume_set_volume(GST_STREAM_VOLUME(volume), GST_STREAM_VOLUME_FORMAT_LINEAR, 1.0);
    g_object_get(G_OBJECT(volume), "volume", &val, NULL);
    fail_unless(val == 1.0);
    fail_unless(val == gst_stream_volume_get_volume(GST_STREAM_VOLUME(volume), GST_STREAM_VOLUME_FORMAT_LINEAR));
...
GST_START_TEST(test_unity_s8)
{
    ...
    volume = setup_volume();
    fail_unless(gst_element_set_state(volume, GST_STATE_PLAYING) == GST_STATE_CHANGE_SUCCESS, "could not set to playing");
...
GST_START_TEST(test_half_s8)
{
    ...
    fail_unless(inbuffer == outbuffer);
    res = (gint8 *) GST_BUFFER_DATA(outbuffer);
    GST_INFO("expected %+5d %+5d  real %+5d %+5d", out[0], out[1], res[0], res[1]);
...
GST_START_TEST(test_unity_s24)
{
    ...
    GST_INFO("expected %+5d %+5d  real %+5d %+5d", in_32[0], in_32[1], res_32[0], res_32[1]);
...
GST_START_TEST(test_half_s24)
{
    ...
    GST_INFO("expected %+5d %+5d  real %+5d %+5d", out_32[0], out_32[1], res_32[0], res_32[1]);
...
GST_START_TEST(test_unity_f32)
{
    ...
    GST_INFO("expected %+1.4f %+1.4f  real %+1.4f %+1.4f", in[0], in[1], res[0], res[1]);
...
GST_START_TEST(test_half_f32)
{
    ...
    fail_unless(inbuffer == outbuffer);
    res = (gfloat *) GST_BUFFER_DATA(outbuffer);
    GST_INFO("expected %+1.4f %+1.4f  real %+1.4f %+1.4f", out[0], out[1], res[0], res[1]);
...
GST_START_TEST(test_wrong_caps)
{
    ...
    fail_unless_equals_int(gst_pad_push(mysrcpad, inbuffer), GST_FLOW_NOT_NEGOTIATED);
...
GST_START_TEST(test_controller_usability)
{
    ...
    gst_interpolation_control_source_set_interpolation_mode(csource, GST_INTERPOLATE_CUBIC);
...
static Suite *volume_suite(void)
{
