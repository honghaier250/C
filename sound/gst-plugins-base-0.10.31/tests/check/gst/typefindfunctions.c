...
GST_START_TEST(test_quicktime_mpeg4video)
{
    ...
    const guint8 qt_redirect_396042[] = { 0x00, 0x00, 0x01, 0x22, 0x6d, 0x6f, 0x6f, 0x76, 0x00, 0x00, 0x01, 0x1a,
...
static GstCaps *typefind_test_file(const gchar * filename)
{
    GstBuffer *buf;
    GError *err = NULL;
    GstCaps *caps = NULL;
...
GST_START_TEST(test_random_data)
{
    ...
    {
        GST_ERROR("typefinder thinks random data is %" GST_PTR_FORMAT ", with a " "probability of %u (seed was %u)", caps, prob, seed);
        gst_caps_unref(caps);
    }
...
static Suite *typefindfunctions_suite(void)
{
