...
static gboolean structure_contains_channel_positions(const GstStructure * s)
{
    return (gst_structure_get_value(s, "channel-positions") != NULL);
}

#if 0
static gboolean fixed_caps_have_channel_positions(const GstCaps * caps)
{
    GstStructure *s;
...
GST_START_TEST(test_buffer_clipping_time)
{
    ...
    gst_segment_set_newsegment(&s, FALSE, 1.0, GST_FORMAT_TIME, 4 * GST_SECOND, 8 * GST_SECOND, 4 * GST_SECOND);
    ...
    gst_segment_init(&s, GST_FORMAT_TIME);
    gst_segment_set_newsegment(&s, FALSE, 1.0, GST_FORMAT_TIME, 4 * GST_SECOND, 12 * GST_SECOND, 4 * GST_SECOND);
    ...
    gst_segment_init(&s, GST_FORMAT_TIME);
    gst_segment_set_newsegment(&s, FALSE, 1.0, GST_FORMAT_TIME, 2 * GST_SECOND, 10 * GST_SECOND, 2 * GST_SECOND);
    ...
    gst_segment_init(&s, GST_FORMAT_TIME);
    gst_segment_set_newsegment(&s, FALSE, 1.0, GST_FORMAT_TIME, 12 * GST_SECOND, 20 * GST_SECOND, 12 * GST_SECOND);
    ...
    gst_segment_init(&s, GST_FORMAT_TIME);
    gst_segment_set_newsegment(&s, FALSE, 1.0, GST_FORMAT_TIME, 0 * GST_SECOND, 10 * GST_SECOND, 0 * GST_SECOND);
...
GST_START_TEST(test_buffer_clipping_samples)
{
    ...
    gst_segment_set_newsegment(&s, FALSE, 1.0, GST_FORMAT_DEFAULT, 400, 800, 400);
    ...
    gst_segment_init(&s, GST_FORMAT_DEFAULT);
    gst_segment_set_newsegment(&s, FALSE, 1.0, GST_FORMAT_DEFAULT, 400, 1200, 400);
    ...
    gst_segment_init(&s, GST_FORMAT_DEFAULT);
    gst_segment_set_newsegment(&s, FALSE, 1.0, GST_FORMAT_DEFAULT, 200, 1000, 200);
    ...
    gst_segment_init(&s, GST_FORMAT_DEFAULT);
    gst_segment_set_newsegment(&s, FALSE, 1.0, GST_FORMAT_DEFAULT, 1200, 2000, 1200);
...
static void init_value_to_channel_layout(GValue * val, GstAudioChannelPosition pos1, GstAudioChannelPosition pos2)
{
    GValue pos = { 0, };
...
GST_START_TEST(test_channel_layout_value_intersect)
{
    ...
    init_value_to_channel_layout(&layout, GST_AUDIO_CHANNEL_POSITION_FRONT_RIGHT, GST_AUDIO_CHANNEL_POSITION_FRONT_LEFT);
    gst_value_list_append_value(&list, &layout);
    g_value_unset(&layout);
    init_value_to_channel_layout(&layout, GST_AUDIO_CHANNEL_POSITION_FRONT_LEFT, GST_AUDIO_CHANNEL_POSITION_FRONT_RIGHT);
    gst_value_list_append_value(&list, &layout);
    g_value_unset(&layout);
    ...
    fail_unless_equals_int(g_value_get_enum(gst_value_array_get_value(&res, 0)), GST_AUDIO_CHANNEL_POSITION_FRONT_LEFT);
    fail_unless_equals_int(g_value_get_enum(gst_value_array_get_value(&res, 1)), GST_AUDIO_CHANNEL_POSITION_FRONT_RIGHT);
    g_value_unset(&res);

    /* this (with rear position) should not yield any results */
    init_value_to_channel_layout(&layout, GST_AUDIO_CHANNEL_POSITION_FRONT_LEFT, GST_AUDIO_CHANNEL_POSITION_REAR_RIGHT);
