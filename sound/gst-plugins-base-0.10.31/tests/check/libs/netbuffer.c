...
GST_START_TEST(test_netbuffer_copy)
{
    ...
    fail_unless(memcmp(GST_BUFFER_DATA(copy), GST_BUFFER_DATA(netbuf), GST_BUFFER_SIZE(copy)) == 0);
    ...
    fail_unless(ipv4_copy == ipv4_addr, "Copied buffer has wrong IPV4 from address");
    ...
    fail_unless(memcmp(ipv6_copy, ipv6_addr, 16) == 0, "Copied buffer has wrong IPv6 destination address");
    fail_unless(port == ipv6_port, "Copied buffer has wrong IPv6 destination port");
...
static Suite *netbuffer_suite(void)
{
