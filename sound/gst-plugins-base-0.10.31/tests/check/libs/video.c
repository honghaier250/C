...
static gboolean video_format_is_packed(GstVideoFormat fmt)
{
    switch (fmt)
    {
...
GST_START_TEST(test_video_formats)
{
    ...
    for (i = 0; i < G_N_ELEMENTS(fourcc_list); ++i)
    {
        ...
        GST_INFO("Fourcc %s, packed=%d", fourcc_list[i].fourcc, gst_video_format_is_packed(fmt));
        ...
        for (w = 1; w <= 65; ++w)
        {
            for (h = 1; h <= 65; ++h)
            {
                ...
                fail_unless_equals_int(gst_video_format_get_row_stride(fmt, 0, w), paintinfo.ystride);
                if (!gst_video_format_is_packed(fmt) && !gst_video_format_is_gray(fmt))
                {
                    /* planar */
                    fail_unless_equals_int(gst_video_format_get_row_stride(fmt, 1, w), paintinfo.ustride);
                    fail_unless_equals_int(gst_video_format_get_row_stride(fmt, 2, w), paintinfo.vstride);
                ...
                if (fmt != GST_VIDEO_FORMAT_Y41B && fmt != GST_VIDEO_FORMAT_Y42B && fmt != GST_VIDEO_FORMAT_Y800)
                {
                    guint cs0, cs1, cs2, cs3;

                    cs0 = gst_video_format_get_component_width(fmt, 0, w) * gst_video_format_get_component_height(fmt, 0, h);
                    cs1 = gst_video_format_get_component_width(fmt, 1, w) * gst_video_format_get_component_height(fmt, 1, h);
                    cs2 = gst_video_format_get_component_width(fmt, 2, w) * gst_video_format_get_component_height(fmt, 2, h);
                    ...
                    {
                        cs3 = gst_video_format_get_component_width(fmt, 3, w) * gst_video_format_get_component_height(fmt, 3, h);
                    ...
                    else
                    {
                        ...
                        /* all components together shouldn't take up more space than size */
                        fail_unless(cs0 + cs1 + cs2 <= size, "cs0 (%d) + cs1 (%d) + cs2 (%d) should be <= size (%d)", cs0, cs1, cs2, size);
                    }
                }
            }
        }
...
GST_START_TEST(test_dar_calc)
{
    ...
    fail_unless(gst_video_calculate_display_ratio(&display_ratio_n, &display_ratio_d, 768, 576, 16, 15, 16, 15));
    fail_unless(display_ratio_n == 4 && display_ratio_d == 3);

    /* video 720x480, par 32/27, display par 1/1 = 16/9 */
    fail_unless(gst_video_calculate_display_ratio(&display_ratio_n, &display_ratio_d, 720, 480, 32, 27, 1, 1));
    fail_unless(display_ratio_n == 16 && display_ratio_d == 9);

    /* video 360x288, par 533333/500000, display par 16/15 = 
     * dar 1599999/1600000 */
    fail_unless(gst_video_calculate_display_ratio(&display_ratio_n, &display_ratio_d, 360, 288, 533333, 500000, 16, 15));
...
GST_START_TEST(test_parse_caps_rgb)
{
    ...
    for (i = 0; i < G_N_ELEMENTS(formats); ++i)
    {
        ...
        gst_caps_set_simple(caps, "width", G_TYPE_INT, 2 * (i + 1), "height", G_TYPE_INT, i + 1, "framerate", GST_TYPE_FRACTION, 15, 1, "pixel-aspect-ratio", GST_TYPE_FRACTION, 1, 1, NULL);
...
GST_START_TEST(test_events)
{
    ...
    fail_unless(gst_video_event_parse_still_frame(e, &in_still), "Failed to parse still frame event");
    fail_unless(gst_video_event_parse_still_frame(e, NULL), "Failed to parse still frame event w/ in_still == NULL");
...
GST_START_TEST(test_convert_frame)
{
    ...
    from_caps = gst_video_format_new_caps(GST_VIDEO_FORMAT_xRGB, 640, 480, 25, 1, 1, 1);
    gst_buffer_set_caps(from_buffer, from_caps);

    to_caps = gst_caps_from_string("something/that, does=(string)not, exist=(boolean)FALSE");

    to_buffer = gst_video_convert_frame(from_buffer, to_caps, GST_CLOCK_TIME_NONE, &error);
    ...
    to_caps = gst_video_format_new_caps(GST_VIDEO_FORMAT_I420, 240, 320, 25, 1, 1, 2);
    to_buffer = gst_video_convert_frame(from_buffer, to_caps, GST_CLOCK_TIME_NONE, &error);
...
static void convert_frame_async_callback(GstBuffer * buf, GError * err, ConvertFrameContext * cf_data)
{
    cf_data->buffer = buf;
    cf_data->error = err;
...
GST_START_TEST(test_convert_frame_async)
{
    ...
    gst_video_convert_frame_async(from_buffer, to_caps, GST_CLOCK_TIME_NONE, (GstVideoConvertFrameCallback) convert_frame_async_callback, &cf_data, NULL);
    ...
    fail_unless(gst_caps_can_intersect(to_caps, GST_BUFFER_CAPS(cf_data.buffer)));
    fail_unless(cf_data.error == NULL);
