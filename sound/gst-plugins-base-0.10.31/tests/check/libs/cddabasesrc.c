...
GST_BOILERPLATE(GstCdFooSrc, gst_cd_foo_src, GstCddaBaseSrc, GST_TYPE_CDDA_BASE_SRC);

static GstBuffer *gst_cd_foo_src_read_sector(GstCddaBaseSrc * src, gint sector);
static gboolean gst_cd_foo_src_open(GstCddaBaseSrc * src, const gchar * device);
static void gst_cd_foo_src_close(GstCddaBaseSrc * src);

static void gst_cd_foo_src_base_init(gpointer g_class)
{
    GstElementClass *element_class = GST_ELEMENT_CLASS(g_class);

    gst_element_class_set_details_simple(element_class, "CD Audio (cdda) Source, FooBar", "Source/File", "Read audio from CD", "Foo Bar <foo@bar.com>");
}

static void gst_cd_foo_src_init(GstCdFooSrc * src, GstCdFooSrcClass * klass)
{
    src->cur_disc = 0;
}

static void gst_cd_foo_src_class_init(GstCdFooSrcClass * klass)
{
...
static gboolean gst_cd_foo_src_open(GstCddaBaseSrc * cddabasesrc, const gchar * device)
{
    GstCddaBaseSrcTrack *tracks;
    GstCdFooSrc *src;
    gint i;
...
static void gst_cd_foo_src_close(GstCddaBaseSrc * cddabasesrc)
{
    ...
    {
        g_assert(g_str_equal(cddabasesrc->mb_discid, src->cur_test->musicbrainz_discid));
    }
}

static GstBuffer *gst_cd_foo_src_read_sector(GstCddaBaseSrc * cddabasesrc, gint sector)
{
    GstBuffer *buf;
...
static inline gboolean tag_list_has_tag(GstTagList * list, const gchar * tag, GType type)
{
    ...
    {
        GST_LOG("tag '%s' in taglist %" GST_PTR_FORMAT " is not of type %s", tag, list, g_type_name(type));
        return FALSE;
    }
...
static void test_uri_parse(const gchar * uri, const gchar * device, gint track)
{
    GstElement *foosrc;
    ...
    fail_unless(gst_uri_handler_set_uri(GST_URI_HANDLER(foosrc), uri) == TRUE, "couldn't set uri %s", uri);
    g_object_get(foosrc, "device", &set_device, "track", &set_track, NULL);
    fail_unless(set_device != NULL);
    fail_unless(strcmp(set_device, device) == 0, "device set was %s, expected %s", set_device, device);
    fail_unless(set_track == track, "track set was %d, expected %d", set_track, track);
    g_free(set_device);
...
GST_START_TEST(test_discid_calculations)
{
    ...
    fail_unless(gst_element_register(NULL, "cdfoosrc", GST_RANK_SECONDARY, GST_TYPE_CD_FOO_SRC));
    ...
    for (i = 0; i < G_N_ELEMENTS(test_discs); ++i)
    {
        ...
        msg = gst_bus_timed_pop_filtered(GST_ELEMENT_BUS(pipeline), GST_CLOCK_TIME_NONE, GST_MESSAGE_TAG);
        ...
        fail_unless(tag_list_has_tag(tags, "musicbrainz-discid-full", G_TYPE_STRING));
        gst_tag_list_free(tags);
        gst_message_unref(msg);

        msg = gst_bus_timed_pop_filtered(GST_ELEMENT_BUS(pipeline), GST_CLOCK_TIME_NONE, GST_MESSAGE_ASYNC_DONE);
        gst_message_unref(msg);
...
GST_START_TEST(test_buffer_timestamps)
{
    ...
    for (i = 0; i < 100; ++i)
    {
        ...
        GST_LOG("buffer, ts=%" GST_TIME_FORMAT ", dur=%" GST_TIME_FORMAT, GST_TIME_ARGS(GST_BUFFER_TIMESTAMP(buf)), GST_TIME_ARGS(GST_BUFFER_DURATION(buf)));
...
GST_START_TEST(test_uri_parsing)
{
    ...
    /* wrong protocol */
    foosrc = gst_element_factory_make("cdfoosrc", "cdfoosrc");
    fail_unless(gst_uri_handler_set_uri(GST_URI_HANDLER(foosrc), "x://") == FALSE);
    fail_unless(gst_uri_handler_set_uri(GST_URI_HANDLER(foosrc), "cddaq://") == FALSE);

    /* cdda://track */
    test_uri_parse("cdda://", "/dev/cdrom", 1);
    test_uri_parse("cdda://2", "/dev/cdrom", 2);
    test_uri_parse("cdda://47", "/dev/cdrom", 47);
    fail_unless(gst_uri_handler_set_uri(GST_URI_HANDLER(foosrc), "cdda://-1") == FALSE);
    fail_unless(gst_uri_handler_set_uri(GST_URI_HANDLER(foosrc), "cdda://what") == FALSE);

    /* cdda://device#track */
    test_uri_parse("cdda:///dev/hdb#1", "/dev/hdb", 1);
    test_uri_parse("cdda://anything#8", "anything", 8);
    fail_unless(gst_uri_handler_set_uri(GST_URI_HANDLER(foosrc), "cdda:///dev/hdb#nonsense") == FALSE);
    fail_unless(gst_uri_handler_set_uri(GST_URI_HANDLER(foosrc), "cdda:///dev/hdb#-2") == FALSE);
...
static Suite *cddabasesrc_suite(void)
{
