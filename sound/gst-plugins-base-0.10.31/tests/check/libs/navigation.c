...
static void nav_send_event(GstNavigation * navigation, GstStructure * structure);

GST_BOILERPLATE_FULL(TestElement, test_element, GstElement, GST_TYPE_ELEMENT, init_interface);

static void test_element_navigation_interface_init(GstNavigationInterface * klass)
{
...
static void init_interface(GType type)
{
    ...
    g_type_add_interface_static(type, GST_TYPE_NAVIGATION, &navigation_iface_info);
}

static void test_element_base_init(gpointer klass)
{
}

static void test_element_class_init(TestElementClass * klass)
{
}

static gboolean test_element_interface_supported(GstImplementsInterface * ifacE, GType interface_type)
{
...
static void nav_send_event(GstNavigation * navigation, GstStructure * structure)
{
    ...
    fail_if(etype == GST_NAVIGATION_EVENT_INVALID, "Received navigation event could not be parsed");
    fail_unless(etype == self->sent_type, "Received navigation event did not match sent");
        ...
        case GST_NAVIGATION_EVENT_MOUSE_BUTTON_RELEASE:
            {
                gint button;
                gdouble x, y;
                fail_unless(gst_navigation_event_parse_mouse_button_event(event, &button, &x, &y));
...
GST_START_TEST(test_events)
{
    ...
    TestElement *test_element = (TestElement *) g_object_new(TEST_ELEMENT_TYPE, NULL);
    ...
    gst_navigation_send_key_event(GST_NAVIGATION(test_element), "key-press", "1");

    test_element->sent_type = GST_NAVIGATION_EVENT_KEY_RELEASE;
    test_element->sent_key = "2";
    gst_navigation_send_key_event(GST_NAVIGATION(test_element), "key-release", "2");

    test_element->sent_type = GST_NAVIGATION_EVENT_MOUSE_MOVE;
    test_element->sent_x = 50;
    test_element->sent_y = 100;
    gst_navigation_send_mouse_event(GST_NAVIGATION(test_element), "mouse-move", 0, 50, 100);
    ...
    gst_navigation_send_mouse_event(GST_NAVIGATION(test_element), "mouse-button-press", 1, 10, 20);
...
GST_START_TEST(test_messages)
{
    ...
    /* GST_NAVIGATION_MESSAGE_MOUSE_OVER */
    {
        ...
        fail_if(m == NULL);
        fail_unless(gst_navigation_message_get_type(m) == GST_NAVIGATION_MESSAGE_MOUSE_OVER);
    ...
    /* GST_NAVIGATION_MESSAGE_COMMANDS_CHANGED */
    {
        ...
        fail_if(m == NULL);
        fail_unless(GST_MESSAGE_SRC(m) == NULL);
        fail_unless(gst_navigation_message_get_type(m) == GST_NAVIGATION_MESSAGE_COMMANDS_CHANGED);
        gst_message_unref(m);
    }
    ...
    /* GST_NAVIGATION_MESSAGE_ANGLES_CHANGED */
    {
        ...
        fail_if(m == NULL);
        fail_unless(GST_MESSAGE_SRC(m) == NULL);
        fail_unless(gst_navigation_message_get_type(m) == GST_NAVIGATION_MESSAGE_ANGLES_CHANGED);
        fail_unless(gst_navigation_message_parse_angles_changed(m, &angle, &angles));
        fail_unless(angle == 1);
...
GST_START_TEST(test_queries)
{
    ...
    /* GST_NAVIGATION_QUERY_COMMANDS */
    {
        ...
        fail_unless(q != NULL);
        fail_unless(gst_navigation_query_get_type(q) == GST_NAVIGATION_QUERY_COMMANDS);
        gst_navigation_query_set_commands(q, 3, GST_NAVIGATION_COMMAND_LEFT, GST_NAVIGATION_COMMAND_MENU1, GST_NAVIGATION_COMMAND_MENU5);
    ...
    /* GST_NAVIGATION_QUERY_ANGLES */
    {
        ...
        fail_unless(q != NULL);
        fail_unless(gst_navigation_query_get_type(q) == GST_NAVIGATION_QUERY_ANGLES);
...
static Suite *navigation_suite(void)
{
