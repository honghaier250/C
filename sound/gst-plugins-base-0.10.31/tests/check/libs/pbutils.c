...
static void missing_msg_check_getters(GstMessage * msg)
{
    gchar *str;
...
GST_START_TEST(test_pb_utils_get_codec_description)
{
    ...
    for (i = 0; i < G_N_ELEMENTS(caps_strings); ++i)
    {
        ...
        fail_unless(caps != NULL, "could not create caps from string '%s'", caps_strings[i]);
...
GST_START_TEST(test_pb_utils_taglist_add_codec_info)
{
    ...
    ASSERT_CRITICAL(fail_if(gst_pb_utils_add_codec_description_to_tag_list(NULL, GST_TAG_VIDEO_CODEC, caps)));
    ASSERT_CRITICAL(fail_if(gst_pb_utils_add_codec_description_to_tag_list(list, NULL, caps)));
    ASSERT_CRITICAL(fail_if(gst_pb_utils_add_codec_description_to_tag_list(list, "asdfa", caps)));
    ASSERT_CRITICAL(fail_if(gst_pb_utils_add_codec_description_to_tag_list(list, GST_TAG_IMAGE, caps)));
    ASSERT_CRITICAL(fail_if(gst_pb_utils_add_codec_description_to_tag_list(list, GST_TAG_VIDEO_CODEC, NULL)));
    /* FIXME: do something here */
    fail_unless(gst_pb_utils_add_codec_description_to_tag_list(list, GST_TAG_VIDEO_CODEC, caps));
...
static void result_cb(GstInstallPluginsReturn result, gpointer user_data)
{
...
static void test_pb_utils_install_plugins_do_callout(gchar ** details, GstInstallPluginsContext * ctx, const gchar * script, GstInstallPluginsReturn expected_result)
{
#ifdef G_OS_UNIX
    ...
    path = g_strdup_printf("%s/gst-plugins-base-unit-test-helper.%s.%lu", g_get_tmp_dir(), (g_get_user_name())? g_get_user_name() : "nobody", (gulong) getpid());
    ...
    fail_unless(ret == GST_INSTALL_PLUGINS_HELPER_MISSING || ret == expected_result, "gst_install_plugins_sync() failed with unexpected ret %d, which is " "neither HELPER_MISSING nor %d", ret, expected_result);

    /* test async callout */
    marker = -333;
    ret = gst_install_plugins_async(details, ctx, result_cb, (gpointer) & marker);
    fail_unless(ret == GST_INSTALL_PLUGINS_HELPER_MISSING || ret == GST_INSTALL_PLUGINS_STARTED_OK, "gst_install_plugins_async() failed with unexpected ret %d", ret);
...
GST_START_TEST(test_pb_utils_install_plugins)
{
    ...
    gchar *details_multi[] = { (char *) "detail1", (char *) "detail1", (char *) "detail2", NULL };

    ctx = gst_install_plugins_context_new();

    ASSERT_CRITICAL(ret = gst_install_plugins_sync(NULL, ctx);
        );
    ASSERT_CRITICAL(ret = gst_install_plugins_async(NULL, ctx, result_cb, (gpointer) & marker);
        );
    ASSERT_CRITICAL(ret = gst_install_plugins_async(details, ctx, NULL, (gpointer) & marker);
        );
    ...
    marker = -333;
    ret = gst_install_plugins_async(details, NULL, result_cb, (gpointer) & marker);
    fail_unless_equals_int(ret, GST_INSTALL_PLUGINS_HELPER_MISSING);
    ...
    test_pb_utils_install_plugins_do_callout(details, NULL, SCRIPT_NO_XID, GST_INSTALL_PLUGINS_NOT_FOUND);

    /* and again with context */
    gst_install_plugins_context_set_xid(ctx, 42);
    test_pb_utils_install_plugins_do_callout(details, ctx, SCRIPT_WITH_XID, GST_INSTALL_PLUGINS_SUCCESS);

    /* and make sure that duplicate detail strings get dropped */
    test_pb_utils_install_plugins_do_callout(details_multi, NULL, SCRIPT_NO_XID, GST_INSTALL_PLUGINS_NOT_FOUND);

    /* and the same again with context */
    gst_install_plugins_context_set_xid(ctx, 42);
    test_pb_utils_install_plugins_do_callout(details_multi, ctx, SCRIPT_WITH_XID, GST_INSTALL_PLUGINS_SUCCESS);
...
GST_START_TEST(test_pb_utils_installer_details)
{
    ...
    /* decoder */
    caps = gst_caps_new_simple("audio/x-spiffy", "spiffyversion", G_TYPE_INT, 2, "channels", G_TYPE_INT, 6, NULL);
    detail1 = gst_missing_decoder_installer_detail_new(caps);
    fail_unless(detail1 != NULL);
    fail_unless(g_str_has_prefix(detail1, "gstreamer|0.10|"));
    fail_unless(g_str_has_suffix(detail1, "|decoder-audio/x-spiffy, spiffyversion=(int)2"));
    ...
    fail_unless(g_str_has_prefix(detail1, "gstreamer|0.10|"));
    fail_unless(g_str_has_suffix(detail1, "|encoder-audio/x-spiffy, spiffyversion=(int)2"));
    fail_unless(detail1 != NULL);
...
static Suite *libgstpbutils_suite(void)
{
