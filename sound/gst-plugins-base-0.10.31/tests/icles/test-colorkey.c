...
static void redraw_overlay(GtkWidget * widget)
{
...
static gboolean handle_resize_cb(GtkWidget * widget, GdkEventConfigure * event, gpointer data)
{
...
static gboolean start_pipeline(gpointer user_data)
{
...
#define COLOR_GRAY 0x7F7F7F
    ...
    g_object_set(G_OBJECT(sink), "autopaint-colorkey", FALSE, "force-aspect-ratio", TRUE, "draw-borders", FALSE, "colorkey", COLOR_GRAY, NULL);
    ...
    arr = gst_property_probe_probe_and_get_values_name(probe, "autopaint-colorkey");
    if (!arr || !arr->n_values)
    {
    ...
    g_signal_connect(bus, "message::state-changed", G_CALLBACK(msg_state_changed), pipeline);
    ...
    g_signal_connect(G_OBJECT(video_window), "configure-event", G_CALLBACK(handle_resize_cb), NULL);
    g_signal_connect(G_OBJECT(video_window), "expose-event", G_CALLBACK(handle_expose_cb), NULL);
