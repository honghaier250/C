...
static void play_file(const gchar * bin, const gint delay, const gchar * uri)
{
    ...
    msg = gst_bus_poll(GST_ELEMENT_BUS(play), GST_MESSAGE_ERROR | GST_MESSAGE_EOS, wait_nanosecs);
    if (msg)
    {
...
static void check_arg(GPtrArray * files, const gchar * arg)
{
    GDir *dir;
