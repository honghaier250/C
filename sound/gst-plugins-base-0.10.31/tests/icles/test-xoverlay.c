...
static gboolean animate_render_rect(gpointer user_data)
{
    if (anim_state.running)
    {
        ...
        gst_x_overlay_set_render_rectangle(anim_state.overlay, r->x, r->y, r->w, r->h);
        gtk_widget_queue_draw(anim_state.widget);
    }
    return TRUE;
}

static gboolean handle_resize_cb(GtkWidget * widget, GdkEventConfigure * event, gpointer user_data)
{
...
static gboolean handle_expose_cb(GtkWidget * widget, GdkEventExpose * event, gpointer user_data)
{
    GstVideoRectangle *r = &anim_state.rect;
    GtkAllocation allocation;
    GdkWindow *window;
    ...
    cairo_rectangle(cr, r->x + r->w, event->area.y, allocation.width - (r->x + r->w), event->area.height);

    cairo_rectangle(cr, event->area.x, 0, event->area.width, r->y);
    cairo_rectangle(cr, event->area.x, r->y + r->h, event->area.width, allocation.height - (r->y + r->h));
...
gint main(gint argc, gchar ** argv)
{
    ...
    g_object_set(G_OBJECT(sink), "handle-events", FALSE, "force-aspect-ratio", force_aspect, "draw-borders", draw_borders, NULL);
    ...
    g_signal_connect(video_window, "configure-event", G_CALLBACK(handle_resize_cb), NULL);
