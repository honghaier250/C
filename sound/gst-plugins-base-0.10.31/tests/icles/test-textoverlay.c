...
static void set_enum_property_by_name(gpointer object, const gchar * prop, const gchar * value)
{
    GParamSpec *pspec;
    GValue val = { 0, };
    GEnumClass *eclass;
...
static void show_text(GstElement * textoverlay, const gchar * txt, const gchar * valign, const gchar * halign, const gchar * line_align)
{
    GstElement *pipe;
...
static void test_textoverlay(int width, int height)
{
    ...
    pstr = g_strdup_printf("videotestsrc pattern=blue ! " "video/x-raw-yuv,width=%d,height=%d ! t.video_sink " "textoverlay name=t font-desc=\"Sans Serif, 20\" ! " " ffmpegcolorspace ! videoscale ! autovideosink", width, height);
    ...
    for (a = 0; a < G_N_ELEMENTS(valigns); ++a)
    {
        for (b = 0; b < G_N_ELEMENTS(haligns); ++b)
        {
            for (c = 0; c < G_N_ELEMENTS(linealigns); ++c)
            {
                ...
                s = g_strdup_printf("line-alignment = %s\n" "&lt;----- halignment = %s -----&gt;\nvalignment = %s", linealigns[c], haligns[b], valigns[a]);
