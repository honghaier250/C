...
static void new_decoded_pad_cb(GstElement * decodebin, GstPad * new_pad, gboolean last, GstElement * pipeline)
{
    GstElement *fakesink;
    GstPad *sinkpad;
    ...
    {
        g_warning("Failed to link %s:%s to %s:%s", GST_DEBUG_PAD_NAME(new_pad), GST_DEBUG_PAD_NAME(sinkpad));
...
static void show_error(const gchar * errmsg, GstBus * bus)
{
    GstMessage *msg;
    GError *err = NULL;
    gchar *dbg = NULL;
...
gint main(gint argc, gchar * argv[])
{
    ...
    g_signal_connect(decodebin, "new-decoded-pad", G_CALLBACK(new_decoded_pad_cb), pipeline);
