...
static void state_cb(GstBus * bus, GstMessage * msg, GstElement * pipeline)
{
...
static void new_decoded_pad_cb(GstElement * decodebin, GstPad * pad, gboolean last, GstElement * pipeline)
{
    ...
    {
        g_printerr("Linked %s:%s to %s:%s\n", GST_DEBUG_PAD_NAME(pad), GST_DEBUG_PAD_NAME(fakesink_pad));
    }
...
gint main(gint argc, gchar * argv[])
{
    ...
    g_signal_connect(bus, "message::state-changed", G_CALLBACK(state_cb), pipeline);
    ...
    g_signal_connect(decoder, "new-decoded-pad", G_CALLBACK(new_decoded_pad_cb), pipeline);
