...
static gboolean update_scale(GstElement * element)
{
    gint64 duration = -1;
    gint64 position = -1;
    ...
    if (gst_element_query_position(element, &format, &position) && position != -1)
    {
    ...
    if (gst_element_query_duration(element, &format, &duration) && duration != -1)
    {
...
static void warning_cb(GstBus * bus, GstMessage * msg, gpointer foo)
{
    GError *err = NULL;
    gchar *dbg = NULL;
...
static void new_clock_cb(GstBus * bus, GstMessage * msg, gpointer nothing)
{
    GstClock *clock;
...
static void clock_lost_cb(GstBus * bus, GstMessage * msg, GstElement * playbin)
{
    GstClock *clock;
...
static void about_to_finish_cb(GstElement * element, gchar * uri[])
{
...
gint main(gint argc, gchar * argv[])
{
    ...
    g_signal_connect(bus, "message::clock-lost", G_CALLBACK(clock_lost_cb), player);

    g_object_set(G_OBJECT(player), "uri", argv[1], NULL);

    arg_count = 2;
    max_count = argc;
    g_signal_connect(player, "about-to-finish", G_CALLBACK(about_to_finish_cb), argv);
