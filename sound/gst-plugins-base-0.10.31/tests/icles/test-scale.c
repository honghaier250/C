...
static GstElement *make_pipeline(gint type)
{
    GstElement *result;
    gchar *pstr;
    ...
    switch (type)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            pstr = g_strdup_printf("videotestsrc peer-alloc=0 ! videoscale ! " "capsfilter name=filter ! " "ximagesink");
            break;
        case 3:
            pstr = g_strdup_printf("videotestsrc peer-alloc=0 ! queue ! videoscale ! " "capsfilter name=filter ! " "ximagesink");
            break;
        case 4:
            pstr = g_strdup_printf("videotestsrc peer-alloc=0 ! videoscale ! queue ! " "capsfilter name=filter ! " "ximagesink");
            break;
        case 5:
            pstr = g_strdup_printf("videotestsrc peer-alloc=0 ! " "capsfilter name=filter ! " "ximagesink");
            break;
        case 6:
            pstr = g_strdup_printf("videotestsrc ! videoscale ! " "capsfilter name=filter ! " "ximagesink");
            break;
        case 7:
            pstr = g_strdup_printf("v4l2src ! ffmpegcolorspace ! videoscale ! " "capsfilter name=filter ! " "ximagesink");
            break;
        default:
            return NULL;
    }
...
int main(int argc, char **argv)
{
    ...
    while (TRUE)
    {
        ...
        for (round = 0; round < MAX_ROUND; round++)
        {
             ...
             * as well */
            capsstr = g_strdup_printf("video/x-raw-rgb, width=(int)%d, height=(int)%d;" "video/x-raw-rgb", width, height);
