...
static void check_position(GstElement * elem, GstQuery * pos, const gchar * info)
{
...
static gboolean print_buffer_ts(GstPad * pad, GstBuffer * buffer, gpointer user_data)
{
...
gint main(gint argc, gchar ** argv)
{
    ...
    sink = gst_element_factory_make((be_quiet ? "fakesink" : "autoaudiosink"), NULL);
    if (!sink)
    {
    ...
    pos_seek = gst_event_new_seek(1.0, GST_FORMAT_TIME, flags, GST_SEEK_TYPE_SET, 3 * GST_SECOND, GST_SEEK_TYPE_NONE, GST_CLOCK_TIME_NONE);
    rate_seek1 = gst_event_new_seek(0.5, GST_FORMAT_TIME, flags, GST_SEEK_TYPE_NONE, GST_CLOCK_TIME_NONE, GST_SEEK_TYPE_NONE, GST_CLOCK_TIME_NONE);
    rate_seek2 = gst_event_new_seek(-1.0, GST_FORMAT_TIME, flags, GST_SEEK_TYPE_NONE, GST_CLOCK_TIME_NONE, GST_SEEK_TYPE_NONE, GST_CLOCK_TIME_NONE);
    ...
    if (gst_element_set_state(bin, GST_STATE_PAUSED) != GST_STATE_CHANGE_FAILURE)
    {
        ...
        /* run for 5 seconds */
        clock_id = gst_clock_new_single_shot_id(clock, gst_clock_get_time(clock) + (5 * GST_SECOND));
        ...
        /* run for 2 seconds */
        clock_id = gst_clock_new_single_shot_id(clock, gst_clock_get_time(clock) + (2 * GST_SECOND));
        if ((wait_ret = gst_clock_id_wait(clock_id, NULL)) != GST_CLOCK_OK)
        {
            GST_WARNING("clock_id_wait returned: %d", wait_ret);
        }
        ...
        /* run for 4 seconds */
        clock_id = gst_clock_new_single_shot_id(clock, gst_clock_get_time(clock) + (4 * GST_SECOND));
        if ((wait_ret = gst_clock_id_wait(clock_id, NULL)) != GST_CLOCK_OK)
        {
            GST_WARNING("clock_id_wait returned: %d", wait_ret);
        }
        ...
        /* run for 7 seconds */
        clock_id = gst_clock_new_single_shot_id(clock, gst_clock_get_time(clock) + (7 * GST_SECOND));
        if ((wait_ret = gst_clock_id_wait(clock_id, NULL)) != GST_CLOCK_OK)
        {
            GST_WARNING("clock_id_wait returned: %d", wait_ret);
        }
