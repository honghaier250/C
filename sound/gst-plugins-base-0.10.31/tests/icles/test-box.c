...
static GstElement *make_pipeline(gint type)
{
    ...
    {
        case 0:
            pstr = g_strdup_printf("videotestsrc ! " CAPS " ! videobox name=box ! videoscale ! " CAPS " ! ffmpegcolorspace ! ximagesink");
            break;
        default:
            return NULL;
    }
...
int main(int argc, char **argv)
{
    ...
    while (TRUE)
    {
        ...
        {
            g_print("box to %dx%d %dx%d (%d/%d)   \r", left, right, top, bottom, round, MAX_ROUND);

            g_object_set(filter, "left", left, "right", right, "top", top, "bottom", bottom, NULL);
