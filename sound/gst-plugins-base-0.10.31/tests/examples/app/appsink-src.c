...
const gchar *audio_caps = "audio/x-raw-int,channels=1,rate=8000,signed=(boolean)true,width=16,depth=16,endianness=1234";
...
int main(int argc, char *argv[])
{
     ...
     * caps. */
    string = g_strdup_printf("filesrc location=\"%s\" ! wavparse ! audioconvert ! audioresample ! appsink caps=\"%s\" name=testsink", filename, audio_caps);
    g_free(filename);
    ...
    g_signal_connect(testsink, "new-buffer", G_CALLBACK(on_new_buffer_from_source), data);
     ...
     * memory. */
    string = g_strdup_printf("appsrc name=testsource caps=\"%s\" ! autoaudiosink", audio_caps);
