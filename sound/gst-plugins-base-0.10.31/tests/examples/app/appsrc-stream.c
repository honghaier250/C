...
static gboolean read_data(App * app)
{
    ...
    GST_DEBUG("feed buffer %p, offset %" G_GUINT64_FORMAT "-%u", buffer, app->offset, len);
...
static void start_feed(GstElement * playbin, guint size, App * app)
{
    if (app->sourceid == 0)
    {
...
static void stop_feed(GstElement * playbin, App * app)
{
    if (app->sourceid != 0)
    {
