...
static void feed_data(GstElement * appsrc, guint size, App * app)
{
    ...
    GST_DEBUG("feed buffer %p, offset %" G_GUINT64_FORMAT "-%u", buffer, app->offset, size);
 ...
 * call to push-buffer. */
static gboolean seek_data(GstElement * appsrc, guint64 position, App * app)
{
...
static void found_source(GObject * object, GObject * orig, GParamSpec * pspec, App * app)
{
    ...
    gst_util_set_object_arg(G_OBJECT(app->appsrc), "stream-type", "random-access");
