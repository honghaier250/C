        ...
        case GST_MESSAGE_ELEMENT:
            {
                ...
                if (strcmp(name, "not-mounted") == 0)
                {
                    ...
                    GFile *file = G_FILE(g_value_get_object(gst_structure_get_value(message->structure, "file")));

                    g_print("not-mounted\n");
                    gst_element_set_state(pipeline, GST_STATE_NULL);
                    gst_bus_set_flushing(bus, TRUE);

                    g_file_mount_enclosing_volume(file, G_MOUNT_MOUNT_NONE, mop, NULL, mount_cb, bus);
