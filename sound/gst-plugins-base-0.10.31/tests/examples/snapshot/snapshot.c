...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2)
    {
        g_print("usage: %s <uri>\n Writes snapshot.png in the current directory", argv[0]);
        exit(-1);
    }

    /* create a new pipeline */
    descr = g_strdup_printf("uridecodebin uri=%s ! ffmpegcolorspace ! videoscale ! " " appsink name=sink caps=\"" CAPS "\"", argv[1]);
