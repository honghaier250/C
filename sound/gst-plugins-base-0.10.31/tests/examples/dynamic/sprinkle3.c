 ...
 * adder */
static SourceInfo *add_source(gdouble freq, gfloat pos)
{
    SourceInfo *info;
...
static void remove_source(SourceInfo * info)
{
...
static SprinkleState *create_state(void)
{
    SprinkleState *state;
...
static void free_state(SprinkleState * state)
{
    SourceInfo *info;
    gint i;
...
static gboolean do_sprinkle(SprinkleState * state)
{
    SourceInfo *info;
    gint i;
    ...
    if (state->count < 20)
    {
        state->infos[0] = add_source((gdouble) ((state->count * 100) + 200), ((gfloat) (state->count % 5) / 2.0 - 1.0));
        state->count++;
    }
    else
    {
    ...
    GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(GST_BIN(pipeline),
                                      ...
                                      GST_DEBUG_GRAPH_SHOW_CAPS_DETAILS | GST_DEBUG_GRAPH_SHOW_STATES, "sprinkle3");
    return TRUE;
}
...
int main(int argc, char *argv[])
{
    ...
    caps = gst_caps_new_simple("audio/x-raw-int", "endianness", G_TYPE_INT, G_LITTLE_ENDIAN, "channels", G_TYPE_INT, 2, "width", G_TYPE_INT, 16, "depth", G_TYPE_INT, 16, "rate", G_TYPE_INT, 44100, "signed", G_TYPE_BOOLEAN, TRUE, NULL);
