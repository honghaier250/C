...
static gboolean read_joystick(GIOChannel * source, GIOCondition condition, gpointer user_data)
{
    ...
    result = g_io_channel_read_chars(source, buf, sizeof(struct js_event), &bytes_read, &err);
    if (err)
    {
...
int main(int argc, char **argv)
{
    ...
    if (g_strrstr(pipeline_spec, "*") != NULL || g_strrstr(pipeline_spec, "?") != NULL)
    {
