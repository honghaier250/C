...
static void gtk_widget_get_allocation(GtkWidget * w, GtkAllocation * a)
{
    *a = w->allocation;
}
#endif
...
static void volume_notify_cb(GstElement * pipeline, GParamSpec * arg, gpointer user_dat);
...
static GstElement *gst_element_factory_make_or_warn(const gchar * type, const gchar * name)
{
...
static void setup_dynamic_link(GstElement * element, const gchar * padname, GstPad * target, GstElement * bin)
{
    dyn_link *connect;
...
static GstElement *make_mod_pipeline(const gchar * location)
{
    GstElement *pipeline;
    ...
    rate_pads = g_list_prepend(rate_pads, gst_element_get_static_pad(decoder, "sink"));

    return pipeline;
}

static GstElement *make_dv_pipeline(const gchar * location)
{
    GstElement *pipeline;
    ...
    setup_dynamic_link(demux, "video", gst_element_get_static_pad(v_queue, "sink"), NULL);
    setup_dynamic_link(demux, "audio", gst_element_get_static_pad(a_queue, "sink"), NULL);
...
static GstElement *make_wav_pipeline(const gchar * location)
{
    GstElement *pipeline;
    GstElement *src, *decoder, *audiosink;
    ...
    setup_dynamic_link(decoder, "src", gst_element_get_static_pad(audiosink, "sink"), NULL);
...
static GstElement *make_flac_pipeline(const gchar * location)
{
    GstElement *pipeline;
    GstElement *src, *decoder, *audiosink;
    GstPad *seekable;
...
static GstElement *make_sid_pipeline(const gchar * location)
{
    GstElement *pipeline;
    GstElement *src, *decoder, *audiosink;
    GstPad *seekable;
...
static GstElement *make_parse_pipeline(const gchar * location)
{
    GstElement *pipeline;
    ...
    seekable_pads = g_list_prepend(seekable_pads, seekable);
    rate_pads = g_list_prepend(rate_pads, seekable);
    rate_pads = g_list_prepend(rate_pads, gst_element_get_static_pad(parser, "sink"));

    return pipeline;
}

static GstElement *make_vorbis_pipeline(const gchar * location)
{
    ...
    setup_dynamic_link(demux, NULL, gst_element_get_static_pad(audio_bin, "sink"), NULL);
...
static GstElement *make_theora_pipeline(const gchar * location)
{
    ...
    setup_dynamic_link(demux, NULL, gst_element_get_static_pad(video_bin, "sink"), NULL);
...
static GstElement *make_vorbis_theora_pipeline(const gchar * location)
{
    ...
    v_convert = gst_element_factory_make_or_warn("ffmpegcolorspace", "v_convert");
    ...
    gst_element_link_many(v_queue, v_decoder, v_convert, v_scale, videosink, NULL);
    ...
    seekable_pads = g_list_prepend(seekable_pads, seekable);
    rate_pads = g_list_prepend(rate_pads, seekable);
    rate_pads = g_list_prepend(rate_pads, gst_element_get_static_pad(a_decoder, "sink"));

    return pipeline;
}

static GstElement *make_avi_msmpeg4v3_mp3_pipeline(const gchar * location)
{
    GstElement *pipeline, *audio_bin, *video_bin;
    GstElement *src, *demux, *a_decoder, *a_convert, *v_decoder, *v_convert;
    GstElement *audiosink, *videosink;
...
static GstElement *make_mp3_pipeline(const gchar * location)
{
    GstElement *pipeline;
...
static GstElement *make_avi_pipeline(const gchar * location)
{
    GstElement *pipeline, *audio_bin, *video_bin;
    ...
    setup_dynamic_link(demux, "audio_00", gst_element_get_static_pad(a_decoder, "sink"), audio_bin);
    ...
    setup_dynamic_link(demux, "video_00", gst_element_get_static_pad(v_decoder, "sink"), video_bin);

    seekable = gst_element_get_static_pad(v_queue, "src");
    seekable_pads = g_list_prepend(seekable_pads, seekable);
    rate_pads = g_list_prepend(rate_pads, seekable);
    rate_pads = g_list_prepend(rate_pads, gst_element_get_static_pad(v_decoder, "sink"));

    return pipeline;
}

static GstElement *make_mpeg_pipeline(const gchar * location)
{
    GstElement *pipeline, *audio_bin, *video_bin;
    ...
    setup_dynamic_link(demux, "audio_c0", gst_element_get_static_pad(audio_bin, "sink"), NULL);
    ...
    setup_dynamic_link(demux, "video_e0", gst_element_get_static_pad(video_bin, "sink"), NULL);
...
static GstElement *make_mpegnt_pipeline(const gchar * location)
{
    GstElement *pipeline, *audio_bin, *video_bin;
    GstElement *src, *demux, *a_decoder, *v_decoder, *v_filter;
    GstElement *audiosink, *videosink;
...
static void playerbin_set_uri(GstElement * player, const gchar * location)
{
    gchar *uri;
...
static GstElement *construct_playerbin(const gchar * name, const gchar * location)
{
    GstElement *player;
...
static GstElement *make_playerbin_pipeline(const gchar * location)
{
    return construct_playerbin("playbin", location);
}

static GstElement *make_playerbin2_pipeline(const gchar * location)
{
    ...
    g_signal_connect(pipeline, "notify::volume", G_CALLBACK(volume_notify_cb), NULL);
    return pipeline;
}

#ifndef GST_DISABLE_PARSE
static GstElement *make_parselaunch_pipeline(const gchar * description)
{
    GstElement *pipeline;
    GError *error = NULL;
...
static gchar *format_value(GtkScale * scale, gdouble value)
{
    ...
    return g_strdup_printf("%02" G_GINT64_FORMAT ":%02" G_GINT64_FORMAT ":%02" G_GINT64_FORMAT, seconds / 60, seconds % 60, subseconds % 100);
}

static gchar *shuttle_format_value(GtkScale * scale, gdouble value)
{
...
G_GNUC_UNUSED static void query_rates(void)
{
    ...
    while (walk)
    {
        ...
        while (seek_formats[i].name)
        {
            ...
            if (gst_pad_query_convert(pad, GST_FORMAT_TIME, GST_SECOND, &format, &value))
            {
...
G_GNUC_UNUSED static void query_positions_elems(void)
{
    ...
    while (walk)
    {
        ...
        while (seek_formats[i].name)
        {
            ...
            if (gst_element_query_position(element, &format, &position) && gst_element_query_duration(element, &format, &total))
            {
                g_print("%s %13" G_GINT64_FORMAT " / %13" G_GINT64_FORMAT " | ", seek_formats[i].name, position, total);
            }
            else
            {
...
G_GNUC_UNUSED static void query_positions_pads(void)
{
    ...
    while (walk)
    {
        ...
        while (seek_formats[i].name)
        {
            ...
            if (gst_pad_query_position(pad, &format, &position) && gst_pad_query_duration(pad, &format, &total))
            {
                g_print("%s %13" G_GINT64_FORMAT " / %13" G_GINT64_FORMAT " | ", seek_formats[i].name, position, total);
            }
            else
...
static gboolean start_seek(GtkWidget * widget, GdkEventButton * event, gpointer user_data);
static gboolean stop_seek(GtkWidget * widget, GdkEventButton * event, gpointer user_data);
static void seek_cb(GtkWidget * widget);

static void set_scale(gdouble value)
{
    g_signal_handlers_block_by_func(hscale, (void *) start_seek, (void *) pipeline);
    g_signal_handlers_block_by_func(hscale, (void *) stop_seek, (void *) pipeline);
    g_signal_handlers_block_by_func(hscale, (void *) seek_cb, (void *) pipeline);
    gtk_adjustment_set_value(adjustment, value);
    g_signal_handlers_unblock_by_func(hscale, (void *) start_seek, (void *) pipeline);
    g_signal_handlers_unblock_by_func(hscale, (void *) stop_seek, (void *) pipeline);
    g_signal_handlers_unblock_by_func(hscale, (void *) seek_cb, (void *) pipeline);
    gtk_widget_queue_draw(hscale);
}

static gboolean update_fill(gpointer data)
{
    if (elem_seek)
    {
        if (seekable_elements)
        {
            ...
            if (gst_element_query(element, query))
            {
                ...
                gst_query_parse_buffering_range(query, &format, &start, &stop, &buffering_total);
                 ...
                 * playback time */
                GST_DEBUG("buffering total %" G_GINT64_FORMAT " ms, left %" G_GINT64_FORMAT " ms", buffering_total, buffering_left);
                GST_DEBUG("start %" G_GINT64_FORMAT ", stop %" G_GINT64_FORMAT, start, stop);
...
static gboolean end_scrub(GtkWidget * widget)
{
...
static gboolean send_event(GstEvent * event)
{
    gboolean res = FALSE;
...
static void do_seek(GtkWidget * widget)
{
    ...
    GST_DEBUG("value=%f, real=%" G_GINT64_FORMAT, gtk_range_get_value(GTK_RANGE(widget)), real);
    ...
    if (rate >= 0)
    {
        s_event = gst_event_new_seek(rate, GST_FORMAT_TIME, flags, GST_SEEK_TYPE_SET, real, GST_SEEK_TYPE_SET, GST_CLOCK_TIME_NONE);
        GST_DEBUG("seek with rate %lf to %" GST_TIME_FORMAT " / %" GST_TIME_FORMAT, rate, GST_TIME_ARGS(real), GST_TIME_ARGS(duration));
    }
    else
    {
        s_event = gst_event_new_seek(rate, GST_FORMAT_TIME, flags, GST_SEEK_TYPE_SET, G_GINT64_CONSTANT(0), GST_SEEK_TYPE_SET, real);
        GST_DEBUG("seek with rate %lf to %" GST_TIME_FORMAT " / %" GST_TIME_FORMAT, rate, GST_TIME_ARGS(0), GST_TIME_ARGS(real));
    }
...
static void seek_cb(GtkWidget * widget)
{
    ...
    if (play_scrub)
    {
        ...
        {
            seek_timeout_id = g_timeout_add(SCRUB_TIME, (GSourceFunc) end_scrub, widget);
        }
    }
}
...
static void set_update_fill(gboolean active)
{
    ...
    if (active)
    {
        if (fill_id == 0)
        {
            fill_id = g_timeout_add(FILL_INTERVAL, (GtkFunction) update_fill, pipeline);
        }
    }
    else
    {
...
static void set_update_scale(gboolean active)
{
    ...
    if (active)
    {
        if (update_id == 0)
        {
            update_id = g_timeout_add(UPDATE_INTERVAL, (GtkFunction) update_scale, pipeline);
        }
    }
    else
    {
...
static gboolean start_seek(GtkWidget * widget, GdkEventButton * event, gpointer user_data)
{
    ...
    {
        changed_id = g_signal_connect(hscale, "value_changed", G_CALLBACK(seek_cb), pipeline);
    }
...
static void accurate_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
    accurate_seek = gtk_toggle_button_get_active(button);
}

static void key_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
    keyframe_seek = gtk_toggle_button_get_active(button);
}

static void loop_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
...
static void flush_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
    flush_seek = gtk_toggle_button_get_active(button);
}

static void scrub_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
    scrub = gtk_toggle_button_get_active(button);
}

static void play_scrub_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
    play_scrub = gtk_toggle_button_get_active(button);
}

static void skip_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
...
static void rate_spinbutton_changed_cb(GtkSpinButton * button, GstPipeline * pipeline)
{
    gboolean res = FALSE;
    GstEvent *s_event;
    GstSeekFlags flags;
    ...
    if (rate >= 0.0)
    {
        s_event = gst_event_new_seek(rate, GST_FORMAT_TIME, flags, GST_SEEK_TYPE_SET, position, GST_SEEK_TYPE_SET, GST_CLOCK_TIME_NONE);
    }
    else
    {
        s_event = gst_event_new_seek(rate, GST_FORMAT_TIME, flags, GST_SEEK_TYPE_SET, G_GINT64_CONSTANT(0), GST_SEEK_TYPE_SET, position);
    }
...
static void update_flag(GstPipeline * pipeline, gint num, gboolean state)
{
    gint flags;
...
static void vis_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
    gboolean state;
...
static void audio_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
    gboolean state;
...
static void video_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
    gboolean state;
...
static void text_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
    gboolean state;
...
static void mute_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
    gboolean mute;
...
static void download_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
    gboolean state;
...
static void buffer_toggle_cb(GtkToggleButton * button, GstPipeline * pipeline)
{
    gboolean state;
...
static void update_streams(GstPipeline * pipeline)
{
    gint i;
...
static void video_combo_cb(GtkComboBox * combo, GstPipeline * pipeline)
{
    gint active;
...
static void audio_combo_cb(GtkComboBox * combo, GstPipeline * pipeline)
{
    gint active;
...
static void text_combo_cb(GtkComboBox * combo, GstPipeline * pipeline)
{
    gint active;
...
static void init_visualization_features(void)
{
    GList *list, *walk;

    vis_entries = g_array_new(FALSE, FALSE, sizeof(VisEntry));

    list = gst_registry_feature_filter(gst_registry_get_default(), filter_features, FALSE, NULL);
...
static void vis_combo_cb(GtkComboBox * combo, GstPipeline * pipeline)
{
    guint index;
    VisEntry *entry;
    GstElement *element;
...
static void volume_spinbutton_changed_cb(GtkSpinButton * button, GstPipeline * pipeline)
{
    gdouble volume;
...
static void volume_notify_cb(GstElement * pipeline, GParamSpec * arg, gpointer user_dat)
{
    ...
    {
        g_signal_handlers_block_by_func(volume_spinbutton, volume_spinbutton_changed_cb, pipeline);
        gtk_spin_button_set_value(GTK_SPIN_BUTTON(volume_spinbutton), new_volume);
        g_signal_handlers_unblock_by_func(volume_spinbutton, volume_spinbutton_changed_cb, pipeline);
    }
}

static void shot_cb(GtkButton * button, gpointer data)
{
    GstBuffer *buffer;
    GstCaps *caps;

    /* convert to our desired format (RGB24) */
    caps = gst_caps_new_simple("video/x-raw-rgb", "bpp", G_TYPE_INT, 24, "depth", G_TYPE_INT, 24,
                               /* Note: we don't ask for a specific width/height here, so that
                                * videoscale can adjust dimensions from a non-1/1 pixel aspect
                                * ratio to a 1/1 pixel-aspect-ratio */
                               "pixel-aspect-ratio", GST_TYPE_FRACTION, 1, 1, "endianness", G_TYPE_INT, G_BIG_ENDIAN, "red_mask", G_TYPE_INT, 0xff0000, "green_mask", G_TYPE_INT, 0x00ff00, "blue_mask", G_TYPE_INT, 0x0000ff, NULL);
    ...
    if (buffer)
    {
        ...
        pixbuf = gdk_pixbuf_new_from_data(GST_BUFFER_DATA(buffer), GDK_COLORSPACE_RGB, FALSE, 8, width, height, GST_ROUND_UP_4(width * 3), NULL, NULL);
...
static void step_cb(GtkButton * button, gpointer data)
{
    GstEvent *event;
    GstFormat format;
    guint64 amount;
    ...
    amount = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(step_amount_spinbutton));
...
static void message_received(GstBus * bus, GstMessage * message, GstPipeline * pipeline)
{
    ...
    g_print("message from \"%s\" (%s): ", GST_STR_NULL(GST_ELEMENT_NAME(GST_MESSAGE_SRC(message))), gst_message_type_get_name(GST_MESSAGE_TYPE(message)));
    if (s)
    {
        gchar *sstr;
...
static void do_shuttle(GstElement * element)
{
    guint64 duration;
    ...
    gst_element_send_event(element, gst_event_new_step(GST_FORMAT_TIME, duration, shuttle_rate, FALSE, FALSE));
}

static void msg_sync_step_done(GstBus * bus, GstMessage * message, GstElement * element)
{
    GstFormat format;
    guint64 amount;
    gdouble rate;
...
static void shuttle_toggled(GtkToggleButton * button, GstElement * element)
{
    gboolean active;
...
static void shuttle_rate_switch(GstElement * element)
{
    GstSeekFlags flags;
    GstEvent *s_event;
    gboolean res;
    ...
    if (play_rate >= 0.0)
    {
        s_event = gst_event_new_seek(play_rate, GST_FORMAT_TIME, flags, GST_SEEK_TYPE_SET, position, GST_SEEK_TYPE_SET, GST_CLOCK_TIME_NONE);
    }
    else
    {
        s_event = gst_event_new_seek(play_rate, GST_FORMAT_TIME, flags, GST_SEEK_TYPE_SET, G_GINT64_CONSTANT(0), GST_SEEK_TYPE_SET, position);
    }
    res = send_event(s_event);
    if (res)
    {
...
static void shuttle_value_changed(GtkRange * range, GstElement * element)
{
    gdouble rate;
...
static void msg_async_done(GstBus * bus, GstMessage * message, GstPipeline * pipeline)
{
...
static void msg_state_changed(GstBus * bus, GstMessage * message, GstPipeline * pipeline)
{
    const GstStructure *s;
...
static void msg_segment_done(GstBus * bus, GstMessage * message, GstPipeline * pipeline)
{
    GstEvent *s_event;
    GstSeekFlags flags;
    gboolean res;
    ...
    s_event = gst_event_new_seek(rate, GST_FORMAT_TIME, flags, GST_SEEK_TYPE_SET, G_GINT64_CONSTANT(0), GST_SEEK_TYPE_SET, duration);

    GST_DEBUG("restart loop with rate %lf to 0 / %" GST_TIME_FORMAT, rate, GST_TIME_ARGS(duration));
 ...
 * message */
static void do_stream_buffering(gint percent)
{
    gchar *bufstr;
...
static void do_download_buffering(gint percent)
{
...
static void msg_buffering(GstBus * bus, GstMessage * message, GstPipeline * data)
{
    gint percent;
...
static void msg_clock_lost(GstBus * bus, GstMessage * message, GstPipeline * data)
{
...
static GstBusSyncReply bus_sync_handler(GstBus * bus, GstMessage * message, GstPipeline * data)
{
...
static gboolean handle_expose_cb(GtkWidget * widget, GdkEventExpose * event, gpointer data)
{
    if (state < GST_STATE_PAUSED)
    {
...
static void msg_eos(GstBus * bus, GstMessage * message, GstPipeline * data)
{
...
static void msg_step_done(GstBus * bus, GstMessage * message, GstPipeline * data)
{
    if (!shuttling)
        message_received(bus, message, data);
}
...
static void connect_bus_signals(GstElement * pipeline)
{
    ...
    g_signal_connect(bus, "message::state-changed", (GCallback) msg_state_changed, pipeline);
    g_signal_connect(bus, "message::segment-done", (GCallback) msg_segment_done, pipeline);
    g_signal_connect(bus, "message::async-done", (GCallback) msg_async_done, pipeline);

    g_signal_connect(bus, "message::new-clock", (GCallback) message_received, pipeline);
    g_signal_connect(bus, "message::clock-lost", (GCallback) msg_clock_lost, pipeline);
    ...
    g_signal_connect(bus, "message::tag", (GCallback) message_received, pipeline);
    g_signal_connect(bus, "message::element", (GCallback) message_received, pipeline);
    g_signal_connect(bus, "message::segment-done", (GCallback) message_received, pipeline);
    g_signal_connect(bus, "message::buffering", (GCallback) msg_buffering, pipeline);
//  g_signal_connect (bus, "message::step-done", (GCallback) msg_step_done,
//      pipeline);
    g_signal_connect(bus, "message::step-start", (GCallback) msg_step_done, pipeline);
    g_signal_connect(bus, "sync-message::step-done", (GCallback) msg_sync_step_done, pipeline);

    gst_object_unref(bus);
}

/* Return GList of paths described in location string */
static GList *handle_wildcards(const gchar * location)
{
    GList *res = NULL;
...
static void delete_event_cb(void)
{
    stop_cb(NULL, NULL);
    gtk_main_quit();
}
...
{
    GtkWidget *window, *hbox, *vbox, *panel, *expander, *pb2vbox, *boxes, *flagtable, *boxes2, *step;
    ...
    if (g_path_is_absolute(pipeline_spec) && (g_strrstr(pipeline_spec, "*") != NULL || g_strrstr(pipeline_spec, "?") != NULL))
    {
    ...
    g_signal_connect(video_window, "expose-event", G_CALLBACK(handle_expose_cb), NULL);
    ...
    gtk_widget_set_tooltip_text(accurate_checkbox, "accurate position is requested, this might be considerably slower for some formats");
    gtk_widget_set_tooltip_text(key_checkbox, "seek to the nearest keyframe. This might be faster but less accurate");
    ...
    gtk_widget_set_tooltip_text(rate_spinbutton, "define the playback rate, " "negative value trigger reverse playback");
    ...
    gtk_widget_set_tooltip_text(skip_checkbox, "Skip frames while playing at high frame rates");
    ...
    /* step expander */
    {
        ...
        gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(format_combo), "time (ms)");
        ...
        gtk_box_pack_start(GTK_BOX(hbox), step_amount_spinbutton, FALSE, FALSE, 2);
        ...
        g_signal_connect(G_OBJECT(step_button), "clicked", G_CALLBACK(step_cb), pipeline);
        ...
        g_signal_connect(shuttle_checkbox, "toggled", G_CALLBACK(shuttle_toggled), pipeline);

        shuttle_adjustment = GTK_ADJUSTMENT(gtk_adjustment_new(0.0, -3.00, 4.0, 0.1, 1.0, 1.0));
        ...
        gtk_range_set_update_policy(GTK_RANGE(shuttle_hscale), GTK_UPDATE_CONTINUOUS);
        g_signal_connect(shuttle_hscale, "value_changed", G_CALLBACK(shuttle_value_changed), pipeline);
        g_signal_connect(shuttle_hscale, "format_value", G_CALLBACK(shuttle_format_value), pipeline);
    ...
    /* seek bar */
    adjustment = GTK_ADJUSTMENT(gtk_adjustment_new(0.0, 0.00, N_GRAD, 0.1, 1.0, 1.0));
    ...
    g_signal_connect(hscale, "button_press_event", G_CALLBACK(start_seek), pipeline);
    g_signal_connect(hscale, "button_release_event", G_CALLBACK(stop_seek), pipeline);
    g_signal_connect(hscale, "format_value", G_CALLBACK(format_value), pipeline);
    ...
    if (pipeline_type == 16)
    {
        ...
        g_signal_connect(G_OBJECT(video_combo), "changed", G_CALLBACK(video_combo_cb), pipeline);
        g_signal_connect(G_OBJECT(audio_combo), "changed", G_CALLBACK(audio_combo_cb), pipeline);
        g_signal_connect(G_OBJECT(text_combo), "changed", G_CALLBACK(text_combo_cb), pipeline);
        ...
        g_signal_connect(G_OBJECT(vis_checkbox), "toggled", G_CALLBACK(vis_toggle_cb), pipeline);
        g_signal_connect(G_OBJECT(audio_checkbox), "toggled", G_CALLBACK(audio_toggle_cb), pipeline);
        g_signal_connect(G_OBJECT(video_checkbox), "toggled", G_CALLBACK(video_toggle_cb), pipeline);
        g_signal_connect(G_OBJECT(text_checkbox), "toggled", G_CALLBACK(text_toggle_cb), pipeline);
        g_signal_connect(G_OBJECT(mute_checkbox), "toggled", G_CALLBACK(mute_toggle_cb), pipeline);
        g_signal_connect(G_OBJECT(download_checkbox), "toggled", G_CALLBACK(download_toggle_cb), pipeline);
        g_signal_connect(G_OBJECT(buffer_checkbox), "toggled", G_CALLBACK(buffer_toggle_cb), pipeline);
        g_signal_connect(G_OBJECT(volume_spinbutton), "value_changed", G_CALLBACK(volume_spinbutton_changed_cb), pipeline);
        /* playbin2 panel for snapshot */
        boxes2 = gtk_hbox_new(FALSE, 0);
        shot_button = gtk_button_new_from_stock(GTK_STOCK_SAVE);
        gtk_widget_set_tooltip_text(shot_button, "save a screenshot .png in the current directory");
        g_signal_connect(G_OBJECT(shot_button), "clicked", G_CALLBACK(shot_cb), pipeline);
        vis_combo = gtk_combo_box_text_new();
        g_signal_connect(G_OBJECT(vis_combo), "changed", G_CALLBACK(vis_combo_cb), pipeline);
    ...
    gtk_widget_set_size_request(GTK_WIDGET(video_window), -1, DEFAULT_VIDEO_HEIGHT);
    ...
    gtk_table_attach_defaults(GTK_TABLE(flagtable), accurate_checkbox, 0, 1, 0, 1);
    ...
    gtk_table_attach_defaults(GTK_TABLE(flagtable), play_scrub_checkbox, 2, 3, 1, 2);
    ...
    gtk_table_attach_defaults(GTK_TABLE(flagtable), rate_spinbutton, 4, 5, 1, 2);
    ...
    g_signal_connect(G_OBJECT(accurate_checkbox), "toggled", G_CALLBACK(accurate_toggle_cb), pipeline);
    g_signal_connect(G_OBJECT(key_checkbox), "toggled", G_CALLBACK(key_toggle_cb), pipeline);
    g_signal_connect(G_OBJECT(loop_checkbox), "toggled", G_CALLBACK(loop_toggle_cb), pipeline);
    g_signal_connect(G_OBJECT(flush_checkbox), "toggled", G_CALLBACK(flush_toggle_cb), pipeline);
    g_signal_connect(G_OBJECT(scrub_checkbox), "toggled", G_CALLBACK(scrub_toggle_cb), pipeline);
    g_signal_connect(G_OBJECT(play_scrub_checkbox), "toggled", G_CALLBACK(play_scrub_toggle_cb), pipeline);
    g_signal_connect(G_OBJECT(skip_checkbox), "toggled", G_CALLBACK(skip_toggle_cb), pipeline);
    g_signal_connect(G_OBJECT(rate_spinbutton), "value_changed", G_CALLBACK(rate_spinbutton_changed_cb), pipeline);
