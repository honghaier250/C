...
static void dynamic_link(GstPadTemplate * templ, GstPad * newpad, gpointer data)
{
    ...
    if (connect->padname == NULL || !strcmp(gst_pad_get_name(newpad), connect->padname))
    {
        if (connect->bin)
...
static gboolean update_scale(gpointer data)
{
    ...
    if (duration > 0)
    {
        gtk_adjustment_set_value(adjustment, position * (gdouble) RANGE_PREC / duration);
...
static void speed_cb(GtkWidget * widget)
{
    GstEvent *s_event;
    gboolean res;
    ...
    s_event = gst_event_new_seek(cur_speed, GST_FORMAT_TIME, 0, GST_SEEK_TYPE_NONE, -1, GST_SEEK_TYPE_NONE, -1);
...
static gboolean do_seek(GtkWidget * widget, gboolean flush, gboolean segment)
{
    ...
    GST_DEBUG("cur:  %lf, %" GST_TIME_FORMAT, cur_range, GST_TIME_ARGS(cur_time));
    GST_DEBUG("prev: %lf, %" GST_TIME_FORMAT, prev_range, GST_TIME_ARGS(prev_time));
    ...
    GST_DEBUG("seek to %" GST_TIME_FORMAT " -- %" GST_TIME_FORMAT ", rate %lf" " on element %s", GST_TIME_ARGS(start), GST_TIME_ARGS(stop), rate, GST_ELEMENT_NAME(pipeline));

    s_event = gst_event_new_seek(rate, GST_FORMAT_TIME, (flush ? GST_SEEK_FLAG_FLUSH : 0) | (segment ? GST_SEEK_FLAG_SEGMENT : 0), GST_SEEK_TYPE_SET, start, GST_SEEK_TYPE_SET, stop);
...
static gboolean stop_seek(GtkWidget * widget, gpointer user_data)
{
...
static void print_message(GstMessage * message)
{
...
int main(int argc, char **argv)
{
    ...
    adjustment = GTK_ADJUSTMENT(gtk_adjustment_new(0.0, 0.0, (gdouble) RANGE_PREC, 0.1, 1.0, 1.0));
    hscale = gtk_hscale_new(adjustment);
    gtk_scale_set_digits(GTK_SCALE(hscale), 2);
    gtk_range_set_update_policy(GTK_RANGE(hscale), GTK_UPDATE_CONTINUOUS);

    sadjustment = GTK_ADJUSTMENT(gtk_adjustment_new(1.0, 0.0, 5.0, 0.1, 1.0, 0.0));
    ...
    schanged_id = g_signal_connect(shscale, "value_changed", G_CALLBACK(speed_cb), pipeline);
    ...
    bus_watch = gst_bus_add_watch_full(bus, G_PRIORITY_HIGH, bus_message, pipeline, NULL);
