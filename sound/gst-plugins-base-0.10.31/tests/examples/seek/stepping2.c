...
static void do_step(GstElement * bin)
{
    gdouble rate;
    ...
    gst_element_send_event(bin, gst_event_new_step(GST_FORMAT_TIME, 40 * GST_MSECOND, rate, FALSE, FALSE));
}

static void handle_sync_message(GstBus * bus, GstMessage * message, gpointer data)
{
    GstElement *bin;
...
static void handle_message(GstBus * bus, GstMessage * message, gpointer data)
{
    switch (message->type)
    {
