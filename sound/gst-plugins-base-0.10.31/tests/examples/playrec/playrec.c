...
static gboolean message_handler(GstBus * bus, GstMessage * message, gpointer user_data)
{
                    ...
                    switch (GST_MESSAGE_TYPE(orig))
                    {
                        case GST_MESSAGE_ASYNC_DONE:
                            ...
                            if (GST_MESSAGE_SRC(orig) == GST_OBJECT_CAST(play_bin))
                            {
                                 ...
                                 * prerolled */
                                g_assert(gst_element_set_state(pipeline, GST_STATE_PLAYING) == GST_STATE_CHANGE_ASYNC);
                            }
                            break;
                        default:
                            break;
...
int main(int argc, char *argv[])
{
    ...
    g_assert(gst_element_set_state(pipeline, GST_STATE_PAUSED) == GST_STATE_CHANGE_NO_PREROLL);

    g_print("waiting for playback preroll\n");
#ifndef ASYNC_VERSION
    /* sync wait for preroll on the playback bin and then go to PLAYING */
    g_assert(gst_element_get_state(play_bin, NULL, NULL, GST_CLOCK_TIME_NONE) == GST_STATE_CHANGE_SUCCESS);
    g_print("prerolled, starting synchronized playback and recording\n");
    /* returns ASYNC because the sink linked to the live source is not
     * prerolled */
    g_assert(gst_element_set_state(pipeline, GST_STATE_PLAYING) == GST_STATE_CHANGE_ASYNC);
