...
SendFile::SendFile():dirty(false), pbn(0), pblist(NULL), passwd(NULL), prn(MAX_SHAREDFILE), prlist(NULL)
{
...
void SendFile::InitSelf()
{
    ...
    filelist = gconf_client_get_list(client, GCONF_PATH "/shared_file_list", GCONF_VALUE_STRING, NULL);
    if (!(passwd = gconf_client_get_string(client, GCONF_PATH "/shared_passwd", NULL)))
        passwd = Strdup("");
    ...
    while (tmp)
    {
        if (Stat((const char *) tmp->data, &st) == -1 || (!S_ISREG(st.st_mode) && !S_ISDIR(st.st_mode)))
        {
        ...
        file = new FileInfo(PbnQuote(), (char *) tmp->data, st.st_size, S_ISREG(st.st_mode) ? IPMSG_FILE_REGULAR : IPMSG_FILE_DIR);
...
void SendFile::WriteShared()
{
    ...
    client = gconf_client_get_default();
    gconf_client_set_list(client, GCONF_PATH "/shared_file_list", GCONF_VALUE_STRING, filelist, NULL);
...
void SendFile::RequestData(int sock, uint32_t fileattr, char *buf)
{
    ...
    gtk_list_store_set(GTK_LIST_STORE(trans.TransModelQuote()), &iter,
                       0, pixbuf, 1, _("send"), 2, filename, 3, pal->NameQuote(), 4, "0B", 5, ptr, 6, "0B/s", 7, 0, 8, 0, 9, 0, 10, file->fileid, 11, file->filesize, 12, file->fileattr, 13, file->filename, 14, pal, -1);
    if (pixbuf)
...
void SendFile::SendFileInfo(GSList * list, gpointer data)
{
    ...
    while (list)
    {
        ...
        snprintf(ptr, MAX_UDPBUF - len, "%" PRIu32 ":%s:%" PRIx64 ":%" PRIx32 ":%" PRIx32 ":\a:", prn, filename, st.st_size, st.st_mtime, S_ISREG(st.st_mode) ? IPMSG_FILE_REGULAR : IPMSG_FILE_DIR);
        free(filename), len += strlen(ptr), ptr = buf + len;
        file = new FileInfo(prn, (char *) list->data, st.st_size, S_ISREG(st.st_mode) ? IPMSG_FILE_REGULAR : IPMSG_FILE_DIR);
...
void SendFile::SendSharedInfo(gpointer data)
{
    ...
    while (tmp)
    {
        ...
        snprintf(ptr, MAX_UDPBUF - len, "%" PRIu32 ":%s:%" PRIx64 ":%" PRIx32 ":%" PRIx32 ":\a:", file->fileid, filename, file->filesize, 0, file->fileattr);
        free(filename), len += strlen(ptr), ptr = buf + len;
        tmp = tmp->next;
    }
...
void SendFile::PickFile(uint32_t fileattr, gpointer data)
{
    ...
    pal = (Pal *) data;
    parent = pal->DialogQuote()? pal->DialogQuote()->DialogQuote() : inter.window;
    title = (GET_MODE(fileattr) == IPMSG_FILE_REGULAR) ? _("Choose sending files") : _("Choose sending folders");
    action = (GET_MODE(fileattr) == IPMSG_FILE_REGULAR) ? GTK_FILE_CHOOSER_ACTION_OPEN : GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER;

    dialog = gtk_file_chooser_dialog_new(title, GTK_WINDOW(parent), action, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT, NULL);
