...
void StatusIcon::CreateStatusIcon()
{
    ...
    pixbuf = gdk_pixbuf_new_from_file_at_size(__LOGO_PATH "/ksim.png", 20, 20, NULL);
    if (!pixbuf)
    {
        pop_error("\n%s \"" __LOGO_PATH "/ksim.png\" %s", _("The notify icon"), _("is lost!"));
        exit(1);
    }
...
void StatusIcon::SwitchWindowMode()
{
    ...
    if (GTK_WIDGET_VISIBLE(inter.window))
    {
        ...
        if (pixbuf)
        {
            ...
            pwarning(Fail, "%s \"" __LOGO_PATH "/ksim.png\" %s", _("The notify icon"), _("is lost!"));
    }
    else
    {
