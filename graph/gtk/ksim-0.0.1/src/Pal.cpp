...
Pal::Pal():ipv4(0), segment(NULL), version(NULL), packetn(0), user(NULL), host(NULL), name(NULL), group(NULL), ad(NULL), sign(NULL), iconfile(NULL), encode(NULL), flags(0), tpointer(NULL), iconpix(NULL), dialog(NULL), mypacketn(0), reply(true)
{
    extern Control ctr;
...
GdkPixbuf *Pal::GetIconPixbuf()
{
    ...
    if (iconpix)
    {                            //对象存在
        if (tpointer && strcmp(tpointer, iconfile) == 0)
        {                        //最新
    ...
    if (!tmp)
    {                            //不是系统图标
        iconpix = gdk_pixbuf_new_from_file_at_size(iconfile, MAX_ICONSIZE, MAX_ICONSIZE, NULL);
        if (iconpix)
            g_object_ref(iconpix);
        return iconpix;
    }
...
void Pal::ViewScroll()
{
    ...
    gtk_text_view_scroll_to_mark(GTK_TEXT_VIEW(dialog->ScrollQuote()), mark, 0.0, TRUE, 0.0, 0.0);
...
bool Pal::RecvIcon(const char *msg, size_t size)
{
    ...
    if (FLAG_ISSET(flags, 2) || (len = strlen(msg) + 1) >= size)
        return false;
    snprintf(file, MAX_PATHBUF, "%s" ICON_PATH "/%" PRIx32, g_get_user_cache_dir(), ipv4);
...
void Pal::RecvFile(const char *msg, size_t size)
{
    ...
    if ((!(ptr = iptux_skip_string(msg, size, 1)) || *ptr == '\0') && !(commandno & IPTUX_SHAREDOPT))
        return;
    ...
    while (tmp)
    {
        ...
        switch (((ChipData *) tmp->data)->type)
        {
            case STRING:
                ...
                gtk_text_buffer_insert_with_tags_by_name(record, &end, header, -1, "blue", NULL);
                g_free(header);
                if (!FLAG_ISSET(flags, 0))
            ...
            case PICTURE:
                ...
                if (gtk_text_iter_get_char(&start) == OCCUPY_OBJECT || gtk_text_iter_forward_find_char(&start, GtkTextCharPredicate(compare_foreach), GUINT_TO_POINTER(OCCUPY_OBJECT), NULL))
                {
                    end = start;
                ...
                {
                    header = getformattime("%s", name);
                    gtk_text_buffer_insert_with_tags_by_name(record, &start, header, -1, "blue", NULL);
...
void Pal::BufferInsertError(GSList * chiplist)
{
    ...
    while (tmp)
    {
        if (((ChipData *) tmp->data)->type == STRING)
            gtk_text_buffer_insert_with_tags_by_name(record, &iter, ((ChipData *) tmp->data)->data, -1, "red", NULL);
        tmp = tmp->next;
    }
