...
void RecvFile::CreateRecvWindow()
{
    ...
    g_signal_connect_swapped(window, "destroy", G_CALLBACK(g_object_unref), file_model);
    box = create_box();
    ...
    g_signal_connect(chooser, "current-folder-changed", G_CALLBACK(ChooserResetLabel), label);
    g_signal_connect(chooser, "current-folder-changed", G_CALLBACK(ChooserResetModel), selection);
    ...
    g_signal_connect_swapped(button, "clicked", G_CALLBACK(AdditionRecvFile), file_model);
...
gpointer RecvFile::DivideFileinfo(char **ptr)
{
    ...
    file = new FileInfo(iptux_get_dec_number(*ptr, 0), ipmsg_get_filename(*ptr, 1), iptux_get_hex64_number(*ptr, 2), iptux_get_hex_number(*ptr, 4));
...
GtkTreeModel *RecvFile::CreateRecvModel()
{
    ...
    model = gtk_list_store_new(11, G_TYPE_BOOLEAN, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_UINT, G_TYPE_UINT, G_TYPE_UINT64, G_TYPE_UINT, G_TYPE_POINTER);
    tmp = filelist;
    while (tmp)
    {
        ...
        gtk_list_store_set(model, &iter, 0, TRUE, 1, file->filename, 2, pal->NameQuote(), 3, ptr, 5, ctr.path, 6, packetn, 7, file->fileid, 8, file->filesize, 9, file->fileattr, 10, pal, -1);
        free(ptr);
...
GtkWidget *RecvFile::CreateRecvView()
{
    ...
    g_signal_connect_swapped(view, "button-press-event", G_CALLBACK(DialogGroup::PopupPickMenu), file_model);
    selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(view));
    ...
    g_signal_connect_swapped(renderer, "toggled", G_CALLBACK(DialogGroup::ViewToggleChange), file_model);
...
void RecvFile::CellEditText(GtkCellRendererText * renderer, gchar * path, gchar * new_text, GtkTreeModel * model)
{
    GtkTreeIter iter;
...
void RecvFile::AdditionRecvFile(GtkTreeModel * model)
{
    ...
    do
    {
        gtk_tree_model_get(model, &iter1, 0, &active, 1, &filename, 3, &filestr, 5, &path, 6, &packetn, 7, &fileid, 8, &filesize, 9, &fileattr, 10, &pal, -1);
        if (!active)
        {
        ...
        gtk_list_store_set(GTK_LIST_STORE(trans.TransModelQuote()),
                           &iter2, 0, pixbuf, 1, _("receive"), 2, filename, 3, pal->NameQuote(), 4, "0B", 5, filestr, 6, "0B/s", 7, 0, 8, 0, 9, packetn, 10, fileid, 11, filesize, 12, fileattr, 13, path, 14, pal, -1);
        g_free(filename), g_free(filestr), g_free(path);
