...
DialogGroup::DialogGroup():pal_view(NULL), record(NULL), input(NULL), accel(NULL), group_model(NULL)
{
}
...
GtkTreeModel *DialogGroup::CreateGroupModel()
{
    ...
    model = gtk_list_store_new(4, G_TYPE_BOOLEAN, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_POINTER);
...
void DialogGroup::InitGroupModel()
{
    ...
    while (tmp)
    {
        ...
        gtk_list_store_set(GTK_LIST_STORE(group_model), &iter, 0, TRUE, 1, pixbuf, 2, pal->NameQuote(), 3, pal, -1);
        if (pixbuf)
...
GtkWidget *DialogGroup::CreateGroupView()
{
    ...
    g_signal_connect_swapped(view, "button-press-event", G_CALLBACK(PopupPickMenu), group_model);
    g_signal_connect(view, "row-activated", G_CALLBACK(ViewItemActivated), group_model);
    ...
    g_signal_connect_swapped(renderer, "toggled", G_CALLBACK(ViewToggleChange), group_model);
...
gboolean DialogGroup::PopupPickMenu(GtkTreeModel * model, GdkEventButton * event)
{
    if (event->button != 3)
        return FALSE;
    gtk_menu_popup(GTK_MENU(CreatePopupMenu(model)), NULL, NULL, NULL, NULL, event->button, event->time);

    return TRUE;
}

void DialogGroup::ViewItemActivated(GtkWidget * view, GtkTreePath * path, GtkTreeViewColumn * column, GtkTreeModel * model)
{
    GtkTreeIter iter;
    gpointer data;
