...
path(NULL), sign(NULL), encode(NULL), palicon(NULL), font(NULL), flags(0), msgtip(NULL), transtip(NULL), volume(1.0), sndfgs(~0), netseg(NULL), dirty(false), table(NULL), iconlist(NULL), pix(3.4)
{
...
void Control::WriteControl()
{
    ...
    gconf_client_set_bool(client, GCONF_PATH "/min_memory_usage", FLAG_ISSET(flags, 5) ? TRUE : FALSE, NULL);
    gconf_client_set_bool(client, GCONF_PATH "/use_enter_key", FLAG_ISSET(flags, 4) ? TRUE : FALSE, NULL);
    gconf_client_set_bool(client, GCONF_PATH "/clearup_history", FLAG_ISSET(flags, 3) ? TRUE : FALSE, NULL);
    gconf_client_set_bool(client, GCONF_PATH "/record_log", FLAG_ISSET(flags, 2) ? TRUE : FALSE, NULL);
    gconf_client_set_bool(client, GCONF_PATH "/open_blacklist", FLAG_ISSET(flags, 1) ? TRUE : FALSE, NULL);
    gconf_client_set_bool(client, GCONF_PATH "/proof_shared", FLAG_ISSET(flags, 0) ? TRUE : FALSE, NULL);
    ...
    gconf_client_set_bool(client, GCONF_PATH "/msgsnd_support", FLAG_ISSET(sndfgs, 2) ? TRUE : FALSE, NULL);
    gconf_client_set_bool(client, GCONF_PATH "/transnd_support", FLAG_ISSET(sndfgs, 1) ? TRUE : FALSE, NULL);
    gconf_client_set_bool(client, GCONF_PATH "/sound_support", FLAG_ISSET(sndfgs, 0) ? TRUE : FALSE, NULL);
...
void Control::ReadControl()
{
    ...
    if (!(mygroup = gconf_client_get_string(client, GCONF_PATH "/belong_group", NULL)))
        mygroup = Strdup("");
...
void Control::CreateTagTable()
{
    ...
    g_object_set(tag, "indent", 10, "foreground", "#1005F0", "font", "Sans Italic 8", NULL);
    gtk_text_tag_table_add(table, tag);
}
...
void Control::UpdateNetSegment(GConfClient * client, bool direc)
{
    ...
    if (direc)
    {
        ...
        gconf_client_set_list(client, GCONF_PATH "/scan_net_segment", GCONF_VALUE_STRING, list, NULL);
    }
    else
    {
        tmp = list = gconf_client_get_list(client, GCONF_PATH "/scan_net_segment", GCONF_VALUE_STRING, NULL);
        while (tmp)
        {
