...
void DetectPal::CreateDetect()
{
    ...
    detect = gtk_dialog_new_with_buttons(_("Detect the pals"), GTK_WINDOW(inter.window), GTK_DIALOG_MODAL, _("Cancel"), GTK_RESPONSE_CANCEL, _("Detect"), GTK_RESPONSE_ACCEPT, NULL);
    ...
    ipstr = my_entry::create_entry(NULL, _("Please Input an IP address.(IPv4 only)!"), true);
