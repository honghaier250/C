...
void *Malloc(size_t size)
{
    ...
    dst = malloc(size);
    if (!dst)
        pwarning(Quit, _("act: allocate memory [%" PRIu32 "]," "warning: %s\n"), size, strerror(errno));
...
mark:subsock = accept(sockfd, addr, addr_len);
    if (subsock == -1)
    {
        if (errno == EINTR)
            goto mark;
        pwarning(Fail, _("act: TCP accepts a new connect (%d),warning: %s\n"), sockfd, strerror(errno));
    }
...
int Stat(const char *path, struct stat64 *buf)
{
    ...
    if (result == -1)
        pwarning(Fail, _("act: look up file's status '%s',warning: %s\n"), path, strerror(errno));
...
ssize_t Read(int fd, void *buf, size_t count)
{
    ...
    do
    {
        ...
        if (size == -1)
        {
            if (errno == EINTR)
                continue;
            pwarning(Fail, _("act: read data (%d)[%" PRIu32 "]," "warning: %s\n"), fd, count, strerror(errno));
            return -1;
        }
        len += size;
    }
...
ssize_t Write(int fd, const void *buf, size_t count)
{
    ...
    do
    {
        ...
        if (size == -1)
        {
            if (errno == EINTR)
                continue;
            pwarning(Fail, _("act: write data (%d)[%" PRIu32 "]," "warning: %s\n"), fd, count, strerror(errno));
            return -1;
        }
        len += size;
    }
...
ssize_t read_ipmsg_fileinfo(int fd, void *buf, size_t count, size_t offset)
{
    ...
    if (offset < count)
        ((char *) buf)[offset] = '\0';
    while (!offset || !strchr((char *) buf, ':') || sscanf((char *) buf, "%" SCNx32, &headsize) != 1 || headsize > offset)
    {
...
GtkWidget *create_window(const gchar * title, gdouble width, gdouble height)
{
    ...
    gtk_window_set_default_size(GTK_WINDOW(window), GINT(ctr.pix * width), GINT(ctr.pix * height));
