...
private:
    ...
    uint32_t RecvData(int sock, int fd, GtkTreeIter * iter, uint64_t filesize, char *buf, uint32_t offset);
    void SendFileData(int sock, GtkTreeIter * iter);
    void SendDirFiles(int sock, GtkTreeIter * iter);
    uint32_t SendData(int sock, int fd, GtkTreeIter * iter, uint64_t filesize, char *buf);
    void EndTransportData(int sock, int fd, GtkTreeIter * iter, const char *pathname);
    void EndTransportDirFiles(GtkTreeIter * iter, char *filename, uint64_t finishsize, uint64_t filesize);
