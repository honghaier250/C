...
void UdpData::UdpDataEntry(in_addr_t ipv4, char *msg, size_t size)
{
    ...
    if (FLAG_ISSET(ctr.flags, 1) && (pal = (Pal *) Ipv4GetPal(ipv4)) && FLAG_ISSET(pal->FlagsQuote(), 3))
        return;
...
void UdpData::SomeoneSendmsg(in_addr_t ipv4, char *msg, size_t size)
{
    ...
    if (commandno & IPMSG_SENDCHECKOPT)
        pal->SendReply(msg);
    if (flag && FLAG_ISSET(ctr.sndfgs, 1) && !(commandno & IPMSG_FILEATTACHOPT))
    ...
    if (flag && (commandno & IPMSG_FILEATTACHOPT))
    {
        if ((commandno & IPTUX_SHAREDOPT) && (commandno & IPTUX_PASSWDOPT))
        {
            ...
            if (passwd && *passwd != '\0')
            {
                ...
                cmd.SendAskShared(inter.udpsock, pal, IPTUX_PASSWDOPT, text);
                free(text);
            }
            free(passwd);
        }
...
void UdpData::SomeoneAskShared(in_addr_t ipv4, char *msg, size_t size)
{
    ...
    if (*sfl.PasswdQuote() == '\0')
        ...
        cmd.SendFileInfo(inter.udpsock, pal, IPTUX_SHAREDOPT | IPTUX_PASSWDOPT, "");
    else
    {
        if ((passwd = ipmsg_get_attach(msg, 5)) && (strcmp(passwd, sfl.PasswdQuote()) == 0))
            thread_create(ThreadFunc(ThreadAskShared), pal, false);
        free(passwd);
    }
}
