...
void TcpData::RecvSublayer(int sock, uint32_t command, char *buf, ssize_t size)
{
    ...
    {
        case IPTUX_ADPICOPT:
            snprintf(path, MAX_PATHBUF, "%s" ADS_PATH "/%" PRIx32, g_get_user_cache_dir(), pal->Ipv4Quote());
            break;
        case IPTUX_MSGPICOPT:
            snprintf(path, MAX_PATHBUF, "%s" PIC_PATH "/%" PRIx32 "-%" PRIx32 "-%" PRIx32, g_get_user_cache_dir(), pal->Ipv4Quote(), packetno, time(NULL));
            break;
        default:
            snprintf(path, MAX_PATHBUF, "%s" IPTUX_PATH "/%" PRIx32 "-%" PRIx32 "-%" PRIx32, g_get_user_cache_dir(), pal->Ipv4Quote(), packetno, time(NULL));
            break;
    }
