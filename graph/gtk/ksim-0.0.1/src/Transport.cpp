...
GtkTreeModel *Transport::CreateTransModel()
{
    ...
    model = gtk_list_store_new(15,
                               GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_INT, G_TYPE_INT, G_TYPE_UINT, G_TYPE_UINT, G_TYPE_UINT64, G_TYPE_UINT, G_TYPE_STRING, G_TYPE_POINTER);
...
void Transport::CreateTransView()
{
    ...
    g_signal_connect(trans_view, "button-press-event", G_CALLBACK(PopupControlMenu), this);
...
void Transport::RecvFileData(GtkTreeIter * iter)
{
    ...
    gdk_threads_enter();
    gtk_tree_model_get(trans_model, iter, 2, &filename, 9, &packetno, 10, &fileid, 11, &filesize, 13, &pathname, 14, &pal, -1);
    gdk_threads_leave();
...
void Transport::RecvDirFiles(GtkTreeIter * iter)
{
    ...
    gdk_threads_enter();
    gtk_tree_model_get(trans_model, iter, 2, &dirname, 9, &packetno, 10, &fileid, 11, &dirsize, 13, &pathname, 14, &pal, -1);
    gdk_threads_leave();
    ...
    while (1)
    {
        if ((size = read_ipmsg_fileinfo(sock, buf, MAX_SOCKBUF, len)) == -1)
            break;
        ...
        if (GET_MODE(fileattr) != IPMSG_FILE_RETPARENT)
        {
            ...
            gdk_threads_enter();
            gtk_list_store_set(GTK_LIST_STORE(trans_model), iter, 2, filename, 4, "0B", 5, ptr, 6, "0B/s", 7, 0, -1);
            gtk_tree_model_get(trans_model, iter, 8, &terminate, -1);
            gdk_threads_leave();
            free(ptr);
            if (terminate == -1)
        ...
        if (size == filesize)
        {
            ...
            gdk_threads_enter();
            gtk_list_store_set(GTK_LIST_STORE(trans_model), iter, 4, ptr, 7, 100, -1);
            gdk_threads_leave();
            free(ptr);
            continue;
        }
...
uint32_t Transport::RecvData(int sock, int fd, GtkTreeIter * iter, uint64_t filesize, char *buf, uint32_t offset)
{
    ...
    do
    {
        size = (MAX_SOCKBUF < filesize - downSize) ? MAX_SOCKBUF : filesize - downSize;
        ...
        if (difftime >= 1)
        {
            ...
            ptr2 = number_to_string_size((uint64_t) ((downSize - oldSize) / difftime), true);
            gdk_threads_enter();
            gtk_list_store_set(GTK_LIST_STORE(trans_model), iter, 4, ptr1, 6, ptr2, 7, GINT(percent(downSize, filesize)), -1);
            gtk_tree_model_get(trans_model, iter, 8, &terminate, -1);
            gdk_threads_leave();
    ...
    ptr1 = number_to_string_size(downSize);
    gdk_threads_enter();
    gtk_list_store_set(GTK_LIST_STORE(trans_model), iter, 4, ptr1, 7, GINT(percent(downSize, filesize)), -1);
    gdk_threads_leave();
    free(ptr1);
...
void Transport::SendFileData(int sock, GtkTreeIter * iter)
{
    ...
    gdk_threads_enter();
    gtk_tree_model_get(trans_model, iter, 2, &filename, 11, &filesize, 13, &pathname, -1);
    gdk_threads_leave();
...
void Transport::SendDirFiles(int sock, GtkTreeIter * iter)
{
    ...
    gdk_threads_enter();
    gtk_tree_model_get(trans_model, iter, 2, &dirname, 11, &dirsize, 13, &pathname, 14, &pal, -1);
    gdk_threads_leave();
    mf.chdir(pathname), g_free(pathname);
...
start:        if (mf.stat(dirt->d_name, &st) == -1 || (!S_ISREG(st.st_mode) && !S_ISDIR(st.st_mode)))
                continue;
            ptr = number_to_string_size(st.st_size);
            gdk_threads_enter();
            gtk_list_store_set(GTK_LIST_STORE(trans_model), iter, 2, dirt->d_name, 4, "0B", 5, ptr, 6, "0B/s", 7, 0, -1);
            gtk_tree_model_get(trans_model, iter, 8, &terminate, -1);
            gdk_threads_leave();
            free(ptr);
            if (terminate == -1)
            ...
            pathname = transfer_encode(dirt->d_name, pal->EncodeQuote(), true);
            filename = ipmsg_set_filename_pal(pathname);
            snprintf(buf, MAX_SOCKBUF, "000:%s:%" PRIx64 ":%" PRIx32 ":", filename, st.st_size, S_ISREG(st.st_mode) ? IPMSG_FILE_REGULAR : IPMSG_FILE_DIR);
            ...
            *(buf + strlen(buf)) = ':';
            ...
            {
                if ((fd = mf.open(dirt->d_name, O_RDONLY | O_LARGEFILE)) == -1)
                    goto end;
...
uint32_t Transport::SendData(int sock, int fd, GtkTreeIter * iter, uint64_t filesize, char *buf)
{
    ...
    do
    {
        ...
        if (difftime >= 1)
        {
            ...
            ptr2 = number_to_string_size((uint64_t) ((sendSize - oldSize) / difftime), true);
            gdk_threads_enter();
            gtk_list_store_set(GTK_LIST_STORE(trans_model), iter, 4, ptr1, 6, ptr2, 7, GINT(percent(sendSize, filesize)), -1);
            gtk_tree_model_get(trans_model, iter, 8, &terminate, -1);
            gdk_threads_leave();
            g_free(ptr1), g_free(ptr2);
            if (terminate == -1)
    ...
    ptr1 = number_to_string_size(sendSize);
    gdk_threads_enter();
    gtk_list_store_set(GTK_LIST_STORE(trans_model), iter, 4, ptr1, 7, GINT(percent(sendSize, filesize)), -1);
    gdk_threads_leave();
    free(ptr1);
...
void Transport::EndTransportData(int sock, int fd, GtkTreeIter * iter, const char *pathname)
{
    GdkPixbuf *pixbuf;
...
void Transport::EndTransportDirFiles(GtkTreeIter * iter, char *filename, uint64_t finishsize, uint64_t filesize)
{
    char *ptr1, *ptr2;
    ...
    gdk_threads_enter();
    gtk_list_store_set(GTK_LIST_STORE(trans_model), iter, 2, filename, 4, ptr1, 5, ptr2, -1);
...
gboolean Transport::PopupControlMenu(GtkWidget * view, GdkEventButton * event, gpointer data)
{
    Transport *trans;
    GtkTreePath *path;
    ...
    if (gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(view), GINT(event->x), GINT(event->y), &path, NULL, NULL, NULL))
    {
        ...
        trans->flag = false;
    gtk_menu_popup(GTK_MENU(trans->CreatePopupMenu()), NULL, NULL, NULL, NULL, event->button, event->time);
...
void Transport::StopTask(gpointer data)
{
    ...
    if (!trans->flag)
        return;
    gtk_list_store_set(GTK_LIST_STORE(trans->trans_model), &trans->opt_iter, 8, -1, -1);
}
...
mark:    gtk_tree_model_get(trans->trans_model, &iter, 8, &terminate, -1);
        if (terminate == 1)
        {
            result = gtk_list_store_remove(GTK_LIST_STORE(trans->trans_model), &iter);
            if (result)
                goto mark;
            break;
        }
