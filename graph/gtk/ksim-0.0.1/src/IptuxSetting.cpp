...
save_path(NULL), ad(NULL), sign(NULL), encode(NULL), palicon(NULL), font(NULL), memory(NULL), etrkey(NULL), tidy(NULL), log(NULL), black(NULL), proof(NULL), sound(NULL), volume(NULL), entry1(NULL), entry2(NULL), ipseg_view(NULL)
{
}
...
void IptuxSetting::CreatePerson(GtkWidget * note)
{
    ...
    myname = my_entry::create_entry(ctr.myname, _("Please Input Your Nickname!"), FALSE);
    ...
    mygroup = my_entry::create_entry(ctr.mygroup, _("Please Input your Group name!"), FALSE);
    ...
    gtk_text_buffer_set_text(gtk_text_view_get_buffer(GTK_TEXT_VIEW(sign)), ctr.sign ? ctr.sign : "", -1);
}
...
void IptuxSetting::CreateSystem(GtkWidget * note)
{
    ...
    encode = my_entry::create_entry(ctr.encode, _("(Before modified," "be SURE to know what you are doing)"), FALSE);
    ...
    memory = gtk_check_button_new_with_label(_("Minimise Memory Usage(NOT Recommended)"));
    ...
    tidy = gtk_check_button_new_with_label(_("Automaticlly Clean up Chatting History"));
...
void IptuxSetting::CreateIpseg(GtkWidget * note)
{
    ...
    g_signal_connect_swapped(button, "clicked", G_CALLBACK(gtk_list_store_clear), ip_model);
    gtk_box_pack_start(GTK_BOX(vbox), button, FALSE, FALSE, 0);
}
...
void IptuxSetting::CreateFuncButton(GtkWidget * hbb)
{
    ...
    g_signal_connect_swapped(button, "clicked", G_CALLBACK(gtk_widget_destroy), setting);
    gtk_box_pack_end(GTK_BOX(hbb), button, FALSE, FALSE, 1);
}
...
GtkTreeModel *IptuxSetting::CreateSndModel()
{
    ...
    gtk_list_store_set(model, &iter, 0, FLAG_ISSET(ctr.sndfgs, 1), 1, _("Message Received"), 2, ctr.msgtip, -1);
    gtk_list_store_append(model, &iter);
    gtk_list_store_set(model, &iter, 0, FLAG_ISSET(ctr.sndfgs, 2), 1, _("Transport Finished"), 2, ctr.transtip, -1);
...
GtkTreeModel *IptuxSetting::CreateIpModel()
{
    ...
    while (tmp)
    {
        ...
        gtk_list_store_append(model, &iter);
        gtk_list_store_set(model, &iter, 0, ns->startip, 1, ns->endip, 2, ns->describe, -1);
        tmp = tmp->next;
    }
...
GtkWidget *IptuxSetting::CreateSndChooser()
{
    ...
    chooser = gtk_file_chooser_button_new(_("Please choose a sound file"), GTK_FILE_CHOOSER_ACTION_OPEN);
...
GtkWidget *IptuxSetting::CreateSndView()
{
    ...
    g_signal_connect_swapped(view, "button-press-event", G_CALLBACK(DialogGroup::PopupPickMenu), snd_model);
    ...
    g_signal_connect_swapped(renderer, "toggled", G_CALLBACK(DialogGroup::ViewToggleChange), snd_model);
...
GtkWidget *IptuxSetting::CreateArchiveChooser()
{
    ...
    chooser = gtk_file_chooser_button_new(_("Save file to"), GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
...
GtkTreeModel *IptuxSetting::CreateIconModel()
{
    ...
    while (tmp)
    {
        si = (SysIcon *) tmp->data;
        if (!si->pixbuf)
            si->pixbuf = gdk_pixbuf_new_from_file_at_size(si->pathname, MAX_ICONSIZE, MAX_ICONSIZE, NULL);
        if (si->pixbuf)
        {
            gtk_list_store_append(model, &iter);
            gtk_list_store_set(model, &iter, 0, si->pixbuf, 1, si->pathname, -1);
        }
        tmp = tmp->next;
    }
    pthread_mutex_unlock(&ctr.mutex);
...
gint IptuxSetting::FileGetItemPos(const char *filename, GtkTreeModel * model)
{
    ...
    if (access(filename, F_OK) != 0 || !(pixbuf = gdk_pixbuf_new_from_file_at_size(filename, MAX_ICONSIZE, MAX_ICONSIZE, NULL)))
        return -1;
...
void IptuxSetting::ObtainPerson()
{
    ...
    {
        snprintf(buf, MAX_PATHBUF, "%s" COMPLEX_PATH "/icon", g_get_user_config_dir());
...
void IptuxSetting::ObtainIpseg()
{
    ...
    if (gtk_tree_model_get_iter_first(ip_model, &iter))
    {
        do
        {
            ...
            gtk_tree_model_get(ip_model, &iter, 0, &ns->startip, 1, &ns->endip, 2, &ns->describe, -1);
...
void IptuxSetting::UpdateNetSegment(const char *filename, GSList ** list, bool dirc)
{
    ...
    if (dirc)
    {
        ...
        {
            ns = (NetSegment *) tmp->data;
            fprintf(stream, "\n%s - %s\t//%s", ns->startip, ns->endip, ns->describe);
            tmp = tmp->next;
        }
    }
    else
    {
        ...
        while (getline(&lineptr, &n, stream) != -1)
        {
            if ((ptr = strchr(lineptr, '#')))
                *ptr = '\0';
            buf[2][0] = '\0';
            if (sscanf(lineptr, "%s - %s //%s", buf[0], buf[1], buf[2]) < 2 || inet_pton(AF_INET, buf[0], &ipv4) <= 0 || inet_pton(AF_INET, buf[1], &ipv4) <= 0)
                continue;
            ns = new NetSegment(Strdup(buf[0]), Strdup(buf[1]), Strdup(buf[2]));
...
void IptuxSetting::CellEditText(GtkCellRendererText * renderer, gchar * path, gchar * new_text, GtkTreeModel * model)
{
    GtkTreeIter iter;
...
void IptuxSetting::ImportNetSegment(gpointer data)
{
    ...
    dialog = gtk_file_chooser_dialog_new(_("Choose the import file"), GTK_WINDOW(setting), GTK_FILE_CHOOSER_ACTION_OPEN, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT, NULL);
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT)
    {
        ...
        while (tmp)
        {
            ns = (NetSegment *) tmp->data;
            gtk_list_store_append(GTK_LIST_STORE(ipst->ip_model), &iter);
            gtk_list_store_set(GTK_LIST_STORE(ipst->ip_model), &iter, 0, ns->startip, 1, ns->endip, 2, ns->describe, -1);
            tmp = tmp->next;
        }
...
void IptuxSetting::ExportNetSegment(gpointer data)
{
    ...
    ipst = (IptuxSetting *) data, list = NULL;
    dialog = gtk_file_chooser_dialog_new(_("Save the export file"), GTK_WINDOW(setting), GTK_FILE_CHOOSER_ACTION_SAVE, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_SAVE, GTK_RESPONSE_ACCEPT, NULL);
    gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_ACCEPT);
    gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(dialog), g_get_home_dir());

    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT && gtk_tree_model_get_iter_first(ipst->ip_model, &iter))
    {
        do
        {
            ns = new NetSegment(NULL, NULL, NULL);
            gtk_tree_model_get(ipst->ip_model, &iter, 0, &ns->startip, 1, &ns->endip, 2, &ns->describe, -1);
            ns->describe = ns->describe ? ns->describe : Strdup("");
            list = g_slist_append(list, ns);
        }
        while (gtk_tree_model_iter_next(ipst->ip_model, &iter));
