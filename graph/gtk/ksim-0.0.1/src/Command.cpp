...
void Command::SendMessage(int sock, pointer data, const char *msg)
{
    ...
    {
        chiplist = g_slist_append(NULL, new ChipData(STRING, _("It's offline, or not receive the data packet!")));
...
bool Command::SendAskData(int sock, pointer data, uint32_t packetno, uint32_t fileid, uint64_t offset)
{
    ...
    pal = (Pal *) data;
    snprintf(attrstr, 43, "%" PRIx32 ":%" PRIx32 ":%" PRIx64, packetno, fileid, offset);
...
void Command::SendSublayer(int sock, pointer data, uint32_t opttype, const char *path)
{
    ...
    if (Connect(sock, (SA *) & addr, sizeof(addr)) == -1 || Write(sock, buf, size) == -1 || (fd = Open(path, O_RDONLY)) == -1)
        return;
