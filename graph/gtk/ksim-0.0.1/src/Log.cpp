...
void Log::CommunicateLog(pointer data, const char *fmt, ...)
{
    ...
    {
        pal = (Pal *) data;
        ptr = getformattime(_("Nickname: %s\tUser: %s\tHost: %s"), pal->NameQuote(), pal->UserQuote(), pal->HostQuote());
    }
    else
...
void Log::SystemLog(const char *fmt, ...)
{
    ...
    if (!FLAG_ISSET(ctr.flags, 2))
        return;
    ptr = getformattime(_("Name: %s\tHost: %s"), g_get_real_name(), g_get_host_name());
    va_start(ap, fmt);
    msg = g_strdup_vprintf(fmt, ap);
    va_end(ap);
    fprintf(system, "%s\n%s%s\n%s\n\n", LOG_START_HEADER, ptr, msg, LOG_END_HEADER);
