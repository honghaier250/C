...
DialogPeer::DialogPeer(gpointer data):dialog(NULL), focus(NULL), scroll(NULL), accel(NULL), pal((Pal *) data)
{
    extern Control ctr;
...
void DialogPeer::CreateDialog()
{
    ...
    g_signal_connect_swapped(dialog, "drag-data-received", G_CALLBACK(DragDataReceived), pal);
...
void DialogPeer::CreateInfoArea(GtkWidget * Chat_Info_hpaned)
{
    ...
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(Info_scrolledwindow), GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
    gtk_container_add(GTK_CONTAINER(Info_alignment), Info_scrolledwindow);
    gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(Info_scrolledwindow), GTK_SHADOW_IN);
...
void DialogPeer::CreateInputArea(GtkWidget * paned)
{
    ...
    g_signal_connect(focus, "drag-data-received", G_CALLBACK(DragPicReceived), gtk_text_view_get_buffer(GTK_TEXT_VIEW(focus)));
    ...
    gtk_widget_add_accelerator(button, "clicked", accel, GDK_Return, FLAG_ISSET(ctr.flags, 4) ? GdkModifierType(0) : GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE);
}
...
void DialogPeer::CreateFileMenu(GtkWidget * menu_bar)
{
    ...
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(SendFile::SendRegular), pal);
    gtk_widget_show(menu_item);

    menu_item = gtk_menu_item_new_with_label(_("Send Folder"));
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(SendFile::SendFolder), pal);
    gtk_widget_show(menu_item);
    ...
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(gtk_widget_destroy), dialog);
    gtk_widget_show(menu_item);
}
...
void DialogPeer::CreateToolMenu(GtkWidget * menu_bar)
{
    ...
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(ClearRecordBuffer), pal->RecordQuote());
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);
    gtk_widget_show(menu_item);
}
...
void DialogPeer::FillPalInfoToBuffer(gpointer data, GtkTextBuffer * buffer, bool sad)
{
    ...
    gtk_text_buffer_insert(buffer, &iter, buf, -1);
    gtk_text_buffer_insert_with_tags_by_name(buffer, &iter, pal->SignQuote()? pal->SignQuote() : _("(lazy)"), -1, "sign", NULL);

    if (!sad || !pal->AdQuote() || !(pixbuf = gdk_pixbuf_new_from_file(pal->AdQuote(), NULL)))
        return;
...
void DialogPeer::DragDataReceived(gpointer data, GdkDragContext * context, gint x, gint y, GtkSelectionData * select, guint info, guint time)
{
    extern SendFile sfl;
    ...
    inet_ntop(AF_INET, &pal->Ipv4Quote(), ipstr, INET_ADDRSTRLEN);
    pop_info(pal->DialogQuote()? pal->DialogQuote()->dialog : inter.window, pal->DialogQuote()? pal->DialogQuote()->focus : NULL, _("File Sent to \n%s[%s] "), pal->NameQuote(), ipstr);
...
void DialogPeer::DragPicReceived(GtkWidget * view, GdkDragContext * context, gint x, gint y, GtkSelectionData * select, guint info, guint time, GtkTextBuffer * buffer)
{
    GdkPixbuf *pixbuf;
    GtkTextIter iter;
    GSList *list, *tmp;
...
void DialogPeer::InsertPixbuf(gpointer data)
{
    ...
    if (!(filename = my_chooser::choose_file(_("Please choose a picture to insert the buffer"), peer->dialog)))
        return;
...
void DialogPeer::SendMessage(gpointer data)
{
    ...
    do
    {
        if ((pixbuf = gtk_text_iter_get_pixbuf(&iter)))
        {
            ...
            snprintf(buf + strlen(buf), MAX_UDPBUF - strlen(buf), "%s%c", ptr, OCCUPY_OBJECT);
            free(ptr);
            piter = iter;        //移动 piter 到新位置
            ptr = g_strdup_printf("%s" IPTUX_PATH "/%" PRIx32, g_get_user_config_dir(), count++);
            gdk_pixbuf_save(pixbuf, ptr, "bmp", NULL, NULL);
            chiplist = g_slist_append(chiplist, new ChipData(PICTURE, ptr));
        }
    }
    while (gtk_text_iter_forward_find_char(&iter, GtkTextCharPredicate(compare_foreach), GUINT_TO_POINTER(ATOM_OBJECT), &end));
    ptr = gtk_text_buffer_get_text(buffer, &piter, &iter, FALSE);
