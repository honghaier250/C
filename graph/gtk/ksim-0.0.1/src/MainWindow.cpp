...
MainWindow::MainWindow():window(NULL), client_paned(NULL), tips(NULL), pal_tree(NULL), tree_model(NULL), accel(NULL)
{
}
...
void MainWindow::ButtonLoginCB(gpointer data)
{
    ...
    if(NULL != inter.window)
    {
        gtk_widget_destroy(inter.window);
        inter.window = NULL;
    }
...
void MainWindow::SetValueToModel(gpointer data, GtkTreeIter * iter)
{
    ...
    gtk_tree_store_set(GTK_TREE_STORE(tree_model), iter, 0, pixbuf, 1, NULL, 2, buf, 3, ctr.font, 4, TRUE, 6, FALSE, 7, data, 8, FALSE, -1);
    if (pixbuf)
...
void MainWindow::MakeItemBlinking(GtkTreeIter * iter, bool blink)
{
    ...
    g_free(color);
    gtk_tree_store_set(GTK_TREE_STORE(tree_model), iter, 1, (!color && blink) ? "#52B838" : NULL, -1);
}
...
void MainWindow::CreateLoginPanel()
{
    extern Control ctr;
    extern struct interactive inter;
    GtkWidget *login_fixed = NULL;

    inter.window = window = gtk_window_new(GTK_WINDOW_POPUP);
    login_fixed = CreateLoginPage();
    gtk_container_add(GTK_CONTAINER(window), login_fixed);
    gtk_window_set_default_size(GTK_WINDOW(window), GINT(ctr.pix * 60), GINT(ctr.pix * 140));

    //显示窗体和控件
    gtk_widget_show_all(window);
}

void MainWindow::CreateAppPanel()
{
    extern Control ctr;
    extern struct interactive inter;
    ...
    GdkWindowHints hints = GdkWindowHints(GDK_HINT_MIN_SIZE | GDK_HINT_MAX_SIZE | GDK_HINT_BASE_SIZE | GDK_HINT_RESIZE_INC | GDK_HINT_WIN_GRAVITY | GDK_HINT_USER_POS | GDK_HINT_USER_SIZE);
    GdkPixbuf *pixbuf;

    inter.window = window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gtk_window_set_title(GTK_WINDOW(window), _("ksim"));
    pixbuf = gdk_pixbuf_new_from_file_at_size(__LOGO_PATH "/ksim.png", 25, 25, NULL);
    if (pixbuf)
    {
        ...
        pwarning(Fail, "%s \"" __LOGO_PATH "/ksim.png\" %s", _("Icon file"), _("NOT Found!"));
    gtk_window_set_geometry_hints(GTK_WINDOW(window), window, &geometry, hints);
    gtk_window_set_default_size(GTK_WINDOW(window), GINT(ctr.pix * 70), GINT(ctr.pix * 150));
    accel = gtk_accel_group_new();
    gtk_window_add_accel_group(GTK_WINDOW(window), accel);

    g_signal_connect_swapped(window, "delete-event", G_CALLBACK(StatusIcon::SwitchWindowMode), NULL);
...
GtkWidget *MainWindow::CreateLoginPage()
{
    ...
    GtkWidget *quit    = gtk_image_new_from_file(__LOGO_PATH "/finish.png");
    GtkWidget *image   = gtk_image_new_from_file(__LOGO_PATH "/login.png");
    GtkWidget *button_login = gtk_button_new_with_label("               安全登录             ");
    ...
    //向列表里添加数据项
    login_methods = g_list_append(login_methods,(void *)"安全登录");
    login_methods = g_list_append(login_methods, (void *)"账户登录");
    login_accounts = g_list_append(login_accounts, (void *)"贺红杰");
    ...
    //设置按钮背景
    gtk_button_set_image(GTK_BUTTON(button_close), quit);
    ...
    gtk_widget_set_size_request(button_login, 210, 33);

    //添加窗体背景图片
    gtk_fixed_put(GTK_FIXED(login_fixed), image, 0, 0);
    gtk_fixed_put(GTK_FIXED(login_fixed), button_login, 10, 312);
    gtk_fixed_put(GTK_FIXED(login_fixed), button_close, 200, 3);
    gtk_fixed_put(GTK_FIXED(login_fixed), combo_methods, 70,208);
    gtk_fixed_put(GTK_FIXED(login_fixed), combo_accounts, 70,238);
    gtk_fixed_put(GTK_FIXED(login_fixed), check, 8,290);
    ...
    //退出按钮(button_close)单击事件
    g_signal_connect(G_OBJECT(button_close),"clicked",G_CALLBACK(iptux_gui_quit),NULL);
    //确定按钮(button_login)单击事件
    g_signal_connect(G_OBJECT(button_login),"clicked",G_CALLBACK(ButtonLoginCB),this);
...
GtkWidget *MainWindow::CreatePalTree()
{
    ...
    gtk_tree_view_column_set_attributes(GTK_TREE_VIEW_COLUMN(column), renderer, "pixbuf", 0, "cell-background", 1, NULL);
    renderer = gtk_cell_renderer_text_new();    //
    gtk_tree_view_column_pack_start(GTK_TREE_VIEW_COLUMN(column), renderer, TRUE);
    g_object_set(renderer, "xalign", 0.0, "wrap-mode", PANGO_WRAP_WORD, "foreground", "#52B838", NULL);
    gtk_tree_view_column_set_attributes(GTK_TREE_VIEW_COLUMN(column), renderer, "text", 2, "font", 3, "visible", 4, NULL);
    renderer = gtk_cell_renderer_text_new();    //
    gtk_tree_view_column_pack_start(GTK_TREE_VIEW_COLUMN(column), renderer, TRUE);
    g_object_set(renderer, "xalign", 0.0, "wrap-mode", PANGO_WRAP_WORD, "foreground", "#52B838", NULL);
    gtk_tree_view_column_set_attributes(GTK_TREE_VIEW_COLUMN(column), renderer, "markup", 5, "visible", 6, NULL);

    g_object_set(view, "has-tooltip", TRUE, NULL);
    g_signal_connect(view, "query-tooltip", G_CALLBACK(TreeQueryTooltip), tree_model);
    g_signal_connect(view, "row-activated", G_CALLBACK(TreeItemActivated), tree_model);
    g_signal_connect(view, "drag-data-received", G_CALLBACK(TreeDragDataReceived), tree_model);
    g_signal_connect(view, "button-release-event", G_CALLBACK(TreeChangeStatus), this);
...
GtkTreeModel *MainWindow::CreatePalTreeModel()
{
    ...
    model = gtk_tree_store_new(9, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_BOOLEAN, G_TYPE_STRING, G_TYPE_BOOLEAN, G_TYPE_POINTER, G_TYPE_BOOLEAN);
...
void MainWindow::InitPalTreeModel()
{
    ...
    while (localip[count << 1])
    {
        ...
        snprintf(buf, MAX_BUF, "<span style=\"italic\" underline=\"single\" size=\"small\" " "foreground=\"#52B838\" weight=\"bold\">%s</span>", ipstr);
        gtk_tree_store_append(GTK_TREE_STORE(tree_model), &iter, NULL);
        gtk_tree_store_set(GTK_TREE_STORE(tree_model), &iter, 0, pixbuf, 1, NULL, 4, FALSE, 5, buf, 6, TRUE, 7, NULL, 8, FALSE, -1);
        if (!localip[(count << 1) + 1])
            break;
        count++;
    }
...
void MainWindow::CreateToolMenu(GtkWidget * menu_bar)
{
    ...
    gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(menu_item), image);
    g_signal_connect(menu_item, "activate", G_CALLBACK(Transport::TransportEntry), NULL);
    gtk_widget_show(menu_item);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);
    ...
    gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(menu_item), image);
    g_signal_connect(menu_item, "activate", G_CALLBACK(IptuxSetting::SettingEntry), NULL);
    gtk_widget_show(menu_item);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);
    ...
    gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(menu_item), image);
    g_signal_connect(menu_item, "activate", G_CALLBACK(DialogGroup::DialogEntry), NULL);
    gtk_widget_show(menu_item);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);
...
GtkWidget *MainWindow::CreatePopupPalMenu(gpointer data)
{
    ...
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(DialogPeer::DialogEntry), data);
    gtk_widget_show(menu_item);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);

    menu_item = gtk_menu_item_new_with_label(_("Send File"));
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(SendFile::SendRegular), data);
    gtk_widget_show(menu_item);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);

    menu_item = gtk_menu_item_new_with_label(_("Send Folder"));
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(SendFile::SendFolder), data);
    gtk_widget_show(menu_item);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);

    menu_item = gtk_menu_item_new_with_label(_("Ask For Shared Files"));
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(DialogPeer::AskSharedFiles), data);
    gtk_widget_show(menu_item);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);

    menu_item = gtk_menu_item_new_with_label(_("Change Information"));
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(RevisePal::ReviseEntry), data);
    gtk_widget_show(menu_item);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);
...
GtkWidget *MainWindow::CreatePopupSectionMenu()
{
    ...
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(SortSectionByIP), this);
    gtk_widget_show(menu_item);
    gtk_menu_shell_append(GTK_MENU_SHELL(submenu), menu_item);

    menu_item = gtk_menu_item_new_with_label(_("By Nickname"));
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(SortSectionByNickname), this);
    gtk_widget_show(menu_item);
    gtk_menu_shell_append(GTK_MENU_SHELL(submenu), menu_item);

    menu_item = gtk_menu_item_new_with_label(_("By Group"));
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(SortSectionByGroup), this);
    gtk_widget_show(menu_item);
    gtk_menu_shell_append(GTK_MENU_SHELL(submenu), menu_item);

    menu_item = gtk_menu_item_new_with_label(_("By Communication"));
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(SortSectionByCommunication), this);
    gtk_widget_show(menu_item);
    gtk_menu_shell_append(GTK_MENU_SHELL(submenu), menu_item);
    ...
    g_signal_connect_swapped(menu_item, "activate", G_CALLBACK(TreeItemChangeStatus), this);
    gtk_widget_show(menu_item);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menu_item);
...
GtkTreeModel *MainWindow::CreatePalListModel()
{
    ...
    model = gtk_list_store_new(7, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_POINTER);
...
GtkWidget *MainWindow::CreatePalListView()
{
    ...
    g_signal_connect(view, "drag-data-received", G_CALLBACK(ListDragDataReceived), model);
...
gboolean MainWindow::TreeQueryTooltip(GtkWidget * view, gint x, gint y, gboolean key, GtkTooltip * tooltip, GtkTreeModel * model)
{
    extern Control ctr;
    ...
    if (key || !gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(view), x, y, &path, NULL, NULL, NULL))
        return FALSE;
    ...
    g_signal_connect_swapped(text_view, "destroy", G_CALLBACK(g_object_unref), buffer);

    return TRUE;
}

void MainWindow::TreeItemActivated(GtkWidget * view, GtkTreePath * path, GtkTreeViewColumn * column, GtkTreeModel * model)
{
    GtkTreeIter iter;
    gpointer data;
...
gboolean MainWindow::TreePopupMenu(GtkWidget * view, GdkEventButton * event, gpointer data)
{
    MainWindow *mw;
    GtkTreeModel *model;
    GtkTreePath *path;
    ...
    if (event->button != 3 || !gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(view), GINT(event->x), GINT(event->y), &path, NULL, NULL, NULL))
        return FALSE;
    ...
    if (pal)
        gtk_menu_popup(GTK_MENU(mw->CreatePopupPalMenu(pal)), NULL, NULL, NULL, NULL, event->button, event->time);
    else
        gtk_menu_popup(GTK_MENU(mw->CreatePopupSectionMenu()), NULL, NULL, NULL, NULL, event->button, event->time);

    return TRUE;
}

gboolean MainWindow::TreeChangeStatus(GtkWidget * view, GdkEventButton * event, gpointer data)
{
    MainWindow *mw;
    GtkTreePath *path;
    GtkTreeModel *model;

    if (event->button != 1 || !gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(view), GINT(event->x), GINT(event->y), &path, NULL, NULL, NULL))
        return FALSE;
...
void MainWindow::TreeItemChangeStatus(gpointer data)
{
    ...
    {
        pixbuf = gdk_pixbuf_new_from_file(__TIP_PATH "/hide.png", NULL);
        gtk_tree_store_set(GTK_TREE_STORE(model), &mw->opt_iter, 0, pixbuf, 8, FALSE, -1);
        gtk_tree_view_collapse_row(GTK_TREE_VIEW(mw->pal_tree), path);
    }
    else
    {
        pixbuf = gdk_pixbuf_new_from_file(__TIP_PATH "/show.png", NULL);
        gtk_tree_store_set(GTK_TREE_STORE(model), &mw->opt_iter, 0, pixbuf, 8, TRUE, -1);
...
void MainWindow::TreeDragDataReceived(GtkWidget * view, GdkDragContext * context, gint x, gint y, GtkSelectionData * select, guint info, guint time, GtkTreeModel * model)
{
    GtkTreePath *path;
    GtkTreeIter iter;
    gpointer data;
...
void MainWindow::AttachFindArea(gpointer data)
{
    ...
    g_signal_connect_swapped(button, "clicked", G_CALLBACK(g_object_unref), gtk_tree_view_get_model(GTK_TREE_VIEW(view)));
...
void MainWindow::FindEntryChanged(GtkWidget * entry, GtkWidget * view)
{
    ...
    while (tmp)
    {
        ...
        tmp = tmp->next;
        inet_ntop(AF_INET, &pal->Ipv4Quote(), ipstr, INET_ADDRSTRLEN);
        if (FLAG_ISSET(pal->FlagsQuote(), 1) && (*text == '\0' || strstr(pal->NameQuote(), text) || strstr(pal->GroupQuote(), text) || strstr(ipstr, text) || strstr(pal->UserQuote(), text) || strstr(pal->HostQuote(), text)))
        {
            pixbuf = pal->GetIconPixbuf();
            gtk_list_store_append(GTK_LIST_STORE(model), &iter);
            gtk_list_store_set(GTK_LIST_STORE(model), &iter, 0, pixbuf, 1, pal->NameQuote(), 2, pal->GroupQuote(), 3, ipstr, 4, pal->UserQuote(), 5, pal->HostQuote(), 6, pal, -1);
            if (pixbuf)
                g_object_unref(pixbuf);
        }
    }
...
void MainWindow::ListItemActivated(GtkWidget * view, GtkTreePath * path, GtkTreeViewColumn * column, GtkTreeModel * model)
{
    GtkTreeIter iter;
    gpointer data;
...
gboolean MainWindow::ListPopupMenu(GtkWidget * view, GdkEventButton * event, gpointer data)
{
    MainWindow *mw;
    GtkTreePath *path;
    GtkTreeModel *model;
...
void MainWindow::ListDragDataReceived(GtkWidget * view, GdkDragContext * context, gint x, gint y, GtkSelectionData * select, guint info, guint time, GtkTreeModel * model)
{
    GtkTreePath *path;
    GtkTreeIter iter;
    gpointer data;
