...
void pop_info(GtkWidget * parent, GtkWidget * fw, const gchar * format, ...)
{
    ...
    va_end(ap);
    dialog = gtk_message_dialog_new_with_markup(GTK_WINDOW(parent), GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_OK, "%s", msg);
    g_free(msg);
...
void pop_warning(GtkWidget * parent, GtkWidget * fw, const gchar * format, ...)
{
    ...
    msg = g_strdup_vprintf(format, ap);
    va_end(ap);
    dialog = gtk_message_dialog_new_with_markup(GTK_WINDOW(parent), GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING, GTK_BUTTONS_OK, "%s", msg);
    g_free(msg);
...
void pop_error(const gchar * format, ...)
{
    ...
    msg = g_strdup_vprintf(format, ap);
    va_end(ap);
    dialog = gtk_message_dialog_new_with_markup(NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, "%s", msg);
    g_free(msg);
