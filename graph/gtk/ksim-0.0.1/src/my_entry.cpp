...
GtkWidget *my_entry::create_entry(const char *text, const char *tip, bool digital)
{
    ...
    if (tip)
    {
        ...
        g_signal_connect(entry, "query-tooltip", G_CALLBACK(QueryTooltip), (gpointer) tip);
    }
    if (digital)
...
gboolean my_entry::QueryTooltip(GtkWidget * widget, gint x, gint y, gboolean key, GtkTooltip * tooltip, gpointer data)
{
    GtkWidget *label;
...
void my_entry::InsertText(GtkEditable * editable, gchar * text, gint length, gint * position, gpointer data)
{
    gint count;
