...
RevisePal::RevisePal(gpointer data):revise(NULL), icon_model(NULL), name(NULL), group(NULL), encode(NULL), icon(NULL), compatible(NULL), pal((Pal *) data)
{
}
...
void RevisePal::CreateRevise()
{
    ...
    revise = gtk_dialog_new_with_buttons(_("Change pal's information"), GTK_WINDOW(inter.window), GTK_DIALOG_MODAL, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_OK, GTK_RESPONSE_OK, NULL);
    ...
    name = my_entry::create_entry(pal->NameQuote(), _("Please input pal's new nickname!"), FALSE);
    ...
    group = my_entry::create_entry(pal->GroupQuote(), _("Please input pal's new group name!"), FALSE);
    ...
    encode = my_entry::create_entry(pal->EncodeQuote(), _("you must understand what you are doing!"), FALSE);
    ...
    g_signal_connect_swapped(button, "clicked", G_CALLBACK(IptuxSetting::AddPalIcon), icon);
    gtk_box_pack_start(GTK_BOX(box), button, FALSE, FALSE, 0);

    box = create_box(FALSE);
    gtk_box_pack_start(GTK_BOX(GTK_DIALOG(revise)->vbox), box, FALSE, FALSE, 10);
    compatible = gtk_check_button_new_with_label(_("Be compatible with iptux protocol"));
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(compatible), FLAG_ISSET(pal->FlagsQuote(), 0));
