...
void pixbuf_shrink_scale_1(GdkPixbuf ** pixbuf, int width, int height)
{
    ...
    {
        scale = ((scale_x = (gdouble) width / _width) < (scale_y = (gdouble) height / _height)) ? scale_x : scale_y;
...
void bind_iptux_port()
{
    ...
    if (bind(tcpsock, (SA *) & addr, sizeof(addr)) == -1 || bind(udpsock, (SA *) & addr, sizeof(addr)) == -1)
    {
...
GSList *get_sys_broadcast_addr(int sock)
{
    ...
    while (count < sum)
    {
        ...
        if (ioctl(sock, SIOCGIFFLAGS, ifr) == -1 || !(ifr->ifr_flags & IFF_BROADCAST) || ioctl(sock, SIOCGIFBRDADDR, ifr) == -1)
            continue;
...
GSList *get_sys_host_addr(int sock)
{
    ...
    while (count < sum)
    {
        ...
        if (strncasecmp(ifr->ifr_name, "lo", 2) == 0 || ioctl(sock, SIOCGIFFLAGS, ifr) == -1 || !(ifr->ifr_flags & IFF_UP) || ioctl(sock, SIOCGIFADDR, ifr) == -1)
            continue;
        addr = (SI *) & ifr->ifr_broadaddr;
        list = g_slist_append(list, GUINT_TO_POINTER(addr->sin_addr.s_addr));
    }
