...
void AboutIptux::CreateAbout()
{
    ...
    gtk_about_dialog_set_copyright(GTK_ABOUT_DIALOG(about), "Copyright © 2008-2009 by Jally");
    gtk_about_dialog_set_comments(GTK_ABOUT_DIALOG(about), _("A GTK+ based LAN Messenger."));
    gtk_about_dialog_set_url_hook(DialogOpenUrl, NULL, NULL);

    gtk_about_dialog_set_website(GTK_ABOUT_DIALOG(about), "http://code.google.com/p/iptux/");
    ...
    pixbuf = gdk_pixbuf_new_from_file(__LOGO_PATH "/ksim.png", NULL);
    if (pixbuf)
    {
...
void AboutIptux::CreateMore()
{
    ...
    more = gtk_dialog_new_with_buttons(_("More about iptux"), NULL, GTK_DIALOG_NO_SEPARATOR, GTK_STOCK_CLOSE, GTK_RESPONSE_CLOSE, NULL);
