...
bool pop_request_quit()
{
    ...
    dialog = gtk_message_dialog_new(GTK_WINDOW(inter.window), GTK_DIALOG_MODAL, GTK_MESSAGE_QUESTION, GTK_BUTTONS_OK_CANCEL, _("There's File Still in Transfering!\n" "\nAre you sure to SOPT and QUIT ?"));
...
bool pop_request_shared(gpointer data)
{
    ...
    gdk_threads_enter();
    dialog = gtk_dialog_new_with_buttons(_("Request for Shared Resources"), GTK_WINDOW(inter.window), GTK_DIALOG_MODAL, _("Refuse"), GTK_RESPONSE_CANCEL, _("Agree"), GTK_RESPONSE_ACCEPT, NULL);
...
char *pop_obtain_passwd()
{
    ...
    gdk_threads_enter();
    dialog = gtk_dialog_new_with_buttons(_("Access Password"), GTK_WINDOW(inter.window), GTK_DIALOG_MODAL, GTK_STOCK_OK, GTK_RESPONSE_OK, NULL);
...
char *pop_passwd_setting(GtkWidget * parent)
{
    ...
    dialog = gtk_dialog_new_with_buttons(_("Enter a New Password"), GTK_WINDOW(parent), GTK_DIALOG_MODAL, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_OK, GTK_RESPONSE_OK, NULL);
    gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_OK);
