...
void ShareFile::CreateShare()
{
    ...
    g_signal_connect_swapped(share, "drag-data-received", G_CALLBACK(DragDataReceived), this);
    ...
    g_signal_connect_swapped(button, "clicked", G_CALLBACK(gtk_widget_destroy), share);
    gtk_box_pack_end(GTK_BOX(bb), button, FALSE, FALSE, 0);
}
...
void ShareFile::AddSharedFiles(GSList * list)
{
    ...
    while (list)
    {
        ...
        FindInsertPosition((char *) list->data, S_ISREG(st.st_mode) ? IPMSG_FILE_REGULAR : IPMSG_FILE_DIR, &iter);
        gtk_list_store_set(GTK_LIST_STORE(share_model), &iter, 1, (char *) list->data, 2, ptr, 4, st.st_size, 5, S_ISREG(st.st_mode) ? IPMSG_FILE_REGULAR : IPMSG_FILE_DIR, -1);
        if (S_ISREG(st.st_mode))
            gtk_list_store_set(GTK_LIST_STORE(share_model), &iter, 0, pixbuf1, 3, _("regular"), -1);
        else
            gtk_list_store_set(GTK_LIST_STORE(share_model), &iter, 0, pixbuf2, 3, _("directory"), -1);
        free(ptr);
        list = list->next;
    }
    if (pixbuf1)
...
void ShareFile::FindInsertPosition(const gchar * path, uint32_t fileattr, GtkTreeIter * iter)
{
    GtkTreeIter sibling;
    gchar *tmp;
    guint attr;
    ...
    do
    {
        ...
        if ((GET_MODE(fileattr) == IPMSG_FILE_DIR && GET_MODE(attr) != IPMSG_FILE_DIR) || (GET_MODE(fileattr) == attr && strcmp(tmp, path) > 0))
        {
            g_free(tmp), sibling = *iter;
            gtk_list_store_insert_before(GTK_LIST_STORE(share_model), iter, &sibling);
            return;
        }
        g_free(tmp);
    }
...
GtkTreeModel *ShareFile::CreateSharedModel()
{
    ...
    model = gtk_list_store_new(6, GDK_TYPE_PIXBUF, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_UINT64, G_TYPE_UINT);
    pixbuf1 = obtain_pixbuf_from_stock(GTK_STOCK_FILE);
    pixbuf2 = obtain_pixbuf_from_stock(GTK_STOCK_DIRECTORY);
    ...
    while (tmp)
    {
        ...
        gtk_list_store_set(model, &iter, 1, file->filename, 2, ptr, 4, file->filesize, 5, file->fileattr, -1);
        if (GET_MODE(file->fileattr) == IPMSG_FILE_REGULAR)
            gtk_list_store_set(model, &iter, 0, pixbuf1, 3, _("regular"), -1);
        else
            gtk_list_store_set(model, &iter, 0, pixbuf2, 3, _("directory"), -1);
        free(ptr);
        tmp = tmp->next;
    }
...
void ShareFile::PickFile(uint32_t fileattr)
{
    ...
    title = (GET_MODE(fileattr) == IPMSG_FILE_REGULAR) ? _("Choose shared files") : _("Choose shared folders");
    action = (GET_MODE(fileattr) == IPMSG_FILE_REGULAR) ? GTK_FILE_CHOOSER_ACTION_OPEN : GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER;

    dialog = gtk_file_chooser_dialog_new(title, GTK_WINDOW(share), action, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT, NULL);
...
mark:    status = gtk_tree_selection_iter_is_selected(selection, &iter);
        if (status)
        {
            result = gtk_list_store_remove(GTK_LIST_STORE(sf->share_model), &iter);
            if (result)
                goto mark;
            break;
        }
...
void ShareFile::ClickApply(gpointer data)
{
    ...
    pthread_mutex_lock(sfl.MutexQuote());
    g_slist_foreach(sfl.PblistQuote(), GFunc(remove_foreach), GINT_TO_POINTER(FILEINFO));
    ...
    do
    {
        gtk_tree_model_get(sf->share_model, &iter, 1, &pathname, 4, &filesize, 5, &fileattr, -1);
...
void ShareFile::DragDataReceived(gpointer data, GdkDragContext * context, gint x, gint y, GtkSelectionData * select, guint info, guint time)
{
    ShareFile *sf;
    GSList *list;
