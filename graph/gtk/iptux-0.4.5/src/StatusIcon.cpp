...
void StatusIcon::CreateStatusIcon()
{
    ...
    pixbuf = gdk_pixbuf_new_from_file_at_size(__LOGO_PATH "/ip-tux.png", 20, 20, NULL);
    if (!pixbuf)
    {
        pop_error("\n%s \"" __LOGO_PATH "/ip-tux.png\" %s", _("The notify icon"), _("is lost!"));
        exit(1);
    }
...
void StatusIcon::SwitchWindowMode()
{
    ...
    if (GTK_WIDGET_VISIBLE(inter.window))
    {
        ...
        pixbuf = gdk_pixbuf_new_from_file_at_size(__LOGO_PATH "/i-tux.png", 20, 20, NULL);
        if (pixbuf)
        {
            ...
            pwarning(Fail, "%s \"" __LOGO_PATH "/i-tux.png\" %s", _("The notify icon"), _("is lost!"));
    }
    else
    {
        ...
        if (pixbuf)
        {
            ...
            pwarning(Fail, "%s \"" __LOGO_PATH "/ip-tux.png\" %s", _("The notify icon"), _("is lost!"));
    }
}
...
void StatusIcon::PopupWorkMenu(GtkStatusIcon * status_icon, guint button, guint activate_time, gpointer data)
{
    StatusIcon *stic;

    stic = (StatusIcon *) data;
    gtk_menu_popup(GTK_MENU(stic->CreatePopupMenu()), NULL, NULL, NULL, NULL, button, activate_time);
}
