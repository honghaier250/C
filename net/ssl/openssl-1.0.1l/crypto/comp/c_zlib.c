...
static int zz_uncompress (Bytef * dest, uLongf * destLen, const Bytef * source, uLong sourceLen);
...
static int zlib_stateful_init (COMP_CTX * ctx)
{
    ...
    struct zlib_state *state = (struct zlib_state *) OPENSSL_malloc (sizeof (struct zlib_state));
                                         ...
                                         unsigned int olen, unsigned char *in, unsigned int ilen)
{
    ...
    fprintf (stderr, "compress(%4d)->%4d %s\n",
             ilen, olen - state->ostream.avail_out, (ilen != olen - state->ostream.avail_out) ? "zlib" : "clear");
#endif
                                       ...
                                       unsigned int olen, unsigned char *in, unsigned int ilen)
{
    ...
    fprintf (stderr, "expand(%4d)->%4d %s\n",
             ilen, olen - state->istream.avail_out, (ilen != olen - state->istream.avail_out) ? "zlib" : "clear");
#endif
...
COMP_METHOD *COMP_zlib (void)
{
    ...
    if (!zlib_loaded)
    {
        ...
        if (zlib_dso != NULL)
        {
            ...
            if (p_compress && p_inflateEnd && p_inflate
                && p_inflateInit_ && p_deflateEnd && p_deflate && p_deflateInit_ && p_zError)
                zlib_loaded++;
        }
    }
...
#if defined(ZLIB) || defined(ZLIB_SHARED)
    {
        ...
        if (zlib_stateful_ex_idx == -1)
        {
            ...
            if (zlib_stateful_ex_idx == -1)
                zlib_stateful_ex_idx = CRYPTO_get_ex_new_index (CRYPTO_EX_INDEX_COMP, 0, NULL, NULL, NULL, NULL);
