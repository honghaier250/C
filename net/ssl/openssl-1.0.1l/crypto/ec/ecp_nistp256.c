...
static void point_add_small (smallfelem x3, smallfelem y3, smallfelem z3,
                             smallfelem x1, smallfelem y1, smallfelem z1, smallfelem x2, smallfelem y2, smallfelem z2)
{
                                                  ...
                                                  const EC_POINT * point, BIGNUM * x, BIGNUM * y, BN_CTX * ctx)
{
    ...
    {
        ECerr (EC_F_EC_GFP_NISTP256_POINT_GET_AFFINE_COORDINATES, EC_R_POINT_AT_INFINITY);
        return 0;
    }
                                ...
                                const BIGNUM * scalars[], BN_CTX * ctx)
{
    ...
    if (scalar != NULL)
    {
        pre = EC_EX_DATA_get_data (group->extra_data,
                                   nistp256_pre_comp_dup, nistp256_pre_comp_free, nistp256_pre_comp_clear_free);
        if (pre)
        ...
        if (!smallfelem_to_BN (x, g_pre_comp[0][1][0]) ||
            !smallfelem_to_BN (y, g_pre_comp[0][1][1]) || !smallfelem_to_BN (z, g_pre_comp[0][1][2]))
        {
        ...
        for (i = 0; i < num_points; ++i)
        {
            ...
            if ((p_scalar != NULL) && (p != NULL))
            {
                ...
                for (j = 2; j <= 16; ++j)
                {
                    if (j & 1)
                    {
                        point_add_small (pre_comp[i][j][0], pre_comp[i][j][1], pre_comp[i][j][2],
    ...
    if ((scalar != NULL) && (have_pre_comp))
    {
        ...
        batch_mul (x_out, y_out, z_out,
                   ...
                   g_secret, mixed, (const smallfelem (*)[17][3]) pre_comp, g_pre_comp);
    }
    else
...
int ec_GFp_nistp256_precompute_mult (EC_GROUP * group, BN_CTX * ctx)
{
    ...
    if ((!BN_to_felem (x_tmp, &group->generator->X)) ||
        (!BN_to_felem (y_tmp, &group->generator->Y)) || (!BN_to_felem (z_tmp, &group->generator->Z)))
        goto err;
    ...
    for (i = 1; i <= 8; i <<= 1)
    {
        ...
        for (j = 0; j < 31; ++j)
        {
            point_double_small (pre->g_pre_comp[0][2 * i][0], pre->g_pre_comp[0][2 * i][1],
...
int ec_GFp_nistp256_have_precompute_mult (const EC_GROUP * group)
{
    if (EC_EX_DATA_get_data (group->extra_data, nistp256_pre_comp_dup,
                             nistp256_pre_comp_free, nistp256_pre_comp_clear_free) != NULL)
        return 1;
    else
        return 0;
}
