...
static void felem_square (largefelem out, const felem in)
{
    ...
    out[6] = ((uint128_t) in[0]) * inx2[6] +
        ((uint128_t) in[1]) * inx2[5] + ((uint128_t) in[2]) * inx2[4] + ((uint128_t) in[3]) * in[3];
    out[7] = ((uint128_t) in[0]) * inx2[7] +
        ((uint128_t) in[1]) * inx2[6] + ((uint128_t) in[2]) * inx2[5] + ((uint128_t) in[3]) * inx2[4];
    out[8] = ((uint128_t) in[0]) * inx2[8] +
        ((uint128_t) in[1]) * inx2[7] +
        ((uint128_t) in[2]) * inx2[6] + ((uint128_t) in[3]) * inx2[5] + ((uint128_t) in[4]) * in[4];
    ...
    out[0] += ((uint128_t) in[1]) * inx4[8] +
        ((uint128_t) in[2]) * inx4[7] + ((uint128_t) in[3]) * inx4[6] + ((uint128_t) in[4]) * inx4[5];

    /* 10 */
    out[1] += ((uint128_t) in[2]) * inx4[8] +
        ((uint128_t) in[3]) * inx4[7] + ((uint128_t) in[4]) * inx4[6] + ((uint128_t) in[5]) * inx2[5];
...
static void felem_mul (largefelem out, const felem in1, const felem in2)
{
    ...
    out[3] = ((uint128_t) in1[0]) * in2[3] +
        ((uint128_t) in1[1]) * in2[2] + ((uint128_t) in1[2]) * in2[1] + ((uint128_t) in1[3]) * in2[0];

    out[4] = ((uint128_t) in1[0]) * in2[4] +
        ((uint128_t) in1[1]) * in2[3] +
        ((uint128_t) in1[2]) * in2[2] + ((uint128_t) in1[3]) * in2[1] + ((uint128_t) in1[4]) * in2[0];
    ...
    out[7] = ((uint128_t) in1[0]) * in2[7] +
        ...
        ((uint128_t) in1[5]) * in2[2] + ((uint128_t) in1[6]) * in2[1] + ((uint128_t) in1[7]) * in2[0];
    ...
    out[0] += ((uint128_t) in1[1]) * in2x2[8] +
        ...
        ((uint128_t) in1[6]) * in2x2[3] + ((uint128_t) in1[7]) * in2x2[2] + ((uint128_t) in1[8]) * in2x2[1];
    ...
    out[3] += ((uint128_t) in1[4]) * in2x2[8] +
        ...
        ((uint128_t) in1[6]) * in2x2[6] + ((uint128_t) in1[7]) * in2x2[5] + ((uint128_t) in1[8]) * in2x2[4];

    out[4] += ((uint128_t) in1[5]) * in2x2[8] +
        ((uint128_t) in1[6]) * in2x2[7] + ((uint128_t) in1[7]) * in2x2[6] + ((uint128_t) in1[8]) * in2x2[5];
                                     ...
                                     const BIGNUM * a, const BIGNUM * b, BN_CTX * ctx)
{
    ...
    if (((curve_p = BN_CTX_get (ctx)) == NULL) ||
        ((curve_a = BN_CTX_get (ctx)) == NULL) || ((curve_b = BN_CTX_get (ctx)) == NULL))
        goto err;
                                                  ...
                                                  const EC_POINT * point, BIGNUM * x, BIGNUM * y, BN_CTX * ctx)
{
    ...
    {
        ECerr (EC_F_EC_GFP_NISTP521_POINT_GET_AFFINE_COORDINATES, EC_R_POINT_AT_INFINITY);
        return 0;
    }
                                ...
                                const BIGNUM * scalars[], BN_CTX * ctx)
{
    ...
    if (((x = BN_CTX_get (ctx)) == NULL) ||
        ...
        ((z = BN_CTX_get (ctx)) == NULL) || ((tmp_scalar = BN_CTX_get (ctx)) == NULL))
        goto err;
    ...
    if (scalar != NULL)
    {
        ...
        if (!felem_to_BN (x, g_pre_comp[1][0]) ||
            !felem_to_BN (y, g_pre_comp[1][1]) || !felem_to_BN (z, g_pre_comp[1][2]))
        {
    ...
    if ((scalar != NULL) && (have_pre_comp))
    {
        ...
        batch_mul (x_out, y_out, z_out,
                   ...
                   g_secret, mixed, (const felem (*)[17][3]) pre_comp, (const felem (*)[3]) g_pre_comp);
    }
    else
...
int ec_GFp_nistp521_precompute_mult (EC_GROUP * group, BN_CTX * ctx)
{
    ...
    for (i = 1; i <= 4; i <<= 1)
    {
        point_double (pre->g_pre_comp[2 * i][0], pre->g_pre_comp[2 * i][1],
                      pre->g_pre_comp[2 * i][2], pre->g_pre_comp[i][0], pre->g_pre_comp[i][1], pre->g_pre_comp[i][2]);
        for (j = 0; j < 129; ++j)
        {
    ...
    point_add (pre->g_pre_comp[12][0], pre->g_pre_comp[12][1],
               ...
               pre->g_pre_comp[8][1], pre->g_pre_comp[8][2],
               0, pre->g_pre_comp[4][0], pre->g_pre_comp[4][1], pre->g_pre_comp[4][2]);
    ...
    for (i = 1; i < 8; ++i)
    {
        ...
        point_add (pre->g_pre_comp[2 * i + 1][0], pre->g_pre_comp[2 * i + 1][1],
                   ...
                   0, pre->g_pre_comp[1][0], pre->g_pre_comp[1][1], pre->g_pre_comp[1][2]);
    }
...
int ec_GFp_nistp521_have_precompute_mult (const EC_GROUP * group)
{
    if (EC_EX_DATA_get_data (group->extra_data, nistp521_pre_comp_dup,
                             nistp521_pre_comp_free, nistp521_pre_comp_clear_free) != NULL)
        return 1;
    else
        return 0;
}
