                                ...
                                const BIGNUM * scalars[], BN_CTX * ctx)
{
    ...
    if (scalar != NULL)
    {
        pre = EC_EX_DATA_get_data (group->extra_data,
                                   nistp224_pre_comp_dup, nistp224_pre_comp_free, nistp224_pre_comp_clear_free);
        if (pre)
        ...
        if (!felem_to_BN (x, g_pre_comp[0][1][0]) ||
            !felem_to_BN (y, g_pre_comp[0][1][1]) || !felem_to_BN (z, g_pre_comp[0][1][2]))
        {
    ...
    if ((scalar != NULL) && (have_pre_comp))
    {
        ...
        batch_mul (x_out, y_out, z_out,
                   ...
                   g_secret, mixed, (const felem (*)[17][3]) pre_comp, g_pre_comp);
    }
    else
...
int ec_GFp_nistp224_precompute_mult (EC_GROUP * group, BN_CTX * ctx)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        point_add (pre->g_pre_comp[i][6][0], pre->g_pre_comp[i][6][1],
                   ...
                   0, pre->g_pre_comp[i][2][0], pre->g_pre_comp[i][2][1], pre->g_pre_comp[i][2][2]);
        ...
        point_add (pre->g_pre_comp[i][12][0], pre->g_pre_comp[i][12][1],
                   ...
                   pre->g_pre_comp[i][8][1], pre->g_pre_comp[i][8][2],
                   0, pre->g_pre_comp[i][4][0], pre->g_pre_comp[i][4][1], pre->g_pre_comp[i][4][2]);
        ...
        for (j = 1; j < 8; ++j)
        {
            ...
            point_add (pre->g_pre_comp[i][2 * j + 1][0], pre->g_pre_comp[i][2 * j + 1][1],
                       ...
                       0, pre->g_pre_comp[i][1][0], pre->g_pre_comp[i][1][1], pre->g_pre_comp[i][1][2]);
        }
    }
...
int ec_GFp_nistp224_have_precompute_mult (const EC_GROUP * group)
{
    if (EC_EX_DATA_get_data (group->extra_data, nistp224_pre_comp_dup,
                             nistp224_pre_comp_free, nistp224_pre_comp_clear_free) != NULL)
        return 1;
    else
        return 0;
}
