...
{
    int status;
    unsigned char digest[MAX_DIGEST_LEN], digestInfo[DIGEST_INFO_LEN], originalDigestInfo[MAX_SIGNATURE_LEN];
    ...
    do
    {
        ...
        if ((originalDigestInfoLen != DIGEST_INFO_LEN) || (R_memcmp((POINTER) originalDigestInfo, (POINTER) digestInfo, DIGEST_INFO_LEN)))
        {
            status = RE_SIGNATURE;
            break;
        }
     ...
     R_RANDOM_STRUCT *randomStruct;    /* random structure */
{
    ...
    int status;
    unsigned char encryptedKeyBlock[MAX_ENCRYPTED_KEY_LEN], signature[MAX_SIGNATURE_LEN], *encryptedKeys[1];
     ...
     R_RSA_PUBLIC_KEY *publicKey;    /* signer's RSA public key */
{
    R_ENVELOPE_CTX context;
    int status;
    unsigned char encryptedKeyBlock[MAX_ENCRYPTED_KEY_LEN], signature[MAX_SIGNATURE_LEN];
    ...
    do
    {
        ...
        if (status = DecryptPEMUpdateFinal(&context, signature, &signatureLen, encryptedSignature, encryptedSignatureLen))
        {
            if ((status == RE_LEN || status == RE_ENCODING))
     ...
     unsigned char *digest;        /* message digest */
{
    ...
    digestInfo[DIGEST_INFO_A_LEN] = (digestAlgorithm == DA_MD2) ? (unsigned char) 2 : (unsigned char) 5;
