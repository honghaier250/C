     ...
     R_RANDOM_STRUCT *randomStruct;
{
    ...
    if (GetParams(&params, "  Set up with parameters 1 or 2? (blank to cancel): "))
        return;

    GetCommand(command, sizeof(command), "  Enter length in bytes of private value (blank to cancel): ");
    if (!*command)
        return;
    ...
    do
    {
        ...
        if (WriteBlock(publicValue, params->primeLen, "  Enter filename to save the public value (blank to cancel): "))
            break;

        if (WriteBlock(privateValue, privateValueLen, "  Enter filename to save the private value (blank to cancel): "))
            break;
    }
    while (0);
...
static void DoComputeAgreedKey()
{
    ...
    if (GetParams(&params, "  Compute with parameters 1 or 2? (blank to cancel): "))
        return;
    ...
    do
    {
        if (ReadBlock(otherPublicValue, &otherPublicValueLen, params->primeLen, "  Enter filename of other party's public value (blank to cancel): "))
            break;
        ...
        if (ReadBlock(privateValue, &privateValueLen, params->primeLen, "  Enter filename of private value (blank to cancel): "))
            break;
        ...
        if (WriteBlock(agreedKey, params->primeLen, "  Enter filename to save the agreed-upon key (blank to cancel): "))
            break;
    }
    while (0);
     ...
     R_RANDOM_STRUCT *randomStruct;
{
    ...
    GetCommand(command, sizeof(command), "  Enter prime size in bits, (16 to 1024) (blank to cancel): ");
    if (!*command)
        return;
    sscanf(command, "%d", &primeBits);

    GetCommand(command, sizeof(command), "  Enter subprime size in bits, (16 to 1024) (blank to cancel): ");
    if (!*command)
        return;
...
static void WriteParams2()
{
    ...
    while (1)
    {
        GetCommand(filename, sizeof(filename), "Enter filename to save the parameters (blank to not save): ");
        if (!*filename)
            return;
