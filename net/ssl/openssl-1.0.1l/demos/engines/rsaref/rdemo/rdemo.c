...
static void DoSignFile()
{
    ...
    do
    {
        ...
        if (WriteBlock(signature, signatureLen, "  Enter filename to save the signature"))
            break;
    }
    while (0);
     ...
     R_RANDOM_STRUCT *randomStruct;
{
    ...
    unsigned char encryptedKey[MAX_ENCRYPTED_KEY_LEN], *encryptedKeys[1], iv[8], partIn[24], partOut[31];
    ...
    do
    {
        ...
        if ((status = R_SealInit(&context, encryptedKeys, &encryptedKeyLen, iv, 1, &publicKey, encryptionAlgorithm, randomStruct)) != 0)
            break;
        ...
        if (WriteBlock(encryptedKey, encryptedKeyLen, "  Enter filename to save the encrypted key"))
            break;
        if (WriteBlock(iv, 8, "  Enter filename to save the initializing vector"))
            break;
    }
    while (0);
...
static void DoOpenFile()
{
    ...
    do
    {
        ...
        if ((status = R_OpenInit(&context, encryptionAlgorithm, encryptedKey, encryptedKeyLen, iv, privateKey)) != 0)
            break;
     ...
     R_RANDOM_STRUCT *randomStruct;
{
    ...
    GetCommand(command, sizeof(command), "  Enter key size in bits, (508 to 1024)");
    if (!*command)
        return;
...
static void WriteKeypair3()
{
    ...
    WriteBigInteger(file, PRIVATE_KEY3.publicExponent, sizeof(PRIVATE_KEY3.publicExponent));
    fprintf(file, "  exponent: ");
    WriteBigInteger(file, PRIVATE_KEY3.exponent, sizeof(PRIVATE_KEY3.exponent));
    fprintf(file, "  prime 1: ");
    WriteBigInteger(file, PRIVATE_KEY3.prime[0], sizeof(PRIVATE_KEY3.prime[0]));
    fprintf(file, "  prime 2: ");
    WriteBigInteger(file, PRIVATE_KEY3.prime[1], sizeof(PRIVATE_KEY3.prime[1]));
    fprintf(file, "  prime exponent 1: ");
    WriteBigInteger(file, PRIVATE_KEY3.primeExponent[0], sizeof(PRIVATE_KEY3.primeExponent[0]));
    fprintf(file, "  prime exponent 2: ");
    WriteBigInteger(file, PRIVATE_KEY3.primeExponent[1], sizeof(PRIVATE_KEY3.primeExponent[1]));
    fprintf(file, "  coefficient: ");
    WriteBigInteger(file, PRIVATE_KEY3.coefficient, sizeof(PRIVATE_KEY3.coefficient));
