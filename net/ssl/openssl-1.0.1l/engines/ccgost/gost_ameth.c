...
static ASN1_STRING *encode_gost_algor_params (const EVP_PKEY * key)
{
    ...
    switch (EVP_PKEY_base_id (key))
    {
        ...
        case NID_id_GostR3410_94:
            ...
            {
                GOSTerr (GOST_F_ENCODE_GOST_ALGOR_PARAMS, GOST_R_INVALID_GOST94_PARMSET);
                ASN1_STRING_free (params);
                params = NULL;
                goto err;
            }
...
static int decode_gost_algor_params (EVP_PKEY * pkey, X509_ALGOR * palg)
{
    ...
    {
        GOSTerr (GOST_F_DECODE_GOST_ALGOR_PARAMS, GOST_R_BAD_KEY_PARAMETERS_FORMAT);
        return 0;
    }
    p = pval->data;
    ...
    if (!gkp)
    {
        GOSTerr (GOST_F_DECODE_GOST_ALGOR_PARAMS, GOST_R_BAD_PKEY_PARAMETERS_FORMAT);
        return 0;
    }
    ...
    switch (op)
    {
        ...
        case ASN1_PKEY_CTRL_PKCS7_ENCRYPT:
            if (arg1 == 0)
            {
                ...
                X509_ALGOR_set0 (alg, OBJ_nid2obj (pkey->type), V_ASN1_SEQUENCE, params);
            }
            return 1;
#ifndef OPENSSL_NO_CMS
...
static int priv_encode_gost (PKCS8_PRIV_KEY_INFO * p8, const EVP_PKEY * pk)
{
    ...
    return PKCS8_pkey_set0 (p8, algobj, 0, V_ASN1_SEQUENCE, params, priv_buf, priv_len);
}

/* --------- printing keys --------------------------------*/
static int print_gost_94 (BIO * out, const EVP_PKEY * pkey, int indent, ASN1_PCTX * pctx, int type)
{
...
static int param_print_gost94 (BIO * out, const EVP_PKEY * pkey, int indent, ASN1_PCTX * pctx)
{
    return print_gost_94 (out, pkey, indent, pctx, 0);
}

static int pub_print_gost94 (BIO * out, const EVP_PKEY * pkey, int indent, ASN1_PCTX * pctx)
{
    return print_gost_94 (out, pkey, indent, pctx, 1);
}

static int priv_print_gost94 (BIO * out, const EVP_PKEY * pkey, int indent, ASN1_PCTX * pctx)
{
    return print_gost_94 (out, pkey, indent, pctx, 2);
}

static int print_gost_01 (BIO * out, const EVP_PKEY * pkey, int indent, ASN1_PCTX * pctx, int type)
{
    int param_nid = NID_undef;
...
static int param_print_gost01 (BIO * out, const EVP_PKEY * pkey, int indent, ASN1_PCTX * pctx)
{
    return print_gost_01 (out, pkey, indent, pctx, 0);
}

static int pub_print_gost01 (BIO * out, const EVP_PKEY * pkey, int indent, ASN1_PCTX * pctx)
{
    return print_gost_01 (out, pkey, indent, pctx, 1);
}

static int priv_print_gost01 (BIO * out, const EVP_PKEY * pkey, int indent, ASN1_PCTX * pctx)
{
...
#define COPYBIGNUM(a,b,x) if (a->x) BN_free(a->x); a->x=BN_dup(b->x);
    COPYBIGNUM (dto, dfrom, p) COPYBIGNUM (dto, dfrom, q) COPYBIGNUM (dto, dfrom, g) if (dto->priv_key)
...
static int pub_encode_gost01 (X509_PUBKEY * pub, const EVP_PKEY * pk)
{
    ...
    X = BN_new ();
    Y = BN_new ();
    EC_POINT_get_affine_coordinates_GFp (EC_KEY_get0_group (ec), pub_key, X, Y, NULL);
...
static int pub_cmp_gost94 (const EVP_PKEY * a, const EVP_PKEY * b)
{
    ...
    if (da && db && da->pub_key && db->pub_key && !BN_cmp (da->pub_key, db->pub_key))
    {
        return 1;
    }
    return 0;
    ...
    *ameth = EVP_PKEY_asn1_new (nid, ASN1_PKEY_SIGPARAM_NULL, pemstr, info);
    ...
    switch (nid)
    {
        case NID_id_GostR3410_94:
            ...
            EVP_PKEY_asn1_set_private (*ameth, priv_decode_gost, priv_encode_gost, priv_print_gost94);

            EVP_PKEY_asn1_set_param (*ameth,
                                     gost94_param_decode, gost94_param_encode,
                                     param_missing_gost94, param_copy_gost94, param_cmp_gost94, param_print_gost94);
            EVP_PKEY_asn1_set_public (*ameth,
                                      pub_decode_gost94, pub_encode_gost94,
                                      pub_cmp_gost94, pub_print_gost94, pkey_size_gost, pkey_bits_gost);

            EVP_PKEY_asn1_set_ctrl (*ameth, pkey_ctrl_gost);
            break;
        case NID_id_GostR3410_2001:
            EVP_PKEY_asn1_set_free (*ameth, pkey_free_gost01);
            EVP_PKEY_asn1_set_private (*ameth, priv_decode_gost, priv_encode_gost, priv_print_gost01);

            EVP_PKEY_asn1_set_param (*ameth,
                                     gost2001_param_decode, gost2001_param_encode,
                                     param_missing_gost01, param_copy_gost01, param_cmp_gost01, param_print_gost01);
            EVP_PKEY_asn1_set_public (*ameth,
                                      pub_decode_gost01, pub_encode_gost01,
                                      pub_cmp_gost01, pub_print_gost01, pkey_size_gost, pkey_bits_gost);
