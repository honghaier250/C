...
static char version[] __initdata = "pg3.c: v1.0 010812: Packet Generator for packet performance testing.\n";
...
struct sk_buff *fill_packet (struct net_device *odev, __u32 saddr)
{
    struct sk_buff *skb;
...
static void pg_inject (void)
{
    ...
    if (1)
    {
        ...
        p += sprintf (p, "OK: %u(c%u+d%u) usec, %u (%dbyte,%dfrags) %upps %uMB/sec",
                      ...
                      ((pc * 1000) / (total / 1000)), (((pc * 1000) / (total / 1000)) * pkt_size) / 1024 / 1024);
    }
...
int proc_pg_busy_read (char *buf, char **start, off_t offset, int len, int *eof, void *data)
{
    char *p;
...
int proc_pg_read (char *buf, char **start, off_t offset, int len, int *eof, void *data)
{
    char *p;
    ...
    p += sprintf (p, "Params: count=%u pkt_size=%u frags %d ipg %u odev \"%s\" dst %s dstmac ",
                  pg_count, pkt_size, nfrags, pg_ipg, pg_outdev, pg_dst);
    for (i = 0; i < 6; i++)
...
unsigned long num_arg (const char *buffer, unsigned long maxlen, unsigned long *num)
{
    int i = 0;
...
int proc_pg_write (struct file *file, const char *buffer, unsigned long count, void *data)
{
    int i = 0, max, len;
