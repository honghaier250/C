    ...
    switch (e->ee_errno)
    {
        ...
        case EHOSTUNREACH:
            if ((e->ee_origin == SO_EE_ORIGIN_ICMP &&
                 e->ee_type == 11 &&
                 e->ee_code == 0) || (e->ee_origin == SO_EE_ORIGIN_ICMP6 && e->ee_type == 3 && e->ee_code == 0))
            {
                if (rethops >= 0)
                {
                    if (rethops <= 64)
...
int main (int argc, char **argv)
{
    ...
    if (!sin.sin6_addr.s6_addr32[0] && !sin.sin6_addr.s6_addr32[1] && sin.sin6_addr.s6_addr32[2] == htonl (0xFFFF))
    {
        mtu = 65535;
        overhead = 28;
        mapped = 1;
    ...
    if (setsockopt (fd, SOL_IPV6, IPV6_MTU_DISCOVER, &on, sizeof (on)) &&
        (on = IPV6_PMTUDISC_DO, setsockopt (fd, SOL_IPV6, IPV6_MTU_DISCOVER, &on, sizeof (on))))
    {
