...
char usage[] = "Usage:    rdisc [-b] [-d] [-s] [-v] [-f] [-a] [-V] [send_address] [receive_address]\n"
#ifdef RDISC_SERVER
...
/*
fork一个子进程，然后结束服父进程，关闭除了socket文件的所有的文件
*/
    int t;
            ...
            switch (*av[0])
            {
                ...
                case 'T':
                    ...
                    if (argc != 0)
                    {
                        ...
                        if (val < 4 || val > 1800)
                        {
                            (void) fprintf (stderr, "Bad Max Advertizement Interval\n");
                            exit (1);
                        }
                        max_adv_int = val;
                        min_adv_int = (max_adv_int * 3 / 4);
...
*/
    ...
    for (;;)
    {
        ...
        cc = recvfrom (s, (char *) packet, len, 0, (struct sockaddr *) &from, &fromlen);
        if (cc < 0)
        {
            if (errno == EINTR)
                continue;
...
*/
    ...
    if (responder && left_until_advertise <= 0)
    {
        ...
        if (ntransmitted < initial_advertisements)
            ...
            left_until_advertise = min_adv_int + ((max_adv_int - min_adv_int) * (random () % 1000) / 1000);
    }
    else
#endif
...
/*
每TIMER_INTERVAL秒(3秒)发生一次中断，进入timer()函数
*/
}
 ...
 */
void solicitor (struct sockaddr_in *sin)
{
...
*/
    ...
    if (isbroadcast (sin))
        ...
        i = sendto (s, (char *) outpack, packetlen, 0, (struct sockaddr *) sin, sizeof (struct sockaddr));
 ...
 * The IP packet will be added on by the kernel.
 */
void advertise (struct sockaddr_in *sin, int lft)
{
...
广播
*/
            cc = sendbcastif (s, (char *) outpack, packetlen, &interfaces[i]);
        else if (ismulticast (sin))

/*
多播
*/
            cc = sendmcastif (s, (char *) outpack, packetlen, sin, &interfaces[i]);
        else
        {
...
*/
             ...
             * address.
             */
            if ((sin->sin_addr.s_addr & ifp->netmask.s_addr) == (ifp->address.s_addr & ifp->netmask.s_addr))
            {
                if (debug)
                {
                    logdebug ("Unicast to %s ", pr_name (sin->sin_addr));
                    logdebug ("on interface %s, %s\n", ifp->name, pr_name (ifp->address));
                }
                cc = sendto (s, (char *) outpack, packetlen, 0, (struct sockaddr *) sin, sizeof (struct sockaddr));
...
/*
hlen是IP报文的长度
ihl是IP首部里的总长度字段，该字段的单位是4字节
*/
    if (cc < hlen + 8)
    {
        if (verbose)
            logtrace ("packet too short (%d bytes) from %s\n", cc, pr_name (from->sin_addr));
        return;
    }
    cc -= hlen;

/*
cc原来是IP报文的长度，需要减去hlen，即IP报文的长度。之后cc就是ICMP报文的长度
*/
...
*/
                    if (verbose)
                        logtrace ("ICMP %s from %s: Bad checksum\n",
                                  pr_type ((int) rap->icmp_type), pr_name (from->sin_addr));
                    return;
                }
                if (rap->icmp_code != 0)
                {
...
*/
                    if (verbose)
                        logtrace ("ICMP %s from %s: Code = %d\n",
                                  pr_type ((int) rap->icmp_type), pr_name (from->sin_addr), rap->icmp_code);
                    return;
                }
...
/*
ICMP路由通告报文地址项长度不应该小于2个字（word），即不小于8字节。一般就是8字节。
*/
                    if (verbose)
                        logtrace ("ICMP %s from %s: Words/addr = %d\n",
                                  pr_type ((int) rap->icmp_type), pr_name (from->sin_addr), rap->icmp_wpa);
                    return;
                }
                if ((unsigned) cc < 8 + rap->icmp_num_addrs * rap->icmp_wpa * 4)
                {

/*
cc现在是ICMP报文的长度
8字节是ICMP报文首部的长度
icmp_num_addrs是报文中蕴含的地址数
icmp_wpa是每个地址的字（word）数，每个字有4个字节
*/
                    if (verbose)
                        logtrace ("ICMP %s from %s: Too short %d, %d\n",
                                  pr_type ((int) rap->icmp_type),
                                  pr_name (from->sin_addr), cc, 8 + rap->icmp_num_addrs * rap->icmp_wpa * 4);
                    return;
                }

                if (verbose)
                    logtrace ("ICMP %s from %s, lifetime %d\n",
                              pr_type ((int) rap->icmp_type), pr_name (from->sin_addr), ntohs (rap->icmp_lifetime));
...
*/
                    ...
                    ap = (struct icmp_ra_addr *) ALLIGN (buf + hlen + 8 + i * rap->icmp_wpa * 4);
...
*/
                    ...
                    if (verbose)
                        logtrace ("\taddress %s, preference 0x%x\n",
                                  pr_name (ina), (unsigned int) ntohl (ap->ira_preference));
                    if (is_directly_connected (ina))

/*
如果是直接连接的
*/
                        record_router (ina, ntohl (ap->ira_preference), ntohs (rap->icmp_lifetime));
...
*/
                    if (verbose)
                        logtrace ("ICMP %s from %s: Bad checksum\n",
                                  pr_type ((int) icp->type), pr_name (from->sin_addr));
                    return;
                }
                if (icp->code != 0)
                {
...
*/
                    if (verbose)
                        logtrace ("ICMP %s from %s: Code = %d\n",
                                  pr_type ((int) icp->type), pr_name (from->sin_addr), icp->code);
                    return;
                }
...
*/
                    if (verbose)
                        logtrace ("ICMP %s from %s: Too short %d, %d\n",
                                  pr_type ((int) icp->type), pr_name (from->sin_addr), cc, ICMP_MINLEN);
                    return;
                }
...
int sendbcast (int s, char *packet, int packetlen)
{
...
int sendbcastif (int s, char *packet, int packetlen, struct interface *ifp)
{
    ...
    cc = sendto (s, packet, packetlen, 0, (struct sockaddr *) &baddr, sizeof (struct sockaddr));
    if (cc != packetlen)
    {
...
int sendmcast (int s, char *packet, int packetlen, struct sockaddr_in *sin)
{
...
int sendmcastif (int s, char *packet, int packetlen, struct sockaddr_in *sin, struct interface *ifp)
{
    int cc;
    ...
    if (setsockopt (s, IPPROTO_IP, IP_MULTICAST_IF, (char *) &mreq, sizeof (mreq)) < 0)
    {
        logperror ("setsockopt (IP_MULTICAST_IF)");
        logerr ("Cannot send multicast packet over interface %s, %s\n", ifp->name, pr_name (mreq.imr_address));
        return (-1);
    }
    cc = sendto (s, packet, packetlen, 0, (struct sockaddr *) sin, sizeof (struct sockaddr));
    if (cc != packetlen)
    {
...
void initifs ()
{
    ...
    if (interfaces)
        interfaces = (struct interface *) ALLIGN (realloc ((char *) interfaces, numifs * sizeof (struct interface)));

/*
已经有内存重新分配内存
*/
    else
        interfaces = (struct interface *) ALLIGN (malloc (numifs * sizeof (struct interface)));
...
#ifdef notdef
        if (debug)
            logdebug ("Found interface %s, flags 0x%x\n", pr_name (interfaces[i].localaddr), interfaces[i].flags);
#endif
        i++;
    }
    num_interfaces = i;
...
/*
所有符合条件的借口都被存储在interfaces[]中，num_interfaces为这些接口的个数
*/
#ifdef notdef
    if (debug)
...
/*
是不是直接连接，也就是对方主机和本地主机的某个网络接口是不是在同一个以太网内
*/
    int i;
...
/*
对方主机和子网掩码位与，和接口IP地址和子网掩码位与相等，则说明接口在同一个以太网内
*/
    }
    return (0);
}
...
/*
INELIGIBLE_PREF:0x80000000
优先级为0x80000000则对应地址不能作为默认路由器地址使用
*/
    tp = table;
    while (tp)
    {
    ...
    while (*tpp != NULL)
    {
        ...
        if (tp->remaining_time <= 0)
        {
            *tpp = tp->next;
            if (tp->in_kernel)
                del_route (tp->router);
            if (best_preference && tp->preference == max)
...
void record_router (struct in_addr router, int preference, int ttl)
{
...
/*
INELIGIBLE_PREF：绝对值最大的负数
*/
    if (debug)
        logdebug ("Recording %s, ttl %d, preference 0x%x\n", pr_name (router), ttl, preference);
...
找到对应路由项
*/
        if (tp->preference > preference && tp->preference == old_max)
            changed_down++;
...
/*
如果内核中可以包含多个路由项目或者路由项是最大优先级的路由项
*/
...
/*
如果内核中只使用优先级最高的路由项
且路由的优先级被降低了，则应该在路由记录中选择那些和最高优先级相等的路由加如内核使用路由中
因为降低优先级的路由项目可能正好是原来优先级最高的路由项
*/
...
/*
如果只使用优先级最高的路由，则一旦发生原有路由的优先级升高或者降低
则应该从内核的原路由中删除那些比最高优先级低的路由
*/
