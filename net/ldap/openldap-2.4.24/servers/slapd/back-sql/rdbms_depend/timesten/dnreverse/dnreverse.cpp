...
LDAPEntriesHandler::LDAPEntriesHandler(TTXlaConnection & conn, const char *ownerP, const char *nameP):TTXlaTableHandler(conn, ownerP, nameP)
{
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        getNullDNs.Prepare(&conn2, "select dn, id from ldap_entries " "where dn_ru is null " "for update", "", stat);
        conn2.Commit(stat);
    }
    catch(TTStatus stat)
    {
