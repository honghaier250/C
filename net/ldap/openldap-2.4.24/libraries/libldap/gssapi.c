...
static char *gsserrstr(char *buf, ber_len_t buf_len, gss_OID mech, int gss_rc, OM_uint32 minor_status)
{
    OM_uint32 min2;
...
#    endif
    ...
    snprintf(buf, buf_len, "gss_rc[%d:%*s] mech[%*s] minor[%u:%*s]",
             ...
             (const char *) (gss_msg.value ? gss_msg.value : ""), (int) mech_msg.length, (const char *) (mech_msg.value ? mech_msg.value : ""), minor_status, (int) minor_msg.length, (const char *) (minor_msg.value ? minor_msg.value : ""));
...
static void sb_sasl_gssapi_init(struct sb_sasl_generic_data *p, ber_len_t * min_send, ber_len_t * max_send, ber_len_t * max_recv)
{
...
#    define FALLBACK_SEND_MAX_SIZE    0x009FFFB8
    /* from MIT 1.5.x */

    gss_rc = gss_wrap_size_limit(&minor_status, gss_ctx, conf_req_flag, GSS_C_QOP_DEFAULT, SEND_MAX_WIRE_SIZE, &max_input_size);
    if (gss_rc != GSS_S_COMPLETE)
    {
        ...
        ber_log_printf(LDAP_DEBUG_ANY, p->sbiod->sbiod_sb->sb_debug, "sb_sasl_gssapi_init: fallback to default wrap size limit\n");
        /*
...
static ber_int_t sb_sasl_gssapi_encode(struct sb_sasl_generic_data *p, unsigned char *buf, ber_len_t len, Sockbuf_Buf * dst)
{
    gss_ctx_id_t gss_ctx = (gss_ctx_id_t) p->ops_private;
    int gss_rc;
    OM_uint32 minor_status;
    ...
    {
        ber_log_printf(LDAP_DEBUG_ANY, p->sbiod->sbiod_sb->sb_debug, "sb_sasl_gssapi_encode: GSS_C_CONF_FLAG was ignored by our gss_wrap()\n");
        return -1;
    }
    ...
    {
        ber_log_printf(LDAP_DEBUG_ANY, p->sbiod->sbiod_sb->sb_debug, "sb_sasl_gssapi_encode: failed to grow the buffer to %lu bytes\n", pkt_len);
        return -1;
    }
...
static ber_int_t sb_sasl_gssapi_decode(struct sb_sasl_generic_data *p, const Sockbuf_Buf * src, Sockbuf_Buf * dst)
{
    ...
    if (conf_req_flag && conf_state == 0)
    {
        ber_log_printf(LDAP_DEBUG_ANY, p->sbiod->sbiod_sb->sb_debug, "sb_sasl_gssapi_encode: GSS_C_CONF_FLAG was ignored by our peer\n");
        return -1;
    }
    ...
    {
        ber_log_printf(LDAP_DEBUG_ANY, p->sbiod->sbiod_sb->sb_debug, "sb_sasl_gssapi_decode: failed to grow the buffer to %lu bytes\n", unwrapped.length);
        return -1;
    }
...
static void sb_sasl_gssapi_reset_buf(struct sb_sasl_generic_data *p, Sockbuf_Buf * buf)
{
...
static int guess_service_principal(LDAP * ld, const char *ldapServiceName, const char *dnsHostName, gss_name_t * principal)
{
...
#    define    __SPNEGO_OID "\053\006\001\005\005\002"
    ...
    gss_rc = gss_init_sec_context(&minor_status, GSS_C_NO_CREDENTIAL, &gss_ctx, principal, req_mech, req_flags, 0, NULL, &input_token, &ret_mech, &output_token, &ret_flags, NULL);
    if (gss_rc == GSS_S_COMPLETE)
    {
