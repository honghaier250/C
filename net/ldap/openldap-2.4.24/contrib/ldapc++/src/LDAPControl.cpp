...
LDAPCtrl::LDAPCtrl(const char *oid, bool critical, const char *data, int length)
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPCtrl::LDAPCtrl()" << endl);
    DEBUG(LDAP_DEBUG_CONSTRUCT | LDAP_DEBUG_PARAMETER, "   oid:" << oid << endl << "   critical:" << critical << endl);
    m_oid = oid;
...
string LDAPCtrl::getOID() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPCtrl::getOID()" << endl);
    return m_oid;
}

bool LDAPCtrl::isCritical() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPCtrl::isCritical()" << endl);
    return m_isCritical;
}

bool LDAPCtrl::hasData() const const
{
    return !m_noData;
}

string LDAPCtrl::getData() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPCtrl::getData()" << endl);
    return m_data;
}

LDAPControl *LDAPCtrl::getControlStruct() const const
{
