...
LDAPAddRequest::LDAPAddRequest(const LDAPAddRequest & req):LDAPRequest(req)
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPAddRequest::LDAPAddRequest(&)" << endl);
    m_entry = new LDAPEntry(*(req.m_entry));
}

LDAPAddRequest::LDAPAddRequest(const LDAPEntry * entry, LDAPAsynConnection * connect, const LDAPConstraints * cons, bool isReferral, const LDAPRequest * parent):LDAPRequest(connect, cons, isReferral, parent)
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPAddRequest::LDAPAddRequest()" << endl);
    DEBUG(LDAP_DEBUG_CONSTRUCT | LDAP_DEBUG_PARAMETER, "   entry:" << entry << endl << "   isReferral:" << isReferral << endl);
