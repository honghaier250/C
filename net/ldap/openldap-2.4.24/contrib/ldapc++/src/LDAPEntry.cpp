...
void LDAPEntry::setAttributes(LDAPAttributeList * attrs)
{
    ...
    DEBUG(LDAP_DEBUG_TRACE | LDAP_DEBUG_PARAMETER, "   attrs:" << *attrs << endl);
    if (m_attrs != 0)
    {
        delete m_attrs;
    }
...
const string & LDAPEntry::getDN() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPEntry::getDN()" << endl);
    return m_dn;
}

const LDAPAttributeList *LDAPEntry::getAttributes() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPEntry::getAttributes()" << endl);
    return m_attrs;
}

const LDAPAttribute *LDAPEntry::getAttributeByName(const std::string & name) const const
{
