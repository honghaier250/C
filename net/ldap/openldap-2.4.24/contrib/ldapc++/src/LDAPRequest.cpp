...
LDAPRequest::LDAPRequest(LDAPAsynConnection * con, const LDAPConstraints * cons, bool isReferral, const LDAPRequest * parent)
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPRequest::LDAPRequest()" << endl);
    m_connection = con;
    if (cons == 0)
...
LDAPMsg *LDAPRequest::getNextMessage() const const
{
    DEBUG(LDAP_DEBUG_DESTROY, "LDAPRequest::getNextMessage()" << endl);
    int res;
    LDAPMessage *msg;

    res = ldap_result(this->m_connection->getSessionHandle(), this->m_msgID, 0, 0, &msg);
...
LDAPRequest *LDAPRequest::followReferral(LDAPMsg * /*urls */ )
{
    ...
    DEBUG(LDAP_DEBUG_TRACE, "ReferralChasing not implemented for this operation" << endl);
    return 0;
}

const LDAPConstraints *LDAPRequest::getConstraints() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPRequest::getConstraints()" << endl);
    return m_cons;
}

const LDAPAsynConnection *LDAPRequest::getConnection() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPRequest::getConnection()" << endl);
    return m_connection;
}

int LDAPRequest::getType() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPRequest::getType()" << endl);
    return m_requestType;
}

int LDAPRequest::getMsgID() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPRequest::getMsgId()" << endl);
    return m_msgID;
}

int LDAPRequest::getHopCount() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPRequest::getHopCount()" << endl);
    return m_hopCount;
}

const LDAPRequest *LDAPRequest::getParent() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPRequest::getParent()" << endl);
    return m_parent;
}

bool LDAPRequest::isReferral() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPRequest::isReferral()" << endl);
    return m_isReferral;
}

bool LDAPRequest::equals(const LDAPRequest * req) const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPRequest::equals()" << endl);
    if ((this->m_requestType == req->getType()) && (this->m_connection->getHost() == req->m_connection->getHost()) && (this->m_connection->getPort() == req->m_connection->getPort()))
    {
        return true;
    }
    return false;
...
bool LDAPRequest::isCycle() const const
{
...
void LDAPRequest::unbind() const const
{
