...
LdifReader::LdifReader(std::istream & input):m_ldifstream(input), m_lineNumber(0)
{
...
int LdifReader::readNextRecord(bool first)
{
        ...
        if (numLine == 0)
        {
            if (type == "version")
            {
                ...
                if (this->m_version != 1)    // there is no other Version than LDIFv1 
                {
                    ...
                    err << "Line " << this->m_lineNumber << ": Unsuported LDIF Version";
            ...
            else if (type == "include")    // ... or it might be an "include" line
            {
                ...
                {
                    std::ostringstream err;
                    err << "Line " << this->m_lineNumber << ": \"include\" not allowed in LDIF version 1.";
                    throw(std::runtime_error(err.str()));
                }
                else
                {
                    std::ostringstream err;
                    err << "Line " << this->m_lineNumber << ": \"include\" not yet suppported.";
                    throw(std::runtime_error(err.str()));
                }
            }
            else
            {
                DEBUG(LDAP_DEBUG_TRACE, " Record doesn't start with a DN" << std::endl);
                std::ostringstream err;
                err << "Line " << this->m_lineNumber << ": LDIF record does not start with a DN.";
                throw(std::runtime_error(err.str()));
            }
        }
        if (numLine == 1)        // might contain "changtype" to indicate a change request
        {
            if (type == "changetype")
            {
                ...
                else if (!this->m_ldifTypeRequest)
                {
                    ...
                    std::ostringstream err;
                    err << "Line " << this->m_lineNumber << ": Change Request in an entry-only LDIF.";
                    throw(std::runtime_error(err.str()));
                }
                if (value == "modify")
                {
                ...
                else
                {
                    ...
                    std::ostringstream err;
                    err << "Line " << this->m_lineNumber << ": Unknown changetype: \"" << value << "\".";
                    throw(std::runtime_error(err.str()));
                }
            }
            else
    ...
    DEBUG(LDAP_DEBUG_TRACE, "<- LdifReader::readRecord() return: " << recordType << std::endl);
...
LDAPEntry LdifReader::getEntryRecord()
{
    ...
    {
        throw(std::runtime_error("The LDIF record: '" + i->second + "' is not a valid LDAP Entry"));
    }
...
int LdifReader::getLdifLine(std::string & ldifline)
{
    ...
    while (m_ldifstream && (m_ldifstream.peek() == ' ' || m_ldifstream.peek() == '\t'))
    {
        std::string cat;
        m_ldifstream.ignore();
...
void LdifReader::splitLine(const std::string & line, std::string & type, std::string & value) const const
{
    std::string::size_type pos = line.find(':');
    if (pos == std::string::npos)
    {
        DEBUG(LDAP_DEBUG_ANY, "Invalid LDIF line. No `:` separator" << std::endl);
        std::ostringstream err;
    ...
    if (delim == ':')
    {
        ...
        {
            value = "";
            DEBUG(LDAP_DEBUG_TRACE, " not enough space in output buffer" << std::endl);
            std::ostringstream err;
            err << "Line " << this->m_lineNumber << ": Can't decode Base64 data. Buffer too small";
            throw(std::runtime_error(err.str()));
        }
    }
    else if (delim == '<')
    {
        ...
        std::ostringstream err;
        err << "Line " << this->m_lineNumber << ": URLs are currently not supported";
        throw(std::runtime_error(err.str()));
    }
    else
    {
...
std::string LdifReader::readIncludeLine(const std::string & line) const const
{
    ...
    if (scheme != "file:")
    {
        DEBUG(LDAP_DEBUG_TRACE, "unsupported scheme: " << scheme << std::endl);
    }
    else if (line[pos] == '/')
    {
        if (line[pos + 1] == '/')
