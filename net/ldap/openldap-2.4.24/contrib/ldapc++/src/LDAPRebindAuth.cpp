...
LDAPRebindAuth::LDAPRebindAuth(const string & dn, const string & pwd)
{
    ...
    DEBUG(LDAP_DEBUG_CONSTRUCT | LDAP_DEBUG_PARAMETER, "   dn:" << dn << endl << "   pwd:" << pwd << endl);
    m_dn = dn;
    m_password = pwd;
}
...
const string & LDAPRebindAuth::getDN() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPRebindAuth::getDN()" << endl);
    return m_dn;
}

const string & LDAPRebindAuth::getPassword() const const
{
