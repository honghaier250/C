...
LDAPSearchReference::LDAPSearchReference(const LDAPRequest * req, LDAPMessage * msg):LDAPMsg(msg)
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPSearchReference::LDAPSearchReference()" << endl;
    )char **ref = 0;
...
{
    DEBUG(LDAP_DEBUG_DESTROY, "LDAPSearchReference::~LDAPSearchReference()" << endl);
}

const LDAPUrlList & LDAPSearchReference::getUrls() const const
{
