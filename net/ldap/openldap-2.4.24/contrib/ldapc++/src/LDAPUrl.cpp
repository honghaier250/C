...
LDAPUrl::LDAPUrl(const std::string & url)
{
    ...
    DEBUG(LDAP_DEBUG_CONSTRUCT | LDAP_DEBUG_PARAMETER, "   url:" << url << endl);
    m_urlString = url;
    m_Filter = "";
    m_Scheme = "ldap";
    m_Scope = 0;
...
int LDAPUrl::getPort() const const
{
    return m_Port;
}
...
int LDAPUrl::getScope() const const
{
    return m_Scope;
}
...
void LDAPUrl::setScope(const std::string & scope)
{
    ...
    {
        throw LDAPUrlException(LDAPUrlException::INVALID_SCOPE, "Scope was:" + scope);
    }
    regenerate = true;
}

const string & LDAPUrl::getURLString() const const
{
    if (regenerate)
    {
        this->components2Url();
...
const string & LDAPUrl::getHost() const const
{
    return m_Host;
}
...
const string & LDAPUrl::getDN() const const
{
    return m_DN;
}
...
const string & LDAPUrl::getFilter() const const
{
    return m_Filter;
}
...
const StringList & LDAPUrl::getAttrs() const const
{
    return m_Attrs;
}
...
const StringList & LDAPUrl::getExtensions() const const
{
    return m_Extensions;
}
...
const std::string & LDAPUrl::getScheme() const const
{
    return m_Scheme;
}
...
{
    if (scheme == "ldap" || scheme == "ldaps" || scheme == "ldapi" || scheme == "cldap")
    {
        m_Scheme = scheme;
        regenerate = true;
    }
    ...
    {
        throw LDAPUrlException(LDAPUrlException::INVALID_SCHEME, "Unknown URL scheme: \"" + scheme + "\"");
    }
}
...
void LDAPUrl::parseUrl()
{
    ...
    {
        throw LDAPUrlException(LDAPUrlException::INVALID_URL, "No colon found in URL");
    }
    ...
    else
    {
        ...
        {
            std::string port = m_urlString.substr(portstart + 1, (pos == std::string::npos ? pos : pos - portstart - 1));
            if (port.length() > 0)
            {
            ...
            DEBUG(LDAP_DEBUG_TRACE, "    Port: <" << m_Port << ">" << std::endl);
        }
        startpos = pos + 1;
    }
    int parserMode = base;
    ...
    {
        pos = m_urlString.find('?', startpos);
        std::string actComponent = m_urlString.substr(startpos, pos - startpos);
        DEBUG(LDAP_DEBUG_TRACE, "    ParserMode:" << parserMode << std::endl);
        DEBUG(LDAP_DEBUG_TRACE, "    ActComponent: <" << actComponent << ">" << std::endl);
        std::string s_scope = "";
        std::string s_ext = "";
        switch (parserMode)
        {
            ...
            case scope:
                ...
                DEBUG(LDAP_DEBUG_TRACE, "    Scope: <" << s_scope << ">" << std::endl);
                break;
            case filter:
                percentDecode(actComponent, m_Filter);
                DEBUG(LDAP_DEBUG_TRACE, "    filter: <" << m_Filter << ">" << std::endl);
                break;
            case extensions:
...
void LDAPUrl::percentDecode(const std::string & src, std::string & out)
{
    ...
    while (pos != std::string::npos)
    {
        ...
        if (i.fail())
        {
            throw LDAPUrlException(LDAPUrlException::URL_DECODING_ERROR, "Invalid percent encoding");
        }
        char j = hex;
        out.push_back(j);
        startpos = pos + 3;
...
void LDAPUrl::string2list(const std::string & src, StringList & sl, bool percentDecode)
{
...
void LDAPUrl::components2Url() const const
{
    ...
    if (!m_Attrs.empty())
    {
        url << "?";
        bool first = true;
        for (StringList::const_iterator i = m_Attrs.begin(); i != m_Attrs.end(); i++)
        {
...
std::string & LDAPUrl::percentEncode(const std::string & src, std::string & dest, int flags) const const
{
    std::ostringstream o;
...
LDAPUrlException::LDAPUrlException(int code, const std::string & msg):m_code(code), m_addMsg(msg)
{
}

int LDAPUrlException::getCode() const const
{
    return m_code;
}

const std::string LDAPUrlException::getAdditionalInfo() const const
{
    return m_addMsg;
}

const std::string LDAPUrlException::getErrorMessage() const const
{
