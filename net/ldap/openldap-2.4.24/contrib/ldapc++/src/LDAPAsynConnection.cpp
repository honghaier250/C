...
LDAPAsynConnection::LDAPAsynConnection(const string & url, int port, LDAPConstraints * cons)
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPAsynConnection::LDAPAsynConnection()" << endl);
    DEBUG(LDAP_DEBUG_CONSTRUCT | LDAP_DEBUG_PARAMETER, "   URL:" << url << endl << "   port:" << port << endl);
    cur_session = 0;
    m_constr = 0;
    // Is this an LDAP URI?
...
void LDAPAsynConnection::init(const string & hostname, int port)
{
    ...
    DEBUG(LDAP_DEBUG_TRACE | LDAP_DEBUG_PARAMETER, "   hostname:" << hostname << endl << "   port:" << port << endl);
...
LDAPMessageQueue *LDAPAsynConnection::bind(const string & dn, const string & passwd, const LDAPConstraints * cons)
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAsynConnection::bind()" << endl);
    DEBUG(LDAP_DEBUG_TRACE | LDAP_DEBUG_PARAMETER, "   dn:" << dn << endl << "   passwd:" << passwd << endl);
...
LDAPMessageQueue *LDAPAsynConnection::saslBind(const std::string & mech, const std::string & cred, const LDAPConstraints * cons)
{
...
LDAPMessageQueue *LDAPAsynConnection::saslInteractiveBind(const std::string & mech, int flags, SaslInteractionHandler * sih, const LDAPConstraints * cons)
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAsynConnection::saslInteractiveBind" << std::endl);
    LDAPSaslInteractiveBind *req = new LDAPSaslInteractiveBind(mech, flags, sih, this, cons);
    try
    {
        LDAPMessageQueue *ret = req->sendRequest();
        return ret;
...
LDAPMessageQueue *LDAPAsynConnection::search(const string & base, int scope, const string & filter, const StringList & attrs, bool attrsOnly, const LDAPConstraints * cons)
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAsynConnection::search()" << endl);
    DEBUG(LDAP_DEBUG_TRACE | LDAP_DEBUG_PARAMETER, "   base:" << base << endl << "   scope:" << scope << endl << "   filter:" << filter << endl);
...
LDAPMessageQueue *LDAPAsynConnection::del(const string & dn, const LDAPConstraints * cons)
{
...
LDAPMessageQueue *LDAPAsynConnection::compare(const string & dn, const LDAPAttribute & attr, const LDAPConstraints * cons)
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAsynConnection::compare()" << endl);
    DEBUG(LDAP_DEBUG_TRACE | LDAP_DEBUG_PARAMETER, "   dn:" << dn << endl << "   attr:" << attr << endl);
...
LDAPMessageQueue *LDAPAsynConnection::add(const LDAPEntry * le, const LDAPConstraints * cons)
{
...
LDAPMessageQueue *LDAPAsynConnection::modify(const string & dn, const LDAPModList * mod, const LDAPConstraints * cons)
{
...
LDAPMessageQueue *LDAPAsynConnection::rename(const string & dn, const string & newRDN, bool delOldRDN, const string & newParentDN, const LDAPConstraints * cons)
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAsynConnection::rename()" << endl);
    DEBUG(LDAP_DEBUG_TRACE | LDAP_DEBUG_PARAMETER, "   dn:" << dn << endl << "   newRDN:" << newRDN << endl << "   newParentDN:" << newParentDN << endl << "   delOldRDN:" << delOldRDN << endl);
...
LDAPMessageQueue *LDAPAsynConnection::extOperation(const string & oid, const string & value, const LDAPConstraints * cons)
{
...
void LDAPAsynConnection::abandon(LDAPMessageQueue * q)
{
    ...
    {
        req = reqStack->top();
        if (ldap_abandon_ext(cur_session, req->getMsgID(), 0, 0) != LDAP_SUCCESS)
        {
            throw LDAPException(this);
        }
        delete req;
...
const LDAPConstraints *LDAPAsynConnection::getConstraints() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAsynConnection::getConstraints()" << endl);
    return m_constr;
}

TlsOptions LDAPAsynConnection::getTlsOptions() const const
{
    return TlsOptions(cur_session);
}

LDAP *LDAPAsynConnection::getSessionHandle() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAsynConnection::getSessionHandle()" << endl);
    return cur_session;
}

const string & LDAPAsynConnection::getHost() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAsynConnection::setHost()" << endl);
    return m_uri.getHost();
}

int LDAPAsynConnection::getPort() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAsynConnection::getPort()" << endl);
    return m_uri.getPort();
}

LDAPAsynConnection *LDAPAsynConnection::referralConnect(const LDAPUrlList & urls, LDAPUrlList::const_iterator & usedUrl, const LDAPConstraints * cons) const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAsynConnection::referralConnect()" << endl) LDAPUrlList::const_iterator conUrl;
    ...
    for (conUrl = urls.begin(); conUrl != urls.end(); conUrl++)
    {
        ...
        DEBUG(LDAP_DEBUG_TRACE, "   connecting to: " << host << ":" << port << endl);
