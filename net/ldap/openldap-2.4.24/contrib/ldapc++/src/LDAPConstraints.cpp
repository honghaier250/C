...
void LDAPConstraints::setAliasDeref(int deref)
{
    ...
    DEBUG(LDAP_DEBUG_TRACE | LDAP_DEBUG_PARAMETER, "   deref:" << deref << endl);
    if ((deref == LDAPConstraints::DEREF_NEVER) || (deref == LDAPConstraints::DEREF_SEARCHING) || (deref == LDAPConstraints::DEREF_FINDING) || (deref == LDAPConstraints::DEREF_ALWAYS))
    {
        m_aliasDeref = deref;
    }
}
...
void LDAPConstraints::setHopLimit(int limit)
{
    ...
    DEBUG(LDAP_DEBUG_TRACE | LDAP_DEBUG_PARAMETER, "   limit:" << limit << endl);
    m_HopLimit = limit;
}
...
LDAPControl **LDAPConstraints::getClCtrlsArray() const const
{
...
timeval *LDAPConstraints::getTimeoutStruct() const const
{
...
bool LDAPConstraints::getReferralChase() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPContstraints::getReferralChase()" << endl);
    return m_referralChase;
}

int LDAPConstraints::getHopLimit() const const
{
