...
LDAPConnection::LDAPConnection(const string & hostname, int port, LDAPConstraints * cons):LDAPAsynConnection(hostname, port, cons)
{
}
...
void LDAPConnection::bind(const string & dn, const string & passwd, LDAPConstraints * cons)
{
...
void LDAPConnection::saslInteractiveBind(const std::string & mech, int flags, SaslInteractionHandler * sih, const LDAPConstraints * cons)
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPConnection::bind" << endl);
    LDAPMessageQueue *msg = 0;
    LDAPResult *res = 0;
...
bool LDAPConnection::compare(const string & dn, const LDAPAttribute & attr, LDAPConstraints * cons)
{
...
void LDAPConnection::modify(const string & dn, const LDAPModList * mods, const LDAPConstraints * cons)
{
...
void LDAPConnection::rename(const string & dn, const string & newRDN, bool delOldRDN, const string & newParentDN, const LDAPConstraints * cons)
{
    ...
    try
    {
        msg = LDAPAsynConnection::rename(dn, newRDN, delOldRDN, newParentDN, cons);
        res = (LDAPResult *) msg->getNext();
    } catch(LDAPException e)
    {
        delete msg;
...
LDAPSearchResults *LDAPConnection::search(const string & base, int scope, const string & filter, const StringList & attrs, bool attrsOnly, const LDAPConstraints * cons)
{
    ...
    try
    {
        ...
        msgq = LDAPAsynConnection::search(base, scope, filter, attrs, attrsOnly, cons);
...
LDAPExtResult *LDAPConnection::extOperation(const string & oid, const string & value, const LDAPConstraints * cons)
{
...
const string & LDAPConnection::getHost() const const
{
    return LDAPAsynConnection::getHost();
}

int LDAPConnection::getPort() const const
{
...
const LDAPConstraints *LDAPConnection::getConstraints() const const
{
    return LDAPAsynConnection::getConstraints();
}

TlsOptions LDAPConnection::getTlsOptions() const const
{
