...
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPAttributeList::LDAPAttributList( )" << endl);
}
...
LDAPAttributeList::LDAPAttributeList(const LDAPAsynConnection * ld, LDAPMessage * msg)
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPAttributeList::LDAPAttributList()" << endl);
    BerElement *ptr = 0;
...
*/ BerValue **values;
    for (; name != 0; name = ldap_next_attribute(ld->getSessionHandle(), msg, ptr))
    {
        values = ldap_get_values_len(ld->getSessionHandle(), msg, name);
...
size_t LDAPAttributeList::size() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAttribute::size()" << endl);
    return m_attrs.size();
}

bool LDAPAttributeList::empty() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAttribute::empty()" << endl);
    return m_attrs.empty();
}

LDAPAttributeList::const_iterator LDAPAttributeList::begin() constconst
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAttribute::begin()" << endl);
    return m_attrs.begin();
}

LDAPAttributeList::const_iterator LDAPAttributeList::end() constconst
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAttribute::end()" << endl);
    return m_attrs.end();
}

const LDAPAttribute *LDAPAttributeList::getAttributeByName(const string & name) const const
{
    ...
    for (i = m_attrs.begin(); i != m_attrs.end(); i++)
    {
        ...
        {
            if (equal(name.begin(), name.end(), tmpType.begin(), nocase_compare))
            {
                return &(*i);
...
void LDAPAttributeList::addAttribute(const LDAPAttribute & attr)
{
    ...
    DEBUG(LDAP_DEBUG_TRACE | LDAP_DEBUG_PARAMETER, "   attr:" << attr << endl);
    ...
    for (i = m_attrs.begin(); i != m_attrs.end(); i++)
    {
        ...
        {
            if (equal(tmpAttrType.begin(), tmpAttrType.end(), attrType.begin(), nocase_compare))
            {
                ...
                DEBUG(LDAP_DEBUG_TRACE, "Attribute" << i->getName() << "already present" << endl);
                done = true;
...
void LDAPAttributeList::delAttribute(const std::string & type)
{
    ...
    for (i = m_attrs.begin(); i != m_attrs.end(); i++)
    {
        ...
        {
            if (equal(type.begin(), type.end(), i->getName().begin(), nocase_compare))
            {
                m_attrs.erase(i);
                break;
            }
...
LDAPMod **LDAPAttributeList::toLDAPModArray() const const
{
