...
LDAPSearchRequest::LDAPSearchRequest(const LDAPSearchRequest & req):LDAPRequest(req)
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPSearchRequest::LDAPSearchRequest(&)" << endl);
    m_base = req.m_base;
    m_scope = req.m_scope;
...
LDAPSearchRequest::LDAPSearchRequest(const string & base, int scope,
                                     ...
                                     LDAPAsynConnection * connect, const LDAPConstraints * cons, bool isReferral, const LDAPRequest * parent):LDAPRequest(connect, cons, isReferral, parent)
{

    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPSearchRequest:LDAPSearchRequest()" << endl);
    DEBUG(LDAP_DEBUG_CONSTRUCT & LDAP_DEBUG_PARAMETER, "   base:" << base << endl << "   scope:" << scope << endl << "   filter:" << filter << endl);
...
LDAPMessageQueue *LDAPSearchRequest::sendRequest()
{
    ...
    ldap_set_option(m_connection->getSessionHandle(), LDAP_OPT_DEREF, &aliasDeref);
...
LDAPRequest *LDAPSearchRequest::followReferral(LDAPMsg * ref)
{
    ...
    if (con != 0)
    {
        if ((usedUrl->getFilter() != "") && (usedUrl->getFilter() != m_filter))
        {
        ...
        if ((ref->getMessageType() == LDAPMsg::SEARCH_REFERENCE) && (m_scope == LDAPAsynConnection::SEARCH_ONE))
        {
    ...
    return new LDAPSearchRequest(usedUrl->getDN(), scope, filter, m_attrs, m_attrsOnly, con, m_cons, true, this);
}

bool LDAPSearchRequest::equals(const LDAPRequest * req) const const
{
    ...
    if (LDAPRequest::equals(req))
    {
        ...
        if ((m_base == sreq->m_base) && (m_scope == sreq->m_scope))
        {
            return true;
        }
    }
