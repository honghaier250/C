...
LDAPModDNRequest::LDAPModDNRequest(const LDAPModDNRequest & req):LDAPRequest(req)
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPModDNRequest::LDAPModDNRequest(&)" << endl);
    m_dn = req.m_dn;
...
LDAPModDNRequest::LDAPModDNRequest(const string & dn, const string & newRDN,
                                   bool deleteOld, const string & newParentDN, LDAPAsynConnection * connect, const LDAPConstraints * cons, bool isReferral, const LDAPRequest * parent):LDAPRequest(connect, cons, isReferral, parent)
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPModDNRequest::LDAPModDNRequest(&)" << endl);
    DEBUG(LDAP_DEBUG_CONSTRUCT | LDAP_DEBUG_PARAMETER, "   dn:" << dn << endl << "   newRDN:" << newRDN << endl << "   deleteOld:" << deleteOld << endl << "   newParent:" << newParentDN << endl);
    m_dn = dn;
    m_newRDN = newRDN;
...
LDAPMessageQueue *LDAPModDNRequest::sendRequest()
{
    ...
    const char *newParentDN = (m_newParentDN == "" ? 0 : m_newParentDN.c_str());
...
LDAPRequest *LDAPModDNRequest::followReferral(LDAPMsg * ref)
{
    ...
    if (con != 0)
    {
        return new LDAPModDNRequest(m_dn, m_newRDN, m_deleteOld, m_newParentDN, con, m_cons, true, this);
    }
    return 0;
}
