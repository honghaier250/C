...
static const int TLS_CONNECT_CB static const int TLS_CONNECT_ARG
#endif
...
void TlsOptions::setOption(tls_option opt, const std::string & value) const const
{
    checkOpt(opt, STRING);
    this->setOption(opt, value.empty()? NULL : (void *) value.c_str());
}

void TlsOptions::setOption(tls_option opt, int value) const const
{
    checkOpt(opt, INT);
    this->setOption(opt, (void *) &value);
}

void TlsOptions::setOption(tls_option opt, void *value) const const
{
...
void TlsOptions::getOption(tls_option opt, void *value) const const
{
...
int TlsOptions::getIntOption(tls_option opt) const const
{
    int value;
    checkOpt(opt, INT);
...
std::string TlsOptions::getStringOption(tls_option opt) const const
{
    char *value;
    checkOpt(opt, STRING);
    ldap_get_option(m_ld, optmap[opt].optval, (void *) &value);
...
void TlsOptions::newCtx() const const
{
