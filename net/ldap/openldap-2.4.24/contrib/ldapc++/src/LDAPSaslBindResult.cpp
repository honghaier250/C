...
LDAPSaslBindResult::LDAPSaslBindResult(const LDAPRequest * req, LDAPMessage * msg):LDAPResult(req, msg)
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPSaslBindResult::LDAPSaslBindResult()" << std::endl);
    BerValue *data = 0;
...
const string & LDAPSaslBindResult::getServerCreds() const const
{
    return m_creds;
}
