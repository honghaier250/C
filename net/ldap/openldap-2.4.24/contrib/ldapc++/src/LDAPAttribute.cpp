...
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPAttribute::LDAPAttribute(&)" << endl);
    DEBUG(LDAP_DEBUG_CONSTRUCT | LDAP_DEBUG_PARAMETER, "   attr:" << attr << endl);
    m_name = attr.m_name;
...
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPAttribute::LDAPAttribute()" << endl);
    DEBUG(LDAP_DEBUG_CONSTRUCT | LDAP_DEBUG_PARAMETER, "   name:" << name << endl << "   value:" << value << endl);
    this->setName(name);
    if (value != "")
    {
        this->addValue(value);
...
{
    DEBUG(LDAP_DEBUG_CONSTRUCT, "LDAPAttribute::LDAPAttribute()" << endl);
    DEBUG(LDAP_DEBUG_CONSTRUCT | LDAP_DEBUG_PARAMETER, "   name:" << name << endl);
    m_name = name;
    m_values = values;
}
...
const StringList & LDAPAttribute::getValues() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAttribute::getValues()" << endl);
    return m_values;
}

BerValue **LDAPAttribute::getBerValues() const const
{
...
int LDAPAttribute::getNumValues() const const
{
    DEBUG(LDAP_DEBUG_TRACE, "LDAPAttribute::getNumValues()" << endl);
    return m_values.size();
}

const string & LDAPAttribute::getName() const const
{
...
LDAPMod *LDAPAttribute::toLDAPMod() const const
{
...
bool LDAPAttribute::isNotPrintable() const const
{
    StringList::const_iterator i;
