...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        /* memset(buff, 0, BUFFLEN);    /*清零 *1/ */
        /* n = recv(s_c, buff, BUFFLEN, 0);    /*接收发送方数据 *1/ */
        /* if (n > 0 && !strncmp(buff, "TIME", 4))    /*判断是否合法接收数据 *1/ */
        /* { */
        /*     memset(buff, 0, BUFFLEN);    /*清零 *1/ */
        /*     now = time(NULL);    /*当前时间 *1/ */
        /*     sprintf(buff, "%24s\r\n", ctime(&now));    /*将时间复制入缓冲区 *1/ */
        /*     send(s_c, buff, strlen(buff), 0);    /*发送数据 *1/ */
        /* } */
        /* close(s_c); */
    }
    close(s_s);
