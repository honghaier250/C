...
shishi_auth (Shishi ** handle, int verbose, char **cname,
             const char *sname, int sock, char *cmd, int port, Shishi_key ** enckey, char *realm)
{
    Shishi_ap *ap;
    ...
    if (rc != SHISHI_OK)
    {
        fprintf (stderr, "error initializing shishi: %s\n", shishi_strerror (rc));
        return 1;
    }
...
static void senderror (int s, char type, char *buf)
{
...
get_auth (int infd, Shishi ** handle, Shishi_ap ** ap,
          Shishi_key ** enckey, const char **err_msg, int *protoversion, int *cksumtype, char **cksum, int *cksumlen)
{
    Shishi_key *key;
...
# ifdef ENCRYPTION
    ...
    {
        *enckey = NULL;
        shishi_authenticator_get_subkey (*handle, shishi_ap_authenticator (*ap), enckey);
    }

# endif

    /* Get authenticator checksum */
    rc = shishi_authenticator_cksum (*handle, shishi_ap_authenticator (*ap), cksumtype, cksum, cksumlen);
    if (rc != SHISHI_OK)
        return rc;
...
# ifdef ENCRYPTION
    if (*protoversion == 1)
    {
        ...
        rc = shishi_encticketpart_get_key (*handle, shishi_tkt_encticketpart (tkt), enckey);
        if (rc != SHISHI_OK)
            return rc;
    }
# endif
...
int readenc (Shishi * h, int sock, char *buf, int *len, shishi_ivector * iv, Shishi_key * enckey, int proto)
{
    char *out;
    ...
    hashsize = shishi_checksum_cksumlen (shishi_cipher_defaultcksumtype (enctype));
    ...
    if (proto == 1)
    {
        rc = shishi_decrypt (h, enckey, iv->keyusage, outbis, dlen, &out, &outlen);
        if (rc != SHISHI_OK)
        {
...
int writeenc (Shishi * h, int sock, char *buf, int wlen, int *len, shishi_ivector * iv, Shishi_key * enckey, int proto)
{
    char *out;
    ...
    if (proto == 2)
    {
        ...
        /* encrypt it */
        rc = shishi_crypto_encrypt (iv->ctx, bufbis, wlen + sizeof (int), &out, &outlen);
    }
    else
    {
        bufbis = malloc (wlen);
        ...
        /* data to encrypt = size + data */
        rc = shishi_encrypt (h, enckey, iv->keyusage, bufbis, wlen, &out, &outlen);
    }
