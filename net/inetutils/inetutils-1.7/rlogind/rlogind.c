...
extern int iruserok (uint32_t raddr, int superuser, const char *ruser, const char *luser);
...
RETSIGTYPE rlogind_sigchld (int sig)
{
    pid_t pid;
...
int rlogind_auth (int fd, struct auth_data *ap)
{
    ...
    hp = gethostbyaddr ((char *) &ap->from.sin_addr, sizeof (struct in_addr), ap->from.sin_family);
    if (hp)
        hostname = hp->h_name;
    ...
    if (verify_hostname || in_local_domain (ap->hostname))
    {
        ...
        for (hp = gethostbyname (ap->hostname); hp && !match; hp->h_addr_list++)
        {
            if (hp->h_addr_list[0] == NULL)
                break;
            match = memcmp (hp->h_addr_list[0], &ap->from.sin_addr, sizeof (ap->from.sin_addr)) == 0;
        }
        if (!match)
        {
...
#endif
    {
        ...
        if (ap->from.sin_family != AF_INET || port >= IPPORT_RESERVED || port < IPPORT_RESERVED / 2)
        {
            syslog (LOG_NOTICE, "Connection from %s on illegal port %d", inet_ntoa (ap->from.sin_addr), port);
            fatal (fd, "Permission denied", 0);
        }
#ifdef IP_OPTIONS
        {
...
void exec_login (int authenticated, struct auth_data *ap)
{
    ...
    {
#ifdef SOLARIS
        execle (path_login, "login", "-p", "-h", ap->hostname, ap->term, "-f", "--", ap->lusername, NULL, ap->env);
#else
        execle (path_login, "login", "-p", "-h", ap->hostname, "-f", ap->lusername, NULL, ap->env);
#endif
    }
    else
    {
#ifdef SOLARIS
        execle (path_login, "login", "-p", "-h", ap->hostname, ap->term, "--", ap->lusername, NULL, ap->env);
#else
        execle (path_login, "login", "-p", "-h", ap->hostname, ap->lusername, NULL, ap->env);
#endif
    }
...
int rlogind_mainloop (int infd, int outfd)
{
    ...
    syslog (LOG_INFO, "Connect from %s:%d", inet_ntoa (auth_data.from.sin_addr), ntohs (auth_data.from.sin_port));

    true = 1;
    if (keepalive && setsockopt (infd, SOL_SOCKET, SO_KEEPALIVE, &true, sizeof true) < 0)
...
int do_rlogin (int infd, struct auth_data *ap)
{
    ...
    if (rc)
        syslog (LOG_ERR, "iruserok failed: rusername=%s, lusername=%s", ap->rusername, ap->lusername);
    return rc;
}

#if defined(KERBEROS) || defined(SHISHI)
int do_krb_login (int infd, struct auth_data *ap, const char **err_msg)
{
    int rc;
...
# ifdef KRB4
int do_krb4_login (int infd, struct auth_data *ap, const char **err_msg)
{
    int rc;
...
#  ifdef ENCRYPTION
    if (encrypt_io)
    {
        ...
        rc = krb_recvauth (authopts, 0, ticket, "rcmd", instance, &ap->from, &faddr, kdata, "", schedule, version);
        des_set_key (kdata->session, schedule);

    }
    else
#  endif
        rc = krb_recvauth (authopts, 0, ticket, "rcmd", instance, &ap->from, NULL, kdata, "", NULL, version);
...
int do_krb5_login (int infd, struct auth_data *ap, const char **err_msg)
{
    ...
    if (!rcache)
    {
        ...
        status = krb5_sname_to_principal (ap->context, 0, 0, KRB5_NT_SRV_HST, &server);
        if (status)
            return status;

        status = krb5_get_server_rcache (ap->context, krb5_princ_component (ap->context, server, 0), &rcache);
    ...
    status = krb5_recvauth (ap->context, &auth_ctx, &infd, NULL, 0, 0, ap->keytab, &ticket);
    if (status)
        return status;

    if ((status = krb5_auth_con_getauthenticator (ap->context, auth_ctx, &authenticator)))
        return status;
    ...
    if ((status = krb5_copy_principal (ap->context, ticket->enc_part2->client, &ap->client)))
        return status;
...
#  ifdef ENCRYPTION
    rc = get_auth (infd, &ad->h, &ad->ap, &ad->enckey, err_msg, &ad->protocol, &cksumtype, &cksum, &cksumlen);
#  else
    rc = get_auth (infd, &ad->h, &ad->ap, NULL, err_msg, &ad->protocol, &cksumtype, &cksum, &cksumlen);
#  endif
    if (rc != SHISHI_OK)
        return rc;
                ...
                case SHISHI_DES3_CBC_HMAC_SHA1_KD:
                    ...
                    ad->ivtab[i]->ctx =
                        shishi_crypto (ad->h, ad->enckey, ad->ivtab[i]->keyusage,
                                       shishi_key_type (ad->enckey), ad->ivtab[i]->iv, ad->ivtab[i]->ivlen);
                    break;
...
#  endif
    ...
    syslog (LOG_INFO | LOG_AUTH, "Kerberos V login from %s on %s\n", ad->lusername, ad->hostname);
    ...
    rc = shishi_checksum (ad->h, ad->enckey, 0, cksumtype, cksumdata, strlen (cksumdata), &compcksum, &compcksumlen);
    free (cksum);
...
RETSIGTYPE cleanup (int signo ARG_UNUSED)
{
    char *p;
...
void fatal (int f, const char *msg, int syserr)
{
    ...
    if (syserr)
        snprintf (bp, sizeof buf - (bp - buf), "rlogind: %s: %s.\r\n", msg, strerror (errno));
    else
