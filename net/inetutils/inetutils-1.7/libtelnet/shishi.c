...
static int Data (TN_Authenticator * ap, int type, unsigned char *d, int c)
{
...
int krb5shishi_init (TN_Authenticator * ap, int server)
{
    if (server)
...
static int delayed_shishi_init (void)
{
    if (shishi_handle)
        return 1;
...
void krb5shishi_cleanup (TN_Authenticator * ap)
{
...
int krb5shishi_send (TN_Authenticator * ap)
{
    int ap_opts;
...
# ifndef DONT_ALWAYS_USE_DES
    ...
    if (shishi_tkt_keytype_fast (tkt) != SHISHI_DES_CBC_CRC &&
        shishi_tkt_keytype_fast (tkt) != SHISHI_DES_CBC_MD4 && shishi_tkt_keytype_fast (tkt) != SHISHI_DES_CBC_MD5)
        rc = shishi_ap_etype_tktoptionsdata (shishi_handle, &auth_handle,
                                             SHISHI_DES_CBC_MD5, tkt, ap_opts, (char *) &type_check, 2);
    else
# endif
        rc = shishi_ap_tktoptionsdata (shishi_handle, &auth_handle, tkt, ap_opts, (char *) &type_check, 2);
    if (rc != SHISHI_OK)
    {
...
# endif
    ...
    if (auth_debug_mode)
    {
        shishi_authenticator_print (shishi_handle, stdout, shishi_ap_authenticator (auth_handle));
...
# ifdef ENCRYPTION
void shishi_init_key (Session_Key * skey, int type)
{
    int32_t etype = shishi_key_type (enckey);

    if (etype == SHISHI_DES_CBC_CRC || etype == SHISHI_DES_CBC_MD4 || etype == SHISHI_DES_CBC_MD5)
        skey->type = SK_DES;
    else
        skey->type = SK_OTHER;
...
void krb5shishi_reply (TN_Authenticator * ap, unsigned char *data, int cnt)
{
...
# endif
        ...
        case KRB_RESPONSE:
            if ((ap->way & AUTH_HOW_MASK) == AUTH_HOW_MUTUAL)
            {
                ...
                if (auth_debug_mode)
                {
                    shishi_aprep_print (shishi_handle, stdout, shishi_ap_rep (auth_handle));
                    shishi_encapreppart_print (shishi_handle, stdout, shishi_ap_encapreppart (auth_handle));
                }
...
int krb5shishi_status (TN_Authenticator * ap, char *name, int level)
{
    char *cname;
    ...
    rc = shishi_encticketpart_client
        (shishi_handle, shishi_tkt_encticketpart (shishi_ap_tkt (auth_handle)), &cname, &cnamelen);

    if (UserNameRequested
        && rc == SHISHI_OK
        && cnamelen == strlen (UserNameRequested) && memcmp (UserNameRequested, cname, cnamelen) == 0)
    {
...
int krb5shishi_is_auth (TN_Authenticator * a, unsigned char *data, int cnt, char *errbuf, int errbuflen)
{
    Shishi_key *key, *key2;
...
# endif
    ...
    if (rc != SHISHI_OK)
    {
        snprintf (errbuf, errbuflen, "Cannot allocate authentication structures: %s", shishi_strerror (rc));
        return 1;
    }
    ...
    if (rc != SHISHI_OK)
    {
        snprintf (errbuf, errbuflen, "Cannot parse authentication information: %s", shishi_strerror (rc));
        return 1;
    }
    ...
    if (shishi_apreq_mutual_required_p (shishi_handle, shishi_ap_req (auth_handle)))
    {
        char *der;
    ...
    rc = shishi_encticketpart_crealm (shishi_handle,
                                      shishi_tkt_encticketpart (shishi_ap_tkt
                                                                (auth_handle)), &cnamerealm, &cnamerealmlen);
    if (rc != SHISHI_OK)
    {
...
void krb5shishi_is (TN_Authenticator * ap, unsigned char *data, int cnt)
{
    int r = 0;
...
void krb5shishi_printsub (unsigned char *data, int cnt, unsigned char *buf, int buflen)
{
    char *p;
