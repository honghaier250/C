...
static int Data (TN_Authenticator * ap, int type, krb5_pointer d, int c)
{
    ...
    if (auth_debug_mode)
    {
        printf ("%s:%d: [%d] (%d)", str_data[3] == TELQUAL_IS ? ">>>IS" : ">>>REPLY", str_data[3], type, c);
        printd (d, c);
        printf ("\r\n");
    }
...
int kerberos5_init (TN_Authenticator * ap, int server)
{
...
void encryption_init (krb5_creds * creds)
{
        ...
        switch (newkey->enctype)
        {
            ...
            default:
                switch (creds->keyblock.enctype)
                {
                    case ENCTYPE_DES_CBC_CRC:
                    case ENCTYPE_DES_CBC_MD5:
                        krb5_copy_keyblock (telnet_context, &creds->keyblock, &session_key);
                        break;
...
int kerberos5_send (TN_Authenticator * ap)
{
    krb5_error_code r;
    ...
    if ((r = krb5_sname_to_principal (telnet_context, RemoteHostName, "host", KRB5_NT_SRV_HST, &creds.server)))
    {
    ...
    if ((r = krb5_get_credentials (telnet_context, 0, ccache, &creds, &new_creds)))
    {
...
# endif
    ...
    krb5_auth_con_setflags (telnet_context, auth_context, KRB5_AUTH_CONTEXT_RET_TIME);
    ...
    r = krb5_mk_req_extended (telnet_context, &auth_context, ap_opts, &check_data, new_creds, &auth);
...
# ifdef ENCRYPTION
void telnet_encrypt_key (Session_Key * skey)
{
    if (session_key)
    {
        skey->type = SK_DES;
...
void kerberos5_reply (TN_Authenticator * ap, unsigned char *data, int cnt)
{
# ifdef ENCRYPTION
    Session_Key skey;
# endif
        ...
        case KRB_ACCEPT:
            ...
            if (cnt)
                printf ("[ Kerberos V5 accepts you as ``%.*s''%s ]\r\n", cnt, data,
                        mutual_complete ? " (server authenticated)" : " (server NOT authenticated)");
            else
...
# endif
        ...
        case KRB_RESPONSE:
            if ((ap->way & AUTH_HOW_MASK) == AUTH_HOW_MUTUAL)
            {
                ...
                if ((r = krb5_rd_rep (telnet_context, auth_context, &inbuf, &reply)))
                {
...
int kerberos5_status (TN_Authenticator * ap, char *name, int level)
{
    if (level < AUTH_USER)
        return level;

    if (UserNameRequested && krb5_kuserok (telnet_context, ticket->enc_part2->client, UserNameRequested))
    {
...
int kerberos5_is_auth (TN_Authenticator * ap, unsigned char *data, int cnt, char *errbuf, int errbuflen)
{
    int r = 0;
...
# endif
    ...
    if (!r)
    {
        ...
        if (!r && !rcache)
        {
            r = krb5_sname_to_principal (telnet_context, 0, 0, KRB5_NT_SRV_HST, &server);
            if (!r)
            {
                r = krb5_get_server_rcache (telnet_context, krb5_princ_component (telnet_context, server, 0), &rcache);
    ...
    if (!r)
        r = krb5_rd_req (telnet_context, &auth_context, &auth, NULL, keytabid, NULL, &ticket);
    if (r)
    {
        snprintf (errbuf, errbuflen, "krb5_rd_req failed: %s", error_message (r));
        return r;
    }
    ...
    if (krb5_princ_component (telnet_context, ticket->server, 0)->length < 256)
    {
        ...
        princ[krb5_princ_component (telnet_context, ticket->server, 0)->length] = '\0';
        if (strcmp ("host", princ))
        {
            snprintf (errbuf, errbuflen, "incorrect service name: \"%s\" != \"host\"", princ);
            return 1;
        }
    }
    else
    ...
    r = krb5_auth_con_getauthenticator (telnet_context, auth_context, &authenticator);
    if (r)
    {
        snprintf (errbuf, errbuflen, "krb5_auth_con_getauthenticator failed: %s", error_message (r));
        return 1;
    }

# ifdef AUTH_ENCRYPT_MASK
    if ((ap->way & AUTH_ENCRYPT_MASK) == AUTH_ENCRYPT_ON && !authenticator->checksum)
    {
        snprintf (errbuf, errbuflen, "authenticator is missing required checksum");
        return 1;
    }
# endif
    ...
    if (authenticator->checksum)
    {
        ...
        if (r)
        {
            snprintf (errbuf, errbuflen, "krb5_auth_con_getkey failed: %s", error_message (r));
            return 1;
        }

        r = krb5_verify_checksum (telnet_context,
                                  cksum->checksum_type, cksum, &type_check, 2, key->contents, key->length);

        if (r)
        {
            snprintf (errbuf, errbuflen, "checksum verification failed: %s", error_message (r));
            return 1;
        }
    ...
    if ((ap->way & AUTH_HOW_MASK) == AUTH_HOW_MUTUAL)
    {
        ...
        {
            snprintf (errbuf, errbuflen, "Make reply failed: %s", error_message (r));
            return 1;
        }
    ...
    {
        krb5_copy_keyblock (telnet_context, ticket->enc_part2->session, &session_key);
    }
    telnet_encrypt_key (&skey);
    return 0;
}
...
# ifdef FORWARD
int kerberos5_is_forward (TN_Authenticator * ap, unsigned char *data, int cnt, char *errbuf, int errbuflen)
{
    int r = 0;
    ...
    if ((r = krb5_auth_con_genaddrs (telnet_context, auth_context,
        ...
        || (r = rd_and_store_for_creds (telnet_context, auth_context, &inbuf, ticket)))
    {
        snprintf (errbuf, errbuflen, "Read forwarded creds failed: %s", error_message (r));
...
void kerberos5_is (TN_Authenticator * ap, unsigned char *data, int cnt)
{
    int r = 0;
    ...
    {
        if (!errbuf[0])
            snprintf (errbuf, sizeof errbuf, "kerberos_is: %s", error_message (r));
...
static char *req_type_str (int type)
{
    switch (type)
    {
        case KRB_REJECT:
...
void kerberos5_printsub (unsigned char *data, int cnt, unsigned char *buf, int buflen)
{
    char *p;
...
void kerberos5_forward (TN_Authenticator * ap)
{
    krb5_error_code r;
    ...
    for (;;)                    /* Fake loop */
    {
        ...
        if ((r = krb5_sname_to_principal (telnet_context, RemoteHostName, "host", KRB5_NT_SRV_HST, &server)))
        {
        ...
        if ((r = krb5_fwd_tgt_creds (telnet_context, auth_context, 0, client,
                                     server, ccache, forward_flags & OPTS_FORWARDABLE_CREDS, &forw_creds)))
        {
