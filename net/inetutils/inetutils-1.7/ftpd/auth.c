...
int auth_user (const char *name, struct credentials *pcred)
{
    pcred->guest = 0;
        ...
        default:
            {
                ...
                if (strcmp (name, "ftp") == 0 || strcmp (name, "anonymous") == 0)
                {
                    ...
                    if (checkuser (PATH_FTPUSERS, "ftp") || checkuser (PATH_FTPUSERS, "anonymous"))
                    {
                    ...
                    {
                        pcred->guest = 1;
                        strcpy (pcred->message, "Guest login ok, type your name as password.");
                    }
                    else
                    {
...
int auth_pass (const char *passwd, struct credentials *pcred)
{
    switch (pcred->auth_type)
    {
#ifdef WITH_PAM
...
int sgetcred (const char *name, struct credentials *pcred)
{
    struct passwd *p;
    ...
    if (p->pw_passwd == NULL || strlen (p->pw_passwd) == 1)
    {
        ...
        if (spw != NULL)
        {
            ...
            if ((spw->sp_expire > 0 && spw->sp_expire < today)
                || (spw->sp_max > 0 && spw->sp_lstchg > 0 && (spw->sp_lstchg + spw->sp_max < today)))
            {
