    ...
    for (count = 0; count < num_msg; count++)
    {
        ...
        if (savemsg)
        {
            ...
            if (pcred->message)    /* XXX: make sure we split newlines correctly */
            {
                ...
                char *s = realloc (pcred->message, len + strlen (msg[count]->msg) + 1);
...
static int pam_doit (struct credentials *pcred)
{
    char *username;
...
int pam_user (const char *username, struct credentials *pcred)
{
    int error;
...
int pam_pass (const char *passwd, struct credentials *pcred)
{
