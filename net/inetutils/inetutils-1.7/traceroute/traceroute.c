...
void trace_init (trace_t * t, const struct sockaddr_in to, const enum trace_type type);
...
void do_try (trace_t * trace, const int hop, const int max_hops, const int max_tries);
...
#define GRP 0
    ...
    {"tries", 'q', "NUM", 0, "send NUM probe packets per hop (default: 3)",
     GRP + 1},
    {"type", 'M', "METHOD", 0, "use METHOD (`icmp' or `udp') for traceroute " "operations", GRP + 1},
#undef GRP
    {NULL}
};
...
void do_try (trace_t * trace, const int hop, const int max_hops, const int max_tries)
{
    ...
    for (tries = 0; tries < max_tries; tries++)
    {
        ...
        else
        {
            ...
            {
                triptime = ((double) now.tv_sec) * 1000.0 + ((double) now.tv_usec) / 1000.0;
                ...
                else
                {
                    if (tries == 0 || prev_addr != trace->from.sin_addr.s_addr)
                        printf (" %s (%s) ", inet_ntoa (trace->from.sin_addr), get_hostname (&trace->from.sin_addr));
...
char *get_hostname (struct in_addr *addr)
{
    ...
    {
        struct hostent *info = gethostbyaddr ((char *) addr, sizeof (*addr), AF_INET);
...
void trace_init (trace_t * t, const struct sockaddr_in to, const enum trace_type type)
{
    ...
    if (t->type == TRACE_UDP)
    {
        ...
        if (setsockopt (t->udpfd, IPPROTO_IP, IP_TTL, ttlp, sizeof (t->ttl)) < 0)
    ...
    if (t->type == TRACE_ICMP || t->type == TRACE_UDP)
    {
        ...
        if (protocol)
        {
            ...
            if (setsockopt (t->icmpfd, IPPROTO_IP, IP_TTL, ttlp, sizeof (t->ttl)) < 0)
                error (EXIT_FAILURE, errno, "setsockopt");
        }
        else
        {
...
void trace_port (trace_t * t, const unsigned short int port)
{
    assert (t);
...
int trace_read (trace_t * t)
{
    ...
    len = recvfrom (t->icmpfd, (char *) data, 56, 0, (struct sockaddr *) &t->from, &siz);
    if (len < 0)
    ...
    switch (t->type)
    {
        ...
        case TRACE_ICMP:
            if (ic->icmp_type != ICMP_TIME_EXCEEDED && ic->icmp_type != ICMP_ECHOREPLY)
                return -1;

            if (ic->icmp_type == ICMP_ECHOREPLY && (ic->icmp_seq != pid || ic->icmp_id != pid))
                return -1;
        ...
        case TRACE_UDP:
            {
                ...
                len = sendto (t->udpfd, (char *) data, sizeof (data), 0, (struct sockaddr *) &t->to, sizeof (t->to));
                if (len < 0)
                {
                    switch (errno)
                    {
        ...
        case TRACE_ICMP:
            {
                ...
                len = sendto (t->icmpfd, (char *) &hdr, sizeof (hdr), 0, (struct sockaddr *) &t->to, sizeof (t->to));
                if (len < 0)
                {
                    switch (errno)
                    {
