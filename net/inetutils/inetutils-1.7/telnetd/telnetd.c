...
typedef unsigned int ip_addr_t;
 ...
 /*FIXME*/ void telnetd_setup (int fd)
{
#ifdef IPV6
...
#ifdef IPV6
    err = getnameinfo ((struct sockaddr *) &saddr, sizeof (saddr), buf, sizeof (buf), NULL, 0, NI_NUMERICHOST);
    if (err)
    {
        const char *errmsg;
    ...
    err = getnameinfo ((struct sockaddr *) &saddr, sizeof (saddr), buf2, sizeof (buf2), NULL, 0, NI_NAMEREQD);
    ...
    if (reverse_lookup)
    {
        ...
        if (err)
        {
            ...
            syslog (LOG_AUTH | LOG_NOTICE, "Forward resolve of %s failed: %s", remote_hostname, errmsg);
            fatal (fd, "Cannot resolve address.");
        }
        ...
        if (aip == NULL)
        {
            syslog (LOG_AUTH | LOG_NOTICE, "None of addresses of %s matched %s", remote_hostname, buf);
            exit (0);
        }
...
#else
    hp = gethostbyaddr ((char *) &saddr.sin_addr.s_addr, sizeof (saddr.sin_addr.s_addr), AF_INET);
    if (reverse_lookup)
    {
        char **ap;
        ...
        if (!hp)
        {
            syslog (LOG_AUTH | LOG_NOTICE, "Can't resolve %s: %s", inet_ntoa (saddr.sin_addr), hstrerror (h_errno));
            fatal (fd, "Cannot resolve address.");
        }
        ...
        if (!hp)
        {
            syslog (LOG_AUTH | LOG_NOTICE, "Forward resolve of %s failed: %s", remote_hostname, hstrerror (h_errno));
            fatal (fd, "Cannot resolve address.");
        }
        ...
        if (ap == NULL)
        {
            syslog (LOG_AUTH | LOG_NOTICE,
                    "None of addresses of %s matched %s", remote_hostname, inet_ntoa (saddr.sin_addr));
            exit (0);
        }
    }
    else
    ...
    if (keepalive && setsockopt (fd, SOL_SOCKET, SO_KEEPALIVE, (char *) &true, sizeof (true)) < 0)
        syslog (LOG_WARNING, "setsockopt (SO_KEEPALIVE): %m");

    if (debug_tcp && setsockopt (fd, SOL_SOCKET, SO_DEBUG, (char *) &true, sizeof (true)) < 0)
...
int telnetd_run ()
{
    ...
    DEBUG (debug_report, 1, debug_output_data ("td: Entering processing loop\r\n"));
    ...
    for (;;)
    {
        ...
        if (FD_ISSET (pty, &ibits))
        {
            ...
            if (his_state_is_will (TELOPT_LFLOW) && (c & (TIOCPKT_NOSTOP | TIOCPKT_DOSTOP)))
            {
                int newflow = c & TIOCPKT_DOSTOP ? 1 : 0;

                if (newflow != flowmode)
                {
                    net_output_data ("%c%c%c%c%c%c", IAC, SB, TELOPT_LFLOW, flowmode ? LFLOW_ON : LFLOW_OFF, IAC, SE);
                }
                pty_get_char (0);
            }
...
void print_hostinfo ()
{
    ...
    if (uname (&u) == 0)
    {
        im = malloc (strlen (UNAME_IM_PREFIX)
                     + strlen (u.sysname) + 1 + strlen (u.release) + strlen (UNAME_IM_SUFFIX) + 1);
        if (im)
            sprintf (im, "%s%s %s%s", UNAME_IM_PREFIX, u.sysname, u.release, UNAME_IM_SUFFIX);
    }
#endif                            /* HAVE_UNAME */
    if (!im)
        im = xstrdup ("\r\n\nUNIX (%l) (%t)\r\n\n");
