...
static int readstream (int p, char *ibuf, int bufsize)
{
...
int net_output_level ()
{
    return nfrontp - nbackp;
}

int net_buffer_is_full ()
{
...
int net_read ()
{
    ...
    {
        netip = netibuf;
        DEBUG (debug_report, 1, debug_output_data ("td: netread %d chars\r\n", ncc));
...
void ptyflush ()
{
    ...
    {
        DEBUG (debug_report, 1, debug_output_data ("td: ptyflush %d chars\r\n", n));
...
int pty_input_putback (const char *str, size_t len)
{
...
void io_drain ()
{
  ...
  again:
    ...
    DEBUG (debug_report, 1, debug_output_data ("td: ttloop read %d chars\r\n", ncc));
    netip = netibuf;
...
void netclear ()
{
    ...
    while (nfrontp > thisitem)
    {
        if (wewant (thisitem))
        {
            ...
            for (next = thisitem; wewant (next) && nfrontp > next; next = nextitem (next))
                ;
 ...
 */
int getterminaltype (char *user_name)
{
    int retval = -1;
    ...
    ttloop (his_do_dont_is_changing (TELOPT_ENCRYPT)
            ...
            || his_will_wont_is_changing (TELOPT_NEW_ENVIRON) || his_will_wont_is_changing (TELOPT_OLD_ENVIRON));
#else
    ...
    {
        static unsigned char sb[] = { IAC, SB, TELOPT_TSPEED, TELQUAL_SEND, IAC, SE };

        net_output_datalen (sb, sizeof sb);
    }
    if (his_state_is_will (TELOPT_XDISPLOC))
    {
        static unsigned char sb[] = { IAC, SB, TELOPT_XDISPLOC, TELQUAL_SEND, IAC, SE };

        net_output_datalen (sb, sizeof sb);
    }
    if (his_state_is_will (TELOPT_NEW_ENVIRON))
    {
        static unsigned char sb[] = { IAC, SB, TELOPT_NEW_ENVIRON, TELQUAL_SEND, IAC, SE };

        net_output_datalen (sb, sizeof sb);
    }
    else if (his_state_is_will (TELOPT_OLD_ENVIRON))
    {
        static unsigned char sb[] = { IAC, SB, TELOPT_OLD_ENVIRON, TELQUAL_SEND, IAC, SE };
        ...
        if (his_state_is_will (TELOPT_TTYPE) && !terminaltypeok (terminaltype))
        {
            ...
            for (;;)
            {
                ...
                if (terminaltypeok (terminaltype))
                    break;
                if ((strcmp (last, terminaltype) == 0) || his_state_is_wont (TELOPT_TTYPE))
                {
                    /*
...
void debug_output_datalen (const char *data, size_t len)
{
    if (debug_open ())
        return;
...
void debug_output_data (const char *fmt, ...)
{
    va_list ap;
 ...
 */
void printoption (register char *fmt, register int option)
{
    if (TELOPT_OK (option))
...
void printsub (int direction, unsigned char *pointer, int length)
{
    ...
    if (direction)
    {
        debug_output_data ("td: %s suboption ", direction == '<' ? "recv" : "send");
        if (length >= 3)
        {
            register int j;
    ...
    switch (pointer[0])
    {
        case TELOPT_TTYPE:
            ...
            switch (pointer[1])
            {
                ...
                default:
                    debug_output_data ("- unknown qualifier %d (0x%x).", pointer[1], pointer[1]);
            }
            break;
                ...
                case LM_MODE:
                    ...
                    }
                    {
                        ...
                        snprintf (tbuf, sizeof (tbuf), "%s%s%s%s%s",
                                  ...
                                  pointer[2] & MODE_LIT_ECHO ? "|LIT_ECHO" : "", pointer[2] & MODE_ACK ? "|ACK" : "");
        ...
        case TELOPT_AUTHENTICATION:
            ...
            switch (pointer[1])
            {
                case TELQUAL_REPLY:
                case TELQUAL_IS:
                    debug_output_data (" %s ", (pointer[1] == TELQUAL_IS) ? "IS" : "REPLY");
                    ...
                    debug_output_data ("%s|%s",
                                       ...
                                       ((pointer[3] & AUTH_HOW_MASK) == AUTH_HOW_MUTUAL) ? "MUTUAL" : "ONE-WAY");
                    ...
                    while (i < length)
                    {
                        ...
                        debug_output_data ("%s|%s ",
                                           ((pointer[i] & AUTH_WHO_MASK) ==
                                            AUTH_WHO_CLIENT) ? "CLIENT" : "SERVER",
                                           ((pointer[i] & AUTH_HOW_MASK) == AUTH_HOW_MUTUAL) ? "MUTUAL" : "ONE-WAY");
                        ++i;
                    }
                    break;
        ...
        case TELOPT_ENCRYPT:
            ...
            switch (pointer[1])
            {
                ...
                case ENCRYPT_IS:
                case ENCRYPT_REPLY:
                    debug_output_data (" %s ", (pointer[1] == ENCRYPT_IS) ? "IS" : "REPLY");
                    if (length < 3)
                    {
                        debug_output_data (" (partial suboption??\?)");
                        break;
 ...
 */
void printdata (register char *tag, register char *ptr, register int cnt)
{
    register int i;
...
static char *_var_long_name (struct line_expander *exp, char *start, int length);
...
char *_var_short_name (struct line_expander *exp)
{
    ...
    switch (*exp->cp++)
    {
        ...
        case 'd':
            time (&t);
            strftime (timebuf, sizeof (timebuf), "%l:%M%P on %A, %d %B %Y", localtime (&t));
