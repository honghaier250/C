...
int startslave (char *host, int autologin, char *autoname)
{
    pid_t pid;
...
static void scrub_env ()
{
    ...
    for (cpp2 = cpp = environ; *cpp; cpp++)
    {
        if (strncmp (*cpp, "LD_", 3)
            && strncmp (*cpp, "_RLD_", 5) && strncmp (*cpp, "LIBPATH=", 8) && strncmp (*cpp, "IFS=", 4))
            *cpp2++ = *cpp;
    }
    *cpp2 = 0;
}
...
void cleanup (int sig)
{
    ...
    if (sig)
    {
        ...
        syslog (LOG_INFO, "child process %ld exited: %d", (long) pid, WEXITSTATUS (status));
    }
