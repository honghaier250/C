...
#define GRP 0
    ...
    {"idle-timeout", 'i', "SECONDS", 0, "set idle timeout value to SECONDS",
     GRP + 1},
    {"request-ttl", 'r', "SECONDS", 0, "set request time-to-live value to " "SECONDS", GRP + 1},
...
static void alarm_handler (int err ARG_UNUSED)
{
...
void talkd_run (int fd)
{
    ...
    while (1)
    {
        ...
        {
            rc = sendto (fd, &resp, sizeof resp, 0, (struct sockaddr *) &msg.ctl_addr, sizeof (msg.ctl_addr));
            if (rc != sizeof resp)
