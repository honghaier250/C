...
static void table_delete (table_t * ptr)
{
    table_t *t;
...
static CTL_MSG *lookup_request (CTL_MSG * request, int (*comp) (table_t *, CTL_MSG *, time_t *))
{
    table_t *ptr;
...
static int fuzzy_comp (table_t * ptr, CTL_MSG * request, time_t * unused ARG_UNUSED)
{
    if (ptr->request.type == LEAVE_INVITE
        && strcmp (request->l_name, ptr->request.r_name) == 0 && strcmp (request->r_name, ptr->request.l_name) == 0)
        return 0;
    return 1;
}
...
static int exact_comp (table_t * ptr, CTL_MSG * request, time_t * now)
{
    if (request->type == ptr->request.type
        && request->pid == ptr->request.pid
        && strcmp (request->r_name, ptr->request.r_name) == 0 && strcmp (request->l_name, ptr->request.l_name) == 0)
    {
        ptr->time = *now;
        return 0;
    }
...
int delete_invite (unsigned long id_num)
{
    table_t *ptr;
