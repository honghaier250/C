...
int process_request (CTL_MSG * msg, struct sockaddr_in *sa_in, CTL_RESPONSE * rp)
{
    CTL_MSG *ptr;
    ...
    {
        syslog (LOG_NOTICE, "dropping request: %s@%s", msg->l_name, inet_ntoa (sa_in->sin_addr));
        return 1;
    }
    ...
    {
        syslog (LOG_WARNING, "Bad control address, family %d", msg->ctl_addr.sa_family);
...
void do_announce (CTL_MSG * mp, CTL_RESPONSE * rp)
{
    ...
    hp = gethostbyaddr ((char *) &os2sin_addr (mp->ctl_addr), sizeof (struct in_addr), AF_INET);
    if (!hp)
    {
    ...
    for (uptr = utmpbuf; uptr < utmpbuf + utmp_count; uptr++)
    {
        if (!strncmp (UT_USER (uptr), name, sizeof (UT_USER (uptr))))
        {
            if (notty)
            {
                ...
                strncpy (ftty + sizeof (PATH_DEV) - 1, uptr->ut_line, sizeof (ftty) - sizeof (PATH_DEV) - 1);
