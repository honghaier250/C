...
static error_t ripe_argp_parser (int key, char *arg, struct argp_state *state)
{
...
static error_t gwhois_argp_parser (int key, char *arg, struct argp_state *state)
{
    char *p, *q;
    ...
    if (!server)
    {
        ...
        switch (server[0])
        {
            case 0:
                ...
                break;
            case 1:
                puts (_("This TLD has no whois server, but you can access the " "whois database at"));
            case 2:
                puts (server + 1);
                exit (0);
            case 3:
...
const char *whichwhois (const char *s)
{
    ...
    if (!strchr (s, '.'))
    {
        ...
        if (verb)
            puts (_("I guess it's a netblock name but I don't know where to" " look it up."));
        return "whois.arin.net";
    }

    /* has dot and hypen and it's not in tld_serv[], WTF is it? */
    if (verb)
        puts (_("I guess it's a domain but I don't know where to look it" " up."));
    return "";
}
...
int is_ripe_server (const char *const *srvtab, const char *name)
{
    struct in_addr addr;
...
char *queryformat (const char *server, const char *flags, const char *query)
{
    ...
    *buf = '\0';
    ...
    isripe = is_ripe_server (ripe_servers, server) || is_ripe_server (ripe_servers_old, server);
    if (isripe)
    ...
    else if (!isripe && strcmp (server, "whois.ncst.ernet.in") == 0 && !strchr (query, ' '))
