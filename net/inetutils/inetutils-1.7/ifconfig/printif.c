...
void put_ulong (format_data_t form, int argc, char *argv[], unsigned long value)
{
    char *fmt;
...
void fh_index (format_data_t form, int argc, char *argv[])
{
    ...
    if (indx == 0)
        error (EXIT_FAILURE, errno, "No index number found for interface `%s'", form->name);
...
#ifdef SIOCGIFADDR
    if (ioctl (form->sfd, SIOCGIFADDR, form->ifr) < 0)
        error (EXIT_FAILURE, errno, "SIOCGIFADDR failed for interface `%s'", form->ifr->ifr_name);
    else
...
#ifdef SIOCGIFNETMASK
    if (ioctl (form->sfd, SIOCGIFNETMASK, form->ifr) < 0)
        error (EXIT_FAILURE, errno, "SIOCGIFNETMASK failed for interface `%s'", form->ifr->ifr_name);
    else
...
# ifdef SIOCGIFFLAGS
    ...
    if (0 == (f = if_nametoflag ("BROADCAST"))
        || (ioctl (form->sfd, SIOCGIFFLAGS, form->ifr) < 0) || ((f & form->ifr->ifr_flags) == 0))
    {
        select_arg (form, argc, argv, 1);
        return;
    }
...
#ifdef SIOCGIFBRDADDR
    if (ioctl (form->sfd, SIOCGIFBRDADDR, form->ifr) < 0)
        error (EXIT_FAILURE, errno, "SIOCGIFBRDADDR failed for interface `%s'", form->ifr->ifr_name);
    else
...
#ifdef SIOCGIFDSTADDR
    if (ioctl (form->sfd, SIOCGIFDSTADDR, form->ifr) < 0)
        error (EXIT_FAILURE, errno, "SIOCGIFDSTADDR failed for interface `%s'", form->ifr->ifr_name);
    else
...
#ifdef SIOCGIFMTU
    if (ioctl (form->sfd, SIOCGIFMTU, form->ifr) < 0)
        error (EXIT_FAILURE, errno, "SIOCGIFMTU failed for interface `%s'", form->ifr->ifr_name);
    else
...
#ifdef SIOCGIFMETRIC
    if (ioctl (form->sfd, SIOCGIFMETRIC, form->ifr) < 0)
        error (EXIT_FAILURE, errno, "SIOCGIFMETRIC failed for interface `%s'", form->ifr->ifr_name);
    else
...
#ifdef SIOCGIFFLAGS
    if (ioctl (form->sfd, SIOCGIFFLAGS, form->ifr) < 0)
        error (EXIT_FAILURE, errno, "SIOCGIFFLAGS failed for interface `%s'", form->ifr->ifr_name);
    else
    {
        if (argc >= 1)
        {
