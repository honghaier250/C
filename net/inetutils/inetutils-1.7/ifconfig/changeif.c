...
/* Set address of interface in IFR. Destroys union in IFR, but leaves
   ...
   can be resolved.  */
int set_address (int sfd, struct ifreq *ifr, char *address)
{
#ifndef SIOCSIFADDR
...
int set_netmask (int sfd, struct ifreq *ifr, char *netmask)
{
...
int set_dstaddr (int sfd, struct ifreq *ifr, char *dstaddr)
{
#ifndef SIOCSIFDSTADDR
    error (0, 0, "don't know how to set an interface peer address on this system");
    return -1;
#else
...
int set_brdaddr (int sfd, struct ifreq *ifr, char *brdaddr)
{
#ifndef SIOCSIFBRDADDR
    error (0, 0, "don't know how to set an interface broadcast address on this system");
    return -1;
#else
...
int set_mtu (int sfd, struct ifreq *ifr, int mtu)
{
#ifndef SIOCSIFMTU
...
int set_metric (int sfd, struct ifreq *ifr, int metric)
{
...
int configure_if (int sfd, struct ifconfig *ifp)
{
    int err = 0;
