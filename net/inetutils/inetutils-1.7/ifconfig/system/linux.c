...
static void print_hwaddr_ether (format_data_t form, unsigned char *data)
{
    *column += printf ("%02X:%02X:%02X:%02X:%02X:%02X", data[0], data[1], data[2], data[3], data[4], data[5]);
}

static void print_hwaddr_arcnet (format_data_t form, unsigned char *data)
{
    *column += printf ("%02X", data[0]);
}

static void print_hwaddr_dlci (format_data_t form, unsigned char *data)
{
    *column += printf ("%i", *((short *) data));
}

static void print_hwaddr_ax25 (format_data_t form, unsigned char *data)
{
    int i = 0;
...
static void print_hwaddr_irda (format_data_t form, unsigned char *data)
{
    *column += printf ("%02X:%02X:%02X:%02X", data[3], data[2], data[1], data[0]);
}

static void print_hwaddr_rose (format_data_t form, unsigned char *data)
{
    *column += printf ("%02X%02X%02X%02X%02X", data[0], data[1], data[2], data[3], data[4]);
}
...
} arphrd_symbols[] =
{
    ...
    {
    "IEEE802_TR", "16/4 Mbps Token Ring (New)", ARPHRD_IEEE802_TR, print_hwaddr_ether},
#endif
#ifdef ARPHRD_VOID
    {
...
struct arphrd_symbol *arphrd_findvalue (int value)
{
...
static int procnet_parse_fields_v1 (char *buf, struct pnd_stats *stats)
{
    int n = sscanf (buf,
...
static int procnet_parse_fields_v2 (char *buf, struct pnd_stats *stats)
{
    int n = sscanf (buf,
...
static int procnet_parse_fields_v3 (char *buf, struct pnd_stats *stats)
{
    int n = sscanf (buf,
...
{
procnet_parse_fields_v1, procnet_parse_fields_v2, procnet_parse_fields_v3};

static int pnd_version (char *buf)
{
...
struct pnd_stats *pnd_stats_locate (const char *name)
{
...
{
    select_arg (form, argc, argv, pnd_stats_locate (form->ifr->ifr_name) ? 0 : 1);
}
...
_IU_DECLARE (tx_fifo_errors) _IU_DECLARE (tx_heartbeat_errors) _IU_DECLARE (tx_window_errors)
...
#ifdef SIOCGIFHWADDR
    if (ioctl (form->sfd, SIOCGIFHWADDR, form->ifr) < 0)
        error (EXIT_FAILURE, errno, "SIOCGIFHWADDR failed for interface `%s'", form->ifr->ifr_name);
    else
    {
        struct arphrd_symbol *arp;
        ...
        if (arp && arp->print_hwaddr)
            arp->print_hwaddr (form, (unsigned char *) form->ifr->ifr_hwaddr.sa_data);
        else
...
#ifdef SIOCGIFTXQLEN
    if (ioctl (form->sfd, SIOCGIFTXQLEN, form->ifr) < 0)
        error (EXIT_FAILURE, errno, "SIOCGIFTXQLEN failed for interface `%s'", form->ifr->ifr_name);
    else
...
void system_parse_opt_set_txqlen (struct ifconfig *ifp, char *arg)
{
    char *end;
    ...
    if (!(ifp->valid & IF_VALID_SYSTEM))
    {
        ...
        if (!ifp->system)
            error (EXIT_FAILURE, errno, "can't get memory for system interface configuration");
        ifp->system->valid = 0;
        ifp->valid |= IF_VALID_SYSTEM;
    }
    if (ifp->system->valid & IF_VALID_TXQLEN)
        error (EXIT_FAILURE, 0, "only one txqlen allowed for interface `%s'", ifp->name);
    ifp->system->txqlen = strtol (arg, &end, 0);
    if (*arg == '\0' || *end != '\0')
        error (EXIT_FAILURE, 0, "txqlen value `%s' for interface `%s' is not a number", arg, ifp->name);
...
static error_t linux_argp_parser (int key, char *arg, struct argp_state *state)
{
...
int system_parse_opt (struct ifconfig **ifpp, char option, char *arg)
{
