...
static int handler (int code, void *closure,
                    struct sockaddr_in *dest, struct sockaddr_in *from, struct ip *ip, icmphdr_t * icmp, int datalen);
int print_echo (int dup, struct ping_stat *stat,
                struct sockaddr_in *dest, struct sockaddr_in *from, struct ip *ip, icmphdr_t * icmp, int datalen);
static int echo_finish (void);

void print_icmp_header (struct sockaddr_in *from, struct ip *ip, icmphdr_t * icmp, int len);
...
int ping_echo (char *hostname)
{
    ...
    if (options & OPT_RROUTE)
    {
        ...
        if (setsockopt (ping->ping_fd, IPPROTO_IP, IP_OPTIONS, rspace, sizeof (rspace)) < 0)
            error (EXIT_FAILURE, errno, NULL);
#else
        error (EXIT_FAILURE, 0, "record route not available in this " "implementation.");
#endif                            /* IP_OPTIONS */
    }

    printf ("PING %s (%s): %d data bytes\n",
            ping->ping_hostname, inet_ntoa (ping->ping_dest.ping_sockaddr.sin_addr), data_length);
         ...
         struct sockaddr_in *dest, struct sockaddr_in *from, struct ip *ip, icmphdr_t * icmp, int datalen)
{
    switch (code)
    {
        case PEV_RESPONSE:
        case PEV_DUPLICATE:
            print_echo (code == PEV_DUPLICATE, (struct ping_stat *) closure, dest, from, ip, icmp, datalen);
            break;
        case PEV_NOECHO:;
...
char *ipaddr2str (struct in_addr ina)
{
    struct hostent *hp;

    if (options & OPT_NUMERIC || !(hp = gethostbyaddr ((char *) &ina, 4, AF_INET)))
...
void print_icmp_header (struct sockaddr_in *from, struct ip *ip, icmphdr_t * icmp, int len)
{
    ...
    if (!(options & OPT_VERBOSE || orig_ip->ip_dst.s_addr == ping->ping_dest.ping_sockaddr.sin_addr.s_addr))
        return;
...
void print_ip_opt (struct ip *ip, int hlen)
{
    u_char *cp;
        ...
        switch (*cp)
        {
            ...
            case IPOPT_RR:
                ...
                if (i <= 0)
                    continue;
                if (i == old_rrlen
                    && cp == (u_char *) (ip + 1) + 2 && !memcmp ((char *) cp, old_rr, i) && !(options & OPT_FLOOD))
                {
                    printf ("\t (same route)");
                    i = ((i + 3) / 4) * 4;
                    hlen -= i;
