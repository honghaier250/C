...
int decode (char *name, CODE * codetab, const char *what)
{
    CODE *cp;
...
static void open_socket ()
{
    ...
    else
    {
        ...
        else if (inet_aton (host, (struct in_addr *) &sockaddr.sinet.sin_addr) != 1)
...
static void send_to_syslog (const char *msg)
{
    char *pbuf;
    ...
    if (logflags & LOG_PID)
        rc = asprintf (&pbuf, "<%d>%.15s %s[%s]: %s", pri, ctime (&now) + 4, tag, pidstr, msg);
    else
        rc = asprintf (&pbuf, "<%d>%.15s %s: %s", pri, ctime (&now) + 4, tag, msg);
    if (rc == -1)
    ...
    if (rc == -1)
        ...
        error (0, errno, "sent less bytes than expected (%lu vs. %lu)", (unsigned long) rc, (unsigned long) len);
}

