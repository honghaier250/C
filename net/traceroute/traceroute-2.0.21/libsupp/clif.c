...
static CLIF_option *find_long (char *arg, char **arg_p, unsigned int match, unsigned int nomatch)
{
    ...
    for (optn = curr.option_list; optn->short_opt || optn->long_opt; optn++)
    {
        ...
        if (*a == '\0' || (*a == '=' && optn->arg_name && !(flags & _CLIF_STRICT_NOEQUAL)))
...
int CLIF_parse_cmdline (int argc, char *argv[],
                        CLIF_option * option_list, CLIF_argument * argument_list, unsigned int parse_flags)
{
    int i, j;
    ...
    if (argument_list)
    {
        ...
        for (argm = argument_list; argm->name; argm++)
        {
            ...
            else
            {
                ...
                {
                    err_report ("Incorrect argument list set in program " "source: more than one optional area.");
                    return -1;
                }
            }
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        {                        /*  short option, or several short options...  */
            ...
            if (!*arg)
            {                    /*  POSIX say: only "stdout specification"... */
                arg--;
                goto handle_arg;
            }
      ...
      long_found:
        ...
        if (optn->arg_name && !opt_arg)
        {
            ...
            if (++i >= argc || !(flags & CLIF_MAY_NOEQUAL))
            {                    /*  missing opt arg   */
                i--;
    ...
    if (num_args > 0)
    {
        ...
        {
            if (j == 0)
                err_report ("Incorrect argument list set: first arg " "cannot be `accompanied with previous'.");
            else
        ...
        for (i = 0; i < num_args; i++)
        {
            ...
            if (argm->function && argm->function (argm, argv[arg_n[i]], i) < 0)
            {
                err_report ("Cannot handle \"%s\" cmdline arg `%s' "
                            "on position %d (argc %d)", argm->name, argv[arg_n[i]], i + 1, arg_n[i]);
                return -1;
            }
        }
...
void CLIF_print_usage (const char *header, const char *progname,
                       const CLIF_option * option_list, const CLIF_argument * argument_list)
{
