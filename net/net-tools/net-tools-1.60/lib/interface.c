    ...
    switch (procnetdev_vsn)
    {
        case 3:
            sscanf (bp,
                    ...
                    &ife->stats.collisions, &ife->stats.tx_carrier_errors, &ife->stats.tx_compressed);
            break;
        case 2:
            sscanf (bp, "%llu %llu %lu %lu %lu %lu %llu %llu %lu %lu %lu %lu %lu",
                    ...
                    &ife->stats.tx_errors,
                    &ife->stats.tx_dropped,
                    &ife->stats.tx_fifo_errors, &ife->stats.collisions, &ife->stats.tx_carrier_errors);
...
static int if_readlist_proc (char *target)
{
    ...
    fmt = proc_gen_fmt (_PATH_PROCNET_DEV, 1, fh, "face", "",    /* parsed separately */
                        ...
                        "packets", "%lu",
                        "errs", "%lu",
                        "drop", "%lu", "fifo", "%lu", "colls", "%lu", "carrier", "%lu", "compressed", "%lu", NULL);
    if (!fmt)
        return -1;
#else
...
int if_fetch (struct interface *ife)
{
    ...
    if (ife->type == ARPHRD_SLIP || ife->type == ARPHRD_CSLIP ||
        ife->type == ARPHRD_SLIP6 || ife->type == ARPHRD_CSLIP6 || ife->type == ARPHRD_ADAPT)
    {
#ifdef SIOCGOUTFILL
        strcpy (ifr.ifr_name, ifname);
...
void ife_print_short (struct interface *ptr)
{
    ...
    if (ptr->statistics_valid)
    {
        printf ("%8llu %6lu %6lu %6lu",
                ptr->stats.rx_packets, ptr->stats.rx_errors, ptr->stats.rx_dropped, ptr->stats.rx_fifo_errors);
        printf ("%8llu %6lu %6lu %6lu ",
                ptr->stats.tx_packets, ptr->stats.tx_errors, ptr->stats.tx_dropped, ptr->stats.tx_fifo_errors);
    }
    else
    {
...
void ife_print_long (struct interface *ptr)
{
    ...
    if (ptr->has_ip)
    {
        printf (_("          %s addr:%s "), ap->name, ap->sprint (&ptr->addr, 1));
        if (ptr->flags & IFF_POINTOPOINT)
        {
    ...
    if (ptr->statistics_valid)
    {
        ...
        printf (_("RX packets:%llu errors:%lu dropped:%lu overruns:%lu frame:%lu\n"),
                ...
                ptr->stats.rx_dropped, ptr->stats.rx_fifo_errors, ptr->stats.rx_frame_errors);
        if (can_compress)
        ...
        printf (_("TX packets:%llu errors:%lu dropped:%lu overruns:%lu carrier:%lu\n"),
                ...
                ptr->stats.tx_dropped, ptr->stats.tx_fifo_errors, ptr->stats.tx_carrier_errors);
        ...
        printf (_("RX bytes:%llu (%lu.%lu %s)  TX bytes:%llu (%lu.%lu %s)\n"),
                ...
                tx, (unsigned long) (short_tx / 10), (unsigned long) (short_tx % 10), Text);
    }
