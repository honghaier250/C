...
static char *X25_print (unsigned char *ptr)
{
...
static char *X25_sprint (struct sockaddr *sap, int numeric)
{
 ...
 */
static int X25_input (int type, char *bufp, struct sockaddr *sap)
{
    unsigned char *ptr;
...
static void X25_herror (char *text)
{
    if (text == NULL)
        fprintf (stderr, "%s\n", X25_errmsg);
    else
...
static int X25_hinput (char *bufp, struct sockaddr *sap)
{
    if (X25_input (0, bufp, sap) < 0)
        return (-1);
