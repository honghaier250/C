...
static int INET_resolve (char *name, struct sockaddr_in *sin, int hostfirst)
{
    ...
    {
        memcpy ((char *) &sin->sin_addr, (char *) hp->h_addr_list[0], sizeof (struct in_addr));
        return 0;
    }
...
static char *INET_sprint (struct sockaddr *sap, int numeric)
{
    ...
    if (INET_rresolve (buff, sizeof (buff), (struct sockaddr_in *) sap, numeric, 0xffffff00) != 0)
        return (NULL);
...
char *INET_sprintmask (struct sockaddr *sap, int numeric, unsigned int netmask)
{
    ...
    if (sap->sa_family == 0xFFFF || sap->sa_family == 0)
        return safe_strncpy (buff, _("[NONE SET]"), sizeof (buff));
    if (INET_rresolve (buff, sizeof (buff), (struct sockaddr_in *) sap, numeric, netmask) != 0)
        return (NULL);
    return (buff);
}
