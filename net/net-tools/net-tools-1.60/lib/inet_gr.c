...
extern char *INET_sprintmask (struct sockaddr *sap, int numeric, unsigned int netmask);
...
int rprint_fib (int ext, int numeric)
{
    ...
    fmt = proc_gen_fmt (_PATH_PROCNET_ROUTE, 0, fp,
                        ...
                        "Flags", "%X",
                        "RefCnt", "%d",
                        "Use", "%d", "Metric", "%d", "Mask", "%128s", "MTU", "%d", "Window", "%d", "IRTT", "%d", NULL);
        ...
        if (ext == 1)
        {
            ...
            if (iflags & RTF_REJECT)
                ...
                printf ("%-15s %-15s %-15s %-5s %-6d %-2d %7d %s\n",
                        net_addr, gate_addr, mask_addr, flags, metric, refcnt, use, iface);
        }
        if (ext == 2)
        {
#if HAVE_RTF_REJECT
            if (iflags & RTF_REJECT)
                ...
                printf ("%-15s %-15s %-15s %-5s %5d %-5d %6d %s\n",
                        net_addr, gate_addr, mask_addr, flags, mss, window, irtt, iface);
        }
        if (ext >= 3)
        {
#if HAVE_RTF_REJECT
            if (iflags & RTF_REJECT)
                ...
                printf ("%-15s %-15s %-15s %-5s %-6d %-3d %6d %-6.6s   %-5d %-6d %d\n",
                        net_addr, gate_addr, mask_addr, flags, metric, refcnt, use, iface, mss, window, irtt);
        }
    }
...
int rprint_cache (int ext, int numeric)
{
    ...
    format =
        proc_guess_fmt (_PATH_PROCNET_RTCACHE, fp, "IRTT", 1, "TOS", 2, "HHRef", 4, "HHUptod", 8, "SpecDst", 16, "HH",
                        32, "ARP", 64, NULL);
    ...
    if (ext == 2)
        printf (_("Source          Destination     Gateway         " "Flags   MSS Window  irtt Iface\n"));

    if (format == 1)
    {
        if (ext >= 3)
            printf (_("Source          Destination     Gateway         "
                      "Flags Metric Ref    Use Iface    " "MSS   Window irtt  HH  Arp\n"));
        ...
        fmt = proc_gen_fmt (_PATH_PROCNET_RTCACHE, 0, fp,
                            ...
                            "Metric", "%d",
                            "Source", "%128s",
                            "MTU", "%d", "Window", "%d", "IRTT", "%d", "HH", "%d", "ARP", "%d", NULL);
    ...
    if (format == 2)
    {
        if (ext >= 3)
            printf (_("Source          Destination     Gateway         "
                      "Flags Metric Ref    Use Iface    " "MSS   Window irtt  TOS HHRef HHUptod     SpecDst\n"));
        fmt = proc_gen_fmt (_PATH_PROCNET_RTCACHE, 0, fp,
                            "Iface", "%16s",
                            "Destination", "%128s",
                            "Gateway", "%128s",
                            ...
                            "MTU", "%d",
                            "Window", "%d",
                            "IRTT", "%d", "TOS", "%d", "HHRef", "%d", "HHUptod", "%d", "SpecDst", "%128s", NULL);
    ...
    while (fgets (buff, 1023, fp))
    {
        if (format == 1)
        {
            num = sscanf (buff, fmt,
                          ...
                          &iflags, &refcnt, &use, &metric, src_addr, &mss, &window, &irtt, &hh, &arp);
            if (num < 12)
                continue;
        }
        if (format == 2)
...
/* possible collision with 2.0 flags U and H */
        ...
        {
            printf ("%-15s %-15s %-15s %-5s %-6d %-2d %7d %s\n",
                    src_addr, dest_addr, gate_addr, flags, metric, refcnt, use, iface);
        }
        if (ext == 2)
        {
            printf ("%-15s %-15s %-15s %-5s %5d %-5d %6d %s\n",
                    src_addr, dest_addr, gate_addr, flags, mss, window, irtt, iface);
        }
        if (format == 1)
        {
            if (ext >= 3)
            {
                printf ("%-15s %-15s %-15s %-5s %-6d %-3d %6d %-6.6s   %-5d %-6d %-5d %-3d %d\n",
                        src_addr, dest_addr, gate_addr, flags, metric, refcnt, use, iface, mss, window, irtt, hh, arp);
            }
        }
        if (format == 2)
        {
