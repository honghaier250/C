...
static void print_masq (struct masq *ms, int numeric_host, int numeric_port, int ext)
{
    ...
    {
        if (has_pdelta)
            printf ("%10lu %5hd %5hd ", ms->initseq, ms->delta, ms->pdelta);
        else
            printf ("%10lu %5hd     - ", ms->initseq, ms->delta);
    }
    ...
    for (nread = 0; nread < nmslist; nread++)
    {
        ...
        if (has_pdelta)
        {
            if ((n = fscanf (f, " %s %lX:%hX %lX:%hX %hX %lX %hd %hd %lu",
                             ...
                             &ms->mport, &ms->initseq, &ms->delta, &ms->pdelta, &ms->expires)) == -1)
                return nread;
        }
        else
        {
            if ((n = fscanf (f, " %s %lX:%hX %lX:%hX %hX %lX %hd %lu",
                             ...
                             (unsigned long *) &ms->src.sin_addr.s_addr, &ms->sport,
                             (unsigned long *) &ms->dst.sin_addr.s_addr, &ms->dport,
                             &ms->mport, &ms->initseq, &ms->delta, &ms->expires)) == -1)
                return nread;
        }
...
int ip_masq_info (int numeric_host, int numeric_port, int ext)
{
    ...
    {
        ntotal += nread;
        mslist = (struct masq *) realloc (mslist, (ntotal + 16) * sizeof (struct masq));
        if (!mslist)
        {
