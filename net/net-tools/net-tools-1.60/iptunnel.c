...
char *Release = RELEASE, *Version = "iptunnel 1.01", *Signature = "Alexey Kuznetsov, <kuznet@ms2.inr.ac.ru>";
...
static int do_tunnels_list (struct ip_tunnel_parm *p)
{
    ...
    unsigned long rx_bytes, rx_packets, rx_errs, rx_drops,
        rx_fifo, rx_frame, tx_bytes, tx_packets, tx_errs, tx_drops, tx_fifo, tx_colls, tx_carrier, rx_multi;
    int type;
    ...
    while (fgets (buf, sizeof (buf), fp) != NULL)
    {
        ...
        if ((p->link && p1.link != p->link) ||
            ...
            (p->iph.saddr && p1.iph.saddr != p->iph.saddr) || (p->i_key && p1.i_key != p->i_key))
            continue;
        print_tunnel (&p1);
        if (show_stats)
        {
...
int do_iptunnel (int argc, char **argv)
{
    if (argc > 0)
    {
        ...
        if (matches (*argv, "show") == 0 || matches (*argv, "lst") == 0 || matches (*argv, "list") == 0)
...
int main (int argc, char **argv)
{
    ...
    while (argc > 1)
    {
        ...
        else if (matches (argv[1], "-stats") == 0 || matches (argv[1], "-statistics") == 0)
        {
            ++show_stats;
        }
