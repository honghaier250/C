...
static char version[] = "mii-tool.c 1.9 2000/04/28 00:56:08 (David Hinds)\n";
...
static unsigned int verbose = 0, opt_version = 0, opt_restart = 0, opt_reset = 0, opt_log = 0, opt_watch = 0;
...
static int parse_media (char *arg)
{
    ...
    if ((*arg != '\0') && (*s == '\0'))
    {
        if ((mask & MII_AN_ABILITY_MASK) && !(mask & ~MII_AN_ABILITY_MASK))
            return mask;
        goto failed;
    }
    else
    {
        ...
        {
            for (i = 0; i < NMEDIA; i++)
                if (strcasecmp (media[i].name, s) == 0)
                    break;
            if (i == NMEDIA)
                goto failed;
            mask |= media[i].value;
    ...
    if (verbose)
    {
        ...
        if (i < NMII)
            ...
            printf ("vendor %02x:%02x:%02x, model %d rev %d\n",
                    ...
                    ((mii_val[2] << 6) | (mii_val[3] >> 10)) & 0xff, (mii_val[3] >> 4) & 0x3f, mii_val[3] & 0x0f);
        printf ("  basic mode:   ");
...
static void watch_one_xcvr (int skfd, char *ifname, int index)
{
    ...
    now = (mdio_read (skfd, MII_BMCR) | (mdio_read (skfd, MII_BMSR) << 16));
...
int main (int argc, char **argv)
{
    ...
    while ((c = getopt_long (argc, argv, "A:F:p:lrRvVw?", longopts, 0)) != EOF)
        ...
        {
            case 'A':
                nway_advertise = parse_media (optarg);
                break;
            case 'F':
                fixed_speed = parse_media (optarg);
                break;
            case 'p':
    ...
    if (opt_watch)
        verbose = 0;
    if (errflag || (fixed_speed & (fixed_speed - 1)) || (fixed_speed && (opt_restart || nway_advertise)))
    {
