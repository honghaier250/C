...
const char *Release = RELEASE,
    *Version = "@(#) slattach 1.21 (1999-11-21)", *Signature = "net-tools, Fred N. van Kempen et al.";
...
static int tty_lock (char *path, int mode)
{
    ...
    if ((opt_l == 0) || ((path == NULL) && (saved_lock == 0)))
        return (0);
    ...
    {                            /* lock */
        ...
        if ((fd = creat (saved_path, 0644)) < 0)
        {
            if (errno != EEXIST)
                if (opt_q == 0)
                    fprintf (stderr, _("slattach: tty_lock: (%s): %s\n"), saved_path, strerror (errno));
            return (-1);
        }
        sprintf (apid, "%10d\n", getpid ());
        ...
        {
            if (opt_q == 0)
                fprintf (stderr, _("slattach: tty_lock: UUCP user %s unknown!\n"), _UID_UUCP);
    ...
    {                            /* unlock */
        ...
        {
            if (opt_q == 0)
                fprintf (stderr, "slattach: tty_unlock: (%s): %s\n", saved_path, strerror (errno));
            return (-1);
        }
        saved_lock = 0;
    }
...
static int tty_get_state (struct termios *tty)
{
    ...
    {
        if (opt_q == 0)
            fprintf (stderr, "slattach: tty_get_state: %s\n", strerror (errno));
        return (-errno);
    }
    return (0);
}
...
static int tty_set_state (struct termios *tty)
{
    ...
    {
        if (opt_q == 0)
            fprintf (stderr, "slattach: tty_set_state: %s\n", strerror (errno));
        return (-errno);
    }
    return (0);
}
...
static int tty_get_disc (int *disc)
{
    ...
    {
        if (opt_q == 0)
            fprintf (stderr, "slattach: tty_get_disc: %s\n", strerror (errno));
        return (-errno);
    }
    return (0);
}
...
static int tty_set_disc (int disc)
{
    ...
    {
        if (opt_q == 0)
            fprintf (stderr, "slattach: tty_set_disc(%d, %d): %s\n", tty_fd, disc, strerror (errno));
        return (-errno);
    }
    return (0);
}
...
static int tty_hangup (void)
{
    ...
    {
        if (opt_q == 0)
            fprintf (stderr, _("slattach: tty_hangup(DROP): %s\n"), strerror (errno));
        return (-errno);
    }
    ...
    {
        if (opt_q == 0)
            fprintf (stderr, _("slattach: tty_hangup(RAISE): %s\n"), strerror (errno));
        return (-errno);
    }
    return (0);
}
...
static int tty_open (char *name, const char *speed)
{
    ...
    if (name != NULL)
    {
        if (name[0] != '/')
        {
            ...
            {
                if (opt_q == 0)
                    fprintf (stderr, _("slattach: tty name too long\n"));
                return (-1);
            }
        ...
        {
            if (opt_q == 0)
                fprintf (stderr, "slattach: tty_open(%s, RW): %s\n", path_open, strerror (errno));
            return (-errno);
        }
        tty_fd = fd;
        if (opt_d)
            printf ("slattach: tty_open: %s (fd=%d) ", path_open, fd);
    }
    else
    {
        tty_fd = 0;
    ...
    {
        if (opt_q == 0)
            fprintf (stderr, _("slattach: tty_open: cannot get current state!\n"));
        return (-errno);
    }
    ...
    {
        if (opt_q == 0)
            fprintf (stderr, _("slattach: tty_open: cannot get current line disc!\n"));
        return (-errno);
    }
    tty_ldisc = tty_sdisc;
    ...
    if (opt_m == 0)
    {
        ...
        {
            if (opt_q == 0)
                fprintf (stderr, _("slattach: tty_open: cannot set RAW mode!\n"));
            return (-errno);
        }
        ...
        if (speed != NULL)
        {
            ...
            {
                if (opt_q == 0)
                    fprintf (stderr, _("slattach: tty_open: cannot set %s bps!\n"), speed);
                return (-errno);
            }
        }
        ...
        if (tty_set_databits (&tty_current, "8") ||
            tty_set_stopbits (&tty_current, "1") || tty_set_parity (&tty_current, "N"))
        {
            if (opt_q == 0)
                fprintf (stderr, _("slattach: tty_open: cannot set 8N1 mode!\n"));
            return (-errno);
        }
...
int main (int argc, char *argv[])
{
    ...
    opterr = 0;
    while ((s = getopt_long (argc, argv, "c:ehlLmnp:qs:vdVk:o:", longopts, NULL)) != EOF)
        switch (s)
        {
            case 'c':
                extcmd = optarg;
    ...
    {
        if (opt_q == 0)
            fprintf (stderr, _("slattach: setvbuf(stdout,0,_IOLBF,0) : %s\n"), strerror (errno));
        exit (1);
    }
