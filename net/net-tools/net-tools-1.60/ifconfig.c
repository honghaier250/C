...
static int get_nmbc_parent (char *parent, unsigned long *nm, unsigned long *bc);

static int set_ifstate (char *parent, unsigned long ip, unsigned long nm, unsigned long bc, int flag);
...
#endif
                default:
                    fprintf (stderr, _("Don't know how to set addresses for family %d.\n"), ap->af);
                    exit (1);
            }
            if (r < 0)
            {
