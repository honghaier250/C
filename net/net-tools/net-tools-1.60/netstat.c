...
static int netrom_info (void)
{
    ...
    while (fgets (buffer, 256, f))
    {
        ...
        ret =
            sscanf (buffer + 30,
                    "%s %*x/%*x %*x/%*x %d %d %d %*d %*d/%*d %*d/%*d %*d/%*d %*d/%*d %*d/%*d %*d %d %d %*d", dev, &st,
                    &vs, &vr, &sendq, &recvq);
        if (ret != 6)
        {
        ...
        printf ("%-9s  %-9s  %-9s  %-6s  %-11s  %03d/%03d  %-6d  %-6d\n",
                buffer, buffer + 10, buffer + 20, dev, _(netrom_state[st]), vr, vs, sendq, recvq);
    }
    fclose (f);
    return 0;
}
...
static void igmp_do_one (int lnr, const char *line)
{
    ...
    {                            /* IPV6 */
        ...
        if (num == 4)
        {
            ...
            sscanf (mcast_addr, "%08X%08X%08X%08X",
                    &in6.s6_addr32[0], &in6.s6_addr32[1], &in6.s6_addr32[2], &in6.s6_addr32[3]);
        ...
        safe_strncpy (mcast_addr, ap->sprint ((struct sockaddr *) &mcastaddr, flag_not), sizeof (mcast_addr));
    ...
    {                            /* IPV4 */
        ...
        else if (line[0] == '\t')
        {
            ...
            sscanf (mcast_addr, "%X", &((struct sockaddr_in *) &mcastaddr)->sin_addr.s_addr);
...
static int x25_info (void)
{
    ...
    while (fgets (buffer, 256, f))
    {
        ...
        printf ("%-15s %-15s %-7s %-3d  %-11s  %02d/%02d  %-6d  %-6d\n",
                buffer, buffer + 11, dev, lci, x25_state[st], vr, vs, sendq, recvq);
    }
    fclose (f);
    return 0;
}
...
{
    INFO_GUTS6 (_PATH_PROCNET_IGMP, _PATH_PROCNET_IGMP6, "AF INET (igmp)", igmp_do_one);
}
...
static void tcp_do_one (int lnr, const char *line)
{
    ...
    if (flag_all || (flag_lst && !rem_port) || (!flag_lst && rem_port))
    {
        ...
        snprintf (buffer, sizeof (buffer), "%s", get_sname (htons (rem_port), "tcp", flag_not & FLAG_NUM_PORT));
        ...
        if (flag_opt)
            switch (timer_run)
            {
                ...
                case 1:
                    snprintf (timers, sizeof (timers), _("on (%2.2f/%ld/%d)"), (double) time_len / HZ, retr, timeout);
                    break;
        ...
        printf ("tcp   %6ld %6ld %-23s %-23s %-12s", rxq, txq, local_addr, rem_addr, _(tcp_state[state]));
...
static void udp_do_one (int lnr, const char *line)
{
    ...
    num = sscanf (line,
                  ...
                  rem_addr, &rem_port, &state, &txq, &rxq, &timer_run, &time_len, &retr, &uid, &timeout, &inode, more);
...
#define notnull(A) (A.sin_addr.s_addr)
    ...
    {
        safe_strncpy (local_addr, ap->sprint ((struct sockaddr *) &localaddr, flag_not), sizeof (local_addr));
        snprintf (buffer, sizeof (buffer), "%s", get_sname (htons (local_port), "udp", flag_not & FLAG_NUM_PORT));
        if ((strlen (local_addr) + strlen (buffer)) > 22)
            local_addr[22 - strlen (buffer)] = '\0';
        strcat (local_addr, ":");
        strcat (local_addr, buffer);

        snprintf (buffer, sizeof (buffer), "%s", get_sname (htons (rem_port), "udp", flag_not & FLAG_NUM_PORT));
        safe_strncpy (rem_addr, ap->sprint ((struct sockaddr *) &remaddr, flag_not), sizeof (rem_addr));
        if ((strlen (rem_addr) + strlen (buffer)) > 22)
            rem_addr[22 - strlen (buffer)] = '\0';
        strcat (rem_addr, ":");
        ...
        printf ("udp   %6ld %6ld %-23s %-23s %-12s", rxq, txq, local_addr, rem_addr, udp_state);
...
static void raw_do_one (int lnr, const char *line)
{
    ...
    if (flag_all || (notnull (remaddr) && !flag_lst) || (!notnull (remaddr) && flag_lst))
    {
        snprintf (buffer, sizeof (buffer), "%s", get_sname (htons (local_port), "raw", flag_not & FLAG_NUM_PORT));
        safe_strncpy (local_addr, ap->sprint ((struct sockaddr *) &localaddr, flag_not), sizeof (local_addr));
        if ((strlen (local_addr) + strlen (buffer)) > 22)
            local_addr[22 - strlen (buffer)] = '\0';
        strcat (local_addr, ":");
        ...
        snprintf (buffer, sizeof (buffer), "%s", get_sname (htons (rem_port), "raw", flag_not & FLAG_NUM_PORT));
        safe_strncpy (rem_addr, ap->sprint ((struct sockaddr *) &remaddr, flag_not), sizeof (rem_addr));
        if ((strlen (rem_addr) + strlen (buffer)) > 22)
            rem_addr[22 - strlen (buffer)] = '\0';
        strcat (rem_addr, ":");
        ...
        if (flag_opt)
            switch (timer_run)
            {
                ...
                default:
                    snprintf (timers, sizeof (timers), _("unkn-%d (%2.2f/%ld/%d)"),
                              timer_run, (double) time_len / 100, retr, timeout);
                    break;
            }
        printf ("raw   %6ld %6ld %-23s %-23s %-12d", rxq, txq, local_addr, rem_addr, state);
...
{
    INFO_GUTS6 (_PATH_PROCNET_RAW, _PATH_PROCNET_RAW6, "AF INET (raw)", raw_do_one);
}
...
static void unix_do_one (int nr, const char *line)
{
    ...
    path[0] = '\0';
    num = sscanf (line, "%p: %lX %lX %lX %X %X %d %s", &d, &refcnt, &proto, &flags, &type, &state, &inode, path);
    if (num < 6)
    {
    ...
    printf ("%-5s %-6ld %-11s %-10s %-13s ", ss_proto, refcnt, ss_flags, ss_type, ss_state);
    if (has & HAS_INODE)
        printf ("%-6d ", inode);
    else
        printf ("-      ");
...
static int ax25_info (void)
{
    ...
    while (fgets (buffer, 256, f))
    {
        ...
        printf ("%-9s  %-9s  %-6s  %-11s  %03d/%03d  %-6d  %-6d\n",
                dst, src, dev, _(ax25_state[st]), vr, vs, sendq, recvq);
    }
    fclose (f);
    return 0;
}
...
static int ipx_info (void)
{
    ...
    {
        sscanf (buf, "%s %s %lX %lX %d %d", sad, dad, &txq, &rxq, &state, &uid);
        if ((st = rindex (sad, ':')))
        {
            *st++ = '\0';
...
int main (int argc, char *argv[])
{
    ...
    flag_arg = flag_tcp + flag_udp + flag_raw + flag_unx + flag_ipx + flag_ax25 + flag_netrom + flag_igmp + flag_x25;
    ...
    if (flag_mas)
    {
        ...
        for (;;)
        {
            i = ip_masq_info (flag_not & FLAG_NUM_HOST, flag_not & FLAG_NUM_PORT, flag_exp);
            if (i || !flag_cnt)
                break;
            sleep (1);
        }
