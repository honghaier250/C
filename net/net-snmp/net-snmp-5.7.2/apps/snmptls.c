 ...
 *
 */
netsnmp_variable_list *cert_row_create (uint32_t priority, int hash_type, const char *fp,
                                        ...
                                        int data_len, uint32_t st, int *row_status_index)
{
    ...
    vl = snmp_varlist_add_variable (&vl, name, name_len, ASN_OCTET_STR, &bin_fp, bin_fp_len);
    netsnmp_require_ptr_LRV (vl, NULL);

    if (map_type_len && map_type)
    {
        name[col_pos] = COL_SNMPTLSTMCERTTOTSN_MAPTYPE;
        vb = snmp_varlist_add_variable (&vl, name, name_len, ASN_OBJECT_ID, map_type, map_type_len * sizeof (oid));
        if (NULL == vb)
        {
    ...
    if (data)
    {
        ...
        vb = snmp_varlist_add_variable (&vl, name, name_len, ASN_OCTET_STR, data, data_len);
        if (NULL == vb)
        {
            snmp_free_varbind (vl);
            return NULL;
    ...
    if (st)
    {
        ...
        vb = snmp_varlist_add_variable (&vl, name, name_len, ASN_INTEGER, &st, sizeof (st));
        if (NULL == vb)
        {
            snmp_free_varbind (vl);
            return NULL;
    ...
    rs = RS_CREATEANDGO;
    vb = snmp_varlist_add_variable (&vl, name, name_len, ASN_INTEGER, &rs, sizeof (rs));
    if (NULL == vb)
    {
        snmp_free_varbind (vl);
        return NULL;
 ...
 *
 */
netsnmp_variable_list *params_row_create (const char *param_name, int hash_type, const char *fp,
 ...
 *
 */
netsnmp_variable_list *addr_row_create (const char *target_name, int hash_type, const char *fp,
                                        const char *identity, uint32_t st, int *row_status_index)
{
    ...
    if (identity)
    {
        ...
        vl = snmp_varlist_add_variable (&vl, name, name_len, ASN_OCTET_STR, identity, strlen (identity));
        netsnmp_require_ptr_LRV (vl, NULL);
    }
    else
        --rs_index;
...
void _parse_storage_type (const char *arg)
{
    netsnmp_pdu dummy;
...
int main (int argc, char **argv)
{
    ...
    if (strcmp (argv[arg], "certToSecName") == 0)
    {
        ...
        DEBUGMSGT (("snmptls", "create pri %d, hash type %d, fp %s", pri, hash_type, fingerprint));
        if (_map_type_str)
        {
            map_type_len = MAX_OID_LEN;
            if (snmp_parse_oid (_map_type_str, map_type, &map_type_len) == NULL)
            {
        ...
        var_list = cert_row_create (pri, hash_type, fingerprint, map_type,
                                    map_type_len, (u_char *) _data, _data_len, _storage_type, &rs_idx);
    }
    else if (strcmp (argv[arg], "targetParamsFingerprint") == 0)
    {
        ...
        DEBUGMSGT (("snmptls", "create %s param fp, hash type %d, fp %s\n", params_name, hash_type, fingerprint));

        var_list = params_row_create (params_name, hash_type, fingerprint, _storage_type, &rs_idx);
    }
    ...
    else if (strcmp (argv[arg], "targetAddr") == 0)
    {
        ...
        var_list = addr_row_create (addr_name, _hash_type, _fp_str, _id_str, _storage_type, &rs_idx);
    }
