...
int main (int argc, char **argv)
{
    ...
    size_t oldKu_len = SNMP_MAXBUF_SMALL,
        ...
        oldkul_len = SNMP_MAXBUF_SMALL, newkul_len = SNMP_MAXBUF_SMALL, keychange_len = SNMP_MAXBUF_SMALL;

    char *s = NULL;

    u_char oldKu[SNMP_MAXBUF_SMALL],
        newKu[SNMP_MAXBUF_SMALL], oldkul[SNMP_MAXBUF_SMALL], newkul[SNMP_MAXBUF_SMALL], keychange[SNMP_MAXBUF_SMALL];
    ...
    local_passphrase_filename = (char *) malloc (sizeof (PASSPHRASE_DIR) + sizeof (PASSPHRASE_FILE) + 4);
    if (!local_passphrase_filename)
    {
        fprintf (stderr, "%s: out of memory!", local_progname);
        exit (-1);
    }
    sprintf (local_passphrase_filename, "%s/%s", PASSPHRASE_DIR, PASSPHRASE_FILE);
...
#ifndef NETSNMP_DISABLE_MD5
                 (transform_type == usmHMACMD5AuthProtocol) ? "usmHMACMD5AuthProtocol" :
#endif
    ...
    {
        engineid_len = hex_to_binary2 (engineid + 2, strlen ((char *) engineid) - 2, (char **) &engineid);
    ...
    rval = generate_Ku (transform_type, USM_LENGTH_OID_TRANSFORM,
                        (u_char *) oldpass, strlen (oldpass), oldKu, &oldKu_len);
    QUITFUN (rval, main_quit);


    rval = generate_Ku (transform_type, USM_LENGTH_OID_TRANSFORM,
                        (u_char *) newpass, strlen (newpass), newKu, &newKu_len);
    QUITFUN (rval, main_quit);
    ...
    DEBUGMSGTL (("encode_keychange", "old Ku (%lu) (from %s): ", (unsigned long) oldKu_len, oldpass));
    ...
    rval = generate_kul (transform_type, USM_LENGTH_OID_TRANSFORM,
                         engineid, engineid_len, oldKu, oldKu_len, oldkul, &oldkul_len);
    QUITFUN (rval, main_quit);


    DEBUGMSGTL (("encode_keychange", "generating old Kul (%lu) (from Ku): ", (unsigned long) oldkul_len));
    for (i = 0; i < (int) oldkul_len; i++)
        DEBUGMSGTL (("encode_keychange", "%02x", (int) (oldkul[i])));
    DEBUGMSGTL (("encode_keychange", "\n"));

    rval = generate_kul (transform_type, USM_LENGTH_OID_TRANSFORM,
                         engineid, engineid_len, newKu, newKu_len, newkul, &newkul_len);
    QUITFUN (rval, main_quit);

    DEBUGMSGTL (("encode_keychange", "generating new Kul (%lu) (from Ku): ", (unsigned long) oldkul_len));
...
void usage_to_file (FILE * ofp)
{
    ...
    c) stdin  -or-  User input from the terminal.\n\n%s\
        " NL, "Only -t is mandatory.  The transform is used to convert P=>Ku, convert\n\
...
int get_user_passphrases (void)
{
  ...
  get_user_passphrases_prompt:
    ...
    if (!oldpass)
    {
        oldpass = obuf = snmp_getpassphrase ((promptindicator) ? "Old passphrase: " : "", visible);
    }
    if (!newpass)
    {
        newpass = nbuf = snmp_getpassphrase ((promptindicator) ? "New passphrase: " : "", visible);
    }
