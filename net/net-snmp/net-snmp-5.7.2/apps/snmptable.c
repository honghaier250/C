...
int main (int argc, char *argv[])
{
    ...
    localdebug = netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DUMP_PACKET);
...
void print_table (void)
{
    ...
    {
        if (column_width != 0)
            sprintf (string_buf, "%%%s%d.%ds", left_justify_flag, column_width + 1, column_width);
        else if (field_separator == NULL)
            sprintf (string_buf, "%%%s%ds", left_justify_flag, column[field].width + 1);
...
void get_field_names (void)
{
    ...
    if (sprint_realloc_objid ((u_char **) & buf, &buf_len, &out_len, 1, root, rootlen - 1))
    {
        table_name = buf;
        buf = NULL;
        buf_len = out_len = 0;
            ...
            if (response->errstat == SNMP_ERR_NOERROR)
            {
                ...
                if (entries >= allocated)
                {
                    ...
                    else
                    {
                        ...
                        memset (data + entries * fields, 0, (allocated - entries) * fields * sizeof (char *));
                        if (show_index)
                ...
                for (vars = response->variables; vars; vars = vars->next_variable)
                {
                    ...
                    if ((vars->name_length < name_length) ||
                        ...
                        memcmp (name, vars->name, name_length * sizeof (oid)) != 0 || vars->type == SNMP_ENDOFMIBVIEW)
                    {
                        /*
                    ...
                    if (!have_current_index)
                    {
                        ...
                        if (!sprint_realloc_objid
                            ((u_char **) & buf, &buf_len, &out_len, 1, vars->name, vars->name_length))
                        {
                            break;
                        }
                        ...
                        {
                            if (netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_EXTENDED_INDEX))
                            {
                                name_p = strchr (buf, '[');
                            }
                            else
                            {
                                switch (netsnmp_ds_get_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_OID_OUTPUT_FORMAT))
                                {
    ...
    while (running)
    {
        ...
        if (status == STAT_SUCCESS)
        {
            if (response->errstat == SNMP_ERR_NOERROR)
            {
                ...
                {
                    out_len = 0;
                    sprint_realloc_objid ((u_char **) & buf, &buf_len, &out_len, 1, vars->name, vars->name_length);
