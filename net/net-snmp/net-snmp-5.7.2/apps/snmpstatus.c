...
int main (int argc, char *argv[])
{
  ...
  retry:
        ...
        if (response->errstat == SNMP_ERR_NOERROR)
        {
            for (vars = response->variables; vars; vars = vars->next_variable)
            {
                ...
                if (vars->name_length == length_sysUpTime &&
                    !memcmp (objid_sysUpTime, vars->name, sizeof (objid_sysUpTime)) && vars->val.integer)
                {
                    uptime = *vars->val.integer;
                }
                if (vars->name_length == length_ipInReceives &&
                    !memcmp (objid_ipInReceives, vars->name, sizeof (objid_ipInReceives)) && vars->val.integer)
                {
                    ipin = *vars->val.integer;
                }
                if (vars->name_length == length_ipOutRequests &&
                    !memcmp (objid_ipOutRequests, vars->name, sizeof (objid_ipOutRequests)) && vars->val.integer)
                {
    ...
    if (transport != NULL && transport->f_fmtaddr != NULL)
    {
        ...
        if (addr_string != NULL)
        {
            printf ("[%s]=>[%s] Up: %s\n", addr_string, sysdescr, uptime_string (uptime, buf));
            free (addr_string);
        }
    }
    else
                ...
                for (vars = response->variables; vars; vars = vars->next_variable)
                {
                             ...
                             && !memcmp (objid_ifInNUCastPkts, vars->name, sizeof (objid_ifInNUCastPkts)))
                    {
                        ...
                        good_var++;
                             ...
                             && !memcmp (objid_ifOutUCastPkts, vars->name, sizeof (objid_ifOutUCastPkts)))
                    {
                        ...
                        good_var++;
                             ...
                             && !memcmp (objid_ifOutNUCastPkts, vars->name, sizeof (objid_ifOutNUCastPkts)))
                    {
                        opackets += *vars->val.integer;
                        snmp_add_null_var (pdu, vars->name, vars->name_length);
                        good_var++;
