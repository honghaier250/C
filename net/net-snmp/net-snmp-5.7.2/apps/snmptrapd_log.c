...
realloc_output_temp_bfr (u_char ** buf, size_t * buf_len, size_t * out_len,
                         int allow_realloc, u_char ** temp_buf, options_type * options)
     /*
      * Function:
      ...
      */
{
    ...
    if (options->precision != UNDEF_PRECISION && temp_to_write > (size_t) options->precision)
    {
    ...
    if ((!options->left_justify) && (temp_to_write < options->width))
    {
        ...
        for (char_to_write = options->width - temp_to_write; char_to_write > 0; char_to_write--)
        {
...
realloc_handle_time_fmt (u_char ** buf, size_t * buf_len, size_t * out_len,
                         int allow_realloc, options_type * options, netsnmp_pdu * pdu)
     /*
      * Function:
      ...
      */
{
    ...
    else if (fmt_cmd == CHR_UP_TIME)
    {
        ...
        {
            case 0:
                sprintf (safe_bfr, "%u:%02u:%02u.%02u", hours, minutes, seconds, centisecs);
                break;
            case 1:
                sprintf (safe_bfr, "1 day, %u:%02u:%02u.%02u", hours, minutes, seconds, centisecs);
                break;
            default:
                sprintf (safe_bfr, "%u days, %u:%02u:%02u.%02u", days, hours, minutes, seconds, centisecs);
        }
    }
    else
    {
     ...
     */
    return realloc_output_temp_bfr (buf, buf_len, out_len, allow_realloc, (u_char **) & safe_bfr, options);
}


static int
realloc_handle_ip_fmt (u_char ** buf, size_t * buf_len, size_t * out_len,
                       int allow_realloc, options_type * options, netsnmp_pdu * pdu, netsnmp_transport * transport)
     /*
      * Function:
      ...
      */
{
    ...
    switch (fmt_cmd)
    {
        case CHR_AGENT_IP:
             ...
             */
            if (!snmp_strcat (&temp_buf, &temp_buf_len, &temp_out_len, 1, (u_char *) inet_ntoa (*agent_inaddr)))
            {
                if (temp_buf != NULL)
                {
                    free (temp_buf);
        ...
        case CHR_AGENT_NAME:
             ...
             */
            if (!netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_APP_NUMERIC_IP))
            {
                host = netsnmp_gethostbyaddr ((char *) pdu->agent_addr, 4, AF_INET);
            }
            if (host != NULL)
            {
                if (!snmp_strcat (&temp_buf, &temp_buf_len, &temp_out_len, 1, (const u_char *) host->h_name))
                {
                    if (temp_buf != NULL)
                    {
                        free (temp_buf);
        ...
        case CHR_PDU_IP:
            ...
            if (transport != NULL && transport->f_fmtaddr != NULL)
            {
                ...
                if (!tstr)
                    goto noip;
                if (!snmp_strcat (&temp_buf, &temp_buf_len, &temp_out_len, 1, (u_char *) tstr))
                {
                    SNMP_FREE (temp_buf);
                    SNMP_FREE (tstr);
                    return 0;
            ...
            {
              noip:
                if (!snmp_strcat (&temp_buf, &temp_buf_len, &temp_out_len, 1, (const u_char *) "<UNKNOWN>"))
                {
                    SNMP_FREE (temp_buf);
                    return 0;
                }
     ...
     * Output with correct justification, leading zeroes, etc.  
     */
    return realloc_output_temp_bfr (buf, buf_len, out_len, allow_realloc, &temp_buf, options);
}
    ...
    switch (fmt_cmd)
    {
        case CHR_PDU_ENT:
            ...
            if (!sprint_realloc_objid
                (&temp_buf, &temp_buf_len, &temp_out_len, 1, pdu->enterprise, pdu->enterprise_length))
            {
                free (temp_buf);
                return 0;
            }
        ...
        case CHR_TRAP_CONTEXTID:
            ...
            if (!sprint_realloc_hexstring
                (&temp_buf, &temp_buf_len, &temp_out_len, 1, pdu->contextEngineID, pdu->contextEngineIDLen))
            {
                free (temp_buf);
                return 0;
            }
    ...
    switch (fmt_cmd)
    {
        ...
        case CHR_TRAP_STYPE:
            ...
            else
            {
                 ...
                 */
                if (!sprint_realloc_objid (&obuf, &obuf_len, &oout_len, 1, trap_oid, trap_oid_len))
                {
                    if (obuf != NULL)
                    {
                        free (obuf);
        ...
        case CHR_TRAP_VARS:
            ...
            for (vars = pdu->variables; vars != NULL; vars = vars->next_variable)
            {
                ...
                if (!sprint_realloc_variable (&temp_buf, &tbuf_len, &tout_len, 1, vars->name, vars->name_length, vars))
                {
                    if (temp_buf != NULL)
                    {
                        free (temp_buf);
...
static int
realloc_handle_wrap_fmt (u_char ** buf, size_t * buf_len, size_t * out_len, int allow_realloc, netsnmp_pdu * pdu)
{
    size_t i = 0;
...
realloc_dispatch_format_cmd (u_char ** buf, size_t * buf_len,
                             ...
                             options_type * options, netsnmp_pdu * pdu, netsnmp_transport * transport)
     /*
      * Function:
      ...
      */
{
    ...
    {
        return realloc_handle_time_fmt (buf, buf_len, out_len, allow_realloc, options, pdu);
    }
    else if (is_agent_cmd (fmt_cmd) || is_pdu_ip_cmd (fmt_cmd))
    {
        return realloc_handle_ip_fmt (buf, buf_len, out_len, allow_realloc, options, pdu, transport);
    }
    else if (is_trap_cmd (fmt_cmd))
    {
        return realloc_handle_trap_fmt (buf, buf_len, out_len, allow_realloc, options, pdu);
    }
    else if (is_auth_cmd (fmt_cmd))
    {
        return realloc_handle_auth_fmt (buf, buf_len, out_len, allow_realloc, options, pdu);
    }
    else if (fmt_cmd == CHR_PDU_ENT || fmt_cmd == CHR_TRAP_CONTEXTID)
    {
        return realloc_handle_ent_fmt (buf, buf_len, out_len, allow_realloc, options, pdu);
    }
    else if (fmt_cmd == CHR_PDU_WRAP)
    {
        return realloc_handle_wrap_fmt (buf, buf_len, out_len, allow_realloc, pdu);
    }
    else
    {
        /*
...
realloc_format_plain_trap (u_char ** buf, size_t * buf_len,
                           size_t * out_len, int allow_realloc, netsnmp_pdu * pdu, netsnmp_transport * transport)
     /*
      * Function:
      ...
      */
{
    ...
    sprintf (safe_bfr, "%.4d-%.2d-%.2d %.2d:%.2d:%.2d ",
             ...
             now_parsed->tm_mday, now_parsed->tm_hour, now_parsed->tm_min, now_parsed->tm_sec);
     ...
     */
    if (!sprint_realloc_objid (buf, buf_len, out_len, allow_realloc, pdu->enterprise, pdu->enterprise_length))
    {
        return 0;
    }
    ...
    if (!snmp_strcat (buf, buf_len, out_len, allow_realloc, (const u_char *) trap_description (pdu->trap_type)))
    {
        return 0;
    }
    if (!snmp_strcat (buf, buf_len, out_len, allow_realloc, (const u_char *) " Trap ("))
    ...
    if (!snmp_strcat (buf, buf_len, out_len, allow_realloc, (const u_char *) uptime_string (pdu->time, safe_bfr)))
    {
        return 0;
    }
    if (!snmp_strcat (buf, buf_len, out_len, allow_realloc, (const u_char *) "\n"))
    ...
    for (vars = pdu->variables; vars != NULL; vars = vars->next_variable)
    {
        ...
        if (!sprint_realloc_variable (buf, buf_len, out_len, allow_realloc, vars->name, vars->name_length, vars))
        {
            return 0;
        }
    }
...
realloc_format_trap (u_char ** buf, size_t * buf_len, size_t * out_len,
                     int allow_realloc, const char *format_str, netsnmp_pdu * pdu, netsnmp_transport * transport)
     /*
      * Function:
    ...
    for (fmt_idx = 0; format_str[fmt_idx] != '\0'; fmt_idx++)
    {
        ...
        switch (state)
        {
                 ...
                 */
                {
                    ...
                    while (j < i && next_chr && next_chr != CHR_FMT_DELIM)
                    {
                        if (next_chr == '\\')
                        {
                             ...
                             */
                            next_chr = format_str[++fmt_idx];
                            if (!realloc_handle_backslash ((u_char **) & sep, &i, &j, 0, next_chr))
                            {
                                return 0;
                            }
                        }
            ...
            case PARSE_BACKSLASH:
                 ...
                 * Found a backslash.  
                 */
                if (!realloc_handle_backslash (buf, buf_len, out_len, allow_realloc, next_chr))
                {
                    return 0;
                }
                state = PARSE_NORMAL;
            ...
            case PARSE_IN_FORMAT:
                ...
                else if (is_fmt_cmd (next_chr))
                {
                    ...
                    if (!realloc_dispatch_format_cmd (buf, buf_len, out_len, allow_realloc, &options, pdu, transport))
                    {
                        return 0;
                    }
                    state = PARSE_NORMAL;
            ...
            case PARSE_GET_PRECISION:
                ...
                {
                    options.cmd = next_chr;
                    if ((options.precision != UNDEF_PRECISION) && (options.width < (size_t) options.precision))
                    {
