...
int main (int argc, char *argv[])
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            {
                if (command == SNMP_MSG_INFORM && response->errstat == SNMP_ERR_NOERROR)
                {
        ...
        switch (vp->type)
        {
            ...
            case ASN_OCTET_STR:
                if (ch == 'd')
                {
                    ...
                    if (!snmp_decimal_to_binary (&(vp->val.string), &buf_len, &val_len, 1, buf))
                    {
                ...
                else if (ch == 'x')
                {
                    ...
                    if (!snmp_hex_to_binary (&(vp->val.string), &buf_len, &val_len, 1, buf))
                    {
