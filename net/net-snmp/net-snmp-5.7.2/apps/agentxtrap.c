...
netsnmp_feature_require (snmp_split_pdu) netsnmp_feature_require (snmp_reset_var_types)
#ifdef __GNUC__
     ...
     static void usage (const char *progname)
{
    ...
    fprintf (stderr, "  -L LOGOPTS\t\tToggle various defaults controlling logging:\n");
...
static int
handle_agentx_response (int operation, netsnmp_session * sp, UNUSED int reqid, netsnmp_pdu * act, UNUSED void *magic)
{
    switch (operation)
    {
...
static void ConnectingEntry (UNUSED tState self)
{
    ...
    if (!
        (t =
         netsnmp_transport_open_client ("agentx",
...
static netsnmp_pdu *pdu_create_opt_context (int command, const char *context, size_t len)
{
...
static void OpeningEntry (UNUSED tState self)
{
    netsnmp_pdu *act = pdu_create_opt_context (AGENTX_MSG_OPEN, context, contextLen);
...
static void OpeningRes (UNUSED tState self, netsnmp_pdu * act)
{
...
static void NotifyingEntry (UNUSED tState self)
{
...
static void NotifyingRes (UNUSED tState self, netsnmp_pdu * act)
{
    if (act->errstat == AGENTX_ERR_NOERROR)
        result = 0;
    else
...
static void ClosingEntry (UNUSED tState self)
{
    /* CLOSE pdu->errstat */
    netsnmp_pdu *act = pdu_create_opt_context (AGENTX_MSG_CLOSE, context, contextLen);
...
static void ClosingRes (UNUSED tState self, netsnmp_pdu * act)
{
    if (act->errstat != AGENTX_ERR_NOERROR)
    {
...
static void ClosingDisconnect (UNUSED tState self)
{
    change_state (&Disconnecting);
}

static void ClosingClose (UNUSED tState self, UNUSED netsnmp_pdu * act)
{
    change_state (&Disconnecting);
}
...
static void DisconnectingEntry (UNUSED tState self)
{
    snmp_sess_close (sessp);
    sessp = NULL;
    change_state (&Exit);
