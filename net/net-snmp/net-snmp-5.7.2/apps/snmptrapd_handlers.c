...
void snmptrapd_parse_traphandle (const char *token, char *line)
{
    ...
    if (!strcmp (buf, "default"))
    {
        ...
        traph = netsnmp_add_global_traphandler (NETSNMPTRAPD_DEFAULT_HANDLER, command_handler);
    }
    else
    {
        cp = buf + strlen (buf) - 1;
        ...
        {
            netsnmp_config_error ("Bad trap OID in traphandle directive: %s", buf);
            return;
        }
...
static void parse_forward (const char *token, char *line)
{
    ...
    if (!strcmp (buf, "default"))
    {
        ...
        if (!strcmp (cptr, "agentx"))
            traph = netsnmp_add_global_traphandler (NETSNMPTRAPD_DEFAULT_HANDLER, axforward_handler);
        else
            traph = netsnmp_add_global_traphandler (NETSNMPTRAPD_DEFAULT_HANDLER, forward_handler);
    }
    else
    {
        cp = buf + strlen (buf) - 1;
...
void snmptrapd_register_configs (void)
{
    ...
    register_config_handler ("snmptrapd", "format1", parse_trap1_fmt, free_trap1_fmt, "format");
    register_config_handler ("snmptrapd", "format2", parse_trap2_fmt, free_trap2_fmt, "format");
    register_config_handler ("snmptrapd", "format",
                             parse_format, NULL, "[print{,1,2}|syslog{,1,2}|execute{,1,2}] format");
    register_config_handler ("snmptrapd", "forward", parse_forward, NULL, "OID|\"default\" destination");
}
...
{
    return netsnmp_add_global_traphandler (NETSNMPTRAPD_DEFAULT_HANDLER, handler);
}
 ...
 */
netsnmp_trapd_handler *netsnmp_add_traphandler (Netsnmp_Trap_Handler * handler, oid * trapOid, int trapOidLen)
{
    ...
    {
        /* XXX - check this! */
        if (snmp_oid_compare (traph2->trapoid, traph2->trapoid_len, trapOid, trapOidLen) <= 0)
            break;
    }
    if (traph2)
    {
         ...
         */
        if (!snmp_oid_compare (traph->trapoid, traph->trapoid_len, traph2->trapoid, traph2->trapoid_len))
        {
            /*
...
netsnmp_trapd_handler *netsnmp_get_traphandler (oid * trapOid, int trapOidLen)
{
    ...
    for (traph = netsnmp_specific_traphandlers; traph; traph = traph->nextt)
    {
        ...
        {
            if (snmp_oid_compare (traph->trapoid, traph->trapoid_len, trapOid, trapOidLen) == 0)
            {
                DEBUGMSGTL (("snmptrapd:lookup", "get_traphandler exact match (%p)\n", traph));
                return traph;
            }
        }
        else
        {
             ...
             */
            if (snmp_oidsubtree_compare (traph->trapoid, traph->trapoid_len, trapOid, trapOidLen) == 0)
            {
 ...
 */
int syslog_handler (netsnmp_pdu * pdu, netsnmp_transport * transport, netsnmp_trapd_handler * handler)
{
    u_char *rbuf = NULL;
    ...
    if (handler && handler->format)
    {
        ...
        if (*handler->format)
        {
            trunc = !realloc_format_trap (&rbuf, &r_len, &o_len, 1, handler->format, pdu, transport);
        }
        else
        {
            free (rbuf);
    ...
    else
    {
        if (pdu->command == SNMP_MSG_TRAP)
        {
            if (syslog_format1)
            {
                ...
                trunc = !realloc_format_trap (&rbuf, &r_len, &o_len, 1, syslog_format1, pdu, transport);

            }
            else if (pdu->trap_type == SNMP_TRAP_ENTERPRISESPECIFIC)
            {
                DEBUGMSGTL (("snmptrapd", "v1 enterprise format\n"));
                trunc = !realloc_format_trap (&rbuf, &r_len, &o_len, 1, SYSLOG_V1_ENTERPRISE_FORMAT, pdu, transport);
            }
            else
            {
                DEBUGMSGTL (("snmptrapd", "v1 standard trap format\n"));
                trunc = !realloc_format_trap (&rbuf, &r_len, &o_len, 1, SYSLOG_V1_STANDARD_FORMAT, pdu, transport);
            }
        }
        else
        {                        /* SNMPv2/3 notifications */
            ...
            {
                DEBUGMSGTL (("snmptrapd", "syslog_format v1 = '%s'\n", syslog_format2));
                trunc = !realloc_format_trap (&rbuf, &r_len, &o_len, 1, syslog_format2, pdu, transport);
            }
            else
            {
                DEBUGMSGTL (("snmptrapd", "v2/3 format\n"));
                trunc = !realloc_format_trap (&rbuf, &r_len, &o_len, 1, SYSLOG_V23_NOTIFICATION_FORMAT, pdu, transport);
            }
        }
    }
 ...
 */
int print_handler (netsnmp_pdu * pdu, netsnmp_transport * transport, netsnmp_trapd_handler * handler)
{
    u_char *rbuf = NULL;
    ...
    else
    {
        if (pdu->command == SNMP_MSG_TRAP)
        {
            if (print_format1)
            {
                ...
                trunc = !realloc_format_trap (&rbuf, &r_len, &o_len, 1, print_format1, pdu, transport);
            }
            else
            {
                DEBUGMSGTL (("snmptrapd", "v1 format\n"));
                trunc = !realloc_format_plain_trap (&rbuf, &r_len, &o_len, 1, pdu, transport);
            }
        }
        else
        {
            if (print_format2)
            {
                ...
                trunc = !realloc_format_trap (&rbuf, &r_len, &o_len, 1, print_format2, pdu, transport);
            }
            else
            {
                DEBUGMSGTL (("snmptrapd", "v2/3 format\n"));
                trunc = !realloc_format_trap (&rbuf, &r_len, &o_len, 1, PRINT_V23_NOTIFICATION_FORMAT, pdu, transport);
            }
        }
    }
 ...
 */
int command_handler (netsnmp_pdu * pdu, netsnmp_transport * transport, netsnmp_trapd_handler * handler)
{
#ifndef USING_UTILITIES_EXECUTE_MODULE
    NETSNMP_LOGONCE ((LOG_WARNING, "support for run_shell_command not available\n"));
    ...
    if (handler && handler->token && *handler->token)
    {
        ...
        if (pdu->command == SNMP_MSG_TRAP)
            ...
            v2_pdu = pdu;
        oldquick = netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_QUICK_PRINT);
        ...
        {
            DEBUGMSGTL (("snmptrapd", "format = '%s'\n", handler->format));
            realloc_format_trap (&rbuf, &r_len, &o_len, 1, handler->format, v2_pdu, transport);
        }
        else
        {
            if (pdu->command == SNMP_MSG_TRAP && exec_format1)
            {
                ...
                realloc_format_trap (&rbuf, &r_len, &o_len, 1, exec_format1, pdu, transport);
            }
            else if (pdu->command != SNMP_MSG_TRAP && exec_format2)
            {
                DEBUGMSGTL (("snmptrapd", "exec v2/3 = '%s'\n", exec_format2));
                realloc_format_trap (&rbuf, &r_len, &o_len, 1, exec_format2, pdu, transport);
            }
            else
            {
                DEBUGMSGTL (("snmptrapd", "execute format\n"));
                realloc_format_trap (&rbuf, &r_len, &o_len, 1, EXECUTE_FORMAT, v2_pdu, transport);
            }
        }
        ...
        netsnmp_ds_set_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_QUICK_PRINT, oldquick);
        if (pdu->command == SNMP_MSG_TRAP)
 ...
 */
int axforward_handler (netsnmp_pdu * pdu, netsnmp_transport * transport, netsnmp_trapd_handler * handler)
{
 ...
 */
int forward_handler (netsnmp_pdu * pdu, netsnmp_transport * transport, netsnmp_trapd_handler * handler)
{
 ...
 *          (presumably)
 */
int notification_handler (netsnmp_pdu * pdu, netsnmp_transport * transport, netsnmp_trapd_handler * handler)
{
