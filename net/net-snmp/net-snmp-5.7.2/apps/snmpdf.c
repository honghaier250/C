    ...
    while (running)
    {
        ...
        if (response && snmp_oid_compare (response->variables->name,
                                          SNMP_MIN (base_length,
                                                    response->variables->name_length), base, base_length) != 0)
            running = 0;
        else
        {
            /*
...
int main (int argc, char *argv[])
{
    ...
    if (ucd_mib == 0)
    {
        ...
        while (vlp)
        {
            ...
            printf ("%-18s %15lu %15lu %15lu %4lu%%\n", descr,
                    ...
                    hsused, hssize ? convert_units (hsused, 100, hssize) : hsused);
    ...
    if (count == 0)
    {
        ...
        while (vlp)
        {
            ...
            add (pdu, "UCD-SNMP-MIB:dskTotal", &(vlp->name[base_length]), vlp->name_length - base_length);
            add (pdu, "UCD-SNMP-MIB:dskUsed", &(vlp->name[base_length]), vlp->name_length - base_length);
