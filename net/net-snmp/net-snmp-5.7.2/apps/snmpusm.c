...
static oid authKeyOid[MAX_OID_LEN] = { 1, 3, 6, 1, 6, 3, 15, 1, 2, 2, 1, 6 }, ownAuthKeyOid[MAX_OID_LEN] =
...
{
1, 3, 6, 1, 6, 3, 15, 1, 2, 2, 1, 3}, usmUserPublic[MAX_OID_LEN] =

{
1, 3, 6, 1, 6, 3, 15, 1, 2, 2, 1, 11}, usmUserStatus[MAX_OID_LEN] =

{
1, 3, 6, 1, 6, 3, 15, 1, 2, 2, 1, 13},

/* diffie helman change key objects */
    usmDHUserAuthKeyChange[MAX_OID_LEN] =
{
1, 3, 6, 1, 3, 101, 1, 1, 2, 1, 1}, usmDHUserPrivKeyChange[MAX_OID_LEN] =

{
1, 3, 6, 1, 3, 101, 1, 1, 2, 1, 3},

#if defined(HAVE_OPENSSL_DH_H) && defined(HAVE_LIBCRYPTO)
    usmDHUserOwnAuthKeyChange[MAX_OID_LEN] =
{
1, 3, 6, 1, 3, 101, 1, 1, 2, 1, 2}, usmDHUserOwnPrivKeyChange[MAX_OID_LEN] =
...
oid *dhauthKeyChange = usmDHUserAuthKeyChange, *dhprivKeyChange = usmDHUserPrivKeyChange;
...
get_USM_DH_key (netsnmp_variable_list * vars, netsnmp_variable_list * dhvar,
                size_t outkey_len, netsnmp_pdu * pdu, const char *keyname, oid * keyoid, size_t keyoid_len)
{
    u_char *dhkeychange;
    ...
    snmp_pdu_add_variable (pdu, keyoid, keyoid_len, ASN_OCTET_STR, dhkeychange, 2 * vars->val_len);
                    ...
                    case 'E':
                        {
                            ...
                            if (optind < argc)
                            {
                                if (argv[optind])
                                {
                                    ...
                                    if (ebuf == NULL)
                                    {
                                        fprintf (stderr, "malloc failure processing -CE option.\n");
                                        exit (1);
                                    }
                                    if (!snmp_hex_to_binary (&ebuf, &ebuf_len, &usmUserEngineIDLen, 1, argv[optind]))
                                    {
                                        fprintf (stderr, "Bad usmUserEngineID value after -CE option.\n");
                                        free (ebuf);
                                        exit (1);
                                    }
...
int main (int argc, char *argv[])
{
    ...
    size_t oldKu_len = SNMP_MAXBUF_SMALL,
        ...
        newkul_len = SNMP_MAXBUF_SMALL, keychange_len = SNMP_MAXBUF_SMALL, keychangepriv_len = SNMP_MAXBUF_SMALL;
    ...
    if (strcmp (argv[arg], CMD_PASSWD_NAME) == 0)
    {
        ...
        {
            session.securityName = strdup (netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SECNAME));
        }
        ...
        if (session.securityAuthProto == NULL)
        {
            ...
            session.securityAuthProto = snmp_duplicate_objid (usmHMACSHA1AuthProtocol, session.securityAuthProtoLen);
#endif
        ...
        else
        {
            ...
            rval = generate_Ku (session.securityAuthProto,
                                session.securityAuthProtoLen, (u_char *) oldpass, strlen (oldpass), oldKu, &oldKu_len);
            ...
            rval = generate_kul (session.securityAuthProto,
                                 ...
                                 usmUserEngineID, usmUserEngineIDLen, oldKu, oldKu_len, oldkul, &oldkul_len);
        ...
        {
            rval = generate_Ku (session.securityAuthProto,
                                session.securityAuthProtoLen, (u_char *) newpass, strlen (newpass), newKu, &newKu_len);
            ...
            rval = generate_kul (session.securityAuthProto,
                                 session.securityAuthProtoLen,
                                 usmUserEngineID, usmUserEngineIDLen, newKu, newKu_len, newkul, &newkul_len);
        ...
        if (doauthkey)
        {
            rval = encode_keychange (session.securityAuthProto,
                                     session.securityAuthProtoLen,
                                     oldkul, oldkul_len, newkul, newkul_len, keychange, &keychange_len);
        ...
        if (doprivkey)
        {
            rval = encode_keychange (session.securityAuthProto,
                                     ...
                                     newkulpriv, newkulpriv_len, keychangepriv, &keychangepriv_len);
        ...
        if (doauthkey)
        {
            setup_oid (authKeyChange, &name_length, usmUserEngineID, usmUserEngineIDLen, session.securityName);
            snmp_pdu_add_variable (pdu, authKeyChange, name_length, ASN_OCTET_STR, keychange, keychange_len);
        }
        if (doprivkey)
        {
            setup_oid (privKeyChange, &name_length2, usmUserEngineID, usmUserEngineIDLen, session.securityName);
            snmp_pdu_add_variable (pdu, privKeyChange, name_length2, ASN_OCTET_STR, keychangepriv, keychangepriv_len);
        }
    ...
    else if (strcmp (argv[arg], CMD_CREATE_NAME) == 0)
    {
        ...
        if (++arg < argc)
        {
             ...
             */
            setup_oid (usmUserStatus, &name_length, usmUserEngineID, usmUserEngineIDLen, argv[arg - 1]);
            if (docreateandwait)
            {
            ...
            setup_oid (usmUserCloneFrom, &name_length, usmUserEngineID, usmUserEngineIDLen, argv[arg - 1]);
            setup_oid (usmUserSecurityName, &name_length2, usmUserEngineID, usmUserEngineIDLen, argv[arg]);
            snmp_pdu_add_variable (pdu, usmUserCloneFrom, name_length,
                                   ASN_OBJECT_ID, (u_char *) usmUserSecurityName, sizeof (oid) * name_length2);
        }
        else
        {
            /*
    ...
    else if (strcmp (argv[arg], CMD_CLONEFROM_NAME) == 0)
    {
        ...
        setup_oid (usmUserStatus, &name_length, usmUserEngineID, usmUserEngineIDLen, argv[arg]);
        longvar = RS_ACTIVE;
        snmp_pdu_add_variable (pdu, usmUserStatus, name_length, ASN_INTEGER, (u_char *) & longvar, sizeof (longvar));
        name_length = USM_OID_LEN;
        setup_oid (usmUserCloneFrom, &name_length, usmUserEngineID, usmUserEngineIDLen, argv[arg]);
    ...
    else if (strcmp (argv[arg], CMD_CHANGEKEY_NAME) == 0)
    {
        ...
        if (doauthkey)
        {
            setup_oid (dhauthKeyChange, &name_length, usmUserEngineID, usmUserEngineIDLen, session.securityName);
        ...
        if (status != SNMPERR_SUCCESS || dhresponse == NULL ||
            dhresponse->errstat != SNMP_ERR_NOERROR || dhresponse->variables->type != ASN_OCTET_STR)
        {
            snmp_sess_perror ("snmpusm", ss);
            if (dhresponse && dhresponse->variables && dhresponse->variables->type != ASN_OCTET_STR)
            {
                fprintf (stderr, "Can't get diffie-helman exchange from the agent\n");
        ...
        if (doauthkey)
        {
            if (get_USM_DH_key (vars, dhvar,
                                ...
                                pdu, "auth", dhauthKeyChange, name_length) != SNMPERR_SUCCESS)
                goto begone;
...
#endif
            if (get_USM_DH_key (vars, dhvar,
                                dhprivKeyLen, pdu, "priv", dhprivKeyChange, name_length2) != SNMPERR_SUCCESS)
                goto begone;
            vars = vars->next_variable;
        }
    ...
    {
        name_length = USM_OID_LEN;
        setup_oid (usmUserPublic, &name_length, usmUserEngineID, usmUserEngineIDLen, session.securityName);
        snmp_pdu_add_variable (pdu, usmUserPublic, name_length,
                               ASN_OCTET_STR, usmUserPublic_val, strlen (usmUserPublic_val));
    }
