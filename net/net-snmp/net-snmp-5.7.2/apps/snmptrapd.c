...
static int
pre_parse (netsnmp_session * session, netsnmp_transport * transport, void *transport_data, int transport_data_length)
{
#if NETSNMP_USE_LIBWRAP
...
void parse_config_agentuser (const char *token, char *cptr)
{
    ...
    else
    {
        ...
        info = getpwnam (cptr);
        if (info)
            netsnmp_ds_set_int (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_USERID, info->pw_uid);
        else
...
void parse_config_agentgroup (const char *token, char *cptr)
{
    if (cptr[0] == '#')
    {
        ...
        {
            netsnmp_ds_set_int (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_GROUPID, gid);
        }
    ...
    else
    {
        ...
        info = getgrnam (cptr);
        if (info)
            netsnmp_ds_set_int (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_GROUPID, info->gr_gid);
        else
...
#endif
{
    ...
    register_config_handler ("snmptrapd", "snmpTrapdAddr", parse_trapd_address, free_trapd_address, "string");

    register_config_handler ("snmptrapd", "doNotLogTraps", parse_config_doNotLogTraps, NULL, "(1|yes|true|0|no|false)");
#if HAVE_GETPID
    register_config_handler ("snmptrapd", "pidFile", parse_config_pidFile, NULL, "string");
#endif
#ifdef HAVE_UNISTD_H
    register_config_handler ("snmptrapd", "agentuser", parse_config_agentuser, NULL, "userid");
    register_config_handler ("snmptrapd", "agentgroup", parse_config_agentgroup, NULL, "groupid");
#endif

    register_config_handler ("snmptrapd", "doNotFork", parse_config_doNotFork, NULL, "(1|yes|true|0|no|false)");

    register_config_handler ("snmptrapd", "ignoreAuthFailure",
                             parse_config_ignoreAuthFailure, NULL, "(1|yes|true|0|no|false)");

    register_config_handler ("snmptrapd", "outputOption", parse_config_outputOption, NULL, "string");
    ...
    while ((arg = getopt (argc, argv, options)) != EOF)
    {
        switch (arg)
        {
            ...
            case 'A':
                netsnmp_ds_set_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_APPEND_LOGFILES, 1);
                break;

            case 'c':
                if (optarg != NULL)
                {
                    netsnmp_ds_set_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_OPTIONALCONFIG, optarg);
                }
                else
                {
                    usage ();
            ...
            case 'F':
                if (optarg != NULL)
                {
                    if ((strncmp (optarg, "print", 5) == 0) ||
                        (strncmp (optarg, "syslog", 6) == 0) || (strncmp (optarg, "execute", 7) == 0))
                    {
            ...
            case 'g':
                if (optarg != NULL)
                {
                    netsnmp_ds_set_int (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_GROUPID, gid = atoi (optarg));
                }
                else
                {
                    usage ();
            ...
            case 'S':
                fprintf (stderr, "Warning: -S option has been withdrawn; use -Ls <facility> instead\n");
                exit (1);
                break;
            ...
            case 'n':
                netsnmp_ds_set_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_APP_NUMERIC_IP, 1);
                break;

            case 'o':
                fprintf (stderr, "Warning: -o option has been withdrawn; use -Lf <file> instead\n");
                exit (1);
                break;
            ...
            case 'P':
                fprintf (stderr, "Warning: -P option has been withdrawn; use -f -Le instead\n");
                exit (1);
                break;

            case 's':
                fprintf (stderr, "Warning: -s option has been withdrawn; use -Lsd instead\n");
                exit (1);
                break;
            ...
            case 'x':
                if (optarg != NULL)
                {
                    netsnmp_ds_set_string (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_X_SOCKET, optarg);
                }
                else
                {
                    usage ();
    ...
    {
#ifndef NETSNMP_FEATURE_REMOVE_LOGGING_SYSLOG
        traph = netsnmp_add_global_traphandler (NETSNMPTRAPD_PRE_HANDLER, syslog_handler);
        traph->authtypes = TRAP_AUTH_LOG;
        snmp_enable_syslog ();
#else                            /* NETSNMP_FEATURE_REMOVE_LOGGING_SYSLOG */
#ifndef NETSNMP_FEATURE_REMOVE_LOGGING_STDIO
        traph = netsnmp_add_global_traphandler (NETSNMPTRAPD_PRE_HANDLER, print_handler);
        traph->authtypes = TRAP_AUTH_LOG;
        snmp_enable_stderr ();
    ...
    init_perl ();
    {
        ...
        snprintf (init_file, sizeof (init_file) - 1, "%s/%s", SNMPSHAREPATH, "snmp_perl_trapd.pl");
        netsnmp_ds_set_string (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_PERL_INIT_FILE, init_file);
    }
#endif
...
#ifdef IFDESCR
        _T
        ("SNMPv2c / SNMPv3 trap/inform receiver from Net-SNMP. Supports MIB objects for IP,ICMP,TCP,UDP, and network interface sub-layers.");
#else
    ...
    switch (nRunType)
    {
        case REGISTER_SERVICE:
            ...
            exit (RegisterService (lpszServiceName,
                                   lpszServiceDisplayName, lpszServiceDescription, &InputOptions, quiet));
            break;
