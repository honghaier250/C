...
{
    register_config_handler ("snmptrapd", "sqlMaxQueue", _parse_queue_fmt, NULL, "integer");
    register_config_handler ("snmptrapd", "sqlSaveInterval", _parse_interval_fmt, NULL, "seconds");
}
 ...
 */
static int netsnmp_mysql_bind (const char *text, size_t text_size, MYSQL_STMT ** stmt, MYSQL_BIND * bind)
{
...
static int netsnmp_mysql_connect (void)
{
    char trap_stmt[] = "INSERT INTO notifications "
        ...
        "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    char vb_stmt[] = "INSERT INTO varbinds " "(trap_id, oid, type, value) VALUES (?,?,?,?)";
    ...
    if (mysql_real_connect (_sql.conn, _sql.host_name, _sql.user_name,
                            _sql.password, _sql.db_name, _sql.port_num, _sql.socket_name, _sql.flags) == NULL)
    {
    ...
    if (0 != netsnmp_mysql_bind (trap_stmt, sizeof (trap_stmt), &_sql.trap_stmt, _tbind))
        goto err;

    if (0 != netsnmp_mysql_bind (vb_stmt, sizeof (vb_stmt), &_sql.vb_stmt, _vbind))
    {
        mysql_stmt_close (_sql.trap_stmt);
        _sql.trap_stmt = NULL;
        goto err;
...
int netsnmp_mysql_init (void)
{
    ...
    {
        DEBUGMSGTL (("sql:init", "mysql not enabled (sqlSaveInterval is <= 0)\n"));
        return 0;
    }
    ...
    _tbind[TBIND_v3_SECURITY_NAME].length = &_tbind[TBIND_v3_SECURITY_NAME].buffer_length;
    _tbind[TBIND_v3_CONTEXT_NAME].length = &_tbind[TBIND_v3_CONTEXT_NAME].buffer_length;
    _tbind[TBIND_v3_SECURITY_ENGINE].buffer_type = MYSQL_TYPE_STRING;
    _tbind[TBIND_v3_SECURITY_ENGINE].length = &_tbind[TBIND_v3_SECURITY_ENGINE].buffer_length;
    _tbind[TBIND_v3_CONTEXT_ENGINE].length = &_tbind[TBIND_v3_CONTEXT_ENGINE].buffer_length;
    ...
    _tbind[TBIND_v3_MSGID].is_null =
        ...
        _tbind[TBIND_v3_SECURITY_NAME].is_null = _tbind[TBIND_v3_SECURITY_ENGINE].is_null = &_no_v3;
    ...
    /** add handler */
    traph = netsnmp_add_global_traphandler (NETSNMPTRAPD_PRE_HANDLER, mysql_handler);
    if (NULL == traph)
    {
...
static void _sql_log (sql_buf * sqlb, void *dontcare)
{
    ...
    snmp_log (LOG_ERR,
              ...
              sqlb->security_level, sqlb->context, sqlb->context_engine, sqlb->security_name, sqlb->security_engine);
...
static void _sql_buf_free (sql_buf * sqlb, void *dontcare)
{
    ...
    if (sqlb->varbinds)
    {
        CONTAINER_CLEAR (sqlb->varbinds, (netsnmp_container_obj_func *) _sql_vb_buf_free, NULL);
 ...
 */
static int _sql_save_trap_info (sql_buf * sqlb, netsnmp_pdu * pdu, netsnmp_transport * transport)
{
    ...
    realloc_format_trap ((u_char **) & sqlb->host, &tmp_size, &buf_host_len_t, 1, "%B", pdu, transport);
    ...
    netsnmp_sprint_realloc_objid_tree ((u_char **) & sqlb->oid, &tmp_size,
                                       &buf_oid_len_t, 1, &oid_overflow, trap_oid, trap_oid_len);
    ...
    tmp_size = 0;
    buf_user_len_t = 0;
    realloc_format_trap ((u_char **) & sqlb->user, &tmp_size, &buf_user_len_t, 1, "%u", pdu, transport);
    sqlb->user_len = buf_user_len_t;

    /** transport */
    sqlb->transport = transport->f_fmtaddr (transport, pdu->transport_data, pdu->transport_data_length);
    ...
    if ((SNMP_MP_MODEL_SNMPv3 + 1) == sqlb->version)
    {
        ...
        if (pdu->contextName)
        {
            sqlb->context = netsnmp_strdup_and_null ((u_char *) pdu->contextName, pdu->contextNameLen);
            sqlb->context_len = pdu->contextNameLen;
        }
        if (pdu->contextEngineID)
        {
            sqlb->context_engine_len =
                binary_to_hex (pdu->contextEngineID, pdu->contextEngineIDLen, &sqlb->context_engine);
        }

        if (pdu->securityName)
        {
            sqlb->security_name = netsnmp_strdup_and_null ((u_char *) pdu->securityName, pdu->securityNameLen);
            sqlb->security_name_len = pdu->securityNameLen;
        }
        if (pdu->securityEngineID)
        {
            sqlb->security_engine_len =
                binary_to_hex (pdu->securityEngineID, pdu->securityEngineIDLen, &sqlb->security_engine);
        }
    }
...
static int _sql_save_varbind_info (sql_buf * sqlb, netsnmp_pdu * pdu)
{
    ...
    while (var)
    {
        ...
        netsnmp_sprint_realloc_objid_tree ((u_char **) & sqlvb->oid, &tmp_size,
                                           &buf_oid_len_t, 1, &oid_overflow, var->name, var->name_length);
        ...
        tmp_size = 0;
        buf_val_len_t = 0;
        sprint_realloc_by_type ((u_char **) & sqlvb->val, &tmp_size, &buf_val_len_t, 1, var, 0, 0, 0);
 ...
 * sql trap handler
 */
int mysql_handler (netsnmp_pdu * pdu, netsnmp_transport * transport, netsnmp_trapd_handler * handler)
{
    sql_buf *sqlb;
...
static void _sql_save (sql_buf * sqlb, void *dontcare)
{
    ...
    if ((SNMP_MP_MODEL_SNMPv3 + 1) == sqlb->version)
    {
        ...
        _tbind[TBIND_v3_CONTEXT_ENGINE].buffer_length = sqlb->context_engine_len;
        ...
        _tbind[TBIND_v3_SECURITY_ENGINE].buffer_length = sqlb->security_engine_len;
    }
    else
    {
        _no_v3 = 1;
    ...
    {
        netsnmp_sql_stmt_error (_sql.trap_stmt, "Could not bind parameters for INSERT");
        _sql_log (sqlb, NULL);
        return;
    }
    ...
    {
        netsnmp_sql_stmt_error (_sql.trap_stmt, "Could not execute insert statement for trap");
        _sql_log (sqlb, NULL);
        return;
    }
    ...
    for (sqlvb = ITERATOR_FIRST (it); sqlvb; sqlvb = ITERATOR_NEXT (it))
    {
        ...
        {
            netsnmp_sql_stmt_error (_sql.vb_stmt, "Could not bind parameters for INSERT");
            _sql_log (sqlb, NULL);
            break;
        }
        ...
        {
            netsnmp_sql_stmt_error (_sql.vb_stmt, "Could not execute insert statement for varbind");
            _sql_log (sqlb, NULL);
            break;
        }
    }
...
static void _sql_process_queue (u_int dontcare, void *meeither)
{
    ...
    CONTAINER_FOR_EACH (_sql.queue, (netsnmp_container_obj_func *) _sql_save, NULL);
    ...
    if (_sql.connected)
    {
        ...
        {                        /* nuts... now what? */
            ...
            CONTAINER_FOR_EACH (_sql.queue, (netsnmp_container_obj_func *) _sql_log, NULL);
        }
    }

    CONTAINER_CLEAR (_sql.queue, (netsnmp_container_obj_func *) _sql_buf_free, NULL);
}
