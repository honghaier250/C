...
void init_netsnmp_trapd_auth (void)
{
    ...
    netsnmp_ds_register_config (ASN_BOOLEAN, "snmptrapd", "disableAuthorization",
                                NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_APP_NO_AUTHORIZATION);
}
...
int netsnmp_trapd_auth (netsnmp_pdu * pdu, netsnmp_transport * transport, netsnmp_trapd_handler * handler)
{
    ...
    if (netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_APP_NO_AUTHORIZATION))
    {
        DEBUGMSGTL (("snmptrapd:auth", "authorization turned off: not checking\n"));
    ...
    {
        if (netsnmp_oid_equals (var->name, var->name_length, snmptrapoid, snmptrapoid_len) == 0)
            break;
    }
...
int netsnmp_trapd_check_auth (int authtypes)
{
    ...
    DEBUGMSGTL (("snmptrapd:auth",
                 ...
                 lastlookup, authtypes, ((authtypes & lastlookup) == authtypes)));
