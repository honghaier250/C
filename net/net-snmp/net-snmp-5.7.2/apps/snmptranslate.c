...
int main (int argc, char *argv[])
{
    ...
    do
    {
        ...
        {
            if (0 == show_all_matched_objects (stdout, current_name, name, &name_length, description, width))
            {
        ...
        if (print == 1)
        {
            ...
            {
#endif                            /* NETSNMP_DISABLE_MIB_LOADING */
                snmp_log (LOG_ERR, "Unable to find a matching object identifier for \"%s\"\n", current_name);
                exit (1);
#ifndef NETSNMP_DISABLE_MIB_LOADING
            }
...
int show_all_matched_objects (FILE * fp, const char *patmatch, oid * name, size_t * name_length, int f_desc,    /* non-zero if descriptions should be shown */
                              int width)
{
