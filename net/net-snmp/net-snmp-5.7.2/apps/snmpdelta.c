...
void sprint_descriptor (char *buffer, struct varInfo *vip)
{
    ...
    if (!sprint_realloc_objid ((u_char **) & buf, &buf_len, &out_len, 1, vip->info_oid, vip->oidlen))
    {
        if (buf != NULL)
        {
            free (buf);
...
int main (int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if (end == current_name)
            ...
            count = end;
        begin = count;
        for (; count < current_name && count < begin + varbindsPerPacket - deltat; count++)
        {
        ...
        if (status == STAT_SUCCESS)
        {
            if (response->errstat == SNMP_ERR_NOERROR)
            {
                if (timestamp)
                {
                    gettimeofday (&tv, (struct timezone *) 0);
                    memcpy (&tm, localtime ((time_t *) & tv.tv_sec), sizeof (tm));
                    if (((period % 60) && (!peaks || ((period * peaks) % 60))) || keepSeconds)
                        sprintf (timestring, " [%02d:%02d:%02d %d/%d]",
                                 tm.tm_hour, tm.tm_min, tm.tm_sec, tm.tm_mon + 1, tm.tm_mday);
                    else
                        sprintf (timestring, " [%02d:%02d %d/%d]", tm.tm_hour, tm.tm_min, tm.tm_mon + 1, tm.tm_mday);
                }
                ...
                for (count = begin; count < end; count++)
                {
                    ...
                    if (vip->oidlen)
                    {
                        ...
                        if (vars->type == ASN_COUNTER64)
                        {
                            ...
                            memcpy (&vip->c64value, vars->val.counter64, sizeof (struct counter64));
                        }
                        else
                        {
                            ...
                            if (++vip->peak_count == peaks)
                            {
                                if (deltat)
                                    sprintf (peakStr,
                                             ...
                                             vip->peak_average / vip->peak_count, period, vip->peak);
                                else
                                    sprintf (peakStr,
                                             " /%d sec: %.0f    (%d sec Peak: %.0f)",
                                             period, vip->peak_average / vip->peak_count, period, vip->peak);
                                vip->peak_average = 0;
                                vip->peak = 0;
                                vip->peak_count = 0;
                                print = 1;
            ...
            else
            {
                ...
                {
                    if (response->errindex <= varbindsPerPacket && response->errindex > 0)
                    {
