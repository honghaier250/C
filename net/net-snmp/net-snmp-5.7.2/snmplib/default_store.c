...
{
    if (storeid < 0 || storeid >= NETSNMP_DS_MAX_IDS || which < 0 || which >= NETSNMP_DS_MAX_SUBIDS)
    {
...
void netsnmp_ds_handle_config (const char *token, char *line)
{
    ...
    for (drsp = netsnmp_ds_configs; drsp != NULL && strcasecmp (token, drsp->token) != 0; drsp = drsp->next);

    if (drsp != NULL)
    {
        DEBUGMSGTL (("netsnmp_ds_handle_config",
                     "setting: token=%s, type=%d, id=%s, which=%d\n",
                     drsp->token, drsp->type, stores[drsp->storeid], drsp->which));
...
int netsnmp_ds_register_config (u_char type, const char *ftype, const char *token, int storeid, int which)
{
    ...
    if (storeid < 0 || storeid >= NETSNMP_DS_MAX_IDS || which < 0 || which >= NETSNMP_DS_MAX_SUBIDS || token == NULL)
    {
        return SNMPERR_GENERR;
    }
    ...
    {
        case ASN_BOOLEAN:
            register_config_handler (ftype, token, netsnmp_ds_handle_config, NULL, "(1|yes|true|0|no|false)");
            break;

        case ASN_INTEGER:
            register_config_handler (ftype, token, netsnmp_ds_handle_config, NULL, "integerValue");
            break;

        case ASN_OCTET_STR:
            register_config_handler (ftype, token, netsnmp_ds_handle_config, NULL, "string");
            break;
...
int netsnmp_ds_register_premib (u_char type, const char *ftype, const char *token, int storeid, int which)
{
    ...
    {
        case ASN_BOOLEAN:
            register_prenetsnmp_mib_handler (ftype, token, netsnmp_ds_handle_config, NULL, "(1|yes|true|0|no|false)");
            break;

        case ASN_INTEGER:
            register_prenetsnmp_mib_handler (ftype, token, netsnmp_ds_handle_config, NULL, "integerValue");
            break;

        case ASN_OCTET_STR:
            register_prenetsnmp_mib_handler (ftype, token, netsnmp_ds_handle_config, NULL, "string");
            break;
