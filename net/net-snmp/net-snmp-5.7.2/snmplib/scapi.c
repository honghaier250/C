...
netsnmp_feature_child_of (usm_support, libnetsnmp) netsnmp_feature_child_of (usm_scapi, usm_support)
 ...
 */
     int sc_generate_keyed_hash (const oid * authtype, size_t authtypelen,
                                 ...
                                 const u_char * message, u_int msglen, u_char * MAC, size_t * maclen)
#if  defined(NETSNMP_USE_INTERNAL_MD5) || defined(NETSNMP_USE_OPENSSL) || defined(NETSNMP_USE_PKCS11) || defined(NETSNMP_USE_INTERNAL_CRYPTO)
{
    int rval = SNMPERR_SUCCESS;
    ...
    if (!authtype || !key || !message || !MAC || !maclen
        || (keylen <= 0) || (msglen <= 0) || (*maclen <= 0) || (authtypelen != USM_LENGTH_OID_TRANSFORM))
    {
    ...
    if (ISTRANSFORM (authtype, HMACMD5Auth))
    {
        if (pkcs_sign (CKM_MD5_HMAC, key, keylen, message, msglen, buf, &buf_len) != SNMPERR_SUCCESS)
        {
            QUITFUN (SNMPERR_GENERR, sc_generate_keyed_hash_quit);
        }
    }
    ...
    if (ISTRANSFORM (authtype, HMACSHA1Auth))
    {
        if (pkcs_sign (CKM_SHA_1_HMAC, key, keylen, message, msglen, buf, &buf_len) != SNMPERR_SUCCESS)
        {
            QUITFUN (SNMPERR_GENERR, sc_generate_keyed_hash_quit);
        }
    }
 ...
 */
     int sc_check_keyed_hash (const oid * authtype, size_t authtypelen,
                              const u_char * key, u_int keylen,
                              const u_char * message, u_int msglen, const u_char * MAC, u_int maclen)
#if defined(NETSNMP_USE_INTERNAL_MD5) || defined(NETSNMP_USE_OPENSSL) || defined(NETSNMP_USE_PKCS11) || defined(NETSNMP_USE_INTERNAL_CRYPTO)
{
    int rval = SNMPERR_SUCCESS;
     ...
     */
    rval = sc_generate_keyed_hash (authtype, authtypelen, key, keylen, message, msglen, buf, &buf_len);
 ...
 */
     int sc_encrypt (const oid * privtype, size_t privtypelen,
                     ...
                     const u_char * plaintext, u_int ptlen, u_char * ciphertext, size_t * ctlen)
#if defined(NETSNMP_USE_OPENSSL) || defined(NETSNMP_USE_INTERNAL_CRYPTO)
{
    ...
    if (!privtype || !key || !iv || !plaintext || !ciphertext || !ctlen
        || (keylen <= 0) || (ivlen <= 0) || (ptlen <= 0) || (*ctlen <= 0) || (privtypelen != USM_LENGTH_OID_TRANSFORM))
    {
...
#ifdef NETSNMP_ENABLE_TESTING_CODE
    {
        ...
        if (buf != NULL)
        {
            if (sprint_realloc_hexstring (&buf, &buf_len, &out_len, 1, iv, ivlen))
            {
            ...
            out_len = 0;
            if (sprint_realloc_hexstring (&buf, &buf_len, &out_len, 1, key, keylen))
            {
                DEBUGMSG (("scapi", "%s\n", buf));
            }
            else
            ...
            out_len = 0;
            if (sprint_realloc_hexstring (&buf, &buf_len, &out_len, 1, plaintext, 16))
            {
    ...
    if (ISTRANSFORM (privtype, DESPriv))
    {
         ...
         * encrypt the data 
         */
        DES_ncbc_encrypt (plaintext, ciphertext, plast, key_sch, (DES_cblock *) my_iv, DES_ENCRYPT);
        if (pad > 0)
        {
            /*
             * then encrypt the pad block 
             */
            DES_ncbc_encrypt (pad_block, ciphertext + plast, pad_size, key_sch, (DES_cblock *) my_iv, DES_ENCRYPT);
...
#elif defined(NETSNMP_USE_PKCS11)
{
    ...
    if (ISTRANSFORM (privtype, DESPriv))
    {
        ...
        rval = pkcs_encrpyt (CKM_DES_CBC, pkcs_des_key,
                             sizeof (pkcs_des_key), iv, ivlen, plaintext, ptlen, ciphertext, ctlen);
    }
...
sc_decrypt (const oid * privtype, size_t privtypelen,
            u_char * key, u_int keylen,
            u_char * iv, u_int ivlen, u_char * ciphertext, u_int ctlen, u_char * plaintext, size_t * ptlen)
#if defined(NETSNMP_USE_OPENSSL) || defined(NETSNMP_USE_INTERNAL_CRYPTO)
{
    ...
    if (!privtype || !key || !iv || !plaintext || !ciphertext || !ptlen
        || (ctlen <= 0) || (*ptlen <= 0) || (*ptlen < ctlen) || (privtypelen != USM_LENGTH_OID_TRANSFORM))
    {
    ...
    if (ISTRANSFORM (privtype, DESPriv))
    {
        ...
        memcpy (my_iv, iv, ivlen);
        DES_cbc_encrypt (ciphertext, plaintext, ctlen, key_sch, (DES_cblock *) my_iv, DES_DECRYPT);
        *ptlen = ctlen;
    }
#endif
#ifdef HAVE_AES
    if (ISTRANSFORM (privtype, AESPriv))
    {
         ...
         * encrypt the data 
         */
        AES_cfb128_encrypt (ciphertext, plaintext, ctlen, &aes_key, my_iv, &new_ivlen, AES_DECRYPT);
        *ptlen = ctlen;
    }
#endif
...
#elif NETSNMP_USE_PKCS11        /* USE PKCS */
{
    ...
    if (ISTRANSFORM (privtype, DESPriv))
    {
        ...
        rval = pkcs_decrpyt (CKM_DES_CBC, pkcs_des_key,
                             sizeof (pkcs_des_key), iv, ivlen, ciphertext, ctlen, plaintext, ptlen);
        *ptlen = ctlen;
    }
 ...
 */
static int
MD5_hmac (const u_char * data, size_t len, u_char * mac, size_t maclen, const u_char * secret, size_t secretlen)
{
...
#define MD5_SECRETKEYLEN 16
    ...
    if (secretlen != MD5_SECRETKEYLEN || secret == NULL || mac == NULL || data == NULL || len <= 0 || maclen <= 0)
    {
        /*
...
static int
SHA1_hmac (const u_char * data, size_t len, u_char * mac, size_t maclen, const u_char * secret, size_t secretlen)
{
...
#define SHA1_SECRETKEYLEN 20
    ...
    if (secretlen != SHA1_SECRETKEYLEN || secret == NULL || mac == NULL || data == NULL || len <= 0 || maclen <= 0)
    {
        /*
