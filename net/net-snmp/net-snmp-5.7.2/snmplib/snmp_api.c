...
static int snmp_resend_request (struct session_list *slp, netsnmp_request_list * rp, int incr_retries);
...
void snmp_error (netsnmp_session * psess, int *p_errno, int *p_snmp_errno, char **p_str)
{
    ...
    if (snmp_errnumber >= SNMPERR_MAX && snmp_errnumber <= SNMPERR_GENERR)
    {
        if (snmp_detail_f)
        {
            snprintf (buf, sizeof (buf), "%s (%s)", api_errors[-snmp_errnumber], snmp_detail);
            buf[sizeof (buf) - 1] = '\0';
            snmp_detail_f = 0;
        }
        else
...
static void _init_snmp (void)
{
    ...
    netsnmp_ds_set_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_HEX_OUTPUT_LENGTH, 16);
    netsnmp_ds_set_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_RETRIES, DEFAULT_RETRIES);

#ifdef NETSNMP_USE_REVERSE_ASNENCODING
    netsnmp_ds_set_boolean (NETSNMP_DS_LIBRARY_ID,
                            NETSNMP_DS_LIB_REVERSE_ENCODE, NETSNMP_DEFAULT_ASNENCODING_DIRECTION);
#endif
}
...
static void register_default_handlers (void)
{
    ...
    netsnmp_ds_register_premib (ASN_OCTET_STR, "snmp", "clientaddr", NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_CLIENT_ADDR);
    ...
    netsnmp_ds_register_config (ASN_BOOLEAN, "snmp",
                                "noContextEngineIDDiscovery", NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_NO_DISCOVERY);
    netsnmp_ds_register_config (ASN_INTEGER, "snmp", "timeout", NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_TIMEOUT);
    netsnmp_ds_register_config (ASN_INTEGER, "snmp", "retries", NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_RETRIES);
...
void init_snmp (const char *type)
{
     ...
     */
    if (type && !netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_APPTYPE))
    {
        netsnmp_ds_set_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_APPTYPE, type);
    }
...
void snmp_store_if_needed (void)
{
    ...
    snmp_store (netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_APPTYPE));
...
static struct session_list *_sess_copy (netsnmp_session * in_session)
{
    ...
    {
        if ((cp = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_COMMUNITY)) != NULL)
        {
    ...
    {
        session->securityLevel = netsnmp_ds_get_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SECLEVEL);
    }
    ...
    if (in_session->securityEngineIDLen > 0)
    {
        ...
        memmove (ucp, in_session->securityEngineID, in_session->securityEngineIDLen);
    ...
    if (in_session->contextEngineIDLen > 0)
    {
        ...
        memmove (ucp, in_session->contextEngineID, in_session->contextEngineIDLen);
    ...
    {
        if ((cp = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_CONTEXT)) != NULL)
            cp = strdup (cp);
        else
    ...
    else if ((cp = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SECNAME)) != NULL)
    {
        cp = strdup (cp);
        if (cp == NULL)
        {
    ...
    snmp_call_callbacks (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_SESSION_INIT, session);
...
int snmpv3_probe_contextEngineID_rfc5343 (void *slp, netsnmp_session * session)
{
    ...
    if (memdup (&pdu->contextEngineID, probeEngineID, probeEngineID_len) != SNMPERR_SUCCESS)
    {
    ...
    if (NULL != response->variables &&
        ...
        NULL != response->variables->val.string && response->variables->val_len > 0)
    {
        if (memdup (&session->contextEngineID,
                    response->variables->val.string, response->variables->val_len) != SNMPERR_SUCCESS)
        {
        ...
        session->securityEngineIDLen = session->contextEngineIDLen = response->variables->val_len;
        ...
        if (snmp_get_do_debugging ())
        {
            ...
            for (i = 0; i < session->securityEngineIDLen; i++)
                DEBUGMSG (("snmp_sess_open", "%02x", session->securityEngineID[i]));
...
int snmpv3_engineID_probe (struct session_list *slp, netsnmp_session * in_session)
{
    ...
    if (session->version == SNMP_VERSION_3 && (0 == (session->flags & SNMP_FLAGS_DONT_PROBE)))
    {
...
int netsnmp_sess_config_transport (netsnmp_container * transport_configuration, netsnmp_transport * transport)
{
    ...
    if (transport_configuration)
    {
        ...
        if (transport->f_config)
        {
            ...
            {
                ret = transport->f_config (transport, config_data->key, config_data->value);
                if (ret)
                {
...
int netsnmp_sess_config_and_open_transport (netsnmp_session * in_session, netsnmp_transport * transport)
{
    ...
    if ((rc = netsnmp_sess_config_transport (in_session->transport_configuration, transport)) != SNMPERR_SUCCESS)
    {
...
static void *_sess_open (netsnmp_session * in_session)
{
...

    {
        ...
        if (NULL != in_session->localname)
        {
            ...
            netsnmp_ds_set_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_CLIENT_ADDR, in_session->localname);
        }

        if (in_session->flags & SNMP_FLAGS_STREAM_SOCKET)
        {
            transport =
                netsnmp_tdomain_transport_full ("snmp", in_session->peername, in_session->local_port, "tcp", NULL);
        }
        else
        {
            transport =
                netsnmp_tdomain_transport_full ("snmp", in_session->peername, in_session->local_port, "udp", NULL);
        }

        if (NULL != clientaddr_save)
            netsnmp_ds_set_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_CLIENT_ADDR, clientaddr_save);
    }
    ...
    /* Optional supplimental transport configuration information and
       final call to actually open the transport */
    if ((rc = netsnmp_sess_config_and_open_transport (in_session, transport)) != SNMPERR_SUCCESS)
    {
        transport = NULL;
        return NULL;
    }
    ...
    if (in_session->flags & SNMP_FLAGS_UDP_BROADCAST)
    {
        ...
        rc = setsockopt (transport->sock, SOL_SOCKET, SO_BROADCAST, (char *) &b, sizeof (b));
                                              ...
                                              int), int (*fpost_parse) (netsnmp_session *, netsnmp_pdu *, int))
{
    ...
    slp = (struct session_list *) snmp_sess_add_ex (in_session, transport,
                                                    fpre_parse, NULL, fpost_parse, NULL, NULL, NULL, NULL);
    if (slp == NULL)
    {
        return NULL;
    }
                        ...
                        netsnmp_pdu * (*fcreate_pdu) (netsnmp_transport *, void *, size_t))
{
    ...
    {
        if (SNMPERR_SUCCESS != transport->f_setup_session (transport, in_session))
        {
            netsnmp_transport_free (transport);
            return NULL;
        }
    ...
    {
        DEBUGMSGTL (("snmp_sess_add", "adding v3 session -- maybe engineID probe now\n"));
...
void *snmp_sess_add (netsnmp_session * in_session,
                     netsnmp_transport * transport,
                     int (*fpre_parse) (netsnmp_session *, netsnmp_transport *,
                                        void *, int), int (*fpost_parse) (netsnmp_session *, netsnmp_pdu *, int))
{
    return snmp_sess_add_ex (in_session, transport, fpre_parse, NULL, fpost_parse, NULL, NULL, NULL, NULL);
}
...
int snmp_sess_close (void *sessp)
{
    ...
    if (slp->session != NULL &&
        (sptr = find_sec_mod (slp->session->securityModel)) != NULL && sptr->session_close != NULL)
    {
    ...
    if (isp)
    {
        ...
        while (rp)
        {
            ...
            if (orp->callback)
            {
                orp->callback (NETSNMP_CALLBACK_OP_TIMED_OUT, slp->session, orp->pdu->reqid, orp->pdu, orp->cb_data);
            }
...
static int snmpv3_verify_msg (netsnmp_request_list * rp, netsnmp_pdu * pdu)
{
    ...
    if (rpdu->contextEngineIDLen != pdu->contextEngineIDLen ||
        memcmp (rpdu->contextEngineID, pdu->contextEngineID, pdu->contextEngineIDLen))
        return 0;
 ...
 */
static int snmpv3_build (u_char ** pkt, size_t * pkt_len, size_t * offset, netsnmp_session * session, netsnmp_pdu * pdu)
{
    int ret;
    ...
    if (pdu->securityEngineIDLen == 0)
    {
        if (session->securityEngineIDLen)
        {
            snmpv3_clone_engineID (&pdu->securityEngineID,
                                   &pdu->securityEngineIDLen, session->securityEngineID, session->securityEngineIDLen);
        }
    }
    ...
    if (pdu->contextEngineIDLen == 0)
    {
        if (session->contextEngineIDLen)
        {
            snmpv3_clone_engineID (&pdu->contextEngineID,
                                   &pdu->contextEngineIDLen, session->contextEngineID, session->contextEngineIDLen);
        }
        else if (pdu->securityEngineIDLen)
        {
            snmpv3_clone_engineID (&pdu->contextEngineID,
                                   &pdu->contextEngineIDLen, pdu->securityEngineID, pdu->securityEngineIDLen);
        }
    }
    ...
    if (pdu->securityModel == SNMP_DEFAULT_SECMODEL)
    {
        ...
        {
            pdu->securityModel =
                se_find_value_in_slist ("snmp_secmods",
                                        netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SECMODEL));
    ...
    {
        if (session->securityModel != SNMP_SEC_MODEL_TSM && session->securityNameLen == 0)
        {
    ...
    {
        ret = snmpv3_packet_realloc_rbuild (pkt, pkt_len, offset, session, pdu, NULL, 0);
    }
    else
    {
#endif
...
static u_char *snmpv3_header_build (netsnmp_session * session, netsnmp_pdu * pdu,
                                    u_char * packet, size_t * out_length, size_t length, u_char ** msg_hdr_e)
{
     ...
     */
    cp = asn_build_sequence (packet, out_length, (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), length);
    if (cp == NULL)
        return NULL;
    if (msg_hdr_e != NULL)
        *msg_hdr_e = cp;
    ...
    cp = asn_build_int (cp, out_length,
                        (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE |
                                  ASN_INTEGER), (long *) &pdu->version, sizeof (pdu->version));
    DEBUGINDENTLESS ();
    if (cp == NULL)
        return NULL;
    ...
    cp = asn_build_sequence (cp, out_length, (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), 0);
    if (cp == NULL)
        return NULL;
    global_hdr_e = cp;
    ...
    cp = asn_build_int (cp, out_length,
                        (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &pdu->msgid, sizeof (pdu->msgid));
    DEBUGINDENTLESS ();
    if (cp == NULL)
        return NULL;
    ...
    cp = asn_build_int (cp, out_length,
                        (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &max_size, sizeof (max_size));
    DEBUGINDENTLESS ();
    if (cp == NULL)
        return NULL;
    ...
    cp = asn_build_string (cp, out_length,
                           (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), &msg_flags, sizeof (msg_flags));
    DEBUGINDENTLESS ();
    if (cp == NULL)
        return NULL;
    ...
    cp = asn_build_int (cp, out_length,
                        (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &sec_model, sizeof (sec_model));
     ...
     */
    pb = asn_build_sequence (global_hdr, out_length, (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), cp - global_hdr_e);
    if (pb == NULL)
        return NULL;
     ...
     */
    pb = asn_build_sequence (packet, out_length, (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), length + (cp - pb0e));
    if (pb == NULL)
        return NULL;
                              ...
                              size_t * offset, netsnmp_session * session, netsnmp_pdu * pdu)
{
    ...
    rc = asn_realloc_rbuild_int (pkt, pkt_len, offset, 1,
                                 (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE |
                                           ASN_INTEGER), &sec_model, sizeof (sec_model));
    DEBUGINDENTLESS ();
    if (rc == 0)
    {
        return 0;
    ...
    rc = asn_realloc_rbuild_string (pkt, pkt_len, offset, 1,
                                    (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE
                                              | ASN_OCTET_STR), &msg_flags, sizeof (msg_flags));
    DEBUGINDENTLESS ();
    if (rc == 0)
    {
        return 0;
    ...
    rc = asn_realloc_rbuild_int (pkt, pkt_len, offset, 1,
                                 (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE |
                                           ASN_INTEGER), &pdu->msgid, sizeof (pdu->msgid));
    DEBUGINDENTLESS ();
    if (rc == 0)
    {
        return 0;
    ...
    rc = asn_realloc_rbuild_sequence (pkt, pkt_len, offset, 1,
                                      (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), *offset - start_offset);
    if (rc == 0)
    {
        return 0;
    }
...
static u_char *snmpv3_scopedPDU_header_build (netsnmp_pdu * pdu, u_char * packet, size_t * out_length, u_char ** spdu_e)
{
    u_char *scopedPdu, *pb;

    pb = scopedPdu = packet;
    pb = asn_build_sequence (pb, out_length, (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), 0);
    if (pb == NULL)
        return NULL;
    if (spdu_e)
        *spdu_e = pb;
...
snmpv3_scopedPDU_header_realloc_rbuild (u_char ** pkt, size_t * pkt_len,
                                        size_t * offset, netsnmp_pdu * pdu, size_t body_len)
{
    size_t start_offset = *offset;
    ...
    rc = asn_realloc_rbuild_string (pkt, pkt_len, offset, 1,
                                    (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE
                                              | ASN_OCTET_STR), (u_char *) pdu->contextName, pdu->contextNameLen);
    DEBUGINDENTLESS ();
    if (rc == 0)
    {
        return 0;
    ...
    rc = asn_realloc_rbuild_string (pkt, pkt_len, offset, 1,
                                    (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE
                                              | ASN_OCTET_STR), pdu->contextEngineID, pdu->contextEngineIDLen);
    DEBUGINDENTLESS ();
    if (rc == 0)
    {
        return 0;
    ...
    rc = asn_realloc_rbuild_sequence (pkt, pkt_len, offset, 1,
                                      (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), *offset - start_offset + body_len);
...
snmpv3_packet_realloc_rbuild (u_char ** pkt, size_t * pkt_len,
                              ...
                              netsnmp_pdu * pdu, u_char * pdu_data, size_t pdu_data_len)
{
    ...
    rc = snmpv3_scopedPDU_header_realloc_rbuild (pkt, pkt_len, offset, pdu, body_len);
    if (rc == 0)
    {
        return -1;
    }
    ...
    rc = snmpv3_header_realloc_rbuild (&hdrbuf, &hdrbuf_len, &hdr_offset, session, pdu);
    if (rc == 0)
    {
        SNMP_FREE (hdrbuf);
        return -1;
    ...
    else
    {
        ...
        {
            snmp_log (LOG_ERR, "security service %d doesn't support reverse encoding.\n", pdu->securityModel);
        }
        rc = -1;
    }
...
snmpv3_packet_build (netsnmp_session * session, netsnmp_pdu * pdu,
                     u_char * packet, size_t * out_length, u_char * pdu_data, size_t pdu_data_len)
{
     ...
     */
    sec_params = snmpv3_header_build (session, pdu, global_data, out_length, 0, NULL);
    if (sec_params == NULL)
        return -1;
...
static int _snmp_build (u_char ** pkt, size_t * pkt_len, size_t * offset, netsnmp_session * session, netsnmp_pdu * pdu)
{
#if !defined(NETSNMP_DISABLE_SNMPV1) || !defined(NETSNMP_DISABLE_SNMPV2C)
    u_char *h0e = NULL;
    ...
    switch (pdu->command)
    {
        ...
        case SNMP_MSG_TRAP:
            ...
            if (pdu->enterprise_length == SNMP_DEFAULT_ENTERPRISE_LENGTH)
            {
                ...
                memmove (pdu->enterprise, DEFAULT_ENTERPRISE, sizeof (DEFAULT_ENTERPRISE));
    ...
    switch (pdu->version)
    {
        ...
        case SNMP_VERSION_2c:
            ...
            if (netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_REVERSE_ENCODE))
            {
                ...
                rc = asn_realloc_rbuild_string (pkt, pkt_len, offset, 1,
                                                (u_char) (ASN_UNIVERSAL |
                                                          ASN_PRIMITIVE |
                                                          ASN_OCTET_STR), pdu->community, pdu->community_len);
                DEBUGINDENTLESS ();
                if (rc == 0)
                {
                    return -1;
                ...
                rc = asn_realloc_rbuild_int (pkt, pkt_len, offset, 1,
                                             (u_char) (ASN_UNIVERSAL |
                                                       ASN_PRIMITIVE |
                                                       ASN_INTEGER), (long *) &version, sizeof (version));
                DEBUGINDENTLESS ();
                if (rc == 0)
                {
                    return -1;
            ...
            else
            {
                 ...
                 */
                cp = asn_build_sequence (*pkt, pkt_len, (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), 0);
                if (cp == NULL)
                {
                    return -1;
                }
...
#endif
#if !defined(NETSNMP_DISABLE_SNMPV1) || !defined(NETSNMP_DISABLE_SNMPV2C)
            asn_build_sequence (*pkt, &length, (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), cp - h0e);
            break;
#endif                            /* support for community based SNMP */
...
u_char *snmp_pdu_build (netsnmp_pdu * pdu, u_char * cp, size_t * out_length)
{
    ...
    if (pdu->command != SNMP_MSG_TRAP)
    {
         ...
         */
        cp = asn_build_int (cp, out_length,
                            (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &pdu->reqid, sizeof (pdu->reqid));
        DEBUGINDENTLESS ();
        if (cp == NULL)
            return NULL;
        ...
        cp = asn_build_int (cp, out_length,
                            (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE |
                                      ASN_INTEGER), &pdu->errstat, sizeof (pdu->errstat));
        DEBUGINDENTLESS ();
        if (cp == NULL)
            return NULL;
        ...
        cp = asn_build_int (cp, out_length,
                            (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE |
                                      ASN_INTEGER), &pdu->errindex, sizeof (pdu->errindex));
        DEBUGINDENTLESS ();
        if (cp == NULL)
            return NULL;
    }
    else
    {
        ...
        cp = asn_build_objid (cp, out_length,
                              (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE |
                                        ASN_OBJECT_ID), (oid *) pdu->enterprise, pdu->enterprise_length);
        DEBUGINDENTLESS ();
        if (cp == NULL)
            return NULL;
        ...
        cp = asn_build_string (cp, out_length, (u_char) (ASN_IPADDRESS | ASN_PRIMITIVE), (u_char *) pdu->agent_addr, 4);
        DEBUGINDENTLESS ();
        if (cp == NULL)
            return NULL;
        ...
        cp = asn_build_int (cp, out_length,
                            (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE |
                                      ASN_INTEGER), (long *) &pdu->trap_type, sizeof (pdu->trap_type));
        DEBUGINDENTLESS ();
        if (cp == NULL)
            return NULL;
        ...
        cp = asn_build_int (cp, out_length,
                            (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE |
                                      ASN_INTEGER), (long *) &pdu->specific_type, sizeof (pdu->specific_type));
        DEBUGINDENTLESS ();
        if (cp == NULL)
            return NULL;
        ...
        cp = asn_build_unsigned_int (cp, out_length,
                                     (u_char) (ASN_TIMETICKS | ASN_PRIMITIVE), &pdu->time, sizeof (pdu->time));
        DEBUGINDENTLESS ();
        if (cp == NULL)
            return NULL;
    }
...
#define VPCACHE_SIZE 50
    ...
    for (vp = pdu->variables, i = VPCACHE_SIZE - 1; vp; vp = vp->next_variable, i--)
    {
        if (i < 0)
        {
            wrapped = notdone = 1;
    ...
    do
    {
        ...
        if (wrapped)
        {
            ...
            for (vp = pdu->variables, i = VPCACHE_SIZE - 1; vp && vp != tmpvp; vp = vp->next_variable, i--)
            {
                if (i < 0)
                {
                    wrapped = 1;
    ...
    if (pdu->command != SNMP_MSG_TRAP)
    {
        ...
        rc = asn_realloc_rbuild_int (pkt, pkt_len, offset, 1,
                                     (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE
                                               | ASN_INTEGER), &pdu->reqid, sizeof (pdu->reqid));
        DEBUGINDENTLESS ();
        if (rc == 0)
        {
            return 0;
    ...
    else
    {
        ...
        DEBUGDUMPHEADER ("send", "agent Address");
        rc = asn_realloc_rbuild_string (pkt, pkt_len, offset, 1,
                                        (u_char) (ASN_IPADDRESS | ASN_PRIMITIVE), (u_char *) pdu->agent_addr, 4);
        DEBUGINDENTLESS ();
        if (rc == 0)
        {
            return 0;
     ...
     */
    rc = asn_realloc_rbuild_sequence (pkt, pkt_len, offset, 1, (u_char) pdu->command, *offset - start_offset);
    return rc;
}
#endif                            /* NETSNMP_USE_REVERSE_ASNENCODING */
...
static int snmp_parse_version (u_char * data, size_t length)
{
    ...
    data = asn_parse_sequence (data, &length, &type, (ASN_SEQUENCE | ASN_CONSTRUCTOR), "version");
    if (data)
    {
...
int snmpv3_parse (netsnmp_pdu * pdu, u_char * data, size_t * length, u_char ** after_header, netsnmp_session * sess)
{
    ...
    data = asn_parse_sequence (data, length, &type, (ASN_SEQUENCE | ASN_CONSTRUCTOR), "message");
    if (data == NULL)
    {
        /*
    ...
    *length -= data - cp;        /* subtract off the length of the header */
    ...
    data = asn_parse_int (data, length, &type, &msg_max_size, sizeof (msg_max_size));
    DEBUGINDENTLESS ();
    if (data == NULL || type != ASN_INTEGER)
    {
    ...
    data = asn_parse_int (data, length, &type, &msg_sec_model, sizeof (msg_sec_model));
    DEBUGINDENTADD (-4);        /* return from global data indent */
    if (data == NULL || type != ASN_INTEGER || msg_sec_model < 1 || msg_sec_model > 0x7fffffff)
    {
    ...
    if (msg_flags & SNMP_MSG_FLAG_PRIV_BIT && !(msg_flags & SNMP_MSG_FLAG_AUTH_BIT))
    {
    ...
    pdu->securityLevel = ((msg_flags & SNMP_MSG_FLAG_AUTH_BIT)
                          ? ((msg_flags & SNMP_MSG_FLAG_PRIV_BIT)
                             ? SNMP_SEC_LEVEL_AUTHPRIV : SNMP_SEC_LEVEL_AUTHNOPRIV) : SNMP_SEC_LEVEL_NOAUTH);
    /*
...
int snmpv3_make_report (netsnmp_pdu * pdu, int error)
{
     ...
     */
    snmp_pdu_add_variable (pdu, err_var, err_var_len, ASN_COUNTER, &ltmp, sizeof (ltmp));
 ...
 */
static int _snmp_parse (void *sessp, netsnmp_session * session, netsnmp_pdu * pdu, u_char * data, size_t length)
{
#if !defined(NETSNMP_DISABLE_SNMPV1) || !defined(NETSNMP_DISABLE_SNMPV2C)
    ...
    switch (pdu->version)
    {
        ...
        case SNMP_VERSION_2c:
            ...
            if (pdu->version != session->version && session->version != SNMP_DEFAULT_VERSION)
            {
                session->s_snmp_errno = SNMPERR_BAD_VERSION;
                return -1;
            }
        ...
        case SNMP_VERSION_3:
            ...
            DEBUGMSGTL (("snmp_parse",
                         ...
                         pdu->securityName, secLevelName[pdu->securityLevel], snmp_api_errstring (result)));

            if (result)
            {
                struct snmp_secmod_def *secmod = find_sec_mod (pdu->securityModel);
                ...
                else
                {
                    ...
                    if (secmod && secmod->handle_report)
                    {
                        ...
                        (*secmod->handle_report) (sessp, slp->transport, session, result, pdu);
                    }
                }
            ...
            if (!netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID,
                ...
                pdu->contextEngineID[2] == 0x00 && pdu->contextEngineID[3] == 0x00 && pdu->contextEngineID[4] == 0x06)
            {
                ...
                if (NULL != pdu->variables && NULL == pdu->variables->next_variable &&
                    /* if it's a GET, match it exactly */
                    ((SNMP_MSG_GET == pdu->command &&
                      snmp_oid_compare (snmpEngineIDoid,
                                        snmpEngineIDoid_len, pdu->variables->name, pdu->variables->name_length) == 0)
                                        ...
                                        snmpEngineIDoid_len, pdu->variables->name, pdu->variables->name_length) > 0)))
                {
                    ...
                    ourEngineID_len = snmpv3_get_engineID ((u_char *) ourEngineID, ourEngineID_len);
                    if (0 != ourEngineID_len)
                    {

                        DEBUGMSGTL (("snmpv3_contextid", "  responding with our engineID\n"));
                        ...
                        if (0 == snmp_sess_send (sessp, pdu2))
                        {
                            ...
                            DEBUGMSGTL (("snmpv3_contextid", "  sent it off!\n"));
                ...
                {
                    snmp_log (LOG_WARNING, "received an odd context engineID probe\n");
                }
            }
...
static int snmp_parse (void *sessp, netsnmp_session * pss, netsnmp_pdu * pdu, u_char * data, size_t length)
{
    int rc;
...
int snmp_pdu_parse (netsnmp_pdu * pdu, u_char * data, size_t * length)
{
    ...
    switch (pdu->command)
    {
        case SNMP_MSG_TRAP:
             ...
             */
            four = 4;
            data = asn_parse_string (data, length, &type, (u_char *) pdu->agent_addr, &four);
            if (data == NULL)
                return -1;
    ...
    data = asn_parse_sequence (data, length, &type, (ASN_SEQUENCE | ASN_CONSTRUCTOR), "varbinds");
    if (data == NULL)
        return -1;
    ...
    while ((int) *length > 0)
    {
        ...
        data = snmp_parse_var_op (data, objid, &vp->name_length, &vp->type, &vp->val_len, &var_val, length);
        if (data == NULL)
            return -1;
        ...
        switch ((short) vp->type)
        {
            ...
            case ASN_UINTEGER:
                ...
                asn_parse_unsigned_int (var_val, &len, &vp->type, (u_long *) vp->val.integer, vp->val_len);
                break;
            ...
            case ASN_OPAQUE_FLOAT:
                ...
                asn_parse_float (var_val, &len, &vp->type, vp->val.floatVal, vp->val_len);
                break;
            case ASN_OPAQUE_DOUBLE:
                vp->val.doubleVal = (double *) vp->buf;
                vp->val_len = sizeof (double);
                asn_parse_double (var_val, &len, &vp->type, vp->val.doubleVal, vp->val_len);
                break;
            case ASN_OPAQUE_I64:
                vp->val.counter64 = (struct counter64 *) vp->buf;
                vp->val_len = sizeof (struct counter64);
                asn_parse_signed_int64 (var_val, &len, &vp->type,
                                        (struct counter64 *) vp->val.counter64, sizeof (*vp->val.counter64));
            ...
            case ASN_BIT_STR:
                ...
                asn_parse_bitstring (var_val, &len, &vp->type, vp->val.bitstring, &vp->val_len);
                break;
            default:
...
u_char *snmpv3_scopedPDU_parse (netsnmp_pdu * pdu, u_char * cp, size_t * length)
{
    ...
    asn_len = *length;
    data = asn_parse_sequence (cp, &asn_len, &type, (ASN_SEQUENCE | ASN_CONSTRUCTOR), "plaintext scopedPDU");
    if (data == NULL)
    {
        return NULL;
    }
    *length -= data - cp;
    ...
    data = asn_parse_string (data, length, &type, pdu->contextEngineID, &pdu->contextEngineIDLen);
    DEBUGINDENTLESS ();
    if (data == NULL)
    {
...
static int _sess_async_send (void *sessp, netsnmp_pdu * pdu, snmp_callback callback, void *cb_data)
{
    ...
    if ((pdu->version == SNMP_VERSION_3) &&
        (pdu->flags & UCD_MSG_FLAG_EXPECT_RESPONSE) &&
        (session->securityEngineIDLen == 0) && (0 == (session->flags & SNMP_FLAGS_DONT_PROBE)))
    {
        int rc;
    ...
    {
        result = isp->hook_realloc_build (session, pdu, &pktbuf, &pktbuf_len, &offset);
        packet = pktbuf;
        length = offset;
    }
    ...
    result = netsnmp_transport_send (transport, packet, length, &(pdu->transport_data), &(pdu->transport_data_length));
...
void snmp_free_pdu (netsnmp_pdu * pdu)
{
     ...
     */
    if ((sptr = find_sec_mod (pdu->securityModel)) != NULL && sptr->pdu_free != NULL)
    {
        (*sptr->pdu_free) (pdu);
    }
...
_sess_process_packet (void *sessp, netsnmp_session * sp,
                      struct snmp_internal_session *isp,
                      netsnmp_transport * transport, void *opaque, int olength, u_char * packetptr, int length)
{
    struct session_list *slp = (struct session_list *) sessp;
    ...
    if (pdu->flags & UCD_MSG_FLAG_RESPONSE_PDU)
    {
        ...
        for (rp = isp->requests; rp; orp = rp, rp = rp->next_request)
        {
            ...
            if (callback == NULL || callback (NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE, sp, pdu->reqid, pdu, magic) == 1)
            {
                if (pdu->command == SNMP_MSG_REPORT)
                {
                    ...
                    else
                    {
                        ...
                        {        /* Set the state to no longer be waiting, since we're done with retries */
                            ((struct synch_state *) magic)->waiting = 0;
                        }
                    }
                    ...
                    if (!sp->securityEngineIDLen && pdu->securityEngineIDLen)
                    {
                        ...
                        memcpy (sp->securityEngineID, pdu->securityEngineID, pdu->securityEngineIDLen);
                        ...
                        if (!sp->contextEngineIDLen)
                        {
                            ...
                            memcpy (sp->contextEngineID, pdu->securityEngineID, pdu->securityEngineIDLen);
    ...
    else
    {
        if (sp->callback)
        {
             ...
             */
            handled = 1;
            sp->callback (NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE, sp, pdu->reqid, pdu, sp->callback_magic);
            /*
...
int _sess_read (void *sessp, netsnmp_large_fd_set * fdset)
{
    ...
    if (!fdset || !(NETSNMP_LARGE_FD_ISSET (transport->sock, fdset)))
    {
        DEBUGMSGTL (("sess_read", "not reading %d (fdset %p set %d)\n",
                     transport->sock, fdset, fdset ? NETSNMP_LARGE_FD_ISSET (transport->sock, fdset) : -9));
        return 0;
    }
    ...
    length = netsnmp_transport_recv (transport, rxbuf, rxbuf_len, &opaque, &olength);
    ...
    if (length <= 0 && transport->flags & NETSNMP_TRANSPORT_FLAG_STREAM)
    {
        ...
        if (sp->callback != NULL)
        {
            ...
            (void) sp->callback (NETSNMP_CALLBACK_OP_DISCONNECT, sp, 0, NULL, sp->callback_magic);
        }
        /*
    ...
    if (transport->flags & NETSNMP_TRANSPORT_FLAG_STREAM)
    {
        ...
        while (isp->packet_len > 0)
        {
            ...
            if ((rc = _sess_process_packet (sessp, sp, isp, transport, ocopy, ocopy ? olength : 0, pptr, pdulen)))
            {
                /*
        ...
        DEBUGMSGTL (("sess_read", "end: memmove(%p, %p, %lu); realloc(%p, %lu)\n",
                     isp->packet, pptr, (unsigned long) isp->packet_len, isp->packet, (unsigned long) isp->packet_len));
    ...
    {
        rc = _sess_process_packet (sessp, sp, isp, transport, opaque, olength, rxbuf, length);
        SNMP_FREE (rxbuf);
        return rc;
    }
}
...
{
    return snmp_sess_select_info_flags (sessp, numfds, fdset, timeout, block, NETSNMP_SELECT_NOFLAGS);
}
 ...
 */
int
snmp_sess_select_info_flags (void *sessp, int *numfds, fd_set * fdset, struct timeval *timeout, int *block, int flags)
{
    int rc;
    ...
    netsnmp_large_fd_set_init (&lfdset, FD_SETSIZE);
    netsnmp_copy_fd_set_to_large_fd_set (&lfdset, fdset);
    rc = snmp_sess_select_info2_flags (sessp, numfds, &lfdset, timeout, block, flags);
    if (netsnmp_copy_large_fd_set_to_fd_set (fdset, &lfdset) < 0)
    {
        snmp_log (LOG_ERR, "Use snmp_sess_select_info2() for processing" " large file descriptors\n");
    }
    netsnmp_large_fd_set_cleanup (&lfdset);
    return rc;
}
...
{
    return snmp_sess_select_info2_flags (sessp, numfds, fdset, timeout, block, NETSNMP_SELECT_NOFLAGS);
}
...
snmp_sess_select_info2_flags (void *sessp, int *numfds,
                              netsnmp_large_fd_set * fdset, struct timeval *timeout, int *block, int flags)
{
    ...
    DEBUGMSGTL (("sess_select", "for %s session%s: ", sessp ? "single" : "all", sessp ? "" : "s"));
    ...
    for (slp = sessp ? sessp : Sessions; slp; slp = next)
    {
        ...
        if (slp->internal != NULL && slp->internal->requests)
        {
            ...
            {
                if (!timerisset (&earliest) || (timerisset (&rp->expireM) && timercmp (&rp->expireM, &earliest, <)))
                {
                    earliest = rp->expireM;
    ...
    if (netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID,
                                NETSNMP_DS_LIB_ALARM_DONT_USE_SIG) && !(flags & NETSNMP_SELECT_NOALARMS))
    {
...
static int snmp_resend_request (struct session_list *slp, netsnmp_request_list * rp, int incr_retries)
{
    ...
    if (isp->hook_realloc_build)
    {
        result = isp->hook_realloc_build (sp, rp->pdu, &pktbuf, &pktbuf_len, &offset);
    ...
    result = netsnmp_transport_send (transport, packet, length,
                                     &(rp->pdu->transport_data), &(rp->pdu->transport_data_length));
...
void snmp_sess_timeout (void *sessp)
{
    ...
    for (rp = isp->requests; rp; rp = rp->next_request)
    {
        ...
        {
            if ((sptr = find_sec_mod (rp->pdu->securityModel)) != NULL && sptr->pdu_timeout != NULL)
            {
                /*
            ...
            if (rp->retries >= sp->retries)
            {
                ...
                if (callback)
                {
                    callback (NETSNMP_CALLBACK_OP_TIMED_OUT, sp, rp->pdu->reqid, rp->pdu, magic);
                }
                if (isp->requests == rp)
                {
                    isp->requests = rp->next_request;
...
static int _check_range (struct tree *tp, long ltmp, int *resptr, const char *errmsg)
{
    char *cp = NULL;
...
netsnmp_variable_list *snmp_pdu_add_variable (netsnmp_pdu * pdu,
                                              const oid * name,
                                              size_t name_length, u_char type, const void *value, size_t len)
{
    return snmp_varlist_add_variable (&pdu->variables, name, name_length, type, value, len);
}
                                                  ...
                                                  size_t name_length, u_char type, const void *value, size_t len)
{
    ...
    if ((0 != rc) || (name != NULL && snmp_set_var_objid (vars, name, name_length)))
    {
        snmp_free_var (vars);
        return NULL;
    }
    ...
    switch (type)
    {
        case 'i':
            ...
            if (!_check_range (tp, ltmp, &result, value))
                break;
#endif                            /* NETSNMP_DISABLE_MIB_LOADING */
            snmp_pdu_add_variable (pdu, name, name_length, ASN_INTEGER, &ltmp, sizeof (ltmp));
            break;
        ...
        case 'u':
            ...
            if (*value && !*ecp)
                snmp_pdu_add_variable (pdu, name, name_length, ASN_UNSIGNED, &ltmp, sizeof (ltmp));
            else
                goto fail;
            break;
        ...
        case '3':
            ...
            ltmp = strtoul (value, &ecp, 10);
            if (*value && !*ecp)
                snmp_pdu_add_variable (pdu, name, name_length, ASN_UINTEGER, &ltmp, sizeof (ltmp));
            else
                goto fail;
            break;
        ...
        case 'c':
            ...
            ltmp = strtoul (value, &ecp, 10);
            if (*value && !*ecp)
                snmp_pdu_add_variable (pdu, name, name_length, ASN_COUNTER, &ltmp, sizeof (ltmp));
            else
                goto fail;
            break;
        ...
        case 'C':
            ...
            if (read64 (&c64tmp, value))
                snmp_pdu_add_variable (pdu, name, name_length, ASN_COUNTER64, &c64tmp, sizeof (c64tmp));
            else
                goto fail;
            break;
        ...
        case 't':
            ...
            ltmp = strtoul (value, &ecp, 10);
            if (*value && !*ecp)
                snmp_pdu_add_variable (pdu, name, name_length, ASN_TIMETICKS, &ltmp, sizeof (long));
            else
                goto fail;
            break;
        ...
        case 'a':
            ...
            if (atmp != (in_addr_t) - 1 || !strcmp (value, "255.255.255.255"))
                snmp_pdu_add_variable (pdu, name, name_length, ASN_IPADDRESS, &atmp, sizeof (atmp));
            else
                goto fail;
            break;
        ...
        case 'o':
            ...
            else
            {
                ...
                {
                    snmp_pdu_add_variable (pdu, name, name_length, ASN_OBJECT_ID, buf, sizeof (oid) * tint);
                }
                else
                {
        ...
        case 'd':
            ...
            if ('s' == type && do_hint && !parse_octet_hint (tp->hint, value, &hintptr, &itmp))
            {
                ...
                {
                    snmp_pdu_add_variable (pdu, name, name_length, ASN_OCTET_STR, hintptr, itmp);
                }
                SNMP_FREE (hintptr);
                hintptr = buf;
                break;
            ...
            if (!_check_range (tp, value_len, &result, "Bad string length"))
                break;
#endif                            /* NETSNMP_DISABLE_MIB_LOADING */
            snmp_pdu_add_variable (pdu, name, name_length, ASN_OCTET_STR, buf_ptr, value_len);
            break;
        ...
        case 'U':
            if (read64 (&c64tmp, value))
                snmp_pdu_add_variable (pdu, name, name_length, ASN_OPAQUE_U64, &c64tmp, sizeof (c64tmp));
            else
                goto fail;
            break;
        ...
        case 'I':
            if (read64 (&c64tmp, value))
                snmp_pdu_add_variable (pdu, name, name_length, ASN_OPAQUE_I64, &c64tmp, sizeof (c64tmp));
            else
                goto fail;
            break;
        ...
        case 'F':
            if (sscanf (value, "%f", &ftmp) == 1)
                snmp_pdu_add_variable (pdu, name, name_length, ASN_OPAQUE_FLOAT, &ftmp, sizeof (ftmp));
            else
                goto fail;
            break;
        ...
        case 'D':
            if (sscanf (value, "%lf", &dtmp) == 1)
                snmp_pdu_add_variable (pdu, name, name_length, ASN_OPAQUE_DOUBLE, &dtmp, sizeof (dtmp));
            else
                goto fail;
            break;
#endif                            /* NETSNMP_WITH_OPAQUE_SPECIAL_TYPES */
  ...
  type_error:
    {
        ...
        snprintf (error_msg, sizeof (error_msg), "Type of attribute is %s, not %s", var_type, value);
