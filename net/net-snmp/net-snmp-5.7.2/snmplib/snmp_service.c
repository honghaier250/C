...
static char **create_word_array_helper (const char *cptr, size_t idx, char *tmp, size_t tmplen)
{
    char *item;
...
static void netsnmp_register_user_domain (const char *token, char *cptr)
{
...

    {
        ...
        if (cp == NULL)
        {
            netsnmp_config_error ("No domain(s) in registration of " "defDomain \"%s\"", application);
            free (application);
            return;
        }
    ...
    if (run && strcmp (run->application, application) == 0)
    {
        ...
        {
            config_perror ("Default transport already registered for this " "application");
...
const char *const *netsnmp_lookup_default_domains (const char *application)
{
    ...
    DEBUGMSGTL (("defaults", "netsnmp_lookup_default_domain(\"%s\") ->", application ? application : "[NIL]"));
    if (res)
    {
 ...
 */
int netsnmp_register_default_target (const char *application, const char *domain, const char *target)
{
    struct netsnmp_lookup_target *run = targets, *prev = NULL;

    int i = 0, res = 0;

    while (run && ((i = strcmp (run->application, application)) < 0 || (i == 0 && strcmp (run->domain, domain) < 0)))
    {
        prev = run;
        run = run->next;
    }
...
static void netsnmp_register_user_target (const char *token, char *cptr)
{
...

    {
        ...
        if (cp == NULL)
        {
            netsnmp_config_error ("No domain and target in registration of " "defTarget \"%s\"", application);
            goto done;
        }
        cp = copy_nword (cp, domain, len);
        if (cp == NULL)
        {
            netsnmp_config_error ("No target in registration of " "defTarget \"%s\" \"%s\"", application, domain);
            goto done;
        }
    ...
    if (run && i == 0 && strcmp (run->domain, domain) == 0)
    {
        ...
        {
            config_perror ("Default target already registered for this " "application-domain combination");
            goto done;
        }
    }
    else
...
const char *netsnmp_lookup_default_target (const char *application, const char *domain)
{
    int i = 0;
    ...
    DEBUGMSGTL (("defaults",
                 ...
                 application ? application : "[NIL]", domain ? domain : "[NIL]", res ? res : "[NIL]"));
    return res;
}
...
void netsnmp_register_service_handlers (void)
{
    register_config_handler ("snmp:", "defDomain",
                             netsnmp_register_user_domain, netsnmp_clear_user_domain, "application domain");
    register_config_handler ("snmp:", "defTarget",
                             netsnmp_register_user_target, netsnmp_clear_user_target, "application domain target");
}
