...
{
    start_alarms = 0;
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_POST_READ_CONFIG, init_alarm_post_config, NULL);
}
...
void snmp_alarm_unregister (unsigned int clientreg)
{
    ...
    for (sa_ptr = thealarms; sa_ptr != NULL && sa_ptr->clientreg != clientreg; sa_ptr = sa_ptr->next)
    {
...
struct snmp_alarm *sa_find_next (void)
{
    ...
    for (a = thealarms; a != NULL; a = a->next)
        if (!(a->flags & SA_FIRED) && (lowest == NULL || timercmp (&a->t_nextM, &lowest->t_nextM, <)))
            lowest = a;
 ...
 *   scheduled.
 */
int netsnmp_get_next_alarm_time (struct timeval *alarm_tm, const struct timeval *now)
{
    struct snmp_alarm *sa_ptr;
...
void set_an_alarm (void)
{
    ...
    if (nextalarm && !netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_ALARM_DONT_USE_SIG))
    {
#ifndef WIN32
 ...
 */
unsigned int
snmp_alarm_register (unsigned int when, unsigned int flags, SNMPAlarmCallback * thecallback, void *clientarg)
{
    struct timeval t;
...
unsigned int snmp_alarm_register_hr (struct timeval t, unsigned int flags, SNMPAlarmCallback * cb, void *cd)
{
    ...
    *s = SNMP_MALLOC_STRUCT (snmp_alarm);
    ...
    DEBUGMSGTL (("snmp_alarm",
                 ...
                 (*s)->clientreg, (long) (*s)->t.tv_sec, ((*s)->t.tv_usec / 1000), (*s)->flags));
