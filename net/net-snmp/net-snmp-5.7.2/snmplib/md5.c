 ...
 */
static void MDblock (MDptr MDp, const unsigned int *X)
{
 ...
 */
int MDupdate (MDptr MDp, const unsigned char *X, unsigned int count)
{
 ...
 */
int MDchecksum (const u_char * data, size_t len, u_char * mac, size_t maclen)
{
    MDstruct md;
 ...
 */
int MDsign (const u_char * data, size_t len, u_char * mac, size_t maclen, const u_char * secret, size_t secretlen)
{
#define HASHKEYLEN 64
    ...
    if (secretlen != 16 || secret == NULL || mac == NULL || data == NULL || len <= 0 || maclen <= 0)
    {
        /*
