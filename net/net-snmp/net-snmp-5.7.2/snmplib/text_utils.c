...
netsnmp_feature_child_of (text_utils, libnetsnmp) netsnmp_feature_provide (text_utils)
 ...
 * parse methods
 */
     void _pm_save_index_string_string (FILE * f, netsnmp_container * cin, int flags);

     void _pm_save_everything (FILE * f, netsnmp_container * cin, int flags);

     void _pm_user_function (FILE * f, netsnmp_container * cin, netsnmp_line_process_info * lpi, int flags);


/*
 * line processors
 */
     int _process_line_tvi (netsnmp_line_info * line_info, void *mem, struct netsnmp_line_process_info_s *lpi);
 ...
 */
     netsnmp_container *netsnmp_file_text_parse (netsnmp_file * f, netsnmp_container * cin,
                                                 int parse_mode, u_int flags, void *context)
{
    ...
    switch (parse_mode)
    {
        ...
        case PM_USER_FUNCTION:
            if (NULL != context)
                _pm_user_function (fin, c, (netsnmp_line_process_info *) context, flags);
            break;
...
netsnmp_feature_child_of (text_token_container_from_file, netsnmp_unused)
     ...
     netsnmp_container *netsnmp_text_token_container_from_file (const char *file, u_int flags,
...
void _pm_user_function (FILE * f, netsnmp_container * cin, netsnmp_line_process_info * lpi, int flags)
{
    ...
    while (fgets (li.line, li.line_max, f) != NULL)
    {
        ...
        if ((!(lpi->flags & PMLP_FLAG_LEAVE_NEWLINE)) && (li.line[li.line_len] == '\n'))
