...
static VOID (*StopFunction) (VOID) = 0L;
     ...
     */
static BOOL UpdateServiceStatus (DWORD dwStatus, DWORD dwErrorCode, DWORD dwWaitHint);
...
VOID ProcessError (WORD eventLogType, LPCTSTR pszMessage, int useGetLastError, int quiet);
     ...
     */
int RegisterService (LPCTSTR lpszServiceName, LPCTSTR lpszServiceDisplayName, LPCTSTR lpszServiceDescription, InputParams * StartUpArg, int quiet)    /* Startup argument to the service */
{
    ...
    TCHAR szRegAppLogKey[] = _T ("SYSTEM\\CurrentControlSet\\Services\\EventLog\\Application\\");
    ...
    TRY
    {
        ...
        RegSetValueEx (hKey, _T ("EventMessageFile"), 0, REG_EXPAND_SZ,
                       (CONST BYTE *) szServicePath, _tcslen (szServicePath) + sizeof (TCHAR));
        ...
        if (lpszServiceDescription != NULL || StartUpArg->Argc > 2)
        {
             ...
             */
            if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, szRegKey, 0, KEY_WRITE, &hKey) != ERROR_SUCCESS)
            {
                _sntprintf (MsgErrorString, CountOf (MsgErrorString), _T ("%s %s"),
                            _T ("is unable to create registry entries"), lpszServiceDisplayName);
                ProcessError (EVENTLOG_ERROR_TYPE, MsgErrorString, 1, quiet);
...
/*  HKEY hKey = NULL;        ?* Key to registry entry */
    TRY
    {
        ...
        {
            if (sStatus.dwCurrentState == SERVICE_RUNNING || sStatus.dwCurrentState == SERVICE_PAUSED)
            {
...
VOID WriteToEventLog (WORD wType, LPCTSTR pszFormat, ...)
{
    ...
    if (hEventSource == NULL)
        return;
    ReportEvent (hEventSource, wType, 0, DISPLAY_MSG, NULL, 1, 0, LogStr, NULL);
...
VOID ProcessError (WORD eventLogType, LPCTSTR pszMessage, int useGetLastError, int quiet)
{
    ...
    if (useGetLastError)
    {
        ...
        FormatMessage (FORMAT_MESSAGE_ALLOCATE_BUFFER |
                       ...
                       MAKELANGID (LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR) & pErrorMsgTemp, 0, NULL);
...
VOID WINAPI ServiceMain (DWORD argc, LPTSTR argv[])
{
    ...
    _sntprintf (szRegKey, CountOf (szRegKey), _T ("%s%s\\%s"),
                _T ("SYSTEM\\CurrentControlSet\\Services\\"), app_name_long, _T ("Parameters"));
    if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, szRegKey, 0, KEY_ALL_ACCESS, &hParamKey) == ERROR_SUCCESS)
    {
        ...
        if (RegQueryInfoKey (hParamKey, NULL, NULL, 0,
                             NULL, NULL, NULL, &TotalParams, NULL, NULL, NULL, NULL) == ERROR_SUCCESS)
        {
            if (TotalParams != 0)
            {
                ...
                if (ArgArray == 0)
                {
                    WriteToEventLog (EVENTLOG_ERROR_TYPE, _T ("Resource failure"));
                    return;
                }
                ...
                for (i = 1; i <= TotalParams; i++)
                {
                     ...
                     */
                    nSize = 0;
                    dwErrorcode = RegQueryValueEx (hParamKey, szRegKey, NULL, &nRegkeyType, NULL, &nSize);
                            ...
                            if (szValue)
                            {
                                ...
                                else
                                {
                                    ...
                                    WriteToEventLog (EVENTLOG_ERROR_TYPE,
                                                     _T ("Querying registry key %s failed: error code %ld"), szRegKey,
                                                     dwErrorcode);
                                }
                            }
                            else
                                WriteToEventLog (EVENTLOG_ERROR_TYPE,
                                                 _T ("Querying registry key %s failed: out of memory"), szRegKey);
                        }
                        else
                            WriteToEventLog (EVENTLOG_ERROR_TYPE, _T ("Type %ld of registry key %s is incorrect"),
                                             nRegkeyType, szRegKey);
                    }
                    else
                        WriteToEventLog (EVENTLOG_ERROR_TYPE, _T ("Querying registry key %s failed: error code %ld"),
    ...
    if (hServiceStatus == 0)
    {
        WriteToEventLog (EVENTLOG_ERROR_TYPE, _T ("RegisterServiceCtrlHandler failed"));
        return;
    }
    ...
    TRY
    {
        ...
        {
            WriteToEventLog (EVENTLOG_ERROR_TYPE, _T ("Couldn't init security attributes"));
            LEAVE;
        }
        hServiceThread =
            (void *) _beginthreadex (&SecurityAttributes, 0, ThreadFunction, (void *) &ThreadInputParams, 0, &threadId);
...
BOOL RunAsService (INT (*ServiceFunction) (INT, LPTSTR *))
{
    ...
    if (StartServiceCtrlDispatcher (ServiceTableEntry) == FALSE)
    {
         ...
         */
        WriteToEventLog (EVENTLOG_ERROR_TYPE, _T ("Couldn't start service - %s"), app_name_long);
    }
...
BOOL SetSimpleSecurityAttributes (SECURITY_ATTRIBUTES * pSecurityAttr)
{
    ...
    if (!pSecurityAttr)
        return FALSE;
    pSecurityDesc = (SECURITY_DESCRIPTOR *) LocalAlloc (LPTR, SECURITY_DESCRIPTOR_MIN_LENGTH);
    if (!pSecurityDesc)
        return FALSE;
    fReturn = InitializeSecurityDescriptor (pSecurityDesc, SECURITY_DESCRIPTOR_REVISION);
    if (fReturn != FALSE)
    {
