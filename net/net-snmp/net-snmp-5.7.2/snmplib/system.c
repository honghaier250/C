...
int netsnmp_daemonize (int quit_immediately, int stderr_log)
{
    ...
    if (i != 0)
    {
        ...
        if (i == -1)
        {
            snmp_log (LOG_ERR, "first fork failed (errno %d) in " "netsnmp_daemonize()\n", errno);
            return -1;
        }
        if (quit_immediately)
        {
...
long get_uptime (void)
{
    ...
    DWORD buffersize = (sizeof (PERF_DATA_BLOCK) + sizeof (PERF_OBJECT_TYPE)), type = REG_EXPAND_SZ;
    ...
    RegQueryValueEx (HKEY_PERFORMANCE_DATA, "Global", NULL, &type, (LPBYTE) perfdata, &buffersize);
    ...
    if (wcsncmp (perfdata->Signature, L"PERF", 4) == 0)
    {
         ...
         */
        return_value = (long) ((perfdata->PerfTime100nSec.QuadPart / (LONGLONG) 100000));
    }
    else
...
in_addr_t get_myaddr (void)
{
    ...
    for (ifrp = ifc.ifc_req; (char *) ifrp < (char *) ifc.ifc_req + ifc.ifc_len;
         ...
         ifrp = (struct ifreq *) (((char *) ifrp) + sizeof (ifrp->ifr_name) + ifrp->ifr_addr.sa_len)
#else
         ifrp++
#endif
        )
...
int netsnmp_gethostbyname_v4 (const char *name, in_addr_t * addr_out)
{
    ...
    if (addrs != NULL)
    {
        memcpy (addr_out, &((struct sockaddr_in *) addrs->ai_addr)->sin_addr, sizeof (in_addr_t));
        freeaddrinfo (addrs);
    }
    else
    {
    ...
    if (hp == NULL)
    {
        DEBUGMSGTL (("get_thisaddr", "hostname (couldn't resolve = %d)\n", err));
        return -1;
    }
    DEBUGMSGTL (("get_thisaddr", "hostname (resolved okay)\n"));
    memcpy (addr_out, hp->h_addr, sizeof (in_addr_t));
...
int netsnmp_getaddrinfo (const char *name, const char *service, const struct addrinfo *hints, struct addrinfo **res)
{
#if HAVE_GETADDRINFO
    struct addrinfo *addrs = NULL;
    ...
    err = val_getaddrinfo (netsnmp_validator_context (), name, NULL, &hint, &addrs, &val_status);
    DEBUGMSGTL (("dns:sec:val", "err %d, val_status %d / %s; trusted: %d\n",
                 err, val_status, p_val_status (val_status), val_istrusted (val_status)));
    if (!val_istrusted (val_status))
    {
        ...
        {
            snmp_log (LOG_WARNING, "WARNING: UNTRUSTED error in DNS resolution for %s!\n", name);
            rc = EAI_FAIL;
        }
        else
        {
            snmp_log (LOG_WARNING, "The authenticity of DNS response is not trusted (%s)\n", p_val_status (val_status));
            rc = EAI_NONAME;
        }

        /** continue anyways if DNSSEC_WARN_ONLY is set */
        if (!netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DNSSEC_WARN_ONLY))
            return rc;
    }
...
struct hostent *netsnmp_gethostbyname (const char *name)
{
    ...
    DEBUGMSGTL (("dns:sec:val", "val_status %d / %s; trusted: %d\n",
                 val_status, p_val_status (val_status), val_istrusted (val_status)));
    if (!val_istrusted (val_status))
    {
        snmp_log (LOG_WARNING, "The authenticity of DNS response is not trusted (%s)\n", p_val_status (val_status));
    ...
    {
        DEBUGMSGTL (("dns:gethostbyname", "warning: response for %s not AF_INET!\n", name));
    }
    else
    {
...
struct hostent *netsnmp_gethostbyaddr (const void *addr, socklen_t len, int type)
{
    ...
    hp = val_gethostbyaddr (netsnmp_validator_context (), addr, len, type, &val_status);
    DEBUGMSGTL (("dns:sec:val", "val_status %d / %s; trusted: %d\n",
                 val_status, p_val_status (val_status), val_istrusted (val_status)));
    if (!val_istrusted (val_status))
    {
    ...
    {
        DEBUGMSGTL (("dns:gethostbyaddr", "warning: response for addr not AF_INET!\n"));
    }
    else
    {
 ...
 */
     int calculate_time_diff (const struct timeval *now, const struct timeval *then)
{
    struct timeval diff;
...
u_int calculate_sectime_diff (const struct timeval *now, const struct timeval *then)
{
    struct timeval diff;
...
int mkdirhier (const char *pathname, mode_t mode, int skiplast)
{
     ...
     *   e.g  "c:/path"
     */
    if ((entry) && (':' == entry[1]) && (('\0' == entry[2]) || ('/' == entry[2])))
    {
        strcat (buf, entry);
