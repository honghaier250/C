...
netsnmp_feature_child_of (find_module, mib_api) netsnmp_feature_child_of (get_tc_description, mib_api)
...
struct tree *find_tree_node (const char *name, int modid)
{
    ...
    for (tp = headtp; tp; tp = tp->next)
    {
        if (tp->label && !label_compare (tp->label, name))
        {
            ...
            for (int_p = tp->module_list, count = 0; count < tp->number_modules; ++count, ++int_p)
                if (*int_p == modid)
                    return (tp);
        }
    }
                 ...
                 */
{
    ...
    for (child1 = tp1->child_list; child1;)
    {
        ...
        for (child2 = tp2->child_list, previous = NULL; child2; previous = child2, child2 = child2->next_peer)
        {
            ...
            if (child1->subid == child2->subid)
            {
                ...
                else if (!strncmp (child2->label, ANON, ANON_LEN))
                {
                    ...
                    for (previous = tp2->child_list; previous; previous = previous->next_peer)
                        previous->parent = tp2;
                    goto next;
                }
                ...
                else
                {
                    ...
                    for (previous = child1->child_list; previous; previous = previous->next_peer)
        ...
        if (nbuckets[i])
        {
            ...
            while (onp)
            {
                snmp_log (LOG_WARNING,
                          ...
                          (onp->parent ? onp->parent : "<no parent>"), onp->subid);
                snmp_log (LOG_WARNING,
 ...
 * Returns 0 on error.
 */
static int getoid (FILE * fp, struct subid_s *id,    /* an array of subids */
                   int length)
...
static struct node *parse_objectid (FILE * fp, char *name)
{
     ...
     */
    for (count = 0, op = loid, nop = loid + 1; count < (length - 1); count++, op++, nop++)
    {
        /*
        ...
        if (op->label && (nop->label || (nop->subid != -1)))
        {
            ...
            if (nop->subid != -1)
                ...
                print_error ("Warning: This entry is pretty silly", np->label, CONTINUE);
...
static int
get_tc (const char *descriptor, int modid, int *tc_index, struct enum_list **ep, struct range_list **rp, char **hint)
{
    int i;
                ...
                if (type == DISPLAYHINT)
                {
                    ...
                    if (type != QUOTESTRING)
                        ...
                        hint = strdup (token);
                         ...
                         netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SAVE_MIB_DESCRS))
                {
            ...
            else
            {
                ...
                if (type == MINACCESS)
                {
                    ...
                    if (type != NOACCESS && type != ACCNOTIFY
                        && type != READONLY && type != WRITEONLY && type != READCREATE && type != READWRITE)
                    {
        ...
        while (type == VARIATION)
        {
            ...
            if (type == ACCESS)
            {
                ...
                if (type != ACCNOTIFY && type != READONLY
                    && type != READWRITE && type != READCREATE && type != WRITEONLY && type != NOTIMPL)
                {
...
#define MAX_IMPORTS    256
    ...
    for (mp = module_head; mp; mp = mp->next)
        if (mp->modid == current_module)
        {
            ...
            mp->imports = (struct module_import *) calloc (import_count, sizeof (struct module_import));
            if (mp->imports == NULL)
                return;
     ...
     void add_module_replacement (const char *old_module, const char *new_module_name, const char *tag, int len)
{
    ...
    mcp = (struct module_compatability *) calloc (1, sizeof (struct module_compatability));
    if (mcp == NULL)
        return;
...
void unload_all_mibs (void)
{
    ...
    for (mcp = module_map_head; mcp; mcp = module_map_head)
    {
        ...
        if (mcp->tag)
            free (NETSNMP_REMOVE_CONST (char *, mcp->tag));
...
static int get_token (FILE * fp, char *token, int maxtlen)
{
    ...
    enum
    { bdigits, xdigits, other } seenSymbols;
        ...
        while ((file = readdir (dir)))
        {
            ...
            if (file->d_name != NULL)
            {
                ...
                if (fname_len > 0 && file->d_name[0] != '.'
                    && file->d_name[0] != '#'
                    && file->d_name[fname_len - 1] != '#' && file->d_name[fname_len - 1] != '~')
                {
...
struct tree *read_all_mibs (void)
{
    ...
    if (gLoop == 1)
    {
        ...
        {
            snprintf (gpMibErrorString, sizeof (gpMibErrorString) - 1,
                      "Error in parsing MIB module(s): %s ! Unable to load corresponding MIB(s)", gMibNames);
        }
    }
