...
netsnmp_feature_child_of (usm_support, usm_all) netsnmp_feature_require (usm_support)
...
usm_check_secLevel_vs_protocols (int level,
                                 ...
                                 u_int authProtocolLen, const oid * privProtocol, u_int privProtocolLen);
int
usm_calc_offsets (size_t globalDataLen,
                  ...
                  size_t * msgPrivParmLen, size_t * otstlen, size_t * seq_len, size_t * msgSecParmLen);
...
int free_enginetime_on_shutdown (int majorid, int minorid, void *serverarg, void *clientarg)
{
...
{
    struct usmStateReference *retval = (struct usmStateReference *) calloc (1, sizeof (struct usmStateReference));
...
int usm_set_usmStateReference_name (struct usmStateReference *ref, char *name, size_t name_len)
{
    MAKE_ENTRY (char, name, name_len, usr_name, usr_name_length);
}

int usm_set_usmStateReference_engine_id (struct usmStateReference *ref, u_char * engine_id, size_t engine_id_len)
{
    MAKE_ENTRY (u_char, engine_id, engine_id_len, usr_engine_id, usr_engine_id_length);
}

int
usm_set_usmStateReference_auth_protocol (struct usmStateReference *ref, oid * auth_protocol, size_t auth_protocol_len)
{
    MAKE_ENTRY (oid, auth_protocol, auth_protocol_len, usr_auth_protocol, usr_auth_protocol_length);
}

int usm_set_usmStateReference_auth_key (struct usmStateReference *ref, u_char * auth_key, size_t auth_key_len)
{
    MAKE_ENTRY (u_char, auth_key, auth_key_len, usr_auth_key, usr_auth_key_length);
}

int
usm_set_usmStateReference_priv_protocol (struct usmStateReference *ref, oid * priv_protocol, size_t priv_protocol_len)
{
    MAKE_ENTRY (oid, priv_protocol, priv_protocol_len, usr_priv_protocol, usr_priv_protocol_length);
}

int usm_set_usmStateReference_priv_key (struct usmStateReference *ref, u_char * priv_key, size_t priv_key_len)
{
    MAKE_ENTRY (u_char, priv_key, priv_key_len, usr_priv_key, usr_priv_key_length);
}

int usm_set_usmStateReference_sec_level (struct usmStateReference *ref, int sec_level)
{
    if (ref == NULL)
        return -1;
...
int usm_clone_usmStateReference (struct usmStateReference *from, struct usmStateReference **to)
{
    ...
    *to = usm_malloc_usmStateReference ();
    ...
    if (usm_set_usmStateReference_name (cloned_usmStateRef, from->usr_name, from->usr_name_length) ||
        ...
        usm_set_usmStateReference_auth_protocol (cloned_usmStateRef, from->usr_auth_protocol,
        ...
        || usm_set_usmStateReference_priv_protocol (cloned_usmStateRef, from->usr_priv_protocol,
 ...
 *    [12] = theTotalLength
 */
int usm_calc_offsets (size_t globalDataLen,    /* SNMPv3Message + HeaderData */
...
{                                /* Size of msgSecurityP. SEQ.              */
     ...
     */
    *msgAuthParmLen = (secLevel == SNMP_SEC_LEVEL_AUTHNOPRIV || secLevel == SNMP_SEC_LEVEL_AUTHPRIV) ? 12 : 0;
    ...
    *msgPrivParmLen = (secLevel == SNMP_SEC_LEVEL_AUTHPRIV) ? 8 : 0;
     ...
     * Calculate lengths.
     */
    if ((engIDlen = asn_predict_length (ASN_OCTET_STR, NULL, secEngineIDLen)) == -1)
    {
        return -1;
    }

    if ((engBtlen = asn_predict_length (ASN_INTEGER, (u_char *) & engineboots, sizeof (long))) == -1)
    {
        return -1;
    }

    if ((engTmlen = asn_predict_length (ASN_INTEGER, (u_char *) & engine_time, sizeof (long))) == -1)
    {
        return -1;
    }
    ...
    if ((authlen = asn_predict_length (ASN_OCTET_STR, NULL, *msgAuthParmLen)) == -1)
    {
        return -1;
    }

    if ((privlen = asn_predict_length (ASN_OCTET_STR, NULL, *msgPrivParmLen)) == -1)
    {
        return -1;
    }
    ...
    *authParamsOffset = globalDataLen + +(*msgSecParmLen - *seq_len)
        + engIDlen + engBtlen + engTmlen + namelen + (authlen - *msgAuthParmLen);

    *privParamsOffset = *authParamsOffset + *msgAuthParmLen + (privlen - *msgPrivParmLen);
 ...
 */
int usm_set_salt (u_char * iv, size_t * iv_length, u_char * priv_salt, size_t priv_salt_length, u_char * msgSalt)
{
     ...
     * Sanity check.
     */
    if (!iv || !iv_length || !priv_salt || (*iv_length != propersize_salt) || (priv_salt_length < propersize_salt))
    {
        return -1;
    }
 ...
 *    The IV result is returned individually for further use.
 */
int usm_set_aes_iv (u_char * iv, size_t * iv_length, u_int net_boots, u_int net_time, u_char * salt)
{
    /*
...
int usm_secmod_generate_out_msg (struct snmp_secmod_outgoing_params *parms)
{
    ...
    return usm_generate_out_msg (parms->msgProcModel,
                                 ...
                                 parms->secStateRef,
                                 parms->secParams, parms->secParamsLen, parms->wholeMsg, parms->wholeMsgLen);
}
 ...
 */
int usm_generate_out_msg (int msgProcModel,    /* (UNUSED) */
...
{                                /* IN/OUT - Len available, len returned. */
    ...
    else
    {
         ...
         */
        if ((user = usm_get_user (secEngineID, secEngineIDLen, secName)) == NULL && secLevel != SNMP_SEC_LEVEL_NOAUTH)
        {
    ...
    if (usm_check_secLevel_vs_protocols (theSecLevel,
                                         theAuthProtocol,
                                         theAuthProtocolLength, thePrivProtocol, thePrivProtocolLength) == 1)
    {
     ...
     */
    if (get_enginetime (theEngineID, theEngineIDLength, &boots_uint, &time_uint, FALSE) == -1)
    {
    ...
    if (usm_calc_offsets (globalDataLen, theSecLevel, theEngineIDLength,
                          ...
                          &msgAuthParmLen, &msgPrivParmLen, &otstlen, &seq_len, &msgSecParmLen) == -1)
    {
    ...
    if (theSecLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        ...
        if (ISTRANSFORM (thePrivProtocol, AESPriv))
        {
            if (!thePrivKey ||
                usm_set_aes_iv (salt, &salt_length,
                                htonl (boots_uint), htonl (time_uint), &ptr[privParamsOffset]) == -1)
            {
        ...
        {
            if (!thePrivKey ||
                (usm_set_salt (salt, &salt_length, thePrivKey + 8, thePrivKeyLength - 8, &ptr[privParamsOffset]) == -1))
            {
        ...
        if (sc_encrypt (thePrivProtocol, thePrivProtocolLength,
                        ...
                        salt, salt_length,
                        scopedPdu, scopedPduLen, &ptr[dataOffset], &encrypted_length) != SNMP_ERR_NOERROR)
        {
        ...
        {
            dump_chunk ("usm/dump", "This data was encrypted:", scopedPdu, scopedPduLen);
         ...
         */
        if ((encrypted_length != (theTotalLength - dataOffset)) || (salt_length != msgPrivParmLen))
        {
    ...
    asn_build_header (&ptr[offSet], &remaining, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), otstlen);

    offSet = ptr_len - remaining;
    asn_build_sequence (&ptr[offSet], &remaining, (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), seq_len);

    offSet = ptr_len - remaining;
    DEBUGDUMPHEADER ("send", "msgAuthoritativeEngineID");
    asn_build_string (&ptr[offSet], &remaining,
                      (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), theEngineID, theEngineIDLength);
    DEBUGINDENTLESS ();

    offSet = ptr_len - remaining;
    DEBUGDUMPHEADER ("send", "msgAuthoritativeEngineBoots");
    asn_build_int (&ptr[offSet], &remaining,
                   (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &boots_long, sizeof (long));
    DEBUGINDENTLESS ();

    offSet = ptr_len - remaining;
    DEBUGDUMPHEADER ("send", "msgAuthoritativeEngineTime");
    asn_build_int (&ptr[offSet], &remaining,
                   (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &time_long, sizeof (long));
    DEBUGINDENTLESS ();

    offSet = ptr_len - remaining;
    DEBUGDUMPHEADER ("send", "msgUserName");
    asn_build_string (&ptr[offSet], &remaining,
                      (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), (u_char *) theName, theNameLength);
    DEBUGINDENTLESS ();
    ...
    asn_build_header (&ptr[offSet],
                      &remaining, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), msgAuthParmLen);

    if (theSecLevel == SNMP_SEC_LEVEL_AUTHNOPRIV || theSecLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        offSet = ptr_len - remaining;
    ...
    offSet = ptr_len - remaining;
    asn_build_header (&ptr[offSet],
                      &remaining, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), msgPrivParmLen);
    ...
    if (theSecLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        ...
        asn_build_header (&ptr[offSet],
                          &remaining,
                          (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), theTotalLength - dataOffset);
    }
    ...
    asn_build_sequence (ptr, &remaining, (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), theTotalLength - 4);
    ...
    if (theSecLevel == SNMP_SEC_LEVEL_AUTHNOPRIV || theSecLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        ...
        if (sc_generate_keyed_hash (theAuthProtocol, theAuthProtocolLength,
                                    ...
                                    ptr, ptr_len, temp_sig, &temp_sig_len) != SNMP_ERR_NOERROR)
        {
            /*
...
int usm_secmod_rgenerate_out_msg (struct snmp_secmod_outgoing_params *parms)
{
    ...
    return usm_rgenerate_out_msg (parms->msgProcModel,
                                  ...
                                  parms->secLevel,
                                  parms->scopedPdu, parms->scopedPduLen,
                                  parms->secStateRef, parms->wholeMsg, parms->wholeMsgLen, parms->wholeMsgOffset);
}

int usm_rgenerate_out_msg (int msgProcModel,    /* (UNUSED) */
                           u_char * globalData,    /* IN */
                           /*
    ...
    )
{
    ...
    if (theSecLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        ...
        if (ISTRANSFORM (thePrivProtocol, AESPriv))
        {
            ...
            if (!thePrivKey || usm_set_aes_iv (salt, &salt_length, htonl (boots_uint), htonl (time_uint), iv) == -1)
            {
                DEBUGMSGTL (("usm", "Can't set AES iv.\n"));
                usm_free_usmStateReference (secStateRef);
        ...
        if (ISTRANSFORM (thePrivProtocol, DESPriv))
        {
            ...
            if (!thePrivKey || (usm_set_salt (salt, &salt_length, thePrivKey + 8, thePrivKeyLength - 8, iv) == -1))
            {
                DEBUGMSGTL (("usm", "Can't set DES-CBC salt.\n"));
                usm_free_usmStateReference (secStateRef);
                SNMP_FREE (ciphertext);
        ...
        if (sc_encrypt (thePrivProtocol, thePrivProtocolLength,
                        thePrivKey, thePrivKeyLength,
                        salt, salt_length, scopedPdu, scopedPduLen, ciphertext, &ciphertextlen) != SNMP_ERR_NOERROR)
        {
            DEBUGMSGTL (("usm", "encryption error.\n"));
            usm_free_usmStateReference (secStateRef);
            SNMP_FREE (ciphertext);
        ...
        *offset = 0;
        rc = asn_realloc_rbuild_string (wholeMsg, wholeMsgLen, offset, 1,
                                        (u_char) (ASN_UNIVERSAL |
                                                  ASN_PRIMITIVE | ASN_OCTET_STR), ciphertext, ciphertextlen);
        if (rc == 0)
        {
        ...
        {
            dump_chunk ("usm/dump", "salt + Encrypted form: ", salt, salt_length);
            dump_chunk ("usm/dump", "wholeMsg:", (*wholeMsg + *wholeMsgLen - *offset), *offset);
        }
#endif
     ...
     */
    rc = asn_realloc_rbuild_string (wholeMsg, wholeMsgLen, offset, 1,
                                    (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), iv, save_salt_length);
    DEBUGINDENTLESS ();
    if (rc == 0)
    {
    ...
    rc = asn_realloc_rbuild_string (wholeMsg, wholeMsgLen, offset, 1,
                                    (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE
                                              | ASN_OCTET_STR), authParams, msgAuthParmLen);
    DEBUGINDENTLESS ();
    if (rc == 0)
    {
    ...
    rc = asn_realloc_rbuild_string (wholeMsg, wholeMsgLen, offset, 1,
                                    (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE
                                              | ASN_OCTET_STR), (u_char *) theName, theNameLength);
    DEBUGINDENTLESS ();
    if (rc == 0)
    {
        DEBUGMSGTL (("usm", "building authParams failed.\n"));
    ...
    rc = asn_realloc_rbuild_string (wholeMsg, wholeMsgLen, offset, 1,
                                    (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE
                                              | ASN_OCTET_STR), theEngineID, theEngineIDLength);
    DEBUGINDENTLESS ();
    if (rc == 0)
    {
    ...
    rc = asn_realloc_rbuild_sequence (wholeMsg, wholeMsgLen, offset, 1,
                                      (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), *offset - sp_offset);
    if (rc == 0)
    {
    ...
    rc = asn_realloc_rbuild_header (wholeMsg, wholeMsgLen, offset, 1,
                                    (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), *offset - sp_offset);
    ...
    *offset += globalDataLen;
     ...
     */
    rc = asn_realloc_rbuild_sequence (wholeMsg, wholeMsgLen, offset, 1,
                                      (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), *offset);
    if (rc == 0)
    {
    ...
    if (theSecLevel == SNMP_SEC_LEVEL_AUTHNOPRIV || theSecLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        ...
        if (sc_generate_keyed_hash (theAuthProtocol, theAuthProtocolLength,
                                    theAuthKey, theAuthKeyLength,
                                    proto_msg, proto_msg_len, temp_sig, &temp_sig_len) != SNMP_ERR_NOERROR)
        {
            SNMP_FREE (temp_sig);
            DEBUGMSGTL (("usm", "Signing failed.\n"));
            usm_free_usmStateReference (secStateRef);
...
usm_parse_security_parameters (u_char * secParams,
                               ...
                               size_t * secNameLen,
                               u_char * signature,
                               size_t * signature_length, u_char * salt, size_t * salt_length, u_char ** data_ptr)
{
    ...
    if ((value_ptr = asn_parse_sequence (parse_ptr, &octet_string_length,
                                         &type_value,
                                         (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), "usm first octet")) == NULL)
    {
        /*
         * RETURN parse error 
         */ return -1;
    ...
    if ((value_ptr = asn_parse_sequence (parse_ptr, &sequence_length,
                                         &type_value, (ASN_SEQUENCE | ASN_CONSTRUCTOR), "usm sequence")) == NULL)
    {
        /*
         * RETURN parse error 
         */ return -1;
    ...
    if ((next_ptr = asn_parse_string (parse_ptr, &remaining_bytes, &type_value, secEngineID, secEngineIDLen)) == NULL)
    {
        DEBUGINDENTLESS ();
        /*
         * RETURN parse error 
    ...
    if ((next_ptr = asn_parse_int (next_ptr, &remaining_bytes, &type_value, &boots_long, sizeof (long))) == NULL)
    {
        DEBUGINDENTLESS ();
        /*
         * RETURN parse error 
    ...
    *boots_uint = (u_int) boots_long;
    ...
    if ((next_ptr = asn_parse_int (next_ptr, &remaining_bytes, &type_value, &time_long, sizeof (long))) == NULL)
    {
        /*
         * RETURN parse error 
         */ return -1;
    ...
    *time_uint = (u_int) time_long;
    ...
    if ((next_ptr = asn_parse_string (next_ptr, &remaining_bytes, &type_value, (u_char *) secName, secNameLen)) == NULL)
    {
        DEBUGINDENTLESS ();
        /*
         * RETURN parse error 
    ...
    if ((next_ptr = asn_parse_string (next_ptr, &remaining_bytes, &type_value, signature, signature_length)) == NULL)
    {
        DEBUGINDENTLESS ();
        /*
         * RETURN parse error 
    ...
    if ((*data_ptr = asn_parse_string (next_ptr, &remaining_bytes, &type_value, salt, salt_length)) == NULL)
    {
        DEBUGINDENTLESS ();
        /*
         * RETURN parse error 
...
usm_check_and_update_timeliness (u_char * secEngineID,
                                 size_t secEngineIDLen, u_int boots_uint, u_int time_uint, int *error)
{
     ...
     */
    if (secEngineIDLen == myIDLength && memcmp (secEngineID, myID, myIDLength) == 0)
    {
        u_int time_difference = myTime > time_uint ? myTime - time_uint : time_uint - myTime;

        if (boots_uint == ENGINEBOOT_MAX || boots_uint != myBoots || time_difference > USM_TIME_WINDOW)
        {
    ...
    else
    {
        ...
        if (get_enginetime_ex (secEngineID, secEngineIDLen,
                               &theirBoots, &theirTime, &theirLastTime, TRUE) != SNMPERR_SUCCESS)
        {
        ...
        if (set_enginetime (secEngineID, secEngineIDLen, boots_uint, time_uint, TRUE) != SNMPERR_SUCCESS)
        {
...
int usm_secmod_process_in_msg (struct snmp_secmod_incoming_params *parms)
{
    ...
    return usm_process_in_msg (parms->msgProcModel,
                               ...
                               parms->scopedPdu,
                               parms->scopedPduLen,
                               parms->maxSizeResponse, parms->secStateRef, parms->sess, parms->msg_flags);
}
 ...
 */
int usm_process_in_msg (int msgProcModel,    /* (UNUSED) */
...
{                                /* IN     - v3 Message flags.              */
    ...
    if ((rc = usm_parse_security_parameters (secParams, remaining,
                                             ...
                                             secName, secNameLen,
                                             signature, &signature_length, salt, &salt_length, &data_ptr)) < 0)
    {
    ...
    if (secStateRef)
    {
         ...
         */
        if (usm_set_usmStateReference_name (*secStateRef, secName, *secNameLen) == -1)
        {
            DEBUGMSGTL (("usm", "%s\n", "Couldn't cache name."));
            return SNMPERR_USM_GENERICERROR;
        }

        if (usm_set_usmStateReference_engine_id (*secStateRef, secEngineID, *secEngineIDLen) == -1)
        {
    ...
    if ((sess && (sess->isAuthoritative == SNMP_SESS_AUTHORITATIVE ||
                  (sess->isAuthoritative == SNMP_SESS_UNKNOWNAUTH &&
                   (msg_flags & SNMP_MSG_FLAG_RPRT_BIT)))) || (!sess && (msg_flags & SNMP_MSG_FLAG_RPRT_BIT)))
    {
    ...
    if ((user = usm_get_user_from_list (secEngineID, *secEngineIDLen,
                                        ...
                                        (((sess && sess->isAuthoritative ==
                                           SNMP_SESS_AUTHORITATIVE) || (!sess)) ? 0 : 1))) == NULL)
    {
        DEBUGMSGTL (("usm", "Unknown User(%s)\n", secName));
    ...
    if (secLevel == SNMP_SEC_LEVEL_AUTHNOPRIV || secLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        if (sc_check_keyed_hash (user->authProtocol, user->authProtocolLen,
                                 ...
                                 wholeMsg, wholeMsgLen, signature, signature_length) != SNMP_ERR_NOERROR)
        {
    ...
    if (secStateRef)
    {
        if (usm_set_usmStateReference_auth_protocol (*secStateRef, user->authProtocol, user->authProtocolLen) == -1)
        {
            DEBUGMSGTL (("usm", "%s\n", "Couldn't cache authentication protocol."));
            return SNMPERR_USM_GENERICERROR;
        }

        if (usm_set_usmStateReference_auth_key (*secStateRef, user->authKey, user->authKeyLen) == -1)
        {
            DEBUGMSGTL (("usm", "%s\n", "Couldn't cache authentication key."));
            return SNMPERR_USM_GENERICERROR;
        }

        if (usm_set_usmStateReference_priv_protocol (*secStateRef, user->privProtocol, user->privProtocolLen) == -1)
        {
            DEBUGMSGTL (("usm", "%s\n", "Couldn't cache privacy protocol."));
            return SNMPERR_USM_GENERICERROR;
        }

        if (usm_set_usmStateReference_priv_key (*secStateRef, user->privKey, user->privKeyLen) == -1)
        {
    ...
    if (secLevel == SNMP_SEC_LEVEL_AUTHNOPRIV || secLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        if (usm_check_and_update_timeliness (secEngineID, *secEngineIDLen, boots_uint, time_uint, &error) == -1)
        {
            return error;
        }
    }
    ...
    {
        set_enginetime (secEngineID, *secEngineIDLen, boots_uint, time_uint, FALSE);
    }
    ...
    if (secLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        ...
        if ((value_ptr = asn_parse_sequence (data_ptr, &remaining,
                                             ...
                                             (ASN_UNIVERSAL | ASN_PRIMITIVE
                                              | ASN_OCTET_STR), "encrypted sPDU")) == NULL)
        {
        ...
        if (sc_decrypt (user->privProtocol, user->privProtocolLen,
                        ...
                        iv, iv_length, value_ptr, remaining, *scopedPdu, scopedPduLen) != SNMP_ERR_NOERROR)
        {
            DEBUGMSGTL (("usm", "%s\n", "Failed decryption."));
            snmp_increment_statistic (STAT_USMSTATSDECRYPTIONERRORS);
            return SNMPERR_USM_DECRYPTIONERROR;
        ...
        if (debug_is_token_registered ("usm/dump") == SNMPERR_SUCCESS)
        {
            ...
            dump_chunk ("usm/dump", "Decrypted chunk:", *scopedPdu, *scopedPduLen);
        }
#endif
    }
    /*
...
void
usm_handle_report (void *sessp, netsnmp_transport * transport, netsnmp_session * session, int result, netsnmp_pdu * pdu)
{
    /*
    ...
    switch (result)
    {
        ...
        case SNMPERR_USM_DECRYPTIONERROR:
            ...
            if (SNMP_CMD_CONFIRMED (pdu->command) || (pdu->command == 0 && (pdu->flags & SNMP_MSG_FLAG_RPRT_BIT)))
            {
                netsnmp_pdu *pdu2;
...
int usm_session_init (netsnmp_session * in_session, netsnmp_session * session)
{
    ...
    if (in_session->securityAuthProtoLen > 0)
    {
        session->securityAuthProto =
            snmp_duplicate_objid (in_session->securityAuthProto, in_session->securityAuthProtoLen);
    ...
    {
        session->securityAuthProto = snmp_duplicate_objid (get_default_authtype (NULL), i);
    ...
    if (in_session->securityPrivProtoLen > 0)
    {
        session->securityPrivProto =
            snmp_duplicate_objid (in_session->securityPrivProto, in_session->securityPrivProtoLen);
    ...
    {
        session->securityPrivProto = snmp_duplicate_objid (get_default_privtype (NULL), i);
        session->securityPrivProtoLen = i;
    }

    if ((in_session->securityAuthKeyLen <= 0) &&
        ((cp = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_AUTHMASTERKEY))))
    {
        ...
        if (!snmp_hex_to_binary (&tmpp, &buflen, &session->securityAuthKeyLen, 0, cp))
        {
            ...
            return SNMP_ERR_GENERR;
             ...
             ((cp = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID,
              ...
              (cp = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_PASSPHRASE))))
    {
    ...
    if ((in_session->securityPrivKeyLen <= 0) &&
        ((cp = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_PRIVMASTERKEY))))
    {
        ...
        /* it will be a hex string */
        if (!snmp_hex_to_binary (&tmpp, &buflen, &session->securityPrivKeyLen, 0, cp))
        {
...
int usm_create_user_from_session (netsnmp_session * session)
{
    ...
    if (SNMP_FLAGS_USER_CREATED == (session->flags & SNMP_FLAGS_USER_CREATED) ||
        ...
        session->version != SNMP_VERSION_3 || session->securityNameLen == 0 || session->securityEngineIDLen == 0)
        return SNMPERR_SUCCESS;
    ...
    user = usm_get_user_from_list (session->securityEngineID,
                                   session->securityEngineIDLen, session->securityName, usm_get_userList (), 0);
    ...
    if (user == NULL)
    {
         ...
         */
        if (memdup (&user->engineID, session->securityEngineID, session->securityEngineIDLen) != SNMPERR_SUCCESS)
        {
            usm_free_user (user);
            return SNMPERR_GENERR;
        }
    ...
    if (user->authProtocol == NULL && session->securityAuthProto != NULL)
    {
        ...
        user->authProtocol = snmp_duplicate_objid (session->securityAuthProto, session->securityAuthProtoLen);
    ...
    if (user->privProtocol == NULL && session->securityPrivProto != NULL)
    {
        ...
        user->privProtocol = snmp_duplicate_objid (session->securityPrivProto, session->securityPrivProtoLen);
    ...
    {
        if (session->securityAuthLocalKey != NULL && session->securityAuthLocalKeyLen != 0)
        {
        ...
        else if (session->securityAuthKey != NULL && session->securityAuthKeyLen != 0)
        {
            SNMP_FREE (user->authKey);
            ...
            if (generate_kul (user->authProtocol, user->authProtocolLen,
                              ...
                              session->securityAuthKeyLen, user->authKey, &user->authKeyLen) != SNMPERR_SUCCESS)
            {
                usm_free_user (user);
                return SNMPERR_GENERR;
            }
        }
        else if ((cp = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_AUTHLOCALIZEDKEY)))
        {
            ...
            user->authKeyLen = 0;
            /* it will be a hex string */
            if (!snmp_hex_to_binary (&user->authKey, &buflen, &user->authKeyLen, 0, cp))
            {
                usm_free_user (user);
                return SNMPERR_GENERR;
            }
    ...
    {
        if (session->securityPrivLocalKey != NULL && session->securityPrivLocalKeyLen != 0)
        {
            /* already localized key passed in.  use it */
        ...
        else if (session->securityPrivKey != NULL && session->securityPrivKeyLen != 0)
        {
            SNMP_FREE (user->privKey);
            ...
            if (generate_kul (user->authProtocol, user->authProtocolLen,
                              ...
                              session->securityPrivKeyLen, user->privKey, &user->privKeyLen) != SNMPERR_SUCCESS)
            {
                usm_free_user (user);
                return SNMPERR_GENERR;
            }
        }
        else if ((cp = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_PRIVLOCALIZEDKEY)))
        {
            ...
            user->privKeyLen = 0;
            /* it will be a hex string */
            if (!snmp_hex_to_binary (&user->privKey, &buflen, &user->privKeyLen, 0, cp))
            {
                usm_free_user (user);
                return SNMPERR_GENERR;
            }
...
static int usm_build_probe_pdu (netsnmp_pdu ** pdu)
{
    ...
    *pdu = snmp_pdu_create (SNMP_MSG_GET);
    ...
    if (user == NULL)
    {
        ...
        user->authProtocol = snmp_duplicate_objid (usmNoAuthProtocol, user->authProtocolLen);
        user->privProtocolLen = sizeof (usmNoPrivProtocol) / sizeof (oid);
        user->privProtocol = snmp_duplicate_objid (usmNoPrivProtocol, user->privProtocolLen);
        usm_add_user (user);
    }
    return 0;
}
...
int usm_discover_engineid (void *slpv, netsnmp_session * session)
{
    ...
    switch (status)
    {
        ...
        default:
            DEBUGMSGTL (("snmp_sess_open",
                         ...
                         snmp_api_errstring (session->s_snmp_errno), session->s_snmp_errno));
            break;
    }
    ...
    if (snmp_get_do_debugging ())
    {
        ...
        for (i = 0; i < slp->session->securityEngineIDLen; i++)
            DEBUGMSG (("snmp_sess_open", "%02x", slp->session->securityEngineID[i]));
    ...
    if (session->engineBoots || session->engineTime)
    {
        set_enginetime (session->securityEngineID,
                        session->securityEngineIDLen, session->engineBoots, session->engineTime, TRUE);
    }
    return SNMPERR_SUCCESS;
}
...
void init_usm (void)
{
    ...
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_POST_PREMIB_READ_CONFIG, init_usm_post_config, NULL);

    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_SHUTDOWN, deinit_usm_post_config, NULL);

    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_SHUTDOWN, free_engineID, NULL);

    register_config_handler ("snmp", "defAuthType", snmpv3_authtype_conf, NULL, "MD5|SHA");
     ...
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_SHUTDOWN, free_enginetime_on_shutdown, NULL);


    type = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_APPTYPE);

    register_config_handler (type, "userSetAuthPass", usm_set_password, NULL, NULL);
    register_config_handler (type, "userSetPrivPass", usm_set_password, NULL, NULL);
    register_config_handler (type, "userSetAuthKey", usm_set_password, NULL, NULL);
    register_config_handler (type, "userSetPrivKey", usm_set_password, NULL, NULL);
    register_config_handler (type, "userSetAuthLocalKey", usm_set_password, NULL, NULL);
    register_config_handler (type, "userSetPrivLocalKey", usm_set_password, NULL, NULL);
}

void init_usm_conf (const char *app)
{
    register_config_handler (app, "usmUser", usm_parse_config_usmUser, NULL, NULL);
     ...
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, usm_store_users, NULL);
}
 ...
 */
int init_usm_post_config (int majorid, int minorid, void *serverarg, void *clientarg)
{
    ...
    if (sc_random ((u_char *) & salt_integer64_1, &salt_integer_len) != SNMPERR_SUCCESS)
    {
        DEBUGMSGTL (("usm", "sc_random() failed: using time() as aes1 salt.\n"));
        salt_integer64_1 = (u_int) time (NULL);
    }
    salt_integer_len = sizeof (salt_integer64_1);
    if (sc_random ((u_char *) & salt_integer64_2, &salt_integer_len) != SNMPERR_SUCCESS)
    {
...
#ifndef NETSNMP_DISABLE_MD5
    noNameUser = usm_create_initial_user ("", usmHMACMD5AuthProtocol, USM_LENGTH_OID_TRANSFORM,
#ifndef NETSNMP_DISABLE_DES
                                          usmDESPrivProtocol,
#else
                                          usmAESPrivProtocol,
...
#else
    noNameUser = usm_create_initial_user ("", usmHMACSHA1AuthProtocol, USM_LENGTH_OID_TRANSFORM,
#ifndef NETSNMP_DISABLE_DES
                                          usmDESPrivProtocol,
#else
                                          usmAESPrivProtocol,
...
int deinit_usm_post_config (int majorid, int minorid, void *serverarg, void *clientarg)
{
...
int usm_check_secLevel (int level, struct usmUser *user)
{
    ...
    DEBUGMSGTL (("comparex", "Comparing: %" NETSNMP_PRIo "u %" NETSNMP_PRIo "u ",
                 usmNoPrivProtocol[0], usmNoPrivProtocol[1]));
    DEBUGMSGOID (("comparex", usmNoPrivProtocol, sizeof (usmNoPrivProtocol) / sizeof (oid)));
    DEBUGMSG (("comparex", "\n"));
                                 ...
                                 u_int authProtocolLen, const oid * privProtocol, u_int privProtocolLen)
{
    ...
    if (level == SNMP_SEC_LEVEL_AUTHPRIV
        &&
        (netsnmp_oid_equals
         (privProtocol, privProtocolLen, usmNoPrivProtocol, sizeof (usmNoPrivProtocol) / sizeof (oid)) == 0))
    {
        DEBUGMSGTL (("usm", "Level: %d\n", level));
    ...
    if ((level == SNMP_SEC_LEVEL_AUTHPRIV
        ...
        &&
        (netsnmp_oid_equals
         (authProtocol, authProtocolLen, usmNoAuthProtocol, sizeof (usmNoAuthProtocol) / sizeof (oid)) == 0))
    {
        DEBUGMSGTL (("usm", "Level: %d\n", level));
        DEBUGMSGTL (("usm", "Auth Protocol: "));
        DEBUGMSGOID (("usm", authProtocol, authProtocolLen));
        ...
        if (ptr->name && !strcmp (ptr->name, name))
        {
            ...
            if (ptr->engineIDLen == engineIDLen &&
                ((ptr->engineID == NULL && engineID == NULL) ||
                 (ptr->engineID != NULL && engineID != NULL && memcmp (ptr->engineID, engineID, engineIDLen) == 0)))
                return ptr;
...
struct usmUser *usm_add_user_to_list (struct usmUser *user, struct usmUser *puserList)
{
    ...
    for (nptr = puserList, pptr = NULL; nptr != NULL; pptr = nptr, nptr = nptr->next)
    {
        ...
        if (!(nptr->engineID == NULL && user->engineID != NULL))
        {
            if (nptr->engineIDLen == user->engineIDLen &&
                ((nptr->engineID == NULL && user->engineID == NULL) ||
                 memcmp (nptr->engineID, user->engineID,
                         user->engineIDLen) == 0) && strlen (nptr->name) > strlen (user->name))
                break;
            ...
            if (nptr->engineIDLen == user->engineIDLen &&
                ((nptr->engineID == NULL && user->engineID == NULL) ||
                 memcmp (nptr->engineID, user->engineID,
                         user->engineIDLen) == 0)
                && strlen (nptr->name) == strlen (user->name) && strcmp (nptr->name, user->name) > 0)
                break;
...
struct usmUser *usm_remove_user_from_list (struct usmUser *user, struct usmUser **ppuserList)
{
     ...
     */
    for (nptr = *ppuserList, pptr = NULL; nptr != NULL; pptr = nptr, nptr = nptr->next)
    {
        if (nptr == user)
            break;
    }
...
struct usmUser *usm_cloneFrom_user (struct usmUser *from, struct usmUser *to)
{
    ...
    if ((to->authProtocol = snmp_duplicate_objid (from->authProtocol, from->authProtocolLen)) != NULL)
    ...
    if ((to->privProtocol = snmp_duplicate_objid (from->privProtocol, from->privProtocolLen)) != NULL)
...
struct usmUser *usm_create_user (void)
{
    ...
    if ((newUser->authProtocol =
         snmp_duplicate_objid (usmNoAuthProtocol, sizeof (usmNoAuthProtocol) / sizeof (oid))) == NULL)
        return usm_free_user (newUser);
    newUser->authProtocolLen = sizeof (usmNoAuthProtocol) / sizeof (oid);

    if ((newUser->privProtocol =
         snmp_duplicate_objid (usmNoPrivProtocol, sizeof (usmNoPrivProtocol) / sizeof (oid))) == NULL)
        return usm_free_user (newUser);
                                         ...
                                         const oid * privProtocol, size_t privProtocolLen)
{
    ...
    if ((newUser->privProtocol = snmp_duplicate_objid (privProtocol, privProtocolLen)) == NULL)
    {
        return usm_free_user (newUser);
    }
    ...
    if ((newUser->authProtocol = snmp_duplicate_objid (authProtocol, authProtocolLen)) == NULL)
    {
        return usm_free_user (newUser);
    }
...
int usm_store_users (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    if (appname == NULL)
    {
        appname = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_APPTYPE);
    }
...
void usm_save_users_from_list (struct usmUser *puserList, const char *token, const char *type)
{
    struct usmUser *uptr;
...
void usm_save_user (struct usmUser *user, const char *token, const char *type)
{
    ...
    *cptr++ = ' ';
    cptr = read_config_save_octet_string (cptr, (u_char *) user->name, (user->name == NULL) ? 0 : strlen (user->name));
    *cptr++ = ' ';
    ...
    *cptr++ = ' ';
    cptr = read_config_save_objid (cptr, user->authProtocol, user->authProtocolLen);
    *cptr++ = ' ';
    cptr = read_config_save_octet_string (cptr, user->authKey, user->authKeyLen);
    *cptr++ = ' ';
    cptr = read_config_save_objid (cptr, user->privProtocol, user->privProtocolLen);
    *cptr++ = ' ';
    cptr = read_config_save_octet_string (cptr, user->privKey, user->privKeyLen);
    *cptr++ = ' ';
    cptr = read_config_save_octet_string (cptr, user->userPublicString, user->userPublicStringLen);
...
struct usmUser *usm_read_user (const char *line)
{
    ...
    line = skip_token_const (line);
    line = read_config_read_octet_string_const (line, &user->engineID, &user->engineIDLen);
    ...
    line = read_config_read_objid_const (line, &user->cloneFrom, &user->cloneFromLen);

    SNMP_FREE (user->authProtocol);
    user->authProtocolLen = 0;

    line = read_config_read_objid_const (line, &user->authProtocol, &user->authProtocolLen);
    line = read_config_read_octet_string_const (line, &user->authKey, &user->authKeyLen);
    SNMP_FREE (user->privProtocol);
    user->privProtocolLen = 0;

    line = read_config_read_objid_const (line, &user->privProtocol, &user->privProtocolLen);
    line = read_config_read_octet_string (line, &user->privKey, &user->privKeyLen);
#ifndef NETSNMP_DISABLE_DES
    if (ISTRANSFORM (user->privProtocol, DESPriv))
    {
    ...
    line = read_config_read_octet_string (line, &user->userPublicString, &user->userPublicStringLen);
    return user;
}
...
void usm_set_user_password (struct usmUser *user, const char *token, char *line)
{
    ...
    if (type == 0)
    {
        ...
        ret = generate_Ku (user->authProtocol, user->authProtocolLen, (u_char *) cp, strlen (cp), userKey, &userKeyLen);
    ...
    if (type < 2)
    {
        ...
        *keyLen = SNMP_MAXBUF_SMALL;
        ret = generate_kul (user->authProtocol, user->authProtocolLen,
                            engineID, engineIDLen, userKey, userKeyLen, *key, keyLen);
        if (ret != SNMPERR_SUCCESS)
        {
...
void usm_parse_create_usmUser (const char *token, char *line)
{
     ...
     */
    ret2 = sc_get_properlength (newuser->authProtocol, newuser->authProtocolLen);
    if (ret2 <= 0)
    {
    ...
    if (strcmp (buf, "-l") == 0)
    {
        ...
        ret = ret2;
        if (!snmp_hex_to_binary (&newuser->authKey, &ret, &newuser->authKeyLen, 0, buf))
        {
    ...
    else
    {
        ...
        ret2 = generate_kul (newuser->authProtocol, newuser->authProtocolLen,
                             ...
                             userKey, userKeyLen, newuser->authKey, &newuser->authKeyLen);
        if (ret2 != SNMPERR_SUCCESS)
        {
            config_perror ("could not generate localized authentication key " "(Kul) from the master key (Ku).");
            usm_free_user (newuser);
            return;
        }
    }
    ...
    {
        memcpy (newuser->privProtocol, usmAESPrivProtocol, sizeof (usmAESPrivProtocol));
        testcase = 1;
        privKeyLen = 16;
    }
#endif
    ...
    else
    {
        ...
        if (ret2 < 0)
        {
            config_perror ("could not get proper key length to use for the " "privacy algorithm.");
            usm_free_user (newuser);
            return;
        }
        ...
        if (strcmp (buf, "-l") == 0)
        {
            ...
            if (!snmp_hex_to_binary (&newuser->privKey, &ret, &newuser->privKeyLen, 0, buf))
            {
                config_perror ("invalid key value argument to -l");
                usm_free_user (newuser);
                return;
        ...
        else
        {
            ...
            ret2 = generate_kul (newuser->authProtocol, newuser->authProtocolLen,
                                 newuser->engineID, newuser->engineIDLen,
                                 userKey, userKeyLen, newuser->privKey, &newuser->privKeyLen);
            if (ret2 != SNMPERR_SUCCESS)
            {
                config_perror ("could not generate localized privacy key " "(Kul) from the master key (Ku).");
                usm_free_user (newuser);
                return;
            }
        }
