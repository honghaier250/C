...
int netsnmp_udp_recvfrom (int s, void *buf, int len, struct sockaddr *from, socklen_t * fromlen, struct sockaddr *dstip,
                          socklen_t * dstlen, int *if_index)
{

    /** udpiv4 just calls udpbase. should we skip directly to there? */
    return netsnmp_udpipv4_recvfrom (s, buf, len, from, fromlen, dstip, dstlen, if_index);
}

int netsnmp_udp_sendto (int fd, struct in_addr *srcip, int if_index, struct sockaddr *remote, void *data, int len)
{
...
void netsnmp_udp_parse_security (const char *token, char *param)
{
    ...
    {
        void *v = malloc (offsetof (com2SecEntry, community) + communityLen + secNameLen + contextNameLen);
                        ...
                        const char *community, size_t community_len, const char **secName, const char **contextName)
{
    ...
    if (opaque == NULL || olength != sizeof (netsnmp_udp_addr_pair) || from->sin_family != AF_INET)
    {
    ...
    DEBUGIF ("netsnmp_udp_getSecName")
    {
        ...
        DEBUGMSGTL (("netsnmp_udp_getSecName", "resolve <\"%s\", 0x%08lx>\n",
                     ztcommunity ? ztcommunity : "<malloc error>", (unsigned long) (from->sin_addr.s_addr)));
    }
    ...
    for (c = com2SecList; c != NULL; c = c->next)
    {
        ...
        if ((community_len == strlen (c->community)) &&
            (memcmp (community, c->community, community_len) == 0) && ((from->sin_addr.s_addr & c->mask) == c->network))
        {
...
netsnmp_transport *netsnmp_udp_create_tstring (const char *str, int local, const char *default_target)
{
...
netsnmp_transport *netsnmp_udp_create_ostring (const u_char * o, size_t o_len, int local)
{
    struct sockaddr_in addr;
