        ...
        if (cm->cmsg_level == SOL_IP && cm->cmsg_type == IP_PKTINFO)
        {
            ...
            *if_index = src->ipi_ifindex;
            DEBUGMSGTL (("udpbase:recv",
                         ...
                         inet_ntoa (((struct sockaddr_in *) dstip)->sin_addr), *if_index));
        }
#elif defined(IP_RECVDSTADDR)
...
int netsnmp_udpbase_sendto (int fd, struct in_addr *srcip, int if_index, struct sockaddr *remote, void *data, int len)
{
    struct iovec iov;
    struct msghdr m = { 0 };
    char cmsg[CMSG_SPACE (cmsg_data_size)];
    ...
    if (t != NULL && t->sock >= 0)
    {
        ...
        if (rc >= 0)
        {
            DEBUGIF ("netsnmp_udp")
            {
                ...
                DEBUGMSGTL (("netsnmp_udp", "recvfrom fd %d got %d bytes (from %s)\n", t->sock, rc, str));
                free (str);
            }
        }
        else
...
int netsnmp_udpbase_send (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;
    ...
    if (opaque != NULL && *opaque != NULL &&
        ((*olength == sizeof (netsnmp_indexed_addr_pair) || (*olength == sizeof (struct sockaddr_in)))))
    {
        addr_pair = (netsnmp_indexed_addr_pair *) (*opaque);
    }
    else if (t != NULL && t->data != NULL && t->data_length == sizeof (netsnmp_indexed_addr_pair))
    {
