...
static char *netsnmp_aal5pvc_fmtaddr (netsnmp_transport * t, void *data, int len)
{
    ...
    else if (t != NULL && t->data != NULL && t->data_length == sizeof (struct sockaddr_atmpvc))
    {
    ...
    else
    {
        ...
        sprintf (tmp, "AAL5 PVC: %hd.%hd.%d", to->sap_addr.itf, to->sap_addr.vpi, to->sap_addr.vci);
        return strdup (tmp);
    }
}
...
static int netsnmp_aal5pvc_recv (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;
    ...
    if (t != NULL && t->sock >= 0)
    {
        ...
        if (rc >= 0)
        {
            DEBUGIF ("netsnmp_aal5pvc")
            {
                ...
                DEBUGMSGTL (("netsnmp_aal5pvc", "recv on fd %d got %d bytes (from %s)\n", t->sock, rc, str));
                free (str);
            }
        }
        else
...
static int netsnmp_aal5pvc_send (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;

    struct sockaddr *to = NULL;

    if (opaque != NULL && *opaque != NULL && *olength == sizeof (struct sockaddr_atmpvc))
    {
...
static int netsnmp_aal5pvc_close (netsnmp_transport * t)
{
    int rc = -1;
...
netsnmp_transport *netsnmp_aal5pvc_transport (struct sockaddr_atmpvc * addr, int local)
{
    ...
    t->domain_length = sizeof (netsnmp_AAL5PVCDomain) / sizeof (netsnmp_AAL5PVCDomain[0]);
    ...
    if (local)
    {
        ...
        if (bind (t->sock, (struct sockaddr *) addr, sizeof (struct sockaddr_atmpvc)) < 0)
        {
    ...
    else
    {
        ...
        if (connect (t->sock, (struct sockaddr *) addr, sizeof (struct sockaddr_atmpvc)) < 0)
        {
...
netsnmp_transport *netsnmp_aal5pvc_create_tstring (const char *str, int local, const char *default_target)
{
    ...
    if (str != NULL)
    {
        ...
        if (sscanf (str, "%hd.%hd.%d", &(addr.sap_addr.itf), &(addr.sap_addr.vpi), &(addr.sap_addr.vci)) == 3)
        {
            return netsnmp_aal5pvc_transport (&addr, local);
        }
        else if (sscanf (str, "%hd.%d", &(addr.sap_addr.vpi), &(addr.sap_addr.vci)) == 2)
        {
            addr.sap_addr.itf = 0;
            return netsnmp_aal5pvc_transport (&addr, local);
        }
...
netsnmp_transport *netsnmp_aal5pvc_create_ostring (const u_char * o, size_t o_len, int local)
{
    struct sockaddr_atmpvc addr;
