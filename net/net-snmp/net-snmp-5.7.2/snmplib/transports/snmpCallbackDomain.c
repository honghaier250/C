...
char *netsnmp_callback_fmtaddr (netsnmp_transport * t, void *data, int len)
{
    ...
    snprintf (buf, SPRINT_MAX_LEN, "callback: %d on fd %d", mystuff->callback_num, mystuff->pipefds[0]);
    return strdup (buf);
}
...
int netsnmp_callback_recv (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;
...
int netsnmp_callback_send (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int from, rc = -1;
...
#else
            rc = write (((netsnmp_callback_info *) other_side->data)->pipefds[1], " ", 1);
#endif
            if (rc < 0 && errno != EINTR)
            {
                break;
...
int netsnmp_callback_hook_parse (netsnmp_session * sp, netsnmp_pdu * pdu, u_char * packetptr, size_t len)
{
...
int netsnmp_callback_hook_build (netsnmp_session * sp, netsnmp_pdu * pdu, u_char * ptk, size_t * len)
{
    /*
    ...
    switch (pdu->version)
    {
        ...
        case SNMP_VERSION_3:
            if (pdu->securityNameLen == 0)
            {
                ...
                memmove (pdu->securityName, sp->securityName, sp->securityNameLen);
...
netsnmp_pdu *netsnmp_callback_create_pdu (netsnmp_transport * transport, void *opaque, size_t olength)
{
    netsnmp_pdu *pdu;

    netsnmp_callback_pass *cp = callback_pop_queue (((netsnmp_callback_info *) transport->data)->callback_num);
                                        ...
                                        int (*fpost_parse) (netsnmp_session *, netsnmp_pdu *, int))
{
    ...
    callback_ss = snmp_add_full (&callback_sess, callback_tr,
                                 ...
                                 NULL, netsnmp_callback_check_packet, netsnmp_callback_create_pdu);
    if (callback_ss)
        callback_ss->local_port = ((netsnmp_callback_info *) callback_tr->data)->callback_num;
    return callback_ss;
}
