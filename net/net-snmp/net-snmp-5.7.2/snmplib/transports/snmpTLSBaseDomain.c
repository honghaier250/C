...
int netsnmp_tlsbase_verify_client_cert (SSL * ssl, _netsnmpTLSBaseData * tlsdata)
{
    /* XXX */
    X509 *remote_cert;
...
void _load_trusted_certs (SSL_CTX * the_ctx)
{
...
SSL_CTX *sslctx_server_setup (const SSL_METHOD * method)
{
...
int netsnmp_tlsbase_session_init (struct netsnmp_transport_s *transport, struct snmp_session *sess)
{
...
static void _parse_client_cert (const char *tok, char *line)
{
    ...
    if (*line == '"')
    {
        ...
        netsnmp_ds_set_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_X509_CLIENT_PUB, buf);
    }
    else
        netsnmp_ds_set_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_X509_CLIENT_PUB, line);
}
...
static void _parse_server_cert (const char *tok, char *line)
{
    ...
    if (*line == '"')
    {
        ...
        netsnmp_ds_set_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_X509_SERVER_PUB, line);
}
...
void netsnmp_tlsbase_ctor (void)
{
    ...
    register_config_handler ("snmp", "clientCert", _parse_client_cert, NULL, NULL);
    ...
    register_config_handler ("snmp", "serverCert", _parse_server_cert, NULL, NULL);
    /*
     * remove cert config ambiguity: localCert, peerCert
     */
    netsnmp_ds_register_config (ASN_OCTET_STR, "snmp", "localCert",
                                NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_TLS_LOCAL_CERT);
    netsnmp_ds_register_config (ASN_OCTET_STR, "snmp", "peerCert", NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_TLS_PEER_CERT);

    /*
     * register our boot-strapping needs
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_POST_PREMIB_READ_CONFIG, tls_bootstrap, NULL);

}

_netsnmpTLSBaseData *netsnmp_tlsbase_allocate_tlsdata (netsnmp_transport * t, int isserver)
{
...
int netsnmp_tlsbase_wrapup_recv (netsnmp_tmStateReference * tmStateRef,
                                 _netsnmpTLSBaseData * tlsdata, void **opaque, int *olength)
{
    int no_auth, no_priv;
    ...
    /* XXX: detect and throw out overflow secname sizes rather
       than truncating. */
    strlcpy (tmStateRef->securityName, tlsdata->securityName, sizeof (tmStateRef->securityName));
