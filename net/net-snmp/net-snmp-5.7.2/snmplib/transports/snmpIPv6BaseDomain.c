...
static unsigned netsnmp_if_nametoindex (const char *ifname)
{
#if defined(WIN32)
    return atoi (ifname);
...
static char *netsnmp_if_indextoname (unsigned ifindex, char *ifname)
{
#if defined(WIN32)
...
char *netsnmp_ipv6_fmtaddr (const char *prefix, netsnmp_transport * t, void *data, int len)
{
    ...
    else
    {
        ...
        if (to->sin6_scope_id && netsnmp_if_indextoname (to->sin6_scope_id, &scope_id[1]))
        {
            scope_id[0] = '%';
        }
#endif
...
int netsnmp_sockaddr_in6 (struct sockaddr_in6 *addr, const char *inpeername, int remote_port)
{
