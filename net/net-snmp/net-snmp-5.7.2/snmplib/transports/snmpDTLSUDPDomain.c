...
netsnmp_feature_require (cert_util) netsnmp_feature_require (sockaddr_size)
...
static int netsnmp_dtls_verify_cookie (SSL * ssl, unsigned char *cookie, unsigned int cookie_len);

static int netsnmp_dtls_gen_cookie (SSL * ssl, unsigned char *cookie, unsigned int *cookie_len);
    ...
    for (cachep = biocache; cachep; cachep = cachep->next)
    {
        ...
        if ((from_addr->sa.sa_family == AF_INET) &&
            ((cachep->sas.sin.sin_addr.s_addr !=
              from_addr->sin.sin_addr.s_addr) || (cachep->sas.sin.sin_port != from_addr->sin.sin_port)))
            continue;
#ifdef NETSNMP_TRANSPORT_UDPIPV6_DOMAIN
                 ...
                 ((cachep->sas.sin6.sin6_port != from_addr->sin6.sin6_port) ||
                  ...
                  (memcmp (cachep->sas.sin6.sin6_addr.s6_addr,
                           from_addr->sin6.sin6_addr.s6_addr, sizeof (from_addr->sin6.sin6_addr.s6_addr)) != 0)))
            continue;
#endif
...
static bio_cache *start_new_cached_connection (netsnmp_transport * t,
                                               netsnmp_sockaddr_storage * remote_addr, int we_are_client)
{
    bio_cache *cachep = NULL;
...
static bio_cache *find_or_create_bio_cache (netsnmp_transport * t,
                                            netsnmp_sockaddr_storage * from_addr, int we_are_client)
{
...
static netsnmp_indexed_addr_pair *_extract_addr_pair (netsnmp_transport * t, void *opaque, int olen)
{
    netsnmp_indexed_addr_pair *addr_pair = NULL;

    if (opaque && olen == sizeof (netsnmp_tmStateReference))
    {
        netsnmp_tmStateReference *tmStateRef = (netsnmp_tmStateReference *) opaque;
    ...
    {
        if (t->data != NULL && t->data_length == sizeof (netsnmp_indexed_addr_pair))
            addr_pair = (netsnmp_indexed_addr_pair *) (t->data);
        else if (t->data != NULL && t->data_length == sizeof (_netsnmpTLSBaseData))
        {
...
static struct sockaddr *_find_remote_sockaddr (netsnmp_transport * t, void *opaque, int olen, int *socklen)
{
 ...
 */
static int _netsnmp_send_queued_dtls_pkts (netsnmp_transport * t, bio_cache * cachep)
{
    int outsize, rc2;
    ...
    if (outsize > 0)
    {
        ...
        rc2 = t->base_transport->f_send (t, outbuf, outsize, (void **) &sa, &socksize);
        if (rc2 == -1)
        {
...
static int _netsnmp_bio_try_and_write_buffered (netsnmp_transport * t, bio_cache * cachep)
{
    int rc;
    ...
    while (rc == -1)
    {
        ...
        if ((errnum == SSL_ERROR_WANT_READ || errnum == SSL_ERROR_WANT_WRITE) && bytesout <= 0)
        {
...
static int _netsnmp_add_buffered_data (bio_cache * cachep, char *buf, size_t size)
{
    if (cachep->write_cache && cachep->write_cache_len > 0)
    {
        ...
        memcpy (cachep->write_cache + cachep->write_cache_len, buf, size);
        cachep->write_cache_len = newsize;
    }
    else
    {
        if (SNMPERR_SUCCESS != memdup ((u_char **) & cachep->write_cache, buf, size))
        {
            /* ack! malloc failure */
            /* XXX: free and close */
            return SNMPERR_GENERR;
...
static int netsnmp_dtlsudp_recv (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;
    ...
    memcpy (tmStateRef->transportDomain,
            netsnmpDTLSUDPDomain, sizeof (netsnmpDTLSUDPDomain[0]) * netsnmpDTLSUDPDomain_len);
    ...
    /* if we don't have a cachep for this connection then
       ...
       side */
    cachep = find_or_create_bio_cache (t, &addr_pair->remote_addr, WE_ARE_SERVER);
    if (NULL == cachep)
    {
    ...
    if (NULL == tlsdata->ssl)
    {
         ...
         */
        snmp_log (LOG_ERR, "DTLSUDP: missing tlsdata!\n");
    ...
    if (0 == (tlsdata->flags & NETSNMP_TLSBASE_CERT_FP_VERIFIED))
    {
        ...
        if (tlsdata->flags & NETSNMP_TLSBASE_IS_CLIENT)
        {
             ...
             */
            if ((verifyresult = netsnmp_tlsbase_verify_server_cert (tlsdata->ssl, tlsdata)) != SNMPERR_SUCCESS)
            {
                ...
                else
                {
                    ...
                    snmp_log (LOG_ERR, "DTLSUDP: failed to verify ssl certificate (of the server)\n");
        ...
        else
        {
            ...
            if ((verifyresult = netsnmp_tlsbase_verify_client_cert (tlsdata->ssl, tlsdata)) != SNMPERR_SUCCESS)
            {
                if (verifyresult == SNMPERR_TLS_NO_CERTIFICATE)
                {
                    /* assume we simply haven't received it yet and there
                ...
                {
                    /* XXX: free needed memory */
                    snmp_log (LOG_ERR, "DTLSUDP: failed to verify ssl certificate (of the client)\n");
    ...
    DEBUGIF ("9:dtlsudp")
    {
        ...
        DEBUGMSGTL (("9:dtlsudp", "recvfrom fd %d got %d bytes (from %s)\n", t->sock, rc, str));
        free (str);
    }
    ...
    if (netsnmp_tlsbase_wrapup_recv (tmStateRef, tlsdata, opaque, olength) != SNMPERR_SUCCESS)
        return SNMPERR_GENERR;
...
static int netsnmp_dtlsudp_send (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;
    ...
    /* Implementation notes:
       ...
       specs) then we create one automatically here.
     */
    if (opaque != NULL && *opaque != NULL && olength != NULL && *olength == sizeof (netsnmp_tmStateReference))
    ...
    if (NULL == (cachep = find_or_create_bio_cache (t, &addr_pair->remote_addr, WE_ARE_CLIENT)))
    {
        snmp_increment_statistic (STAT_TLSTM_SNMPTLSTMSESSIONOPENERRORS);
        return -1;
    }
    ...
    if (cachep->write_cache)
    {
        if (SNMPERR_GENERR == _netsnmp_bio_try_and_write_buffered (t, cachep))
        {
            ...
            if (_netsnmp_add_buffered_data (cachep, buf, size) != SNMPERR_SUCCESS)
            {
                /* XXX: free and close */
            }
            return -1;
...
static int netsnmp_dtlsudp_close (netsnmp_transport * t)
{
    ...
    if (cachep->write_cache_len > 0)
    {
        ...
        DEBUGMSGTL (("dtlsudp:close", "%" NETSNMP_PRIz "d bytes remain in write_cache\n", cachep->write_cache_len));
        ...
        if (cachep->flags & NETSNMP_BIO_CONNECTED)
        {
            ...
            for (i = 0; i < 6 && cachep->write_cache_len != 0; ++i)
            {
                ...
                if (rc > 0)
                {
                    ...
                    opaque_len = 0;
                    netsnmp_dtlsudp_recv (t, buf, sizeof (buf), &opaque, &opaque_len);
                    SNMP_FREE (opaque);
                }
            }                    /* for loop */
        }
...
char *netsnmp_dtlsudp_fmtaddr (netsnmp_transport * t, void *data, int len)
{
    int sa_len;
...
static netsnmp_transport *_transport_common (netsnmp_transport * t, int local)
{
    char *tmp = NULL;
    ...
    {
        _netsnmpTLSBaseData *tlsdata = netsnmp_tlsbase_allocate_tlsdata (t, local);
...
char *netsnmp_dtlsudp6_fmtaddr (netsnmp_transport * t, void *data, int len)
{
    int sa_len;
...
netsnmp_transport *netsnmp_dtlsudp6_transport (struct sockaddr_in6 * addr, int local)
{
    netsnmp_transport *t = NULL;
...
netsnmp_transport *netsnmp_dtlsudp_create_tstring (const char *str, int isserver, const char *default_target)
{
#ifdef NETSNMP_TRANSPORT_UDPIPV6_DOMAIN
    ...
    if (!isserver && t && t->data && str)
    {
        ...
        {
            sprintf (buf, "%.*s", (int) SNMP_MIN (cp - str, sizeof (buf) - 1), str);
        }
        else
        {
...
netsnmp_transport *netsnmp_dtlsudp_create_ostring (const u_char * o, size_t o_len, int local)
{
    struct sockaddr_in addr;
...
void netsnmp_dtlsudp_ctor (void)
{
    ...
    if (!openssl_addr_index6)
        openssl_addr_index6 = SSL_get_ex_new_index (0, indexname6, NULL, NULL, NULL);
#endif

    dtlsudpDomain.name = netsnmpDTLSUDPDomain;
    dtlsudpDomain.name_length = netsnmpDTLSUDPDomain_len;
    dtlsudpDomain.prefix = (const char **) calloc (num_prefixes + 1, sizeof (char *));
...
int netsnmp_dtls_gen_cookie (SSL * ssl, unsigned char *cookie, unsigned int *cookie_len)
{
    ...
    HMAC (EVP_sha1 (), cookie_secret, NETSNMP_COOKIE_SECRET_LENGTH, buffer, length, result, &resultlength);
    OPENSSL_free (buffer);
...
int netsnmp_dtls_verify_cookie (SSL * ssl, unsigned char *cookie, unsigned int cookie_len)
{
    unsigned char *buffer, result[EVP_MAX_MD_SIZE];
...
#endif
        default:
            snmp_log (LOG_ERR, "dtls: unknown address family %d generating a cookie\n", peer->sa.sa_family);
            return 0;
    }
    buffer = malloc (length);
    if (buffer == NULL)
