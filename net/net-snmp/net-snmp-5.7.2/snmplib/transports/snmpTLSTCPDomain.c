...
static char *netsnmp_tlstcp_fmtaddr (netsnmp_transport * t, void *data, int len)
{
    if (NULL == data || 0 == len || 0 == ((char *) data)[0])
        return strdup ("TLSTCP: unknown");
    else if (len == sizeof (netsnmp_indexed_addr_pair) || len == sizeof (struct sockaddr_in))
...
static int netsnmp_tlstcp_copy (netsnmp_transport * oldt, netsnmp_transport * newt)
{
    ...
    if (oldtlsdata->addr)
        memdup ((u_char **) & newtlsdata->addr, oldtlsdata->addr, sizeof (*oldtlsdata->addr));

    return 0;
}

static int netsnmp_tlstcp_recv (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;
    ...
    if (NULL == tlsdata->ssl)
    {
        snmp_log (LOG_ERR, "tlstcp received an invalid invocation without ssl data\n");
        return -1;
    }
    ...
    memcpy (tmStateRef->transportDomain,
            netsnmpTLSTCPDomain, sizeof (netsnmpTLSTCPDomain[0]) * netsnmpTLSTCPDomain_len);
    ...
    DEBUGIF ("tlstcp")
    {
        ...
        DEBUGMSGTL (("tlstcp", "recvfrom fd %d got %d bytes (from %s)\n", t->sock, rc, str));
        free (str);
    }
...
static int netsnmp_tlstcp_send (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;
...
static int netsnmp_tlstcp_close (netsnmp_transport * t)
{
    _netsnmpTLSBaseData *tlsdata;
...
static int netsnmp_tlstcp_accept (netsnmp_transport * t)
{
    BIO *accepted_bio;
     ...
     */
    if ((rc = netsnmp_tlsbase_verify_client_cert (ssl, tlsdata)) != SNMPERR_SUCCESS)
    {
...
netsnmp_transport *netsnmp_tlstcp_open (netsnmp_transport * t)
{
    _netsnmpTLSBaseData *tlsdata;
    ...
    netsnmp_assert_or_return (sizeof (_netsnmpTLSBaseData) == t->data_length, NULL);
...
netsnmp_transport *netsnmp_tlstcp_transport (const char *addr_string, int isserver)
{
    ...
    if (!isserver && tlsdata && addr_string)
    {
        ...
        {
            sprintf (buf, "%.*s", (int) SNMP_MIN (cp - addr_string, sizeof (buf) - 1), addr_string);
        }
        else
        {
...
netsnmp_transport *netsnmp_tlstcp_create_tstring (const char *str, int local, const char *default_target)
{
    char buf[SPRINT_MAX_LEN];
...
netsnmp_transport *netsnmp_tlstcp_create_ostring (const u_char * o, size_t o_len, int local)
{
    char buf[SPRINT_MAX_LEN];
