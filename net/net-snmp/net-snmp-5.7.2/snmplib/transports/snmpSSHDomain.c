 ...
 * public either
 */
int netsnmp_sockaddr_in2 (struct sockaddr_in *addr, const char *inpeername, const char *default_target);
...
static char *netsnmp_ssh_fmtaddr (netsnmp_transport * t, void *data, int len)
{
    ...
    else
    {
        ...
        sprintf (tmp, "SSH: [%s]:%hd", inet_ntoa (to->sin_addr), ntohs (to->sin_port));
        return strdup (tmp);
    }
}
...
static int netsnmp_ssh_recv (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;
    ...
    if (t != NULL && addr_pair && addr_pair->channel)
    {
        ...
        while (rc < 0)
        {
            ...
            if (rc < 0)
            {                    /* XXX: from tcp; ssh equiv?:  && errno != EINTR */
    ...
    else if (t != NULL)
    {
            ...
            if (addr_pair->username[0] == '\0')
            {
                ...
                for (cmsgptr = CMSG_FIRSTHDR (&msg); cmsgptr != NULL; cmsgptr = CMSG_NXTHDR (&msg, cmsgptr))
                {
                    if (cmsgptr->cmsg_level == SOL_SOCKET && cmsgptr->cmsg_type == SCM_CREDENTIALS)
                    {
                        ...
                        strlcpy (addr_pair->username, user_pw->pw_name, sizeof (addr_pair->username));
                    }
                ...
                if (addr_pair->username[0] == '\0')
                {
                    snmp_log (LOG_ERR, "failed to extract username from sshd connected unix socket\n");
                    return -1;
                }
        ...
        while (rc < 0)
        {
            ...
            if (rc < 0 && errno != EINTR)
            {
                DEBUGMSGTL (("ssh", " read on stdin failed: %d (\"%s\")\n", errno, strerror (errno)));
                break;
            }
            if (rc == 0)
            {
...
/* XXX: need to check the username, but addr_pair doesn't exist! */
    ...
    if (iamclient && 0)
    {
        ...
        strlcpy (tmStateRef->securityName, addr_pair->username, sizeof (tmStateRef->securityName));
    }
    else
    {
#ifdef SNMPSSHDOMAIN_USE_EXTERNAL_PIPE
        ...
        /* XXX: detect and throw out overflow secname sizes rather
           than truncating. */
        strlcpy (tmStateRef->securityName, getenv ("USER"), sizeof (tmStateRef->securityName));
#endif                            /* ! SNMPSSHDOMAIN_USE_EXTERNAL_PIPE */
    }
...
static int netsnmp_ssh_send (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;
    ...
    if (opaque != NULL && *opaque != NULL && *olength == sizeof (netsnmp_tmStateReference))
    {
    ...
    if (NULL != t && NULL != addr_pair && NULL != addr_pair->channel)
    {
        if (addr_pair->username[0] == '\0')
        {
            strlcpy (addr_pair->username, tmStateRef->securityName, sizeof (addr_pair->username));
        }
                 ...
                 strlen (addr_pair->username) != tmStateRef->securityNameLen)
        {
            ...
            snmp_log (LOG_ERR,
                      "netsnmp_ssh_send was passed a tmStateReference with a securityName not equal to previous messages\n");
            return -1;
        }
        while (rc < 0)
        {
...
static int netsnmp_ssh_close (netsnmp_transport * t)
{
    int rc = -1;
...
static int netsnmp_ssh_accept (netsnmp_transport * t)
{
#ifdef SNMPSSHDOMAIN_USE_EXTERNAL_PIPE
    ...
    if (t != NULL && t->sock >= 0)
    {
        ...
        /* set the SO_PASSCRED option so we can receive the remote uid */
        {
            ...
            setsockopt (newsock, SOL_SOCKET, SO_PASSCRED, (void *) &one, sizeof (one));
        }
...
netsnmp_transport *netsnmp_ssh_transport (struct sockaddr_in * addr, int local)
{
    ...
    const char *sockpath = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID,
    ...
    if (local)
    {
        ...
        if (NULL == sockpath)
        {
            sprintf (tmpsockpath, "%s/%s", get_persistent_directory (), DEFAULT_SOCK_NAME);
        ...
        /* set the SO_PASSCRED option so we can receive the remote uid */
        {
            ...
            setsockopt (t->sock, SOL_SOCKET, SO_PASSCRED, (void *) &one, sizeof (one));
        }
        ...
        /* set the socket permissions */
        {
             ...
             * Sshdomain socket
             */
            int sshdomain_sock_perm = netsnmp_ds_get_int (NETSNMP_DS_LIBRARY_ID,
                                                          NETSNMP_DS_SSHDOMAIN_SOCK_PERM);

            int sshdomain_sock_user = netsnmp_ds_get_int (NETSNMP_DS_LIBRARY_ID,
                                                          NETSNMP_DS_SSHDOMAIN_SOCK_USER);

            int sshdomain_sock_group = netsnmp_ds_get_int (NETSNMP_DS_LIBRARY_ID,
                                                           NETSNMP_DS_SSHDOMAIN_SOCK_GROUP);

            DEBUGMSGTL (("ssh", "here: %s, %d, %d, %d\n",
                         unaddr->sun_path, sshdomain_sock_perm, sshdomain_sock_user, sshdomain_sock_group));
            ...
            if (sshdomain_sock_user || sshdomain_sock_group)
            {
                ...
                chown (unaddr->sun_path, sshdomain_sock_user, sshdomain_sock_group);
            }
        }
    ...
    else
    {
        ...
        username = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SSH_USERNAME);
        ...
        keyfilepub = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SSH_PUBKEY);
        ...
        keyfilepriv = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SSH_PRIVKEY);
         ...
         */
        fingerprint = libssh2_hostkey_hash (addr_pair->session, LIBSSH2_HOSTKEY_HASH_MD5);
        DEBUGMSGTL (("ssh", "Fingerprint: "));
        for (i = 0; i < 16; i++)
        {
            DEBUGMSG (("ssh", "%02x", (unsigned char) fingerprint[i]));
        }
        DEBUGMSG (("ssh", "\n"));

        /* check what authentication methods are available */
        userauthlist = libssh2_userauth_list (addr_pair->session, username, strlen (username));
        ...
        {
            /* public key */
            if (libssh2_userauth_publickey_fromfile (addr_pair->session, username, keyfilepub, keyfilepriv, NULL))
            {
            ...
            {
                DEBUGMSG (("ssh", "\tAuthentication by public key succeeded.\n"));
            }
        }
        else
        {
        ...
        /* Request a channel */
        if (!(addr_pair->channel = libssh2_channel_open_session (addr_pair->session)))
        {
...
netsnmp_transport *netsnmp_ssh_create_tstring (const char *str, int local, const char *default_target)
{
...
netsnmp_transport *netsnmp_ssh_create_ostring (const u_char * o, size_t o_len, int local)
{
    struct sockaddr_in addr;
...
void sshdomain_parse_socket (const char *token, char *cptr)
{
    ...
    if (socket_perm)
    {
        ...
        netsnmp_ds_set_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_SSHDOMAIN_SOCK_PERM, s_perm);
    ...
    if (socket_user)
    {
        ...
        if (uid != 0)
            netsnmp_ds_set_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_SSHDOMAIN_SOCK_USER, uid);
    ...
    if (socket_group)
    {
        ...
        if (gid != 0)
            netsnmp_ds_set_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_SSHDOMAIN_SOCK_GROUP, gid);
...
void netsnmp_ssh_ctor (void)
{
    ...
    register_config_handler ("snmp", "sshtosnmpsocketperms",
                             &sshdomain_parse_socket, NULL, "socketperms [username [groupname]]");

    netsnmp_ds_register_config (ASN_OCTET_STR, "snmp", "sshtosnmpsocket",
                                NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SSHTOSNMP_SOCKET);

    netsnmp_ds_register_config (ASN_OCTET_STR, "snmp", "sshusername",
                                NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SSH_USERNAME);
    ...
    netsnmp_ds_register_config (ASN_OCTET_STR, "snmp", "sshprivatekey",
                                NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SSH_PRIVKEY);
