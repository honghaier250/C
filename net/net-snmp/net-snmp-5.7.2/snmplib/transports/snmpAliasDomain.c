...
void parse_alias_config (const char *token, char *line)
{
    ...
    netsnmp_data_list_add_node (&alias_memory,
                                netsnmp_create_data_list (aliasname, strdup (transportdef), &free_wrapper));
}
...
netsnmp_transport *netsnmp_alias_create_tstring (const char *str, int local, const char *default_target)
{
...
netsnmp_transport *netsnmp_alias_create_ostring (const u_char * o, size_t o_len, int local)
{
...
void netsnmp_alias_ctor (void)
{
    ...
    register_config_handler ("snmp", "alias", parse_alias_config, free_alias_config, "NAME TRANSPORT_DEFINITION");
}
