...
static char *netsnmp_unix_fmtaddr (netsnmp_transport * t, void *data, int len)
{
...
static int netsnmp_unix_recv (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;
...
static int netsnmp_unix_send (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;
...
netsnmp_transport *netsnmp_unix_transport (struct sockaddr_un *addr, int local)
{
    ...
    t->domain_length = sizeof (netsnmp_UnixDomain) / sizeof (netsnmp_UnixDomain[0]);
...
netsnmp_transport *netsnmp_unix_create_tstring (const char *string, int local, const char *default_target)
{
...
netsnmp_transport *netsnmp_unix_create_ostring (const u_char * o, size_t o_len, int local)
{
    struct sockaddr_un addr;
...
netsnmp_unix_getSecName (void *opaque, int olength,
                         const char *community, size_t community_len, const char **secName, const char **contextName)
{
    ...
    if (opaque == NULL || olength != sizeof (struct sockaddr_un) || to->sun_family != AF_UNIX)
    {
        DEBUGMSGTL (("netsnmp_unix_getSecName", "no unix destine address in PDU?\n"));
        return 1;
    }
    ...
    {
        DEBUGMSGTL (("netsnmp_unix_getSecName", "compare <\"%s\",to socket %s>", c->community, c->sockpath));
        if ((community_len == strlen (c->community)) && (memcmp (community, c->community, community_len) == 0) &&
...
void netsnmp_unix_agent_config_tokens_register (void)
{
    ...
    register_app_config_handler ("com2secunix", netsnmp_unix_parse_security,
                                 netsnmp_unix_com2SecList_free, "[-Cn CONTEXT] secName sockpath community");
#endif                            /* support for community based SNMP */
}
