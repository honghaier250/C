...
int netsnmp_sockaddr_in (struct sockaddr_in *addr, const char *inpeername, int remote_port)
{
    ...
    if (inpeername != NULL && *inpeername != '\0')
    {
        ...
        if (port != NULL)
        {
            ...
            else
            {
                ...
                {
                    DEBUGMSGTL (("netsnmp_sockaddr_in", "servname not numeric\n"));
                    free (peername);
                    return 0;
                }
            }
        ...
        if (host != NULL)
        {
            ...
            else
            {
                ...
                if (ret < 0)
                {
                    DEBUGMSGTL (("netsnmp_sockaddr_in", "couldn't resolve hostname\n"));
                    free (peername);
                    return 0;
                }
...
char *netsnmp_ipv4_fmtaddr (const char *prefix, netsnmp_transport * t, void *data, int len)
{
    ...
    else
    {
        ...
        if (to == NULL)
        {
            snprintf (tmp, sizeof (tmp), "%s: unknown->[%s]:%hu", prefix,
                      inet_ntoa (addr_pair->local_addr.sin.sin_addr), ntohs (addr_pair->local_addr.sin.sin_port));
        }
        ...
        {
            snprintf (tmp, sizeof (tmp), "%s: [%s]:%hu->", prefix, inet_ntoa (to->sin_addr), ntohs (to->sin_port));
