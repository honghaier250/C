...
static char *netsnmp_ipx_fmtaddr (netsnmp_transport * t, void *data, int len)
{
...
static int netsnmp_ipx_recv (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;
...
static int netsnmp_ipx_send (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;

    struct sockaddr *to = NULL;

    if (opaque != NULL && *opaque != NULL && *olength == sizeof (struct sockaddr_ipx))
    {
        to = (struct sockaddr *) (*opaque);
    }
    else if (t != NULL && t->data != NULL && t->data_length == sizeof (struct sockaddr_ipx))
    {
...
static int netsnmp_ipx_close (netsnmp_transport * t)
{
    int rc = -1;
...
int netsnmp_sockaddr_ipx2 (struct sockaddr_ipx *addr, const char *peername, const char *default_target)
{
    ...
    DEBUGMSGTL (("netsnmp_sockaddr_ipx",
                 ...
                 addr, peername ? peername : "[NIL]", default_target ? default_target : "[NIL]"));
    ...
    DEBUGMSGTL (("netsnmp_sockaddr_ipx", "Address: %s:%s/%s\n",
                 network ? network : "[NIL]", node ? node : "[NIL]", port ? port : "[NIL]"));
    ...
    {
        DEBUGMSGTL (("netsnmp_sockaddr_ipx", "failed to parse network part of address\n"));
        free (def);
        free (input);
        return 0;
    }
    ...
    if (sscanf (node, "%2hhx%2hhx%2hhx%2hhx%2hhx%2hhx%*c",
                ...
                &addr->sipx_node[2], &addr->sipx_node[3], &addr->sipx_node[4], &addr->sipx_node[5]) == 6)
    {
        DEBUGMSGTL (("netsnmp_sockaddr_ipx", "node parsed okay\n"));
    }
    else
    {
        DEBUGMSGTL (("netsnmp_sockaddr_ipx", "failed to parse node part of address\n"));
        free (def);
        free (input);
        return 0;
    }
    ...
    {
        DEBUGMSGTL (("netsnmp_sockaddr_ipx", "failed to parse port part of address\n"));
        free (def);
        free (input);
        return 0;
    }
...
int netsnmp_sockaddr_ipx (struct sockaddr_ipx *addr, const char *peername)
{
...
netsnmp_transport *netsnmp_ipx_create_tstring (const char *str, int local, const char *default_target)
{
...
netsnmp_transport *netsnmp_ipx_create_ostring (const u_char * o, size_t o_len, int local)
{
    struct sockaddr_ipx addr;
