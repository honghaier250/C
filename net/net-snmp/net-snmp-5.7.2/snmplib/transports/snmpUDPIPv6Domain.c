...
static char *netsnmp_udp6_fmtaddr (netsnmp_transport * t, void *data, int len)
{
...
static int netsnmp_udp6_recv (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;
    ...
    if (t != NULL && t->sock >= 0)
    {
        ...
        if (rc >= 0)
        {
            DEBUGIF ("netsnmp_udp6")
            {
                ...
                DEBUGMSGTL (("netsnmp_udp6", "recvfrom fd %d got %d bytes (from %s)\n", t->sock, rc, str));
                free (str);
            }
        }
        else
...
static int netsnmp_udp6_send (netsnmp_transport * t, void *buf, int size, void **opaque, int *olength)
{
    int rc = -1;

    struct sockaddr *to = NULL;

    if (opaque != NULL && *opaque != NULL && *olength == sizeof (struct sockaddr_in6))
    {
...
netsnmp_transport *netsnmp_udp6_transport (struct sockaddr_in6 * addr, int local)
{
    ...
    t->domain_length = sizeof (netsnmp_UDPIPv6Domain) / sizeof (netsnmp_UDPIPv6Domain[0]);
...
create_com2Sec6Entry (const struct addrinfo *const run,
                      ...
                      const size_t communityLen, com2Sec6Entry ** const begin, com2Sec6Entry ** const end)
{
    const struct sockaddr_in6 *const run_addr = (const struct sockaddr_in6 *) run->ai_addr;
...

    {
        ...
        DEBUGMSGTL (("netsnmp_udp6_parse_security",
                     ...
                     inet_ntop (AF_INET6, &run_addr->sin6_addr,
                                buf1, sizeof (buf1)), inet_ntop (AF_INET6, mask, buf2, sizeof (buf2)), secName));
    }

    {
        /* Allocate all the needed chunks */
        void *const v = malloc (offsetof (com2Sec6Entry, community) + communityLen + secNameLen + contextNameLen);
...
void netsnmp_udp6_parse_security (const char *token, char *param)
{
     ...
     */
    {
...

        {
            ...
            if (isdefault)
            {
                memset (&pton_addr.sin6_addr.s6_addr, '\0', sizeof (struct in6_addr));
            }
                ...
                for (run = res; run && !failed; run = run->ai_next)
                    failed =
                        create_com2Sec6Entry (run, &mask,
                                              secName, secNameLen,
                                              contextName, contextNameLen, community, communityLen, &begin, &end);
...
netsnmp_udp6_getSecName (void *opaque, int olength,
                         const char *community, int community_len, const char **secName, const char **contextName)
{
    ...
    for (c = com2Sec6List; c != NULL; c = c->next)
    {
        ...
        if ((community_len == (int) strlen (c->community)) && (memcmp (community, c->community, community_len) == 0))
        {
            int i, ok = 1;

            for (i = 0; ok && i < 16; ++i)
                if ((from->sin6_addr.s6_addr[i] & c->mask.s6_addr[i]) != c->network.s6_addr[i])
                    ok = 0;
            if (ok)
            {
...
netsnmp_transport *netsnmp_udp6_create_tstring (const char *str, int local, const char *default_target)
{
...
netsnmp_transport *netsnmp_udp6_create_ostring (const u_char * o, size_t o_len, int local)
{
    struct sockaddr_in6 addr;
