 ...
 * size as possible)
 */
static int _sock_buffer_maximize (int s, int optname, const char *buftype, int size)
{
    int curbuf = 0;
     ...
     */
    if ((getsockopt (s, SOL_SOCKET, optname, (void *) &curbuf, &curbuflen) == 0) && (curbuflen == sizeof (int)))
    {
        ...
        if (size <= curbuf)
        {
            DEBUGMSGTL (("verbose:socket:buffer:max", "Requested %s <= current buffer\n", buftype));
            return curbuf;
        }
        ...
        {
            mid = (lo + hi) / 2;
            if (setsockopt (s, SOL_SOCKET, optname, (void *) &mid, sizeof (int)) == 0)
            {
        ...
        {
            DEBUGMSGTL (("socket:buffer:max", "Maximized %s: %d\n", buftype, curbuf));
        }
    }
    else
    {
...
static int _sock_buffer_size_get (int optname, int local, const char **buftype)
{
    ...
    if (optname == SO_SNDBUF)
    {
        if (local)
        {
            size = netsnmp_ds_get_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SERVERSENDBUF);
        ...
        {
            size = netsnmp_ds_get_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_CLIENTSENDBUF);
    ...
    else if (optname == SO_RCVBUF)
    {
        if (local)
        {
            size = netsnmp_ds_get_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SERVERRECVBUF);
        ...
        {
            size = netsnmp_ds_get_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_CLIENTRECVBUF);
...
#   ifndef  SO_SNDBUF
    ...
    {
        DEBUGMSGTL (("socket:buffer", "Changing socket send buffer is not supported\n"));
        return -1;
    }
#   endif                        /*SO_SNDBUF */
#   ifndef  SO_RCVBUF
    ...
    {
        DEBUGMSGTL (("socket:buffer", "Changing socket receive buffer is not supported\n"));
        return -1;
    }
#   endif                        /*SO_RCVBUF */
    ...
    if ((getsockopt (s, SOL_SOCKET, optname, (void *) &curbuf, &curbuflen) == 0) && (curbuflen == sizeof (int)))
    {
        ...
        if (curbuf >= size)
        {
            DEBUGMSGT (("verbose:socket:buffer", "New %s size is smaller than original!\n", buftype));
        }
    }
    ...
    if (size <= 0)
    {
        DEBUGMSGT (("socket:buffer", "%s not valid or not specified; using OS default(%d)\n", buftype, curbuf));
        return curbuf;
    }
