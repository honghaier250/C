...
void init_snmp_logging (void)
{
    ...
    register_prenetsnmp_mib_handler ("snmp", "logOption", parse_config_logOption, NULL, "string");
...
int snmp_stderrlog_status (void)
{
    ...
    for (logh = logh_head; logh; logh = logh->next)
        if (logh->enabled && (logh->type == NETSNMP_LOGHANDLER_STDOUT || logh->type == NETSNMP_LOGHANDLER_STDERR))
        {
            return 1;
        }
...
void snmp_enable_syslog_ident (const char *ident, const int facility)
{
    ...
    if (eventlog_h == NULL)
    {
         ...
         */
        fprintf (stderr, "Could not open event log for %s. " "Last error: 0x%x\n", ident, GetLastError ());
        enable = 0;
    }
#else
...
void snmp_enable_stderrlog (void)
{
    ...
    for (logh = logh_head; logh; logh = logh->next)
        if (logh->type == NETSNMP_LOGHANDLER_STDOUT || logh->type == NETSNMP_LOGHANDLER_STDERR)
        {
            netsnmp_enable_this_loghandler (logh);
            found = 1;
        }
    ...
    if (!found)
    {
        logh = netsnmp_register_loghandler (NETSNMP_LOGHANDLER_STDERR, LOG_DEBUG);
        if (logh)
            logh->token = strdup ("stderr");
    }
}
...
void snmp_enable_calllog (void)    /* XXX - or take a callback routine ??? */
{
    ...
    if (!found)
    {
        logh = netsnmp_register_loghandler (NETSNMP_LOGHANDLER_CALLBACK, LOG_DEBUG);
        if (logh)
...
int netsnmp_add_loghandler (netsnmp_log_handler * logh)
{
    ...
    for (i = LOG_EMERG; i <= logh->priority; i++)
        if (!logh_priorities[i] || logh_priorities[i]->priority >= logh->priority)
...
int log_handler_stdouterr (netsnmp_log_handler * logh, int pri, const char *str)
{
    ...
    if (netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_LOG_TIMESTAMP) && newline)
    {
...
int log_handler_file (netsnmp_log_handler * logh, int pri, const char *str)
{
     ...
     */
    if (netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_LOG_TIMESTAMP) && logh->imagic)
    {
        sprintf_stamp (NULL, sbuf);
    }
    else
...
void snmp_log_string (int priority, const char *str)
{
    ...
    {
        stderr_enabled = 0;
        log_handler_stdouterr (&lh, LOG_INFO, "Log handling defined - disabling stderr\n");
    }
#endif                            /* NETSNMP_FEATURE_REMOVE_LOGGING_STDIO */
