...
NETSNMP_INLINE void netsnmp_data_list_add_node (netsnmp_data_list ** head, netsnmp_data_list * node)
{
    ...
    if (0 == strcmp (node->name, (*head)->name))
    {
        ...
        snmp_log (LOG_WARNING, "WARNING: adding duplicate key '%s' to data list\n", node->name);
    }
...
NETSNMP_INLINE netsnmp_data_list *netsnmp_data_list_add_data (netsnmp_data_list ** head, const char *name,
...
netsnmp_register_save_list (netsnmp_data_list ** datalist,
                            ...
                            Netsnmp_Read_List_Data * data_list_read_ptr, Netsnmp_Free_List_Data * data_list_free_ptr)
{
    ...
    if (!info->type)
    {
        info->type = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_APPTYPE);
    }

    /* function which will save the data */
    info->data_list_save_ptr = data_list_save_ptr;
    if (data_list_save_ptr)
        snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, netsnmp_save_all_data_callback, info);
    ...
    if (data_list_read_ptr)
    {
        ...
        netsnmp_add_list_data (&saveHead, netsnmp_create_data_list (token, info, NULL));
        register_config_handler (type, token, netsnmp_read_data_callback, NULL /* XXX */ , NULL);
    }
...
int netsnmp_save_all_data_callback (int major, int minor, void *serverarg, void *clientarg)
{
    ...
    netsnmp_save_all_data (*(info->datalist), info->type, info->token, info->data_list_save_ptr);
    return SNMP_ERR_NOERROR;
}
...
netsnmp_save_all_data (netsnmp_data_list * head,
                       const char *type, const char *token, Netsnmp_Save_List_Data * data_list_save_ptr)
{
    ...
    for (; head; head = head->next)
    {
        if (head->name)
        {
            ...
            cp = buf + strlen (buf);
            cp = read_config_save_octet_string (cp, (u_char *) head->name, strlen (head->name));
            *cp++ = ' ';

            /* call registered function to save the rest */
            if (!(data_list_save_ptr) (cp, sizeof (buf) - strlen (buf), head->data))
            {
...
void netsnmp_read_data_callback (const char *token, char *line)
{
    ...
    /* add to the datalist */
    netsnmp_add_list_data (info->datalist, netsnmp_create_data_list (dataname, data, info->data_list_free_ptr));
