...
static void _asn_size_err (const char *str, size_t wrongsize, size_t rightsize)
{
    ...
    snprintf (ebuf, sizeof (ebuf), "%s size %lu: s/b %lu", str, (unsigned long) wrongsize, (unsigned long) rightsize);
...
static void _asn_length_err (const char *str, size_t wrongsize, size_t rightsize)
{
    ...
    snprintf (ebuf, sizeof (ebuf),
              "%s length %lu too large: exceeds %lu", str, (unsigned long) wrongsize, (unsigned long) rightsize);
    ebuf[sizeof (ebuf) - 1] = 0;
    ERROR_MSG (ebuf);
}
 ...
 */
static int _asn_parse_length_check (const char *str, const u_char * bufp, const u_char * data, u_long plen, size_t dlen)
{
    char ebuf[128];
    ...
    {
        snprintf (ebuf, sizeof (ebuf),
                  "%s: message overflow: %d len + %d delta > %d len", str, (int) plen, (int) header_len, (int) dlen);
        ebuf[sizeof (ebuf) - 1] = 0;
        ERROR_MSG (ebuf);
        return 1;
    }
 ...
 */
static int _asn_build_header_check (const char *str, const u_char * data, size_t datalen, size_t typedlen)
{
    char ebuf[128];
 ...
 * @return 0 on success 1 on error 
 */
static int _asn_realloc_build_header_check (const char *str, u_char ** pkt, const size_t * pkt_len, size_t typedlen)
{
    char ebuf[128];
...
static int _asn_bitstring_check (const char *str, size_t asn_length, u_char datum)
{
    char ebuf[128];
...
u_char *asn_parse_unsigned_int (u_char * data, size_t * datalength, u_char * type, u_long * intp, size_t intsize)
{
    ...
    *type = *bufp++;
    if (*type != ASN_COUNTER && *type != ASN_GAUGE && *type != ASN_TIMETICKS && *type != ASN_UINTEGER)
    {
        _asn_type_err (errpre, *type);
        return NULL;
    }
...
u_char *asn_parse_string (u_char * data, size_t * datalength, u_char * type, u_char * str, size_t * strlength)
{
    ...
    *type = *bufp++;
    if (*type != ASN_OCTET_STR && *type != ASN_IPADDRESS && *type != ASN_OPAQUE && *type != ASN_NSAP)
    {
        _asn_type_err (errpre, *type);
        return NULL;
    }
...
u_char *asn_parse_header (u_char * data, size_t * datalength, u_char * type)
{
    ...
    *type = *bufp;
    ...
    if ((*type == ASN_OPAQUE) && (*bufp == ASN_OPAQUE_TAG1))
    {
        ...
        if (_asn_parse_length_check ("parse opaque header", bufp, data, asn_length, *datalength))
            return NULL;
    }
 ...
 *
 */
u_char *asn_parse_sequence (u_char * data, size_t * datalength, u_char * type, u_char expected_type,    /* must be this type */
                            const char *estr)
{                                /* error message prefix */
    ...
    if (data && (*type != expected_type))
    {
        ...
        snprintf (ebuf, sizeof (ebuf), "%s header type %02X: s/b %02X", estr, (u_char) * type, (u_char) expected_type);
        ebuf[sizeof (ebuf) - 1] = 0;
        ERROR_MSG (ebuf);
        return NULL;
    }
...
u_char *asn_build_header (u_char * data, size_t * datalength, u_char type, size_t length)
{
    ...
    {
        snprintf (ebuf, sizeof (ebuf),
                  "bad header length < 1 :%lu, %lu", (unsigned long) *datalength, (unsigned long) length);
        ebuf[sizeof (ebuf) - 1] = 0;
        ERROR_MSG (ebuf);
        return NULL;
    }
...
u_char *asn_parse_length (u_char * data, u_long * length)
{
    ...
    if (lengthbyte & ASN_LONG_LEN)
    {
        ...
        {
            snprintf (ebuf, sizeof (ebuf),
                      "%s: data length %d > %lu not supported", errpre, lengthbyte, (unsigned long) sizeof (long));
            ebuf[sizeof (ebuf) - 1] = 0;
            ERROR_MSG (ebuf);
            return NULL;
        }
...
u_char *asn_build_length (u_char * data, size_t * datalength, size_t length)
{
    ...
    if (length < 0x80)
    {
        ...
        {
            snprintf (ebuf, sizeof (ebuf),
                      "%s: bad length < 1 :%lu, %lu", errpre, (unsigned long) *datalength, (unsigned long) length);
            ebuf[sizeof (ebuf) - 1] = 0;
            ERROR_MSG (ebuf);
            return NULL;
        }
    ...
    else if (length <= 0xFF)
    {
        ...
        {
            snprintf (ebuf, sizeof (ebuf),
                      "%s: bad length < 2 :%lu, %lu", errpre, (unsigned long) *datalength, (unsigned long) length);
            ebuf[sizeof (ebuf) - 1] = 0;
            ERROR_MSG (ebuf);
            return NULL;
        }
    ...
    {                            /* 0xFF < length <= 0xFFFF */
        ...
        {
            snprintf (ebuf, sizeof (ebuf),
                      "%s: bad length < 3 :%lu, %lu", errpre, (unsigned long) *datalength, (unsigned long) length);
            ebuf[sizeof (ebuf) - 1] = 0;
            ERROR_MSG (ebuf);
            return NULL;
        }
...
u_char *asn_parse_objid (u_char * data, size_t * datalength, u_char * type, oid * objid, size_t * objidlength)
{
    ...
    *type = *bufp++;
    ...
    bufp = asn_parse_length (bufp, &asn_length);
    if (_asn_parse_length_check ("parse objid", bufp, data, asn_length, *datalength))
        return NULL;
...
u_char *asn_build_objid (u_char * data, size_t * datalength, u_char type, oid * objid, size_t objidlength)
{
     ...
     */
    for (i = 1, objid_val = first_objid_val, op = objid + 2; i < (int) objidlength; i++)
    {
        if (i != 1)
...
u_char *asn_parse_unsigned_int64 (u_char * data,
                                  size_t * datalength, u_char * type, struct counter64 * cp, size_t countersize)
{
    /*
     * ASN.1 integer ::= 0x02 asnlength byte {byte}*
     */
    ...
    *type = *bufp++;
    ...
    if ((*type == ASN_OPAQUE) &&
        ...
        (*bufp == ASN_OPAQUE_TAG1) && ((*(bufp + 1) == ASN_OPAQUE_COUNTER64) || (*(bufp + 1) == ASN_OPAQUE_U64)))
    {
        /*
        ...
        *type = *(bufp + 1);
         ...
         */
        bufp = asn_parse_length (bufp + 2, &asn_length);
        if (_asn_parse_length_check ("parse opaque uint64", bufp, data, asn_length, *datalength))
            return NULL;
    }
#endif                            /* NETSNMP_WITH_OPAQUE_SPECIAL_TYPES */
 ...
 *          Returns NULL on any error.
 */
u_char *asn_build_unsigned_int64 (u_char * data,
                                  size_t * datalength, u_char type, const struct counter64 * cp, size_t countersize)
{
    /*
     * ASN.1 integer ::= 0x02 asnlength byte {byte}*
     */
    ...
    *type = *bufp++;
        ...
        (*bufp == ASN_OPAQUE_TAG1) && (*(bufp + 1) == ASN_OPAQUE_I64))
    {
        ...
        *type = *(bufp + 1);
         ...
         */
        bufp = asn_parse_length (bufp + 2, &asn_length);
        if (_asn_parse_length_check ("parse opaque int64", bufp, data, asn_length, *datalength))
            return NULL;
    }
    /*
 ...
 *          Returns NULL on any error.
 */
u_char *asn_build_signed_int64 (u_char * data,
                                size_t * datalength, u_char type, const struct counter64 * cp, size_t countersize)
{
    /*
     * ASN.1 integer ::= 0x02 asnlength byte {byte}*
...
u_char *asn_parse_float (u_char * data, size_t * datalength, u_char * type, float *floatp, size_t floatsize)
{
    ...
    *type = *bufp++;
    bufp = asn_parse_length (bufp, &asn_length);
    if (_asn_parse_length_check ("parse float", bufp, data, asn_length, *datalength))
        return NULL;
     ...
     */
    if ((*type == ASN_OPAQUE) &&
        (asn_length == ASN_OPAQUE_FLOAT_BER_LEN) && (*bufp == ASN_OPAQUE_TAG1) && (*(bufp + 1) == ASN_OPAQUE_FLOAT))
    {
         ...
         */
        bufp = asn_parse_length (bufp + 2, &asn_length);
        if (_asn_parse_length_check ("parse opaque float", bufp, data, asn_length, *datalength))
            return NULL;
 ...
 */
u_char *asn_build_float (u_char * data, size_t * datalength, u_char type, const float *floatp, size_t floatsize)
{
    union
    {
        float floatVal;
...
u_char *asn_parse_double (u_char * data, size_t * datalength, u_char * type, double *doublep, size_t doublesize)
{
    ...
    *type = *bufp++;
    bufp = asn_parse_length (bufp, &asn_length);
    if (_asn_parse_length_check ("parse double", bufp, data, asn_length, *datalength))
        return NULL;
     ...
     */
    if ((*type == ASN_OPAQUE) &&
        (asn_length == ASN_OPAQUE_DOUBLE_BER_LEN) && (*bufp == ASN_OPAQUE_TAG1) && (*(bufp + 1) == ASN_OPAQUE_DOUBLE))
    {
         ...
         */
        bufp = asn_parse_length (bufp + 2, &asn_length);
        if (_asn_parse_length_check ("parse opaque double", bufp, data, asn_length, *datalength))
            return NULL;
 ...
 *          Returns NULL on any error.
 */
u_char *asn_build_double (u_char * data, size_t * datalength, u_char type, const double *doublep, size_t doublesize)
{
    long tmp;
    ...
    if (pkt != NULL && pkt_len != NULL)
    {
        ...
        if (snmp_realloc (pkt, pkt_len))
        {
            ...
            DEBUGMSGTL (("asn_realloc",
                         ...
                         *pkt, (unsigned) (*pkt_len - old_pkt_len), *pkt, (unsigned) old_pkt_len));
 ...
 */
int asn_realloc_rbuild_length (u_char ** pkt, size_t * pkt_len, size_t * offset, int r, size_t length)
{
    static const char *errpre = "build length";
    ...
    if (length <= 0x7f)
    {
        ...
        {
            snprintf (ebuf, sizeof (ebuf),
                      "%s: bad length < 1 :%ld, %lu", errpre, (long) (*pkt_len - *offset), (unsigned long) length);
            ebuf[sizeof (ebuf) - 1] = 0;
            ERROR_MSG (ebuf);
            return 0;
        }
 ...
 * @return 1 on success, 0 on error
 */
int asn_realloc_rbuild_header (u_char ** pkt, size_t * pkt_len, size_t * offset, int r, u_char type, size_t length)
{
    char ebuf[128];
    ...
    if (asn_realloc_rbuild_length (pkt, pkt_len, offset, r, length))
    {
        ...
        {
            snprintf (ebuf, sizeof (ebuf),
                      "bad header length < 1 :%ld, %lu", (long) (*pkt_len - *offset), (unsigned long) length);
            ebuf[sizeof (ebuf) - 1] = 0;
            ERROR_MSG (ebuf);
            return 0;
        }
...
asn_realloc_rbuild_int (u_char ** pkt, size_t * pkt_len,
                        size_t * offset, int r, u_char type, const long *intp, size_t intsize)
{
    static const char *errpre = "build int";
    ...
    *(*pkt + *pkt_len - (++*offset)) = (u_char) integer;
    ...
    {
        if (_asn_realloc_build_header_check (errpre, pkt, pkt_len, (*offset - start_offset)))
        {
            return 0;
        }
        else
...
asn_realloc_rbuild_string (u_char ** pkt, size_t * pkt_len,
                           size_t * offset, int r, u_char type, const u_char * str, size_t strlength)
{
...
asn_realloc_rbuild_unsigned_int (u_char ** pkt, size_t * pkt_len,
                                 size_t * offset, int r, u_char type, const u_long * intp, size_t intsize)
{
    static const char *errpre = "build uint";
...
int asn_realloc_rbuild_sequence (u_char ** pkt, size_t * pkt_len, size_t * offset, int r, u_char type, size_t length)
{
...
asn_realloc_rbuild_objid (u_char ** pkt, size_t * pkt_len,
                          size_t * offset, int r, u_char type, const oid * objid, size_t objidlength)
{
    /*
     * ASN.1 objid ::= 0x06 asnlength subidentifier {subidentifier}*
     * subidentifier ::= {leadingbyte}* lastbyte
...
int asn_realloc_rbuild_null (u_char ** pkt, size_t * pkt_len, size_t * offset, int r, u_char type)
{
    /*
     * ASN.1 null ::= 0x05 0x00
     */
...
asn_realloc_rbuild_unsigned_int64 (u_char ** pkt, size_t * pkt_len,
                                   size_t * offset, int r, u_char type, const struct counter64 *cp, size_t countersize)
{
    /*
     * ASN.1 integer ::= 0x02 asnlength byte {byte}*
     */
    ...
    *(*pkt + *pkt_len - (++*offset)) = (u_char) low;
    ...
    if (type == ASN_OPAQUE_COUNTER64)
    {
        ...
        *(*pkt + *pkt_len - (++*offset)) = ASN_OPAQUE_TAG1;
        ...
        {
            if (_asn_realloc_build_header_check ("build counter u64", pkt, pkt_len, intsize + 3))
            {
                return 0;
            }
        }
    ...
    else
    {
        ...
        {
            if (_asn_realloc_build_header_check ("build uint64", pkt, pkt_len, intsize))
            {
                return 0;
            }
        }
...
asn_realloc_rbuild_float (u_char ** pkt, size_t * pkt_len,
                          size_t * offset, int r, u_char type, const float *floatp, size_t floatsize)
{
    size_t start_offset = *offset;
...
asn_realloc_rbuild_double (u_char ** pkt, size_t * pkt_len,
                           size_t * offset, int r, u_char type, const double *doublep, size_t doublesize)
{
    size_t start_offset = *offset;
