...
netsnmp_feature_require (snmpv3_probe_contextEngineID_rfc5343) netsnmp_feature_require (row_create)
#include <unistd.h>
     ...
     void init_tsm (void)
{
    ...
    if (!def)
    {
        snmp_log (LOG_ERR, "Unable to malloc snmp_secmod struct, not registering TSM\n");
        return;
    }
    ...
    netsnmp_ds_register_config (ASN_BOOLEAN, "snmp", "tsmUseTransportPrefix",
                                NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_TSM_USE_PREFIX);
}
...
static int tsm_session_init (netsnmp_session * sess)
{
    DEBUGMSGTL (("tsm", "TSM: Reached our session initialization callback\n"));
...
static void tsm_free_state_ref (void *ptr)
{
...
static int tsm_clone_pdu (netsnmp_pdu * pdu, netsnmp_pdu * pdu2)
{
    ...
    memdup ((u_char **) & newref->tmStateRef, oldref->tmStateRef, sizeof (*oldref->tmStateRef));
    return SNMPERR_SUCCESS;
}
...
int tsm_rgenerate_out_msg (struct snmp_secmod_outgoing_params *parms)
{
    ...
    else
    {
        ...
        if (netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_TSM_USE_PREFIX))
        {
            ...
            if (strchr (parms->secName, ':') == 0 ||
                ...
                strncmp (parms->secName, prefix, strlen (prefix)) != 0 || parms->secName[strlen (prefix)] != ':')
            {
            ...
            memcpy (tmStateRef->securityName,
                    parms->secName + strlen (prefix) + 1, parms->secNameLen - strlen (prefix) - 1);
        ...
        else
        {
            /* 4.2, step 2: If the snmpTsmConfigurationUsePrefix object is
               ...
               of securityName. */
            memcpy (tmStateRef->securityName, parms->secName, parms->secNameLen);
    ...
    *offset += parms->globalDataLen;
    memcpy (*wholeMsg + *wholeMsgLen - *offset, parms->globalData, parms->globalDataLen);
    ...
    if (parms->pdu->transport_data && parms->pdu->transport_data != tmStateRef)
    {
    ...
    if (SNMPERR_SUCCESS != memdup ((u_char **) & parms->pdu->transport_data, tmStateRef, sizeof (*tmStateRef)))
    {
...
int tsm_process_in_msg (struct snmp_secmod_incoming_params *parms)
{
    u_char type_value;
    ...
    netsnmp_assert_or_return (ourEngineID_len != 0 && ourEngineID_len <= *parms->secEngineIDLen, SNMPERR_GENERR);
    ...
    /* Section 5.2, step 2: If tmStateReference does not refer to a
       ...
       message. */
    if (!parms->pdu->transport_data || sizeof (netsnmp_tmStateReference) != parms->pdu->transport_data_length)
    {
    ...
    if (netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_TSM_USE_PREFIX))
    {
        ...
        if (netsnmp_oid_equals (netsnmp_snmpSSHDomain,
                                ...
                                tmStateRef->transportDomain, tmStateRef->transportDomainLen) == 0)
        {
            prefix = "ssh";
        }
         ...
         */
        snprintf (parms->secName, *parms->secNameLen, "%s:%s", prefix, tmStateRef->securityName);
    }
    else
    {
    ...
    *parms->secStateRef = tsmSecRef;
    ...
    if ((data_ptr = asn_parse_sequence (parms->secParams, &remaining,
                                        &type_value,
                                        (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), "tsm first octet")) == NULL)
    {
        /*
         * RETURN parse error 
         */
