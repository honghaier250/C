     ...
     int init_snmp_enum (const char *type)
{
    ...
    snmp_enum_lists = (struct snmp_enum_list ***) calloc (1, sizeof (struct snmp_enum_list **) * SE_MAX_IDS);
    if (!snmp_enum_lists)
        return SE_NOMEM;
...
int se_store_in_list (struct snmp_enum_list *new_list, unsigned int major, unsigned int minor)
{
    int ret = SE_OK;
...
int se_add_pair_to_slist (const char *listname, char *label, int value)
{
    ...
    if (!created)
    {
        struct snmp_enum_list_str *sptr = SNMP_MALLOC_STRUCT (snmp_enum_list_str);
