     ...
     static void *_iterator_get (iterator_info * ii, const void *key)
{
    ...
    {
        for (; (NULL != tmp.val) && (SNMP_ERR_NOERROR == rc); rc = ii->get_next (ii->user_ctx, &loop_ctx, &tmp))
        {
...
static void *_iterator_get_next (iterator_info * ii, const void *key)
{
    ...
    if (SNMP_ERR_NOERROR == rc)
    {
         ...
         */
        for (; (NULL != tmp.val) && (rc == SNMP_ERR_NOERROR); rc = ii->get_next (ii->user_ctx, &loop_ctx, &tmp))
        {
            ...
            {                    /* get first */
                ...
                {
                    snmp_log (LOG_ERR, "illegal reuse of data context in " "container_iterator\n");
                    rc = SNMP_ERR_GENERR;
                    break;
                }
            ...
            if (cmp > 0)
            {
                 ...
                 * current best match.
                 */
                if ((NULL == key) || (NULL == best_val.val) || ((cmp = ii->c.compare (tmp.val, best_val.val)) < 0))
                {
     ...
     */
    if ((best_ctx.val != NULL) && (best_ctx.val != loop_ctx.val) && (ii->cleanup_loop_ctx))
...
static size_t _iterator_size (iterator_info * ii)
{
    ...
    for (ii->get_first (ii->user_ctx, &loop_ctx, &tmp); NULL != tmp.val; ii->get_next (ii->user_ctx, &loop_ctx, &tmp))
        ++count;
...
static void _iterator_for_each (iterator_info * ii, netsnmp_container_obj_func * f, void *ctx)
{
    netsnmp_ref_void loop_ctx = { NULL };
    netsnmp_ref_void tmp = { NULL };
...
static void _iterator_clear (netsnmp_container * container, netsnmp_container_obj_func * f, void *context)
{
...
netsnmp_container_iterator_set_data_cb (netsnmp_container * c,
                                        Netsnmp_Iterator_Data * insert_data,
                                        Netsnmp_Iterator_Data * remove_data, Netsnmp_Iterator_Op * get_size)
{
