...
netsnmp_feature_child_of (read_config_register_const_config_handler, netsnmp_unused)
     ...
     static struct config_line *internal_register_config_handler (const char *type_param,
                                                                  const char *token,
                                                                  void (*parser) (const char *, char *),
                                                                  void (*releaser) (void), const char *help, int when)
{
    ...
    if (strchr (type, ':'))
    {
        ...
        while (cptr)
        {
            ...
            ltmp2 = internal_register_config_handler (c, token, parser, releaser, help, when);
        }
        return ltmp2;
    }
...
struct config_line *register_prenetsnmp_mib_handler (const char *type,
                                                     const char *token,
                                                     void (*parser) (const char *, char *),
                                                     void (*releaser) (void), const char *help)
{
    return internal_register_config_handler (type, token, parser, releaser, help, PREMIB_CONFIG);
}

#ifndef NETSNMP_FEATURE_REMOVE_READ_CONFIG_REGISTER_APP_PRENETSNMP_MIB_HANDLER
struct config_line *register_app_prenetsnmp_mib_handler (const char *token,
                                                         void (*parser) (const char *, char *),
                                                         void (*releaser) (void), const char *help)
{
...
struct config_line *register_const_config_handler (const char *type,
                                                   const char *token,
                                                   void (*parser) (const char *, const char *),
                                                   void (*releaser) (void), const char *help)
{
    return internal_register_config_handler (type, token,
                                             (void (*)(const char *, char *)) parser, releaser, help, NORMAL_CONFIG);
}
#endif                            /* NETSNMP_FEATURE_REMOVE_READ_CONFIG_REGISTER_CONST_CONFIG_HANDLER */

struct config_line *register_app_config_handler (const char *token,
                                                 void (*parser) (const char *, char *),
                                                 void (*releaser) (void), const char *help)
{
...
int run_config_handler (struct config_line *lptr, const char *token, char *cptr, int when)
{
    ...
    if (lptr != NULL)
    {
        ...
        {
            DEBUGMSGTL (("read_config:parser", "Found a parser.  Calling it: %s / %s\n", token, cptr));
            /*
            ...
            DEBUGMSGTL (("9:read_config:parser", "%s handler not registered for this time\n", token));
    }
    else if (when != PREMIB_CONFIG && !netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_NO_TOKEN_WARNINGS))
    {
...
int snmp_config_when (char *line, int when)
{
    ...
    if (cptr[0] == '[')
    {
        ...
        if (lptr == NULL)
        {
            netsnmp_config_error ("No handlers regestered for type %s.", cptr + 1);
            return SNMPERR_GENERR;
        }
    ...
    if (lptr == NULL && netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_NO_TOKEN_WARNINGS))
    {
...
int netsnmp_config (char *line)
{
    ...
    netsnmp_config_remember (line);    /* always remember it so it's read
                                     ...
                                     * call */
    if (netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_HAVE_READ_CONFIG))
    {
    ...
    while (ifile)
    {
        ...
        if ((cptr = skip_white (line)))
        {
            ...
            if (token[0] == '[')
            {
                ...
                {
                    netsnmp_config_error ("no matching ']' for type %s.", &token[1]);
                    continue;
                }
                ...
                if (lptr == NULL)
                {
                    netsnmp_config_error ("No handlers regestered for type %s.", &token[1]);
                    continue;
                }
            ...
            else if ((token[0] == 'i') && (strncasecmp (token, "include", 7) == 0))
            {
                if (strcasecmp (token, "include") == 0)
                {
                    if (when != PREMIB_CONFIG &&
                        !netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_NO_TOKEN_WARNINGS))
                    {
                        netsnmp_config_warn ("Ambiguous token '%s' - use 'includeSearch' (or 'includeFile') instead.",
                                             token);
                    }
                    continue;
                }
                else if (strcasecmp (token, "includedir") == 0)
                {
                    ...
                    while ((entry = readdir (d)) != NULL)
                    {
                        if (entry->d_name && entry->d_name[0] != '.')
                        {
                            ...
                            {
                                snprintf (fname, SNMP_MAXPATH, "%s/%s", cptr, entry->d_name);
                ...
                else if (strcasecmp (token, "includefile") == 0)
                {
                    ...
                    if (read_config (fname, line_handler, when) != SNMPERR_SUCCESS && when != PREMIB_CONFIG)
...
int read_configs_optional (const char *optional_config, int when)
{
    ...
    DEBUGMSGTL (("read_configs_optional", "reading optional configuration tokens for %s\n", type));
    ...
    while (cp)
    {
        ...
        {
            DEBUGMSGTL (("read_config:opt", "Reading optional config file: \"%s\"\n", cp));
...
void read_configs (void)
{
    ...
    snmp_call_callbacks (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_PRE_READ_CONFIG, NULL);
    ...
    netsnmp_ds_set_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_HAVE_READ_CONFIG, 1);
...
void read_premib_configs (void)
{
    ...
    snmp_call_callbacks (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_PRE_PREMIB_READ_CONFIG, NULL);
    ...
    netsnmp_ds_set_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_HAVE_READ_PREMIB_CONFIG, 1);
...
{
    netsnmp_ds_set_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_CONFIGURATION_DIR, dir);
}
...
const char *get_configuration_directory (void)
{
    ...
    if (NULL == netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_CONFIGURATION_DIR))
    {
        ...
        snprintf (defaultPath, sizeof (defaultPath), "%s%c%s%c%s%s%s%s",
                  ...
                  ((homepath == NULL) ? "" : homepath), ((homepath == NULL) ? "" : "/.snmp"));
        defaultPath[sizeof (defaultPath) - 1] = 0;
        set_configuration_directory (defaultPath);
    }
    return (netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_CONFIGURATION_DIR));
}
...
{
    netsnmp_ds_set_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_PERSISTENT_DIR, dir);
}
...
{
    if (NULL == netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_PERSISTENT_DIR))
    {
    ...
    return (netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_PERSISTENT_DIR));
}
...
{
    netsnmp_ds_set_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_TEMP_FILE_PATTERN, pattern);
}
...
{
    if (NULL == netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_TEMP_FILE_PATTERN))
    {
        set_temp_file_pattern (NETSNMP_TEMP_FILE_PATTERN);
    }
    return (netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_TEMP_FILE_PATTERN));
}
                           ...
                           int when, const char *perspath, const char *persfile)
{
    ...
    while ((!done) && (*cptr2 != 0))
    {
        ...
        if (strncmp (cptr2, perspath, strlen (perspath)) == 0 ||
            (persfile != NULL && strncmp (cptr2, persfile, strlen (persfile)) == 0))
        {
            ...
            {
                snprintf (configfile, sizeof (configfile), "%s/%s.%d.conf", cptr2, ctmp->fileHeader, j);
        ...
        snprintf (configfile, sizeof (configfile), "%s/%s.conf", cptr2, ctmp->fileHeader);
        configfile[sizeof (configfile) - 1] = 0;
        if (read_config (configfile, ctmp->start, when) == SNMPERR_SUCCESS)
            ret = SNMPERR_SUCCESS;
        snprintf (configfile, sizeof (configfile), "%s/%s.local.conf", cptr2, ctmp->fileHeader);
        configfile[sizeof (configfile) - 1] = 0;
        if (read_config (configfile, ctmp->start, when) == SNMPERR_SUCCESS)
            ret = SNMPERR_SUCCESS;
...
int read_config_files_of_type (int when, struct config_files *ctmp)
{
    ...
    if (netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID,
        ...
        || netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DISABLE_CONFIG_LOAD) || (NULL == ctmp))
        return ret;
    ...
    if (envconfpath == NULL)
    {
         ...
         */
        if (read_config_files_in_path (confpath, ctmp, when, perspath, persfile) == SNMPERR_SUCCESS)
            ret = SNMPERR_SUCCESS;
        free (perspath);
        perspath = strdup (get_persistent_directory ());
        if (read_config_files_in_path (perspath, ctmp, when, perspath, persfile) == SNMPERR_SUCCESS)
            ret = SNMPERR_SUCCESS;
    }
    else
    {
         ...
         */
        if (read_config_files_in_path (envconfpath, ctmp, when, perspath, persfile) == SNMPERR_SUCCESS)
            ret = SNMPERR_SUCCESS;
    }
    free (perspath);
    return ret;
...
void read_config_print_usage (const char *lead)
{
    ...
    for (ctmp = config_files; ctmp != NULL; ctmp = ctmp->next)
    {
        ...
        for (ltmp = ctmp->start; ltmp != NULL; ltmp = ltmp->next)
        {
            ...
            if (ltmp->help)
            {
                snmp_log (LOG_INFO, "%s%s%-24s %s\n", lead, lead, ltmp->config_token, ltmp->help);
            }
            else
            {
                DEBUGIF ("read_config_usage")
...
void read_config_store (const char *type, const char *line)
{
    ...
    if (netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID,
                                NETSNMP_DS_LIB_DONT_PERSIST_STATE)
        || netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DISABLE_PERSISTENT_LOAD))
        return;
    ...
    {
        snprintf (file, sizeof (file), "%s/%s.conf", get_persistent_directory (), type);
...
{
    read_config_store (netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_APPTYPE), line);
}
...
void snmp_save_persistent (const char *type)
{
    ...
    if (netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID,
                                NETSNMP_DS_LIB_DONT_PERSIST_STATE)
        || netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DISABLE_PERSISTENT_SAVE))
        return;
    ...
    if (stat (file, &statbuf) == 0)
    {
        for (j = 0; j <= NETSNMP_MAX_PERSISTENT_BACKUPS; j++)
        {
            snprintf (fileold, sizeof (fileold), "%s/%s.%d.conf", get_persistent_directory (), type, j);
...
void snmp_clean_persistent (const char *type)
{
    ...
    if (stat (file, &statbuf) == 0)
    {
        for (j = 0; j <= NETSNMP_MAX_PERSISTENT_BACKUPS; j++)
        {
            snprintf (file, sizeof (file), "%s/%s.%d.conf", get_persistent_directory (), type, j);
            file[sizeof (file) - 1] = 0;
            if (stat (file, &statbuf) == 0)
            {
 ...
 */
static void config_vlog (int level, const char *levelmsg, const char *str, va_list args)
{
    char tmpbuf[256];
...
char *read_config_save_octet_string (char *saveto, u_char * str, size_t len)
{
     ...
     */
    for (i = 0, cp = str; i < (int) len && cp && (isalpha (*cp) || isdigit (*cp) || *cp == ' '); cp++, i++);
...
{
    return NETSNMP_REMOVE_CONST (char *, read_config_read_octet_string_const (readfrom, str, len));
}
...
{
    return NETSNMP_REMOVE_CONST (char *, read_config_read_objid_const (readfrom, objid, len));
}
...
{
    return read_config_store_data_prefix (' ', type, storeto, dataptr, (len ? *len : 0));
}
...
char *read_config_store_data_prefix (char prefix, int type, char *storeto, void *dataptr, size_t len)
{
    ...
    if (dataptr && storeto)
        switch (type)
        {
            ...
            default:
                DEBUGMSGTL (("read_config_store_data_prefix", "Fail: Unknown type: %d", type));
                return NULL;
        }
    return NULL;
}
