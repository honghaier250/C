...
netsnmp_feature_child_of (usm_support, libnetsnmp) netsnmp_feature_child_of (usm_lcd_time, usm_support)
 ...
 */
     int get_enginetime (const u_char * engineID,
                         u_int engineID_len, u_int * engineboot, u_int * engine_time, u_int authenticated)
{
...
get_enginetime_ex (u_char * engineID,
                   u_int engineID_len,
                   u_int * engineboot, u_int * engine_time, u_int * last_engine_time, u_int authenticated)
{
    int rval = SNMPERR_SUCCESS;
 ...
 */
int
set_enginetime (const u_char * engineID, u_int engineID_len, u_int engineboot, u_int engine_time, u_int authenticated)
{
...
Enginetime search_enginetime_list (const u_char * engineID, u_int engineID_len)
{
    ...
    {
        if ((engineID_len == e->engineID_len) && !memcmp (e->engineID, engineID, engineID_len))
        {
            break;
        }
    }
...
void dump_etimelist_entry (Enginetime e, int count)
{
    ...
    DEBUGMSGTL (("dump_etimelist", "%s%s (len=%d) <%d,%d>\n", tabs, s, e->engineID_len, e->engineTime, e->engineBoot));
    DEBUGMSGTL (("dump_etimelist", "%s%ld (%ld)", tabs,
                 e->lastReceivedEngineTime, snmpv3_local_snmpEngineTime () - e->lastReceivedEngineTime));
