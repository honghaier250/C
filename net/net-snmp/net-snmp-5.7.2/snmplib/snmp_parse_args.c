...
void snmp_parse_args_descriptions (FILE * outf)
{
    ...
    fprintf (outf, "  -L LOGOPTS\t\tToggle various defaults controlling logging:\n");
...
netsnmp_parse_args (int argc,
                    char **argv,
                    netsnmp_session * session, const char *localOpts, void (*proc) (int, char *const *, int), int flags)
{
...
#define SNMPV3_CMD_OPTIONS
            ...
            case 'l':
                    ...
                    || !strcasecmp (optarg, "noauth") || !strcasecmp (optarg, "nanp"))
                {
                    session->securityLevel = SNMP_SEC_LEVEL_NOAUTH;
                         ...
                         || !strcmp (optarg, "2") || !strcasecmp (optarg, "auth") || !strcasecmp (optarg, "anp"))
                {
                    session->securityLevel = SNMP_SEC_LEVEL_AUTHNOPRIV;
                }
                else if (!strcasecmp (optarg, "authPriv")
                         || !strcmp (optarg, "3") || !strcasecmp (optarg, "priv") || !strcasecmp (optarg, "ap"))
                {
...
#endif
    {
        if (Cpsz == NULL)
        {
            Cpsz = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_COMMUNITY);
            if (Cpsz == NULL)
            {
                if (netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_IGNORE_NO_COMMUNITY))
                {
                    DEBUGMSGTL (("snmp_parse_args", "ignoring that the community string is not present\n"));
...
snmp_parse_args (int argc,
                 char **argv, netsnmp_session * session, const char *localOpts, void (*proc) (int, char *const *, int))
{
