...
netsnmp_feature_child_of (container_directory, container_types)
    ...
    netsnmp_feature_require (file_utils) netsnmp_feature_require (container_free_all)
 ...
 */
     netsnmp_container *netsnmp_directory_container_read_some (netsnmp_container * user_container,
                                                               const char *dirname,
                                                               ...
                                                               void *filter_ctx, u_int flags)
{
    ...
    {
        DEBUGMSGTL (("directory:container", "no support for relative path with recursion\n"));
        return NULL;
    }
    ...
    if (NULL == container)
    {
        ...
        {
            container = netsnmp_container_find ("nsfile_directory_container:" "binary_array");
            if (container)
            {
                container->compare = (netsnmp_container_compare *) netsnmp_file_compare_name;
                container->free_item = (netsnmp_container_obj_func *) netsnmp_file_container_free;
            }
        }
        else
    ...
    while ((file = readdir (dir)))
    {
        ...
        {
            if (_insert_nsfile (container, file->d_name, filter ? &statbuf : NULL, flags) < 0)
                break;
        }
        else
        {
            ...
            if (NULL == dup)
            {
                snmp_log (LOG_ERR, "strdup failed while building directory container\n");
                break;
            }
