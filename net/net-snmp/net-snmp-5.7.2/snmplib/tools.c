...
netsnmp_feature_child_of (memory_realloc, memory_wrappers) netsnmp_feature_child_of (memory_free, memory_wrappers)
 ...
 */
u_int netsnmp_binary_to_hex (u_char ** dest, size_t * dest_len, int allow_realloc, const u_char * input, size_t len)
{
    u_int olen = (len * 2) + 1;
...
#define eb(b)    ( *(esp+b) & 0xff )
    ...
    s += sprintf (s, "enterprise %d, ", ((*(esp + 0) & 0x7f) << 24) |
                  ((*(esp + 1) & 0xff) << 16) | ((*(esp + 2) & 0xff) << 8) | ((*(esp + 3) & 0xff)));
    /*
     * XXX  Ick. 
     */
    ...
    switch ((int) *esp++)
    {
        ...
        case 3:                /* MAC address. */
            ...
            s += sprintf (s, "%02X:%02X:%02X:%02X:%02X:%02X", eb (0), eb (1), eb (2), eb (3), eb (4), eb (5));
...
char *netsnmp_getenv (const char *name)
{
    ...
    if (temp == NULL)
    {
        ...
        if (RegOpenKeyExA (HKEY_CURRENT_USER, "SOFTWARE\\Net-SNMP", 0, KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS)
        {

            if (RegQueryValueExA (hKey, name, NULL, &key_value_type, NULL,    /* Just get the size */
                                  &key_value_size) == ERROR_SUCCESS)
            {
                ...
                if (RegQueryValueExA (hKey, name, NULL, &key_value_type, key_value, &key_value_size) == ERROR_SUCCESS)
                {
                }
                temp = (char *) key_value;
            }
    ...
    if (temp == NULL)
    {
        if (RegOpenKeyExA (HKEY_LOCAL_MACHINE, "SOFTWARE\\Net-SNMP", 0, KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS)
        {
