...
void vacm_save (const char *token, const char *type)
{
    ...
    for (aptr = accessList; aptr != NULL; aptr = aptr->next)
    {
        if (aptr->storageType == ST_NONVOLATILE)
        {
            ...
            if (aptr->views[VACM_VIEW_READ][0] || aptr->views[VACM_VIEW_WRITE][0] || aptr->views[VACM_VIEW_NOTIFY][0])
...
void vacm_save_view (struct vacm_viewEntry *view, const char *token, const char *type)
{
    ...
    cptr = read_config_save_octet_string (cptr, (u_char *) view->viewName + 1, view->viewName[0]);
    *cptr++ = ' ';
    cptr = read_config_save_objid (cptr, view->viewSubtree + 1, view->viewSubtreeLen - 1);
    *cptr++ = ' ';
    cptr = read_config_save_octet_string (cptr, (u_char *) view->viewMask, view->viewMaskLen);
...
void vacm_parse_config_view (const char *token, const char *line)
{
    ...
    line = read_config_read_objid_const (line, (oid **) & viewSubtree, &view.viewSubtreeLen);
 ...
 */
void vacm_save_access (struct vacm_accessEntry *access_entry, const char *token, const char *type)
{
    char line[4096];
    ...
    line[sizeof (line) - 1] = 0;
    cptr = &line[strlen (line)];    /* the NULL */
    cptr = read_config_save_octet_string (cptr, (u_char *) access_entry->groupName + 1, access_entry->groupName[0] + 1);
    *cptr++ = ' ';
    cptr =
        read_config_save_octet_string (cptr,
                                       (u_char *) access_entry->contextPrefix + 1, access_entry->contextPrefix[0] + 1);
...
void vacm_save_auth_access (struct vacm_accessEntry *access_entry, const char *token, const char *type, int authtype)
{
    char line[4096];
...
char *_vacm_parse_config_access_common (struct vacm_accessEntry **aptr, const char *line)
{
    ...
    *aptr = vacm_getAccessEntry (access.groupName, access.contextPrefix, access.securityModel, access.securityLevel);
    if (!*aptr)
        *aptr = vacm_createAccessEntry (access.groupName,
                                        access.contextPrefix, access.securityModel, access.securityLevel);
    if (!*aptr)
        return NULL;
...
struct vacm_viewEntry *netsnmp_view_get (struct vacm_viewEntry *head, const char *viewName,
                                         oid * viewSubtree, size_t viewSubtreeLen, int mode)
{
    ...
    {
        if (!memcmp (view, vp->viewName, glen + 1) && viewSubtreeLen >= (vp->viewSubtreeLen - 1))
        {
            int mask = 0x80;
                    ...
                    vpret = vp;
                         ...
                         || (vp->viewSubtreeLen == vpret->viewSubtreeLen
                             && snmp_oid_compare (vp->viewSubtree + 1,
                                                  vp->viewSubtreeLen - 1,
                                                  vpret->viewSubtree + 1, vpret->viewSubtreeLen - 1) > 0))
                {
                    vpret = vp;
                }
            }
 ...
 */
int
netsnmp_view_subtree_check (struct vacm_viewEntry *head, const char *viewName, oid * viewSubtree, size_t viewSubtreeLen)
{
        ...
        if (!memcmp (view, vp->viewName, glen + 1))
        {
                ...
                if (found)
                {
                    ...
                    if (vpShorter == NULL
                        ...
                        || (vp->viewSubtreeLen == vpShorter->viewSubtreeLen
                            && snmp_oid_compare (vp->viewSubtree + 1,
                                                 vp->viewSubtreeLen - 1,
                                                 vpShorter->viewSubtree + 1, vpShorter->viewSubtreeLen - 1) > 0))
                    {
                        vpShorter = vp;
                    }
                }
            ...
            else
            {
                ...
                if (found)
                {
                     ...
                     */
                    DEBUGMSGTL (("9:vacm:checkSubtree", " %s matched?\n", vp->viewName));
                    if (vpLonger != NULL && (vpLonger->viewType != vp->viewType))
                    {
...
struct vacm_viewEntry *netsnmp_view_create (struct vacm_viewEntry **head, const char *viewName,
                                            oid * viewSubtree, size_t viewSubtreeLen)
{
    ...
    while (lp)
    {
        ...
        cmp2 = snmp_oid_compare (lp->viewSubtree, lp->viewSubtreeLen, vp->viewSubtree, vp->viewSubtreeLen);
        if (cmp == 0 && cmp2 > 0)
            break;
        if (cmp > 0)
            break;
...
void netsnmp_view_destroy (struct vacm_viewEntry **head, const char *viewName, oid * viewSubtree, size_t viewSubtreeLen)
{
    struct vacm_viewEntry *vp, *lastvp = NULL;

    if ((*head) && !strcmp ((*head)->viewName + 1, viewName)
        && (*head)->viewSubtreeLen == viewSubtreeLen
        && !memcmp ((char *) (*head)->viewSubtree, (char *) viewSubtree, viewSubtreeLen * sizeof (oid)))
    {
        vp = (*head);
        (*head) = (*head)->next;
    }
    else
    {
        for (vp = (*head); vp; vp = vp->next)
        {
            if (!strcmp (vp->viewName + 1, viewName)
                ...
                && !memcmp ((char *) vp->viewSubtree, (char *) viewSubtree, viewSubtreeLen * sizeof (oid)))
                break;
            lastvp = vp;
        }
        if (!vp || !lastvp)
...
struct vacm_groupEntry *vacm_getGroupEntry (int securityModel, const char *securityName)
{
    ...
    for (vp = groupList; vp; vp = vp->next)
    {
        if ((securityModel == vp->securityModel
             || vp->securityModel == SNMP_SEC_MODEL_ANY) && !memcmp (vp->securityName, secname, glen + 1))
            return vp;
    }
    return NULL;
}
...
struct vacm_groupEntry *vacm_createGroupEntry (int securityModel, const char *securityName)
{
    ...
    while (lg)
    {
        if (lg->securityModel > securityModel)
            break;
        if (lg->securityModel == securityModel && (cmp = memcmp (lg->securityName, gp->securityName, glen + 1)) > 0)
            break;
        /*
...
void vacm_destroyGroupEntry (int securityModel, const char *securityName)
{
    ...
    if (groupList && groupList->securityModel == securityModel && !strcmp (groupList->securityName + 1, securityName))
    {
        vp = groupList;
    ...
    else
    {
        for (vp = groupList; vp; vp = vp->next)
        {
            if (vp->securityModel == securityModel && !strcmp (vp->securityName + 1, securityName))
                break;
            lastvp = vp;
        }
        if (!vp || !lastvp)
...
struct vacm_accessEntry *_vacm_choose_best (struct vacm_accessEntry *current, struct vacm_accessEntry *candidate)
{
    /*
    ...
    if ((!current) ||
         ...
         */
        (current->securityModel == SNMP_SEC_MODEL_ANY && candidate->securityModel != SNMP_SEC_MODEL_ANY) ||
        /* b) if the subset of entries with vacmAccessContextPrefix
         *    matching the contextName in the message is
         *    not empty, then discard the rest
         */
        (current->contextMatch == CONTEXT_MATCH_PREFIX && candidate->contextMatch == CONTEXT_MATCH_EXACT) ||
        /* c) discard all entries with ContextPrefixes shorter
         *    than the longest one remaining in the set
         */
        (current->contextMatch == CONTEXT_MATCH_PREFIX && current->contextPrefix[0] < candidate->contextPrefix[0]) ||
        ...
        if ((securityModel == vp->securityModel
            ...
            ((vp->contextMatch == CONTEXT_MATCH_EXACT
             ...
             || (vp->contextMatch == CONTEXT_MATCH_PREFIX
                 && clen >= vp->contextPrefix[0]
                 && (memcmp (vp->contextPrefix + 1, context + 1, vp->contextPrefix[0]) == 0))))
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
void vacm_destroyAccessEntry (const char *groupName, const char *contextPrefix, int securityModel, int securityLevel)
{
    ...
    if (accessList && accessList->securityModel == securityModel
        ...
        && !strcmp (accessList->groupName + 1, groupName) && !strcmp (accessList->contextPrefix + 1, contextPrefix))
    {
        vp = accessList;
    ...
    else
    {
        for (vp = accessList; vp; vp = vp->next)
        {
            if (vp->securityModel == securityModel
                && vp->securityLevel == securityLevel
                && !strcmp (vp->groupName + 1, groupName) && !strcmp (vp->contextPrefix + 1, contextPrefix))
                break;
            lastvp = vp;
        }
        if (!vp || !lastvp)
 ...
 */
struct vacm_viewEntry *vacm_getViewEntry (const char *viewName, oid * viewSubtree, size_t viewSubtreeLen, int mode)
{
    return netsnmp_view_get (viewList, viewName, viewSubtree, viewSubtreeLen, mode);
}

int vacm_checkSubtree (const char *viewName, oid * viewSubtree, size_t viewSubtreeLen)
{
    return netsnmp_view_subtree_check (viewList, viewName, viewSubtree, viewSubtreeLen);
}

struct vacm_viewEntry *vacm_createViewEntry (const char *viewName, oid * viewSubtree, size_t viewSubtreeLen)
{
    return netsnmp_view_create (&viewList, viewName, viewSubtree, viewSubtreeLen);
}
