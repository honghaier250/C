...
netsnmp_feature_child_of (openssl_err_log, netsnmp_unused) netsnmp_feature_child_of (cert_dump_names, netsnmp_unused)
#include <ctype.h>
...
static char *_cert_get_name (X509 * ocert, int which, char **buf, int *len, int flags)
{
 ...
 */
char *netsnmp_openssl_cert_get_subjectName (X509 * ocert, char **buf, int *len)
{
    X509_NAME *osubj_name;
 ...
 */
char *netsnmp_openssl_cert_get_commonName (X509 * ocert, char **buf, int *len)
{
...
void netsnmp_openssl_cert_dump_names (X509 * ocert)
{
    ...
    for (i = 0; i < X509_NAME_entry_count (osubj_name); i++)
    {
        ...
        DEBUGMSGT (("9:cert:dump:names", "[%02d] NID type %d, ASN type %d\n", i, onid, oname_entry->value->type));
...
static char *_cert_get_extension (X509_EXTENSION * oext, char **buf, int *len, int flags)
{
    int space;
    ...
    if (!buf_ptr)
    {
        snmp_log (LOG_ERR, "not enough space or error in allocation for extenstion\n");
        BIO_vfree (bio);
        return NULL;
    }
...
X509_EXTENSION *_cert_get_extension_at (X509 * ocert, int pos, char **buf, int *len, int flags)
{
    X509_EXTENSION *oext;
...
/** instead of exposing this function, make helper functions for each
 * field, like netsnmp_openssl_cert_get_subjectAltName, below */
static char *_cert_get_extension_str_at (X509 * ocert, int pos, char **buf, int *len, int flags)
{
    X509_EXTENSION *oext;
...
/** instead of exposing this function, make helper functions for each
 * field, like netsnmp_openssl_cert_get_subjectAltName, below */
X509_EXTENSION *_cert_get_extension_id (X509 * ocert, int which, char **buf, int *len, int flags)
{
    int pos;
...
/** instead of exposing this function, make helper functions for each
 * field, like netsnmp_openssl_cert_get_subjectAltName, below */
static char *_cert_get_extension_id_str (X509 * ocert, int which, char **buf, int *len, int flags)
{
    int pos;
...
static char *_extract_oname (const GENERAL_NAME * oname)
{
        ...
        case GEN_IPADD:
            if (oname->d.iPAddress->length == 4)
            {
                sprintf (ipbuf, "%d.%d.%d.%d", oname->d.iPAddress->data[0],
                         oname->d.iPAddress->data[1], oname->d.iPAddress->data[2], oname->d.iPAddress->data[3]);
                rtn = strdup (ipbuf);
            }
            else if ((oname->d.iPAddress->length == 16) || (oname->d.iPAddress->length == 20))
            {
                char *pos = ipbuf;
 ...
 * be responsbile for releasing.
 */
char *netsnmp_openssl_cert_get_subjectAltNames (X509 * ocert, char **buf, int *len)
{
...
void netsnmp_openssl_cert_dump_extensions (X509 * ocert)
{
    ...
    for (i = 0; i < num_extensions; i++)
    {
        ...
        while (lf)
        {                        /* log remaining parts of multiline string */
            str = ++lf;
            if (*str == '\0')
                break;
            lf = strchr (str, '\n');
...
char *netsnmp_openssl_cert_get_fingerprint (X509 * ocert, int alg)
{
    ...
    if (_nid2ht (nid) != alg)
    {
        DEBUGMSGT (("openssl:fingerprint", "WARNING: alg %d does not match cert alg %d\n", alg, _nid2ht (nid)));
    }
...
*/
char *_cert_get_san_type (X509 * ocert, int mapType)
{
    ...
    onames = (GENERAL_NAMES *) X509_get_ext_d2i (ocert, NID_subject_alt_name, NULL, NULL);
    if (NULL == onames)
        return NULL;
    ...
    for (i = 0; i < count; ++i)
    {
        ...
        {
            if ((TSNM_tlstmCertSANDNSName == mapType) || (TSNM_tlstmCertSANAny == mapType))
            {
        ...
        {
            if ((TSNM_tlstmCertSANIpAddress == mapType) || (TSNM_tlstmCertSANAny == mapType))
                buf = _extract_oname (oname);
            break;
        }
        else if (GEN_EMAIL == oname->type)
        {
            if ((TSNM_tlstmCertSANRFC822Name == mapType) || (TSNM_tlstmCertSANAny == mapType))
            {
                buf = _extract_oname (oname);
                lower = strchr (buf, '@');
                if (NULL == lower)
                {
                    DEBUGMSGT (("openssl:secname:extract", "email %s has no '@'!\n", buf));
                }
                else
                {
                    ++lower;
    ...
    DEBUGMSGT (("openssl:cert:extension:san", "#%d type %d: %s\n", i, oname ? oname->type : -1, buf ? buf : "NULL"));

    return buf;
}

char *netsnmp_openssl_extract_secname (netsnmp_cert_map * cert_map, netsnmp_cert_map * peer_cert)
{
    char *rtn = NULL;
    ...
    switch (cert_map->mapType)
    {
        ...
        case TSNM_tlstmCertCommonName:
            rtn = netsnmp_openssl_cert_get_commonName (cert_map->ocert, NULL, NULL);
            break;
        default:
    ...
    if (rtn)
    {
        ...
        if (strlen (rtn) > 32)
        {
            DEBUGMSGT (("openssl:secname:extract", "secName longer than 32 chars! dropping...\n"));
            SNMP_FREE (rtn);
        }
    }
    else
...
void netsnmp_openssl_null_checks (SSL * ssl, int *null_auth, int *null_cipher)
{
