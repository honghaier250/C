...
netsnmp_feature_child_of (tdomain_transport_oid, transport_all) netsnmp_feature_child_of (sockaddr_size, transport_all)
...
void init_snmp_transport (void)
{
    netsnmp_ds_register_config (ASN_BOOLEAN,
                                ...
                                NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DONT_LOAD_HOST_FILES);
}
...
int netsnmp_transport_send (netsnmp_transport * t, void *packet, int length, void **opaque, int *olength)
{
    ...
    dumpPacket = netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DUMP_PACKET);
    debugLength = (SNMPERR_SUCCESS == debug_is_token_registered ("transport:send"));
...
int netsnmp_transport_recv (netsnmp_transport * t, void *packet, int length, void **opaque, int *olength)
{
    int debugLength;
    ...
    debugLength = (SNMPERR_SUCCESS == debug_is_token_registered ("transport:recv"));
...
int netsnmp_tdomain_support (const oid * in_oid, size_t in_len, const oid ** out_oid, size_t * out_len)
{
...
int netsnmp_tdomain_register (netsnmp_tdomain * n)
{
    ...
    if (n != NULL)
    {
        for (d = domain_list; d != NULL; d = d->next)
        {
            if (netsnmp_oid_equals (n->name, n->name_length, d->name, d->name_length) == 0)
            {
                /*
                 * Already registered.  
                 */
...
static netsnmp_tdomain *find_tdomain (const char *spec)
{
    ...
    for (d = domain_list; d != NULL; d = d->next)
    {
        ...
        for (i = 0; d->prefix[i] != NULL; i++)
            ...
            {
                DEBUGMSGTL (("tdomain", "Found domain \"%s\" from specifier \"%s\"\n", d->prefix[0], spec));
                return d;
            }
    }
...
static int netsnmp_is_fqdn (const char *thename)
{
    ...
    while (*thename)
    {
        if (*thename != '.' && !isupper ((unsigned char) *thename) &&
            !islower ((unsigned char) *thename) && !isdigit ((unsigned char) *thename) && *thename != '-')
        {
            return 0;
        }
        thename++;
...
netsnmp_transport *netsnmp_tdomain_transport_full (const char *application,
                                                   ...
                                                   const char *default_domain, const char *default_target)
{
    ...
    DEBUGMSGTL (("tdomain",
                 ...
                 default_domain ? default_domain : "[NIL]", default_target ? default_target : "[NIL]"));

    /* see if we can load a host-name specific set of conf files */
    if (!netsnmp_ds_get_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DONT_LOAD_HOST_FILES) && netsnmp_is_fqdn (str))
    {
        ...
        if (!have_added_handler)
        {
            ...
            netsnmp_ds_register_config (ASN_OCTET_STR,
                                        "snmp", "transport", NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_HOSTNAME);
        }
        ...
        prev_hostname = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_HOSTNAME);
        if (prev_hostname)
        ...
        if (NULL != (newhost = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_HOSTNAME)))
        {
            strlcpy (buf, newhost, sizeof (buf));
            str = buf;
        }

        netsnmp_ds_set_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_HOSTNAME, prev_hostname);
    ...
    if (match == NULL)
    {
        ...
        if (addr && *addr == '/')
        {
            DEBUGMSGTL (("tdomain", "Address starts with '/', so assume \"unix\" " "domain\n"));
        ...
        else
        {
            ...
            if (spec == NULL)
            {
                DEBUGMSGTL (("tdomain", "No default domain found, assume \"udp\"\n"));
    ...
    for (;;)
    {
        if (match)
        {
            ...
            DEBUGMSGTL (("tdomain",
                         ...
                         "default address \"%s\"\n", match->prefix[0], addr ? addr : "[NIL]", addr2 ? addr2 : "[NIL]"));
...
{
    return netsnmp_tdomain_transport_full ("snmp", str, local, default_domain, NULL);
}


#ifndef NETSNMP_FEATURE_REMOVE_TDOMAIN_TRANSPORT_OID
netsnmp_transport *netsnmp_tdomain_transport_oid (const oid * dom,
                                                  size_t dom_len, const u_char * o, size_t o_len, int local)
{
    netsnmp_tdomain *d;
...
netsnmp_transport *netsnmp_transport_open_server (const char *application, const char *str)
{
...
int netsnmp_transport_add_to_list (netsnmp_transport_list ** transport_list, netsnmp_transport * transport)
{
...
/**  removes a transport from a linked list of transports.
     Returns 1 on failure, 0 on success */
int netsnmp_transport_remove_from_list (netsnmp_transport_list ** transport_list, netsnmp_transport * transport)
{
...
int netsnmp_transport_config_compare (netsnmp_transport_config * left, netsnmp_transport_config * right)
{
    return strcmp (left->key, right->key);
}

netsnmp_transport_config *netsnmp_transport_create_config (char *key, char *value)
{
    netsnmp_transport_config *entry = SNMP_MALLOC_TYPEDEF (netsnmp_transport_config);
