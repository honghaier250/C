...
void snmpv3_secLevel_conf (const char *word, char *cptr)
{
    ...
    {
        netsnmp_ds_set_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SECLEVEL, secLevel);
    }
    else
    {
...
NETSNMP_IMPORT int
snmpv3_options (char *optarg, netsnmp_session * session, char **Apsz, char **Xpsz, int argc, char *const *argv);
int snmpv3_options (char *optarg, netsnmp_session * session, char **Apsz, char **Xpsz, int argc, char *const *argv)
{
    char *cp = optarg;
    ...
    switch (*cp)
    {
        ...
        case 'l':
            ...
            else if (!strcasecmp (optarg, "authNoPriv") || !strcmp (optarg, "2") || !strcasecmp (optarg, "anp"))
            {
                session->securityLevel = SNMP_SEC_LEVEL_AUTHNOPRIV;
            }
            else if (!strcasecmp (optarg, "authPriv") || !strcmp (optarg, "3") || !strcasecmp (optarg, "ap"))
            {
        ...
        case 'm':
            {
                ...
                if (!snmp_hex_to_binary (&tmpp, &bufSize, &session->securityAuthKeyLen, 0, optarg))
                {
        ...
        case 'M':
            {
                ...
                if (!snmp_hex_to_binary (&tmpp, &bufSize, &session->securityPrivKeyLen, 0, optarg))
                {
                    fprintf (stderr, "Bad key value after -3M flag.\n");
                    return (-1);
                }
...
{
    int enterpriseid = htonl (NETSNMP_ENTERPRISE_OID), netsnmpoid = htonl (NETSNMP_OID), localsetup = (eidp) ? 0 : 1;
    ...
    switch (localEngineIDType)
    {
        case ENGINEID_TYPE_TEXT:
            if (NULL == text)
            {
                snmp_log (LOG_ERR, "Can't set up engineID of type text from an empty string.\n");
                return -1;
            }
...
void version_conf (const char *word, char *cptr)
{
    ...
    {
        netsnmp_ds_set_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SNMPVERSION, NETSNMP_DS_SNMP_VERSION_1);    /* bogus value */
        valid = 1;
    }
#endif
    ...
    {
        netsnmp_ds_set_int (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_SNMPVERSION, NETSNMP_DS_SNMP_VERSION_3);
        valid = 1;
    }
    if (!valid)
    {
...
void exactEngineID_conf (const char *word, char *cptr)
{
    ...
    {
        netsnmp_config_error ("exactEngineID '%s' too long; truncating to %d bytes", cptr, MAX_ENGINEID_LENGTH);
...
void init_snmpv3 (const char *type)
{
     ...
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_POST_READ_CONFIG, init_snmpv3_post_config, NULL);

    snmp_register_callback (SNMP_CALLBACK_LIBRARY,
                            SNMP_CALLBACK_POST_PREMIB_READ_CONFIG, init_snmpv3_post_premib_config, NULL);
    /*
     * we need to be called back later 
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, snmpv3_store, (void *) strdup (type));
    ...
    register_prenetsnmp_mib_handler (type, "exactEngineID", exactEngineID_conf, NULL, NULL);
    ...
    netsnmp_ds_register_config (ASN_OCTET_STR, "snmp", "defContext", NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_CONTEXT);
    ...
    netsnmp_ds_register_config (ASN_OCTET_STR, "snmp", "defAuthMasterKey",
                                NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_AUTHMASTERKEY);
    netsnmp_ds_register_config (ASN_OCTET_STR, "snmp", "defPrivMasterKey",
                                NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_PRIVMASTERKEY);
    ...
    netsnmp_ds_register_config (ASN_OCTET_STR, "snmp", "defPrivLocalizedKey",
                                NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_PRIVLOCALIZEDKEY);
    register_config_handler ("snmp", "defVersion", version_conf, NULL, "1|2c|3");

    register_config_handler ("snmp", "defSecurityLevel",
                             snmpv3_secLevel_conf, NULL, "noAuthNoPriv|authNoPriv|authPriv");
}
...
int init_snmpv3_post_config (int majorid, int minorid, void *serverarg, void *clientarg)
{
    ...
    if (engineIDLen != oldEngineIDLength ||
        oldEngineID == NULL || c_engineID == NULL || memcmp (oldEngineID, c_engineID, engineIDLen) != 0)
    {
        engineBoots = 1;
    }
     ...
     */
    set_enginetime (c_engineID, engineIDLen, snmpv3_local_snmpEngineBoots (), snmpv3_local_snmpEngineTime (), TRUE);
#endif                            /* NETSNMP_SECMOD_USM */
...
int init_snmpv3_post_premib_config (int majorid, int minorid, void *serverarg, void *clientarg)
{
    if (!engineIDIsSet)
...
int snmpv3_store (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    if (engineIDLen)
    {
        ...
        read_config_save_octet_string (line + strlen (line), c_engineID, engineIDLen);
        read_config_store (type, line);
    }
    return SNMPERR_SUCCESS;
}                                /* snmpv3_store() */
 ...
 *
 */
int snmpv3_clone_engineID (u_char ** dest, size_t * destlen, u_char * src, size_t srclen)
{
    if (!dest || !destlen)
        return 0;
