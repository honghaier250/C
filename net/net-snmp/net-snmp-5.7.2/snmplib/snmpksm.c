...
static int init_snmpksm_post_config (int majorid, int minorid, void *serverarg, void *clientarg)
{
...
void init_ksm (void)
{
    ...
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_POST_READ_CONFIG, init_snmpksm_post_config, NULL);
    ...
    if (kcontext == NULL)
    {
        ...
        if (retval)
        {
            DEBUGMSGTL (("ksm", "krb5_init_context failed (%s), not " "registering KSM\n", error_message (retval)));
            return;
        }
    }
    ...
    if (!def)
    {
        DEBUGMSGTL (("ksm", "Unable to malloc snmp_secmod struct, not " "registering KSM\n"));
        return;
    }
...
static int ksm_insert_cache (long msgid, krb5_auth_context auth_context, u_char * secName, size_t secNameLen)
{
...
static struct ksm_cache_entry *ksm_get_cache (long msgid)
{
    struct ksm_cache_entry *entry;
...
static void ksm_decrement_ref_count (long msgid)
{
    ...
    if (ksm_hash_table[bucket] && ksm_hash_table[bucket]->msgid == msgid)
    {
        ...
        for (entry1 = ksm_hash_table[bucket], entry = entry1->next; entry != NULL; entry1 = entry, entry = entry->next)
            if (entry->msgid == msgid)
            {
...
static void ksm_increment_ref_count (long msgid)
{
    struct ksm_cache_entry *entry = ksm_get_cache (msgid);

    if (!entry)
    {
        DEBUGMSGTL (("ksm", "Unable to find cache entry for msgid %ld " "for increment\n", msgid));
        return;
    }
...
static int ksm_session_init (netsnmp_session * sess)
{
...
static void ksm_free_state_ref (void *ptr)
{
...
static int ksm_free_pdu (netsnmp_pdu * pdu)
{
...
static int ksm_clone_pdu (netsnmp_pdu * pdu, netsnmp_pdu * pdu2)
{
...
int ksm_rgenerate_out_msg (struct snmp_secmod_outgoing_params *parms)
{
    ...
    struct ksm_secStateRef *ksm_state = (struct ksm_secStateRef *) parms->secStateRef;
    ...
    if (!ksm_state)
    {
        ...
        retcode =
            krb5_mk_req (kcontext, &auth_context,
                         ...
                         (char *) service_name, parms->session->peername, NULL, cc, &outdata);
        ...
        DEBUGMSGTL (("ksm", "KSM: ticket retrieved successfully for \"%s/%s\" "
                     "(may not be actual ticket sname)\n", service_name, parms->session->peername));
    ...
    if (parms->secLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
...
#ifdef NETSNMP_USE_KERBEROS_MIT
        retcode = krb5_c_encrypt_length (kcontext, subkey->enctype, parms->scopedPduLen, &encrypted_length);
        ...
        encrypted_length = krb5_get_wrapped_length (kcontext, heim_crypto, parms->scopedPduLen);
#else                            /* NETSNMP_USE_KERBEROS_MIT */
        ...
        encrypted_length = krb5_encrypt_size (parms->scopedPduLen, eblock.crypto_entry);
#endif                            /* NETSNMP_USE_KERBEROS_MIT */
        ...
        retcode = krb5_c_encrypt (kcontext, subkey, KSM_KEY_USAGE_ENCRYPTION, &ivector, &input, &output);

#elif defined OLD_HEIMDAL        /* NETSNMP_USE_KERBEROS_MIT */

        krb5_data_zero (&encrypted_scoped_pdu);
        retcode = krb5_encrypt (kcontext, heim_crypto, KSM_KEY_USAGE_ENCRYPTION,
                                parms->scopedPdu, parms->scopedPduLen, &encrypted_scoped_pdu);
        if (retcode == 0)
        {
        ...
        retcode = krb5_encrypt (kcontext, (krb5_pointer) parms->scopedPdu,
                                (krb5_pointer) encrypted_data, parms->scopedPduLen, &eblock, ivector.data);
        ...
        *offset = 0;
        ...
        rc = asn_realloc_rbuild_string (wholeMsg, parms->wholeMsgLen,
                                        ...
                                        (u_char) (ASN_UNIVERSAL |
                                                  ASN_PRIMITIVE | ASN_OCTET_STR), encrypted_data, encrypted_length);
    ...
    rc = asn_realloc_rbuild_int (wholeMsg, parms->wholeMsgLen,
                                 offset, 1,
                                 (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), (long *) &zero, sizeof (zero));
    ...
    rc = asn_realloc_rbuild_string (wholeMsg, parms->wholeMsgLen,
                                    offset, 1,
                                    (u_char) (ASN_UNIVERSAL |
                                              ASN_PRIMITIVE | ASN_OCTET_STR), (u_char *) outdata.data, outdata.length);
...
#ifdef NETSNMP_USE_KERBEROS_MIT
    retcode = krb5_c_keyed_checksum_types (kcontext, subkey->enctype, &numcksumtypes, &cksumtype_array);
    ...
    if (numcksumtypes <= 0)
    {
        DEBUGMSGTL (("ksm", "We received a list of zero cksumtypes for this " "enctype (%d)\n", subkey->enctype));
    ...
    DEBUGMSGTL (("ksm", "KSM: Choosing checksum type of %d (subkey type " "of %d)\n", cksumtype, subkey->enctype));
...
#ifdef OLD_HEIMDAL
    {
        ...
        retval = krb5_create_checksum (kcontext, heim_crypto,
                                       KSM_KEY_USAGE_CHECKSUM, 0, parms->scopedPdu, parms->scopedPduLen, &pdu_checksum);
        if (retval)
        {
    ...
    {
#endif                            /* OLD_HEIMDAL */
        DEBUGMSGTL (("ksm", "Checksum type %d is not a collision-proof " "checksum\n", cksumtype));
    ...
    {
        retval = krb5_checksumsize (kcontext, cksumtype, &CHECKSUM_LENGTH (&pdu_checksum));
        if (retval)
        {
            DEBUGMSGTL (("ksm", "Unable to determine checksum length: %s\n", error_message (retval)));
            snmp_set_detail (error_message (retcode));
...
rc = asn_realloc_rbuild_header (wholeMsg, parms->wholeMsgLen,
                                ...
                                (u_char) (ASN_UNIVERSAL |
                                          ASN_PRIMITIVE | ASN_OCTET_STR), CHECKSUM_LENGTH (&pdu_checksum));
...
rc = asn_realloc_rbuild_int (wholeMsg, parms->wholeMsgLen,
                             parms->wholeMsgOffset, 1,
                             (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &tmp, sizeof (tmp));
...
rc = asn_realloc_rbuild_sequence (wholeMsg, parms->wholeMsgLen,
                                  parms->wholeMsgOffset, 1,
                                  (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), *offset - seq_offset);
...
rc = asn_realloc_rbuild_header (wholeMsg, parms->wholeMsgLen,
                                parms->wholeMsgOffset, 1,
                                (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), *offset - seq_offset);
...
memcpy (*wholeMsg + *parms->wholeMsgLen - *offset, parms->globalData, parms->globalDataLen);

rc = asn_realloc_rbuild_sequence (wholeMsg, parms->wholeMsgLen,
                                  offset, 1, (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), *offset);
...
input.length = *offset;
retcode = krb5_c_make_checksum (kcontext, cksumtype, subkey, KSM_KEY_USAGE_CHECKSUM, &input, &pdu_checksum);

#elif defined(OLD_HEIMDAL)        /* NETSNMP_USE_KERBEROS_MIT */

retcode = krb5_create_checksum (kcontext, heim_crypto,
                                KSM_KEY_USAGE_CHECKSUM, cksumtype,
                                *wholeMsg + *parms->wholeMsgLen - *offset, *offset, &pdu_checksum);
#else                            /* NETSNMP_USE_KERBEROS_MIT */

retcode = krb5_calculate_checksum (kcontext, cksumtype, *wholeMsg +
                                   *parms->wholeMsgLen - *offset,
                                   *offset, (krb5_pointer) subkey->contents, subkey->length, &pdu_checksum);
...
for (i = 0; i < CHECKSUM_LENGTH (&pdu_checksum); i++)
    DEBUGMSG (("ksm", " %02x", (unsigned int) CHECKSUM_CONTENTS (&pdu_checksum)[i]));
...
if (!ksm_state)
{
    if ((retval = ksm_insert_cache (parms->pdu->msgid, auth_context,
                                    (u_char *) parms->secName, parms->secNameLen)) != SNMPERR_SUCCESS)
        goto error;
    auth_context = NULL;
}
...
int ksm_process_in_msg (struct snmp_secmod_incoming_params *parms)
{
    long temp;
    ...
    size_t length = parms->wholeMsgLen - (u_int) (parms->secParams - parms->wholeMsg);
    ...
    if ((current = asn_parse_sequence (current, &length, &type,
                                       (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), "ksm first octet")) == NULL)
    {
    ...
    if ((current = asn_parse_sequence (current, &length, &type,
                                       (ASN_SEQUENCE | ASN_CONSTRUCTOR), "ksm sequence")) == NULL)
    {
    ...
    if ((current = asn_parse_int (current, &length, &type, &temp, sizeof (temp))) == NULL)
    {
        DEBUGMSGTL (("ksm", "Security parameter checksum type parsing" "failed\n"));
    ...
    if ((current = asn_parse_sequence (current, &cksumlength, &type,
                                       (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), "ksm checksum")) == NULL)
    {
    ...
    if ((current = asn_parse_sequence (current, &length, &type,
                                       (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), "ksm ap_req")) == NULL)
    {
        DEBUGMSGTL (("ksm", "KSM security parameter AP_REQ/REP parsing " "failed\n"));
    ...
    if ((current = asn_parse_int (current, &length, &type, &hint, sizeof (hint))) == NULL)
    {
        ...
        && (((char *) ap_req.data)[0] == 0x6e || ((char *) ap_req.data)[0] == 0x4e))
    {
        ...
        retcode = krb5_rd_req (kcontext, &auth_context, &ap_req, NULL, keytab, &flags, &ticket);
        ...
        retcode = krb5_unparse_name (kcontext, TICKET_CLIENT (ticket), &cname);
...
if (ap_req.length && (ap_req.data[0] == 0x6f || ap_req.data[0] == 0x4f))
{
#else                            /* NETSNMP_USE_KERBEROS_HEIMDAL */
    }
    else
if (ap_req.length && (((char *) ap_req.data)[0] == 0x6f || ((char *) ap_req.data)[0] == 0x4f))
{
#endif                            /* NETSNMP_USE_KERBEROS_HEIMDAL */
    /*
    ...
    if (!entry)
    {
        DEBUGMSGTL (("ksm", "KSM: Unable to find auth_context for PDU with " "message ID of %ld\n", parms->pdu->msgid));
        retval = SNMPERR_KRB5;
        goto error;
    }
...
retcode = krb5_c_verify_checksum (kcontext, subkey, KSM_KEY_USAGE_CHECKSUM, &input, &checksum, &valid);
#elif defined(OLD_HEIMDAL)        /* NETSNMP_USE_KERBEROS_MIT */
retcode = krb5_crypto_init (kcontext, subkey, 0, &heim_crypto);
if (retcode)
{
    ...
    retcode = krb5_verify_checksum (kcontext, heim_crypto,
                                    KSM_KEY_USAGE_CHECKSUM, parms->wholeMsg, parms->wholeMsgLen, &checksum);
#else                            /* NETSNMP_USE_KERBEROS_MIT */
retcode = krb5_verify_checksum (kcontext, cksumtype, &checksum,
                                parms->wholeMsg, parms->wholeMsgLen, (krb5_pointer) subkey->contents, subkey->length);
#endif                            /* NETSNMP_USE_KERBEROS_MIT */
...
if (retcode)
{
    ...
    if (!valid)
    {
        DEBUGMSGTL (("ksm", "Computed checksum did not match supplied " "checksum!\n"));
        retval = SNMPERR_KRB5;
    ...
    if (parms->secLevel == SNMP_SEC_LEVEL_AUTHPRIV)
    {
        ...
        if ((current = asn_parse_sequence (current, &length, &type,
                                           (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), "ksm pdu")) == NULL)
        {
        ...
        retcode = krb5_c_decrypt (kcontext, subkey, KSM_KEY_USAGE_ENCRYPTION, &ivector, &in_crypt, &output);
#elif defined (OLD_HEIMDAL)        /* NETSNMP_USE_KERBEROS_MIT */
        retcode = krb5_decrypt (kcontext, heim_crypto, KSM_KEY_USAGE_ENCRYPTION, current, length, &output);
        if (retcode == 0)
        {
        ...
        retcode = krb5_decrypt (kcontext, (krb5_pointer) current, *parms->scopedPdu, length, &eblock, ivector.data);
    ...
    *parms->maxSizeResponse = parms->maxMsgSize - 200;
    ...
    if (!response)
    {
        ...
        *parms->secNameLen = strlen (cname);
        ...
        if (!ksm_state)
        {
            DEBUGMSGTL (("ksm", "KSM unable to malloc memory for " "ksm_secStateRef\n"));
            retval = SNMPERR_MALLOC;
            goto error;
        }
  ...
  error:
    if (retval == SNMPERR_ASN_PARSE_ERR && snmp_increment_statistic (STAT_SNMPINASNPARSEERRS) == 0)
