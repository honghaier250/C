...
void netsnmp_external_event_info (int *numfds, fd_set * readfds, fd_set * writefds, fd_set * exceptfds)
{
    ...
    {
        snmp_log (LOG_ERR, "Use netsnmp_external_event_info2() for processing" " large file descriptors\n");
    }
...
void netsnmp_external_event_info2 (int *numfds,
                                   ...
                                   netsnmp_large_fd_set * writefds, netsnmp_large_fd_set * exceptfds)
{
    int i;
...
void netsnmp_dispatch_external_events (int *count, fd_set * readfds, fd_set * writefds, fd_set * exceptfds)
{
    ...
    if (netsnmp_copy_large_fd_set_to_fd_set (readfds, &lreadfds) < 0
        ...
        || netsnmp_copy_large_fd_set_to_fd_set (exceptfds, &lexceptfds) < 0)
    {
        snmp_log (LOG_ERR, "Use netsnmp_dispatch_external_events2() for processing" " large file descriptors\n");
    }
                                        ...
                                        netsnmp_large_fd_set * writefds, netsnmp_large_fd_set * exceptfds)
{
    ...
    for (i = 0; *count && (i < external_readfdlen) && !external_fd_unregistered; i++)
    {
    ...
    for (i = 0; *count && (i < external_writefdlen) && !external_fd_unregistered; i++)
    {
    ...
    for (i = 0; *count && (i < external_exceptfdlen) && !external_fd_unregistered; i++)
    {
