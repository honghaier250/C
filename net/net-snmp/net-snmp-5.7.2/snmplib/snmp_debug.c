...
NETSNMP_IMPORT void debug_config_register_tokens (const char *configtoken, char *tokens);
...
NETSNMP_IMPORT void debug_config_turn_on_debugging (const char *configtoken, char *line);
    ...
    else
    {
        for (i = 0; i < debug_num_tokens; i++)
        {
            if (dbg_tokens[i].token_name &&
                strncmp (dbg_tokens[i].token_name, token, strlen (dbg_tokens[i].token_name)) == 0)
            {
...
int debug_disable_token_logs (const char *token)
{
    ...
    else
    {
        for (i = 0; i < debug_num_tokens; i++)
        {
            if (strncmp (dbg_tokens[i].token_name, token, strlen (dbg_tokens[i].token_name)) == 0)
            {
...
void debugmsg_suboid (const char *token, const oid * theoid, size_t len)
{
    ...
    netsnmp_sprint_realloc_objid (&buf, &buf_len, &out_len, 1, &buf_overflow, theoid, len);
    if (buf_overflow)
    {
        if (buf != NULL)
        {
...
void debugmsg_var (const char *token, netsnmp_variable_list * var)
{
    ...
    if (sprint_realloc_variable (&buf, &buf_len, &out_len, 1, var->name, var->name_length, var))
    {
        if (buf != NULL)
        {
            debugmsg (token, "%s", buf);
...
void debug_config_register_tokens (const char *configtoken UNUSED, char *tokens UNUSED)
{
}

NETSNMP_IMPORT void debug_config_turn_on_debugging (const char *configtoken, char *line);

void debug_config_turn_on_debugging (const char *configtoken UNUSED, char *line UNUSED)
{
}

void debug_register_tokens (const char *tokens UNUSED)
{
}
...
int debug_enable_token_logs (const char *token UNUSED)
{
    return SNMPERR_GENERR;
}

int debug_disable_token_logs (const char *token UNUSED)
{
    return SNMPERR_GENERR;
}

int debug_is_token_registered (const char *token UNUSED)
{
    return SNMPERR_GENERR;
}

void debugmsg (const char *token UNUSED, const char *format UNUSED, ...)
{
}

void debugmsg_oid (const char *token UNUSED, const oid * theoid UNUSED, size_t len UNUSED)
{
}

void debugmsg_suboid (const char *token UNUSED, const oid * theoid UNUSED, size_t len UNUSED)
{
}

void debugmsg_var (const char *token UNUSED, netsnmp_variable_list * var UNUSED)
{
}

void
debugmsg_oidrange (const char *token UNUSED, const oid * theoid UNUSED,
                   size_t len UNUSED, size_t var_subid UNUSED, oid range_ubound UNUSED)
{
}

void debugmsg_hex (const char *token UNUSED, const u_char * thedata UNUSED, size_t len UNUSED)
{
}

void debugmsg_hextli (const char *token UNUSED, const u_char * thedata UNUSED, size_t len UNUSED)
{
}

void debugmsgtoken (const char *token UNUSED, const char *format UNUSED, ...)
{
}

void debug_combo_nc (const char *token UNUSED, const char *format UNUSED, ...)
{
}

void snmp_set_do_debugging (int val UNUSED)
{
}
...
{
    register_prenetsnmp_mib_handler ("snmp", "doDebugging", debug_config_turn_on_debugging, NULL, "(1|0)");
    register_prenetsnmp_mib_handler ("snmp", "debugTokens", debug_config_register_tokens, NULL, "token[,token...]");
}
