...
NETSNMP_STATIC_INLINE int _callback_lock (int major, int minor, const char *warn, int do_assert)
{
    ...
    DEBUGMSGTL (("9:callback:lock", "locked (%s,%s)\n",
                 types[major], (SNMP_CALLBACK_LIBRARY == major) ? SNMP_STRORNULL (lib[minor]) : "null"));
#endif
    ...
    {
        if (NULL != warn)
            snmp_log (LOG_WARNING, "lock in _callback_lock sleeps more than 100 milliseconds in %s\n", warn);
        if (do_assert)
...
int snmp_call_callbacks (int major, int minor, void *caller_arg)
{
    ...
    DEBUGMSGTL (("callback", "END calling callbacks for maj=%d min=%d (%d called)\n", major, minor, count));
...
int snmp_unregister_callback (int major, int minor, SNMPCallback * target, void *arg, int matchargs)
{
    ...
    while (scp != NULL)
    {
        if ((scp->sc_callback == target) && (!matchargs || (scp->sc_client_arg == arg)))
        {
...
int netsnmp_callback_clear_client_arg (void *ptr, int i, int j)
{
    ...
    for (; i < MAX_CALLBACK_IDS; i++, j = 0)
    {
        for (; j < MAX_CALLBACK_SUBIDS; j++)
        {
            ...
            while (scp != NULL)
            {
                if ((NULL != scp->sc_callback) && (scp->sc_client_arg != NULL) && (scp->sc_client_arg == ptr))
                {
