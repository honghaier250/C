...
static CK_RV get_session_handle (CK_MECHANISM_TYPE, CK_FLAGS, CK_SESSION_HANDLE_PTR);

static CK_RV get_slot_session_handle (netsnmp_pkcs_slot_session *, CK_SESSION_HANDLE_PTR);
 ...
 * mechanism.
 */
static CK_RV get_session_handle (CK_MECHANISM_TYPE mech_type, CK_FLAGS flag, CK_SESSION_HANDLE_PTR sess)
{
    CK_RV rv = CKR_OK;
    ...
    if (i == slotcount)
    {
        DEBUGMSGTL (("pkcs_init", "No cryptographic provider for %s", mech_type));
 ...
 */
static CK_RV get_slot_session_handle (netsnmp_pkcs_slot_session * p, CK_SESSION_HANDLE_PTR sess)
{
    CK_RV rv = CKR_OK;
    ...
    if (p->hdl == NULL)
    {
        ...
        if (rv != CKR_OK)
        {
            DEBUGMSGTL (("get_slot_session_handle", "can not open PKCS #11 session: %s", pkcserr_string (rv)));
        }
    }
    *sess = p->hdl;
           ...
           u_char * msg, u_int msglen, u_char * mac, size_t * maclen)
{
    ...
    if (get_session_handle (mech_type, CKF_SIGN, &hSession) != CKR_OK || hSession == NULL)
    {
        QUITFUN (SNMPERR_GENERR, pkcs_sign_quit);
    }

    /* create a key object */
    if (C_CreateObject (hSession, template, (sizeof (template) / sizeof (CK_ATTRIBUTE)), &hkey) != CKR_OK)
    {
        QUITFUN (SNMPERR_GENERR, pkcs_sign_quit);
    }
 ...
 */
int pkcs_digest (CK_MECHANISM_TYPE mech_type, u_char * msg, u_int msglen, u_char * digest, size_t * digestlen)
{
    int rval = SNMPERR_SUCCESS;
    ...
    if (get_session_handle (mech_type, CKF_DIGEST, &hSession) != CKR_OK || hSession == NULL)
    {
...
pkcs_encrpyt (CK_MECHANISM_TYPE mech_type, u_char * key, u_int keylen,
              u_char * iv, u_int ivlen, u_char * plaintext, u_int ptlen, u_char * ciphertext, size_t * ctlen)
{
    int rval = SNMPERR_SUCCESS;
    ...
    if (get_session_handle (mech_type, CKF_ENCRYPT, &hSession) != CKR_OK || hSession == NULL)
    {
    ...
    for (offset = 0; offset < ptlen; offset += pad_size)
    {
        ...
        if (C_EncryptUpdate (hSession, plaintext + offset, pad_size, ciphertext + offset, ctlen) != CKR_OK)
        {
            QUITFUN (SNMPERR_GENERR, pkcs_encrypt_quit);
        }
    }
              ...
              u_char * iv, u_int ivlen, u_char * ciphertext, u_int ctlen, u_char * plaintext, size_t * ptlen)
{
    ...
    if (get_session_handle (mech_type, CKF_DECRYPT, &hSession) != CKR_OK || hSession == NULL)
    {
    ...
    if (C_DecryptUpdate (hSession, ciphertext, ctlen, plaintext, ptlen) != CKR_OK)
    {
        QUITFUN (SNMPERR_GENERR, pkcs_decrypt_quit);
    }
 ...
 */
int pkcs_generate_Ku (CK_MECHANISM_TYPE mech_type, u_char * passphrase, u_int pplen, u_char * Ku, size_t * kulen)
{
