 ...
 */
     static int snmp_synch_input (int op, netsnmp_session * session, int reqid, netsnmp_pdu * pdu, void *magic);
...
static int snmp_synch_input (int op, netsnmp_session * session, int reqid, netsnmp_pdu * pdu, void *magic)
{
    ...
    if (op == NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE && pdu)
    {
        if (pdu->command == SNMP_MSG_REPORT)
        {
            ...
            if (SNMPV3_IGNORE_UNAUTH_REPORTS || rpt_type == SNMPERR_NOT_IN_TIME_WINDOW)
            {
                state->waiting = 1;
            }
            state->pdu = NULL;
        ...
        else
        {
            ...
            snprintf (msg_buf, sizeof (msg_buf), "Expected RESPONSE-PDU but got %s-PDU", snmp_pdu_type (pdu->command));
...
static netsnmp_pdu *_clone_pdu_header (netsnmp_pdu * pdu)
{
    ...
    if (snmp_clone_mem ((void **) &newpdu->enterprise, pdu->enterprise,
        ...
        || snmp_clone_mem ((void **) &newpdu->transport_data, pdu->transport_data, pdu->transport_data_length))
    {
    ...
    if ((sptr = find_sec_mod (newpdu->securityModel)) != NULL && sptr->pdu_clone != NULL)
    {
        /*
...
static netsnmp_variable_list *_copy_varlist (netsnmp_variable_list * var,    /* source varList */
 ...
 * Returns 0 if failure.
 */
static netsnmp_pdu *_copy_pdu_vars (netsnmp_pdu * pdu,    /* source PDU */
...
netsnmp_pdu *snmp_fix_pdu (netsnmp_pdu * pdu, int command)
{
    ...
    if ((pdu->command != SNMP_MSG_RESPONSE)
        ...
        || (NULL == pdu->variables) || (pdu->errindex > (int) snmp_varbind_len (pdu)) || (pdu->errindex <= 0))
    {
...
{
    for (; var_ptr != NULL && var_ptr->type != type; var_ptr = var_ptr->next_variable);

    return var_ptr;
}
#endif                            /* NETSNMP_FEATURE_REMOVE_FIND_VARIND_OF_TYPE */

netsnmp_variable_list *find_varbind_in_list (netsnmp_variable_list * vblist, const oid * name, size_t len)
{
...
int snmp_set_var_value (netsnmp_variable_list * vars, const void *value, size_t len)
{
    ...
    switch (vars->type)
    {
        ...
        case ASN_UINTEGER:
            ...
            {
                const unsigned long long *val_ullong = (const unsigned long long *) value;
...
int snmp_sess_synch_response (void *sessp, netsnmp_pdu * pdu, netsnmp_pdu ** response)
{
    ...
    while (state->waiting)
    {
        ...
        tvp = &timeout;
        timerclear (tvp);
        snmp_sess_select_info_flags (sessp, &numfds, &fdset, tvp, &block, NETSNMP_SELECT_NOALARMS);
        if (block == 1)
            tvp = NULL;            /* block without timeout */
        count = select (numfds, &fdset, NULL, NULL, tvp);
        if (count > 0)
...
netsnmp_session *netsnmp_query_get_default_session (void)
{
    ...
    {
        if (!netsnmp_ds_get_string (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_INTERNAL_SECNAME))
        {
            snmp_log (LOG_WARNING, "iquerySecName has not been configured - internal queries will fail\n");
        }
        else
        {
            snmp_log (LOG_WARNING, "default session is not available - internal queries will fail\n");
        }
        warning_logged = 1;
    }
 ...
 */
static int _query (netsnmp_variable_list * list, int request, netsnmp_session * session)
{
  ...
  retry:
    ...
    if (ret == SNMP_ERR_NOERROR)
    {
        if (response->errstat != SNMP_ERR_NOERROR)
        {
            ...
            if (response->errindex != 0)
            {
                ...
                for (count = 1, vtmp = response->variables;
                     vtmp && count != response->errindex; vtmp = vtmp->next_variable, count++)
                     /*EMPTY*/;
                if (vtmp)
        ...
        {
            for (vb1 = response->variables, vb2 = list; vb1; vb1 = vb1->next_variable, vb2 = vb2->next_variable)
            {
...
int netsnmp_query_walk (netsnmp_variable_list * list, netsnmp_session * session)
{
    ...
    while (ret == SNMP_ERR_NOERROR &&
           snmp_oidtree_compare (list->name, list->name_length, vb->name, vb->name_length) == 0)
    {
...
static netsnmp_variable_list *_get_vb_num (netsnmp_variable_list * vars, int index)
{
 ...
 * cleanup
 */
static int _row_status_state_cleanup (netsnmp_state_machine_input * input, netsnmp_state_machine_step * step)
{
    rowcreate_state *ctx;
 ...
 */
static int _row_status_state_activate (netsnmp_state_machine_input * input, netsnmp_state_machine_step * step)
{
    rowcreate_state *ctx;
    ...
    var = snmp_varlist_add_variable (&var, rs_var->name, rs_var->name_length, rs_var->type, &val, sizeof (val));
 ...
 */
static int _row_status_state_single_value_cols (netsnmp_state_machine_input * input, netsnmp_state_machine_step * step)
{
    rowcreate_state *ctx;
 ...
 */
static int
_row_status_state_multiple_values_cols (netsnmp_state_machine_input * input, netsnmp_state_machine_step * step)
{
    rowcreate_state *ctx;
    ...
    {
        for (last = vars, var = last->next_variable; var; last = var, var = var->next_variable)
        {
            if (var == row_status)
            {
 ...
 */
static int
_row_status_state_single_value_createAndWait (netsnmp_state_machine_input * input, netsnmp_state_machine_step * step)
{
    rowcreate_state *ctx;
 ...
 */
static int
_row_status_state_all_values_createAndWait (netsnmp_state_machine_input * input, netsnmp_state_machine_step * step)
{
    rowcreate_state *ctx;
 ...
 */
static int
_row_status_state_all_values_createAndGo (netsnmp_state_machine_input * input, netsnmp_state_machine_step * step)
{
    rowcreate_state *ctx;
...
{
    netsnmp_state_machine_step rc_cleanup = { "row_create_cleanup", 0, _row_status_state_cleanup,
        0, NULL, NULL, 0, NULL
    };
    netsnmp_state_machine_step rc_activate = { "row_create_activate", 0, _row_status_state_activate,
        0, NULL, NULL, 0, NULL
    };
    netsnmp_state_machine_step rc_sv_cols = { "row_create_single_value_cols", 0,
        _row_status_state_single_value_cols, 0, &rc_activate, NULL, 0, NULL
    };
    netsnmp_state_machine_step rc_mv_cols = { "row_create_multiple_values_cols", 0,
        _row_status_state_multiple_values_cols, 0, &rc_activate, &rc_sv_cols,
        0, NULL
    };
    netsnmp_state_machine_step rc_sv_caw = { "row_create_single_value_createAndWait", 0,
        _row_status_state_single_value_createAndWait, 0, &rc_mv_cols, NULL,
        0, NULL
    };
    netsnmp_state_machine_step rc_av_caw = { "row_create_all_values_createAndWait", 0,
        _row_status_state_all_values_createAndWait, 0, &rc_activate,
        &rc_sv_caw, 0, NULL
    };
    netsnmp_state_machine_step rc_av_cag = { "row_create_all_values_createAndGo", 0,
