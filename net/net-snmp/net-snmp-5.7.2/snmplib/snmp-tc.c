...
char check_rowstatus_with_storagetype_transition (int oldValue, int newValue, int oldStorage)
{
    /*
     * can not destroy permanent or readonly rows
     */
    if ((RS_DESTROY == newValue) && ((SNMP_STORAGE_PERMANENT == oldStorage) || (SNMP_STORAGE_READONLY == oldStorage)))
        return SNMP_ERR_WRONGVALUE;
     ...
     char check_storage_transition (int oldValue, int newValue)
{
    ...
    switch (oldValue)
    {
        ...
        case SNMP_STORAGE_NONVOLATILE:
            if (newValue == SNMP_STORAGE_PERMANENT || newValue == SNMP_STORAGE_READONLY)
                return SNMP_ERR_INCONSISTENTVALUE;
    }
