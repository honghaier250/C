...
netsnmp_feature_child_of (oid_stash, oid_stash_all) netsnmp_feature_child_of (oid_stash_no_free, oid_stash_all)
                                     ...
                                     const oid * lookup, size_t lookup_len, void *mydata)
{
    ...
    for (curnode = *root, i = 0; i < lookup_len; i++)
    {
        ...
        if (!tmpp)
        {
             ...
             */
            tmpp = curnode->children[lookup[i] % curnode->children_size] = netsnmp_oid_stash_create_node ();
            tmpp->value = lookup[i];
            tmpp->parent = curnode;
        }
        else
                                                             ...
                                                             oid * lookup, size_t lookup_len)
{
    ...
    do
    {
        ...
        {
            for (loopp = curnode->children[j]; loopp; loopp = loopp->next_sibling)
            {
 ...
 */
     void *netsnmp_oid_stash_get_data (netsnmp_oid_stash_node * root, const oid * lookup, size_t lookup_len)
{
    netsnmp_oid_stash_node *ret;
     ...
     int netsnmp_oid_stash_store_all (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    netsnmp_oid_stash_store (*(sinfo->root), sinfo->token, sinfo->dumpfn, oidbase, 0);
    return SNMP_ERR_NOERROR;
}
...
netsnmp_oid_stash_store (netsnmp_oid_stash_node * root,
                         const char *tokenname, NetSNMPStashDump * dumpfn, oid * curoid, size_t curoid_len)
{
        ...
        if (root->children[i])
        {
            for (tmpp = root->children[i]; tmpp; tmpp = tmpp->next_sibling)
            {
                ...
                if (tmpp->thedata)
                {
                    ...
                    cp = read_config_save_objid (buf + strlen (buf), curoid, curoid_len + 1);
                    *cp++ = ' ';
                    *cp = '\0';
                    if ((*dumpfn) (cp, sizeof (buf) - strlen (buf), tmpp->thedata, tmpp))
                        read_config_store (appname, buf);
                }
                netsnmp_oid_stash_store (tmpp, tokenname, dumpfn, curoid, curoid_len + 1);
            }
        }
    }
}
...
void oid_stash_dump (netsnmp_oid_stash_node * root, char *prefix)
{
    ...
    for (i = 0; i < root->children_size; i++)
    {
        if (root->children[i])
        {
            for (tmpp = root->children[i]; tmpp; tmpp = tmpp->next_sibling)
            {
                printf ("%s%" NETSNMP_PRIo "d@%d: %s\n", prefix, tmpp->value, i, (tmpp->thedata) ? "DATA" : "");
