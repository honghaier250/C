...
netsnmp_feature_child_of (cert_util_all, libnetsnmp) netsnmp_feature_child_of (cert_util, cert_util_all)
...
#define CERT_INDEX_FORMAT  1
     ...
     static int _cert_fn_compare (netsnmp_cert_common * lhs, netsnmp_cert_common * rhs);

     static int _cert_fn_ncompare (netsnmp_cert_common * lhs, netsnmp_cert_common * rhs);

     static void _find_partner (netsnmp_cert * cert, netsnmp_key * key);

     static netsnmp_cert *_find_issuer (netsnmp_cert * cert);

     static netsnmp_void_array *_cert_find_subset_fn (const char *filename, const char *directory);
...
void _netsnmp_release_trustcerts (void)
{
...
void _setup_trusted_certs (void)
{
...
{
    const char *trustCert_help = TRUSTCERT_CONFIG_TOKEN " FINGERPRINT|FILENAME";

    register_config_handler ("snmp", TRUSTCERT_CONFIG_TOKEN,
                             _parse_trustcert, _netsnmp_release_trustcerts, trustCert_help);
    _setup_containers ();
    ...
    /** map types */
    se_add_pair_to_slist ("cert_map_type", strdup ("cn"), TSNM_tlstmCertCommonName);
    se_add_pair_to_slist ("cert_map_type", strdup ("ip"), TSNM_tlstmCertSANIpAddress);
    se_add_pair_to_slist ("cert_map_type", strdup ("rfc822"), TSNM_tlstmCertSANRFC822Name);
    se_add_pair_to_slist ("cert_map_type", strdup ("dns"), TSNM_tlstmCertSANDNSName);
    se_add_pair_to_slist ("cert_map_type", strdup ("any"), TSNM_tlstmCertSANAny);
    se_add_pair_to_slist ("cert_map_type", strdup ("sn"), TSNM_tlstmCertSpecified);
...
void netsnmp_certs_load (void)
{
    ...
    DEBUGIF ("cert:dump")
    {
        ...
        ITERATOR_RELEASE (itr);
        DEBUGMSGT (("cert:dump", "-------------------- Certificates -----------------\n"));
        netsnmp_cert_dump_all ();
        DEBUGMSGT (("cert:dump", "------------------------ End ----------------------\n"));
    }
}
...
static netsnmp_container *_get_cert_container (const char *use)
{
    netsnmp_container *c;
...
static netsnmp_cert *_new_cert (const char *dirname, const char *filename, int certType,
                                int hashType, const char *fingerprint, const char *common_name, const char *subject)
{
    netsnmp_cert *cert;
...
static netsnmp_key *_new_key (const char *dirname, const char *filename)
{
    netsnmp_key *key;
    ...
    {
        snmp_log (LOG_ERR, "refusing to read world readable or writable key %s\n", fn);
        return NULL;
    }
...
void netsnmp_cert_free (netsnmp_cert * cert)
{
    ...
    DEBUGMSGT (("9:cert:struct:free", "freeing cert 0x%#lx, %s (fp %s; CN %s)\n",
                ...
                cert->fingerprint ? cert->fingerprint : "UNK", cert->common_name ? cert->common_name : "UNK"));
...
static int _cert_compare (netsnmp_cert * lhs, netsnmp_cert * rhs)
{
    ...
    netsnmp_assert ((lhs->fingerprint != NULL) && (rhs->fingerprint != NULL));

    /** ignore hash type? */
    return strcmp (lhs->fingerprint, rhs->fingerprint);
}

static int _cert_path_compare (netsnmp_cert_common * lhs, netsnmp_cert_common * rhs)
{
    int rc;
...
static int _cert_cn_compare (netsnmp_cert * lhs, netsnmp_cert * rhs)
{
    ...
    return _cert_path_compare ((netsnmp_cert_common *) lhs, (netsnmp_cert_common *) rhs);
}
...
static int _cert_ext_type (const char *ext)
{
...
static int _type_from_filename (const char *filename)
{
    char *pos;
 ...
 */
static int _cert_cert_filter (const char *filename)
{
...
static int _certindex_add (const char *dirname, int i)
{
    ...
    if (SE_OK != rc)
    {
        snmp_log (LOG_ERR, "adding certindex dirname failed; " "%d (%s) not added\n", i, dirname);
        return -1;
    }
 ...
 */
static void _certindexes_load (void)
{
    DIR *dir;
     ...
     */
    snprintf (filename, sizeof (filename), "%s/cert_indexes", get_persistent_directory ());
    ...
    while ((file = readdir (dir)))
    {
        ...
        snprintf (filename, sizeof (filename), "%s/cert_indexes/%d", get_persistent_directory (), i);
        filename[sizeof (filename) - 1] = 0;
        fp = fopen (filename, "r");
        if (!fp)
        {
 ...
 */
static char *_certindex_lookup (const char *dirname)
{
    int i;
...
static FILE *_certindex_new (const char *dirname)
{
    FILE *fp;
 ...
 *
 */
static X509 *netsnmp_ocert_get (netsnmp_cert * cert)
{
    BIO *certbio;
    ...
    switch (cert->info.type)
    {
            ...
            if (NULL == cert->key)
            {
                ...
                if (NULL != okey)
                {
                    ...
                    if (NULL != key)
                    {
                        ...
                        {
                            DEBUGMSGT (("cert:read:key:add", "error inserting key into container\n"));
                            netsnmp_key_free (key);
                            key = NULL;
                        }
                        else
        ...
        case NS_CERT_TYPE_PKCS12:
            ...
            if ((NULL != p12) && (PKCS12_verify_mac (p12, "", 0) || PKCS12_verify_mac (p12, NULL, 0)))
    ...
    {
        cert->subject = X509_NAME_oneline (X509_get_subject_name (ocert), NULL, 0);
    ...
    if (NULL == cert->fingerprint)
    {
        ...
        cert->fingerprint = netsnmp_openssl_cert_get_fingerprint (ocert, cert->hash_type);
    }

    if (NULL == cert->common_name)
    {
        cert->common_name = netsnmp_openssl_cert_get_commonName (ocert, NULL, NULL);
...
EVP_PKEY *netsnmp_okey_get (netsnmp_key * key)
{
    BIO *keybio;
...
static netsnmp_cert *_find_issuer (netsnmp_cert * cert)
{
    ...
    for (i = 0; (NULL == issuer) && (i < matching->size); ++i)
    {
        ...
        if ((NULL == candidate->ocert) && (netsnmp_ocert_get (candidate) == NULL))
            continue;
...
int netsnmp_cert_load_x509 (netsnmp_cert * cert)
{
    ...
    /** load key */
    if ((NULL != cert->key) && (NULL == cert->key->okey) && (netsnmp_okey_get (cert->key) == NULL))
    {
    ...
    for (; cert && cert->issuer; cert = cert->issuer_cert)
    {
        ...
        if (NULL == cert->issuer_cert)
        {
            ...
            if (NULL == cert->issuer_cert)
            {
                DEBUGMSGT (("cert:load:warn", "couldn't load CA chain for cert %s\n", cert->info.filename));
        ...
        /** get issuer ocert */
        if ((NULL == cert->issuer_cert->ocert) && (netsnmp_ocert_get (cert->issuer_cert) == NULL))
        {
...
static int _add_certfile (const char *dirname, const char *filename, FILE * index)
{
    X509 *ocert;
    ...
    if (NS_CERT_TYPE_KEY == type)
    {
        ...
        if (-1 == CONTAINER_INSERT (_keys, key))
        {
            DEBUGMSGT (("cert:key:file:add:err", "error inserting key into container\n"));
            netsnmp_key_free (key);
            key = NULL;
        }
    }
    else
    {
        ...
        {
            DEBUGMSGT (("cert:file:add:err", "error inserting cert into container\n"));
            netsnmp_cert_free (cert);
            cert = NULL;
        }
    }
    ...
    if (index)
    {
        ...
        if (cert)
            fprintf (index, "c:%s %d %d %s '%s' '%s'\n", filename,
                     cert->info.type, cert->hash_type, cert->fingerprint, cert->common_name, cert->subject);
        else if (key)
            fprintf (index, "k:%s\n", filename);
    }
...
static int _cert_read_index (const char *dirname, struct stat *dirstat)
{
    FILE *index;
    ...
    newer =
        netsnmp_directory_container_read_some (NULL, dirname,
                                               ...
                                               NETSNMP_DIR_NSFILE | NETSNMP_DIR_NSFILE_STATS);
    if (newer)
    {
    ...
    while (1)
    {
        ...
        if ('c' == tmpstr[0])
        {
            ...
            type = atoi (type_str);
            hash = atoi (hash_str);
            cert = (void *) _new_cert (dirname, filename, type, hash, fingerprint, common_name, subject);
            if (cert && 0 == CONTAINER_INSERT (found, cert))
                ++count;
            else
            {
                DEBUGMSGT (("cert:index:add", "error inserting cert into container\n"));
                netsnmp_cert_free (cert);
                cert = NULL;
            }
        }
        else if ('k' == tmpstr[0])
        {
            ...
            {
                DEBUGMSGT (("cert:index:add:key", "error inserting key into container\n"));
                netsnmp_key_free (key);
            }
        }
        else
...
static int _add_certdir (const char *dirname)
{
    FILE *index;
    ...
    if (NULL == index)
    {
        DEBUGMSGT (("9:cert:index:dir", "error opening index for cert directory\n"));
    ...
    {
        DEBUGMSGT (("cert:index:dir", "error creating container for cert files\n"));
        goto err_index;
    }
    ...
    if (NULL == it)
    {
        DEBUGMSGT (("cert:index:dir", "error creating iterator for cert files\n"));
        goto err_container;
    }
...
static void _cert_indexes_load (void)
{
    ...
    subdirs[0] = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_CERT_EXTRA_SUBDIR);
    confpath_copy = strdup (confpath);
    for (dir = strtok_r (confpath_copy, ENV_SEPARATOR, &st); dir; dir = strtok_r (NULL, ENV_SEPARATOR, &st))
    {
...
static void _cert_print (netsnmp_cert * c, void *context)
{
    if (NULL == c)
        return;
    ...
    DEBUGMSGT (("cert:dump", "   type %d flags 0x%x (%s)\n",
                c->info.type, c->info.allowed_uses, _mode_str (c->info.allowed_uses)));
    DEBUGIF ("9:cert:dump")
    {
        if (NS_CERT_TYPE_KEY != c->info.type)
        {
            ...
            if (c->fingerprint)
                DEBUGMSGT (("9:cert:dump", "   fingerprint: %s(%d):%s\n",
                            se_find_label_in_slist ("cert_hash_alg", c->hash_type), c->hash_type, c->fingerprint));
        }
...
static void _key_print (netsnmp_key * k, void *context)
{
    if (NULL == k)
        return;
...
netsnmp_cert *netsnmp_cert_find (int what, int where, void *hint)
{
    ...
    if (NS_CERTKEY_DEFAULT == where)
    {
        ...
        switch (what)
        {
            case NS_CERT_IDENTITY:    /* want my ID */
                fp = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_TLS_LOCAL_CERT);
                ...
                if (!fp)
                {
                    ...
                    fp = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, tmp ?
                                                NETSNMP_DS_LIB_X509_SERVER_PUB : NETSNMP_DS_LIB_X509_CLIENT_PUB);
                }
                if (!fp)
                {
                    /* As a special case, use the application type to
                       ...
                       from. */
                    return netsnmp_cert_find (what, NS_CERTKEY_FILE,
                                              netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_APPTYPE));
                }
                break;
            case NS_CERT_REMOTE_PEER:
                fp = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_TLS_PEER_CERT);

                /** temp backwards compability; remove in 5.7 */
                if (!fp)
                    fp = netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_X509_SERVER_PUB);
                break;
            default:
    ...
    if (!(result->info.allowed_uses & what))
    {
        DEBUGMSGT (("cert:find:err",
                    ...
                    what, _mode_str (result->info.allowed_uses), result->info.allowed_uses));
        return NULL;
    }
...
int netsnmp_cert_check_vb_fingerprint (const netsnmp_variable_list * var)
{
    ...
    if (!(0x01 & var->val_len))
    {                            /* odd len */
        DEBUGMSGT (("cert:varbind:fingerprint", "expecting odd length for fingerprint\n"));
...
netsnmp_tls_fingerprint_parse (const u_char * binary_fp, int fp_len,
                               char **fp_str_ptr, u_int * fp_str_len, int realloc, u_char * hash_type_ptr)
{
    int needed;
    ...
    *hash_type_ptr = binary_fp[0];
    netsnmp_binary_to_hex ((u_char **) fp_str_ptr, &fp_str_size, realloc, &binary_fp[1], fp_len - 1);
 ...
 */
int
netsnmp_tls_fingerprint_build (int hash_type, const char *hex_fp, u_char ** tls_fp, size_t * tls_fp_len, int realloc)
{
    int hex_fp_len, rc;
...
int netsnmp_cert_trust (SSL_CTX * ctx, netsnmp_cert * thiscert)
{
    ...
    netsnmp_assert_or_msgreturn (NULL != thiscert, "NULL certificate passed in", SNMPERR_GENERR);
    netsnmp_assert_or_msgreturn (NULL != thiscert->info.dir,
                                 "NULL certificate directory name passed in", SNMPERR_GENERR);
    netsnmp_assert_or_msgreturn (NULL != thiscert->info.filename,
                                 "NULL certificate filename name passed in", SNMPERR_GENERR);

    /* get the trusted certificate store and the certificate to load into it */
    certstore = SSL_CTX_get_cert_store (ctx);
    netsnmp_assert_or_msgreturn (NULL != certstore, "failed to get certificate trust store", SNMPERR_GENERR);
    cert = netsnmp_ocert_get (thiscert);
    netsnmp_assert_or_msgreturn (NULL != cert, "failed to get certificate from netsnmp_cert", SNMPERR_GENERR);

    /* Put the certificate into the store */
    fingerprint = netsnmp_openssl_cert_get_fingerprint (cert, -1);
    DEBUGMSGTL (("cert:trust", "putting trusted cert %p = %s in certstore %p\n", cert, fingerprint, certstore));
 ...
 * @return SNMPERR_GENERR  : on failure
 */
int netsnmp_cert_trust_ca (SSL_CTX * ctx, netsnmp_cert * thiscert)
{
    netsnmp_assert_or_msgreturn (NULL != thiscert, "NULL certificate passed in", SNMPERR_GENERR);
...
static const char *_where_str (u_int what)
{
    switch (what)
    {
        case NS_CERTKEY_DEFAULT:
            return "DEFAULT";
        case NS_CERTKEY_FILE:
            return "FILE";
        case NS_CERTKEY_FINGERPRINT:
            return "FINGERPRINT";
        case NS_CERTKEY_MULTIPLE:
            return "MULTIPLE";
        case NS_CERTKEY_CA:
            return "CA";
        case NS_CERTKEY_SAN_RFC822:
            return "SAN_RFC822";
        case NS_CERTKEY_SAN_DNS:
            return "SAN_DNS";
        case NS_CERTKEY_SAN_IPADDR:
            return "SAN_IPADDR";
        case NS_CERTKEY_COMMON_NAME:
            return "COMMON_NAME";
        case NS_CERTKEY_TARGET_PARAM:
            return "TARGET_PARAM";
        case NS_CERTKEY_TARGET_ADDR:
            return "TARGET_ADDR";
    }
 ...
 */
static void _reduce_subset (netsnmp_void_array * matching, const char *filename)
{
    ...
    for (; i < matching->size;)
    {
        ...
        if (('.' == cc->filename[pos]) && (NS_CERT_TYPE_UNKNOWN != _cert_ext_type (&cc->filename[pos + 1])))
        {
            ++i;
            continue;
        }
 ...
 */
static void _reduce_subset_dir (netsnmp_void_array * matching, const char *directory)
{
    netsnmp_cert_common *cc;
...
static netsnmp_void_array *_cert_find_subset_common (const char *filename, netsnmp_container * container)
{
...
static netsnmp_void_array *_find_subset_fp (netsnmp_container * certs, const char *fp)
{
    ...
    netsnmp_assert_or_msgreturn (fp_container, "cert2sn_fp container missing", NULL);
...
#if 0                            /* not used yet */
static netsnmp_key *_key_find_fn (const char *filename)
{
...
static int _time_filter (netsnmp_file * f, struct stat *idx)
{
    ...
    if (f && idx && f->stats && ((f->stats->st_mtime >= idx->st_mtime) || (f->stats->st_ctime >= idx->st_mtime)))
...
static void _init_tlstmCertToTSN (void)
{
    ...
    register_config_handler (NULL, MAP_CONFIG_TOKEN, _parse_map, _purge_config_entries, certSecName_help);
}

netsnmp_cert_map *netsnmp_cert_map_alloc (char *fingerprint, X509 * ocert)
{
...
static void _map_free (netsnmp_cert_map * map, void *context)
{
    netsnmp_cert_map_free (map);
}

static int _map_compare (netsnmp_cert_map * lhs, netsnmp_cert_map * rhs)
{
    netsnmp_assert ((lhs != NULL) && (rhs != NULL));
...
static int _map_fp_compare (netsnmp_cert_map * lhs, netsnmp_cert_map * rhs)
{
    int rc;
...
static int _map_fp_ncompare (netsnmp_cert_map * lhs, netsnmp_cert_map * rhs)
{
    netsnmp_assert ((lhs != NULL) && (rhs != NULL));

    return strncmp (lhs->fingerprint, rhs->fingerprint, strlen (rhs->fingerprint));
}
...
netsnmp_container *netsnmp_cert_map_container_create (int with_fp)
{
    ...
    if (NULL == fp)
    {
        snmp_log (LOG_ERR, "error creating sub-container for tlstmCertToTSNTable\n");
...
netsnmp_cert_map *netsnmp_certToTSN_parse_common (char **line)
{
    ...
    *line = read_config_read_octet_string (*line, (u_char **) & tmp, &len);
    ...
    {
        map->fingerprint = netsnmp_openssl_cert_get_fingerprint (tmpcert->ocert, -1);
    }
...
static void _parse_map (const char *token, char *line)
{
    ...
    {
        netsnmp_cert_map_free (map);
        netsnmp_config_error (MAP_CONFIG_TOKEN ": duplicate priority for certificate map");
    }
}

static int _fill_cert_map (netsnmp_cert_map * cert_map, netsnmp_cert_map * entry)
{
 ...
 */
int netsnmp_cert_get_secname_maps (netsnmp_container * cert_maps)
{
    netsnmp_iterator *itr;
    ...
    for (; cert_map; cert_map = ITERATOR_NEXT (itr))
    {
        ...
        results = _find_subset_fp (netsnmp_cert_map_container (), cert_map->fingerprint);
        if (NULL == results)
        {
        ...
        if (results->size > 1)
        {
            for (j = 1; j < results->size; ++j)
            {
                ...
                new_cert_map = netsnmp_cert_map_alloc (entry->fingerprint, entry->ocert);
                if (NULL == new_cert_map)
                {
                    snmp_log (LOG_ERR, "could not allocate new cert map entry\n");
                    goto fail;
                }
...
static void _init_tlstmParams (void)
{
    const char *params_help = PARAMS_CONFIG_TOKEN " targetParamsName hashType:fingerPrint";
    ...
    if (NULL == _tlstmParams)
        snmp_log (LOG_ERR, "error creating sub-container for tlstmParamsTable\n");
    else
        _tlstmParams->container_name = strdup ("tlstmParams");

    register_config_handler (NULL, PARAMS_CONFIG_TOKEN, _parse_params, NULL, params_help);
}
...
snmpTlstmParams *netsnmp_tlstmParams_create (const char *name, int hashType, const char *fp, int fp_len)
{
...
static char *_find_tlstmParams_fingerprint (const char *name)
{
...
static void _init_tlstmAddr (void)
{
    const char *addr_help = ADDR_CONFIG_TOKEN " targetAddrName hashType:fingerprint serverIdentity";
    ...
    if (NULL == _tlstmAddr)
        snmp_log (LOG_ERR, "error creating sub-container for tlstmAddrTable\n");
    else
        _tlstmAddr->container_name = strdup ("tlstmAddr");

    register_config_handler (NULL, ADDR_CONFIG_TOKEN, _parse_addr, NULL, addr_help);
}
...
netsnmp_tlstmAddr_restore_common (char **line, char *name, size_t * name_len,
                                  char *id, size_t * id_len, char *fp, size_t * fp_len, u_char * ht)
{
...
static void _parse_addr (const char *token, char *line)
{
    ...
    char name[SNMPADMINLENGTH + 1], id[SNMPADMINLENGTH + 1], fingerprint[SNMPTLSFINGERPRINT_MAX_LEN + 1];

    size_t name_len = sizeof (name), id_len = sizeof (id), fp_len = sizeof (fingerprint);

    u_char hashType;

    int rc;

    rc = netsnmp_tlstmAddr_restore_common (&line, name, &name_len, id, &id_len, fingerprint, &fp_len, &hashType);
    if (rc < 0)
        return;
