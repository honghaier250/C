...
extern "C"
{
   ...
   /** header */
    ...
    typedef void (netsnmp_object_monitor_callback) (netsnmp_monitor_callback_header *);
    ...
    int netsnmp_monitor_unregister (oid * object,
                                    size_t oid_len,
                                    int priority, void *watcher_data, netsnmp_object_monitor_callback * cb);

    /*
     * check to see if a registration exists for an object/event combination
     */
    int netsnmp_monitor_check_registered (int event, oid * oid, int oid_len);
     ...
     */
    void netsnmp_notify_cooperative (int event, oid * object, size_t len, char oid_steal, void *object_info);
