...
static int __snprint_value (char *, size_t, netsnmp_variable_list *, struct tree *, int, int);
...
static int __add_var_val_str (netsnmp_pdu *, oid *, int, char *, int, int);
     ...
     struct tree *tp;
{
    ...
    return (tp && (__get_type_str (tp->type, buf) || (tp->parent && __get_type_str (tp->parent->type, buf))));
}

static int __translate_appl_type (typestr)
     char *typestr;
{
...
static int __translate_asn_type (type)
     int type;
{
    switch (type)
    {
...
static int __snprint_value (buf, buf_len, var, tp, type, flag)
     char *buf;
...
static struct tree *__tag2oid (tag, iid, oid_arr, oid_arr_len, type, best_guess)
     char *tag;
     ...
     int best_guess;
{
   ...
   /*********************************************************/
    ...
    else if (strchr (tag, '.') || strchr (tag, ':'))
    {
        ...
        {                        /* make sure it's not a numeric tag */
            newname_len = MAX_OID_LEN;
            if (read_objid (tag, newname, &newname_len))
            {                    /* long name */
                rtp = tp = get_tree (newname, newname_len, get_tree_head ());
            }
        }
 ...
 */
static int __concat_oid_str (doid_arr, doid_arr_len, soid_str)
     oid *doid_arr;
 ...
 */
static int __add_var_val_str (pdu, name, name_length, val, len, type)
     netsnmp_pdu *pdu;
     ...
     int type;
{
    ...
    {
        pdu->variables = vars = (netsnmp_variable_list *) calloc (1, sizeof (netsnmp_variable_list));
    }
    else
    {
        for (vars = pdu->variables; vars->next_variable; vars = vars->next_variable)
             /*EXIT*/;
        vars->next_variable = (netsnmp_variable_list *) calloc (1, sizeof (netsnmp_variable_list));
...
static int __send_sync_pdu (ss, pdu, response, retry_nosuch, err_str, err_num, err_ind)
     netsnmp_session *ss;
     ...
     int *err_ind;
{
  ...
  retry:
    ...
    Py_END_ALLOW_THREADS if ((*response == NULL) && (status == STAT_SUCCESS))
        status = STAT_ERROR;
...
static int py_netsnmp_attr_string (PyObject * obj, char *attr_name, char **val, Py_ssize_t * len)
{
    *val = NULL;
...
static int py_netsnmp_attr_set_string (PyObject * obj, char *attr_name, char *val, size_t len)
{
    int ret = -1;

    if (obj && attr_name)
    {
        PyObject *val_obj = (val ? Py_BuildValue ("s#", val, len) : Py_BuildValue (""));
 ...
 */
static void __py_netsnmp_update_session_errors (PyObject * session, char *err_str, int err_num, int err_ind)
{
...
static PyObject *netsnmp_create_session (PyObject * self, PyObject * args)
{
    ...
    if (!PyArg_ParseTuple (args, "issiii", &version, &community, &peer, &lport, &retries, &timeout))
        return NULL;
...
static PyObject *netsnmp_create_session_v3 (PyObject * self, PyObject * args)
{
    ...
    if (!PyArg_ParseTuple (args, "isiiisisssssssii", &version,
                           ...
                           &auth_proto, &auth_pass, &priv_proto, &priv_pass, &eng_boots, &eng_time))
        return NULL;
    ...
    session.securityEngineIDLen =
        hex_to_binary2 ((unsigned char *) sec_eng_id, STRLEN (sec_eng_id), (char **) &session.securityEngineID);
    session.contextEngineIDLen =
        hex_to_binary2 ((unsigned char *) sec_eng_id, STRLEN (sec_eng_id), (char **) &session.contextEngineID);
    ...
    else if (!strcmp (priv_proto, "DEFAULT"))
    {
        ...
        session.securityPrivProto = snmp_duplicate_objid (p, session.securityPrivProtoLen);
    }
    else
    {
        if (verbose)
...
static PyObject *netsnmp_create_session_tunneled (PyObject * self, PyObject * args)
{
    ...
    if (!PyArg_ParseTuple (args, "isiiisissssss", &version,
                           &peer, &lport, &retries, &timeout,
                           &sec_name, &sec_level,
                           &context_eng_id, &context, &our_identity, &their_identity, &their_hostname, &trust_cert))
        return NULL;
    ...
    if (!session.transport_configuration)
    {
        ...
        session.transport_configuration = netsnmp_container_find ("transport_configuration:fifo");
        if (!session.transport_configuration)
        {
        ...
        session.transport_configuration->compare = (netsnmp_container_compare *) netsnmp_transport_config_compare;
    }

    if (our_identity && our_identity[0] != '\0')
        CONTAINER_INSERT (session.transport_configuration, netsnmp_transport_create_config ("localCert", our_identity));
    ...
    if (trust_cert && trust_cert[0] != '\0')
        CONTAINER_INSERT (session.transport_configuration, netsnmp_transport_create_config ("trust_cert", trust_cert));
    ...
    if (oid_arr && args)
    {
        ...
        if (varlist)
        {
            ...
            while (varlist_iter && (varbind = PyIter_Next (varlist_iter)))
            {
                ...
                {
                    if (verbose)
                        printf ("error: get: unknown object ID (%s)", (tag ? tag : "<null>"));
                    snmp_free_pdu (pdu);
                    goto done;
                }
        ...
        __send_sync_pdu (ss, pdu, &response, retry_nosuch, err_str, &err_num, &err_ind);
        ...
        for (vars = (response ? response->variables : NULL), varlist_ind = 0;
             vars && (varlist_ind < varlist_len); vars = vars->next_variable, varlist_ind++)
        {
            ...
            if (PyObject_HasAttrString (varbind, "tag"))
            {
                ...
                *(str_buf + 1) = '\0';
                ...
                tp = netsnmp_sprint_realloc_objid_tree (&str_bufp, &str_buf_len,
                                                        &out_len, 0, &buf_over, vars->name, vars->name_length);
                if (_debug_level)
                ...
                PyTuple_SetItem (val_tuple, varlist_ind,
                                 (len ? Py_BuildValue ("s#", str_buf, len) : Py_BuildValue ("")));
...
static PyObject *netsnmp_walk (PyObject * self, PyObject * args)
{
    ...
    if (args)
    {
        ...
        while (varlist_iter && (varbind = PyIter_Next (varlist_iter)))
        {
            ...
            {
                tp = __tag2oid (tag, iid, oid_arr[varlist_ind], &oid_arr_len[varlist_ind], NULL, best_guess);
            }
            ...
            {
                if (verbose)
                    printf ("error: walk: unknown object ID (%s)", (tag ? tag : "<null>"));
                snmp_free_pdu (pdu);
                goto done;
            }
            /* release reference when done */
        ...
        for (vars = pdu->variables, varlist_ind = 0; vars != NULL; vars = vars->next_variable, varlist_ind++)
        {

            oid_arr_broken_check[varlist_ind] = calloc (MAX_OID_LEN, sizeof (oid));

            oid_arr_broken_check_len[varlist_ind] = vars->name_length;
            memcpy (oid_arr_broken_check[varlist_ind], vars->name, vars->name_length * sizeof (oid));
        }
        ...
        while (notdone)
        {
            ...
            if (!response || !response->variables || status != STAT_SUCCESS || response->errstat != SNMP_ERR_NOERROR)
            {
                notdone = 0;
            }
            else
            {
                ...
                for (vars = (response ? response->variables : NULL),
                     ...
                     vars = vars->next_variable, varlist_ind++, oldvars = (oldvars ? oldvars->next_variable : NULL))
                {

                    if ((vars->name_length < oid_arr_len[varlist_ind]) ||
                        (memcmp (oid_arr[varlist_ind], vars->name, oid_arr_len[varlist_ind] * sizeof (oid)) != 0))
                    {
                        notdone = 0;
                        break;
                    }
                    ...
                    memcpy (oid_arr_broken_check[varlist_ind], vars->name, sizeof (oid) * vars->name_length);
                    oid_arr_broken_check_len[varlist_ind] = vars->name_length;
...
static PyObject *netsnmp_getbulk (PyObject * self, PyObject * args)
{
    ...
    if (oid_arr && args)
    {
        ...
        if (!PyArg_ParseTuple (args, "OiiO", &session, &nonrepeaters, &maxrepetitions, &varlist))
        {
            goto done;
        }
        ...
        if (varlist && (varbinds = PyObject_GetAttrString (varlist, "varbinds")))
        {
            ...
            if (py_netsnmp_attr_long (session, "UseNumeric"))
            {
                ...
                netsnmp_ds_set_int (NETSNMP_DS_LIBRARY_ID,
                                    NETSNMP_DS_LIB_OID_OUTPUT_FORMAT, NETSNMP_OID_OUTPUT_NUMERIC);
            }
            ...
            if (response && response->variables)
            {
                ...
                for (vars = response->variables, varbind_ind = 0; vars; vars = vars->next_variable, varbind_ind++)
                {
                    ...
                    if (PyObject_HasAttrString (varbind, "tag"))
                    {
                        ...
                        *(str_buf + 1) = '\0';
                        ...
                        PyTuple_SetItem (val_tuple, varbind_ind, Py_BuildValue ("s#", str_buf, len));
    ...
    if (oid_arr && args)
    {
        ...
        if (varlist)
        {
            ...
            while (varlist_iter && (varbind = PyIter_Next (varlist_iter)))
            {
                ...
                {
                    if (verbose)
                        printf ("error: set: unknown object ID (%s)", (tag ? tag : "<null>"));
                    snmp_free_pdu (pdu);
                    goto done;
                }
                ...
                if (type == TYPE_INTEGER && use_enums && tp && tp->enums)
                {
                    for (ep = tp->enums; ep; ep = ep->next)
                    {
                        ...
                        {
                            snprintf ((char *) tmp_val_str, sizeof (tmp_val_str), "%d", ep->value);
                            break;
                        }
                    }
                }
                len = (int) tmplen;
                status = __add_var_val_str (pdu, oid_arr, oid_arr_len, (char *) tmp_val_str, len, type);
        ...
        status = __send_sync_pdu (ss, pdu, &response, NO_RETRY_NOSUCH, err_str, &err_num, &err_ind);
        __py_netsnmp_update_session_errors (session, err_str, err_num, err_ind);
