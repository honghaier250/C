...
int init_kmem (const char *file)
{
    ...
    if (!kd && !netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_NO_ROOT_ACCESS))
    {
...
int klookup (unsigned long off, void *target, size_t siz)
{
    ...
    {
#if HAVE_KVM_OPENFILES
        snmp_log (LOG_ERR, "kvm_read(*, %lx, %p, %zx) = %d: %s\n", off, target, siz, result, kvm_geterr (kd));
#else
        snmp_log (LOG_ERR, "kvm_read(*, %lx, %p, %d) = %d: ", off, target, siz, result);
 ...
 */
static off_t klseek (off_t base)
{
 ...
 */
static int klread (char *buf, int buflen)
{
