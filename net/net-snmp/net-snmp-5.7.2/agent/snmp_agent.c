...
netsnmp_feature_child_of (addrcache_age, netsnmp_unused) netsnmp_feature_child_of (delete_subtree_cache, netsnmp_unused)
...
NETSNMP_STATIC_INLINE void _reorder_getbulk (netsnmp_agent_session * asp)
{
    ...
    for (i = 0; i < r - 1; i++)
    {
        for (j = 0; j < repeats; j++)
        {
            asp->bulkcache[i * repeats + j]->next_variable = asp->bulkcache[(i + 1) * repeats + j];
        }
    }
    ...
    {
        asp->bulkcache[(r - 1) * repeats + j]->next_variable = asp->bulkcache[j + 1];
    }
    ...
    for (i = 0; i < repeats; i++)
    {
        ...
        {
            all_eoMib = 1;
            for (j = 1, prev = asp->bulkcache[i]; j < r; j++, prev = prev->next_variable)
            {
...
int netsnmp_addrcache_add (const char *addr)
{
        ...
        {                        /* If used */
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (oldest < 0)
                        oldest = i;
                    else if (timercmp (&addrCache[i].lastHitM, &addrCache[oldest].lastHitM, <))
                        oldest = i;
                }                /* fresh */
            }                    /* used, no match */
        }                        /* used */
    ...
    if ((-1 == rc) && (NULL != addr))
    {
        ...
        {                        /* Otherwise, replace oldest entry */
            if (netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_VERBOSE))
                snmp_log (LOG_INFO, "Purging address from address cache: %s", addrCache[oldest].addr);
    ...
    if ((log_addresses && (1 == rc)) || netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_VERBOSE))
    {
...
netsnmp_agent_check_packet (netsnmp_session * session,
                            netsnmp_transport * transport, void *transport_data, int transport_data_length)
{
...
int netsnmp_agent_check_parse (netsnmp_session * session, netsnmp_pdu * pdu, int result)
{
    if (result == 0)
    {
        if (snmp_get_do_logging () && netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_VERBOSE))
        {
            ...
            for (var_ptr = pdu->variables; var_ptr != NULL; var_ptr = var_ptr->next_variable)
            {
                ...
                if (c_oid)
                {
                    if (!sprint_realloc_objid (&c_oid, &c_oidlen, &c_outlen, 1, var_ptr->name, var_ptr->name_length))
                    {
...
int netsnmp_register_agent_nsap (netsnmp_transport * t)
{
    ...
    s->flags = netsnmp_ds_get_int (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_FLAGS);
    ...
    if (netsnmp_sess_config_transport (s->transport_configuration, t) != SNMPERR_SUCCESS)
    {
        SNMP_FREE (s);
        SNMP_FREE (n);
        return -1;
...
void netsnmp_deregister_agent_nsap (int handle)
{
    ...
    if (main_session_deregistered)
    {
        ...
        {
            DEBUGMSGTL (("snmp_agent", "WARNING: main_session ptr changed from %p to NULL\n", main_session));
            main_session = NULL;
        }
    }
}
...
int init_master_agent (void)
{
     ...
     */
    cptr = netsnmp_ds_get_string (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_PORTS);
    ...
    do
    {
        ...
        {
            DEBUGMSGTL (("snmp_agent", "init_master_agent; pseudo-transport \"none\" " "requested\n"));
            break;
        }
        ...
        {
            snmp_log (LOG_ERR, "Error registering specified transport \"%s\" as an " "agent NSAP\n", cptr);
            return 1;
        }
        else
        {
            DEBUGMSGTL (("snmp_agent", "init_master_agent; \"%s\" registered as an agent " "NSAP\n", cptr));
        }
    }
    while (st && *st != '\0');
    SNMP_FREE (buf);
...
#ifdef USING_AGENTX_MASTER_MODULE
    if (netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_AGENTX_MASTER) == 1)
        real_init_master ();
#endif
#ifdef USING_SMUX_MODULE
    if (should_init ("smux"))
...
{
    netsnmp_agent_session *asp = (netsnmp_agent_session *) calloc (1, sizeof (netsnmp_agent_session));
...
int netsnmp_check_for_delegated (netsnmp_agent_session * asp)
{
    ...
    {
        for (request = asp->treecache[i].requests_begin; request; request = request->next)
        {
...
int netsnmp_remove_delegated_requests_for_session (netsnmp_session * sess)
{
    ...
    if (count)
    {
        DEBUGMSGTL (("snmp_agent", "removed %d delegated request(s) for session " "%8p\n", count, sess));
        netsnmp_check_outstanding_agent_requests ();
    }
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
        if ((asp->pdu->command == SNMP_MSG_SET) && (asp->pdu->version == SNMP_VERSION_1))
        {
            switch (asp->status)
            {
...
#define INCLUDE_V2ERRORS_IN_V1STATS
    ...
    if ((status == SNMP_ERR_NOERROR) && (asp->pdu))
    {
        ...
        snmp_increment_statistic_by ((asp->pdu->command == SNMP_MSG_SET ?
                                      ...
                                      STAT_SNMPINTOTALREQVARS), count_varbinds (asp->pdu->variables));
#else                            /* NETSNMP_NO_WRITE_SUPPORT */
        snmp_increment_statistic_by (STAT_SNMPINTOTALREQVARS, count_varbinds (asp->pdu->variables));
#endif                            /* NETSNMP_NO_WRITE_SUPPORT */
    ...
    if (asp->pdu)
    {
        ...
        if (!snmp_send (asp->session, asp->pdu) && asp->session->s_snmp_errno != SNMPERR_SUCCESS)
        {
            netsnmp_variable_list *var_ptr;

            snmp_perror ("send response");
            for (var_ptr = asp->pdu->variables; var_ptr != NULL; var_ptr = var_ptr->next_variable)
            {
                size_t c_oidlen = 256, c_outlen = 0;
...
int handle_snmp_packet (int op, netsnmp_session * session, int reqid, netsnmp_pdu * pdu, void *magic)
{
     ...
     */
    if (pdu->version == SNMP_VERSION_3 && session->s_snmp_errno == SNMPERR_USM_AUTHENTICATIONFAILURE)
    {
...
netsnmp_request_info *netsnmp_add_varbind_to_cache (netsnmp_agent_session * asp, int vbcount,
                                                    netsnmp_variable_list * varbind_ptr, netsnmp_subtree * tp)
{
    ...
    if (tp && (asp->pdu->command == SNMP_MSG_GETNEXT || asp->pdu->command == SNMP_MSG_GETBULK))
    {
        int result;

        int prefix_len;

        prefix_len = netsnmp_oid_find_prefix (tp->start_a, tp->start_len, tp->end_a, tp->end_len);
        if (prefix_len < 1)
        {
        ...
        {
            result = netsnmp_acm_check_subtree (asp->pdu, tp->start_a, prefix_len);
        }
    ...
    else
    {
        ...
        if (tp->global_cacheid)
        {
             ...
             */
            if (asp->cache_store && -1 != (cacheid = netsnmp_get_local_cachid (asp->cache_store, tp->global_cacheid)))
            {
            }
            else
            {
                cacheid = ++(asp->treecache_num);
                netsnmp_get_or_add_local_cachid (&asp->cache_store, tp->global_cacheid, cacheid);
...
#define CACHE_GROW_SIZE 16
                ...
                asp->treecache =
                    (netsnmp_tree_cache *) realloc (asp->treecache, sizeof (netsnmp_tree_cache) * asp->treecache_len);
                if (asp->treecache == NULL)
                    return NULL;
                memset (&(asp->treecache[cacheid]), 0x00, sizeof (netsnmp_tree_cache) * (CACHE_GROW_SIZE));
            }
         ...
         */
        if (asp->pdu->command == SNMP_MSG_GETNEXT || asp->pdu->command == SNMP_MSG_GETBULK)
        {
...
int check_acm (netsnmp_agent_session * asp, u_char type)
{
    ...
    for (i = 0; i <= asp->treecache_num; i++)
    {
        for (request = asp->treecache[i].requests_begin; request; request = request->next)
        {
             ...
             */
            earliest = 0;
            for (j = request->repeat, vb = request->requestvb_start; vb && j > -1; j--, vb = vb->next_variable)
            {
                    ...
                    if (view != VACM_SUCCESS)
                    {
                        ...
                        if (request->repeat < request->orig_repeat)
                        {
                            ...
                            if (j > -1 && vb->next_variable &&
                                vb->next_variable->type != ASN_NULL && vb->next_variable->type != ASN_PRIV_RETRY)
                            {
                                for (k = j, vbc = vb, vb2 = vb->next_variable;
                                     k > -2 && vbc && vb2; k--, vbc = vb2, vb2 = vb2->next_variable)
                                {
...
int netsnmp_create_subtree_cache (netsnmp_agent_session * asp)
{
    ...
    if (asp->treecache == NULL && asp->treecache_len == 0)
    {
        ...
        asp->treecache = (netsnmp_tree_cache *) calloc (asp->treecache_len, sizeof (netsnmp_tree_cache));
        if (asp->treecache == NULL)
            return SNMP_ERR_GENERR;
    }
    asp->treecache_num = -1;
    ...
    if (asp->pdu->command == SNMP_MSG_GETBULK)
    {
        ...
        {
            int maxbulk = netsnmp_ds_get_int (NETSNMP_DS_APPLICATION_ID,
                                              NETSNMP_DS_AGENT_MAX_GETBULKREPEATS);

            int maxresponses = netsnmp_ds_get_int (NETSNMP_DS_APPLICATION_ID,
            ...
            if (asp->pdu->errindex > maxbulk)
            {
                ...
                DEBUGMSGTL (("snmp_agent", "truncating number of getbulk repeats to %ld\n", asp->pdu->errindex));
            }

            asp->bulkcache =
                (netsnmp_variable_list **) malloc ((n + asp->pdu->errindex * r) * sizeof (struct varbind_list *));
    ...
    for (varbind_ptr = asp->pdu->variables; varbind_ptr; varbind_ptr = vbsave)
    {
        ...
        if (asp->pdu->command == SNMP_MSG_GETBULK)
        {
            ...
            else
            {
                ...
                if (asp->pdu->errindex > 0)
                {
                    ...
                    {
                        vbptr->next_variable = SNMP_MALLOC_STRUCT (variable_list);
                        /*
         ...
         */
        tp = netsnmp_subtree_find (varbind_ptr->name, varbind_ptr->name_length, NULL, asp->pdu->contextName);
        ...
        {
            case SNMP_MSG_GET:
                view = in_a_view (varbind_ptr->name, &varbind_ptr->name_length, asp->pdu, varbind_ptr->type);
                if (view != VACM_SUCCESS)
        ...
        if (view == VACM_SUCCESS)
        {
            request = netsnmp_add_varbind_to_cache (asp, vbcount, varbind_ptr, tp);
    ...
    for (i = 0; i < asp->vbcount; i++)
    {
        ...
        if (asp->requests[i].requestvb->type == ASN_NULL)
        {
            if (!netsnmp_add_varbind_to_cache (asp, asp->requests[i].index,
                                               asp->requests[i].requestvb, asp->requests[i].subtree->next))
            {
        ...
        else if (asp->requests[i].requestvb->type == ASN_PRIV_RETRY)
        {
            ...
            if (!netsnmp_add_varbind_to_cache (asp, asp->requests[i].index,
                                               asp->requests[i].requestvb, asp->requests[i].subtree))
            {
                SNMP_FREE (old_treecache);
            }
        }
...
int netsnmp_check_all_requests_error (netsnmp_agent_session * asp, int look_for_specific)
{
    ...
    for (i = 0; i < asp->vbcount; ++i)
    {
        if ((SNMP_ERR_NOERROR != asp->requests[i].status) &&
            (!look_for_specific || asp->requests[i].status == look_for_specific))
...
int netsnmp_check_requests_status (netsnmp_agent_session * asp, netsnmp_request_info * requests, int look_for_specific)
{
    ...
    while (requests)
    {
        ...
        if (requests->status != SNMP_ERR_NOERROR &&
            ...
            && (look_for_specific || asp->index == 0 || requests->index < asp->index))
        {
...
int netsnmp_check_all_requests_status (netsnmp_agent_session * asp, int look_for_specific)
{
    ...
    for (i = 0; i <= asp->treecache_num; i++)
    {
        netsnmp_check_requests_status (asp, asp->treecache[i].requests_begin, look_for_specific);
    }
    return asp->status;
}
...
{
    int i, retstatus = SNMP_ERR_NOERROR, status = SNMP_ERR_NOERROR, final_status = SNMP_ERR_NOERROR;
    ...
    for (i = 0; i <= asp->treecache_num; i++)
    {
        ...
        if (NULL != asp->treecache[i].subtree->reginfo)
        {
            ...
            status = netsnmp_call_handlers (reginfo, asp->reqinfo, asp->treecache[i].requests_begin);
        }
        else
            status = SNMP_ERR_GENERR;
        ...
        switch (asp->mode)
        {
            ...
            case MODE_SET_COMMIT:
                retstatus = netsnmp_check_requests_status (asp,
                                                           asp->treecache[i].requests_begin, SNMP_ERR_COMMITFAILED);
                break;

            case MODE_SET_UNDO:
                retstatus = netsnmp_check_requests_status (asp, asp->treecache[i].requests_begin, SNMP_ERR_UNDOFAILED);
                break;
#endif                            /* NETSNMP_NO_WRITE_SUPPORT */

            default:
                retstatus = netsnmp_check_requests_status (asp, asp->treecache[i].requests_begin, 0);
                break;
        }
...
void netsnmp_check_outstanding_agent_requests (void)
{
    ...
    while (netsnmp_agent_queued_list)
    {
         ...
         */
        netsnmp_assert ((!netsnmp_processing_set) || (netsnmp_processing_set == netsnmp_agent_queued_list));
         ...
         */
#ifndef NETSNMP_NO_WRITE_SUPPORT
        if ((netsnmp_agent_queued_list->pdu->command == SNMP_MSG_SET) && (agent_delegated_list))
        {
        ...
        DEBUGMSGTL (("snmp_agent", "processing queued request, asp = %8p\n", asp));
...
int check_getnext_results (netsnmp_agent_session * asp)
{
    ...
    {
        for (request = old_treecache[i].requests_begin; request; request = request->next)
        {
            ...
            if (special)
            {
                if (!request->inclusive)
                {
                    ...
                    snmp_set_var_typed_value (request->requestvb, ASN_PRIV_RETRY, NULL, 0);
                }
            ...
            if (snmp_oid_compare (request->requestvb->name,
                                  request->requestvb->name_length, request->range_end, request->range_end_len) >= 0)
            {
                /*
                 ...
                 */
                DEBUGMSGTL (("check_getnext_results", "request response %d out of range\n", request->index));
                /*
                 ...
                 */
                snmp_set_var_objid (request->requestvb, request->range_end, request->range_end_len);
...
int handle_getnext_loop (netsnmp_agent_session * asp)
{
    ...
    while (netsnmp_running)
    {
        ...
        DEBUGIF ("results")
        {
            ...
            for (var_ptr = asp->pdu->variables; var_ptr; var_ptr = var_ptr->next_variable)
            {
                DEBUGMSGTL (("results", "\t"));
...
int netsnmp_handle_request (netsnmp_agent_session * asp, int status)
{
     ...
     */
    if ((0 == netsnmp_check_delegated_chain_for (asp)) && (asp != netsnmp_processing_set))
    {
        /*
        ...
        if (netsnmp_processing_set)
        {
            ...
            DEBUGMSGTL (("snmp_agent", "request queued while processing set, " "asp = %8p\n", asp));
            return 1;
        }
        ...
        if (asp->pdu->command == SNMP_MSG_SET)
        {
            ...
            {
                DEBUGMSGTL (("snmp_agent", "SET request queued while " "delegated requests finish, asp = %8p\n", asp));
                netsnmp_add_queued (asp);
                return 1;
            }
        }
...
int handle_pdu (netsnmp_agent_session * asp)
{
    ...
    switch (asp->mode)
    {
        case SNMP_MSG_GET:
             ...
             */
            if (status == SNMP_ERR_NOERROR)
                snmp_replace_var_types (asp->pdu->variables, ASN_NULL, SNMP_NOSUCHINSTANCE);
            break;
...
int netsnmp_request_set_error (netsnmp_request_info * request, int error_value)
{
    ...
    return _request_set_error (request, request->agent_req_info->mode, error_value);
}
...
int
netsnmp_set_all_requests_error (netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests, int error_value)
{
