...
netsnmp_feature_child_of (agent_index_all, libnetsnmpagent) netsnmp_feature_child_of (remove_index, agent_index_all)
    /*
...
netsnmp_variable_list *register_oid_index (oid * name, size_t name_len, oid * value, size_t value_len)
{
...
netsnmp_variable_list *register_index (netsnmp_variable_list * varbind, int flags, netsnmp_session * ss)
{
    ...
    res = 1;
    res2 = 1;
    for (idxptr = snmp_index_head; idxptr != NULL; prev_oid_ptr = idxptr, idxptr = idxptr->next_oid)
    {
        if ((res = snmp_oid_compare (varbind->name, varbind->name_length,
                                     idxptr->varbind->name, idxptr->varbind->name_length)) <= 0)
            break;
    }
    ...
    if (res == 0 && idxptr)
    {
        ...
        {
            for (idxptr2 = idxptr; idxptr2 != NULL; prev_idx_ptr = idxptr2, idxptr2 = idxptr2->next_idx)
            {
                    ...
                    case ASN_OBJECT_ID:
                        res2 =
                            snmp_oid_compare (varbind->val.objid,
                                              ...
                                              idxptr2->varbind->val.objid, idxptr2->varbind->val_len / sizeof (oid));
                        break;
                    default:
                        return NULL;    /* wrong type */
                }
    ...
    if (NULL == snmp_varlist_add_variable (&new_index->varbind,
                                           varbind->name,
                                           varbind->name_length, varbind->type, varbind->val.string, varbind->val_len))
    {
        /*
            ...
            case ASN_OBJECT_ID:
                ...
                else
                {
                    ...
                    if ((varbind->name_length + 1) * sizeof (oid) <= 40)
                    {
                        ...
                        new_index->varbind->val_len = (varbind->name_length + 1) * sizeof (oid);
                    }
                    else
                    {
                        /*
...
int unregister_index (netsnmp_variable_list * varbind, int remember, netsnmp_session * ss)
{
    struct snmp_index *idxptr, *idxptr2;
...
int unregister_oid_index (oid * name, size_t name_len, oid * value, size_t value_len)
{
    netsnmp_variable_list varbind;
...
void dump_idx_registry (void)
{
    ...
    {
        sout_len = 0;
        if (sprint_realloc_objid (&sbuf, &sbuf_len, &sout_len, 1, idxptr->varbind->name, idxptr->varbind->name_length))
        {
            printf ("%s indexes:\n", sbuf);
        }
        else
            ...
            switch (idxptr2->varbind->type)
            {
                case ASN_INTEGER:
                    printf ("    %ld for session %8p, allocated %d\n",
                            *idxptr2->varbind->val.integer, idxptr2->session, idxptr2->allocated);
                    break;
                case ASN_OCTET_STR:
                    printf ("    \"%s\" for session %8p, allocated %d\n",
                            idxptr2->varbind->val.string, idxptr2->session, idxptr2->allocated);
                    break;
                case ASN_OBJECT_ID:
                    eout_len = 0;
     ...
     unsigned long count_indexes (oid * name, size_t namelen, int include_unallocated)
{
    ...
    {
        if (netsnmp_oid_equals (name, namelen, i->varbind->name, i->varbind->name_length) == 0)
        {
