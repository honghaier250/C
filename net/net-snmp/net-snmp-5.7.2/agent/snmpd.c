...
netsnmp_feature_want (logging_file) netsnmp_feature_want (logging_stdio) netsnmp_feature_want (logging_syslog)
...
#ifdef SIGHUP
RETSIGTYPE SnmpdReconfig (int a)
{
    reconfig = 1;
...
RETSIGTYPE SnmpdCatchRandomSignal (int a)
{
...
#endif
{
...
#endif
#endif                            /* NETSNMP_FEATURE_REMOVE_LOGGING_SYSLOG */
    netsnmp_ds_set_string (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_APPTYPE, app_name);
    ...
    while ((arg = getopt (argc, argv, options)) != EOF)
    {
        switch (arg)
        {
            ...
            case 'd':
                netsnmp_ds_set_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DUMP_PACKET, ++snmp_dump_packet);
                break;
            ...
            case 'l':
                ...
                if (optarg != NULL)
                {
                    ...
                    {
                        fprintf (stderr, "%s: logfile path too long (limit %d chars)\n", argv[0], PATH_MAX);
                        exit (1);
                    }
            ...
            case 'U':
                netsnmp_ds_toggle_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_LEAVE_PIDFILE);
                break;
            ...
            case 'x':
                ...
                netsnmp_ds_set_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_AGENTX_MASTER, 1);
                break;
#endif
    ...
    {
        netsnmp_ds_set_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_ROLE, agent_mode);
    }
    ...
    if (!netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_LEAVE_PIDFILE) && (pid_file != NULL))
    {
        unlink (pid_file);
    }
#ifdef WIN32SERVICE
...
static int receive (void)
{
    ...
    while (netsnmp_running)
    {
      ...
      reselect:
        ...
        if (tvp)
            DEBUGMSGTL (("timer", "tvp %ld.%ld\n", (long) tvp->tv_sec, (long) tvp->tv_usec));
...
#ifdef IFDESCR
        _T
        ("SNMPv2c / SNMPv3 command responder from Net-SNMP. Supports MIB objects for IP,ICMP,TCP,UDP, and network interface sub-layers.");
#else
