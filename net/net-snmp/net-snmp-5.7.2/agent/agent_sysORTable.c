...
static void erase (data_node entry)
{
    ...
    snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_UNREG_SYSOR, &entry->data);
...
void netsnmp_sysORTable_foreach (void (*f) (const struct sysORTable *, void *), void *c)
{
...
int register_sysORTable_sess (oid * oidin, size_t oidlen, const char *descr, netsnmp_session * ss)
{
    data_node entry;
    ...
    snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_REG_SYSOR, &entry->data);
...
int unregister_sysORTable_sess (oid * oidin, size_t oidlen, netsnmp_session * ss)
{
    ...
    if (table)
    {
        ...
        do
        {
            ...
            run = run->next;
            if (tmp->data.OR_sess == ss && snmp_oid_compare (oidin, oidlen, tmp->data.OR_oid, tmp->data.OR_oidlen) == 0)
            {
                erase (tmp);
                any_unregistered = 1;
            }
...
{
    DEBUGMSGTL (("agent/sysORTable", "sysORTable unregistering session %p\n", ss));
    ...
    if (table)
    {
        ...
        do
        {
            ...
            if (((ss->flags & SNMP_FLAGS_SUBSESSION) &&
                 tmp->data.OR_sess == ss) ||
                (!(ss->flags & SNMP_FLAGS_SUBSESSION) && tmp->data.OR_sess && tmp->data.OR_sess->subsession == ss))
                erase (tmp);
        }
        while (table && run != table);
    }

    DEBUGMSGTL (("agent/sysORTable", "sysORTable unregistering session %p done\n", ss));
}

static int register_sysOR_callback (int majorID, int minorID, void *serverarg, void *clientarg)
{
    struct sysORTable *parms = (struct sysORTable *) serverarg;

    return register_sysORTable_sess (parms->OR_oid, parms->OR_oidlen, parms->OR_descr, parms->OR_sess);
}

static int unregister_sysOR_by_session_callback (int majorID, int minorID, void *serverarg, void *clientarg)
{
...
static int unregister_sysOR_callback (int majorID, int minorID, void *serverarg, void *clientarg)
{
    struct sysORTable *parms = (struct sysORTable *) serverarg;

    return unregister_sysORTable_sess (parms->OR_oid, parms->OR_oidlen, parms->OR_sess);
}
...
void init_agent_sysORTable (void)
{
    ...
    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_REQ_REG_SYSOR, register_sysOR_callback, NULL);
    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_REQ_UNREG_SYSOR, unregister_sysOR_callback, NULL);
    snmp_register_callback (SNMP_CALLBACK_APPLICATION,
                            SNMPD_CALLBACK_REQ_UNREG_SYSOR_SESS, unregister_sysOR_by_session_callback, NULL);
}
