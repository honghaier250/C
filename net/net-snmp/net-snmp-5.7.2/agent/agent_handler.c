                                                                   ...
                                                                   size_t reg_oid_len, int modes)
{
    ...
    if (handler)
    {
        rv = netsnmp_handler_registration_create (name, handler, reg_oid, reg_oid_len, modes);
        if (!rv)
...
int netsnmp_register_handler (netsnmp_handler_registration * reginfo)
{
    ...
    return netsnmp_register_mib (reginfo->handlerName,
                                 ...
                                 reginfo->contextName, reginfo->timeout, flags, reginfo, 1);
}
...
int netsnmp_unregister_handler (netsnmp_handler_registration * reginfo)
{
    return unregister_mib_context (reginfo->rootoid, reginfo->rootoid_len,
                                   reginfo->priority,
                                   reginfo->range_subid, reginfo->range_ubound, reginfo->contextName);
}
...
int netsnmp_register_handler_nocallback (netsnmp_handler_registration * reginfo)
{
    ...
    return netsnmp_register_mib (reginfo->handler->handler_name,
                                 ...
                                 reginfo->priority,
                                 reginfo->range_subid,
                                 reginfo->range_ubound, NULL, reginfo->contextName, reginfo->timeout, 0, reginfo, 0);
}
                               ...
                               netsnmp_mib_handler * handler, const char *before_what)
{
    ...
    if (before_what)
    {
        ...
        for (nexth = reginfo->handler; nexth; prevh = nexth, nexth = nexth->next)
        {
                      ...
                      netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    do
    {
        ...
        if (!nh)
        {
            ...
            {
                snmp_log (LOG_ERR, "no access method specified in handler %s.", next_handler->handler_name);
                return SNMP_ERR_GENERR;
            }
            /*
        ...
        DEBUGMSGTL (("handler:calling", "calling handler %s for mode %s\n",
                     next_handler->handler_name, se_find_label_in_slist ("agent_mode", reqinfo->mode)));
...
netsnmp_handler_registration *netsnmp_handler_registration_dup (netsnmp_handler_registration * reginfo)
{
    ...
    if (r != NULL)
    {
        ...
        {
            r->rootoid = snmp_duplicate_objid (reginfo->rootoid, reginfo->rootoid_len);
            if (r->rootoid == NULL)
            {
                netsnmp_handler_registration_free (r);
                return NULL;
...
{
    netsnmp_add_list_data (&handler_reg, netsnmp_create_data_list (name, (void *) handler, handler_free_callback));
                                     ...
                                     netsnmp_mib_handler * handler, const char *before_what)
{
    ...
    for (tptr = tp; tptr != NULL; tptr = tptr->next)
    {
        ...
        if (strcmp (tptr->label_a, name) == 0)
        {
            ...
            netsnmp_inject_handler_before (tptr->reginfo, _clone_handler (handler), before_what);
        }
        else if (tptr->reginfo != NULL &&
                 tptr->reginfo->handlerName != NULL && strcmp (tptr->reginfo->handlerName, name) == 0)
        {
        ...
        else
        {
            for (mh = tptr->reginfo->handler; mh != NULL; mh = mh->next)
            {
                ...
                {
                    DEBUGMSGTL (("injectHandler", "not injecting handler into %s\n", mh->handler_name));
                }
            }
        }
    }
...
void parse_injectHandler_conf (const char *token, char *cptr)
{
    ...
    if (!handler)
    {
        netsnmp_config_error ("no \"%s\" handler registered.", handler_to_insert);
        return;
    }
    ...
    for (stc = get_top_context_cache (); stc; stc = stc->next)
    {
        ...
        netsnmp_inject_handler_into_subtree (stc->first_subtree, reg_name, handler, cptr);
    }
}
 ...
 */
static int handler_mark_inject_handler_done (int majorID, int minorID, void *serverarg, void *clientarg)
{
    doneit = 1;
    return 0;
}
...
void netsnmp_init_handler_conf (void)
{
    snmpd_register_config_handler ("injectHandler",
                                   parse_injectHandler_conf, NULL, "injectHandler NAME INTONAME [BEFORE_OTHER_NAME]");
    snmp_register_callback (SNMP_CALLBACK_LIBRARY,
                            SNMP_CALLBACK_POST_READ_CONFIG, handler_mark_inject_handler_done, NULL);
    ...
    se_add_pair_to_slist ("agent_mode", strdup ("SET_RESERVE1"), MODE_SET_RESERVE1);
    se_add_pair_to_slist ("agent_mode", strdup ("SET_RESERVE2"), MODE_SET_RESERVE2);
    ...
    se_add_pair_to_slist ("babystep_mode", strdup ("pre-request"), MODE_BSTEP_PRE_REQUEST);
    se_add_pair_to_slist ("babystep_mode", strdup ("object_lookup"), MODE_BSTEP_OBJECT_LOOKUP);
    se_add_pair_to_slist ("babystep_mode", strdup ("check_value"), MODE_BSTEP_CHECK_VALUE);
    se_add_pair_to_slist ("babystep_mode", strdup ("row_create"), MODE_BSTEP_ROW_CREATE);
    se_add_pair_to_slist ("babystep_mode", strdup ("undo_setup"), MODE_BSTEP_UNDO_SETUP);
    se_add_pair_to_slist ("babystep_mode", strdup ("set_value"), MODE_BSTEP_SET_VALUE);
    se_add_pair_to_slist ("babystep_mode", strdup ("check_consistency"), MODE_BSTEP_CHECK_CONSISTENCY);
    se_add_pair_to_slist ("babystep_mode", strdup ("undo_set"), MODE_BSTEP_UNDO_SET);
    se_add_pair_to_slist ("babystep_mode", strdup ("commit"), MODE_BSTEP_COMMIT);
    se_add_pair_to_slist ("babystep_mode", strdup ("undo_commit"), MODE_BSTEP_UNDO_COMMIT);
    se_add_pair_to_slist ("babystep_mode", strdup ("irreversible_commit"), MODE_BSTEP_IRREVERSIBLE_COMMIT);
    se_add_pair_to_slist ("babystep_mode", strdup ("undo_cleanup"), MODE_BSTEP_UNDO_CLEANUP);
    se_add_pair_to_slist ("babystep_mode", strdup ("post_request"), MODE_BSTEP_POST_REQUEST);
     ...
     *          I'm betting not...
     */
    se_add_pair_to_slist ("handler_can_mode", strdup ("GET/GETNEXT"), HANDLER_CAN_GETANDGETNEXT);
    se_add_pair_to_slist ("handler_can_mode", strdup ("SET"), HANDLER_CAN_SET);
    se_add_pair_to_slist ("handler_can_mode", strdup ("GETBULK"), HANDLER_CAN_GETBULK);
    se_add_pair_to_slist ("handler_can_mode", strdup ("BABY_STEP"), HANDLER_CAN_BABY_STEP);
}
