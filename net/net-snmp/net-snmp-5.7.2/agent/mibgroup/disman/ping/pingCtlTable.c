...
oid pingCtlTable_variables_oid[] = { 1, 3, 6, 1, 2, 1, 80, 1, 2 };

static const int pingCtlTable_variables_oid_len =
    sizeof (pingCtlTable_variables_oid) / sizeof (pingCtlTable_variables_oid[0]);
...
void init_pingCtlTable (void)
{
     ...
     */
    REGISTER_MIB ("pingCtlTable", pingCtlTable_variables, variable2, pingCtlTable_variables_oid);


    /*
     * register our config handler(s) to deal with registrations 
     */
    snmpd_register_config_handler ("pingCtlTable", parse_pingCtlTable, NULL, NULL);

    /*
     * we need to be called back later to store our data 
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_pingCtlTable, NULL);
...
{
    snmp_unregister_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_pingCtlTable, NULL, 1);
...
struct pingCtlTable_data *create_pingCtlTable_data (void)
{
 ...
 */
int pingCtlTable_add (struct pingCtlTable_data *thedata)
{
    ...
    snmp_varlist_add_variable (&vars, NULL, 0, ASN_OCTET_STR,
                               thedata->pingCtlOwnerIndex, thedata->pingCtlOwnerIndexLen);
    snmp_varlist_add_variable (&vars, NULL, 0, ASN_OCTET_STR, thedata->pingCtlTestName, thedata->pingCtlTestNameLen);
...
int pingResultsTable_add (struct pingCtlTable_data *thedata)
{
    ...
    if (thedata->pingResults != NULL)
    {
        ...
        snmp_varlist_add_variable (&vars_list, NULL, 0, ASN_OCTET_STR, p->pingCtlOwnerIndex, p->pingCtlOwnerIndexLen);
        snmp_varlist_add_variable (&vars_list, NULL, 0, ASN_OCTET_STR, p->pingCtlTestName, p->pingCtlTestNameLen);
...
int pingProbeHistoryTable_add (struct pingProbeHistoryTable_data *thedata)
{
    ...
    if (thedata != NULL)
    {
        snmp_varlist_add_variable (&vars_list, NULL, 0, ASN_OCTET_STR,
                                   thedata->pingCtlOwnerIndex, thedata->pingCtlOwnerIndexLen);
        snmp_varlist_add_variable (&vars_list, NULL, 0, ASN_OCTET_STR,
                                   thedata->pingCtlTestName, thedata->pingCtlTestNameLen);
        snmp_varlist_add_variable (&vars_list, NULL, 0, ASN_UNSIGNED,
                                   &thedata->pingProbeHistoryIndex, sizeof (thedata->pingProbeHistoryIndex));
        ...
        header_complex_add_data (&pingProbeHistoryTableStorage, vars_list, thedata);
...
int pingProbeHistoryTable_addall (struct pingCtlTable_data *thedata)
{
    netsnmp_variable_list *vars_list;
    ...
    for (p = thedata->pingProbeHis; p; p = p->next)
    {
        ...
        snmp_varlist_add_variable (&vars_list, NULL, 0, ASN_OCTET_STR, p->pingCtlTestName, p->pingCtlTestNameLen);
        snmp_varlist_add_variable (&vars_list, NULL, 0, ASN_UNSIGNED,
                                   &p->pingProbeHistoryIndex, sizeof (p->pingProbeHistoryIndex));
 ...
 */
void parse_pingCtlTable (const char *token, char *line)
{
    size_t tmpint;

    struct pingCtlTable_data *StorageTmp = SNMP_MALLOC_STRUCT (pingCtlTable_data);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line, &StorageTmp->pingCtlOwnerIndex, &StorageTmp->pingCtlOwnerIndexLen);
    ...
    line = read_config_read_data (ASN_OCTET_STR, line, &StorageTmp->pingCtlTestName, &StorageTmp->pingCtlTestNameLen);
    ...
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->pingCtlTargetAddressType, &tmpint);

    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->pingCtlTargetAddress, &StorageTmp->pingCtlTargetAddressLen);
    ...
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingCtlDataSize, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingCtlTimeOut, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingCtlProbeCount, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->pingCtlAdminStatus, &tmpint);

    line = read_config_read_data (ASN_OCTET_STR, line, &StorageTmp->pingCtlDataFill, &StorageTmp->pingCtlDataFillLen);
    ...
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingCtlFrequency, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingCtlMaxRows, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->pingCtlStorageType, &tmpint);

    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->pingCtlTrapGeneration, &StorageTmp->pingCtlTrapGenerationLen);
    ...
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingCtlTrapProbeFailureFilter, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingCtlTrapTestFailureFilter, &tmpint);

    line = read_config_read_data (ASN_OBJECT_ID, line, &StorageTmp->pingCtlType, &StorageTmp->pingCtlTypeLen);
    ...
    line = read_config_read_data (ASN_OCTET_STR, line, &StorageTmp->pingCtlDescr, &StorageTmp->pingCtlDescrLen);
    ...
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->pingCtlSourceAddressType, &tmpint);

    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->pingCtlSourceAddress, &StorageTmp->pingCtlSourceAddressLen);
    ...
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->pingCtlIfIndex, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->pingCtlByPassRouteTable, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingCtlDSField, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->pingCtlRowStatus, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingProbeHistoryMaxIndex, &tmpint);
 ...
 */
int store_pingCtlTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (hcindex = pingCtlTableStorage; hcindex != NULL; hcindex = hcindex->next)
    {
        ...
        if (StorageTmp->storageType != ST_READONLY)
        {
            ...
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->pingCtlOwnerIndex, &StorageTmp->pingCtlOwnerIndexLen);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->pingCtlTestName, &StorageTmp->pingCtlTestNameLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->pingCtlTargetAddressType, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->pingCtlTargetAddress, &StorageTmp->pingCtlTargetAddressLen);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingCtlDataSize, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingCtlTimeOut, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingCtlProbeCount, &tmpint);

            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->pingCtlAdminStatus, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->pingCtlDataFill, &StorageTmp->pingCtlDataFillLen);

            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingCtlFrequency, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingCtlMaxRows, &tmpint);

            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->pingCtlStorageType, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->pingCtlTrapGeneration, &StorageTmp->pingCtlTrapGenerationLen);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingCtlTrapProbeFailureFilter, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingCtlTrapTestFailureFilter, &tmpint);
            cptr = read_config_store_data (ASN_OBJECT_ID, cptr, &StorageTmp->pingCtlType, &StorageTmp->pingCtlTypeLen);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr, &StorageTmp->pingCtlDescr, &StorageTmp->pingCtlDescrLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->pingCtlSourceAddressType, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->pingCtlSourceAddress, &StorageTmp->pingCtlSourceAddressLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->pingCtlIfIndex, &tmpint);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->pingCtlByPassRouteTable, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingCtlDSField, &tmpint);

            if (StorageTmp->pingCtlRowStatus == RS_ACTIVE)
                StorageTmp->pingCtlRowStatus = RS_NOTINSERVICE;

            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->pingCtlRowStatus, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingProbeHistoryMaxIndex, &tmpint);
                                 ...
                                 oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
     ...
     */
    if ((StorageTmp = header_complex (pingCtlTableStorage, vp, name, length, exact, var_len, write_method)) == NULL)
    {
...
unsigned long pingProbeHistoryTable_count (struct pingCtlTable_data *thedata)
{
    ...
    for (hciptr2 = pingProbeHistoryTableStorage; hciptr2 != NULL; hciptr2 = hciptr2->next)
    {
...
void pingProbeHistoryTable_delLast (struct pingCtlTable_data *thedata)
{
    struct header_complex_index *hciptr2 = NULL;
    ...
    for (hcilast = hciptr2 = pingProbeHistoryTableStorage; hciptr2 != NULL; hciptr2 = hciptr2->next)
    {
        if (snmp_oid_compare (newoid, newoid_len, hciptr2->name, newoid_len) == 0)
        {

            StorageTmp = header_complex_get_from_oid (pingProbeHistoryTableStorage, hciptr2->name, hciptr2->namelen);
    ...
    StorageDel = header_complex_extract_entry (&pingProbeHistoryTableStorage, hcilast);
    for (hciptr2 = pingCtlTableStorage; hciptr2; hciptr2 = hciptr2->next)
    {
        ...
        if (tmp->pingProbeHis == StorageDel)
        {
            ...
            DEBUGMSGTL (("pingProbeHistoryTable", "deleting the last one succeeded!\n"));
            break;
        }
    }
    if (StorageDel)
    ...
    DEBUGMSGTL (("pingProbeHistoryTable", "delete the last one success!\n"));
}
...
struct addrinfo *Host_serv (const char *host, const char *serv, int family, int socktype)
{
    ...
    if ((n = netsnmp_getaddrinfo (host, serv, &hints, &res)) != 0)
    {
        ...
        snmp_log (LOG_ERR, "host_serv error for %s, %s",
                  (host == NULL) ? "(no hostname)" : host, (serv == NULL) ? "(no service name)" : serv);
#endif
    }
 ...
 * send trap 
 */
void send_ping_trap (struct pingCtlTable_data *item, oid * trap_oid, size_t trap_oid_len)
{
    ...
    oid pingCtlTargetAddress[] = { 1, 3, 6, 1, 2, 1, 80, 1, 2, 1, 4 };
    oid pingResultsMinRtt[] = { 1, 3, 6, 1, 2, 1, 80, 1, 3, 1, 4 };
    oid pingResultsMaxRtt[] = { 1, 3, 6, 1, 2, 1, 80, 1, 3, 1, 5 };
    oid pingResultsAverageRtt[] = { 1, 3, 6, 1, 2, 1, 80, 1, 3, 1, 6 };
    oid pingResultsProbeResponses[] = { 1, 3, 6, 1, 2, 1, 80, 1, 3, 1, 7 };
    oid pingResultsSendProbes[] = { 1, 3, 6, 1, 2, 1, 80, 1, 3, 1, 8 };

    snmp_varlist_add_variable (&vars, NULL, 0, ASN_OCTET_STR, item->pingCtlOwnerIndex, item->pingCtlOwnerIndexLen);
    snmp_varlist_add_variable (&vars, NULL, 0, ASN_OCTET_STR, item->pingCtlTestName, item->pingCtlTestNameLen);
    ...
    header_complex_generate_oid (newoid, &newoid_len, pingCtlTargetAddress,
                                 sizeof (pingCtlTargetAddress) / sizeof (oid), vars);

    snmp_varlist_add_variable (&var_list, newoid,
                               newoid_len,
                               ASN_OCTET_STR, (u_char *) item->pingCtlTargetAddress, item->pingCtlTargetAddressLen);
    ...
    header_complex_generate_oid (newoid, &newoid_len, pingResultsMinRtt,
                                 sizeof (pingResultsMinRtt) / sizeof (oid), vars);

    snmp_varlist_add_variable (&var_list, newoid,
                               newoid_len,
                               ASN_UNSIGNED,
                               (u_char *) & (StorageTmp->pingResultsMinRtt), sizeof (StorageTmp->pingResultsMinRtt));
    /*
     * pingResultsMaxRtt 
     */
    memset (newoid, '\0', newoid_len);
    header_complex_generate_oid (newoid, &newoid_len, pingResultsMaxRtt,
                                 sizeof (pingResultsMaxRtt) / sizeof (oid), vars);

    snmp_varlist_add_variable (&var_list, newoid,
                               newoid_len,
                               ASN_UNSIGNED,
                               (u_char *) & (StorageTmp->pingResultsMaxRtt), sizeof (StorageTmp->pingResultsMaxRtt));
    ...
    header_complex_generate_oid (newoid, &newoid_len, pingResultsAverageRtt,
                                 sizeof (pingResultsAverageRtt) / sizeof (oid), vars);

    snmp_varlist_add_variable (&var_list, newoid,
                               newoid_len,
                               ASN_UNSIGNED,
                               (u_char *) & (StorageTmp->pingResultsAverageRtt),
    ...
    header_complex_generate_oid (newoid, &newoid_len,
                                 pingResultsProbeResponses, sizeof (pingResultsProbeResponses) / sizeof (oid), vars);

    snmp_varlist_add_variable (&var_list, newoid,
                               newoid_len,
                               ASN_UNSIGNED,
                               (u_char *) & (StorageTmp->pingResultsProbeResponses),
                               sizeof (StorageTmp->pingResultsProbeResponses));
    /*
     * pingResultsSendProbes 
     */
    memset (newoid, '\0', newoid_len);
    header_complex_generate_oid (newoid, &newoid_len, pingResultsSendProbes,
                                 sizeof (pingResultsSendProbes) / sizeof (oid), vars);

    snmp_varlist_add_variable (&var_list, newoid,
                               newoid_len,
                               ASN_UNSIGNED,
                               (u_char *) & (StorageTmp->pingResultsSendProbes),
...
readloop (struct pingCtlTable_data *item, struct addrinfo *ai, int datalen,
          unsigned long *minrtt, unsigned long *maxrtt, unsigned long *averagertt, pid_t pid)
{
    ...
    for (current_probe_temp = 1; current_probe_temp <= item->pingCtlProbeCount; current_probe_temp++)
    {
        time_t timep;
...
unsigned long round_double (double number)
{
         ...
         int flag, struct pingProbeHistoryTable_data *current_temp, pid_t pid)
{
    ...
    if (flag == 0)
    {
        ...
        DEBUGMSGTL (("pingCtlTable", "ICMP type = %d (%sa reply)\n",
                     icmp->icmp_type, icmp->icmp_type == ICMP_ECHOREPLY ? "" : "not a "));
        ...
        if (icmp->icmp_type == ICMP_ECHOREPLY)
        {
            ...
            rtt =
                round_double ((1000000 * (tvrecv->tv_sec - tvsend->tv_sec) + tvrecv->tv_usec - tvsend->tv_usec) / 1000);

            snmp_varlist_add_variable (&vars, NULL, 0, ASN_OCTET_STR,
                                       item->pingCtlOwnerIndex, item->pingCtlOwnerIndexLen);
            snmp_varlist_add_variable (&vars, NULL, 0, ASN_OCTET_STR, item->pingCtlTestName, item->pingCtlTestNameLen);
            ...
            else
            {
                ...
                if (rtt > *maxrtt)
                    *maxrtt = rtt;
                *sumrtt = (*sumrtt) + rtt;
                *averagertt = round_double ((*sumrtt) / (StorageNew->pingResultsProbeResponses + 1));
            }
            ...
            StorageNew->pingResultsProbeResponses = StorageNew->pingResultsProbeResponses + 1;
            StorageNew->pingResultsSendProbes = StorageNew->pingResultsSendProbes + 1;
            StorageNew->pingResultsRttSumOfSquares = StorageNew->pingResultsRttSumOfSquares + rtt * rtt;
            ...
            temp->pingCtlOwnerIndex = (char *) malloc (item->pingCtlOwnerIndexLen + 1);
            memcpy (temp->pingCtlOwnerIndex, item->pingCtlOwnerIndex, item->pingCtlOwnerIndexLen + 1);
            temp->pingCtlOwnerIndex[item->pingCtlOwnerIndexLen] = '\0';
            temp->pingCtlOwnerIndexLen = item->pingCtlOwnerIndexLen;

            temp->pingCtlTestName = (char *) malloc (item->pingCtlTestNameLen + 1);
            memcpy (temp->pingCtlTestName, item->pingCtlTestName, item->pingCtlTestNameLen + 1);
            ...
            memdup (&temp->pingProbeHistoryTime, date_n_time (&timep, &temp->pingProbeHistoryTimeLen), 11);
    ...
    else if (flag == 1)
    {
        ...
        if (item->pingProbeHis != NULL)
        {
            ...
            {
                if (pingProbeHistoryTable_add (current_temp) != SNMPERR_SUCCESS)
                    DEBUGMSGTL (("pingProbeHistoryTable", "registered an entry error\n"));
            }
            else
            {
        ...
        if ((item->pingCtlTrapGeneration[0] & PINGTRAPGENERATION_PROBEFAILED) != 0)
        {
            if (probeFailed >= item->pingCtlTrapProbeFailureFilter)
                send_ping_trap (item, pingProbeFailed, sizeof (pingProbeFailed) / sizeof (oid));
        }
    ...
    {
        if ((item->pingCtlTrapGeneration[0] & PINGTRAPGENERATION_TESTCOMPLETED) != 0)
        {
            send_ping_trap (item, pingTestCompleted, sizeof (pingTestCompleted) / sizeof (oid));
        }
        else if ((item->pingCtlTrapGeneration[0] & PINGTRAPGENERATION_TESTFAILED) != 0)
        {

            if (testFailed >= item->pingCtlTrapTestFailureFilter)
                send_ping_trap (item, pingTestFailed, sizeof (pingTestFailed) / sizeof (oid));
        }

        else if ((item->pingCtlTrapGeneration[0] & PINGTRAPGENERATION_PROBEFAILED) != 0)
        {;
        }
        else
        {
...
void send_v4 (int datalen, pid_t pid, int nsent, int sockfd, char *sendbuf)
{
    int len;
...
void run_ping (unsigned int clientreg, void *clientarg)
...
/* run_ping(struct pingCtlTable_data *item) */
{
    ...
    if (item->pingCtlTargetAddressType == 1 || item->pingCtlTargetAddressType == 16)
    {
    ...
    else if (item->pingCtlTargetAddressType == 2)
    {
        ...
        err = setsockopt (icmp_sock, SOL_ICMPV6, ICMP6_FILTER, &filter, sizeof (struct icmp6_filter));
        ...
        if (1)
        {
            ...
            if (setsockopt (icmp_sock, IPPROTO_IPV6, IPV6_HOPLIMIT, &on, sizeof (on)) == -1)
            {
        ...
        DEBUGMSGTL (("pingCtlTable", "PING %s(%s) ", hostname, pr_addr (&whereto.sin6_addr, options)));
        if (flowlabel)
            DEBUGMSGTL (("pingCtlTable", ", flow 0x%05x, ", (unsigned) ntohl (flowlabel)));
        if (device || (options & F_NUMERIC))
        {
            DEBUGMSGTL (("pingCtlTable", "from %s %s: ", pr_addr_n (&source.sin6_addr), device ? : ""));
        }
        DEBUGMSGTL (("pingCtlTable", "%d data bytes\n", datalen));

        setup (icmp_sock, options, uid, timeout, preload, interval, datalen,
               (char *) outpack, &ident, &start_time, &screen_width, &deadline);
...
void init_resultsTable (struct pingCtlTable_data *item)
{
    struct pingResultsTable_data *StorageTmp = NULL;
    ...
    StorageTmp->pingCtlOwnerIndex = (char *) malloc (item->pingCtlOwnerIndexLen + 1);
    memcpy (StorageTmp->pingCtlOwnerIndex, item->pingCtlOwnerIndex, item->pingCtlOwnerIndexLen + 1);
    StorageTmp->pingCtlOwnerIndex[item->pingCtlOwnerIndexLen] = '\0';
    StorageTmp->pingCtlOwnerIndexLen = item->pingCtlOwnerIndexLen;

    StorageTmp->pingCtlTestName = (char *) malloc (item->pingCtlTestNameLen + 1);
    memcpy (StorageTmp->pingCtlTestName, item->pingCtlTestName, item->pingCtlTestNameLen + 1);
...
int modify_ResultsOper (struct pingCtlTable_data *thedata, long val)
{
    netsnmp_variable_list *vars = NULL;
...
int pingResultsTable_del (struct pingCtlTable_data *thedata)
{
    ...
    for (hciptr2 = pingResultsTableStorage; hciptr2; hciptr2 = next)
    {
        ...
        if (snmp_oid_compare (newoid, newoid_len, hciptr2->name, newoid_len) == 0)
        {
            StorageDel = header_complex_extract_entry (&pingResultsTableStorage, hciptr2);
            if (StorageDel != NULL)
            {
...
int pingProbeHistoryTable_del (struct pingCtlTable_data *thedata)
{
    struct header_complex_index *hciptr2, *next;
    ...
    for (hciptr2 = pingProbeHistoryTableStorage; hciptr2; hciptr2 = next)
    {
        ...
        if (snmp_oid_compare (newoid, newoid_len, hciptr2->name, newoid_len) == 0)
        {
            StorageDel = header_complex_extract_entry (&pingProbeHistoryTableStorage, hciptr2);
            if (StorageDel != NULL)
            {
                SNMP_FREE (StorageDel->pingCtlOwnerIndex);
                SNMP_FREE (StorageDel->pingCtlTestName);
                                ...
                                u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    size_t newlen = name_len - (sizeof (pingCtlTable_variables_oid) / sizeof (oid) + 3 - 1);


    DEBUGMSGTL (("pingCtlTable", "write_pingCtlTargetAddressType entering action=%d...  \n", action));
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            {
                snmp_log (LOG_ERR, "write to pingCtlTargetAddressType not ASN_INTEGER\n");
                            ...
                            u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            {
                snmp_log (LOG_ERR, "write to pingCtlTargetAddress not ASN_OCTET_STR\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
        ...
        case ACTION:
            ...
            StorageTmp->pingCtlTargetAddress = (char *) malloc (var_val_len + 1);
            if (StorageTmp->pingCtlTargetAddress == NULL)
            {
                exit (1);
            }
                       ...
                       u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "pingCtlDataSize entering action=%d...  \n", action));
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            {
                snmp_log (LOG_ERR, "write to pingCtlDataSize not ASN_UNSIGNED\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                      ...
                      u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "pingCtlTimeOut entering action=%d...  \n", action));
                         ...
                         u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "pingCtlProbeCount entering action=%d...  \n", action));
                          ...
                          u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "pingCtlAdminStatus entering action=%d...  \n", action));
    ...
    switch (action)
    {
        ...
        case COMMIT:
             ...
             * permanently.  Make sure that anything done here can't fail! 
             */
            if (StorageTmp->pingCtlAdminStatus == 1 && StorageTmp->pingCtlRowStatus == RS_ACTIVE)
            {
                StorageTmp->pingResults->pingResultsOperStatus = 1;
                modify_ResultsOper (StorageTmp, 1);
                if (StorageTmp->pingCtlFrequency != 0)
                    StorageTmp->timer_id =
                        snmp_alarm_register (StorageTmp->pingCtlFrequency, SA_REPEAT, run_ping, StorageTmp);
                else
                    StorageTmp->timer_id = snmp_alarm_register (1, 0, run_ping, StorageTmp);

            }
            else if (StorageTmp->pingCtlAdminStatus == 2 && StorageTmp->pingCtlRowStatus == RS_ACTIVE)
            {
                        ...
                        u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "pingCtlFrequency entering action=%d...  \n", action));
                      ...
                      u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "pingCtlMaxRows entering action=%d...  \n", action));
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_UNSIGNED)
            {
                snmp_log (LOG_ERR, "write to pingCtlMaxRows not ASN_UNSIGNED\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                          ...
                          u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "pingCtlStorageType entering action=%d...  \n", action));
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_INTEGER)
            {
                snmp_log (LOG_ERR, "write to pingCtlStorageType not ASN_INTEGER\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                             ...
                             u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        ...
        case ACTION:
            ...
            StorageTmp->pingCtlTrapGeneration = (char *) malloc (var_val_len + 1);
            if (StorageTmp->pingCtlTrapGeneration == NULL)
            {
                exit (1);
            }
                                     ...
                                     size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "pingCtlTrapProbeFailureFilter entering action=%d...  \n", action));
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_UNSIGNED)
            {
                snmp_log (LOG_ERR, "write to pingCtlTrapProbeFailureFilter not ASN_UNSIGNED\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
        ...
        case ACTION:
            ...
            if ((*((long *) var_val)) >= 0 && (*((long *) var_val)) <= 15)
                StorageTmp->pingCtlTrapProbeFailureFilter = *((long *) var_val);
            else
                                    ...
                                    size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "pingCtlTrapTestFailureFilter entering action=%d...  \n", action));
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_UNSIGNED)
            {
                snmp_log (LOG_ERR, "write to pingCtlTrapTestFailureFilter not ASN_UNSIGNED\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                                ...
                                u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "pingCtlSourceAddressType entering action=%d...  \n", action));
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_INTEGER)
            {
                snmp_log (LOG_ERR, "write to pingCtlSourceAddressType not ASN_INTEGER\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                            ...
                            u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_OCTET_STR)
            {
                snmp_log (LOG_ERR, "write to pingCtlSourceAddress not ASN_OCTET_STR\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
        ...
        case ACTION:
            ...
            StorageTmp->pingCtlSourceAddress = (char *) malloc (var_val_len + 1);
            if (StorageTmp->pingCtlSourceAddress == NULL)
            {
                exit (1);
            }
                      ...
                      u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "pingCtlIfIndex entering action=%d...  \n", action));
                               ...
                               u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "pingCtlByPassRouteTable entering action=%d...  \n", action));
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_INTEGER)
            {
                snmp_log (LOG_ERR, "write to pingCtlByPassRouteTable not ASN_INTEGER\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                      ...
                      u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "pingCtlDSField entering action=%d...  \n", action));
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_UNSIGNED)
            {
                snmp_log (LOG_ERR, "write to pingCtlDSField not ASN_UNSIGNED\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                        ...
                        u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("pingCtlTable", "var_pingCtlTable: Entering...  action=%ul\n", action));
    StorageTmp =
        header_complex (pingCtlTableStorage, NULL,
                        &name[sizeof (pingCtlTable_variables_oid) / sizeof (oid) + 3 - 1], &newlen, 1, NULL, NULL);
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            else
            {
                ...
                if (StorageTmp->pingCtlRowStatus == RS_ACTIVE && set_value != RS_DESTROY)
                {
                    /*
        ...
        case RESERVE2:
            ...
            if (StorageTmp == NULL)
            {
                ...
                if (header_complex_parse_oid
                    (&(name[sizeof (pingCtlTable_variables_oid) / sizeof (oid) + 2]), newlen, vars) != SNMPERR_SUCCESS)
                {
                    snmp_free_varbind (vars);
                ...
                if (vp->val_len <= 32)
                {
                    ...
                    memcpy (StorageNew->pingCtlOwnerIndex, vp->val.string, vp->val_len);
                ...
                if (vp->val_len <= 32)
                {
                    ...
                    memcpy (StorageNew->pingCtlTestName, vp->val.string, vp->val_len);
        ...
        case ACTION:
            ...
            if (StorageTmp == NULL)
            {
                ...
                if (StorageNew != NULL)
#if 1
                    DEBUGMSGTL (("pingCtlTable", "write_pingCtlRowStatus entering new=%d...  \n", action));
#endif
            ...
            else
            {
                ...
                hciptr = header_complex_find_entry (pingCtlTableStorage, StorageTmp);
                StorageDel = header_complex_extract_entry (&pingCtlTableStorage, hciptr);
        ...
        case UNDO:
            ...
            if (StorageTmp == NULL)
            {
                 ...
                 */
                hciptr = header_complex_find_entry (pingCtlTableStorage, StorageNew);
                StorageDel = header_complex_extract_entry (&pingCtlTableStorage, hciptr);
                free_pingCtlTable_data (StorageDel);
                StorageDel = NULL;
            }
        ...
        case COMMIT:
            ...
            {
                if (StorageTmp && StorageTmp->pingCtlRowStatus == RS_CREATEANDGO)
                {
                    StorageTmp->pingCtlRowStatus = RS_ACTIVE;
                }
                else if (StorageTmp && StorageTmp->pingCtlRowStatus == RS_CREATEANDWAIT)
                {
            ...
            {
#if 1
                DEBUGMSGTL (("pingCtlTable", "write_pingCtlRowStatus entering runbefore \n"));
...
static inline int
schedule_exit (int next, int *deadline, long *npackets, long *nreceived, long *ntransmitted, long *tmax)
{
...
static inline int in_flight (__u16 * acked, long *nreceived, long *ntransmitted, long *nerrors)
{
...
static inline void acknowledge (__u16 seq, __u16 * acked, long *ntransmitted, int *pipesize)
{
...
static inline void advance_ntransmitted (__u16 * acked, long *ntransmitted)
{
    (*ntransmitted)++;
    /*
...
static inline void update_interval (int uid, int interval, int *rtt_addend, int *rtt)
{
...
int __schedule_exit (int next, long *nreceived, long *tmax)
{
...
pinger (int icmp_sock, int preload, int cmsglen, char *cmsgbuf,
        ...
        int *confirm_flag, int *confirm, int *pipesize, struct timeval *cur_time)
{
    static int tokens;
    ...
    else
    {
        ...
        if (!interval)
        {
             ...
             */
            if (ntokens < MININTERVAL && in_flight (acked, nreceived, ntransmitted, nerrors) >= preload)
  ...
  resend:
    i = send_v6 (icmp_sock, cmsglen, cmsgbuf, whereto, datalen, timing, outpack, ident, ntransmitted, confirm);
    ...
    if (i == 0)
    {
        ...
        if (!(options & F_QUIET) && (options & F_FLOOD))
        {
            ...
            if ((preload < (*screen_width)
                ...
                || in_flight (acked, nreceived, ntransmitted, nerrors) < (*screen_width))
    ...
    {
        if ((i = receive_error_msg (icmp_sock, whereto, options, ident, nerrors)) > 0)
        {
            /*
...
void sock_setbufs (int icmp_sock, int alloc, int preload)
{
    int rcvbuf, hold;
...
main_loop (struct pingCtlTable_data *item, int icmp_sock, int preload,
           ...
           int working_recverr, char *outpack, int *ident, struct timeval *start_time, int *screen_width, int *deadline)
{
    char addrbuf[128];
    ...
    for (;;)
    {
         ...
         */
        if (status_snapshot)
            status (timing, &rtt, &nreceived, &nrepeats, &ntransmitted, &tmin, &tmax, &tsum, &tsum2);
        ...
        do
        {
            ...
            next =
                pinger (icmp_sock, preload, cmsglen, cmsgbuf, whereto,
                        ...
                        &ntransmitted, &nerrors, &confirm_flag, &confirm, &pipesize, &cur_time);
            DEBUGMSGTL (("pingCtlTable", "1:next=%d\n", next));
            next = schedule_exit (next, deadline, &npackets, &nreceived, &ntransmitted, &tmax);
        ...
        {
            int recv_expected = in_flight (&acked, &nreceived, &ntransmitted, &nerrors);
        ...
        for (;;)
        {
            ...
            if (cc < 0)
            {
                ...
                /* timeout finish                                 */
                if (!receive_error_msg (icmp_sock, whereto, options, ident, &nerrors))
                {
                    if (errno)
                    {
                        perror ("ping: recvmsg");
            ...
            else
            {
                ...
                not_ours =
                    parse_reply (&series, item, &msg, cc, addrbuf,
                                 ...
                                 &tmax, &tsum, &tsum2, &confirm_flag, &confirm, &pipesize, &current_temp);
            }
             ...
             */
            if (in_flight (&acked, &nreceived, &ntransmitted, &nerrors) == 0)
                break;
...
gather_statistics (int *series, struct pingCtlTable_data *item, __u8 * ptr,
                   ...
                   int *confirm_flag, int *confirm, int *pipesize, struct pingProbeHistoryTable_data *current_temp)
{
    int dupflag = 0;
    ...
    *series = 0;
    ...
    if (timing && cc >= 8 + sizeof (struct timeval))
    {
        ...
        if (triptime < 0)
        {
            snmp_log (LOG_INFO, "Warning: invalid timestamp in ICMP response.\n");
            triptime = 0;
            if (!(options & F_LATENCY))
                options |= F_LATENCY;
        }
    ...
    *confirm = *confirm_flag;
    ...
    else
    {
        ...
        DEBUGMSGTL (("pingCtlTable", "%d bytes from %s: icmp_seq=%u", cc, from, seq));
        ...
        if (timing)
        {
            if (triptime >= 100000)
                ...
                DEBUGMSGTL (("pingCtlTable", " time=%ld.%01ld ms", triptime / 1000, (triptime % 1000) / 100));
            else if (triptime >= 1000)
                DEBUGMSGTL (("pingCtlTable", " time=%ld.%02ld ms", triptime / 1000, (triptime % 1000) / 10));
            else
                DEBUGMSGTL (("pingCtlTable", " time=%ld.%03ld ms", triptime / 1000, triptime % 1000));
        }
        if (dupflag)
    ...
    StorageNew->pingResultsAverageRtt = (*tsum) / (StorageNew->pingResultsProbeResponses + 1);
    StorageNew->pingResultsProbeResponses = StorageNew->pingResultsProbeResponses + 1;
    StorageNew->pingResultsSendProbes = StorageNew->pingResultsSendProbes + 1;
    ...
    StorageNew->pingResultsLastGoodProbe_time = timep;
    free (StorageNew->pingResultsLastGoodProbe);
    memdup (&StorageNew->pingResultsLastGoodProbe, date_n_time (&timep, &StorageNew->pingResultsLastGoodProbeLen), 11);
...
finish (int options, char *hostname, int interval, int timing, int *rtt,
        ...
        long long *tsum, long long *tsum2, int *pipesize, struct timeval *cur_time)
{
    ...
    if (*ntransmitted)
    {
        ...
        DEBUGMSGTL (("pingCtlTable", ", time %ldms", 1000 * tv.tv_sec + tv.tv_usec / 1000));
    }
    putchar ('\n');
...
status (int timing, int *rtt, long *nreceived, long *nrepeats,
        long *ntransmitted, long *tmin, long *tmax, long long *tsum, long long *tsum2)
{
    int loss = 0;
    ...
    DEBUGMSGTL (("pingCtlTable", "\n%ld/%ld packets, %d%% loss", *ntransmitted, *nreceived, loss));
    ...
    if ((*nreceived) && timing)
    {
        ...
        DEBUGMSGTL (("pingCtlTable", ", min/avg/ewma/max = %ld.%03ld/%lu.%03ld"
                     ...
                     (*rtt) / 8000, ((*rtt) / 8) % 1000, (*tmax) / 1000, (*tmax) % 1000));
    }
    DEBUGMSGTL (("pingCtlTable", "\n"));
}
...
int receive_error_msg (int icmp_sock, struct sockaddr_in6 *whereto, int options, int *ident, long *nerrors)
{
    int res;
    ...
    else if (e->ee_origin == SO_EE_ORIGIN_ICMP6)
    {
        if (res < sizeof (icmph) ||
            ...
            icmph.icmp6_type != ICMP6_ECHO_REQUEST || icmph.icmp6_id != *ident)
        {
            /*
             ...
             int *confirm_flag, int *confirm, int *pipesize, struct pingProbeHistoryTable_data *current_temp)
{
    ...
    if (icmph->icmp6_type == ICMP6_ECHO_REPLY)
    {
        ...
        if (gather_statistics (series, item, (__u8 *) (icmph + 1), cc,
                               ...
                               tmin, tmax, tsum, tsum2, confirm_flag, confirm, pipesize, current_temp))
            return 0;
    }
    else
    {
        ...
        {
            if (icmph1->icmp6_type != ICMP6_ECHO_REQUEST || icmph1->icmp6_id != *ident)
                return 1;
            acknowledge (icmph1->icmp6_seq, acked, ntransmitted, pipesize);
            if (working_recverr)
                return 0;
            (*nerrors)++;
            if (options & F_FLOOD)
            ...
            DEBUGMSGTL (("pingCtlTable", "From %s: icmp_seq=%u ",
                         pr_addr (&from->sin6_addr, options), icmph1->icmp6_seq));
        }
        else
        {
            /*
            ...
            if (!(options & F_VERBOSE) || uid)
                return 1;
            DEBUGMSGTL (("pingCtlTable", "From %s: ", pr_addr (&from->sin6_addr, options)));
        }
...
void install_filter (int icmp_sock, int *ident)
{
    static int once;
     ...
     */
    id = htons (*ident);
    insns[1] = (struct sock_filter) BPF_JUMP (BPF_JMP | BPF_JEQ | BPF_K, id, 0, 1);
...
static char *pr_addr (struct in6_addr *addr, int options)
{
    ...
    if (!(options & F_NUMERIC))
        hp = netsnmp_gethostbyaddr ((__u8 *) addr, sizeof (struct in6_addr), AF_INET6);
