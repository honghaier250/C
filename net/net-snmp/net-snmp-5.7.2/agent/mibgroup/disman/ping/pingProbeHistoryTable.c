...
oid pingProbeHistoryTable_variables_oid[] = { 1, 3, 6, 1, 2, 1, 80, 1, 4 };
...
int pingProbeHistoryTable_inadd (struct pingProbeHistoryTable_data *thedata);

void pingProbeHistoryTable_cleaner (struct header_complex_index *thestuff)
{
...
void init_pingProbeHistoryTable (void)
{
     ...
     */
    snmpd_register_config_handler ("pingProbeHistoryTable", parse_pingProbeHistoryTable, NULL, NULL);

    /*
     * we need to be called back later to store our data 
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_pingProbeHistoryTable, NULL);
...
void parse_pingProbeHistoryTable (const char *token, char *line)
{
    size_t tmpint;

    struct pingProbeHistoryTable_data *StorageTmp = SNMP_MALLOC_STRUCT (pingProbeHistoryTable_data);
    ...
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingProbeHistoryIndex, &tmpint);
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingProbeHistoryResponse, &tmpint);
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->pingProbeHistoryStatus, &tmpint);
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->pingProbeHistoryLastRC, &tmpint);

    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->pingProbeHistoryTime, &StorageTmp->pingProbeHistoryTimeLen);
...
int store_pingProbeHistoryTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (hcindex = pingProbeHistoryTableStorage; hcindex != NULL; hcindex = hcindex->next)
    {
        ...
        if (StorageTmp->storageType != ST_READONLY)
        {
            ...
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingProbeHistoryIndex, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingProbeHistoryResponse, &tmpint);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->pingProbeHistoryStatus, &tmpint);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->pingProbeHistoryLastRC, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->pingProbeHistoryTime, &StorageTmp->pingProbeHistoryTimeLen);
                                          ...
                                          size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    *write_method = NULL;
     ...
     */
    if ((StorageTmp =
         header_complex (pingProbeHistoryTableStorage, vp, name, length, exact, var_len, write_method)) == NULL)
    {
