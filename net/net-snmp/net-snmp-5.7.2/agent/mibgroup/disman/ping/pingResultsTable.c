 ...
 *
 */
oid pingResultsTable_variables_oid[] = { 1, 3, 6, 1, 2, 1, 80, 1, 3 };
...
int pingResultsTable_inadd (struct pingResultsTable_data *thedata);

void pingResultsTable_cleaner (struct header_complex_index *thestuff)
{
...
void parse_pingResultsTable (const char *token, char *line)
{
    size_t tmpint;

    struct pingResultsTable_data *StorageTmp = SNMP_MALLOC_STRUCT (pingResultsTable_data);
    ...
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->pingResultsOperStatus, &tmpint);
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->pingResultsIpTargetAddressType, &tmpint);
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->pingResultsIpTargetAddress, &StorageTmp->pingResultsIpTargetAddressLen);
    if (StorageTmp->pingResultsIpTargetAddress == NULL)
    {
        config_perror ("invalid specification for pingResultsIpTargetAddress");
        return;
    }

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingResultsMinRtt, &tmpint);
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingResultsMaxRtt, &tmpint);
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingResultsAverageRtt, &tmpint);
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingResultsProbeResponses, &tmpint);
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingResultsSendProbes, &tmpint);
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->pingResultsRttSumOfSquares, &tmpint);
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->pingResultsLastGoodProbe, &StorageTmp->pingResultsLastGoodProbeLen);
    if (StorageTmp->pingResultsLastGoodProbe == NULL)
    {
        config_perror ("invalid specification for pingResultsLastGoodProbe!");
        return;
    }
...
int store_pingResultsTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (hcindex = pingResultsTableStorage; hcindex != NULL; hcindex = hcindex->next)
    {
        ...
        if (StorageTmp->storageType != ST_READONLY)
        {
            ...
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->pingResultsOperStatus, &tmpint);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->pingResultsIpTargetAddressType, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->pingResultsIpTargetAddress,
                                        &StorageTmp->pingResultsIpTargetAddressLen);

            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingResultsMinRtt, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingResultsMaxRtt, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingResultsAverageRtt, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingResultsProbeResponses, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingResultsSendProbes, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->pingResultsRttSumOfSquares, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                     ...
                                     size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    *write_method = NULL;
     ...
     */
    if ((StorageTmp = header_complex (pingResultsTableStorage, vp, name, length, exact, var_len, write_method)) == NULL)
    {
