...
netsnmp_feature_require (iquery) netsnmp_feature_child_of (sched_nextrowtime, netsnmp_unused)
...
static void _sched_callback (unsigned int reg, void *magic)
{
    ...
    DEBUGMSGOID (("disman:schedule:callback", entry->schedVariable, entry->schedVariable_len));
    ...
    snmp_set_var_typed_value (&assign, ASN_INTEGER, (u_char *) & entry->schedValue, sizeof (entry->schedValue));

    ret = netsnmp_query_set (&assign, entry->session);
    if (ret != SNMP_ERR_NOERROR)
    {
        DEBUGMSGTL (("disman:schedule:callback", "assignment failed (%d)\n", ret));
        entry->schedFailures++;
 ...
 */
static int _bit_next_day (char *day_pattern, char weekday_pattern, int day, int month, int year)
{
    char buf[4];
...
void sched_nextTime (struct schedTable_entry *entry)
{
    ...
    switch (entry->schedType)
    {
        ...
        case SCHED_TYPE_CALENDAR:
            ...
            if (_bit_allClear (entry->schedMinute, 8) ||
                ...
                _bit_allClear (entry->schedMonth, 2) || _bit_allClear (&entry->schedWeekDay, 1))
            {
            ...
            if (_bit_set (entry->schedMonth, now_tm.tm_mon))
            {
                ...
                if (_bit_set (&entry->schedWeekDay, now_tm.tm_wday) &&
                    (_bit_set (entry->schedDay, now_tm.tm_mday - 1) || _bit_set (entry->schedDay, 31 + rev_day)))
                {
                    ...
                    if (_bit_set (entry->schedHour, now_tm.tm_hour))
                    {
                        ...
                        next_tm.tm_min = _bit_next (entry->schedMinute, now_tm.tm_min, 8);
                    }
                    else
                    {
                        next_tm.tm_min = -1;
                    ...
                    if (next_tm.tm_min == -1)
                    {
                        ...
                        next_tm.tm_hour = _bit_next (entry->schedHour, now_tm.tm_hour, 3);
                    }
                }
                else
                {
                ...
                if (next_tm.tm_hour == -1)
                {
                    ...
                    next_tm.tm_mday = _bit_next_day (entry->schedDay,
                                                     entry->schedWeekDay, now_tm.tm_mday, mon, now_tm.tm_year);
                }
            }
            else
            {
            ...
            {
                next_tm.tm_mon = _bit_next (entry->schedMonth, next_tm.tm_mon, 2);
                if (next_tm.tm_mon == -1)
                {
                    next_tm.tm_year++;
                ...
                if (mon == 1 && (next_tm.tm_year % 4 == 0))
                    mon = 12;
                next_tm.tm_mday = _bit_next_day (entry->schedDay, entry->schedWeekDay, -1, mon, next_tm.tm_year);
    ...
    entry->schedCallbackID = snmp_alarm_register (entry->schedNextRun - now, 0, _sched_callback, entry);
    return;
}
 ...
 */
netsnmp_tdata_row *schedTable_createEntry (const char *schedOwner, const char *schedName)
{
    ...
    if (schedOwner)
    {
        ...
        netsnmp_tdata_row_add_index (row, ASN_OCTET_STR, entry->schedOwner, strlen (schedOwner));
    }
    else
        netsnmp_tdata_row_add_index (row, ASN_OCTET_STR, "", 0);

    memcpy (entry->schedName, schedName, strlen (schedName));
    netsnmp_tdata_row_add_index (row, ASN_OCTET_STR, entry->schedName, strlen (schedName));
    /*
...
void schedTable_removeEntry (netsnmp_tdata_row * row)
{
    ...
    entry = (struct schedTable_entry *) netsnmp_tdata_remove_and_delete_row (schedule_table, row);
    if (entry)
    {
