     ...
     void init_schedConf (void)
{
     ...
     */
    snmpd_register_config_handler ("repeat", parse_sched_periodic, NULL, "repeat period  OID = value");
    snmpd_register_config_handler ("cron", parse_sched_timed, NULL, "cron * * * * * OID = value");
    snmpd_register_config_handler ("at", parse_sched_timed, NULL, "at   * * * * * OID = value");
    ...
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_schedTable, NULL);
    schedEntries = 0;
}
 ...
 */
void _sched_convert_bits (char *cron_spec, char *bit_buf, int bit_buf_len, int max_val, int startAt1)
{
    char *cp = cron_spec;
...
void parse_schedTable (const char *token, char *line)
{
    ...
    vp = entry->schedDescr;
    line = read_config_read_data (ASN_OCTET_STR, line, &vp, &len);
    line = read_config_read_data (ASN_UNSIGNED, line, &entry->schedInterval, NULL);
...
int store_schedTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (row = netsnmp_tdata_row_first (schedule_table); row; row = netsnmp_tdata_row_next (schedule_table, row))
    {
