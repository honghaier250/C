...
netsnmp_feature_require (table_dataset) netsnmp_feature_require (table_set_multi_add_default_row)
     ...
     void initialize_table_mteEventTable (void)
{
     ...
     * Adding indexes
     */
    DEBUGMSGTL (("initialize_table_mteEventTable", "adding indexes to table mteEventTable\n"));
    netsnmp_table_set_add_indexes (table_set, ASN_OCTET_STR,    /* index: mteOwner */
                                   ASN_PRIV_IMPLIED_OCTET_STR,    /* index: mteEventName */
                                   0);

    DEBUGMSGTL (("initialize_table_mteEventTable", "adding column types to table mteEventTable\n"));
    netsnmp_table_set_multi_add_default_row (table_set,
                                             ...
                                             COLUMN_MTEEVENTENTRYSTATUS, ASN_INTEGER, 1, NULL, 0, 0);
    ...
    netsnmp_register_table_data_set (netsnmp_create_handler_registration
                                     ("mteEventTable",
                                      ...
                                      mteEventTable_oid, mteEventTable_oid_len, HANDLER_CAN_RWRITE), table_set, NULL);
}
...
void init_mteEventTable (void)
{
    ...
    snmpd_register_config_handler ("notificationEvent", parse_notificationEvent,
                                   NULL, "notificationEvent NAME TRAP_OID [[-w] EXTRA_OID ...]");

    snmpd_register_config_handler ("linkUpDownNotifications",
                                   parse_linkUpDownNotifications, NULL, "linkUpDownNotifications (yes|no)");
}
...
void parse_notificationEvent (const char *token, char *line)
{
    ...
    for (row = table_set->table->first_row; row; row = row->next)
    {
        if (strcmp (row->indexes->val.string, owner) == 0 &&
            strcmp (row->indexes->next_variable->val.string, name_buf) == 0)
        {
    ...
    netsnmp_table_row_add_index (row, ASN_PRIV_IMPLIED_OCTET_STR, name_buf, strlen (name_buf));


    /* columns */
    netsnmp_set_row_column (row, COLUMN_MTEEVENTNOTIFICATION, ASN_OBJECT_ID,
                            (char *) oid_buf, oid_buf_len * sizeof (oid));
    netsnmp_set_row_column (row, COLUMN_MTEEVENTNOTIFICATIONOBJECTSOWNER, ASN_OCTET_STR, owner, strlen (owner));
    netsnmp_set_row_column (row, COLUMN_MTEEVENTNOTIFICATIONOBJECTS, ASN_OCTET_STR, name_buf, strlen (name_buf));
    ...
    /* columns */
    tc = (u_char) 0x80;
    netsnmp_set_row_column (row, COLUMN_MTEEVENTACTIONS, ASN_OCTET_STR, &tc, 1);
    tlong = MTETRIGGERENABLED_TRUE;
    netsnmp_set_row_column (row, COLUMN_MTEEVENTENABLED, ASN_INTEGER, (char *) &tlong, sizeof (tlong));
    tlong = RS_ACTIVE;
    netsnmp_set_row_column (row, COLUMN_MTEEVENTENTRYSTATUS, ASN_INTEGER, (char *) &tlong, sizeof (tlong));
    ...
    while (cp)
    {
        ...
        mte_add_object_to_table ("snmpd.conf", name_buf, oid_buf, oid_buf_len, wild);
        wild = 1;
    }
}
                ...
                oid * name_oid, size_t name_oid_len, const char *eventobjowner, const char *eventobjname)
{
    ...
    for (row = table_set->table->first_row; row; row = row->next)
    {
        if (strcmp (row->indexes->val.string, eventobjowner) == 0 &&
            strcmp (row->indexes->next_variable->val.string, eventobjname) == 0)
        {
            /* run this event */
            col1 = (netsnmp_table_data_set_storage *) row->data;

            tc = netsnmp_table_data_set_find_column (col1, COLUMN_MTEEVENTACTIONS);
            ...
            tc = netsnmp_table_data_set_find_column (col1, COLUMN_MTEEVENTENABLED);
            ...
            for (notif_row = mteEventNotif_table_set->table->first_row; notif_row; notif_row = notif_row->next)
            {
                if (strcmp (notif_row->indexes->val.string,
                            eventobjowner) == 0 &&
                    strcmp (notif_row->indexes->next_variable->val.string, eventobjname) == 0)
                {
                    ...
                    snmp_varlist_add_variable (&var_list, objid_snmptrap,
                                               ...
                                               sizeof (oid), ASN_OBJECT_ID, (u_char *) tc->data.objid, tc->data_len);
                    ...
                    if (no && no->data.string && noo && noo->data.string && name_oid)
                    {
                        char *tmpowner = netsnmp_strdup_and_null (noo->data.string,
                                                                  noo->data_len);

                        char *tmpname = netsnmp_strdup_and_null (no->data.string,
                                                                 no->data_len);

                        DEBUGMSGTL (("mteEventTable:send_events", "Adding objects for owner=%s name=%s", tmpowner,
                                     tmpname));
                        mte_add_objects (var_list, item, tmpowner, tmpname, name_oid + item->mteTriggerValueIDLen,
