     ...
     void init_expObjectConf (void)
{
     ...
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_expOTable, NULL);
}
...
char *_parse_expOCols (char *line, struct expObject *entry)
{
    void *vp;
    ...
    line = read_config_read_data (ASN_OBJECT_ID, line, &vp, &entry->expObjectID_len);
    ...
    line = read_config_read_data (ASN_OBJECT_ID, line, &vp, &entry->expObjDeltaD_len);
    ...
    line = read_config_read_data (ASN_OBJECT_ID, line, &vp, &entry->expObjCond_len);
...
void parse_expOTable (const char *token, char *line)
{
...
int store_expOTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (row = netsnmp_tdata_row_first (expObject_table_data);
         row; row = netsnmp_tdata_row_next (expObject_table_data, row))
    {
