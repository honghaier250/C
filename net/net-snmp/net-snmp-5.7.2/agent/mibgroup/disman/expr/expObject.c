 ...
 */
struct expObject *expObject_createEntry (const char *expOwner, const char *expName, long expIndex, int fixed)
{
...
netsnmp_tdata_row *expObject_createRow (const char *expOwner, const char *expName, long expIndex, int fixed)
{
    ...
    netsnmp_tdata_row_add_index (row, ASN_OCTET_STR, entry->expOwner, expOwner_len);
    if (expName)
        memcpy (entry->expName, expName, expName_len);
    netsnmp_tdata_row_add_index (row, ASN_OCTET_STR, entry->expName, expName_len);
    entry->expObjectIndex = expIndex;
    netsnmp_tdata_row_add_index (row, ASN_INTEGER, &entry->expObjectIndex, sizeof (long));
 ...
 */
void expObject_removeEntry (netsnmp_tdata_row * row)
{
    struct expObject *entry;

    if (!row)
        return;                    /* Nothing to remove */
    entry = (struct expObject *) netsnmp_tdata_remove_and_delete_row (expObject_table_data, row);
    if (entry)
    {
        if (entry->vars)
            snmp_free_varbind (entry->vars);
        if (entry->old_vars)
            snmp_free_varbind (entry->old_vars);
        if (entry->dvars)
            snmp_free_varbind (entry->dvars);
        if (entry->old_dvars)
            snmp_free_varbind (entry->old_dvars);
        if (entry->cvars)
            snmp_free_varbind (entry->cvars);
        SNMP_FREE (entry);
    }
}


netsnmp_tdata_row *expObject_getFirst (const char *expOwner, const char *expName)
{
    netsnmp_tdata_row *row;
    ...
    snmp_set_var_typed_value (&owner_var, ASN_OCTET_STR, (const u_char *) expOwner, strlen (expOwner));
    snmp_set_var_typed_value (&name_var, ASN_OCTET_STR, (const u_char *) expName, strlen (expName));
    ...
    if ((strcmp (entry->expOwner, expOwner) != 0) || (strcmp (entry->expName, expName) != 0))
        return NULL;

    return row;
}

netsnmp_tdata_row *expObject_getNext (netsnmp_tdata_row * thisRow)
{
...
netsnmp_variable_list *_expObject_buildList (oid * root, size_t root_len, size_t prefix_len,
                                             netsnmp_variable_list * template_list)
{
    ...
    if (prefix_len)
    {
        ...
        for (vp1 = template_list; vp1; vp1 = vp1->next_variable)
        {
            ...
            {
                vp2->next_variable = (netsnmp_variable_list *) SNMP_MALLOC_TYPEDEF (netsnmp_variable_list);
...
void expObject_getData (struct expExpression *expr, struct expObject *obj)
{
    ...
    else
    {
        if (!(obj->flags & EXP_OBJ_FLAG_OWILD))
             ...
             */
            var = _expObject_buildList (obj->expObjectID, obj->expObjectID_len, 0, NULL);
        else
        {
             ...
             */
            var = _expObject_buildList (obj->expObjectID, obj->expObjectID_len, expr->expPrefix_len, expr->pvars);
        }
     ...
     */
    if ((obj->expObjectSampleType != EXPSAMPLETYPE_ABSOLUTE) && (obj->flags & EXP_OBJ_FLAG_DDISC))
    {

        if (obj->flags & EXP_OBJ_FLAG_DWILD)
            var = _expObject_buildList (obj->expObjDeltaD, obj->expObjDeltaD_len, expr->expPrefix_len, expr->pvars);
        else
            var = _expObject_buildList (obj->expObjDeltaD, obj->expObjDeltaD_len, 0, NULL);
        netsnmp_query_get (var, expr->session);
        if (obj->old_dvars)
    ...
    if (obj->expObjCond_len)
    {
        if (obj->flags & EXP_OBJ_FLAG_CWILD)
            var = _expObject_buildList (obj->expObjCond, obj->expObjCond_len, expr->expPrefix_len, expr->pvars);
        else
            var = _expObject_buildList (obj->expObjCond, obj->expObjCond_len, 0, NULL);
        /*
