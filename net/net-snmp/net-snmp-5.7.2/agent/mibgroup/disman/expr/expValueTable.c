...
void init_expValueTable (void)
{
    ...
    reg =
        netsnmp_create_handler_registration ("expValueTable",
                                             ...
                                             expValueTable_oid, expValueTable_oid_len, HANDLER_CAN_RONLY);

    table_info = SNMP_MALLOC_TYPEDEF (netsnmp_table_registration_info);
    netsnmp_table_helper_add_indexes (table_info, ASN_OCTET_STR,    /* expExpressionOwner */
...
netsnmp_variable_list *expValueTable_getEntry (netsnmp_variable_list * indexes, int mode, unsigned int colnum)
{
    ...
    if (!indexes || !indexes->next_variable || !indexes->next_variable->next_variable)
    {
    ...
    {
        exp = expExpression_getEntry ((char *) indexes->val.string, (char *) indexes->next_variable->val.string);
    ...
    if (mode == MODE_GET)
    {
        ...
        if (vp->val.objid[0] != 0)
        {
            DEBUGMSGTL (("disman:expr:val", "non-zero instance (%" NETSNMP_PRIo "d)\n", vp->val.objid[0]));
        ...
        if (exp->expPrefix_len == 0)
        {
             ...
             *     expression is .0.0.0
             */
            if (vp->val_len != 3 * sizeof (oid) || vp->val.objid[1] != 0 || vp->val.objid[2] != 0)
            {
        ...
        else
        {
             ...
             */
            res = expValue_evaluateExpression (exp, vp->val.objid + 1, vp->val_len / sizeof (oid) - 1);
    ...
    else
    {
        ...
        {
            DEBUGMSGTL (("disman:expr:val", "non-zero next instance (%" NETSNMP_PRIo "d)\n", vp->val.objid[0]));
        ...
        if (plen == 0)
        {
             ...
             */
            if ((vp->val_len > 2 * sizeof (oid)) || (vp->val_len == 2 * sizeof (oid) && vp->val.objid[1] != 0))
            {
             ...
             *   and evaluate it.
             */
            snmp_set_var_typed_value (indexes, ASN_OCTET_STR, (u_char *) exp->expOwner, strlen (exp->expOwner));
            snmp_set_var_typed_value (indexes->next_variable, ASN_OCTET_STR,
                                      (u_char *) exp->expName, strlen (exp->expName));
            snmp_set_var_typed_value (vp, ASN_PRIV_IMPLIED_OBJECT_ID, (u_char *) nullInstance, 3 * sizeof (oid));
            res = expValue_evaluateExpression (exp, NULL, 0);
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    if (snmp_oid_compare (vp2->name + plen, vp2->name_length - plen, vp->name, vp->name_length) < 0)
                    {
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_GETNEXT:
            for (request = requests; request; request = request->next)
            {
                ...
                value = expValueTable_getEntry (tinfo->indexes, reqinfo->mode, tinfo->colnum);
                if (!value || !value->val.integer)
                {
                    netsnmp_set_request_error (reqinfo, request,
                                               (reqinfo->mode == MODE_GET) ? SNMP_NOSUCHINSTANCE : SNMP_ENDOFMIBVIEW);
                    continue;
                }
                ...
                switch (tinfo->colnum)
                {
                    case COLUMN_EXPVALUECOUNTER32VAL:
                        snmp_set_var_typed_integer (request->requestvb, ASN_COUNTER, *value->val.integer);
                        break;
                    case COLUMN_EXPVALUEUNSIGNED32VAL:
                        snmp_set_var_typed_integer (request->requestvb, ASN_UNSIGNED, *value->val.integer);
                        break;
                    case COLUMN_EXPVALUETIMETICKSVAL:
                        snmp_set_var_typed_integer (request->requestvb, ASN_TIMETICKS, *value->val.integer);
                        break;
                    case COLUMN_EXPVALUEINTEGER32VAL:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, *value->val.integer);
                        break;
                    case COLUMN_EXPVALUEIPADDRESSVAL:
                        snmp_set_var_typed_integer (request->requestvb, ASN_IPADDRESS, *value->val.integer);
                        break;
                    case COLUMN_EXPVALUEOCTETSTRINGVAL:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR, value->val.string, value->val_len);
                        break;
