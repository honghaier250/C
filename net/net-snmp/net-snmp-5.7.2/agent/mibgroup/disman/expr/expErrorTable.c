     ...
     void init_expErrorTable (void)
{
    ...
    reg = netsnmp_create_handler_registration ("expErrorTable",
                                               ...
                                               expErrorTable_oid, expErrorTable_oid_len, HANDLER_CAN_RWRITE);
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    case COLUMN_EXPERRORTIME:
                        snmp_set_var_typed_integer (request->requestvb, ASN_TIMETICKS, entry->expErrorTime);
                        break;
                    case COLUMN_EXPERRORINDEX:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, entry->expErrorIndex);
                        break;
                    case COLUMN_EXPERRORCODE:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, entry->expErrorCode);
                        break;
