...
netsnmp_feature_require (table_tdata_insert_row) netsnmp_feature_require (iquery_pdu_session)
     ...
     void init_expExpressionTable (void)
{
    ...
    reg = netsnmp_create_handler_registration ("expExpressionTable",
                                               ...
                                               expExpressionTable_oid, expExpressionTable_oid_len, HANDLER_CAN_RWRITE);
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                entry = (struct expExpression *) netsnmp_tdata_extract_entry (request);
                ...
                switch (tinfo->colnum)
                {
                    case COLUMN_EXPEXPRESSION:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR,
                                                  (u_char *) entry->expExpression, strlen (entry->expExpression));
                        break;
                    case COLUMN_EXPEXPRESSIONVALUETYPE:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, entry->expValueType);
                        break;
                    case COLUMN_EXPEXPRESSIONCOMMENT:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR,
                                                  (u_char *) entry->expComment, strlen (entry->expComment));
                        break;
                    case COLUMN_EXPEXPRESSIONDELTAINTERVAL:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, entry->expDeltaInterval);
                        break;
                    case COLUMN_EXPEXPRESSIONPREFIX:
                        ...
                        if (entry->expPrefix_len)
                        {
                            snmp_set_var_typed_value (request->requestvb, ASN_OBJECT_ID,
                                                      (u_char *) entry->expPrefix, entry->expPrefix_len * sizeof (oid));
                        }
                        else
                        {
                    ...
                    case COLUMN_EXPEXPRESSIONERRORS:
                        snmp_set_var_typed_integer (request->requestvb, ASN_COUNTER, entry->expErrorCount);
                        break;
                    case COLUMN_EXPEXPRESSIONENTRYSTATUS:
                        /* What would indicate 'notReady' ? */
                        ret = (entry->flags & EXP_FLAG_ACTIVE) ? RS_ACTIVE : RS_NOTINSERVICE;
        ...
        case MODE_SET_RESERVE1:
            for (request = requests; request; request = request->next)
            {
                ...
                {
                    case COLUMN_EXPEXPRESSION:
                        ret = netsnmp_check_vb_type_and_max_size (request->requestvb, ASN_OCTET_STR, EXP_STR3_LEN);
                    ...
                    case COLUMN_EXPEXPRESSIONVALUETYPE:
                        ...
                        break;
                    case COLUMN_EXPEXPRESSIONCOMMENT:
                        ret = netsnmp_check_vb_type_and_max_size (request->requestvb, ASN_OCTET_STR, EXP_STR2_LEN);
                        if (ret != SNMP_ERR_NOERROR)
                        {
                            netsnmp_set_request_error (reqinfo, request, ret);
                            return SNMP_ERR_NOERROR;
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    case COLUMN_EXPEXPRESSION:
                        ...
                        memcpy (entry->expExpression, request->requestvb->val.string, request->requestvb->val_len);
                        break;
                    case COLUMN_EXPEXPRESSIONVALUETYPE:
                    ...
                    case COLUMN_EXPEXPRESSIONCOMMENT:
                        ...
                        memcpy (entry->expComment, request->requestvb->val.string, request->requestvb->val_len);
                        break;
                    case COLUMN_EXPEXPRESSIONDELTAINTERVAL:
