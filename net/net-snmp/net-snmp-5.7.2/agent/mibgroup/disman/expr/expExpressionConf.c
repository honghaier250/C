     ...
     void init_expExpressionConf (void)
{
     ...
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_expETable, NULL);
}
...
char *_parse_expECols (char *line, struct expExpression *entry)
{
    void *vp;
    ...
    line = read_config_read_data (ASN_OBJECT_ID, line, &vp, &entry->expPrefix_len);
...
void parse_expression (const char *token, char *line)
{
    ...
    for (cp = copy_nword (line, buf, SPRINT_MAX_LEN);; cp = copy_nword (cp, buf, SPRINT_MAX_LEN))
    {
        ...
        if (buf[0] == '-')
        {
            switch (buf[1])
            {
                case 't':        /*  type */
                    ...
                    {
                        case 'c':
                            type = EXPVALTYPE_COUNTER;
                            break;
                        case 'u':
                            type = EXPVALTYPE_UNSIGNED;
                            break;
                        case 't':
                            type = EXPVALTYPE_TIMETICKS;
                            break;
                        case 'i':
                            type = EXPVALTYPE_INTEGER;
                            break;
                        case 'a':
                            type = EXPVALTYPE_IPADDRESS;
                            break;
                        case 's':
                            type = EXPVALTYPE_STRING;
                            break;
                        case 'o':
                            type = EXPVALTYPE_OID;
                            break;
                        case 'C':
                            type = EXPVALTYPE_COUNTER64;
                            break;
                    }
                    break;
                case 'u':        /*  user */
    ...
    while (cp && *cp)
    {
        ...
        object->flags |= EXP_OBJ_FLAG_VALID | EXP_OBJ_FLAG_ACTIVE | EXP_OBJ_FLAG_OWILD;
        /*
    ...
    entry->flags |= EXP_FLAG_VALID | EXP_FLAG_ACTIVE;
    expExpression_enable (entry);
    DEBUGMSG (("disman:expr:conf", "(%s, %s)\n", ename, entry->expExpression));
}

void parse_expETable (const char *token, char *line)
{
...
int store_expETable (int majorID, int minorID, void *serverarg, void *clientarg)
{
