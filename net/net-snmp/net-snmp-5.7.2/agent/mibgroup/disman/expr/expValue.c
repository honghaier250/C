...
void _expValue_setError (struct expExpression *exp, int reason,
                         oid * suffix, size_t suffix_len, netsnmp_variable_list * var);
...
void init_expValue (void)
{
     ...
     */
netsnmp_variable_list *_expValue_evalParam (netsnmp_variable_list * expIdx, int param, oid * suffix, size_t suffix_len)
{
     ...
     */
    if (!var || !expIdx || !expIdx->next_variable || !expIdx->next_variable->next_variable)
        return NULL;

    *expIdx->next_variable->next_variable->val.integer = param;
    obj = (struct expObject *) netsnmp_tdata_row_entry (netsnmp_tdata_row_get_byidx (expObject_table_data, expIdx));
    if (!obj)
    {
        /*
    ...
    if (obj->expObjectSampleType != EXPSAMPLETYPE_ABSOLUTE && obj->old_vars == NULL)
    {
        /*
    ...
    if (obj->flags & EXP_OBJ_FLAG_OWILD)
    {
        ...
        while (val_var)
        {
            if (snmp_oid_compare (val_var->name + n, val_var->name_length - n, suffix, suffix_len))
                break;
     ...
     */
    if (obj->expObjCond_len && (!cond_var || *cond_var->val.integer == 0))
    {
        /*
    ...
    if (dd_var && odd_var && *dd_var->val.integer != *odd_var->val.integer)
    {
        /*
    ...
    switch (obj->expObjectSampleType)
    {
        ...
        case EXPSAMPLETYPE_CHANGED:
            if (val_var->val_len != oval_var->val_len)
                n = 1;
            else if (memcmp (val_var->val.string, oval_var->val.string, val_var->val_len) != 0)
                n = 1;
            else
                n = 0;
     ...
     */
int _expParse_integer (char *start, char **end)
{
    int n;
...
netsnmp_variable_list *_expValue_evalOperator (netsnmp_variable_list * left,
                                               netsnmp_variable_list * op, netsnmp_variable_list * right)
{
    int n;
    ...
    switch (*op->val.integer)
    {
        case EXP_OPERATOR_ADD:
            n = *left->val.integer + *right->val.integer;
            break;
        ...
        case EXP_OPERATOR_BITNEGATE:
            n = 99;                /* *left->val.integer ~ *right->val.integer; */
            break;
...
netsnmp_variable_list *_expValue_evalFunction (netsnmp_variable_list * func)
{
...
netsnmp_variable_list *_expValue_evalExpr (netsnmp_variable_list * expIdx,
                                           char *exprRaw, char **exprEnd, oid * suffix, size_t suffix_len)
{
    ...
    for (cp1 = exprRaw; cp1 && *cp1;)
    {
        switch (*cp1)
        {
            ...
            case '(':
                 ...
                 */
                var = _expValue_evalExpr (expIdx, cp1 + 1, &cp2, suffix, suffix_len);
                if (var && var->type == ASN_NULL)
                {
                ...
                else if (*cp2 != ')')
                {
                    ...
                    snmp_set_var_typed_integer (var, ASN_INTEGER, EXPERRCODE_PARENTHESIS);
                    var->type = ASN_NULL;
            ...
            case '.':            /* OID */
                ...
                var = snmp_varlist_add_variable (&exprAlDente, NULL, 0,
                                                 ASN_OBJECT_ID, (u_char *) oid_buf, i * sizeof (oid));
                break;
              DIGIT:
            case '0':
            case '1':
            ...
            case '9':
                ...
                if (neg)
                    n = -n;
                var = snmp_varlist_add_variable (&exprAlDente, NULL, 0, ASN_INTEGER, (u_char *) & n, sizeof (n));
                vtail = var;
                var = NULL;
                neg = 0;
                cp1 = cp2;
            ...
            case '"':            /* String Constant */
                ...
                if (*cp2 != '"')
                {
                    ...
                    snmp_set_var_typed_integer (var, ASN_INTEGER, EXPERRCODE_SYNTAX);
                    var->type = ASN_NULL;
                    var->data = (void *) (cp2 - exprRaw);
                    return var;
                }
                n = cp2 - cp1;
                var = snmp_varlist_add_variable (&exprAlDente, NULL, 0, ASN_OCTET_STR, (u_char *) cp1 + 1, n);
                vtail = var;
                var = NULL;
                break;
            ...
            case 's':            /* sum / string{B,E,C}  */
                var = snmp_varlist_add_variable (&exprAlDente, NULL, 0, ASN_OCTET_STR, (u_char *) cp1, 3);
                /* XXX */
            ...
            default:
                if (isalpha (*cp1))
                {
                    ...
                    snmp_set_var_typed_integer (var, ASN_INTEGER, EXPERRCODE_FUNCTION);
                    var->type = ASN_NULL;
                    var->data = (void *) (cp1 - exprRaw);
                    return var;
                }
                else if (!isspace (*cp1))
                {
                    ...
                    snmp_set_var_typed_integer (var, ASN_INTEGER, EXPERRCODE_OPERATOR);
                    var->type = ASN_NULL;
                    var->data = (void *) (cp1 - exprRaw);
                    return var;
                }
...
netsnmp_variable_list *_expValue_evalExpr2 (netsnmp_variable_list * exprAlD)
{
    ...
    for (vp = exprAlD; vp; vp = vp1)
    {
        ...
        if (vp->type == ASN_PRIV_OPERATOR)
        {
            ...
            {
                snmp_set_var_typed_integer (vp, ASN_INTEGER, EXPERRCODE_SYNTAX);
                vp->type = ASN_NULL;
                snmp_free_var (stack);
                return vp;
            }
 ...
 *  Main API 
 * ============= */
netsnmp_variable_list *expValue_evaluateExpression (struct expExpression * exp, oid * suffix, size_t suffix_len)
{
    ...
    snmp_set_var_typed_value (&owner_var, ASN_OCTET_STR, (u_char *) exp->expOwner, strlen (exp->expOwner));
    snmp_set_var_typed_value (&name_var, ASN_OCTET_STR, (u_char *) exp->expName, strlen (exp->expName));
    n = 99;                        /* dummy value */
    snmp_set_var_typed_value (&param_var, ASN_INTEGER, (u_char *) & n, sizeof (long));
...
void
_expValue_setError (struct expExpression *exp, int reason, oid * suffix, size_t suffix_len, netsnmp_variable_list * var)
{
    if (!exp)
        return;
    exp->expErrorCount++;
