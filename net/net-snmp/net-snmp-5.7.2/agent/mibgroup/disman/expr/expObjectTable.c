     ...
     void init_expObjectTable (void)
{
    ...
    reg = netsnmp_create_handler_registration ("expObjectTable",
                                               ...
                                               expObjectTable_oid, expObjectTable_oid_len, HANDLER_CAN_RWRITE);
                ...
                switch (tinfo->colnum)
                {
                    case COLUMN_EXPOBJECTID:
                        snmp_set_var_typed_value (request->requestvb, ASN_OBJECT_ID,
                                                  (u_char *) entry->expObjectID, entry->expObjectID_len * sizeof (oid));
                        break;
                    case COLUMN_EXPOBJECTIDWILDCARD:
                        ret = (entry->flags & EXP_OBJ_FLAG_OWILD) ? TV_TRUE : TV_FALSE;
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, ret);
                        break;
                    case COLUMN_EXPOBJECTSAMPLETYPE:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, entry->expObjectSampleType);
                        break;
                    ...
                    case COLUMN_EXPOBJECTDISCONTINUITYIDWILDCARD:
                        if (entry->expObjectSampleType == 1)
                            continue;
                        ret = (entry->flags & EXP_OBJ_FLAG_DWILD) ? TV_TRUE : TV_FALSE;
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, ret);
                        break;
                    case COLUMN_EXPOBJECTDISCONTINUITYIDTYPE:
                        if (entry->expObjectSampleType == 1)
                            continue;
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, entry->expObjDiscontinuityType);
                        break;
                    case COLUMN_EXPOBJECTCONDITIONAL:
                        snmp_set_var_typed_value (request->requestvb, ASN_OBJECT_ID,
                                                  (u_char *) entry->expObjCond, entry->expObjCond_len * sizeof (oid));
                        break;
                    case COLUMN_EXPOBJECTCONDITIONALWILDCARD:
                        ret = (entry->flags & EXP_OBJ_FLAG_CWILD) ? TV_TRUE : TV_FALSE;
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, ret);
                        break;
                    case COLUMN_EXPOBJECTENTRYSTATUS:
                        /* What would indicate 'notReady' ? */
                        ret = (entry->flags & EXP_OBJ_FLAG_ACTIVE) ? RS_ACTIVE : RS_NOTINSERVICE;
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, ret);
                        break;
                }
            }
                    ...
                    case COLUMN_EXPOBJECTENTRYSTATUS:
                        switch (*request->requestvb->val.integer)
                        {
                            ...
                            case RS_CREATEANDWAIT:
                                ...
                                memcpy (expOwner, tinfo->indexes->val.string, tinfo->indexes->val_len);
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    case COLUMN_EXPOBJECTID:
                        ...
                        memcpy (entry->expObjectID, request->requestvb->val.string, request->requestvb->val_len);
                    ...
                    case COLUMN_EXPOBJECTDELTADISCONTINUITYID:
                        ...
                        memcpy (entry->expObjDeltaD, request->requestvb->val.string, request->requestvb->val_len);
                        ...
                        {
                            exp = expExpression_getEntry (entry->expOwner, entry->expName);
                            memcpy (exp->expPrefix, entry->expObjDeltaD, MAX_OID_LEN * sizeof (oid));
                    ...
                    case COLUMN_EXPOBJECTDISCONTINUITYIDWILDCARD:
                        ...
                        break;
                    case COLUMN_EXPOBJECTDISCONTINUITYIDTYPE:
                        entry->expObjDiscontinuityType = *request->requestvb->val.integer;
                        break;
                    case COLUMN_EXPOBJECTCONDITIONAL:
                        memset (entry->expObjCond, 0, sizeof (entry->expObjCond));
                        memcpy (entry->expObjCond, request->requestvb->val.string, request->requestvb->val_len);
