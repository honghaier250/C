...
netsnmp_tdata *expr_table_data;
     ...
     */
void init_expr_table_data (void)
{
...
void _mteExpr_dump (void)
{
    ...
    for (row = netsnmp_tdata_row_first (expr_table_data); row; row = netsnmp_tdata_row_next (expr_table_data, row))
    {
        ...
        DEBUGMSG (("disman:expr:dump", "(%s, %s)", row->indexes->val.string, row->indexes->next_variable->val.string));
 ...
 */
struct expExpression *expExpression_createEntry (const char *expOwner, const char *expName, int fixed)
{
    netsnmp_tdata_row *row;
...
netsnmp_tdata_row *expExpression_createRow (const char *expOwner, const char *expName, int fixed)
{
 ...
 */
void expExpression_removeEntry (netsnmp_tdata_row * row)
{
    struct expExpression *entry;

    if (!row)
        return;                    /* Nothing to remove */
    entry = (struct expExpression *) netsnmp_tdata_remove_and_delete_row (expr_table_data, row);
    if (entry)
    {
...
{
    return (struct expExpression *) netsnmp_tdata_row_entry (netsnmp_tdata_row_first (expr_table_data));
}

struct expExpression *expExpression_getNextEntry (const char *owner, const char *name)
{
    ...
    snmp_set_var_typed_value (&owner_var, ASN_OCTET_STR, (const u_char *) owner, strlen (owner));
    snmp_set_var_typed_value (&name_var, ASN_OCTET_STR, (const u_char *) name, strlen (name));
    owner_var.next_variable = &name_var;

    return (struct expExpression *)
        netsnmp_tdata_row_entry (netsnmp_tdata_row_next_byidx (expr_table_data, &owner_var));
}

struct expExpression *expExpression_getEntry (const char *owner, const char *name)
{
    netsnmp_variable_list owner_var, name_var;
    ...
    return (struct expExpression *) netsnmp_tdata_row_entry (netsnmp_tdata_row_get_byidx (expr_table_data, &owner_var));
}
 ...
 */
void expExpression_getData (unsigned int reg, void *clientarg)
{
    ...
    if ((entry->expExpression[0] == '\0') || !(entry->flags & EXP_FLAG_ACTIVE) || !(entry->flags & EXP_FLAG_VALID))
        return;
     ...
     */
    for (row = expObject_getFirst (entry->expOwner, entry->expName); row; row = expObject_getNext (row))
    {
...
void expExpression_enable (struct expExpression *entry)
{
    ...
    {
        entry->alarm = snmp_alarm_register (entry->expDeltaInterval, SA_REPEAT, expExpression_getData, entry);
        expExpression_getData (entry->alarm, (void *) entry);
    }
}

void expExpression_disable (struct expExpression *entry)
{
    if (!entry)
        return;
