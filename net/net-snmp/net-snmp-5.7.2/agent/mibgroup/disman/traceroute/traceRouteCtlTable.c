...
#include "header_complex.h"
     oid traceRouteCtlTable_variables_oid[] = { 1, 3, 6, 1, 2, 1, 81, 1, 2 };
...
int traceRouteResultsTable_add (struct traceRouteCtlTable_data *thedata);

int traceRouteResultsTable_del (struct traceRouteCtlTable_data *thedata);
...
void init_traceRouteCtlTable (void)
{
     ...
     */
    REGISTER_MIB ("traceRouteCtlTable", traceRouteCtlTable_variables, variable2, traceRouteCtlTable_variables_oid);

    /*
     * register our config handler(s) to deal with registrations 
     */
    snmpd_register_config_handler ("traceRouteCtlTable", parse_traceRouteCtlTable, NULL, NULL);

    /*
     * we need to be called back later to store our data 
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_traceRouteCtlTable, NULL);

    DEBUGMSGTL (("traceRouteCtlTable", "done.\n"));
}


void init_trResultsTable (struct traceRouteCtlTable_data *item)
{
    ...
    host = (char *) malloc (sizeof (char) * (item->traceRouteCtlTargetAddressLen + 1));
    ...
    memset (host, '\0', sizeof (char) * (item->traceRouteCtlTargetAddressLen + 1));
    ...
    StorageTmp->traceRouteCtlOwnerIndex = (char *) malloc (sizeof (char) * (item->traceRouteCtlOwnerIndexLen + 1));
    if (StorageTmp->traceRouteCtlOwnerIndex == NULL)
    {
        DEBUGMSGTL (("traceRouteCtlTable", "traceRouteCtlOwnerIndex malloc %s\n", strerror (errno)));
        exit (1);
    }

    memcpy (StorageTmp->traceRouteCtlOwnerIndex, item->traceRouteCtlOwnerIndex, item->traceRouteCtlOwnerIndexLen + 1);
    StorageTmp->traceRouteCtlOwnerIndex[item->traceRouteCtlOwnerIndexLen] = '\0';
    StorageTmp->traceRouteCtlOwnerIndexLen = item->traceRouteCtlOwnerIndexLen;

    StorageTmp->traceRouteCtlTestName = (char *) malloc (sizeof (char) * (item->traceRouteCtlTestNameLen + 1));
    if (StorageTmp->traceRouteCtlTestName == NULL)
    {
        DEBUGMSGTL (("traceRouteCtlTable", "traceRouteCtlTestName malloc %s\n", strerror (errno)));
        exit (1);
    }

    memcpy (StorageTmp->traceRouteCtlTestName, item->traceRouteCtlTestName, item->traceRouteCtlTestNameLen + 1);
    StorageTmp->traceRouteCtlTestName[item->traceRouteCtlTestNameLen] = '\0';
    StorageTmp->traceRouteCtlTestNameLen = item->traceRouteCtlTestNameLen;

    StorageTmp->traceRouteResultsOperStatus = 1;

    if (item->traceRouteCtlTargetAddressType == 1 || item->traceRouteCtlTargetAddressType == 16)
    {
        ...
        else
        {
            ...
            StorageTmp->traceRouteResultsIpTgtAddr =
                (char *) malloc (sizeof (char) * (strlen (inet_ntoa (to->sin_addr)) + 1));
            if (StorageTmp->traceRouteResultsIpTgtAddr == NULL)
            {
                DEBUGMSGTL (("traceRouteCtlTable", "traceRouteResultsIpTgtAddr malloc %s\n", strerror (errno)));
                exit (1);
            }

            memcpy (StorageTmp->traceRouteResultsIpTgtAddr,
                    inet_ntoa (to->sin_addr), strlen (inet_ntoa (to->sin_addr)) + 1);
            StorageTmp->traceRouteResultsIpTgtAddr[strlen (inet_ntoa (to->sin_addr))] = '\0';
            StorageTmp->traceRouteResultsIpTgtAddrLen = strlen (inet_ntoa (to->sin_addr));
        }
    }
    if (item->traceRouteCtlTargetAddressType == 2)
    {
        ...
        register struct sockaddr_in6 *to = (struct sockaddr_in6 *) &whereto;
        ...
        if (inet_pton (AF_INET6, host, &to->sin6_addr) > 0)
        {
            ...
            StorageTmp->traceRouteResultsIpTgtAddr = (char *) malloc (sizeof (char) * (strlen (host) + 1));
            if (StorageTmp->traceRouteResultsIpTgtAddr == NULL)
            {
                DEBUGMSGTL (("traceRouteCtlTable", "traceRouteResultsIpTgtAddr malloc %s\n", strerror (errno)));
                exit (1);
            }
            memset (StorageTmp->traceRouteResultsIpTgtAddr, '\0', sizeof (char) * (strlen (host) + 1));
        ...
        else
        {
            ...
            if (hp != NULL)
            {
                ...
                StorageTmp->traceRouteResultsIpTgtAddrType = 2;
                StorageTmp->traceRouteResultsIpTgtAddr = (char *) malloc (sizeof (char) * (strlen (hostname) + 1));
                if (StorageTmp->traceRouteResultsIpTgtAddr == NULL)
                {
                    DEBUGMSGTL (("traceRouteCtlTable", "traceRouteResultsIpTgtAddr malloc %s\n", strerror (errno)));
                    exit (1);
                ...
                memset (StorageTmp->traceRouteResultsIpTgtAddr, '\0', sizeof (char) * (strlen (host) + 1));
                memcpy (StorageTmp->traceRouteResultsIpTgtAddr, hostname, strlen (hostname) + 1);
                StorageTmp->traceRouteResultsIpTgtAddr[strlen (hostname)] = '\0';
                StorageTmp->traceRouteResultsIpTgtAddrLen = strlen (hostname);
            }
            else
            {
                DEBUGMSGTL (("traceRouteCtlTable", "traceroute: unknown host %s\n", host));
    ...
    if (item->traceRouteResults != NULL)
    {
        ...
        {
            DEBUGMSGTL (("traceRouteResultsTable", "init an entry error\n"));
        }
    }
...
int modify_trResultsOper (struct traceRouteCtlTable_data *thedata, long val)
{
    netsnmp_variable_list *vars = NULL;
    ...
    if ((StorageTmp = header_complex_get (traceRouteResultsTableStorage, vars)) == NULL)
    {
        snmp_free_varbind (vars);
        vars = NULL;
...
struct traceRouteCtlTable_data *create_traceRouteCtlTable_data (void)
{
    ...
    StorageNew->traceRouteCtlDescrLen = strlen (StorageNew->traceRouteCtlDescr);
 ...
 */
int traceRouteCtlTable_add (struct traceRouteCtlTable_data *thedata)
{
    netsnmp_variable_list *vars = NULL;
    ...
    if (header_complex_add_data (&traceRouteCtlTableStorage, vars, thedata) == NULL)
    {
        vars = NULL;
...
int traceRouteResultsTable_add (struct traceRouteCtlTable_data *thedata)
{
    ...
    if (thedata->traceRouteResults != NULL)
    {
        ...
        header_complex_add_data (&traceRouteResultsTableStorage, vars_list, p);
...
int traceRouteProbeHistoryTable_add (struct traceRouteProbeHistoryTable_data *thedata)
{
    netsnmp_variable_list *vars_list = NULL;
    ...
    if (thedata != NULL)
    {
        ...
        if (header_complex_add_data (&traceRouteProbeHistoryTableStorage, vars_list, thedata) == NULL)
        {
            vars_list = NULL;
            return SNMP_ERR_INCONSISTENTNAME;
        }
...
int traceRouteProbeHistoryTable_addall (struct traceRouteCtlTable_data *thedata)
{
    netsnmp_variable_list *vars_list = NULL;
    ...
    if (thedata->traceRouteProbeHis != NULL)
        do
        {
            ...
            if (header_complex_add_data (&traceRouteProbeHistoryTableStorage, vars_list, p) == NULL)
            {
                vars_list = NULL;
                return SNMP_ERR_INCONSISTENTNAME;
            }
            else
            {
                ...
                if (traceRouteProbeHistoryTableStorage != NULL)
                    do
                    {
                        DEBUGMSGTL (("traceRouteProbeHistoryTable", "adding data,vars_oid="));
                        DEBUGMSGOID (("traceRouteProbeHistoryTable", temp->name, temp->namelen));
...
int traceRouteHopsTable_add (struct traceRouteHopsTable_data *thedata)
{
    netsnmp_variable_list *vars_list = NULL;
    ...
    if (thedata != NULL)
    {
        ...
        if (header_complex_add_data (&traceRouteHopsTableStorage, vars_list, thedata) == NULL)
        {
            vars_list = NULL;
            return SNMP_ERR_INCONSISTENTNAME;
        }
...
int traceRouteHopsTable_addall (struct traceRouteCtlTable_data *thedata)
{
    netsnmp_variable_list *vars_list = NULL;
    ...
    if (thedata->traceRouteHops != NULL)
    {
        do
        {
            ...
            if (header_complex_add_data (&traceRouteHopsTableStorage, vars_list, p) == NULL)
            {
                vars_list = NULL;
                return SNMP_ERR_INCONSISTENTNAME;
            }
...
unsigned long traceRouteProbeHistoryTable_count (struct traceRouteCtlTable_data *thedata)
{
    ...
    vars = NULL;
    for (hciptr2 = traceRouteProbeHistoryTableStorage; hciptr2 != NULL; hciptr2 = hciptr2->next)
    {
        if (snmp_oid_compare (newoid, newoid_len, hciptr2->name, newoid_len) == 0)
        {
            count = count + 1;
        }
    }
...
unsigned long traceRouteHopsTable_count (struct traceRouteCtlTable_data *thedata)
{
    struct header_complex_index *hciptr2 = NULL;
    ...
    vars = NULL;
    for (hciptr2 = traceRouteHopsTableStorage; hciptr2 != NULL; hciptr2 = hciptr2->next)
    {
        if (snmp_oid_compare (newoid, newoid_len, hciptr2->name, newoid_len) == 0)
        {
            count = count + 1;
...
void traceRouteProbeHistoryTable_delLast (struct traceRouteCtlTable_data *thedata)
{
    struct header_complex_index *hciptr2 = NULL;
    ...
    for (hcilast = hciptr2 = traceRouteProbeHistoryTableStorage; hciptr2 != NULL; hciptr2 = hciptr2->next)
    {
        if (snmp_oid_compare (newoid, newoid_len, hciptr2->name, newoid_len) == 0)
        {

            StorageTmp =
                header_complex_get_from_oid (traceRouteProbeHistoryTableStorage, hciptr2->name, hciptr2->namelen);
    ...
    DEBUGMSGTL (("traceRouteProbeHistoryTable", "delete the last one success!\n"));
    vars = NULL;
}



void traceRouteCtlTable_cleaner (struct header_complex_index *thestuff)
{
    ...
    {
        StorageDel = header_complex_extract_entry (&traceRouteCtlTableStorage, hciptr);
        if (StorageDel != NULL)
        {
 ...
 */
void parse_traceRouteCtlTable (const char *token, char *line)
{
    size_t tmpint;

    struct traceRouteCtlTable_data *StorageTmp = SNMP_MALLOC_STRUCT (traceRouteCtlTable_data);
    ...
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->traceRouteCtlTargetAddressType, &tmpint);

    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->traceRouteCtlTargetAddress, &StorageTmp->traceRouteCtlTargetAddressLen);
    if (StorageTmp->traceRouteCtlTargetAddress == NULL)
    {
        config_perror ("invalid specification for traceRouteCtlTargetAddress");
        return;
    }

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->traceRouteCtlByPassRouteTable, &tmpint);
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->traceRouteCtlDataSize, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->traceRouteCtlTimeOut, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->traceRouteCtlProbesPerHop, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->traceRouteCtlPort, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->traceRouteCtlMaxTtl, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->traceRouteCtlDSField, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->traceRouteCtlSourceAddressType, &tmpint);

    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->traceRouteCtlSourceAddress, &StorageTmp->traceRouteCtlSourceAddressLen);
    if (StorageTmp->traceRouteCtlSourceAddress == NULL)
    {
        config_perror ("invalid specification for traceRouteCtlSourceAddress");
        return;
    }

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->traceRouteCtlIfIndex, &tmpint);

    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->traceRouteCtlMiscOptions, &StorageTmp->traceRouteCtlMiscOptionsLen);
    if (StorageTmp->traceRouteCtlMiscOptions == NULL)
    {
        config_perror ("invalid specification for traceRouteCtlMiscOptions");
        return;
    }

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->traceRouteCtlMaxFailures, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->traceRouteCtlDontFragment, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->traceRouteCtlInitialTtl, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->traceRouteCtlFrequency, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->traceRouteCtlStorageType, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->traceRouteCtlAdminStatus, &tmpint);

    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->traceRouteCtlDescr, &StorageTmp->traceRouteCtlDescrLen);
    ...
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->traceRouteCtlMaxRows, &tmpint);

    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->traceRouteCtlTrapGeneration, &StorageTmp->traceRouteCtlTrapGenerationLen);
    if (StorageTmp->traceRouteCtlTrapGeneration == NULL)
    {
        config_perror ("invalid specification for traceRouteCtlTrapGeneration");
        return;
    }

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->traceRouteCtlCreateHopsEntries, &tmpint);

    line =
        read_config_read_data (ASN_OBJECT_ID, line, &StorageTmp->traceRouteCtlType, &StorageTmp->traceRouteCtlTypeLen);
    ...
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->traceRouteCtlRowStatus, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->traceRouteProbeHistoryMaxIndex, &tmpint);
 ...
 */
int store_traceRouteCtlTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (hcindex = traceRouteCtlTableStorage; hcindex != NULL; hcindex = hcindex->next)
    {
        ...
        if (StorageTmp->storageType != ST_READONLY)
        {
            ...
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->traceRouteCtlTestName, &StorageTmp->traceRouteCtlTestNameLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->traceRouteCtlTargetAddressType, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->traceRouteCtlTargetAddress,
                                        &StorageTmp->traceRouteCtlTargetAddressLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->traceRouteCtlByPassRouteTable, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->traceRouteCtlDataSize, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->traceRouteCtlTimeOut, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->traceRouteCtlProbesPerHop, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->traceRouteCtlPort, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->traceRouteCtlMaxTtl, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->traceRouteCtlDSField, &tmpint);

            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->traceRouteCtlSourceAddressType, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->traceRouteCtlSourceAddress,
                                        &StorageTmp->traceRouteCtlSourceAddressLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->traceRouteCtlIfIndex, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->traceRouteCtlMiscOptions,
                                        &StorageTmp->traceRouteCtlMiscOptionsLen);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->traceRouteCtlMaxFailures, &tmpint);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->traceRouteCtlDontFragment, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->traceRouteCtlInitialTtl, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->traceRouteCtlFrequency, &tmpint);

            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->traceRouteCtlStorageType, &tmpint);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->traceRouteCtlAdminStatus, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->traceRouteCtlDescr, &StorageTmp->traceRouteCtlDescrLen);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->traceRouteCtlMaxRows, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->traceRouteCtlTrapGeneration,
                                        &StorageTmp->traceRouteCtlTrapGenerationLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->traceRouteCtlCreateHopsEntries, &tmpint);
            cptr =
                read_config_store_data (ASN_OBJECT_ID, cptr,
                                        &StorageTmp->traceRouteCtlType, &StorageTmp->traceRouteCtlTypeLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->traceRouteCtlRowStatus, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->traceRouteProbeHistoryMaxIndex, &tmpint);
                                       ...
                                       size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
     ...
     */
    if ((StorageTmp =
         header_complex (traceRouteCtlTableStorage, vp, name, length, exact, var_len, write_method)) == NULL)
    {
...
int traceRouteResultsTable_del (struct traceRouteCtlTable_data *thedata)
{
    ...
    for (hciptr2 = traceRouteResultsTableStorage; hciptr2 != NULL; hciptr2 = hciptr2->next)
    {
        if (snmp_oid_compare (newoid, newoid_len, hciptr2->name, newoid_len) == 0)
        {
            header_complex_extract_entry (&traceRouteResultsTableStorage, hciptr2);
...
int traceRouteProbeHistoryTable_del (struct traceRouteCtlTable_data *thedata)
{
    struct header_complex_index *hciptr2 = NULL;
    ...
    for (hciptr2 = traceRouteProbeHistoryTableStorage; hciptr2 != NULL; hciptr2 = hciptr2->next)
    {
        if (snmp_oid_compare (newoid, newoid_len, hciptr2->name, newoid_len) == 0)
        {
            header_complex_extract_entry (&traceRouteProbeHistoryTableStorage, hciptr2);
            DEBUGMSGTL (("traceRouteProbeHistoryTable", "delete  success!\n"));
...
int traceRouteHopsTable_del (struct traceRouteCtlTable_data *thedata)
{
    struct header_complex_index *hciptr2 = NULL;
...
void send_traceRoute_trap (struct traceRouteCtlTable_data *item, oid * trap_oid, size_t trap_oid_len)
{
    ...
    oid traceRouteCtlTargetAddress[] = { 1, 3, 6, 1, 2, 1, 81, 1, 2, 1, 4 };
    oid traceRouteHopsIpTgAddress[] = { 1, 3, 6, 1, 2, 1, 81, 1, 5, 1, 3 };
    ...
    snmp_varlist_add_variable (&var_list, objid_snmptrap,
                               ...
                               ASN_OBJECT_ID, (u_char *) trap_oid, trap_oid_len * sizeof (oid));
    ...
    header_complex_generate_oid (newoid, &newoid_len,
                                 traceRouteCtlTargetAddress, sizeof (traceRouteCtlTargetAddress) / sizeof (oid), vars);

    snmp_varlist_add_variable (&var_list, newoid,
                               newoid_len,
                               ASN_OCTET_STR,
                               (u_char *) item->traceRouteCtlTargetAddress, item->traceRouteCtlTargetAddressLen);

    for (hciptr = traceRouteHopsTableStorage; hciptr != NULL; hciptr = hciptr->next)
    {
        memset (indexoid, '\0', MAX_OID_LEN * sizeof (oid));
        header_complex_generate_oid (indexoid, &indexoid_len, NULL, 0, vars);
        if (snmp_oid_compare (indexoid, indexoid_len, hciptr->name, indexoid_len) == 0)
        {
            StorageHops =
                (struct traceRouteHopsTable_data *)
                header_complex_get_from_oid (traceRouteHopsTableStorage, hciptr->name, hciptr->namelen);
            memset (tempoid, '\0', MAX_OID_LEN * sizeof (oid));
            header_complex_generate_oid (tempoid, &tempoid_len,
                                         traceRouteHopsIpTgAddress,
                                         sizeof (traceRouteHopsIpTgAddress) / sizeof (oid), vars);
            ...
            memset (newoid, '\0', MAX_OID_LEN * sizeof (oid));
            header_complex_generate_oid (newoid, &newoid_len, tempoid, tempoid_len, var_hops);
            snmp_varlist_add_variable (&var_list, newoid, newoid_len,
                                       ASN_OCTET_STR,
                                       (u_char *) StorageHops->traceRouteHopsIpTgtAddress,
                                       StorageHops->traceRouteHopsIpTgtAddressLen);
                                      ...
                                      size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    size_t newlen = name_len - (sizeof (traceRouteCtlTable_variables_oid) / sizeof (oid) + 3 - 1);
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            {
                snmp_log (LOG_ERR, "write to traceRouteCtlTargetAddressType not ASN_INTEGER\n");
                                  ...
                                  u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            {
                snmp_log (LOG_ERR, "write to traceRouteCtlTargetAddress not ASN_OCTET_STR\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
        ...
        case ACTION:
            ...
            StorageTmp->traceRouteCtlTargetAddress = (char *) malloc (var_val_len + 1);
            if (StorageTmp->traceRouteCtlTargetAddress == NULL)
            {
                exit (1);
            }
            memcpy (StorageTmp->traceRouteCtlTargetAddress, var_val, var_val_len);
                             ...
                             u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            {
                snmp_log (LOG_ERR, "write to traceRouteCtlDataSize not ASN_UNSIGNED\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                         ...
                         u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_UNSIGNED)
            {
                snmp_log (LOG_ERR, "write to traceRouteCtlTargetAddressType not ASN_UNSIGNED\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                                      ...
                                      size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_INTEGER)
            {
                snmp_log (LOG_ERR, "write to traceRouteCtlMaxRows not ASN_INTEGER\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                                  ...
                                  u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        ...
        case ACTION:
            ...
            StorageTmp->traceRouteCtlSourceAddress = (char *) malloc (var_val_len + 1);
            if (StorageTmp->traceRouteCtlSourceAddress == NULL)
            {
                exit (1);
            }
            memcpy (StorageTmp->traceRouteCtlSourceAddress, var_val, var_val_len + 1);
                                ...
                                u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        ...
        case ACTION:
            ...
            StorageTmp->traceRouteCtlMiscOptions = (char *) malloc (var_val_len + 1);
            if (StorageTmp->traceRouteCtlMiscOptions == NULL)
            {
                exit (1);
            }
            memcpy (StorageTmp->traceRouteCtlMiscOptions, var_val, var_val_len + 1);
                                ...
                                u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_UNSIGNED)
            {
                snmp_log (LOG_ERR, "write to traceRouteCtlTrapTestFailureFilter not ASN_UNSIGNED\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                              ...
                              u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_UNSIGNED)
            {
                snmp_log (LOG_ERR, "write to traceRouteCtlSourceAddressType not ASN_UNSIGNED\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            if ((StorageTmp->traceRouteCtlStorageType == 2
                 || StorageTmp->traceRouteCtlStorageType == 3) && (set_value == 4 || set_value == 5))
            {
                                ...
                                u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_INTEGER)
            {
                snmp_log (LOG_ERR, "write to traceRouteCtlIfIndex not ASN_INTEGER\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
        ...
        case COMMIT:
            ...
            StorageNew = header_complex_get (traceRouteResultsTableStorage, vars);

            if (StorageTmp->traceRouteCtlAdminStatus == 1 && StorageTmp->traceRouteCtlRowStatus == RS_ACTIVE)
            {
                if (StorageNew == NULL)
                    init_trResultsTable (StorageTmp);
                else
                {
                    StorageTmp->traceRouteResults->traceRouteResultsOperStatus = 1;
                    modify_trResultsOper (StorageTmp, 1);
                }
                if (StorageTmp->traceRouteCtlFrequency != 0)
                    StorageTmp->timer_id =
                        snmp_alarm_register (StorageTmp->traceRouteCtlFrequency, SA_REPEAT, run_traceRoute, StorageTmp);
                else
                    StorageTmp->timer_id = snmp_alarm_register (1, 0, run_traceRoute, StorageTmp);

            }
            else if (StorageTmp->traceRouteCtlAdminStatus == 2 && StorageTmp->traceRouteCtlRowStatus == RS_ACTIVE)
            {
                ...
                {
                    StorageTmp->traceRouteResults->traceRouteResultsOperStatus = 2;
                            ...
                            u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_UNSIGNED)
            {
                snmp_log (LOG_ERR, "write to traceRouteCtlDSField not ASN_UNSIGNED\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                                   ...
                                   u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        ...
        case ACTION:
            ...
            StorageTmp->traceRouteCtlTrapGeneration = (char *) malloc (var_val_len + 1);
            if (StorageTmp->traceRouteCtlTrapGeneration == NULL)
            {
                exit (1);
            }

            memcpy (StorageTmp->traceRouteCtlTrapGeneration, var_val, var_val_len + 1);
                                      ...
                                      size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_INTEGER)
            {
                snmp_log (LOG_ERR, "write to traceRouteCtlDSField not ASN_INTEGER\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                         ...
                         u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            {
                snmp_log (LOG_ERR, "write to traceRouteCtlType not ASN_OBJECT_ID\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
                              ...
                              u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    {
        snmp_log (LOG_ERR, "write to traceRouteCtlRowStatus not ASN_INTEGER\n");
        return SNMP_ERR_WRONGTYPE;
    }
    if (StorageTmp && StorageTmp->storageType == ST_READONLY)
    {
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            else
            {
                ...
                if (StorageTmp->traceRouteCtlRowStatus == RS_ACTIVE && set_value != RS_DESTROY)
                {
                    /*
        ...
        case RESERVE2:
            ...
            if (StorageTmp == NULL)
            {
                ...
                if (header_complex_parse_oid
                    (&
                     (name
                      [sizeof (traceRouteCtlTable_variables_oid) / sizeof (oid) + 2]), newlen, vars) != SNMPERR_SUCCESS)
                {
                    /*
                     * XXX: free, zero vars 
                     */
                ...
                if (vars->val_len <= 32)
                {
                    StorageNew->traceRouteCtlOwnerIndex = malloc (vars->val_len + 1);
                    memcpy (StorageNew->traceRouteCtlOwnerIndex, vars->val.string, vars->val_len);
                ...
                else
                {
                    ...
                    memcpy (StorageNew->traceRouteCtlOwnerIndex, vars->val.string, 32);
                ...
                if (vars->val_len <= 32)
                {
                    StorageNew->traceRouteCtlTestName = malloc (vars->val_len + 1);
                    memcpy (StorageNew->traceRouteCtlTestName, vars->val.string, vars->val_len);
                ...
                else
                {
                    ...
                    memcpy (StorageNew->traceRouteCtlTestName, vars->val.string, 32);
        ...
        case ACTION:
            ...
            if (StorageTmp == NULL)
            {
                ...
                {
#if 1
                    DEBUGMSGTL (("traceRouteCtlTable", "write_traceRouteCtlRowStatus entering new=%d...  \n", action));
#endif
            ...
            else
            {
                ...
                hciptr = header_complex_find_entry (traceRouteCtlTableStorage, StorageTmp);
                StorageDel = header_complex_extract_entry (&traceRouteCtlTableStorage, hciptr);
        ...
        case COMMIT:
            ...
            {
                if (StorageTmp && StorageTmp->traceRouteCtlRowStatus == RS_CREATEANDGO)
                {
                    StorageTmp->traceRouteCtlRowStatus = RS_ACTIVE;
                }
                else if (StorageTmp && StorageTmp->traceRouteCtlRowStatus == RS_CREATEANDWAIT)
                {
                    ...
                    if (StorageTmp->traceRouteCtlFrequency != 0)
                        StorageTmp->timer_id =
                            snmp_alarm_register (StorageTmp->traceRouteCtlFrequency,
                                                 SA_REPEAT, run_traceRoute, StorageTmp);
                    else
                        StorageTmp->timer_id = snmp_alarm_register (1, 0, run_traceRoute, StorageTmp);
...
void run_traceRoute (unsigned int clientreg, void *clientarg)
{
    ...
    if (item->traceRouteCtlInitialTtl > item->traceRouteCtlMaxTtl)
    {
        DEBUGMSGTL (("traceRouteCtlTable",
                     ...
                     item->traceRouteCtlInitialTtl, item->traceRouteCtlMaxTtl));
        return;
    }
    ...
    if (item->traceRouteCtlTargetAddressType == 1 || item->traceRouteCtlTargetAddressType == 16)
    {
        ...
        hostname = (char *) malloc (item->traceRouteCtlTargetAddressLen + 1);
        if (hostname == NULL)
            return;
        memcpy (hostname, item->traceRouteCtlTargetAddress, item->traceRouteCtlTargetAddressLen + 1);
        ...
        if (outip == NULL)
        {
            DEBUGMSGTL (("traceRouteCtlTable", "malloc: %s\n", strerror (errno)));
            exit (1);
        }
        ...
        outudp->len = htons ((u_short) (packlen - (sizeof (*outip) + optlen)));
        outdata = (struct outdata *) (outudp + 1);

        cp = "icmp";
        if ((pe = getprotobyname (cp)) == NULL)
        {
            DEBUGMSGTL (("traceRouteCtlTable", "unknown protocol %s\n", cp));
            exit (1);
        }
        ...
        {
            DEBUGMSGTL (("traceRouteCtlTable", "open \"%s\": %s\n", devnull, strerror (errno)));
            exit (1);
        }
        if ((s = socket (AF_INET, SOCK_RAW, pe->p_proto)) < 0)
        {
            DEBUGMSGTL (("traceRouteCtlTable", "icmp socket: %s\n", strerror (errno)));
            exit (1);
        }
        if (options & SO_DEBUG)
        ...
        if (sndsock < 0)
        {
            DEBUGMSGTL (("traceRouteCtlTable", "raw socket: %s\n", strerror (errno)));
            exit (1);
        }
        ...
        {
            DEBUGMSGTL (("traceRouteCtlTable", "SO_SNDBUF: %s\n", strerror (errno)));
            exit (1);
        }
#endif
#ifdef IP_HDRINCL
        ...
        {
            DEBUGMSGTL (("traceRouteCtlTable", "IP_HDRINCL: %s\n", strerror (errno)));
            exit (1);
        }
#else
#ifdef IP_TOS
        ...
        {
            DEBUGMSGTL (("traceRouteCtlTable", "setsockopt tos %d: %s\n", strerror (errno)));
            exit (1);
        }
#endif
#endif
        ...
        if (n < 0)
        {
            DEBUGMSGTL (("traceRouteCtlTable", " ifaddrlist: %s\n", errbuf));
            exit (1);
        }
        if (n == 0)
        {
            DEBUGMSGTL (("traceRouteCtlTable", " Can't find any network interfaces\n"));
        ...
        if (device != NULL)
        {
            ...
            if (i <= 0)
            {
                DEBUGMSGTL (("traceRouteCtlTable", " Can't find interface %.32s\n", device));
        ...
        if (source == NULL)
        {
            ...
            {
                DEBUGMSGTL (("traceRouteCtlTable", " findsaddr: %s\n", err));
                exit (1);
            }
        ...
        else
        {
            ...
            if (device != NULL)
            {
                ...
                if (i <= 0)
                {
                    DEBUGMSGTL (("traceRouteCtlTable", " %s is not on interface %.32s\n", source, device));
        ...
        {
            DEBUGMSGTL (("traceRouteCtlTable", " bind: %s\n", strerror (errno)));
            exit (1);
        }
#endif
        DEBUGMSGTL (("traceRouteCtlTable", " to %s (%s)", hostname, inet_ntoa (to->sin_addr)));

        if (source)
            DEBUGMSGTL (("traceRouteCtlTable", " from %s", source));

        DEBUGMSGTL (("traceRouteCtlTable", ", %lu hops max, %d byte packets\n", item->traceRouteCtlMaxTtl, packlen));
        (void) fflush (stderr);
        ...
        if ((StorageResults = header_complex_get (traceRouteResultsTableStorage, vars_results)) == NULL)
            return;
        snmp_free_varbind (vars_results);
        vars_results = NULL;


        for (ttl = item->traceRouteCtlInitialTtl; ttl <= item->traceRouteCtlMaxTtl; ++ttl)
        {
            ...
            if (item->traceRouteCtlCreateHopsEntries == 1)
            {
                if (ttl == item->traceRouteCtlInitialTtl)
                {
                    ...
                    for (hciptr2 = traceRouteHopsTableStorage; hciptr2 != NULL; hciptr2 = hciptr2->next)
                    {
                        if (snmp_oid_compare (newoid, newoid_len, hciptr2->name, newoid_len) == 0)
                        {
                            StorageTmp = header_complex_extract_entry (&traceRouteHopsTableStorage, hciptr2);

                            old_HopsAddress[k] = (char *) malloc (StorageTmp->traceRouteHopsIpTgtAddressLen + 1);
                            ...
                            memdup ((u_char **) & old_HopsAddress[k],
                                    ...
                                    StorageTmp->traceRouteHopsIpTgtAddressLen + 1);
                            old_HopsAddress[k][StorageTmp->traceRouteHopsIpTgtAddressLen] = '\0';
                ...
                temp->traceRouteCtlOwnerIndex = (char *) malloc (item->traceRouteCtlOwnerIndexLen + 1);
                memcpy (temp->traceRouteCtlOwnerIndex,
                        item->traceRouteCtlOwnerIndex, item->traceRouteCtlOwnerIndexLen + 1);
                temp->traceRouteCtlOwnerIndex[item->traceRouteCtlOwnerIndexLen] = '\0';
                temp->traceRouteCtlOwnerIndexLen = item->traceRouteCtlOwnerIndexLen;

                temp->traceRouteCtlTestName = (char *) malloc (item->traceRouteCtlTestNameLen + 1);
                memcpy (temp->traceRouteCtlTestName, item->traceRouteCtlTestName, item->traceRouteCtlTestNameLen + 1);
                temp->traceRouteCtlTestName[item->traceRouteCtlTestNameLen] = '\0';
                temp->traceRouteCtlTestNameLen = item->traceRouteCtlTestNameLen;
                ...
                if (item->traceRouteHops != NULL)
                    ...
                    if (traceRouteHopsTable_add (current_temp) != SNMPERR_SUCCESS)
            ...
            for (probe = 0; probe < nprobes; ++probe)
            {
                ...
                send_probe (to, ++seq, ttl, &t1, outip, outudp, packlen,
                            optlen, hostname, ident, sndsock, port, outdata);
                ++sentfirst;
                while ((cc = wait_for_reply (s, from, &t1, packet, waittime)) != 0)
                {
                    (void) gettimeofday (&t2, &tz);
                    timep = 0;
                    time (&timep);
                    i = packet_ok (packet, cc, from, seq, ident, pmtu, port);
                    /*
                     * Skip short packet 
                     */
                    if (i == 0)
                    ...
                    if (!gotlastaddr || from->sin_addr.s_addr != lastaddr)
                    {
                        ...
                        hlen = ip->ip_hl << 2;
                        cc -= hlen;
                        DEBUGMSGTL (("traceRouteCtlTable", " %s", inet_ntoa (from->sin_addr)));
                    ...
                    else
                    {
                        ...
                        if (Rtt > maxRtt)
                            maxRtt = Rtt;
                        sumRtt = (sumRtt) + Rtt;
                        averageRtt = round ((double) (sumRtt) / (double) responseProbe);
                        sumOfSquare = sumOfSquare + Rtt * Rtt;
                    }

                    StorageResults->traceRouteResultsCurProbeCount = probe + 1;
                    if (i == -2)
                    {
#ifndef ARCHAIC
                        ip = (struct ip *) packet;
                ...
                if (item->traceRouteCtlMaxRows != 0)
                {
                    ...
                    temp_his = SNMP_MALLOC_STRUCT (traceRouteProbeHistoryTable_data);
                    temp_his->traceRouteCtlOwnerIndex = (char *) malloc (item->traceRouteCtlOwnerIndexLen + 1);
                    memcpy (temp_his->traceRouteCtlOwnerIndex,
                            item->traceRouteCtlOwnerIndex, item->traceRouteCtlOwnerIndexLen + 1);
                    temp_his->traceRouteCtlOwnerIndex[item->traceRouteCtlOwnerIndexLen] = '\0';
                    temp_his->traceRouteCtlOwnerIndexLen = item->traceRouteCtlOwnerIndexLen;

                    temp_his->traceRouteCtlTestName = (char *) malloc (item->traceRouteCtlTestNameLen + 1);
                    memcpy (temp_his->traceRouteCtlTestName,
                            item->traceRouteCtlTestName, item->traceRouteCtlTestNameLen + 1);
                    temp_his->traceRouteCtlTestName[item->traceRouteCtlTestNameLen] = '\0';
                    temp_his->traceRouteCtlTestNameLen = item->traceRouteCtlTestNameLen;

                    /* add lock to protect */
                    pthread_mutex_t counter_mutex = PTHREAD_MUTEX_INITIALIZER;

                    pthread_mutex_lock (&counter_mutex);
                    if (item->traceRouteProbeHistoryMaxIndex >= (unsigned long) (2147483647))
                        item->traceRouteProbeHistoryMaxIndex = 0;
                    temp_his->traceRouteProbeHistoryIndex = ++(item->traceRouteProbeHistoryMaxIndex);
                    pthread_mutex_unlock (&counter_mutex);
                    /* endsadsadsad */
                    ...
                    temp_his->traceRouteProbeHistoryHAddr = (char *) malloc (strlen (inet_ntoa (from->sin_addr)) + 1);
                    strcpy (temp_his->traceRouteProbeHistoryHAddr, (inet_ntoa (from->sin_addr)));
                    temp_his->traceRouteProbeHistoryHAddr[strlen (inet_ntoa (from->sin_addr))] = '\0';
                    temp_his->traceRouteProbeHistoryHAddrLen = strlen (inet_ntoa (from->sin_addr));
                    ...
                    memdup (&temp_his->traceRouteProbeHistoryTime,
                            date_n_time (&timep, &temp_his->traceRouteProbeHistoryTimeLen), 11);
                    if (probe == 0)
                    ...
                    {
                        if (traceRouteProbeHistoryTable_count (item) < item->traceRouteCtlMaxRows)
                        {
                            if (traceRouteProbeHistoryTable_add (current_temp_his) != SNMPERR_SUCCESS)
                                DEBUGMSGTL (("traceRouteProbeHistoryTable", "registered an entry error\n"));
                        }
                        else
                        {
                ...
                if (item->traceRouteCtlCreateHopsEntries == 1)
                {
                    ...
                    if ((current = header_complex_get (traceRouteHopsTableStorage, vars_hops)) == NULL)
                        return;
                    snmp_free_varbind (vars_hops);
                    vars_hops = NULL;

                    current->traceRouteHopsIpTgtAddressType = 1;
                    current->traceRouteHopsIpTgtAddress = (char *) malloc (strlen (inet_ntoa (from->sin_addr)) + 1);
                    current->traceRouteHopsIpTgtAddress = strdup (inet_ntoa (from->sin_addr));
                    current->traceRouteHopsIpTgtAddress[strlen (inet_ntoa (from->sin_addr))] = '\0';
                    current->traceRouteHopsIpTgtAddressLen = strlen (inet_ntoa (from->sin_addr));
                    if (count != 0)
                    {
                        if (strcmp (old_HopsAddress[index - 1], current->traceRouteHopsIpTgtAddress) != 0)
                            flag = 1;
                    }
                    ...
                    memdup (&current->traceRouteHopsLastGoodProbe,
                            date_n_time (&timep, &current->traceRouteHopsLastGoodProbeLen), 11);
                }
            ...
            if (got_there || (unreachable > 0 && unreachable >= nprobes - 1))
            {
                ...
                if (got_there != 0)
                {
                    StorageResults->traceRouteResultsTestAttempts = StorageResults->traceRouteResultsTestAttempts + 1;

                    StorageResults->traceRouteResultsTestSuccesses = StorageResults->traceRouteResultsTestSuccesses + 1;

                    StorageResults->traceRouteResultsLastGoodPath_time = timep;
                    memdup (&StorageResults->traceRouteResultsLastGoodPath,
                            date_n_time (&timep, &StorageResults->traceRouteResultsLastGoodPathLen), 11);
                    if ((item->traceRouteCtlTrapGeneration[0] & TRACEROUTETRAPGENERATION_TESTCOMPLETED) != 0)
                    {
                        DEBUGMSGTL (("traceRouteProbeHistoryTable", "TEST completed!\n"));
                        send_traceRoute_trap (item, traceRouteTestCompleted,
                                              sizeof (traceRouteTestCompleted) / sizeof (oid));
                    }
                }
                ...
                {
                    StorageResults->traceRouteResultsTestAttempts = StorageResults->traceRouteResultsTestAttempts + 1;
                    if ((item->traceRouteCtlTrapGeneration[0] & TRACEROUTETRAPGENERATION_TESTFAILED) != 0)
                    {
                        DEBUGMSGTL (("traceRouteProbeHistoryTable", "test Failed!\n"));
                        send_traceRoute_trap (item, traceRouteTestFailed, sizeof (traceRouteTestFailed) / sizeof (oid));
                    }
                }
                break;

            }
            else if (ttl == item->traceRouteCtlMaxTtl && (probe + 1) == nprobes)
            {
                StorageResults->traceRouteResultsTestAttempts = StorageResults->traceRouteResultsTestAttempts + 1;
        ...
        if (flag == 1)
        {
            ...
            send_traceRoute_trap (item, traceRoutePathChange, sizeof (traceRoutePathChange) / sizeof (oid));
        }
    ...
    if (item->traceRouteCtlTargetAddressType == 2)
    {
        ...
        int i = 0, on = 0, probe = 0, seq = 0, tos = 0, ttl = 0;
        ...
        Fprintf (stderr, "traceroute to %s (%s)", hostname, inet_ntop (AF_INET6, &to->sin6_addr, pa, 64));

        Fprintf (stderr, " from %s", inet_ntop (AF_INET6, &saddr.sin6_addr, pa, 64));
        ...
        for (ttl = item->traceRouteCtlInitialTtl; ttl <= item->traceRouteCtlMaxTtl; ++ttl)
        {
            ...
            for (probe = 0; probe < nprobes; ++probe)
            {
                ...
                send_probe_v6 (++seq, ttl, sendbuff, ident, &tz, sndsock, datalen, &whereto, hostname);
                reset_timer = 1;

                while ((cc = wait_for_reply_v6 (icmp_sock, &from, reset_timer, waittime, icmp_sock, packet)) != 0)
                {
                    gettimeofday (&t2, &tz);
                    timep = 0;
                    time (&timep);
                    if ((i = packet_ok_v6 (packet, cc, &from, seq, &t1, ident)))
                    {
                        reset_timer = 1;
                        if (memcmp (&from.sin6_addr, &lastaddr, sizeof (struct in6_addr)))
                        {

                            memcpy (&lastaddr, &from.sin6_addr, sizeof (struct in6_addr));
                        }
                ...
                if (item->traceRouteCtlMaxRows != 0)
                {
                    ...
                    temp_his->traceRouteProbeHistoryHAddr =
                        (char *) malloc (strlen (inet_ntop (AF_INET6, &from.sin6_addr, pa, 64)) + 1);
                    temp_his->traceRouteProbeHistoryHAddr = strdup (inet_ntop (AF_INET6, &from.sin6_addr, pa, 64));
                    temp_his->traceRouteProbeHistoryHAddr[strlen
                                                          (inet_ntop (AF_INET6, &from.sin6_addr, pa, 64))] = '\0';
                    temp_his->traceRouteProbeHistoryHAddrLen = strlen (inet_ntop (AF_INET6, &from.sin6_addr, pa, 64));
                ...
                if (item->traceRouteCtlCreateHopsEntries == 1)
                {
                    ...
                    current->traceRouteHopsIpTgtAddress =
                        (char *) malloc (strlen (inet_ntop (AF_INET6, &from.sin6_addr, pa, 64)) + 1);
                    current->traceRouteHopsIpTgtAddress = strdup (inet_ntop (AF_INET6, &from.sin6_addr, pa, 64));
                    current->traceRouteHopsIpTgtAddress[strlen (inet_ntop (AF_INET6, &from.sin6_addr, pa, 64))] = '\0';
                    ...
                    current->traceRouteHopsIpTgtAddressLen = strlen (inet_ntop (AF_INET6, &from.sin6_addr, pa, 64));
                    current->traceRouteHopsMinRtt = minRtt;
                    current->traceRouteHopsMaxRtt = maxRtt;
                    current->traceRouteHopsAverageRtt = averageRtt;
                    current->traceRouteHopsRttSumOfSquares = sumOfSquare;
...
wait_for_reply (register int sock, register struct sockaddr_in *fromp,
                register const struct timeval *tp, u_char * packet, int waittime)
{
    fd_set fds;
    ...
    if (select (sock + 1, &fds, NULL, NULL, &wait) > 0)
        cc = recvfrom (sock, (char *) packet, 512, 0, (struct sockaddr *) fromp, &fromlen);
    return (cc);
}


int
wait_for_reply_v6 (int sock, struct sockaddr_in6 *from, int reset_timer, int waittime, int icmp_sock, u_char * packet)
{
    fd_set fds;
    ...
    {
        cc = recvfrom (icmp_sock, (char *) packet, 512, 0, (struct sockaddr *) from, &fromlen);
    }
...
send_probe (struct sockaddr_in *whereto, register int seq, int ttl,
            ...
            char *hostname, u_short ident, int sndsock, u_short port, struct outdata *outdata)
{
    register int cc = 0;
    ...
    outip->ip_sum = in_checksum ((u_short *) outip, sizeof (*outip) + optlen);
    if (outip->ip_sum == 0)
        outip->ip_sum = 0xffff;
    ...
    *outip = tip;
    ...
    cc = sendto (sndsock, (char *) outudp, packlen - (sizeof (*outip) + optlen), 0, whereto, sizeof (*whereto));
    if (cc > 0)
...
send_probe_v6 (int seq, int ttl, char *sendbuff, pid_t ident,
               struct timezone *tz, int sndsock, int datalen, struct sockaddr_in6 *whereto, char *hostname)
{
    ...
    do
    {
        i = sendto (sndsock, sendbuff, datalen, 0, (struct sockaddr *) whereto, sizeof (struct sockaddr_in6));
    }
...
unsigned long deltaT (struct timeval *t1p, struct timeval *t2p)
{
    register unsigned long dt;

    dt = (unsigned long) ((long) (t2p->tv_sec - t1p->tv_sec) * 1000 + (long) (t2p->tv_usec - t1p->tv_usec) / 1000);
    return (dt);
}
           ...
           register int seq, u_short ident, int pmtu, u_short port)
{
    ...
    if ((type == ICMP_TIMXCEED && code == ICMP_TIMXCEED_INTRANS) || type == ICMP_UNREACH || type == ICMP_ECHOREPLY)
    {
         ...
         */
        if (hlen + 12 <= cc &&
            hip->ip_p == IPPROTO_UDP && up->source == htons (ident) && up->dest == htons (port + seq))
...
int packet_ok_v6 (u_char * buf, int cc, struct sockaddr_in6 *from, int seq, struct timeval *tv, pid_t ident)
{
...
u_short in_checksum (register u_short * addr, register int len)
{
...
int ifaddrlist (register struct ifaddrlist **ipaddrp, register char *errbuf)
{
...
#define MAX_IPADDR (sizeof(ibuf) / sizeof(ibuf[0]))
    ...
    {
        if (errno == EINVAL)
            (void) sprintf (errbuf, "SIOCGIFCONF: ifreq struct too small (%d bytes)", (int) sizeof (ibuf));
        else
    ...
    for (; ifrp < ifend; ifrp = ifnext)
    {
        ...
        if (ioctl (fd, SIOCGIFFLAGS, (char *) &ifr) < 0)
        {
            ...
            (void) sprintf (errbuf, "SIOCGIFFLAGS: %.*s: %s",
                            (int) sizeof (ifr.ifr_name), ifr.ifr_name, strerror (errno));
            (void) close (fd);
            return (-1);
        }
        ...
        {
            (void) sprintf (errbuf, "SIOCGIFADDR: %s: %s", device, strerror (errno));
            (void) close (fd);
            return (-1);
        }
