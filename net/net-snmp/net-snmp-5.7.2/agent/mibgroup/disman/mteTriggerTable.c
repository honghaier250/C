...
netsnmp_feature_require (check_vb_uint) netsnmp_feature_require (string_time_to_secs)
...
void init_mteTriggerTable (void)
{
    ...
    snmp_log (LOG_WARNING, "mteTriggerTable has been disabled because " "the callback transport is not available.\n");
    return;
#endif
     ...
     */
    REGISTER_MIB ("mteTriggerTable", mteTriggerTable_variables, variable2, mteTriggerTable_variables_oid);
    ...
    snmpd_register_config_handler ("defaultMonitors", parse_default_monitors, NULL, "yes|no");

    /*
     * we need to be called back later to store our data 
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_mteTriggerTable, NULL);


    /*
     * place any other initialization junk you need here 
     */
    se_add_pair_to_slist ("mteBooleanOperators", strdup ("!="), MTETRIGGERBOOLEANCOMPARISON_UNEQUAL);
    se_add_pair_to_slist ("mteBooleanOperators", strdup ("=="), MTETRIGGERBOOLEANCOMPARISON_EQUAL);
    se_add_pair_to_slist ("mteBooleanOperators", strdup ("<"), MTETRIGGERBOOLEANCOMPARISON_LESS);
    se_add_pair_to_slist ("mteBooleanOperators", strdup ("<="), MTETRIGGERBOOLEANCOMPARISON_LESSOREQUAL);
    se_add_pair_to_slist ("mteBooleanOperators", strdup (">"), MTETRIGGERBOOLEANCOMPARISON_GREATER);
    se_add_pair_to_slist ("mteBooleanOperators", strdup (">="), MTETRIGGERBOOLEANCOMPARISON_GREATEROREQUAL);

    DEBUGMSGTL (("mteTriggerTable", "done.\n"));
}

struct mteTriggerTable_data *create_mteTriggerTable_data (void)
{
    ...
    StorageNew->mteTriggerContextNameWildcard = MTETRIGGERCONTEXTNAMEWILDCARD_FALSE;
    ...
    StorageNew->mteTriggerDeltaDiscontinuityIDLen = sizeof (sysUpTimeInstance) / sizeof (oid);
    StorageNew->mteTriggerDeltaDiscontinuityIDWildcard = TV_FALSE;
    StorageNew->mteTriggerDeltaDiscontinuityIDType = MTETRIGGERDELTADISCONTINUITYIDTYPE_TIMETICKS;
    StorageNew->mteTriggerExistenceTest = strdup ("");
    StorageNew->mteTriggerExistenceTest[0] = (char) (MTETRIGGEREXISTENCETEST_PRESENT | MTETRIGGEREXISTENCETEST_ABSENT);
    StorageNew->mteTriggerExistenceTestLen = 1;
    StorageNew->mteTriggerExistenceStartup = strdup ("");
    StorageNew->mteTriggerExistenceStartup[0] = (char) (MTETRIGGEREXISTENCESTARTUP_PRESENT);
    ...
    StorageNew->mteTriggerBooleanComparison = MTETRIGGERBOOLEANCOMPARISON_UNEQUAL;
    ...
    StorageNew->mteTriggerThresholdStartup = MTETRIGGERTHRESHOLDSTARTUP_RISINGORFALLING;
 ...
 */
int mteTriggerTable_add (struct mteTriggerTable_data *thedata)
{
...
void parse_default_monitors (const char *token, char *line)
{
    char buf[SPRINT_MAX_LEN];

    char *mte_default_user = netsnmp_ds_get_string (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_INTERNAL_SECNAME);
...
void parse_simple_monitor (const char *token, char *line)
{
        ...
        switch (buf[1])
        {
            ...
            case 'o':
                ...
                if (StorageNew->mteTriggerObjectsOwnerLen == 0)
                {
                    ...
                    StorageNew->mteTriggerObjectsOwnerLen = strlen ("snmpd.conf");
                }
    ...
    else
    {
        ...
        if (!cp)
        {
            ...
            if (eventname[0] != '\0')
            {
                StorageNew->mteTriggerExistenceEventOwner = strdup ("snmpd.conf");
                StorageNew->mteTriggerExistenceEventOwnerLen = strlen (StorageNew->mteTriggerExistenceEventOwner);
                StorageNew->mteTriggerExistenceEvent = strdup (eventname);
                StorageNew->mteTriggerExistenceEventLen = strlen (eventname);
            }
         ...
         */
        cp = copy_nword (cp, buf, sizeof (buf));
        if ((StorageNew->mteTriggerBooleanComparison = se_find_value_in_slist ("mteBooleanOperators", buf)) == -1)
        {
        ...
        if (eventname[0] != '\0')
        {
            StorageNew->mteTriggerBooleanEventOwner = strdup ("snmpd.conf");
            StorageNew->mteTriggerBooleanEventOwnerLen = strlen (StorageNew->mteTriggerBooleanEventOwner);
            StorageNew->mteTriggerBooleanEvent = strdup (eventname);
            StorageNew->mteTriggerBooleanEventLen = strlen (eventname);
        }
    }
    mteTriggerTable_add (StorageNew);
...
void parse_mteTriggerTable (const char *token, char *line)
{
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line, &StorageTmp->mteTriggerComment, &StorageTmp->mteTriggerCommentLen);
    ...
    line = read_config_read_data (ASN_OCTET_STR, line, &StorageTmp->mteTriggerTest, &StorageTmp->mteTriggerTestLen);
    ...
    line =
        read_config_read_data (ASN_OBJECT_ID, line, &StorageTmp->mteTriggerValueID, &StorageTmp->mteTriggerValueIDLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->mteTriggerTargetTag, &StorageTmp->mteTriggerTargetTagLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->mteTriggerContextName, &StorageTmp->mteTriggerContextNameLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->mteTriggerObjectsOwner, &StorageTmp->mteTriggerObjectsOwnerLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line, &StorageTmp->mteTriggerObjects, &StorageTmp->mteTriggerObjectsLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->mteTriggerExistenceTest, &StorageTmp->mteTriggerExistenceTestLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->mteTriggerExistenceStartup, &StorageTmp->mteTriggerExistenceStartupLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               ...
                               &StorageTmp->mteTriggerExistenceObjectsOwnerLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->mteTriggerExistenceObjects, &StorageTmp->mteTriggerExistenceObjectsLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               ...
                               &StorageTmp->mteTriggerExistenceEventOwnerLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->mteTriggerExistenceEvent, &StorageTmp->mteTriggerExistenceEventLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               ...
                               &StorageTmp->mteTriggerBooleanObjectsOwnerLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->mteTriggerBooleanObjects, &StorageTmp->mteTriggerBooleanObjectsLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->mteTriggerBooleanEventOwner, &StorageTmp->mteTriggerBooleanEventOwnerLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->mteTriggerBooleanEvent, &StorageTmp->mteTriggerBooleanEventLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               ...
                               &StorageTmp->mteTriggerThresholdObjectsOwnerLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->mteTriggerThresholdObjects, &StorageTmp->mteTriggerThresholdObjectsLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               ...
                               &StorageTmp->mteTriggerThresholdRisingEventOwnerLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               ...
                               &StorageTmp->mteTriggerThresholdRisingEventLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               ...
                               &StorageTmp->mteTriggerThresholdFallingEventOwnerLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               ...
                               &StorageTmp->mteTriggerThresholdDeltaRisingEventLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               ...
                               &StorageTmp->mteTriggerThresholdDeltaFallingEventLen);
    ...
    if (StorageTmp->have_copied_auth_info)
    {
        ...
        if (!netsnmp_tdomain_support (tmpoid, tmpint, &StorageTmp->pdu_tDomain, &StorageTmp->pdu_tDomainLen))
        {
         ...
         * can be NULL?  Yes.  
         */
        line = read_config_read_data (ASN_OCTET_STR, line, &(StorageTmp->pdu_transport), &StorageTmp->pdu_transportLen);
        ...
        line =
            read_config_read_data (ASN_OCTET_STR, line,
                                   &StorageTmp->pdu_securityName, &StorageTmp->pdu_securityNameLen);
     ...
     */
    if (StorageTmp->mteTriggerEnabled == MTETRIGGERENABLED_TRUE && StorageTmp->mteTriggerEntryStatus == RS_ACTIVE)
 ...
 */
int store_mteTriggerTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (hcindex = mteTriggerTableStorage; hcindex != NULL; hcindex = hcindex->next)
    {
        ...
        if (StorageTmp->storageType == ST_NONVOLATILE)
        {
            ...
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->mteTriggerName, &StorageTmp->mteTriggerNameLen);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->mteTriggerComment, &StorageTmp->mteTriggerCommentLen);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->mteTriggerTest, &StorageTmp->mteTriggerTestLen);
            ...
            cptr =
                read_config_store_data (ASN_OBJECT_ID, cptr,
                                        &StorageTmp->mteTriggerValueID, &StorageTmp->mteTriggerValueIDLen);
            ...
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->mteTriggerObjects, &StorageTmp->mteTriggerObjectsLen);
            ...
            cptr =
                read_config_store_data (ASN_INTEGER, cptr,
                                        &StorageTmp->mteTriggerDeltaDiscontinuityIDWildcard, &tmpint);
            ...
            if (StorageTmp->have_copied_auth_info)
            {
                ...
                cptr =
                    read_config_store_data (ASN_OBJECT_ID, cptr,
                                            (void *) (&StorageTmp->pdu_tDomain), &StorageTmp->pdu_tDomainLen);
                cptr =
                    read_config_store_data (ASN_OCTET_STR, cptr,
                                            &StorageTmp->pdu_transport, &StorageTmp->pdu_transportLen);
                cptr =
                    read_config_store_data (ASN_OCTET_STR, cptr,
                                            &StorageTmp->pdu_community, &StorageTmp->pdu_community_len);
                cptr =
                    read_config_store_data (ASN_OCTET_STR, cptr,
                                            &StorageTmp->pdu_securityName, &StorageTmp->pdu_securityNameLen);
            }
 ...
 */
unsigned char *var_mteTriggerTable (struct variable *vp,
                                    oid * name,
                             ...
                             u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    StorageTmp =
        header_complex (mteTriggerTableStorage, NULL,
                        &name[sizeof (mteTriggerTable_variables_oid) / sizeof (oid) + 3 - 1], &newlen, 1, NULL, NULL);
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            else
            {
                ...
                if (StorageTmp->mteTriggerEntryStatus == RS_ACTIVE && set_value != RS_DESTROY)
                {
                    /*
        ...
        case RESERVE2:
            ...
            if (StorageTmp == NULL)
            {
                ...
                if (header_complex_parse_oid
                    (&
                     (name
                      [sizeof (mteTriggerTable_variables_oid) / sizeof (oid) + 2]), newlen, vars) != SNMPERR_SUCCESS)
                {
                    /*
                     * XXX: free, zero vars 
                     */
        ...
        case ACTION:
            ...
            else
            {
                 ...
                 */
                hciptr = header_complex_find_entry (mteTriggerTableStorage, StorageTmp);
                StorageDel = header_complex_extract_entry (&mteTriggerTableStorage, hciptr);
            }
            break;
        ...
        case COMMIT:
            ...
            else
            {
                ...
                else if (StorageTmp && StorageTmp->mteTriggerEntryStatus == RS_CREATEANDWAIT)
                {
                    StorageTmp->mteTriggerEntryStatus = RS_NOTINSERVICE;
                }
            }
            if (StorageTmp && StorageTmp->mteTriggerEntryStatus == RS_ACTIVE && !StorageTmp->have_copied_auth_info)
            {
                ...
                if (pdu)
                {
                    ...
                    {
                        StorageTmp->pdu_transport = malloc (pdu->transport_data_length);
                        memcpy (StorageTmp->pdu_transport, pdu->transport_data, pdu->transport_data_length);
                    }
                    StorageTmp->pdu_transportLen = pdu->transport_data_length;
                    if (pdu->community)
                    {
                        StorageTmp->pdu_community = calloc (1, pdu->community_len + 1);
                        memcpy (StorageTmp->pdu_community, pdu->community, pdu->community_len);
                    ...
                    if (pdu->securityName)
                    {
                        StorageTmp->pdu_securityName = calloc (1, pdu->securityNameLen + 1);
                        memcpy (StorageTmp->pdu_securityName, pdu->securityName, pdu->securityNameLen);
...
send_mte_trap (struct mteTriggerTable_data *item,
               ...
               long *value, const char *objowner, const char *objname, const char *reason)
{
    ...
    snmp_varlist_add_variable (&var_list, mteHotTrigger,
                               ...
                               ASN_OCTET_STR, (u_char *) item->mteTriggerName, item->mteTriggerNameLen);
    ...
    snmp_varlist_add_variable (&var_list, mteHotContextName,
                               ...
                               ASN_OCTET_STR, (u_char *) item->mteTriggerContextName, item->mteTriggerContextNameLen);

    snmp_varlist_add_variable (&var_list, mteHotOID,
                               sizeof (mteHotOID) / sizeof (oid),
                               ASN_OBJECT_ID, (u_char *) name_oid, sizeof (oid) * name_oid_len);
    ...
    else
    {
        ...
        snmp_varlist_add_variable (&var_list, mteHotValue,
                                   sizeof (mteHotValue) / sizeof (oid), ASN_INTEGER, (u_char *) value, sizeof (value));
    }
    ...
    mte_add_objects (var_list, item, item->mteTriggerObjectsOwner,
                     ...
                     name_oid + item->mteTriggerValueIDLen, name_oid_len - item->mteTriggerValueIDLen);
    /*
...
void last_state_clean (void *data)
{
 ...
 */
netsnmp_pdu *mte_get_response (struct mteTriggerTable_data *item, netsnmp_pdu * pdu)
{
    ...
    memdup ((u_char **) & pdu->transport_data, item->pdu_transport, item->pdu_transportLen);
    ...
    memdup ((u_char **) & pdu->contextName, item->mteTriggerContextName, item->mteTriggerContextNameLen);
    pdu->contextNameLen = item->mteTriggerContextNameLen;
    memdup ((u_char **) & pdu->securityName, item->pdu_securityName, item->pdu_securityNameLen);
    ...
    if (item->mteTriggerTargetTagLen == 0)
    {
        ...
        if (status != SNMP_ERR_NOERROR || response->errstat != SNMP_ERR_NOERROR)
        {
            /*
             * xxx 
             */
            ...
            if (response)
            {
                DEBUGMSGTL (("mteTriggerTable",
                             ...
                             status, errstr,
                             response->errstat, snmp_api_errstring (response->errstat), response->version));
            }
            else
            {
                DEBUGMSGTL (("mteTriggerTable",
...
int mte_discontinuity_occurred (struct mteTriggerTable_data *item)
{
    ...
    if (item->mteTriggerDeltaDiscontinuityIDLen == 0 ||
        (snmp_oid_compare (item->mteTriggerDeltaDiscontinuityID,
                           ...
                           sysUpTimeInstance, sizeof (sysUpTimeInstance) / sizeof (oid)) == 0))
    {
    ...
    else
    {
        ...
        snmp_add_null_var (pdu, item->mteTriggerDeltaDiscontinuityID, item->mteTriggerDeltaDiscontinuityIDLen);
        ...
        else
        {
            if (item->mteTriggerDeltaDiscontinuityIDType ==
                ...
                item->mteTriggerDeltaDiscontinuityIDType == MTETRIGGERDELTADISCONTINUITYIDTYPE_TIMESTAMP)
            {
                if (response->errstat == SNMPERR_SUCCESS)
                {
                    if (response->variables != NULL && response->variables->type == ASN_TIMETICKS)
                    {
                    ...
                    else
                    {
                        ...
                        if (response->variables != NULL &&
                            (response->variables->type == SNMP_NOSUCHOBJECT
                             ...
                             SNMP_NOSUCHINSTANCE || response->variables->type == SNMP_ENDOFMIBVIEW))
                        {
                            /*
                             * noSuchName I guess.  
                             */
...
void mte_run_trigger (unsigned int clientreg, void *clientarg)
{

    struct mteTriggerTable_data *item = (struct mteTriggerTable_data *) clientarg;
    ...
    do
    {
        ...
        if (item->mteTriggerValueIDWildcard == TV_TRUE &&
            ((response->variables->type >= SNMP_NOSUCHOBJECT &&
             ...
             snmp_oid_compare (item->mteTriggerValueID,
                               item->mteTriggerValueIDLen, response->variables->name, item->mteTriggerValueIDLen) != 0))
        {
        ...
        if (((item->mteTriggerTest[0] & MTETRIGGERTEST_BOOLEAN) ||
            ...
            response->errstat == SNMPERR_SUCCESS && !mte_is_integer_type (response->variables->type))
        {
            long failure = MTE_FAILURE_BADTYPE;

            send_mte_trap (item, mteTriggerFailure,
                           sizeof (mteTriggerFailure) / sizeof (oid),
                           next_oid, next_oid_len, &failure, NULL, NULL, "failure: bad type");
            /*
         ...
         */
        if (response->errstat == SNMPERR_SUCCESS && response->variables->val.integer)
            memdup ((unsigned char **) &value,
                    (unsigned char *) response->variables->val.integer, sizeof (*response->variables->val.integer));
        else
            value = NULL;

        snprint_variable (buf, sizeof (buf), next_oid, next_oid_len, response->variables);
        buf[sizeof (buf) - 1] = '\0';
        DEBUGMSGTL (("mteTriggerTable", "received %s (type %d)\n", buf, response->variables->type));

        /*
         * see if we have old values for this 
         */
        laststate = header_complex_get_from_oid (item->hc_storage_old, next_oid, next_oid_len);
        if (laststate)
        {
        ...
        if (item->mteTriggerTest[0] & MTETRIGGERTEST_EXISTENCE)
        {
            if ((item->mteTriggerExistenceTest[0] &
                ...
                && value && !old_value &&
                (item->started || (item->mteTriggerExistenceStartup[0] & MTETRIGGEREXISTENCESTARTUP_PRESENT)))
            {
                /*
                ...
                send_mte_trap (item, mteTriggerFired,
                               ...
                               item->mteTriggerExistenceObjects, "existence: present");
                run_mte_events (item, next_oid, next_oid_len,
                                item->mteTriggerExistenceEventOwner, item->mteTriggerExistenceEvent);
            }

            if ((item->mteTriggerExistenceTest[0] &
                 MTETRIGGEREXISTENCETEST_CHANGED) && value && old_value && *old_value != *value)
            {
                /*
                 * XXX: if mteTriggerExistenceTest is not "present", for
                 * example, and then turned on when has been previously
                ...
                send_mte_trap (item, mteTriggerFired,
                               ...
                               next_oid, next_oid_len,
                               value, item->mteTriggerExistenceObjectsOwner,
                               item->mteTriggerExistenceObjects, "existence: changed");
                run_mte_events (item, next_oid, next_oid_len,
                                item->mteTriggerExistenceEventOwner, item->mteTriggerExistenceEvent);
            }
        }
        ...
        if ((item->mteTriggerTest[0] & MTETRIGGERTEST_BOOLEAN) &&
            ((item->mteTriggerSampleType ==
             ...
             || (item->mteTriggerSampleType == MTETRIGGERSAMPLETYPE_DELTAVALUE && value && old_value)))
        {
            ...
            if (boolresult &&
                ((item->mteTriggerBooleanStartup ==
                  MTETRIGGERBOOLEANSTARTUP_TRUE && lastbool == (char) -1) || lastbool != boolresult))
            {
                send_mte_trap (item, mteTriggerFired,
                               sizeof (mteTriggerFired) / sizeof (oid),
                               next_oid, next_oid_len,
                ...
                run_mte_events (item, next_oid, next_oid_len,
                                item->mteTriggerBooleanEventOwner, item->mteTriggerBooleanEvent);
            }
            ...
            if (((item->started == MTE_STARTED && laststate &&
                 ...
                 (item->started != MTE_STARTED &&
                  (item->mteTriggerThresholdStartup ==
                   ...
                   || item->mteTriggerThresholdStartup ==
                   MTETRIGGERTHRESHOLDSTARTUP_RISINGORFALLING))) && (*value >= item->mteTriggerThresholdRising))
            {
                send_mte_trap (item, mteTriggerRising,
                               ...
                               item->mteTriggerThresholdObjects, "threshold: rising");
                run_mte_events (item, next_oid, next_oid_len,
                                item->mteTriggerThresholdRisingEventOwner, item->mteTriggerThresholdRisingEvent);
            }
            if (((item->started == MTE_STARTED && laststate &&
                 ...
                 (item->started != MTE_STARTED &&
                  (item->mteTriggerThresholdStartup ==
                   ...
                   || item->mteTriggerThresholdStartup ==
                   MTETRIGGERTHRESHOLDSTARTUP_RISINGORFALLING))) && (*value <= item->mteTriggerThresholdFalling))
            {
                send_mte_trap (item, mteTriggerFalling,
                               sizeof (mteTriggerFalling) / sizeof (oid),
                               next_oid, next_oid_len, value,
                               item->mteTriggerThresholdObjectsOwner,
                               item->mteTriggerThresholdObjects, "threshold: falling");
                run_mte_events (item, next_oid, next_oid_len,
                                item->mteTriggerThresholdFallingEventOwner, item->mteTriggerThresholdFallingEvent);
            }
        ...
        if (value)
        {
            ...
            header_complex_add_data_by_oid (&item->hc_storage, next_oid, next_oid_len, new_last_state);

            /*
             * set our notion of the current known threshold state 
             */
            if (lastthresh == MTE_THRESHOLD_LOW &&
                *value >= item->mteTriggerThresholdRising && *value > item->mteTriggerThresholdFalling)
    ...
    if (item->mteTriggerExistenceTest[0] & MTETRIGGEREXISTENCETEST_ABSENT)
    {
         ...
         * XXX: broken 
         */
        if ((item->mteTriggerExistenceStartup[0] & MTETRIGGEREXISTENCESTARTUP_ABSENT))
        {
            /*
        ...
        for (iter = item->hc_storage_old; iter; iter = iter->next)
        {
            ...
            send_mte_trap (item, mteTriggerFired,
                           ...
                           item->mteTriggerExistenceObjects, "existence: absent");
        }
...
void mte_enable_trigger (struct mteTriggerTable_data *item)
{
    ...
    if (item->mteTriggerFrequency > 0)
    {
        DEBUGMSGTL (("mteTriggertable", "Enabling trigger for %s/%s @ %u\n",
                     item->mteOwner, item->mteTriggerName, item->mteTriggerFrequency));
        item->alarmreg = snmp_alarm_register (item->mteTriggerFrequency, SA_REPEAT, mte_run_trigger, item);
    }
}
