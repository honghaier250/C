     ...
     void init_mteTriggerConf (void)
{
    ...
    snmpd_register_const_config_handler ("monitor",
                                         parse_mteMonitor,
                                         NULL, "triggername [-I] [-i OID | -o OID]* [-e event] expression ");
    snmpd_register_const_config_handler ("defaultMonitors", parse_default_mteMonitors, NULL, "yes|no");
    snmpd_register_const_config_handler ("linkUpDownNotifications", parse_linkUpDown_traps, NULL, "yes|no");
    ...
    snmpd_register_config_handler ("_mteTExTable", parse_mteTExTable, NULL, NULL);
    snmpd_register_config_handler ("_mteTBlTable", parse_mteTBlTable, NULL, NULL);
    snmpd_register_config_handler ("_mteTThTable", parse_mteTThTable, NULL, NULL);

    /*
     * ... and backwards compatability with the previous implementation.
     */
    snmpd_register_config_handler ("mteTriggerTable", parse_mteTriggerTable, NULL, NULL);

    /*
     * Register to save (non-fixed) entries when the agent shuts down
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_mteTTable, NULL);
    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_PRE_UPDATE_CONFIG, clear_mteTTable, NULL);
}
...
struct mteTrigger *_find_mteTrigger_entry (const char *owner, char *tname)
{
    ...
    snmp_set_var_typed_value (&tname_var, ASN_PRIV_IMPLIED_OCTET_STR, tname, strlen (tname));
...
struct mteTrigger *_find_typed_mteTrigger_entry (const char *owner, char *tname, int type)
{
...
int _mteTrigger_callback_enable (int majorID, int minorID, void *serverargs, void *clientarg)
{
...
void parse_mteMonitor (const char *token, const char *line)
{
    ...
    long flags = MTE_TRIGGER_FLAG_ENABLED |
        ...
        MTE_TRIGGER_FLAG_FIXED | MTE_TRIGGER_FLAG_VWILD | MTE_TRIGGER_FLAG_SYSUPT | MTE_TRIGGER_FLAG_VALID;
    long idx = 0;
    ...
    for (cp = copy_nword_const (line, buf, SPRINT_MAX_LEN);; cp = copy_nword_const (cp, buf, SPRINT_MAX_LEN))
    {
        ...
        if (buf[0] == '-')
        {
            switch (buf[1])
            {
                ...
                case 'd':        /* discontinuity OID (implies delta sample) */
                    ...
                    if (snmp_oid_compare (name_buf, name_buf_len, _sysUpTime_instance, _sysUpTime_inst_len) != 0)
    ...
    if (NULL == sess)
    {
        ...
        if (NULL == sess)
        {
            config_perror ("You must specify a default user name using the agentSecName token\n");
            mteObjects_removeEntries ("snmpd.conf", tname);
            return;
        }
    }
    ...
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_POST_READ_CONFIG, _mteTrigger_callback_enable, entry);
    return;
}
...
void parse_linkUpDown_traps (const char *token, const char *line)
{
    ...
    if (strncmp (line, "yes", 3) == 0)
    {
        ...
        /* ifOperStatus */
        parse_mteMonitor ("monitor", "-r 60 -S -e _linkUp   \"linkUp\"   .1.3.6.1.2.1.2.2.1.8 != 2");
        parse_mteMonitor ("monitor", "-r 60 -S -e _linkDown \"linkDown\" .1.3.6.1.2.1.2.2.1.8 == 2");
    }
}
...
void parse_default_mteMonitors (const char *token, const char *line)
{
    if (strncmp (line, "yes", 3) == 0)
    {
        ...
        parse_mteMonitor ("monitor", "-o prNames -o prErrMessage   \"process table\" prErrorFlag  != 0");
        parse_mteMonitor ("monitor", "-o memErrorName -o memSwapErrorMsg \"memory\"  memSwapError != 0");
        parse_mteMonitor ("monitor", "-o extNames -o extOutput     \"extTable\"      extResult    != 0");
        parse_mteMonitor ("monitor", "-o dskPath -o dskErrorMsg    \"dskTable\"      dskErrorFlag != 0");
        parse_mteMonitor ("monitor", "-o laNames -o laErrMessage   \"laTable\"       laErrorFlag  != 0");
        parse_mteMonitor ("monitor", "-o fileName -o fileErrorMsg  \"fileTable\"    fileErrorFlag != 0");
        parse_mteMonitor ("monitor", "-o snmperrErrMessage         \"snmperrs\"  snmperrErrorFlag != 0");
    }
    return;
}
...
char *_parse_mteTCols (char *line, struct mteTrigger *entry, int bcomp)
{
    ...
    line = read_config_read_data (ASN_OBJECT_ID, line, &vp, &entry->mteTriggerValueID_len);
    if (bcomp)
    {
        /*
    ...
    line = read_config_read_data (ASN_UNSIGNED, line, &entry->mteTriggerFrequency, NULL);
...
char *_parse_mteTDCols (char *line, struct mteTrigger *entry, int bcomp)
{
    ...
    line = read_config_read_data (ASN_OBJECT_ID, line, &vp, &entry->mteDeltaDiscontID_len);
    line = read_config_read_data (ASN_UNSIGNED, line, &tmp, NULL);
    if (bcomp)
    {
        if (tmp == TV_TRUE)
    ...
    line = read_config_read_data (ASN_UNSIGNED, line, &entry->mteDeltaDiscontIDType, NULL);
...
char *_parse_mteTBlCols (char *line, struct mteTrigger *entry, int bcomp)
{
    ...
    if (bcomp)
    {
        line = read_config_read_data (ASN_UNSIGNED, line, &entry->mteTBoolComparison, NULL);
        line = read_config_read_data (ASN_INTEGER, line, &entry->mteTBoolValue, NULL);
        line = read_config_read_data (ASN_UNSIGNED, line, &tmp, NULL);
        if (tmp == TV_TRUE)
            entry->flags |= MTE_TRIGGER_FLAG_BSTART;
    }
...
char *_parse_mteTThCols (char *line, struct mteTrigger *entry, int bcomp)
{
    ...
    line = read_config_read_data (ASN_UNSIGNED, line, &entry->mteTThStartup, NULL);
    line = read_config_read_data (ASN_INTEGER, line, &entry->mteTThRiseValue, NULL);
    line = read_config_read_data (ASN_INTEGER, line, &entry->mteTThFallValue, NULL);
    line = read_config_read_data (ASN_INTEGER, line, &entry->mteTThDRiseValue, NULL);
    line = read_config_read_data (ASN_INTEGER, line, &entry->mteTThDFallValue, NULL);
...
int store_mteTTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (row = netsnmp_tdata_row_first (trigger_table_data);
         row; row = netsnmp_tdata_row_next (trigger_table_data, row))
    {
        ...
        cptr = read_config_store_data (ASN_OCTET_STR, cptr, &cp, &tint);
        tint = entry->flags &
            (MTE_TRIGGER_FLAG_VWILD | MTE_TRIGGER_FLAG_CWILD | MTE_TRIGGER_FLAG_ENABLED | MTE_TRIGGER_FLAG_ACTIVE);
        cptr = read_config_store_data (ASN_UNSIGNED, cptr, &tint, NULL);
        ...
        if (entry->mteTriggerTest & MTE_TRIGGER_THRESHOLD)
        {
            ...
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &entry->mteTThStartup, NULL);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &entry->mteTThRiseValue, NULL);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &entry->mteTThFallValue, NULL);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &entry->mteTThDRiseValue, NULL);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &entry->mteTThDFallValue, NULL);
...
int clear_mteTTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    {
        struct mteTrigger *entry = (struct mteTrigger *) netsnmp_tdata_remove_and_delete_row (trigger_table_data, row);

        if (entry)
        {
            /* Remove from the callbacks list and disable triggers */
            snmp_unregister_callback (SNMP_CALLBACK_LIBRARY,
                                      SNMP_CALLBACK_POST_READ_CONFIG, _mteTrigger_callback_enable, entry, 0);
