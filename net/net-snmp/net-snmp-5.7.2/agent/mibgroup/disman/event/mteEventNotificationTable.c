...
netsnmp_feature_require (table_tdata)
    ...
    netsnmp_feature_require (check_vb_type_and_max_size) netsnmp_feature_require (check_vb_oid)
     ...
     void init_mteEventNotificationTable (void)
{
    ...
    reg = netsnmp_create_handler_registration ("mteEventNotificationTable",
                                               ...
                                               mteEventNotificationTable_oid_len, HANDLER_CAN_RWRITE);
#else                            /* !NETSNMP_NO_WRITE_SUPPORT */
    reg = netsnmp_create_handler_registration ("mteEventNotificationTable",
                                               mteEventNotificationTable_handler,
                                               mteEventNotificationTable_oid,
                                               mteEventNotificationTable_oid_len, HANDLER_CAN_RONLY);
#endif                            /* !NETSNMP_NO_WRITE_SUPPORT */
                ...
                switch (tinfo->colnum)
                {
                    ...
                    case COLUMN_MTEEVENTNOTIFICATIONOBJECTSOWNER:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR,
                                                  (u_char *) entry->mteNotifyOwner, strlen (entry->mteNotifyOwner));
                        break;
                    case COLUMN_MTEEVENTNOTIFICATIONOBJECTS:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR,
                                                  (u_char *) entry->mteNotifyObjects, strlen (entry->mteNotifyObjects));
                        break;
                }
            }
            break;
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    case COLUMN_MTEEVENTNOTIFICATION:
                        ...
                        memcpy (entry->mteNotification, request->requestvb->val.objid, request->requestvb->val_len);
                        entry->mteNotification_len = request->requestvb->val_len / sizeof (oid);
                        break;
                    case COLUMN_MTEEVENTNOTIFICATIONOBJECTSOWNER:
                        memset (entry->mteNotifyOwner, 0, sizeof (entry->mteNotifyOwner));
                        memcpy (entry->mteNotifyOwner, request->requestvb->val.string, request->requestvb->val_len);
                        break;
                    case COLUMN_MTEEVENTNOTIFICATIONOBJECTS:
                        memset (entry->mteNotifyObjects, 0, sizeof (entry->mteNotifyObjects));
                        memcpy (entry->mteNotifyObjects, request->requestvb->val.string, request->requestvb->val_len);
                        break;
                }
            }
            break;
