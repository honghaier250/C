...
netsnmp_feature_require (table_tdata)
    ...
    netsnmp_feature_require (check_vb_oid) netsnmp_feature_require (check_vb_truthvalue)
    ...
    switch (reqinfo->mode)
    {
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    ...
                    case COLUMN_MTETRIGGERDELTADISCONTINUITYIDWILDCARD:
                        ret = (entry->flags & MTE_TRIGGER_FLAG_DWILD) ? TV_TRUE : TV_FALSE;
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, ret);
                        break;
                    case COLUMN_MTETRIGGERDELTADISCONTINUITYIDTYPE:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, entry->mteDeltaDiscontIDType);
                        break;
                }
            }
            break;
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    ...
                    case COLUMN_MTETRIGGERDELTADISCONTINUITYIDWILDCARD:
                        ...
                        break;
                    case COLUMN_MTETRIGGERDELTADISCONTINUITYIDTYPE:
                        ret = netsnmp_check_vb_int_range (request->requestvb, MTE_DELTAD_TTICKS, MTE_DELTAD_DATETIME);
                        if (ret != SNMP_ERR_NOERROR)
                        {
                            netsnmp_set_request_error (reqinfo, request, ret);
                            return SNMP_ERR_NOERROR;
        ...
        case MODE_SET_COMMIT:
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                {
                    case COLUMN_MTETRIGGERDELTADISCONTINUITYID:
                        if (snmp_oid_compare (request->requestvb->val.objid,
                                              ...
                                              _sysUpTime_instance, _sysUpTime_inst_len) != 0)
                        {
                            ...
                            entry->mteDeltaDiscontID_len = request->requestvb->val_len / sizeof (oid);
