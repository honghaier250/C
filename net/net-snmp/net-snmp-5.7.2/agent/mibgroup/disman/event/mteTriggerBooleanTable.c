...
netsnmp_feature_require (table_tdata)
    ...
    netsnmp_feature_require (check_vb_type_and_max_size) netsnmp_feature_require (check_vb_truthvalue)
    ...
    switch (reqinfo->mode)
    {
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    case COLUMN_MTETRIGGERBOOLEANCOMPARISON:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, entry->mteTBoolComparison);
                        break;
                    case COLUMN_MTETRIGGERBOOLEANVALUE:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, entry->mteTBoolValue);
                        break;
                    case COLUMN_MTETRIGGERBOOLEANSTARTUP:
                        ret = (entry->flags & MTE_TRIGGER_FLAG_BSTART) ? TV_TRUE : TV_FALSE;
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, ret);
                        break;
                    case COLUMN_MTETRIGGERBOOLEANOBJECTSOWNER:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR,
                                                  (u_char *) entry->mteTBoolObjOwner, strlen (entry->mteTBoolObjOwner));
                        break;
                    case COLUMN_MTETRIGGERBOOLEANOBJECTS:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR,
                                                  (u_char *) entry->mteTBoolObjects, strlen (entry->mteTBoolObjects));
                        break;
                    case COLUMN_MTETRIGGERBOOLEANEVENTOWNER:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR,
                                                  (u_char *) entry->mteTBoolEvOwner, strlen (entry->mteTBoolEvOwner));
                        break;
                    case COLUMN_MTETRIGGERBOOLEANEVENT:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR,
                                                  (u_char *) entry->mteTBoolEvent, strlen (entry->mteTBoolEvent));
                        break;
                }
            }
            break;
        ...
        case MODE_SET_RESERVE1:
            for (request = requests; request; request = request->next)
            {
                ...
                {
                    case COLUMN_MTETRIGGERBOOLEANCOMPARISON:
                        ret = netsnmp_check_vb_int_range (request->requestvb, MTE_BOOL_UNEQUAL, MTE_BOOL_GREATEREQUAL);
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    ...
                    case COLUMN_MTETRIGGERBOOLEANOBJECTSOWNER:
                        ...
                        memcpy (entry->mteTBoolObjOwner, request->requestvb->val.string, request->requestvb->val_len);
                        break;
                    case COLUMN_MTETRIGGERBOOLEANOBJECTS:
                        memset (entry->mteTBoolObjects, 0, sizeof (entry->mteTBoolObjects));
                        memcpy (entry->mteTBoolObjects, request->requestvb->val.string, request->requestvb->val_len);
                        break;
                    case COLUMN_MTETRIGGERBOOLEANEVENTOWNER:
                        memset (entry->mteTBoolEvOwner, 0, sizeof (entry->mteTBoolEvOwner));
                        memcpy (entry->mteTBoolEvOwner, request->requestvb->val.string, request->requestvb->val_len);
                        break;
                    case COLUMN_MTETRIGGERBOOLEANEVENT:
                        memset (entry->mteTBoolEvent, 0, sizeof (entry->mteTBoolEvent));
                        memcpy (entry->mteTBoolEvent, request->requestvb->val.string, request->requestvb->val_len);
                        break;
                }
            }
            break;
