...
netsnmp_feature_child_of (mteevent, libnetsnmpmibs) netsnmp_feature_child_of (mteevent_removeentry, mteevent)
 ...
 */
netsnmp_tdata_row *mteEvent_createEntry (const char *mteOwner, const char *mteEName, int fixed)
{
    struct mteEvent *entry;
    ...
    netsnmp_table_row_add_index (row, ASN_PRIV_IMPLIED_OCTET_STR, entry->mteEName, mteEName_len);
...
void mteEvent_removeEntry (netsnmp_tdata_row * row)
{
    ...
    entry = (struct mteEvent *) netsnmp_tdata_remove_and_delete_row (event_table_data, row);
    SNMP_FREE (entry);
}
...
int _mteEvent_fire_notify (struct mteEvent *event, struct mteTrigger *trigger, oid * suffix, size_t sfx_len);

#ifndef NETSNMP_NO_WRITE_SUPPORT
int _mteEvent_fire_set (struct mteEvent *event, struct mteTrigger *trigger, oid * suffix, size_t sfx_len);
...
#ifdef __NOT_NEEDED
void _insert_internal_objects (netsnmp_variable_list * vblist, char *oname, struct mteTrigger *trigger)
{
    ...
    if ((!strcmp (oname, "_mteTriggerFired")) ||
        ...
        (!strcmp (oname, "_mteTriggerFalling")) || (!strcmp (oname, "_triggerFire")))
    {
