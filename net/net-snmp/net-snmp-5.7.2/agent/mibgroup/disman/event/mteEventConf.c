     ...
     void init_mteEventConf (void)
{
    ...
    snmpd_register_config_handler ("notificationEvent",
                                   parse_notificationEvent, NULL, "eventname notifyOID [-m] [-i OID|-o OID]*");
    snmpd_register_config_handler ("setEvent", parse_setEvent, NULL, "eventname [-I] OID = value");

    netsnmp_ds_register_config (ASN_BOOLEAN,
                                netsnmp_ds_get_string (NETSNMP_DS_LIBRARY_ID,
                                                       NETSNMP_DS_LIB_APPTYPE),
                                "strictDisman", NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_STRICT_DISMAN);
    ...
    snmpd_register_config_handler ("_mteENotTable", parse_mteENotTable, NULL, NULL);
    snmpd_register_config_handler ("_mteESetTable", parse_mteESetTable, NULL, NULL);

    /*
     * Register to save (non-fixed) entries when the agent shuts down
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_mteETable, NULL);
    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_PRE_UPDATE_CONFIG, clear_mteETable, NULL);
}
 ...
 * ============================== */
     ...
     */
static struct mteEvent *_find_mteEvent_entry (const char *owner, const char *ename)
{
    ...
    snmp_set_var_typed_value (&ename_var, ASN_PRIV_IMPLIED_OCTET_STR, ename, strlen (ename));
...
static struct mteEvent *_find_typed_mteEvent_entry (const char *owner, const char *ename, int type)
{
...
void parse_notificationEvent (const char *token, char *line)
{
    ...
    if (cp && *cp == '-' && *(cp + 1) == 'm')
    {
        ...
        {
            idx++;
            object = mteObjects_addOID ("snmpd.conf", ename, idx, var->vblabel, wild);
     ...
     */
    entry = _find_typed_mteEvent_entry ("snmpd.conf", ename + 2, MTE_EVENT_NOTIFICATION);
    if (!entry)
    {
...
void parse_mteENotTable (const char *token, char *line)
{
    ...
    line = read_config_read_data (ASN_OBJECT_ID, line, &vp, &entry->mteNotification_len);
    len = MTE_STR1_LEN;
...
void parse_mteESetTable (const char *token, char *line)
{
     ...
     */
    vp = entry->mteSetOID;
    line = read_config_read_data (ASN_OBJECT_ID, line, &vp, &entry->mteSetOID_len);
...
int store_mteETable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (row = netsnmp_tdata_row_first (event_table_data); row; row = netsnmp_tdata_row_next (event_table_data, row))
    {
        ...
        if (entry->mteEventActions & MTE_EVENT_NOTIFICATION)
        {
            ...
            cptr = read_config_store_data (ASN_OCTET_STR, cptr, &cp, &tint);
            vp = entry->mteNotification;
            cptr = read_config_store_data (ASN_OBJECT_ID, cptr, &vp, &entry->mteNotification_len);
        ...
        if (entry->mteEventActions & MTE_EVENT_SET)
        {
            ...
            cptr = read_config_store_data (ASN_OCTET_STR, cptr, &cp, &tint);
            vp = entry->mteSetOID;
            cptr = read_config_store_data (ASN_OBJECT_ID, cptr, &vp, &entry->mteSetOID_len);
...
int clear_mteETable (int majorID, int minorID, void *serverarg, void *clientarg)
{
     ...
     */
    memset (&owner_var, 0, sizeof (netsnmp_variable_list));
    snmp_set_var_typed_value (&owner_var, ASN_OCTET_STR, "snmpd.conf", strlen ("snmpd.conf"));
