     ...
     void init_mteObjectsTable (void)
{
    ...
    reg = netsnmp_create_handler_registration ("mteObjectsTable",
                                               ...
                                               mteObjectsTable_oid, mteObjectsTable_oid_len, HANDLER_CAN_RWRITE);
#else                            /* !NETSNMP_NO_WRITE_SUPPORT */
    reg = netsnmp_create_handler_registration ("mteObjectsTable",
                                               mteObjectsTable_handler,
                                               mteObjectsTable_oid, mteObjectsTable_oid_len, HANDLER_CAN_RONLY);
#endif                            /* !NETSNMP_NO_WRITE_SUPPORT */
                ...
                switch (tinfo->colnum)
                {
                    case COLUMN_MTEOBJECTSID:
                        snmp_set_var_typed_value (request->requestvb, ASN_OBJECT_ID,
                                                  (u_char *) entry->mteObjectID, entry->mteObjectID_len * sizeof (oid));
                        break;
                    case COLUMN_MTEOBJECTSIDWILDCARD:
                        ret = (entry->flags & MTE_OBJECT_FLAG_WILD) ? TV_TRUE : TV_FALSE;
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, ret);
                        break;
                    case COLUMN_MTEOBJECTSENTRYSTATUS:
                        ret = (entry->flags & MTE_OBJECT_FLAG_ACTIVE) ? RS_ACTIVE : RS_NOTINSERVICE;
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, ret);
                        break;
                }
            }
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (tinfo->colnum)
                {
                    case COLUMN_MTEOBJECTSID:
                        ...
                        memcpy (entry->mteObjectID, request->requestvb->val.objid, request->requestvb->val_len);
