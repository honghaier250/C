...
void init_mteObjectsConf (void)
{
    ...
    snmpd_register_config_handler ("mteObjectsTable", parse_mteOTable, NULL, NULL);
    /*
     * Register to save (non-fixed) entries when the agent shuts down
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_mteOTable, NULL);
    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_PRE_UPDATE_CONFIG, clear_mteOTable, NULL);
}
...
void parse_mteOTable (const char *token, char *line)
{
    ...
    line = read_config_read_data (ASN_OBJECT_ID, line, &vp, &entry->mteObjectID_len);
...
int store_mteOTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (row = netsnmp_tdata_row_first (objects_table_data);
         row; row = netsnmp_tdata_row_next (objects_table_data, row))
    {
        ...
        tint = strlen (cp);
        cptr = read_config_store_data (ASN_OCTET_STR, cptr, &cp, &tint);
        cptr = read_config_store_data (ASN_UNSIGNED, cptr, &entry->mteOIndex, NULL);
        vp = entry->mteObjectID;
        cptr = read_config_store_data (ASN_OBJECT_ID, cptr, &vp, &entry->mteObjectID_len);
