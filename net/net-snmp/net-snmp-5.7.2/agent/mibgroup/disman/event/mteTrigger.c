...
netsnmp_feature_child_of (mtetrigger, libnetsnmpmibs) netsnmp_feature_child_of (mtetrigger_removeentry, mtetrigger)
 ...
 */
netsnmp_tdata_row *mteTrigger_createEntry (const char *mteOwner, char *mteTName, int fixed)
{
    struct mteTrigger *entry;
    ...
    netsnmp_table_row_add_index (row, ASN_OCTET_STR, entry->mteOwner, mteOwner_len);
    if (mteTName)
        memcpy (entry->mteTName, mteTName, mteTName_len);
    netsnmp_table_row_add_index (row, ASN_PRIV_IMPLIED_OCTET_STR, entry->mteTName, mteTName_len);

    /* entry->mteTriggerTest         = MTE_TRIGGER_BOOLEAN; */
    entry->mteTriggerValueID_len = 2;    /* .0.0 */
    entry->mteTriggerFrequency = 600;
    memcpy (entry->mteDeltaDiscontID, _sysUpTime_instance, sizeof (_sysUpTime_instance));
...
void mteTrigger_removeEntry (netsnmp_tdata_row * row)
{
    ...
    entry = (struct mteTrigger *) netsnmp_tdata_remove_and_delete_row (trigger_table_data, row);
    if (entry)
    {
...
void mteTrigger_run (unsigned int reg, void *clientarg)
{
    ...
    if (!(entry->flags & MTE_TRIGGER_FLAG_ENABLED) ||
        !(entry->flags & MTE_TRIGGER_FLAG_ACTIVE) || !(entry->flags & MTE_TRIGGER_FLAG_VALID))
    {
        return;
    }
    ...
    snmp_set_var_objid (var, entry->mteTriggerValueID, entry->mteTriggerValueID_len);
    ...
    while (vp1)
    {
        ...
        if (vp2)
        {
            cmp = snmp_oid_compare (vp1->name, vp1->name_length, vp2->name, vp2->name_length);
            if (cmp < 0)
            {
                /*
                ...
                if (!vtmp)
                {
                    _mteTrigger_failure ("failed to create mteTrigger temp varbind");
                    snmp_free_varbind (var);
                    return;
                }
                vtmp->type = ASN_NULL;
    ...
    if ((entry->mteTriggerTest & MTE_TRIGGER_EXISTENCE) && (entry->mteTExEvent[0] != '\0'))
    {
        ...
        if (!entry->old_results)
        {
            ...
            if (entry->mteTExTest & entry->mteTExStartup & MTE_EXIST_PRESENT)
            {
                ...
                {
                    DEBUGMSGTL (("disman:event:trigger:fire", "Firing initial existence test: "));
                    DEBUGMSGOID (("disman:event:trigger:fire", vp1->name, vp1->name_length));
            ...
            {
                if (!(entry->flags & MTE_TRIGGER_FLAG_VWILD) && var->type == ASN_NULL)
                {
                    DEBUGMSGTL (("disman:event:trigger:fire", "Firing initial existence test: "));
                    DEBUGMSGOID (("disman:event:trigger:fire", var->name, var->name_length));
        ...
        {
            for (vp1 = var, vp2 = entry->old_results; vp1; vp1 = vp1->next_variable, vp2 = vp2->next_variable)
            {
                ...
                if ((entry->mteTExTest & MTE_EXIST_PRESENT) && (vp1->type != ASN_NULL) && (vp2->type == ASN_NULL))
                {
                ...
                else if ((entry->mteTExTest & MTE_EXIST_ABSENT) && (vp1->type == ASN_NULL) && (vp2->type != ASN_NULL))
                {
                    ...
                    reason = "(absent)";
                         ...
                         ((vp1->val_len != vp2->val_len) ||
                          (memcmp (vp1->val.string, vp2->val.string, vp1->val_len) != 0)))
                {
                    /*
                ...
                if (entry->mteTriggerFired)
                {
                     ...
                     *   so fire the trigger.
                     */
                    DEBUGMSGTL (("disman:event:trigger:fire", "Firing existence test: "));
                    DEBUGMSGOID (("disman:event:trigger:fire", vp1->name, vp1->name_length));
    ...
    if ((entry->mteTriggerTest & MTE_TRIGGER_BOOLEAN) || (entry->mteTriggerTest & MTE_TRIGGER_THRESHOLD))
    {
        ...
        switch (var->type)
        {
            ...
            default:
                 ...
                 */
                DEBUGMSGTL (("disman:event:trigger:fire", "Returned non-integer result(s): "));
                DEBUGMSGOID (("disman:event:trigger:fire", var->name, var->name_length));
                DEBUGMSG (("disman:event:trigger:fire", " (boolean/threshold) %d\n", var->type));;
        ...
        if (entry->flags & MTE_TRIGGER_FLAG_DELTA)
        {
            ...
            if (!(entry->flags & MTE_TRIGGER_FLAG_SYSUPT))
            {
                 ...
                 */
                DEBUGMSGTL (("disman:event:delta", "retrieve discontinuity marker(s): "));
                DEBUGMSGOID (("disman:event:delta", entry->mteDeltaDiscontID, entry->mteDeltaDiscontID_len));
                DEBUGMSG (("disman:event:delta", " %s\n", (entry->flags & MTE_TRIGGER_FLAG_DWILD ? " (wild)" : "")));

                dvar = (netsnmp_variable_list *) SNMP_MALLOC_TYPEDEF (netsnmp_variable_list);
                if (!dvar)
                {
                    _mteTrigger_failure ("failed to create mteTrigger delta query varbind");
                    return;
                }
                snmp_set_var_objid (dvar, entry->mteDeltaDiscontID, entry->mteDeltaDiscontID_len);
            ...
            {
                DEBUGMSGTL (("disman:event:delta", "single discontinuity: (sysUT)\n"));
                snmp_free_varbind (entry->old_results);
            ...
            if (!(entry->flags & MTE_TRIGGER_FLAG_DWILD) &&
                !(entry->flags & MTE_TRIGGER_FLAG_SYSUPT) &&
                (!entry->old_deltaDs || (entry->old_deltaDs->val.integer != dvar->val.integer)))
            {
            ...
            if (entry->flags & MTE_TRIGGER_FLAG_DWILD)
            {
                ...
                while (vp1)
                {
                     ...
                     */
                    cmp = snmp_oid_compare (vp1->name + n, vp1->name_length - n, vp2->name + n2, vp2->name_length - n2);
                    if (cmp < 0)
                    {
                        /*
                         ...
                         */
                        vtmp = (netsnmp_variable_list *) SNMP_MALLOC_TYPEDEF (netsnmp_variable_list);
                        if (!vtmp)
                        {
                            _mteTrigger_failure ("failed to create mteTrigger discontinuity varbind");
                            snmp_free_varbind (dvar);
                            return;
                        }
                        snmp_set_var_objid (vtmp, entry->mteDeltaDiscontID, entry->mteDeltaDiscontID_len);
        ...
        for (vp1 = var; vp1; vp1 = vp1->next_variable)
        {
            ...
            if (entry->flags & MTE_TRIGGER_FLAG_DELTA)
            {
                if (entry->flags & MTE_TRIGGER_FLAG_DWILD)
                {
                     ...
                     */
                    if ((dv1->type == ASN_NULL) || (dv1->type != dv2->type) || (*dv1->val.integer != *dv2->val.integer))
                    {
                        /*
                        ...
                        DEBUGMSGOID (("disman:event:delta", vp1->name, vp1->name_length));
                        DEBUGMSG (("disman:event:delta", " \n"));
                        vp2 = vp2->next_variable;
                        continue;
                    }
            ...
            DEBUGMSGTL (("disman:event:delta", "Bool comparison: (%ld %s %ld) %d\n",
                         value, _ops[entry->mteTBoolComparison], entry->mteTBoolValue, cmp));
            ...
            if (cmp)
            {
                if (vp1->index & MTE_ARMED_BOOLEAN)
                {
                    ...
                    {
                        DEBUGMSGTL (("disman:event:trigger:fire", "Firing boolean test: "));
                        DEBUGMSGOID (("disman:event:trigger:fire", vp1->name, vp1->name_length));
    ...
    if ((entry->mteTriggerTest & MTE_TRIGGER_THRESHOLD) &&
        ((entry->mteTThRiseEvent[0] != '\0') || (entry->mteTThFallEvent[0] != '\0')))
    {
        ...
        for (vp1 = var; vp1; vp1 = vp1->next_variable)
        {
            ...
            if (value >= entry->mteTThRiseValue)
            {
                if (cmp & MTE_ARMED_TH_RISE)
                {
                    ...
                    {
                        DEBUGMSGTL (("disman:event:trigger:fire", "Firing rising threshold test: "));
                        DEBUGMSGOID (("disman:event:trigger:fire", vp1->name, vp1->name_length));
                        DEBUGMSG (("disman:event:trigger:fire", "%s\n", (entry->old_results ? "" : " (startup)")));
                        /*
                        ...
                        if (entry->mteTThRiseEvent[0] != '\0')
                        {
                            ...
                            mteEvent_fire (entry->mteTThRiseOwner,
                                           entry->mteTThRiseEvent, entry, vp1->name + n, vp1->name_length - n);
                        }
                    }
                }
            }
            ...
            if (value <= entry->mteTThFallValue)
            {
                if (cmp & MTE_ARMED_TH_FALL)
                {
                    ...
                    {
                        DEBUGMSGTL (("disman:event:trigger:fire", "Firing falling threshold test: "));
                        DEBUGMSGOID (("disman:event:trigger:fire", vp1->name, vp1->name_length));
                        DEBUGMSG (("disman:event:trigger:fire", "%s\n", (entry->old_results ? "" : " (startup)")));
                        /*
                        ...
                        if (entry->mteTThRiseEvent[0] != '\0')
                        {
                            ...
                            mteEvent_fire (entry->mteTThFallOwner,
                                           entry->mteTThFallEvent, entry, vp1->name + n, vp1->name_length - n);
                        }
                    }
                }
            }
     ...
     */
    if ((entry->mteTriggerTest & MTE_TRIGGER_THRESHOLD) &&
        ((entry->mteTThDRiseEvent[0] != '\0') || (entry->mteTThDFallEvent[0] != '\0')))
    {
        ...
        if (entry->flags & MTE_TRIGGER_FLAG_DELTA)
        {
            DEBUGMSGTL (("disman:event:trigger", "Delta-threshold on delta-sample\n"));
        }
        else if (vp2 != NULL)
        {
            for (vp1 = var; vp1; vp1 = vp1->next_variable)
            {
                ...
                if (value >= entry->mteTThDRiseValue)
                {
                    ...
                    {
                        DEBUGMSGTL (("disman:event:trigger:fire", "Firing rising delta threshold test: "));
                        DEBUGMSGOID (("disman:event:trigger:fire", vp1->name, vp1->name_length));
                        ...
                        if (entry->mteTThDRiseEvent[0] != '\0')
                        {
                            ...
                            mteEvent_fire (entry->mteTThDRiseOwner,
                                           entry->mteTThDRiseEvent, entry, vp1->name + n, vp1->name_length - n);
                        }
                    }
                }
                ...
                if (value <= entry->mteTThDFallValue)
                {
                    ...
                    {
                        DEBUGMSGTL (("disman:event:trigger:fire", "Firing falling delta threshold test: "));
                        DEBUGMSGOID (("disman:event:trigger:fire", vp1->name, vp1->name_length));
                        DEBUGMSG (("disman:event:trigger:fire", "\n"));
                        cmp &= ~MTE_ARMED_TH_DFALL;
                        cmp |= MTE_ARMED_TH_DRISE;
                        ...
                        if (entry->mteTThDRiseEvent[0] != '\0')
                        {
                            ...
                            mteEvent_fire (entry->mteTThDFallOwner,
                                           entry->mteTThDFallEvent, entry, vp1->name + n, vp1->name_length - n);
                        }
                    }
                }
                vp1->index = cmp;
