...
void init_mteObjects (void)
{
     ...
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_POST_READ_CONFIG, _init_default_mteObject_lists, NULL);
}
...
int _init_default_mteObject_lists (int majorID, int minorID, void *serverargs, void *clientarg)
{
 ...
 */
netsnmp_tdata_row *mteObjects_createEntry (const char *owner, const char *oname, int index, int flags)
{
    struct mteObject *entry;
    ...
    netsnmp_tdata_row_add_index (row, ASN_INTEGER, &entry->mteOIndex, sizeof (long));
     ...
     */
    row2 = netsnmp_tdata_row_get_byoid (objects_table_data, row->oid_index.oids, row->oid_index.len);
    if (row2)
    {
                                     ...
                                     const char *oid_name_buf, int wild)
{
    ...
    row = mteObjects_createEntry (owner, oname, index, MTE_OBJECT_FLAG_FIXED | MTE_OBJECT_FLAG_NEXT);
...
void mteObjects_removeEntry (netsnmp_tdata_row * row)
{
    ...
    entry = (struct mteObject *) netsnmp_tdata_remove_and_delete_row (objects_table_data, row);
    SNMP_FREE (entry);
}
...
int mteObjects_vblist (netsnmp_variable_list * vblist, char *owner, char *oname, oid * suffix, size_t sfx_len)
{
    netsnmp_tdata_row *row;
...
mteObjects_internal_vblist (netsnmp_variable_list * vblist,
                            char *oname, struct mteTrigger *trigger, netsnmp_session * sess)
{
    ...
    if (!strcmp (oname, "_triggerFire"))
    {
        ...
        snmp_varlist_add_variable (&var,
                                   ...
                                   ASN_OCTET_STR, trigger->mteTName, strlen (trigger->mteTName));
        snmp_varlist_add_variable (&var,
                                   mteHotTarget, OID_LENGTH (mteHotTarget),
                                   ASN_OCTET_STR, trigger->mteTriggerTarget, strlen (trigger->mteTriggerTarget));
        snmp_varlist_add_variable (&var,
                                   mteHotContext, OID_LENGTH (mteHotContext),
                                   ASN_OCTET_STR, trigger->mteTriggerContext, strlen (trigger->mteTriggerContext));
        snmp_varlist_add_variable (&var,
        ...
        snmp_varlist_add_variable (&var,
                                   ...
                                   trigger->mteTriggerFired->val.string, trigger->mteTriggerFired->val_len);
    }
    else if ((!strcmp (oname, "_linkUpDown")))
    {
        ...
        snmp_varlist_add_variable (&var,
                                   ifIndexOid, OID_LENGTH (ifIndexOid), ASN_INTEGER, &if_index, sizeof (if_index));

        /* Set up a dummy varbind for ifAdminStatus... */
        snmp_varlist_add_variable (&var,
                                   ifAdminStatus, OID_LENGTH (ifAdminStatus),
                                   ASN_INTEGER,
                                   trigger->mteTriggerFired->val.integer, trigger->mteTriggerFired->val_len);
    ...
    {
        DEBUGMSGTL (("disman:event:objects", "Unknown internal objects tag (%s)\n", oname));
        return 1;
    }
