...
oid expObjectTable_variables_oid[] = { 1, 3, 6, 1, 2, 1, 90, 1, 2, 3 };
...
void init_expObjectTable (void)
{
     ...
     */
    REGISTER_MIB ("expObjectTable", expObjectTable_variables, variable2, expObjectTable_variables_oid);

    /*
     * register our config handler(s) to deal with registrations 
     */
    snmpd_register_config_handler ("expObjectTable", parse_expObjectTable, NULL, NULL);

    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_expObjectTable, NULL);

    DEBUGMSGTL (("expObjectTable", "done.\n"));
}


struct expObjectTable_data *create_expObjectTable_data (void)
{
    ...
    memdup ((unsigned char **)
            &(StorageNew->expObjectDeltaDiscontinuityID), (unsigned char *) TimeInstance, sizeof (TimeInstance));
    StorageNew->expObjectDeltaDiscontinuityIDLen = sizeof (TimeInstance) / sizeof (oid);




    StorageNew->expObjectDiscontinuityIDWildcard = EXPOBJCETDISCONTINUITYIDWILDCARD_FALSE;
    StorageNew->expObjectDiscontinuityIDType = EXPOBJECTDISCONTINUITYIDTYPE_TIMETICKS;
    ...
    StorageNew->expObjectConditionalWildcard = EXPOBJECTCONDITIONALWILDCARD_FALSE;
    StorageNew->storageType = ST_NONVOLATILE;

    return StorageNew;
}

int expObjectTable_add (struct expObjectTable_data *thedata)
{
 ...
 */
void parse_expObjectTable (const char *token, char *line)
{
    size_t tmpint;

    struct expObjectTable_data *StorageTmp = SNMP_MALLOC_STRUCT (expObjectTable_data);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->expExpressionOwner, &StorageTmp->expExpressionOwnerLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line, &StorageTmp->expExpressionName, &StorageTmp->expExpressionNameLen);
    ...
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->expObjectIndex, &tmpint);

    line = read_config_read_data (ASN_OBJECT_ID, line, &StorageTmp->expObjectID, &StorageTmp->expObjectIDLen);
    ...
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->expObjectIDWildcard, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->expObjectSampleType, &tmpint);
    ...
    {
        config_perror ("invalid specification for expObjectDeltaDiscontinuityID");
        return;
    }

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->expObjectDiscontinuityIDWildcard, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->expObjectDiscontinuityIDType, &tmpint);

    line =
        read_config_read_data (ASN_OBJECT_ID, line,
                               &StorageTmp->expObjectConditional, &StorageTmp->expObjectConditionalLen);
    ...
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->expObjectConditionalWildcard, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->expObjectEntryStatus, &tmpint);
 ...
 */
int store_expObjectTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (hcindex = expObjectTableStorage; hcindex != NULL; hcindex = hcindex->next)
    {
        ...
        if (StorageTmp->storageType == ST_NONVOLATILE)
        {
            ...
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->expExpressionName, &StorageTmp->expExpressionNameLen);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->expObjectIndex, &tmpint);
            cptr = read_config_store_data (ASN_OBJECT_ID, cptr, &StorageTmp->expObjectID, &StorageTmp->expObjectIDLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->expObjectIDWildcard, &tmpint);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->expObjectSampleType, &tmpint);
            cptr =
                read_config_store_data (ASN_OBJECT_ID, cptr,
                                        &StorageTmp->expObjectDeltaDiscontinuityID,
                                        &StorageTmp->expObjectDeltaDiscontinuityIDLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->expObjectDiscontinuityIDWildcard, &tmpint);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->expObjectDiscontinuityIDType, &tmpint);
            cptr =
                read_config_store_data (ASN_OBJECT_ID, cptr,
                                        &StorageTmp->expObjectConditional, &StorageTmp->expObjectConditionalLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->expObjectConditionalWildcard, &tmpint);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->expObjectEntryStatus, &tmpint);
 ...
 */
unsigned char *var_expObjectTable (struct variable *vp,
                                   oid * name,
                                   size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
     ...
     */
    if ((StorageTmp = header_complex (expObjectTableStorage, vp, name, length, exact, var_len, write_method)) == NULL)
    {
    ...
    switch (vp->magic)
    {
        ...
        case EXPOBJECTDELTADISCONTINUITYID:
            ...
            *var_len = StorageTmp->expObjectDeltaDiscontinuityIDLen * sizeof (oid);
                   ...
                   u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    size_t newlen = name_len - (sizeof (expObjectTable_variables_oid) / sizeof (oid) + 3 - 1);


    DEBUGMSGTL (("expObjectTable", "write_expObjectID entering action=%d...  \n", action));
    if ((StorageTmp =
    ...
    switch (action)
    {
        ...
        case ACTION:
            ...
            memdup ((u_char **) & StorageTmp->expObjectID, var_val, var_val_len);
                           ...
                           u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("expObjectTable", "write_expObjectIDWildcard entering action=%d...  \n", action));
    if ((StorageTmp =
         header_complex (expObjectTableStorage, NULL,
                         &name[sizeof (expObjectTable_variables_oid) /
                               sizeof (oid) + 3 - 1], &newlen, 1, NULL, NULL)) == NULL)
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            {
                snmp_log (LOG_ERR, "write to expObjectIDWildcard not ASN_INTEGER\n");
                return SNMP_ERR_WRONGTYPE;
            }
            if (StorageTmp->storageType != ST_NONVOLATILE)
                return SNMP_ERR_NOTWRITABLE;
                           ...
                           u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("expObjectTable", "write_expObjectSampleType entering action=%d...  \n", action));
    if ((StorageTmp =
         header_complex (expObjectTableStorage, NULL,
                         &name[sizeof (expObjectTable_variables_oid) /
                               sizeof (oid) + 3 - 1], &newlen, 1, NULL, NULL)) == NULL)
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_INTEGER)
            {
                snmp_log (LOG_ERR, "write to expObjectSampleTypenot ASN_INTEGER\n");
                return SNMP_ERR_WRONGTYPE;
            }
            if (StorageTmp->storageType != ST_NONVOLATILE)
                return SNMP_ERR_NOTWRITABLE;
                                     ...
                                     size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("expObjectTable", "write_expObjectDeltaDiscontinuityID entering action=%d...  \n", action));
    if ((StorageTmp =
         header_complex (expObjectTableStorage, NULL,
                         &name[sizeof (expObjectTable_variables_oid) /
                               sizeof (oid) + 3 - 1], &newlen, 1, NULL, NULL)) == NULL)
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_OBJECT_ID)
            {
                snmp_log (LOG_ERR, "write to expObjectDeltaDiscontinuityID not ASN_OBJECT_ID\n");
                return SNMP_ERR_WRONGTYPE;
            }
            if (StorageTmp->storageType != ST_NONVOLATILE)
                return SNMP_ERR_NOTWRITABLE;
        ...
        case ACTION:
            ...
            memdup ((u_char **) & StorageTmp->expObjectDeltaDiscontinuityID, var_val, var_val_len);
            StorageTmp->expObjectDeltaDiscontinuityIDLen = var_val_len / sizeof (oid);
            break;
                                        ...
                                        size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("expObjectTable", "write_expObjectDiscontinuityIDWildcard entering action=%d...  \n", action));
    if ((StorageTmp =
         header_complex (expObjectTableStorage, NULL,
                         &name[sizeof (expObjectTable_variables_oid) /
                               sizeof (oid) + 3 - 1], &newlen, 1, NULL, NULL)) == NULL)
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_INTEGER)
            {
                snmp_log (LOG_ERR, "write to expObjectDiscontinuityIDWildcard not ASN_INTEGER\n");
                return SNMP_ERR_WRONGTYPE;
            }
            if (StorageTmp->storageType != ST_NONVOLATILE)
                return SNMP_ERR_NOTWRITABLE;
                                    ...
                                    size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_INTEGER)
            {
                snmp_log (LOG_ERR, "write to expObjectDiscontinuityIDType not ASN_INTEGER\n");
                return SNMP_ERR_WRONGTYPE;
            }
            if (StorageTmp->storageType != ST_NONVOLATILE)
                return SNMP_ERR_NOTWRITABLE;
                            ...
                            u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("expObjectTable", "write_expObjectConditional entering action=%d...  \n", action));
    if ((StorageTmp =
         header_complex (expObjectTableStorage, NULL,
                         &name[sizeof (expObjectTable_variables_oid) /
                               sizeof (oid) + 3 - 1], &newlen, 1, NULL, NULL)) == NULL)
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_OBJECT_ID)
            {
                snmp_log (LOG_ERR, "write to expObjectConditional not ASN_OBJECT_ID\n");
                return SNMP_ERR_WRONGTYPE;
            }
            if (StorageTmp->storageType != ST_NONVOLATILE)
                return SNMP_ERR_NOTWRITABLE;
        ...
        case ACTION:
            ...
            memdup ((u_char **) & StorageTmp->expObjectConditional, var_val, var_val_len);
                                    ...
                                    size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("expObjectTable", "write_expObjectConditionalWildcard entering action=%d...  \n", action));
    if ((StorageTmp =
         header_complex (expObjectTableStorage, NULL,
                         &name[sizeof (expObjectTable_variables_oid) /
                               sizeof (oid) + 3 - 1], &newlen, 1, NULL, NULL)) == NULL)
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_INTEGER)
            {
                snmp_log (LOG_ERR, "write to expObjectConditionalWildcard not ASN_INTEGER\n");
                return SNMP_ERR_WRONGTYPE;
            }
            if (StorageTmp->storageType != ST_NONVOLATILE)
                return SNMP_ERR_NOTWRITABLE;
                            ...
                            u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    StorageTmp =
        header_complex (expObjectTableStorage, NULL,
                        &name[sizeof (expObjectTable_variables_oid) / sizeof (oid) + 3 - 1], &newlen, 1, NULL, NULL);
    ...
    {
        snmp_log (LOG_ERR, "write to expObjectEntryStatus not ASN_INTEGER\n");
        return SNMP_ERR_WRONGTYPE;
    }
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            else
            {
                ...
                if (StorageTmp->expObjectEntryStatus == RS_ACTIVE && set_value != RS_DESTROY)
                {
                    /*
        ...
        case RESERVE2:
            ...
            if (StorageTmp == NULL)
            {
                ...
                if (header_complex_parse_oid
                    (&
                     (name[sizeof (expObjectTable_variables_oid) / sizeof (oid) + 2]), newlen, vars) != SNMPERR_SUCCESS)
                {
                    /*
                     * XXX: free, zero vars 
                     */
                ...
                memcpy (StorageNew->expExpressionOwner, vp->val.string, vp->val_len);
                ...
                memcpy (StorageNew->expExpressionName, vp->val.string, vp->val_len);
        ...
        case ACTION:
            ...
            else
            {
                 ...
                 */
                hciptr = header_complex_find_entry (expObjectTableStorage, StorageTmp);
                StorageDel = header_complex_extract_entry (&expObjectTableStorage, hciptr);
            }
            break;
        ...
        case COMMIT:
            ...
            {
                if (StorageTmp && StorageTmp->expObjectEntryStatus == RS_CREATEANDGO)
                {
                    StorageTmp->expObjectEntryStatus = RS_ACTIVE;
                }
                else if (StorageTmp && StorageTmp->expObjectEntryStatus == RS_CREATEANDWAIT)
                {
