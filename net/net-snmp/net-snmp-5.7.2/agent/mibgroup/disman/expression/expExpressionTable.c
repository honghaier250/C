 ...
 *   variable below.
 */
     oid expExpressionTable_variables_oid[] = { 1, 3, 6, 1, 2, 1, 90, 1, 2, 1 };
...
void init_expExpressionTable (void)
{
     ...
     */
    REGISTER_MIB ("expExpressionTable", expExpressionTable_variables, variable2, expExpressionTable_variables_oid);

    /*
     * register our config handler(s) to deal with registrations 
     */
    snmpd_register_config_handler ("expExpressionTable", parse_expExpressionTable, NULL, NULL);


    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_expExpressionTable, NULL);


    DEBUGMSGTL (("expExpressionTable", "done.\n"));
}

struct expExpressionTable_data *create_expExpressionTable_data (void)
{
...
int expExpressionTable_add (struct expExpressionTable_data *thedata)
{
 ...
 */
void parse_expExpressionTable (const char *token, char *line)
{
    size_t tmpint;

    oid *tmpoid = NULL;

    struct expExpressionTable_data *StorageTmp = SNMP_MALLOC_STRUCT (expExpressionTable_data);
    ...
    line = read_config_read_data (ASN_OCTET_STR, line, &StorageTmp->expExpression, &StorageTmp->expExpressionLen);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->expExpressionValueType, &tmpint);

    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->expExpressionComment, &StorageTmp->expExpressionCommentLen);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->expExpressionDeltaInterval, &tmpint);

    line =
        read_config_read_data (ASN_OBJECT_ID, line,
                               &StorageTmp->expExpressionPrefix, &StorageTmp->expExpressionPrefixLen);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->expExpressionErrors, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->expExpressionEntryStatus, &tmpint);
 ...
 */
int store_expExpressionTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (hcindex = expExpressionTableStorage; hcindex != NULL; hcindex = hcindex->next)
    {
        ...
        if (StorageTmp->storageType == ST_NONVOLATILE)
        {
            ...
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr, &StorageTmp->expExpression, &StorageTmp->expExpressionLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->expExpressionValueType, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->expExpressionComment, &StorageTmp->expExpressionCommentLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->expExpressionDeltaInterval, &tmpint);
            cptr =
                read_config_store_data (ASN_OBJECT_ID, cptr,
                                        &StorageTmp->expExpressionPrefix, &StorageTmp->expExpressionPrefixLen);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->expExpressionErrors, &tmpint);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->expExpressionEntryStatus, &tmpint);
 ...
 */
unsigned char *var_expExpressionTable (struct variable *vp,
                                       oid * name,
                                       size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    struct expExpressionTable_data *StorageTmp = NULL;

    DEBUGMSGTL (("expExpressionTable", "var_expExpressionTable: Entering...  \n"));

    /*
     * this assumes you have registered all your data properly
     */
    if ((StorageTmp =
         header_complex (expExpressionTableStorage, vp, name, length, exact, var_len, write_method)) == NULL)
    {
                     ...
                     u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    size_t newlen = name_len - (sizeof (expExpressionTable_variables_oid) / sizeof (oid) + 3 - 1);


    DEBUGMSGTL (("expExpressionTable", "write_expExpression entering action=%d...  \n", action));
    if ((StorageTmp =
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            {
                snmp_log (LOG_ERR, "write to expExpression not ASN_OCTET_STR\n");
        ...
        case ACTION:
            ...
            memdup ((u_char **) & StorageTmp->expExpression, var_val, var_val_len);
                              ...
                              u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    DEBUGMSGTL (("expExpressionTable", "write_expExpressionValueType entering action=%d...  \n", action));
    if ((StorageTmp =
         header_complex (expExpressionTableStorage, NULL,
                         &name[sizeof (expExpressionTable_variables_oid) /
                               sizeof (oid) + 3 - 1], &newlen, 1, NULL, NULL)) == NULL)
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            {
                snmp_log (LOG_ERR, "write to expExpressionValueType not ASN_INTEGER\n");
                return SNMP_ERR_WRONGTYPE;
            }
            if (StorageTmp->storageType != ST_NONVOLATILE)
                return SNMP_ERR_NOTWRITABLE;
                            ...
                            u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_OCTET_STR)
            {
                snmp_log (LOG_ERR, "write to expExpressionComment not ASN_OCTET_STR\n");
                return SNMP_ERR_WRONGTYPE;
            }
            if (StorageTmp->storageType != ST_NONVOLATILE)
                return SNMP_ERR_NOTWRITABLE;
        ...
        case ACTION:
            ...
            memdup ((u_char **) & StorageTmp->expExpressionComment, var_val, var_val_len);
                                  ...
                                  u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            if (var_val_type != ASN_INTEGER)
            {
                snmp_log (LOG_ERR, "write to expExpressionDeltalnterval not ASN_INTEGER\n");
                return SNMP_ERR_WRONGTYPE;
            }
            if (StorageTmp->storageType != ST_NONVOLATILE)
                return SNMP_ERR_NOTWRITABLE;
                                ...
                                u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    {
        snmp_log (LOG_ERR, "write to expExpressionEntryStatus not ASN_INTEGER\n");
        return SNMP_ERR_WRONGTYPE;
    }
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            else
            {
                ...
                if (StorageTmp->expExpressionEntryStatus == RS_ACTIVE && set_value != RS_DESTROY)
                {
                    /*
        ...
        case RESERVE2:
            ...
            if (StorageTmp == NULL)
            {
                ...
                if (header_complex_parse_oid
                    (&
                     (name
                      [sizeof (expExpressionTable_variables_oid) / sizeof (oid) + 2]), newlen, vars) != SNMPERR_SUCCESS)
                {
                    /*
                     * XXX: free, zero vars 
                     */
        ...
        case ACTION:
            ...
            else
            {
                 ...
                 */
                hciptr = header_complex_find_entry (expExpressionTableStorage, StorageTmp);
                StorageDel = header_complex_extract_entry (&expExpressionTableStorage, hciptr);
            }
            break;
        ...
        case COMMIT:
            ...
            {
                if (StorageTmp && StorageTmp->expExpressionEntryStatus == RS_CREATEANDGO)
                {
                    StorageTmp->expExpressionEntryStatus = RS_ACTIVE;
                }
                else if (StorageTmp && StorageTmp->expExpressionEntryStatus == RS_CREATEANDWAIT)
                {
                    StorageTmp->expExpressionEntryStatus = RS_NOTINSERVICE;
                }
            }
            if (StorageTmp && StorageTmp->expExpressionEntryStatus == RS_ACTIVE && !StorageTmp->have_copied_auth_info)
            {
                ...
                if (pdu)
                {
                    DEBUGMSGTL (("expExpressionTest", "copying PDU auth info\n"));
