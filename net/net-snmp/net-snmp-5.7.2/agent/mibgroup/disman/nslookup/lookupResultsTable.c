...
oid lookupResultsTable_variables_oid[] = { 1, 3, 6, 1, 2, 1, 82, 1, 4 };
...
int lookupResultsTable_inadd (struct lookupResultsTable_data *thedata);

void lookupResultsTable_cleaner (struct header_complex_index *thestuff)
{
    ...
    {
        StorageDel = header_complex_extract_entry (&lookupResultsTableStorage, hciptr);
        if (StorageDel != NULL)
        {
...
void init_lookupResultsTable (void)
{
     ...
     */
    REGISTER_MIB ("lookupResultsTable", lookupResultsTable_variables, variable2, lookupResultsTable_variables_oid);


    /*
     * register our config handler(s) to deal with registrations 
     */
    snmpd_register_config_handler ("lookupResultsTable", parse_lookupResultsTable, NULL, NULL);

    /*
     * we need to be called back later to store our data 
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_lookupResultsTable, NULL);
...
void parse_lookupResultsTable (const char *token, char *line)
{
    size_t tmpint;

    struct lookupResultsTable_data *StorageTmp = SNMP_MALLOC_STRUCT (lookupResultsTable_data);
    ...
    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->lookupResultsIndex, &tmpint);
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->lookupResultsAddressType, &tmpint);
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->lookupResultsAddress, &StorageTmp->lookupResultsAddressLen);
...
int store_lookupResultsTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (hcindex = lookupResultsTableStorage; hcindex != NULL; hcindex = hcindex->next)
    {
        ...
        if (StorageTmp->storagetype != ST_READONLY)
        {
            ...
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->lookupCtlOperationName, &StorageTmp->lookupCtlOperationNameLen);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->lookupResultsIndex, &tmpint);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->lookupResultsAddressType, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->lookupResultsAddress, &StorageTmp->lookupResultsAddressLen);
...
int lookupResultsTable_inadd (struct lookupResultsTable_data *thedata)
{
    ...
    snmp_varlist_add_variable (&vars_list, NULL, 0, ASN_UNSIGNED,
                               (char *) &thedata->lookupResultsIndex, sizeof (thedata->lookupResultsIndex));
    ...
    header_complex_add_data (&lookupResultsTableStorage, vars_list, thedata);
 ...
 */
unsigned char *var_lookupResultsTable (struct variable *vp,
                                       oid * name,
                                       size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    *write_method = NULL;
     ...
     */
    if ((StorageTmp =
         header_complex (lookupResultsTableStorage, vp, name, length, exact, var_len, write_method)) == NULL)
    {
        return NULL;
    }
