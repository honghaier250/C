 ...
 */
     oid lookupCtlTable_variables_oid[] = { 1, 3, 6, 1, 2, 1, 82, 1, 3 };
...
void init_lookupCtlTable (void)
{
     ...
     */
    REGISTER_MIB ("lookupCtlTable", lookupCtlTable_variables, variable2, lookupCtlTable_variables_oid);


    /*
     * register our config handler(s) to deal with registrations 
     */
    snmpd_register_config_handler ("lookupCtlTable", parse_lookupCtlTable, NULL, NULL);

    /*
     * we need to be called back later to store our data 
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_lookupCtlTable, NULL);

    DEBUGMSGTL (("lookupCtlTable", "done.\n"));
}

struct lookupTable_data *create_lookupTable_data (void)
{
 ...
 */
int lookupCtlTable_add (struct lookupTable_data *thedata)
{
    ...
    snmp_varlist_add_variable (&vars, NULL, 0, ASN_OCTET_STR,
                               (char *) thedata->lookupCtlOperationName, thedata->lookupCtlOperationNameLen);


    if (header_complex_add_data (&lookupCtlTableStorage, vars, thedata) == NULL)
    {
...
int lookupResultsTable_add (struct lookupTable_data *thedata)
{
    ...
    if (thedata->ResultsTable != NULL)
        do
        {
            snmp_varlist_add_variable (&vars_list, NULL, 0, ASN_OCTET_STR,
                                       (char *) p->lookupCtlOwnerIndex, p->lookupCtlOwnerIndexLen);
            snmp_varlist_add_variable (&vars_list, NULL, 0, ASN_OCTET_STR,
                                       (char *) p->lookupCtlOperationName, p->lookupCtlOperationNameLen);
            snmp_varlist_add_variable (&vars_list, NULL, 0, ASN_UNSIGNED,
                                       (char *) &p->lookupResultsIndex, sizeof (p->lookupResultsIndex));
...
void lookupCtlTable_cleaner (struct header_complex_index *thestuff)
{
    ...
    {
        StorageDel = header_complex_extract_entry (&lookupCtlTableStorage, hciptr);
        if (StorageDel != NULL)
        {
 ...
 */
void parse_lookupCtlTable (const char *token, char *line)
{
    size_t tmpint;
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->lookupCtlOwnerIndex, &StorageTmp->lookupCtlOwnerIndexLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->lookupCtlOperationName, &StorageTmp->lookupCtlOperationNameLen);
    ...
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->lookupCtlTargetAddressType, &tmpint);

    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->lookupCtlTargetAddress, &StorageTmp->lookupCtlTargetAddressLen);
    ...
    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->lookupCtlOperStatus, &tmpint);

    line = read_config_read_data (ASN_UNSIGNED, line, &StorageTmp->lookupCtlTime, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->lookupCtlRc, &tmpint);

    line = read_config_read_data (ASN_INTEGER, line, &StorageTmp->lookupCtlRowStatus, &tmpint);
 ...
 */
int store_lookupCtlTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (hcindex = lookupCtlTableStorage; hcindex != NULL; hcindex = hcindex->next)
    {
        ...
        if (StorageTmp->storagetype != ST_READONLY)
        {
            ...
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->lookupCtlOperationName, &StorageTmp->lookupCtlOperationNameLen);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->lookupCtlTargetAddressType, &tmpint);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->lookupCtlTargetAddress, &StorageTmp->lookupCtlTargetAddressLen);

            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->lookupCtlOperStatus, &tmpint);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->lookupCtlTime, &tmpint);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->lookupCtlRc, &tmpint);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->lookupCtlRowStatus, &tmpint);
 ...
 */
unsigned char *var_lookupCtlTable (struct variable *vp,
                                   oid * name,
                                   size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    *write_method = NULL;
     ...
     */
    if ((StorageTmp = header_complex (lookupCtlTableStorage, vp, name, length, exact, var_len, write_method)) == NULL)
    {
...
static struct lookupResultsTable_data *add_result (struct lookupTable_data *item, int index,
                                                   int iatype, const void *data, size_t data_len)
{
    ...
    memcpy (temp->lookupCtlOwnerIndex, item->lookupCtlOwnerIndex, item->lookupCtlOwnerIndexLen + 1);
    ...
    memcpy (temp->lookupCtlOperationName, item->lookupCtlOperationName, item->lookupCtlOperationNameLen + 1);
...
void run_lookup (struct lookupTable_data *item)
{
    long addressType;
    ...
    if (addressType == INETADDRESSTYPE_IPV4)
    {
        ...
        if (lookup == NULL)
        {
            DEBUGMSGTL (("lookupCtlTable", "Can't get a network host entry for ipv4 address: %s\n", address));
        ...
        else
        {
            ...
            {
                current = temp = add_result (item, n, INETADDRESSTYPE_DNS, lookup->h_name, strlen (lookup->h_name));
                n = n + 1;
            }
            ...
            {
                temp = add_result (item, n, INETADDRESSTYPE_DNS, lookup->h_aliases[i], strlen (lookup->h_aliases[i]));
                current->next = temp;
                current = temp;
                i = i + 1;
                n = n + 1;
        ...
        if (item->ResultsTable != NULL)
            if (lookupResultsTable_add (item) != SNMPERR_SUCCESS)
                DEBUGMSGTL (("lookupResultsTable", "registered an entry error\n"));
        SNMP_FREE (address);
        return;
    }
    ...
    else if (addressType == INETADDRESSTYPE_DNS)
    {
        ...
        netsnmp_get_monotonic_clock (&tpend);
        timeuse4 = 1000000 * (tpend.tv_sec - tpstart.tv_sec) + tpend.tv_usec - tpstart.tv_usec;
        if (lookup == NULL)
        {
            DEBUGMSGTL (("lookupCtlTable", "Can't get a network host entry for %s\n", address));
            modify_lookupCtlRc (item, h_errno);
        }
        else
        {
        ...
        netsnmp_get_monotonic_clock (&tpend);
        timeuse6 = 1000000 * (tpend.tv_sec - tpstart.tv_sec) + tpend.tv_usec - tpstart.tv_usec;
        ...
        else
        {
            ...
            while (aip)
            {
                ...
                {
                    case AF_INET:
                        inet_ntop (aip->ai_family, &((struct sockaddr_in *) aip->ai_addr)->sin_addr, buf, sizeof (buf));
                        buflen = strlen (buf);
                        temp = add_result (item, n, INETADDRESSTYPE_IPV4, buf, buflen);
                        break;
                    case AF_INET6:
                        inet_ntop (aip->ai_family,
                                   &((struct sockaddr_in6 *) aip->ai_addr)->sin6_addr, buf, sizeof (buf));
                        buflen = strlen (buf);
                        temp = add_result (item, n, INETADDRESSTYPE_IPV6, buf, buflen);
                        break;
                    default:
...
int modify_lookupCtlOperStatus (struct lookupTable_data *thedata, long val)
{
    ...
    if ((StorageTmp = header_complex_get (lookupCtlTableStorage, vars)) == NULL)
    {
...
int lookupResultsTable_del (struct lookupTable_data *thedata)
{
    ...
    snmp_free_varbind (vars);
    vars = NULL;
    for (hciptr2 = lookupResultsTableStorage; hciptr2 != NULL; hciptr2 = hciptr2->next)
    {
        if (snmp_oid_compare (newoid, newoid_len, hciptr2->name, newoid_len) == 0)
        {
            StorageDel = header_complex_extract_entry (&lookupResultsTableStorage, hciptr2);
            if (StorageDel != NULL)
            {
                                  ...
                                  u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    size_t newlen = name_len - (sizeof (lookupCtlTable_variables_oid) / sizeof (oid) + 3 - 1);
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            {
                snmp_log (LOG_ERR, "write to lookupCtlTargetAddressType not ASN_INTEGER\n");
                              ...
                              u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            {
                snmp_log (LOG_ERR, "write to lookupCtlTargetAddress not ASN_OCTET_STR\n");
                return SNMP_ERR_WRONGTYPE;
            }
            break;
        ...
        case ACTION:
            ...
            if ((StorageTmp->lookupCtlTargetAddress = (char *) malloc (var_val_len + 1)) == NULL)
            {
                          ...
                          u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    StorageTmp =
        header_complex (lookupCtlTableStorage, NULL,
                        &name[sizeof (lookupCtlTable_variables_oid) / sizeof (oid) + 3 - 1], &newlen, 1, NULL, NULL);
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            else
            {
                ...
                if (StorageTmp->lookupCtlRowStatus == RS_ACTIVE && set_value != RS_DESTROY)
                {
                    /*
        ...
        case RESERVE2:
            ...
            if (StorageTmp == NULL)
            {
                ...
                if (header_complex_parse_oid
                    (&
                     (name[sizeof (lookupCtlTable_variables_oid) / sizeof (oid) + 2]), newlen, vars) != SNMPERR_SUCCESS)
                {
                    /*
                     * XXX: free, zero vars 
                     */
                ...
                memcpy (StorageNew->lookupCtlOwnerIndex, vp->val.string, vp->val_len);
                ...
                memcpy (StorageNew->lookupCtlOperationName, vp->val.string, vp->val_len);
        ...
        case ACTION:
            ...
            if (StorageTmp == NULL)
            {
                ...
                if (StorageNew != NULL)
                    DEBUGMSGTL (("lookupCtlTable", "write_lookupCtlRowStatus entering new=%d...  \n", action));
            ...
            else
            {
                 ...
                 */
                DEBUGMSGTL (("lookupCtlTable", "write_lookupCtlTable_delete 1 \n"));
                hciptr = header_complex_find_entry (lookupCtlTableStorage, StorageTmp);

                StorageDel = header_complex_extract_entry (&lookupCtlTableStorage, hciptr);
                lookupResultsTable_del (StorageTmp);

                DEBUGMSGTL (("lookupCtlTable", "write_lookupCtlTable_delete  \n"));
        ...
        case COMMIT:
            ...
            {
                if (StorageTmp && StorageTmp->lookupCtlRowStatus == RS_CREATEANDGO)
                {
                    StorageTmp->lookupCtlRowStatus = RS_ACTIVE;
                }
                else if (StorageTmp && StorageTmp->lookupCtlRowStatus == RS_CREATEANDWAIT)
                {
