...
void init_mteObjectsTable (void)
{
     ...
     */
    REGISTER_MIB ("mteObjectsTable", mteObjectsTable_variables, variable2, mteObjectsTable_variables_oid);


    /*
     * register our config handler(s) to deal with registrations 
     */
    snmpd_register_config_handler ("mteObjectsTable", parse_mteObjectsTable, NULL, NULL);


    /*
     * we need to be called back later to store our data 
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_mteObjectsTable, NULL);
 ...
 */
int mteObjectsTable_add (struct mteObjectsTable_data *thedata)
{
 ...
 */
void parse_mteObjectsTable (const char *token, char *line)
{
    size_t tmpint;
 ...
 */
int store_mteObjectsTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (hcindex = mteObjectsTableStorage; hcindex != NULL; hcindex = hcindex->next)
    {
        ...
        if (StorageTmp->storagetype != ST_READONLY)
        {
            ...
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->mteObjectsName, &StorageTmp->mteObjectsNameLen);
            cptr = read_config_store_data (ASN_UNSIGNED, cptr, &StorageTmp->mteObjectsIndex, &tmpint);
            cptr =
                read_config_store_data (ASN_OBJECT_ID, cptr, &StorageTmp->mteObjectsID, &StorageTmp->mteObjectsIDLen);
 ...
 */
unsigned char *var_mteObjectsTable (struct variable *vp,
                                    oid * name,
                                    size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    *write_method = NULL;
     ...
     */
    if ((StorageTmp = header_complex (mteObjectsTableStorage, vp, name, length, exact, var_len, write_method)) == NULL)
    {
                             ...
                             u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    StorageTmp =
        header_complex (mteObjectsTableStorage, NULL,
                        &name[sizeof (mteObjectsTable_variables_oid) / sizeof (oid) + 3 - 1], &newlen, 1, NULL, NULL);
    ...
    switch (action)
    {
        ...
        case RESERVE2:
            ...
            if (StorageTmp == NULL)
            {
                ...
                if (header_complex_parse_oid
                    (&
                     (name
                      [sizeof (mteObjectsTable_variables_oid) / sizeof (oid) + 2]), newlen, vars) != SNMPERR_SUCCESS)
                {
                    /*
                     * XXX: free, zero vars 
                     */
                ...
                StorageNew->mteOwner = netsnmp_strdup_and_null (vp->val.string, vp->val_len);
                StorageNew->mteOwnerLen = vp->val_len;
                vp = vp->next_variable;
                StorageNew->mteObjectsName = netsnmp_strdup_and_null (vp->val.string, vp->val_len);
        ...
        case ACTION:
            ...
            else
            {
                 ...
                 */
                hciptr = header_complex_find_entry (mteObjectsTableStorage, StorageTmp);
                StorageDel = header_complex_extract_entry (&mteObjectsTableStorage, hciptr);
            }
            break;
        ...
        case COMMIT:
            ...
            else
            {
                ...
                else if (StorageTmp && StorageTmp->mteObjectsEntryStatus == RS_CREATEANDWAIT)
                {
                 ...
                 const char *name, oid * suffix, size_t suffix_len)
{
    ...
    if (vars == NULL || item == NULL || owner == NULL || name == NULL || hcptr == NULL)
        return;
    ...
    while (hcptr &&
           (strcmp (((struct mteObjectsTable_data *) hcptr->data)->mteOwner,
                    owner) != 0 || strcmp (((struct mteObjectsTable_data *) hcptr->data)->mteObjectsName, name) != 0))
        hcptr = hcptr->next;
                   ...
                   owner) == 0 && strcmp (((struct mteObjectsTable_data *) hcptr->data)->mteObjectsName, name) == 0)
    {
        ...
        struct mteObjectsTable_data *node = (struct mteObjectsTable_data *) hcptr->data;
        ...
        if (node->mteObjectsIDWildcard == MTEOBJECTSIDWILDCARD_TRUE && suffix && suffix_len > 0)
        {
            ...
            memcpy (&theoid[node->mteObjectsIDLen], suffix, suffix_len * sizeof (oid));
        }
        ...
        if (response)
        {
            if (vars)
            {
                ...
                DEBUGMSGOID (("mteObjectsTable", response->variables->name, response->variables->name_length));
...
int mte_add_object_to_table (const char *owner, const char *objname, oid * oidname, size_t oidname_len, int iswild)
{
    ...
    StorageNew->mteObjectsName = netsnmp_strdup_and_null (objname, strlen (objname));
    ...
    while (hcptr &&
           (strcmp (((struct mteObjectsTable_data *) hcptr->data)->mteOwner,
                    owner) != 0 ||
            strcmp (((struct mteObjectsTable_data *) hcptr->data)->mteObjectsName, objname) != 0))
        hcptr = hcptr->next;
    ...
    if (hcptr)
    {
        ...
        lastnode = hcptr;
        while (hcptr &&
               strcmp (((struct mteObjectsTable_data *) hcptr->data)->mteOwner, owner) == 0
               && strcmp (((struct mteObjectsTable_data *) hcptr->data)->mteObjectsName, objname) == 0)
        {
            lastnode = hcptr;
            hcptr = hcptr->next;
        }
        StorageNew->mteObjectsIndex = ((struct mteObjectsTable_data *) lastnode->data)->mteObjectsIndex + 1;
    }
    else
    {
