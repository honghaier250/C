        ...
        for (ksp = kstat_fd->kc_chain; ksp != NULL; ksp = ksp->ks_next)
        {
            ...
            if (strcmp (ksp->ks_module, "cpu_stat") == 0)
            {
                ...
                if ((ksp->ks_type != KSTAT_TYPE_RAW) ||
                    (ksp->ks_data_size != sizeof cs) || (kstat_read (kstat_fd, ksp, &cs) == -1))
                {
                 ...
                 */
                cs.cpu_sysinfo.cpu[CPU_WAIT] = cs.cpu_sysinfo.wait[W_IO] + cs.cpu_sysinfo.wait[W_PIO];
    ...
    if (number_of_snapshots > 0)
    {
        ...
        swapin =
            (uint_t) ((css_new->css_swapin -
                       css_old->css_swapin) * (hrtime_t) 1000 * sysconf (_SC_PAGESIZE) / 1024 / time_diff);
        /*
         * LINTED cast needed, really 
         */
        swapout =
            (uint_t) ((css_new->css_swapout -
                       css_old->css_swapout) * (hrtime_t) 1000 * sysconf (_SC_PAGESIZE) / 1024 / time_diff);
        /*
         * LINTED cast needed, really 
         */
        ...
        cpu_perc[CPU_SYSTEM] =
            (long) ((((css_new->css_cpu[CPU_KERNEL] -
                      ...
                      + (css_new->css_cpu[CPU_WAIT] - css_old->css_cpu[CPU_WAIT])) * 100 + (cpu_sum / 2)) / cpu_sum);
    }
                                      ...
                                      size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    switch (vp->magic)
    {
        ...
        case IORAWRECEIVE:
             ...
             */
        default:
            snmp_log (LOG_ERR, "vmstat_solaris2: Error in request, no match for %d.\n", vp->magic);
    }
    return (NULL);
