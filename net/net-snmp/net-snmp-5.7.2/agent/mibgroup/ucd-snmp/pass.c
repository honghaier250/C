...
void init_pass (void)
{
    snmpd_register_config_handler ("pass", pass_parse_config, pass_free_config, "miboid command");
}
...
void pass_parse_config (const char *token, char *cptr)
{
    ...
    if (numpassthrus > 1)
    {
        etmp = (struct extensible **) malloc (((sizeof (struct extensible *)) * numpassthrus));
        if (etmp == NULL)
            return;

        for (i = 0, ptmp = (struct extensible *) passthrus; i < numpassthrus && ptmp != NULL; i++, ptmp = ptmp->next)
            etmp[i] = ptmp;
                             ...
                             oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    for (i = 1; i <= numpassthrus; i++)
    {
        ...
        rtest = snmp_oidtree_compare (name, *length, passthru->miboid, passthru->miblen);
        if ((exact && rtest == 0) || (!exact && rtest <= 0))
        {
            ...
            if (exact)
                snprintf (passthru->command, sizeof (passthru->command), "%s -g %s", passthru->name, buf);
            else
                snprintf (passthru->command, sizeof (passthru->command), "%s -n %s", passthru->name, buf);
         ...
         size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    {
        passthru = get_exten_instance (passthrus, i);
        rtest = snmp_oidtree_compare (name, name_len, passthru->miboid, passthru->miblen);
        if (rtest <= 0)
        {
            if (action != ACTION)
            ...
            snprintf (passthru->command, sizeof (passthru->command), "%s -s %s ", passthru->name, buf);
            passthru->command[sizeof (passthru->command) - 1] = 0;
            ...
            DEBUGMSGTL (("ucd-snmp/pass", "pass-running:  %s", passthru->command));
            exec_command (passthru);
            DEBUGMSGTL (("ucd-snmp/pass", "pass-running returned: %s", passthru->output));
