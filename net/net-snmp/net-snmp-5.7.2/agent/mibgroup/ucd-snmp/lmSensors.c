 ...
 */
unsigned char *var_lmSensorsTable (struct variable *vp,
                                   oid * name,
                                   size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    if (header_simple_table (vp, name, length, exact, var_len, write_method, n_sensors) == MATCH_FAILED)
    {
        ret = NULL;
        goto leaving;
    }
...
static int read_num_sensor (picl_nodehdl_t childh, const char *prop, int scale, int *value)
{
    ...
    error_code = (picl_get_propinfo_by_name (childh, prop, &sensor_info, &sensorh));
...
static int read_enum_sensor (picl_nodehdl_t childh, const char **options, u_int * value)
{
    picl_nodehdl_t sensorh;
                    ...
                    const char propval[PICL_PROPNAMELEN_MAX], int typ, int scale)
{
    ...
    {
        DEBUGMSGTL (("ucd-snmp/lmSensors", "There are too many sensors of type %d\n", typ));
    }
    else
    {
        ...
        if (error_code == PICL_SUCCESS)
        {
            ...
            snprintf (sensor_array[typ].sensor[sensor_array[typ].n].name, (PICL_PROPNAMELEN_MAX - 1), "%s", propname);
...
static void
process_enum_sensor (picl_nodehdl_t childh, const char propname[PICL_PROPNAMELEN_MAX], int typ, const char **options)
{
    int value = 0;
...
static void process_individual_fan (picl_nodehdl_t childh, const char propname[PICL_PROPNAMELEN_MAX])
{
    process_num_sensor (childh, propname, "AtoDSensorValue", FAN_TYPE, 1);
}


static void process_newtype_fan (picl_nodehdl_t childh, const char propname[PICL_PROPNAMELEN_MAX])
{
    process_num_sensor (childh, propname, "Speed", FAN_TYPE, 1);
}


static void process_temperature_sensor (picl_nodehdl_t childh, const char propname[PICL_PROPNAMELEN_MAX])
{
    process_num_sensor (childh, propname, "Temperature", TEMP_TYPE, 1000);
}                                /* MIB asks for mC */

static void process_voltage_sensor (picl_nodehdl_t childh, const char propname[PICL_PROPNAMELEN_MAX])
{
    process_num_sensor (childh, propname, "Voltage", VOLT_TYPE, 1000);
}                                /* MIB asks for mV */

static void process_digital_sensor (picl_nodehdl_t childh, const char propname[PICL_PROPNAMELEN_MAX])
{
    process_num_sensor (childh, propname, "AtoDSensorValue", VOLT_TYPE, 1);
}


static void process_switch (picl_nodehdl_t childh, const char propname[PICL_PROPNAMELEN_MAX])
{
...
static void process_led (picl_nodehdl_t childh, const char propname[PICL_PROPNAMELEN_MAX])
{
...
static void process_i2c (picl_nodehdl_t childh, const char propname[PICL_PROPNAMELEN_MAX])
{
...
static int process_sensors (int level, picl_nodehdl_t nodeh)
{
    ...
    error_code = picl_get_propval_by_name (nodeh, PICL_PROP_CHILD, &childh, sizeof (picl_nodehdl_t));
    if (error_code != PICL_SUCCESS)
    {
        DEBUGMSGTL (("ucd-snmp/lmSensors", "picl_get_propval_by_name(%s) %d\n", PICL_PROP_CHILD, error_code));
        return (error_code);
    }
    ...
    while (error_code == PICL_SUCCESS)
    {
        ...
        error_code = picl_get_propval_by_name (childh, PICL_PROP_NAME, propname, (PICL_PROPNAMELEN_MAX - 1));
        if (error_code != PICL_SUCCESS)
        {                        /*we found a node with no name.  Impossible.! */
            DEBUGMSGTL (("ucd-snmp/lmSensors", "picl_get_propval_by_name(%s) = %d\n", PICL_PROP_NAME, error_code));
            return (error_code);
        }

        error_code = picl_get_propval_by_name (childh, PICL_PROP_CLASSNAME, propclass, sizeof (propclass));
        if (error_code != PICL_SUCCESS)
        {                        /*we found a node with no class.  Impossible.! */
...
*/
        ...
        if (!(strstr (propclass, "picl") && (strstr (propname, "frutree") || strstr (propname, "obp"))))
        {
            error_code = process_sensors (level, childh);
            DEBUGMSGTL (("ucd-snmp/lmSensors", "process_sensors(%s) returned %d\n", propname, error_code));
        }

        /* get next child node at this level */
        error_code = picl_get_propval_by_name (childh, PICL_PROP_PEER, &nexth, sizeof (picl_nodehdl_t));
        if (error_code != PICL_SUCCESS)
        {                        /* no more children - buh bye */
            ...
            else
            {
                ...
                for (i = 0; i < kp->ks_ndata; i++)
                {
                    DEBUGMSG (("ucd-snmp/lmSensors", "found instance %d fan type %d speed %d OK %d bustedfan %d\n",
                               fan_info->instance, fan_info->type, fan_info->fanspeed, fan_info->fans_ok,
                               fan_info->fanflt_num));
            ...
            else
            {
                ...
                for (i = 0; i < kp->ks_ndata; i++)
                {
                    DEBUGMSG (("ucd-snmp/lmSensors",
                               "found instance %d psupply temp mC %d %dW OK %d share %d limit %d\n",
                ...
                for (i = 0; i < kp->ks_ndata; i++)
                {
                    switch (enc_info->type)
                    {
                        case ENVCTRL_ENCL_FSP:
                            ...
                            strlcpy (sensor_array[typ].sensor[other].name, "FSP", MAX_NAME);
                            other++;
                            break;
                        case ENVCTRL_ENCL_AMBTEMPR:
                            ...
                            strlcpy (sensor_array[typ].sensor[temp].name, "Ambient", MAX_NAME);
                            temp++;
                            break;
                        case ENVCTRL_ENCL_BACKPLANE4:
                            ...
                            typ = 3;    /* misc */
                            sensor_array[typ].sensor[other].value = enc_info->value;
                            strlcpy (sensor_array[typ].sensor[other].name, "Backplane4", MAX_NAME);
                            other++;
                            break;
                        case ENVCTRL_ENCL_BACKPLANE8:
                            ...
                            typ = 3;    /* misc */
                            sensor_array[typ].sensor[other].value = enc_info->value;
                            strlcpy (sensor_array[typ].sensor[other].name, "Backplane8", MAX_NAME);
                            other++;
                            break;
    ...
    while ((chip = sensors_get_detected_chips (&chip_nr)))
    {
        ...
        while ((data = sensors_get_all_features (*chip, &a, &b)))
        {
            ...
            if ((data->mode & SENSORS_MODE_R) &&
                ...
                !sensors_get_label (*chip, data->number, &label) && !sensors_get_feature (*chip, data->number, &val))
            {
                int type = -1;
                ...
                DEBUGMSGTL (("sensors", "sensor %s, value %d\n",
                             array->sensor[array->n].name, array->sensor[array->n].value));
                array->n++;
            }                    /* end if data-mode */
            if (label)
            {
...
static void free_sensor_arrays ()
{
    unsigned int i = 0;
