...
netsnmp_feature_require (get_exten_instance) netsnmp_feature_require (parse_miboid)
...
void init_extensible (void)
{
     ...
     */
    oid extensible_variables_oid[] = { NETSNMP_UCDAVIS_MIB, NETSNMP_SHELLMIBNUM, 1 };

    /*
     * register ourselves with the agent to handle our mib tree 
     */
    REGISTER_MIB ("ucd-snmp/extensible", extensible_extensible_variables, variable2, extensible_variables_oid);

    snmpd_register_config_handler ("exec", extensible_parse_config,
                                   extensible_free_config, "[miboid] name program arguments");
    snmpd_register_config_handler ("sh", extensible_parse_config,
                                   extensible_free_config, "[miboid] name program-or-script arguments");
    snmpd_register_config_handler ("execfix", execfix_parse_config, NULL, "exec-or-sh-name program [arguments...]");
    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_PRE_UPDATE_CONFIG, extensible_unregister, NULL);
}
...
void extensible_parse_config (const char *token, char *cptr)
{
    ...
    if (isdigit (*cptr))
    {
         ...
         */
        config_perror
            ("WARNING: This output format is not valid, and is only retained for backward compatibility - Please consider using the 'extend' directive instead");
    ...
    if (ptmp->miblen > 0)
    {
        ...
        register_mib (token,
                      ...
                      sizeof (*extensible_relocatable_variables), ptmp->miboid, ptmp->miblen);
        ...
        if (scount > 1 && pp != &extens)
        {
            ...
            struct extensible **etmp = (struct extensible **) malloc (((sizeof (struct extensible *)) * scount));

            if (etmp == NULL)
                return;            /* XXX memory alloc error */
            for (i = 0, ptmp = *pp; i < scount && ptmp != 0; i++, ptmp = ptmp->next)
                etmp[i] = ptmp;
            qsort (etmp, scount, sizeof (struct extensible *), pass_compare);
...
int extensible_unregister (int major, int minor, void *serverarg, void *clientarg)
{
...
struct extensible *get_exec_by_name (char *name)
{
    ...
    for (etmp = extens; etmp != NULL && strcmp (etmp->name, name) != 0; etmp = etmp->next);

    if (NULL == etmp)
        for (etmp = relocs; etmp != NULL && strcmp (etmp->name, name) != 0; etmp = etmp->next);
    ...
    if ((exten = get_exten_instance (extens, name[*length - 1])))
    {
        switch (vp->magic)
        {
            ...
            case ERRORFLAG:    /* return code from the process */
                ...
                {
                    exten->result = run_exec_command (exten->command, NULL, exten->output, &len);
                }
                else
                {
                    exten->result = run_shell_command (exten->command, NULL, exten->output, &len);
                }
...
netsnmp_subtree *find_extensible (netsnmp_subtree * tp, oid * tname, size_t tnamelen, int exact)
{
    size_t tmp;
    ...
    static netsnmp_subtree mysubtree[2] = { {NULL, 0, NULL, 0, NULL, 0, NULL, 0, 0, NULL, NULL, 0, 0, 0,
    ...
    mysubtree[0].variables_len = sizeof (extensible_relocatable_variables) / sizeof (*extensible_relocatable_variables);
