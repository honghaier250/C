...
netsnmp_feature_require (handler_mark_requests_as_delegated) netsnmp_feature_require (request_set_error_idx)
            ...
            while (*optarg)
            {
                switch (*optarg++)
                {
                    case 'n':
                        ...
                        break;
                    case 'c':
                        netsnmp_ds_set_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_IGNORE_NO_COMMUNITY, 1);
                        break;
                    default:
...
void proxy_parse_config (const char *token, char *line)
{
    ...
    netsnmp_ds_set_boolean (NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_IGNORE_NO_COMMUNITY, 0);
     ...
     */
    listpp = &proxies;
    while (*listpp && snmp_oid_compare (newp->name, newp->name_len, (*listpp)->name, (*listpp)->name_len) > 0)
    {
        listpp = &((*listpp)->next);
    }
    ...
    *listpp = newp;
    ...
    reg = netsnmp_create_handler_registration ("proxy", proxy_handler, newp->name, newp->name_len, HANDLER_CAN_RWRITE);
...
void proxy_free_config (void)
{
    ...
    while (proxies)
    {
        ...
        unregister_mib_context (rm->name, rm->name_len, DEFAULT_MIB_PRIORITY, 0, 0, rm->context);
 ...
 */
int proxy_fill_in_session (netsnmp_mib_handler * handler, netsnmp_agent_request_info * reqinfo, void **configured)
{
    ...
    if (session->version == SNMP_VERSION_1 || session->version == SNMP_VERSION_2c)
    {
        ...
        if (session->community_len == 0)
        {
            ...
            if (reqinfo->asp == NULL || reqinfo->asp->pdu == NULL || reqinfo->asp->pdu->community_len == 0)
            {
                return 0;
            }
            ...
            *configured = strdup ("-c");
            ...
            sprintf ((char *) session->community, "%.*s",
                     (int) session->community_len, (const char *) reqinfo->asp->pdu->community);
        }
    }
#endif
 ...
 */
void proxy_free_filled_in_session_args (netsnmp_session * session, void **configured)
{
...
void init_proxy (void)
{
    snmpd_register_config_handler ("proxy", proxy_parse_config,
                                   proxy_free_config, "[snmpcmd args] host oid [remoteoid]");
}
               ...
               netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    while (request)
    {
        ...
        if (sp->base_len &&
            reqinfo->mode == MODE_GETNEXT && (snmp_oid_compare (ourname, ourlength, sp->base, sp->base_len) < 0))
        {
        ...
        else if (sp->base_len > 0)
        {
            ...
            if (ourlength > sp->name_len)
                memcpy (&(sp->base[sp->base_len]), &(ourname[sp->name_len]), sizeof (oid) * (ourlength - sp->name_len));
            ourlength = ourlength - sp->name_len + sp->base_len;
            ourname = sp->base;
        }

        snmp_pdu_add_variable (pdu, ourname, ourlength,
                               request->requestvb->type, request->requestvb->val.string, request->requestvb->val_len);
        request->delegated = 1;
    ...
    snmp_async_send (sp->sess, pdu, proxy_got_response,
                     netsnmp_create_delegated_cache (handler, reginfo, reqinfo, requests, (void *) sp));
...
int proxy_got_response (int operation, netsnmp_session * sess, int reqid, netsnmp_pdu * pdu, void *cb_data)
{
    ...
    switch (operation)
    {
        ...
        case NETSNMP_CALLBACK_OP_RECEIVED_MESSAGE:
            ...
            if (pdu->errstat != SNMP_ERR_NOERROR)
            {
                ...
                if ((cache->reqinfo->mode == MODE_GETNEXT) && (SNMP_ERR_NOSUCHNAME == pdu->errstat))
                {
                ...
                else if (cache->reqinfo->mode == MODE_SET_ACTION)
                {
                    ...
                    netsnmp_handler_mark_requests_as_delegated (requests, REQUEST_IS_NOT_DELEGATED);
                    netsnmp_request_set_error_idx (requests, pdu->errstat, pdu->errindex);
                }
#endif                            /* !NETSNMP_NO_WRITE_SUPPORT */
                else
                {
                ...
                for (var = vars, request = requests; request && var; request = request->next, var = var->next_variable)
                {
                    /*
                     * XXX - should this be done here?
                     *       Or wait until we know it's OK?
                     */
                    snmp_set_var_typed_value (request->requestvb, var->type, var->val.string, var->val_len);
                        ...
                        if (sp->base_len)
                        {
                            ...
                            if (var->name_length > sp->base_len)
                                memcpy (&myname[sp->name_len],
                                        &var->name[sp->base_len], sizeof (oid) * (var->name_length - sp->base_len));
                            snmp_set_var_objid (request->requestvb, myname, myname_len);
                        }
                        else
                        {
                            snmp_set_var_objid (request->requestvb, var->name, var->name_length);
                        }
                    }
                }
            ...
            if (request || var)
            {
                ...
                if (pdu)
                    snmp_free_pdu (pdu);
                snmp_log (LOG_ERR, "response to proxy request illegal.  We're screwed.\n");
                netsnmp_set_request_error (cache->reqinfo, requests, SNMP_ERR_GENERR);
            }
