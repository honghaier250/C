...
static void disk_free_config (void)
{
    ...
    for (entry = netsnmp_fsys_get_first (); entry != NULL; entry = netsnmp_fsys_get_next (entry))
    {
        ...
        case ERRORMSG:
            ...
            if ((entry->minspace >= 0) && (val < entry->minspace))
                snprintf (errmsg, sizeof (errmsg),
                          "%s: less than %d free (= %d)", entry->path, entry->minspace, (int) val);
            else if ((entry->minpercent >= 0) && (_percent (entry->avail, entry->size) < entry->minpercent))
                snprintf (errmsg, sizeof (errmsg),
                          "%s: less than %d%% free (= %d%%)",
                          entry->path, entry->minpercent, _percent (entry->avail, entry->size));
