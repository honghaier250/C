...
void init_loadave (void)
{
     ...
     */
    REGISTER_MIB ("ucd-snmp/loadave", extensible_loadave_variables, variable2, loadave_variables_oid);

    laConfigSet = 0;

    snmpd_register_config_handler ("load", loadave_parse_config, loadave_free_config, "max1 [max5] [max15]");
     ...
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, loadave_store_config, NULL);
...
void loadave_parse_config (const char *token, char *cptr)
{
    ...
    if (strcmp (token, "pload") == 0)
    {
        if (laConfigSet < 0)
        {
            snmp_log (LOG_WARNING, "ignoring attempted override of read-only load\n");
            return;
        }
        else
        {
                ...
                u_char * var_val, u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:            /* Check values for acceptability */
            ...
            {
                DEBUGMSGTL (("ucd-snmp/loadave", "write to laConfig not ASN_OCTET_STR\n"));
                return SNMP_ERR_WRONGTYPE;
            }
            if (var_val_len > 8 || var_val_len <= 0)
            {
                DEBUGMSGTL (("ucd-snmp/loadave", "write to laConfig: bad length\n"));
        ...
        case RESERVE2:            /* Allocate memory and similar resources */
            {
                ...
                {
                    errno = old_errno;
                    DEBUGMSGTL (("ucd-snmp/loadave", "write to laConfig: invalid value\n"));
        ...
        case ERRORMSG:
            if (maxload[name[*length - 1] - 1] != 0 && avenrun[name[*length - 1] - 1] >= maxload[name[*length - 1] - 1])
            {
                snprintf (errmsg, sizeof (errmsg),
                          ...
                          (name[*length - 1] ==
                           1) ? 1 : ((name[*length - 1] == 2) ? 5 : 15), avenrun[name[*length - 1] - 1]);
