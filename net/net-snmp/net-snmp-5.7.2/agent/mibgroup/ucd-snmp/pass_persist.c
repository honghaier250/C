...
void init_pass_persist (void)
{
    snmpd_register_config_handler ("pass_persist",
                                   pass_persist_parse_config, pass_persist_free_config, "miboid program");
...
void pass_persist_parse_config (const char *token, char *cptr)
{
    ...
    {
        for (tcptr = cptr; *tcptr != 0 && *tcptr != '#' && *tcptr != ';'; tcptr++);
    ...
    register_mib_priority ("pass_persist",
                           ...
                           sizeof (struct variable2), 1, (*ppass)->miboid, (*ppass)->miblen, (*ppass)->mibpriority);
    ...
    {
        etmp = (struct extensible **) malloc (((sizeof (struct extensible *)) * numpersistpassthrus));
        if (etmp == NULL)
            return;
        for (i = 0, ptmp = (struct extensible *) persistpassthrus;
             i < numpersistpassthrus && ptmp != NULL; i++, ptmp = ptmp->next)
            etmp[i] = ptmp;
        qsort (etmp, numpersistpassthrus, sizeof (struct extensible *), pass_persist_compare);
                                     ...
                                     size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    for (i = 1; i <= numpersistpassthrus; i++)
    {
        ...
        rtest = snmp_oidtree_compare (name, *length, persistpassthru->miboid, persistpassthru->miblen);
        if ((exact && rtest == 0) || (!exact && rtest <= 0))
        {
            ...
            if (exact)
                snprintf (persistpassthru->command, sizeof (persistpassthru->command), "get\n%s\n", buf);
            else
                snprintf (persistpassthru->command, sizeof (persistpassthru->command), "getnext\n%s\n", buf);
            ...
            if ((file = persist_pipes[i].fIn))
            {
                ...
                *write_method = setPassPersist;
                ...
                if (newlen == 0 || fgets (buf, sizeof (buf), file) == NULL || fgets (buf2, sizeof (buf2), file) == NULL)
                {
                    *var_len = 0;
                    close_persist_pipe (i);
                    return (NULL);
                ...
                u_char * var_val, u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    {
        persistpassthru = get_exten_instance (persistpassthrus, i);
        rtest = snmp_oidtree_compare (name, name_len, persistpassthru->miboid, persistpassthru->miblen);
        if (rtest <= 0)
        {
            if (action != ACTION)
            ...
            snprintf (persistpassthru->command, sizeof (persistpassthru->command), "set\n%s\n", buf);
            persistpassthru->command[sizeof (persistpassthru->command) - 1] = 0;
            netsnmp_internal_pass_set_format (buf, var_val, var_val_type, var_val_len);
            strlcat (persistpassthru->command, buf, sizeof (persistpassthru->command));
...
static int init_persist_pipes (void)
{
     ...
     */
    persist_pipes = (struct persist_pipe_type *) malloc (sizeof (struct persist_pipe_type) * (numpersistpassthrus + 1));
    if (persist_pipes)
    {
...
static int open_persist_pipe (int iindex, char *command)
{
     ...
     */
    {
        ...
        {
            DEBUGMSGTL (("ucd-snmp/pass_persist", "open_persist_pipe: Got %s instead of PONG!\n", buf));
            close_persist_pipe (iindex);
            recurse = 0;
            return 0;
        }
...
static int write_persist_pipe (int iindex, const char *data)
{
    ...
    if (persist_pipes[iindex].pid == NETSNMP_NO_SUCH_PROCESS)
    {
        DEBUGMSGTL (("ucd-snmp/pass_persist", "write_persist_pipe: not writing %s, process is non-existent", data));
        return 0;
    }
    ...
    {
        DEBUGMSGTL (("ucd-snmp/pass_persist", "write_persist_pipe: sigaction failed: %d", errno));
    }
