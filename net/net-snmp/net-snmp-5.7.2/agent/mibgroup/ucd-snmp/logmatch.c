...
static void updateLogmatch (int iindex)
{
    ...
    if (stat (logmatchTable[iindex].filename, &sb) == 0)
    {
        ...
        if ((logmatchTable[iindex].logfile = fopen (logmatchTable[iindex].filename, "r")))
        {
            ...
            while (fgets (inbuf, sizeof (inbuf), logmatchTable[iindex].logfile))
            {
                ...
                matchResultCode = regexec (&(logmatchTable[iindex].regexBuffer), inbuf, 0, NULL, REG_NOTEOL);
    ...
    if (anyChanges && (perfile = fopen (perfilename, "w")))
    {
        ...
        fprintf (perfile, "%lu %lu %lu %s\n",
                 ...
                 logmatchTable[iindex].currentMatchCounter,
                 logmatchTable[iindex].globalMatchCounter, logmatchTable[iindex].filename);
...
static void updateLogmatch_Scheduled (unsigned int registrationNumber, struct logmatchstat *logmatchtable)
{
...
static void logmatch_parse_config (const char *token, char *cptr)
{
    ...
    if (logmatchCount < MAXLOGMATCH)
    {
        ...
        sscanf (cptr, "%255s%c%255s%c %d %255c\n",
                ...
                &space_path, &(logmatchTable[logmatchCount].frequency), logmatchTable[logmatchCount].regEx);
        ...
        if (logmatchTable[logmatchCount].frequency > 0)
        {
            snmp_alarm_register (logmatchTable[logmatchCount].frequency,
                                 SA_REPEAT,
                                 (SNMPAlarmCallback *) updateLogmatch_Scheduled, &(logmatchTable[logmatchCount]));
        }
...
static u_char *var_logmatch_table (struct variable *vp,
                                   oid * name,
...
void init_logmatch (void)
{
    ...
    snmpd_register_config_handler ("logmatch", logmatch_parse_config,
                                   logmatch_free_config, "logmatch name path cycletime regex");
