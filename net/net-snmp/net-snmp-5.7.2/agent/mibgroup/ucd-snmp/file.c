...
void init_file (void)
{
    ...
    snmpd_register_config_handler ("file", file_parse_config, file_free_config, "file [maxsize]");
                        ...
                        oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    switch (vp->magic)
    {
        ...
        case FILE_MSG:
            if (file->max >= 0 && file->size > file->max)
                snprintf (error, sizeof (error), FILE_ERROR_MSG, file->name, file->max, file->size);
            else
                strcpy (error, "");
