...
void init_proc (void)
{
     ...
     */
    REGISTER_MIB ("ucd-snmp/proc", extensible_proc_variables, variable2, proc_variables_oid);

    snmpd_register_config_handler ("proc", proc_parse_config, proc_free_config, "process-name [max-num] [min-num]");
    snmpd_register_config_handler ("procfix", procfix_parse_config, NULL, "process-name program [arguments...]");
}
...
static struct myproc *get_proc_by_name (char *name)
{
    ...
    for (ptmp = procwatch; ptmp != NULL && strcmp (ptmp->name, name) != 0; ptmp = ptmp->next);
    return ptmp;
}
    ...
    if ((proc = get_proc_instance (procwatch, name[*length - 1])))
    {
        switch (vp->magic)
        {
            ...
            case ERRORMSG:
                ...
                {
                    if (long_ret > 0)
                        snprintf (errmsg, sizeof (errmsg), "Too few %s running (# = %d)", proc->name, (int) long_ret);
                    else
                        snprintf (errmsg, sizeof (errmsg), "No %s process running", proc->name);
                }
                else if (proc->max && long_ret > proc->max)
                {
                    snprintf (errmsg, sizeof (errmsg), "Too many %s running (# = %d)", proc->name, (int) long_ret);
                }
                else if (proc->min == 0 && proc->max == 0 && long_ret > 0)
                {
                    snprintf (errmsg, sizeof (errmsg), "%s process should not be running.", proc->name);
                }
                else
                {
                    errmsg[0] = 0;
...
int sh_count_procs (char *procname)
{
    ...
    {
        if ((pbase = (struct kinfo_proc *) realloc (pbase, nproc + sizeof (struct kinfo_proc))) == 0)
            return -1;
        onproc = nproc;
    ...
    for (;;)
    {
        do
        {
            while (thisproc == 0)
            {
                ...
                nproc -= thisproc;
                if (lseek (kmem, (off_t) procp, L_SET) == -1 || (nread = read (kmem, (char *) procs, psize)) < 0)
                {
                    /*
                     * warn("read proc"); 
                     */
            ...
            if ((absolute_proc_number % 32) == 0)
                bitmap = getword ((unsigned int) proc_nl[X_PROC_BITMAP].n_value + ((absolute_proc_number / 32) * 4));
...
static struct user *getuser (struct proc *aproc)
{
    ...
    {
        if (!getstruct ((off_t) aproc->p_smap, "aproc->p_smap", (off_t) & dmap, sizeof (dmap)))
        {
            /*
