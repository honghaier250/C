...
void init_diskio (void)
{
    ...
    netsnmp_ds_register_config (ASN_BOOLEAN, app, "diskio_exclude_loop",
                                NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_DISKIO_NO_LOOP);
    netsnmp_ds_register_config (ASN_BOOLEAN, app, "diskio_exclude_ram",
                                NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_DISKIO_NO_RAM);
...
static void diskio_free_config (void)
{
    netsnmp_ds_set_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_DISKIO_NO_FD, 0);
    netsnmp_ds_set_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_DISKIO_NO_LOOP, 0);
    netsnmp_ds_set_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_DISKIO_NO_RAM, 0);
}
#endif
...
u_char *var_diskio (struct variable * vp,
...
static int getstats (void)
{
    ...
    if (ndisk != size / dmib[2])
    {
        ...
        for (i = 0, tp = strtok (t, " "); tp && i < ndisk; i++, tp = strtok (NULL, " "))
        {
            dkname[i] = strdup (tp);
        }
        free (t);
...
{
    if (netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_DISKIO_NO_FD) && !(strncmp (name, "fd", 2)))
        return 1;
    if (netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID,
                                NETSNMP_DS_AGENT_DISKIO_NO_LOOP) && !(strncmp (name, "loop", 4)))
        return 1;
    if (netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID,
                                NETSNMP_DS_AGENT_DISKIO_NO_RAM) && !(strncmp (name, "ram", 3)))
        return 1;
    return 0;
}
...
static int collect_drive_stats (io_registry_entry_t driver, long *stats)
{
    CFNumberRef number;
 ...
 */
static int handle_drive (io_registry_entry_t drive, struct drivestats *dstat)
{
...
/*    fprintf(stderr, "device has no parent\n"); */
    ...
    if (IOObjectConformsTo (parent, "IOBlockStorageDriver"))
    {
        ...
        status = IORegistryEntryCreateCFProperties (drive, &properties, kCFAllocatorDefault, kNilOptions);
        if (status != KERN_SUCCESS)
        {
            snmp_log (LOG_ERR, "diskio: device has no properties\n");
                    ...
                    oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    if (header_simple_table (vp, name, length, exact, var_len, write_method, num_drives))
    {
        return NULL;
    }
                    ...
                    oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    if (header_simple_table (vp, name, length, exact, var_len, write_method, ps_numdisks))
        return NULL;
