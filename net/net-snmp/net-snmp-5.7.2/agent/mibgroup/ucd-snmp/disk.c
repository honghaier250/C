...
static void add_device (char *path, char *device, int minspace, int minpercent, int override);

static void modify_disk_parameters (int index, int minspace, int minpercent);
...
void init_disk (void)
{
     ...
     */
    REGISTER_MIB ("ucd-snmp/disk", extensible_disk_variables, variable2, disk_variables_oid);

    snmpd_register_config_handler ("disk", disk_parse_config, disk_free_config, "path [ minspace | minpercent% ]");
    snmpd_register_config_handler ("includeAllDisks", disk_parse_config_all, disk_free_config, "minpercent%");
    allDisksIncluded = 0;
}
...
void modify_disk_parameters (int index, int minspace, int minpercent)
{
...
static void find_and_add_allDisks (int minpercent)
{
    ...
    else
    {
        ...
        netsnmp_config_warn ("Couldn't find device for disk %s", disks[numdisks].path);
        disks[numdisks].minimumspace = -1;
        disks[numdisks].minpercent = -1;
        disks[numdisks].path[0] = 0;
    }
...
static int fill_dsk_entry (int disknum, struct dsk_entry *entry)
{
...
#define filesys sb.iu_fs
    ...
    entry->dskPercentInode = vfs.f_files == 0 ? 100.0 :
        (int) ((double) (vfs.f_files - vfs.f_ffree) / (double) (vfs.f_files) * 100.0 + 0.5);
#endif
    ...
    free = filesys.fs_cstotal.cs_nbfree * filesys.fs_frag + filesys.fs_cstotal.cs_nffree;
    used = totalblks - free;
    availblks = totalblks * (100 - filesys.fs_minfree) / 100;
    avail = availblks > used ? availblks - used : 0;
    entry->dskPercent =
        totalblks == 0 ? 0 : availblks == 0 ? 100 : (int) ((double) used / (double) totalblks * 100.0 + 0.5);
            ...
            if (entry.dskErrorFlag)
            {
                if (disks[disknum].minimumspace >= 0)
                    snprintf (errmsg, sizeof (errmsg),
                              ...
                              disks[disknum].path, disks[disknum].minimumspace, (int) entry.dskAvail);
                else
                    snprintf (errmsg, sizeof (errmsg),
                              "%s: less than %d%% free (= %d%%)",
                              disks[disknum].path, disks[disknum].minpercent, (int) entry.dskPercent);
