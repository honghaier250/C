...
static void dlmod_load_module (struct dlmod *dlm)
{
    ...
    if (dlm->path[0] == '/')
    {
        ...
        if (dlm->handle == NULL)
        {
            snprintf (dlm->error, sizeof (dlm->error), "dlopen failed: %s", dlerror ());
    ...
    }
    {
        ...
        {
            dlclose (dlm->handle);
            snprintf (dlm->error, sizeof (dlm->error), "dlsym failed: can't find \'%s\'", sym_init);
            dlm->status = DLMOD_ERROR;
            return;
        }
        dl_init ();
...
static u_char *var_dlmod (struct variable *vp,
                          oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    *var_len = sizeof (int);    /* assume an integer and change later
    ...
    if (header_dlmod (vp, name, length, exact, var_len, write_method) == MATCH_FAILED)
        return NULL;
...
static struct dlmod *header_dlmodEntry (struct variable *vp,
                                        oid * name,
...
static u_char *var_dlmodEntry (struct variable *vp,
                               oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    /*
     * variables we may use later
...
void init_dlmod (void)
{
    ...
    snmpd_register_config_handler ("dlmod", dlmod_parse_config, dlmod_free_config, "module-name module-path");
