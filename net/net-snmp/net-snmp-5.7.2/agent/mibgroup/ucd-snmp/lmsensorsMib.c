...
void initialize_lmSensorsTable (const char *tableName, const oid * tableOID, netsnmp_container_op * filter, int mult);
...
void init_lmsensorsMib (void)
{
     ...
     */
    initialize_lmSensorsTable ("lmTempSensorsTable", lmTempSensorsTable_oid, _sensor_filter_temp, 1000);    /* MIB asks for mC */
    initialize_lmSensorsTable ("lmFanSensorsTable", lmFanSensorsTable_oid, _sensor_filter_fan, 1);
    initialize_lmSensorsTable ("lmVoltSensorsTable", lmVoltSensorsTable_oid, _sensor_filter_volt, 1000);    /* MIB asks for mV */
    initialize_lmSensorsTable ("lmMiscSensorsTable", lmMiscSensorsTable_oid, _sensor_filter_misc, 1);
}
...
void initialize_lmSensorsTable (const char *tableName, const oid * tableOID, netsnmp_container_op * filter, int mult)
{
     ...
     */
    reg = netsnmp_create_handler_registration (tableName, lmSensorsTables_handler,
    ...
    if (cache)
    {
        netsnmp_inject_handler_before (reg, netsnmp_cache_handler_get (cache), "table_container");
    }
...
int _sensor_filter_volt (netsnmp_container * c, const void *v)
{
    ...
    return (((sp->type == NETSNMP_SENSOR_TYPE_VOLTAGE_DC) || (sp->type == NETSNMP_SENSOR_TYPE_VOLTAGE_AC)) ? 0 : 1);
}
...
int _sensor_filter_misc (netsnmp_container * c, const void *v)
{
    ...
    return (((sp->type == NETSNMP_SENSOR_TYPE_TEMPERATURE) ||
             ...
             (sp->type == NETSNMP_SENSOR_TYPE_VOLTAGE_DC) || (sp->type == NETSNMP_SENSOR_TYPE_VOLTAGE_AC)) ? 1 : 0);
}
 ...
 */
int
lmSensorsTables_handler (netsnmp_mib_handler * handler,
                         ...
                         netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            ...
            {
                sensor_info = (netsnmp_sensor_info *) netsnmp_container_table_extract_context (request);
                if (!sensor_info)
                {
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_LMSENSORS_INDEX:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, sensor_info->idx.oids[0]);
                        break;
                    ...
                    case COLUMN_LMSENSORS_VALUE:
                        ...
                        snmp_set_var_typed_integer (request->requestvb, ASN_GAUGE, (int) (mult * sensor_info->value));
                        break;
                    default:
