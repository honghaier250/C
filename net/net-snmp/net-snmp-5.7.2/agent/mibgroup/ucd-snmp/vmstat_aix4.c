...
void init_vmstat_aix4 (void)
{
     ...
     */
    REGISTER_MIB ("ucd-snmp/vmstat", extensible_vmstat_variables, variable2, vmstat_variables_oid);
    ...
    {
        snmp_log (LOG_WARNING, "vmstat_aix4 (init): snmp_alarm_register failed.\n");
    }
    /*
    ...
    {
        snmp_log (LOG_ERR, "vmstat_aix4 (init): snmp_alarm_register failed, cannot service requests.\n");
    }
...
static int take_snapshot (struct cpu_stat_snapshot *css)
{
    ...
    if ((css->css_time == 0) || (current_time > css->css_time + 2000000))
    {
        /*
...
static void update_stats (unsigned int registrationNumber, void *clientarg)
{
    ...
    DEBUGMSGTL (("ucd-snmp/vmstat_aix4.c:update_stats", "updating stats\n"));
    ...
    {
        snmp_log (LOG_WARNING, "vmstat_aix4 (update_stats): Something went wrong with take_snapshot.\n");
        return;
    }
    ...
    if (number_of_snapshots > 0)
    {
        ...
        if (snapshot[0].css_cpus != snapshot[1].css_cpus)
        {
            ...
            {
                snmp_log (LOG_NOTICE, "vmstat_aix4 (update_stats): Lost at least one CPU, RIP.\n");
            }
            /*
            ...
            if (snmp_alarm_register (5, NULL, update_stats, NULL) == 0)
            {
                snmp_log (LOG_WARNING, "vmstat_aix4 (update_stats): snmp_alarm_register failed.\n");
            }
            return;
        }
         ...
         */
        time_diff = (snapshot[0].css_time - snapshot[number_of_snapshots].css_time) / 1000;

        DEBUGMSGTL (("ucd-snmp/vmstat_aix4.c:update_stats", "time_diff: %lld\n", time_diff));
         ...
         */
        swapin = (uint_t) ((css_new->css_swapin - css_old->css_swapin) * getpagesize () / 1024 / time_diff);
        /*
         * LINTED cast needed, really 
         */
        swapout = (uint_t) ((css_new->css_swapout - css_old->css_swapout) * getpagesize () / 1024 / time_diff);
        /*
         * LINTED cast needed, really 
         */
        blocks_read = (uint_t) ((css_new->css_blocks_read - css_old->css_blocks_read) / time_diff);
        /*
         * LINTED cast needed, really 
         */
        blocks_write = (uint_t) ((css_new->css_blocks_write - css_old->css_blocks_write) / time_diff);
        /*
         * LINTED cast needed, really 
         */
        interrupts = (uint_t) ((css_new->css_interrupts - css_old->css_interrupts) / time_diff);
        /*
         * LINTED cast needed, really 
         */
        context_sw = (uint_t) ((css_new->css_context_sw - css_old->css_context_sw) / time_diff);
                                      ...
                                      size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    switch (vp->magic)
    {
        ...
        case CPURAWSYSTEM:
             ...
             * LINTED has to be 'long' 
             */
            long_ret = (long) ((raw_values.css_cpu[CPU_SYSTEM] + raw_values.css_cpu[CPU_WAIT]) / raw_values.css_cpus);
            return ((u_char *) (&long_ret));
        case CPURAWIDLE:
            take_snapshot (&raw_values);
            /*
        ...
        case CPURAWKERNEL:
             ...
             * LINTED has to be 'long' 
             */
            long_ret = (long) (raw_values.css_cpu[CPU_SYSTEM] / raw_values.css_cpus);
            return ((u_char *) (&long_ret));
        case IORAWSENT:
        ...
        case SYSRAWCONTEXT:
             ...
             */
        default:
            snmp_log (LOG_ERR, "vmstat_aix4: Error in request, no match found.\n");
    }
    return (NULL);
