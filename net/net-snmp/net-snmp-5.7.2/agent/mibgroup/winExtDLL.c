 ...
 */
     ...
     static int register_netsnmp_handler (winextdll_view * const ext_dll_view_info);
     ...
     static int append_windows_varbind_list (netsnmp_variable_list **
                                             const net_snmp_varbinds, const SnmpVarBindList * const win_varbinds);
     static int append_windows_varbind (netsnmp_variable_list **
                                        const net_snmp_varbinds, const SnmpVarBind * const win_varbind);
     static int convert_to_windows_varbind_list (SnmpVarBindList *
                                                 pVarBindList, netsnmp_variable_list * netsnmp_varbinds);
     static int convert_win_snmp_err (const int win_snmp_err);

     static winextdll_view *lookup_view_by_oid (oid * const name, const size_t name_len);

     static int snmp_oid_compare_n_w (const oid * name1, size_t len1, const UINT * name2, UINT len2);

     static int snmp_oid_compare_w_n (const UINT * name1, UINT len1, const oid * name2, size_t len2);

     static int netsnmp_oid_is_subtree_n_w (const oid * name1, size_t len1, const UINT * name2, UINT len2);
     ...
     static UINT *copy_oid_to_new_windows_oid (AsnObjectIdentifier *
                                               const windows_oid, const oid * const name, const size_t name_len);
     static int snmp_set_var_objid_w (netsnmp_variable_list * var, const UINT * name, UINT name_length);

     static netsnmp_variable_list *snmp_varlist_add_variable_w (netsnmp_variable_list ** varlist,
     ...
     static void send_trap (const AsnObjectIdentifier * const,
                            const AsnInteger, const AsnInteger, const AsnTimeticks, const SnmpVarBindList * const);
...
void init_winExtDLL (void)
{
    ...
    for (i = 0; i < s_winextdll.size; i++)
    {
        ...
        if (ext_dll_info->pfSnmpExtensionQuery == NULL && ext_dll_info->pfSnmpExtensionQueryEx == NULL)
        {
            snmp_log (LOG_ERR, "error in extension DLL %s: SNMP query function missing.\n", ext_dll_info->dll_name);
        }
        ...
        view.idLength = 0;
        view.ids = 0;
        result = ext_dll_info->pfSnmpExtensionInit (GetTickCount () / 10, &ext_dll_info->subagentTrapEvent, &view);

        if (!result)
        {
            snmp_log (LOG_ERR, "init_winExtDLL: initialization of DLL %s failed.\n", ext_dll_info->dll_name);
        ...
        {
            xarray_push_back (&s_trapevent, &ext_dll_info->subagentTrapEvent);
        ...
        copy_oid_n_w (ext_dll_view_info.name, &ext_dll_view_info.name_length, view.ids, view.idLength);
        xarray_push_back (&s_winextdll_view, &ext_dll_view_info);

        /*
         * Loop looking for more supported views. 
         */
        while (ext_dll_info->pfSnmpExtensionInitEx && ext_dll_info->pfSnmpExtensionInitEx (&view))
        {
            memset (&ext_dll_view_info, 0, sizeof (ext_dll_view_info));
            ext_dll_view_info.winextdll_info = ext_dll_info;
            copy_oid_n_w (ext_dll_view_info.name, &ext_dll_view_info.name_length, view.ids, view.idLength);
    ...
    DEBUGMSG (("winExtDLL", "init_winExtDLL: registered %d OID ranges.\n", s_winextdll_view.size));
...
void shutdown_winExtDLL (void)
{
    ...
    for (i = s_winextdll_view.size - 1; i >= 0; i--)
    {
        ...
        if (v && v->my_handler)
        {
            ...
            {
                DEBUGMSG (("winExtDLL",
                           "unregistering handler for DLL %s and OID prefix ", v->winextdll_info->dll_name));
...
static int basename_equals (const char *path, const char *basename)
{
    ...
    return path_len >= basename_len + 1
        && path[path_len - basename_len - 1] == '\\' && stricmp (path + path_len - basename_len, basename) == 0;
}
 ...
 */
int register_netsnmp_handler (winextdll_view * const ext_dll_view_info)
{
    ...
    previously_registered_view = lookup_view_by_oid (ext_dll_view_info->name, ext_dll_view_info->name_length);
    ...
    if (previously_registered_view)
    {
        ...
        buffer_large_enough =
            sprint_realloc_objid ((u_char **) & oid_name, &oid_namelen,
                                  &outlen, 1, ext_dll_view_info->name, ext_dll_view_info->name_length);
        snmp_log (LOG_INFO, "OID range %s%s: replacing handler %s by %s.\n",
                  oid_name ? oid_name : "",
                  buffer_large_enough ? "" : " [TRUNCATED]",
                  previously_registered_view->winextdll_info->dll_name, ext_dll_view_info->winextdll_info->dll_name);
        if (oid_name)
            free (oid_name);
    ...
    else
    {
        ...
        ext_dll_view_info->my_handler
            = netsnmp_create_handler_registration (ext_dll_info->dll_name,
                                                   ...
                                                   ext_dll_view_info->name_length, HANDLER_CAN_RWRITE);

        if (ext_dll_view_info->my_handler)
        {
            ext_dll_view_info->my_handler->handler->myvoid = ext_dll_view_info;
            if (netsnmp_register_handler (ext_dll_view_info->my_handler) == MIB_REGISTERED_OK)
            {
                DEBUGIF ("winExtDLL")
                {
                    DEBUGMSG (("winExtDLL", "registering handler for DLL %s and OID prefix ", ext_dll_info->dll_name));
                    DEBUGMSGOID (("winExtDLL", ext_dll_view_info->name, ext_dll_view_info->name_length));
                    DEBUGMSG (("winExtDLL", " ("));
                    DEBUGMSGSUBOID (("winExtDLL", ext_dll_view_info->name, ext_dll_view_info->name_length));
                    DEBUGMSG (("winExtDLL", ").\n"));
                }
                return 1;
            }
 ...
 *   information.
 */
static context_info *alloc_context_info (const int index)
{
    context_info *p;
 ...
 * @param[in] index Varbind index in original PDU.
 */
static void free_context_info (const int index)
{
 ...
 * @param[in] index Varbind index in original PDU.
 */
static AsnOctetString *get_context_info (const int index)
{
    context_info *p;
               ...
               netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    ext_dll_info = ext_dll_view_info->winextdll_info;
#if ! defined(NDEBUG)
    netsnmp_assert (ext_dll_view_info == lookup_view_by_oid (reginfo->rootoid, reginfo->rootoid_len));
#endif

    if (ext_dll_info == 0)
    {
        DEBUGMSG (("winExtDLL", "internal error: no matching extension DLL found.\n"));
        netsnmp_assert (0);
    ...
    for (request = requests; request; request = request->next)
    {
        ...
        if (reqinfo->mode == MODE_GETNEXT
            && snmp_oid_compare_w_n (win_varbinds.list[0].name.ids,
                                     win_varbinds.list[0].name.idLength, reginfo->rootoid, reginfo->rootoid_len) < 0)
        {

            SnmpUtilOidFree (&win_varbinds.list[0].name);
            memset (&win_varbinds.list[0].name, 0, sizeof (win_varbinds.list[0].name));
            copy_oid_to_new_windows_oid (&win_varbinds.list[0].name, reginfo->rootoid, reginfo->rootoid_len);
        }
        ...
        {
            result = ext_dll_info->pfSnmpExtensionQuery ((BYTE) nRequestType, &win_varbinds, &ErrorStatus, &ErrorIndex);
        }
        else
        {
            snmp_log (LOG_ERR, "error in extension DLL %s: SNMP query function missing.\n", ext_dll_info->dll_name);
        ...
        if (!result)
        {
            snmp_log (LOG_ERR, "extension DLL %s: SNMP query function failed.\n", ext_dll_info->dll_name);
            rc = SNMP_ERR_GENERR;
            goto free_win_varbinds;
        }
        ...
        if (rc != SNMP_ERR_NOERROR)
        {
            ...
            netsnmp_request_set_error (requests, rc);
            if (rc == SNMP_NOSUCHOBJECT || rc == SNMP_NOSUCHINSTANCE || rc == SNMP_ERR_NOSUCHNAME)
                rc = SNMP_ERR_NOERROR;
            goto free_win_varbinds;
        }
        ...
        else if (reqinfo->mode == MODE_GETNEXT)
        {
            ...
            if (netsnmp_oid_is_subtree_n_w (ext_dll_view_info->name,
                ...
                && snmp_oid_compare_n_w (varbind->name, varbind->name_length,
                                         win_varbind->name.ids, win_varbind->name.idLength) < 0)
            {
                /*
                 * Copy the OID returned by the extension DLL to the
                 * Net-SNMP varbind.
                 */
                snmp_set_var_objid_w (varbind, win_varbind->name.ids, win_varbind->name.idLength);
                copy_value = TRUE;
            }
        }
        if (copy_value)
        {
            ...
            if (result_vb)
            {
                snmp_set_var_typed_value (varbind, result_vb->type, result_vb->val.string, result_vb->val_len);
      ...
      free_win_varbinds:
        if (reqinfo->mode == MODE_SET_COMMIT || reqinfo->mode == MODE_SET_UNDO || reqinfo->mode == MODE_SET_FREE)
 ...
 */
void read_extension_dlls_from_registry ()
{
    DEBUGMSGTL (("winExtDLL", "read_extension_dlls_from_registry called\n"));

    read_extension_dlls_from_registry_at ("SYSTEM\\CurrentControlSet\\Services\\SNMP\\Parameters\\ExtensionAgents");
    read_extension_dlls_from_registry_at ("SYSTEM\\CurrentControlSet\\Control\\SNMP\\Parameters\\ExtensionAgents");
}

void read_extension_dlls_from_registry_at (const char *const subkey)
{
    DWORD retCode;
    ...
    retCode = RegOpenKeyExA (HKEY_LOCAL_MACHINE, subkey, 0, KEY_QUERY_VALUE, &hKey);
    ...
    if (retCode == ERROR_SUCCESS)
    {
        for (i = 0;; i++)
        {
            ...
            retCode = RegEnumValue (hKey, i, valueName, &valueSize, NULL, &dataType, (BYTE *) data, &dataSize);
            ...
            if (dataType == REG_SZ)
            {
                ...
                ext_dll_info.dll_name = read_extension_dll_path_from_registry (data);
                if (ext_dll_info.dll_name)
                {
...
char *read_extension_dll_path_from_registry (const TCHAR * keyName)
{
    HKEY hKey;
    ...
    retCode = RegOpenKeyExA (HKEY_LOCAL_MACHINE, keyName, 0, KEY_QUERY_VALUE, &hKey);

    if (retCode != ERROR_SUCCESS)
        return 0;

    retCode = RegQueryValueExA (hKey, "Pathname", NULL, &key_value_type, (BYTE *) valueName, &key_value_size);
...
void subagentTrapCheck (unsigned int clientreg, void *clientarg)
{
    while (1)
    {
        ...
        dwWaitResult = WaitForMultipleObjects (s_trapevent.size, &TRAPEVENT (0), FALSE, 0);
        ...
        for (j = 0; j < 100; j++)
        {
            ...
            bResult = ext_dll_info->pfSnmpExtensionTrap (&Enterprise,
                                                         &GenericTrap, &SpecificTrap, &TimeStamp, &TrapVarbinds);

            if (!bResult)
                break;

            send_trap (&Enterprise, GenericTrap, SpecificTrap, TimeStamp, &TrapVarbinds);
...
send_trap (const AsnObjectIdentifier * const pEnterprise,
           ...
           const AsnInteger SpecificTrap, const AsnTimeticks TimeStamp, const SnmpVarBindList * const pTrapVarbinds)
{
    /*
    ...
    snmp_varlist_add_variable (&notification_vars,
                               ...
                               ASN_TIMETICKS, (const u_char *) &TimeStamp, sizeof (TimeStamp));
    ...
    if (GenericTrap == SNMP_GENERICTRAP_ENTERSPECIFIC)
    {
         ...
         */
        copy_oid_n_w (vb2_oid, &vb2_oid_len, pEnterprise->ids, pEnterprise->idLength);
    ...
    snmp_varlist_add_variable (&notification_vars,
                               ...
                               ASN_OBJECT_ID, (u_char *) vb2_oid, vb2_oid_len * sizeof (vb2_oid[0]));
                             ...
                             const net_snmp_varbinds, const SnmpVarBindList * const win_varbinds)
{
    ...
    {
        status = append_windows_varbind (net_snmp_varbinds, &win_varbinds->list[i]);
...
static int
append_windows_varbind (netsnmp_variable_list ** const net_snmp_varbinds, const SnmpVarBind * const win_varbind)
{
    switch (win_varbind->value.asnType)
    {
        case MS_ASN_INTEGER:
            snmp_varlist_add_variable_w (net_snmp_varbinds, win_varbind->name.ids,
                                         ...
                                         sizeof (win_varbind->value.asnValue.number));
            break;
        case MS_ASN_BITS:
            snmp_varlist_add_variable_w (net_snmp_varbinds, win_varbind->name.ids,
                                         win_varbind->name.idLength,
        ...
        case MS_ASN_NULL:
            snmp_varlist_add_variable_w (net_snmp_varbinds, win_varbind->name.ids,
                                         win_varbind->name.idLength, ASN_NULL, 0, 0);
            break;
        case MS_ASN_OBJECTIDENTIFIER:
            snmp_varlist_add_variable_w (net_snmp_varbinds, win_varbind->name.ids,
                                         win_varbind->name.idLength,
                                         ASN_OBJECT_ID,
                                         win_varbind->value.asnValue.object.ids,
                                         win_varbind->value.asnValue.object.idLength * sizeof (oid));
            break;
        ...
        case MS_ASN_SEQUENCE:
            snmp_varlist_add_variable_w (net_snmp_varbinds, win_varbind->name.ids,
                                         win_varbind->name.idLength,
                                         ASN_SEQUENCE,
                                         win_varbind->value.asnValue.sequence.stream,
                                         win_varbind->value.asnValue.sequence.length);
            break;
        case MS_ASN_IPADDRESS:
            snmp_varlist_add_variable_w (net_snmp_varbinds, win_varbind->name.ids,
                                         win_varbind->name.idLength,
                                         ASN_IPADDRESS,
                                         win_varbind->value.asnValue.address.stream,
                                         win_varbind->value.asnValue.address.length);
            break;
        case MS_ASN_COUNTER32:
            snmp_varlist_add_variable_w (net_snmp_varbinds, win_varbind->name.ids,
                                         ...
                                         sizeof (win_varbind->value.asnValue.counter));
            break;
        case MS_ASN_GAUGE32:
            snmp_varlist_add_variable_w (net_snmp_varbinds, win_varbind->name.ids,
                                         win_varbind->name.idLength,
                                         ...
                                         sizeof (win_varbind->value.asnValue.gauge));
            break;
        case MS_ASN_TIMETICKS:
            snmp_varlist_add_variable_w (net_snmp_varbinds, win_varbind->name.ids,
                                         ...
                                         sizeof (win_varbind->value.asnValue.ticks));
            break;
        case MS_ASN_OPAQUE:    // AsnOctetString
            snmp_varlist_add_variable_w (net_snmp_varbinds, win_varbind->name.ids,
                                         win_varbind->name.idLength,
                                         ASN_OPAQUE,
                                         win_varbind->value.asnValue.arbitrary.stream,
                                         win_varbind->value.asnValue.arbitrary.length);
            break;
        case MS_ASN_COUNTER64:
            snmp_varlist_add_variable_w (net_snmp_varbinds, win_varbind->name.ids,
                                         ...
                                         sizeof (win_varbind->value.asnValue.counter64));
            break;
...
static netsnmp_variable_list *snmp_varlist_add_variable_w (netsnmp_variable_list ** varlist, const UINT * name,
                                                           UINT name_length, u_char type, const void *value, size_t len)
{
    netsnmp_static_assert (sizeof (oid) == sizeof (UINT));
    return snmp_varlist_add_variable (varlist, (const oid *) name, name_length, type, value, len);
}
 ...
 */
int convert_to_windows_varbind_list (SnmpVarBindList * pVarBindList, netsnmp_variable_list * varbind)
{
    ...
    pVarBindList->len = 1;
    pVarBindList->list = (SnmpVarBind *) SnmpUtilMemAlloc (pVarBindList->len * sizeof (pVarBindList->list[0]));
    ...
    if (varbind->name && !copy_oid_to_new_windows_oid (&win_varbind->name, varbind->name, varbind->name_length))
        goto generr;
    ...
    switch (varbind->type)
    {
        ...
        case ASN_BIT_STR:
            ...
            win_varbind->value.asnValue.string.stream = winsnmp_memdup (varbind->val.string, varbind->val_len);
            win_varbind->value.asnValue.string.length = (UINT) (varbind->val_len);
        ...
        case ASN_COUNTER64:
            ...
            win_varbind->value.asnValue.counter64.HighPart = varbind->val.counter64->high;
            win_varbind->value.asnValue.counter64.LowPart = varbind->val.counter64->low;
            break;
        default:
            netsnmp_assert (0);
            goto generr;
...
int convert_win_snmp_err (const int win_snmp_err)
{
    switch (win_snmp_err)
    {
...
static winextdll_view *lookup_view_by_oid (oid * const name, const size_t name_len)
{
    ...
    for (i = 0; i < s_winextdll_view.size; i++)
    {
        if (netsnmp_oid_equals (WINEXTDLL_VIEW (i).name,
                                WINEXTDLL_VIEW (i).name_length, name, name_len) == 0 && WINEXTDLL_VIEW (i).my_handler)
        {
 ...
 */
static void
copy_oid (oid * const to_name, size_t * const to_name_len, const oid * const from_name, const size_t from_name_len)
{
    int j;
 ...
 */
static void
copy_oid_n_w (oid * const to_name, size_t * const to_name_len, const UINT * const from_name, const UINT from_name_len)
{
    netsnmp_static_assert (sizeof (oid) == sizeof (UINT));
 ...
 */
static UINT *copy_oid_to_new_windows_oid (AsnObjectIdentifier * const windows_oid,
                                          const oid * const name, const size_t name_len)
{
    ...
    windows_oid->ids = (UINT *) winsnmp_memdup (name, sizeof (windows_oid->ids[0]) * name_len);
...
static void *xarray_push_back (xarray * a, const void *elem)
{
    ...
    {
        netsnmp_assert (a->size < a->reserved);
        return memcpy ((char *) (a->p) + a->elem_size * a->size++, elem, a->elem_size);
    }
    return NULL;
}
...
static void xarray_erase (xarray * a, void *const elem)
{
    ...
    netsnmp_assert ((const char *) elem + a->elem_size <= (char *) a->p + a->size * a->elem_size);
    netsnmp_assert (((const char *) elem - (char *) a->p) % a->elem_size == 0);

    a->size--;
    memmove ((char *) elem, (char *) elem + a->elem_size,
             a->size - ((const char *) elem - (char *) a->p) / a->elem_size);
}
#endif
