...
netsnmp_feature_require (check_vb_storagetype)
    ...
    netsnmp_feature_require (check_vb_type_and_max_size) netsnmp_feature_require (table_iterator_insert_context)
     ...
     void init_register_nsVacm_context (const char *context)
{
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
    reg = netsnmp_create_handler_registration ("nsVacmAccessTable", nsVacmAccessTable_handler,
    ...
    netsnmp_table_helper_add_indexes (table_info, ASN_OCTET_STR,    /* index: vacmGroupName */
...
netsnmp_variable_list *nsVacmAccessTable_get_first_data_point (void **my_loop_context,
                                                               ...
                                                               netsnmp_iterator_info * mydata)
{
    ...
    *my_loop_context = vacm_scanAccessNext ();
    nsViewIdx = 0;
    return nsVacmAccessTable_get_next_data_point (my_loop_context, my_data_context, put_index_data, mydata);
}

netsnmp_variable_list *nsVacmAccessTable_get_next_data_point (void **my_loop_context,
                                                              void **my_data_context,
                                                              netsnmp_variable_list * put_index_data,
                                                              netsnmp_iterator_info * mydata)
{
  ...
  newView:
    ...
    if (entry)
    {
        ...
        idx = idx->next_variable;
        snmp_set_var_value (idx, (u_char *) & entry->securityModel, sizeof (entry->securityModel));
        idx = idx->next_variable;
        snmp_set_var_value (idx, (u_char *) & entry->securityLevel, sizeof (entry->securityLevel));
        /*
                           ...
                           netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_GET:
            ...
            {
                entry = (struct vacm_accessEntry *) netsnmp_extract_iterator_context (request);
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_NSVACMCONTEXTMATCH:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, entry->contextMatch);
                        break;
                    case COLUMN_NSVACMVIEWNAME:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR,
                                                  (u_char *) entry->views[viewIdx], strlen (entry->views[viewIdx]));
                        break;
                    ...
                    case COLUMN_NSVACMACCESSSTATUS:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, entry->status);
                        break;
                }
            }
            break;
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_NSVACMCONTEXTMATCH:
                        ...
                        break;
                    case COLUMN_NSVACMVIEWNAME:
                        ret = netsnmp_check_vb_type_and_max_size (request->requestvb, ASN_OCTET_STR, VACM_MAX_STRING);
                        break;
                    case COLUMN_VACMACCESSSTORAGETYPE:
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (table_info->colnum)
                {
                    ...
                    case COLUMN_NSVACMVIEWNAME:
                        ...
                        memcpy (entry->views[viewIdx], request->requestvb->val.string, request->requestvb->val_len);
                        break;
                    case COLUMN_VACMACCESSSTORAGETYPE:
