     ...
     void init_nsCache (void)
{
    ...
    netsnmp_register_scalar (netsnmp_create_handler_registration ("nsCacheTimeout", handle_nsCacheTimeout,
                                                                  nsCacheTimeout_oid, OID_LENGTH (nsCacheTimeout_oid),
                                                                  HANDLER_CAN_RWRITE));
    netsnmp_register_scalar (netsnmp_create_handler_registration ("nsCacheEnabled", handle_nsCacheEnabled,
     ...
     */
    netsnmp_register_table_iterator2 (netsnmp_create_handler_registration ("tzCacheTable", handle_nsCacheTable,
                                                                           nsCacheTable_oid,
                                                                           OID_LENGTH (nsCacheTable_oid),
                                                                           HANDLER_CAN_RWRITE), iinfo);
}
...
{
    long cache_default_timeout = netsnmp_ds_get_int (NETSNMP_DS_APPLICATION_ID,
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                snmp_set_var_typed_value (request->requestvb, ASN_INTEGER,
                                          (u_char *) & cache_default_timeout, sizeof (cache_default_timeout));
            }
            break;
        ...
        case MODE_SET_COMMIT:
            netsnmp_ds_set_int (NETSNMP_DS_APPLICATION_ID,
                                NETSNMP_DS_AGENT_CACHE_TIMEOUT, *requests->requestvb->val.integer);
            break;
                       ...
                       netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_GET:
            enabled = (netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_NO_CACHING) ? NSCACHE_STATUS_ENABLED    /* Actually True/False */
                       : NSCACHE_STATUS_DISABLED);
            for (request = requests; request; request = request->next)
            {
                snmp_set_var_typed_value (request->requestvb, ASN_INTEGER, (u_char *) & enabled, sizeof (enabled));
            }
            break;
        ...
        case MODE_SET_COMMIT:
            ...
            if (enabled == NSCACHE_STATUS_DISABLED)
                enabled = 0;
            netsnmp_ds_set_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_NO_CACHING, enabled);
            break;
#endif                            /* !NETSNMP_NO_WRITE_SUPPORT */
    }
...
netsnmp_variable_list *get_first_cache_entry (void **loop_context, void **data_context,
                                              netsnmp_variable_list * index, netsnmp_iterator_info * data)
{
    ...
    snmp_set_var_value (index, (u_char *) cache_head->rootoid, sizeof (oid) * cache_head->rootoid_len);
    *loop_context = (void *) cache_head;
    *data_context = (void *) cache_head;
    return index;
}

netsnmp_variable_list *get_next_cache_entry (void **loop_context, void **data_context,
                                             netsnmp_variable_list * index, netsnmp_iterator_info * data)
{
    ...
    snmp_set_var_value (index, (u_char *) cache->rootoid, sizeof (oid) * cache->rootoid_len);
                        ...
                        status = (cache_entry->enabled ?
                                  (cache_entry->timestampM ?
                                   (!netsnmp_ready_monotonic (cache_entry->timestampM,
                                    ...
                                    NSCACHE_STATUS_EXPIRED) : NSCACHE_STATUS_EMPTY) : NSCACHE_STATUS_DISABLED);
                        snmp_set_var_typed_value (request->requestvb, ASN_INTEGER,
                                                  (u_char *) & status, sizeof (status));
                        break;
                ...
                switch (table_info->colnum)
                {
                    ...
                    case NSCACHE_STATUS:
                        ...
                        if (!((status == NSCACHE_STATUS_ENABLED) ||
                              (status == NSCACHE_STATUS_DISABLED) || (status == NSCACHE_STATUS_EMPTY)))
                        {
                            netsnmp_set_request_error (reqinfo, request, SNMP_ERR_WRONGVALUE);
                            return SNMP_ERR_WRONGVALUE;
                        }
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (table_info->colnum)
                {
                    case NSCACHE_TIMEOUT:
                        ...
                        {
                            struct snmp_alarm *sa = sa_find_specific (cache_entry->timer_id);
