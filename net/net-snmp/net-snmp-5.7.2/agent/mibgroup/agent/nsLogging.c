     ...
     void init_nsLogging (void)
{
    ...
    netsnmp_table_helper_add_indexes (table_info, ASN_INTEGER, ASN_PRIV_IMPLIED_OCTET_STR, 0);
     ...
     */
    netsnmp_register_table_iterator2 (netsnmp_create_handler_registration ("tzLoggingTable", handle_nsLoggingTable,
                                                                           nsLoggingTable_oid,
                                                                           OID_LENGTH (nsLoggingTable_oid),
...
netsnmp_variable_list *get_first_logging_entry (void **loop_context, void **data_context,
                                                netsnmp_variable_list * index, netsnmp_iterator_info * data)
{
    ...
    if (logh_head->token)
        snmp_set_var_value (index->next_variable, (const u_char *) logh_head->token, strlen (logh_head->token));
    else
...
netsnmp_variable_list *get_next_logging_entry (void **loop_context, void **data_context,
                                               netsnmp_variable_list * index, netsnmp_iterator_info * data)
{
    long temp;
    ...
    snmp_set_var_value (index, (u_char *) & temp, sizeof (temp));
    if (logh->token)
        snmp_set_var_value (index->next_variable, (const u_char *) logh->token, strlen (logh->token));
    else
        snmp_set_var_value (index->next_variable, NULL, 0);
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (table_info->colnum)
                {
                    case NSLOGGING_TYPE:
                        ...
                        temp = logh->type;
                        snmp_set_var_typed_value (request->requestvb, ASN_INTEGER, (u_char *) & temp, sizeof (temp));
                        break;
                    ...
                    case NSLOGGING_STATUS:
                        ...
                        temp = (logh->type ? (logh->enabled ? RS_ACTIVE : RS_NOTINSERVICE) : RS_NOTREADY);
                        snmp_set_var_typed_value (request->requestvb, ASN_INTEGER, (u_char *) & temp, sizeof (temp));
                        break;
                        ...
                        switch (*request->requestvb->val.integer)
                        {
                            ...
                            case RS_CREATEANDGO:
                                ...
                                logh = netsnmp_register_loghandler (
                                                                       ...
                                                                       NETSNMP_LOGHANDLER_STDOUT, *idx->val.integer);
                                if (!logh)
                                {
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (table_info->colnum)
                {
                    ...
                    case NSLOGGING_STATUS:
                        ...
                        if (logh && (temp == RS_CREATEANDGO || temp == RS_CREATEANDWAIT))
                        {
