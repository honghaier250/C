...
netsnmp_feature_require (extract_table_row_data) netsnmp_feature_require (table_data_delete_table)
...
extend_registration_block *_register_extend (oid * base, size_t len)
{
    ...
    oid_buf[len] = 2;
#ifndef NETSNMP_NO_WRITE_SUPPORT
    reg = netsnmp_create_handler_registration ("nsExtendConfigTable", handle_nsExtendConfigTable,
    ...
    oid_buf[len] = 3;
    reg = netsnmp_create_handler_registration ("nsExtendOut1Table", handle_nsExtendOutput1Table,
                                               oid_buf, len + 1, HANDLER_CAN_RONLY);
    rc = netsnmp_register_table_data (reg, dinfo, tinfo);
    if (rc != SNMPERR_SUCCESS)
        goto bail;
    ...
    oid_buf[len] = 4;
    reg = netsnmp_create_handler_registration ("nsExtendOut2Table", handle_nsExtendOutput2Table,
                                               oid_buf, len + 1, HANDLER_CAN_RONLY);
     ...
     */
    oid_buf[len] = 1;
    reg = netsnmp_create_handler_registration ("nsExtendNumEntries", NULL, oid_buf, len + 1, HANDLER_CAN_RONLY);
    winfo = netsnmp_create_watcher_info (&(eptr->num_entries), sizeof (eptr->num_entries),
...
int extend_clear_callback (int majorID, int minorID, void *serverarg, void *clientarg)
{
...
void init_extend (void)
{
    ...
    compatability_entries = (netsnmp_old_extend *) calloc (max_compatability_entries, sizeof (netsnmp_old_extend));
    REGISTER_MIB ("ucd-extensible", old_extensible_variables, variable2, old_extensible_variables_oid);
#endif

    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_PRE_UPDATE_CONFIG, extend_clear_callback, NULL);
}
...
int extend_load_cache (netsnmp_cache * cache, void *magic)
{
    ...
    if (ret >= 0)
    {
        ...
        if (extension->numlines > 1)
        {
            ...
            memcpy (extension->lines, line_buf, sizeof (char *) * extension->numlines);
        }
        else
        {
...
void _free_extension (netsnmp_extend * extension, extend_registration_block * ereg)
{
    ...
    if (ereg)
    {
        ...
        if (!eptr)
        {
            snmp_log (LOG_ERR, "extend: fell off end of list before finding extension\n");
            return;
        }
        if (eprev)
...
netsnmp_extend *_new_extension (char *exec_name, int exec_flags, extend_registration_block * ereg)
{
    ...
    extension->cache = netsnmp_cache_create (0, extend_load_cache, extend_free_cache, NULL, 0);
    if (extension->cache)
    ...
    extension->row = row;
    netsnmp_table_row_add_index (row, ASN_OCTET_STR, exec_name, strlen (exec_name));
     ...
     */
    for (eptr1 = ereg->ehead, eptr2 = NULL; eptr1; eptr2 = eptr1, eptr1 = eptr1->next)
    {
                            ...
                            netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    for (request = requests; request; request = request->next)
    {
        ...
        DEBUGMSGOID (("nsExtendTable:config", request->requestvb->name, request->requestvb->name_length));
            ...
            case MODE_GET:
                switch (table_info->colnum)
                {
                    ...
                    case COLUMN_EXTCFG_ARGS:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR,
                                                  extension->args, (extension->args) ? strlen (extension->args) : 0);
                        break;
                    case COLUMN_EXTCFG_INPUT:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR,
                                                  extension->input, (extension->input) ? strlen (extension->input) : 0);
                        break;
                    ...
                    case COLUMN_EXTCFG_EXECTYPE:
                        ...
                        snmp_set_var_typed_value (request->requestvb, ASN_INTEGER, (u_char *) & i, sizeof (i));
                        break;
                    case COLUMN_EXTCFG_RUNTYPE:
                        i = ((extension->flags & NS_EXTEND_FLAGS_WRITEABLE) ?
                             NS_EXTEND_RTYPE_RWRITE : NS_EXTEND_RTYPE_RONLY);
                        snmp_set_var_typed_value (request->requestvb, ASN_INTEGER, (u_char *) & i, sizeof (i));
                        break;

                    case COLUMN_EXTCFG_STORAGE:
                        i = ((extension->flags & NS_EXTEND_FLAGS_CONFIG) ? ST_PERMANENT : ST_VOLATILE);
                        snmp_set_var_typed_value (request->requestvb, ASN_INTEGER, (u_char *) & i, sizeof (i));
                        break;
                    case COLUMN_EXTCFG_STATUS:
                        i = ((extension->flags & NS_EXTEND_FLAGS_ACTIVE) ? RS_ACTIVE : RS_NOTINSERVICE);
                        snmp_set_var_typed_value (request->requestvb, ASN_INTEGER, (u_char *) & i, sizeof (i));
                        break;
            ...
            case MODE_SET_RESERVE1:
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_EXTCFG_COMMAND:
                         ...
                         */
                        if (request->requestvb->val_len == 0 || request->requestvb->val.string[0] != '/')
                        {
                    ...
                    case COLUMN_EXTCFG_RUNTYPE:
                         ...
                         */
                        if ((extension && extension->flags & NS_EXTEND_FLAGS_CONFIG) && i != 3)
                        {
                            netsnmp_set_request_error (reqinfo, request, SNMP_ERR_INCONSISTENTVALUE);
                            return SNMP_ERR_INCONSISTENTVALUE;
                        }
                    ...
                    case COLUMN_EXTCFG_STATUS:
                        ...
                        switch (i)
                        {
                            ...
                            case RS_CREATEANDWAIT:
                                ...
                                extension = _new_extension ((char *) table_info->indexes->val.string, 0, eptr);
                                if (!extension)
                                {    /* failed */
            ...
            case MODE_SET_ACTION:
                switch (table_info->colnum)
                {
                    case COLUMN_EXTCFG_COMMAND:
                        ...
                        extension->command = netsnmp_strdup_and_null (request->requestvb->val.string,
                                                                      request->requestvb->val_len);
                        break;
                    case COLUMN_EXTCFG_ARGS:
                        extension->old_args = extension->args;
                        extension->args = netsnmp_strdup_and_null (request->requestvb->val.string,
                                                                   request->requestvb->val_len);
                        break;
                    case COLUMN_EXTCFG_INPUT:
                        extension->old_input = extension->input;
                        extension->input = netsnmp_strdup_and_null (request->requestvb->val.string,
                                                                    request->requestvb->val_len);
                        break;
                    case COLUMN_EXTCFG_STATUS:
                        i = *request->requestvb->val.integer;
    ...
    if (need_to_validate)
    {
        for (request = requests; request; request = request->next)
        {
            ...
            switch (table_info->colnum)
            {
                case COLUMN_EXTCFG_STATUS:
                    i = *request->requestvb->val.integer;
                    if ((i == RS_ACTIVE || i == RS_CREATEANDGO) && !(extension && extension->command && extension->command[0] == '/'    /* &&
                             ...
                             netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    for (request = requests; request; request = request->next)
    {
        ...
        DEBUGMSGOID (("nsExtendTable:output1", request->requestvb->name, request->requestvb->name_length));
            ...
            case MODE_GET:
                ...
                switch (table_info->colnum)
                {
                    ...
                    case COLUMN_EXTOUT1_OUTPUT1:
                        ...
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR, extension->output, len);
                        break;
                    case COLUMN_EXTOUT1_OUTPUT2:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR,
                                                  extension->output, (extension->output) ? extension->out_len : 0);
                        break;
 ...
 */
netsnmp_extend *_extend_find_entry (netsnmp_request_info * request, netsnmp_table_request_info * table_info, int mode)
{
    netsnmp_extend *eptr;
    ...
    if (!request || !table_info || !table_info->indexes || !table_info->indexes->next_variable)
    {
        DEBUGMSGTL (("nsExtendTable:output2", "invalid invocation\n"));
        return NULL;
    }

    ereg = _find_extension_block (request->requestvb->name, request->requestvb->name_length);
    ...
    if (mode == MODE_GET)
    {
        DEBUGMSGTL (("nsExtendTable:output2", "GET: %s / %ld\n ",
                     table_info->indexes->val.string, *table_info->indexes->next_variable->val.integer));
        for (eptr = ereg->ehead; eptr; eptr = eptr->next)
        {
        ...
        if (eptr)
        {
             ...
             */
            if (!(eptr->flags & NS_EXTEND_FLAGS_ACTIVE) || (netsnmp_cache_check_and_reload (eptr->cache) < 0))
                return NULL;
    ...
    else
    {
        ...
        if (eptr)
        {
             ...
             */
            snmp_set_var_value (table_info->indexes, eptr->token, strlen (eptr->token));
            snmp_set_var_value (table_info->indexes->next_variable, (const u_char *) &line_idx, sizeof (line_idx));
        }
    }
                             ...
                             netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    for (request = requests; request; request = request->next)
    {
        ...
        DEBUGMSGOID (("nsExtendTable:output2", request->requestvb->name, request->requestvb->name_length));
            ...
            case MODE_GETNEXT:
                switch (table_info->colnum)
                {
                    case COLUMN_EXTOUT2_OUTLINE:
                        ...
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR, cp, len);
                        break;
                    default:
                        netsnmp_set_request_error (reqinfo, request, SNMP_NOSUCHOBJECT);
                        continue;
...
u_char *var_extensible_old (struct variable * vp,
