...
static inline void atoip (int pos)
{
    ...
    for (i = 0; i < 32; i += 8)
    {
        unsigned long value = (((rule[pos]) >= '0' && rule[pos] <= '9') ? rule[pos] - '0' : rule[pos] - 'A' + 10);

        pos++;
        value = (value << 4) + (((rule[pos]) >= '0' && rule[pos] <= '9') ? rule[pos] - '0' : rule[pos] - 'A' + 10);
        pos++;
        ret_val |= (value << i);
    }
}
...
static unsigned long int getflags (void)
{
    ...
    {
        int value = (((rule[i]) >= '0' && rule[i] <= '9') ? rule[i] - '0' : rule[i] - 'A' + 10);
