...
{
    static oid sctpAssocRemAddrTable_oid[] = { 1, 3, 6, 1, 2, 1, 104, 1, 5 };
    size_t sctpAssocRemAddrTable_oid_len = OID_LENGTH (sctpAssocRemAddrTable_oid);
    ...
    reg =
        netsnmp_create_handler_registration ("sctpAssocRemAddrTable",
                                             ...
                                             sctpAssocRemAddrTable_oid_len, HANDLER_CAN_RONLY);
    if (NULL == reg)
    {
        snmp_log (LOG_ERR, "error creating handler registration for sctpAssocRemAddrTable\n");
        goto bail;
    }

    container = netsnmp_container_find ("sctpAssocRemAddrTable:table_container");
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "error creating container for sctpAssocRemAddrTable\n");
        goto bail;
    }
    ...
    if (NULL == table_info)
    {
        snmp_log (LOG_ERR, "error allocating table registration for sctpAssocRemAddrTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error injecting container_table handler for sctpAssocRemAddrTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error registering table handler for sctpAssocRemAddrTable\n");
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                if (request->processed)
                    continue;
                table_entry = (sctpAssocRemAddrTable_entry *) netsnmp_container_table_extract_context (request);
                table_info = netsnmp_extract_table_info (request);
                if ((NULL == table_entry) || (NULL == table_info))
                {
                    snmp_log (LOG_ERR, "could not extract table entry or info for sctpAssocRemAddrTable\n");
                ...
                switch (table_info->colnum)
                {
                    ...
                    case COLUMN_SCTPASSOCREMADDRRTO:
                        ...
                        snmp_set_var_typed_integer (request->requestvb, ASN_UNSIGNED, table_entry->sctpAssocRemAddrRTO);
                        break;
                    case COLUMN_SCTPASSOCREMADDRMAXPATHRTX:
                        ...
                        snmp_set_var_typed_integer (request->requestvb,
                                                    ASN_UNSIGNED, table_entry->sctpAssocRemAddrMaxPathRtx);
                        break;
                    ...
                    case COLUMN_SCTPASSOCREMADDRSTARTTIME:
                        ...
                        snmp_set_var_typed_integer (request->requestvb,
                                                    ASN_TIMETICKS, table_entry->sctpAssocRemAddrStartTime);
                        break;
                    default:
...
{
    sctpAssocRemAddrTable_entry *entry = SNMP_MALLOC_TYPEDEF (sctpAssocRemAddrTable_entry);
...
int sctpAssocRemAddrTable_entry_update_index (sctpAssocRemAddrTable_entry * entry)
{
    int err = 0;
    ...
    snmp_set_var_value (&var_sctpAssocRemAddr,
                        ...
                        entry->sctpAssocRemAddr_len * sizeof (entry->sctpAssocRemAddr[0]));
...
void sctpAssocRemAddrTable_entry_copy (sctpAssocRemAddrTable_entry * from, sctpAssocRemAddrTable_entry * to)
{
    memcpy (to, from, sizeof (sctpAssocRemAddrTable_entry));
    to->oid_index.oids = to->oid_tmp;
}

void sctpAssocRemAddrTable_entry_free (sctpAssocRemAddrTable_entry * entry)
{
    SNMP_FREE (entry);
}

netsnmp_container *sctpAssocRemAddrTable_get_container (void)
{
    return sctpAssocRemAddrTable_container;
}


static void sctpAssocRemAddrTable_entry_clear (void *what, void *magic)
{
    sctpAssocRemAddrTable_entry_free (what);
}

void sctpAssocRemAddrTable_container_clear (netsnmp_container * container)
{
