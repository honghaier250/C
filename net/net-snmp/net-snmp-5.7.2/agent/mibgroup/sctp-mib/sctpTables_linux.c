 ...
 */
static int convert_address (char *token, char *addr_buffer, u_long * addr_type, u_long * addr_len)
{
    int family;
 ...
 * with "<->". 
 */
static int parse_assoc_local_addresses (sctpAssocTable_entry * entry, sctpTables_containers * containers)
{
    char *token;
    ...
    while ((token = strtok (NULL, " ")))
    {
        ...
        ret =
            convert_address (ip, localAddr->sctpAssocLocalAddr,
                             &localAddr->sctpAssocLocalAddrType, &localAddr->sctpAssocLocalAddr_len);
        ...
        ret = sctpAssocLocalAddrTable_add_or_update (containers->sctpAssocLocalAddrTable, localAddr);
        if (ret != SNMP_ERR_NOERROR)
            return SNMP_ERR_GENERR;
    }
    return SNMP_ERR_NOERROR;
 ...
 */
static int parse_assoc_remote_addresses (sctpAssocTable_entry * entry)
{
    char *token;
        ...
        if (token[0] == '*')
        {
            ...
            ret =
                convert_address (token + 1, entry->sctpAssocRemPrimAddr,
                                 &entry->sctpAssocRemPrimAddrType, &entry->sctpAssocRemPrimAddr_len);
        }
    }
    return ret;
}

static int parse_assoc_line (char *line, sctpTables_containers * containers)
{
    ...
    if (ret != SNMP_ERR_NOERROR)
    {
        DEBUGMSGTL (("sctp:tables:load:assoc", "error adding/updating the entry in container\n"));
        return ret;
    }
 ...
 */
static int load_assoc (sctpTables_containers * containers)
{
    FILE *f;
    ...
    if (f == NULL)
    {
        DEBUGMSGTL (("sctp:tables:load:assoc", "arch load failed: can't open" ASSOC_FILE "\n"));
        return SNMP_ERR_GENERR;
    }
    ...
    while (fgets (line, sizeof (line), f) != NULL)
    {
        ...
        if (ret != SNMP_ERR_NOERROR)
        {
            DEBUGMSGTL (("sctp:tables:load:assoc", "error parsing the line\n"));
        }
    }
    fclose (f);
...
static int parse_remaddr_line (char *line, sctpTables_containers * containers)
{
    char *token;
    ...
    ret = convert_address (token, entry->sctpAssocRemAddr, &entry->sctpAssocRemAddrType, &entry->sctpAssocRemAddr_len);
    if (ret < 0)
    {
        ret = SNMP_ERR_GENERR;
        goto error;
    ...
    ret = sctpAssocRemAddrTable_add_or_update (containers->sctpAssocRemAddrTable, entry);
    if (ret != SNMP_ERR_NOERROR)
    {
        DEBUGMSGTL (("sctp:load:remaddr", "error adding/updating the entry in container\n"));
        return ret;
    }
 ...
 * or updated entries as valid (so the missing, i.e. invalid, can be deleted).
 */
static int load_remaddr (sctpTables_containers * containers)
{
    FILE *f;
    ...
    if (f == NULL)
    {
        DEBUGMSGTL (("sctp:load:remaddr", "arch load failed: can't open" REMADDR_FILE "\n"));
        return SNMP_ERR_GENERR;
    }
...
int sctpTables_arch_load (sctpTables_containers * containers, u_long * flags)
{
    int ret = SNMP_ERR_NOERROR;
