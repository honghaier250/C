...
{
    static oid sctpLookupRemPortTable_oid[] = { 1, 3, 6, 1, 2, 1, 104, 1, 7 };
    size_t sctpLookupRemPortTable_oid_len = OID_LENGTH (sctpLookupRemPortTable_oid);
    ...
    reg =
        netsnmp_create_handler_registration ("sctpLookupRemPortTable",
                                             ...
                                             sctpLookupRemPortTable_oid_len, HANDLER_CAN_RONLY);
    if (NULL == reg)
    {
        snmp_log (LOG_ERR, "error creating handler registration for sctpLookupRemPortTable\n");
        goto bail;
    }

    container = netsnmp_container_find ("sctpLookupRemPortTable:table_container");
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "error creating container for sctpLookupRemPortTable\n");
        goto bail;
    }
    ...
    if (NULL == table_info)
    {
        snmp_log (LOG_ERR, "error allocating table registration for sctpLookupRemPortTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error injecting container_table handler for sctpLookupRemPortTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error registering table handler for sctpLookupRemPortTable\n");
...
void shutdown_table_sctpLookupRemPortTable (void)
{
    ...
    sctpLookupRemPortTable_container_clear (sctpLookupRemPortTable_container);
}
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                if (request->processed)
                    continue;
                table_entry = (sctpLookupRemPortTable_entry *) netsnmp_container_table_extract_context (request);
                table_info = netsnmp_extract_table_info (request);
                if ((NULL == table_entry) || (NULL == table_info))
                {
                    snmp_log (LOG_ERR, "could not extract table entry or info for sctpLookupRemPortTable\n");
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_SCTPLOOKUPREMPORTSTARTTIME:
                        ...
                        snmp_set_var_typed_integer (request->requestvb,
                                                    ASN_TIMETICKS, table_entry->sctpLookupRemPortStartTime);
                        break;
                    default:
...
{
    sctpLookupRemPortTable_entry *entry = SNMP_MALLOC_TYPEDEF (sctpLookupRemPortTable_entry);
...
int sctpLookupRemPortTable_entry_update_index (sctpLookupRemPortTable_entry * entry)
{
    ...
    snmp_set_var_value (&var_sctpAssocRemPort, (u_char *) & entry->sctpAssocRemPort, sizeof (entry->sctpAssocRemPort));
    ...
    err =
        build_oid_noalloc (entry->oid_index.oids, entry->oid_index.len,
                           &entry->oid_index.len, NULL, 0, &var_sctpAssocRemPort);
    if (err)
...
void sctpLookupRemPortTable_entry_copy (sctpLookupRemPortTable_entry * from, sctpLookupRemPortTable_entry * to)
{
...
void sctpLookupRemPortTable_entry_free (sctpLookupRemPortTable_entry * entry)
{
    SNMP_FREE (entry);
}

netsnmp_container *sctpLookupRemPortTable_get_container (void)
{
    return sctpLookupRemPortTable_container;
}


static void sctpLookupRemPortTable_entry_clear (void *what, void *magic)
{
    sctpLookupRemPortTable_entry_free (what);
}

void sctpLookupRemPortTable_container_clear (netsnmp_container * container)
{
