...
{
    static oid sctpLookupRemIPAddrTable_oid[] = { 1, 3, 6, 1, 2, 1, 104, 1, 10 };
    size_t sctpLookupRemIPAddrTable_oid_len = OID_LENGTH (sctpLookupRemIPAddrTable_oid);
    ...
    reg =
        netsnmp_create_handler_registration ("sctpLookupRemIPAddrTable",
                                             ...
                                             sctpLookupRemIPAddrTable_oid_len, HANDLER_CAN_RONLY);
    if (NULL == reg)
    {
        snmp_log (LOG_ERR, "error creating handler registration for sctpLookupRemIPAddrTable\n");
        goto bail;
    }

    container = netsnmp_container_find ("sctpLookupRemIPAddrTable:table_container");
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "error creating container for sctpLookupRemIPAddrTable\n");
        goto bail;
    }
    ...
    if (NULL == table_info)
    {
        snmp_log (LOG_ERR, "error allocating table registration for sctpLookupRemIPAddrTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error injecting container_table handler for sctpLookupRemIPAddrTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error registering table handler for sctpLookupRemIPAddrTable\n");
...
void shutdown_table_sctpLookupRemIPAddrTable (void)
{
    ...
    sctpLookupRemIPAddrTable_container_clear (sctpLookupRemIPAddrTable_container);
}
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                if (request->processed)
                    continue;
                table_entry = (sctpLookupRemIPAddrTable_entry *) netsnmp_container_table_extract_context (request);
                table_info = netsnmp_extract_table_info (request);
                if ((NULL == table_entry) || (NULL == table_info))
                {
                    snmp_log (LOG_ERR, "could not extract table entry or info for sctpLookupRemIPAddrTable\n");
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_SCTPLOOKUPREMIPADDRSTARTTIME:
                        ...
                        snmp_set_var_typed_integer (request->requestvb,
                                                    ASN_TIMETICKS, table_entry->sctpLookupRemIPAddrStartTime);
                        break;
                    default:
...
sctpLookupRemIPAddrTable_entry *sctpLookupRemIPAddrTable_entry_create (void)
{
    sctpLookupRemIPAddrTable_entry *entry = SNMP_MALLOC_TYPEDEF (sctpLookupRemIPAddrTable_entry);
...
int sctpLookupRemIPAddrTable_entry_update_index (sctpLookupRemIPAddrTable_entry * entry)
{
     ...
     */
    memset (&var_sctpAssocRemIPAddrType, 0, sizeof (var_sctpAssocRemIPAddrType));
    ...
    snmp_set_var_value (&var_sctpAssocRemIPAddrType,
                        (u_char *) & entry->sctpAssocRemAddrType, sizeof (entry->sctpAssocRemAddrType));
    ...
    snmp_set_var_value (&var_sctpAssocRemIPAddr, (u_char *) & entry->sctpAssocRemAddr, entry->sctpAssocRemAddr_len);
    ...
    err =
        build_oid_noalloc (entry->oid_index.oids, entry->oid_index.len,
                           &entry->oid_index.len, NULL, 0, &var_sctpAssocRemIPAddrType);
    if (err)
...
void sctpLookupRemIPAddrTable_entry_copy (sctpLookupRemIPAddrTable_entry * from, sctpLookupRemIPAddrTable_entry * to)
{
...
void sctpLookupRemIPAddrTable_entry_free (sctpLookupRemIPAddrTable_entry * entry)
{
    SNMP_FREE (entry);
}

netsnmp_container *sctpLookupRemIPAddrTable_get_container (void)
{
    return sctpLookupRemIPAddrTable_container;
}


static void sctpLookupRemIPAddrTable_entry_clear (void *what, void *magic)
{
    sctpLookupRemIPAddrTable_entry_free (what);
}

void sctpLookupRemIPAddrTable_container_clear (netsnmp_container * container)
{
