...
{
    static oid sctpLookupRemPrimIPAddrTable_oid[] = { 1, 3, 6, 1, 2, 1, 104, 1, 9 };
    size_t sctpLookupRemPrimIPAddrTable_oid_len = OID_LENGTH (sctpLookupRemPrimIPAddrTable_oid);
    ...
    reg =
        netsnmp_create_handler_registration ("sctpLookupRemPrimIPAddrTable",
                                             ...
                                             sctpLookupRemPrimIPAddrTable_oid_len, HANDLER_CAN_RONLY);
    if (NULL == reg)
    {
        snmp_log (LOG_ERR, "error creating handler registration for sctpLookupRemPrimIPAddrTable\n");
        goto bail;
    }

    container = netsnmp_container_find ("sctpLookupRemPrimIPAddrTable:table_container");
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "error creating container for sctpLookupRemPrimIPAddrTable\n");
        goto bail;
    }
    ...
    if (NULL == table_info)
    {
        snmp_log (LOG_ERR, "error allocating table registration for sctpLookupRemPrimIPAddrTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error injecting container_table handler for sctpLookupRemPrimIPAddrTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error registering table handler for sctpLookupRemPrimIPAddrTable\n");
...
void shutdown_table_sctpLookupRemPrimIPAddrTable (void)
{
    ...
    sctpLookupRemPrimIPAddrTable_container_clear (sctpLookupRemPrimIPAddrTable_container);
}
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                if (request->processed)
                    continue;
                table_entry = (sctpLookupRemPrimIPAddrTable_entry *) netsnmp_container_table_extract_context (request);
                table_info = netsnmp_extract_table_info (request);
                if ((NULL == table_entry) || (NULL == table_info))
                {
                    snmp_log (LOG_ERR, "could not extract table entry or info for sctpLookupRemPrimIPAddrTable\n");
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_SCTPLOOKUPREMPRIMIPADDRSTARTTIME:
                        ...
                        snmp_set_var_typed_integer (request->requestvb,
                                                    ASN_TIMETICKS, table_entry->sctpLookupRemPrimIPAddrStartTime);
                        break;
                    default:
...
sctpLookupRemPrimIPAddrTable_entry *sctpLookupRemPrimIPAddrTable_entry_create (void)
{
    sctpLookupRemPrimIPAddrTable_entry *entry = SNMP_MALLOC_TYPEDEF (sctpLookupRemPrimIPAddrTable_entry);

    if (entry != NULL)
    {
        entry->oid_index.len = SCTP_LOOKUP_REM_PRIM_IP_ADDR_TABLE_INDEX_SIZE;
...
int sctpLookupRemPrimIPAddrTable_entry_update_index (sctpLookupRemPrimIPAddrTable_entry * entry)
{
     ...
     */
    memset (&var_sctpAssocRemPrimAddrType, 0, sizeof (var_sctpAssocRemPrimAddrType));
    ...
    snmp_set_var_value (&var_sctpAssocRemPrimAddrType,
                        (u_char *) & entry->sctpAssocRemPrimAddrType, sizeof (entry->sctpAssocRemPrimAddrType));
    ...
    snmp_set_var_value (&var_sctpAssocRemPrimAddr,
                        (u_char *) & entry->sctpAssocRemPrimAddr, entry->sctpAssocRemPrimAddr_len);
    ...
    err =
        build_oid_noalloc (entry->oid_index.oids, entry->oid_index.len,
                           &entry->oid_index.len, NULL, 0, &var_sctpAssocRemPrimAddrType);
    if (err)
...
sctpLookupRemPrimIPAddrTable_entry_copy (sctpLookupRemPrimIPAddrTable_entry
                                         * from, sctpLookupRemPrimIPAddrTable_entry * to)
{
...
void sctpLookupRemPrimIPAddrTable_entry_free (sctpLookupRemPrimIPAddrTable_entry * entry)
{
    SNMP_FREE (entry);
}

netsnmp_container *sctpLookupRemPrimIPAddrTable_get_container (void)
{
    return sctpLookupRemPrimIPAddrTable_container;
}


static void sctpLookupRemPrimIPAddrTable_entry_clear (void *what, void *magic)
{
    sctpLookupRemPrimIPAddrTable_entry_free (what);
}

void sctpLookupRemPrimIPAddrTable_container_clear (netsnmp_container * container)
{
    CONTAINER_CLEAR (container, sctpLookupRemPrimIPAddrTable_entry_clear, NULL);
}
