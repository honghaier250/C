...
{
    static oid sctpLookupRemHostNameTable_oid[] = { 1, 3, 6, 1, 2, 1, 104, 1, 8 };
    size_t sctpLookupRemHostNameTable_oid_len = OID_LENGTH (sctpLookupRemHostNameTable_oid);
    ...
    reg =
        netsnmp_create_handler_registration ("sctpLookupRemHostNameTable",
                                             ...
                                             sctpLookupRemHostNameTable_oid_len, HANDLER_CAN_RONLY);
    if (NULL == reg)
    {
        snmp_log (LOG_ERR, "error creating handler registration for sctpLookupRemHostNameTable\n");
        goto bail;
    }

    container = netsnmp_container_find ("sctpLookupRemHostNameTable:table_container");
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "error creating container for sctpLookupRemHostNameTable\n");
        goto bail;
    }
    ...
    if (NULL == table_info)
    {
        snmp_log (LOG_ERR, "error allocating table registration for sctpLookupRemHostNameTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error injecting container_table handler for sctpLookupRemHostNameTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error registering table handler for sctpLookupRemHostNameTable\n");
...
void shutdown_table_sctpLookupRemHostNameTable (void)
{
    ...
    sctpLookupRemHostNameTable_container_clear (sctpLookupRemHostNameTable_container);
}
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                if (request->processed)
                    continue;
                table_entry = (sctpLookupRemHostNameTable_entry *) netsnmp_container_table_extract_context (request);
                table_info = netsnmp_extract_table_info (request);
                if ((NULL == table_entry) || (NULL == table_info))
                {
                    snmp_log (LOG_ERR, "could not extract table entry or info for sctpLookupRemHostNameTable\n");
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_SCTPLOOKUPREMHOSTNAMESTARTTIME:
                        ...
                        snmp_set_var_typed_integer (request->requestvb,
                                                    ASN_TIMETICKS, table_entry->sctpLookupRemHostNameStartTime);
                        break;
                    default:
...
sctpLookupRemHostNameTable_entry *sctpLookupRemHostNameTable_entry_create (void)
{
    sctpLookupRemHostNameTable_entry *entry = SNMP_MALLOC_TYPEDEF (sctpLookupRemHostNameTable_entry);
...
int sctpLookupRemHostNameTable_entry_update_index (sctpLookupRemHostNameTable_entry * entry)
{
    ...
    snmp_set_var_value (&var_sctpAssocRemHostName,
                        (u_char *) & entry->sctpAssocRemHostName, entry->sctpAssocRemHostName_len);
    ...
    err =
        build_oid_noalloc (entry->oid_index.oids, entry->oid_index.len,
                           &entry->oid_index.len, NULL, 0, &var_sctpAssocRemHostName);
    if (err)
...
void
sctpLookupRemHostNameTable_entry_copy (sctpLookupRemHostNameTable_entry * from, sctpLookupRemHostNameTable_entry * to)
{
...
void sctpLookupRemHostNameTable_entry_free (sctpLookupRemHostNameTable_entry * entry)
{
    SNMP_FREE (entry);
}

netsnmp_container *sctpLookupRemHostNameTable_get_container (void)
{
    return sctpLookupRemHostNameTable_container;
}


static void sctpLookupRemHostNameTable_entry_clear (void *what, void *magic)
{
    sctpLookupRemHostNameTable_entry_free (what);
}

void sctpLookupRemHostNameTable_container_clear (netsnmp_container * container)
{
    CONTAINER_CLEAR (container, sctpLookupRemHostNameTable_entry_clear, NULL);
}
