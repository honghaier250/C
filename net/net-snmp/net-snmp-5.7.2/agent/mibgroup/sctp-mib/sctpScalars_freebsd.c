...
int netsnmp_access_sctp_params_arch_load (netsnmp_sctp_params * sctp_params)
{
    ...
    if (sysctlbyname ("net.inet.sctp.rto_max", &sctp_params->rto_max, &len, NULL, 0) < 0)
    {
        return (-1);
    }
    if (sysctlbyname ("net.inet.sctp.rto_min", &sctp_params->rto_min, &len, NULL, 0) < 0)
    {
        return (-1);
    }
    if (sysctlbyname ("net.inet.sctp.rto_initial", &sctp_params->rto_initial, &len, NULL, 0) < 0)
    {
        return (-1);
    }
    if (sysctlbyname ("net.inet.sctp.valid_cookie_life", &sctp_params->val_cookie_life, &len, NULL, 0) < 0)
    {
        return (-1);
    }
    if (sysctlbyname ("net.inet.sctp.init_rtx_max", &sctp_params->max_init_retr, &len, NULL, 0) < 0)
    {
        return (-1);
    }
    return 0;
