 ...
 */
static int load_uint_file (const char *filename, u_int * value)
{
    FILE *f;
...
void netsnmp_access_sctp_stats_arch_init ()
{
}

int netsnmp_access_sctp_stats_arch_load (netsnmp_sctp_stats * sctp_stats)
{
    FILE *f;
    ...
    DEBUGMSGTL (("sctp:scalars:stats:arch_load", "netsnmp_access_sctp_stats_arch_load called"));
    ...
    while (fgets (line, sizeof (line), f) != NULL)
    {
        ...
        if (delimiter == NULL)
        {
            DEBUGMSGTL (("sctp:scalars:stats:arch_load", "Malformed line, cannot find '\\t'!\n"));
            fclose (f);
            return -1;
        }
        errno = 0;
        ...
        if (errno != 0)
        {
            DEBUGMSGTL (("sctp:scalars:stats:arch_load", "Malformed value!'\n"));
            fclose (f);
            return -1;
        }
        ...
        if (ret < 0)
        {
            DEBUGMSGTL (("sctp:scalars:stats:arch_load", "Unknown entry!'\n"));
            fclose (f);
            return ret;
        }
    }
...
void netsnmp_access_sctp_params_arch_init ()
{
}

int netsnmp_access_sctp_params_arch_load (netsnmp_sctp_params * sctp_params)
{
    int ret;

    DEBUGMSGTL (("sctp:scalars:params:arch_load", "netsnmp_access_sctp_params_arch_load called"));
    ...
    ret = load_uint_file (PROC_VAL_COOKIE_LIFE, &sctp_params->val_cookie_life);
    if (ret < 0)
        return ret;
