...
static int parse_assoc_local_addresses (sctpTables_containers * containers, struct xsctp_laddr *xladdr)
{
    int ret;
    ...
    if (xladdr->address.sa.sa_family == AF_INET)
    {
        ...
        memcpy (entry->sctpAssocLocalAddr, &xladdr->address.sin.sin_addr, entry->sctpAssocLocalAddr_len);
    }
    else if (xladdr->address.sa.sa_family == AF_INET6)
    {
        ...
        memcpy (entry->sctpAssocLocalAddr, &xladdr->address.sin6.sin6_addr, entry->sctpAssocLocalAddr_len);
    }
...
static int parse_assoc_xstcb (sctpTables_containers * containers, struct xsctp_tcb *xstcb)
{
    int ret;
...
static int parse_remaddr_xraddr (sctpTables_containers * containers, struct xsctp_raddr *xraddr)
{
    int ret;
    ...
    if (xraddr->address.sa.sa_family == AF_INET)
    {
        ...
        memcpy (entry->sctpAssocRemAddr, &xraddr->address.sin.sin_addr, entry->sctpAssocRemAddr_len);
    }
    else if (xraddr->address.sa.sa_family == AF_INET6)
    {
        entry->sctpAssocRemAddrType = INETADDRESSTYPE_IPV6;
        entry->sctpAssocRemAddr_len = 16;
        memcpy (entry->sctpAssocRemAddr, &xraddr->address.sin6.sin6_addr, entry->sctpAssocRemAddr_len);
    }
