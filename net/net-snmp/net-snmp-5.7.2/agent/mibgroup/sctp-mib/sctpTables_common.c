...
netsnmp_feature_require (container_lifo)
     static void sctpAssocTable_collect_invalid (void *what, void *magic)
{
 ...
 */
void sctpAssocTable_delete_invalid (netsnmp_container * assocTable)
{
    netsnmp_container *to_delete = netsnmp_container_find ("lifo");

    CONTAINER_FOR_EACH (assocTable, sctpAssocTable_collect_invalid, to_delete);
...
static void sctpAssocRemAddrTable_collect_invalid (void *what, void *magic)
{
 ...
 * load, if the entry is not updated). 
 */
void sctpAssocRemAddrTable_delete_invalid (netsnmp_container * remAddrTable)
{
    netsnmp_container *to_delete = netsnmp_container_find ("lifo");

    CONTAINER_FOR_EACH (remAddrTable, sctpAssocRemAddrTable_collect_invalid, to_delete);
...
static void sctpAssocLocalAddrTable_collect_invalid (void *what, void *magic)
{
 ...
 * load, if the entry is not updated). 
 */
void sctpAssocLocalAddrTable_delete_invalid (netsnmp_container * localAddrTable)
{
    netsnmp_container *to_delete = netsnmp_container_find ("lifo");

    CONTAINER_FOR_EACH (localAddrTable, sctpAssocLocalAddrTable_collect_invalid, to_delete);
...
int sctpAssocRemAddrTable_add_or_update (netsnmp_container * remAddrTable, sctpAssocRemAddrTable_entry * entry)
{
    /*
...
int sctpAssocLocalAddrTable_add_or_update (netsnmp_container * localAddrTable, sctpAssocLocalAddrTable_entry * entry)
{
    /*
     * we have full sctpAssocLocalAddrTable entry, update or add it in the container 
     */
    ...
    else
    {
        ...
        {
            entry->sctpAssocLocalAddrStartTime = netsnmp_get_agent_uptime ();
        }
...
int sctpAssocTable_add_or_update (netsnmp_container * assocTable, sctpAssocTable_entry * entry)
{
    /*
    ...
    if (old != NULL)
    {
        ...
        if (timestamp == 0 && entry->sctpAssocStartTime == 0 && entry->sctpAssocState >= SCTPASSOCSTATE_ESTABLISHED)
    ...
    else
    {
         ...
         * the entry is new, add it there 
         */
        if (entry->sctpAssocStartTime == 0 && entry->sctpAssocState >= SCTPASSOCSTATE_ESTABLISHED)
        {
...
static void sctpTables_add_local_port (sctpAssocTable_entry * assoc, sctpTables_containers * containers)
{
    ...
    if (entry == NULL)
    {
        DEBUGMSGTL (("sctp:tables:fill_lookup", "cannot create sctpLookupLocalPortTable_entry"));
        return;
    }
...
static void sctpTables_add_rem_port (sctpAssocTable_entry * assoc, sctpTables_containers * containers)
{
    ...
    if (entry == NULL)
    {
        DEBUGMSGTL (("sctp:tables:fill_lookup", "cannot create sctpLookupRemPortTable_entry"));
        return;
    }
...
static void sctpTables_add_rem_hostname (sctpAssocTable_entry * assoc, sctpTables_containers * containers)
{
    ...
    if (entry == NULL)
    {
        DEBUGMSGTL (("sctp:tables:fill_lookup", "cannot create sctpLookupRemHostNameTable_entry"));
        return;
    }

    entry->sctpAssocId = assoc->sctpAssocId;
    entry->sctpAssocRemHostName_len = assoc->sctpAssocRemHostName_len;
    memcpy (entry->sctpAssocRemHostName, assoc->sctpAssocRemHostName, assoc->sctpAssocRemHostName_len);
...
static void sctpTables_add_rem_prim_ip_addr (sctpAssocTable_entry * assoc, sctpTables_containers * containers)
{
    ...
    if (entry == NULL)
    {
        DEBUGMSGTL (("sctp:tables:fill_lookup", "cannot create sctpLookupRemPrimIPAddrTable_entry"));
        return;
    }
    ...
    memcpy (entry->sctpAssocRemPrimAddr, assoc->sctpAssocRemPrimAddr, assoc->sctpAssocRemPrimAddr_len);
...
static void sctpTables_fill_lookup_assoc (void *what, void *magic)
{
    sctpAssocTable_entry *entry = what;
...
static void sctpTables_add_rem_ip_addr (sctpAssocRemAddrTable_entry * rem_addr, sctpTables_containers * containers)
{
    ...
    if (entry == NULL)
    {
        DEBUGMSGTL (("sctp:tables:fill_lookup", "cannot create sctpLookupRemIPAddrTable_entry"));
        return;
    }
    ...
    memcpy (entry->sctpAssocRemAddr, rem_addr->sctpAssocRemAddr, rem_addr->sctpAssocRemAddr_len);
    entry->sctpLookupRemIPAddrStartTime = rem_addr->sctpAssocRemAddrStartTime;

    sctpLookupRemIPAddrTable_entry_update_index (entry);
    CONTAINER_INSERT (containers->sctpLookupRemIPAddrTable, entry);
}

static void sctpTables_fill_lookup_rem_addr (void *what, void *magic)
{
    sctpAssocRemAddrTable_entry *entry = what;
...
int sctpTables_fill_lookup (sctpTables_containers * containers)
{
    /*
     * clear all the lookup tables 
     */
    sctpLookupLocalPortTable_container_clear (containers->sctpLookupLocalPortTable);
    sctpLookupRemPortTable_container_clear (containers->sctpLookupRemPortTable);
    sctpLookupRemHostNameTable_container_clear (containers->sctpLookupRemHostNameTable);
    sctpLookupRemPrimIPAddrTable_container_clear (containers->sctpLookupRemPrimIPAddrTable);
    sctpLookupRemIPAddrTable_container_clear (containers->sctpLookupRemIPAddrTable);

    /*
     * fill the lookup tables 
     */
    CONTAINER_FOR_EACH (containers->sctpAssocTable, sctpTables_fill_lookup_assoc, containers);
    CONTAINER_FOR_EACH (containers->sctpAssocRemAddrTable, sctpTables_fill_lookup_rem_addr, containers);
...
int sctpTables_load (void)
{
    ...
    containers.sctpAssocRemAddrTable = sctpAssocRemAddrTable_get_container ();
    containers.sctpAssocLocalAddrTable = sctpAssocLocalAddrTable_get_container ();
    containers.sctpLookupLocalPortTable = sctpLookupLocalPortTable_get_container ();
    containers.sctpLookupRemPortTable = sctpLookupRemPortTable_get_container ();
    containers.sctpLookupRemHostNameTable = sctpLookupRemHostNameTable_get_container ();
    containers.sctpLookupRemPrimIPAddrTable = sctpLookupRemPrimIPAddrTable_get_container ();
    containers.sctpLookupRemIPAddrTable = sctpLookupRemIPAddrTable_get_container ();
    ...
    if (flags & SCTP_TABLES_LOAD_FLAG_DELETE_INVALID)
    {
        ...
        sctpAssocRemAddrTable_delete_invalid (containers.sctpAssocRemAddrTable);
        sctpAssocLocalAddrTable_delete_invalid (containers.sctpAssocLocalAddrTable);
    }
