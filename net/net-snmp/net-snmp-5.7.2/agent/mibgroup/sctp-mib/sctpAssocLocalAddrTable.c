...
{
    static oid sctpAssocLocalAddrTable_oid[] = { 1, 3, 6, 1, 2, 1, 104, 1, 4 };
    size_t sctpAssocLocalAddrTable_oid_len = OID_LENGTH (sctpAssocLocalAddrTable_oid);
    ...
    reg =
        netsnmp_create_handler_registration ("sctpAssocLocalAddrTable",
                                             ...
                                             sctpAssocLocalAddrTable_oid_len, HANDLER_CAN_RONLY);
    if (NULL == reg)
    {
        snmp_log (LOG_ERR, "error creating handler registration for sctpAssocLocalAddrTable\n");
        goto bail;
    }

    container = netsnmp_container_find ("sctpAssocLocalAddrTable:table_container");
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "error creating container for sctpAssocLocalAddrTable\n");
        goto bail;
    }
    ...
    if (NULL == table_info)
    {
        snmp_log (LOG_ERR, "error allocating table registration for sctpAssocLocalAddrTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error injecting container_table handler for sctpAssocLocalAddrTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error registering table handler for sctpAssocLocalAddrTable\n");
...
void shutdown_table_sctpAssocLocalAddrTable (void)
{
    ...
    sctpAssocLocalAddrTable_container_clear (sctpAssocLocalAddrTable_container);
}
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                if (request->processed)
                    continue;
                table_entry = (sctpAssocLocalAddrTable_entry *) netsnmp_container_table_extract_context (request);
                table_info = netsnmp_extract_table_info (request);
                if ((NULL == table_entry) || (NULL == table_info))
                {
                    snmp_log (LOG_ERR, "could not extract table entry or info for sctpAssocLocalAddrTable\n");
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_SCTPASSOCLOCALADDRSTARTTIME:
                        ...
                        snmp_set_var_typed_integer (request->requestvb,
                                                    ASN_TIMETICKS, table_entry->sctpAssocLocalAddrStartTime);
                        break;
                    default:
...
{
    sctpAssocLocalAddrTable_entry *entry = SNMP_MALLOC_TYPEDEF (sctpAssocLocalAddrTable_entry);
...
int sctpAssocLocalAddrTable_entry_update_index (sctpAssocLocalAddrTable_entry * entry)
{
    int err = 0;
    ...
    memset (&var_sctpAssocLocalAddrType, 0, sizeof (var_sctpAssocLocalAddrType));
    ...
    snmp_set_var_value (&var_sctpAssocLocalAddrType,
                        (u_char *) & entry->sctpAssocLocalAddrType, sizeof (entry->sctpAssocLocalAddrType));
    snmp_set_var_value (&var_sctpAssocLocalAddr,
                        (u_char *) & entry->sctpAssocLocalAddr,
                        entry->sctpAssocLocalAddr_len * sizeof (entry->sctpAssocLocalAddr[0]));
...
void sctpAssocLocalAddrTable_entry_copy (sctpAssocLocalAddrTable_entry * from, sctpAssocLocalAddrTable_entry * to)
{
    memcpy (to, from, sizeof (sctpAssocLocalAddrTable_entry));
    to->oid_index.oids = to->oid_tmp;
}

void sctpAssocLocalAddrTable_entry_free (sctpAssocLocalAddrTable_entry * entry)
{
    SNMP_FREE (entry);
}

netsnmp_container *sctpAssocLocalAddrTable_get_container (void)
{
    return sctpAssocLocalAddrTable_container;
}

static void sctpAssocLocalAddrTable_entry_clear (void *what, void *magic)
{
    sctpAssocLocalAddrTable_entry_free (what);
}

void sctpAssocLocalAddrTable_container_clear (netsnmp_container * container)
{
