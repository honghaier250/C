...
{
    static oid sctpLookupLocalPortTable_oid[] = { 1, 3, 6, 1, 2, 1, 104, 1, 6 };
    size_t sctpLookupLocalPortTable_oid_len = OID_LENGTH (sctpLookupLocalPortTable_oid);
    ...
    reg =
        netsnmp_create_handler_registration ("sctpLookupLocalPortTable",
                                             ...
                                             sctpLookupLocalPortTable_oid_len, HANDLER_CAN_RONLY);
    if (NULL == reg)
    {
        snmp_log (LOG_ERR, "error creating handler registration for sctpLookupLocalPortTable\n");
        goto bail;
    }

    container = netsnmp_container_find ("sctpLookupLocalPortTable:table_container");
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "error creating container for sctpLookupLocalPortTable\n");
        goto bail;
    }
    ...
    if (NULL == table_info)
    {
        snmp_log (LOG_ERR, "error allocating table registration for sctpLookupLocalPortTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error injecting container_table handler for sctpLookupLocalPortTable\n");
        goto bail;
    }
    ...
    {
        snmp_log (LOG_ERR, "error registering table handler for sctpLookupLocalPortTable\n");
...
void shutdown_table_sctpLookupLocalPortTable (void)
{
    ...
    sctpLookupLocalPortTable_container_clear (sctpLookupLocalPortTable_container);
}
        ...
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                if (request->processed)
                    continue;
                table_entry = (sctpLookupLocalPortTable_entry *) netsnmp_container_table_extract_context (request);
                table_info = netsnmp_extract_table_info (request);
                if ((NULL == table_entry) || (NULL == table_info))
                {
                    snmp_log (LOG_ERR, "could not extract table entry or info for sctpLookupLocalPortTable\n");
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_SCTPLOOKUPLOCALPORTSTARTTIME:
                        ...
                        snmp_set_var_typed_integer (request->requestvb,
                                                    ASN_TIMETICKS, table_entry->sctpLookupLocalPortStartTime);
                        break;
                    default:
...
{
    sctpLookupLocalPortTable_entry *entry = SNMP_MALLOC_TYPEDEF (sctpLookupLocalPortTable_entry);
...
int sctpLookupLocalPortTable_entry_update_index (sctpLookupLocalPortTable_entry * entry)
{
    ...
    snmp_set_var_value (&var_sctpAssocLocalPort,
                        (u_char *) & entry->sctpAssocLocalPort, sizeof (entry->sctpAssocLocalPort));
    ...
    err =
        build_oid_noalloc (entry->oid_index.oids, entry->oid_index.len,
                           &entry->oid_index.len, NULL, 0, &var_sctpAssocLocalPort);
    if (err)
...
netsnmp_container *sctpLookupLocalPortTable_get_container (void)
{
    return sctpLookupLocalPortTable_container;
}


static void sctpLookupLocalPortTable_entry_clear (void *what, void *magic)
{
