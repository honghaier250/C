...
void init_snmpMPDStats (void)
{
    ...
    REGISTER_SYSOR_ENTRY (reg, "The MIB for Message Processing and Dispatching.");
    REGISTER_MIB ("snmpv3/snmpMPDStats", snmpMPDStats_variables, variable2, snmpMPDStats_variables_oid);
}

u_char *var_snmpMPDStats (struct variable *vp,
                          oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    *var_len = sizeof (long_ret);    /* assume an integer and change later if not */
     ...
     */
    tmagic = vp->magic;
    if ((tmagic >= 0) && (tmagic <= (STAT_MPD_STATS_END - STAT_MPD_STATS_START)))
    {
