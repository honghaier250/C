...
usm_parse_oid (oid * oidIndex, size_t oidLen,
               unsigned char **engineID, size_t * engineIDLen, unsigned char **name, size_t * nameLen)
{
    int nameL;
 ...
 */
u_char *var_usmUser (struct variable * vp,
                     oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    *var_len = sizeof (long_ret);
    ...
    if (vp->magic != USMUSERSPINLOCK)
    {
        ...
        {
            for (nptr = usm_get_userList (), uptr = NULL; nptr != NULL; nptr = nptr->next)
            {
                           ...
                           u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    else if (action == RESERVE2)
    {
        ...
        if (uptr->userStatus == RS_ACTIVE || uptr->userStatus == RS_NOTREADY || uptr->userStatus == RS_NOTINSERVICE)
        {
            ...
            else if (snmp_oid_compare
                     ((oid *) var_val, var_val_len / sizeof (oid), uptr->authProtocol, uptr->authProtocolLen) == 0)
            {
                /*
        ...
        else
        {
            ...
            if (snmp_oid_compare
                ...
                || snmp_oid_compare ((oid *) var_val,
                                     ...
                                     usmHMACMD5AuthProtocol, sizeof (usmHMACMD5AuthProtocol) / sizeof (oid)) == 0
#endif
                || snmp_oid_compare ((oid *) var_val,
                                     var_val_len / sizeof (oid),
                                     usmHMACSHA1AuthProtocol, sizeof (usmHMACSHA1AuthProtocol) / sizeof (oid)) == 0)
            {
                if (uptr->cloneFrom != NULL)
                {
                    optr = uptr->authProtocol;
                            ...
                            u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    else if (action == ACTION)
    {
        ...
        if (decode_keychange (uptr->authProtocol, uptr->authProtocolLen,
                              uptr->authKey, uptr->authKeyLen, var_val, var_val_len, buf, &buflen) != SNMPERR_SUCCESS)
        {
        ...
        if (uptr->userStatus == RS_ACTIVE || uptr->userStatus == RS_NOTREADY || uptr->userStatus == RS_NOTINSERVICE)
        {
                 ...
                 usmNoPrivProtocol, sizeof (usmNoPrivProtocol) / sizeof (oid)) == 0)
            {
                ...
                uptr->privProtocolLen = var_val_len / sizeof (oid);
                     ...
                     ((oid *) var_val, var_val_len / sizeof (oid), uptr->privProtocol, uptr->privProtocolLen) == 0)
            {
                /*
                 * But it's also okay to set it to the same thing as it
                 * currently is.  
        ...
        else
        {
            ...
            else
            {
                if (snmp_oid_compare
                    ...
                    && snmp_oid_compare
                    ((oid *) var_val, var_val_len / sizeof (oid),
                     usmDESPrivProtocol, sizeof (usmDESPrivProtocol) / sizeof (oid)) != 0
#endif
                    && snmp_oid_compare
                    ((oid *) var_val, var_val_len / sizeof (oid),
                     usmAESPrivProtocol, sizeof (usmAESPrivProtocol) / sizeof (oid)) != 0)
                {
                    return SNMP_ERR_WRONGVALUE;
                }
            }
                            ...
                            u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    else if (action == ACTION)
    {
        ...
        if (decode_keychange (uptr->authProtocol, uptr->authProtocolLen,
                              uptr->privKey, uptr->privKeyLen, var_val, var_val_len, buf, &buflen) != SNMPERR_SUCCESS)
        {
            DEBUGMSGTL (("usmUser", "%s: ... failed\n", fname));
            return SNMP_ERR_GENERR;
        }
                          ...
                          u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    else if (action == RESERVE2)
    {
        ...
        if ((long_ret == ST_VOLATILE || long_ret == ST_NONVOLATILE) &&
            (uptr->userStorageType == ST_VOLATILE || uptr->userStorageType == ST_NONVOLATILE))
        {
        ...
        else
        {
            ...
            if (long_ret == ST_READONLY &&
                ...
                (uptr->userStatus == RS_ACTIVE || uptr->userStatus == RS_NOTINSERVICE))
            {
                return SNMP_ERR_WRONGVALUE;
            }
