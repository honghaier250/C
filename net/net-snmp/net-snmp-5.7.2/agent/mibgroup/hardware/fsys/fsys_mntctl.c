...
int _fsys_remote (char *device, int type, char *host)
{
...
void netsnmp_fsys_arch_load (void)
{
    ...
    for (i = 0; i < ret; i++, aixcurr = (struct vmount *) ((char *) aixcurr + aixcurr->vmt_length))
    {
        ...
        strlcpy (entry->device, vmt2dataptr (aixcurr, VMT_OBJECT), sizeof (entry->device));
