...
#include "mounts.h"
     ...
     */
    else if (!strcmp (typename, MOUNT_FFS) || !strcmp (typename, MOUNT_UFS))
    ...
    else if (!strcmp (typename, MOUNT_MSDOS) || !strcmp (typename, MOUNT_MSDOSFS))
...
void netsnmp_fsys_arch_load (void)
{
    ...
    for (i = 0; i < n; i++)
    {
        entry = netsnmp_fsys_by_path (stats[i].f_mntonname, NETSNMP_FS_FIND_CREATE);
        if (!entry)
            continue;
