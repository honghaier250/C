...
int netsnmp_cpu_arch_load (netsnmp_cache * cache, void *magic)
{
    ...
    for (i = 0; i < cpu_num; i++)
    {
        ...
        cpu2->sys2_ticks = (unsigned long long) vminfo[i].v_time[V_CPU_KERNEL] + vminfo[i].v_time[V_CPU_STREAM];
        ...
        cpu->sys2_ticks += (unsigned long long) vminfo[i].v_time[V_CPU_KERNEL] + vminfo[i].v_time[V_CPU_STREAM];
