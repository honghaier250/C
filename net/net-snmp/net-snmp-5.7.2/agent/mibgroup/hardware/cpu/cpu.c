     ...
     void init_cpu (void)
{
    ...
    if (_cpuAutoUpdate)
    {
        ...
        _cpu_cache = netsnmp_cache_create (5, netsnmp_cpu_arch_load, NULL, nsCPU, OID_LENGTH (nsCPU));
}
...
static void _cpu_update_stats (unsigned int reg, void *magic)
{
    ...
    for (cpu = _cpu_head; cpu; cpu = cpu->next)
    {
        cpu->total_ticks = cpu->user_ticks +
            ...
            cpu->wait_ticks +
            cpu->kern_ticks +
            cpu->intrpt_ticks + cpu->sirq_ticks + cpu->steal_ticks + cpu->guest_ticks + cpu->guestnice_ticks;
    }
}
