...
int netsnmp_cpu_arch_load (netsnmp_cache * cache, void *magic)
{
    ...
    cpu->sys2_ticks =
        (unsigned long long) sinfo_gen->cpu[CPU_KERNEL] + (unsigned long long) sinfo_gen->cpu[CPU_SXBRK] +
        (unsigned long long) sinfo_gen->cpu[CPU_INTR];
    ...
    for (i = 0; i < cpu_count; ++i)
    {
        ...
        cpu->sys2_ticks =
            (unsigned long long) sinfo_cpus[i].cpu[CPU_KERNEL] + (unsigned long long) sinfo_cpus[i].cpu[CPU_SXBRK] +
            (unsigned long long) sinfo_cpus[i].cpu[CPU_INTR];
