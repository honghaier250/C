 ...
 */
static int read_num_sensor (picl_nodehdl_t childh, const char *propval, float *value)
{
...
static int process_num_sensor (picl_nodehdl_t childh, const char *propname, const char *propval, int typ)
{
...
static int read_enum_sensor (picl_nodehdl_t childh, float *value, const char **options)
{
    picl_nodehdl_t sensorh;
...
static int process_enum_sensor (picl_nodehdl_t childh, const char *propname, int typ, const char **options)
{
    netsnmp_sensor_info *sp;
...
static int process_sensors (int level, picl_nodehdl_t nodeh)
{
    ...
    error_code = picl_get_propval_by_name (nodeh, PICL_PROP_CHILD, &childh, sizeof (childh));
    if (error_code != PICL_SUCCESS)
    {
    ...
    {
        error_code = picl_get_propval_by_name (childh, PICL_PROP_NAME, propname, sizeof (propname) - 1);
        if (error_code != PICL_SUCCESS)
        {
        ...
        error_code = picl_get_propval_by_name (childh, PICL_PROP_CLASSNAME, propclass, sizeof (propclass) - 1);
        if (error_code != PICL_SUCCESS)
        {
         ...
         */
        if ((strstr (propclass, "picl")) || (strstr (propclass, "frutree")) || (strstr (propclass, "obp")))
        {
        ...
        {
            process_num_sensor (childh, propname, "AtoDSensorValue", NETSNMP_SENSOR_TYPE_RPM);
        }
        else if (strstr (propclass, "fan"))
        {
            process_num_sensor (childh, propname, "Speed", NETSNMP_SENSOR_TYPE_RPM);
        }
        else if (strstr (propclass, "temperature-sensor"))
        {
            process_num_sensor (childh, propname, "Temperature", NETSNMP_SENSOR_TYPE_TEMPERATURE);
        }
        ...
        {
            process_enum_sensor (childh, propname, NETSNMP_SENSOR_TYPE_OTHER, switch_settings);
        }
        else if (strstr (propclass, "led"))
        {
            process_enum_sensor (childh, propname, NETSNMP_SENSOR_TYPE_OTHER, led_settings);
        }
         ...
         */
        error_code = picl_get_propval_by_name (childh, PICL_PROP_PEER, &nexth, sizeof (nexth));
        if (error_code != PICL_SUCCESS)
        {
            /* That's All Folks! */
            return ((error_code == PICL_PROPNOTFOUND) ? PICL_SUCCESS : error_code);
        }
        childh = nexth;
    }
...
int netsnmp_sensor_arch_load (netsnmp_cache * cache, void *vp)
{
    int error_code;
