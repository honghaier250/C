    ...
    else
    {
        ...
        for (i = 0; i < kp->ks_ndata; i++)
        {
            ...
            if (sp)
            {
                ...
                snprintf (sp->descr, 255, "fan type %s number %d", fantypes[fan_info->type], fan_info->instance);
            }
    ...
    else
    {
        ...
        for (i = 0; i < kp->ks_ndata; i++)
        {
            ...
            switch (enc_info->type)
            {
                ...
                default:
                    DEBUGMSGTL (("sensors:arch:detail", "Unrecognised Enclosure entry (%d)n", enc_info->type));
            }
