...
void init_hw_sensors (void)
{
    ...
    if (_sensorAutoUpdate)
    {
        ...
        snmp_alarm_register (_sensorAutoUpdate, SA_REPEAT, _sensor_update_stats, NULL);
    }
    else
    {
        _sensor_cache = netsnmp_cache_create (5, netsnmp_sensor_load, netsnmp_sensor_free, NULL, 0);
 ...
 */
void _sensor_update_stats (unsigned int clientreg, void *data)
{
    _sensor_free ();
    _sensor_load ();
}
 ...
 */
int netsnmp_sensor_load (netsnmp_cache * cache, void *data)
{
    return _sensor_load ();
}
 ...
 */
void netsnmp_sensor_free (netsnmp_cache * cache, void *data)
{
    _sensor_free ();
}
...
static void _sensor_free (void)
{
    ...
    for (sp = CONTAINER_FIRST (_sensor_container); sp; sp = CONTAINER_NEXT (_sensor_container, sp))
    {
 ...
 */
netsnmp_sensor_info *sensor_by_name (const char *name, int create_type)
{
    netsnmp_sensor_info *sp;
