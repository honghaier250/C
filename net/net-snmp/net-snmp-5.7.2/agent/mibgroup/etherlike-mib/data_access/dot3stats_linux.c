...
struct ifname *dot3stats_interface_name_list_get (struct ifname *list_head, int *retval)
{
    struct ifaddrs *addrs = NULL, *p = NULL;

    struct ifname *nameptr1 = NULL, *nameptr2 = NULL;

    DEBUGMSGTL (("access:dot3StatsTable:interface_name_list_get", "called\n"));

    if ((getifaddrs (&addrs)) < 0)
    {
        DEBUGMSGTL (("access:dot3StatsTable:interface_name_list_get", "getifaddrs failed\n"));
...
int dot3stats_interface_name_list_free (struct ifname *list_head)
{
    ...
    DEBUGMSGTL (("access:dot3StatsTable:interface_name_list_free", "called\n"));

    if (!list_head)
    {
        snmp_log (LOG_ERR, "access:dot3StatsTable:interface_name_list_free: invalid pointer list_head");
        DEBUGMSGTL (("access:dot3StatsTable:interface_name_list_free", "invalid pointer list_head\n"));
        return -1;
    }
...
int dot3stats_interface_ioctl_ifindex_get (int fd, const char *name)
{
    ...
    if (rc < 0)
    {
        DEBUGMSGTL (("access:dot3StatsTable:interface_ioctl_ifindex_get", "error on interface '%s'\n", name));
...
static int get_nlmsg (const struct sockaddr_nl *who, struct nlmsghdr *m, void *arg)
{
...
static int rtnl_dump_filter (struct rtnl_handle *rth, rtnl_filter_t filter, void *arg1, rtnl_filter_t junk, void *arg2)
{
...
int _dot3Stats_netlink_get_errorcntrs (dot3StatsTable_rowreq_ctx * rowreq_ctx, const char *name)
{
 ...
 */
static int getulongfromsysclassnetstatistics (const char *ifname, const char *ctrname, u_long * valuep)
{
    char path[256];
 ...
 */
void interface_dot3stats_get_errorcounters (dot3StatsTable_rowreq_ctx * rowreq_ctx, const char *name)
{
    u_long value;
...
int interface_ioctl_dot3stats_get (dot3StatsTable_rowreq_ctx * rowreq_ctx, int fd, const char *name)
{
    ...
    DEBUGMSGTL (("access:dot3StatsTable:interface_ioctl_dot3Stats_get", "called\n"));
    ...
    if (!eth_strings)
    {
        DEBUGMSGTL (("access:dot3StatsTable:interface_ioctl_dot3Stats_get", "no memory available\n"));
...
int interface_ioctl_dot3stats_duplex_get (dot3StatsTable_rowreq_ctx * rowreq_ctx, int fd, const char *name)
{
    ...
    DEBUGMSGTL (("access:dot3StatsTable:interface_ioctl_dot3Stats_duplex_get", "called\n"));
    ...
    if (err < 0)
    {
        DEBUGMSGTL (("access:dot3StatsTable:interface_ioctl_dot3Stats_duplex_get", "ETHTOOL_GSET failed\n"));
    ...
    DEBUGMSGTL (("access:dot3StatsTable:interface_ioctl_dot3Stats_duplex_get", "ETHTOOL_GSET processed\n"));
    return err;
#else
    return -2;
#endif
 ...
 * RETURNS: nothing. fields not set if data not available
 */
void interface_sysclassnet_dot3stats_get (dot3StatsTable_rowreq_ctx * rowreq_ctx, const char *name)
{
    u_long value;
...
int _dot3Stats_ioctl_get (int fd, int which, struct ifreq *ifrq, const char *name)
{
    ...
    if (NULL == name)
    {
        DEBUGMSGTL (("access:dot3StatsTable:ioctl", "_dot3Stats_ioctl_get invalid ifname '%s'\n", name));
    ...
    if (fd < 0)
    {
        ...
        if (ourfd < 0)
        {
            DEBUGMSGTL (("access:dot3StatsTable:ioctl", "dot3Stats_ioctl_get couldn't create a socket\n"));
            snmp_log (LOG_ERR, "access:dot3StatsTable:ioctl, _dot3Stats_ioctl_get error on interface '%s'\n", name);
    ...
    if (rc < 0)
    {
        DEBUGMSGTL (("access:dot3StatsTable:ioctl", "dot3Stats_ioctl_get ioctl %d returned %d\n", which, rc));
        rc = -3;
    }
