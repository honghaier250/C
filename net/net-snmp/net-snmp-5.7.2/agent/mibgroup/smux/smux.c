...
netsnmp_feature_require (snprint_objid)
     ...
     static int smux_build (u_char, long, oid *, size_t *, u_char, u_char *, size_t, u_char *, size_t *);
     ...
     u_char *var_smux (struct variable *, oid *, size_t *, int, size_t *, WriteMethod ** write_method);
...
void init_smux (void)
{
    snmpd_register_config_handler ("smuxpeer", smux_parse_peer_auth, smux_free_peer_auth, "OID-IDENTITY PASSWORD");
    snmpd_register_config_handler ("smuxsocket", smux_parse_smux_socket, NULL, "SMUX bind address");
}
...
void real_init_smux (void)
{
    ...
    smux_socket = netsnmp_ds_get_string (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_SMUX_SOCKET);
     ...
     */
    if (setsockopt (smux_listen_sd, SOL_SOCKET, SO_REUSEADDR, (char *) &one, sizeof (one)) < 0)
    {
        snmp_log_perror ("[init_smux] setsockopt(SO_REUSEADDR) failed");
    }
#endif                            /* SO_REUSEADDR */

    if (bind (smux_listen_sd, (struct sockaddr *) &lo_socket, sizeof (lo_socket)) < 0)
    {
...
#ifdef    SO_KEEPALIVE
    if (setsockopt (smux_listen_sd, SOL_SOCKET, SO_KEEPALIVE, (char *) &one, sizeof (one)) < 0)
    {
...
u_char *var_smux (struct variable * vp,
                  oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    *write_method = var_smux_write;
    ...
    {
        if (0 >= snmp_oidtree_compare (vp->name, vp->namelen, rptr->sr_name, rptr->sr_name_len))
            break;
    }
    if (rptr == NULL)
        return NULL;
    ...
    valptr = smux_snmp_process (exact, name, length, var_len, &val_type, rptr->sr_fd);

    if (valptr == NULL)
        return NULL;

    if ((snmp_oidtree_compare (name, *length, rptr->sr_name, rptr->sr_name_len)) != 0)
    {
        /*
...
var_smux_write (int action,
                u_char * var_val, u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    smux_reg *rptr;
    ...
    for (rptr = ActiveRegs; rptr; rptr = rptr->sr_next)
    {
        if (!snmp_oidtree_compare (name, name_len, rptr->sr_name, rptr->sr_name_len))
            break;
    }
    ...
    switch (action)
    {
        case RESERVE1:
             ...
             */
            var_len += (*(var_val + 1) & ASN_LONG_LEN) ? var_len + ((*(var_val + 1) & 0x7F) + 2) : 2;
            ...
            if ((smux_build ((u_char) SMUX_SET, smux_reqid,
                             name, &name_length, var_val_type, dataptr, datalen, buf, &len)) < 0)
            {
            ...
            while (1)
            {
                 ...
                 */
                if ((len = recvfrom (rptr->sr_fd, (void *) buf, SMUXMAXPKTSIZE, MSG_PEEK, NULL, NULL)) <= 0)
                {
                ...
                DEBUGMSGTL (("smux", "[var_smux_write] Peeked at %" NETSNMP_PRIz "d bytes\n", len));
                ...
                DEBUGMSGTL (("smux", "[var_smux_write] Received %" NETSNMP_PRIz "d bytes\n", len));
...
int smux_process (int fd)
{
    ...
    do
    {
        length = recvfrom (fd, (char *) data, SMUXMAXPKTSIZE, MSG_PEEK, NULL, NULL);
    }
    while ((length == -1) && ((errno == EINTR) || (errno == EAGAIN)));
...
static int smux_pdu_process (int fd, u_char * data, size_t length)
{
    ...
    DEBUGMSGTL (("smux", "[smux_pdu_process] Processing %" NETSNMP_PRIz "d bytes\n", length));
...
static u_char *smux_open_process (int fd, u_char * ptr, size_t * len, int *fail)
{
    ...
    DEBUGMSGTL (("smux",
                 "[smux_open_process] version %ld, len %" NETSNMP_PRIz "u, type %d\n", version, *len, (int) type));

    oid_name_len = MAX_OID_LEN;
    if ((ptr = asn_parse_objid (ptr, len, &type, oid_name, &oid_name_len)) == NULL)
    {
    ...
    if ((ptr = asn_parse_string (ptr, len, &type, (u_char *) descr, &string_len)) == NULL)
    {
    ...
    string_len = SMUXMAXSTRLEN;
    if ((ptr = asn_parse_string (ptr, len, &type, (u_char *) passwd, &string_len)) == NULL)
    {
    ...
    snmp_log (LOG_INFO, "accepted smux peer: oid %s, descr %s\n", oid_print, descr);
    *fail = FALSE;
    return ptr;
}
...
static int smux_auth_peer (oid * name, size_t namelen, char *passwd, int fd)
{
    ...
    for (i = 0; i < nauths; i++)
    {
        if (snmp_get_do_debugging ())
        {
            snprint_objid (oid_print, sizeof (oid_print), Auths[i]->sa_oid, Auths[i]->sa_oid_len);
            DEBUGMSGTL (("smux:auth", "[smux_auth_peer] Checking OID: %s (%d)\n", oid_print, i));
        }
        if (snmp_oid_compare (Auths[i]->sa_oid, Auths[i]->sa_oid_len, name, namelen) == 0)
        {
            if (snmp_get_do_debugging ())
            {
            ...
            if (!(strcmp (Auths[i]->sa_passwd, passwd)) && (Auths[i]->sa_active_fd == -1))
            {
                /*
...
static u_char *smux_close_process (int fd, u_char * ptr, size_t * len)
{
    ...
    DEBUGMSGTL (("smux", "[smux_close_process] close from peer on fd %d reason %ld\n", fd, down));
    smux_peer_cleanup (fd);
...
static u_char *smux_rreq_process (int sd, u_char * ptr, size_t * len)
{
    ...
    if ((ptr = asn_parse_int (ptr, len, &type, &priority, sizeof (priority))) == NULL)
    {
    ...
    if ((ptr = asn_parse_int (ptr, len, &type, &operation, sizeof (operation))) == NULL)
    {
    ...
    else if ((operation == SMUX_REGOP_REGISTER_RO) || (operation == SMUX_REGOP_REGISTER_RW))
    {
        if (priority < -1)
        {
            DEBUGMSGTL (("smux", "[smux_rreq_process] peer fd %d invalid priority %ld", sd, priority));
            smux_send_rrsp (sd, -1);
            return NULL;
        }
        ...
        for (rptr = ActiveRegs; rptr; rptr = rptr->sr_next)
        {
            result = snmp_oid_compare (oid_name, oid_name_len, rptr->sr_name, rptr->sr_name_len);
            if (result == 0)
            {
        ...
        if (register_mib ("smux", (struct variable *)
                          ...
                          1, nrptr->sr_name, nrptr->sr_name_len) != SNMPERR_SUCCESS)
        {
 ...
 */
static smux_reg *smux_find_match (smux_reg * regs, int sd, oid * oid_name, size_t oid_name_len, long priority)
{
...
static void smux_replace_active (smux_reg * actptr, smux_reg * pasptr)
{
    ...
    register_mib ("smux", (struct variable *) smux_variables,
                  sizeof (struct variable2), 1, pasptr->sr_name, pasptr->sr_name_len);
    free (actptr);
}
...
static void smux_list_detach (smux_reg ** head, smux_reg * m_remove)
{
    ...
    for (rptr = *head, rptr2 = rptr->sr_next; rptr2; rptr2 = rptr2->sr_next, rptr = rptr->sr_next)
    {
        if (rptr2 == m_remove)
        {
...
static smux_reg *smux_find_replacement (oid * name, size_t name_len)
{
    ...
    {
        if (!snmp_oidtree_compare (rptr->sr_name, rptr->sr_name_len, name, name_len))
        {
            ...
            else if ((difflen == bestlen) && (rptr->sr_priority < bestptr->sr_priority))
                bestptr = rptr;
        }
    }
    return bestptr;
...
u_char *smux_snmp_process (int exact, oid * objid, size_t * len, size_t * return_len, u_char * return_type, int sd)
{
    ...
    if (smux_build (type, smux_reqid, objid, len, 0, NULL, *len, packet, (size_t *) & length) < 0)
    {
    ...
    while (1)
    {
         ...
         * peek at what's received 
         */
        length = recvfrom (sd, (char *) result, SMUXMAXPKTSIZE, MSG_PEEK, NULL, NULL);
        if (length <= 0)
        {
...
static u_char *smux_parse (u_char * rsp, oid * objid, size_t * oidlen, size_t * return_len, u_char * return_type)
{
    ...
    if ((ptr = asn_parse_int (ptr, &length, &type, &reqid, sizeof (reqid))) == NULL)
    {
        DEBUGMSGTL (("smux", "[smux_parse] parse of reqid failed\n"));
        return NULL;
    }
    if ((ptr = asn_parse_int (ptr, &length, &type, &errstat, sizeof (errstat))) == NULL)
    {
        DEBUGMSGTL (("smux", "[smux_parse] parse of error status failed\n"));
        return NULL;
    }
    if ((ptr = asn_parse_int (ptr, &length, &type, &errindex, sizeof (errindex))) == NULL)
    {
...
static u_char *smux_parse_var (u_char * varbind,
                               size_t * varbindlength,
                               oid * objid, size_t * oidlen, size_t * varlength, u_char * vartype)
{
    ...
    ptr = snmp_parse_var_op (ptr, var_name, &var_name_len, vartype, &var_val_len, &var_val, &len);
    ...
    *oidlen = var_name_len;
    ...
    len = SMUXMAXPKTSIZE;
    DEBUGMSGTL (("smux",
                 "[smux_parse_var] Asn coded len of var %" NETSNMP_PRIz "u, type %d\n", var_val_len, (int) *vartype));

    switch ((short) *vartype)
    {
        case ASN_INTEGER:
            *varlength = sizeof (long);
            asn_parse_int (var_val, &len, vartype, (long *) &smux_long, *varlength);
        ...
        case ASN_UINTEGER:
            *varlength = sizeof (u_long);
            asn_parse_unsigned_int (var_val, &len, vartype, (u_long *) & smux_ulong, *varlength);
            return (u_char *) & smux_ulong;
            break;
        case ASN_COUNTER64:
            *varlength = sizeof (smux_counter64);
            asn_parse_unsigned_int64 (var_val, &len, vartype, (struct counter64 *) &smux_counter64, *varlength);
...
smux_build (u_char type,
            long reqid,
            oid * objid,
            size_t * oidlen, u_char val_type, u_char * val, size_t val_len, u_char * packet, size_t * length)
{
    ...
    len = ptr - save2;
    asn_build_sequence (save2, &len, (ASN_SEQUENCE | ASN_CONSTRUCTOR), (ptr - save2 - 4));
...
static void smux_peer_cleanup (int sd)
{
    ...
    for (rptr = ActiveRegs; rptr; rptr = rptr2)
    {
        ...
        if (rptr->sr_fd == sd)
        {
            smux_list_detach (&ActiveRegs, rptr);
            unregister_mib (rptr->sr_name, rptr->sr_name_len);
            if ((nrptr = smux_find_replacement (rptr->sr_name, rptr->sr_name_len)) != NULL)
            {
                smux_list_detach (&PassiveRegs, nrptr);
                smux_list_add (&ActiveRegs, nrptr);
                register_mib ("smux", (struct variable *)
                              smux_variables, sizeof (struct variable2), 1, nrptr->sr_name, nrptr->sr_name_len);
            }
            free (rptr);
        }
    }
...
static u_char *smux_trap_process (u_char * rsp, size_t * len)
{
    ...
    if ((ptr = asn_parse_objid (ptr, len, &vartype, (oid *) & sa_enterpriseoid, &sa_enterpriseoid_len)) == NULL)
    {
    ...
    if (((ptr = asn_parse_string (ptr, len,
                                  &vartype, smux_str, &datalen)) == NULL) || (vartype != (u_char) ASN_IPADDRESS))
    {
    ...
    if (((ptr = asn_parse_unsigned_int (ptr, len,
                                        ...
                                        datalen)) == NULL) || (vartype != (u_char) ASN_TIMETICKS))
    {
    ...
    while (ptr && *len)
    {
         ...
         */
        var_name_len = MAX_OID_LEN;
        ptr = snmp_parse_var_op (ptr, var_name, &var_name_len, &vartype, &var_val_len, (u_char **) & var_val, len);
        ...
        switch ((short) vartype)
        {
            case ASN_INTEGER:
                ...
                asn_parse_int (var_val, &maxlen, &vartype, (long *) &smux_long, var_val_len);
            ...
            case ASN_UINTEGER:
                ...
                asn_parse_unsigned_int (var_val, &maxlen, &vartype, (u_long *) & smux_ulong, var_val_len);
                var_val = (u_char *) & smux_ulong;
                break;
            case ASN_COUNTER64:
                var_val_len = sizeof (smux_counter64);
                asn_parse_unsigned_int64 (var_val, &maxlen, &vartype,
                                          (struct counter64 *) &smux_counter64, var_val_len);
            ...
            case ASN_IPADDRESS:
                ...
                if ((var_val = asn_parse_header (var_val, &maxlen, &vartype)) == NULL)
                    return NULL;
                memcpy ((u_char *) & (smux_sa.sin_addr.s_addr), var_val, var_val_len);
            ...
            case ASN_OCTET_STR:
                ...
                asn_parse_string (var_val, &maxlen, &vartype, smux_str, &var_val_len);
                var_val = smux_str;
                break;
            case ASN_OBJECT_ID:
                var_val_len = MAX_OID_LEN;
                asn_parse_objid (var_val, &maxlen, &vartype, smux_objid, &var_val_len);
            ...
            case ASN_BIT_STR:
                ...
                if (len == NULL)
                    return NULL;
                var_val_len = SMUXMAXSTRLEN;
                asn_parse_bitstring (var_val, &maxlen, &vartype, smux_str, &var_val_len);
