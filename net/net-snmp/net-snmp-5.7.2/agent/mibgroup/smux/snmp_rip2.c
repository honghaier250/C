...
u_char *var_rip2 (struct variable *vp, oid * name, int *length, int exact, int *var_len, WriteMethod ** write_method)
{
    u_char *var;

    int result;

    DEBUGMSGTL (("smux/snmp_rip2", "[var_rip2] var len %d, oid requested Len %d-", *var_len, *length));
    ...
    *write_method = NULL;
     ...
     */
    result = snmp_oid_compare (name, *length, max_rip_mib, sizeof (max_rip_mib) / sizeof (u_int));
     ...
     */
    result = snmp_oid_compare (name, *length, min_rip_mib, sizeof (min_rip_mib) / sizeof (u_int));
    if (exact && (result < 0))
    {
        DEBUGMSGTL (("smux/snmp_rip2", "Exact but doesn't match length %d, size %d\n", *length, sizeof (min_rip_mib)));
        return NULL;
    }
    ...
    DEBUGMSGTL (("smux/snmp_rip2", "[var_rip2] var len %d, oid obtained Len %d-", *var_len, *length));
    DEBUGMSGOID (("smux/snmp_rip2", name, *length));
    DEBUGMSG (("smux/snmp_rip2", "\n"));
