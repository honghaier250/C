...
u_char *var_ospf (struct variable *vp, oid * name, int *length, int exact, int *var_len, WriteMethod ** write_method)
{
    u_char *var;

    int result;

    DEBUGMSGTL (("smux/snmp_ospf", "[var_ospf] var len %d, oid requested Len %d-", *var_len, *length));
    ...
    *write_method = NULL;
     ...
     */
    result = snmp_oid_compare (name, *length, max_ospf_mib, sizeof (max_ospf_mib) / sizeof (u_int));
     ...
     */
    result = snmp_oid_compare (name, *length, min_ospf_mib, sizeof (min_ospf_mib) / sizeof (u_int));
    if (exact && (result < 0))
    {
        DEBUGMSGTL (("smux/snmp_ospf", "Exact but doesn't match length %d, size %d\n", *length, sizeof (min_ospf_mib)));
        return NULL;
    }
    ...
    DEBUGMSGTL (("smux/snmp_ospf", "[var_ospf] var len %d, oid obtained Len %d-", *var_len, *length));
    DEBUGMSGOID (("smux/snmp_ospf", name, *length));
    DEBUGMSG (("smux/snmp_ospf", "\n"));
