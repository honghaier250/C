...
netsnmp_feature_require (container_lifo) netsnmp_feature_require (ipaddress_entry_update)
 ...
 */
void ipAddressTable_container_init (netsnmp_container ** container_ptr_ptr, netsnmp_cache * cache)
{
    DEBUGMSGTL (("verbose:ipAddressTable:ipAddressTable_container_init", "called\n"));

    if (NULL == container_ptr_ptr)
    {
        snmp_log (LOG_ERR, "bad container param to ipAddressTable_container_init\n");
        return;
    }
     ...
     * assigned...
     */
    *container_ptr_ptr = netsnmp_container_find ("ipAddressTable:table_container");
    if (NULL != *container_ptr_ptr)
    {
        (*container_ptr_ptr)->container_name = strdup ("ipAddressTable");
        ipAddressTable_container_load (*container_ptr_ptr);
        CONTAINER_FOR_EACH (*container_ptr_ptr, (netsnmp_container_obj_func *) _clear_times, NULL);
    }
    ...
    cache->flags |=
        (NETSNMP_CACHE_DONT_AUTO_RELEASE | NETSNMP_CACHE_DONT_FREE_EXPIRED
         | NETSNMP_CACHE_DONT_FREE_BEFORE_LOAD | NETSNMP_CACHE_AUTO_RELOAD | NETSNMP_CACHE_DONT_INVALIDATE_ON_SET);
...
{
    DEBUGMSGTL (("verbose:ipAddressTable:ipAddressTable_container_shutdown", "called\n"));
 ...
 * check entry for update
 */
static void _check_entry_for_updates (ipAddressTable_rowreq_ctx * rowreq_ctx, void **magic)
{
    ...
    else
    {
         ...
         */
        if (netsnmp_access_ipaddress_entry_update (rowreq_ctx->data, ipaddress_entry) > 0)
 ...
 * add new entry
 */
static void _add_new_entry (netsnmp_ipaddress_entry * ipaddress_entry, netsnmp_container * container)
{
                                     ...
                                     ipaddress_entry->ia_address, ipaddress_entry->ia_address_len)))
    {
        ...
        rowreq_ctx->ipAddressLastChanged = rowreq_ctx->ipAddressCreated = netsnmp_get_agent_uptime ();
    }
    else
    {
        if (NULL != rowreq_ctx)
        {
            snmp_log (LOG_ERR, "error setting index while loading " "ipAddressTable cache.\n");
            ipAddressTable_release_rowreq_ctx (rowreq_ctx);
        }
        else
        {
            snmp_log (LOG_ERR, "memory allocation failed while loading " "ipAddressTable cache.\n");
            netsnmp_access_ipaddress_entry_free (ipaddress_entry);
        }
...
int ipAddressTable_container_load (netsnmp_container * container)
{
    ...
    DEBUGMSGTL (("verbose:ipAddressTable:ipAddressTable_cache_load", "called\n"));
     ...
     */
    ipaddress_container =
        netsnmp_access_ipaddress_container_load (NULL, NETSNMP_ACCESS_IPADDRESS_LOAD_ADDL_IDX_BY_ADDR);
    /*
    ...
    tmp_ptr[1] = NULL;
    CONTAINER_FOR_EACH (container, (netsnmp_container_obj_func *) _check_entry_for_updates, tmp_ptr);

    /*
     * now add any new interfaces
     */
    CONTAINER_FOR_EACH (ipaddress_container, (netsnmp_container_obj_func *) _add_new_entry, container);

    /*
     * free the container. we've either claimed each entry, or released it,
     * so the access function doesn't need to clear the container.
     */
    netsnmp_access_ipaddress_container_free (ipaddress_container, NETSNMP_ACCESS_IPADDRESS_FREE_DONT_CLEAR);
 ...
 */
int
ipAddressTable_validate_index (ipAddressTable_registration * ipAddressTable_reg, ipAddressTable_rowreq_ctx * rowreq_ctx)
{
    int rc = MFD_SUCCESS;

    DEBUGMSGTL (("verbose:ipAddressTable:ipAddressTable_validate_index", "called\n"));
     ...
     *
     */
    if ((4 != rowreq_ctx->tbl_idx.ipAddressAddr_len) && (16 != rowreq_ctx->tbl_idx.ipAddressAddr_len))
    {
        snmp_log (LOG_WARNING, "invalid index for a new row in the " "ipAddressTable table.\n");
        /*
    ...
    {
        memcpy (rowreq_ctx->data->ia_address, rowreq_ctx->tbl_idx.ipAddressAddr, rowreq_ctx->tbl_idx.ipAddressAddr_len);
