 ...
 */
int ipv4InterfaceTable_init_data (ipv4InterfaceTable_registration * ipv4InterfaceTable_reg)
{
    DEBUGMSGTL (("verbose:ipv4InterfaceTable:ipv4InterfaceTable_init_data", "called\n"));
 ...
 */
void ipv4InterfaceTable_container_init (netsnmp_container ** container_ptr_ptr)
{
    DEBUGMSGTL (("verbose:ipv4InterfaceTable:ipv4InterfaceTable_container_init", "called\n"));

    if (NULL == container_ptr_ptr)
    {
        snmp_log (LOG_ERR, "bad container param to ipv4InterfaceTable_container_init\n");
        return;
    }
 ...
 */
void ipv4InterfaceTable_check_entry_for_updates (const ifTable_rowreq_ctx * ift_rrc, netsnmp_interface_entry * entry)
{
    ...
    DEBUGMSGTL (("verbose:ipv4InterfaceTable:check_entry_for_updates", "called\n"));
    ...
    if (NULL == ip4if_rrc)
    {
         ...
         */
        if ((NULL != entry) && (entry->ns_flags & NETSNMP_INTERFACE_FLAGS_HAS_IPV4))
        {
            /*
             * yes.
             */
    ...
    else
    {
         ...
         */
        if ((NULL == entry) || (0 == (entry->ns_flags & NETSNMP_INTERFACE_FLAGS_HAS_IPV4)))
        {
            /*
             * no
             */
        ...
        else
        {
            ...
            if ((entry->retransmit_v4 !=
                 ift_rrc->data.ifentry->retransmit_v4) || (entry->reasm_max_v4 != ift_rrc->data.ifentry->reasm_max_v4))
            {
                DEBUGMSGTL (("ipv4InterfaceTable:check_entry_for_updates",
 ...
 */
void ipv4InterfaceTable_container_shutdown (netsnmp_container * container_ptr)
{
    DEBUGMSGTL (("verbose:ipv4InterfaceTable:ipv4InterfaceTable_container_shutdown", "called\n"));

    if (NULL == container_ptr)
    {
        snmp_log (LOG_ERR, "bad params to ipv4InterfaceTable_container_shutdown\n");
        return;
    }
 ...
 *
 */
int ipv4InterfaceTable_container_load (netsnmp_container * container)
{
 ...
 * @retval MFD_ERROR       : other error.
 */
int ipv4InterfaceTable_row_prep (ipv4InterfaceTable_rowreq_ctx * rowreq_ctx)
{
    DEBUGMSGTL (("verbose:ipv4InterfaceTable:ipv4InterfaceTable_row_prep", "called\n"));
