 ...
 */
     int ipDefaultRouterTable_init_data (ipDefaultRouterTable_registration * ipDefaultRouterTable_reg)
{
 ...
 */
void ipDefaultRouterTable_container_init (netsnmp_container ** container_ptr_ptr, netsnmp_cache * cache)
{
    DEBUGMSGTL (("verbose:ipDefaultRouterTable:ipDefaultRouterTable_container_init", "called\n"));

    if (NULL == container_ptr_ptr)
    {
        snmp_log (LOG_ERR, "bad container param to ipDefaultRouterTable_container_init\n");
        return;
    }
    ...
    *container_ptr_ptr = NULL;
    ...
    if (NULL == cache)
    {
        snmp_log (LOG_ERR, "bad cache param to ipDefaultRouterTable_container_init\n");
        return;
    }
...
void ipDefaultRouterTable_container_shutdown (netsnmp_container * container_ptr)
{
    ...
    {
        snmp_log (LOG_ERR, "bad params to ipDefaultRouterTable_container_shutdown\n");
        return;
    }
 ...
 * add new entry
 */
static void _add_new_entry (netsnmp_defaultrouter_entry * defaultrouter_entry, netsnmp_container * container)
{
     ...
     */
    rowreq_ctx = ipDefaultRouterTable_allocate_rowreq_ctx (defaultrouter_entry, NULL);
    if ((NULL != rowreq_ctx) &&
        (MFD_SUCCESS ==
    ...
    else
    {
        if (NULL != rowreq_ctx)
        {
            snmp_log (LOG_ERR, "error setting index while loading " "ipDefaultRouterTable cache.\n");
            ipDefaultRouterTable_release_rowreq_ctx (rowreq_ctx);
        }
        else
        {
            snmp_log (LOG_ERR, "memory allocation failed while loading " "ipDefaultRouterTable cache.\n");
            netsnmp_access_defaultrouter_entry_free (defaultrouter_entry);
        }
...
int ipDefaultRouterTable_container_load (netsnmp_container * container)
{
    ...
    defaultrouter_container =
        netsnmp_access_defaultrouter_container_load (NULL, NETSNMP_ACCESS_DEFAULTROUTER_LOAD_ADDL_IDX_BY_ADDR);
     ...
     */
    CONTAINER_FOR_EACH (defaultrouter_container, (netsnmp_container_obj_func *) _add_new_entry, container);

    /*
     * free the container. we've either claimed each entry, or released it,
     * so the access function doesn't need to clear the container.
     */
    netsnmp_access_defaultrouter_container_free (defaultrouter_container, NETSNMP_ACCESS_DEFAULTROUTER_FREE_DONT_CLEAR);
