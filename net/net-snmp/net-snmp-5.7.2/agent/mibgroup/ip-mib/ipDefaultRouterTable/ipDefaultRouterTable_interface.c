 ...
 */
     ...
     static void _ipDefaultRouterTable_container_init (ipDefaultRouterTable_interface_ctx * if_ctx);

     static void _ipDefaultRouterTable_container_shutdown (ipDefaultRouterTable_interface_ctx * if_ctx);
...
ipDefaultRouterTable_registration *ipDefaultRouterTable_registration_set (ipDefaultRouterTable_registration * newreg)
{
...
{
    netsnmp_baby_steps_access_methods *access_multiplexer = &ipDefaultRouterTable_if_ctx.access_multiplexer;

    netsnmp_table_registration_info *tbl_info = &ipDefaultRouterTable_if_ctx.tbl_info;
    ...
    reginfo =
        netsnmp_handler_registration_create ("ipDefaultRouterTable",
                                             ...
                                             ipDefaultRouterTable_oid_size, HANDLER_CAN_BABY_STEP | HANDLER_CAN_RONLY);
    if (NULL == reginfo)
    {
    ...
    handler =
        netsnmp_container_table_handler_get (tbl_info,
                                             ipDefaultRouterTable_if_ctx.container, TABLE_CONTAINER_KEY_NETSNMP_INDEX);
    netsnmp_inject_handler (reginfo, handler);
 ...
 */
int ipDefaultRouterTable_index_to_oid (netsnmp_index * oid_idx, ipDefaultRouterTable_mib_index * mib_idx)
{
     ...
     * set up varbinds
     */
    memset (&var_ipDefaultRouterAddressType, 0x00, sizeof (var_ipDefaultRouterAddressType));
    var_ipDefaultRouterAddressType.type = ASN_INTEGER;
    memset (&var_ipDefaultRouterAddress, 0x00, sizeof (var_ipDefaultRouterAddress));
    var_ipDefaultRouterAddress.type = ASN_OCTET_STR;
    memset (&var_ipDefaultRouterIfIndex, 0x00, sizeof (var_ipDefaultRouterIfIndex));
    ...
    snmp_set_var_value (&var_ipDefaultRouterAddressType,
                        (u_char *) & mib_idx->ipDefaultRouterAddressType, sizeof (mib_idx->ipDefaultRouterAddressType));
    ...
    snmp_set_var_value (&var_ipDefaultRouterIfIndex,
                        (u_char *) & mib_idx->ipDefaultRouterIfIndex, sizeof (mib_idx->ipDefaultRouterIfIndex));


    err = build_oid_noalloc (oid_idx->oids, oid_idx->len, &oid_idx->len, NULL, 0, &var_ipDefaultRouterAddressType);
    if (err)
 ...
 */
int ipDefaultRouterTable_index_from_oid (netsnmp_index * oid_idx, ipDefaultRouterTable_mib_index * mib_idx)
{
    int err = SNMP_ERR_NOERROR;
    ...
    if (err == SNMP_ERR_NOERROR)
    {
         ...
         */
        if (var_ipDefaultRouterAddress.val_len > sizeof (mib_idx->ipDefaultRouterAddress))
            err = SNMP_ERR_GENERR;
        else
        {
            ...
            mib_idx->ipDefaultRouterAddress_len =
                var_ipDefaultRouterAddress.val_len / sizeof (mib_idx->ipDefaultRouterAddress[0]);
        }
 ...
 */
ipDefaultRouterTable_rowreq_ctx *ipDefaultRouterTable_allocate_rowreq_ctx (ipDefaultRouterTable_data * data,
                                                                           void *user_init_ctx)
{
    ipDefaultRouterTable_rowreq_ctx *rowreq_ctx = SNMP_MALLOC_TYPEDEF (ipDefaultRouterTable_rowreq_ctx);

    DEBUGMSGTL (("internal:ipDefaultRouterTable:ipDefaultRouterTable_allocate_rowreq_ctx", "called\n"));

    if (NULL == rowreq_ctx)
    {
        snmp_log (LOG_ERR, "Couldn't allocate memory for a " "ipDefaultRouterTable_rowreq_ctx.\n");
        return NULL;
    }
    else
    {
    ...
    {
        if (SNMPERR_SUCCESS != ipDefaultRouterTable_rowreq_ctx_init (rowreq_ctx, user_init_ctx))
        {
 ...
 */
void ipDefaultRouterTable_release_rowreq_ctx (ipDefaultRouterTable_rowreq_ctx * rowreq_ctx)
{
                                       ...
                                       netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    {
        DEBUGMSGTL (("internal:ipDefaultRouterTable", "skipping additional pre_request\n"));
    ...
    if (MFD_SUCCESS != rc)
    {
         ...
         */
        DEBUGMSGTL (("ipDefaultRouterTable", "error %d from " "ipDefaultRouterTable_pre_request\n", rc));
...
_mfd_ipDefaultRouterTable_post_request (netsnmp_mib_handler * handler,
                                        ...
                                        * reginfo,
                                        netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ipDefaultRouterTable_rowreq_ctx *rowreq_ctx = netsnmp_container_table_row_extract (requests);
    ...
    {
        DEBUGMSGTL (("internal:ipDefaultRouterTable", "waiting for last post_request\n"));
        return SNMP_ERR_NOERROR;
    }
    ...
    if (MFD_SUCCESS != rc)
    {
         ...
         * nothing we can do about it but log it
         */
        DEBUGMSGTL (("ipDefaultRouterTable", "error %d from " "ipDefaultRouterTable_post_request\n", rc));
    }
...
_ipDefaultRouterTable_get_column (ipDefaultRouterTable_rowreq_ctx * rowreq_ctx, netsnmp_variable_list * var, int column)
{
    ...
    switch (column)
    {
        ...
        case COLUMN_IPDEFAULTROUTERLIFETIME:
            ...
            rc = ipDefaultRouterLifetime_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_IPDEFAULTROUTERPREFERENCE:
            ...
            rc = ipDefaultRouterPreference_get (rowreq_ctx, (u_long *) var->val.string);
            break;

        default:
            if (IPDEFAULTROUTERTABLE_MIN_COL <= column && column <= IPDEFAULTROUTERTABLE_MAX_COL)
            {
                DEBUGMSGTL (("internal:ipDefaultRouterTable:_mfd_ipDefaultRouterTable_get_column",
                             "assume column %d is reserved\n", column));
                rc = MFD_SKIP;
            }
            else
            {
                snmp_log (LOG_ERR, "unknown column %d in _ipDefaultRouterTable_get_column\n", column);
            }
            break;
    }
                                      ...
                                      netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    for (; requests; requests = requests->next)
    {
        ...
        rc = _ipDefaultRouterTable_get_column (rowreq_ctx, requests->requestvb, tri->colnum);
        if (rc)
        {
            if (MFD_SKIP == rc)
            {
...
static int _cache_load (netsnmp_cache * cache, void *vmagic)
{
    ...
    {
        snmp_log (LOG_ERR, "invalid cache for ipDefaultRouterTable_cache_load\n");
        return -1;
    }
     ...
     * call user code
     */
    return ipDefaultRouterTable_container_load ((netsnmp_container *) cache->magic);
}                                /* _cache_load */
 ...
 * @internal
 */
static void _container_item_free (ipDefaultRouterTable_rowreq_ctx * rowreq_ctx, void *context)
{
    DEBUGMSGTL (("internal:ipDefaultRouterTable:_container_item_free", "called\n"));
...
static void _container_free (netsnmp_container * container)
{
    ...
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "invalid container in ipDefaultRouterTable_container_free\n");
        return;
    }
 ...
 */
void _ipDefaultRouterTable_container_init (ipDefaultRouterTable_interface_ctx * if_ctx)
{
    ...
    if_ctx->cache = netsnmp_cache_create (30,    /* timeout in seconds */
                                          ...
                                          ipDefaultRouterTable_oid, ipDefaultRouterTable_oid_size);
    ...
    if (NULL == if_ctx->container)
        if_ctx->container = netsnmp_container_find ("ipDefaultRouterTable:table_container");
    if (NULL == if_ctx->container)
    {
        snmp_log (LOG_ERR, "error creating container in " "ipDefaultRouterTable_container_init\n");
        return;
    }
...
ipDefaultRouterTable_rowreq_ctx *ipDefaultRouterTable_row_find_by_mib_index (ipDefaultRouterTable_mib_index * mib_idx)
{
    ...
    rowreq_ctx = CONTAINER_FIND (ipDefaultRouterTable_if_ctx.container, &oid_idx);
