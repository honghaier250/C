 ...
 */
     ...
     static void _ipAddressPrefixTable_container_init (ipAddressPrefixTable_interface_ctx * if_ctx);

     static void _ipAddressPrefixTable_container_shutdown (ipAddressPrefixTable_interface_ctx * if_ctx);
...
ipAddressPrefixTable_registration *ipAddressPrefixTable_registration_set (ipAddressPrefixTable_registration * newreg)
{
...
{
    netsnmp_baby_steps_access_methods *access_multiplexer = &ipAddressPrefixTable_if_ctx.access_multiplexer;

    netsnmp_table_registration_info *tbl_info = &ipAddressPrefixTable_if_ctx.tbl_info;
    ...
    {
        snmp_log (LOG_ERR, "could not initialize container for ipAddressPrefixTable\n");
        return;
    }
    ...
    reginfo =
        netsnmp_handler_registration_create ("ipAddressPrefixTable",
                                             ...
                                             ipAddressPrefixTable_oid_size, HANDLER_CAN_BABY_STEP | HANDLER_CAN_RONLY);
    if (NULL == reginfo)
    {
    ...
    handler =
        netsnmp_container_table_handler_get (tbl_info,
                                             ipAddressPrefixTable_if_ctx.container, TABLE_CONTAINER_KEY_NETSNMP_INDEX);
    netsnmp_inject_handler (reginfo, handler);
 ...
 */
int ipAddressPrefixTable_index_to_oid (netsnmp_index * oid_idx, ipAddressPrefixTable_mib_index * mib_idx)
{
     ...
     * set up varbinds
     */
    memset (&var_ipAddressPrefixIfIndex, 0x00, sizeof (var_ipAddressPrefixIfIndex));
    ...
    snmp_set_var_value (&var_ipAddressPrefixIfIndex,
                        (u_char *) & mib_idx->ipAddressPrefixIfIndex, sizeof (mib_idx->ipAddressPrefixIfIndex));

    /*
     * ipAddressPrefixType(2)/InetAddressType/ASN_INTEGER/long(u_long)//l/a/w/E/r/d/h 
     */
    snmp_set_var_value (&var_ipAddressPrefixType,
                        (u_char *) & mib_idx->ipAddressPrefixType, sizeof (mib_idx->ipAddressPrefixType));
    ...
    snmp_set_var_value (&var_ipAddressPrefixLength,
                        (u_char *) & mib_idx->ipAddressPrefixLength, sizeof (mib_idx->ipAddressPrefixLength));


    err = build_oid_noalloc (oid_idx->oids, oid_idx->len, &oid_idx->len, NULL, 0, &var_ipAddressPrefixIfIndex);
    if (err)
 ...
 */
int ipAddressPrefixTable_index_from_oid (netsnmp_index * oid_idx, ipAddressPrefixTable_mib_index * mib_idx)
{
    int err = SNMP_ERR_NOERROR;
     ...
     */
    err = parse_oid_indexes (oid_idx->oids, oid_idx->len, &var_ipAddressPrefixIfIndex);
    if (err == SNMP_ERR_NOERROR)
    {
         ...
         */
        if (var_ipAddressPrefixPrefix.val_len > sizeof (mib_idx->ipAddressPrefixPrefix))
            err = SNMP_ERR_GENERR;
        else
        {
            memcpy (mib_idx->ipAddressPrefixPrefix,
                    var_ipAddressPrefixPrefix.val.string, var_ipAddressPrefixPrefix.val_len);
            mib_idx->ipAddressPrefixPrefix_len =
                var_ipAddressPrefixPrefix.val_len / sizeof (mib_idx->ipAddressPrefixPrefix[0]);
        }
 ...
 */
ipAddressPrefixTable_rowreq_ctx *ipAddressPrefixTable_allocate_rowreq_ctx (void *user_init_ctx)
{
    ipAddressPrefixTable_rowreq_ctx *rowreq_ctx = SNMP_MALLOC_TYPEDEF (ipAddressPrefixTable_rowreq_ctx);

    DEBUGMSGTL (("internal:ipAddressPrefixTable:ipAddressPrefixTable_allocate_rowreq_ctx", "called\n"));

    if (NULL == rowreq_ctx)
    {
        snmp_log (LOG_ERR, "Couldn't allocate memory for a " "ipAddressPrefixTable_rowreq_ctx.\n");
        return NULL;
    }
    ...
    {
        if (SNMPERR_SUCCESS != ipAddressPrefixTable_rowreq_ctx_init (rowreq_ctx, user_init_ctx))
        {
 ...
 */
void ipAddressPrefixTable_release_rowreq_ctx (ipAddressPrefixTable_rowreq_ctx * rowreq_ctx)
{
                                       ...
                                       * agtreq_info, netsnmp_request_info * requests)
{
    ...
    {
        DEBUGMSGTL (("internal:ipAddressPrefixTable", "skipping additional pre_request\n"));
    ...
    if (MFD_SUCCESS != rc)
    {
         ...
         */
        DEBUGMSGTL (("ipAddressPrefixTable", "error %d from " "ipAddressPrefixTable_pre_request\n", rc));
...
_mfd_ipAddressPrefixTable_post_request (netsnmp_mib_handler * handler, netsnmp_handler_registration
                                        ...
                                        * agtreq_info, netsnmp_request_info * requests)
{
    ipAddressPrefixTable_rowreq_ctx *rowreq_ctx = netsnmp_container_table_row_extract (requests);
    ...
    {
        DEBUGMSGTL (("internal:ipAddressPrefixTable", "waiting for last post_request\n"));
        return SNMP_ERR_NOERROR;
    }
    ...
    if (MFD_SUCCESS != rc)
    {
         ...
         * nothing we can do about it but log it
         */
        DEBUGMSGTL (("ipAddressPrefixTable", "error %d from " "ipAddressPrefixTable_post_request\n", rc));
    }
...
_ipAddressPrefixTable_get_column (ipAddressPrefixTable_rowreq_ctx * rowreq_ctx, netsnmp_variable_list * var, int column)
{
    ...
    switch (column)
    {
        ...
        case COLUMN_IPADDRESSPREFIXORIGIN:
            ...
            rc = ipAddressPrefixOrigin_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_IPADDRESSPREFIXONLINKFLAG:
            var->val_len = sizeof (u_long);
            var->type = ASN_INTEGER;
            rc = ipAddressPrefixOnLinkFlag_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_IPADDRESSPREFIXAUTONOMOUSFLAG:
            var->val_len = sizeof (u_long);
            var->type = ASN_INTEGER;
            rc = ipAddressPrefixAutonomousFlag_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_IPADDRESSPREFIXADVPREFERREDLIFETIME:
            ...
            rc = ipAddressPrefixAdvPreferredLifetime_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_IPADDRESSPREFIXADVVALIDLIFETIME:
            var->val_len = sizeof (u_long);
            var->type = ASN_UNSIGNED;
            rc = ipAddressPrefixAdvValidLifetime_get (rowreq_ctx, (u_long *) var->val.string);
            break;

        default:
            snmp_log (LOG_ERR, "unknown column %d in _ipAddressPrefixTable_get_column\n", column);
            break;
    }
                                      ...
                                      netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    for (; requests; requests = requests->next)
    {
        ...
        rc = _ipAddressPrefixTable_get_column (rowreq_ctx, requests->requestvb, tri->colnum);
        if (rc)
        {
            if (MFD_SKIP == rc)
            {
...
static int _cache_load (netsnmp_cache * cache, void *vmagic)
{
    ...
    {
        snmp_log (LOG_ERR, "invalid cache for ipAddressPrefixTable_cache_load\n");
        return -1;
    }
     ...
     * call user code
     */
    return ipAddressPrefixTable_container_load ((netsnmp_container *) cache->magic);
}                                /* _cache_load */
 ...
 * @internal
 */
static void _container_item_free (ipAddressPrefixTable_rowreq_ctx * rowreq_ctx, void *context)
{
    DEBUGMSGTL (("internal:ipAddressPrefixTable:_container_item_free", "called\n"));
...
static void _container_free (netsnmp_container * container)
{
    ...
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "invalid container in ipAddressPrefixTable_container_free\n");
        return;
    }
 ...
 */
void _ipAddressPrefixTable_container_init (ipAddressPrefixTable_interface_ctx * if_ctx)
{
    ...
    if_ctx->cache = netsnmp_cache_create (30,    /* timeout in seconds */
                                          ...
                                          ipAddressPrefixTable_oid, ipAddressPrefixTable_oid_size);
    ...
    if (NULL == if_ctx->container)
        if_ctx->container = netsnmp_container_find ("ipAddressPrefixTable:table_container");
    if (NULL == if_ctx->container)
    {
        snmp_log (LOG_ERR, "error creating container in " "ipAddressPrefixTable_container_init\n");
        return;
    }
...
ipAddressPrefixTable_rowreq_ctx *ipAddressPrefixTable_row_find_by_mib_index (ipAddressPrefixTable_mib_index * mib_idx)
{
    ...
    rowreq_ctx = CONTAINER_FIND (ipAddressPrefixTable_if_ctx.container, &oid_idx);
