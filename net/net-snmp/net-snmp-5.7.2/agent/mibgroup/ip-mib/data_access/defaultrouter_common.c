 ...
 */
static int _access_defaultrouter_entry_compare_addr (const void *lhs, const void *rhs);

static void _access_defaultrouter_entry_release (netsnmp_defaultrouter_entry * entry, void *unused);
 ...
 * the header file.
 */
extern int netsnmp_arch_defaultrouter_entry_init (netsnmp_defaultrouter_entry * entry);

extern int netsnmp_arch_defaultrouter_container_load (netsnmp_container * container, u_int load_flags);
 ...
 */
netsnmp_container *netsnmp_access_defaultrouter_container_load (netsnmp_container * container, u_int load_flags)
{
    int rc;
    ...
    if (0 != rc)
    {
        netsnmp_access_defaultrouter_container_free (container, NETSNMP_ACCESS_DEFAULTROUTER_FREE_NOFLAGS);
        container = NULL;
    }
...
void netsnmp_access_defaultrouter_container_free (netsnmp_container * container, u_int free_flags)
{
    DEBUGMSGTL (("access:defaultrouter:container", "free\n"));

    if (NULL == container)
    {
        snmp_log (LOG_ERR, "invalid container for netsnmp_access_defaultrouter_free\n");
        return;
    }
    ...
    if (!(free_flags & NETSNMP_ACCESS_DEFAULTROUTER_FREE_DONT_CLEAR))
    {
         ...
         * free all items.
         */
        CONTAINER_CLEAR (container, (netsnmp_container_obj_func *) _access_defaultrouter_entry_release, NULL);
    }
...
netsnmp_defaultrouter_entry *netsnmp_access_defaultrouter_entry_create (void)
{
    ...
    netsnmp_defaultrouter_entry *entry = SNMP_MALLOC_TYPEDEF (netsnmp_defaultrouter_entry);
 ...
 */
int netsnmp_access_defaultrouter_entry_update (netsnmp_defaultrouter_entry * lhs, netsnmp_defaultrouter_entry * rhs)
{
    int changed = 0;
 ...
 * @retval 0   : no error
 */
int netsnmp_access_defaultrouter_entry_copy (netsnmp_defaultrouter_entry * lhs, netsnmp_defaultrouter_entry * rhs)
{
    lhs->dr_addresstype = rhs->dr_addresstype;
    lhs->dr_address_len = rhs->dr_address_len;
    memcpy (lhs->dr_address, rhs->dr_address, rhs->dr_address_len);
