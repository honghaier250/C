 ...
 * @retval !0 errors
 */
int netsnmp_arch_defaultrouter_container_load (netsnmp_container * container, u_int load_flags)
{
    int rc = 0;
...
static int _load (netsnmp_container * container)
{
    ...
    DEBUGMSGTL (("access:defaultrouter",
                 "cannot get default router information" "as netlink socket is not available\n"));
    return -1;
#else
    int rc = 0;
     ...
     */
    unsigned char *rcvbuf = rcvbufmem + sizeof (intmax_t) - (((intptr_t) rcvbufmem) % sizeof (intmax_t));

    unsigned char *sndbuf = sndbufmem + sizeof (intmax_t) - (((intptr_t) sndbufmem) % sizeof (intmax_t));
    ...
    if (setsockopt (nlsk, SOL_SOCKET, SO_RCVBUF, &rcvbuf_size, sizeof (rcvbuf_size)) < 0)
    {
        snmp_log (LOG_ERR, "Could not open netlink socket : %s\n", strerror (errno));
        close (nlsk);
        return -1;
     ...
     */
    count = sendto (nlsk, sndbuf, hdr->nlmsg_len, 0, (struct sockaddr *) &addr, sizeof (struct sockaddr_nl));
    if (count < 0)
    {
    ...
    do
    {
         ...
         * Get the message
         */
        count = recvfrom (nlsk, rcvbuf, RCVBUF_SIZE, 0, (struct sockaddr *) &addr, &sock_len);
        if (count < 0)
        {
        ...
        while (NLMSG_OK (nlmhp, count))
        {
            ...
            while (RTA_OK (rtap, rtcount))
            {
                switch (rtap->rta_type)
                {
                    ...
                    case RTA_CACHEINFO:
                        rtci = RTA_DATA (rtap);
                        if ((rtmp->rtm_flags & RTM_F_CLONED) || (rtci && rtci->rta_expires))
                        {
            ...
            if (address_len != 0 && if_index != -1 && lifetime != 0 && preference != -3)
            {
                ...
                if (CONTAINER_INSERT (container, entry) < 0)
                {
                    DEBUGMSGTL (("access:arp:container",
                                 "error with defaultrouter_entry: " "insert into container failed.\n"));
