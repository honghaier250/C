 ...
 */
int netsnmp_access_systemstats_container_arch_load (netsnmp_container * container, u_int load_flags)
{
    int rc;
 ...
 * @retval <0 error
 */
static int _systemstats (mibgroup_e mib, netsnmp_container * container, u_int load_flags)
{
    mib2_ipIfStatsEntry_t ipe, iptot;

    req_e req = GET_FIRST;

    int ipversion = (mib == MIB_IP6) ? MIB2_INETADDRESSTYPE_ipv6 : MIB2_INETADDRESSTYPE_ipv4;

    memset (&iptot, '\0', sizeof (iptot));

    while (getMibstat (mib, &ipe, sizeof (ipe), req, &Get_everything, NULL) == 0)
    {
        req = GET_NEXT;
...
static void _add_ipstats (mib2_ipIfStatsEntry_t * o1, mib2_ipIfStatsEntry_t * o2)
{
 ...
 */
static int _insert_entry (netsnmp_container * container, mib2_ipIfStatsEntry_t * ipe)
{
    int i;

    netsnmp_systemstats_entry *ep = netsnmp_access_systemstats_entry_create (ipe->ipIfStatsIPVersion, 0,
                                                                             "ipSystemStatsTable");
    ...
    ep->stats.HCInReceives.low = ipe->ipIfStatsHCInReceives & 0xffffffff;
    ep->stats.HCInReceives.high = ipe->ipIfStatsHCInReceives >> 32;
    ep->stats.HCInOctets.low = ipe->ipIfStatsHCInOctets & 0xffffffff;
    ...
    ep->stats.HCInForwDatagrams.low = ipe->ipIfStatsHCInForwDatagrams & 0xffffffff;
    ep->stats.HCInForwDatagrams.high = ipe->ipIfStatsHCInForwDatagrams >> 32;
    ...
    ep->stats.HCInDelivers.low = ipe->ipIfStatsHCInDelivers & 0xffffffff;
    ep->stats.HCInDelivers.high = ipe->ipIfStatsHCInDelivers >> 32;
    ep->stats.HCOutRequests.low = ipe->ipIfStatsHCOutRequests & 0xffffffff;
    ep->stats.HCOutRequests.high = ipe->ipIfStatsHCOutRequests >> 32;
    ep->stats.HCOutNoRoutes.low = ipe->ipIfStatsOutNoRoutes & 0xffffffff;
    ep->stats.HCOutNoRoutes.high = 0;
    ep->stats.HCOutForwDatagrams.low = ipe->ipIfStatsHCOutForwDatagrams & 0xffffffff;
    ep->stats.HCOutForwDatagrams.high = ipe->ipIfStatsHCOutForwDatagrams >> 32;
    ...
    ep->stats.HCOutTransmits.low = ipe->ipIfStatsHCOutTransmits & 0xffffffff;
    ...
    ep->stats.HCInMcastOctets.low = ipe->ipIfStatsHCInMcastOctets & 0xffffffff;
    ep->stats.HCInMcastOctets.high = ipe->ipIfStatsHCInMcastOctets >> 32;
    ep->stats.HCOutMcastPkts.low = ipe->ipIfStatsHCOutMcastPkts & 0xffffffff;
    ep->stats.HCOutMcastPkts.high = ipe->ipIfStatsHCOutMcastPkts >> 32;
    ep->stats.HCOutMcastOctets.low = ipe->ipIfStatsHCOutMcastOctets & 0xffffffff;
    ...
    ep->stats.HCOutBcastPkts.low = ipe->ipIfStatsHCOutBcastPkts & 0xffffffff;
