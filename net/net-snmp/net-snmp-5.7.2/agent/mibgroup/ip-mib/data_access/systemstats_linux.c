...
static int _additional_systemstats_v4 (netsnmp_systemstats_entry * entry, u_int load_flags);
...
static int _systemstats_v4 (netsnmp_container * container, u_int load_flags)
{
    ...
    {
        DEBUGMSGTL (("access:systemstats", "Failed to load Systemstats Table (linux1)\n"));
    ...
    {
        fclose (devin);
        snmp_log (LOG_ERR, "unexpected header length in /proc/net/snmp." " %d != 224\n", len);
        return -4;
    }
    ...
    if (start)
    {
        ...
        *stats++ = 0;            /* null terminate name */
        ...
        entry = netsnmp_access_systemstats_entry_create (1, 0, "ipSystemStatsTable.ipv4");
        if (NULL == entry)
        {
        ...
        scan_count = sscanf (stats,
                             ...
                             &scan_vals[15], &scan_vals[16], &scan_vals[17], &scan_vals[18]);
        DEBUGMSGTL (("access:systemstats", "  read %d values\n", scan_count));

        if (scan_count != 19)
        {
            snmp_log (LOG_ERR, "error scanning systemstats data (expected %d, got %d)\n", 19, scan_count);
...
static int _additional_systemstats_v4 (netsnmp_systemstats_entry * entry, u_int load_flags)
{
    ...
    DEBUGMSGTL (("access:systemstats:container:arch", "load addtional v4 (flags %u)\n", load_flags));

    if (!(devin = fopen ("/proc/net/netstat", "r")))
    {
        DEBUGMSGTL (("access:systemstats", "cannot open /proc/net/netstat\n"));
    ...
    while (fgets (line, sizeof (line), devin))
    {
        if (strncmp (IP_EXT_HEAD, line, sizeof (IP_EXT_HEAD) - 1) == 0)
        {
            ...
            memset (scan_vals, 0x0, sizeof (scan_vals));
            scan_count = sscanf (line, "%*s"    /* ignore `IpExt:' */
            ...
            {
                snmp_log (LOG_ERR,
                          "error scanning addtional systemstats data" "(minimum expected %d, got %d)\n", 6, scan_count);
                retval = -4;
                break;
            }
    ...
    if (retval < 0)
        DEBUGMSGTL (("access:systemstats", "/proc/net/netstat does not include addtional stats\n"));
 ...
 */
static int _systemstats_v6_load_file (netsnmp_systemstats_entry * entry, FILE * devin)
{
    char line[1024];
 ...
 */
static int _systemstats_v6_load_systemstats (netsnmp_container * container, u_int load_flags)
{
    FILE *devin;
    ...
    entry = netsnmp_access_systemstats_entry_create (2, 0, "ipSystemStatsTable.ipv6");
    if (NULL == entry)
        return -3;
    ...
    {
        DEBUGMSGTL (("access:systemstats", "Failed to load Systemstats Table (linux1), cannot open %s\n", filename));
        return 0;
    }
 ...
 */
static int _systemstats_v6_load_ifstats (netsnmp_container * container, u_int load_flags)
{
    DIR *dev_snmp6_dir;
    ...
    {
        DEBUGMSGTL (("access:ifstats", "Failed to load IPv6 IfStats Table (linux)\n"));
        return 0;
    }
    ...
    while ((dev_snmp6_entry = readdir (dev_snmp6_dir)) != NULL)
    {
        ...
        else
        {
            ...
            {
                snmp_log (LOG_ERR, "%s doesn't include ifIndex line", dev_filename);
                fclose (devin);
                continue;
            }
        ...
        entry = netsnmp_access_systemstats_entry_create (2, scan_val, "ipIfStatsTable.ipv6");
        if (NULL == entry)
        {
            fclose (devin);
