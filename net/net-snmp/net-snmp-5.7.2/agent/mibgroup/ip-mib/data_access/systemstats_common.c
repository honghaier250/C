 ...
 * the header file.
 */
extern int netsnmp_access_systemstats_container_arch_load (netsnmp_container * container, u_int load_flags);

extern void netsnmp_access_systemstats_arch_init (void);
 ...
 */
netsnmp_container *netsnmp_access_systemstats_container_load (netsnmp_container * container, u_int load_flags)
{
    int rc;
    ...
    if (0 != rc)
    {
        netsnmp_access_systemstats_container_free (container, NETSNMP_ACCESS_SYSTEMSTATS_FREE_NOFLAGS);
        container = NULL;
    }
...
void netsnmp_access_systemstats_container_free (netsnmp_container * container, u_int free_flags)
{
    ...
    if (!(free_flags & NETSNMP_ACCESS_SYSTEMSTATS_FREE_DONT_CLEAR))
    {
         ...
         * free all items.
         */
        CONTAINER_CLEAR (container, (netsnmp_container_obj_func *) _entry_release, NULL);
    }
...
/**
 */
netsnmp_systemstats_entry *netsnmp_access_systemstats_entry_get_by_index (netsnmp_container * container, oid index)
{
    netsnmp_index tmp;
    ...
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "invalid container for netsnmp_access_systemstats_entry_get_by_index\n");
        return NULL;
    }
...
/**
 */
netsnmp_systemstats_entry *netsnmp_access_systemstats_entry_create (int version, int if_index, const char *tableName)
{
    netsnmp_systemstats_entry *entry = SNMP_MALLOC_TYPEDEF (netsnmp_systemstats_entry);
                                               ...
                                               netsnmp_systemstats_entry * new_vals)
{
    ...
    if ((NULL == prev_vals) || (NULL == new_vals) ||
        (prev_vals->index[0] != new_vals->index[0]) || (prev_vals->index[1] != new_vals->index[1]))
        return -1;
    ...
    else
    {
         ...
         */
        memcpy (&prev_vals->stats.columnAvail[0], &new_vals->stats.columnAvail[0],
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTNOROUTES])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCOutNoRoutes,
                                                     ...
                                                     &prev_vals->old_stats->HCOutNoRoutes, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTDISCARDS])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCOutDiscards,
                                                     ...
                                                     &prev_vals->old_stats->HCOutDiscards, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTFRAGREQDS])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCOutFragReqds,
                                                     ...
                                                     &prev_vals->old_stats->HCOutFragReqds, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTFRAGFAILS])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCOutFragFails,
                                                     ...
                                                     &prev_vals->old_stats->HCOutFragFails, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTFRAGCREATES])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCOutFragCreates,
                                                     ...
                                                     &prev_vals->old_stats->HCOutFragCreates, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCINFORWDATAGRAMS])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCInForwDatagrams,
                                                     ...
                                                     &prev_vals->old_stats->HCInForwDatagrams, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTREQUESTS])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCOutRequests,
                                                     ...
                                                     &prev_vals->old_stats->HCOutRequests, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTFORWDATAGRAMS])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCOutForwDatagrams,
                                                     ...
                                                     &prev_vals->old_stats->HCOutForwDatagrams, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTTRANSMITS])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCOutTransmits,
                                                     ...
                                                     &prev_vals->old_stats->HCOutTransmits, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCINMCASTPKTS])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCInMcastPkts,
                                                     ...
                                                     &prev_vals->old_stats->HCInMcastPkts, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCINMCASTOCTETS])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCInMcastOctets,
                                                     ...
                                                     &prev_vals->old_stats->HCInMcastOctets, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTMCASTPKTS])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCOutMcastPkts,
                                                     ...
                                                     &prev_vals->old_stats->HCOutMcastPkts, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCOUTMCASTOCTETS])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCOutMcastOctets,
                                                     ...
                                                     &prev_vals->old_stats->HCOutMcastOctets, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
        ...
        if (new_vals->stats.columnAvail[IPSYSTEMSTATSTABLE_HCINBCASTPKTS])
            if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.HCInBcastPkts,
                                                     ...
                                                     &prev_vals->old_stats->HCInBcastPkts, &need_wrap_check))
                DEBUGMSGTL (("access:systemstats",
 ...
 * @retval  0 : no error
 */
int netsnmp_access_systemstats_entry_update (netsnmp_systemstats_entry * lhs, netsnmp_systemstats_entry * rhs)
{
    DEBUGMSGTL (("access:systemstats", "copy\n"));

    if ((NULL == lhs) || (NULL == rhs) || (lhs->index[0] != rhs->index[0]) || (lhs->index[1] != rhs->index[1]))
        return -1;
