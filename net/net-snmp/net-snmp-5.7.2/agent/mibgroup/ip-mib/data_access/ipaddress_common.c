 ...
 */
     static int _access_ipaddress_entry_compare_addr (const void *lhs, const void *rhs);

     static void _access_ipaddress_entry_release (netsnmp_ipaddress_entry * entry, void *unused);
 ...
 * the header file.
 */
     extern int netsnmp_arch_ipaddress_container_load (netsnmp_container * container, u_int load_flags);

     extern int netsnmp_arch_ipaddress_entry_init (netsnmp_ipaddress_entry * entry);

     extern int netsnmp_arch_ipaddress_entry_copy (netsnmp_ipaddress_entry * lhs, netsnmp_ipaddress_entry * rhs);

     extern void netsnmp_arch_ipaddress_entry_cleanup (netsnmp_ipaddress_entry * entry);

     extern int netsnmp_arch_ipaddress_create (netsnmp_ipaddress_entry * entry);

     extern int netsnmp_arch_ipaddress_delete (netsnmp_ipaddress_entry * entry);
     ...
     netsnmp_container *netsnmp_access_ipaddress_container_init (u_int flags)
{
    ...
    {
        netsnmp_container *container2 = netsnmp_container_find ("ipaddress_addr:access_ipaddress:table_container");
 ...
 */
netsnmp_container *netsnmp_access_ipaddress_container_load (netsnmp_container * container, u_int load_flags)
{
    int rc;
    ...
    if (0 != rc)
    {
        netsnmp_access_ipaddress_container_free (container, NETSNMP_ACCESS_IPADDRESS_FREE_NOFLAGS);
        container = NULL;
    }
...
void netsnmp_access_ipaddress_container_free (netsnmp_container * container, u_int free_flags)
{
    ...
    if (!(free_flags & NETSNMP_ACCESS_IPADDRESS_FREE_DONT_CLEAR))
    {
         ...
         * free all items.
         */
        CONTAINER_CLEAR (container, (netsnmp_container_obj_func *) _access_ipaddress_entry_release, NULL);
    }
...
{
    netsnmp_ipaddress_entry *entry = SNMP_MALLOC_TYPEDEF (netsnmp_ipaddress_entry);
...
int netsnmp_access_ipaddress_entry_set (netsnmp_ipaddress_entry * entry)
{
    ...
    {
        DEBUGMSGT (("access:ipaddress:set", "cant find name for index %" NETSNMP_PRIo "d\n", entry->if_index));
        return -1;
    }
    ...
    {
        DEBUGMSGT (("access:ipaddress:set", "non-volatile storagetypes unsupported\n"));
        return -1;
    }
 ...
 */
int netsnmp_access_ipaddress_entry_update (netsnmp_ipaddress_entry * lhs, netsnmp_ipaddress_entry * rhs)
{
    int rc, changed = 0;
 ...
 * @retval 0   : no error
 */
int netsnmp_access_ipaddress_entry_copy (netsnmp_ipaddress_entry * lhs, netsnmp_ipaddress_entry * rhs)
{
    int rc;
...
netsnmp_ipaddress_flags_copy (u_long * ipAddressPrefixAdvPreferredLifetime,
                              ...
                              u_long * ia_valid_lifetime, u_char * ia_onlink_flag, u_char * ia_autonomous_flag)
{
...
netsnmp_ipaddress_prefix_origin_copy (u_long * ipAddressPrefixOrigin,
                                      u_char ia_origin, int flags, u_long ipAddressAddrType)
{
