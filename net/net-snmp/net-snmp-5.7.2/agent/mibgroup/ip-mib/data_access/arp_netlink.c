...
/**
 */
netsnmp_arp_access *netsnmp_access_arp_create (u_int init_flags,
                                               NetsnmpAccessArpUpdate * update_hook,
                                               NetsnmpAccessArpGC * gc_hook,
                                               int *cache_timeout, int *cache_flags, char *cache_expired)
{
...
static int fillup_entry_info (netsnmp_arp_entry * entry, struct nlmsghdr *nlmp)
{
    ...
    if (tb[NDA_DST])
    {
        ...
        memcpy (entry->arp_ipaddress, RTA_DATA (tb[NDA_DST]), entry->arp_ipaddress_len);
    }
    if (tb[NDA_LLADDR])
    {
        ...
        memcpy (entry->arp_physaddress, RTA_DATA (tb[NDA_LLADDR]), entry->arp_physaddress_len);
    }
