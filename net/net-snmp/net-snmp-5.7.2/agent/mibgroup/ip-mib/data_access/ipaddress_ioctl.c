 ...
 */
     _ioctl_extras *netsnmp_ioctl_ipaddress_extras_get (netsnmp_ipaddress_entry * entry)
{
 ...
 */
_ioctl_extras *netsnmp_ioctl_ipaddress_entry_init (netsnmp_ipaddress_entry * entry)
{
 ...
 * @retval <0: error
 */
int netsnmp_ioctl_ipaddress_entry_copy (netsnmp_ipaddress_entry * lhs, netsnmp_ipaddress_entry * rhs)
{
 ...
 */
int _netsnmp_ioctl_ipaddress_container_load_v4 (netsnmp_container * container, int idx_offset)
{
    ...
    for (i = 0; i < interfaces; ++i, ++ifrp)
    {
        ...
        if (addr_info.bcastflg)
        {
            ...
            memcpy (bcastentry->ia_address, &addr_info.addr, bcastentry->ia_address_len);
        }
#endif
        ...
        si = (struct sockaddr_in *) &ifrp->ifr_addr;
        entry->ia_prefix_len = netsnmp_ipaddress_ipv4_prefix_len (ntohl (si->sin_addr.s_addr));
        if (bcastentry)
        ...
        if (bcastentry)
        {
            ...
            {
                DEBUGMSGTL (("access:ipaddress:container",
                             "error with ipaddress_entry: insert broadcast entry into container failed.\n"));
...
int _netsnmp_ioctl_ipaddress_set_v4 (netsnmp_ipaddress_entry * entry)
{
    ...
    if ('\0' == extras->name[0])
    {
        ...
        snprintf (ifrq.ifr_name, sizeof (ifrq.ifr_name), "%s:%d", name, alias_idx);
