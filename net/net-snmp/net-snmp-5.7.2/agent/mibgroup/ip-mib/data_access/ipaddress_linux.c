...
netsnmp_feature_require (find_prefix_info) netsnmp_feature_child_of (ipaddress_arch_entry_copy, ipaddress_common)
...
#include "ipaddress_ioctl.h"
     ...
     int netsnmp_access_ipaddress_extra_prefix_info (int index, u_long * preferedlt, ulong * validlt, char *addr);
#endif
 ...
 */
int netsnmp_arch_ipaddress_entry_copy (netsnmp_ipaddress_entry * lhs, netsnmp_ipaddress_entry * rhs)
{
    int rc;
...
int _load_v6 (netsnmp_container * container, int idx_offset)
{
    ...
    DEBUGMSGTL (("access:ipaddress:container",
                 "cannot get ip address information" "as netlink socket is not available\n"));
    return -1;
#else
    FILE *in;
...
#define PROCFILE "/proc/net/if_inet6"
    ...
    while (fgets (line, sizeof (line), in))
    {
        ...
        else
        {
            ...
            DEBUGMSGTL (("access:ipaddress:ipv6", "unknown flags 0x%x\n", flags));
        }
        ...
        if (netsnmp_access_ipaddress_extra_prefix_info
            (entry->if_index, &entry->ia_prefered_lifetime, &entry->ia_valid_lifetime, addr) < 0)
        {
