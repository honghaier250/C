 ...
 */
     ...
     static void _ipv6ScopeZoneIndexTable_container_init (ipv6ScopeZoneIndexTable_interface_ctx * if_ctx);

     static void _ipv6ScopeZoneIndexTable_container_shutdown (ipv6ScopeZoneIndexTable_interface_ctx * if_ctx);
...
ipv6ScopeZoneIndexTable_registration *ipv6ScopeZoneIndexTable_registration_get (void)
{
...
{
    netsnmp_baby_steps_access_methods *access_multiplexer = &ipv6ScopeZoneIndexTable_if_ctx.access_multiplexer;

    netsnmp_table_registration_info *tbl_info = &ipv6ScopeZoneIndexTable_if_ctx.tbl_info;
    ...
    {
        handler = netsnmp_cache_handler_get (ipv6ScopeZoneIndexTable_if_ctx.cache);
        netsnmp_inject_handler (reginfo, handler);
    }
 ...
 */
int ipv6ScopeZoneIndexTable_index_to_oid (netsnmp_index * oid_idx, ipv6ScopeZoneIndexTable_mib_index * mib_idx)
{
     ...
     * set up varbinds
     */
    memset (&var_ipv6ScopeZoneIndexIfIndex, 0x00, sizeof (var_ipv6ScopeZoneIndexIfIndex));
    ...
    snmp_set_var_value (&var_ipv6ScopeZoneIndexIfIndex,
                        (u_char *) & mib_idx->ipv6ScopeZoneIndexIfIndex, sizeof (mib_idx->ipv6ScopeZoneIndexIfIndex));


    err = build_oid_noalloc (oid_idx->oids, oid_idx->len, &oid_idx->len, NULL, 0, &var_ipv6ScopeZoneIndexIfIndex);
    if (err)
 ...
 */
int ipv6ScopeZoneIndexTable_index_from_oid (netsnmp_index * oid_idx, ipv6ScopeZoneIndexTable_mib_index * mib_idx)
{
    int err = SNMP_ERR_NOERROR;
     ...
     */
    err = parse_oid_indexes (oid_idx->oids, oid_idx->len, &var_ipv6ScopeZoneIndexIfIndex);
 ...
 */
ipv6ScopeZoneIndexTable_rowreq_ctx *ipv6ScopeZoneIndexTable_allocate_rowreq_ctx (ipv6ScopeZoneIndexTable_data * data,
                                                                                 void *user_init_ctx)
{
    ipv6ScopeZoneIndexTable_rowreq_ctx *rowreq_ctx = SNMP_MALLOC_TYPEDEF (ipv6ScopeZoneIndexTable_rowreq_ctx);

    DEBUGMSGTL (("internal:ipv6ScopeZoneIndexTable:ipv6ScopeZoneIndexTable_allocate_rowreq_ctx", "called\n"));

    if (NULL == rowreq_ctx)
    {
        snmp_log (LOG_ERR, "Couldn't allocate memory for a " "ipv6ScopeZoneIndexTable_rowreq_ctx.\n");
        return NULL;
    }
    else
    {
    ...
    {
        if (SNMPERR_SUCCESS != ipv6ScopeZoneIndexTable_rowreq_ctx_init (rowreq_ctx, user_init_ctx))
        {
                                          ...
                                          netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    {
        DEBUGMSGTL (("internal:ipv6ScopeZoneIndexTable", "skipping additional pre_request\n"));
    ...
    if (MFD_SUCCESS != rc)
    {
         ...
         */
        DEBUGMSGTL (("ipv6ScopeZoneIndexTable", "error %d from " "ipv6ScopeZoneIndexTable_pre_request\n", rc));
                                           ...
                                           netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    {
        DEBUGMSGTL (("internal:ipv6ScopeZoneIndexTable", "waiting for last post_request\n"));
        return SNMP_ERR_NOERROR;
    }
    ...
    if (MFD_SUCCESS != rc)
    {
         ...
         * nothing we can do about it but log it
         */
        DEBUGMSGTL (("ipv6ScopeZoneIndexTable", "error %d from " "ipv6ScopeZoneIndexTable_post_request\n", rc));
    }
 ...
 * wrapper
 */
static ipv6ScopeZoneIndexTable_rowreq_ctx *_mfd_ipv6ScopeZoneIndexTable_rowreq_from_index (netsnmp_index * oid_idx,
                                                                                           int *rc_ptr)
{
                                     ...
                                     rowreq_ctx, netsnmp_variable_list * var, int column)
{
    ...
    switch (column)
    {
        ...
        case COLUMN_IPV6SCOPEZONEINDEXLINKLOCAL:
            ...
            rc = ipv6ScopeZoneIndexLinkLocal_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_IPV6SCOPEZONEINDEXADMINLOCAL:
            var->val_len = sizeof (u_long);
            var->type = ASN_UNSIGNED;
            rc = ipv6ScopeZoneIndexAdminLocal_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_IPV6SCOPEZONEINDEXSITELOCAL:
            var->val_len = sizeof (u_long);
            var->type = ASN_UNSIGNED;
            rc = ipv6ScopeZoneIndexSiteLocal_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_IPV6SCOPEZONEINDEXORGANIZATIONLOCAL:
            var->val_len = sizeof (u_long);
            var->type = ASN_UNSIGNED;
            rc = ipv6ScopeZoneIndexOrganizationLocal_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        default:
            if (IPV6SCOPEZONEINDEXTABLE_MIN_COL <= column && column <= IPV6SCOPEZONEINDEXTABLE_MAX_COL)
            {
                DEBUGMSGTL (("internal:ipv6ScopeZoneIndexTable:_mfd_ipv6ScopeZoneIndexTable_get_column",
                             "assume column %d is reserved\n", column));
                rc = MFD_SKIP;
            }
            else
            {
                snmp_log (LOG_ERR, "unknown column %d in _ipv6ScopeZoneIndexTable_get_column\n", column);
            }
            break;
    }
                                         ...
                                         netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    for (; requests; requests = requests->next)
    {
        ...
        rc = _ipv6ScopeZoneIndexTable_get_column (rowreq_ctx, requests->requestvb, tri->colnum);
        if (rc)
        {
            if (MFD_SKIP == rc)
            {
 ...
 * @internal
 */
static void _container_item_free (ipv6ScopeZoneIndexTable_rowreq_ctx * rowreq_ctx, void *context)
{
    DEBUGMSGTL (("internal:ipv6ScopeZoneIndexTable:_container_item_free", "called\n"));
...
{
    DEBUGMSGTL (("internal:ipv6ScopeZoneIndexTable:_container_free", "called\n"));

    if (NULL == container)
    {
        snmp_log (LOG_ERR, "invalid container in ipv6ScopeZoneIndexTable_container_free\n");
        return;
    }
...
void _ipv6ScopeZoneIndexTable_container_init (ipv6ScopeZoneIndexTable_interface_ctx * if_ctx)
{
    ...
    if_ctx->cache = netsnmp_cache_create (30,    /* timeout in seconds */
                                          ...
                                          ipv6ScopeZoneIndexTable_oid, ipv6ScopeZoneIndexTable_oid_size);
    ...
    {
        if_ctx->container = netsnmp_container_find ("ipv6ScopeZoneIndexTable:table_container");
        if (if_ctx->container)
            if_ctx->container->container_name = strdup ("ipv6ScopeZoneIndexTable");
    }
    if (NULL == if_ctx->container)
    {
        snmp_log (LOG_ERR, "error creating container in " "ipv6ScopeZoneIndexTable_container_init\n");
        return;
    }
    if (NULL != if_ctx->cache)
...
ipv6ScopeZoneIndexTable_rowreq_ctx *ipv6ScopeZoneIndexTable_row_find_by_mib_index
    (ipv6ScopeZoneIndexTable_mib_index * mib_idx)
{
    ipv6ScopeZoneIndexTable_rowreq_ctx *rowreq_ctx;
...
static int _cache_load (netsnmp_cache * cache, void *vmagic)
{
    ...
    {
        snmp_log (LOG_ERR, "invalid cache for ipScopeZoneIndexTable_cache_load\n");
        return -1;
    }
     ...
     * call user code
     */
    return ipv6ScopeZoneIndexTable_container_load ((netsnmp_container *) cache->magic);
}                                /* _cache_load */
...
static void _cache_free (netsnmp_cache * cache, void *magic)
{
    ...
    if ((NULL == cache) || (NULL == cache->magic))
    {
        snmp_log (LOG_ERR, "invalid cache in ipScopeZoneIndexTable_cache_free\n");
        return;
    }
