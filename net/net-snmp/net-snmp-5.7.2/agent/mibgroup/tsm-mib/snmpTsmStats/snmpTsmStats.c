...
netsnmp_feature_require (helper_statistics) netsnmp_feature_child_of (shutdown_snmptsmsession, netsnmp_unused)
     ...
     void init_snmpTsmStats (void)
{
    ...
    _myreg = netsnmp_create_handler_registration ("snmpTsmStats", NULL,
                                                  snmpTsmStats_oid, OID_LENGTH (snmpTsmStats_oid), HANDLER_CAN_RONLY);
    if (NULL == _myreg)
    {
        snmp_log (LOG_ERR, "failed to create handler registration for " "snmpTsmStats\n");
        return;
    }
...
void shutdown_snmpTsmSession (void)
{
    if (_myreg)
    {
