...
netsnmp_feature_require (set_agent_uptime)
     ...
     */
     int agentx_synch_input (int op, netsnmp_session * session, int reqid, netsnmp_pdu * pdu, void *magic)
{
...
int agentx_synch_response (netsnmp_session * ss, netsnmp_pdu * pdu, netsnmp_pdu ** response)
{
...
int agentx_open_session (netsnmp_session * ss)
{
    ...
    if (pdu == NULL)
        return 0;
    timeout = netsnmp_ds_get_int (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_AGENTX_TIMEOUT);
    if (timeout < 0)
        pdu->time = 0;
    else
...
agentx_register (netsnmp_session * ss, oid start[], size_t startlen,
                 int priority, int range_subid, oid range_ubound, int timeout, u_char flags, const char *contextName)
{
    netsnmp_pdu *pdu, *response;
    ...
    if (range_subid)
    {
        snmp_pdu_add_variable (pdu, start, startlen, ASN_OBJECT_ID, (u_char *) start, startlen * sizeof (oid));
...
agentx_unregister (netsnmp_session * ss, oid start[], size_t startlen,
                   int priority, int range_subid, oid range_ubound, const char *contextName)
{
    netsnmp_pdu *pdu, *response;
...
netsnmp_variable_list *agentx_register_index (netsnmp_session * ss, netsnmp_variable_list * varbind, int flags)
{
    netsnmp_pdu *pdu, *response;
...
int agentx_add_agentcaps (netsnmp_session * ss, const oid * agent_cap, size_t agent_caplen, const char *descr)
{
    netsnmp_pdu *pdu, *response;
...
int agentx_remove_agentcaps (netsnmp_session * ss, const oid * agent_cap, size_t agent_caplen)
{
    netsnmp_pdu *pdu, *response;
