...
netsnmp_feature_require (unix_socket_paths) netsnmp_feature_require (free_agent_snmp_session_by_session)
     void real_init_master (void)
{
    ...
    {
        agentx_sockets = strdup (netsnmp_ds_get_string (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_X_SOCKET));
        ...
        if (agentx_sockets[0] != '/')
        {
            ...
            if (agentx_sockets[5] != '/')
            {
                snmp_log (LOG_ERR, "Error: %s transport is not supported, disabling agentx/master.\n", agentx_sockets);
    ...
    sess.timeout = netsnmp_ds_get_int (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_AGENTX_TIMEOUT);
    sess.retries = netsnmp_ds_get_int (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_AGENTX_RETRIES);
    ...
    DEBUGMSGTL (("agentx/master", "initializing...   DONE\n"));
         ...
         */
int agentx_got_response (int operation, netsnmp_session * session, int reqid, netsnmp_pdu * pdu, void *magic)
{
    ...
    DEBUGMSGTL (("agentx/master", "got response errstat=%ld, (req=0x%x,trans="
                 "0x%x,sess=0x%x)\n",
                 pdu->errstat, (unsigned) pdu->reqid, (unsigned) pdu->transid, (unsigned) pdu->sessid));
    ...
    if (pdu->errstat != AGENTX_ERR_NOERROR)
    {
        ...
        return 1;
             ...
             cache->reqinfo->mode == MODE_GETNEXT || cache->reqinfo->mode == MODE_GETBULK)
    {
        /*
        ...
        if (request || var)
        {
             ...
             */
            snmp_log (LOG_ERR, "response to agentx request illegal.  bailing out.\n");
            netsnmp_set_request_error (cache->reqinfo, requests, SNMP_ERR_GENERR);
        }
                       ...
                       netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    while (request)
    {
        ...
        if (reqinfo->mode == MODE_GETNEXT || reqinfo->mode == MODE_GETBULK)
        {
            ...
            if (snmp_oid_compare (nptr, nlen, request->subtree->start_a, request->subtree->start_len) < 0)
            {
                DEBUGMSGTL (("agentx/master", "inexact request preceeding region ("));
                DEBUGMSGOID (("agentx/master", request->subtree->start_a, request->subtree->start_len));
                DEBUGMSG (("agentx/master", ")\n"));
            ...
            if (request->inclusive)
            {
                ...
                DEBUGMSGOID (("agentx/master", request->range_end, request->range_end_len));
                DEBUGMSG (("agentx/master", "\n"));
                snmp_pdu_add_variable (pdu, nptr, nlen, ASN_PRIV_INCL_RANGE,
                                       (u_char *) request->range_end, request->range_end_len * sizeof (oid));
                request->inclusive = 0;
            }
            else
            {
     ...
     */
    if (pdu->command != AGENTX_MSG_CLEANUPSET)
        cb_data = netsnmp_create_delegated_cache (handler, reginfo, reqinfo, requests, (void *) ax_session);
    else
        cb_data = NULL;
