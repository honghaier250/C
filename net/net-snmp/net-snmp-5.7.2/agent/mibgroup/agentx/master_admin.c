...
netsnmp_feature_require (allocate_globalcacheid) netsnmp_feature_require (remove_index)
     netsnmp_session *find_agentx_session (netsnmp_session * session, int sessid)
{
    netsnmp_session *sp;
...
int open_agentx_session (netsnmp_session * session, netsnmp_pdu * pdu)
{
    ...
    sp->securityAuthProto = snmp_duplicate_objid (pdu->variables->name, pdu->variables->name_length);
...
int register_agentx_list (netsnmp_session * session, netsnmp_pdu * pdu)
{
    ...
    sprintf (buf, "AgentX subagent %ld, session %8p, subsession %8p", sp->sessid, session, sp);
    /*
    ...
    switch (netsnmp_register_mib (buf, NULL, 0, 1,
                                  ...
                                  sp, (char *) pdu->community, pdu->time, flags, reg, 1))
    {
...
int unregister_agentx_list (netsnmp_session * session, netsnmp_pdu * pdu)
{
    ...
    if (pdu->range_subid != 0)
    {
        ...
        rc = netsnmp_unregister_mib_table_row (pdu->variables->name,
                                               pdu->variables->name_length,
                                               pdu->priority, pdu->range_subid, ubound, (char *) pdu->community);
    }
    else
    {
        rc = unregister_mib_context (pdu->variables->name,
                                     pdu->variables->name_length, pdu->priority, 0, 0, (char *) pdu->community);
    }
...
int add_agent_caps_list (netsnmp_session * session, netsnmp_pdu * pdu)
{
    ...
    description = netsnmp_strdup_and_null (pdu->variables->val.string, pdu->variables->val_len);
    register_sysORTable_sess (pdu->variables->name, pdu->variables->name_length, description, sp);
    free (description);
    return AGENTX_ERR_NOERROR;
}
...
int remove_agent_caps_list (netsnmp_session * session, netsnmp_pdu * pdu)
{
    ...
    rc = unregister_sysORTable_sess (pdu->variables->name, pdu->variables->name_length, sp);
...
int agentx_notify (netsnmp_session * session, netsnmp_pdu * pdu)
{
    ...
    if (snmp_oid_compare (var->name, var->name_length, sysuptime_oid, sysuptime_oid_len) == 0)
    {
        var = var->next_variable;
    }

    if (!var || snmp_oid_compare (var->name, var->name_length, snmptrap_oid, snmptrap_oid_len) != 0)
        return AGENTX_ERR_PROCESSING_ERROR;
...
int handle_master_agentx_packet (int operation, netsnmp_session * session, int reqid, netsnmp_pdu * pdu, void *magic)
{
    ...
    {
        DEBUGMSGTL (("agentx/master", "transport connect on session %8p\n", session));
        return 1;
    }
    ...
    DEBUGMSGTL (("agentx/master", "handle pdu (req=0x%lx,trans=0x%lx,sess=0x%lx)\n",
                 (unsigned long) pdu->reqid, (unsigned long) pdu->transid, (unsigned long) pdu->sessid));
    ...
    DEBUGMSGTL (("agentx/master", "send response, stat %d (req=0x%lx,trans="
                 "0x%lx,sess=0x%lx)\n",
                 asp->status, (unsigned long) pdu->reqid, (unsigned long) pdu->transid, (unsigned long) pdu->sessid));
