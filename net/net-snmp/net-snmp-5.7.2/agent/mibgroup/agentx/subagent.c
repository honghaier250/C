...
netsnmp_feature_child_of (agentx_enable_subagent, agentx_subagent) netsnmp_feature_require (remove_trap_session)
     ...
     int handle_subagent_response (int op, netsnmp_session * session, int reqid, netsnmp_pdu * pdu, void *magic);

#ifndef NETSNMP_NO_WRITE_SUPPORT
     int handle_subagent_set_response (int op, netsnmp_session * session, int reqid, netsnmp_pdu * pdu, void *magic);
...
static void subagent_init_callback_session (void)
{
    ...
    {
        agentx_callback_sess = netsnmp_callback_open (callback_master_num, handle_subagent_response, NULL, NULL);
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
struct agent_netsnmp_set_info *save_set_vars (netsnmp_session * ss, netsnmp_pdu * pdu)
{
...
struct agent_netsnmp_set_info *restore_set_vars (netsnmp_session * sess, netsnmp_pdu * pdu)
{
    struct agent_netsnmp_set_info *ptr;
...
int handle_agentx_packet (int operation, netsnmp_session * session, int reqid, netsnmp_pdu * pdu, void *magic)
{
    ...
    if (operation == NETSNMP_CALLBACK_OP_DISCONNECT)
    {
        ...
        snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_INDEX_STOP, (void *) session);
    ...
    if (pdu->command == AGENTX_MSG_GET || pdu->command == AGENTX_MSG_GETNEXT || pdu->command == AGENTX_MSG_GETBULK)
    {
    ...
    switch (pdu->command)
    {
        ...
        case AGENTX_MSG_COMMITSET:
            ...
            {
                SNMP_FREE (smagic);
                snmp_log (LOG_WARNING, "dropping bad AgentX request (wrong mode %d)\n", asi->mode);
                send_agentx_error (session, pdu, AGENTX_ERR_PROCESSING_ERROR, 0);
                return 1;
            }
        ...
        case AGENTX_MSG_CLEANUPSET:
            ...
            if (asi->mode == SNMP_MSG_INTERNAL_SET_RESERVE1 || asi->mode == SNMP_MSG_INTERNAL_SET_RESERVE2)
            {
    ...
    result = snmp_async_send (agentx_callback_sess, internal_pdu, mycallback, retmagic);
    if (result == 0)
    {
...
int handle_subagent_response (int op, netsnmp_session * session, int reqid, netsnmp_pdu * pdu, void *magic)
{
    ...
    if (pdu->command == SNMP_MSG_INTERNAL_SET_FREE ||
        pdu->command == SNMP_MSG_INTERNAL_SET_UNDO || pdu->command == SNMP_MSG_INTERNAL_SET_COMMIT)
    {
    ...
    if (smagic->original_command == AGENTX_MSG_GETNEXT)
    {
        ...
        for (u = smagic->ovars, v = pdu->variables; u != NULL && v != NULL; u = u->next_variable, v = v->next_variable)
        {
            if (snmp_oid_compare (u->val.objid, u->val_len / sizeof (oid), nullOid, nullOidLen / sizeof (oid)) != 0)
            {
                /*
                 * The master agent requested scoping for this variable.  
                 */
                rc = snmp_oid_compare (v->name, v->name_length, u->val.objid, u->val_len / sizeof (oid));
                DEBUGMSGTL (("agentx/subagent", "result "));
                DEBUGMSGOID (("agentx/subagent", v->name, v->name_length));
                DEBUGMSG (("agentx/subagent", " scope to "));
                DEBUGMSGOID (("agentx/subagent", u->val.objid, u->val_len / sizeof (oid)));
                ...
                if (rc >= 0)
                {
                    ...
                    DEBUGMSGTL (("agentx/subagent", "scope violation -- return endOfMibView\n"));
                }
            }
            else
            {
...
int handle_subagent_set_response (int op, netsnmp_session * session, int reqid, netsnmp_pdu * pdu, void *magic)
{
    ...
    DEBUGMSGTL (("agentx/subagent",
                 ...
                 (unsigned) pdu->command, (unsigned) pdu->reqid, (unsigned) pdu->transid, (unsigned) pdu->sessid));
    pdu = snmp_clone_pdu (pdu);
    ...
    if (asi->mode == SNMP_MSG_INTERNAL_SET_RESERVE1)
    {
        ...
        if (!pdu->errstat)
        {
            ...
            result = snmp_async_send (agentx_callback_sess, pdu, handle_subagent_set_response, asi);
            if (result == 0)
            {
                snmp_free_pdu (pdu);
            }
            DEBUGMSGTL (("agentx/subagent", "  going from RESERVE1 -> RESERVE2\n"));
            return 1;
        }
    }
    else
    {
        if (asi->mode == SNMP_MSG_INTERNAL_SET_FREE ||
            asi->mode == SNMP_MSG_INTERNAL_SET_UNDO || asi->mode == SNMP_MSG_INTERNAL_SET_COMMIT)
        {
...
int agentx_registration_callback (int majorID, int minorID, void *serverarg, void *clientarg)
{
    struct register_parameters *reg_parms = (struct register_parameters *) serverarg;
    ...
    if (minorID == SNMPD_CALLBACK_REGISTER_OID)
        return agentx_register (agentx_ss,
                                ...
                                reg_parms->range_ubound, reg_parms->timeout, reg_parms->flags, reg_parms->contextName);
    else
        return agentx_unregister (agentx_ss,
                                  reg_parms->name, reg_parms->namelen,
                                  reg_parms->priority,
                                  reg_parms->range_subid, reg_parms->range_ubound, reg_parms->contextName);
}
...
static int subagent_shutdown (int majorID, int minorID, void *serverarg, void *clientarg)
{
...
int subagent_open_master_session (void)
{
    ...
    if (main_session)
    {
        snmp_log (LOG_WARNING, "AgentX session to master agent attempted to be re-opened.\n");
        return -1;
    }
    ...
    agentx_socket = netsnmp_ds_get_string (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_X_SOCKET);
    ...
    if (t == NULL)
    {
        ...
        if (!netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_NO_CONNECTION_WARNINGS))
        {
            ...
            snprintf (buf, sizeof (buf), "Warning: "
                      "Failed to connect to the agentx master agent (%s)", agentx_socket ? agentx_socket : "[NIL]");
    ...
    main_session =
        snmp_add_full (&sess, t, NULL, agentx_parse, NULL, NULL, agentx_realloc_build, agentx_check_packet, NULL);
    ...
    if (main_session == NULL)
    {
        if (!netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_NO_CONNECTION_WARNINGS))
        {
            ...
            snprintf (buf, sizeof (buf), "Error: "
                      "Failed to create the agentx master agent session (%s)", agentx_socket);
            snmp_sess_perror (buf, &sess);
        }
    ...
    snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_INDEX_START, (void *) main_session);
...
static void agentx_reopen_sysORTable (const struct sysORTable *data, void *v)
{
    ...
    agentx_add_agentcaps (agentx_ss, data->OR_oid, data->OR_oidlen, data->OR_descr);
}
...
static int subagent_register_ping_alarm (int majorID, int minorID, void *serverarg, void *clientarg)
{

    netsnmp_session *ss = (netsnmp_session *) clientarg;

    int ping_interval = netsnmp_ds_get_int (NETSNMP_DS_APPLICATION_ID,
                                            NETSNMP_DS_AGENT_AGENTX_PING_INTERVAL);
    ...
    if (ss)
    {
         ...
         */
        ss->securityModel = snmp_alarm_register (ping_interval, SA_REPEAT, agentx_check_session, ss);
    }
    else
    {
        /*
         ...
         */
        DEBUGMSGTL (("agentx/subagent", "subagent not properly attached, postponing registration till later....\n"));
        snmp_alarm_register (ping_interval, SA_REPEAT, agentx_reopen_session, NULL);
    }
    return 0;
}
