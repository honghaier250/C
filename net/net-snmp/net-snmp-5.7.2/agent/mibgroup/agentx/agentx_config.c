...
netsnmp_feature_require (user_information) netsnmp_feature_require (string_time_to_secs)
...
void agentx_parse_agentx_retries (const char *token, char *cptr)
{
    ...
    netsnmp_ds_set_int (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_AGENTX_RETRIES, x);
}
...
{
    DEBUGMSGTL (("agentx_register_app_config_handler", "registering .conf token for \"%s\"\n", token));
