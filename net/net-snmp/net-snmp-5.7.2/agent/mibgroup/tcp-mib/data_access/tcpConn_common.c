 ...
 */
static void _access_tcpconn_entry_release (netsnmp_tcpconn_entry * entry, void *unused);
 ...
 * the header file.
 */
extern int netsnmp_arch_tcpconn_container_load (netsnmp_container * container, u_int load_flags);

extern int netsnmp_arch_tcpconn_entry_init (netsnmp_tcpconn_entry * entry);

extern int netsnmp_arch_tcpconn_entry_copy (netsnmp_tcpconn_entry * lhs, netsnmp_tcpconn_entry * rhs);

extern void netsnmp_arch_tcpconn_entry_cleanup (netsnmp_tcpconn_entry * entry);
 ...
 */
netsnmp_container *netsnmp_access_tcpconn_container_load (netsnmp_container * container, u_int load_flags)
{
    int rc;
    ...
    if (0 != rc)
    {
        netsnmp_access_tcpconn_container_free (container, NETSNMP_ACCESS_TCPCONN_FREE_NOFLAGS);
        container = NULL;
    }
...
void netsnmp_access_tcpconn_container_free (netsnmp_container * container, u_int free_flags)
{
    ...
    if (!(free_flags & NETSNMP_ACCESS_TCPCONN_FREE_DONT_CLEAR))
    {
         ...
         * free all items.
         */
        CONTAINER_CLEAR (container, (netsnmp_container_obj_func *) _access_tcpconn_entry_release, NULL);
    }
 ...
 */
int netsnmp_access_tcpconn_entry_update (netsnmp_tcpconn_entry * lhs, netsnmp_tcpconn_entry * rhs)
{
    int rc, changed = 0;
