...
int netsnmp_arch_tcpconn_entry_init (netsnmp_tcpconn_entry * ep)
{
    init_kernel_sunos5 ();
    return 0;
}

void netsnmp_arch_tcpconn_entry_cleanup (netsnmp_tcpconn_entry * ep)
{
    /*
     * Do nothing
     */
...
int netsnmp_arch_tcpconn_entry_copy (netsnmp_tcpconn_entry * ep1, netsnmp_tcpconn_entry * ep2)
{
    /*
     * Do nothing
     */
...
int netsnmp_arch_tcpconn_entry_delete (netsnmp_tcpconn_entry * ep)
{
    /*
     * Not implemented 
     */
...
int netsnmp_arch_tcpconn_container_load (netsnmp_container * container, u_int load_flag)
{
    int rc;
...
static int _load_tcpconn_table_v4 (netsnmp_container * container, int flag)
{
    ...
    while (getMibstat (MIB_TCP_CONN, &tc, sizeof (tc), req, &Get_everything, 0) == 0)
    {
        req = GET_NEXT;
        if ((flag & NETSNMP_ACCESS_TCPCONN_LOAD_ONLYLISTEN &&
             tc.tcpConnState != MIB2_TCP_listen) ||
            (flag & NETSNMP_ACCESS_TCPCONN_LOAD_NOLISTEN && tc.tcpConnState == MIB2_TCP_listen))
        {
            continue;
        }
...
#if defined(NETSNMP_ENABLE_IPV6) && defined(SOLARIS_HAVE_IPV6_MIB_SUPPORT)
static int _load_tcpconn_table_v6 (netsnmp_container * container, int flag)
{
    ...
    while (getMibstat (MIB_TCP6_CONN, &tc6, sizeof (tc6), req, &Get_everything, 0) == 0)
    {
        req = GET_NEXT;
        if ((flag & NETSNMP_ACCESS_TCPCONN_LOAD_ONLYLISTEN &&
             tc6.tcp6ConnState != MIB2_TCP_listen) ||
            (flag & NETSNMP_ACCESS_TCPCONN_LOAD_NOLISTEN && tc6.tcp6ConnState == MIB2_TCP_listen))
        {
            continue;
        }
        ep = netsnmp_access_tcpconn_entry_create ();
        ...
        (void) memcpy (&ep->loc_addr, &tc6.tcp6ConnLocalAddress, ep->loc_addr_len);
