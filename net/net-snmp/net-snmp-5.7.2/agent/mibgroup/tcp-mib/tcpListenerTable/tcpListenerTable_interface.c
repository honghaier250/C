 ...
 */
     ...
     static void _tcpListenerTable_container_init (tcpListenerTable_interface_ctx * if_ctx);

     static void _tcpListenerTable_container_shutdown (tcpListenerTable_interface_ctx * if_ctx);
...
tcpListenerTable_registration *tcpListenerTable_registration_set (tcpListenerTable_registration * newreg)
{
 ...
 */
void _tcpListenerTable_initialize_interface (tcpListenerTable_registration * reg_ptr, u_long flags)
{
    netsnmp_baby_steps_access_methods *access_multiplexer = &tcpListenerTable_if_ctx.access_multiplexer;

    netsnmp_table_registration_info *tbl_info = &tcpListenerTable_if_ctx.tbl_info;
    ...
    {
        snmp_log (LOG_ERR, "could not initialize container for tcpListenerTable\n");
        return;
    }
    ...
    handler =
        netsnmp_container_table_handler_get (tbl_info,
                                             tcpListenerTable_if_ctx.container, TABLE_CONTAINER_KEY_NETSNMP_INDEX);
    netsnmp_inject_handler (reginfo, handler);
 ...
 */
void _tcpListenerTable_shutdown_interface (tcpListenerTable_registration * reg_ptr)
{
    /*
 ...
 */
int tcpListenerTable_index_to_oid (netsnmp_index * oid_idx, tcpListenerTable_mib_index * mib_idx)
{
     ...
     * set up varbinds
     */
    memset (&var_tcpListenerLocalAddressType, 0x00, sizeof (var_tcpListenerLocalAddressType));
    var_tcpListenerLocalAddressType.type = ASN_INTEGER;
    memset (&var_tcpListenerLocalAddress, 0x00, sizeof (var_tcpListenerLocalAddress));
     ...
     */
    var_tcpListenerLocalAddressType.next_variable = &var_tcpListenerLocalAddress;
    var_tcpListenerLocalAddress.next_variable = &var_tcpListenerLocalPort;
    var_tcpListenerLocalPort.next_variable = NULL;


    DEBUGMSGTL (("verbose:tcpListenerTable:tcpListenerTable_index_to_oid", "called\n"));
    ...
    snmp_set_var_value (&var_tcpListenerLocalPort,
                        (u_char *) & mib_idx->tcpListenerLocalPort, sizeof (mib_idx->tcpListenerLocalPort));


    err = build_oid_noalloc (oid_idx->oids, oid_idx->len, &oid_idx->len, NULL, 0, &var_tcpListenerLocalAddressType);
    if (err)
 ...
 */
int tcpListenerTable_index_from_oid (netsnmp_index * oid_idx, tcpListenerTable_mib_index * mib_idx)
{
    int err = SNMP_ERR_NOERROR;
    ...
    DEBUGMSGTL (("verbose:tcpListenerTable:tcpListenerTable_index_from_oid", "called\n"));

    /*
     * parse the oid into the individual index components
     */
    err = parse_oid_indexes (oid_idx->oids, oid_idx->len, &var_tcpListenerLocalAddressType);
    if (err == SNMP_ERR_NOERROR)
    {
         ...
         */
        if (var_tcpListenerLocalAddress.val_len > sizeof (mib_idx->tcpListenerLocalAddress))
            err = SNMP_ERR_GENERR;
        else
        {
            memcpy (mib_idx->tcpListenerLocalAddress,
                    var_tcpListenerLocalAddress.val.string, var_tcpListenerLocalAddress.val_len);
            mib_idx->tcpListenerLocalAddress_len =
                var_tcpListenerLocalAddress.val_len / sizeof (mib_idx->tcpListenerLocalAddress[0]);
        }
 ...
 */
tcpListenerTable_rowreq_ctx *tcpListenerTable_allocate_rowreq_ctx (tcpListenerTable_data * data, void *user_init_ctx)
{
    tcpListenerTable_rowreq_ctx *rowreq_ctx = SNMP_MALLOC_TYPEDEF (tcpListenerTable_rowreq_ctx);

    DEBUGMSGTL (("internal:tcpListenerTable:tcpListenerTable_allocate_rowreq_ctx", "called\n"));

    if (NULL == rowreq_ctx)
    {
        snmp_log (LOG_ERR, "Couldn't allocate memory for a " "tcpListenerTable_rowreq_ctx.\n");
        return NULL;
    }
    else
    {
    ...
    {
        if (SNMPERR_SUCCESS != tcpListenerTable_rowreq_ctx_init (rowreq_ctx, user_init_ctx))
        {
 ...
 */
void tcpListenerTable_release_rowreq_ctx (tcpListenerTable_rowreq_ctx * rowreq_ctx)
{
                                   ...
                                   netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    {
        DEBUGMSGTL (("internal:tcpListenerTable", "skipping additional pre_request\n"));
    ...
    if (MFD_SUCCESS != rc)
    {
         ...
         */
        DEBUGMSGTL (("tcpListenerTable", "error %d from " "tcpListenerTable_pre_request\n", rc));
                                    ...
                                    netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    {
        DEBUGMSGTL (("internal:tcpListenerTable", "waiting for last post_request\n"));
        return SNMP_ERR_NOERROR;
    }

    packet_rc = netsnmp_check_all_requests_error (agtreq_info->asp, 0);
    rc = tcpListenerTable_post_request (tcpListenerTable_if_ctx.user_ctx, packet_rc);
    if (MFD_SUCCESS != rc)
    {
        /*
         * nothing we can do about it but log it
         */
        DEBUGMSGTL (("tcpListenerTable", "error %d from " "tcpListenerTable_post_request\n", rc));
    }
...
_tcpListenerTable_get_column (tcpListenerTable_rowreq_ctx * rowreq_ctx, netsnmp_variable_list * var, int column)
{
    ...
    switch (column)
    {
        ...
        default:
            snmp_log (LOG_ERR, "unknown column %d in _tcpListenerTable_get_column\n", column);
            break;
    }
                                  ...
                                  netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    for (; requests; requests = requests->next)
    {
        ...
        rc = _tcpListenerTable_get_column (rowreq_ctx, requests->requestvb, tri->colnum);
        if (rc)
        {
            if (MFD_SKIP == rc)
            {
 ...
 * @internal
 */
static void _container_item_free (tcpListenerTable_rowreq_ctx * rowreq_ctx, void *context)
{
...
static void _container_free (netsnmp_container * container)
{
    ...
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "invalid container in tcpListenerTable_container_free\n");
        return;
    }
...
void _tcpListenerTable_container_init (tcpListenerTable_interface_ctx * if_ctx)
{
    ...
    if_ctx->cache = netsnmp_cache_create (30,    /* timeout in seconds */
                                          _cache_load, _cache_free, tcpListenerTable_oid, tcpListenerTable_oid_size);
    ...
    {
        if_ctx->container = netsnmp_container_find ("tcpListenerTable:table_container");
        if (if_ctx->container)
            if_ctx->container->container_name = strdup ("tcpListenerTable");
    }
    if (NULL == if_ctx->container)
    {
        snmp_log (LOG_ERR, "error creating container in " "tcpListenerTable_container_init\n");
        return;
    }
 ...
 */
void _tcpListenerTable_container_shutdown (tcpListenerTable_interface_ctx * if_ctx)
{
...
tcpListenerTable_rowreq_ctx *tcpListenerTable_row_find_by_mib_index (tcpListenerTable_mib_index * mib_idx)
{
    ...
    rowreq_ctx = (tcpListenerTable_rowreq_ctx *) CONTAINER_FIND (tcpListenerTable_if_ctx.container, &oid_idx);
