 ...
 */
int tcpConnectionTable_init_data (tcpConnectionTable_registration * tcpConnectionTable_reg)
{
    DEBUGMSGTL (("verbose:tcpConnectionTable:tcpConnectionTable_init_data", "called\n"));
 ...
 */
void tcpConnectionTable_container_init (netsnmp_container ** container_ptr_ptr, netsnmp_cache * cache)
{
    DEBUGMSGTL (("verbose:tcpConnectionTable:tcpConnectionTable_container_init", "called\n"));

    if (NULL == container_ptr_ptr)
    {
        snmp_log (LOG_ERR, "bad container param to tcpConnectionTable_container_init\n");
        return;
    }
    ...
    *container_ptr_ptr = NULL;
    ...
    if (NULL == cache)
    {
        snmp_log (LOG_ERR, "bad cache param to tcpConnectionTable_container_init\n");
        return;
    }
...
void tcpConnectionTable_container_shutdown (netsnmp_container * container_ptr)
{
    ...
    {
        snmp_log (LOG_ERR, "bad params to tcpConnectionTable_container_shutdown\n");
        return;
    }
 ...
 * add new entry
 */
static void _add_connection (netsnmp_tcpconn_entry * entry, netsnmp_container * container)
{
    ...
    if ((NULL != rowreq_ctx) &&
        (MFD_SUCCESS == tcpConnectionTable_indexes_set (rowreq_ctx,
                                                        ...
                                                        entry->loc_port,
                                                        entry->rmt_addr_len,
                                                        entry->rmt_addr, entry->rmt_addr_len, entry->rmt_port)))
    {
        if (CONTAINER_INSERT (container, rowreq_ctx))
        {
            NETSNMP_LOGONCE ((LOG_DEBUG, "Error inserting entry to tcpConnectionTable," " entry already exists.\n"));
    ...
    else
    {
        if (rowreq_ctx)
        {
            snmp_log (LOG_ERR, "error setting index while loading " "tcpConnectionTable cache.\n");
            tcpConnectionTable_release_rowreq_ctx (rowreq_ctx);
        }
        else
        {
            snmp_log (LOG_ERR, "memory allocation failed while loading " "tcpConnectionTable cache.\n");
...
int tcpConnectionTable_container_load (netsnmp_container * container)
{
     ...
     */
    CONTAINER_FOR_EACH (raw_data, (netsnmp_container_obj_func *) _add_connection, container);

    /*
     * free the container. we've either claimed each entry, or released it,
     * so the dal function doesn't need to clear the container.
     */
    netsnmp_access_tcpconn_container_free (raw_data, NETSNMP_ACCESS_TCPCONN_FREE_DONT_CLEAR);
...
{
    DEBUGMSGTL (("verbose:tcpConnectionTable:tcpConnectionTable_row_prep", "called\n"));
