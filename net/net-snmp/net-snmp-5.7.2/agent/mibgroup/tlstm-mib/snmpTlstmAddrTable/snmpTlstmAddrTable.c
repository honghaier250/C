...
netsnmp_feature_require (table_tdata_extract_table) netsnmp_feature_require (table_tdata_remove_row)
#ifndef NETSNMP_NO_WRITE_SUPPORT
    netsnmp_feature_require (check_vb_storagetype)
netsnmp_feature_require (check_vb_type_and_max_size) netsnmp_feature_require (table_tdata_insert_row)
 ...
 ***********************************************************************/
     ...
     */
     ...
     netsnmp_tdata_row *tlstmAddrTable_createEntry (netsnmp_tdata * table_data,
                                                    char *snmpTargetAddrName, size_t snmpTargetAddrName_len);
     void tlstmAddrTable_removeEntry (netsnmp_tdata * table_data, netsnmp_tdata_row * row);
     ...
     void init_snmpTlstmAddrTable (void)
{
    ...
    DEBUGMSGTL (("tlstmAddrTable:init", "initializing table tlstmAddrTable\n"));

    reg =
        netsnmp_create_handler_registration ("tlstmAddrTable",
                                             tlstmAddrTable_handler, reg_oid, reg_oid_len, HANDLER_CAN_RWRITE);
    ...
    cache = netsnmp_cache_create (30, (NetsnmpCacheLoad *) _cache_load,
                                  (NetsnmpCacheFree *) _cache_free, reg_oid, reg_oid_len);
    if (NULL == cache)
    {
    ...
    reg = netsnmp_create_handler_registration ("snmpTlstmAddrCount",
                                               _count_handler, reg_oid, OID_LENGTH (reg_oid), HANDLER_CAN_RONLY);
    if (NULL == reg)
        snmp_log (LOG_ERR, "could not create handler for snmpTlstmAddrCount\n");
    else
    {
        netsnmp_register_scalar (reg);
        if (cache)
            netsnmp_inject_handler_before (reg, netsnmp_cache_handler_get (cache), "snmpTlstmAddrCount");
    }

    reg_oid[10] = 8;
    reg = netsnmp_create_handler_registration ("snmpTlstmAddrTableLastChanged", NULL, reg_oid,
                                               OID_LENGTH (reg_oid), HANDLER_CAN_RONLY);
    watcher = netsnmp_create_watcher_info ((void *) &_last_changed,
                                           sizeof (_last_changed), ASN_TIMETICKS, WATCHER_FIXED_SIZE);
    if ((NULL == reg) || (NULL == watcher))
        snmp_log (LOG_ERR, "could not create handler for snmpTlstmAddrTableLastChanged\n");
    else
                                               ...
                                               char *snmpTargetAddrName, size_t snmpTargetAddrName_len)
{
     ...
     * populate index
     */
    memcpy (entry->snmpTargetAddrName, snmpTargetAddrName, snmpTargetAddrName_len);
    entry->snmpTargetAddrName_len = snmpTargetAddrName_len;
    netsnmp_tdata_row_add_index (row, ASN_PRIV_IMPLIED_OCTET_STR, entry->snmpTargetAddrName, snmpTargetAddrName_len);
 ...
 */
static tlstmAddrTable_undo *_allocUndo (tlstmAddrTable_entry * entry)
{
    if (!entry)
        return NULL;
 ...
 * free undo resources 
 */
static void _freeUndo (tlstmAddrTable_entry * entry)
{
    if (!entry || !entry->undo)
        return;
                        ...
                        netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    DEBUGMSGTL (("tlstmAddrTable:handler", "Processing mode %s (%d) request\n",
                 se_find_label_in_slist ("agent_mode", reqinfo->mode), reqinfo->mode));
     ...
     */
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                table_entry = (tlstmAddrTable_entry *) netsnmp_tdata_extract_entry (request);
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_SNMPTLSTMADDRSERVERFINGERPRINT:
                        {
                            ...
                            if ((table_entry->hashType == 0) || (table_entry->tlstmAddrServerFingerprint_len == 0))
                                offset = 0;
                            else
                            {
                            ...
                            if (ret == SNMP_ERR_NOERROR)
                                snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR, bin, offset);
                        }
                    ...
                    case COLUMN_SNMPTLSTMADDRSTORAGETYPE:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, table_entry->tlstmAddrStorageType);
                        break;    /* case COLUMN_SNMPTLSTMADDRSTORAGETYPE */
                    case COLUMN_SNMPTLSTMADDRROWSTATUS:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, table_entry->tlstmAddrRowStatus);
     ...
     */
        case MODE_SET_RESERVE1:
            for (request = requests; request; request = request->next)
            {
                ...
                if ((NULL != table_entry) && (ST_READONLY == table_entry->tlstmAddrStorageType))
                {
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_SNMPTLSTMADDRSERVERFINGERPRINT:
                        ret = netsnmp_check_vb_type_and_max_size
                            (request->requestvb, ASN_OCTET_STR, sizeof (table_entry->tlstmAddrServerFingerprint));
                ...
                /** check len/algorithm MIB requirements */
                    ...
                    case COLUMN_SNMPTLSTMADDRSERVERIDENTITY:
                        ret = netsnmp_check_vb_type_and_max_size
                            (request->requestvb, ASN_OCTET_STR, sizeof (table_entry->tlstmAddrServerIdentity));
                        break;    /* case COLUMN_SNMPTLSTMADDRSERVERIDENTITY */
                    case COLUMN_SNMPTLSTMADDRSTORAGETYPE:
                        ret = netsnmp_check_vb_storagetype
                            (request->requestvb, (table_entry ? table_entry->tlstmAddrStorageType : ST_NONE));
                        break;    /* case COLUMN_SNMPTLSTMADDRSTORAGETYPE */
                    case COLUMN_SNMPTLSTMADDRROWSTATUS:
                        ret = netsnmp_check_vb_rowstatus_with_storagetype
                            (request->requestvb,
                             ...
                             (table_entry ? table_entry->tlstmAddrStorageType : ST_NONE));
                        break;    /* case COLUMN_SNMPTLSTMADDRROWSTATUS */
                    default:
                        ret = SNMP_ERR_NOTWRITABLE;
                }                /* switch colnum */
     ...
     */
        case MODE_SET_RESERVE2:
            for (request = requests; request; request = request->next)
            {
                ...
                if (!table_entry)
                {
                    table_row = tlstmAddrTable_createEntry
                        (table_data, (char *) table_info->indexes->val.string, table_info->indexes->val_len);
                    if (!table_row)
                    {
            ...
            /** don't allow multiple sets of same column */
                ...
                {
                    DEBUGMSGT (("tlstmAddrTable:reserve2", "multiple sets to col %d in request\n", table_info->colnum));
     ...
     */
        case MODE_SET_FREE:
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                table_data = netsnmp_tdata_extract_table (request);
                table_entry = (tlstmAddrTable_entry *) table_row ? table_row->data : NULL;
        ...
        case MODE_SET_ACTION:
                ...
                switch (table_info->colnum)
                {
                    ...
                    case COLUMN_SNMPTLSTMADDRSERVERIDENTITY:
                        memcpy (table_entry->undo->tlstmAddrServerIdentity,
                                table_entry->tlstmAddrServerIdentity, sizeof (table_entry->tlstmAddrServerIdentity));
                        table_entry->undo->tlstmAddrServerIdentity_len = table_entry->tlstmAddrServerIdentity_len;
                        memset (table_entry->tlstmAddrServerIdentity, 0, sizeof (table_entry->tlstmAddrServerIdentity));
                        memcpy (table_entry->tlstmAddrServerIdentity,
                                request->requestvb->val.string, request->requestvb->val_len);
                        table_entry->tlstmAddrServerIdentity_len = request->requestvb->val_len;
                        break;    /* case COLUMN_SNMPTLSTMADDRSERVERIDENTITY */
                    case COLUMN_SNMPTLSTMADDRSTORAGETYPE:
                        table_entry->undo->tlstmAddrStorageType = table_entry->tlstmAddrStorageType;
                        table_entry->tlstmAddrStorageType = *request->requestvb->val.integer;
                        break;    /* case COLUMN_SNMPTLSTMADDRSTORAGETYPE */
                    case COLUMN_SNMPTLSTMADDRROWSTATUS:
                        table_entry->undo->tlstmAddrRowStatus = table_entry->tlstmAddrRowStatus;
                        table_entry->tlstmAddrRowStatus = *request->requestvb->val.integer;
                        break;    /* case COLUMN_SNMPTLSTMADDRROWSTATUS */
                }                /* switch colnum */
                if (ret != SNMP_ERR_NOERROR)
                    break;
            ...
            if (ret == SNMP_ERR_NOERROR)
            {
                ...
                /** assume consistency */
                    ...
                    {
                        DEBUGMSGTL (("tlstmAddrTable", "fingerprint must not " "be empty for wildcard (*) identity\n"));
                    ...
                    else if (RS_IS_GOING_ACTIVE (table_entry->tlstmAddrRowStatus))
                    {
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_SNMPTLSTMADDRSERVERFINGERPRINT:
                        ...
                        memcpy (table_entry->tlstmAddrServerFingerprint,
                                ...
                                sizeof (table_entry->tlstmAddrServerFingerprint));
                        table_entry->tlstmAddrServerFingerprint_len = table_entry->undo->tlstmAddrServerFingerprint_len;
                    ...
                    case COLUMN_SNMPTLSTMADDRSERVERIDENTITY:
                        ...
                        table_entry->tlstmAddrServerIdentity_len = table_entry->undo->tlstmAddrServerIdentity_len;
                        break;    /* case COLUMN_SNMPTLSTMADDRSERVERIDENTITY */
                    case COLUMN_SNMPTLSTMADDRSTORAGETYPE:
                        /*
                         * restore tlstmAddrStorageType value 
                         */
                        table_entry->tlstmAddrStorageType = table_entry->undo->tlstmAddrStorageType;
                        break;    /* case COLUMN_SNMPTLSTMADDRSTORAGETYPE */
                    case COLUMN_SNMPTLSTMADDRROWSTATUS:
                        /*
                         * restore tlstmAddrRowStatus value 
                         */
                        table_entry->tlstmAddrRowStatus = table_entry->undo->tlstmAddrRowStatus;
                        break;    /* case COLUMN_SNMPTLSTMADDRROWSTATUS */
                }                /* switch colnum */
            }                    /* for requests */
     ...
     */
        case MODE_SET_COMMIT:
            for (request = requests; request; request = request->next)
            {
                ...
                if ((RS_NOTREADY == table_entry->tlstmAddrRowStatus) && table_entry->undo->is_consistent)
            ...
            /** release undo data for requests with no rowstatus */
                if (table_entry->undo && !table_entry->undo->req[COLUMN_SNMPTLSTMADDRROWSTATUS] != 0)
                {

                    _freeUndo (table_entry);

                /** update active addrs */
                    if ((0 == table_entry->addr_flags) && (table_entry->tlstmAddrRowStatus == RS_ACTIVE))
                        _addrs_add (table_entry);
                    else if ((0 != table_entry->addr_flags) && (table_entry->tlstmAddrRowStatus == RS_DESTROY))
                ...
                netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    snmp_set_var_typed_value (requests->requestvb, ASN_GAUGE, (u_char *) & val, sizeof (val));
...
static netsnmp_tdata_row *_entry_from_addr (snmpTlstmAddr * addr)
{
    netsnmp_tdata_row *row;
    ...
    if (addr->fingerprint)
    {
        ...
        if (entry->tlstmAddrServerFingerprint_len > sizeof (entry->tlstmAddrServerFingerprint))
            entry->tlstmAddrServerFingerprint_len = sizeof (entry->tlstmAddrServerFingerprint) - 1;
        memcpy (entry->tlstmAddrServerFingerprint, addr->fingerprint, entry->tlstmAddrServerFingerprint_len);
    ...
    if (addr->identity)
    {
        ...
        if (entry->tlstmAddrServerIdentity_len > sizeof (entry->tlstmAddrServerIdentity))
            entry->tlstmAddrServerIdentity_len = sizeof (entry->tlstmAddrServerIdentity) - 1;
        memcpy (entry->tlstmAddrServerIdentity, addr->identity, entry->tlstmAddrServerIdentity_len);
...
static int _cache_load (netsnmp_cache * cache, netsnmp_tdata * table)
{
    ...
    if (NULL == itr)
    {
        DEBUGMSGTL (("tlstmAddrTable:cache:load", "cant get iterator\n"));
        return -1;
    }
...
static void _cache_free (netsnmp_cache * cache, netsnmp_tdata * table)
{
    ...
    if (NULL == tbl_itr)
    {
        DEBUGMSGTL (("tlstmAddrTable:cache:free", "cant get entry iterator\n"));
        return;
    }
...
static int _tlstmAddrTable_save_rows (int majorID, int minorID, void *serverarg, void *clientarg);

static void _tlstmAddrTable_row_restore_mib (const char *token, char *buf);
...
static void _tlstmAddr_init_persistence (void)
{
    ...
    register_config_handler (NULL, mib_token, _tlstmAddrTable_row_restore_mib, NULL, NULL);
    rc = snmp_register_callback (SNMP_CALLBACK_LIBRARY,
                                 SNMP_CALLBACK_STORE_DATA, _tlstmAddrTable_save_rows, _table_data->container);

    if (rc != SNMP_ERR_NOERROR)
        snmp_log (LOG_ERR, "error registering for STORE_DATA callback " "in _tlstmAddrTable_init_persistence\n");
}

static int _save_entry (tlstmAddrTable_entry * entry, void *type)
{
     ...
     * build the line
     */
    netsnmp_assert (0 == entry->snmpTargetAddrName[entry->snmpTargetAddrName_len]);
    netsnmp_assert (0 == entry->tlstmAddrServerFingerprint[entry->tlstmAddrServerFingerprint_len]);
    snprintf (buf, sizeof (buf), "%s %s --%s %s %s %d", mib_token,
              entry->snmpTargetAddrName, hashType,
              entry->tlstmAddrServerFingerprint, entry->tlstmAddrServerIdentity, entry->tlstmAddrRowStatus);
    buf[sizeof (buf) - 1] = 0;
...
static int _save_addrs (snmpTlstmAddr * addrs, void *app_type)
{
    char buf[SNMP_MAXBUF_SMALL], *hashType;
...
static int _tlstmAddrTable_save_rows (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    if (NULL != active_addrs)
    {
        ...
        for (; addr; addr = ITERATOR_NEXT (addrs_itr))
        {
            ...
            if ((addr->flags & TLSTM_ADDR_FROM_CONFIG) || !(addr->flags & TLSTM_ADDR_NONVOLATILE))
                continue;
    ...
    else
    {
        ...
        for (; row; row = ITERATOR_NEXT (tbl_itr))
        {
             ...
             */
            if ((entry->tlstmAddrRowStatus == RS_ACTIVE) || (entry->tlstmAddrStorageType != ST_NONVOLATILE))
                continue;
...
static void _tlstmAddrTable_row_restore_mib (const char *token, char *buf)
{
    ...
    rc = netsnmp_tlstmAddr_restore_common (&buf, name, &name_len, id, &id_len, fingerprint, &fp_len, &hashType);
    if (rc < 0)
        return;
