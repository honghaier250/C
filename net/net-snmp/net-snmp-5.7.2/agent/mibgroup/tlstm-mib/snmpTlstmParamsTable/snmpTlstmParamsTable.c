...
netsnmp_feature_require (check_vb_rowstatus_with_storagetype) netsnmp_feature_require (table_tdata_insert_row)
     ...
     void init_snmpTlstmParamsTable (void)
{
    ...
    reg = netsnmp_create_handler_registration
        ("snmpTlstmParamsTable", snmpTlstmParamsTable_handler, reg_oid, reg_oid_len, HANDLER_CAN_RWRITE);
    ...
    cache = netsnmp_cache_create (SNMPTLSTMPARAMSTABLE_TIMEOUT,
                                  snmpTlstmParamsTable_load, snmpTlstmParamsTable_free, reg_oid, reg_oid_len);
    if (NULL == cache)
    {
    ...
    if (NULL == reg)
        snmp_log (LOG_ERR, "could not create handler for snmpTlstmParamsCount\n");
    else
    {
        netsnmp_register_scalar (reg);
        if (cache)
            netsnmp_inject_handler_before (reg, netsnmp_cache_handler_get (cache), "snmpTlstmParamsCount");
    }

    reg_oid[10] = 5;
    reg = netsnmp_create_handler_registration ("snmpTlstmParamsTableLastChanged", NULL, reg_oid,
    ...
    if ((NULL == reg) || (NULL == watcher))
        snmp_log (LOG_ERR, "could not create handler for snmpTlstmParamsTableLastChanged\n");
    else
...
netsnmp_tdata_row *snmpTlstmParamsTable_createEntry (netsnmp_tdata * table_data,
                                                     char *snmpTargetParamsName, size_t snmpTargetParamsName_len)
{
    snmpTlstmParamsTable_entry *entry;

    netsnmp_tdata_row *row;

    if ((NULL == snmpTargetParamsName) || (snmpTargetParamsName_len > sizeof (entry->snmpTargetParamsName)))
        return NULL;
    ...
    DEBUGIF ("snmpTlstmParamTable:entry:create")
    {
        ...
        DEBUGMSGT (("tlstmParamsTable:entry:create", "entry %s %p / row %p\n", name, entry, row));
    }

    /*
     * populate index
     */
    memcpy (entry->snmpTargetParamsName, snmpTargetParamsName, snmpTargetParamsName_len);
    entry->snmpTargetParamsName_len = snmpTargetParamsName_len;
    netsnmp_tdata_row_add_index (row, ASN_PRIV_IMPLIED_OCTET_STR,
                                 entry->snmpTargetParamsName, snmpTargetParamsName_len);
...
static snmpTlstmParamsTable_undo *_allocUndo (snmpTlstmParamsTable_entry * entry)
{
    if (!entry)
        return NULL;
...
/* free undo resources */
static void _freeUndo (snmpTlstmParamsTable_entry * entry)
{
    if (!entry || !entry->undo)
        return;
...
void snmpTlstmParamsTable_removeEntry (netsnmp_tdata * table_data, netsnmp_tdata_row * row)
{
    snmpTlstmParamsTable_entry *entry;
...
static void _params_tweak_storage (snmpTlstmParamsTable_entry * entry)
{
    ...
    if (NULL == params)
    {
        DEBUGMSGTL (("tlstmParamsTable:params:tweak", "couldn't find params!\n"));
        return;
    }
...
static netsnmp_tdata_row *_entry_from_params (snmpTlstmParams * params)
{
    netsnmp_tdata_row *row;

    snmpTlstmParamsTable_entry *entry;

    row = snmpTlstmParamsTable_createEntry (NULL, params->name, strlen (params->name));
    if (NULL == row)
    {
    ...
    {
        entry->snmpTlstmParamsClientFingerprint_len = strlen (params->fingerprint);
        if (entry->snmpTlstmParamsClientFingerprint_len > sizeof (entry->snmpTlstmParamsClientFingerprint))
            entry->snmpTlstmParamsClientFingerprint_len = sizeof (entry->snmpTlstmParamsClientFingerprint) - 1;
...
static int snmpTlstmParamsTable_load (netsnmp_cache * cache, void *vmagic)
{
    ...
    if (NULL == itr)
    {
        DEBUGMSGTL (("tlstmParamsTable:cache:load", "cant get iterator\n"));
        return -1;
    }
...
static void snmpTlstmParamsTable_free (netsnmp_cache * cache, void *vmagic)
{
    netsnmp_tdata *table = (netsnmp_tdata *) vmagic;
    ...
    if (NULL == tbl_itr)
    {
        DEBUGMSGTL (("tlstmParamsTable:cache:free", "cant get entry iterator\n"));
        return;
    }
     ...
     */
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                table_entry = (snmpTlstmParamsTable_entry *) netsnmp_tdata_extract_entry (request);
     ...
     */
        case MODE_SET_RESERVE1:
            for (request = requests; request; request = request->next)
            {
                ...
                if ((NULL != table_entry) && (ST_READONLY == table_entry->snmpTlstmParamsStorageType))
                {
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_SNMPTLSTMPARAMSCLIENTFINGERPRINT:
                        ret =
                            netsnmp_check_vb_type_and_max_size (request->requestvb, ASN_OCTET_STR,
                    ...
                    case COLUMN_SNMPTLSTMPARAMSSTORAGETYPE:
                        ret = netsnmp_check_vb_storagetype (request->requestvb,
                                                            (table_entry ? table_entry->
                                                             snmpTlstmParamsStorageType : ST_NONE));
                        break;    /* case COLUMN_SNMPTLSTMPARAMSSTORAGETYPE */

                    case COLUMN_SNMPTLSTMPARAMSROWSTATUS:
                        ret = netsnmp_check_vb_rowstatus_with_storagetype (request->requestvb,
                                                                           (table_entry ? table_entry->
                                                                            snmpTlstmParamsRowStatus : RS_NONEXISTENT),
                                                                           (table_entry ? table_entry->
                                                                            snmpTlstmParamsStorageType : ST_NONE));
                        break;    /* case COLUMN_SNMPTLSTMPARAMSROWSTATUS */
                    default:
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (table_info->colnum)
                {
                    ...
                    case COLUMN_SNMPTLSTMPARAMSSTORAGETYPE:
                        ...
                        table_entry->snmpTlstmParamsStorageType = table_entry->undo->snmpTlstmParamsStorageType;
                        break;    /* case COLUMN_SNMPTLSTMPARAMSSTORAGETYPE */
     ...
     */
        case MODE_SET_COMMIT:
            for (request = requests; request; request = request->next)
            {
                ...
                if ((RS_NOTREADY == table_entry->snmpTlstmParamsRowStatus) && table_entry->undo->is_consistent)
            ...
            /** release undo data for requests with no rowstatus */
                if (table_entry->undo && !table_entry->undo->req[COLUMN_SNMPTLSTMPARAMSROWSTATUS] != 0)
                {
                    _freeUndo (table_entry);

                /** update active addrs */
                    if ((0 == table_entry->params_flags) && (table_entry->snmpTlstmParamsRowStatus == RS_ACTIVE))
                        _params_add (table_entry);
                    else if ((0 != table_entry->params_flags) && (table_entry->snmpTlstmParamsRowStatus == RS_DESTROY))
...
static int _tlstmParamsTable_save (int majorID, int minorID, void *serverarg, void *clientarg);
 ...
 */
void _tlstmParams_init_persistence (void)
{
    int rc;

    register_config_handler (NULL, mib_token, _tlstmParamsTable_row_restore_mib, NULL, NULL);
    rc = snmp_register_callback (SNMP_CALLBACK_LIBRARY,
                                 SNMP_CALLBACK_STORE_DATA, _tlstmParamsTable_save, _table_data->container);

    if (rc != SNMP_ERR_NOERROR)
        snmp_log (LOG_ERR, "error registering for STORE_DATA callback " "in _tlstmParams_init_persistence\n");
}
...
static int _tlstmParamsTable_save (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    if ((CONTAINER_SIZE (active_params) == 0) && (CONTAINER_SIZE (_table_data->container) == 0))
    ...
    if (NULL != active_params)
    {
        ...
        if (NULL == params_itr)
        {
            DEBUGMSGTL (("tlstmParamsTable:save", "cant get params iterator\n"));
            params = NULL;
        }
        else
        ...
        for (; params; params = ITERATOR_NEXT (params_itr))
        {
            ...
            if ((params->flags & TLSTM_PARAMS_FROM_CONFIG) || !(params->flags & TLSTM_PARAMS_NONVOLATILE))
                continue;
    ...
    else
    {
        ...
        for (; row; row = ITERATOR_NEXT (tbl_itr))
        {
             ...
             */
            if ((entry->snmpTlstmParamsRowStatus == RS_ACTIVE) || (entry->snmpTlstmParamsStorageType != ST_NONVOLATILE))
                continue;
...
static int _save_entry (snmpTlstmParamsTable_entry * entry, void *type)
{
     ...
     * build the line
     */
    netsnmp_assert (0 == entry->snmpTargetParamsName[entry->snmpTargetParamsName_len]);
    netsnmp_assert (0 == entry->snmpTlstmParamsClientFingerprint[entry->snmpTlstmParamsClientFingerprint_len]);
    snprintf (buf, sizeof (buf), "%s %s %s %s %d", mib_token,
              entry->snmpTargetParamsName, hashType,
              entry->snmpTlstmParamsClientFingerprint, entry->snmpTlstmParamsRowStatus);
    buf[sizeof (buf) - 1] = 0;
...
static int _save_params (snmpTlstmParams * params, void *app_type)
{
    ...
    snprintf (buf, sizeof (buf), "%s %s --%s %s %d", mib_token, params->name, hashType, params->fingerprint, RS_ACTIVE);
...
static void _tlstmParamsTable_row_restore_mib (const char *token, char *buf)
{
    ...
    else
    {
        ...
        row = snmpTlstmParamsTable_createEntry (_table_data, params->name, strlen (params->name));
        if (!row)
            return;
        ...
        entry->snmpTlstmParamsClientFingerprint_len = strlen (entry->snmpTlstmParamsClientFingerprint);
        entry->snmpTlstmParamsStorageType = ST_NONVOLATILE;
