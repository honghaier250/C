...
netsnmp_feature_require (table_tdata_remove_row) netsnmp_feature_require (tls_fingerprint_build)
...
void init_snmpTlstmCertToTSNTable_context (const char *contextName)
{
    ...
    const char *mib_map_help = MAP_MIB_CONFIG_TOKEN " table persistence (internal use)";

    DEBUGMSGTL (("tlstmCertToSN:init", "initializing table tlstmCertToTSNTable\n"));

    reg = netsnmp_create_handler_registration
        ("tlstmCertToTSNTable", tlstmCertToTSNTable_handler, reg_oid, reg_oid_len, HANDLER_CAN_RWRITE);
    if (NULL == reg)
    {
        snmp_log (LOG_ERR, "error creating handler registration for tlstmCertToSN\n");
        return;
    }
    ...
    if (NULL == _table)
    {
        snmp_log (LOG_ERR, "error creating tdata table for tlstmCertToTSNTable\n");
        return;
    }
    info = SNMP_MALLOC_TYPEDEF (netsnmp_table_registration_info);
    if (NULL == info)
    {
        snmp_log (LOG_ERR, "error creating table info for tlstmCertToTSNTable\n");
    ...
    if (NULL == reg)
        snmp_log (LOG_ERR, "could not create handler for snmpTlstmCertToTSNCount\n");
    else
    {
        if (NULL != contextName)
            reg->contextName = strdup (contextName);
    ...
    reg_oid[10] = 2;
    reg = netsnmp_create_handler_registration ("snmpTlstmCertToTSNTableLastChanged", NULL, reg_oid,
     ...
     * persistence
     */
    register_config_handler (NULL, MAP_MIB_CONFIG_TOKEN, _parse_mib_maps, NULL, mib_map_help);
    if (snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, _save_maps, NULL) != SNMP_ERR_NOERROR)
        snmp_log (LOG_ERR, "error registering for STORE_DATA callback " "for certToTSN\n");
 ...
 */
netsnmp_tdata_row *tlstmCertToTSNTable_createEntry (netsnmp_tdata * table, u_long tlstmCertToTSNID)
{
    ...
    netsnmp_tdata_row_add_index (row, ASN_UNSIGNED, &(entry->tlstmCertToTSNID), sizeof (entry->tlstmCertToTSNID));
    /*
 ...
 */
static certToTSN_undo *_allocUndo (certToTSN_entry * entry)
{
    if (!entry)
        return NULL;
 ...
 * free undo resources 
 */
static void _freeUndo (certToTSN_entry * entry)
{
    if (!entry || !entry->undo)
        return;
 ...
 */
void tlstmCertToTSNTable_removeEntry (netsnmp_tdata * table, netsnmp_tdata_row * row)
{
    certToTSN_entry *entry;
                    ...
                    case COL_SNMPTLSTMCERTTOTSN_FINGERPRINT:
                        {
                            ...
                            rc = netsnmp_tls_fingerprint_build (entry->hashType, entry->fingerprint, &ptr, &len, 0);
                            if (SNMPERR_SUCCESS != rc)
                                netsnmp_set_request_error (reqinfo, request, SNMP_ERR_GENERR);
                            else
                                snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR, bin, len);
                        }
                        break;    /* case COL_SNMPTLSTMCERTTOTSN_FINGERPRINT */
                    case COL_SNMPTLSTMCERTTOTSN_MAPTYPE:
                        tsnm[tsnm_pos] = entry->mapType;
                        snmp_set_var_typed_value (request->requestvb, ASN_OBJECT_ID, tsnm, sizeof (tsnm));
                        break;    /* case COL_SNMPTLSTMCERTTOTSN_MAPTYPE */
                    case COL_SNMPTLSTMCERTTOTSN_DATA:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR, entry->data, entry->data_len);
                        break;    /* case COL_SNMPTLSTMCERTTOTSN_DATA */
                    case COL_SNMPTLSTMCERTTOTSN_STORAGETYPE:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, entry->storageType);
                        break;    /* case COL_SNMPTLSTMCERTTOTSN_STORAGETYPE */
                    case COL_SNMPTLSTMCERTTOTSN_ROWSTATUS:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, entry->rowStatus);
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (info->colnum)
                {
                    case COL_SNMPTLSTMCERTTOTSN_FINGERPRINT:
                        ret = netsnmp_check_vb_type_and_max_size
                            (request->requestvb, ASN_OCTET_STR, sizeof (entry->fingerprint));
                ...
                /** check len/algorithm MIB requirements */
                    ...
                    case COL_SNMPTLSTMCERTTOTSN_MAPTYPE:
                        ...
                        if (ret == SNMP_ERR_NOERROR)
                        {
                            if (_oid2type (request->requestvb->val.objid,
                                           request->requestvb->val_len) > TSNM_tlstmCert_MAX)
                    ...
                    case COL_SNMPTLSTMCERTTOTSN_DATA:
                        ...
                        break;    /* case COL_SNMPTLSTMCERTTOTSN_DATA */
                    case COL_SNMPTLSTMCERTTOTSN_STORAGETYPE:
                        ret = netsnmp_check_vb_storagetype (request->requestvb, (entry ? entry->storageType : ST_NONE));
                        break;    /* case COL_SNMPTLSTMCERTTOTSN_STORAGETYPE */
                    case COL_SNMPTLSTMCERTTOTSN_ROWSTATUS:
                        ret = netsnmp_check_vb_rowstatus_with_storagetype
                            (request->requestvb,
                             (entry ? entry->rowStatus : RS_NONEXISTENT), (entry ? entry->storageType : ST_NONE));
                        break;    /* case COL_SNMPTLSTMCERTTOTSN_ROWSTATUS */
                    default:
                        ret = SNMP_ERR_NOTWRITABLE;
                }                /* switch colnum */
     ...
     */
        case MODE_SET_RESERVE2:
            for (request = requests; request; request = request->next)
            {
                ...
                if (!entry)
                {
                    row = tlstmCertToTSNTable_createEntry (table, *info->indexes->val.integer);
                    if (!row)
                    {
            ...
            /** allocate undo structure, if needed */
                ...
                {
                    DEBUGMSGT (("tlstmCertToSN:reserve2", "multiple sets to col %d in request\n", info->colnum));
     ...
     */
        case MODE_SET_ACTION:
            ...
            /** reserve2 should enforce this */
                ...
                switch (info->colnum)
                {
                    case COL_SNMPTLSTMCERTTOTSN_FINGERPRINT:
                        {
                            ...
                            memcpy (entry->undo->fingerprint, entry->fingerprint, sizeof (entry->fingerprint));
                            ...
                            (void) netsnmp_tls_fingerprint_parse (vb->val.string, vb->val_len,
                                                                  (char **) &tmp, &size, 0, &entry->hashType);
                    ...
                    case COL_SNMPTLSTMCERTTOTSN_MAPTYPE:
                        ...
                        entry->mapType = _oid2type (request->requestvb->val.objid, request->requestvb->val_len);
                        break;    /* case COL_SNMPTLSTMCERTTOTSN_MAPTYPE */
                    case COL_SNMPTLSTMCERTTOTSN_DATA:
                        memcpy (entry->undo->data, entry->data, sizeof (entry->data));
                        entry->undo->data_len = entry->data_len;
                        memset (entry->data, 0, sizeof (entry->data));
                        memcpy (entry->data, request->requestvb->val.string, request->requestvb->val_len);
            ...
            /** assume consistency */
                ...
                if (0 == entry->fingerprint_len)
                {
                    DEBUGMSGTL (("tlstmCertToTSNTable:inconsistent", "fingerprint must not be empty\n"));
                    entry->undo->is_consistent = 0;
                }
                else if ((TSNM_tlstmCertSpecified == entry->mapType) && (0 == entry->data_len))
                {
                    DEBUGMSGTL (("tlstmCertToTSNTable:inconsistent", "must specify Data for CertSpecified identity\n"));
                    entry->undo->is_consistent = 0;
                }

                if ((RS_IS_ACTIVE (entry->rowStatus)) &&
                    ((!entry->undo->req[COL_SNMPTLSTMCERTTOTSN_ROWSTATUS]) || (RS_IS_ACTIVE (entry->undo->rowStatus))))
                {
                    /*
                ...
                else if (RS_DESTROY == entry->rowStatus)
                {
                    ...
                    {
                        DEBUGMSGTL (("tlstmCertToTSNTable:inconsistent", "can't destroy active row\n"));
                        netsnmp_assert (entry->undo->req[COL_SNMPTLSTMCERTTOTSN_ROWSTATUS]);
                        ret = SNMP_ERR_INCONSISTENTVALUE;
                        request = entry->undo->req[COL_SNMPTLSTMCERTTOTSN_ROWSTATUS];
                    }
     ...
     */
        case MODE_SET_UNDO:
            for (request = requests; request; request = request->next)
            {
                ...
                {
                    case COL_SNMPTLSTMCERTTOTSN_FINGERPRINT:
                        memcpy (entry->fingerprint, entry->undo->fingerprint, sizeof (entry->fingerprint));
     ...
     */
        case MODE_SET_COMMIT:
            for (request = requests; request; request = request->next)
            {
                ...
                else if ((RS_NOTINSERVICE == entry->rowStatus) && (0 == entry->undo->is_consistent))
                    entry->rowStatus = RS_NOTREADY;
            ...
            /** release undo data for requests with no rowstatus */
                if (entry->undo && !entry->undo->req[COL_SNMPTLSTMCERTTOTSN_ROWSTATUS])
                {
                    ...
                    if ((0 == entry->map_flags) && (entry->rowStatus == RS_ACTIVE))
                        _cert_map_add (entry);
                    else if ((0 != entry->map_flags) && (entry->rowStatus == RS_DESTROY))
...
static netsnmp_tdata_row *_entry_from_map (netsnmp_cert_map * map)
{
    netsnmp_tdata_row *row;
...
static int _cache_load (netsnmp_cache * cache, netsnmp_tdata * table)
{
    ...
    if (NULL == map_itr)
    {
        DEBUGMSGTL (("tlstmCertToTSNTable:cache:load", "cant get map iterator\n"));
        return -1;
    }
...
static void _cache_free (netsnmp_cache * cache, netsnmp_tdata * table)
{
    ...
    if (NULL == tbl_itr)
    {
        DEBUGMSGTL (("tlstmCertToTSNTable:cache:free", "cant get map iterator\n"));
        return;
    }
...
static int _save_entry (certToTSN_entry * entry, void *app_type)
{
...
static int _save_map (netsnmp_cert_map * map, int row_status, void *app_type)
{
    char buf[SNMP_MAXBUF_SMALL], *hashType, *mapType, *data = NULL;
    ...
    {
        DEBUGMSGT (("tlstmCertToTSNTable:save", "skipping RO/permanent/volatile row\n"));
        return SNMP_ERR_NOERROR;
    }
    ...
    snprintf (buf, sizeof (buf), "%s %d --%s %s --%s %s %d",
              MAP_MIB_CONFIG_TOKEN, map->priority, hashType, map->fingerprint, mapType, data ? data : "", row_status);
...
static int _save_maps (int majorID, int minorID, void *serverarg, void *clientarg)
{
    char sep[] = "\n##############################################################";
    ...
    if ((NULL == maps) || ((CONTAINER_SIZE (maps) == 0) && (CONTAINER_SIZE (_table->container) == 0)))
    ...
    else
    {
        ...
        for (; row; row = ITERATOR_NEXT (tbl_itr))
        {
             ...
             */
            if ((entry->rowStatus == RS_ACTIVE) || (entry->storageType != ST_NONVOLATILE))
                continue;
...
static oid _oid2type (oid * val, int val_len)
{
    ...
    if ((val[OID_LENGTH (_tsnm_base)] > TSNM_tlstmCert_MAX) || (0 == val[OID_LENGTH (_tsnm_base)]))
        return -3;
