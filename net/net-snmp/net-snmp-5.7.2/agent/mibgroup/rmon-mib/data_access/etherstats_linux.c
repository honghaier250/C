...
struct ifname *etherstats_interface_name_list_get (struct ifname *list_head, int *retval)
{
    struct ifaddrs *addrs = NULL, *p = NULL;

    struct ifname *nameptr1 = NULL, *nameptr2 = NULL;

    DEBUGMSGTL (("access:etherStatsTable:interface_name_list_get", "called\n"));

    if ((getifaddrs (&addrs)) < 0)
    {
        DEBUGMSGTL (("access:etherStatsTable:interface_name_list_get", "getifaddrs failed\n"));
    ...
    for (p = addrs; p; p = p->ifa_next)
    {
        ...
        if (!list_head)
        {
            ...
            {
                DEBUGMSGTL (("access:etherStatsTable:interface_name_list_get", "memory allocation failed\n"));
...
int etherstats_interface_name_list_free (struct ifname *list_head)
{
    ...
    DEBUGMSGTL (("access:etherStatsTable:interface_name_list_free", "called\n"));

    if (!list_head)
    {
        snmp_log (LOG_ERR, "access:etherStatsTable:interface_name_list_free: invalid pointer list_head");
        DEBUGMSGTL (("access:etherStatsTable:interface_name_list_free", "invalid pointer list_head\n"));
        return -1;
    }
...
int etherstats_interface_ioctl_ifindex_get (int fd, const char *name)
{
    ...
    if (rc < 0)
    {
        DEBUGMSGTL (("access:etherStats:ioctl", "ifindex_get error on inerface '%s'\n", name));
...
int interface_ioctl_etherstats_get (etherStatsTable_rowreq_ctx * rowreq_ctx, int fd, const char *name)
{
    ...
    DEBUGMSGTL (("access:etherStatsTable:interface_ioctl_etherstats_get", "called\n"));
    ...
    if (!eth_strings)
    {
        DEBUGMSGTL (("access:etherStatsTable:interface_ioctl_etherstats_get", "no memory available\n"));
...
int _etherStats_ioctl_get (int fd, int which, struct ifreq *ifrq, const char *name)
{
    ...
    if (NULL == name)
    {
        DEBUGMSGTL (("access:etherStatsTable:ioctl", "_etherStats_ioctl_get invalid ifname '%s'\n", name));
    ...
    if (fd < 0)
    {
        ...
        if (ourfd < 0)
        {
            DEBUGMSGTL (("access:etherStatsTable:ioctl", "_etherStats_ioctl_get couldn't create a socket\n"));
            snmp_log (LOG_ERR, "access:etherStatsTable:ioctl, _etherStats_ioctl_get error on inerface '%s'\n", name);
    ...
    if (rc < 0)
    {
        DEBUGMSGTL (("access:etherStatsTable:ioctl", "_etherStats_ioctl_get ioctl %d returned %d\n", which, rc));
        rc = -3;
    }
