 ...
 */
int etherStatsTable_init_data (etherStatsTable_registration * etherStatsTable_reg)
{
    DEBUGMSGTL (("verbose:etherStatsTable:etherStatsTable_init_data", "called\n"));
 ...
 */
void etherStatsTable_container_init (netsnmp_container ** container_ptr_ptr, netsnmp_cache * cache)
{
    DEBUGMSGTL (("verbose:etherStatsTable:etherStatsTable_container_init", "called\n"));

    if (NULL == container_ptr_ptr)
    {
        snmp_log (LOG_ERR, "bad container param to etherStatsTable_container_init\n");
        return;
    }
    ...
    *container_ptr_ptr = NULL;
    ...
    if (NULL == cache)
    {
        snmp_log (LOG_ERR, "bad cache param to etherStatsTable_container_init\n");
        return;
    }
 ...
 */
void etherStatsTable_container_shutdown (netsnmp_container * container_ptr)
{
    DEBUGMSGTL (("verbose:etherStatsTable:etherStatsTable_container_shutdown", "called\n"));

    if (NULL == container_ptr)
    {
        snmp_log (LOG_ERR, "bad params to etherStatsTable_container_shutdown\n");
        return;
    }
 ...
 *
 */
int etherStatsTable_container_load (netsnmp_container * container)
{
    etherStatsTable_rowreq_ctx *rowreq_ctx;

    size_t count = 0;

    DEBUGMSGTL (("verbose:etherStatsTable:etherStatsTable_container_load", "called\n"));
    ...
    for (p = list_head; p; p = p->ifn_next)
    {
        ...
        if (MFD_SUCCESS != etherStatsTable_indexes_set (rowreq_ctx, etherStatsIndex))
        {
            snmp_log (LOG_ERR, "error setting index while loading " "etherStatsTable data.\n");
 ...
 * @retval MFD_ERROR       : other error.
 */
int etherStatsTable_row_prep (etherStatsTable_rowreq_ctx * rowreq_ctx)
{
...
etherStatsTable_validate_index (etherStatsTable_registration *
                                etherStatsTable_reg, etherStatsTable_rowreq_ctx * rowreq_ctx)
{
    int rc = MFD_SUCCESS;

    DEBUGMSGTL (("verbose:etherStatsTable:etherStatsTable_validate_index", "called\n"));
    ...
    if (1)
    {
        snmp_log (LOG_WARNING, "invalid index for a new row in the " "etherStatsTable table.\n");
        /*
