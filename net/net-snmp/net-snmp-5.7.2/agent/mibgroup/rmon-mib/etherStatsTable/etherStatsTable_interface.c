...
netsnmp_feature_require (table_container_row_insert) netsnmp_feature_require (check_all_requests_error)
 ...
 */
     ...
     static void _etherStatsTable_container_init (etherStatsTable_interface_ctx * if_ctx);

     static void _etherStatsTable_container_shutdown (etherStatsTable_interface_ctx * if_ctx);
...
etherStatsTable_registration *etherStatsTable_registration_set (etherStatsTable_registration * newreg)
{
...
_etherStatsTable_undo_column (etherStatsTable_rowreq_ctx * rowreq_ctx, netsnmp_variable_list * var, int column);

NETSNMP_STATIC_INLINE int _etherStatsTable_check_indexes (etherStatsTable_rowreq_ctx * rowreq_ctx);
 ...
 */
void _etherStatsTable_initialize_interface (etherStatsTable_registration * reg_ptr, u_long flags)
{
    netsnmp_baby_steps_access_methods *access_multiplexer = &etherStatsTable_if_ctx.access_multiplexer;

    netsnmp_table_registration_info *tbl_info = &etherStatsTable_if_ctx.tbl_info;
    ...
    {
        snmp_log (LOG_ERR, "could not initialize container for etherStatsTable\n");
        return;
    }
     ...
     */
    access_multiplexer->object_syntax_checks = _mfd_etherStatsTable_check_objects;
    ...
    access_multiplexer->irreversible_commit = _mfd_etherStatsTable_irreversible_commit;

    /*************************************************
     *
     * Create a registration, save our reg data, register table.
     */
    DEBUGMSGTL (("etherStatsTable:init_etherStatsTable", "Registering etherStatsTable as a mibs-for-dummies table.\n"));
    handler = netsnmp_baby_steps_access_multiplexer_get (access_multiplexer);
    reginfo =
        netsnmp_handler_registration_create ("etherStatsTable", handler,
                                             etherStatsTable_oid,
                                             etherStatsTable_oid_size, HANDLER_CAN_BABY_STEP | HANDLER_CAN_RWRITE);
    if (NULL == reginfo)
    {
    ...
    handler =
        netsnmp_container_table_handler_get (tbl_info,
                                             etherStatsTable_if_ctx.container, TABLE_CONTAINER_KEY_NETSNMP_INDEX);
    netsnmp_inject_handler (reginfo, handler);
 ...
 */
void _etherStatsTable_shutdown_interface (etherStatsTable_registration * reg_ptr)
{
    /*
...
void etherStatsTable_valid_columns_set (netsnmp_column_info * vc)
{
 ...
 */
int etherStatsTable_index_to_oid (netsnmp_index * oid_idx, etherStatsTable_mib_index * mib_idx)
{
    ...
    DEBUGMSGTL (("verbose:etherStatsTable:etherStatsTable_index_to_oid", "called\n"));

    /*
     * etherStatsIndex(1)/INTEGER32/ASN_INTEGER/long(long)//l/A/w/e/R/d/h 
     */
    snmp_set_var_value (&var_etherStatsIndex, (u_char *) & mib_idx->etherStatsIndex, sizeof (mib_idx->etherStatsIndex));


    err = build_oid_noalloc (oid_idx->oids, oid_idx->len, &oid_idx->len, NULL, 0, &var_etherStatsIndex);
    if (err)
 ...
 */
int etherStatsTable_index_from_oid (netsnmp_index * oid_idx, etherStatsTable_mib_index * mib_idx)
{
    int err = SNMP_ERR_NOERROR;
    ...
    DEBUGMSGTL (("verbose:etherStatsTable:etherStatsTable_index_from_oid", "called\n"));

    /*
     * parse the oid into the individual index components
     */
    err = parse_oid_indexes (oid_idx->oids, oid_idx->len, &var_etherStatsIndex);
    if (err == SNMP_ERR_NOERROR)
    {
        /*
         * copy out values
         */
        mib_idx->etherStatsIndex = *((long *) var_etherStatsIndex.val.string);
...
etherStatsTable_data *etherStatsTable_allocate_data (void)
{
    ...
    DEBUGMSGTL (("verbose:etherStatsTable:etherStatsTable_allocate_data", "called\n"));

    if (NULL == rtn)
    {
        snmp_log (LOG_ERR, "unable to malloc memory for new " "etherStatsTable_data.\n");
    }
 ...
 */
void etherStatsTable_release_data (etherStatsTable_data * data)
{
    DEBUGMSGTL (("verbose:etherStatsTable:etherStatsTable_release_data", "called\n"));
 ...
 */
etherStatsTable_rowreq_ctx *etherStatsTable_allocate_rowreq_ctx (void *user_init_ctx)
{
    etherStatsTable_rowreq_ctx *rowreq_ctx = SNMP_MALLOC_TYPEDEF (etherStatsTable_rowreq_ctx);

    DEBUGMSGTL (("internal:etherStatsTable:etherStatsTable_allocate_rowreq_ctx", "called\n"));

    if (NULL == rowreq_ctx)
    {
        snmp_log (LOG_ERR, "Couldn't allocate memory for a " "etherStatsTable_rowreq_ctx.\n");
        return NULL;
    }
    ...
    {
        if (SNMPERR_SUCCESS != etherStatsTable_rowreq_ctx_init (rowreq_ctx, user_init_ctx))
        {
 ...
 */
void etherStatsTable_release_rowreq_ctx (etherStatsTable_rowreq_ctx * rowreq_ctx)
{
                                  ...
                                  netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    {
        DEBUGMSGTL (("internal:etherStatsTable", "skipping additional pre_request\n"));
    ...
    if (MFD_SUCCESS != rc)
    {
         ...
         */
        DEBUGMSGTL (("etherStatsTable", "error %d from " "etherStatsTable_pre_request\n", rc));
...
_mfd_etherStatsTable_post_request (netsnmp_mib_handler * handler,
                                   ...
                                   netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    etherStatsTable_rowreq_ctx *rowreq_ctx = netsnmp_container_table_row_extract (requests);
    ...
    {
        DEBUGMSGTL (("internal:etherStatsTable", "waiting for last post_request\n"));
        return SNMP_ERR_NOERROR;
    }
    ...
    if ((MFD_SUCCESS != packet_rc) && etherStatsTable_dirty_get ())
    {
         ...
         * the dirty flags.
         */
        snmp_log (LOG_WARNING, "etherStatsTable dirty flag set in post_request " "but status != SUCCESS.\n");
    }

    rc = etherStatsTable_post_request (etherStatsTable_if_ctx.user_ctx, packet_rc);
    if (MFD_SUCCESS != rc)
    {
        /*
         * nothing we can do about it but log it
         */
        DEBUGMSGTL (("etherStatsTable", "error %d from " "etherStatsTable_post_request\n", rc));
    }
 ...
 * wrapper
 */
static etherStatsTable_rowreq_ctx *_mfd_etherStatsTable_rowreq_from_index (netsnmp_index * oid_idx, int *rc_ptr)
{
    ...
    *rc_ptr = _etherStatsTable_check_indexes (rowreq_ctx);
    if (MFD_SUCCESS != *rc_ptr)
    {
        netsnmp_assert ((*rc_ptr == SNMP_ERR_NOCREATION) || (*rc_ptr == SNMP_ERR_INCONSISTENTNAME));
        etherStatsTable_release_rowreq_ctx (rowreq_ctx);
        return NULL;
    }
 ...
 */
NETSNMP_STATIC_INLINE int
_etherStatsTable_get_column (etherStatsTable_rowreq_ctx * rowreq_ctx, netsnmp_variable_list * var, int column)
{
    int rc = SNMPERR_SUCCESS;

    DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column", "called for %d\n", column));
    ...
    switch (column)
    {
        ...
        case COLUMN_ETHERSTATSDATASOURCE:
            if (!(COLUMN_ETHERSTATSDATASOURCE_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsDataSource) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->type = ASN_OBJECT_ID;
            rc = etherStatsDataSource_get (rowreq_ctx, (oid **) & var->val.string, &var->val_len);
            break;
        ...
        case COLUMN_ETHERSTATSDROPEVENTS:
            if (!(COLUMN_ETHERSTATSDROPEVENTS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsDropEvents) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsDropEvents_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSOCTETS:
            if (!(COLUMN_ETHERSTATSOCTETS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsOctets) doesn't exist\n", column));
                return MFD_SKIP;
            }
        ...
        case COLUMN_ETHERSTATSPKTS:
            if (!(COLUMN_ETHERSTATSPKTS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsPkts) doesn't exist\n", column));
                return MFD_SKIP;
            }
        ...
        case COLUMN_ETHERSTATSBROADCASTPKTS:
            if (!(COLUMN_ETHERSTATSBROADCASTPKTS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsBroadcastPkts) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsBroadcastPkts_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSMULTICASTPKTS:
            if (!(COLUMN_ETHERSTATSMULTICASTPKTS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsMulticastPkts) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsMulticastPkts_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSCRCALIGNERRORS:
            if (!(COLUMN_ETHERSTATSCRCALIGNERRORS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsCRCAlignErrors) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsCRCAlignErrors_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSUNDERSIZEPKTS:
            ...
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsUndersizePkts) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsUndersizePkts_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSOVERSIZEPKTS:
            if (!(COLUMN_ETHERSTATSOVERSIZEPKTS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsOversizePkts) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsOversizePkts_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSFRAGMENTS:
            if (!(COLUMN_ETHERSTATSFRAGMENTS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsFragments) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsFragments_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSJABBERS:
            if (!(COLUMN_ETHERSTATSJABBERS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsJabbers) doesn't exist\n", column));
                return MFD_SKIP;
            }
        ...
        case COLUMN_ETHERSTATSCOLLISIONS:
            if (!(COLUMN_ETHERSTATSCOLLISIONS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsCollisions) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsCollisions_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSPKTS64OCTETS:
            if (!(COLUMN_ETHERSTATSPKTS64OCTETS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsPkts64Octets) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsPkts64Octets_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSPKTS65TO127OCTETS:
            if (!(COLUMN_ETHERSTATSPKTS65TO127OCTETS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsPkts65to127Octets) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsPkts65to127Octets_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSPKTS128TO255OCTETS:
            if (!(COLUMN_ETHERSTATSPKTS128TO255OCTETS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsPkts128to255Octets) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsPkts128to255Octets_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSPKTS256TO511OCTETS:
            if (!(COLUMN_ETHERSTATSPKTS256TO511OCTETS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsPkts256to511Octets) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsPkts256to511Octets_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSPKTS512TO1023OCTETS:
            if (!(COLUMN_ETHERSTATSPKTS512TO1023OCTETS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsPkts512to1023Octets) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsPkts512to1023Octets_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSPKTS1024TO1518OCTETS:
            if (!(COLUMN_ETHERSTATSPKTS1024TO1518OCTETS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsPkts1024to1518Octets) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->val_len = sizeof (u_long);
            var->type = ASN_COUNTER;
            rc = etherStatsPkts1024to1518Octets_get (rowreq_ctx, (u_long *) var->val.string);
            break;
        ...
        case COLUMN_ETHERSTATSOWNER:
            if (!(COLUMN_ETHERSTATSOWNER_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsOwner) doesn't exist\n", column));
                return MFD_SKIP;
            }

            var->type = ASN_OCTET_STR;
            rc = etherStatsOwner_get (rowreq_ctx, (char **) &var->val.string, &var->val_len);
            break;
        ...
        case COLUMN_ETHERSTATSSTATUS:
            if (!(COLUMN_ETHERSTATSSTATUS_FLAG & rowreq_ctx->column_exists_flags))
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
                             "column %d (etherStatsStatus) doesn't exist\n", column));
                return MFD_SKIP;
            }
        ...
        default:
            if (ETHERSTATSTABLE_MIN_COL <= column && column <= ETHERSTATSTABLE_MAX_COL)
            {
                DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_column",
            ...
            {
                snmp_log (LOG_ERR, "unknown column %d in _etherStatsTable_get_column.\n", column);
            }
            break;
    }
                                 ...
                                 netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_get_values", "called\n"));

    netsnmp_assert (NULL != rowreq_ctx);

    DEBUGMSGTL (("9:etherStatsTable:_mfd_etherStatsTable_get_values", "exists %u\n", rowreq_ctx->column_exists_flags));
    ...
    for (; requests; requests = requests->next)
    {
        ...
        rc = _etherStatsTable_get_column (rowreq_ctx, requests->requestvb, tri->colnum);
        if (rc)
        {
            if (MFD_SKIP == rc)
            {
...
NETSNMP_STATIC_INLINE int _etherStatsTable_check_indexes (etherStatsTable_rowreq_ctx * rowreq_ctx)
{
    ...
    DEBUGMSGTL (("internal:etherStatsTable:_etherStatsTable_check_indexes", "called\n"));
    ...
    if ((SNMPERR_SUCCESS == rc)
        && ((rowreq_ctx->tbl_idx.etherStatsIndex < 1) || (rowreq_ctx->tbl_idx.etherStatsIndex > 65535)))
    {
     ...
     */
    return etherStatsTable_validate_index (etherStatsTable_if_ctx.user_ctx, rowreq_ctx);
 ...
 */
NETSNMP_STATIC_INLINE int
_etherStatsTable_check_column (etherStatsTable_rowreq_ctx * rowreq_ctx, netsnmp_variable_list * var, int column)
{
    int rc = SNMPERR_SUCCESS;

    DEBUGMSGTL (("internal:etherStatsTable:_etherStatsTable_check_column", "called for %d\n", column));
    ...
    switch (column)
    {
        ...
        case COLUMN_ETHERSTATSDATASOURCE:
            rc = netsnmp_check_vb_type_and_max_size (var, ASN_OBJECT_ID,
                                                     sizeof (rowreq_ctx->data.etherStatsDataSource));
            if (SNMPERR_SUCCESS != rc)
            {
                DEBUGMSGTL (("etherStatsTable:_etherStatsTable_check_column:etherStatsDataSource",
                             "varbind validation failed (eg bad type or size)\n"));
            }
            else
            {
                rc = etherStatsDataSource_check_value (rowreq_ctx, (oid *) var->val.string, var->val_len);
                if ((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) && (MFD_NOT_VALID_NOW != rc))
                {
                    snmp_log (LOG_ERR, "bad rc %d from etherStatsDataSource_check_value\n", rc);
                    rc = SNMP_ERR_GENERR;
                }
            }
            break;
        ...
        case COLUMN_ETHERSTATSPKTS1024TO1518OCTETS:
             ...
             */
        case COLUMN_ETHERSTATSOWNER:
            rc = netsnmp_check_vb_type_and_max_size (var, ASN_OCTET_STR, sizeof (rowreq_ctx->data.etherStatsOwner));
            /*
             * check defined range(s). 
             */
            if ((SNMPERR_SUCCESS == rc) && ((var->val_len < 0) || (var->val_len > 127)))
            {
                rc = SNMP_ERR_WRONGLENGTH;
            }
            if (SNMPERR_SUCCESS != rc)
            {
                DEBUGMSGTL (("etherStatsTable:_etherStatsTable_check_column:etherStatsOwner",
                             "varbind validation failed (eg bad type or size)\n"));
            }
            else
            {
                rc = etherStatsOwner_check_value (rowreq_ctx, (char *) var->val.string, var->val_len);
                if ((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) && (MFD_NOT_VALID_NOW != rc))
                {
                    snmp_log (LOG_ERR, "bad rc %d from etherStatsOwner_check_value\n", rc);
                    rc = SNMP_ERR_GENERR;
                }
            }
            break;
             ...
             */
        case COLUMN_ETHERSTATSSTATUS:
            rc = netsnmp_check_vb_type_and_size (var, ASN_INTEGER, sizeof (rowreq_ctx->data.etherStatsStatus));
            /*
            ...
            if ((SNMPERR_SUCCESS == rc)
                ...
                && (*var->val.integer != ENTRYSTATUS_UNDERCREATION) && (*var->val.integer != ENTRYSTATUS_INVALID))
            {
                rc = SNMP_ERR_WRONGVALUE;
            }
            if (SNMPERR_SUCCESS != rc)
            {
                DEBUGMSGTL (("etherStatsTable:_etherStatsTable_check_column:etherStatsStatus",
                             "varbind validation failed (eg bad type or size)\n"));
            }
            else
            {
                rc = etherStatsStatus_check_value (rowreq_ctx, *((u_long *) var->val.string));
                if ((MFD_SUCCESS != rc) && (MFD_NOT_VALID_EVER != rc) && (MFD_NOT_VALID_NOW != rc))
                {
                    snmp_log (LOG_ERR, "bad rc %d from etherStatsStatus_check_value\n", rc);
                    rc = SNMP_ERR_GENERR;
                }
            }
            break;
        ...
        default:
            ...
            rc = SNMP_ERR_GENERR;
            snmp_log (LOG_ERR, "unknown column %d in _etherStatsTable_check_column\n", column);
    }
                                    ...
                                    netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    for (; requests; requests = requests->next)
    {
        ...
        rc = _etherStatsTable_check_column (rowreq_ctx, requests->requestvb, tri->colnum);
        if (rc)
        {
            netsnmp_request_set_error (requests, SNMP_VALIDATE_ERR (rc));
            break;
 ...
 */
NETSNMP_STATIC_INLINE int _etherStatsTable_undo_setup_column (etherStatsTable_rowreq_ctx * rowreq_ctx, int column)
{
    int rc = SNMPERR_SUCCESS;
    ...
    switch (column)
    {
        ...
        default:
            snmp_log (LOG_ERR, "unknown column %d in _etherStatsTable_undo_setup_column\n", column);
            break;
    }
                                 ...
                                 netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_undo_setup", "called\n"));
    ...
    if (MFD_SUCCESS != rc)
    {
        DEBUGMSGTL (("etherStatsTable:mfd", "error %d from " "etherStatsTable_undo_setup\n", rc));
        netsnmp_request_set_error_all (requests, SNMP_VALIDATE_ERR (rc));
    }
    else
    {
        ...
        for (; requests; requests = requests->next)
        {
            ...
            rc = _etherStatsTable_undo_setup_column (rowreq_ctx, tri->colnum);
            if (MFD_SUCCESS != rc)
            {
                DEBUGMSGTL (("etherStatsTable:mfd", "error %d from " "etherStatsTable_undo_setup_column\n", rc));
                                   ...
                                   netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    if (MFD_SUCCESS != rc)
    {
         ...
         * nothing we can do about it but log it
         */
        DEBUGMSGTL (("etherStatsTable:mfd", "error %d from " "etherStatsTable_undo_cleanup\n", rc));
    }
 ...
 */
NETSNMP_STATIC_INLINE int
_etherStatsTable_set_column (etherStatsTable_rowreq_ctx * rowreq_ctx, netsnmp_variable_list * var, int column)
{
    int rc = SNMPERR_SUCCESS;

    DEBUGMSGTL (("internal:etherStatsTable:_etherStatsTable_set_column", "called for %d\n", column));
    ...
    switch (column)
    {
        ...
        case COLUMN_ETHERSTATSDATASOURCE:
            rowreq_ctx->column_set_flags |= COLUMN_ETHERSTATSDATASOURCE_FLAG;
            rc = etherStatsDataSource_set (rowreq_ctx, (oid *) var->val.string, var->val_len);
            break;
        ...
        case COLUMN_ETHERSTATSOWNER:
            rowreq_ctx->column_set_flags |= COLUMN_ETHERSTATSOWNER_FLAG;
            rc = etherStatsOwner_set (rowreq_ctx, (char *) var->val.string, var->val_len);
            break;
        ...
        case COLUMN_ETHERSTATSSTATUS:
            rowreq_ctx->column_set_flags |= COLUMN_ETHERSTATSSTATUS_FLAG;
            rc = etherStatsStatus_set (rowreq_ctx, *((u_long *) var->val.string));
            break;

        default:
            snmp_log (LOG_ERR, "unknown column %d in _etherStatsTable_set_column\n", column);
            rc = SNMP_ERR_GENERR;
            break;
    }
                                 ...
                                 netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_set_values", "called\n"));
    ...
    for (; requests; requests = requests->next)
    {
        ...
        rc = _etherStatsTable_set_column (rowreq_ctx, requests->requestvb, tri->colnum);
        if (MFD_SUCCESS != rc)
        {
            DEBUGMSGTL (("etherStatsTable:mfd", "error %d from " "etherStatsTable_set_column\n", rc));
            netsnmp_set_request_error (agtreq_info, requests, SNMP_VALIDATE_ERR (rc));
        }
    }                            /* for results */
                             ...
                             netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    DEBUGMSGTL (("internal:etherStatsTable:_mfd_etherStatsTable_commit", "called\n"));
    ...
    if (MFD_SUCCESS != rc)
    {
        DEBUGMSGTL (("etherStatsTable:mfd", "error %d from " "etherStatsTable_commit\n", rc));
        netsnmp_request_set_error_all (requests, SNMP_VALIDATE_ERR (rc));
    }
                                  ...
                                  netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    if (MFD_SUCCESS != rc)
    {
         ...
         * nothing we can do about it but log it
         */
        DEBUGMSGTL (("etherStatsTable:mfd", "error %d from " "etherStatsTable_undo_commit\n", rc));
    }

    if (rowreq_ctx->rowreq_flags & MFD_ROW_DIRTY)
    {
        snmp_log (LOG_WARNING, "etherStatsTable row dirty flag still set after undo_commit\n");
...
_etherStatsTable_undo_column (etherStatsTable_rowreq_ctx * rowreq_ctx, netsnmp_variable_list * var, int column)
{
    ...
    DEBUGMSGTL (("internal:etherStatsTable:_etherStatsTable_undo_column", "called for %d\n", column));
    ...
    switch (column)
    {
        ...
        default:
            snmp_log (LOG_ERR, "unknown column %d in _etherStatsTable_undo_column\n", column);
            break;
    }
                                  ...
                                  netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    if (MFD_SUCCESS != rc)
    {
         ...
         * nothing we can do about it but log it
         */
        DEBUGMSGTL (("etherStatsTable:mfd", "error %d from " "etherStatsTable_undo\n", rc));
    }
    ...
    for (; requests; requests = requests->next)
    {
        ...
        rc = _etherStatsTable_undo_column (rowreq_ctx, requests->requestvb, tri->colnum);
        if (MFD_SUCCESS != rc)
        {
            /*
             * nothing we can do about it but log it
             */
            DEBUGMSGTL (("etherStatsTable:mfd", "error %d from " "etherStatsTable_undo_column\n", rc));
        }
    }                            /* for results */
                                          ...
                                          netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    else
    {
        ...
        {
            DEBUGMSGTL (("internal:etherStatsTable:_mfd_irreversible_commit",
                         "updating exists (%#x) w/set (%#x) = %#x\n", rowreq_ctx->column_exists_flags,
                         rowreq_ctx->column_set_flags,
                         (rowreq_ctx->column_exists_flags | rowreq_ctx->column_set_flags)));
            rowreq_ctx->column_exists_flags |= rowreq_ctx->column_set_flags;
            rowreq_ctx->column_set_flags = 0;
        }
        if (rowreq_ctx->rowreq_flags & MFD_ROW_CREATED)
...
static int _cache_load (netsnmp_cache * cache, void *vmagic)
{
    ...
    {
        snmp_log (LOG_ERR, "invalid cache for etherStatsTable_cache_load\n");
        return -1;
    }
     ...
     * call user code
     */
    return etherStatsTable_container_load ((netsnmp_container *) cache->magic);
}                                /* _cache_load */
 ...
 * @internal
 */
static void _container_item_free (etherStatsTable_rowreq_ctx * rowreq_ctx, void *context)
{
    DEBUGMSGTL (("internal:etherStatsTable:_container_item_free", "called\n"));
...
static void _container_free (netsnmp_container * container)
{
    ...
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "invalid container in etherStatsTable_container_free\n");
        return;
    }
 ...
 */
void _etherStatsTable_container_init (etherStatsTable_interface_ctx * if_ctx)
{
    DEBUGMSGTL (("internal:etherStatsTable:_etherStatsTable_container_init", "called\n"));

    /*
     * cache init
     */
    if_ctx->cache = netsnmp_cache_create (30,    /* timeout in seconds */
                                          _cache_load, _cache_free, etherStatsTable_oid, etherStatsTable_oid_size);
    ...
    if (NULL == if_ctx->container)
        if_ctx->container = netsnmp_container_find ("etherStatsTable:table_container");
    if (NULL == if_ctx->container)
    {
        snmp_log (LOG_ERR, "error creating container in " "etherStatsTable_container_init\n");
        return;
    }
 ...
 */
void _etherStatsTable_container_shutdown (etherStatsTable_interface_ctx * if_ctx)
{
...
etherStatsTable_rowreq_ctx *etherStatsTable_row_find_by_mib_index (etherStatsTable_mib_index * mib_idx)
{
    etherStatsTable_rowreq_ctx *rowreq_ctx;
    ...
    rowreq_ctx = CONTAINER_FIND (etherStatsTable_if_ctx.container, &oid_idx);
