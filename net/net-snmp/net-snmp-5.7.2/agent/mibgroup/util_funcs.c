...
netsnmp_feature_child_of (string_append_int, util_funcs) netsnmp_feature_child_of (internal_mib_table, util_funcs)
...
mib_table_t Initialise_Table (int size, int timeout, RELOAD * reload, COMPARE * compare)
{
...
prefix_cbx *net_snmp_create_prefix_info (unsigned long OnLinkFlag, unsigned long AutonomousFlag, char *in6ptr)
{
...
int net_snmp_find_prefix_info (prefix_cbx ** head, char *address, prefix_cbx * node_to_find)
{
    int iret;
...
int net_snmp_search_update_prefix_info (prefix_cbx ** head, prefix_cbx * node_to_use, int functionality)
{
...
int net_snmp_delete_prefix_info (prefix_cbx ** head, char *address)
{
    ...
    for (temp_node = *head, prev_node = NULL; temp_node; prev_node = temp_node, temp_node = temp_node->next_info)
    {
