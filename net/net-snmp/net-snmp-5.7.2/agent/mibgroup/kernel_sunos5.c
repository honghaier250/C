...
static mibcache Mibcache[MIBCACHE_SIZE + 1] = {
...
static mibmap Mibmap[MIBCACHE_SIZE + 1] = {
    {MIB2_SYSTEM, 0,},
    {MIB2_INTERFACES, 0,},
    {MIB2_AT, 0,},
    {MIB2_IP, 0,},
...
getmib (int groupname, int subgroupname, void **statbuf, size_t * size,
        size_t entrysize, req_e req_type, void *resp, size_t * length, int (*comp) (void *, void *), void *arg);

static int
getif (mib2_ifEntry_t * ifbuf, size_t size, req_e req_type, mib2_ifEntry_t * resp,
       size_t * length, int (*comp) (void *, void *), void *arg);
static void set_if_info (mib2_ifEntry_t * ifp, unsigned index, char *name, uint64_t flags, int mtu);
...
static int _dlpi_get_phys_address (int fd, char *paddr, int maxlen, int *paddrlen);
...
static int Name_cmp (void *, void *);

static void init_mibcache_element (mibcache * cp);
 ...
 */
    ...
    static void kernel_sunos5_cache_age (unsigned int regnumber, void *data)
    {
    ...
    void init_kernel_sunos5 (void)
    {
        ...
        if (creg == 0)
        {
            alarm_id = snmp_alarm_register (5, NULL, kernel_sunos5_cache_age, NULL);
            DEBUGMSGTL (("kernel_sunos5", "registered alarm %d with period 5s\n", alarm_id));
            alarm_id = snmp_alarm_register (period, SA_REPEAT, kernel_sunos5_cache_age, (void *) period);
 ...
 */
    ...
    int getKstatInt (const char *classname, const char *statname, const char *varname, int *value)
    {
        ...
        if (named == NULL)
        {
            DEBUGMSGTL (("kernel_sunos5", "no var %s for class %s stat %s\n",
                         varname, classname ? classname : "NULL", statname ? statname : "NULL"));
            goto Return;
        }
        ...
        switch (named->data_type)
        {
            ...
            default:
                snmp_log (LOG_ERR,
                          ...
                          statname ? statname : "NULL", varname ? varname : "NULL", named->data_type);
                ret = -1;        /* fail */
                break;
        }
      Return:
    ...
    int getKstat (const char *statname, const char *varname, void *value)
    {
        ...
        for (i = 0; i < ks->ks_ndata; i++)
        {
            DEBUGMSGTL (("kernel_sunos5",
                         ...
                         kstat_data[i].ks_name, kstat_data[i].ks_class, kstat_data[i].ks_type, kstat_data[i].ks_flags));
    ...
    int getKstatString (const char *statname, const char *varname, char *value, size_t value_len)
    {
        kstat_ctl_t *ksc;
 ...
 */
                    ...
                    req_e req_type, int (*comp) (void *, void *), void *arg)
    {
        ...
        DEBUGMSGTL (("kernel_sunos5", "... cache_valid %d time %ld ttl %d now %ld\n",
                     cache_valid, cachep->cache_time, (int) cachep->cache_ttl, time (NULL)));
        if (cache_valid)
        {
            /*
             * Is it really? 
        ...
        if (cache_valid)
        {
            ...
            if (req_type == GET_NEXT)
            {
                result = getentry (req_type,
                                   ...
                                   cachep->cache_length -
                                   (cachep->cache_last_found * entrysize), entrysize, &ep, comp, arg);
            }
            else
            {
                result = getentry (req_type, cachep->cache_addr, cachep->cache_length, entrysize, &ep, comp, arg);
            }
        }
            ...
            (result == NEED_NEXT && cachep->cache_flags & CACHE_MOREDATA))
        {
            ...
            if (grid == MIB_INTERFACES)
            {
                rc = getif ((mib2_ifEntry_t *) cachep->cache_addr,
                            cachep->cache_size, req_type, (mib2_ifEntry_t *) & ep, &length, comp, arg);
            }
            else
            {
                rc = getmib (mibgr, mibtb, &(cachep->cache_addr),
                             &(cachep->cache_size), entrysize, req_type, &ep, &length, comp, arg);
            }
 ...
 */
    ...
    static found_e
        getentry (req_e req_type, void *bufaddr, size_t len,
                  size_t entrysize, void *resp, int (*comp) (void *, void *), void *arg)
    {
 ...
 */
                ...
                size_t entrysize, req_e req_type, void *resp, size_t * length, int (*comp) (void *, void *), void *arg)
    {
        ...
        for (;;)
        {
            ...
            if (strbuf.len >= sizeof (struct T_error_ack) && tea->PRIM_type == T_ERROR_ACK)
            {
                /* Protocol error */
                ...
                switch (rc)
                {
                    ...
                    case 0:
                        ...
                        if (req_type == GET_NEXT && result == NEED_NEXT)
                             ...
                             */
                            req_type = GET_FIRST;
                        result = getentry (req_type, (void *) strbuf.buf, strbuf.len, entrysize, resp, comp, arg);
            ...
            {                    /* Restore buffers, continue search */
                strbuf.buf = buf;
                strbuf.maxlen = BUFSIZE;
            }
        }
 ...
 */
    static int _dlpi_get_phys_address (int fd, char *addr, int maxlen, int *addrlen)
    {
 ...
 */
               ...
               mib2_ifEntry_t * resp, size_t * length, int (*comp) (void *, void *), void *arg)
    {
         ...
         */
      Again:
        for (i = 0, ifnp = ifname, ifp = (mib2_ifEntry_t *) ifbuf; ifnp->if_index != 0 && (i < nentries); ifnp++)
        {
            ...
            if ((fd = _dlpi_open (ifnp->if_name)) != -1)
            {
                ...
                (void) _dlpi_get_phys_address (fd, ifp->ifPhysAddress.o_bytes,
                                               sizeof (ifp->ifPhysAddress.o_bytes), &ifp->ifPhysAddress.o_length);
                (void) _dlpi_get_iftype (fd, &ifp->ifType);
                close (fd);
            }

            set_if_info (ifp, ifnp->if_index, ifnp->if_name, if_flags, lifrp->lifr_metric);
        ...
        result = getentry (req_type, (void *) ifbuf, size, sizeof (mib2_ifEntry_t), (void *) resp, comp, arg);
               ...
               mib2_ifEntry_t * resp, size_t * length, int (*comp) (void *, void *), void *arg)
    {
      ...
      Again:
        for (i = 0, ifp = (mib2_ifEntry_t *) ifbuf, ifrp = ifconf.ifc_req;
             ((char *) ifrp < ((char *) ifconf.ifc_buf + ifconf.ifc_len)) && (i < nentries); i++, ifp++, ifrp++, idx++)
        {
            ...
            if (getMibstat (MIB_IP_NET, &Media, sizeof (mib2_ipNetToMediaEntry_t), GET_EXACT, &Name_cmp, ifrp) == 0)
            {
        ...
        if ((result != FOUND) && (i == nentries) && ((char *) ifrp < (char *) ifconf.ifc_buf + ifconf.ifc_len))
        {
            /*
             * We reached the end of supplied buffer, but there is
             * some more stuff to read, so continue.
    ...
    static int get_if_stats (mib2_ifEntry_t * ifp)
    {
        ...
        ifp->ifInNUcastPkts = (uint32_t) (ifp->ifHCInBroadcastPkts + ifp->ifHCInMulticastPkts);
        ...
        ifp->ifOutNUcastPkts = (uint32_t) (ifp->ifHCOutBroadcastPkts + ifp->ifHCOutMulticastPkts);
        return (0);
    }
 ...
 */
    int Get_everything (void *x, void *y)
    {
 ...
 */
    static int Name_cmp (void *ifrp, void *ep)
    {
        ...
        if ((strncmp (Ep->ipNetToMediaIfIndex.o_bytes,
                      ...
                      Ep->ipNetToMediaIfIndex.o_length) == 0) && (s->sin_addr.s_addr == Ep->ipNetToMediaNetAddress))
        {
            return 0;
        }
        else
 ...
 */
    int solaris2_if_nametoindex (const char *Name, int Len)
    {
...
int ip20comp (void *ifname, void *ipp)
{
...
int ARP_Cmp_Addr (void *addr, void *ep)
{
    DEBUGMSGTL (("kernel_sunos5", "ARP: %lx <> %lx\n",
                 ((mib2_ipNetToMediaEntry_t *) ep)->ipNetToMediaNetAddress, *(IpAddress *) addr));
    if (((mib2_ipNetToMediaEntry_t *) ep)->ipNetToMediaNetAddress == *(IpAddress *) addr)
    {
        return 0;
    }
    else
...
int main (int argc, char **argv)
{
    ...
    while ((rc = getMibstat (MIB_INTERFACES, &ifstat, sizeof (mib2_ifEntry_t), req_type, &IF_cmp, &idx)) == 0)
    {
        idx = ifstat.ifIndex;
        DEBUGMSGTL (("kernel_sunos5", "Ifname = %s\n", ifstat.ifDescr.o_bytes));
        req_type = GET_NEXT;
    }
    rc = getMibstat (MIB_IP_ADDR, &ipbuf, sizeof (mib2_ipAddrEntry_t), req_type, ip20comp, argv[1]);
    ...
    while ((rc =
            getMibstat (MIB_IP_NET, &entry,
                        sizeof (mib2_ipNetToMediaEntry_t), req_type, &ARP_Cmp_Addr, &LastAddr)) == 0)
    {
