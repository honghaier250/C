    ...
    else
    {
        ...
        for (ap = all_routes; ap < all_routes_end; ap += rtp->rtm_msglen)
        {
            ...
            if (!(rtp->rtm_addrs & RTA_DST))
                continue;
            cp = (u_char *) get_in_address ((struct sockaddr *) (rtp + 1), rtp->rtm_addrs, RTA_DST);
            if (cp == NULL)
                return NULL;
    ...
    switch (vp->magic)
    {
        case IPROUTEDEST:
            *var_len = sizeof (addr_ret);
            return (u_char *) get_in_address ((struct sockaddr *) (rtp + 1), rtp->rtm_addrs, RTA_DST);
        case IPROUTEIFINDEX:
        ...
        case IPROUTENEXTHOP:
            *var_len = sizeof (addr_ret);
            return (u_char *) get_in_address ((struct sockaddr *) (rtp + 1), rtp->rtm_addrs, RTA_GATEWAY);
        case IPROUTETYPE:
        ...
        case IPROUTEPROTO:
            long_return = (rtp->rtm_flags & RTF_DYNAMIC)
                ? 10 : (rtp->rtm_flags & RTF_STATIC) ? 2 : (rtp->rtm_flags & RTF_DYNAMIC) ? 4 : 1;
            return (u_char *) & long_return;
        case IPROUTEAGE:
#if NETSNMP_NO_DUMMY_VALUES
            return NULL;
        ...
        case IPROUTEMASK:
            *var_len = sizeof (addr_ret);
            ...
            {
                return (u_char *) get_in_address ((struct sockaddr *) (rtp + 1), rtp->rtm_addrs, RTA_NETMASK);
            }
        case IPROUTEINFO:
            *var_len = nullOidLen;
    ...
    switch (vp->magic)
    {
        ...
        case IPROUTEMASK:
            *var_len = sizeof (addr_ret);
            ...
            {
#ifndef linux
                if (!NETSNMP_KLOOKUP (rthead[RtIndex]->rt_ifp, (char *) &rt_ifnet, sizeof (rt_ifnet)))
                {
                    DEBUGMSGTL (("mibII/var_route", "klookup failed\n"));
                    return NULL;
                }
                if (!NETSNMP_KLOOKUP (rt_ifnet.if_addrlist, (char *) &rt_ifnetaddr, sizeof (rt_ifnetaddr)))
                {
                    DEBUGMSGTL (("mibII/var_route", "klookup failed\n"));
                    return NULL;
                }
...
#define    IP_ROUTEADDR_OFF    10
    ...
    *write_method = NULL;        /* write_rte;  XXX:  SET support not really implemented */
    ...
    for (Nextentry.ipRouteDest = (u_long) - 2, req_type = GET_FIRST;; Nextentry = entry, req_type = GET_NEXT)
    {
        if (getMibstat (MIB_IP_ROUTE, &entry, sizeof (mib2_ipRouteEntry_t), req_type, &IP_Cmp_Route, &Nextentry) != 0)
            break;
    ...
    *var_len = sizeof (long_return);
    ...
    switch (vp->magic)
    {
        ...
        case IPROUTEIFINDEX:
            ...
            long_return = netsnmp_access_interface_index_find (Lowentry.ipRouteIfIndex.o_bytes);
#else
            long_return = Interface_Index_By_Name (Lowentry.ipRouteIfIndex.o_bytes, Lowentry.ipRouteIfIndex.o_length);
#endif
            return (u_char *) & long_return;
        case IPROUTEMETRIC1:
...
#if CHECK_RT_FLAGS
        if (((rt.rt_flags & RTF_CLONING) != RTF_CLONING) && ((rt.rt_flags & RTF_LLINFO) != RTF_LLINFO))
        {
#endif
            /*
...
#define AF_UNSPEC AF_INET
        ...
        for (table = 0; table < NUM_ROUTE_SYMBOLS; table++)
    {
        ...
        auto_nlist (route_symbols[table], (char *) routehash, hashsize * sizeof (struct mbuf *));
...
#if HAVE_SYS_MBUF_H
netsnmp_feature_require (string_append_int) netsnmp_feature_require (interface_legacy)
     static void Route_Scan_Reload (void)
{
...
static void Route_Scan_Reload (void)
{
    ...
    while (fgets (line, sizeof (line), in))
    {
        ...
        if (rtsize >= rtallocate)
        {
            rthead = (struct rtentry **) realloc ((char *) rthead, 2 * rtallocate * sizeof (struct rtentry *));
