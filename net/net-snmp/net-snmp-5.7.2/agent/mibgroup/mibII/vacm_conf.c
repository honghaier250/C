...
{
    snmpd_register_config_handler ("group", vacm_parse_group, vacm_free_group, "name v1|v2c|usm|... security");
    snmpd_register_config_handler ("access", vacm_parse_access,
                                   vacm_free_access, "name context model level prefix read write notify");
    snmpd_register_config_handler ("setaccess", vacm_parse_setaccess,
                                   vacm_free_access, "name context model level prefix viewname viewval");
    snmpd_register_config_handler ("view", vacm_parse_view, vacm_free_view, "name type subtree [mask]");
    snmpd_register_const_config_handler ("vacmView", vacm_parse_config_view, NULL, NULL);
    snmpd_register_const_config_handler ("vacmGroup", vacm_parse_config_group, NULL, NULL);
    snmpd_register_const_config_handler ("vacmAccess", vacm_parse_config_access, NULL, NULL);
    snmpd_register_const_config_handler ("vacmAuthAccess", vacm_parse_config_auth_access, NULL, NULL);

    /* easy community auth handler */
    snmpd_register_config_handler ("authcommunity",
                                   vacm_parse_authcommunity,
                                   NULL,
                                   "authtype1,authtype2 community [default|hostname|network/bits [oid|-V view [context]]]");

    /* easy user auth handler */
    snmpd_register_config_handler ("authuser",
                                   vacm_parse_authuser,
                                   NULL,
                                   "authtype1,authtype2 [-s secmodel] user [noauth|auth|priv [oid|-V view [context]]]");
    /* easy group auth handler */
    snmpd_register_config_handler ("authgroup",
                                   vacm_parse_authuser,
                                   NULL,
                                   "authtype1,authtype2 [-s secmodel] group [noauth|auth|priv [oid|-V view [context]]]");
     ...
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_PRE_READ_CONFIG, vacm_standard_views, NULL);
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_POST_READ_CONFIG, vacm_warn_if_not_configured, NULL);
}
...
void init_vacm_conf (void)
{
     ...
     */
    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_ACM_CHECK, vacm_in_view_callback, NULL);
    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_ACM_CHECK_INITIAL, vacm_in_view_callback, NULL);
    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_ACM_CHECK_SUBTREE, vacm_in_view_callback, NULL);
}
...
_vacm_parse_access_common (const char *token, char *param, char **st,
                           char **name, char **context, int *imodel, int *ilevel, int *iprefix)
{
...
void vacm_parse_authaccess (const char *token, char *confline)
{
    ...
    if (!ap)
    {
        ...
        DEBUGMSGTL (("vacm:conf:authaccess", "no existing access found; creating a new one\n"));
    }
    else
    {
        DEBUGMSGTL (("vacm:conf:authaccess", "existing access found, using it\n"));
    }
    if (!ap)
    {
...
void vacm_parse_setaccess (const char *token, char *param)
{
    ...
    if (_vacm_parse_access_common (token, param, &st, &name, &context, &imodel, &ilevel, &iprefix) == PARSE_FAIL)
    {
        return;
    }
    ...
    {
        DEBUGMSGTL (("vacm:conf:setaccess", "existing access found, using it\n"));
    }
    if (!ap)
    {
        config_perror ("failed to create access entry");
...
vacm_gen_com2sec (int commcount, const char *community, const char *addressname,
                  ...
                  char *viewname, size_t viewname_len, int version, const char *context)
{
    ...
    if (context && *context)
        snprintf (line, sizeof (line), "-Cn %s %s %s '%s'", context, secname, addressname, community);
    else
...
{
    vacm_create_simple (token, confline, VACM_CREATE_SIMPLE_V3, VACM_VIEW_READ_BIT | VACM_VIEW_WRITE_BIT);
}

void vacm_parse_rouser (const char *token, char *confline)
{
    vacm_create_simple (token, confline, VACM_CREATE_SIMPLE_V3, VACM_VIEW_READ_BIT);
}
...
{
    vacm_create_simple (token, confline, VACM_CREATE_SIMPLE_COMIPV4, VACM_VIEW_READ_BIT | VACM_VIEW_WRITE_BIT);
}

void vacm_parse_rocommunity6 (const char *token, char *confline)
{
    vacm_create_simple (token, confline, VACM_CREATE_SIMPLE_COMIPV6, VACM_VIEW_READ_BIT);
}

void vacm_parse_rwcommunity6 (const char *token, char *confline)
{
    vacm_create_simple (token, confline, VACM_CREATE_SIMPLE_COMIPV6, VACM_VIEW_READ_BIT | VACM_VIEW_WRITE_BIT);
}


void vacm_create_simple (const char *token, char *confline, int parsetype, int viewtypes)
{
    char line[SPRINT_MAX_LEN];
    ...
    if (parsetype == VACM_CREATE_SIMPLE_COMIPV4 || parsetype == VACM_CREATE_SIMPLE_COM)
    {
        vacm_gen_com2sec (commcount, community, addressname,
                          "com2sec", &netsnmp_udp_parse_security,
                          secname, sizeof (secname), view_ptr, sizeof (viewname), commversion, context);
    }
#endif

#ifdef NETSNMP_TRANSPORT_UNIX_DOMAIN
    if (parsetype == VACM_CREATE_SIMPLE_COMUNIX || parsetype == VACM_CREATE_SIMPLE_COM)
    {
        if (*context)
            snprintf (line, sizeof (line), "-Cn %s %s %s '%s'", context, secname, addressname, community);
        else
...
#ifdef NETSNMP_TRANSPORT_UDPIPV6_DOMAIN
    if (parsetype == VACM_CREATE_SIMPLE_COMIPV6 || parsetype == VACM_CREATE_SIMPLE_COM)
    {
        vacm_gen_com2sec (commcount, community, addressname,
        ...
        else
        {
            ...
            for (tmp = grpname; *tmp; tmp++)
                if (!isalnum ((unsigned char) (*tmp)))
                    *tmp = '_';
            snprintf (line, sizeof (line), "%s %s \"%s\"", grpname, model, secname);
            line[sizeof (line) - 1] = 0;
            DEBUGMSGTL ((token, "passing: %s %s\n", "group", line));
            vacm_parse_group ("group", line);
        }
    ...
    if ((viewtypes == VACM_VIEW_READ_BIT) || (viewtypes == (VACM_VIEW_READ_BIT | VACM_VIEW_WRITE_BIT)))
    {
        ...
        snprintf (line, sizeof (line),
                  ...
                  model, authlevel, (ctxprefix ? "prefix" : "exact"), viewname, rw, rw);
        line[sizeof (line) - 1] = 0;
        DEBUGMSGTL ((token, "passing: %s %s\n", "access", line));
        ...
        for (i = 0; i <= VACM_MAX_VIEWS; i++)
        {
            if (viewtypes & (1 << i))
            {
                snprintf (line, sizeof (line),
                          ...
                          grpname, context[0] ? context : "\"\"",
                          model, authlevel,
                          (ctxprefix ? "prefix" : "exact"), se_find_label_in_slist (VACM_VIEW_ENUM_NAME, i), viewname);
                line[sizeof (line) - 1] = 0;
                DEBUGMSGTL ((token, "passing: %s %s\n", "setaccess", line));
...
int vacm_standard_views (int majorID, int minorID, void *serverarg, void *clientarg)
{
    char line[SPRINT_MAX_LEN];
...
int vacm_warn_if_not_configured (int majorID, int minorID, void *serverarg, void *clientarg)
{
...
#define NETSNMP_DS_APP_NO_AUTHORIZATION 17
        ...
        if (!strcmp (name, "snmptrapd") &&
            !netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_APP_NO_AUTHORIZATION))
        {
            snmp_log (LOG_WARNING,
                      "Warning: no access control information configured.\n"
                      "  (Config search path: %s)\n"
...
int vacm_in_view_callback (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    if (view_parms == NULL)
        return 1;
    retval = vacm_in_view (view_parms->pdu, view_parms->name, view_parms->namelen, view_parms->check_subtree);
    if (retval != 0)
 ...
 *    \<securityName\> \<groupName\> \<viewName\> \<viewType\>
 */
int vacm_check_view (netsnmp_pdu * pdu, oid * name, size_t namelen, int check_subtree, int viewtype)
{
    return vacm_check_view_contents (pdu, name, namelen, check_subtree, viewtype, VACM_CHECK_VIEW_CONTENTS_NO_FLAGS);
}

int vacm_check_view_contents (netsnmp_pdu * pdu, oid * name, size_t namelen, int check_subtree, int viewtype, int flags)
{
    struct vacm_accessEntry *ap;
...
#define CONTEXTNAMEINDEXLEN 32
...
#endif
    {
        ...
        if (snmp_get_do_debugging ())
        {
            ...
            DEBUGMSGTL (("mibII/vacm_vars", "vacm_in_view: ver=%ld, community=%s\n", pdu->version, buf));
            free (buf);
        }
...
#endif
        DEBUGMSGTL (("mibII/vacm_vars", "vacm_in_view: No security name found\n"));
    ...
    if (!(flags & VACM_CHECK_VIEW_CONTENTS_DNE_CONTEXT_OK) && !netsnmp_subtree_find_first (contextNameIndex))
    {
        /*
    ...
    ap = vacm_getAccessEntry (gp->groupName, contextNameIndex, pdu->securityModel, pdu->securityLevel);
    if (ap == NULL)
    {
        DEBUGMSG (("mibII/vacm_vars", "\n"));
