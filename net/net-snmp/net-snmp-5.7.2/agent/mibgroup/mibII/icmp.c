    ...
    for (i = 0; i < 2; i++)
    {
        switch (i)
        {
            case 0:
                ...
                break;
            default:
                memset (&icmp_stats_table[i], 0, sizeof (struct icmp_stats_table_entry));
                ...
                icmp_stats_table[i].icmpStatsOutErrors = v6icmp.icmp6OutDestUnreachs +
                    v6icmp.icmp6OutPktTooBigs + v6icmp.icmp6OutTimeExcds + v6icmp.icmp6OutParmProblems;
                break;
        }
                                              ...
                                              netsnmp_variable_list * index, netsnmp_iterator_info * data)
{
     ...
     *set IP version
     */
    snmp_set_var_typed_value (idx, ASN_INTEGER, (u_char *) & icmp_stats_table[i].ipVer, sizeof (uint32_t));
                                                  ...
                                                  netsnmp_variable_list * index, netsnmp_iterator_info * data)
{
    ...
    /* set IP version */
    snmp_set_var_typed_value (idx, ASN_INTEGER, (u_char *) & icmp_msg_stats_table[i].ipVer, sizeof (uint32_t));

    /* set packet type */
    idx = idx->next_variable;
    snmp_set_var_typed_value (idx, ASN_INTEGER,
                              (u_char *) & icmp_msg_stats_table[i].icmpMsgStatsType, sizeof (uint32_t));
...
void init_icmp (void)
{
    ...
    rc = netsnmp_inject_handler (scalar_reginfo,
                                 netsnmp_get_cache_handler (ICMP_STATS_CACHE_TIMEOUT,
                                                            icmp_load, icmp_free, icmp_oid, OID_LENGTH (icmp_oid)));
    if (rc != SNMPERR_SUCCESS)
        goto bail;
#endif
#ifdef linux
    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                DEBUGMSGOID (("mibII/icmp", requestvb->name, requestvb->name_length));
                DEBUGMSG (("mibII/icmp", "\n"));
                ...
                switch (subid)
                {
                    ...
                    case ICMPINMSGS:
                        ret_value = icmpstat.icps_badcode +
                            icmpstat.icps_tooshort + icmpstat.icps_checksum + icmpstat.icps_badlen;
                ...
                snmp_set_var_typed_value (request->requestvb, ASN_COUNTER, (u_char *) & ret_value, sizeof (ret_value));
            }
            break;
