...
void init_interfaces (void)
{
     ...
     */
    REGISTER_MIB ("mibII/interfaces", interfaces_variables, variable3, interfaces_variables_oid);
 ...
 */
static int if_type_from_name (const char *pcch)
{
...
u_char *var_interfaces (struct variable * vp,
                        oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
...
static int Interface_Scan_By_Index (int iindex, struct if_msghdr *if_msg, char *if_name, struct small_ifaddr *sifa)
{
    u_char *cp;
            ...
            case RTM_IFINFO:
                {
                    ...
                    if (ifp->ifm_index == iindex)
                    {
                        ...
                        if (a == NULL)
                            return 0;
                        sprintf (if_name, "%.*s", ((const u_char *) a)[5], ((const struct sockaddr_in *) a)->sin_zero);
                        *if_msg = *ifp;
                        ++have_ifinfo;
                    }
                }
            ...
            case RTM_NEWADDR:
                {
                    ...
                    if ((NULL != sifa) && (ifap->ifam_index == iindex))
                    {
                        ...
                        ia = get_in_address ((char *) (ifap + 1) + 8, ifap->ifam_addrs &= ~RTA_NETMASK, RTA_IFA);
                        if (ia == NULL)
                            return 0;

                        sifa->sifa_addr = *ia;
                        ia = get_in_address ((char *) (ifap + 1) + 8, ifap->ifam_addrs &= ~RTA_NETMASK, RTA_BRD);
                        if (ia == NULL)
                            return 0;
...
u_char *var_ifEntry (struct variable *vp,
                     oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    int interface;
    ...
    switch (vp->magic)
    {
        ...
        case NETSNMP_IFSPEED:
            ...
            else
            {
                ...
                long_return = (u_long) if_msg.ifm_data.ifi_baudrate.ifs_value << if_msg.ifm_data.ifi_baudrate.ifs_log2;
#else
        ...
        case NETSNMP_IFLASTCHANGE:
            ...
            {
                long_return = (u_long) ((if_msg.ifm_data.ifi_lastchange.tv_sec - starttime.tv_sec) * 100 + (
#if STRUCT_IFNET_HAS_IF_LASTCHANGE_TV_NSEC
                                                                                                               if_msg.
                                                                                                               ifm_data.
                                                                                                               ifi_lastchange.
...
int Interface_Scan_Next (short *Index, char *Name, struct ifnet *Retifnet, struct in_ifaddr *Retin_ifaddr)
{
    return 0;
}

int Interface_Scan_NextInt (int *Index, char *Name, struct ifnet *Retifnet, struct in_ifaddr *Retin_ifaddr)
{
    return 0;
}
                     ...
                     oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    switch (vp->magic)
    {
        ...
        case NETSNMP_IFPHYSADDRESS:
            ...
            *var_len = 0;
            ...
            if ((return_buf[0] == 0) && (return_buf[1] == 0) &&
                (return_buf[2] == 0) && (return_buf[3] == 0) && (return_buf[4] == 0) && (return_buf[5] == 0))
                *var_len = 0;
            else
                *var_len = 6;
#endif
        ...
        case NETSNMP_IFLASTCHANGE:
             ...
             */
            if (ifnet.if_lastchange.tv_sec == 0 && ifnet.if_lastchange.tv_usec == 0)
                long_return = 0;
...
static int IF_cmp (void *addr, void *ep)
{
                     ...
                     oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    if (getMibstat (MIB_INTERFACES, &ifstat, sizeof (mib2_ifEntry_t), GET_EXACT, &IF_cmp, &interface) != 0)
    {
...
unsigned int getIfSpeed (int fd, struct ifreq ifr, unsigned int defaultspeed)
{
    ...
    extern unsigned int
        netsnmp_linux_interface_get_if_speed (int fd, const char *name, unsigned long long defaultspeed);
...
void Interface_Scan_Init (void)
{
    ...
    const char *scan_line_2_2 =
        ...
        " %*" SCNuMAX " %*" SCNuMAX " %*" SCNuMAX " %*" SCNuMAX
        " %" SCNuMAX " %" SCNuMAX " %" SCNuMAX " %" SCNuMAX " %*" SCNuMAX " %" SCNuMAX;
    ...
    while (fgets (line, sizeof (line), devin))
    {
             ...
             sscanf (stats, scan_line_to_use, &rec_oct, &rec_pkt, &rec_err,
                     ...
                     &coll) != 9) || (scan_line_to_use == scan_line_2_0
                                      && sscanf (stats, scan_line_to_use,
                                                 &rec_pkt, &rec_err, &snd_pkt, &snd_err, &coll) != 5))
        {
            if ((scan_line_to_use == scan_line_2_2) && !strstr (line, "No statistics available"))
                snmp_log (LOG_ERR, "/proc/net/dev data format error, line ==|%s|", line);
            continue;
        }
        ...
        *ptr = 0;
        ...
        strlcpy (ifrq.ifr_name, ifname, sizeof (ifrq.ifr_name));
        nnew->if_flags = ioctl (fd, SIOCGIFFLAGS, &ifrq) < 0 ? 0 : ifrq.ifr_flags;
        ...
        strlcpy (ifrq.ifr_name, ifname, sizeof (ifrq.ifr_name));
        nnew->if_metric = ioctl (fd, SIOCGIFMETRIC, &ifrq) < 0 ? 0 : ifrq.ifr_metric;

#ifdef SIOCGIFMTU
        strlcpy (ifrq.ifr_name, ifname, sizeof (ifrq.ifr_name));
        nnew->if_mtu = (ioctl (fd, SIOCGIFMTU, &ifrq) < 0) ? 0 : ifrq.ifr_mtu;
#else
        nnew->if_mtu = 0;
#endif
...
int Interface_Scan_NextInt (int *Index, char *Name, struct ifnet *Retifnet, struct in_ifaddr *dummy)
{
    struct ifnet ifnet;
...
#ifdef linux
int Interface_Index_By_Name (char *Name, int Len)
{
    int ifIndex = 0;
    ...
    Interface_Scan_Init ();
    while (Interface_Scan_NextInt (&ifIndex, ifName, NULL, NULL) && strcmp (Name, ifName));
    return ifIndex;
}
#endif
...
#if defined(hpux11)
int Interface_Scan_Next (short *Index, char *Name, nmapi_phystat * Retifnet)
{
    int returnIndex = 0;
...
int Interface_Scan_NextInt (int *Index, char *Name, nmapi_phystat * Retifnet)
{
...
#define CP(x)    ((char *)(x))
                    ...
                    cp = (CP (in_ifaddr.ia_ifa.ifa_addr) - CP (ia)) + CP (&in_ifaddr);
...
static int Interface_Scan_By_Index (int Index, char *Name, nmapi_phystat * Retifnet)
{
    int i;
...
static int Interface_Scan_By_Index (int Index, char *Name, struct ifnet *Retifnet, struct in_ifaddr *Retin_ifaddr)
{
    int i;
...
static int Interface_Get_Ether_By_Index (int Index, u_char * EtherAddr)
{
    int i;
...
#define ifa_next    ifa_list.tqe_next
    ...
    while (ifaddraddr)
    {
        ...
        if (sadl.sdl_family == AF_LINK
            && (saveifnet.if_type == IFT_ETHER || saveifnet.if_type == IFT_ISO88025 || saveifnet.if_type == IFT_FDDI))
        {
            memcpy (arpcom.ac_enaddr, sadl.sdl_data + sadl.sdl_nlen, sizeof (arpcom.ac_enaddr));
            break;
        }
...
static int header_interfaces (struct variable *, oid *, size_t *, int, size_t *, WriteMethod ** write);

static int header_ifEntry (struct variable *, oid *, size_t *, int, size_t *, WriteMethod ** write);

u_char *var_ifEntry (struct variable *, oid *, size_t *, int, size_t *, WriteMethod ** write);
...
{
    int naddrs, ilen, bit;
    static int mib[6] = { CTL_NET, PF_ROUTE, 0, AF_LINK, NET_RT_IFLIST, 0 };
    char *cp;
...
static int get_phys_address (int iindex, char **ap, int *len)
{
    int i;
                     ...
                     oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    switch (vp->magic)
    {
        ...
        case NETSNMP_IFOPERSTATUS:
            ...
            return (u_char *) & long_return;
        case NETSNMP_IFLASTCHANGE:
            if (ifmd.ifmd_data.ifi_lastchange.tv_sec == 0 && ifmd.ifmd_data.ifi_lastchange.tv_usec == 0)
            {
                long_return = 0;
            }
            ...
            else
            {
                long_return = (u_long)
                    ((ifmd.ifmd_data.ifi_lastchange.tv_sec -
                      starttime.tv_sec) * 100 + ((ifmd.ifmd_data.ifi_lastchange.tv_usec - starttime.tv_usec) / 10000));
            }
            return (u_char *) & long_return;
        case NETSNMP_IFINOCTETS:
    ...
    switch (vp->magic)
    {
        ...
        case NETSNMP_IFADMINSTATUS:
            ...
            *write_method = writeIfEntry;
            ...
            return (u_char *) & long_return;
        case NETSNMP_IFOPERSTATUS:
            long_return = (MIB_IF_OPER_STATUS_OPERATIONAL == ifRow.dwOperStatus) ? 1 : 2;
            return (u_char *) & long_return;
        case NETSNMP_IFLASTCHANGE:
              ...
              u_char * var_val, u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:            /* Check values for acceptability */
             ...
             */
            if (!(((int) (*var_val) == MIB_IF_ADMIN_STATUS_UP) || ((int) (*var_val) == MIB_IF_ADMIN_STATUS_DOWN)))
            {
