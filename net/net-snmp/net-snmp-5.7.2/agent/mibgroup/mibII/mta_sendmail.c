...
static void open_sendmailst (BOOL config)
{
    ...
    if (sendmailst_fh == -1)
    {
        print_error (LOG_ERR, config, TRUE,
                     "mibII/mta_sendmail.c:open_sendmailst", "could not open file \"%s\"", sendmailst_fn);
        return;
    }
    ...
    if (((struct statisticsV8_10 *) stats)->stat_magic == STAT_MAGIC)
    {
        ...
        if (((struct statisticsV8_12_QUAR *) stats)->stat_version ==
            ...
            sizeof (struct statisticsV8_12_QUAR) && filelen == sizeof (struct statisticsV8_12_QUAR))
        {
         ...
         if (((struct statisticsV8_10 *) stats)->stat_version ==
                 ...
                 sizeof (struct statisticsV8_10) && filelen == sizeof (struct statisticsV8_10))
        {
            DEBUGMSGTL (("mibII/mta_sendmail.c:open_sendmailst",
                         "looks like file \"%s\" has been created by sendmail V8.10.0 or newer\n", sendmailst_fn));
            ...
            stats_size = sizeof (struct statisticsV8_10);
                 ...
                 sizeof (struct statisticsV8_9) && filelen == sizeof (struct statisticsV8_9))
        {
            DEBUGMSGTL (("mibII/mta_sendmail.c:open_sendmailst",
        ...
        else
        {
            print_error (LOG_WARNING, config, FALSE,
                         ...
                         "could not guess version of statistics file \"%s\"", sendmailst_fn);
            while (close (sendmailst_fh) == -1 && errno == EINTR)
            {
                /*
                 * do nothing 
    ...
    else
    {
        if (((struct statisticsV8_8 *) stats)->stat_size ==
            sizeof (struct statisticsV8_8) && filelen == sizeof (struct statisticsV8_8))
        {
            DEBUGMSGTL (("mibII/mta_sendmail.c:open_sendmailst",
...
static void add_queuegroup (const char *name, char *path)
{
    ...
    {                            /* multiple queue dirs */
        ...
        *p = '\0';
        ...
        while ((dirp = readdir (dp)) != NULL)
        {
            if (!strncmp (dirp->d_name, p, strlen (p)) && dirp->d_name[0] != '.')
            {
                /*
                ...
                snprintf (subdir->dir, FILENAMELEN - 5, "%s/%s", parentdir, dirp->d_name);
                subdir->next = new;
                new = subdir;
            }
        }
...
static BOOL read_sendmailcf (BOOL config)
{
    ...
    {
        print_error (LOG_ERR, config, TRUE,
                     "mibII/mta_sendmail.c:read_sendmailcf", "could not open file \"%s\"", sendmailcf_fn);
        return FALSE;
    }
        ...
        switch (line[0])
        {
            ...
            case 'M':
                ...
                else
                {
                    print_error (LOG_WARNING, config, FALSE,
                                 "mibII/mta_sendmail.c:read_sendmailcf",
                                 "found too many mailers in config file \"%s\"", sendmailcf_fn);
                }
                ...
                switch (line[1])
                {
                    case ' ':
                        ...
                        if (*filename++ != '=')
                        {
                            print_error (LOG_WARNING, config, FALSE,
                                         "mibII/mta_sendmail.c:read_sendmailcf",
                                         "line %d in config file \"%s\" ist missing an '='", linenr, sendmailcf_fn);
                            break;
                        }
                        ...
                        else
                        {
                            print_error (LOG_CRIT, config, FALSE,
                                         "mibII/mta_sendmail.c:read_sendmailcf", "This shouldn't happen.");
                            abort ();
                        }
                        break;
            ...
            case 'Q':
                ...
                if (*p == '\0')
                {
                    print_error (LOG_WARNING, config, FALSE,
                                 "mibII/mta_sendmail.c:read_sendmailcf",
                                 "line %d config file \"%s\" contains a weird queuegroup", linenr, sendmailcf_fn);
                    break;
                }
...
static void mta_sendmail_parse_config (const char *token, char *line)
{
    if (strlen (line) > FILENAMELEN)
    {                            /* Might give some false alarm, but better to be safe than sorry */
...
void init_mta_sendmail (void)
{
    REGISTER_MIB ("mibII/mta_sendmail", mta_variables, variable3, mta_variables_oid);
 ...
 *
 */
unsigned char *var_mtaEntry (struct variable *vp,
 ...
 *
 */
unsigned char *var_mtaGroupEntry (struct variable *vp,
                                  oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    static long long_ret;
    ...
    *var_len = sizeof (long);    /* default to 'long' results */
     ...
     * bump there 
     */
    if (!exact && row < mailers && (vp->magic == MTAGROUPSTOREDMESSAGES || vp->magic == MTAGROUPSTOREDVOLUME))
    {
        row = mailers;
        name[*length - 1] = row + 1;
    }
