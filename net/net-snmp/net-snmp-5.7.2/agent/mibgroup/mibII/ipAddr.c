 ...
 */
     ...
     u_char *var_ipAddrEntry (struct variable *vp,
...
#define IP_ADDRINDEX_OFF    10
    ...
    if (*length == IP_ADDRNAME_LENGTH)    /* Assume that the input name is the lowest */
        ...
        lowest[0] = 0xff;
    for (NextAddr = (u_long) - 1, req_type = GET_FIRST;; NextAddr = entry.ipAdEntAddr, req_type = GET_NEXT)
    {
        if (getMibstat (MIB_IP_ADDR, &entry, sizeof (mib2_ipAddrEntry_t), req_type, &IP_Cmp, &NextAddr) != 0)
            break;
                ...
                && (((NextAddr == (u_long) - 1))
                    ...
                    (snmp_oid_compare
                     (current, IP_ADDRNAME_LENGTH, lowest,
                      IP_ADDRNAME_LENGTH) < 0) || (snmp_oid_compare (name, *length, lowest, IP_ADDRNAME_LENGTH) == 0)))
            {
                /*
                 * if new one is greater than input and closer to input than
    ...
    *var_len = sizeof (long_return);
    switch (vp->magic)
    {
        ...
        case IPADIFINDEX:
            ...
            long_return = netsnmp_access_interface_index_find (Lowentry.ipAdEntIfIndex.o_bytes);
#else
            long_return = Interface_Index_By_Name (Lowentry.ipAdEntIfIndex.o_bytes, Lowentry.ipAdEntIfIndex.o_length);
#endif
            return (u_char *) & long_return;
        case IPADNETMASK:
            *var_len = sizeof (addr_ret);
                         ...
                         oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    for (i = 0; i < nifs; i++)
    {
        ...
        *op++ = *cp++;
        ...
        {
            if ((snmp_oid_compare (current, 14, name, *length) > 0) &&
                (lowinterface < 0 || (snmp_oid_compare (current, 14, lowest, 14) < 0)))
            {
                /*
