...
#define PRIVRW    (NETSNMP_SNMPV2ANY | 0x5000)
     ...
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_vacm, NULL);

    /*
     * register ourselves with the agent to handle our mib tree 
     */
    REGISTER_MIB ("mibII/vacm:sec2group", vacm_sec2group, variable1, vacm_sec2group_oid);
    ...
    if (vp->magic != VACMVIEWSPINLOCK)
    {
        ...
        if (exact)
        {
            ...
            *cp = 0;
            ...
            gp = vacm_getViewEntry (viewName, &subtree[1], subtreeLen - 1, VACM_MODE_IGNORE_MASK);
            if (gp != NULL)
            {
        ...
        else
        {
            ...
            while ((gp = vacm_scanViewNext ()) != NULL)
            {
                ...
                cmp2 = snmp_oid_compare (gp->viewSubtree, gp->viewSubtreeLen, subtree, subtreeLen);
                if (cmp == 0 && cmp2 > 0)
                    break;
                if (cmp > 0)
                    break;
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
int sec2group_parse_oid (oid * oidIndex, size_t oidLen, int *model, unsigned char **name, size_t * nameLen)
{
    int nameL;
...
struct vacm_groupEntry *sec2group_parse_groupEntry (oid * name, size_t name_len)
{
    ...
    if (sec2group_parse_oid
        (&name[SEC2GROUP_MIB_LENGTH], name_len - SEC2GROUP_MIB_LENGTH, &model, (u_char **) & newName, &nameLen))
        return NULL;
                                      ...
                                      size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == COMMIT)
    {
        ...
        if ((long_ret == ST_VOLATILE || long_ret == ST_NONVOLATILE) &&
            (geptr->storageType == ST_VOLATILE || geptr->storageType == ST_NONVOLATILE))
        {
                                 ...
                                 u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1)
    {
        ...
        if (sec2group_parse_oid (&name[SEC2GROUP_MIB_LENGTH],
                                 name_len - SEC2GROUP_MIB_LENGTH, &model, (u_char **) & newName, &nameLen))
        {
            return SNMP_ERR_INCONSISTENTNAME;
        }
...
access_parse_oid (oid * oidIndex, size_t oidLen,
                  ...
                  unsigned char **contextPrefix, size_t * contextPrefixLen, int *model, int *level)
{
...
struct vacm_accessEntry *access_parse_accessEntry (oid * name, size_t name_len)
{
    ...
    if (access_parse_oid
        (&name[ACCESS_MIB_LENGTH], name_len - ACCESS_MIB_LENGTH,
         (u_char **) & newGroupName, &groupNameLen, (u_char **) & newContextPrefix, &contextPrefixLen, &model, &level))
        return NULL;
...
view_parse_oid (oid * oidIndex, size_t oidLen,
                unsigned char **viewName, size_t * viewNameLen, oid ** subtree, size_t * subtreeLen)
{
...
struct vacm_viewEntry *view_parse_viewEntry (oid * name, size_t name_len)
{
    ...
    if (view_parse_oid (&name[VIEW_MIB_LENGTH], name_len - VIEW_MIB_LENGTH,
                        (u_char **) & newViewName, &viewNameLen, (oid **) & newViewSubtree, &viewSubtreeLen))
        return NULL;

    vptr = vacm_getViewEntry (newViewName, &newViewSubtree[1], viewSubtreeLen - 1, VACM_MODE_IGNORE_MASK);
    free (newViewName);
    free (newViewSubtree);
    ...
    if (action == RESERVE1)
    {
        ...
        else
        {
            ...
            if (long_ret == RS_CREATEANDGO || long_ret == RS_CREATEANDWAIT)
            {
                 ...
                 * Generate a new group entry.  
                 */
                if ((vptr = vacm_createViewEntry (newViewName, &newViewSubtree[1], viewSubtreeLen - 1)) == NULL)
                {
                    free (newViewName);
                    free (newViewSubtree);
                    return SNMP_ERR_GENERR;
    ...
    else if (action == RESERVE2)
    {
        ...
        else
        {
            if ((newValue == ST_VOLATILE || newValue == ST_NONVOLATILE) &&
                (vptr->viewStorageType == ST_VOLATILE || vptr->viewStorageType == ST_NONVOLATILE))
            {
