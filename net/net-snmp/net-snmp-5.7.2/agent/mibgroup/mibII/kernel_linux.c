...
netsnmp_feature_child_of (linux_read_ip6_stat, linux_ip6_stat_all)
     int decode_icmp_msg (char *line, char *data, struct icmp4_msg_mib *msg)
{
...
int linux_read_mibII_stats (void)
{
    ...
    while (line == fgets (line, sizeof (line), in))
    {
        if (!strncmp (line, IP_STATS_LINE, IP_STATS_PREFIX_LEN))
        {
            sscanf (line, IP_STATS_LINE,
                    ...
                    &cached_ip_mib.ipFragOKs, &cached_ip_mib.ipFragFails, &cached_ip_mib.ipFragCreates);
        ...
        else if (!strncmp (line, ICMP_STATS_LINE, ICMP_STATS_PREFIX_LEN))
        {
            sscanf (line, ICMP_STATS_LINE,
                    ...
                    &cached_icmp_mib.icmpOutAddrMasks, &cached_icmp_mib.icmpOutAddrMaskReps);
        }
        else if (!strncmp (line, ICMP_MSG_STATS_LINE, ICMP_MSG_STATS_PREFIX_LEN))
        {
            ...
            if (decode_icmp_msg (line + ICMP_MSG_STATS_PREFIX_LEN,
                                 data + ICMP_MSG_STATS_PREFIX_LEN, &cached_icmp4_msg_mib) < 0)
            {
                continue;
            }
            ret = 1;
        ...
        else if (!strncmp (line, UDP_STATS_LINE, UDP_STATS_PREFIX_LEN))
        {
            sscanf (line, UDP_STATS_LINE,
                    ...
                    &cached_udp_mib.udpNoPorts, &cached_udp_mib.udpInErrors, &cached_udp_mib.udpOutDatagrams);
        }
    }
    fclose (in);
...
#ifdef NETSNMP_ENABLE_IPV6
    DEBUGMSGTL (("mibII/kernel_linux/ip6stats", "Reading /proc/net/snmp6 stats\n"));
    if (NULL == (in = fopen ("/proc/net/snmp6", "r")))
    {
        DEBUGMSGTL (("mibII/kernel_linux/ip6stats", "Failed to open /proc/net/snmp6\n"));
        return -1;
    }
...
int linux_read_icmp_msg_stat (struct icmp_mib *icmpstat, struct icmp4_msg_mib *icmpmsgstat, int *flag)
{
    int ret;
...
int linux_read_icmp6_parse (struct icmp6_mib *icmp6stat, struct icmp6_msg_mib *icmp6msgstat, int *support)
{
#ifdef NETSNMP_ENABLE_IPV6
    FILE *in;
...
#ifdef NETSNMP_ENABLE_IPV6
    DEBUGMSGTL (("mibII/kernel_linux/icmp6stats", "Reading /proc/net/snmp6 stats\n"));
    if (NULL == (in = fopen ("/proc/net/snmp6", "r")))
    {
        DEBUGMSGTL (("mibII/kernel_linux/icmp6stats", "Failed to open /proc/net/snmp6\n"));
        return -1;
    }
    ...
    while (NULL != fgets (line, sizeof (line), in))
    {
        ...
        *endp = '\0';
        ...
        if (!match)
            DEBUGMSGTL (("mibII/kernel_linux/icmp6stats", "%s is an unknown tag\n", line));
    }
...
int linux_read_icmp6_msg_stat (struct icmp6_mib *icmp6stat, struct icmp6_msg_mib *icmp6msgstat, int *support)
{
...
int linux_read_udp6_stat (struct udp6_mib *udp6stat)
{
...
#ifdef NETSNMP_ENABLE_IPV6
    DEBUGMSGTL (("mibII/kernel_linux/udp6stats", "Reading /proc/net/snmp6 stats\n"));
    if (NULL == (in = fopen ("/proc/net/snmp6", "r")))
    {
        DEBUGMSGTL (("mibII/kernel_linux/udp6stats", "Failed to open /proc/net/snmp6\n"));
        return -1;
    }
    ...
    while (NULL != fgets (line, sizeof (line), in))
    {
        ...
        *endp = '\0';
        ...
        {
            DEBUGMSGTL (("mibII/kernel_linux/udp6stats", "%s is an unknown tag\n", line));
        }
    }
