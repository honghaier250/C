...
u_char *var_atEntry (struct variable *vp,
...
static int AT_Cmp (void *addr, void *ep)
{
    ...
    index = netsnmp_access_interface_index_find (mp->ipNetToMediaIfIndex.o_bytes);
#else
    index = Interface_Index_By_Name (mp->ipNetToMediaIfIndex.o_bytes, mp->ipNetToMediaIfIndex.o_length);
#endif
    DEBUGMSGTL (("mibII/at", "......... AT_Cmp %lx<>%lx %d<>%d (%.5s)\n",
                 (unsigned long) mp->ipNetToMediaNetAddress,
                 (unsigned long) ((if_ip_t *) addr)->ipAddr,
                 ((if_ip_t *) addr)->ifIdx, index, mp->ipNetToMediaIfIndex.o_bytes));
...
#define AT_IFINDEX_OFF    10
    ...
    for (NextAddr.ipAddr = (u_long) - 1, NextAddr.ifIdx = 255, req_type =
         GET_FIRST;;
         NextAddr.ipAddr = entry.ipNetToMediaNetAddress, NextAddr.ifIdx = current[AT_IFINDEX_OFF], req_type = GET_NEXT)
    {
        if (getMibstat (MIB_IP_NET, &entry, sizeof (mib2_ipNetToMediaEntry_t), req_type, &AT_Cmp, &NextAddr) != 0)
            break;
#ifdef NETSNMP_INCLUDE_IFTABLE_REWRITES
        entry.ipNetToMediaIfIndex.o_bytes[entry.ipNetToMediaIfIndex.o_length] = '\0';
        current[AT_IFINDEX_OFF] = netsnmp_access_interface_index_find (entry.ipNetToMediaIfIndex.o_bytes);
#else
        current[AT_IFINDEX_OFF] =
            Interface_Index_By_Name (entry.ipNetToMediaIfIndex.o_bytes, entry.ipNetToMediaIfIndex.o_length);
#endif
        if (current[6] == 3)
        {                        /* AT group oid */
    ...
    *write_method = 0;
    switch (vp->magic)
    {
        case IPMEDIAIFINDEX:
            *var_len = sizeof long_return;
#ifdef NETSNMP_INCLUDE_IFTABLE_REWRITES
            Lowentry.ipNetToMediaIfIndex.o_bytes[Lowentry.ipNetToMediaIfIndex.o_length] = '\0';
            long_return = netsnmp_access_interface_index_find (Lowentry.ipNetToMediaIfIndex.o_bytes);
#else
    ...
    while (fgets (line, sizeof (line), in))
    {
        ...
        if (i >= arptab_curr_max_size)
        {
            struct arptab *newtab = (struct arptab *)
                realloc (at, (sizeof (struct arptab) * (arptab_curr_max_size + ARP_CACHE_INCR)));
            if (newtab == at)
            {
                snmp_log (LOG_ERR,
                          "Error allocating more space for arpcache.  "
                          "Cache will continue to be limited to %d entries", arptab_curr_max_size);
                break;
            }
            else
            {
...
#ifdef ARP_SCAN_FOUR_ARGUMENTS
static int ARP_Scan_Next (in_addr_t * IPAddr, char *PhysAddr, int *PhysAddrLen, u_long * ifType, u_short * ifIndex)
#else
static int ARP_Scan_Next (in_addr_t * IPAddr, char *PhysAddr, int *PhysAddrLen, u_long * ifType)
#endif
{
#ifndef NETSNMP_CAN_USE_SYSCTL
#ifdef linux
    if (arptab_current < arptab_size)
    {
        ...
        *ifIndex = at[arptab_current].if_index;
        memcpy (PhysAddr, &at[arptab_current].at_enaddr, sizeof (at[arptab_current].at_enaddr));
    ...
    if (arptab_current < arptab_size)
    {
        ...
        *IPAddr = at[arptab_current].NetAddr;
        memcpy (PhysAddr, at[arptab_current].PhysAddr.o_bytes, at[arptab_current].PhysAddr.o_length);
    ...
    while (arptab_current < arptab_size)
    {
        ...
        if (at_ptr == (auto_nlist_value (ARPTAB_SYMBOL) + arptab_current * sizeof (struct arphd)))
        {
            /*
             * Usused 
             */
...
write_arp (int action,
           u_char * var_val, u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t length)
{
        ...
        case UNDO:
            ...
            if (oldarp_row != NULL)
            {
                ...
                if (oldarp_row->dwAddr != arp_row->dwAddr)
                {
                    ...
                    {
                        snmp_log (LOG_ERR, "Error while deleting added row, status : %lu\n", status);
                        retval = SNMP_ERR_UNDOFAILED;
                    }
                }
                free (oldarp_row);
        ...
        case COMMIT:
            ...
            if (create_flag)
            {
                if (arp_row->dwPhysAddrLen != 0)
                {
                    ...
                    {
                        snmp_log (LOG_ERR, "Inside COMMIT: CreateIpNetEntry failed, status %lu\n", status);
                        retval = SNMP_ERR_COMMITFAILED;
                    }
                }
                else
