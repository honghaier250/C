...
void init_udp (void)
{
    ...
    netsnmp_inject_handler (reginfo,
                            netsnmp_get_cache_handler (UDP_STATS_CACHE_TIMEOUT,
                                                       udp_load, udp_free, udp_oid, OID_LENGTH (udp_oid)));
#endif

    REGISTER_SYSOR_ENTRY (udp_module_oid, "The MIB module for managing UDP implementations");
    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                DEBUGMSGOID (("mibII/udpScalar", requestvb->name, requestvb->name_length));
...
int udp_load (netsnmp_cache * cache, void *vmagic)
{
     ...
     */
    ret_value = getMibstat (MIB_UDP, &udpstat, sizeof (mib2_udp_t), GET_FIRST, &Get_everything, NULL);
