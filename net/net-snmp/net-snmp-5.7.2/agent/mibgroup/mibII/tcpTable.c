...
netsnmp_feature_child_of (tcptable_all, libnetsnmpmibs) netsnmp_feature_child_of (tcp_count_connections, tcptable_all)
#ifdef hpux11
     ...
     void init_tcpTable (void)
{
    ...
    netsnmp_table_helper_add_indexes (table_info, ASN_IPADDRESS, ASN_INTEGER, ASN_IPADDRESS, ASN_INTEGER, 0);
    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                DEBUGMSGOID (("mibII/tcpTable", requestvb->name, requestvb->name_length));
                ...
                switch (subid)
                {
                    case TCPCONNSTATE:
                        ...
                        snmp_set_var_typed_value (requestvb, ASN_INTEGER, (u_char *) & state, sizeof (state));
                        break;
                    ...
                    case TCPCONNLOCALPORT:
                        ...
                        snmp_set_var_typed_value (requestvb, ASN_INTEGER, (u_char *) & port, sizeof (port));
                        break;
                                            ...
                                            netsnmp_variable_list * index, netsnmp_iterator_info * data)
{
    ...
    snmp_set_var_value (idx, (u_char *) & port, sizeof (tcp_head[i].TCPTABLE_LOCALADDRESS));
#else
    ...
    snmp_set_var_value (idx, (u_char *) & port, sizeof (tcp_head[i].TCPTABLE_REMOTEADDRESS));
#else
...
int tcpTable_load (netsnmp_cache * cache, void *vmagic)
{
    ...
    while (line == fgets (line, sizeof (line), in))
    {
        ...
        if (6 != sscanf (line,
                         ...
                         &pcb.inp_laddr.s_addr, &lp, &pcb.inp_faddr.s_addr, &fp, &state, &uid))
            continue;
...
{
    if (memcmp ((mib2_tcpConnEntry_t *) ep, (mib2_tcpConnEntry_t *) addr, sizeof (mib2_tcpConnEntry_t)) == 0)
        return (0);
    else
        return (1);
}
...
int tcpTable_load (netsnmp_cache * cache, void *vmagic)
{
    ...
    if (getMibstat (MIB_TCP_CONN, &entry, sizeof (mib2_tcpConnEntry_t), GET_FIRST, &TCP_Cmp, &entry) != 0)
    {
    ...
    while (1)
    {
        ...
        if (getMibstat (MIB_TCP_CONN, &entry, sizeof (mib2_tcpConnEntry_t), GET_NEXT, &TCP_Cmp, &entry) != 0)
            break;
    }
...
int tcpTable_load (netsnmp_cache * cache, void *vmagic)
{
     ...
     */
    len = 0;
    if (sysctl (sname, 4, 0, &len, 0, 0) < 0 || (tcpcb_buf = malloc (len)) == NULL)
        return -1;
