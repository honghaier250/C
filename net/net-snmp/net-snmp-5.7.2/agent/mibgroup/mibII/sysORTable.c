     ...
     static void register_foreach (const struct sysORTable *data, void *dummy)
{
    ...
    if (!entry)
    {
        snmp_log (LOG_ERR, "could not allocate storage, sysORTable is inconsistent\n");
    }
    else
    {
        ...
        do
        {
            ...
            for (value = (sysORTable_entry *) it->curr (it);
                 value && value->sysORIndex < cur; value = (sysORTable_entry *) ITERATOR_NEXT (it))
            {
            }
...
static int unregister_cb (int major, int minor, void *serv, void *client)
{
    ...
    DEBUGMSGTL (("mibII/sysORTable/unregister_cb", "unregister_cb(%d, %d, %p, %p)\n", major, minor, serv, client));
                    ...
                    netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    DEBUGMSGTL (("mibII/sysORTable/sysORTable_handler", "sysORTable_handler called\n"));
    ...
    while (request)
    {
        ...
        if (NULL == (table_info = netsnmp_extract_table_info (request)))
        {
            ...
            snmp_set_var_typed_value (request->requestvb, SNMP_ERR_GENERR, NULL, 0);
        }
        else if (NULL == (table_entry = (sysORTable_entry *) netsnmp_container_table_extract_context (request)))
        {
        ...
        else
        {
            switch (table_info->colnum)
            {
                ...
                case COLUMN_SYSORDESCR:
                    ...
                    break;
                case COLUMN_SYSORUPTIME:
                    snmp_set_var_typed_integer (request->requestvb, ASN_TIMETICKS, table_entry->data->OR_uptime);
                    break;
                default:
                    netsnmp_set_request_error (reqinfo, request, SNMP_NOSUCHOBJECT);
                    break;
...
void init_sysORTable (void)
{
    ...
    sysORTable_table_info = SNMP_MALLOC_TYPEDEF (netsnmp_table_registration_info);
    ...
    netsnmp_table_helper_add_indexes (sysORTable_table_info, ASN_INTEGER,
                                                  /** index: sysORIndex */
                                      0);
    ...
    sysORLastChange_reg =
        netsnmp_create_handler_registration ("mibII/sysORLastChange", NULL,
                                             sysORLastChange_oid, OID_LENGTH (sysORLastChange_oid), HANDLER_CAN_RONLY);
    netsnmp_init_watcher_info (&sysORLastChange_winfo,
                               &sysORLastChange, sizeof (u_long), ASN_TIMETICKS, WATCHER_FIXED_SIZE);
    netsnmp_register_watched_scalar (sysORLastChange_reg, &sysORLastChange_winfo);

    sysORTable_reg =
        netsnmp_create_handler_registration ("mibII/sysORTable", sysORTable_handler,
                                             sysORTable_oid, OID_LENGTH (sysORTable_oid), HANDLER_CAN_RONLY);
    netsnmp_container_table_register (sysORTable_reg, sysORTable_table_info, table, TABLE_CONTAINER_KEY_NETSNMP_INDEX);
     ...
     * Register callbacks
     */
    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_REG_SYSOR, register_cb, NULL);
    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_UNREG_SYSOR, unregister_cb, NULL);

#ifdef USING_MIBII_SYSTEM_MIB_MODULE
    if (++system_module_count == 3)
        REGISTER_SYSOR_TABLE (system_module_oid, system_module_oid_len, "The MIB module for SNMPv2 entities");
#endif
}
...
void shutdown_sysORTable (void)
{
    ...
    snmp_unregister_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_UNREG_SYSOR, unregister_cb, NULL, 1);
    snmp_unregister_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_REG_SYSOR, register_cb, NULL, 1);
