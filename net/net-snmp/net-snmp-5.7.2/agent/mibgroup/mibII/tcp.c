...
void init_tcp (void)
{
    ...
    netsnmp_inject_handler (reginfo,
                            netsnmp_get_cache_handler (TCP_STATS_CACHE_TIMEOUT,
                                                       tcp_load, tcp_free, tcp_oid, OID_LENGTH (tcp_oid)));
#endif

    REGISTER_SYSOR_ENTRY (tcp_module_oid, "The MIB module for managing TCP implementations");
    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                DEBUGMSGOID (("mibII/tcpScalar", requestvb->name, requestvb->name_length));
...
int tcp_load (netsnmp_cache * cache, void *vmagic)
{
     ...
     */
    ret_value = getMibstat (MIB_TCP, &tcpstat, sizeof (mib2_tcp_t), GET_FIRST, &Get_everything, NULL);
