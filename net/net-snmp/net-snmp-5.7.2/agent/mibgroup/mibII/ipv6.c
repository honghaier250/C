...
#endif
     static int header_ipv6 (register struct variable *, oid *, size_t *, int, size_t *, WriteMethod **);

     static int header_ipv6_scan (register struct variable *, oid *, size_t *, int, size_t *, WriteMethod **, int, int);
...
#if 0
config_load_mib (MIB .55 .1, 8, ipv6_variables) config_add_mib (IPV6 - TC) config_add_mib (IPV6 - MIB)
#endif
...
#if 0
config_load_mib (MIB .56 .1, 8, ipv6icmp_variables) config_add_mib (IPV6 - ICMP - MIB)
#endif
...
#if 0
config_load_mib (1.3 .6 .1 .3 .87 .1, 7, ipv6udp_variables) config_add_mib (IPV6 - UDP - MIB)
#endif
...
#if 0
config_load_mib (1.3 .6 .1 .3 .86 .1, 7, ipv6tcp_variables) config_add_mib (IPV6 - TCP - MIB)
#endif
     void init_ipv6 (void)
{
    /*
     ...
     */
    REGISTER_MIB ("mibII/ipv6", ipv6_variables, variable3, ipv6_variables_oid);
    REGISTER_MIB ("mibII/icmpv6", ipv6icmp_variables, variable3, ipv6icmp_variables_oid);
    REGISTER_MIB ("mibII/ipv6udp", ipv6udp_variables, variable2, ipv6udp_variables_oid);
    REGISTER_MIB ("mibII/ipv6tcp", ipv6tcp_variables, variable2, ipv6tcp_variables_oid);
}

static int header_ipv6 (register struct variable *vp,
                        /*
...
static int header_ipv6_scan (register struct variable *vp,
                             /*
                              * IN - pointer to variable entry that points here 
                              */
                             oid * name,    /* IN/OUT - input name requested, output name found */
...
static int if_countifindex (void)
{
#ifndef HAVE_IF_NAMEINDEX
    return -1;
#else
...
static char *if_getname (int idx)
{
#ifndef HAVE_IF_NAMEINDEX
    return NULL;
#else
...
#ifdef notused
static int if_getindex (const char *name)
{
#ifndef HAVE_IF_NAMEINDEX
    return -1;
#else
 ...
 * KAME dependent part 
 */
static int if_getifnet (int idx, struct ifnet *result)
{
    caddr_t q;
...
static int if_getifmibdata (int idx, struct ifmibdata *result)
{
    int mib[6] = {
...
u_char *var_ipv6 (register struct variable * vp,
                  oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    int i;

    if (header_ipv6 (vp, name, length, exact, var_len, write_method) == MATCH_FAILED)
    {
        return NULL;
    }
#if defined(HAVE_SYS_SYSCTL_H) && defined(CTL_NET)
     ...
     */
    {
        ...
        switch (vp->magic)
        {
            case IPV6FORWARDING:
                ...
                if (sysctl (name, sizeof (name) / sizeof (name[0]), &result, &resultsiz, 0, 0) < 0)
                {
                    DEBUGMSGTL (("mibII/ipv6", "sysctl(CTL_NET, PF_INET6, IPPROTO_IPV6, %s)\n", namestr));
                    break;
                }
                else
                {
...
u_char *var_ifv6Entry (register struct variable * vp,
                       oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
#ifndef HAVE_IF_NAMEINDEX
    return NULL;
    ...
    if (header_ipv6_scan (vp, name, length, exact, var_len, write_method, 1, max) == MATCH_FAILED)
    {
        return NULL;
    }
...
u_char *var_icmpv6Entry (register struct variable * vp,
                         oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
#ifndef HAVE_IF_NAMEINDEX
    return NULL;
...
u_char *var_udp6 (register struct variable * vp,
                  oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    oid newname[MAX_OID_LEN];
        ...
        )
    {
        ...
        if (IN6_IS_ADDR_LINKLOCAL (&in6pcb.inp_laddr6))
            newname[j++] = ntohs (*(uint16_t *) & in6pcb.inp_laddr6.s6_addr[2]);
        else
            newname[j++] = 0;
#else
        for (i = 0; i < sizeof (struct in6_addr); i++)
        ...
        if (IN6_IS_ADDR_LINKLOCAL (&in6pcb.in6p_laddr))
            newname[j++] = ntohs (*(uint16_t *) & in6pcb.in6p_laddr.s6_addr[2]);
        else
            newname[j++] = 0;
#endif
         /*XXX*/ DEBUGMSGTL (("mibII/ipv6", "var_udp6 new: %d %d ", (int) vp->namelen, j));
...
#if defined(openbsd4)
            if (IN6_IS_ADDR_LINKLOCAL (&in6pcb.inp_laddr6))
                long_return = ntohs (*(uint16_t *) & in6pcb.inp_laddr6.s6_addr[2]);
            else
                long_return = 0;
#else
            if (IN6_IS_ADDR_LINKLOCAL (&in6pcb.in6p_laddr))
                long_return = ntohs (*(uint16_t *) & in6pcb.in6p_laddr.s6_addr[2]);
            else
                long_return = 0;
#endif
            return (u_char *) & long_return;
...
#ifdef TCP6
u_char *var_tcp6 (register struct variable * vp,
                  oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    oid newname[MAX_OID_LEN];
        ...
        )
    {
        ...
        DEBUGMSGTL (("mibII/ipv6", "var_tcp6 new: %d %d ", (int) vp->namelen, j));
        DEBUGMSGOID (("mibII/ipv6", newname, j));
        DEBUGMSG (("mibII/ipv6", " %d\n", exact));
...
static void linux_read_ip6_stat (struct ip6_mib *ip6stat)
{
    if (!ip6stat)
        return;
        ...
        case IPV6IFPHYSADDRESS:
            {
                ...
                else
                {
                    ...
                    *var_len = (buf.ifr_hwaddr.sa_data[0] |
                                ...
                                buf.ifr_hwaddr.sa_data[4] | buf.ifr_hwaddr.sa_data[5]) ? 6 : 0;
                    ok = 1;
                }
                close (s);
...
struct if_nameindex *linux_if_nameindex (void)
{
    ...
    if (f)
    {
        ...
        while (!feof (f))
        {
            if (fscanf (f, "%*s %lx %*x %*x %*x %s", &if_index, if_name) != 2)
                continue;
            if (if_index == 0)
                continue;
            ...
            if (maxidx < 0 || maxidx < if_index)
            {
                ...
                tmp = realloc (iflist, (sizeof (struct if_nameindex)) * (last_if_count + 2));
                if (!tmp)
                {
