    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            for (request = requests; request; request = request->next)
            {
                ...
                DEBUGMSGOID (("mibII/udpTable", requestvb->name, requestvb->name_length));
                ...
                switch (subid)
                {
                    case UDPLOCALADDRESS:
                        ...
                        snmp_set_var_typed_value (requestvb, ASN_IPADDRESS, (u_char *) & addr, sizeof (uint32_t));
#else
                                            ...
                                            netsnmp_variable_list * index, netsnmp_iterator_info * data)
{
    ...
    snmp_set_var_value (index, (u_char *) & port, sizeof (udp_head[i].UDPTABLE_LOCALADDRESS));
#else
...
int udpTable_load (netsnmp_cache * cache, void *vmagic)
{
    ...
    while (line == fgets (line, sizeof (line), in))
    {
        ...
        if (3 != sscanf (line, "%*d: %x:%x %*x:%*x %x", &pcb.inp_laddr.s_addr, &lport, &state))
            continue;
...
{
    if (memcmp ((mib2_udpEntry_t *) ep, (mib2_udpEntry_t *) addr, sizeof (mib2_udpEntry_t)) == 0)
        return (0);
    else
        return (1);
}
...
int udpTable_load (netsnmp_cache * cache, void *vmagic)
{
    ...
    if (getMibstat (MIB_UDP_LISTEN, &entry, sizeof (mib2_udpEntry_t), GET_FIRST, &UDP_Cmp, &entry) != 0)
    {
    ...
    while (1)
    {
        ...
        if (getMibstat (MIB_UDP_LISTEN, &entry, sizeof (mib2_udpEntry_t), GET_NEXT, &UDP_Cmp, &entry) != 0)
            break;
    }
...
int udpTable_load (netsnmp_cache * cache, void *vmagic)
{
     ...
     */
    len = 0;
    if (sysctl (sname, 4, 0, &len, 0, 0) < 0 || (udpcb_buf = malloc (len)) == NULL)
        return -1;
