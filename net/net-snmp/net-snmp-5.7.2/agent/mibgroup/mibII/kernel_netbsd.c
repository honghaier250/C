...
int netbsd_read_icmp_stat (struct icmp_mib *mib)
{
    ...
    mib->icmpInMsgs = icmpstat[ICMP_STAT_BADCODE]
        + icmpstat[ICMP_STAT_TOOSHORT] + icmpstat[ICMP_STAT_CHECKSUM] + icmpstat[ICMP_STAT_BADLEN];
    ...
    mib->icmpInTimestampReps = icmpstat[ICMP_STAT_INHIST + ICMP_TSTAMPREPLY];
    ...
    mib->icmpOutMsgs = icmpstat[ICMP_STAT_OLDSHORT] + icmpstat[ICMP_STAT_OLDICMP];
    for (i = 0; i <= ICMP_MAXTYPE; i++)
        mib->icmpOutMsgs += icmpstat[ICMP_STAT_OUTHIST + i];
    mib->icmpOutErrors = icmpstat[ICMP_STAT_OLDSHORT] + icmpstat[ICMP_STAT_OLDICMP];
    ...
    mib->icmpOutSrcQuenchs = icmpstat[ICMP_STAT_OUTHIST + ICMP_SOURCEQUENCH];
    ...
    mib->icmpOutTimestampReps = icmpstat[ICMP_STAT_OUTHIST + ICMP_TSTAMPREPLY];
...
int netbsd_read_ip_stat (struct ip_mib *mib)
{
    ...
    mib->ipInHdrErrors = ipstat[IP_STAT_BADSUM]
        + ipstat[IP_STAT_TOOSHORT] + ipstat[IP_STAT_TOOSMALL] + ipstat[IP_STAT_BADHLEN] + ipstat[IP_STAT_BADLEN];
    ...
    mib->ipReasmFails = ipstat[IP_STAT_FRAGDROPPED] + ipstat[IP_STAT_FRAGTIMEOUT];
...
int netbsd_read_tcp_stat (struct tcp_mib *mib)
{
    ...
    mib->tcpOutSegs = tcpstat[TCP_STAT_SNDTOTAL] - tcpstat[TCP_STAT_SNDREXMITPACK];
    mib->tcpRetransSegs = tcpstat[TCP_STAT_SNDREXMITPACK];
    mib->tcpInErrs = tcpstat[TCP_STAT_RCVBADSUM]
        + tcpstat[TCP_STAT_RCVBADOFF] + tcpstat[TCP_STAT_RCVMEMDROP] + tcpstat[TCP_STAT_RCVSHORT];
    mib->tcpOutRsts = tcpstat[TCP_STAT_SNDCTRL] - tcpstat[TCP_STAT_CLOSED];
...
int netbsd_read_udp_stat (struct udp_mib *mib)
{
    ...
    mib->udpInErrors = udpstat[UDP_STAT_HDROPS] + udpstat[UDP_STAT_BADSUM]    /* + udpstat[UDP_STAT_DISCARD] /* FIXME */
