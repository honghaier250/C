                  ...
                  netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    switch (reqinfo->mode)
    {
        case MODE_GET:
            ...
            DEBUGMSGOID (("override", requests->requestvb->name, requests->requestvb->name_length));
            DEBUGMSG (("override", "\n"));
        ...
        case MODE_SET_RESERVE2:
            if (memdup ((u_char **) & data->set_space,
                        requests->requestvb->val.string, requests->requestvb->val_len) == SNMPERR_GENERR)
    ...
    switch (type)
    {
        ...
        case ASN_BIT_STR:
            if (buf[0] == '0' && buf[1] == 'x')
            {
                 ...
                 * hex 
                 */
                thedata->value_len = hex_to_binary2 ((u_char *) (buf + 2), strlen (buf) - 2, (char **) &thedata->value);
            }
            else
            {
        ...
        case ASN_OBJECT_ID:
            read_config_read_objid (buf, (oid **) & thedata->value, &thedata->value_len);
