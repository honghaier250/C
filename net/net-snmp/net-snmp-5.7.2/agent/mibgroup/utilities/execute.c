...
int run_shell_command (char *command, char *input, char *output, int *out_len)    /* Or realloc style ? */
{
#if HAVE_SYSTEM
    ...
    if (input)
    {
        ...
        if (output)
        {
            ...
            snprintf (shellline, sizeof (shellline), "(%s) < \"%s\" > \"%s\"", command, ifname, ofname);
...
int run_exec_command (char *command, char *input, char *output, int *out_len)    /* Or realloc style ? */
{
#if HAVE_EXECV
    int ipipe[2];
    ...
    else if (pid > 0)
    {
        ...
        {
            DEBUGMSGTL (("run:exec", "invalid params; no output will be returned\n"));
            output = NULL;
        }
        if (output)
        {
            ...
            if (0 == count)
            {
                ...
                if ((rc = waitpid (pid, &result, WNOHANG)) <= 0)
                {
                    if (rc < 0)
                    {
                        ...
                        DEBUGMSGTL (("verbose:run:exec", "      child not done!?!\n"));;
                }
                else
                {
