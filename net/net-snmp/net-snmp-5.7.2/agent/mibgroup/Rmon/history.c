        ...
        case RESERVE2:
            ...
            switch (leaf_id)
            {
                case Leaf_historyControlDataSource:
                    snmp_status = AGUTIL_get_oid_value (var_val, var_val_type, var_val_len, &cloned_body->data_source);
                ...
                case Leaf_historyControlInterval:
                    snmp_status = AGUTIL_get_int_value (var_val, var_val_type,
                                                        ...
                                                        MAX_historyControlInterval, (long *) &cloned_body->interval);
                    if (SNMP_ERR_NOERROR != snmp_status)
                    {
                        return snmp_status;
                    }
#if 0
                    if (RMON1_ENTRY_UNDER_CREATION != hdr->status && cloned_body->interval != body->interval)
                        return SNMP_ERR_BADVALUE;
#endif
                    break;
 ...
 */
unsigned char *var_historyControlTable (struct variable *vp,
                                        oid * name,
...
static void compute_delta (ETH_STATS_T * delta, ETH_STATS_T * newval, ETH_STATS_T * prevval)
{
...
static void history_get_backet (unsigned int clientreg, void *clientarg)
{
...
{                                /* create the body: alloc it and set defaults */
    ...
    ROWDATAAPI_init (&body->scrlr, HIST_DEF_BUCK_REQ, MAX_BUCKETS_IN_CRTL_ENTRY, sizeof (DATA_ENTRY_T), NULL);
...
int history_Activate (RMON_ENTRY_T * eptr)
{
     ...
     */
    body->timer_id = snmp_alarm_register (body->interval, SA_REPEAT, history_get_backet, eptr);
    return 0;
}
...
int history_Copy (RMON_ENTRY_T * eptr)
{
    ...
    {
        ROWDATAAPI_set_size (&body->scrlr, clone->scrlr.data_requested, (u_char) (RMON1_ENTRY_VALID == eptr->status));
    }
    ...
    if (body->interval != clone->interval)
    {
        ...
        {
            snmp_alarm_unregister (body->timer_id);
            body->timer_id = snmp_alarm_register (clone->interval, SA_REPEAT, history_get_backet, eptr);
        }
    ...
    if (snmp_oid_compare
        (clone->data_source.objid, clone->data_source.length, body->data_source.objid, body->data_source.length))
    {
 ...
 */
unsigned char *var_etherHistoryTable (struct variable *vp,
                                      oid * name,
                                      size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    static long long_ret;
    ...
    *write_method = NULL;
    hdr = ROWDATAAPI_header_DataEntry (vp, name, length, exact, var_len,
                                       table_ptr, &history_extract_scroller, sizeof (DATA_ENTRY_T), &theBucket);
    if (!hdr)
        return NULL;
...
int add_hist_entry (int ctrl_index, int ifIndex, u_long interval, u_long requested)
{
...
{
    REGISTER_MIB ("historyControlTable", historyControlTable_variables, variable2, historyControlTable_variables_oid);
    REGISTER_MIB ("etherHistoryTable", etherHistoryTable_variables, variable2, etherHistoryTable_variables_oid);

    ROWAPI_init_table (&HistoryCtrlTable, "History", 0, &history_Create, NULL,    /* &history_Clone, */
                       NULL,    /* &history_Delete, */
                       &history_Validate, &history_Activate, &history_Deactivate, &history_Copy);
