 ...
 */
     ...
     static int data_destructor (SCROLLER_T * scrlr, void *free_me)
{
...
{                                /* create the body: alloc it and set defaults */
    ...
    ROWDATAAPI_init (&body->scrlr,
                     MAX_LOG_ENTRIES_PER_CTRL, MAX_LOG_ENTRIES_PER_CTRL, sizeof (DATA_ENTRY_T), &data_destructor);
...
{                                /* init logTable */
    ...
    ROWDATAAPI_set_size (&body->scrlr, body->scrlr.data_requested, (u_char) (RMON1_ENTRY_VALID == eptr->status));
            ...
            switch (leaf_id)
            {
                ...
                case Leaf_event_description:
                    ...
                    snmp_status = AGUTIL_get_string_value (var_val, var_val_type,
                                                           var_val_len, MAX_event_description, 1, NULL, char_temp);
                ...
                case Leaf_event_type:
                    snmp_status = AGUTIL_get_int_value (var_val, var_val_type,
                                                        var_val_len, EVENT_NONE, EVENT_LOG_AND_TRAP, &long_temp);
                    if (SNMP_ERR_NOERROR != snmp_status)
                    {
                        return snmp_status;
                    }
                ...
                case Leaf_event_community:
                    ...
                    if (!char_temp)
                        return SNMP_ERR_TOOBIG;
                    snmp_status = AGUTIL_get_string_value (var_val, var_val_type,
                                                           var_val_len, MAX_event_community, 1, NULL, char_temp);
                    if (SNMP_ERR_NOERROR != snmp_status)
                    {
                        AGFREE (char_temp);
                        return snmp_status;
                ...
                case Leaf_eventStatus:
                    snmp_status = AGUTIL_get_int_value (var_val, var_val_type,
                                                        var_val_len,
                                                        RMON1_ENTRY_VALID, RMON1_ENTRY_INVALID, &long_temp);
                    if (SNMP_ERR_NOERROR != snmp_status)
                    {
                        return snmp_status;
                    }
...
unsigned char *var_eventTable (struct variable *vp,
...
unsigned char *var_logTable (struct variable *vp,
                             oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    static long long_ret;
    ...
    *write_method = NULL;
    hdr = ROWDATAAPI_header_DataEntry (vp, name, length, exact, var_len,
                                       table_ptr, &event_extract_scroller, sizeof (DATA_ENTRY_T), &theEntry);
    if (!hdr)
        return NULL;
...
static char *create_explanaition (CRTL_ENTRY_T * evptr, u_char is_rising,
                                  u_long alarm_index, u_long event_index,
                                  oid * alarmed_var,
                                  size_t alarmed_var_length,
                                  u_long value, u_long the_threshold, u_long sample_type, char *alarm_descr)
{
#define UNEQ_LENGTH    (1 + 11 + 4 + 11 + 1 + 20)
    ...
    snprintf (expl, UNEQ_LENGTH, "=%ld %s= %ld :%ld, %ld",
              (unsigned long) value,
              is_rising ? ">" : "<", (unsigned long) the_threshold, (long) alarm_index, (long) event_index);
...
event_send_trap (CRTL_ENTRY_T * evptr, u_char is_rising,
                 u_int alarm_index,
                 u_int value, u_int the_threshold, oid * alarmed_var, size_t alarmed_var_length, u_int sample_type)
{
    static oid objid_snmptrap[] = { 1, 3, 6, 1, 6, 3, 1, 1, 4, 1, 0 };
    static oid rmon1_trap_oid[] = { 1, 3, 6, 1, 2, 1, 16, 0, 0 };
    static oid alarm_index_oid[] = { 1, 3, 6, 1, 2, 1, 16, 3, 1, 1, 1, 0 };
    static oid alarmed_var_oid[] = { 1, 3, 6, 1, 2, 1, 16, 3, 1, 1, 3, 0 };
    static oid sample_type_oid[] = { 1, 3, 6, 1, 2, 1, 16, 3, 1, 1, 4, 0 };
    ...
    snmp_varlist_add_variable (&var_list, objid_snmptrap,
                               ...
                               ASN_OBJECT_ID, (u_char *) rmon1_trap_oid, sizeof (rmon1_trap_oid));
    snmp_varlist_add_variable (&var_list, alarm_index_oid,
                               OID_LENGTH (alarm_index_oid), ASN_INTEGER, (u_char *) & alarm_index, sizeof (u_int));
    snmp_varlist_add_variable (&var_list, alarmed_var_oid,
                               OID_LENGTH (alarmed_var_oid),
                               ASN_OBJECT_ID, (u_char *) alarmed_var, alarmed_var_length * sizeof (oid));
    snmp_varlist_add_variable (&var_list, sample_type_oid,
                               OID_LENGTH (sample_type_oid), ASN_INTEGER, (u_char *) & sample_type, sizeof (u_int));
    snmp_varlist_add_variable (&var_list, value_oid,
                               OID_LENGTH (value_oid), ASN_INTEGER, (u_char *) & value, sizeof (u_int));
    snmp_varlist_add_variable (&var_list, threshold_oid,
                               OID_LENGTH (threshold_oid), ASN_INTEGER, (u_char *) & the_threshold, sizeof (u_int));
...
static void event_save_log (CRTL_ENTRY_T * body, char *event_descr)
{
...
event_api_send_alarm (u_char is_rising,
                      ...
                      oid * alarmed_var,
                      size_t alarmed_var_length,
                      u_long sample_type, u_long value, u_long the_threshold, char *alarm_descr)
{
    RMON_ENTRY_T *eptr;
    ...
    if (EVENT_TRAP == evptr->event_type || EVENT_LOG_AND_TRAP == evptr->event_type)
    {
        event_send_trap (evptr, is_rising, alarm_index, value,
                         the_threshold, alarmed_var, alarmed_var_length, sample_type);
    }
    ...
    if (EVENT_LOG == evptr->event_type || EVENT_LOG_AND_TRAP == evptr->event_type)
    {
        ...
        explain = create_explanaition (evptr, is_rising,
                                       ...
                                       alarmed_var, alarmed_var_length, value, the_threshold, sample_type, alarm_descr);
        /*
...
int add_event_entry (int ctrl_index, char *event_description, EVENT_TYPE_T event_type, char *event_community)
{
...
{
    REGISTER_MIB ("eventTable", eventTable_variables, variable2, eventTable_variables_oid);
