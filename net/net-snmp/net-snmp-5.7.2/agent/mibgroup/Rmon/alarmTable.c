...
netsnmp_feature_require (check_vb_type_and_max_size) netsnmp_feature_require (table_tdata_extract_table)
...
void initialize_table_alarmTable (void)
{
    ...
    reg =
        netsnmp_create_handler_registration ("alarmTable",
                                             alarmTable_handler,
                                             alarmTable_oid, alarmTable_oid_len, HANDLER_CAN_RWRITE);
...
event_api_send_alarm (u_char is_rising,
                      ...
                      u_long sample_type, u_long value, u_long the_threshold, const char *alarm_descr);
...
void alarmTable_run (unsigned int reg, void *clientarg)
{
    ...
    snmp_set_var_objid (var, entry->alarmVariable, entry->alarmVariable_len);
    ...
    if (ALARM_RISING != entry->prev_alarm && entry->alarmValue >= entry->alarmRisingThreshold)
    {
        if (SNMP_ERR_NOERROR == event_api_send_alarm (1, entry->alarmIndex,
                                                      ...
                                                      entry->alarmValue, entry->alarmRisingThreshold, "Rising"))
            entry->prev_alarm = ALARM_RISING;
        else
            snmp_log (LOG_ERR, "failed to send rising alarm\n");
    }
    else if (ALARM_FALLING != entry->prev_alarm && entry->alarmValue <= entry->alarmFallingThreshold)
    {
        if (SNMP_ERR_NOERROR == event_api_send_alarm (0, entry->alarmIndex,
                                                      ...
                                                      entry->alarmVariable_len,
                                                      entry->alarmSampleType,
                                                      entry->alarmValue, entry->alarmFallingThreshold, "Falling"))
...
void alarmTable_enable (struct alarmTable_entry *entry)
{
    if (!entry)
        return;
    ...
    if (entry->alarmInterval)
    {
        ...
        entry->alarm_reg = snmp_alarm_register (entry->alarmInterval, SA_REPEAT, alarmTable_run, entry);
    }
}

void alarmTable_disable (struct alarmTable_entry *entry)
{
    if (!entry)
        return;
 ...
 */
netsnmp_tdata_row *alarmTable_createEntry (netsnmp_tdata * table_data, long alarmIndex)
{
    ...
    netsnmp_tdata_row_add_index (row, ASN_INTEGER, &(entry->alarmIndex), sizeof (entry->alarmIndex));
 ...
 */
void alarmTable_removeEntry (netsnmp_tdata * table_data, netsnmp_tdata_row * row)
{
    struct alarmTable_entry *entry;
    ...
    entry = (struct alarmTable_entry *) netsnmp_tdata_remove_and_delete_row (table_data, row);
    if (entry)
    {
                    ...
                    netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_GET:
            ...
            {
                table_entry = (struct alarmTable_entry *) netsnmp_tdata_extract_entry (request);
                table_info = netsnmp_extract_table_info (request);

                switch (table_info->colnum)
                {
                    case COLUMN_ALARMINDEX:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, table_entry->alarmIndex);
                        break;
                    case COLUMN_ALARMINTERVAL:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, table_entry->alarmInterval);
                        break;
                    case COLUMN_ALARMVARIABLE:
                        snmp_set_var_typed_value (request->requestvb, ASN_OBJECT_ID,
                                                  (u_char *) table_entry->alarmVariable,
                                                  table_entry->alarmVariable_len * sizeof (oid));
                        break;
                    case COLUMN_ALARMSAMPLETYPE:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, table_entry->alarmSampleType);
                        break;
                    case COLUMN_ALARMVALUE:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, table_entry->alarmValue);
                        break;
                    case COLUMN_ALARMSTARTUPALARM:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, table_entry->alarmStartupAlarm);
                        break;
                    case COLUMN_ALARMRISINGTHRESHOLD:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, table_entry->alarmRisingThreshold);
                        break;
                    ...
                    case COLUMN_ALARMOWNER:
                        snmp_set_var_typed_value (request->requestvb, ASN_OCTET_STR,
                                                  (u_char *) table_entry->alarmOwner, table_entry->alarmOwner_len);
                        break;
                    case COLUMN_ALARMSTATUS:
                        snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, table_entry->alarmStatus);
                        break;
                }
            }
            break;
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (table_info->colnum)
                {
                    ...
                    case COLUMN_ALARMSTATUS:
                        if (!table_row)
                        {
                            table_row = alarmTable_createEntry (table_data, *table_info->indexes->val.integer);
                            if (table_row)
                            {
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                switch (table_info->colnum)
                {
                    case COLUMN_ALARMINTERVAL:
                        ...
                        break;
                    case COLUMN_ALARMVARIABLE:
                        table_entry->alarmVariable_len = request->requestvb->val_len / sizeof (oid);
                        memset (table_entry->alarmVariable, 0, sizeof (table_entry->alarmVariable));
                        memcpy (table_entry->alarmVariable, request->requestvb->val.objid, request->requestvb->val_len);
                        break;
                    case COLUMN_ALARMSAMPLETYPE:
                    ...
                    case COLUMN_ALARMFALLINGEVENTINDEX:
                        ...
                        break;
                    case COLUMN_ALARMOWNER:
                        memcpy (table_entry->old_alarmOwner, table_entry->alarmOwner, sizeof (table_entry->alarmOwner));
                        table_entry->old_alarmOwner_len = table_entry->alarmOwner_len;
                        memset (table_entry->alarmOwner, 0, sizeof (table_entry->alarmOwner));
                        memcpy (table_entry->alarmOwner, request->requestvb->val.string, request->requestvb->val_len);
                    ...
                    case COLUMN_ALARMVARIABLE:
                        ...
                        else
                        {
                            memcpy (table_entry->alarmVariable,
                                    table_entry->old_alarmVariable, sizeof (table_entry->alarmVariable));
                            memset (table_entry->old_alarmVariable, 0, sizeof (table_entry->alarmVariable));
                            table_entry->alarmVariable_len = table_entry->old_alarmVariable_len;
                        }
                        break;
                    case COLUMN_ALARMSAMPLETYPE:
                        if (table_entry && !table_entry->valid)
                    ...
                    case COLUMN_ALARMOWNER:
                        ...
                        else
                        {
                            memcpy (table_entry->alarmOwner,
                                    table_entry->old_alarmOwner, sizeof (table_entry->alarmOwner));
                            memset (table_entry->old_alarmOwner, 0, sizeof (table_entry->alarmOwner));
                            table_entry->alarmOwner_len = table_entry->old_alarmOwner_len;
                        }
                        break;
                    case COLUMN_ALARMSTATUS:
                        if (table_entry && !table_entry->valid)
