...
static int fetch_var_val (oid * name, size_t namelen, u_long * new_value)
{
    ...
    memcpy (called_var.name, tree_ptr->name_a, tree_ptr->namelen * sizeof (oid));

    if (tree_ptr->reginfo &&
        tree_ptr->reginfo->handler && tree_ptr->reginfo->handler->next && tree_ptr->reginfo->handler->next->myvoid)
    {
    ...
    if (s_var_ptr)
    {
        ...
        {
            called_var.namelen = tree_ptr->namelen;
            ...
            access = (*(s_var_ptr->findVar)) (&called_var, name, &namelen, 1, &var_len, &write_method);

            if (access && snmp_oid_compare (name, namelen, tree_ptr->end_a, tree_ptr->end_len) > 0)
            {
...
int alarm_Validate (RMON_ENTRY_T * eptr)
{
    ...
    {
        ag_trace ("alarm_Validate failed: %lu must be > %lu", body->rising_threshold, body->falling_threshold);
...
int alarm_Activate (RMON_ENTRY_T * eptr)
{
    ...
    kuku_cnt = 0;
#endif
    ierr = fetch_var_val (body->var_name.objid, body->var_name.length, &body->last_abs_value);
    if (SNMP_ERR_NOERROR != ierr)
    {
    ...
    if (SAMPLE_TYPE_ABSOLUTE != body->sample_type)
    {
         ...
         * check startup alarm 
         */
        if (ALARM_RISING == body->startup_type || ALARM_BOTH == body->startup_type)
        {
            if (body->last_abs_value >= body->rising_threshold)
            {
                event_api_send_alarm (1, eptr->ctrl_index,
                                      ...
                                      body->var_name.objid,
                                      body->var_name.length,
                                      ALARM_RISING, body->value, body->rising_threshold, "Startup Rising");
            }
        }

        if (ALARM_FALLING == body->startup_type || ALARM_BOTH == body->startup_type)
        {
            if (body->last_abs_value <= body->falling_threshold)
            {
                event_api_send_alarm (0, eptr->ctrl_index,
                                      ...
                                      body->var_name.objid,
                                      body->var_name.length,
                                      ALARM_RISING, body->value, body->falling_threshold, "Startup Falling");
            }
        }

    }

    body->timer_id = snmp_alarm_register (body->interval, SA_REPEAT, alarm_check_var, eptr);
    return 0;
}
