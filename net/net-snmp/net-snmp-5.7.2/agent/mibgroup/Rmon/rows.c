...
static void rowapi_too_long_creation_callback (unsigned int clientreg, void *clientarg)
{
    RMON_ENTRY_T *eptr;
...
int ROWAPI_new (TABLE_DEFINTION_T * table_ptr, u_long ctrl_index)
{
    ...
    enew->timer_id = snmp_alarm_register (MAX_CREATION_TIME, 0, rowapi_too_long_creation_callback, enew);
    ag_trace ("Entry %ld in %s has been created", enew->ctrl_index, table_ptr->name);
    return 0;
}
...
ROWAPI_init_table (TABLE_DEFINTION_T * table_ptr,
                   ...
                   ENTRY_CALLBACK_T * ClbkActivate, ENTRY_CALLBACK_T * ClbkDeactivate, ENTRY_CALLBACK_T * ClbkCopy)
{
    table_ptr->name = name;
    if (!table_ptr->name)
...
int ROWAPI_action_check (TABLE_DEFINTION_T * table_ptr, u_long ctrl_index)
{
    ...
    if (RMON1_ENTRY_UNDER_CREATION != eptr->status)
    {
         ...
         */
        if (eptr->new_owner && (!eptr->owner || strncmp (eptr->new_owner, eptr->owner, MAX_OWNERSTRING)))
        {
        ...
        case RMON1_ENTRY_VALID:
            ...
            if (RMON1_ENTRY_UNDER_CREATION != eptr->status)
            {
                ag_trace ("Validate %s: entry %ld has wrong status %d",
                          table_ptr->name, (long) ctrl_index, (int) eptr->status);
                return SNMP_ERR_BADVALUE;
            }
...
void ROWDATAAPI_set_size (SCROLLER_T * scrlr, u_long data_requested, u_char do_allocation)
{
    long dlong;
...
RMON_ENTRY_T *ROWDATAAPI_header_DataEntry (struct variable * vp, oid * name,
                                           size_t * length, int exact,
                                           size_t * var_len,
                                           TABLE_DEFINTION_T * table_ptr,
                                           ...
                                           size_t data_size, void *entry_ptr)
{
    ...
    if (ctrl_indx)
        data_index = ((int) (vp->namelen + 1) >= (int) *length) ? 0 : name[vp->namelen + 1];
    else
        data_index = 0;
