 ...
 */
     int snmpNotifyFilterTable_init_data (snmpNotifyFilterTable_registration * snmpNotifyFilterTable_reg)
{
...
void snmpNotifyFilterTable_container_init (netsnmp_container ** container_ptr_ptr)
{
    ...
    {
        snmp_log (LOG_ERR, "bad container param to snmpNotifyFilterTable_container_init\n");
        return;
    }
...
void snmpNotifyFilterTable_container_shutdown (netsnmp_container * container_ptr)
{
    ...
    {
        snmp_log (LOG_ERR, "bad params to snmpNotifyFilterTable_container_shutdown\n");
        return;
    }
    ...
    while (1)
    {
        ...
        if (MFD_SUCCESS !=
            snmpNotifyFilterTable_indexes_set (rowreq_ctx,
                                               ...
                                               snmpNotifyFilterSubtree, snmpNotifyFilterSubtree_len))
        {
            snmp_log (LOG_ERR, "error setting index while loading " "snmpNotifyFilterTable data.\n");
 ...
 */
int snmpNotifyFilterTable_snmpNotifyFilterProfileName_check_index (snmpNotifyFilterTable_rowreq_ctx * rowreq_ctx)
{
    DEBUGMSGTL (("verbose:snmpNotifyFilterTable:snmpNotifyFilterTable_snmpNotifyFilterProfileName_check_index",
                 "called\n"));
 ...
 */
int snmpNotifyFilterSubtree_check_index (snmpNotifyFilterTable_rowreq_ctx * rowreq_ctx)
{
...
snmpNotifyFilterTable_validate_index (snmpNotifyFilterTable_registration *
                                      snmpNotifyFilterTable_reg, snmpNotifyFilterTable_rowreq_ctx * rowreq_ctx)
{
    int rc = MFD_SUCCESS;
 ...
 */
struct vacm_viewEntry *snmpNotifyFilterTable_vacm_view_subtree (const char *profile)
{
    ...
    for (i = 0, j = 0; i < s->size; ++i)
    {
        ...
        memcpy (&tmp[j].viewName[1], rowreq->tbl_idx.snmpNotifyFilterProfileName, tmp[j].viewName[0]);

        tmp[j].viewSubtree[0] = rowreq->tbl_idx.snmpNotifyFilterSubtree_len;
        memcpy (&tmp[j].viewSubtree[1], rowreq->tbl_idx.snmpNotifyFilterSubtree, tmp[j].viewSubtree[0] * sizeof (oid));
        tmp[j].viewSubtreeLen = tmp[j].viewSubtree[0] + 1;

        tmp[j].viewMaskLen = rowreq->data.snmpNotifyFilterMask_len;
        memcpy (tmp[j].viewMask, rowreq->data.snmpNotifyFilterMask, tmp[j].viewMaskLen * sizeof (oid));
