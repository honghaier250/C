...
struct targetParamTable_struct *snmpTargetParamTable_create (void)
{
    ...
    newEntry = (struct targetParamTable_struct *) malloc (sizeof (struct targetParamTable_struct));
 ...
 */
void snmpTargetParamTable_addToList (struct targetParamTable_struct *newEntry, struct targetParamTable_struct **listPtr)
{
 ...
 */
void
snmpTargetParamTable_remFromList (struct targetParamTable_struct *oldEntry, struct targetParamTable_struct **listPtr)
{
                                                              ...
                                                              oid * name, size_t * length, int exact)
{
    ...
    for (temp_struct = aPTable; temp_struct != NULL; temp_struct = temp_struct->next)
    {
...
void init_snmpTargetParamsEntry (void)
{
    ...
    REGISTER_MIB ("target/snmpTargetParamsEntry",
                  snmpTargetParamsEntry_variables, variable2, snmpTargetParamsEntry_variables_oid);

    snmpd_register_config_handler ("targetParams", snmpd_parse_config_targetParams, (void (*)(void)) 0, NULL);

    /*
     * we need to be called back later 
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_snmpTargetParamsEntry, NULL);
...
int snmpTargetParams_addParamName (struct targetParamTable_struct *entry, char *cptr)
{
    size_t len;
...
int snmpTargetParams_addMPModel (struct targetParamTable_struct *entry, char *cptr)
{
    if (cptr == NULL)
    {
...
int snmpTargetParams_addSecModel (struct targetParamTable_struct *entry, char *cptr)
{
    if (cptr == NULL)
    {
...
int snmpTargetParams_addSecName (struct targetParamTable_struct *entry, char *cptr)
{
    if (cptr == NULL)
    {
...
int snmpTargetParams_addSecLevel (struct targetParamTable_struct *entry, char *cptr)
{
    if (cptr == NULL)
    {
...
int snmpTargetParams_addStorageType (struct targetParamTable_struct *entry, char *cptr)
{
    if (cptr == NULL)
    {
     ...
     */
             ...
             (entry->storageType != SNMP_STORAGE_PERMANENT) && (entry->storageType != SNMP_STORAGE_READONLY))
    {
...
int snmpTargetParams_addRowStatus (struct targetParamTable_struct *entry, char *cptr)
{
    if (cptr == NULL)
    {
     ...
     */
              ...
              != SNMP_ROW_ACTIVE) &&
             (entry->rowStatus != SNMP_ROW_NOTINSERVICE) && (entry->rowStatus != SNMP_ROW_NOTREADY))
    {
...
void snmpd_parse_config_targetParams (const char *token, char *char_ptr)
{
    ...
    DEBUGMSGTL (("snmpTargetParamsEntry",
                 ...
                 newEntry->secName, newEntry->secLevel, newEntry->storageType, newEntry->rowStatus));
 ...
 */
int store_snmpTargetParamsEntry (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    if ((curr_struct = aPTable) != NULL)
    {
        while (curr_struct != NULL)
        {
            if ((curr_struct->storageType == SNMP_STORAGE_NONVOLATILE ||
                ...
                && (curr_struct->rowStatus == SNMP_ROW_ACTIVE || curr_struct->rowStatus == SNMP_ROW_NOTINSERVICE))
            {
                snprintf (line, sizeof (line),
                          "targetParams %s %i %i %s %i %i %i\n",
                          curr_struct->paramName, curr_struct->mpModel,
                          curr_struct->secModel, curr_struct->secName,
                          curr_struct->secLevel, curr_struct->storageType, curr_struct->rowStatus);
...
u_char *var_snmpTargetParamsEntry (struct variable * vp,
                                   oid * name,
                                   size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    *var_len = sizeof (long_ret);
     ...
     */
    if ((temp_struct = search_snmpTargetParamsTable (vp->name, vp->namelen, name, length, exact)) == NULL)
    {
        return NULL;
    }
                               ...
                               u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    else if (action == RESERVE2)
    {
        ...
        params = search_snmpTargetParamsTable (snmpTargetParamsOID, snmpTargetParamsOIDLen, name, &name_len, 1);
        if (params == NULL)
        {
        ...
        else
        {
            ...
            if (params->rowStatus == SNMP_ROW_NOTREADY && snmpTargetParams_rowStatusCheck (params))
            {
    ...
    else if (action == COMMIT)
    {
        ...
        if ((params = search_snmpTargetParamsTable (snmpTargetParamsOID,
                                                    snmpTargetParamsOIDLen, name, &name_len, 1)) != NULL)
        {
    ...
    else if (action == FREE || action == UNDO)
    {
        ...
        if ((params = search_snmpTargetParamsTable (snmpTargetParamsOID,
                                                    snmpTargetParamsOIDLen, name, &name_len, 1)) != NULL)
        {
            if (params->storageType != SNMP_STORAGE_READONLY && params->rowStatus != SNMP_ROW_ACTIVE)
            {
                params->mpModel = old_mp;
                if (params->rowStatus == SNMP_ROW_NOTINSERVICE && !snmpTargetParams_rowStatusCheck (params))
                {
                                ...
                                u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1)
    {
        if (var_val_type != ASN_INTEGER)
        {
            DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamsSecModel: not ASN_INTEGER\n"));
            return SNMP_ERR_WRONGTYPE;
        }
        if (var_val_len != sizeof (long))
        {
            DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamsSecModel: bad length\n"));
            return SNMP_ERR_WRONGLENGTH;
        }
        if (long_ret <= 0)
        {
            DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamsSecModel: secModel out of range\n"));
    ...
    else if (action == RESERVE2)
    {
        ...
        if (params == NULL)
        {
            DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamsSecModel: BAD OID\n"));
            return SNMP_ERR_INCONSISTENTNAME;
        }
        else
        {
            ...
            if (params->storageType == SNMP_STORAGE_READONLY)
            {
                DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamSecModel: row is read only\n"));
                return SNMP_ERR_NOTWRITABLE;
            }
            /*
             * Check if row is active.  
                                ...
                                u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1)
    {
        if (var_val_type != ASN_INTEGER)
        {
            DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamsSecLevel: not ASN_INTEGER\n"));
            return SNMP_ERR_WRONGTYPE;
        }
        if (var_val_len != sizeof (long))
        {
            DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamsSecLevel: bad length\n"));
            return SNMP_ERR_WRONGLENGTH;
        }
    ...
    else if (action == RESERVE2)
    {
        ...
        if (params == NULL)
        {
            DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamsSecLevel: BAD OID\n"));
            return SNMP_ERR_INCONSISTENTNAME;
        }
        else
        {
            ...
            if (params->storageType == SNMP_STORAGE_READONLY)
            {
                DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamSecLevel: row is read only\n"));
                return SNMP_ERR_NOTWRITABLE;
            }
            /*
             * Check if row is active.  
                                   ...
                                   u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1)
    {
        ...
        if (long_ret != SNMP_STORAGE_OTHER && long_ret != SNMP_STORAGE_VOLATILE && long_ret != SNMP_STORAGE_NONVOLATILE)
        {
            DEBUGMSGTL (("snmpTargetParamsEntry",
    ...
    else if (action == RESERVE2)
    {
        ...
        if (params == NULL)
        {
            DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamsStorageType: BAD OID\n"));
            return SNMP_ERR_INCONSISTENTNAME;
        }
        else
        {
             ...
             * Row exists, check if it is changeable.  
             */
            if (params->storageType == SNMP_STORAGE_READONLY || params->storageType == SNMP_STORAGE_PERMANENT)
            {
                DEBUGMSGTL (("snmpTargetParamsEntry",
                               ...
                               u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1)
    {
        ...
        {
            DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamsSecName: not ASN_OCTET_STR\n"));
            return SNMP_ERR_WRONGTYPE;
        }
        if (var_val_len > 255)
        {
            DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamsSecName: bad length\n"));
            return SNMP_ERR_WRONGLENGTH;
        }
    }
    else if (action == RESERVE2)
    {
        ...
        if (params == NULL)
        {
            DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamsSecName: BAD OID\n"));
            return SNMP_ERR_INCONSISTENTNAME;
        }
        else
        {
            ...
            if (params->storageType == SNMP_STORAGE_READONLY)
            {
                DEBUGMSGTL (("snmpTargetParamsEntry", "write to snmpTargetParamSecName: row is read only\n"));
                return SNMP_ERR_NOTWRITABLE;
            }
            /*
             * Check if row is active.  
    ...
    if (action == RESERVE1)
    {
        ...
        if (params != NULL)
        {
            ...
            if (params->storageType == SNMP_STORAGE_PERMANENT)
            {
                ...
                {
                    DEBUGMSGTL (("snmpTargetParamsEntry", "unable to destroy permanent row\n"));
                    return SNMP_ERR_INCONSISTENTVALUE;
                }
            }
        }
        else
        {
            ...
            if (value == SNMP_ROW_CREATEANDGO || value == SNMP_ROW_CREATEANDWAIT)
            {
                ...
                {
                    DEBUGMSGTL (("snmpTargetParamsEntry", "couldn't malloc() new row\n"));
                    return SNMP_ERR_RESOURCEUNAVAILABLE;
                }
            }
        }
