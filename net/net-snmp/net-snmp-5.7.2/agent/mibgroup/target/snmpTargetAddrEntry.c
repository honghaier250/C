...
struct targetAddrTable_struct *snmpTargetAddrTable_create (void)
{
    ...
    newEntry = (struct targetAddrTable_struct *) malloc (sizeof (struct targetAddrTable_struct));
 ...
 */
void snmpTargetAddrTable_addToList (struct targetAddrTable_struct *newEntry, struct targetAddrTable_struct **listPtr)
{
 ...
 */
void snmpTargetAddrTable_remFromList (struct targetAddrTable_struct *oldEntry, struct targetAddrTable_struct **listPtr)
{
...
struct targetAddrTable_struct *search_snmpTargetAddrTable (oid * baseName,
                                                           size_t baseNameLen, oid * name, size_t * length, int exact)
{
    ...
    for (temp_struct = aAddrTable; temp_struct != NULL; temp_struct = temp_struct->next)
    {
...
{
    if ((entry->tDomainLen == 0) || (entry->tAddress == NULL) || (entry->params == NULL))
        return 0;
    else
        return 1;
...
void init_snmpTargetAddrEntry (void)
{
    ...
    snmpd_register_config_handler ("targetAddr", snmpd_parse_config_targetAddr, (void (*)(void)) 0, NULL);

    /*
     * we need to be called back later 
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_snmpTargetAddrEntry, NULL);
...
void shutdown_snmpTargetAddrEntry (void)
{
    ...
    snmp_unregister_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_snmpTargetAddrEntry, NULL, FALSE);
}

int snmpTargetAddr_addName (struct targetAddrTable_struct *entry, char *cptr)
{
    size_t len;
...
int snmpTargetAddr_addTAddress (struct targetAddrTable_struct *entry, char *cptr, size_t len)
{
    if (cptr == NULL)
    {
...
int snmpTargetAddr_addRetryCount (struct targetAddrTable_struct *entry, char *cptr)
{
    if (cptr == NULL)
    {
...
int snmpTargetAddr_addStorageType (struct targetAddrTable_struct *entry, char *cptr)
{
    if (cptr == NULL)
    {
             ...
             (entry->storageType != SNMP_STORAGE_PERMANENT) && (entry->storageType != SNMP_STORAGE_READONLY))
    {
        DEBUGMSGTL (("snmpTargetAddrEntry",
                     ...
                     SNMP_STORAGE_NONVOLATILE, SNMP_STORAGE_PERMANENT, SNMP_STORAGE_READONLY));
        return (0);
    }
    return (1);
...
int snmpTargetAddr_addRowStatus (struct targetAddrTable_struct *entry, char *cptr)
{
    if (cptr == NULL)
    {
...
void snmpd_parse_config_targetAddr (const char *token, char *char_ptr)
{
    ...
    cptr = read_config_read_octet_string (cptr, (u_char **) & newEntry->tAddress, &newEntry->tAddressLen);
    ...
    {
        snprintf (&buff[strlen (buff)], sizeof (buff) - strlen (buff) - 1, ".%d", (int) newEntry->tDomain[i]);
        buff[sizeof (buff) - 1] = 0;
    }
    snprintf (&buff[strlen (buff)], sizeof (buff) - strlen (buff) - 1,
              " %s %d %d %s %s %d %d\n",
              newEntry->tAddress, newEntry->timeout, newEntry->retryCount,
              newEntry->tagList, newEntry->params, newEntry->storageType, newEntry->rowStatus);
    buff[sizeof (buff) - 1] = 0;
 ...
 */
int store_snmpTargetAddrEntry (int majorID, int minorID, void *serverarg, void *clientarg)
{
                ...
                && (curr_struct->rowStatus == SNMP_ROW_ACTIVE || curr_struct->rowStatus == SNMP_ROW_NOTINSERVICE))
            {
                ...
                for (i = 0; i < curr_struct->tDomainLen; i++)
                {
                    snprintf (&line[strlen (line)],
                              sizeof (line) - strlen (line) - 1, ".%i", (int) curr_struct->tDomain[i]);
                    line[sizeof (line) - 1] = 0;
                }
                strlcat (line, " ", sizeof (line));
                read_config_save_octet_string (&line[strlen (line)], curr_struct->tAddress, curr_struct->tAddressLen);

                snprintf (&line[strlen (line)], sizeof (line) - strlen (line) - 1,
                          " %i %i \"%s\" %s %i %i",
                          curr_struct->timeout,
                          curr_struct->retryCount, curr_struct->tagList,
                          curr_struct->params, curr_struct->storageType, curr_struct->rowStatus);
                line[sizeof (line) - 1] = 0;
...
u_char *var_snmpTargetAddrEntry (struct variable * vp,
                                 oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    *var_len = sizeof (long_ret);
     ...
     */
    if ((temp_struct = search_snmpTargetAddrTable (vp->name, vp->namelen, name, length, exact)) == NULL)
    {
        return NULL;
    }
                             ...
                             u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1)
    {
        ...
        if ((var_val_len > (MAX_OID_LEN * sizeof (oid))) || (var_val_len % sizeof (oid) != 0))
        {
    ...
    else if (action == RESERVE2)
    {
        ...
        if ((target = search_snmpTargetAddrTable (snmpTargetAddrOID, snmpTargetAddrOIDLen, name, &name_len, 1)) == NULL)
        {
        ...
        else
        {
            ...
            {
                DEBUGMSGTL (("snmpTargetAddrEntry", "write to snmpTargetAddrTDomain: not allowed in active row.\n"));
             ...
             */
            if ((target->rowStatus == SNMP_ROW_NOTREADY) && (snmpTargetAddr_rowStatusCheck (target) != 0))
            {
    ...
    else if (action == FREE || action == UNDO)
    {
        ...
        if ((target = search_snmpTargetAddrTable (snmpTargetAddrOID, snmpTargetAddrOIDLen, name, &name_len, 1)) != NULL)
        {
            if (target->storageType != SNMP_STORAGE_READONLY && target->rowStatus != SNMP_ROW_ACTIVE)
            {
                memcpy ((u_char *) target->tDomain, (u_char *) old_oid, (old_oid_len * sizeof (oid)));
                target->tDomainLen = old_oid_len;
                if (target->rowStatus == SNMP_ROW_NOTINSERVICE && snmpTargetAddr_rowStatusCheck (target) == 0)
                {
                              ...
                              u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    else if (action == RESERVE2)
    {
        ...
        else
        {
            ...
            if (target->rowStatus == SNMP_ROW_ACTIVE)
            {
                DEBUGMSGTL (("snmpTargetAddrEntry", "write to snmpTargetAddrTAddress: not allowed in active row.\n"));
                return SNMP_ERR_INCONSISTENTVALUE;
            }
                             ...
                             u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    else if (action == RESERVE2)
    {
        ...
        if ((temp_struct =
             search_snmpTargetAddrTable (snmpTargetAddrOID, snmpTargetAddrOIDLen, name, &name_len, 1)) == NULL)
        {
                            ...
                            u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    else if (action == RESERVE2)
    {
        ...
        else
        {
            ...
            if (target->rowStatus == SNMP_ROW_ACTIVE)
            {
                DEBUGMSGTL (("snmpTargetAddrEntry", "write to snmpTargetAddrParams: not allowed in active row.\n"));
                return SNMP_ERR_INCONSISTENTVALUE;
            }
                                 ...
                                 u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    else if (action == RESERVE2)
    {
        ...
        {
            if (target->storageType == SNMP_STORAGE_PERMANENT || target->storageType == SNMP_STORAGE_READONLY)
            {
                DEBUGMSGTL (("snmpTargetAddrEntry",
                               ...
                               u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1)
    {
        ...
        target = search_snmpTargetAddrTable (snmpTargetAddrOID, snmpTargetAddrOIDLen, name, &name_len, 1);
        ...
        if (target != NULL)
        {
            ...
            if (target->storageType == SNMP_STORAGE_PERMANENT)
            {
                if (value == RS_DESTROY)
                {
                    DEBUGMSGTL (("snmpTargetAddrEntry", "unable to destroy permanent row\n"));
                    return SNMP_ERR_INCONSISTENTVALUE;
                }
            }
        }
        else
        {
            ...
            if (value == RS_CREATEANDGO || value == RS_CREATEANDWAIT)
            {
                ...
                {
                    DEBUGMSGTL (("snmpTargetAddrEntry", "couldn't malloc() new row\n"));
                    return SNMP_ERR_RESOURCEUNAVAILABLE;
                }
            }
        }
...
u_char *var_targetSpinLock (struct variable * vp,
                            oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
