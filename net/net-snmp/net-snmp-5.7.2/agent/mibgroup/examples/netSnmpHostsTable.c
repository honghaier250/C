...
netsnmp_feature_require (oid_stash_get_data) netsnmp_feature_require (oid_stash_add_data)
     ...
     void netSnmpHostsTable_free_undoInfo (void *vptr)
{
...
{
    static oid netSnmpHostsTable_oid[] = { 1, 3, 6, 1, 4, 1, 8072, 2, 2, 2 };
    ...
    my_handler = netsnmp_create_handler_registration ("netSnmpHostsTable",
                                                      ...
                                                      OID_LENGTH (netSnmpHostsTable_oid), HANDLER_CAN_RWRITE);

    if (!my_handler || !table_info || !iinfo)
    {
        snmp_log (LOG_ERR, "malloc failed in initialize_table_netSnmpHostsTable");
     ...
     */
    DEBUGMSGTL (("initialize_table_netSnmpHostsTable", "Registering table netSnmpHostsTable as a table iterator\n"));
        ...
        switch (reqinfo->mode)
        {
            case MODE_GET:
                    ...
                    case COLUMN_NETSNMPHOSTADDRESSTYPE:
                        {
                            ...
                            retval = get_netSnmpHostAddressType (data_context, &retval_len);
                    ...
                    case COLUMN_NETSNMPHOSTADDRESS:
                        {
                            ...
                            retval = get_netSnmpHostAddress (data_context, &retval_len);
                    ...
                    case COLUMN_NETSNMPHOSTSTORAGE:
                        {
                            ...
                            retval = get_netSnmpHostStorage (data_context, &retval_len);
                            snmp_set_var_typed_value (var, ASN_INTEGER, (const u_char *) retval, retval_len);
                        }
                        break;
                    ...
                    case COLUMN_NETSNMPHOSTROWSTATUS:
                        {
                            ...
                            retval = get_netSnmpHostRowStatus (data_context, &retval_len);
                            snmp_set_var_typed_value (var, ASN_INTEGER, (const u_char *) retval, retval_len);
                        }
                        break;
                ...
                /** We shouldn't get here */
                        snmp_log (LOG_ERR, "problem encountered in netSnmpHostsTable_handler: unknown column\n");
                }
                break;
            ...
            case MODE_SET_RESERVE1:
                ...
                if (!ci)
                {
                    ...
                    if (!data_context)
                    {
                        ci->data_context = netSnmpHostsTable_create_data_context (table_info->indexes);
                        ci->new_row = 1;
                    }
                    else
                    {
            ...
            case MODE_SET_RESERVE2:
                switch (table_info->colnum)
                {
                    ...
                    /** first, get the old value */
                            retval = get_netSnmpHostAddressType (ci->data_context, &retval_len);
                            if (retval)
                            {
                    ...
                    /** check the new value, possibly against the
                        older value for a valid state transition */
                            ret =
                                check_netSnmpHostAddressType (request->requestvb->type,
                                                              (long *) request->requestvb->val.string,
                                                              request->requestvb->val_len, retval, retval_len);
                            if (ret != 0)
                            {
                    ...
                    /** first, get the old value */
                            retval = get_netSnmpHostAddress (ci->data_context, &retval_len);
                            if (retval)
                            {
                                ui = SNMP_MALLOC_STRUCT (undoInfo);
                                ui->len = retval_len;
                    ...
                    /** first, get the old value */
                            retval = get_netSnmpHostStorage (ci->data_context, &retval_len);
                            if (retval)
                            {
                                ui = SNMP_MALLOC_STRUCT (undoInfo);
                                ui->len = retval_len;
                    ...
                    /** first, get the old value */
                            retval = get_netSnmpHostRowStatus (ci->data_context, &retval_len);
                            if (retval)
                            {
                                ui = SNMP_MALLOC_STRUCT (undoInfo);
                                ui->len = retval_len;
                    ...
                    /** check the new value, possibly against the
                        older value for a valid state transition */
                            ret =
                                check_netSnmpHostRowStatus (request->requestvb->type,
                                                            (long *) request->requestvb->val.string,
                                                            request->requestvb->val_len, retval, retval_len);
                            if (ret != 0)
                            {
            ...
            case MODE_SET_COMMIT:
                if (!ci->have_committed)
                {
                    ...
                    netSnmpHostsTable_commit_row (&ci->data_context, ci->new_row);
                    ci->have_committed = 1;
                }
                break;
                    ...
                    case COLUMN_NETSNMPHOSTADDRESSTYPE:
                        {
                            ...
                            retval = set_netSnmpHostAddressType (ci->data_context, ui->ptr, ui->len);
                            if (retval)
                            {
                    ...
                    case COLUMN_NETSNMPHOSTADDRESS:
                        {
                            ...
                            ui = netsnmp_oid_stash_get_data (undoStorage, suffix, suffix_len);
                            retval = set_netSnmpHostAddress (ci->data_context, ui->ptr, ui->len);
                            if (retval)
                            {
                                netsnmp_set_request_error (reqinfo, request, SNMP_ERR_UNDOFAILED);
                            }
                    ...
                    case COLUMN_NETSNMPHOSTSTORAGE:
                        {
                            ...
                            ui = netsnmp_oid_stash_get_data (undoStorage, suffix, suffix_len);
                            retval = set_netSnmpHostStorage (ci->data_context, ui->ptr, ui->len);
                            if (retval)
                            {
                                netsnmp_set_request_error (reqinfo, request, SNMP_ERR_UNDOFAILED);
                            }
                    ...
                    case COLUMN_NETSNMPHOSTROWSTATUS:
                        {
                            ...
                            ui = netsnmp_oid_stash_get_data (undoStorage, suffix, suffix_len);
                            retval = set_netSnmpHostRowStatus (ci->data_context, ui->ptr, ui->len);
                            if (retval)
                            {
                                netsnmp_set_request_error (reqinfo, request, SNMP_ERR_UNDOFAILED);
                            }
            ...
            default:
                snmp_log (LOG_ERR, "problem encountered in netSnmpHostsTable_handler: unsupported mode\n");
        }
    }
