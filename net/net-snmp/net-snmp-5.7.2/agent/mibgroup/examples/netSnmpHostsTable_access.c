...
*/
netsnmp_variable_list *netSnmpHostsTable_get_first_data_point (void **my_loop_context,
                                                               ...
                                                               put_index_data, netsnmp_iterator_info * mydata)
{
    ...
    *my_loop_context = loopctx;
    ...
    return netSnmpHostsTable_get_next_data_point (my_loop_context, my_data_context, put_index_data, mydata);
}
...
netsnmp_variable_list *netSnmpHostsTable_get_next_data_point (void **my_loop_context,
                                                              void **my_data_context,
                                                              netsnmp_variable_list *
                                                              put_index_data, netsnmp_iterator_info * mydata)
{
    ...
    while (loopctx->filep)
    {
        if (!loopctx->current_ptr)
        {
            ...
            loopctx->current_ptr = copy_nword (loopctx->current_ptr, tmpstring, sizeof (tmpstring));
        ...
        loopctx->current_ptr = copy_nword (loopctx->current_ptr, loopctx->hostname, sizeof (loopctx->hostname));

        snmp_set_var_value (put_index_data, (u_char *) loopctx->hostname, strlen (loopctx->hostname));
...
void *netSnmpHostsTable_context_convert_function (void *loop_context, netsnmp_iterator_info * iinfo)
{
 ...
 */
void *netSnmpHostsTable_create_data_context (netsnmp_variable_list * index_data)
{
    my_data_info *datactx = SNMP_MALLOC_TYPEDEF (my_data_info);

    if (!datactx)
        return NULL;
    strlcpy (datactx->hostname, (const char *) index_data->val.string, sizeof (datactx->hostname));
    return datactx;
}

void netSnmpHostsTable_data_free (void *data, netsnmp_iterator_info * iinfo)
{
    free (data);
}

void netSnmpHostsTable_loop_free (void *loopctx, netsnmp_iterator_info * iinfo)
{
    free (loopctx);
}
...
int netSnmpHostsTable_commit_row (void **my_data_context, int new_or_del)
{
    ...
    if (!foundit && new_or_del != -1)
    {
        ...
        fprintf (out, "%d.%d.%d.%d\t%s\n",
                 ...
                 (0x00ff0000 & datactx->theaddr) >> 16, (0xff000000 & datactx->theaddr) >> 24, datactx->hostname);
    }
...
long *get_netSnmpHostAddressType (void *data_context, size_t * ret_len)
{
...
int set_netSnmpHostAddressType (void *data_context, long *val, size_t val_len)
{
    return SNMP_ERR_NOERROR;    /* always ipv4 */
}

char *get_netSnmpHostAddress (void *data_context, size_t * ret_len)
{
...
int set_netSnmpHostAddress (void *data_context, char *val, size_t val_len)
{
    my_data_info *datainfo = data_context;
...
long *get_netSnmpHostStorage (void *data_context, size_t * ret_len)
{
...
int set_netSnmpHostStorage (void *data_context, long *val, size_t val_len)
{
    return SNMP_ERR_NOERROR;
}

long *get_netSnmpHostRowStatus (void *data_context, size_t * ret_len)
{
...
int set_netSnmpHostRowStatus (void *data_context, long *val, size_t val_len)
{
    /* XXX */
    return SNMP_ERR_NOERROR;
}
