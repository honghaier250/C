 ...
 */
     int check_netSnmpHostAddressType (int type, long *val, size_t val_len, long *old_val, size_t old_val_len)
{
    ...
    return check_netSnmpHostAddressType_local (type, val, val_len, old_val, old_val_len);
}
...
int check_netSnmpHostAddress (int type, char *val, size_t val_len, char *old_val, size_t old_val_len)
{
    ...
    /** looks ok, call the local version of the same function. */
    return check_netSnmpHostAddress_local (type, val, val_len, old_val, old_val_len);
}
...
int check_netSnmpHostStorage (int type, long *val, size_t val_len, long *old_val, size_t old_val_len)
{
    ...
    ret = check_storage_transition ((old_val) ? *old_val : SNMP_STORAGE_NONE, *val);
    if (ret)
        return ret;

    /** looks ok, call the local version of the same function. */
    return check_netSnmpHostStorage_local (type, val, val_len, old_val, old_val_len);
}
...
int check_netSnmpHostRowStatus (int type, long *val, size_t val_len, long *old_val, size_t old_val_len)
{
    ...
    ret = check_rowstatus_transition ((old_val) ? *old_val : RS_NONEXISTENT, *val);
    if (ret)
        return ret;

    /** looks ok, call the local version of the same function. */
    return check_netSnmpHostRowStatus_local (type, val, val_len, old_val, old_val_len);
}
