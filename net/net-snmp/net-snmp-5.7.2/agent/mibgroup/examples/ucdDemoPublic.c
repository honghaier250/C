...
{
    REGISTER_MIB ("examples/ucdDemoPublic", ucdDemoPublic_variables, variable2, ucdDemoPublic_variables_oid);
...
unsigned char *var_ucdDemoPublic (struct variable *vp,
                           ...
                           u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == COMMIT)
    {
        sprintf ((char *) publicString, "%.*s",
                 (int) SNMP_MIN (sizeof (publicString) - 1, var_val_len), (const char *) var_val);
    }
    return SNMP_ERR_NOERROR;
}
