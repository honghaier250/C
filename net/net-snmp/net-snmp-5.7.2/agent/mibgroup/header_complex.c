 ...
 */
int header_complex_parse_oid (oid * oidIndex, size_t oidLen, netsnmp_variable_list * data)
{
...
void header_complex_generate_oid (oid * name,    /* out */
                                  size_t * length,    /* out */
                                  oid * prefix, size_t prefix_len, netsnmp_variable_list * data)
{
 ...
 */
void *header_complex_get (struct header_complex_index *datalist, netsnmp_variable_list * index)
{
...
void *header_complex_get_from_oid (struct header_complex_index *datalist, oid * searchfor, size_t searchfor_len)
{
    struct header_complex_index *nptr;

    for (nptr = datalist; nptr != NULL; nptr = nptr->next)
    {
        if (netsnmp_oid_equals (searchfor, searchfor_len, nptr->name, nptr->namelen) == 0)
            return nptr->data;
    }
    return NULL;
}
                                                            ...
                                                            int dont_allow_duplicates)
{
    ...
    ret = header_complex_maybe_add_data_by_oid (thedata, newoid, newoid_len, data, dont_allow_duplicates);
    /*
...
struct header_complex_index *_header_complex_add_between (struct header_complex_index **thedata,
                                                                   ...
                                                                   int dont_allow_duplicates)
{
    ...
    for (hciptrn = *thedata, hciptrp = NULL; hciptrn != NULL; hciptrp = hciptrn, hciptrn = hciptrn->next)
    {
        /*
         ...
         */
        rc = snmp_oid_compare (hciptrn->name, hciptrn->namelen, newoid, newoid_len);
        if (rc > 0)
            break;
        else if (0 == rc)
        {
            snmp_log (LOG_WARNING, "header_complex_add_data_by_oid with " "duplicate index.\n");
            if (dont_allow_duplicates)
                return NULL;
        }
    }

    return _header_complex_add_between (thedata, hciptrp, hciptrn, newoid, newoid_len, data);
}

struct header_complex_index *header_complex_add_data_by_oid (struct header_complex_index **thedata,
                                                             oid * newoid, size_t newoid_len, void *data)
{
    return header_complex_maybe_add_data_by_oid (thedata, newoid, newoid_len, data, 0);
}
...
void *header_complex_extract_entry (struct header_complex_index **thetop, struct header_complex_index *thespot)
{
    ...
    if (thespot == NULL)
    {
        DEBUGMSGTL (("header_complex_extract_entry", "Null pointer asked to be extracted\n"));
        return NULL;
    }
 ...
 */
void header_complex_free_entry (struct header_complex_index *theentry, HeaderComplexCleaner * cleaner)
{
    void *data;
...
struct header_complex_index *header_complex_find_entry (struct header_complex_index *thestuff, void *theentry)
{
