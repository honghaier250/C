...
void init_hr_disk (void)
{
    ...
    Add_HR_Disk_entry ("/dev/rdsk/c%dt%dd0s%d", 0, 7, 0, 15, "/dev/rdsk/c%dt%dd0s0", 0, 7);
    Add_HR_Disk_entry ("/dev/rdsk/c%dd%ds%d", 0, 7, 0, 15, "/dev/rdsk/c%dd%ds0", 0, 7);
#elif defined(darwin)
    ...
    snmpd_register_config_handler ("ignoredisk", parse_disk_config, free_disk_config, "name");
}
        ...
        switch (di_ptr->item_type)
        {
            case ITEM_STRING:
                ...
                if (!strncmp (name, (const char *) di_ptr->item_details, len))
                    result = match_disk_config_item (name + len, di_ptr->item_next);
                break;
            case ITEM_SET:
                if (*name)
                {
                    ...
                    if ((*d_set)[c / 8] & (unsigned char) (1 << (c % 8)))
                        result = match_disk_config_item (name + 1, di_ptr->item_next);
                }
                break;
            case ITEM_STAR:
                if (di_ptr->item_next)
                {
                    for (; !result && *name; name++)
                        result = match_disk_config_item (name, di_ptr->item_next);
                }
                else
                {
                    result = 1;
...
u_char *var_hrdisk (struct variable * vp,
                    oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    int disk_idx;
...
Add_HR_Disk_entry (const char *devpart_string,
                   int first_ctl,
                   int last_ctl,
                   int first_dev, int last_dev, const char *devfull_string, int first_partn, int last_partn)
{
    ...
    if (nbr_created == 0 || MAX_NUMBER_DISK_TYPES < HR_number_disk_types)
    {
        ...
        DEBUGMSGTL (("host/hr_disk", "Add_HR_Disk_entry '%s' completed, %d created\n", devpart_string, nbr_created));
#endif
}
        ...
        while (HRD_index < max_disks)
        {
            ...
            if (HRD_history[iindex] == -1)
            {
                ...
                if (match_disk_config (string))
                {
                     ...
                     */
                    DEBUGMSGTL (("host/hr_disk", "Get_Next_HR_Disk: %s ignored\n", string));
            ...
            if (fd != -1)
            {
                ...
                DEBUGMSGTL (("host/hr_disk", "Get_Next_HR_Disk: can't query %s\n", string));
            }
            else
            {
...
int Get_Next_HR_Disk_Partition (char *string, size_t str_len, int HRP_index)
{
    ...
    if (disk_devices[HRD_type_index].disk_partition_last -
        disk_devices[HRD_type_index].disk_partition_first + 1 <= HRP_index)
    {
        return -1;
    }
...
int Get_HR_Disk_Label (char *string, size_t str_len, const char *devfull)
{
    ...
    if (NULL == desc)
    {
        snmp_log (LOG_ERR, "diskmgr: couldn't get disk description for %s, skipping\n", devfull);
        CFRelease (disk);
        CFRelease (sess_ref);
        strlcpy (string, devfull, str_len);
        return -1;
    ...
    /** model */
    str_ref = (CFStringRef) CFDictionaryGetValue (desc, kDADiskDescriptionMediaNameKey);
    if (str_ref)
    {
...
#endif
#ifdef HAVE_LINUX_HDREG_H
    strlcpy (HRD_savedModel, (const char *) HRD_info.model, sizeof (HRD_savedModel));
#endif
#ifdef DIOCGDINFO
    strlcpy (HRD_savedModel, dktypenames[HRD_info.d_type], sizeof (HRD_savedModel));
#endif
#ifdef darwin
...
static int Query_Disk (int fd, const char *devfull)
{
    ...
    {                            /* SCSI hard disk, md and LVM devices */
        ...
        if (result != -1)
        {
            ...
            if (HRD_type_index == 1)
                snprintf ((char *) HRD_info.model, sizeof (HRD_info.model) - 1, "SCSI disk (%s)", devfull);
            else if (HRD_type_index >= 4)
                snprintf ((char *) HRD_info.model, sizeof (HRD_info.model) - 1,
                          "LVM volume (%s)", devfull + strlen ("/dev/mapper/"));
            else
                snprintf ((char *) HRD_info.model, sizeof (HRD_info.model) - 1, "RAID disk (%s)", devfull);
    ...
    number_ref = (CFNumberRef) CFDictionaryGetValue (desc, kDADiskDescriptionMediaSizeKey);
    if (number_ref)
    ...
    /** writable?  */
    bool_ref = (CFBooleanRef) CFDictionaryGetValue (desc, kDADiskDescriptionMediaWritableKey);
    if (bool_ref)
    {
    ...
    /** removable?  */
    bool_ref = (CFBooleanRef) CFDictionaryGetValue (desc, kDADiskDescriptionMediaRemovableKey);
    if (bool_ref)
    {
    ...
    /** get type */
    str_ref = (CFStringRef) CFDictionaryGetValue (desc, kDADiskDescriptionMediaTypeKey);
    if (str_ref)
    {
    ...
    {
        str_ref = (CFStringRef) CFDictionaryGetValue (desc, kDADiskDescriptionDeviceProtocolKey);
        if (str_ref)
        {
    ...
    /** model */
    str_ref = (CFStringRef) CFDictionaryGetValue (desc, kDADiskDescriptionDeviceModelKey);
    if (str_ref)
    {
        strlcpy (HRD_model, CFStringGetCStringPtr (str_ref, sys_encoding), sizeof (HRD_model));
...
{
#ifdef DIOC_DESCRIBE
    if ((HRD_savedFlags & WRITE_PROTECT_FLAG) || (HRD_savedDev_type == CDROM_DEV_TYPE))
        return (2);                /* read only */
#endif
...
static int Is_It_Removeable (void)
{
    ...
    if ((HRD_savedIntf_type == PC_FDC_INTF) ||
        ...
        (HRD_savedDev_type == MO_DEV_TYPE) || (HRD_savedDev_type == CDROM_DEV_TYPE))
        return (1);                /* true */
#endif
    ...
    if ((HRD_savedCtrl_type == DKC_CDROM) ||
        ...
        (HRD_savedCtrl_type == DKC_PCMCIA_MEM) || (HRD_savedCtrl_type == DKC_PCMCIA_ATA))
        return (1);                /* true */
#endif
...
static void Add_LVM_Disks (void)
{
    ...
    res = regcomp (&lvol, "[0-9a-zA-Z+_\\.-]+-[0-9a-zA-Z+_\\.-]+", REG_EXTENDED | REG_NOSUB);
    if (res != 0)
    {
        char error[200];

        regerror (res, &lvol, error, sizeof (error) - 1);
        DEBUGMSGTL (("host/hr_disk", "Add_LVM_Disks: cannot compile regexp: %s", error));
        return;
    }
    ...
    while ((d = readdir (dir)) != NULL)
    {
        ...
        if (res == 0)
        {
            ...
            if (path == NULL)
            {
                DEBUGMSGTL (("host/hr_disk", "Add_LVM_Disks: cannot allocate memory for device %s", d->d_name));
                break;
            }
            ...
            {
                DEBUGMSGTL (("host/hr_disk", "Add_LVM_Disks: maximum count of LVM devices reached"));
                break;
            }
        }
    }
