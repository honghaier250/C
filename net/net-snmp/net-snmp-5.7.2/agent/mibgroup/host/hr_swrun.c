...
typedef DWORD (WINAPI * GETMODULEFILENAME) (HANDLE hProcess, HMODULE hModule, LPTSTR lpstrFIleName, DWORD nSize);

typedef DWORD (WINAPI * GETPROCESSMEMORYINFO) (HANDLE hProcess, PPROCESS_MEMORY_COUNTERS pmc, DWORD nSize);
...
static BOOL WINAPI dummyprocessmodules (HANDLE hProcess, HMODULE * lphModule, DWORD cb, LPDWORD lpcbNeeded)
{
...
static DWORD WINAPI GetModuleFileNameEx95 (HANDLE hProcess, HMODULE hModule, LPTSTR lpstrFileName, DWORD n)
{
    HANDLE h;
...
static time_t __stdcall to_time_t (PFILETIME ptr)
{
    long rem;
...
#ifndef linux
#ifndef dynix
        DEBUGMSG (("host/hr_swrun", "(index %d (entry #%d) ....", pid, current_proc_entry));
#else
...
#if defined(linux)
static char *skip_to_next_field (char *cp)
{
...
static char *get_proc_file_line (char *fmt, int pid, char *buf, int buflen)
{
    ...
    *buf = '\0';
    ...
    if (((fp = fopen (string, "r")) == NULL) || (fgets (buf, buflen, fp) == NULL))
    {
        if (fp)
            fclose (fp);
        return NULL;
...
static char *get_proc_stat_field (int pid, char *buf, int buflen, int skip)
{
    int i;
...
static char *get_proc_name_from_cmdline (int pid, char *buf, int buflen)
{
    return get_proc_file_line ("/proc/%d/cmdline", pid, buf, buflen);
}

static char *get_proc_name_from_status (int pid, char *buf, int buflen)
{
    char *cp, *cp2;
...
u_char *var_hrswrun (struct variable * vp,
                     oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    int pid = 0;
    ...
    switch (vp->magic)
    {
        ...
        case HRSWRUNPERF_CPU:
             ...
             */
#elif defined(dynix)
            long_return = lowpsinfo.pr_time.tv_sec * 100 + lowpsinfo.pr_time.tv_nsec / 10000000;
#elif defined(solaris2)
#if _SLASH_PROC_METHOD_
            long_return = proc_buf ? proc_buf->pr_time.tv_sec * 100 + proc_buf->pr_time.tv_nsec / 10000000 : 0;
#else
            long_return = proc_buf->p_utime * 100 + proc_buf->p_stime * 100;
#endif
#elif HAVE_KVM_GETPROC2
            long_return = proc_table[LowProcIndex].p_uticks +
                proc_table[LowProcIndex].p_sticks + proc_table[LowProcIndex].p_iticks;
#elif HAVE_KVM_GETPROCS
            ...
            long_return = proc_table[LowProcIndex].kp_lwp.kl_uticks +
                proc_table[LowProcIndex].kp_lwp.kl_sticks + proc_table[LowProcIndex].kp_lwp.kl_iticks;
#elif defined(dragonfly)
            long_return = proc_table[LowProcIndex].kp_eproc.e_uticks +
                proc_table[LowProcIndex].kp_eproc.e_sticks + proc_table[LowProcIndex].kp_eproc.e_iticks;
#else
            long_return = proc_table[LowProcIndex].kp_proc.p_uticks +
                proc_table[LowProcIndex].kp_proc.p_sticks + proc_table[LowProcIndex].kp_proc.p_iticks;
#endif
#elif defined(linux)
...
#elif defined(aix4) || defined(aix5) || defined(aix6) || defined(aix7)
            long_return = proc_table[LowProcIndex].pi_ru.ru_utime.tv_sec * 100 + proc_table[LowProcIndex].pi_ru.ru_utime.tv_usec / 10000000 +    /* nanoseconds */
                proc_table[LowProcIndex].pi_ru.ru_stime.tv_sec * 100 + proc_table[LowProcIndex].pi_ru.ru_stime.tv_usec / 10000000;    /* nanoseconds */
#else
            long_return = proc_table[LowProcIndex].p_utime.tv_sec * 100 +
                proc_table[LowProcIndex].p_utime.tv_usec / 10000 +
                proc_table[LowProcIndex].p_stime.tv_sec * 100 + proc_table[LowProcIndex].p_stime.tv_usec / 10000;
#endif
            return (u_char *) & long_return;
...
# endif
            ...
            long_return = proc_table[LowProcIndex].p_vm_tsize +
                proc_table[LowProcIndex].p_vm_ssize + proc_table[LowProcIndex].p_vm_dsize;
            ...
            long_return = proc_table[LowProcIndex].kp_eproc.e_vm.vm_tsize +
                proc_table[LowProcIndex].kp_eproc.e_vm.vm_ssize + proc_table[LowProcIndex].kp_eproc.e_vm.vm_dsize;
            long_return = long_return * (getpagesize () / 1024);
#endif
#elif defined(linux)
...
#elif defined(aix4) || defined(aix5) || defined(aix6) || defined(aix7)
    {
        ...
        for (;;)
        {
            ...
            got = getprocs (proc_table + nproc, sizeof (proc_table[0]),
                            0, sizeof (struct fdsinfo), &proc_index, avail - nproc);
            nproc += got;
            if (nproc < avail)
            {
                break;
