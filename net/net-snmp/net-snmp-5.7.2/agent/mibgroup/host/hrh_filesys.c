...
#define    PART_DUMP    1
     ...
     int header_hrhfilesys (struct variable *, oid *, size_t *, int, size_t *, WriteMethod **);
...
u_char *var_hrhfilesys (struct variable *vp,
                        oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    int fsys_idx;
    ...
    switch (vp->magic)
    {
        ...
        case HRFSYS_TYPE:
            fsys_type_id[fsys_type_len - 1] =
                (HRFS_entry->type > _NETSNMP_FS_TYPE_LOCAL ? NETSNMP_FS_TYPE_OTHER : HRFS_entry->type);
