...
#define UTMP_FILE _PATH_UTMP
     ...
     *********************/
     ...
     static int set_solaris_bootcommand_parameter (int action, u_char * var_val, u_char var_val_type,
...
u_char *var_hrsys (struct variable * vp,
                   oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
...
static int set_solaris_eeprom_parameter (const char *key, const char *value, size_t var_val_len)
{
...
static long get_max_solaris_processes (void)
{
    ...
    if (maxprocs == -1)
    {
        if ((ksc = kstat_open ()) != NULL &&
            (ks = kstat_lookup (ksc, "unix", 0, "var")) != NULL && (kstat_read (ksc, ks, &v) != -1))
        {
