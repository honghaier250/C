...
u_char *var_hrfilesys (struct variable *vp,
                       oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    int fsys_idx;
    ...
    switch (vp->magic)
    {
        ...
        case HRFSYS_ACCESS:
            ...
            return (u_char *) & long_return;
        case HRFSYS_BOOT:
            if (HRFS_entry->HRFS_mount[0] == '/' && HRFS_entry->HRFS_mount[1] == 0)
...
void Init_HR_FileSys (void)
{
    ...
    size = 0;
    if (aixmnt != NULL)
        free (aixmnt);            /* something leaks, make it idiot-safe */
    aixmnt = NULL;
    aixcurr = NULL;
    HRFS_index = 0;
...
int Get_Next_HR_FileSys (void)
{
    ...
    if ((char *) aixcurr >= (char *) HRFS_entry)
        aixcurr = NULL;
    switch (HRFS_entry->HRFS_type)
    {
#ifdef MNT_NAMEFS
    ...
    if (HRFS_index == SE_DNE)
    {
        ...
        se_add_pair_to_slist ("filesys", strdup (HRFS_entry->HRFS_name), HRFS_index);
    }
...
{
#if HAVE_GETFSSTAT && !defined(MFSNAMELEN)
    if ((HRFS_entry->HRFS_type == MOUNT_NFS) || (HRFS_entry->HRFS_type == MOUNT_AFS))
#elif defined(aix4) || defined(aix5) || defined(aix6) || defined(aix7)
...
char *cook_device (char *dev)
{
    ...
    if (!strncmp (dev, RAW_DEVICE_PREFIX, strlen (RAW_DEVICE_PREFIX)))
    {
        ...
        strlcat (cooked_dev, dev + strlen (RAW_DEVICE_PREFIX), sizeof (cooked_dev));
    }
    else
    {
