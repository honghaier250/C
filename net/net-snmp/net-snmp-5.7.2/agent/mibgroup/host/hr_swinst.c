...
#define HRSWINST_ENTRY_NAME_LENGTH    11
    ...
    do
    {
        ...
        while ((swinst_idx = Get_Next_HR_SWInst ()) != -1)
        {
            ...
            if ((!exact && (result < 0)) && (LowIndex == -1 || swinst_idx < LowIndex))
            {
                LowIndex = swinst_idx;
                err = Save_HR_SW_info (LowIndex);
...
u_char *var_hrswinst (struct variable * vp,
                      oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    SWI_t *swi = &_myswi;        /* XXX static for now */
        ...
        case HRSWINST_DATE:
            {
                ...
                if (swi->swi_directory != NULL)
                {
                    snprintf (string, sizeof (string), "%s/%s", swi->swi_directory, swi->swi_name);
                    string[sizeof (string) - 1] = 0;
...
#ifdef    HAVE_LIBRPM
static void Check_HRSW_cache (void *xxx)
{
    SWI_t *swi = (SWI_t *) xxx;
...
int Save_HR_SW_info (int ix)
{
    ...
    if (1 <= ix && ix <= swi->swi_nrec && ix != swi->swi_prevx)
    {
...

        {
            ...
            mi = rpmtsInitIterator (swi->swi_rpmts, RPMDBI_PACKAGES, &offset, sizeof (offset));
        ...
        if (h == NULL)
        {
            DEBUGMSGTL (("host/hr_swinst",
                         "RPM cache has probably expired when reading entry %d, " "reloading...\n", ix));
            swi->swi_timestamp = 0;
            return -1;
        }
        if (swi->swi_h != NULL)
...
#ifdef    HAVE_LIBRPM
void Mark_HRSW_token (void)
{
}

void Release_HRSW_token (void)
{
    SWI_t *swi = &_myswi;        /* XXX static for now */
