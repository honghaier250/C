...
void *header_hrstoreEntry (struct variable *, oid *, size_t *, int, size_t *, WriteMethod **);
...
void init_hrh_storage (void)
{
    ...
    netsnmp_register_scalar (netsnmp_create_handler_registration ("host/hrMemorySize", handle_memsize,
                                                                  hrMemorySize_oid, OID_LENGTH (hrMemorySize_oid),
                                                                  HANDLER_CAN_RONLY));
    REGISTER_MIB ("host/hr_storage", hrstore_variables, variable2, hrStorageTable_oid);
    ...
    snmpd_register_config_handler ("storageUseNFS", parse_storage_config, NULL, "1 | 2\t\t(1 = enable, 2 = disable)");
}
...
void *header_hrstoreEntry (struct variable *vp,
...
u_char *var_hrstore (struct variable * vp,
                     oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    int store_idx = 0;
  ...
  really_try_next:
    ...
    if (HRFS_entry &&
        ...
        netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID,
                                NETSNMP_DS_AGENT_SKIPNFSINHOSTRESOURCES) && Check_HR_FileSys_NFS ())
        return NULL;
    ...
    switch (vp->magic)
    {
        ...
        case HRSTORE_UNITS:
            if (store_idx > NETSNMP_MEM_TYPE_MAX)
            {
                if (netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_REALSTORAGEUNITS))
