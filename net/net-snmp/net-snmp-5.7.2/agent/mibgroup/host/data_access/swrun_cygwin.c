...
int netsnmp_arch_swrun_container_load (netsnmp_container * container, u_int flags)
{
    ...
    while (curproc = (struct external_pinfo *) cygwin_internal (query, curpid | CW_NEXTPID))
    {

        curpid = curproc->pid;
        if ((PID_EXITED & curproc.process_state) || (~0xffff & curproc.exitcode))
            continue;
        ...
        else if (query == CW_GETPINFO_FULL)
        {
            ...
            if (h)
            {
                ...
                {
                    entry->hrSWRunPath_len = snprintf (entry->hrSWRunPath,
                                                       sizeof (entry->hrSWRunPath) - 1, "%s", string);
                }
            }
        }
        /*
        ...
        if (entry->hrSWRunPath_len)
        {
            ...
            entry->hrSWRunName_len = snprintf (entry->hrSWRunName, sizeof (entry->hrSWRunName) - 1, "%s", cp2);
