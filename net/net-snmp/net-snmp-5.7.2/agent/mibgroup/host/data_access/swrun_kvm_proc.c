...
void netsnmp_arch_swrun_init (void)
{
    ...
    {
        if ((NULL != (ks = kstat_lookup (ksc, "unix", 0, "var"))) && (-1 != kstat_read (ksc, ks, &v)))
        {
            _swrun_max = v.v_proc;
        }
        kstat_close (ksc);
...
int netsnmp_arch_swrun_container_load (netsnmp_container * container, u_int flags)
{
    ...
    while (NULL != (procentry_p = readdir (procdir)))
    {
        ...
        entry->hrSWRunName_len = snprintf (entry->hrSWRunName,
                                           sizeof (entry->hrSWRunName) - 1, "%s", proc_buf->p_user.u_comm);
        /*
        ...
        *cp = '\0';                /* End of argv[0] */
        entry->hrSWRunPath_len = snprintf (entry->hrSWRunPath,
                                           sizeof (entry->hrSWRunPath) - 1, "%s", proc_buf->p_user.u_psargs);
        ...
        *cp = ' ';                /* Restore u_psargs value */
         ...
         */
        entry->hrSWRunType = (SSYS & proc_buf->p_flag) ? 2    /* kernel process */
            : 4                    /*  application   */
            ;
