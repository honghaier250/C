...
int netsnmp_arch_swrun_container_load (netsnmp_container * container, u_int flags)
{
    ...
    for (i = 0; i < nproc; i++)
    {
        ...
        entry->hrSWRunName_len = snprintf (entry->hrSWRunName,
                                           sizeof (entry->hrSWRunName) - 1, "%s", proc_table[i].pst_ucomm);
        /*
        ...
        *cp1 = '\0';            /* End of argv[0] */
        entry->hrSWRunPath_len = snprintf (entry->hrSWRunPath,
                                           sizeof (entry->hrSWRunPath) - 1, "%s", proc_table[i].pst_cmd);
        entry->hrSWRunParameters_len = snprintf (entry->hrSWRunParameters,
                                                 sizeof (entry->hrSWRunParameters) - 1, "%s", cp1 + 1);
        *cp1 = ' ';                /* Restore pst_cmd value */

        entry->hrSWRunType = (PS_SYS & proc_table[i].pst_flag) ? 2    /* kernel process */
            : 4                    /*  application   */
            ;
