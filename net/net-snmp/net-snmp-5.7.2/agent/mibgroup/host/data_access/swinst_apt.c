...
void netsnmp_swinst_arch_init (void)
{
    ...
    snprintf (apt_fmt, SNMP_MAXBUF, "%%%d[^#]#%%%d[^#]#%%%d[^#]#%%%d[^#]#%%%d[^#]#%%%ds",
              SNMP_MAXBUF, SNMP_MAXBUF, SNMP_MAXBUF, SNMP_MAXBUF, SNMP_MAXBUF, SNMP_MAXBUF);
}
...
{
    FILE *p =
        popen ("dpkg-query --show --showformat '${Package}#${Version}#${Section}#${Priority}#${Essential}#${Status}\n'",
               "r");
    ...
    while (fgets (buf, BUFSIZ, p))
    {
        ...
        entry->swName_len = snprintf (entry->swName, sizeof (entry->swName), "%s-%s", package, version);
        if (entry->swName_len >= sizeof (entry->swName))
            entry->swName_len = sizeof (entry->swName) - 1;
        entry->swType = (strcmp (essential, "yes") == 0) ? 2    /* operatingSystem */
            : 4;                /*  application    */
