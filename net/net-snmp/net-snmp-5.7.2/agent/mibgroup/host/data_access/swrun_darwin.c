...
int netsnmp_arch_swrun_container_load (netsnmp_container * container, u_int flags)
{
    ...
    for (i = 0; i < num_entries; i++)
    {
         ...
         */
        if ((NULL == processes[i].kp_proc.p_comm) || (0 == processes[i].kp_proc.p_pid))
        {
            DEBUGMSGTL (("swrun:load:arch",
                         " skipping p_comm '%s', pid %5d, p_pstat %d\n",
                         processes[i].kp_proc.p_comm ?
                         processes[i].kp_proc.p_comm : "NULL",
                         processes[i].kp_proc.p_pid, processes[i].kp_proc.p_stat));
            continue;
        }

        DEBUGMSGTL (("swrun:load:arch", " %s pid %5d\n", processes[i].kp_proc.p_comm, processes[i].kp_proc.p_pid));
        ...
        entry->hrSWRunName_len = snprintf (entry->hrSWRunName,
                                           sizeof (entry->hrSWRunName) - 1, "%s", processes[i].kp_proc.p_comm);
         ...
         */
        rc = proc_pidinfo (processes[i].kp_proc.p_pid, PROC_PIDTASKALLINFO, 0, &taskinfo, sizeof (taskinfo));
        ...
        else
        {
            ...
            at.uq = taskinfo.ptinfo.pti_total_user + taskinfo.ptinfo.pti_total_system;
...
static int _set_command_name_jaguar (netsnmp_swrun_entry * entry)
{
    ...
    DEBUGMSGTL (("swrun:load:arch:_cn", SWRUNINDENT "kernel name %s\n", command));
    if (strncmp (command, entry->hrSWRunName, sizeof (entry->hrSWRunName) - 1))
    {
        ...
        DEBUGMSGTL (("swrun:load:arch:_cn", "**" SWRUNINDENT "updated name to %s\n", entry->hrSWRunName));
        return 0;
    }
...
static int _set_command_name (netsnmp_swrun_entry * entry)
{
    ...
    while (nargs && cp < &arg_buf[procargssize])
    {
        ...
        {
            strlcat (&entry->hrSWRunParameters[entry->hrSWRunParameters_len], argN, sizeof (entry->hrSWRunParameters));
    ...
    if (strncmp (command, entry->hrSWRunName, sizeof (entry->hrSWRunName) - 1))
    {
        strlcpy (entry->hrSWRunName, command, sizeof (entry->hrSWRunName));
        entry->hrSWRunName_len = strlen (entry->hrSWRunName);
        DEBUGMSGTL (("swrun:load:arch:_cn", " **updated name to %s\n", entry->hrSWRunName));
    }
