...
int netsnmp_arch_swrun_container_load (netsnmp_container * container, u_int flags)
{
    ...
    while (NULL != (procentry_p = readdir (procdir)))
    {
        ...
        entry->hrSWRunName_len
            = sprintf (entry->hrSWRunName, "%.*s", (int) sizeof (entry->hrSWRunName) - 1, psinfo.pr_fname);
        /*
        ...
        *cp = '\0';                /* End of argv[0] */
        entry->hrSWRunPath_len
            = sprintf (entry->hrSWRunPath, "%.*s", (int) sizeof (entry->hrSWRunPath) - 1, psinfo.pr_psargs);

        entry->hrSWRunParameters_len
            = sprintf (entry->hrSWRunParameters, "%.*s", (int) sizeof (entry->hrSWRunParameters) - 1, cp + 1);
        *cp = ' ';                /* Restore pr_psargs value */

        /*
         * check for system processes
         */
        entry->hrSWRunType = (PR_ISSYS & psinfo.pr_flag) ? 2    /* kernel process */
            : 4                    /*  application   */
            ;
