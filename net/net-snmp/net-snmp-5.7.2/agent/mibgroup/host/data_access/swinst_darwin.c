...
netsnmp_feature_require (container_directory) netsnmp_feature_require (date_n_time)
 ...
 */
     ...
     static int _check_bundled_app (CFURLRef currentURL, CFStringRef * name, CFStringRef * info, const char *path);

     static int _check_classic_app (CFURLRef currentURL, CFStringRef * name, CFStringRef * info, const char *path);
...
#ifdef TEST
            , "/Developer/Applications", "/Volumes/audX/Applications (Mac OS 9)"
#endif
    };
...
static int _add_applications_in_dir (netsnmp_container * container, const char *path)
{
    ...
    for (file = ITERATOR_FIRST (it); file; file = ITERATOR_NEXT (it), CFRelease (currentPath), CFRelease (currentURL))
    {

        int rc2 = 0;

        currentPath =
            CFStringCreateWithCStringNoCopy (kCFAllocatorDefault, file, kCFStringEncodingUTF8, kCFAllocatorNull);
        currentURL = CFURLCreateWithFileSystemPath (kCFAllocatorDefault, currentPath, kCFURLPOSIXPathStyle, true);
        LSCopyItemInfoForURL (currentURL, kLSRequestBasicFlagsOnly | kLSRequestAppTypeFlags, &itemInfoRecord);
        if ((0 == itemInfoRecord.flags) ||
            (kLSItemInfoIsPlainFile == itemInfoRecord.flags) ||
            (itemInfoRecord.flags & kLSItemInfoIsInvisible) || (itemInfoRecord.flags & kLSItemInfoIsAliasFile))
        {
            continue;
        }

        /** recurse on non-application containers (i.e. directory) */
        if ((itemInfoRecord.flags & kLSItemInfoIsContainer) && (!(itemInfoRecord.flags & kLSItemInfoIsApplication)))
        {
        ...
        if ((itemInfoRecord.flags & kLSItemInfoIsPackage) || (itemInfoRecord.flags & kLSItemInfoIsContainer))
        {
            rc2 = _check_bundled_app (currentURL, &prodName, &version, file);
        }
        else if ((itemInfoRecord.flags & kLSItemInfoIsClassicApp) || (itemInfoRecord.flags & kLSItemInfoIsPlainFile))
        {
        ...
        if (rc2)
        {                        /* not an app. if directory, recurse; else continue */
            _dump_flags (itemInfoRecord.flags);
            if (1 == rc2)
                netsnmp_directory_container_read (files, file, 0);
            continue;
        ...
        entry->swName_len =
            snprintf (entry->swName, sizeof (entry->swName),
                      "%s %s", CFStringGetCStringPtr (prodName, 0), CFStringGetCStringPtr (version, 0));
...
int _check_bundled_app (CFURLRef currentURL, CFStringRef * prodName, CFStringRef * version, const char *file)
{
    ...
    *prodName = (CFStringRef) CFDictionaryGetValue (infoDict, CFSTR ("CFBundleName"));
    if (NULL == *prodName)
    {
        *prodName = (CFStringRef) CFDictionaryGetValue (infoDict, CFSTR ("CFBundleDisplayName"));
        if (NULL == *prodName)
        {
            *prodName = (CFStringRef) CFDictionaryGetValue (infoDict, CFSTR ("CFBundleExecutable"));
        }
    }
    if (NULL == *prodName)
    {
    ...
    *version = (CFStringRef) CFDictionaryGetValue (infoDict, CFSTR ("CFBundleShortVersionString"));
    if (NULL == *version)
    {
        *version = (CFStringRef) CFDictionaryGetValue (infoDict, CFSTR ("CFBundleVersion"));
        if (*version == NULL)
            *version = (CFStringRef) CFDictionaryGetValue (infoDict, CFSTR ("CFBundleGetInfoString"));
    }
    if (NULL == *version)
    {
...
static int _check_classic_app (CFURLRef currentURL, CFStringRef * prodName, CFStringRef * version, const char *file)
{
    /*
    ...
    *prodName = CFURLCopyLastPathComponent (currentURL);
    ...
    if (versHandle != NULL)
    {
        *version = CFStringCreateWithPascalString (kCFAllocatorDefault,
        ...
        if (*version == NULL)
        {
            ...
            longVersionPtr = (StringPtr) (((Ptr) longVersionPtr) + 1 + ((unsigned char) *longVersionPtr));
            *version = CFStringCreateWithPascalString (kCFAllocatorDefault, longVersionPtr, kCFStringEncodingMacRoman);
        }
    ...
    if (*version == NULL)
    {
        DEBUGMSGTL (("swinst:arch:darwin", "\tmissing classic/file version: %s\n", file));
        SNMP_CFRelease (*prodName);
        return -1;
    }
