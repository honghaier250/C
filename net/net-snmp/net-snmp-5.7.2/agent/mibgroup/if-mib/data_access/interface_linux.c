...
netsnmp_feature_require (create_prefix_info) netsnmp_feature_child_of (interface_arch_set_admin_status, interface_all)
...
#ifdef HAVE_LINUX_ETHTOOL_H
     unsigned long long
         netsnmp_linux_interface_get_if_speed_mii (int fd, const char *name, unsigned long long defaultspeed);
#endif
 ...
 */
void _arch_interface_has_ipv6 (oid if_index, u_int * flags, netsnmp_container * addr_container)
{
#ifdef NETSNMP_ENABLE_IPV6
    ...
    *flags &= ~NETSNMP_INTERFACE_FLAGS_HAS_IPV6;
    ...
    if (NULL == addr_container)
    {
        ...
        addr_container = netsnmp_access_ipaddress_container_load (NULL, addr_container_flags);
        if (NULL == addr_container)
        {
            DEBUGMSGTL (("access:ifcontainer", "couldn't get ip addresses container\n"));
            return;
        }
    }
    else
    ...
    if (NULL == addr_it)
    {
        DEBUGMSGTL (("access:ifcontainer", "couldn't get ip addresses iterator\n"));
    ...
    for (; addr_entry; addr_entry = ITERATOR_NEXT (addr_it))
    {
         ...
         */
        if ((if_index != addr_entry->if_index) || (4 == addr_entry->ia_address_len))
            continue;
...
static void _arch_interface_description_get (netsnmp_interface_entry * entry)
{
    ...
    descr = pci_lookup_name (pci_access, buf, sizeof (buf),
                             PCI_LOOKUP_VENDOR | PCI_LOOKUP_DEVICE, vendor_id, device_id, 0, 0);
    if (descr)
    {
        free (entry->descr);
    ...
    {
        DEBUGMSGTL (("access:interface", "Failed pci_lookup_name vendor=%#hx device=%#hx\n", vendor_id, device_id));
    }
}
#endif
...
static void _arch_interface_flags_v6_get (netsnmp_interface_entry * entry)
{
     ...
     */
    snprintf (line, sizeof (line), "/proc/sys/net/ipv6/conf/%s/forwarding", entry->name);
    if (!(fin = fopen (line, "r")))
    {
        DEBUGMSGTL (("access:interface", "Failed to open %s\n", line));
    }
 ...
 * @internal
 */
static int _parse_stats (netsnmp_interface_entry * entry, char *stats, int expected)
{
    /*
     * scanline_2_2:
     *  [               IN                        ]
    ...
    if ((*stats == 'N') && (0 == strncmp (stats, "No statistics available", strlen ("No statistics available"))))
        return -1;
    ...
    {
        scan_count = sscanf (stats, scan_line_to_use, &rec_pkt, &rec_err, &snd_pkt, &snd_err, &coll);
        if (scan_count == expected)
        {
            entry->ns_flags &= ~NETSNMP_INTERFACE_FLAGS_HAS_MCAST_PKTS;
            rec_oct = rec_drop = 0;
    ...
    if (scan_count != expected)
    {
        snmp_log (LOG_ERR, "error scanning interface data (expected %d, got %d)\n", expected, scan_count);
        return scan_count;
    }
 ...
 */
int netsnmp_arch_interface_container_load (netsnmp_container * container, u_int load_flags)
{
    FILE *devin;
    ...
    {
        DEBUGMSGTL (("access:interface", "Failed to load Interface Table (linux1)\n"));
    ...
    if (0 == scan_expected)
    {
        ...
        {
            scan_expected = 10;
            DEBUGMSGTL (("access:interface", "using linux 2.2 kernel /proc/net/dev\n"));
        }
        else
        {
            scan_expected = 5;
            DEBUGMSGTL (("access:interface", "using linux 2.0 kernel /proc/net/dev\n"));
        }
    }
    ...
    while (fgets (line, sizeof (line), devin))
    {
        ...
        {
            snmp_log (LOG_ERR, "interface data format error 2 (%d < 6), line ==|%s|\n", (int) (stats - line), line);
        }
        ...
        *stats++ = 0;            /* null terminate name */
        ...
        {
            DEBUGMSGTL (("9:access:ifcontainer", "interface '%s' excluded by ip version\n", ifstart));
            continue;
        }
        ...
        if (IANAIFTYPE_ETHERNETCSMACD == entry->type)
        {
            ...
            speed = netsnmp_linux_interface_get_if_speed (fd, entry->name, defaultspeed);
...
int netsnmp_arch_set_admin_status (netsnmp_interface_entry * entry, int ifAdminStatus_val)
{
    ...
    return netsnmp_access_interface_ioctl_flags_set (-1, entry, IFF_UP, and_complement);
}
 ...
 */
unsigned long long netsnmp_linux_interface_get_if_speed (int fd, const char *name, unsigned long long defaultspeed)
{
    int ret;
