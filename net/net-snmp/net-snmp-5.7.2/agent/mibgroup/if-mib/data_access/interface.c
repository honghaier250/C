 ...
 */
     static int _access_interface_entry_compare_name (const void *lhs, const void *rhs);

#ifndef NETSNMP_ACCESS_INTERFACE_NOARCH
     static void _access_interface_entry_release (netsnmp_interface_entry * entry, void *unused);
#endif
 ...
 */
     ...
     extern int netsnmp_arch_interface_container_load (netsnmp_container * container, u_int load_flags);

     extern int netsnmp_arch_set_admin_status (netsnmp_interface_entry * entry, int ifAdminStatus);
...
{
    snmpd_register_config_handler ("interface", _parse_interface_config, _free_interface_config, "name type speed");
}
 ...
 */
#ifndef NETSNMP_ACCESS_INTERFACE_NOARCH
netsnmp_container *netsnmp_access_interface_container_load (netsnmp_container * container, u_int load_flags)
{
    int rc;
    ...
    if (0 != rc)
    {
        netsnmp_access_interface_container_free (container, NETSNMP_ACCESS_INTERFACE_FREE_NOFLAGS);
        container = NULL;
    }
...
void netsnmp_access_interface_container_free (netsnmp_container * container, u_int free_flags)
{
    ...
    if (!(free_flags & NETSNMP_ACCESS_INTERFACE_FREE_DONT_CLEAR))
    {
         ...
         * free all items.
         */
        CONTAINER_CLEAR (container, (netsnmp_container_obj_func *) _access_interface_entry_release, NULL);
    }
...
/**
 */
netsnmp_interface_entry *netsnmp_access_interface_entry_get_by_index (netsnmp_container * container, oid index)
{
    netsnmp_index tmp;
    ...
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "invalid container for netsnmp_access_interface_entry_get_by_index\n");
        return NULL;
    }
...
/**
 */
netsnmp_interface_entry *netsnmp_access_interface_entry_get_by_name (netsnmp_container * container, const char *name)
{
    ...
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "invalid container for netsnmp_access_interface_entry_get_by_name\n");
        return NULL;
    }

    if (NULL == container->next)
    {
        snmp_log (LOG_ERR, "secondary index missing for netsnmp_access_interface_entry_get_by_name\n");
        return NULL;
    }
...
/**
 */
netsnmp_interface_entry *netsnmp_access_interface_entry_create (const char *name, oid if_index)
{
...
int Interface_Scan_Next (short *index, char *name, netsnmp_interface_entry ** entry, void *dc)
{
    int returnIndex = 0;
...
int Interface_Scan_NextInt (int *index, char *name, netsnmp_interface_entry ** entry, void *dc)
{
 ...
 */
#ifndef NETSNMP_ACCESS_INTERFACE_NOARCH
int netsnmp_access_interface_entry_set_admin_status (netsnmp_interface_entry * entry, int ifAdminStatus)
{
    int rc;
...
static int _access_interface_entry_compare_name (const void *lhs, const void *rhs)
{
    return strcmp (((const netsnmp_interface_entry *) lhs)->name, ((const netsnmp_interface_entry *) rhs)->name);
}

/**
 */
static void _access_interface_entry_release (netsnmp_interface_entry * entry, void *context)
{
...
static void _access_interface_entry_save_name (const char *name, oid index)
{
    ...
    if (tmp == SE_DNE)
    {
        ...
        DEBUGMSGTL (("access:interface:ifIndex", "saved ifIndex %" NETSNMP_PRIo "u for %s\n", index, name));
    }
 ...
 */
int
netsnmp_access_interface_entry_update_stats (netsnmp_interface_entry * prev_vals, netsnmp_interface_entry * new_vals)
{
    ...
    if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.ibytes,
                                             &new_vals->stats.ibytes, &prev_vals->old_stats->ibytes, &need_wrap_check))
    ...
    if (new_vals->ns_flags & NETSNMP_INTERFACE_FLAGS_CALCULATE_UCAST)
    {
        if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.iall,
                                                 &new_vals->stats.iall, &prev_vals->old_stats->iall, &need_wrap_check))
            DEBUGMSGTL (("access:interface", "Error expanding packet count to 64bits\n"));
    }
    else
    {
        if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.iucast,
            ...
            DEBUGMSGTL (("access:interface", "Error expanding ifHCInUcastPkts to 64bits\n"));
    }

    if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.iucast,
                                             &new_vals->stats.iucast, &prev_vals->old_stats->iucast, &need_wrap_check))
        DEBUGMSGTL (("access:interface", "Error expanding ifHCInUcastPkts to 64bits\n"));

    if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.imcast,
                                             &new_vals->stats.imcast, &prev_vals->old_stats->imcast, &need_wrap_check))
        DEBUGMSGTL (("access:interface", "Error expanding ifHCInMulticastPkts to 64bits\n"));

    if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.ibcast,
                                             &new_vals->stats.ibcast, &prev_vals->old_stats->ibcast, &need_wrap_check))
        DEBUGMSGTL (("access:interface", "Error expanding ifHCInBroadcastPkts to 64bits\n"));

    if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.obytes,
                                             &new_vals->stats.obytes, &prev_vals->old_stats->obytes, &need_wrap_check))
        DEBUGMSGTL (("access:interface", "Error expanding ifHCOutOctets to 64bits\n"));

    if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.oucast,
                                             &new_vals->stats.oucast, &prev_vals->old_stats->oucast, &need_wrap_check))
        DEBUGMSGTL (("access:interface", "Error expanding ifHCOutUcastPkts to 64bits\n"));

    if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.omcast,
                                             &new_vals->stats.omcast, &prev_vals->old_stats->omcast, &need_wrap_check))
        DEBUGMSGTL (("access:interface", "Error expanding ifHCOutMulticastPkts to 64bits\n"));

    if (0 != netsnmp_c64_check32_and_update (&prev_vals->stats.obcast,
                                             &new_vals->stats.obcast, &prev_vals->old_stats->obcast, &need_wrap_check))
        DEBUGMSGTL (("access:interface", "Error expanding ifHCOutBroadcastPkts to 64bits\n"));
...
int netsnmp_access_interface_entry_calculate_stats (netsnmp_interface_entry * entry)
{
    ...
    {
        u64Subtract (&entry->stats.iall, &entry->stats.imcast, &entry->stats.iucast);
    }
    return 0;
}
 ...
 * @retval  0 : no error
 */
int netsnmp_access_interface_entry_copy (netsnmp_interface_entry * lhs, netsnmp_interface_entry * rhs)
{
    DEBUGMSGTL (("access:interface", "copy\n"));

    if ((NULL == lhs) || (NULL == rhs) ||
        (NULL == lhs->name) || (NULL == rhs->name) || (0 != strncmp (lhs->name, rhs->name, strlen (rhs->name))))
        return -1;
...
netsnmp_conf_if_list *netsnmp_access_interface_entry_overrides_get (const char *name)
{
