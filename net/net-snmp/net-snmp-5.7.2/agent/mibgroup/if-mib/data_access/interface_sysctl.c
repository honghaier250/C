...
void netsnmp_sysctl_ifmedia_to_speed (int media, u_int * speed, u_int * speed_high)
{
    *speed = 0;
    *speed_high = 0;
...
int netsnmp_sysctl_get_if_speed (char *name, u_int * speed, u_int * speed_high)
{
    int s;
    ...
    *speed_high = 0;
    ...
    if (*speed == 0 && (media_list = (int *) malloc (ifmr.ifm_count * sizeof (int))) != NULL)
    {
        ...
        if (ioctl (s, SIOCGIFMEDIA, (caddr_t) & ifmr) == 0)
        {
            ...
            for (i = 0; i < ifmr.ifm_count; i++)
            {
                ...
                netsnmp_sysctl_ifmedia_to_speed (media_list[i], &t_speed, &t_speed_high);
                if (t_speed_high > m_speed_high || (t_speed_high == m_speed_high && t_speed > m_speed))
                {
    ...
    DEBUGMSGTL (("access:interface:container:sysctl", "%s: speed: %u, speed_high: %u\n", name, *speed, *speed_high));
...
int netsnmp_arch_interface_container_load (netsnmp_container * container, u_int load_flags)
{
    ...
    DEBUGMSGTL (("access:interface:container:sysctl", "load (flags %u)\n", load_flags));
    ...
    if (if_list == NULL)
    {
        snmp_log (LOG_ERR, "could not allocate memory for interface info " "(%zu bytes)\n", if_list_size);
        return -3;
    }
    else
    {
        DEBUGMSGTL (("access:interface:container:sysctl", "allocated %zu bytes for if_list\n", if_list_size));
    }
    ...
    for (cp = if_list; cp < if_list + if_list_size; cp += ifp->ifm_msglen)
    {
        ...
        netsnmp_sysctl_get_if_speed (entry->name, &entry->speed, &entry->speed_high);
    ...
    for (cp = if_list; cp < if_list + if_list_size; cp += ifa->ifam_msglen)
    {
        ...
        DEBUGMSGTL (("access:interface:container:sysctl", "%s: flags=0x%x\n", entry->name, entry->ns_flags));
    }
...
int netsnmp_arch_set_admin_status (netsnmp_interface_entry * entry, int ifAdminStatus_val)
{
    ...
    /* not implemented */
    snmp_log (LOG_ERR, "netsnmp_arch_set_admin_status not (yet) implemented " "for BSD sysctl.\n");
