...
int netsnmp_arch_interface_container_load (netsnmp_container * container, u_int l_flags)
{
    ...
    if (error)
    {
        DEBUGMSGTL (("access:interface:container:arch", "error %d, free container\n", error));
 ...
 * @internal
 */
static int _set_ip_flags_v4 (netsnmp_interface_entry * entry, mib2_ifEntry_t * ife)
{
    ...
    if (_get_v4addr (ife, &ipv4e) > 0)
    {
        ...
        entry->ns_flags |= NETSNMP_INTERFACE_FLAGS_HAS_IPV4 | NETSNMP_INTERFACE_FLAGS_HAS_V4_REASMMAX;
 ...
 * @internal
 */
static int _set_ip_flags_v6 (netsnmp_interface_entry * entry, mib2_ifEntry_t * ife)
{
#ifdef SOLARIS_HAVE_IPV6_MIB_SUPPORT
    ...
    if (_get_v6addr (ife, &ipv6e) > 0)
    {
        ...
        if (ipv6e.ipv6AddrIdentifierLen <= sizeof (entry->v6_if_id))
        {
            ...
            (void) memcpy (&entry->v6_if_id, &ipv6e.ipv6AddrIdentifier, entry->v6_if_id_len);
        ...
        entry->ns_flags |=
            ...
            NETSNMP_INTERFACE_FLAGS_HAS_V6_RETRANSMIT | NETSNMP_INTERFACE_FLAGS_HAS_V6_REACHABLE;
