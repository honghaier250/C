 ...
 */
     static int _ioctl_get (int fd, int which, struct ifreq *ifrq, const char *name)
{
    int ourfd = -1, rc = 0;

    DEBUGMSGTL (("verbose:access:interface:ioctl", "ioctl %d for %s\n", which, name));
 ...
 */
int netsnmp_access_interface_ioctl_physaddr_get (int fd, netsnmp_interface_entry * ifentry)
{
    struct ifreq ifrq;
 ...
 * @retval -3 : ioctl call failed
 */
int netsnmp_access_interface_ioctl_flags_get (int fd, netsnmp_interface_entry * ifentry)
{
    struct ifreq ifrq;
...
netsnmp_access_interface_ioctl_flags_set (int fd,
                                          netsnmp_interface_entry * ifentry, unsigned int flags, int and_complement)
{
    struct ifreq ifrq;
 ...
 * @retval -3 : ioctl call failed
 */
int netsnmp_access_interface_ioctl_mtu_get (int fd, netsnmp_interface_entry * ifentry)
{
    struct ifreq ifrq;
...
oid netsnmp_access_interface_ioctl_ifindex_get (int fd, const char *name)
{
    ...
    if (rc < 0)
    {
        DEBUGMSGTL (("access:interface:ioctl", "ifindex_get error on inerface '%s'\n", name));
        return 0;
    }
 ...
 */
int netsnmp_access_interface_ioctl_has_ipv4 (int sd, const char *if_name, int if_index, u_int * flags)
{
    int i, interfaces = 0;
