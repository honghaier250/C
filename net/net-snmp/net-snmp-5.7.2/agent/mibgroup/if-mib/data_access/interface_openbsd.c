...
#define starttime (*(const struct timeval*)netsnmp_get_agent_starttime())
     int netsnmp_openbsd_interface_get_if_speed (char *name, u_int * speed, u_int * speed_high);
...
int netsnmp_arch_interface_container_load (netsnmp_container * container, u_int load_flags)
{
    ...
    {
        DEBUGMSGTL (("access:interface:container:arch", "allocated %u bytes for if_list\n", if_list_size));
    }
    ...
    for (cp = if_list; cp < if_list + if_list_size; cp += ifp->ifm_msglen)
    {
        ...
        DEBUGMSGTL (("access:interface:container:arch", "created entry %u for %s\n", entry->index, entry->name));
    ...
    for (cp = if_list; cp < if_list + if_list_size; cp += ifa->ifam_msglen)
    {
        ...
        DEBUGMSGTL (("access:interface:container:arch", "%s: flags=0x%x\n", entry->name, entry->ns_flags));
    }
...
void _openbsd_interface_ifmedia_to_speed (int media, u_int * speed, u_int * speed_high)
{
    *speed = 0;
    *speed_high = 0;
...
int netsnmp_openbsd_interface_get_if_speed (char *name, u_int * speed, u_int * speed_high)
{
    ...
    *speed_high = 0;
    ...
    if (*speed == 0 && (media_list = (int *) malloc (ifmr.ifm_count * sizeof (int))) != NULL)
    {
        ...
        if (ioctl (s, SIOCGIFMEDIA, (caddr_t) & ifmr) == 0)
        {
            ...
            for (i = 0; i < ifmr.ifm_count; i++)
            {
                ...
                if (t_speed_high > m_speed_high || (t_speed_high == m_speed_high && t_speed > t_speed))
                {
    ...
    DEBUGMSGTL (("access:interface:container:arch", "%s: speed: %u, speed_high: %u\n", name, *speed, *speed_high));
