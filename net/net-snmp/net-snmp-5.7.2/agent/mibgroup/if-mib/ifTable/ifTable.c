     ...
     */
    {
        ...
        myreg =
            netsnmp_create_handler_registration ("if number",
                                                 _if_number_handler, reg_oid, OID_LENGTH (reg_oid), HANDLER_CAN_RONLY);
...
ifPhysAddress_get (ifTable_rowreq_ctx * rowreq_ctx,
                   char **ifPhysAddress_val_ptr_ptr, size_t * ifPhysAddress_val_ptr_len_ptr)
{
   ...
   /** we should have a non-NULL pointer and enough storage */
    ...
    if ((rowreq_ctx->data.ifPhysAddress[0] == 0) &&
        ...
        (rowreq_ctx->data.ifPhysAddress[4] == 0) && (rowreq_ctx->data.ifPhysAddress[5] == 0))
    {
        /*
         * all 0s = empty string
         */
         ...
         (rowreq_ctx->data.ifPhysAddress_len * sizeof (rowreq_ctx->data.ifPhysAddress[0]))))
    {
        ...
        (*ifPhysAddress_val_ptr_ptr) =
            (char *) malloc (rowreq_ctx->data.ifPhysAddress_len * sizeof (rowreq_ctx->data.ifPhysAddress[0]));
        if (NULL == (*ifPhysAddress_val_ptr_ptr))
        {
            snmp_log (LOG_ERR, "could not allocate memory\n");
            return MFD_ERROR;
    ...
    memcpy ((*ifPhysAddress_val_ptr_ptr), rowreq_ctx->data.ifPhysAddress, (*ifPhysAddress_val_ptr_len_ptr));
...
int ifSpecific_get (ifTable_rowreq_ctx * rowreq_ctx, oid ** ifSpecific_val_ptr_ptr, size_t * ifSpecific_val_ptr_len_ptr)
{
   ...
   /** we should have a non-NULL pointer and enough storage */
    ...
    (*ifSpecific_val_ptr_len_ptr) = rowreq_ctx->data.ifSpecific_len * sizeof (rowreq_ctx->data.ifSpecific[0]);
    memcpy ((*ifSpecific_val_ptr_ptr), rowreq_ctx->data.ifSpecific, (*ifSpecific_val_ptr_len_ptr));
#else
    /*
     * hard coded
     */
...
int ifTable_commit (ifTable_rowreq_ctx * rowreq_ctx)
{
    ...
    if (save_flags & COLUMN_IFADMINSTATUS_FLAG)
    {
         ...
         */
        rc = netsnmp_access_interface_entry_set_admin_status (rowreq_ctx->data.ifentry, rowreq_ctx->data.ifAdminStatus);
        if (0 != rc)
        {
