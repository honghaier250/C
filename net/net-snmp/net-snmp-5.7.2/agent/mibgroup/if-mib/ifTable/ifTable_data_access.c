 ...
 */
     ...
     static void _delete_missing_interface (ifTable_rowreq_ctx * rowreq_ctx, netsnmp_container * container);
...
{
    if (strcmp (line, "yes") == 0 || strcmp (line, "y") == 0 || strcmp (line, "true") == 0 || strcmp (line, "1") == 0)
    {
        replace_old = 1;
        return;
    }
    if (strcmp (line, "no") == 0 || strcmp (line, "n") == 0 || strcmp (line, "false") == 0 || strcmp (line, "0") == 0)
    {
        replace_old = 0;
        return;
    }
...
int ifTable_init_data (ifTable_registration * ifTable_reg)
{
     ...
     */
    snmpd_register_config_handler ("interface_fadeout", parse_interface_fadeout, NULL, "interface_fadeout seconds");
 ...
 */
void ifTable_container_init (netsnmp_container ** container_ptr_ptr, netsnmp_cache * cache)
{
...
void
send_linkUpDownNotifications (oid * notification_oid, size_t notification_oid_len, int if_index, int if_admin_status,
                              int if_oper_status)
{
    /*
     ...
     */
    snmp_varlist_add_variable (&notification_vars,
                               if_index_oid, if_index_oid_len, ASN_INTEGER, (u_char *) & if_index, sizeof (if_index));
    ...
    snmp_varlist_add_variable (&notification_vars,
                               ...
                               ASN_INTEGER, (u_char *) & if_oper_status, sizeof (if_oper_status));
 ...
 *
 */
static void _check_interface_entry_for_updates (ifTable_rowreq_ctx * rowreq_ctx, cd_container * cdc)
{
    char oper_changed = 0;
...
#ifdef USING_IF_MIB_IFXTABLE_IFXTABLE_MODULE
            rowreq_ctx->data.ifCounterDiscontinuityTime = netsnmp_get_agent_uptime ();
#endif
        }
        ...
        if (rowreq_ctx->data.ifLinkUpDownTrapEnable == 1)
        {
            if (rowreq_ctx->data.ifOperStatus == IFOPERSTATUS_UP)
            {
                ...
                send_linkUpDownNotifications (notification_oid, OID_LENGTH (notification_oid),
                                              ...
                                              rowreq_ctx->data.ifAdminStatus, rowreq_ctx->data.ifOperStatus);
            }
 ...
 */
static void _check_and_replace_old (netsnmp_interface_entry * ifentry, netsnmp_container * container)
{
    netsnmp_iterator *it;
    ...
    CONTAINER_FOR_EACH (to_delete, (netsnmp_container_obj_func *) _delete_missing_interface, container);
 ...
 * add new entry
 */
static void _add_new_interface (netsnmp_interface_entry * ifentry, netsnmp_container * container)
{
    ifTable_rowreq_ctx *rowreq_ctx;
    ...
    if ((NULL != rowreq_ctx) && (MFD_SUCCESS == ifTable_indexes_set (rowreq_ctx, ifentry->index)))
    {
         ...
         */
        netsnmp_assert (0 == (ifentry->ns_flags & NETSNMP_INTERFACE_FLAGS_HAS_LASTCHANGE));
        if (0 == _first_load)
        {
            rowreq_ctx->data.ifLastChange = netsnmp_get_agent_uptime ();
    ...
    else
    {
        ...
        {
            snmp_log (LOG_ERR, "memory allocation failed while loading " "ifTable cache.\n");
            netsnmp_access_interface_entry_free (ifentry);
        }
    }
}
...
int ifTable_container_load (netsnmp_container * container)
{
     ...
     */
    cdc.current = netsnmp_access_interface_container_load (NULL, NETSNMP_ACCESS_INTERFACE_INIT_NOFLAGS);
    if (NULL == cdc.current)
     ...
     */
    CONTAINER_FOR_EACH (container, (netsnmp_container_obj_func *) _check_interface_entry_for_updates, &cdc);
    ...
    if (NULL != cdc.deleted)
    {
        CONTAINER_FOR_EACH (cdc.deleted, (netsnmp_container_obj_func *) _delete_missing_interface, container);
     ...
     */
    CONTAINER_FOR_EACH (cdc.current, (netsnmp_container_obj_func *) _add_new_interface, container);

    /*
     * free the container. we've either claimed each ifentry, or released it,
     * so the dal function doesn't need to clear the container.
     */
    netsnmp_access_interface_container_free (cdc.current, NETSNMP_ACCESS_INTERFACE_FREE_DONT_CLEAR);
