 ...
 */
     ...
     static void _udpEndpointTable_container_init (udpEndpointTable_interface_ctx * if_ctx);

     static void _udpEndpointTable_container_shutdown (udpEndpointTable_interface_ctx * if_ctx);
...
udpEndpointTable_registration *udpEndpointTable_registration_set (udpEndpointTable_registration * newreg)
{
 ...
 */
void _udpEndpointTable_initialize_interface (udpEndpointTable_registration * reg_ptr, u_long flags)
{
    netsnmp_baby_steps_access_methods *access_multiplexer = &udpEndpointTable_if_ctx.access_multiplexer;
    ...
    {
        snmp_log (LOG_ERR, "could not initialize container for udpEndpointTable\n");
        return;
    }
    ...
    handler =
        netsnmp_container_table_handler_get (tbl_info,
                                             udpEndpointTable_if_ctx.container, TABLE_CONTAINER_KEY_NETSNMP_INDEX);
    netsnmp_inject_handler (reginfo, handler);
 ...
 */
void _udpEndpointTable_shutdown_interface (udpEndpointTable_registration * reg_ptr)
{
    /*
 ...
 */
int udpEndpointTable_index_to_oid (netsnmp_index * oid_idx, udpEndpointTable_mib_index * mib_idx)
{
     ...
     * set up varbinds
     */
    memset (&var_udpEndpointLocalAddressType, 0x00, sizeof (var_udpEndpointLocalAddressType));
    var_udpEndpointLocalAddressType.type = ASN_INTEGER;
    memset (&var_udpEndpointLocalAddress, 0x00, sizeof (var_udpEndpointLocalAddress));
    ...
    memset (&var_udpEndpointRemoteAddressType, 0x00, sizeof (var_udpEndpointRemoteAddressType));
    var_udpEndpointRemoteAddressType.type = ASN_INTEGER;
    memset (&var_udpEndpointRemoteAddress, 0x00, sizeof (var_udpEndpointRemoteAddress));
     ...
     */
    var_udpEndpointLocalAddressType.next_variable = &var_udpEndpointLocalAddress;
    ...
    var_udpEndpointRemoteAddressType.next_variable = &var_udpEndpointRemoteAddress;
    ...
    DEBUGMSGTL (("verbose:udpEndpointTable:udpEndpointTable_index_to_oid", "called\n"));
    ...
    snmp_set_var_value (&var_udpEndpointLocalPort,
                        (u_char *) & mib_idx->udpEndpointLocalPort, sizeof (mib_idx->udpEndpointLocalPort));
    ...
    snmp_set_var_value (&var_udpEndpointRemotePort,
                        (u_char *) & mib_idx->udpEndpointRemotePort, sizeof (mib_idx->udpEndpointRemotePort));

    /*
     * udpEndpointInstance(7)/UNSIGNED32/ASN_UNSIGNED/u_long(u_long)//l/a/w/e/R/d/h 
     */
    snmp_set_var_value (&var_udpEndpointInstance,
                        (u_char *) & mib_idx->udpEndpointInstance, sizeof (mib_idx->udpEndpointInstance));


    err = build_oid_noalloc (oid_idx->oids, oid_idx->len, &oid_idx->len, NULL, 0, &var_udpEndpointLocalAddressType);
    if (err)
 ...
 */
int udpEndpointTable_index_from_oid (netsnmp_index * oid_idx, udpEndpointTable_mib_index * mib_idx)
{
    int err = SNMP_ERR_NOERROR;
    ...
    DEBUGMSGTL (("verbose:udpEndpointTable:udpEndpointTable_index_from_oid", "called\n"));

    /*
     * parse the oid into the individual index components
     */
    err = parse_oid_indexes (oid_idx->oids, oid_idx->len, &var_udpEndpointLocalAddressType);
    if (err == SNMP_ERR_NOERROR)
    {
         ...
         */
        if (var_udpEndpointLocalAddress.val_len > sizeof (mib_idx->udpEndpointLocalAddress))
            err = SNMP_ERR_GENERR;
        else
        {
            memcpy (mib_idx->udpEndpointLocalAddress,
                    var_udpEndpointLocalAddress.val.string, var_udpEndpointLocalAddress.val_len);
            mib_idx->udpEndpointLocalAddress_len =
                var_udpEndpointLocalAddress.val_len / sizeof (mib_idx->udpEndpointLocalAddress[0]);
        }
         ...
         */
        if (var_udpEndpointRemoteAddress.val_len > sizeof (mib_idx->udpEndpointRemoteAddress))
            err = SNMP_ERR_GENERR;
        else
        {
            memcpy (mib_idx->udpEndpointRemoteAddress,
                    var_udpEndpointRemoteAddress.val.string, var_udpEndpointRemoteAddress.val_len);
            mib_idx->udpEndpointRemoteAddress_len =
                var_udpEndpointRemoteAddress.val_len / sizeof (mib_idx->udpEndpointRemoteAddress[0]);
        }
...
{
    udpEndpointTable_rowreq_ctx *rowreq_ctx = SNMP_MALLOC_TYPEDEF (udpEndpointTable_rowreq_ctx);

    DEBUGMSGTL (("internal:udpEndpointTable:udpEndpointTable_allocate_rowreq_ctx", "called\n"));

    if (NULL == rowreq_ctx)
    {
        snmp_log (LOG_ERR, "Couldn't allocate memory for a " "udpEndpointTable_rowreq_ctx.\n");
        return NULL;
    }
 ...
 */
void udpEndpointTable_release_rowreq_ctx (udpEndpointTable_rowreq_ctx * rowreq_ctx)
{
                                   ...
                                   netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    {
        DEBUGMSGTL (("internal:udpEndpointTable", "skipping additional pre_request\n"));
    ...
    if (MFD_SUCCESS != rc)
    {
         ...
         */
        DEBUGMSGTL (("udpEndpointTable", "error %d from " "udpEndpointTable_pre_request\n", rc));
                                    ...
                                    netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    {
        DEBUGMSGTL (("internal:udpEndpointTable", "waiting for last post_request\n"));
        return SNMP_ERR_NOERROR;
    }

    packet_rc = netsnmp_check_all_requests_error (agtreq_info->asp, 0);
    rc = udpEndpointTable_post_request (udpEndpointTable_if_ctx.user_ctx, packet_rc);
    if (MFD_SUCCESS != rc)
    {
        /*
         * nothing we can do about it but log it
         */
        DEBUGMSGTL (("udpEndpointTable", "error %d from " "udpEndpointTable_post_request\n", rc));
    }
...
_udpEndpointTable_get_column (udpEndpointTable_rowreq_ctx * rowreq_ctx, netsnmp_variable_list * var, int column)
{
    ...
    switch (column)
    {
        ...
        default:
            snmp_log (LOG_ERR, "unknown column %d in _udpEndpointTable_get_column\n", column);
            break;
    }
                                  ...
                                  netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * requests)
{
    ...
    for (; requests; requests = requests->next)
    {
        ...
        rc = _udpEndpointTable_get_column (rowreq_ctx, requests->requestvb, tri->colnum);
        if (rc)
        {
            if (MFD_SKIP == rc)
            {
 ...
 * @internal
 */
static void _container_item_free (udpEndpointTable_rowreq_ctx * rowreq_ctx, void *context)
{
...
static void _container_free (netsnmp_container * container)
{
    ...
    if (NULL == container)
    {
        snmp_log (LOG_ERR, "invalid container in udpEndpointTable_container_free\n");
        return;
    }
...
void _udpEndpointTable_container_init (udpEndpointTable_interface_ctx * if_ctx)
{
    ...
    if_ctx->cache = netsnmp_cache_create (30,    /* timeout in seconds */
                                          _cache_load, _cache_free, udpEndpointTable_oid, udpEndpointTable_oid_size);
    ...
    if (NULL == if_ctx->container)
        if_ctx->container = netsnmp_container_find ("udpEndpointTable:table_container");
    if (NULL == if_ctx->container)
    {
        snmp_log (LOG_ERR, "error creating container in " "udpEndpointTable_container_init\n");
        return;
    }
 ...
 */
void _udpEndpointTable_container_shutdown (udpEndpointTable_interface_ctx * if_ctx)
{
...
udpEndpointTable_rowreq_ctx *udpEndpointTable_row_find_by_mib_index (udpEndpointTable_mib_index * mib_idx)
{
    ...
    rowreq_ctx = (udpEndpointTable_rowreq_ctx *) CONTAINER_FIND (udpEndpointTable_if_ctx.container, &oid_idx);
