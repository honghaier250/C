 ...
 */
netsnmp_container *netsnmp_access_udp_endpoint_container_load (netsnmp_container * container, u_int load_flags)
{
    int rc;
...
void netsnmp_access_udp_endpoint_container_free (netsnmp_container * container, u_int free_flags)
{
    DEBUGMSGTL (("access:udp_endpoint:container", "free\n"));

    if (NULL == container)
    {
        snmp_log (LOG_ERR, "invalid container for netsnmp_access_udp_endpoint_free\n");
        return;
    }
    ...
    if (!(free_flags & NETSNMP_ACCESS_UDP_ENDPOINT_FREE_DONT_CLEAR))
    {
         ...
         * free all items.
         */
        CONTAINER_CLEAR (container, (netsnmp_container_obj_func *) netsnmp_access_udp_endpoint_entry_free, NULL);
    }
...
{
    netsnmp_udp_endpoint_entry *entry = SNMP_MALLOC_TYPEDEF (netsnmp_udp_endpoint_entry);
