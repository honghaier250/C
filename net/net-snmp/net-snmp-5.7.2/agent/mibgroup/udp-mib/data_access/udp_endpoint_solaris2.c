...
netsnmp_feature_child_of (udp_endpoint_writable, udp_endpoint_all)
     ...
     int netsnmp_arch_udp_endpoint_entry_init (netsnmp_udp_endpoint_entry * ep)
{
    init_kernel_sunos5 ();
    return 0;
}

void netsnmp_arch_udp_endpoint_entry_cleanup (netsnmp_udp_endpoint_entry * ep)
{
    /*
     * Do nothing
     */
...
int netsnmp_arch_udp_endpoint_entry_copy (netsnmp_udp_endpoint_entry * ep1, netsnmp_udp_endpoint_entry * ep2)
{
    /*
     * Do nothing
     */
...
int netsnmp_arch_udp_endpoint_delete (netsnmp_udp_endpoint_entry * ep)
{
    /*
     * Not implemented 
     */
...
int netsnmp_arch_udp_endpoint_container_load (netsnmp_container * container, u_int load_flag)
{
    int rc;
...
static int _load_udp_endpoint_table_v4 (netsnmp_container * container, int flag)
{
    ...
    while (getMibstat (MIB_UDP_LISTEN, &ue, sizeof (ue), req, &Get_everything, 0) == 0)
    {
        req = GET_NEXT;
        ...
        else
        {
            ...
            memcpy (&ep->rmt_addr, &ue.udpEntryInfo.ue_RemoteAddress, ep->rmt_addr_len);
        }
...
#if defined(NETSNMP_ENABLE_IPV6) && defined(SOLARIS_HAVE_IPV6_MIB_SUPPORT)
static int _load_udp_endpoint_table_v6 (netsnmp_container * container, int flag)
{
    netsnmp_udp_endpoint_entry *ep;
    ...
    while (getMibstat (MIB_UDP6_ENDPOINT, &ue6, sizeof (ue6), req, &Get_everything, 0) == 0)
    {
        req = GET_NEXT;
        ep = netsnmp_access_udp_endpoint_entry_create ();
        if (ep == NULL)
        ...
        else
        {
            ...
            (void) memcpy (&ep->rmt_addr, &ue6.udp6EntryInfo.ue_RemoteAddress, ep->rmt_addr_len);
        }
