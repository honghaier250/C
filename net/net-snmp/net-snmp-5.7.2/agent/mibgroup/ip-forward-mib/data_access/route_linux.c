...
static int _load_ipv4 (netsnmp_container * container, u_long * index)
{
    ...
    while (fgets (line, sizeof (line), in))
    {
        ...
        if (8 != rc)
        {
            snmp_log (LOG_ERR, "/proc/net/route data format error (%d!=8), line ==|%s|", rc, line);
        ...
        entry->rt_proto = (flags & RTF_DYNAMIC) ? IANAIPROUTEPROTOCOL_ICMP : IANAIPROUTEPROTOCOL_LOCAL;
...
static int _load_ipv6 (netsnmp_container * container, u_long * index)
{
    ...
    while (fgets (line, sizeof (line), in))
    {
        ...
        if (7 != rc)
        {
            snmp_log (LOG_ERR, "/proc/net/ipv6_route data format error (%d!=8), " "line ==|%s|", rc, line);
            continue;
        }
        ...
        {
            snmp_log (LOG_ERR, "unknown interface in /proc/net/ipv6_route " "('%s')\n", c_name);
            netsnmp_access_route_entry_free (entry);
            continue;
        }
        /*
        ...
        netsnmp_hex_to_binary (&temp_uchar_ptr, &buf_len, &buf_offset, 0, c_dest, NULL);
        ...
        netsnmp_hex_to_binary (&temp_uchar_ptr, &buf_len, &buf_offset, 0, c_next, NULL);
 ...
 */
int netsnmp_access_route_container_arch_load (netsnmp_container * container, u_int load_flags)
{
    u_long count = 0;

    int rc;

    DEBUGMSGTL (("access:route:container", "route_container_arch_load (flags %x)\n", load_flags));
