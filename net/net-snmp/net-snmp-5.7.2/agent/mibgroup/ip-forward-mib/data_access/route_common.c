 ...
 * the header file.
 */
extern int netsnmp_access_route_container_arch_load (netsnmp_container * container, u_int load_flags);

extern int netsnmp_arch_route_create (netsnmp_route_entry * entry);

extern int netsnmp_arch_route_delete (netsnmp_route_entry * entry);
 ...
 */
netsnmp_container *netsnmp_access_route_container_load (netsnmp_container * container, u_int load_flags)
{
    int rc;
...
void netsnmp_access_route_container_free (netsnmp_container * container, u_int free_flags)
{
    ...
    if (!(free_flags & NETSNMP_ACCESS_ROUTE_FREE_DONT_CLEAR))
    {
         ...
         * free all items.
         */
        CONTAINER_CLEAR (container, (netsnmp_container_obj_func *) _access_route_entry_release, NULL);
    }
 ...
 * @retval 0   : no error
 */
int netsnmp_access_route_entry_copy (netsnmp_route_entry * lhs, netsnmp_route_entry * rhs)
{
    ...
    if (NULL != rhs->rt_policy)
    {
        ...
        {
            snmp_clone_mem ((void **) &lhs->rt_policy, rhs->rt_policy, rhs->rt_policy_len * sizeof (oid));
        }
    }
    ...
    if (NULL != rhs->rt_info)
        snmp_clone_mem ((void **) &lhs->rt_info, rhs->rt_info, rhs->rt_info_len * sizeof (oid));
