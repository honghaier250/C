...
static oid snmpNotifyFullCompliance[] = { SNMP_OID_SNMPMODULES, 13, 3, 1, 3 };    /* SNMP-NOTIFICATION-MIB::snmpNotifyFullCompliance */
...
static int _checkFilter (const char *paramName, netsnmp_pdu * pdu)
{
    ...
    /*
       ...
       management target.
     */
    profileName = get_FilterProfileName (paramName, strlen (paramName), &profileNameLen);
    if (NULL == profileName)
    {
     ...
     */
    trap_var = find_varbind_in_list (pdu->variables, snmptrap_oid, snmptrap_oid_len);
    if (NULL != trap_var)
    {
        /*
        ...
        if ((NULL == vp) || (SNMP_VIEW_INCLUDED != vp->viewType))
        {
            ...
            DEBUGMSGOID (("send_notifications", trap_var->val.objid, trap_var->val_len / sizeof (oid)));
    ...
    for (var = pdu->variables; var; var = var->next_variable)
    {
        ...
        vp = netsnmp_view_get (head, profileName, var->name, var->name_length, VACM_MODE_FIND);
...
int send_notifications (int major, int minor, void *serverarg, void *clientarg)
{
    ...
    for (hptr = snmpNotifyTableStorage; hptr; hptr = hptr->next)
    {
        ...
        for (sptr = sess; sptr; sptr = sptr->next)
        {
            ...
            if (sptr->version == SNMP_VERSION_1 && minor == SNMPD_CALLBACK_SEND_TRAP1)
            {
                send = 1;
            }
            else
...
int notifyTable_register_notifications (int major, int minor, void *serverarg, void *clientarg)
{
    ...
    if (i == MAX_ENTRIES)
    {
        snmp_log (LOG_ERR, "Can't register new trap destination: max limit reached: %d", MAX_ENTRIES);
        snmp_sess_close (ss);
        return (0);
    }
    ...
    if (!t)
    {
        snmp_log (LOG_ERR, "Cannot add new trap destination, transport is closed.");
        snmp_sess_close (ss);
        return 0;
    }
    ...
    if (ss->version == SNMP_VERSION_3)
    {
        ...
        memcpy ((void *) pptr->secName, (void *) ss->securityName, ss->securityNameLen);
    ...
    else
    {
        ...
        if (ss->community && (ss->community_len > 0))
        {
            ...
            memcpy ((void *) pptr->secName, (void *) ss->community, ss->community_len);
 ...
 */
int notifyTable_unregister_notifications (int major, int minor, void *serverarg, void *clientarg)
{
...
void init_snmpNotifyTable (void)
{
     ...
     */
    REGISTER_MIB ("snmpNotifyTable", snmpNotifyTable_variables, variable2, snmpNotifyTable_variables_oid);


    /*
     * register our config handler(s) to deal with registrations 
     */
    snmpd_register_config_handler ("snmpNotifyTable", parse_snmpNotifyTable, NULL, NULL);


    /*
     * we need to be called back later to store our data 
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_snmpNotifyTable, NULL);

#ifndef DISABLE_SNMPV1
    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_SEND_TRAP1, send_notifications, NULL);
#endif
    snmp_register_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_SEND_TRAP2, send_notifications, NULL);
    snmp_register_callback (SNMP_CALLBACK_APPLICATION,
                            SNMPD_CALLBACK_REGISTER_NOTIFICATIONS, notifyTable_register_notifications, NULL);
    snmp_register_callback (SNMP_CALLBACK_APPLICATION,
                            SNMPD_CALLBACK_PRE_UPDATE_CONFIG, notifyTable_unregister_notifications, NULL);

    /*
     * place any other initialization junk you need here 
     */

    REGISTER_SYSOR_ENTRY (snmpNotifyFullCompliance, "The MIB modules for managing SNMP Notification, plus filtering.");
...
void shutdown_snmpNotifyTable (void)
{
    ...
    notifyTable_unregister_notifications (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_PRE_UPDATE_CONFIG, NULL, NULL);

    snmp_unregister_callback (SNMP_CALLBACK_APPLICATION,
                              SNMPD_CALLBACK_PRE_UPDATE_CONFIG, notifyTable_unregister_notifications, NULL, FALSE);
    snmp_unregister_callback (SNMP_CALLBACK_APPLICATION,
                              SNMPD_CALLBACK_REGISTER_NOTIFICATIONS, notifyTable_register_notifications, NULL, FALSE);
    snmp_unregister_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_SEND_TRAP2, send_notifications, NULL, FALSE);
#ifndef DISABLE_SNMPV1
    snmp_unregister_callback (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_SEND_TRAP1, send_notifications, NULL, FALSE);
#endif
    snmp_unregister_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_snmpNotifyTable, NULL, FALSE);
...
int snmpNotifyTable_add (struct snmpNotifyTable_data *thedata)
{
    ...
    if (header_complex_maybe_add_data (&snmpNotifyTableStorage, vars, thedata, 1) != NULL)
    {
 ...
 */
int store_snmpNotifyTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (hcindex = snmpNotifyTableStorage; hcindex != NULL; hcindex = hcindex->next)
    {
            ...
            (StorageTmp->snmpNotifyStorageType == ST_PERMANENT))
        {
            ...
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr,
                                        &StorageTmp->snmpNotifyName, &StorageTmp->snmpNotifyNameLen);
            cptr =
                read_config_store_data (ASN_OCTET_STR, cptr, &StorageTmp->snmpNotifyTag, &StorageTmp->snmpNotifyTagLen);
 ...
 */
unsigned char *var_snmpNotifyTable (struct variable *vp,
                                    oid * name,
                                    size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    if ((StorageTmp = (struct snmpNotifyTable_data *)
         header_complex ((struct header_complex_index *)
                         snmpNotifyTableStorage, vp, name, length, exact, var_len, write_method)) == NULL)
    {
        found = 0;
    }
                             ...
                             u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            if (value != SNMP_STORAGE_OTHER && value != SNMP_STORAGE_VOLATILE && value != SNMP_STORAGE_NONVOLATILE)
            {
                return SNMP_ERR_WRONGVALUE;
            }
            break;
                           ...
                           u_char var_val_type, size_t var_val_len, u_char * statP, oid * name, size_t name_len)
{
    ...
    switch (action)
    {
        case RESERVE1:
             ...
             * memory reseveration, final preparation... 
             */
            if (StorageTmp == NULL && (set_value == RS_CREATEANDGO || set_value == RS_CREATEANDWAIT))
            {
                /*
                 * creation 
                 */
                ...
                if (header_complex_parse_oid
                    (&
                     (name
                      [sizeof (snmpNotifyTable_variables_oid) / sizeof (oid) + 2]), newlen, vars) != SNMPERR_SUCCESS)
                {
                    /*
                     * XXX: free, zero vars 
                     */
        ...
        case UNDO:
            ...
            if (StorageTmp == NULL && (set_value == RS_CREATEANDGO || set_value == RS_CREATEANDWAIT))
            {
                 ...
                 */
                hciptr = header_complex_find_entry (snmpNotifyTableStorage, StorageNew);
                StorageDel = (struct snmpNotifyTable_data *)
                    header_complex_extract_entry ((struct header_complex_index **) &snmpNotifyTableStorage, hciptr);
                /*
                 * XXX: free it 
        ...
        case COMMIT:
            ...
            else if (StorageTmp && StorageTmp->snmpNotifyRowStatus == RS_CREATEANDWAIT)
            {
