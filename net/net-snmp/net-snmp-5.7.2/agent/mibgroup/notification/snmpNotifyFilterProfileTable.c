...
void init_snmpNotifyFilterProfileTable (void)
{
    ...
    REGISTER_MIB ("snmpNotifyFilterProfileTable",
                  snmpNotifyFilterProfileTable_variables, variable2, snmpNotifyFilterProfileTable_variables_oid);


    /*
     * register our config handler(s) to deal with registrations 
     */
    snmpd_register_config_handler ("snmpNotifyFilterProfileTable", parse_snmpNotifyFilterProfileTable, NULL, NULL);
     ...
     */
    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_STORE_DATA, store_snmpNotifyFilterProfileTable, NULL);
...
int snmpNotifyFilterProfileTable_add (struct snmpNotifyFilterProfileTable_data *thedata)
{
    ...
    snmp_varlist_add_variable (&vars, NULL, 0, ASN_PRIV_IMPLIED_OCTET_STR,
                               (u_char *) thedata->snmpTargetParamsName, thedata->snmpTargetParamsNameLen);

    if (header_complex_maybe_add_data (&snmpNotifyFilterProfileTableStorage, vars, thedata, 1) != NULL)
    {
...
void parse_snmpNotifyFilterProfileTable (const char *token, char *line)
{
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->snmpTargetParamsName, &StorageTmp->snmpTargetParamsNameLen);
    ...
    line =
        read_config_read_data (ASN_OCTET_STR, line,
                               &StorageTmp->snmpNotifyFilterProfileName, &StorageTmp->snmpNotifyFilterProfileNameLen);
 ...
 */
int store_snmpNotifyFilterProfileTable (int majorID, int minorID, void *serverarg, void *clientarg)
{
    ...
    for (hcindex = snmpNotifyFilterProfileTableStorage; hcindex != NULL; hcindex = hcindex->next)
    {
            ...
            (StorageTmp->snmpNotifyFilterProfileStorType == ST_PERMANENT))
        {
            ...
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->snmpNotifyFilterProfileStorType, NULL);
            cptr = read_config_store_data (ASN_INTEGER, cptr, &StorageTmp->snmpNotifyFilterProfileRowStatus, NULL);
 ...
 */
unsigned char *var_snmpNotifyFilterProfileTable (struct variable *vp,
                                                 oid * name,
                                                 size_t * length,
                                                 int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    if ((StorageTmp = (struct snmpNotifyFilterProfileTable_data *)
         header_complex ((struct header_complex_index *)
                         snmpNotifyFilterProfileTableStorage, vp, name, length, exact, var_len, write_method)) == NULL)
    {
        found = 0;
    }
    ...
    switch (action)
    {
        case RESERVE1:
            ...
            if (StorageTmp == NULL && (set_value == RS_CREATEANDGO || set_value == RS_CREATEANDWAIT))
            {
                ...
                memdup ((u_char **) & (StorageNew->snmpTargetParamsName), vars->val.string, vars->val_len);
                ...
                if (StorageTmp)
                {
                    ...
                    StorageDel = (struct snmpNotifyFilterProfileTable_data *)
                        header_complex_extract_entry ((struct
                                                      ...
                                                      &snmpNotifyFilterProfileTableStorage, hciptr);
                }
...
char *get_FilterProfileName (const char *paramName, size_t paramName_len, size_t * profileName_len)
{
    netsnmp_variable_list *vars = NULL;
     ...
     */
    snmp_varlist_add_variable (&vars, NULL, 0, ASN_PRIV_IMPLIED_OCTET_STR, (const u_char *) paramName, paramName_len);

    /*
     * get the data from the header_complex storage 
     */
    data = (struct snmpNotifyFilterProfileTable_data *) header_complex_get (snmpNotifyFilterProfileTableStorage, vars);
