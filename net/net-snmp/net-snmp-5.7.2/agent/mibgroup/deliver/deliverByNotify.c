...
#define ESTIMATE_VAR_SIZE(v) (v?(v->name_length + v->val_len + 8):0)
     ...
     oid data_notification_oid[MAX_OID_LEN] = { 1, 3, 6, 1, 4, 1, 8072, 3, 1, 5, 4, 0, 1 };

size_t data_notification_oid_len = 13;

oid netsnmp_periodic_time_oid[MAX_OID_LEN] = { 1, 3, 6, 1, 4, 1, 8072, 3, 1, 5, 3, 1, 0 };

size_t netsnmp_periodic_time_oid_len = 13;

oid netsnmp_message_number_oid[MAX_OID_LEN] = { 1, 3, 6, 1, 4, 1, 8072, 3, 1, 5, 3, 2, 0 };

size_t netsnmp_message_number_oid_len = 13;

oid netsnmp_max_message_number_oid[MAX_OID_LEN] = { 1, 3, 6, 1, 4, 1, 8072, 3, 1, 5, 3, 3, 0 };
...
static int _deliver_compare (deliver_by_notify * lhs, deliver_by_notify * rhs)
{
...
void init_deliverByNotify (void)
{
    ...
    snmpd_register_config_handler ("deliverByNotify",
                                   &parse_deliver_config, &free_deliver_config, "[-p] [-m] [-s maxsize] FREQUENCY OID");

    snmpd_register_config_handler ("deliverByNotifyMaxPacketSize", &parse_deliver_maxsize_config, NULL, "sizeInBytes");

    snmpd_register_config_handler ("deliverByNotifyOid", &parse_data_notification_oid_config, NULL,    /* XXX: reset to default */
                                   "OID");

    snmpd_register_config_handler ("deliverByNotifyFrequencyOid", &parse_periodic_time_oid_config, NULL,    /* XXX: reset to default */
                                   "OID");

    snmpd_register_config_handler ("deliverByNotifyMessageNumberOid", &parse_message_number_oid_config, NULL,    /* XXX: reset to default */
                                   "OID");

    snmpd_register_config_handler ("deliverByNotifyMaxMessageNumberOid", &parse_max_message_number_oid_config, NULL,    /* XXX: reset to default */
                                   "OID");
    ...
    {
        snmp_log (LOG_ERR, "deliverByNotify: failed to initialize our data container\n");
        return;
    }
...
void _parse_config_oid (const char *token, char *line, oid * oid_store, size_t * oid_store_len)
{
...
void parse_data_notification_oid_config (const char *token, char *line)
{
    _parse_config_oid (token, line, data_notification_oid, &data_notification_oid_len);
}

void parse_periodic_time_oid_config (const char *token, char *line)
{
    _parse_config_oid (token, line, netsnmp_periodic_time_oid, &netsnmp_periodic_time_oid_len);
}

void parse_message_number_oid_config (const char *token, char *line)
{
    _parse_config_oid (token, line, netsnmp_message_number_oid, &netsnmp_message_number_oid_len);
}

void parse_max_message_number_oid_config (const char *token, char *line)
{
    _parse_config_oid (token, line, netsnmp_max_message_number_oid, &netsnmp_max_message_number_oid_len);
}

void parse_deliver_config (const char *token, char *line)
{
    const char *cp = line;
    ...
    snmp_alarm_register (calculate_time_until_next_run (new_notify, NULL), 0, &deliver_execute, NULL);
...
void parse_deliver_maxsize_config (const char *token, char *line)
{
    default_max_size = atoi (line);
}

static void _free_deliver_obj (deliver_by_notify * obj, void *context)
{
...
{
    default_max_size = DEFAULT_MAX_DELIVER_SIZE;
    CONTAINER_CLEAR (deliver_container, (netsnmp_container_obj_func *) _free_deliver_obj, NULL);
    if (alarm_reg)
    {
...
void deliver_execute (unsigned int clientreg, void *clientarg)
{
    ...
    for (obj = ITERATOR_FIRST (iterator); obj; obj = ITERATOR_NEXT (iterator))
    {
        ...
        while (walker)
        {
            ...
            snmp_varlist_add_variable (&deliver_notification,
                                       ...
                                       ASN_OBJECT_ID, data_notification_oid, data_notification_oid_len * sizeof (oid));
            ...
            if (!(obj->flags & NETSNMP_DELIVER_NO_PERIOD_OID))
            {
                ...
                snmp_varlist_add_variable (&deliver_notification,
                                           ...
                                           ASN_UNSIGNED, (const void *) &tmp_long, sizeof (tmp_long));
                estimated_pkt_size += ESTIMATE_VAR_SIZE (deliver_notification);
            }
            ...
            if (!(obj->flags & NETSNMP_DELIVER_NO_MSG_COUNTS))
            {
                snmp_varlist_add_variable (&deliver_notification,
                                           ...
                                           ASN_UNSIGNED, (const void *) &message_count, sizeof (message_count));
                estimated_pkt_size += ESTIMATE_VAR_SIZE (deliver_notification);
                ...
                vartmp = snmp_varlist_add_variable (&deliver_notification,
                                                    ...
                                                    ASN_UNSIGNED,
                                                    (const void *) &max_message_count, sizeof (max_message_count));
                estimated_pkt_size += ESTIMATE_VAR_SIZE (deliver_notification);
            ...
            while (walker)
            {
                snmp_varlist_add_variable (&deliver_notification,
                                           ...
                                           walker->type, walker->val.string, walker->val_len);
                ...
                if (obj->max_packet_size > 0 && estimated_pkt_size + ESTIMATE_VAR_SIZE (walker) >= obj->max_packet_size)
                {
                    break;
                }
            }
