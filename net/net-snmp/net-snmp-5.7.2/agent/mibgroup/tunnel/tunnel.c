 ...
 */
extern unsigned char *var_ifEntry (struct variable *, oid *, size_t *, int, size_t *, WriteMethod **);
#endif
...
{
    REGISTER_SYSOR_ENTRY (sysORTable_reg, "RFC 2667 TUNNEL-MIB implementation for " "Linux 2.2.x kernels.");

    /*
     * register ourselves with the agent to handle our mib tree 
     */
    REGISTER_MIB ("tunnel", tunnel_variables, variable4, tunnel_variables_oid);

    snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_SHUTDOWN, term_tunnel, NULL);
...
static int getType (int index)
{
    ...
    struct variable ifType_variable = { 3, ASN_INTEGER, NETSNMP_OLDAPI_RONLY,
    ...
    p = var_ifEntry (&ifType_variable, name, &length, 1 /* exact */ , &var_len, &write_method);
    if (!p)
        return 0;
...
static const char *getName (int index)
{
    ...
    struct variable ifName_variable = { 2, ASN_INTEGER, NETSNMP_OLDAPI_RONLY,
    ...
    p = var_ifEntry (&ifName_variable, name, &length, 1 /* exact */ , &var_len, &write_method);
    if (!p)
        return NULL;
...
static struct tunnel *updateTunnel (struct tunnel *tunnel)
{
    ...
    if (!parm)
    {
        DEBUGMSGTL (("tunnel", "updateTunnel(): getTunnelParm(\"%s\") returned NULL\n", tunnel->ifname));
        tunnel->active = 0;
        return NULL;
    }
...
static void updateTunnels (void)
{
    ...
    {
        DEBUGMSG (("tunnel", "updateTunnels(): looking for new index=%d\n", max_index));
...
static struct tunnel *getTunnelByConfigOid (oid * name, size_t * length)
{
    ...
    for (tunnel = tunnels; tunnel; tunnel = tunnel->next)
    {
        ...
        if (!snmp_oid_compare (tname, 4 + 4 + 1 + 1, &name[tunnel_len + 3], (*length) - tunnel_len - 3))
        {
            if (!tunnel->active)
                break;
            DEBUGMSG (("tunnel", "%s (index=%" NETSNMP_PRIo "u)\n", tunnel->ifname, tunnel->ifindex));
...
static struct tunnel *getNextTunnelByConfigOid (oid * name, size_t * length)
{
    ...
    for (tunnel = tunnels; tunnel; tunnel = tunnel->next)
    {
        if (!tunnel->active)
            continue;
        fillConfigOid (tname, tunnel);
        if (snmp_oid_compare (tname, 10, &name[tunnel_len + 3], (*length) - tunnel_len - 3) > 0)
        {
            if (!last_tunnel)
            {
                last_tunnel = tunnel;
...
writeLocalAddress (int action, unsigned char *var_val,
                   unsigned char var_val_type, size_t var_val_len, unsigned char *statP, oid * name, size_t name_len)
{
                                  ...
                                  oid * name, size_t * length, int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    else
    {
        ...
        {
            memcpy ((char *) name, (char *) tunnel_variables_oid, tunnel_len * sizeof (oid));
        }
                                      ...
                                      int exact, size_t * var_len, WriteMethod ** write_method)
{
    ...
    {
        if (snmp_oid_compare (name, *length, tunnel_configEntry_oid, tunnel_configEntry_len) < 0)
        {
            *length = 0;
        }
        if ((*length) < tunnel_len)
        ...
        for (i = MAX (*length, tunnel_len + 3); i < tunnel_len + 3 + 4 + 4 + 1 + 1; i++)
        {
            name[i] = 0;
        }
