 ...
 */
NETSNMP_STATIC_INLINE lookup_cache *lookup_cache_find (const char *context, const oid * name, size_t name_len,
                                                       int *retcmp)
{
    lookup_cache_context *cptr;
    ...
    for (i = 0; i < cptr->thecachecount && i < lookup_cache_size; i++)
    {
        if (cptr->cache[i].previous->start_a)
            cmp = snmp_oid_compare (name, name_len,
                                    cptr->cache[i].previous->start_a, cptr->cache[i].previous->start_len);
        else
            cmp = 1;
        if (cmp >= 0)
        {
...
void netsnmp_subtree_free (netsnmp_subtree * a)
{
    if (a != NULL)
    {
        if (a->variables != NULL && netsnmp_oid_equals (a->name_a, a->namelen, a->start_a, a->start_len) == 0)
        {
...
netsnmp_subtree *netsnmp_subtree_deepcopy (netsnmp_subtree * a)
{
    ...
    if (b != NULL)
    {
        ...
        if (a->variables != NULL)
        {
            b->variables = (struct variable *) malloc (a->variables_len * a->variables_width);
            if (b->variables != NULL)
            {
...
NETSNMP_INLINE void netsnmp_subtree_change_next (netsnmp_subtree * ptr, netsnmp_subtree * thenext)
{
    ptr->next = thenext;
    if (thenext)
        netsnmp_oid_compare_ll (ptr->start_a, ptr->start_len, thenext->start_a, thenext->start_len, &thenext->oid_off);
}
...
NETSNMP_INLINE void netsnmp_subtree_change_prev (netsnmp_subtree * ptr, netsnmp_subtree * theprev)
{
    ptr->prev = theprev;
    if (theprev)
        netsnmp_oid_compare_ll (theprev->start_a, theprev->start_len, ptr->start_a, ptr->start_len, &ptr->oid_off);
}
...
netsnmp_subtree *netsnmp_subtree_split (netsnmp_subtree * current, oid name[], int name_len)
{
    ...
    for (vp = current->variables; i > 0; i--)
    {
        ...
        rc = snmp_oid_compare (vp->name, vp->namelen, name + current->namelen, name_len - current->namelen);
        ...
        rc2 = snmp_oid_compare (vp->name, common_len, name + current->namelen, common_len);
    ...
    if (current->variables_len > 0 && IS_DELEGATED ((u_char) current->variables[0].type))
    {
    ...
    if (current->children)
    {
        new_sub->children = netsnmp_subtree_split (current->children, name, name_len);
    }
...
int netsnmp_subtree_load (netsnmp_subtree * new_sub, const char *context_name)
{
    ...
    if (!netsnmp_subtree_find_first (context_name))
    {
        ...
        if (!inloop)
        {
            ...
            netsnmp_register_null_context (snmp_duplicate_objid (iso, 1), 1, context_name);
            netsnmp_register_null_context (snmp_duplicate_objid (joint_ccitt_iso, 1), 1, context_name);
            inloop = 0;
        }
    }
    ...
    tree1 = netsnmp_subtree_find (new_sub->start_a, new_sub->start_len, NULL, context_name);
    ...
    if (tree1 == NULL)
    {
        tree2 = netsnmp_subtree_find_next (new_sub->start_a, new_sub->start_len, NULL, context_name);
    }
    else
    {
        tree2 = tree1->next;
    ...
    if (tree1 == NULL)
    {
        ...
        if (tree2 && snmp_oid_compare (new_sub->end_a, new_sub->end_len, tree2->start_a, tree2->start_len) > 0)
        {
    ...
    else
    {
        ...
        if (netsnmp_oid_equals (new_sub->start_a, new_sub->start_len, tree1->start_a, tree1->start_len) != 0)
        {
            tree1 = netsnmp_subtree_split (tree1, new_sub->start_a, new_sub->start_len);
        }
        ...
        switch (snmp_oid_compare (new_sub->end_a, new_sub->end_len, tree1->end_a, tree1->end_len))
        {
                ...
                if (next && (next->namelen == new_sub->namelen) && (next->priority == new_sub->priority))
                {
                    ...
                    {            /* ignore root OID dups */
                        ...
                        netsnmp_sprint_realloc_objid ((u_char **) & buf, &buf_len, &out_len,
                                                      1, &buf_overflow, new_sub->start_a, new_sub->start_len);
                        snmp_log (LOG_ERR,
                                  "duplicate registration: MIB modules %s and %s (oid %s%s).\n",
                                  next->label_a, new_sub->label_a, buf ? buf : "", buf_overflow ? " [TRUNCATED]" : "");
                        free (buf);
                    }
            ...
            case 1:
                ...
                {
                    netsnmp_subtree *new2 = netsnmp_subtree_split (new_sub, tree1->end_a, tree1->end_len);
...
netsnmp_subtree *netsnmp_subtree_find_prev (const oid * name, size_t len, netsnmp_subtree * subtree,
...
#define WTEST_OPTIMIZATION 1
    ...
    for (; myptr != NULL; previous = myptr, myptr = myptr->next)
    {
        ...
        if (!(ll_off && myptr->oid_off && myptr->oid_off > ll_off) &&
            netsnmp_oid_compare_ll (name, len, myptr->start_a, myptr->start_len, &ll_off) < 0)
        {
#else
                                            ...
                                            netsnmp_subtree * subtree, const char *context_name)
{
    ...
    else if (subtree != NULL && snmp_oid_compare (name, len, subtree->start_a, subtree->start_len) < 0)
    {
        return subtree;
    }
    else
...
netsnmp_subtree *netsnmp_subtree_find (const oid * name, size_t len, netsnmp_subtree * subtree,
                                       const char *context_name)
{
    netsnmp_subtree *myptr;

    myptr = netsnmp_subtree_find_prev (name, len, subtree, context_name);
    if (myptr && myptr->end_a && snmp_oid_compare (name, len, myptr->end_a, myptr->end_len) < 0)
    {
        return myptr;
    }
...
netsnmp_register_mib (const char *moduleName,
                      ...
                      netsnmp_session * ss,
                      const char *context,
                      int timeout, int flags, netsnmp_handler_registration * reginfo, int perform_callback)
{
    ...
    if (((NULL == context) && (NULL != reginfo->contextName)) ||
        ...
        (((NULL != context) && (NULL != reginfo->contextName)) && (0 != strcmp (context, reginfo->contextName))))
    {
    ...
    if (res == MIB_REGISTERED_OK && range_subid != 0)
    {
        ...
        for (i = mibloc[range_subid - 1] + 1; i <= (int) range_ubound; i++)
        {
            ...
            if (sub2 == NULL)
            {
                unregister_mib_context (mibloc, mibloclen, priority, range_subid, range_ubound, context);
    ...
    if (res == MIB_REGISTERED_OK && perform_callback)
    {
        ...
        reg_parms.contextName = context;
        snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_REGISTER_OID, &reg_parms);
    }
...
register_mib_context (const char *moduleName,
                      ...
                      int priority,
                      int range_subid,
                      oid range_ubound, netsnmp_session * ss, const char *context, int timeout, int flags)
{
    return netsnmp_register_old_api (moduleName, var, varsize, numvars,
                                     mibloc, mibloclen, priority,
                                     range_subid, range_ubound, ss, context, timeout, flags);
}
...
register_mib_range (const char *moduleName,
                    ...
                    size_t numvars,
                    const oid * mibloc,
                    size_t mibloclen, int priority, int range_subid, oid range_ubound, netsnmp_session * ss)
{
    return register_mib_context (moduleName, var, varsize, numvars,
                                 mibloc, mibloclen, priority, range_subid, range_ubound, ss, "", -1, 0);
}
...
register_mib_priority (const char *moduleName,
                       struct variable *var,
                       size_t varsize, size_t numvars, const oid * mibloc, size_t mibloclen, int priority)
{
    return register_mib_range (moduleName, var, varsize, numvars, mibloc, mibloclen, priority, 0, 0, NULL);
}
...
register_mib (const char *moduleName,
              struct variable *var, size_t varsize, size_t numvars, const oid * mibloc, size_t mibloclen)
{
    return register_mib_priority (moduleName, var, varsize, numvars, mibloc, mibloclen, DEFAULT_MIB_PRIORITY);
}
 ...
 */
int
unregister_mib_context (oid * name, size_t len, int priority, int range_subid, oid range_ubound, const char *context)
{
    ...
    while (unregistering)
    {
        ...
        list = netsnmp_subtree_find (name, len, netsnmp_subtree_find_first (context), context);
        if (list == NULL)
        {
            return MIB_NO_SUCH_REGISTRATION;
        }

        for (child = list, prev = NULL; child != NULL; prev = child, child = child->children)
        {
            if (netsnmp_oid_equals (child->name_a, child->namelen, name, len) == 0 && child->priority == priority)
            {
                break;            /* found it */
            }
        }
        ...
        for (list = myptr->next; list != NULL; list = next)
        {
            ...
            for (child = list, prev = NULL; child != NULL; prev = child, child = child->children)
            {
                if ((netsnmp_oid_equals (child->name_a, child->namelen,
                                         name, len) == 0) && (child->priority == priority))
                {
                    netsnmp_subtree_unload (child, prev, context);
    ...
    reg_parms.contextName = context;
    snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_UNREGISTER_OID, &reg_parms);
...
void unregister_mibs_by_session (netsnmp_session * ss)
{
    ...
    for (contextptr = get_top_context_cache (); contextptr != NULL; contextptr = contextptr->next)
    {
        for (list = contextptr->first_subtree; list != NULL; list = list2)
        {
            ...
            for (child = list, prev = NULL; child != NULL; child = next_child)
            {
                ...
                if (((!ss || ss->flags & SNMP_FLAGS_SUBSESSION) &&
                     child->session == ss) ||
                    (!(!ss || ss->flags & SNMP_FLAGS_SUBSESSION) && child->session && child->session->subsession == ss))
                {
                    ...
                    snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_UNREGISTER_OID, &rp);
                    SNMP_FREE (rp.name);
                }
                else
                {
...
#endif
        case SNMP_VERSION_3:
            snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_ACM_CHECK, &view_parms);
...
#endif
        case SNMP_VERSION_3:
            snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_ACM_CHECK_INITIAL, &view_parms);
            return view_parms.errorcode;
    }
    return 1;
}
...
#endif
        case SNMP_VERSION_3:
            snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_ACM_CHECK_SUBTREE, &view_parms);
            return view_parms.errorcode;
    }
    return 1;
}
...
netsnmp_feature_child_of (get_session_for_oid, netsnmp_unused)
     ...
     netsnmp_session *get_session_for_oid (const oid * name, size_t len, const char *context_name)
{
    netsnmp_subtree *myptr;

    myptr = netsnmp_subtree_find_prev (name, len, netsnmp_subtree_find_first (context_name), context_name);
...
#ifdef USING_AGENTX_SUBAGENT_MODULE
    netsnmp_ds_set_boolean (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_ROLE, role);
#endif
}
...
int remove_tree_entry (oid * name, size_t len)
{
    ...
    return unregister_mib_context (name, len, sub->priority, sub->range_subid, sub->range_ubound, "");
...
void dump_registry (void)
{
    ...
    if ((s = (u_char *) calloc (sl, 1)) != NULL &&
        (e = (u_char *) calloc (sl, 1)) != NULL && (v = (u_char *) calloc (sl, 1)) != NULL)
    {
        ...
        for (ptr = context_subtrees; ptr; ptr = ptr->next)
        {
            ...
            for (myptr = ptr->first_subtree; myptr != NULL; myptr = myptr->next)
            {
                ...
                if (!sprint_realloc_objid (&s, &sl, &sl_o, 1, myptr->start_a, myptr->start_len))
                {
                    break;
                }
                if (!sprint_realloc_objid (&e, &el, &el_o, 1, myptr->end_a, myptr->end_len))
                {
                    break;
                }
                ...
                for (myptr2 = myptr; myptr2 != NULL; myptr2 = myptr2->children)
                {
                    if (myptr2->label_a && myptr2->label_a[0])
                    {
                        ...
                        {
                            struct variable *vp = (struct variable *) myptr2->reginfo->handler->myvoid;

                            if (!sprint_realloc_objid (&s, &sl, &sl_o, 1, vp->name, vp->namelen))
                            {
                                continue;
                            }
                            printf ("\t%s[%s] %p var %s\n", myptr2->label_a,
                                    myptr2->reginfo->handlerName ?
                                    myptr2->reginfo->handlerName : "no-name", myptr2->reginfo, s);
                        }
                        else
                        {
                            printf ("\t%s %s %p\n", myptr2->label_a,
                                    myptr2->reginfo->handlerName ?
                                    myptr2->reginfo->handlerName : "no-handler-name", myptr2->reginfo);
                        }
                    }
                }
            }
