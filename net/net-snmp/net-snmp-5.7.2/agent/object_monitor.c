...
static int check_registered (unsigned int event, oid * o, int o_l, watcher_list ** pWl, monitor_info ** pMi);
 ...
 * 
 */
void netsnmp_monitor_init (void)
{
    if (!need_init)
        return;
...
netsnmp_monitor_register (oid * object, size_t oid_len, int priority,
                          unsigned int events, void *watcher_data, netsnmp_object_monitor_callback * cb)
{
    monitor_info *mi;
 ...
 */
int
netsnmp_monitor_unregister (oid * object, size_t oid_len, int priority, void *wd, netsnmp_object_monitor_callback * cb)
{
    ...
    while (mi)
    {
        if ((mi->cb == cb) && (mi->priority == priority) && (mi->watcher_data == wd))
            break;
        last = mi;
        mi = mi->next;
    }
 ...
 */
void netsnmp_notify_monitor (netsnmp_monitor_callback_header * cbh)
{
 ...
 */
int netsnmp_monitor_check_registered (int event, oid * o, int o_l)
{
 ...
 */
void netsnmp_monitor_process_callbacks (void)
{
    netsnmp_assert (need_init == 0);
 ...
 *                    freed).
 */
void netsnmp_notify_cooperative (int event, oid * o, size_t o_len, char o_steal, void *object_info)
{
    ...
    if (NULL == cbh)
    {
        snmp_log (LOG_ERR, "could not allocate memory for " "cooperative callback");
        return;
    }
...
static int insert_watcher (oid * object, size_t oid_len, monitor_info * mi)
{
    watcher_list *wl = find_watchers (object, oid_len);
 ...
 *@internal
 */
inline void insert_ready (callback_placeholder * new_cbr)
{
...
static void move_pending_to_ready (void)
{
    ...
    while (callback_pending_list)
    {
        ...
        if (0 == check_registered (cbp->event, cbp->monitored_object.oids, cbp->monitored_object.len, &wl, &mi))
        {
 ...
 *
 */
void dummy_callback (netsnmp_monitor_callback_header * cbh)
{
    printf ("Callback received.\n");
}

void dump_watchers (netsnmp_index * oah, void *)
{
...
void main (int argc, char **argv)
{
     ...
     * insert an object
     */
    rc = netsnmp_monitor_register (object, object_len, 0, EVENT_ROW_ADD, (void *) 0xdeadbeef, dummy_callback);
    printf ("insert an object: %d\n", rc);

    /*
     * insert same object, new priority
     */
    netsnmp_monitor_register (object, object_len, 10, EVENT_ROW_ADD, (void *) 0xdeadbeef, dummy_callback);
    printf ("insert same object, new priority: %d\n", rc);

    /*
     * insert same object, same priority, new data
     */
    netsnmp_monitor_register (object, object_len, 10, EVENT_ROW_ADD, (void *) 0xbeefdead, dummy_callback);
