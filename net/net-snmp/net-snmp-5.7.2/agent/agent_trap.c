...
netsnmp_feature_child_of (send_v3trap, netsnmp_unused) netsnmp_feature_child_of (send_trap_pdu, netsnmp_unused)
     struct trap_sink
     {
...
{
    if (snmp_callback_available (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_REGISTER_NOTIFICATIONS) == SNMPERR_SUCCESS)
    {
        /*
        ...
        snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_REGISTER_NOTIFICATIONS, (void *) &args);
    }
    else
    {
        /*
...
#if !defined(NETSNMP_DISABLE_SNMPV1) || !defined(NETSNMP_DISABLE_SNMPV2C)
static int create_trap_session2 (const char *sink, const char *sinkport, char *com, int version, int pdutype)
{
    netsnmp_transport *t;
...
int create_trap_session (char *sink, u_short sinkport, char *com, int version, int pdutype)
{
    ...
    if (sinkport != 0)
    {
        ...
        snmp_log (LOG_NOTICE,
                  "Using a separate port number is deprecated, please correct " "the sink specification instead");
    }
    return create_trap_session2 (sink, sinkport ? buf : NULL, com, version, pdutype);
}

#endif                            /* support for community based SNMP */

#ifndef NETSNMP_DISABLE_SNMPV1
static int create_v1_trap_session (char *sink, const char *sinkport, char *com)
{
    return create_trap_session2 (sink, sinkport, com, SNMP_VERSION_1, SNMP_MSG_TRAP);
}
#endif

#ifndef NETSNMP_DISABLE_SNMPV2C
static int create_v2_trap_session (const char *sink, const char *sinkport, char *com)
{
    return create_trap_session2 (sink, sinkport, com, SNMP_VERSION_2c, SNMP_MSG_TRAP2);
}

static int create_v2_inform_session (const char *sink, const char *sinkport, char *com)
{
    return create_trap_session2 (sink, sinkport, com, SNMP_VERSION_2c, SNMP_MSG_INFORM);
}
#endif
...
netsnmp_pdu *convert_v2pdu_to_v1 (netsnmp_pdu * template_v2pdu)
{
     ...
     */
    if (!vblist || snmp_oid_compare (vblist->name, vblist->name_length, sysuptime_oid, sysuptime_oid_len))
    {
        snmp_log (LOG_WARNING, "send_trap: no v2 sysUptime varbind to set from\n");
     ...
     */
    if (!vblist || snmp_oid_compare (vblist->name, vblist->name_length, snmptrap_oid, snmptrap_oid_len))
    {
    ...
    for (var = vblist->next_variable; var; var = var->next_variable)
    {
        ...
        {
            snmp_log (LOG_WARNING, "send_trap: v1 traps can't carry Counter64 varbinds\n");
            snmp_free_pdu (template_v1pdu);
            return NULL;
        }
    }
     ...
     */
    if (!snmp_oid_compare (vblist->val.objid, OID_LENGTH (trap_prefix), trap_prefix, OID_LENGTH (trap_prefix)))
    {
        /*
         ...
         */
        template_v1pdu->trap_type = vblist->val.objid[OID_LENGTH (trap_prefix)] - 1;
        template_v1pdu->specific_type = 0;

        var = find_varbind_in_list (vblist, snmptrapenterprise_oid, snmptrapenterprise_oid_len);
        if (var)
        {
            template_v1pdu->enterprise_length = var->val_len / sizeof (oid);
            template_v1pdu->enterprise = snmp_duplicate_objid (var->val.objid, template_v1pdu->enterprise_length);
        }
        else
        {
    ...
    else
    {
        ...
        template_v1pdu->enterprise = snmp_duplicate_objid (vblist->val.objid, len);
...
netsnmp_pdu *convert_v1pdu_to_v2 (netsnmp_pdu * template_v1pdu)
{
    ...
    {
        memcpy (enterprise, template_v1pdu->enterprise, template_v1pdu->enterprise_length * sizeof (oid));
    ...
    if (!snmp_varlist_add_variable (&var,
                                    ...
                                    ASN_OBJECT_ID, (u_char *) enterprise, enterprise_len * sizeof (oid)))
    {
        snmp_log (LOG_WARNING, "send_trap: failed to insert copied snmpTrapOID varbind\n");
    ...
    if (!snmp_varlist_add_variable (&var,
                                    ...
                                    ASN_TIMETICKS, (u_char *) & (template_v1pdu->time), sizeof (template_v1pdu->time)))
    {
        snmp_log (LOG_WARNING, "send_trap: failed to insert copied sysUptime varbind\n");
        snmp_free_pdu (template_v2pdu);
        return NULL;
    }
    var->next_variable = template_v2pdu->variables;
     ...
     */
    var = find_varbind_in_list (template_v2pdu->variables, agentaddr_oid, agentaddr_oid_len);
    if (!var && (template_v1pdu->agent_addr[0]
                 || template_v1pdu->agent_addr[1] || template_v1pdu->agent_addr[2] || template_v1pdu->agent_addr[3]))
    {
        if (!snmp_varlist_add_variable (&(template_v2pdu->variables),
                                        agentaddr_oid, agentaddr_oid_len,
                                        ASN_IPADDRESS,
                                        (u_char *) & (template_v1pdu->agent_addr), sizeof (template_v1pdu->agent_addr)))
            snmp_log (LOG_WARNING, "send_trap: failed to append snmpTrapAddr varbind\n");
    }
    var = find_varbind_in_list (template_v2pdu->variables, community_oid, community_oid_len);
    if (!var && template_v1pdu->community)
    {
        if (!snmp_varlist_add_variable (&(template_v2pdu->variables),
                                        community_oid, community_oid_len,
                                        ASN_OCTET_STR, template_v1pdu->community, template_v1pdu->community_len))
            snmp_log (LOG_WARNING, "send_trap: failed to append snmpTrapCommunity varbind\n");
    }
    var = find_varbind_in_list (template_v2pdu->variables, snmptrapenterprise_oid, snmptrapenterprise_oid_len);
    if (!var)
    {
        if (!snmp_varlist_add_variable (&(template_v2pdu->variables),
            ...
            snmp_log (LOG_WARNING, "send_trap: failed to append snmpEnterprise varbind\n");
    }
...
netsnmp_send_traps (int trap, int specific,
                    ...
                    netsnmp_variable_list * vars, const char *context, int flags)
{
    netsnmp_pdu *template_v1pdu;
    ...
    if (trap == -1)
    {
        ...
        if (!vblist)
        {
            snmp_log (LOG_WARNING, "send_trap: called with NULL v2 information\n");
            return -1;
        }
        template_v2pdu = snmp_pdu_create (SNMP_MSG_TRAP2);
        if (!template_v2pdu)
        {
            snmp_log (LOG_WARNING, "send_trap: failed to construct v2 template PDU\n");
         ...
         */
        if (!snmp_oid_compare (vblist->name, vblist->name_length, sysuptime_oid, sysuptime_oid_len))
        {
        ...
        else
        {
            ...
            if (!var)
            {
                snmp_log (LOG_WARNING, "send_trap: failed to insert sysUptime varbind\n");
                snmp_free_pdu (template_v2pdu);
                snmp_free_varbind (vblist);
                return -1;
            }
         ...
         */
        if (!trap_vb || snmp_oid_compare (trap_vb->name, trap_vb->name_length, snmptrap_oid, snmptrap_oid_len))
        {
            snmp_log (LOG_WARNING, "send_trap: no v2 trapOID varbind provided\n");
            snmp_free_pdu (template_v2pdu);
            return -1;
        }
        if (!snmp_oid_compare (vblist->val.objid, OID_LENGTH (trap_prefix), trap_prefix, OID_LENGTH (trap_prefix)))
        {
            ...
            if (!var &&
                !snmp_varlist_add_variable (&(template_v2pdu->variables),
                                            snmptrapenterprise_oid, snmptrapenterprise_oid_len,
                                            ASN_OBJECT_ID, enterprise, enterprise_length * sizeof (oid)))
            {
                snmp_log (LOG_WARNING, "send_trap: failed to add snmpEnterprise to v2 trap\n");
                snmp_free_pdu (template_v2pdu);
                return -1;
            }
        }
        ...
        if (!template_v1pdu)
        {
            snmp_log (LOG_WARNING, "send_trap: failed to convert v2->v1 template PDU\n");
        }
    ...
    else
    {
        ...
        if (!template_v1pdu)
        {
            snmp_log (LOG_WARNING, "send_trap: failed to construct v1 template PDU\n");
            snmp_free_varbind (vblist);
            return -1;
        }
        ...
        if (snmp_clone_mem ((void **) &template_v1pdu->enterprise, enterprise, enterprise_length * sizeof (oid)))
        {
            snmp_log (LOG_WARNING, "send_trap: failed to set v1 enterprise OID\n");
            snmp_free_varbind (vblist);
            snmp_free_pdu (template_v1pdu);
            return -1;
        }
        ...
        if (!template_v2pdu)
        {
            snmp_log (LOG_WARNING, "send_trap: failed to convert v1->v2 template PDU\n");
        }
    }
    ...
    if (template_v1pdu)
    {
        ...
        v1trapaddress = netsnmp_ds_get_string (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_TRAP_ADDR);
    ...
    if (template_v1pdu)
        snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_SEND_TRAP1, template_v1pdu);
    if (template_v2pdu)
        snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_SEND_TRAP2, template_v2pdu);
    snmp_free_pdu (template_v1pdu);
    snmp_free_pdu (template_v2pdu);
    return 0;
}
...
send_enterprise_trap_vars (int trap,
                           int specific, const oid * enterprise, int enterprise_length, netsnmp_variable_list * vars)
{
    netsnmp_send_traps (trap, specific, enterprise, enterprise_length, vars, NULL, 0);
    return;
}
...
int handle_inform_response (int op, netsnmp_session * session, int reqid, netsnmp_pdu * pdu, void *magic)
{
    ...
    switch (op)
    {
        ...
        case NETSNMP_CALLBACK_OP_TIMED_OUT:
            DEBUGMSGTL (("trap", "received a timeout sending an inform for reqid=%d\n", reqid));
            break;
...
void send_trap_to_sess (netsnmp_session * sess, netsnmp_pdu * template_pdu)
{
...
#ifndef NETSNMP_DISABLE_SNMPV1
    if (sess->version == SNMP_VERSION_1 && (template_pdu->command != SNMP_MSG_TRAP))
    ...
    {
        if ((sess->version == SNMP_VERSION_3) && (pdu->command == SNMP_MSG_TRAP2) && (sess->securityEngineIDLen == 0))
        {
...
void send_trap_vars (int trap, int specific, netsnmp_variable_list * vars)
{
    if (trap == SNMP_TRAP_ENTERPRISESPECIFIC)
        send_enterprise_trap_vars (trap, specific, objid_enterprisetrap, OID_LENGTH (objid_enterprisetrap), vars);
    else
        send_enterprise_trap_vars (trap, specific, trap_version_id, OID_LENGTH (trap_version_id), vars);
}
...
{
    if (trap == SNMP_TRAP_ENTERPRISESPECIFIC)
        netsnmp_send_traps (trap, specific, objid_enterprisetrap, OID_LENGTH (objid_enterprisetrap), vars, context, 0);
    else
        netsnmp_send_traps (trap, specific, trap_version_id, OID_LENGTH (trap_version_id), vars, context, 0);
...
{
    netsnmp_send_traps (-1, -1, trap_version_id, OID_LENGTH (trap_version_id), vars, context, 0);
}
...
void snmpd_parse_config_trapsess (const char *word, char *cptr)
{
    ...
    if ((rc = netsnmp_sess_config_and_open_transport (&session, transport)) != SNMPERR_SUCCESS)
    {
     ...
     */
    if (ss->version == SNMP_VERSION_3 && traptype != SNMP_MSG_INFORM && ss->securityEngineIDLen == 0)
    {
        u_char tmp[SPRINT_MAX_LEN];
