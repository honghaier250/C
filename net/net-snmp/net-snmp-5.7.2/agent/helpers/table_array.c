...
build_new_oid (netsnmp_handler_registration * reginfo,
               netsnmp_table_request_info * tblreq_info, netsnmp_index * row, netsnmp_request_info * current)
{
    ...
    memcpy (&coloid[reginfo->rootoid_len + 2], row->oids, row->len * sizeof (oid));

    snmp_set_var_objid (current->requestvb, coloid, reginfo->rootoid_len + 2 + row->len);
}
...
static void release_netsnmp_request_groups (void *vp)
{
    ...
    CONTAINER_FOR_EACH (c, (netsnmp_container_obj_func *) release_netsnmp_request_group, NULL);
    CONTAINER_FREE (c);
}
