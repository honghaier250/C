...
netsnmp_watcher_info *netsnmp_create_watcher_info6 (void *data, size_t size, u_char type,
                                                    int flags, size_t max_size, size_t * size_p)
{
    netsnmp_watcher_info *winfo = SNMP_MALLOC_TYPEDEF (netsnmp_watcher_info);

    if (winfo)
        netsnmp_init_watcher_info6 (winfo, data, size, type, flags, max_size, size_p);
    return winfo;
}
...
{
    return netsnmp_init_watcher_info6 (winfo, data, size, type, (flags ? flags : WATCHER_FIXED_SIZE), size,    /* Probably wrong for non-fixed
                                                                                                             * size data */
                                       NULL);
}
...
NETSNMP_STATIC_INLINE void set_data (netsnmp_watcher_info * winfo, void *data, size_t size)
{
...
NETSNMP_STATIC_INLINE netsnmp_watcher_cache *netsnmp_watcher_cache_create (const void *data, size_t size)
{
    netsnmp_watcher_cache *res = (netsnmp_watcher_cache *) malloc (sizeof (netsnmp_watcher_cache) + size - 1);
                                ...
                                netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    DEBUGMSGOID (("helper:watcher", requests->requestvb->name, requests->requestvb->name_length));
    ...
    switch (reqinfo->mode)
    {
             ...
             */
        case MODE_GET:
            snmp_set_var_typed_value (requests->requestvb, winfo->type, winfo->data, get_data_size (winfo));
            break;
        ...
        case MODE_SET_RESERVE1:
            if (requests->requestvb->type != winfo->type)
            {
                ...
                handler->flags |= MIB_HANDLER_AUTO_NEXT_OVERRIDE_ONCE;
                     ...
                     ((winfo->flags & WATCHER_FIXED_SIZE) && requests->requestvb->val_len != get_data_size (winfo)))
            {
                netsnmp_set_request_error (reqinfo, requests, SNMP_ERR_WRONGLENGTH);
                handler->flags |= MIB_HANDLER_AUTO_NEXT_OVERRIDE_ONCE;
            }
            else if ((winfo->flags & WATCHER_SIZE_STRLEN) &&
                     (memchr (requests->requestvb->val.string, '\0', requests->requestvb->val_len) != NULL))
            {
        ...
        case MODE_SET_RESERVE2:
             ...
             */
            old_data = netsnmp_watcher_cache_create (winfo->data, get_data_size (winfo));
            if (old_data == NULL)
            {
                ...
                netsnmp_request_add_list_data (requests, netsnmp_create_data_list ("watcher", old_data, &free_wrapper));
            break;
        ...
        case MODE_SET_ACTION:
             ...
             * update current 
             */
            set_data (winfo, (void *) requests->requestvb->val.string, requests->requestvb->val_len);
            break;
...
netsnmp_mib_handler *netsnmp_get_watched_timestamp_handler (void)
{
    ...
    ret = netsnmp_create_handler ("watcher-timestamp", netsnmp_watched_timestamp_handler);
    if (ret)
    {
        ret->flags |= MIB_HANDLER_AUTO_NEXT;
    }
                                   ...
                                   netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    DEBUGMSGTL (("helper:watcher:timestamp", "Got request:  %d\n", reqinfo->mode));
    DEBUGMSGTL (("helper:watcher:timestamp", "  oid:"));
    DEBUGMSGOID (("helper:watcher:timestamp", requests->requestvb->name, requests->requestvb->name_length));
...
netsnmp_mib_handler *netsnmp_get_watched_spinlock_handler (void)
{
    ...
    ret = netsnmp_create_handler ("watcher-spinlock", netsnmp_watched_spinlock_handler);
    if (ret)
    {
        ret->flags |= MIB_HANDLER_AUTO_NEXT;
    }
...
int netsnmp_register_watched_spinlock (netsnmp_handler_registration * reginfo, int *spinlock)
{
    ...
    winfo = netsnmp_create_watcher_info ((void *) spinlock, sizeof (int), ASN_INTEGER, WATCHER_FIXED_SIZE);
    netsnmp_inject_handler (reginfo, whandler);
                                  ...
                                  netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    DEBUGMSGTL (("helper:watcher:spinlock", "Got request:  %d\n", reqinfo->mode));
    DEBUGMSGTL (("helper:watcher:spinlock", "  oid:"));
    DEBUGMSGOID (("helper:watcher:spinlock", requests->requestvb->name, requests->requestvb->name_length));
...
register_scalar_watcher (const char *name,
                         ...
                         void *data, size_t size, u_char type, Netsnmp_Node_Handler * subhandler, int mode)
{
    netsnmp_handler_registration *reginfo = NULL;

    netsnmp_mib_handler *whandler = NULL;

    netsnmp_watcher_info *watchinfo = netsnmp_create_watcher_info (data, size, type, WATCHER_FIXED_SIZE);
    ...
    if (watchinfo && whandler)
    {
        ...
        netsnmp_owns_watcher_info (whandler);
        reginfo = netsnmp_create_handler_registration (name, subhandler, reg_oid, reg_oid_len, mode);
    }
...
netsnmp_register_ulong_scalar (const char *name,
                               const oid * reg_oid, size_t reg_oid_len, u_long * it, Netsnmp_Node_Handler * subhandler)
{
    return register_scalar_watcher (name, reg_oid, reg_oid_len,
                                    (void *) it, sizeof (u_long), ASN_UNSIGNED, subhandler, HANDLER_CAN_RWRITE);
}
...
{
    return register_scalar_watcher (name, reg_oid, reg_oid_len,
                                    (void *) it, sizeof (u_long), ASN_UNSIGNED, subhandler, HANDLER_CAN_RONLY);
}
...
netsnmp_register_long_scalar (const char *name,
                              const oid * reg_oid, size_t reg_oid_len, long *it, Netsnmp_Node_Handler * subhandler)
{
    return register_scalar_watcher (name, reg_oid, reg_oid_len,
                                    (void *) it, sizeof (long), ASN_INTEGER, subhandler, HANDLER_CAN_RWRITE);
}
...
{
    return register_scalar_watcher (name, reg_oid, reg_oid_len,
                                    (void *) it, sizeof (long), ASN_INTEGER, subhandler, HANDLER_CAN_RONLY);
}
...
netsnmp_register_int_scalar (const char *name,
                             const oid * reg_oid, size_t reg_oid_len, int *it, Netsnmp_Node_Handler * subhandler)
{
    return register_scalar_watcher (name, reg_oid, reg_oid_len,
                                    (void *) it, sizeof (int), ASN_INTEGER, subhandler, HANDLER_CAN_RWRITE);
}
...
{
    return register_scalar_watcher (name, reg_oid, reg_oid_len,
                                    (void *) it, sizeof (int), ASN_INTEGER, subhandler, HANDLER_CAN_RONLY);
}
...
{
    return register_scalar_watcher (name, reg_oid, reg_oid_len,
                                    (void *) it, sizeof (u_long), ASN_COUNTER, subhandler, HANDLER_CAN_RONLY);
}
