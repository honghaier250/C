...
netsnmp_mib_handler *netsnmp_get_scalar_group_handler (oid first, oid last)
{
    ...
    ret = netsnmp_create_handler ("scalar_group", netsnmp_scalar_group_helper_handler);
    if (ret)
    {
                                     ...
                                     netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    cmp = snmp_oid_compare (requests->requestvb->name, namelen, reginfo->rootoid, reginfo->rootoid_len);
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_SET_FREE:
            ...
            if (cmp != 0 || requests->requestvb->name_length <= reginfo->rootoid_len)
            {
                /*
        ...
        case MODE_GETNEXT:
            ...
            if (!requests->delegated &&
                (requests->requestvb->type == ASN_NULL ||
                 requests->requestvb->type == SNMP_NOSUCHOBJECT || requests->requestvb->type == SNMP_NOSUCHINSTANCE))
            {
                snmp_set_var_objid (requests->requestvb, reginfo->rootoid, reginfo->rootoid_len - 1);
