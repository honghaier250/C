...
netsnmp_feature_provide (instance) netsnmp_feature_child_of (instance, mib_helpers)
 ...
 */
     ...
     static netsnmp_num_file_instance *netsnmp_num_file_instance_ref (netsnmp_num_file_instance * nfi)
{
    nfi->refcnt++;
    return nfi;
}
                                           ...
                                           const char *contextName)
{
    ...
    if (subhandler)
    {
        myreg = netsnmp_create_handler_registration (name, subhandler, reg_oid, reg_oid_len, modes);
    ...
    {
        myreg = netsnmp_create_handler_registration (name, scalarh, reg_oid, reg_oid_len, modes);
        myreg->handler->myvoid = it;
        myreg->handler->data_clone = (void *(*)(void *)) netsnmp_num_file_instance_ref;
        myreg->handler->data_free = (void (*)(void *)) netsnmp_num_file_instance_deref;
    }
    if (contextName)
...
{
    return
        netsnmp_register_watched_instance2 (netsnmp_create_handler_registration
...
netsnmp_feature_child_of (read_only_uint_instance, instance)
     ...
     int netsnmp_register_read_only_uint_instance (const char *name,
                                                   const oid * reg_oid,
                                                   size_t reg_oid_len,
...
netsnmp_feature_child_of (uint_instance, instance)
     ...
     int netsnmp_register_uint_instance (const char *name,
                                         const oid * reg_oid, size_t reg_oid_len,
                                         unsigned int *it, Netsnmp_Node_Handler * subhandler)
{
    return
...
netsnmp_feature_child_of (register_read_only_int_instance, netsnmp_unused)
     ...
     int register_read_only_int_instance (const char *name,
                                          const oid * reg_oid, size_t reg_oid_len,
                                          int *it, Netsnmp_Node_Handler * subhandler)
{
    return netsnmp_register_read_only_int_instance (name, reg_oid, reg_oid_len, it, subhandler);
}
...
{
    netsnmp_handler_registration *myreg =
        netsnmp_create_handler_registration (name, subhandler, reg_oid, reg_oid_len, HANDLER_CAN_RONLY);
    if (myreg && contextName)
        myreg->contextName = strdup (contextName);
    return netsnmp_register_watched_instance2 (myreg, netsnmp_create_watcher_info ((void *) it, sizeof (u_long),
                                                                                   ASN_UNSIGNED, WATCHER_FIXED_SIZE));
}
...
{
    netsnmp_handler_registration *myreg =
        netsnmp_create_handler_registration (name, subhandler, reg_oid, reg_oid_len, HANDLER_CAN_RWRITE);
    if (myreg && contextName)
        myreg->contextName = strdup (contextName);
    return netsnmp_register_watched_instance2 (myreg, netsnmp_create_watcher_info ((void *) it, sizeof (u_long),
                                                                                   ASN_UNSIGNED, WATCHER_FIXED_SIZE));
...
netsnmp_feature_child_of (register_read_only_counter32_instance_context, instance)
     ...
     int netsnmp_register_read_only_counter32_instance_context (const char *name,
                                                                const oid * reg_oid,
                                                                size_t reg_oid_len,
                                                                u_long * it,
                                                           ...
                                                           Netsnmp_Node_Handler * subhandler, const char *contextName)
{
    ...
    if (myreg && contextName)
        myreg->contextName = strdup (contextName);
    return netsnmp_register_watched_instance2 (myreg, netsnmp_create_watcher_info ((void *) it, sizeof (long),
...
netsnmp_feature_child_of (register_long_instance_context, instance)
     ...
     int netsnmp_register_long_instance_context (const char *name,
                                                 const oid * reg_oid, size_t reg_oid_len,
                                                 long *it, Netsnmp_Node_Handler * subhandler, const char *contextName)
{
    netsnmp_handler_registration *myreg =
        netsnmp_create_handler_registration (name, subhandler, reg_oid, reg_oid_len, HANDLER_CAN_RWRITE);
    if (myreg && contextName)
...
netsnmp_feature_child_of (register_int_instance_context, instance)
     ...
     int netsnmp_register_int_instance_context (const char *name,
                                                const oid * reg_oid,
                                                size_t reg_oid_len,
                                                int *it, Netsnmp_Node_Handler * subhandler, const char *contextName)
{
    netsnmp_handler_registration *myreg =
        netsnmp_create_handler_registration (name, subhandler, reg_oid, reg_oid_len, HANDLER_CAN_RWRITE);
    if (myreg && contextName)
        myreg->contextName = strdup (contextName);
    return netsnmp_register_watched_instance2 (myreg, netsnmp_create_watcher_info ((void *) it, sizeof (int),
                                                                                   ASN_INTEGER, WATCHER_FIXED_SIZE));
}
...
netsnmp_feature_child_of (read_only_int_instance_context, instance)
     ...
     int register_read_only_int_instance_context (const char *name,
                                                  const oid * reg_oid, size_t reg_oid_len,
                                                  int *it, Netsnmp_Node_Handler * subhandler, const char *contextName)
{
    return netsnmp_register_read_only_int_instance_context (name, reg_oid, reg_oid_len, it, subhandler, contextName);
}
                                             ...
                                             Netsnmp_Node_Handler * subhandler, const char *contextName)
{
    ...
    myreg = get_reg (name, "file_num_handler", reg_oid, reg_oid_len, nfi,
                     mode, netsnmp_instance_num_file_handler, subhandler, contextName);
    if (NULL == myreg)
    {
                                   ...
                                   netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_GET:
            ...
            rc = fscanf (nfi->filep, (nfi->type == ASN_INTEGER) ? "%ld" : "%lu", &it);
            fclose (nfi->filep);
            nfi->filep = NULL;
            if (rc != 1)
            {
            ...
            snmp_set_var_typed_value (requests->requestvb, nfi->type, (u_char *) & it, sizeof (it));
            break;
        ...
        case MODE_SET_RESERVE2:
             ...
             */
            if (fscanf (nfi->filep, (nfi->type == ASN_INTEGER) ? "%ld" : "%lu", &it) != 1)
            {
            ...
            netsnmp_request_add_list_data (requests,
                                           netsnmp_create_data_list (INSTANCE_HANDLER_NAME, it_save, &free_wrapper));
            break;
        ...
        case MODE_SET_ACTION:
            ...
            rc = fprintf (nfi->filep, (nfi->type == ASN_INTEGER) ? "%ld" : "%lu", it);
            if (rc < 0)
            {
        ...
        case MODE_SET_UNDO:
            it = *((u_int *) netsnmp_request_get_list_data (requests, INSTANCE_HANDLER_NAME));
            rc = fprintf (nfi->filep, (nfi->type == ASN_INTEGER) ? "%ld" : "%lu", it);
            if (rc < 0)
...
#endif                            /* NETSNMP_NO_WRITE_SUPPORT */
        default:
            snmp_log (LOG_ERR, "netsnmp_instance_num_file_handler: illegal mode\n");
            netsnmp_set_request_error (reqinfo, requests, SNMP_ERR_GENERR);
            return SNMP_ERR_NOERROR;
    }
                                 ...
                                 netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    cmp = snmp_oid_compare (requests->requestvb->name,
                            requests->requestvb->name_length, reginfo->rootoid, reginfo->rootoid_len);
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_GETNEXT:
            if (cmp < 0 || (cmp == 0 && requests->inclusive))
            {
                ...
                snmp_set_var_objid (requests->requestvb, reginfo->rootoid, reginfo->rootoid_len);
            ...
            break;
        default:
            snmp_log (LOG_ERR, "netsnmp_instance_helper_handler: illegal mode\n");
            netsnmp_set_request_error (reqinfo, requests, SNMP_ERR_GENERR);
            return SNMP_ERR_NOERROR;
    }
    /*
