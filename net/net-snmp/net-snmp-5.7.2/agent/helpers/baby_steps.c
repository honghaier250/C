...
netsnmp_feature_provide (baby_steps) netsnmp_feature_child_of (baby_steps, mib_helpers)
...
static netsnmp_baby_steps_modes *netsnmp_baby_steps_modes_ref (netsnmp_baby_steps_modes * md)
{
    md->refcnt++;
    return md;
}
    ...
    for (i = 0; i < BABY_STEPS_PER_MODE_MAX; ++i)
    {
        ...
        if (BSTEP_USE_ORIGINAL != mode_map_ptr[i])
        {
            ...
            if ((MODE_SET_UNDO == save_mode) &&
                (MODE_BSTEP_UNDO_COMMIT == mode_map_ptr[i]) && !(BABY_STEP_COMMIT & bs_modes->completed))
            {
                DEBUGMSGTL (("baby_steps", "   skipping commit undo (no commit)\n"));
                continue;
            }
            else if ((MODE_SET_FREE == save_mode) &&
                     (MODE_BSTEP_UNDO_CLEANUP == mode_map_ptr[i]) && !(BABY_STEP_UNDO_SETUP & bs_modes->completed))
            {
                DEBUGMSGTL (("baby_steps", "   skipping undo cleanup (no undo setup)\n"));
                continue;
            }
#endif                            /* NETSNMP_NO_WRITE_SUPPORT */
            ...
            {
                DEBUGMSGTL (("baby_steps", "   skipping mode (not registered)\n"));
                continue;
            }
...
{
    netsnmp_register_handler_by_name ("baby_steps", netsnmp_baby_steps_handler_get (BABY_STEP_ALL));
}
...
netsnmp_mib_handler *netsnmp_baby_steps_access_multiplexer_get (netsnmp_baby_steps_access_methods * am)
{
    ...
    mh = netsnmp_create_handler ("baby_steps_mux", _baby_steps_access_multiplexer);
    if (!mh)
        return NULL;
                                ...
                                netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    netsnmp_assert ((handler != NULL) && (reginfo != NULL) && (reqinfo != NULL) && (requests != NULL));
    ...
    {
        rc = SNMP_ERR_GENERR;
        snmp_log (LOG_ERR, "baby steps multiplexer handler called for a mode " "with no handler\n");
