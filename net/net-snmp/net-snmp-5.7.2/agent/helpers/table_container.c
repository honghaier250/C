 ...
 */
     ...
     static void *_find_next_row (netsnmp_container * c, netsnmp_table_request_info * tblreq, void *key);
...
static container_table_data *netsnmp_container_table_data_clone (container_table_data * tad)
{
    ++tad->refcnt;
    return tad;
}
                                                          ...
                                                          netsnmp_container * container, char key_type)
{
    ...
    handler = netsnmp_create_handler ("table_container", _container_table_handler);
    if ((NULL == tad) || (NULL == handler))
    {
        ...
        snmp_log (LOG_ERR, "malloc failure in netsnmp_container_table_register\n");
        return NULL;
    }
...
int netsnmp_container_table_unregister (netsnmp_handler_registration * reginfo)
{
    ...
    tad = (container_table_data *) netsnmp_find_handler_data_by_name (reginfo, "table_container");
    if (tad)
    {
...
{
    return (netsnmp_container *) netsnmp_request_get_list_data (request, TABLE_CONTAINER_CONTAINER);
}
...
void netsnmp_container_table_row_insert (netsnmp_request_info * request, netsnmp_index * row)
{
    ...
    for (; req; req = req->next)
    {
        ...
        {
            netsnmp_request_add_list_data (req, netsnmp_create_data_list (TABLE_CONTAINER_ROW, row, NULL));
        }
    }
}
...
_set_key (container_table_data * tad, netsnmp_request_info * request,
          netsnmp_table_request_info * tblreq_info, void **key, netsnmp_index * index)
{
...
_data_lookup (netsnmp_handler_registration * reginfo,
              netsnmp_agent_request_info * agtreq_info, netsnmp_request_info * request, container_table_data * tad)
{
    ...
    netsnmp_assert ((NULL != tblreq_info) && (tblreq_info->colnum <= tad->tblreg_info->max_column));
    ...
    if ((agtreq_info->mode == MODE_GETNEXT) || (agtreq_info->mode == MODE_GETBULK))
    {
        ...
        if (row)
        {
            ...
            if (TABLE_CONTAINER_KEY_NETSNMP_INDEX == tad->key_type)
            {
                ...
                memcpy (tblreq_info->index_oid, row->oids, row->len * sizeof (oid));
            ...
            {
                netsnmp_table_build_oid_from_index (reginfo, request, tblreq_info);
            }
        }
        else
        {
    ...
    else
    {
        ...
        if (NULL == row)
        {
            ...
            {                    /* no create */
                ...
                netsnmp_set_request_error (agtreq_info, request, SNMP_NOSUCHINSTANCE);
#ifndef NETSNMP_NO_WRITE_SUPPORT
            }
#endif                            /* NETSNMP_NO_WRITE_SUPPORT */
        }
    ...
    {
        if (NULL != row)
            netsnmp_request_add_list_data (request, netsnmp_create_data_list (TABLE_CONTAINER_ROW, row, NULL));
        netsnmp_request_add_list_data (request, netsnmp_create_data_list (TABLE_CONTAINER_CONTAINER, tad->table, NULL));
    }
}
