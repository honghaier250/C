...
netsnmp_feature_child_of (table_iterator_row_count, table_iterator_all)
    ...
    netsnmp_feature_require (data_list_get_list_node) netsnmp_feature_require (oid_stash_add_data)
 ...
 * ================================== */
     ...
     */
     ...
     netsnmp_iterator_info *netsnmp_iterator_create_table (Netsnmp_First_Data_Point * firstDP,
...
NETSNMP_INLINE void netsnmp_insert_iterator_context (netsnmp_request_info * request, void *data)
{
    ...
    for (; req; req = req->next)
    {
        ...
        {
            netsnmp_request_add_list_data (req, netsnmp_create_data_list (TABLE_ITERATOR_NAME, data, NULL));
        }
    }
}
...
static ti_cache_info *netsnmp_iterator_remember (netsnmp_request_info * request,
                                                 ...
                                                 void *callback_loop_context, netsnmp_iterator_info * iinfo)
{
    ...
    if (!ti_info)
    {
        ...
        netsnmp_request_add_list_data (request,
                                       netsnmp_create_data_list (TI_REQUEST_CACHE, ti_info, netsnmp_free_ti_cache));
    }
                                       ...
                                       netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_GET_STASH:
            ...
            netsnmp_request_add_list_data (reqtmp,
                                           netsnmp_create_data_list (TABLE_HANDLER_NAME, (void *) table_info, NULL));
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                if (!ti_info)
                {
                    ...
                    netsnmp_request_add_list_data (request,
                                                   ...
                                                   (TI_REQUEST_CACHE, ti_info, netsnmp_free_ti_cache));
                }
        ...
        )
    {
        ...
        while (notdone)
        {
            ...
            index_search =
                (iinfo->get_first_data_point) (&callback_loop_context, &callback_data_context, index_search, iinfo);
            ...
            while (index_search)
            {
                ...
                for (request = requests; request; request = request->next)
                {
                    ...
                    switch (reqinfo->mode)
                    {
                        ...
                        case MODE_SET_RESERVE1:
                            ...
                            build_oid_noalloc (myname, MAX_OID_LEN, &myname_len, coloid, coloid_len, index_search);
                            if (snmp_oid_compare (myname, myname_len,
                                                  request->requestvb->name, request->requestvb->name_length) == 0)
                            {
                                /* 
                                 * keep this
                                 */
                            ...
                            else
                            {
                                ...
                                {
                                    (iinfo->free_data_context) (callback_data_context, iinfo);
                                }
                            }
                            break;
                        ...
                        case MODE_GET_STASH:
                            ...
                            if (reqtmp)
                                ldata = netsnmp_get_list_node (reqtmp->parent_data, TABLE_ITERATOR_NAME);
                            if (!ldata)
                            {
                                netsnmp_request_add_list_data (reqtmp,
                                                               netsnmp_create_data_list
                                                               (TABLE_ITERATOR_NAME, callback_data_context, NULL));
                            }
                            else
                            {
                                /* may have changed */
                            ...
                            for (i = table_reg_info->min_column; i <= (int) table_reg_info->max_column; i++)
                            {
                                ...
                                netsnmp_call_next_handler (handler, reginfo, reqinfo, reqtmp);
                                reqtmp->requestvb = NULL;
                                reqtmp->processed = 0;
                                if (vb->type != ASN_NULL)
                                {    /* XXX, not all */
                                    netsnmp_oid_stash_add_data (cinfo, myname, myname_len, vb);
                                }
                                else
                                {
                                    snmp_free_var (vb);
                        ...
                        case MODE_GETNEXT:
                            ...
                            if (netsnmp_check_getnext_reply
                                (request, coloid, coloid_len, index_search, &ti_info->results))
                            {
                                if (netsnmp_iterator_remember (request,
                        ...
                        default:
                            snmp_log (LOG_ERR, "table_iterator called with unsupported mode\n");
                            break;    /* XXX return */
                ...
                index_search =
                    (iinfo->get_next_data_point) (&callback_loop_context, &callback_data_context, index_search, iinfo);
            ...
            switch (reqinfo->mode)
            {
                ...
                case MODE_GETNEXT:
                    if (ti_info->best_match_len)
                        snmp_set_var_objid (request->requestvb, ti_info->best_match, ti_info->best_match_len);
                    else
                    {
                        coloid[reginfo->rootoid_len + 1] = netsnmp_table_next_column (table_info);
                ...
                case MODE_SET_RESERVE1:
                    ...
                    if (ti_info->data_context)
                        ...
                        netsnmp_request_add_list_data (request,
                                                       netsnmp_create_data_list
                                                       (TABLE_ITERATOR_NAME, ti_info->data_context, NULL));
                    break;
