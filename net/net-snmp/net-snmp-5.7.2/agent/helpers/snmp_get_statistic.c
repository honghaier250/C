...
netsnmp_feature_provide (helper_statistics) netsnmp_feature_child_of (helper_statistics, mib_helpers)
                                               ...
                                               netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    {
        const oid idx = requests->requestvb->name[reginfo->rootoid_len - 2] + (oid) (uintptr_t) handler->myvoid;
        ...
        snmp_set_var_typed_value (requests->requestvb, ASN_COUNTER, (const u_char *) &value, sizeof (value));
...
{
    netsnmp_mib_handler *ret = netsnmp_create_handler ("get_statistic",
...
int netsnmp_register_statistic_handler (netsnmp_handler_registration * reginfo, oid start, int begin, int end)
{
    netsnmp_inject_handler (reginfo, netsnmp_get_statistic_handler (begin - start));
