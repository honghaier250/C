...
netsnmp_feature_require (oid_stash_get_data) netsnmp_feature_require (oid_stash_add_data)
...
NETSNMP_INLINE void
netsnmp_table_dataset_remove_and_delete_row (netsnmp_table_data_set * table, netsnmp_table_row * row)
{
    netsnmp_table_data_set_storage *data;
    ...
    data = (netsnmp_table_data_set_storage *) netsnmp_table_data_remove_and_delete_row (table->table, row);
...
netsnmp_table_row *netsnmp_table_data_set_clone_row (netsnmp_table_row * row)
{
    ...
    if (data)
    {
             ...
             newrowdata = &((*newrowdata)->next), data = data->next)
        {
            ...
            memdup ((u_char **) newrowdata, (u_char *) data, sizeof (netsnmp_table_data_set_storage));
            if (!*newrowdata)
            {
            ...
            if (data->data.voidp)
            {
                memdup ((u_char **) & ((*newrowdata)->data.voidp), (u_char *) data->data.voidp, data->data_len);
...
netsnmp_table_row *netsnmp_table_data_set_create_row_from_defaults (netsnmp_table_data_set_storage * defrow)
{
    ...
    {
        netsnmp_set_row_column (row, defrow->column, defrow->type, defrow->data.voidp, defrow->data_len);
#ifndef NETSNMP_NO_WRITE_SUPPORT
        if (defrow->writable)
...
netsnmp_table_set_add_default_row (netsnmp_table_data_set * table_set,
                                   unsigned int column,
                                   int type, int writable, void *default_value, size_t default_value_len)
{
    ...
    if (default_value)
    {
        memdup ((u_char **) & (new_col->data.voidp), (u_char *) default_value, default_value_len);
    ...
    else
    {
        ...
        for (ptr = table_set->default_row, pptr = NULL; ptr; pptr = ptr, ptr = ptr->next)
        {
...
netsnmp_register_table_data_set (netsnmp_handler_registration * reginfo,
                                 netsnmp_table_data_set * data_set, netsnmp_table_registration_info * table_info)
{
    int ret;
    ...
    if (NULL == table_info->indexes && data_set->table->indexes_template)
    {
         ...
         * copy the indexes in 
         */
        table_info->indexes = snmp_clone_varbind (data_set->table->indexes_template);
    }
                                       ...
                                       * reginfo, netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    {
        netsnmp_table_data_set *datatable = (netsnmp_table_data_set *) handler->myvoid;

        const oid *const suffix = requests->requestvb->name + reginfo->rootoid_len + 2;

        const size_t suffix_len = requests->requestvb->name_length - (reginfo->rootoid_len + 2);
        ...
        if (MODE_IS_SET (reqinfo->mode))
        {
             ...
             */
            stashp = netsnmp_table_dataset_get_or_create_stash (reqinfo, datatable, table_info);
            if (NULL == stashp)
            {
            ...
            if (!newrowstash)
            {
                if (!row)
                {
                    if (datatable->allow_creation)
                    {
                         ...
                         */
                        newrowstash = netsnmp_table_data_set_create_newrowstash (datatable, table_info);
                ...
                netsnmp_oid_stash_add_data (stashp, suffix, suffix_len, newrowstash);
            }
            else
            {
                newrow = newrowstash->newrow;
        ...
        switch (reqinfo->mode)
        {
            ...
            case MODE_SET_RESERVE2:
                ...
                data->data.string = (u_char *)
                    netsnmp_strdup_and_null (request->requestvb->val.string, request->requestvb->val_len);
                if (!data->data.string)
                {
            ...
            case MODE_SET_ACTION:
                ...
                if (newrowstash->state != STATE_ACTION)
                {
                    ...
                    {
                        netsnmp_table_dataset_replace_row (datatable, row, newrow);
                    }
                }
                /*
                ...
                if (newrowstash->created)
                {
                    netsnmp_request_add_list_data (request, netsnmp_create_data_list (TABLE_DATA_NAME, newrow, NULL));
                }
                break;
            ...
            case MODE_SET_UNDO:
                ...
                if (newrowstash->state != STATE_UNDO)
                {
                    ...
                    {
                        netsnmp_table_dataset_replace_row (datatable, newrow, row);
                        netsnmp_table_dataset_delete_row (newrow);
                    }
                    newrow = NULL;
                }
            ...
            default:
                snmp_log (LOG_ERR, "table_dataset: unknown mode passed into the handler\n");
                return SNMP_ERR_GENERR;
        }
    }
...
{
    return (netsnmp_table_data_set *) netsnmp_request_get_list_data (request, TABLE_DATA_SET_NAME);
}
...
netsnmp_table_data_set_storage *netsnmp_extract_table_data_set_column (netsnmp_request_info * request,
                                                                       unsigned int column)
{
    netsnmp_table_data_set_storage *data = (netsnmp_table_data_set_storage *) netsnmp_extract_table_row_data (request);
...
void netsnmp_register_auto_data_table (netsnmp_table_data_set * table_set, char *registration_name)
{
    ...
    netsnmp_add_list_data (&auto_tables, netsnmp_create_data_list (registration_name, tables, free));    /* XXX */
}
...
{
    netsnmp_remove_list_node (&auto_tables, registration_name ? registration_name : table_set->table->name);
}
#endif                            /* NETSNMP_FEATURE_REMOVE_TABLE_DATASET_UNREGISTER_AUTO_DATA_TABLE */

#ifndef NETSNMP_DISABLE_MIB_LOADING
static void _table_set_add_indexes (netsnmp_table_data_set * table_set, struct tree *tp)
{
    ...
    for (index = tp->indexes; index; index = index->next)
    {
        ...
        {
            config_pwarn ("can't instatiate table since " "I don't know anything about one index");
        ...
        DEBUGMSGTL (("table_set_add_table", "adding default index of type %d\n", type));
...
void netsnmp_config_parse_table_set (const char *token, char *line)
{
    ...
    {
        config_pwarn ("can't instatiate table since I can't parse the table name");
        return;
    }
    if (NULL == (tp = get_tree (table_name, table_name_length, get_tree_head ())))
    {
        config_pwarn ("can't instatiate table since " "I can't find mib information about it");
        return;
    }

    if (NULL == (tp = tp->child_list) || NULL == tp->child_list)
    {
        config_pwarn ("can't instatiate table since it doesn't appear to be " "a proper table (no children)");
        return;
    }
    ...
    if (NULL != tp->augments)
    {
        ...
        {
            config_pwarn ("can't instatiate table since " "I can't find mib information about augment table");
    ...
    for (tp = tp->child_list; tp; tp = tp->next_peer)
    {
        ...
        switch (tp->access)
        {
            ...
            case MIB_ACCESS_READONLY:
                DEBUGMSGTL (("table_set_add_table", "adding column %ld of type %d\n", tp->subid, type));
    ...
    netsnmp_register_table_data_set (netsnmp_create_handler_registration
                                     (line, NULL, table_name, table_name_length, HANDLER_CAN_RWRITE), table_set, NULL);
...
void netsnmp_config_parse_add_row (const char *token, char *line)
{
    ...
    for (vb = tables->table_set->table->indexes_template; vb; vb = vb->next_variable)
    {
        if (!line)
        {
    ...
    for (dr = tables->table_set->default_row; dr; dr = dr->next)
    {
        if (!line)
        {
            config_pwarn ("missing a data value. " "All columns must be specified.");
...
#ifndef NETSNMP_NO_WRITE_SUPPORT
netsnmp_oid_stash_node **netsnmp_table_dataset_get_or_create_stash (netsnmp_agent_request_info * reqinfo,
                                                                    ...
                                                                    netsnmp_table_request_info * table_info)
{
    ...
    rc = snprintf (buf, sizeof (buf), "dataset_row_stash:%s:", datatable->table->name);
    ...
    len = sizeof (buf) - rc;
    rc = snprint_objid (&buf[rc], len, table_info->index_oid, table_info->index_oid_len);
    if (-1 == rc)
    {
    ...
    stashp = (netsnmp_oid_stash_node **) netsnmp_table_get_or_create_row_stash (reqinfo, (u_char *) buf);
    return stashp;
}

#ifndef NETSNMP_FEATURE_REMOVE_TABLE_DATASET_GET_NEWROW
netsnmp_table_row *netsnmp_table_dataset_get_newrow (netsnmp_request_info * request,
...
int netsnmp_set_row_column (netsnmp_table_row * row, unsigned int column, int type, const void *value, size_t value_len)
{
    ...
    if (data->type != type && data->type != SNMP_NOSUCHINSTANCE && type != SNMP_NOSUCHINSTANCE)
        return SNMPERR_GENERR;
