...
netsnmp_table_row *netsnmp_table_data_clone_row (netsnmp_table_row * row)
{
    ...
    if (row->index_oid)
    {
        newrow->index_oid = snmp_duplicate_objid (row->index_oid, row->index_oid_len);
        if (!newrow->index_oid)
        {
            free (newrow->indexes);
            free (newrow);
...
int netsnmp_table_data_add_row (netsnmp_table_data * table, netsnmp_table_row * row)
{
    ...
    if (!row->index_oid)
    {
        snmp_log (LOG_ERR, "illegal data attempted to be added to table %s (no index)\n", table->name);
        return SNMPERR_GENERR;
    }
    ...
    {
        rc = snmp_oid_compare (prevrow->index_oid, prevrow->index_oid_len, row->index_oid, row->index_oid_len);
        if (0 == rc)
            dup = 1;
    }
    else
    ...
    if (rc > 0)
    {
        for (nextrow = table->first_row, prevrow = NULL; nextrow != NULL; prevrow = nextrow, nextrow = nextrow->next)
        {
            ...
            rc = snmp_oid_compare (nextrow->index_oid, nextrow->index_oid_len, row->index_oid, row->index_oid_len);
            if (rc > 0)
                break;
            if (0 == rc)
            {
    ...
    if (dup)
    {
         ...
         */
        snmp_log (LOG_WARNING, "duplicate table data attempted to be entered. row exists\n");
        return SNMPERR_GENERR;
    }
...
void
netsnmp_table_data_replace_row (netsnmp_table_data * table, netsnmp_table_row * origrow, netsnmp_table_row * newrow)
{
...
int netsnmp_table_data_copy_row (netsnmp_table_row * old_row, netsnmp_table_row * new_row)
{
    ...
    if (old_row->index_oid)
        new_row->index_oid = snmp_duplicate_objid (old_row->index_oid, old_row->index_oid_len);
...
netsnmp_register_table_data (netsnmp_handler_registration * reginfo,
                             netsnmp_table_data * table, netsnmp_table_registration_info * table_info)
{
                                   ...
                                   netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    netsnmp_table_registration_info *table_reg_info = netsnmp_find_table_registration_info (reginfo);
    ...
    for (request = requests; request; request = request->next)
    {
        ...
        switch (reqinfo->mode)
        {
            ...
            case MODE_SET_RESERVE1:
                ...
                netsnmp_request_add_list_data (request, netsnmp_create_data_list (TABLE_DATA_TABLE, table, NULL));
        }
        ...
        switch (reqinfo->mode)
        {
            ...
            case MODE_GETBULK:    /* XXXWWW */
                ...
                result = snmp_oid_compare (request->requestvb->name,
                                           request->requestvb->name_length, reginfo->rootoid, reginfo->rootoid_len);
                regresult = snmp_oid_compare (request->requestvb->name,
                                              SNMP_MIN (request->requestvb->name_length,
                                                        reginfo->rootoid_len), reginfo->rootoid, reginfo->rootoid_len);
                if (regresult == 0 && request->requestvb->name_length < reginfo->rootoid_len)
                    regresult = -1;
                ...
                else if (regresult == 0 && request->requestvb->name_length == reginfo->rootoid_len + 1 &&
                ...
                else if (regresult == 0 && request->requestvb->name_length == reginfo->rootoid_len + 2 &&
                         /* entry node must be 1, but any column is ok */
                         request->requestvb->name[reginfo->rootoid_len] == 1)
                {
                    /*
                    ...
                    for (row = table->first_row; row; row = row->next)
                    {
                        ...
                        result =
                            snmp_oid_compare (row->index_oid,
                                              ...
                                              reginfo->rootoid_len,
                                              request->requestvb->name_length - 2 - reginfo->rootoid_len);
                        if (result == 0)
                        {
                            /*
                ...
                {
                    valid_request = 1;
                    netsnmp_request_add_list_data (request, netsnmp_create_data_list (TABLE_DATA_ROW, row, NULL));
                    /*
                    ...
                    netsnmp_table_data_build_result (reginfo, reqinfo, request,
                                                     row, table_info->colnum, ASN_NULL, NULL, 0);
                }
                else
            ...
            case MODE_GET:
                 ...
                 * find the row in question 
                 */
                if (request->requestvb->name_length < (reginfo->rootoid_len + 3))
                {                /* table.entry.column... */
                    /*
                     * request too short 
                     */
    ...
    if (valid_request && (reqinfo->mode == MODE_GETNEXT || reqinfo->mode == MODE_GETBULK))
    {
        /*
...
{
    return (netsnmp_table_data *) netsnmp_request_get_list_data (request, TABLE_DATA_TABLE);
}
...
{
    return (netsnmp_table_row *) netsnmp_request_get_list_data (request, TABLE_DATA_ROW);
}
...
void netsnmp_insert_table_row (netsnmp_request_info * request, netsnmp_table_row * row)
{
    ...
    build_oid_noalloc (this_oid, MAX_OID_LEN, &this_oid_len, base_oid, 2, this_index);
    ...
    for (; req; req = req->next)
    {
        ...
        build_oid_noalloc (that_oid, MAX_OID_LEN, &that_oid_len, base_oid, 2, that_index);

        /*
         * This request has the same index values,
         * so add the newly-created row information.
         */
        if (snmp_oid_compare (this_oid, this_oid_len, that_oid, that_oid_len) == 0)
        {
            netsnmp_request_add_list_data (req, netsnmp_create_data_list (TABLE_DATA_ROW, row, NULL));
        }
    }
}
...
netsnmp_table_data_build_result (netsnmp_handler_registration * reginfo,
                                 ...
                                 netsnmp_request_info * request,
                                 netsnmp_table_row * row,
                                 int column, u_char type, u_char * result_data, size_t result_data_len)
{
    ...
    if (reqinfo->mode == MODE_GETNEXT || reqinfo->mode == MODE_GETBULK)
    {
        ...
        snmp_set_var_objid (request->requestvb, build_space, reginfo->rootoid_len + 2 + row->index_oid_len);
    }
    snmp_set_var_typed_value (request->requestvb, type, result_data, result_data_len);
...
netsnmp_table_row *netsnmp_table_data_get_from_oid (netsnmp_table_data * table, oid * searchfor, size_t searchfor_len)
{
    ...
    {
        if (row->index_oid && snmp_oid_compare (searchfor, searchfor_len, row->index_oid, row->index_oid_len) == 0)
            return row;
    }
    return NULL;
}
...
netsnmp_table_row *netsnmp_table_data_row_get (netsnmp_table_data * table, netsnmp_table_row * row)
{
    if (!table || !row)
        return NULL;
    return netsnmp_table_data_get_from_oid (table, row->index_oid, row->index_oid_len);
}
...
netsnmp_table_row *netsnmp_table_data_row_next_byoid (netsnmp_table_data * table, oid * instance, size_t len)
{
    ...
    for (row = table->first_row; row; row = row->next)
    {
        if (snmp_oid_compare (row->index_oid, row->index_oid_len, instance, len) > 0)
            return row;
    }
    return NULL;
}
...
{
    netsnmp_table_row *row = netsnmp_table_data_row_get_byoid (table, instance, len);
...
{
    netsnmp_table_row *row = netsnmp_table_data_row_next_byoid (table, instance, len);
