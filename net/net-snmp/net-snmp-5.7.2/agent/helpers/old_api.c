                          ...
                          oid range_ubound, netsnmp_session * ss, const char *context, int timeout, int flags)
{
    ...
    for (i = 0; i < numvars; i++)
    {
        ...
        netsnmp_handler_registration *reginfo = SNMP_MALLOC_TYPEDEF (netsnmp_handler_registration);

        if (reginfo == NULL)
            return SNMP_ERR_GENERR;

        vp = netsnmp_duplicate_variable ((struct variable *) ((char *) var + varsize * i));
...
netsnmp_register_mib_table_row (const char *moduleName,
                                ...
                                size_t mibloclen,
                                int priority,
                                int var_subid, netsnmp_session * ss, const char *context, int timeout, int flags)
{
    ...
    for (i = 0; i < numvars; i++)
    {
        ...
        memcpy ((u_char *) (r->rootoid + (var_subid - vr->namelen)), vr->name, vr->namelen * sizeof (oid));
    ...
    if (rc == MIB_REGISTERED_OK)
    {
        ...
        rc = snmp_call_callbacks (SNMP_CALLBACK_APPLICATION, SNMPD_CALLBACK_REGISTER_OID, &reg_parms);
    }
    ...
    for (; requests; requests = requests->next)
    {
        ...
        switch (reqinfo->mode)
        {
            ...
            case MODE_SET_RESERVE1:
                ...
                if (vp && vp->findVar)
                {
                    memcpy (tmp_name, requests->requestvb->name, requests->requestvb->name_length * sizeof (oid));
                    tmp_len = requests->requestvb->name_length;
                    access = (*(vp->findVar)) (cvp, tmp_name, &tmp_len, exact, &len, &write_method);
...
#endif                            /* !NETSNMP_NO_WRITE_SUPPORT */
                        snmp_set_var_typed_value (requests->requestvb, cvp->type, access, len);
                }
                else
                {
                    /*
                    ...
                    if (access == NULL)
                    {
                        if (netsnmp_oid_equals (requests->requestvb->name,
                                                requests->requestvb->name_length, save, savelen) != 0)
                        {
                            DEBUGMSGTL (("old_api", "evil_client: %s\n", reginfo->handlerName));
                            memcpy (requests->requestvb->name, save, savelen * sizeof (oid));
                ...
                if (!cacheptr)
                    return netsnmp_set_request_error (reqinfo, requests, SNMP_ERR_RESOURCEUNAVAILABLE);
                cacheptr->data = access;
                cacheptr->write_method = write_method;
                write_method = NULL;
                netsnmp_request_add_list_data (requests,
                                               netsnmp_create_data_list (OLD_API_NAME, cacheptr, &free_wrapper));
                /*
            ...
            default:
                 ...
                 */
                cacheptr = (netsnmp_old_api_cache *) netsnmp_request_get_list_data (requests, OLD_API_NAME);
                ...
                if (cacheptr == NULL || cacheptr->write_method == NULL)
                {
                     ...
                     */
                    return netsnmp_set_request_error (reqinfo, requests, SNMP_ERR_NOTWRITABLE);
                }
