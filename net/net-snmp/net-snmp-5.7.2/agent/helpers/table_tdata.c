...
{
    netsnmp_tdata *tdata = (netsnmp_tdata *) netsnmp_request_get_list_data (request, TABLE_TDATA_TABLE);
...
{
    netsnmp_tdata_row *row = (netsnmp_tdata_row *) netsnmp_tdata_extract_row (request);
...
int netsnmp_tdata_compare_subtree_oid (netsnmp_tdata_row * row, oid * compareto, size_t compareto_len)
{
    ...
    return snmp_oidtree_compare (index->oids, index->len, compareto, compareto_len);
}
