...
netsnmp_feature_child_of (row_merge, row_merge_all) netsnmp_feature_child_of (row_merge_all, mib_helpers)
...
{
    netsnmp_inject_handler (reginfo, netsnmp_get_row_merge_handler (reginfo->rootoid_len + 1));
...
netsnmp_row_merge_status *netsnmp_row_merge_status_get (netsnmp_handler_registration * reginfo,
                                                        netsnmp_agent_request_info * reqinfo, int create_missing)
{
...
int netsnmp_row_merge_status_last (netsnmp_handler_registration * reginfo, netsnmp_agent_request_info * reqinfo)
{
    ...
    return (rm_status->count == 1) ? 1 : (rm_status->current == rm_status->rows);
}
                                  ...
                                  netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    for (count = 0, request = requests; request; request = request->next)
    {
        DEBUGIF ("helper:row_merge")
        {
            ...
            DEBUGMSGOID (("helper:row_merge", request->requestvb->name, request->requestvb->name_length));
            DEBUGMSG (("helper:row_merge", "\n"));
        }
        count++;
    }
    ...
    if ((0 != rm_status->count) && (rm_status->count != count))
    {
         ...
         */
        if ((reqinfo->mode != MODE_GET) && (reqinfo->mode != MODE_GETNEXT) && (reqinfo->mode != MODE_GETBULK))
        {
            netsnmp_assert ((NULL != rm_status->saved_requests) && (NULL != rm_status->saved_status));
        }
    ...
    if (0 == rm_status->count)
    {
         ...
         */
        rm_status->saved_requests = (netsnmp_request_info **) calloc (count + 1, sizeof (netsnmp_request_info *));
    ...
    for (i = 0; i < count; i++)
    {
        ...
        for (j = i + 1; j < count; j++)
        {
            ...
            DEBUGMSGOID (("helper:row_merge",
                          saved_requests[j]->requestvb->name, saved_requests[j]->requestvb->name_length));
            if (!snmp_oid_compare (saved_requests[i]->requestvb->name + SKIP_OID,
    ...
    for (i = 0; i < count; i++)
    {
         ...
         */
        rm_status->current++;
        ret = netsnmp_call_next_handler (handler, reginfo, reqinfo, saved_requests[i]);
        if (ret != SNMP_ERR_NOERROR)
        {
            snmp_log (LOG_WARNING, "bad rc (%d) from next handler in row_merge\n", ret);
...
{
    netsnmp_register_handler_by_name ("row_merge", netsnmp_get_row_merge_handler (-1));
}
