...
netsnmp_feature_child_of (registration_owns_table_info, table_all) netsnmp_feature_child_of (table_sparse, table_all)
...
{
    return (netsnmp_table_request_info *) netsnmp_request_get_list_data (request, TABLE_HANDLER_NAME);
}
...
{
    return (netsnmp_table_registration_info *) netsnmp_find_handler_data_by_name (reginfo, TABLE_HANDLER_NAME);
}
                      ...
                      netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    DEBUGIF ("helper:table:req")
    {
        DEBUGMSGTL (("helper:table:req",
                     ...
                     se_find_label_in_slist ("agent_mode", reqinfo->mode), reqinfo->mode, handler->handler_name));
        DEBUGMSGOID (("helper:table:req", reginfo->rootoid, reginfo->rootoid_len));
        ...
        for (request = requests; request; request = request->next)
        {
            ...
            if (reqinfo->mode == MODE_SET_RESERVE1)
            {
                DEBUGIF ("helper:table:set")
                {
                    ...
                    out_len = 0;
                    if (sprint_realloc_by_type (&buf, &buf_len, &out_len, 1, var, NULL, NULL, NULL))
                    {
            ...
            if (snmp_oid_compare (reginfo->rootoid, reginfo->rootoid_len, var->name, tmp_len) > 0)
            {
             ...
             * table.1 (table.ENTRY).  
             */
            else if ((var->name_length > reginfo->rootoid_len) && (var->name[reginfo->rootoid_len] != 1))
            {
            ...
            if (NULL == tbl_req_info)
            {
                ...
                netsnmp_request_add_list_data (request,
                                               ...
                                               (TABLE_HANDLER_NAME, (void *) tbl_req_info, table_data_free_func));
            }
            else
            {
            ...
            if (var->name_length > oid_column_pos)
            {
                ...
                {
                    DEBUGMSGTL (("helper:table:col", "    but it's less than min (%d)\n", tbl_info->min_column));
                    if (reqinfo->mode == MODE_GETNEXT)
                    {
                        /*
                ...
                {
                    tbl_req_info->colnum = netsnmp_closest_column (var->name[oid_column_pos], tbl_info->valid_columns);
            ...
            for (tmp_idx = 0, vb = tbl_req_info->indexes;
                 tmp_idx < tbl_info->number_indexes; ++tmp_idx, vb = vb->next_variable)
            {
                ...
                if (parse_one_oid_index (&tmp_name, &parsed_oid_len, vb, 1) != SNMPERR_SUCCESS)
                {
                    incomplete = 1;
            ...
            DEBUGIF ("helper:table:results")
            {
                ...
                DEBUGMSGTL (("helper:table:results",
                             "  column: %d, indexes: %d", tbl_req_info->colnum, tbl_req_info->number_indexes));
                for (vb = tbl_req_info->indexes, count = 0;
                     vb && count < tbl_req_info->number_indexes; count++, vb = vb->next_variable)
                {
                    out_len = 0;
                    if (sprint_realloc_by_type (&buf, &buf_len, &out_len, 1, vb, NULL, NULL, NULL))
                    {
                        DEBUGMSG (("helper:table:results",
                                   "   index: type=%d(%02x), value=%s", vb->type, vb->type, buf));
                    }
                    else
                    {
                        if (buf != NULL)
                        {
                            DEBUGMSG (("helper:table:results",
                                       "   index: type=%d(%02x), value=%s [TRUNCATED]", vb->type, vb->type, buf));
                        }
                        else
                        {
                            DEBUGMSG (("helper:table:results",
                                       "   index: type=%d(%02x), value=[NIL] [TRUNCATED]", vb->type, vb->type));
                        }
                    }
                }
                if (buf != NULL)
            ...
            if ((reqinfo->mode != MODE_GETNEXT) &&
                ((tbl_req_info->number_indexes != tbl_info->number_indexes) || (tmp_len != -1)))
            {

                DEBUGMSGTL (("helper:table", "invalid index(es) for table - skipping\n"));
                             ...
                             netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
     ...
     */
    if ((table_helper_handler != handler->access_method) && (NULL != handler->next))
    {
        /*
         * always warn if called without our own handler. If we
         * have our own handler, use custom bit 1 to only log once.
         */
        if ((sparse_table_helper_handler != handler->access_method) || !(handler->flags & MIB_HANDLER_CUSTOM1))
        {
    ...
    if (reqinfo->mode == MODE_GETNEXT)
    {
        for (request = requests; request; request = request->next)
        {
            if ((request->requestvb->type == ASN_NULL && request->processed) || request->delegated)
                continue;
            ...
            if (request->requestvb->type == SNMP_NOSUCHOBJECT || request->requestvb->type == SNMP_ENDOFMIBVIEW)
            {
                /*
                ...
                if (0 != table_info->colnum)
                {
                    ...
                    snmp_set_var_objid (request->requestvb, coloid, reginfo->rootoid_len + 2);
...
{
    return netsnmp_create_handler (SPARSE_TABLE_HANDLER_NAME, sparse_table_helper_handler);
}
...
int netsnmp_sparse_table_register (netsnmp_handler_registration * reginfo, netsnmp_table_registration_info * tabreq)
{
    ...
    handler1 = netsnmp_create_handler (SPARSE_TABLE_HANDLER_NAME, sparse_table_helper_handler);
    if (NULL == handler1)
        return SNMP_ERR_GENERR;
                         ...
                         netsnmp_request_info * reqinfo, netsnmp_table_request_info * table_info)
{
    ...
    if (build_oid (&var->name, &var->name_length,
                   tmpoid, reginfo->rootoid_len + 2, table_info->indexes) != SNMPERR_SUCCESS)
        return SNMPERR_GENERR;
                                    ...
                                    netsnmp_request_info * reqinfo, netsnmp_table_request_info * table_info)
{
    ...
    memcpy (&tmpoid[len], table_info->index_oid, table_info->index_oid_len * sizeof (oid));
...
int netsnmp_update_indexes_from_variable_list (netsnmp_table_request_info * tri)
{
    ...
    return build_oid_noalloc (tri->index_oid, sizeof (tri->index_oid), &tri->index_oid_len, NULL, 0, tri->indexes);
}
...
netsnmp_check_getnext_reply (netsnmp_request_info * request,
                             oid * prefix,
                             size_t prefix_len, netsnmp_variable_list * newvar, netsnmp_variable_list ** outvar)
{
    oid myname[MAX_OID_LEN];

    size_t myname_len;

    build_oid_noalloc (myname, MAX_OID_LEN, &myname_len, prefix, prefix_len, newvar);
    /*
    ...
    if ((!(*outvar) || snmp_oid_compare (myname + prefix_len,
                                         ...
                                         (*outvar)->name + prefix_len, (*outvar)->name_length - prefix_len) < 0))
    {
        /*
         * and greater than the requested oid 
         */
        if (snmp_oid_compare (myname, myname_len, request->requestvb->name, request->requestvb->name_length) > 0)
        {
            /*
            ...
            if (!*outvar)
                *outvar = snmp_clone_varbind (newvar);
                 ...
                 */
                snmp_set_var_typed_value (*outvar, newvar->type, newvar->val.string, newvar->val_len);
...
netsnmp_table_registration_info *netsnmp_table_registration_info_clone (netsnmp_table_registration_info * tri)
{
...
static void table_helper_cleanup (netsnmp_agent_request_info * reqinfo, netsnmp_request_info * request, int status)
{
...
netsnmp_oid_stash_node **netsnmp_table_get_or_create_row_stash (netsnmp_agent_request_info * reqinfo,
                                                                const u_char * storage_name)
{
    netsnmp_oid_stash_node **stashp = NULL;

    stashp = (netsnmp_oid_stash_node **) netsnmp_agent_get_list_data (reqinfo, (const char *) storage_name);
    ...
    if (!stashp)
    {
        ...
        netsnmp_agent_add_list_data (reqinfo,
                                     netsnmp_create_data_list ((const char *) storage_name,
                                                               stashp, _row_stash_data_list_free));
    }
    return stashp;
}
...
unsigned int netsnmp_table_next_column (netsnmp_table_request_info * table_info)
{
    ...
    if (table_info->reg_info->valid_columns)
        return netsnmp_closest_column (table_info->colnum + 1, table_info->reg_info->valid_columns);
