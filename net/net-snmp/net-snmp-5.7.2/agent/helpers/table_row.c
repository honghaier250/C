...
netsnmp_feature_child_of (table_row_all, mib_helpers) netsnmp_feature_child_of (table_row_extract, table_row_all)
...
netsnmp_table_row_register (netsnmp_handler_registration * reginfo,
                            netsnmp_table_registration_info * tabreg, void *row, netsnmp_variable_list * index)
{
    ...
    reg2 = netsnmp_create_handler_registration (tmp, _table_row_default_handler,
                                                reginfo->rootoid, reginfo->rootoid_len, reginfo->modes);
    ...
    build_oid_noalloc (&row_oid[row_oid_len], MAX_OID_LEN - row_oid_len, &len, NULL, 0, index);
    row_oid_len += len;
                    ...
                    netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    for (req = requests; req; req = req->next)
        netsnmp_request_add_list_data (req, netsnmp_create_data_list (TABLE_ROW_DATA, row, NULL));
                            ...
                            netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    for (req = requests; req; req = req->next)
    {
        ...
        if ((table_info->colnum >= tabreg->min_column) || (table_info->colnum <= tabreg->max_column))
        {
