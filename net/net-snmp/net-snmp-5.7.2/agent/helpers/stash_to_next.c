...
netsnmp_feature_provide (stash_to_next) netsnmp_feature_child_of (stash_to_next, mib_helpers)
#ifdef NETSNMP_FEATURE_REQUIRE_STASH_TO_NEXT
    netsnmp_feature_require (oid_stash) netsnmp_feature_require (oid_stash_add_data)
#endif
...
{
    netsnmp_mib_handler *handler = netsnmp_create_handler ("stash_to_next",
    ...
    if (MODE_GET_STASH == reqinfo->mode)
    {
        ...
        while (!finished)
        {
            ...
            if (!snmp_oid_compare (reginfo->rootoid, reginfo->rootoid_len,
                                   vb->name, namelen) && vb->type != ASN_NULL && vb->type != SNMP_ENDOFMIBVIEW)
            {
                /*
                 * This result is relevant so save it, and prepare
                 * the request varbind for the next query.
                 */
                netsnmp_oid_stash_add_data (cinfo, vb->name, vb->name_length, snmp_clone_varbind (vb));
                /*
