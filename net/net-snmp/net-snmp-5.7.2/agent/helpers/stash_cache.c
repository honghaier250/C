...
netsnmp_feature_provide (stash_cache) netsnmp_feature_child_of (stash_cache, mib_helpers)
#ifdef NETSNMP_FEATURE_REQUIRE_STASH_CACHE
    netsnmp_feature_require (oid_stash)
netsnmp_feature_require (oid_stash_iterate) netsnmp_feature_require (oid_stash_get_data)
#endif
...
netsnmp_mib_handler *netsnmp_get_timed_bare_stash_cache_handler (int timeout, oid * rootoid, size_t rootoid_len)
{
    ...
    cinfo = netsnmp_cache_create (timeout, _netsnmp_stash_cache_load, _netsnmp_stash_cache_free, rootoid, rootoid_len);
...
{
    netsnmp_mib_handler *handler = netsnmp_get_timed_bare_stash_cache_handler (timeout, rootoid, rootoid_len);
        ...
        case MODE_GET:
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                if (cdata && cdata->val.string && cdata->val_len)
                {
                    ...
                    snmp_set_var_typed_value (request->requestvb, cdata->type, cdata->val.string, cdata->val_len);
                }
            }
            break;
            ...
            for (request = requests; request; request = request->next)
            {
                ...
                if (cnode && cnode->thedata)
                {
                    ...
                    if (cdata->val.string && cdata->name && cdata->name_length)
                    {
                        ...
                        DEBUGMSG (("helper:stash_cache", "\n"));
                        snmp_set_var_typed_value (request->requestvb, cdata->type, cdata->val.string, cdata->val_len);
                        snmp_set_var_objid (request->requestvb, cdata->name, cdata->name_length);
                    }
                }
            }
            break;
...
int _netsnmp_stash_cache_load (netsnmp_cache * cache, void *magic)
{
    ...
    netsnmp_agent_add_list_data (reqinfo, netsnmp_create_data_list (STASH_CACHE_NAME, &cinfo->cache, NULL));
...
{
    netsnmp_register_handler_by_name ("stash_cache", netsnmp_get_stash_cache_handler ());
}
