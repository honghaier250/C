...
{
    netsnmp_mib_handler *handler = netsnmp_create_handler ("bulk_to_next",
    ...
    for (request = requests; request; request = request->next)
    {
        if (request->repeat > 0 &&
            ...
            (snmp_oid_compare (request->requestvb->name,
                               ...
                               request->range_end, request->range_end_len) < 0) && request->requestvb->next_variable)
        {
            request->repeat--;
    ...
    if (MODE_GETBULK == reqinfo->mode)
    {
        ...
        DEBUGIF ("bulk_to_next")
        {
            ...
            while (req)
            {
                ...
                DEBUGMSGOID (("bulk_to_next", req->requestvb->name, req->requestvb->name_length));
