...
netsnmp_feature_child_of (cache_find_by_oid, cache_handler) netsnmp_feature_child_of (cache_get_head, cache_handler)
...
netsnmp_cache *netsnmp_cache_find_by_oid (const oid * rootoid, int rootoid_len)
{
    ...
    {
        if (0 == netsnmp_oid_equals (cache->rootoid, cache->rootoid_len, rootoid, rootoid_len))
            return cache;
    }
...
netsnmp_cache *netsnmp_cache_create (int timeout, NetsnmpCacheLoad * load_hook,
                                     NetsnmpCacheFree * free_hook, const oid * rootoid, int rootoid_len)
{
    ...
    if (0 == cache->timeout)
        cache->timeout = netsnmp_ds_get_int (NETSNMP_DS_APPLICATION_ID, NETSNMP_DS_AGENT_CACHE_TIMEOUT);
...
int netsnmp_cache_free (netsnmp_cache * cache)
{
    ...
    for (pos = cache_head; pos; pos = pos->next)
    {
        if (pos == cache)
        {
            ...
            sprint_realloc_objid ((u_char **) & buf, &buf_len, &out_len, 1, pos->rootoid, pos->rootoid_len);
            snmp_log (LOG_WARNING, "not freeing cache with root OID %s (still in list)\n", buf);
            free (buf);
...
unsigned int netsnmp_cache_timer_start (netsnmp_cache * cache)
{
    ...
    {
        snmp_log (LOG_ERR, "cache_timer_start called but auto_reload not set.\n");
        return 0;
    }

    cache->timer_id = snmp_alarm_register (cache->timeout, SA_REPEAT, _timer_reload, cache);
    if (0 == cache->timer_id)
    {
...
/** returns a cache handler that can be injected into a given handler chain.  
 */
netsnmp_mib_handler *netsnmp_get_cache_handler (int timeout, NetsnmpCacheLoad * load_hook,
                                                NetsnmpCacheFree * free_hook, const oid * rootoid, int rootoid_len)
{
    netsnmp_mib_handler *ret = NULL;
    ...
    if (ret)
    {
        cache = netsnmp_cache_create (timeout, load_hook, free_hook, rootoid, rootoid_len);
        ret->myvoid = (void *) cache;
...
netsnmp_feature_child_of (netsnmp_register_cache_handler, netsnmp_unused)
     ...
     int netsnmp_register_cache_handler (netsnmp_handler_registration * reginfo,
                                         int timeout, NetsnmpCacheLoad * load_hook, NetsnmpCacheFree * free_hook)
{
    netsnmp_mib_handler *handler = NULL;

    handler = netsnmp_get_cache_handler (timeout, load_hook, free_hook, reginfo->rootoid, reginfo->rootoid_len);
...
void netsnmp_cache_reqinfo_insert (netsnmp_cache * cache, netsnmp_agent_request_info * reqinfo, const char *name)
{
    ...
    if (NULL == netsnmp_agent_get_list_data (reqinfo, cache_name))
    {
        ...
        netsnmp_agent_add_list_data (reqinfo, netsnmp_create_data_list (cache_name, cache, NULL));
    }
...
int netsnmp_cache_check_expired (netsnmp_cache * cache)
{
    ...
    if (!cache->valid || (NULL == cache->timestampM) || (-1 == cache->timeout))
        ...
        cache->expired = netsnmp_ready_monotonic (cache->timestampM, 1000 * cache->timeout);
                              ...
                              netsnmp_agent_request_info * reqinfo, netsnmp_request_info * requests)
{
    ...
    DEBUGMSGTL (("helper:cache_handler", "Got request (%d) for %s: ", reqinfo->mode, reginfo->handlerName));
    ...
    if (netsnmp_ds_get_boolean (NETSNMP_DS_APPLICATION_ID,
                                NETSNMP_DS_AGENT_NO_CACHING) || !cache || !cache->enabled || !cache->load_cache)
    {
    ...
    switch (reqinfo->mode)
    {
        ...
        case MODE_SET_COMMIT:
            if (cache->valid && !(cache->flags & NETSNMP_CACHE_DONT_INVALIDATE_ON_SET))
            {
...
static int _cache_load (netsnmp_cache * cache)
{
     ...
     */
    if ((!cache_outstanding_valid) && (!(cache->flags & NETSNMP_CACHE_DONT_FREE_EXPIRED)))
    {
        snmp_alarm_register (CACHE_RELEASE_FREQUENCY, 0, release_cached_resources, NULL);
