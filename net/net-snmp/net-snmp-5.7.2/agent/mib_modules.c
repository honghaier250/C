...
static int _shutdown_mib_modules (int majorID, int minorID, void *serve, void *client)
{
    if (!need_shutdown)
    {
...
#  include "mib_module_inits.h"
    ...
    if (once == 0)
    {
        ...
        once = 1;
        rc = snmp_register_callback (SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_SHUTDOWN, _shutdown_mib_modules, NULL);

        if (rc != SNMP_ERR_NOERROR)
            snmp_log (LOG_ERR, "error registering for SHUTDOWN callback " "for mib modules\n");
    }
}
