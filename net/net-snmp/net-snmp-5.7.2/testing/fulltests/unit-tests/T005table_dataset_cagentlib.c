...
th = netsnmp_create_handler_registration ("unit-test handler", NULL, Oid, OID_LENGTH (Oid), HANDLER_CAN_RWRITE);
OK (th, "table handler registration");

OK (netsnmp_register_table_data_set (th, tds, NULL) == SNMPERR_SUCCESS, "table data set registration");
...
for (i = 1; i <= 2; i++)
{
    for (j = 1; j <= 2; j++)
    {
        ...
        OK (netsnmp_set_row_column (row, COL5, ASN_OCTET_STR, "test",
                                    sizeof ("test") - 1) == SNMPERR_SUCCESS, "set OCTET_STR column");
    }
}
