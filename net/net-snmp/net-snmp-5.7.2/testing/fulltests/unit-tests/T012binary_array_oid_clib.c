...
for (i = 0; i < sizeof (b) / sizeof (b[0]); ++i)
{
    ...
    if (c->compare (b[i], i_last) < 0)
        OKF (ip && c->compare (b[i], ip) < 0,
             ("Successor of b[%d] = %" NETSNMP_PRIo "d is %" NETSNMP_PRIo "d", i, b[i]->oids[0], ip->oids[0]));
    else
        OKF (!ip, ("No successor found for b[%d] = %" NETSNMP_PRIo "d", i, b[i]->oids[0]));
}
...
for (i = 0; i < sizeof (a) / sizeof (a[0]); ++i)
{
    ...
    if (c->compare (a[i], i_last) < 0)
        OKF (ip && c->compare (ip, a[i]) > 0,
             ("Successor of a[%d] = %" NETSNMP_PRIo "d is %" NETSNMP_PRIo "d", i, a[i]->oids[0], ip->oids[0]));
    else
        OKF (!ip, ("No successor found for a[%d] = %" NETSNMP_PRIo "d", i, a[i]->oids[0]));
}
