...

{
    ...
    for (i = 0; i < sizeof (intval) / sizeof (intval[0]); ++i)
    {
        ...
        build_result = asn_build_int (encoded, &encoded_length, ASN_INTEGER, &intval[i], sizeof (intval[i]));
        OKF (build_result + encoded_length == encoded + sizeof (encoded), ("asn_build_int(%ld)", intval[i]));
        decoded_length = sizeof (encoded) - encoded_length;
        parse_result = asn_parse_int (encoded, &decoded_length, &decoded_type, &decoded_value, sizeof (decoded_value));
...

{
    ...
    for (i = 0; i < sizeof (intval) / sizeof (intval[0]); ++i)
    {
        ...
        encoded_length = sizeof (encoded);
        build_result = asn_build_unsigned_int (encoded, &encoded_length, ASN_UINTEGER, &intval[i], sizeof (intval[i]));
        OKF (build_result + encoded_length == encoded + sizeof (encoded), ("asn_build_unsigned_int(%lu)", intval[i]));
        decoded_length = sizeof (encoded) - encoded_length;
        parse_result = asn_parse_unsigned_int (encoded, &decoded_length,
                                               &decoded_type, &decoded_value, sizeof (decoded_value));
...

{
    ...
    for (i = 0; i < sizeof (intval) / sizeof (intval[0]); ++i)
    {
        ...
        build_result = asn_build_signed_int64 (encoded, &encoded_length,
                                               ASN_OPAQUE_I64, &intval[i], sizeof (intval[i]));
...

{
    ...
    for (i = 0; i < sizeof (intval) / sizeof (intval[0]); ++i)
    {
        ...
        build_result = asn_build_unsigned_int64 (encoded, &encoded_length,
                                                 ASN_COUNTER64, &intval[i], sizeof (intval[i]));
        OKF (build_result + encoded_length == encoded + sizeof (encoded),
