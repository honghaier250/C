...
OK (netsnmp_register_default_domain ("a", "") == 0, "register an empty token in domain 'a'");

OK (netsnmp_register_default_domain ("b", "alfa") == 0, "register a single token in domain 'b'");

OK (netsnmp_register_default_domain ("c", "alfa beta") == 0, "register two tokens in domain 'c'");
...

{
    ...
    OK (netsnmp_config (cfg) == SNMPERR_SUCCESS, "register user target 'alfa:user-alfa:b' for application 'b'");
}

{
    char cfg[] = "defTarget=b gamma user-gamma:b";

    OK (netsnmp_config (cfg) == SNMPERR_SUCCESS, "register user target 'gamma:user-gamma:b' for application 'b'");
}

{
    char cfg[] = "defTarget=c alfa user-alfa:c";

    OK (netsnmp_config (cfg) == SNMPERR_SUCCESS, "register user target 'alfa:user-alfa:c' for application 'c'");
}
