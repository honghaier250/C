...
void usage (void)
{
    printf (USAGE
            ...
            NL "    -r              Test sc_random()." NL "" NL, local_progname);
...
int test_dorandom (void)
{
    int rval = SNMPERR_SUCCESS, origrequest = (1024 * 2), origrequest_short = 19, shortcount = 7, i;
    ...
    {
        FAILED (SNMPERR_GENERR, "sc_random() returned different than requested.");
    }
...
int test_dokeyedhash (void)
{
    int rval = SNMPERR_SUCCESS, bigstring_len = strlen ((const char *) BIGSTRING), secret_len = strlen ((const char *) BIGSECRET), properlength, mlcount = 0;    /* MAC Length count.   */
  ...
  test_dokeyedhash_again:
    ...
    rval =
        sc_generate_keyed_hash (usmHMACMD5AuthProtocol,
                                ...
                                secret_len, BIGSTRING, bigstring_len, hashbuf, &hblen);
    ...
    rval =
        sc_check_keyed_hash (usmHMACMD5AuthProtocol,
                             USM_LENGTH_OID_TRANSFORM, BIGSECRET, secret_len, BIGSTRING, bigstring_len, hashbuf, hblen);
    ...
    if (hashbuf_len[mlcount] > properlength)
    {
        if (hblen != properlength)
        {
            FAILED (SNMPERR_GENERR, "Wrong SHA1 hash length returned.  (1)");
        }
...
int test_docrypt (void)
{
    int rval = SNMPERR_SUCCESS,
        ...
        secret_len = BYTESIZE (SNMP_TRANS_PRIVLEN_1DES), iv_len = BYTESIZE (SNMP_TRANS_PRIVLEN_1DES_IV);

    size_t buf_len = LOCAL_MAXBUF;

    size_t cryptbuf_len = LOCAL_MAXBUF;

    u_char buf[LOCAL_MAXBUF], cryptbuf[LOCAL_MAXBUF], secret[LOCAL_MAXBUF], iv[LOCAL_MAXBUF];
    ...
    rval = sc_encrypt (usmDESPrivProtocol, USM_LENGTH_OID_TRANSFORM,
                       secret, secret_len, iv, iv_len, BIGSTRING, bigstring_len, cryptbuf, &cryptbuf_len);
    FAILED (rval, "sc_encrypt() return code.");

    rval = sc_decrypt (usmDESPrivProtocol, USM_LENGTH_OID_TRANSFORM,
                       secret, secret_len, iv, iv_len, cryptbuf, cryptbuf_len, buf, &buf_len);
    ...
    FAILED ((memcmp (buf, BIGSTRING, bigstring_len) != 0), "Decrypted buffer is the same as the original plaintext.");
    return failcount;
}                                /* end test_docrypt() */
