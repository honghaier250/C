...
void usage (FILE * ofp)
{
    fprintf (ofp,
             ...
             NL "    -s <seconds>    Seconds to pause.  (Default: 0.)" NL NL, local_progname);
...
int test_hashindex (void)
{
    ...
    OUTPUT ("Visual spot check of hash index outputs.  " "(Success or failure not noted.)");
...
int test_etime (void)
{
    ...
    {
        FAILED (rval, "MAKENEW_ENGINE_RECORD returned success for " "missing record.");
    }
    ...
    fprintf (stdout, "# Sleeping for %d second%s... ", sleeptime, (sleeptime == 1) ? "" : "s");
    ...
    if ((etime < 20) || (eboot < 2))
    {
        FAILED (SNMPERR_GENERR, "get_enginetime() returned bad values.  (1)");
    }
    ...
    {
        FAILED (SNMPERR_GENERR, "get_enginetime() returned bad values.  (2)");
    }
