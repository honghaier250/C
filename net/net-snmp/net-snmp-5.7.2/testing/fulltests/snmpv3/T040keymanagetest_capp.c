...
void usage (FILE * ofp)
{
    fprintf (ofp,
             ...
             NL "    -u            generate_Ku()." NL "    -q            be quiet." NL "" NL, local_progname);
...
int test_genKu (void)
{
    int rval = SNMPERR_SUCCESS, failcount = 0, properlength = BYTESIZE (SNMP_TRANS_AUTHLEN_HMACMD5);
  ...
  test_genKu_again:
    ...
    rval = generate_Ku (hashtype, USM_LENGTH_OID_TRANSFORM,
                        (const u_char *) passphrase, strlen (passphrase), Ku, &kulen);
...
int test_genkul (void)
{
    int rval = SNMPERR_SUCCESS, failcount = 0, properlength, engineID_len, isdefault = FALSE;
    ...
    if (tolower (*(engineID + 1)) == 'x')
    {
        ...
        FAILED ((engineID_len < 0), "Could not resolve hex engineID.");
        engineID = (u_char *) s;
    ...
    if (!bequiet)
        fprintf (stdout, "# engineID%s (len=%d):  %s\n",
                 (isdefault) ? " (default)" : "", engineID_len, ((s != 0) ? ((u_char *) s) : engineID));
    SNMP_FREE (s);
  ...
  test_genkul_again_master:
    ...
    rval = generate_Ku (hashtype_Ku, USM_LENGTH_OID_TRANSFORM, (u_char *) passphrase, strlen (passphrase), Ku, &kulen);
    FAILED (rval, "generate_Ku().");

    binary_to_hex (Ku, kulen, &s);
    if (!bequiet)
        fprintf (stdout, "# Master Ku using \"%s\": \t%s\n", hashname_Ku, s);
    free_zero (s, kulen);
  ...
  test_genkul_again_local:
    ...
    rval = generate_kul (hashtype_kul, USM_LENGTH_OID_TRANSFORM, engineID, engineID_len, Ku, kulen, kul, &kul_len);

    if ((hashtype_Ku == usmHMACMD5AuthProtocol) && (hashtype_kul == usmHMACSHA1AuthProtocol))
    {
        FAILED ((rval == SNMPERR_SUCCESS),
                "generate_kul SHOULD fail when Ku length is " "less than hash transform length.");
    }
    else
    {
        FAILED (rval, "generate_kul().");

        FAILED (kul_len != properlength, "checking if kul length is right for the given hashtype.");

        binary_to_hex (kul, kul_len, &s);
        fprintf (stdout, "# kul (%s) (len=%" NETSNMP_PRIz "u):  %s\n",
                 ((hashtype_Ku == usmHMACMD5AuthProtocol) ? "MD5" : "SHA"), kul_len, s);
        free_zero (s, kul_len);
    }
...
int test_keychange (void)
{
    int rval = SNMPERR_SUCCESS,
        failcount = 0,
        properlength = BYTESIZE (SNMP_TRANS_AUTHLEN_HMACMD5),
        oldkey_len, newkey_len, isdefault_new = FALSE, isdefault_old = FALSE;
    ...
    u_char oldkey_buf[LOCAL_MAXBUF], newkey_buf[LOCAL_MAXBUF], temp_buf[LOCAL_MAXBUF], keychange_buf[LOCAL_MAXBUF];
    ...
    {
        newkey_len = hex_to_binary2 ((const u_char *) newkey + 2, newkey_len - 2, &s);
        FAILED (newkey_len < 0, "Could not resolve hex newkey.");
    ...
    {
        oldkey_len = hex_to_binary2 ((const u_char *) oldkey + 2, oldkey_len - 2, &s);
        FAILED (oldkey_len < 0, "Could not resolve hex oldkey.");
  ...
  test_keychange_again:
    ...
    rval = encode_keychange (hashtype, USM_LENGTH_OID_TRANSFORM,
                             oldkey_buf, properlength, newkey_buf, properlength, keychange_buf, &keychange_len);
    FAILED (rval, "encode_keychange().");

    FAILED (keychange_len != (properlength * 2),
            "KeyChange string (encoded) is not proper length " "for this hash transform.");
    ...
    rval = decode_keychange (hashtype, USM_LENGTH_OID_TRANSFORM,
                             oldkey_buf, properlength, keychange_buf, properlength * 2, temp_buf, &temp_len);
    FAILED (rval, "decode_keychange().");

    FAILED (temp_len != properlength, "decoded newkey is not proper length for " "this hash transform.");
    ...
    FAILED (memcmp (newkey_buf, temp_buf, temp_len), "newkey did not decode properly.");
    ...
    if (hashtype == usmHMACMD5AuthProtocol)
    {
        ...
        properlength = BYTESIZE (SNMP_TRANS_PRIVLEN_1DES) + BYTESIZE (SNMP_TRANS_PRIVLEN_1DES_IV);
