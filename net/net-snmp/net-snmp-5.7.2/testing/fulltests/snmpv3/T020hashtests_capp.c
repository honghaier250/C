...
int main (int argc, char **argv)
{
    ...
    sc_hash (usmHMACSHA1AuthProtocol, OID_LENGTH (usmHMACSHA1AuthProtocol), buf, sizeof (buf) - 1, MAC, &MAC_LEN);

    print_hash ("sha1 hash", MAC, MAC_LEN);

    memset (MAC, 0, MAC_LEN);
    sc_hash (usmHMACMD5AuthProtocol, OID_LENGTH (usmHMACMD5AuthProtocol), buf, sizeof (buf) - 1, MAC, &MAC_LEN);
    ...
    sc_generate_keyed_hash (usmHMACSHA1AuthProtocol,
                            ...
                            sha1key, sizeof (sha1key), buf, sizeof (buf) - 1, MAC, &MAC_LEN);
    ...
    result =
        sc_check_keyed_hash (usmHMACSHA1AuthProtocol,
                             OID_LENGTH (usmHMACSHA1AuthProtocol),
                             sha1key, sizeof (sha1key), buf, sizeof (buf) - 1, sha1proper, 12);
    if (0 == result)
    {
    ...
    sc_generate_keyed_hash (usmHMACMD5AuthProtocol,
                            ...
                            md5key, sizeof (md5key), buf, sizeof (buf) - 1, MAC, &MAC_LEN);
    ...
    result =
        sc_check_keyed_hash (usmHMACMD5AuthProtocol,
                             OID_LENGTH (usmHMACMD5AuthProtocol),
                             md5key, sizeof (md5key), buf, sizeof (buf) - 1, md5proper, 12);
    ...
    MAC_LEN = 20;
    memset (MAC, 0, MAC_LEN);
    generate_Ku (usmHMACSHA1AuthProtocol, OID_LENGTH (usmHMACSHA1AuthProtocol), buf, sizeof (buf) - 1, MAC, &MAC_LEN);
    print_hash ("sha1 Ku", MAC, MAC_LEN);

    MAC_LEN = 16;
    memset (MAC, 0, MAC_LEN);
    generate_Ku (usmHMACMD5AuthProtocol, OID_LENGTH (usmHMACMD5AuthProtocol), buf, sizeof (buf) - 1, MAC, &MAC_LEN);
