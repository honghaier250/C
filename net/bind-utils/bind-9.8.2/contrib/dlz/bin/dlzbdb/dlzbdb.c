...
isc_result_t bdb_opendb (DBTYPE db_type, DB ** db_out, const char *db_name, int flags);

void put_data (isc_boolean_t dns_data, char *input_key, char *input_data);
...
isc_result_t bulk_write (char type, DB * database, DBC * dbcursor, DBT * bdbkey, DBT * bdbdata);

void operation_add (void);

void operation_bulk (void);

void operation_listOrDelete (isc_boolean_t dlt);
...
isc_result_t bdb_opendb (DBTYPE db_type, DB ** db_out, const char *db_name, int flags)
{
    ...
    {
        fprintf (stderr, "BDB could not initialize %s database. BDB error: %s", db_name, db_strerror (result));
    ...
    {
        fprintf (stderr, "BDB could not set flags for %s database. BDB error: %s", db_name, db_strerror (result));
        return ISC_R_FAILURE;
    }
    ...
    /* open the database. */
    if ((result = (*db_out)->open (*db_out, NULL, db_file, db_name, db_type, createFlag, 0)) != 0)
    {
        fprintf (stderr, "BDB could not open %s database in %s. BDB error: %s", db_name, db_file, db_strerror (result));
        return ISC_R_FAILURE;
    }
...
void put_data (isc_boolean_t dns_data, char *input_key, char *input_data)
{
    ...
    if (bdbres != 0)
    {
        fprintf (stderr, "BDB could not insert data.  Error: %s", db_strerror (bdbres));
        quit (5);
    }
}
...
void insert_data (void)
{
        ...
        switch (token.type)
        {
            case isc_tokentype_string:
                if (data_type == 'u')
                {
                    ...
                    if (strlen (token.value.as_pointer) > 1 || (data_type != 'd' && data_type != 'D' &&
            ...
            case isc_tokentype_eof:
                ...
                if ((data_type != 'u' && isc_buffer_usedlength (&buf) > 0) || data_type == 'b')
                {
                    ...
                    {
                        put_data (isc_boolean_false, (char *) &data_arr, (char *) &data_arr2);
                    }
                    else if (data_type == 'b')
                    {
  ...
  data_cleanup:
    ...
    fprintf (stderr, "Unknown error processing tokens during \"add\" or "
             "\"bulk\" operation.\nStoped processing on line %lu.", isc_lex_getsourceline (lexer));
}
...
isc_result_t openBDB (void)
{
    ...
    if (bdbres != 0)
    {
        fprintf (stderr, "BDB environment could not be created. BDB error: %s", db_strerror (bdbres));
    ...
    if (create_allowed == isc_boolean_true)
    {
        ...
        bdbres = db.dbenv->open (db.dbenv, db_envdir, DB_INIT_CDB | DB_INIT_MPOOL | DB_CREATE, 0);
    }
    else
    {                            /* not allowed to create new files. */
        bdbres = db.dbenv->open (db.dbenv, db_envdir, DB_INIT_CDB | DB_INIT_MPOOL, 0);
    }
    if (bdbres != 0)
    {
...
void operation_add (void)
{
    ...
    checkInvalidOption (list_everything, isc_boolean_true, "e", "for add operation");
...
void operation_bulk (void)
{
    ...
    checkInvalidOption (list_everything, isc_boolean_true, "e", "for bulk load operation");
...
isc_result_t bulk_write (char type, DB * database, DBC * dbcursor, DBT * bdbkey, DBT * bdbdata)
{
    ...
    for (;;)
    {
        ...
        bdbres = dbcursor->c_get (dbcursor, bdbkey, bdbdata, DB_MULTIPLE_KEY | DB_NEXT);
...
void operation_listOrDelete (isc_boolean_t dlt)
{
    ...
    if (dlt == isc_boolean_true)
    {
        ...
        checkInvalidOption (list_everything, isc_boolean_true, "e", "for delete operation");
        checkInvalidOption (create_allowed, isc_boolean_true, "n", "for delete operation");
    }
    else if (key != NULL || zone != NULL || host != NULL)
    {
        ...
        checkInvalidOption (list_everything, isc_boolean_true, "e", "for list when k, z, or h are specified");
        checkInvalidOption (create_allowed, isc_boolean_true, "n", "for list operation");
    }
    else if (c_ip != NULL || c_zone != NULL)
    {
        checkInvalidOption (list_everything, isc_boolean_true, "e", "for list when c or i are specified");
        checkInvalidOption (create_allowed, isc_boolean_true, "n", "for list operation");
    }
    ...
    {
        if (bulk_write ('c', db.client, db.cursor, &bdbkey, &bdbdata) != ISC_R_SUCCESS)
            return;
        memset (&bdbkey, 0, sizeof (bdbkey));
        memset (&bdbdata, 0, sizeof (bdbdata));
    ...
    if (key != NULL)
    {
        ...
        else
        {
            ...
            {
                printf ("KEY | DATA\n");
                printf ("%lu | %.*s\n", *(u_long *) bdbkey.data, (int) bdbdata.size, (char *) bdbdata.data);
            }
    ...
    if (c_zone != NULL)
    {
        ...
        if (dlt == isc_boolean_true)
        {
            ...
            bdbres = db.client->cursor (db.client, NULL, &db.cursor, DB_WRITECURSOR);
        }
        else
        {
    ...
    {
        fprintf (stderr, "Unexpected error during list operation " "BDB error: %s", db_strerror (bdbres));
    }
...
int main (int argc, char **argv)
{
    ...
    switch (operation)
    {
        ...
        case bulk:
            ...
            break;
        default:
            fprintf (stderr, "\nNo operation was selected. " "Select an operation (l d a f)");
            quit (2);
            break;
    }
