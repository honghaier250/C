 ...
 */
     ...
     static isc_boolean_t is_safe (const char *input)
{
    unsigned int i;
...
static isc_result_t create_path_helper (char *out, const char *in, config_data_t * cd)
{
...
static isc_result_t
create_path (const char *zone, const char *host, const char *client, config_data_t * cd, char **path)
{
     ...
     * NON-NULL
     */
    REQUIRE ((host == NULL && client == NULL) || (host != NULL && client == NULL) || (host == NULL && client != NULL));
...
static isc_result_t
process_dir (isc_dir_t dir, void *passback, config_data_t * cd, dlist_t * dir_list, unsigned int basedirlen)
{
    ...
    while (isc_dir_read (&dir) == ISC_R_SUCCESS)
    {
        ...
        {
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Filesystem driver " "ttl must be a postive number");
        }

        /* pass data back to Bind */
        if (dir_list == NULL)
            result = dns_sdlz_putrr ((dns_sdlzlookup_t *) passback, type, ttl, data);
        else
            result = dns_sdlz_putnamedrr ((dns_sdlzallnodes_t *) passback, (char *) host, type, ttl, data);
...
static isc_result_t fs_allowzonexfr (void *driverarg, void *dbdata, const char *name, const char *client)
{
...
static isc_result_t fs_allnodes (const char *zone, void *driverarg, void *dbdata, dns_sdlzallnodes_t * allnodes)
{
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Unable to open %s directory to read entries.", basepath);
...
static isc_result_t fs_findzone (void *driverarg, void *dbdata, const char *name)
{
...
static isc_result_t
fs_lookup (const char *zone, const char *name, void *driverarg, void *dbdata, dns_sdlzlookup_t * lookup)
{
    isc_result_t result;
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (1), "Filesystem driver lookup() Checking for path: '%s'\n", path);
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Unable to open %s directory to read entries.", path);
        result = ISC_R_FAILURE;
        goto complete_lkup;
    }
...
static isc_result_t fs_create (const char *dlzname, unsigned int argc, char *argv[], void *driverarg, void **dbdata)
{
    config_data_t *cd;
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Filesystem driver requires " "6 command line args.");
        return (ISC_R_FAILURE);
    }
    ...
    if (argv[1][len - 1] != '\\' && argv[1][len - 1] != '/')
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR,
                       "Base dir parameter for filesystem driver " "should end with %s", "either '/' or '\\' ");
        return (ISC_R_FAILURE);
    }
  ...
  no_mem:
    ...
    /* write error message */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Filesystem driver unable to " "allocate memory for config data.");
...
isc_result_t dlz_fs_init (void)
{
     ...
     */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Registering DLZ filesystem driver.");

    result = dns_sdlzregister ("filesystem", &dlz_fs_methods, NULL,
                               DNS_SDLZFLAG_RELATIVEOWNER | DNS_SDLZFLAG_RELATIVERDATA, ns_g_mctx, &dlz_fs);
    if (result != ISC_R_SUCCESS)
    {
...
void dlz_fs_clear (void)
{
     ...
     */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Unregistering DLZ filesystem driver.");
