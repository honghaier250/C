...
static char *mysqldrv_escape_string (MYSQL * mysql, const char *instr)
{
...
mysql_get_resultset (const char *zone, const char *record,
                     const char *client, unsigned int query, void *dbdata, MYSQL_RES ** rs)
{
    isc_result_t result;
    ...
    switch (query)
    {
        ...
        default:
             ...
             * screwed up!
             */
            UNEXPECTED_ERROR (__FILE__, __LINE__, "Incorrect query flag passed to " "mysql_get_resultset");
...
static isc_result_t mysql_process_rs (dns_sdlzlookup_t * lookup, MYSQL_RES * rs)
{
            ...
            case 3:
                ...
                if (*endp != '\0' || ttl < 0)
                {
                    isc_log_write (dns_lctx,
                                   ...
                                   DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "mysql driver ttl must be " "a postive number");
                }
...
static isc_result_t mysql_findzone (void *driverarg, void *dbdata, const char *name)
{
    isc_result_t result;
...
static isc_result_t mysql_allowzonexfr (void *driverarg, void *dbdata, const char *name, const char *client)
{
    isc_result_t result;
 ...
 * query.
 */
static isc_result_t mysql_allnodes (const char *zone, void *driverarg, void *dbdata, dns_sdlzallnodes_t * allnodes)
{
    isc_result_t result;
    ...
    while (row != NULL)
    {
        ...
        {
            /* tell Bind about it. */
            result = dns_sdlz_putnamedrr (allnodes, safeGet (row[2]), safeGet (row[1]), ttl, safeGet (row[3]));
        }
        else
        {
            /*
...
static isc_result_t mysql_authority (const char *zone, void *driverarg, void *dbdata, dns_sdlzlookup_t * lookup)
{
    isc_result_t result;
...
static isc_result_t
mysql_lookup (const char *zone, const char *name, void *driverarg, void *dbdata, dns_sdlzlookup_t * lookup)
{
    isc_result_t result;
 ...
 */
static isc_result_t mysql_create (const char *dlzname, unsigned int argc, char *argv[], void *driverarg, void **dbdata)
{
    isc_result_t result;
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "mysql driver requires " "at least 4 command line args.");
        return (ISC_R_FAILURE);
    }
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "mysql driver requires a dbname parameter.");
        result = ISC_R_FAILURE;
        goto full_cleanup;
    }
    ...
    else
    {
        ...
        {
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Mysql driver port " "must be a positive number.");
    ...
    {
        case 4:
            result = build_sqldbinstance (ns_g_mctx, NULL, NULL, NULL, argv[2], argv[3], NULL, &dbi);
            break;
        case 5:
            result = build_sqldbinstance (ns_g_mctx, NULL, NULL, argv[4], argv[2], argv[3], NULL, &dbi);
            break;
        case 6:
            result = build_sqldbinstance (ns_g_mctx, argv[5], NULL, argv[4], argv[2], argv[3], NULL, &dbi);
            break;
        case 7:
            result = build_sqldbinstance (ns_g_mctx, argv[5], argv[6], argv[4], argv[2], argv[3], NULL, &dbi);
            break;
        case 8:
            result = build_sqldbinstance (ns_g_mctx, argv[5], argv[6], argv[4], argv[2], argv[3], argv[7], &dbi);
            break;
        default:
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "mysql driver could not create " "database instance object.");
        result = ISC_R_FAILURE;
        goto cleanup;
    }
    ...
    if (mysql_options ((MYSQL *) dbi->dbconn, MYSQL_OPT_RECONNECT, &auto_reconnect) != 0)
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_WARNING,
                       "mysql driver failed to set " "MYSQL_OPT_RECONNECT option, continuing");
    }
#endif

    for (j = 0; dbc == NULL && j < 4; j++)
        dbc = mysql_real_connect ((MYSQL *) dbi->dbconn, host, user, pass, dbname, port, socket, flags);
...
static void mysql_destroy (void *driverarg, void *dbdata)
{
    dbinstance_t *dbi;
 ...
 */
isc_result_t dlz_mysql_init (void)
{
    isc_result_t result;
     ...
     */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Registering DLZ mysql driver.");
    ...
    result = dns_sdlzregister ("mysql", &dlz_mysql_methods, NULL,
                               ...
                               DNS_SDLZFLAG_RELATIVERDATA | DNS_SDLZFLAG_THREADSAFE, ns_g_mctx, &dlz_mysql);
...
void dlz_mysql_clear (void)
{
     ...
     */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Unregistering DLZ mysql driver.");
