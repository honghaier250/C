...
static size_t postgres_makesafe (char *to, const char *from, size_t length)
{
 ...
 */
static void postgres_destroy_dblist (db_list_t * dblist)
{
...
static dbinstance_t *postgres_find_avail_conn (db_list_t * dblist)
{
    dbinstance_t *dbi = NULL;
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   ...
                   "Postgres driver unable to find available connection " "after searching %d times", count);
    return NULL;
}
...
static char *postgres_escape_string (const char *instr)
{
...
postgres_get_resultset (const char *zone, const char *record,
                        const char *client, unsigned int query, void *dbdata, PGresult ** rs)
{
    isc_result_t result;
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d Getting DBI", dlz_thread_num);
#endif
    ...
    /* temporary logging message */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d Got DBI - checking query", dlz_thread_num);
#endif
    ...
    switch (query)
    {
        ...
        default:
             ...
             * screwed up!
             */
            UNEXPECTED_ERROR (__FILE__, __LINE__, "Incorrect query flag passed to " "postgres_get_resultset");
    ...
    /* temporary logging message */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d checked query", dlz_thread_num);
#endif
    ...
    {                            /* no string passed, set the string pointer to NULL */
        dbi->zone = NULL;
    }

#if 0
    /* temporary logging message */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE, DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d did zone", dlz_thread_num);
#endif
    ...
    /* temporary logging message */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d did record", dlz_thread_num);
#endif
    ...
    /* temporary logging message */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d did client", dlz_thread_num);
#endif
    ...
    /* temporary logging message */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d built query", dlz_thread_num);
#endif
    ...
    /* temporary logging message */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d query is '%s'", dlz_thread_num, querystring);
#endif
    ...
    for (j = 0; j < 3; j++)
    {
        ...
        /* temporary logging message */
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d executing query for %d time", dlz_thread_num, j);
#endif
        ...
        *rs = PQexec ((PGconn *) dbi->dbconn, querystring);
        ...
        for (i = 0; *rs == NULL && i < 3; i++)
        {
            ...
            /* temporary logging message */
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d resetting connection", dlz_thread_num);
#endif
            result = ISC_R_FAILURE;
        ...
        if (PQresultStatus (*rs) == PGRES_TUPLES_OK)
        {
            ...
            /* temporary logging message */
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d rs ok", dlz_thread_num);
#endif
            break;
        }
        else
        {
            ...
            /* temporary logging message */
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d clearing rs", dlz_thread_num);
#endif
  ...
  cleanup:
    ...
    /* temporary logging message */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d cleaning up", dlz_thread_num);
#endif
    ...
    /* temporary logging message */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d unlocking mutex", dlz_thread_num);
#endif
    ...
    /* temporary logging message */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "%d returning", dlz_thread_num);
#endif
...
static isc_result_t postgres_process_rs (dns_sdlzlookup_t * lookup, PGresult * rs)
{
    isc_result_t result;
    ...
    for (i = 0; i < rows; i++)
    {
        switch (fields)
        {
            ...
            case 2:
                 ...
                 */
                result = dns_sdlz_putrr (lookup, PQgetvalue (rs, i, 0), 86400, PQgetvalue (rs, i, 1));
                break;
            case 3:
                ...
                if (*endp != '\0' || ttl < 0)
                {
                    isc_log_write (dns_lctx,
                                   ...
                                   "Postgres driver ttl must be " "a positive number");
                }
                result = dns_sdlz_putrr (lookup, PQgetvalue (rs, i, 1), ttl, PQgetvalue (rs, i, 2));
                break;
            default:
                /*
                ...
                if (*endp != '\0' || ttl < 0)
                {
                    isc_log_write (dns_lctx,
                                   DNS_LOGCATEGORY_DATABASE,
                                   DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Postgres driver ttl must be " "a postive number");
                }
                /* ok, now tell Bind about it. */
                result = dns_sdlz_putrr (lookup, PQgetvalue (rs, i, 1), ttl, tmpString);
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_ERROR,
                           "dns_sdlz_putrr returned error. " "Error code was: %s", isc_result_totext (result));
            return (ISC_R_FAILURE);
        }
    }
...
static isc_result_t postgres_findzone (void *driverarg, void *dbdata, const char *name)
{
    isc_result_t result;
...
static isc_result_t postgres_allowzonexfr (void *driverarg, void *dbdata, const char *name, const char *client)
{
    isc_result_t result;
 ...
 * query.
 */
static isc_result_t postgres_allnodes (const char *zone, void *driverarg, void *dbdata, dns_sdlzallnodes_t * allnodes)
{
    isc_result_t result;
    ...
    for (i = 0; i < rows; i++)
    {
        ...
        if (fields == 4)
        {
            ...
            result = dns_sdlz_putnamedrr (allnodes,
                                          PQgetvalue (rs, i, 2), PQgetvalue (rs, i, 1), ttl, PQgetvalue (rs, i, 3));
        }
        else
        {
            /*
            ...
            /* tell Bind about it. */
            result = dns_sdlz_putnamedrr (allnodes, PQgetvalue (rs, i, 2), PQgetvalue (rs, i, 1), ttl, tmpString);
            isc_mem_free (ns_g_mctx, tmpString);
        }
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_ERROR,
                           "dns_sdlz_putnamedrr returned error. " "Error code was: %s", isc_result_totext (result));
            return (ISC_R_FAILURE);
        }
    }
...
static isc_result_t postgres_authority (const char *zone, void *driverarg, void *dbdata, dns_sdlzlookup_t * lookup)
{
    isc_result_t result;
...
static isc_result_t
postgres_lookup (const char *zone, const char *name, void *driverarg, void *dbdata, dns_sdlzlookup_t * lookup)
{
    isc_result_t result;
 ...
 */
static isc_result_t
postgres_create (const char *dlzname, unsigned int argc, char *argv[], void *driverarg, void **dbdata)
{
    isc_result_t result;
...
/* seed random # generator */
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (1), "Postgres driver running multithreaded");
#else                            /* ISC_PLATFORM_USETHREADS */
    /* if debugging, let user know we are single threaded. */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (1), "Postgres driver running single threaded");
#endif                            /* ISC_PLATFORM_USETHREADS */
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Postgres driver requires at least " "4 command line args.");
        return (ISC_R_FAILURE);
    }
    ...
    for (i = 0; i < dbcount; i++)
    {
        ...
        {
            case 5:
                result = build_sqldbinstance (ns_g_mctx, NULL, NULL, NULL, argv[3], argv[4], NULL, &dbi);
                break;
            case 6:
                result = build_sqldbinstance (ns_g_mctx, NULL, NULL, argv[5], argv[3], argv[4], NULL, &dbi);
                break;
            case 7:
                result = build_sqldbinstance (ns_g_mctx, argv[6], NULL, argv[5], argv[3], argv[4], NULL, &dbi);
                break;
            case 8:
                result = build_sqldbinstance (ns_g_mctx, argv[6], argv[7], argv[5], argv[3], argv[4], NULL, &dbi);
                break;
            default:
        ...
        if (PQstatus ((PGconn *) dbi->dbconn) != CONNECTION_OK)
        {
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           ...
                           "database connection number %u " "after 4 attempts", i + 1);
            goto cleanup;
        }
 ...
 */
    static void postgres_destroy (void *driverarg, void *dbdata)
    {
 ...
 */
    isc_result_t dlz_postgres_init (void)
    {
        isc_result_t result;
         ...
         */
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Registering DLZ postgres driver.");
        ...
        result = dns_sdlzregister ("postgres", &dlz_postgres_methods, NULL,
                                   ...
                                   DNS_SDLZFLAG_RELATIVERDATA | DNS_SDLZFLAG_THREADSAFE, ns_g_mctx, &dlz_postgres);
 ...
 */
    void dlz_postgres_clear (void)
    {
         ...
         */
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Unregistering DLZ postgres driver.");
