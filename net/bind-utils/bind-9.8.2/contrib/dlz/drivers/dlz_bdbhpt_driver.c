...
static isc_result_t bdbhpt_findzone (void *driverarg, void *dbdata, const char *name);
...
static isc_result_t bdbhpt_parse_data (char *in, bdbhpt_parsed_data_t * pd)
{
    ...
    if (*endp != '\0' || pd->ttl < 0)
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "bdbhpt driver ttl must be a postive number");
        return ISC_R_FAILURE;
    }
...
static isc_result_t bdbhpt_allowzonexfr (void *driverarg, void *dbdata, const char *name, const char *client)
{
    isc_result_t result;
...
static void bdbhpt_cleanup (bdbhpt_instance_t * db)
{
...
static isc_result_t
bdbhpt_lookup (const char *zone, const char *name, void *driverarg, void *dbdata, dns_sdlzlookup_t * lookup)
{
    ...
    flags = DB_SET;
    while ((bdbhptres = data_cursor->c_get (data_cursor, &key, &data, flags)) == 0)
    {
...
static isc_result_t
bdbhpt_opendb (DB_ENV * db_env, DBTYPE db_type, DB ** db, const char *db_name, char *db_file, int flags)
{
    ...
    if ((result = db_create (db, db_env, 0)) != 0)
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       ...
                       "bdbhpt could not initialize %s database. " "bdbhpt error: %s", db_name, db_strerror (result));
        return ISC_R_FAILURE;
    }
    ...
    if ((result = (*db)->set_flags (*db, flags)) != 0)
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       ...
                       "bdbhpt error: %s", db_name, db_strerror (result));
        return ISC_R_FAILURE;
    }
    ...
    if ((result = (*db)->open (*db, NULL, db_file, db_name, db_type, DB_RDONLY | bdbhpt_threads, 0)) != 0)
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       ...
                       "bdbhpt error: %s", db_name, db_file, db_strerror (result));
        return ISC_R_FAILURE;
    }
...
static isc_result_t bdbhpt_create (const char *dlzname, unsigned int argc, char *argv[], void *driverarg, void **dbdata)
{
    isc_result_t result;
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "bdbhpt driver requires at least " "3 command line args.");
        return (ISC_R_FAILURE);
    }
    ...
    switch ((char) *argv[1])
    {
        ...
        case 't':
            ...
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (1), "bdbhpt driver using transactional mode.");
            break;
            /*
        ...
        case 'c':
            ...
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (1), "bdbhpt driver using concurrent mode.");
            break;
            /*
        ...
        case 'p':
            ...
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (1), "bdbhpt driver using private mode.");
            break;
        default:
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_ERROR,
                           "bdbhpt driver requires the operating mode "
                           "be set to P or C or T.  You specified '%s'", argv[1]);
            return (ISC_R_FAILURE);
    }
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Could not allocate memory for " "database instance object.");
        return (ISC_R_NOMEMORY);
    }
    ...
    if (bdbhptres != 0)
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR,
                       "bdbhpt environment could not be created. " "bdbhpt error: %s", db_strerror (bdbhptres));
        result = ISC_R_FAILURE;
        goto init_cleanup;
    }

    /* open bdbhpt environment */
    bdbhptres = db->dbenv->open (db->dbenv, argv[2], bdbFlags | bdbhpt_threads | DB_CREATE, 0);
    if (bdbhptres != 0)
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR,
                       "bdbhpt environment at '%s' could not be opened."
                       " bdbhpt error: %s", argv[2], db_strerror (bdbhptres));
        result = ISC_R_FAILURE;
        goto init_cleanup;
    }

    /* open dlz_data database. */
    result = bdbhpt_opendb (db->dbenv, DB_UNKNOWN, &db->data, dlz_data, argv[3], DB_DUP | DB_DUPSORT);
    if (result != ISC_R_SUCCESS)
        goto init_cleanup;

    /* open dlz_xfr database. */
    result = bdbhpt_opendb (db->dbenv, DB_UNKNOWN, &db->xfr, dlz_xfr, argv[3], DB_DUP | DB_DUPSORT);
    if (result != ISC_R_SUCCESS)
        goto init_cleanup;

    /* open dlz_zone database. */
    result = bdbhpt_opendb (db->dbenv, DB_UNKNOWN, &db->zone, dlz_zone, argv[3], 0);
    if (result != ISC_R_SUCCESS)
        goto init_cleanup;

    /* open dlz_client database. */
    result = bdbhpt_opendb (db->dbenv, DB_UNKNOWN, &db->client, dlz_client, argv[3], DB_DUP | DB_DUPSORT);
    if (result != ISC_R_SUCCESS)
        goto init_cleanup;
...
static void bdbhpt_destroy (void *driverarg, void *dbdata)
{
    UNUSED (driverarg);
 ...
 */
isc_result_t dlz_bdbhpt_init (void)
{
    isc_result_t result;
     ...
     */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Registering DLZ bdbhpt driver.");

    result = dns_sdlzregister ("bdbhpt", &dlz_bdbhpt_methods, NULL,
                               DNS_SDLZFLAG_RELATIVEOWNER |
                               DNS_SDLZFLAG_RELATIVERDATA | DNS_SDLZFLAG_THREADSAFE, ns_g_mctx, &dlz_bdbhpt);
    if (result != ISC_R_SUCCESS)
    {
...
void dlz_bdbhpt_clear (void)
{
     ...
     */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Unregistering DLZ bdbhpt driver.");
