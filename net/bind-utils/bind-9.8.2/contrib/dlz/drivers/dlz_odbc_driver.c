...
static SQLSMALLINT safeLen (void *a)
{
    if (a == NULL)
        return 0;
...
static void destroy_odbc_instance (odbc_instance_t * odbc_inst)
{
...
static isc_result_t odbc_connect (odbc_instance_t * dbi, odbc_db_t ** dbc)
{
    ...
    else
    {
        ...
        if (ndb == NULL)
        {
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Odbc driver unable to allocate memory");
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Odbc driver unable to connect");
...
static dbinstance_t *odbc_find_avail_conn (db_list_t * dblist)
{
    dbinstance_t *dbi = NULL;
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   ...
                   "Odbc driver unable to find available " "connection after searching %d times", count);
    return NULL;
}
...
static char *odbc_escape_string (const char *instr)
{
...
odbc_get_resultset (const char *zone, const char *record,
                    const char *client, unsigned int query, void *dbdata, dbinstance_t ** r_dbi)
{
    ...
    switch (query)
    {
        ...
        default:
             ...
             * screwed up!
             */
            UNEXPECTED_ERROR (__FILE__, __LINE__, "Incorrect query flag passed to " "odbc_get_resultset");
...
static isc_result_t odbc_getField (SQLHSTMT * stmnt, SQLSMALLINT field, char **data)
{
    ...
    if (sqlOK (SQLColAttribute (stmnt, field, SQL_DESC_DISPLAY_SIZE, NULL, 0, NULL, &size)) && size > 0)
    {
...
static isc_result_t odbc_getManyFields (SQLHSTMT * stmnt, SQLSMALLINT startField, SQLSMALLINT endField, char **retData)
{
...
static isc_result_t odbc_process_rs (dns_sdlzlookup_t * lookup, dbinstance_t * dbi)
{
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Odbc driver unable to process result set");
        result = ISC_R_FAILURE;
        ...
        switch (fields)
        {
            ...
            default:
                ...
                if ((result = odbc_getField (stmnt, 1, &ttl_s))
                    ...
                    == ISC_R_SUCCESS && (result = odbc_getManyFields (stmnt, 3, fields, &data)) == ISC_R_SUCCESS)
                {
                    ...
                    if (*endp != '\0' || ttl < 0)
                    {
                        isc_log_write (dns_lctx,
                                       ...
                                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Odbc driver ttl must " "be a postive number");
                        result = ISC_R_FAILURE;
                    }
                    else
                    {
...
static isc_result_t odbc_findzone (void *driverarg, void *dbdata, const char *name)
{
    ...
    if (result == ISC_R_SUCCESS && !sqlOK (SQLFetch (((odbc_db_t *) (dbi->dbconn))->stmnt)))
    {
...
static isc_result_t odbc_allowzonexfr (void *driverarg, void *dbdata, const char *name, const char *client)
{
    isc_result_t result;
...
static isc_result_t odbc_allnodes (const char *zone, void *driverarg, void *dbdata, dns_sdlzallnodes_t * allnodes)
{
    ...
    {                            /* gotta have at least 4 columns */
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Odbc driver too few fields returned by " "all nodes query");
        result = ISC_R_FAILURE;
        goto allnodes_cleanup;
    }
...
static isc_result_t odbc_authority (const char *zone, void *driverarg, void *dbdata, dns_sdlzlookup_t * lookup)
{
    isc_result_t result;
...
static isc_result_t
odbc_lookup (const char *zone, const char *name, void *driverarg, void *dbdata, dns_sdlzlookup_t * lookup)
{
    isc_result_t result;
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Odbc driver unable to return " "result set for lookup query");
        return (ISC_R_FAILURE);
    }
    /* lookup and authority result sets are processed in the same manner */
    /* odbc_process_rs does the job for both functions. */
 ...
 */
static isc_result_t odbc_create (const char *dlzname, unsigned int argc, char *argv[], void *driverarg, void **dbdata)
{
    isc_result_t result;
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (1), "Odbc driver running multithreaded");
#else                            /* ISC_PLATFORM_USETHREADS */
    /* if debugging, let user know we are single threaded. */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (1), "Odbc driver running single threaded");
#endif                            /* ISC_PLATFORM_USETHREADS */
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Odbc driver requires at least " "4 command line args.");
        return (ISC_R_FAILURE);
    }
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Odbc driver cannot accept more than " "7 command line args.");
        return (ISC_R_FAILURE);
    }
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "Odbc driver database connection count " "must be positive.");
        return (ISC_R_FAILURE);
    }
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "odbc driver requires a dns parameter.");
        result = ISC_R_FAILURE;
        goto cleanup;
    }
    ...
    if (odbc_inst->sql_env == NULL)
    {
        ...
        sqlRes = SQLAllocHandle (SQL_HANDLE_ENV, SQL_NULL_HANDLE, &(odbc_inst->sql_env));
        if (!sqlOK (sqlRes))
        {
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_INFO, "Odbc driver unable to allocate memory");
            result = ISC_R_NOMEMORY;
            goto cleanup;
        }
        /*set ODBC version = 3 */
        sqlRes = SQLSetEnvAttr (odbc_inst->sql_env, SQL_ATTR_ODBC_VERSION, (void *) SQL_OV_ODBC3, 0);
        if (!sqlOK (sqlRes))
        {
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_DLZ, ISC_LOG_INFO, "Unable to configure ODBC environment");
            result = ISC_R_NOMEMORY;
            goto cleanup;
        }
    }
    ...
    for (i = 0; i < dbcount; i++)
    {
        ...
        {
            case 5:
                result = build_sqldbinstance (ns_g_mctx, NULL, NULL, NULL, argv[3], argv[4], NULL, &db);
                break;
            case 6:
                result = build_sqldbinstance (ns_g_mctx, NULL, NULL, argv[5], argv[3], argv[4], NULL, &db);
                break;
            case 7:
                result = build_sqldbinstance (ns_g_mctx, argv[6], NULL, argv[5], argv[3], argv[4], NULL, &db);
                break;
            case 8:
                result = build_sqldbinstance (ns_g_mctx, argv[6], argv[7], argv[5], argv[3], argv[4], NULL, &db);
                break;
            default:
 ...
 */
    ...
    static void odbc_destroy (void *driverarg, void *dbdata)
    {
        UNUSED (driverarg);
 ...
 */
    isc_result_t dlz_odbc_init (void)
    {
        isc_result_t result;
         ...
         */
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Registering DLZ odbc driver.");
        ...
        result = dns_sdlzregister ("odbc", &dlz_odbc_methods, NULL,
                                   ...
                                   DNS_SDLZFLAG_RELATIVERDATA | DNS_SDLZFLAG_THREADSAFE, ns_g_mctx, &dlz_odbc);
