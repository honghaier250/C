...
static isc_result_t stub_dlz_allnodes (const char *zone, void *driverarg, void *dbdata, dns_sdlzallnodes_t * allnodes)
{
    ...
    result = dns_sdlz_putnamedrr (allnodes, cd->myname, "soa", 86400,
                                  "web root.localhost. " "0 28800 7200 604800 86400");
...
static isc_result_t stub_dlz_allowzonexfr (void *driverarg, void *dbdata, const char *name, const char *client)
{
    UNUSED (driverarg);
    UNUSED (dbdata);
    UNUSED (name);
...
static isc_result_t stub_dlz_authority (const char *zone, void *driverarg, void *dbdata, dns_sdlzlookup_t * lookup)
{
    isc_result_t result;
...
static isc_result_t stub_dlz_findzonedb (void *driverarg, void *dbdata, const char *name)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "dlz_stub findzone looking for '%s'", name);
...
static isc_result_t
stub_dlz_lookup (const char *zone, const char *name, void *driverarg, void *dbdata, dns_sdlzlookup_t * lookup)
{
    isc_result_t result;
...
static isc_result_t
stub_dlz_create (const char *dlzname, unsigned int argc, char *argv[], void *driverarg, void **dbdata)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   ...
                   "Zone: %s, Name: %s IP: %s", dlzname, argv[1], argv[2], argv[3]);
...
static void stub_dlz_destroy (void *driverarg, void *dbdata)
{
    config_data_t *cd;
     ...
     */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Unloading DLZ_stub driver.");
 ...
 */
isc_result_t dlz_stub_init (void)
{
    isc_result_t result;
     ...
     */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Registering DLZ_stub driver.");

    result = dns_sdlzregister ("dlz_stub", &dlz_stub_methods, NULL,
                               DNS_SDLZFLAG_RELATIVEOWNER | DNS_SDLZFLAG_RELATIVERDATA, ns_g_mctx, &dlz_stub);
    if (result != ISC_R_SUCCESS)
    {
...
void dlz_stub_clear (void)
{
     ...
     */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Unregistering DLZ_stub driver.");
