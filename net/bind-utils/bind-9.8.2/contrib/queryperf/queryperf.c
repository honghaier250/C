...
enum directives_enum
{ V_SERVER, V_PORT, V_MAXQUERIES, V_MAXWAIT };
 ...
 */
static unsigned short get_uint16 (unsigned char *buf)
{
    unsigned short ret;
...
void show_usage (void)
{
    fprintf (stderr,
             ...
             DEF_MAX_QUERIES_OUTSTANDING, DEF_QUERY_TIMEOUT, DEF_BUFFER_SIZE, DEF_RTTARRAY_SIZE, DEF_RTTARRAY_UNIT);
}
 ...
 */
int set_datafile (char *new_file)
{
    char *dfname_tmp;
    ...
    {
        fprintf (stderr, "Error allocating memory for datafile name: " "%s\n", new_file);
        return (-1);
    }
 ...
 *   Return a non-negative integer otherwise
 */
int set_server_port (char *new_port)
{
    ...
    {
        if (server_port != NULL && new_port != NULL && strcmp (server_port, new_port) == 0)
            return (0);
        ...
        {
            fprintf (stderr, "Error allocating memory for server port: " "%s\n", new_port);
            return (-1);
        }
...
int set_server_sa (void)
{
    ...
    if ((error = getaddrinfo (server_to_query, server_port, &hints, &res)) != 0)
    {
 ...
 */
int set_max_queries (unsigned int new_max)
{
    ...
    if (new_max < 0)
    {
        fprintf (stderr, "Unable to change max outstanding queries: " "must be positive and non-zero: %u\n", new_max);
        return (-1);
    }
...
int parse_args (int argc, char **argv)
{
    ...
    while ((c = getopt (argc, argv, "f:q:t:i:nd:s:p:1l:b:eDcvr:T::u:H:h")) != -1)
    {
        switch (c)
        {
            case 'f':
            ...
            case 'q':
                ...
                {
                    fprintf (stderr, "Option requires a positive " "integer value: -%c %s\n", c, optarg);
                    return (-1);
                }
                break;
            ...
            case 'd':
                ...
                {
                    fprintf (stderr, "Error setting datafile " "name: %s\n", optarg);
                    return (-1);
                }
                break;
            ...
            case 'p':
                if (is_uint (optarg, &uint_arg_val) == TRUE && uint_arg_val < MAX_PORT)
                {
            ...
            case 'l':
                ...
                {
                    fprintf (stderr, "Option requires a positive " "integer: -%c %s\n", c, optarg);
                    return (-1);
                }
                break;
...
int open_socket (void)
{
    ...
    {
        fprintf (stderr, "Error: getaddrinfo for bind socket failed: %s\n", gai_strerror (ret));
        return (-1);
    }

    if ((sock = socket (res->ai_family, SOCK_DGRAM, res->ai_protocol)) == -1)
    {
    ...
    if (res->ai_family == AF_INET6)
    {
        ...
        {
            fprintf (stderr, "Warning: setsockopt(IPV6_V6ONLY) failed\n");
        }
    }
#endif
...
int close_socket (void)
{
    if (socket4 != -1)
    {
        if (close (socket4) != 0)
        {
            fprintf (stderr, "Error: unable to close IPv4 socket\n");
            return (-1);
        }
    }
    ...
    if (socket6 != -1)
    {
        if (close (socket6) != 0)
        {
            fprintf (stderr, "Error: unable to close IPv6 socket\n");
            return (-1);
        }
    }
...
int reset_rttarray (int size)
{
    ...
    if (size > 0)
    {
        ...
        rttarray_interval = malloc (size * sizeof (rttarray_interval[0]));
        if (rttarray_interval == NULL)
        {
            fprintf (stderr, "Error: allocating memory for RTT array\n");
            return (-1);
        ...
        memset (rttarray_interval, 0, size * sizeof (rttarray_interval[0]));
    }
...
int setup (int argc, char **argv)
{
    ...
    {
        fprintf (stderr, "%s: Unable to set default max outstanding " "queries\n", argv[0]);
        return (-1);
    }
...
void set_timenow (struct timeval *tv)
{
    ...
    {
        fprintf (stderr, "Error in gettimeofday(). Using inaccurate " "time() instead\n");
        tv->tv_sec = time (NULL);
        tv->tv_usec = 0;
    }
}
 ...
 */
void addtv (struct timeval *tv1, struct timeval *tv2, struct timeval *tv_result)
{
 ...
 */
double difftv (struct timeval tv1, struct timeval tv2)
{
...
int timelimit_reached (void)
{
    ...
    if (setup_phase == TRUE)
    {
        if (difftv (time_now, time_of_program_start) < (double) (run_timelimit + HARD_TIMEOUT_EXTRA))
            return (FALSE);
        else
            return (TRUE);
    }
    ...
    {
        if (difftv (time_now, time_of_first_query) < (double) run_timelimit)
            return (FALSE);
        else
            return (TRUE);
    }
 ...
 */
int keep_sending (int *reached_end_input)
{
    ...
    if ((*reached_end_input == FALSE) && (timelimit_reached () == FALSE))
        return (TRUE);
    else if ((*reached_end_input == TRUE) && (run_only_once == FALSE) && (timelimit_reached () == FALSE))
    {
        rewind (datafile_ptr);
 ...
 */
unsigned int queries_outstanding (void)
{
 ...
 */
int next_input_line (char *line, int n)
{
    char *result;
    ...
    while ((result != NULL) && ((line[0] == COMMENT_CHAR) || (line[0] == '\n')));
 ...
 */
int identify_directive (char *dir)
{
 ...
 */
void update_config (char *config_change_desc)
{
    ...
    {
        fprintf (stderr, "Ignoring configuration change: %s", config_change_desc);
        return;
    }
    ...
    {
        fprintf (stderr, "Invalid config: Space before directive or " "no directive present: %s\n", conf_copy);
        return;
    }
    ...
    {
        fprintf (stderr, "Config warning: " "trailing garbage: %s\n", conf_copy);
    }
    ...
    switch (directive_number)
    {
        ...
        case V_SERVER:
            ...
            {
                fprintf (stderr, "Config change overridden by command " "line: %s\n", directive);
                return;
            }

            if (set_server (config_value) == -1)
            {
                fprintf (stderr, "Set server error: unable to change " "the server name to '%s'\n", config_value);
                return;
            }
            ...
            if (set_server_sa () == -1)
            {
                fprintf (stderr, "Set server error: unable to resolve " "a new server '%s'\n", config_value);
                return;
            }
            if (old_af != server_ai->ai_family)
            {
                ...
                {
                    /* XXX: this is fatal */
                    fprintf (stderr, "Set server error: " "unable to open a new socket " "for '%s'\n", config_value);
                    exit (1);
                }
            }
        ...
        case V_PORT:
            ...
            if ((check == TRUE) && (uint_val > 0))
            {
                ...
                {
                    fprintf (stderr, "Invalid config: Bad value for" " %s: %s\n", directive, config_value);
                }
                else
                {
                    if (set_server_sa () == -1)
                    {
                        fprintf (stderr, "Failed to set a new port\n");
                        return;
                    }
                }
            }
 ...
 *   Return a non-negative integer otherwise
 */
int parse_query (char *input, char *qname, int qnlen, int *qtype)
{
 ...
 *   Return a non-negative integer otherwise
 */
int dispatch_query (unsigned short int id, char *dom, int qt)
{
    ...
    buffer_len = res_mkquery (QUERY, dom, C_IN, qt, NULL, 0, NULL, packet_buffer, PACKETSZ);
    if (buffer_len == -1)
    {
    ...
    bytes_sent = sendto (query_socket, packet_buffer, buffer_len, 0, server_ai->ai_addr, server_ai->ai_addrlen);
    if (bytes_sent == -1)
    {
 ...
 */
void send_query (char *query_desc)
{
    ...
    if (dispatch_query (use_query_id, domain, query_type) == -1)
    {
        ...
        if (getnameinfo (server_ai->ai_addr, server_ai->ai_addrlen,
                         serveraddr, sizeof (serveraddr), NULL, 0, NI_NUMERICHOST) == 0)
        {
            addrstr = serveraddr;
        }
        else
    ...
    for (count = 0; (status[count].in_use == TRUE) && (count < max_queries_outstanding); count++);

    if (status[count].in_use == TRUE)
    {
        fprintf (stderr, "Unexpected error: We have run out of " "status[] space!\n");
        return;
    }
...
void register_rtt (struct timeval *timestamp)
{
    int i;
 ...
 */
void register_response (unsigned short int id, unsigned int rcode)
{
    unsigned int ct = 0;
    ...
    for (; (ct < query_status_allocated) && (found == FALSE); ct++)
    {
        if ((status[ct].in_use == TRUE) && (status[ct].id == id))
        {
            ...
            if (status[ct].desc)
            {
                printf ("> %s %s\n", rcode_strings[rcode], status[ct].desc);
                free (status[ct].desc);
            }
            if (countrcodes)
                rcodecounts[rcode]++;
    ...
    if (found == FALSE)
    {
        ...
        {
            fprintf (stderr, "Warning: Received a response with an " "unexpected (maybe timed out) id: %u\n", id);
        }
    }
}
 ...
 */
void process_single_response (int sockfd)
{
    ...
    if ((numbytes = recvfrom (sockfd, in_buf, MAX_BUFFER_LEN, 0, from_addr, &addr_len)) == -1)
    {
 ...
 */
void process_responses (int adjust_rate)
{
    double wait;
    ...
    if (adjust_rate == TRUE)
    {
        ...
        u = time_of_first_query_sec + query_interval * num_queries_sent;
...
void retire_old_queries (int sending)
{
     ...
     */
    if (target_qps > 0 && sending == TRUE && count == 0 && queries_outstanding () == max_queries_outstanding)
    {
        struct timeval next, now;

        double n;

        n = time_of_first_query_sec + query_interval * num_queries_sent;
    ...
    for (; count < query_status_allocated; count++)
    {
        ...
        if ((status[count].in_use == TRUE) && (difftv (curr_time, status[count].sent_timestamp) >= (double) timeout))
        {
...
void print_histogram (unsigned int total)
{
    ...
    for (i = 0; i < rttarray_size; i++)
    {
        ...
        fprintf (fp, "%.6lf %.3lf\n", (double) (i * rttarray_unit) + (double) rttarray_unit / 2, ratio);
    }
...
print_statistics (int intermediate, unsigned int sent, unsigned int timed_out,
                  ...
                  double rmax, double rmin, double rtotal, unsigned int roverflows, unsigned int *rarray)
{
    ...
    else
    {
        ...
        per_lost2 = (100.0 * (double) (timed_out - possibly_delayed)) / (double) sent;
    ...
    else
    {
        ...
        queries_per_sec2 = (double) (num_queries_completed + possibly_delayed) / run_time;

        queries_per_sec_total = (double) sent / difftv (*end_query, *first_query);
    }
    ...
    if (num_queries_completed > 0)
    {
        ...
        for (i = 0; i < rttarray_size; i++)
        {
            if (rarray[i] != 0)
            {
                ...
                mean = (double) (i * rttarray_unit) + (double) rttarray_unit / 2;
    ...
    if (!intermediate)
    {
        ...
        if (run_only_once == FALSE)
            ...
            printf ("  Ended due to:         reaching %s\n", ((runs_through_file == 0) ? "time limit" : "end of file"));
    ...
    printf ("  Started at:           %s", ctime ((const time_t *) &start_time.tv_sec));
    printf ("  Finished at:          %s", ctime ((const time_t *) &end_perf->tv_sec));
...
void print_interval_statistics ()
{
    ...
    set_timenow (&time_now);
    if (difftv (time_now, time_of_first_query_interval) <= (double) print_interval)
        return;
    ...
    print_statistics (TRUE, num_queries_sent_interval,
                      ...
                      rtt_total_interval, rtt_overflows_interval, rttarray_interval);
    ...
    if (rttarray_interval != NULL)
    {
        memset (rttarray_interval, 0, sizeof (rttarray_interval[0]) * rttarray_size);
    }
}
...
int main (int argc, char **argv)
{
    ...
    while ((sending = keep_sending (&got_eof)) == TRUE || queries_outstanding () > 0)
    {
        print_interval_statistics ();
        adjust_rate = FALSE;

        while ((sending = keep_sending (&got_eof)) == TRUE && queries_outstanding () < max_queries_outstanding)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (target_qps > 0 && (num_queries_sent % max_queries_outstanding) == 0)
                    {
                        adjust_rate = TRUE;
                    }
                }
    ...
    print_statistics (FALSE, num_queries_sent, num_queries_timed_out,
                      ...
                      &time_of_end_of_run, &time_of_stop_sending, rtt_max, rtt_min, rtt_total, rtt_overflows, rttarray);
