...
void usage ()
{
    (void) fprintf (stderr, "Usage: %s: [-v] [-d nnn] hostname\n", progname);
    exit (2);
}
     ...
     char *dottedquad;            /* input string with dotted quad */
{
    ...
    {
        case 4:
            (void) sprintf (addrbuf, "%u.%u.%u.%u.%s.", a[3] & 0xff, a[2] & 0xff, a[1] & 0xff, a[0] & 0xff, ARPA_ROOT);
            break;

        case 3:
            (void) sprintf (addrbuf, "%u.%u.%u.%s.", a[2] & 0xff, a[1] & 0xff, a[0] & 0xff, ARPA_ROOT);
            break;

        case 2:
            (void) sprintf (addrbuf, "%u.%u.%s.", a[1] & 0xff, a[0] & 0xff, ARPA_ROOT);
            break;

        case 1:
            (void) sprintf (addrbuf, "%u.%s.", a[0] & 0xff, ARPA_ROOT);
            break;
 ...
 */
char *getlocbyname (name, is_network)
     const char *name;
 ...
 */
char *getlocbynet (name, addr, mask)
     char *name;
     ...
     struct in_addr *mask;
{
    ...
    if (list != NULL)
    {
        ...
        if (mask != NULL && mask->s_addr < list->addr.s_addr)
        {
            ...
            strncpy (newname, in_addr_arpa (inet_ntoa (inet_makeaddr (a, 0))), sizeof newname);
 ...
 ****************************************************************/
int skipToData (cp, type, class, ttl, dlen, endOfMsg)
     u_char *cp;
...
/* 
   ...
   book "DNS and BIND" (O'Reilly).
 */
char *findRR (domain, requested_type)
     char *domain;
...
struct list_in_addr *findA (domain)
     char *domain;
{
    ...
    if ((responseLen = res_query (domain,    /* the domain we care about   */
                                  C_IN,    /* Internet class records     */
                                  T_A, (u_char *) & response,    /*response buffer */
                                  sizeof (response)))    /*buffer size    */
        < 0)
    {                            /*If negative    */
        rr_errno = h_errno;
