...
static DBusHandlerResult default_message_filter (DBusConnection * connection, DBusMessage * message, void *p)
{
    DBusConnectionState *cs = p;

    uint32_t type = dbus_message_get_type (message), serial = dbus_message_get_serial (message);
    ...
    const char
        ...
        *sender = dbus_message_get_sender (message), *signature = dbus_message_get_signature (message);
...
uint8_t dbus_svc_add_filter (DBusConnectionState * cs, dbus_svc_MessageHandler mh, void *obj, int n_matches,...)
{
    DBusError error;
    ...
    {
        if (cs->eh != 0L)
            (*(cs->eh)) ("dbus_svc_add_filter: dbus_connection_add_filter failed");
        va_end (va);
        return (0);
    }
    ...
    if (n_matches)
    {
        ...
        while (n_matches--)
        {
            ...
            {
                if (cs->eh != 0L)
                    (*(cs->eh)) ("dbus_svc_add_filter: dbus_bus_add_match failed for %s: %s %s",
...
uint8_t dbus_svc_get_args_va (DBusConnectionState * cs, DBusMessage * msg, dbus_svc_DataType firstType, va_list va)
{
    DBusError error;
...
uint8_t dbus_svc_get_args (DBusConnectionState * cs, DBusMessage * msg, dbus_svc_DataType firstType, ...)
{
    va_list va;
...
uint8_t
    ...
    (DBusConnectionState * cs,
     ...
     const char *path, const char *interface, const char *member, dbus_svc_DataType firstType, va_list va)
{
    DBusMessageIter iter;

    char *e;

    DBusMessage *msg = dbus_svc_new_message (cs,
                                             type,
                                             reply_serial,
                                             destination,
                                             path,
...
uint8_t
    ...
    (DBusConnectionState * cs,
     ...
     uint32_t * new_serial,
     const char *destination,
     const char *path, const char *interface, const char *member, dbus_svc_DataType firstType,...)
{
    uint8_t r;
...
dbus_svc_MessageHandle
    ...
    (DBusConnectionState * cs,
     dbus_svc_MessageType type,
     int32_t reply_serial, const char *destination, const char *path, const char *interface, const char *member)
{
...
extern uint8_t dbus_svc_send_message (DBusConnectionState * cs, dbus_svc_MessageHandle msg, uint32_t * new_serial)
{
    if (!dbus_connection_send (cs->connection, msg, new_serial))
    {
        if (cs->eh != 0L)
...
dbus_svc_MessageHandle
    ...
    (DBusConnectionState * cs,
     const char *destination,
     const char *path, const char *member, const char *interface, dbus_svc_DataType firstType,...)
{
    ...
    if ((message = dbus_message_new_method_call (destination, path, interface, member)) == 0L)
    {
        if (cs->eh != 0L)
    ...
    if ((reply = dbus_connection_send_with_reply_and_block (cs->connection, message, reply_timeout, &error)) == 0L)
    {
        if (cs->eh != 0L)
            (*(cs->eh)) ("dbus_svc_call: dbus_message_send_with_reply_and_block failed: %s %s",
...
dbus_svc_MessageIterator dbus_svc_message_iterator_new (DBusConnectionState * cs, DBusMessage * msg)
{
...
uint32_t dbus_svc_message_next_arg_type (DBusConnectionState * cs, dbus_svc_MessageIterator iter)
{
    void *p = cs;
...
void dbus_svc_message_next_arg (DBusConnectionState * cs, dbus_svc_MessageIterator iter, void *value)
{
    void *p = cs;
...
uint32_t dbus_svc_message_element_type (DBusConnectionState * cs, dbus_svc_MessageIterator iter)
{
    void *p = cs;
...
void dbus_svc_message_get_elements (DBusConnectionState * cs, dbus_svc_MessageIterator iter, uint32_t * n, void *array)
{
    void *p = cs;
...
static void remove_timeout (DBusTimeout * timeout, void *csp)
{
    ...
    if ((to != 0L) && (to->to == timeout))
    {
        ...
        (*(cs->eh)) ("remove_timeout: can't happen?!?: timeout %p did not record data %p %p",
...
static void toggle_timeout (DBusTimeout * timeout, void *csp)
{
    ...
    if ((top != 0L) && ((to = *top) != 0L) && (to->to == timeout))
    {
        ...
        (*(cs->eh)) ("toggle_timeout: can't happen?!?: timeout %p %s %p %p", timeout,
                     ((to == 0L) ? "did not record data" : "not found"), to, ((to != 0L) ? to->to : 0L));
}
...
static void process_timeout (const void *p, const VISIT which, const int level)
{
    ...
    if ((tpp != 0L) && (*tpp != 0L) && ((which == postorder) || (which == leaf)))
    {
        ...
        if (then != 0.0)
        {
            ...
            {
                if (cs->dh != 0L)
                    (*(cs->dh)) ("handle_timeout: %d - %f %f %f", dbus_timeout_get_interval (to->to), then, now,
                                 interval);
...
static isc_result_t
    connection_setup
    (DBusConnection * connection,
     DBUS_SVC * dbus, dbus_svc_WatchHandler wh, dbus_svc_ErrorHandler eh, dbus_svc_ErrorHandler dh, void *wh_arg)
{
    *dbus = dbcs_new (connection);
    ...
    if (!dbus_connection_set_watch_functions
        ((*dbus)->connection, add_watch, remove_watch, toggle_watch, *dbus, no_free))
    {
        if ((*dbus)->eh != 0L)
            (*((*dbus)->eh)) ("connection_setup: dbus_connection_set_watch_functions failed");
        goto fail;
    }

    if (!dbus_connection_set_timeout_functions
        (connection, add_timeout, remove_timeout, toggle_timeout, *dbus, no_free))
    {
        if ((*dbus)->eh != 0L)
            (*((*dbus)->eh)) ("connection_setup: dbus_connection_set_timeout_functions failed");
        goto fail;
    }

    dbus_connection_set_dispatch_status_function (connection, dispatch_status, *dbus, no_free);
     ...
     DBUS_SVC * dbus, dbus_svc_WatchHandler wh, dbus_svc_ErrorHandler eh, dbus_svc_ErrorHandler dh, void *wh_arg)
{
    ...
    switch (bus)
    {
        ...
        case DBUS_PRIVATE_SYSTEM:
            ...
            {
                if (eh)
                    (*eh) ("dbus_bus_register failed: %s %s", error.name, error.message);
        ...
        case DBUS_PRIVATE_SESSION:
            ...
            {
                if (eh)
                    (*eh) ("dbus_svc_init failed: DBUS_SESSION_BUS_ADDRESS environment variable not set");
                return ISC_R_FAILURE;
            }
        ...
        default:
            if (eh)
                (*eh) ("dbus_svc_init failed: unknown bus type %d", bus);
            return ISC_R_FAILURE;
    }
    ...
    switch (dbus_bus_request_name (connection, name,
...
void dbus_svc_shutdown (DBusConnectionState * cs)
{
    if (!dbus_connection_set_watch_functions (cs->connection, NULL, NULL, NULL, NULL, NULL))
        if (cs->eh != 0L)
            (*(cs->eh)) ("connection_shutdown: dbus_connection_set_watch_functions: No Memory."
                         "Setting watch functions to NULL failed.");

    if (!dbus_connection_set_timeout_functions (cs->connection, NULL, NULL, NULL, NULL, NULL))
        if (cs->eh != 0L)
            (*(cs->eh)) ("connection_shutdown: dbus_connection_set_timeout_functions: No Memory."
