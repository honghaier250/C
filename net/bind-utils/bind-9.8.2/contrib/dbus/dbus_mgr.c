...
typedef struct dbus_mgr_sock_s
{
    int fd;
...
typedef ISC_LIST (dns_name_t) DNSNameList;
     ...
     static void dbus_mgr_watch_handler (int fd, dbus_svc_WatchFlags flags, void *mgrp);

     static dbus_svc_HandlerResult dbus_mgr_message_handler (DBusMsgHandlerArgs);

     static void dbus_mgr_close_socket (const void *p, const VISIT which, const int level);

     static void dbus_mgr_destroy_socket (void *p);

     static void dbus_mgr_free_dhc (void *p);

     static void dbus_mgr_watches_selected (isc_task_t * t, isc_event_t * ev);

     static isc_result_t dbus_mgr_init_dbus (ns_dbus_mgr_t *);

     static isc_result_t dbus_mgr_record_initial_fwdtable (ns_dbus_mgr_t *);

     static dns_fwdtable_t *dbus_mgr_get_fwdtable (void);

     static void dbus_mgr_free_initial_fwdtable (ns_dbus_mgr_t *);

     static uint8_t dbus_mgr_subscribe_to_dhcdbd (ns_dbus_mgr_t *);

     static void dbus_mgr_dbus_shutdown_handler (ns_dbus_mgr_t *);

     static int dbus_mgr_log_err (const char *fmt, ...)
{
    va_list va;

    va_start (va, fmt);
    isc_log_vwrite (ns_g_lctx, NS_LOGCATEGORY_DBUS, NS_LOGMODULE_DBUS, ISC_LOG_NOTICE, fmt, va);
    va_end (va);
    return 0;
}

static int dbus_mgr_log_dbg (const char *fmt, ...)
{
    va_list va;

    va_start (va, fmt);
    isc_log_vwrite (ns_g_lctx, NS_LOGCATEGORY_DBUS, NS_LOGMODULE_DBUS, ISC_LOG_DEBUG (80), fmt, va);
    va_end (va);
    return 0;
}

static int dbus_mgr_log_info (const char *fmt, ...)
{
    va_list va;

    va_start (va, fmt);
    isc_log_vwrite (ns_g_lctx, NS_LOGCATEGORY_DBUS, NS_LOGMODULE_DBUS, ISC_LOG_DEBUG (1), fmt, va);
    va_end (va);
    return 0;
}
...
isc_result_t
    dbus_mgr_create
    (isc_mem_t * mctx,
     isc_taskmgr_t * taskmgr, isc_socketmgr_t * socketmgr, isc_timermgr_t * timermgr, ns_dbus_mgr_t ** dbus_mgr)
{
    isc_result_t result;
    ...
    *dbus_mgr = 0L;
    ...
    if ((result = isc_task_create (taskmgr, 100, &(mgr->task))) != ISC_R_SUCCESS)
        goto cleanup_mgr;
    ...
    if ((result = dbus_mgr_record_initial_fwdtable (mgr)) != ISC_R_SUCCESS)
        goto cleanup_mgr;

    if ((result = dbus_mgr_init_dbus (mgr)) != ISC_R_SUCCESS)
        goto cleanup_mgr;
...
static isc_result_t dbus_mgr_init_dbus (ns_dbus_mgr_t * mgr)
{
    ...
    result = dbus_svc_init (DBUS_PRIVATE_SYSTEM, destination, &mgr->dbus, dbus_mgr_watch_handler, 0L, 0L, mgr);
    ...
    if (mgr->timer != 0L)
    {
        isc_timer_reset (mgr->timer, isc_timertype_inactive, NULL, NULL, ISC_TRUE);
    }
...
static uint8_t dbus_mgr_subscribe_to_dhcdbd (ns_dbus_mgr_t * mgr)
{
    DBUS_SVC dbus = mgr->dbus;
    char subs[1024], path[1024],
        dhcdbd_destination[] = "com.redhat.dhcp", *ddp[1] = { &(dhcdbd_destination[0]) }, *dhcdbd_name = 0L;
    ...
    if (mgr->dhcdbd_name == 0L)
    {
        msg = dbus_svc_call
            (dbus,
             ...
             "/org/freedesktop/DBus", "GetNameOwner", "org.freedesktop.DBus", TYPE_STRING, &ddp, TYPE_INVALID);
        if (msg == 0L)
            return 0;

        if (!dbus_svc_get_args (dbus, msg, TYPE_STRING, &(dhcdbd_name), TYPE_INVALID))
            return 0;
    ...
    for (i = 0; i < n_opts; i++)
    {
        msg = dbus_svc_call (dbus, "com.redhat.dhcp", path, "binary", subs, TYPE_STRING, &(options[i]), TYPE_INVALID);
        if (msg == 0L)
            return 0;
...
void dbus_mgr_shutdown (ns_dbus_mgr_t * mgr)
{
    if (mgr->timer != 0L)
...
static void dbus_mgr_restart_dbus (isc_task_t * t, isc_event_t * ev)
{
...
static void dbus_mgr_handle_dbus_shutdown_event (isc_task_t * t, isc_event_t * ev)
{
    ...
    if (mgr->timer != 0L)
    {
        isc_timer_reset (mgr->timer, isc_timertype_ticker, &tick, &tock, ISC_TRUE);
    }
    else if (isc_timer_create
             (mgr->timermgr,
              isc_timertype_ticker,
              &tick, &tock, mgr->task, dbus_mgr_restart_dbus, mgr, &(mgr->timer)) != ISC_R_SUCCESS)
    {
        dbus_mgr_log_err ("D-BUS service cannot be restored.");
    }
}

static void dbus_mgr_dbus_shutdown_handler (ns_dbus_mgr_t * mgr)
{
    isc_event_t *dbus_shutdown_event = isc_event_allocate (mgr->mctx,
                                                           mgr->task,
                                                           1,
...
static dns_view_t *dbus_mgr_get_localhost_view (void)
{
    dns_view_t *view;
    ...
    for (view = ISC_LIST_HEAD (ns_g_server->viewlist); view != NULL; view = ISC_LIST_NEXT (view, link))
    {
        ...
        if (((view->matchclients != 0L)    /* 0L: accept "any" */
             && ((dns_acl_match (&localhost, NULL,    /* unsigned queries */
                                 view->matchclients, &(ns_g_server->aclenv), &match, NULL    /* no match list */
                  ) != ISC_R_SUCCESS) || (match <= 0))) || ((view->matchdestinations != 0L)    /* 0L: accept "any" */
                                                            && ((dns_acl_match (&localhost, NULL,    /* unsigned queries */
                                                                                view->matchdestinations, &(ns_g_server->aclenv), &match, NULL    /* no match list */
...
static dns_fwdtable_t *dbus_mgr_get_view_and_fwdtable (dns_view_t ** viewp)
{
...
{
    char n1buf[DNS_NAME_FORMATSIZE] = "", *n1p = &(n1buf[0]), n2buf[DNS_NAME_FORMATSIZE] = "", *n2p = &(n2buf[0]);
...
static void dbus_mgr_record_initial_forwarder (dns_name_t * name, dns_forwarders_t * fwdr, void *mp)
{
    ...
    for (sa = ISC_LIST_HEAD (fwdr->addrs); sa != 0L; sa = ISC_LIST_NEXT (sa, link))
    {
...
static isc_result_t dbus_mgr_record_initial_fwdtable (ns_dbus_mgr_t * mgr)
{
...
static void dbus_mgr_free_initial_forwarder (void *p)
{
    ...
    for (sa = ISC_LIST_HEAD (ifwdr->sa); sa != 0L; sa = ISC_LIST_HEAD (ifwdr->sa))
    {
...
static void dbus_mgr_free_initial_fwdtable (ns_dbus_mgr_t * mgr)
{
    tdestroy (mgr->ifwdt, dbus_mgr_free_initial_forwarder);
    mgr->ifwdt = 0L;
}

static void dbus_mgr_log_forwarders (const char *pfx, dns_name_t * name, SockAddrList * saList)
{
    isc_sockaddr_t *sa;
    ...
    for (sa = ISC_LIST_HEAD (*saList); sa != 0L; sa = ISC_LIST_NEXT (sa, link))
    {
...
static
    ...
    (ns_dbus_mgr_t * mgr, DNSNameList * nameList, SockAddrList * saList, dns_fwdpolicy_t fwdpolicy)
{
    ...
    for (dnsName = ISC_LIST_HEAD (*nameList); dnsName != NULL; dnsName = ISC_LIST_NEXT (dnsName, link))
    {
        fwdr = 0L;
        if ((dns_fwdtable_find_exact (fwdtable, dnsName, &fwdr) != ISC_R_SUCCESS) || (fwdr == 0L))
        {
            if (ISC_LIST_HEAD (*saList) == 0L)
                continue;
            ...
            if (result == ISC_R_SUCCESS)
            {
                result = dns_fwdtable_add (fwdtable, dnsName, (isc_sockaddrlist_t *) saList, fwdpolicy);
        ...
        if (result == ISC_R_SUCCESS)
        {
            ...
            for (sa = ISC_LIST_HEAD (fwdr->addrs); sa != 0L; sa = ISC_LIST_HEAD (fwdr->addrs))
            {
                if (ISC_LINK_LINKED (sa, link))
...
static void
    ...
    (ns_dbus_mgr_t * mgr, char *domains, DNSNameList * nameList, char *error_name, char *error_message)
{
    ...
    {                            /* name loop */
        *endName = '\0';
        ...
        result = dns_name_fromtext (dnsName, &buffer, (*(endp - 1) != '.') ? dns_rootname : NULL, 0, NULL);
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            for (dnsName = ISC_LIST_HEAD (*nameList); (dnsName != 0L); dnsName = ISC_LIST_HEAD (*nameList))
            {
...
static isc_result_t
    ...
    (ns_dbus_mgr_t * mgr,
     dbus_svc_MessageIterator iter, SockAddrList * saList, uint8_t * fwdpolicy, char *error_name, char *error_message)
{
    DBUS_SVC dbus = mgr->dbus;

    isc_sockaddr_t *nsSA = 0L, *nsSA_Q = 0L;

    uint32_t argType = dbus_svc_message_next_arg_type (dbus, iter), length;
    ...
    do
    {
        switch (argType)
        {
            ...
            case TYPE_STRING:
                ...
                if (ip != 0L)
                {
                    ...
                    else if (strspn (ip, "0123456789AaBbCcDdEeFf:.") == length)
                    {
                        nsSA = isc_mem_get (mgr->mctx, sizeof (isc_sockaddr_t));
                        if (nsSA != 0L)
                        {
            ...
            case TYPE_UINT16:
                ...
                if ((nsSA == 0L) || (nsSA->type.sa.sa_family == AF_UNSPEC))
                    break;
                else if (nsSA->type.sa.sa_family == AF_INET)
                    dbus_svc_message_next_arg (dbus, iter, &(nsSA->type.sin.sin_port));
                else if (nsSA->type.sa.sa_family == AF_INET6)
        ...
        if ((nsSA != 0L) && (nsSA->type.sa.sa_family == AF_UNSPEC))
        {
            ...
            nsSA = 0L;
            for (nsSA = ISC_LIST_HEAD (*saList); (nsSA != 0L); nsSA = ISC_LIST_HEAD (*saList))
            {
...
static void
    ...
    (ns_dbus_mgr_t * mgr,
     ...
     uint32_t serial,
     const char *path, const char *member, const char *interface, const char *sender, dbus_svc_MessageHandle msg)
{
...
static int dbus_mgr_msg_append_dns_name (DBUS_SVC dbus, dbus_svc_MessageHandle msg, dns_name_t * name)
{
...
static
    ...
    (DBUS_SVC dbus, dbus_svc_MessageHandle msg, dns_forwarders_t * fwdr, DBusMgrOutputInterface outputType)
{
    isc_sockaddr_t *sa;
    char policyBuf[16] = "", *pbp[1] = { &(policyBuf[0]) }, addressBuf[64] = "", *abp[1] =
    {
    &(addressBuf[0])};
    uint8_t *byteArray[1];
    ...
    {
        if (!dbus_svc_message_append_args (dbus, msg, TYPE_BYTE, &(fwdr->fwdpolicy), TYPE_INVALID))
            return 0;
    }
    else if (outputType == OUTPUT_TEXT)
    {
        sprintf (policyBuf, "%s",
                 (fwdr->fwdpolicy == dns_fwdpolicy_none)
                 ? "none" : (fwdr->fwdpolicy == dns_fwdpolicy_first) ? "first" : "only");
        if (!dbus_svc_message_append_args (dbus, msg, TYPE_STRING, pbp, TYPE_INVALID))
            return 0;
    }
    else
        return 0;
        ...
        if (outputType == OUTPUT_BINARY)
        {
            if (sa->type.sa.sa_family == AF_INET)
            {
                if (!dbus_svc_message_append_args
                    (dbus, msg, TYPE_UINT32, &(sa->type.sin.sin_addr.s_addr), TYPE_INVALID))
                    return 0;

                if (!dbus_svc_message_append_args (dbus, msg, TYPE_UINT16, &(sa->type.sin.sin_port), TYPE_INVALID))
                    return 0;
            }
            else if (sa->type.sa.sa_family == AF_INET6)
            {
                ...
                if (!dbus_svc_message_append_args
                    (dbus, msg, TYPE_ARRAY, TYPE_BYTE, &byteArray, sizeof (struct in6_addr), TYPE_INVALID))
                    return 0;

                if (!dbus_svc_message_append_args (dbus, msg, TYPE_UINT16, &(sa->type.sin6.sin6_port), TYPE_INVALID))
                    return 0;
            }
            else
                continue;
        ...
        else if (outputType == OUTPUT_TEXT)
        {
            if (sa->type.sa.sa_family == AF_INET)
            {
                if (inet_ntop (AF_INET, &(sa->type.sin.sin_addr), addressBuf, sizeof (addressBuf)) == 0L)
                    continue;
                if (!dbus_svc_message_append_args (dbus, msg, TYPE_STRING, abp, TYPE_INVALID))
                    return 0;
...
static void forwarders_to_msg (dns_name_t * name, dns_forwarders_t * fwdr, void *mp)
{
    DBusMgrMsg *m = mp;
     ...
     const char *path, const char *member, const char *interface, const char *sender, dbus_svc_MessageHandle msg)
{
    ...
    if ((!dbus_svc_get_args (dbus, msg, TYPE_STRING, &domain, TYPE_INVALID)) || (domain == 0L) || (*domain == '\0'))
    {
    ...
    result = dns_name_fromtext (dnsName, &buffer, dns_rootname, 0, NULL);
    ...
    if ((dns_fwdtable_find_closest (fwdtable, dnsName, foundname, &fwdr) == ISC_R_SUCCESS) && (fwdr != 0L))
    {
    ...
    else
    {
        ...
        if (outputType == OUTPUT_BINARY)
        {
            dbus_svc_message_append_args (dbus, msg, TYPE_UINT32, &(result), TYPE_INVALID);
        }
        else
        {
...
static void dbus_mgr_check_dhcdbd_state (ns_dbus_mgr_t * mgr, dbus_svc_MessageHandle msg)
{
    DBUS_SVC dbus = mgr->dbus;

    char *name_owned = 0L, *old_owner = 0L, *new_owner = 0L;

    if (!dbus_svc_get_args (dbus, msg,
                            TYPE_STRING, &name_owned, TYPE_STRING, &old_owner, TYPE_STRING, &new_owner, TYPE_INVALID))
        return;
    ...
    if (strcmp (name_owned, "com.redhat.dhcp") == 0)
    {
        ...
        if ((mgr->dhcdbd_name == 0L) || (strcmp (mgr->dhcdbd_name, new_owner) != 0))
        {
            if (mgr->dhcdbd_name != 0L)
            {
                isc_mem_put (mgr->mctx, mgr->dhcdbd_name, strlen (mgr->dhcdbd_name) + 1);
    ...
    else if ((mgr->dhcdbd_name != 0L) && (strcmp (mgr->dhcdbd_name, name_owned) == 0L) && (*new_owner == '\0'))
    {
...
static
    dns_name_t *dbus_mgr_if_reverse_ip_name (ns_dbus_mgr_t * mgr, struct in_addr ip_address, struct in_addr subnet_mask)
{
    ...
    if (p > name)
    {
        ...
        result = dns_name_fromtext (dns_name, &buffer, dns_rootname, 0, NULL);
...
static void dbus_mgr_free_dhc (void *p)
{
    ...
    for (sa = ISC_LIST_HEAD (d_if->dns); sa != NULL; sa = ISC_LIST_HEAD (d_if->dns))
    {
        if (ISC_LINK_LINKED (sa, link))
            ISC_LIST_UNLINK (d_if->dns, sa, link);
        isc_mem_put (ns_g_mctx, sa, sizeof (isc_sockaddr_t));
    }
    for (dn = ISC_LIST_HEAD (d_if->dn); dn != NULL; dn = ISC_LIST_HEAD (d_if->dn))
    {
...
static void
    ...
    (ns_dbus_mgr_t * mgr, const char *path, const char *member, dbus_svc_MessageHandle msg)
{
    DBUS_SVC dbus = mgr->dbus;
    ...
    if (((d_ifpp = tfind (&dif, &(mgr->dhc_if), dbus_mgr_dhc_if_comparator)) == 0L) || ((d_if = *d_ifpp) == 0L))
    {
    ...
    if (strcmp (member, "reason") == 0)
    {
        if ((!dbus_svc_get_args (dbus, msg,
                                 ...
                                 TYPE_INVALID))
            || (value == 0L) || (length != sizeof (uint32_t)) || (*((uint32_t *) value) > DHC_END_OPTIONS))
        {
        ...
        switch (dhc_state)
        {
            ...
            case DHC_END_OPTIONS:
                switch (d_if->dhc_state)
                {
                    ...
                    case DHC_RENEW:
                    case DHC_REBIND:
                        if ((d_if->previous_state != DHC_INVALID) && (d_if->previous_state != DHC_RELEASE))
                            break;
                    ...
                    case DHC_BOUND:
                        ...
                        if ((ISC_LIST_HEAD (d_if->dn) != NULL) && (ISC_LIST_HEAD (d_if->dns) != NULL))
                        {
                            dbus_mgr_log_err
                                ("D-BUS: dhclient for interface %s acquired new lease - creating forwarders.", if_name);
                            result = dbus_mgr_set_forwarders (mgr, &(d_if->dn), &(d_if->dns), dns_fwdpolicy_only);
                            if (result != ISC_R_SUCCESS)
                            {
                                dbus_mgr_log_err ("D-BUS: forwarder configuration failed: %s",
                    ...
                    case DHC_RELEASE:
                        ...
                        if (ISC_LIST_HEAD (d_if->dn) != NULL)
                        {
                            dbus_mgr_log_err ("D-BUS: dhclient for interface %s released lease - removing forwarders.",
                                              if_name);
                            for (sa = ISC_LIST_HEAD (d_if->dns); sa != 0L; sa = ISC_LIST_HEAD (d_if->dns))
                            {
                                if (ISC_LINK_LINKED (sa, link))
                                    ISC_LIST_UNLINK (d_if->dns, sa, link);
                                isc_mem_put (mgr->mctx, sa, sizeof (isc_sockaddr_t));
                            ...
                            for (dn = ISC_LIST_HEAD (d_if->dn); dn != 0L; dn = ISC_LIST_NEXT (dn, link))
                            {
                            ...
                            if (result != ISC_R_SUCCESS)
                            {
                                dbus_mgr_log_err ("D-BUS: removal of forwarders failed: %s",
                                                  isc_result_totext (result));
                            }

                            for (dn = ISC_LIST_HEAD (d_if->dn); dn != 0L; dn = ISC_LIST_HEAD (d_if->dn))
                            {
                                if (ISC_LINK_LINKED (dn, link))
                                    ISC_LIST_UNLINK (d_if->dn, dn, link);
                            ...
                            for (ifwdr = ISC_LIST_HEAD (ifwdrList); ifwdr != 0L; ifwdr = ISC_LIST_HEAD (ifwdrList))
                            {
                                ...
                                result = dbus_mgr_set_forwarders (mgr, &nameList, &(ifwdr->sa), ifwdr->fwdpolicy);
                                if (result != ISC_R_SUCCESS)
                                {
                                    dbus_mgr_log_err ("D-BUS: restore of forwarders failed: %s",
                                                      isc_result_totext (result));
                                }
                            }
                        }
    ...
    else if (strcmp (member, "domain_name") == 0)
    {
        if ((!dbus_svc_get_args (dbus, msg,
                                 TYPE_STRING, &opt_name,
                                 TYPE_ARRAY, TYPE_BYTE, &value, &length,
                                 TYPE_INVALID)) || (value == 0L) || (length == 0))
        {
    ...
    else if (strcmp (member, "domain_name_servers") == 0)
    {
        if ((!dbus_svc_get_args (dbus, msg,
                                 TYPE_STRING, &opt_name,
                                 TYPE_ARRAY, TYPE_BYTE, &value, &length,
    ...
    else if (strcmp (member, "ip_address") == 0)
    {
        if ((!dbus_svc_get_args (dbus, msg,
                                 TYPE_STRING, &opt_name,
                                 TYPE_ARRAY, TYPE_BYTE, &value, &length,
                                 TYPE_INVALID)) || (value == 0L) || (length != sizeof (struct in_addr)))
        {
    ...
    else if (strcmp (member, "subnet_mask") == 0)
    {
        if ((!dbus_svc_get_args (dbus, msg,
                                 TYPE_STRING, &opt_name,
                                 TYPE_ARRAY, TYPE_BYTE, &value, &length,
...
static dbus_svc_HandlerResult dbus_mgr_message_handler (DBusMsgHandlerArgs)
{
    ...
    if ((type == SIGNAL) && (strcmp (path, "/org/freedesktop/DBus/Local") == 0))
    {
        if (strcmp (member, "Disconnected") == 0)
            dbus_mgr_dbus_shutdown_handler (mgr);
             ...
             && (strcmp (member, "NameOwnerChanged") == 0) && (strcmp (signature, "sss") == 0))
    {
             ...
             && (strcmp (interface, "com.redhat.dhcp.subscribe.binary") == 0))
    {
        dbus_mgr_handle_dhcdbd_message (mgr, path, member, msg);
             ...
             && (strcmp (destination, DBUSMGR_DESTINATION) == 0) && (strcmp (path, DBUSMGR_OBJECT_PATH) == 0))
    {
        if (strcmp (member, "SetForwarders") == 0)
            dbus_mgr_handle_set_forwarders (mgr, dbus, reply_expected, serial, path, member, interface, sender, msg);
        else if (strcmp (member, "GetForwarders") == 0)
        {
            if (*signature != '\0')
                dbus_mgr_handle_get_forwarders (dbus, reply_expected, serial, path, member, interface, sender, msg);
            else
                dbus_mgr_handle_list_forwarders (dbus, reply_expected, serial, path, member, interface, sender, msg);
        }
        else if (reply_expected)
        {
...
static void dbus_mgr_read_watch_activated (isc_task_t * t, isc_event_t * ev)
{
...
static void dbus_mgr_write_watch_activated (isc_task_t * t, isc_event_t * ev)
{
    DBusMgrSocket *sfd = (DBusMgrSocket *) (ev->ev_arg);
...
static int dbus_mgr_socket_comparator (const void *p1, const void *p2)
{
    return ((((const DBusMgrSocket *) p1)->fd
             == ((const DBusMgrSocket *) p2)->fd) ? 0
            : (((const DBusMgrSocket *) p1)->fd > ((const DBusMgrSocket *) p2)->fd) ? 1 : -1);
}
...
static void dbus_mgr_watch_handler (int fd, dbus_svc_WatchFlags flags, void *mgrp)
{
    ...
    if (((spp = tfind (&sockFd, &(mgr->sockets), dbus_mgr_socket_comparator)) == 0L) || ((sfd = *spp) == 0L))
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            dbus_mgr_log_err ("dbus_mgr: isc_socket_create failed: %s", isc_result_totext (result));
        ...
        if ((flags & WATCH_READ) == WATCH_READ)
        {
            if (sfd->ser == 0L)
            {
                sfd->ser = (isc_socketevent_t *)
                    isc_event_allocate
                    (mgr->mctx, mgr->task,
                     ISC_SOCKEVENT_READ_READY, dbus_mgr_read_watch_activated, sfd, sizeof (isc_socketevent_t));
        ...
        if ((flags & WATCH_WRITE) == WATCH_WRITE)
        {
            if (sfd->sew == 0L)
            {
                sfd->sew = (isc_socketevent_t *)
                    isc_event_allocate
                    (mgr->mctx, mgr->task,
                     ISC_SOCKEVENT_WRITE_READY, dbus_mgr_write_watch_activated, sfd, sizeof (isc_socketevent_t));
                if (sfd->sew == 0L)
                {
                    dbus_mgr_log_err ("dbus_mgr: out of memory");
                    tdelete (sfd, &(mgr->sockets), dbus_mgr_socket_comparator);
        ...
        if ((sfd->ser != 0L) || (sfd->sew != 0L))
        {
            if (sfd->sel == 0L)
            {
                sfd->sel = (isc_socketevent_t *)
                    isc_event_allocate
                    (mgr->mctx, mgr->task,
                     ISC_SOCKEVENT_SELECTED, dbus_mgr_watches_selected, mgr, sizeof (isc_socketevent_t));
                if (sfd->sel == 0L)
                {
                    dbus_mgr_log_err ("dbus_mgr: out of memory");
                    tdelete (sfd, &(mgr->sockets), dbus_mgr_socket_comparator);
...
static void dbus_mgr_close_socket (const void *p, const VISIT which, const int level)
{
    ...
    if ((spp == 0L) || ((sfd = *spp) == 0L) || ((which != leaf) && (which != postorder)))
        return;
