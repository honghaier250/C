 ...
 */
static isc_result_t dirdb_lookup (const char *zone, const char *name, void *dbdata, dns_sdblookup_t * lookup)
{
    char filename[255];
    ...
    if (strcmp (name, "@") == 0)
        ...
        snprintf (filename, sizeof (filename), "%s/%s", (char *) dbdata, name);
    ...
    else if (S_ISCHR (statbuf.st_mode) || S_ISBLK (statbuf.st_mode))
    {
        CHECKN (snprintf (buf, sizeof (buf),
                          ...
                          S_ISCHR (statbuf.st_mode) ? "chr" : "blk", major (statbuf.st_rdev), minor (statbuf.st_rdev)));
    ...
    else if (S_ISLNK (statbuf.st_mode))
    {
        ...
        buf[n] = 0;
        CHECKN (snprintf (buf, sizeof (buf), "\"symlink\" \"%s\"", filename2));
        CHECK (dns_sdb_putrr (lookup, "txt", 3600, buf));
    }
    ...
    {
        CHECKN (snprintf (buf, sizeof (buf), "\"file\" \"size = %u\"", (unsigned int) statbuf.st_size));
        CHECK (dns_sdb_putrr (lookup, "txt", 3600, buf));
    }
 ...
 */
static isc_result_t dirdb_authority (const char *zone, void *dbdata, dns_sdblookup_t * lookup)
{
    isc_result_t result;
 ...
 * level directory.
 */
static isc_result_t dirdb_create (const char *zone, int argc, char **argv, void *driverdata, void **dbdata)
{
    UNUSED (zone);
    UNUSED (driverdata);
 ...
 */
static void dirdb_destroy (const char *zone, void *driverdata, void **dbdata)
{
    UNUSED (zone);
    UNUSED (driverdata);
...
isc_result_t dirdb_init (void)
{
    ...
    return (dns_sdb_register ("dir", &dirdb_methods, ns_g_mctx, flags, ns_g_mctx, &dirdb));
}
