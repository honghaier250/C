...
static isc_result_t tcldb_driver_create (isc_mem_t * mctx, tcldb_driver_t ** driverp)
{
    int tclres;
    ...
    if (tclres != TCL_OK)
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                       ...
                       "initializing tcldb: " "loading 'lookup.tcl' failed: %s", driver->interp->result);
...
static void tcldb_driver_destroy (tcldb_driver_t ** driverp)
{
 ...
 */
static isc_result_t tcldb_lookup (const char *zone, const char *name, void *dbdata, dns_sdblookup_t * lookup)
{
    isc_result_t result = ISC_R_SUCCESS;
    ...
    if (tclres != TCL_OK)
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                       DNS_LOGMODULE_SDB, ISC_LOG_ERROR,
                       "zone '%s': tcl lookup function failed: %s", zone, driver->interp->result);
  ...
  malformed:
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                   DNS_LOGMODULE_SDB, ISC_LOG_ERROR,
                   "zone '%s': " "malformed return value from tcl lookup function: %s", zone, driver->interp->result);
    result = ISC_R_FAILURE;
  fail:
    return (result);
}
 ...
 */
static isc_result_t tcldb_create (const char *zone, int argc, char **argv, void *driverdata, void **dbdata)
{
...
isc_result_t tcldb_init (void)
{
    ...
    return (dns_sdb_register ("tcl", &tcldb_methods, the_driver, flags, ns_g_mctx, &tcldb));
}
