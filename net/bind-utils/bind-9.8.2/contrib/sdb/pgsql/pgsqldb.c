...
static void pgsqldb_destroy (const char *zone, void *driverdata, void **dbdata);
 ...
 */
static isc_result_t pgsqldb_lookup (const char *zone, const char *name, void *dbdata, dns_sdblookup_t * lookup)
{
    isc_result_t result;
 ...
 * allnodes structure.
 */
static isc_result_t pgsqldb_allnodes (const char *zone, void *dbdata, dns_sdballnodes_t * allnodes)
{
    struct dbinfo *dbi = dbdata;
    ...
    snprintf (str, sizeof (str), "SELECT TTL,NAME,RDTYPE,RDATA FROM \"%s\" ORDER BY NAME", dbi->table);
 ...
 */
static isc_result_t pgsqldb_create (const char *zone, int argc, char **argv, void *driverdata, void **dbdata)
{
    struct dbinfo *dbi;
...
isc_result_t pgsqldb_init (void)
{
    ...
    flags = 0;
    return (dns_sdb_register ("pgsql", &pgsqldb_methods, NULL, flags, ns_g_mctx, &pgsqldb));
}
