...
void addrdata (dns_name_t * name, dns_ttl_t ttl, dns_rdata_t * rdata)
{
    ...
    snprintf (str, sizeof (str),
              ...
              " VALUES ('%s', %d, '%s', '%s')", dbtable, canonnamearray, ttl, canontypearray, canondataarray);
    printf ("%s\n", str);
    res = PQexec (conn, str);
...
int main (int argc, char **argv)
{
    ...
    snprintf (str, sizeof (str), "CREATE TABLE %s " "(NAME TEXT, TTL INTEGER, RDTYPE TEXT, RDATA TEXT)", dbtable);
    printf ("%s\n", str);
    res = PQexec (conn, str);
    if (!res || PQresultStatus (res) != PGRES_COMMAND_OK)
    {
