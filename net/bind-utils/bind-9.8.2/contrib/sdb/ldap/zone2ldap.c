...
/* Add to RR list */
void add_to_rr_list (char *dn, char *name, char *type, char *data, unsigned int ttl, unsigned int flags);

/* Error checking */
void isc_result_check (isc_result_t res, char *errorstr);

/* Generate LDIF Format files */
void generate_ldap (dns_name_t * dnsname, dns_rdata_t * rdata, unsigned int ttl);
...
static char *objectClasses[] = { "top", "dNSZone", NULL };
...
int main (int *argc, char **argv)
{
    ...
    result = dns_db_create (mctx, "rbt", zone, dns_dbtype_zone, dns_rdataclass_in, 0, NULL, &db);
 ...
 */
ldap_info *locate_by_dn (char *dn)
{
    ldap_info *tmp;
...
void add_to_rr_list (char *dn, char *name, char *type, char *data, unsigned int ttl, unsigned int flags)
{
    ...
    else
    {
        ...
        {
            sprintf (ldap_type_buffer, "%sRecord", type);
            if (!strncmp (ldap_type_buffer, tmp->attrs[i]->mod_type, strlen (tmp->attrs[i]->mod_type)))
            {
                attrlist = get_attr_list_size (tmp->attrs[i]->mod_values);
                tmp->attrs[i]->mod_values =
                    (char **) realloc (tmp->attrs[i]->mod_values, sizeof (char *) * (attrlist + 1));
        ...
        tmp->attrs = (LDAPMod **) realloc (tmp->attrs, sizeof (LDAPMod) * ++(tmp->attrcnt));
        if (tmp->attrs == NULL)
        {
            fprintf (stderr, "realloc: %s\n", strerror (errno));
            ldap_unbind_s (conn);
...
char **hostname_to_dn_list (char *hostname, char *zone, unsigned int flags)
{
    ...
    for (tmp = strrchr (zname, '.'); tmp != (char *) 0; tmp = strrchr (zname, '.'))
    {
        *tmp++ = '\0';
        dn_buffer[i++] = tmp;
    }
...
void usage ()
{
    fprintf (stderr, "zone2ldap -D [BIND DN] -w [BIND PASSWORD] -b [BASE DN] -z [ZONE] -f [ZONE FILE] -h [LDAP HOST]
