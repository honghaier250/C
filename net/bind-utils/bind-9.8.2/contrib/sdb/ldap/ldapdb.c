...
static LDAP **ldapdb_getconn (struct ldapdb_data *data)
{
    ...
    conndata = ldapdb_find ((struct ldapdb_entry *) threaddata->data, data->hostport, strlen (data->hostport));
    if (conndata == NULL)
    {
...
static void ldapdb_bind (struct ldapdb_data *data, LDAP ** ldp)
{
...
/* callback routines */
static isc_result_t ldapdb_lookup (const char *zone, const char *name, void *dbdata, dns_sdblookup_t * lookup)
{
...
static isc_result_t ldapdb_create (const char *zone, int argc, char **argv, void *driverdata, void **dbdata)
{
    ...
    if ((argc < 2) || (argv[0] != strstr (argv[0], "ldap://")) || ((defaultttl = atoi (argv[1])) < 1))
        return (ISC_R_FAILURE);
    ...
    else
    {
        ...
        data->filteronelen =
            strlen (filter) + strlen (zone) + strlen ("(&(zoneName=)(relativeDomainName=))") + MAXNAMELEN + 1;
    }
    ...
    if (*data->hostport == '[' && (s = strchr (data->hostname, ']')) != NULL)
        *s++ = '\0';
    else
        s = data->hostname;
    s = strchr (s, ':');
...
isc_result_t ldapdb_init (void)
{
    ...
    ldapdb_lock (0);
    return (dns_sdb_register ("ldap", &ldapdb_methods, NULL, flags, ns_g_mctx, &ldapdb));
}
