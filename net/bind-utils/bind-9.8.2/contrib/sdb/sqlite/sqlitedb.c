...
static isc_result_t db_connect (dbinfo_t * dbi)
{
...
static int sqlitedb_lookup_cb (void *p, int cc, char **cv, char **cn)
{
...
static isc_result_t sqlitedb_lookup (const char *zone, const char *name, void *dbdata, dns_sdblookup_t * lookup)
 ...
 */
{
    ...
    sql = sqlite3_mprintf ("SELECT TTL,RDTYPE,RDATA FROM \"%q\" WHERE " "lower(NAME) = lower('%q')", dbi->table, name);

    result = sqlite3_exec (dbi->db, sql, &sqlitedb_lookup_cb, &parm, &errmsg);
    sqlite3_free (sql);
...
static int sqlitedb_allnodes_cb (void *p, int cc, char **cv, char **cn)
{
...
static isc_result_t sqlitedb_allnodes (const char *zone, void *dbdata, dns_sdballnodes_t * allnodes)
{
    dbinfo_t *dbi = (dbinfo_t *) dbdata;
    ...
    sql = sqlite3_mprintf ("SELECT TTL,NAME,RDTYPE,RDATA FROM \"%q\" ORDER BY NAME", dbi->table);

    result = sqlite3_exec (dbi->db, sql, &sqlitedb_allnodes_cb, &parm, &errmsg);
    sqlite3_free (sql);
...
static void sqlitedb_destroy (const char *zone, void *driverdata, void **dbdata)
{
    dbinfo_t *dbi = *dbdata;
 ...
 */
static isc_result_t sqlitedb_create (const char *zone, int argc, char **argv, void *driverdata, void **dbdata)
{
    dbinfo_t *dbi;
...
isc_result_t sqlitedb_init (void)
{
    ...
    flags = 0;
    return (dns_sdb_register ("sqlite", &sqlitedb_methods, NULL, flags, ns_g_mctx, &sqlitedb));
}
