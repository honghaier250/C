...
static void closeandexit (int status)
{
    if (dbi.db)
    {
...
static void check_result (isc_result_t result, const char *message)
{
...
static int add_rdata_cb (void *parm, int cc, char **cv, char **cn)
{
    UNUSED (parm);
    UNUSED (cc);
    UNUSED (cv);
...
static void addrdata (dns_name_t * name, dns_ttl_t ttl, dns_rdata_t * rdata)
{
    ...
    sql = sqlite3_mprintf ("INSERT INTO %q (NAME, TTL, RDTYPE, RDATA)"
                           " VALUES ('%q', %d, '%q', '%q') ", dbi.table, namearray, ttl, typearray, dataarray);
    printf ("%s\n", sql);
...
int main (int argc, char *argv[])
{
    ...
    db = NULL;
    result = dns_db_create (mctx, "rbt", origin, dns_dbtype_zone, dns_rdataclass_in, 0, NULL, &db);
    ...
    sql = sqlite3_mprintf ("CREATE TABLE %q " "(NAME TEXT, TTL INTEGER, RDTYPE TEXT, RDATA TEXT) ", dbi.table);
    printf ("%s\n", sql);
    res = sqlite3_exec (dbi.db, sql, NULL, NULL, &errmsg);
    sqlite3_free (sql);
    if (res != SQLITE_OK)
