...
static isc_result_t bdb_create (const char *zone, int argc, char **argv, void *unused, void **dbdata)
{
    int ret;
    ...
    if (db_create ((DB **) dbdata, NULL, 0) != 0)
    {
        ...
        isc_log_iwrite (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                        ...
                        ISC_MSGSET_GENERAL, ISC_MSG_FATALERROR, "db_create");
    ...
    if (isc_file_exists (*argv) != ISC_TRUE)
    {
        isc_log_iwrite (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                        DNS_LOGMODULE_SDB, ISC_LOG_CRITICAL, isc_msgcat,
                        ISC_MSGSET_GENERAL, ISC_MSG_FATALERROR, "isc_file_exists: %s", *argv);
        return ISC_R_FAILURE;
    }

    if ((ret = (*(DB **) dbdata)->open (*(DB **) dbdata, *argv, NULL, DB_HASH, DB_RDONLY, 0)) != 0)
    {
        isc_log_iwrite (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                        DNS_LOGMODULE_SDB, ISC_LOG_CRITICAL,
                        isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_FATALERROR, "DB->open: %s", db_strerror (ret));
        return ISC_R_FAILURE;
    }
    return ISC_R_SUCCESS;
}

static isc_result_t bdb_lookup (const char *zone, const char *name, void *dbdata, dns_sdblookup_t * l)
{
    int ret;
    ...
    if ((ret = ((DB *) dbdata)->cursor ((DB *) dbdata, NULL, &c, 0)) != 0)
    {
        isc_log_iwrite (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                        ...
                        isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_FAILED, "DB->cursor: %s", db_strerror (ret));
        return ISC_R_FAILURE;
    }
    ...
    while (ret == 0)
    {
        ...
        if (dns_sdb_putrr (l, type, ttl, rdata) != ISC_R_SUCCESS)
        {
            isc_log_iwrite (dns_lctx,
                            ...
                            DNS_LOGMODULE_SDB, ISC_LOG_ERROR,
                            isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_FAILED, "dns_sdb_putrr");
            return ISC_R_FAILURE;
        }
...
static isc_result_t bdb_allnodes (const char *zone, void *dbdata, dns_sdballnodes_t * n)
{
    int ret;
    ...
    while (c->c_get (c, &key, &data, DB_NEXT) == 0)
    {
        ...
        if (dns_sdb_putnamedrr (n, key.data, type, ttl, rdata) != ISC_R_SUCCESS)
        {
            isc_log_iwrite (dns_lctx,
                            DNS_LOGCATEGORY_DATABASE,
                            DNS_LOGMODULE_SDB, ISC_LOG_ERROR,
                            isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_FAILED, "dns_sdb_putnamedrr");
            return ISC_R_FAILURE;
        }
...
static isc_result_t bdb_destroy (const char *zone, void *unused, void **dbdata)
{
...
isc_result_t bdb_init (void)
{
    ...
    return dns_sdb_register (DRIVERNAME, &bdb_methods, NULL, 0, ns_g_mctx, &bdb_imp);
}
