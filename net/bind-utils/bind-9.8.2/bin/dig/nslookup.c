...
static isc_boolean_t short_form = ISC_TRUE,
    ...
    section_additional = ISC_TRUE, recurse = ISC_TRUE, aaonly = ISC_FALSE, nofail = ISC_TRUE;
...
static isc_result_t
printsection (dig_query_t * query, dns_message_t * msg, isc_boolean_t headers, dns_section_t section)
{
...
static isc_result_t
detailsection (dig_query_t * query, dns_message_t * msg, isc_boolean_t headers, dns_section_t section)
{
    isc_result_t result, loopresult;
    ...
    for (;;)
    {
        ...
        for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
        {
            if (section == DNS_SECTION_QUESTION)
            {
                dns_name_format (name, namebuf, sizeof (namebuf));
                printf ("\t%s, ", namebuf);
                dns_rdatatype_format (rdataset->type, namebuf, sizeof (namebuf));
                printf ("type = %s, ", namebuf);
                dns_rdataclass_format (rdataset->rdclass, namebuf, sizeof (namebuf));
                printf ("class = %s\n", namebuf);
            }
            loopresult = dns_rdataset_first (rdataset);
            while (loopresult == ISC_R_SUCCESS)
...
isc_result_t printmessage (dig_query_t * query, dns_message_t * msg, isc_boolean_t headers)
{
    ...
    if (msg->rcode != 0)
    {
        ...
        dns_name_format (query->lookup->name, nametext, sizeof (nametext));
        printf ("** server can't find %s: %s\n",
                (msg->rcode != dns_rcode_nxdomain) ? nametext : query->lookup->textname, rcode_totext (msg->rcode));
...
static void show_settings (isc_boolean_t full, isc_boolean_t serv_only)
{
    ...
    printf ("\nSet options:\n");
    printf ("  %s\t\t\t%s\t\t%s\n",
            tcpmode ? "vc" : "novc", short_form ? "nodebug" : "debug", debugging ? "d2" : "nod2");
    printf ("  %s\t\t%s\n", usesearch ? "search" : "nosearch", recurse ? "recurse" : "norecurse");
    printf ("  timeout = %d\t\tretry = %d\tport = %d\n", timeout, tries, port);
    printf ("  querytype = %-8s\tclass = %s\n", deftype, defclass);
    printf ("  srchlist = ");
    for (listent = ISC_LIST_HEAD (search_list); listent != NULL; listent = ISC_LIST_NEXT (listent, link))
    {
...
static void addlookup (char *opt)
{
    ...
    if (get_reverse (store, sizeof (store), opt, lookup->ip6_int, ISC_TRUE) == ISC_R_SUCCESS)
    {
