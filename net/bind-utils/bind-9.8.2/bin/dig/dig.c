...
static isc_boolean_t short_form = ISC_FALSE, printcmd = ISC_TRUE,
    ...
    multiline = ISC_FALSE, nottl = ISC_FALSE, noclass = ISC_FALSE, onesoa = ISC_FALSE;
...
{
    fputs ("Usage:  dig [@global-server] [domain] [q-type] [q-class] {q-opt}\n"
...
void received (int bytes, isc_sockaddr_t * from, dig_query_t * query)
{
    ...
    if (query->lookup->stats && !short_form)
    {
        ...
        if (query->lookup->doing_xfr)
        {
            printf (";; XFR size: %u records (messages %u, "
                    "bytes %" ISC_PRINT_QUADFORMAT "u)\n", query->rr_count, query->msg_count, query->byte_count);
        }
        else
        {
        ...
        {
            if (!validated)
                puts (";; WARNING -- Some TSIG could not " "be validated");
        }
    ...
    else if (query->lookup->identify && !short_form)
    {
        ...
        printf (";; Received %" ISC_PRINT_QUADFORMAT "u bytes "
                ...
                query->byte_count : (isc_uint64_t) bytes, fromtext, query->servname, (int) diff / 1000);
    }
}
 ...
 */
static isc_result_t
short_answer (dns_message_t * msg, dns_messagetextflag_t flags, isc_buffer_t * buf, dig_query_t * query)
{
    dns_name_t *name;
...
#ifdef DIG_SIGCHASE
isc_result_t printrdataset (dns_name_t * owner_name, dns_rdataset_t * rdataset, isc_buffer_t * target)
{
    isc_result_t result;
    ...
    if (multiline || (nottl && noclass))
        result = dns_master_stylecreate (&style, styleflags, 24, 24, 24, 32, 80, 8, mctx);
    else if (nottl || noclass)
        result = dns_master_stylecreate (&style, styleflags, 24, 24, 32, 40, 80, 8, mctx);
    else
        result = dns_master_stylecreate (&style, styleflags, 24, 32, 40, 48, 80, 8, mctx);
...
isc_result_t printmessage (dig_query_t * query, dns_message_t * msg, isc_boolean_t headers)
{
    ...
    debug ("printmessage(%s %s %s)", headers ? "headers" : "noheaders",
           query->lookup->comments ? "comments" : "nocomments", short_form ? "short_form" : "long_form");
    ...
    if (onesoa && query->lookup->rdtype == dns_rdatatype_axfr)
        flags |= (query->msg_count == 0) ? DNS_MESSAGETEXTFLAG_ONESOA : DNS_MESSAGETEXTFLAG_OMITSOA;
    ...
    if (query->lookup->comments && !short_form)
    {
        ...
        if (headers)
        {
            printf (";; ->>HEADER<<- opcode: %s, status: %s, "
                    "id: %u\n", opcodetext[msg->opcode], rcode_totext (msg->rcode), msg->id);
            printf (";; flags:");
            ...
            printf ("; QUERY: %u, ANSWER: %u, "
                    ...
                    msg->counts[DNS_SECTION_AUTHORITY], msg->counts[DNS_SECTION_ADDITIONAL]);

            if (msg != query->lookup->sendmsg &&
                (msg->flags & DNS_MESSAGEFLAG_RD) != 0 && (msg->flags & DNS_MESSAGEFLAG_RA) == 0)
                printf (";; WARNING: recursion requested " "but not available\n");
        }
        if (msg != query->lookup->sendmsg && extrabytes != 0U)
            printf (";; WARNING: Messages has %u extra byte%s at " "end\n", extrabytes, extrabytes != 0 ? "s" : "");
    }
  ...
  repopulate_buffer:
    ...
    {
        result = dns_message_pseudosectiontotext (msg, DNS_PSEUDOSECTION_OPT, style, flags, buf);
        if (result == ISC_R_NOSPACE)
        {
          buftoosmall:
            len += OUTPUTBUF;
    ...
    if (query->lookup->section_question && headers)
    {
        if (!short_form)
        {
            result = dns_message_sectiontotext (msg, DNS_SECTION_QUESTION, style, flags, buf);
            if (result == ISC_R_NOSPACE)
                goto buftoosmall;
    ...
    if (query->lookup->section_answer)
    {
        if (!short_form)
        {
            result = dns_message_sectiontotext (msg, DNS_SECTION_ANSWER, style, flags, buf);
            if (result == ISC_R_NOSPACE)
                goto buftoosmall;
            check_result (result, "dns_message_sectiontotext");
        }
    ...
    if (query->lookup->section_authority)
    {
        if (!short_form)
        {
            result = dns_message_sectiontotext (msg, DNS_SECTION_AUTHORITY, style, flags, buf);
            if (result == ISC_R_NOSPACE)
                goto buftoosmall;
            check_result (result, "dns_message_sectiontotext");
        }
    ...
    if (query->lookup->section_additional)
    {
        if (!short_form)
        {
            result = dns_message_sectiontotext (msg, DNS_SECTION_ADDITIONAL, style, flags, buf);
            if (result == ISC_R_NOSPACE)
                goto buftoosmall;
            check_result (result, "dns_message_sectiontotext");
            /*
            ...
            if (headers)
            {
                result = dns_message_pseudosectiontotext (msg, DNS_PSEUDOSECTION_TSIG, style, flags, buf);
                if (result == ISC_R_NOSPACE)
                    goto buftoosmall;
                check_result (result, "dns_message_pseudosectiontotext");
                result = dns_message_pseudosectiontotext (msg, DNS_PSEUDOSECTION_SIG0, style, flags, buf);
                if (result == ISC_R_NOSPACE)
                    goto buftoosmall;
                check_result (result, "dns_message_pseudosectiontotext");
            }
    ...
    printf ("%.*s", (int) isc_buffer_usedlength (buf), (char *) isc_buffer_base (buf));
    isc_buffer_free (&buf);
...
static void printgreeting (int argc, char **argv, dig_lookup_t * lookup)
{
    ...
    if (printcmd)
    {
        ...
        snprintf (lookup->cmdline, sizeof (lookup->cmdline), "%s; <<>> DiG " VERSION " <<>>", first ? "\n" : "");
        i = 1;
        while (i < argc)
        {
            snprintf (append, sizeof (append), " %s", argv[i++]);
            remaining = sizeof (lookup->cmdline) - strlen (lookup->cmdline) - 1;
        ...
        {
            snprintf (append, sizeof (append), "; (%d server%s found)\n", addresscount, addresscount > 1 ? "s" : "");
            remaining = sizeof (lookup->cmdline) - strlen (lookup->cmdline) - 1;
            strncat (lookup->cmdline, append, remaining);
        }
        if (first)
        ...
        case 'b':
            switch (cmd[1])
            {
                ...
                case 'u':        /* bufsize */
                    ...
                    if (!state)
                        goto invalid_option;
                    result = parse_uint (&num, value, COMMSIZE, "buffer size");
                    if (result != ISC_R_SUCCESS)
                ...
                case 'e':
                    switch (cmd[2])
                    {
                        ...
                        case 't':    /* retry / retries */
                            ...
                            if (!state)
                                goto invalid_option;
                            result = parse_uint (&lookup->retries, value, MAXTRIES - 1, "retries");
                            if (result != ISC_R_SUCCESS)
        ...
        case 't':
            switch (cmd[1])
            {
                ...
                case 'i':        /* timeout */
                    ...
                    if (!state)
                        goto invalid_option;
                    result = parse_uint (&timeout, value, MAXTIMEOUT, "timeout");
                    if (result != ISC_R_SUCCESS)
                ...
                case 'r':
                    switch (cmd[2])
                    {
                        ...
                        case 'i':    /* tries */
                            ...
                            if (!state)
                                goto invalid_option;
                            result = parse_uint (&lookup->retries, value, MAXTRIES, "tries");
                            if (result != ISC_R_SUCCESS)
                        ...
                        case 'u':    /* trusted-key */
                            ...
                            if (!state)
                                goto invalid_option;
                            n = strlcpy (trustedkey, ptr, sizeof (trustedkey));
...
dash_option (char *option, char *next, dig_lookup_t ** lookup,
             ...
             isc_boolean_t config_only, int argc, char **argv, isc_boolean_t * firstarg)
{
    ...
    switch (opt)
    {
        case 'b':
            ...
            if (hash != NULL)
            {
                result = parse_uint (&num, hash + 1, MAXPORT, "port number");
                if (result != ISC_R_SUCCESS)
        ...
        case 'c':
            ...
            *open_type_class = ISC_FALSE;
            ...
            if (result == ISC_R_SUCCESS)
            {
                ...
                fprintf (stderr, ";; Warning, ignoring " "invalid class %s\n", value);
            return (value_from_next);
        case 'f':
            batchname = value;
            return (value_from_next);
        ...
        case 'q':
            if (!config_only)
            {
                ...
                *need_clone = ISC_TRUE;
                strncpy ((*lookup)->textname, value, sizeof ((*lookup)->textname));
                (*lookup)->textname[sizeof ((*lookup)->textname) - 1] = 0;
                (*lookup)->trace_root = ISC_TF ((*lookup)->trace || (*lookup)->ns_search_only);
        ...
        case 't':
            *open_type_class = ISC_FALSE;
            ...
            else
            {
                ...
                if (result == ISC_R_SUCCESS && rdtype == dns_rdatatype_ixfr)
                {
            ...
            if (result == ISC_R_SUCCESS)
            {
                if ((*lookup)->rdtypeset)
                {
                    fprintf (stderr, ";; Warning, " "extra type option\n");
                }
                if (rdtype == dns_rdatatype_ixfr)
                {
                    ...
                    (*lookup)->rdtypeset = ISC_TRUE;
                    result = parse_uint (&serial, &value[5], MAXSERIAL, "serial number");
                    if (result != ISC_R_SUCCESS)
                ...
                fprintf (stderr, ";; Warning, ignoring " "invalid type %s\n", value);
            return (value_from_next);
        case 'y':
        ...
        case 'x':
            if (*need_clone)
                *lookup = clone_lookup (default_lookup, ISC_TRUE);
            *need_clone = ISC_TRUE;
            if (get_reverse (textname, sizeof (textname), value, ip6_int, ISC_FALSE) == ISC_R_SUCCESS)
            {
                strncpy ((*lookup)->textname, textname, sizeof ((*lookup)->textname));
                debug ("looking up %s", (*lookup)->textname);
                (*lookup)->trace_root = ISC_TF ((*lookup)->trace || (*lookup)->ns_search_only);
    ...
    if (!is_batchfile)
    {
        ...
        if (batchfp != NULL)
        {
            while (fgets (batchline, sizeof (batchline), batchfp) != 0)
            {
                ...
                while ((bargv[bargc] != NULL) && (bargc < 62))
                {
                    bargc++;
                    bargv[bargc] = next_token (&input, " \t\r\n");
                }
                ...
                for (i = 0; i < bargc; i++)
                    debug (".digrc argv %d: %s", i, bargv[i]);
        ...
        else if (rv[0][0] == '-')
        {
            if (rc <= 1)
            {
                if (dash_option (&rv[0][1], NULL,
                                 &lookup, &open_type_class, &need_clone, config_only, argc, argv, &firstarg))
                {
                    rc--;
                    rv++;
                }
        ...
        else
        {
            ...
            if (open_type_class)
            {
                ...
                else
                {
                    ...
                    if (result == ISC_R_SUCCESS && rdtype == dns_rdatatype_ixfr)
                    {
                        fprintf (stderr, ";; Warning, " "ixfr requires a " "serial number\n");
                        continue;
                    }
                }
                if (result == ISC_R_SUCCESS)
                {
                    ...
                    if (rdtype == dns_rdatatype_ixfr)
                    {
                        ...
                        lookup->rdtype = dns_rdatatype_ixfr;
                        lookup->rdtypeset = ISC_TRUE;
                        result = parse_uint (&serial, &rv[0][5], MAXSERIAL, "serial number");
                        if (result != ISC_R_SUCCESS)
                            fatal ("Couldn't parse " "serial number");
                        lookup->ixfr_serial = serial;
                        lookup->section_question = plusquest;
            ...
            if (!config_only)
            {
                if (need_clone)
                    lookup = clone_lookup (default_lookup, ISC_TRUE);
                need_clone = ISC_TRUE;
                strncpy (lookup->textname, rv[0], sizeof (lookup->textname));
                lookup->textname[sizeof (lookup->textname) - 1] = 0;
                lookup->trace_root = ISC_TF (lookup->trace || lookup->ns_search_only);
                lookup->new_search = ISC_TRUE;
                if (firstarg)
