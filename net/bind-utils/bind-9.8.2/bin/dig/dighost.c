...
isc_boolean_t
    ...
    free_now = ISC_FALSE,
    cancel_now = ISC_FALSE, usesearch = ISC_FALSE, showsearch = ISC_FALSE, qr = ISC_FALSE, is_dst_up = ISC_FALSE;
in_port_t port = 53;
...
static isc_result_t output_filter (isc_buffer_t * buffer, unsigned int used_org, isc_boolean_t absolute);

static idn_result_t append_textname (char *name, const char *origin, size_t namesize);
...
dns_rdataset_t *sigchase_scanname (dns_rdatatype_t type,
                                   dns_rdatatype_t covers, isc_boolean_t * lookedup, dns_name_t * rdata_name);
dns_rdataset_t *chase_scanname_section (dns_message_t * msg,
                                        dns_name_t * name, dns_rdatatype_t type, dns_rdatatype_t covers, int section);
isc_result_t advanced_rrsearch (dns_rdataset_t ** rdataset,
                                dns_name_t * name,
                                dns_rdatatype_t type, dns_rdatatype_t covers, isc_boolean_t * lookedup);
isc_result_t sigchase_verify_sig_key (dns_name_t * name,
                                      dns_rdataset_t * rdataset,
                                      dst_key_t * dnsseckey, dns_rdataset_t * sigrdataset, isc_mem_t * mctx);
isc_result_t sigchase_verify_sig (dns_name_t * name,
                                  dns_rdataset_t * rdataset,
                                  dns_rdataset_t * keyrdataset, dns_rdataset_t * sigrdataset, isc_mem_t * mctx);
isc_result_t sigchase_verify_ds (dns_name_t * name,
                                 dns_rdataset_t * keyrdataset, dns_rdataset_t * dsrdataset, isc_mem_t * mctx);
void sigchase (dns_message_t * msg);

void print_rdata (dns_rdata_t * rdata, isc_mem_t * mctx);

void print_rdataset (dns_name_t * name, dns_rdataset_t * rdataset, isc_mem_t * mctx);

void dup_name (dns_name_t * source, dns_name_t * target, isc_mem_t * mctx);
...
dns_rdataset_t *search_type (dns_name_t * name, dns_rdatatype_t type, dns_rdatatype_t covers);

isc_result_t contains_trusted_key (dns_name_t * name,
                                   dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset, isc_mem_t * mctx);
void print_type (dns_rdatatype_t type);

isc_result_t prove_nx_domain (dns_message_t * msg,
                              dns_name_t * name,
                              dns_name_t * rdata_name, dns_rdataset_t ** rdataset, dns_rdataset_t ** sigrdataset);
...
isc_result_t opentmpkey (isc_mem_t * mctx, const char *file, char **tempp, FILE ** fp);
...
isc_result_t grandfather_pb_test (dns_name_t * zone_name, dns_rdataset_t * sigrdataset);

isc_result_t child_of_zone (dns_name_t * name, dns_name_t * zone_name, dns_name_t * child_name);
...
static void copy_server_list (lwres_conf_t * confdata, dig_serverlist_t * dest)
{
    ...
    for (i = 0; i < confdata->nsnext; i++)
    {
        ...
        lwres_net_ntop (af, confdata->nameservers[i].address, tmp, sizeof (tmp));
...
void set_nameserver (char *opt)
{
    ...
    result = bind9_getaddresses (opt, 0, sockaddrs, DIG_MAX_ADDRESSES, &count);
    if (result != ISC_R_SUCCESS)
        fatal ("couldn't get address for '%s': %s", opt, isc_result_totext (result));
...
dig_lookup_t *clone_lookup (dig_lookup_t * lookold, isc_boolean_t servers)
{
    ...
    dns_name_copy (dns_fixedname_name (&lookold->fdomain), dns_fixedname_name (&looknew->fdomain), NULL);

    if (servers)
        clone_server_list (lookold->my_server_list, &looknew->my_server_list);
    return (looknew);
}
...
dig_lookup_t *requeue_lookup (dig_lookup_t * lookold, isc_boolean_t servers)
{
    ...
    debug ("before insertion, init@%p -> %p, new@%p -> %p", lookold, lookold->link.next, looknew, looknew->link.next);
    ISC_LIST_PREPEND (lookup_list, looknew, link);
    debug ("after insertion, init -> %p, new = %p, new -> %p", lookold, looknew, looknew->link.next);
    return (looknew);
}


static void setup_text_key (void)
{
    isc_result_t result;
    ...
    result = dns_tsigkey_create (&keyname, hmacname, secretstore, secretsize, ISC_FALSE, NULL, 0, 0, mctx, NULL, &key);
  failure:
    if (result != ISC_R_SUCCESS)
...
isc_result_t parse_uint (isc_uint32_t * uip, const char *value, isc_uint32_t max, const char *desc)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        printf ("invalid %s '%s': %s\n", desc, value, isc_result_totext (result));
        return (result);
    }
    *uip = n;
    return (ISC_R_SUCCESS);
...
void parse_hmac (const char *hmac)
{
    ...
    {
        fprintf (stderr, ";; Warning, ignoring " "invalid TSIG algorithm %s\n", buf);
    }
}
...
static void setup_file_key (void)
{
    isc_result_t result;
    ...
    result = dst_key_fromnamedfile (keyfile, NULL, DST_TYPE_PRIVATE | DST_TYPE_KEY, mctx, &dstkey);
    ...
    result = dns_tsigkey_createfromkey (dst_key_name (dstkey), hmacname,
                                        dstkey, ISC_FALSE, NULL, 0, 0, mctx, NULL, &key);
    if (result != ISC_R_SUCCESS)
    {
        printf (";; Couldn't create key %s: %s\n", keynametext, isc_result_totext (result));
        goto failure;
...
static dig_searchlist_t *make_searchlist_entry (char *domain)
{
...
void setup_system (void)
{
    ...
    lwresult = lwres_context_create (&lwctx, mctx, mem_alloc, mem_free, lwresflags);
 ...
 */
static void
add_opt (dns_message_t * msg, isc_uint16_t udpsize, isc_uint16_t edns, isc_boolean_t dnssec, isc_boolean_t nsid)
{
...
static void check_if_done (void)
{
    ...
    if (ISC_LIST_EMPTY (lookup_list) && current_lookup == NULL && sendcount == 0)
    {
        INSIST (sockcount == 0);
        INSIST (recvcount == 0);
        debug ("shutting down");
...
static void clear_query (dig_query_t * query)
{
    ...
    if (ISC_LINK_LINKED (&query->recvbuf, link))
        ISC_LIST_DEQUEUE (query->recvlist, &query->recvbuf, link);
    if (ISC_LINK_LINKED (&query->lengthbuf, link))
        ISC_LIST_DEQUEUE (query->lengthlist, &query->lengthbuf, link);
...
void destroy_lookup (dig_lookup_t * lookup)
{
    ...
    while (s != NULL)
    {
        ...
        ptr = s;
        s = ISC_LIST_NEXT (s, link);
        ISC_LIST_DEQUEUE (lookup->my_server_list, (dig_server_t *) ptr, link);
...
#if DIG_SIGCHASE_TD
        if (current_lookup->do_topdown && !current_lookup->rdtype_sigchaseset)
        {
            ...
            for (i = 0; i < tk_list.nb_tk; i++)
            {
                ...
                if (dns_name_issubdomain (&query_name, key_name) == ISC_TRUE)
            ...
            if (trustedkey == NULL)
            {
                ...
                printf (" isn't a subdomain of any Trusted Keys" ": +sigchase option is disable\n");
                current_lookup->sigchase = ISC_FALSE;
            ...
            current_lookup->rdtype_sigchase = current_lookup->rdtype;
            current_lookup->rdtype_sigchaseset = current_lookup->rdtypeset;
            current_lookup->rdtype = dns_rdatatype_ns;

            current_lookup->qrdtype_sigchase = current_lookup->qrdtype;
            current_lookup->qrdtype = dns_rdatatype_ns;

            current_lookup->rdclass_sigchase = current_lookup->rdclass;
            current_lookup->rdclass_sigchaseset = current_lookup->rdclassset;
            current_lookup->rdclass = dns_rdataclass_in;

            strncpy (current_lookup->textnamesigchase, current_lookup->textname, MXNAME);

            current_lookup->trace_root_sigchase = ISC_TRUE;

            result = isc_buffer_allocate (mctx, &b, BUFSIZE);
            check_result (result, "isc_buffer_allocate");
            result = dns_name_totext (dst_key_name (trustedkey), ISC_FALSE, b);
            check_result (result, "dns_name_totext");
            isc_buffer_usedregion (b, &r);
            r.base[r.length] = '\0';
            strncpy (current_lookup->textname, (char *) r.base, MXNAME);
            isc_buffer_free (&b);

            nameFromString (current_lookup->textnamesigchase, &chase_name);
...
static int followup_lookup (dns_message_t * msg, dig_query_t * query, dns_section_t section)
{
    ...
    for (result = dns_message_firstname (msg, section);
         result == ISC_R_SUCCESS; result = dns_message_nextname (msg, section))
    {
        name = NULL;
        ...
        {
            rdataset = NULL;
            result = dns_message_findtype (name, dns_rdatatype_soa, 0, &rdataset);
            if (result == ISC_R_SUCCESS)
                return (0);
        }
        rdataset = NULL;
        result = dns_message_findtype (name, dns_rdatatype_ns, 0, &rdataset);
        if (result != ISC_R_SUCCESS)
            continue;
        ...
        if (query->lookup->trace && !query->lookup->trace_root)
        {
            ...
            namereln = dns_name_fullcompare (name, domain, &order, &nlabels);
        ...
        for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
        {
            ...
            if (query->lookup->trace_root && query->lookup->nsfound >= MXSERV)
                break;
            ...
            if (!success)
            {
                success = ISC_TRUE;
                lookup_counter++;
                lookup = requeue_lookup (query->lookup, ISC_FALSE);
                cancel_lookup (query->lookup);
                lookup->doing_xfr = ISC_FALSE;
                if (!lookup->trace_root && section == DNS_SECTION_ANSWER)
            ...
            {
                debug ("couldn't get address for '%s': %s", namestr, isc_result_totext (lresult));
    ...
    {
        fatal ("couldn't get address for '%s': %s", bad_namestr, isc_result_totext (result));
    }

    if (lookup == NULL && section == DNS_SECTION_ANSWER && (query->lookup->trace || query->lookup->ns_search_only))
    ...
    if (numLookups > 1)
    {
        ...
        i = numLookups;
        for (srv = ISC_LIST_HEAD (lookup->my_server_list); srv != NULL; srv = ISC_LIST_HEAD (lookup->my_server_list))
        {
            INSIST (i > 0);
            isc_random_get (&j);
            j %= i;
 ...
 */
static isc_boolean_t next_origin (dig_query_t * query)
{
    dig_lookup_t *lookup;
...
static void insert_soa (dig_lookup_t * lookup)
{
    ...
    isc_buffer_init (&lookup->rdatabuf, lookup->rdatastore, sizeof (lookup->rdatastore));

    result = dns_message_gettemprdata (lookup->sendmsg, &rdata);
    check_result (result, "dns_message_gettemprdata");

    result = dns_rdata_fromstruct (rdata, lookup->rdclass, dns_rdatatype_soa, &soa, &lookup->rdatabuf);
 ...
 */
void setup_lookup (dig_lookup_t * lookup)
{
    isc_result_t result;
    ...
    result = dns_message_create (mctx, DNS_MESSAGE_INTENTRENDER, &lookup->sendmsg);
    ...
    isc_buffer_init (&lookup->namebuf, lookup->namespace, sizeof (lookup->namespace));
    isc_buffer_init (&lookup->onamebuf, lookup->onamespace, sizeof (lookup->onamespace));
     ...
     * TLD.
     */
    mr = idn_encodename (IDN_LOCALCONV | IDN_DELIMMAP, lookup->textname, utf8_textname, sizeof (utf8_textname));
    ...
    {
        mr = idn_encodename (IDN_LOCALCONV | IDN_DELIMMAP, lookup->origin->origin, utf8_origin, sizeof (utf8_origin));
        idn_check_result (mr, "convert origin to UTF-8");
        mr = append_textname (utf8_textname, utf8_origin, sizeof (utf8_textname));
        idn_check_result (mr, "append origin to textname");
    }
    mr = idn_encodename (idnoptions | IDN_LOCALMAP | IDN_NAMEPREP |
                         IDN_IDNCONV | IDN_LENCHECK, utf8_textname, idn_textname, sizeof (idn_textname));
    idn_check_result (mr, "convert UTF-8 textname to IDN encoding");
#else
    if (lookup->origin != NULL)
    {
        debug ("trying origin %s", lookup->origin->origin);
        result = dns_message_gettempname (lookup->sendmsg, &lookup->oname);
        check_result (result, "dns_message_gettempname");
        ...
        result = dns_name_fromtext (lookup->oname, &b, dns_rootname, 0, &lookup->onamebuf);
        if (result != ISC_R_SUCCESS)
        {
            dns_message_puttempname (lookup->sendmsg, &lookup->name);
            dns_message_puttempname (lookup->sendmsg, &lookup->oname);
            fatal ("'%s' is not in legal name syntax (%s)", lookup->origin->origin, isc_result_totext (result));
        }
        ...
        else
        {
            ...
            isc_buffer_add (&b, len);
            result = dns_name_fromtext (lookup->name, &b, lookup->oname, 0, &lookup->namebuf);
        }
        if (result != ISC_R_SUCCESS)
        {
            dns_message_puttempname (lookup->sendmsg, &lookup->name);
            dns_message_puttempname (lookup->sendmsg, &lookup->oname);
            fatal ("'%s' is not in legal name syntax (%s)", lookup->textname, isc_result_totext (result));
        }
        dns_message_puttempname (lookup->sendmsg, &lookup->oname);
    }
    else
#endif
    {
        ...
        else
        {
            ...
            isc_buffer_add (&b, len);
            result = dns_name_fromtext (lookup->name, &b, dns_rootname, 0, &lookup->namebuf);
#else
            len = strlen (lookup->textname);
            isc_buffer_init (&b, lookup->textname, len);
            isc_buffer_add (&b, len);
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            fatal ("'%s' is not a legal name " "(%s)", lookup->textname, isc_result_totext (result));
        }
    }
    ...
    if (lookup->recurse && lookup->rdtype != dns_rdatatype_axfr && lookup->rdtype != dns_rdatatype_ixfr)
    {
    ...
    dns_message_addname (lookup->sendmsg, lookup->name, DNS_SECTION_QUESTION);
    ...
    if ((lookup->rdtype == dns_rdatatype_axfr) || (lookup->rdtype == dns_rdatatype_ixfr))
    {
        /*
    ...
    add_question (lookup->sendmsg, lookup->name, lookup->rdclass, lookup->rdtype);
    ...
    result = dns_message_renderbegin (lookup->sendmsg, &cctx, &lookup->renderbuf);
    ...
    if (lookup->udpsize > 0 || lookup->dnssec || lookup->edns > -1)
    {
        ...
        if (lookup->edns < 0)
            lookup->edns = 0;
        add_opt (lookup->sendmsg, lookup->udpsize, lookup->edns, lookup->dnssec, lookup->nsid);
    }

    result = dns_message_rendersection (lookup->sendmsg, DNS_SECTION_QUESTION, 0);
    check_result (result, "dns_message_rendersection");
    result = dns_message_rendersection (lookup->sendmsg, DNS_SECTION_AUTHORITY, 0);
    check_result (result, "dns_message_rendersection");
    ...
    for (serv = ISC_LIST_HEAD (lookup->my_server_list); serv != NULL; serv = ISC_LIST_NEXT (serv, link))
    {
    ...
    {
        extrabytes = 0;
        printmessage (ISC_LIST_HEAD (lookup->q), lookup->sendmsg, ISC_TRUE);
    }
}
...
static void send_done (isc_task_t * _task, isc_event_t * event)
{
    ...
    for (b = ISC_LIST_HEAD (sevent->bufferlist); b != NULL; b = ISC_LIST_HEAD (sevent->bufferlist))
...
static void cancel_lookup (dig_lookup_t * lookup)
{
    ...
    while (query != NULL)
    {
        ...
        if (query->sock != NULL)
        {
            isc_socket_cancel (query->sock, global_task, ISC_SOCKCANCEL_ALL);
            check_if_done ();
        }
        else
        {
...
static void bringup_timer (dig_query_t * query, unsigned int default_timeout)
{
    ...
    result = isc_timer_create (timermgr, isc_timertype_once, NULL,
                               &l->interval, global_task, connect_timeout, l, &l->timer);
...
static void force_timeout (dig_lookup_t * l, dig_query_t * query)
{
    ...
    event = isc_event_allocate (mctx, query, ISC_TIMEREVENT_IDLE, connect_timeout, l, sizeof (isc_event_t));
    if (event == NULL)
    {
        fatal ("isc_event_allocate: %s", isc_result_totext (ISC_R_NOMEMORY));
    }
...
static void send_tcp_connect (dig_query_t * query)
{
    ...
    if (specified_source && (isc_sockaddr_pf (&query->sockaddr) != isc_sockaddr_pf (&bind_address)))
    {
        printf (";; Skipping server %s, incompatible " "address family\n", query->servname);
        query->waiting_connect = ISC_FALSE;
        next = ISC_LIST_NEXT (query, link);
        l = query->lookup;
        clear_query (query);
    ...
    result = isc_socket_create (socketmgr, isc_sockaddr_pf (&query->sockaddr), isc_sockettype_tcp, &query->sock);
    check_result (result, "isc_socket_create");
    sockcount++;
    debug ("sockcount=%d", sockcount);
    if (specified_source)
        result = isc_socket_bind (query->sock, &bind_address, ISC_SOCKET_REUSEADDRESS);
    else
    {
    ...
    result = isc_socket_connect (query->sock, &query->sockaddr, global_task, connect_done, query);
...
static void send_udp (dig_query_t * query)
{
    ...
    if (!query->recv_made)
    {
        ...
        result = isc_socket_create (socketmgr, isc_sockaddr_pf (&query->sockaddr), isc_sockettype_udp, &query->sock);
        check_result (result, "isc_socket_create");
        sockcount++;
        debug ("sockcount=%d", sockcount);
        if (specified_source)
        ...
        {
            isc_sockaddr_anyofpf (&bind_any, isc_sockaddr_pf (&query->sockaddr));
            result = isc_socket_bind (query->sock, &bind_any, 0);
        }
        check_result (result, "isc_socket_bind");
        ...
        debug ("recving with lookup=%p, query=%p, sock=%p", query->lookup, query, query->sock);
        result = isc_socket_recvv (query->sock, &query->recvlist, 1, global_task, recv_done, query);
    ...
    result = isc_socket_sendtov (query->sock, &query->sendlist, global_task, send_done, query, &query->sockaddr, NULL);
...
static void tcp_length_done (isc_task_t * task, isc_event_t * event)
{
    ...
    result = isc_socket_recvv (query->sock, &query->recvlist, length, task, recv_done, query);
    check_result (result, "isc_socket_recvv");
    recvcount++;
    debug ("resubmitted recv request with length %d, recvcount=%d", length, recvcount);
    isc_event_free (&event);
    UNLOCK_LOOKUP;
}
...
static void launch_next_query (dig_query_t * query, isc_boolean_t include_question)
{
    ...
    result = isc_socket_recvv (query->sock, &query->lengthlist, 0, global_task, tcp_length_done, query);
    check_result (result, "isc_socket_recvv");
    recvcount++;
    debug ("recvcount=%d", recvcount);
    if (!query->first_soa_rcvd)
    {
        ...
        TIME_NOW (&query->time_sent);
        query->waiting_senddone = ISC_TRUE;
        result = isc_socket_sendv (query->sock, &query->sendlist, global_task, send_done, query);
...
static void connect_done (isc_task_t * task, isc_event_t * event)
{
    ...
    if (sevent->result != ISC_R_SUCCESS)
    {
        ...
        debug ("unsuccessful connection: %s", isc_result_totext (sevent->result));
        isc_sockaddr_format (&query->sockaddr, sockstr, sizeof (sockstr));
        if (sevent->result != ISC_R_CANCELED)
            printf (";; Connection to %s(%s) for %s failed: "
                    "%s.\n", sockstr, query->servname, query->lookup->textname, isc_result_totext (sevent->result));
        isc_socket_detach (&query->sock);
        sockcount--;
        INSIST (sockcount >= 0);
    ...
    do
    {
        ...
        for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
        {
            ...
            do
            {
                 ...
                 * it's an SOA
                 */
                if ((!query->first_soa_rcvd) && (rdata.type != dns_rdatatype_soa))
                {
                    puts ("; Transfer failed.  " "Didn't start with SOA answer.");
                    return (ISC_TRUE);
                }
                if ((!query->second_rr_rcvd) && (rdata.type != dns_rdatatype_soa))
                {
                ...
                if (!query->first_soa_rcvd)
                {
                    ...
                    debug ("this is the first serial %u", serial);
                    if (ixfr && isc_serial_ge (ixfr_serial, serial))
                    {
                        debug ("got up to date " "response");
                        goto doexit;
                    }
                    goto next_rdata;
                }
                ...
                if (!query->second_rr_rcvd)
                {
                    ...
                    debug ("this is the second serial %u", serial);
                    query->second_rr_rcvd = ISC_TRUE;
                ...
                if (query->first_rr_serial == serial)
                {
                    ...
                    {
                        query->first_repeat_rcvd = ISC_TRUE;
                        goto next_rdata;
                    }
...
static void recv_done (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        debug ("no longer pending.  Got %s", isc_result_totext (sevent->result));
        query->waiting_connect = ISC_FALSE;
    ...
    if (!l->tcp_mode &&
        !isc_sockaddr_compare (&sevent->address, &query->sockaddr,
                               ...
                               ISC_SOCKADDR_CMPPORT | ISC_SOCKADDR_CMPSCOPE | ISC_SOCKADDR_CMPSCOPEZERO))
    {
        ...
        if ((!isc_sockaddr_eqaddr (&query->sockaddr, &any) &&
            ...
            isc_sockaddr_getport (&query->sockaddr) != isc_sockaddr_getport (&sevent->address))
        {
            isc_sockaddr_format (&sevent->address, buf1, sizeof (buf1));
            isc_sockaddr_format (&query->sockaddr, buf2, sizeof (buf2));
            printf (";; reply from unexpected source: %s," " expected %s\n", buf1, buf2);
            match = ISC_FALSE;
        }
    }
            ...
            if (result == ISC_R_SUCCESS)
            {
                if (!query->first_soa_rcvd || query->warn_id)
                    printf (";; %s: ID mismatch: "
                            "expected ID %u, got %u\n",
                            query->first_soa_rcvd ? "WARNING" : "ERROR", l->sendmsg->id, id);
                if (query->first_soa_rcvd)
                    fail = ISC_FALSE;
    ...
    if (key != NULL)
    {
        if (l->querysig == NULL)
        {
            ...
            result = dns_message_getquerytsig (l->sendmsg, mctx, &l->querysig);
    ...
    if (msg->counts[DNS_SECTION_QUESTION] != 0)
    {
        ...
        for (result = dns_message_firstname (msg, DNS_SECTION_QUESTION);
             result == ISC_R_SUCCESS && match; result = dns_message_nextname (msg, DNS_SECTION_QUESTION))
        {
            dns_name_t *name = NULL;
            ...
            for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
            {
                if (l->rdtype != rdataset->type || l->rdclass != rdataset->rdclass || !dns_name_equal (l->name, name))
                {
                    char namestr[DNS_NAME_FORMATSIZE];
    ...
    if ((msg->flags & DNS_MESSAGEFLAG_TC) != 0 && !l->ignore && !l->tcp_mode)
    {
        ...
        (check_ra && (msg->flags & DNS_MESSAGEFLAG_RA) == 0 && l->recurse))
    {
         ...
         */
        if ((ISC_LIST_HEAD (l->q) != query) || (ISC_LIST_NEXT (query, link) != NULL))
        {
            if (l->comments == ISC_TRUE)
                printf (";; Got %s from %s, "
                        "trying next server\n",
                        msg->rcode == dns_rcode_servfail ?
                        "SERVFAIL reply" : "recursion not available", query->servname);
            clear_query (query);
            check_next_lookup (l);
            dns_message_destroy (&msg);
            isc_event_free (&event);
    ...
    if (l->doing_xfr && l->xfr_q == NULL)
    {
        ...
        if (timeout != INT_MAX && l->timer != NULL)
        {
            ...
            debug ("have local timeout of %d", local_timeout);
            isc_interval_set (&l->interval, local_timeout, 0);
            result = isc_timer_reset (l->timer, isc_timertype_once, NULL, &l->interval, ISC_FALSE);
    ...
    {
        if (msg->rcode != dns_rcode_noerror && (l->origin != NULL || l->need_search))
        {
        ...
        else if (l->trace)
        {
            ...
            if (l->trace_root || (l->ns_search_only && count > 0))
            {
                ...
                n = followup_lookup (msg, query, DNS_SECTION_ANSWER);
                l->trace_root = ISC_FALSE;
            }
            else if (count == 0)
                n = followup_lookup (msg, query, DNS_SECTION_AUTHORITY);
            if (n == 0)
                docancel = ISC_TRUE;
        }
        else
        ...
        if (do_sigchase)
        {
            chase_msg = isc_mem_allocate (mctx, sizeof (dig_message_t));
            if (chase_msg == NULL)
            {
                fatal ("Memory allocation failure in %s:%d", __FILE__, __LINE__);
            }
            ISC_LIST_INITANDAPPEND (chase_message_list, chase_msg, link);
            if (dns_message_create (mctx, DNS_MESSAGE_INTENTPARSE, &msg_temp) != ISC_R_SUCCESS)
            {
                fatal ("dns_message_create in %s:%d", __FILE__, __LINE__);
            }
            ...
            chase_msg2 = isc_mem_allocate (mctx, sizeof (dig_message_t));
            if (chase_msg2 == NULL)
            {
                fatal ("Memory allocation failure in %s:%d", __FILE__, __LINE__);
            }
            ISC_LIST_INITANDAPPEND (chase_message_list2, chase_msg2, link);
            chase_msg2->msg = msg;
        }
#endif
    }
    ...
    else
    {
        ...
        if (!query->lookup->ns_search_only || query->lookup->trace_root || docancel)
        {
#ifdef DIG_SIGCHASE
            if (!do_sigchase)
#endif
 ...
 */
isc_result_t get_address (char *host, in_port_t port, isc_sockaddr_t * sockaddr)
{
    int count;
...
int getaddresses (dig_lookup_t * lookup, const char *host, isc_result_t * resultp)
{
    ...
    result = bind9_getaddresses (host, 0, sockaddrs, DIG_MAX_ADDRESSES, &count);
    if (resultp != NULL)
        *resultp = result;
    if (result != ISC_R_SUCCESS)
    {
        if (resultp == NULL)
            fatal ("couldn't get address for '%s': %s", host, isc_result_totext (result));
        return 0;
    }
...
void cancel_all (void)
{
    ...
    if (current_lookup != NULL)
    {
        ...
        while (q != NULL)
        {
            debug ("canceling query %p, belonging to %p", q, current_lookup);
...
static void initialize_idn (void)
{
    ...
    r = idn_nameinit (1);
    if (r != idn_success)
        fatal ("idn api initialization failed: %s", idn_result_tostring (r));
...
void dump_database_section (dns_message_t * msg, int section)
{
    ...
    do
    {
        ...
        for (rdataset = ISC_LIST_HEAD (msg_name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
        {
...
void dump_database (void)
{
    ...
    for (msg = ISC_LIST_HEAD (chase_message_list); msg != NULL; msg = ISC_LIST_NEXT (msg, link))
    {
        if (dns_message_firstname (msg->msg, DNS_SECTION_ANSWER) == ISC_R_SUCCESS)
            dump_database_section (msg->msg, DNS_SECTION_ANSWER);

        if (dns_message_firstname (msg->msg, DNS_SECTION_AUTHORITY) == ISC_R_SUCCESS)
            dump_database_section (msg->msg, DNS_SECTION_AUTHORITY);

        if (dns_message_firstname (msg->msg, DNS_SECTION_ADDITIONAL) == ISC_R_SUCCESS)
...
dns_rdataset_t *search_type (dns_name_t * name, dns_rdatatype_t type, dns_rdatatype_t covers)
{
    ...
    for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
    {
        ...
        else if ((type == dns_rdatatype_rrsig) && (rdataset->type == dns_rdatatype_rrsig))
        {
            result = dns_rdataset_first (rdataset);
            ...
            if ((siginfo.covered == covers) || (covers == dns_rdatatype_any))
            {
...
dns_rdataset_t *chase_scanname_section (dns_message_t * msg, dns_name_t * name,
                                        dns_rdatatype_t type, dns_rdatatype_t covers, int section)
{
    dns_rdataset_t *rdataset;
...
dns_rdataset_t *chase_scanname (dns_name_t * name, dns_rdatatype_t type, dns_rdatatype_t covers)
{
    dns_rdataset_t *rdataset = NULL;

    dig_message_t *msg;

    for (msg = ISC_LIST_HEAD (chase_message_list2); msg != NULL; msg = ISC_LIST_NEXT (msg, link))
    {
        if (dns_message_firstname (msg->msg, DNS_SECTION_ANSWER) == ISC_R_SUCCESS)
            rdataset = chase_scanname_section (msg->msg, name, type, covers, DNS_SECTION_ANSWER);
        if (rdataset != NULL)
            return (rdataset);
        if (dns_message_firstname (msg->msg, DNS_SECTION_AUTHORITY) == ISC_R_SUCCESS)
            rdataset = chase_scanname_section (msg->msg, name, type, covers, DNS_SECTION_AUTHORITY);
        if (rdataset != NULL)
            return (rdataset);
        if (dns_message_firstname (msg->msg, DNS_SECTION_ADDITIONAL) == ISC_R_SUCCESS)
            rdataset = chase_scanname_section (msg->msg, name, type, covers, DNS_SECTION_ADDITIONAL);
        if (rdataset != NULL)
            return (rdataset);
    }
...
dns_rdataset_t *sigchase_scanname (dns_rdatatype_t type, dns_rdatatype_t covers,
...
void insert_trustedkey (dst_key_t ** keyp)
{
    if (*keyp == NULL)
        return;
...
isc_result_t get_trusted_key (isc_mem_t * mctx)
{
    ...
    while (fgets (buf, sizeof (buf), fp) != NULL)
    {
        ...
        fclose (fptemp);
        result = dst_key_fromnamedfile (filetemp, NULL, DST_TYPE_PUBLIC, mctx, &key);
...
static void nameFromString (const char *str, dns_name_t * p_ret)
{
    ...
    result = dns_name_fromtext (dns_fixedname_name (&fixedname), &buffer, dns_rootname, DNS_NAME_DOWNCASE, NULL);
...
isc_result_t prepare_lookup (dns_name_t * name)
{
    ...
    for (result = dns_rdataset_first (chase_nsrdataset);
         result == ISC_R_SUCCESS; result = dns_rdataset_next (chase_nsrdataset))
    {
        char namestr[DNS_NAME_FORMATSIZE];
...
#define __FOLLOW_GLUE__
        ...
        result = advanced_rrsearch (&rdataset, &ns.name, dns_rdatatype_aaaa, dns_rdatatype_any, &true);
        if (result == ISC_R_SUCCESS)
        {
            for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
            {
                ...
                isc_buffer_usedregion (b, &r);
                r.base[r.length] = '\0';
                strncpy (namestr, (char *) r.base, DNS_NAME_FORMATSIZE);
                isc_buffer_free (&b);
        ...
        rdataset = NULL;
        result = advanced_rrsearch (&rdataset, &ns.name, dns_rdatatype_a, dns_rdatatype_any, &true);
        if (result == ISC_R_SUCCESS)
        {
            for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
            {
...
isc_result_t child_of_zone (dns_name_t * name, dns_name_t * zone_name, dns_name_t * child_name)
{
    ...
    if (name_reln != dns_namereln_subdomain || dns_name_countlabels (name) <= dns_name_countlabels (zone_name) + 1)
    {
        printf ("\n;; ERROR : ");
    ...
    dns_name_getlabelsequence (name,
                               ...
                               dns_name_countlabels (zone_name) - 1, dns_name_countlabels (zone_name) + 1, child_name);
    return (ISC_R_SUCCESS);
}
...
isc_result_t initialization (dns_name_t * name)
{
    ...
    result = advanced_rrsearch (&chase_nsrdataset, name, dns_rdatatype_ns, dns_rdatatype_any, &true);
    if (result != ISC_R_SUCCESS)
    {
 ...
 */
isc_result_t
contains_trusted_key (dns_name_t * name, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset, isc_mem_t * mctx)
{
    isc_result_t result;
    ...
    do
    {
        ...
        result = dns_dnssec_keyfromrdata (name, &rdata, mctx, &dnsseckey);
        check_result (result, "dns_dnssec_keyfromrdata");


        for (i = 0; i < tk_list.nb_tk; i++)
        {
            if (dst_key_compare (tk_list.key[i], dnsseckey) == ISC_TRUE)
            {
                dns_rdata_reset (&rdata);

                printf (";; Ok, find a Trusted Key in the " "DNSKEY RRset: %d\n", dst_key_id (dnsseckey));
                if (sigchase_verify_sig_key (name, rdataset, dnsseckey, sigrdataset, mctx) == ISC_R_SUCCESS)
                {
                     ...
                     dns_rdataset_t * keyrdataset, dns_rdataset_t * sigrdataset, isc_mem_t * mctx)
{
    ...
    do
    {
        ...
        result = dns_dnssec_keyfromrdata (name, &keyrdata, mctx, &dnsseckey);
        check_result (result, "dns_dnssec_keyfromrdata");

        result = sigchase_verify_sig_key (name, rdataset, dnsseckey, sigrdataset, mctx);
        if (result == ISC_R_SUCCESS)
        {
                         ...
                         dst_key_t * dnsseckey, dns_rdataset_t * sigrdataset, isc_mem_t * mctx)
{
    ...
    do
    {
        ...
        if (siginfo.keyid == dst_key_id (dnsseckey))
        {
            ...
            result = dns_dnssec_verify (name, rdataset, dnsseckey, ISC_FALSE, mctx, &sigrdata);

            printf (";; VERIFYING ");
            print_type (rdataset->type);
            printf (" RRset for ");
            dns_name_print (name, stdout);
            printf (" with DNSKEY:%d: %s\n", dst_key_id (dnsseckey), isc_result_totext (result));
...
isc_result_t
sigchase_verify_ds (dns_name_t * name, dns_rdataset_t * keyrdataset, dns_rdataset_t * dsrdataset, isc_mem_t * mctx)
{
    isc_result_t result;
    ...
    do
    {
        ...
        do
        {
            ...
            if (dsinfo.key_tag == dst_key_id (dnsseckey))
            {
                ...
                result = dns_ds_buildrdata (name, &keyrdata, dsinfo.digest_type, dsbuf, &newdsrdata);
                ...
                if (result != ISC_R_SUCCESS)
                {
                    ...
                    dst_key_free (&dnsseckey);
                    dns_rdata_freestruct (&dsinfo);
                    printf ("Oops: impossible to build" " new DS rdata\n");
                    return (result);
                }
                ...
                {
                    printf (";; OK a DS valids a DNSKEY" " in the RRset\n");
                    printf (";; Now verify that this" " DNSKEY validates the " "DNSKEY RRset\n");

                    result = sigchase_verify_sig_key (name, keyrdataset, dnsseckey, chase_sigkeyrdataset, mctx);
                    if (result == ISC_R_SUCCESS)
                    {
                        dns_rdata_reset (&keyrdata);
                        dns_rdata_reset (&newdsrdata);
                ...
                {
                    printf (";; This DS is NOT the DS for" " the chasing KEY: FAILED\n");
                }
                   ...
                   dns_rdatatype_t type, dns_rdatatype_t covers, isc_boolean_t * lookedup)
{
    ...
    if ((*rdataset = sigchase_scanname (type, covers, lookedup, name)) == NULL)
    {
        if (tmplookedup)
            return (ISC_R_FAILURE);
        return (ISC_R_NOTFOUND);
...
void sigchase_td (dns_message_t * msg)
{
    ...
    if ((result = dns_message_firstname (msg, DNS_SECTION_ANSWER)) == ISC_R_SUCCESS)
    {
        dns_message_currentname (msg, DNS_SECTION_ANSWER, &name);
    ...
    else
    {
        if (!current_lookup->trace_root_sigchase)
        {
            result = dns_message_firstname (msg, DNS_SECTION_AUTHORITY);
            if (result == ISC_R_SUCCESS)
                dns_message_currentname (msg, DNS_SECTION_AUTHORITY, &name);
            chase_nsrdataset
                = chase_scanname_section (msg, name, dns_rdatatype_ns, dns_rdatatype_any, DNS_SECTION_AUTHORITY);
            dup_name (name, &chase_authority_name, mctx);
            if (chase_nsrdataset != NULL)
            {
                have_delegation_ns = ISC_TRUE;
                printf ("no response but there is a delegation" " in authority section:");
                dns_name_print (name, stdout);
                printf ("\n");
            }
            else
            {
                printf ("no response and no delegation in " "authority section but a reference" " to: ");
                dns_name_print (name, stdout);
                printf ("\n");
                error_message = msg;
            }
    ...
    if (have_answer)
    {
        chase_rdataset
            = chase_scanname_section (msg, &chase_name,
                                      current_lookup->rdtype_sigchase, dns_rdatatype_any, DNS_SECTION_ANSWER);
        if (chase_rdataset != NULL)
            have_response = ISC_TRUE;
    }

    result = advanced_rrsearch (&chase_keyrdataset,
                                &chase_current_name, dns_rdatatype_dnskey, dns_rdatatype_any, &chase_keylookedup);
    ...
    result = advanced_rrsearch (&chase_sigkeyrdataset,
                                &chase_current_name, dns_rdatatype_rrsig, dns_rdatatype_dnskey, &chase_sigkeylookedup);
    if (result == ISC_R_FAILURE)
    {
    ...
    if (!chase_dslookedup && !chase_nslookedup)
    {
        ...
        {
            result = contains_trusted_key (&chase_current_name, chase_keyrdataset, chase_sigkeyrdataset, mctx);
        }
        else
        {
            INSIST (chase_dsrdataset != NULL);
            INSIST (chase_sigdsrdataset != NULL);
            result = sigchase_verify_ds (&chase_current_name, chase_keyrdataset, chase_dsrdataset, mctx);
        }
    ...
    if (have_response || (!have_delegation_ns && !have_response))
    {
        ...
        if (have_response)
        {
            result = advanced_rrsearch (&chase_sigrdataset,
                                        &chase_name, dns_rdatatype_rrsig, current_lookup->rdtype_sigchase, &true);
            if (result == ISC_R_FAILURE)
            {
        ...
        {
            result = advanced_rrsearch (&chase_sigrdataset,
                                        &chase_authority_name, dns_rdatatype_rrsig, dns_rdatatype_any, &true);
            if (result == ISC_R_FAILURE)
            {
        ...
        result = grandfather_pb_test (&chase_current_name, chase_sigrdataset);
        if (result != ISC_R_SUCCESS)
        {
            dns_name_t tmp_name;
            ...
            result = child_of_zone (&chase_name, &chase_current_name, &tmp_name);
            if (dns_name_dynamic (&chase_authority_name))
                free_name (&chase_authority_name, mctx);
            dup_name (&tmp_name, &chase_authority_name, mctx);
            printf (";; and we try to continue chain of trust" " validation of the zone: ");
    ...
    if (have_delegation_ns)
    {
        ...
        result = advanced_rrsearch (&chase_nsrdataset,
                                    &chase_authority_name, dns_rdatatype_ns, dns_rdatatype_any, &chase_nslookedup);
        if (result == ISC_R_FAILURE)
        {
        ...
        result = advanced_rrsearch (&chase_dsrdataset,
                                    &chase_authority_name, dns_rdatatype_ds, dns_rdatatype_any, &chase_dslookedup);
        if (result == ISC_R_FAILURE)
        {
        ...
        result = advanced_rrsearch (&chase_sigdsrdataset,
                                    &chase_authority_name, dns_rdatatype_rrsig, dns_rdatatype_ds, &true);
        if (result != ISC_R_SUCCESS)
        {
            printf ("\n;; DSset is missing to continue validation:" " FAILED\n\n");
            goto cleanandgo;
        ...
        printf ("\n;; RRSIGset of DSset\n");
        print_rdataset (&chase_authority_name, chase_sigdsrdataset, mctx);
        INSIST (chase_sigdsrdataset != NULL);

        result = sigchase_verify_sig (&chase_authority_name,
                                      chase_dsrdataset, chase_keyrdataset, chase_sigdsrdataset, mctx);
        if (result != ISC_R_SUCCESS)
        {
    ...
    if (error_message != NULL)
    {
        ...
        result = prove_nx (error_message, &chase_name,
                           ...
                           current_lookup->rdtype_sigchase, &rdata_name, &rdataset, &sigrdataset);
        if (rdataset == NULL || sigrdataset == NULL || dns_name_countlabels (&rdata_name) == 0)
        {
            printf ("\n;; Impossible to verify the non-existence," " the NSEC RRset can't be validated:" " FAILED\n\n");
            goto cleanandgo;
        }
        ret = sigchase_verify_sig (&rdata_name, rdataset, chase_keyrdataset, sigrdataset, mctx);
  ...
  finalstep:
    result = advanced_rrsearch (&chase_rdataset, &chase_name,
                                current_lookup->rdtype_sigchase, dns_rdatatype_any, &true);
    if (result == ISC_R_FAILURE)
    {
        printf ("\n;; RRsig of RRset is missing to continue validation" " SHOULD NOT APPEND: FAILED\n\n");
        goto cleanandgo;
    }
    result = sigchase_verify_sig (&chase_name, chase_rdataset, chase_keyrdataset, chase_sigrdataset, mctx);
    if (result != ISC_R_SUCCESS)
    {
...
isc_result_t getneededrr (dns_message_t * msg)
{
    ...
    if (chase_rdataset == NULL)
    {
        result = advanced_rrsearch (&chase_rdataset, name, dns_rdatatype_any, dns_rdatatype_any, &true);
        if (result != ISC_R_SUCCESS)
        {
    ...
    if (chase_sigrdataset == NULL)
    {
        result = advanced_rrsearch (&chase_sigrdataset, name,
                                    dns_rdatatype_rrsig, chase_rdataset->type, &chase_siglookedup);
        if (result == ISC_R_FAILURE)
        {
    ...
    {
        result = advanced_rrsearch (&chase_keyrdataset,
                                    &chase_signame, dns_rdatatype_dnskey, dns_rdatatype_any, &chase_keylookedup);
        if (result == ISC_R_FAILURE)
        {
            printf ("\n;; DNSKEY is missing to continue validation:" " FAILED\n\n");
            free_name (&chase_signame, mctx);
    ...
    {
        result = advanced_rrsearch (&chase_sigkeyrdataset,
                                    &chase_signame, dns_rdatatype_rrsig, dns_rdatatype_dnskey, &chase_sigkeylookedup);
        if (result == ISC_R_FAILURE)
        {
    ...
    if (chase_dsrdataset == NULL)
    {
        result = advanced_rrsearch (&chase_dsrdataset, &chase_signame,
                                    dns_rdatatype_ds, dns_rdatatype_any, &chase_dslookedup);
        if (result == ISC_R_FAILURE)
        {
    ...
    if (chase_dsrdataset != NULL)
    {
         ...
         */
        result = advanced_rrsearch (&chase_sigdsrdataset, &chase_signame, dns_rdatatype_rrsig, dns_rdatatype_ds, &true);
        if (result == ISC_R_FAILURE)
        {
            printf (";; WARNING : NO RRSIG DS : RRSIG DS" " should come with DS\n");
            /*
        ...
        else
        {
            ...
            print_rdataset (&chase_signame, chase_sigdsrdataset, mctx);
        }
    }
    return (1);
}
...
void sigchase_bu (dns_message_t * msg)
{
    ...
    if (ret == ISC_R_ADDRNOTAVAIL)
    {
        ...
        result = prove_nx (msg, &query_name, current_lookup->rdclass,
                           current_lookup->rdtype, &rdata_name, &rdataset, &sigrdataset);
        free_name (&query_name, mctx);
        if (rdataset == NULL || sigrdataset == NULL || dns_name_countlabels (&rdata_name) == 0)
        {
            printf ("\n;; Impossible to verify the Non-existence," " the NSEC RRset can't be validated: " "FAILED\n\n");
        ...
        printf (";; An NSEC prove the non-existence of a answers," " Now we want validate this NSEC\n");
    ...
    if (result != ISC_R_SUCCESS)
    {
        free_name (&chase_name, mctx);
        free_name (&chase_signame, mctx);
        printf (";; No DNSKEY is valid to check the RRSIG" " of the RRset: FAILED\n");
        clean_trustedkey ();
        return;
    }
    printf (";; OK We found DNSKEY (or more) to validate the RRset\n");

    result = contains_trusted_key (&chase_signame, chase_keyrdataset, chase_sigkeyrdataset, mctx);
    if (result == ISC_R_SUCCESS)
    {
        free_name (&chase_name, mctx);
        free_name (&chase_signame, mctx);
    ...
    if (chase_dsrdataset == NULL)
    {
        free_name (&chase_name, mctx);
        free_name (&chase_signame, mctx);
        printf (";; the DNSKEY isn't trusted-key and there isn't" " DS to validate the DNSKEY: FAILED\n");
        clean_trustedkey ();
        return;
    }

    result = sigchase_verify_ds (&chase_signame, chase_keyrdataset, chase_dsrdataset, mctx);
    if (result != ISC_R_SUCCESS)
    {
        free_name (&chase_signame, mctx);
        free_name (&chase_name, mctx);
        printf (";; ERROR no DS validates a DNSKEY in the" " DNSKEY RRset: FAILED\n");
        clean_trustedkey ();
        return;
    }
    else
...
prove_nx_domain (dns_message_t * msg,
                 dns_name_t * name, dns_name_t * rdata_name, dns_rdataset_t ** rdataset, dns_rdataset_t ** sigrdataset)
{
    isc_result_t ret = ISC_R_FAILURE;
    ...
    if ((result = dns_message_firstname (msg, DNS_SECTION_AUTHORITY)) != ISC_R_SUCCESS)
    {
        printf (";; nothing in authority section : impossible to" " validate the non-existence : FAILED\n");
        return (ISC_R_FAILURE);
    }
    ...
    do
    {
        ...
        nsecset = search_type (nsecname, dns_rdatatype_nsec, dns_rdatatype_any);
        if (nsecset == NULL)
            continue;

        printf ("There is a NSEC for this zone in the" " AUTHORITY section:\n");
        print_rdataset (nsecname, nsecset, mctx);

        for (result = dns_rdataset_first (nsecset); result == ISC_R_SUCCESS; result = dns_rdataset_next (nsecset))
        {
            ...
            signsecset
                = chase_scanname_section (msg, nsecname,
                                          dns_rdatatype_rrsig, dns_rdatatype_nsec, DNS_SECTION_AUTHORITY);
            if (signsecset == NULL)
            {
                printf (";; no RRSIG NSEC in authority section:"
                        " impossible to validate the " "non-existence: FAILED\n");
                return (ISC_R_FAILURE);
            }
            ...
            if ((inf_name (nsecname, &nsecstruct.next) == 1 &&
                ...
                (inf_name (name, nsecname) == 1 && inf_name (&nsecstruct.next, name) == 1))
            {
    ...
    while (dns_message_nextname (msg, DNS_SECTION_AUTHORITY) == ISC_R_SUCCESS);
               ...
               dns_name_t * rdata_name, dns_rdataset_t ** rdataset, dns_rdataset_t ** sigrdataset)
{
    ...
    signsecset = chase_scanname_section (msg, name, dns_rdatatype_rrsig, dns_rdatatype_nsec, DNS_SECTION_AUTHORITY);
    if (signsecset == NULL)
    {
...
prove_nx (dns_message_t * msg, dns_name_t * name, dns_rdataclass_t class,
          dns_rdatatype_t type, dns_name_t * rdata_name, dns_rdataset_t ** rdataset, dns_rdataset_t ** sigrdataset)
{
    isc_result_t ret;

    dns_rdataset_t *nsecset = NULL;

    printf ("We want to prove the non-existence of a type of rdata %d" " or of the zone: \n", type);

    if ((ret = dns_message_firstname (msg, DNS_SECTION_AUTHORITY)) != ISC_R_SUCCESS)
    {
        printf (";; nothing in authority section : impossible to" " validate the non-existence : FAILED\n");
        return (ISC_R_FAILURE);
    }

    nsecset = chase_scanname_section (msg, name, dns_rdatatype_nsec, dns_rdatatype_any, DNS_SECTION_AUTHORITY);
    if (nsecset != NULL)
    {
        printf ("We have a NSEC for this zone :OK\n");
        ret = prove_nx_type (msg, name, nsecset, class, type, rdata_name, rdataset, sigrdataset);
        if (ret != ISC_R_SUCCESS)
        {
    ...
    {
        printf ("there is no NSEC for this zone: validating " "that the zone doesn't exist\n");
        ret = prove_nx_domain (msg, name, rdata_name, rdataset, sigrdataset);
        return (ret);
    }
    /* Never get here */
}
