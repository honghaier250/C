...
ISC_LANG_BEGINDECLS typedef struct dig_lookup dig_lookup_t;
...
/*% The dig_lookup structure */
     struct dig_lookup
     {
         ...
         waiting_connect, doing_xfr, ns_search_only,    /*%< dig +nssearch, host -C */
             identify,            /*%< Append an "on server <foo>" message */
             identify_previous_line,    /*% Prepend a "Nameserver <foo>:"
                                           message, with newline and tab */
         ignore, recurse, aaonly, adflag, cdflag, trace,    /*% dig +trace */
             trace_root,        /*% initial query for either +trace or +nssearch */
         tcp_mode, ip6_int, comments, stats, section_question, section_answer, section_authority, section_additional, servfail_stops, new_search, need_search, done_as_is, besteffort, dnssec, nsid;    /*% Name Server ID (RFC 5001) */
#ifdef DIG_SIGCHASE
         isc_boolean_t sigchase;
#if DIG_SIGCHASE_TD
         isc_boolean_t do_topdown, trace_root_sigchase, rdtype_sigchaseset, rdclass_sigchaseset;
...
/*% The dig_query structure */
     struct dig_query
     {
         ...
         isc_boolean_t waiting_connect,
             pending_free,
             waiting_senddone, first_pass, first_soa_rcvd, second_rr_rcvd, first_repeat_rcvd, recv_made, warn_id;
         ...
         char *servname;
         char *userarg;
         isc_bufferlist_t sendlist, recvlist, lengthlist;
         isc_buffer_t recvbuf, lengthbuf, slbuf;
         char *recvspace, lengthspace[4], slspace[4];
         isc_socket_t *sock;
 ...
 */
     ...
     extern isc_boolean_t check_ra, have_ipv4, have_ipv6, specified_source, usesearch, showsearch, qr;
...
isc_result_t get_reverse (char *reverse, size_t len, char *value, isc_boolean_t ip6_int, isc_boolean_t strict);
     ...
     ISC_PLATFORM_NORETURN_PRE void fatal (const char *format, ...) ISC_FORMAT_PRINTF (1, 2) ISC_PLATFORM_NORETURN_POST;

     void debug (const char *format, ...) ISC_FORMAT_PRINTF (1, 2);
...
/* Chasing functions */
     ...
     void chase_sig (dns_message_t * msg);
#endif
