...
ISC_PLATFORM_NORETURN_PRE static void show_usage (void) ISC_PLATFORM_NORETURN_POST;

     static void show_usage (void)
{
    fputs ("Usage: host [-aCdlriTwv] [-c class] [-N ndots] [-t type] [-W time]\n"
...
printsection (dns_message_t * msg, dns_section_t sectionid,
              const char *section_name, isc_boolean_t headers, dig_query_t * query)
{
        ...
        for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
        {
                ...
                !((!list_addresses &&
                  ...
                  (list_addresses &&
                   (rdataset->type == dns_rdatatype_a ||
                    ...
                    rdataset->type == dns_rdatatype_ns || rdataset->type == dns_rdatatype_ptr))))
                continue;
            if (!short_form)
            {
                result = dns_rdataset_totext (rdataset, print_name, ISC_FALSE, no_rdata, &target);
            ...
            else
            {
                ...
                while (loopresult == ISC_R_SUCCESS)
                {
                    ...
                    char typebuf2[DNS_RDATATYPE_FORMATSIZE + 20];
                    ...
                    snprintf (typebuf2, sizeof (typebuf2), "has %s record", typebuf);
                    rtt = typebuf2;
                  found:
                    say_message (print_name, rtt, &rdata, query);
...
static void chase_cnamechain (dns_message_t * msg, dns_name_t * qname)
{
    ...
    {
        rdataset = NULL;
        result = dns_message_findname (msg, DNS_SECTION_ANSWER, qname, dns_rdatatype_cname, 0, NULL, &rdataset);
        if (result != ISC_R_SUCCESS)
            return;
...
isc_result_t printmessage (dig_query_t * query, dns_message_t * msg, isc_boolean_t headers)
{
    ...
    if (msg->rcode != 0)
    {
        ...
        printf ("Host %s not found: %d(%s)\n",
                ...
                query->lookup->textname, msg->rcode, rcode_totext (msg->rcode));
        return (ISC_R_SUCCESS);
    }
    ...
    if (default_lookups && query->lookup->rdtype == dns_rdatatype_a)
    {
        ...
        if (lookup != NULL)
        {
            strncpy (lookup->textname, namestr, sizeof (lookup->textname));
    ...
    if (!short_form)
    {
        ...
        if (opt != NULL)
            printf (";; EDNS: version: %u, udp=%u\n",
                    (unsigned int) ((opt->ttl & 0x00ff0000) >> 16), (unsigned int) opt->rdclass);
        tsigname = NULL;
    ...
    {
        printf ("\n");
        result = printsection (msg, DNS_SECTION_QUESTION, "QUESTION", ISC_TRUE, query);
        if (result != ISC_R_SUCCESS)
            return (result);
    }
    if (!ISC_LIST_EMPTY (msg->sections[DNS_SECTION_ANSWER]))
    {
        if (!short_form)
            printf ("\n");
        result = printsection (msg, DNS_SECTION_ANSWER, "ANSWER", ISC_TF (!short_form), query);
        if (result != ISC_R_SUCCESS)
            return (result);
    }

    if (!ISC_LIST_EMPTY (msg->sections[DNS_SECTION_AUTHORITY]) && !short_form)
    {
        printf ("\n");
    ...
    if (!ISC_LIST_EMPTY (msg->sections[DNS_SECTION_ADDITIONAL]) && !short_form)
    {
        printf ("\n");
    ...
    {
        printf ("\n");
        result = printrdata (msg, tsig, tsigname, "PSEUDOSECTION TSIG", ISC_TRUE);
        if (result != ISC_R_SUCCESS)
            return (result);
    }
    if (!short_form)
    ...
    if (short_form && !default_lookups && ISC_LIST_EMPTY (msg->sections[DNS_SECTION_ANSWER]))
    {
        char namestr[DNS_NAME_FORMATSIZE];
        ...
        dns_name_format (query->lookup->name, namestr, sizeof (namestr));
        dns_rdatatype_format (query->lookup->rdtype, typestr, sizeof (typestr));
    ...
    while ((c = isc_commandline_parse (argc, argv, optstring)) != -1)
    {
        switch (c)
        {
            case 'm':
                ...
                else if (!strcasecmp ("record", isc_commandline_argument) == 0)
                    isc_mem_debugging |= ISC_MEM_DEBUGRECORD;
                else if (strcasecmp ("usage", isc_commandline_argument) == 0)
...
static void parse_args (isc_boolean_t is_batchfile, int argc, char **argv)
{
    ...
    while ((c = isc_commandline_parse (argc, argv, optstring)) != -1)
    {
        switch (c)
        {
            ...
            case 'r':
                ...
                break;
            case 't':
                if (strncasecmp (isc_commandline_argument, "ixfr=", 5) == 0)
                {
                    rdtype = dns_rdatatype_ixfr;
                    /* XXXMPA add error checking */
                    serial = strtoul (isc_commandline_argument + 5, NULL, 10);
                ...
                if (!lookup->rdtypeset || lookup->rdtype != dns_rdatatype_axfr)
                ...
                else if (rdtype == dns_rdatatype_a || rdtype == dns_rdatatype_aaaa || rdtype == dns_rdatatype_mx)
                {
    ...
    if (get_reverse (store, sizeof (store), hostname, lookup->ip6_int, ISC_TRUE) == ISC_R_SUCCESS)
    {
