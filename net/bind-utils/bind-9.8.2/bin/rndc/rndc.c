...
ISC_PLATFORM_NORETURN_PRE static void usage (int status) ISC_PLATFORM_NORETURN_POST;
...
static void get_addresses (const char *host, in_port_t port)
{
    ...
    if (*host == '/')
    {
        result = isc_sockaddr_frompath (&serveraddrs[nserveraddrs], host);
    ...
    else
    {
        ...
        result = bind9_getaddresses (host, port, &serveraddrs[nserveraddrs], count, &found);
        nserveraddrs += found;
    }
    if (result != ISC_R_SUCCESS)
...
static void rndc_recvnonce (isc_task_t * task, isc_event_t * event)
{
    ...
    DO ("create message", isccc_cc_createmessage (1, NULL, NULL, ++serial, now, now + 60, &request));
    ...
    DO ("schedule recv", isccc_ccmsg_readmessage (&ccmsg, task, rndc_recvdone, NULL));
    recvs++;
    DO ("send message", isc_socket_send (sock, &r, task, rndc_senddone, NULL));
    sends++;
...
static void rndc_connected (isc_task_t * task, isc_event_t * event)
{
    ...
    if (sevent->result != ISC_R_SUCCESS)
    {
        isc_sockaddr_format (&serveraddrs[currentaddr], socktext, sizeof (socktext));
        if (sevent->result != ISC_R_CANCELED && ++currentaddr < nserveraddrs)
        {
            notify ("connection failed: %s: %s", socktext, isc_result_totext (sevent->result));
            isc_socket_detach (&sock);
            isc_event_free (&event);
            ...
            fatal ("connect failed: %s: %s", socktext, isc_result_totext (sevent->result));
    }
    ...
    DO ("schedule recv", isccc_ccmsg_readmessage (&ccmsg, task, rndc_recvnonce, NULL));
    recvs++;
    DO ("send message", isc_socket_send (sock, &r, task, rndc_senddone, NULL));
    sends++;
    isc_event_free (&event);
...
static void rndc_startconnect (isc_sockaddr_t * addr, isc_task_t * task)
{
    ...
    DO ("connect", isc_socket_connect (sock, addr, task, rndc_connected, NULL));
    connects++;
}
...
static void
parse_config (isc_mem_t * mctx, isc_log_t * log, const char *keyname, cfg_parser_t ** pctxp, cfg_obj_t ** configp)
{
    isc_result_t result;
    ...
    if (!isc_file_exists (conffile))
    {
        ...
        if (!isc_file_exists (conffile))
            fatal ("neither %s nor %s was found", admin_conffile, admin_keyfile);
        key_only = ISC_TRUE;
    }
    ...
    if (!key_only)
    {
        ...
        if (servers != NULL)
        {
            for (elt = cfg_list_first (servers); elt != NULL; elt = cfg_list_next (elt))
            {
                const char *name;
    ...
    {
        DO ("get default key", cfg_map_get (options, "default-key", &defkey));
        keyname = cfg_obj_asstring (defkey);
    }
    else if (!key_only)
    ...
    else
    {
        ...
        for (elt = cfg_list_first (keys); elt != NULL; elt = cfg_list_next (elt))
        {
            ...
            if (strcasecmp (cfg_obj_asstring (cfg_map_getname (key)), keyname) == 0)
                break;
        }
        if (elt == NULL)
        ...
        for (element = cfg_list_first (addresses); element != NULL; element = cfg_list_next (element))
        {
            ...
            if (!cfg_obj_issockaddr (address))
            {
                ...
                if (cfg_obj_isuint32 (obj))
                {
                    ...
                    if (myport > ISC_UINT16_MAX || myport == 0)
                        fatal ("port %u out of range", myport);
                }
                else
                    myport = remoteport;
                if (nserveraddrs < SERVERADDRS)
                    ...
                    fprintf (stderr, "too many address: " "%s: dropped\n", name);
                continue;
            }
            ...
            else
            {
                ...
                isc_sockaddr_format (&sa, socktext, sizeof (socktext));
                fprintf (stderr, "too many address: %s: dropped\n", socktext);
            }
        }
    }
...
int main (int argc, char **argv)
{
    ...
    while ((ch = isc_commandline_parse (argc, argv, "b:c:hk:Mmp:s:Vy:")) != -1)
    {
        switch (ch)
        {
            case 'b':
                if (inet_pton (AF_INET, isc_commandline_argument, &in) == 1)
                {
                    isc_sockaddr_fromin (&local4, &in, 0);
                    local4set = ISC_TRUE;
                }
                else if (inet_pton (AF_INET6, isc_commandline_argument, &in6) == 1)
                {
            ...
            case 'p':
                ...
                if (remoteport > 65535 || remoteport == 0)
                    fatal ("port '%s' out of range", isc_commandline_argument);
                break;
    ...
    DO ("creating log channel",
        isc_log_createchannel (logconfig, "stderr",
                               ISC_LOG_TOFILEDESC, ISC_LOG_INFO, &logdest, ISC_LOG_PRINTTAG | ISC_LOG_PRINTLEVEL));
    DO ("enabling log channel", isc_log_usechannel (logconfig, "stderr", NULL, NULL));
