...
static OUT rfc4634_1_sha256 = {
    "B0344C61D8DB38535CA8AFCEAF0BF12B881DC200C9833DA726E9376C2E32" "CFF7",
    ISC_SHA256_DIGESTLENGTH
};

static OUT rfc4634_1_sha384 = {
    "AFD03944D84895626B0825F4AB46907F15F9DADBE4101EC682AA034C7CEB" "C59CFAEA9EA9076EDE7F4AF152E8B2FA9CB6",
    ISC_SHA384_DIGESTLENGTH
};

static OUT rfc4634_1_sha512 = {
    "87AA7CDEA5EF619D4FF0B4241A1D6CB02379F4E2CE4EC2787AD0B30545E1"
        "7CDEDAA833B7D6B8A702038B274EAEA3F4E4BE9D914EEB61F1702E696C20" "3A126854",
...
static OUT rfc4634_2_sha256 = {
    "5BDCC146BF60754E6A042426089575C75A003F089D2739839DEC58B964EC" "3843",
    ISC_SHA256_DIGESTLENGTH
};

static OUT rfc4634_2_sha384 = {
    "AF45D2E376484031617F78D2B58A6B1B9C7EF464F5A01B47E42EC3736322" "445E8E2240CA5E69E2C78B3239ECFAB21649",
    ISC_SHA384_DIGESTLENGTH
};

static OUT rfc4634_2_sha512 = {
    "164B7A7BFCF819E2E395FBE73B56E0A387BD64222E831FD610270CD7EA25"
        "05549758BF75C05A994A6D034F65F8F0E6FDCAEAB1A34D4A6B4B636E070A" "38BCE737",
    ISC_SHA512_DIGESTLENGTH
};
...
static OUT rfc4634_3_sha256 = {
    "773EA91E36800E46854DB8EBD09181A72959098B3EF8C122D9635514CED5" "65FE",
    ISC_SHA256_DIGESTLENGTH
};

static OUT rfc4634_3_sha384 = {
    "88062608D3E6AD8A0AA2ACE014C8A86F0AA635D947AC9FEBE83EF4E55966" "144B2A5AB39DC13814B94E3AB6E101A34F27",
    ISC_SHA384_DIGESTLENGTH
};

static OUT rfc4634_3_sha512 = {
    "FA73B0089D56A284EFB0F0756C890BE9B1B5DBDD8EE81A3655F83E33B227"
        "9D39BF3E848279A722C806B485A47E67C807B946A337BEE8942674278859" "E13292FB",
    ISC_SHA512_DIGESTLENGTH
};
...
static OUT rfc4634_4_sha256 = {
    "82558A389A443C0EA4CC819899F2083A85F0FAA3E578F8077A2E3FF46729" "665B",
    ISC_SHA256_DIGESTLENGTH
};

static OUT rfc4634_4_sha384 = {
    "3E8A69B7783C25851933AB6290AF6CA77A9981480850009CC5577C6E1F57" "3B4E6801DD23C4A7D679CCF8A386C674CFFB",
    ISC_SHA384_DIGESTLENGTH
};

static OUT rfc4634_4_sha512 = {
    "B0BA465637458C6990E5A8C5F61D4AF7E576D97FF94B872DE76F8050361E"
        "E3DBA91CA5C11AA25EB4D679275CC5788063A5F19741120C4F2DE2ADEBEB" "10A298DD",
    ISC_SHA512_DIGESTLENGTH
};



static const char *d2str (char *buf, unsigned int buf_len, const unsigned char *d, unsigned int d_len)
{
    unsigned int i, l;
 ...
 */
static void ck (const char *name, const IN * in, const OUT * out)
{
    ...
    {
        t_info ("%s(%s)\n%9s %s\n%9s %s\n", name, str_name, "is", buf, "should be", out->str);
        ++nprobs;
        return;
    }
...
static void t_hash (const char *hname, HASH_INIT init, UPDATE update, FINAL final, IN * in, OUT * out)
{
    union
    {
        unsigned char b[1024];
 ...
 */
static void t_sha224 (IN * in, OUT * out)
{
    isc_sha224_t ctx;
...
static void t_hashes (IN * in, OUT * out_sha1, OUT * out_sha224, OUT * out_md5)
{
    t_hash ("SHA1", (HASH_INIT) isc_sha1_init, (UPDATE) isc_sha1_update, (FINAL) isc_sha1_final, in, out_sha1);
    t_sha224 (in, out_sha224);
    t_hash ("md5", (HASH_INIT) isc_md5_init, (UPDATE) isc_md5_update, (FINAL) isc_md5_final, in, out_md5);
}
 ...
 */
static void t_md5hmac (IN * in, OUT * out)
{
    isc_hmacmd5_t ctx;
...
static void t_hmac (const char *hname, HMAC_INIT init, UPDATE update, SIGN sign, IN * in, OUT * out)
{
    union
    {
        unsigned char b[1024];
...
static void t_hmacs (IN * in, OUT * out_sha1, OUT * out_sha224, OUT * out_sha256, OUT * out_sha384, OUT * out_sha512)
{
    t_hmac ("HMAC-SHA1", (HMAC_INIT) isc_hmacsha1_init,
            (UPDATE) isc_hmacsha1_update, (SIGN) isc_hmacsha1_sign, in, out_sha1);
    t_hmac ("HMAC-SHA224", (HMAC_INIT) isc_hmacsha224_init,
            (UPDATE) isc_hmacsha224_update, (SIGN) isc_hmacsha224_sign, in, out_sha224);
    t_hmac ("HMAC-SHA256", (HMAC_INIT) isc_hmacsha256_init,
            (UPDATE) isc_hmacsha256_update, (SIGN) isc_hmacsha256_sign, in, out_sha256);
    t_hmac ("HMAC-SHA384", (HMAC_INIT) isc_hmacsha384_init,
            (UPDATE) isc_hmacsha384_update, (SIGN) isc_hmacsha384_sign, in, out_sha384);
    t_hmac ("HMAC-SHA512", (HMAC_INIT) isc_hmacsha512_init,
            (UPDATE) isc_hmacsha512_update, (SIGN) isc_hmacsha512_sign, in, out_sha512);
}
...
static void t1 (void)
{
     ...
     */
    t_hmacs (&rfc4634_1, &rfc4634_1_sha1, &rfc4634_1_sha224, &rfc4634_1_sha256, &rfc4634_1_sha384, &rfc4634_1_sha512);
    t_hmacs (&rfc4634_2, &rfc4634_2_sha1, &rfc4634_2_sha224, &rfc4634_2_sha256, &rfc4634_2_sha384, &rfc4634_2_sha512);
    t_hmacs (&rfc4634_3, &rfc4634_3_sha1, &rfc4634_3_sha224, &rfc4634_3_sha256, &rfc4634_3_sha384, &rfc4634_3_sha512);
    t_hmacs (&rfc4634_4, &rfc4634_4_sha1, &rfc4634_4_sha224, &rfc4634_4_sha256, &rfc4634_4_sha384, &rfc4634_4_sha512);
