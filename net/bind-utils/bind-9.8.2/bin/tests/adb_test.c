...
static ISC_LIST (client_t) clients;
     ...
     static void check_result (isc_result_t result, const char *format, ...) ISC_FORMAT_PRINTF (2, 3);
...
static void lookup_callback (isc_task_t * task, isc_event_t * ev)
{
    client_t *client;
    ...
    printf ("NAME %s:\n\tTask %p got event %p type %08x from %p, client %p\n\terr4: %s  err6: %s\n",
            ...
            isc_result_totext (client->find->result_v4), isc_result_totext (client->find->result_v6));
...
static void create_managers (void)
{
    isc_result_t result;
...
static void create_view (void)
{
     ...
     */
    cache = NULL;
    result = dns_cache_create (mctx, taskmgr, timermgr, dns_rdataclass_in, "rbt", 0, NULL, &cache);
...

    {
        ...
        RUNTIME_CHECK (dns_dispatch_getudp (dispatchmgr, socketmgr,
                                            ...
                                            17, 19, attrs, attrs, &disp4) == ISC_R_SUCCESS);
        INSIST (disp4 != NULL);

        attrs = DNS_DISPATCHATTR_IPV6 | DNS_DISPATCHATTR_UDP;
        RUNTIME_CHECK (dns_dispatch_getudp (dispatchmgr, socketmgr,
                                            taskmgr, &any6, 512, 6, 1024,
                                            17, 19, attrs, attrs, &disp6) == ISC_R_SUCCESS);
        INSIST (disp6 != NULL);
...
static void lookup (const char *target)
{
    dns_name_t name;
    ...
    result = dns_adb_createfind (adb, t2, lookup_callback, client,
                                 &client->name, dns_rootname, 0, options, now, NULL, view->dstport, &client->find);
    if (result != ISC_R_SUCCESS)
    ...
    else
    {
        printf ("NAME %s:  err4 %s, err6 %s\n",
                target, isc_result_totext (client->find->result_v4), isc_result_totext (client->find->result_v6));
...
int main (int argc, char **argv)
{
    ...
    cmp = NULL;
    RUNTIME_CHECK (isc_mempool_create (mctx, sizeof (client_t), &cmp) == ISC_R_SUCCESS);
