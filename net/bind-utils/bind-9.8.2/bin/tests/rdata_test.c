...
static void viastruct (dns_rdata_t * rdata, isc_mem_t * mctx, dns_rdata_t * rdata2, isc_buffer_t * b)
{
    isc_result_t result;
    ...
    if (result != ISC_R_SUCCESS)
        fprintf (stdout, "viastruct: tostruct %d %d return %s\n",
                 rdata->type, rdata->rdclass, dns_result_totext (result));
    else
    ...
    else
    {
        ...
        if (result != ISC_R_SUCCESS)
            fprintf (stdout,
                     ...
                     rdata->type, rdata->rdclass, dns_result_totext (result));
        else if (rdata->length != rdata2->length || memcmp (rdata->data, rdata2->data, rdata->length) != 0)
        {
            isc_uint32_t i;
                    ...
                    case dns_rdataclass_hs:
                        {
                            ...
                            result = dns_rdata_fromstruct (rdata2, rdc, rdt, &hs_a, b);
                            break;
                        }
                    case dns_rdataclass_in:
                        {
                            ...
                            result = dns_rdata_fromstruct (rdata2, rdc, rdt, &in_a, b);
                            break;
                        }
                    default:
                        result = ISC_R_NOTIMPLEMENTED;
        ...
        case dns_rdatatype_nsap_ptr:
            {
                ...
                result = dns_rdata_fromstruct (rdata2, rdc, rdt, &in_nsap_ptr, b);
                break;
            }
        case dns_rdatatype_null:
            {
...
int main (int argc, char *argv[])
{
    ...
    dns_rdata_init (&last);
    while ((result = isc_lex_gettoken (lex, options | ISC_LEXOPT_NUMBER, &token)) == ISC_R_SUCCESS)
    {
        if (debug)
        ...
        if (token.type == isc_tokentype_number)
        {
            ...
            if (result != ISC_R_SUCCESS)
            {
                fprintf (stdout, "dns_rdatatype_totext " "returned %s(%d)\n", dns_result_totext (result), result);
                fflush (stdout);
                need_eol = 1;
                continue;
            }
        ...
        else if (token.type == isc_tokentype_string)
        {
            ...
            if (result != ISC_R_SUCCESS)
            {
                fprintf (stdout, "dns_rdatatype_fromtext " "returned %s(%d)\n", dns_result_totext (result), result);
                fflush (stdout);
                need_eol = 1;
                continue;
            }
            fprintf (stdout, "type = %.*s(%d)\n",
                     (int) token.value.as_textregion.length, token.value.as_textregion.base, type);
        }
        else
            continue;
        ...
        {
            result = dns_rdataclass_fromtext (&class, &token.value.as_textregion);
            if (result != ISC_R_SUCCESS)
            {
            ...
            fprintf (stdout, "class = %.*s(%d)\n",
                     (int) token.value.as_textregion.length, token.value.as_textregion.base, class);
        }
        else
            continue;
        ...
        result = dns_rdata_fromtext (&rdata, class, type, lex, NULL, 0, mctx, &dbuf, NULL);
        if (result != ISC_R_SUCCESS)
        {
            fprintf (stdout, "dns_rdata_fromtext returned %s(%d)\n", dns_result_totext (result), result);
            fflush (stdout);
            continue;
        }
        if (raw)
        ...
        if (wire)
        {
            ...
            if (result != ISC_R_SUCCESS)
            {
                fprintf (stdout, "dns_compress_init returned %s(%d)\n", dns_result_totext (result), result);
                continue;
            }
            ...
            if (result != ISC_R_SUCCESS)
            {
                fprintf (stdout, "dns_rdata_towire returned %s(%d)\n", dns_result_totext (result), result);
                continue;
            }
            ...
            if (raw > 2)
            {
                ...
                {
                    fprintf (stdout, "%02x", ((unsigned char *) wbuf.base)[i + wbuf.current]);
                    if ((++i % 20) == 0)
                        fputs ("\n", stdout);
                    else if (i == wbuf.used)
                        fputs ("\n", stdout);
            ...
            result = dns_rdata_fromwire (&rdata, class, type, &wbuf, &dctx, 0, &dbuf);
            dns_decompress_invalidate (&dctx);
            if (result != ISC_R_SUCCESS)
            {
                fprintf (stdout, "dns_rdata_fromwire returned %s(%d)\n", dns_result_totext (result), result);
                fflush (stdout);
                continue;
            }
        }
