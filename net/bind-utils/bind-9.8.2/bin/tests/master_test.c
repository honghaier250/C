...
static isc_result_t print_dataset (void *arg, dns_name_t * owner, dns_rdataset_t * dataset)
{
    char buf[64 * 1024];
...
int main (int argc, char *argv[])
{
    ...
    if (argv[1])
    {
        ...
        result = dns_name_fromtext (&origin, &source, dns_rootname, 0, &target);
        if (result != ISC_R_SUCCESS)
        {
        ...
        result = dns_master_loadfile (argv[1], &origin, &origin, dns_rdataclass_in, 0, &callbacks, mctx);
