...
static void recvquery (isc_task_t * task, isc_event_t * event)
{
    ...
    result = dns_tkey_processdhresponse (query, response, ourkey, &nonce, &tsigkey, ring);
    ...
    printf ("%.*s\n", (int) isc_buffer_usedlength (&keynamebuf), (char *) isc_buffer_base (&keynamebuf));
...
static void sendquery (isc_task_t * task, isc_event_t * event)
{
    ...
    result = dns_name_fromtext (dns_fixedname_name (&keyname), &namestr, NULL, 0, NULL);
    ...
    result = dns_name_fromtext (dns_fixedname_name (&ownername), &namestr, NULL, 0, NULL);
    CHECK ("dns_name_fromtext", result);
    ...
    result = dns_tsigkey_create (dns_fixedname_name (&keyname),
                                 ...
                                 isc_buffer_usedlength (&keybuf), ISC_FALSE, NULL, 0, 0, mctx, ring, &initialkey);
    ...
    result = dns_tkey_builddhquery (query, ourkey,
                                    dns_fixedname_name (&ownername), DNS_TSIG_HMACMD5_NAME, &nonce, 3600);
    CHECK ("dns_tkey_builddhquery", result);

    request = NULL;
    result = dns_request_create (requestmgr, query, &address, 0, initialkey, TIMEOUT, task, recvquery, query, &request);
...
int main (int argc, char *argv[])
{
    ...
    result = isc_entropy_getdata (ectx, noncedata, sizeof (noncedata), NULL, ISC_ENTROPY_BLOCKING);
