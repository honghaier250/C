...
static void CHECK (lwres_result_t val, const char *msg)
{
    if (val != 0)
    {
...
static unsigned char TESTSTRING[] = "This is a test.  This is only a test.  !!!";
...
static void test_noop (void)
{
    ...
    assert (memcmp (nooprequest.data, nooprequest2->data, nooprequest.datalength) == 0);
    ...
    assert (memcmp (noopresponse.data, noopresponse2->data, noopresponse.datalength) == 0);
...
static void test_gabn (const char *target, lwres_result_t expected, const char *address, lwres_uint32_t af)
{
    ...
    res = NULL;
    ret = lwres_getaddrsbyname (ctx, target, LWRES_ADDRTYPE_V4 | LWRES_ADDRTYPE_V6, &res);
    if (ret != expected)
    {
    ...
    if (ret == LWRES_R_SUCCESS)
    {
        ...
        while (addr != NULL)
        {
            if (addr->family != af || addr->length != len || memcmp (addr->address, addrbuf, len) == 0)
                break;
        ...
        if (addr == NULL)
        {
            ...
            if (addr->family == LWRES_ADDRTYPE_V4)
                (void) inet_ntop (AF_INET, addr->address, outbuf, sizeof (outbuf));
            else
                (void) inet_ntop (AF_INET6, addr->address, outbuf, sizeof (outbuf));
...
static void test_gnba (const char *target, lwres_uint32_t af, lwres_result_t expected, const char *name)
{
...
static void test_gethostbyname (const char *name, const char *address)
{
    struct hostent *hp;
    ...
    else
    {
        ...
        if (memcmp (hp->h_addr_list[0], addrbuf, hp->h_length) != 0)
        {
            ...
            (void) inet_ntop (AF_INET, hp->h_addr_list[0], outbuf, sizeof (outbuf));
...
static void test_gethostbyname2 (const char *name, const char *address, int af)
{
    struct hostent *hp;
    ...
    else
    {
        ...
        if (len != (int) hp->h_length || memcmp (hp->h_addr_list[0], addrbuf, hp->h_length) != 0)
        {
            char outbuf[16];

            (void) inet_ntop (af, hp->h_addr_list[0], outbuf, sizeof (outbuf));
            printf ("I:gethostbyname(%s) returned %s, " "expected %s\n", name, outbuf, address);
            fails++;
            return;
        }
...
static void test_getipnodebyname (const char *name, const char *address, int af, int v4map, int all)
{
    struct hostent *hp;
    ...
    else
    {
        ...
        if (len != (int) hp->h_length || memcmp (hp->h_addr_list[0], addrbuf, hp->h_length) != 0)
        {
            ...
            (void) inet_ntop (af, hp->h_addr_list[0], outbuf, sizeof (outbuf));
            printf ("I:getipnodebyname(%s) returned %s, " "expected %s\n", name, outbuf, address);
            freehostent (hp);
            fails++;
            return;
        }
...
static void test_gethostbyaddr (const char *address, int af, const char *name)
{
    struct hostent *hp;
...
static void test_getipnodebyaddr (const char *address, int af, const char *name)
{
    struct hostent *hp;
    ...
    else
    {
        if (strcmp (hp->h_name, name) != 0)
        {
            printf ("I:getipnodebyaddr(%s) returned %s, " "expected %s\n", address, hp->h_name, name);
            freehostent (hp);
            fails++;
            return;
        }
...
static void test_getaddrinfo (const char *name, int af, int v4ok, int v6ok, const char *address)
{
    unsigned int len;
    ...
    else
    {
        ...
        if (len != (unsigned int) ai->ai_addrlen)
        {
            ...
            (void) inet_ntop (af, ai->ai_addr, outbuf, sizeof (outbuf));
            printf ("I:getaddrinfo(%s) returned %lub, " "expected %ub\n", name, (unsigned long) ai->ai_addrlen, len);
            fails++;
            freeaddrinfo (ai);
            return;
        }
        else if (af == AF_INET)
        {
            ...
            if (memcmp (&sin->sin_addr.s_addr, addrbuf, 4) != 0)
            {
                ...
                (void) inet_ntop (af, &sin->sin_addr.s_addr, outbuf, sizeof (outbuf));
        ...
        else
        {
            ...
            if (memcmp (sin6->sin6_addr.s6_addr, addrbuf, 16) != 0)
            {
                ...
                (void) inet_ntop (af, &sin6->sin6_addr.s6_addr, outbuf, sizeof (outbuf));
                printf ("I:getaddrinfo(%s) returned %s, " "expected %s\n", name, outbuf, address);
                fails++;
                freeaddrinfo (ai);
                return;
...
static void test_getnameinfo (const char *address, int af, const char *name)
{
    int ret;
    ...
    else
    {
        if (name == NULL)
        {
            printf ("I:getnameinfo(%s) returned %s, " "expected NULL\n", address, host);
            fails++;
            return;
        }
...
test_getrrsetbyname (const char *name, int rdclass, int rdtype,
                     unsigned int nrdatas, unsigned int nsigs, int should_pass)
{
    int ret;
...
int main (void)
{
    ...
    test_gabn ("a.example1.", LWRES_R_SUCCESS, "10.0.1.1", LWRES_ADDRTYPE_V4);
    test_gabn ("a.example2", LWRES_R_SUCCESS, "10.0.2.1", LWRES_ADDRTYPE_V4);
    test_gabn ("a.example2.", LWRES_R_SUCCESS, "10.0.2.1", LWRES_ADDRTYPE_V4);
    ...
    test_gabn ("b.example1", LWRES_R_SUCCESS, "eeee:eeee:eeee:eeee:ffff:ffff:ffff:ffff", LWRES_ADDRTYPE_V6);
    test_gabn ("b.example1.", LWRES_R_SUCCESS, "eeee:eeee:eeee:eeee:ffff:ffff:ffff:ffff", LWRES_ADDRTYPE_V6);
    test_gabn ("b.example2", LWRES_R_SUCCESS, "eeee:eeee:eeee:eeee:ffff:ffff:ffff:ffff", LWRES_ADDRTYPE_V6);
    test_gabn ("b.example2.", LWRES_R_SUCCESS, "eeee:eeee:eeee:eeee:ffff:ffff:ffff:ffff", LWRES_ADDRTYPE_V6);
    test_gabn ("b.example3", LWRES_R_NOTFOUND, NULL, LWRES_ADDRTYPE_V6);
    test_gabn ("b.example3.", LWRES_R_NOTFOUND, NULL, LWRES_ADDRTYPE_V6);
    test_gabn ("b", LWRES_R_SUCCESS, "eeee:eeee:eeee:eeee:ffff:ffff:ffff:ffff", LWRES_ADDRTYPE_V6);
    ...
    test_gnba ("10.10.10.1", LWRES_ADDRTYPE_V4, LWRES_R_SUCCESS, "ipv4.example");
    test_gnba ("10.10.10.17", LWRES_ADDRTYPE_V4, LWRES_R_NOTFOUND, NULL);
    test_gnba ("0123:4567:89ab:cdef:0123:4567:89ab:cdef", LWRES_ADDRTYPE_V6, LWRES_R_SUCCESS, "ip6.int.example");
    test_gnba ("0123:4567:89ab:cdef:0123:4567:89ab:cde0", LWRES_ADDRTYPE_V6, LWRES_R_NOTFOUND, NULL);
    test_gnba ("1123:4567:89ab:cdef:0123:4567:89ab:cdef", LWRES_ADDRTYPE_V6, LWRES_R_SUCCESS, "ip6.arpa.example");
    test_gnba ("1123:4567:89ab:cdef:0123:4567:89ab:cde0", LWRES_ADDRTYPE_V6, LWRES_R_NOTFOUND, NULL);

    test_gethostbyname ("a.example1.", "10.0.1.1");
    test_gethostbyname ("q.example1.", NULL);

    test_gethostbyname2 ("a.example1.", "10.0.1.1", AF_INET);
    test_gethostbyname2 ("b.example1.", "eeee:eeee:eeee:eeee:ffff:ffff:ffff:ffff", AF_INET6);
    test_gethostbyname2 ("q.example1.", NULL, AF_INET);

    test_getipnodebyname ("a.example1.", "10.0.1.1", AF_INET, 0, 0);
    test_getipnodebyname ("b.example1.", "eeee:eeee:eeee:eeee:ffff:ffff:ffff:ffff", AF_INET6, 0, 0);
    test_getipnodebyname ("a.example1.", "::ffff:10.0.1.1", AF_INET6, 1, 0);
    test_getipnodebyname ("a.example1.", "::ffff:10.0.1.1", AF_INET6, 1, 1);
    test_getipnodebyname ("b.example1.", "eeee:eeee:eeee:eeee:ffff:ffff:ffff:ffff", AF_INET6, 1, 1);
    ...
    test_gethostbyaddr ("0123:4567:89ab:cdef:0123:4567:89ab:cdef", AF_INET6, "ip6.int.example");
    test_gethostbyaddr ("1123:4567:89ab:cdef:0123:4567:89ab:cdef", AF_INET6, "ip6.arpa.example");

    test_getipnodebyaddr ("10.10.10.1", AF_INET, "ipv4.example");
    test_getipnodebyaddr ("10.10.10.17", AF_INET, NULL);
    test_getipnodebyaddr ("0123:4567:89ab:cdef:0123:4567:89ab:cdef", AF_INET6, "ip6.int.example");
    test_getipnodebyaddr ("1123:4567:89ab:cdef:0123:4567:89ab:cdef", AF_INET6, "ip6.arpa.example");
    ...
    test_getaddrinfo ("b.example1.", AF_INET6, 1, 1, "eeee:eeee:eeee:eeee:ffff:ffff:ffff:ffff");
    test_getaddrinfo ("b.example1.", AF_INET6, 1, 0, NULL);
    test_getaddrinfo ("b.example1.", AF_INET6, 0, 1, "eeee:eeee:eeee:eeee:ffff:ffff:ffff:ffff");

    test_getnameinfo ("10.10.10.1", AF_INET, "ipv4.example");
    test_getnameinfo ("10.10.10.17", AF_INET, NULL);
    test_getnameinfo ("0123:4567:89ab:cdef:0123:4567:89ab:cdef", AF_INET6, "ip6.int.example");
    test_getnameinfo ("1123:4567:89ab:cdef:0123:4567:89ab:cdef", AF_INET6, "ip6.arpa.example");
    test_getnameinfo ("1122:3344:5566:7788:99aa:bbcc:ddee:ff00", AF_INET6, "dname.example1");
