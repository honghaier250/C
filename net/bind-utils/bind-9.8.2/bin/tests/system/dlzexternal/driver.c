...
static isc_boolean_t single_valued (const char *type)
{
          ...
          const char *name, const char *type, dns_ttl_t ttl, const char *data)
{
    ...
    for (i = 0; i < MAX_RECORDS; i++)
    {
        if (strcasecmp (name, list[i].name) == 0 &&
            strcasecmp (type, list[i].type) == 0 && strcasecmp (data, list[i].data) == 0 && ttl == list[i].ttl)
        {
            break;
        }
    }
...
isc_result_t dlz_create (const char *dlzname, unsigned int argc, char *argv[], void **dbdata, ...)
{
    ...
    sprintf (soa_data, "%s hostmaster.%s 123 900 600 86400 3600", state->zone_name, state->zone_name);
 ...
 * Look up one record
 */
isc_result_t dlz_lookup (const char *zone, const char *name, void *dbdata, dns_sdlzlookup_t * lookup)
{
    struct dlz_example_data *state = (struct dlz_example_data *) dbdata;
    ...
    for (i = 0; i < MAX_RECORDS; i++)
    {
        if (strcasecmp (state->current[i].name, full_name) == 0)
        {
            ...
            found = ISC_TRUE;
            result = state->putrr (lookup, state->current[i].type, state->current[i].ttl, state->current[i].data);
...
isc_result_t dlz_allnodes (const char *zone, void *dbdata, dns_sdlzallnodes_t * allnodes)
{
    ...
    for (i = 0; i < MAX_RECORDS; i++)
    {
        ...
        result = state->putnamedrr (allnodes, state->current[i].name,
                                    state->current[i].type, state->current[i].ttl, state->current[i].data);
        if (result != ISC_R_SUCCESS)
            return (result);
    }
        ...
        for (i = 0; i < MAX_RECORDS; i++)
        {
            if (strlen (state->adds[i].name) > 0U)
            {
                add_name (state, &state->current[0],
                          state->adds[i].name, state->adds[i].type, state->adds[i].ttl, state->adds[i].data);
            }
        }
        for (i = 0; i < MAX_RECORDS; i++)
        {
            if (strlen (state->deletes[i].name) > 0U)
            {
                del_name (state, &state->current[0],
                          ...
                          state->deletes[i].type, state->deletes[i].ttl, state->deletes[i].data);
            }
        }
    }
    else
...
dlz_ssumatch (const char *signer, const char *name, const char *tcpaddr,
              const char *type, const char *key, isc_uint32_t keydatalen, unsigned char *keydata, void *dbdata)
{
    struct dlz_example_data *state = (struct dlz_example_data *) dbdata;
...
static isc_result_t
modrdataset (struct dlz_example_data *state, const char *name, const char *rdatastr, struct record *list)
{
