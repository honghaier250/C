...
static isc_result_t t1_add_callback (void *arg, dns_name_t * owner, dns_rdataset_t * dataset);
...
static isc_result_t t1_add_callback (void *arg, dns_name_t * owner, dns_rdataset_t * dataset)
{
    ...
    result = dns_rdataset_totext (dataset, owner, ISC_FALSE, ISC_FALSE, &target);
...
static int test_master (char *testfile, char *origin, char *class, isc_result_t exp_result)
{
    int result;
    ...
    dns_result = dns_name_fromtext (&dns_origin, &source, dns_rootname, 0, &target);
    ...
    dns_result = dns_master_loadfile (testfile, &dns_origin, &dns_origin, rdataclass, ISC_TRUE, &callbacks, T1_mctx);
...
static int test_master_x (const char *filename)
{
    FILE *fp;
    ...
    if (fp != NULL)
    {
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 4)
            {
                result = test_master (Tokens[0], Tokens[1], Tokens[2], t_dns_result_fromtext (Tokens[3]));
            }
            else
            {
...
static const char *a1 = "dns_master_loadfile loads a valid master file and " "returns ISC_R_SUCCESS";
...
static const char *a2 =
    "dns_master_loadfile returns ISC_R_UNEXPECTEDEND when the " "masterfile input ends unexpectedly";
...
static const char *a3 = "dns_master_loadfile returns DNS_R_NOOWNER when the " "an ownername is not specified";
...
static const char *a4 = "dns_master_loadfile accepts broken zone files "
    "where the first record has an undefined TTL, " "as long as it is a SOA";
...
static const char *a6 = "dns_master_loadfile understands DNSKEY RR specifications " "containing key material";
...
static const char *a7 = "dns_master_loadfile understands DNSKEY RR specifications " "containing no key material";
...
static const char *a8 = "dns_master_loadfile understands $INCLUDE";
...
static const char *a9 = "dns_master_loadfile understands $INCLUDE with failure";
...
static const char *a10 = "dns_master_loadfile non-empty blank lines";
...
static const char *a11 = "dns_master_loadfile allow leading zeros in SOA";
