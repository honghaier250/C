...
static void detail (dns_rbt_t * rbt, dns_name_t * name)
{
    ...
    result = dns_rbt_findnode (rbt, name, foundname, &node1, &chain, DNS_RBTFIND_EMPTYDATA, NULL, NULL);
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        result = dns_name_concatenate (foundname, origin, fullname, NULL);
        if (result == ISC_R_SUCCESS)
            print_name (fullname);
        else
            printf ("%s\n", dns_result_totext (result));
        ...
        if (nodes_should_match && node1 != node2)
            printf ("  nodes returned from each function " "DO NOT match!\n");
...
static void iterate (dns_rbt_t * rbt, isc_boolean_t forward)
{
    ...
    isc_result_t (*move) (dns_rbtnodechain_t * chain, dns_name_t * name, dns_name_t * origin);
        ...
        if (*command != '\0')
        {
            if (CMDCHECK ("add"))
            {
                ...
                if (name != NULL)
                {
                    ...
                    result = dns_rbt_addname (rbt, name, name);
                    PRINTERR (result);
                }
            ...
            else if (CMDCHECK ("delete"))
            {
                ...
                if (name != NULL)
                {
                    ...
                    result = dns_rbt_deletename (rbt, name, ISC_FALSE);
                    PRINTERR (result);
            ...
            else if (CMDCHECK ("nuke"))
            {
                ...
                if (name != NULL)
                {
                    printf ("nuking name %s " "and its descendants\n", arg);
                    result = dns_rbt_deletename (rbt, name, ISC_TRUE);
                    PRINTERR (result);
                    delete_name (name, NULL);
                }
            ...
            else if (CMDCHECK ("search"))
            {
                ...
                if (name != NULL)
                {
                    printf ("searching for name %s ... ", arg);

                    dns_fixedname_init (&fixedname);
                    foundname = dns_fixedname_name (&fixedname);
                    data = NULL;

                    result = dns_rbt_findname (rbt, name, 0, foundname, &data);
                    switch (result)
                    {
                        case ISC_R_SUCCESS:
                            printf ("found exact: ");
            ...
            {
                printf ("a(dd) NAME, d(elete) NAME, " "s(earch) NAME, p(rint), or q(uit)\n");
