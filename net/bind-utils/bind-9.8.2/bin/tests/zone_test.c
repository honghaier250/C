...
static void usage ()
{
    fprintf (stderr, "usage: zone_test [-dqsSM] [-c class] [-f file] zone\n");
    exit (1);
}

static void setup (const char *zonename, const char *filename, const char *classname)
{
    isc_result_t result;
    ...
    result = dns_rdataclass_fromtext (&rdclass, (isc_textregion_t *) (void *) &region);
...
static void query (void)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        fprintf (stderr, "%s() returned %s\n", "dns_zone_getdb", dns_result_totext (result));
        return;
    }
    ...
    do
    {
        ...
        result = dns_name_fromtext (dns_fixedname_name (&name), &buffer, dns_rootname, 0, NULL);
        ERRCONT (result, "dns_name_fromtext");

        result = dns_db_find (db, dns_fixedname_name (&name), NULL /*vesion */ ,
                              dns_rdatatype_a, 0 /*options */ ,
                              0 /*time */ ,
                              NULL /*nodep */ ,
                              dns_fixedname_name (&found), &rdataset, &sigset);
        fprintf (stderr, "%s() returned %s\n", "dns_db_find", dns_result_totext (result));
        switch (result)
        {
...
int main (int argc, char **argv)
{
    ...
    while ((c = isc_commandline_parse (argc, argv, "cdf:m:qsMS")) != EOF)
    {
        switch (c)
        {
            ...
            case 'm':
                ...
                inet_pton (AF_INET, isc_commandline_argument, &addr.type.sin.sin_addr);
    ...
    RUNTIME_CHECK (isc_taskmgr_create (mctx, 2, 0, &taskmgr) == ISC_R_SUCCESS);
    RUNTIME_CHECK (isc_timermgr_create (mctx, &timermgr) == ISC_R_SUCCESS);
    RUNTIME_CHECK (isc_socketmgr_create (mctx, &socketmgr) == ISC_R_SUCCESS);
    RUNTIME_CHECK (dns_zonemgr_create (mctx, taskmgr, timermgr, socketmgr, &zonemgr) == ISC_R_SUCCESS);
    if (filename == NULL)
