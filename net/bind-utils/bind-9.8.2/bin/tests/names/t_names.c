...
static char *ctoh (unsigned char c)
{
    int val;
...
static void pbuf (isc_buffer_t * pbuf)
{
    size_t len;
...
static int chkdata (unsigned char *buf, size_t buflen, char *exp_data, size_t exp_data_len)
{
    int result;
    ...
    if (buflen == exp_data_len)
    {
        ...
        {
            t_info ("bad data at position %lu, " "got 0x%.2x, expected 0x%.2x\n", (unsigned long) cnt, *p, *q);
            result = cnt + 1;
        }
        (void) free (data);
    }
 ...
 */
static int getmsg (char *datafile_name, unsigned char *buf, int buflen, isc_buffer_t * pbuf)
{
    int c;
...
static int bustline (char *line, char **toks)
{
    int cnt;
...
static int hname_to_tname (char *src, char *target, size_t len)
{
    int i;
...
static isc_result_t dname_from_tname (char *name, dns_name_t * dns_name)
{
    int len;
...
static void t_dns_name_init (void)
{
    int rval;
...
static void t_dns_name_invalidate (void)
{
    int rval;
...
static const char *a5 = "dns_name_setbuffer dedicates a buffer for use " "with 'name'";

static void t_dns_name_setbuffer (void)
{
    int result;
...
static void t_dns_name_hasbuffer (void)
{
    int result;
...
static const char *a7 = "dns_name_isabsolute returns ISC_TRUE if 'name' ends " "in the root label";

static int test_dns_name_isabsolute (char *test_name, isc_boolean_t expected)
{
    dns_name_t name;
...
static void t_dns_name_isabsolute (void)
{
    int line;
    ...
    if (fp != NULL)
    {
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 2)
            {
                 ...
                 */
                result = test_dns_name_isabsolute (Tokens[0], atoi (Tokens[1]) == 0 ? ISC_FALSE : ISC_TRUE);
            }
            else
            {
...
static const char *a8 = "dns_name_hash(name, case_sensitive) returns "
    "a hash of 'name' which is case_sensitive if case_sensitive " "is true";

/*%
 * a9 merged with a8.
 */

static int test_dns_name_hash (char *test_name1, char *test_name2, isc_boolean_t csh_match, isc_boolean_t cish_match)
{
    int rval;
...
static void t_dns_name_hash (void)
{
    int line;
    ...
    if (fp != NULL)
    {
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 4)
            {
                 ...
                 */
                result = test_dns_name_hash (Tokens[0],
                                             Tokens[1],
                                             atoi (Tokens[2]) == 0 ?
                                             ISC_FALSE : ISC_TRUE, atoi (Tokens[3]) == 0 ? ISC_FALSE : ISC_TRUE);
            }
            else
            {
                t_info ("bad datafile format at line %d\n", line);
...
static const char *a10 =
    ...
    "and sets nbitsp to the number of bits name1 and name2 " "have in common";

/*%
 * a11 thru a22 merged into a10.
 */
static const char *dns_namereln_to_text (dns_namereln_t reln)
{
    const char *p;
...
static int
test_dns_name_fullcompare (char *name1, char *name2, dns_namereln_t exp_dns_reln, int exp_order, int exp_nlabels)
{
    int result;
    ...
    t_info ("testing names %s and %s for relation %s\n", name1, name2, dns_namereln_to_text (exp_dns_reln));
    ...
    if (dns_result == ISC_R_SUCCESS)
    {
        ...
        if (dns_result == ISC_R_SUCCESS)
        {
            dns_reln = dns_name_fullcompare (&dns_name1, &dns_name2, &order, &nlabels);

            if (dns_reln != exp_dns_reln)
            {
                ++nfails;
                t_info ("expected relationship of %s, got %s\n",
                        dns_namereln_to_text (exp_dns_reln), dns_namereln_to_text (dns_reln));
            }
            /*
             * Normalize order.
             */
...
static void t_dns_name_fullcompare (void)
{
    int line;
    ...
    if (fp != NULL)
    {
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 6)
            {
                ...
                result = test_dns_name_fullcompare (Tokens[0], Tokens[1], reln, atoi (Tokens[3]), atoi (Tokens[4]));
            }
            else
            {
                t_info ("bad format at line %d\n", line);
...
static const char *a23 =
    ...
    "the relative ordering under the DNSSEC ordering relationship " "of name1 and name2";

/*%
 * a24 thru a29 merged into a23.
 */

static int test_dns_name_compare (char *name1, char *name2, int exp_order)
{
    int result;
    ...
    t_info ("testing %s %s %s\n", name1, exp_order == 0 ? "==" : (exp_order == -1 ? "<" : ">"), name2);
...
static void t_dns_name_compare (void)
{
    int line;
    ...
    if (fp != NULL)
    {
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 3)
            {
                 ...
                 * name1, name2, order.
                 */
                result = test_dns_name_compare (Tokens[0], Tokens[1], atoi (Tokens[2]));
            }
            else
            {
                t_info ("bad datafile format at line %d\n", line);
...
static const char *a30 =
    ...
    "about the relative ordering of name1 and name2 as if they " "are part of rdata in DNSSEC canonical form";

/*%
 * a31, a32 merged into a30.
 */

static int test_dns_name_rdatacompare (char *name1, char *name2, int exp_order)
{
    int result;
...
static void t_dns_name_rdatacompare (void)
{
    int line;
    ...
    if (fp != NULL)
    {
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 3)
            {
                 ...
                 * name1, name2, order.
                 */
                result = test_dns_name_rdatacompare (Tokens[0], Tokens[1], atoi (Tokens[2]));
            }
            else
            {
                t_info ("bad datafile format at line %d\n", line);
...
static const char *a33 =
    ...
    "dns_name_issubdomain(name1, name2) returns true, " "otherwise it returns false.";

/*%
 * a34 merged into a33.
 */

static int test_dns_name_issubdomain (char *name1, char *name2, isc_boolean_t exp_rval)
{
    int result;
        ...
        if (dns_result == ISC_R_SUCCESS)
        {
            ...
            if (rval != exp_rval)
            {
                t_info ("expected return value of %s, got %s\n",
                        exp_rval == ISC_TRUE ? "true" : "false", rval == ISC_TRUE ? "true" : "false");
                result = T_FAIL;
            }
            else
                result = T_PASS;
...
static void t_dns_name_issubdomain (void)
{
    int line;
    ...
    if (fp != NULL)
    {
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 3)
            {
                 ...
                 */
                result = test_dns_name_issubdomain (Tokens[0], Tokens[1], atoi (Tokens[2]) == 0 ? ISC_FALSE : ISC_TRUE);
            }
            else
            {
                t_info ("bad datafile format at line %d\n", line);
...
static const char *a35 = "dns_name_countlabels(name) returns the number " "of labels in name";

static int test_dns_name_countlabels (char *test_name, unsigned int exp_nlabels)
{
    int result;
...
static void t_dns_name_countlabels (void)
{
    int line;
    ...
    if (fp != NULL)
    {
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 2)
            {
                 ...
                 * name, nlabels.
                 */
                result = test_dns_name_countlabels (Tokens[0], atoi (Tokens[1]));
            }
            else
            {
                t_info ("bad format at line %d\n", line);
...
static const char *a36 =
    ...
    "dns_name_getlabel(name, n, labelp) initializes labelp " "to point to the nth label in name";
...
static int test_dns_name_getlabel (char *test_name1, int label1_pos, char *test_name2, int label2_pos)
{
    int result;
    ...
    if (dns_result == ISC_R_SUCCESS)
    {
        ...
        if (dns_result == ISC_R_SUCCESS)
        {
            ...
            for (cnt = 0; cnt < dns_label1.length; ++cnt)
            {
                if (*p++ != *q++)
                {
                    t_info ("labels differ at position %d", cnt);
                    ++nfails;
                }
            }
            if (nfails == 0)
        ...
        {
            t_info ("dname_from_tname failed, result == %s", dns_result_totext (result));
        }
    }
    else
    {
...
static void t_dns_name_getlabel (void)
{
    int line;
    ...
    if (fp != NULL)
    {
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 4)
            {
                 ...
                 * name1, name2, nlabels.
                 */
                result = test_dns_name_getlabel (Tokens[0], atoi (Tokens[1]), Tokens[2], atoi (Tokens[3]));
            }
            else
            {
                t_info ("bad format at line %d\n", line);
...
static const char *a37 =
    ...
    "initializes target to point to the n label sequence of " "labels in source starting with first";
...
static int
test_dns_name_getlabelsequence (char *test_name1, int label1_start, char *test_name2, int label2_start, int range)
{
    int result;
    ...
    if (dns_result == ISC_R_SUCCESS)
    {
        ...
        if (dns_result == ISC_R_SUCCESS)
        {
            ...
            dns_name_getlabelsequence (&dns_name1, label1_start, range, &dns_targetname1);
            dns_name_getlabelsequence (&dns_name2, label2_start, range, &dns_targetname2);
        ...
        {
            t_info ("dname_from_tname failed, result == %s", dns_result_totext (dns_result));
        }
    }
    else
    {
...
static void t_dns_name_getlabelsequence (void)
{
    int line;
    ...
    if (fp != NULL)
    {
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 5)
            {
                 ...
                 * name1, name2, nlabels.
                 */
                result = test_dns_name_getlabelsequence (Tokens[0],
                                                         atoi (Tokens[1]),
                                                         Tokens[2], atoi (Tokens[3]), atoi (Tokens[4]));
            }
            else
            {
                t_info ("bad format at line %d\n", line);
...
static const char *a38 =
    "dns_name_fromregion(name, region) converts a DNS name " "from a region representation to a name representation";

static int test_dns_name_fromregion (char *test_name)
{
    int result;
    ...
    if (dns_result == ISC_R_SUCCESS)
    {
        ...
        dns_namereln = dns_name_fullcompare (&dns_name1, &dns_name2, &order, &nlabels);
...
static void t_dns_name_fromregion (void)
{
    int line;
...
static const char *a39 =
    "dns_name_toregion(name, region) converts a DNS name " "from a region representation to a name representation";

static void t_dns_name_toregion (void)
{
    int line;
...
static const char *a40 =
    ...
    "upper case to lower case during conversion " "if downcase is true.";

static int test_dns_name_fromtext (char *test_name1, char *test_name2, char *test_origin, unsigned int downcase)
{
    int result;
    ...
    dns_result = dns_name_fromtext (&dns_name3, &txtbuf3, NULL, 0, &binbuf3);
    if (dns_result != ISC_R_SUCCESS)
    {
    ...
    dns_result = dns_name_fromtext (&dns_name1, &txtbuf1, &dns_name3, downcase, &binbuf1);
    if (dns_result != ISC_R_SUCCESS)
    {
    ...
    dns_result = dns_name_fromtext (&dns_name2, &txtbuf2, NULL, 0, &binbuf2);
    if (dns_result != ISC_R_SUCCESS)
    {
...
static void t_dns_name_fromtext (void)
{
    int line;
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 4)
            {
                ...
                result = test_dns_name_fromtext (Tokens[0],
                                                 Tokens[1], Tokens[2], atoi (Tokens[3]) == 0 ? 0 : DNS_NAME_DOWNCASE);
            }
            else
            {
                t_info ("bad format at line %d\n", line);
...
static const char *a41 =
    ...
    "at target, and adds a final '.' to the name if " "omit_final_dot is false";

static int test_dns_name_totext (char *test_name, isc_boolean_t omit_final)
{
    int result;
...
static void t_dns_name_totext (void)
{
    int line;
    ...
    if (fp != NULL)
    {
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 2)
            {
                 ...
                 */
                result = test_dns_name_totext (Tokens[0], atoi (Tokens[1]) == 0 ? ISC_FALSE : ISC_TRUE);
            }
            else
            {
                t_info ("bad format at line %d\n", line);
...
static const char *a42 =
    ...
    "lower case conversion if downcase is true, and returns " "ISC_R_SUCCESS";
     ...
     * disabled them.
     */
static const char *a43 = "when a label length is invalid, dns_name_fromwire() " "returns DNS_R_FORMERR";

static const char *a44 = "when a label type is invalid, dns_name_fromwire() " "returns DNS_R_BADLABELTYPE";
#endif

static const char *a45 = "when a name length is invalid, dns_name_fromwire() " "returns DNS_R_FORMERR";

static const char *a46 = "when a compression type is invalid, dns_name_fromwire() " "returns DNS_R_DISALLOWED";

static const char *a47 =
    "when a bad compression pointer is encountered, " "dns_name_fromwire() returns DNS_R_BADPOINTER";

static const char *a48 = "when input ends unexpected, dns_name_fromwire() " "returns ISC_R_UNEXPECTEDEND";

static const char *a49 =
    "when there is not enough space in target, "
    "dns_name_fromwire(name, source, dcts, downcase, target) " "returns ISC_R_NOSPACE";

static int
test_dns_name_fromwire (char *datafile_name, int testname_offset, int downcase,
                        unsigned int dc_method, char *exp_name, isc_result_t exp_result, size_t buflen)
{
    int result;
    ...
    dns_result = dns_name_fromwire (&dns_name1, &iscbuf1, &dctx, downcase ? ISC_TRUE : ISC_FALSE, &iscbuf2);
...
static void t_dns_name_fromwire_x (const char *testfile, size_t buflen)
{
    int line;
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 6)
            {
                ...
                result = test_dns_name_fromwire (Tokens[0],
                                                 atoi (Tokens[1]),
                                                 atoi (Tokens[2]), dc_method, Tokens[4], exp_result, buflen);
            }
            else
            {
                t_info ("bad format at line %d\n", line);
...
static void t_dns_name_fromwire (void)
{
...
static const char *a51 =
    ...
    "by the compression context cctx, stores the result in " "target and returns DNS_SUCCESS";

static const char *a52 =
    "when not enough space exists in target, " "dns_name_towire(name, cctx, target) returns ISC_R_NOSPACE";
    ...
    if (dns_result == ISC_R_SUCCESS)
    {
        ...
        if (dns_result == exp_result)
        {
            if (exp_result == ISC_R_SUCCESS)
            {
                 ...
                 */
                val = chkdata (buf3, iscbuf3.used, exp_data, exp_data_len);
                if (val == 0)
                    result = T_PASS;
                else
                    result = T_FAIL;
...
static void t_dns_name_towire_x (const char *testfile, size_t buflen)
{
    int line;
    ...
    if (fp != NULL)
    {
        ...
        while ((p = t_fgetbs (fp)) != NULL)
        {
            ...
            if (cnt == 5)
            {
                ...
                result = test_dns_name_towire (Tokens[0], dc_method, Tokens[2], atoi (Tokens[3]), exp_result, buflen);
            }
            else
            {
                t_info ("bad format at line %d\n", line);
...
static const char *a53 =
    ...
    "in target, canonicalizes any bitstring labels " "and returns ISC_R_SUCCESS";

static void t_dns_name_concatenate (void)
{
