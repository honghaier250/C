...
static void log_init (void)
{
    ...
    RUNTIME_CHECK (isc_log_createchannel (lcfg, "_default",
                                          ISC_LOG_TOFILEDESC, ISC_LOG_DYNAMIC, &destination, flags) == ISC_R_SUCCESS);
    RUNTIME_CHECK (isc_log_usechannel (lcfg, "_default", NULL, NULL) == ISC_R_SUCCESS);
    isc_log_setdebuglevel (lctx, level);
}

static void print_addresses (dns_adbfind_t * find)
{
    dns_adbaddrinfo_t *address;

    for (address = ISC_LIST_HEAD (find->list); address != NULL; address = ISC_LIST_NEXT (address, publink))
    {
...
static void do_find (isc_boolean_t want_event)
{
    isc_result_t result;
    ...
    result = dns_adb_createfind (view->adb, task, adb_callback, NULL,
                                 ...
                                 dns_rootname, 0, options, 0, dns_fixedname_name (&target), 0, &find);
...
static void adb_callback (isc_task_t * etask, isc_event_t * event)
{
...
static void run (isc_task_t * task, isc_event_t * event)
{
    UNUSED (task);
    do_find (ISC_TRUE);
    isc_event_free (&event);
...
int main (int argc, char *argv[])
{
    ...
    RUNTIME_CHECK (isc_hash_create (mctx, ectx, DNS_NAME_MAXWIRE) == ISC_R_SUCCESS);
    ...
    taskmgr = NULL;
    RUNTIME_CHECK (isc_taskmgr_create (mctx, workers, 0, &taskmgr) == ISC_R_SUCCESS);
    task = NULL;
    RUNTIME_CHECK (isc_task_create (taskmgr, 0, &task) == ISC_R_SUCCESS);
    isc_task_setname (task, "byname", NULL);

    dispatchmgr = NULL;
    RUNTIME_CHECK (dns_dispatchmgr_create (mctx, NULL, &dispatchmgr) == ISC_R_SUCCESS);
    ...
    RUNTIME_CHECK (dns_cache_create (mctx, taskmgr, timermgr,
                                     dns_rdataclass_in, "rbt", 0, NULL, &cache) == ISC_R_SUCCESS);

    view = NULL;
    RUNTIME_CHECK (dns_view_create (mctx, dns_rdataclass_in, "default", &view) == ISC_R_SUCCESS);

    {
        ...
        if (isc_net_probeipv4 () == ISC_R_SUCCESS)
        {
            ...
            RUNTIME_CHECK (dns_dispatch_getudp (dispatchmgr,
                                                socketmgr,
                                                taskmgr, &any4,
                                                512, 6, 1024, 17, 19, attrs, attrs, &disp4) == ISC_R_SUCCESS);
            INSIST (disp4 != NULL);
        }
        ...
        if (isc_net_probeipv6 () == ISC_R_SUCCESS)
        {
            ...
            RUNTIME_CHECK (dns_dispatch_getudp (dispatchmgr,
                                                socketmgr,
                                                taskmgr, &any6,
                                                512, 6, 1024, 17, 19, attrs, attrs, &disp6) == ISC_R_SUCCESS);
            INSIST (disp6 != NULL);
        }

        RUNTIME_CHECK (dns_view_createresolver (view, taskmgr, 10,
                                                socketmgr, timermgr, 0, dispatchmgr, disp4, disp6) == ISC_R_SUCCESS);
...

    {
        ...
        RUNTIME_CHECK (dns_fwdtable_add (view->fwdtable, dns_rootname, &sal, dns_fwdpolicy_only) == ISC_R_SUCCESS);
    }
    ...
    isc_buffer_init (&b, argv[isc_commandline_index], strlen (argv[isc_commandline_index]));
    isc_buffer_add (&b, strlen (argv[isc_commandline_index]));
    dns_fixedname_init (&name);
    dns_fixedname_init (&target);
    RUNTIME_CHECK (dns_name_fromtext (dns_fixedname_name (&name), &b, dns_rootname, 0, NULL) == ISC_R_SUCCESS);
