...
int main (int argc, char **argv)
{
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            fprintf (stdout, "isc_interfaceiter_current: %s", isc_result_totext (result));
            continue;
        }
        fprintf (stdout, "%s %d %x\n", ifdata.name, ifdata.af, ifdata.flags);
        ...
        {
            res = inet_ntop (ifdata.af, &ifdata.dstaddress.type, buf, sizeof (buf));
        ...
        {
            fprintf (stdout, "isc_interfaceiter_next: %s", isc_result_totext (result));
            continue;
        }
    }
