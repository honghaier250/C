...
static void done (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        for (name = ISC_LIST_HEAD (bevent->names); name != NULL; name = ISC_LIST_NEXT (name, link))
        {
...
int main (int argc, char *argv[])
{
    ...
    na.family = AF_INET;
    if (inet_pton (AF_INET, argv[isc_commandline_index], (char *) &na.type.in) != 1)
    {
        na.family = AF_INET6;
        if (inet_pton (AF_INET6, argv[isc_commandline_index], (char *) &na.type.in6) != 1)
        {
    ...
    result = dns_byaddr_create (mctx, &na, view, options, task, done, NULL, &byaddr);
