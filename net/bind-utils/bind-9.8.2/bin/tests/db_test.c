...
static ISC_LIST (dbinfo) dbs;
     ...
     static void print_result (const char *message, isc_result_t result)
{

    if (message == NULL)
        message = "";
    printf ("%s%sresult %08x: %s\n", message, (*message == '\0') ? "" : " ", result, isc_result_totext (result));
}

static void print_rdataset (dns_name_t * name, dns_rdataset_t * rdataset)
{
    isc_buffer_t text;
    ...
    result = dns_rdataset_totext (rdataset, name, ISC_FALSE, ISC_FALSE, &text);
...
static void print_rdatasets (dns_name_t * name, dns_rdatasetiter_t * rdsiter)
{
    isc_result_t result;
...
static dbinfo *select_db (char *origintext)
{
    ...
    for (dbi = ISC_LIST_HEAD (dbs); dbi != NULL; dbi = ISC_LIST_NEXT (dbi, link))
    {
...
static void list (dbinfo * dbi, char *seektext)
{
    ...
    if (dbi->dbiterator == NULL)
    {
        ...
        {
            if (dbi->version != NULL)
                dns_db_attachversion (dbi->db, dbi->version, &dbi->iversion);
            else
        ...
        if (result == ISC_R_SUCCESS)
        {
            if (seektext != NULL)
            {
                ...
                result = dns_name_fromtext (seekname, &source, dns_db_origin (dbi->db), 0, NULL);
                if (result == ISC_R_SUCCESS)
                    result = dns_dbiterator_seek (dbi->dbiterator, seekname);
            }
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        if (result != ISC_R_SUCCESS && result != DNS_R_NEWORIGIN)
            break;
        result = dns_db_allrdatasets (dbi->db, node, dbi->iversion, 0, &rdsiter);
        if (result != ISC_R_SUCCESS)
        {
...
static isc_result_t load (const char *filename, const char *origintext, isc_boolean_t cache)
{
    dns_fixedname_t forigin;
    ...
    result = dns_db_create (mctx, dbtype, origin,
                            cache ? dns_dbtype_cache : dns_dbtype_zone, dns_rdataclass_in, 0, NULL, &dbi->db);
    if (result != ISC_R_SUCCESS)
    {
...
int main (int argc, char *argv[])
{
    ...
    RUNTIME_CHECK (dns_dbtable_create (mctx, dns_rdataclass_in, &dbtable) == ISC_R_SUCCESS);



    strcpy (dbtype, "rbt");
    while ((ch = isc_commandline_parse (argc, argv, "c:d:t:z:P:Q:glpqvT")) != -1)
    {
        switch (ch)
        {
            case 'c':
                ...
                if (result != ISC_R_SUCCESS)
                    printf ("cache load(%s) %08x: %s\n", isc_commandline_argument, result, isc_result_totext (result));
                break;
            case 'd':
            ...
            case 'z':
                ...
                if (origintext == NULL)
                    ...
                    origintext++;    /* Skip '/'. */
                result = load (isc_commandline_argument, origintext, ISC_FALSE);
                if (result != ISC_R_SUCCESS)
    ...
    while (!done)
    {
        ...
        if (strcmp (s, "!R") == 0)
        {
            ...
            dns_db_currentversion (dbi->db, &dbi->rversions[dbi->rcount]);
        ...
        else if (strcmp (s, "!G") == 0)
        {
            if ((options & DNS_DBFIND_GLUEOK) != 0)
                ...
                options |= DNS_DBFIND_GLUEOK;
            printf ("glue ok = %s\n", ((options & DNS_DBFIND_GLUEOK) != 0) ? "TRUE" : "FALSE");
            continue;
        }
        else if (strcmp (s, "!GV") == 0)
        {
            if ((options & DNS_DBFIND_VALIDATEGLUE) != 0)
                ...
                options |= DNS_DBFIND_VALIDATEGLUE;
            printf ("validate glue = %s\n", ((options & DNS_DBFIND_VALIDATEGLUE) != 0) ? "TRUE" : "FALSE");
            continue;
        }
        else if (strcmp (s, "!WC") == 0)
        {
            if ((options & DNS_DBFIND_NOWILD) != 0)
                ...
                options |= DNS_DBFIND_NOWILD;
            printf ("wildcard matching = %s\n", ((options & DNS_DBFIND_NOWILD) == 0) ? "TRUE" : "FALSE");
            continue;
        }
        ...
        else if (strncmp (s, "!DB ", 4) == 0)
        {
            ...
            else
            {
                ...
                version = NULL;
                origin = dns_rootname;
                printf ("database not found; " "now searching all databases\n");
            }
            continue;
        }
        ...
        if (dbi == NULL)
        {
            ...
            db = NULL;
            result = dns_dbtable_find (dbtable, &name, zcoptions, &db);
            ...
            result = dns_name_totext (dns_db_origin (db), ISC_FALSE, &tb1);
            if (result != ISC_R_SUCCESS)
            {
                printf ("\n");
                print_result ("", result);
            ...
            printf ("\ndatabase = %.*s (%s)\n", (int) r1.length, r1.base, (dns_db_iszone (db)) ? "zone" : "cache");
        }
        node = NULL;
        dns_rdataset_init (&rdataset);
        ...
        if (find_zonecut && dns_db_iscache (db))
        {
            ...
            result = dns_db_findzonecut (db, &name, zcoptions, 0, &node, fname, &rdataset, &sigrdataset);
        }
        else
        {
            result = dns_db_find (db, &name, version, type, options, 0, &node, fname, &rdataset, &sigrdataset);
        }
        ...
        if (found_as && !quiet)
        {
            ...
            printf ("found %.*s as %.*s\n", (int) r1.length, r1.base, (int) r2.length, r2.base);
        }
        ...
        else
        {
            ...
            if (dbi != NULL && addmode && !found_as)
            {
                ...
                if (dns_db_iszone (db))
                    ...
                    addopts = 0;
                result = dns_db_addrdataset (db, node, version, 0, &rdataset, addopts, NULL);
                if (result != ISC_R_SUCCESS)
                    print_result ("", result);
                if (printnode)
                    dns_db_printnode (db, node, stdout);
            ...
            {
                result = dns_db_deleterdataset (db, node, version, type, 0);
                if (result != ISC_R_SUCCESS)
                    print_result ("", result);
                if (printnode)
                    dns_db_printnode (db, node, stdout);
    ...
    if (time_lookups)
    {
        ...
        printf ("elapsed time: %lu.%06lu seconds\n",
                (unsigned long) (usec / 1000000), (unsigned long) (usec % 1000000));
    }
