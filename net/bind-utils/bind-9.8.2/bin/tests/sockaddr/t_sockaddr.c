...
static int test_isc_sockaddr_eqaddrprefix (void)
{
...
static void t1 (void)
{
    ...
    t_assert ("isc_sockaddr_eqaddrprefix", 1, T_REQUIRED,
              ...
              "prefixes of a and b are equal, and ISC_FALSE when " "they are not equal");
    result = test_isc_sockaddr_eqaddrprefix ();
    t_result (result);
}

static int test_isc_netaddr_masktoprefixlen (void)
{
    struct in_addr na_a;
...
static void t2 (void)
{
    ...
    t_assert ("isc_netaddr_masktoprefixlen", 1, T_REQUIRED,
              "isc_netaddr_masktoprefixlen() calculates " "correct prefix lengths ");
