...
static void recvdone (isc_task_t * task, isc_event_t * event)
{
    isc_socketevent_t *sevent = (isc_socketevent_t *) event;
...
static void buildquery (void)
{
    ...
    result = isc_socket_sendto (s, &r, task1, senddone, NULL, &address, NULL);
...
int main (int argc, char *argv[])
{
    ...
    s = NULL;
    RUNTIME_CHECK (isc_socket_create (socketmgr, PF_INET, isc_sockettype_udp, &s) == ISC_R_SUCCESS);
    ...
    key = NULL;
    result = dst_key_fromfile (name, 4017, DNS_KEYALG_DSA, DST_TYPE_PUBLIC | DST_TYPE_PRIVATE, NULL, mctx, &key);
