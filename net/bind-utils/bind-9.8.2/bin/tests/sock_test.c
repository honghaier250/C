...
static void my_shutdown (isc_task_t * task, isc_event_t * event)
{
...
static void my_send (isc_task_t * task, isc_event_t * event)
{
    ...
    printf ("my_send: %s task %p\n\t(sock %p, base %p, length %d, n %d, "
            "result %d)\n",
            (char *) (event->ev_arg), task, sock, dev->region.base, dev->region.length, dev->n, dev->result);
...
static void my_recv (isc_task_t * task, isc_event_t * event)
{
    ...
    printf ("Socket %s (sock %p, base %p, length %d, n %d, result %d)\n",
            (char *) (event->ev_arg), sock, dev->region.base, dev->region.length, dev->n, dev->result);
    if (dev->address.type.sa.sa_family == AF_INET6)
    {
        inet_ntop (AF_INET6, &dev->address.type.sin6.sin6_addr, host, sizeof (host));
        printf ("\tFrom: %s port %d\n", host, ntohs (dev->address.type.sin6.sin6_port));
    }
    else
    {
        inet_ntop (AF_INET, &dev->address.type.sin.sin_addr, host, sizeof (host));
        printf ("\tFrom: %s port %d\n", host, ntohs (dev->address.type.sin.sin_port));
    }
...
static void my_http_get (isc_task_t * task, isc_event_t * event)
{
    isc_socket_t *sock;
...
static void my_connect (isc_task_t * task, isc_event_t * event)
{
    isc_socket_t *sock;
...
static void my_listen (isc_task_t * task, isc_event_t * event)
{
    char *name = event->ev_arg;
    ...
    if (dev->result == ISC_R_SUCCESS)
    {
         ...
         */
        newtask = NULL;
        RUNTIME_CHECK (isc_task_create (manager, 0, &newtask) == ISC_R_SUCCESS);
        isc_socket_recv (dev->newsocket, &region, 1, newtask, my_recv, event->ev_arg);
...
int main (int argc, char *argv[])
{
     ...
     */
    manager = NULL;
    RUNTIME_CHECK (isc_taskmgr_create (mctx, workers, 0, &manager) == ISC_R_SUCCESS);
    ...
    RUNTIME_CHECK (isc_task_onshutdown (t1, my_shutdown, "1") == ISC_R_SUCCESS);
    RUNTIME_CHECK (isc_task_onshutdown (t2, my_shutdown, "2") == ISC_R_SUCCESS);
    ...
    RUNTIME_CHECK (isc_socket_create (socketmgr, pf, isc_sockettype_tcp, &so1) == ISC_R_SUCCESS);
    ...
    RUNTIME_CHECK (isc_timer_create (timgr, isc_timertype_once, &expires,
                                     &interval, t1, timeout, so1, &ti1) == ISC_R_SUCCESS);
    ...
    RUNTIME_CHECK (isc_socket_create (socketmgr, isc_sockaddr_pf (&sockaddr),
                                      isc_sockettype_tcp, &so2) == ISC_R_SUCCESS);

    RUNTIME_CHECK (isc_socket_connect (so2, &sockaddr, t2, my_connect, "so2") == ISC_R_SUCCESS);
