...
static void hex_dump (const char *msg, void *data, unsigned int length)
{
    unsigned int len;
...
static void CHECK (const char *msg, isc_result_t result)
{
...
int main (int argc, char **argv)
{
    ...
    mctx = NULL;
    CHECK ("isc_mem_create()", isc_mem_create (0, 0, &mctx));

    ent = NULL;
    CHECK ("isc_entropy_create()", isc_entropy_create (mctx, &ent));

    isc_entropy_stats (ent, stderr);

#if 1
    CHECK ("isc_entropy_createfilesource() 1", isc_entropy_createfilesource (ent, "/dev/random"));
    CHECK ("isc_entropy_createfilesource() 2", isc_entropy_createfilesource (ent, "/dev/random"));
#else
    CHECK ("isc_entropy_createfilesource() 3", isc_entropy_createfilesource (ent, "/tmp/foo"));
#endif

    fprintf (stderr, "Reading 32 bytes of GOOD random data only, partial OK\n");
  ...
  any:
    isc_entropy_stats (ent, stderr);
    CHECK ("isc_entropy_getdata() pseudorandom", isc_entropy_getdata (ent, buffer, 128, NULL, 0));
    ...
    result = isc_entropy_getdata (ent, buffer, sizeof (buffer), &returned, flags);
