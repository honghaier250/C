...
static void shutdown_action (isc_task_t * task, isc_event_t * event)
{
    t_info *info = event->ev_arg;
    ...
    if (strcmp (info->name, "0") == 0)
    {
        ...
        nevent = isc_event_allocate (info->mctx, info, T2_SHUTDOWNOK, t2_shutdown, &tasks[1], sizeof (*event));
...
static void foo_event (isc_task_t * task, isc_event_t * event)
{
    printf ("task(%p) foo\n", task);
    isc_event_free (&event);
}
...
static void tick (isc_task_t * task, isc_event_t * event)
{
    ...
    if (strcmp (info->name, "1") == 0)
    {
        ...
        else if (info->ticks >= 15 && info->exiting)
        {
            isc_timer_detach (&info->timer);
            isc_task_detach (&info->task);
            nevent = isc_event_allocate (info->mctx, info, T2_SHUTDOWNDONE, t1_shutdown, &tasks[0], sizeof (*event));
            RUNTIME_CHECK (nevent != NULL);
    ...
    {
        isc_timer_detach (&info->timer);
        nevent = isc_event_allocate (info->mctx, info, FOO_EVENT, foo_event, task, sizeof (*event));
        RUNTIME_CHECK (nevent != NULL);
...
static t_info *new_task (isc_mem_t * mctx, const char *name)
{
    t_info *ti;
    ...
    RUNTIME_CHECK (isc_task_create (task_manager, 0, &ti->task) == ISC_R_SUCCESS);
    RUNTIME_CHECK (isc_task_onshutdown (ti->task, shutdown_action, ti) == ISC_R_SUCCESS);

    isc_time_settoepoch (&expires);
    isc_interval_set (&interval, 1, 0);
    RUNTIME_CHECK (isc_timer_create (timer_manager, isc_timertype_ticker,
                                     &expires, &interval, ti->task, tick, ti, &ti->timer) == ISC_R_SUCCESS);
...
int main (int argc, char *argv[])
{
    ...
    RUNTIME_CHECK (isc_taskmgr_create (mctx, workers, 0, &task_manager) == ISC_R_SUCCESS);
    RUNTIME_CHECK (isc_timermgr_create (mctx, &timer_manager) == ISC_R_SUCCESS);
