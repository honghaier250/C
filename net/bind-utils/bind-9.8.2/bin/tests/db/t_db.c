          ...
          const char *model, isc_mem_t * mctx, dns_db_t ** db)
{
    ...
    dns_result = dns_name_fromtext (dns_fixedname_name (&dns_origin), &origin_buffer, NULL, 0, NULL);
    ...
    dns_result = dns_db_create (mctx, db_type, dns_fixedname_name (&dns_origin), dbtype, rdataclass, 0, NULL, db);
    if (dns_result != ISC_R_SUCCESS)
...
static int t_dns_db_load (char **av)
{
    char *filename;
    ...
    if (dns_result != exp_load_result)
    {
        t_info ("dns_db_load returned %s, expected %s\n",
                dns_result_totext (dns_result), dns_result_totext (exp_load_result));
        dns_db_detach (&db);
        isc_hash_destroy ();
        isc_entropy_detach (&ectx);
        isc_mem_destroy (&mctx);
    ...
    dns_result = dns_name_fromtext (dns_fixedname_name (&dns_findname), &findname_buffer, NULL, 0, NULL);
    if (dns_result != ISC_R_SUCCESS)
    {
        t_info ("dns_name_fromtext failed %s\n", dns_result_totext (dns_result));
        dns_db_detach (&db);
    ...
    dns_result = dns_db_find (db,
                              ...
                              versionp,
                              rdatatype,
                              DNS_DBFIND_GLUEOK, 0, &nodep, dns_fixedname_name (&dns_foundname), &rdataset, NULL);

    if (dns_result != exp_find_result)
    {
        t_info ("dns_db_find returned %s, expected %s\n",
                dns_result_totext (dns_result), dns_result_totext (exp_find_result));
        result = T_FAIL;
    }
    else
    {
...
static const char *a1 =
    "A call to dns_db_load(db, filename) loads the contents of " "the database in filename into db.";
...
static const char *a2 = "When the database db has cache semantics, a call to " "dns_db_iscache(db) returns ISC_TRUE.";
...
static int
test_dns_db_zc_x (const char *filename, dns_dbtype_t dbtype, isc_boolean_t (*cf) (dns_db_t *), isc_boolean_t exp_result)
{
...
static void t2 (void)
{
    ...
    result = test_dns_db_zc_x ("dns_db_iscache_1_data", dns_dbtype_cache, dns_db_iscache, ISC_TRUE);
    t_result (result);
}


static const char *a3 = "When the database db has zone semantics, a call to " "dns_db_iscache(db) returns ISC_FALSE.";
...
static void t3 (void)
{
    ...
    result = test_dns_db_zc_x ("dns_db_iscache_2_data", dns_dbtype_zone, dns_db_iscache, ISC_FALSE);
    t_result (result);
}


static const char *a4 = "When the database db has zone semantics, a call to " "dns_db_iszone(db) returns ISC_TRUE.";
...
static void t4 (void)
{
    ...
    result = test_dns_db_zc_x ("dns_db_iszone_1_data", dns_dbtype_zone, dns_db_iszone, ISC_TRUE);
    t_result (result);
}


static const char *a5 = "When the database db has cache semantics, a call to " "dns_db_iszone(db) returns ISC_FALSE.";
...
static void t5 (void)
{
    ...
    result = test_dns_db_zc_x ("dns_db_iszone_2_data", dns_dbtype_cache, dns_db_iszone, ISC_FALSE);
    t_result (result);
}

static int t_dns_db_origin (char **av)
{
...
static const char *a6 = "A call to dns_db_origin(db) returns the origin of the database.";
...
static const char *a7 = "A call to dns_db_class(db) returns the class of the database.";


#define    CLASSBUFLEN    256

static int t_dns_db_class (char **av)
{
    ...
    else
    {
        ...
        dns_rdataclass_format (db_rdataclass, classbuf, sizeof (classbuf));
...
static const char *a8 = "A call to dns_db_currentversion() opens the current " "version for reading.";

static int t_dns_db_currentversion (char **av)
{
    char *filename;
    ...
    if (dns_result != ISC_R_SUCCESS)
    {
        t_info ("dns_rdatatype_fromtext %s failed %s\n", findtype, dns_result_totext (dns_result));
        dns_db_detach (&db);
        isc_hash_destroy ();
        isc_entropy_detach (&ectx);
        isc_mem_destroy (&mctx);
    ...
    dns_result = dns_db_find (db,
                              dns_fixedname_name (&dns_findname),
                              cversionp, rdatatype, 0, 0, &nodep, dns_fixedname_name (&dns_foundname), &rdataset, NULL);
    ...
    dns_result = dns_db_find (db,
                              dns_fixedname_name (&dns_findname),
                              nversionp, rdatatype, 0, 0, &nodep, dns_fixedname_name (&dns_foundname), &rdataset, NULL);
    ...
    dns_result = dns_db_find (db, dns_fixedname_name (&dns_findname),
                              ...
                              0, &nodep, dns_fixedname_name (&dns_foundname), &rdataset, NULL);
...
static const char *a9 = "A call to dns_db_newversion() opens a new version for " "reading and writing.";

static int t_dns_db_newversion (char **av)
{
    ...
    dns_result = dns_name_fromtext (dns_fixedname_name (&dns_newname), &newname_buffer, NULL, 0, NULL);
    if (dns_result != ISC_R_SUCCESS)
    {
        t_info ("dns_name_fromtext failed %s\n", dns_result_totext (dns_result));
        dns_db_detach (&db);
    ...
    nodep = NULL;
    dns_result = dns_db_findnode (db, dns_fixedname_name (&dns_newname), ISC_TRUE, &nodep);
    if (dns_result != ISC_R_SUCCESS)
    {
    ...
    if (dns_result != ISC_R_SUCCESS)
    {
        t_info ("dns_rdatatype_fromtext %s failed %s\n", newtype, dns_result_totext (dns_result));
        dns_db_detachnode (db, &nodep);
        dns_db_detach (&db);
        isc_hash_destroy ();
        isc_entropy_detach (&ectx);
    ...
    dns_result = dns_db_addrdataset (db, nodep, nversionp, 0, &added_rdataset, 0, NULL);
    if (dns_result != ISC_R_SUCCESS)
    {
    ...
    dns_result = dns_db_find (db, dns_fixedname_name (&dns_newname),
                              ...
                              dns_fixedname_name (&dns_foundname), &found_rdataset, NULL);
...
static const char *a10 =
    ...
    "causes all changes made in the version to take effect, " "and returns ISC_R_SUCCESS.";

static int t_dns_db_closeversion_1 (char **av)
{
    char *filename;
    ...
    dns_result = dns_name_fromtext (dns_fixedname_name (&dns_existingname), &name_buffer, NULL, 0, NULL);
    if (dns_result != ISC_R_SUCCESS)
    {
        t_info ("dns_name_fromtext failed %s\n", dns_result_totext (dns_result));
        dns_db_detach (&db);
    ...
    if (dns_result != ISC_R_SUCCESS)
    {
        t_info ("dns_rdatatype_fromtext %s failed %s\n", existing_type, dns_result_totext (dns_result));
        dns_db_detachnode (db, &nodep);
        dns_db_detach (&db);
        isc_hash_destroy ();
        isc_entropy_detach (&ectx);
    ...
    nodep = NULL;
    dns_result = dns_db_findnode (db, dns_fixedname_name (&dns_existingname), ISC_FALSE, &nodep);
    if (dns_result != ISC_R_SUCCESS)
    {
    ...
    isc_buffer_add (&name_buffer, len);
    dns_result = dns_name_fromtext (dns_fixedname_name (&dns_newname), &name_buffer, NULL, 0, NULL);
    if (dns_result != ISC_R_SUCCESS)
    {
        t_info ("dns_name_fromtext failed %s\n", dns_result_totext (dns_result));
        dns_db_closeversion (db, &nversionp, ISC_FALSE);
    ...
    dns_result = dns_db_find (db,
                              ...
                              cversionp,
                              new_rdatatype, 0, 0, &nodep, dns_fixedname_name (&dns_foundname), &found_rdataset, NULL);
    ...
    dns_result = dns_db_find (db, dns_fixedname_name (&dns_existingname),
                              ...
                              0, 0, &nodep, dns_fixedname_name (&dns_foundname), &found_rdataset, NULL);
    ...
    if ((dns_result != ISC_R_NOTFOUND) && (dns_result != DNS_R_NXDOMAIN))
    {
        dns_rdataset_disassociate (&found_rdataset);
        dns_db_detachnode (db, &nodep);
        t_info ("dns_db_find %s returned %s\n", existing_name, dns_result_totext (dns_result));
        ++nfails;
    }
...
static const char *a11 =
    ...
    "causes all changes made in the version to to be rolled back, " "and returns ISC_R_SUCCESS.";

static int t_dns_db_closeversion_2 (char **av)
{
    char *filename;
    ...
    if ((dns_result == ISC_R_NOTFOUND) || (dns_result == DNS_R_NXDOMAIN) || (dns_result == DNS_R_NXRRSET))
    {
    ...
    if ((dns_result != ISC_R_NOTFOUND) && (dns_result != DNS_R_NXDOMAIN))
    {
        t_info ("dns_db_find %s returned %s\n", new_name, dns_result_totext (dns_result));
        dns_rdataset_disassociate (&found_rdataset);
        dns_db_detachnode (db, &nodep);
        dns_db_closeversion (db, &cversionp, ISC_FALSE);
        dns_db_detach (&db);
    ...
    dns_result = dns_db_find (db, dns_fixedname_name (&dns_existingname),
                              ...
                              &nodep, dns_fixedname_name (&dns_foundname), &found_rdataset, NULL);
...
static const char *a12 =
    ...
    "which expire at or before 'now'. If 'now' is zero, then the current  " "time will be used.";

static int t_dns_db_expirenode (char **av)
{
    char *filename;
    ...
    dns_result = dns_db_find (db,
                              ...
                              NULL,
                              dns_rdatatype_any,
                              0, find_expire_time, &nodep, dns_fixedname_name (&dns_foundname), &rdataset, NULL);
...
static const char *a13 =
    ...
    "to point to the node and returns ISC_R_SUCCESS, otherwise " "it returns ISC_R_NOTFOUND.";

static int t_dns_db_findnode_1 (char **av)
{
    char *filename;
    ...
    isc_buffer_add (&name_buffer, len);
    dns_result = dns_name_fromtext (dns_fixedname_name (&dns_name), &name_buffer, NULL, 0, NULL);
    if (dns_result != ISC_R_SUCCESS)
    {
        t_info ("dns_name_fromtext failed %s\n", dns_result_totext (dns_result));
        dns_db_detach (&db);
    ...
    dns_result = dns_db_findnode (db, dns_fixedname_name (&dns_name), ISC_FALSE, &nodep);
    if (dns_result != exp_result)
    {
        t_info ("dns_db_findnode failed %s\n", dns_result_totext (dns_result));
        if (dns_result == ISC_R_SUCCESS)
    ...
    if (dns_result == ISC_R_SUCCESS)
    {
        ...
        dns_result = dns_db_findrdataset (db, nodep, cversionp, rdatatype, 0, 0, &rdataset, NULL);
        if (dns_result == ISC_R_SUCCESS)
        {
            dns_rdataset_disassociate (&rdataset);
            result = T_PASS;
...
static const char *a14 =
    ...
    "creates the node, initializes nodep to point to the node, " "and returns ISC_R_SUCCESS.";

static int t_dns_db_findnode_2 (char **av)
{
    char *filename;
     ...
     * Add it.
     */
    dns_result = dns_db_findnode (db, dns_fixedname_name (&dns_name), ISC_TRUE, &nodep);
    if (dns_result != ISC_R_SUCCESS)
    {
        t_info ("dns_db_findnode %s\n", dns_result_totext (dns_result));
        dns_db_detach (&db);
    ...
    dns_result = dns_db_find (db,
                              ...
                              cversionp,
                              dns_rdatatype_any, 0, 0, &newnodep, dns_fixedname_name (&dns_foundname), &rdataset, NULL);
    if ((dns_result != ISC_R_NOTFOUND) && (dns_result != DNS_R_NXDOMAIN))
    {
    ...
    {
        t_info ("dns_db_findnode %s failed %s\n", newname, dns_result_totext (dns_result));
        ++nfails;
    }
...
static int t_dns_db_find_x (char **av)
{
    char *dbfile;
    ...
    dns_result = dns_db_find (db,
                              dns_fixedname_name (&dns_findname),
                              cversionp,
                              rdatatype, opts, ftime, &nodep, dns_fixedname_name (&dns_foundname), &rdataset, NULL);

    if (dns_result != exp_result)
    {
        t_info ("dns_db_find %s %s unexpectedly returned %s, "
                "expected %s\n", findname, findtype, dns_result_totext (dns_result), dns_result_totext (exp_result));
        result = T_FAIL;
    }
    else
    {
...
static const char *a16 =
    ...
    "dns_db_find(db, name, version, type, options, now, ...)  " "returns DNS_R_GLUE.";
...
static const char *a17 =
    "A call to dns_db_find() returns DNS_R_DELEGATION when the data " "requested is beneath a zone cut.";
...
static const char *a19 = "A call to dns_db_find() returns DNS_R_DNAME when the data " "requested is beneath a DNAME.";
...
static const char *a21 = "A call to dns_db_find() returns DNS_R_NXDOMAIN when name " "does not exist.";
...
static const char *a22 =
    "A call to dns_db_find() returns DNS_R_NXRRSET when " "the desired name exists, but the desired type does not.";
...
static const char *a23 =
    ...
    "returns ISC_R_NOTFOUND when the desired name does not exist, " "and no delegation could be found.";
