...
static void ltick (isc_task_t * task, isc_event_t * event)
{
    UNUSED (task);
    printf ("** ltick%s **\n", (event->ev_attributes & ISC_EVENTATTR_CANCELED) != 0 ? " (canceled)" : "");
...
int main (int argc, char *argv[])
{
    ...
    RUNTIME_CHECK (isc_taskmgr_create (mctx, 3, 0, &taskmgr) == ISC_R_SUCCESS);
    ...
    RUNTIME_CHECK (isc_ratelimiter_setinterval (rlim, &linterval) == ISC_R_SUCCESS);
    ...
    for (i = 0; i < NEVENTS; i++)
    {
        ...
        RUNTIME_CHECK (isc_timer_create (timermgr,
                                         ...
                                         &uinterval, g_task, schedule[i].fun, NULL, &timers[i]) == ISC_R_SUCCESS);
    }
