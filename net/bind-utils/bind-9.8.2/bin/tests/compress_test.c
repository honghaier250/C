...
unsigned char plain[] = "\003yyy\003foo\0\003bar\003yyy\003foo\0\003" "bar\003yyy\003foo\0\003xxx\003bar\003foo";
...
void test (unsigned int, dns_name_t *, dns_name_t *, dns_name_t *, unsigned char *, unsigned int);
...
int main (int argc, char *argv[])
{
    ...
    test (DNS_COMPRESS_GLOBAL14, &name1, &name2, &name3, plain, sizeof (plain));
      ...
      dns_name_t * name3, unsigned char *result, unsigned int length)
{
    ...
    if (verbose)
    {
        ...
        switch (allowed)
        {
            case DNS_COMPRESS_NONE:
                s = "DNS_COMPRESS_NONE";
                break;
            case DNS_COMPRESS_GLOBAL14:
                s = "DNS_COMPRESS_GLOBAL14";
                break;
    ...
    if (raw)
    {
        ...
        {
            fprintf (stdout, "%02x", ((unsigned char *) source.base)[i]);
            if ((++i % 20) == 0)
                fputs ("\n", stdout);
    ...
    RUNTIME_CHECK (dns_name_fromwire (&name, &source, &dctx, ISC_FALSE, &target) == ISC_R_SUCCESS);
    ...
    if (raw)
    {
        ...
        {
            fprintf (stdout, "%02x", ((unsigned char *) target.base)[i]);
            if ((++i % 20) == 0)
                fputs ("\n", stdout);
            else if (i == target.used)
                fputs ("\n", stdout);
            else
                fputs (" ", stdout);
        }
