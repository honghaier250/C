...
static void absolute_inflate (struct timeval *vt, struct timeval *rt)
{
    double d;
    ...
    if ((epoch.tv_sec > vt->tv_sec) || ((epoch.tv_sec == vt->tv_sec) && (epoch.tv_usec > vt->tv_usec)))
        return;
...
static void absolute_deflate (struct timeval *rt, struct timeval *vt)
{
    double d;
    ...
    if ((epoch.tv_sec > rt->tv_sec) || ((epoch.tv_sec == rt->tv_sec) && (epoch.tv_usec > rt->tv_usec)))
        return;
...
static void interval_inflate (struct timeval *vt, struct timeval *rt)
{
...
static void interval_deflate (struct timeval *rt, struct timeval *vt)
{
    struct timeval now, tv;
...
#ifdef SYS_select
int select (int nfds, fd_set * rfds, fd_set * wfds, fd_set * xfds, struct timeval *timeout)
{
#ifdef VIRTUAL_TIME
    struct timeval tv;
    ...
    if (epoch.tv_sec == 0 || timeout == NULL || (timeout->tv_sec == 0 && timeout->tv_usec == 0))
        ...
        struct kevent *eventlist, int nevents, const struct timespec *timeout)
{
    ...
    if (epoch.tv_sec == 0 || timeout == NULL || (timeout->tv_sec == 0 && timeout->tv_nsec == 0))
    ...
    return syscall (SYS_kevent, kq, changelist, nchanges, eventlist, nevents, &ts);
#else
    return syscall (SYS_kevent, kq, changelist, nchanges, eventlist, nevents, timeout);
#endif
}
