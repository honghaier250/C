...
static void test_atomic_xadd ()
{
    ...
    t_assert ("test_atomic_xadd", 1, T_REQUIRED, "%s", "ensure that isc_atomic_xadd() works.");
    ...
    for (i = 0; i < TASKS; i++)
    {
        ...
        event = isc_event_allocate (mctx, NULL, 1000, do_xadd, &counters[i], sizeof (struct isc_event));
...
static void test_atomic_xaddq ()
{
    ...
    t_assert ("test_atomic_xaddq", 1, T_REQUIRED, "%s", "ensure that isc_atomic_xaddq() works.");
    ...
    for (i = 0; i < TASKS; i++)
    {
        tasks[i] = NULL;
        CHECK (isc_task_create (task_manager, 0, &tasks[i]));
        event = isc_event_allocate (mctx, NULL, 1000, do_xaddq, &counters[i], sizeof (struct isc_event));
        isc_task_sendanddetach (&tasks[i], &event);
    }
