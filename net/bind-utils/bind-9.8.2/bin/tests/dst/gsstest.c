...
static void console (isc_task_t * task, isc_event_t * event)
{
    char buf[32];
    ...
    for (;;)
    {
        ...
        {
            ev = isc_event_allocate (mctx, (void *) 1, 1, initctx1, NULL, sizeof (*event));
        ...
        {
            ev = isc_event_allocate (mctx, (void *) 1, 1, sendquery, NULL, sizeof (*event));
            isc_task_send (task, &ev);
            return;
        }
...
static void recvresponse (isc_task_t * task, isc_event_t * event)
{
    ...
    result2 = dns_request_getresponse (reqev->request, response, DNS_MESSAGEPARSE_PRESERVEORDER);
    isc_buffer_init (&outbuf, output, sizeof (output));
    result = dns_message_totext (response, &dns_master_style_debug, 0, &outbuf);
    ...
    event = isc_event_allocate (mctx, (void *) 1, 1, console, NULL, sizeof (*event));
...
static void sendquery (isc_task_t * task, isc_event_t * event)
{
    ...
    result = dns_name_fromtext (dns_fixedname_name (&queryname), &buf, dns_rootname, 0, NULL);
    ...
    dns_rdataset_makequestion (qrdataset, dns_rdataclass_in, dns_rdatatype_a);
    ISC_LIST_APPEND (qname->list, qrdataset, link);
    dns_message_addname (message, qname, DNS_SECTION_QUESTION);

    result = dns_request_create (requestmgr, message, &address, 0, tsigkey,
                                 TIMEOUT, task, recvresponse, message, &request);
    CHECK ("dns_request_create", result);

    printf ("Submitting query:\n");
    isc_buffer_init (&outbuf, output, sizeof (output));
    result = dns_message_totext (message, &dns_master_style_debug, 0, &outbuf);
    CHECK ("dns_message_totext", result);
    printf ("%.*s\n", (int) isc_buffer_usedlength (&outbuf), (char *) isc_buffer_base (&outbuf));
...
static void initctx2 (isc_task_t * task, isc_event_t * event)
{
    dns_requestevent_t *reqev = (dns_requestevent_t *) event;
    ...
    result = dns_request_getresponse (reqev->request, response, DNS_MESSAGEPARSE_PRESERVEORDER);
    ...
    result = dns_tkey_processgssresponse (query, response,
                                          dns_fixedname_name (&gssname), &gssctx, &outtoken, &tsigkey, ring, NULL);
    gssctx = *gssctxp;
...
static void initctx1 (isc_task_t * task, isc_event_t * event)
{
    ...
    result = dns_name_fromtext (dns_fixedname_name (&servername), &buf, dns_rootname, 0, NULL);
    CHECK ("dns_name_fromtext", result);
    ...
    result = dns_name_fromtext (dns_fixedname_name (&gssname), &buf, dns_rootname, 0, NULL);
    CHECK ("dns_name_fromtext", result);
    ...
    result = dns_tkey_buildgssquery (query, dns_fixedname_name (&servername),
                                     dns_fixedname_name (&gssname), NULL, 36000, &gssctx, ISC_TRUE, mctx, NULL);
    CHECK ("dns_tkey_buildgssquery", result);

    printf ("Sending context token to server\n");
    request = NULL;
    result = dns_request_create (requestmgr, query, &address, 0, NULL, TIMEOUT, task, initctx2, query, &request);
    CHECK ("dns_request_create", result);
...
int main (int argc, char *argv[])
{
    ...
    attrs = DNS_DISPATCHATTR_UDP | DNS_DISPATCHATTR_MAKEQUERY | DNS_DISPATCHATTR_IPV4;
    attrmask = DNS_DISPATCHATTR_UDP | DNS_DISPATCHATTR_TCP | DNS_DISPATCHATTR_IPV4 | DNS_DISPATCHATTR_IPV6;
    dispatchv4 = NULL;
    RUNCHECK (dns_dispatch_getudp (dispatchmgr, socketmgr, taskmgr,
                                   &bind_any, 4096, 4, 2, 3, 5, attrs, attrmask, &dispatchv4));
    requestmgr = NULL;
    RUNCHECK (dns_requestmgr_create (mctx, timermgr, socketmgr, taskmgr, dispatchmgr, dispatchv4, NULL, &requestmgr));
    ...
    sock = NULL;
    RUNCHECK (isc_socket_create (socketmgr, PF_INET, isc_sockettype_udp, &sock));
