...
static void use (dst_key_t * key, isc_mem_t * mctx)
{
    isc_result_t ret;
    ...
    {
        printf ("contextcreate(%d) returned: %s\n", dst_key_alg (key), isc_result_totext (ret));
        return;
    }
...
static void dns (dst_key_t * key, isc_mem_t * mctx)
{
    ...
    if (ret != ISC_R_SUCCESS)
        return;
    ret = dst_key_fromdns (dst_key_name (key), dns_rdataclass_in, &buf1, mctx, &newkey);
    ...
    match = ISC_TF (r1.length == r2.length && memcmp (r1.base, r2.base, r1.length) == 0);
    printf ("compare(%d): %s\n", dst_key_alg (key), match ? "true" : "false");
    dst_key_free (&newkey);
}

static void io (dns_name_t * name, int id, int alg, int type, isc_mem_t * mctx)
{
    dst_key_t *key = NULL;
...
static void dh (dns_name_t * name1, int id1, dns_name_t * name2, int id2, isc_mem_t * mctx)
{
...
static void generate (int alg, isc_mem_t * mctx)
{
    isc_result_t ret;

    dst_key_t *key = NULL;

    ret = dst_key_generate (dns_rootname, alg, 512, 0, 0, 0, dns_rdataclass_in, mctx, &key);
...
int main (void)
{
    ...
    io (name, 54622, DST_ALG_RSAMD5, DST_TYPE_PRIVATE | DST_TYPE_PUBLIC, mctx);
