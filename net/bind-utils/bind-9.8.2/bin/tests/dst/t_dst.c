...
static void cleandir (char *path)
{
    DIR *dirp;
...
static void use (dst_key_t * key, isc_mem_t * mctx, isc_result_t exp_result, int *nfails)
{
    ...
    if (ret != exp_result)
    {
        t_info ("dst_context_create(%d) returned (%s) expected (%s)\n",
                dst_key_alg (key), dst_result_totext (ret), dst_result_totext (exp_result));
        ++*nfails;
        return;
    }
...
static void
io (dns_name_t * name, int id, int alg, int type, isc_mem_t * mctx, isc_result_t exp_result, int *nfails, int *nprobs)
{
    dst_key_t *key = NULL;
...
static void generate (int alg, isc_mem_t * mctx, int size, int *nfails)
{
    isc_result_t ret;

    dst_key_t *key = NULL;

    ret = dst_key_generate (dns_rootname, alg, size, 0, 0, 0, dns_rdataclass_in, mctx, &key);
    if (ret != ISC_R_SUCCESS)
    {
...
static const char *a1 =
    ...
    "sign and verify data using the RSA, DSA and MD5 algorithms, " "and compute Diffie-Hellman shared secrets.";
static void t1 (void)
{
    isc_mem_t *mctx;
    ...
    io (name, 23616, DST_ALG_DSA, DST_TYPE_PRIVATE | DST_TYPE_PUBLIC, mctx, ISC_R_SUCCESS, &nfails, &nprobs);
    t_info ("testing use of stored keys [2]\n");
    io (name, 54622, DST_ALG_RSAMD5, DST_TYPE_PRIVATE | DST_TYPE_PUBLIC, mctx, ISC_R_SUCCESS, &nfails, &nprobs);

    t_info ("testing use of stored keys [3]\n");
    io (name, 49667, DST_ALG_DSA, DST_TYPE_PRIVATE | DST_TYPE_PUBLIC, mctx, DST_R_NULLKEY, &nfails, &nprobs);
    t_info ("testing use of stored keys [4]\n");
    io (name, 2, DST_ALG_RSAMD5, DST_TYPE_PRIVATE | DST_TYPE_PUBLIC, mctx, DST_R_NULLKEY, &nfails, &nprobs);
 ...
 */
static int sig_tofile (char *path, isc_buffer_t * buf)
{
    int rval;
 ...
 */
static int sig_fromfile (char *path, isc_buffer_t * iscbuf)
{
    int rval;
...
t2_sigchk (char *datapath, char *sigpath, char *keyname,
           int id, int alg, int type, isc_mem_t * mctx, char *expected_result, int *nfails, int *nprobs)
{
    int rval;
    ...
    if (((exp_res == 0) && (isc_result != ISC_R_SUCCESS)) || ((exp_res != 0) && (isc_result == ISC_R_SUCCESS)))
    {

        t_info ("dst_context_verify returned %s, expected %s\n", isc_result_totext (isc_result), expected_result);
        dst_context_destroy (&ctx);
        ++*nfails;
    }
...
static const char *a2 =
    "the dst module provides the capability to " "verify data signed with the RSA and DSA algorithms";

/*
 * av ==  datafile, sigpath, keyname, keyid, alg, exp_result.
 */
static int t2_vfy (char **av)
{
    char *datapath;
    ...
    t2_sigchk (datapath, sigpath, keyname, keyid,
               algid, DST_TYPE_PRIVATE | DST_TYPE_PUBLIC, mctx, exp_result, &nfails, &nprobs);
