...
static void tx_sde (isc_task_t * task, isc_event_t * event)
{
...
static void tx_te (isc_task_t * task, isc_event_t * event)
{
    isc_result_t isc_result;
...
t_timers_x (isc_timertype_t timertype, isc_time_t * expires,
            isc_interval_t * interval, void (*action) (isc_task_t *, isc_event_t *))
{
    char *p;
    ...
    Tx_timer = NULL;
    isc_result = isc_timer_create (timermgr, timertype, expires, interval, task, action, (void *) timertype, &Tx_timer);
...
static const char *a2 =
    ...
    "specified task when the current time reaches or exceeds the time " "specified by 'expires'.";
...
static void t3_te (isc_task_t * task, isc_event_t * event)
{
    isc_result_t isc_result;
...
static void t4_te (isc_task_t * task, isc_event_t * event)
{
    ...
    if (Tx_eventcnt < 3)
    {
        ...
        if (Tx_eventcnt == 2)
        {
            ...
            {
                isc_interval_set (&interval, 0, 0);
                isc_result = isc_timer_reset (Tx_timer, isc_timertype_once, &expires, &interval, ISC_FALSE);
                if (isc_result != ISC_R_SUCCESS)
                {
...
static const char *a4 =
    "A call to isc_timer_reset() changes the timer's type, expires and " "interval values to the given values.";
...
static void t5_start_event (isc_task_t * task, isc_event_t * event)
{
    isc_result_t isc_result;
...
static void t5_tick_event (isc_task_t * task, isc_event_t * event)
{
    isc_result_t isc_result;
    ...
    if (T5_eventcnt == 1)
    {
        ...
        isc_result = isc_timer_reset (T5_tickertimer, isc_timertype_ticker, &expires, &interval, ISC_TRUE);
        if (isc_result != ISC_R_SUCCESS)
        {
            t_info ("isc_timer_reset failed %s\n", isc_result_totext (isc_result));
            ++T5_nfails;
...
static void t5_once_event (isc_task_t * task, isc_event_t * event)
{
...
static void t5_shutdown_event (isc_task_t * task, isc_event_t * event)
{
...
static int t_timers5 (void)
{
    ...
    isc_result = isc_timer_create (timermgr, isc_timertype_ticker,
                                   &expires, &interval, T5_task1, t5_tick_event, NULL, &T5_tickertimer);
    ...
    isc_result = isc_timer_create (timermgr, isc_timertype_once,
                                   &expires, &interval, T5_task2, t5_once_event, NULL, &T5_oncetimer);
