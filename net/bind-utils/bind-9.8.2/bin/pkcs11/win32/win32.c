...
C_OpenSession (CK_SLOT_ID slotID,
               ...
               CK_RV (*Notify) (CK_SESSION_HANDLE hSession,
                                CK_NOTIFICATION event, CK_VOID_PTR pApplication), CK_SESSION_HANDLE_PTR phSession)
{
...
CK_RV
C_CreateObject (CK_SESSION_HANDLE hSession, CK_ATTRIBUTE_PTR pTemplate, CK_ULONG usCount, CK_OBJECT_HANDLE_PTR phObject)
{
...
CK_RV
C_GetAttributeValue (CK_SESSION_HANDLE hSession, CK_OBJECT_HANDLE hObject, CK_ATTRIBUTE_PTR pTemplate, CK_ULONG usCount)
{
    CK_C_GetAttributeValue sym;

    if (hPK11 == NULL)
        return 0xfe;
    sym = (CK_C_GetAttributeValue) GetProcAddress (hPK11, "C_GetAttributeValue");
    if (sym == NULL)
        return 0xff;
...
CK_RV
C_SetAttributeValue (CK_SESSION_HANDLE hSession, CK_OBJECT_HANDLE hObject, CK_ATTRIBUTE_PTR pTemplate, CK_ULONG usCount)
{
    CK_C_SetAttributeValue sym;

    if (hPK11 == NULL)
        return 0xfe;
    sym = (CK_C_SetAttributeValue) GetProcAddress (hPK11, "C_SetAttributeValue");
    if (sym == NULL)
        return 0xff;
    return (*sym) (hSession, hObject, pTemplate, usCount);
}
...
CK_RV C_FindObjectsInit (CK_SESSION_HANDLE hSession, CK_ATTRIBUTE_PTR pTemplate, CK_ULONG usCount)
{
    ...
    if (hPK11 == NULL)
        return 0xfe;
    sym = (CK_C_FindObjectsInit) GetProcAddress (hPK11, "C_FindObjectsInit");
    if (sym == NULL)
        return 0xff;
...
C_FindObjects (CK_SESSION_HANDLE hSession,
               CK_OBJECT_HANDLE_PTR phObject, CK_ULONG usMaxObjectCount, CK_ULONG_PTR pusObjectCount)
{
...
CK_RV C_FindObjectsFinal (CK_SESSION_HANDLE hSession)
{
    ...
    if (hPK11 == NULL)
        return 0xfe;
    sym = (CK_C_FindObjectsFinal) GetProcAddress (hPK11, "C_FindObjectsFinal");
    if (sym == NULL)
        return 0xff;
    return (*sym) (hSession);
}
                   ...
                   CK_OBJECT_HANDLE_PTR phPrivateKey, CK_OBJECT_HANDLE_PTR phPublicKey)
{
    ...
    if (hPK11 == NULL)
        return 0xfe;
    sym = (CK_C_GenerateKeyPair) GetProcAddress (hPK11, "C_GenerateKeyPair");
    if (sym == NULL)
        return 0xff;
    return (*sym) (hSession,
                   pMechanism,
                   ...
                   pPrivateKeyTemplate, usPrivateKeyAttributeCount, phPrivateKey, phPublicKey);
}
