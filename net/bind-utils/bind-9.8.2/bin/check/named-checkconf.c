...
{
    fprintf (stderr, "usage: %s [-h] [-j] [-p] [-v] [-z] [-t directory] " "[named.conf]\n", program);
    exit (1);
}
...
static isc_result_t config_get (const cfg_obj_t ** maps, const char *name, const cfg_obj_t ** obj)
{
    int i;
...
configure_zone (const char *vclass, const char *view,
                const cfg_obj_t * zconfig, const cfg_obj_t * vconfig, const cfg_obj_t * config, isc_mem_t * mctx)
{
    int i = 0;
    ...
    if (result != ISC_R_SUCCESS)
        fprintf (stderr, "%s/%s/%s: %s\n", view, zname, zclass, dns_result_totext (result));
    return (result);
}
                ...
                const cfg_obj_t * vconfig, isc_mem_t * mctx)
{
    ...
    for (element = cfg_list_first (zonelist); element != NULL; element = cfg_list_next (element))
    {
        ...
        tresult = configure_zone (vclass, view, zconfig, vconfig, config, mctx);
...
int main (int argc, char **argv)
{
    ...
    if (cfg_parse_file (parser, conffile, &cfg_type_namedconf, &config) != ISC_R_SUCCESS)
        exit (1);
