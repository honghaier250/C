...
static const char *algs = "RSA | RSAMD5 | DH | DSA | RSASHA1 |"
    " NSEC3DSA | NSEC3RSASHA1 |" " RSASHA256 | RSASHA512 | ECCGOST";
...
int main (int argc, char **argv)
{
    ...
    while ((ch = isc_commandline_parse (argc, argv, "3a:Cc:E:f:K:kl:n:p:t:v:yFhGP:A:R:I:D:")) != -1)
    {
        switch (ch)
        {
            case '3':
    ...
    if (strchr (label, ':') == NULL && engine != NULL && strlen (engine) != 0U)
    {
        char *l;
    ...
    if ((flags & DNS_KEYFLAG_OWNERMASK) == DNS_KEYOWNER_ZONE && alg == DNS_KEYALG_DH)
    ...
    /* associate the key */
    ret = dst_key_fromlabel (name, alg, flags, protocol, rdclass, engine, label, NULL, mctx, &key);
    ...
    if (key_collision (key, name, directory, mctx, &exact))
    {
        ...
        if (avoid_collisions)
            fatal ("%s: %s could collide with another key upon " "revokation\n", program, filename);

        fprintf (stderr, "%s: WARNING: Key %s could collide with "
                 "another key upon revokation.  If you plan "
                 "to revoke keys, destroy this key and " "generate a different one.\n", program, filename);
    }
