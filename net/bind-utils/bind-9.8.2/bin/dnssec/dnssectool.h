...
ISC_PLATFORM_NORETURN_PRE void fatal (const char *format, ...) ISC_FORMAT_PRINTF (1, 2) ISC_PLATFORM_NORETURN_POST;
     ...
     void vbprintf (int level, const char *fmt, ...) ISC_FORMAT_PRINTF (2, 3);
...
isc_boolean_t
key_collision (dst_key_t * key, dns_name_t * name, const char *dir, isc_mem_t * mctx, isc_boolean_t * exact);
