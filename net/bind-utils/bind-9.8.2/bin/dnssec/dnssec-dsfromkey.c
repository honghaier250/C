...
static isc_result_t loadsetfromfile (char *filename, dns_rdataset_t * rdataset)
{
    isc_result_t result;
    ...
    result = dns_db_findrdataset (db, node, NULL, dns_rdatatype_dnskey, 0, 0, rdataset, NULL);
...
static void loadkey (char *filename, unsigned char *key_buf, unsigned int key_buf_size, dns_rdata_t * rdata)
{
    ...
    result = dst_key_fromnamedfile (filename, NULL, DST_TYPE_PUBLIC, mctx, &key);
    if (result != ISC_R_SUCCESS)
        fatal ("invalid keyfile name %s: %s", filename, isc_result_totext (result));
...
int main (int argc, char **argv)
{
    ...
    while ((ch = isc_commandline_parse (argc, argv, "12Aa:c:d:Ff:K:l:sv:h")) != -1)
    {
        switch (ch)
        {
            case '1':
    ...
    if (algname != NULL)
    {
        if (strcasecmp (algname, "SHA1") == 0 || strcasecmp (algname, "SHA-1") == 0)
            dtype = DNS_DSDIGEST_SHA1;
        else if (strcasecmp (algname, "SHA256") == 0 || strcasecmp (algname, "SHA-256") == 0)
            dtype = DNS_DSDIGEST_SHA256;
    ...
    result = dst_lib_init (mctx, ectx, ISC_ENTROPY_BLOCKING | ISC_ENTROPY_GOODONLY);
    if (result != ISC_R_SUCCESS)
    ...
    else
    {
        ...
        loadkey (argv[isc_commandline_index], key_buf, DST_KEY_MAXSIZE, &rdata);
