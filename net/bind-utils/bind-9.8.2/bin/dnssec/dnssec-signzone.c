...
static void dumpnode (dns_name_t * name, dns_dbnode_t * node)
{
    ...
    if (outputformat != dns_masterformat_text)
        return;
    result = dns_master_dumpnodetostream (mctx, gdb, gversion, node, name, masterstyle, fp);
             ...
             dns_ttl_t ttl, dns_diff_t * add, const char *logmsg)
{
    ...
    result = dns_dnssec_sign (name, rdataset, key, &starttime, &jendtime, mctx, &b, &trdata);
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        dst_key_format (key, keystr, sizeof (keystr));
        fatal ("dnskey '%s' failed to sign data: %s", keystr, isc_result_totext (result));
    }
    INCSTAT (nsigned);

    if (tryverify)
    {
        result = dns_dnssec_verify (name, rdataset, key, ISC_TRUE, mctx, &trdata);
        if (result == ISC_R_SUCCESS)
        {
    ...
    tuple = NULL;
    result = dns_difftuple_create (mctx, DNS_DIFFOP_ADD, name, ttl, &trdata, &tuple);
    check_result (result, "dns_difftuple_create");
    dns_diff_append (add, &tuple);
}

static inline isc_boolean_t issigningkey (dns_dnsseckey_t * key)
{
    return (key->force_sign || key->hint_sign);
}

static inline isc_boolean_t iszonekey (dns_dnsseckey_t * key)
{
    return (ISC_TF (dns_name_equal (dst_key_name (key->key), gorigin) && dst_key_iszonekey (key->key)));
}

static inline isc_boolean_t isksk (dns_dnsseckey_t * key)
{
    return (key->ksk);
}

static inline isc_boolean_t iszsk (dns_dnsseckey_t * key)
{
...
static dns_dnsseckey_t *keythatsigned_unlocked (dns_rdata_rrsig_t * rrsig)
{
    ...
    for (key = ISC_LIST_HEAD (keylist); key != NULL; key = ISC_LIST_NEXT (key, link))
    {
        if (rrsig->keyid == dst_key_id (key->key) &&
            rrsig->algorithm == dst_key_alg (key->key) && dns_name_equal (&rrsig->signer, dst_key_name (key->key)))
            return (key);
    }
    return (NULL);
}
...
static dns_dnsseckey_t *keythatsigned (dns_rdata_rrsig_t * rrsig)
{
    ...
    result = dst_key_fromfile (&rrsig->signer, rrsig->keyid,
                               rrsig->algorithm, DST_TYPE_PUBLIC, directory, mctx, &pubkey);
    if (result != ISC_R_SUCCESS)
    {
        isc_rwlock_unlock (&keylist_lock, isc_rwlocktype_write);
        return (NULL);
    ...
    result = dst_key_fromfile (&rrsig->signer, rrsig->keyid,
                               rrsig->algorithm, DST_TYPE_PUBLIC | DST_TYPE_PRIVATE, directory, mctx, &privkey);
    if (result == ISC_R_SUCCESS)
    {
        dst_key_free (&pubkey);
...
static isc_boolean_t expecttofindkey (dns_name_t * name)
{
    ...
    fatal ("failure looking for '%s DNSKEY' in database: %s", namestr, isc_result_totext (result));
    /* NOTREACHED */
    return (ISC_FALSE);            /* removes a warning */
}

static inline isc_boolean_t setverifies (dns_name_t * name, dns_rdataset_t * set, dst_key_t * key, dns_rdata_t * rrsig)
{
    isc_result_t result;
...
static void signset (dns_diff_t * del, dns_diff_t * add, dns_dbnode_t * node, dns_name_t * name, dns_rdataset_t * set)
{
    ...
    result = dns_db_findrdataset (gdb, node, gversion, dns_rdatatype_rrsig, set->type, 0, &sigset, NULL);
    ...
    if (result != ISC_R_SUCCESS)
        fatal ("failed while looking for '%s RRSIG %s': %s", namestr, typestr, isc_result_totext (result));
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        if (isc_serial_gt (rrsig.timesigned, rrsig.timeexpire))
        {
            ...
            vbprintf (2, "\trrsig by %s dropped - " "invalid validity period\n", sigstr);
        }
        else if (key == NULL && !future && expecttofindkey (&rrsig.signer))
        {
            /* rrsig is dropped and not replaced */
            vbprintf (2, "\trrsig by %s dropped - " "private dnskey not found\n", sigstr);
        }
        ...
        {
            if (!expired && rrsig.originalttl == set->ttl && setverifies (name, set, key->key, &sigrdata))
            {
            ...
            else
            {
                vbprintf (2, "\trrsig by %s dropped - %s\n",
                          ...
                          rrsig.originalttl != set->ttl ? "ttl change" : "failed to verify");
                wassignedby[key->index] = ISC_TRUE;
                resign = ISC_TRUE;
            }
        }
        ...
        if (keep)
        {
            ...
            if (sigset.ttl != ttl)
            {
                ...
                tuple = NULL;
                result = dns_difftuple_create (mctx, DNS_DIFFOP_DEL, name, sigset.ttl, &sigrdata, &tuple);
                check_result (result, "dns_difftuple_create");
                dns_diff_append (del, &tuple);
                result = dns_difftuple_create (mctx, DNS_DIFFOP_ADD, name, ttl, &sigrdata, &tuple);
                check_result (result, "dns_difftuple_create");
                dns_diff_append (add, &tuple);
            }
        }
        ...
        if (resign)
        {
            ...
            signwithkey (name, set, key->key, ttl, add, "resigning with dnskey");
            nowsignedby[key->index] = ISC_TRUE;
        }
    ...
    for (key = ISC_LIST_HEAD (keylist); key != NULL; key = ISC_LIST_NEXT (key, link))
    {
        ...
        if (set->type == dns_rdatatype_dnskey && dns_name_equal (name, gorigin))
        {
            ...
            have_ksk = isksk (key);
            for (tmpkey = ISC_LIST_HEAD (keylist); tmpkey != NULL; tmpkey = ISC_LIST_NEXT (tmpkey, link))
            {
            ...
            if (isksk (key) || !have_ksk || (iszsk (key) && !keyset_kskonly))
                signwithkey (name, set, key->key, ttl, add, "signing with dnskey");
        }
        else if (iszsk (key))
        {
            signwithkey (name, set, key->key, ttl, add, "signing with dnskey");
...
static void hashlist_add_dns_name (hashlist_t * l, /*const */ dns_name_t * name,
                                   unsigned int hashalg, unsigned int iterations,
                                   const unsigned char *salt, size_t salt_length, isc_boolean_t speculative)
{
    ...
    len = isc_iterated_hash (hash, hashalg, iterations, salt, salt_length, name->ndata, name->length);
    if (verbose)
    {
...
static void addnowildcardhash (hashlist_t * l, /*const */ dns_name_t * name,
                               unsigned int hashalg, unsigned int iterations,
                               const unsigned char *salt, size_t salt_length)
{
    ...
    hashlist_add_dns_name (l, wild, hashalg, iterations, salt, salt_length, ISC_TRUE);
}
...
static void opendb (const char *prefix, dns_name_t * name, dns_rdataclass_t rdclass, dns_db_t ** dbp)
{
    ...
    result = dns_db_create (mctx, "rbt", dns_rootname, dns_dbtype_zone, rdclass, 0, NULL, dbp);
...
static isc_result_t loadds (dns_name_t * name, isc_uint32_t ttl, dns_rdataset_t * dsset)
{
    ...
    if (db != NULL)
    {
        ...
        if (result == ISC_R_SUCCESS)
        {
            ...
            result = dns_db_findrdataset (db, node, NULL, dns_rdatatype_ds, 0, 0, dsset, NULL);
    ...
    result = dns_db_findrdataset (db, node, NULL, dns_rdatatype_dnskey, 0, 0, &keyset, NULL);
    if (result != ISC_R_SUCCESS)
    {
        dns_db_detachnode (db, &node);
        dns_db_detach (&db);
    ...
    for (result = dns_rdataset_first (&keyset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&keyset))
    {
        dns_rdata_init (&key);
        dns_rdata_init (&ds);
        dns_rdataset_current (&keyset, &key);
        result = dns_ds_buildrdata (name, &key, DNS_DSDIGEST_SHA1, dsbuf, &ds);
        check_result (result, "dns_ds_buildrdata");

        result = dns_difftuple_create (mctx, DNS_DIFFOP_ADD, name, ttl, &ds, &tuple);
        check_result (result, "dns_difftuple_create");
        dns_diff_append (&diff, &tuple);

        dns_rdata_reset (&ds);
        result = dns_ds_buildrdata (name, &key, DNS_DSDIGEST_SHA256, dsbuf, &ds);
        check_result (result, "dns_ds_buildrdata");
...
static isc_boolean_t delegation (dns_name_t * name, dns_dbnode_t * node, isc_uint32_t * ttlp)
{
    ...
    result = dns_db_findrdataset (gdb, node, gversion, dns_rdatatype_ns, 0, 0, &nsset, NULL);
...
static isc_boolean_t secure (dns_name_t * name, dns_dbnode_t * node)
{
    ...
    dns_rdataset_init (&dsset);
    result = dns_db_findrdataset (gdb, node, gversion, dns_rdatatype_ds, 0, 0, &dsset, NULL);
...
static void signname (dns_dbnode_t * node, dns_name_t * name)
{
    ...
    if (result != ISC_R_NOMORE)
        fatal ("rdataset iteration for name '%s' failed: %s", namestr, isc_result_totext (result));

    dns_rdatasetiter_destroy (&rdsiter);

    result = dns_diff_applysilently (&del, gdb, gversion);
    if (result != ISC_R_SUCCESS)
        fatal ("failed to delete SIGs at node '%s': %s", namestr, isc_result_totext (result));

    result = dns_diff_applysilently (&add, gdb, gversion);
    if (result != ISC_R_SUCCESS)
        fatal ("failed to add SIGs at node '%s': %s", namestr, isc_result_totext (result));

    dns_diff_clear (&del);
    dns_diff_clear (&add);
}

static inline isc_boolean_t active_node (dns_dbnode_t * node)
{
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        if (rdataset.type != dns_rdatatype_nsec &&
            rdataset.type != dns_rdatatype_nsec3 && rdataset.type != dns_rdatatype_rrsig)
            active = ISC_TRUE;
        dns_rdataset_disassociate (&rdataset);
        if (!active)
            result = dns_rdatasetiter_next (rdsiter);
    ...
    if (result != ISC_R_NOMORE)
        fatal ("rdataset iteration failed: %s", isc_result_totext (result));
    ...
    if (!active && nsec_datatype == dns_rdatatype_nsec)
    {
        ...
        {
            dns_rdatasetiter_current (rdsiter, &rdataset);
            result = dns_db_deleterdataset (gdb, node, gversion, rdataset.type, rdataset.covers);
    ...
    else
    {
             ...
             result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (rdsiter))
        {
             ...
             * NSEC3.
             */
            if (nsec_datatype == dns_rdatatype_nsec3 && (type == dns_rdatatype_nsec || covers == dns_rdatatype_nsec))
            {
                result = dns_db_deleterdataset (gdb, node, gversion, type, covers);
                check_result (result, "dns_db_deleterdataset(nsec/rrsig)");
                continue;
            }
            ...
            for (result = dns_rdatasetiter_first (rdsiter2);
                 !found && result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (rdsiter2))
            {
            ...
            if (!found)
            {
                if (result != ISC_R_NOMORE)
                    fatal ("rdataset iteration failed: %s", isc_result_totext (result));
                result = dns_db_deleterdataset (gdb, node, gversion, type, covers);
                check_result (result, "dns_db_deleterdataset(rrsig)");
            }
            else if (result != ISC_R_NOMORE && result != ISC_R_SUCCESS)
                fatal ("rdataset iteration failed: %s", isc_result_totext (result));
        }
        if (result != ISC_R_NOMORE)
            fatal ("rdataset iteration failed: %s", isc_result_totext (result));
...
static void get_soa_ttls (void)
{
    ...
    result = dns_db_find (gdb, gorigin, gversion, dns_rdatatype_soa, 0, 0, NULL, name, &soaset, NULL);
    if (result != ISC_R_SUCCESS)
        fatal ("failed to find an SOA at the zone apex: %s", isc_result_totext (result));
...
static isc_result_t setsoaserial (isc_uint32_t serial)
{
    ...
    result = dns_db_findrdataset (gdb, node, gversion, dns_rdatatype_soa, 0, 0, &rdataset, NULL);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    ...
    if (new_serial == old_serial || (new_serial - old_serial) > 0x7fffffffU)
        fprintf (stderr, "%s: warning: Serial number not advanced, " "zone may not transfer\n", program);

    dns_soa_setserial (new_serial, &rdata);

    result = dns_db_deleterdataset (gdb, node, gversion, dns_rdatatype_soa, 0);
    check_result (result, "dns_db_deleterdataset");
    if (result != ISC_R_SUCCESS)
        goto cleanup;
 ...
 */
static void cleannode (dns_db_t * db, dns_dbversion_t * version, dns_dbnode_t * node)
{
    dns_rdatasetiter_t *rdsiter = NULL;
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        if (destroy)
        {
            dresult = dns_db_deleterdataset (db, node, version, dns_rdatatype_rrsig, covers);
...
goodsig (dns_rdata_t * sigrdata, dns_name_t * name, dns_rdataset_t * keyrdataset, dns_rdataset_t * rdataset)
{
    ...
    for (result = dns_rdataset_first (keyrdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (keyrdataset))
    {
        dns_rdata_t rdata = DNS_RDATA_INIT;

        dns_rdataset_current (keyrdataset, &rdata);
        dns_rdata_tostruct (&rdata, &key, NULL);
        result = dns_dnssec_keyfromrdata (gorigin, &rdata, mctx, &dstkey);
        if (result != ISC_R_SUCCESS)
            return (ISC_FALSE);
        if (sig.algorithm != key.algorithm ||
            sig.keyid != dst_key_id (dstkey) || !dns_name_equal (&sig.signer, gorigin))
        {
            dst_key_free (&dstkey);
            continue;
        }
        result = dns_dnssec_verify (name, rdataset, dstkey, ISC_FALSE, mctx, sigrdata);
        dst_key_free (&dstkey);
        if (result == ISC_R_SUCCESS)
            return (ISC_TRUE);
    }
           ...
           dns_rdataset_t * keyrdataset, unsigned char *ksk_algorithms, unsigned char *bad_algorithms)
{
    ...
    result = dns_db_allrdatasets (gdb, node, gversion, 0, &rdsiter);
    check_result (result, "dns_db_allrdatasets()");
    for (result = dns_rdatasetiter_first (rdsiter); result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (rdsiter))
    {
    ...
    for (result = dns_rdataset_first (&sigrdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&sigrdataset))
    {
        ...
        if ((set_algorithms[sig.algorithm] != 0) || (ksk_algorithms[sig.algorithm] == 0))
            continue;
    ...
    if (memcmp (set_algorithms, ksk_algorithms, sizeof (set_algorithms)))
    {
        ...
        type_format (rdataset->type, typebuf, sizeof (typebuf));
        for (i = 0; i < 256; i++)
            if ((ksk_algorithms[i] != 0) && (set_algorithms[i] == 0))
            {
                dns_secalg_format (i, algbuf, sizeof (algbuf));
                fprintf (stderr, "Missing %s signature for " "%s %s\n", algbuf, namebuf, typebuf);
                bad_algorithms[i] = 1;
            }
    }
    dns_rdataset_disassociate (&sigrdataset);
            ...
            dns_rdataset_t * keyrdataset, unsigned char *ksk_algorithms, unsigned char *bad_algorithms)
{
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        if (rdataset.type != dns_rdatatype_rrsig &&
            ...
            (!delegation || rdataset.type == dns_rdatatype_ds || rdataset.type == dns_rdatatype_nsec))
        {
            verifyset (&rdataset, name, node, keyrdataset, ksk_algorithms, bad_algorithms);
        }
        dns_rdataset_disassociate (&rdataset);
        result = dns_rdatasetiter_next (rdsiter);
    }
...
static void verifyzone (void)
{
    ...
    if (result != ISC_R_SUCCESS)
        fatal ("failed to find the zone's origin: %s", isc_result_totext (result));
    ...
    result = dns_db_findrdataset (gdb, node, gversion, dns_rdatatype_dnskey, 0, 0, &keyset, &keysigs);
    if (result != ISC_R_SUCCESS)
        fatal ("cannot find DNSKEY rrset\n");

    result = dns_db_findrdataset (gdb, node, gversion, dns_rdatatype_soa, 0, 0, &soaset, &soasigs);
    dns_db_detachnode (gdb, &node);
    if (result != ISC_R_SUCCESS)
    ...
    for (result = dns_rdataset_first (&keyset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&keyset))
    {
        ...
        else if ((dnskey.flags & DNS_KEYFLAG_REVOKE) != 0)
        {
            if ((dnskey.flags & DNS_KEYFLAG_KSK) != 0 &&
                !dns_dnssec_selfsigns (&rdata, gorigin, &keyset, &keysigs, ISC_FALSE, mctx))
            {
                char namebuf[DNS_NAME_FORMATSIZE];
                ...
                dns_name_format (gorigin, namebuf, sizeof (namebuf));
                isc_buffer_init (&buf, buffer, sizeof (buffer));
                result = dns_rdata_totext (&rdata, NULL, &buf);
                check_result (result, "dns_rdata_totext");
                fatal ("revoked KSK is not self signed:\n"
                       "%s DNSKEY %.*s", namebuf, (int) isc_buffer_usedlength (&buf), buffer);
            }
            if ((dnskey.flags & DNS_KEYFLAG_KSK) != 0 && revoked_ksk[dnskey.algorithm] != 255)
                revoked_ksk[dnskey.algorithm]++;
            else if ((dnskey.flags & DNS_KEYFLAG_KSK) == 0 && revoked_zsk[dnskey.algorithm] != 255)
                revoked_zsk[dnskey.algorithm]++;
        }
        else if ((dnskey.flags & DNS_KEYFLAG_KSK) != 0)
        {
            if (dns_dnssec_selfsigns (&rdata, gorigin, &keyset, &keysigs, ISC_FALSE, mctx))
            {
        ...
        else if (dns_dnssec_selfsigns (&rdata, gorigin, &keyset, &keysigs, ISC_FALSE, mctx))
        {
#ifdef ALLOW_KSKLESS_ZONES
        ...
        else if (dns_dnssec_signs (&rdata, gorigin, &soaset, &soasigs, ISC_FALSE, mctx))
        {
            if (zsk_algorithms[dnskey.algorithm] != 255)
                zsk_algorithms[dnskey.algorithm]++;
        }
    ...
    {
        if (!ignore_kskflag)
            fprintf (stderr, "No self signing KSK found. Using " "self signed ZSK's for active " "algorithm list.\n");
        memcpy (ksk_algorithms, self_algorithms, sizeof (ksk_algorithms));
        if (!allzsksigned)
            fprintf (stderr, "warning: not all ZSK's are self " "signed.\n");
    }
#else
    if (!goodksk)
    {
    ...
    while (!done)
    {
        ...
        verifynode (name, node, isdelegation, &keyset, ksk_algorithms, bad_algorithms);
        result = dns_dbiterator_next (dbiter);
        nextnode = NULL;
        while (result == ISC_R_SUCCESS)
        {
            result = dns_dbiterator_current (dbiter, &nextnode, nextname);
            check_dns_dbiterator_current (result);
            if (!dns_name_issubdomain (nextname, gorigin) ||
                (zonecut != NULL && dns_name_issubdomain (nextname, zonecut)))
            {
        ...
        if (result == ISC_R_NOMORE)
        {
            ...
            fatal ("iterating through the database failed: %s", isc_result_totext (result));
        dns_db_detachnode (gdb, &node);
    }
    ...
    for (result = dns_dbiterator_first (dbiter); result == ISC_R_SUCCESS; result = dns_dbiterator_next (dbiter))
    {
        result = dns_dbiterator_current (dbiter, &node, name);
        check_dns_dbiterator_current (result);
        verifynode (name, node, ISC_FALSE, &keyset, ksk_algorithms, bad_algorithms);
        dns_db_detachnode (gdb, &node);
    }
    dns_dbiterator_destroy (&dbiter);
    ...
    for (i = 0; i < 256; i++)
    {
        ...
        {
            if (first)
                fprintf (stderr, "The zone is not fully signed " "for the following algorithms:");
            dns_secalg_format (i, algbuf, sizeof (algbuf));
            fprintf (stderr, " %s", algbuf);
            first = ISC_FALSE;
        }
    ...
    if (goodksk || ignore_kskflag)
    {
        ...
        for (i = 0; i < 256; i++)
        {
            if ((ksk_algorithms[i] != 0) ||
                (standby_ksk[i] != 0) ||
                (revoked_zsk[i] != 0) || (zsk_algorithms[i] != 0) || (standby_zsk[i] != 0) || (revoked_zsk[i] != 0))
            {
                dns_secalg_format (i, algbuf, sizeof (algbuf));
                fprintf (stderr, "Algorithm: %s: KSKs: "
                         "%u active, %u stand-by, %u revoked\n",
                         algbuf, ksk_algorithms[i], standby_ksk[i], revoked_ksk[i]);
                fprintf (stderr, "%*sZSKs: "
                         "%u active, %u %s, %u revoked\n",
                         (int) strlen (algbuf) + 13, "",
                         zsk_algorithms[i], standby_zsk[i], keyset_kskonly ? "present" : "stand-by", revoked_zsk[i]);
            }
        }
    }
}
...
static void signapex (void)
{
    ...
    if (result == ISC_R_NOMORE)
        ...
        fatal ("failure iterating database: %s", isc_result_totext (result));
}
...
static void assignwork (isc_task_t * task, isc_task_t * worker)
{
    ...
    while (!found)
    {
        ...
        result = dns_db_findrdataset (gdb, node, gversion, nsec_datatype, 0, 0, &nsec, NULL);
        ...
        {
            if (dns_name_issubdomain (name, gorigin) && (zonecut == NULL || !dns_name_issubdomain (name, zonecut)))
            {
                if (delegation (name, node, NULL))
                {
                    dns_fixedname_init (&fzonecut);
                    zonecut = dns_fixedname_name (&fzonecut);
                    dns_name_copy (name, zonecut, NULL);
                    if (!OPTOUT (nsec3flags) || secure (name, node))
                        found = ISC_TRUE;
                }
                else
                    found = ISC_TRUE;
    ...
    sevent = (sevent_t *) isc_event_allocate (mctx, task, SIGNER_EVENT_WORK, sign, NULL, sizeof (sevent_t));
    if (sevent == NULL)
...
static void sign (isc_task_t * task, isc_event_t * event)
{
    ...
    wevent = (sevent_t *) isc_event_allocate (mctx, task, SIGNER_EVENT_WRITE, writenode, NULL, sizeof (sevent_t));
    if (wevent == NULL)
        fatal ("failed to allocate event\n");
    wevent->node = node;
    wevent->fname = fname;
...
static void add_ds (dns_name_t * name, dns_dbnode_t * node, isc_uint32_t nsttl)
{
    ...
    result = dns_db_findrdataset (gdb, node, gversion, dns_rdatatype_ds, 0, 0, &dsset, &sigdsset);
    if (result == ISC_R_SUCCESS)
    {
        dns_rdataset_disassociate (&dsset);
        result = dns_db_deleterdataset (gdb, node, gversion, dns_rdatatype_ds, 0);
        check_result (result, "dns_db_deleterdataset");
    }
    ...
    if (result == ISC_R_SUCCESS)
    {
        result = dns_db_addrdataset (gdb, node, gversion, 0, &dsset, 0, NULL);
        check_result (result, "dns_db_addrdataset");
        dns_rdataset_disassociate (&dsset);
    ...
    {
        result = dns_db_deleterdataset (gdb, node, gversion, dns_rdatatype_rrsig, dns_rdatatype_ds);
        check_result (result, "dns_db_deleterdataset");
        dns_rdataset_disassociate (&sigdsset);
    }
}
 ...
 */
static void remove_records (dns_dbnode_t * node, dns_rdatatype_t which)
{
    isc_result_t result;
    ...
    for (result = dns_rdatasetiter_first (rdsiter); result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (rdsiter))
    {
        ...
        if (type == which || covers == which)
        {
            ...
            if (which == dns_rdatatype_nsec3param && !update_chain)
                fatal ("Zone contains NSEC3 chains.  Use -u " "to update to NSEC.");
            result = dns_db_deleterdataset (gdb, node, gversion, type, covers);
            check_result (result, "dns_db_deleterdataset()");
            continue;
        }
...
static void nsecify (void)
{
    ...
    for (result = dns_dbiterator_first (dbiter); result == ISC_R_SUCCESS; result = dns_dbiterator_next (dbiter))
    {
             ...
             result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (rdsiter))
        {
            ...
            dns_rdataset_disassociate (&rdataset);
            result = dns_db_deleterdataset (gdb, node, gversion, type, covers);
            check_result (result, "dns_db_deleterdataset(nsec3param/rrsig)");
        }
        dns_rdatasetiter_destroy (&rdsiter);
        dns_db_detachnode (gdb, &node);
    }
    ...
    while (!done)
    {
        ...
        result = dns_nsec_build (gdb, gversion, node, nextname, zone_soa_min_ttl);
...
static void addnsec3param (const unsigned char *salt, size_t salt_length, unsigned int iterations)
{
    dns_dbnode_t *node = NULL;
    ...
    result = dns_rdata_fromstruct (&rdata, gclass, dns_rdatatype_nsec3param, &nsec3param, &b);
     ...
     */
    result = dns_db_deleterdataset (gdb, node, gversion, dns_rdatatype_nsec3param, 0);
    if (result == DNS_R_UNCHANGED)
        result = ISC_R_SUCCESS;
    check_result (result, "dddnsec3param: dns_db_deleterdataset()");

    result = dns_db_addrdataset (gdb, node, gversion, 0, &rdataset, DNS_DBADD_MERGE, NULL);
    if (result == DNS_R_UNCHANGED)
        result = ISC_R_SUCCESS;
...
addnsec3 (dns_name_t * name, dns_dbnode_t * node,
          const unsigned char *salt, size_t salt_length, unsigned int iterations, hashlist_t * hashlist, dns_ttl_t ttl)
{
    ...
    result = dns_nsec3_hashname (&hashname, hash, &hash_length,
                                 name, gorigin, dns_hash_sha1, iterations, salt, salt_length);
    ...
    result = dns_nsec3_buildrdata (gdb, gversion, node,
                                   ...
                                   salt, salt_length, nexthash, ISC_SHA1_DIGESTLENGTH, nsec3buffer, &rdata);
    ...
    result = dns_rdatalist_tordataset (&rdatalist, &rdataset);
    check_result (result, "dns_rdatalist_tordataset()");
    result = dns_db_findnsec3node (gdb, dns_fixedname_name (&hashname), ISC_TRUE, &nsec3node);
    check_result (result, "addnsec3: dns_db_findnode()");
    result = dns_db_addrdataset (gdb, nsec3node, gversion, 0, &rdataset, 0, NULL);
    if (result == DNS_R_UNCHANGED)
        result = ISC_R_SUCCESS;
            ...
            const unsigned char *salt, size_t salt_length, hashlist_t * hashlist)
{
    ...
    result = dns_db_findrdataset (gdb, node, gversion, dns_rdatatype_nsec3, 0, 0, &rdataset, NULL);
    if (result != ISC_R_SUCCESS)
        return;
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        if (exists && nsec3.hash == hashalg &&
            ...
            nsec3.salt_length == salt_length && !memcmp (nsec3.salt, salt, salt_length))
            continue;
        rdatalist.rdclass = rdata.rdclass;
        rdatalist.type = rdata.type;
        rdatalist.covers = 0;
        ...
        check_result (result, "dns_rdatalist_tordataset()");
        result = dns_db_subtractrdataset (gdb, node, gversion, &delrdataset, 0, NULL);
     ...
     */
    result = dns_db_deleterdataset (gdb, node, gversion, dns_rdatatype_rrsig, dns_rdatatype_nsec3);
    if (result != ISC_R_SUCCESS && result != DNS_R_UNCHANGED)
        check_result (result, "dns_db_deleterdataset(RRSIG(NSEC3))");
}

static void rrset_remove_duplicates (dns_name_t * name, dns_rdataset_t * rdataset, dns_diff_t * diff)
{
    dns_difftuple_t *tuple = NULL;
    ...
    for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
    {
             ...
             result == ISC_R_SUCCESS; result = dns_rdataset_next (&tmprdataset))
        {
            ...
            {
                result = dns_difftuple_create (mctx, DNS_DIFFOP_DEL, name, rdataset->ttl, &rdata2, &tuple);
                check_result (result, "dns_difftuple_create");
                dns_diff_append (diff, &tuple);
            }
        }
          ...
          const unsigned char *salt, size_t salt_length, hashlist_t * hashlist)
{
    ...
    while (!done)
    {
        ...
        while (result == ISC_R_SUCCESS)
        {
            ...
            if (delegation (nextname, nextnode, &nsttl))
            {
                ...
                if (OPTOUT (nsec3flags) && !secure (nextname, nextnode))
                {
                    dns_db_detachnode (gdb, &nextnode);
                    result = dns_dbiterator_next (dbiter);
                    continue;
        ...
        if (result == ISC_R_NOMORE)
        {
            ...
            fatal ("iterating through the database failed: %s", isc_result_totext (result));
        dns_name_downcase (name, name, NULL);
        hashlist_add_dns_name (hashlist, name, hashalg, iterations, salt, salt_length, ISC_FALSE);
        dns_db_detachnode (gdb, &node);
        /*
        ...
        addnowildcardhash (hashlist, name, hashalg, iterations, salt, salt_length);
        count = dns_name_countlabels (nextname);
        while (count > nlabels + 1)
        {
            count--;
            dns_name_split (nextname, count, NULL, nextname);
            hashlist_add_dns_name (hashlist, nextname, hashalg, iterations, salt, salt_length, ISC_FALSE);
            addnowildcardhash (hashlist, nextname, hashalg, iterations, salt, salt_length);
        }
    }
    dns_dbiterator_destroy (&dbiter);
    ...
    for (result = dns_dbiterator_first (dbiter); result == ISC_R_SUCCESS; result = dns_dbiterator_next (dbiter))
    {
        result = dns_dbiterator_current (dbiter, &node, name);
        check_dns_dbiterator_current (result);
        nsec3clean (name, node, hashalg, iterations, salt, salt_length, hashlist);
        dns_db_detachnode (gdb, &node);
    }
    dns_dbiterator_destroy (&dbiter);
    ...
    while (!done)
    {
         ...
         */
        dns_dbiterator_pause (dbiter);
        addnsec3 (name, node, salt, salt_length, iterations, hashlist, zone_soa_min_ttl);
        dns_db_detachnode (gdb, &node);
        /*
        ...
        while (count > nlabels + 1)
        {
            count--;
            dns_name_split (nextname, count, NULL, nextname);
            addnsec3 (nextname, NULL, salt, salt_length, iterations, hashlist, zone_soa_min_ttl);
        }
    }
    dns_dbiterator_destroy (&dbiter);
}
...
static void loadzone (char *file, char *origin, dns_rdataclass_t rdclass, dns_db_t ** db)
{
    ...
    if (result != ISC_R_SUCCESS)
        fatal ("failed converting name '%s' to dns format: %s", origin, isc_result_totext (result));

    result = dns_db_create (mctx, "rbt", name, dns_dbtype_zone, rdclass, 0, NULL, db);
    check_result (result, "dns_db_create()");

    result = dns_db_load2 (*db, file, inputformat);
    if (result != ISC_R_SUCCESS && result != DNS_R_SEENINCLUDE)
        fatal ("failed loading zone from '%s': %s", file, isc_result_totext (result));
}
...
static void loadzonekeys (isc_boolean_t preserve_keys, isc_boolean_t load_public)
{
    ...
    result = dns_db_findrdataset (gdb, node, currentversion, dns_rdatatype_soa, 0, 0, &rdataset, &soasigs);
    if (result != ISC_R_SUCCESS)
        goto cleanup;

    /* Preserve the TTL of the DNSKEY RRset, if any */
    dns_rdataset_disassociate (&rdataset);
    result = dns_db_findrdataset (gdb, node, currentversion, dns_rdatatype_dnskey, 0, 0, &rdataset, &keysigs);
    ...
    {
        fprintf (stderr, "User-specified TTL (%d) conflicts " "with existing DNSKEY RRset TTL.\n", keyttl);
        fprintf (stderr, "Imported keys will use the RRSet " "TTL (%d) instead.\n", rdataset.ttl);
    }
    keyttl = rdataset.ttl;

    /* Load keys corresponding to the existing DNSKEY RRset. */
    result = dns_dnssec_keylistfromrdataset (gorigin, directory, mctx,
                                             &rdataset, &keysigs, &soasigs, preserve_keys, load_public, &keylist);
    if (result != ISC_R_SUCCESS)
        fatal ("failed to load the zone keys: %s", isc_result_totext (result));
...
static void loadexplicitkeys (char *keyfiles[], int n, isc_boolean_t setksk)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        result = dst_key_fromnamedfile (keyfiles[i], directory, DST_TYPE_PUBLIC | DST_TYPE_PRIVATE, mctx, &newkey);
        if (result != ISC_R_SUCCESS)
            fatal ("cannot load dnskey %s: %s", keyfiles[i], isc_result_totext (result));

        if (!dns_name_equal (gorigin, dst_key_name (newkey)))
            fatal ("key %s not at origin\n", keyfiles[i]);

        if (!dst_key_isprivate (newkey))
            fatal ("cannot sign zone with non-private dnskey %s", keyfiles[i]);

        /* Skip any duplicates */
        for (key = ISC_LIST_HEAD (keylist); key != NULL; key = ISC_LIST_NEXT (key, link))
        {
            if (dst_key_id (key->key) == dst_key_id (newkey) && dst_key_alg (key->key) == dst_key_alg (newkey))
                break;
        }
...
static void build_final_keylist ()
{
    isc_result_t result;
    ...
    result = dns_dnssec_findmatchingkeys (gorigin, directory, mctx, &matchkeys);
    if (result == ISC_R_NOTFOUND)
        result = ISC_R_SUCCESS;
     ...
     */
    dns_dnssec_updatekeys (&keylist, &matchkeys, NULL, gorigin, keyttl, &diff, ignore_kskflag, mctx, report);

    dns_name_format (gorigin, name, sizeof (name));

    result = dns_diff_applysilently (&diff, gdb, ver);
    if (result != ISC_R_SUCCESS)
        fatal ("failed to update DNSKEY RRset at node '%s': %s", name, isc_result_totext (result));
...
static void warnifallksk (dns_db_t * db)
{
    ...
    dns_rdataset_init (&rdataset);
    result = dns_db_findrdataset (db, node, currentversion, dns_rdatatype_dnskey, 0, 0, &rdataset, NULL);
    if (result != ISC_R_SUCCESS)
        fatal ("failed to find keys at the zone apex: %s", isc_result_totext (result));
    result = dns_rdataset_first (&rdataset);
    check_result (result, "dns_rdataset_first");
    while (result == ISC_R_SUCCESS)
    {
    ...
    if (!have_non_ksk && !ignore_kskflag)
    {
        if (disable_zone_check)
            ...
            fatal ("No non-KSK DNSKEY found; " "supply a ZSK or use '-z'.");
    }
}

static void
set_nsec3params (isc_boolean_t update_chain, isc_boolean_t set_salt, isc_boolean_t set_optout, isc_boolean_t set_iter)
{
    isc_result_t result;
    ...
    result = dns_db_getnsec3parameters (gdb, ver, &orig_hash, NULL, &orig_iter, orig_salt, &orig_saltlen);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    ...
    {
        if (salt_length != orig_saltlen || memcmp (saltbuf, orig_salt, salt_length) != 0)
            fatal ("An NSEC3 chain exists with a different salt. " "Use -u to update it.");
    }
    else if (!set_salt)
    {
    ...
    if (!update_chain && set_iter)
    {
        if (nsec3iter != orig_iter)
            fatal ("An NSEC3 chain exists with different " "iterations. Use -u to update it.");
    }
    else if (!set_iter)
        nsec3iter = orig_iter;
    ...
    result = dns_nsec3_hashname (&fname, NULL, NULL,
                                 gorigin, gorigin, dns_hash_sha1, orig_iter, orig_salt, orig_saltlen);
    ...
    result = dns_db_findrdataset (gdb, node, ver, dns_rdatatype_nsec3, 0, 0, &rdataset, NULL);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    ...
    if (!update_chain && set_optout)
    {
        if (nsec3flags != nsec3.flags)
            fatal ("An NSEC3 chain exists with%s OPTOUT. "
                   ...
                   OPTOUT (nsec3.flags) ? "AA" : "A", OPTOUT (nsec3.flags) ? "clear" : "set");
    }
    else if (!set_optout)
...
static void writeset (const char *prefix, dns_rdatatype_t type)
{
    ...
    const dns_master_style_t *style = (type == dns_rdatatype_dnskey) ? masterstyle : dsstyle;
    ...
    for (key = ISC_LIST_HEAD (keylist); key != NULL; key = ISC_LIST_NEXT (key, link))
    {
        ...
        {
            result = dns_ds_buildrdata (gorigin, &rdata, DNS_DSDIGEST_SHA1, dsbuf, &ds);
            check_result (result, "dns_ds_buildrdata");
            if (type == dns_rdatatype_dlv)
                ds.type = dns_rdatatype_dlv;
            result = dns_difftuple_create (mctx, DNS_DIFFOP_ADD, name, 0, &ds, &tuple);
            check_result (result, "dns_difftuple_create");
            dns_diff_append (&diff, &tuple);

            dns_rdata_reset (&ds);
            result = dns_ds_buildrdata (gorigin, &rdata, DNS_DSDIGEST_SHA256, dsbuf, &ds);
            check_result (result, "dns_ds_buildrdata");
            if (type == dns_rdatatype_dlv)
                ds.type = dns_rdatatype_dlv;
            result = dns_difftuple_create (mctx, DNS_DIFFOP_ADD, name, 0, &ds, &tuple);
            ...
            result = dns_difftuple_create (mctx, DNS_DIFFOP_ADD, gorigin, zone_soa_min_ttl, &rdata, &tuple);
        check_result (result, "dns_difftuple_create");
        dns_diff_append (&diff, &tuple);
    }

    result = dns_db_create (mctx, "rbt", dns_rootname, dns_dbtype_zone, gclass, 0, NULL, &db);
    check_result (result, "dns_db_create");
...
#ifdef USE_PKCS11
    fprintf (stderr, "\t\tname of an OpenSSL engine to use " "(default is \"pkcs11\")\n");
#else
...
static void
print_stats (isc_time_t * timer_start, isc_time_t * timer_finish, isc_time_t * sign_start, isc_time_t * sign_finish)
{
...
#define CMDLINE_FLAGS \
        ...
        switch (ch)
        {
            case '3':
                ...
                if (strcmp (isc_commandline_argument, "-") != 0)
                {
                    ...
                    isc_buffer_init (&target, saltbuf, sizeof (saltbuf));
            ...
            case 'd':
                ...
                if (result != ISC_R_SUCCESS)
                    fatal ("cannot open directory %s: %s", dsdir, isc_result_totext (result));
                break;
            ...
            case 'i':
                ...
                if (*endp != '\0' || cycle < 0)
                    fatal ("cycle period must be numeric and " "positive");
                break;
            ...
            case 'l':
                ...
                result = dns_name_fromtext (dlv, &b, dns_rootname, 0, NULL);
    ...
    if (result != ISC_R_SUCCESS)
        fatal ("could not initialize dst: %s", isc_result_totext (result));
    ...
    if (output == NULL)
    {
        ...
        output = isc_mem_allocate (mctx, strlen (file) + strlen (".signed") + 1);
        if (output == NULL)
            fatal ("out of memory");
    ...
    if (serialformatstr != NULL)
    {
        ...
        else if (strcasecmp (serialformatstr, "increment") == 0 || strcasecmp (serialformatstr, "incr") == 0)
    ...
    result = dns_master_stylecreate (&dsstyle, DNS_STYLEFLAG_NO_TTL, 0, 24, 0, 0, 0, 8, mctx);
    ...
    if (IS_NSEC3)
    {
        ...
        hashlist_init (&hashlist, dns_db_nodecount (gdb) * 2, hash_length);
        result = dns_nsec_nseconly (gdb, gversion, &answer);
        check_result (result, "dns_nsec_nseconly");
        if (answer)
            fatal ("NSEC3 generation requested with " "NSEC only DNSKEY");
    }
    ...
    if (IS_NSEC3)
    {
        ...
        if (nsec3iter > max)
            fatal ("NSEC3 iterations too big for weakest DNSKEY " "strength. Maximum iterations allowed %u.", max);
    }
    ...
    if (IS_NSEC3)
        nsec3ify (dns_hash_sha1, nsec3iter, salt, salt_length, &hashlist);
    else
        nsecify ();
    ...
    if (result != ISC_R_SUCCESS)
        fatal ("failed to open temporary output file: %s", isc_result_totext (result));
    removefile = ISC_TRUE;
    ...
    if (result != ISC_R_SUCCESS)
        fatal ("failed to create task manager: %s", isc_result_totext (result));
    ...
    if (!finished)
    {
        ...
        for (i = 0; i < (int) ntasks; i++)
        {
            ...
            if (result != ISC_R_SUCCESS)
                fatal ("failed to start task: %s", isc_result_totext (result));
        }
        (void) isc_app_run ();
        if (!finished)
    ...
    if (outputformat != dns_masterformat_text)
    {
        result = dns_master_dumptostream2 (mctx, gdb, gversion, masterstyle, outputformat, fp);
    ...
    if (printstats)
    {
        ...
        print_stats (&timer_start, &timer_finish, &sign_start, &sign_finish);
    }
