        ...
        case 'm':
            switch (suffix[1])
            {
                ...
                case 'i':
                    return (offset * 60);
                case '\0':
                    fatal ("'%s' ambiguous: use 'mi' for minutes " "or 'mo' for months", str);
                default:
...
void check_keyversion (dst_key_t * key, char *keystr)
{
    ...
    if (major < DST_MAJOR_VERSION || minor < DST_MINOR_VERSION)
        fatal ("Key %s has incompatible format version %d.%d, "
               "use -f to force upgrade to new version.", keystr, major, minor);
    if (minor > DST_MINOR_VERSION)
        fatal ("Key %s has incompatible format version %d.%d, "
               "use -f to force downgrade to current version.", keystr, major, minor);
}
...
void set_keyversion (dst_key_t * key)
{
    ...
    if (major != DST_MAJOR_VERSION || minor != DST_MINOR_VERSION)
        dst_key_setprivateformat (key, DST_MAJOR_VERSION, DST_MINOR_VERSION);
