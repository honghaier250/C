...
#define CMDLINE_FLAGS "3A:a:b:Cc:D:d:E:eFf:Gg:hI:i:K:km:n:P:p:qR:r:S:s:T:t:v:"
        ...
        switch (ch)
        {
            ...
            case 'f':
                ...
                if (toupper (c) == 'K')
                    ...
                    fatal ("unknown flag '%s'", isc_commandline_argument);
                break;
            case 'g':
            ...
            case 'K':
                ...
                if (ret != ISC_R_SUCCESS)
                    fatal ("cannot open directory %s: %s", directory, isc_result_totext (ret));
                break;
            case 'k':
            ...
            case 'p':
                ...
                if (*endp != '\0' || protocol < 0 || protocol > 255)
                    fatal ("-p must be followed by a number " "[0..255]");
                break;
            case 'q':
                quiet = ISC_TRUE;
                break;
            ...
            case 's':
                ...
                if (*endp != '\0' || signatory < 0 || signatory > 15)
                    fatal ("-s must be followed by a number " "[0..15]");
                break;
            case 'T':
                if (strcasecmp (isc_commandline_argument, "KEY") == 0)
                    options |= DST_TYPE_KEY;
                else if (strcasecmp (isc_commandline_argument, "DNSKEY") == 0)
                    /* default behavior */
                    ;
                else
                    fatal ("unknown type '%s'", isc_commandline_argument);
                break;
            case 't':
            ...
            case 'D':
                ...
                {
                    setdel = ISC_TRUE;
                    delete = strtotime (isc_commandline_argument, now, now);
                }
                else
                {
                    unsetdel = ISC_TRUE;
    ...
    ret = dst_lib_init2 (mctx, ectx, engine, ISC_ENTROPY_BLOCKING | ISC_ENTROPY_GOODONLY);
    if (ret != ISC_R_SUCCESS)
    ...
    if (predecessor == NULL)
    {
        ...
        isc_buffer_init (&buf, argv[isc_commandline_index], strlen (argv[isc_commandline_index]));
        isc_buffer_add (&buf, strlen (argv[isc_commandline_index]));
        ret = dns_name_fromtext (name, &buf, dns_rootname, 0, NULL);
        if (ret != ISC_R_SUCCESS)
            fatal ("invalid key name %s: %s", argv[isc_commandline_index], isc_result_totext (ret));
        ...
        if (use_nsec3 &&
            ...
            alg != DST_ALG_RSASHA256 && alg != DST_ALG_RSASHA512 && alg != DST_ALG_ECCGOST)
        {
            fatal ("%s is incompatible with NSEC3; " "do not use the -3 option", algname);
        }
        ...
        if (size < 0)
        {
            if (use_default)
            {
                if ((kskflag & DNS_KEYFLAG_KSK) != 0)
                    ...
                    size = 1024;
                if (verbose > 0)
                    fprintf (stderr, "key size not " "specified; defaulting " "to %d\n", size);
            }
        ...
        if (!oldstyle && prepub > 0)
        {
            ...
            if ((activate - prepub) < now)
                fatal ("Time until activation is shorter " "than the\n\tprepublication interval.");
        }
    }
    else
    {
        ...
        ret = dst_key_fromnamedfile (predecessor, directory, DST_TYPE_PUBLIC | DST_TYPE_PRIVATE, mctx, &prevkey);
        if (ret != ISC_R_SUCCESS)
            fatal ("Invalid keyfile %s: %s", filename, isc_result_totext (ret));
        ...
        if (major != DST_MAJOR_VERSION || minor < DST_MINOR_VERSION)
            fatal ("Key %s has incompatible format version %d.%d\n\t"
                   "It is not possible to generate a successor key.", keystr, major, minor);

        ret = dst_key_gettime (prevkey, DST_TIME_ACTIVATE, &when);
        if (ret != ISC_R_SUCCESS)
            fatal ("Key %s has no activation date.\n\t"
                   "You must use dnssec-settime -A to set one " "before generating a successor.", keystr);

        ret = dst_key_gettime (prevkey, DST_TIME_INACTIVE, &activate);
        if (ret != ISC_R_SUCCESS)
            fatal ("Key %s has no inactivation date.\n\t"
                   "You must use dnssec-settime -I to set one " "before generating a successor.", keystr);
        ...
        if (publish < now)
            fatal ("Key %s becomes inactive\n\t"
                   ...
                   "dnssec-settime -I,\n\t"
                   "or use the -i option to set a shorter " "prepublication interval.", keystr);
    ...
    switch (alg)
    {
        ...
        case DST_ALG_HMACMD5:
            ...
            if ((dbits % 8) != 0)
                fatal ("HMAC-MD5 digest bits %d not divisible by 8", dbits);
            break;
        case DST_ALG_HMACSHA1:
            ...
            if ((dbits % 8) != 0)
                fatal ("HMAC-SHA1 digest bits %d not divisible by 8", dbits);
            break;
        case DST_ALG_HMACSHA224:
            ...
            if ((dbits % 8) != 0)
                fatal ("HMAC-SHA224 digest bits %d not divisible by 8", dbits);
            break;
        case DST_ALG_HMACSHA256:
            ...
            if ((dbits % 8) != 0)
                fatal ("HMAC-SHA256 digest bits %d not divisible by 8", dbits);
            break;
        case DST_ALG_HMACSHA384:
            ...
            if ((dbits % 8) != 0)
                fatal ("HMAC-SHA384 digest bits %d not divisible by 8", dbits);
            break;
        case DST_ALG_HMACSHA512:
            ...
            if ((dbits % 8) != 0)
                fatal ("HMAC-SHA512 digest bits %d not divisible by 8", dbits);
            break;
    }

    if (!(alg == DNS_KEYALG_RSAMD5 || alg == DNS_KEYALG_RSASHA1 ||
          alg == DNS_KEYALG_NSEC3RSASHA1 || alg == DNS_KEYALG_RSASHA256 ||
          alg == DNS_KEYALG_RSASHA512 || alg == DST_ALG_ECCGOST) && rsa_exp != 0)
    ...
    {                            /* KEY / HMAC */
        if (strcasecmp (nametype, "host") == 0 || strcasecmp (nametype, "entity") == 0)
    ...
    if ((options & DST_TYPE_KEY) != 0)    /* KEY / HMAC */
        flags |= signatory;
    else if ((flags & DNS_KEYOWNER_ZONE) != 0)
    {                            /* DNSKEY */
        flags |= kskflag;
        flags |= revflag;
    }
    ...
    else if ((options & DST_TYPE_KEY) == 0 && protocol != DNS_KEYPROTO_DNSSEC)
    ...
    if ((flags & DNS_KEYFLAG_OWNERMASK) == DNS_KEYOWNER_ZONE &&
        (alg == DNS_KEYALG_DH || alg == DST_ALG_HMACMD5 ||
         ...
         alg == DST_ALG_HMACSHA256 || alg == DST_ALG_HMACSHA384 || alg == DST_ALG_HMACSHA512))
        fatal ("a key with algorithm '%s' cannot be a zone key", algname);
    ...
    do
    {
        ...
        if (!quiet && show_progress)
        {
            ...
            ret = dst_key_generate2 (name, alg, size, param, flags, protocol, rdclass, mctx, &key, &progress);
            putc ('\n', stderr);
            fflush (stderr);
        }
        else
        {
            ret = dst_key_generate2 (name, alg, size, param, flags, protocol, rdclass, mctx, &key, NULL);
        }
            ...
            if (setrev)
            {
                if (kskflag == 0)
                    fprintf (stderr, "%s: warning: Key is "
                             ...
                             "was used. Revoking a ZSK is " "legal, but undefined.\n", program);
        ...
        else
        {
            if (setpub || setact || setrev || setinact ||
                setdel || unsetpub || unsetact || unsetrev || unsetinact || unsetdel || genonly)
                fatal ("cannot use -C together with " "-P, -A, -R, -I, -D, or -G options");
            /*
            ...
            if (verbose > 0)
            {
                ...
                if (ret == ISC_R_SUCCESS)
                    fprintf (stderr,
                             ...
                             "key upon revokation.  " "Generating a new key\n", program, filename);
            }
    ...
    if (conflict)
        fatal ("cannot generate a null key due to possible key ID " "collision");
