...
static void
sendrequest (isc_sockaddr_t * srcaddr, isc_sockaddr_t * destaddr, dns_message_t * msg, dns_request_t ** request);

ISC_PLATFORM_NORETURN_PRE static void
fatal (const char *format, ...) ISC_FORMAT_PRINTF (1, 2) ISC_PLATFORM_NORETURN_POST;
     ...
     static void
         send_gssrequest (isc_sockaddr_t * srcaddr, isc_sockaddr_t * destaddr,
                          dns_message_t * msg, dns_request_t ** request, gss_ctx_id_t context);
...
#define STATUS_SYNTAX    (isc_uint16_t)3
     ...
     static void setup_entropy (isc_mem_t * mctx, const char *randomfile, isc_entropy_t ** ectx)
{
    isc_result_t result;
    ...
    result = isc_entropy_usebestsource (*ectx, &source, randomfile, usekeyboard);

    if (result != ISC_R_SUCCESS)
        fatal ("could not initialize entropy source: %s", isc_result_totext (result));
...
static inline void check_result (isc_result_t result, const char *msg)
{
    if (result != ISC_R_SUCCESS)
...
static void reset_system (void)
{
    ...
    {
        result = dns_message_create (mctx, DNS_MESSAGE_INTENTRENDER, &updatemsg);
...
static void setup_keystr (void)
{
    ...
    debug ("keycreate");
    result = dns_tsigkey_create (keyname, hmacname, secret, secretlen, ISC_FALSE, NULL, 0, 0, mctx, NULL, &tsigkey);
    if (result != ISC_R_SUCCESS)
...
static isc_result_t read_sessionkey (isc_mem_t * mctx, isc_log_t * lctx)
{
    ...
    result = cfg_parse_file (pctx, keyfile, &cfg_type_sessionkey, &sessionkey);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
...
static void setup_keyfile (isc_mem_t * mctx, isc_log_t * lctx)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        fprintf (stderr, "could not read key from %.*s.{private,key}: "
                 "%s\n", basenamelen (keyfile), keyfile, isc_result_totext (result));
        return;
    }
    ...
    if (hmacname != NULL)
    {
        result = dns_tsigkey_createfromkey (dst_key_name (dstkey),
                                            hmacname, dstkey, ISC_FALSE, NULL, 0, 0, mctx, NULL, &tsigkey);
        dst_key_free (&dstkey);
        if (result != ISC_R_SUCCESS)
        {
...
static void setup_system (void)
{
    ...
    else
    {
        ...
        for (i = 0; i < ns_total; i++)
        {
            ...
            else
            {
                ...
                isc_sockaddr_fromin6 (&servers[i], &in6, dnsport);
            }
        }
    }
    ...
    if (have_ipv6)
    {
        ...
        result = dns_dispatch_getudp (dispatchmgr, socketmgr, taskmgr,
                                      &bind_any6, PACKETSIZE, 4, 2, 3, 5, attrs, attrmask, &dispatchv6);
    ...
    if (have_ipv4)
    {
        ...
        result = dns_dispatch_getudp (dispatchmgr, socketmgr, taskmgr,
                                      &bind_any, PACKETSIZE, 4, 2, 3, 5, attrs, attrmask, &dispatchv4);
        check_result (result, "dns_dispatch_getudp (v4)");
    }

    result = dns_requestmgr_create (mctx, timermgr,
                                    socketmgr, taskmgr, dispatchmgr, dispatchv4, dispatchv6, &requestmgr);
...
static void get_address (char *host, in_port_t port, isc_sockaddr_t * sockaddr)
{
    int count;
...
static void parse_args (int argc, char **argv, isc_mem_t * mctx, isc_entropy_t ** ectx)
{
    ...
    while ((ch = isc_commandline_parse (argc, argv, PARSE_ARGS_FMT)) != -1)
    {
        switch (ch)
        {
            ...
            case 'l':
                ...
                break;
            case 'L':
                result = isc_parse_uint32 (&i, isc_commandline_argument, 10);
                if (result != ISC_R_SUCCESS)
                {
                    fprintf (stderr, "bad library debug value " "'%s'\n", isc_commandline_argument);
                    exit (1);
                }
                logdebuglevel = i;
                break;
            ...
            case 'o':
                ...
                break;
            case 'p':
                result = isc_parse_uint16 (&dnsport, isc_commandline_argument, 10);
                if (result != ISC_R_SUCCESS)
                {
                ...
                break;
            case 't':
                result = isc_parse_uint32 (&timeout, isc_commandline_argument, 10);
                if (result != ISC_R_SUCCESS)
                {
                ...
                break;
            case 'u':
                result = isc_parse_uint32 (&udp_timeout, isc_commandline_argument, 10);
                if (result != ISC_R_SUCCESS)
                {
                ...
                break;
            case 'r':
                result = isc_parse_uint32 (&udp_retries, isc_commandline_argument, 10);
                if (result != ISC_R_SUCCESS)
                {
    ...
    if (usegsstsig)
    {
        fprintf (stderr, "%s: cannot specify -g    or -o, " "program not linked with GSS API Library\n", argv[0]);
        exit (1);
    }
#endif
    ...
    if (argv[isc_commandline_index] != NULL)
    {
        ...
        {
            result = isc_stdio_open (argv[isc_commandline_index], "r", &input);
            if (result != ISC_R_SUCCESS)
            {
                fprintf (stderr, "could not open '%s': %s\n", argv[isc_commandline_index], isc_result_totext (result));
                exit (1);
            }
        }
             ...
             dns_rdatatype_t rdatatype, dns_message_t * msg, dns_rdata_t * rdata)
{
    ...
    if (*cmdline != 0)
    {
        ...
        check_result (result, "isc_buffer_allocate");
        result = dns_rdata_fromtext (NULL, rdataclass, rdatatype, lex, dns_rootname, 0, mctx, buf, &callbacks);
        isc_lex_destroy (&lex);
        if (result == ISC_R_SUCCESS)
        {
...
static isc_uint16_t evaluate_server (char *cmdline)
{
    ...
    else
    {
        ...
        {
            fprintf (stderr, "port '%s' is out of range " "(1 to 65535)\n", word);
            return (STATUS_SYNTAX);
        }
    }
...
static isc_uint16_t update_addordelete (char *cmdline, isc_boolean_t isdelete)
{
  ...
  parseclass:
    ...
    else
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            fprintf (stderr, "'%s' is not a valid class or type: " "%s\n", word, isc_result_totext (result));
            goto failure;
        }
    }
...
static void show_message (FILE * stream, dns_message_t * msg, const char *description)
{
    ...
    do
    {
        if (bufsz > MAXTEXT)
        {
            fprintf (stderr, "could not allocate large enough " "buffer to display message\n");
            exit (1);
        }
        if (buf != NULL)
            isc_buffer_free (&buf);
    ...
    fprintf (stream, "%s\n%.*s", description, (int) isc_buffer_usedlength (buf), (char *) isc_buffer_base (buf));
    isc_buffer_free (&buf);
}
...
static void send_update (dns_name_t * zonename, isc_sockaddr_t * master, isc_sockaddr_t * srcaddr)
{
    isc_result_t result;
    ...
    result = dns_request_createvia3 (requestmgr, updatemsg, srcaddr,
                                     ...
                                     udp_timeout, udp_retries, global_task, update_completed, NULL, &request);
...
static void recvsoa (isc_task_t * task, isc_event_t * event)
{
    ...
    if (result == DNS_R_TSIGERRORSET && userserver != NULL)
    {
        ...
        result = dns_request_createvia3 (requestmgr, soaquery,
                                         ...
                                         FIND_TIMEOUT * 20, FIND_TIMEOUT, 3, global_task, recvsoa, reqinfo, &request);
 ...
 */
static void get_ticket_realm (isc_mem_t * mctx)
{
    krb5_context ctx;
...
static void start_gssrequest (dns_name_t * master)
{
    ...
    if (result != ISC_R_SUCCESS)
        fatal ("dns_tsigkeyring_create failed: %s", isc_result_totext (result));
    ...
    result = isc_string_printf (servicename, sizeof (servicename), "DNS/%s%s", namestr, realm ? realm : "");
    if (result != ISC_R_SUCCESS)
        fatal ("isc_string_printf(servicename) failed: %s", isc_result_totext (result));
    isc_buffer_init (&buf, servicename, strlen (servicename));
    isc_buffer_add (&buf, strlen (servicename));
    result = dns_name_fromtext (servname, &buf, dns_rootname, 0, NULL);
    if (result != ISC_R_SUCCESS)
        fatal ("dns_name_fromtext(servname) failed: %s", isc_result_totext (result));

    dns_fixedname_init (&fkname);
    keyname = dns_fixedname_name (&fkname);

    isc_random_get (&val);
    result = isc_string_printf (keystr, sizeof (keystr), "%u.sig-%s", val, namestr);
    if (result != ISC_R_SUCCESS)
        fatal ("isc_string_printf(keystr) failed: %s", isc_result_totext (result));
    ...
    if (result != ISC_R_SUCCESS)
        fatal ("dns_name_fromtext(keyname) failed: %s", isc_result_totext (result));
    ...
    if (result != ISC_R_SUCCESS)
        fatal ("dns_message_create failed: %s", isc_result_totext (result));

    /* Build first request. */
    context = GSS_C_NO_CONTEXT;
    result = dns_tkey_buildgssquery (rmsg, keyname, servname, NULL, 0, &context, use_win2k_gsstsig, mctx, &err_message);
    if (result == ISC_R_FAILURE)
        fatal ("tkey query failed: %s", err_message != NULL ? err_message : "unknown error");
    if (result != ISC_R_SUCCESS)
        fatal ("dns_tkey_buildgssquery failed: %s", isc_result_totext (result));
...
static void recvgss (isc_task_t * task, isc_event_t * event)
{
    ...
    if (debugging)
        show_message (stderr, rcvmsg, "recvmsg reply from GSS-TSIG query");

    if (rcvmsg->rcode == dns_rcode_formerr && !tried_other_gsstsig)
    {
        ddebug ("recvgss trying %s GSS-TSIG", use_win2k_gsstsig ? "Standard" : "Win2k");
    ...
    result = dns_tkey_gssnegotiate (tsigquery, rcvmsg, servname,
                                    &context, &tsigkey, gssring, use_win2k_gsstsig, &err_message);
    switch (result)
    {

        case DNS_R_CONTINUE:
            send_gssrequest (localaddr, kserver, tsigquery, &request, context);
            break;
        ...
        default:
            fatal ("dns_tkey_negotiategss: %s %s", isc_result_totext (result), err_message != NULL ? err_message : "");
    }
...
static void start_update (void)
{
    ...
    result = dns_message_create (mctx, DNS_MESSAGE_INTENTRENDER, &soaquery);
    check_result (result, "dns_message_create");
    ...
    else
    {
        ...
        if (section == DNS_SECTION_UPDATE &&
            !dns_name_equal (firstname, dns_rootname) && tmprdataset->type == dns_rdatatype_ds)
        {
