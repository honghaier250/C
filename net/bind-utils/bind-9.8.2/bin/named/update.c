...
static void
update_log (ns_client_t * client, dns_zone_t * zone, int level, const char *fmt, ...) ISC_FORMAT_PRINTF (4, 5);

     static void update_log (ns_client_t * client, dns_zone_t * zone, int level, const char *fmt, ...)
{
    va_list ap;
    ...
    dns_rdataclass_format (dns_zone_getclass (zone), classbuf, sizeof (classbuf));
    ...
    ns_client_log (client, NS_LOGCATEGORY_UPDATE, NS_LOGMODULE_UPDATE,
                   level, "updating zone '%s/%s': %s", namebuf, classbuf, message);
}
               ...
               dns_acl_t * updateacl, dns_ssutable_t * ssutable)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        dns_rdataclass_format (client->view->rdclass, classbuf, sizeof (classbuf));

        level = (updateacl == NULL && ssutable == NULL) ? ISC_LOG_INFO : ISC_LOG_ERROR;

        ns_client_log (client, NS_LOGCATEGORY_UPDATE_SECURITY,
                       NS_LOGMODULE_UPDATE, level, "update '%s/%s' denied due to allow-query", namebuf, classbuf);
    }
    else if (updateacl == NULL && ssutable == NULL)
    {
        ...
        ns_client_log (client, NS_LOGCATEGORY_UPDATE_SECURITY,
                       NS_LOGMODULE_UPDATE, ISC_LOG_INFO, "update '%s/%s' denied", namebuf, classbuf);
    }
    return (result);
}
...
checkupdateacl (ns_client_t * client, dns_acl_t * acl, const char *message,
                dns_name_t * zonename, isc_boolean_t slave, isc_boolean_t has_ssutable)
{
    char namebuf[DNS_NAME_FORMATSIZE];
    ...
    if (client->signer != NULL)
    {
        ...
        ns_client_log (client, NS_LOGCATEGORY_UPDATE_SECURITY,
                       NS_LOGMODULE_UPDATE, ISC_LOG_INFO, "signer \"%s\" %s", namebuf, msg);
    }
    ...
    ns_client_log (client, NS_LOGCATEGORY_UPDATE_SECURITY,
                   NS_LOGMODULE_UPDATE, level, "%s '%s/%s' %s", message, namebuf, classbuf, msg);
    return (result);
}
 ...
 */
static isc_result_t
foreach_rrset (dns_db_t * db, dns_dbversion_t * ver, dns_name_t * name, rrset_func * action, void *action_data)
{
    isc_result_t result;
    ...
    for (result = dns_rdatasetiter_first (iter); result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (iter))
    {
 ...
 */
static isc_result_t
foreach_node_rr (dns_db_t * db, dns_dbversion_t * ver, dns_name_t * name, rr_func * rr_action, void *rr_action_data)
{
...
foreach_rr (dns_db_t * db, dns_dbversion_t * ver, dns_name_t * name,
            dns_rdatatype_t type, dns_rdatatype_t covers, rr_func * rr_action, void *rr_action_data)
{
    ...
    if (type == dns_rdatatype_any)
        return (foreach_node_rr (db, ver, name, rr_action, rr_action_data));

    node = NULL;
    if (type == dns_rdatatype_nsec3 || (type == dns_rdatatype_rrsig && covers == dns_rdatatype_nsec3))
    ...
    dns_rdataset_init (&rdataset);
    result = dns_db_findrdataset (db, node, ver, type, covers, (isc_stdtime_t) 0, &rdataset, NULL);
    if (result == ISC_R_NOTFOUND)
    {
        result = ISC_R_SUCCESS;
        goto cleanup_node;
...
rrset_exists (dns_db_t * db, dns_dbversion_t * ver, dns_name_t * name,
              dns_rdatatype_t type, dns_rdatatype_t covers, isc_boolean_t * exists)
{
    isc_result_t result;

    result = foreach_rr (db, ver, name, type, covers, rrset_exists_action, NULL);
 ...
 */
static isc_result_t
rrset_visible (dns_db_t * db, dns_dbversion_t * ver, dns_name_t * name, dns_rdatatype_t type, isc_boolean_t * visible)
{
    isc_result_t result;
    ...
    result = dns_db_find (db, name, ver, type, DNS_DBFIND_NOWILD,
                          (isc_stdtime_t) 0, NULL, dns_fixedname_name (&fixed), NULL, NULL);
    switch (result)
    {
        case ISC_R_SUCCESS:
            *visible = ISC_TRUE;
...
{
    UNUSED (data);
    if (rrset->type != dns_rdatatype_cname && !dns_rdatatype_isdnssec (rrset->type))
        return (ISC_R_EXISTS);
    return (ISC_R_SUCCESS);
}
 ...
 */
static isc_result_t
cname_incompatible_rrset_exists (dns_db_t * db, dns_dbversion_t * ver, dns_name_t * name, isc_boolean_t * exists)
{
    isc_result_t result;
...
static isc_result_t ssu_checkrule (void *data, dns_rdataset_t * rrset)
{
     ...
     */
    if (rrset->type == dns_rdatatype_rrsig || rrset->type == dns_rdatatype_nsec)
        return (ISC_R_SUCCESS);
    result = dns_ssutable_checkrules (ssuinfo->table, ssuinfo->signer,
                                      ssuinfo->name, ssuinfo->tcpaddr, rrset->type, ssuinfo->key);
    return (result == ISC_TRUE ? ISC_R_SUCCESS : ISC_R_FAILURE);
}

static isc_boolean_t
ssu_checkall (dns_db_t * db, dns_dbversion_t * ver, dns_name_t * name,
              dns_ssutable_t * ssutable, dns_name_t * signer, isc_netaddr_t * tcpaddr, dst_key_t * key)
{
    isc_result_t result;
...
static isc_result_t temp_append (dns_diff_t * diff, dns_name_t * name, dns_rdata_t * rdata)
{
    ...
    CHECK (dns_difftuple_create (diff->mctx, DNS_DIFFOP_EXISTS, name, 0, rdata, &tuple));
        ...
        *typep = t->rdata.type;
        ...
        while (t != NULL && dns_name_equal (&t->name, name))
        {
            ...
            *typep = type = t->rdata.type;
             ...
             */
            while (t != NULL && dns_name_equal (&t->name, name) && t->rdata.type == type)
            {
            ...
            result = temp_check_rrset (ISC_LIST_HEAD (u_rrs.tuples), ISC_LIST_HEAD (d_rrs.tuples));
            if (result != ISC_R_SUCCESS)
                goto failure;
...
static isc_boolean_t type_not_soa_nor_ns_p (dns_rdata_t * update_rr, dns_rdata_t * db_rr)
{
    ...
    return ((db_rr->type != dns_rdatatype_soa &&
             ...
             db_rr->type != dns_rdatatype_rrsig && db_rr->type != dns_rdatatype_nsec) ? ISC_TRUE : ISC_FALSE);
}
...
{
    UNUSED (update_rr);
    return ((db_rr->type != dns_rdatatype_rrsig && db_rr->type != dns_rdatatype_nsec) ? ISC_TRUE : ISC_FALSE);
}
...
{
    UNUSED (update_rr);
    return ((db_rr->type == dns_rdatatype_rrsig) ? ISC_TRUE : ISC_FALSE);
}
...
static isc_boolean_t rr_equal_p (dns_rdata_t * update_rr, dns_rdata_t * db_rr)
{
     ...
     */
    return (dns_rdata_casecompare (update_rr, db_rr) == 0 ? ISC_TRUE : ISC_FALSE);
}
...
static isc_boolean_t replaces_p (dns_rdata_t * update_rr, dns_rdata_t * db_rr)
{
    ...
    if (db_rr->type == dns_rdatatype_rrsig)
    {
        ...
        if (dbsig.keyid == updatesig.keyid &&
            dbsig.covered == updatesig.covered && dbsig.algorithm == updatesig.algorithm)
            return (ISC_TRUE);
    }
    if (db_rr->type == dns_rdatatype_wks)
    {
        ...
        return (memcmp (db_rr->data, update_rr->data, 5) == 0 ? ISC_TRUE : ISC_FALSE);
    }
    ...
    if (db_rr->type == dns_rdatatype_nsec3param)
    {
        ...
        if (db_rr->data[0] == update_rr->data[0] &&
            memcmp (db_rr->data + 2, update_rr->data + 2, update_rr->length - 2) == 0)
            return (ISC_TRUE);
    }
    return (ISC_FALSE);
}
...
static isc_result_t delete_if_action (void *data, rr_t * rr)
{
    ...
    if ((*ctx->predicate) (ctx->update_rr, &rr->rdata))
    {
        ...
        result = update_one_rr (ctx->db, ctx->ver, ctx->diff, DNS_DIFFOP_DEL, ctx->name, rr->ttl, &rr->rdata);
        return (result);
    }
    else
    {
...
delete_if (rr_predicate * predicate, dns_db_t * db, dns_dbversion_t * ver,
           dns_name_t * name, dns_rdatatype_t type, dns_rdatatype_t covers, dns_rdata_t * update_rr, dns_diff_t * diff)
{
...
static isc_result_t add_rr_prepare_action (void *data, rr_t * rr)
{
    ...
    {
        CHECK (dns_difftuple_create (ctx->del_diff.mctx, DNS_DIFFOP_DEL, ctx->name, rr->ttl, &rr->rdata, &tuple));
    ...
    if (rr->ttl != ctx->update_rr_ttl)
    {
        ...
        if (!equal)
        {
            CHECK (dns_difftuple_create (ctx->add_diff.mctx,
                                         DNS_DIFFOP_ADD, ctx->name, ctx->update_rr_ttl, &rr->rdata, &tuple));
...
get_current_rr (dns_message_t * msg, dns_section_t section,
                ...
                dns_rdata_t * rdata, dns_rdatatype_t * covers, dns_ttl_t * ttl, dns_rdataclass_t * update_class)
{
    dns_rdataset_t *rdataset;
 ...
 */
static isc_result_t
check_soa_increment (dns_db_t * db, dns_dbversion_t * ver, dns_rdata_t * update_rdata, isc_boolean_t * ok)
{
...
static isc_result_t namelist_append_name (dns_diff_t * list, dns_name_t * name)
{
    ...
    CHECK (dns_difftuple_create (list->mctx, DNS_DIFFOP_EXISTS, name, 0, &dummy_rdata, &tuple));
...
static isc_result_t namelist_append_subdomain (dns_db_t * db, dns_name_t * name, dns_diff_t * affected)
{
    ...
    for (result = dns_dbiterator_seek (dbit, name); result == ISC_R_SUCCESS; result = dns_dbiterator_next (dbit))
    {
...
static isc_result_t is_non_nsec_action (void *data, dns_rdataset_t * rrset)
{
    ...
    if (!(rrset->type == dns_rdatatype_nsec ||
          ...
          (rrset->type == dns_rdatatype_rrsig &&
           (rrset->covers == dns_rdatatype_nsec || rrset->covers == dns_rdatatype_nsec3))))
        return (ISC_R_EXISTS);
    return (ISC_R_SUCCESS);
}
 ...
 */
static isc_result_t
non_nsec_rrset_exists (dns_db_t * db, dns_dbversion_t * ver, dns_name_t * name, isc_boolean_t * exists)
{
    isc_result_t result;
           ...
           isc_boolean_t * flag, isc_boolean_t * cut, isc_boolean_t * unsecure)
{
    ...
    result = dns_db_find (db, name, ver, dns_rdatatype_any,
                          ...
                          (isc_stdtime_t) 0, NULL, dns_fixedname_name (&foundname), NULL, NULL);
        ...
        *cut = ISC_TRUE;
        if (unsecure != NULL)
        {
            ...
            if (dns_db_find (db, name, ver, dns_rdatatype_ds, 0,
                             (isc_stdtime_t) 0, NULL, dns_fixedname_name (&foundname), NULL, NULL) == DNS_R_NXRRSET)
                *unsecure = ISC_TRUE;
            else
                *unsecure = ISC_FALSE;
        }
    ...
    else if (result == DNS_R_GLUE || result == DNS_R_DNAME || result == DNS_R_DELEGATION || result == DNS_R_NXDOMAIN)
    {
        *flag = ISC_FALSE;
        *cut = ISC_FALSE;
        if (unsecure != NULL)
...
next_active (ns_client_t * client, dns_zone_t * zone, dns_db_t * db,
             dns_dbversion_t * ver, dns_name_t * oldname, dns_name_t * newname, isc_boolean_t forward)
{
    isc_result_t result;
    ...
    do
    {
        ...
        if (result == ISC_R_NOMORE)
        {
            ...
            if (wraps == 2)
            {
                update_log (client, zone, ISC_LOG_ERROR, "secure zone with no NSECs");
        ...
        else
        {
            ...
            result = dns_db_find (db, newname, ver, dns_rdatatype_soa, DNS_DBFIND_NOWILD, 0, NULL, found, NULL, NULL);
            if (result == ISC_R_SUCCESS ||
                result == DNS_R_EMPTYNAME ||
                result == DNS_R_NXRRSET ||
                result == DNS_R_CNAME || (result == DNS_R_DELEGATION && dns_name_equal (newname, found)))
            {
                has_nsec = ISC_TRUE;
                result = ISC_R_SUCCESS;
            }
...
add_nsec (ns_client_t * client, dns_zone_t * zone, dns_db_t * db,
          dns_dbversion_t * ver, dns_name_t * name, dns_ttl_t nsecttl, dns_diff_t * diff)
{
    isc_result_t result;
     ...
     */
    CHECK (delete_if (true_p, db, ver, name, dns_rdatatype_nsec, 0, NULL, diff));
    /*
     * Add the new NSEC and record the change.
     */
    CHECK (dns_difftuple_create (diff->mctx, DNS_DIFFOP_ADD, name, nsecttl, &rdata, &tuple));
...
static isc_result_t add_placeholder_nsec (dns_db_t * db, dns_dbversion_t * ver, dns_name_t * name, dns_diff_t * diff)
{
    ...
    CHECK (dns_difftuple_create (diff->mctx, DNS_DIFFOP_ADD, name, 0, &rdata, &tuple));
    CHECK (do_one_tuple (&tuple, db, ver, diff));
  failure:
    return (result);
}
                ...
                isc_mem_t * mctx, unsigned int maxkeys, dst_key_t ** keys, unsigned int *nkeys)
{
    ...
    CHECK (dns_dnssec_findzonekeys2 (db, ver, node, dns_db_origin (db), directory, mctx, maxkeys, keys, nkeys));
  failure:
    if (node != NULL)
        dns_db_detachnode (db, &node);
    return (result);
...
add_sigs (ns_client_t * client, dns_zone_t * zone, dns_db_t * db,
          ...
          isc_stdtime_t inception, isc_stdtime_t expire, isc_boolean_t check_ksk, isc_boolean_t keyset_kskonly)
{
    isc_result_t result;
    ...
    if (type == dns_rdatatype_nsec3)
        ...
        CHECK (dns_db_findnode (db, name, ISC_FALSE, &node));
    CHECK (dns_db_findrdataset (db, node, ver, type, 0, (isc_stdtime_t) 0, &rdataset, NULL));
    dns_db_detachnode (db, &node);
...
#define ALG(x) dst_key_alg(x)
    ...
    if (!added_sig)
    {
        update_log (client, zone, ISC_LOG_ERROR, "found no active private keys, " "unable to generate any signatures");
             ...
             dns_diff_t * diff, dst_key_t ** keys, unsigned int nkeys)
{
    ...
    result = dns_db_findrdataset (db, node, ver, dns_rdatatype_rrsig,
                                  dns_rdatatype_dnskey, (isc_stdtime_t) 0, &rdataset, NULL);
    dns_db_detachnode (db, &node);
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        for (i = 0; i < nkeys; i++)
        {
            if (rrsig.keyid == dst_key_id (keys[i]))
            {
                ...
                result = update_one_rr (db, ver, diff, DNS_DIFFOP_DEL, name, rdataset.ttl, &rdata);
                break;
            }
        }
        /*
                  ...
                  isc_stdtime_t inception, isc_stdtime_t expire, isc_boolean_t check_ksk, isc_boolean_t keyset_kskonly)
{
    ...
    for (result = dns_rdatasetiter_first (iter); result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (iter))
    {
         ...
         */
        if ((type == dns_rdatatype_rrsig) || (cut && type != dns_rdatatype_ds))
            continue;
        result = rrset_exists (db, ver, name, dns_rdatatype_rrsig, type, &flag);
        if (result != ISC_R_SUCCESS)
            goto cleanup_iterator;
        if (flag)
            continue;;
        result = add_sigs (client, zone, db, ver, name, type, diff,
                           keys, nkeys, inception, expire, check_ksk, keyset_kskonly);
        if (result != ISC_R_SUCCESS)
            goto cleanup_iterator;
    }
    if (result == ISC_R_NOMORE)
                   ...
                   dns_diff_t * diff, isc_uint32_t sigvalidityinterval)
{
    ...
    result = find_zone_keys (zone, db, newver, client->mctx, DNS_MAXZONEKEYS, zone_keys, &nkeys);
    if (result != ISC_R_SUCCESS)
    {
        update_log (client, zone, ISC_LOG_ERROR, "could not get zone keys for secure dynamic update");
        goto failure;
    }
     ...
     * and one doesn't.
     */
    check_ksk = ISC_TF ((dns_zone_getoptions (zone) & DNS_ZONEOPT_UPDATECHECKKSK) != 0);
    keyset_kskonly = ISC_TF ((dns_zone_getoptions (zone) & DNS_ZONEOPT_DNSKEYKSKONLY) != 0);
    ...
    CHECK (dns_db_findnode (db, dns_db_origin (db), ISC_FALSE, &node));
    dns_rdataset_init (&rdataset);
    CHECK (dns_db_findrdataset (db, node, newver, dns_rdatatype_soa, 0, (isc_stdtime_t) 0, &rdataset, NULL));
    ...
    while (t != NULL)
    {
        ...
        while (t != NULL && dns_name_equal (&t->name, name))
        {
             ...
             */
            if (type == dns_rdatatype_dnskey)
                del_keysigs (db, newver, name, &sig_diff, zone_keys, nkeys);
            else
                CHECK (delete_if (true_p, db, newver, name, dns_rdatatype_rrsig, type, NULL, &sig_diff));
            ...
            if (flag)
            {
                CHECK (add_sigs (client, zone, db, newver, name,
                                 type, &sig_diff, zone_keys, nkeys, inception, expire, check_ksk, keyset_kskonly));
            }
          skip:
    ...
    for (t = ISC_LIST_HEAD (diffnames.tuples); t != NULL; t = ISC_LIST_NEXT (t, link))
    {
        CHECK (non_nsec_rrset_exists (db, newver, &t->name, &flag));
        if (!flag)
        {
            CHECK (delete_if (true_p, db, newver, &t->name, dns_rdatatype_any, 0, NULL, &sig_diff));
        }
    }
    update_log (client, zone, ISC_LOG_DEBUG (3), "removed any orphaned NSEC records");

    /*
     * See if we need to build NSEC or NSEC3 chains.
     */
    CHECK (dns_private_chains (db, newver, privatetype, &build_nsec, &build_nsec3));
    if (!build_nsec)
        goto update_nsec3;
    ...
    for (t = ISC_LIST_HEAD (diffnames.tuples); t != NULL; t = ISC_LIST_NEXT (t, link))
    {
         ...
         */
        CHECK (next_active (client, zone, db, newver, &t->name, prevname, ISC_FALSE));
    ...
    for (t = ISC_LIST_HEAD (diffnames.tuples); t != NULL; t = ISC_LIST_NEXT (t, link))
    {
        ...
        CHECK (rrset_exists (db, oldver, &t->name, dns_rdatatype_ns, 0, &ns_existed));
        CHECK (rrset_exists (db, oldver, &t->name, dns_rdatatype_dname, 0, &dname_existed));
        CHECK (rrset_exists (db, newver, &t->name, dns_rdatatype_ns, 0, &ns_exists));
        CHECK (rrset_exists (db, newver, &t->name, dns_rdatatype_dname, 0, &dname_exists));
     ...
     * the NSEC chain.
     */
    for (t = ISC_LIST_HEAD (affected.tuples); t != NULL; t = ISC_LIST_NEXT (t, link))
    {
        ...
        if (!flag)
        {
             ...
             */
            CHECK (delete_if (true_p, db, newver, name, dns_rdatatype_nsec, 0, NULL, &nsec_diff));
            CHECK (delete_if (rrsig_p, db, newver, name, dns_rdatatype_any, 0, NULL, diff));
        }
        else
        {
            /*
            ...
            {
                CHECK (rrset_exists (db, newver, name, dns_rdatatype_nsec, 0, &flag));
                if (!flag)
                    CHECK (add_placeholder_nsec (db, newver, name, diff));
            }
            CHECK (add_exposed_sigs (client, zone, db, newver, name,
                                     cut, &sig_diff, zone_keys, nkeys, inception, expire, check_ksk, keyset_kskonly));
        }
    }
    ...
    for (t = ISC_LIST_HEAD (affected.tuples); t != NULL; t = ISC_LIST_NEXT (t, link))
    {
        CHECK (rrset_exists (db, newver, &t->name, dns_rdatatype_nsec, 0, &flag));
        if (flag)
        {
            /*
             ...
             */
            CHECK (add_nsec (client, zone, db, newver, &t->name, nsecttl, &nsec_diff));
        }
    }
    ...
    update_log (client, zone, ISC_LOG_DEBUG (3), "signing rebuilt NSEC chain");

    /* Update RRSIG NSECs. */
    for (t = ISC_LIST_HEAD (nsec_mindiff.tuples); t != NULL; t = ISC_LIST_NEXT (t, link))
    {
        if (t->op == DNS_DIFFOP_DEL)
        {
            CHECK (delete_if (true_p, db, newver, &t->name, dns_rdatatype_rrsig, dns_rdatatype_nsec, NULL, &sig_diff));
        }
        else if (t->op == DNS_DIFFOP_ADD)
        {
            CHECK (add_sigs (client, zone, db, newver, &t->name,
                             dns_rdatatype_nsec, &sig_diff,
                             zone_keys, nkeys, inception, expire, check_ksk, keyset_kskonly));
        }
        else
        {
            INSIST (0);
  ...
  update_nsec3:
    ...
    if (!build_nsec3)
    {
        update_log (client, zone, ISC_LOG_DEBUG (3), "no NSEC3 chains to rebuild");
        goto failure;
    }
    ...
    while (t != NULL)
    {
        ...
        if (t->rdata.type == dns_rdatatype_nsec || t->rdata.type == dns_rdatatype_rrsig)
        {
            t = ISC_LIST_NEXT (t, link);
            continue;
        }
        ...
        CHECK (rrset_exists (db, oldver, name, dns_rdatatype_ns, 0, &ns_existed));
        CHECK (rrset_exists (db, oldver, name, dns_rdatatype_dname, 0, &dname_existed));
        CHECK (rrset_exists (db, newver, name, dns_rdatatype_ns, 0, &ns_exists));
        CHECK (rrset_exists (db, newver, name, dns_rdatatype_dname, 0, &dname_exists));
    ...
    for (t = ISC_LIST_HEAD (affected.tuples); t != NULL; t = ISC_LIST_NEXT (t, link))
    {
        ...
        {
            CHECK (delete_if (rrsig_p, db, newver, name, dns_rdatatype_any, 0, NULL, diff));
            CHECK (dns_nsec3_delnsec3sx (db, newver, name, privatetype, &nsec_diff));
        }
        else
        {
            CHECK (add_exposed_sigs (client, zone, db, newver, name,
                                     cut, &sig_diff, zone_keys, nkeys, inception, expire, check_ksk, keyset_kskonly));
            CHECK (dns_nsec3_addnsec3sx (db, newver, name, nsecttl, unsecure, privatetype, &nsec_diff));
        }
    }
    ...
    update_log (client, zone, ISC_LOG_DEBUG (3), "signing rebuilt NSEC3 chain");
    ...
    for (t = ISC_LIST_HEAD (nsec_mindiff.tuples); t != NULL; t = ISC_LIST_NEXT (t, link))
    {
        if (t->op == DNS_DIFFOP_DEL)
        {
            CHECK (delete_if (true_p, db, newver, &t->name, dns_rdatatype_rrsig, dns_rdatatype_nsec3, NULL, &sig_diff));
        }
        else if (t->op == DNS_DIFFOP_ADD)
        {
            CHECK (add_sigs (client, zone, db, newver, &t->name,
                             dns_rdatatype_nsec3,
                             &sig_diff, zone_keys, nkeys, inception, expire, check_ksk, keyset_kskonly));
        }
        else
        {
            INSIST (0);
...
static isc_result_t send_update_event (ns_client_t * client, dns_zone_t * zone)
{
    ...
    event = (update_event_t *)
        isc_event_allocate (client->mctx, client, DNS_EVENT_UPDATE, update_action, NULL, sizeof (*event));
    if (event == NULL)
        FAIL (ISC_R_NOMEMORY);
    event->zone = zone;
...
static void respond (ns_client_t * client, isc_result_t result)
{
  ...
  msg_failure:
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_UPDATE, NS_LOGMODULE_UPDATE,
                   ISC_LOG_ERROR, "could not create update response message: %s", isc_result_totext (msg_result));
...
void ns_update_start (ns_client_t * client, isc_result_t sigresult)
{
    ...
    if (ISC_LIST_NEXT (zone_rdataset, link) != NULL)
        FAILC (DNS_R_FORMERR, "update zone section contains multiple RRs");
    ...
    switch (dns_zone_gettype (zone))
    {
        ...
        case dns_zone_slave:
            CHECK (checkupdateacl (client, dns_zone_getforwardacl (zone),
                                   "update forwarding", zonename, ISC_TRUE, ISC_FALSE));
...
static isc_result_t remove_orphaned_ds (dns_db_t * db, dns_dbversion_t * newver, dns_diff_t * diff)
{
    ...
    for (tupple = ISC_LIST_HEAD (diff->tuples); tupple != NULL; tupple = ISC_LIST_NEXT (tupple, link))
    {
        if (!((tupple->op == DNS_DIFFOP_DEL &&
               tupple->rdata.type == dns_rdatatype_ns) ||
              (tupple->op == DNS_DIFFOP_ADD && tupple->rdata.type == dns_rdatatype_ds)))
            continue;
        CHECK (rrset_exists (db, newver, &tupple->name, dns_rdatatype_ns, 0, &ns_exists));
        if (ns_exists && !dns_name_equal (&tupple->name, dns_db_origin (db)))
            continue;
        CHECK (delete_if (true_p, db, newver, &tupple->name, dns_rdatatype_ds, 0, NULL, &temp_diff));
    }
    result = ISC_R_SUCCESS;

  failure:
    for (tupple = ISC_LIST_HEAD (temp_diff.tuples); tupple != NULL; tupple = ISC_LIST_HEAD (temp_diff.tuples))
    {
...
check_mx (ns_client_t * client, dns_zone_t * zone, dns_db_t * db, dns_dbversion_t * newver, dns_diff_t * diff)
{
    ...
    {
        if (t->op != DNS_DIFFOP_ADD || t->rdata.type != dns_rdatatype_mx)
            continue;
        ...
        isaddress = ISC_FALSE;
        if ((options & DNS_RDATA_CHECKMX) != 0 && strlcpy (tmp, namebuf, sizeof (tmp)) < sizeof (tmp))
        {
            if (tmp[strlen (tmp) - 1] == '.')
                tmp[strlen (tmp) - 1] = '\0';
            if (inet_aton (tmp, &addr) == 1 || inet_pton (AF_INET6, tmp, &addr6) == 1)
                isaddress = ISC_TRUE;
        }
        ...
        if (isaddress && (options & DNS_RDATA_CHECKMXFAIL) != 0)
        {
            update_log (client, zone, ISC_LOG_ERROR,
                        "%s/MX: '%s': %s", ownerbuf, namebuf, dns_result_totext (DNS_R_MXISADDRESS));
            ok = ISC_FALSE;
        }
        else if (isaddress)
        {
            update_log (client, zone, ISC_LOG_WARNING,
                        "%s/MX: warning: '%s': %s", ownerbuf, namebuf, dns_result_totext (DNS_R_MXISADDRESS));
        }
        ...
        if ((options & DNS_ZONEOPT_CHECKINTEGRITY) == 0)
            continue;
        result = dns_db_find (db, &mx.mx, newver, dns_rdatatype_a, 0, 0, NULL, foundname, NULL, NULL);
        if (result == ISC_R_SUCCESS)
            continue;

        if (result == DNS_R_NXRRSET)
        {
            result = dns_db_find (db, &mx.mx, newver, dns_rdatatype_aaaa, 0, 0, NULL, foundname, NULL, NULL);
            if (result == ISC_R_SUCCESS)
                continue;
        }
        ...
        {
            update_log (client, zone, ISC_LOG_ERROR,
                        "%s/MX '%s' has no address records " "(A or AAAA)", ownerbuf, namebuf);
            ok = ISC_FALSE;
        }
        else if (result == DNS_R_CNAME)
        {
            update_log (client, zone, ISC_LOG_ERROR, "%s/MX '%s' is a CNAME (illegal)", ownerbuf, namebuf);
            ok = ISC_FALSE;
        }
        else if (result == DNS_R_DNAME)
        {
            ...
            update_log (client, zone, ISC_LOG_ERROR,
                        "%s/MX '%s' is below a DNAME '%s' (illegal)", ownerbuf, namebuf, altbuf);
            ok = ISC_FALSE;
        }
    }
...
static isc_result_t
get_iterations (dns_db_t * db, dns_dbversion_t * ver, dns_rdatatype_t privatetype, unsigned int *iterationsp)
{
    dns_dbnode_t *node = NULL;
    ...
    if (result != ISC_R_SUCCESS)
        return (result);
    result = dns_db_findrdataset (db, node, ver, dns_rdatatype_nsec3param, 0, (isc_stdtime_t) 0, &rdataset, NULL);
    if (result == ISC_R_NOTFOUND)
        goto try_private;
    if (result != ISC_R_SUCCESS)
        goto failure;
  ...
  try_private:
    ...
    result = dns_db_findrdataset (db, node, ver, privatetype, 0, (isc_stdtime_t) 0, &rdataset, NULL);
    if (result == ISC_R_NOTFOUND)
        goto success;
    if (result != ISC_R_SUCCESS)
        goto failure;
 ...
 */
static isc_result_t
check_dnssec (ns_client_t * client, dns_zone_t * zone, dns_db_t * db, dns_dbversion_t * ver, dns_diff_t * diff)
{
    ...
    if (nseconly && nsec3)
    {
        update_log (client, zone, ISC_LOG_ERROR, "NSEC only DNSKEYs and NSEC3 chains not allowed");
        result = DNS_R_REFUSED;
        goto failure;
    }
    ...
    {
        update_log (client, zone, ISC_LOG_ERROR,
                    "too many NSEC3 iterations (%u) for " "weakest DNSKEY (%u)", iterations, max);
        result = DNS_R_REFUSED;
        goto failure;
    }
                        ...
                        dns_dbversion_t * ver, dns_diff_t * diff)
{
    ...
    update_log (client, zone, ISC_LOG_DEBUG (3), "checking for NSEC3PARAM changes");
    ...
    for (tuple = ISC_LIST_HEAD (diff->tuples); tuple != NULL; tuple = next)
    {
        ...
        if (tuple->rdata.type != dns_rdatatype_nsec3param || !dns_name_equal (name, &tuple->name))
            continue;
     ...
     * delayed changes.
     */
    for (tuple = ISC_LIST_HEAD (temp_diff.tuples); tuple != NULL; tuple = next)
    {
        if (tuple->op == DNS_DIFFOP_ADD)
        {
            if (!ttl_good)
            ...
            while (next != NULL)
            {
                ...
                if (next->op == DNS_DIFFOP_DEL &&
                    next->rdata.length == tuple->rdata.length && !memcmp (next_data, tuple_data, next->rdata.length))
                {
    ...
    for (tuple = ISC_LIST_HEAD (temp_diff.tuples); tuple != NULL; tuple = next)
    {
        ...
        if ((tuple->rdata.data[1] & ~DNS_NSEC3FLAG_OPTOUT) != 0)
        {
            ...
            op = (tuple->op == DNS_DIFFOP_DEL) ? DNS_DIFFOP_ADD : DNS_DIFFOP_DEL;
            CHECK (dns_difftuple_create (diff->mctx, op, name, ttl, &tuple->rdata, &newtuple));
        ...
        if (tuple->op == DNS_DIFFOP_ADD)
        {
            ...
            while (next != NULL)
            {
                ...
                if (next->op != DNS_DIFFOP_DEL ||
                    ...
                    next_data[3] != tuple_data[3] || memcmp (next_data + 4, tuple_data + 4, tuple->rdata.length - 4))
                {
                    next = ISC_LIST_NEXT (next, link);
                    continue;
                }
             ...
             */
            dns_nsec3param_toprivate (&tuple->rdata, &rdata, privatetype, buf, sizeof (buf));
             ...
             */
            next = ISC_LIST_NEXT (tuple, link);
            CHECK (dns_difftuple_create (diff->mctx, DNS_DIFFOP_DEL, name, ttl, &tuple->rdata, &newtuple));
            CHECK (do_one_tuple (&newtuple, db, ver, diff));
            ISC_LIST_UNLINK (temp_diff.tuples, tuple, link);
            dns_diff_appendminimal (diff, &tuple);
            dns_rdata_reset (&rdata);
    ...
    for (tuple = ISC_LIST_HEAD (temp_diff.tuples); tuple != NULL; tuple = next)
    {
        ...
        CHECK (dns_difftuple_create (diff->mctx, DNS_DIFFOP_ADD, name, ttl, &tuple->rdata, &newtuple));
        CHECK (do_one_tuple (&newtuple, db, ver, diff));
        ISC_LIST_UNLINK (temp_diff.tuples, tuple, link);
        dns_diff_appendminimal (diff, &tuple);
        dns_rdata_reset (&rdata);
...
static isc_result_t
rollback_private (dns_db_t * db, dns_rdatatype_t privatetype, dns_dbversion_t * ver, dns_diff_t * diff)
{
    dns_diff_t temp_diff;
    ...
    for (tuple = ISC_LIST_HEAD (diff->tuples); tuple != NULL; tuple = next)
    {
        ...
        if (tuple->rdata.type != privatetype || !dns_name_equal (name, &tuple->name))
            continue;
        ...
        if (tuple->op == DNS_DIFFOP_DEL &&
            tuple->rdata.length == 5 && tuple->rdata.data[0] != 0 && tuple->rdata.data[4] != 0)
            continue;
    ...
    {
        op = (tuple->op == DNS_DIFFOP_DEL) ? DNS_DIFFOP_ADD : DNS_DIFFOP_DEL;
        CHECK (dns_difftuple_create (mctx, op, name, tuple->ttl, &tuple->rdata, &newtuple));
...
add_signing_records (dns_db_t * db, dns_rdatatype_t privatetype, dns_dbversion_t * ver, dns_diff_t * diff)
{
     ...
     */
    for (tuple = ISC_LIST_HEAD (temp_diff.tuples); tuple != NULL; tuple = ISC_LIST_HEAD (temp_diff.tuples))
    {
...
static isc_boolean_t isdnssec (dns_db_t * db, dns_dbversion_t * ver, dns_rdatatype_t privatetype)
{
    ...
    result = dns_private_chains (db, ver, privatetype, &build_nsec, &build_nsec3);
    RUNTIME_CHECK (result == ISC_R_SUCCESS);
...
static void update_action (isc_task_t * task, isc_event_t * event)
{
     ...
     */
    CHECK (checkqueryacl (client, dns_zone_getqueryacl (zone), zonename, dns_zone_getupdateacl (zone), ssutable));
    ...
    for (result = dns_message_firstname (request, DNS_SECTION_PREREQUISITE);
         result == ISC_R_SUCCESS; result = dns_message_nextname (request, DNS_SECTION_PREREQUISITE))
    {
        ...
        get_current_rr (request, DNS_SECTION_PREREQUISITE, zoneclass, &name, &rdata, &covers, &ttl, &update_class);

        if (ttl != 0)
            PREREQFAILC (DNS_R_FORMERR, "prerequisite TTL is not zero");

        if (!dns_name_issubdomain (name, zonename))
            PREREQFAILN (DNS_R_NOTZONE, name, "prerequisite name is out of zone");

        if (update_class == dns_rdataclass_any)
        {
            if (rdata.length != 0)
                PREREQFAILC (DNS_R_FORMERR, "class ANY prerequisite " "RDATA is not empty");
            if (rdata.type == dns_rdatatype_any)
            {
                CHECK (name_exists (db, ver, name, &flag));
                if (!flag)
                {
                    PREREQFAILN (DNS_R_NXDOMAIN, name, "'name in use' " "prerequisite not " "satisfied");
                }
            }
            else
            {
                CHECK (rrset_exists (db, ver, name, rdata.type, covers, &flag));
                if (!flag)
                {
                    /* RRset does not exist. */
                    PREREQFAILNT (DNS_R_NXRRSET, name, rdata.type,
                                  "'rrset exists (value independent)' " "prerequisite not satisfied");
                }
            }
        }
        ...
        {
            if (rdata.length != 0)
                PREREQFAILC (DNS_R_FORMERR, "class NONE prerequisite " "RDATA is not empty");
            if (rdata.type == dns_rdatatype_any)
            {
                CHECK (name_exists (db, ver, name, &flag));
                if (flag)
                {
                    PREREQFAILN (DNS_R_YXDOMAIN, name, "'name not in use' " "prerequisite not " "satisfied");
                }
            }
            else
            {
                ...
                if (flag)
                {
                    ...
                    PREREQFAILNT (DNS_R_YXRRSET, name,
                                  rdata.type, "'rrset does not exist' " "prerequisite not " "satisfied");
                }
            }
        }
        else if (update_class == zoneclass)
        {
            ...
            if (result != ISC_R_SUCCESS)
            {
                UNEXPECTED_ERROR (__FILE__, __LINE__, "temp entry creation failed: %s", dns_result_totext (result));
    ...
    if (ISC_LIST_HEAD (temp.tuples) != NULL)
    {
        ...
        if (result != ISC_R_SUCCESS)
            FAILC (result, "'RRset exists (value dependent)' " "prerequisite not satisfied");

        dns_fixedname_init (&tmpnamefixed);
        tmpname = dns_fixedname_name (&tmpnamefixed);
        result = temp_check (mctx, &temp, db, ver, tmpname, &type);
        if (result != ISC_R_SUCCESS)
            FAILNT (result, tmpname, type, "'RRset exists (value dependent)' " "prerequisite not satisfied");
    }
     ...
     */
    if (ssutable == NULL)
        CHECK (checkupdateacl (client, dns_zone_getupdateacl (zone), "update", zonename, ISC_FALSE, ISC_FALSE));
    else if (client->signer == NULL && !TCPCLIENT (client))
        CHECK (checkupdateacl (client, NULL, "update", zonename, ISC_FALSE, ISC_TRUE));

    if (dns_zone_getupdatedisabled (zone))
        FAILC (DNS_R_REFUSED, "dynamic update temporarily disabled "
               "because the zone is frozen.  Use " "'rndc thaw' to re-enable updates.");
    ...
    for (result = dns_message_firstname (request, DNS_SECTION_UPDATE);
         result == ISC_R_SUCCESS; result = dns_message_nextname (request, DNS_SECTION_UPDATE))
    {
        dns_name_t *name = NULL;
        ...
        get_current_rr (request, DNS_SECTION_UPDATE, zoneclass, &name, &rdata, &covers, &ttl, &update_class);
        ...
        {
            if (ttl != 0 || rdata.length != 0 || (dns_rdatatype_ismeta (rdata.type) && rdata.type != dns_rdatatype_any))
                FAILC (DNS_R_FORMERR, "meta-RR in update");
        }
        else if (update_class == dns_rdataclass_none)
        {
        ...
        {
            update_log (client, zone, ISC_LOG_WARNING, "update RR has incorrect class %d", update_class);
            FAIL (DNS_R_FORMERR);
        }
        ...
        if (rdata.type == dns_rdatatype_nsec3)
        {
            FAILC (DNS_R_REFUSED, "explicit NSEC3 updates are not allowed " "in secure zones");
        }
        else if (rdata.type == dns_rdatatype_nsec)
        {
            FAILC (DNS_R_REFUSED, "explicit NSEC updates are not allowed " "in secure zones");
        }
        else if (rdata.type == dns_rdatatype_rrsig && !dns_name_equal (name, zonename))
        {
            FAILC (DNS_R_REFUSED,
                   "explicit RRSIG updates are currently " "not supported in secure zones except " "at the apex");
        }
        ...
        if (ssutable != NULL)
        {
            ...
            if (rdata.type != dns_rdatatype_any)
            {
                if (!dns_ssutable_checkrules (ssutable, client->signer, name, tcpaddr, rdata.type, tsigkey))
                    FAILC (DNS_R_REFUSED, "rejected by secure update");
            }
            else
            {
                if (!ssu_checkall (db, ver, name, ssutable, client->signer, tcpaddr, tsigkey))
                    FAILC (DNS_R_REFUSED, "rejected by secure update");
            }
        }
    }
         ...
         result == ISC_R_SUCCESS; result = dns_message_nextname (request, DNS_SECTION_UPDATE))
    {
        ...
        if (update_class == zoneclass)
        {
            ...
            if (rdata.type == dns_rdatatype_md || rdata.type == dns_rdatatype_mf)
            {
                char typebuf[DNS_RDATATYPE_FORMATSIZE];

                dns_rdatatype_format (rdata.type, typebuf, sizeof (typebuf));
                update_log (client, zone, LOGLEVEL_PROTOCOL, "attempt to add %s ignored", typebuf);
                continue;
            }
            if ((rdata.type == dns_rdatatype_ns || rdata.type == dns_rdatatype_dname) && dns_name_iswildcard (name))
            {
                char typebuf[DNS_RDATATYPE_FORMATSIZE];

                dns_rdatatype_format (rdata.type, typebuf, sizeof (typebuf));
                update_log (client, zone, LOGLEVEL_PROTOCOL, "attempt to add wildcard %s record " "ignored", typebuf);
                continue;
            }
            if (rdata.type == dns_rdatatype_cname)
            {
                CHECK (cname_incompatible_rrset_exists (db, ver, name, &flag));
                if (flag)
                {
                    update_log (client, zone,
                                LOGLEVEL_PROTOCOL, "attempt to add CNAME " "alongside non-CNAME " "ignored");
                    continue;
                }
            }
            else
            {
                CHECK (rrset_exists (db, ver, name, dns_rdatatype_cname, 0, &flag));
                if (flag && !dns_rdatatype_isdnssec (rdata.type))
                {
                    update_log (client, zone, LOGLEVEL_PROTOCOL, "attempt to add non-CNAME " "alongside CNAME ignored");
                    continue;
                }
            }
            if (rdata.type == dns_rdatatype_soa)
            {
                ...
                CHECK (rrset_exists (db, ver, name, dns_rdatatype_soa, 0, &flag));
                if (!flag)
                {
                    update_log (client, zone, LOGLEVEL_PROTOCOL, "attempt to create 2nd " "SOA ignored");
                    continue;
                }
                CHECK (check_soa_increment (db, ver, &rdata, &ok));
                if (!ok)
                {
                    update_log (client, zone,
                                LOGLEVEL_PROTOCOL, "SOA update failed to " "increment serial, " "ignoring it");
                    continue;
                }
            ...
            {
                update_log (client, zone, LOGLEVEL_PROTOCOL,
                            "attempt to add a private type " "(%u) record rejected internal " "use only", privatetype);
                continue;
            }
            ...
            if (rdata.type == dns_rdatatype_nsec3param)
            {
                ...
                {
                    update_log (client, zone,
                                LOGLEVEL_PROTOCOL, "attempt to add NSEC3PARAM " "record with non OPTOUT " "flag");
                    continue;
                }
            }

            if ((options & DNS_ZONEOPT_CHECKWILDCARD) != 0 && dns_name_internalwildcard (name))
            {
                char namestr[DNS_NAME_FORMATSIZE];

                dns_name_format (name, namestr, sizeof (namestr));
                update_log (client, zone, LOGLEVEL_PROTOCOL,
                            "warning: ownername '%s' contains " "a non-terminal wildcard", namestr);
            }
            ...
            if (isc_log_wouldlog (ns_g_lctx, LOGLEVEL_PROTOCOL))
            {
                ...
                dns_name_format (name, namestr, sizeof (namestr));
                dns_rdatatype_format (rdata.type, typestr, sizeof (typestr));
                update_log (client, zone, LOGLEVEL_PROTOCOL, "adding an RR at '%s' %s", namestr, typestr);
            }
            ...
            /* Prepare the affected RRset for the addition. */
            {
                ...
                CHECK (foreach_rr (db, ver, name, rdata.type, covers, add_rr_prepare_action, &ctx));
        ...
        else if (update_class == dns_rdataclass_any)
        {
            if (rdata.type == dns_rdatatype_any)
            {
                if (isc_log_wouldlog (ns_g_lctx, LOGLEVEL_PROTOCOL))
                {
                    ...
                    dns_name_format (name, namestr, sizeof (namestr));
                    update_log (client, zone, LOGLEVEL_PROTOCOL, "delete all rrsets from " "name '%s'", namestr);
                }
                if (dns_name_equal (name, zonename))
                {
                    CHECK (delete_if (type_not_soa_nor_ns_p, db, ver, name, dns_rdatatype_any, 0, &rdata, &diff));
                }
                else
                {
                    CHECK (delete_if (type_not_dnssec, db, ver, name, dns_rdatatype_any, 0, &rdata, &diff));
                }
            }
            else if (dns_name_equal (name, zonename) &&
                     (rdata.type == dns_rdatatype_soa || rdata.type == dns_rdatatype_ns))
            {
                update_log (client, zone, LOGLEVEL_PROTOCOL, "attempt to delete all SOA " "or NS records ignored");
                continue;
            }
            else
            {
                if (isc_log_wouldlog (ns_g_lctx, LOGLEVEL_PROTOCOL))
                {
                    ...
                    dns_name_format (name, namestr, sizeof (namestr));
                    dns_rdatatype_format (rdata.type, typestr, sizeof (typestr));
                    update_log (client, zone, LOGLEVEL_PROTOCOL, "deleting rrset at '%s' %s", namestr, typestr);
                }
                CHECK (delete_if (true_p, db, ver, name, rdata.type, covers, &rdata, &diff));
            }
        }
        else if (update_class == dns_rdataclass_none)
        {
            ...
            if (dns_name_equal (name, zonename))
            {
                if (rdata.type == dns_rdatatype_soa)
                {
                    update_log (client, zone, LOGLEVEL_PROTOCOL, "attempt to delete SOA " "ignored");
                    continue;
                }
                if (rdata.type == dns_rdatatype_ns)
                {
                    ...
                    CHECK (rr_count (db, ver, name, dns_rdatatype_ns, 0, &count));
                    if (count == 1)
                    {
                        update_log (client, zone, LOGLEVEL_PROTOCOL, "attempt to " "delete last " "NS ignored");
                        continue;
                    }
                }
            }
            dns_name_format (name, namestr, sizeof (namestr));
            dns_rdatatype_format (rdata.type, typestr, sizeof (typestr));
            update_log (client, zone, LOGLEVEL_PROTOCOL, "deleting an RR at %s %s", namestr, typestr);
            CHECK (delete_if (rr_equal_p, db, ver, name, rdata.type, covers, &rdata, &diff));
        }
    }
    if (result != ISC_R_NOMORE)
        FAIL (result);
    ...
    if (!ISC_LIST_EMPTY (diff.tuples))
    {
        ...
        CHECK (rrset_exists (db, ver, zonename, dns_rdatatype_dnskey, 0, &has_dnskey));
...
#define ALLOW_SECURE_TO_INSECURE(zone) \
    ((dns_zone_getoptions(zone) & DNS_ZONEOPT_SECURETOINSECURE) != 0)
        ...
        {
            CHECK (rrset_exists (db, oldver, zonename, dns_rdatatype_dnskey, 0, &had_dnskey));
            if (had_dnskey && !has_dnskey)
            {
                update_log (client, zone, LOGLEVEL_PROTOCOL,
                            "update rejected: all DNSKEY "
                            "records removed and " "'dnssec-secure-to-insecure' " "not set");
                result = DNS_R_REFUSED;
                goto failure;
            }
        }
        ...
        if (!has_dnskey)
        {
             ...
             */
            CHECK (dns_nsec3param_deletechains (db, ver, zone, &diff));
        }
        else if (has_dnskey && isdnssec (db, ver, privatetype))
        {
            ...
            result = update_signatures (client, zone, db, oldver, ver, &diff, interval);
            if (result != ISC_R_SUCCESS)
            {
                update_log (client, zone,
                            ISC_LOG_ERROR, "RRSIG/NSEC/NSEC3 update failed: %s", isc_result_totext (result));
                goto failure;
            }
        }
        ...
        if (journalfile != NULL)
        {
            update_log (client, zone, LOGLEVEL_DEBUG, "writing journal %s", journalfile);

            journal = NULL;
            result = dns_journal_open (mctx, journalfile, ISC_TRUE, &journal);
            if (result != ISC_R_SUCCESS)
         ...
         */
        update_log (client, zone, LOGLEVEL_DEBUG, "committing update transaction");
        ...
        for (tuple = ISC_LIST_HEAD (diff.tuples); tuple != NULL; tuple = ISC_LIST_NEXT (tuple, link))
        {
            ...
            result = dns_zone_signwithkey (zone, algorithm, keyid, ISC_TF (tuple->op == DNS_DIFFOP_DEL));
            if (result != ISC_R_SUCCESS)
            {
                update_log (client, zone, ISC_LOG_ERROR, "dns_zone_signwithkey failed: %s", dns_result_totext (result));
            }
        }
        ...
        for (tuple = ISC_LIST_HEAD (diff.tuples); tuple != NULL; tuple = ISC_LIST_NEXT (tuple, link))
        {
            ...
            if (tuple->rdata.type != privatetype || tuple->op != DNS_DIFFOP_ADD)
                continue;
            ...
            {
                update_log (client, zone, ISC_LOG_ERROR,
                            "dns_zone_addnsec3chain failed: %s", dns_result_totext (result));
            }
        }
    }
    else
...
static void forward_action (isc_task_t * task, isc_event_t * event)
{
    ...
    result = dns_zone_forwardupdate (zone, client->message, forward_callback, event);
    if (result != ISC_R_SUCCESS)
    {
        uev->ev_type = DNS_EVENT_UPDATEDONE;
        uev->ev_action = forward_fail;
...
static isc_result_t send_forward_event (ns_client_t * client, dns_zone_t * zone)
{
    ...
    event = (update_event_t *)
        isc_event_allocate (client->mctx, client, DNS_EVENT_UPDATE, forward_action, NULL, sizeof (*event));
    if (event == NULL)
        FAIL (ISC_R_NOMEMORY);
    event->zone = zone;
    event->result = ISC_R_SUCCESS;
