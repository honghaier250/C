...
static void byaddr_done (isc_task_t * task, isc_event_t * event)
{
    ...
    ns_lwdclient_log (50, "byaddr event result = %s", isc_result_totext (bevent->result));
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        if (client->na.family != AF_INET6 || (client->options & DNS_BYADDROPT_IPV6INT) != 0)
        {
            if (result == DNS_R_NCACHENXDOMAIN ||
                result == DNS_R_NCACHENXRRSET || result == DNS_R_NXDOMAIN || result == DNS_R_NXRRSET)
    ...
    for (name = ISC_LIST_HEAD (bevent->names); name != NULL; name = ISC_LIST_NEXT (name, link))
    {
        ...
        ns_lwdclient_log (50, "found name '%.*s'",
                          (int) (client->recv_buffer.used - b.used), (char *) (b.base) + b.used);
        ...
        else
        {
            ...
            gnba->aliaslen[naliases] = client->recv_buffer.used - b.used;
            gnba->naliases++;
        }
    }
    ...
    lwres = lwres_gnbaresponse_render (cm->lwctx, gnba, &client->pkt, &lwb);
...
static void start_byaddr (ns_lwdclient_t * client)
{
    isc_result_t result;
    ...
    result = dns_byaddr_create (cm->mctx, &client->na, cm->view,
                                client->options, cm->task, byaddr_done, client, &client->byaddr);
    if (result != ISC_R_SUCCESS)
    {
...
void ns_lwdclient_processgnba (ns_lwdclient_t * client, lwres_buffer_t * b)
{
    ...
    result = lwres_gnbarequest_parse (cm->lwctx, b, &client->pkt, &req);
    ...
    ns_lwdclient_log (50, "client %p looking for addrtype %08x", client, req->addr.family);
