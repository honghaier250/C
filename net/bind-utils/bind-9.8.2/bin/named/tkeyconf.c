...
isc_result_t
ns_tkeyctx_fromconfig (const cfg_obj_t * options, isc_mem_t * mctx, isc_entropy_t * ectx, dns_tkeyctx_t ** tctxp)
{
    isc_result_t result;
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        RETERR (dst_key_fromfile (name, (dns_keytag_t) n, DNS_KEYALG_DH, type, NULL, mctx, &tctx->dhkey));
    }
