...
static void linux_setcaps (cap_t caps)
{
#ifndef HAVE_LIBCAP
    ...
    if (syscall (SYS_capset, &caphead, &cap) < 0)
    {
        ...
        ns_main_earlyfatal (SETCAPS_FUNC "failed: %s:"
                            " please ensure that the capset kernel" " module is loaded.  see insmod(8)", strbuf);
    }
}
...
static void linux_initialprivs (void)
{
    cap_t caps;
...
#ifdef HAVE_SYS_PRCTL_H
static void linux_keepcaps (void)
{
    char strbuf[ISC_STRERRORSIZE];
...
void ns_os_started (void)
{
    ...
    if (dfd[0] != -1 && dfd[1] != -1)
    {
        if (write (dfd[1], &buf, 1) != 1)
            ns_main_earlyfatal ("unable to signal parent that we " "otherwise started successfully.");
        close (dfd[1]);
        dfd[0] = dfd[1] = -1;
    }
}
...
{
    if (devnullfd != STDIN_FILENO && devnullfd != STDOUT_FILENO && devnullfd != STDERR_FILENO)
    {
        close (devnullfd);
        devnullfd = -1;
    }
...
FILE *ns_os_openfile (const char *filename, mode_t mode, isc_boolean_t switch_user)
{
    ...
    {
        isc__strerror (errno, strbuf, sizeof (strbuf));
        ns_main_earlywarning ("couldn't strdup() '%s': %s", filename, strbuf);
        return (NULL);
    }
    ...
    if (switch_user && runas_pw != NULL)
    {
        ...
        if (fd == -1)
        {
            ...
            if (fd != -1)
            {
                ns_main_earlywarning ("Required root " "permissions to open " "'%s'.", filename);
            }
            else
            {
                ns_main_earlywarning ("Could not open " "'%s'.", filename);
            }
            ns_main_earlywarning ("Please check file and " "directory permissions " "or reconfigure the filename.");
    ...
    {
        isc__strerror (errno, strbuf, sizeof (strbuf));
        ns_main_earlywarning ("could not open file '%s': %s", filename, strbuf);
        return (NULL);
    }
    ...
    {
        isc__strerror (errno, strbuf, sizeof (strbuf));
        ns_main_earlywarning ("could not fdopen() file '%s': %s", filename, strbuf);
    }
...
void ns_os_shutdownmsg (char *command, isc_buffer_t * text)
{
    ...
    n = snprintf ((char *) isc_buffer_used (text), isc_buffer_availablelength (text), "pid: %ld", (long) pid);
