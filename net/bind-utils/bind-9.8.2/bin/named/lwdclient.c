...
void ns_lwdclient_log (int level, const char *format, ...)
{
    ...
    isc_log_vwrite (dns_lctx, DNS_LOGCATEGORY_DATABASE, DNS_LOGMODULE_ADB, ISC_LOG_DEBUG (level), format, args);
    va_end (args);
}
...
isc_result_t ns_lwdclientmgr_create (ns_lwreslistener_t * listener, unsigned int nclients, isc_taskmgr_t * taskmgr)
{
    ...
    if (lwres_context_create (&cm->lwctx, cm->mctx,
                              ns__lwresd_memalloc, ns__lwresd_memfree, LWRES_CONTEXT_SERVERMODE) != ISC_R_SUCCESS)
        goto errout;
    ...
    for (i = 0; i < nclients; i++)
    {
        ...
        if (client != NULL)
        {
            ns_lwdclient_log (50, "created client %p, manager %p", client, cm);
     ...
     */
    result = isc_task_onshutdown (cm->task, lwdclientmgr_shutdown_callback, cm);
    if (result != ISC_R_SUCCESS)
        goto errout;
...
static void lwdclientmgr_destroy (ns_lwdclientmgr_t * cm)
{
    ...
    while (client != NULL)
    {
        ns_lwdclient_log (50, "destroying client %p, manager %p", client, cm);
        ISC_LIST_UNLINK (cm->idle, client, link);
...
isc_result_t ns_lwdclient_startrecv (ns_lwdclientmgr_t * cm)
{
    ...
    result = isc_socket_recv (cm->sock, &r, 0, cm->task, ns_lwdclient_recv, client);
    if (result != ISC_R_SUCCESS)
        return (result);
...
static void lwdclientmgr_shutdown_callback (isc_task_t * task, isc_event_t * ev)
{
    ...
    ns_lwdclient_log (50, "got shutdown event, task %p, lwdclientmgr %p", task, cm);
...
void ns_lwdclient_send (isc_task_t * task, isc_event_t * ev)
{
    ...
    ns_lwdclient_log (50, "task %p for client %p got send-done event", task, client);

    if (client->sendbuf != client->buffer)
        lwres_context_freemem (cm->lwctx, client->sendbuf, client->sendlength);
    client->sendbuf = NULL;
    client->sendlength = 0;
...
isc_result_t ns_lwdclient_sendreply (ns_lwdclient_t * client, isc_region_t * r)
{
    ...
    if (client->pktinfo_valid)
        ...
        pktinfo = NULL;
    return (isc_socket_sendto (cm->sock, r, cm->task, ns_lwdclient_send, client, &client->address, pktinfo));
}
