...
static void log_rr (dns_name_t * name, dns_rdata_t * rdata, isc_uint32_t ttl)
{
    ...
    rdl.ttl = ttl;
    if (rdata->type == dns_rdatatype_sig || rdata->type == dns_rdatatype_rrsig)
    ...
    result = dns_rdataset_totext (&rds, name, ISC_FALSE, ISC_FALSE, &buf);
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        isc_log_write (XFROUT_RR_LOGARGS, "%.*s", (int) isc_buffer_usedlength (&buf), (char *) isc_buffer_base (&buf));
    }
    else
    {
...
ixfr_rrstream_create (isc_mem_t * mctx,
                      ...
                      isc_uint32_t begin_serial, isc_uint32_t end_serial, rrstream_t ** sp)
{
    ixfr_rrstream_t *s;
    ...
    CHECK (dns_journal_open (mctx, journal_filename, ISC_FALSE, &s->journal));
...
static void ixfr_rrstream_destroy (rrstream_t ** rsp)
{
...
static isc_result_t axfr_rrstream_first (rrstream_t * rs)
{
    ...
    for (;;)
    {
        ...
        dns_rriterator_current (&s->it, &name_dummy, &ttl_dummy, NULL, &rdata);
...
static isc_result_t soa_rrstream_create (isc_mem_t * mctx, dns_db_t * db, dns_dbversion_t * ver, rrstream_t ** sp)
{
    ...
    CHECK (dns_db_createsoatuple (db, ver, mctx, DNS_DIFFOP_EXISTS, &s->soa_tuple));
 ...
 */
static isc_result_t
compound_rrstream_create (isc_mem_t * mctx, rrstream_t ** soa_stream, rrstream_t ** data_stream, rrstream_t ** sp)
{
...
xfrout_ctx_create (isc_mem_t * mctx, ns_client_t * client,
                   ...
                   unsigned int maxtime, unsigned int idletime, isc_boolean_t many_answers, xfrout_ctx_t ** xfrp);
...
xfrout_log1 (ns_client_t * client, dns_name_t * zonename,
             dns_rdataclass_t rdclass, int level, const char *fmt, ...) ISC_FORMAT_PRINTF (5, 6);

     static void xfrout_log (xfrout_ctx_t * xfr, int level, const char *fmt, ...) ISC_FORMAT_PRINTF (3, 4);

/**************************************************************************/

     void ns_xfr_start (ns_client_t * client, dns_rdatatype_t reqtype)
{
    isc_result_t result;
    ...
    ns_client_log (client, DNS_LOGCATEGORY_XFER_OUT, NS_LOGMODULE_XFER_OUT, ISC_LOG_DEBUG (6), "%s request", mnemonic);
    /*
     * Apply quota.
     */
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (XFROUT_COMMON_LOGARGS, ISC_LOG_WARNING,
                       "%s request denied: %s", mnemonic, isc_result_totext (result));
        goto failure;
    }
    ...
    result = dns_zt_find (client->view->zonetable, question_name, 0, NULL, &zone);
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        if (client->view->dlzdatabase != NULL)
        {
            ...
            if (result == ISC_R_NOPERM)
            {
                ...
                dns_name_format (question_name, _buf1, sizeof (_buf1));
                dns_rdataclass_format (question_class, _buf2, sizeof (_buf2));
                ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                               NS_LOGMODULE_XFER_OUT, ISC_LOG_ERROR, "zone transfer '%s/%s' denied", _buf1, _buf2);
                goto failure;
            }
            if (result != ISC_R_SUCCESS)
    ...
    xfrout_log1 (client, question_name, question_class, ISC_LOG_DEBUG (6), "%s question section OK", mnemonic);
    ...
    for (result = dns_message_firstname (request, DNS_SECTION_AUTHORITY);
         result == ISC_R_SUCCESS; result = dns_message_nextname (request, DNS_SECTION_AUTHORITY))
    {
        soa_name = NULL;
        ...
        for (soa_rdataset = ISC_LIST_HEAD (soa_name->list);
             soa_rdataset != NULL; soa_rdataset = ISC_LIST_NEXT (soa_rdataset, link))
        {
            /*
             * Ignore non-SOA data.
             */
            ...
            if (result == ISC_R_SUCCESS)
                FAILC (DNS_R_FORMERR, "IXFR authority section " "has multiple SOAs");
            have_soa = ISC_TRUE;
            goto got_soa;
        }
    }
  got_soa:
    ...
    xfrout_log1 (client, question_name, question_class, ISC_LOG_DEBUG (6), "%s authority section OK", mnemonic);
    ...
    if (!is_dlz)
    {
        ns_client_aclmsg ("zone transfer", question_name, reqtype, client->view->rdclass, msg, sizeof (msg));
        CHECK (ns_client_checkacl (client, NULL, msg, dns_zone_getxfracl (zone), ISC_TRUE, ISC_LOG_ERROR));
    }

    /*
     * AXFR over UDP is not possible.
     */
    if (reqtype == dns_rdatatype_axfr && (client->attributes & NS_CLIENTATTR_TCP) == 0)
    ...
    CHECK (dns_db_createsoatuple (db, ver, mctx, DNS_DIFFOP_EXISTS, &current_soa_tuple));
    ...
    if (reqtype == dns_rdatatype_ixfr)
    {
         ...
         */
        if (DNS_SERIAL_GE (begin_serial, current_serial) || (client->attributes & NS_CLIENTATTR_TCP) == 0)
        {
        ...
        if (journalfile != NULL)
            result = ixfr_rrstream_create (mctx, journalfile, begin_serial, current_serial, &data_stream);
        else
            result = ISC_R_NOTFOUND;
        if (result == ISC_R_NOTFOUND || result == ISC_R_RANGE)
        {
            xfrout_log1 (client, question_name, question_class,
                         ISC_LOG_DEBUG (4), "IXFR version not in journal, " "falling back to AXFR");
            mnemonic = "AXFR-style IXFR";
            goto axfr_fallback;
        }
        CHECK (result);
    ...
    CHECK (compound_rrstream_create (mctx, &soa_stream, &data_stream, &stream));
    soa_stream = NULL;
    data_stream = NULL;
  ...
  have_stream:
    ...
    if (is_dlz)
        CHECK (xfrout_ctx_create (mctx, client, request->id,
                                  ...
                                  tsigbuf, 3600, 3600, (format == dns_many_answers) ? ISC_TRUE : ISC_FALSE, &xfr));
    else
        CHECK (xfrout_ctx_create (mctx, client, request->id,
                                  question_name, reqtype, question_class,
                                  zone, db, ver, quota, stream,
                                  ...
                                  (format == dns_many_answers) ? ISC_TRUE : ISC_FALSE, &xfr));
    ...
    if (is_poll)
        ...
        xfrout_log1 (client, question_name, question_class,
                     ISC_LOG_INFO, "%s started%s%s", mnemonic, (xfr->tsigkey != NULL) ? ": TSIG " : "", keyname);
  ...
  failure:
    ...
    {
        ns_client_log (client, DNS_LOGCATEGORY_XFER_OUT,
                       NS_LOGMODULE_XFER_OUT, ISC_LOG_DEBUG (3), "zone transfer setup failed");
...
xfrout_ctx_create (isc_mem_t * mctx, ns_client_t * client, unsigned int id,
                   ...
                   unsigned int idletime, isc_boolean_t many_answers, xfrout_ctx_t ** xfrp)
{
    xfrout_ctx_t *xfr;
    ...
    xfr->txmemlen = 0;
    xfr->nmsg = 0;
    xfr->many_answers = many_answers, xfr->sends = 0;
    ...
    CHECK (dns_timer_setidle (xfr->client->timer, maxtime, idletime, ISC_FALSE));
...
static void sendstream (xfrout_ctx_t * xfr)
{
    ...
    else
    {
        ...
        CHECK (dns_message_create (xfr->mctx, DNS_MESSAGE_INTENTRENDER, &tcpmsg));
        msg = tcpmsg;
        ...
        if (xfr->nmsg == 0)
        {
            ...
            dns_rdataset_makequestion (qrdataset, xfr->client->message->rdclass, xfr->qtype);
    ...
    for (n_rrs = 0;; n_rrs++)
    {
        ...
        if (size >= r.length)
        {
            ...
            if (n_rrs == 0)
            {
                xfrout_log (xfr, ISC_LOG_WARNING, "RR too large for zone transfer " "(%d bytes)", size);
    ...
    if ((xfr->client->attributes & NS_CLIENTATTR_TCP) != 0)
    {
        ...
        isc_buffer_putuint16 (&xfr->txlenbuf, (isc_uint16_t) used.length);
        region.base = xfr->txlenbuf.base;
        region.length = 2 + used.length;
        xfrout_log (xfr, ISC_LOG_DEBUG (8), "sending TCP message of %d bytes", used.length);
        CHECK (isc_socket_send (xfr->client->tcpsocket,    /* XXX */
                                &region, xfr->client->task, xfrout_senddone, xfr));
        xfr->sends++;
    }
    else
    {
...
static void xfrout_fail (xfrout_ctx_t * xfr, isc_result_t result, const char *msg)
{
    ...
    xfrout_log (xfr, ISC_LOG_ERROR, "%s: %s", msg, isc_result_totext (result));
    xfrout_maybe_destroy (xfr);
}
...
static void xfrout_maybe_destroy (xfrout_ctx_t * xfr)
{
    ...
    if (xfr->sends > 0)
    {
         ...
         */
        isc_socket_cancel (xfr->client->tcpsocket, xfr->client->task, ISC_SOCKCANCEL_SEND);
    }
    else
    {
...
xfrout_logv (ns_client_t * client, dns_name_t * zonename,
             dns_rdataclass_t rdclass, int level, const char *fmt, va_list ap) ISC_FORMAT_PRINTF (5, 0);
                      ...
                      dns_rdataclass_t rdclass, int level, const char *fmt, va_list ap)
{
    ...
    ns_client_log (client, DNS_LOGCATEGORY_XFER_OUT,
                   NS_LOGMODULE_XFER_OUT, level, "transfer of '%s/%s': %s", namebuf, classbuf, msgbuf);
}
 ...
 */
static void
xfrout_log1 (ns_client_t * client, dns_name_t * zonename, dns_rdataclass_t rdclass, int level, const char *fmt, ...)
{
    va_list ap;
