...
ISC_PLATFORM_NORETURN_PRE static void fatal (const char *msg, isc_result_t result) ISC_PLATFORM_NORETURN_POST;

     static void ns_server_reload (isc_task_t * task, isc_event_t * event);

     static isc_result_t
         ns_listenelt_fromconfig (const cfg_obj_t * listener, const cfg_obj_t * config,
                                  cfg_aclconfctx_t * actx, isc_mem_t * mctx, ns_listenelt_t ** target);
     static isc_result_t
         ns_listenlist_fromconfig (const cfg_obj_t * listenlist, const cfg_obj_t * config,
                                   cfg_aclconfctx_t * actx, isc_mem_t * mctx, ns_listenlist_t ** target);
     ...
     static isc_result_t
         configure_alternates (const cfg_obj_t * config, dns_view_t * view, const cfg_obj_t * alternates);
                             ...
                             cfg_aclconfctx_t * actx, isc_mem_t * mctx, dns_acl_t ** aclp)
{
    ...
    result = cfg_acl_fromconfig (aclobj, config, ns_g_lctx, actx, mctx, 0, aclp);
                         ...
                         cfg_aclconfctx_t * actx, isc_mem_t * mctx, dns_acl_t ** aclp)
{
     ...
     */
    result = cfg_acl_fromconfig (aclobj, config, ns_g_lctx, actx, mctx, 3, aclp);
...
configure_view_nametable (const cfg_obj_t * vconfig, const cfg_obj_t * config,
                          const char *confname, const char *conftuplename, isc_mem_t * mctx, dns_rbt_t ** rbtp)
{
    isc_result_t result;
    ...
    for (element = cfg_list_first (obj); element != NULL; element = cfg_list_next (element))
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            cfg_obj_log (nameobj, ns_g_lctx, ISC_LOG_ERROR,
                         "failed to add %s for %s: %s", str, confname, isc_result_totext (result));
            goto cleanup;
        }
                   ...
                   isc_boolean_t managed, dst_key_t ** target, isc_mem_t * mctx)
{
    ...
    if (managed)
    {
        ...
        if (strcasecmp (initmethod, "initial-key") != 0)
        {
            cfg_obj_log (key, ns_g_lctx, ISC_LOG_ERROR,
                         "managed key '%s': " "invalid initialization method '%s'", keynamestr, initmethod);
    ...
    else
    {
        ...
        CHECK (ns_config_getclass (classobj, dns_rdataclass_in, &viewclass));
    }
    ...
    if ((keystruct.algorithm == DST_ALG_RSASHA1 ||
         keystruct.algorithm == DST_ALG_RSAMD5) && r.length > 1 && r.base[0] == 1 && r.base[1] == 3)
        cfg_obj_log (key, ns_g_lctx, ISC_LOG_WARNING,
                     "%s key '%s' has a weak exponent", managed ? "managed" : "trusted", keynamestr);

    CHECK (dns_rdata_fromstruct (NULL, keystruct.common.rdclass, keystruct.common.rdtype, &keystruct, &rrdatabuf));
    ...
    CHECK (dst_key_fromdns (keyname, viewclass, &rrdatabuf, mctx, &dstkey));
  ...
  cleanup:
    ...
    {
        cfg_obj_log (key, ns_g_lctx, ISC_LOG_ERROR,
                     "ignoring %s key for '%s': no crypto support", managed ? "managed" : "trusted", keynamestr);
    }
    else if (result == DST_R_UNSUPPORTEDALG)
    {
        cfg_obj_log (key, ns_g_lctx, ISC_LOG_WARNING,
                     "skipping %s key for '%s': %s",
                     managed ? "managed" : "trusted", keynamestr, isc_result_totext (result));
    }
    else
    {
        cfg_obj_log (key, ns_g_lctx, ISC_LOG_ERROR,
...
load_view_keys (const cfg_obj_t * keys, const cfg_obj_t * vconfig,
                dns_view_t * view, isc_boolean_t managed, dns_name_t * keyname, isc_mem_t * mctx)
{
    ...
    for (elt = cfg_list_first (keys); elt != NULL; elt = cfg_list_next (elt))
    {
        ...
        for (elt2 = cfg_list_first (keylist); elt2 != NULL; elt2 = cfg_list_next (elt2))
        {
            key = cfg_listelt_value (elt2);
            result = dstkey_fromconfig (vconfig, key, managed, &dstkey, mctx);
             ...
             */
            if (keyname != NULL && !dns_name_equal (keyname, dst_key_name (dstkey)))
            {
                dst_key_free (&dstkey);
                continue;
            }
...
configure_view_dnsseckeys (dns_view_t * view, const cfg_obj_t * vconfig,
                           ...
                           isc_boolean_t auto_dlv, isc_boolean_t auto_root, isc_mem_t * mctx)
{
    ...
    if (strcmp (view->name, "_bind") == 0 && view->rdclass == dns_rdataclass_chaos)
    {
        return (ISC_R_SUCCESS);
    }
    ...
    if (vconfig != NULL)
    {
        ...
        if (voptions != NULL)
        {
            ...
            (void) cfg_map_get (voptions, "managed-keys", &view_managed_keys);
            maps[i++] = voptions;
        }
    }
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_ERROR, "couldn't create keytable");
    ...
    if (auto_dlv && view->rdclass == dns_rdataclass_in)
    {
        ...
        isc_log_write (ns_g_lctx, DNS_LOGCATEGORY_SECURITY,
                       NS_LOGMODULE_SERVER, ISC_LOG_WARNING, "using built-in DLV key for view %s", view->name);
        ...
        if (bindkeys != NULL)
        {
            (void) cfg_map_get (bindkeys, "trusted-keys", &builtin_keys);
            (void) cfg_map_get (bindkeys, "managed-keys", &builtin_managed_keys);
        }
        else
        {
            (void) cfg_map_get (ns_g_config, "trusted-keys", &builtin_keys);
            (void) cfg_map_get (ns_g_config, "managed-keys", &builtin_managed_keys);
        }

        if (builtin_keys != NULL)
            CHECK (load_view_keys (builtin_keys, vconfig, view, ISC_FALSE, view->dlv, mctx));
        if (builtin_managed_keys != NULL)
            CHECK (load_view_keys (builtin_managed_keys, vconfig, view, ISC_TRUE, view->dlv, mctx));
    }
    ...
    if (auto_root && view->rdclass == dns_rdataclass_in)
    {
        ...
        isc_log_write (ns_g_lctx, DNS_LOGCATEGORY_SECURITY,
                       NS_LOGMODULE_SERVER, ISC_LOG_WARNING, "using built-in root key for view %s", view->name);
        ...
        if (builtin_keys != NULL)
            CHECK (load_view_keys (builtin_keys, vconfig, view, ISC_FALSE, dns_rootname, mctx));
        if (builtin_managed_keys != NULL)
            CHECK (load_view_keys (builtin_managed_keys, vconfig, view, ISC_TRUE, dns_rootname, mctx));
    }

    CHECK (load_view_keys (view_keys, vconfig, view, ISC_FALSE, NULL, mctx));
    CHECK (load_view_keys (view_managed_keys, vconfig, view, ISC_TRUE, NULL, mctx));

    if (view->rdclass == dns_rdataclass_in)
    {
        CHECK (load_view_keys (global_keys, vconfig, view, ISC_FALSE, NULL, mctx));
        CHECK (load_view_keys (global_managed_keys, vconfig, view, ISC_TRUE, NULL, mctx));
    }
...
static isc_result_t mustbesecure (const cfg_obj_t * mbs, dns_resolver_t * resolver)
{
    ...
    dns_fixedname_init (&fixed);
    name = dns_fixedname_name (&fixed);
    for (element = cfg_list_first (mbs); element != NULL; element = cfg_list_next (element))
    {
 ...
 */
static isc_result_t
get_view_querysource_dispatch (const cfg_obj_t ** maps, int af, dns_dispatch_t ** dispatchp, isc_boolean_t is_firstview)
{
    ...
    result = dns_dispatch_getudp (ns_g_dispatchmgr, ns_g_socketmgr,
                                  ...
                                  maxdispatchbuffers, 32768, 16411, 16433, attrs, attrmask, &disp);
    if (result != ISC_R_SUCCESS)
    {
        isc_sockaddr_t any;
        ...
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_ERROR, "could not get query source dispatcher (%s)", buf);
        return (result);
    }
...
static isc_result_t configure_order (dns_order_t * order, const cfg_obj_t * ent)
{
    ...
    result = ns_config_getclass (cfg_tuple_get (ent, "class"), dns_rdataclass_any, &rdclass);
    if (result != ISC_R_SUCCESS)
        return (result);

    result = ns_config_gettype (cfg_tuple_get (ent, "type"), dns_rdatatype_any, &rdtype);
    if (result != ISC_R_SUCCESS)
        return (result);
    ...
    isc_buffer_add (&b, strlen (str));
    dns_fixedname_init (&fixed);
    result = dns_name_fromtext (dns_fixedname_name (&fixed), &b, dns_rootname, 0, NULL);
    if (result != ISC_R_SUCCESS)
        return (result);
    ...
    if (addroot)
    {
        result = dns_order_add (order, dns_rootname, rdtype, rdclass, mode);
        if (result != ISC_R_SUCCESS)
            return (result);
    }

    return (dns_order_add (order, dns_fixedname_name (&fixed), rdtype, rdclass, mode));
}
...
static isc_result_t configure_peer (const cfg_obj_t * cpeer, isc_mem_t * mctx, dns_peer_t ** peerp)
{
    ...
    if (obj != NULL)
    {
        ...
        if (strcasecmp (str, "many-answers") == 0)
            CHECK (dns_peer_settransferformat (peer, dns_many_answers));
    ...
    if (obj != NULL)
    {
        result = dns_peer_settransfersource (peer, cfg_obj_assockaddr (obj));
        if (result != ISC_R_SUCCESS)
            goto cleanup;
    ...
    if (obj != NULL)
    {
        result = dns_peer_setnotifysource (peer, cfg_obj_assockaddr (obj));
        if (result != ISC_R_SUCCESS)
            goto cleanup;
        ns_add_reserved_dispatch (ns_g_server, cfg_obj_assockaddr (obj));
    }
...
static isc_result_t disable_algorithms (const cfg_obj_t * disabled, dns_resolver_t * resolver)
{
    ...
    for (element = cfg_list_first (algorithms); element != NULL; element = cfg_list_next (element))
    {
        isc_textregion_t r;
        ...
        if (result != ISC_R_SUCCESS)
        {
            cfg_obj_log (cfg_listelt_value (element), ns_g_lctx, ISC_LOG_ERROR, "invalid algorithm");
            CHECK (result);
        }
...
static isc_boolean_t on_disable_list (const cfg_obj_t * disablelist, dns_name_t * zonename)
{
    ...
    for (element = cfg_list_first (disablelist); element != NULL; element = cfg_list_next (element))
    {
...
static void check_dbtype (dns_zone_t ** zonep, unsigned int dbtypec, const char **dbargv, isc_mem_t * mctx)
{
    char **argv = NULL;
...
static isc_result_t setquerystats (dns_zone_t * zone, isc_mem_t * mctx, isc_boolean_t on)
{
    isc_result_t result;
    ...
    if (on)
    {
        result = isc_stats_create (mctx, &zoneqrystats, dns_nsstatscounter_max);
        if (result != ISC_R_SUCCESS)
            return (result);
    }
...
static ns_cache_t *cachelist_find (ns_cachelist_t * cachelist, const char *cachename)
{
    ns_cache_t *nsc;

    for (nsc = ISC_LIST_HEAD (*cachelist); nsc != NULL; nsc = ISC_LIST_NEXT (nsc, link))
    {
...
static isc_boolean_t cache_reusable (dns_view_t * originview, dns_view_t * view, isc_boolean_t new_zero_no_soattl)
{
    if (originview->checknames != view->checknames ||
        ...
        originview->maxcachettl != view->maxcachettl || originview->maxncachettl != view->maxncachettl)
    {
        return (ISC_FALSE);
    }
...
cache_sharable (dns_view_t * originview, dns_view_t * view,
                isc_boolean_t new_zero_no_soattl, unsigned int new_cleaning_interval, isc_uint32_t new_max_cache_size)
{
    /*
    ...
    if (dns_cache_getcleaninginterval (originview->cache) !=
        new_cleaning_interval || dns_cache_getcachesize (originview->cache) != new_max_cache_size)
    {
        return (ISC_FALSE);
    }
 ...
 */
static isc_result_t dlzconfigure_callback (dns_view_t * view, dns_zone_t * zone)
{
    ...
    return ns_zone_configure_writeable_dlz (view->dlzdatabase, zone, zclass, origin);
}
               ...
               unsigned int prefixlen, const char *server, const char *contact)
{
    ...
    {
        prefixlen -= 8;
        sprintf (cp, "%x.%x.", s6[prefixlen / 8] & 0xf, (s6[prefixlen / 8] >> 4) & 0xf);
        cp += 4;
    }
    strcat (cp, "ip6.arpa.");
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL, NS_LOGMODULE_SERVER,
                   ISC_LOG_INFO, "dns64 reverse zone%s%s: %s", sep, viewname, reverse);
...
static isc_result_t configure_rpz (dns_view_t * view, const cfg_listelt_t * element)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        cfg_obj_log (rpz_obj, ns_g_lctx, DNS_RPZ_ERROR_LEVEL, "invalid zone '%s'", str);
        goto cleanup;
    }

    result = dns_name_fromstring2 (&new->nsdname, DNS_RPZ_NSDNAME_ZONE, origin, DNS_NAME_DOWNCASE, view->mctx);
    if (result != ISC_R_SUCCESS)
    {
        cfg_obj_log (rpz_obj, ns_g_lctx, DNS_RPZ_ERROR_LEVEL, "invalid zone '%s'", str);
        goto cleanup;
    ...
    if (result != ISC_R_SUCCESS)
    {
        cfg_obj_log (rpz_obj, ns_g_lctx, DNS_RPZ_ERROR_LEVEL, "unknown zone '%s'", str);
        goto cleanup;
    }

    if (dns_zone_gettype (zone) != dns_zone_master && dns_zone_gettype (zone) != dns_zone_slave)
    {
        cfg_obj_log (rpz_obj, ns_g_lctx, DNS_RPZ_ERROR_LEVEL, "zone '%s' is neither master nor slave", str);
        dns_zone_detach (&zone);
    ...
    for (old = ISC_LIST_HEAD (view->rpz_zones); old != new; old = ISC_LIST_NEXT (old, link))
    {
        ++new->num;
        if (dns_name_equal (&old->origin, &new->origin))
        {
            cfg_obj_log (rpz_obj, ns_g_lctx, DNS_RPZ_ERROR_LEVEL, "duplicate '%s'", str);
    ...
    if (new->policy == DNS_RPZ_POLICY_CNAME)
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            cfg_obj_log (rpz_obj, ns_g_lctx, DNS_RPZ_ERROR_LEVEL, "invalid cname '%s'", str);
            goto cleanup;
        }
    }
...
configure_view (dns_view_t * view, cfg_obj_t * config, cfg_obj_t * vconfig,
                ...
                isc_mem_t * mctx, cfg_aclconfctx_t * actx, isc_boolean_t need_hints)
{
    const cfg_obj_t *maps[4];
    ...
    if (obj != NULL && cfg_obj_asboolean (obj))
    {
        ...
        CHECK (dns_acache_create (&view->acache, cmctx, ns_g_taskmgr, ns_g_timermgr));
    ...
    if (view->acache != NULL)
    {
        ...
        INSIST (result == ISC_R_SUCCESS);
        dns_acache_setcleaninginterval (view->acache, cfg_obj_asuint32 (obj) * 60);
        ...
        else
        {
            ...
            if (value > ISC_UINT32_MAX)
            {
                cfg_obj_log (obj, ns_g_lctx, ISC_LOG_ERROR,
                             "'max-acache-size " "%" ISC_PRINT_QUADFORMAT "d' is too large", value);
                result = ISC_R_RANGE;
                goto cleanup;
            }
    ...
    CHECK (configure_view_acl (vconfig, config, "allow-query", NULL, actx, ns_g_mctx, &view->queryacl));
    if (view->queryacl == NULL)
    {
        CHECK (configure_view_acl (NULL, ns_g_config, "allow-query", NULL, actx, ns_g_mctx, &view->queryacl));
    }
     ...
     */
    for (element = cfg_list_first (zonelist); element != NULL; element = cfg_list_next (element))
    {
        const cfg_obj_t *zconfig = cfg_listelt_value (element);

        CHECK (configure_zone (config, zconfig, vconfig, mctx, view, actx, ISC_FALSE));
    }
    ...
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_INFO, "loading additional zones for view '%s'", view->name);
        ...
        for (element = cfg_list_first (zonelist); element != NULL; element = cfg_list_next (element))
        {
            ...
            CHECK (configure_zone (config, zconfig, vconfig, mctx, view, actx, ISC_TRUE));
        }
    }
    ...
    else
    {
        ...
        {
            cfg_obj_log (obj, ns_g_lctx, ISC_LOG_ERROR,
                         "'max-cache-size " "%" ISC_PRINT_QUADFORMAT "d' is too large", value);
            result = ISC_R_RANGE;
            goto cleanup;
        }
    ...
    {
        resopts |= DNS_RESOLVER_CHECKNAMES | DNS_RESOLVER_CHECKNAMESFAIL;
    ...
    if (result == ISC_R_SUCCESS && strcmp (view->name, "_bind") && strcmp (view->name, "_meta"))
    {
        const cfg_listelt_t *element;
        ...
        for (element = cfg_list_first (obj); element != NULL; element = cfg_list_next (element))
        {
            ...
            cfg_obj_asnetprefix (cfg_map_getname (map), &na, &prefixlen);
            ...
            {
                sp = &suffix;
                isc_netaddr_fromsockaddr (sp, cfg_obj_assockaddr (obj));
            }
            else
                sp = NULL;
            ...
            if (obj != NULL)
            {
                result = cfg_acl_fromconfig (obj, config, ns_g_lctx, actx, mctx, 0, &clients);
                if (result != ISC_R_SUCCESS)
                    goto cleanup;
            }
            obj = NULL;
            ...
            if (obj != NULL)
            {
                result = cfg_acl_fromconfig (obj, config, ns_g_lctx, actx, mctx, 0, &mapped);
                if (result != ISC_R_SUCCESS)
                    goto cleanup;
            }
            obj = NULL;
            ...
            if (obj != NULL)
            {
                result = cfg_acl_fromconfig (obj, config, ns_g_lctx, actx, mctx, 0, &excluded);
                if (result != ISC_R_SUCCESS)
                    goto cleanup;
            }
            ...
            result = dns_dns64_create (mctx, &na, prefixlen, sp, clients, mapped, excluded, dns64options, &dns64);
            if (result != ISC_R_SUCCESS)
                goto cleanup;
            dns_dns64_append (&view->dns64, dns64);
            view->dns64cnt++;
            result = dns64_reverse (view, mctx, &na, prefixlen, server, contact);
            if (result != ISC_R_SUCCESS)
                goto cleanup;
            if (clients != NULL)
    ...
    if (nsc != NULL)
    {
        if (!cache_sharable (nsc->primaryview, view, zero_no_soattl, cleaning_interval, max_cache_size))
        {
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_SERVER, ISC_LOG_ERROR,
                           "views %s and %s can't share the cache "
                           "due to configuration parameter mismatch", nsc->primaryview->name, view->name);
            result = ISC_R_FAILURE;
            goto cleanup;
        }
    ...
    else
    {
        ...
        {
            result = dns_viewlist_find (&ns_g_server->viewlist, cachename, view->rdclass, &pview);
            ...
            if (pview != NULL)
            {
                if (!cache_reusable (pview, view, zero_no_soattl))
                {
                    isc_log_write (ns_g_lctx,
                                   ...
                                   "for view %s due to " "configuration parameter " "mismatch", view->name);
                }
                else
                {
                    ...
                    isc_log_write (ns_g_lctx,
                                   NS_LOGCATEGORY_GENERAL,
                                   NS_LOGMODULE_SERVER, ISC_LOG_DEBUG (3), "reusing existing cache");
        ...
        if (cache == NULL)
        {
            ...
            CHECK (dns_cache_create3 (cmctx, hmctx, ns_g_taskmgr,
                                      ns_g_timermgr, view->rdclass, cachename, "rbt", 0, NULL, &cache));
            isc_mem_detach (&cmctx);
     ...
     */
    CHECK (get_view_querysource_dispatch (maps, AF_INET, &dispatch4, ISC_TF (ISC_LIST_PREV (view, link) == NULL)));
    CHECK (get_view_querysource_dispatch (maps, AF_INET6, &dispatch6, ISC_TF (ISC_LIST_PREV (view, link) == NULL)));
    if (dispatch4 == NULL && dispatch6 == NULL)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "unable to obtain neither an IPv4 nor" " an IPv6 dispatch");
        result = ISC_R_UNEXPECTED;
        goto cleanup;
    }
    CHECK (dns_view_createresolver (view, ns_g_taskmgr, 31,
                                    ns_g_socketmgr, ns_g_timermgr, resopts, ns_g_dispatchmgr, dispatch4, dispatch6));
    ...
    if (max_cache_size != 0)
    {
        ...
        if (view != nsc->primaryview && max_adb_size > MAX_ADB_SIZE_FOR_CACHESHARE)
        {
            max_adb_size = MAX_ADB_SIZE_FOR_CACHESHARE;
            if (!nsc->adbsizeadjusted)
            {
                dns_adb_setadbsize (nsc->primaryview->adb, MAX_ADB_SIZE_FOR_CACHESHARE);
    ...
    if (disabled != NULL)
    {
        for (element = cfg_list_first (disabled); element != NULL; element = cfg_list_next (element))
            CHECK (disable_algorithms (cfg_listelt_value (element), view->resolver));
    }
    ...
    if (forwarders != NULL)
        CHECK (configure_forward (config, view, dns_rootname, forwarders, forwardtype));
    ...
    if (view->hints == NULL)
    {
        ...
        if (need_hints)
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_SERVER, ISC_LOG_WARNING, "no root hints for view '%s'", view->name);
    }
    ...
    {
        CHECK (dns_tsigkeyring_add (ring, ns_g_server->session_keyname, ns_g_server->sessionkey));
    }
     ...
     */
    result = dns_viewlist_find (&ns_g_server->viewlist, view->name, view->rdclass, &pview);
    if (result != ISC_R_NOTFOUND && result != ISC_R_SUCCESS)
        goto cleanup;
    if (pview != NULL)
    {
     ...
     */
    {
        ...
        for (element = cfg_list_first (peers); element != NULL; element = cfg_list_next (element))
        {
     ...
     */
    {
        ...
        for (element = cfg_list_first (rrsetorder); element != NULL; element = cfg_list_next (element))
        {
     ...
     */
    CHECK (configure_view_acl (vconfig, config, "match-clients", NULL, actx, ns_g_mctx, &view->matchclients));
    CHECK (configure_view_acl (vconfig, config, "match-destinations", NULL, actx, ns_g_mctx, &view->matchdestinations));
    ...
    if (view->recursion && !view->additionalfromauth)
    {
        cfg_obj_log (obj, ns_g_lctx, ISC_LOG_WARNING,
                     "'additional-from-auth no' is only supported " "with 'recursion no'");
    ...
    {
        cfg_obj_log (obj, ns_g_lctx, ISC_LOG_WARNING,
                     "'additional-from-cache no' is only supported " "with 'recursion no'");
     ...
     */
    CHECK (configure_view_acl (vconfig, config, "allow-query-cache", NULL, actx, ns_g_mctx, &view->cacheacl));
    CHECK (configure_view_acl (vconfig, config, "allow-query-cache-on", NULL, actx, ns_g_mctx, &view->cacheonacl));
    if (view->cacheonacl == NULL)
        CHECK (configure_view_acl (NULL, ns_g_config,
                                   "allow-query-cache-on", NULL, actx, ns_g_mctx, &view->cacheonacl));
    if (strcmp (view->name, "_bind") != 0)
    {
        CHECK (configure_view_acl (vconfig, config, "allow-recursion", NULL, actx, ns_g_mctx, &view->recursionacl));
        CHECK (configure_view_acl (vconfig, config, "allow-recursion-on",
                                   NULL, actx, ns_g_mctx, &view->recursiononacl));
    }
    ...
    if (view->cacheacl == NULL && view->recursion)
        CHECK (configure_view_acl (vconfig, config, "allow-query", NULL, actx, ns_g_mctx, &view->cacheacl));
    if (view->recursion && view->recursionacl == NULL && view->cacheacl != NULL)
    ...
    if (view->recursionacl == NULL && view->recursion)
        CHECK (configure_view_acl (NULL, ns_g_config, "allow-recursion", NULL, actx, ns_g_mctx, &view->recursionacl));
    if (view->recursiononacl == NULL && view->recursion)
        CHECK (configure_view_acl (NULL, ns_g_config,
                                   "allow-recursion-on", NULL, actx, ns_g_mctx, &view->recursiononacl));
    if (view->cacheacl == NULL)
    {
        if (view->recursion)
            CHECK (configure_view_acl (NULL, ns_g_config, "allow-query-cache", NULL, actx, ns_g_mctx, &view->cacheacl));
        else
     ...
     */
    CHECK (configure_view_acl (vconfig, config, "deny-answer-addresses", "acl", actx, ns_g_mctx, &view->denyansweracl));
    CHECK (configure_view_nametable (vconfig, config, "deny-answer-addresses",
                                     "except-from", ns_g_mctx, &view->answeracl_exclude));
    ...
    CHECK (configure_view_nametable (vconfig, config, "deny-answer-aliases",
                                     "name", ns_g_mctx, &view->denyanswernames));
    CHECK (configure_view_nametable (vconfig, config, "deny-answer-aliases",
                                     "except-from", ns_g_mctx, &view->answernames_exclude));

    /*
     * Configure sortlist, if set
     */
    CHECK (configure_view_sortlist (vconfig, config, actx, ns_g_mctx, &view->sortlist));
    ...
    if (view->notifyacl == NULL)
        CHECK (configure_view_acl (NULL, ns_g_config, "allow-notify", NULL, actx, ns_g_mctx, &view->notifyacl));
    if (view->transferacl == NULL)
        CHECK (configure_view_acl (NULL, ns_g_config, "allow-transfer", NULL, actx, ns_g_mctx, &view->transferacl));
    if (view->updateacl == NULL)
        CHECK (configure_view_acl (NULL, ns_g_config, "allow-update", NULL, actx, ns_g_mctx, &view->updateacl));
    if (view->upfwdacl == NULL)
        CHECK (configure_view_acl (NULL, ns_g_config,
                                   "allow-update-forwarding", NULL, actx, ns_g_mctx, &view->upfwdacl));
    ...
    INSIST (result == ISC_R_SUCCESS);
    dns_resolver_setclientsperquery (view->resolver, cfg_obj_asuint32 (obj), max_clients_per_query);
    ...
    CHECK (configure_view_acl (vconfig, config, "filter-aaaa", NULL, actx, ns_g_mctx, &view->v4_aaaa_acl));
#endif
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        if (cfg_obj_isvoid (cfg_tuple_get (dlvobj, "trust-anchor")))
        {
            ...
            else if (!strcasecmp (dom, "auto"))
            {
                auto_dlv = ISC_TRUE;
                obj = NULL;
                result = cfg_map_get (ns_g_defaults, "dnssec-lookaside", &obj);
            }
        }
    }
    ...
    if (result == ISC_R_SUCCESS)
    {
        for (element = cfg_list_first (obj); element != NULL; element = cfg_list_next (element))
        {
            ...
            obj = cfg_listelt_value (element);
            str = cfg_obj_asstring (cfg_tuple_get (obj, "trust-anchor"));
            isc_buffer_init (&b, str, strlen (str));
            isc_buffer_add (&b, strlen (str));
            dlv = dns_fixedname_name (&view->dlv_fixed);
            CHECK (dns_name_fromtext (dlv, &b, dns_rootname, DNS_NAME_DOWNCASE, NULL));
     ...
     * "security roots".
     */
    CHECK (configure_view_dnsseckeys (view, vconfig, config, bindkeys, auto_dlv, auto_root, mctx));
    ...
    if (obj == NULL && disablelist == NULL && view->rdclass == dns_rdataclass_in)
    {
        rfc1918 = ISC_FALSE;
    ...
    if (empty_zones_enable && !lwresd_g_useresolvconf)
    {
        ...
        isc_boolean_t logit;
        const char *empty_dbtype[4] = { "_builtin", "empty", NULL, NULL };
        int empty_dbtypec = 4;
        ...
        logit = ISC_TRUE;
        for (empty = empty_zones[empty_zone].zone; empty != NULL; empty = empty_zones[++empty_zone].zone)
        {
             ...
             */
            CHECK (dns_name_fromtext (name, &buffer, dns_rootname, 0, NULL));
            if (disablelist != NULL && on_disable_list (disablelist, name))
                continue;
            ...
            if (!rfc1918 && empty_zones[empty_zone].rfc1918)
            {
                if (logit)
                {
                    isc_log_write (ns_g_lctx,
                                   ...
                                   "Warning%s%s: "
                                   "'empty-zones-enable/"
                                   "disable-empty-zone' " "not set: disabling " "RFC 1918 empty zones", sep, viewname);
                    logit = ISC_FALSE;
                }
                continue;
            }
            ...
            if (pview != NULL)
            {
                ...
                dns_view_detach (&pview);
                if (zone != NULL)
                    check_dbtype (&zone, empty_dbtypec, empty_dbtype, mctx);
                if (zone != NULL)
                {
                    dns_zone_setview (zone, view);
                    CHECK (dns_view_addzone (view, zone));
            ...
            dns_zone_settype (zone, dns_zone_master);
            dns_zone_setstats (zone, ns_g_server->zonestats);
            CHECK (dns_zone_setdbtype (zone, empty_dbtypec, empty_dbtype));
            if (view->queryacl != NULL)
                dns_zone_setqueryacl (zone, view->queryacl);
            if (view->queryonacl != NULL)
                dns_zone_setqueryonacl (zone, view->queryonacl);
            ...
            CHECK (dns_view_addzone (view, zone));
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_SERVER, ISC_LOG_INFO, "automatic empty zone%s%s: %s", sep, viewname, empty);
            dns_zone_detach (&zone);
        }
    }
...
static isc_result_t configure_alternates (const cfg_obj_t * config, dns_view_t * view, const cfg_obj_t * alternates)
{
    ...
    if (alternates != NULL)
    {
        ...
        if (cfg_obj_isuint32 (portobj))
        {
            ...
            {
                cfg_obj_log (portobj, ns_g_lctx, ISC_LOG_ERROR, "port '%u' out of range", val);
                return (ISC_R_RANGE);
            }
            port = (in_port_t) val;
        }
    ...
    for (element = cfg_list_first (addresses); element != NULL; element = cfg_list_next (element))
    {
        ...
        if (!cfg_obj_issockaddr (alternate))
        {
            ...
            const char *str = cfg_obj_asstring (cfg_tuple_get (alternate, "name"));
            ...
            CHECK (dns_resolver_addalternate (view->resolver, NULL, name, myport));
            continue;
        }
                   ...
                   const cfg_obj_t * forwarders, const cfg_obj_t * forwardtype)
{
    ...
    for (element = cfg_list_first (faddresses); element != NULL; element = cfg_list_next (element))
    {
    ...
    {
        if (forwardtype != NULL)
            cfg_obj_log (forwarders, ns_g_lctx, ISC_LOG_WARNING, "no forwarders seen; disabling " "forwarding");
    ...
    result = dns_fwdtable_add (view->fwdtable, origin, &addresses, fwdpolicy);
    if (result != ISC_R_SUCCESS)
    {
        ...
        cfg_obj_log (forwarders, ns_g_lctx, ISC_LOG_WARNING,
                     "could not set up forwarding for domain '%s': %s", namebuf, isc_result_totext (result));
        goto cleanup;
    }
...
static isc_result_t get_viewinfo (const cfg_obj_t * vconfig, const char **namep, dns_rdataclass_t * classp)
{
    ...
    if (vconfig != NULL)
    {
        ...
        result = ns_config_getclass (classobj, dns_rdataclass_in, &viewclass);
    }
    else
    {
        viewname = "_default";
                ...
                cfg_aclconfctx_t * aclconf, isc_boolean_t added)
{
    ...
    CHECK (dns_name_fromtext (dns_fixedname_name (&fixorigin), &buffer, dns_rootname, 0, NULL));
    origin = dns_fixedname_name (&fixorigin);

    CHECK (ns_config_getclass (cfg_tuple_get (zconfig, "class"), view->rdclass, &zclass));
    if (zclass != view->rdclass)
    {
        ...
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_ERROR, "zone '%s': wrong class for view '%s'", zname, vname);
        result = ISC_R_FAILURE;
        goto cleanup;
    }
    ...
    if (typeobj == NULL)
    {
        cfg_obj_log (zconfig, ns_g_lctx, ISC_LOG_ERROR, "zone '%s' 'type' not specified", zname);
    ...
    if (strcasecmp (ztypestr, "hint") == 0)
    {
        ...
        {
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_SERVER, ISC_LOG_ERROR, "zone '%s': 'file' not specified", zname);
            result = ISC_R_FAILURE;
            goto cleanup;
        }
        if (dns_name_equal (origin, dns_rootname))
        {
            ...
            if (result != ISC_R_SUCCESS)
            {
                isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                               ...
                               "from '%s': %s", hintsfile, isc_result_totext (result));
                goto cleanup;
            }
            /*
        ...
        {
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_SERVER, ISC_LOG_WARNING, "ignoring non-root hint zone '%s'", zname);
            result = ISC_R_SUCCESS;
        }
    ...
    if (strcasecmp (ztypestr, "forward") == 0)
    {
        ...
        result = configure_forward (config, view, origin, forwarders, forwardtype);
        goto cleanup;
    }
    ...
    if (result == ISC_R_SUCCESS)
    {
         ...
         */
        cfg_obj_log (zconfig, ns_g_lctx, ISC_LOG_ERROR, "zone '%s' already exists", zname);
    ...
    if (cfg_map_get (zoptions, "forwarders", &forwarders) == ISC_R_SUCCESS)
    {
        forwardtype = NULL;
        (void) cfg_map_get (zoptions, "forward", &forwardtype);
        CHECK (configure_forward (config, view, origin, forwarders, forwardtype));
    }
...
static isc_result_t add_keydata_zone (dns_view_t * view, const char *directory, isc_mem_t * mctx)
{
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   ...
                   "set up managed keys zone for view %s, file '%s'", view->name, filename);
...
static isc_result_t directory_callback (const char *clausename, const cfg_obj_t * obj, void *arg)
{
    ...
    if (!isc_file_ischdiridempotent (directory))
        cfg_obj_log (obj, ns_g_lctx, ISC_LOG_WARNING, "option 'directory' contains relative path '%s'", directory);

    result = isc_dir_chdir (directory);
    if (result != ISC_R_SUCCESS)
    {
        cfg_obj_log (obj, ns_g_lctx, ISC_LOG_ERROR,
                     "change directory to '%s' failed: %s", directory, isc_result_totext (result));
        return (result);
    }
...
static void scan_interfaces (ns_server_t * server, isc_boolean_t verbose)
{
     ...
     */
    dns_aclenv_copy (&server->aclenv, ns_interfacemgr_getaclenv (server->interfacemgr));

    server->aclenv.match_mapped = match_mapped;
}

static isc_result_t
add_listenelt (isc_mem_t * mctx, ns_listenlist_t * list, isc_sockaddr_t * addr, isc_boolean_t wcardport_ok)
{
    ...
    if (!isc_sockaddr_equal (&any_sa6, addr) && (wcardport_ok || isc_sockaddr_getport (addr) != 0))
    {
        ...
        result = dns_iptable_addprefix (src_acl->iptable, &netaddr, 128, ISC_TRUE);
        if (result != ISC_R_SUCCESS)
            goto clean;

        result = ns_listenelt_create (mctx, isc_sockaddr_getport (addr), src_acl, &lelt);
        if (result != ISC_R_SUCCESS)
            goto clean;
...
static void adjust_interfaces (ns_server_t * server, isc_mem_t * mctx)
{
    ...
    for (result = dns_zone_first (server->zonemgr, &zone);
         result == ISC_R_SUCCESS; next = NULL, result = dns_zone_next (zone, &next), zone = next)
    {
        dns_view_t *zoneview;
        ...
        for (view = ISC_LIST_HEAD (server->viewlist);
             view != NULL && view != zoneview; view = ISC_LIST_NEXT (view, link))
            ;
        if (view == NULL)
            continue;
  ...
  fail:
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   ...
                   "could not adjust the listen-on list; " "some interfaces may not work");
    goto clean;
}
...
set_limit (const cfg_obj_t ** maps, const char *configname,
           const char *description, isc_resource_t resourceid, isc_resourcevalue_t defaultvalue)
{
    const cfg_obj_t *obj = NULL;
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL, NS_LOGMODULE_SERVER,
                   ...
                   "set maximum %s to %" ISC_PRINT_QUADFORMAT "u: %s", description, value, isc_result_totext (result));
}
...
static void portset_fromconf (isc_portset_t * portset, const cfg_obj_t * ports, isc_boolean_t positive)
{
    ...
    for (element = cfg_list_first (ports); element != NULL; element = cfg_list_next (element))
    {
        ...
        else
        {
            ...
            {
                isc_portset_removerange (portset, loport, hiport);
            }
        }
    }
}
...
generate_session_key (const char *filename, const char *keynamestr,
                      ...
                      isc_uint16_t bits, isc_mem_t * mctx, dns_tsigkey_t ** tsigkeyp)
{
    isc_result_t result = ISC_R_SUCCESS;
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   NS_LOGMODULE_SERVER, ISC_LOG_INFO, "generating session key for dynamic DNS");

    /* generate key */
    result = dst_key_generate (keyname, algtype, bits, 1, 0, DNS_KEYPROTO_ANY, dns_rdataclass_in, mctx, &key);
    if (result != ISC_R_SUCCESS)
        return (result);
    ...
    CHECK (dns_tsigkey_createfromkey (dst_key_name (key), algname, key,
                                      ISC_FALSE, NULL, now, now, mctx, NULL, &tsigkey));
    ...
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_ERROR, "could not create %s", filename);
        result = ISC_R_NOPERM;
        goto cleanup;
    }
    ...
    fprintf (fp, "key \"%s\" {\n"
             "\talgorithm %s;\n"
             "\tsecret \"%.*s\";\n};\n", keynamestr, algstr,
             (int) isc_buffer_usedlength (&key_txtbuffer), (char *) isc_buffer_base (&key_txtbuffer));
  ...
  cleanup:
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   NS_LOGMODULE_SERVER, ISC_LOG_ERROR,
                   "failed to generate session key " "for dynamic DNS: %s", isc_result_totext (result));
    if (tsigkey != NULL)
...
static isc_result_t configure_session_key (const cfg_obj_t ** maps, ns_server_t * server, isc_mem_t * mctx)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        cfg_obj_log (obj, ns_g_lctx, ISC_LOG_ERROR, "session-keyalg: "
                     "unsupported or unknown algorithm '%s'%s", algstr, server->session_keyfile != NULL ? s : "");
        return (result);
    }
    ...
    if (keyfile == NULL)
    {
        ...
        need_createnew = ISC_TRUE;
             ...
             server->session_keyalg != algtype || server->session_keybits != bits)
    {
        need_deleteold = ISC_TRUE;
        need_createnew = ISC_TRUE;
    }
    ...
    if (need_createnew)
    {
        ...
        CHECK (generate_session_key (keyfile, keynamestr, keyname, algstr,
                                     algname, algtype, bits, mctx, &server->sessionkey));
    }
                ...
                cfg_parser_t * parser, cfg_aclconfctx_t * actx)
{
    ...
    if (result == ISC_R_SUCCESS)
        result = cfg_parse_file (nzparser, view->new_zone_file, &cfg_type_newzones, &nzconfig);
    if (result == ISC_R_SUCCESS)
    {
...
static isc_result_t load_configuration (const char *filename, ns_server_t * server, isc_boolean_t first_time)
{
    ...
    if (first_time)
    {
        ...
        RUNTIME_CHECK (cfg_map_get (ns_g_config, "options", &ns_g_defaults) == ISC_R_SUCCESS);
    }
    ...
    if (!(ns_g_lwresdonly && lwresd_g_useresolvconf))
    {
        ...
        result = cfg_parse_file (conf_parser, filename, &cfg_type_namedconf, &config);
    }
     ...
     */
    if (ns_g_lwresdonly && (lwresd_g_useresolvconf || (!ns_g_conffileset && result == ISC_R_FILENOTFOUND)))
    {
        isc_log_write (ns_g_lctx,
                       NS_LOGCATEGORY_GENERAL, NS_LOGMODULE_SERVER,
                       ISC_LOG_INFO, "loading configuration from '%s'", lwresd_g_resolvconffile);
        if (conf_parser != NULL)
            cfg_parser_destroy (&conf_parser);
        CHECK (cfg_parser_create (ns_g_mctx, ns_g_lctx, &conf_parser));
        result = ns_lwresd_parseeresolvconf (ns_g_mctx, conf_parser, &config);
    }
    CHECK (result);
    ...
    INSIST (result == ISC_R_SUCCESS);
    CHECKM (setstring (server, &server->bindkeysfile, cfg_obj_asstring (obj)), "strdup");

    if (access (server->bindkeysfile, R_OK) == 0)
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_INFO,
                       "reading built-in trusted " "keys from file '%s'", server->bindkeysfile);

        CHECK (cfg_parser_create (ns_g_mctx, ns_g_lctx, &bindkeys_parser));

        result = cfg_parse_file (bindkeys_parser, server->bindkeysfile, &cfg_type_bindkeys, &bindkeys);
        CHECK (result);
    }
    ...
    if (result == ISC_R_SUCCESS && (isc_resourcevalue_t) maxsocks > nfiles)
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       ...
                       " is smaller than max sockets (%u)", nfiles, maxsocks);
    }
    ...
    if (reserved + 128U > maxsocks && maxsocks != 0)
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_WARNING,
                       "less than 128 UDP sockets available after " "applying 'reserved-sockets' and 'maxsockets'");
    }
    ...
    configure_server_quota (maps, "recursive-clients", &server->recursionquota);
    ...
    CHECK (configure_view_acl (NULL, config, "blackhole", NULL, ns_g_aclconfctx, ns_g_mctx, &server->blackholeacl));
    ...
    CHECKM (ns_statschannels_configure (ns_g_server, config, ns_g_aclconfctx), "configuring statistics server(s)");

    /*
     * Configure sets of UDP query source ports.
     */
    CHECKM (isc_portset_create (ns_g_mctx, &v4portset), "creating UDP port set");
    CHECKM (isc_portset_create (ns_g_mctx, &v6portset), "creating UDP port set");
    ...
    {
        CHECKM (isc_net_getudpportrange (AF_INET, &udpport_low, &udpport_high), "get the default UDP/IPv4 port range");
        ...
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_INFO,
                       "using default UDP/IPv4 port range: [%d, %d]", udpport_low, udpport_high);
    }
    ...
    {
        CHECKM (isc_net_getudpportrange (AF_INET6, &udpport_low, &udpport_high), "get the default UDP/IPv6 port range");
        if (udpport_low == udpport_high)
        ...
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_INFO,
                       "using default UDP/IPv6 port range: [%d, %d]", udpport_low, udpport_high);
    }
     ...
     */
    {
        ...
        {
            /* check return code? */
            (void) ns_listenlist_fromconfig (clistenon, config, ns_g_aclconfctx, ns_g_mctx, &listenon);
        }
        else if (!ns_g_lwresdonly)
        {
             ...
             */
            CHECK (ns_listenlist_default (ns_g_mctx, listen_port, ISC_TRUE, &listenon));
        }
        if (listenon != NULL)
        {
     ...
     */
    {
        ...
        else if (!ns_g_lwresdonly)
        {
            ...
            CHECK (ns_listenlist_default (ns_g_mctx, listen_port, enable, &listenon));
        }
        if (listenon != NULL)
        {
    ...
    {
        CHECK (isc_timer_reset (server->interface_timer, isc_timertype_inactive, NULL, NULL, ISC_TRUE));
    }
    else if (server->interface_interval != interface_interval)
    {
        isc_interval_set (&interval, interface_interval, 0);
        CHECK (isc_timer_reset (server->interface_timer, isc_timertype_ticker, NULL, &interval, ISC_FALSE));
    }
    ...
    {
        CHECK (isc_timer_reset (server->heartbeat_timer, isc_timertype_inactive, NULL, NULL, ISC_TRUE));
    }
    else if (server->heartbeat_interval != heartbeat_interval)
    {
        isc_interval_set (&interval, heartbeat_interval, 0);
        CHECK (isc_timer_reset (server->heartbeat_timer, isc_timertype_ticker, NULL, &interval, ISC_FALSE));
    }
    server->heartbeat_interval = heartbeat_interval;

    isc_interval_set (&interval, 1200, 0);
    CHECK (isc_timer_reset (server->pps_timer, isc_timertype_ticker, NULL, &interval, ISC_FALSE));
    ...
    for (element = cfg_list_first (views); element != NULL; element = cfg_list_next (element))
    {
        ...
        CHECK (setup_newzones (view, config, vconfig, conf_parser, ns_g_aclconfctx));
    ...
    if (views == NULL)
    {
        ...
        CHECK (setup_newzones (view, config, NULL, conf_parser, ns_g_aclconfctx));
     ...
     */
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   NS_LOGMODULE_SERVER, ISC_LOG_INFO, "sizing zone task pool based on %d zones", num_zones);
    ...
    for (element = cfg_list_first (views); element != NULL; element = cfg_list_next (element))
    {
        ...
        CHECK (configure_view (view, config, vconfig, &cachelist, bindkeys, ns_g_mctx, ns_g_aclconfctx, ISC_TRUE));
    ...
    if (views == NULL)
    {
        ...
        CHECK (configure_view (view, config, NULL, &cachelist, bindkeys, ns_g_mctx, ns_g_aclconfctx, ISC_TRUE));
        dns_view_freeze (view);
        dns_view_detach (&view);
    }
     ...
     */
    builtin_views = NULL;
    RUNTIME_CHECK (cfg_map_get (ns_g_config, "view", &builtin_views) == ISC_R_SUCCESS);
    for (element = cfg_list_first (builtin_views); element != NULL; element = cfg_list_next (element))
    {
        cfg_obj_t *vconfig = cfg_listelt_value (element);

        CHECK (create_view (vconfig, &builtin_viewlist, &view));
        CHECK (configure_view (view, config, vconfig, &cachelist, bindkeys, ns_g_mctx, ns_g_aclconfctx, ISC_FALSE));
        dns_view_freeze (view);
        dns_view_detach (&view);
        view = NULL;
    }
    ...
    if (options != NULL)
    {
        ...
        CHECKM (ns_tkeyctx_fromconfig (options, ns_g_mctx, ns_g_entropy, &t), "configuring TKEY");
     ...
     */
    CHECKM (ns_controls_configure (ns_g_server->controls, config, ns_g_aclconfctx), "binding control channel(s)");

    /*
     * Bind the lwresd port(s).
     */
    CHECKM (ns_lwresd_configure (ns_g_mctx, config), "binding lightweight resolver ports");
    ...
    if (first_time)
    {
        ...
        {
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_SERVER, ISC_LOG_INFO, "no source of entropy found");
        }
        else
        {
            ...
            if (result != ISC_R_SUCCESS)
                isc_log_write (ns_g_lctx,
                               ...
                               NS_LOGMODULE_SERVER,
                               ISC_LOG_INFO,
                               "could not open entropy source " "%s: %s", randomdev, isc_result_totext (result));
            ...
            if (ns_g_fallbackentropy != NULL)
            {
                if (result != ISC_R_SUCCESS)
                {
                    isc_log_write (ns_g_lctx,
                                   NS_LOGCATEGORY_GENERAL,
                                   NS_LOGMODULE_SERVER,
                                   ISC_LOG_INFO, "using pre-chroot entropy source " "%s", PATH_RANDOMDEV);
                    isc_entropy_detach (&ns_g_entropy);
                    isc_entropy_attach (ns_g_fallbackentropy, &ns_g_entropy);
                }
    ...
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_ERROR, "the working directory is not writable");
    }
    ...
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_INFO, "ignoring config file logging " "statement due to -g option");
    }
    else
    {
        ...
        CHECKM (isc_logconfig_create (ns_g_lctx, &logc), "creating new logging configuration");

        logobj = NULL;
        (void) cfg_map_get (config, "logging", &logobj);
        if (logobj != NULL)
        {
            CHECKM (ns_log_configure (logc, logobj), "configuring logging");
        }
        else
        {
            CHECKM (ns_log_setdefaultchannels (logc), "setting up default logging channels");
            CHECKM (ns_log_setunmatchedcategory (logc), "setting up default 'category unmatched'");
            CHECKM (ns_log_setdefaultcategory (logc), "setting up default 'category default'");
        }
        ...
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_DEBUG (1), "now using logging configuration from " "config file");
    }
    ...
    if (first_time)
    {
        ...
        else
        {
            ...
            if (categories != NULL)
            {
                ...
                for (element = cfg_list_first (categories); element != NULL; element = cfg_list_next (element))
                {
    ...
    obj = NULL;
    if (options != NULL && cfg_map_get (options, "memstatistics", &obj) == ISC_R_SUCCESS)
        ns_g_memstatistics = cfg_obj_asboolean (obj);
    else
        ns_g_memstatistics = ISC_TF ((isc_mem_debugging & ISC_MEM_DEBUGRECORD) != 0);
    ...
    INSIST (result == ISC_R_SUCCESS);
    CHECKM (setstring (server, &server->statsfile, cfg_obj_asstring (obj)), "strdup");

    obj = NULL;
    result = ns_config_get (maps, "dump-file", &obj);
    INSIST (result == ISC_R_SUCCESS);
    CHECKM (setstring (server, &server->dumpfile, cfg_obj_asstring (obj)), "strdup");

    obj = NULL;
    result = ns_config_get (maps, "secroots-file", &obj);
    INSIST (result == ISC_R_SUCCESS);
    CHECKM (setstring (server, &server->secrootsfile, cfg_obj_asstring (obj)), "strdup");

    obj = NULL;
    result = ns_config_get (maps, "recursing-file", &obj);
    INSIST (result == ISC_R_SUCCESS);
    CHECKM (setstring (server, &server->recfile, cfg_obj_asstring (obj)), "strdup");
  ...
  cleanup:
    ...
    for (view = ISC_LIST_HEAD (viewlist); view != NULL; view = view_next)
    {
        ...
        if (result == ISC_R_SUCCESS && strcmp (view->name, "_bind") != 0)
            (void) dns_zt_apply (view->zonetable, ISC_FALSE, removed, view);
        dns_view_detach (&view);
    }
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL, NS_LOGMODULE_SERVER,
                   ISC_LOG_DEBUG (1), "load_configuration: %s", isc_result_totext (result));

    return (result);
}

static isc_result_t load_zones (ns_server_t * server, isc_boolean_t stop)
{
    isc_result_t result;
...
static void run_server (isc_task_t * task, isc_event_t * event)
{
    ...
    CHECKFATAL (dns_dispatchmgr_create (ns_g_mctx, ns_g_entropy, &ns_g_dispatchmgr), "creating dispatch manager");
    ...
    CHECKFATAL (ns_interfacemgr_create (ns_g_mctx, ns_g_taskmgr,
                                        ...
                                        &server->interfacemgr), "creating interface manager");

    CHECKFATAL (isc_timer_create (ns_g_timermgr, isc_timertype_inactive,
                                  NULL, NULL, server->task,
                                  interface_timer_tick, server, &server->interface_timer), "creating interface timer");

    CHECKFATAL (isc_timer_create (ns_g_timermgr, isc_timertype_inactive,
                                  NULL, NULL, server->task,
                                  heartbeat_timer_tick, server, &server->heartbeat_timer), "creating heartbeat timer");

    CHECKFATAL (isc_timer_create (ns_g_timermgr, isc_timertype_inactive,
                                  NULL, NULL, server->task, pps_timer_tick,
                                  server, &server->pps_timer), "creating pps timer");

    CHECKFATAL (cfg_parser_create (ns_g_mctx, NULL, &ns_g_parser), "creating default configuration parser");

    if (ns_g_lwresdonly)
        CHECKFATAL (load_configuration (lwresd_g_conffile, server, ISC_TRUE), "loading configuration");
    else
        CHECKFATAL (load_configuration (ns_g_conffile, server, ISC_TRUE), "loading configuration");
    ...
    ns_os_started ();
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL, NS_LOGMODULE_SERVER, ISC_LOG_NOTICE, "running");
}
...
static void shutdown_server (isc_task_t * task, isc_event_t * event)
{
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL, NS_LOGMODULE_SERVER,
                   ISC_LOG_INFO, "shutting down%s", flush ? ": flushing changes" : "");
    ...
    for (view = ISC_LIST_HEAD (server->viewlist); view != NULL; view = view_next)
    {
        view_next = ISC_LIST_NEXT (view, link);
...
void ns_server_create (isc_mem_t * mctx, ns_server_t ** serverp)
{
    ...
    CHECKFATAL (dns_rootns_create (mctx, dns_rdataclass_in, NULL, &server->in_roothints), "setting up root hints");

    CHECKFATAL (isc_mutex_init (&server->reload_event_lock), "initializing reload event lock");
    server->reload_event =
        isc_event_allocate (ns_g_mctx, server, NS_EVENT_RELOAD, ns_server_reload, server, sizeof (isc_event_t));
    CHECKFATAL (server->reload_event == NULL ? ISC_R_NOMEMORY : ISC_R_SUCCESS, "allocating reload event");

    CHECKFATAL (dst_lib_init2 (ns_g_mctx, ns_g_entropy, ns_g_engine, ISC_ENTROPY_GOODONLY), "initializing DST");

    server->tkeyctx = NULL;
    CHECKFATAL (dns_tkeyctx_create (ns_g_mctx, ns_g_entropy, &server->tkeyctx), "creating TKEY context");

    /*
     * Setup the server task, which is responsible for coordinating
     * startup and shutdown of the server.
     */
    CHECKFATAL (isc_task_create (ns_g_taskmgr, 0, &server->task), "creating server task");
    isc_task_setname (server->task, "server", server);
    CHECKFATAL (isc_task_onshutdown (server->task, shutdown_server, server), "isc_task_onshutdown");
    CHECKFATAL (isc_app_onrun (ns_g_mctx, server->task, run_server, server), "isc_app_onrun");
    ...
    CHECKFATAL (dns_zonemgr_create (ns_g_mctx, ns_g_taskmgr, ns_g_timermgr,
                                    ns_g_socketmgr, &server->zonemgr), "dns_zonemgr_create");
    CHECKFATAL (dns_zonemgr_setsize (server->zonemgr, 1000), "dns_zonemgr_setsize");

    server->statsfile = isc_mem_strdup (server->mctx, "named.stats");
    CHECKFATAL (server->statsfile == NULL ? ISC_R_NOMEMORY : ISC_R_SUCCESS, "isc_mem_strdup");
    server->nsstats = NULL;
    ...
    CHECKFATAL (isc_stats_create (server->mctx, &server->sockstats, isc_sockstatscounter_max), "isc_stats_create");
    isc_socketmgr_setstats (ns_g_socketmgr, server->sockstats);

    server->bindkeysfile = isc_mem_strdup (server->mctx, "bind.keys");
    CHECKFATAL (server->bindkeysfile == NULL ? ISC_R_NOMEMORY : ISC_R_SUCCESS, "isc_mem_strdup");

    server->dumpfile = isc_mem_strdup (server->mctx, "named_dump.db");
    CHECKFATAL (server->dumpfile == NULL ? ISC_R_NOMEMORY : ISC_R_SUCCESS, "isc_mem_strdup");

    server->secrootsfile = isc_mem_strdup (server->mctx, "named.secroots");
    CHECKFATAL (server->secrootsfile == NULL ? ISC_R_NOMEMORY : ISC_R_SUCCESS, "isc_mem_strdup");

    server->recfile = isc_mem_strdup (server->mctx, "named.recursing");
    CHECKFATAL (server->recfile == NULL ? ISC_R_NOMEMORY : ISC_R_SUCCESS, "isc_mem_strdup");
    ...
    CHECKFATAL (isc_stats_create (ns_g_mctx, &server->nsstats, dns_nsstatscounter_max), "dns_stats_create (server)");

    CHECKFATAL (dns_rdatatypestats_create (ns_g_mctx, &server->rcvquerystats), "dns_stats_create (rcvquery)");

    CHECKFATAL (dns_opcodestats_create (ns_g_mctx, &server->opcodestats), "dns_stats_create (opcode)");

    CHECKFATAL (isc_stats_create (ns_g_mctx, &server->zonestats, dns_zonestatscounter_max), "dns_stats_create (zone)");

    CHECKFATAL (isc_stats_create (ns_g_mctx, &server->resolverstats,
                                  dns_resstatscounter_max), "dns_stats_create (resolver)");

    server->flushonshutdown = ISC_FALSE;
    server->log_queries = ISC_FALSE;

    server->controls = NULL;
    CHECKFATAL (ns_controls_create (server, &server->controls), "ns_controls_create");
...
static void fatal (const char *msg, isc_result_t result)
{
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL, NS_LOGMODULE_SERVER,
...
static void end_reserved_dispatches (ns_server_t * server, isc_boolean_t all)
{
    ...
    for (dispatch = ISC_LIST_HEAD (server->dispatches); dispatch != NULL; dispatch = nextdispatch)
    {
...
void ns_add_reserved_dispatch (ns_server_t * server, const isc_sockaddr_t * addr)
{
    ...
    for (dispatch = ISC_LIST_HEAD (server->dispatches); dispatch != NULL; dispatch = ISC_LIST_NEXT (dispatch, link))
    {
    ...
    result = dns_dispatch_getudp (ns_g_dispatchmgr, ns_g_socketmgr,
                                  ...
                                  1000, 32768, 16411, 16433, attrs, attrmask, &dispatch->dispatch);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
  ...
  cleanup:
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   NS_LOGMODULE_SERVER, ISC_LOG_WARNING,
                   "unable to create dispatch for reserved port %s: %s", addrbuf, isc_result_totext (result));
}
...
static isc_result_t loadconfig (ns_server_t * server)
{
    ...
    result = load_configuration (ns_g_lwresdonly ? lwresd_g_conffile : ns_g_conffile, server, ISC_FALSE);
    if (result == ISC_R_SUCCESS)
    {
        end_reserved_dispatches (server, ISC_FALSE);
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_INFO, "reloading configuration succeeded");
    }
    else
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_ERROR,
                       "reloading configuration failed: %s", isc_result_totext (result));
    }
    return (result);
}
...
static isc_result_t reload (ns_server_t * server)
{
    ...
    if (result == ISC_R_SUCCESS)
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_INFO, "reloading zones succeeded");
    else
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_ERROR, "reloading zones failed: %s", isc_result_totext (result));
...
static void reconfig (ns_server_t * server)
{
    ...
    if (result == ISC_R_SUCCESS)
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_INFO, "any newly configured zones are now loaded");
    else
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_ERROR, "loading new zones failed: %s", isc_result_totext (result));
...
static void ns_server_reload (isc_task_t * task, isc_event_t * event)
{
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   NS_LOGMODULE_SERVER, ISC_LOG_INFO, "received SIGHUP signal to reload zones");
    (void) reload (server);
 ...
 * set '*zonep' to NULL.
 */
static isc_result_t zone_from_args (ns_server_t * server, char *args, dns_zone_t ** zonep, const char **zonename)
{
    char *input, *ptr;
    ...
    if (viewtxt == NULL)
    {
        result = dns_viewlist_findzone (&server->viewlist,
                                        dns_fixedname_name (&name), ISC_TF (classtxt == NULL), rdclass, zonep);
    }
    else
    {
        result = dns_viewlist_find (&server->viewlist, viewtxt, rdclass, &view);
        if (result != ISC_R_SUCCESS)
            goto fail1;

        result = dns_zt_find (view->zonetable, dns_fixedname_name (&name), 0, NULL, zonep);
        dns_view_detach (&view);
    }
 ...
 */
isc_result_t ns_server_retransfercommand (ns_server_t * server, char *args)
{
    isc_result_t result;
...
isc_result_t ns_server_reloadcommand (ns_server_t * server, char *args, isc_buffer_t * text)
{
    ...
    if (msg != NULL && strlen (msg) < isc_buffer_availablelength (text))
        isc_buffer_putmem (text, (const unsigned char *) msg, strlen (msg) + 1);
    return (result);
}
...
isc_result_t ns_server_togglequerylog (ns_server_t * server)
{
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   NS_LOGMODULE_SERVER, ISC_LOG_INFO, "query logging is now %s", server->log_queries ? "on" : "off");
    return (ISC_R_SUCCESS);
}
                          ...
                          cfg_aclconfctx_t * actx, isc_mem_t * mctx, ns_listenlist_t ** target)
{
    ...
    for (element = cfg_list_first (listenlist); element != NULL; element = cfg_list_next (element))
    {
        ns_listenelt_t *delt = NULL;

        const cfg_obj_t *listener = cfg_listelt_value (element);

        result = ns_listenelt_fromconfig (listener, config, actx, mctx, &delt);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
                         ...
                         cfg_aclconfctx_t * actx, isc_mem_t * mctx, ns_listenelt_t ** target)
{
    ...
    else
    {
        if (cfg_obj_asuint32 (portobj) >= ISC_UINT16_MAX)
        {
            cfg_obj_log (portobj, ns_g_lctx, ISC_LOG_ERROR,
                         "port value '%u' is out of range", cfg_obj_asuint32 (portobj));
            return (ISC_R_RANGE);
        }
    ...
    result = cfg_acl_fromconfig (cfg_tuple_get (listener, "acl"), config, ns_g_lctx, actx, mctx, 0, &delt->acl);
    if (result != ISC_R_SUCCESS)
    {
...
isc_result_t ns_server_dumpstats (ns_server_t * server)
{
    ...
    CHECKMF (isc_stdio_open (server->statsfile, "a", &fp), "could not open statistics dump file", server->statsfile);
  ...
  cleanup:
    ...
    if (result == ISC_R_SUCCESS)
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL, NS_LOGMODULE_SERVER, ISC_LOG_INFO, "dumpstats complete");
    else
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_ERROR, "dumpstats failed: %s", dns_result_totext (result));
    return (result);
}
...
static isc_result_t add_view_tolist (struct dumpcontext *dctx, dns_view_t * view)
{
     ...
     */
    for (vle = ISC_LIST_HEAD (dctx->viewlist); vle != NULL; vle = ISC_LIST_NEXT (vle, link))
        if (vle->view == view)
            return (ISC_R_SUCCESS);
    ...
    if (dctx->dumpzones)
        result = dns_zt_apply (view->zonetable, ISC_TRUE, add_zone_tolist, dctx);
    return (result);
}
  ...
  resume:
    if (dctx->dumpcache && dns_view_iscacheshared (dctx->view->view))
    {
        fprintf (dctx->fp,
                 ...
                 dctx->view->view->name, dns_cache_getname (dctx->view->view->cache));
    }
    else if (dctx->zone == NULL && dctx->cache == NULL && dctx->dumpcache)
    {
        ...
        if (dctx->cache != NULL)
        {
            ...
            result = dns_master_dumptostreaminc (dctx->mctx,
                                                 dctx->cache, NULL,
                                                 style, dctx->fp, dctx->task, dumpdone, dctx, &dctx->mdctx);
    ...
    if (dctx->cache != NULL)
    {
        ...
        dns_resolver_printbadcache (dctx->view->view->resolver, dctx->fp);
        dns_db_detach (&dctx->cache);
    }
    if (dctx->dumpzones)
    {
    ...
    if (result == ISC_R_SUCCESS)
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL, NS_LOGMODULE_SERVER, ISC_LOG_INFO, "dumpdb complete");
  cleanup:
    if (result != ISC_R_SUCCESS)
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_ERROR, "dumpdb failed: %s", dns_result_totext (result));
...
isc_result_t ns_server_dumpdb (ns_server_t * server, char *args)
{
    ...
    CHECKMF (isc_stdio_open (server->dumpfile, "w", &dctx->fp), "could not open dump file", server->dumpfile);

    sep = (args == NULL) ? "" : ": ";
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   NS_LOGMODULE_SERVER, ISC_LOG_INFO, "dumpdb started%s%s", sep, (args != NULL) ? args : "");
...
isc_result_t ns_server_dumpsecroots (ns_server_t * server, char *args)
{
    dns_view_t *view;
  ...
  cleanup:
    ...
    if (fp != NULL)
        (void) isc_stdio_close (fp);
    if (result == ISC_R_SUCCESS)
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL, NS_LOGMODULE_SERVER, ISC_LOG_INFO, "dumpsecroots complete");
    else
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_ERROR, "dumpsecroots failed: %s", dns_result_totext (result));
    return (result);
}
...
isc_result_t ns_server_dumprecursing (ns_server_t * server)
{
    ...
    CHECKMF (isc_stdio_open (server->recfile, "w", &fp), "could not open dump file", server->recfile);
    fprintf (fp, ";\n; Recursing Queries\n;\n");
  ...
  cleanup:
    ...
    if (result == ISC_R_SUCCESS)
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL, NS_LOGMODULE_SERVER, ISC_LOG_INFO, "dumprecursing complete");
    else
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_ERROR, "dumprecursing failed: %s", dns_result_totext (result));
    return (result);
}
...
isc_result_t ns_server_setdebuglevel (ns_server_t * server, char *args)
{
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   NS_LOGMODULE_SERVER, ISC_LOG_INFO, "debug level is now %d", ns_g_debuglevel);
    return (ISC_R_SUCCESS);
}

isc_result_t ns_server_validation (ns_server_t * server, char *args)
{
    char *ptr, *viewname;
    ...
    if (!strcasecmp (ptr, "on") || !strcasecmp (ptr, "yes") || !strcasecmp (ptr, "enable") || !strcasecmp (ptr, "true"))
        enable = ISC_TRUE;
...
isc_result_t ns_server_flushcache (ns_server_t * server, char *args)
{
    ...
    if (viewname != NULL)
    {
        ...
        for (view = ISC_LIST_HEAD (server->viewlist); view != NULL; view = ISC_LIST_NEXT (view, link))
        {
            if (strcasecmp (viewname, view->name) != 0)
                continue;
            found = ISC_TRUE;
            for (nsc = ISC_LIST_HEAD (server->cachelist); nsc != NULL; nsc = ISC_LIST_NEXT (nsc, link))
            {
    ...
    for (nsc = ISC_LIST_HEAD (server->cachelist); nsc != NULL; nsc = ISC_LIST_NEXT (nsc, link))
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           ...
                           nsc->primaryview->name, isc_result_totext (result));
        }
    }
    ...
    for (view = ISC_LIST_HEAD (server->viewlist); view != NULL; view = ISC_LIST_NEXT (view, link))
    {
        ...
        for (nsc = ISC_LIST_HEAD (server->cachelist); nsc != NULL; nsc = ISC_LIST_NEXT (nsc, link))
        {
            ...
            if (result != ISC_R_SUCCESS)
            {
                ...
                isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                               NS_LOGMODULE_SERVER, ISC_LOG_ERROR,
                               "fixing cache in view '%s' " "failed: %s", view->name, isc_result_totext (result));
            }
        }
    }
    ...
    if (flushed && found)
    {
        if (viewname != NULL)
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_SERVER, ISC_LOG_INFO, "flushing cache in view '%s' succeeded", viewname);
        else
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_SERVER, ISC_LOG_INFO, "flushing caches in all views succeeded");
        result = ISC_R_SUCCESS;
    }
    else
    {
        if (!found)
        {
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_SERVER, ISC_LOG_ERROR,
                           "flushing cache in view '%s' failed: " "view not found", viewname);
            result = ISC_R_NOTFOUND;
        }
        else
            result = ISC_R_FAILURE;
...
isc_result_t ns_server_flushname (ns_server_t * server, char *args)
{
    ...
    for (view = ISC_LIST_HEAD (server->viewlist); view != NULL; view = ISC_LIST_NEXT (view, link))
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           ...
                           "failed: %s", target, view->name, isc_result_totext (result));
        }
    }
    if (flushed && found)
    {
        if (viewname != NULL)
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_SERVER, ISC_LOG_INFO,
                           "flushing name '%s' in cache view '%s' " "succeeded", target, viewname);
        else
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_SERVER, ISC_LOG_INFO,
                           "flushing name '%s' in all cache views " "succeeded", target);
        result = ISC_R_SUCCESS;
    }
    else
    {
        if (!found)
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_SERVER, ISC_LOG_ERROR,
                           "flushing name '%s' in cache view '%s' " "failed: view not found", target, viewname);
        result = ISC_R_FAILURE;
    }
    isc_task_endexclusive (server->task);
    return (result);
...
isc_result_t ns_server_status (ns_server_t * server, isc_buffer_t * text)
{
    ...
    xferrunning = dns_zonemgr_getcount (server->zonemgr, DNS_ZONESTATE_XFERRUNNING);
    xferdeferred = dns_zonemgr_getcount (server->zonemgr, DNS_ZONESTATE_XFERDEFERRED);
    ...
    n = snprintf ((char *) isc_buffer_used (text), isc_buffer_availablelength (text), "version: %s%s%s%s\n"
                  ...
                  "tcp clients: %d/%d\n" "server is up and running", ns_g_version, ob, alt, cb,
#ifdef ISC_PLATFORM_USETHREADS
                  ...
                  server->recursionquota.max, server->tcpquota.used, server->tcpquota.max);
    ...
    for (;;)
    {
        ...
        if (tkey != NULL)
        {
            ...
            if (strcmp (namestr, target) == 0)
            {
                (*foundkeys)++;
                dns_rbtnodechain_invalidate (&chain);
                (void) dns_rbt_deletename (ring->keys, &tkey->name, ISC_FALSE);
                goto again;
            }
        }
...
isc_result_t ns_server_tsigdelete (ns_server_t * server, char *command, isc_buffer_t * text)
{
    ...
    for (view = ISC_LIST_HEAD (server->viewlist); view != NULL; view = ISC_LIST_NEXT (view, link))
    {
        if (viewname == NULL || strcmp (view->name, viewname) == 0)
        {
            ...
            result = delete_keynames (view->dynamickeys, target, &foundkeys);
            RWUNLOCK (&view->dynamickeys->lock, isc_rwlocktype_write);
            if (result != ISC_R_SUCCESS)
            {
                isc_task_endexclusive (server->task);
                return (result);
    ...
    n = snprintf ((char *) isc_buffer_used (text),
                  isc_buffer_availablelength (text), "%d tsig keys deleted.\n", foundkeys);
    if (n >= isc_buffer_availablelength (text))
        return (ISC_R_NOSPACE);
    isc_buffer_add (text, n);
...
static isc_result_t
list_keynames (dns_view_t * view, dns_tsig_keyring_t * ring, isc_buffer_t * text, unsigned int *foundkeys)
{
    char namestr[DNS_NAME_FORMATSIZE];
...
isc_result_t ns_server_tsiglist (ns_server_t * server, isc_buffer_t * text)
{
    ...
    for (view = ISC_LIST_HEAD (server->viewlist); view != NULL; view = ISC_LIST_NEXT (view, link))
    {
        ...
        result = list_keynames (view, view->statickeys, text, &foundkeys);
        ...
        result = list_keynames (view, view->dynamickeys, text, &foundkeys);
    ...
    if (foundkeys == 0)
    {
        n = snprintf ((char *) isc_buffer_used (text), isc_buffer_availablelength (text), "no tsig keys found.\n");
        if (n >= isc_buffer_availablelength (text))
            return (ISC_R_NOSPACE);
        isc_buffer_add (text, n);
    }
 ...
 */
isc_result_t ns_server_rekey (ns_server_t * server, char *args)
{
    isc_result_t result;
...
isc_result_t ns_server_freeze (ns_server_t * server, isc_boolean_t freeze, char *args, isc_buffer_t * text)
{
    ...
    if (zone == NULL)
    {
        ...
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_INFO,
                       "%s all zones: %s", freeze ? "freezing" : "thawing", isc_result_totext (tresult));
        return (tresult);
    }
    type = dns_zone_gettype (zone);
    if (type != dns_zone_master)
    ...
    if (freeze)
    {
        if (frozen)
        {
            msg = "WARNING: The zone was already frozen.\n"
                "Someone else may be editing it or " "it may still be re-loading.";
            result = DNS_R_FROZEN;
        }
        if (result == ISC_R_SUCCESS)
        {
            ...
            if (result != ISC_R_SUCCESS)
                msg = "Flushing the zone updates to " "disk failed.";
        }
        if (result == ISC_R_SUCCESS)
        {
    ...
    else
    {
        if (frozen)
        {
            ...
            switch (result)
            {
                case ISC_R_SUCCESS:
                case DNS_R_UPTODATE:
                    msg = "The zone reload and thaw was " "successful.";
                    result = ISC_R_SUCCESS;
                    break;
                case DNS_R_CONTINUE:
    ...
    dns_rdataclass_format (dns_zone_getclass (zone), classstr, sizeof (classstr));
    dns_name_format (dns_zone_getorigin (zone), zonename, sizeof (zonename));
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   NS_LOGMODULE_SERVER, ISC_LOG_INFO,
                   "%s zone '%s/%s'%s%s: %s",
                   freeze ? "freezing" : "thawing", zonename, classstr, sep, vname, isc_result_totext (result));
    dns_zone_detach (&zone);
    return (result);
}
...
isc_result_t ns_smf_add_message (isc_buffer_t * text)
{
    ...
    n = snprintf ((char *) isc_buffer_used (text), isc_buffer_availablelength (text), "use svcadm(1M) to manage named");
    if (n >= isc_buffer_availablelength (text))
        return (ISC_R_NOSPACE);
    isc_buffer_add (text, n);
    return (ISC_R_SUCCESS);
 ...
 */
isc_result_t ns_server_add_zone (ns_server_t * server, char *args)
{
    isc_result_t result;
    ...
    CHECK (cfg_parse_buffer (parser, &argbuf, &cfg_type_addzoneconf, &config));
    ...
    dns_view_thaw (view);
    result = configure_zone (cfg->config, parms, vconfig, server->mctx, view, cfg->actx, ISC_FALSE);
    dns_view_freeze (view);
    isc_task_endexclusive (server->task);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_INFO, "addzone failed; reverting.");
    ...
    {
        CHECK (isc_stdio_write (classname, strlen (classname), 1, fp, NULL));
        CHECK (isc_stdio_write (" ", 1, 1, fp, NULL));
    }
    ...
    fp = NULL;
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   NS_LOGMODULE_SERVER, ISC_LOG_INFO, "zone %s added to view %s via addzone", zonename, viewname);
 ...
 */
isc_result_t ns_server_del_zone (ns_server_t * server, char *args)
{
    isc_result_t result;
    ...
    if (ifp != NULL && result == ISC_R_SUCCESS)
    {
        ...
        CHECK (isc_string_printf (buf, 1023, "%s.%ld", filename, (long) getpid ()));
        ...
        else
        {
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_SERVER, ISC_LOG_WARNING,
                           "deleted zone %s was missing from " "new zone file", zonename);
            goto cleanup;
        }
    }
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   NS_LOGMODULE_SERVER, ISC_LOG_INFO, "zone %s removed via delzone", zonename);
