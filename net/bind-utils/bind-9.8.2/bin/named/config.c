...
isc_result_t ns_config_get (const cfg_obj_t ** maps, const char *name, const cfg_obj_t ** obj)
{
    int i;
...
isc_result_t ns_checknames_get (const cfg_obj_t ** maps, const char *which, const cfg_obj_t ** obj)
{
    ...
    for (i = 0;; i++)
    {
        ...
        if (cfg_map_get (maps[i], "check-names", &checknames) == ISC_R_SUCCESS)
        {
            ...
            for (element = cfg_list_first (checknames); element != NULL; element = cfg_list_next (element))
            {
...
isc_result_t ns_config_getclass (const cfg_obj_t * classobj, dns_rdataclass_t defclass, dns_rdataclass_t * classp)
{
    ...
    if (result != ISC_R_SUCCESS)
        cfg_obj_log (classobj, ns_g_lctx, ISC_LOG_ERROR, "unknown class '%s'", r.base);
    return (result);
}
...
isc_result_t ns_config_gettype (const cfg_obj_t * typeobj, dns_rdatatype_t deftype, dns_rdatatype_t * typep)
{
    ...
    if (result != ISC_R_SUCCESS)
        cfg_obj_log (typeobj, ns_g_lctx, ISC_LOG_ERROR, "unknown type '%s'", r.base);
    return (result);
}
...
ns_config_getiplist (const cfg_obj_t * config, const cfg_obj_t * list,
                     in_port_t defport, isc_mem_t * mctx, isc_sockaddr_t ** addrsp, isc_uint32_t * countp)
{
    int count, i = 0;
    ...
    for (element = cfg_list_first (addrlist); element != NULL; element = cfg_list_next (element), i++)
    {
        INSIST (i < count);
...
void ns_config_putiplist (isc_mem_t * mctx, isc_sockaddr_t ** addrsp, isc_uint32_t count)
{
    INSIST (addrsp != NULL && *addrsp != NULL);
  ...
  resume:
    for (; element != NULL; element = cfg_list_next (element))
    {
        ...
        if (!cfg_obj_issockaddr (addr))
        {
            ...
            {
                cfg_obj_log (addr, ns_g_lctx, ISC_LOG_ERROR, "masters \"%s\" not found", listname);
        ...
        if (result != ISC_R_SUCCESS)
            goto cleanup;
        result = dns_name_dup (dns_fixedname_name (&fname), mctx, keys[i]);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
        i++;
    }
...
void ns_config_putipandkeylist (isc_mem_t * mctx, isc_sockaddr_t ** addrsp, dns_name_t *** keysp, isc_uint32_t count)
{
    unsigned int i;
...
isc_result_t ns_config_getport (const cfg_obj_t * config, in_port_t * portp)
{
    ...
    {
        cfg_obj_log (portobj, ns_g_lctx, ISC_LOG_ERROR, "port '%u' out of range", cfg_obj_asuint32 (portobj));
        return (ISC_R_RANGE);
    }
...
struct keyalgorithms
{
    const char *str;
    enum
    { hmacnone, hmacmd5, hmacsha1, hmacsha224,
...
isc_result_t
ns_config_getkeyalgorithm2 (const char *str, dns_name_t ** name, unsigned int *typep, isc_uint16_t * digestbits)
{
    int i;
    ...
    for (i = 0; algorithms[i].str != NULL; i++)
    {
        ...
        if (strncasecmp (algorithms[i].str, str, len) == 0 &&
            (str[len] == '\0' || (algorithms[i].size != 0 && str[len] == '-')))
            break;
    }
