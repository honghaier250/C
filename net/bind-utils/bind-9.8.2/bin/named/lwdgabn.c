...
static void setup_addresses (ns_lwdclient_t * client, dns_adbfind_t * find, unsigned int at)
{
    ...
    while (ai != NULL && client->gabn.naddrs < LWRES_MAX_ADDRS)
    {
        ...
        ns_lwdclient_log (50, "adding address %p, family %d, length %d", addr->address, addr->family, addr->length);
...
static void sort_addresses (ns_lwdclient_t * client)
{
    ...
    ns_sortlist_byaddrsetup (lwresd->view->sortlist, &remote, &order, &arg);
    if (order == NULL)
    {
    ...
    for (i = 0; i < naddrs; i++)
    {
        result = lwaddr_netaddr_fromlwresaddr (&addrs[i].address, &client->addrs[i]);
    ...
    for (i = 0; i < naddrs; i++)
    {
        result = lwaddr_lwresaddr_fromnetaddr (&client->addrs[i], &addrs[i].address);
        INSIST (result == ISC_R_SUCCESS);
    }
...
static void generate_reply (ns_lwdclient_t * client)
{
    ...
    lwres = lwres_gabnresponse_render (cm->lwctx, &client->gabn, &client->pkt, &lwb);
...
static isc_result_t add_alias (ns_lwdclient_t * client)
{
     ...
     */
    result = dns_name_totext (dns_fixedname_name (&client->target_name), ISC_TRUE, &client->recv_buffer);
    if (result != ISC_R_SUCCESS)
        return (result);
...
static void restart_find (ns_lwdclient_t * client)
{
  ...
  find_again:
    ...
    result = dns_adb_createfind (client->clientmgr->view->adb,
                                 ...
                                 dns_fixedname_name (&client->target_name),
                                 client->clientmgr->view->dstport, &client->find);
    ...
    if (result == DNS_R_ALIAS)
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            ns_lwdclient_log (50, "out of buffer space adding alias");
            ns_lwdclient_errorpktsend (client, LWRES_R_FAILURE);
            return;
        }
        goto find_again;
    ...
    ns_lwdclient_log (50, "find returned %d (%s)", result, isc_result_totext (result));
     ...
     */
    if (NEED_V4 (client) && ((client->find->query_pending & DNS_ADBFIND_INET) == 0))
    {
        ns_lwdclient_log (50, "client %p ipv4 satisfied by find %p", client, client->find);
        claimed = ISC_TRUE;
        client->v4find = client->find;
    }
     ...
     */
    if (NEED_V6 (client) && ((client->find->query_pending & DNS_ADBFIND_INET6) == 0))
    {
        ns_lwdclient_log (50, "client %p ipv6 satisfied by find %p", client, client->find);
        claimed = ISC_TRUE;
        client->v6find = client->find;
    }
...
static isc_result_t start_find (ns_lwdclient_t * client)
{
    isc_result_t result;
...
static void init_gabn (ns_lwdclient_t * client)
{
    int i;
...
void ns_lwdclient_processgabn (ns_lwdclient_t * client, lwres_buffer_t * b)
{
    ...
    result = lwres_gabnrequest_parse (client->clientmgr->lwctx, b, &client->pkt, &req);
    ...
    ns_lwdclient_log (50, "client %p looking for addrtypes %08x", client, client->find_wanted);
