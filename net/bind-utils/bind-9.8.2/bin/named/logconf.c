 ...
 */
static isc_result_t category_fromconf (const cfg_obj_t * ccat, isc_logconfig_t * lctx)
{
    isc_result_t result;
    ...
    if (category == NULL)
    {
        cfg_obj_log (ccat, ns_g_lctx, ISC_LOG_ERROR, "unknown logging category '%s' ignored", catname);
        /*
    ...
    for (element = cfg_list_first (destinations); element != NULL; element = cfg_list_next (element))
    {
        const cfg_obj_t *channel = cfg_listelt_value (element);

        const char *channelname = cfg_obj_asstring (channel);

        result = isc_log_usechannel (lctx, channelname, category, module);
        if (result != ISC_R_SUCCESS)
        {
            isc_log_write (ns_g_lctx, CFG_LOGCATEGORY_CONFIG,
                           NS_LOGMODULE_SERVER, ISC_LOG_ERROR,
                           "logging channel '%s': %s", channelname, isc_result_totext (result));
            return (result);
        }
    }
    return (ISC_R_SUCCESS);
 ...
 */
static isc_result_t channel_fromconf (const cfg_obj_t * channel, isc_logconfig_t * lctx)
{
    isc_result_t result;
    ...
    if (fileobj != NULL)
    {
        ...
        const cfg_obj_t *versionsobj = cfg_tuple_get (fileobj, "versions");
        ...
        if (sizeobj != NULL && cfg_obj_isuint64 (sizeobj) && cfg_obj_asuint64 (sizeobj) < ISC_OFFSET_MAXIMUM)
    ...
    else if (syslogobj != NULL)
    {
        ...
        if (cfg_obj_isstring (syslogobj))
        {
            ...
            (void) isc_syslog_facilityfromstring (facilitystr, &facility);
        }
    ...
    result = isc_log_createchannel (lctx, channelname, type, level, &dest, flags);
    ...
    if (result == ISC_R_SUCCESS && type == ISC_LOG_TOFILE)
    {
        ...
        if (result == ISC_R_SUCCESS || result == ISC_R_FILENOTFOUND)
        {
            ...
            if (result != ISC_R_SUCCESS)
            {
                syslog (LOG_ERR, "isc_stdio_open '%s' failed: %s", dest.file.name, isc_result_totext (result));
                fprintf (stderr, "isc_stdio_open '%s' failed: %s", dest.file.name, isc_result_totext (result));
            }
            else
        ...
        {
            syslog (LOG_ERR, "isc_file_isplainfile '%s' failed: %s", dest.file.name, isc_result_totext (result));
            fprintf (stderr, "isc_file_isplainfile '%s' failed: %s", dest.file.name, isc_result_totext (result));
        }
    }
