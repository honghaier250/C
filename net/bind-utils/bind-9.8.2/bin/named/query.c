...
query_findclosestnsec3 (dns_name_t * qname, dns_db_t * db,
                        ...
                        dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset,
                        dns_name_t * fname, isc_boolean_t exact, dns_name_t * found);

static inline void log_queryerror (ns_client_t * client, isc_result_t result, int line, int level);
 ...
 */
static inline void inc_stats (ns_client_t * client, isc_statscounter_t counter)
{
...
static inline void query_freefreeversions (ns_client_t * client, isc_boolean_t everything)
{
         ...
         dbversion != NULL; dbversion = dbversion_next, i++)
    {
        ...
        if (i > 3 || everything)
        {
            ...
            isc_mem_put (client->mctx, dbversion, sizeof (*dbversion));
        }
    }
}
...
static inline void query_putrdataset (ns_client_t * client, dns_rdataset_t ** rdatasetp)
{
...
static inline void query_reset (ns_client_t * client, isc_boolean_t everything)
{
     ...
     */
    for (dbversion = ISC_LIST_HEAD (client->query.activeversions); dbversion != NULL; dbversion = dbversion_next)
    {
        dbversion_next = ISC_LIST_NEXT (dbversion, link);
        dns_db_closeversion (dbversion->db, &dbversion->version, ISC_FALSE);
        dns_db_detach (&dbversion->db);
        ISC_LIST_INITANDAPPEND (client->query.freeversions, dbversion, link);
    }
    ...
    {
        isc_mem_put (client->mctx, client->query.dns64_aaaaok, client->query.dns64_aaaaoklen * sizeof (isc_boolean_t));
    ...
    for (dbuf = ISC_LIST_HEAD (client->query.namebufs); dbuf != NULL; dbuf = dbuf_next)
    {
    ...
    client->query.attributes = (NS_QUERYATTR_RECURSIONOK | NS_QUERYATTR_CACHEOK | NS_QUERYATTR_SECURE);
    ...
    if (client->query.rpz_st != NULL)
    {
        ...
        if (everything)
        {
            isc_mem_put (client->mctx, client->query.rpz_st, sizeof (*client->query.rpz_st));
...
static inline isc_result_t query_newnamebuf (ns_client_t * client)
{
    isc_buffer_t *dbuf;
...
static inline isc_buffer_t *query_getnamebuf (ns_client_t * client)
{
    isc_buffer_t *dbuf;
...
static inline void query_keepname (ns_client_t * client, dns_name_t * name, isc_buffer_t * dbuf)
{
    isc_region_t r;
...
static inline void query_releasename (ns_client_t * client, dns_name_t ** namep)
{
    ...
    {
        INSIST ((client->query.attributes & NS_QUERYATTR_NAMEBUFUSED) != 0);
        client->query.attributes &= ~NS_QUERYATTR_NAMEBUFUSED;
    }
...
static inline dns_name_t *query_newname (ns_client_t * client, isc_buffer_t * dbuf, isc_buffer_t * nbuf)
{
    dns_name_t *name;
...
static inline dns_rdataset_t *query_newrdataset (ns_client_t * client)
{
    dns_rdataset_t *rdataset;
...
static inline isc_result_t query_newdbversion (ns_client_t * client, unsigned int n)
{
    unsigned int i;
...
static inline ns_dbversion_t *query_getdbversion (ns_client_t * client)
{
    isc_result_t result;
...
static inline ns_dbversion_t *query_findversion (ns_client_t * client, dns_db_t * db)
{
    ns_dbversion_t *dbversion;
                      ...
                      dns_zone_t * zone, dns_db_t * db, dns_dbversion_t ** versionp)
{
     ...
     */
    if (!client->view->additionalfromauth && client->query.authdbset && db != client->query.authdb)
     ...
     */
    if (dns_zone_gettype (zone) == dns_zone_staticstub && !RECURSIONOK (client))
    {
        return (DNS_R_REFUSED);
    }
    ...
    if (queryacl == NULL)
    {
        ...
        if ((client->query.attributes & NS_QUERYATTR_QUERYOKVALID) != 0)
        {
            ...
            if ((client->query.attributes & NS_QUERYATTR_QUERYOK) == 0)
            {
                dbversion->queryok = ISC_FALSE;
                return (DNS_R_REFUSED);
            }
    ...
    if ((options & DNS_GETDB_NOLOG) == 0)
    {
        ...
        if (result == ISC_R_SUCCESS)
        {
            ...
            {
                ns_client_aclmsg ("query", name, qtype, client->view->rdclass, msg, sizeof (msg));
                ns_client_log (client,
                               DNS_LOGCATEGORY_SECURITY, NS_LOGMODULE_QUERY, ISC_LOG_DEBUG (3), "%s approved", msg);
            }
        }
        else
        {
            ns_client_aclmsg ("query", name, qtype, client->view->rdclass, msg, sizeof (msg));
            ns_client_log (client, DNS_LOGCATEGORY_SECURITY, NS_LOGMODULE_QUERY, ISC_LOG_INFO, "%s denied", msg);
        }
    }
...
query_getzonedb (ns_client_t * client, dns_name_t * name, dns_rdatatype_t qtype,
                 unsigned int options, dns_zone_t ** zonep, dns_db_t ** dbp, dns_dbversion_t ** versionp)
{
    isc_result_t result;
    ...
    result = query_validatezonedb (client, name, qtype, options, zone, db, versionp);
...
rpz_log_rewrite (ns_client_t * client, const char *disabled,
                 dns_rpz_policy_t policy, dns_rpz_type_t type, dns_name_t * rpz_qname)
{
    ...
    ns_client_log (client, DNS_LOGCATEGORY_RPZ, NS_LOGMODULE_QUERY,
                   ...
                   disabled, dns_rpz_type2str (type), dns_rpz_policy2str (policy), qname_buf, rpz_qname_buf);
}

static void
rpz_log_fail (ns_client_t * client, int level,
              dns_rpz_type_t rpz_type, dns_name_t * name, const char *str, isc_result_t result)
{
    ...
    ns_client_log (client, NS_LOGCATEGORY_QUERY_EERRORS,
                   ...
                   dns_rpz_type2str (rpz_type), namebuf1, namebuf2, str, isc_result_totext (result));
}
           ...
           dns_zone_t ** zonep, dns_db_t ** dbp, dns_dbversion_t ** versionp)
{
    ...
    result = query_getzonedb (client, rpz_qname, dns_rdatatype_any, DNS_GETDB_IGNOREACL, zonep, dbp, &rpz_version);
    if (result == ISC_R_SUCCESS)
    {
        if (isc_log_wouldlog (ns_g_lctx, DNS_RPZ_DEBUG_LEVEL2))
        {
            ...
            ns_client_log (client, DNS_LOGCATEGORY_RPZ,
                           ...
                           "try rpz %s rewrite %s via %s", dns_rpz_type2str (rpz_type), namebuf1, namebuf2);
        }
        *versionp = rpz_version;
        return (ISC_R_SUCCESS);
    }
    rpz_log_fail (client, DNS_RPZ_ERROR_LEVEL, rpz_type, rpz_qname, "query_getzonedb() ", result);
    return (result);
}

static inline isc_result_t
query_getcachedb (ns_client_t * client, dns_name_t * name, dns_rdatatype_t qtype, dns_db_t ** dbp, unsigned int options)
{
    isc_result_t result;
    ...
    if (check_acl)
    {
        ...
        result = ns_client_checkaclsilent (client, NULL, client->view->cacheacl, ISC_TRUE);
        if (result == ISC_R_SUCCESS)
        {
            /*
            ...
            if (log && isc_log_wouldlog (ns_g_lctx, ISC_LOG_DEBUG (3)))
            {
                ns_client_aclmsg ("query (cache)", name, qtype, client->view->rdclass, msg, sizeof (msg));
                ns_client_log (client,
                               DNS_LOGCATEGORY_SECURITY, NS_LOGMODULE_QUERY, ISC_LOG_DEBUG (3), "%s approved", msg);
            }
        }
...
query_isduplicate (ns_client_t * client, dns_name_t * name, dns_rdatatype_t type, dns_name_t ** mnamep)
{
    ...
    {
        result = dns_message_findname (client->message, section, name, type, 0, &mname, NULL);
        if (result == ISC_R_SUCCESS)
        {
            /*
...
static isc_result_t query_addadditional (void *arg, dns_name_t * name, dns_rdatatype_t qtype)
{
     ...
     * additional data.
     */
    result = query_getzonedb (client, name, qtype, DNS_GETDB_NOLOG, &zone, &db, &version);
    if (result != ISC_R_SUCCESS)
        goto try_cache;
    ...
    result = dns_db_find (db, name, version, type, client->query.dboptions,
                          client->now, &node, fname, rdataset, sigrdataset);
    if (result == ISC_R_SUCCESS)
    {
        if (sigrdataset != NULL && !dns_db_issecure (db) && dns_rdataset_isassociated (sigrdataset))
  ...
  try_cache:
    ...
    result = dns_db_find (db, name, version, type,
                          ...
                          client->now, &node, fname, rdataset, sigrdataset);
    if (result == DNS_R_GLUE && validate (client, db, fname, rdataset, sigrdataset))
  ...
  try_glue:
    ...
    result = dns_db_find (db, name, version, type,
                          ...
                          client->now, &node, fname, rdataset, sigrdataset);
    if (!(result == ISC_R_SUCCESS || result == DNS_R_ZONECUT || result == DNS_R_GLUE))
        goto cleanup;
  ...
  found:
     ...
     */
    mname = NULL;
    if (dns_rdataset_isassociated (rdataset) && !query_isduplicate (client, fname, type, &mname))
    {
        if (mname != NULL)
        {
    ...
    if (qtype == dns_rdatatype_a)
    {
        ...
        if (query_isduplicate (client, fname, dns_rdatatype_a, NULL))
            goto aaaa_lookup;
        result = dns_db_findrdataset (db, node, version, dns_rdatatype_a, 0, client->now, rdataset, sigrdataset);
        ...
        if (result == ISC_R_SUCCESS)
        {
            ...
            have_a = ISC_TRUE;
#endif
            if (!query_isduplicate (client, fname, dns_rdatatype_a, &mname))
            {
                if (mname != NULL)
                {
                    query_releasename (client, &fname);
      ...
      aaaa_lookup:
        if (query_isduplicate (client, fname, dns_rdatatype_aaaa, NULL))
            goto addname;
        result = dns_db_findrdataset (db, node, version, dns_rdatatype_aaaa, 0, client->now, rdataset, sigrdataset);
        if (result == DNS_R_NCACHENXDOMAIN)
            goto addname;
        if (result == DNS_R_NCACHENXRRSET)
        {
            ...
            if (have_a &&
                (client->filter_aaaa == dns_v4_aaaa_break_dnssec ||
                 (client->filter_aaaa == dns_v4_aaaa_filter &&
                  (!WANTDNSSEC (client) || sigrdataset == NULL || !dns_rdataset_isassociated (sigrdataset)))))
                goto addname;
#endif
            if (!query_isduplicate (client, fname, dns_rdatatype_aaaa, &mname))
            {
                if (mname != NULL)
                {
                    query_releasename (client, &fname);
  ...
  addname:
    ...
    if (type == dns_rdatatype_srv && trdataset != NULL)
    {
         ...
         * as well.
         */
        eresult = dns_rdataset_additionaldata (trdataset, query_addadditional, client);
    }
...
query_discardcache (ns_client_t * client, dns_rdataset_t * rdataset_base,
                    ...
                    dns_dbversion_t ** versionp, dns_dbnode_t ** nodep, dns_name_t * fname)
{
    dns_rdataset_t *rdataset;
    ...
    (void) dns_rdataset_putadditional (client->view->acache, rdataset_base, additionaltype, type);
}

static inline isc_result_t
query_iscachevalid (dns_zone_t * zone, dns_db_t * db, dns_db_t * db0, dns_dbversion_t * version)
{
...
static isc_result_t query_addadditional2 (void *arg, dns_name_t * name, dns_rdatatype_t qtype)
{
    ...
    if (qtype != dns_rdatatype_a || client->view->acache == NULL)
    {
         ...
         */
        return (query_addadditional (additionalctx->client, name, qtype));
    }
    ...
    result = dns_rdataset_getadditional (rdataset_base, additionaltype,
                                         ...
                                         &cdb, &cversion, &cnode, &cfname, client->message, client->now);
    if (result != ISC_R_SUCCESS)
        goto findauthdb;
    if (zone == NULL)
    {
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        query_discardcache (client, rdataset_base, additionaltype, type, &zone, &cdb, &cversion, &cnode, &cfname);
        goto findauthdb;
    }
    ...
    result = query_validatezonedb (client, name, qtype, DNS_GETDB_NOLOG, zone, cdb, NULL);
    if (result != ISC_R_SUCCESS)
    {
        query_discardcache (client, rdataset_base, additionaltype, type, &zone, &cdb, &cversion, &cnode, &cfname);
        goto try_cache;
  ...
  findauthdb:
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        (void) dns_rdataset_setadditional (rdataset_base, additionaltype,
                                           type, client->view->acache, NULL, NULL, NULL, NULL, NULL);
        goto try_cache;
    }
     ...
     */
    node = NULL;
    result = dns_db_find (db, name, version, type, client->query.dboptions, client->now, &node, fname, NULL, NULL);
    if (result == ISC_R_SUCCESS)
        goto found;

    /* Cache the negative result */
    (void) dns_rdataset_setadditional (rdataset_base, additionaltype,
                                       type, client->view->acache, zone, db, version, NULL, fname);
  ...
  try_cache:
    ...
    result = dns_db_find (db, name, version, type,
                          client->query.dboptions |
                          DNS_DBFIND_GLUEOK | DNS_DBFIND_ADDITIONALOK, client->now, &node, fname, NULL, NULL);
    if (result == ISC_R_SUCCESS)
        goto found;
  ...
  findglue:
    dns_db_attach (client->query.gluedb, &db);
    result = dns_db_find (db, name, version, type,
                          client->query.dboptions | DNS_DBFIND_GLUEOK, client->now, &node, fname, NULL, NULL);
    if (!(result == ISC_R_SUCCESS || result == DNS_R_ZONECUT || result == DNS_R_GLUE))
    {
        /* cache the negative result */
        (void) dns_rdataset_setadditional (rdataset_base, additionaltype,
                                           type, client->view->acache, NULL, db, version, NULL, fname);
        goto cleanup;
    }
  ...
  found:
    ...
    if (additionaltype == dns_rdatasetadditional_fromcache && query_isduplicate (client, fname, dns_rdatatype_a, NULL))
        goto aaaa_lookup;
    /*
  ...
  setcache:
    ...
    if (needadditionalcache == ISC_TRUE &&
        (additionaltype == dns_rdatasetadditional_fromauth || additionaltype == dns_rdatasetadditional_fromglue))
    {
        (void) dns_rdataset_setadditional (rdataset_base, additionaltype,
                                           type, client->view->acache, zone, db, version, node, &cfname);
    }

  foundcache:
    need_sigrrset = ISC_FALSE;
    mname0 = NULL;
    for (crdataset = ISC_LIST_HEAD (cfname.list); crdataset != NULL; crdataset = crdataset_next)
    {
        dns_name_t *mname;
        ...
        mname = NULL;
        if (crdataset->type == dns_rdatatype_a || crdataset->type == dns_rdatatype_aaaa)
        {
        ...
        else if (crdataset->type == dns_rdatatype_rrsig && need_sigrrset && WANTDNSSEC (client))
        {
            ISC_LIST_UNLINK (cfname.list, crdataset, link);
            ISC_LIST_APPEND (fname->list, crdataset, link);
...
static inline void query_addrdataset (ns_client_t * client, dns_name_t * fname, dns_rdataset_t * rdataset)
{
    ...
    if (client->view->order != NULL)
        rdataset->attributes |= dns_order_find (client->view->order, fname, rdataset->type, rdataset->rdclass);
    ...
    (void) dns_rdataset_additionaldata (rdataset, query_addadditional2, &additionalctx);
    CTRACE ("query_addrdataset: done");
}

static isc_result_t
query_dns64 (ns_client_t * client, dns_name_t ** namep, dns_rdataset_t * rdataset,
             dns_rdataset_t * sigrdataset, isc_buffer_t * dbuf, dns_section_t section)
{
    ...
    result = dns_message_findname (client->message, section,
                                   name, dns_rdatatype_aaaa, rdataset->covers, &mname, &mrdataset);
    if (result == ISC_R_SUCCESS)
    {
        /*
    ...
    if (rdataset->trust != dns_trust_secure && (section == DNS_SECTION_ANSWER || section == DNS_SECTION_AUTHORITY))
        client->query.attributes &= ~NS_QUERYATTR_SECURE;

    isc_netaddr_fromsockaddr (&netaddr, &client->peeraddr);

    result = isc_buffer_allocate (client->mctx, &buffer, view->dns64cnt * 16 * dns_rdataset_count (rdataset));
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    result = dns_message_gettemprdataset (client->message, &dns64_rdataset);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    result = dns_message_gettemprdatalist (client->message, &dns64_rdatalist);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    ...
    if (client->query.dns64_ttl != ISC_UINT32_MAX)
        dns64_rdatalist->ttl = ISC_MIN (rdataset->ttl, client->query.dns64_ttl);
    else
    ...
    for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
    {
        for (dns64 = ISC_LIST_HEAD (client->view->dns64); dns64 != NULL; dns64 = dns_dns64_next (dns64))
        {
            ...
            result = dns_dns64_aaaafroma (dns64, &netaddr,
                                          client->signer, &ns_g_server->aclenv, flags, rdata.data, r.base);
            if (result != ISC_R_SUCCESS)
            {
            ...
            result = dns_message_gettemprdata (client->message, &dns64_rdata);
            if (result != ISC_R_SUCCESS)
                goto cleanup;
            dns_rdata_init (dns64_rdata);
            dns_rdata_fromregion (dns64_rdata, dns_rdataclass_in, dns_rdatatype_aaaa, &r);
            ISC_LIST_APPEND (dns64_rdatalist->rdata, dns64_rdata, link);
            dns64_rdata = NULL;
            dns_rdata_reset (&rdata);
        }
    }
  ...
  cleanup:
    ...
    if (dns64_rdatalist != NULL)
    {
        for (dns64_rdata = ISC_LIST_HEAD (dns64_rdatalist->rdata);
             dns64_rdata != NULL; dns64_rdata = ISC_LIST_HEAD (dns64_rdatalist->rdata))
        {
            ISC_LIST_UNLINK (dns64_rdatalist->rdata, dns64_rdata, link);
            dns_message_puttemprdata (client->message, &dns64_rdata);
        }
...
query_filter64 (ns_client_t * client, dns_name_t ** namep,
                dns_rdataset_t * rdataset, isc_buffer_t * dbuf, dns_section_t section)
{
    dns_name_t *name, *mname;
    ...
    myrdatalist = NULL;
    result = dns_message_findname (client->message, section,
                                   name, dns_rdatatype_aaaa, rdataset->covers, &mname, &myrdataset);
    if (result == ISC_R_SUCCESS)
    {
        /*
         * We've already got an RRset of the given name and type.
    ...
    result = isc_buffer_allocate (client->mctx, &buffer, 16 * dns_rdataset_count (rdataset));
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    ...
    for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
    {
        ...
        dns_rdata_fromregion (myrdata, dns_rdataclass_in, dns_rdatatype_aaaa, &r);
  ...
  cleanup:
    ...
    if (myrdatalist != NULL)
    {
        for (myrdata = ISC_LIST_HEAD (myrdatalist->rdata);
             myrdata != NULL; myrdata = ISC_LIST_HEAD (myrdatalist->rdata))
        {
...
query_addrrset (ns_client_t * client, dns_name_t ** namep,
                dns_rdataset_t ** rdatasetp, dns_rdataset_t ** sigrdatasetp, isc_buffer_t * dbuf, dns_section_t section)
{
    dns_name_t *name, *mname;
    ...
    mrdataset = NULL;
    result = dns_message_findname (client->message, section,
                                   name, rdataset->type, rdataset->covers, &mname, &mrdataset);
    if (result == ISC_R_SUCCESS)
    {
        /*
         * We've already got an RRset of the given name and type.
              ...
              unsigned int override_ttl, isc_boolean_t isassociated)
{
    ...
    if (result == ISC_R_SUCCESS)
    {
        result = dns_db_findrdataset (db, node, version, dns_rdatatype_soa, 0, client->now, rdataset, sigrdataset);
    }
    else
    {
        ...
        result = dns_db_find (db, name, version, dns_rdatatype_soa,
                              client->query.dboptions, 0, &node, fname, rdataset, sigrdataset);
    }
    if (result != ISC_R_SUCCESS)
    {
        /*
    ...
    else
    {
        ...
        if (override_ttl != ISC_UINT32_MAX && override_ttl < rdataset->ttl)
        {
        ...
        if (sigrdataset != NULL)
            ...
            sigrdatasetp = NULL;
        query_addrrset (client, &name, &rdataset, sigrdatasetp, NULL, DNS_SECTION_AUTHORITY);
    }
...
static inline isc_result_t query_addns (ns_client_t * client, dns_db_t * db, dns_dbversion_t * version)
{
    ...
    if (result == ISC_R_SUCCESS)
    {
        result = dns_db_findrdataset (db, node, version, dns_rdatatype_ns, 0, client->now, rdataset, sigrdataset);
    }
    else
    {
    ...
    if (result != ISC_R_SUCCESS)
    {
        CTRACE ("query_addns: " "dns_db_findrdataset or dns_db_find failed");
        /*
...
static isc_result_t
query_add_cname (ns_client_t * client, dns_name_t * qname, dns_name_t * tname, dns_trust_t trust, dns_ttl_t ttl)
{
    dns_rdataset_t *rdataset;
    ...
    query_addrrset (client, &aname, &rdataset, NULL, NULL, DNS_SECTION_ANSWER);
    if (rdataset != NULL)
    {
        if (dns_rdataset_isassociated (rdataset))
            dns_rdataset_disassociate (rdataset);
...
mark_secure (ns_client_t * client, dns_db_t * db, dns_name_t * name,
             isc_uint32_t ttl, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    isc_result_t result;
    ...
    (void) dns_db_addrdataset (db, node, NULL, client->now, rdataset, 0, NULL);
    (void) dns_db_addrdataset (db, node, NULL, client->now, sigrdataset, 0, NULL);
    dns_db_detachnode (db, &node);
}
         ...
         dns_rdataset_t * keyrdataset, dst_key_t ** keyp)
{
    ...
    if (!dns_rdataset_isassociated (keyrdataset))
    {
        ...
        result = dns_db_findrdataset (db, node, NULL, dns_rdatatype_dnskey, 0, client->now, keyrdataset, NULL);
        dns_db_detachnode (db, &node);
        if (result != ISC_R_SUCCESS)
            return (ISC_FALSE);
    ...
    for (; result == ISC_R_SUCCESS; result = dns_rdataset_next (keyrdataset))
    {
        ...
        result = dst_key_fromdns (&rrsig->signer, rdata.rdclass, &b, client->mctx, keyp);
        if (result != ISC_R_SUCCESS)
            continue;
        if (rrsig->algorithm == (dns_secalg_t) dst_key_alg (*keyp) &&
            rrsig->keyid == (dns_keytag_t) dst_key_id (*keyp) && dst_key_iszonekey (*keyp))
        {
            secure = ISC_TRUE;
            break;
        }
        ...
        dns_rdata_t * rdata, isc_mem_t * mctx, isc_boolean_t acceptexpired)
{
  ...
  again:
    result = dns_dnssec_verify2 (name, rdataset, key, ignore, mctx, rdata, NULL);
          ...
          dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    ...
    for (result = dns_rdataset_first (sigrdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (sigrdataset))
    {
        ...
        if (result != ISC_R_SUCCESS)
            return (ISC_FALSE);
        if (!dns_resolver_algorithm_supported (client->view->resolver, name, rrsig.algorithm))
            continue;
        ...
        do
        {
            if (!get_key (client, db, &rrsig, &keyrdataset, &key))
                break;
            if (verify (key, name, rdataset, &rdata, client->mctx, client->view->acceptexpired))
            {
                dst_key_free (&key);
                dns_rdataset_disassociate (&keyrdataset);
                mark_secure (client, db, name, rrsig.originalttl, rdataset, sigrdataset);
                return (ISC_TRUE);
            }
            dst_key_free (&key);
        }
...
static void query_addbestns (ns_client_t * client)
{
     ...
     */
    result = query_getdb (client, client->query.qname, dns_rdatatype_ns, 0, &zone, &db, &version, &is_zone);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
  ...
  db_find:
    ...
    else
    {
        result = dns_db_findzonecut (db, client->query.qname,
                                     client->query.dboptions, client->now, &node, fname, rdataset, sigrdataset);
        if (result == ISC_R_SUCCESS)
        {
            if (zfname != NULL && !dns_name_issubdomain (fname, zfname))
            {
                /*
    ...
    if ((DNS_TRUST_PENDING (rdataset->trust) ||
        ...
        && !validate (client, db, fname, rdataset, sigrdataset) && !PENDINGOK (client->query.dboptions))
        goto cleanup;

    if ((DNS_TRUST_GLUE (rdataset->trust) ||
         (sigrdataset != NULL && DNS_TRUST_GLUE (sigrdataset->trust))) &&
        !validate (client, db, fname, rdataset, sigrdataset) && SECURE (client) && WANTDNSSEC (client))
        goto cleanup;
    ...
    if (!WANTDNSSEC (client))
        query_putrdataset (client, &sigrdataset);
    query_addrrset (client, &fname, &rdataset, &sigrdataset, dbuf, DNS_SECTION_AUTHORITY);
...
static void
query_addds (ns_client_t * client, dns_db_t * db, dns_dbnode_t * node, dns_dbversion_t * version, dns_name_t * name)
{
    dns_fixedname_t fixed;
     ...
     */
    result = dns_db_findrdataset (db, node, version, dns_rdatatype_ds, 0, client->now, rdataset, sigrdataset);
    /*
     * If we didn't find it, look for an NSEC.
     */
    if (result == ISC_R_NOTFOUND)
        result = dns_db_findrdataset (db, node, version, dns_rdatatype_nsec, 0, client->now, rdataset, sigrdataset);
    if (result != ISC_R_SUCCESS && result != ISC_R_NOTFOUND)
        goto addnsec3;
    if (!dns_rdataset_isassociated (rdataset) || !dns_rdataset_isassociated (sigrdataset))
        goto addnsec3;
  ...
  addnsec3:
    ...
    query_findclosestnsec3 (name, db, version, client, rdataset,
                            sigrdataset, fname, ISC_TRUE, dns_fixedname_name (&fixed));
    if (!dns_rdataset_isassociated (rdataset))
        goto cleanup;
    query_addrrset (client, &fname, &rdataset, &sigrdataset, dbuf, DNS_SECTION_AUTHORITY);
    /*
    ...
    if (!dns_name_equal (name, dns_fixedname_name (&fixed)))
    {
        ...
        dns_name_getlabelsequence (name, dns_name_countlabels (name) - count, count, dns_fixedname_name (&fixed));
        ...
        query_findclosestnsec3 (dns_fixedname_name (&fixed), db, version,
                                client, rdataset, sigrdataset, fname, ISC_FALSE, NULL);
        if (!dns_rdataset_isassociated (rdataset))
            goto cleanup;
        query_addrrset (client, &fname, &rdataset, &sigrdataset, dbuf, DNS_SECTION_AUTHORITY);
    }
...
query_addwildcardproof (ns_client_t * client, dns_db_t * db,
                        dns_dbversion_t * version, dns_name_t * name, isc_boolean_t ispositive, isc_boolean_t nodata)
{
    isc_buffer_t *dbuf, b;
  ...
  again:
    ...
    result = dns_db_find (db, name, version, dns_rdatatype_nsec, options, 0, &node, fname, rdataset, sigrdataset);
    if (node != NULL)
        dns_db_detachnode (db, &node);
    ...
    if (!dns_rdataset_isassociated (rdataset))
    {
        ...
        while (result == DNS_R_NXDOMAIN)
        {
            ...
            result = dns_db_find (db, cname, version, dns_rdatatype_nsec, options, 0, NULL, fname, NULL, NULL);
        }
        /*
         * Add closest (provable) encloser NSEC3.
         */
        query_findclosestnsec3 (cname, db, NULL, client, rdataset, sigrdataset, fname, ISC_TRUE, cname);
        if (!dns_rdataset_isassociated (rdataset))
            goto cleanup;
        if (!ispositive)
            query_addrrset (client, &fname, &rdataset, &sigrdataset, dbuf, DNS_SECTION_AUTHORITY);
        ...
        query_findclosestnsec3 (wname, db, NULL, client, rdataset, sigrdataset, fname, ISC_FALSE, NULL);
        if (!dns_rdataset_isassociated (rdataset))
            goto cleanup;
        query_addrrset (client, &fname, &rdataset, &sigrdataset, dbuf, DNS_SECTION_AUTHORITY);
         ...
         */
        result = dns_name_concatenate (dns_wildcardname, cname, wname, NULL);
        if (result != ISC_R_SUCCESS)
            goto cleanup;

        query_findclosestnsec3 (wname, db, NULL, client, rdataset, sigrdataset, fname, nodata, NULL);
        if (!dns_rdataset_isassociated (rdataset))
            goto cleanup;
        query_addrrset (client, &fname, &rdataset, &sigrdataset, dbuf, DNS_SECTION_AUTHORITY);
    ...
    else if (result == DNS_R_NXDOMAIN)
    {
        ...
        if (result == ISC_R_SUCCESS)
        {
            (void) dns_name_fullcompare (name, fname, &order, &olabels);
            (void) dns_name_fullcompare (name, &nsec.next, &order, &nlabels);
            /*
            ...
            result = dns_name_concatenate (dns_wildcardname, wname, wname, NULL);
            if (result == ISC_R_SUCCESS)
                have_wname = ISC_TRUE;
                      ...
                      dns_rdataset_t ** rdatasetp, dns_rdataset_t ** sigrdatasetp)
{
    ...
    {
        query_addrrset (client, namep, rdatasetp, sigrdatasetp, NULL, DNS_SECTION_AUTHORITY);
        return;
    }
    ...
    /* XXX */
    query_addwildcardproof (client, db, version, client->query.qname, ISC_TRUE, ISC_FALSE);
    ...
    RUNTIME_CHECK (dns_name_concatenate (dns_wildcardname, fname, fname, NULL) == ISC_R_SUCCESS);
    query_addrrset (client, &fname, rdatasetp, sigrdatasetp, dbuf, DNS_SECTION_AUTHORITY);
}
...
static void query_resume (isc_task_t * task, isc_event_t * event)
{
    ...
    else
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            {
                dns_resolver_logfetch (fetch, ns_g_lctx, logcategory, NS_LOGMODULE_QUERY, errorloglevel, ISC_FALSE);
            }
        }
    }
               ...
               dns_name_t * qdomain, dns_rdataset_t * nameservers, isc_boolean_t resuming)
{
    ...
    {
        result = isc_quota_attach (&ns_g_server->recursionquota, &client->recursionquota);
        if (result == ISC_R_SOFTQUOTA)
        {
            ...
            if (now != last)
            {
                ...
                ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                               ...
                               client->recursionquota->used, client->recursionquota->soft, client->recursionquota->max);
            }
        ...
        else if (result == ISC_R_QUOTA)
        {
            ...
            if (now != last)
            {
                ...
                ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                               ...
                               ns_g_server->recursionquota.max, isc_result_totext (result));
            }
            ns_client_killoldestquery (client);
        }
        if (result == ISC_R_SUCCESS && !client->mortal && (client->attributes & NS_CLIENTATTR_TCP) == 0)
        {
            ...
            if (result != ISC_R_SUCCESS)
            {
                ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                               NS_LOGMODULE_QUERY,
                               ISC_LOG_WARNING, "ns_client_replace() failed: %s", isc_result_totext (result));
    ...
    result = dns_resolver_createfetch2 (client->view->resolver,
                                        ...
                                        client->task,
                                        query_resume, client, rdataset, sigrdataset, &client->query.fetch);
...
static inline void rpz_clean (dns_zone_t ** zonep, dns_db_t ** dbp, dns_dbnode_t ** nodep, dns_rdataset_t ** rdatasetp)
{
    ...
    if (zonep != NULL && *zonep != NULL)
        dns_zone_detach (zonep);
    if (rdatasetp != NULL && *rdatasetp != NULL && dns_rdataset_isassociated (*rdatasetp))
...
rpz_rrset_find (ns_client_t * client, dns_rpz_type_t rpz_type,
                ...
                dns_db_t ** dbp, dns_dbversion_t * version, dns_rdataset_t ** rdatasetp, isc_boolean_t resuming)
{
    dns_rpz_st_t *st;
    ...
    if ((st->state & DNS_RPZ_RECURSING) != 0)
    {
        ...
        INSIST (*rdatasetp == NULL || !dns_rdataset_isassociated (*rdatasetp));
        ...
        *rdatasetp = st->r.r_rdataset;
        ...
        if (result == DNS_R_DELEGATION)
        {
            rpz_log_fail (client, DNS_RPZ_ERROR_LEVEL, rpz_type, name, "rpz_rrset_find(1) ", result);
    ...
    else
    {
        ...
        version = NULL;
        zone = NULL;
        result = query_getdb (client, name, type, 0, &zone, dbp, &version, &is_zone);
        if (result != ISC_R_SUCCESS)
        {
            rpz_log_fail (client, DNS_RPZ_ERROR_LEVEL, rpz_type, name, "rpz_rrset_find(2) ", result);
            st->m.policy = DNS_RPZ_POLICY_ERROR;
            if (zone != NULL)
                dns_zone_detach (&zone);
            return (result);
    ...
    result = dns_db_find (*dbp, name, version, type, DNS_DBFIND_GLUEOK, client->now, &node, found, *rdatasetp, NULL);
    if (result == DNS_R_DELEGATION && is_zone && USECACHE (client))
    {
        ...
        result = dns_db_find (*dbp, name, version, dns_rdatatype_ns, 0, client->now, &node, found, *rdatasetp, NULL);
    }
    ...
    if (result == DNS_R_DELEGATION)
    {
        ...
        else
        {
            ...
            result = query_recurse (client, type, st->r_name, NULL, NULL, resuming);
            if (result == ISC_R_SUCCESS)
            {
                st->state |= DNS_RPZ_RECURSING;
 ...
 */
static isc_result_t rpz_rewrite_ip (ns_client_t * client, dns_rdataset_t * rdataset, dns_rpz_type_t rpz_type)
{
    dns_rpz_st_t *st;
    ...
    zone = NULL;
    db = NULL;
    for (rpz = ISC_LIST_HEAD (client->view->rpz_zones); rpz != NULL; rpz = ISC_LIST_NEXT (rpz, link))
    {
        /*
         ...
         */
        version = NULL;
        result = rpz_getdb (client, rpz_type, &rpz->origin, &zone, &db, &version);
        if (result != ISC_R_SUCCESS)
        {
         ...
         * best hit.
         */
        (void) dns_db_rpz_findips (rpz, rpz_type, zone, db, version, rdataset, st, client->query.rpz_st->qname);
        rpz_clean (&zone, &db, NULL, NULL);
    }
    return (ISC_R_SUCCESS);
}
                   ...
                   dns_db_t ** dbp, dns_dbversion_t * version, dns_rdataset_t ** rdatasetp, isc_boolean_t resuming)
{
    ...
    result = rpz_rrset_find (client, rpz_type, name, type, dbp, version, rdatasetp, resuming);
    switch (result)
    {
        case ISC_R_SUCCESS:
        ...
        case DNS_R_CNAME:
        case DNS_R_DNAME:
            rpz_log_fail (client, DNS_RPZ_DEBUG_LEVEL1, rpz_type, name, "NS address rewrite rrset ", result);
            result = ISC_R_SUCCESS;
            break;
        default:
            if (client->query.rpz_st->m.policy != DNS_RPZ_POLICY_ERROR)
            {
                ...
                rpz_log_fail (client, DNS_RPZ_ERROR_LEVEL, rpz_type, name, "NS address rewrite rrset ", result);
            }
            break;
    }
    return (result);
...
rpz_rewrite_rrsets (ns_client_t * client, dns_rpz_type_t rpz_type,
                    dns_name_t * name, dns_rdatatype_t type, dns_rdataset_t ** rdatasetp, isc_boolean_t resuming)
{
    dns_rpz_st_t *st;
    ...
    if ((st->state & DNS_RPZ_DONE_IPv4) == 0 &&
        ((rpz_type == DNS_RPZ_TYPE_NSIP) ?
         ...
         (st->state & DNS_RPZ_HAVE_IP)) != 0 && (type == dns_rdatatype_any || type == dns_rdatatype_a))
    {
        result = rpz_rewrite_rrset (client, rpz_type, dns_rdatatype_a, name, &ipdb, version, rdatasetp, resuming);
        if (result == ISC_R_SUCCESS)
    ...
    if (result == ISC_R_SUCCESS &&
        ((rpz_type == DNS_RPZ_TYPE_NSIP) ?
         ...
         (st->state & DNS_RPZ_HAVE_IP)) != 0 && (type == dns_rdatatype_any || type == dns_rdatatype_aaaa))
    {
        result = rpz_rewrite_rrset (client, rpz_type, dns_rdatatype_aaaa, name, &ipdb, version, rdatasetp, resuming);
    }
    if (ipdb != NULL)
        dns_db_detach (&ipdb);
    return (result);
...
rpz_find (ns_client_t * client, dns_rdatatype_t qtype, dns_name_t * qnamef,
          ...
          dns_dbnode_t ** nodep, dns_rdataset_t ** rdatasetp, dns_rpz_policy_t * policyp)
{
    ...
    *versionp = NULL;
    ...
    dns_fixedname_init (&fixed);
    found = dns_fixedname_name (&fixed);
    result = dns_db_find (*dbp, qnamef, *versionp, dns_rdatatype_any, 0, client->now, nodep, found, *rdatasetp, NULL);
    if (result == ISC_R_SUCCESS)
    {
        dns_rdatasetiter_t *rdsiter;

        rdsiter = NULL;
        result = dns_db_allrdatasets (*dbp, *nodep, *versionp, 0, &rdsiter);
        if (result != ISC_R_SUCCESS)
        {
            dns_db_detachnode (*dbp, nodep);
            rpz_log_fail (client, DNS_RPZ_ERROR_LEVEL, rpz_type, qnamef, "allrdatasets()", result);
            *policyp = DNS_RPZ_POLICY_ERROR;
            return (DNS_R_SERVFAIL);
        }
        for (result = dns_rdatasetiter_first (rdsiter);
             result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (rdsiter))
        {
            dns_rdatasetiter_current (rdsiter, *rdatasetp);
            if ((*rdatasetp)->type == dns_rdatatype_cname || (*rdatasetp)->type == qtype)
                break;
            dns_rdataset_disassociate (*rdatasetp);
        }
        ...
        if (result != ISC_R_SUCCESS)
        {
            if (result != ISC_R_NOMORE)
            {
                rpz_log_fail (client, DNS_RPZ_ERROR_LEVEL, rpz_type, qnamef, "rdatasetiter", result);
                *policyp = DNS_RPZ_POLICY_ERROR;
                return (DNS_R_SERVFAIL);
            }
            /*
            ...
            if (qtype == dns_rdatatype_rrsig || qtype == dns_rdatatype_sig)
                ...
                result = dns_db_find (*dbp, qnamef, *versionp, qtype, 0, client->now, nodep, found, *rdatasetp, NULL);
        }
    }
    switch (result)
    {
        case ISC_R_SUCCESS:
            ...
            else
            {
                ...
                if ((policy == DNS_RPZ_POLICY_RECORD ||
                     policy == DNS_RPZ_POLICY_WILDCNAME) && qtype != dns_rdatatype_cname && qtype != dns_rdatatype_any)
                    result = DNS_R_CNAME;
            }
            break;
        case DNS_R_DNAME:
        ...
        default:
            dns_db_detach (dbp);
            dns_zone_detach (zonep);
            rpz_log_fail (client, DNS_RPZ_ERROR_LEVEL, rpz_type, qnamef, "", result);
                  ...
                  dns_rpz_type_t rpz_type, dns_rdataset_t ** rdatasetp)
{
    ...
    for (rpz = ISC_LIST_HEAD (client->view->rpz_zones); rpz != NULL; rpz = ISC_LIST_NEXT (rpz, link))
    {
        ...
        for (;;)
        {
            result = dns_name_concatenate (prefix, suffix, rpz_qname, NULL);
            if (result == ISC_R_SUCCESS)
                break;
            ...
            if (labels < 2)
            {
                rpz_log_fail (client, DNS_RPZ_ERROR_LEVEL, rpz_type, suffix, "concatentate() ", result);
                return (ISC_R_SUCCESS);
            }
            if (labels + 1 == dns_name_countlabels (qname))
            {
                rpz_log_fail (client, DNS_RPZ_DEBUG_LEVEL1, rpz_type, suffix, "concatentate() ", result);
            }
         ...
         */
        result = rpz_find (client, qtype, rpz_qname, qname, rpz_type, &zone, &db, &version, &node, rdatasetp, &policy);
        switch (result)
        {
            case DNS_R_NXDOMAIN:
            case DNS_R_EMPTYNAME:
            ...
            default:
                ...
                if (st->m.policy != DNS_RPZ_POLICY_MISS &&
                    ...
                    (st->m.type < rpz_type || (st->m.type == rpz_type && 0 >= dns_name_compare (rpz_qname, st->qname))))
                    continue;
                ...
                {
                    rpz_log_rewrite (client, "disabled ", policy, rpz_type, rpz_qname);
                    continue;
                }
...
static void
rpz_rewrite_ns_skip (ns_client_t * client, dns_name_t * nsname, isc_result_t result, int level, const char *str)
{
    dns_rpz_st_t *st;
    ...
    if (str != NULL)
        rpz_log_fail (client, level, DNS_RPZ_TYPE_NSIP, nsname, str, result);
    if (st->r.ns_rdataset != NULL && dns_rdataset_isassociated (st->r.ns_rdataset))
...
rpz_rewrite (ns_client_t * client, dns_rdatatype_t qtype, isc_result_t qresult, isc_boolean_t resuming)
{
    ...
    switch (qresult)
    {
        ...
        case DNS_R_BROKENCHAIN:
            rpz_log_fail (client, DNS_RPZ_DEBUG_LEVEL3, DNS_RPZ_TYPE_QNAME,
                          client->query.qname, "stop on qresult in rpz_rewrite()", qresult);
            return (ISC_R_SUCCESS);
        default:
            rpz_log_fail (client, DNS_RPZ_DEBUG_LEVEL1, DNS_RPZ_TYPE_QNAME,
                          client->query.qname, "stop on unrecognized qresult in rpz_rewrite()", qresult);
            return (ISC_R_SUCCESS);
    }
    ...
    if ((st->state & DNS_RPZ_DONE_QNAME) == 0)
    {
         ...
         */
        result = rpz_rewrite_name (client, qtype, client->query.qname, DNS_RPZ_TYPE_QNAME, &rdataset);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
     ...
     */
    if ((st->state & DNS_RPZ_DONE_QNAME_IP) == 0 && (st->state & DNS_RPZ_HAVE_IP) != 0 && ck_ip)
    {
        result = rpz_rewrite_rrsets (client, DNS_RPZ_TYPE_IP, client->query.qname, qtype, &rdataset, resuming);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
        st->state &= ~DNS_RPZ_DONE_IPv4;
     ...
     */
    if ((st->state & (DNS_RPZ_HAVE_NSIPv4 | DNS_RPZ_HAVE_NSIPv6 | DNS_RPZ_HAVE_NSDNAME)) == 0)
    {
        result = ISC_R_SUCCESS;
        goto cleanup;
    }
    ...
    while (st->r.label > 1)
    {
        ...
        else
        {
            ...
            dns_name_split (client->query.qname, st->r.label, NULL, nsname);
        }
        if (st->r.ns_rdataset == NULL || !dns_rdataset_isassociated (st->r.ns_rdataset))
        {
            dns_db_t *db = NULL;
            ...
            result = rpz_rrset_find (client, DNS_RPZ_TYPE_NSDNAME,
                                     nsname, dns_rdatatype_ns, &db, NULL, &st->r.ns_rdataset, resuming);
            if (db != NULL)
                dns_db_detach (&db);
            ...
            switch (result)
            {
                ...
                case DNS_R_CNAME:
                case DNS_R_DNAME:
                    rpz_rewrite_ns_skip (client, nsname, result, 0, NULL);
                    continue;
                case ISC_R_TIMEDOUT:
                case DNS_R_BROKENCHAIN:
                case ISC_R_FAILURE:
                    rpz_rewrite_ns_skip (client, nsname, result, DNS_RPZ_DEBUG_LEVEL3, "NS db_find() ");
                    continue;
                default:
                    rpz_rewrite_ns_skip (client, nsname, result, DNS_RPZ_INFO_LEVEL, "unrecognized NS db_find() ");
                    continue;
            }
        }
        /*
        ...
        do
        {
            ...
            if (result != ISC_R_SUCCESS)
            {
                rpz_log_fail (client, DNS_RPZ_ERROR_LEVEL, DNS_RPZ_TYPE_NSIP, nsname, "rdata_tostruct() ", result);
                st->m.policy = DNS_RPZ_POLICY_ERROR;
                goto cleanup;
            }
            /*
             ...
             */
            if ((st->state & DNS_RPZ_DONE_NSDNAME) == 0 && (st->state & DNS_RPZ_HAVE_NSDNAME) != 0)
            {
                result = rpz_rewrite_name (client, qtype, &ns.name, DNS_RPZ_TYPE_NSDNAME, &rdataset);
                if (result != ISC_R_SUCCESS)
                {
                    dns_rdata_freestruct (&ns);
                    goto cleanup;
             ...
             */
            result = rpz_rewrite_rrsets (client, DNS_RPZ_TYPE_NSIP, &ns.name, dns_rdatatype_any, &rdataset, resuming);
            dns_rdata_freestruct (&ns);
            if (result != ISC_R_SUCCESS)
                goto cleanup;
            st->state &= ~(DNS_RPZ_DONE_NSDNAME | DNS_RPZ_DONE_IPv4);
  ...
  cleanup:
    if (st->m.policy != DNS_RPZ_POLICY_MISS &&
        st->m.policy != DNS_RPZ_POLICY_ERROR && st->m.rpz->policy != DNS_RPZ_POLICY_GIVEN)
        st->m.policy = st->m.rpz->policy;
    if (st->m.policy == DNS_RPZ_POLICY_MISS ||
        st->m.policy == DNS_RPZ_POLICY_PASSTHRU || st->m.policy == DNS_RPZ_POLICY_ERROR)
    {
...
rpz_add_cname (ns_client_t * client, dns_rpz_st_t * st, dns_name_t * cname, dns_name_t * fname, isc_buffer_t * dbuf)
{
    ...
    if (labels > 2 && dns_name_iswildcard (cname))
    {
        ...
        dns_name_split (client->query.qname, 1, dns_fixedname_name (&prefix), NULL);
        dns_fixedname_init (&suffix);
        dns_name_split (cname, labels - 1, NULL, dns_fixedname_name (&suffix));
        result = dns_name_concatenate (dns_fixedname_name (&prefix), dns_fixedname_name (&suffix), fname, NULL);
    ...
    if (result != ISC_R_SUCCESS)
        return (result);
    query_keepname (client, fname, dbuf);
    result = query_add_cname (client, client->query.qname, fname, dns_trust_authanswer, st->m.ttl);
    if (result != ISC_R_SUCCESS)
        return (result);
    rpz_log_rewrite (client, "", st->m.policy, st->m.type, st->qname);
...
static void setup_query_sortlist (ns_client_t * client)
{
    ...
    isc_netaddr_fromsockaddr (&netaddr, &client->peeraddr);
    switch (ns_sortlist_setup (client->view->sortlist, &netaddr, &order_arg))
    {
...
static void query_addnoqnameproof (ns_client_t * client, dns_rdataset_t * rdataset)
{
    ...
    query_addrrset (client, &fname, &neg, &negsig, dbuf, DNS_SECTION_AUTHORITY);
...
static inline void answer_in_glue (ns_client_t * client, dns_rdatatype_t qtype)
{
    dns_name_t *name;
    ...
    msg = client->message;
    for (name = ISC_LIST_HEAD (msg->sections[section]); name != NULL; name = ISC_LIST_NEXT (name, link))
...
static void warn_rfc1918 (ns_client_t * client, dns_name_t * fname, dns_rdataset_t * rdataset)
{
    ...
    for (i = 0; i < (sizeof (rfc1918names) / sizeof (*rfc1918names)); i++)
    {
        if (dns_name_issubdomain (fname, &rfc1918names[i]))
        {
            ...
            result = dns_ncache_getrdataset (rdataset, &rfc1918names[i], dns_rdatatype_soa, &found);
            if (result != ISC_R_SUCCESS)
                return;
            ...
            result = dns_rdata_tostruct (&rdata, &soa, NULL);
            RUNTIME_CHECK (result == ISC_R_SUCCESS);
            if (dns_name_equal (&soa.origin, &prisoner) && dns_name_equal (&soa.contact, &hostmaster))
            {
                char buf[DNS_NAME_FORMATSIZE];

                dns_name_format (fname, buf, sizeof (buf));
                ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                               NS_LOGMODULE_QUERY, ISC_LOG_WARNING, "RFC 1918 response from " "Internet for %s", buf);
            }
                        ...
                        dns_name_t * fname, isc_boolean_t exact, dns_name_t * found)
{
    ...
    result = dns_db_getnsec3parameters (db, version, &hash, NULL, &iterations, salt, &salt_length);
    if (result != ISC_R_SUCCESS)
        return;
  ...
  again:
    dns_fixedname_init (&fixed);
    result = dns_nsec3_hashname (&fixed, NULL, NULL, &name, dns_db_origin (db), hash, iterations, salt, salt_length);
    if (result != ISC_R_SUCCESS)
        return;

    dboptions = client->query.dboptions | DNS_DBFIND_FORCENSEC3;
    result = dns_db_find (db, dns_fixedname_name (&fixed), version,
                          dns_rdatatype_nsec3, dboptions, client->now, NULL, fname, rdataset, sigrdataset);
    ...
    if (result == DNS_R_NXDOMAIN)
    {
        ...
        if (found != NULL && optout &&
            dns_name_fullcompare (&name, dns_db_origin (db), &order, &count) == dns_namereln_subdomain)
        {
            dns_rdataset_disassociate (rdataset);
            if (dns_rdataset_isassociated (sigrdataset))
                dns_rdataset_disassociate (sigrdataset);
            ...
            ns_client_log (client, DNS_LOGCATEGORY_DNSSEC,
                           NS_LOGMODULE_QUERY, ISC_LOG_DEBUG (3), "looking for closest provable encloser");
            goto again;
        }
        if (exact)
            ns_client_log (client, DNS_LOGCATEGORY_DNSSEC,
    ...
    else if (result != ISC_R_SUCCESS)
    {
        ...
        ns_client_log (client, DNS_LOGCATEGORY_DNSSEC,
                       NS_LOGMODULE_QUERY, ISC_LOG_WARNING, "expected covering NSEC3, got an exact match");
    if (found != NULL)
...
#ifdef ALLOW_FILTER_AAAA_ON_V4
static isc_boolean_t is_v4_client (ns_client_t * client)
{
    if (isc_sockaddr_pf (&client->peeraddr) == AF_INET)
        return (ISC_TRUE);
    if (isc_sockaddr_pf (&client->peeraddr) == AF_INET6 && IN6_IS_ADDR_V4MAPPED (&client->peeraddr.type.sin6.sin6_addr))
        return (ISC_TRUE);
    return (ISC_FALSE);
}
#endif
...
static isc_uint32_t dns64_ttl (dns_db_t * db, dns_dbversion_t * version)
{
    ...
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    dns_rdataset_init (&rdataset);
    result = dns_db_findrdataset (db, node, version, dns_rdatatype_soa, 0, 0, &rdataset, NULL);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    result = dns_rdataset_first (&rdataset);
    if (result != ISC_R_SUCCESS)
...
static isc_boolean_t dns64_aaaaok (ns_client_t * client, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    ...
    isc_netaddr_fromsockaddr (&netaddr, &client->peeraddr);
    if (dns_dns64_aaaaok (dns64, &netaddr, client->signer, &ns_g_server->aclenv, flags, rdataset, aaaaok, count))
    {
        for (i = 0; i < count; i++)
        {
...
static isc_result_t query_find (ns_client_t * client, dns_fetchevent_t * event, dns_rdatatype_t qtype)
{
  ...
  restart:
    ...
    if (client->view->checknames &&
        !dns_rdata_checkowner (client->query.qname, client->message->rdclass, qtype, ISC_FALSE))
    {
        ...
        dns_rdataclass_format (client->message->rdclass, classname, sizeof (classname));
        ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                       NS_LOGMODULE_QUERY, ISC_LOG_ERROR, "check-names failure %s/%s/%s", namebuf, typename, classname);
    ...
    if (dns_rdatatype_atparent (qtype) && !dns_name_equal (client->query.qname, dns_rootname))
        ...
        (options & DNS_GETDB_NOEXACT) != 0 && qtype == dns_rdatatype_ds)
    {
        ...
        tresult = query_getzonedb (client, client->query.qname, qtype, DNS_GETDB_PARTIAL, &tzone, &tdb, &tversion);
        if (tresult == ISC_R_SUCCESS)
        {
            options &= ~DNS_GETDB_NOEXACT;
            query_putrdataset (client, &rdataset);
    ...
    if (result != ISC_R_SUCCESS)
    {
        if (result == DNS_R_REFUSED)
        {
            ...
            {
                inc_stats (client, dns_nsstatscounter_recurserej);
            }
            else
  ...
  resume:
    ...
    if (!ISC_LIST_EMPTY (client->view->rpz_zones) &&
        ...
        (client->query.rpz_st == NULL ||
         (client->query.rpz_st->state & DNS_RPZ_REWRITTEN) == 0) && !dns_name_equal (client->query.qname, dns_rootname))
    {
        ...
        if (rpz_st->m.policy != DNS_RPZ_POLICY_MISS &&
            rpz_st->m.policy != DNS_RPZ_POLICY_PASSTHRU && rpz_st->m.policy != DNS_RPZ_POLICY_ERROR)
        {
            if (rpz_st->m.type == DNS_RPZ_TYPE_QNAME)
            {
                result = dns_name_copy (client->query.qname, fname, NULL);
                RUNTIME_CHECK (result == ISC_R_SUCCESS);
            }
            ...
            else if (rdataset != NULL && dns_rdataset_isassociated (rdataset))
            {
                dns_rdataset_disassociate (rdataset);
            }
            node = rpz_st->m.node;
            ...
            switch (rpz_st->m.policy)
            {
                ...
                case DNS_RPZ_POLICY_RECORD:
                    ...
                    if (type == dns_rdatatype_any && result != DNS_R_CNAME && dns_rdataset_isassociated (rdataset))
                        dns_rdataset_disassociate (rdataset);
                    break;
                case DNS_RPZ_POLICY_WILDCNAME:
                    ...
                    RUNTIME_CHECK (result == ISC_R_SUCCESS);
                    dns_rdata_reset (&rdata);
                    result = rpz_add_cname (client, rpz_st, &cname.cname, fname, dbuf);
                    if (result != ISC_R_SUCCESS)
                        goto cleanup;
                    fname = NULL;
                ...
                case DNS_RPZ_POLICY_CNAME:
                     ...
                     */
                    result = rpz_add_cname (client, rpz_st, &rpz_st->m.rpz->cname, fname, dbuf);
                    if (result != ISC_R_SUCCESS)
                        goto cleanup;
                    fname = NULL;
                    want_restart = ISC_TRUE;
            ...
            query_putrdataset (client, &sigrdataset);
            is_zone = ISC_TRUE;
            rpz_log_rewrite (client, "", rpz_st->m.policy, rpz_st->m.type, rpz_st->qname);
        }
    }
    ...
    switch (result)
    {
        ...
        case ISC_R_NOTFOUND:
            ...
            else
            {
                ...
                result = dns_db_find (db, dns_rootname,
                                      NULL, dns_rdatatype_ns, 0, client->now, &node, fname, rdataset, sigrdataset);
            }
            if (result != ISC_R_SUCCESS)
            {
                /*
                ...
                if (RECURSIONOK (client))
                {
                    result = query_recurse (client, qtype, client->query.qname, NULL, NULL, resuming);
                    if (result == ISC_R_SUCCESS)
                    {
                        client->query.attributes |= NS_QUERYATTR_RECURSING;
                        if (dns64)
        ...
        case DNS_R_DELEGATION:
            ...
            if (is_zone)
            {
                 ...
                 * child zone if the query type is DS.
                 */
                if (!RECURSIONOK (client) && (options & DNS_GETDB_NOEXACT) != 0 && qtype == dns_rdatatype_ds)
                {
                    dns_db_t *tdb = NULL;
                    ...
                    result = query_getzonedb (client,
                                              client->query.qname, qtype, DNS_GETDB_PARTIAL, &tzone, &tdb, &tversion);
                    if (result == ISC_R_SUCCESS)
                    {
                        options &= ~DNS_GETDB_NOEXACT;
                        query_putrdataset (client, &rdataset);
                ...
                if (!USECACHE (client) || !RECURSIONOK (client))
                {
                    ...
                    dns_fixedname_init (&fixed);
                    dns_name_copy (fname, dns_fixedname_name (&fixed), NULL);
                    ...
                    if (sigrdataset != NULL)
                        ...
                        sigrdatasetp = NULL;
                    query_addrrset (client, &fname, &rdataset, sigrdatasetp, dbuf, DNS_SECTION_AUTHORITY);
                    client->query.gluedb = NULL;
                    if (WANTDNSSEC (client))
                        query_addds (client, db, node, version, dns_fixedname_name (&fixed));
                }
                else
                {
                    /*
            ...
            {
                if (zfname != NULL &&
                    (!dns_name_issubdomain (fname, zfname) || (is_staticstub_zone && dns_name_equal (fname, zfname))))
                {
                    /*
                ...
                if (RECURSIONOK (client))
                {
                    ...
                    if (dns_rdatatype_atparent (type))
                        ...
                        result = query_recurse (client, dns_rdatatype_a, client->query.qname, NULL, NULL, resuming);
                    else
                        result = query_recurse (client, qtype, client->query.qname, fname, rdataset, resuming);
                ...
                else
                {
                     ...
                     */
                    client->query.attributes |= NS_QUERYATTR_CACHEGLUEOK;
        ...
        case DNS_R_NXRRSET:
          iszone_nxrrset:
...
#endif
            {
                ...
                if (dns64_excluded)
                    break;
                     ...
                     client->message->rdclass == dns_rdataclass_in && qtype == dns_rdatatype_aaaa)
            {
                /*
             ...
             */
          nxrrset_rrsig:
            if (!dns_rdataset_isassociated (rdataset) && WANTDNSSEC (client))
            {
                if ((fname->attributes & DNS_NAMEATTR_WILDCARD) == 0)
                {
                    ...
                    query_findclosestnsec3 (qname, db, version, client, rdataset, sigrdataset, fname, ISC_TRUE, found);
                    /*
                     * Did we find the closest provable encloser
                     * instead? If so add the nearest to the
                     * closest provable encloser.
                     */
                    if (dns_rdataset_isassociated (rdataset) && !dns_name_equal (qname, found))
                    {
                        unsigned int count;
                        ...
                        dns_name_getlabelsequence (qname, skip, count, found);
                        ...
                        query_findclosestnsec3 (found, db,
                                                version, client, rdataset, sigrdataset, fname, ISC_FALSE, NULL);
                    }
                }
                else
                {
                    query_releasename (client, &fname);
                    query_addwildcardproof (client, db, version, client->query.qname, ISC_FALSE, ISC_TRUE);
                }
            }
            if (dns_rdataset_isassociated (rdataset))
            {
             ...
             * Add SOA.
             */
            result = query_addsoa (client, db, version, ISC_UINT32_MAX, dns_rdataset_isassociated (rdataset));
            if (result != ISC_R_SUCCESS)
            {
                QUERY_ERROR (result);
                goto cleanup;
            ...
            {
                if (dns_rdataset_isassociated (rdataset))
                    query_addnxrrsetnsec (client, db, version, &fname, &rdataset, &sigrdataset);
            }
            goto cleanup;
        ...
        case DNS_R_NXDOMAIN:
             ...
             */
            if (qtype == dns_rdatatype_soa && zone != NULL && dns_zone_getzeronosoattl (zone))
                result = query_addsoa (client, db, version, 0, dns_rdataset_isassociated (rdataset));
            else
                result = query_addsoa (client, db, version, ISC_UINT32_MAX, dns_rdataset_isassociated (rdataset));
            if (result != ISC_R_SUCCESS)
            {
            ...
            if (WANTDNSSEC (client))
            {
                 ...
                 */
                if (dns_rdataset_isassociated (rdataset))
                    query_addrrset (client, &fname, &rdataset, &sigrdataset, NULL, DNS_SECTION_AUTHORITY);
                query_addwildcardproof (client, db, version, client->query.qname, ISC_FALSE, ISC_FALSE);
            }
        ...
        case DNS_R_NCACHENXRRSET:
          ncache_nxrrset:
            ...
            if (result == DNS_R_NCACHENXDOMAIN &&
                ...
                client->message->rdclass == dns_rdataclass_in && dns_name_countlabels (fname) == 7)
             ...
             */
            query_keepname (client, fname, dbuf);
            dns_message_addname (client->message, fname, DNS_SECTION_AUTHORITY);
            ISC_LIST_APPEND (fname->list, rdataset, link);
            fname = NULL;
            rdataset = NULL;
            goto cleanup;
        ...
        case DNS_R_CNAME:
            ...
            if (sigrdataset != NULL)
                ...
                sigrdatasetp = NULL;
            if (WANTDNSSEC (client) && (fname->attributes & DNS_NAMEATTR_WILDCARD) != 0)
            {
            ...
            if (NOQNAME (rdataset) && WANTDNSSEC (client))
                ...
                noqname = NULL;
            query_addrrset (client, &fname, &rdataset, sigrdatasetp, dbuf, DNS_SECTION_ANSWER);
            if (noqname != NULL)
        ...
        case DNS_R_DNAME:
             ...
             */
            namereln = dns_name_fullcompare (client->query.qname, fname, &order, &nlabels);
             ...
             */
            result = query_add_cname (client, client->query.qname, fname, trdataset->trust, trdataset->ttl);
            if (result != ISC_R_SUCCESS)
                goto cleanup;
            /*
    ...
    if (client->view->v4_aaaa != dns_v4_aaaa_ok &&
        is_v4_client (client) &&
        ns_client_checkaclsilent (client, NULL, client->view->v4_aaaa_acl, ISC_TRUE) == ISC_R_SUCCESS)
    ...
    if (type == dns_rdatatype_any)
    {
        ...
        while (result == ISC_R_SUCCESS)
        {
            ...
            if (is_zone && qtype == dns_rdatatype_any &&
                !dns_db_issecure (db) && dns_rdatatype_isdnssec (rdataset->type))
            {
                /*
            ...
            else if ((qtype == dns_rdatatype_any || rdataset->type == qtype) && rdataset->type != 0)
            {
#ifdef ALLOW_FILTER_AAAA_ON_V4
                ...
                if (NOQNAME (rdataset) && WANTDNSSEC (client))
                    ...
                    noqname = NULL;
                query_addrrset (client, fname != NULL ? &fname : &tname, &rdataset, NULL, NULL, DNS_SECTION_ANSWER);
                if (noqname != NULL)
                    query_addnoqnameproof (client, noqname);
                n++;
                INSIST (tname != NULL);
        ...
        else if (client->filter_aaaa != dns_v4_aaaa_ok && have_aaaa && have_a && (!have_sig || !WANTDNSSEC (client)))
            client->attributes |= NS_CLIENTATTR_FILTER_AAAA;
#endif
        if (fname != NULL)
        ...
        if (n == 0)
        {
             ...
             */
            if ((qtype == dns_rdatatype_rrsig || qtype == dns_rdatatype_sig) && result == ISC_R_NOMORE)
            {
                if (!is_zone)
                {
                    authoritative = ISC_FALSE;
                ...
                if (dns_db_issecure (db))
                {
                    ...
                    ns_client_log (client,
                                   ...
                                   NS_LOGMODULE_QUERY, ISC_LOG_WARNING, "missing signature " "for %s", namebuf);
                }
    ...
    else
    {
             ...
             (!WANTDNSSEC (client) || sigrdataset == NULL || !dns_rdataset_isassociated (sigrdataset))))
        {
            if (qtype == dns_rdatatype_aaaa)
            {
                ...
                result = dns_db_findrdataset (db, node, version, dns_rdatatype_a, 0, client->now, trdataset, NULL);
                ...
                if (result == ISC_R_SUCCESS)
                {
                    client->attributes |= NS_CLIENTATTR_FILTER_AAAA;
                         ...
                         !RECURSIONOK (client) || (result != DNS_R_DELEGATION && result != ISC_R_NOTFOUND))
                {
                    client->attributes &= ~NS_CLIENTATTR_FILTER_AAAA;
                }
                else
                {
                    ...
                    if (result == ISC_R_SUCCESS)
                    {
                        client->attributes |= NS_CLIENTATTR_FILTER_AAAA_RC;
                        client->query.attributes |= NS_QUERYATTR_RECURSING;
                    }
                }

            }
            else if (qtype == dns_rdatatype_a && (client->attributes & NS_CLIENTATTR_FILTER_AAAA_RC) != 0)
            {
                client->attributes &= ~NS_CLIENTATTR_FILTER_AAAA_RC;
                client->attributes |= NS_CLIENTATTR_FILTER_AAAA;
                dns_rdataset_disassociate (rdataset);
        ...
        if (qtype == dns_rdatatype_aaaa && !dns64_exclude &&
            !ISC_LIST_EMPTY (client->view->dns64) &&
            client->message->rdclass == dns_rdataclass_in && !dns64_aaaaok (client, rdataset, sigrdataset))
        {
            /*
             * Look to see if there are A records for this
             * name.
         ...
         */
        if (is_zone && qtype == dns_rdatatype_ns && dns_name_equal (client->query.qname, dns_rootname))
            client->query.attributes &= ~NS_QUERYATTR_NOADDITIONAL;

        if (dns64)
        {
            qtype = type = dns_rdatatype_aaaa;
            result = query_dns64 (client, &fname, rdataset, sigrdataset, dbuf, DNS_SECTION_ANSWER);
            dns_rdataset_disassociate (rdataset);
            dns_message_puttemprdataset (client->message, &rdataset);
            if (result == ISC_R_NOMORE)
            {
                ...
                if (dns64_exclude)
                {
                     ...
                     */
                    (void) query_addsoa (client, db, version, 600, ISC_FALSE);
                    goto cleanup;
                }
#endif
                if (is_zone)
        ...
        {
            query_filter64 (client, &fname, rdataset, dbuf, DNS_SECTION_ANSWER);
            query_putrdataset (client, &rdataset);
        }
        else
            query_addrrset (client, &fname, &rdataset, sigrdatasetp, dbuf, DNS_SECTION_ANSWER);
  ...
  addauth:
    ...
    if (!want_restart && !NOAUTHORITY (client))
    {
        if (is_zone)
        {
            if (!((qtype == dns_rdatatype_ns ||
                   qtype == dns_rdatatype_any) && dns_name_equal (client->query.qname, dns_db_origin (db))))
    ...
    if (need_wildcardproof && dns_db_issecure (db))
        query_addwildcardproof (client, db, version, dns_fixedname_name (&wildcardname), ISC_TRUE, ISC_FALSE);
  cleanup:
    ...
    if (rpz_st != NULL && (rpz_st->state & DNS_RPZ_RECURSING) == 0)
    {
        rpz_clean (&rpz_st->m.zone, &rpz_st->m.db, &rpz_st->m.node, &rpz_st->m.rdataset);
    ...
    if (eresult != ISC_R_SUCCESS && (!PARTIALANSWER (client) || WANTRECURSION (client)))
    {
    ...
    else if (!RECURSING (client))
    {
         ...
         */
        if (ISC_LIST_EMPTY (client->message->sections[DNS_SECTION_ANSWER]) &&
            client->message->rcode == dns_rcode_noerror && (qtype == dns_rdatatype_a || qtype == dns_rdatatype_aaaa))
            answer_in_glue (client, qtype);

        if (client->message->rcode == dns_rcode_nxdomain && client->view->auth_nxdomain == ISC_TRUE)
            client->message->flags |= DNS_MESSAGEFLAG_AA;
...
static inline void log_query (ns_client_t * client, unsigned int flags, unsigned int extflags)
{
    char namebuf[DNS_NAME_FORMATSIZE];
    ...
    ns_client_log (client, NS_LOGCATEGORY_QUERIES, NS_LOGMODULE_QUERY,
                   ...
                   ((flags & DNS_MESSAGEFLAG_CD) != 0) ? "C" : "", onbuf);
}
...
void ns_query_start (ns_client_t * client)
{
    ...
    if (client->view->minimalresponses)
        client->query.attributes |= (NS_QUERYATTR_NOAUTHORITY | NS_QUERYATTR_NOADDITIONAL);
    ...
    if ((client->view->cachedb == NULL) || (!client->view->additionalfromcache))
    {
         ...
         * recursion.
         */
        client->query.attributes &= ~(NS_QUERYATTR_RECURSIONOK | NS_QUERYATTR_CACHEOK);
    }
    else if ((client->attributes & NS_CLIENTATTR_RA) == 0 || (message->flags & DNS_MESSAGEFLAG_RD) == 0)
    {
        /*
    ...
    if (dns_rdatatype_ismeta (qtype))
    {
        switch (qtype)
        {
            ...
            case dns_rdatatype_tkey:
                result = dns_tkey_processquery (client->message, ns_g_server->tkeyctx, client->view->dynamickeys);
                if (result == ISC_R_SUCCESS)
                    query_send (client);
                else
                    query_error (client, result, __LINE__);
     ...
     */
    if (client->opt != NULL && client->udpsize <= 512U && (client->attributes & NS_CLIENTATTR_TCP) == 0)
        client->query.attributes |= (NS_QUERYATTR_NOAUTHORITY | NS_QUERYATTR_NOADDITIONAL);
     ...
     */
    if (message->flags & DNS_MESSAGEFLAG_CD || qtype == dns_rdatatype_rrsig)
    {
