...
static isc_boolean_t command_compare (const char *text, const char *command)
{
    ...
    if (strncasecmp (text, command, commandlen) == 0 &&
        (text[commandlen] == '\0' || text[commandlen] == ' ' || text[commandlen] == '\t'))
        return (ISC_TRUE);
    return (ISC_FALSE);
}
...
isc_result_t ns_control_docommand (isccc_sexpr_t * message, isc_buffer_t * text)
{
     ...
     */
    if (command_compare (command, NS_COMMAND_NULL) || command_compare (command, NS_COMMAND_STATUS))
    {
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   NS_LOGMODULE_CONTROL, log_level, "received control channel command '%s'", command);
    ...
    {
        result = ns_server_freeze (ns_g_server, ISC_TRUE, command, text);
    }
    else if (command_compare (command, NS_COMMAND_UNFREEZE) || command_compare (command, NS_COMMAND_THAW))
    {
        result = ns_server_freeze (ns_g_server, ISC_FALSE, command, text);
    }
    ...
    else if (command_compare (command, NS_COMMAND_SIGN) || command_compare (command, NS_COMMAND_LOADKEYS))
    {
    ...
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_CONTROL, ISC_LOG_WARNING, "unknown control channel command '%s'", command);
