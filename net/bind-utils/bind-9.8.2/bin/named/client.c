...
void ns_client_settimeout (ns_client_t * client, unsigned int seconds)
{
    ...
    result = isc_timer_reset (client->timer, isc_timertype_once, NULL, &interval, ISC_FALSE);
    client->timerset = ISC_TRUE;
    if (result != ISC_R_SUCCESS)
    {
        ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                       NS_LOGMODULE_CLIENT, ISC_LOG_ERROR, "setting timeout: %s", isc_result_totext (result));
        /* Continue anyway. */
    }
}
...
static isc_boolean_t exit_check (ns_client_t * client)
{
     ...
     */
    if (client->nupdates == 0 && client->newstate == NS_CLIENTSTATE_FREED && client->view != NULL)
    ...
    if (client->state == NS_CLIENTSTATE_WORKING)
    {
        ...
        if (!(client->nsends == 0 && client->nrecvs == 0 && client->references == 0))
        {
            /*
    ...
    if (client->state == NS_CLIENTSTATE_READING)
    {
        ...
        if (client->timerset)
        {
            (void) isc_timer_reset (client->timer, isc_timertype_inactive, NULL, NULL, ISC_TRUE);
    ...
    if (client->state == NS_CLIENTSTATE_READY)
    {
        ...
        if (client->naccepts > 0)
            isc_socket_cancel (client->tcplistener, client->task, ISC_SOCKCANCEL_ACCEPT);
        ...
        if (client->nrecvs > 0)
            isc_socket_cancel (client->udpsocket, client->task, ISC_SOCKCANCEL_RECV);
        if (!(client->nrecvs == 0))
        {
    ...
    if (client->state == NS_CLIENTSTATE_INACTIVE)
    {
        ...
        if (client->manager != NULL)
        {
            ...
            if (manager->exiting &&
                ...
                ISC_LIST_EMPTY (manager->inactive) && ISC_LIST_EMPTY (manager->recursing))
...
static void ns_client_checkactive (ns_client_t * client)
{
    if (client->mortal)
    {
        ...
        if (TCP_CLIENT (client) && !ns_g_clienttest)
        {
            ...
            if (client->interface->ntcpcurrent < client->interface->ntcptarget)
...
void ns_client_next (ns_client_t * client, isc_result_t result)
{
    ...
    REQUIRE (NS_CLIENT_VALID (client));
    REQUIRE (client->state == NS_CLIENTSTATE_WORKING || client->state == NS_CLIENTSTATE_READING);

    CTRACE ("next");

    if (result != ISC_R_SUCCESS)
        ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                       NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (3), "request failed: %s", isc_result_totext (result));
...
static void client_senddone (isc_task_t * task, isc_event_t * event)
{
    ...
    if (sevent->result != ISC_R_SUCCESS)
        ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                       ...
                       "error sending response: %s", isc_result_totext (sevent->result));
...
client_allocsendbuf (ns_client_t * client, isc_buffer_t * buffer,
                     isc_buffer_t * tcpbuffer, isc_uint32_t length, unsigned char *sendbuf, unsigned char **datap)
{
    unsigned char *data;
    ...
    INSIST (datap != NULL);
    INSIST ((tcpbuffer == NULL && length != 0) || (tcpbuffer != NULL && length == 0));
...
static isc_result_t client_sendpkg (ns_client_t * client, isc_buffer_t * buffer)
{
    ...
    else
    {
        ...
        if (ns_g_server->blackholeacl != NULL &&
            dns_acl_match (&netaddr, NULL,
                           ns_g_server->blackholeacl, &ns_g_server->aclenv, &match, NULL) == ISC_R_SUCCESS && match > 0)
            return (DNS_R_BLACKHOLED);
        sockflags |= ISC_SOCKFLAG_NORETRY;
    }

    if ((client->attributes & NS_CLIENTATTR_PKTINFO) != 0 && (client->attributes & NS_CLIENTATTR_MULTICAST) == 0)
    ...
    if (result == ISC_R_SUCCESS || result == ISC_R_INPROGRESS)
    {
        client->nsends++;
        if (result == ISC_R_SUCCESS)
            client_senddone (client->task, (isc_event_t *) client->sendevent);
        result = ISC_R_SUCCESS;
    }
    return (result);
}
...
void ns_client_sendraw (ns_client_t * client, dns_message_t * message)
{
    ...
    result = client_allocsendbuf (client, &buffer, NULL, mr->length, sendbuf, &data);
    if (result != ISC_R_SUCCESS)
        goto done;
...
void ns_client_send (ns_client_t * client)
{
    ...
    if (result != ISC_R_SUCCESS)
        goto done;
    result = dns_message_rendersection (client->message, DNS_SECTION_ANSWER, DNS_MESSAGERENDER_PARTIAL | render_opts);
    if (result == ISC_R_NOSPACE)
    {
        client->message->flags |= DNS_MESSAGEFLAG_TC;
        goto renderend;
    ...
    result = dns_message_rendersection (client->message,
                                        DNS_SECTION_AUTHORITY, DNS_MESSAGERENDER_PARTIAL | render_opts);
    if (result == ISC_R_NOSPACE)
    {
        client->message->flags |= DNS_MESSAGEFLAG_TC;
        goto renderend;
    ...
    if (result != ISC_R_SUCCESS)
        goto done;
    result = dns_message_rendersection (client->message, DNS_SECTION_ADDITIONAL, preferred_glue | render_opts);
  ...
  renderend:
    ...
    if (opt_included)
    {
        isc_stats_increment (ns_g_server->nsstats, dns_nsstatscounter_edns0out);
    }
    if (client->message->tsigkey != NULL)
    {
        isc_stats_increment (ns_g_server->nsstats, dns_nsstatscounter_tsigout);
    }
    if (client->message->sig0key != NULL)
    {
        isc_stats_increment (ns_g_server->nsstats, dns_nsstatscounter_sig0out);
    }
    if ((client->message->flags & DNS_MESSAGEFLAG_TC) != 0)
        isc_stats_increment (ns_g_server->nsstats, dns_nsstatscounter_truncatedresp);
...
void ns_client_error (ns_client_t * client, isc_result_t result)
{
     ...
     */
    if (rcode == dns_rcode_formerr && ns_client_dropport (isc_sockaddr_getport (&client->peeraddr)) != DROPPORT_NO)
    {
        char buf[64];
        ...
        ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                       ...
                       "dropped error (%.*s) response: suspicious port", (int) isc_buffer_usedlength (&b), buf);
        ns_client_next (client, ISC_R_SUCCESS);
        return;
    }
#endif
    ...
    if (rcode == dns_rcode_formerr)
    {
        if (isc_sockaddr_equal (&client->peeraddr,
            ...
            message->id == client->formerrcache.id && client->requesttime - client->formerrcache.time < 2)
        {
            /* Drop packet. */
            ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                           NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (1), "possible error packet loop, " "FORMERR dropped");
            ns_client_next (client, result);
            return;
        }
...
static inline isc_result_t client_addopt (ns_client_t * client)
{
    ...
    if (client->attributes & NS_CLIENTATTR_WANTNSID &&
        (ns_g_server->server_id != NULL || ns_g_server->server_usehostname))
    {
        /*
        ...
        result = isc_buffer_allocate (client->mctx, &buffer, rdata->length);
        if (result != ISC_R_SUCCESS)
            goto no_nsid;
...
static inline isc_boolean_t allowed (isc_netaddr_t * addr, dns_name_t * signer, dns_acl_t * acl)
{
    int match;
    ...
    if (acl == NULL)
        return (ISC_TRUE);
    result = dns_acl_match (addr, signer, acl, &ns_g_server->aclenv, &match, NULL);
...
ns_client_isself (dns_view_t * myview, dns_tsigkey_t * mykey,
                  isc_sockaddr_t * srcaddr, isc_sockaddr_t * dstaddr, dns_rdataclass_t rdclass, void *arg)
{
    dns_view_t *view;
    ...
    for (view = ISC_LIST_HEAD (ns_g_server->viewlist); view != NULL; view = ISC_LIST_NEXT (view, link))
    {
        ...
        if (allowed (&netsrc, tsig, view->matchclients) && allowed (&netdst, tsig, view->matchdestinations))
            break;
    }
...
static void client_request (isc_task_t * task, isc_event_t * event)
{
    ...
    INSIST (client->state == TCP_CLIENT (client) ? NS_CLIENTSTATE_READING : NS_CLIENTSTATE_READY);
        ...
        else
        {
            if (result != ISC_R_CANCELED)
                isc_log_write (ns_g_lctx, NS_LOGCATEGORY_CLIENT,
                               ...
                               "down due to fatal receive " "error: %s", isc_result_totext (result));
            isc_task_shutdown (client->task);
        }
        goto cleanup;
    }
...
#if NS_CLIENT_DROPPORT
    if (ns_client_dropport (isc_sockaddr_getport (&client->peeraddr)) == DROPPORT_REQUEST)
    {
        ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                       NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (10), "dropped request: suspicious port");
        ns_client_next (client, ISC_R_SUCCESS);
        goto cleanup;
    }
#endif
    ...
    ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                   NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (3), "%s request", TCP_CLIENT (client) ? "TCP" : "UDP");
    ...
    if (!TCP_CLIENT (client))
    {
        ...
        if (ns_g_server->blackholeacl != NULL &&
            dns_acl_match (&netaddr, NULL, ns_g_server->blackholeacl,
                           &ns_g_server->aclenv, &match, NULL) == ISC_R_SUCCESS && match > 0)
        {
            ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                           NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (10), "blackholed UDP datagram");
            ns_client_next (client, ISC_R_SUCCESS);
            goto cleanup;
        }
    }
    ...
    {
        ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                       NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (2), "dropping multicast request");
    ...
    {
        isc_stats_increment (ns_g_server->nsstats, dns_nsstatscounter_requestv4);
    }
    else
    {
        isc_stats_increment (ns_g_server->nsstats, dns_nsstatscounter_requestv6);
    }
    if (TCP_CLIENT (client))
        isc_stats_increment (ns_g_server->nsstats, dns_nsstatscounter_tcp);
    ...
    dns_opcodestats_increment (ns_g_server->opcodestats, client->message->opcode);
    ...
    if (opt != NULL)
    {
        ...
        {
            isc_stats_increment (ns_g_server->nsstats, dns_nsstatscounter_badednsver);
    ...
    {
        ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                       NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (1), "message class could not be determined");
        ns_client_dumpmessage (client, "message class could not be determined");
    ...
    if ((client->interface->flags & NS_INTERFACEFLAG_ANYADDR) == 0)
        isc_netaddr_fromsockaddr (&client->destaddr, &client->interface->addr);
    else
    {
        ...
        if (TCP_CLIENT (client))
            result = isc_socket_getsockname (client->tcpsocket, &sockaddr);
        if (result == ISC_R_SUCCESS)
            isc_netaddr_fromsockaddr (&client->destaddr, &sockaddr);
        if (result != ISC_R_SUCCESS &&
            client->interface->addr.type.sa.sa_family == AF_INET6 && (client->attributes & NS_CLIENTATTR_PKTINFO) != 0)
        {
            /*
             ...
             */
            isc_netaddr_fromin6 (&client->destaddr, &client->pktinfo.ipi6_addr);
            if (IN6_IS_ADDR_LINKLOCAL (&client->pktinfo.ipi6_addr))
                isc_netaddr_setzone (&client->destaddr, client->pktinfo.ipi6_ifindex);
            result = ISC_R_SUCCESS;
        }
        if (result != ISC_R_SUCCESS)
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__,
                              "failed to get request's " "destination: %s", isc_result_totext (result));
            ns_client_next (client, ISC_R_SUCCESS);
            goto cleanup;
        }
    }
    ...
    for (view = ISC_LIST_HEAD (ns_g_server->viewlist); view != NULL; view = ISC_LIST_NEXT (view, link))
    {
        if (client->message->rdclass == view->rdclass || client->message->rdclass == dns_rdataclass_any)
        {
            ...
            sigresult = dns_message_rechecksig (client->message, view);
            ...
            if (allowed (&netaddr, tsig, view->matchclients) &&
                ...
                !((client->message->flags & DNS_MESSAGEFLAG_RD) == 0 && view->matchrecursiveonly))
            {
    ...
    if (view == NULL)
    {
        ...
        ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                       NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (1), "no matching view in class '%s'", classname);
    ...
    ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                   NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (5), "using view '%s'", view->name);
    ...
    if (result != ISC_R_NOTFOUND)
    {
        ...
        {
            isc_stats_increment (ns_g_server->nsstats, dns_nsstatscounter_tsigin);
        }
        else
        {
            isc_stats_increment (ns_g_server->nsstats, dns_nsstatscounter_sig0in);
        }
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                       NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (3), "request has valid signature: %s", namebuf);
        client->signer = &client->signername;
    }
    else if (result == ISC_R_NOTFOUND)
    {
        ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                       NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (3), "request is not signed");
    }
    else if (result == DNS_R_NOIDENTITY)
    {
        ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                       NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (3), "request is signed by a nonauthoritative key");
    }
    else
    {
        char tsigrcode[64];
        ...
        isc_stats_increment (ns_g_server->nsstats, dns_nsstatscounter_invalidsig);
        signame = NULL;
        if (dns_message_gettsig (client->message, &signame) != NULL)
        {
            char namebuf[DNS_NAME_FORMATSIZE];
            ...
            {
                dns_name_format (client->message->tsigkey->creator, cnamebuf, sizeof (cnamebuf));
                ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                               NS_LOGMODULE_CLIENT,
                               ISC_LOG_ERROR,
                               "request has invalid signature: "
                               "TSIG %s (%s): %s (%s)", namebuf, cnamebuf, isc_result_totext (result), tsigrcode);
            }
            else
            {
                ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                               ...
                               ISC_LOG_ERROR,
                               "request has invalid signature: "
                               "TSIG %s: %s (%s)", namebuf, isc_result_totext (result), tsigrcode);
            }
        }
        else
        {
            ...
            ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                           ...
                           "request has invalid signature: %s (%s)", isc_result_totext (result), tsigrcode);
        }
        /*
         ...
         */
        if (!(client->message->tsigstatus == dns_tsigerror_badkey && client->message->opcode == dns_opcode_update))
        {
    ...
    if (client->view->resolver != NULL &&
        ...
        ns_client_checkaclsilent (client, &client->destaddr,
                                  ...
                                  ISC_TRUE) == ISC_R_SUCCESS &&
        ns_client_checkaclsilent (client, &client->destaddr, client->view->cacheonacl, ISC_TRUE) == ISC_R_SUCCESS)
        ra = ISC_TRUE;
    ...
    ns_client_log (client, DNS_LOGCATEGORY_SECURITY, NS_LOGMODULE_CLIENT,
                   ISC_LOG_DEBUG (3), ra ? "recursion available" : "recursion not available");
...
static void client_timeout (isc_task_t * task, isc_event_t * event)
{
    ...
    REQUIRE (event != NULL);
    REQUIRE (event->ev_type == ISC_TIMEREVENT_LIFE || event->ev_type == ISC_TIMEREVENT_IDLE);
    client = event->ev_arg;
    REQUIRE (NS_CLIENT_VALID (client));
    REQUIRE (task == client->task);
...
static isc_result_t client_create (ns_clientmgr_t * manager, ns_client_t ** clientp)
{
    ...
    result = isc_timer_create (manager->timermgr, isc_timertype_inactive,
                               NULL, NULL, client->task, client_timeout, client, &client->timer);
    if (result != ISC_R_SUCCESS)
        goto cleanup_task;
    client->timerset = ISC_FALSE;
    ...
    client->sendevent = (isc_socketevent_t *)
        isc_event_allocate (client->mctx, client,
                            ISC_SOCKEVENT_SENDDONE, client_senddone, client, sizeof (isc_socketevent_t));
    ...
    client->recvevent = (isc_socketevent_t *)
        isc_event_allocate (client->mctx, client,
                            ISC_SOCKEVENT_RECVDONE, client_request, client, sizeof (isc_socketevent_t));
    ...
    ISC_EVENT_INIT (&client->ctlevent, sizeof (client->ctlevent), 0, NULL,
                    NS_EVENT_CLIENTCONTROL, client_start, client, client, NULL, NULL);
    /*
...
static void client_newconn (isc_task_t * task, isc_event_t * event)
{
    ...
    if (nevent->result == ISC_R_SUCCESS)
    {
        ...
        client->peeraddr_valid = ISC_TRUE;
        ns_client_log (client, NS_LOGCATEGORY_CLIENT, NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (3), "new TCP connection");
    }
    else
    {
        /*
         ...
         */
        ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                       NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (3), "accept failed: %s", isc_result_totext (nevent->result));
    }
    ...
    if (nevent->result == ISC_R_SUCCESS)
    {
        ...
        {
            ns_client_log (client, DNS_LOGCATEGORY_SECURITY,
                           NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (10), "blackholed connection attempt");
            client->newstate = NS_CLIENTSTATE_READY;
            (void) exit_check (client);
            goto freeevent;
        }
         ...
         */
        result = isc_quota_attach (&ns_g_server->tcpquota, &client->tcpquota);
        if (result == ISC_R_SUCCESS)
            result = ns_client_replace (client);
        if (result != ISC_R_SUCCESS)
        {
            ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                           NS_LOGMODULE_CLIENT, ISC_LOG_WARNING, "no more TCP clients: %s", isc_result_totext (result));
        }
...
static void client_udprecv (ns_client_t * client)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_socket_recv2() failed: %s", isc_result_totext (result));
        /*
...
void ns_client_attach (ns_client_t * source, ns_client_t ** targetp)
{
    ...
    ns_client_log (source, NS_LOGCATEGORY_CLIENT,
                   NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (10), "ns_client_attach: ref = %d", source->references);
    *targetp = source;
}
...
void ns_client_detach (ns_client_t ** clientp)
{
    ...
    *clientp = NULL;
    ns_client_log (client, NS_LOGCATEGORY_CLIENT,
                   NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (10), "ns_client_detach: ref = %d", client->references);
    (void) exit_check (client);
}
...
isc_result_t ns_client_replace (ns_client_t * client)
{
    ...
    result = ns_clientmgr_createclients (client->manager,
                                         1, client->interface, (TCP_CLIENT (client) ? ISC_TRUE : ISC_FALSE));
    if (result != ISC_R_SUCCESS)
        return (result);
...
isc_result_t
ns_clientmgr_create (isc_mem_t * mctx, isc_taskmgr_t * taskmgr, isc_timermgr_t * timermgr, ns_clientmgr_t ** managerp)
{
...
void ns_clientmgr_destroy (ns_clientmgr_t ** managerp)
{
    ...
    for (client = ISC_LIST_HEAD (manager->recursing); client != NULL; client = ISC_LIST_NEXT (client, link))
        isc_task_shutdown (client->task);

    for (client = ISC_LIST_HEAD (manager->active); client != NULL; client = ISC_LIST_NEXT (client, link))
        isc_task_shutdown (client->task);

    for (client = ISC_LIST_HEAD (manager->inactive); client != NULL; client = ISC_LIST_NEXT (client, link))
        isc_task_shutdown (client->task);

    if (ISC_LIST_EMPTY (manager->active) && ISC_LIST_EMPTY (manager->inactive) && ISC_LIST_EMPTY (manager->recursing))
...
isc_result_t
ns_clientmgr_createclients (ns_clientmgr_t * manager, unsigned int n, ns_interface_t * ifp, isc_boolean_t tcp)
{
...
isc_result_t
ns_client_checkaclsilent (ns_client_t * client, isc_netaddr_t * netaddr, dns_acl_t * acl, isc_boolean_t default_allow)
{
    isc_result_t result;
    ...
    result = dns_acl_match (netaddr, client->signer, acl, &ns_g_server->aclenv, &match, NULL);
...
ns_client_checkacl (ns_client_t * client, isc_sockaddr_t * sockaddr,
                    const char *opname, dns_acl_t * acl, isc_boolean_t default_allow, int log_level)
{
    isc_result_t result;
    ...
    result = ns_client_checkaclsilent (client, sockaddr ? &netaddr : NULL, acl, default_allow);

    if (result == ISC_R_SUCCESS)
        ns_client_log (client, DNS_LOGCATEGORY_SECURITY, NS_LOGMODULE_CLIENT, ISC_LOG_DEBUG (3), "%s approved", opname);
    else
        ns_client_log (client, DNS_LOGCATEGORY_SECURITY, NS_LOGMODULE_CLIENT, log_level, "%s denied", opname);
    return (result);
}
                ...
                isc_logmodule_t * module, int level, const char *fmt, va_list ap)
{
    ...
    isc_log_write (ns_g_lctx, category, module, level, "client %s%s%s: %s", peerbuf, sep, name, msgbuf);
}
                  ...
                  dns_rdataclass_t rdclass, char *buf, size_t len)
{
    ...
    (void) snprintf (buf, len, "%s '%s/%s/%s'", msg, namebuf, typebuf, classbuf);
}
...
static void ns_client_dumpmessage (ns_client_t * client, const char *reason)
{
    ...
    do
    {
        ...
        result = dns_message_totext (client->message, &dns_master_style_debug, 0, &buffer);
        if (result == ISC_R_NOSPACE)
        {
            ...
            ns_client_log (client, NS_LOGCATEGORY_UNMATCHED,
                           ...
                           "%s\n%.*s", reason, (int) isc_buffer_usedlength (&buffer), buf);
    }
...
void ns_client_dumprecursing (FILE * f, ns_clientmgr_t * manager)
{
    ...
    while (client != NULL)
    {
        ns_client_name (client, peerbuf, sizeof (peerbuf));
        if (client->view != NULL &&
            strcmp (client->view->name, "_bind") != 0 && strcmp (client->view->name, "_default") != 0)
        {
            name = client->view->name;
            sep = ": view ";
        }
        ...
        if (client->query.qname != client->query.origqname && client->query.origqname != NULL)
        {
            origfor = " for ";
            dns_name_format (client->query.origqname, original, sizeof (original));
        }
        else
        {
            origfor = "";
        ...
        fprintf (f, "; client %s%s%s: id %u '%s/%s/%s'%s%s "
                 "requesttime %d\n", peerbuf, sep, name,
                 client->message->id, namebuf, typebuf, classbuf, origfor, original, client->requesttime);
