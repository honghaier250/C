...
ns_sortlisttype_t ns_sortlist_setup (dns_acl_t * acl, isc_netaddr_t * clientaddr, const void **argp)
{
    ...
    for (i = 0; i < acl->length; i++)
    {
        ...
        if (dns_aclelement_match (clientaddr, NULL, try_elt, &ns_g_server->aclenv, &matched_elt))
        {
            if (order_elt != NULL)
            {
                ...
                else if (order_elt->type == dns_aclelementtype_localhost && ns_g_server->aclenv.localhost != NULL)
                {
                    *argp = ns_g_server->aclenv.localhost;
                    return (NS_SORTLISTTYPE_2ELEMENT);
                }
                else if (order_elt->type == dns_aclelementtype_localnets && ns_g_server->aclenv.localnets != NULL)
                {
...
int ns_sortlist_addrorder2 (const isc_netaddr_t * addr, const void *arg)
{
    ...
    (void) dns_acl_match (addr, NULL, sortacl, &ns_g_server->aclenv, &match, NULL);
    if (match > 0)
        return (match);
    else if (match < 0)
...
int ns_sortlist_addrorder1 (const isc_netaddr_t * addr, const void *arg)
{
    ...
    if (dns_aclelement_match (addr, NULL, matchelt, &ns_g_server->aclenv, NULL))
    {
        return (0);
    }
    else
    ...
    switch (sortlisttype)
    {
        ...
        case NS_SORTLISTTYPE_NONE:
            *orderp = NULL;
            break;
        default:
            UNEXPECTED_ERROR (__FILE__, __LINE__, "unexpected return from ns_sortlist_setup(): " "%d", sortlisttype);
            break;
    }
}
