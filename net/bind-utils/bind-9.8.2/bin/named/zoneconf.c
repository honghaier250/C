...
configure_zone_acl (const cfg_obj_t * zconfig, const cfg_obj_t * vconfig,
                    ...
                    void (*setzacl) (dns_zone_t *, dns_acl_t *), void (*clearzacl) (dns_zone_t *))
{
    isc_result_t result;
  ...
  parse_acl:
    result = cfg_acl_fromconfig (aclobj, config, ns_g_lctx, actx, dns_zone_getmctx (zone), 0, &acl);
    if (result != ISC_R_SUCCESS)
        return (result);
    (*setzacl) (zone, acl);
...
static isc_result_t configure_zone_ssutable (const cfg_obj_t * zconfig, dns_zone_t * zone, const char *zname)
{
    ...
    if (cfg_obj_isstring (updatepolicy) && strcmp ("local", cfg_obj_asstring (updatepolicy)) == 0)
    {
        autoddns = ISC_TRUE;
        updatepolicy = NULL;
    }
    ...
    for (element = cfg_list_first (updatepolicy); element != NULL; element = cfg_list_next (element))
    {
        ...
        result = dns_name_fromtext (dns_fixedname_name (&fident), &b, dns_rootname, 0, NULL);
        if (result != ISC_R_SUCCESS)
        {
            cfg_obj_log (identity, ns_g_lctx, ISC_LOG_ERROR, "'%s' is not a valid name", str);
            goto cleanup;
        }
        ...
        if (usezone)
        {
            result = dns_name_copy (dns_zone_getorigin (zone), dns_fixedname_name (&fname), NULL);
            if (result != ISC_R_SUCCESS)
            {
                cfg_obj_log (identity, ns_g_lctx, ISC_LOG_ERROR,
                             "error copying origin: %s", isc_result_totext (result));
                goto cleanup;
            }
        }
        else
        {
            ...
            isc_buffer_init (&b, str, strlen (str));
            isc_buffer_add (&b, strlen (str));
            result = dns_name_fromtext (dns_fixedname_name (&fname), &b, dns_rootname, 0, NULL);
            if (result != ISC_R_SUCCESS)
            {
                cfg_obj_log (identity, ns_g_lctx, ISC_LOG_ERROR, "'%s' is not a valid name", str);
                goto cleanup;
        ...
        for (element2 = cfg_list_first (typelist); element2 != NULL; element2 = cfg_list_next (element2))
        {
            ...
            if (result != ISC_R_SUCCESS)
            {
                cfg_obj_log (identity, ns_g_lctx, ISC_LOG_ERROR, "'%s' is not a valid type", str);
        ...
        result = dns_ssutable_addrule (table, grant,
                                       dns_fixedname_name (&fident), mtype, dns_fixedname_name (&fname), n, types);
        if (types != NULL)
            isc_mem_put (mctx, types, n * sizeof (dns_rdatatype_t));
        if (result != ISC_R_SUCCESS)
        {
    ...
    if (autoddns)
    {
        ...
        if (ns_g_server->session_keyname == NULL)
        {
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           ...
                           "failed to enable auto DDNS policy " "for zone %s: session key not found", zname);
        ...
        result = dns_ssutable_addrule (table, ISC_TRUE,
                                       ...
                                       DNS_SSUMATCHTYPE_SUBDOMAIN, dns_zone_getorigin (zone), 1, &any);
...
configure_staticstub_serveraddrs (const cfg_obj_t * zconfig, dns_zone_t * zone,
                                  ...
                                  dns_rdatalist_t * rdatalist_a, dns_rdatalist_t * rdatalist_aaaa)
{
    ...
    for (element = cfg_list_first (zconfig); element != NULL; element = cfg_list_next (element))
    {
        ...
        dns_rdata_fromregion (rdata, dns_zone_getclass (zone), rdatalist->type, &region);
     ...
     */
    if (ISC_LIST_EMPTY (rdatalist_a->rdata) && ISC_LIST_EMPTY (rdatalist_aaaa->rdata))
    {
        return (ISC_R_SUCCESS);
    }
    ...
    dns_rdata_init (rdata);
    dns_rdata_fromregion (rdata, dns_zone_getclass (zone), dns_rdatatype_ns, &region);
                                  ...
                                  dns_rdatalist_t * rdatalist, const char *zname)
{
    ...
    for (element = cfg_list_first (zconfig); element != NULL; element = cfg_list_next (element))
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            cfg_obj_log (zconfig, ns_g_lctx, ISC_LOG_ERROR, "server-name '%s' is not a valid " "name", str);
            return (result);
        }
        if (dns_name_issubdomain (nsname, dns_zone_getorigin (zone)))
        {
            cfg_obj_log (zconfig, ns_g_lctx, ISC_LOG_ERROR,
                         "server-name '%s' must not be a " "subdomain of zone name '%s'", str, zname);
            return (ISC_R_FAILURE);
        }
 ...
 */
static isc_result_t
configure_staticstub (const cfg_obj_t * zconfig, dns_zone_t * zone, const char *zname, const char *dbtype)
{
    int i = 0;
    ...
    RETERR (dns_db_create (mctx, dbtype, dns_zone_getorigin (zone),
                           dns_dbtype_stub, dns_zone_getclass (zone), 0, NULL, &db));
    ...
    {
        INSIST (obj != NULL);
        result = configure_staticstub_serveraddrs (obj, zone, &rdatalist_ns, &rdatalist_a, &rdatalist_aaaa);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
    }
    ...
    {
        INSIST (obj != NULL);
        result = configure_staticstub_servernames (obj, zone, &rdatalist_ns, zname);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
    }
    ...
    if (ISC_LIST_EMPTY (rdatalist_ns.rdata))
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_SERVER, ISC_LOG_ERROR,
                       "No NS record is configured for a " "static-stub zone '%s'", zname);
    ...
    RUNTIME_CHECK (dns_rdatalist_tordataset (&rdatalist_ns, &rdataset) == ISC_R_SUCCESS);
    result = dns_db_addrdataset (db, apexnode, dbversion, 0, &rdataset, 0, NULL);
    ...
    {
        RUNTIME_CHECK (dns_rdatalist_tordataset (&rdatalist_a, &rdataset) == ISC_R_SUCCESS);
        result = dns_db_addrdataset (db, apexnode, dbversion, 0, &rdataset, 0, NULL);
        dns_rdataset_disassociate (&rdataset);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
    ...
    {
        RUNTIME_CHECK (dns_rdatalist_tordataset (&rdatalist_aaaa, &rdataset) == ISC_R_SUCCESS);
        result = dns_db_addrdataset (db, apexnode, dbversion, 0, &rdataset, 0, NULL);
        dns_rdataset_disassociate (&rdataset);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
  ...
  cleanup:
    ...
    for (i = 0; rdatalists[i] != NULL; i++)
    {
        while ((rdata = ISC_LIST_HEAD (rdatalists[i]->rdata)) != NULL)
        {
            ...
            isc_mem_put (mctx, rdata, sizeof (*rdata) + region.length);
        }
    }
 ...
 */
static inline dns_zonetype_t zonetype_fromconfig (const cfg_obj_t * map)
{
...
ns_zone_configure (const cfg_obj_t * config, const cfg_obj_t * vconfig,
                   const cfg_obj_t * zconfig, cfg_aclconfctx_t * ac, dns_zone_t * zone)
{
    isc_result_t result;
    ...
    if (vconfig != NULL)
        RETERR (ns_config_getclass (cfg_tuple_get (vconfig, "class"), dns_rdataclass_in, &vclass));
    else
    ...
    RETERR (ns_config_getclass (cfg_tuple_get (zconfig, "class"), vclass, &zclass));
     ...
     */
    if (ztype == dns_zone_master && cpval == default_dbtype && filename == NULL)
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
    ...
    if (ztype == dns_zone_slave)
        RETERR (configure_zone_acl (zconfig, vconfig, config,
                                    allow_notify, ac, zone, dns_zone_setnotifyacl, dns_zone_clearnotifyacl));
    /*
     * XXXAG This probably does not make sense for stubs.
     */
    RETERR (configure_zone_acl (zconfig, vconfig, config,
                                allow_query, ac, zone, dns_zone_setqueryacl, dns_zone_clearqueryacl));
    ...
    if (zonestats_on)
    {
        RETERR (isc_stats_create (mctx, &zoneqrystats, dns_nsstatscounter_max));
    }
    ...
    if (ztype != dns_zone_stub && ztype != dns_zone_staticstub)
    {
        ...
        if (result == ISC_R_SUCCESS)
        {
            ...
            result = ns_config_getiplist (config, obj, 0, mctx, &addrs, &addrcount);
            if (result != ISC_R_SUCCESS)
                return (result);
            result = dns_zone_setalsonotify (zone, addrs, addrcount);
        ...
        INSIST (result == ISC_R_SUCCESS && obj != NULL);
        dns_zone_setoption (zone, DNS_ZONEOPT_NOTIFYTOSOA, cfg_obj_asboolean (obj));

        dns_zone_setisself (zone, ns_client_isself, NULL);

        RETERR (configure_zone_acl (zconfig, vconfig, config,
                                    allow_transfer, ac, zone, dns_zone_setxfracl, dns_zone_clearxfracl));
        ...
        else
        {
            ...
            if (value > ISC_UINT32_MAX / 2)
            {
                cfg_obj_log (obj, ns_g_lctx,
                             ISC_LOG_ERROR, "'max-journal-size " "%" ISC_PRINT_QUADFORMAT "d' " "is too large", value);
                RETERR (ISC_R_RANGE);
            }
        ...
        else if (!strcasecmp (cfg_obj_asstring (obj), "master") && ztype == dns_zone_master)
            ixfrdiff = ISC_TRUE;
        else if (!strcasecmp (cfg_obj_asstring (obj), "slave") && ztype == dns_zone_slave)
            ixfrdiff = ISC_TRUE;
        else
            ixfrdiff = ISC_FALSE;
        ...
        INSIST (result == ISC_R_SUCCESS && obj != NULL);
        dns_zone_setoption (zone, DNS_ZONEOPT_CHECKSIBLING, cfg_obj_asboolean (obj));
        ...
        INSIST (result == ISC_R_SUCCESS && obj != NULL);
        dns_zone_setoption (zone, DNS_ZONEOPT_NSEC3TESTZONE, cfg_obj_asboolean (obj));
    }
    ...
    if (ztype == dns_zone_master)
    {
        ...
        RETERR (configure_zone_acl (zconfig, vconfig, config,
                                    allow_update, ac, zone, dns_zone_setupdateacl, dns_zone_clearupdateacl));

        updateacl = dns_zone_getupdateacl (zone);
        if (updateacl != NULL && dns_acl_isinsecure (updateacl))
            isc_log_write (ns_g_lctx, DNS_LOGCATEGORY_SECURITY,
                           NS_LOGMODULE_SERVER, ISC_LOG_WARNING,
                           "zone '%s' allows updates by IP " "address, which is insecure", zname);
        ...
        INSIST (result == ISC_R_SUCCESS && obj != NULL);
        dns_zone_setoption (zone, DNS_ZONEOPT_UPDATECHECKKSK, cfg_obj_asboolean (obj));

        obj = NULL;
        result = ns_config_get (maps, "dnssec-dnskey-kskonly", &obj);
        INSIST (result == ISC_R_SUCCESS && obj != NULL);
        dns_zone_setoption (zone, DNS_ZONEOPT_DNSKEYKSKONLY, cfg_obj_asboolean (obj));
    }
    else if (ztype == dns_zone_slave)
    {
        RETERR (configure_zone_acl (zconfig, vconfig, config,
                                    allow_update_forwarding, ac, zone,
                                    dns_zone_setforwardacl, dns_zone_clearforwardacl));
    }
    ...
    if (ztype == dns_zone_master)
    {
        ...
        INSIST (result == ISC_R_SUCCESS && obj != NULL);
        dns_zone_setoption (zone, DNS_ZONEOPT_CHECKINTEGRITY, cfg_obj_asboolean (obj));
        ...
        INSIST (result == ISC_R_SUCCESS && obj != NULL);
        dns_zone_setoption (zone, DNS_ZONEOPT_SECURETOINSECURE, cfg_obj_asboolean (obj));
    ...
    switch (ztype)
    {
        ...
        case dns_zone_stub:
            ...
            if (obj != NULL)
            {
                addrs = NULL;
                keynames = NULL;
                RETERR (ns_config_getipandkeylist (config, obj, mctx, &addrs, &keynames, &count));
                result = dns_zone_setmasterswithkeys (zone, addrs, keynames, count);
                if (count != 0)
                    ns_config_putipandkeylist (mctx, &addrs, &keynames, count);
                else
            ...
            dns_zone_setoption (zone, DNS_ZONEOPT_TRYTCPREFRESH, cfg_obj_asboolean (obj));
            break;

        case dns_zone_staticstub:
            RETERR (configure_staticstub (zoptions, zone, zname, default_dbtype));
            break;
...
isc_boolean_t ns_zone_reusable (dns_zone_t * zone, const cfg_obj_t * zconfig)
{
    ...
    {
        dns_zone_log (zone, ISC_LOG_DEBUG (1), "not reusable: type mismatch");
        return (ISC_FALSE);
    }
    ...
    {
        dns_zone_log (zone, ISC_LOG_DEBUG (1), "not reusable: staticstub");
        return (ISC_FALSE);
    }
    ...
    if (!((cfilename == NULL && zfilename == NULL) ||
          (cfilename != NULL && zfilename != NULL && strcmp (cfilename, zfilename) == 0)))
    {
        dns_zone_log (zone, ISC_LOG_DEBUG (1), "not reusable: filename mismatch");
        return (ISC_FALSE);
    }
