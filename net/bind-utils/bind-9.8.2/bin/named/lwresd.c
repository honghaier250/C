...
isc_result_t ns_lwresd_parseeresolvconf (isc_mem_t * mctx, cfg_parser_t * pctx, cfg_obj_t ** configp)
{
    ...
    lwctx = NULL;
    lwresult = lwres_context_create (&lwctx, mctx, ns__lwresd_memalloc, ns__lwresd_memfree, LWRES_CONTEXT_SERVERMODE);
    ...
    if (lwc->nsnext > 0)
    {
        ...
        {
            CHECK (lwaddr_sockaddr_fromlwresaddr (&sa, &lwc->nameservers[i], ns_g_port));
    ...
    if (lwc->sortlistnxt > 0)
    {
        ...
        for (i = 0; i < lwc->sortlistnxt; i++)
        {
            ...
            if (result != ISC_R_SUCCESS)
            {
                ...
                isc_netaddr_format (&na, addrtext, sizeof (addrtext));
                isc_log_write (ns_g_lctx,
                               NS_LOGCATEGORY_GENERAL,
                               NS_LOGMODULE_LWRESD,
                               ISC_LOG_ERROR, "processing sortlist: '%s' is " "not a valid netmask", addrtext);
                goto cleanup;
            }
    ...
    if (lwc->lwnext > 0)
    {
        ...
        {
            CHECK (lwaddr_sockaddr_fromlwresaddr (&sa, &lwc->lwservers[i], 0));
            isc_netaddr_fromsockaddr (&na, &sa);
            CHECK (buffer_putstr (&b, "\t\t"));
            CHECK (isc_netaddr_totext (&na, &b));
            CHECK (buffer_putstr (&b, ";\n"));
...
isc_result_t ns_lwdmanager_create (isc_mem_t * mctx, const cfg_obj_t * lwres, ns_lwresd_t ** lwresdp)
{
    ...
    result = dns_viewlist_find (&ns_g_server->viewlist, vname, vclass, &lwresd->view);
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_LWRESD, ISC_LOG_WARNING, "couldn't find view %s", vname);
        goto fail;
    }
    ...
    {
        lwresd->search = NULL;
        result = ns_lwsearchlist_create (lwresd->mctx, &lwresd->search);
        if (result != ISC_R_SUCCESS)
        {
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           NS_LOGMODULE_LWRESD, ISC_LOG_WARNING, "couldn't create searchlist");
            goto fail;
        }
        for (element = cfg_list_first (searchobj); element != NULL; element = cfg_list_next (element))
        {
            ...
            isc_buffer_init (&namebuf, searchstr, strlen (searchstr));
            isc_buffer_add (&namebuf, strlen (searchstr));
            result = dns_name_fromtext (name, &namebuf, dns_rootname, 0, NULL);
            if (result != ISC_R_SUCCESS)
            {
                isc_log_write (ns_g_lctx,
                               NS_LOGCATEGORY_GENERAL,
                               NS_LOGMODULE_LWRESD, ISC_LOG_WARNING, "invalid name %s in searchlist", searchstr);
                continue;
            }
            ...
            if (result != ISC_R_SUCCESS)
            {
                isc_log_write (ns_g_lctx,
                               NS_LOGCATEGORY_GENERAL,
                               NS_LOGMODULE_LWRESD, ISC_LOG_WARNING, "couldn't update searchlist");
                goto fail;
            }
        }
    }
...
static isc_result_t listener_bind (ns_lwreslistener_t * listener, isc_sockaddr_t * address)
{
    ...
    sock = NULL;
    result = isc_socket_create (ns_g_socketmgr, pf, isc_sockettype_udp, &sock);
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_LWRESD, ISC_LOG_WARNING,
                       "failed to create lwres socket: %s", isc_result_totext (result));
        return (result);
    }

    result = isc_socket_bind (sock, &listener->address, ISC_SOCKET_REUSEADDRESS);
    if (result != ISC_R_SUCCESS)
    {
        ...
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_LWRESD, ISC_LOG_WARNING,
                       "failed to add lwres socket: %s: %s", socktext, isc_result_totext (result));
        return (result);
    }
    listener->sock = sock;
    return (ISC_R_SUCCESS);
...
static isc_result_t listener_startclients (ns_lwreslistener_t * listener)
{
    ...
    for (i = 0; i < NTASKS && result == ISC_R_SUCCESS; i++)
        result = ns_lwdclientmgr_create (listener, NRECVS, ns_g_taskmgr);
    ...
    while (cm != NULL)
    {
        ...
        if (result != ISC_R_SUCCESS)
            isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                           ...
                           "could not start lwres " "client handler: %s", isc_result_totext (result));
...
static isc_result_t find_listener (isc_sockaddr_t * address, ns_lwreslistener_t ** listenerp)
{
    ...
    for (listener = ISC_LIST_HEAD (listeners); listener != NULL; listener = ISC_LIST_NEXT (listener, link))
    {
                    ...
                    isc_mem_t * mctx, ns_lwreslistenerlist_t * newlisteners)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        isc_log_write (ns_g_lctx, ISC_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_LWRESD, ISC_LOG_WARNING,
                       "lwres failed to configure %s: %s", socktext, isc_result_totext (result));
        return (result);
    }
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        isc_log_write (ns_g_lctx, ISC_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_LWRESD, ISC_LOG_WARNING,
                       "lwres: failed to start %s: %s", socktext, isc_result_totext (result));
        ns_lwreslistener_detach (&listener);
        return (ISC_R_SUCCESS);
    }
    ...
    else
    {
        isc_sockaddr_format (address, socktext, sizeof (socktext));
        isc_log_write (ns_g_lctx, ISC_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_LWRESD, ISC_LOG_NOTICE, "lwres listening on %s", socktext);
    }
...
isc_result_t ns_lwresd_configure (isc_mem_t * mctx, const cfg_obj_t * config)
{
     ...
     */
    for (element = cfg_list_first (lwreslist); element != NULL; element = cfg_list_next (element))
    {
        in_port_t port;
        ...
        if (listenerslist == NULL)
        {
            ...
            CHECK (configure_listener (&address, lwresd, mctx, &newlisteners));
        }
        else
        {
            isc_uint32_t i;

            CHECK (ns_config_getiplist (config, listenerslist, port, mctx, &addrs, &count));
            for (i = 0; i < count; i++)
                CHECK (configure_listener (&addrs[i], lwresd, mctx, &newlisteners));
    ...
    while (!ISC_LIST_EMPTY (listeners))
    {
        ...
        isc_log_write (ns_g_lctx, ISC_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_LWRESD, ISC_LOG_NOTICE, "lwres no longer listening on %s", socktext);
    }
