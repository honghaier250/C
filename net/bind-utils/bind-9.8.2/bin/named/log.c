...
isc_result_t ns_log_setdefaultchannels (isc_logconfig_t * lcfg)
{
    ...
    if (!ns_g_logstderr)
    {
        ...
        result = isc_log_createchannel (lcfg, "default_debug",
                                        ISC_LOG_TOFILE,
                                        ISC_LOG_DYNAMIC, &destination, ISC_LOG_PRINTTIME | ISC_LOG_DEBUGONLY);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
    }
    ...
    result = isc_log_createchannel (lcfg, "default_syslog", ISC_LOG_TOSYSLOG, ISC_LOG_INFO, &destination, 0);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
#endif
...
isc_result_t ns_log_setsafechannels (isc_logconfig_t * lcfg)
{
    ...
    if (!ns_g_logstderr)
    {
        result = isc_log_createchannel (lcfg, "default_debug", ISC_LOG_TONULL, ISC_LOG_DYNAMIC, NULL, 0);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
...
isc_result_t ns_log_setdefaultcategory (isc_logconfig_t * lcfg)
{
    ...
    if (!ns_g_logstderr)
    {
        result = isc_log_usechannel (lcfg, "default_syslog", ISC_LOGCATEGORY_DEFAULT, NULL);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
    }

    result = isc_log_usechannel (lcfg, "default_debug", ISC_LOGCATEGORY_DEFAULT, NULL);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
...
isc_result_t ns_log_setunmatchedcategory (isc_logconfig_t * lcfg)
{
    ...
    result = isc_log_usechannel (lcfg, "null", NS_LOGCATEGORY_UNMATCHED, NULL);
    return (result);
}
