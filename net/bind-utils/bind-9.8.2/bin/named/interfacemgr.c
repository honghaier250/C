...
ns_interfacemgr_create (isc_mem_t * mctx, isc_taskmgr_t * taskmgr,
                        isc_socketmgr_t * socketmgr, dns_dispatchmgr_t * dispatchmgr, ns_interfacemgr_t ** mgrp)
{
    isc_result_t result;
...
static isc_result_t
ns_interface_create (ns_interfacemgr_t * mgr, isc_sockaddr_t * addr, const char *name, ns_interface_t ** ifpret)
{
    ns_interface_t *ifp;
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (IFMGR_COMMON_LOGARGS, ISC_LOG_ERROR,
                       "ns_clientmgr_create() failed: %s", isc_result_totext (result));
...
static isc_result_t ns_interface_listenudp (ns_interface_t * ifp)
{
    ...
    result = dns_dispatch_getudp (ifp->mgr->dispatchmgr, ns_g_socketmgr,
                                  ...
                                  4096, 1000, 32768, 8219, 8237, attrs, attrmask, &ifp->udpdispatch);
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (IFMGR_COMMON_LOGARGS, ISC_LOG_ERROR,
                       "could not listen on UDP socket: %s", isc_result_totext (result));
    ...
    if (result != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "UDP ns_clientmgr_createclients(): %s", isc_result_totext (result));
        goto addtodispatch_failure;
    }
    return (ISC_R_SUCCESS);

  addtodispatch_failure:
    dns_dispatch_changeattributes (ifp->udpdispatch, 0, DNS_DISPATCHATTR_NOLISTEN);
...
static isc_result_t ns_interface_accepttcp (ns_interface_t * ifp)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (IFMGR_COMMON_LOGARGS, ISC_LOG_ERROR, "creating TCP socket: %s", isc_result_totext (result));
...
#endif
    result = isc_socket_bind (ifp->tcpsocket, &ifp->addr, ISC_SOCKET_REUSEADDRESS);
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (IFMGR_COMMON_LOGARGS, ISC_LOG_ERROR, "binding TCP socket: %s", isc_result_totext (result));
        goto tcp_bind_failure;
    }
    result = isc_socket_listen (ifp->tcpsocket, ns_g_listen);
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (IFMGR_COMMON_LOGARGS, ISC_LOG_ERROR, "listening on TCP socket: %s", isc_result_totext (result));
    ...
    result = ns_clientmgr_createclients (ifp->clientmgr, ifp->ntcptarget, ifp, ISC_TRUE);
    if (result != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "TCP ns_clientmgr_createclients(): %s", isc_result_totext (result));
...
ns_interface_setup (ns_interfacemgr_t * mgr, isc_sockaddr_t * addr,
                    const char *name, ns_interface_t ** ifpret, isc_boolean_t accept_tcp)
{
    isc_result_t result;
 ...
 */
static ns_interface_t *find_matching_interface (ns_interfacemgr_t * mgr, isc_sockaddr_t * addr)
{
    ns_interface_t *ifp;

    for (ifp = ISC_LIST_HEAD (mgr->interfaces); ifp != NULL; ifp = ISC_LIST_NEXT (ifp, link))
    {
...
static void purge_old_interfaces (ns_interfacemgr_t * mgr)
{
    ...
    for (ifp = ISC_LIST_HEAD (mgr->interfaces); ifp != NULL; ifp = next)
    {
        ...
        if (ifp->generation != mgr->generation)
        {
            ...
            isc_log_write (IFMGR_COMMON_LOGARGS, ISC_LOG_INFO, "no longer listening on %s", sabuf);
            ns_interface_shutdown (ifp);
            ns_interface_detach (&ifp);
        }
    }
...
static isc_result_t setup_locals (ns_interfacemgr_t * mgr, isc_interface_t * interface)
{
    ...
    result = dns_iptable_addprefix (mgr->aclenv.localhost->iptable, netaddr, prefixlen, ISC_TRUE);
    if (result != ISC_R_SUCCESS)
        return (result);
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (IFMGR_COMMON_LOGARGS,
                       ...
                       "localnets ACL: %s", interface->name, isc_result_totext (result));
        return (ISC_R_SUCCESS);
    }

    result = dns_iptable_addprefix (mgr->aclenv.localnets->iptable, netaddr, prefixlen, ISC_TRUE);
    if (result != ISC_R_SUCCESS)
        return (result);
...
static void setup_listenon (ns_interfacemgr_t * mgr, isc_interface_t * interface, in_port_t port)
{
    ...
    for (old = ISC_LIST_HEAD (mgr->listenon); old != NULL; old = ISC_LIST_NEXT (old, link))
...
static isc_result_t do_scan (ns_interfacemgr_t * mgr, ns_listenlist_t * ext_listen, isc_boolean_t verbose)
{
    ...
    if (isc_net_probeipv6 () == ISC_R_SUCCESS)
        ...
        isc_log_write (IFMGR_COMMON_LOGARGS, verbose ? ISC_LOG_INFO : ISC_LOG_DEBUG (1), "no IPv6 interfaces found");
#endif

    if (isc_net_probeipv4 () == ISC_R_SUCCESS)
        scan_ipv4 = ISC_TRUE;
    else
        isc_log_write (IFMGR_COMMON_LOGARGS, verbose ? ISC_LOG_INFO : ISC_LOG_DEBUG (1), "no IPv4 interfaces found");
     ...
     */
#ifndef ISC_ALLOW_MAPPED
    if (scan_ipv6 == ISC_TRUE && isc_net_probe_ipv6only () != ISC_R_SUCCESS)
    {
        ipv6only = ISC_FALSE;
        log_explicit = ISC_TRUE;
    }
#endif
    if (scan_ipv6 == ISC_TRUE && isc_net_probe_ipv6pktinfo () != ISC_R_SUCCESS)
    {
    ...
    {
        for (le = ISC_LIST_HEAD (mgr->listenon6->elts); le != NULL; le = ISC_LIST_NEXT (le, link))
        {
            ...
            {
                isc_log_write (IFMGR_COMMON_LOGARGS,
                               ISC_LOG_INFO, "listening on IPv6 " "interfaces, port %u", le->port);
                result = ns_interface_setup (mgr, &listen_addr, "<any>", &ifp, ISC_TRUE);
                if (result == ISC_R_SUCCESS)
                    ifp->flags |= NS_INTERFACEFLAG_ANYADDR;
                else
                    isc_log_write (IFMGR_COMMON_LOGARGS, ISC_LOG_ERROR, "listening on all IPv6 " "interfaces failed");
                /* Continue. */
            }
        }
    }
    ...
    for (result = isc_interfaceiter_first (iter); result == ISC_R_SUCCESS; result = isc_interfaceiter_next (iter))
    {
         ...
         */
        if (family == AF_INET && isc_netaddr_equal (&interface.address, &zero_address))
        {
            continue;
        }
        if (family == AF_INET6 && isc_netaddr_equal (&interface.address, &zero_address6))
        {
            continue;
        }
        ...
        dolistenon = ISC_TRUE;
        for (le = ISC_LIST_HEAD (ll->elts); le != NULL; le = ISC_LIST_NEXT (le, link))
        {
            int match;
            ...
            if (family == AF_INET)
            {
                isc_netaddr_fromin (&listen_netaddr, &interface.address.type.in);
            }
            else
            {
                isc_netaddr_fromin6 (&listen_netaddr, &interface.address.type.in6);
                isc_netaddr_setzone (&listen_netaddr, interface.address.zone);
            }
            isc_sockaddr_fromnetaddr (&listen_sockaddr, &listen_netaddr, le->port);

            /*
             * See if the address matches the listen-on statement;
             * if not, ignore the interface.
             */
            (void) dns_acl_match (&listen_netaddr, NULL, le->acl, &mgr->aclenv, &match, NULL);
            if (match <= 0)
                continue;
             ...
             */
            if (family == AF_INET6 && ipv6only && ipv6pktinfo && listenon_is_ip6_any (le))
            ...
            if (adjusting == ISC_TRUE)
            {
                ...
                match = 0;
                for (ele = ISC_LIST_HEAD (ext_listen->elts); ele != NULL; ele = ISC_LIST_NEXT (ele, link))
                {
                    (void) dns_acl_match (&listen_netaddr, NULL, ele->acl, NULL, &match, NULL);
                    if (match > 0 && (ele->port == le->port || ele->port == 0))
                        break;
                    else
                        match = 0;
                }
            ...
            else
            {
                ...
                if (adjusting == ISC_FALSE && ipv6_wildcard == ISC_TRUE)
                    continue;

                if (log_explicit && family == AF_INET6 && !adjusting && listenon_is_ip6_any (le))
                {
                    isc_log_write (IFMGR_COMMON_LOGARGS,
                ...
                isc_log_write (IFMGR_COMMON_LOGARGS,
                               ...
                               "%s, %s",
                               (adjusting == ISC_TRUE) ?
                               "additionally " : "", (family == AF_INET) ? "IPv4" : "IPv6", interface.name, sabuf);

                result = ns_interface_setup (mgr,
                                             &listen_sockaddr,
                                             interface.name, &ifp, (adjusting == ISC_TRUE) ? ISC_FALSE : ISC_TRUE);
                ...
                if (result != ISC_R_SUCCESS)
                {
                    isc_log_write (IFMGR_COMMON_LOGARGS,
                                   ...
                                   "%s failed; interface "
                                   "ignored", (family == AF_INET) ? "IPv4" : "IPv6", interface.name);
                }
                /* Continue. */
            }
      ...
      ignore_interface:
        isc_log_write (IFMGR_COMMON_LOGARGS,
                       ...
                       (family == AF_INET) ? "IPv4" : "IPv6", interface.name, isc_result_totext (result));
        continue;
    }
    if (result != ISC_R_NOMORE)
        UNEXPECTED_ERROR (__FILE__, __LINE__, "interface iteration failed: %s", isc_result_totext (result));
    else
        result = ISC_R_SUCCESS;
  cleanup_iter:
...
static void ns_interfacemgr_scan0 (ns_interfacemgr_t * mgr, ns_listenlist_t * ext_listen, isc_boolean_t verbose)
{
     ...
     * be expected.
     */
    if (ext_listen == NULL && ISC_LIST_EMPTY (mgr->interfaces) && !ns_g_lwresdonly)
    {
        isc_log_write (IFMGR_COMMON_LOGARGS, ISC_LOG_WARNING, "not listening on any interfaces");
    }
}

void ns_interfacemgr_scan (ns_interfacemgr_t * mgr, isc_boolean_t verbose)
{
    ns_interfacemgr_scan0 (mgr, NULL, verbose);
}

void ns_interfacemgr_adjust (ns_interfacemgr_t * mgr, ns_listenlist_t * list, isc_boolean_t verbose)
{
