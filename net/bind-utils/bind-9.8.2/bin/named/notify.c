...
static void notify_log (ns_client_t * client, int level, const char *fmt, ...)
{
    ...
    va_start (ap, fmt);
    ns_client_logv (client, DNS_LOGCATEGORY_NOTIFY, NS_LOGMODULE_NOTIFY, level, fmt, ap);
    va_end (ap);
}
...
void ns_notify_start (ns_client_t * client)
{
    ...
    {
        notify_log (client, ISC_LOG_NOTICE, "notify question section contains multiple RRs");
        goto formerr;
    }
    ...
    {
        notify_log (client, ISC_LOG_NOTICE, "notify question section contains no SOA");
        goto formerr;
    }
    ...
    if (tsigkey != NULL)
    {
        ...
        if (tsigkey->generated)
        {
            ...
            dns_name_format (tsigkey->creator, cnamebuf, sizeof (cnamebuf));
            snprintf (tsigbuf, sizeof (tsigbuf), ": TSIG '%s' (%s)", namebuf, cnamebuf);
        }
        else
        {
    ...
    switch (dns_zone_gettype (zone))
    {
        ...
        case dns_zone_stub:    /* Allow dialup passive to work. */
            notify_log (client, ISC_LOG_INFO, "received notify for zone '%s'%s", namebuf, tsigbuf);
            respond (client, dns_zone_notifyreceive (zone, ns_client_getsockaddr (client), request));
            break;
        default:
            goto notauth;
    }
  ...
  notauth:
    notify_log (client, ISC_LOG_NOTICE, "received notify for zone '%s'%s: not authoritative", namebuf, tsigbuf);
