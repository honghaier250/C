...
{
    if (listener->exiting && !listener->listening && ISC_LIST_EMPTY (listener->connections))
...
static void maybe_free_connection (controlconnection_t * conn)
{
    ...
    if (conn->sending)
    {
        isc_socket_cancel (conn->sock, listener->task, ISC_SOCKCANCEL_SEND);
        return;
    }
...
static void shutdown_listener (controllistener_t * listener)
{
    ...
    if (!listener->exiting)
    {
        ...
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_CONTROL, ISC_LOG_NOTICE, "stopping command channel on %s", socktext);
        if (listener->type == isc_sockettype_unix)
            isc_socket_cleanunix (&listener->address, ISC_TRUE);
        listener->exiting = ISC_TRUE;
    }

    for (conn = ISC_LIST_HEAD (listener->connections); conn != NULL; conn = next)
    {
    ...
    if (listener->listening)
        isc_socket_cancel (listener->sock, listener->task, ISC_SOCKCANCEL_ACCEPT);
...
static isc_boolean_t address_ok (isc_sockaddr_t * sockaddr, dns_acl_t * acl)
{
    ...
    result = dns_acl_match (&netaddr, NULL, acl, &ns_g_server->aclenv, &match, NULL);
...
static isc_result_t control_listen (controllistener_t * listener)
{
    ...
    if (result != ISC_R_SUCCESS)
        UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_socket_listen() failed: %s", isc_result_totext (result));
    return (result);
}
...
static inline void log_invalid (isccc_ccmsg_t * ccmsg, isc_result_t result)
{
    char socktext[ISC_SOCKADDR_FORMATSIZE];
    ...
    isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                   ...
                   "invalid command from %s: %s", socktext, isc_result_totext (result));
}
...
static void control_recvmessage (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        if (conn->ccmsg.result != ISC_R_CANCELED && conn->ccmsg.result != ISC_R_EOF)
    ...
    for (key = ISC_LIST_HEAD (listener->keys); key != NULL; key = ISC_LIST_NEXT (key, link))
    {
     ...
     */
    if (isccc_cc_lookupuint32 (_ctrl, "_exp", &exp) == ISC_R_SUCCESS && now > exp)
    {
    ...
    if (conn->nonce != 0 && (isccc_cc_lookupuint32 (_ctrl, "_nonce", &nonce) != ISC_R_SUCCESS || conn->nonce != nonce))
    {
        log_invalid (&conn->ccmsg, ISCCC_R_BADAUTH);
        goto cleanup_request;
    }
    ...
    if (_ctrl == NULL || isccc_cc_defineuint32 (_ctrl, "_nonce", conn->nonce) == NULL)
        goto cleanup_response;
...
static isc_result_t newconnection (controllistener_t * listener, isc_socket_t * sock)
{
    ...
    result = isc_timer_create (ns_g_timermgr, isc_timertype_once,
                               NULL, &interval, listener->task, control_timeout, conn, &conn->timer);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
...
static void controls_shutdown (ns_controls_t * controls)
{
    ...
    for (listener = ISC_LIST_HEAD (controls->listeners); listener != NULL; listener = next)
    {
        /*
    ...
    for (keyid = ISC_LIST_HEAD (*keyids); keyid != NULL; keyid = next)
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            cfg_obj_log (control, ns_g_lctx, ISC_LOG_WARNING,
                         "couldn't find key '%s' for use with " "command channel %s", keyid->keyname, socktext);
        ...
        else
        {
            ...
            if (ns_config_getkeyalgorithm (algstr, NULL, NULL) != ISC_R_SUCCESS)
            {
                cfg_obj_log (control, ns_g_lctx,
                             ...
                             "key '%s' for use with command " "channel %s", algstr, keyid->keyname, socktext);
                ISC_LIST_UNLINK (*keyids, keyid, link);
                free_controlkey (keyid, mctx);
                continue;
            }
            ...
            if (result != ISC_R_SUCCESS)
            {
                cfg_obj_log (keydef, ns_g_lctx, ISC_LOG_WARNING,
                             "secret for key '%s' on "
                             "command channel %s: %s", keyid->keyname, socktext, isc_result_totext (result));
                ISC_LIST_UNLINK (*keyids, keyid, link);
                free_controlkey (keyid, mctx);
                continue;
            }
            ...
            {
                cfg_obj_log (keydef, ns_g_lctx, ISC_LOG_WARNING,
                             "couldn't register key '%s': " "out of memory", keyid->keyname);
                ISC_LIST_UNLINK (*keyids, keyid, link);
                free_controlkey (keyid, mctx);
                break;
            }
            memcpy (keyid->secret.base, isc_buffer_base (&b), keyid->secret.length);
        }
    }
}
...
static isc_result_t get_rndckey (isc_mem_t * mctx, controlkeylist_t * keyids)
{
    ...
    keyid->keyname = isc_mem_strdup (mctx, cfg_obj_asstring (cfg_map_getname (key)));
    ...
    if (ns_config_getkeyalgorithm (algstr, NULL, NULL) != ISC_R_SUCCESS)
    {
        cfg_obj_log (key, ns_g_lctx,
                     ISC_LOG_WARNING,
                     "unsupported algorithm '%s' in "
                     "key '%s' for use with command " "channel", algstr, keyid->keyname);
        goto cleanup;
    }
    ...
    if (result != ISC_R_SUCCESS)
    {
        cfg_obj_log (key, ns_g_lctx, ISC_LOG_WARNING,
                     "secret for key '%s' on command channel: %s", keyid->keyname, isc_result_totext (result));
        goto cleanup;
    }
    ...
    if (keyid->secret.base == NULL)
    {
        cfg_obj_log (key, ns_g_lctx, ISC_LOG_WARNING, "couldn't register key '%s': " "out of memory", keyid->keyname);
        CHECK (ISC_R_NOMEMORY);
    }
    memcpy (keyid->secret.base, isc_buffer_base (&b), keyid->secret.length);
              ...
              isc_sockaddr_t * addr, cfg_aclconfctx_t * aclconfctx, const char *socktext, isc_sockettype_t type)
{
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        {
            allow = cfg_tuple_get (control, "allow");
            result = cfg_acl_fromconfig (allow, config, ns_g_lctx, aclconfctx, mctx, 0, &new_acl);
        }
        else
        {
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        if (control_keylist != NULL)
        {
            result = controlkeylist_fromcfg (control_keylist, listener->mctx, &listener->keys);
            if (result == ISC_R_SUCCESS)
                register_keys (control, global_keylist, &listener->keys, listener->mctx, socktext);
        }
        else
        ...
        if (result != ISC_R_SUCCESS && control != NULL)
            cfg_obj_log (control, ns_g_lctx, ISC_LOG_WARNING,
                         "couldn't install keys for " "command channel %s: %s", socktext, isc_result_totext (result));
    }
    ...
    if (result == ISC_R_SUCCESS)
        result = isc_socket_create (ns_g_socketmgr, isc_sockaddr_pf (&listener->address), type, &listener->sock);
    if (result == ISC_R_SUCCESS)
    ...
    if (result == ISC_R_SUCCESS)
        result = isc_socket_bind (listener->sock, &listener->address, ISC_SOCKET_REUSEADDRESS);

    if (result == ISC_R_SUCCESS && type == isc_sockettype_unix)
    {
        listener->perm = cfg_obj_asuint32 (cfg_tuple_get (control, "perm"));
        listener->owner = cfg_obj_asuint32 (cfg_tuple_get (control, "owner"));
        listener->group = cfg_obj_asuint32 (cfg_tuple_get (control, "group"));
        result = isc_socket_permunix (&listener->address, listener->perm, listener->owner, listener->group);
    }
    if (result == ISC_R_SUCCESS)
    ...
    {
        isc_log_write (ns_g_lctx, NS_LOGCATEGORY_GENERAL,
                       NS_LOGMODULE_CONTROL, ISC_LOG_NOTICE, "command channel listening on %s", socktext);
        *listenerp = listener;
...
isc_result_t ns_controls_configure (ns_controls_t * cp, const cfg_obj_t * config, cfg_aclconfctx_t * aclconfctx)
{
    ...
    if (controlslist != NULL)
    {
        for (element = cfg_list_first (controlslist); element != NULL; element = cfg_list_next (element))
        {
            ...
            for (element2 = cfg_list_first (inetcontrols); element2 != NULL; element2 = cfg_list_next (element2))
            {
                ...
                isc_log_write (ns_g_lctx,
                               ...
                               NS_LOGMODULE_CONTROL, ISC_LOG_DEBUG (9), "processing control channel %s", socktext);

                update_listener (cp, &listener, control, config, &addr, aclconfctx, socktext, isc_sockettype_tcp);
                ...
                if (listener != NULL)
                     ...
                     */
                    add_listener (cp, &listener, control, config, &addr, aclconfctx, socktext, isc_sockettype_tcp);
        ...
        for (element = cfg_list_first (controlslist); element != NULL; element = cfg_list_next (element))
        {
            ...
            for (element2 = cfg_list_first (unixcontrols); element2 != NULL; element2 = cfg_list_next (element2))
            {
                ...
                result = isc_sockaddr_frompath (&addr, cfg_obj_asstring (path));
                if (result != ISC_R_SUCCESS)
                {
                    isc_log_write (ns_g_lctx,
                                   NS_LOGCATEGORY_GENERAL,
                                   ...
                                   ISC_LOG_DEBUG (9),
                                   "control channel '%s': %s", cfg_obj_asstring (path), isc_result_totext (result));
                    continue;
                }
                ...
                isc_log_write (ns_g_lctx,
                               NS_LOGCATEGORY_GENERAL,
                               NS_LOGMODULE_CONTROL,
                               ISC_LOG_DEBUG (9), "processing control channel '%s'", cfg_obj_asstring (path));

                update_listener (cp, &listener, control, config,
                                 &addr, aclconfctx, cfg_obj_asstring (path), isc_sockettype_unix);
                ...
                if (listener != NULL)
                    ...
                    add_listener (cp, &listener, control,
                                  config, &addr, aclconfctx, cfg_obj_asstring (path), isc_sockettype_unix);
    ...
    else
    {
        ...
        for (i = 0; i < 2; i++)
        {
            ...
            update_listener (cp, &listener, NULL, NULL, &addr, NULL, socktext, isc_sockettype_tcp);
            ...
            if (listener != NULL)
                 ...
                 * This is a new listener.
                 */
                add_listener (cp, &listener, NULL, NULL, &addr, NULL, socktext, isc_sockettype_tcp);
