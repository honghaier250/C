 ...
 * warn when it isn't.
 */
static void version_check (const char *progname)
{
...
{
    if (devnullfd != _fileno (stdin) && devnullfd != _fileno (stdout) && devnullfd != _fileno (stderr))
    {
        close (devnullfd);
        devnullfd = -1;
    }
...
static int safe_open (const char *filename, int mode, isc_boolean_t append)
{
    int fd;
...
FILE *ns_os_openfile (const char *filename, int mode, isc_boolean_t switch_user)
{
...
void ns_os_writepidfile (const char *filename, isc_boolean_t first_time)
{
    ...
    lockfile = ns_os_openfile (filename, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH, ISC_FALSE);
    if (lockfile == NULL)
    {
        free (pidfile);
        pidfile = NULL;
