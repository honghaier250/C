...
static int t_initconf (const char *path);

static int t_dumpconf (const char *path);
...
static void t_sighandler (int sig)
{
    T_int = sig;
}
...
int main (int argc, char **argv)
{
    ...
    while ((c = isc_commandline_parse (argc, argv, ":at:c:d:n:huxq:b:")) != -1)
    {
        ...
        else if (c == 'n')
        {
            ...
            while (pts->pfv != NULL)
            {
                if (!strcmp (pts->func_name, isc_commandline_argument))
                {
                    if (first)
                    {
                        memset (T_tvec, 0, sizeof (T_tvec));
            ...
            if (subprocs)
            {
                ...
                else if (T_pid > 0)
                {
                    ...
                    while (deadpid != T_pid)
                    {
                        ...
                        else if ((deadpid == -1) && ((errno == ECHILD) || (errno == ESRCH)))
                            break;
                    }
...
void t_assert (const char *component, int anum, int class, const char *what, ...)
{
    va_list args;

    (void) printf ("T:%s:%d:%s\n", component, anum, class == T_REQUIRED ? "A" : "C");
...
isc_result_t t_dns_result_fromtext (char *name)
{
...
unsigned int t_dc_method_fromtext (char *name)
{
    unsigned int dc_method;
