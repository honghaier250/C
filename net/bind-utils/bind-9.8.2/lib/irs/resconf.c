...
static isc_result_t add_server (isc_mem_t * mctx, const char *address_str, isc_sockaddrlist_t * nameservers)
{
    int error;
...
static isc_result_t create_addr (const char *buffer, isc_netaddr_t * addr, int convert_zero)
{
    struct in_addr v4;
...
static isc_result_t resconf_parsesortlist (irs_resconf_t * conf, FILE * fp)
{
    ...
    while (strlen (word) > 0U)
    {
        ...
        else
        {
            ...
            memset (&conf->sortlist[idx].mask.type, 0xff, sizeof (conf->sortlist[idx].mask.type));
        }
...
static isc_result_t resconf_parseoption (irs_resconf_t * conf, FILE * fp)
{
    int delim;
...
static isc_result_t add_search (irs_resconf_t * conf, char *domain)
{
...
isc_result_t irs_resconf_load (isc_mem_t * mctx, const char *filename, irs_resconf_t ** confp)
{
    FILE *fp = NULL;
...
void irs_resconf_destroy (irs_resconf_t ** confp)
{
    irs_resconf_t *conf;
...
isc_sockaddrlist_t *irs_resconf_getnameservers (irs_resconf_t * conf)
{
    REQUIRE (IRS_RESCONF_VALID (conf));
...
irs_resconf_searchlist_t *irs_resconf_getsearchlist (irs_resconf_t * conf)
{
    REQUIRE (IRS_RESCONF_VALID (conf));
...
unsigned int irs_resconf_getndots (irs_resconf_t * conf)
{
    REQUIRE (IRS_RESCONF_VALID (conf));
