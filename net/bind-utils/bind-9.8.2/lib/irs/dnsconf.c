...
static isc_result_t configure_dnsseckeys (irs_dnsconf_t * conf, cfg_obj_t * cfgobj, dns_rdataclass_t rdclass)
{
    ...
    for (element = cfg_list_first (keys); element != NULL; element = cfg_list_next (element))
    {
        ...
        for (element2 = cfg_list_first (keylist); element2 != NULL; element2 = cfg_list_next (element2))
        {
            ...
            result = dns_rdata_fromstruct (NULL,
                                           keystruct.common.rdclass, keystruct.common.rdtype, &keystruct, &rrdatabuf);
            if (result != ISC_R_SUCCESS)
                return (result);
            ...
            result = dns_name_fromtext (keyname_base, &namebuf, dns_rootname, 0, NULL);
            if (result != ISC_R_SUCCESS)
                return (result);
...
isc_result_t irs_dnsconf_load (isc_mem_t * mctx, const char *filename, irs_dnsconf_t ** confp)
{
    irs_dnsconf_t *conf;
...
void irs_dnsconf_destroy (irs_dnsconf_t ** confp)
{
    irs_dnsconf_t *conf;
...
irs_dnsconf_dnskeylist_t *irs_dnsconf_gettrustedkeys (irs_dnsconf_t * conf)
{
    REQUIRE (IRS_DNSCONF_VALID (conf));
