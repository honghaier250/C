...
static struct addrinfo
    ...
    *ai_clone (struct addrinfo *oai, int family), *ai_alloc (int family, int addrlen);
#ifdef AF_LOCAL
static int get_local (const char *name, int socktype, struct addrinfo **res);
#endif

static int resolve_name (int family, const char *hostname, int flags, struct addrinfo **aip, int socktype, int port);
...
int getaddrinfo (const char *hostname, const char *servname, const struct addrinfo *hints, struct addrinfo **res)
{
    ...
    if (hostname != NULL && (family == AF_LOCAL || (family == 0 && *hostname == '/')))
        return (get_local (hostname, socktype, res));

    if (servname != NULL && (family == AF_LOCAL || (family == 0 && *servname == '/')))
    ...
    if (hostname != NULL && (family == 0 || (flags & AI_NUMERICHOST) != 0))
    {
        ...
        if (strchr (hostname, '%') != NULL)
        {
            ...
            if (p != NULL)
                scopeid = (isc_uint32_t) strtoul (p + 1, &ep, 10);
        ...
        else if (ntmp[0] != '\0' && inet_pton (AF_INET6, ntmp, abuf) == 1)
        {
...
#endif
                if (getnameinfo (ai->ai_addr, ai->ai_addrlen, nbuf, sizeof (nbuf), NULL, 0, NI_NUMERICHOST) == 0)
                {
    ...
    if (hostname == NULL && (flags & AI_PASSIVE) == 0)
    {
        ...
        err = resolve_name (family, hostname, flags, &ai_list, socktype, port);
               ...
               const char *domain, gai_resstate_t ** statep)
{
    ...
    if (need_v6)
    {
        ...
        {
            if (state->trans4 != NULL)
                isc_mem_put (mctx, state->trans4, sizeof (*state->trans4));
            isc_mem_put (mctx, state, sizeof (*state));
            return (ISC_R_NOMEMORY);
        }
...
static isc_result_t
make_resstates (isc_mem_t * mctx, const char *hostname, gai_statehead_t * head, irs_resconf_t * resconf)
{
    isc_result_t result;
    ...
    for (searchent = ISC_LIST_HEAD (*searchlist); searchent != NULL; searchent = ISC_LIST_NEXT (searchent, link))
    {
        resstate = NULL;
        result = make_resstate (mctx, head, hostname, (const char *) searchent->domain, &resstate);
        if (result != ISC_R_SUCCESS)
            break;
     ...
     */
    if (dns_name_countlabels (resstate0->qname) > irs_resconf_getndots (resconf) + 1)
    {
    ...
    if (result != ISC_R_SUCCESS)
    {
        while ((resstate = ISC_LIST_HEAD (head->resstates)) != NULL)
        {
            ...
            {
                isc_mem_put (mctx, resstate->trans4, sizeof (*resstate->trans4));
            }
            if (resstate->trans6 != NULL)
            {
                isc_mem_put (mctx, resstate->trans6, sizeof (*resstate->trans6));
            }
...
static void process_answer (isc_task_t * task, isc_event_t * event)
{
    int error = 0, family;
            ...
            for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
            {
                ...
                switch (family)
                {
                    case AF_INET:
                        ...
                        SIN (ai->ai_addr)->sin_port = resstate->head->ai_port;
                        memcpy (&SIN (ai->ai_addr)->sin_addr, &rdata_a.in_addr, 4);
                        dns_rdata_freestruct (&rdata_a);
                        break;
                    case AF_INET6:
                        dns_rdataset_current (rdataset, &rdata);
                        dns_rdata_tostruct (&rdata, &rdata_aaaa, NULL);
                        SIN6 (ai->ai_addr)->sin6_port = resstate->head->ai_port;
                        memcpy (&SIN6 (ai->ai_addr)->sin6_addr, &rdata_aaaa.in6_addr, 16);
                ...
                if ((resstate->head->ai_flags & AI_CANONNAME) != 0)
                {
                    ai->ai_canonname = strdup ((const char *) r.base);
                    if (ai->ai_canonname == NULL)
                    {
                        error = EAI_MEMORY;
                        goto done;
        ...
        (resstate->trans6 == NULL || !resstate->trans6->is_inprogress))
    {
        ...
        if (resstate == ISC_LIST_HEAD (resstate->head->resstates))
        {
            if ((resstate->trans4 != NULL &&
                ...
                (resstate->trans6 != NULL && resstate->trans6->ai_sentinel.ai_next != NULL))
            {
                gai_resstate_t *rest;

                for (rest = ISC_LIST_NEXT (resstate, link); rest != NULL; rest = ISC_LIST_NEXT (rest, link))
                {
                    if (rest->trans4 != NULL && rest->trans4->xid != NULL)
                        dns_client_cancelresolve (rest->trans4->xid);
                    if (rest->trans6 != NULL && rest->trans6->xid != NULL)
                        dns_client_cancelresolve (rest->trans6->xid);
                }
            }
            else
            {
                 ...
                 */
                ISC_LIST_UNLINK (resstate->head->resstates, resstate, link);
                ISC_LIST_APPEND (resstate->head->resstates, resstate, link);
            }
        }
    }
}
...
static int resolve_name (int family, const char *hostname, int flags, struct addrinfo **aip, int socktype, int port)
{
    ...
    for (resstate = ISC_LIST_HEAD (head.resstates); resstate != NULL; resstate = ISC_LIST_NEXT (resstate, link))
    {
        if (resstate->trans4 != NULL)
        {
            result = dns_client_startresolve (client,
                                              ...
                                              dns_rdataclass_in,
                                              dns_rdatatype_a,
                                              0, task, process_answer, resstate->trans4, &resstate->trans4->xid);
            if (result == ISC_R_SUCCESS)
            {
        ...
        if (resstate->trans6 != NULL)
        {
            result = dns_client_startresolve (client,
                                              ...
                                              dns_rdataclass_in,
                                              dns_rdatatype_aaaa,
                                              0, task, process_answer, resstate->trans6, &resstate->trans6->xid);
            if (result == ISC_R_SUCCESS)
            {
    ...
    while ((resstate = ISC_LIST_HEAD (head.resstates)) != NULL)
    {
        ...
        if (*aip == NULL)
        {
            ...
            if (resstate->trans4 != NULL)
            {
                sentinel4 = resstate->trans4->ai_sentinel.ai_next;
                resstate->trans4->ai_sentinel.ai_next = NULL;
            }
            if (resstate->trans6 != NULL)
            {
                sentinel6 = resstate->trans6->ai_sentinel.ai_next;
...
static char *irs_strsep (char **stringp, const char *delim)
{
    char *string = *stringp;
...
static void set_order (int family, int (**net_order) (const char *, int, struct addrinfo **, int, int))
{
    char *order, *tok;
...
static struct addrinfo *ai_concat (struct addrinfo *ai1, struct addrinfo *ai2)
{
    ...
    for (ai_tmp = ai1; ai_tmp != NULL && ai_tmp->ai_next != NULL; ai_tmp = ai_tmp->ai_next)
        ;
