...
static void free_specific_context (void *arg)
{
...
static isc_result_t thread_key_init ()
{
    isc_result_t result;
    ...
    if (!thread_key_initialized)
    {
        ...
        if (!thread_key_initialized && isc_thread_key_create (&irs_context_key, free_specific_context) != 0)
        {
...
isc_result_t irs_context_get (irs_context_t ** contextp)
{
...
isc_result_t irs_context_create (irs_context_t ** contextp)
{
    isc_result_t result;
    ...
    result = dns_client_setservers (client, dns_rdataclass_in, NULL, nameservers);
    if (result != ISC_R_SUCCESS)
        goto fail;
    ...
    for (trustedkey = ISC_LIST_HEAD (*trustedkeys); trustedkey != NULL; trustedkey = ISC_LIST_NEXT (trustedkey, link))
    {
        result = dns_client_addtrustedkey (client, dns_rdataclass_in, trustedkey->keyname, trustedkey->keydatabuf);
        if (result != ISC_R_SUCCESS)
            goto fail;
    }
...
void irs_context_destroy (irs_context_t ** contextp)
{
    irs_context_t *context;
    ...
    ctxs_destroy (NULL, &context->actx, &context->taskmgr, &context->socketmgr, &context->timermgr);
...
isc_mem_t *irs_context_getmctx (irs_context_t * context)
{
    REQUIRE (IRS_CONTEXT_VALID (context));
...
isc_appctx_t *irs_context_getappctx (irs_context_t * context)
{
    REQUIRE (IRS_CONTEXT_VALID (context));
...
isc_taskmgr_t *irs_context_gettaskmgr (irs_context_t * context)
{
    REQUIRE (IRS_CONTEXT_VALID (context));
...
isc_timermgr_t *irs_context_gettimermgr (irs_context_t * context)
{
    REQUIRE (IRS_CONTEXT_VALID (context));
...
isc_task_t *irs_context_gettask (irs_context_t * context)
{
    REQUIRE (IRS_CONTEXT_VALID (context));
...
dns_client_t *irs_context_getdnsclient (irs_context_t * context)
{
    REQUIRE (IRS_CONTEXT_VALID (context));
...
irs_resconf_t *irs_context_getresconf (irs_context_t * context)
{
    REQUIRE (IRS_CONTEXT_VALID (context));
...
irs_dnsconf_t *irs_context_getdnsconf (irs_context_t * context)
{
    REQUIRE (IRS_CONTEXT_VALID (context));
