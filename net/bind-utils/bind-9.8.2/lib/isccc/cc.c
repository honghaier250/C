...
static isc_result_t table_towire (isccc_sexpr_t * alist, isccc_region_t * target);

static isc_result_t list_towire (isccc_sexpr_t * alist, isccc_region_t * target);

static isc_result_t value_towire (isccc_sexpr_t * elt, isccc_region_t * target)
{
    size_t len;
...
static isc_result_t table_towire (isccc_sexpr_t * alist, isccc_region_t * target)
{
    ...
    for (elt = isccc_alist_first (alist); elt != NULL; elt = ISCCC_SEXPR_CDR (elt))
    {
...
static isc_result_t list_towire (isccc_sexpr_t * list, isccc_region_t * target)
{
    isc_result_t result;
...
static isc_result_t sign (unsigned char *data, unsigned int length, unsigned char *hmd5, isccc_region_t * secret)
{
    isc_hmacmd5_t ctx;
...
isc_result_t isccc_cc_towire (isccc_sexpr_t * alist, isccc_region_t * target, isccc_region_t * secret)
{
    ...
    if (result != ISC_R_SUCCESS)
        return (result);
    if (secret != NULL)
        return (sign (signed_rstart, (target->rstart - signed_rstart), hmd5_rstart, secret));
    return (ISC_R_SUCCESS);
}

static isc_result_t verify (isccc_sexpr_t * alist, unsigned char *data, unsigned int length, isccc_region_t * secret)
{
    isc_hmacmd5_t ctx;
...
static isc_result_t table_fromwire (isccc_region_t * source, isccc_region_t * secret, isccc_sexpr_t ** alistp);

static isc_result_t list_fromwire (isccc_region_t * source, isccc_sexpr_t ** listp);

static isc_result_t value_fromwire (isccc_region_t * source, isccc_sexpr_t ** valuep)
{
    ...
    *alistp = alist;
    ...
    if (secret != NULL)
    {
        if (checksum_rstart != NULL)
            result = verify (alist, checksum_rstart, (source->rend - checksum_rstart), secret);
        else
...
isc_result_t isccc_cc_fromwire (isccc_region_t * source, isccc_sexpr_t ** alistp, isccc_region_t * secret)
{
    unsigned int size;
...
createmessage (isc_uint32_t version, const char *from, const char *to,
               ...
               isccc_time_t expires, isccc_sexpr_t ** alistp, isc_boolean_t want_expires)
{
    ...
    if (isccc_cc_defineuint32 (_ctrl, "_ser", serial) == NULL ||
        ...
        (want_expires && isccc_cc_defineuint32 (_ctrl, "_exp", expires) == NULL))
        goto bad;
...
isccc_cc_createmessage (isc_uint32_t version, const char *from, const char *to,
                        isc_uint32_t serial, isccc_time_t now, isccc_time_t expires, isccc_sexpr_t ** alistp)
{
    return (createmessage (version, from, to, serial, now, expires, alistp, ISC_TRUE));
}

isc_result_t isccc_cc_createack (isccc_sexpr_t * message, isc_boolean_t ok, isccc_sexpr_t ** ackp)
{
    char *_frm, *_to;
...
isc_result_t
isccc_cc_createresponse (isccc_sexpr_t * message, isccc_time_t now, isccc_time_t expires, isccc_sexpr_t ** alistp)
{
    char *_frm, *_to, *type;
     ...
     */
    alist = NULL;
    result = isccc_cc_createmessage (1, _to, _frm, serial, now, expires, &alist);
    if (result != ISC_R_SUCCESS)
        return (result);
    ...
    if (_data == NULL)
        return (ISC_R_FAILURE);
    if (isccc_cc_definestring (_ctrl, "_rpl", "1") == NULL || isccc_cc_definestring (_data, "type", type) == NULL)
    {
        isccc_sexpr_free (&alist);
        return (ISC_R_NOMEMORY);
    }
...
isccc_sexpr_t *isccc_cc_definestring (isccc_sexpr_t * alist, const char *key, const char *str)
{
    size_t len;
...
isccc_sexpr_t *isccc_cc_defineuint32 (isccc_sexpr_t * alist, const char *key, isc_uint32_t i)
{
    char b[100];
...
isc_result_t isccc_cc_lookupstring (isccc_sexpr_t * alist, const char *key, char **strp)
{
    isccc_sexpr_t *kv, *v;
...
isc_result_t isccc_cc_lookupuint32 (isccc_sexpr_t * alist, const char *key, isc_uint32_t * uintp)
{
    isccc_sexpr_t *kv, *v;
    ...
    if (kv != NULL)
    {
        ...
        {
            if (uintp != NULL)
                *uintp = (isc_uint32_t) strtoul (isccc_sexpr_tostring (v), NULL, 10);
            return (ISC_R_SUCCESS);
        }
        else
            return (ISC_R_EXISTS);
...
static void symtab_undefine (char *key, unsigned int type, isccc_symvalue_t value, void *arg)
{
    UNUSED (type);
    UNUSED (value);
    UNUSED (arg);
...
static isc_boolean_t symtab_clean (char *key, unsigned int type, isccc_symvalue_t value, void *arg)
{
    isccc_time_t *now;
...
{
    return (isccc_symtab_create (11897, symtab_undefine, NULL, ISC_FALSE, symtabp));
}

void isccc_cc_cleansymtab (isccc_symtab_t * symtab, isccc_time_t now)
{
    isccc_symtab_foreach (symtab, symtab_clean, &now);
}

static isc_boolean_t has_whitespace (const char *str)
{
    char c;
...
isc_result_t isccc_cc_checkdup (isccc_symtab_t * symtab, isccc_sexpr_t * message, isccc_time_t now)
{
    const char *_frm;
     ...
     */
    if (has_whitespace (_frm) || has_whitespace (_to) || has_whitespace (_ser) || has_whitespace (_tim))
        return (ISC_R_FAILURE);
    ...
    result = isccc_symtab_define (symtab, key, ISCCC_SYMTYPE_CCDUP, value, isccc_symexists_reject);
    if (result != ISC_R_SUCCESS)
    {
        free (key);
        return (result);
