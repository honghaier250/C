...
isccc_symtab_create (unsigned int size,
                     isccc_symtabundefaction_t undefine_action,
                     void *undefine_arg, isc_boolean_t case_sensitive, isccc_symtab_t ** symtabp)
{
...
static inline void free_elt (isccc_symtab_t * symtab, unsigned int bucket, elt_t * elt)
{
    ISC_LIST_UNLINK (symtab->table[bucket], elt, link);
    if (symtab->undefine_action != NULL)
        (symtab->undefine_action) (elt->key, elt->type, elt->value, symtab->undefine_arg);
    free (elt);
}
...
void isccc_symtab_destroy (isccc_symtab_t ** symtabp)
{
    ...
    {
        for (elt = ISC_LIST_HEAD (symtab->table[i]); elt != NULL; elt = nelt)
        {
...
static inline unsigned int hash (const char *key, isc_boolean_t case_sensitive)
{
    const char *s;
