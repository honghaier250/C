...
static void recv_length (isc_task_t * task, isc_event_t * ev_in)
{
    ...
    result = isc_socket_recv (ccmsg->sock, &region, 0, task, recv_message, ccmsg);
...
static void recv_message (isc_task_t * task, isc_event_t * ev_in)
{
    isc_socketevent_t *ev = (isc_socketevent_t *) ev_in;
...
isc_result_t isccc_ccmsg_readmessage (isccc_ccmsg_t * ccmsg, isc_task_t * task, isc_taskaction_t action, void *arg)
{
    ...
    ISC_EVENT_INIT (&ccmsg->event, sizeof (isc_event_t), 0, 0, ISCCC_EVENT_CCMSG, action, arg, ccmsg, NULL, NULL);

    region.base = (unsigned char *) &ccmsg->size;
    region.length = 4;            /* isc_uint32_t */
    result = isc_socket_recv (ccmsg->sock, &region, 0, ccmsg->task, recv_length, ccmsg);
...
#if 0
void isccc_ccmsg_freebuffer (isccc_ccmsg_t * ccmsg)
{
    REQUIRE (VALID_CCMSG (ccmsg));
