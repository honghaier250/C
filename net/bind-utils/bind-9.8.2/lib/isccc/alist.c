...
isccc_sexpr_t *isccc_alist_assq (isccc_sexpr_t * alist, const char *key)
{
    ...
    while (alist != NULL)
    {
        ...
        caar = CAR (car);
        if (caar->type == ISCCC_SEXPRTYPE_STRING && strcmp (caar->value.as_string, key) == 0)
            return (car);
        alist = CDR (alist);
    }
...
isccc_sexpr_t *isccc_alist_define (isccc_sexpr_t * alist, const char *key, isccc_sexpr_t * value)
{
...
isccc_sexpr_t *isccc_alist_definestring (isccc_sexpr_t * alist, const char *key, const char *str)
{
    isccc_sexpr_t *v, *kv;
...
isccc_sexpr_t *isccc_alist_definebinary (isccc_sexpr_t * alist, const char *key, isccc_region_t * r)
{
    isccc_sexpr_t *v, *kv;
...
isccc_sexpr_t *isccc_alist_lookup (isccc_sexpr_t * alist, const char *key)
{
    isccc_sexpr_t *kv;
...
void isccc_alist_prettyprint (isccc_sexpr_t * sexpr, unsigned int indent, FILE * stream)
{
    ...
    if (isccc_alist_alistp (sexpr))
    {
        fprintf (stream, "{\n");
        indent += 4;
        for (elt = isccc_alist_first (sexpr); elt != NULL; elt = CDR (elt))
        {
            kv = CAR (elt);
            ...
            fprintf (stream, "%.*s%s => ", (int) indent, spaces, isccc_sexpr_tostring (k));
