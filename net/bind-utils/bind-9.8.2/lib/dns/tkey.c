...
process_dhtkey (dns_message_t * msg, dns_name_t * signer, dns_name_t * name,
                ...
                dns_rdata_tkey_t * tkeyout, dns_tsig_keyring_t * ring, dns_namelist_t * namelist)
{
    ...
    {
        tkey_log ("process_dhtkey: algorithms other than " "hmac-md5 are not supported");
        tkeyout->error = dns_tsigerror_badalg;
        return (ISC_R_SUCCESS);
    }
    ...
    for (result = dns_message_firstname (msg, DNS_SECTION_ADDITIONAL);
         result == ISC_R_SUCCESS && !found_key; result = dns_message_nextname (msg, DNS_SECTION_ADDITIONAL))
    {
        keyname = NULL;
        dns_message_currentname (msg, DNS_SECTION_ADDITIONAL, &keyname);
        keyset = NULL;
        result = dns_message_findtype (keyname, dns_rdatatype_key, 0, &keyset);
        if (result != ISC_R_SUCCESS)
            continue;

        for (result = dns_rdataset_first (keyset);
             result == ISC_R_SUCCESS && !found_key; result = dns_rdataset_next (keyset))
        {
            dns_rdataset_current (keyset, &keyrdata);
            pubkey = NULL;
            result = dns_dnssec_keyfromrdata (keyname, &keyrdata, msg->mctx, &pubkey);
            if (result != ISC_R_SUCCESS)
            {
    ...
    dns_rdata_fromregion (&ourkeyrdata, dns_rdataclass_any, dns_rdatatype_key, &ourkeyr);
    ...
    if (result != ISC_R_SUCCESS)
    {
        tkey_log ("process_dhtkey: failed to compute shared secret: %s", isc_result_totext (result));
        goto failure;
    }
    dst_key_free (&pubkey);
    ...
    result = isc_entropy_getdata (tctx->ectx, randomdata, TKEY_RANDOM_AMOUNT, NULL, 0);
    if (result != ISC_R_SUCCESS)
    {
        tkey_log ("process_dhtkey: failed to obtain entropy: %s", isc_result_totext (result));
        goto failure;
    }
    ...
    RETERR (dns_tsigkey_create (name, &tkeyin->algorithm,
                                ...
                                ISC_TRUE, signer, tkeyin->inception, tkeyin->expire, ring->mctx, ring, NULL));
...
process_gsstkey (dns_name_t * name, dns_rdata_tkey_t * tkeyin,
                 dns_tkeyctx_t * tctx, dns_rdata_tkey_t * tkeyout, dns_tsig_keyring_t * ring)
{
    isc_result_t result = ISC_R_SUCCESS;
    ...
    {
        tkey_log ("process_gsstkey(): no tkey-gssapi-credential " "or tkey-gssapi-keytab configured");
        return (ISC_R_NOPERM);
    }
    ...
    result = dst_gssapi_acceptctx (tctx->gsscred, tctx->gssapi_keytab,
                                   &intoken, &outtoken, &gss_ctx, dns_fixedname_name (&principal), tctx->mctx);
    ...
    if (tsigkey == NULL)
    {
        ...
        RETERR (dst_key_fromgssapi (name, gss_ctx, ring->mctx, &dstkey, &intoken));
        /*
        ...
        RETERR (dns_tsigkey_createfromkey (name, &tkeyin->algorithm,
                                           dstkey, ISC_TRUE,
                                           dns_fixedname_name (&principal), now, expire, ring->mctx, ring, NULL));
        dst_key_free (&dstkey);
    ...
    if (outtoken)
    {
        tkeyout->key = isc_mem_get (tkeyout->mctx, isc_buffer_usedlength (outtoken));
        if (tkeyout->key == NULL)
        {
        ...
        memcpy (tkeyout->key, isc_buffer_base (outtoken), isc_buffer_usedlength (outtoken));
...
process_deletetkey (dns_name_t * signer, dns_name_t * name,
                    dns_rdata_tkey_t * tkeyin, dns_rdata_tkey_t * tkeyout, dns_tsig_keyring_t * ring)
{
    isc_result_t result;
...
isc_result_t dns_tkey_processquery (dns_message_t * msg, dns_tkeyctx_t * tctx, dns_tsig_keyring_t * ring)
{
    ...
    switch (tkeyin.mode)
    {
        case DNS_TKEYMODE_DIFFIEHELLMAN:
            tkeyout.error = dns_rcode_noerror;
            RETERR (process_dhtkey (msg, signer, keyname, &tkeyin, tctx, &tkeyout, ring, &namelist));
            break;
        case DNS_TKEYMODE_GSSAPI:
            tkeyout.error = dns_rcode_noerror;
            RETERR (process_gsstkey (keyname, &tkeyin, tctx, &tkeyout, ring));
            break;
        case DNS_TKEYMODE_DELETE:
            tkeyout.error = dns_rcode_noerror;
            RETERR (process_deletetkey (signer, keyname, &tkeyin, &tkeyout, ring));
            break;
  ...
  failure_with_tkey:
    ...
    result = dns_rdata_fromstruct (&rdata, tkeyout.common.rdclass, tkeyout.common.rdtype, &tkeyout, &tkeyoutbuf);
...
static isc_result_t buildquery (dns_message_t * msg, dns_name_t * name, dns_rdata_tkey_t * tkey, isc_boolean_t win2k)
{
    ...
    dns_rdataset_makequestion (question, dns_rdataclass_any, dns_rdatatype_tkey);

    RETERR (isc_buffer_allocate (msg->mctx, &dynbuf, 4096));
    RETERR (dns_message_gettemprdata (msg, &rdata));

    RETERR (dns_rdata_fromstruct (rdata, dns_rdataclass_any, dns_rdatatype_tkey, tkey, dynbuf));
...
dns_tkey_builddhquery (dns_message_t * msg, dst_key_t * key, dns_name_t * name,
                       dns_name_t * algorithm, isc_buffer_t * nonce, isc_uint32_t lifetime)
{
...
dns_tkey_buildgssquery (dns_message_t * msg, dns_name_t * name, dns_name_t * gname,
                        ...
                        gss_ctx_id_t * context, isc_boolean_t win2k, isc_mem_t * mctx, char **err_message)
{
    dns_rdata_tkey_t tkey;
    ...
    result = dst_gssapi_initctx (gname, NULL, &token, context, mctx, err_message);
...
static isc_result_t find_tkey (dns_message_t * msg, dns_name_t ** name, dns_rdata_t * rdata, int section)
{
    ...
    while (result == ISC_R_SUCCESS)
    {
        *name = NULL;
        ...
        tkeyset = NULL;
        result = dns_message_findtype (*name, dns_rdatatype_tkey, 0, &tkeyset);
        if (result == ISC_R_SUCCESS)
        {
            result = dns_rdataset_first (tkeyset);
            if (result != ISC_R_SUCCESS)
...
dns_tkey_processdhresponse (dns_message_t * qmsg, dns_message_t * rmsg,
                            dst_key_t * key, isc_buffer_t * nonce, dns_tsigkey_t ** outkey, dns_tsig_keyring_t * ring)
{
    ...
    if (rtkey.error != dns_rcode_noerror ||
        ...
        !dns_name_equal (&rtkey.algorithm, &qtkey.algorithm) || rmsg->rcode != dns_rcode_noerror)
    {
        tkey_log ("dns_tkey_processdhresponse: tkey mode invalid " "or error set(1)");
    ...
    ourkeyname = NULL;
    ourkeyset = NULL;
    RETERR (dns_message_findname (rmsg, DNS_SECTION_ANSWER, &keyname, dns_rdatatype_key, 0, &ourkeyname, &ourkeyset));

    result = dns_message_firstname (rmsg, DNS_SECTION_ANSWER);
    while (result == ISC_R_SUCCESS)
    {
        theirkeyname = NULL;
        dns_message_currentname (rmsg, DNS_SECTION_ANSWER, &theirkeyname);
        if (dns_name_equal (theirkeyname, ourkeyname))
            goto next;
        theirkeyset = NULL;
        result = dns_message_findtype (theirkeyname, dns_rdatatype_key, 0, &theirkeyset);
        if (result == ISC_R_SUCCESS)
        {
    ...
    if (theirkeyset == NULL)
    {
        tkey_log ("dns_tkey_processdhresponse: failed to find server " "key");
        result = ISC_R_NOTFOUND;
        goto failure;
    }

    dns_rdataset_current (theirkeyset, &theirkeyrdata);
    RETERR (dns_dnssec_keyfromrdata (theirkeyname, &theirkeyrdata, rmsg->mctx, &theirkey));
    ...
    result = dns_tsigkey_create (tkeyname, &rtkey.algorithm,
                                 ...
                                 NULL, rtkey.inception, rtkey.expire, rmsg->mctx, ring, outkey);
    isc_buffer_free (&shared);
                             ...
                             dns_tsig_keyring_t * ring, char **err_message)
{
     ...
     */
    result = find_tkey (qmsg, &tkeyname, &qtkeyrdata, DNS_SECTION_ADDITIONAL);
    if (result == ISC_R_NOTFOUND)
        result = find_tkey (qmsg, &tkeyname, &qtkeyrdata, DNS_SECTION_ANSWER);
    if (result != ISC_R_SUCCESS)
        goto failure;
    ...
    if (rtkey.error != dns_rcode_noerror ||
        rtkey.mode != DNS_TKEYMODE_GSSAPI || !dns_name_equal (&rtkey.algorithm, &qtkey.algorithm))
    {
        tkey_log ("dns_tkey_processgssresponse: tkey mode invalid " "or error set(2) %d", rtkey.error);
    ...
    RETERR (dst_gssapi_initctx (gname, &intoken, outtoken, context, ring->mctx, err_message));

    RETERR (dst_key_fromgssapi (dns_rootname, *context, rmsg->mctx, &dstkey, NULL));
...
isc_result_t dns_tkey_processdeleteresponse (dns_message_t * qmsg, dns_message_t * rmsg, dns_tsig_keyring_t * ring)
{
    ...
    if (rtkey.error != dns_rcode_noerror ||
        ...
        !dns_name_equal (&rtkey.algorithm, &qtkey.algorithm) || rmsg->rcode != dns_rcode_noerror)
    {
        tkey_log ("dns_tkey_processdeleteresponse: tkey mode invalid " "or error set(3)");
        result = DNS_R_INVALIDTKEY;
        dns_rdata_freestruct (&qtkey);
        dns_rdata_freestruct (&rtkey);
        goto failure;
...
dns_tkey_gssnegotiate (dns_message_t * qmsg, dns_message_t * rmsg,
                       ...
                       dns_tsigkey_t ** outkey, dns_tsig_keyring_t * ring, isc_boolean_t win2k, char **err_message)
{
    dns_rdata_t rtkeyrdata = DNS_RDATA_INIT, qtkeyrdata = DNS_RDATA_INIT;
    ...
    if (win2k == ISC_TRUE)
        RETERR (find_tkey (qmsg, &tkeyname, &qtkeyrdata, DNS_SECTION_ANSWER));
    else
    ...
    if (rtkey.error != dns_rcode_noerror ||
        rtkey.mode != DNS_TKEYMODE_GSSAPI || !dns_name_equal (&rtkey.algorithm, &qtkey.algorithm))
    {
        tkey_log ("dns_tkey_processdhresponse: tkey mode invalid " "or error set(4)");
        result = DNS_R_INVALIDTKEY;
        goto failure;
    }
    ...
    result = dst_gssapi_initctx (server, &intoken, &outtoken, context, ring->mctx, err_message);
    if (result != DNS_R_CONTINUE && result != ISC_R_SUCCESS)
        return (result);
