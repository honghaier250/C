...
static void default_fromtext_callback (dns_rdatacallbacks_t * callbacks, const char *, ...) ISC_FORMAT_PRINTF (2, 3);
     ...
     static inline int
         getquad (const void *src, struct in_addr *dst, isc_lex_t * lexer, dns_rdatacallbacks_t * callbacks)
{
    int result;
    ...
    if (result == 1 && callbacks != NULL && inet_pton (AF_INET, src, &tmp) != 1)
    {
        ...
        (*callbacks->warn) (callbacks, "%s:%lu: \"%s\" "
                            "is not a decimal dotted quad", name, isc_lex_getsourceline (lexer), src);
    }
    return (result);
}
...
int dns_rdata_casecompare (const dns_rdata_t * rdata1, const dns_rdata_t * rdata2)
{
    ...
    CASECOMPARESWITCH if (use_default)
    {
        isc_region_t r1;
 ...
 */
static isc_result_t
rdata_validate (isc_buffer_t * src, isc_buffer_t * dest, dns_rdataclass_t rdclass, dns_rdatatype_t type)
{
...
dns_rdata_tofmttext (dns_rdata_t * rdata, dns_name_t * origin,
                     unsigned int flags, unsigned int width, const char *linebreak, isc_buffer_t * target)
{
    dns_rdata_textctx_t tctx;
...
isc_boolean_t
dns_rdata_checkowner (dns_name_t * name, dns_rdataclass_t rdclass, dns_rdatatype_t type, isc_boolean_t wildcard)
{
...
static isc_result_t txt_fromtext (isc_textregion_t * source, isc_buffer_t * target)
{
    ...
    while (n-- != 0)
    {
        ...
        escape = ISC_FALSE;
        if (nrem == 0)
            return ((tregion.length <= 256U) ? ISC_R_NOSPACE : DNS_R_SYNTAX);
        *t++ = c;
        nrem--;
    }
    if (escape)
...
static isc_result_t byte_btoa (int c, isc_buffer_t * target, struct state *state)
{
    ...
    if (bcount == 3)
    {
        ...
        else
        {
            ...
            if (tr.length < 5)
                return (ISC_R_NOSPACE);
            tr.base[0] = atob_digits[(tmpword / (isc_int32_t) (85 * 85 * 85 * 85)) + tmp];
...
static void warn_badmx (isc_token_t * token, isc_lex_t * lexer, dns_rdatacallbacks_t * callbacks)
{
    ...
    if (lexer != NULL)
    {
        ...
        (*callbacks->warn) (callbacks, "%s:%u: warning: '%s': %s",
                            file, line, DNS_AS_STR (*token), dns_result_totext (DNS_R_MXISADDRESS));
    }
}
...
static void warn_badname (dns_name_t * name, isc_lex_t * lexer, dns_rdatacallbacks_t * callbacks)
{
    ...
    if (lexer != NULL)
    {
        ...
        (*callbacks->warn) (callbacks, "%s:%u: warning: %s: %s",
                            file, line, namebuf, dns_result_totext (DNS_R_BADNAME));
    }
}
        ...
        switch (token->type)
        {
            case isc_tokentype_eol:
                (*callback) (callbacks, "%s: %s:%lu: near eol: %s",
                             "dns_rdata_fromtext", name, line, dns_result_totext (result));
                break;
            ...
            case isc_tokentype_number:
                (*callback) (callbacks, "%s: %s:%lu: near %lu: %s",
                             "dns_rdata_fromtext", name, line, token->value.as_ulong, dns_result_totext (result));
                break;
            case isc_tokentype_string:
            case isc_tokentype_qstring:
                (*callback) (callbacks, "%s: %s:%lu: near '%s': %s",
                             "dns_rdata_fromtext", name, line, DNS_AS_STR (*token), dns_result_totext (result));
                break;
            default:
...
{
    if ((dns_rdatatype_attributes (type) & DNS_RDATATYPEATTR_NOTQUESTION) != 0)
        return (ISC_TRUE);
    return (ISC_FALSE);
}
...
{
    if ((dns_rdatatype_attributes (type) & DNS_RDATATYPEATTR_QUESTIONONLY) != 0)
        return (ISC_TRUE);
    return (ISC_FALSE);
}
...
{
    if ((dns_rdatatype_attributes (type) & (DNS_RDATATYPEATTR_DNSSEC | DNS_RDATATYPEATTR_ZONECUTAUTH)) != 0)
        return (ISC_TRUE);
    return (ISC_FALSE);
}
