...
#define SEND_BUFFER_SIZE 2048
     ...
     static void notify_log (dns_zone_t * zone, int level, const char *fmt, ...) ISC_FORMAT_PRINTF (3, 4);
...
/* ondestroy example */
     ...
     static isc_result_t zonemgr_getio (dns_zonemgr_t * zmgr, isc_boolean_t high,
                                        isc_task_t * task, isc_taskaction_t action, void *arg, dns_io_t ** iop);
     ...
     static isc_result_t
         zone_get_from_db (dns_zone_t * zone, dns_db_t * db, unsigned int *nscount,
                           ...
                           isc_uint32_t * expire, isc_uint32_t * minimum, unsigned int *errors);
     ...
     static isc_result_t delete_nsec (dns_db_t * db, dns_dbversion_t * ver,
                                      dns_dbnode_t * node, dns_name_t * name, dns_diff_t * diff);
     static void zone_rekey (dns_zone_t * zone);

     static isc_boolean_t delsig_ok (dns_rdata_rrsig_t * rrsig_ptr, dst_key_t ** keys, unsigned int nkeys);
 ...
 */
static inline void inc_stats (dns_zone_t * zone, isc_statscounter_t counter)
{
    if (zone->stats != NULL)
...
isc_result_t dns_zone_create (dns_zone_t ** zonep, isc_mem_t * mctx)
{
    ...
    ISC_EVENT_INIT (&zone->ctlevent, sizeof (zone->ctlevent), 0, NULL,
                    DNS_EVENT_ZONECONTROL, zone_shutdown, zone, zone, NULL, NULL);
    *zonep = zone;
...
static void zone_free (dns_zone_t * zone)
{
    ...
    /* Unmanaged objects */
    for (signing = ISC_LIST_HEAD (zone->signing); signing != NULL; signing = ISC_LIST_HEAD (zone->signing))
    {
    ...
    for (nsec3chain = ISC_LIST_HEAD (zone->nsec3chain);
         nsec3chain != NULL; nsec3chain = ISC_LIST_HEAD (zone->nsec3chain))
    {
    ...
    RUNTIME_CHECK (dns_zone_setmasterswithkeys (zone, NULL, NULL, 0) == ISC_R_SUCCESS);
...
void dns_zone_setclass (dns_zone_t * zone, dns_rdataclass_t rdclass)
{
     ...
     */
    LOCK_ZONE (zone);
    REQUIRE (zone->rdclass == dns_rdataclass_none || zone->rdclass == rdclass);
...
isc_result_t dns_zone_getserial2 (dns_zone_t * zone, isc_uint32_t * serialp)
{
    ...
    if (zone->db != NULL)
    {
        result = zone_get_from_db (zone, zone->db, NULL, NULL, serialp, NULL, NULL, NULL, NULL, NULL);
    }
    else
...
static void zone_freedbargs (dns_zone_t * zone)
{
    ...
    if (zone->db_argv != NULL)
    {
        ...
        isc_mem_put (zone->mctx, zone->db_argv, zone->db_argc * sizeof (*zone->db_argv));
    }
    zone->db_argc = 0;
    zone->db_argv = NULL;
}
...
void dns_zone_setacache (dns_zone_t * zone, dns_acache_t * acache)
{
    ...
    if (zone->db != NULL)
    {
        ...
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__, "dns_acache_setdb() failed: %s", isc_result_totext (result));
        }
    }
    ZONEDB_UNLOCK (&zone->dblock, isc_rwlocktype_read);
    UNLOCK_ZONE (zone);
 ...
 */
static isc_boolean_t zone_isdynamic (dns_zone_t * zone)
{
    REQUIRE (DNS_ZONE_VALID (zone));
    ...
    return (ISC_TF (zone->type == dns_zone_slave ||
                    ...
                    (!zone->update_disabled && zone->update_acl != NULL && !dns_acl_isnone (zone->update_acl))));
}


static isc_result_t zone_load (dns_zone_t * zone, unsigned int flags)
{
    isc_result_t result;
    ...
    rbt = strcmp (zone->db_argv[0], "rbt") == 0 || strcmp (zone->db_argv[0], "rbt64") == 0;
    ...
    if (zone->masterfile != NULL)
    {
        ...
        if (!isc_time_isepoch (&zone->loadtime) &&
            (flags & DNS_ZONELOADFLAG_NOSTAT) != 0 && zone->rpz_zone == dns_rpz_needed ())
        {
            result = ISC_R_SUCCESS;
            goto cleanup;
        }
        ...
        if (result == ISC_R_SUCCESS)
        {
            if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_LOADED) &&
                ...
                isc_time_compare (&filetime, &zone->loadtime) <= 0 && zone->rpz_zone == dns_rpz_needed ())
            {
                dns_zone_log (zone, ISC_LOG_DEBUG (1), "skipping load: master file " "older than last load");
    ...
    if (zone->type == dns_zone_master &&
        ...
        (zone->db_argc < 2 || strcmp (zone->db_argv[1], "empty") != 0) && DNS_ZONE_FLAG (zone, DNS_ZONEFLG_LOADED))
    {
        result = ISC_R_SUCCESS;
        goto cleanup;
    }

    if ((zone->type == dns_zone_slave || zone->type == dns_zone_stub) && rbt)
    {
        if (zone->masterfile == NULL || !isc_file_exists (zone->masterfile))
        {
            if (zone->masterfile != NULL)
            {
                dns_zone_log (zone, ISC_LOG_DEBUG (1), "no master file");
            }
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "loading zone: creating database: %s", isc_result_totext (result));
        goto cleanup;
    }
    ...
    if (!dns_db_ispersistent (db))
    {
        ...
        else
        {
            ...
            if (zone->type == dns_zone_master)
            {
                dns_zone_log (zone, ISC_LOG_ERROR, "loading zone: " "no master file configured");
                goto cleanup;
            }
            dns_zone_log (zone, ISC_LOG_INFO, "loading zone: " "no master file configured: continuing");
        }
    }
...
static unsigned int get_master_options (dns_zone_t * zone)
{
    ...
    if (zone->type == dns_zone_master &&
        ((zone->update_acl != NULL && !dns_acl_isnone (zone->update_acl)) || zone->ssutable != NULL))
...
static void zone_gotreadhandle (isc_task_t * task, isc_event_t * event)
{
    ...
    result = dns_master_loadfileinc3 (load->zone->masterfile,
                                      ...
                                      &load->callbacks, task,
                                      zone_loaddone, load,
                                      &load->zone->lctx, load->zone->mctx, load->zone->masterformat);
    if (result != ISC_R_SUCCESS && result != DNS_R_CONTINUE && result != DNS_R_SEENINCLUDE)
        goto fail;
    return;
...
static void zone_gotwritehandle (isc_task_t * task, isc_event_t * event)
{
    ...
    result = dns_master_dumpinc2 (zone->mctx, zone->db, version,
                                  ...
                                  zone->masterfile, zone->task, dump_done, zone, &zone->dctx, zone->masterformat);
...
static isc_result_t zone_startload (dns_db_t * db, dns_zone_t * zone, isc_time_t loadtime)
{
    ...
    if (zone->zmgr != NULL && zone->db != NULL && zone->task != NULL)
    {
        ...
        result = dns_db_beginload (db, &load->callbacks.add, &load->callbacks.add_private);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
        result = zonemgr_getio (zone->zmgr, ISC_TRUE, zone->task, zone_gotreadhandle, load, &zone->readio);
        if (result != ISC_R_SUCCESS)
        {
            /*
             ...
             */
            (void) dns_db_endload (load->db, &load->callbacks.add_private);
            goto cleanup;
        }
        else
            result = DNS_R_CONTINUE;
    ...
    else
    {
        ...
        result = dns_master_loadfile3 (zone->masterfile, &zone->origin,
                                       ...
                                       options, zone->sigresigninginterval, &callbacks, zone->mctx, zone->masterformat);
...
static isc_boolean_t zone_check_mx (dns_zone_t * zone, dns_db_t * db, dns_name_t * name, dns_name_t * owner)
{
    ...
    result = dns_db_find (db, name, NULL, dns_rdatatype_a, 0, 0, NULL, foundname, NULL, NULL);
    if (result == ISC_R_SUCCESS)
        return (ISC_TRUE);

    if (result == DNS_R_NXRRSET)
    {
        result = dns_db_find (db, name, NULL, dns_rdatatype_aaaa, 0, 0, NULL, foundname, NULL, NULL);
        if (result == ISC_R_SUCCESS)
            return (ISC_TRUE);
    }
    ...
    if (result == DNS_R_NXRRSET || result == DNS_R_NXDOMAIN || result == DNS_R_EMPTYNAME)
    {
        if (!DNS_ZONE_OPTION (zone, DNS_ZONEOPT_CHECKMXFAIL))
            level = ISC_LOG_WARNING;
        dns_zone_log (zone, level, "%s/MX '%s' has no address records (A or AAAA)", ownerbuf, namebuf);
        return ((level == ISC_LOG_WARNING) ? ISC_TRUE : ISC_FALSE);
    }

    if (result == DNS_R_CNAME)
    {
        if (DNS_ZONE_OPTION (zone, DNS_ZONEOPT_WARNMXCNAME) || DNS_ZONE_OPTION (zone, DNS_ZONEOPT_IGNOREMXCNAME))
            level = ISC_LOG_WARNING;
        if (!DNS_ZONE_OPTION (zone, DNS_ZONEOPT_IGNOREMXCNAME))
            dns_zone_log (zone, level, "%s/MX '%s' is a CNAME (illegal)", ownerbuf, namebuf);
        return ((level == ISC_LOG_WARNING) ? ISC_TRUE : ISC_FALSE);
    }
    ...
    if (result == DNS_R_DNAME)
    {
        ...
        if (!DNS_ZONE_OPTION (zone, DNS_ZONEOPT_IGNOREMXCNAME))
        {
            ...
            dns_zone_log (zone, level, "%s/MX '%s' is below a DNAME" " '%s' (illegal)", ownerbuf, namebuf, altbuf);
        }
        return ((level == ISC_LOG_WARNING) ? ISC_TRUE : ISC_FALSE);
    }
...
static isc_boolean_t zone_check_srv (dns_zone_t * zone, dns_db_t * db, dns_name_t * name, dns_name_t * owner)
{
    ...
    if (result == DNS_R_NXRRSET || result == DNS_R_NXDOMAIN || result == DNS_R_EMPTYNAME)
    {
        dns_zone_log (zone, level, "%s/SRV '%s' has no address records (A or AAAA)", ownerbuf, namebuf);
    ...
    if (result == DNS_R_CNAME)
    {
        if (DNS_ZONE_OPTION (zone, DNS_ZONEOPT_WARNSRVCNAME) || DNS_ZONE_OPTION (zone, DNS_ZONEOPT_IGNORESRVCNAME))
            level = ISC_LOG_WARNING;
        if (!DNS_ZONE_OPTION (zone, DNS_ZONEOPT_IGNORESRVCNAME))
            dns_zone_log (zone, level, "%s/SRV '%s' is a CNAME (illegal)", ownerbuf, namebuf);
        return ((level == ISC_LOG_WARNING) ? ISC_TRUE : ISC_FALSE);
    }
    ...
    if (result == DNS_R_DNAME)
    {
        ...
        {
            dns_name_format (foundname, altbuf, sizeof altbuf);
            dns_zone_log (zone, level, "%s/SRV '%s' is below a " "DNAME '%s' (illegal)", ownerbuf, namebuf, altbuf);
        }
        return ((level == ISC_LOG_WARNING) ? ISC_TRUE : ISC_FALSE);
    }
...
static isc_boolean_t zone_check_glue (dns_zone_t * zone, dns_db_t * db, dns_name_t * name, dns_name_t * owner)
{
    ...
    result = dns_db_find (db, name, NULL, dns_rdatatype_a, DNS_DBFIND_GLUEOK, 0, NULL, foundname, &a, NULL);
    ...
    if (result == DNS_R_NXRRSET || result == DNS_R_DELEGATION || result == DNS_R_GLUE)
    {
        tresult = dns_db_find (db, name, NULL, dns_rdatatype_aaaa, DNS_DBFIND_GLUEOK, 0, NULL, foundname, &aaaa, NULL);
    ...
    dns_name_format (owner, ownerbuf, sizeof ownerbuf);
    dns_name_format (name, namebuf, sizeof namebuf);
    if (result == DNS_R_NXRRSET || result == DNS_R_NXDOMAIN || result == DNS_R_EMPTYNAME || result == DNS_R_DELEGATION)
    {
        const char *what;
        ...
        if (result != DNS_R_DELEGATION || required || DNS_ZONE_OPTION (zone, DNS_ZONEOPT_CHECKSIBLING))
        {
            dns_zone_log (zone, level, "%s/NS '%s' has no %s" "address records (A or AAAA)", ownerbuf, namebuf, what);
            /*
             * Log missing address record.
             */
            if (result == DNS_R_DELEGATION && zone->checkns != NULL)
                (void) (zone->checkns) (zone, name, owner, &a, &aaaa);
            /* XXX950 make fatal for 9.5.0. */
            /* answer = ISC_FALSE; */
        }
    }
    ...
    {
        dns_zone_log (zone, level, "%s/NS '%s' is a CNAME (illegal)", ownerbuf, namebuf);
        /* XXX950 make fatal for 9.5.0. */
        /* answer = ISC_FALSE; */
    }
    ...
    {
        dns_name_format (foundname, altbuf, sizeof altbuf);
        dns_zone_log (zone, level, "%s/NS '%s' is below a DNAME '%s' (illegal)", ownerbuf, namebuf, altbuf);
        /* XXX950 make fatal for 9.5.0. */
        /* answer = ISC_FALSE; */
    }
...
static isc_boolean_t zone_rrset_check_dup (dns_zone_t * zone, dns_name_t * owner, dns_rdataset_t * rdataset)
{
    ...
    for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
    {
        ...
        for (result = dns_rdataset_first (&tmprdataset);
             result == ISC_R_SUCCESS; result = dns_rdataset_next (&tmprdataset))
        {
            ...
            if (dns_rdata_casecompare (&rdata1, &rdata2) == 0)
            {
                ...
                {
                    dns_name_format (owner, ownerbuf, sizeof ownerbuf);
                    dns_rdatatype_format (rdata1.type, typebuf, sizeof (typebuf));
                    format = ISC_FALSE;
                }
                dns_zone_log (zone, level, "%s/%s has " "semantically identical records", ownerbuf, typebuf);
...
static isc_boolean_t zone_check_dup (dns_zone_t * zone, dns_db_t * db)
{
    ...
    for (result = dns_dbiterator_first (dbiterator); result == ISC_R_SUCCESS; result = dns_dbiterator_next (dbiterator))
    {
        ...
        for (result = dns_rdatasetiter_first (rdsit); result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (rdsit))
        {
...
static isc_boolean_t integrity_checks (dns_zone_t * zone, dns_db_t * db)
{
    ...
    while (result == ISC_R_SUCCESS)
    {
         ...
         */
        if (!dns_name_issubdomain (name, &zone->origin) ||
            (dns_name_countlabels (bottom) > 0 && dns_name_issubdomain (name, bottom)))
            goto next;
        ...
        result = dns_db_findrdataset (db, node, NULL, dns_rdatatype_ns, 0, 0, &rdataset, NULL);
        if (result != ISC_R_SUCCESS)
            goto checkmx;
        /*
      ...
      checkmx:
        result = dns_db_findrdataset (db, node, NULL, dns_rdatatype_mx, 0, 0, &rdataset, NULL);
        if (result != ISC_R_SUCCESS)
            goto checksrv;
        result = dns_rdataset_first (&rdataset);
        while (result == ISC_R_SUCCESS)
      ...
      checksrv:
        if (zone->rdclass != dns_rdataclass_in)
            goto next;
        result = dns_db_findrdataset (db, node, NULL, dns_rdatatype_srv, 0, 0, &rdataset, NULL);
        if (result != ISC_R_SUCCESS)
            goto next;
        result = dns_rdataset_first (&rdataset);
        while (result == ISC_R_SUCCESS)
...
static void zone_check_dnskeys (dns_zone_t * zone, dns_db_t * db)
{
    ...
    dns_rdataset_init (&rdataset);
    result = dns_db_findrdataset (db, node, version, dns_rdatatype_dnskey, dns_rdatatype_none, 0, &rdataset, NULL);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        if ((dnskey.algorithm == DST_ALG_RSASHA1 ||
             dnskey.algorithm == DST_ALG_RSAMD5) && dnskey.datalen > 1 && dnskey.data[0] == 1 && dnskey.data[1] == 3)
        {
            if (dnskey.algorithm == DST_ALG_RSASHA1)
            {
                logit = !foundrsa;
            ...
            if (logit)
                dns_zone_log (zone, ISC_LOG_WARNING,
                              "weak %s (%u) key found " "(exponent=3)", algorithm, dnskey.algorithm);
...
static void resume_signingwithkey (dns_zone_t * zone)
{
    ...
    dns_db_currentversion (zone->db, &version);
    dns_rdataset_init (&rdataset);
    result = dns_db_findrdataset (zone->db, node, version, zone->privatetype, dns_rdatatype_none, 0, &rdataset, NULL);
    if (result != ISC_R_SUCCESS)
    {
    ...
    {
        dns_rdataset_current (&rdataset, &rdata);
        if (rdata.length != 5 || rdata.data[0] == 0 || rdata.data[4] != 0)
        {
            dns_rdata_reset (&rdata);
            continue;
        }

        result = zone_signwithkey (zone, rdata.data[0], (rdata.data[1] << 8) | rdata.data[2], ISC_TF (rdata.data[3]));
        if (result != ISC_R_SUCCESS)
        {
            dns_zone_log (zone, ISC_LOG_ERROR, "zone_signwithkey failed: %s", dns_result_totext (result));
        }
        dns_rdata_reset (&rdata);
    }
    dns_rdataset_disassociate (&rdataset);
...
static isc_result_t zone_addnsec3chain (dns_zone_t * zone, dns_rdata_nsec3param_t * nsec3param)
{
    ...
    dns_zone_log (zone, ISC_LOG_INFO,
                  "zone_addnsec3chain(%u,%s,%u,%s)", nsec3param->hash, flags, nsec3param->iterations, saltbuf);
    for (current = ISC_LIST_HEAD (zone->nsec3chain); current != NULL; current = ISC_LIST_NEXT (current, link))
    {
        if (current->db == zone->db &&
            ...
            && !memcmp (current->nsec3param.salt, nsec3param->salt, nsec3param->salt_length))
    ...
    if (zone->db != NULL)
    {
        ...
        result = dns_db_createiterator (nsec3chain->db, options, &nsec3chain->dbiterator);
        if (result == ISC_R_SUCCESS)
...
static void resume_addnsec3chain (dns_zone_t * zone)
{
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        if (!dns_nsec3param_fromprivate (&private, &rdata, buf, sizeof (buf)))
            continue;
        result = dns_rdata_tostruct (&rdata, &nsec3param, NULL);
        RUNTIME_CHECK (result == ISC_R_SUCCESS);
        if ((nsec3param.flags & DNS_NSEC3FLAG_CREATE) != 0 || (nsec3param.flags & DNS_NSEC3FLAG_REMOVE) != 0)
        {
            result = zone_addnsec3chain (zone, &nsec3param);
            if (result != ISC_R_SUCCESS)
            {
                dns_zone_log (zone, ISC_LOG_ERROR, "zone_addnsec3chain failed: %s", dns_result_totext (result));
            }
        }
    }
    dns_rdataset_disassociate (&rdataset);
...
static void set_resigntime (dns_zone_t * zone)
{
    ...
    dns_rdataset_init (&rdataset);
    dns_fixedname_init (&fixed);
    result = dns_db_getsigningtime (zone->db, &rdataset, dns_fixedname_name (&fixed));
    if (result != ISC_R_SUCCESS)
    {
        isc_time_settoepoch (&zone->resigntime);
        return;
...
static isc_result_t check_nsec3param (dns_zone_t * zone, dns_db_t * db)
{
    ...
    isc_boolean_t dynamic = (zone->type == dns_zone_master) ? zone_isdynamic (zone) : ISC_FALSE;

    dns_rdataset_init (&rdataset);
    result = dns_db_findnode (db, &zone->origin, ISC_FALSE, &node);
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "nsec3param lookup failure: %s", dns_result_totext (result));
        return (result);
    }
    dns_db_currentversion (db, &version);

    result = dns_db_findrdataset (db, node, version, dns_rdatatype_nsec3param, dns_rdatatype_none, 0, &rdataset, NULL);
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        dns_rdata_reset (&rdata);
        INSIST (result == ISC_R_SUCCESS);
        if (DNS_ZONE_OPTION (zone, DNS_ZONEOPT_NSEC3TESTZONE) && nsec3param.hash == DNS_NSEC3_UNKNOWNALG && !dynamic)
        {
            dns_zone_log (zone, ISC_LOG_WARNING, "nsec3 test \"unknown\" hash algorithm found: %u", nsec3param.hash);
            ok = ISC_TRUE;
        }
        else if (!dns_nsec3_supportedhash (nsec3param.hash))
        {
            if (dynamic)
            {
                dns_zone_log (zone, ISC_LOG_ERROR,
                              "unsupported nsec3 hash algorithm" " in dynamic zone: %u", nsec3param.hash);
                ...
                dns_zone_log (zone, ISC_LOG_WARNING, "unsupported nsec3 hash algorithm: %u", nsec3param.hash);
        }
        else
            ok = ISC_TRUE;
    }
    ...
    {
        result = DNS_R_BADZONE;
        dns_zone_log (zone, ISC_LOG_ERROR, "no supported nsec3 hash algorithm");
    }
 ...
 * keydata->removehd).
 */
static void set_refreshkeytimer (dns_zone_t * zone, dns_rdata_keydata_t * key, isc_stdtime_t now)
{
...
create_keydata (dns_zone_t * zone, dns_db_t * db, dns_dbversion_t * ver,
                dns_diff_t * diff, dns_keytable_t * keytable, dns_keynode_t ** keynodep, isc_boolean_t * changed)
{
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        dns_rdata_fromregion (&rdata, dst_key_class (key), dns_rdatatype_dnskey, &r);

        /* DSTKEY to KEYDATA. */
        CHECK (dns_rdata_tostruct (&rdata, &dnskey, NULL));
        CHECK (dns_keydata_fromdnskey (&keydata, &dnskey, now, 0, 0, NULL));

        /* KEYDATA to rdata. */
        dns_rdata_reset (&rdata);
        isc_buffer_init (&keyb, key_buf, sizeof (key_buf));
        CHECK (dns_rdata_fromstruct (&rdata, zone->rdclass, dns_rdatatype_keydata, &keydata, &keyb));

        /* Add rdata to zone. */
        CHECK (update_one_rr (db, ver, diff, DNS_DIFFOP_ADD, dst_key_name (key), 0, &rdata));
        *changed = ISC_TRUE;
 ...
 */
static isc_result_t
delete_keydata (dns_db_t * db, dns_dbversion_t * ver, dns_diff_t * diff, dns_name_t * name, dns_rdataset_t * rdataset)
{
    dns_rdata_t rdata = DNS_RDATA_INIT;
    ...
    for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
    {
        dns_rdata_reset (&rdata);
        dns_rdataset_current (rdataset, &rdata);
        uresult = update_one_rr (db, ver, diff, DNS_DIFFOP_DEL, name, 0, &rdata);
...
static isc_result_t compute_tag (dns_name_t * name, dns_rdata_dnskey_t * dnskey, isc_mem_t * mctx, dns_keytag_t * tag)
{
    ...
    dns_rdata_fromstruct (&rdata, dnskey->common.rdclass, dns_rdatatype_dnskey, dnskey, &buffer);
...
static void load_secroots (dns_zone_t * zone, dns_name_t * name, dns_rdataset_t * rdataset)
{
    ...
    if (trusted == 0 && pending != 0)
    {
        ...
        dns_name_format (name, namebuf, sizeof namebuf);
        dns_zone_log (zone, ISC_LOG_ERROR, "No valid trust anchors for '%s'!", namebuf);
        dns_zone_log (zone, ISC_LOG_ERROR, "%d key(s) revoked, %d still pending", revoked, pending);
        dns_zone_log (zone, ISC_LOG_ERROR, "All queries to '%s' will fail", namebuf);
...
update_one_rr (dns_db_t * db, dns_dbversion_t * ver, dns_diff_t * diff,
               dns_diffop_t op, dns_name_t * name, dns_ttl_t ttl, dns_rdata_t * rdata)
{
...
static isc_result_t increment_soa_serial (dns_db_t * db, dns_dbversion_t * ver, dns_diff_t * diff, isc_mem_t * mctx)
{
 ...
 */
static isc_result_t zone_journal (dns_zone_t * zone, dns_diff_t * diff, const char *caller)
{
    ...
    if (journalfile != NULL)
    {
        result = dns_journal_open (zone->mctx, journalfile, ISC_TRUE, &journal);
        if (result != ISC_R_SUCCESS)
        {
            dns_zone_log (zone, ISC_LOG_ERROR, "%s:dns_journal_open -> %s\n", caller, dns_result_totext (result));
            return (result);
        }
...
static isc_result_t add_soa (dns_zone_t * zone, dns_db_t * db)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "add_soa:dns_db_newversion -> %s\n", dns_result_totext (result));
        goto failure;
    }

    /* Build SOA record */
    result = dns_soa_buildrdata (&zone->origin, dns_rootname, zone->rdclass, 0, 0, 0, 0, 0, buf, &rdata);
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "add_soa:dns_soa_buildrdata -> %s\n", dns_result_totext (result));
        goto failure;
    }

    result = update_one_rr (db, ver, &diff, DNS_DIFFOP_ADD, &zone->origin, 0, &rdata);
...
static isc_result_t sync_keyzone (dns_zone_t * zone, dns_db_t * db)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "sync_keyzone:dns_db_newversion -> %s\n", dns_result_totext (result));
        goto failure;
    }
    ...
    for (result = dns_rriterator_first (&rrit); result == ISC_R_SUCCESS; result = dns_rriterator_nextrrset (&rrit))
    {
        ...
        if ((result != ISC_R_SUCCESS && result != DNS_R_PARTIALMATCH) || dns_keynode_managed (keynode) == ISC_FALSE)
        {
    ...
    while (result == DNS_R_NEWORIGIN || result == ISC_R_SUCCESS)
    {
        ...
        if (dns_keynode_managed (keynode))
        {
            ...
            result = dns_db_find (db, keyname, ver,
                                  ...
                                  DNS_DBFIND_NOWILD, 0, NULL, dns_fixedname_name (&fname), NULL, NULL);
            if (result != ISC_R_SUCCESS)
                result = create_keydata (zone, db, ver, &diff, sr, &keynode, &changed);
            if (result != ISC_R_SUCCESS)
                break;
        }
      skip:
...
static isc_result_t zone_postload (dns_zone_t * zone, dns_db_t * db, isc_time_t loadtime, isc_result_t result)
{
        ...
        if (zone->type == dns_zone_slave || zone->type == dns_zone_stub)
        {
            if (result == ISC_R_FILENOTFOUND)
                ...
                dns_zone_log (zone, ISC_LOG_ERROR,
                              ...
                              "failed: %s", zone->masterfile, dns_result_totext (result));
        }
        else
        {
            int level = ISC_LOG_ERROR;

            if (zone->type == dns_zone_key && result == ISC_R_FILENOTFOUND)
                level = ISC_LOG_DEBUG (1);
            dns_zone_log (zone, level,
                          "loading from master file %s failed: %s", zone->masterfile, dns_result_totext (result));
            nomaster = ISC_TRUE;
        }
    ...
    dns_zone_log (zone, ISC_LOG_DEBUG (2), "number of nodes in database: %u", dns_db_nodecount (db));
     ...
     */
    if (zone->journal != NULL &&
        !DNS_ZONE_OPTION (zone, DNS_ZONEOPT_NOMERGE) && !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_LOADED))
    {
        if (zone->type == dns_zone_master && (zone->update_acl != NULL || zone->ssutable != NULL))
            options = DNS_JOURNALOPT_RESIGN;
        else
            options = 0;
        result = dns_journal_rollforward2 (zone->mctx, db, options, zone->sigresigninginterval, zone->journal);
        if (result != ISC_R_SUCCESS && result != ISC_R_NOTFOUND &&
            result != DNS_R_UPTODATE && result != DNS_R_NOJOURNAL && result != ISC_R_RANGE)
        {
            dns_zone_log (zone, ISC_LOG_ERROR, "journal rollforward failed: %s", dns_result_totext (result));
            goto cleanup;
        }
        if (result == ISC_R_NOTFOUND || result == ISC_R_RANGE)
        {
            dns_zone_log (zone, ISC_LOG_ERROR, "journal rollforward failed: " "journal out of sync with zone");
            goto cleanup;
        }
        dns_zone_log (zone, ISC_LOG_DEBUG (1),
                      "journal rollforward completed " "successfully: %s", dns_result_totext (result));
        if (result == ISC_R_SUCCESS)
            needdump = ISC_TRUE;
    }
     ...
     */
    INSIST (db != NULL);
    result = zone_get_from_db (zone, db, &nscount, &soacount, &serial, &refresh, &retry, &expire, &minimum, &errors);
    if (result != ISC_R_SUCCESS && zone->type != dns_zone_key)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "could not find NS and/or SOA records");
    }
    ...
    switch (zone->type)
    {
        ...
        case dns_zone_stub:
            if (soacount != 1)
            {
                dns_zone_log (zone, ISC_LOG_ERROR, "has %d SOA records", soacount);
                result = DNS_R_BADZONE;
            }
            if (nscount == 0)
            {
            ...
            if (zone->type == dns_zone_master &&
                DNS_ZONE_OPTION (zone, DNS_ZONEOPT_CHECKINTEGRITY) && !integrity_checks (zone, db))
            {
                result = DNS_R_BADZONE;
                goto cleanup;
            }
            ...
            if (zone->type == dns_zone_master &&
                DNS_ZONE_OPTION (zone, DNS_ZONEOPT_CHECKDUPRR) && !zone_check_dup (zone, db))
            {
                result = DNS_R_BADZONE;
                goto cleanup;
            }
            ...
            if (zone->db != NULL)
            {
                 ...
                 */
                result = zone_get_from_db (zone, zone->db, NULL, NULL, &oldserial, NULL, NULL, NULL, NULL, NULL);
                RUNTIME_CHECK (result == ISC_R_SUCCESS);
                if (DNS_ZONE_OPTION (zone, DNS_ZONEOPT_IXFRFROMDIFFS) && !isc_serial_gt (serial, oldserial))
                {
                    ...
                    dns_zone_log (zone, ISC_LOG_ERROR,
                                  ...
                                  "new serial (%u) out of range " "[%u - %u]", serial, serialmin, serialmax);
                    result = DNS_R_BADZONE;
                    goto cleanup;
                }
                else if (!isc_serial_ge (serial, oldserial))
                    dns_zone_log (zone, ISC_LOG_ERROR, "zone serial (%u/%u) has gone " "backwards", serial, oldserial);
                else if (serial == oldserial && !hasinclude && strcmp (zone->db_argv[0], "_builtin") != 0)
                    dns_zone_log (zone, ISC_LOG_ERROR,
                                  "zone serial (%u) unchanged. " "zone may fail to transfer " "to slaves.", serial);
            }

            if (zone->type == dns_zone_master &&
                (zone->update_acl != NULL || zone->ssutable != NULL) &&
                zone->sigresigninginterval < (3 * refresh) && dns_db_issecure (db))
            {
                dns_zone_log (zone, ISC_LOG_WARNING, "sig-re-signing-interval less than " "3 * refresh.");
            }
            ...
            if (zone->type == dns_zone_slave || zone->type == dns_zone_stub)
            {
                ...
                if (result == ISC_R_SUCCESS)
                    DNS_ZONE_TIME_ADD (&t, zone->expire, &zone->expiretime);
                else
                    DNS_ZONE_TIME_ADD (&now, zone->retry, &zone->expiretime);

                delay = isc_random_jitter (zone->retry, (zone->retry * 3) / 4);
                DNS_ZONE_TIME_ADD (&now, delay, &zone->refreshtime);
                if (isc_time_compare (&zone->refreshtime, &zone->expiretime) >= 0)
                    zone->refreshtime = now;
            }
            break;
        ...
        default:
            UNEXPECTED_ERROR (__FILE__, __LINE__, "unexpected zone type %d", zone->type);
     ...
     */
    if (zone->type == dns_zone_master && DNS_ZONEKEY_OPTION (zone, DNS_ZONEKEY_MAINTAIN))
    ...
    if (zone->task != NULL)
    {
        ...
        if (zone->type == dns_zone_master && zone_isdynamic (zone) && dns_db_issecure (db))
        {
            dns_name_t *name;
            ...
            if (result == ISC_R_SUCCESS)
            {
                ...
                dns_name_format (name, namebuf, sizeof (namebuf));
                dns_rdatatype_format (next.covers, typebuf, sizeof (typebuf));
                dns_zone_log (zone, ISC_LOG_DEBUG (3),
                              "next resign: %s/%s in %d seconds", namebuf, typebuf, next.resign - timenow);
                dns_rdataset_disassociate (&next);
            }
            else
                dns_zone_log (zone, ISC_LOG_WARNING, "signed dynamic zone has no " "resign event scheduled");
        }
    ...
    if (!dns_db_ispersistent (db))
        dns_zone_log (zone, ISC_LOG_INFO, "loaded serial %u%s", serial, dns_db_issecure (db) ? " (DNSSEC signed)" : "");

    zone->loadtime = loadtime;
    return (result);

  cleanup:
    if (zone->type == dns_zone_slave || zone->type == dns_zone_stub || zone->type == dns_zone_key)
    {
        if (zone->journal != NULL)
...
zone_check_ns (dns_zone_t * zone, dns_db_t * db, dns_dbversion_t * version, dns_name_t * name, isc_boolean_t logit)
{
    isc_result_t result;
    ...
    result = dns_db_find (db, name, version, dns_rdatatype_a, 0, 0, NULL, foundname, NULL, NULL);
    if (result == ISC_R_SUCCESS)
        return (ISC_TRUE);

    if (result == DNS_R_NXRRSET)
    {
        result = dns_db_find (db, name, version, dns_rdatatype_aaaa, 0, 0, NULL, foundname, NULL, NULL);
        if (result == ISC_R_SUCCESS)
            return (ISC_TRUE);
    }
    ...
    if (result == DNS_R_NXRRSET || result == DNS_R_NXDOMAIN || result == DNS_R_EMPTYNAME)
    {
        ...
        {
            dns_name_format (name, namebuf, sizeof namebuf);
            dns_zone_log (zone, level, "NS '%s' has no address " "records (A or AAAA)", namebuf);
        }
        return (ISC_FALSE);
    }
    ...
    if (result == DNS_R_CNAME)
    {
        ...
        {
            dns_name_format (name, namebuf, sizeof namebuf);
            dns_zone_log (zone, level, "NS '%s' is a CNAME " "(illegal)", namebuf);
        }
        return (ISC_FALSE);
    }
    ...
    if (result == DNS_R_DNAME)
    {
        if (logit)
        {
            dns_name_format (name, namebuf, sizeof namebuf);
            dns_name_format (foundname, altbuf, sizeof altbuf);
            dns_zone_log (zone, level, "NS '%s' is below a DNAME " "'%s' (illegal)", namebuf, altbuf);
        }
        return (ISC_FALSE);
    }
...
zone_count_ns_rr (dns_zone_t * zone, dns_db_t * db, dns_dbnode_t * node,
                  dns_dbversion_t * version, unsigned int *nscount, unsigned int *errors, isc_boolean_t logit)
{
    isc_result_t result;
    ...
    dns_rdataset_init (&rdataset);
    result = dns_db_findrdataset (db, node, version, dns_rdatatype_ns, dns_rdatatype_none, 0, &rdataset, NULL);
    if (result == ISC_R_NOTFOUND)
    {
        INSIST (!dns_rdataset_isassociated (&rdataset));
        goto success;
    ...
    while (result == ISC_R_SUCCESS)
    {
        if (errors != NULL && zone->rdclass == dns_rdataclass_in &&
            (zone->type == dns_zone_master || zone->type == dns_zone_slave))
        {
            dns_rdata_init (&rdata);
            dns_rdataset_current (&rdataset, &rdata);
            result = dns_rdata_tostruct (&rdata, &ns, NULL);
            RUNTIME_CHECK (result == ISC_R_SUCCESS);
            if (dns_name_issubdomain (&ns.name, &zone->origin) && !zone_check_ns (zone, db, version, &ns.name, logit))
                ecount++;
        }
        count++;
        result = dns_rdataset_next (&rdataset);
...
zone_load_soa_rr (dns_db_t * db, dns_dbnode_t * node, dns_dbversion_t * version,
                  ...
                  isc_uint32_t * retry, isc_uint32_t * expire, isc_uint32_t * minimum)
{
    isc_result_t result;
    ...
    dns_rdataset_init (&rdataset);
    result = dns_db_findrdataset (db, node, version, dns_rdatatype_soa, dns_rdatatype_none, 0, &rdataset, NULL);
    if (result == ISC_R_NOTFOUND)
    {
        INSIST (!dns_rdataset_isassociated (&rdataset));
        if (soacount != NULL)
                  ...
                  isc_uint32_t * expire, isc_uint32_t * minimum, unsigned int *errors)
{
    ...
    {
        result = zone_count_ns_rr (zone, db, node, version, nscount, errors, ISC_TRUE);
        if (result != ISC_R_SUCCESS)
            answer = result;
    }

    if (soacount != NULL || serial != NULL || refresh != NULL || retry != NULL || expire != NULL || minimum != NULL)
    {
        result = zone_load_soa_rr (db, node, version, soacount, serial, refresh, retry, expire, minimum);
        if (result != ISC_R_SUCCESS)
            answer = result;
    }
...
isc_result_t dns_zone_setalsonotify (dns_zone_t * zone, const isc_sockaddr_t * notify, isc_uint32_t count)
{
    ...
    if (zone->notify != NULL)
    {
        isc_mem_put (zone->mctx, zone->notify, zone->notifycnt * sizeof (*new));
        zone->notify = NULL;
        zone->notifycnt = 0;
    }
    if (count != 0)
...
static isc_boolean_t same_masters (const isc_sockaddr_t * old, const isc_sockaddr_t * new, isc_uint32_t count)
{
    unsigned int i;
...
static isc_boolean_t same_keynames (dns_name_t ** old, dns_name_t ** new, isc_uint32_t count)
{
    ...
    for (i = 0; i < count; i++)
    {
        if (old[i] == NULL && new[i] == NULL)
            continue;
        if (old[i] == NULL || new[i] == NULL || !dns_name_equal (old[i], new[i]))
            return (ISC_FALSE);
    }
    return (ISC_TRUE);
}
...
dns_zone_setmasterswithkeys (dns_zone_t * zone,
                             const isc_sockaddr_t * masters, dns_name_t ** keynames, isc_uint32_t count)
{
    isc_sockaddr_t *new;
    ...
    if (count != zone->masterscnt ||
        !same_masters (zone->masters, masters, count) || !same_keynames (zone->masterkeynames, keynames, count))
    {
        if (zone->request != NULL)
    ...
    {
        isc_mem_put (zone->mctx, zone->masters, zone->masterscnt * sizeof (*new));
        zone->masters = NULL;
    }
    if (zone->masterkeynames != NULL)
    {
        for (i = 0; i < zone->masterscnt; i++)
        {
            ...
            {
                dns_name_free (zone->masterkeynames[i], zone->mctx);
                isc_mem_put (zone->mctx, zone->masterkeynames[i], sizeof (dns_name_t));
                zone->masterkeynames[i] = NULL;
            }
        }
        isc_mem_put (zone->mctx, zone->masterkeynames, zone->masterscnt * sizeof (dns_name_t *));
        zone->masterkeynames = NULL;
    }
    if (zone->mastersok != NULL)
    {
        isc_mem_put (zone->mctx, zone->mastersok, zone->masterscnt * sizeof (isc_boolean_t));
        zone->mastersok = NULL;
    }
    zone->masterscnt = 0;
    /*
    ...
    if (keynames != NULL)
    {
        ...
        for (i = 0; i < count; i++)
        {
            if (keynames[i] != NULL)
            {
                newname[i] = isc_mem_get (zone->mctx, sizeof (dns_name_t));
                if (newname[i] == NULL)
                    goto allocfail;
                dns_name_init (newname[i], NULL);
                result = dns_name_dup (keynames[i], zone->mctx, newname[i]);
                if (result != ISC_R_SUCCESS)
                {
                  allocfail:
                    for (i = 0; i < count; i++)
                        if (newname[i] != NULL)
                            dns_name_free (newname[i], zone->mctx);
                    isc_mem_put (zone->mctx, new, count * sizeof (*new));
                    isc_mem_put (zone->mctx, newok, count * sizeof (*newok));
                    isc_mem_put (zone->mctx, newname, count * sizeof (*newname));
                    goto unlock;
                }
            }
        }
...
static inline isc_boolean_t was_dumping (dns_zone_t * zone)
{
...
find_zone_keys (dns_zone_t * zone, dns_db_t * db, dns_dbversion_t * ver,
                isc_mem_t * mctx, unsigned int maxkeys, dst_key_t ** keys, unsigned int *nkeys)
{
    isc_result_t result;
    ...
    result = dns_dnssec_findzonekeys2 (db, ver, node, dns_db_origin (db), directory, mctx, maxkeys, keys, nkeys);
    if (result == ISC_R_NOTFOUND)
        result = ISC_R_SUCCESS;
  failure:
    if (node != NULL)
...
static isc_result_t
offline (dns_db_t * db, dns_dbversion_t * ver, dns_diff_t * diff, dns_name_t * name, dns_ttl_t ttl, dns_rdata_t * rdata)
{
    isc_result_t result;

    if ((rdata->flags & DNS_RDATA_OFFLINE) != 0)
        return (ISC_R_SUCCESS);
    result = update_one_rr (db, ver, diff, DNS_DIFFOP_DELRESIGN, name, ttl, rdata);
    if (result != ISC_R_SUCCESS)
        return (result);
    rdata->flags |= DNS_RDATA_OFFLINE;
    result = update_one_rr (db, ver, diff, DNS_DIFFOP_ADDRESIGN, name, ttl, rdata);
    return (result);
}
...
static void set_key_expiry_warning (dns_zone_t * zone, isc_stdtime_t when, isc_stdtime_t now)
{
    ...
    if (when <= now)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "DNSKEY RRSIG(s) have expired");
        isc_time_settoepoch (&zone->keywarntime);
    }
    else if (when < now + 7 * 24 * 3600)
    {
        ...
        dns_zone_log (zone, ISC_LOG_WARNING, "DNSKEY RRSIG(s) will expire within 7 days: %s", timebuf);
        delta = when - now;
    ...
    else
    {
        ...
        isc_time_formattimestamp (&zone->refreshkeytime, timebuf, 80);
        dns_zone_log (zone, ISC_LOG_NOTICE, "setting keywarntime to %s", timebuf);
    }
}
...
static isc_boolean_t delsig_ok (dns_rdata_rrsig_t * rrsig_ptr, dst_key_t ** keys, unsigned int nkeys)
{
    ...
    for (i = 0; i < nkeys; i++)
    {
        if (rrsig_ptr->algorithm == dst_key_alg (keys[i]) && (dst_key_isprivate (keys[i])) && !KSK (keys[i]))
            return (ISC_TRUE);
    }
    ...
    for (i = 0; i < nkeys; i++)
    {
        if ((rrsig_ptr->algorithm == dst_key_alg (keys[i])) && (rrsig_ptr->keyid == dst_key_id (keys[i])))
            return (ISC_FALSE);
    }
          ...
          unsigned int nkeys, isc_stdtime_t now, isc_boolean_t incremental)
{
    ...
    if (result != ISC_R_SUCCESS)
        goto failure;
    result = dns_db_findrdataset (db, node, ver, dns_rdatatype_rrsig, type, (isc_stdtime_t) 0, &rdataset, NULL);
    dns_db_detachnode (db, &node);
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        if (type != dns_rdatatype_dnskey)
        {
            ...
            {
                result = update_one_rr (db, ver, diff, DNS_DIFFOP_DELRESIGN, name, rdataset.ttl, &rdata);
                if (incremental)
                    changed = ISC_TRUE;
                if (result != ISC_R_SUCCESS)
                    break;
            ...
            else
            {
                ...
                if (incremental)
                {
                    result = offline (db, ver, diff, name, rdataset.ttl, &rdata);
                    changed = ISC_TRUE;
                    if (result != ISC_R_SUCCESS)
                        break;
                }
                ...
                if (zone->log_key_expired_timer <= now)
                {
                    ...
                    dns_secalg_format (rrsig.algorithm, algbuf, sizeof (algbuf));
                    dns_zone_log (zone, ISC_LOG_WARNING,
                                  "Key %s/%s/%d "
                                  "missing or inactive "
                                  "and has no replacement: " "retaining signatures.", origin, algbuf, rrsig.keyid);
                    zone->log_key_expired_timer = now + 3600;
                }
            }
            continue;
        }
        ...
        for (i = 0; i < nkeys; i++)
        {
            if (rrsig.algorithm == dst_key_alg (keys[i]) && rrsig.keyid == dst_key_id (keys[i]))
            {
                found = ISC_TRUE;
                /*
                ...
                {
                    isc_int64_t timeexpire = dns_time64_from32 (rrsig.timeexpire);

                    if (warn != 0 && warn > timeexpire)
                        warn = timeexpire;
                    if (rdata.flags & DNS_RDATA_OFFLINE)
                    {
                        if (maybe == 0 || maybe > timeexpire)
                            maybe = timeexpire;
                        break;
                    }
                    if (warn == 0)
    ...
    if (warn > 0)
    {
        ...
        if (warn == stdwarn)
            ...
            dns_zone_log (zone, ISC_LOG_ERROR, "key expiry warning time out of range");
#endif
    }
  failure:
    if (node != NULL)
...
add_sigs (dns_db_t * db, dns_dbversion_t * ver, dns_name_t * name,
          ...
          isc_stdtime_t expire, isc_boolean_t check_ksk, isc_boolean_t keyset_kskonly)
{
    isc_result_t result;
    ...
    if (result != ISC_R_SUCCESS)
        goto failure;
    result = dns_db_findrdataset (db, node, ver, type, 0, (isc_stdtime_t) 0, &rdataset, NULL);
    dns_db_detachnode (db, &node);
    if (result == ISC_R_NOTFOUND)
    {
        INSIST (!dns_rdataset_isassociated (&rdataset));
    ...
    for (i = 0; i < nkeys; i++)
    {
        ...
        CHECK (dns_dnssec_sign (name, &rdataset, keys[i], &inception, &expire, mctx, &buffer, &sig_rdata));
        /* Update the database and journal with the RRSIG. */
        /* XXX inefficient - will cause dataset merging */
        CHECK (update_one_rr (db, ver, diff, DNS_DIFFOP_ADDRESIGN, name, rdataset.ttl, &sig_rdata));
...
static void zone_resigninc (dns_zone_t * zone)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_resigninc:dns_db_newversion -> %s\n", dns_result_totext (result));
        goto failure;
    }

    result = find_zone_keys (zone, db, version, zone->mctx, DNS_MAXZONEKEYS, zone_keys, &nkeys);
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_resigninc:find_zone_keys -> %s\n", dns_result_totext (result));
        goto failure;
    }
    ...
    if (result != ISC_R_SUCCESS && result != ISC_R_NOTFOUND)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_resigninc:dns_db_getsigningtime -> %s\n", dns_result_totext (result));
    }
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        if (covers == dns_rdatatype_soa || i++ > zone->signatures || resign > stop)
            break;

        result = del_sigs (zone, db, version, name, covers, &sig_diff, zone_keys, nkeys, now, ISC_TRUE);
        if (result != ISC_R_SUCCESS)
        {
            dns_zone_log (zone, ISC_LOG_ERROR, "zone_resigninc:del_sigs -> %s\n", dns_result_totext (result));
            break;
        }

        result = add_sigs (db, version, name, covers, &sig_diff,
                           zone_keys, nkeys, zone->mctx, inception, expire, check_ksk, keyset_kskonly);
        if (result != ISC_R_SUCCESS)
        {
            dns_zone_log (zone, ISC_LOG_ERROR, "zone_resigninc:add_sigs -> %s\n", dns_result_totext (result));
            break;
        }
        result = dns_db_getsigningtime (db, &rdataset, dns_fixedname_name (&fixed));
    ...
    result = del_sigs (zone, db, version, &zone->origin, dns_rdatatype_soa, &sig_diff, zone_keys, nkeys, now, ISC_TRUE);
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_resigninc:del_sigs -> %s\n", dns_result_totext (result));
        goto failure;
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_resigninc:increment_soa_serial -> %s\n", dns_result_totext (result));
        goto failure;
    }
    ...
    result = add_sigs (db, version, &zone->origin, dns_rdatatype_soa,
                       &sig_diff, zone_keys, nkeys, zone->mctx, inception, soaexpire, check_ksk, keyset_kskonly);
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_resigninc:add_sigs -> %s\n", dns_result_totext (result));
        goto failure;
...
static isc_result_t
next_active (dns_db_t * db, dns_dbversion_t * version, dns_name_t * oldname, dns_name_t * newname, isc_boolean_t bottom)
{
    isc_result_t result;
    ...
    do
    {
        ...
        if (bottom && dns_name_issubdomain (newname, oldname) && !dns_name_equal (newname, oldname))
        {
            dns_db_detachnode (db, &node);
            continue;
        }
...
signed_with_key (dns_db_t * db, dns_dbnode_t * node, dns_dbversion_t * version, dns_rdatatype_t type, dst_key_t * key)
{
    ...
    dns_rdataset_init (&rdataset);
    result = dns_db_findrdataset (db, node, version, dns_rdatatype_rrsig, type, 0, &rdataset, NULL);
    if (result != ISC_R_SUCCESS)
    {
        INSIST (!dns_rdataset_isassociated (&rdataset));
        return (ISC_FALSE);
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        result = dns_rdata_tostruct (&rdata, &rrsig, NULL);
        INSIST (result == ISC_R_SUCCESS);
        if (rrsig.algorithm == dst_key_alg (key) && rrsig.keyid == dst_key_id (key))
        {
            dns_rdataset_disassociate (&rdataset);
            return (ISC_TRUE);
        }
...
add_nsec (dns_db_t * db, dns_dbversion_t * version, dns_name_t * name,
          dns_dbnode_t * node, dns_ttl_t ttl, isc_boolean_t bottom, dns_diff_t * diff)
{
    dns_fixedname_t fixed;
    ...
    CHECK (update_one_rr (db, version, diff, DNS_DIFFOP_ADD, name, ttl, &rdata));
  failure:
    return (result);
}
             ...
             dns_diff_t * diff, isc_int32_t * signatures, isc_mem_t * mctx)
{
    ...
    isc_boolean_t seen_soa, seen_ns, seen_rr, seen_dname, seen_nsec, seen_nsec3, seen_ds;
    ...
    dns_rdataset_init (&rdataset);
    isc_buffer_init (&buffer, data, sizeof (data));
    seen_rr = seen_soa = seen_ns = seen_dname = seen_nsec = seen_nsec3 = seen_ds = ISC_FALSE;
    for (result = dns_rdatasetiter_first (iterator); result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (iterator))
    {
    ...
    if (build_nsec3 && !seen_nsec3 && seen_rr)
    {
        ...
        CHECK (dns_nsec3_addnsec3s (db, version, name, minimum, unsecure, diff));
        (*signatures)--;
    }
    /*
    ...
    {
        dns_rdatasetiter_current (iterator, &rdataset);
        if (rdataset.type == dns_rdatatype_soa || rdataset.type == dns_rdatatype_rrsig)
            goto next_rdataset;
        if (rdataset.type == dns_rdatatype_dnskey)
        {
            ...
            goto next_rdataset;
        if (*delegation && rdataset.type != dns_rdatatype_ds && rdataset.type != dns_rdatatype_nsec)
            goto next_rdataset;
        if (signed_with_key (db, node, version, rdataset.type, key))
            goto next_rdataset;
        /* Calculate the signature, creating a RRSIG RDATA. */
        isc_buffer_clear (&buffer);
        CHECK (dns_dnssec_sign (name, &rdataset, key, &inception, &expire, mctx, &buffer, &rdata));
        /* Update the database and journal with the RRSIG. */
        /* XXX inefficient - will cause dataset merging */
        CHECK (update_one_rr (db, version, diff, DNS_DIFFOP_ADDRESIGN, name, rdataset.ttl, &rdata));
        dns_rdata_reset (&rdata);
        (*signatures)--;
      next_rdataset:
        dns_rdataset_disassociate (&rdataset);
              ...
              dns_ttl_t minimum, isc_boolean_t update_only, dns_diff_t * diff)
{
    ...
    {
        dns_rdataset_init (&rdataset);
        result = dns_db_findrdataset (db, node, version, dns_rdatatype_nsec, dns_rdatatype_none, 0, &rdataset, NULL);
        if (dns_rdataset_isassociated (&rdataset))
            dns_rdataset_disassociate (&rdataset);
        if (result == ISC_R_NOTFOUND)
            goto success;
...
updatesignwithkey (dns_zone_t * zone, dns_signing_t * signing,
                   dns_dbversion_t * version, isc_boolean_t build_nsec3, dns_ttl_t minimum, dns_diff_t * diff)
{
    isc_result_t result;
    ...
    result = dns_db_findrdataset (signing->db, node, version,
                                  zone->privatetype, dns_rdatatype_none, 0, &rdataset, NULL);
    if (result == ISC_R_NOTFOUND)
    {
        INSIST (!dns_rdataset_isassociated (&rdataset));
        result = ISC_R_SUCCESS;
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        if (rdata.length != 5 ||
            ...
            rdata.data[1] != ((signing->keyid >> 8) & 0xff) || rdata.data[2] != (signing->keyid & 0xff))
        {
            have_rr = ISC_TRUE;
            dns_rdata_reset (&rdata);
            continue;
        ...
        if (!signing->delete && rdata.data[4] != 0)
        {
            ...
            CHECK (update_one_rr (signing->db, version, diff, DNS_DIFFOP_DEL, &zone->origin, rdataset.ttl, &rdata));
        dns_rdata_reset (&rdata);
    }
    if (result == ISC_R_NOMORE)
        result = ISC_R_SUCCESS;
    if (!signing->delete && !seen_done)
    {
        ...
        CHECK (update_one_rr (signing->db, version, diff, DNS_DIFFOP_ADD, &zone->origin, rdataset.ttl, &rdata));
    }
    else if (!have_rr)
    {
         ...
         */
        if (build_nsec3)
            CHECK (dns_nsec3_addnsec3s (signing->db, version, origin, minimum, ISC_FALSE, diff));
        CHECK (updatesecure (signing->db, version, origin, minimum, ISC_TRUE, diff));
    }
                  ...
                  isc_boolean_t active, dns_rdatatype_t privatetype, dns_diff_t * diff)
{
    ...
    RUNTIME_CHECK (result == ISC_R_SUCCESS);
    result = dns_db_findrdataset (db, node, ver, dns_rdatatype_nsec3param, 0, 0, &rdataset, NULL);
    if (result == ISC_R_NOTFOUND)
        goto try_private;
    if (result != ISC_R_SUCCESS)
        goto failure;
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        if (nsec3param.hash != chain->nsec3param.hash ||
            ...
            memcmp (nsec3param.salt, chain->nsec3param.salt, nsec3param.salt_length))
        {
            dns_rdata_reset (&rdata);
            continue;
        }

        CHECK (update_one_rr (db, ver, diff, DNS_DIFFOP_DEL, name, rdataset.ttl, &rdata));
        dns_rdata_reset (&rdata);
    }
    if (result != ISC_R_NOMORE)
        goto failure;
  ...
  try_private:
     ...
     */
    result = dns_db_findrdataset (db, node, ver, privatetype, 0, 0, &rdataset, NULL);
    if (result == ISC_R_NOTFOUND)
        goto add;
    if (result != ISC_R_SUCCESS)
        goto failure;
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        CHECK (update_one_rr (db, ver, diff, DNS_DIFFOP_DEL, name, rdataset.ttl, &private));
        dns_rdata_reset (&rdata);
    }
    if (result != ISC_R_NOMORE)
        goto failure;
  ...
  add:
    ...
    CHECK (dns_rdata_fromstruct (&rdata, dns_db_class (db), dns_rdatatype_nsec3param, &chain->nsec3param, &buffer));
...
delete_nsec (dns_db_t * db, dns_dbversion_t * ver, dns_dbnode_t * node, dns_name_t * name, dns_diff_t * diff)
{
    ...
    result = dns_db_findrdataset (db, node, ver, dns_rdatatype_nsec, 0, 0, &rdataset, NULL);
    if (result == ISC_R_NOTFOUND)
        return (ISC_R_SUCCESS);
    if (result != ISC_R_SUCCESS)
        return (result);
...
deletematchingnsec3 (dns_db_t * db, dns_dbversion_t * ver, dns_dbnode_t * node,
                     dns_name_t * name, const dns_rdata_nsec3param_t * param, dns_diff_t * diff)
{
    dns_rdataset_t rdataset;
    ...
    dns_rdataset_init (&rdataset);
    result = dns_db_findrdataset (db, node, ver, dns_rdatatype_nsec3, 0, 0, &rdataset, NULL);
    if (result == ISC_R_NOTFOUND)
        return (ISC_R_SUCCESS);
    if (result != ISC_R_SUCCESS)
        return (result);
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        if (nsec3.hash != param->hash ||
            ...
            nsec3.salt_length != param->salt_length || memcmp (nsec3.salt, param->salt, nsec3.salt_length))
            continue;
        CHECK (update_one_rr (db, ver, diff, DNS_DIFFOP_DEL, name, rdataset.ttl, &rdata));
    }
    if (result == ISC_R_NOMORE)
...
static isc_result_t
need_nsec_chain (dns_db_t * db, dns_dbversion_t * ver, const dns_rdata_nsec3param_t * param, isc_boolean_t * answer)
{
    dns_dbnode_t *node = NULL;
    ...
    *answer = ISC_FALSE;
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        if (myparam.hash == param->hash &&
            ...
            myparam.salt_length == param->salt_length && !memcmp (myparam.salt, param->salt, myparam.salt_length))
            continue;
        /*
...
update_sigs (dns_diff_t * diff, dns_db_t * db, dns_dbversion_t * version,
             ...
             isc_boolean_t check_ksk, isc_boolean_t keyset_kskonly, dns_diff_t * sig_diff)
{
    dns_difftuple_t *tuple;

    isc_result_t result;

    for (tuple = ISC_LIST_HEAD (diff->tuples); tuple != NULL; tuple = ISC_LIST_HEAD (diff->tuples))
    {
        result = del_sigs (zone, db, version, &tuple->name,
                           tuple->rdata.type, sig_diff, zone_keys, nkeys, now, ISC_FALSE);
        if (result != ISC_R_SUCCESS)
        {
            dns_zone_log (zone, ISC_LOG_ERROR, "update_sigs:del_sigs -> %s\n", dns_result_totext (result));
            return (result);
        }
        ...
        if (result != ISC_R_SUCCESS)
        {
            dns_zone_log (zone, ISC_LOG_ERROR, "update_sigs:add_sigs -> %s\n", dns_result_totext (result));
            return (result);
        }
        ...
        do
        {
            ...
            while (next != NULL &&
                   (tuple->rdata.type != next->rdata.type || !dns_name_equal (&tuple->name, &next->name)))
...
static void zone_nsec3chain (dns_zone_t * zone)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_nsec3chain:dns_db_newversion -> %s\n", dns_result_totext (result));
        goto failure;
    }
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_nsec3chain:find_zone_keys -> %s\n", dns_result_totext (result));
        goto failure;
    }
    ...
    while (nsec3chain != NULL && nodes-- > 0 && signatures > 0)
    {
        ...
        if (first)
        {
            ...
            result = dns_db_find (db, name, version, dns_rdatatype_soa, DNS_DBFIND_NOWILD, 0, NULL, found, NULL, NULL);
            if ((result == DNS_R_DELEGATION || result == DNS_R_DNAME) && !dns_name_equal (name, found))
            {
                /*
        ...
        seen_soa = seen_ns = seen_dname = seen_ds = seen_nsec = ISC_FALSE;
        for (result = dns_rdatasetiter_first (iterator);
             result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (iterator))
        {
            dns_rdatasetiter_current (iterator, &rdataset);
         ...
         */
        dns_dbiterator_pause (nsec3chain->dbiterator);
        result = dns_nsec3_addnsec3 (db, version, name, &nsec3chain->nsec3param, zone->minimum, unsecure, &nsec3_diff);
        if (result != ISC_R_SUCCESS)
        {
      ...
      next_addnode:
        ...
        do
        {
            ...
            {
                CHECK (fixup_nsec3param (db, version, nsec3chain, ISC_FALSE, privatetype, &param_diff));
                LOCK_ZONE (zone);
                ISC_LIST_UNLINK (zone->nsec3chain, nsec3chain, link);
                UNLOCK_ZONE (zone);
                ISC_LIST_APPEND (cleanup, nsec3chain, link);
            ...
            if (result == ISC_R_NOMORE)
            {
                ...
                {
                    CHECK (fixup_nsec3param (db, version, nsec3chain, ISC_TRUE, privatetype, &param_diff));
            ...
            {
                dns_zone_log (zone, ISC_LOG_ERROR,
                              "zone_nsec3chain:" "dns_dbiterator_next -> %s\n", dns_result_totext (result));
                goto failure;
            }
            else if (delegation)
            {
                dns_dbiterator_current (nsec3chain->dbiterator, &node, nextname);
                dns_db_detachnode (db, &node);
    ...
    while (nsec3chain != NULL && nodes-- > 0 && signatures > 0)
    {
         ...
         */
        if (first && !updatensec && (nsec3chain->nsec3param.flags & DNS_NSEC3FLAG_NONSEC) == 0)
        {
            result = need_nsec_chain (db, version, &nsec3chain->nsec3param, &buildnsecchain);
            if (result != ISC_R_SUCCESS)
            {
                dns_zone_log (zone, ISC_LOG_ERROR,
                              "zone_nsec3chain:" "need_nsec_chain -> %s\n", dns_result_totext (result));
                goto failure;
            }
        }

        if (first)
            dns_zone_log (zone, ISC_LOG_DEBUG (3), "zone_nsec3chain:" "buildnsecchain = %u\n", buildnsecchain);
        ...
        if (!buildnsecchain)
        {
            ...
            if (first)
            {
                result = fixup_nsec3param (db, version, nsec3chain, ISC_TRUE, privatetype, &param_diff);
                if (result != ISC_R_SUCCESS)
                {
                    dns_zone_log (zone, ISC_LOG_ERROR,
                                  "zone_nsec3chain:" "fixup_nsec3param -> %s\n", dns_result_totext (result));
                    goto failure;
                }
            }
             ...
             */
            result = deletematchingnsec3 (db, version, node, name, &nsec3chain->nsec3param, &nsec3_diff);
            if (result != ISC_R_SUCCESS)
            {
                dns_zone_log (zone, ISC_LOG_ERROR,
                              "zone_nsec3chain:" "deletematchingnsec3 -> %s\n", dns_result_totext (result));
                goto failure;
            }
        ...
        seen_soa = seen_ns = seen_dname = seen_nsec3 = seen_nsec = seen_rr = ISC_FALSE;
        for (result = dns_rdatasetiter_first (iterator);
             result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (iterator))
        {
            dns_rdatasetiter_current (iterator, &rdataset);
        ...
        {
            dns_dbiterator_pause (nsec3chain->dbiterator);
            CHECK (add_nsec (db, version, name, node, zone->minimum, delegation, &nsec_diff));
        }
      ...
      next_removenode:
        ...
        do
        {
            ...
            if (result == ISC_R_NOMORE)
            {
                ...
                ISC_LIST_APPEND (cleanup, nsec3chain, link);
                dns_dbiterator_pause (nsec3chain->dbiterator);
                result = fixup_nsec3param (db, version, nsec3chain, ISC_FALSE, privatetype, &param_diff);
                if (result != ISC_R_SUCCESS)
                {
                    dns_zone_log (zone, ISC_LOG_ERROR,
                                  "zone_nsec3chain:" "fixup_nsec3param -> %s\n", dns_result_totext (result));
    ...
    {
        isc_boolean_t rebuild_nsec = ISC_FALSE, rebuild_nsec3 = ISC_FALSE;
        ...
        if (rebuild_nsec)
        {
            if (nsec3chain != NULL)
                dns_dbiterator_pause (nsec3chain->dbiterator);
            result = updatesecure (db, version, &zone->origin, zone->minimum, ISC_TRUE, &nsec_diff);
            if (result != ISC_R_SUCCESS)
            {
                dns_zone_log (zone, ISC_LOG_ERROR,
        ...
        if (rebuild_nsec3)
        {
            result = dns_nsec3_addnsec3s (db, version, dns_db_origin (db), zone->minimum, ISC_FALSE, &nsec3_diff);
            if (result != ISC_R_SUCCESS)
            {
                dns_zone_log (zone, ISC_LOG_ERROR,
                              "zone_nsec3chain:" "dns_nsec3_addnsec3s -> %s\n", dns_result_totext (result));
                goto failure;
            }
        }
    }
    ...
    result = update_sigs (&nsec3_diff, db, version, zone_keys,
                          nkeys, zone, inception, expire, now, check_ksk, keyset_kskonly, &sig_diff);
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_nsec3chain:" "update_sigs -> %s\n", dns_result_totext (result));
        goto failure;
    }
    ...
    if (updatensec)
    {
        if (nsec3chain != NULL)
            dns_dbiterator_pause (nsec3chain->dbiterator);
        result = updatesecure (db, version, &zone->origin, zone->minimum, ISC_FALSE, &nsec_diff);
        if (result != ISC_R_SUCCESS)
        {
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_nsec3chain:" "del_sigs -> %s\n", dns_result_totext (result));
        goto failure;
    }
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_nsec3chain:" "add_sigs -> %s\n", dns_result_totext (result));
        goto failure;
    }
    ...
    LOCK_ZONE (zone);
    for (nsec3chain = ISC_LIST_HEAD (zone->nsec3chain);
         nsec3chain != NULL; nsec3chain = ISC_LIST_NEXT (nsec3chain, link))
        dns_dbiterator_pause (nsec3chain->dbiterator);
    UNLOCK_ZONE (zone);
...
del_sig (dns_db_t * db, dns_dbversion_t * version, dns_name_t * name,
         dns_dbnode_t * node, unsigned int nkeys, dns_secalg_t algorithm, isc_uint16_t keyid, dns_diff_t * diff)
{
    dns_rdata_rrsig_t rrsig;
    ...
    for (result = dns_rdatasetiter_first (iterator); result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (iterator))
    {
        ...
        if (nkeys == 0 && rdataset.type == dns_rdatatype_nsec)
        {
                 ...
                 result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
            {
                ...
                dns_rdataset_current (&rdataset, &rdata);
                CHECK (update_one_rr (db, version, diff, DNS_DIFFOP_DEL, name, rdataset.ttl, &rdata));
            }
            if (result != ISC_R_NOMORE)
                goto failure;
            dns_rdataset_disassociate (&rdataset);
        ...
        for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
        {
            ...
            if (rrsig.algorithm != algorithm || rrsig.keyid != keyid)
                continue;
            CHECK (update_one_rr (db, version, diff, DNS_DIFFOP_DELRESIGN, name, rdataset.ttl, &rdata));
        }
        dns_rdataset_disassociate (&rdataset);
        if (result != ISC_R_NOMORE)
            break;
...
static void zone_sign (dns_zone_t * zone)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_sign:dns_db_newversion -> %s\n", dns_result_totext (result));
        goto failure;
    }
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_sign:find_zone_keys -> %s\n", dns_result_totext (result));
        goto failure;
    }
    ...
    CHECK (dns_private_chains (db, version, zone->privatetype, &build_nsec, &build_nsec3));
    ...
    while (signing != NULL && nodes-- > 0 && signatures > 0)
    {
        ...
        if (first && signing->delete)
        {
            ...
            for (i = 0, j = 0; i < nkeys; i++)
            {
                 ...
                 */
                if (ALG (zone_keys[i]) == signing->algorithm && dst_key_id (zone_keys[i]) == signing->keyid)
                {
                    if (KSK (zone_keys[i]))
                        dst_key_free (&zone_keys[i]);
                    continue;
        ...
        if (signing->delete)
        {
            ...
            CHECK (del_sig (db, version, name, node, nkeys, signing->algorithm, signing->keyid, &sig_diff));
        }
        ...
        for (i = 0; i < nkeys; i++)
        {
             ...
             */
            if (!signing->delete &&
                (dst_key_alg (zone_keys[i]) != signing->algorithm || dst_key_id (zone_keys[i]) != signing->keyid))
                continue;
             ...
             */
            if (signing->delete && ALG (zone_keys[i]) != signing->algorithm)
                continue;
            ...
            if (check_ksk && !REVOKE (zone_keys[i]))
            {
                ...
                for (j = 0; j < nkeys; j++)
                {
                    if (j == i || ALG (zone_keys[i]) != ALG (zone_keys[j]))
                        continue;
            ...
            CHECK (sign_a_node (db, name, node, version, build_nsec3,
                                ...
                                ISC_TF (both && keyset_kskonly), &delegation, &sig_diff, &signatures, zone->mctx));
            /*
        ...
        do
        {
            ...
            if (result == ISC_R_NOMORE)
            {
                ...
                if (nkeys != 0 && build_nsec)
                {
                     ...
                     */
                    result = updatesecure (db, version, &zone->origin, zone->minimum, ISC_FALSE, &post_diff);
                    if (result != ISC_R_SUCCESS)
                    {
                        dns_zone_log (zone, ISC_LOG_ERROR, "updatesecure -> %s\n", dns_result_totext (result));
                        goto failure;
                    }
                }
                result = updatesignwithkey (zone, signing, version, build_nsec3, zone->minimum, &post_diff);
                if (result != ISC_R_SUCCESS)
                {
                    dns_zone_log (zone, ISC_LOG_ERROR, "updatesignwithkey " "-> %s\n", dns_result_totext (result));
                    goto failure;
                }
            ...
            {
                dns_zone_log (zone, ISC_LOG_ERROR, "zone_sign:dns_dbiterator_next -> %s\n", dns_result_totext (result));
                goto failure;
            }
            else if (delegation)
            {
                dns_dbiterator_current (signing->dbiterator, &node, nextname);
                dns_db_detachnode (db, &node);
                if (!dns_name_issubdomain (nextname, name))
                    break;
            }
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_sign:del_sigs -> %s\n", dns_result_totext (result));
        goto failure;
    }
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_sign:increment_soa_serial -> %s\n", dns_result_totext (result));
        goto failure;
    }
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "zone_sign:add_sigs -> %s\n", dns_result_totext (result));
        goto failure;
    }
...
static void normalize_key (dns_rdata_t * rr, dns_rdata_t * target, unsigned char *data, int size)
{
    dns_rdata_dnskey_t dnskey;
    ...
    switch (rr->type)
    {
        case dns_rdatatype_dnskey:
            ...
            dns_rdata_fromstruct (target, rr->rdclass, dns_rdatatype_dnskey, &dnskey, &buf);
            break;
...
static isc_boolean_t matchkey (dns_rdataset_t * rdset, dns_rdata_t * rr)
{
    ...
    for (result = dns_rdataset_first (rdset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdset))
    {
        dns_rdata_reset (&rdata);
 ...
 */
static inline isc_stdtime_t refresh_time (dns_keyfetch_t * kfetch, isc_boolean_t retry)
{
    isc_result_t result;
...
static isc_result_t minimal_update (dns_keyfetch_t * kfetch, dns_dbversion_t * ver, dns_diff_t * diff)
{
    ...
    for (result = dns_rdataset_first (&kfetch->keydataset);
         result == ISC_R_SUCCESS; result = dns_rdataset_next (&kfetch->keydataset))
    {
        dns_rdata_reset (&rdata);
        dns_rdataset_current (&kfetch->keydataset, &rdata);

        /* Delete old version */
        CHECK (update_one_rr (kfetch->db, ver, diff, DNS_DIFFOP_DEL, name, 0, &rdata));
        ...
        CHECK (update_one_rr (kfetch->db, ver, diff, DNS_DIFFOP_ADD, name, 0, &rdata));
    }
    result = ISC_R_SUCCESS;
  failure:
    return (result);
...
static isc_boolean_t revocable (dns_keyfetch_t * kfetch, dns_rdata_keydata_t * keydata)
{
    ...
    isc_buffer_init (&keyb, key_buf, sizeof (key_buf));
    dns_keydata_todnskey (keydata, &dnskey, NULL);
    dns_rdata_fromstruct (&rr, keydata->common.rdclass, dns_rdatatype_dnskey, &dnskey, &keyb);
    ...
    for (result = dns_rdataset_first (&kfetch->dnskeysigset);
         result == ISC_R_SUCCESS; result = dns_rdataset_next (&kfetch->dnskeysigset))
    {
        dns_fixedname_t fixed;
        ...
        if (dst_key_alg (dstkey) == sig.algorithm &&
            (dst_key_id (dstkey) == sig.keyid || dst_key_rid (dstkey) == sig.keyid))
        {
            result = dns_dnssec_verify2 (keyname,
                                         &kfetch->dnskeyset,
                                         dstkey, ISC_FALSE, mctx, &sigrr, dns_fixedname_name (&fixed));

            dns_zone_log (kfetch->zone, ISC_LOG_DEBUG (3),
                          "Confirm revoked DNSKEY is self-signed: " "%s", dns_result_totext (result));
...
static void keyfetch_done (isc_task_t * task, isc_event_t * event)
{
    ...
    /* Fetch failed */
    if (eresult != ISC_R_SUCCESS || !dns_rdataset_isassociated (&kfetch->dnskeyset))
    {
        dns_zone_log (zone, ISC_LOG_WARNING,
         ...
         result == ISC_R_SUCCESS; result = dns_rdataset_next (&kfetch->dnskeysigset))
    {
        ...
        while (result == ISC_R_SUCCESS)
        {
            ...
            if (dst_key_alg (dstkey) == sig.algorithm && dst_key_id (dstkey) == sig.keyid)
            {
                result = dns_dnssec_verify2 (keyname,
                                             &kfetch->dnskeyset,
                                             dstkey, ISC_FALSE, zone->view->mctx, &sigrr, dns_fixedname_name (&fixed));

                dns_zone_log (zone, ISC_LOG_DEBUG (3),
                              "Verifying DNSKEY set for zone " "'%s': %s", namebuf, dns_result_totext (result));

                if (result == ISC_R_SUCCESS)
                {
                    kfetch->dnskeyset.trust = dns_trust_secure;
                    kfetch->dnskeysigset.trust = dns_trust_secure;
                    dns_keytable_detachkeynode (secroots, &keynode);
                    break;
                }
            }

            result = dns_keytable_nextkeynode (secroots, keynode, &nextnode);
            dns_keytable_detachkeynode (secroots, &keynode);
            keynode = nextnode;
        }
         ...
         result == ISC_R_SUCCESS; result = dns_rdataset_next (&kfetch->keydataset))
    {
        ...
        if (!matchkey (&kfetch->dnskeyset, &keydatarr))
        {
            ...
            {
                dns_zone_log (zone, ISC_LOG_WARNING,
                              "Pending key unexpectedly missing " "from %s; restarting acceptance " "timer", namebuf);
            ...
            {
                dns_zone_log (zone, ISC_LOG_WARNING, "Active key unexpectedly missing " "from %s", namebuf);
            ...
            {
                /* Delete old version */
                CHECK (update_one_rr (kfetch->db, ver, &diff, DNS_DIFFOP_DEL, keyname, 0, &keydatarr));
            }
            ...
            dns_rdata_reset (&keydatarr);
            isc_buffer_init (&keyb, key_buf, sizeof (key_buf));
            dns_rdata_fromstruct (&keydatarr, zone->rdclass, dns_rdatatype_keydata, &keydata, &keyb);

            /* Insert updated version */
            CHECK (update_one_rr (kfetch->db, ver, &diff, DNS_DIFFOP_ADD, keyname, 0, &keydatarr));
    ...
    for (result = dns_rdataset_first (&kfetch->dnskeyset);
         result == ISC_R_SUCCESS; result = dns_rdataset_next (&kfetch->dnskeyset))
    {
        ...
        if (matchkey (&kfetch->keydataset, &dnskeyrr))
        {
            ...
            if (revoked && revocable (kfetch, &keydata))
            {
                ...
                else if (keydata.removehd == 0)
                {
                    ...
                    dns_view_untrust (zone->view, keyname, &dnskey, mctx);
            ...
            else if (revoked)
            {
                if (secure && keydata.removehd == 0)
                {
                    dns_zone_log (zone, ISC_LOG_WARNING,
                                  "Active key for zone "
                                  "'%s' is revoked but " "did not self-sign; " "ignoring.", namebuf);
                    continue;
                }
            }
            else if (secure)
        ...
        else if (secure)
        {
            ...
            if (initializing)
            {
                ...
                CHECK (compute_tag (keyname, &dnskey, mctx, &tag));
                dns_zone_log (zone, ISC_LOG_WARNING,
                              "Initializing automatic trust "
                              "anchor management for zone '%s'; "
                              "DNSKEY ID %d is now trusted, "
                              "waiving the normal 30-day " "waiting period.", namebuf, tag);
                trustkey = ISC_TRUE;
            }
        }
        ...
        else if (newkey)
        {
            ...
            dns_rdata_tostruct (&dnskeyrr, &dnskey, NULL);
            dns_keydata_fromdnskey (&keydata, &dnskey, 0, 0, 0, NULL);
...
static void zone_refreshkeys (dns_zone_t * zone)
{
    ...
    for (result = dns_rriterator_first (&rrit); result == ISC_R_SUCCESS; result = dns_rriterator_nextrrset (&rrit))
    {
        ...
        if (kdset == NULL || kdset->type != dns_rdatatype_keydata || !dns_rdataset_isassociated (kdset))
            continue;
         ...
         */
        for (result = dns_rdataset_first (kdset); result == ISC_R_SUCCESS; result = dns_rdataset_next (kdset))
        {
            dns_rdata_reset (&rdata);
            ...
            {
                CHECK (update_one_rr (db, ver, &diff, DNS_DIFFOP_DEL, name, ttl, &rdata));
                continue;
            }
        ...
        else
        {
            ...
            dns_zone_log (zone, ISC_LOG_WARNING, "Failed to create fetch for " "DNSKEY update");
            fetch_err = ISC_TRUE;
        }
    }
    if (!ISC_LIST_EMPTY (diff.tuples))
  ...
  failure:
    if (fetch_err)
    {
        ...
        isc_time_formattimestamp (&zone->refreshkeytime, timebuf, 80);
        dns_zone_log (zone, ISC_LOG_DEBUG (1), "retry key refresh: %s", timebuf);
...
static void zone_maintenance (dns_zone_t * zone)
{
    ...
    switch (zone->type)
    {
        ...
        case dns_zone_stub:
            LOCK_ZONE (zone);
            if (isc_time_compare (&now, &zone->expiretime) >= 0 && DNS_ZONE_FLAG (zone, DNS_ZONEFLG_LOADED))
            {
                zone_expire (zone);
                zone->refreshtime = now;
            }
    ...
    switch (zone->type)
    {
        case dns_zone_slave:
        case dns_zone_stub:
            if (!DNS_ZONE_FLAG (zone, DNS_ZONEFLG_DIALREFRESH) && isc_time_compare (&now, &zone->refreshtime) >= 0)
    ...
    switch (zone->type)
    {
        ...
        case dns_zone_key:
            ...
            if (zone->masterfile != NULL &&
                ...
                DNS_ZONE_FLAG (zone, DNS_ZONEFLG_LOADED) && DNS_ZONE_FLAG (zone, DNS_ZONEFLG_NEEDDUMP))
            {
            ...
            if (!dumping)
            {
                ...
                if (result != ISC_R_SUCCESS)
                    dns_zone_log (zone, ISC_LOG_WARNING, "dump failed: %s", dns_result_totext (result));
            }
            break;
        default:
            break;
    ...
    switch (zone->type)
    {
        case dns_zone_key:
            if (isc_time_compare (&now, &zone->refreshkeytime) >= 0 &&
                DNS_ZONE_FLAG (zone, DNS_ZONEFLG_LOADED) && !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_REFRESHING))
                zone_refreshkeys (zone);
            break;
        case dns_zone_master:
            if (!isc_time_isepoch (&zone->refreshkeytime) && isc_time_compare (&now, &zone->refreshkeytime) >= 0)
                zone_rekey (zone);
        default:
            break;
    }
    ...
    switch (zone->type)
    {
        ...
        case dns_zone_slave:
             ...
             */
            if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_NEEDNOTIFY) && isc_time_compare (&now, &zone->notifytime) >= 0)
                zone_notify (zone, &now);
            /*
             * Do we need to sign/resign some RRsets?
             */
            if (!isc_time_isepoch (&zone->signingtime) && isc_time_compare (&now, &zone->signingtime) >= 0)
                zone_sign (zone);
            else if (!isc_time_isepoch (&zone->resigntime) && isc_time_compare (&now, &zone->resigntime) >= 0)
                zone_resigninc (zone);
            else if (!isc_time_isepoch (&zone->nsec3chaintime) && isc_time_compare (&now, &zone->nsec3chaintime) >= 0)
                zone_nsec3chain (zone);
            /*
             * Do we need to issue a key expiry warning.
             */
            if (!isc_time_isepoch (&zone->keywarntime) && isc_time_compare (&now, &zone->keywarntime) >= 0)
                set_key_expiry_warning (zone, zone->key_expiry, isc_time_seconds (&now));
            break;
        default:
            break;
    }
...
void dns_zone_refresh (dns_zone_t * zone)
{
    ...
    if (zone->masterscnt == 0)
    {
        ...
        if ((oldflags & DNS_ZONEFLG_NOMASTERS) == 0)
            dns_zone_log (zone, ISC_LOG_ERROR, "cannot refresh: no masters");
        goto unlock;
    }
     ...
     */
    isc_interval_set (&i, isc_random_jitter (zone->retry, zone->retry / 4), 0);
    result = isc_time_nowplusinterval (&zone->refreshtime, &i);
    if (result != ISC_R_SUCCESS)
        dns_zone_log (zone, ISC_LOG_WARNING, "isc_time_nowplusinterval() failed: %s", dns_result_totext (result));
...
isc_result_t dns_zone_flush (dns_zone_t * zone)
{
    ...
    if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_NEEDDUMP) && zone->masterfile != NULL)
    {
...
static void zone_needdump (dns_zone_t * zone, unsigned int delay)
{
     ...
     */
    if (zone->masterfile == NULL || DNS_ZONE_FLAG (zone, DNS_ZONEFLG_LOADED) == 0)
        return;
    ...
    if (isc_time_isepoch (&zone->dumptime) || isc_time_compare (&zone->dumptime, &dumptime) > 0)
...
static void dump_done (void *arg, isc_result_t result)
{
    ...
    if (result == ISC_R_SUCCESS && zone->journal != NULL && zone->journalsize != -1)
    {
        ...
        {
            tresult = dns_journal_compact (zone->mctx, zone->journal, serial, zone->journalsize);
            switch (tresult)
            {
                case ISC_R_SUCCESS:
                case ISC_R_NOSPACE:
                case ISC_R_NOTFOUND:
                    dns_zone_log (zone, ISC_LOG_DEBUG (3), "dns_journal_compact: %s", dns_result_totext (tresult));
                    break;
                default:
                    dns_zone_log (zone, ISC_LOG_ERROR, "dns_journal_compact failed: %s", dns_result_totext (tresult));
                    break;
            }
        }
    ...
    if (result != ISC_R_SUCCESS && result != ISC_R_CANCELED)
    {
        ...
        zone_needdump (zone, DNS_DUMP_DELAY);
             ...
             DNS_ZONE_FLAG (zone, DNS_ZONEFLG_NEEDDUMP) && DNS_ZONE_FLAG (zone, DNS_ZONEFLG_LOADED))
    {
        DNS_ZONE_CLRFLAG (zone, DNS_ZONEFLG_NEEDDUMP);
        DNS_ZONE_SETFLAG (zone, DNS_ZONEFLG_DUMPING);
        isc_time_settoepoch (&zone->dumptime);
...
static isc_result_t zone_dump (dns_zone_t * zone, isc_boolean_t compact)
{
 ...
 */
  ...
  redo:
    ...
    if (compact)
    {
        ...
        result = zonemgr_getio (zone->zmgr, ISC_FALSE, zone->task, zone_gotwritehandle, zone, &zone->writeio);
        if (result != ISC_R_SUCCESS)
            zone_idetach (&dummy);
        else
            result = DNS_R_CONTINUE;
    ...
    {
        dns_db_currentversion (db, &version);
        result = dns_master_dump2 (zone->mctx, db, version, &dns_master_style_default, masterfile, masterformat);
        dns_db_closeversion (db, &version, ISC_FALSE);
    }
  fail:
    if (db != NULL)
...
static isc_result_t
dumptostream (dns_zone_t * zone, FILE * fd, const dns_master_style_t * style, dns_masterformat_t format)
{
    isc_result_t result;
    ...
    dns_db_currentversion (db, &version);
    result = dns_master_dumptostream2 (zone->mctx, db, version, style, format, fd);
    dns_db_closeversion (db, &version, ISC_FALSE);
    dns_db_detach (&db);
    return (result);
}
...
isc_result_t
dns_zone_dumptostream2 (dns_zone_t * zone, FILE * fd, dns_masterformat_t format, const dns_master_style_t * style)
{
...
{
    return dumptostream (zone, fd, &dns_master_style_default, dns_masterformat_text);
}

isc_result_t dns_zone_fulldumptostream (dns_zone_t * zone, FILE * fd)
{
    return dumptostream (zone, fd, &dns_master_style_full, dns_masterformat_text);
}
...
static void notify_cancel (dns_zone_t * zone)
{
    ...
    for (notify = ISC_LIST_HEAD (zone->notifies); notify != NULL; notify = ISC_LIST_NEXT (notify, link))
    {
        if (notify->find != NULL)
...
static void forward_cancel (dns_zone_t * zone)
{
    ...
    for (forward = ISC_LIST_HEAD (zone->forwards); forward != NULL; forward = ISC_LIST_NEXT (forward, link))
    {
...
static isc_boolean_t notify_isqueued (dns_zone_t * zone, dns_name_t * name, isc_sockaddr_t * addr)
{
    dns_notify_t *notify;
    ...
    for (notify = ISC_LIST_HEAD (zone->notifies); notify != NULL; notify = ISC_LIST_NEXT (notify, link))
    {
        if (notify->request != NULL)
            continue;
        if (name != NULL && dns_name_dynamic (&notify->ns) && dns_name_equal (name, &notify->ns))
            return (ISC_TRUE);
...
static isc_boolean_t notify_isself (dns_zone_t * zone, isc_sockaddr_t * dst)
{
    ...
    if (result != ISC_R_SUCCESS && result != ISC_R_NOTFOUND)
        return (ISC_FALSE);
    isself = (zone->isself) (zone->view, key, &src, dst, zone->rdclass, zone->isselfarg);
    if (key != NULL)
...
static void notify_find_address (dns_notify_t * notify)
{
    ...
    REQUIRE (DNS_NOTIFY_VALID (notify));
    options = DNS_ADBFIND_WANTEVENT | DNS_ADBFIND_INET | DNS_ADBFIND_INET6 | DNS_ADBFIND_RETURNLAME;
    ...
    result = dns_adb_createfind (notify->zone->view->adb,
                                 ...
                                 options, 0, NULL, notify->zone->view->dstport, &notify->find);
...
static isc_result_t notify_send_queue (dns_notify_t * notify)
{
    ...
    e = isc_event_allocate (notify->mctx, NULL,
                            DNS_EVENT_NOTIFYSENDTOADDR, notify_send_toaddr, notify, sizeof (isc_event_t));
    if (e == NULL)
        return (ISC_R_NOMEMORY);
    e->ev_arg = notify;
    e->ev_sender = NULL;
    result = isc_ratelimiter_enqueue (notify->zone->zmgr->rl, notify->zone->task, &e);
    if (result != ISC_R_SUCCESS)
        isc_event_free (&e);
    return (result);
}
...
static void notify_send_toaddr (isc_task_t * task, isc_event_t * event)
{
     ...
     * mapped form.
     */
    if (isc_sockaddr_pf (&notify->dst) == PF_INET6 && IN6_IS_ADDR_V4MAPPED (&notify->dst.type.sin6.sin6_addr))
    {
        isc_sockaddr_format (&notify->dst, addrbuf, sizeof (addrbuf));
        notify_log (notify->zone, ISC_LOG_DEBUG (3), "notify: ignoring IPv6 mapped IPV4 address: %s", addrbuf);
        result = ISC_R_CANCELED;
        goto cleanup;
    }
    ...
    if (result != ISC_R_SUCCESS && result != ISC_R_NOTFOUND)
    {
        notify_log (notify->zone, ISC_LOG_ERROR, "NOTIFY to %s not " "sent. Peer TSIG key lookup failure.", addrbuf);
        goto cleanup_message;
    }

    notify_log (notify->zone, ISC_LOG_DEBUG (3), "sending notify to %s", addrbuf);
    if (notify->zone->view->peers != NULL)
    {
        dns_peer_t *peer = NULL;

        result = dns_peerlist_peerbyaddr (notify->zone->view->peers, &dstip, &peer);
        if (result == ISC_R_SUCCESS)
        {
    ...
    result = dns_request_createvia2 (notify->zone->view->requestmgr,
                                     ...
                                     timeout * 3, timeout, notify->zone->task, notify_done, notify, &notify->request);
    if (result == ISC_R_SUCCESS)
    {
        if (isc_sockaddr_pf (&notify->dst) == AF_INET)
        {
            inc_stats (notify->zone, dns_zonestatscounter_notifyoutv4);
        }
        else
        {
            inc_stats (notify->zone, dns_zonestatscounter_notifyoutv6);
        }
    }
...
static void notify_send (dns_notify_t * notify)
{
    ...
    for (ai = ISC_LIST_HEAD (notify->find->list); ai != NULL; ai = ISC_LIST_NEXT (ai, publink))
    {
        dst = ai->sockaddr;
        ...
        if (notify_isself (notify->zone, &dst))
            continue;
        new = NULL;
        result = notify_create (notify->mctx, (notify->flags & DNS_NOTIFY_NOSOA), &new);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
...
static void zone_notify (dns_zone_t * zone, isc_time_t * now)
{
    ...
    if (notifytype == dns_notifytype_masteronly && zone->type != dns_zone_master)
        return;
    ...
    result = dns_db_findrdataset (zonedb, node, version, dns_rdatatype_soa, dns_rdatatype_none, 0, &soardset, NULL);
    if (result != ISC_R_SUCCESS)
        goto cleanup2;
    ...
    for (i = 0; i < zone->notifycnt; i++)
    {
        ...
        if (!loggednotify)
        {
            notify_log (zone, ISC_LOG_INFO, "sending notifies (serial %u)", serial);
    ...
    result = dns_db_findrdataset (zonedb, node, version, dns_rdatatype_ns, dns_rdatatype_none, 0, &nsrdset, NULL);
    if (result != ISC_R_SUCCESS)
        goto cleanup3;
    ...
    while (result == ISC_R_SUCCESS)
    {
         ...
         * configured to do so.
         */
        if (!DNS_ZONE_OPTION (zone, DNS_ZONEOPT_NOTIFYTOSOA) && dns_name_compare (&master, &ns.name) == 0)
        {
...
save_nsrrset (dns_message_t * message, dns_name_t * name, dns_db_t * db, dns_dbversion_t * version)
{
    ...
    result = dns_message_findname (message, DNS_SECTION_ANSWER, name,
                                   dns_rdatatype_ns, dns_rdatatype_none, NULL, &nsrdataset);
    if (result != ISC_R_SUCCESS)
        goto fail;
     ...
     * Add glue rdatasets.
     */
    for (result = dns_rdataset_first (nsrdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (nsrdataset))
    {
        ...
        result = dns_message_findname (message, DNS_SECTION_ADDITIONAL,
                                       &ns.name, dns_rdatatype_aaaa, dns_rdatatype_none, NULL, &rdataset);
        if (result == ISC_R_SUCCESS)
        {
            result = dns_db_findnode (db, &ns.name, ISC_TRUE, &node);
            if (result != ISC_R_SUCCESS)
                goto fail;
        ...
        rdataset = NULL;
        result = dns_message_findname (message, DNS_SECTION_ADDITIONAL,
                                       &ns.name, dns_rdatatype_a, dns_rdatatype_none, NULL, &rdataset);
        if (result == ISC_R_SUCCESS)
        {
            result = dns_db_findnode (db, &ns.name, ISC_TRUE, &node);
            if (result != ISC_R_SUCCESS)
...
static void stub_callback (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        if (revent->result == ISC_R_TIMEDOUT && !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_NOEDNS))
        {
            LOCK_ZONE (zone);
            DNS_ZONE_SETFLAG (zone, DNS_ZONEFLG_NOEDNS);
            UNLOCK_ZONE (zone);
            dns_zone_log (zone, ISC_LOG_DEBUG (1),
                          "refreshing stub: timeout retrying " " without EDNS master %s (source %s)", master, source);
            goto same_master;
        }
        dns_zonemgr_unreachableadd (zone->zmgr, &zone->masteraddr, &zone->sourceaddr, &now);
        dns_zone_log (zone, ISC_LOG_INFO,
                      "could not refresh stub from master %s"
                      " (source %s): %s", master, source, dns_result_totext (revent->result));
        goto next_master;
    }
    ...
    if (msg->rcode != dns_rcode_noerror)
    {
            ...
            (msg->rcode == dns_rcode_servfail || msg->rcode == dns_rcode_notimp || msg->rcode == dns_rcode_formerr))
        {
            dns_zone_log (zone, ISC_LOG_DEBUG (1),
                          ...
                          "without EDNS master %s (source %s)", (int) rb.used, rcode, master, source);
            LOCK_ZONE (zone);
            DNS_ZONE_SETFLAG (zone, DNS_ZONEFLG_NOEDNS);
            UNLOCK_ZONE (zone);
            goto same_master;
        ...
        dns_zone_log (zone, ISC_LOG_INFO,
                      "refreshing stub: "
                      "unexpected rcode (%.*s) from %s (source %s)", (int) rb.used, rcode, master, source);
        goto next_master;
    }
    ...
    if ((msg->flags & DNS_MESSAGEFLAG_TC) != 0)
    {
        ...
        {
            dns_zone_log (zone, ISC_LOG_INFO,
                          "refreshing stub: truncated TCP " "response from master %s (source %s)", master, source);
            goto next_master;
        }
        LOCK_ZONE (zone);
    ...
    if ((msg->flags & DNS_MESSAGEFLAG_AA) == 0)
    {
        dns_zone_log (zone, ISC_LOG_INFO, "refreshing stub: "
                      "non-authoritative answer from " "master %s (source %s)", master, source);
        goto next_master;
    }
    ...
    {
        dns_zone_log (zone, ISC_LOG_INFO,
                      "refreshing stub: unexpected CNAME response " "from master %s (source %s)", master, source);
        goto next_master;
    }
    ...
    {
        dns_zone_log (zone, ISC_LOG_INFO,
                      "refreshing stub: no NS records in response " "from master %s (source %s)", master, source);
        goto next_master;
    }
    ...
    {
        dns_zone_log (zone, ISC_LOG_INFO,
                      "refreshing stub: unable to save NS records " "from master %s (source %s)", master, source);
        goto next_master;
    }
  ...
  next_master:
    ...
    while (zone->curmaster < zone->masterscnt && zone->mastersok[zone->curmaster]);
    DNS_ZONE_CLRFLAG (zone, DNS_ZONEFLG_NOEDNS);
    if (exiting || zone->curmaster >= zone->masterscnt)
    {
        ...
        if (!exiting &&
            DNS_ZONE_OPTION (zone, DNS_ZONEOPT_USEALTXFRSRC) && !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_USEALTXFRSRC))
        {
            /*
 ...
 */
static void refresh_callback (isc_task_t * task, isc_event_t * event)
{
    ...
    if (revent->result != ISC_R_SUCCESS)
    {
        if (revent->result == ISC_R_TIMEDOUT && !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_NOEDNS))
        {
            ...
            UNLOCK_ZONE (zone);
            dns_zone_log (zone, ISC_LOG_DEBUG (1),
                          "refresh: timeout retrying without EDNS " "master %s (source %s)", master, source);
            goto same_master;
        }
        if (revent->result == ISC_R_TIMEDOUT && !dns_request_usedtcp (revent->request))
        {
            dns_zone_log (zone, ISC_LOG_INFO,
                          "refresh: retry limit for " "master %s exceeded (source %s)", master, source);
            /* Try with slave with TCP. */
            if (zone->type == dns_zone_slave && DNS_ZONE_OPTION (zone, DNS_ZONEOPT_TRYTCPREFRESH))
            {
                if (!dns_zonemgr_unreachable (zone->zmgr, &zone->masteraddr, &zone->sourceaddr, &now))
                {
                    LOCK_ZONE (zone);
                ...
                dns_zone_log (zone, ISC_LOG_DEBUG (1),
                              ...
                              "as master %s (source %s) is " "unreachable (cached)", master, source);
            }
        }
        else
            dns_zone_log (zone, ISC_LOG_INFO,
                          "refresh: failure trying master "
                          "%s (source %s): %s", master, source, dns_result_totext (revent->result));
        goto next_master;
    }
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_INFO,
                      "refresh: failure trying master "
                      "%s (source %s): %s", master, source, dns_result_totext (result));
        goto next_master;
    }
    ...
    if (msg->rcode != dns_rcode_noerror)
    {
            ...
            (msg->rcode == dns_rcode_servfail || msg->rcode == dns_rcode_notimp || msg->rcode == dns_rcode_formerr))
        {
            dns_zone_log (zone, ISC_LOG_DEBUG (1),
                          ...
                          "EDNS master %s (source %s)", (int) rb.used, rcode, master, source);
            LOCK_ZONE (zone);
            DNS_ZONE_SETFLAG (zone, DNS_ZONEFLG_NOEDNS);
            UNLOCK_ZONE (zone);
            goto same_master;
        ...
        dns_zone_log (zone, ISC_LOG_INFO,
                      ...
                      "master %s (source %s)", (int) rb.used, rcode, master, source);
        /*
         * Perhaps AXFR/IXFR is allowed even if SOA queries aren't.
         */
        if (msg->rcode == dns_rcode_refused && zone->type == dns_zone_slave)
            goto tcp_transfer;
        goto next_master;
    }
    ...
    if ((msg->flags & DNS_MESSAGEFLAG_TC) != 0)
    {
        if (zone->type == dns_zone_slave)
        {
            dns_zone_log (zone, ISC_LOG_INFO,
                          ...
                          "initiating TCP zone xfer " "for master %s (source %s)", master, source);
            LOCK_ZONE (zone);
            DNS_ZONE_SETFLAG (zone, DNS_ZONEFLG_SOABEFOREAXFR);
            UNLOCK_ZONE (zone);
            goto tcp_transfer;
        ...
        else
        {
            ...
            {
                dns_zone_log (zone, ISC_LOG_INFO,
                              "refresh: truncated TCP response " "from master %s (source %s)", master, source);
                goto next_master;
            }
            LOCK_ZONE (zone);
            DNS_ZONE_SETFLAG (zone, DNS_ZONEFLG_USEVC);
    ...
    {
        dns_zone_log (zone, ISC_LOG_INFO,
                      "refresh: non-authoritative answer from " "master %s (source %s)", master, source);
        goto next_master;
    }
    ...
    if (cnamecnt != 0)
    {
        dns_zone_log (zone, ISC_LOG_INFO, "refresh: CNAME at top of zone " "in master %s (source %s)", master, source);
        goto next_master;
    }
    ...
    {
        dns_zone_log (zone, ISC_LOG_INFO, "refresh: referral response " "from master %s (source %s)", master, source);
        goto next_master;
    }
    ...
    {
        dns_zone_log (zone, ISC_LOG_INFO, "refresh: NODATA response " "from master %s (source %s)", master, source);
        goto next_master;
    }
    ...
    {
        dns_zone_log (zone, ISC_LOG_INFO,
                      "refresh: answer SOA count (%d) != 1 " "from master %s (source %s)", soacnt, master, source);
        goto next_master;
    }
    /*
     * Extract serial
    ...
    {
        dns_zone_log (zone, ISC_LOG_INFO,
                      "refresh: unable to get SOA record " "from master %s (source %s)", master, source);
        goto next_master;
    }
    ...
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_INFO, "refresh: dns_rdataset_first() failed");
        goto next_master;
    }
    ...
    if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_LOADED))
    {
        ...
        RUNTIME_CHECK (result == ISC_R_SUCCESS);
        zone_debuglog (zone, me, 1, "serial: new %u, old %u", serial, oldserial);
    }
    else
        zone_debuglog (zone, me, 1, "serial: new %u, old not loaded", serial);

    if (!DNS_ZONE_FLAG (zone, DNS_ZONEFLG_LOADED) ||
        DNS_ZONE_FLAG (zone, DNS_ZONEFLG_FORCEXFER) || isc_serial_gt (serial, oldserial))
    {
        if (dns_zonemgr_unreachable (zone->zmgr, &zone->masteraddr, &zone->sourceaddr, &now))
        {
            dns_zone_log (zone, ISC_LOG_INFO,
                          ...
                          zone->type == dns_zone_slave ? "zone transfer" : "NS query", master, source);
            goto next_master;
        }
      tcp_transfer:
        isc_event_free (&event);
    ...
    else if (isc_serial_eq (soa.serial, oldserial))
    {
        if (zone->masterfile != NULL)
        {
            ...
            if (result == ISC_R_SUCCESS &&
                !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_NEEDDUMP) && !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_DUMPING))
            {
            ...
            if (result == ISC_R_FILENOTFOUND)
            {
                ...
                dns_zone_log (zone, ISC_LOG_ERROR,
                              ...
                              "modification time of '%s': %s", zone->masterfile, dns_result_totext (result));
        }
        DNS_ZONE_JITTER_ADD (&now, zone->refresh, &zone->refreshtime);
        DNS_ZONE_TIME_ADD (&now, zone->expire, &zone->expiretime);
    ...
    else
    {
        if (!DNS_ZONE_OPTION (zone, DNS_ZONEOPT_MULTIMASTER))
            dns_zone_log (zone, ISC_LOG_INFO, "serial number (%u) "
                          "received from master %s < ours (%u)", soa.serial, master, oldserial);
        else
  ...
  next_master:
    ...
    if (zone->curmaster >= zone->masterscnt)
    {
        ...
        if (DNS_ZONE_OPTION (zone, DNS_ZONEOPT_USEALTXFRSRC) && !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_USEALTXFRSRC))
        {
            /*
             * Did we get a good answer from all the masters?
             */
...
static void queue_soa_query (dns_zone_t * zone)
{
    ...
    e = isc_event_allocate (zone->mctx, NULL, DNS_EVENT_ZONE, soa_query, zone, sizeof (isc_event_t));
    if (e == NULL)
    {
        cancel_refresh (zone);
        return;
...
static inline isc_result_t create_query (dns_zone_t * zone, dns_rdatatype_t rdtype, dns_message_t ** messagep)
{
    dns_message_t *message = NULL;
...
static isc_result_t add_opt (dns_message_t * message, isc_uint16_t udpsize, isc_boolean_t reqnsid)
{
    dns_rdataset_t *rdataset = NULL;
...
static void soa_query (isc_task_t * task, isc_event_t * event)
{
    ...
    LOCK_ZONE (zone);
    if (((event->ev_attributes & ISC_EVENTATTR_CANCELED) != 0) ||
        DNS_ZONE_FLAG (zone, DNS_ZONEFLG_EXITING) || zone->view->requestmgr == NULL)
    {
        if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_EXITING))
            cancel = ISC_FALSE;
        goto cleanup;
  ...
  again:
     ...
     */
    if ((zone->masterkeynames != NULL) && (zone->masterkeynames[zone->curmaster] != NULL))
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            dns_name_format (keyname, namebuf, sizeof (namebuf));
            dns_zone_log (zone, ISC_LOG_ERROR, "unable to find key: %s", namebuf);
            goto skip_master;
        }
    }
    if (key == NULL)
    {
        ...
        if (result != ISC_R_SUCCESS && result != ISC_R_NOTFOUND)
        {
            ...
            dns_zone_log (zone, ISC_LOG_ERROR, "unable to find TSIG key for %s", addrbuf);
            goto skip_master;
        }
    }
    ...
    if (zone->view->peers != NULL)
    {
        ...
        result = dns_peerlist_peerbyaddr (zone->view->peers, &masterip, &peer);
        if (result == ISC_R_SUCCESS)
        {
            result = dns_peer_getsupportedns (peer, &edns);
            if (result == ISC_R_SUCCESS && !edns)
                DNS_ZONE_SETFLAG (zone, DNS_ZONEFLG_NOEDNS);
            result = dns_peer_gettransfersource (peer, &zone->sourceaddr);
            if (result == ISC_R_SUCCESS)
                have_xfrsource = ISC_TRUE;
            if (zone->view->resolver != NULL)
                udpsize = dns_resolver_getudpsize (zone->view->resolver);
    ...
    switch (isc_sockaddr_pf (&zone->masteraddr))
    {
        case PF_INET:
            ...
            {
                if (isc_sockaddr_equal (&zone->altxfrsource4, &zone->xfrsource4))
                    goto skip_master;
        ...
        case PF_INET6:
            if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_USEALTXFRSRC))
            {
                if (isc_sockaddr_equal (&zone->altxfrsource6, &zone->xfrsource6))
                    goto skip_master;
    ...
    options = DNS_ZONE_FLAG (zone, DNS_ZONEFLG_USEVC) ? DNS_REQUESTOPT_TCP : 0;

    if (!DNS_ZONE_FLAG (zone, DNS_ZONEFLG_NOEDNS))
    {
        result = add_opt (message, udpsize, reqnsid);
        if (result != ISC_R_SUCCESS)
            zone_debuglog (zone, me, 1, "unable to add opt record: %s", dns_result_totext (result));
    }
    ...
    {
        zone_idetach (&dummy);
        zone_debuglog (zone, me, 1, "dns_request_createvia2() failed: %s", dns_result_totext (result));
        goto cleanup;
    }
    else
    {
...
static void ns_query (dns_zone_t * zone, dns_rdataset_t * soardataset, dns_stub_t * stub)
{
    ...
    REQUIRE (DNS_ZONE_VALID (zone));
    REQUIRE ((soardataset != NULL && stub == NULL) || (soardataset == NULL && stub != NULL));
    ...
    if (stub == NULL)
    {
        ...
        else
        {
            ...
            result = dns_db_create (zone->mctx, zone->db_argv[0],
                                    ...
                                    zone->rdclass, zone->db_argc - 1, zone->db_argv + 1, &stub->db);
            if (result != ISC_R_SUCCESS)
            {
                dns_zone_log (zone, ISC_LOG_ERROR,
                              "refreshing stub: " "could not create " "database: %s", dns_result_totext (result));
                goto cleanup;
            }
        ...
        {
            dns_zone_log (zone, ISC_LOG_INFO, "refreshing stub: "
                          "dns_db_newversion() failed: %s", dns_result_totext (result));
            goto cleanup;
        }
        ...
        {
            dns_zone_log (zone, ISC_LOG_INFO, "refreshing stub: "
                          "dns_db_findnode() failed: %s", dns_result_totext (result));
            goto cleanup;
        }

        result = dns_db_addrdataset (stub->db, node, stub->version, 0, soardataset, 0, NULL);
        dns_db_detachnode (stub->db, &node);
        if (result != ISC_R_SUCCESS)
        {
            dns_zone_log (zone, ISC_LOG_INFO,
                          "refreshing stub: " "dns_db_addrdataset() failed: %s", dns_result_totext (result));
            goto cleanup;
        }
    }
    ...
    result = dns_request_createvia2 (zone->view->requestmgr, message,
                                     ...
                                     timeout, zone->task, stub_callback, stub, &zone->request);
    if (result != ISC_R_SUCCESS)
    {
        zone_debuglog (zone, me, 1, "dns_request_createvia() failed: %s", dns_result_totext (result));
        goto cleanup;
    }
    dns_message_destroy (&message);
    goto unlock;
 ...
 */
static void zone_shutdown (isc_task_t * task, isc_event_t * event)
{
    ...
    if (zone->zmgr != NULL)
    {
        ...
        {
            ISC_LIST_UNLINK (zone->zmgr->waiting_for_xfrin, zone, statelink);
            linked = ISC_TRUE;
            zone->statelist = NULL;
        }
    ...
    if (!DNS_ZONE_FLAG (zone, DNS_ZONEFLG_FLUSH) || !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_DUMPING))
    {
...
static void zone_settimer (dns_zone_t * zone, isc_time_t * now)
{
    ...
    switch (zone->type)
    {
        case dns_zone_master:
            ...
            if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_NEEDDUMP) && !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_DUMPING))
            {
                INSIST (!isc_time_isepoch (&zone->dumptime));
                if (isc_time_isepoch (&next) || isc_time_compare (&zone->dumptime, &next) < 0)
                    next = zone->dumptime;
            }
            if (!DNS_ZONE_FLAG (zone, DNS_ZONEFLG_REFRESHING) && !isc_time_isepoch (&zone->refreshkeytime))
            {
                if (isc_time_isepoch (&next) || isc_time_compare (&zone->refreshkeytime, &next) < 0)
                    next = zone->refreshkeytime;
            }
            if (!isc_time_isepoch (&zone->resigntime))
            {
                if (isc_time_isepoch (&next) || isc_time_compare (&zone->resigntime, &next) < 0)
                    next = zone->resigntime;
            }
            if (!isc_time_isepoch (&zone->keywarntime))
            {
                if (isc_time_isepoch (&next) || isc_time_compare (&zone->keywarntime, &next) < 0)
                    next = zone->keywarntime;
            }
            if (!isc_time_isepoch (&zone->signingtime))
            {
                if (isc_time_isepoch (&next) || isc_time_compare (&zone->signingtime, &next) < 0)
                    next = zone->signingtime;
            }
            if (!isc_time_isepoch (&zone->nsec3chaintime))
            {
                if (isc_time_isepoch (&next) || isc_time_compare (&zone->nsec3chaintime, &next) < 0)
        ...
        case dns_zone_slave:
            if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_NEEDNOTIFY))
                next = zone->notifytime;
         /*FALLTHROUGH*/ case dns_zone_stub:
            if (!DNS_ZONE_FLAG (zone, DNS_ZONEFLG_REFRESH) &&
                !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_NOMASTERS) &&
                !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_NOREFRESH) && !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_LOADING))
            {
                INSIST (!isc_time_isepoch (&zone->refreshtime));
                if (isc_time_isepoch (&next) || isc_time_compare (&zone->refreshtime, &next) < 0)
                    next = zone->refreshtime;
            }
            if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_LOADED))
            {
                INSIST (!isc_time_isepoch (&zone->expiretime));
                if (isc_time_isepoch (&next) || isc_time_compare (&zone->expiretime, &next) < 0)
        ...
        case dns_zone_key:
            ...
            if (!DNS_ZONE_FLAG (zone, DNS_ZONEFLG_REFRESHING))
            {
                if (isc_time_isepoch (&next) ||
                    (!isc_time_isepoch (&zone->refreshkeytime) && isc_time_compare (&zone->refreshkeytime, &next) < 0))
                    next = zone->refreshkeytime;
            }
            break;
    ...
    if (isc_time_isepoch (&next))
    {
        ...
        result = isc_timer_reset (zone->timer, isc_timertype_inactive, NULL, NULL, ISC_TRUE);
        if (result != ISC_R_SUCCESS)
            dns_zone_log (zone, ISC_LOG_ERROR, "could not deactivate zone timer: %s", isc_result_totext (result));
    }
    else
    {
        if (isc_time_compare (&next, now) <= 0)
            next = *now;
        result = isc_timer_reset (zone->timer, isc_timertype_once, &next, NULL, ISC_TRUE);
        if (result != ISC_R_SUCCESS)
            dns_zone_log (zone, ISC_LOG_ERROR, "could not reset zone timer: %s", isc_result_totext (result));
    }
}

static void cancel_refresh (dns_zone_t * zone)
{
...
static isc_result_t notify_createmessage (dns_zone_t * zone, unsigned int flags, dns_message_t ** messagep)
{
    ...
    dns_rdataset_makequestion (temprdataset, zone->rdclass, dns_rdatatype_soa);
    ...
    dns_rdataset_init (&rdataset);
    result = dns_db_findrdataset (zonedb, node, version, dns_rdatatype_soa, dns_rdatatype_none, 0, &rdataset, NULL);
    if (result != ISC_R_SUCCESS)
        goto soa_cleanup;
    result = dns_rdataset_first (&rdataset);
    if (result != ISC_R_SUCCESS)
...
isc_result_t dns_zone_notifyreceive (dns_zone_t * zone, isc_sockaddr_t * from, dns_message_t * msg)
{
    ...
    if (msg->counts[DNS_SECTION_QUESTION] == 0 ||
        dns_message_findname (msg, DNS_SECTION_QUESTION, &zone->origin,
                              dns_rdatatype_soa, dns_rdatatype_none, NULL, NULL) != ISC_R_SUCCESS)
    {
        UNLOCK_ZONE (zone);
        if (msg->counts[DNS_SECTION_QUESTION] == 0)
        {
            dns_zone_log (zone, ISC_LOG_NOTICE, "NOTIFY with no " "question section from: %s", fromtext);
            return (DNS_R_FORMERR);
        }
        dns_zone_log (zone, ISC_LOG_NOTICE, "NOTIFY zone does not match");
        return (DNS_R_NOTIMP);
    }
    ...
    for (i = 0; i < zone->masterscnt; i++)
    {
        ...
        if (zone->view->aclenv.match_mapped &&
            IN6_IS_ADDR_V4MAPPED (&from->type.sin6.sin6_addr) && isc_sockaddr_pf (&zone->masters[i]) == AF_INET)
        {
    ...
    if (i >= zone->masterscnt && zone->notify_acl != NULL &&
        dns_acl_match (&netaddr, NULL, zone->notify_acl,
                       &zone->view->aclenv, &match, NULL) == ISC_R_SUCCESS && match > 0)
    {
        /* Accept notify. */
    }
    ...
    {
        UNLOCK_ZONE (zone);
        dns_zone_log (zone, ISC_LOG_INFO, "refused notify from non-master: %s", fromtext);
    ...
    if (msg->counts[DNS_SECTION_ANSWER] > 0 &&
        DNS_ZONE_FLAG (zone, DNS_ZONEFLG_LOADED) && !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_NOREFRESH))
    {
        result = dns_message_findname (msg, DNS_SECTION_ANSWER,
                                       &zone->origin, dns_rdatatype_soa, dns_rdatatype_none, NULL, &rdataset);
        if (result == ISC_R_SUCCESS)
            result = dns_rdataset_first (rdataset);
        if (result == ISC_R_SUCCESS)
        {
            ...
            {
                dns_zone_log (zone, ISC_LOG_INFO, "notify from %s: " "zone is up to date", fromtext);
                UNLOCK_ZONE (zone);
                return (ISC_R_SUCCESS);
            }
        }
    ...
    if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_REFRESH))
    {
        ...
        UNLOCK_ZONE (zone);
        dns_zone_log (zone, ISC_LOG_INFO, "notify from %s: refresh in progress, " "refresh check queued", fromtext);
        return (ISC_R_SUCCESS);
    }
    zone->notifyfrom = *from;
...
static void zone_namerd_tostr (dns_zone_t * zone, char *buf, size_t length)
{
    ...
    if (result != ISC_R_SUCCESS && isc_buffer_availablelength (&buffer) >= (sizeof ("<UNKNOWN>") - 1))
    ...
    if (zone->view != NULL && strcmp (zone->view->name, "_bind") != 0 &&
        strcmp (zone->view->name, "_default") != 0 && strlen (zone->view->name) < isc_buffer_availablelength (&buffer))
    {
        isc_buffer_putstr (&buffer, "/");
...
static void zone_viewname_tostr (dns_zone_t * zone, char *buf, size_t length)
{
    ...
    else if (strlen (zone->view->name) < isc_buffer_availablelength (&buffer))
    {
        isc_buffer_putstr (&buffer, zone->view->name);
    }
    else
...
static void zone_debuglog (dns_zone_t * zone, const char *me, int debuglevel, const char *fmt, ...)
{
    va_list ap;
...
static int message_count (dns_message_t * msg, dns_section_t section, dns_rdatatype_t type)
{
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        for (curr = ISC_LIST_TAIL (name->list); curr != NULL; curr = ISC_LIST_PREV (curr, link))
        {
            if (curr->type == type)
                count++;
        }
...
static void notify_done (isc_task_t * task, isc_event_t * event)
{
    ...
    if (result == ISC_R_SUCCESS)
        result = dns_message_create (notify->zone->mctx, DNS_MESSAGE_INTENTPARSE, &message);
    if (result == ISC_R_SUCCESS)
        result = dns_request_getresponse (revent->request, message, DNS_MESSAGEPARSE_PRESERVEORDER);
    if (result == ISC_R_SUCCESS)
        result = dns_rcode_totext (message->rcode, &buf);
    if (result == ISC_R_SUCCESS)
        notify_log (notify->zone, ISC_LOG_DEBUG (3), "notify response from %s: %.*s", addrbuf, (int) buf.used, rcode);
    else
        notify_log (notify->zone, ISC_LOG_DEBUG (2), "notify to %s failed: %s", addrbuf, dns_result_totext (result));
     ...
     */
    isc_event_free (&event);
    if (message != NULL && message->rcode == dns_rcode_formerr && (notify->flags & DNS_NOTIFY_NOSOA) == 0)
    {
    ...
    else
    {
        if (result == ISC_R_TIMEDOUT)
            notify_log (notify->zone, ISC_LOG_DEBUG (1), "notify to %s: retries exceeded", addrbuf);
        notify_destroy (notify, ISC_FALSE);
    }
    if (message != NULL)
        dns_message_destroy (&message);
...
static isc_result_t zone_replacedb (dns_zone_t * zone, dns_db_t * db, isc_boolean_t dump)
{
    ...
    result = zone_get_from_db (zone, db, &nscount, &soacount, NULL, NULL, NULL, NULL, NULL, NULL);
    if (result == ISC_R_SUCCESS)
    {
        if (soacount != 1)
        {
    ...
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "retrieving SOA and NS records failed: %s", dns_result_totext (result));
        return (result);
    }
    ...
    if (zone->db != NULL && zone->journal != NULL &&
        DNS_ZONE_OPTION (zone, DNS_ZONEOPT_IXFRFROMDIFFS) && !DNS_ZONE_FLAG (zone, DNS_ZONEFLG_FORCEXFER))
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            dns_zone_log (zone, ISC_LOG_ERROR, "ixfr-from-differences: unable to get " "new serial");
            goto fail;
        }
        ...
        result = zone_get_from_db (zone, zone->db, NULL, NULL, &oldserial, NULL, NULL, NULL, NULL, NULL);
        RUNTIME_CHECK (result == ISC_R_SUCCESS);
        if (zone->type == dns_zone_slave && !isc_serial_gt (serial, oldserial))
        {
            isc_uint32_t serialmin, serialmax;
        ...
        result = dns_db_diff (zone->mctx, db, ver, zone->db, NULL, zone->journal);
        if (result != ISC_R_SUCCESS)
            goto fail;
        if (dump)
            zone_needdump (zone, DNS_DUMP_DELAY);
        ...
        {
            result = dns_journal_compact (zone->mctx, zone->journal, serial, zone->journalsize);
            switch (result)
            {
                case ISC_R_SUCCESS:
                case ISC_R_NOSPACE:
                case ISC_R_NOTFOUND:
                    dns_zone_log (zone, ISC_LOG_DEBUG (3), "dns_journal_compact: %s", dns_result_totext (result));
                    break;
                default:
                    dns_zone_log (zone, ISC_LOG_ERROR, "dns_journal_compact failed: %s", dns_result_totext (result));
                    break;
            }
        }
    }
    else
    {
        if (dump && zone->masterfile != NULL)
        {
             ...
             */
            if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_FORCEXFER) && remove (zone->masterfile) < 0 && errno != ENOENT)
            {
                ...
                isc_log_write (dns_lctx,
                               ...
                               DNS_LOGMODULE_ZONE,
                               ISC_LOG_WARNING, "unable to remove masterfile " "'%s': '%s'", zone->masterfile, strbuf);
            }
        ...
        if (dump && zone->journal != NULL)
        {
            ...
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                           DNS_LOGMODULE_ZONE, ISC_LOG_DEBUG (3), "removing journal file");
            if (remove (zone->journal) < 0 && errno != ENOENT)
            {
                ...
                isc_log_write (dns_lctx,
                               DNS_LOGCATEGORY_GENERAL,
                               DNS_LOGMODULE_ZONE,
                               ISC_LOG_WARNING, "unable to remove journal " "'%s': '%s'", zone->journal, strbuf);
            }
        }
    }
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL, DNS_LOGMODULE_ZONE, ISC_LOG_DEBUG (3), "replacing zone database");
...
static void zone_xfrdone (dns_zone_t * zone, isc_result_t result)
{
    ...
    dns_zone_log (zone, ISC_LOG_DEBUG (1), "zone transfer finished: %s", dns_result_totext (result));
    ...
    switch (result)
    {
        case ISC_R_SUCCESS:
            DNS_ZONE_SETFLAG (zone, DNS_ZONEFLG_NEEDNOTIFY);
         /*FALLTHROUGH*/ case DNS_R_UPTODATE:
            ...
            result = zone_get_from_db (zone, zone->db, &nscount,
                                       &soacount, &serial, &refresh, &retry, &expire, &minimum, NULL);
            ZONEDB_UNLOCK (&zone->dblock, isc_rwlocktype_read);
            if (result == ISC_R_SUCCESS)
            {
                if (soacount != 1)
                    dns_zone_log (zone, ISC_LOG_ERROR,
                                  "transferred zone " "has %d SOA record%s", soacount, (soacount != 0) ? "s" : "");
                if (nscount == 0)
                {
                    dns_zone_log (zone, ISC_LOG_ERROR, "transferred zone " "has no NS records");
                    if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_HAVETIMERS))
                    {
                        zone->refresh = DNS_ZONE_DEFAULTREFRESH;
                        zone->retry = DNS_ZONE_DEFAULTRETRY;
            ...
            if (result == ISC_R_SUCCESS && xfrresult == ISC_R_SUCCESS)
            {
                ...
                if (zone->tsigkey != NULL)
                {
                    ...
                    dns_name_format (&zone->tsigkey->name, namebuf, sizeof (namebuf));
                    snprintf (buf, sizeof (buf), ": TSIG '%s'", namebuf);
                }
                else
                    buf[0] = '\0';
                dns_zone_log (zone, ISC_LOG_INFO, "transferred serial %u%s", serial, buf);
            }
            ...
            if (zone->masterfile != NULL || zone->journal != NULL)
            {
                ...
                if (result != ISC_R_SUCCESS && zone->masterfile != NULL)
                    result = isc_file_settime (zone->masterfile, &now);
                /* Someone removed the file from underneath us! */
                if (result == ISC_R_FILENOTFOUND && zone->masterfile != NULL)
                {
    ...
    {
        result = dns_journal_compact (zone->mctx, zone->journal, zone->compact_serial, zone->journalsize);
        switch (result)
        {
            case ISC_R_SUCCESS:
            case ISC_R_NOSPACE:
...
static void zone_loaddone (void *arg, isc_result_t result)
{
    ...
    if (tresult != ISC_R_SUCCESS && (result == ISC_R_SUCCESS || result == DNS_R_SEENINCLUDE))
        result = tresult;
     ...
     */
    if ((result == ISC_R_SUCCESS || result == DNS_R_SEENINCLUDE) && DNS_ZONE_FLAG (load->zone, DNS_ZONEFLG_THAW))
        zone->update_disabled = ISC_FALSE;
...
static void queue_xfrin (dns_zone_t * zone)
{
    ...
    {
        dns_zone_logc (zone, DNS_LOGCATEGORY_XFER_IN, ISC_LOG_INFO, "zone transfer deferred due to quota");
    }
    else if (result != ISC_R_SUCCESS)
    {
        dns_zone_logc (zone, DNS_LOGCATEGORY_XFER_IN, ISC_LOG_ERROR,
                       "starting zone transfer: %s", isc_result_totext (result));
    }
}
...
static void got_transfer_quota (isc_task_t * task, isc_event_t * event)
{
    ...
    if (dns_zonemgr_unreachable (zone->zmgr, &zone->masteraddr, &zone->sourceaddr, &now))
    {
        ...
        dns_zone_log (zone, ISC_LOG_INFO,
                      ...
                      "master %s (source %s) is unreachable (cached)", master, source);
        result = ISC_R_CANCELED;
        goto cleanup;
    }
    ...
    {
        dns_zone_log (zone, ISC_LOG_DEBUG (1),
                      "no database exists yet, requesting AXFR of " "initial version from %s", master);
        xfrtype = dns_rdatatype_axfr;
    }
    else if (DNS_ZONE_OPTION (zone, DNS_ZONEOPT_IXFRFROMDIFFS))
    {
        dns_zone_log (zone, ISC_LOG_DEBUG (1), "ixfr-from-differences "
                      "set, requesting %sAXFR from %s", soa_before, master);
        if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_SOABEFOREAXFR))
    ...
    {
        dns_zone_log (zone, ISC_LOG_DEBUG (1), "forced reload, requesting AXFR of " "initial version from %s", master);
        xfrtype = dns_rdatatype_axfr;
    }
    else if (DNS_ZONE_FLAG (zone, DNS_ZONEFLAG_NOIXFR))
    {
        dns_zone_log (zone, ISC_LOG_DEBUG (1), "retrying with AXFR from %s due to " "previous IXFR failure", master);
        xfrtype = dns_rdatatype_axfr;
        LOCK_ZONE (zone);
    ...
    else
    {
        ...
        if (peer != NULL && dns_peer_getrequestixfr (peer, &use_ixfr) == ISC_R_SUCCESS)
        {
            ;                    /* Using peer setting */
        }
        else
        ...
        {
            dns_zone_log (zone, ISC_LOG_DEBUG (1), "IXFR disabled, requesting %sAXFR from %s", soa_before, master);
            if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_SOABEFOREAXFR))
                xfrtype = dns_rdatatype_soa;
            else
                xfrtype = dns_rdatatype_axfr;
        ...
        {
            dns_zone_log (zone, ISC_LOG_DEBUG (1), "requesting IXFR from %s", master);
    ...
    if (zone->tsigkey == NULL)
        result = dns_view_getpeertsig (zone->view, &masterip, &zone->tsigkey);

    if (result != ISC_R_SUCCESS && result != ISC_R_NOTFOUND)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "could not get TSIG key for zone transfer: %s", isc_result_totext (result));
    }
    ...
    result = dns_xfrin_create2 (zone, xfrtype, &masteraddr, &sourceaddr,
                                ...
                                zone->zmgr->timermgr, zone->zmgr->socketmgr, zone->task, zone_xfrdone, &zone->xfr);
    if (result == ISC_R_SUCCESS)
    {
        LOCK_ZONE (zone);
...
static isc_result_t sendtomaster (dns_forward_t * forward)
{
    ...
    result = dns_request_createraw (forward->zone->view->requestmgr,
                                    forward->msgbuf, &src, &forward->addr, DNS_REQUESTOPT_TCP, 15 /* XXX */ ,
                                    forward->zone->task, forward_callback, forward, &forward->request);
    if (result == ISC_R_SUCCESS)
    {
        if (!ISC_LINK_LINKED (forward, link))
...
static void forward_callback (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        dns_zone_log (zone, ISC_LOG_INFO,
                      "could not forward dynamic update to %s: %s", master, dns_result_totext (revent->result));
        goto next_master;
    }
    ...
    result = dns_request_getresponse (revent->request, msg,
                                      DNS_MESSAGEPARSE_PRESERVEORDER | DNS_MESSAGEPARSE_CLONEBUFFER);
    if (result != ISC_R_SUCCESS)
        goto next_master;
        ...
        case dns_rcode_notauth:
            {
                ...
                dns_zone_log (zone, ISC_LOG_WARNING,
                              ...
                              "unexpected response: master %s returned: %.*s", master, (int) rb.used, rcode);
                goto next_master;
            }
  ...
  next_master:
    ...
    {
        /* call callback */
        dns_zone_log (zone, ISC_LOG_DEBUG (3), "exhausted dynamic update forwarder list");
...
isc_result_t
dns_zone_forwardupdate (dns_zone_t * zone, dns_message_t * msg, dns_updatecallback_t callback, void *callback_arg)
{
    dns_forward_t *forward;
...
dns_zonemgr_create (isc_mem_t * mctx, isc_taskmgr_t * taskmgr,
                    isc_timermgr_t * timermgr, isc_socketmgr_t * socketmgr, dns_zonemgr_t ** zmgrp)
{
    dns_zonemgr_t *zmgr;
    ...
    result = isc_ratelimiter_create (mctx, timermgr, zmgr->task, &zmgr->rl);
    if (result != ISC_R_SUCCESS)
        goto free_task;
...
isc_result_t dns_zonemgr_managezone (dns_zonemgr_t * zmgr, dns_zone_t * zone)
{
    ...
    result = isc_timer_create (zmgr->timermgr, isc_timertype_inactive,
                               NULL, NULL, zone->task, zone_timer, zone, &zone->timer);
...
isc_result_t dns_zonemgr_forcemaint (dns_zonemgr_t * zmgr)
{
    ...
    for (p = ISC_LIST_HEAD (zmgr->zones); p != NULL; p = ISC_LIST_NEXT (p, link))
    {
...
void dns_zonemgr_shutdown (dns_zonemgr_t * zmgr)
{
    ...
    RWLOCK (&zmgr->rwlock, isc_rwlocktype_read);
    for (zone = ISC_LIST_HEAD (zmgr->zones); zone != NULL; zone = ISC_LIST_NEXT (zone, link))
    {
        LOCK_ZONE (zone);
        forward_cancel (zone);
        UNLOCK_ZONE (zone);
...
isc_result_t dns_zonemgr_setsize (dns_zonemgr_t * zmgr, int num_zones)
{
    ...
    if (zmgr->zonetasks == NULL)
        result = isc_taskpool_create (zmgr->taskmgr, zmgr->mctx, ntasks, 2, &pool);
    else
...
static void zmgr_resume_xfrs (dns_zonemgr_t * zmgr, isc_boolean_t multi)
{
    ...
    for (zone = ISC_LIST_HEAD (zmgr->waiting_for_xfrin); zone != NULL; zone = next)
    {
        isc_result_t result;
        ...
        {
            dns_zone_log (zone, ISC_LOG_DEBUG (1), "starting zone transfer: %s", isc_result_totext (result));
            break;
        }
    }
}
...
static isc_result_t zmgr_start_xfrin_ifquota (dns_zonemgr_t * zmgr, dns_zone_t * zone)
{
    ...
    for (x = ISC_LIST_HEAD (zmgr->xfrin_in_progress); x != NULL; x = ISC_LIST_NEXT (x, statelink))
    {
        isc_netaddr_t xip;
  ...
  gotquota:
     ...
     */
    e = isc_event_allocate (zmgr->mctx, zmgr, DNS_EVENT_ZONESTARTXFRIN, got_transfer_quota, zone, sizeof (isc_event_t));
    if (e == NULL)
        return (ISC_R_NOMEMORY);
               ...
               isc_task_t * task, isc_taskaction_t action, void *arg, dns_io_t ** iop)
{
    ...
    if (io == NULL)
        return (ISC_R_NOMEMORY);
    io->event = isc_event_allocate (zmgr->mctx, task, DNS_EVENT_IOREADY, action, arg, sizeof (*io->event));
    if (io->event == NULL)
    {
...
static void zone_saveunique (dns_zone_t * zone, const char *path, const char *templat)
{
    ...
    dns_zone_log (zone, ISC_LOG_WARNING, "unable to load from '%s'; "
                  "renaming file to '%s' for failure analysis and " "retransferring.", path, buf);
...
static void dns_zonemgr_dbdestroyed (isc_task_t * task, isc_event_t * event)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                   DNS_LOGMODULE_ZONE, ISC_LOG_DEBUG (3), "database (%p) destroyed", (void *) db);
}
#endif
...
isc_boolean_t
dns_zonemgr_unreachable (dns_zonemgr_t * zmgr, isc_sockaddr_t * remote, isc_sockaddr_t * local, isc_time_t * now)
{
    unsigned int i;
            ...
            isc_sockaddr_equal (&zmgr->unreachable[i].local, local))
        {
            ...
            if (result == ISC_R_SUCCESS)
            {
                ...
                isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                               ...
                               "master %s (source %s) deleted " "from unreachable cache", master, source);
            }
            break;
        }
    }
...
void
dns_zonemgr_unreachableadd (dns_zonemgr_t * zmgr, isc_sockaddr_t * remote, isc_sockaddr_t * local, isc_time_t * now)
{
    isc_uint32_t seconds = isc_time_seconds (now);
...
void dns_zone_dialup (dns_zone_t * zone)
{
    ...
    zone_debuglog (zone, "dns_zone_dialup", 3,
                   "notify = %d, refresh = %d",
                   DNS_ZONE_FLAG (zone, DNS_ZONEFLG_DIALNOTIFY), DNS_ZONE_FLAG (zone, DNS_ZONEFLG_DIALREFRESH));

    if (DNS_ZONE_FLAG (zone, DNS_ZONEFLG_DIALNOTIFY))
        dns_zone_notify (zone);
    if (zone->type != dns_zone_master && DNS_ZONE_FLAG (zone, DNS_ZONEFLG_DIALREFRESH))
        dns_zone_refresh (zone);
}
...
void dns_zone_setdialup (dns_zone_t * zone, dns_dialuptype_t dialup)
{
    ...
    LOCK_ZONE (zone);
    DNS_ZONE_CLRFLAG (zone, DNS_ZONEFLG_DIALNOTIFY | DNS_ZONEFLG_DIALREFRESH | DNS_ZONEFLG_NOREFRESH);
    switch (dialup)
    {
...
unsigned int dns_zonemgr_getcount (dns_zonemgr_t * zmgr, int state)
{
    ...
    switch (state)
    {
        case DNS_ZONESTATE_XFERRUNNING:
            for (zone = ISC_LIST_HEAD (zmgr->xfrin_in_progress); zone != NULL; zone = ISC_LIST_NEXT (zone, statelink))
                count++;
            break;
        case DNS_ZONESTATE_XFERDEFERRED:
            for (zone = ISC_LIST_HEAD (zmgr->waiting_for_xfrin); zone != NULL; zone = ISC_LIST_NEXT (zone, statelink))
                count++;
            break;
...
isc_result_t dns_zone_checknames (dns_zone_t * zone, dns_name_t * name, dns_rdata_t * rdata)
{
    ...
    if (!ok)
    {
        ...
        dns_zone_log (zone, level, "%s/%s: %s", namebuf, typebuf, dns_result_totext (DNS_R_BADOWNERNAME));
        if (fail)
    ...
    if (!ok)
    {
        ...
        dns_rdatatype_format (rdata->type, typebuf, sizeof (typebuf));
        dns_zone_log (zone, level, "%s/%s: %s: %s ", namebuf, typebuf, namebuf2, dns_result_totext (DNS_R_BADNAME));
        if (fail)
...
isc_result_t dns_zone_signwithkey (dns_zone_t * zone, dns_secalg_t algorithm, isc_uint16_t keyid, isc_boolean_t delete)
{
    ...
    dns_zone_log (zone, ISC_LOG_NOTICE, "dns_zone_signwithkey(algorithm=%u, keyid=%u)", algorithm, keyid);
    LOCK_ZONE (zone);
...
static isc_result_t
zone_signwithkey (dns_zone_t * zone, dns_secalg_t algorithm, isc_uint16_t keyid, isc_boolean_t delete)
{
    dns_signing_t *signing;
    ...
    for (current = ISC_LIST_HEAD (zone->signing); current != NULL; current = ISC_LIST_NEXT (current, link))
    {
        if (current->db == zone->db && current->algorithm == signing->algorithm && current->keyid == signing->keyid)
        {
...
static void logmsg (const char *format, ...)
{
    ...
    isc_log_vwrite (dns_lctx, DNS_LOGCATEGORY_GENERAL, DNS_LOGMODULE_ZONE, ISC_LOG_DEBUG (1), format, args);
    va_end (args);
}
...
static isc_result_t next_keyevent (dst_key_t * key, isc_stdtime_t * timep)
{
    ...
    for (i = 0; i <= DST_MAX_TIMES; i++)
    {
        ...
        if (result == ISC_R_SUCCESS && event > now && (then == 0 || event < then))
            then = event;
    }
...
static isc_result_t
rr_exists (dns_db_t * db, dns_dbversion_t * ver, dns_name_t * name, const dns_rdata_t * rdata, isc_boolean_t * flag)
{
    dns_rdataset_t rdataset;
    ...
    result = dns_db_findrdataset (db, node, ver, rdata->type, 0, (isc_stdtime_t) 0, &rdataset, NULL);
    if (result == ISC_R_NOTFOUND)
    {
        *flag = ISC_FALSE;
        result = ISC_R_SUCCESS;
...
add_signing_records (dns_db_t * db, dns_rdatatype_t privatetype,
                     dns_dbversion_t * ver, dns_diff_t * diff, isc_boolean_t sign_all)
{
    ...
    for (tuple = ISC_LIST_HEAD (diff->tuples); tuple != NULL; tuple = ISC_LIST_NEXT (tuple, link))
    {
        ...
        if (sign_all || tuple->op == DNS_DIFFOP_DEL)
        {
            ...
            if (flag)
                continue;
            CHECK (dns_difftuple_create (diff->mctx, DNS_DIFFOP_ADD, name, 0, &rdata, &newtuple));
        ...
        if (flag)
        {
            CHECK (dns_difftuple_create (diff->mctx, DNS_DIFFOP_DEL, name, 0, &rdata, &newtuple));
            CHECK (do_one_tuple (&newtuple, db, ver, diff));
            INSIST (newtuple == NULL);
        }
    }
...
static isc_result_t
sign_apex (dns_zone_t * zone, dns_db_t * db, dns_dbversion_t * ver, dns_diff_t * diff, dns_diff_t * sig_diff)
{
    isc_result_t result;
    ...
    result = find_zone_keys (zone, db, ver, zone->mctx, DNS_MAXZONEKEYS, zone_keys, &nkeys);
    if (result != ISC_R_SUCCESS)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "sign_apex:find_zone_keys -> %s\n", dns_result_totext (result));
        return (result);
    }
    ...
    for (tuple = ISC_LIST_HEAD (diff->tuples); tuple != NULL; tuple = ISC_LIST_NEXT (tuple, link))
    {
        if (tuple->rdata.type == dns_rdatatype_dnskey && dns_name_equal (&tuple->name, &zone->origin))
            break;
    }
    ...
    if (tuple == NULL)
    {
        result = del_sigs (zone, db, ver, &zone->origin,
                           dns_rdatatype_dnskey, sig_diff, zone_keys, nkeys, now, ISC_FALSE);
        if (result != ISC_R_SUCCESS)
        {
            dns_zone_log (zone, ISC_LOG_ERROR, "sign_apex:del_sigs -> %s\n", dns_result_totext (result));
            goto failure;
        }
    ...
    result = update_sigs (diff, db, ver, zone_keys, nkeys, zone,
                          inception, soaexpire, now, check_ksk, keyset_kskonly, sig_diff);
...
static isc_boolean_t dnskey_sane (dns_zone_t * zone, dns_db_t * db, dns_dbversion_t * ver, dns_diff_t * diff)
{
    ...
    for (tuple = ISC_LIST_HEAD (diff->tuples); tuple != NULL; tuple = ISC_LIST_NEXT (tuple, link))
    {
        ...
        if (tuple->rdata.type != dns_rdatatype_dnskey || tuple->op != DNS_DIFFOP_ADD)
            continue;

        alg = tuple->rdata.data[3];
        if (alg == DST_ALG_RSAMD5 || alg == DST_ALG_RSASHA1 || alg == DST_ALG_DSA || alg == DST_ALG_ECC)
        {
            nseconly = ISC_TRUE;
            break;
        }
    ...
    if (!nsec3)
        CHECK (dns_nsec3_activex (db, ver, ISC_FALSE, privatetype, &nsec3));

    /* Refuse to allow NSEC3 with NSEC-only keys */
    if (nseconly && nsec3)
    {
        dns_zone_log (zone, ISC_LOG_ERROR, "NSEC only DNSKEYs and NSEC3 chains not allowed");
        goto failure;
    }
...
static isc_result_t clean_nsec3param (dns_zone_t * zone, dns_db_t * db, dns_dbversion_t * ver, dns_diff_t * diff)
{
    ...
    result = dns_db_findrdataset (db, node, ver, dns_rdatatype_dnskey, dns_rdatatype_none, 0, &rdataset, NULL);
    if (dns_rdataset_isassociated (&rdataset))
        dns_rdataset_disassociate (&rdataset);
    if (result != ISC_R_NOTFOUND)
        goto failure;
...
static isc_result_t add_chains (dns_zone_t * zone, dns_db_t * db, dns_dbversion_t * ver, dns_diff_t * diff)
{
    ...
    CHECK (dns_private_chains (db, ver, zone->privatetype, NULL, &build_nsec3));
    if (build_nsec3)
        CHECK (dns_nsec3_addnsec3sx (db, ver, origin, zone->minimum, ISC_FALSE, zone->privatetype, diff));
...
static void zone_rekey (dns_zone_t * zone)
{
    ...
    CHECK (dns_db_findrdataset (db, node, ver, dns_rdatatype_soa, dns_rdatatype_none, 0, &soaset, &soasigs));
    ttl = soaset.ttl;
    dns_rdataset_disassociate (&soaset);

    /* Get the DNSKEY rdataset */
    result = dns_db_findrdataset (db, node, ver, dns_rdatatype_dnskey, dns_rdatatype_none, 0, &keyset, &keysigs);
    if (result == ISC_R_SUCCESS)
    {
        ttl = keyset.ttl;
        result = dns_dnssec_keylistfromrdataset (&zone->origin, dir,
                                                 mctx, &keyset, &keysigs, &soasigs, ISC_FALSE, ISC_FALSE, &dnskeys);
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        result = dns_dnssec_updatekeys (&dnskeys, &keys, &rmkeys,
                                        &zone->origin, ttl, &diff, ISC_TF (!check_ksk), mctx, logmsg);
        ...
        if (result != ISC_R_SUCCESS)
        {
            dns_zone_log (zone, ISC_LOG_ERROR, "zone_rekey:"
                          "couldn't update zone keys: %s", isc_result_totext (result));
            goto trylater;
        }
         ...
         */
        for (key = ISC_LIST_HEAD (dnskeys); key != NULL; key = ISC_LIST_NEXT (key, link))
        {
            if (!key->first_sign)
                continue;
        ...
        if ((newactive || fullsign || !ISC_LIST_EMPTY (diff.tuples)) && dnskey_sane (zone, db, ver, &diff))
        {
            CHECK (dns_diff_apply (&diff, db, ver));
            CHECK (clean_nsec3param (zone, db, ver, &diff));
            CHECK (add_signing_records (db, zone->privatetype, ver, &diff, ISC_TF (newalg || fullsign)));
    ...
    if (commit)
    {
        ...
        {
            for (key = ISC_LIST_HEAD (rmkeys); key != NULL; key = ISC_LIST_NEXT (key, link))
            {
                result = zone_signwithkey (zone, dst_key_alg (key->key), dst_key_id (key->key), ISC_TRUE);
                if (result != ISC_R_SUCCESS)
                {
                    dns_zone_log (zone, ISC_LOG_ERROR, "zone_signwithkey failed: %s", dns_result_totext (result));
                }
        ...
        if (fullsign)
        {
            ...
            for (key = ISC_LIST_HEAD (dnskeys); key != NULL; key = ISC_LIST_NEXT (key, link))
            {
                ...
                result = zone_signwithkey (zone, dst_key_alg (key->key), dst_key_id (key->key), ISC_FALSE);
                if (result != ISC_R_SUCCESS)
                {
                    dns_zone_log (zone, ISC_LOG_ERROR, "zone_signwithkey failed: %s", dns_result_totext (result));
                }
         ...
         */
        for (tuple = ISC_LIST_HEAD (sig_diff.tuples); tuple != NULL; tuple = ISC_LIST_NEXT (tuple, link))
        {
            unsigned char buf[DNS_NSEC3PARAM_BUFFERSIZE];
            ...
            if (tuple->rdata.type != zone->privatetype || tuple->op != DNS_DIFFOP_ADD)
                continue;

            if (!dns_nsec3param_fromprivate (&tuple->rdata, &rdata, buf, sizeof (buf)))
                continue;
            result = dns_rdata_tostruct (&rdata, &nsec3param, NULL);
            RUNTIME_CHECK (result == ISC_R_SUCCESS);
            if (nsec3param.flags == 0)
    ...
    if (DNS_ZONEKEY_OPTION (zone, DNS_ZONEKEY_MAINTAIN))
    {
        ...
        for (key = ISC_LIST_HEAD (dnskeys); key != NULL; key = ISC_LIST_NEXT (key, link))
        {
            ...
            DNS_ZONE_TIME_ADD (&timenow, then - now, &timethen);
            LOCK_ZONE (zone);
            if (isc_time_compare (&timethen, &zone->refreshkeytime) < 0)
            {
                zone->refreshkeytime = timethen;
            }
            UNLOCK_ZONE (zone);
...
isc_result_t dns_zone_nscheck (dns_zone_t * zone, dns_db_t * db, dns_dbversion_t * version, unsigned int *errors)
{
    ...
    if (result != ISC_R_SUCCESS)
        return (result);
    result = zone_count_ns_rr (zone, db, node, version, NULL, errors, ISC_FALSE);
    dns_db_detachnode (db, &node);
    return (result);
}
