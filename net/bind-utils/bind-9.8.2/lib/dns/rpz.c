 ...
 */
void dns_rpz_cidr_free (dns_rpz_cidr_t ** cidrp)
{
 ...
 */
void dns_rpz_view_destroy (dns_view_t * view)
{
    dns_rpz_zone_t *zone;
 ...
 */
void dns_rpz_set_need (isc_boolean_t need)
{
    have_rpz_zones = need;
}

isc_boolean_t dns_rpz_needed (void)
{
 ...
 */
isc_result_t dns_rpz_new_cidr (isc_mem_t * mctx, dns_name_t * origin, dns_rpz_cidr_t ** rbtdb_cidr)
{
    isc_result_t result;
    ...
    result = dns_name_fromstring2 (&cidr->ip_name, DNS_RPZ_IP_ZONE, origin, DNS_NAME_DOWNCASE, mctx);
    ...
    result = dns_name_fromstring2 (&cidr->nsip_name, DNS_RPZ_NSIP_ZONE, origin, DNS_NAME_DOWNCASE, mctx);
    if (result != ISC_R_SUCCESS)
    {
    ...
    result = dns_name_fromstring2 (&cidr->nsdname_name, DNS_RPZ_NSDNAME_ZONE, origin, DNS_NAME_DOWNCASE, mctx);
    if (result != ISC_R_SUCCESS)
    {
 ...
 */
void dns_rpz_enabled (dns_rpz_cidr_t * cidr, dns_rpz_st_t * st)
{
    if (cidr == NULL)
        return;
    if (cidr->root != NULL && (cidr->root->flags & DNS_RPZ_CIDR_FG_IP) != 0)
        st->state |= DNS_RPZ_HAVE_IP;
    if (cidr->root != NULL && (cidr->root->flags & DNS_RPZ_CIDR_FG_NSIPv4) != 0)
        st->state |= DNS_RPZ_HAVE_NSIPv4;
    if (cidr->root != NULL && (cidr->root->flags & DNS_RPZ_CIDR_FG_NSIPv6) != 0)
...
get_flags (const dns_rpz_cidr_key_t * ip, dns_rpz_cidr_bits_t prefix, dns_rpz_type_t rpz_type)
{
    if (rpz_type == DNS_RPZ_TYPE_NSIP)
    {
        if (prefix >= 96 && ip->w[0] == 0 && ip->w[1] == 0 && ip->w[2] == ADDR_V4MAPPED)
            return (DNS_RPZ_CIDR_FG_NSIP_DATA | DNS_RPZ_CIDR_FG_NSIPv4);
        else
            return (DNS_RPZ_CIDR_FG_NSIP_DATA | DNS_RPZ_CIDR_FG_NSIPv6);
    }
    else
    {
 ...
 */
static void set_node_flags (dns_rpz_cidr_node_t * node, dns_rpz_type_t rpz_type)
{
    dns_rpz_cidr_flags_t flags;
 ...
 */
static dns_rpz_cidr_node_t *new_node (dns_rpz_cidr_t * cidr, const dns_rpz_cidr_key_t * ip,
                                      dns_rpz_cidr_bits_t bits, dns_rpz_cidr_flags_t flags)
{
    ...
    node->flags = flags & ~(DNS_RPZ_CIDR_FG_IP_DATA | DNS_RPZ_CIDR_FG_NSIP_DATA);
...
static void badname (int level, dns_name_t * name, const char *comment)
{
    ...
    if (isc_log_wouldlog (dns_lctx, level))
    {
        ...
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_RPZ,
                       DNS_LOGMODULE_RBTDB, level, "invalid rpz \"%s\"%s", printname, comment);
    }
}
...
ip2name (dns_rpz_cidr_t * cidr, const dns_rpz_cidr_key_t * tgt_ip,
         dns_rpz_cidr_bits_t tgt_prefix, dns_rpz_type_t type, dns_name_t * canon_name, dns_name_t * search_name)
{
...
#define INET6_ADDRSTRLEN 46
    ...
    if (tgt_prefix > 96 && tgt_ip->w[0] == 0 && tgt_ip->w[1] == 0 && tgt_ip->w[2] == ADDR_V4MAPPED)
    {
        len = snprintf (str, sizeof (str), "%d.%d.%d.%d.%d",
                        tgt_prefix - 96,
                        tgt_ip->w[3] & 0xff,
                        (tgt_ip->w[3] >> 8) & 0xff, (tgt_ip->w[3] >> 16) & 0xff, (tgt_ip->w[3] >> 24) & 0xff);
    ...
    else
    {
        ...
        {
            w[i * 2 + 1] = ((tgt_ip->w[DNS_RPZ_CIDR_WORDS - 1 - i] >> 16) & 0xffff);
        ...
        {
            if (w[i] != 0 || zeros || i >= DNS_RPZ_CIDR_WORDS * 2 - 1 || w[i + 1] != 0)
            {
                INSIST ((size_t) len <= sizeof (str));
                n = snprintf (&str[len], sizeof (str) - len, ".%x", w[i++]);
                if (n < 0)
                    return (ISC_R_FAILURE);
                len += n;
            }
            else
            {
                zeros = ISC_TRUE;
                INSIST ((size_t) len <= sizeof (str));
                n = snprintf (&str[len], sizeof (str) - len, ".zz");
                if (n < 0)
                    return (ISC_R_FAILURE);
                len += n;
                i += 2;
    ...
    if (canon_name != NULL)
    {
        ...
        result = dns_name_fromtext (canon_name, &buffer, dns_rootname, 0, NULL);
        if (result != ISC_R_SUCCESS)
            return (result);
    }
    if (search_name != NULL)
 ...
 */
static dns_rpz_type_t set_type (dns_rpz_cidr_t * cidr, dns_name_t * name)
{
...
name2ipkey (dns_rpz_cidr_t * cidr, int level, dns_name_t * src_name,
            dns_rpz_type_t type, dns_rpz_cidr_key_t * tgt_ip, dns_rpz_cidr_bits_t * tgt_prefix)
{
    isc_buffer_t buffer;
    ...
    dns_name_split (src_name, dns_name_countlabels (&cidr->ip_name), name, NULL);
    ...
    else
    {
         ...
         */
        *tgt_prefix = (dns_rpz_cidr_bits_t) prefix;
        for (i = 0; ip_labels > 0 && i < DNS_RPZ_CIDR_WORDS * 2; ip_labels--)
        {
            if (cp[0] == 'z' && cp[1] == 'z' && (cp[2] == '.' || cp[2] == '\0') && i <= 6)
            {
                do
                {
                    if ((i & 1) == 0)
    ...
    if (ISC_R_SUCCESS != ip2name (cidr, tgt_ip, (dns_rpz_cidr_bits_t) prefix,
                                  type, NULL, name) || !dns_name_equal (src_name, name))
    {
 ...
 */
static int ffbit (dns_rpz_cidr_word_t w)
{
    int bit;
           ...
           const dns_rpz_cidr_key_t * key2, dns_rpz_cidr_bits_t bits2)
{
     ...
     */
    for (i = 0, bit = 0; bit <= maxbit; i++, bit += DNS_RPZ_CIDR_WORD_BITS)
    {
 ...
 *    ISC_R_NOMEMORY
 */
static isc_result_t search (dns_rpz_cidr_t * cidr, const dns_rpz_cidr_key_t * tgt_ip, dns_rpz_cidr_bits_t tgt_prefix, dns_rpz_type_t type, isc_boolean_t create, dns_rpz_cidr_node_t ** found)    /* NULL or longest match node */
{
    ...
    data_flag = flags & (DNS_RPZ_CIDR_FG_IP_DATA | DNS_RPZ_CIDR_FG_NSIP_DATA);
    ...
    for (;;)
    {
        ...
        if (dbit == tgt_prefix)
        {
            ...
            new_parent = new_node (cidr, tgt_ip, tgt_prefix, cur->flags);
            if (new_parent == NULL)
                return (ISC_R_NOMEMORY);
 ...
 */
void dns_rpz_cidr_addip (dns_rpz_cidr_t * cidr, dns_name_t * name)
{
    ...
    if (ISC_R_SUCCESS != name2ipkey (cidr, DNS_RPZ_ERROR_LEVEL, name, type, &tgt_ip, &tgt_prefix))
        return;

    if (ISC_R_EXISTS == search (cidr, &tgt_ip, tgt_prefix, type,
                                ISC_TRUE, NULL) && isc_log_wouldlog (dns_lctx, DNS_RPZ_ERROR_LEVEL))
    {
        char printname[DNS_NAME_FORMATSIZE];

        dns_name_format (name, printname, sizeof (printname));
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_RPZ,
                       DNS_LOGMODULE_RBTDB, DNS_RPZ_ERROR_LEVEL, "duplicate rpz name \"%s\"", printname);
    }
}
 ...
 *    The tree write lock must be held by the caller.
 */
void dns_rpz_cidr_deleteip (dns_rpz_cidr_t * cidr, dns_name_t * name)
{
    dns_rpz_cidr_key_t tgt_ip;
     ...
     */
    if (ISC_R_SUCCESS != name2ipkey (cidr, DNS_RPZ_DEBUG_LEVEL3, name, type, &tgt_ip, &tgt_prefix))
        return;
    if (ISC_R_SUCCESS != search (cidr, &tgt_ip, tgt_prefix, type, ISC_FALSE, &tgt))
    {
        if (isc_log_wouldlog (dns_lctx, DNS_RPZ_ERROR_LEVEL))
        {
            ...
            dns_name_format (name, printname, sizeof (printname));
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_RPZ,
                           DNS_LOGMODULE_RBTDB, DNS_RPZ_ERROR_LEVEL, "missing rpz node \"%s\"", printname);
        }
        return;
    }
    ...
    for (parent = tgt; parent != NULL; parent = parent->parent)
    {
        if ((parent->flags & data_flag) != 0 ||
            ...
            (parent->child[1] != NULL && (parent->child[1]->flags & flags) != 0))
            break;
        parent->flags &= ~flags;
    }
    ...
    do
    {
        ...
        if ((tgt->flags & (DNS_RPZ_CIDR_FG_IP_DATA | DNS_RPZ_CIDR_FG_NSIP_DATA)) != 0)
            return;
...
dns_rpz_cidr_find (dns_rpz_cidr_t * cidr, const isc_netaddr_t * netaddr,
                   dns_rpz_type_t type, dns_name_t * canon_name, dns_name_t * search_name, dns_rpz_cidr_bits_t * prefix)
{
    dns_rpz_cidr_key_t tgt_ip;
    ...
    *prefix = found->bits;
    return (ip2name (cidr, &found->ip, found->bits, type, canon_name, search_name));
}

/*
 * Translate CNAME rdata to a QNAME response policy action.
 */
dns_rpz_policy_t dns_rpz_decode_cname (dns_rdataset_t * rdataset, dns_name_t * selfname)
{
