...
dns_difftuple_create (isc_mem_t * mctx,
                      dns_diffop_t op, dns_name_t * name, dns_ttl_t ttl, dns_rdata_t * rdata, dns_difftuple_t ** tp)
{
    dns_difftuple_t *t;
...
{
    return (dns_difftuple_create (orig->mctx, orig->op, &orig->name, orig->ttl, &orig->rdata, copyp));
}
...
void dns_diff_appendminimal (dns_diff_t * diff, dns_difftuple_t ** tuplep)
{
    ...
    for (ot = ISC_LIST_HEAD (diff->tuples); ot != NULL; ot = next_ot)
    {
        ...
        if (dns_name_equal (&ot->name, &(*tuplep)->name) &&
            dns_rdata_compare (&ot->rdata, &(*tuplep)->rdata) == 0 && ot->ttl == (*tuplep)->ttl)
        {
            ISC_LIST_UNLINK (diff->tuples, ot, link);
            if ((*tuplep)->op == ot->op)
            {
                UNEXPECTED_ERROR (__FILE__, __LINE__, "unexpected non-minimal diff");
            }
            else
            {
...
static isc_stdtime_t setresign (dns_rdataset_t * modified, isc_uint32_t delta)
{
...
isc_result_t dns_diff_sort (dns_diff_t * diff, dns_diff_compare_func * compare)
{
    ...
    for (p = ISC_LIST_HEAD (diff->tuples); p != NULL; p = ISC_LIST_NEXT (p, link))
        length++;
    if (length == 0)
        return (ISC_R_SUCCESS);
...
static isc_result_t
diff_tuple_tordataset (dns_difftuple_t * t, dns_rdata_t * rdata, dns_rdatalist_t * rdl, dns_rdataset_t * rds)
{
    REQUIRE (DNS_DIFFTUPLE_VALID (t));
    REQUIRE (rdl != NULL);
    REQUIRE (rds != NULL);
