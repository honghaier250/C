...
dns_ds_buildrdata (dns_name_t * owner, dns_rdata_t * key,
                   unsigned int digest_type, unsigned char *buffer, dns_rdata_t * rdata)
{
...
#define CHECK(x)                    \
    if ((x) != 1) {                    \
        ...
        case DNS_DSDIGEST_GOST:
            ...
            dns_name_toregion (name, &r);
            CHECK (EVP_DigestUpdate (&ctx, (const void *) r.base, (size_t) r.length));
            dns_rdata_toregion (key, &r);
            INSIST (r.length >= 4);
            CHECK (EVP_DigestUpdate (&ctx, (const void *) r.base, (size_t) r.length));
    ...
    return (dns_rdata_fromstruct (rdata, key->rdclass, dns_rdatatype_ds, &ds, &b));
}

isc_boolean_t dns_ds_digest_supported (unsigned int digest_type)
{
#ifdef HAVE_OPENSSL_GOST
    return (ISC_TF (digest_type == DNS_DSDIGEST_SHA1 ||
                    digest_type == DNS_DSDIGEST_SHA256 || digest_type == DNS_DSDIGEST_GOST));
#else
    return (ISC_TF (digest_type == DNS_DSDIGEST_SHA1 || digest_type == DNS_DSDIGEST_SHA256));
#endif
}
