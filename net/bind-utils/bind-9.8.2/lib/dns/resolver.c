...
static isc_result_t ncache_adderesult (dns_message_t * message,
                                       ...
                                       isc_boolean_t optout, dns_rdataset_t * ardataset, isc_result_t * eresultp);
 ...
 */
static inline void inc_stats (dns_resolver_t * res, isc_statscounter_t counter)
{
...
valcreate (fetchctx_t * fctx, dns_adbaddrinfo_t * addrinfo, dns_name_t * name,
           ...
           dns_rdataset_t * sigrdataset, unsigned int valoptions, isc_task_t * task)
{
    ...
    valarg = isc_mem_get (fctx->res->buckets[fctx->bucketnum].mctx, sizeof (*valarg));
    if (valarg == NULL)
    ...
    result = dns_validator_create (fctx->res->view, name, type, rdataset,
                                   sigrdataset, fctx->rmessage, valoptions, task, validated, valarg, &validator);
    if (result == ISC_R_SUCCESS)
    {
        ...
        isc_mem_put (fctx->res->buckets[fctx->bucketnum].mctx, valarg, sizeof (*valarg));
    return (result);
}
...
static isc_boolean_t rrsig_fromchildzone (fetchctx_t * fctx, dns_rdataset_t * rdataset)
{
    ...
    for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
    {
        ...
        namereln = dns_name_fullcompare (&rrsig.signer, &fctx->domain, &order, &labels);
...
static isc_boolean_t fix_mustbedelegationornxdomain (dns_message_t * message, fetchctx_t * fctx)
{
    ...
    if (message->counts[DNS_SECTION_ANSWER] != 0 &&
        (fctx->type == dns_rdatatype_ns ||
         ...
         fctx->type == dns_rdatatype_any || fctx->type == dns_rdatatype_rrsig || fctx->type == dns_rdatatype_dnskey))
    {
        ...
        while (result == ISC_R_SUCCESS)
        {
            ...
            for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
            {
                 ...
                 * RRsig from child?
                 */
                if (type == dns_rdatatype_rrsig && rrsig_fromchildzone (fctx, rdataset))
                    return (ISC_FALSE);
                /*
                ...
                if (fctx->type == dns_rdatatype_any &&
                    (type == dns_rdatatype_ns ||
                     type == dns_rdatatype_ds || type == dns_rdatatype_soa || type == dns_rdatatype_dnskey))
                    return (ISC_FALSE);
            }
     ...
     */
    if (fctx->type == dns_rdatatype_ds && message->counts[DNS_SECTION_ANSWER] == 0)
        return (ISC_FALSE);
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        {
            type = rdataset->type;
            if (type == dns_rdatatype_soa && dns_name_equal (name, domain))
                keep_auth = ISC_TRUE;

            if (type != dns_rdatatype_ns && type != dns_rdatatype_soa && type != dns_rdatatype_rrsig)
                continue;
            ...
            if (dns_name_equal (name, domain))
            {
                 ...
                 * case.
                 */
                if (message->counts[DNS_SECTION_ANSWER] == 0 && fctx->type == dns_rdatatype_any)
                    return (ISC_FALSE);
            }
...
static inline isc_result_t fctx_starttimer (fetchctx_t * fctx)
{
    /*
     ...
     */
    return (isc_timer_reset (fctx->timer, isc_timertype_once, &fctx->expires, NULL, ISC_TRUE));
}

static inline void fctx_stoptimer (fetchctx_t * fctx)
{
    isc_result_t result;
     ...
     */
    result = isc_timer_reset (fctx->timer, isc_timertype_inactive, NULL, NULL, ISC_TRUE);
    if (result != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_timer_reset(): %s", isc_result_totext (result));
    }
}


static inline isc_result_t fctx_startidletimer (fetchctx_t * fctx, isc_interval_t * interval)
{
    /*
     * Start the idle timer for fctx.  The lifetime timer continues
     * to be in effect.
     */
    return (isc_timer_reset (fctx->timer, isc_timertype_once, &fctx->expires, interval, ISC_FALSE));
}
...
static inline void resquery_destroy (resquery_t ** queryp)
{
    resquery_t *query;
...
static void
fctx_cancelquery (resquery_t ** queryp, dns_dispatchevent_t ** deventp, isc_time_t * finish, isc_boolean_t no_response)
{
    fetchctx_t *fctx;
    ...
    if (finish != NULL || no_response)
    {
        if (finish != NULL)
        {
            ...
            {
                inc_stats (fctx->res, dns_resstatscounter_queryrtt0);
            }
            else if (rttms < DNS_RESOLVER_QRYRTTCLASS1)
            {
                inc_stats (fctx->res, dns_resstatscounter_queryrtt1);
            }
            else if (rttms < DNS_RESOLVER_QRYRTTCLASS2)
            {
                inc_stats (fctx->res, dns_resstatscounter_queryrtt2);
            }
            ...
            {
                inc_stats (fctx->res, dns_resstatscounter_queryrtt4);
            }
            else
            {
                inc_stats (fctx->res, dns_resstatscounter_queryrtt5);
            }
        }
        else
        {
    ...
    {
        dns_adb_changeflags (fctx->adb, query->addrinfo, FCTX_ADDRINFO_TRIED, FCTX_ADDRINFO_TRIED);
    }
    ...
    if (finish != NULL)
        for (addrinfo = ISC_LIST_HEAD (fctx->forwaddrs); addrinfo != NULL; addrinfo = ISC_LIST_NEXT (addrinfo, publink))
            if (UNMARKED (addrinfo))
                dns_adb_adjustsrtt (fctx->adb, addrinfo, 0, factor);

    if (finish != NULL && TRIEDFIND (fctx))
        for (find = ISC_LIST_HEAD (fctx->finds); find != NULL; find = ISC_LIST_NEXT (find, publink))
            for (addrinfo = ISC_LIST_HEAD (find->list); addrinfo != NULL; addrinfo = ISC_LIST_NEXT (addrinfo, publink))
                if (UNMARKED (addrinfo))
                    dns_adb_adjustsrtt (fctx->adb, addrinfo, 0, factor);

    if (finish != NULL && TRIEDALT (fctx))
    {
        for (addrinfo = ISC_LIST_HEAD (fctx->altaddrs); addrinfo != NULL; addrinfo = ISC_LIST_NEXT (addrinfo, publink))
            if (UNMARKED (addrinfo))
                dns_adb_adjustsrtt (fctx->adb, addrinfo, 0, factor);
        for (find = ISC_LIST_HEAD (fctx->altfinds); find != NULL; find = ISC_LIST_NEXT (find, publink))
            for (addrinfo = ISC_LIST_HEAD (find->list); addrinfo != NULL; addrinfo = ISC_LIST_NEXT (addrinfo, publink))
                if (UNMARKED (addrinfo))
                    dns_adb_adjustsrtt (fctx->adb, addrinfo, 0, factor);
    }
    ...
    if (RESQUERY_CONNECTING (query))
    {
        ...
        {
            isc_socket_cancel (query->tcpsocket, NULL, ISC_SOCKCANCEL_CONNECT);
        }
...
static void fctx_cleanupaltfinds (fetchctx_t * fctx)
{
    ...
    for (find = ISC_LIST_HEAD (fctx->altfinds); find != NULL; find = next_find)
    {
        next_find = ISC_LIST_NEXT (find, publink);
...
static void fctx_cleanupforwaddrs (fetchctx_t * fctx)
{
    ...
    for (addr = ISC_LIST_HEAD (fctx->forwaddrs); addr != NULL; addr = next_addr)
    {
...
static void fctx_cleanupaltaddrs (fetchctx_t * fctx)
{
    ...
    for (addr = ISC_LIST_HEAD (fctx->altaddrs); addr != NULL; addr = next_addr)
    {
        next_addr = ISC_LIST_NEXT (addr, publink);
...
static inline void fctx_stopeverything (fetchctx_t * fctx, isc_boolean_t no_response)
{
...
static inline void fctx_sendevents (fetchctx_t * fctx, isc_result_t result, int line)
{
    ...
    for (event = ISC_LIST_HEAD (fctx->events); event != NULL; event = next_event)
    {
        ...
        INSIST (result != ISC_R_SUCCESS ||
                ...
                fctx->type == dns_rdatatype_rrsig || fctx->type == dns_rdatatype_sig);

        /*
         * Negative results must be indicated in event->result.
         */
        if (dns_rdataset_isassociated (event->rdataset) && NEGATIVE (event->rdataset))
        {
            INSIST (event->result == DNS_R_NCACHENXDOMAIN || event->result == DNS_R_NCACHENXRRSET);
        }
    ...
    if ((fctx->attributes & FCTX_ATTR_HAVEANSWER) != 0 &&
        fctx->spilled && (count < fctx->res->spillatmax || fctx->res->spillatmax == 0))
    {
        LOCK (&fctx->res->lock);
        if (count == fctx->res->spillat && !fctx->res->exiting)
        {
            ...
            fctx->res->spillat += 5;
            if (fctx->res->spillat > fctx->res->spillatmax && fctx->res->spillatmax != 0)
            ...
            result = isc_timer_reset (fctx->res->spillattimer, isc_timertype_ticker, NULL, &i, ISC_TRUE);
            RUNTIME_CHECK (result == ISC_R_SUCCESS);
        }
        UNLOCK (&fctx->res->lock);
        if (logit)
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_RESOLVER,
                           DNS_LOGMODULE_RESOLVER, ISC_LOG_NOTICE, "clients-per-query increased to %u", new_spillat);
    }
}

static inline void log_edns (fetchctx_t * fctx)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_EDNS_DISABLED,
                   ...
                   "success resolving '%s' (in '%s'?) after %s", fctx->info, domainbuf, fctx->reason);
    ...
    else
    {
        switch (sevent->result)
        {
            ...
            case ISC_R_CONNREFUSED:
                 ...
                 * No route to remote.
                 */
                add_bad (fctx, query->addrinfo, sevent->result, badns_unreachable);
...
fctx_addopt (dns_message_t * message, unsigned int version, isc_uint16_t udpsize, isc_boolean_t request_nsid)
{
    dns_rdataset_t *rdataset;
...
static inline void fctx_setretryinterval (fetchctx_t * fctx, unsigned int rtt)
{
...
static isc_result_t fctx_query (fetchctx_t * fctx, dns_adbaddrinfo_t * addrinfo, unsigned int options)
{
    ...
    query = isc_mem_get (res->buckets[fctx->bucketnum].mctx, sizeof (*query));
    if (query == NULL)
    {
    ...
    if (res->view->peers != NULL)
    {
        ...
        result = dns_peerlist_peerbyaddr (res->view->peers, &dstip, &peer);
        if (result == ISC_R_SUCCESS)
        {
    ...
    if ((query->options & DNS_FETCHOPT_TCP) != 0)
    {
        ...
        if (!have_addr)
        {
            ...
            {
                case PF_INET:
                    result = dns_dispatch_getlocaladdress (res->dispatchv4, &addr);
                    break;
                case PF_INET6:
                    result = dns_dispatch_getlocaladdress (res->dispatchv6, &addr);
                    break;
                default:
        ...
        result = isc_socket_create (res->socketmgr, pf, isc_sockettype_tcp, &query->tcpsocket);
        if (result != ISC_R_SUCCESS)
            goto cleanup_query;
    ...
    else
    {
        if (have_addr)
        {
            ...
            result = dns_dispatch_getudp (res->dispatchmgr,
                                          res->socketmgr,
                                          res->taskmgr, &addr,
                                          4096, 1000, 32768, 16411, 16433, attrs, attrmask, &query->dispatch);
            if (result != ISC_R_SUCCESS)
                goto cleanup_query;
        }
        else
    ...
    if (res->view->resquerystats != NULL)
        dns_rdatatypestats_increment (res->view->resquerystats, fctx->type);
  ...
  cleanup_query:
    ...
    {
        query->magic = 0;
        isc_mem_put (res->buckets[fctx->bucketnum].mctx, query, sizeof (*query));
    }
...
static isc_boolean_t bad_edns (fetchctx_t * fctx, isc_sockaddr_t * address)
{
    ...
    for (sa = ISC_LIST_HEAD (fctx->bad_edns); sa != NULL; sa = ISC_LIST_NEXT (sa, link))
    {
...
static void add_bad_edns (fetchctx_t * fctx, isc_sockaddr_t * address)
{
    ...
    sa = isc_mem_get (fctx->res->buckets[fctx->bucketnum].mctx, sizeof (*sa));
    if (sa == NULL)
        return;
...
static isc_boolean_t triededns (fetchctx_t * fctx, isc_sockaddr_t * address)
{
    isc_sockaddr_t *sa;

    for (sa = ISC_LIST_HEAD (fctx->edns); sa != NULL; sa = ISC_LIST_NEXT (sa, link))
    {
        if (isc_sockaddr_equal (sa, address))
            return (ISC_TRUE);
    }
...
static isc_boolean_t triededns512 (fetchctx_t * fctx, isc_sockaddr_t * address)
{
    isc_sockaddr_t *sa;

    for (sa = ISC_LIST_HEAD (fctx->edns512); sa != NULL; sa = ISC_LIST_NEXT (sa, link))
    {
        if (isc_sockaddr_equal (sa, address))
            return (ISC_TRUE);
    }
...
static isc_result_t resquery_send (resquery_t * query)
{
    ...
    if ((query->options & DNS_FETCHOPT_TCP) != 0)
    {
        ...
        isc_buffer_init (&query->buffer, query->data + 2, sizeof (query->data) - 2);
        buffer = &tcpbuffer;
    }
    else
    {
    ...
    result = dns_dispatch_addresponse2 (query->dispatch,
                                        ...
                                        task, resquery_response, query, &query->id, &query->dispentry, res->socketmgr);
    if (result != ISC_R_SUCCESS)
        goto cleanup_temps;
     ...
     */
    if ((query->options & DNS_FETCHOPT_RECURSIVE) != 0 || ISFORWARDER (query->addrinfo))
    ...
    {
        result = dns_view_issecuredomain (res->view, &fctx->name, &secure_domain);
        if (result != ISC_R_SUCCESS)
    ...
    result = dns_message_rendersection (fctx->qmessage, DNS_SECTION_QUESTION, 0);
    if (result != ISC_R_SUCCESS)
        goto cleanup_message;
    ...
    if ((query->addrinfo->flags & DNS_FETCHOPT_NOEDNS0) == 0 &&
        peer != NULL && dns_peer_getsupportedns (peer, &useedns) == ISC_R_SUCCESS && !useedns)
    {
    ...
    if (fctx->timeout)
    {
        if ((triededns512 (fctx, &query->addrinfo->sockaddr) ||
             fctx->timeouts >= (MAX_EDNS0_TIMEOUTS * 2)) && (query->options & DNS_FETCHOPT_NOEDNS0) == 0)
        {
            query->options |= DNS_FETCHOPT_NOEDNS0;
            fctx->reason = "disabling EDNS";
        }
        else if ((triededns (fctx, &query->addrinfo->sockaddr) ||
                  fctx->timeouts >= MAX_EDNS0_TIMEOUTS) && (query->options & DNS_FETCHOPT_NOEDNS0) == 0)
        {
            query->options |= DNS_FETCHOPT_EDNS512;
            fctx->reason = "reducing the advertised EDNS UDP " "packet size to 512 octets";
        }
    ...
    if ((query->options & DNS_FETCHOPT_NOEDNS0) == 0)
    {
        if ((query->addrinfo->flags & DNS_FETCHOPT_NOEDNS0) == 0)
        {
            ...
            result = fctx_addopt (fctx->qmessage, version, udpsize, reqnsid);
    ...
    result = dns_message_rendersection (fctx->qmessage, DNS_SECTION_ADDITIONAL, 0);
    if (result != ISC_R_SUCCESS)
        goto cleanup_message;
    ...
    {
        dns_tsigkey_attach (dns_message_gettsigkey (fctx->qmessage), &query->tsigkey);
        result = dns_message_getquerytsig (fctx->qmessage, fctx->res->mctx, &query->tsig);
        if (result != ISC_R_SUCCESS)
            goto cleanup_message;
    }
    ...
    if ((query->options & DNS_FETCHOPT_TCP) == 0)
    {
        ...
        if (query->exclusivesocket)
        {
            result = isc_socket_connect (socket, address, task, resquery_udpconnected, query);
            if (result != ISC_R_SUCCESS)
                goto cleanup_message;
            connecting = ISC_TRUE;
            query->connects++;
     ...
     */
    result = isc_socket_sendto (socket, &r, task, resquery_senddone, query, address, NULL);
    if (result != ISC_R_SUCCESS)
    {
        if (connecting)
        {
    ...
    else
    {
        switch (sevent->result)
        {
            case ISC_R_SUCCESS:
                ...
                if (isc_sockaddr_pf (&query->addrinfo->sockaddr) == AF_INET)
                    attrs |= DNS_DISPATCHATTR_IPV4;
                else
                    attrs |= DNS_DISPATCHATTR_IPV6;
                ...
                result = dns_dispatch_createtcp (query->dispatchmgr,
                                                 query->tcpsocket,
                                                 query->fctx->res->taskmgr, 4096, 2, 1, 1, 3, attrs, &query->dispatch);
...
static void fctx_finddone (isc_task_t * task, isc_event_t * event)
{
    ...
    else if (SHUTTINGDOWN (fctx) && fctx->pending == 0 && fctx->nqueries == 0 && ISC_LIST_EMPTY (fctx->validators))
    {
        /*
...
static inline isc_boolean_t bad_server (fetchctx_t * fctx, isc_sockaddr_t * address)
{
    isc_sockaddr_t *sa;

    for (sa = ISC_LIST_HEAD (fctx->bad); sa != NULL; sa = ISC_LIST_NEXT (sa, link))
    {
        if (isc_sockaddr_equal (sa, address))
            return (ISC_TRUE);
    }
...
static inline isc_boolean_t mark_bad (fetchctx_t * fctx)
{
    dns_adbfind_t *curr;
     ...
     */
    for (curr = ISC_LIST_HEAD (fctx->finds); curr != NULL; curr = ISC_LIST_NEXT (curr, publink))
    {
        for (addrinfo = ISC_LIST_HEAD (curr->list); addrinfo != NULL; addrinfo = ISC_LIST_NEXT (addrinfo, publink))
        {
     ...
     */
    for (curr = ISC_LIST_HEAD (fctx->altfinds); curr != NULL; curr = ISC_LIST_NEXT (curr, publink))
    {
        for (addrinfo = ISC_LIST_HEAD (curr->list); addrinfo != NULL; addrinfo = ISC_LIST_NEXT (addrinfo, publink))
        {
            if (bad_server (fctx, &addrinfo->sockaddr))
...
static void add_bad (fetchctx_t * fctx, dns_adbaddrinfo_t * addrinfo, isc_result_t reason, badnstype_t badtype)
{
    ...
    *sa = *address;
    ...
    if (reason == DNS_R_UNEXPECTEDRCODE && fctx->rmessage->rcode == dns_rcode_servfail && ISFORWARDER (addrinfo))
        return;
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_LAME_SERVERS,
                   ...
                   dns_result_totext (reason), spc, code, namebuf, typebuf, classbuf, addrbuf);
}
...
static void sort_finds (dns_adbfindlist_t * findlist)
{
    ...
    for (curr = ISC_LIST_HEAD (*findlist); curr != NULL; curr = ISC_LIST_NEXT (curr, publink))
        sort_adbfind (curr);
...
findname (fetchctx_t * fctx, dns_name_t * name, in_port_t port,
          unsigned int options, unsigned int flags, isc_stdtime_t now, isc_boolean_t * need_alternate)
{
    ...
    result = dns_adb_createfind (fctx->adb,
                                 ...
                                 &fctx->name, fctx->type, options, now, NULL, res->view->dstport, &find);
    if (result != ISC_R_SUCCESS)
    {
    ...
    else if (!ISC_LIST_EMPTY (find->list))
    {
        ...
        if (flags != 0 || port != 0)
        {
            for (ai = ISC_LIST_HEAD (find->list); ai != NULL; ai = ISC_LIST_NEXT (ai, publink))
            {
                ai->flags |= flags;
                if (port != 0)
    ...
    else
    {
        ...
        if ((find->options & DNS_ADBFIND_WANTEVENT) != 0)
        {
            ...
            if (need_alternate != NULL &&
                ...
                ((res->dispatchv4 == NULL &&
                  find->result_v6 != DNS_R_NXDOMAIN) || (res->dispatchv6 == NULL && find->result_v4 != DNS_R_NXDOMAIN)))
        ...
        else
        {
            ...
            if (need_alternate != NULL && !*need_alternate &&
                ((res->dispatchv4 == NULL &&
                  find->result_v6 == DNS_R_NXRRSET) || (res->dispatchv6 == NULL && find->result_v4 == DNS_R_NXRRSET)))
                *need_alternate = ISC_TRUE;
            dns_adb_destroyfind (&find);
        }
    }
...
static isc_result_t fctx_getaddresses (fetchctx_t * fctx, isc_boolean_t badcache)
{
    ...
    if (sa == NULL)
    {
         ...
         */
        if (dns_rdatatype_atparent (fctx->type) && dns_name_countlabels (name) > 1)
        {
        ...
        result = dns_fwdtable_find2 (fctx->res->view->fwdtable, name, domain, &forwarders);
        if (result == ISC_R_SUCCESS)
        {
            sa = ISC_LIST_HEAD (forwarders->addrs);
            fctx->fwdpolicy = forwarders->fwdpolicy;
            if (fctx->fwdpolicy == dns_fwdpolicy_only && isstrictsubdomain (domain, &fctx->domain))
            {
                isc_mem_t *mctx;
    ...
    while (sa != NULL)
    {
        if ((isc_sockaddr_pf (sa) == AF_INET &&
             fctx->res->dispatchv4 == NULL) || (isc_sockaddr_pf (sa) == AF_INET6 && fctx->res->dispatchv6 == NULL))
        {
            sa = ISC_LIST_NEXT (sa, link);
            continue;
        }
        ai = NULL;
        result = dns_adb_findaddrinfo (fctx->adb, sa, &ai, 0);    /* XXXMLG */
        if (result == ISC_R_SUCCESS)
        {
            ...
            if (cur != NULL)
                ISC_LIST_INSERTBEFORE (fctx->forwaddrs, cur, ai, publink);
            else
    ...
    for (result = dns_rdataset_first (&fctx->nameservers);
         result == ISC_R_SUCCESS; result = dns_rdataset_next (&fctx->nameservers))
    {
        ...
        findname (fctx, &ns.name, 0, stdoptions, 0, now, &need_alternate);
        dns_rdata_reset (&rdata);
    ...
    if (need_alternate)
    {
        ...
        for (a = ISC_LIST_HEAD (fctx->res->alternates); a != NULL; a = ISC_LIST_NEXT (a, link))
        {
            if (!a->isaddress)
            {
                findname (fctx, &a->_u._n.name, a->_u._n.port, stdoptions, FCTX_ADDRINFO_FORWARDER, now, NULL);
                continue;
            }
        ...
        else
        {
            ...
            if (badcache &&
                (fctx->type == dns_rdatatype_dnskey ||
                 fctx->type == dns_rdatatype_dlv || fctx->type == dns_rdatatype_ds) && result == ISC_R_SUCCESS)
                dns_resolver_addbadcache (fctx->res, &fctx->name, fctx->type, &expire);
...
static inline void possibly_mark (fetchctx_t * fctx, dns_adbaddrinfo_t * addr)
{
    isc_netaddr_t na;
    ...
    if (blackhole != NULL)
    {
        ...
        if (dns_acl_match (&ipaddr, NULL, blackhole, &res->view->aclenv, &match, NULL) == ISC_R_SUCCESS && match > 0)
            aborted = ISC_TRUE;
    }

    if (peer != NULL && dns_peer_getbogus (peer, &bogus) == ISC_R_SUCCESS && bogus)
        aborted = ISC_TRUE;
...
static inline dns_adbaddrinfo_t *fctx_nextaddress (fetchctx_t * fctx)
{
    ...
    for (addrinfo = ISC_LIST_HEAD (fctx->altaddrs); addrinfo != NULL; addrinfo = ISC_LIST_NEXT (addrinfo, publink))
    {
        if (!UNMARKED (addrinfo))
            continue;
        possibly_mark (fctx, addrinfo);
        if (UNMARKED (addrinfo) && (faddrinfo == NULL || addrinfo->srtt < faddrinfo->srtt))
        {
            if (faddrinfo != NULL)
...
static isc_boolean_t fctx_destroy (fetchctx_t * fctx)
{
    ...
    for (sa = ISC_LIST_HEAD (fctx->edns); sa != NULL; sa = next_sa)
    {
        next_sa = ISC_LIST_NEXT (sa, link);
    ...
    for (sa = ISC_LIST_HEAD (fctx->edns512); sa != NULL; sa = next_sa)
    {
        next_sa = ISC_LIST_NEXT (sa, link);
    ...
    if (res->buckets[bucketnum].exiting && ISC_LIST_EMPTY (res->buckets[bucketnum].fctxs))
        return (ISC_TRUE);
...
static void fctx_timeout (isc_task_t * task, isc_event_t * event)
{
    ...
    else
    {
        ...
        if (query != NULL && isc_time_compare (&tevent->due, &query->start) >= 0)
        {
            fctx_cancelquery (&query, NULL, NULL, ISC_TRUE);
        }
        fctx->attributes &= ~FCTX_ATTR_ADDRWAIT;
...
static void fctx_shutdown (fetchctx_t * fctx)
{
    ...
    if (fctx->state != fetchstate_init)
    {
        ...
        isc_task_send (fctx->res->buckets[fctx->bucketnum].task, &cevent);
    }
}
...
static void fctx_doshutdown (isc_task_t * task, isc_event_t * event)
{
    ...
    INSIST (fctx->state == fetchstate_active || fctx->state == fetchstate_done);
    ...
    if (fctx->references == 0 && fctx->pending == 0 && fctx->nqueries == 0 && ISC_LIST_EMPTY (fctx->validators))
        bucket_empty = fctx_destroy (fctx);
...
static void fctx_start (isc_task_t * task, isc_event_t * event)
{
    ...
    else
    {
        ...
        ISC_EVENT_INIT (event, sizeof (*event), 0, NULL,
                        DNS_EVENT_FETCHCONTROL, fctx_doshutdown, fctx, NULL, NULL, NULL);
    }
...
fctx_join (fetchctx_t * fctx, isc_task_t * task, isc_sockaddr_t * client,
           ...
           dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset, dns_fetch_t * fetch)
{
    isc_task_t *clone;
    ...
    event = (dns_fetchevent_t *)
        isc_event_allocate (fctx->res->mctx, clone, DNS_EVENT_FETCHDONE, action, arg, sizeof (*event));
    if (event == NULL)
    {
...
static inline void log_ns_ttl (fetchctx_t * fctx, const char *where)
{
    char namebuf[DNS_NAME_FORMATSIZE];
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_RESOLVER,
                   ...
                   fctx, where, namebuf, domainbuf, fctx->ns_ttl_ok, fctx->ns_ttl);
}
             ...
             unsigned int options, unsigned int bucketnum, fetchctx_t ** fctxp)
{
    ...
    if (domain == NULL)
    {
        ...
        dns_fixedname_init (&fixed);
        domain = dns_fixedname_name (&fixed);
        result = dns_fwdtable_find2 (fctx->res->view->fwdtable, fwdname, domain, &forwarders);
        if (result == ISC_R_SUCCESS)
            fctx->fwdpolicy = forwarders->fwdpolicy;
        ...
        if (fctx->fwdpolicy != dns_fwdpolicy_only)
        {
            ...
            result = dns_view_findzonecut (res->view, name, domain, 0, findoptions, ISC_TRUE, &fctx->nameservers, NULL);
            if (result != ISC_R_SUCCESS)
                goto cleanup_name;
            result = dns_name_dup (domain, res->buckets[bucketnum].mctx, &fctx->domain);
            if (result != ISC_R_SUCCESS)
            {
                dns_rdataset_disassociate (&fctx->nameservers);
                goto cleanup_name;
    ...
    fctx->qmessage = NULL;
    result = dns_message_create (res->buckets[bucketnum].mctx, DNS_MESSAGE_INTENTRENDER, &fctx->qmessage);

    if (result != ISC_R_SUCCESS)
        goto cleanup_domain;

    fctx->rmessage = NULL;
    result = dns_message_create (res->buckets[bucketnum].mctx, DNS_MESSAGE_INTENTPARSE, &fctx->rmessage);
    ...
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_time_nowplusinterval: %s", isc_result_totext (iresult));
    ...
    iresult = isc_timer_create (res->timermgr, isc_timertype_inactive,
                                NULL, NULL, res->buckets[bucketnum].task, fctx_timeout, fctx, &fctx->timer);
    if (iresult != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_timer_create: %s", isc_result_totext (iresult));
        result = ISC_R_UNEXPECTED;
        goto cleanup_rmessage;
    }
 ...
 * Handle Responses
 */
static inline isc_boolean_t is_lame (fetchctx_t * fctx)
{
    ...
    if (message->rcode != dns_rcode_noerror && message->rcode != dns_rcode_nxdomain)
        return (ISC_FALSE);
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
        {
            ...
            if (rdataset->type != dns_rdatatype_ns)
                continue;
            namereln = dns_name_fullcompare (name, &fctx->domain, &order, &labels);
            if (namereln == dns_namereln_equal && (message->flags & DNS_MESSAGEFLAG_AA) != 0)
                return (ISC_FALSE);
            if (namereln == dns_namereln_subdomain)
                return (ISC_FALSE);
            return (ISC_TRUE);
...
static inline void log_lame (fetchctx_t * fctx, dns_adbaddrinfo_t * addrinfo)
{
    char namebuf[DNS_NAME_FORMATSIZE];
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_LAME_SERVERS,
                   DNS_LOGMODULE_RESOLVER, ISC_LOG_INFO,
                   "lame server resolving '%s' (in '%s'?): %s", namebuf, domainbuf, addrbuf);
}

static inline void log_formerr (fetchctx_t * fctx, const char *format, ...)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_RESOLVER,
                   ...
                   "DNS format error from %s resolving %s%s%s: %s", nsbuf, fctx->info, clmsg, clbuf, msgbuf);
}

static inline isc_result_t same_question (fetchctx_t * fctx)
{
    isc_result_t result;
    ...
    if (fctx->type != rdataset->type || fctx->res->rdclass != rdataset->rdclass || !dns_name_equal (&fctx->name, name))
    {
        char namebuf[DNS_NAME_FORMATSIZE];
        ...
        log_formerr (fctx, "question section mismatch: got %s/%s/%s", namebuf, class, type);
        return (DNS_R_FORMERR);
    }
...
static void clone_results (fetchctx_t * fctx)
{
    ...
    for (event = ISC_LIST_NEXT (hevent, ev_link); event != NULL; event = ISC_LIST_NEXT (event, ev_link))
    {
        ...
        INSIST (!(hevent->sigrdataset == NULL && event->sigrdataset != NULL));
        if (hevent->sigrdataset != NULL &&
            dns_rdataset_isassociated (hevent->sigrdataset) && event->sigrdataset != NULL)
            dns_rdataset_clone (hevent->sigrdataset, event->sigrdataset);
    }
}
...
static isc_boolean_t maybe_destroy (fetchctx_t * fctx, isc_boolean_t locked)
{
    ...
    for (validator = ISC_LIST_HEAD (fctx->validators); validator != NULL; validator = next_validator)
    {
...
static void validated (isc_task_t * task, isc_event_t * event)
{
    ...
    isc_mem_put (res->buckets[fctx->bucketnum].mctx, valarg, sizeof (*valarg));
     ...
     */
    if (vevent->result == ISC_R_SUCCESS && !negative && vevent->rdataset != NULL && CHAINING (vevent->rdataset))
    {
    ...
    if (hevent != NULL)
    {
        if (!negative && !chaining &&
            (fctx->type == dns_rdatatype_any || fctx->type == dns_rdatatype_rrsig || fctx->type == dns_rdatatype_sig))
        {
            /*
    ...
    if (vevent->result != ISC_R_SUCCESS)
    {
        ...
        if (fctx->vresult != DNS_R_BROKENCHAIN)
        {
            ...
            if (result == ISC_R_SUCCESS)
                (void) dns_db_deleterdataset (fctx->cache, node, NULL, vevent->type, 0);
            if (result == ISC_R_SUCCESS && vevent->sigrdataset != NULL)
                (void) dns_db_deleterdataset (fctx->cache, node, NULL, dns_rdatatype_rrsig, vevent->type);
            if (result == ISC_R_SUCCESS)
        ...
        if (fctx->vresult == DNS_R_BROKENCHAIN && !negative)
        {
            ...
            if (result == ISC_R_SUCCESS)
            {
                (void) dns_db_addrdataset (fctx->cache, node, NULL, now, vevent->rdataset, 0, NULL);
            }
            if (result == ISC_R_SUCCESS && vevent->sigrdataset != NULL)
                (void) dns_db_addrdataset (fctx->cache, node, NULL, now, vevent->sigrdataset, 0, NULL);
            if (result == ISC_R_SUCCESS)
                dns_db_detachnode (fctx->cache, &node);
        }
        result = fctx->vresult;
        ...
        else if (result == DNS_R_BROKENCHAIN)
        {
            ...
            if (negative &&
                (fctx->type == dns_rdatatype_dnskey ||
                 fctx->type == dns_rdatatype_dlv || fctx->type == dns_rdatatype_ds) && tresult == ISC_R_SUCCESS)
                dns_resolver_addbadcache (res, &fctx->name, fctx->type, &expire);
            fctx_done (fctx, result, __LINE__);    /* Locks bucket. */
        }
        else
    ...
    if (negative)
    {
        ...
        if (fctx->type == dns_rdatatype_soa && covers == dns_rdatatype_any && res->zero_no_soa_ttl)
            ttl = 0;

        result = ncache_adderesult (fctx->rmessage, fctx->cache, node,
                                    covers, now, ttl, vevent->optout, ardataset, &eresult);
        if (result != ISC_R_SUCCESS)
            goto noanswer_response;
    ...
    if (vevent->proofs[DNS_VALIDATOR_NOQNAMEPROOF] != NULL)
    {
        ...
        {
            result = dns_rdataset_addclosest (vevent->rdataset, vevent->proofs[DNS_VALIDATOR_CLOSESTENCLOSER]);
            RUNTIME_CHECK (result == ISC_R_SUCCESS);
        }
    }
    ...
    result = dns_db_addrdataset (fctx->cache, node, NULL, now, vevent->rdataset, 0, ardataset);
    ...
    {
        result = dns_db_addrdataset (fctx->cache, node, NULL, now, vevent->sigrdataset, 0, asigrdataset);
        if (result != ISC_R_SUCCESS && result != DNS_R_UNCHANGED)
            goto noanswer_response;
    }
  ...
  answer_response:
    ...
    {
        name = NULL;
        dns_message_currentname (fctx->rmessage, DNS_SECTION_AUTHORITY, &name);
        for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
        {
            if ((rdataset->type != dns_rdatatype_ns &&
                 rdataset->type != dns_rdatatype_nsec) || rdataset->trust != dns_trust_secure)
                continue;
            for (sigrdataset = ISC_LIST_HEAD (name->list);
                 sigrdataset != NULL; sigrdataset = ISC_LIST_NEXT (sigrdataset, link))
            {
                if (sigrdataset->type != dns_rdatatype_rrsig || sigrdataset->covers != rdataset->type)
                    continue;
                break;
            }
            if (sigrdataset == NULL || sigrdataset->trust != dns_trust_secure)
                continue;
            result = dns_db_findnode (fctx->cache, name, ISC_TRUE, &nsnode);
            if (result != ISC_R_SUCCESS)
                continue;

            result = dns_db_addrdataset (fctx->cache, nsnode, NULL, now, rdataset, 0, NULL);
            if (result == ISC_R_SUCCESS)
                result = dns_db_addrdataset (fctx->cache, nsnode, NULL, now, sigrdataset, 0, NULL);
            dns_db_detachnode (fctx->cache, &nsnode);
            if (result != ISC_R_SUCCESS)
                continue;
        }
        result = dns_message_nextname (fctx->rmessage, DNS_SECTION_AUTHORITY);
    }
    ...
    if (hevent != NULL)
    {
        ...
        RUNTIME_CHECK (dns_name_copy (vevent->name, dns_fixedname_name (&hevent->foundname), NULL) == ISC_R_SUCCESS);
...
static inline isc_result_t
cache_name (fetchctx_t * fctx, dns_name_t * name, dns_adbaddrinfo_t * addrinfo, isc_stdtime_t now)
{
    ...
    {
        result = dns_view_issecuredomain (res->view, name, &secure_domain);
        if (result != ISC_R_SUCCESS)
            return (result);
    ...
    asigrdataset = NULL;
    event = NULL;
    if ((name->attributes & DNS_NAMEATTR_ANSWER) != 0 && !need_validation)
    {
        ...
        if (event != NULL)
        {
            ...
            if ((fctx->type != dns_rdatatype_any &&
                 ...
                 fctx->type != dns_rdatatype_sig) || (name->attributes & DNS_NAMEATTR_CHAINING) != 0)
            {
    ...
    for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
    {
        ...
        if (CHECKNAMES (rdataset))
        {
            ...
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_RESOLVER,
                           DNS_LOGMODULE_RESOLVER, ISC_LOG_NOTICE,
                           "check-names %s %s/%s/%s", fail ? "failure" : "warning", namebuf, typebuf, classbuf);
            if (fail)
            {
                if (ANSWER (rdataset))
                {
         ...
         * them.)
         */
        if (secure_domain && rdataset->trust != dns_trust_glue && !EXTERNAL (rdataset))
        {
            dns_trust_t trust;
            ...
            if (!need_validation || !ANSWER (rdataset))
            {
                ...
                result = dns_db_addrdataset (fctx->cache, node, NULL, now, rdataset, 0, addedrdataset);
                if (result == DNS_R_UNCHANGED)
                {
                    result = ISC_R_SUCCESS;
                    if (!need_validation && ardataset != NULL && NEGATIVE (ardataset))
                    {
                        /*
                ...
                if (sigrdataset != NULL)
                {
                    ...
                    result = dns_db_addrdataset (fctx->cache, node, NULL, now, sigrdataset, 0, addedrdataset);
                    if (result == DNS_R_UNCHANGED)
                        result = ISC_R_SUCCESS;
                    if (result != ISC_R_SUCCESS)
                        break;
            ...
            if (ANSWER (rdataset) && need_validation)
            {
                    ...
                    fctx->type != dns_rdatatype_rrsig && fctx->type != dns_rdatatype_sig)
                {
                     ...
                     * contain useful keys.
                     */
                    INSIST (valrdataset == NULL && valsigrdataset == NULL);
                ...
                else
                {
                     ...
                     */
                    result = valcreate (fctx, addrinfo, name, rdataset->type, rdataset, sigrdataset, valoptions, task);
                    /*
        ...
        else if (!EXTERNAL (rdataset))
        {
            ...
            if (rdataset->trust == dns_trust_glue &&
                (rdataset->type == dns_rdatatype_ns ||
                 (rdataset->type == dns_rdatatype_rrsig && rdataset->covers == dns_rdatatype_ns)))
            {
                /*
             ...
             */
            result = dns_db_addrdataset (fctx->cache, node, NULL, now, rdataset, options, addedrdataset);
            if (result == DNS_R_UNCHANGED)
            {
                if (ANSWER (rdataset) && ardataset != NULL && NEGATIVE (ardataset))
                {
                    /*
    ...
    if (valrdataset != NULL)
        result = valcreate (fctx, addrinfo, name, fctx->type, valrdataset, valsigrdataset, valoptions, task);
    ...
    if (result == ISC_R_SUCCESS && have_answer)
    {
        ...
        if (event != NULL)
        {
            ...
            if (dns_rdataset_isassociated (event->rdataset) && NEGATIVE (event->rdataset))
            {
                INSIST (eresult == DNS_R_NCACHENXDOMAIN || eresult == DNS_R_NCACHENXRRSET);
            }
...
static inline isc_result_t cache_message (fetchctx_t * fctx, dns_adbaddrinfo_t * addrinfo, isc_stdtime_t now)
{
    isc_result_t result;
    ...
    for (section = DNS_SECTION_ANSWER; section <= DNS_SECTION_ADDITIONAL; section++)
    {
                   ...
                   isc_boolean_t optout, dns_rdataset_t * ardataset, isc_result_t * eresultp)
{
    ...
    result = dns_ncache_addoptout (message, cache, node, covers, now, maxttl, optout, ardataset);
...
static inline isc_result_t
ncache_message (fetchctx_t * fctx, dns_adbaddrinfo_t * addrinfo, dns_rdatatype_t covers, isc_stdtime_t now)
{
    isc_result_t result, eresult;
    ...
    if (secure_domain)
    {
        ...
        {
            tname = NULL;
            dns_message_currentname (fctx->rmessage, DNS_SECTION_AUTHORITY, &tname);
            for (trdataset = ISC_LIST_HEAD (tname->list);
                 trdataset != NULL; trdataset = ISC_LIST_NEXT (trdataset, link))
    ...
    if (need_validation)
    {
        ...
        result = valcreate (fctx, addrinfo, name, fctx->type,
                            NULL, NULL, valoptions, res->buckets[fctx->bucketnum].task);
        /*
    ...
    if (fctx->type == dns_rdatatype_soa && covers == dns_rdatatype_any && fctx->res->zero_no_soa_ttl)
        ttl = 0;

    result = ncache_adderesult (fctx->rmessage, fctx->cache, node, covers, now, ttl, ISC_FALSE, ardataset, &eresult);
    if (result != ISC_R_SUCCESS)
        goto unlock;
...
static inline void
mark_related (dns_name_t * name, dns_rdataset_t * rdataset, isc_boolean_t external, isc_boolean_t gluing)
{
...
static isc_result_t check_section (void *arg, dns_name_t * addname, dns_rdatatype_t type, dns_section_t section)
{
    ...
    name = NULL;
    rdataset = NULL;
    result = dns_message_findname (fctx->rmessage, section, addname, dns_rdatatype_any, 0, &name, NULL);
    if (result == ISC_R_SUCCESS)
    {
        ...
        if (type == dns_rdatatype_a)
        {
            for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
            {
                if (rdataset->type == dns_rdatatype_rrsig)
                    ...
                    rtype = rdataset->type;
                if (rtype == dns_rdatatype_a || rtype == dns_rdatatype_aaaa)
        ...
        else
        {
            ...
            if (result == ISC_R_SUCCESS)
            {
                 ...
                 */
                rdataset = NULL;
                result = dns_message_findtype (name, dns_rdatatype_rrsig, type, &rdataset);
                if (result == ISC_R_SUCCESS)
                    mark_related (name, rdataset, external, gluing);
            }
        }
...
#endif
#if CHECK_FOR_GLUE_IN_ANSWER
static isc_result_t check_answer (void *arg, dns_name_t * addname, dns_rdatatype_t type)
{
...
static void chase_additional (fetchctx_t * fctx)
{
  ...
  again:
    ...
    for (result = dns_message_firstname (fctx->rmessage, section);
         result == ISC_R_SUCCESS; result = dns_message_nextname (fctx->rmessage, section))
    {
        dns_name_t *name = NULL;

        dns_rdataset_t *rdataset;

        dns_message_currentname (fctx->rmessage, DNS_SECTION_ADDITIONAL, &name);
        ...
        for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
        {
            ...
            {
                rdataset->attributes &= ~DNS_RDATASETATTR_CHASE;
                (void) dns_rdataset_additionaldata (rdataset, check_related, fctx);
                rescan = ISC_TRUE;
            }
        }
    }
...
static inline isc_result_t cname_target (dns_rdataset_t * rdataset, dns_name_t * tname)
{
    isc_result_t result;
              ...
              dns_name_t * oname, dns_fixedname_t * fixeddname)
{
    ...
    if (namereln != dns_namereln_subdomain)
    {
        ...
        log_formerr (fctx, "unrelated DNAME in answer: " "%s is not in %s", qbuf, obuf);
        return (DNS_R_FORMERR);
    }
    dns_fixedname_init (&prefix);
    dns_name_split (qname, nlabels, dns_fixedname_name (&prefix), NULL);
    dns_fixedname_init (fixeddname);
    result = dns_name_concatenate (dns_fixedname_name (&prefix), &dname.dname, dns_fixedname_name (fixeddname), NULL);
    dns_rdata_freestruct (&dname);
    return (result);
}
...
static isc_boolean_t is_answeraddress_allowed (dns_view_t * view, dns_name_t * name, dns_rdataset_t * rdataset)
{
    ...
    if (view->answeracl_exclude != NULL)
    {
        ...
        result = dns_rbt_findnode (view->answeracl_exclude, name, NULL, &node, NULL, 0, NULL, NULL);
    ...
    for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
    {
        ...
        result = dns_acl_match (&netaddr, NULL, view->denyansweracl, &view->aclenv, &match, NULL);
        ...
        if (result == ISC_R_SUCCESS && match > 0)
        {
            ...
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_RESOLVER,
                           DNS_LOGMODULE_RESOLVER, ISC_LOG_NOTICE,
                           "answer address %s denied for %s/%s/%s", addrbuf, namebuf, typebuf, classbuf);
            return (ISC_FALSE);
        }
    }
...
is_answertarget_allowed (dns_view_t * view, dns_name_t * name,
                         dns_rdatatype_t type, dns_name_t * tname, dns_name_t * domain)
{
    isc_result_t result;
    ...
    {
        result = dns_rbt_findnode (view->answernames_exclude, name, NULL, &node, NULL, 0, NULL, NULL);
        if (result == ISC_R_SUCCESS || result == DNS_R_PARTIALMATCH)
            return (ISC_TRUE);
    }
     ...
     */
    result = dns_rbt_findnode (view->denyanswernames, tname, NULL, &node, NULL, 0, NULL, NULL);
    if (result == ISC_R_SUCCESS || result == DNS_R_PARTIALMATCH)
    {
        dns_name_format (name, qnamebuf, sizeof (qnamebuf));
        dns_name_format (tname, tnamebuf, sizeof (tnamebuf));
        dns_rdatatype_format (type, typebuf, sizeof (typebuf));
        dns_rdataclass_format (view->rdclass, classbuf, sizeof (classbuf));
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_RESOLVER,
                       DNS_LOGMODULE_RESOLVER, ISC_LOG_NOTICE,
                       "%s target %s denied for %s/%s", typebuf, tnamebuf, qnamebuf, classbuf);
        return (ISC_FALSE);
    }
...
static void trim_ns_ttl (fetchctx_t * fctx, dns_name_t * name, dns_rdataset_t * rdataset)
{
    ...
    if (fctx->ns_ttl_ok && rdataset->ttl > fctx->ns_ttl)
    {
        ...
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_RESOLVER,
                       ...
                       "%u -> %u", fctx, ns_namebuf, namebuf, tbuf, rdataset->ttl, fctx->ns_ttl);
...
static isc_result_t noanswer_response (fetchctx_t * fctx, dns_name_t * oqname, unsigned int look_in_options)
{
    ...
    if (message->rcode == dns_rcode_nxdomain ||
        (message->counts[DNS_SECTION_ANSWER] == 0 && message->counts[DNS_SECTION_AUTHORITY] == 0))
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        if (dns_name_issubdomain (name, &fctx->domain))
        {
            ...
            for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
            {
                ...
                if (((type == dns_rdatatype_ns || type == dns_rdatatype_soa) && !dns_name_issubdomain (qname, name)))
                {
                    char qbuf[DNS_NAME_FORMATSIZE];
                    ...
                    dns_name_format (qname, qbuf, sizeof (qbuf));
                    log_formerr (fctx, "unrelated %s %s in " "%s authority section", tbuf, qbuf, nbuf);
                    return (DNS_R_FORMERR);
                }
                if (type == dns_rdatatype_ns)
                {
                    ...
                    if (rdataset->type == dns_rdatatype_ns)
                    {
                        ...
                        {
                            log_formerr (fctx, "multiple NS " "RRsets in " "authority " "section");
                            return (DNS_R_FORMERR);
                        }
                        ns_name = name;
                ...
                if (type == dns_rdatatype_soa)
                {
                    ...
                    {
                        if (soa_name != NULL && name != soa_name)
                        {
                            log_formerr (fctx, "multiple SOA " "RRs in " "authority " "section");
                            return (DNS_R_FORMERR);
                        }
                        soa_name = name;
                    }
    ...
    if (ns_rdataset != NULL && dns_name_equal (&fctx->domain, ns_name) && !dns_name_equal (ns_name, dns_rootname))
        ...
        if (dns_name_issubdomain (name, &fctx->domain))
        {
            for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
            {
                ...
                else if (type == dns_rdatatype_ds)
                {
                    ...
                    if (ns_name == NULL)
                    {
                        log_formerr (fctx, "DS with no " "referral");
                        return (DNS_R_FORMERR);
                    }
                    if (rdataset->type == dns_rdatatype_ds)
                    {
                        if (ds_name != NULL && name != ds_name)
                        {
                            log_formerr (fctx, "DS doesn't " "match " "referral " "(NS)");
                            return (DNS_R_FORMERR);
                        }
                        ds_name = name;
                    }
    ...
    if (negative_response && message->rcode == dns_rcode_noerror &&
        ...
        dns_name_equal (soa_name, qname) && !dns_name_equal (qname, dns_rootname))
    ...
    if (!negative_response && ns_name != NULL && oqname == NULL)
    {
        ...
        (void) dns_rdataset_additionaldata (ns_rdataset, check_related, fctx);
#if CHECK_FOR_GLUE_IN_ANSWER
        /*
        ...
        if ((look_in_options & LOOK_FOR_GLUE_IN_ANSWER) != 0 &&
            ...
            (void) dns_rdataset_additionaldata (ns_rdataset, check_answer, fctx);
#endif
        fctx->attributes &= ~FCTX_ATTR_GLUING;
        /*
        ...
        dns_name_free (&fctx->domain, fctx->res->buckets[fctx->bucketnum].mctx);
        if (dns_rdataset_isassociated (&fctx->nameservers))
            dns_rdataset_disassociate (&fctx->nameservers);
        dns_name_init (&fctx->domain, NULL);
        result = dns_name_dup (ns_name, fctx->res->buckets[fctx->bucketnum].mctx, &fctx->domain);
        if (result != ISC_R_SUCCESS)
            return (result);
        fctx->attributes |= FCTX_ATTR_WANTCACHE;
        fctx->ns_ttl_ok = ISC_FALSE;
...
static isc_result_t answer_response (fetchctx_t * fctx)
{
    ...
    while (!done && result == ISC_R_SUCCESS)
    {
        ...
        if (dns_name_equal (name, qname))
        {
            ...
            for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
            {
                 ...
                 */
                if ((rdataset->type == dns_rdatatype_a ||
                     rdataset->type == dns_rdatatype_aaaa) && !is_answeraddress_allowed (view, name, rdataset))
                {
                    return (DNS_R_SERVFAIL);
                }
                ...
                else if (rdataset->type == dns_rdatatype_rrsig && rdataset->covers == type && !found_cname)
                {
                    /*
                ...
                else if (rdataset->type == dns_rdatatype_cname && !found_type)
                {
                    /*
                     ...
                     */
                    if (type == dns_rdatatype_rrsig ||
                        type == dns_rdatatype_dnskey || type == dns_rdatatype_nsec || type == dns_rdatatype_nsec3)
                    {
                        char buf[DNS_RDATATYPE_FORMATSIZE];

                        dns_rdatatype_format (fctx->type, buf, sizeof (buf));
                    ...
                    aflag = DNS_RDATASETATTR_ANSWER;
                    result = cname_target (rdataset, &tname);
                    if (result != ISC_R_SUCCESS)
                        return (result);
                    /* Apply filters on the target name. */
                    if (!is_answertarget_allowed (view, name, rdataset->type, &tname, &fctx->domain))
                    {
                        return (DNS_R_SERVFAIL);
                    }
                }
                else if (rdataset->type == dns_rdatatype_rrsig
                         && rdataset->covers == dns_rdatatype_cname && !found_type)
                {
                    /*
                     * We're looking for something else,
                ...
                if (found)
                {
                    ...
                    if (!chaining)
                    {
                        ...
                        if (aflag == DNS_RDATASETATTR_ANSWER)
                            have_answer = ISC_TRUE;
                        name->attributes |= DNS_NAMEATTR_ANSWER;
                    ...
                    if (want_chaining)
                    {
                        ...
                        rdataset->attributes |= DNS_RDATASETATTR_CHAINING;
                        qname = &tname;
                    }
                }
                /*
        ...
        else
        {
            ...
            for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
            {
                ...
                if (rdataset->type == dns_rdatatype_dname)
                {
                    ...
                    POST (want_chaining);
                    aflag = DNS_RDATASETATTR_ANSWER;
                    result = dname_target (fctx, rdataset, qname, name, &dname);
                    ...
                    if (!is_answertarget_allowed (view, qname, rdataset->type, dname_name, &fctx->domain))
                    {
                        return (DNS_R_SERVFAIL);
                    }
                }
                else if (rdataset->type == dns_rdatatype_rrsig && rdataset->covers == dns_rdatatype_dname)
                {
                    /*
                     * We've found a signature that
                     * covers the DNAME.
                ...
                if (found)
                {
                    ...
                    if (found_dname)
                    {
                        ...
                        result = dns_name_copy (dns_fixedname_name (&dname), dns_fixedname_name (&fqname), NULL);
                        if (result != ISC_R_SUCCESS)
                            return (result);
                        wanted_chaining = ISC_TRUE;
                        name->attributes |= DNS_NAMEATTR_CHAINING;
    ...
    {
        log_formerr (fctx, "CNAME/DNAME chain complete, but RCODE " "indicates error");
        return (DNS_R_FORMERR);
    }
...
static isc_boolean_t fctx_decreference (fetchctx_t * fctx)
{
    ...
    if (fctx->references == 0)
    {
         ...
         */
        if (fctx->pending == 0 && fctx->nqueries == 0 && ISC_LIST_EMPTY (fctx->validators) && SHUTTINGDOWN (fctx))
        {
            /*
...
static void resume_dslookup (isc_task_t * task, isc_event_t * event)
{
    ...
    else if (fevent->result == ISC_R_SUCCESS)
    {
        ...
        dns_name_free (&fctx->domain, fctx->res->buckets[bucketnum].mctx);
        dns_name_init (&fctx->domain, NULL);
        result = dns_name_dup (&fctx->nsname, fctx->res->buckets[bucketnum].mctx, &fctx->domain);
        if (result != ISC_R_SUCCESS)
        {
            fctx_done (fctx, DNS_R_SERVFAIL, __LINE__);
            goto cleanup;
    ...
    else
    {
        ...
        if (dns_rdataset_isassociated (&fctx->nsfetch->private->nameservers))
        {
            dns_rdataset_clone (&fctx->nsfetch->private->nameservers, &nameservers);
...
static inline void checknamessection (dns_message_t * message, dns_section_t section)
{
    isc_result_t result;
        ...
        for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
        {
            for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
            {
                ...
                if (!dns_rdata_checkowner (name, rdata.rdclass,
                                           rdata.type, ISC_FALSE) || !dns_rdata_checknames (&rdata, name, NULL))
                {
                    rdataset->attributes |= DNS_RDATASETATTR_CHECKNAMES;
                }
                dns_rdata_reset (&rdata);
            }
        }
 ...
 */
static isc_result_t log_nsid (dns_rdataset_t * opt, resquery_t * query, int level, isc_mem_t * mctx)
{
    ...
    *p = '\0';
    ...
    isc_sockaddr_format (&query->addrinfo->sockaddr, addrbuf, sizeof (addrbuf));
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_RESOLVER,
                   DNS_LOGMODULE_RESOLVER, level, "received NSID '%s' from %s", buf, addrbuf);
...
static void log_packet (dns_message_t * message, int level, isc_mem_t * mctx)
{
    isc_buffer_t buffer;
    ...
    do
    {
        ...
        result = dns_message_totext (message, &dns_master_style_debug, 0, &buffer);
        if (result == ISC_R_NOSPACE)
        {
            ...
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_RESOLVER,
                           ...
                           "received packet:\n%.*s", (int) isc_buffer_usedlength (&buffer), buf);
    }
...
static isc_boolean_t iscname (fetchctx_t * fctx)
{
    ...
    result = dns_message_findname (fctx->rmessage, DNS_SECTION_ANSWER, &fctx->name, dns_rdatatype_cname, 0, NULL, NULL);
...
static isc_boolean_t betterreferral (fetchctx_t * fctx)
{
    ...
    for (result = dns_message_firstname (message, DNS_SECTION_AUTHORITY);
         result == ISC_R_SUCCESS; result = dns_message_nextname (message, DNS_SECTION_AUTHORITY))
    {
        name = NULL;
        dns_message_currentname (message, DNS_SECTION_AUTHORITY, &name);
...
static void resquery_response (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        if (devent->result == ISC_R_EOF && (query->options & DNS_FETCHOPT_NOEDNS0) == 0)
        {
            /*
        ...
        else
        {
            ...
            if (query->exclusivesocket &&
                (devent->result == ISC_R_HOSTUNREACH ||
                 ...
                 devent->result == ISC_R_CONNREFUSED || devent->result == ISC_R_CANCELED))
            {
        ...
        switch (result)
        {
            case ISC_R_UNEXPECTEDEND:
                if (!message->question_ok ||
                    (message->flags & DNS_MESSAGEFLAG_TC) == 0 || (options & DNS_FETCHOPT_TCP) != 0)
                {
                    /*
    ...
    if (opt == NULL &&
        (message->rcode == dns_rcode_noerror ||
         ...
         message->rcode == dns_rcode_yxdomain) && bad_edns (fctx, &query->addrinfo->sockaddr))
    {
        char addrbuf[ISC_SOCKADDR_FORMATSIZE];
    ...
    if (message->rcode != dns_rcode_noerror && message->rcode != dns_rcode_nxdomain)
    {
        if (((message->rcode == dns_rcode_formerr ||
             ...
             (message->rcode == dns_rcode_servfail &&
              dns_message_getopt (message) == NULL)) && (query->options & DNS_FETCHOPT_NOEDNS0) == 0)
        {
            /*
        ...
        else if (message->rcode == dns_rcode_badvers)
        {
            ...
            flags = (version << DNS_FETCHOPT_EDNSVERSIONSHIFT) | DNS_FETCHOPT_EDNSVERSIONSET;
            mask = DNS_FETCHOPT_EDNSVERSIONMASK | DNS_FETCHOPT_EDNSVERSIONSET;
            switch (version)
            {
                case 0:
     ...
     * Is the server lame?
     */
    if (fctx->res->lame_ttl != 0 && !ISFORWARDER (query->addrinfo) && is_lame (fctx))
    {
        inc_stats (fctx->res, dns_resstatscounter_lame);
        log_lame (fctx, query->addrinfo);
        result = dns_adb_marklame (fctx->adb, query->addrinfo, &fctx->name, fctx->type, now + fctx->res->lame_ttl);
        if (result != ISC_R_SUCCESS)
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_RESOLVER,
                           DNS_LOGMODULE_RESOLVER, ISC_LOG_ERROR,
                           "could not mark server as lame: %s", isc_result_totext (result));
    ...
    if (!ISFORWARDER (query->addrinfo) &&
        ...
        !dns_name_equal (&fctx->domain, &fctx->name) && fix_mustbedelegationornxdomain (message, fctx))
    {
        char namebuf[DNS_NAME_FORMATSIZE];
        ...
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DELEGATION_ONLY,
                       ...
                       "from %s", domainbuf, namebuf, typebuf, classbuf, addrbuf);
    }
     ...
     */
    if (message->counts[DNS_SECTION_ANSWER] > 0 &&
        (message->rcode == dns_rcode_noerror || message->rcode == dns_rcode_nxdomain))
    {
        /*
         * [normal case]
         ...
         */
        if ((message->flags & DNS_MESSAGEFLAG_AA) != 0 || ISFORWARDER (query->addrinfo))
        ...
        else
        {
            if (fctx->type == dns_rdatatype_ns)
            {
                 ...
                 */
                result = noanswer_response (fctx, NULL, LOOK_FOR_NS_IN_ANSWER);
            }
            else
            {
                /*
                 ...
                 * such a corner case.
                 */
                result = noanswer_response (fctx, NULL, LOOK_FOR_GLUE_IN_ANSWER);
            }
    ...
    if (keep_trying)
    {
        ...
        if (get_nameservers)
        {
            ...
            result = dns_view_findzonecut (fctx->res->view,
                                           name, fname, now, findoptions, ISC_TRUE, &fctx->nameservers, NULL);
            if (result != ISC_R_SUCCESS)
            {
            ...
            dns_name_free (&fctx->domain, fctx->res->buckets[fctx->bucketnum].mctx);
            dns_name_init (&fctx->domain, NULL);
            result = dns_name_dup (fname, fctx->res->buckets[fctx->bucketnum].mctx, &fctx->domain);
            if (result != ISC_R_SUCCESS)
            {
                fctx_done (fctx, DNS_R_SERVFAIL, __LINE__);
                return;
 ...
 *** Resolver Methods
 ***/
static void destroy_badcache (dns_resolver_t * res)
{
    ...
    if (res->badcache != NULL)
    {
        for (i = 0; i < res->badhash; i++)
            for (bad = res->badcache[i]; bad != NULL; bad = next)
            {
                next = bad->next;
                isc_mem_put (res->mctx, bad, sizeof (*bad) + bad->name.length);
                res->badcount--;
            }
        isc_mem_put (res->mctx, res->badcache, sizeof (*res->badcache) * res->badhash);
        res->badcache = NULL;
        res->badhash = 0;
...
static void destroy (dns_resolver_t * res)
{
    ...
    isc_mem_put (res->mctx, res->buckets, res->nbuckets * sizeof (fctxbucket_t));
    if (res->dispatchv4 != NULL)
...
static void send_shutdown_events (dns_resolver_t * res)
{
    ...
    for (event = ISC_LIST_HEAD (res->whenshutdown); event != NULL; event = next_event)
    {
        next_event = ISC_LIST_NEXT (event, ev_link);
...
static void spillattimer_countdown (isc_task_t * task, isc_event_t * event)
{
    ...
    if (res->spillat <= res->spillatmin)
    {
        result = isc_timer_reset (res->spillattimer, isc_timertype_inactive, NULL, NULL, ISC_TRUE);
        RUNTIME_CHECK (result == ISC_R_SUCCESS);
    }
    count = res->spillat;
    UNLOCK (&res->lock);
    if (logit)
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_RESOLVER,
                       DNS_LOGMODULE_RESOLVER, ISC_LOG_NOTICE, "clients-per-query decreased to %u", count);
...
dns_resolver_create (dns_view_t * view,
                     ...
                     unsigned int options,
                     dns_dispatchmgr_t * dispatchmgr,
                     dns_dispatch_t * dispatchv4, dns_dispatch_t * dispatchv6, dns_resolver_t ** resp)
{
    dns_resolver_t *res;
    ...
    if (dispatchv4 != NULL)
    {
        ...
        res->exclusivev4 = ISC_TF ((dispattr & DNS_DISPATCHATTR_EXCLUSIVE) != 0);
    }
    ...
    if (dispatchv6 != NULL)
    {
        ...
        res->exclusivev6 = ISC_TF ((dispattr & DNS_DISPATCHATTR_EXCLUSIVE) != 0);
    }
    ...
    result = isc_timer_create (timermgr, isc_timertype_inactive, NULL, NULL,
                               task, spillattimer_countdown, res, &res->spillattimer);
  ...
  cleanup_buckets:
    ...
    isc_mem_put (view->mctx, res->buckets, res->nbuckets * sizeof (fctxbucket_t));
...
static void prime_done (isc_task_t * task, isc_event_t * event)
{
    ...
    if (fevent->result == ISC_R_SUCCESS && res->view->cache != NULL && res->view->hints != NULL)
    {
...
void dns_resolver_prime (dns_resolver_t * res)
{
    ...
    if (want_priming)
    {
        ...
        result = dns_resolver_createfetch (res, dns_rootname,
                                           dns_rdatatype_ns,
                                           NULL, NULL, NULL, 0,
                                           res->buckets[0].task, prime_done, res, rdataset, NULL, &res->primefetch);
        UNLOCK (&res->primelock);
        if (result != ISC_R_SUCCESS)
        {
            LOCK (&res->lock);
...
void dns_resolver_shutdown (dns_resolver_t * res)
{
    ...
    if (!res->exiting)
    {
        ...
        for (i = 0; i < res->nbuckets; i++)
        {
            ...
            for (fctx = ISC_LIST_HEAD (res->buckets[i].fctxs); fctx != NULL; fctx = ISC_LIST_NEXT (fctx, link))
                fctx_shutdown (fctx);
            if (res->dispatchv4 != NULL && !res->exclusivev4)
            {
                sock = dns_dispatch_getsocket (res->dispatchv4);
                isc_socket_cancel (sock, res->buckets[i].task, ISC_SOCKCANCEL_ALL);
            }
...
fctx_match (fetchctx_t * fctx, dns_name_t * name, dns_rdatatype_t type, unsigned int options)
{
    /*
     * Don't match fetch contexts that are shutting down.
     */
    if (fctx->cloned || fctx->state == fetchstate_done || ISC_LIST_EMPTY (fctx->events))
        return (ISC_FALSE);
...
static inline void log_fetch (dns_name_t * name, dns_rdatatype_t type)
{
    char namebuf[DNS_NAME_FORMATSIZE];
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_RESOLVER,
                   DNS_LOGMODULE_RESOLVER, level, "createfetch: %s %s", namebuf, typebuf);
}
...
dns_resolver_createfetch (dns_resolver_t * res, dns_name_t * name,
                          ...
                          dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset, dns_fetch_t ** fetchp)
{
    return (dns_resolver_createfetch2 (res, name, type, domain,
                                       nameservers, forwarders, NULL, 0,
                                       options, task, action, arg, rdataset, sigrdataset, fetchp));
}
                           ...
                           dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset, dns_fetch_t ** fetchp)
{
    ...
    if ((options & DNS_FETCHOPT_UNSHARED) == 0)
    {
        for (fctx = ISC_LIST_HEAD (res->buckets[bucketnum].fctxs); fctx != NULL; fctx = ISC_LIST_NEXT (fctx, link))
        {
    ...
    if (fctx != NULL && client != NULL)
    {
        ...
        for (fevent = ISC_LIST_HEAD (fctx->events); fevent != NULL; fevent = ISC_LIST_NEXT (fevent, ev_link))
        {
            if (fevent->client != NULL && fevent->id == id && isc_sockaddr_equal (fevent->client, client))
            {
    ...
    if (fctx == NULL)
    {
        result = fctx_create (res, name, type, domain, nameservers, options, bucketnum, &fctx);
        if (result != ISC_R_SUCCESS)
            goto unlock;
        new_fctx = ISC_TRUE;
    }

    result = fctx_join (fctx, task, client, id, action, arg, rdataset, sigrdataset, fetch);
    if (new_fctx)
    {
        if (result == ISC_R_SUCCESS)
        {
            ...
            ISC_EVENT_INIT (event, sizeof (*event), 0, NULL,
                            DNS_EVENT_FETCHCONTROL, fctx_start, fctx, NULL, NULL, NULL);
...
dns_resolver_logfetch (dns_fetch_t * fetch, isc_log_t * lctx,
                       isc_logcategory_t * category, isc_logmodule_t * module, int level, isc_boolean_t duplicateok)
{
    fetchctx_t *fctx;
    ...
    if (!fctx->logged || duplicateok)
    {
        ...
        isc_log_write (lctx, category, module, level,
                       ...
                       fctx->neterr, fctx->badresp, fctx->adberr, fctx->findfail, fctx->valfail);
...
isc_result_t
dns_resolver_addalternate (dns_resolver_t * resolver, isc_sockaddr_t * alt, dns_name_t * name, in_port_t port)
{
    alternate_t *a;
        ...
        for (bad = resolver->badcache[i]; bad != NULL; bad = next)
        {
            ...
            if (n < 0 || dns_name_equal (name, &bad->name))
            {
                if (prev == NULL)
                    ...
                    prev->next = bad->next;
                isc_mem_put (resolver->mctx, bad, sizeof (*bad) + bad->name.length);
                resolver->badcount--;
            }
            else
                prev = bad;
...
static void resizehash (dns_resolver_t * resolver, isc_time_t * now, isc_boolean_t grow)
{
    ...
    new = isc_mem_get (resolver->mctx, sizeof (*resolver->badcache) * newsize);
    if (new == NULL)
        return;
    ...
    isc_mem_put (resolver->mctx, resolver->badcache, sizeof (*resolver->badcache) * resolver->badhash);
...
void dns_resolver_addbadcache (dns_resolver_t * resolver, dns_name_t * name, dns_rdatatype_t type, isc_time_t * expire)
{
    ...
    if (resolver->badcache == NULL)
    {
        resolver->badcache = isc_mem_get (resolver->mctx, sizeof (*resolver->badcache) * DNS_BADCACHE_SIZE);
        if (resolver->badcache == NULL)
            goto cleanup;
        resolver->badhash = DNS_BADCACHE_SIZE;
        memset (resolver->badcache, 0, sizeof (*resolver->badcache) * resolver->badhash);
    }
    ...
    if (bad == NULL)
    {
        ...
        if (resolver->badcount < resolver->badhash * 2 && resolver->badhash > DNS_BADCACHE_SIZE)
...
isc_boolean_t
dns_resolver_getbadcache (dns_resolver_t * resolver, dns_name_t * name, dns_rdatatype_t type, isc_time_t * now)
{
    dns_badcache_t *bad, *prev, *next;
...
void dns_resolver_printbadcache (dns_resolver_t * resolver, FILE * fp)
{
    ...
    for (i = 0; i < resolver->badhash; i++)
    {
        ...
        for (bad = resolver->badcache[i]; bad != NULL; bad = next)
        {
            ...
            t /= 1000;
            fprintf (fp, "; %s/%s [ttl " "%" ISC_PLATFORM_QUADFORMAT "u]\n", namebuf, typebuf, t);
        }
    }
...
isc_result_t dns_resolver_disable_algorithm (dns_resolver_t * resolver, dns_name_t * name, unsigned int alg)
{
    ...
    if (resolver->algorithms == NULL)
    {
        result = dns_rbt_create (resolver->mctx, free_algorithm, resolver->mctx, &resolver->algorithms);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
    }
    ...
    if (result == ISC_R_SUCCESS || result == ISC_R_EXISTS)
    {
        ...
        if (algorithms == NULL || len > *algorithms)
        {
            ...
            *new = len;
            node->data = new;
            if (algorithms != NULL)
                isc_mem_put (resolver->mctx, algorithms, *algorithms);
        }
        else
...
isc_boolean_t dns_resolver_digest_supported (dns_resolver_t * resolver, unsigned int digest)
{
...
isc_result_t dns_resolver_setmustbesecure (dns_resolver_t * resolver, dns_name_t * name, isc_boolean_t value)
{
    ...
    if (resolver->mustbesecure == NULL)
    {
        result = dns_rbt_create (resolver->mctx, NULL, NULL, &resolver->mustbesecure);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
    }
    result = dns_rbt_addname (resolver->mustbesecure, name, value ? &yes : &no);
  cleanup:
#if USE_MBSLOCK
    RWUNLOCK (&resolver->mbslock, isc_rwlocktype_write);
#endif
...
void
dns_resolver_getclientsperquery (dns_resolver_t * resolver, isc_uint32_t * cur, isc_uint32_t * min, isc_uint32_t * max)
{
    REQUIRE (VALID_RESOLVER (resolver));
