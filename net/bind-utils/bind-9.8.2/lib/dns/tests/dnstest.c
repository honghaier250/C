...
static isc_result_t create_managers ()
{
    isc_result_t result;
...
isc_result_t dns_test_begin (FILE * logfile, isc_boolean_t start_managers)
{
    ...
    if (logfile != NULL)
    {
        ...
        CHECK (isc_log_createchannel (logconfig, "stderr", ISC_LOG_TOFILEDESC, ISC_LOG_DYNAMIC, &destination, 0));
...
isc_result_t dns_test_setupzonemgr ()
{
    isc_result_t result;
...
isc_result_t dns_test_loaddb (dns_db_t ** db, dns_dbtype_t dbtype, const char *origin, const char *testfile)
{
    ...
    result = dns_db_create (mctx, "rbt", name, dbtype, dns_rdataclass_in, 0, NULL, db);
    if (result != ISC_R_SUCCESS)
        return (result);
