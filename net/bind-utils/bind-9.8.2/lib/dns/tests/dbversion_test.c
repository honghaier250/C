...
static void callback (const char *file, int line, isc_assertiontype_t type, const char *cond)
{
    UNUSED (file);
    UNUSED (line);
    UNUSED (type);
...
static void setup_db ()
{
    ...
    result = dns_db_create (mctx, "rbt", dns_rootname, dns_dbtype_zone, dns_rdataclass_in, 0, NULL, &db1);
    ATF_REQUIRE_EQ (result, ISC_R_SUCCESS);
    dns_db_newversion (db1, &v1);

    result = dns_db_create (mctx, "rbt", dns_rootname, dns_dbtype_zone, dns_rdataclass_in, 0, NULL, &db2);
    ATF_REQUIRE_EQ (result, ISC_R_SUCCESS);
...
static void find (isc_assertioncallback_t callback)
{
    ...
    result = dns_db_find (db1, dns_rootname, VERSION (callback),
                          dns_rdatatype_soa, 0, 0, NULL, dns_fixedname_name (&fixed), &rdataset, NULL);
    if (callback != NULL)
...
static void allrdatasets (isc_assertioncallback_t callback)
{
    ...
    isc_assertion_setcallback (callback);
    result = dns_db_allrdatasets (db1, node, VERSION (callback), 0, &iterator);
    if (callback != NULL)
...
static void findrdataset (isc_assertioncallback_t callback)
{
    ...
    isc_assertion_setcallback (callback);
    result = dns_db_findrdataset (db1, node, VERSION (callback), dns_rdatatype_soa, 0, 0, &rdataset, NULL);
    if (callback != NULL)
...
static void deleterdataset (isc_assertioncallback_t callback)
{
    ...
    isc_assertion_setcallback (callback);
    result = dns_db_deleterdataset (db1, node, VERSION (callback), dns_rdatatype_soa, 0);
    if (callback != NULL)
...
static void subtract (isc_assertioncallback_t callback)
{
    ...
    isc_assertion_setcallback (callback);
    result = dns_db_subtractrdataset (db1, node, VERSION (callback), &rdataset, 0, NULL);
    if (callback != NULL)
...
static void addrdataset (isc_assertioncallback_t callback)
{
    ...
    isc_assertion_setcallback (callback);
    result = dns_db_addrdataset (db1, node, VERSION (callback), 0, &rdataset, 0, NULL);
    if (callback != NULL)
...
static void getnsec3parameters (isc_assertioncallback_t callback)
{
    ...
    isc_assertion_setcallback (callback);
    result = dns_db_getnsec3parameters (db1, VERSION (callback), &hash, &flags, &iterations, salt, &salt_length);
    if (callback != NULL)
        atf_tc_fail ("dns_db_dump did not assert");
    ATF_REQUIRE_EQ (result, ISC_R_NOTFOUND);
...
static void resigned (isc_assertioncallback_t callback)
{
    ...
    result = dns_rdata_fromstruct (&rdata, dns_rdataclass_in, dns_rdatatype_rrsig, &rrsig, &b);
    ATF_REQUIRE_EQ (result, ISC_R_SUCCESS);
