...
static int test_master (const char *testfile)
{
    isc_result_t result;
    ...
    result = dns_name_fromtext (&dns_origin, &source, dns_rootname, 0, &target);
    ...
    result = dns_master_loadfile (testfile, &dns_origin, &dns_origin, dns_rdataclass_in, ISC_TRUE, &callbacks, mctx);
    return (result);
}
...
{
    atf_tc_set_md_var (tc, "descr", "dns_master_loadfile() loads a " "valid master file and returns success");
}
...
{
    atf_tc_set_md_var (tc, "descr", "dns_master_loadfile() returns " "DNS_R_UNEXPECTED when file ends " "too soon");
}
...
ATF_TC_HEAD (master_noowner, tc)
{
    atf_tc_set_md_var (tc, "descr", "dns_master_loadfile() accepts broken "
                       "zones with no TTL for first record " "if it is an SOA");
}
...
ATF_TC_HEAD (master_nottl, tc)
{
    atf_tc_set_md_var (tc, "descr", "dns_master_loadfile() returns "
                       "DNS_R_NOOWNER when no owner name " "is specified");
}
...
{
    atf_tc_set_md_var (tc, "descr", "dns_master_loadfile() returns "
                       "DNS_R_BADCLASS when record class " "doesn't match zone class");
}
...
{
    atf_tc_set_md_var (tc, "descr", "dns_master_loadfile() understands " "DNSKEY with key material");
}
...
{
    atf_tc_set_md_var (tc, "descr", "dns_master_loadfile() understands " "DNSKEY with no key material");
}
...
{
    atf_tc_set_md_var (tc, "descr", "dns_master_loadfile() understands " "$INCLUDE");
}
...
{
    atf_tc_set_md_var (tc, "descr", "dns_master_loadfile() understands " "$INCLUDE failures");
}
...
{
    atf_tc_set_md_var (tc, "descr", "dns_master_loadfile() handles " "non-empty blank lines");
}
...
{
    atf_tc_set_md_var (tc, "descr", "dns_master_loadfile() allows " "leading zeroes in SOA");
}
...
ATF_TC_BODY (master_totext, tc)
{
    ...
    isc_buffer_init (&target, buf, BIGBUFLEN);
    result = dns_master_rdatasettotext (dns_rootname, &rdataset, &dns_master_style_debug, &target);
    ATF_CHECK_EQ (result, ISC_R_SUCCESS);
