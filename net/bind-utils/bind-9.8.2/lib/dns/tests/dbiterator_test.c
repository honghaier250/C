...
static isc_result_t setup_db (const char *testfile, dns_dbtype_t dbtype, dns_db_t ** db)
{
    isc_result_t result;
    ...
    result = dns_db_create (mctx, "rbt", &dns_origin, dbtype, dns_rdataclass_in, 0, NULL, db);
    if (result != ISC_R_SUCCESS)
        return (result);
...
static void test_create (const atf_tc_t * tc)
{
    ...
    result = setup_db (atf_tc_get_md_var (tc, "X-filename"), dns_dbtype_cache, &db);
    ATF_REQUIRE_EQ (result, ISC_R_SUCCESS);
...
static void test_walk (const atf_tc_t * tc)
{
    ...
    for (result = dns_dbiterator_first (iter); result == ISC_R_SUCCESS; result = dns_dbiterator_next (iter))
    {
...
static void test_reverse (const atf_tc_t * tc)
{
    ...
    for (result = dns_dbiterator_last (iter); result == ISC_R_SUCCESS; result = dns_dbiterator_prev (iter))
    {
        result = dns_dbiterator_current (iter, &node, name);
        if (result == DNS_R_NEWORIGIN)
            result = ISC_R_SUCCESS;
...
ATF_TC_HEAD (seek, tc)
{
    atf_tc_set_md_var (tc, "descr", "walk database starting at " "a particular node");
    atf_tc_set_md_var (tc, "X-filename", "testdata/dbiterator/zone1.data");
...
{
    atf_tc_set_md_var (tc, "descr", "walk database starting at an " "empty nonterminal node");
    atf_tc_set_md_var (tc, "X-filename", "testdata/dbiterator/zone1.data");
}
...
{
    atf_tc_set_md_var (tc, "descr", "attempt to walk database starting " "at a nonexistent node");
    atf_tc_set_md_var (tc, "X-filename", "testdata/dbiterator/zone1.data");
}
