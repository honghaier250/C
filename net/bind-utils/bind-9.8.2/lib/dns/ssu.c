...
static inline void destroy (dns_ssutable_t * table)
{
    isc_mem_t *mctx;
    ...
    while (!ISC_LIST_EMPTY (table->rules))
    {
        ...
        if (rule->types != NULL)
            isc_mem_put (mctx, rule->types, rule->ntypes * sizeof (dns_rdatatype_t));
...
dns_ssutable_addrule (dns_ssutable_t * table, isc_boolean_t grant,
                      ...
                      dns_name_t * name, unsigned int ntypes, dns_rdatatype_t * types)
{
    dns_ssurule_t *rule;
    ...
    if (ntypes > 0)
    {
        rule->types = isc_mem_get (mctx, ntypes * sizeof (dns_rdatatype_t));
        if (rule->types == NULL)
        {
            result = ISC_R_NOMEMORY;
            goto failure;
  ...
  failure:
    ...
    if (rule->types != NULL)
        isc_mem_put (mctx, rule->types, ntypes * sizeof (dns_rdatatype_t));
    isc_mem_put (mctx, rule, sizeof (dns_ssurule_t));

    return (result);
}

static inline isc_boolean_t isusertype (dns_rdatatype_t type)
{
    return (ISC_TF (type != dns_rdatatype_ns && type != dns_rdatatype_soa && type != dns_rdatatype_rrsig));
}
    ...
    switch (tcpaddr->family)
    {
        ...
        case AF_INET6:
            ...
            result = isc_string_printf (buf, sizeof (buf),
                                        ...
                                        ap[1] & 0x0f, (ap[1] >> 4) & 0x0f, ap[0] & 0x0f, (ap[0] >> 4) & 0x0f);
            RUNTIME_CHECK (result == ISC_R_SUCCESS);
            break;
        default:
            INSIST (0);
    ...
    switch (tcpaddr->family)
    {
        case AF_INET:
            ...
            result = isc_string_printf (buf, sizeof (buf),
                                        ...
                                        l & 0xf, (l >> 4) & 0xf,
                                        (l >> 8) & 0xf, (l >> 12) & 0xf,
                                        (l >> 16) & 0xf, (l >> 20) & 0xf, (l >> 24) & 0xf, (l >> 28) & 0xf);
            RUNTIME_CHECK (result == ISC_R_SUCCESS);
            break;
        case AF_INET6:
            ap = tcpaddr->type.in6.s6_addr;
...
dns_ssutable_checkrules (dns_ssutable_t * table, dns_name_t * signer,
                         dns_name_t * name, isc_netaddr_t * tcpaddr, dns_rdatatype_t type, const dst_key_t * key)
{
    dns_ssurule_t *rule;
    ...
    for (rule = ISC_LIST_HEAD (table->rules); rule != NULL; rule = ISC_LIST_NEXT (rule, link))
    {
        ...
        switch (rule->matchtype)
        {
            ...
            case DNS_SSUMATCHTYPE_SELFWILD:
                ...
                result = dns_name_concatenate (dns_wildcardname, signer, wildcard, NULL);
                if (result != ISC_R_SUCCESS)
                    continue;
                ...
                break;
            case DNS_SSUMATCHTYPE_SELFKRB5:
                if (!dst_gssapi_identitymatchesrealmkrb5 (signer, name, rule->identity))
                    continue;
                break;
            case DNS_SSUMATCHTYPE_SELFMS:
                if (!dst_gssapi_identitymatchesrealmms (signer, name, rule->identity))
                    continue;
                break;
            case DNS_SSUMATCHTYPE_SUBDOMAINKRB5:
                if (!dns_name_issubdomain (name, rule->name))
                    continue;
                if (!dst_gssapi_identitymatchesrealmkrb5 (signer, NULL, rule->identity))
                    continue;
                break;
            case DNS_SSUMATCHTYPE_SUBDOMAINMS:
                if (!dns_name_issubdomain (name, rule->name))
                    continue;
                if (!dst_gssapi_identitymatchesrealmms (signer, NULL, rule->identity))
                    continue;
                break;
            case DNS_SSUMATCHTYPE_TCPSELF:
                dns_fixedname_init (&fixed);
                ...
                if (dns_name_iswildcard (rule->identity))
                {
                    if (!dns_name_matcheswildcard (tcpself, rule->identity))
                        continue;
                }
                else
                {
            ...
            case DNS_SSUMATCHTYPE_6TO4SELF:
                ...
                if (dns_name_iswildcard (rule->identity))
                {
                    if (!dns_name_matcheswildcard (stfself, rule->identity))
                        continue;
                }
                else
                {
            ...
            case DNS_SSUMATCHTYPE_EXTERNAL:
                if (!dns_ssu_external_match (rule->identity, signer, name, tcpaddr, type, key, table->mctx))
                    continue;
                break;
            case DNS_SSUMATCHTYPE_DLZ:
                if (!dns_dlz_ssumatch (table->dlzdatabase, signer, name, tcpaddr, type, key))
                    continue;
                break;
        }
        ...
        if (rule->ntypes == 0)
        {
             ...
             * the type.
             */
            if (rule->matchtype != DNS_SSUMATCHTYPE_DLZ && !isusertype (type))
                continue;
        }
        else
        {
            ...
            {
                if (rule->types[i] == dns_rdatatype_any || rule->types[i] == type)
                    break;
            }
            if (i == rule->ntypes)
                continue;
