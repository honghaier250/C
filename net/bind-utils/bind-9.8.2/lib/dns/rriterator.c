...
isc_result_t dns_rriterator_first (dns_rriterator_t * it)
{
    ...
    {
        it->result = dns_dbiterator_current (it->dbit, &it->node, dns_fixedname_name (&it->fixedname));
        if (it->result != ISC_R_SUCCESS)
            return (it->result);

        it->result = dns_db_allrdatasets (it->db, it->node, it->ver, it->now, &it->rdatasetit);
        if (it->result != ISC_R_SUCCESS)
            return (it->result);
...
dns_rriterator_current (dns_rriterator_t * it, dns_name_t ** name,
                        isc_uint32_t * ttl, dns_rdataset_t ** rdataset, dns_rdata_t ** rdata)
{
