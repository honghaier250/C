...
dns_ncache_add (dns_message_t * message, dns_db_t * cache, dns_dbnode_t * node,
                dns_rdatatype_t covers, isc_stdtime_t now, dns_ttl_t maxttl, dns_rdataset_t * addedrdataset)
{
    return (dns_ncache_addoptout (message, cache, node, covers, now, maxttl, ISC_FALSE, addedrdataset));
}

isc_result_t
dns_ncache_addoptout (dns_message_t * message, dns_db_t * cache,
                      dns_dbnode_t * node, dns_rdatatype_t covers,
                      isc_stdtime_t now, dns_ttl_t maxttl, isc_boolean_t optout, dns_rdataset_t * addedrdataset)
{
    isc_result_t result;
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        if ((name->attributes & DNS_NAMEATTR_NCACHE) != 0)
        {
            ...
            {
                if ((rdataset->attributes & DNS_RDATASETATTR_NCACHE) == 0)
                    continue;
                type = rdataset->type;
                if (type == dns_rdatatype_rrsig)
                    type = rdataset->covers;
                if (type == dns_rdatatype_soa || type == dns_rdatatype_nsec || type == dns_rdatatype_nsec3)
                {
                    if (ttl > rdataset->ttl)
                        ttl = rdataset->ttl;
                    ...
                    if (r.length < 3)
                        return (ISC_R_NOSPACE);
                    isc_buffer_putuint16 (&buffer, rdataset->type);
                    isc_buffer_putuint8 (&buffer, (unsigned char) rdataset->trust);
                    /*
    ...
    if (trust == 0xffff)
    {
        if ((message->flags & DNS_MESSAGEFLAG_AA) != 0 && message->counts[DNS_SECTION_ANSWER] == 0)
        {
            /*
    ...
    RUNTIME_CHECK (dns_rdatalist_tordataset (&ncrdatalist, &ncrdataset) == ISC_R_SUCCESS);
    ...
    return (dns_db_addrdataset (cache, node, NULL, now, &ncrdataset, 0, addedrdataset));
}

isc_result_t
dns_ncache_towire (dns_rdataset_t * rdataset, dns_compress_t * cctx,
                   isc_buffer_t * target, unsigned int options, unsigned int *countp)
{
    dns_rdata_t rdata = DNS_RDATA_INIT;
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        for (i = 0; i < rcount; i++)
        {
            ...
            if ((options & DNS_NCACHETOWIRE_OMITDNSSEC) != 0 && dns_rdatatype_isdnssec (type))
                continue;
                           ...
                           dns_rdatatype_t covers, dns_rdataset_t * rdataset)
{
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        if (type != dns_rdatatype_rrsig || !dns_name_equal (&tname, name))
        {
            result = dns_rdataset_next (&clone);
            dns_rdata_reset (&rdata);
            continue;
        ...
        sigregion.base = raw;
        dns_rdata_reset (&rdata);
        dns_rdata_fromregion (&rdata, rdataset->rdclass, dns_rdatatype_rrsig, &sigregion);
...
void dns_ncache_current (dns_rdataset_t * ncacherdataset, dns_name_t * found, dns_rdataset_t * rdataset)
{
    ...
    if (type == dns_rdatatype_rrsig)
    {
        ...
        sigregion.base = raw;
        dns_rdata_reset (&rdata);
        dns_rdata_fromregion (&rdata, rdataset->rdclass, rdataset->type, &sigregion);
        (void) dns_rdata_tostruct (&rdata, &rrsig, NULL);
