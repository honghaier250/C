...
void dns_rdatalist_init (dns_rdatalist_t * rdatalist)
{
...
void isc__rdatalist_disassociate (dns_rdataset_t * rdataset)
{
    UNUSED (rdataset);
}
...
unsigned int isc__rdatalist_count (dns_rdataset_t * rdataset)
{
    ...
    count = 0;
    for (rdata = ISC_LIST_HEAD (rdatalist->rdata); rdata != NULL; rdata = ISC_LIST_NEXT (rdata, link))
        count++;
...
isc_result_t isc__rdatalist_addnoqname (dns_rdataset_t * rdataset, dns_name_t * name)
{
    ...
    for (rdset = ISC_LIST_HEAD (name->list); rdset != NULL; rdset = ISC_LIST_NEXT (rdset, link))
    {
        if (rdset->rdclass != rdataset->rdclass)
            continue;
        if (rdset->type == dns_rdatatype_nsec || rdset->type == dns_rdatatype_nsec3)
            neg = rdset;
    }
    if (neg == NULL)
    ...
    for (rdset = ISC_LIST_HEAD (name->list); rdset != NULL; rdset = ISC_LIST_NEXT (rdset, link))
    {
        if (rdset->type == dns_rdatatype_rrsig && rdset->covers == neg->type)
            negsig = rdset;
    }
...
isc_result_t
isc__rdatalist_getnoqname (dns_rdataset_t * rdataset, dns_name_t * name, dns_rdataset_t * neg, dns_rdataset_t * negsig)
{
    ...
    for (rdataset = ISC_LIST_HEAD (noqname->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
    {
        if (rdataset->rdclass != rdclass)
            continue;
        if (rdataset->type == dns_rdatatype_nsec || rdataset->type == dns_rdatatype_nsec3)
            tneg = rdataset;
    }
    if (tneg == NULL)
        return (ISC_R_NOTFOUND);
    ...
    for (rdataset = ISC_LIST_HEAD (noqname->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
    {
        if (rdataset->type == dns_rdatatype_rrsig && rdataset->covers == tneg->type)
            tnegsig = rdataset;
    }
    if (tnegsig == NULL)
        return (ISC_R_NOTFOUND);
...
isc_result_t
isc__rdatalist_getclosest (dns_rdataset_t * rdataset, dns_name_t * name, dns_rdataset_t * neg, dns_rdataset_t * negsig)
{
    dns_rdataclass_t rdclass = rdataset->rdclass;
    ...
    for (rdataset = ISC_LIST_HEAD (closest->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
    {
        if (rdataset->rdclass != rdclass)
            continue;
        if (rdataset->type == dns_rdatatype_nsec || rdataset->type == dns_rdatatype_nsec3)
