...
isc_result_t
dns_fwdtable_add (dns_fwdtable_t * fwdtable, dns_name_t * name, isc_sockaddrlist_t * addrs, dns_fwdpolicy_t fwdpolicy)
{
    isc_result_t result;
    ...
    for (sa = ISC_LIST_HEAD (*addrs); sa != NULL; sa = ISC_LIST_NEXT (sa, link))
    {
                    ...
                    dns_name_t * foundname, dns_forwarders_t ** forwardersp)
{
    ...
    result = dns_rbt_findname (fwdtable->table, name, 0, foundname, (void **) forwardersp);
    if (result == DNS_R_PARTIALMATCH)
