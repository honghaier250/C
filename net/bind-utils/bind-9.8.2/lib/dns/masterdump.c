...
LIBDNS_EXTERNAL_DATA const dns_master_style_t dns_master_style_default = {
    DNS_STYLEFLAG_OMIT_OWNER |
        ...
        DNS_STYLEFLAG_OMIT_TTL | DNS_STYLEFLAG_TTL | DNS_STYLEFLAG_COMMENT | DNS_STYLEFLAG_MULTILINE,
    24, 24, 24, 32, 80, 8
};
...
LIBDNS_EXTERNAL_DATA const dns_master_style_t dns_master_style_explicitttl = {
    DNS_STYLEFLAG_OMIT_OWNER |
        DNS_STYLEFLAG_OMIT_CLASS |
        DNS_STYLEFLAG_REL_OWNER | DNS_STYLEFLAG_REL_DATA | DNS_STYLEFLAG_COMMENT | DNS_STYLEFLAG_MULTILINE,
    24, 32, 32, 40, 80, 8
};

LIBDNS_EXTERNAL_DATA const dns_master_style_t dns_master_style_cache = {
    DNS_STYLEFLAG_OMIT_OWNER |
        DNS_STYLEFLAG_OMIT_CLASS | DNS_STYLEFLAG_MULTILINE | DNS_STYLEFLAG_TRUST | DNS_STYLEFLAG_NCACHE,
    24, 32, 32, 40, 80, 8
};
...
struct dns_dumpctx
{
    ...
    dns_masterformat_t format;
      isc_result_t (*dumpsets) (isc_mem_t * mctx, dns_name_t * name,
                                dns_rdatasetiter_t * rdsiter, dns_totext_ctx_t * ctx, isc_buffer_t * buffer, FILE * f);
};
#endif                            /* BIND9 */
...
static isc_result_t totext_ctx_init (const dns_master_style_t * style, dns_totext_ctx_t * ctx)
{
    ...
    if ((ctx->style.flags & DNS_STYLEFLAG_MULTILINE) != 0)
    {
        ...
        result = indent (&col, ctx->style.rdata_column, ctx->style.tab_width, &buf);
        /*
...
static isc_result_t ncache_summary (dns_rdataset_t * rdataset, isc_boolean_t omit_final_dot, isc_buffer_t * target)
{
    ...
    do
    {
        ...
        for (result = dns_rdataset_first (&rds); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rds))
        {
            ...
            else
            {
                ...
                CHECK (str_totext (" ", target));
                CHECK (dns_rdata_tofmttext (&rdata, dns_rootname, 0, 0, " ", target));
                CHECK (str_totext ("\n", target));
            }
        }
...
rdataset_totext (dns_rdataset_t * rdataset,
                 dns_name_t * owner_name, dns_totext_ctx_t * ctx, isc_boolean_t omit_final_dot, isc_buffer_t * target)
{
    isc_result_t result;
    ...
    while (result == ISC_R_SUCCESS)
    {
         ...
         * Owner name.
         */
        if (owner_name != NULL && !((ctx->style.flags & DNS_STYLEFLAG_OMIT_OWNER) != 0 && !first))
        {
        ...
        if ((ctx->style.flags & DNS_STYLEFLAG_NO_TTL) == 0 &&
            !((ctx->style.flags & DNS_STYLEFLAG_OMIT_TTL) != 0 && current_ttl_valid && rdataset->ttl == current_ttl))
        {
            char ttlbuf[64];
            ...
            length = snprintf (ttlbuf, sizeof (ttlbuf), "%u", rdataset->ttl);
        ...
        if ((ctx->style.flags & DNS_STYLEFLAG_NO_CLASS) == 0 &&
            ((ctx->style.flags & DNS_STYLEFLAG_OMIT_CLASS) == 0 || ctx->class_printed == ISC_FALSE))
        {
        ...
        if ((rdataset->attributes & DNS_RDATASETATTR_NEGATIVE) != 0)
        {
             ...
             */
            RETERR (ncache_summary (rdataset, omit_final_dot, target));
            break;
        }
        else
        {
            ...
            RETERR (dns_rdata_tofmttext (&rdata,
                                         ctx->origin,
                                         ctx->style.flags,
                                         ctx->style.line_length - ctx->style.rdata_column, ctx->linebreak, target));
...
dns_rdataset_totext (dns_rdataset_t * rdataset,
                     ...
                     isc_boolean_t omit_final_dot, isc_boolean_t question, isc_buffer_t * target)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "could not set master file style");
    ...
    if (question)
        return (question_totext (rdataset, owner_name, &ctx, omit_final_dot, target));
    else
        return (rdataset_totext (rdataset, owner_name, &ctx, omit_final_dot, target));
}

isc_result_t
dns_master_rdatasettotext (dns_name_t * owner_name,
                           dns_rdataset_t * rdataset, const dns_master_style_t * style, isc_buffer_t * target)
{
    dns_totext_ctx_t ctx;
    ...
    return (rdataset_totext (rdataset, owner_name, &ctx, ISC_FALSE, target));
}
                           ...
                           dns_rdataset_t * rdataset, const dns_master_style_t * style, isc_buffer_t * target)
{
    ...
    return (question_totext (rdataset, owner_name, &ctx, ISC_FALSE, target));
}
               ...
               dns_totext_ctx_t * ctx, isc_buffer_t * buffer, FILE * f)
{
    ...
    if ((ctx->style.flags & DNS_STYLEFLAG_TTL) != 0)
    {
        if (ctx->current_ttl_valid == ISC_FALSE || ctx->current_ttl != rdataset->ttl)
        {
            if ((ctx->style.flags & DNS_STYLEFLAG_COMMENT) != 0)
            {
                isc_buffer_clear (buffer);
                result = dns_ttl_totext (rdataset->ttl, ISC_TRUE, buffer);
                INSIST (result == ISC_R_SUCCESS);
                isc_buffer_usedregion (buffer, &r);
                fprintf (f, "$TTL %u\t; %.*s\n", rdataset->ttl, (int) r.length, (char *) r.base);
            }
            else
            {
                fprintf (f, "$TTL %u\n", rdataset->ttl);
    ...
    for (;;)
    {
        ...
        result = rdataset_totext (rdataset, name, ctx, ISC_FALSE, buffer);
        if (result != ISC_R_NOSPACE)
            break;
    ...
    if (result != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "master file write failed: %s", isc_result_totext (result));
        return (result);
    }
...
{
    return (dump_order (*((const dns_rdataset_t * const *) a)) - dump_order (*((const dns_rdataset_t * const *) b)));
}
                     ...
                     dns_rdatasetiter_t * rdsiter, dns_totext_ctx_t * ctx, isc_buffer_t * buffer, FILE * f)
{
  ...
  again:
    for (i = 0; itresult == ISC_R_SUCCESS && i < MAXSORT; itresult = dns_rdatasetiter_next (rdsiter), i++)
    {
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (((rds->attributes & DNS_RDATASETATTR_NEGATIVE) != 0) && (ctx->style.flags & DNS_STYLEFLAG_NCACHE) == 0)
        {
            /* Omit negative cache entries */
        }
        else
        {
            isc_result_t result = dump_rdataset (mctx, name, rds, ctx,
                                                 buffer, f);
        ...
        if (ctx->style.flags & DNS_STYLEFLAG_RESIGN && rds->attributes & DNS_RDATASETATTR_RESIGN)
        {
            isc_buffer_t b;
 ...
 */
static isc_result_t
dump_rdataset_raw (isc_mem_t * mctx, dns_name_t * name, dns_rdataset_t * rdataset, isc_buffer_t * buffer, FILE * f)
{
    isc_result_t result;
  ...
  restart:
    ...
    do
    {
         ...
         * continue?).
         */
        if (isc_buffer_availablelength (buffer) < sizeof (dlen) + r.length)
        {
            int newlength;
    ...
    if (result != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "raw master file write failed: %s", isc_result_totext (result));
        return (result);
    }
                    ...
                    dns_rdatasetiter_t * rdsiter, dns_totext_ctx_t * ctx, isc_buffer_t * buffer, FILE * f)
{
    ...
    for (result = dns_rdatasetiter_first (rdsiter); result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (rdsiter))
    {
        ...
        if (((rdataset.attributes & DNS_RDATASETATTR_NEGATIVE) != 0) && (ctx->style.flags & DNS_STYLEFLAG_NCACHE) == 0)
        {
            /* Omit negative cache entries */
        }
        else
        {
            result = dump_rdataset_raw (mctx, name, &rdataset, buffer, f);
        }
...
static isc_result_t closeandrename (FILE * f, isc_result_t result, const char *temp, const char *file)
{
    ...
    if (result != ISC_R_SUCCESS && logit)
    {
        isc_log_write (dns_lctx, ISC_LOGCATEGORY_GENERAL,
                       ...
                       "dumping master file: %s: fsync: %s", temp, isc_result_totext (result));
        logit = ISC_FALSE;
    }
    ...
    if (result != ISC_R_SUCCESS && logit)
    {
        isc_log_write (dns_lctx, ISC_LOGCATEGORY_GENERAL,
                       DNS_LOGMODULE_MASTERDUMP, ISC_LOG_ERROR,
                       "dumping master file: %s: fclose: %s", temp, isc_result_totext (result));
        logit = ISC_FALSE;
    }
    if (result == ISC_R_SUCCESS)
    ...
    if (result != ISC_R_SUCCESS && logit)
    {
        isc_log_write (dns_lctx, ISC_LOGCATEGORY_GENERAL,
                       DNS_LOGMODULE_MASTERDUMP, ISC_LOG_ERROR,
                       "dumping master file: rename: %s: %s", file, isc_result_totext (result));
    }
    return (result);
}
...
static void dump_quantum (isc_task_t * task, isc_event_t * event)
{
    ...
    if (dctx->file != NULL)
    {
        tresult = closeandrename (dctx->f, result, dctx->tmpfile, dctx->file);
...
static isc_result_t task_send (dns_dumpctx_t * dctx)
{
    ...
    event = isc_event_allocate (dctx->mctx, NULL, DNS_EVENT_DUMPQUANTUM, dump_quantum, dctx, sizeof (*event));
    if (event == NULL)
        return (ISC_R_NOMEMORY);
...
dumpctx_create (isc_mem_t * mctx, dns_db_t * db, dns_dbversion_t * version,
                const dns_master_style_t * style, FILE * f, dns_dumpctx_t ** dctxp, dns_masterformat_t format)
{
    dns_dumpctx_t *dctx;
    ...
    if (dctx->format == dns_masterformat_text && (dctx->tctx.style.flags & DNS_STYLEFLAG_REL_OWNER) != 0)
    {
...
static isc_result_t dumptostreaminc (dns_dumpctx_t * dctx)
{
        ...
        switch (dctx->format)
        {
            ...
            case dns_masterformat_raw:
                ...
                isc_log_write (dns_lctx,
                               ...
                               ISC_LOG_INFO, "dumping master file in raw " "format: stdtime is not 32bits");
                now32 = 0;
#else
                now32 = dctx->now;
#endif
                ...
                result = isc_stdio_write (buffer.base, 1, isc_buffer_usedlength (&buffer), dctx->f, NULL);
                if (result != ISC_R_SUCCESS)
                    return (result);
                isc_buffer_clear (&buffer);
                break;
    ...
    while (result == ISC_R_SUCCESS && (dctx->nodes == 0 || nodes--))
    {
        ...
        {
            dns_name_t *origin = dns_fixedname_name (&dctx->tctx.origin_fixname);
        ...
        result = dns_db_allrdatasets (dctx->db, node, dctx->version, dctx->now, &rdsiter);
        if (result != ISC_R_SUCCESS)
        {
            dns_db_detachnode (dctx->db, &node);
            goto fail;
        }
        result = (dctx->dumpsets) (dctx->mctx, name, rdsiter, &dctx->tctx, &buffer, dctx->f);
    ...
    if (dctx->nodes != 0 && result == ISC_R_SUCCESS)
    {
        ...
        else
        {
            ...
            isc_log_write (dns_lctx, ISC_LOGCATEGORY_GENERAL,
                           DNS_LOGMODULE_MASTERDUMP,
                           ISC_LOG_DEBUG (1), "dumptostreaminc(%p) new nodes -> %d\n", dctx, dctx->nodes);
        }
...
dns_master_dumptostreaminc (isc_mem_t * mctx, dns_db_t * db,
                            ...
                            dns_dumpdonefunc_t done, void *done_arg, dns_dumpctx_t ** dctxp)
{
    ...
    result = dumpctx_create (mctx, db, version, style, f, &dctx, dns_masterformat_text);
    if (result != ISC_R_SUCCESS)
        return (result);
...
dns_master_dumptostream (isc_mem_t * mctx, dns_db_t * db,
                         dns_dbversion_t * version, const dns_master_style_t * style, FILE * f)
{
    return (dns_master_dumptostream2 (mctx, db, version, style, dns_masterformat_text, f));
}

isc_result_t
dns_master_dumptostream2 (isc_mem_t * mctx, dns_db_t * db,
                          dns_dbversion_t * version,
                          const dns_master_style_t * style, dns_masterformat_t format, FILE * f)
{
    dns_dumpctx_t *dctx = NULL;
...
static isc_result_t opentmp (isc_mem_t * mctx, const char *file, char **tempp, FILE ** fp)
{
    FILE *f = NULL;
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (dns_lctx, ISC_LOGCATEGORY_GENERAL,
                       DNS_LOGMODULE_MASTERDUMP, ISC_LOG_ERROR,
                       "dumping master file: %s: open: %s", tempname, isc_result_totext (result));
        goto cleanup;
    }
    *tempp = tempname;
    *fp = f;
...
dns_master_dumpinc (isc_mem_t * mctx, dns_db_t * db, dns_dbversion_t * version,
                    ...
                    isc_task_t * task, dns_dumpdonefunc_t done, void *done_arg, dns_dumpctx_t ** dctxp)
{
    return (dns_master_dumpinc2 (mctx, db, version, style, filename, task,
                                 done, done_arg, dctxp, dns_masterformat_text));
}
...
dns_master_dump (isc_mem_t * mctx, dns_db_t * db, dns_dbversion_t * version,
                 const dns_master_style_t * style, const char *filename)
{
    return (dns_master_dump2 (mctx, db, version, style, filename, dns_masterformat_text));
}

isc_result_t
dns_master_dump2 (isc_mem_t * mctx, dns_db_t * db, dns_dbversion_t * version,
                  const dns_master_style_t * style, const char *filename, dns_masterformat_t format)
{
    FILE *f = NULL;
...
dns_master_dumpnodetostream (isc_mem_t * mctx, dns_db_t * db,
                             dns_dbversion_t * version,
                             dns_dbnode_t * node, dns_name_t * name, const dns_master_style_t * style, FILE * f)
{
    isc_result_t result;
...
dns_master_dumpnode (isc_mem_t * mctx, dns_db_t * db, dns_dbversion_t * version,
                     dns_dbnode_t * node, dns_name_t * name, const dns_master_style_t * style, const char *filename)
{
    FILE *f = NULL;
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (dns_lctx, ISC_LOGCATEGORY_GENERAL,
                       DNS_LOGMODULE_MASTERDUMP, ISC_LOG_ERROR,
                       "dumping node to file: %s: open: %s", filename, isc_result_totext (result));
        return (ISC_R_UNEXPECTED);
    }

    result = dns_master_dumpnodetostream (mctx, db, version, node, name, style, f);
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (dns_lctx, ISC_LOGCATEGORY_GENERAL,
                       DNS_LOGMODULE_MASTERDUMP, ISC_LOG_ERROR,
                       "dumping master file: %s: dump: %s", filename, isc_result_totext (result));
        (void) isc_stdio_close (f);
        return (ISC_R_UNEXPECTED);
    }
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (dns_lctx, ISC_LOGCATEGORY_GENERAL,
                       DNS_LOGMODULE_MASTERDUMP, ISC_LOG_ERROR,
                       "dumping master file: %s: close: %s", filename, isc_result_totext (result));
        return (ISC_R_UNEXPECTED);
    }
...
dns_master_stylecreate (dns_master_style_t ** stylep, unsigned int flags,
                        ...
                        unsigned int line_length, unsigned int tab_width, isc_mem_t * mctx)
{
    dns_master_style_t *style;
