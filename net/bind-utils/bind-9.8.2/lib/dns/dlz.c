...
static ISC_LIST (dns_dlzimplementation_t) dlz_implementations;
     ...
     static void dlz_initialize (void)
{
 ...
 */
static inline dns_dlzimplementation_t *dlz_impfind (const char *name)
{
    dns_dlzimplementation_t *imp;

    for (imp = ISC_LIST_HEAD (dlz_implementations); imp != NULL; imp = ISC_LIST_NEXT (imp, link))
...
isc_result_t dns_dlzallowzonexfr (dns_view_t * view, dns_name_t * name, isc_sockaddr_t * clientaddr, dns_db_t ** dbp)
{
    ...
    result = (*allowzonexfr) (dlzdatabase->implementation->driverarg,
                              dlzdatabase->dbdata, dlzdatabase->mctx, view->rdclass, name, clientaddr, dbp);
               ...
               unsigned int argc, char *argv[], dns_dlzdb_t ** dbp)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_INFO, "Loading '%s' using driver %s", dlzname, drivername);
    ...
    if (impinfo == NULL)
    {
        ...
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       ...
                       "unsupported DLZ database driver '%s'." "  %s not loaded.", drivername, dlzname);
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "DLZ driver loaded successfully.");
        return (ISC_R_SUCCESS);
    }
    else
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_ERROR, "DLZ driver failed to load.");
    }
...
void dns_dlzdestroy (dns_dlzdb_t ** dbp)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE, DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Unloading DLZ driver.");
...
isc_result_t dns_dlzfindzone (dns_view_t * view, dns_name_t * name, unsigned int minlabels, dns_db_t ** dbp)
{
    ...
    for (i = namelabels; i > minlabels && i > 1; i--)
    {
        ...
        result = (*findzone) (dlzdatabase->implementation->driverarg,
                              dlzdatabase->dbdata, dlzdatabase->mctx, view->rdclass, zonename, dbp);
                 ...
                 void *driverarg, isc_mem_t * mctx, dns_dlzimplementation_t ** dlzimp)
{
    ...
    /* Write debugging message to log */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Registering DLZ driver '%s'", drivername);
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "DLZ Driver '%s' already registered", drivername);
...
void dns_dlzunregister (dns_dlzimplementation_t ** dlzimp)
{
    ...
    /* Write debugging message to log */
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (2), "Unregistering DLZ driver.");
 ...
 */
isc_result_t dns_dlz_writeablezone (dns_view_t * view, const char *zone_name)
{
    ...
    result = dns_name_fromtext (dns_fixedname_name (&fixorigin), &buffer, dns_rootname, 0, NULL);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    ...
    {
        result = dns_ssutable_createdlz (dlzdatabase->mctx, &dlzdatabase->ssutable, view->dlzdatabase);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
    }
 ...
 */
isc_result_t dns_dlzconfigure (dns_view_t * view, isc_result_t (*callback) (dns_view_t *, dns_zone_t *))
{
    ...
    result = impl->methods->configure (impl->driverarg, dlzdatabase->dbdata, view);
    return (result);
}
                  ...
                  dns_rdatatype_t type, const dst_key_t * key)
{
    ...
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_DLZ, ISC_LOG_INFO, "No ssumatch method for DLZ database");
        return (ISC_FALSE);
    }

    r = impl->methods->ssumatch (signer, name, tcpaddr, type, key, impl->driverarg, dlzdatabase->dbdata);
    return (r);
}
