 ...
 */
isc_boolean_t
dns_compress_findglobal (dns_compress_t * cctx, const dns_name_t * name, dns_name_t * prefix, isc_uint16_t * offset)
{
...
static inline unsigned int name_length (const dns_name_t * name)
{
    isc_region_t r;
...
void dns_compress_add (dns_compress_t * cctx, const dns_name_t * name, const dns_name_t * prefix, isc_uint16_t offset)
{
    ...
    while (count > 0)
    {
        ...
        {
            node = isc_mem_get (cctx->mctx, sizeof (dns_compressnode_t));
            if (node == NULL)
                return;
        }
