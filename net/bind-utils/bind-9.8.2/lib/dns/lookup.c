...
static inline isc_result_t start_fetch (dns_lookup_t * lookup)
{
    isc_result_t result;
    ...
    result = dns_resolver_createfetch (lookup->view->resolver,
                                       ...
                                       &lookup->rdataset, &lookup->sigrdataset, &lookup->fetch);
...
static isc_result_t build_event (dns_lookup_t * lookup)
{
    ...
    result = dns_name_dup (dns_fixedname_name (&lookup->name), lookup->mctx, name);
...
static isc_result_t view_find (dns_lookup_t * lookup, dns_name_t * foundname)
{
    ...
    result = dns_view_find (lookup->view, name, type, 0, 0, ISC_FALSE,
                            ...
                            foundname, &lookup->rdataset, &lookup->sigrdataset);
    return (result);
}
...
static void lookup_find (dns_lookup_t * lookup, dns_fetchevent_t * event)
{
    ...
    do
    {
        ...
        if (event == NULL && !lookup->canceled)
        {
            ...
            INSIST (!dns_rdataset_isassociated (&lookup->sigrdataset));
            /*
             * If we have restarted then clear the old node.                 */
            if (lookup->event->node != NULL)
            {
                INSIST (lookup->event->db != NULL);
                dns_db_detachnode (lookup->event->db, &lookup->event->node);
            }
        ...
        switch (result)
        {
            case ISC_R_SUCCESS:
                ...
                if (event->node != NULL)
                    dns_db_attachnode (lookup->event->db, event->node, &lookup->event->node);
                break;
            case DNS_R_CNAME:
                /*
            ...
            case DNS_R_DNAME:
                ...
                result = dns_name_concatenate (prefix, &dname.dname, name, NULL);
    ...
    if (send_event)
    {
        ...
        isc_task_sendanddetach (&lookup->task, (isc_event_t **) & lookup->event);
                   ...
                   isc_taskaction_t action, void *arg, dns_lookup_t ** lookupp)
{
    ...
    ievent = isc_event_allocate (mctx, lookup, DNS_EVENT_LOOKUPDONE, action, arg, sizeof (*lookup->event));
    if (ievent == NULL)
    {
        result = ISC_R_NOMEMORY;
        goto cleanup_lookup;
