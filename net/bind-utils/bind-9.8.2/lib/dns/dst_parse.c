...
static int find_value (const char *s, const unsigned int alg)
{
    ...
    {
        if (strcasecmp (s, map[i].tag) == 0 && (TAG_ALG (map[i].value) == alg))
            return (map[i].value);
    }
    return (-1);
}
...
static int check_rsa (const dst_private_t * priv)
{
    ...
    if (have[TAG_RSA_ENGINE & mask])
        ok = have[TAG_RSA_MODULUS & mask] && have[TAG_RSA_PUBLICEXPONENT & mask] && have[TAG_RSA_LABEL & mask];
    else
        ok = have[TAG_RSA_MODULUS & mask] &&
            ...
            have[TAG_RSA_EXPONENT1 & mask] && have[TAG_RSA_EXPONENT2 & mask] && have[TAG_RSA_COEFFICIENT & mask];
    return (ok ? 0 : -1);
}
...
static int check_hmac_md5 (const dst_private_t * priv, isc_boolean_t old)
{
    ...
    if (priv->nelements != HMACMD5_NTAGS)
    {
         ...
         */
        if (old && priv->nelements == OLD_HMACMD5_NTAGS && priv->elements[0].tag == TAG_HMACMD5_KEY)
            return (0);
        return (-1);
    }
    /*
...
static int check_data (const dst_private_t * priv, const unsigned int alg, isc_boolean_t old)
{
...
isc_result_t
dst__privstruct_parse (dst_key_t * key, unsigned int alg, isc_lex_t * lex, isc_mem_t * mctx, dst_private_t * priv)
{
    int n = 0, major, minor;
...
#define READLINE(lex, opt, token)                \
    ...
    if (token.type != isc_tokentype_string || strcmp (DST_AS_STR (token), PRIVATE_KEY_STR) != 0)
    {
     ...
     */
    NEXTTOKEN (lex, opt, &token);
    if (token.type != isc_tokentype_string || strcmp (DST_AS_STR (token), ALGORITHM_STR) != 0)
    {
        ret = DST_R_INVALIDPRIVATEKEY;
        goto fail;
    }
    ...
    if (token.type != isc_tokentype_number || token.value.as_ulong != (unsigned long) dst_key_alg (key))
    {
        ret = DST_R_INVALIDPRIVATEKEY;
        goto fail;
    }
