 ...
 * or verifying.
 */
static isc_result_t gssapi_create_signverify_ctx (dst_key_t * key, dst_context_t * dctx)
{
    ...
    ctx->buffer = NULL;
    result = isc_buffer_allocate (dctx->mctx, &ctx->buffer, INITIAL_BUFFER_SIZE);
 ...
 */
static void gssapi_destroy_signverify_ctx (dst_context_t * dctx)
{
 ...
 */
static isc_result_t gssapi_adddata (dst_context_t * dctx, const isc_region_t * data)
{
    dst_gssapi_signverifyctx_t *ctx = dctx->ctxdata.gssctx;
 ...
 * Sign.
 */
static isc_result_t gssapi_sign (dst_context_t * dctx, isc_buffer_t * sig)
{
    dst_gssapi_signverifyctx_t *ctx = dctx->ctxdata.gssctx;
    ...
    {
        gss_log (3, "GSS sign error: %s", gss_error_tostring (gret, minor, buf, sizeof (buf)));
 ...
 * Verify.
 */
static isc_result_t gssapi_verify (dst_context_t * dctx, const isc_region_t * sig)
{
    dst_gssapi_signverifyctx_t *ctx = dctx->ctxdata.gssctx;
    ...
    if (gret != GSS_S_COMPLETE)
    {
        gss_log (3, "GSS verify error: %s", gss_error_tostring (gret, minor, err, sizeof (err)));
        if (gret == GSS_S_DEFECTIVE_TOKEN ||
            ...
            gret == GSS_S_CONTEXT_EXPIRED || gret == GSS_S_NO_CONTEXT || gret == GSS_S_FAILURE)
...
static isc_boolean_t gssapi_compare (const dst_key_t * key1, const dst_key_t * key2)
{
...
static isc_result_t gssapi_generate (dst_key_t * key, int unused, void (*callback) (int))
{
    UNUSED (key);
    UNUSED (unused);
    UNUSED (callback);
...
static isc_boolean_t gssapi_isprivate (const dst_key_t * key)
{
    UNUSED (key);
    return (ISC_TRUE);
}
...
static isc_result_t gssapi_restore (dst_key_t * key, const char *keystr)
{
    ...
    major = gss_import_sec_context (&minor, &gssbuffer, &key->keydata.gssctx);
...
static isc_result_t gssapi_dump (dst_key_t * key, isc_mem_t * mctx, char **buffer, int *length)
{
    OM_uint32 major, minor;
    ...
    major = gss_export_sec_context (&minor, &key->keydata.gssctx, &gssbuffer);
    if (major != GSS_S_COMPLETE)
    {
...
isc_result_t dst__gssapi_init (dst_func_t ** funcp)
{
    REQUIRE (funcp != NULL);
    if (*funcp == NULL)
