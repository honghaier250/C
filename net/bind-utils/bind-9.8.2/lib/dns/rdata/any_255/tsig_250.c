...
static inline isc_result_t fromtext_any_tsig (ARGS_FROMTEXT)
{
     ...
     */
    RETERR (isc_lex_getmastertoken (lexer, &token, isc_tokentype_string, ISC_FALSE));
    if (dns_tsigrcode_fromtext (&rcode, &token.value.as_textregion) != ISC_R_SUCCESS)
    {
...
static inline isc_result_t totext_any_tsig (ARGS_TOTEXT)
{
    ...
    sigtime = ((isc_uint64_t) sr.base[0] << 40) |
        ...
        ((isc_uint64_t) sr.base[3] << 16) | ((isc_uint64_t) sr.base[4] << 8) | (isc_uint64_t) sr.base[5];
    ...
    *bufp-- = ' ';
    ...
    RETERR (isc_base64_totext (&sigr, tctx->width - 2, tctx->linebreak, target));
    if ((tctx->flags & DNS_STYLEFLAG_MULTILINE) != 0)
        RETERR (str_totext (" ) ", target));
    else
        RETERR (str_totext (" ", target));
...
static inline isc_result_t fromstruct_any_tsig (ARGS_FROMSTRUCT)
{
    ...
    RETERR (uint32_tobuffer ((isc_uint32_t) (tsig->timesigned & 0xffffffffU), target));
...
static inline isc_boolean_t checkowner_any_tsig (ARGS_CHECKOWNER)
{
...
static inline isc_boolean_t checknames_any_tsig (ARGS_CHECKNAMES)
{
