...
static inline isc_result_t totext_dnskey (ARGS_TOTEXT)
{
    ...
    if ((tctx->flags & DNS_STYLEFLAG_COMMENT) != 0 && algorithm == DNS_KEYALG_PRIVATEDNS)
    {
        dns_name_t name;
...
static inline isc_boolean_t checkowner_dnskey (ARGS_CHECKOWNER)
{
...
static inline isc_boolean_t checknames_dnskey (ARGS_CHECKNAMES)
{
