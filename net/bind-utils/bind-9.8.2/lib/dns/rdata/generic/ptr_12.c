...
static inline isc_result_t fromtext_ptr (ARGS_FROMTEXT)
{
    ...
    if (rdclass == dns_rdataclass_in &&
        (options & DNS_RDATA_CHECKNAMES) != 0 && (options & DNS_RDATA_CHECKREVERSE) != 0)
    {
        isc_boolean_t ok;
...
static inline isc_boolean_t checkowner_ptr (ARGS_CHECKOWNER)
{
...
static inline isc_boolean_t checknames_ptr (ARGS_CHECKNAMES)
{
    isc_region_t region;
    ...
    if (dns_name_issubdomain (owner, &in_addr_arpa) ||
        dns_name_issubdomain (owner, &ip6_arpa) || dns_name_issubdomain (owner, &ip6_int))
    {
        dns_rdata_toregion (rdata, &region);
        dns_name_init (&name, NULL);
        dns_name_fromregion (&name, &region);
