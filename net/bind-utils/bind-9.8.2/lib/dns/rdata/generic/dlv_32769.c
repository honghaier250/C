...
static inline isc_result_t fromwire_dlv (ARGS_FROMWIRE)
{
    ...
    if (sr.length < 4 ||
        ...
        (sr.base[3] == DNS_DSDIGEST_GOST && sr.length < 4 + ISC_GOST_DIGESTLENGTH))
...
static inline isc_boolean_t checkowner_dlv (ARGS_CHECKOWNER)
{
...
static inline isc_boolean_t checknames_dlv (ARGS_CHECKNAMES)
{
