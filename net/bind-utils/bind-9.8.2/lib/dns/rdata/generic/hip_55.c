...
static inline isc_result_t fromstruct_hip (ARGS_FROMSTRUCT)
{
    ...
    REQUIRE ((hip->servers == NULL && hip->servers_len == 0) || (hip->servers != NULL && hip->servers_len != 0));
    ...
    myhip = *hip;
    for (result = dns_rdata_hip_first (&myhip); result == ISC_R_SUCCESS; result = dns_rdata_hip_next (&myhip))
        /* empty */ ;
...
static inline isc_boolean_t checkowner_hip (ARGS_CHECKOWNER)
{
...
static inline isc_boolean_t checknames_hip (ARGS_CHECKNAMES)
{
