...
static inline isc_result_t fromtext_loc (ARGS_FROMTEXT)
{
    ...
    if (*e == '.')
    {
        ...
        if (*l != '\0' && callbacks != NULL)
        {
            ...
            (*callbacks->warn) (callbacks, "%s: %s:%u: '%s' extra "
                                "precision digits ignored", "dns_rdata_fromtext", file, line, DNS_AS_STR (token));
        }
    }
    else
        s1 *= 1000;
  ...
  getalt:
     ...
     * Size: optional.
     */
    RETERR (isc_lex_getmastertoken (lexer, &token, isc_tokentype_string, ISC_TRUE));
...
static inline isc_result_t totext_loc (ARGS_TOTEXT)
{
    ...
    char buf[sizeof ("89 59 59.999 N 179 59 59.999 E " "42849672.95m 90000000m 90000000m 90000000m")];
    ...
    sprintf (buf, "%d %d %d.%03d %s %d %d %d.%03d %s %s%ld.%02ldm %s %s %s",
             ...
             d2, m2, s2, fs2, east ? "E" : "W", below ? "-" : "", altitude / 100, altitude % 100, sbuf, hbuf, vbuf);
...
static inline isc_result_t fromwire_loc (ARGS_FROMWIRE)
{
     ...
     */
    latitude = uint32_fromregion (&sr);
    if (latitude < (0x80000000UL - 90 * 3600000) || latitude > (0x80000000UL + 90 * 3600000))
        return (ISC_R_RANGE);
    isc_region_consume (&sr, 4);
     ...
     */
    longitude = uint32_fromregion (&sr);
    if (longitude < (0x80000000UL - 180 * 3600000) || longitude > (0x80000000UL + 180 * 3600000))
        return (ISC_R_RANGE);
...
static inline isc_result_t fromstruct_loc (ARGS_FROMSTRUCT)
{
    ...
    if (loc->v.v0.latitude < (0x80000000UL - 90 * 3600000) || loc->v.v0.latitude > (0x80000000UL + 90 * 3600000))
        return (ISC_R_RANGE);
    RETERR (uint32_tobuffer (loc->v.v0.latitude, target));

    if (loc->v.v0.longitude < (0x80000000UL - 180 * 3600000) || loc->v.v0.longitude > (0x80000000UL + 180 * 3600000))
        return (ISC_R_RANGE);
...
static inline isc_boolean_t checkowner_loc (ARGS_CHECKOWNER)
{
...
static inline isc_boolean_t checknames_loc (ARGS_CHECKNAMES)
{
