...
static inline isc_result_t totext_opt (ARGS_TOTEXT)
{
    ...
    while (r.length > 0)
    {
        ...
        if (length > 0)
        {
            ...
            or = r;
            or.length = length;
            RETERR (isc_base64_totext (&or, tctx->width - 2, tctx->linebreak, target));
...
static inline isc_boolean_t checkowner_opt (ARGS_CHECKOWNER)
{
...
static inline isc_boolean_t checknames_opt (ARGS_CHECKNAMES)
{
