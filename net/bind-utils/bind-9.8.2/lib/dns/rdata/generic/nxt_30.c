...
static inline isc_result_t fromtext_nxt (ARGS_FROMTEXT)
{
    ...
    do
    {
        ...
        else if (dns_rdatatype_fromtext (&covered, &token.value.as_textregion) == DNS_R_UNKNOWN)
...
static inline isc_result_t fromwire_nxt (ARGS_FROMWIRE)
{
    ...
    if (sr.length > 0 && (sr.base[0] & 0x80) == 0 && ((sr.length > 16) || sr.base[sr.length - 1] == 0))
...
static inline isc_boolean_t checkowner_nxt (ARGS_CHECKOWNER)
{
...
static inline isc_boolean_t checknames_nxt (ARGS_CHECKNAMES)
{
