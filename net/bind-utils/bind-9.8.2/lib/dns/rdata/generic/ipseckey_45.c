...
static inline isc_result_t totext_ipseckey (ARGS_TOTEXT)
{
    ...
    if (region.length > 0U)
    {
        ...
        RETERR (isc_base64_totext (&region, tctx->width - 2, tctx->linebreak, target));
    }
...
static inline isc_result_t tostruct_ipseckey (ARGS_TOSTRUCT)
{
    ...
    {
        ipseckey->key = mem_maybedup (mctx, region.base, ipseckey->keylength);
...
static inline isc_boolean_t checkowner_ipseckey (ARGS_CHECKOWNER)
{
...
static inline isc_boolean_t checknames_ipseckey (ARGS_CHECKNAMES)
{
