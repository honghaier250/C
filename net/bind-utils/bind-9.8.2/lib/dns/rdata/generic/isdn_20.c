...
static inline isc_result_t fromtext_isdn (ARGS_FROMTEXT)
{
    ...
    if (token.type != isc_tokentype_string && token.type != isc_tokentype_qstring)
    {
...
static inline isc_boolean_t checkowner_isdn (ARGS_CHECKOWNER)
{
...
static inline isc_boolean_t checknames_isdn (ARGS_CHECKNAMES)
{
