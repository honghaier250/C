...
static inline isc_result_t totext_tkey (ARGS_TOTEXT)
{
    ...
    RETERR (isc_base64_totext (&dr, tctx->width - 2, tctx->linebreak, target));
    if ((tctx->flags & DNS_STYLEFLAG_MULTILINE) != 0)
        RETERR (str_totext (" ) ", target));
    else
        RETERR (str_totext (" ", target));
...
static inline isc_boolean_t checkowner_tkey (ARGS_CHECKOWNER)
{
...
static inline isc_boolean_t checknames_tkey (ARGS_CHECKNAMES)
{
...
static inline isc_result_t casecompare_tkey (ARGS_COMPARE)
{
