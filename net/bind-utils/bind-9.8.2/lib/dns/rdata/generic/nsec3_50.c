...
static inline isc_result_t fromtext_nsec3 (ARGS_FROMTEXT)
{
    ...
    do
    {
        ...
        if (token.type != isc_tokentype_string)
            break;
        RETTOK (dns_rdatatype_fromtext (&covered, &token.value.as_textregion));
...
static inline isc_boolean_t checkowner_nsec3 (ARGS_CHECKOWNER)
{
...
static inline isc_boolean_t checknames_nsec3 (ARGS_CHECKNAMES)
{
