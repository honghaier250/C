...
static inline isc_result_t fromtext_in_wks (ARGS_FROMTEXT)
{
    ...
    do
    {
        ...
        if (*e == 0)
            ;
        else if (!mygetservbyname (service, ps, &port) && !mygetservbyname (DNS_AS_STR (token), ps, &port))
...
static inline isc_result_t fromstruct_in_wks (ARGS_FROMSTRUCT)
{
    ...
    REQUIRE ((wks->map != NULL && wks->map_len <= 8 * 1024) || wks->map_len == 0);
...
static inline isc_boolean_t checkowner_in_wks (ARGS_CHECKOWNER)
{
...
static inline isc_boolean_t checknames_in_wks (ARGS_CHECKNAMES)
{
