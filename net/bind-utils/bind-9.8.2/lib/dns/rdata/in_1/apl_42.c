...
static inline isc_result_t totext_in_apl (ARGS_TOTEXT)
{
    ...
    while (sr.length > 0)
    {
        ...
        n = snprintf (txt, sizeof (txt), "%s%s%u:", sep, neg ? "!" : "", afi);
...
isc_result_t dns_rdata_apl_first (dns_rdata_in_apl_t * apl)
{
    isc_uint32_t length;
...
isc_result_t dns_rdata_apl_next (dns_rdata_in_apl_t * apl)
{
    isc_uint32_t length;
...
isc_result_t dns_rdata_apl_current (dns_rdata_in_apl_t * apl, dns_rdata_apl_ent_t * ent)
{
    isc_uint32_t length;
...
static inline isc_boolean_t checkowner_in_apl (ARGS_CHECKOWNER)
{
