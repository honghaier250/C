 ...
 */
typedef isc_result_t dlz_dlopen_findzonedb_t (void *dbdata, const char *name);

/*
 * dlz_dlopen_lookup() is required for all DLZ external drivers
 */
typedef isc_result_t dlz_dlopen_lookup_t (const char *zone, const char *name, void *dbdata, dns_sdlzlookup_t * lookup);
 ...
 * want to support dynamic updates
 */
typedef isc_result_t dlz_dlopen_configure_t (dns_view_t * view, void *dbdata);
...
typedef isc_boolean_t dlz_dlopen_ssumatch_t (const char *signer,
                                             ...
                                             const char *type,
                                             const char *key,
                                             isc_uint32_t keydatalen, unsigned char *keydata, void *dbdata);
