...
static void destroynode (dns_ecdbnode_t * node)
{
    ...
    while ((header = ISC_LIST_HEAD (node->rdatasets)) != NULL)
    {
        ...
        headersize = dns_rdataslab_size ((unsigned char *) header, sizeof (*header));
...
static void
bind_rdataset (dns_ecdb_t * ecdb, dns_ecdbnode_t * node, rdatasetheader_t * header, dns_rdataset_t * rdataset)
{
    unsigned char *raw;
             ...
             isc_stdtime_t now, dns_rdataset_t * rdataset, unsigned int options, dns_rdataset_t * addedrdataset)
{
     ...
     */
    for (header = ISC_LIST_HEAD (ecdbnode->rdatasets); header != NULL; header = ISC_LIST_NEXT (header, link))
    {
        INSIST (header->type != rdataset->type || header->covers != rdataset->covers);
    }

    result = dns_rdataslab_fromrdataset (rdataset, mctx, &r, sizeof (rdatasetheader_t));
    if (result != ISC_R_SUCCESS)
        goto unlock;
