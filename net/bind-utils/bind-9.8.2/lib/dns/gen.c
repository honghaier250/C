...
char *funname (const char *, char *);

void doswitch (const char *, const char *, const char *, const char *, const char *, const char *);

void dodecl (char *, char *, char *);

void add (int, const char *, int, const char *, const char *);

void sd (int, const char *, const char *, char);

void insert_into_typenames (int, const char *, const char *);
...
char *funname (const char *s, char *buf)
{
    char *b = buf;
...
void
doswitch (const char *name, const char *function, const char *args, const char *tsw, const char *csw, const char *res)
{
    struct tt *tt;
    ...
    for (tt = types; tt != NULL; tt = tt->next)
    {
        ...
        if (tt->rdclass == 0)
            fprintf (stdout,
                     "\tcase %d:%s %s_%s(%s); break;", tt->type, result, function, funname (tt->typename, buf1), args);
        else
            fprintf (stdout,
                     "\t\tcase %d:%s %s_%s_%s(%s); break;",
                     tt->rdclass, result, function, funname (tt->classname, buf1), funname (tt->typename, buf2), args);
        fputs (" \\\n", stdout);
        lasttype = tt->type;
    }
    if (subswitch)
...
void dodecl (char *type, char *function, char *args)
{
    struct tt *tt;
    ...
    for (tt = types; tt; tt = tt->next)
        if (tt->rdclass)
            fprintf (stdout,
                     ...
                     type, function, funname (tt->classname, buf1), funname (tt->typename, buf2), args);
        else
            fprintf (stdout, "static inline %s %s_%s(%s);\n", type, function, funname (tt->typename, buf1), args);
}

static struct ttnam *find_typename (int type)
{
    int i;

    for (i = 0; i < TYPENAMES; i++)
    {
        if (typenames[i].typename[0] != 0 && typenames[i].type == type)
            return (&typenames[i]);
    }
    return (NULL);
}

void insert_into_typenames (int type, const char *typename, const char *attr)
{
    ...
    for (i = 0; i < TYPENAMES; i++)
    {
        if (typenames[i].typename[0] != 0 && typenames[i].type == type && strcmp (typename, typenames[i].typename) != 0)
        {
    ...
    {
        fprintf (stderr, "Error:  type %d has different attributes: " "%s, %s\n", type, ttn->attr, attr);
        exit (1);
    }
...
void add (int rdclass, const char *classname, int type, const char *typename, const char *dirname)
{
...
void sd (int rdclass, const char *classname, const char *dirname, char filetype)
{
...
int main (int argc, char **argv)
{
    ...
    while (next_file (&dir))
    {
        if (sscanf (dir.filename, "%10[0-9a-z]_%d", classname, &rdclass) != 2)
            continue;
    ...
    if (code)
    {
        ...
        doswitch ("FROMTEXTSWITCH", "fromtext", FROMTEXTARGS, FROMTEXTTYPE, FROMTEXTCLASS, FROMTEXTDEF);
        doswitch ("TOTEXTSWITCH", "totext", TOTEXTARGS, TOTEXTTYPE, TOTEXTCLASS, TOTEXTDEF);
        doswitch ("FROMWIRESWITCH", "fromwire", FROMWIREARGS, FROMWIRETYPE, FROMWIRECLASS, FROMWIREDEF);
        doswitch ("TOWIRESWITCH", "towire", TOWIREARGS, TOWIRETYPE, TOWIRECLASS, TOWIREDEF);
        doswitch ("COMPARESWITCH", "compare", COMPAREARGS, COMPARETYPE, COMPARECLASS, COMPAREDEF);
        doswitch ("CASECOMPARESWITCH", "casecompare", COMPAREARGS, COMPARETYPE, COMPARECLASS, COMPAREDEF);
        doswitch ("FROMSTRUCTSWITCH", "fromstruct", FROMSTRUCTARGS, FROMSTRUCTTYPE, FROMSTRUCTCLASS, FROMSTRUCTDEF);
        doswitch ("TOSTRUCTSWITCH", "tostruct", TOSTRUCTARGS, TOSTRUCTTYPE, TOSTRUCTCLASS, TOSTRUCTDEF);
        doswitch ("FREESTRUCTSWITCH", "freestruct", FREESTRUCTARGS, FREESTRUCTTYPE, FREESTRUCTCLASS, FREESTRUCTDEF);
        doswitch ("ADDITIONALDATASWITCH", "additionaldata",
                  ADDITIONALDATAARGS, ADDITIONALDATATYPE, ADDITIONALDATACLASS, ADDITIONALDATADEF);
        doswitch ("DIGESTSWITCH", "digest", DIGESTARGS, DIGESTTYPE, DIGESTCLASS, DIGESTDEF);
        doswitch ("CHECKOWNERSWITCH", "checkowner", CHECKOWNERARGS, CHECKOWNERTYPE, CHECKOWNERCLASS, CHECKOWNERDEF);
        doswitch ("CHECKNAMESSWITCH", "checknames", CHECKNAMESARGS, CHECKNAMESTYPE, CHECKNAMESCLASS, CHECKNAMESDEF);
...
#define RESERVED "DNS_RDATATYPEATTR_RESERVED"
    ...
    else if (type_enum)
    {
        ...
        for (tt = types; tt != NULL; tt = tt->next)
            if (tt->type != lasttype)
                fprintf (stdout, "\tdns_rdatatype_%s = %d,\n", funname (tt->typename, buf1), lasttype = tt->type);
        ...
        fprintf (stdout, "#define dns_rdatatype_ixfr\t" "((dns_rdatatype_t)dns_rdatatype_ixfr)\n");
...
#undef PRINTCLASS
    ...
    else if (structs)
    {
        ...
        for (tt = types; tt != NULL; tt = tt->next)
        {
            sprintf (buf, "%s/%s_%d.h", tt->dirname, tt->typename, tt->type);
    ...
    {
        for (tt = types; tt != NULL; tt = tt->next)
            fprintf (stdout, "%s:\t%s/%s_%d.h\n", file, tt->dirname, tt->typename, tt->type);
    }
