...
isc_result_t dns_keytable_delete (dns_keytable_t * keytable, dns_name_t * keyname)
{
    ...
    RWLOCK (&keytable->rwlock, isc_rwlocktype_write);
    result = dns_rbt_findnode (keytable->table, keyname, NULL, &node, NULL, DNS_RBTFIND_NOOPTIONS, NULL, NULL);
    if (result == ISC_R_SUCCESS)
    {
        if (node->data != NULL)
...
isc_result_t dns_keytable_deletekeynode (dns_keytable_t * keytable, dst_key_t * dstkey)
{
    ...
    knode = node->data;
    if (knode->next == NULL && (knode->key == NULL || dst_key_compare (knode->key, dstkey) == ISC_TRUE))
    {
        result = dns_rbt_deletenode (keytable->table, node, ISC_FALSE);
        goto finish;
    }
...
dns_keytable_findkeynode (dns_keytable_t * keytable, dns_name_t * name,
                          dns_secalg_t algorithm, dns_keytag_t tag, dns_keynode_t ** keynodep)
{
    isc_result_t result;
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        for (knode = data; knode != NULL; knode = knode->next)
        {
            ...
            if (algorithm == dst_key_alg (knode->key) && tag == dst_key_id (knode->key))
                break;
        }
        if (knode != NULL)
        {
...
isc_result_t
dns_keytable_findnextkeynode (dns_keytable_t * keytable, dns_keynode_t * keynode, dns_keynode_t ** nextnodep)
{
    isc_result_t result;
...
isc_result_t dns_keytable_issecuredomain (dns_keytable_t * keytable, dns_name_t * name, isc_boolean_t * wantdnssecp)
{
    isc_result_t result;
