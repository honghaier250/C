...
#ifdef GSSAPI
static inline void name_to_gbuffer (dns_name_t * name, isc_buffer_t * buffer, gss_buffer_desc * gbuffer)
{
...
static void log_cred (const gss_cred_id_t cred)
{
    ...
    {
        gss_log (3, "failed gss_inquire_cred: %s", gss_error_tostring (gret, minor, buf, sizeof (buf)));
        return;
    }

    gret = gss_display_name (&minor, gname, &gbuffer, NULL);
    if (gret != GSS_S_COMPLETE)
        gss_log (3, "failed gss_display_name: %s", gss_error_tostring (gret, minor, buf, sizeof (buf)));
    else
    {
        switch (usage)
        {
        ...
        gss_log (3, "gss cred: \"%s\", %s, %lu", (char *) gbuffer.value, usage_text, (unsigned long) lifetime);
    }
    ...
    if (gret == GSS_S_COMPLETE)
    {
        if (gbuffer.length != 0U)
        {
            ...
            if (gret != GSS_S_COMPLETE)
                gss_log (3, "failed gss_release_buffer: %s", gss_error_tostring (gret, minor, buf, sizeof (buf)));
        }
    }

    gret = gss_release_name (&minor, &gname);
    if (gret != GSS_S_COMPLETE)
        gss_log (3, "failed gss_release_name: %s", gss_error_tostring (gret, minor, buf, sizeof (buf)));
}
#endif
 ...
 */
static void check_config (const char *gss_name)
{
    const char *p;
    ...
    {
        gss_log (ISC_LOG_ERROR, "tkey-gssapi-credential (%s) " "should start with 'DNS/'", gss_name);
        return;
    }
    ...
    if (strcasecmp (p + 1, krb5_realm) != 0)
    {
        gss_log (ISC_LOG_ERROR, "default realm from krb5.conf (%s) "
                 "does not match tkey-gssapi-credential (%s)", krb5_realm, gss_name);
        krb5_free_context (krb5_ctx);
        return;
    }
    krb5_free_context (krb5_ctx);
...
isc_result_t dst_gssapi_acquirecred (dns_name_t * name, isc_boolean_t initiate, gss_cred_id_t * cred)
{
#ifdef GSSAPI
    ...
    if (name != NULL)
    {
        ...
        if (gret != GSS_S_COMPLETE)
        {
            ...
            gss_log (3, "failed gss_import_name: %s", gss_error_tostring (gret, minor, buf, sizeof (buf)));
    ...
    /* Get the credentials. */
    if (gname != NULL)
        gss_log (3, "acquiring credentials for %s", (char *) gnamebuf.value);
    else
    {
    ...
    gret = gss_acquire_cred (&minor, gname, GSS_C_INDEFINITE, &mech_oid_set, usage, cred, &mechs, &lifetime);

    if (gret != GSS_S_COMPLETE)
    {
        gss_log (3, "failed to acquire %s credentials for %s: %s",
                 initiate ? "initiate" : "accept",
                 (gname != NULL) ? (char *) gnamebuf.value : "?", gss_error_tostring (gret, minor, buf, sizeof (buf)));
        check_config ((char *) array);
        return (ISC_R_FAILURE);
    }

    gss_log (4, "acquired %s credentials for %s",
             initiate ? "initiate" : "accept", (gname != NULL) ? (char *) gnamebuf.value : "?");
...
isc_boolean_t dst_gssapi_identitymatchesrealmkrb5 (dns_name_t * signer, dns_name_t * name, dns_name_t * realm)
{
    ...
    *sname = '\0';
    ...
    if (name != NULL)
    {
        if ((strcasecmp (sname, nbuf) == 0) && (strcmp (rname, rbuf) == 0))
...
isc_boolean_t dst_gssapi_identitymatchesrealmms (dns_name_t * signer, dns_name_t * name, dns_name_t * realm)
{
    ...
    *sname = '\0';
    ...
    if (name != NULL)
    {
        if ((strcasecmp (sname, nbuf) == 0) && (strcmp (rname, rbuf) == 0) && (strcasecmp (nname, rbuf) == 0))
            return (isc_boolean_true);
    }
    else
    {
...
isc_result_t dst_gssapi_releasecred (gss_cred_id_t * cred)
{
    ...
    if (gret != GSS_S_COMPLETE)
    {
        ...
        gss_log (3, "failed releasing credential: %s", gss_error_tostring (gret, minor, buf, sizeof (buf)));
    }
    *cred = NULL;
 ...
 */
static void gss_err_message (isc_mem_t * mctx, isc_uint32_t major, isc_uint32_t minor, char **err_message)
{
    char buf[1024];
...
dst_gssapi_initctx (dns_name_t * name, isc_buffer_t * intoken,
                    isc_buffer_t * outtoken, gss_ctx_id_t * gssctx, isc_mem_t * mctx, char **err_message)
{
#ifdef GSSAPI
    isc_region_t r;
    ...
    gret = gss_init_sec_context (&minor, GSS_C_NO_CREDENTIAL, gssctx,
                                 ...
                                 0, NULL, gintokenp, NULL, &gouttoken, &ret_flags, NULL);

    if (gret != GSS_S_COMPLETE && gret != GSS_S_CONTINUE_NEEDED)
    {
        gss_err_message (mctx, gret, minor, err_message);
        gss_log (3, "Failure initiating security context: %s", *err_message);
        result = ISC_R_FAILURE;
        goto out;
    }
...
dst_gssapi_acceptctx (gss_cred_id_t cred,
                      ...
                      gss_ctx_id_t * ctxout, dns_name_t * principal, isc_mem_t * mctx)
{
#ifdef GSSAPI
    isc_region_t r;
    ...
    gss_buffer_desc gnamebuf = GSS_C_EMPTY_BUFFER, gintoken, gouttoken = GSS_C_EMPTY_BUFFER;
    ...
    if (gssapi_keytab != NULL)
    {
        ...
        if (gret != GSS_S_COMPLETE)
        {
            gss_log (3, "failed "
                     ...
                     gssapi_keytab, gss_error_tostring (gret, 0, buf, sizeof (buf)));
    ...
    gret = gss_accept_sec_context (&minor, &context, cred, &gintoken,
                                   GSS_C_NO_CHANNEL_BINDINGS, &gname, NULL, &gouttoken, NULL, NULL, NULL);
    ...
    switch (gret)
    {
        ...
        case GSS_S_FAILURE:
            ...
            /* fall through */
        default:
            gss_log (3, "failed gss_accept_sec_context: %s", gss_error_tostring (gret, minor, buf, sizeof (buf)));
            return (result);
    }
    ...
    if (gret == GSS_S_COMPLETE)
    {
         ...
         */
        if (gnamebuf.length > 0U && ((char *) gnamebuf.value)[gnamebuf.length - 1] == '\0')
            gnamebuf.length--;

        gss_log (3, "gss-api source name (accept) is %.*s", (int) gnamebuf.length, (char *) gnamebuf.value);

        GBUFFER_TO_REGION (gnamebuf, r);
        isc_buffer_init (&namebuf, r.base, r.length);
        isc_buffer_add (&namebuf, r.length);

        RETERR (dns_name_fromtext (principal, &namebuf, dns_rootname, 0, NULL));
...
isc_result_t dst_gssapi_deletectx (isc_mem_t * mctx, gss_ctx_id_t * gssctx)
{
    ...
    if (gret != GSS_S_COMPLETE)
    {
        ...
        gss_log (3, "Failure deleting security context %s", gss_error_tostring (gret, minor, buf, sizeof (buf)));
    }
    return (ISC_R_SUCCESS);
#else
    UNUSED (mctx);
...
char *gss_error_tostring (isc_uint32_t major, isc_uint32_t minor, char *buf, size_t buflen)
{
#ifdef GSSAPI
    gss_buffer_desc msg_minor = GSS_C_EMPTY_BUFFER, msg_major = GSS_C_EMPTY_BUFFER;

    OM_uint32 msg_ctx, minor_stat;

    /* Handle major status */
    msg_ctx = 0;
    (void) gss_display_status (&minor_stat, major, GSS_C_GSS_CODE, GSS_C_NULL_OID, &msg_ctx, &msg_major);

    /* Handle minor status */
    msg_ctx = 0;
    (void) gss_display_status (&minor_stat, minor, GSS_C_MECH_CODE, GSS_C_NULL_OID, &msg_ctx, &msg_minor);

    snprintf (buf, buflen, "GSSAPI error: Major = %s, Minor = %s.", (char *) msg_major.value, (char *) msg_minor.value);
    ...
    return (buf);
#else
    snprintf (buf, buflen, "GSSAPI error: Major = %u, Minor = %u.", major, minor);

    return (buf);
#endif
}

void gss_log (int level, const char *fmt, ...)
{
    va_list ap;

    va_start (ap, fmt);
    isc_log_vwrite (dns_lctx, DNS_LOGCATEGORY_GENERAL, DNS_LOGMODULE_TKEY, ISC_LOG_DEBUG (level), fmt, ap);
    va_end (ap);
}
