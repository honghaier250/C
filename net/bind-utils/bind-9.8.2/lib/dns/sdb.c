...
dns_sdb_register (const char *drivername, const dns_sdbmethods_t * methods,
                  void *driverdata, unsigned int flags, isc_mem_t * mctx, dns_sdbimplementation_t ** sdbimp)
{
    ...
    REQUIRE ((flags & ~(DNS_SDBFLAG_RELATIVEOWNER | DNS_SDBFLAG_RELATIVERDATA | DNS_SDBFLAG_THREADSAFE)) == 0);
    ...
    imp->dbimp = NULL;
    result = dns_db_register (drivername, dns_sdb_create, imp, mctx, &imp->dbimp);
    if (result != ISC_R_SUCCESS)
        goto cleanup_mutex;
    *sdbimp = imp;
...
static inline unsigned int initial_size (unsigned int len)
{
    unsigned int size;
                  ...
                  const unsigned char *rdatap, unsigned int rdlen)
{
    ...
    dns_rdata_fromregion (rdata, rdatalist->rdclass, rdatalist->type, &region);
...
isc_result_t dns_sdb_putrr (dns_sdblookup_t * lookup, const char *type, dns_ttl_t ttl, const char *data)
{
    ...
    do
    {
        ...
        result = dns_rdata_fromtext (NULL,
                                     ...
                                     typeval, lex, origin, 0, mctx, &rb, &lookup->callbacks);
    ...
    result = dns_sdb_putrdata (lookup, typeval, ttl, isc_buffer_base (&rb), isc_buffer_usedlength (&rb));
  failure:
    if (p != NULL)
        isc_mem_put (mctx, p, size);
    if (lex != NULL)
...
static isc_result_t getnode (dns_sdballnodes_t * allnodes, const char *name, dns_sdbnode_t ** nodep)
{
    ...
    if (sdbnode == NULL || !dns_name_equal (sdbnode->name, newname))
    {
        ...
        if (allnodes->origin == NULL && dns_name_equal (newname, &sdb->common.origin))
...
isc_result_t
dns_sdb_putnamedrr (dns_sdballnodes_t * allnodes, const char *name, const char *type, dns_ttl_t ttl, const char *data)
{
    isc_result_t result;
...
dns_sdb_putnamedrdata (dns_sdballnodes_t * allnodes, const char *name,
                       dns_rdatatype_t type, dns_ttl_t ttl, const void *rdata, unsigned int rdlen)
{
    isc_result_t result;
...
isc_result_t dns_sdb_putsoa (dns_sdblookup_t * lookup, const char *mname, const char *rname, isc_uint32_t serial)
{
    ...
    n = snprintf (str, sizeof (str), "%s %s %u %u %u %u %u",
                  mname, rname, serial,
                  SDB_DEFAULT_REFRESH, SDB_DEFAULT_RETRY, SDB_DEFAULT_EXPIRE, SDB_DEFAULT_MINIMUM);
...
static isc_result_t findnode (dns_db_t * db, dns_name_t * name, isc_boolean_t create, dns_dbnode_t ** nodep)
{
    ...
    MAYBE_UNLOCK (sdb);
    if (result != ISC_R_SUCCESS && !(result == ISC_R_NOTFOUND && isorigin && imp->methods->authority != NULL))
    {
        destroynode (node);
        return (result);
    }
                ...
                dns_rdataclass_t rdclass, unsigned int argc, char *argv[], void *driverarg, dns_db_t ** dbp)
{
    ...
    {
        MAYBE_LOCK (sdb);
        result = imp->methods->create (sdb->zone, argc, argv, imp->driverdata, &sdb->dbdata);
        MAYBE_UNLOCK (sdb);
        if (result != ISC_R_SUCCESS)
...
static void rdatasetiter_destroy (dns_rdatasetiter_t ** iteratorp)
{
    ...
    isc_mem_put (sdbiterator->common.db->mctx, sdbiterator, sizeof (sdb_rdatasetiter_t));
    *iteratorp = NULL;
}
...
static void rdatasetiter_current (dns_rdatasetiter_t * iterator, dns_rdataset_t * rdataset)
{
    ...
    list_tordataset (sdbiterator->current, iterator->db, iterator->node, rdataset);
}
