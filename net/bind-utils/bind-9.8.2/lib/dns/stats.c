...
isc_result_t dns_rdatatypestats_create (isc_mem_t * mctx, dns_stats_t ** statsp)
{
    ...
    return (create_stats (mctx, dns_statstype_rdtype, rdtypecounter_max, statsp));
}
...
isc_result_t dns_rdatasetstats_create (isc_mem_t * mctx, dns_stats_t ** statsp)
{
    ...
    return (create_stats (mctx, dns_statstype_rdataset, (rdtypecounter_max * 2) + 1, statsp));
}
...
{
    REQUIRE (DNS_STATS_VALID (stats) && stats->type == dns_statstype_rdataset);
...
void dns_generalstats_dump (dns_stats_t * stats, dns_generalstats_dumper_t dump_fn, void *arg, unsigned int options)
{
    ...
    isc_stats_dump (stats->counters, (isc_stats_dumper_t) dump_fn, arg, options);
}
...
void
dns_rdatatypestats_dump (dns_stats_t * stats, dns_rdatatypestats_dumper_t dump_fn, void *arg0, unsigned int options)
{
    rdatadumparg_t arg;
...
static void rdataset_dumpcb (isc_statscounter_t counter, isc_uint64_t value, void *arg)
{
    ...
    else if (counter < rdtypenxcounter_max)
    {
        dump_rdentry (counter - rdtypecounter_max, value,
                      DNS_RDATASTATSTYPE_ATTR_NXRRSET, rdatadumparg->fn, rdatadumparg->arg);
    }
    else
    {
        dump_rdentry (0, value, DNS_RDATASTATSTYPE_ATTR_NXDOMAIN, rdatadumparg->fn, rdatadumparg->arg);
    }
}
...
isc_result_t dns_stats_alloccounters (isc_mem_t * mctx, isc_uint64_t ** ctrp)
{
    ...
    isc_uint64_t *p = isc_mem_get (mctx, DNS_STATS_NCOUNTERS * sizeof (isc_uint64_t));
