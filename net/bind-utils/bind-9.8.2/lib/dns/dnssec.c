...
static int rdata_compare_wrapper (const void *rdata1, const void *rdata2)
{
    return (dns_rdata_compare ((const dns_rdata_t *) rdata1, (const dns_rdata_t *) rdata2));
}
...
isc_result_t dns_dnssec_keyfromrdata (dns_name_t * name, dns_rdata_t * rdata, isc_mem_t * mctx, dst_key_t ** key)
{
    ...
    INSIST (key != NULL);
    INSIST (*key == NULL);
    REQUIRE (rdata->type == dns_rdatatype_key || rdata->type == dns_rdatatype_dnskey);
...
static isc_result_t digest_sig (dst_context_t * ctx, dns_rdata_t * sigrdata, dns_rdata_rrsig_t * sig)
{
    ...
    RUNTIME_CHECK (dns_name_downcase (&sig->signer, dns_fixedname_name (&fname), NULL) == ISC_R_SUCCESS);
                 ...
                 isc_mem_t * mctx, isc_buffer_t * buffer, dns_rdata_t * sigrdata)
{
    ...
    ret = dns_rdata_fromstruct (&tmpsigrdata, sig.common.rdclass, sig.common.rdtype, &sig, databuf);
    if (ret != ISC_R_SUCCESS)
    ...
    ret = dns_rdata_fromstruct (sigrdata, sig.common.rdclass, sig.common.rdtype, &sig, buffer);
...
dns_dnssec_verify2 (dns_name_t * name, dns_rdataset_t * set, dst_key_t * key,
                    isc_boolean_t ignoretime, isc_mem_t * mctx, dns_rdata_t * sigrdata, dns_name_t * wild)
{
    dns_rdata_rrsig_t sig;
    ...
    if (labels - sig.labels > 0)
        dns_name_split (dns_fixedname_name (&fnewname), sig.labels + 1, NULL, dns_fixedname_name (&fnewname));
    ...
    if (ret == ISC_R_SUCCESS && labels - sig.labels > 0)
    {
        if (wild != NULL)
            RUNTIME_CHECK (dns_name_concatenate (dns_wildcardname,
                                                 dns_fixedname_name (&fnewname), wild, NULL) == ISC_R_SUCCESS);
...
dns_dnssec_verify (dns_name_t * name, dns_rdataset_t * set, dst_key_t * key,
                   isc_boolean_t ignoretime, isc_mem_t * mctx, dns_rdata_t * sigrdata)
{
    isc_result_t result;

    result = dns_dnssec_verify2 (name, set, key, ignoretime, mctx, sigrdata, NULL);
    if (result == DNS_R_FROMWILDCARD)
        result = ISC_R_SUCCESS;
    return (result);
}

static isc_boolean_t key_active (dst_key_t * key, isc_stdtime_t now)
{
    isc_result_t result;
                          ...
                          unsigned int maxkeys, dst_key_t ** keys, unsigned int *nkeys)
{
    ...
    *nkeys = 0;
    dns_rdataset_init (&rdataset);
    RETERR (dns_db_findrdataset (db, node, ver, dns_rdatatype_dnskey, 0, 0, &rdataset, NULL));
    ...
    while (result == ISC_R_SUCCESS && count < maxkeys)
    {
        ...
        if (!is_zone_key (pubkey) || (dst_key_flags (pubkey) & DNS_KEYTYPE_NOAUTH) != 0)
            goto next;
        /* Corrupted .key file? */
        ...
        result = dst_key_fromfile (dst_key_name (pubkey),
                                   dst_key_id (pubkey),
                                   dst_key_alg (pubkey),
                                   DST_TYPE_PUBLIC | DST_TYPE_PRIVATE, directory, mctx, &keys[count]);
        ...
        if (result == ISC_R_FILENOTFOUND)
        {
            ...
            {
                dst_key_setflags (pubkey, flags & ~DNS_KEYFLAG_REVOKE);
                result = dst_key_fromfile (dst_key_name (pubkey),
                                           dst_key_id (pubkey),
                                           dst_key_alg (pubkey),
                                           DST_TYPE_PUBLIC | DST_TYPE_PRIVATE, directory, mctx, &keys[count]);
                if (result == ISC_R_SUCCESS && dst_key_pubcompare (pubkey, keys[count], ISC_FALSE))
                {
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            dns_name_format (dst_key_name (pubkey), keybuf, sizeof (keybuf));
            dns_secalg_format (dst_key_alg (pubkey), algbuf, sizeof (algbuf));
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                           DNS_LOGMODULE_DNSSEC, ISC_LOG_WARNING,
                           "dns_dnssec_findzonekeys2: error "
                           "reading private key file %s/%s/%d: %s",
                           keybuf, algbuf, dst_key_id (pubkey), isc_result_totext (result));
        }
...
dns_dnssec_findzonekeys (dns_db_t * db, dns_dbversion_t * ver,
                         ...
                         unsigned int maxkeys, dst_key_t ** keys, unsigned int *nkeys)
{
    return (dns_dnssec_findzonekeys2 (db, ver, node, name, NULL, mctx, maxkeys, keys, nkeys));
}
...
isc_result_t dns_dnssec_signmessage (dns_message_t * msg, dst_key_t * key)
{
     ...
     */
    RETERR (dns_rdata_fromstruct (NULL, dns_rdataclass_any, dns_rdatatype_sig /* SIG(0) */ ,
                                  &sig, &databuf));
    ...
    RETERR (dns_rdata_fromstruct (rdata, dns_rdataclass_any, dns_rdatatype_sig /* SIG(0) */ ,
                                  &sig, dynbuf));
                      ...
                      isc_boolean_t ignoretime, isc_mem_t * mctx)
{
    ...
    return (dns_dnssec_signs (rdata, name, rdataset, sigrdataset, ignoretime, mctx));

}

isc_boolean_t
dns_dnssec_signs (dns_rdata_t * rdata, dns_name_t * name,
                  dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset, isc_boolean_t ignoretime, isc_mem_t * mctx)
{
    ...
    for (result = dns_rdataset_first (sigrdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (sigrdataset))
    {
        ...
        {
            result = dns_dnssec_verify2 (name, rdataset, dstkey, ignoretime, mctx, &sigrdata, NULL);
            if (result == ISC_R_SUCCESS)
            {
                dst_key_free (&dstkey);
                return (ISC_TRUE);
                             ...
                             isc_mem_t * mctx, dns_dnsseckeylist_t * keylist)
{
    ...
    while (isc_dir_read (&dir) == ISC_R_SUCCESS)
    {
        if (dir.entry.name[0] == 'K' &&
            ...
            dir.entry.name[len + 1] == '+' && strncasecmp (dir.entry.name + 1, namebuf, len) == 0)
        {
            ...
            result = dst_key_fromnamedfile (dir.entry.name,
                                            directory, DST_TYPE_PUBLIC | DST_TYPE_PRIVATE, mctx, &dstkey);
            ...
            if (result != ISC_R_SUCCESS)
            {
                isc_log_write (dns_lctx,
                               ...
                               "error reading key file %s: %s", dir.entry.name, isc_result_totext (result));
                continue;
            }
 ...
 */
static void addkey (dns_dnsseckeylist_t * keylist, dst_key_t ** newkey, isc_boolean_t savekeys, isc_mem_t * mctx)
{
    dns_dnsseckey_t *key;
    ...
    for (key = ISC_LIST_HEAD (*keylist); key != NULL; key = ISC_LIST_NEXT (key, link))
    {
        if (dst_key_id (key->key) == dst_key_id (*newkey) &&
            ...
            dns_name_equal (dst_key_name (key->key), dst_key_name (*newkey)))
            break;
    }
...
static isc_result_t mark_active_keys (dns_dnsseckeylist_t * keylist, dns_rdataset_t * rrsigs)
{
    ...
    for (key = ISC_LIST_HEAD (*keylist); key != NULL; key = ISC_LIST_NEXT (key, link))
    {
        ...
        for (result = dns_rdataset_first (&sigs); result == ISC_R_SUCCESS; result = dns_rdataset_next (&sigs))
        {
            dns_rdata_rrsig_t sig;
...
dns_dnssec_keylistfromrdataset (dns_name_t * origin,
                                ...
                                isc_boolean_t public, dns_dnsseckeylist_t * keylist)
{
    dns_rdataset_t keys;
    ...
    for (result = dns_rdataset_first (&keys); result == ISC_R_SUCCESS; result = dns_rdataset_next (&keys))
    {
        dns_rdata_reset (&rdata);
        ...
        result = dst_key_fromfile (dst_key_name (pubkey),
                                   dst_key_id (pubkey),
                                   dst_key_alg (pubkey), DST_TYPE_PUBLIC | DST_TYPE_PRIVATE, directory, mctx, &privkey);
        ...
        if (result == ISC_R_FILENOTFOUND)
        {
            ...
            if ((flags & DNS_KEYFLAG_REVOKE) != 0)
            {
                ...
                if (result == ISC_R_SUCCESS && dst_key_pubcompare (pubkey, privkey, ISC_FALSE))
                {
...
static isc_result_t make_dnskey (dst_key_t * key, unsigned char *buf, int bufsize, dns_rdata_t * target)
{
    ...
    dns_rdata_fromregion (target, dst_key_class (key), dns_rdatatype_dnskey, &r);
    return (ISC_R_SUCCESS);
}

static isc_result_t
publish_key (dns_diff_t * diff, dns_dnsseckey_t * key, dns_name_t * origin,
             dns_ttl_t ttl, isc_mem_t * mctx, isc_boolean_t allzsk, void (*report) (const char *, ...))
{
    isc_result_t result;
    ...
    report ("Fetching %s %d/%s from key %s.",
            ...
            dst_key_id (key->key), alg, key->source == dns_keysource_user ? "file" : "repository");
    ...
    /* publish key */
    RETERR (dns_difftuple_create (mctx, DNS_DIFFOP_ADD, origin, ttl, &dnskey, &tuple));
...
remove_key (dns_diff_t * diff, dns_dnsseckey_t * key, dns_name_t * origin,
            dns_ttl_t ttl, isc_mem_t * mctx, const char *reason, void (*report) (const char *, ...))
{
    isc_result_t result;
    ...
    dns_secalg_format (dst_key_alg (key->key), alg, sizeof (alg));
    report ("Removing %s key %d/%s from DNSKEY RRset.", reason, dst_key_id (key->key), alg);

    RETERR (make_dnskey (key->key, buf, sizeof (buf), &dnskey));
    RETERR (dns_difftuple_create (mctx, DNS_DIFFOP_DEL, origin, ttl, &dnskey, &tuple));
    dns_diff_appendminimal (diff, &tuple);
    result = ISC_R_SUCCESS;
                       ...
                       isc_mem_t * mctx, void (*report) (const char *, ...))
{
     ...
     */
    for (key = ISC_LIST_HEAD (*keys); key != NULL; key = ISC_LIST_NEXT (key, link))
    {
        if (key->source == dns_keysource_user && (key->hint_publish || key->force_publish))
        {
            RETERR (publish_key (diff, key, origin, ttl, mctx, allzsk, report));
        }
    }
    ...
    for (key1 = ISC_LIST_HEAD (*newkeys); key1 != NULL; key1 = next)
    {
        ...
        for (key2 = ISC_LIST_HEAD (*keys); key2 != NULL; key2 = ISC_LIST_NEXT (key2, link))
        {
            if (dst_key_pubcompare (key1->key, key2->key, ISC_TRUE))
            {
                int r1, r2;

                r1 = dst_key_flags (key1->key) & DNS_KEYFLAG_REVOKE;
                r2 = dst_key_flags (key2->key) & DNS_KEYFLAG_REVOKE;
        ...
        if (key2 == NULL)
        {
            ...
            if (key1->source != dns_keysource_zoneapex && (key1->hint_publish || key1->force_publish))
            {
                RETERR (publish_key (diff, key1, origin, ttl, mctx, allzsk, report));
        ...
        if (key1->hint_remove)
        {
            RETERR (remove_key (diff, key2, origin, ttl, mctx, "expired", report));
        ...
        else if (key_revoked && (dst_key_flags (key1->key) & DNS_KEYFLAG_REVOKE) != 0)
        {
             ...
             * in the new one.
             */
            RETERR (remove_key (diff, key2, origin, ttl, mctx, "revoked", report));
            ISC_LIST_UNLINK (*keys, key2, link);
            if (removed != NULL)
                ISC_LIST_APPEND (*removed, key2, link);
            else
        ...
        {
            if (!key2->is_active && (key1->hint_sign || key1->force_sign))
