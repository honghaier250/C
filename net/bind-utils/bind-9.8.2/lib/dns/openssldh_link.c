...
static isc_result_t openssldh_computesecret (const dst_key_t * pub, const dst_key_t * priv, isc_buffer_t * secret)
{
    DH *dhpub, *dhpriv;
...
static isc_boolean_t openssldh_compare (const dst_key_t * key1, const dst_key_t * key2)
{
    int status;
    ...
    status = BN_cmp (dh1->p, dh2->p) || BN_cmp (dh1->g, dh2->g) || BN_cmp (dh1->pub_key, dh2->pub_key);
...
static isc_boolean_t openssldh_paramcompare (const dst_key_t * key1, const dst_key_t * key2)
{
    int status;
    ...
    status = BN_cmp (dh1->p, dh2->p) || BN_cmp (dh1->g, dh2->g);
...
static isc_boolean_t openssldh_isprivate (const dst_key_t * key)
{
...
static isc_result_t openssldh_todns (const dst_key_t * key, isc_buffer_t * data)
{
    ...
    if (dh->g == &bn2 && (dh->p == &bn768 || dh->p == &bn1024 || dh->p == &bn1536))
    {
        plen = 1;
        glen = 0;
    }
...
static isc_result_t openssldh_fromdns (dst_key_t * key, isc_buffer_t * data)
{
    DH *dh;
...
static isc_result_t openssldh_tofile (const dst_key_t * key, const char *directory)
{
    int i;
...
static isc_result_t openssldh_parse (dst_key_t * key, isc_lex_t * lexer, dst_key_t * pub)
{
    dst_private_t priv;
...
#define DST_RET(a) {ret = a; goto err;}
    ...
    if ((key->key_size == 768 || key->key_size == 1024 || key->key_size == 1536) && BN_cmp (dh->g, &bn2) == 0)
    {
        ...
        else if (key->key_size == 1536 && BN_cmp (dh->p, &bn1536) == 0)
        {
            BN_free (dh->p);
            BN_free (dh->g);
            dh->p = &bn1536;
...
static void openssldh_cleanup (void)
{
    BN_free (&bn2);
    BN_free (&bn768);
    BN_free (&bn1024);
...
isc_result_t dst__openssldh_init (dst_func_t ** funcp)
{
    REQUIRE (funcp != NULL);
    if (*funcp == NULL)
    {
