...
#define DNS_AS_STR(t) ((t).value.as_textregion.base)
     ...
     static isc_result_t
         commit (dns_rdatacallbacks_t *, dns_loadctx_t *, rdatalist_head_t *, dns_name_t *, const char *, unsigned int);

     static isc_boolean_t is_glue (rdatalist_head_t *, dns_name_t *);

     static dns_rdatalist_t *grow_rdatalist (int, dns_rdatalist_t *, int, rdatalist_head_t *,
                                             rdatalist_head_t *, isc_mem_t * mctx);

     static dns_rdata_t *grow_rdata (int, dns_rdata_t *, int, rdatalist_head_t *, rdatalist_head_t *, isc_mem_t *);
          ...
          isc_boolean_t eol, dns_rdatacallbacks_t * callbacks)
{
    ...
    options |= ISC_LEXOPT_EOL | ISC_LEXOPT_EOF | ISC_LEXOPT_DNSMULTILINE | ISC_LEXOPT_ESCAPE;
        ...
        switch (result)
        {
            ...
            default:
                (*callbacks->error) (callbacks,
                                     ...
                                     isc_lex_getsourceline (lex), isc_result_totext (result));
                return (result);
        }
     /*NOTREACHED*/}
    if (eol != ISC_TRUE)
        if (token->type == isc_tokentype_eol || token->type == isc_tokentype_eof)
        {
            (*callbacks->error) (callbacks,
                                 ...
                                 isc_lex_getsourcename (lex),
                                 isc_lex_getsourceline (lex), (token->type == isc_tokentype_eol) ? "line" : "file");
...
static void loadctx_destroy (dns_loadctx_t * lctx)
{
    ...
    if (lctx->f != NULL)
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_stdio_close() failed: %s", isc_result_totext (result));
        }
    }
...
loadctx_create (dns_masterformat_t format, isc_mem_t * mctx,
                ...
                dns_loaddonefunc_t done, void *done_arg, isc_lex_t * lex, dns_loadctx_t ** lctxp)
{
    dns_loadctx_t *lctx;
    ...
    REQUIRE ((task == NULL && done == NULL) || (task != NULL && done != NULL));
...
static isc_result_t genname (char *name, int it, char *buffer, size_t length)
{
    ...
    while (*name != '\0')
    {
        if (*name == '$')
        {
            ...
            if (*name == '{')
            {
                n = sscanf (name, "{%d,%u,%1[doxXnN]}", &delta, &width, mode);
                switch (n)
                {
                    case 1:
                        break;
                    case 2:
                        n = snprintf (fmt, sizeof (fmt), "%%0%ud", width);
                        break;
                    case 3:
                        if (mode[0] == 'n' || mode[0] == 'N')
                            nibblemode = ISC_TRUE;
                        n = snprintf (fmt, sizeof (fmt), "%%0%u%c", width, mode[0]);
                        break;
                    default:
                        return (DNS_R_SYNTAX);
                }
            ...
            if (nibblemode)
                n = nibbles (numbuf, sizeof (numbuf), width, mode[0], it + delta);
            else
...
static isc_result_t openfile_raw (dns_loadctx_t * lctx, const char *master_file)
{
    ...
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_stdio_open() failed: %s", isc_result_totext (result));
    }
...
static isc_result_t
generate (dns_loadctx_t * lctx, char *range, char *lhs, char *gtype, char *rhs, const char *source, unsigned int line)
{
    ...
    if (n < 2 || stop < start)
    {
        (*callbacks->error) (callbacks, "%s: %s:%lu: invalid range '%s'", "$GENERATE", source, line, range);
        result = DNS_R_SYNTAX;
        goto insist_cleanup;
    }
    if (n == 2)
    ...
    if (result != ISC_R_SUCCESS)
    {
        (*callbacks->error) (callbacks, "%s: %s:%lu: unknown RR type '%s'", "$GENERATE", source, line, gtype);
        goto insist_cleanup;
    }
    ...
    for (i = start; i <= stop; i += step)
    {
        ...
        result = dns_name_fromtext (owner, &buffer, ictx->origin, 0, NULL);
        if (result != ISC_R_SUCCESS)
            goto error_cleanup;

        if ((lctx->options & DNS_MASTER_ZONE) != 0 &&
            (lctx->options & DNS_MASTER_SLAVE) == 0 &&
            (lctx->options & DNS_MASTER_KEY) == 0 && !dns_name_issubdomain (owner, lctx->top))
        {
             ...
             */
            (*callbacks->warn) (callbacks, "%s:%lu: " "ignoring out-of-zone data (%s)", source, line, namebuf);
            continue;
        }
        ...
        result = dns_rdata_fromtext (&rdata, lctx->zclass, type,
                                     lctx->lex, ictx->origin, 0, lctx->mctx, &target, callbacks);
  ...
  error_cleanup:
    if (result == ISC_R_NOMEMORY)
        (*callbacks->error) (callbacks, "$GENERATE: %s", dns_result_totext (result));
    else
        (*callbacks->error) (callbacks, "$GENERATE: %s:%lu: %s", source, line, dns_result_totext (result));
...
static void limit_ttl (dns_rdatacallbacks_t * callbacks, const char *source, unsigned int line, isc_uint32_t * ttlp)
{
    if (*ttlp > 0x7fffffffUL)
    {
        (callbacks->warn) (callbacks,
                           "%s: %s:%lu: "
                           "$TTL %lu > MAXTTL, " "setting $TTL to 0", "dns_master_load", source, line, *ttlp);
        *ttlp = 0;
    }
}
...
static isc_result_t check_ns (dns_loadctx_t * lctx, isc_token_t * token, const char *source, unsigned long line)
{
    ...
    if (result != ISC_R_SUCCESS)
        (*callback) (lctx->callbacks, "%s:%lu: NS record '%s' "
                     "appears to be an address", source, line, DNS_AS_STR (*token));
    if (tmp != NULL)
...
static void
check_wildcard (dns_incctx_t * ictx, const char *source, unsigned long line, dns_rdatacallbacks_t * callbacks)
{
    dns_name_t *name;
    ...
    if (dns_name_internalwildcard (name))
    {
        ...
        (*callbacks->warn) (callbacks, "%s:%lu: warning: ownername "
                            "'%s' contains an non-terminal wildcard", source, line, namebuf);
    }
}
...
static isc_result_t load_text (dns_loadctx_t * lctx)
{
    ...
    do
    {
        ...
        GETTOKEN (lctx->lex, ISC_LEXOPT_INITIALWS | ISC_LEXOPT_QSTRING, &token, ISC_TRUE);
        line = isc_lex_getsourceline (lctx->lex);
        ...
        else if (token.type == isc_tokentype_string || token.type == isc_tokentype_qstring)
        {
            ...
            {
                GETTOKEN (lctx->lex, 0, &token, ISC_FALSE);
                result = dns_ttl_fromtext (&token.value.as_textregion, &lctx->ttl);
                if (MANYERRS (lctx, result))
                {
                    SETRESULT (lctx, result);
                    lctx->ttl = 0;
            ...
            else if (strcasecmp (DNS_AS_STR (token), "$INCLUDE") == 0)
            {
                COMMITALL;
                if ((lctx->options & DNS_MASTER_NOINCLUDE) != 0)
                {
                    (callbacks->error) (callbacks, "%s: %s:%lu: $INCLUDE not allowed", "dns_master_load", source, line);
                ...
                if (ttl_offset != 0)
                {
                    (callbacks->error) (callbacks,
                                        "%s: %s:%lu: $INCLUDE "
                                        "may not be used with $DATE", "dns_master_load", source, line);
                    result = DNS_R_SYNTAX;
                    goto insist_and_cleanup;
                }
                GETTOKEN (lctx->lex, ISC_LEXOPT_QSTRING, &token, ISC_FALSE);
                if (include_file != NULL)
                    isc_mem_free (mctx, include_file);
                include_file = isc_mem_strdup (mctx, DNS_AS_STR (token));
                if (include_file == NULL)
                {
                    result = ISC_R_NOMEMORY;
                    goto log_and_cleanup;
                ...
                if (token.type == isc_tokentype_eol || token.type == isc_tokentype_eof)
                {
                     ...
                     * No origin field.
                     */
                    result = pushfile (include_file, ictx->origin, lctx);
                    if (MANYERRS (lctx, result))
                    {
                        SETRESULT (lctx, result);
            ...
            else if (strcasecmp (DNS_AS_STR (token), "$DATE") == 0)
            {
                ...
                result = dns_time64_fromtext (DNS_AS_STR (token), &dump_time64);
                if (MANYERRS (lctx, result))
                {
                    SETRESULT (lctx, result);
                    LOGIT (result);
                ...
                if (dump_time != dump_time64)
                {
                    UNEXPECTED_ERROR (__FILE__, __LINE__,
                                      "%s: %s:%lu: $DATE outside epoch", "dns_master_load", source, line);
                ...
                if (dump_time > current_time)
                {
                    UNEXPECTED_ERROR (__FILE__, __LINE__,
                                      "%s: %s:%lu: "
                                      "$DATE in future, using current date", "dns_master_load", source, line);
            ...
            else if (strcasecmp (DNS_AS_STR (token), "$GENERATE") == 0)
            {
                /*
                 * Lazy cleanup.
                 */
                ...
                /* TTL? */
                if (dns_ttl_fromtext (&token.value.as_textregion, &lctx->ttl) == ISC_R_SUCCESS)
                {
                    limit_ttl (callbacks, source, line, &lctx->ttl);
                ...
                /* CLASS? */
                if (rdclass == 0 && dns_rdataclass_fromtext (&rdclass, &token.value.as_textregion) == ISC_R_SUCCESS)
                    GETTOKEN (lctx->lex, 0, &token, ISC_FALSE);
                /* TYPE */
                ...
                {
                    (*callbacks->error) (callbacks, "%s: %s:%lu: no TTL specified", "dns_master_load", source, line);
                    result = DNS_R_NOTTL;
                    if (MANYERRS (lctx, result))
                    {
                        SETRESULT (lctx, result);
                ...
                result = generate (lctx, range, lhs, gtype, rhs, source, line);
                if (MANYERRS (lctx, result))
                {
                    SETRESULT (lctx, result);
                }
            ...
            else if (strncasecmp (DNS_AS_STR (token), "$", 1) == 0)
            {
                (callbacks->error) (callbacks,
                                    "%s: %s:%lu: "
                                    "unknown $ directive '%s'", "dns_master_load", source, line, DNS_AS_STR (token));
                result = DNS_R_SYNTAX;
                if (MANYERRS (lctx, result))
                {
                    SETRESULT (lctx, result);
            ...
            isc_buffer_init (&buffer, token.value.as_region.base, token.value.as_region.length);
            isc_buffer_add (&buffer, token.value.as_region.length);
            isc_buffer_setactive (&buffer, token.value.as_region.length);
            result = dns_name_fromtext (new_name, &buffer, ictx->origin, 0, NULL);
            if (MANYERRS (lctx, result))
            {
                SETRESULT (lctx, result);
                LOGIT (result);
            ...
            if (finish_origin)
            {
                if (ictx->origin_in_use != -1)
                    ictx->in_use[ictx->origin_in_use] = ISC_FALSE;
             ...
             */
            if (ictx->glue != NULL && dns_name_compare (ictx->glue, new_name) != 0)
            {
                result = commit (callbacks, lctx, &glue_list, ictx->glue, source, ictx->glue_line);
                if (MANYERRS (lctx, result))
                {
                    SETRESULT (lctx, result);
                }
                ...
                if (ictx->glue_in_use != -1)
                    ictx->in_use[ictx->glue_in_use] = ISC_FALSE;
                ictx->glue_in_use = -1;
                ictx->glue = NULL;
             ...
             * have.
             */
            if ((ictx->glue == NULL) && (ictx->current == NULL || dns_name_compare (ictx->current, new_name) != 0))
            {
                if (current_has_delegation && is_glue (&current_list, new_name))
                {
                    rdcount_save = rdcount;
                    rdlcount_save = rdlcount;
                    target_save = target;
                    ...
                    ictx->in_use[ictx->glue_in_use] = ISC_TRUE;
                }
                else
                {
                    result = commit (callbacks, lctx, &current_list, ictx->current, source, ictx->current_line);
                    if (MANYERRS (lctx, result))
                    {
                        SETRESULT (lctx, result);
                    }
                    ...
                    if (ictx->current_in_use != -1)
                        ictx->in_use[ictx->current_in_use] = ISC_FALSE;
                    ictx->current_in_use = new_in_use;
                    ictx->in_use[ictx->current_in_use] = ISC_TRUE;
                 ...
                 */
                if ((lctx->options & DNS_MASTER_CHECKWILDCARD) != 0)
                    check_wildcard (ictx, source, line, callbacks);

            }
            if ((lctx->options & DNS_MASTER_ZONE) != 0 &&
                (lctx->options & DNS_MASTER_SLAVE) == 0 &&
                (lctx->options & DNS_MASTER_KEY) == 0 && !dns_name_issubdomain (new_name, lctx->top))
            {
                char namebuf[DNS_NAME_FORMATSIZE];

                dns_name_format (new_name, namebuf, sizeof (namebuf));
                /*
                 * Ignore out-of-zone data.
                 */
                (*callbacks->warn) (callbacks, "%s:%lu: " "ignoring out-of-zone data (%s)", source, line, namebuf);
        ...
        else
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__,
                              ...
                              "unexpected token type (%d)", source, line, token.type);
            result = ISC_R_UNEXPECTED;
            if (MANYERRS (lctx, result))
            {
                SETRESULT (lctx, result);
        ...
        if (initialws)
        {
            ...
            {
                (*callbacks->error) (callbacks, "%s:%lu: no current owner name", source, line);
        ...
        if (token.type != isc_tokentype_string)
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_lex_gettoken() returned unexpected token type");
            result = ISC_R_UNEXPECTED;
            if (MANYERRS (lctx, result))
            {
                SETRESULT (lctx, result);
        ...
        if (result != ISC_R_SUCCESS)
        {
            (*callbacks->warn) (callbacks,
                                ...
                                source, line, token.value.as_textregion.length, token.value.as_textregion.base);
            if (MANYERRS (lctx, result))
            {
                SETRESULT (lctx, result);
                read_till_eol = ISC_TRUE;
        ...
        if (rdclass != 0 && rdclass != lctx->zclass)
        {
          bad_class:
            ...
            dns_rdataclass_format (lctx->zclass, classname2, sizeof (classname2));
            (*callbacks->error) (callbacks,
                                 "%s:%lu: class '%s' != " "zone class '%s'", source, line, classname1, classname2);
         ...
         */
        if ((lctx->options & DNS_MASTER_ZONE) != 0 &&
            (lctx->options & DNS_MASTER_SLAVE) == 0 && (type == dns_rdatatype_md || type == dns_rdatatype_mf))
        {
            ...
            (*callbacks->error) (callbacks,
                                 "%s:%lu: %s '%s': %s", source, line, "type", typename, dns_result_totext (result));
            if (MANYERRS (lctx, result))
            {
                SETRESULT (lctx, result);
            }
        ...
        {
            new_rdata = grow_rdata (rdata_size + RDSZ, rdata, rdata_size, &current_list, &glue_list, mctx);
            if (new_rdata == NULL)
            {
                result = ISC_R_NOMEMORY;
                goto log_and_cleanup;
         ...
         */
        if (type == dns_rdatatype_ns && lctx->zclass == dns_rdataclass_in && (lctx->options & DNS_MASTER_CHECKNS) != 0)
        {
        ...
        if ((lctx->options & DNS_MASTER_CHECKNAMES) != 0)
        {
            ...
            name = (ictx->glue != NULL) ? ictx->glue : ictx->current;
            ok = dns_rdata_checkowner (name, lctx->zclass, type, ISC_TRUE);
            if (!ok)
            {
                char namebuf[DNS_NAME_FORMATSIZE];
                ...
                if ((lctx->options & DNS_MASTER_CHECKNAMESFAIL) != 0)
                {
                    (*callbacks->error) (callbacks, "%s:%lu: %s: %s", source, line, namebuf, desc);
                    if (MANYERRS (lctx, result))
                    {
                        SETRESULT (lctx, result);
                    }
                ...
                {
                    (*callbacks->warn) (callbacks, "%s:%lu: %s: %s", source, line, namebuf, desc);
                }
            }
            if (type == dns_rdatatype_ptr &&
                (dns_name_issubdomain (name, &in_addr_arpa) ||
                 dns_name_issubdomain (name, &ip6_arpa) || dns_name_issubdomain (name, &ip6_int)))
                options |= DNS_RDATA_CHECKREVERSE;
        }
        ...
        result = dns_rdata_fromtext (&rdata[rdcount], lctx->zclass,
                                     type, lctx->lex, ictx->origin, options, lctx->mctx, &target, callbacks);
        if (MANYERRS (lctx, result))
        {
            SETRESULT (lctx, result);
            continue;
        ...
        if (type == dns_rdatatype_soa &&
            (lctx->options & DNS_MASTER_ZONE) != 0 && dns_name_compare (ictx->current, lctx->top) != 0)
        {
            char namebuf[DNS_NAME_FORMATSIZE];

            dns_name_format (ictx->current, namebuf, sizeof (namebuf));
            (*callbacks->error) (callbacks, "%s:%lu: SOA " "record not at top of zone (%s)", source, line, namebuf);
        ...
        if (!lctx->ttl_known && !lctx->default_ttl_known)
        {
            if (type == dns_rdatatype_soa)
            {
                (*callbacks->warn) (callbacks, "%s:%lu: no TTL specified; " "using SOA MINTTL instead", source, line);
            ...
            {
                (*callbacks->warn) (callbacks, "%s:%lu: no TTL specified; " "zone rejected", source, line);
                result = DNS_R_NOTTL;
                if (MANYERRS (lctx, result))
                {
                    SETRESULT (lctx, result);
        ...
        {
            (*callbacks->warn) (callbacks, "%s:%lu: " "using RFC1035 TTL semantics", source, line);
        ...
        if (type == dns_rdatatype_rrsig && lctx->warn_sigexpired)
        {
            ...
            result = dns_rdata_tostruct (&rdata[rdcount], &sig, NULL);
            RUNTIME_CHECK (result == ISC_R_SUCCESS);
            if (isc_serial_lt (sig.timeexpire, now))
            {
                (*callbacks->warn) (callbacks, "%s:%lu: " "signature has expired", source, line);
                lctx->warn_sigexpired = ISC_FALSE;
            }
        }

        if ((type == dns_rdatatype_sig || type == dns_rdatatype_nxt) &&
            lctx->warn_tcr && (lctx->options & DNS_MASTER_ZONE) != 0 && (lctx->options & DNS_MASTER_SLAVE) == 0)
        {
            (*callbacks->warn) (callbacks, "%s:%lu: old style DNSSEC " " zone detected", source, line);
        ...
        if (this == NULL)
        {
            ...
            {
                new_rdatalist =
                    grow_rdatalist (rdatalist_size + RDLSZ, rdatalist, rdatalist_size, &current_list, &glue_list, mctx);
            ...
            if (ictx->glue != NULL)
                ...
                ISC_LIST_INITANDPREPEND (current_list, this, link);
        }
        else if (this->ttl != lctx->ttl)
        {
            (*callbacks->warn) (callbacks, "%s:%lu: " "TTL set to prior TTL (%lu)", source, line, this->ttl);
            lctx->ttl = this->ttl;
        }
  ...
  cleanup:
    ...
    if (rdatalist != NULL)
        isc_mem_put (mctx, rdatalist, rdatalist_size * sizeof (*rdatalist));
    if (rdata != NULL)
...
static isc_result_t pushfile (const char *master_file, dns_name_t * origin, dns_loadctx_t * lctx)
{
    ...
    if (ictx->glue != NULL || ictx->current != NULL)
    {
        ...
        new->current = dns_fixedname_name (&new->fixed[new->current_in_use]);
        new->in_use[new->current_in_use] = ISC_TRUE;
        dns_name_toregion ((ictx->glue != NULL) ? ictx->glue : ictx->current, &r);
...
static inline isc_result_t read_and_check (isc_boolean_t do_read, isc_buffer_t * buffer, size_t len, FILE * f)
{
    isc_result_t result;
...
static isc_result_t load_raw (dns_loadctx_t * lctx)
{
    ...
    if (lctx->first)
    {
        ...
        size_t hdrlen = sizeof (format) + sizeof (version) + sizeof (dumptime);
        ...
        if (result != ISC_R_SUCCESS)
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_stdio_read failed: %s", isc_result_totext (result));
            return (result);
        }
        ...
        {
            (*callbacks->error) (callbacks, "dns_master_load: " "file format mismatch");
     ...
     */
    for (loop_cnt = 0; (lctx->loop_cnt == 0 || loop_cnt < lctx->loop_cnt); loop_cnt++)
    {
        ...
        result = isc_stdio_read (target.base, 1, sizeof (totallen), lctx->f, NULL);
        if (result == ISC_R_EOF)
        {
            result = ISC_R_SUCCESS;
            done = ISC_TRUE;
        ...
        minlen = sizeof (totallen) + sizeof (isc_uint16_t) +
            sizeof (isc_uint16_t) + sizeof (isc_uint16_t) + sizeof (isc_uint32_t) + sizeof (isc_uint32_t);
        if (totallen < minlen)
        {
            result = ISC_R_RANGE;
            goto cleanup;
        ...
        result = read_and_check (sequential_read, &target, sizeof (namelen), lctx->f);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
        ...
        result = read_and_check (sequential_read, &target, namelen, lctx->f);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
        ...
        if (rdcount > rdata_size)
        {
            ...
            new_rdata = grow_rdata (rdcount + RDSZ, rdata, rdata_size, &head, &dummy, mctx);
            if (new_rdata == NULL)
            {
                result = ISC_R_NOMEMORY;
                goto cleanup;
      ...
      continue_read:
        for (i = 0; i < rdcount; i++)
        {
            ...
            if (sequential_read && isc_buffer_availablelength (&target) < MINTSIZ)
            {
                unsigned int j;
                ...
                for (j = 0; j < i; j++)
                {
                    ISC_LIST_UNLINK (rdatalist.rdata, &rdata[j], link);
            ...
            /* rdata length */
            result = read_and_check (sequential_read, &target, sizeof (rdlen), lctx->f);
            if (result != ISC_R_SUCCESS)
                goto cleanup;
            rdlen = isc_buffer_getuint16 (&target);

            /* rdata */
            result = read_and_check (sequential_read, &target, rdlen, lctx->f);
            if (result != ISC_R_SUCCESS)
                goto cleanup;
            isc_buffer_setactive (&target, (unsigned int) rdlen);
            isc_buffer_activeregion (&target, &r);
            isc_buffer_forward (&target, (unsigned int) rdlen);
            dns_rdata_fromregion (&rdata[i], rdatalist.rdclass, rdatalist.type, &r);
  ...
  cleanup:
    ...
    {
        (*callbacks->error) (callbacks, "dns_master_load: %s", dns_result_totext (result));
    }
...
dns_master_loadfile (const char *master_file, dns_name_t * top,
                     dns_name_t * origin,
                     dns_rdataclass_t zclass, unsigned int options, dns_rdatacallbacks_t * callbacks, isc_mem_t * mctx)
{
...
dns_master_loadfile2 (const char *master_file, dns_name_t * top,
                      ...
                      dns_rdatacallbacks_t * callbacks, isc_mem_t * mctx, dns_masterformat_t format)
{
    return (dns_master_loadfile3 (master_file, top, origin, zclass, options, 0, callbacks, mctx, format));
}
                      ...
                      dns_rdatacallbacks_t * callbacks, isc_mem_t * mctx, dns_masterformat_t format)
{
    ...
    result = loadctx_create (format, mctx, options, resign, top, zclass,
                             origin, callbacks, NULL, NULL, NULL, NULL, &lctx);
    if (result != ISC_R_SUCCESS)
        return (result);
                        ...
                        void *done_arg, dns_loadctx_t ** lctxp, isc_mem_t * mctx)
{
    return (dns_master_loadfileinc3 (master_file, top, origin, zclass,
                                     options, 0, callbacks, task, done, done_arg, lctxp, mctx, dns_masterformat_text));
}
...
dns_master_loadfileinc2 (const char *master_file, dns_name_t * top,
                         ...
                         unsigned int options, dns_rdatacallbacks_t * callbacks,
                         isc_task_t * task, dns_loaddonefunc_t done,
                         void *done_arg, dns_loadctx_t ** lctxp, isc_mem_t * mctx, dns_masterformat_t format)
{
    return (dns_master_loadfileinc3 (master_file, top, origin, zclass,
                                     options, 0, callbacks, task, done, done_arg, lctxp, mctx, format));
}
...
dns_master_loadfileinc3 (const char *master_file, dns_name_t * top,
                         ...
                         dns_loadctx_t ** lctxp, isc_mem_t * mctx, dns_masterformat_t format)
{
    dns_loadctx_t *lctx = NULL;
    ...
    result = loadctx_create (format, mctx, options, resign, top, zclass,
                             origin, callbacks, task, done, done_arg, NULL, &lctx);
    if (result != ISC_R_SUCCESS)
        return (result);
                       ...
                       dns_rdatacallbacks_t * callbacks, isc_mem_t * mctx)
{
    ...
    result = loadctx_create (dns_masterformat_text, mctx, options, 0, top,
                             zclass, origin, callbacks, NULL, NULL, NULL, NULL, &lctx);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
...
dns_master_loadstreaminc (FILE * stream, dns_name_t * top, dns_name_t * origin,
                          dns_rdataclass_t zclass, unsigned int options,
                          dns_rdatacallbacks_t * callbacks, isc_task_t * task,
                          dns_loaddonefunc_t done, void *done_arg, dns_loadctx_t ** lctxp, isc_mem_t * mctx)
{
    isc_result_t result;
    ...
    result = loadctx_create (dns_masterformat_text, mctx, options, 0, top,
                             zclass, origin, callbacks, task, done, done_arg, NULL, &lctx);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
...
dns_master_loadbuffer (isc_buffer_t * buffer, dns_name_t * top,
                       dns_name_t * origin, dns_rdataclass_t zclass,
                       unsigned int options, dns_rdatacallbacks_t * callbacks, isc_mem_t * mctx)
{
    isc_result_t result;
                      ...
                      unsigned int options, dns_rdatacallbacks_t * callbacks, isc_mem_t * mctx)
{
    ...
    result = loadctx_create (dns_masterformat_text, mctx, options, 0, top,
                             zclass, origin, callbacks, NULL, NULL, NULL, lex, &lctx);
    if (result != ISC_R_SUCCESS)
        return (result);
                         ...
                         dns_loaddonefunc_t done, void *done_arg, dns_loadctx_t ** lctxp, isc_mem_t * mctx)
{
    ...
    result = loadctx_create (dns_masterformat_text, mctx, options, 0, top,
                             zclass, origin, callbacks, task, done, done_arg, lex, &lctx);
    if (result != ISC_R_SUCCESS)
        return (result);
 ...
 */
static dns_rdatalist_t *grow_rdatalist (int new_len, dns_rdatalist_t * old, int old_len,
 ...
 */
static dns_rdata_t *grow_rdata (int new_len, dns_rdata_t * old, int old_len,
                                rdatalist_head_t * current, rdatalist_head_t * glue, isc_mem_t * mctx)
{
    dns_rdata_t *new;
...
commit (dns_rdatacallbacks_t * callbacks, dns_loadctx_t * lctx,
        rdatalist_head_t * head, dns_name_t * owner, const char *source, unsigned int line)
{
    dns_rdatalist_t *this;
    ...
    do
    {
        ...
        RUNTIME_CHECK (dns_rdatalist_tordataset (this, &dataset) == ISC_R_SUCCESS);
        dataset.trust = dns_trust_ultimate;
        /*
         * If this is a secure dynamic zone set the re-signing time.
         */
        if (dataset.type == dns_rdatatype_rrsig && (lctx->options & DNS_MASTER_RESIGN) != 0)
        {
            dataset.attributes |= DNS_RDATASETATTR_RESIGN;
            dns_name_format (owner, namebuf, sizeof (namebuf));
            dataset.resign = resign_fromlist (this, lctx->resign);
        }
        result = ((*callbacks->add) (callbacks->add_private, owner, &dataset));
        if (result == ISC_R_NOMEMORY)
        {
            (*error) (callbacks, "dns_master_load: %s", dns_result_totext (result));
        }
        else if (result != ISC_R_SUCCESS)
        {
            dns_name_format (owner, namebuf, sizeof (namebuf));
            if (source != NULL)
            {
                (*error) (callbacks, "%s: %s:%lu: %s: %s",
                          "dns_master_load", source, line, namebuf, dns_result_totext (result));
            }
            else
            {
                (*error) (callbacks, "%s: %s: %s", "dns_master_load", namebuf, dns_result_totext (result));
            }
        }
        if (MANYERRS (lctx, result))
            SETRESULT (lctx, result);
...
static isc_boolean_t is_glue (rdatalist_head_t * head, dns_name_t * owner)
{
    dns_rdatalist_t *this;
...
static isc_result_t task_send (dns_loadctx_t * lctx)
{
    ...
    event = isc_event_allocate (lctx->mctx, NULL, DNS_EVENT_MASTERQUANTUM, load_quantum, lctx, sizeof (*event));
    if (event == NULL)
        return (ISC_R_NOMEMORY);
