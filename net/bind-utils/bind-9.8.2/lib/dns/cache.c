...
static isc_result_t
cache_cleaner_init (dns_cache_t * cache, isc_taskmgr_t * taskmgr, isc_timermgr_t * timermgr, cache_cleaner_t * cleaner);
...
static inline isc_result_t cache_create_db (dns_cache_t * cache, dns_db_t ** db)
{
    return (dns_db_create (cache->mctx, cache->db_type, dns_rootname,
                           dns_dbtype_cache, cache->rdclass, cache->db_argc, cache->db_argv, db));
}

isc_result_t
dns_cache_create (isc_mem_t * cmctx, isc_taskmgr_t * taskmgr,
                  isc_timermgr_t * timermgr, dns_rdataclass_t rdclass,
                  const char *db_type, unsigned int db_argc, char **db_argv, dns_cache_t ** cachep)
{
    return (dns_cache_create3 (cmctx, cmctx, taskmgr, timermgr, rdclass, "", db_type, db_argc, db_argv, cachep));
}
...
{
    return (dns_cache_create3 (cmctx, cmctx, taskmgr, timermgr, rdclass, cachename, db_type, db_argc, db_argv, cachep));
}
                   ...
                   unsigned int db_argc, char **db_argv, dns_cache_t ** cachep)
{
    ...
    if (cache->db_argc != 0)
    {
        cache->db_argv = isc_mem_get (cmctx, cache->db_argc * sizeof (char *));
        ...
        {
            cache->db_argv[i] = isc_mem_strdup (cmctx, db_argv[i - extra]);
  ...
  cleanup_dbargv:
    ...
    if (cache->db_argv != NULL)
        isc_mem_put (cmctx, cache->db_argv, cache->db_argc * sizeof (char *));
  cleanup_dbtype:
...
static void cache_free (dns_cache_t * cache)
{
    ...
    if (cache->db_argv != NULL)
    {
        ...
        isc_mem_put (cache->mctx, cache->db_argv, cache->db_argc * sizeof (char *));
    }
    ...
    if (free_cache)
    {
        ...
        if (result != ISC_R_SUCCESS)
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           ...
                           "error dumping cache: %s ", isc_result_totext (result));
...
#ifdef BIND9
    LOCK (&cache->filelock);
    result = dns_master_dump (cache->mctx, cache->db, NULL, &dns_master_style_cache, cache->filename);
    UNLOCK (&cache->filelock);
    return (result);
#else
...
void dns_cache_setcleaninginterval (dns_cache_t * cache, unsigned int t)
{
    ...
    if (t == 0)
    {
        result = isc_timer_reset (cache->cleaner.cleaning_timer, isc_timertype_inactive, NULL, NULL, ISC_TRUE);
    }
    else
    {
        isc_interval_set (&interval, cache->cleaner.cleaning_interval, 0);
        result = isc_timer_reset (cache->cleaner.cleaning_timer, isc_timertype_ticker, NULL, &interval, ISC_FALSE);
    }
    if (result != ISC_R_SUCCESS)
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_CACHE, ISC_LOG_WARNING,
                       "could not set cache cleaning interval: %s", isc_result_totext (result));
...
cache_cleaner_init (dns_cache_t * cache, isc_taskmgr_t * taskmgr, isc_timermgr_t * timermgr, cache_cleaner_t * cleaner)
{
    ...
    result = dns_db_createiterator (cleaner->cache->db, ISC_FALSE, &cleaner->iterator);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    ...
    if (taskmgr != NULL && timermgr != NULL)
    {
        ...
        result = isc_task_onshutdown (cleaner->task, cleaner_shutdown_action, cache);
        if (result != ISC_R_SUCCESS)
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__,
                              "cache cleaner: " "isc_task_onshutdown() failed: %s", dns_result_totext (result));
            goto cleanup;
        }

        result = isc_timer_create (timermgr, isc_timertype_inactive,
                                   NULL, NULL, cleaner->task, cleaning_timer_action, cleaner, &cleaner->cleaning_timer);
        if (result != ISC_R_SUCCESS)
        {
        ...
        cleaner->resched_event =
            isc_event_allocate (cache->mctx, cleaner,
                                DNS_EVENT_CACHECLEAN, incremental_cleaning_action, cleaner, sizeof (isc_event_t));
        ...
        cleaner->overmem_event =
            isc_event_allocate (cache->mctx, cleaner,
                                DNS_EVENT_CACHEOVERMEM, overmem_cleaning_action, cleaner, sizeof (isc_event_t));
...
static void begin_cleaning (cache_cleaner_t * cleaner)
{
    ...
    if (result != ISC_R_SUCCESS)
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_CACHE, ISC_LOG_WARNING,
                       "cache cleaner could not create " "iterator: %s", isc_result_totext (result));
    else
    {
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__,
                              "cache cleaner: " "dns_dbiterator_first() failed: %s", dns_result_totext (result));
            dns_dbiterator_destroy (&cleaner->iterator);
        }
    ...
    else
    {
        ...
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       ...
                       "begin cache cleaning, mem inuse %lu", (unsigned long) isc_mem_inuse (cleaner->cache->mctx));
...
static void end_cleaning (cache_cleaner_t * cleaner, isc_event_t * event)
{
    ...
    dns_cache_setcleaninginterval (cleaner->cache, cleaner->cleaning_interval);
...
static void cleaning_timer_action (isc_task_t * task, isc_event_t * event)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE, DNS_LOGMODULE_CACHE,
                   ISC_LOG_DEBUG (1), "cache cleaning timer fired, " "cleaner state = %d", cleaner->state);
...
static void incremental_cleaning_action (isc_task_t * task, isc_event_t * event)
{
    ...
    if (cleaner->state == cleaner_s_done)
    {
        ...
        {
            dns_dbiterator_destroy (&cleaner->iterator);
            (void) dns_db_createiterator (cleaner->cache->db, ISC_FALSE, &cleaner->iterator);
            cleaner->replaceiterator = ISC_FALSE;
        }
        UNLOCK (&cleaner->lock);
    ...
    while (n_names-- > 0)
    {
        ...
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__,
                              "cache cleaner: dns_dbiterator_current() " "failed: %s", dns_result_totext (result));
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            if (result != ISC_R_NOMORE)
                UNEXPECTED_ERROR (__FILE__, __LINE__,
                                  "cache cleaner: " "dns_dbiterator_next() " "failed: %s", dns_result_totext (result));
            else if (cleaner->overmem)
            {
                ...
                if (result == ISC_R_SUCCESS)
                {
                    isc_log_write (dns_lctx,
                                   ...
                                   DNS_LOGMODULE_CACHE,
                                   ISC_LOG_DEBUG (1), "cache cleaner: " "still overmem, " "reset and try again");
                    continue;
                }
            }
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE, DNS_LOGMODULE_CACHE,
                   ...
                   "mem inuse %lu, sleeping", cleaner->increment, (unsigned long) isc_mem_inuse (cleaner->cache->mctx));
...
isc_result_t dns_cache_clean (dns_cache_t * cache, isc_stdtime_t now)
{
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        result = dns_dbiterator_current (iterator, &node, (dns_name_t *) NULL);
        if (result != ISC_R_SUCCESS)
            break;
        ...
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__,
                              "cache cleaner: dns_db_expirenode() " "failed: %s", dns_result_totext (result));
            /*
             * Continue anyway.
             */
        }
...
static void water (void *arg, int mark)
{
    ...
    if (cache->cleaner.overmem_event != NULL)
        isc_task_send (cache->cleaner.task, &cache->cleaner.overmem_event);

    UNLOCK (&cache->cleaner.lock);
}

void dns_cache_setcachesize (dns_cache_t * cache, isc_uint32_t size)
{
...
isc_uint32_t dns_cache_getcachesize (dns_cache_t * cache)
{
    isc_uint32_t size;
...
isc_result_t dns_cache_flush (dns_cache_t * cache)
{
    ...
    if (cache->cleaner.state == cleaner_s_idle)
    {
        if (cache->cleaner.iterator != NULL)
            dns_dbiterator_destroy (&cache->cleaner.iterator);
        (void) dns_db_createiterator (db, ISC_FALSE, &cache->cleaner.iterator);
    }
    else
    {
...
isc_result_t dns_cache_flushname (dns_cache_t * cache, dns_name_t * name)
{
    ...
    result = dns_db_allrdatasets (cache->db, node, NULL, (isc_stdtime_t) 0, &iter);
    if (result != ISC_R_SUCCESS)
        goto cleanup_node;
    ...
    for (result = dns_rdatasetiter_first (iter); result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (iter))
    {
        ...
        result = dns_db_deleterdataset (cache->db, node, NULL, rdataset.type, rdataset.covers);
