...
static inline dns_dbimplementation_t *impfind (const char *name)
{
...
dns_db_create (isc_mem_t * mctx, const char *db_type, dns_name_t * origin,
               dns_dbtype_t type, dns_rdataclass_t rdclass, unsigned int argc, char *argv[], dns_db_t ** dbp)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   DNS_LOGMODULE_DB, ISC_LOG_ERROR, "unsupported database type '%s'", db_type);
...
#ifdef BIND9
isc_result_t dns_db_beginload (dns_db_t * db, dns_addrdatasetfunc_t * addp, dns_dbload_t ** dbloadp)
{
    /*
     * Begin loading 'db'.
     */
...
isc_result_t dns_db_endload (dns_db_t * db, dns_dbload_t ** dbloadp)
{
    /*
     * Finish loading 'db'.
     */
...
isc_result_t dns_db_load3 (dns_db_t * db, const char *filename, dns_masterformat_t format, unsigned int options)
{
    ...
    result = dns_master_loadfile2 (filename, &db->origin, &db->origin,
                                   db->rdclass, options, &callbacks, db->mctx, format);
     ...
     */
    if (eresult != ISC_R_SUCCESS && (result == ISC_R_SUCCESS || result == DNS_R_SEENINCLUDE))
        result = eresult;
...
{
    return ((db->methods->dump) (db, version, filename, dns_masterformat_text));
}

isc_result_t
dns_db_dump2 (dns_db_t * db, dns_dbversion_t * version, const char *filename, dns_masterformat_t masterformat)
{
    /*
             ...
             dns_dbnode_t ** nodep, dns_name_t * foundname, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    ...
    REQUIRE (rdataset == NULL || (DNS_RDATASET_VALID (rdataset) && !dns_rdataset_isassociated (rdataset)));
    REQUIRE (sigrdataset == NULL || (DNS_RDATASET_VALID (sigrdataset) && !dns_rdataset_isassociated (sigrdataset)));

    return ((db->methods->find) (db, name, version, type, options, now, nodep, foundname, rdataset, sigrdataset));
}
                    ...
                    dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    ...
    return ((db->methods->findzonecut) (db, name, options, now, nodep, foundname, rdataset, sigrdataset));
}
...
isc_result_t dns_db_createiterator (dns_db_t * db, unsigned int flags, dns_dbiterator_t ** iteratorp)
{
    /*
                     ...
                     isc_stdtime_t now, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    ...
    return ((db->methods->findrdataset) (db, node, version, type, covers, now, rdataset, sigrdataset));
}
                    ...
                    isc_stdtime_t now, dns_rdataset_t * rdataset, unsigned int options, dns_rdataset_t * addedrdataset)
{
    ...
    REQUIRE (((db->attributes & DNS_DBATTR_CACHE) == 0 && version != NULL) ||
             ((db->attributes & DNS_DBATTR_CACHE) != 0 && version == NULL && (options & DNS_DBADD_MERGE) == 0));
    REQUIRE ((options & DNS_DBADD_EXACT) == 0 || (options & DNS_DBADD_MERGE) != 0);
    REQUIRE (DNS_RDATASET_VALID (rdataset));
    REQUIRE (dns_rdataset_isassociated (rdataset));
    REQUIRE (rdataset->rdclass == db->rdclass);
    REQUIRE (addedrdataset == NULL ||
             (DNS_RDATASET_VALID (addedrdataset) && !dns_rdataset_isassociated (addedrdataset)));

    return ((db->methods->addrdataset) (db, node, version, now, rdataset, options, addedrdataset));
}
                         ...
                         unsigned int options, dns_rdataset_t * newrdataset)
{
    ...
    REQUIRE (dns_rdataset_isassociated (rdataset));
    REQUIRE (rdataset->rdclass == db->rdclass);
    REQUIRE (newrdataset == NULL || (DNS_RDATASET_VALID (newrdataset) && !dns_rdataset_isassociated (newrdataset)));

    return ((db->methods->subtractrdataset) (db, node, version, rdataset, options, newrdataset));
}

isc_result_t
dns_db_deleterdataset (dns_db_t * db, dns_dbnode_t * node,
                       dns_dbversion_t * version, dns_rdatatype_t type, dns_rdatatype_t covers)
{
    /*
...
dns_db_getnsec3parameters (dns_db_t * db, dns_dbversion_t * version,
                           ...
                           isc_uint16_t * iterations, unsigned char *salt, size_t * salt_length)
{
    REQUIRE (DNS_DB_VALID (db));
    REQUIRE (dns_db_iszone (db) == ISC_TRUE);

    if (db->methods->getnsec3parameters != NULL)
        return ((db->methods->getnsec3parameters) (db, version, hash, flags, iterations, salt, salt_length));
...
void dns_db_rpz_enabled (dns_db_t * db, dns_rpz_st_t * st)
{
                    ...
                    dns_rdataset_t * ardataset, dns_rpz_st_t * st, dns_name_t * query_qname)
{
    if (db->methods->rpz_findips == NULL)
        return (ISC_R_NOTIMPLEMENTED);
    return ((db->methods->rpz_findips) (rpz, rpz_type, zone, db, version, ardataset, st, query_qname));
}
