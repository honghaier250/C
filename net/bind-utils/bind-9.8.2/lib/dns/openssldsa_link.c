...
static isc_result_t openssldsa_createctx (dst_key_t * key, dst_context_t * dctx)
{
#if USE_EVP
...
static void openssldsa_destroyctx (dst_context_t * dctx)
{
#if USE_EVP
...
static isc_result_t openssldsa_adddata (dst_context_t * dctx, const isc_region_t * data)
{
#if USE_EVP
    EVP_MD_CTX *evp_md_ctx = dctx->ctxdata.evp_md_ctx;
...
static isc_result_t openssldsa_sign (dst_context_t * dctx, isc_buffer_t * sig)
{
...
static isc_result_t openssldsa_verify (dst_context_t * dctx, const isc_region_t * sig)
{
    dst_key_t *key = dctx->key;
...
static isc_boolean_t openssldsa_compare (const dst_key_t * key1, const dst_key_t * key2)
{
    int status;
    ...
    status = BN_cmp (dsa1->p, dsa2->p) ||
        BN_cmp (dsa1->q, dsa2->q) || BN_cmp (dsa1->g, dsa2->g) || BN_cmp (dsa1->pub_key, dsa2->pub_key);
...
static isc_result_t openssldsa_generate (dst_key_t * key, int unused, void (*callback) (int))
{
    DSA *dsa;
    ...
    result = dst__entropy_getdata (rand_array, sizeof (rand_array), ISC_FALSE);
    ...
    if (!DSA_generate_parameters_ex (dsa, key->key_size, rand_array, ISC_SHA1_DIGESTLENGTH, NULL, NULL, &cb))
    {
        DSA_free (dsa);
        return (dst__openssl_toresult (DST_R_OPENSSLFAILURE));
    }
#else
    dsa = DSA_generate_parameters (key->key_size, rand_array, ISC_SHA1_DIGESTLENGTH, NULL, NULL, NULL, NULL);
    if (dsa == NULL)
        return (dst__openssl_toresult (DST_R_OPENSSLFAILURE));
#endif
...
static isc_boolean_t openssldsa_isprivate (const dst_key_t * key)
{
    DSA *dsa = key->keydata.dsa;
...
static void openssldsa_destroy (dst_key_t * key)
{
    DSA *dsa = key->keydata.dsa;
...
static isc_result_t openssldsa_fromdns (dst_key_t * key, isc_buffer_t * data)
{
    DSA *dsa;
...
static isc_result_t openssldsa_tofile (const dst_key_t * key, const char *directory)
{
    int cnt = 0;
...
static isc_result_t openssldsa_parse (dst_key_t * key, isc_lex_t * lexer, dst_key_t * pub)
{
    dst_private_t priv;
...
isc_result_t dst__openssldsa_init (dst_func_t ** funcp)
{
    REQUIRE (funcp != NULL);
    if (*funcp == NULL)
