...
static void list_tordataset (dns_rdatalist_t * rdatalist,
                             dns_db_t * db, dns_dbnode_t * node, dns_rdataset_t * rdataset);
...
static void sdlz_log (int level, const char *fmt, ...)
{
    ...
    va_start (ap, fmt);
    isc_log_vwrite (dns_lctx, DNS_LOGCATEGORY_DATABASE, DNS_LOGMODULE_DLZ, ISC_LOG_DEBUG (level), fmt, ap);
    va_end (ap);
}
...
static inline unsigned int initial_size (const char *data)
{
...
static void rdatasetiter_destroy (dns_rdatasetiter_t ** iteratorp)
{
    ...
    isc_mem_put (sdlziterator->common.db->mctx, sdlziterator, sizeof (sdlz_rdatasetiter_t));
    *iteratorp = NULL;
}
...
static void rdatasetiter_current (dns_rdatasetiter_t * iterator, dns_rdataset_t * rdataset)
{
    ...
    list_tordataset (sdlziterator->current, iterator->db, iterator->node, rdataset);
}
...
static isc_result_t newversion (dns_db_t * db, dns_dbversion_t ** versionp)
{
    ...
    result = sdlz->dlzimp->methods->newversion (origin, sdlz->dlzimp->driverarg, sdlz->dbdata, versionp);
    if (result != ISC_R_SUCCESS)
    {
        sdlz_log (ISC_LOG_ERROR, "sdlz newversion on origin %s failed : %s", origin, isc_result_totext (result));
        return (result);
    }
...
static void closeversion (dns_db_t * db, dns_dbversion_t ** versionp, isc_boolean_t commit)
{
    ...
    sdlz->dlzimp->methods->closeversion (origin, commit, sdlz->dlzimp->driverarg, sdlz->dbdata, versionp);
    if (*versionp != NULL)
        sdlz_log (ISC_LOG_ERROR, "sdlz closeversion on origin %s failed", origin);
...
static isc_result_t createnode (dns_sdlz_db_t * sdlz, dns_sdlznode_t ** nodep)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_mutex_init() failed: %s", isc_result_totext (result));
...
static isc_result_t findnode (dns_db_t * db, dns_name_t * name, isc_boolean_t create, dns_dbnode_t ** nodep)
{
    ...
    result = sdlz->dlzimp->methods->lookup (zonestr, namestr, sdlz->dlzimp->driverarg, sdlz->dbdata, node);
    ...
    {
        result = sdlz->dlzimp->methods->lookup (zonestr, "*", sdlz->dlzimp->driverarg, sdlz->dbdata, node);
    }
    ...
    if (isorigin && sdlz->dlzimp->methods->authority != NULL)
    {
        ...
        result = (*authority) (zonestr, sdlz->dlzimp->driverarg, sdlz->dbdata, node);
        MAYBE_UNLOCK (sdlz->dlzimp);
    ...
    if (node->name == NULL)
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            isc_mem_put (sdlz->common.mctx, node->name, sizeof (dns_name_t));
            destroynode (node);
            return (result);
        }
    }
...
static isc_result_t createiterator (dns_db_t * db, unsigned int options, dns_dbiterator_t ** iteratorp)
{
    ...
    if ((options & DNS_DB_NSEC3ONLY) != 0 || (options & DNS_DB_NONSEC3) != 0)
        return (ISC_R_NOTIMPLEMENTED);
    ...
    MAYBE_LOCK (sdlz->dlzimp);
    result = sdlz->dlzimp->methods->allnodes (zonestr, sdlz->dlzimp->driverarg, sdlz->dbdata, sdlziter);
    MAYBE_UNLOCK (sdlz->dlzimp);
    if (result != ISC_R_SUCCESS)
    {
      ...
      dns_dbnode_t ** nodep, dns_name_t * foundname, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    ...
    for (i = olabels; i <= nlabels; i++)
    {
        ...
        if (i < nlabels)
        {
            result = findrdataset (db, node, version, dns_rdatatype_dname, 0, now, rdataset, sigrdataset);
            if (result == ISC_R_SUCCESS)
            {
                result = DNS_R_DNAME;
                break;
        ...
        {
            result = findrdataset (db, node, version, dns_rdatatype_ns, 0, now, rdataset, sigrdataset);
            if (result == ISC_R_SUCCESS)
            {
         ...
         * Look for the qtype.
         */
        result = findrdataset (db, node, version, type, 0, now, rdataset, sigrdataset);
        if (result == ISC_R_SUCCESS)
            break;
        ...
        {
            result = findrdataset (db, node, version, dns_rdatatype_cname, 0, now, rdataset, sigrdataset);
            if (result == ISC_R_SUCCESS)
            {
                result = DNS_R_CNAME;
                break;
              ...
              isc_stdtime_t now, dns_rdatasetiter_t ** iteratorp)
{
    ...
    REQUIRE (version == NULL || version == (void *) &sdlz->dummy_version || version == sdlz->future_version);
...
modrdataset (dns_db_t * db, dns_dbnode_t * node, dns_dbversion_t * version,
             dns_rdataset_t * rdataset, unsigned int options, dns_sdlzmodrdataset_t mod_function)
{
    dns_sdlz_db_t *sdlz = (dns_sdlz_db_t *) db;
    ...
    result = dns_master_rdatasettotext (sdlznode->name, rdataset, style, buffer);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    ...
    MAYBE_LOCK (sdlz->dlzimp);
    result = mod_function (name, rdatastr, sdlz->dlzimp->driverarg, sdlz->dbdata, version);
    MAYBE_UNLOCK (sdlz->dlzimp);
             ...
             isc_stdtime_t now, dns_rdataset_t * rdataset, unsigned int options, dns_rdataset_t * addedrdataset)
{
    ...
    result = modrdataset (db, node, version, rdataset, options, sdlz->dlzimp->methods->addrdataset);
    return (result);
}
                  ...
                  dns_rdataset_t * rdataset, unsigned int options, dns_rdataset_t * newrdataset)
{
    ...
    result = modrdataset (db, node, version, rdataset, options, sdlz->dlzimp->methods->subtractrdataset);
    return (result);
}
                ...
                dns_rdatatype_t type, dns_rdatatype_t covers)
{
    ...
    MAYBE_LOCK (sdlz->dlzimp);
    result = sdlz->dlzimp->methods->delrdataset (name, b_type, sdlz->dlzimp->driverarg, sdlz->dbdata, version);
    MAYBE_UNLOCK (sdlz->dlzimp);
...
static isc_result_t getoriginnode (dns_db_t * db, dns_dbnode_t ** nodep)
{
    ...
    if (result != ISC_R_SUCCESS)
        sdlz_log (ISC_LOG_ERROR, "sdlz getoriginnode failed : %s", isc_result_totext (result));
    return (result);
}
...
dns_sdlzallowzonexfr (void *driverarg, void *dbdata, isc_mem_t * mctx,
                      dns_rdataclass_t rdclass, dns_name_t * name, isc_sockaddr_t * clientaddr, dns_db_t ** dbp)
{
    isc_buffer_t b;
...
static isc_result_t
dns_sdlzcreate (isc_mem_t * mctx, const char *dlzname, unsigned int argc, char *argv[], void *driverarg, void **dbdata)
{
    dns_sdlzimplementation_t *imp;
...
static isc_result_t dns_sdlzconfigure (void *driverarg, void *dbdata, dns_view_t * view)
{
    isc_result_t result;
...
dns_sdlzssumatch (dns_name_t * signer, dns_name_t * name, isc_netaddr_t * tcpaddr,
                  dns_rdatatype_t type, const dst_key_t * key, void *driverarg, void *dbdata)
{
    dns_sdlzimplementation_t *imp;
    ...
    ret = imp->methods->ssumatch (b_signer, b_name, b_addr, b_type, b_key,
                                  token_len, token_len != 0 ? token_region.base : NULL, imp->driverarg, dbdata);
    MAYBE_UNLOCK (imp);
    return (ret);
}
...
isc_result_t dns_sdlz_putrr (dns_sdlzlookup_t * lookup, const char *type, dns_ttl_t ttl, const char *data)
{
    ...
    do
    {
        ...
        result = dns_rdata_fromtext (rdata, rdatalist->rdclass,
                                     rdatalist->type, lex, origin, ISC_FALSE, mctx, rdatabuf, &lookup->callbacks);
        if (result != ISC_R_SUCCESS)
...
isc_result_t
dns_sdlz_putnamedrr (dns_sdlzallnodes_t * allnodes, const char *name, const char *type, dns_ttl_t ttl, const char *data)
{
    ...
    if (sdlznode == NULL || !dns_name_equal (sdlznode->name, newname))
    {
        ...
        if (allnodes->origin == NULL && dns_name_equal (newname, &sdlz->common.origin))
...
isc_result_t dns_sdlz_putsoa (dns_sdlzlookup_t * lookup, const char *mname, const char *rname, isc_uint32_t serial)
{
    ...
    n = snprintf (str, sizeof str, "%s %s %u %u %u %u %u",
                  mname, rname, serial,
                  SDLZ_DEFAULT_REFRESH, SDLZ_DEFAULT_RETRY, SDLZ_DEFAULT_EXPIRE, SDLZ_DEFAULT_MINIMUM);
...
dns_sdlzregister (const char *drivername, const dns_sdlzmethods_t * methods,
                  void *driverarg, unsigned int flags, isc_mem_t * mctx, dns_sdlzimplementation_t ** sdlzimp)
{
    ...
    REQUIRE ((flags & ~(DNS_SDLZFLAG_RELATIVEOWNER | DNS_SDLZFLAG_RELATIVERDATA | DNS_SDLZFLAG_THREADSAFE)) == 0);
...
isc_result_t dns_sdlz_setdb (dns_dlzdb_t * dlzdatabase, dns_rdataclass_t rdclass, dns_name_t * name, dns_db_t ** dbp)
{
    ...
    result = dns_sdlzcreateDBP (dlzdatabase->mctx,
                                dlzdatabase->implementation->driverarg, dlzdatabase->dbdata, name, rdclass, dbp);
    return (result);
}
