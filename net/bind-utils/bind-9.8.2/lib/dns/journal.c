...
static inline isc_uint32_t decode_uint32 (unsigned char *p)
{
    return ((p[0] << 24) + (p[1] << 16) + (p[2] << 8) + (p[3] << 0));
}

static inline void encode_uint32 (isc_uint32_t val, unsigned char *p)
{
...
isc_result_t
dns_db_createsoatuple (dns_db_t * db, dns_dbversion_t * ver, isc_mem_t * mctx, dns_diffop_t op, dns_difftuple_t ** tp)
{
    isc_result_t result;
    ...
    result = dns_db_findrdataset (db, node, ver, dns_rdatatype_soa, 0, (isc_stdtime_t) 0, &rdataset, NULL);
    if (result != ISC_R_SUCCESS)
        goto freenode;
    ...
    result = dns_difftuple_create (mctx, op, zonename, rdataset.ttl, &rdata, tp);
...
static isc_result_t journal_seek (dns_journal_t * j, isc_uint32_t offset)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR, "%s: seek: %s", j->filename, isc_result_totext (result));
...
static isc_result_t journal_read (dns_journal_t * j, void *mem, size_t nbytes)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        if (result == ISC_R_EOF)
            return (ISC_R_NOMORE);
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR, "%s: read: %s", j->filename, isc_result_totext (result));
        return (ISC_R_UNEXPECTED);
    }
    j->offset += nbytes;
    return (ISC_R_SUCCESS);
...
static isc_result_t journal_write (dns_journal_t * j, void *mem, size_t nbytes)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR, "%s: write: %s", j->filename, isc_result_totext (result));
        return (ISC_R_UNEXPECTED);
    }
    j->offset += nbytes;
    return (ISC_R_SUCCESS);
...
static isc_result_t journal_fsync (dns_journal_t * j)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR, "%s: flush: %s", j->filename, isc_result_totext (result));
        return (ISC_R_UNEXPECTED);
    }
    result = isc_stdio_sync (j->fp);
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR, "%s: fsync: %s", j->filename, isc_result_totext (result));
        return (ISC_R_UNEXPECTED);
    }
    return (ISC_R_SUCCESS);
}
...
static isc_result_t
journal_write_xhdr (dns_journal_t * j, isc_uint32_t size, isc_uint32_t serial0, isc_uint32_t serial1)
{
    journal_rawxhdr_t raw;
...
static isc_result_t journal_file_create (isc_mem_t * mctx, const char *filename)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR, "%s: create: %s", filename, isc_result_totext (result));
        return (ISC_R_UNEXPECTED);
    }
    ...
    size = sizeof (journal_rawheader_t) + index_size * sizeof (journal_rawpos_t);
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR, "%s: write: %s", filename, isc_result_totext (result));
        (void) isc_stdio_close (fp);
        (void) isc_file_remove (filename);
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR, "%s: close: %s", filename, isc_result_totext (result));
        (void) isc_file_remove (filename);
        return (ISC_R_UNEXPECTED);
    }
              ...
              isc_boolean_t create, dns_journal_t ** journalp)
{
    ...
    if (result == ISC_R_FILENOTFOUND)
    {
        if (create)
        {
            isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_DEBUG (1),
                           "journal file %s does not exist, " "creating it", j->filename);
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR, "%s: open: %s", j->filename, isc_result_totext (result));
    ...
    if (memcmp (rawheader.h.format, initial_journal_header.format, sizeof (initial_journal_header.format)) != 0)
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR, "%s: journal format not recognized", j->filename);
        FAIL (ISC_R_UNEXPECTED);
    }
    ...
    if (j->header.index_size != 0)
    {
        ...
        j->index = isc_mem_get (mctx, j->header.index_size * sizeof (journal_pos_t));
        if (j->index == NULL)
            FAIL (ISC_R_NOMEMORY);
    ...
    j->state = write ? JOURNAL_STATE_WRITE : JOURNAL_STATE_READ;
  ...
  failure:
    ...
    if (j->index != NULL)
    {
        isc_mem_put (j->mctx, j->index, j->header.index_size * sizeof (journal_rawpos_t));
        j->index = NULL;
    }
    if (j->fp != NULL)
...
isc_result_t dns_journal_open (isc_mem_t * mctx, const char *filename, isc_boolean_t write, dns_journal_t ** journalp)
{
    isc_result_t result;
    ...
    if (result == ISC_R_NOTFOUND)
    {
        ...
        result = isc_string_printf (backup, sizeof (backup), "%.*s.jbk", namelen, filename);
        if (result != ISC_R_SUCCESS)
            return (result);
...
static int ixfr_order (const void *av, const void *bv)
{
    ...
    r = (b->rdata.type == dns_rdatatype_soa) - (a->rdata.type == dns_rdatatype_soa);
    if (r != 0)
        return (r);
...
static isc_result_t journal_next (dns_journal_t * j, journal_pos_t * pos)
{
    ...
    if (xhdr.serial0 != pos->serial)
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR,
                       ...
                       "expected serial %u, got %u", j->filename, pos->serial, xhdr.serial0);
        return (ISC_R_UNEXPECTED);
    }
     ...
     */
    if ((isc_offset_t) (pos->offset + sizeof (journal_rawxhdr_t) + xhdr.size) < pos->offset)
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR, "%s: offset too large", j->filename);
        return (ISC_R_UNEXPECTED);
    }
...
static void index_find (dns_journal_t * j, isc_uint32_t serial, journal_pos_t * best_guess)
{
    ...
    for (i = 0; i < j->header.index_size; i++)
    {
        if (POS_VALID (j->index[i]) &&
            DNS_SERIAL_GE (serial, j->index[i].serial) && DNS_SERIAL_GT (j->index[i].serial, best_guess->serial))
...
isc_result_t dns_journal_begin_transaction (dns_journal_t * j)
{
    ...
    {
        offset = sizeof (journal_rawheader_t) + j->header.index_size * sizeof (journal_rawpos_t);
    }
    else
    {
...
isc_result_t dns_journal_writediff (dns_journal_t * j, dns_diff_t * diff)
{
    ...
    for (t = ISC_LIST_HEAD (diff->tuples); t != NULL; t = ISC_LIST_NEXT (t, link))
    {
        ...
        {
            if (j->x.n_soa < 2)
                j->x.pos[j->x.n_soa].serial = dns_soa_getserial (&t->rdata);
            j->x.n_soa++;
        }
    ...
    for (t = ISC_LIST_HEAD (diff->tuples); t != NULL; t = ISC_LIST_NEXT (t, link))
    {
         ...
         * Write the RR header.
         */
        isc_buffer_putuint32 (&buffer, t->name.length + 10 + t->rdata.length);
        /*
...
isc_result_t dns_journal_commit (dns_journal_t * j)
{
    ...
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR,
                       "%s: malformed transaction: %d SOAs", j->filename, j->x.n_soa);
        return (ISC_R_UNEXPECTED);
    }
    if (!(DNS_SERIAL_GT (j->x.pos[1].serial, j->x.pos[0].serial) ||
          (bind8_compat && j->x.pos[1].serial == j->x.pos[0].serial)))
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR,
                       "%s: malformed transaction: serial number " "would decrease", j->filename);
        return (ISC_R_UNEXPECTED);
    }
    if (!JOURNAL_EMPTY (&j->header))
    {
        if (j->x.pos[0].serial != j->header.end.serial)
        {
            isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR,
                           ...
                           "%s last serial %u != "
                           "transaction first serial %u", j->filename, j->header.end.serial, j->x.pos[0].serial);
            return (ISC_R_UNEXPECTED);
        }
    }
    ...
    if (!JOURNAL_EMPTY (&j->header))
    {
        while (!DNS_SERIAL_GT (j->x.pos[1].serial, j->header.begin.serial))
        {
    ...
    CHECK (journal_write_xhdr (j, (j->x.pos[1].offset - j->x.pos[0].offset) -
                               sizeof (journal_rawxhdr_t), j->x.pos[0].serial, j->x.pos[1].serial));
...
void dns_journal_destroy (dns_journal_t ** journalp)
{
    ...
    if (j->rawindex != NULL)
        isc_mem_put (j->mctx, j->rawindex, j->header.index_size * sizeof (journal_rawpos_t));
    if (j->index != NULL)
        isc_mem_put (j->mctx, j->index, j->header.index_size * sizeof (journal_pos_t));
...
static isc_result_t roll_forward (dns_journal_t * j, dns_db_t * db, unsigned int options, isc_uint32_t resign)
{
    ...
    for (result = dns_journal_first_rr (j); result == ISC_R_SUCCESS; result = dns_journal_next_rr (j))
    {
        dns_name_t *name;
        ...
        {
            isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR,
                           "%s: journal file corrupt: missing " "initial SOA", j->filename);
            FAIL (ISC_R_UNEXPECTED);
        }
        if ((options & DNS_JOURNALOPT_RESIGN) != 0)
            op = (n_soa == 1) ? DNS_DIFFOP_DELRESIGN : DNS_DIFFOP_ADDRESIGN;
        else
            op = (n_soa == 1) ? DNS_DIFFOP_DEL : DNS_DIFFOP_ADD;

        CHECK (dns_difftuple_create (diff.mctx, op, name, ttl, rdata, &tuple));
        dns_diff_append (&diff, &tuple);

        if (++n_put > 100)
        {
            isc_log_write (JOURNAL_DEBUG_LOGARGS (3), "%s: applying diff to database (%u)", j->filename, db_serial);
            (void) dns_diff_print (&diff, NULL);
    ...
    if (n_put != 0)
    {
        isc_log_write (JOURNAL_DEBUG_LOGARGS (3), "%s: applying final diff to database (%u)", j->filename, db_serial);
        (void) dns_diff_print (&diff, NULL);
        CHECK (dns_diff_apply (&diff, db, ver));
        dns_diff_clear (&diff);
    }
  ...
  failure:
    if (ver != NULL)
        dns_db_closeversion (db, &ver, result == ISC_R_SUCCESS ? ISC_TRUE : ISC_FALSE);
                          ...
                          isc_uint32_t resign, const char *filename)
{
    ...
    if (result == ISC_R_NOTFOUND)
    {
        isc_log_write (JOURNAL_DEBUG_LOGARGS (3), "no journal file, but that's OK");
...
isc_result_t dns_journal_print (isc_mem_t * mctx, const char *filename, FILE * file)
{
    ...
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR,
                       "journal open failure: %s: %s", isc_result_totext (result), filename);
        return (result);
    }
    ...
    for (result = dns_journal_first_rr (j); result == ISC_R_SUCCESS; result = dns_journal_next_rr (j))
    {
        ...
        CHECK (dns_difftuple_create (diff.mctx, n_soa == 1 ?
                                     DNS_DIFFOP_DEL : DNS_DIFFOP_ADD, name, ttl, rdata, &tuple));
        dns_diff_append (&diff, &tuple);
  ...
  failure:
    isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR, "%s: cannot print: journal file corrupt", j->filename);
...
static isc_result_t read_one_rr (dns_journal_t * j)
{
    ...
    if (j->it.xpos == j->it.xsize)
    {
        ...
        {
            isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR,
                           "%s: journal corrupt: empty transaction", j->filename);
            FAIL (ISC_R_UNEXPECTED);
        }
        if (xhdr.serial0 != j->it.current_serial)
        {
            isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR,
                           "%s: journal file corrupt: "
                           "expected serial %u, got %u", j->filename, j->it.current_serial, xhdr.serial0);
            FAIL (ISC_R_UNEXPECTED);
        }
    ...
    {
        isc_log_write (JOURNAL_COMMON_LOGARGS, ISC_LOG_ERROR,
                       "%s: journal corrupt: impossible RR size " "(%d bytes)", j->filename, rrhdr.size);
        FAIL (ISC_R_UNEXPECTED);
    }
     ...
     */
    isc_buffer_setactive (&j->it.source, j->it.source.used - j->it.source.current);
    CHECK (dns_name_fromwire (&j->it.name, &j->it.source, &j->it.dctx, 0, &j->it.target));
    ...
    CHECK (dns_rdata_fromwire (&j->it.rdata, rdclass, rdtype, &j->it.source, &j->it.dctx, 0, &j->it.target));
    j->it.ttl = ttl;
...
get_name_diff (dns_db_t * db, dns_dbversion_t * ver, isc_stdtime_t now,
               dns_dbiterator_t * dbit, dns_name_t * name, dns_diffop_t op, dns_diff_t * diff)
{
    isc_result_t result;
    ...
    for (result = dns_rdatasetiter_first (rdsiter); result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (rdsiter))
    {
        ...
        for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
        {
            ...
            dns_rdataset_current (&rdataset, &rdata);
            result = dns_difftuple_create (diff->mctx, op, name, rdataset.ttl, &rdata, &tuple);
            if (result != ISC_R_SUCCESS)
            {
                dns_rdataset_disassociate (&rdataset);
...
diff_namespace (isc_mem_t * mctx,
                ...
                dns_db_t * dbb, dns_dbversion_t * dbverb, unsigned int options, dns_diff_t * resultdiff)
{
    dns_db_t *db[2];
    ...
    for (;;)
    {
        for (i = 0; i < 2; i++)
        {
            if (!have[i] && itresult[i] == ISC_R_SUCCESS)
            {
                CHECK (get_name_diff (db[i], ver[i], 0, dbit[i],
                                      ...
                                      i == 0 ? DNS_DIFFOP_ADD : DNS_DIFFOP_DEL, &diff[i]));
        ...
        for (i = 0; i < 2; i++)
        {
            if (!have[!i])
            {
                ISC_LIST_APPENDLIST (resultdiff->tuples, diff[i].tuples, link);
        ...
        t = dns_name_compare (dns_fixedname_name (&fixname[0]), dns_fixedname_name (&fixname[1]));
        if (t < 0)
        {
            ISC_LIST_APPENDLIST (resultdiff->tuples, diff[0].tuples, link);
            INSIST (ISC_LIST_EMPTY (diff[0].tuples));
            have[0] = ISC_FALSE;
            continue;
        }
        if (t > 0)
        {
            ISC_LIST_APPENDLIST (resultdiff->tuples, diff[1].tuples, link);
            INSIST (ISC_LIST_EMPTY (diff[1].tuples));
            have[1] = ISC_FALSE;
            continue;
        }
...
dns_db_diff (isc_mem_t * mctx,
             dns_db_t * dba, dns_dbversion_t * dbvera,
             dns_db_t * dbb, dns_dbversion_t * dbverb, const char *journal_filename)
{
    isc_result_t result;
    ...
    CHECK (diff_namespace (mctx, dba, dbvera, dbb, dbverb, DNS_DB_NONSEC3, &resultdiff));
    CHECK (diff_namespace (mctx, dba, dbvera, dbb, dbverb, DNS_DB_NSEC3ONLY, &resultdiff));
...
isc_result_t dns_journal_compact (isc_mem_t * mctx, char *filename, isc_uint32_t serial, isc_uint32_t target_size)
{
    ...
    result = isc_string_printf (newname, sizeof (newname), "%.*s.jnw", namelen, filename);
    if (result != ISC_R_SUCCESS)
        return (result);
    ...
    if (DNS_SERIAL_GT (j->header.begin.serial, serial) || DNS_SERIAL_GT (serial, j->header.end.serial))
    {
        dns_journal_destroy (&j);
        return (ISC_R_RANGE);
    }
     ...
     */
    indexend = sizeof (journal_rawheader_t) + j->header.index_size * sizeof (journal_rawpos_t);
    ...
    for (i = 0; i < j->header.index_size; i++)
    {
        if (POS_VALID (j->index[i]) &&
            ...
            ((isc_uint32_t) (j->header.end.offset - j->index[i].offset)
             >= target_size / 2) && j->index[i].offset > best_guess.offset)
            best_guess = j->index[i];
    }
    ...
    while (current_pos.serial != serial)
    {
        ...
        if (DNS_SERIAL_GE (serial, current_pos.serial) &&
            ((isc_uint32_t) (j->header.end.offset - current_pos.offset)
             >= (target_size / 2)) && current_pos.offset > best_guess.offset)
    ...
    if (copy_length != 0)
    {
        ...
        {
            unsigned int len = (copy_length - i) > size ? size : (copy_length - i);
...
static isc_result_t index_to_disk (dns_journal_t * j)
{
