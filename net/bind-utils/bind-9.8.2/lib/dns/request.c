...
dns_requestmgr_create (isc_mem_t * mctx,
                       ...
                       isc_taskmgr_t * taskmgr,
                       dns_dispatchmgr_t * dispatchmgr,
                       dns_dispatch_t * dispatchv4, dns_dispatch_t * dispatchv6, dns_requestmgr_t ** requestmgrp)
{
    dns_requestmgr_t *requestmgr;
...
static void mgr_shutdown (dns_requestmgr_t * requestmgr)
{
    ...
    if (!requestmgr->exiting)
    {
        ...
        for (request = ISC_LIST_HEAD (requestmgr->requests); request != NULL; request = ISC_LIST_NEXT (request, link))
        {
...
static void requestmgr_attach (dns_requestmgr_t * source, dns_requestmgr_t ** targetp)
{
    ...
    *targetp = source;
    ...
    req_log (ISC_LOG_DEBUG (3), "requestmgr_attach: %p: eref %d iref %d", source, source->eref, source->iref);
}
...
void dns_requestmgr_attach (dns_requestmgr_t * source, dns_requestmgr_t ** targetp)
{
    ...
    *targetp = source;
    ...
    req_log (ISC_LOG_DEBUG (3), "dns_requestmgr_attach: %p: eref %d iref %d", source, source->eref, source->iref);
}
...
void dns_requestmgr_detach (dns_requestmgr_t ** requestmgrp)
{
    ...
    {
        INSIST (requestmgr->exiting && ISC_LIST_HEAD (requestmgr->requests) == NULL);
        need_destroy = ISC_TRUE;
    }
    UNLOCK (&requestmgr->lock);
...
static void send_shutdown_events (dns_requestmgr_t * requestmgr)
{
     ...
     */
    for (event = ISC_LIST_HEAD (requestmgr->whenshutdown); event != NULL; event = next_event)
    {
...
static inline isc_result_t req_send (dns_request_t * request, isc_task_t * task, isc_sockaddr_t * address)
{
    isc_region_t r;
     ...
     * at this moment.
     */
    result = isc_socket_sendto (socket, &r, task, req_senddone, request, address, NULL);
    if (result == ISC_R_SUCCESS)
...
static isc_result_t new_request (isc_mem_t * mctx, dns_request_t ** requestp)
{
    ...
    ISC_EVENT_INIT (&request->ctlevent, sizeof (request->ctlevent), 0, NULL,
                    DNS_EVENT_REQUESTCONTROL, do_cancel, request, NULL, NULL, NULL);
...
static isc_boolean_t isblackholed (dns_dispatchmgr_t * dispatchmgr, isc_sockaddr_t * destaddr)
{
    ...
    if (blackhole != NULL)
    {
        ...
        if (dns_acl_match (&netaddr, NULL, blackhole, NULL, &match, NULL) == ISC_R_SUCCESS && match > 0)
            drop = ISC_TRUE;
    }
    if (drop)
    {
                     ...
                     isc_sockaddr_t * destaddr, dns_dispatch_t ** dispatchp)
{
    ...
    result = isc_socket_create (requestmgr->socketmgr, isc_sockaddr_pf (destaddr), isc_sockettype_tcp, &socket);
    if (result != ISC_R_SUCCESS)
        return (result);
    ...
    result = dns_dispatch_createtcp (requestmgr->dispatchmgr,
                                     socket, requestmgr->taskmgr, 4096, 2, 1, 1, 3, attrs, dispatchp);
  cleanup:
                   ...
                   isc_sockaddr_t * destaddr, dns_dispatch_t ** dispatchp)
{
    ...
    return (dns_dispatch_getudp (requestmgr->dispatchmgr,
                                 ...
                                 requestmgr->taskmgr,
                                 srcaddr, 4096, 1000, 32768, 16411, 16433, attrs, attrmask, dispatchp));
}

static isc_result_t
get_dispatch (isc_boolean_t tcp, dns_requestmgr_t * requestmgr,
              isc_sockaddr_t * srcaddr, isc_sockaddr_t * destaddr, dns_dispatch_t ** dispatchp)
{
    isc_result_t result;

    if (tcp)
        result = create_tcp_dispatch (requestmgr, srcaddr, destaddr, dispatchp);
    else
        result = find_udp_dispatch (requestmgr, srcaddr, destaddr, dispatchp);
    return (result);
}
...
static isc_result_t set_timer (isc_timer_t * timer, unsigned int timeout, unsigned int udpresend)
{
    ...
    if (result == ISC_R_SUCCESS)
        result = isc_timer_reset (timer, timertype, &expires, &interval, ISC_FALSE);
    return (result);
}
...
dns_request_createraw (dns_requestmgr_t * requestmgr, isc_buffer_t * msgbuf,
                       ...
                       isc_task_t * task, isc_taskaction_t action, void *arg, dns_request_t ** requestp)
{
    return (dns_request_createraw3 (requestmgr, msgbuf, srcaddr, destaddr,
                                    options, timeout, 0, 0, task, action, arg, requestp));
}
                        ...
                        isc_taskaction_t action, void *arg, dns_request_t ** requestp)
{
    ...
    return (dns_request_createraw3 (requestmgr, msgbuf, srcaddr, destaddr,
                                    options, timeout, udptimeout, udpretries, task, action, arg, requestp));
}
                        ...
                        isc_task_t * task, isc_taskaction_t action, void *arg, dns_request_t ** requestp)
{
    ...
    request->event = (dns_requestevent_t *)
        isc_event_allocate (mctx, task, DNS_EVENT_REQUESTDONE, action, arg, sizeof (dns_requestevent_t));
    ...
    result = get_dispatch (tcp, requestmgr, srcaddr, destaddr, &request->dispatch);
    if (result != ISC_R_SUCCESS)
        goto cleanup;

    result = dns_dispatch_addresponse2 (request->dispatch, destaddr, task,
                                        req_response, request, &id, &request->dispentry, requestmgr->socketmgr);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    ...
    result = isc_buffer_allocate (mctx, &request->query, r.length + (tcp ? 2 : 0));
    if (result != ISC_R_SUCCESS)
        goto cleanup;
    if (tcp)
    ...
    if (tcp)
    {
        result = isc_socket_connect (socket, destaddr, task, req_connected, request);
        if (result != ISC_R_SUCCESS)
            goto unlink;
  ...
  cleanup:
    ...
    req_destroy (request);
    req_log (ISC_LOG_DEBUG (3), "dns_request_createraw: failed %s", dns_result_totext (result));
    return (result);
}
                    ...
                    isc_taskaction_t action, void *arg, dns_request_t ** requestp)
{
    return (dns_request_createvia3 (requestmgr, message, NULL, address,
                                    options, key, timeout, 0, 0, task, action, arg, requestp));
}
                        ...
                        isc_task_t * task, isc_taskaction_t action, void *arg, dns_request_t ** requestp)
{
    ...
    if (udptimeout != 0)
        udpretries = timeout / udptimeout;
    return (dns_request_createvia3 (requestmgr, message, srcaddr, destaddr,
                                    options, key, timeout, udptimeout, udpretries, task, action, arg, requestp));
}
                        ...
                        isc_taskaction_t action, void *arg, dns_request_t ** requestp)
{
  ...
  cleanup:
    if (tclone != NULL)
        isc_task_detach (&tclone);
    req_destroy (request);
    req_log (ISC_LOG_DEBUG (3), "dns_request_createvia: failed %s", dns_result_totext (result));
    return (result);
}

static isc_result_t
req_render (dns_message_t * message, isc_buffer_t ** bufferp, unsigned int options, isc_mem_t * mctx)
{
...
isc_result_t dns_request_getresponse (dns_request_t * request, dns_message_t * message, unsigned int options)
{
    ...
    req_log (ISC_LOG_DEBUG (3), "dns_request_getresponse: request %p", request);
...
static void req_response (isc_task_t * task, isc_event_t * event)
{
    ...
    req_log (ISC_LOG_DEBUG (3), "req_response: request %p: %s", request, dns_result_totext (devent->result));
...
static void req_log (int level, const char *fmt, ...)
{
    ...
    va_start (ap, fmt);
    isc_log_vwrite (dns_lctx, DNS_LOGCATEGORY_GENERAL, DNS_LOGMODULE_REQUEST, level, fmt, ap);
    va_end (ap);
}
