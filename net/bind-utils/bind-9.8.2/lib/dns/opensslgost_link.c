...
static isc_result_t opensslgost_todns (const dst_key_t * key, isc_buffer_t * data);

static isc_result_t opensslgost_createctx (dst_key_t * key, dst_context_t * dctx)
{
...
static void opensslgost_destroyctx (dst_context_t * dctx)
{
...
static isc_result_t opensslgost_adddata (dst_context_t * dctx, const isc_region_t * data)
{
    EVP_MD_CTX *evp_md_ctx = dctx->ctxdata.evp_md_ctx;
...
static isc_result_t opensslgost_sign (dst_context_t * dctx, isc_buffer_t * sig)
{
...
static isc_result_t opensslgost_verify (dst_context_t * dctx, const isc_region_t * sig)
{
    dst_key_t *key = dctx->key;
...
static isc_boolean_t opensslgost_compare (const dst_key_t * key1, const dst_key_t * key2)
{
...
static isc_result_t opensslgost_generate (dst_key_t * key, int unused, void (*callback) (int))
{
    EVP_PKEY_CTX *ctx;
...
static void opensslgost_destroy (dst_key_t * key)
{
    EVP_PKEY *pkey = key->keydata.pkey;
...
static isc_result_t opensslgost_fromdns (dst_key_t * key, isc_buffer_t * data)
{
    isc_region_t r;
...
static isc_result_t opensslgost_tofile (const dst_key_t * key, const char *directory)
{
    EVP_PKEY *pkey;
...
static isc_result_t opensslgost_parse (dst_key_t * key, isc_lex_t * lexer, dst_key_t * pub)
{
    dst_private_t priv;
    ...
    if (d2i_PrivateKey (NID_id_GostR3410_2001, &pkey, &p, (long) priv.elements[0].length) == NULL)
...
static void opensslgost_cleanup (void)
{
    if (e != NULL)
    {
        ENGINE_finish (e);
...
isc_result_t dst__opensslgost_init (dst_func_t ** funcp)
{
    REQUIRE (funcp != NULL);
    ...
    if ((opensslgost_digest == NULL) ||
        ...
        (ENGINE_ctrl_cmd_string (e, "CRYPT_PARAMS", "id-Gost28147-89-CryptoPro-A-ParamSet", 0) <= 0))
    {
        ENGINE_finish (e);
        ENGINE_free (e);
        e = NULL;
