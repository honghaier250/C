...
static isc_boolean_t ignore (dns_rdata_t * param, dns_rdataset_t * privateset)
{
    ...
    for (result = dns_rdataset_first (privateset); result == ISC_R_SUCCESS; result = dns_rdataset_next (privateset))
    {
...
dns_private_chains (dns_db_t * db, dns_dbversion_t * ver,
                    dns_rdatatype_t privatetype, isc_boolean_t * build_nsec, isc_boolean_t * build_nsec3)
{
    dns_dbnode_t *node;
    ...
    result = dns_db_findrdataset (db, node, ver, dns_rdatatype_nsec, 0, (isc_stdtime_t) 0, &nsecset, NULL);

    if (result != ISC_R_SUCCESS && result != ISC_R_NOTFOUND)
        goto failure;

    result = dns_db_findrdataset (db, node, ver, dns_rdatatype_nsec3param, 0, (isc_stdtime_t) 0, &nsec3paramset, NULL);
    if (result != ISC_R_SUCCESS && result != ISC_R_NOTFOUND)
        goto failure;

    if (dns_rdataset_isassociated (&nsecset) && dns_rdataset_isassociated (&nsec3paramset))
    {
        if (build_nsec != NULL)
            *build_nsec = ISC_TRUE;
        if (build_nsec3 != NULL)
    ...
    if (dns_rdataset_isassociated (&nsecset))
    {
        ...
        for (result = dns_rdataset_first (&privateset);
             result == ISC_R_SUCCESS; result = dns_rdataset_next (&privateset))
        {
            unsigned char buf[DNS_NSEC3PARAM_BUFFERSIZE];
    ...
    for (result = dns_rdataset_first (&privateset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&privateset))
    {
        ...
        if (!dns_nsec3param_fromprivate (&private, &rdata, buf, sizeof (buf)))
        {
             ...
             * zone with a key.
             */
            if (private.length == 5 && private.data[0] != 0 && private.data[3] == 0 && private.data[4] == 0)
                signing = ISC_TRUE;
        }
        else
        {
