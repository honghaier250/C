...
dns_order_add (dns_order_t * order, dns_name_t * name,
               dns_rdatatype_t rdtype, dns_rdataclass_t rdclass, unsigned int mode)
{
    dns_order_ent_t *ent;

    REQUIRE (DNS_ORDER_VALID (order));
    REQUIRE (mode == DNS_RDATASETATTR_RANDOMIZE ||
             mode == DNS_RDATASETATTR_FIXEDORDER || mode == 0 /* DNS_RDATASETATTR_CYCLIC */ );
    ...
    RUNTIME_CHECK (dns_name_copy (name, dns_fixedname_name (&ent->name), NULL) == ISC_R_SUCCESS);
    ent->rdtype = rdtype;
    ent->rdclass = rdclass;
    ent->mode = mode;
...
static inline isc_boolean_t match (dns_name_t * name1, dns_name_t * name2)
{
...
unsigned int dns_order_find (dns_order_t * order, dns_name_t * name, dns_rdatatype_t rdtype, dns_rdataclass_t rdclass)
{
    ...
    for (ent = ISC_LIST_HEAD (order->ents); ent != NULL; ent = ISC_LIST_NEXT (ent, link))
    {
        if (ent->rdtype != rdtype && ent->rdtype != dns_rdatatype_any)
            continue;
        if (ent->rdclass != rdclass && ent->rdclass != dns_rdataclass_any)
            continue;
