...
static isc_result_t check_hints (dns_db_t * db)
{
    ...
    (void) dns_db_find (db, dns_rootname, NULL, dns_rdatatype_ns, 0, now, NULL, name, &rootns, NULL);
...
isc_result_t dns_rootns_create (isc_mem_t * mctx, dns_rdataclass_t rdclass, const char *filename, dns_db_t ** target)
{
    ...
    if (filename != NULL)
    {
        ...
        result = dns_master_loadfile (filename, &db->origin,
                                      &db->origin, db->rdclass, DNS_MASTER_HINT, &callbacks, db->mctx);
    }
    ...
    if (check_hints (db) != ISC_R_SUCCESS)
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                       ...
                       "extra data in root hints '%s'", (filename != NULL) ? filename : "<BUILT-IN>");
    *target = db;
    return (ISC_R_SUCCESS);
...
static void report (dns_view_t * view, dns_name_t * name, isc_boolean_t missing, dns_rdata_t * rdata)
{
    ...
    if (strcmp (view->name, "_bind") != 0 && strcmp (view->name, "_default") != 0)
    {
        viewname = view->name;
        sep = ": view ";
    }
    ...
    if (missing)
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                       DNS_LOGMODULE_HINTS, ISC_LOG_WARNING,
                       "checkhints%s%s: %s/%s (%s) missing from hints", sep, viewname, namebuf, typebuf, databuf);
    else
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                       DNS_LOGMODULE_HINTS, ISC_LOG_WARNING,
                       "checkhints%s%s: %s/%s (%s) extra record " "in hints", sep, viewname, namebuf, typebuf, databuf);
}
...
static void
check_address_records (dns_view_t * view, dns_db_t * hints, dns_db_t * db, dns_name_t * name, isc_stdtime_t now)
{
    ...
    hresult = dns_db_find (hints, name, NULL, dns_rdatatype_a, 0, now, NULL, foundname, &hintrrset, NULL);
    rresult = dns_db_find (db, name, NULL, dns_rdatatype_a, DNS_DBFIND_GLUEOK, now, NULL, foundname, &rootrrset, NULL);
    if (hresult == ISC_R_SUCCESS && (rresult == ISC_R_SUCCESS || rresult == DNS_R_GLUE))
    {
    ...
    if (hresult == ISC_R_NOTFOUND && (rresult == ISC_R_SUCCESS || rresult == DNS_R_GLUE))
    {
        result = dns_rdataset_first (&rootrrset);
        while (result == ISC_R_SUCCESS)
        {
     ...
     * Check AAAA records.
     */
    hresult = dns_db_find (hints, name, NULL, dns_rdatatype_aaaa, 0, now, NULL, foundname, &hintrrset, NULL);
    rresult = dns_db_find (db, name, NULL, dns_rdatatype_aaaa,
                           DNS_DBFIND_GLUEOK, now, NULL, foundname, &rootrrset, NULL);
    if (hresult == ISC_R_SUCCESS && (rresult == ISC_R_SUCCESS || rresult == DNS_R_GLUE))
    {
        result = dns_rdataset_first (&rootrrset);
        while (result == ISC_R_SUCCESS)
...
void dns_root_checkhints (dns_view_t * view, dns_db_t * hints, dns_db_t * db)
{
    ...
    result = dns_db_find (hints, dns_rootname, NULL, dns_rdatatype_ns, 0, now, NULL, name, &hintns, NULL);
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                       DNS_LOGMODULE_HINTS, ISC_LOG_WARNING,
                       "checkhints%s%s: unable to get root NS rrset "
                       "from hints: %s", sep, viewname, dns_result_totext (result));
        goto cleanup;
    }

    result = dns_db_find (db, dns_rootname, NULL, dns_rdatatype_ns, 0, now, NULL, name, &rootns, NULL);
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                       DNS_LOGMODULE_HINTS, ISC_LOG_WARNING,
                       "checkhints%s%s: unable to get root NS rrset "
                       "from cache: %s", sep, viewname, dns_result_totext (result));
        goto cleanup;
    }
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                           DNS_LOGMODULE_HINTS, ISC_LOG_WARNING,
                           "checkhints%s%s: unable to find root " "NS '%s' in hints", sep, viewname, namebuf);
        }
        else
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_GENERAL,
                           DNS_LOGMODULE_HINTS, ISC_LOG_WARNING,
                           "checkhints%s%s: extra NS '%s' in hints", sep, viewname, namebuf);
        }
        dns_rdata_reset (&rdata);
