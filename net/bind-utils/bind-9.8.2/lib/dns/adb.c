 ...
 */
     ...
     static inline dns_adblameinfo_t *new_adblameinfo (dns_adb_t *, dns_name_t *, dns_rdatatype_t);
     ...
     static inline dns_adbaddrinfo_t *new_adbaddrinfo (dns_adb_t *, dns_adbentry_t *, in_port_t);

     static inline dns_adbfetch_t *new_adbfetch (dns_adb_t *);

     static inline void free_adbfetch (dns_adb_t *, dns_adbfetch_t **);

     static inline dns_adbname_t *find_name_and_lock (dns_adb_t *, dns_name_t *, unsigned int, int *);

     static inline dns_adbentry_t *find_entry_and_lock (dns_adb_t *, isc_sockaddr_t *, int *, isc_stdtime_t);
     ...
     static void print_namehook_list (FILE *, const char *legend,
                                      dns_adbnamehooklist_t * list, isc_boolean_t debug, isc_stdtime_t now);
     ...
     static inline void inc_entry_refcnt (dns_adb_t *, dns_adbentry_t *, isc_boolean_t);

     static inline isc_boolean_t dec_entry_refcnt (dns_adb_t *, isc_boolean_t, dns_adbentry_t *, isc_boolean_t);
     ...
     static isc_result_t fetch_name (dns_adbname_t *, isc_boolean_t, dns_rdatatype_t);
...
static void DP (int level, const char *format, ...) ISC_FORMAT_PRINTF (2, 3);
     ...
     static void DP (int level, const char *format, ...)
{
    va_list args;

    va_start (args, format);
    isc_log_vwrite (dns_lctx, DNS_LOGCATEGORY_DATABASE, DNS_LOGMODULE_ADB, level, format, args);
    va_end (args);
}
 ...
 */
static inline void inc_stats (dns_adb_t * adb, isc_statscounter_t counter)
{
...
static void grow_entries (isc_task_t * task, isc_event_t * ev)
{
    ...
    if (newentries == NULL || newdeadentries == NULL ||
        newentrylocks == NULL || newentry_sd == NULL || newentry_refcnt == NULL)
        goto cleanup;
    ...
    isc_mem_put (adb->mctx, adb->entries, sizeof (*adb->entries) * adb->nentries);
    isc_mem_put (adb->mctx, adb->deadentries, sizeof (*adb->deadentries) * adb->nentries);
    isc_mem_put (adb->mctx, adb->entrylocks, sizeof (*adb->entrylocks) * adb->nentries);
    isc_mem_put (adb->mctx, adb->entry_sd, sizeof (*adb->entry_sd) * adb->nentries);
    isc_mem_put (adb->mctx, adb->entry_refcnt, sizeof (*adb->entry_refcnt) * adb->nentries);
  ...
  cleanup:
    ...
    if (newdeadentries != NULL)
        isc_mem_put (adb->mctx, newdeadentries, sizeof (*newdeadentries) * n);
    if (newentrylocks != NULL)
        isc_mem_put (adb->mctx, newentrylocks, sizeof (*newentrylocks) * n);
    if (newentry_sd != NULL)
        isc_mem_put (adb->mctx, newentry_sd, sizeof (*newentry_sd) * n);
    if (newentry_refcnt != NULL)
        isc_mem_put (adb->mctx, newentry_refcnt, sizeof (*newentry_refcnt) * n);
  done:
...
static void grow_names (isc_task_t * task, isc_event_t * ev)
{
    ...
    if (newnames == NULL || newdeadnames == NULL ||
        newnamelocks == NULL || newname_sd == NULL || newname_refcnt == NULL)
        goto cleanup;
    ...
    isc_mem_put (adb->mctx, adb->deadnames, sizeof (*adb->deadnames) * adb->nnames);
    isc_mem_put (adb->mctx, adb->namelocks, sizeof (*adb->namelocks) * adb->nnames);
    isc_mem_put (adb->mctx, adb->name_sd, sizeof (*adb->name_sd) * adb->nnames);
    isc_mem_put (adb->mctx, adb->name_refcnt, sizeof (*adb->name_refcnt) * adb->nnames);
  ...
  cleanup:
    ...
    if (newname_refcnt != NULL)
        isc_mem_put (adb->mctx, newname_refcnt, sizeof (*newname_refcnt) * n);
  done:
    isc_task_endexclusive (task);
...
static isc_result_t import_rdataset (dns_adbname_t * adbname, dns_rdataset_t * rdataset, isc_stdtime_t now)
{
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        foundentry = find_entry_and_lock (adb, &sockaddr, &addr_bucket, now);
        if (foundentry == NULL)
        {
            dns_adbentry_t *entry;
        ...
        {
            for (anh = ISC_LIST_HEAD (*hookhead); anh != NULL; anh = ISC_LIST_NEXT (anh, plink))
  ...
  fail:
    ...
    if (rdataset->trust == dns_trust_glue || rdataset->trust == dns_trust_additional)
    ...
    if (rdtype == dns_rdatatype_a)
    {
        DP (NCACHE_LEVEL, "expire_v4 set to MIN(%u,%u) import_rdataset", adbname->expire_v4, now + rdataset->ttl);
        adbname->expire_v4 = ISC_MIN (adbname->expire_v4, now + rdataset->ttl);
    }
    else
    {
        DP (NCACHE_LEVEL, "expire_v6 set to MIN(%u,%u) import_rdataset", adbname->expire_v6, now + rdataset->ttl);
        adbname->expire_v6 = ISC_MIN (adbname->expire_v6, now + rdataset->ttl);
    }
 ...
 */
static isc_boolean_t kill_name (dns_adbname_t ** n, isc_eventtype_t ev)
{
    dns_adbname_t *name;
 ...
 */
static isc_boolean_t check_expire_namehooks (dns_adbname_t * name, isc_stdtime_t now)
{
    dns_adb_t *adb;
 ...
 * Requires the name's bucket be locked.
 */
static inline void link_name (dns_adb_t * adb, int bucket, dns_adbname_t * name)
{
 ...
 * Requires the name's bucket be locked.
 */
static inline isc_boolean_t unlink_name (dns_adb_t * adb, dns_adbname_t * name)
{
    int bucket;
 ...
 */
static inline void link_entry (dns_adb_t * adb, int bucket, dns_adbentry_t * entry)
{
    int i;
 ...
 * Requires the entry's bucket be locked.
 */
static inline isc_boolean_t unlink_entry (dns_adb_t * adb, dns_adbentry_t * entry)
{
    int bucket;
...
static inline void violate_locking_hierarchy (isc_mutex_t * have, isc_mutex_t * want)
{
 ...
 */
static isc_boolean_t shutdown_names (dns_adb_t * adb)
{
    unsigned int bucket;
    ...
    for (bucket = 0; bucket < adb->nnames; bucket++)
    {
        ...
        else
        {
            ...
            while (name != NULL)
            {
                ...
                INSIST (result == ISC_FALSE);
                result = kill_name (&name, DNS_EVENT_ADBSHUTDOWN);
                name = next_name;
            }
        }
 ...
 * checked after calling this function.
 */
static isc_boolean_t shutdown_entries (dns_adb_t * adb)
{
    unsigned int bucket;
 ...
 */
static void cancel_fetches_at_name (dns_adbname_t * name)
{
 ...
 */
static isc_boolean_t clean_namehooks (dns_adb_t * adb, dns_adbnamehooklist_t * namehooks)
{
    dns_adbentry_t *entry;
    ...
    while (namehook != NULL)
    {
        ...
        if (entry != NULL)
        {
            ...
            result = dec_entry_refcnt (adb, overmem, entry, ISC_FALSE);
        }
...
static void clean_target (dns_adb_t * adb, dns_name_t * target)
{
...
static isc_result_t
set_target (dns_adb_t * adb, dns_name_t * name, dns_name_t * fname, dns_rdataset_t * rdataset, dns_name_t * target)
{
    isc_result_t result;
    ...
    else
    {
        ...
        result = dns_name_concatenate (prefix, &dname.dname, new_target, NULL);
 ...
 * Assumes the name bucket is locked.
 */
static void clean_finds_at_name (dns_adbname_t * name, isc_eventtype_t evtype, unsigned int addrs)
{
    isc_event_t *ev;
    ...
    DP (ENTER_LEVEL, "ENTER clean_finds_at_name, name %p, evtype %08x, addrs %08x", name, evtype, addrs);
    ...
    while (find != NULL)
    {
        ...
        if (process)
        {
            ...
            DP (DEF_LEVEL, "sending event %p to task %p for find %p", ev, task, find);
...
static inline void check_exit (dns_adb_t * adb)
{
    isc_event_t *event;
...
static inline isc_boolean_t dec_adb_irefcnt (dns_adb_t * adb)
{
    isc_event_t *event;
...
static inline void inc_adb_irefcnt (dns_adb_t * adb)
{
    LOCK (&adb->reflock);
    adb->irefcnt++;
    UNLOCK (&adb->reflock);
...
static inline void inc_adb_erefcnt (dns_adb_t * adb)
{
    LOCK (&adb->reflock);
    adb->erefcnt++;
    UNLOCK (&adb->reflock);
...
static inline void inc_entry_refcnt (dns_adb_t * adb, dns_adbentry_t * entry, isc_boolean_t lock)
{
    int bucket;
...
dec_entry_refcnt (dns_adb_t * adb, isc_boolean_t overmem, dns_adbentry_t * entry, isc_boolean_t lock)
{
    int bucket;
    ...
    if (entry->refcnt == 0 &&
        (adb->entry_sd[bucket] || entry->expires == 0 || overmem || (entry->flags & ENTRY_IS_DEAD) != 0))
    {
...
static inline dns_adbname_t *new_adbname (dns_adb_t * adb, dns_name_t * dnsname)
{
    dns_adbname_t *name;
...
static inline void free_adbname (dns_adb_t * adb, dns_adbname_t ** name)
{
    dns_adbname_t *n;
...
static inline dns_adbnamehook_t *new_adbnamehook (dns_adb_t * adb, dns_adbentry_t * entry)
{
    dns_adbnamehook_t *nh;
...
static inline void free_adbnamehook (dns_adb_t * adb, dns_adbnamehook_t ** namehook)
{
    dns_adbnamehook_t *nh;
...
static inline dns_adblameinfo_t *new_adblameinfo (dns_adb_t * adb, dns_name_t * qname, dns_rdatatype_t qtype)
{
...
static inline void free_adblameinfo (dns_adb_t * adb, dns_adblameinfo_t ** lameinfo)
{
    dns_adblameinfo_t *li;
...
static inline dns_adbentry_t *new_adbentry (dns_adb_t * adb)
{
    dns_adbentry_t *e;
    ...
    adb->entriescnt++;
    if (!adb->growentries_sent && adb->entriescnt > (adb->nentries * 8))
    {
...
static inline void free_adbentry (dns_adb_t * adb, dns_adbentry_t ** entry)
{
    dns_adbentry_t *e;
...
static inline dns_adbfind_t *new_adbfind (dns_adb_t * adb)
{
    dns_adbfind_t *h;
    ...
    ISC_EVENT_INIT (&h->event, sizeof (isc_event_t), 0, 0, 0, NULL, NULL, NULL, NULL, h);
...
static inline dns_adbfetch_t *new_adbfetch (dns_adb_t * adb)
{
    dns_adbfetch_t *f;
...
static inline void free_adbfetch (dns_adb_t * adb, dns_adbfetch_t ** fetch)
{
    dns_adbfetch_t *f;
...
static inline isc_boolean_t free_adbfind (dns_adb_t * adb, dns_adbfind_t ** findp)
{
    dns_adbfind_t *find;
 ...
 */
static inline dns_adbaddrinfo_t *new_adbaddrinfo (dns_adb_t * adb, dns_adbentry_t * entry, in_port_t port)
{
...
static inline void free_adbaddrinfo (dns_adb_t * adb, dns_adbaddrinfo_t ** ainfo)
{
    dns_adbaddrinfo_t *ai;
 ...
 */
static inline dns_adbname_t *find_name_and_lock (dns_adb_t * adb, dns_name_t * name, unsigned int options, int *bucketp)
{
    ...
    while (adbname != NULL)
    {
        if (!NAME_DEAD (adbname))
        {
            if (dns_name_equal (name, &adbname->name)
                && GLUEHINT_OK (adbname, options) && STARTATZONE_MATCHES (adbname, options))
                return (adbname);
        }
 ...
 * the bucket changes.
 */
static inline dns_adbentry_t *find_entry_and_lock (dns_adb_t * adb, isc_sockaddr_t * addr, int *bucketp,
                                                   isc_stdtime_t now)
{
    ...
    for (entry = ISC_LIST_HEAD (adb->entries[bucket]); entry != NULL; entry = entry_next)
    {
...
entry_is_lame (dns_adb_t * adb, dns_adbentry_t * entry, dns_name_t * qname, dns_rdatatype_t qtype, isc_stdtime_t now)
{
    ...
    while (li != NULL)
    {
         ...
         */
        if (li != NULL && !is_bad && li->qtype == qtype && dns_name_equal (qname, &li->qname))
            is_bad = ISC_TRUE;
...
copy_namehook_lists (dns_adb_t * adb, dns_adbfind_t * find, dns_name_t * qname,
                     dns_rdatatype_t qtype, dns_adbname_t * name, isc_stdtime_t now)
{
    dns_adbnamehook_t *namehook;
    ...
    if (find->options & DNS_ADBFIND_INET)
    {
        ...
        while (namehook != NULL)
        {
            ...
            if (!FIND_RETURNLAME (find) && entry_is_lame (adb, entry, qname, qtype, now))
            {
...
static void check_stale_name (dns_adb_t * adb, int bucket, isc_stdtime_t now)
{
    ...
    for (victims = 0; victim != NULL && victims < max_victims && scans < 10; victim = next_victim)
    {
        ...
        if (!NAME_FETCH (victim) && (overmem || victim->last_used + ADB_STALE_MARGIN <= now))
        {
            RUNTIME_CHECK (kill_name (&victim, DNS_EVENT_ADBCANCELED) == ISC_FALSE);
            victims++;
        }
 ...
 */
static isc_boolean_t check_expire_entry (dns_adb_t * adb, dns_adbentry_t ** entryp, isc_stdtime_t now)
{
    dns_adbentry_t *entry;
...
static void destroy (dns_adb_t * adb)
{
    adb->magic = 0;
                ...
                isc_taskmgr_t * taskmgr, dns_adb_t ** newadb)
{
    ...
    ISC_EVENT_INIT (&adb->cevent, sizeof (adb->cevent), 0, NULL,
                    DNS_EVENT_ADBCONTROL, shutdown_task, adb, adb, NULL, NULL);
    ...
    ISC_EVENT_INIT (&adb->growentries, sizeof (adb->growentries), 0, NULL,
                    DNS_EVENT_ADBGROWENTRIES, grow_entries, adb, adb, NULL, NULL);
    adb->growentries_sent = ISC_FALSE;
    ...
    ISC_EVENT_INIT (&adb->grownames, sizeof (adb->grownames), 0, NULL,
                    DNS_EVENT_ADBGROWNAMES, grow_names, adb, adb, NULL, NULL);
    adb->grownames_sent = ISC_FALSE;
...
void dns_adb_whenshutdown (dns_adb_t * adb, isc_task_t * task, isc_event_t ** eventp)
{
    ...
    *eventp = NULL;
    ...
    if (adb->shutting_down && zeroirefcnt && isc_mempool_getallocated (adb->ahmp) == 0)
    {
        /*
...
dns_adb_createfind (dns_adb_t * adb, isc_task_t * task, isc_taskaction_t action,
                    ...
                    isc_stdtime_t now, dns_name_t * target, in_port_t port, dns_adbfind_t ** findp)
{
    dns_adbfind_t *find;
    ...
    if (!EXPIRE_OK (adbname->expire_target, now))
    {
         ...
         * Yes, it is.
         */
        DP (DEF_LEVEL, "dns_adb_createfind: name %p is an alias (cached)", adbname);
        alias = ISC_TRUE;
        goto post_copy;
    }
     ...
     * in the database.
     */
    if (!NAME_HAS_V4 (adbname) && EXPIRE_OK (adbname->expire_v4, now) && WANT_INET (wanted_addresses))
    {
        result = dbfind_name (adbname, now, dns_rdatatype_a);
        if (result == ISC_R_SUCCESS)
        {
            DP (DEF_LEVEL, "dns_adb_createfind: found A for name %p in db", adbname);
            goto v6;
        }
        ...
        {
            DP (DEF_LEVEL, "dns_adb_createfind: name %p is an alias", adbname);
            alias = ISC_TRUE;
            goto post_copy;
        }
  ...
  v6:
    if (!NAME_HAS_V6 (adbname) && EXPIRE_OK (adbname->expire_v6, now) && WANT_INET6 (wanted_addresses))
    {
        result = dbfind_name (adbname, now, dns_rdatatype_aaaa);
        if (result == ISC_R_SUCCESS)
        {
            DP (DEF_LEVEL, "dns_adb_createfind: found AAAA for name %p", adbname);
            goto fetch;
        }
  ...
  fetch:
    ...
    if (wanted_fetches != 0 && !(FIND_AVOIDFETCHES (find) && have_address))
    {
         ...
         * Start V4.
         */
        if (WANT_INET (wanted_fetches) && fetch_name (adbname, start_at_zone, dns_rdatatype_a) == ISC_R_SUCCESS)
        {
            DP (DEF_LEVEL, "dns_adb_createfind: started A fetch for name %p", adbname);
        }

        /*
         * Start V6.
         */
        if (WANT_INET6 (wanted_fetches) && fetch_name (adbname, start_at_zone, dns_rdatatype_aaaa) == ISC_R_SUCCESS)
        {
            DP (DEF_LEVEL, "dns_adb_createfind: " "started AAAA fetch for name %p", adbname);
        }
    }
  ...
  post_copy:
    ...
    if (want_event)
    {
        ...
        DP (DEF_LEVEL, "createfind: attaching find %p to adbname %p", find, adbname);
    }
    else
    {
        /*
...
void dns_adb_destroyfind (dns_adbfind_t ** findp)
{
    ...
    *findp = NULL;
    ...
    while (ai != NULL)
    {
        ...
        ai->entry = NULL;
        INSIST (DNS_ADBENTRY_VALID (entry));
        RUNTIME_CHECK (dec_entry_refcnt (adb, overmem, entry, ISC_TRUE) == ISC_FALSE);
...
static void dump_adb (dns_adb_t * adb, FILE * f, isc_boolean_t debug, isc_stdtime_t now)
{
    unsigned int i;
    ...
    if (debug)
        fprintf (f, "; addr %p, erefcnt %u, irefcnt %u, finds out %u\n",
                 adb, adb->erefcnt, adb->irefcnt, isc_mempool_getallocated (adb->nhmp));
    ...
    for (i = 0; i < adb->nnames; i++)
    {
        ...
        for (; name != NULL; name = ISC_LIST_NEXT (name, plink))
        {
            ...
            fprintf (f, " [v4 %s] [v6 %s]", errnames[name->fetch_err], errnames[name->fetch6_err]);
...
static void dump_entry (FILE * f, dns_adbentry_t * entry, isc_boolean_t debug, isc_stdtime_t now)
{
    ...
    fprintf (f, ";\t%s [srtt %u] [flags %08x]", addrbuf, entry->srtt, entry->flags);
    if (entry->expires != 0)
        fprintf (f, " [ttl %d]", entry->expires - now);
    fprintf (f, "\n");
    for (li = ISC_LIST_HEAD (entry->lameinfo); li != NULL; li = ISC_LIST_NEXT (li, plink))
    {
        fprintf (f, ";\t\t");
...
void dns_adb_dumpfind (dns_adbfind_t * find, FILE * f)
{
    ...
    fprintf (f, ";\tqpending %08x partial %08x options %08x flags %08x\n",
             find->query_pending, find->partial_result, find->options, find->flags);
    ...
    while (ai != NULL)
    {
        ...
        {
            case AF_INET:
                tmpp = inet_ntop (AF_INET, &sa->type.sin.sin_addr, tmp, sizeof (tmp));
                break;
            case AF_INET6:
                tmpp = inet_ntop (AF_INET6, &sa->type.sin6.sin6_addr, tmp, sizeof (tmp));
                break;
            default:
                tmpp = "UnkFamily";
        }
        ...
        fprintf (f, "\t\tentry %p, flags %08x" " srtt %u addr %s\n", ai->entry, ai->flags, ai->srtt, tmpp);
...
static void print_dns_name (FILE * f, dns_name_t * name)
{
...
static void
print_namehook_list (FILE * f, const char *legend, dns_adbnamehooklist_t * list, isc_boolean_t debug, isc_stdtime_t now)
{
    dns_adbnamehook_t *nh;

    for (nh = ISC_LIST_HEAD (*list); nh != NULL; nh = ISC_LIST_NEXT (nh, plink))
    {
        if (debug)
            fprintf (f, ";\tHook(%s) %p\n", legend, nh);
...
static inline void print_fetch (FILE * f, dns_adbfetch_t * ft, const char *type)
{
    fprintf (f, "\t\tFetch(%s): %p -> { fetch %p }\n", type, ft, ft->fetch);
}

static void print_fetch_list (FILE * f, dns_adbname_t * n)
{
    if (NAME_FETCH_A (n))
...
static void print_find_list (FILE * f, dns_adbname_t * name)
{
    dns_adbfind_t *find;
...
static isc_result_t dbfind_name (dns_adbname_t * adbname, isc_stdtime_t now, dns_rdatatype_t rdtype)
{
    isc_result_t result;
    ...
    result = dns_view_find2 (adb->view, &adbname->name, rdtype, now,
                             ...
                             ISC_TRUE : ISC_FALSE, NULL, NULL, fname, &rdataset, NULL);
    ...
    switch (result)
    {
        ...
        case DNS_R_NXRRSET:
            ...
            if (rdtype == dns_rdatatype_a)
            {
                ...
                DP (NCACHE_LEVEL, "adb name %p: Caching auth negative entry for A", adbname);
            ...
            {
                DP (NCACHE_LEVEL, "adb name %p: Caching auth negative entry for AAAA", adbname);
        ...
        case DNS_R_NCACHENXRRSET:
            ...
            if (rdtype == dns_rdatatype_a)
            {
                ...
                if (result == DNS_R_NCACHENXDOMAIN)
                    ...
                    adbname->fetch_err = FIND_ERR_NXRRSET;
                DP (NCACHE_LEVEL, "adb name %p: Caching negative entry for A (ttl %u)", adbname, rdataset.ttl);
            }
            else
            {
                DP (NCACHE_LEVEL, "adb name %p: Caching negative entry for AAAA (ttl %u)", adbname, rdataset.ttl);
        ...
        case DNS_R_DNAME:
            ...
            result = set_target (adb, &adbname->name, fname, &rdataset, &adbname->target);
            if (result == ISC_R_SUCCESS)
            {
                result = DNS_R_ALIAS;
                DP (NCACHE_LEVEL, "adb name %p: caching alias target", adbname);
...
static void fetch_callback (isc_task_t * task, isc_event_t * ev)
{
    ...
    else if (NAME_FETCH_AAAA (name) && (name->fetch_aaaa->fetch == dev->fetch))
    {
    ...
    if (NCACHE_RESULT (dev->result))
    {
        ...
        {
            DP (NCACHE_LEVEL, "adb fetch name %p: " "caching negative entry for A (ttl %u)", name, dev->rdataset->ttl);
            name->expire_v4 = ISC_MIN (name->expire_v4, dev->rdataset->ttl + now);
        ...
        else
        {
            DP (NCACHE_LEVEL, "adb fetch name %p: "
                "caching negative entry for AAAA (ttl %u)", name, dev->rdataset->ttl);
            name->expire_v6 = ISC_MIN (name->expire_v6, dev->rdataset->ttl + now);
            if (dev->result == DNS_R_NCACHENXDOMAIN)
    ...
    if (dev->result == DNS_R_CNAME || dev->result == DNS_R_DNAME)
    {
        ...
        clean_target (adb, &name->target);
        name->expire_target = INT_MAX;
        result = set_target (adb, &name->name, dns_fixedname_name (&dev->foundname), dev->rdataset, &name->target);
        if (result == ISC_R_SUCCESS)
        {
            DP (NCACHE_LEVEL, "adb fetch name %p: caching alias target", name);
    ...
    if (dev->result != ISC_R_SUCCESS)
    {
        ...
        DP (DEF_LEVEL, "adb: fetch of '%s' %s failed: %s",
            buf, address_type == DNS_ADBFIND_INET ? "A" : "AAAA", dns_result_totext (dev->result));
...
static isc_result_t fetch_name (dns_adbname_t * adbname, isc_boolean_t start_at_zone, dns_rdatatype_t type)
{
    isc_result_t result;
    ...
    if (start_at_zone)
    {
        DP (ENTER_LEVEL, "fetch_name: starting at zone for name %p", adbname);
        dns_fixedname_init (&fixed);
        name = dns_fixedname_name (&fixed);
        result = dns_view_findzonecut2 (adb->view, &adbname->name, name, 0, 0, ISC_TRUE, ISC_FALSE, &rdataset, NULL);
                  ...
                  dns_rdatatype_t qtype, isc_stdtime_t expire_time)
{
    ...
    while (li != NULL && (li->qtype != qtype || !dns_name_equal (qname, &li->qname)))
...
void dns_adb_flushname (dns_adb_t * adb, dns_name_t * name)
{
    ...
    while (adbname != NULL)
    {
        ...
        {
            RUNTIME_CHECK (kill_name (&adbname, DNS_EVENT_ADBCANCELED) == ISC_FALSE);
        }
        adbname = nextname;
    }
    UNLOCK (&adb->namelocks[bucket]);
...
static void water (void *arg, int mark)
{
    ...
    DP (ISC_LOG_DEBUG (1), "adb reached %s water mark", overmem ? "high" : "low");
}

void dns_adb_setadbsize (dns_adb_t * adb, isc_uint32_t size)
{
