...
dns_nsec_buildrdata (dns_db_t * db, dns_dbversion_t * version,
                     dns_dbnode_t * node, dns_name_t * target, unsigned char *buffer, dns_rdata_t * rdata)
{
    isc_result_t result;
...
isc_boolean_t dns_nsec_typepresent (dns_rdata_t * nsec, dns_rdatatype_t type)
{
    ...
    for (i = 0; i < nsecstruct.len; i += len)
    {
        ...
        if (type < (window * 256) + len * 8)
            present = ISC_TF (bit_isset (&nsecstruct.typebits[i], type % 256));
        break;
    }
...
isc_result_t dns_nsec_nseconly (dns_db_t * db, dns_dbversion_t * version, isc_boolean_t * answer)
{
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        if (dnskey.algorithm == DST_ALG_RSAMD5 ||
            dnskey.algorithm == DST_ALG_RSASHA1 || dnskey.algorithm == DST_ALG_DSA || dnskey.algorithm == DST_ALG_ECC)
            break;
    }
    dns_rdataset_disassociate (&rdataset);
    if (result == ISC_R_SUCCESS)
