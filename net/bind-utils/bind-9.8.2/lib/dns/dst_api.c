 ...
 */
     static dst_key_t *get_key_struct (dns_name_t * name,
                                       ...
                                       unsigned int bits, dns_rdataclass_t rdclass, isc_mem_t * mctx);
     ...
     static isc_result_t buildfilename (dns_name_t * name,
                                        dns_keytag_t id,
                                        unsigned int alg, unsigned int type, const char *directory, isc_buffer_t * out);
     ...
     static isc_result_t addsuffix (char *filename, int len,
                                    const char *dirname, const char *ofilename, const char *suffix);
...
isc_result_t dst_lib_init2 (isc_mem_t * mctx, isc_entropy_t * ectx, const char *engine, unsigned int eflags)
{
     ...
     * by libcrypto.
     */
    result = isc_mem_createx2 (0, 0, default_memalloc, default_memfree, NULL, &dst__memory_pool, 0);
    if (result != ISC_R_SUCCESS)
        return (result);
    ...
    RETERR (dst__opensslrsa_init (&dst_t_func[DST_ALG_RSAMD5], DST_ALG_RSAMD5));
    RETERR (dst__opensslrsa_init (&dst_t_func[DST_ALG_RSASHA1], DST_ALG_RSASHA1));
    RETERR (dst__opensslrsa_init (&dst_t_func[DST_ALG_NSEC3RSASHA1], DST_ALG_NSEC3RSASHA1));
    RETERR (dst__opensslrsa_init (&dst_t_func[DST_ALG_RSASHA256], DST_ALG_RSASHA256));
    RETERR (dst__opensslrsa_init (&dst_t_func[DST_ALG_RSASHA512], DST_ALG_RSASHA512));
...
isc_result_t dst_context_sign (dst_context_t * dctx, isc_buffer_t * sig)
{
    ...
    if (key->func->isprivate == NULL || key->func->isprivate (key) == ISC_FALSE)
        return (DST_R_NOTPRIVATEKEY);
...
isc_result_t dst_key_computesecret (const dst_key_t * pub, const dst_key_t * priv, isc_buffer_t * secret)
{
    ...
    if (pub->key_alg != priv->key_alg || pub->func->computesecret == NULL || priv->func->computesecret == NULL)
...
isc_result_t dst_key_tofile (const dst_key_t * key, int type, const char *directory)
{
    ...
    if ((type & DST_TYPE_PRIVATE) && (key->key_flags & DNS_KEYFLAG_TYPEMASK) != DNS_KEYTYPE_NOKEY)
...
dst_key_fromfile (dns_name_t * name, dns_keytag_t id,
                  unsigned int alg, int type, const char *directory, isc_mem_t * mctx, dst_key_t ** keyp)
{
    ...
    if (!dns_name_equal (name, key->key_name) || id != key->key_id || alg != key->key_alg)
    {
        dst_key_free (&key);
...
isc_result_t
dst_key_fromnamedfile (const char *filename, const char *dirname, int type, isc_mem_t * mctx, dst_key_t ** keyp)
{
    isc_result_t result;
    ...
    if (newfilename == NULL)
        return (ISC_R_NOMEMORY);
    result = addsuffix (newfilename, newfilenamelen, dirname, filename, ".key");
    ...
    key = get_key_struct (pubkey->key_name, pubkey->key_alg,
                          pubkey->key_flags, pubkey->key_proto, 0, pubkey->key_class, mctx);
    if (key == NULL)
    {
        dst_key_free (&pubkey);
        return (ISC_R_NOMEMORY);
    ...
    result = addsuffix (newfilename, newfilenamelen, dirname, filename, ".private");
    INSIST (result == ISC_R_SUCCESS);
...
isc_result_t dst_key_todns (const dst_key_t * key, isc_buffer_t * target)
{
    ...
    if (key->key_flags & DNS_KEYFLAG_EXTENDED)
    {
        if (isc_buffer_availablelength (target) < 2)
            return (ISC_R_NOSPACE);
        isc_buffer_putuint16 (target, (isc_uint16_t) ((key->key_flags >> 16) & 0xffff));
    }
                 ...
                 isc_buffer_t * source, isc_mem_t * mctx, dst_key_t ** keyp)
{
    ...
    result = frombuffer (name, alg, flags, proto, rdclass, source, mctx, &key);
    if (result != ISC_R_SUCCESS)
        return (result);
    key->key_id = id;
    key->key_rid = rid;
...
dst_key_frombuffer (dns_name_t * name, unsigned int alg,
                    ...
                    dns_rdataclass_t rdclass, isc_buffer_t * source, isc_mem_t * mctx, dst_key_t ** keyp)
{
    dst_key_t *key = NULL;
    ...
    result = frombuffer (name, alg, flags, protocol, rdclass, source, mctx, &key);
    if (result != ISC_R_SUCCESS)
        return (result);
...
isc_result_t
dst_key_fromgssapi (dns_name_t * name, gss_ctx_id_t gssctx, isc_mem_t * mctx, dst_key_t ** keyp, isc_region_t * intoken)
{
    dst_key_t *key;
    ...
    key = get_key_struct (name, DST_ALG_GSSAPI, 0, DNS_KEYPROTO_DNSSEC, 0, dns_rdataclass_in, mctx);
    if (key == NULL)
        return (ISC_R_NOMEMORY);
    ...
    if (intoken != NULL)
    {
         ...
         */
        RETERR (isc_buffer_allocate (key->mctx, &key->key_tkeytoken, intoken->length));
...
dst_key_fromlabel (dns_name_t * name, int alg, unsigned int flags,
                   ...
                   const char *engine, const char *label, const char *pin, isc_mem_t * mctx, dst_key_t ** keyp)
{
    dst_key_t *key;
...
{
    return (dst_key_generate2 (name, alg, bits, param, flags, protocol, rdclass, mctx, keyp, NULL));
}
...
dst_key_generate2 (dns_name_t * name, unsigned int alg,
                   unsigned int bits, unsigned int param,
                   unsigned int flags, unsigned int protocol,
                   dns_rdataclass_t rdclass, isc_mem_t * mctx, dst_key_t ** keyp, void (*callback) (int))
{
    dst_key_t *key;
...
comparekeys (const dst_key_t * key1, const dst_key_t * key2,
             isc_boolean_t match_revoked_key, isc_boolean_t (*compare) (const dst_key_t * key1, const dst_key_t * key2))
{
    REQUIRE (dst_initialized == ISC_TRUE);
    ...
    if (key1->key_id != key2->key_id)
    {
        ...
        if (key1->key_alg == DST_ALG_RSAMD5)
            return (ISC_FALSE);
        if ((key1->key_flags & DNS_KEYFLAG_REVOKE) == (key2->key_flags & DNS_KEYFLAG_REVOKE))
            return (ISC_FALSE);
        if (key1->key_id != key2->key_rid && key1->key_rid != key2->key_id)
            return (ISC_FALSE);
    }
...
isc_boolean_t dst_key_paramcompare (const dst_key_t * key1, const dst_key_t * key2)
{
    ...
    if (key1 == NULL || key2 == NULL)
        return (ISC_FALSE);
    if (key1->key_alg == key2->key_alg &&
        key1->func->paramcompare != NULL && key1->func->paramcompare (key1, key2) == ISC_TRUE)
        return (ISC_TRUE);
    else
        return (ISC_FALSE);
}
...
isc_result_t dst_key_buildfilename (const dst_key_t * key, int type, const char *directory, isc_buffer_t * out)
{
    ...
    REQUIRE (VALID_KEY (key));
    REQUIRE (type == DST_TYPE_PRIVATE || type == DST_TYPE_PUBLIC || type == 0);

    return (buildfilename (key->key_name, key->key_id, key->key_alg, type, directory, out));
}
...
void dst_key_format (const dst_key_t * key, char *cp, unsigned int size)
{
    ...
    dns_secalg_format ((dns_secalg_t) dst_key_alg (key), algstr, sizeof (algstr));
 ...
 */
static dst_key_t *get_key_struct (dns_name_t * name, unsigned int alg,
                                  unsigned int flags, unsigned int protocol,
                                  unsigned int bits, dns_rdataclass_t rdclass, isc_mem_t * mctx)
{
    dst_key_t *key;
...
#define BADTOKEN() { \
    ...
    ret = dns_name_fromtext (dns_fixedname_name (&name), &b, dns_rootname, 0, NULL);
    if (ret != ISC_R_SUCCESS)
        goto cleanup;
    ...
    ret = dns_rdata_fromtext (&rdata, rdclass, keytype, lex, NULL, ISC_FALSE, mctx, &b, NULL);
    if (ret != ISC_R_SUCCESS)
        goto cleanup;

    ret = dst_key_fromdns (dns_fixedname_name (&name), rdclass, &b, mctx, keyp);
    if (ret != ISC_R_SUCCESS)
        goto cleanup;
...
static void printtime (const dst_key_t * key, int type, const char *tag, FILE * stream)
{
    ...
    fprintf (stream, "%s: %.*s (%.*s)\n", tag, (int) r.length, r.base, (int) strlen (output) - 1, output);
    return;
...
static isc_result_t write_public_key (const dst_key_t * key, int type, const char *directory)
{
    ...
    {
        access = 0;
        isc_fsaccess_add (ISC_FSACCESS_OWNER, ISC_FSACCESS_READ | ISC_FSACCESS_WRITE, &access);
    ...
    if ((type & DST_TYPE_KEY) == 0)
    {
        fprintf (fp, "; This is a %s%s-signing key, keyid %d, for ",
                 ...
                 "revoked " : "", (key->key_flags & DNS_KEYFLAG_KSK) != 0 ? "key" : "zone", key->key_id);
               ...
               unsigned int alg, unsigned int type, const char *directory, isc_buffer_t * out)
{
    ...
    if (directory != NULL)
    {
        ...
        if (strlen (directory) > 0U && directory[strlen (directory) - 1] != '/')
...
frombuffer (dns_name_t * name, unsigned int alg, unsigned int flags,
            unsigned int protocol, dns_rdataclass_t rdclass, isc_buffer_t * source, isc_mem_t * mctx, dst_key_t ** keyp)
{
    dst_key_t *key;
...
static isc_result_t algorithm_status (unsigned int alg)
{
    ...
    if (alg == DST_ALG_RSAMD5 || alg == DST_ALG_RSASHA1 ||
        ...
        alg == DST_ALG_NSEC3RSASHA1 || alg == DST_ALG_RSASHA256 || alg == DST_ALG_RSASHA512 || alg == DST_ALG_ECCGOST)
        return (DST_R_NOCRYPTO);
#endif
    return (DST_R_UNSUPPORTEDALG);
}

static isc_result_t addsuffix (char *filename, int len, const char *odirname, const char *ofilename, const char *suffix)
{
    ...
    if (odirname == NULL)
        ...
        n = snprintf (filename, len, "%s/%.*s%s", odirname, olen, ofilename, suffix);
    if (n < 0)
...
unsigned int dst__entropy_status (void)
{
    ...
    if (first)
    {
        ...
        flags &= ~ISC_ENTROPY_GOODONLY;
        ret = isc_entropy_getdata (dst_entropy_pool, buf, sizeof (buf), NULL, flags);
        INSIST (ret == ISC_R_SUCCESS);
        isc_entropy_putdata (dst_entropy_pool, buf, sizeof (buf), 2 * sizeof (buf));
        first = ISC_FALSE;
    }
#endif
