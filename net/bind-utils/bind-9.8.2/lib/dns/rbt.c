...
static inline dns_rbtnode_t *find_up (dns_rbtnode_t * node)
{
    dns_rbtnode_t *root;
...
#ifdef DNS_RBT_USEHASH
static inline void hash_node (dns_rbt_t * rbt, dns_rbtnode_t * node, dns_name_t * name);
...
static void dns_rbt_addonlevel (dns_rbtnode_t * node, dns_rbtnode_t * current, int order, dns_rbtnode_t ** rootp);

static void dns_rbt_deletefromlevel (dns_rbtnode_t * delete, dns_rbtnode_t ** rootp);

static isc_result_t dns_rbt_deletetree (dns_rbt_t * rbt, dns_rbtnode_t * node);

static void dns_rbt_deletetreeflat (dns_rbt_t * rbt, unsigned int quantum, dns_rbtnode_t ** nodep);

/*
 * Initialize a red/black tree of trees.
 */
isc_result_t dns_rbt_create (isc_mem_t * mctx, void (*deleter) (void *, void *), void *deleter_arg, dns_rbt_t ** rbtp)
{
#ifdef DNS_RBT_USEHASH
    isc_result_t result;
#endif
...
isc_result_t dns_rbt_destroy2 (dns_rbt_t ** rbtp, unsigned int quantum)
{
    ...
    if (rbt->hashtable != NULL)
        isc_mem_put (rbt->mctx, rbt->hashtable, rbt->hashsize * sizeof (dns_rbtnode_t *));
...
static inline isc_result_t chain_name (dns_rbtnodechain_t * chain, dns_name_t * name, isc_boolean_t include_chain_end)
{
    dns_name_t nodename;
...
static inline isc_result_t move_chain_to_last (dns_rbtnodechain_t * chain, dns_rbtnode_t * node)
{
    do
    {
        /*
...
isc_result_t dns_rbt_addnode (dns_rbt_t * rbt, dns_name_t * name, dns_rbtnode_t ** nodep)
{
    ...
    do
    {
        ...
        compared = dns_name_fullcompare (add_name, &current_name, &order, &common_labels);
        ...
        else
        {
            ...
            if (compared == dns_namereln_subdomain)
            {
                 ...
                 * in the next level.
                 */
                dns_name_split (add_name, common_labels, add_name, NULL);
                ...
                INSIST (*root == NULL || (IS_ROOT (*root) && PARENT (*root) == current));
            ...
            else
            {
                ...
                INSIST (compared == dns_namereln_commonancestor || compared == dns_namereln_contains);
                 ...
                 * for time.
                 */
                if (chain.level_count == (sizeof (chain.levels) / sizeof (*chain.levels)))
                {
                 ...
                 * parts of them.
                 */
                dns_name_split (&current_name, common_labels, prefix, suffix);
                result = create_node (rbt->mctx, suffix, &new_current);
                ...
                if (RIGHT (new_current) != NULL)
                    PARENT (RIGHT (new_current)) = new_current;
                if (*root == current)
                    *root = new_current;
                ...
                rbt->nodecount++;
                dns_name_getlabelsequence (name, nlabels - hlabels, hlabels, new_name);
                hash_node (rbt, new_current, new_name);

                if (common_labels == dns_name_countlabels (add_name))
                {
                    /*
...
isc_result_t dns_rbt_addname (dns_rbt_t * rbt, dns_name_t * name, void *data)
{
     ...
     */
    if (result == ISC_R_SUCCESS || (result == ISC_R_EXISTS && DATA (node) == NULL))
    {
        DATA (node) = data;
...
dns_rbt_findnode (dns_rbt_t * rbt, dns_name_t * name, dns_name_t * foundname,
                  ...
                  unsigned int options, dns_rbtfindcallback_t callback, void *callback_arg)
{
    ...
    {
        NODENAME (current, &current_name);
        compared = dns_name_fullcompare (search_name, &current_name, &order, &common_labels);
        ...
        if (compared == dns_namereln_none)
        {
          ...
          hashagain:
             ...
             * Hash includes tail.
             */
            dns_name_getlabelsequence (name, nlabels - tlabels, hlabels + tlabels, &hash_name);
            hash = dns_name_fullhash (&hash_name, ISC_FALSE);
            dns_name_getlabelsequence (search_name, nlabels - tlabels, tlabels, &hash_name);

            for (hnode = rbt->hashtable[hash % rbt->hashsize]; hnode != NULL; hnode = hnode->hashnext)
            {
        ...
        else
        {
            ...
            if (compared == dns_namereln_subdomain)
            {
              subdomain:
                 ...
                 */
                dns_name_split (search_name, common_labels, search_name, NULL);
                hlabels += common_labels;
                /*
                 * This might be the closest enclosing name.
                 */
                if (DATA (current) != NULL || (options & DNS_RBTFIND_EMPTYDATA) != 0)
                    *node = current;
                ...
                {
                    result = chain_name (chain, callback_name, ISC_FALSE);
                    if (result != ISC_R_SUCCESS)
                    {
                        dns_rbtnodechain_reset (chain);
                        return (result);
                    ...
                    result = (callback) (current, callback_name, callback_arg);
    ...
    else
    {
        ...
        if (*node != NULL)
        {
            ...
            if (foundname != NULL)
            {
                ...
                result = chain_name (chain, foundname, ISC_FALSE);
        ...
        else
        {
            ...
            if (compared == dns_namereln_subdomain)
            {
                ...
                INSIST (chain->level_matches < chain->level_count);
                chain->end = chain->levels[--chain->level_count];
            ...
            else
            {
                ...
                if (order > 0)
                {
                    if (DOWN (current) != NULL)
                    {
                        ...
                        result2 = move_chain_to_last (chain, DOWN (current));
                ...
                else
                {
                    ...
                    result2 = dns_rbtnodechain_prev (chain, NULL, NULL);
                    if (result2 == ISC_R_SUCCESS || result2 == DNS_R_NEWORIGIN)
                        ;        /* Nothing. */
 ...
 */
isc_result_t
dns_rbt_findname (dns_rbt_t * rbt, dns_name_t * name, unsigned int options, dns_name_t * foundname, void **data)
{
    ...
    if (node != NULL && (DATA (node) != NULL || (options & DNS_RBTFIND_EMPTYDATA) != 0))
        *data = DATA (node);
    else
        result = ISC_R_NOTFOUND;
...
isc_result_t dns_rbt_deletename (dns_rbt_t * rbt, dns_name_t * name, isc_boolean_t recurse)
{
     ...
     */
    result = dns_rbt_findnode (rbt, name, NULL, &node, NULL, DNS_RBTFIND_NOOPTIONS, NULL, NULL);
...
isc_result_t dns_rbt_deletenode (dns_rbt_t * rbt, dns_rbtnode_t * node, isc_boolean_t recurse)
{
    ...
    {
        if (recurse)
            RUNTIME_CHECK (dns_rbt_deletetree (rbt, DOWN (node)) == ISC_R_SUCCESS);
        else
        {
     ...
     */
    dns_rbt_deletefromlevel (node, parent == NULL ? &rbt->root : &DOWN (parent));
...
char *dns_rbt_formatnodename (dns_rbtnode_t * node, char *printname, unsigned int size)
{
    ...
    if (result == ISC_R_SUCCESS)
        ...
        snprintf (printname, size, "<error building name: %s>", dns_result_totext (result));
...
static isc_result_t create_node (isc_mem_t * mctx, dns_name_t * name, dns_rbtnode_t ** nodep)
{
     ...
     */
    node = (dns_rbtnode_t *) isc_mem_get (mctx, sizeof (*node) + region.length + labels + 1);
...
#ifdef DNS_RBT_USEHASH
static inline void hash_add_node (dns_rbt_t * rbt, dns_rbtnode_t * node, dns_name_t * name)
{
    unsigned int hash;
...
static void rehash (dns_rbt_t * rbt)
{
    ...
    rbt->hashtable = isc_mem_get (rbt->mctx, rbt->hashsize * sizeof (dns_rbtnode_t *));
    if (rbt->hashtable == NULL)
    {
...
static void dns_rbt_addonlevel (dns_rbtnode_t * node, dns_rbtnode_t * current, int order, dns_rbtnode_t ** rootp)
{
    ...
    REQUIRE (rootp != NULL);
    REQUIRE (DNS_RBTNODE_VALID (node) && LEFT (node) == NULL && RIGHT (node) == NULL);
...
static void dns_rbt_deletefromlevel (dns_rbtnode_t * delete, dns_rbtnode_t ** rootp)
{
    ...
    INSIST ((IS_ROOT (delete) && *rootp == delete) ||
            (!IS_ROOT (delete) && (LEFT (PARENT (delete)) == delete || RIGHT (PARENT (delete)) == delete)));
    ...
    else
    {
        ...
        else if (LEFT (PARENT (delete)) == delete)
...
static void dns_rbt_indent (int depth)
{
    int i;
...
static void dns_rbt_printtree (dns_rbtnode_t * root, dns_rbtnode_t * parent, int depth)
{
...
isc_result_t
dns_rbtnodechain_current (dns_rbtnodechain_t * chain, dns_name_t * name, dns_name_t * origin, dns_rbtnode_t ** node)
{
    isc_result_t result = ISC_R_SUCCESS;
...
isc_result_t dns_rbtnodechain_prev (dns_rbtnodechain_t * chain, dns_name_t * name, dns_name_t * origin)
{
    ...
    else if (chain->level_count > 0)
    {
         ...
         */
        if (origin != NULL && (chain->level_count > 0 || OFFSETLEN (predecessor) > 1))
            new_origin = ISC_TRUE;
    }
    ...
    if (predecessor != NULL)
    {
        ...
        if (new_origin)
        {
            ...
            result = dns_rbtnodechain_current (chain, name, NULL, NULL);
...
isc_result_t
dns_rbtnodechain_first (dns_rbtnodechain_t * chain, dns_rbt_t * rbt, dns_name_t * name, dns_name_t * origin)
{
    isc_result_t result;
