...
static isc_result_t
maybe_numeric (unsigned int *valuep, isc_textregion_t * source, unsigned int max, isc_boolean_t hex_allowed)
{
    isc_result_t result;
...
static isc_result_t
dns_mnemonic_fromtext (unsigned int *valuep, isc_textregion_t * source, struct tbl *table, unsigned int max)
{
    isc_result_t result;
    ...
    for (i = 0; table[i].name != NULL; i++)
    {
        ...
        if (n == source->length && strncasecmp (source->base, table[i].name, n) == 0)
        {
...
#define COMPARE(string, rdclass) \
    ...
    switch (tolower ((unsigned char) source->base[0]))
    {
        ...
        case 'c':
            ...
            if (source->length > 5 &&
                source->length < (5 + sizeof ("65000")) && strncasecmp ("class", source->base, 5) == 0)
            {
