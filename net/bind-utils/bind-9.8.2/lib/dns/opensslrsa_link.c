...
static isc_result_t opensslrsa_createctx (dst_key_t * key, dst_context_t * dctx)
{
#if USE_EVP
    ...
    REQUIRE (dctx->key->key_alg == DST_ALG_RSAMD5 ||
             ...
             dctx->key->key_alg == DST_ALG_RSASHA256 || dctx->key->key_alg == DST_ALG_RSASHA512);
        ...
        case DST_ALG_RSASHA256:
            {
                ...
                sha256ctx = isc_mem_get (dctx->mctx, sizeof (isc_sha256_t));
                if (sha256ctx == NULL)
                    return (ISC_R_NOMEMORY);
        ...
        case DST_ALG_RSASHA512:
            {
                ...
                sha512ctx = isc_mem_get (dctx->mctx, sizeof (isc_sha512_t));
                if (sha512ctx == NULL)
                    return (ISC_R_NOMEMORY);
...
static void opensslrsa_destroyctx (dst_context_t * dctx)
{
#if USE_EVP
        ...
        case DST_ALG_RSASHA256:
            {
                ...
                if (sha256ctx != NULL)
                {
                    ...
                    isc_mem_put (dctx->mctx, sha256ctx, sizeof (isc_sha256_t));
        ...
        case DST_ALG_RSASHA512:
            {
                ...
                if (sha512ctx != NULL)
                {
                    ...
                    isc_mem_put (dctx->mctx, sha512ctx, sizeof (isc_sha512_t));
...
static isc_result_t opensslrsa_adddata (dst_context_t * dctx, const isc_region_t * data)
{
#if USE_EVP
    EVP_MD_CTX *evp_md_ctx = dctx->ctxdata.evp_md_ctx;
#endif
 ...
 */
static unsigned char sha256_prefix[] = { 0x30, 0x31, 0x30, 0x0d, 0x06, 0x09, 0x60, 0x86, 0x48,
    0x01, 0x65, 0x03, 0x04, 0x02, 0x01, 0x05, 0x00, 0x04, 0x20
};

static unsigned char sha512_prefix[] = { 0x30, 0x51, 0x30, 0x0d, 0x06, 0x09, 0x60, 0x86, 0x48,
...
static isc_result_t opensslrsa_sign (dst_context_t * dctx, isc_buffer_t * sig)
{
    ...
    switch (dctx->key->key_alg)
    {
        ...
        case DST_ALG_RSASHA512:
            ...
            status = RSA_private_encrypt (digestlen + prefixlen, digest, r.base, rsa, RSA_PKCS1_PADDING);
            if (status < 0)
                status = 0;
            else
                siglen = status;
...
static isc_result_t opensslrsa_verify (dst_context_t * dctx, const isc_region_t * sig)
{
    dst_key_t *key = dctx->key;
    ...
    switch (dctx->key->key_alg)
    {
        ...
        case DST_ALG_NSEC3RSASHA1:
            INSIST (type != 0);
            status = RSA_verify (type, digest, digestlen, sig->base, RSA_size (rsa), rsa);
            break;
        ...
        case DST_ALG_RSASHA512:
            {
                ...
                status = RSA_public_decrypt (sig->length, sig->base, original, rsa, RSA_PKCS1_PADDING);
                if (status <= 0)
                    return (DST_R_VERIFYFAILURE);
...
static isc_boolean_t opensslrsa_compare (const dst_key_t * key1, const dst_key_t * key2)
{
    int status;
    ...
    status = BN_cmp (rsa1->n, rsa2->n) || BN_cmp (rsa1->e, rsa2->e);

    if (status != 0)
        return (ISC_FALSE);

#if USE_EVP
    if ((rsa1->flags & RSA_FLAG_EXT_PKEY) != 0 || (rsa2->flags & RSA_FLAG_EXT_PKEY) != 0)
    {
        if ((rsa1->flags & RSA_FLAG_EXT_PKEY) == 0 || (rsa2->flags & RSA_FLAG_EXT_PKEY) == 0)
            return (ISC_FALSE);
        /*
    ...
    if (rsa1->d != NULL || rsa2->d != NULL)
    {
        if (rsa1->d == NULL || rsa2->d == NULL)
            return (ISC_FALSE);
        status = BN_cmp (rsa1->d, rsa2->d) || BN_cmp (rsa1->p, rsa2->p) || BN_cmp (rsa1->q, rsa2->q);
...
static isc_result_t opensslrsa_generate (dst_key_t * key, int exp, void (*callback) (int))
{
#if OPENSSL_VERSION_NUMBER > 0x00908000L
    BN_GENCB cb;
...
static isc_boolean_t opensslrsa_isprivate (const dst_key_t * key)
{
#if USE_EVP
...
static isc_result_t opensslrsa_fromdns (dst_key_t * key, isc_buffer_t * data)
{
    RSA *rsa;
...
static isc_result_t opensslrsa_tofile (const dst_key_t * key, const char *directory)
{
    int i;
...
static isc_result_t rsa_check (RSA * rsa, RSA * pub)
{
...
static isc_result_t opensslrsa_parse (dst_key_t * key, isc_lex_t * lexer, dst_key_t * pub)
{
    dst_private_t priv;
    ...
    for (i = 0; i < priv.nelements; i++)
    {
        ...
        switch (priv.elements[i].tag)
        {
            ...
            case TAG_RSA_PIN:
                continue;
            default:
                bn = BN_bin2bn (priv.elements[i].data, priv.elements[i].length, NULL);
                if (bn == NULL)
                    DST_RET (ISC_R_NOMEMORY);
        }
...
static isc_result_t opensslrsa_fromlabel (dst_key_t * key, const char *engine, const char *label, const char *pin)
{
#ifdef USE_ENGINE
    ENGINE *e = NULL;
...
isc_result_t dst__opensslrsa_init (dst_func_t ** funcp, unsigned char algorithm)
{
    REQUIRE (funcp != NULL);
