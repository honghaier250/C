...
towiresorted (dns_rdataset_t * rdataset, const dns_name_t * owner_name,
              ...
              isc_boolean_t partial, unsigned int options, unsigned int *countp, void **state)
{
    ...
    else if ((rdataset->attributes & DNS_RDATASETATTR_NEGATIVE) != 0)
    {
        ...
        return (dns_ncache_towire (rdataset, cctx, target, ncache_opts, countp));
    }
    else
    {
     ...
     */
    if (!question && count > 1 && (!WANT_FIXED (rdataset) || order != NULL) && rdataset->type != dns_rdatatype_rrsig)
        shuffle = ISC_TRUE;
    ...
    if (shuffle)
    {
        ...
        else
        {
            ...
            {
                if (order != NULL)
                    sorted[i].key = (*order) (&shuffled[j], order_arg);
                else
                    sorted[i].key = 0;    /* Unused */
         ...
         */
        if (order != NULL)
            qsort (sorted, count, sizeof (sorted[0]), towire_compare);
    }
    ...
    do
    {
        ...
        if (!question)
        {
            ...
            if (result != ISC_R_SUCCESS)
                goto rollback;
            INSIST ((target->used >= rdlen.used + 2) && (target->used - rdlen.used - 2 < 65536));
            isc_buffer_putuint16 (&rdlen, (isc_uint16_t) (target->used - rdlen.used - 2));
            added++;
        }
...
dns_rdataset_towiresorted (dns_rdataset_t * rdataset,
                           ...
                           const void *order_arg, unsigned int options, unsigned int *countp)
{
    return (towiresorted (rdataset, owner_name, cctx, target, order, order_arg, ISC_FALSE, options, countp, NULL));
}
...
dns_rdataset_towirepartial (dns_rdataset_t * rdataset,
                            ...
                            isc_buffer_t * target,
                            dns_rdatasetorderfunc_t order,
                            const void *order_arg, unsigned int options, unsigned int *countp, void **state)
{
    REQUIRE (state == NULL);    /* XXX remove when implemented */
    return (towiresorted (rdataset, owner_name, cctx, target, order, order_arg, ISC_TRUE, options, countp, state));
}

isc_result_t
dns_rdataset_towire (dns_rdataset_t * rdataset,
                     dns_name_t * owner_name,
                     dns_compress_t * cctx, isc_buffer_t * target, unsigned int options, unsigned int *countp)
{
    return (towiresorted (rdataset, owner_name, cctx, target, NULL, NULL, ISC_FALSE, options, countp, NULL));
}
...
isc_result_t
dns_rdataset_getnoqname (dns_rdataset_t * rdataset, dns_name_t * name, dns_rdataset_t * neg, dns_rdataset_t * negsig)
{
    REQUIRE (DNS_RDATASET_VALID (rdataset));
    REQUIRE (rdataset->methods != NULL);
...
isc_result_t
dns_rdataset_getclosest (dns_rdataset_t * rdataset, dns_name_t * name, dns_rdataset_t * neg, dns_rdataset_t * negsig)
{
    REQUIRE (DNS_RDATASET_VALID (rdataset));
    REQUIRE (rdataset->methods != NULL);
                            ...
                            dns_dbnode_t ** nodep, dns_name_t * fname, dns_message_t * msg, isc_stdtime_t now)
{
    ...
    if (acache != NULL && rdataset->methods->getadditional != NULL)
    {
        return ((rdataset->methods->getadditional) (rdataset, type,
                                                    qtype, acache, zonep, dbp, versionp, nodep, fname, msg, now));
    }
...
dns_rdataset_setadditional (dns_rdataset_t * rdataset,
                            ...
                            dns_acache_t * acache,
                            dns_zone_t * zone,
                            dns_db_t * db, dns_dbversion_t * version, dns_dbnode_t * node, dns_name_t * fname)
{
    REQUIRE (DNS_RDATASET_VALID (rdataset));
    REQUIRE (rdataset->methods != NULL);
    ...
    {
        return ((rdataset->methods->setadditional) (rdataset, type, qtype, acache, zone, db, version, node, fname));
    }
...
dns_rdataset_putadditional (dns_acache_t * acache,
                            dns_rdataset_t * rdataset, dns_rdatasetadditional_t type, dns_rdatatype_t qtype)
{
    REQUIRE (DNS_RDATASET_VALID (rdataset));
    REQUIRE (rdataset->methods != NULL);
    ...
    {
        return ((rdataset->methods->putadditional) (acache, rdataset, type, qtype));
    }
