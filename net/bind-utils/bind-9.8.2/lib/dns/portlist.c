...
isc_result_t dns_portlist_add (dns_portlist_t * portlist, int af, in_port_t port)
{
    ...
    if (portlist->allocated <= portlist->active)
    {
        ...
        {
            memcpy (el, portlist->list, portlist->allocated * sizeof (*el));
            isc_mem_put (portlist->mctx, portlist->list, portlist->allocated * sizeof (*el));
        }
        portlist->list = el;
...
void dns_portlist_detach (dns_portlist_t ** portlistp)
{
    ...
    *portlistp = NULL;
    ...
    if (count == 0)
    {
        ...
        if (portlist->list != NULL)
            isc_mem_put (portlist->mctx, portlist->list, portlist->allocated * sizeof (*portlist->list));
        DESTROYLOCK (&portlist->lock);
        isc_mem_putanddetach (&portlist->mctx, portlist, sizeof (*portlist));
    }
}
