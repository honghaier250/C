 ...
 */
     ...
     static isc_result_t rdataset_getnoqname (dns_rdataset_t * rdataset,
                                              dns_name_t * name, dns_rdataset_t * neg, dns_rdataset_t * negsig);
     ...
     static isc_result_t rdataset_getadditional (dns_rdataset_t * rdataset,
                                                 ...
                                                 dns_dbnode_t ** nodep,
                                                 dns_name_t * fname, dns_message_t * msg, isc_stdtime_t now);
     static isc_result_t rdataset_setadditional (dns_rdataset_t * rdataset,
                                                 ...
                                                 dns_zone_t * zone,
                                                 dns_db_t * db,
                                                 dns_dbversion_t * version, dns_dbnode_t * node, dns_name_t * fname);
     static isc_result_t rdataset_putadditional (dns_acache_t * acache,
                                                 dns_rdataset_t * rdataset,
                                                 dns_rdatasetadditional_t type, dns_rdatatype_t qtype);
     static inline isc_boolean_t need_headerupdate (rdatasetheader_t * header, isc_stdtime_t now);

     static void update_header (dns_rbtdb_t * rbtdb, rdatasetheader_t * header, isc_stdtime_t now);

     static void expire_header (dns_rbtdb_t * rbtdb, rdatasetheader_t * header, isc_boolean_t tree_locked);
...
static void free_rbtdb (dns_rbtdb_t * rbtdb, isc_boolean_t log, isc_event_t * event)
{
    ...
    if (rbtdb->current_version != NULL)
    {
        ...
        isc_refcount_decrement (&rbtdb->current_version->references, &refs);
        INSIST (refs == 0);
        UNLINK (rbtdb->open_versions, rbtdb->current_version, link);
        isc_refcount_destroy (&rbtdb->current_version->references);
        isc_mem_put (rbtdb->common.mctx, rbtdb->current_version, sizeof (rbtdb_version_t));
    }
    ...
    for (;;)
    {
        ...
        if (result == ISC_R_QUOTA)
        {
            ...
            if (rbtdb->quantum != 0)
                rbtdb->quantum = adjust_quantum (rbtdb->quantum, &start);
            if (event == NULL)
                event = isc_event_allocate (rbtdb->common.mctx,
                                            NULL,
                                            DNS_EVENT_FREESTORAGE, free_rbtdb_callback, rbtdb, sizeof (isc_event_t));
            if (event == NULL)
                continue;
    ...
    if (log)
    {
        if (dns_name_dynamic (&rbtdb->common.origin))
            dns_name_format (&rbtdb->common.origin, buf, sizeof (buf));
        else
            strcpy (buf, "<UNKNOWN>");
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_CACHE, ISC_LOG_DEBUG (1), "done free_rbtdb(%s)", buf);
    }
    if (dns_name_dynamic (&rbtdb->common.origin))
    ...
    if (rbtdb->rdatasets != NULL)
    {
        ...
        isc_mem_put (rbtdb->common.mctx, rbtdb->rdatasets, rbtdb->node_lock_count * sizeof (rdatasetheaderlist_t));
    }
    /*
    ...
    if (rbtdb->deadnodes != NULL)
    {
        ...
        isc_mem_put (rbtdb->common.mctx, rbtdb->deadnodes, rbtdb->node_lock_count * sizeof (rbtnodelist_t));
    }
    /*
    ...
    if (rbtdb->heaps != NULL)
    {
        ...
        isc_mem_put (rbtdb->hmctx, rbtdb->heaps, rbtdb->node_lock_count * sizeof (isc_heap_t *));
    }
    ...
    isc_mem_put (rbtdb->common.mctx, rbtdb->node_locks, rbtdb->node_lock_count * sizeof (rbtdb_nodelock_t));
...
static inline void maybe_free_rbtdb (dns_rbtdb_t * rbtdb)
{
    ...
    if (inactive != 0)
    {
        ...
        if (want_free)
        {
            ...
            if (dns_name_dynamic (&rbtdb->common.origin))
                ...
                strcpy (buf, "<UNKNOWN>");
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           DNS_LOGMODULE_CACHE, ISC_LOG_DEBUG (1), "calling free_rbtdb(%s)", buf);
...
static inline rbtdb_version_t *allocate_version (isc_mem_t * mctx, rbtdb_serial_t serial,
...
static isc_result_t newversion (dns_db_t * db, dns_dbversion_t ** versionp)
{
    ...
    version = allocate_version (rbtdb->common.mctx, rbtdb->next_serial, 1, ISC_TRUE);
    if (version != NULL)
    {
        version->rbtdb = rbtdb;
        ...
        if (version->havensec3)
        {
            ...
            memcpy (version->salt, rbtdb->current_version->salt, version->salt_length);
        }
        else
        {
            version->flags = 0;
...
static inline void free_noqname (isc_mem_t * mctx, struct noqname **noqname)
{

    if (dns_name_dynamic (&(*noqname)->name))
        dns_name_free (&(*noqname)->name, mctx);
    if ((*noqname)->neg != NULL)
        isc_mem_put (mctx, (*noqname)->neg, dns_rdataslab_size ((*noqname)->neg, 0));
    if ((*noqname)->negsig != NULL)
        isc_mem_put (mctx, (*noqname)->negsig, dns_rdataslab_size ((*noqname)->negsig, 0));
    isc_mem_put (mctx, *noqname, sizeof (**noqname));
    *noqname = NULL;
}

static inline void init_rdataset (dns_rbtdb_t * rbtdb, rdatasetheader_t * h)
{
...
static inline rdatasetheader_t *new_rdataset (dns_rbtdb_t * rbtdb, isc_mem_t * mctx)
{
    rdatasetheader_t *h;
...
static inline void free_rdataset (dns_rbtdb_t * rbtdb, isc_mem_t * mctx, rdatasetheader_t * rdataset)
{
    unsigned int size;

    int idx;

    if (EXISTS (rdataset) && (rdataset->attributes & RDATASET_ATTR_STATCOUNT) != 0)
    {
...
static inline void rollback_node (dns_rbtnode_t * node, rbtdb_serial_t serial)
{
    ...
    for (header = node->data; header != NULL; header = header->next)
    {
        ...
        for (dcurrent = header->down; dcurrent != NULL; dcurrent = dcurrent->down)
        {
...
static inline void clean_stale_headers (dns_rbtdb_t * rbtdb, isc_mem_t * mctx, rdatasetheader_t * top)
{
...
static inline void clean_cache_node (dns_rbtdb_t * rbtdb, dns_rbtnode_t * node)
{
    ...
    for (current = node->data; current != NULL; current = top_next)
    {
         ...
         */
        if ((current->attributes & (RDATASET_ATTR_NONEXISTENT | RDATASET_ATTR_STALE)) != 0)
        {
            if (top_prev != NULL)
...
static inline void clean_zone_node (dns_rbtdb_t * rbtdb, dns_rbtnode_t * node, rbtdb_serial_t least_serial)
{
    ...
    for (current = node->data; current != NULL; current = top_next)
    {
         ...
         */
        dparent = current;
        for (dcurrent = current->down; dcurrent != NULL; dcurrent = down_next)
        {
            down_next = dcurrent->down;
            INSIST (dcurrent->serial <= dparent->serial);
            if (dcurrent->serial == dparent->serial || IGNORE (dcurrent))
            {
                if (down_next != NULL)
...
static void delete_node (dns_rbtdb_t * rbtdb, dns_rbtnode_t * node)
{
    ...
    switch (node->nsec)
    {
        ...
        case DNS_RBT_NSEC_HAS_NSEC:
             ...
             */
            nsecnode = NULL;
            result = dns_rbt_findnode (rbtdb->nsec, name, NULL, &nsecnode, NULL, DNS_RBTFIND_EMPTYDATA, NULL, NULL);
            if (result != ISC_R_SUCCESS)
            {
                isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                               DNS_LOGMODULE_CACHE, ISC_LOG_WARNING,
                               "delete_node: " "dns_rbt_findnode(nsec): %s", isc_result_totext (result));
            }
            else
            {
                result = dns_rbt_deletenode (rbtdb->nsec, nsecnode, ISC_FALSE);
                if (result != ISC_R_SUCCESS)
                {
                    isc_log_write (dns_lctx,
                                   ...
                                   ISC_LOG_WARNING,
                                   "delete_nsecnode(): "
                                   "dns_rbt_deletenode(nsecnode): %s", isc_result_totext (result));
                }
            }
            result = dns_rbt_deletenode (rbtdb->tree, node, ISC_FALSE);
#ifdef BIND9
    ...
    if (result != ISC_R_SUCCESS)
    {
        isc_log_write (dns_lctx,
                       DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_CACHE,
                       ISC_LOG_WARNING, "delete_nsecnode(): " "dns_rbt_deletenode: %s", isc_result_totext (result));
    }
}
...
static void cleanup_dead_nodes (dns_rbtdb_t * rbtdb, int bucketnum)
{
    ...
    while (node != NULL && count > 0)
    {
         ...
         */
        INSIST (dns_rbtnode_refcurrent (node) == 0 && node->data == NULL);
 ...
 * Caller must be holding the node lock.
 */
static inline void new_reference (dns_rbtdb_t * rbtdb, dns_rbtnode_t * node)
{
 ...
 */
static inline void reactivate_node (dns_rbtdb_t * rbtdb, dns_rbtnode_t * node, isc_rwlocktype_t treelocktype)
{
     ...
     */
    if (!ISC_LIST_EMPTY (rbtdb->deadnodes[node->locknum]) && treelocktype == isc_rwlocktype_write)
    {
    ...
    if (ISC_LINK_LINKED (node, deadlink) || maybe_cleanup)
    {
        ...
        if (ISC_LINK_LINKED (node, deadlink))
            ISC_LIST_UNLINK (rbtdb->deadnodes[node->locknum], node, deadlink);
        if (maybe_cleanup)
...
decrement_reference (dns_rbtdb_t * rbtdb, dns_rbtnode_t * node,
                     rbtdb_serial_t least_serial, isc_rwlocktype_t nlock, isc_rwlocktype_t tlock, isc_boolean_t pruning)
{
    isc_result_t result;
    ...
    if (tlock != isc_rwlocktype_write)
    {
        ...
        if (tlock == isc_rwlocktype_read)
            ...
            result = isc_rwlock_trylock (&rbtdb->tree_lock, isc_rwlocktype_write);
    ...
    if (write_locked)
    {
        ...
        if (!pruning && node->parent != NULL &&
            node->parent->down == node && node->left == NULL && node->right == NULL && rbtdb->task != NULL)
        {
            isc_event_t *ev;
            ...
            ev = isc_event_allocate (rbtdb->common.mctx, NULL,
                                     DNS_EVENT_RBTPRUNE, prune_tree, node, sizeof (isc_event_t));
            if (ev != NULL)
            {
                new_reference (rbtdb, node);
                db = NULL;
            ...
            else
            {
                ...
                isc_log_write (dns_lctx,
                               DNS_LOGCATEGORY_DATABASE,
                               DNS_LOGMODULE_CACHE,
                               ISC_LOG_INFO, "decrement_reference: failed to " "allocate pruning event");
        ...
        else
        {
            if (isc_log_wouldlog (dns_lctx, ISC_LOG_DEBUG (1)))
            {
                ...
                isc_log_write (dns_lctx,
                               ...
                               "decrement_reference: "
                               "delete from rbt: %p %s",
                               node, dns_rbt_formatnodename (node, printname, sizeof (printname)));
            }
...
static void prune_tree (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        parent = node->parent;
        decrement_reference (rbtdb, node, 0, isc_rwlocktype_write, isc_rwlocktype_write, ISC_TRUE);
        ...
        if (parent != NULL && parent->down == NULL)
        {
            ...
            if (ISC_LINK_LINKED (parent, deadlink))
                ISC_LIST_UNLINK (rbtdb->deadnodes[locknum], parent, deadlink);
...
static inline void
make_least_version (dns_rbtdb_t * rbtdb, rbtdb_version_t * version, rbtdb_changedlist_t * cleanup_list)
{
    /*
...
static inline void cleanup_nondirty (rbtdb_version_t * version, rbtdb_changedlist_t * cleanup_list)
{
     ...
     */
    for (changed = HEAD (version->changed_list); changed != NULL; changed = next_changed)
    {
...
static void iszonesecure (dns_db_t * db, rbtdb_version_t * version, dns_dbnode_t * origin)
{
    ...
    result = dns_db_findrdataset (db, origin, version, dns_rdatatype_dnskey, 0, 0, &keyset, NULL);
    if (result == ISC_R_SUCCESS)
    {
    ...
    result = dns_db_findrdataset (db, origin, version, dns_rdatatype_nsec, 0, 0, &nsecset, &signsecset);
    if (result == ISC_R_SUCCESS)
    {
...
static void setnsec3parameters (dns_db_t * db, rbtdb_version_t * version)
{
    ...
    NODE_LOCK (&(rbtdb->node_locks[node->locknum].lock), isc_rwlocktype_read);
    for (header = node->data; header != NULL; header = header_next)
    {
        ...
        if (header != NULL && (header->type == dns_rdatatype_nsec3param))
        {
            /*
            ...
            while (count-- > 0U)
            {
                ...
                raw += length;
                dns_rdata_fromregion (&rdata, rbtdb->common.rdclass, dns_rdatatype_nsec3param, &region);
                result = dns_rdata_tostruct (&rdata, &nsec3param, NULL);
                INSIST (result == ISC_R_SUCCESS);
                dns_rdata_reset (&rdata);

                if (nsec3param.hash != DNS_NSEC3_UNKNOWNALG && !dns_nsec3_supportedhash (nsec3param.hash))
                    continue;

                if (nsec3param.flags != 0)
                    continue;

                memcpy (version->salt, nsec3param.salt, nsec3param.salt_length);
  ...
  unlock:
    NODE_UNLOCK (&(rbtdb->node_locks[node->locknum].lock), isc_rwlocktype_read);
...
static void closeversion (dns_db_t * db, dns_dbversion_t ** versionp, isc_boolean_t commit)
{
    ...
    if (version->writer)
    {
        if (commit)
        {
            ...
            isc_refcount_decrement (&cur_version->references, &cur_ref);
            if (cur_ref == 0)
            {
            ...
            if (EMPTY (rbtdb->open_versions))
            {
                 ...
                 * version.
                 */
                make_least_version (rbtdb, version, &cleanup_list);
            }
            else
            {
                /*
            ...
            if (cur_ref == 0)
            {
                ...
                APPENDLIST (version->changed_list, cleanup_version->changed_list, link);
            }
            /*
             ...
             */
            isc_refcount_increment0 (&version->references, &cur_ref);
            INSIST (cur_ref == 1);
    ...
    else
    {
        if (version != rbtdb->current_version)
        {
            ...
            if (version->serial == rbtdb->least_serial)
            {
                 ...
                 * version.
                 */
                make_least_version (rbtdb, least_greater, &cleanup_list);
            }
            else
            {
                /*
                 ...
                 */
                APPENDLIST (least_greater->changed_list, version->changed_list, link);
            }
        }
    ...
    if (cleanup_version != NULL)
    {
        ...
        isc_mem_put (rbtdb->common.mctx, cleanup_version, sizeof (*cleanup_version));
    }

    /*
     * Commit/rollback re-signed headers.
     */
    for (header = HEAD (resigned_list); header != NULL; header = HEAD (resigned_list))
    {
        nodelock_t *lock;
        ...
        decrement_reference (rbtdb, header->node, least_serial, isc_rwlocktype_write, isc_rwlocktype_none, ISC_FALSE);
    ...
    if (!EMPTY (cleanup_list))
    {
        ...
        if (rbtdb->task != NULL)
            event = isc_event_allocate (rbtdb->common.mctx, NULL,
                                        ...
                                        cleanup_dead_nodes_callback, rbtdb, sizeof (isc_event_t));
        if (event == NULL)
        {
            /*
        ...
        for (changed = HEAD (cleanup_list); changed != NULL; changed = next_changed)
        {
            nodelock_t *lock;
            ...
            decrement_reference (rbtdb, rbtnode, least_serial, isc_rwlocktype_write, tlock, ISC_FALSE);

            NODE_UNLOCK (lock, isc_rwlocktype_write);

            isc_mem_put (rbtdb->common.mctx, changed, sizeof (*changed));
        }
        if (event != NULL)
        {
            isc_refcount_increment (&rbtdb->references, NULL);
...
static isc_result_t
findnodeintree (dns_rbtdb_t * rbtdb, dns_rbt_t * tree, dns_name_t * name, isc_boolean_t create, dns_dbnode_t ** nodep)
{
    dns_rbtnode_t *node = NULL;
    ...
    result = dns_rbt_findnode (tree, name, NULL, &node, NULL, DNS_RBTFIND_EMPTYDATA, NULL, NULL);
    if (result != ISC_R_SUCCESS)
    {
...
#else
            node->locknum = dns_name_hash (&nodename, ISC_TRUE) % rbtdb->node_lock_count;
#endif
            if (tree == rbtdb->tree)
            {
...
static isc_result_t zone_zonecut_callback (dns_rbtnode_t * node, dns_name_t * name, void *arg)
{
    ...
    NODE_LOCK (&(search->rbtdb->node_locks[node->locknum].lock), isc_rwlocktype_read);
    ...
    for (header = node->data; header != NULL; header = header_next)
    {
        ...
        if (header->type == dns_rdatatype_ns ||
            header->type == dns_rdatatype_dname || header->type == RBTDB_RDATATYPE_SIGDNAME)
        {
            do
            {
            ...
            if (header != NULL)
            {
                ...
                else if (node != onode || IS_STUB (search->rbtdb))
                {
                    /*
     ...
     */
    if (!IS_CACHE (search->rbtdb) && !IS_STUB (search->rbtdb) && ns_header != NULL)
    {
        /*
    ...
    if (found != NULL)
    {
        ...
        else
        {
            ...
            RUNTIME_CHECK (dns_name_copy (name, zcname, NULL) == ISC_R_SUCCESS);
    ...
    NODE_UNLOCK (&(search->rbtdb->node_locks[node->locknum].lock), isc_rwlocktype_read);
...
bind_rdataset (dns_rbtdb_t * rbtdb, dns_rbtnode_t * node,
               rdatasetheader_t * header, isc_stdtime_t now, dns_rdataset_t * rdataset)
{
    unsigned char *raw;            /* RDATASLAB */
...
setup_delegation (rbtdb_search_t * search, dns_dbnode_t ** nodep,
                  dns_name_t * foundname, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    isc_result_t result;
    ...
    {
        NODE_LOCK (&(search->rbtdb->node_locks[node->locknum].lock), isc_rwlocktype_read);
        bind_rdataset (search->rbtdb, node, search->zonecut_rdataset, search->now, rdataset);
        if (sigrdataset != NULL && search->zonecut_sigrdataset != NULL)
            bind_rdataset (search->rbtdb, node, search->zonecut_sigrdataset, search->now, sigrdataset);
        NODE_UNLOCK (&(search->rbtdb->node_locks[node->locknum].lock), isc_rwlocktype_read);
    }
...
valid_glue (rbtdb_search_t * search, dns_name_t * name, rbtdb_rdatatype_t type, dns_rbtnode_t * node)
{
    unsigned char *raw;            /* RDATASLAB */
    ...
    else if (type != dns_rdatatype_a && type != dns_rdatatype_aaaa && type != dns_rdatatype_a6)
    {
        return (ISC_FALSE);
    }
...
static inline isc_boolean_t activeempty (rbtdb_search_t * search, dns_rbtnodechain_t * chain, dns_name_t * name)
{
    ...
    {
        node = NULL;
        result = dns_rbtnodechain_current (chain, &prefix, origin, &node);
        if (result != ISC_R_SUCCESS)
            break;
        NODE_LOCK (&(rbtdb->node_locks[node->locknum].lock), isc_rwlocktype_read);
        for (header = node->data; header != NULL; header = header->next)
        {
            if (header->serial <= search->serial && !IGNORE (header) && EXISTS (header))
                break;
        }
        NODE_UNLOCK (&(rbtdb->node_locks[node->locknum].lock), isc_rwlocktype_read);
        if (header != NULL)
...
static inline isc_boolean_t activeemtpynode (rbtdb_search_t * search, dns_name_t * qname, dns_name_t * wname)
{
    dns_fixedname_t fnext;
    ...
    do
    {
        if ((check_prev && dns_name_issubdomain (prev, &rname)) || (check_next && dns_name_issubdomain (next, &rname)))
        {
            answer = ISC_TRUE;
            break;
        }
...
static inline isc_result_t find_wildcard (rbtdb_search_t * search, dns_rbtnode_t ** nodep, dns_name_t * qname)
{
    unsigned int i, j;
    ...
    do
    {
        ...
        if (wild)
        {
            ...
            result = dns_name_concatenate (dns_wildcardname, &name, wname, NULL);
            j = i;
            while (result == ISC_R_SUCCESS && j != 0)
            {
                ...
                result = dns_name_concatenate (wname, &name, wname, NULL);
            }
            if (result != ISC_R_SUCCESS)
                break;
            ...
            result = dns_rbt_findnode (rbtdb->tree, wname, NULL, &wnode, &wchain, DNS_RBTFIND_EMPTYDATA, NULL, NULL);
            if (result == ISC_R_SUCCESS)
            {
                nodelock_t *lock;
                ...
                for (header = wnode->data; header != NULL; header = header->next)
                {
                    if (header->serial <= search->serial && !IGNORE (header) && EXISTS (header))
                        break;
                }
                NODE_UNLOCK (lock, isc_rwlocktype_read);
                if (header != NULL || activeempty (search, &wchain, wname))
                {
...
static isc_boolean_t matchparams (rdatasetheader_t * header, rbtdb_search_t * search)
{
    ...
    while (count-- > 0)
    {
        ...
        region.base = raw;
        region.length = rdlen;
        dns_rdata_fromregion (&rdata, search->rbtdb->common.rdclass, dns_rdatatype_nsec3, &region);
        raw += rdlen;
...
previous_closest_nsec (dns_rdatatype_t type, rbtdb_search_t * search,
                       ...
                       dns_rbtnode_t ** nodep, dns_rbtnodechain_t * nsecchain, isc_boolean_t * firstp)
{
    ...
    if (type == dns_rdatatype_nsec3)
    {
        ...
        if (result != ISC_R_SUCCESS && result != DNS_R_NEWORIGIN)
            return (result);
        result = dns_rbtnodechain_current (&search->chain, name, origin, nodep);
        return (result);
    }
    ...
    for (;;)
    {
        if (*firstp)
        {
            ...
            *firstp = ISC_FALSE;
            ...
            result = dns_rbt_findnode (search->rbtdb->nsec,
                                       target, NULL, &nsecnode, nsecchain, DNS_RBTFIND_NOOPTIONS, NULL, NULL);
            if (result == ISC_R_SUCCESS)
            {
                /*
            ...
            {
                result = dns_rbtnodechain_current (nsecchain, name, origin, NULL);
        ...
        *nodep = NULL;
        result = dns_rbt_findnode (search->rbtdb->tree, target, NULL,
                                   nodep, &search->chain, DNS_RBTFIND_NOOPTIONS, NULL, NULL);
        if (result == ISC_R_SUCCESS)
            return (result);
        ...
        if (result != DNS_R_PARTIALMATCH && result != ISC_R_NOTFOUND)
        {
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           ...
                           "previous_closest_nsec(): %s", isc_result_totext (result));
            return (DNS_R_BADDB);
        }
    }
}
...
find_closest_nsec (rbtdb_search_t * search, dns_dbnode_t ** nodep,
                   ...
                   dns_rdataset_t * sigrdataset, dns_rbt_t * tree, dns_db_secure_t secure)
{
    ...
    do
    {
        ...
        if (!empty_node)
        {
            if (found != NULL && search->rbtversion->havensec3 &&
                found->type == dns_rdatatype_nsec3 && !matchparams (found, search))
            {
                empty_node = ISC_TRUE;
                found = NULL;
                foundsig = NULL;
                result = previous_closest_nsec (type, search, name, origin, &prevnode, NULL, NULL);
            }
            else if (found != NULL && (foundsig != NULL || !need_sig))
            {
                 ...
                 * the case.
                 */
                result = dns_name_concatenate (name, origin, foundname, NULL);
                if (result == ISC_R_SUCCESS)
                {
                    if (nodep != NULL)
                    {
                    ...
                    if (foundsig != NULL)
                        bind_rdataset (search->rbtdb, node, foundsig, search->now, sigrdataset);
                }
            }
            else if (found == NULL && foundsig == NULL)
            {
                 ...
                 */
                empty_node = ISC_TRUE;
                result = previous_closest_nsec (type, search, name, origin, &prevnode, &nsecchain, &first);
            }
            else
            {
                /*
...
zone_find (dns_db_t * db, dns_name_t * name, dns_dbversion_t * version,
           ...
           dns_dbnode_t ** nodep, dns_name_t * foundname, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    dns_rbtnode_t *node = NULL;
    ...
    INSIST (version == NULL || ((rbtdb_version_t *) version)->rbtdb == (dns_rbtdb_t *) db);
     ...
     */
    tree = (options & DNS_DBFIND_FORCENSEC3) != 0 ? search.rbtdb->nsec3 : search.rbtdb->tree;
    result = dns_rbt_findnode (tree, name, foundname, &node,
                               &search.chain, DNS_RBTFIND_EMPTYDATA, zone_zonecut_callback, &search);
    ...
    if (result == DNS_R_PARTIALMATCH)
    {
      partial_match:
        if (search.zonecut != NULL)
        {
            result = setup_delegation (&search, nodep, foundname, rdataset, sigrdataset);
            goto tree_exit;
        }
        ...
        if ((search.rbtversion->secure == dns_db_secure &&
            ...
            (search.options & DNS_DBFIND_FORCENSEC) != 0 || (search.options & DNS_DBFIND_FORCENSEC3) != 0)
        {
            result = find_closest_nsec (&search, nodep, foundname,
                                        rdataset, sigrdataset, tree, search.rbtversion->secure);
            if (result == ISC_R_SUCCESS)
  ...
  found:
    ...
    else
    {
        ...
        if (node->find_callback &&
            ((node != search.rbtdb->origin_node && !dns_rdatatype_atparent (type)) || IS_STUB (search.rbtdb)))
    ...
    for (header = node->data; header != NULL; header = header_next)
    {
        ...
        if (header != NULL)
        {
             ...
             */
            if (header->type == dns_rdatatype_nsec3 && !matchparams (header, &search))
            {
                NODE_UNLOCK (lock, isc_rwlocktype_read);
                goto partial_match;
            }
             ...
             * remember it.
             */
            if (header->type == type || type == dns_rdatatype_any || (header->type == dns_rdatatype_cname && cname_ok))
            {
                /*
                ...
                if (header->type == dns_rdatatype_cname && cname_ok)
                {
                    ...
                    if (cnamesig != NULL)
                        ...
                        sigtype = RBTDB_RDATATYPE_SIGCNAME;
                }
                /*
            ...
            else if (header->type == dns_rdatatype_nsec && !search.rbtversion->havensec3)
            {
                /*
            ...
            else if (header->type == RBTDB_RDATATYPE_SIGNSEC && !search.rbtversion->havensec3)
            {
                /*
            ...
            else if (cname_ok && header->type == RBTDB_RDATATYPE_SIGCNAME)
            {
                /*
    ...
    if (found == NULL)
    {
        ...
        if (search.rbtversion->secure == dns_db_secure &&
            !search.rbtversion->havensec3 && (nsecheader == NULL || nsecsig == NULL))
        {
            /*
            ...
            NODE_UNLOCK (lock, isc_rwlocktype_read);
            result = find_closest_nsec (&search, nodep, foundname,
                                        rdataset, sigrdataset, search.rbtdb->tree, search.rbtversion->secure);
            if (result == ISC_R_SUCCESS)
                result = DNS_R_EMPTYWILD;
            goto tree_exit;
        }
        if ((search.options & DNS_DBFIND_FORCENSEC) != 0 && nsecheader == NULL)
        {
            /*
        ...
        if ((search.rbtversion->secure == dns_db_secure &&
             !search.rbtversion->havensec3) || (search.options & DNS_DBFIND_FORCENSEC) != 0)
        {
            bind_rdataset (search.rbtdb, node, nsecheader, 0, rdataset);
            if (nsecsig != NULL)
                bind_rdataset (search.rbtdb, node, nsecsig, 0, sigrdataset);
        }
        if (wild)
    ...
    if (type != found->type && type != dns_rdatatype_any && found->type == dns_rdatatype_cname)
    {
        /*
    ...
    else if (search.zonecut != NULL)
    {
        ...
        if (search.zonecut == node)
        {
             ...
             * validated updates.
             */
            if (type == dns_rdatatype_nsec || type == dns_rdatatype_nsec3 || type == dns_rdatatype_key)
                result = ISC_R_SUCCESS;
        ...
        if (result == DNS_R_GLUE &&
            (search.options & DNS_DBFIND_VALIDATEGLUE) != 0 && !valid_glue (&search, foundname, type, node))
        {
            NODE_UNLOCK (lock, isc_rwlocktype_read);
            result = setup_delegation (&search, nodep, foundname, rdataset, sigrdataset);
            goto tree_exit;
    ...
    if (type != dns_rdatatype_any)
    {
        ...
        if (foundsig != NULL)
            bind_rdataset (search.rbtdb, node, foundsig, 0, sigrdataset);
    }
  ...
  tree_exit:
    ...
    if (search.need_cleanup)
    {
        ...
        NODE_LOCK (lock, isc_rwlocktype_read);
        decrement_reference (search.rbtdb, node, 0, isc_rwlocktype_read, isc_rwlocktype_none, ISC_FALSE);
        NODE_UNLOCK (lock, isc_rwlocktype_read);
    }
...
static isc_result_t cache_zonecut_callback (dns_rbtnode_t * node, dns_name_t * name, void *arg)
{
    ...
    for (header = node->data; header != NULL; header = header_next)
    {
        ...
        if (header->rdh_ttl <= search->now)
        {
            ...
            if ((header->rdh_ttl <= search->now - RBTDB_VIRTUAL) &&
                (locktype == isc_rwlocktype_write || NODE_TRYUPGRADE (lock) == ISC_R_SUCCESS))
            {
                /*
        ...
        else if (header->type == dns_rdatatype_dname && EXISTS (header))
        {
            dname_header = header;
            header_prev = header;
        }
        else if (header->type == RBTDB_RDATATYPE_SIGDNAME && EXISTS (header))
        {
            sigdname_header = header;
            header_prev = header;
        }
    ...
    if (dname_header != NULL &&
        (!DNS_TRUST_PENDING (dname_header->trust) || (search->options & DNS_DBFIND_PENDINGOK) != 0))
    {
        /*
         * We increment the reference count on node to ensure that
         * search->zonecut_rdataset will still be valid later.
    ...
    do
    {
        ...
        for (header = node->data; header != NULL; header = header_next)
        {
            ...
            if (header->rdh_ttl <= search->now)
            {
                 ...
                 */
                if ((header->rdh_ttl <= search->now -
                     RBTDB_VIRTUAL) && (locktype == isc_rwlocktype_write || NODE_TRYUPGRADE (lock) == ISC_R_SUCCESS))
                {
                    /*
                    ...
                    if (dns_rbtnode_refcurrent (node) == 0)
                    {
                        ...
                        clean_stale_headers (search->rbtdb, m, header);
                        if (header_prev != NULL)
                            header_prev->next = header->next;
                        else
                            node->data = header->next;
                        free_rdataset (rbtdb, m, header);
                    }
                    else
                    {
                        header->attributes |= RDATASET_ATTR_STALE;
        ...
        if (found != NULL)
        {
            ...
            if (foundname != NULL)
            {
                ...
                while (result == ISC_R_SUCCESS && i > 0)
                {
                    ...
                    dns_name_init (&name, NULL);
                    dns_rbt_namefromnode (level_node, &name);
                    result = dns_name_concatenate (foundname, &name, foundname, NULL);
                }
                if (result != ISC_R_SUCCESS)
                {
                    *nodep = NULL;
...
find_coveringnsec (rbtdb_search_t * search, dns_dbnode_t ** nodep,
                   isc_stdtime_t now, dns_name_t * foundname, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    dns_rbtnode_t *node;
    ...
    sigmatchtype = RBTDB_RDATATYPE_VALUE (dns_rdatatype_rrsig, dns_rdatatype_nsec);
    ...
    do
    {
        ...
        for (header = node->data; header != NULL; header = header_next)
        {
                    ...
                    (locktype == isc_rwlocktype_write || NODE_TRYUPGRADE (lock) == ISC_R_SUCCESS))
                {
                    ...
                    if (dns_rbtnode_refcurrent (node) == 0)
                    {
                        ...
                        if (header_prev != NULL)
                            ...
                            node->data = header->next;
                        free_rdataset (search->rbtdb, m, header);
                    }
                    else
                    {
                        header->attributes |= RDATASET_ATTR_STALE;
            ...
            if (NONEXISTENT (header) || RBTDB_RDATATYPE_BASE (header->type) == 0)
            {
                header_prev = header;
                continue;
            }
        ...
        if (found != NULL)
        {
            ...
            if (result != ISC_R_SUCCESS)
                goto unlock_node;
            bind_rdataset (search->rbtdb, node, found, now, rdataset);
            if (foundsig != NULL)
                bind_rdataset (search->rbtdb, node, foundsig, now, sigrdataset);
            new_reference (search->rbtdb, node);
            *nodep = node;
 ...
 */
#ifdef BIND9
static void get_rpz_enabled (dns_db_t * db, dns_rpz_st_t * st)
{
    dns_rbtdb_t *rbtdb;
...
rpz_findips (dns_rpz_zone_t * rpz, dns_rpz_type_t rpz_type,
             ...
             dns_rdataset_t * ardataset, dns_rpz_st_t * st, dns_name_t * query_qname)
{
    dns_rbtdb_t *rbtdb;
    ...
    for (result = dns_rdataset_first (ardataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (ardataset))
    {
        dns_rdata_t rdata = DNS_RDATA_INIT;
        ...
        result = dns_rpz_cidr_find (rbtdb->rpz_cidr, &netaddr, rpz_type, selfname, qname, &prefix);
        if (result != ISC_R_SUCCESS)
            continue;
        ...
        if (st->m.policy != DNS_RPZ_POLICY_MISS &&
            ...
            (st->m.type < rpz_type ||
             (st->m.type == rpz_type &&
              (st->m.prefix > prefix || (st->m.prefix == prefix && 0 > dns_name_rdatacompare (st->qname, qname))))))
            continue;
         ...
         */
        node = NULL;
        result = dns_rbt_findnode (rbtdb->tree, qname, NULL, &node, NULL, 0, NULL, NULL);
        if (result != ISC_R_SUCCESS)
        {
            ...
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_RPZ,
                           ...
                           "rpz_findips findnode(%s): %s", namebuf, isc_result_totext (result));
            continue;
        }
        /*
        ...
        result = dns_db_findrdataset (db, node, version, ardataset->type, 0, 0, &zrdataset, NULL);
        if (result != ISC_R_SUCCESS)
            result = dns_db_findrdataset (db, node, version, dns_rdatatype_cname, 0, 0, &zrdataset, NULL);
        if (result == ISC_R_SUCCESS)
        {
            ...
            {
                rpz_policy = dns_rpz_decode_cname (&zrdataset, selfname);
                if (rpz_policy == DNS_RPZ_POLICY_RECORD || rpz_policy == DNS_RPZ_POLICY_WILDCNAME)
                    result = DNS_R_CNAME;
            }
            ttl = zrdataset.ttl;
        }
        ...
        if (rpz->policy != DNS_RPZ_POLICY_GIVEN)
        {
            ...
            {
                if (isc_log_wouldlog (dns_lctx, DNS_RPZ_INFO_LEVEL))
                {
                    char qname_buf[DNS_NAME_FORMATSIZE];

                    char rpz_qname_buf[DNS_NAME_FORMATSIZE];

                    dns_name_format (query_qname, qname_buf, sizeof (qname_buf));
                    dns_name_format (qname, rpz_qname_buf, sizeof (rpz_qname_buf));
                    ...
                    isc_log_write (dns_lctx,
                                   ...
                                   dns_rpz_policy2str (rpz_policy), qname_buf, rpz_qname_buf);
                }
                continue;
            }
        ...
        if ((rpz_policy == DNS_RPZ_POLICY_RECORD || rpz_policy == DNS_RPZ_POLICY_WILDCNAME) && result != DNS_R_NXRRSET)
        {
            ...
            dns_dbnode_t ** nodep, dns_name_t * foundname, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    ...
    result = dns_rbt_findnode (search.rbtdb->tree, name, foundname, &node,
                               &search.chain, DNS_RBTFIND_EMPTYDATA, cache_zonecut_callback, &search);

    if (result == DNS_R_PARTIALMATCH)
    {
        if ((search.options & DNS_DBFIND_COVERINGNSEC) != 0)
        {
            result = find_coveringnsec (&search, nodep, now, foundname, rdataset, sigrdataset);
        ...
        {
          find_ns:
            result = find_deepest_zonecut (&search, node, nodep, foundname, rdataset, sigrdataset);
            goto tree_exit;
        }
    }
    else if (result != ISC_R_SUCCESS)
        ...
        else if (EXISTS (header))
        {
            ...
            if (header->type == type ||
                (type == dns_rdatatype_any &&
                 RBTDB_RDATATYPE_BASE (header->type) != 0) || (cname_ok && header->type == dns_rdatatype_cname))
            {
                /*
                 * We've found the answer.
                 */
                found = header;
                if (header->type == dns_rdatatype_cname && cname_ok && cnamesig != NULL)
                {
                    /*
            ...
            else if (header->type == RBTDB_RDATATYPE_NCACHEANY || header->type == negtype)
            {
                /*
    ...
    if (found == NULL ||
        ...
        (DNS_TRUST_PENDING (found->trust) && ((options & DNS_DBFIND_PENDINGOK) == 0)))
    {
        /*
        ...
        if (nsheader != NULL)
        {
            ...
            bind_rdataset (search.rbtdb, node, nsheader, search.now, rdataset);
            if (need_headerupdate (nsheader, search.now))
                update = nsheader;
            if (nssig != NULL)
            {
                bind_rdataset (search.rbtdb, node, nssig, search.now, sigrdataset);
    ...
    if (type != dns_rdatatype_any || result == DNS_R_NCACHENXDOMAIN || result == DNS_R_NCACHENXRRSET)
    {
        bind_rdataset (search.rbtdb, node, found, search.now, rdataset);
        if (need_headerupdate (found, search.now))
            update = found;
  ...
  node_exit:
    if ((update != NULL || updatesig != NULL) && locktype != isc_rwlocktype_write)
    {
        NODE_UNLOCK (lock, locktype);
        NODE_LOCK (lock, isc_rwlocktype_write);
        locktype = isc_rwlocktype_write;
                   ...
                   dns_name_t * foundname, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
     ...
     */
    result = dns_rbt_findnode (search.rbtdb->tree, name, foundname, &node, &search.chain, rbtoptions, NULL, &search);
    ...
    if (need_headerupdate (found, search.now) || (foundsig != NULL && need_headerupdate (foundsig, search.now)))
    {
        if (locktype != isc_rwlocktype_write)
        {
            NODE_UNLOCK (lock, locktype);
...
static void detachnode (dns_db_t * db, dns_dbnode_t ** targetp)
{
    ...
    if (decrement_reference (rbtdb, node, 0, isc_rwlocktype_read, isc_rwlocktype_none, ISC_FALSE))
    {
        if (isc_refcount_current (&nodelock->references) == 0 && nodelock->exiting)
        {
            inactive = ISC_TRUE;
        }
    }
...
static isc_result_t expirenode (dns_db_t * db, dns_dbnode_t * node, isc_stdtime_t now)
{
    ...
    if (isc_mem_isovermem (rbtdb->common.mctx))
    {
        ...
        if (log)
            isc_log_write (dns_lctx, category, module, level,
                           ...
                           dns_rbt_formatnodename (rbtnode, printname, sizeof (printname)));
    }
     ...
     */
    NODE_LOCK (&rbtdb->node_locks[rbtnode->locknum].lock, isc_rwlocktype_write);
    ...
    for (header = rbtnode->data; header != NULL; header = header->next)
        if (header->rdh_ttl <= now - RBTDB_VIRTUAL)
        {
            ...
            rbtnode->dirty = 1;
            if (log)
                isc_log_write (dns_lctx, category, module, level, "overmem cache: stale %s", printname);
        }
        else if (force_expire)
        {
            ...
            else if (log)
            {
                isc_log_write (dns_lctx, category, module,
                               level, "overmem cache: " "reprieve by RETAIN() %s", printname);
            }
        }
        else if (isc_mem_isovermem (rbtdb->common.mctx) && log)
            isc_log_write (dns_lctx, category, module, level, "overmem cache: saved %s", printname);

    NODE_UNLOCK (&rbtdb->node_locks[rbtnode->locknum].lock, isc_rwlocktype_write);
...
static void printnode (dns_db_t * db, dns_dbnode_t * node, FILE * out)
{
    ...
    NODE_LOCK (&rbtdb->node_locks[rbtnode->locknum].lock, isc_rwlocktype_read);
    ...
    if (rbtnode->data != NULL)
    {
        ...
        for (current = rbtnode->data; current != NULL; current = top_next)
        {
            top_next = current->next;
            first = ISC_TRUE;
            ...
            do
            {
                ...
                fprintf (out,
                         ...
                         current->rdh_ttl, current->trust, current->attributes, current->resign);
    ...
    NODE_UNLOCK (&rbtdb->node_locks[rbtnode->locknum].lock, isc_rwlocktype_read);
}
...
static isc_result_t createiterator (dns_db_t * db, unsigned int options, dns_dbiterator_t ** iteratorp)
{
    ...
    rbtdbiter->common.relative_names = ISC_TF ((options & DNS_DB_RELATIVENAMES) != 0);
...
zone_findrdataset (dns_db_t * db, dns_dbnode_t * node, dns_dbversion_t * version,
                   ...
                   isc_stdtime_t now, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    dns_rbtdb_t *rbtdb = (dns_rbtdb_t *) db;
    ...
    if (found != NULL)
    {
        ...
        if (foundsig != NULL)
            bind_rdataset (rbtdb, rbtnode, foundsig, now, sigrdataset);
    }

    NODE_UNLOCK (&rbtdb->node_locks[rbtnode->locknum].lock, isc_rwlocktype_read);

    if (close_version)
        closeversion (db, (dns_dbversion_t **) (void *) (&rbtversion), ISC_FALSE);
    ...
    for (header = node->data; header != NULL; header = header_next)
    {
        ...
        else
        {
            ...
            if (rdtype != dns_rdatatype_key &&
                rdtype != dns_rdatatype_sig && rdtype != dns_rdatatype_nsec && rdtype != dns_rdatatype_rrsig)
            {
                /*
     ...
     dns_rdataset_t * addedrdataset, isc_stdtime_t now)
{
    ...
    if (rbtversion == NULL && !newheader_nx)
    {
        ...
        else
        {
            ...
            for (topheader = rbtnode->data; topheader != NULL; topheader = topheader->next)
            {
                if (topheader->type == RBTDB_RDATATYPE_NCACHEANY)
                    break;
            }
            if (topheader != NULL && EXISTS (topheader) && topheader->rdh_ttl > now)
            {
                /*
                 * Found one.
                 */
                if (trust < topheader->trust)
                {
                    ...
                    if (addedrdataset != NULL)
                        bind_rdataset (rbtdb, rbtnode, topheader, now, addedrdataset);
    ...
    for (topheader = rbtnode->data; topheader != NULL; topheader = topheader->next)
    {
        if (topheader->type == newheader->type || topheader->type == negtype)
            break;
  ...
  find_header:
    ...
    if (header != NULL)
    {
         ...
         */
        if (rbtversion == NULL && trust < header->trust && (header->rdh_ttl > now || header_nx))
        {
            free_rdataset (rbtdb, rbtdb->common.mctx, newheader);
            if (addedrdataset != NULL)
                bind_rdataset (rbtdb, rbtnode, header, now, addedrdataset);
            return (DNS_R_UNCHANGED);
        }
        ...
        if (merge)
        {
            ...
            if ((options & DNS_DBADD_EXACTTTL) != 0 && newheader->rdh_ttl != header->rdh_ttl)
                result = DNS_R_NOTEXACT;
            else if (newheader->rdh_ttl != header->rdh_ttl)
                flags |= DNS_RDATASLAB_FORCE;
            if (result == ISC_R_SUCCESS)
                result = dns_rdataslab_merge ((unsigned char *) header,
                                              (unsigned char *) newheader,
                                              (unsigned int) (sizeof (*newheader)),
                                              rbtdb->common.mctx,
                                              rbtdb->common.rdclass, (dns_rdatatype_t) header->type, flags, &merged);
            if (result == ISC_R_SUCCESS)
            {
                /*
                ...
                if (loading && RESIGN (newheader) && RESIGN (header) && header->resign < newheader->resign)
        ...
        if (IS_CACHE (rbtdb) && header->rdh_ttl > now &&
            ...
            dns_rdataslab_equalx ((unsigned char *) header,
                                  (unsigned char *) newheader,
                                  (unsigned int) (sizeof (*newheader)),
                                  rbtdb->common.rdclass, (dns_rdatatype_t) header->type))
        {
            /*
            ...
            if (header->noqname == NULL && newheader->noqname != NULL)
            {
                header->noqname = newheader->noqname;
                newheader->noqname = NULL;
            }
            if (header->closest == NULL && newheader->closest != NULL)
            {
         ...
         */
        if (IS_CACHE (rbtdb) && header->rdh_ttl > now &&
            header->type == dns_rdatatype_ns && !header_nx && !newheader_nx && header->trust <= newheader->trust)
        {
        ...
        if (IS_CACHE (rbtdb) && header->rdh_ttl > now &&
            ...
            dns_rdataslab_equal ((unsigned char *) header,
                                 (unsigned char *) newheader, (unsigned int) (sizeof (*newheader))))
        {
            /*
             * Honour the new ttl if it is less than the
             * older one.
        ...
        INSIST (rbtversion == NULL || rbtversion->serial >= topheader->serial);
        ...
        else
        {
            ...
            if (rbtversion == NULL)
            {
                ...
                if (sigheader != NULL)
                {
                    ...
                    sigheader->attributes |= RDATASET_ATTR_STALE;
                }
            }
            idx = newheader->node->locknum;
            if (IS_CACHE (rbtdb))
            {
                ISC_LIST_PREPEND (rbtdb->rdatasets[idx], newheader, link);
                /*
     ...
     */
    if (rbtversion != NULL && cname_and_other_data (rbtnode, rbtversion->serial))
...
static inline isc_boolean_t delegating_type (dns_rbtdb_t * rbtdb, dns_rbtnode_t * node, rbtdb_rdatatype_t type)
{
    if (IS_CACHE (rbtdb))
    {
        if (type == dns_rdatatype_dname)
            ...
            return (ISC_FALSE);
             ...
             (type == dns_rdatatype_ns && (node != rbtdb->origin_node || IS_STUB (rbtdb))))
        return (ISC_TRUE);
    return (ISC_FALSE);
}

static inline isc_result_t addnoqname (dns_rbtdb_t * rbtdb, rdatasetheader_t * newheader, dns_rdataset_t * rdataset)
{
    struct noqname *noqname;
...
static inline isc_result_t addclosest (dns_rbtdb_t * rbtdb, rdatasetheader_t * newheader, dns_rdataset_t * rdataset)
{
    struct noqname *closest;
...
addrdataset (dns_db_t * db, dns_dbnode_t * node, dns_dbversion_t * version,
             isc_stdtime_t now, dns_rdataset_t * rdataset, unsigned int options, dns_rdataset_t * addedrdataset)
{
    dns_rbtdb_t *rbtdb = (dns_rbtdb_t *) db;
    ...
    if (rbtdb->common.methods == &zone_methods)
        REQUIRE (((rbtnode->nsec == DNS_RBT_NSEC_NSEC3 &&
                  ...
                  (rbtnode->nsec != DNS_RBT_NSEC_NSEC3 &&
                   rdataset->type != dns_rdatatype_nsec3 && rdataset->covers != dns_rdatatype_nsec3)));
    ...
    result = dns_rdataslab_fromrdataset (rdataset, rbtdb->common.mctx, &region, sizeof (rdatasetheader_t));
    if (result != ISC_R_SUCCESS)
        return (result);
     ...
     */
    if (rbtnode->nsec != DNS_RBT_NSEC_HAS_NSEC && rdataset->type == dns_rdatatype_nsec)
        newnsec = ISC_TRUE;
    else
        newnsec = ISC_FALSE;
    ...
    if (result == ISC_R_SUCCESS)
        result = add (rbtdb, rbtnode, rbtversion, newheader, options, ISC_FALSE, addedrdataset, now);
...
subtractrdataset (dns_db_t * db, dns_dbnode_t * node, dns_dbversion_t * version,
                  dns_rdataset_t * rdataset, unsigned int options, dns_rdataset_t * newrdataset)
{
    dns_rbtdb_t *rbtdb = (dns_rbtdb_t *) db;
    ...
    if (header != NULL && EXISTS (header))
    {
        ...
        if (result == ISC_R_SUCCESS)
            result = dns_rdataslab_subtract ((unsigned char *) header,
                                             (unsigned char *) newheader,
                                             (unsigned int) (sizeof (*newheader)),
                                             rbtdb->common.mctx,
                                             rbtdb->common.rdclass, (dns_rdatatype_t) header->type, flags, &subresult);
        if (result == ISC_R_SUCCESS)
        {
            free_rdataset (rbtdb, rbtdb->common.mctx, newheader);
                ...
                dns_rdatatype_t type, dns_rdatatype_t covers)
{
    ...
    result = add (rbtdb, rbtnode, rbtversion, newheader, DNS_DBADD_FORCE, ISC_FALSE, NULL, 0);
...
static isc_result_t loadnode (dns_rbtdb_t * rbtdb, dns_name_t * name, dns_rbtnode_t ** nodep, isc_boolean_t hasnsec)
{
    ...
    if (nsecresult == ISC_R_EXISTS)
    {
        ...
        isc_log_write (dns_lctx,
                       DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_CACHE, ISC_LOG_WARNING, "addnode: NSEC node already exists");
#endif
        (*nodep)->nsec = DNS_RBT_NSEC_HAS_NSEC;
        return (noderesult);
    }
    ...
    if (nsecresult != ISC_R_SUCCESS)
        isc_log_write (dns_lctx,
                       ...
                       "dns_rbt_addnode(NSEC): %s", isc_result_totext (nsecresult), isc_result_totext (noderesult));
    return (noderesult);
}
...
static isc_result_t loading_addrdataset (void *arg, dns_name_t * name, dns_rdataset_t * rdataset)
{
     ...
     */
    if (rdataset->type == dns_rdatatype_soa && !IS_CACHE (rbtdb) && !dns_name_equal (name, &rbtdb->common.origin))
        return (DNS_R_NOTZONETOP);

    if (rdataset->type != dns_rdatatype_nsec3 && rdataset->covers != dns_rdatatype_nsec3)
        add_empty_wildcards (rbtdb, name);
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        node->locknum = node->hashval % rbtdb->node_lock_count;
#else
        node->locknum = dns_name_hash (&foundname, ISC_TRUE) % rbtdb->node_lock_count;
#endif
    }
    ...
    newheader = (rdatasetheader_t *) region.base;
    init_rdataset (rbtdb, newheader);
    set_ttl (rbtdb, newheader, rdataset->ttl + loadctx->now);    /* XXX overflow check */
    newheader->type = RBTDB_RDATATYPE_VALUE (rdataset->type, rdataset->covers);
    newheader->attributes = 0;
    newheader->trust = rdataset->trust;
    newheader->serial = 1;
    ...
    result = add (rbtdb, node, rbtdb->current_version, newheader, DNS_DBADD_MERGE, ISC_TRUE, NULL, 0);
    if (result == ISC_R_SUCCESS && delegating_type (rbtdb, node, rdataset->type))
        node->find_callback = 1;
...
static isc_result_t beginload (dns_db_t * db, dns_addrdatasetfunc_t * addp, dns_dbload_t ** dbloadp)
{
    ...
    REQUIRE ((rbtdb->attributes & (RBTDB_ATTR_LOADED | RBTDB_ATTR_LOADING)) == 0);
...
static isc_result_t endload (dns_db_t * db, dns_dbload_t ** dbloadp)
{
    rbtdb_load_t *loadctx;
...
static isc_result_t
dump (dns_db_t * db, dns_dbversion_t * version, const char *filename, dns_masterformat_t masterformat)
{
    dns_rbtdb_t *rbtdb;
...
getnsec3parameters (dns_db_t * db, dns_dbversion_t * version, dns_hash_t * hash,
                    isc_uint8_t * flags, isc_uint16_t * iterations, unsigned char *salt, size_t * salt_length)
{
    dns_rbtdb_t *rbtdb;
...
static isc_result_t setsigningtime (dns_db_t * db, dns_rdataset_t * rdataset, isc_stdtime_t resign)
{
    ...
    NODE_LOCK (&rbtdb->node_locks[header->node->locknum].lock, isc_rwlocktype_write);
    ...
    if (header->heap_index != 0)
    {
        ...
        if (resign == 0)
        {
            isc_heap_delete (rbtdb->heaps[header->node->locknum], header->heap_index);
            header->heap_index = 0;
        }
        else if (resign < oldresign)
            isc_heap_increased (rbtdb->heaps[header->node->locknum], header->heap_index);
        else if (resign > oldresign)
            isc_heap_decreased (rbtdb->heaps[header->node->locknum], header->heap_index);
    }
    ...
    NODE_UNLOCK (&rbtdb->node_locks[header->node->locknum].lock, isc_rwlocktype_write);
    return (result);
}
...
static isc_result_t getsigningtime (dns_db_t * db, dns_rdataset_t * rdataset, dns_name_t * foundname)
{
    ...
    for (i = 0; i < rbtdb->node_lock_count; i++)
    {
        ...
        if (this == NULL)
        {
            NODE_UNLOCK (&rbtdb->node_locks[i].lock, isc_rwlocktype_read);
            continue;
        }
        if (header == NULL)
            header = this;
    ...
    NODE_UNLOCK (&rbtdb->node_locks[header->node->locknum].lock, isc_rwlocktype_read);
...
static void resigned (dns_db_t * db, dns_rdataset_t * rdataset, dns_dbversion_t * version)
{
    ...
    NODE_UNLOCK (&rbtdb->node_locks[node->locknum].lock, isc_rwlocktype_write);
    RWUNLOCK (&rbtdb->tree_lock, isc_rwlocktype_write);
}

static dns_stats_t *getrrsetstats (dns_db_t * db)
{
    dns_rbtdb_t *rbtdb = (dns_rbtdb_t *) db;
...
isc_result_t
    ...
    (isc_mem_t * mctx, dns_name_t * origin, dns_dbtype_t type,
     dns_rdataclass_t rdclass, unsigned int argc, char *argv[], void *driverarg, dns_db_t ** dbp)
{
    dns_rbtdb_t *rbtdb;
    ...
    if (IS_CACHE (rbtdb))
    {
        ...
        rbtdb->rdatasets = isc_mem_get (mctx, rbtdb->node_lock_count * sizeof (rdatasetheaderlist_t));
        if (rbtdb->rdatasets == NULL)
        {
            result = ISC_R_NOMEMORY;
     ...
     * Create the heaps.
     */
    rbtdb->heaps = isc_mem_get (hmctx, rbtdb->node_lock_count * sizeof (isc_heap_t *));
    if (rbtdb->heaps == NULL)
    {
        result = ISC_R_NOMEMORY;
    ...
    for (i = 0; i < (int) rbtdb->node_lock_count; i++)
    {
        result = isc_heap_create (hmctx, sooner, set_index, 0, &rbtdb->heaps[i]);
        if (result != ISC_R_SUCCESS)
            goto cleanup_heaps;
    }
     ...
     */
    rbtdb->deadnodes = isc_mem_get (mctx, rbtdb->node_lock_count * sizeof (rbtnodelist_t));
    if (rbtdb->deadnodes == NULL)
    {
        result = ISC_R_NOMEMORY;
        goto cleanup_heaps;
    ...
    if (!IS_CACHE (rbtdb))
    {
...
#ifdef DNS_RBT_USEHASH
        rbtdb->origin_node->locknum = rbtdb->origin_node->hashval % rbtdb->node_lock_count;
#else
        rbtdb->origin_node->locknum = dns_name_hash (&name, ISC_TRUE) % rbtdb->node_lock_count;
#endif
        /*
         ...
         */
        nsec3node = NULL;
        result = dns_rbt_addnode (rbtdb->nsec3, &rbtdb->common.origin, &nsec3node);
        if (result != ISC_R_SUCCESS)
        {
            INSIST (result != ISC_R_EXISTS);
            free_rbtdb (rbtdb, ISC_FALSE, NULL);
...
#ifdef DNS_RBT_USEHASH
        nsec3node->locknum = nsec3node->hashval % rbtdb->node_lock_count;
#else
        nsec3node->locknum = dns_name_hash (&name, ISC_TRUE) % rbtdb->node_lock_count;
#endif
    }
    ...
    memset (rbtdb->current_version->salt, 0, sizeof (rbtdb->current_version->salt));
    rbtdb->future_version = NULL;
  ...
  cleanup_deadnodes:
    isc_mem_put (mctx, rbtdb->deadnodes, rbtdb->node_lock_count * sizeof (rbtnodelist_t));
    ...
    if (rbtdb->heaps != NULL)
    {
        for (i = 0; i < (int) rbtdb->node_lock_count; i++)
            if (rbtdb->heaps[i] != NULL)
                isc_heap_destroy (&rbtdb->heaps[i]);
        isc_mem_put (mctx, rbtdb->heaps, rbtdb->node_lock_count * sizeof (isc_heap_t *));
    }

  cleanup_rdatasets:
    if (rbtdb->rdatasets != NULL)
        isc_mem_put (mctx, rbtdb->rdatasets, rbtdb->node_lock_count * sizeof (rdatasetheaderlist_t));
  cleanup_rrsetstats:
    if (rbtdb->rrsetstats != NULL)
        dns_stats_detach (&rbtdb->rrsetstats);

  cleanup_node_locks:
    isc_mem_put (mctx, rbtdb->node_locks, rbtdb->node_lock_count * sizeof (rbtdb_nodelock_t));
 ...
 */
    ...
    static void rdatasetiter_destroy (dns_rdatasetiter_t ** iteratorp)
    {
        ...
        if (rbtiterator->common.version != NULL)
            closeversion (rbtiterator->common.db, &rbtiterator->common.version, ISC_FALSE);
        detachnode (rbtiterator->common.db, &rbtiterator->common.node);
        isc_mem_put (rbtiterator->common.db->mctx, rbtiterator, sizeof (*rbtiterator));
        ...
        for (header = rbtnode->data; header != NULL; header = top_next)
        {
            ...
            do
            {
                if (header->serial <= serial && !IGNORE (header))
                {
                     ...
                     * rdatasets to work.
                     */
                    if (NONEXISTENT (header) || (now != 0 && now > header->rdh_ttl))
                        header = NULL;
                    break;
                }
                else
                ...
                do
                {
                    if (header->serial <= serial && !IGNORE (header))
                    {
                        ...
                        if ((header->attributes &
                             RDATASET_ATTR_NONEXISTENT) != 0 || (now != 0 && now > header->rdh_ttl))
                            header = NULL;
                        break;
                    }
                    else
    ...
    static void rdatasetiter_current (dns_rdatasetiter_t * iterator, dns_rdataset_t * rdataset)
    {
        ...
        bind_rdataset (rbtdb, rbtnode, header, rbtiterator->common.now, rdataset);
 ...
 */
    ...
    static inline void reference_iter_node (rbtdb_dbiterator_t * rbtdbiter)
    {
    ...
    static inline void dereference_iter_node (rbtdb_dbiterator_t * rbtdbiter)
    {
        dns_rbtdb_t *rbtdb = (dns_rbtdb_t *) rbtdbiter->common.db;
        ...
        lock = &rbtdb->node_locks[node->locknum].lock;
        NODE_LOCK (lock, isc_rwlocktype_read);
        decrement_reference (rbtdb, node, 0, isc_rwlocktype_read, rbtdbiter->tree_locked, ISC_FALSE);
        NODE_UNLOCK (lock, isc_rwlocktype_read);
    ...
    static void flush_deletions (rbtdb_dbiterator_t * rbtdbiter)
    {
        ...
        if (rbtdbiter->delete != 0)
        {
            ...
            isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                           ...
                           rbtdbiter->delete, dns_rbt_nodecount (rbtdb->tree));
    ...
    static inline void resume_iteration (rbtdb_dbiterator_t * rbtdbiter)
    {
        dns_rbtdb_t *rbtdb = (dns_rbtdb_t *) rbtdbiter->common.db;
    ...
    static isc_result_t dbiterator_first (dns_dbiterator_t * iterator)
    {
        ...
        if (rbtdbiter->result != ISC_R_SUCCESS && rbtdbiter->result != ISC_R_NOMORE)
        ...
        {
            rbtdbiter->current = &rbtdbiter->nsec3chain;
            result = dns_rbtnodechain_first (rbtdbiter->current, rbtdb->nsec3, name, origin);
        }
        else
        {
            rbtdbiter->current = &rbtdbiter->chain;
            result = dns_rbtnodechain_first (rbtdbiter->current, rbtdb->tree, name, origin);
        ...
        if (result == ISC_R_SUCCESS || result == DNS_R_NEWORIGIN)
        {
            result = dns_rbtnodechain_current (rbtdbiter->current, NULL, NULL, &rbtdbiter->node);
            if (result == ISC_R_SUCCESS)
            {
    ...
    static isc_result_t dbiterator_last (dns_dbiterator_t * iterator)
    {
        ...
        {
            rbtdbiter->current = &rbtdbiter->nsec3chain;
            result = dns_rbtnodechain_last (rbtdbiter->current, rbtdb->nsec3, name, origin);
        }
        if (!rbtdbiter->nsec3only && result == ISC_R_NOTFOUND)
        {
            rbtdbiter->current = &rbtdbiter->chain;
            result = dns_rbtnodechain_last (rbtdbiter->current, rbtdb->tree, name, origin);
        }
        if (result == ISC_R_SUCCESS || result == DNS_R_NEWORIGIN)
        {
            result = dns_rbtnodechain_current (rbtdbiter->current, NULL, NULL, &rbtdbiter->node);
    ...
    static isc_result_t dbiterator_seek (dns_dbiterator_t * iterator, dns_name_t * name)
    {
        ...
        if (rbtdbiter->result != ISC_R_SUCCESS &&
            rbtdbiter->result != ISC_R_NOTFOUND && rbtdbiter->result != ISC_R_NOMORE)
            return (rbtdbiter->result);
        ...
        if (rbtdbiter->nsec3only)
        {
            ...
            result = dns_rbt_findnode (rbtdb->nsec3, name, NULL,
                                       &rbtdbiter->node, rbtdbiter->current, DNS_RBTFIND_EMPTYDATA, NULL, NULL);
        }
        ...
        else
        {
            ...
            if (result == DNS_R_PARTIALMATCH)
            {
                ...
                tresult = dns_rbt_findnode (rbtdb->nsec3, name, NULL,
                                            &node, &rbtdbiter->nsec3chain, DNS_RBTFIND_EMPTYDATA, NULL, NULL);
        ...
        if (result == ISC_R_SUCCESS)
        {
            result = dns_rbtnodechain_current (rbtdbiter->current, iname, origin, NULL);
            if (result == ISC_R_SUCCESS)
            {
                rbtdbiter->new_origin = ISC_TRUE;
                reference_iter_node (rbtdbiter);
        ...
        if (result == ISC_R_SUCCESS || result == DNS_R_PARTIALMATCH)
        {
            ...
            tresult = dns_rbtnodechain_current (rbtdbiter->current, iname, origin, NULL);
            if (tresult == ISC_R_SUCCESS)
            {
                rbtdbiter->new_origin = ISC_TRUE;
                reference_iter_node (rbtdbiter);
        ...
        rbtdbiter->result = (result == DNS_R_PARTIALMATCH) ? ISC_R_SUCCESS : result;
#endif
 ...
 */
                                ...
                                dns_name_t * fname, dns_message_t * msg, isc_stdtime_t now)
    {
        ...
        result = dns_acache_getentry (entry, zonep, dbp, versionp, nodep, fname, msg, now);
                                ...
                                dns_dbversion_t * version, dns_dbnode_t * node, dns_name_t * fname)
    {
        ...
        newcbarg->node = NULL;
        dns_db_attachnode ((dns_db_t *) rbtdb, (dns_dbnode_t *) rbtnode, &newcbarg->node);
        newentry = NULL;
        result = dns_acache_createentry (acache, (dns_db_t *) rbtdb, acache_callback, newcbarg, &newentry);
        if (result != ISC_R_SUCCESS)
            goto fail;
        /* Set cache data in the new entry. */
        result = dns_acache_setentry (acache, newentry, zone, db, version, node, fname);
        if (result != ISC_R_SUCCESS)
            goto fail;
        ...
        if (acarray == NULL)
        {
            ...
            acarray = isc_mem_get (rbtdb->common.mctx, total_count * sizeof (acachectl_t));
      ...
      fail:
        if (newcbarg != NULL)
        {
            if (newentry != NULL)
            {
                acache_cancelentry (rbtdb->common.mctx, newentry, &newcbarg);
 ...
 */
    static inline isc_boolean_t need_headerupdate (rdatasetheader_t * header, isc_stdtime_t now)
    {
        if ((header->attributes & (RDATASET_ATTR_NONEXISTENT | RDATASET_ATTR_STALE)) != 0)
            return (ISC_FALSE);

#if DNS_RBTDB_LIMITLRUUPDATE
        if (header->type == dns_rdatatype_ns ||
            (header->trust == dns_trust_glue &&
             (header->type == dns_rdatatype_a || header->type == dns_rdatatype_aaaa)))
        {
            /*
 ...
 */
    static void
        overmem_purge (dns_rbtdb_t * rbtdb, unsigned int locknum_start, isc_stdtime_t now, isc_boolean_t tree_locked)
    {
        ...
        for (locknum = (locknum_start + 1) % rbtdb->node_lock_count;
             locknum != locknum_start && purgecount > 0; locknum = (locknum + 1) % rbtdb->node_lock_count)
        {
            NODE_LOCK (&rbtdb->node_locks[locknum].lock, isc_rwlocktype_write);
    ...
    static void expire_header (dns_rbtdb_t * rbtdb, rdatasetheader_t * header, isc_boolean_t tree_locked)
    {
        ...
        if (dns_rbtnode_refcurrent (header->node) == 0)
        {
            ...
            decrement_reference (rbtdb, header->node, 0,
                                 ...
                                 tree_locked ? isc_rwlocktype_write : isc_rwlocktype_none, ISC_FALSE);
        }
    }
