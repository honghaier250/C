...
validator_logv (dns_validator_t * val, isc_logcategory_t * category,
     ...
     static void validator_log (dns_validator_t * val, int level, const char *fmt, ...) ISC_FORMAT_PRINTF (3, 4);

     static void
         validator_logcreate (dns_validator_t * val,
                              dns_name_t * name, dns_rdatatype_t type, const char *caller, const char *operation);
 ...
 */
     static inline void markanswer (dns_validator_t * val, const char *where)
{
    ...
    if (val->event->sigrdataset != NULL)
        dns_rdataset_settrust (val->event->sigrdataset, dns_trust_answer);
}

static inline void marksecure (dns_validatorevent_t * event)
{
...
static inline isc_boolean_t exit_check (dns_validator_t * val)
{
    /*
     * Caller must be holding the lock.
     */
 ...
 */
static inline isc_boolean_t dlv_algorithm_supported (dns_validator_t * val)
{
    ...
    for (result = dns_rdataset_first (&val->dlv); result == ISC_R_SUCCESS; result = dns_rdataset_next (&val->dlv))
    {
        ...
        if (!dns_resolver_algorithm_supported (val->view->resolver, val->event->name, dlv.algorithm))
            continue;

#ifdef HAVE_OPENSSL_GOST
        if (dlv.digest_type != DNS_DSDIGEST_SHA256 &&
            dlv.digest_type != DNS_DSDIGEST_SHA1 && dlv.digest_type != DNS_DSDIGEST_GOST)
            continue;
#else
        if (dlv.digest_type != DNS_DSDIGEST_SHA256 && dlv.digest_type != DNS_DSDIGEST_SHA1)
            continue;
#endif
...
static isc_boolean_t isdelegation (dns_name_t * name, dns_rdataset_t * rdataset, isc_result_t dbresult)
{
    ...
    {
        result = dns_ncache_getrdataset (rdataset, name, dns_rdatatype_nsec, &set);
  ...
  trynsec3:
    ...
    for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
    {
        ...
        for (result = dns_rdataset_first (&set); result == ISC_R_SUCCESS; result = dns_rdataset_next (&set))
        {
            dns_rdata_reset (&rdata);
            dns_rdataset_current (&set, &rdata);
            ...
            length = isc_iterated_hash (hash, nsec3.hash,
                                        nsec3.iterations, nsec3.salt, nsec3.salt_length, name->ndata, name->length);
            if (length != isc_buffer_usedlength (&buffer))
                continue;
            order = memcmp (hash, owner, length);
            if (order == 0)
            {
                found = dns_nsec3_typepresent (&rdata, dns_rdatatype_ns);
                dns_rdataset_disassociate (&set);
                return (found);
            }
            ...
            if ((scope < 0 && order > 0 &&
                ...
                (scope >= 0 && (order > 0 || memcmp (hash, nsec3.next, length) < 0)))
            {
                dns_rdataset_disassociate (&set);
                return (ISC_TRUE);
            }
...
static void fetch_callback_validator (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        validator_log (val, ISC_LOG_DEBUG (3), "keyset with trust %s", dns_trust_totext (rdataset->trust));
        /*
        ...
        if (result == DNS_R_NOVALIDSIG && (val->attributes & VALATTR_TRIEDVERIFY) == 0)
        {
            saved_result = result;
            validator_log (val, ISC_LOG_DEBUG (3), "falling back to insecurity proof");
    ...
    {
        validator_log (val, ISC_LOG_DEBUG (3), "fetch_callback_validator: got %s", isc_result_totext (eresult));
...
static void dsfetched (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        validator_log (val, ISC_LOG_DEBUG (3), "dsset with trust %s", dns_trust_totext (rdataset->trust));
        val->dsset = &val->frdataset;
        result = validatezonekey (val);
        if (result != DNS_R_WAIT)
            validator_done (val, result);
    }
    else if (eresult == DNS_R_CNAME || eresult == DNS_R_NXRRSET || eresult == DNS_R_NCACHENXRRSET || eresult == DNS_R_SERVFAIL)    /* RFC 1034 parent? */
    {
        validator_log (val, ISC_LOG_DEBUG (3), "falling back to insecurity proof (%s)", dns_result_totext (eresult));
        val->attributes |= VALATTR_INSECURITY;
        result = proveunsecure (val, ISC_FALSE, ISC_FALSE);
        if (result != DNS_R_WAIT)
            validator_done (val, result);
    ...
    {
        validator_log (val, ISC_LOG_DEBUG (3), "dsfetched: got %s", isc_result_totext (eresult));
        if (eresult == ISC_R_CANCELED)
            validator_done (val, eresult);
        else
            validator_done (val, DNS_R_BROKENCHAIN);
...
static void dsfetched2 (isc_task_t * task, isc_event_t * event)
{
    ...
    validator_log (val, ISC_LOG_DEBUG (3), "in dsfetched2: %s", dns_result_totext (eresult));
    LOCK (&val->lock);
    if (CANCELED (val))
    {
        validator_done (val, ISC_R_CANCELED);
    }
    else if (eresult == DNS_R_CNAME || eresult == DNS_R_NXRRSET || eresult == DNS_R_NCACHENXRRSET)
    {
        /*
        ...
        if (eresult != DNS_R_CNAME && isdelegation (tname, &val->frdataset, eresult))
        {
            if (val->mustbesecure)
            {
                validator_log (val, ISC_LOG_WARNING, "must be secure failure, no DS" " and this is a delegation");
    ...
    else if (eresult == ISC_R_SUCCESS || eresult == DNS_R_NXDOMAIN || eresult == DNS_R_NCACHENXDOMAIN)
    {
        /*
         ...
         * validation.
         */
        result = proveunsecure (val, ISC_TF (eresult == ISC_R_SUCCESS), ISC_TRUE);
        if (result != DNS_R_WAIT)
            validator_done (val, result);
    }
    else
...
static void keyvalidated (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        validator_log (val, ISC_LOG_DEBUG (3), "keyset with trust %s", dns_trust_totext (val->frdataset.trust));
        /*
         * Only extract the dst key if the keyset is secure.
         */
    ...
    else
    {
        ...
        validator_log (val, ISC_LOG_DEBUG (3), "keyvalidated: got %s", isc_result_totext (eresult));
        validator_done (val, DNS_R_BROKENCHAIN);
    }
    want_destroy = exit_check (val);
    UNLOCK (&val->lock);
...
static void dsvalidated (isc_task_t * task, isc_event_t * event)
{
    ...
    else if (eresult == ISC_R_SUCCESS)
    {
        ...
        validator_log (val, ISC_LOG_DEBUG (3),
                       ...
                       "dsset" : "ds non-existance", dns_trust_totext (val->frdataset.trust));
        have_dsset = ISC_TF (val->frdataset.type == dns_rdatatype_ds);
        name = dns_fixedname_name (&val->fname);
        if ((val->attributes & VALATTR_INSECURITY) != 0 &&
            val->frdataset.covers == dns_rdatatype_ds &&
            NEGATIVE (&val->frdataset) && isdelegation (name, &val->frdataset, DNS_R_NCACHENXRRSET))
        {
            if (val->mustbesecure)
            {
                validator_log (val, ISC_LOG_WARNING, "must be secure failure, no DS " "and this is a delegation");
    ...
    else
    {
        ...
        validator_log (val, ISC_LOG_DEBUG (3), "dsvalidated: got %s", isc_result_totext (eresult));
        validator_done (val, DNS_R_BROKENCHAIN);
    }
    want_destroy = exit_check (val);
    UNLOCK (&val->lock);
...
static void cnamevalidated (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        validator_log (val, ISC_LOG_DEBUG (3), "cname with trust %s", dns_trust_totext (val->frdataset.trust));
        result = proveunsecure (val, ISC_FALSE, ISC_TRUE);
        if (result != DNS_R_WAIT)
            validator_done (val, result);
    }
    else
    {
        ...
        validator_log (val, ISC_LOG_DEBUG (3), "cnamevalidated: got %s", isc_result_totext (eresult));
        validator_done (val, DNS_R_BROKENCHAIN);
    }
    want_destroy = exit_check (val);
    UNLOCK (&val->lock);
...
nsecnoexistnodata (dns_validator_t * val, dns_name_t * name, dns_name_t * nsecname,
                   dns_rdataset_t * nsecset, isc_boolean_t * exists, isc_boolean_t * data, dns_name_t * wild)
{
    int order;
    ...
    if (result != ISC_R_SUCCESS)
    {
        validator_log (val, ISC_LOG_DEBUG (3), "failure processing NSEC set");
        return (result);
    }
    ...
    if (order < 0)
    {
         ...
         */
        validator_log (val, ISC_LOG_DEBUG (3), "NSEC does not cover name, before NSEC");
        return (ISC_R_IGNORE);
    }
    ...
    if (order == 0)
    {
         ...
         */
        atparent = (olabels != 1) && dns_rdatatype_atparent (val->event->type);
        ...
        else if (atparent && ns && soa)
        {
             ...
             * It can not be legitimately used here.
             */
            validator_log (val, ISC_LOG_DEBUG (3), "ignoring child nsec");
            return (ISC_R_IGNORE);
        }
        if (val->event->type == dns_rdatatype_cname ||
            val->event->type == dns_rdatatype_nxt ||
            val->event->type == dns_rdatatype_nsec ||
            val->event->type == dns_rdatatype_key || !dns_nsec_typepresent (&rdata, dns_rdatatype_cname))
        {
            *exists = ISC_TRUE;
            *data = dns_nsec_typepresent (&rdata, val->event->type);
            validator_log (val, ISC_LOG_DEBUG (3), "nsec proves name exists (owner) data=%d", *data);
    ...
    if (relation == dns_namereln_subdomain &&
        dns_nsec_typepresent (&rdata, dns_rdatatype_ns) && !dns_nsec_typepresent (&rdata, dns_rdatatype_soa))
    {
        /*
         * This NSEC record is from somewhere higher in
         * the DNS, and at the parent of a delegation.
    ...
    if (order == 0)
    {
        ...
        validator_log (val, ISC_LOG_DEBUG (3), "ignoring nsec matches next name");
        return (ISC_R_IGNORE);
    }
    ...
    if (order < 0 && !dns_name_issubdomain (nsecname, &nsec.next))
    {
         ...
         */
        dns_rdata_freestruct (&nsec);
        validator_log (val, ISC_LOG_DEBUG (3), "ignoring nsec because name is past end of range");
        return (ISC_R_IGNORE);
    }

    if (order > 0 && relation == dns_namereln_subdomain)
    {
        validator_log (val, ISC_LOG_DEBUG (3), "nsec proves name exist (empty)");
        dns_rdata_freestruct (&nsec);
        *exists = ISC_TRUE;
        *data = ISC_FALSE;
        return (ISC_R_SUCCESS);
    ...
    if (wild != NULL)
    {
        ...
        if (olabels > nlabels)
        {
            ...
            dns_name_getlabelsequence (nsecname, labels - olabels, olabels, &common);
        }
        else
        {
            labels = dns_name_countlabels (&nsec.next);
            dns_name_getlabelsequence (&nsec.next, labels - nlabels, nlabels, &common);
        }
        result = dns_name_concatenate (dns_wildcardname, &common, wild, NULL);
        if (result != ISC_R_SUCCESS)
        {
            dns_rdata_freestruct (&nsec);
            validator_log (val, ISC_LOG_DEBUG (3), "failure generating wildcard name");
            return (result);
        }
    }
    dns_rdata_freestruct (&nsec);
...
nsec3noexistnodata (dns_validator_t * val, dns_name_t * name,
                    ...
                    isc_boolean_t * setnearest, dns_name_t * closest, dns_name_t * nearest)
{
    ...
    REQUIRE ((exists == NULL && data == NULL) || (exists != NULL && data != NULL));
    REQUIRE (nsec3set != NULL && nsec3set->type == dns_rdatatype_nsec3);
    REQUIRE ((setclosest == NULL && closest == NULL) || (setclosest != NULL && closest != NULL));
    REQUIRE ((setnearest == NULL && nearest == NULL) || (setnearest != NULL && nearest != NULL));

    result = dns_rdataset_first (nsec3set);
    if (result != ISC_R_SUCCESS)
    {
        validator_log (val, ISC_LOG_DEBUG (3), "failure processing NSEC3 set");
        return (result);
    }
     ...
     */
    if (dns_name_countlabels (zonename) == 0 || dns_name_issubdomain (zone, zonename))
    ...
    while (qlabels >= zlabels)
    {
        length = isc_iterated_hash (hash, nsec3.hash, nsec3.iterations,
                                    nsec3.salt, nsec3.salt_length, qname->ndata, qname->length);
        /*
         * The computed hash length should match.
         */
        if (length != nsec3.next_length)
        {
            validator_log (val, ISC_LOG_DEBUG (3), "ignoring NSEC bad length %u vs %u", length, nsec3.next_length);
            return (ISC_R_IGNORE);
        }
        ...
        if (first && order == 0)
        {
            ...
            if (ns && !soa)
            {
                if (!atparent)
                {
                     ...
                     */
                    validator_log (val, ISC_LOG_DEBUG (3), "ignoring parent NSEC3");
                    return (ISC_R_IGNORE);
                }
            }
            else if (atparent && ns && soa)
            {
                 ...
                 * It can not be legitimately used here.
                 */
                validator_log (val, ISC_LOG_DEBUG (3), "ignoring child NSEC3");
                return (ISC_R_IGNORE);
            }
            if (val->event->type == dns_rdatatype_cname ||
                val->event->type == dns_rdatatype_nxt ||
                val->event->type == dns_rdatatype_nsec ||
                val->event->type == dns_rdatatype_key || !dns_nsec3_typepresent (&rdata, dns_rdatatype_cname))
            {
                *exists = ISC_TRUE;
                *data = dns_nsec3_typepresent (&rdata, val->event->type);
                validator_log (val, ISC_LOG_DEBUG (3), "NSEC3 proves name exists (owner) " "data=%d", *data);
                return (ISC_R_SUCCESS);
            }
            validator_log (val, ISC_LOG_DEBUG (3), "NSEC3 proves CNAME exists");
            return (ISC_R_IGNORE);
        }

        if (order == 0 &&
            dns_nsec3_typepresent (&rdata, dns_rdatatype_ns) && !dns_nsec3_typepresent (&rdata, dns_rdatatype_soa))
        {
            /*
        ...
        if (order == 0)
        {
                 ...
                 !dns_nsec3_typepresent (&rdata, dns_rdatatype_ns)))
            {
                ...
                validator_log (val, ISC_LOG_DEBUG (3), "NSEC3 indicates potential " "closest encloser: '%s'", namebuf);
                dns_name_copy (qname, closest, NULL);
                *setclosest = ISC_TRUE;
            }
            dns_name_format (qname, namebuf, sizeof (namebuf));
            validator_log (val, ISC_LOG_DEBUG (3), "NSEC3 at super-domain %s", namebuf);
            return (answer);
        }
            ...
            (scope >= 0 && (order > 0 || memcmp (hash, nsec3.next, length) < 0)))
        {
            ...
            dns_name_format (qname, namebuf, sizeof (namebuf));
            validator_log (val, ISC_LOG_DEBUG (3), "NSEC3 proves " "name does not exist: '%s'", namebuf);
            if (nearest != NULL && (dns_name_countlabels (nearest) == 0 || dns_name_issubdomain (nearest, qname)))
            {
            ...
            *data = ISC_FALSE;
            ...
            {
                if ((nsec3.flags & DNS_NSEC3FLAG_OPTOUT) != 0)
                    validator_log (val, ISC_LOG_DEBUG (3), "NSEC3 indicates optout");
                *optout = ISC_TF (nsec3.flags & DNS_NSEC3FLAG_OPTOUT);
            }
...
static void authvalidated (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        validator_log (val, ISC_LOG_DEBUG (3), "authvalidated: got %s", isc_result_totext (result));
    ...
    else
    {
        ...
        if (rdataset->type == dns_rdatatype_nsec &&
            ...
            nsecnoexistnodata (val, val->event->name, devent->name, rdataset, &exists, &data, wild) == ISC_R_SUCCESS)
        {
            if (exists && !data)
            {
                val->attributes |= VALATTR_FOUNDNODATA;
                if (NEEDNODATA (val))
                    proofs[DNS_VALIDATOR_NODATAPROOF] = devent->name;
            }
            if (!exists)
            {
                 ...
                 */
                if (NEEDNOQNAME (val))
                    proofs[DNS_VALIDATOR_NOQNAMEPROOF] = devent->name;
            }
        }
 ...
 */
static inline isc_result_t view_find (dns_validator_t * val, dns_name_t * name, dns_rdatatype_t type)
{
    ...
    if (isc_time_now (&now) == ISC_R_SUCCESS && dns_resolver_getbadcache (val->view->resolver, name, type, &now))
    {

        dns_name_format (name, namebuf, sizeof (namebuf));
        dns_rdatatype_format (type, typebuf, sizeof (typebuf));
        validator_log (val, ISC_LOG_INFO, "bad cache hit (%s/%s)", namebuf, typebuf);
    ...
    result = dns_view_find (val->view, name, type, 0, options,
                            ISC_FALSE, NULL, NULL, foundname, &val->frdataset, &val->fsigrdataset);
    ...
    else if (result == DNS_R_COVERINGNSEC)
    {
        ...
        if (val->frdataset.trust != dns_trust_secure)
        {
            validator_log (val, ISC_LOG_DEBUG (3), "covering nsec: trust %s", dns_trust_totext (val->frdataset.trust));
            goto notfound;
        }
        ...
        if (dns_nsec_typepresent (&rdata, dns_rdatatype_ns) && !dns_nsec_typepresent (&rdata, dns_rdatatype_soa))
        {
            /* Parent NSEC record. */
            if (dns_name_issubdomain (name, foundname))
            {
                validator_log (val, ISC_LOG_DEBUG (3), "covering nsec: for parent");
                goto notfound;
            }
        }
        result = dns_rdata_tostruct (&rdata, &nsec, NULL);
        ...
        if (dns_name_compare (foundname, &nsec.next) >= 0)
        {
            ...
            if (!dns_name_issubdomain (name, &nsec.next))
            {
                 ...
                 */
                dns_rdata_freestruct (&nsec);
                validator_log (val, ISC_LOG_DEBUG (3), "covering nsec: not in zone");
                goto notfound;
            }
        }
        else if (dns_name_compare (name, &nsec.next) >= 0)
        {
             ...
             */
            dns_rdata_freestruct (&nsec);
            validator_log (val, ISC_LOG_DEBUG (3), "covering nsec: not in range");
            goto notfound;
        }
        if (isc_log_wouldlog (dns_lctx, ISC_LOG_DEBUG (3)))
        {
            ...
            validator_log (val, ISC_LOG_DEBUG (3), "covering nsec found: '%s' '%s' '%s'", buf1, buf2, buf3);
        }
        if (dns_rdataset_isassociated (&val->frdataset))
            dns_rdataset_disassociate (&val->frdataset);
        if (dns_rdataset_isassociated (&val->fsigrdataset))
        ...
        result = DNS_R_NCACHENXDOMAIN;
             ...
             result != DNS_R_EMPTYNAME && result != DNS_R_NXRRSET && result != ISC_R_NOTFOUND)
    {
        goto notfound;
    }
    return (result);
                ...
                dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    ...
    {
        if (parent->event != NULL && parent->event->type == type && dns_name_equal (parent->event->name, name) &&
            /*
            ...
            (parent->event->type != dns_rdatatype_nsec3 ||
             ...
             parent->event->message == NULL || parent->event->rdataset != NULL || parent->event->sigrdataset != NULL))
        {
            validator_log (val, ISC_LOG_DEBUG (3),
              ...
              isc_taskaction_t callback, const char *caller)
{
    ...
    {
        validator_log (val, ISC_LOG_DEBUG (3), "deadlock found (create_fetch)");
    ...
    return (dns_resolver_createfetch (val->view->resolver, name, type,
                                      NULL, NULL, NULL, 0,
                                      val->event->ev_sender,
                                      callback, val, &val->frdataset, &val->fsigrdataset, &val->fetch));
}
...
create_validator (dns_validator_t * val, dns_name_t * name, dns_rdatatype_t type,
                  dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset, isc_taskaction_t action, const char *caller)
{
    isc_result_t result;

    if (check_deadlock (val, name, type, rdataset, sigrdataset))
    {
        validator_log (val, ISC_LOG_DEBUG (3), "deadlock found (create_validator)");
        return (DNS_R_NOVALIDSIG);
    }

    validator_logcreate (val, name, type, caller, "validator");
    result = dns_validator_create (val->view, name, type,
                                   rdataset, sigrdataset, NULL, 0, val->task, action, val, &val->subvalidator);
    if (result == ISC_R_SUCCESS)
    {
...
static isc_result_t get_dst_key (dns_validator_t * val, dns_rdata_rrsig_t * siginfo, dns_rdataset_t * rdataset)
{
    ...
    do
    {
        ...
        INSIST (val->key == NULL);
        result = dst_key_fromdns (&siginfo->signer, rdata.rdclass, &b, val->view->mctx, &val->key);
        if (result != ISC_R_SUCCESS)
            goto failure;
        if (siginfo->algorithm ==
            (dns_secalg_t) dst_key_alg (val->key) &&
            siginfo->keyid == (dns_keytag_t) dst_key_id (val->key) && dst_key_iszonekey (val->key))
        {
            if (foundold)
                /*
...
static isc_result_t get_key (dns_validator_t * val, dns_rdata_rrsig_t * siginfo)
{
     ...
     */
    namereln = dns_name_fullcompare (val->event->name, &siginfo->signer, &order, &nlabels);
    if (namereln != dns_namereln_subdomain && namereln != dns_namereln_equal)
    ...
    else
    {
         ...
         * the same name.
         */
        if (val->event->rdataset->type == dns_rdatatype_soa || val->event->rdataset->type == dns_rdatatype_ns)
        {
            const char *typename;
            ...
            if (val->event->rdataset->type == dns_rdatatype_soa)
                ...
                typename = "NS";
            validator_log (val, ISC_LOG_DEBUG (3), "%s signer mismatch", typename);
            return (DNS_R_CONTINUE);
        }
    }
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        if ((DNS_TRUST_PENDING (val->frdataset.trust) ||
             DNS_TRUST_ANSWER (val->frdataset.trust)) && dns_rdataset_isassociated (&val->fsigrdataset))
        {
            /*
            ...
            result = create_validator (val, &siginfo->signer,
                                       ...
                                       &val->frdataset, &val->fsigrdataset, keyvalidated, "get_key");
            if (result != ISC_R_SUCCESS)
                return (result);
            return (DNS_R_WAIT);
        }
    ...
    else if (result == ISC_R_NOTFOUND)
    {
         ...
         */
        result = create_fetch (val, &siginfo->signer, dns_rdatatype_dnskey, fetch_callback_validator, "get_key");
        if (result != ISC_R_SUCCESS)
            return (result);
        return (DNS_R_WAIT);
    }
             ...
             result == DNS_R_NCACHENXRRSET ||
             result == DNS_R_EMPTYNAME || result == DNS_R_NXDOMAIN || result == DNS_R_NXRRSET)
    {
        /*
    ...
    if (dns_rdataset_isassociated (&val->frdataset) && val->keyset != &val->frdataset)
        dns_rdataset_disassociate (&val->frdataset);
    if (dns_rdataset_isassociated (&val->fsigrdataset))
        dns_rdataset_disassociate (&val->fsigrdataset);
...
static isc_boolean_t isselfsigned (dns_validator_t * val)
{
    ...
    for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
    {
        ...
        for (result = dns_rdataset_first (sigrdataset);
             result == ISC_R_SUCCESS; result = dns_rdataset_next (sigrdataset))
        {
            ...
            if (sig.algorithm != key.algorithm || sig.keyid != keytag || !dns_name_equal (name, &sig.signer))
                continue;
            ...
            result = dns_dnssec_verify2 (name, rdataset, dstkey,
                                         ISC_TRUE, mctx, &sigrdata, dns_fixedname_name (&fixed));
            dst_key_free (&dstkey);
            if (result != ISC_R_SUCCESS)
                continue;
...
static isc_result_t verify (dns_validator_t * val, dst_key_t * key, dns_rdata_t * rdata, isc_uint16_t keyid)
{
  ...
  again:
    result = dns_dnssec_verify2 (val->event->name, val->event->rdataset, key, ignore, val->view->mctx, rdata, wild);
    if ((result == DNS_R_SIGEXPIRED || result == DNS_R_SIGFUTURE) && val->view->acceptexpired)
    {
        ignore = ISC_TRUE;
        goto again;
    }
    if (ignore && (result == ISC_R_SUCCESS || result == DNS_R_FROMWILDCARD))
        validator_log (val, ISC_LOG_INFO,
                       "accepted expired %sRRSIG (keyid=%u)", (result == DNS_R_FROMWILDCARD) ? "wildcard " : "", keyid);
    else if (result == DNS_R_SIGEXPIRED || result == DNS_R_SIGFUTURE)
        validator_log (val, ISC_LOG_INFO,
                       "verify failed due to bad signature (keyid=%u): " "%s", keyid, isc_result_totext (result));
    else
        validator_log (val, ISC_LOG_DEBUG (3), "verify rdataset (keyid=%u): %s", keyid, isc_result_totext (result));
...
static isc_result_t validate (dns_validator_t * val, isc_boolean_t resume)
{
    ...
    for (; result == ISC_R_SUCCESS; result = dns_rdataset_next (event->sigrdataset))
    {
        ...
        if (val->siginfo == NULL)
        {
            val->siginfo = isc_mem_get (val->view->mctx, sizeof (*val->siginfo));
            if (val->siginfo == NULL)
         ...
         */
        if (!dns_resolver_algorithm_supported (val->view->resolver, event->name, val->siginfo->algorithm))
        {
            resume = ISC_FALSE;
            continue;
        }
        ...
        do
        {
            result = verify (val, val->key, &rdata, val->siginfo->keyid);
            if (result == ISC_R_SUCCESS)
                break;
            if (val->keynode != NULL)
            {
                ...
                result = dns_keytable_findnextkeynode (val->keytable, val->keynode, &nextnode);
            ...
            {
                if (get_dst_key (val, val->siginfo, val->keyset) != ISC_R_SUCCESS)
                    break;
            }
        }
        while (1);
        if (result != ISC_R_SUCCESS)
            validator_log (val, ISC_LOG_DEBUG (3), "failed to verify rdataset");
        else
        {
            isc_uint32_t ttl;
            ...
            isc_stdtime_get (&now);
            ttl = ISC_MIN (event->rdataset->ttl, ISC_MIN (val->siginfo->originalttl, val->siginfo->timeexpire - now));
        ...
        if (NEEDNOQNAME (val))
        {
            ...
            {
                validator_log (val, ISC_LOG_DEBUG (3), "no message available for noqname proof");
                return (DNS_R_NOVALIDSIG);
            }
            validator_log (val, ISC_LOG_DEBUG (3), "looking for noqname proof");
            return (nsecvalidate (val, ISC_FALSE));
        }
        else if (result == ISC_R_SUCCESS)
        {
            marksecure (event);
            validator_log (val, ISC_LOG_DEBUG (3), "marking as secure, " "noqname proof not needed");
            return (result);
        }
        else
        {
            validator_log (val, ISC_LOG_DEBUG (3), "verify failure: %s", isc_result_totext (result));
            resume = ISC_FALSE;
        }
    }
    if (result != ISC_R_NOMORE)
    {
        validator_log (val, ISC_LOG_DEBUG (3), "failed to iterate signatures: %s", isc_result_totext (result));
        return (result);
    }
...
static isc_result_t checkkey (dns_validator_t * val, dns_rdata_t * keyrdata, isc_uint16_t keyid, dns_secalg_t algorithm)
{
         ...
         result == ISC_R_SUCCESS; result = dns_rdataset_next (val->event->sigrdataset))
    {
        ...
        if (dstkey == NULL)
        {
            result = dns_dnssec_keyfromrdata (val->event->name, keyrdata, val->view->mctx, &dstkey);
            if (result != ISC_R_SUCCESS)
                /*
...
keyfromds (dns_validator_t * val, dns_rdataset_t * rdataset, dns_rdata_t * dsrdata,
           isc_uint8_t digest, isc_uint16_t keyid, dns_secalg_t algorithm, dns_rdata_t * keyrdata)
{
    dns_keytag_t keytag;
    ...
    for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
    {
        ...
        result = dns_ds_buildrdata (val->event->name, keyrdata, digest, dsbuf, &newdsrdata);
        if (result != ISC_R_SUCCESS)
        {
            validator_log (val, ISC_LOG_DEBUG (3), "dns_ds_buildrdata() -> %s", dns_result_totext (result));
            continue;
        }
...
static isc_result_t dlv_validatezonekey (dns_validator_t * val)
{
    ...
    for (result = dns_rdataset_first (&val->dlv); result == ISC_R_SUCCESS; result = dns_rdataset_next (&val->dlv))
    {
        ...
        if (dlv.digest_type == DNS_DSDIGEST_SHA256 && dlv.length == ISC_SHA256_DIGESTLENGTH)
        {
    ...
    for (result = dns_rdataset_first (&val->dlv); result == ISC_R_SUCCESS; result = dns_rdataset_next (&val->dlv))
    {
        ...
        if (!dns_resolver_digest_supported (val->view->resolver, dlv.digest_type))
            continue;
        ...
        result = keyfromds (val, &trdataset, &dlvrdata, dlv.digest_type, dlv.key_tag, dlv.algorithm, &keyrdata);
        if (result != ISC_R_SUCCESS)
        {
            dns_rdataset_disassociate (&trdataset);
            validator_log (val, ISC_LOG_DEBUG (3), "no DNSKEY matching DLV");
            continue;
        }

        validator_log (val, ISC_LOG_DEBUG (3), "Found matching DLV record: checking for signature");
        /*
        ...
        if (result == ISC_R_SUCCESS)
            break;
        validator_log (val, ISC_LOG_DEBUG (3), "no RRSIG matching DLV key");
    }
    if (result == ISC_R_SUCCESS)
    {
    ...
    else if (result == ISC_R_NOMORE && !supported_algorithm)
    {
        if (val->mustbesecure)
        {
            validator_log (val, ISC_LOG_WARNING, "must be secure failure," "no supported algorithm/digest (dlv)");
            return (DNS_R_MUSTBESECURE);
        }
        validator_log (val, ISC_LOG_DEBUG (3), "no supported algorithm/digest (dlv)");
...
static isc_result_t validatezonekey (dns_validator_t * val)
{
    ...
    if (val->dsset == NULL)
    {
             ...
             result == ISC_R_SUCCESS; result = dns_rdataset_next (val->event->sigrdataset))
        {
            ...
            result = dns_keytable_findkeynode (val->keytable, val->event->name, sig.algorithm, sig.keyid, &keynode);
            if (result == ISC_R_NOTFOUND &&
                dns_keytable_finddeepestmatch (val->keytable, val->event->name, found) != ISC_R_SUCCESS)
            {
                if (val->mustbesecure)
                {
                    validator_log (val, ISC_LOG_WARNING, "must be secure failure, " "not beneath secure root");
                    return (DNS_R_MUSTBESECURE);
                }
                else
            ...
            while (result == ISC_R_SUCCESS)
            {
                ...
                result = dns_keytable_findnextkeynode (val->keytable, keynode, &nextnode);
                dns_keytable_detachkeynode (val->keytable, &keynode);
                keynode = nextnode;
            }
            if (result == ISC_R_SUCCESS)
            {
                marksecure (event);
                validator_log (val, ISC_LOG_DEBUG (3), "signed by trusted key; " "marking as secure");
                return (result);
            }
        }
        ...
        if (atsep)
        {
            ...
            validator_log (val, ISC_LOG_NOTICE,
                           ...
                           "the DNSKEY RRset and also matches a " "trusted key for '%s'", namebuf);
            validator_log (val, ISC_LOG_NOTICE, "please check the 'trusted-keys' for " "'%s' in named.conf.", namebuf);
        ...
        if (dns_name_equal (event->name, dns_rootname))
        {
            ...
            {
                validator_log (val, ISC_LOG_DEBUG (3), "root key failed to validate");
                return (DNS_R_NOVALIDSIG);
            }
            else
            {
                validator_log (val, ISC_LOG_DEBUG (3), "no trusted root key");
      ...
      find_ds:
        ...
        if (result == ISC_R_SUCCESS)
        {
                 ...
                 DNS_TRUST_ANSWER (val->frdataset.trust)) && dns_rdataset_isassociated (&val->fsigrdataset))
            {
                result = create_validator (val,
                                           val->event->name,
                                           dns_rdatatype_ds,
                                           &val->frdataset, &val->fsigrdataset, dsvalidated, "validatezonekey");
                if (result != ISC_R_SUCCESS)
                    return (result);
                return (DNS_R_WAIT);
            }
            else if (DNS_TRUST_PENDING (val->frdataset.trust))
            {
                 ...
                 */
                dns_rdataset_disassociate (&val->frdataset);
                validator_log (val, ISC_LOG_DEBUG (2), "unsigned DS record");
                return (DNS_R_NOVALIDSIG);
            }
            else
            {
        ...
        else if (result == ISC_R_NOTFOUND)
        {
             ...
             */
            result = create_fetch (val, val->event->name, dns_rdatatype_ds, dsfetched, "validatezonekey");
            if (result != ISC_R_SUCCESS)
                return (result);
            return (DNS_R_WAIT);
        }
                 ...
                 result == DNS_R_NXDOMAIN || result == DNS_R_NXRRSET || result == DNS_R_CNAME)
        {
            /*
    ...
    if (val->dsset->trust < dns_trust_secure)
    {
        if (val->mustbesecure)
        {
            validator_log (val, ISC_LOG_WARNING, "must be secure failure," " insecure DS");
            return (DNS_R_MUSTBESECURE);
        }
     ...
     */
    memset (digest_types, 1, sizeof (digest_types));
    for (result = dns_rdataset_first (val->dsset); result == ISC_R_SUCCESS; result = dns_rdataset_next (val->dsset))
    {
        ...
        if (!dns_resolver_algorithm_supported (val->view->resolver, val->event->name, ds.algorithm))
            continue;

        if (ds.digest_type == DNS_DSDIGEST_SHA256 && ds.length == ISC_SHA256_DIGESTLENGTH)
        {
            digest_types[DNS_DSDIGEST_SHA1] = 0;
            break;
        }
    ...
    for (result = dns_rdataset_first (val->dsset); result == ISC_R_SUCCESS; result = dns_rdataset_next (val->dsset))
    {
        ...
        if (!dns_resolver_digest_supported (val->view->resolver, ds.digest_type))
            continue;
         ...
         */
        result = keyfromds (val, &trdataset, &dsrdata, ds.digest_type, ds.key_tag, ds.algorithm, &keyrdata);
        if (result != ISC_R_SUCCESS)
        {
            dns_rdataset_disassociate (&trdataset);
        ...
        if (result == ISC_R_SUCCESS)
            break;
        validator_log (val, ISC_LOG_DEBUG (3), "no RRSIG matching DS key");
    }
    if (result == ISC_R_SUCCESS)
    {
        marksecure (event);
    ...
    else if (result == ISC_R_NOMORE && !supported_algorithm)
    {
        if (val->mustbesecure)
        {
            validator_log (val, ISC_LOG_WARNING, "must be secure failure, " "no supported algorithm/digest (DS)");
            return (DNS_R_MUSTBESECURE);
        }
        validator_log (val, ISC_LOG_DEBUG (3), "no supported algorithm/digest (DS)");
        markanswer (val, "validatezonekey (3)");
        return (ISC_R_SUCCESS);
    }
    else
    {
        validator_log (val, ISC_LOG_INFO, "no valid signature found (DS)");
        return (DNS_R_NOVALIDSIG);
    }
}
...
static isc_result_t val_rdataset_first (dns_validator_t * val, dns_name_t ** namep, dns_rdataset_t ** rdatasetp)
{
    ...
    else
    {
        ...
        if (result == ISC_R_SUCCESS)
            dns_ncache_current (val->event->rdataset, *namep, *rdatasetp);
    }
    return (result);
}
...
static isc_result_t checkwildcard (dns_validator_t * val, dns_rdatatype_t type, dns_name_t * zonename)
{
    ...
    {
        validator_log (val, ISC_LOG_DEBUG (3), "in checkwildcard: no wildcard to check");
        return (ISC_R_SUCCESS);
    }
    ...
    for (result = val_rdataset_first (val, &name, &rdataset);
         result == ISC_R_SUCCESS; result = val_rdataset_next (val, &name, &rdataset))
    {
        if (rdataset->type != type || rdataset->trust != dns_trust_secure)
            continue;

        if (rdataset->type == dns_rdatatype_nsec &&
            (NEEDNODATA (val) || NEEDNOWILDCARD (val)) &&
            !FOUNDNODATA (val) && !FOUNDNOWILDCARD (val) &&
            nsecnoexistnodata (val, wild, name, rdataset, &exists, &data, NULL) == ISC_R_SUCCESS)
        {
            dns_name_t **proofs = val->event->proofs;
        ...
        if (rdataset->type == dns_rdatatype_nsec3 &&
            ...
            nsec3noexistnodata (val, wild, name, rdataset,
                                zonename, &exists, &data, NULL, NULL, NULL, NULL, NULL, NULL) == ISC_R_SUCCESS)
        {
            dns_name_t **proofs = val->event->proofs;
...
static isc_result_t findnsec3proofs (dns_validator_t * val)
{
    ...
    for (result = val_rdataset_first (val, &name, &rdataset);
         result == ISC_R_SUCCESS; result = val_rdataset_next (val, &name, &rdataset))
    {
        if (rdataset->type != dns_rdatatype_nsec3 || rdataset->trust != dns_trust_secure)
            continue;

        result = nsec3noexistnodata (val, val->event->name,
                                     name, rdataset, zonename, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
    ...
    if (dns_name_countlabels (dns_fixedname_name (&val->closest)) != 0)
    {
        ...
        dns_name_format (dns_fixedname_name (&val->closest), namebuf, sizeof (namebuf));
        validator_log (val, ISC_LOG_DEBUG (3), "closest encloser from " "wildcard signature '%s'", namebuf);
         ...
         result == ISC_R_SUCCESS; result = val_rdataset_next (val, &name, &rdataset))
    {
        ...
        (void) nsec3noexistnodata (val, val->event->name, name, rdataset,
                                   ...
                                   &unknown, setclosestp, &setnearest, closestp, nearest);
        if (setclosest)
    ...
    if (dns_name_countlabels (closest) > 0 &&
        dns_name_countlabels (nearest) == dns_name_countlabels (closest) + 1 && dns_name_issubdomain (nearest, closest))
    {
        val->attributes |= VALATTR_FOUNDCLOSEST;
        result = dns_name_concatenate (dns_wildcardname, closest, dns_fixedname_name (&val->wild), NULL);
        RUNTIME_CHECK (result == ISC_R_SUCCESS);
    }
    else
    {
     ...
     */
    if (FOUNDNOQNAME (val) && FOUNDCLOSEST (val) && ((NEEDNODATA (val) && !FOUNDNODATA (val)) || NEEDNOWILDCARD (val)))
    {
...
static isc_result_t validate_authority (dns_validator_t * val, isc_boolean_t resume)
{
    ...
    for (; result == ISC_R_SUCCESS; result = dns_message_nextname (message, DNS_SECTION_AUTHORITY))
    {
        ...
        for (; rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
        {
            ...
            if (val->event->type == dns_rdatatype_dnskey &&
                rdataset->type == dns_rdatatype_nsec && dns_name_equal (name, val->event->name))
            {
            ...
            result = create_validator (val, name, rdataset->type,
                                       rdataset, sigrdataset, authvalidated, "validate_authority");
            if (result != ISC_R_SUCCESS)
                return (result);
            val->authcount++;
            return (DNS_R_WAIT);
...
static isc_result_t validate_ncache (dns_validator_t * val, isc_boolean_t resume)
{
    ...
    for (; result == ISC_R_SUCCESS; result = dns_rdataset_next (val->event->rdataset))
    {
        ...
        result = dns_ncache_getsigrdataset (val->event->rdataset, name, rdataset->type, &val->fsigrdataset);
        if (result == ISC_R_SUCCESS)
        ...
        val->currentset = rdataset;
        result = create_validator (val, name, rdataset->type, rdataset, sigrdataset, authvalidated, "validate_ncache");
        if (result != ISC_R_SUCCESS)
            return (result);
        val->authcount++;
        return (DNS_R_WAIT);
...
static isc_result_t nsecvalidate (dns_validator_t * val, isc_boolean_t resume)
{
    ...
    if (!NEEDNODATA (val) && !NEEDNOWILDCARD (val) && NEEDNOQNAME (val))
    {
        ...
        {
            validator_log (val, ISC_LOG_DEBUG (3), "marking as secure, noqname proof found");
            marksecure (val->event);
            return (ISC_R_SUCCESS);
        }
        else if (FOUNDOPTOUT (val) && dns_name_countlabels (dns_fixedname_name (&val->wild)) != 0)
        {
            validator_log (val, ISC_LOG_DEBUG (3), "optout proof found");
        ...
        {
            validator_log (val, ISC_LOG_DEBUG (3), "unknown NSEC3 hash algorithm found");
    ...
    if ((NEEDNODATA (val) && (FOUNDNODATA (val) || FOUNDOPTOUT (val))) ||
        (NEEDNOQNAME (val) && FOUNDNOQNAME (val) &&
         NEEDNOWILDCARD (val) && FOUNDNOWILDCARD (val) && FOUNDCLOSEST (val)))
    {
        if ((val->attributes & VALATTR_FOUNDOPTOUT) != 0)
            val->event->optout = ISC_TRUE;
        validator_log (val, ISC_LOG_DEBUG (3), "nonexistence proof(s) found");
        if (val->event->message == NULL)
            marksecure (val->event);
        return (ISC_R_SUCCESS);
    }
...
static isc_boolean_t check_ds (dns_validator_t * val, dns_name_t * name, dns_rdataset_t * rdataset)
{
    ...
    for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
    {
        ...
        if (dns_resolver_digest_supported (val->view->resolver,
                                           ds.digest_type) &&
            dns_resolver_algorithm_supported (val->view->resolver, name, ds.algorithm))
        {
            dns_rdata_reset (&dsrdata);
            return (ISC_TRUE);
        }
...
static void dlvvalidated (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        validator_log (val, ISC_LOG_DEBUG (3), "dlvset with trust %s", dns_trust_totext (val->frdataset.trust));
    ...
    else
    {
        ...
        validator_log (val, ISC_LOG_DEBUG (3), "dlvvalidated: got %s", isc_result_totext (eresult));
        validator_done (val, DNS_R_BROKENCHAIN);
    }
    want_destroy = exit_check (val);
    UNLOCK (&val->lock);
...
static void dlvfetched (isc_task_t * task, isc_event_t * event)
{
    ...
    INSIST (val->event != NULL);
    validator_log (val, ISC_LOG_DEBUG (3), "in dlvfetched: %s", dns_result_totext (eresult));

    LOCK (&val->lock);
    if (eresult == ISC_R_SUCCESS)
    {
        dns_name_format (dns_fixedname_name (&val->dlvsep), namebuf, sizeof (namebuf));
        dns_rdataset_clone (&val->frdataset, &val->dlv);
        val->havedlvsep = ISC_TRUE;
        if (dlv_algorithm_supported (val))
        {
        ...
        {
            validator_log (val, ISC_LOG_DEBUG (3), "DLV %s found with no supported algorithms", namebuf);
            markanswer (val, "dlvfetched (1)");
            validator_done (val, ISC_R_SUCCESS);
        }
    }
    else if (eresult == DNS_R_NXRRSET ||
             eresult == DNS_R_NXDOMAIN || eresult == DNS_R_NCACHENXRRSET || eresult == DNS_R_NCACHENXDOMAIN)
    {
        ...
        {
            validator_log (val, ISC_LOG_DEBUG (3), "DLV lookup: %s", dns_result_totext (result));
            if (result != DNS_R_WAIT)
                validator_done (val, result);
        }
    }
    ...
    {
        validator_log (val, ISC_LOG_DEBUG (3), "DLV lookup: %s", dns_result_totext (eresult));
        validator_done (val, eresult);
    }
    want_destroy = exit_check (val);
    UNLOCK (&val->lock);
...
static isc_result_t startfinddlvsep (dns_validator_t * val, dns_name_t * unsecure)
{
    ...
    validator_log (val, ISC_LOG_DEBUG (3), "plain DNSSEC returns unsecure (%s): looking for DLV", namebuf);

    if (dns_name_issubdomain (val->event->name, val->view->dlv))
    {
        validator_log (val, ISC_LOG_WARNING, "must be secure failure, " " %s is under DLV (startfinddlvsep)", namebuf);
        return (DNS_R_MUSTBESECURE);
    }
...
static isc_result_t finddlvsep (dns_validator_t * val, isc_boolean_t resume)
{
    ...
    if (!resume)
    {
        ...
        {
            dns_name_format (val->event->name, namebuf, sizeof (namebuf));
            validator_log (val, ISC_LOG_WARNING, "must be secure failure, " "%s is under DLV (finddlvsep)", namebuf);
            return (DNS_R_MUSTBESECURE);
        }
    ...
    while (dns_name_countlabels (dlvname) >= dns_name_countlabels (val->view->dlv) + val->dlvlabels)
    {
        dns_name_format (dlvname, namebuf, sizeof (namebuf));
        validator_log (val, ISC_LOG_DEBUG (3), "looking for DLV %s", namebuf);
        result = view_find (val, dlvname, dns_rdatatype_dlv);
        if (result == ISC_R_SUCCESS)
        {
            if (DNS_TRUST_PENDING (val->frdataset.trust) && dns_rdataset_isassociated (&val->fsigrdataset))
            {
                dns_fixedname_init (&val->fname);
                dns_name_copy (dlvname, dns_fixedname_name (&val->fname), NULL);
                result = create_validator (val,
                                           dns_fixedname_name (&val->fname),
                                           dns_rdatatype_dlv,
                                           &val->frdataset, &val->fsigrdataset, dlvvalidated, "finddlvsep");
                if (result != ISC_R_SUCCESS)
                    return (result);
                return (DNS_R_WAIT);
            }
            if (val->frdataset.trust < dns_trust_secure)
            {
                validator_log (val, ISC_LOG_DEBUG (3), "DLV not validated");
                return (DNS_R_NOVALIDSIG);
            }
            val->havedlvsep = ISC_TRUE;
            dns_rdataset_clone (&val->frdataset, &val->dlv);
        ...
        if (result == ISC_R_NOTFOUND)
        {
            result = create_fetch (val, dlvname, dns_rdatatype_dlv, dlvfetched, "finddlvsep");
            if (result != ISC_R_SUCCESS)
                return (result);
            return (DNS_R_WAIT);
        }
        if (result != DNS_R_NXRRSET &&
            ...
            result != DNS_R_EMPTYNAME && result != DNS_R_NCACHENXRRSET && result != DNS_R_NCACHENXDOMAIN)
            return (result);
        /*
...
static isc_result_t proveunsecure (dns_validator_t * val, isc_boolean_t have_ds, isc_boolean_t resume)
{
    ...
    else
    {
        ...
        if (have_ds && val->frdataset.trust >= dns_trust_secure &&
            !check_ds (val, dns_fixedname_name (&val->fname), &val->frdataset))
        {
            dns_name_format (dns_fixedname_name (&val->fname), namebuf, sizeof (namebuf));
            if ((val->view->dlv == NULL || DLVTRIED (val)) && val->mustbesecure)
            {
                validator_log (val, ISC_LOG_WARNING,
                               "must be secure failure at '%s', " "can't fall back to DLV", namebuf);
                result = DNS_R_MUSTBESECURE;
                goto out;
            }
            validator_log (val, ISC_LOG_DEBUG (3), "no supported algorithm/digest (%s/DS)", namebuf);
            if (val->view->dlv == NULL || DLVTRIED (val))
            {
            ...
            return (startfinddlvsep (val, dns_fixedname_name (&val->fname)));
        }
        val->labels++;
    }

    for (; val->labels <= dns_name_countlabels (val->event->name); val->labels++)
    {
        ...
        validator_log (val, ISC_LOG_DEBUG (3), "checking existence of DS at '%s'", namebuf);
        ...
        if (result == DNS_R_NXRRSET || result == DNS_R_NCACHENXRRSET)
        {
             ...
             */
            if (DNS_TRUST_PENDING (val->frdataset.trust) || DNS_TRUST_ANSWER (val->frdataset.trust))
            {
                result = create_validator (val, tname,
                                           dns_rdatatype_ds, &val->frdataset, NULL, dsvalidated, "proveunsecure");
                if (result != ISC_R_SUCCESS)
                    goto out;
                return (DNS_R_WAIT);
            }
            ...
            if (result == DNS_R_NXRRSET &&
                ...
                dns_view_findzonecut2 (val->view, tname, found,
                                       ...
                                       NULL, NULL) == ISC_R_SUCCESS && dns_name_equal (tname, found))
            {
                if (val->mustbesecure)
                {
                    validator_log (val, ISC_LOG_WARNING, "must be secure failure, " "no DS at zone cut");
                    return (DNS_R_MUSTBESECURE);
                }
                if (val->view->dlv == NULL || DLVTRIED (val))
                {
            ...
            if (val->frdataset.trust < dns_trust_secure)
            {
                 ...
                 */
                validator_log (val, ISC_LOG_WARNING, "can't validate existing " "negative responses (no DS)");
            ...
            if (isdelegation (tname, &val->frdataset, result))
            {
                if (val->mustbesecure)
                {
                    validator_log (val, ISC_LOG_WARNING, "must be secure failure, " "%s is a delegation", namebuf);
                    return (DNS_R_MUSTBESECURE);
                }
                if (val->view->dlv == NULL || DLVTRIED (val))
                {
        ...
        else if (result == DNS_R_CNAME)
        {
            if (DNS_TRUST_PENDING (val->frdataset.trust) || DNS_TRUST_ANSWER (val->frdataset.trust))
            {
                result = create_validator (val, tname,
                                           dns_rdatatype_cname,
                                           &val->frdataset, NULL, cnamevalidated, "proveunsecure " "(cname)");
                if (result != ISC_R_SUCCESS)
                    goto out;
                return (DNS_R_WAIT);
            }
        ...
        else if (result == ISC_R_SUCCESS)
        {
            ...
            if (val->frdataset.trust >= dns_trust_secure)
            {
                if (!check_ds (val, tname, &val->frdataset))
                {
                    ...
                    if (val->mustbesecure)
                    {
                        validator_log (val,
                                       ISC_LOG_WARNING,
                                       "must be secure failure, " "no supported algorithm/" "digest (%s/DS)", namebuf);
                        result = DNS_R_MUSTBESECURE;
                        goto out;
                    }
                    if (val->view->dlv == NULL || DLVTRIED (val))
            ...
            result = create_validator (val, tname, dns_rdatatype_ds,
                                       &val->frdataset, &val->fsigrdataset, dsvalidated, "proveunsecure");
            if (result != ISC_R_SUCCESS)
                goto out;
            return (DNS_R_WAIT);
        }
        else if (result == DNS_R_NXDOMAIN || result == DNS_R_NCACHENXDOMAIN)
        {
            /*
            ...
            else if (DNS_TRUST_PENDING (val->frdataset.trust) || DNS_TRUST_ANSWER (val->frdataset.trust))
            {
                /*
                 * If we have "trust == answer" then this namespace
                 * has switched from insecure to should be secure.
        ...
        else if (result == ISC_R_NOTFOUND)
        {
             ...
             */
            result = create_fetch (val, tname, dns_rdatatype_ds, dsfetched2, "proveunsecure");
            if (result != ISC_R_SUCCESS)
                goto out;
            return (DNS_R_WAIT);
        }
...
static void validator_start (isc_task_t * task, isc_event_t * event)
{
    ...
    if ((val->options & DNS_VALIDATOR_DLV) != 0 && val->event->rdataset != NULL)
    {
        validator_log (val, ISC_LOG_DEBUG (3), "looking for DLV");
        result = startfinddlvsep (val, dns_rootname);
    }
    else if (val->event->rdataset != NULL && val->event->sigrdataset != NULL)
    {
        isc_result_t saved_result;
         ...
         */
        validator_log (val, ISC_LOG_DEBUG (3), "attempting positive response validation");
    ...
    else if (val->event->rdataset != NULL && val->event->rdataset->type != 0)
    {
        /*
         ...
         */
        INSIST (dns_rdataset_isassociated (val->event->rdataset));
        validator_log (val, ISC_LOG_DEBUG (3), "attempting insecurity proof");

        val->attributes |= VALATTR_INSECURITY;
        result = proveunsecure (val, ISC_FALSE, ISC_FALSE);
        if (result == DNS_R_NOTINSECURE)
            validator_log (val, ISC_LOG_INFO, "got insecure response; " "parent indicates it should be secure");
    }
    else if (val->event->rdataset == NULL && val->event->sigrdataset == NULL)
    {
        /*
         ...
         */
        validator_log (val, ISC_LOG_DEBUG (3), "attempting negative response validation");
    ...
    else if (val->event->rdataset != NULL && NEGATIVE (val->event->rdataset))
    {
        /*
         * This is a nonexistence validation.
         */
...
dns_validator_create (dns_view_t * view, dns_name_t * name, dns_rdatatype_t type,
                      ...
                      isc_task_t * task, isc_taskaction_t action, void *arg, dns_validator_t ** validatorp)
{
    isc_result_t result = ISC_R_FAILURE;
    ...
    REQUIRE (name != NULL);
    REQUIRE (rdataset != NULL || (rdataset == NULL && sigrdataset == NULL && message != NULL));
    ...
    event = (dns_validatorevent_t *)
        isc_event_allocate (view->mctx, task,
                            DNS_EVENT_VALIDATORSTART, validator_start, NULL, sizeof (dns_validatorevent_t));
    if (event == NULL)
    {
                ...
                isc_logmodule_t * module, int level, const char *fmt, va_list ap)
{
    ...
    if (val->event != NULL && val->event->name != NULL)
    {
        ...
        dns_name_format (val->event->name, namebuf, sizeof (namebuf));
        dns_rdatatype_format (val->event->type, typebuf, sizeof (typebuf));
        isc_log_write (dns_lctx, category, module, level,
                       "%.*svalidating @%p: %s %s: %s", depth, spaces, val, namebuf, typebuf, msgbuf);
    }
    else
    {
        isc_log_write (dns_lctx, category, module, level, "%.*svalidator @%p: %s", depth, spaces, val, msgbuf);
    }
}

static void validator_log (dns_validator_t * val, int level, const char *fmt, ...)
{
    va_list ap;
    ...
    validator_logv (val, DNS_LOGCATEGORY_DNSSEC, DNS_LOGMODULE_VALIDATOR, level, fmt, ap);
    va_end (ap);
}
                     ...
                     dns_name_t * name, dns_rdatatype_t type, const char *caller, const char *operation)
{
    ...
    validator_log (val, ISC_LOG_DEBUG (9), "%s: creating %s for %s %s", caller, operation, namestr, typestr);
}
