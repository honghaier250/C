...
xfrin_create (isc_mem_t * mctx,
              ...
              dns_rdatatype_t reqtype,
              isc_sockaddr_t * masteraddr,
              isc_sockaddr_t * sourceaddr, dns_tsigkey_t * tsigkey, dns_xfrin_ctx_t ** xfrp);
...
xfrin_logv (int level, const char *zonetext, isc_sockaddr_t * masteraddr,
     ...
     static void xfrin_log (dns_xfrin_ctx_t * xfr, int level, const char *fmt, ...) ISC_FORMAT_PRINTF (3, 4);
     ...
     static isc_result_t axfr_init (dns_xfrin_ctx_t * xfr)
{
    ...
    CHECK (dns_db_beginload (xfr->db, &xfr->axfr.add_func, &xfr->axfr.add_private));
...
static isc_result_t axfr_makedb (dns_xfrin_ctx_t * xfr, dns_db_t ** dbp)
{
    return (dns_db_create (xfr->mctx,    /* XXX */
                           "rbt",    /* XXX guess */
                           &xfr->name, dns_dbtype_zone, xfr->rdclass, 0, NULL,    /* XXX guess */
                           dbp));
}

static isc_result_t
axfr_putdata (dns_xfrin_ctx_t * xfr, dns_diffop_t op, dns_name_t * name, dns_ttl_t ttl, dns_rdata_t * rdata)
{
    isc_result_t result;
...
static isc_result_t axfr_apply (dns_xfrin_ctx_t * xfr)
{
    ...
    CHECK (dns_diff_load (&xfr->diff, xfr->axfr.add_func, xfr->axfr.add_private));
    xfr->difflen = 0;
...
static isc_result_t ixfr_init (dns_xfrin_ctx_t * xfr)
{
    ...
    {
        xfrin_log (xfr, ISC_LOG_ERROR, "got incremental response to AXFR request");
    ...
    if (journalfile != NULL)
        CHECK (dns_journal_open (xfr->mctx, journalfile, ISC_TRUE, &xfr->ixfr.journal));
...
static isc_result_t
ixfr_putdata (dns_xfrin_ctx_t * xfr, dns_diffop_t op, dns_name_t * name, dns_ttl_t ttl, dns_rdata_t * rdata)
{
    isc_result_t result;
...
static isc_result_t xfr_rr (dns_xfrin_ctx_t * xfr, dns_name_t * name, isc_uint32_t ttl, dns_rdata_t * rdata)
{
    ...
    if (rdata->type == dns_rdatatype_none || dns_rdatatype_ismeta (rdata->type))
        FAIL (DNS_R_FORMERR);
  ...
  redo:
    switch (xfr->state)
    {
        case XFRST_SOAQUERY:
            ...
            {
                xfrin_log (xfr, ISC_LOG_ERROR, "non-SOA response to SOA query");
                FAIL (DNS_R_FORMERR);
            }
            xfr->end_serial = dns_soa_getserial (rdata);
            if (!DNS_SERIAL_GT (xfr->end_serial, xfr->ixfr.request_serial) && !dns_zone_isforced (xfr->zone))
            {
                xfrin_log (xfr, ISC_LOG_DEBUG (3),
                           "requested serial %u, "
                           "master has %u, not updating", xfr->ixfr.request_serial, xfr->end_serial);
                FAIL (DNS_R_UPTODATE);
            }
        ...
        case XFRST_INITIALSOA:
            if (rdata->type != dns_rdatatype_soa)
            {
                xfrin_log (xfr, ISC_LOG_ERROR, "first RR in zone transfer must be SOA");
                FAIL (DNS_R_FORMERR);
            }
            /*
            ...
            xfr->end_serial = dns_soa_getserial (rdata);
            if (xfr->reqtype == dns_rdatatype_ixfr &&
                !DNS_SERIAL_GT (xfr->end_serial, xfr->ixfr.request_serial) && !dns_zone_isforced (xfr->zone))
            {
                /*
        ...
        case XFRST_FIRSTDATA:
             ...
             */
            if (xfr->reqtype == dns_rdatatype_ixfr &&
                rdata->type == dns_rdatatype_soa && xfr->ixfr.request_serial == dns_soa_getserial (rdata))
            {
                xfrin_log (xfr, ISC_LOG_DEBUG (3), "got incremental response");
                CHECK (ixfr_init (xfr));
                xfr->state = XFRST_IXFR_DELSOA;
            }
            else
            {
                xfrin_log (xfr, ISC_LOG_DEBUG (3), "got nonincremental response");
                CHECK (axfr_init (xfr));
        ...
        case XFRST_IXFR_ADD:
            if (rdata->type == dns_rdatatype_soa)
            {
                ...
                else if (soa_serial != xfr->ixfr.current_serial)
                {
                    xfrin_log (xfr, ISC_LOG_ERROR,
                               "IXFR out of sync: " "expected serial %u, got %u", xfr->ixfr.current_serial, soa_serial);
                    FAIL (DNS_R_FORMERR);
                }
                else
                {
            ...
            if (rdata->type == dns_rdatatype_ns && dns_name_iswildcard (name))
        ...
        case XFRST_AXFR:
             ...
             */
            if (rdata->type == dns_rdatatype_a && rdata->rdclass != xfr->rdclass && xfr->rdclass != dns_rdataclass_in)
                break;
...
dns_xfrin_create (dns_zone_t * zone, dns_rdatatype_t xfrtype,
                  ...
                  isc_socketmgr_t * socketmgr, isc_task_t * task, dns_xfrindone_t done, dns_xfrin_ctx_t ** xfrp)
{
    isc_sockaddr_t sourceaddr;
    ...
    return (dns_xfrin_create2 (zone, xfrtype, masteraddr, &sourceaddr,
                               tsigkey, mctx, timermgr, socketmgr, task, done, xfrp));
}
                   ...
                   isc_task_t * task, dns_xfrindone_t done, dns_xfrin_ctx_t ** xfrp)
{
    ...
    CHECK (xfrin_create (mctx, zone, db, task, timermgr, socketmgr, zonename,
                         dns_zone_getclass (zone), xfrtype, masteraddr, sourceaddr, tsigkey, &xfr));
  ...
  failure:
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        xfrin_log1 (ISC_LOG_ERROR, zonetext, masteraddr, "zone transfer setup failed");
    }
    return (result);
}
...
static void xfrin_cancelio (dns_xfrin_ctx_t * xfr)
{
    if (xfr->connects > 0)
    {
        isc_socket_cancel (xfr->socket, xfr->task, ISC_SOCKCANCEL_CONNECT);
    }
    else if (xfr->recvs > 0)
    {
...
static void xfrin_fail (dns_xfrin_ctx_t * xfr, isc_result_t result, const char *msg)
{
    ...
    {
        xfrin_log (xfr, ISC_LOG_ERROR, "%s: %s", msg, isc_result_totext (result));
        if (xfr->is_ixfr)
              ...
              isc_sockaddr_t * sourceaddr, dns_tsigkey_t * tsigkey, dns_xfrin_ctx_t ** xfrp)
{
    ...
    CHECK (isc_timer_create (timermgr, isc_timertype_inactive, NULL, NULL, task, xfrin_timeout, xfr, &xfr->timer));
    CHECK (dns_timer_setidle (xfr->timer, dns_zone_getmaxxfrin (xfr->zone), dns_zone_getidlein (xfr->zone), ISC_FALSE));
...
static isc_result_t xfrin_start (dns_xfrin_ctx_t * xfr)
{
    ...
    CHECK (isc_socket_create (xfr->socketmgr, isc_sockaddr_pf (&xfr->sourceaddr), isc_sockettype_tcp, &xfr->socket));
    isc_socket_setname (xfr->socket, "xfrin", NULL);
#ifndef BROKEN_TCP_BIND_BEFORE_CONNECT
    CHECK (isc_socket_bind (xfr->socket, &xfr->sourceaddr, ISC_SOCKET_REUSEADDRESS));
#endif
    CHECK (isc_socket_connect (xfr->socket, &xfr->masteraddr, xfr->task, xfrin_connect_done, xfr));
    xfr->connects++;
    return (ISC_R_SUCCESS);
  failure:
...
static void xfrin_connect_done (isc_task_t * task, isc_event_t * event)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        {
            TIME_NOW (&now);
            dns_zonemgr_unreachableadd (zmgr, &xfr->masteraddr, &xfr->sourceaddr, &now);
        }
        goto failure;
    }
...
static isc_result_t xfrin_send_request (dns_xfrin_ctx_t * xfr)
{
    ...
    if (xfr->reqtype == dns_rdatatype_ixfr)
    {
        ...
        CHECK (dns_db_createsoatuple (xfr->db, ver, xfr->mctx, DNS_DIFFOP_EXISTS, &soatuple));
        ...
        CHECK (dns_db_getsoaserial (xfr->db, NULL, &xfr->ixfr.request_serial));
    ...
    lregion.base = length;
    lregion.length = 2;
    CHECK (isc_socket_send (xfr->socket, &lregion, xfr->task, xfrin_sendlen_done, xfr));
    xfr->sends++;
...
static void xfrin_sendlen_done (isc_task_t * task, isc_event_t * event)
{
    ...
    isc_buffer_usedregion (&xfr->qbuffer, &region);
    CHECK (isc_socket_send (xfr->socket, &region, xfr->task, xfrin_send_done, xfr));
    xfr->sends++;
  failure:
    if (result != ISC_R_SUCCESS)
...
static void xfrin_send_done (isc_task_t * task, isc_event_t * event)
{
    ...
    CHECK (dns_tcpmsg_readmessage (&xfr->tcpmsg, xfr->task, xfrin_recv_done, xfr));
    xfr->recvs++;
  failure:
    isc_event_free (&event);
    if (result != ISC_R_SUCCESS)
...
static void xfrin_recv_done (isc_task_t * task, isc_event_t * ev)
{
    ...
    xfrin_log (xfr, ISC_LOG_DEBUG (7), "received %u bytes", tcpmsg->buffer.used);
    ...
    result = dns_message_parse (msg, &tcpmsg->buffer, DNS_MESSAGEPARSE_PRESERVEORDER);

    if (result != ISC_R_SUCCESS || msg->rcode != dns_rcode_noerror || (xfr->checkid && msg->id != xfr->id))
    {
        if (result == ISC_R_SUCCESS)
            result = ISC_RESULTCLASS_DNSRCODE + msg->rcode;
        /*XXX*/ if (result == ISC_R_SUCCESS || result == DNS_R_NOERROR)
            result = DNS_R_UNEXPECTEDID;
        if (xfr->reqtype == dns_rdatatype_axfr || xfr->reqtype == dns_rdatatype_soa)
            goto failure;
        xfrin_log (xfr, ISC_LOG_DEBUG (3), "got %s, retrying with AXFR", isc_result_totext (result));
      try_axfr:
        dns_message_destroy (&msg);
        xfrin_reset (xfr);
     ...
     */
    if (xfr->reqtype == dns_rdatatype_ixfr && xfr->state == XFRST_INITIALSOA && msg->counts[DNS_SECTION_ANSWER] == 0)
    {
        xfrin_log (xfr, ISC_LOG_DEBUG (3), "empty answer section, retrying with AXFR");
        goto try_axfr;
    }

    if (xfr->reqtype == dns_rdatatype_soa && (msg->flags & DNS_MESSAGEFLAG_AA) == 0)
    {
    ...
    if (result != ISC_R_SUCCESS)
    {
        xfrin_log (xfr, ISC_LOG_DEBUG (3), "TSIG check failed: %s", isc_result_totext (result));
        goto failure;
    }

    for (result = dns_message_firstname (msg, DNS_SECTION_ANSWER);
         result == ISC_R_SUCCESS; result = dns_message_nextname (msg, DNS_SECTION_ANSWER))
    {
        dns_rdataset_t *rds;
    ...
    {
        xfr->sincetsig++;
        if (xfr->sincetsig > 100 || xfr->nmsg == 0 || xfr->state == XFRST_AXFR_END || xfr->state == XFRST_IXFR_END)
        {
...
static void maybe_free (dns_xfrin_ctx_t * xfr)
{
    ...
    xfrin_log (xfr, ISC_LOG_INFO,
               ...
               (unsigned int) (msecs / 1000), (unsigned int) (msecs % 1000), (unsigned int) persec);
 ...
 */
static void xfrin_logv (int level, const char *zonetext, isc_sockaddr_t * masteraddr, const char *fmt, va_list ap)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_XFER_IN,
                   DNS_LOGMODULE_XFER_IN, level, "transfer of '%s' from %s: %s", zonetext, mastertext, msgtext);
}
...
static void xfrin_log1 (int level, const char *zonetext, isc_sockaddr_t * masteraddr, const char *fmt, ...)
{
    va_list ap;
