...
static void set_bit (unsigned char *array, unsigned int index, unsigned int bit)
{
...
static unsigned int bit_isset (unsigned char *array, unsigned int index)
{
...
dns_nsec3_buildrdata (dns_db_t * db, dns_dbversion_t * version,
                      ...
                      const unsigned char *nexthash, size_t hash_length, unsigned char *buffer, dns_rdata_t * rdata)
{
    isc_result_t result;
    ...
    *p++ = hash_length;
    ...
    for (result = dns_rdatasetiter_first (rdsiter); result == ISC_R_SUCCESS; result = dns_rdatasetiter_next (rdsiter))
    {
            ...
            rdataset.type != dns_rdatatype_nsec3 && rdataset.type != dns_rdatatype_rrsig)
        {
             ...
             */
            if (rdataset.type == dns_rdatatype_soa || rdataset.type == dns_rdatatype_ds)
                need_rrsig = ISC_TRUE;
     ...
     */
    if (bit_isset (bm, dns_rdatatype_ns) && !bit_isset (bm, dns_rdatatype_soa))
    {
        for (i = 0; i <= max_type; i++)
        {
            if (bit_isset (bm, i) && !dns_rdatatype_iszonecutauth ((dns_rdatatype_t) i))
                set_bit (bm, i, 0);
        }
    }
...
isc_boolean_t dns_nsec3_typepresent (dns_rdata_t * rdata, dns_rdatatype_t type)
{
    ...
    for (i = 0; i < nsec3.len; i += len)
    {
        ...
        if (type < (window * 256) + len * 8)
            present = ISC_TF (bit_isset (&nsec3.typebits[i], type % 256));
        break;
    }
...
dns_nsec3_hashname (dns_fixedname_t * result,
                    ...
                    dns_hash_t hashalg, unsigned int iterations, const unsigned char *salt, size_t saltlength)
{
    ...
    /* hash the node name */
    len = isc_iterated_hash (rethash, hashalg, iterations, salt, saltlength, downcased->ndata, downcased->length);
    if (len == 0U)
        return (DNS_R_BADALG);
    ...
    return (dns_name_fromtext (dns_fixedname_name (result), &namebuffer, origin, 0, NULL));
}
...
static isc_result_t name_exists (dns_db_t * db, dns_dbversion_t * version, dns_name_t * name, isc_boolean_t * exists)
{
    ...
    result = dns_db_allrdatasets (db, node, version, (isc_stdtime_t) 0, &iter);
    if (result != ISC_R_SUCCESS)
        goto cleanup_node;
...
static isc_boolean_t match_nsec3param (const dns_rdata_nsec3_t * nsec3, const dns_rdata_nsec3param_t * nsec3param)
{
    if (nsec3->hash == nsec3param->hash &&
        ...
        nsec3->salt_length == nsec3param->salt_length && !memcmp (nsec3->salt, nsec3param->salt, nsec3->salt_length))
        return (ISC_TRUE);
    return (ISC_FALSE);
}
        ...
        const dns_rdata_nsec3param_t * nsec3param, dns_diff_t * diff)
{
    ...
    dns_rdataset_init (&rdataset);
    result = dns_db_findrdataset (db, node, version, dns_rdatatype_nsec3, 0, (isc_stdtime_t) 0, &rdataset, NULL);
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        result = dns_difftuple_create (diff->mctx, DNS_DIFFOP_DEL, name, rdataset.ttl, &rdata, &tuple);
        if (result != ISC_R_SUCCESS)
            goto failure;
...
static isc_boolean_t better_param (dns_rdataset_t * nsec3paramset, dns_rdata_t * param)
{
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        if (rdataset.type != dns_rdatatype_nsec3param)
        {
            ...
            if (!dns_nsec3param_fromprivate (&tmprdata, &rdata, buf, sizeof (buf)))
                continue;
        }
        else
            dns_rdataset_current (&rdataset, &rdata);
        ...
        if (rdata.data[0] != param->data[0] ||
            ...
            rdata.data[4] != param->data[4] || memcmp (&rdata.data[5], &param->data[5], param->data[4]))
            continue;
...
static isc_result_t
find_nsec3 (dns_rdata_nsec3_t * nsec3, dns_rdataset_t * rdataset, const dns_rdata_nsec3param_t * nsec3param)
{
    isc_result_t result;
                    ...
                    dns_ttl_t nsecttl, isc_boolean_t unsecure, dns_diff_t * diff)
{
    ...
    CHECK (dns_nsec3_hashname (&fixed, nexthash, &next_length, name, origin, hash, iterations, salt, salt_length));
    ...
    result = dns_db_findrdataset (db, newnode, version, dns_rdatatype_nsec3, 0, (isc_stdtime_t) 0, &rdataset, NULL);
    /*
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        if (result == ISC_R_SUCCESS)
        {
            ...
            {
                result = dns_nsec3_delnsec3 (db, version, name, nsec3param, diff);
                goto failure;
            }
            else
    ...
    do
    {
        ...
        CHECK (dns_rdata_fromstruct (&rdata, rdataset.rdclass, dns_rdatatype_nsec3, &nsec3, &buffer));
        CHECK (dns_difftuple_create (diff->mctx, DNS_DIFFOP_ADD, prev, rdataset.ttl, &rdata, &tuple));
  ...
  addnsec3:
    ...
    CHECK (dns_nsec3_buildrdata (db, version, node, hash, flags, iterations,
                                 salt, salt_length, nexthash, next_length, nsec3buf, &rdata));
    dns_db_detachnode (db, &node);
     ...
     */
    CHECK (dns_difftuple_create (diff->mctx, DNS_DIFFOP_ADD, hashname, nsecttl, &rdata, &tuple));
    CHECK (do_one_tuple (&tuple, db, version, diff));
    INSIST (tuple == NULL);
    dns_rdata_reset (&rdata);
    ...
    do
    {
        ...
        CHECK (dns_nsec3_hashname (&fixed, nexthash, &next_length,
                                   &empty, origin, hash, iterations, salt, salt_length));
        ...
        do
        {
            ...
            result = dns_db_findrdataset (db, node, version,
                                          dns_rdatatype_nsec3, 0, (isc_stdtime_t) 0, &rdataset, NULL);
            dns_db_detachnode (db, &node);
            if (result != ISC_R_SUCCESS)
                continue;
            result = find_nsec3 (&nsec3, &rdataset, nsec3param);
        ...
        CHECK (dns_nsec3_buildrdata (db, version, NULL, hash, flags,
                                     iterations, salt, salt_length, nexthash, next_length, nsec3buf, &rdata));
        /*
         * Delete the old NSEC3 and record the change.
         */
        CHECK (delete (db, version, hashname, nsec3param, diff));
...
dns_nsec3_addnsec3s (dns_db_t * db, dns_dbversion_t * version,
                     dns_name_t * name, dns_ttl_t nsecttl, isc_boolean_t unsecure, dns_diff_t * diff)
{
    dns_dbnode_t *node = NULL;
    ...
    result = dns_db_findrdataset (db, node, version, dns_rdatatype_nsec3param, 0, 0, &rdataset, NULL);
    dns_db_detachnode (db, &node);
    if (result == ISC_R_NOTFOUND)
        return (ISC_R_SUCCESS);
    if (result != ISC_R_SUCCESS)
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
         ...
         */
        CHECK (dns_nsec3_addnsec3 (db, version, name, &nsec3param, nsecttl, unsecure, diff));
    }
    if (result == ISC_R_NOMORE)
        result = ISC_R_SUCCESS;
...
isc_boolean_t dns_nsec3param_fromprivate (dns_rdata_t * src, dns_rdata_t * target, unsigned char *buf, size_t buflen)
{
    ...
    result = dns_rdata_fromwire (target, src->rdclass, dns_rdatatype_nsec3param, &buf1, &dctx, 0, &buf2);
...
dns_nsec3param_toprivate (dns_rdata_t * src, dns_rdata_t * target,
                          dns_rdatatype_t privatetype, unsigned char *buf, size_t buflen)
{
...
#ifdef BIND9
isc_result_t dns_nsec3param_deletechains (dns_db_t * db, dns_dbversion_t * ver, dns_zone_t * zone, dns_diff_t * diff)
{
    dns_dbnode_t *node = NULL;
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        CHECK (dns_difftuple_create (diff->mctx, DNS_DIFFOP_DEL, origin, rdataset.ttl, &rdata, &tuple));
        CHECK (do_one_tuple (&tuple, db, ver, diff));
        INSIST (tuple == NULL);

        dns_nsec3param_toprivate (&rdata, &private, privatetype, buf, sizeof (buf));
        ...
        if (!flag)
        {
            CHECK (dns_difftuple_create (diff->mctx, DNS_DIFFOP_ADD, origin, 0, &private, &tuple));
            CHECK (do_one_tuple (&tuple, db, ver, diff));
            INSIST (tuple == NULL);
        }
        dns_rdata_reset (&rdata);
  ...
  try_private:
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        CHECK (dns_difftuple_create (diff->mctx, DNS_DIFFOP_DEL, origin, 0, &rdata, &tuple));
        CHECK (do_one_tuple (&tuple, db, ver, diff));
        INSIST (tuple == NULL);
        ...
        if (!flag)
        {
            CHECK (dns_difftuple_create (diff->mctx, DNS_DIFFOP_ADD, origin, 0, &rdata, &tuple));
            CHECK (do_one_tuple (&tuple, db, ver, diff));
            INSIST (tuple == NULL);
        }
        dns_rdata_reset (&rdata);
...
dns_nsec3_addnsec3sx (dns_db_t * db, dns_dbversion_t * version,
                      ...
                      isc_boolean_t unsecure, dns_rdatatype_t type, dns_diff_t * diff)
{
    dns_dbnode_t *node = NULL;
    ...
    result = dns_db_findrdataset (db, node, version, type, 0, 0, &prdataset, NULL);
  ...
  try_private:
     ...
     * Update each active NSEC3 chain.
     */
    for (result = dns_rdataset_first (&prdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&prdataset))
    {
        ...
        if (!dns_nsec3param_fromprivate (&rdata1, &rdata2, buf, sizeof (buf)))
            continue;
...
static isc_result_t deleteit (dns_db_t * db, dns_dbversion_t * ver, dns_name_t * name, isc_boolean_t * yesno)
{
    ...
    if (result == DNS_R_EMPTYNAME || result == ISC_R_SUCCESS || result == DNS_R_ZONECUT)
    {
        *yesno = ISC_FALSE;
        return (ISC_R_SUCCESS);
    }
    if (result == DNS_R_GLUE || result == DNS_R_DNAME || result == DNS_R_DELEGATION || result == DNS_R_NXDOMAIN)
    {
        *yesno = ISC_TRUE;
        return (ISC_R_SUCCESS);
    }
                      ...
                      dns_rdatatype_t privatetype, dns_diff_t * diff)
{
  ...
  try_private:
    if (privatetype == 0)
        goto success;
    result = dns_db_findrdataset (db, node, version, privatetype, 0, 0, &rdataset, NULL);
    if (result == ISC_R_NOTFOUND)
        goto success;
    if (result != ISC_R_SUCCESS)
        goto failure;
...
dns_nsec3_activex (dns_db_t * db, dns_dbversion_t * version,
                   isc_boolean_t complete, dns_rdatatype_t privatetype, isc_boolean_t * answer)
{
    dns_dbnode_t *node = NULL;
...
isc_result_t
dns_nsec3_maxiterations (dns_db_t * db, dns_dbversion_t * version, isc_mem_t * mctx, unsigned int *iterationsp)
{
    dns_dbnode_t *node = NULL;
    ...
    dns_rdataset_init (&rdataset);
    result = dns_db_findrdataset (db, node, version, dns_rdatatype_dnskey, 0, 0, &rdataset, NULL);
    dns_db_detachnode (db, &node);
    if (result == ISC_R_NOTFOUND)
    {
        *iterationsp = 0;
    ...
    for (result = dns_rdataset_first (&rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (&rdataset))
    {
        ...
        CHECK (dst_key_fromdns (dns_db_origin (db), rdataset.rdclass, &buffer, mctx, &key));
