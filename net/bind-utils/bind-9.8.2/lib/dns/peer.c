...
void dns_peerlist_addpeer (dns_peerlist_t * peers, dns_peer_t * peer)
{
     ...
     */
    for (p = ISC_LIST_HEAD (peers->elements); p != NULL; p = ISC_LIST_NEXT (p, next))
...
isc_result_t dns_peerlist_peerbyaddr (dns_peerlist_t * servers, isc_netaddr_t * addr, dns_peer_t ** retval)
{
    ...
    while (server != NULL)
    {
        if (isc_netaddr_eqprefix (addr, &server->address, server->prefixlen))
            break;
...
static void peer_delete (dns_peer_t ** peer)
{
    ...
    {
        isc_mem_put (mem, p->transfer_source, sizeof (*p->transfer_source));
    }
...
isc_result_t dns_peer_settransfersource (dns_peer_t * peer, const isc_sockaddr_t * transfer_source)
{
    ...
    {
        isc_mem_put (peer->mem, peer->transfer_source, sizeof (*peer->transfer_source));
        peer->transfer_source = NULL;
    }
    if (transfer_source != NULL)
    {
        peer->transfer_source = isc_mem_get (peer->mem, sizeof (*peer->transfer_source));
        if (peer->transfer_source == NULL)
            return (ISC_R_NOMEMORY);
...
isc_result_t dns_peer_setnotifysource (dns_peer_t * peer, const isc_sockaddr_t * notify_source)
{
    ...
    {
        isc_mem_put (peer->mem, peer->notify_source, sizeof (*peer->notify_source));
        peer->notify_source = NULL;
    }
    if (notify_source != NULL)
    {
        peer->notify_source = isc_mem_get (peer->mem, sizeof (*peer->notify_source));
        if (peer->notify_source == NULL)
            return (ISC_R_NOMEMORY);
...
isc_result_t dns_peer_setquerysource (dns_peer_t * peer, const isc_sockaddr_t * query_source)
{
    ...
    {
        isc_mem_put (peer->mem, peer->query_source, sizeof (*peer->query_source));
        peer->query_source = NULL;
    }
    if (query_source != NULL)
    {
        peer->query_source = isc_mem_get (peer->mem, sizeof (*peer->query_source));
        if (peer->query_source == NULL)
            return (ISC_R_NOMEMORY);
