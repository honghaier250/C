...
static void tsig_log (dns_tsigkey_t * key, int level, const char *fmt, ...) ISC_FORMAT_PRINTF (3, 4);
     ...
     static void tsig_log (dns_tsigkey_t * key, int level, const char *fmt, ...)
{
    ...
    if (key != NULL && key->generated)
        isc_log_write (dns_lctx,
                       ...
                       level, "tsig key '%s' (%s): %s", namestr, creatorstr, message);
    else
        isc_log_write (dns_lctx,
                       DNS_LOGCATEGORY_DNSSEC, DNS_LOGMODULE_TSIG, level, "tsig key '%s': %s", namestr, message);
}
...
dns_tsigkey_createfromkey (dns_name_t * name, dns_name_t * algorithm,
                           ...
                           isc_stdtime_t expire, isc_mem_t * mctx, dns_tsig_keyring_t * ring, dns_tsigkey_t ** key)
{
    dns_tsigkey_t *tkey;
    ...
    else if (dns_name_equal (algorithm, DNS_TSIG_HMACSHA224_NAME))
    {
        ...
        if (dstkey != NULL && dst_key_alg (dstkey) != DST_ALG_HMACSHA224)
        {
            ret = DNS_R_BADALG;
            goto cleanup_name;
        }
    ...
    else if (dns_name_equal (algorithm, DNS_TSIG_HMACSHA256_NAME))
    {
        ...
        if (dstkey != NULL && dst_key_alg (dstkey) != DST_ALG_HMACSHA256)
        {
            ret = DNS_R_BADALG;
            goto cleanup_name;
        }
    ...
    else if (dns_name_equal (algorithm, DNS_TSIG_HMACSHA384_NAME))
    {
        ...
        if (dstkey != NULL && dst_key_alg (dstkey) != DST_ALG_HMACSHA384)
        {
            ret = DNS_R_BADALG;
            goto cleanup_name;
        }
    ...
    else if (dns_name_equal (algorithm, DNS_TSIG_HMACSHA512_NAME))
    {
        ...
        if (dstkey != NULL && dst_key_alg (dstkey) != DST_ALG_HMACSHA512)
        {
            ret = DNS_R_BADALG;
            goto cleanup_name;
        }
    ...
    else
    {
        ...
        (void) dns_name_downcase (tkey->algorithm, tkey->algorithm, NULL);
    }
    ...
    if (dstkey != NULL && dst_key_size (dstkey) < 64 &&
        !dns_name_equal (algorithm, DNS_TSIG_GSSAPI_NAME) && !dns_name_equal (algorithm, DNS_TSIG_GSSAPIMS_NAME))
    {
        char namestr[DNS_NAME_FORMATSIZE];

        dns_name_format (name, namestr, sizeof (namestr));
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DNSSEC,
                       DNS_LOGMODULE_TSIG, ISC_LOG_INFO, "the key '%s' is too short to be secure", namestr);
    }
...
static void cleanup_ring (dns_tsig_keyring_t * ring)
{
  ...
  again:
    ...
    for (;;)
    {
        ...
        {
            if (tkey->generated
                && isc_refcount_current (&tkey->refs) == 1 && tkey->inception != tkey->expire && tkey->expire < now)
            {
...
static isc_result_t restore_key (dns_tsig_keyring_t * ring, isc_stdtime_t now, FILE * fp)
{
    ...
    result = dst_key_restore (name, dstalg, DNS_KEYOWNER_ENTITY,
                              DNS_KEYPROTO_DNSSEC, dns_rdataclass_in, ring->mctx, keystr, &dstkey);
    if (result != ISC_R_SUCCESS)
        return (result);

    result = dns_tsigkey_createfromkey (name, algorithm, dstkey,
                                        ISC_TRUE, creator, inception, expire, ring->mctx, ring, NULL);
    if (dstkey != NULL)
        dst_key_free (&dstkey);
    return (result);
}
...
static void dump_key (dns_tsigkey_t * tkey, FILE * fp)
{
    ...
    if (result == ISC_R_SUCCESS)
        fprintf (fp, "%s %s %u %u %s %.*s\n", namestr, creatorstr,
                 tkey->inception, tkey->expire, algorithmstr, length, buffer);
    if (buffer != NULL)
                    ...
                    isc_stdtime_t expire, isc_mem_t * mctx, dns_tsig_keyring_t * ring, dns_tsigkey_t ** key)
{
    ...
    if (dns_name_equal (algorithm, DNS_TSIG_HMACMD5_NAME))
    {
        if (secret != NULL)
        {
            ...
            result = dst_key_frombuffer (name, DST_ALG_HMACMD5,
                                         DNS_KEYOWNER_ENTITY,
                                         DNS_KEYPROTO_DNSSEC, dns_rdataclass_in, &b, mctx, &dstkey);
            if (result != ISC_R_SUCCESS)
                return (result);
        }
    }
    ...
    result = dns_tsigkey_createfromkey (name, algorithm, dstkey,
                                        generated, creator, inception, expire, mctx, ring, key);
    if (dstkey != NULL)
        dst_key_free (&dstkey);
    return (result);
}
...
isc_result_t dns_tsig_sign (dns_message_t * msg)
{
    ...
    if (key->key != NULL && tsig.error != dns_tsigerror_badsig)
    {
        ...
        if (response)
        {
            ...
            ret = dns_rdata_tostruct (&querytsigrdata, &querytsig, NULL);
            if (ret != ISC_R_SUCCESS)
                goto cleanup_context;
            isc_buffer_putuint16 (&databuf, querytsig.siglen);
            if (isc_buffer_availablelength (&databuf) < querytsig.siglen)
            {
                ret = ISC_R_NOSPACE;
                goto cleanup_context;
            }
            isc_buffer_putmem (&databuf, querytsig.signature, querytsig.siglen);
    ...
    if (ret != ISC_R_SUCCESS)
        goto cleanup_rdata;
    ret = dns_rdata_fromstruct (rdata, dns_rdataclass_any, dns_rdatatype_tsig, &tsig, dynbuf);
    if (ret != ISC_R_SUCCESS)
        goto cleanup_dynbuf;
...
isc_result_t
dns_tsig_verify (isc_buffer_t * source, dns_message_t * msg, dns_tsig_keyring_t * ring1, dns_tsig_keyring_t * ring2)
{
    dns_rdata_any_tsig_t tsig, querytsig;
     ...
     */
    if (response &&
        (!dns_name_equal (keyname, &tsigkey->name) || !dns_name_equal (&tsig.algorithm, &querytsig.algorithm)))
    {
        msg->tsigstatus = dns_tsigerror_badkey;
        tsig_log (msg->tsigkey, 2, "key name and algorithm do not match");
    ...
    if (tsigkey == NULL)
    {
        ret = ISC_R_NOTFOUND;
        if (ring1 != NULL)
            ret = dns_tsigkey_find (&tsigkey, keyname, &tsig.algorithm, ring1);
        if (ret == ISC_R_NOTFOUND && ring2 != NULL)
            ret = dns_tsigkey_find (&tsigkey, keyname, &tsig.algorithm, ring2);
        if (ret != ISC_R_SUCCESS)
        {
            msg->tsigstatus = dns_tsigerror_badkey;
            ret = dns_tsigkey_create (keyname, &tsig.algorithm,
                                      NULL, 0, ISC_FALSE, NULL, now, now, mctx, NULL, &msg->tsigkey);
            if (ret != ISC_R_SUCCESS)
                return (ret);
        ...
        alg == DST_ALG_HMACSHA384 || alg == DST_ALG_HMACSHA512)
    {
        ...
        if (tsig.siglen > 0 && (tsig.siglen < 10 || tsig.siglen < ((siglen + 1) / 2)))
        {
            tsig_log (msg->tsigkey, 2, "signature length below minimum");
            return (DNS_R_FORMERR);
        }
        if (tsig.siglen > 0 && digestbits != 0 && tsig.siglen < ((digestbits + 1) / 8))
        {
            msg->tsigstatus = dns_tsigerror_badtrunc;
            tsig_log (msg->tsigkey, 2, "truncated signature length too small");
            return (DNS_R_TSIGVERIFYFAILURE);
        }
    ...
    if (tsig.siglen > 0)
    {
        ...
        if (ret == DST_R_VERIFYFAILURE)
        {
            ...
            tsig_log (msg->tsigkey, 2, "signature failed to verify(1)");
            goto cleanup_context;
        }
    ...
    else if (tsig.error != dns_tsigerror_badsig && tsig.error != dns_tsigerror_badkey)
    {
        msg->tsigstatus = dns_tsigerror_badsig;
...
static isc_result_t tsig_verify_tcp (isc_buffer_t * source, dns_message_t * msg)
{
    ...
    if (msg->tsig != NULL)
    {
         ...
         * Do the key name and algorithm match that of the query?
         */
        if (!dns_name_equal (keyname, &tsigkey->name) || !dns_name_equal (&tsig.algorithm, &querytsig.algorithm))
        {
            msg->tsigstatus = dns_tsigerror_badkey;
            ret = DNS_R_TSIGVERIFYFAILURE;
            tsig_log (msg->tsigkey, 2, "key name and algorithm do not match");
    ...
    if (has_tsig)
    {
        ...
        {
            msg->tsigstatus = dns_tsigerror_badsig;
            tsig_log (msg->tsigkey, 2, "signature failed to verify(2)");
            ret = DNS_R_TSIGVERIFYFAILURE;
            goto cleanup_context;
        }
        else if (ret != ISC_R_SUCCESS)
...
isc_result_t
dns_tsigkey_find (dns_tsigkey_t ** tsigkey, dns_name_t * name, dns_name_t * algorithm, dns_tsig_keyring_t * ring)
{
    dns_tsigkey_t *key;
     ...
     */
    if (key->inception != key->expire && isc_serial_lt (key->inception, now))
    {
        RWUNLOCK (&ring->lock, isc_rwlocktype_read);
        adjust_lru (key);
        return (ISC_R_NOTFOUND);
