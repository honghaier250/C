...
isc_result_t dns_byaddr_createptrname (isc_netaddr_t * address, isc_boolean_t nibble, dns_name_t * name)
{
    ...
    return (dns_byaddr_createptrname2 (address, DNS_BYADDROPT_IPV6INT, name));
}
...
static inline isc_result_t copy_ptr_targets (dns_byaddr_t * byaddr, dns_rdataset_t * rdataset)
{
    isc_result_t result;
...
static void bevent_destroy (isc_event_t * event)
{
    ...
    for (name = ISC_LIST_HEAD (bevent->names); name != NULL; name = next_name)
    {
...
dns_byaddr_create (isc_mem_t * mctx, isc_netaddr_t * address, dns_view_t * view,
                   unsigned int options, isc_task_t * task, isc_taskaction_t action, void *arg, dns_byaddr_t ** byaddrp)
{
    isc_result_t result;
    ...
    ISC_EVENT_INIT (byaddr->event, sizeof (*byaddr->event), 0, NULL,
                    DNS_EVENT_BYADDRDONE, action, arg, byaddr, bevent_destroy, mctx);
    ...
    result = dns_byaddr_createptrname2 (address, options, dns_fixedname_name (&byaddr->name));
    if (result != ISC_R_SUCCESS)
        goto cleanup_lock;

    byaddr->lookup = NULL;
    result = dns_lookup_create (mctx, dns_fixedname_name (&byaddr->name),
                                dns_rdatatype_ptr, view, 0, task, lookup_done, byaddr, &byaddr->lookup);
    if (result != ISC_R_SUCCESS)
        goto cleanup_lock;
