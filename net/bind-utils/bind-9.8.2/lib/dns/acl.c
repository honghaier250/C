...
static isc_boolean_t dns_acl_isanyornone (dns_acl_t * acl, isc_boolean_t pos)
{
    ...
    if (acl == NULL ||
        acl->iptable == NULL ||
        acl->iptable->radix == NULL || acl->iptable->radix->head == NULL || acl->iptable->radix->head->prefix == NULL)
        return (ISC_FALSE);
    ...
    if (acl->iptable->radix->head->prefix->bitlen == 0 &&
        ...
        acl->iptable->radix->head->data[1] && *(isc_boolean_t *) (acl->iptable->radix->head->data[0]) == pos)
        return (ISC_TRUE);
...
dns_acl_match (const isc_netaddr_t * reqaddr,
               ...
               const dns_acl_t * acl, const dns_aclenv_t * env, int *match, const dns_aclelement_t ** matchelt)
{
    ...
    if (env == NULL || env->match_mapped == ISC_FALSE ||
        reqaddr->family != AF_INET6 || !IN6_IS_ADDR_V4MAPPED (&reqaddr->type.in6))
        addr = reqaddr;
    else
    {
...
isc_result_t dns_acl_merge (dns_acl_t * dest, dns_acl_t * source, isc_boolean_t pos)
{
    ...
    if (dest->length + source->length > dest->alloc)
    {
        ...
        memcpy (newmem, dest->elements, dest->length * sizeof (dns_aclelement_t));

        /* Release the memory for the old elements array */
        isc_mem_put (dest->mctx, dest->elements, dest->alloc * sizeof (dns_aclelement_t));
    ...
    for (i = 0; i < source->length; i++)
    {
        ...
        dest->elements[nelem + i].node_num = source->elements[i].node_num + dest->node_count;

        /* Duplicate nested acl. */
        if (source->elements[i].type == dns_aclelementtype_nestedacl && source->elements[i].nestedacl != NULL)
            dns_acl_attach (source->elements[i].nestedacl, &dest->elements[nelem + i].nestedacl);

        /* Duplicate key name. */
        if (source->elements[i].type == dns_aclelementtype_keyname)
        {
            dns_name_init (&dest->elements[nelem + i].keyname, NULL);
            result = dns_name_dup (&source->elements[i].keyname, dest->mctx, &dest->elements[nelem + i].keyname);
            if (result != ISC_R_SUCCESS)
                return result;
        }
        ...
        {
            dest->elements[nelem + i].negative = source->elements[i].negative;
        }
    }
...
dns_aclelement_match (const isc_netaddr_t * reqaddr,
                      const dns_name_t * reqsigner,
                      const dns_aclelement_t * e, const dns_aclenv_t * env, const dns_aclelement_t ** matchelt)
{
    ...
    result = dns_acl_match (reqaddr, reqsigner, inner, env, &indirectmatch, matchelt);
...
static void destroy (dns_acl_t * dacl)
{
    ...
    if (dacl->elements != NULL)
        isc_mem_put (dacl->mctx, dacl->elements, dacl->alloc * sizeof (dns_aclelement_t));
    if (dacl->name != NULL)
...
static void is_insecure (isc_prefix_t * prefix, void **data)
{
    ...
    {
        case AF_INET:
            if (bitlen == 32 && htonl (prefix->add.sin.s_addr) == INADDR_LOOPBACK)
                return;
            break;
        case AF_INET6:
...
isc_boolean_t dns_acl_isinsecure (const dns_acl_t * a)
{
    ...
    RUNTIME_CHECK (isc_once_do (&insecure_prefix_once, initialize_action) == ISC_R_SUCCESS);
