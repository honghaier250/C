 ...
 */
     ...
     static inline void unlink_dbentries (dns_acache_t * acache, dns_acacheentry_t * ent);

     static inline isc_result_t finddbent (dns_acache_t * acache, dns_db_t * db, dbentry_t ** dbentryp);
     ...
     static isc_result_t acache_cleaner_init (dns_acache_t * acache,
                                              isc_timermgr_t * timermgr, acache_cleaner_t * cleaner);
...
static void shutdown_entries (dns_acache_t * acache)
{
     ...
     */
    for (entry = ISC_LIST_HEAD (acache->entries); entry != NULL; entry = entry_next)
    {
        ...
        ACACHE_UNLOCK (&acache->entrylocks[entry->locknum], isc_rwlocktype_write);
...
static void shutdown_buckets (dns_acache_t * acache)
{
    ...
    for (i = 0; i < DBBUCKETS; i++)
    {
        ...
        {
            INSIST (ISC_LIST_EMPTY (dbent->originlist) && ISC_LIST_EMPTY (dbent->referlist));
...
static void destroy_entry (dns_acacheentry_t * entry)
{
    dns_acache_t *acache;
...
static void destroy (dns_acache_t * acache)
{
    ...
    isc_mem_put (acache->mctx, acache->entrylocks, sizeof (*acache->entrylocks) * DEFAULT_ACACHE_ENTRY_LOCK_COUNT);
...
static inline isc_result_t finddbent (dns_acache_t * acache, dns_db_t * db, dbentry_t ** dbentryp)
{
    ...
    bucket = isc_hash_calc ((const unsigned char *) &db, sizeof (db), ISC_TRUE) % DBBUCKETS;

    for (dbentry = ISC_LIST_HEAD (acache->dbbucket[bucket]); dbentry != NULL; dbentry = ISC_LIST_NEXT (dbentry, link))
    {
        if (dbentry->db == db)
            break;
    }
...
static inline void clear_entry (dns_acache_t * acache, dns_acacheentry_t * entry)
{
    ...
    if (entry->foundname)
    {
        ...
        for (rdataset = ISC_LIST_HEAD (entry->foundname->list); rdataset != NULL; rdataset = rdataset_next)
        {
            ...
            ISC_LIST_UNLINK (entry->foundname->list, rdataset, link);
        ...
        isc_mem_put (acache->mctx, entry->foundname, sizeof (*entry->foundname));
...
static isc_result_t acache_cleaner_init (dns_acache_t * acache, isc_timermgr_t * timermgr, acache_cleaner_t * cleaner)
{
    ...
    if (timermgr != NULL)
    {
        ...
        result = isc_task_onshutdown (acache->task, acache_cleaner_shutdown_action, acache);
        if (result != ISC_R_SUCCESS)
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__,
                              "acache cleaner: " "isc_task_onshutdown() failed: %s", dns_result_totext (result));
            goto cleanup;
        }
        ...
        result = isc_timer_create (timermgr, isc_timertype_inactive,
                                   NULL, NULL,
                                   acache->task, acache_cleaning_timer_action, cleaner, &cleaner->cleaning_timer);
        if (result != ISC_R_SUCCESS)
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_timer_create() failed: %s", dns_result_totext (result));
        ...
        cleaner->resched_event =
            isc_event_allocate (acache->mctx, cleaner,
                                ...
                                acache_incremental_cleaning_action, cleaner, sizeof (isc_event_t));
        ...
        cleaner->overmem_event =
            isc_event_allocate (acache->mctx, cleaner,
                                DNS_EVENT_ACACHEOVERMEM, acache_overmem_cleaning_action, cleaner, sizeof (isc_event_t));
...
static void begin_cleaning (acache_cleaner_t * cleaner)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   ...
                   "begin acache cleaning, mem inuse %lu", (unsigned long) isc_mem_inuse (cleaner->acache->mctx));
...
static void end_cleaning (acache_cleaner_t * cleaner, isc_event_t * event)
{
    ...
    if (isc_refcount_current (&cleaner->current_entry->references) == 1)
    {
        ...
        {
            ISC_LIST_UNLINK (acache->entries, cleaner->current_entry, link);
        }
    }
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE, DNS_LOGMODULE_ACACHE,
                   ...
                   acache->stats.overmem, acache->stats.overmem_nocreates, acache->stats.nomem);
    reset_stats (acache);
    ...
    dns_acache_setcleaninginterval (cleaner->acache, cleaner->cleaning_interval);

    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE, DNS_LOGMODULE_ACACHE,
                   ISC_LOG_DEBUG (1), "end acache cleaning, "
                   "%lu entries cleaned, mem inuse %lu",
                   cleaner->ncleaned, (unsigned long) isc_mem_inuse (cleaner->acache->mctx));

    if (cleaner->overmem)
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_ACACHE, ISC_LOG_NOTICE, "acache is still in overmem state " "after cleaning");
    }
...
static void acache_cleaning_timer_action (isc_task_t * task, isc_event_t * event)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE, DNS_LOGMODULE_ACACHE,
                   ISC_LOG_DEBUG (1), "acache cleaning timer fired, " "cleaner state = %d", cleaner->state);
...
entry_stale (acache_cleaner_t * cleaner, dns_acacheentry_t * entry, isc_stdtime32_t now32, unsigned int interval)
{
    /*
    ...
    if (cleaner->overmem)
    {
        unsigned int passed = now32 - entry->lastused;    /* <= interval */
            ...
            if (cleaner->overmem)
            {
                ...
                if (entry != NULL)
                {
                    ...
                    isc_log_write (dns_lctx,
                                   DNS_LOGCATEGORY_DATABASE,
                                   DNS_LOGMODULE_ACACHE,
                                   ISC_LOG_DEBUG (1), "acache cleaner: " "still overmem, " "reset and try again");
                    continue;
                }
            }
...
static void acache_overmem_cleaning_action (isc_task_t * task, isc_event_t * event)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE, DNS_LOGMODULE_ACACHE,
                   ...
                   "overmem = %d, state = %d", cleaner->overmem, cleaner->state);
...
static void water (void *arg, int mark)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                   ...
                   overmem ? "high" : "low", (unsigned long) isc_mem_inuse (acache->mctx));
...
isc_result_t
dns_acache_create (dns_acache_t ** acachep, isc_mem_t * mctx, isc_taskmgr_t * taskmgr, isc_timermgr_t * timermgr)
{
    int i;
    ...
    if (result != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_task_create() failed(): %s", dns_result_totext (result));
        result = ISC_R_UNEXPECTED;
        goto cleanup;
    }
    isc_task_setname (acache->task, "acachetask", acache);
    ISC_EVENT_INIT (&acache->cevent, sizeof (acache->cevent), 0, NULL,
                    DNS_EVENT_ACACHECONTROL, shutdown_task, NULL, NULL, NULL, NULL);
    ...
    acache->entrylocks = isc_mem_get (mctx, sizeof (*acache->entrylocks) * DEFAULT_ACACHE_ENTRY_LOCK_COUNT);
    ...
    for (i = 0; i < DEFAULT_ACACHE_ENTRY_LOCK_COUNT; i++)
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            while (i-- > 0)
                ACACHE_DESTROYLOCK (&acache->entrylocks[i]);
            isc_mem_put (mctx, acache->entrylocks, sizeof (*acache->entrylocks) * DEFAULT_ACACHE_ENTRY_LOCK_COUNT);
            acache->entrylocks = NULL;
            goto cleanup;
        }
    }
...
dns_acache_createentry (dns_acache_t * acache, dns_db_t * origdb,
                        void (*callback) (dns_acacheentry_t *, void **), void *cbarg, dns_acacheentry_t ** entryp)
{
...
dns_acache_getentry (dns_acacheentry_t * entry, dns_zone_t ** zonep,
                     ...
                     dns_dbnode_t ** nodep, dns_name_t * fname, dns_message_t * msg, isc_stdtime_t now)
{
    isc_result_t result = ISC_R_SUCCESS;
    ...
    else
    {
        ...
        for (erdataset = ISC_LIST_HEAD (entry->foundname->list);
             erdataset != NULL; erdataset = ISC_LIST_NEXT (erdataset, link))
        {
                     ...
                     dns_dbnode_t * node, dns_name_t * fname)
{
    ...
    if (node != NULL)
    {
        ...
        entry->foundname = isc_mem_get (acache->mctx, sizeof (*entry->foundname));
        ...
        result = dns_name_dup (fname, acache->mctx, entry->foundname);
        if (result != ISC_R_SUCCESS)
            goto fail;

        for (ardataset = ISC_LIST_HEAD (fname->list); ardataset != NULL; ardataset = ISC_LIST_NEXT (ardataset, link))
        {
            crdataset = isc_mem_get (acache->mctx, sizeof (*crdataset));
            if (crdataset == NULL)
            {
                result = ISC_R_NOMEMORY;
                goto fail;
            ...
            ISC_LIST_APPEND (entry->foundname->list, crdataset, link);
        }
    }
...
void dns_acache_cancelentry (dns_acacheentry_t * entry)
{
...
void dns_acache_setcleaninginterval (dns_acache_t * acache, unsigned int t)
{
    ...
    if (t == 0)
    {
        result = isc_timer_reset (acache->cleaner.cleaning_timer, isc_timertype_inactive, NULL, NULL, ISC_TRUE);
    }
    else
    {
        isc_interval_set (&interval, acache->cleaner.cleaning_interval, 0);
        result = isc_timer_reset (acache->cleaner.cleaning_timer, isc_timertype_ticker, NULL, &interval, ISC_FALSE);
    }
    if (result != ISC_R_SUCCESS)
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_ACACHE, ISC_LOG_WARNING,
                       "could not set acache cleaning interval: %s", isc_result_totext (result));
    else
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DATABASE,
                       DNS_LOGMODULE_ACACHE, ISC_LOG_NOTICE, "acache %p cleaning interval set to %d.", acache, t);
 ...
 */
void dns_acache_setcachesize (dns_acache_t * acache, isc_uint32_t size)
{
    ...
    if (size == 0 || hiwater == 0 || lowater == 0)
        ...
        isc_mem_setwater (acache->mctx, water, acache, hiwater, lowater);
}
