...
isc_result_t
dns_rdataslab_fromrdataset (dns_rdataset_t * rdataset, isc_mem_t * mctx, isc_region_t * region, unsigned int reservelen)
{
    struct xrdata *x;
    ...
    *rawbuf++ = (nitems & 0x00ff);
    ...
    for (i = 0; i < nalloc; i++)
    {
        ...
        *rawbuf++ = (length & 0x00ff);
        ...
        if (rdataset->type == dns_rdatatype_rrsig)
        {
            *rawbuf++ |= (x[i].rdata.flags & DNS_RDATA_OFFLINE) ? DNS_RDATASLAB_OFFLINE : 0;
        }
...
dns_rdataslab_tordataset (unsigned char *slab, unsigned int reservelen,
                          ...
                          dns_rdatatype_t covers, dns_ttl_t ttl, dns_rdataset_t * rdataset)
{
    REQUIRE (slab != NULL);
 ...
 */
static inline void
rdata_from_slab (unsigned char **current, dns_rdataclass_t rdclass, dns_rdatatype_t type, dns_rdata_t * rdata)
{
...
rdata_in_slab (unsigned char *slab, unsigned int reservelen,
               dns_rdataclass_t rdclass, dns_rdatatype_t type, dns_rdata_t * rdata)
{
...
dns_rdataslab_merge (unsigned char *oslab, unsigned char *nslab,
                     ...
                     dns_rdataclass_t rdclass, dns_rdatatype_t type, unsigned int flags, unsigned char **tslabp)
{
...
dns_rdataslab_equalx (unsigned char *slab1, unsigned char *slab2,
                      unsigned int reservelen, dns_rdataclass_t rdclass, dns_rdatatype_t type)
{
    unsigned char *current1, *current2;
