...
static void hexdump (const char *msg, const char *msg2, void *base, size_t len)
{
    unsigned char *p;
...
static inline dns_msgblock_t *msgblock_allocate (isc_mem_t *, unsigned int, unsigned int);
 ...
 * is free, return NULL.
 */
static inline dns_msgblock_t *msgblock_allocate (isc_mem_t * mctx, unsigned int sizeof_type, unsigned int count)
{
 ...
 * NULL.
 */
static inline void *msgblock_internalget (dns_msgblock_t * block, unsigned int sizeof_type)
{
    void *ptr;
    ...
    ptr = (((unsigned char *) block) + sizeof (dns_msgblock_t) + (sizeof_type * block->remaining));

    return (ptr);
}

static inline void msgblock_reset (dns_msgblock_t * block)
{
 ...
 */
static inline void msgblock_free (isc_mem_t * mctx, dns_msgblock_t * block, unsigned int sizeof_type)
{
    unsigned int length;
 ...
 * uses)
 */
static inline isc_result_t newbuffer (dns_message_t * msg, unsigned int size)
{
    isc_result_t result;
...
static inline isc_buffer_t *currentbuffer (dns_message_t * msg)
{
    isc_buffer_t *dynbuf;
...
static inline void releaserdata (dns_message_t * msg, dns_rdata_t * rdata)
{
    ISC_LIST_PREPEND (msg->freerdata, rdata, link);
}

static inline dns_rdata_t *newrdata (dns_message_t * msg)
{
    ...
    if (rdata == NULL)
    {
        msgblock = msgblock_allocate (msg->mctx, sizeof (dns_rdata_t), RDATA_COUNT);
        if (msgblock == NULL)
            return (NULL);
...
static inline void releaserdatalist (dns_message_t * msg, dns_rdatalist_t * rdatalist)
{
    ISC_LIST_PREPEND (msg->freerdatalist, rdatalist, link);
}

static inline dns_rdatalist_t *newrdatalist (dns_message_t * msg)
{
    dns_msgblock_t *msgblock;
    ...
    if (rdatalist == NULL)
    {
        msgblock = msgblock_allocate (msg->mctx, sizeof (dns_rdatalist_t), RDATALIST_COUNT);
        if (msgblock == NULL)
            return (NULL);
...
static inline dns_offsets_t *newoffsets (dns_message_t * msg)
{
    dns_msgblock_t *msgblock;
    ...
    if (offsets == NULL)
    {
        msgblock = msgblock_allocate (msg->mctx, sizeof (dns_offsets_t), OFFSET_COUNT);
        if (msgblock == NULL)
            return (NULL);
...
static inline void msginitheader (dns_message_t * m)
{
    m->id = 0;
    m->flags = 0;
    m->rcode = 0;
...
static inline void msginitprivate (dns_message_t * m)
{
    unsigned int i;
...
static inline void msginittsig (dns_message_t * m)
{
 ...
 */
static inline void msginit (dns_message_t * m)
{
    msginitheader (m);
    msginitprivate (m);
    msginittsig (m);
...
static inline void msgresetnames (dns_message_t * msg, unsigned int first_section)
{
    unsigned int i;
...
isc_result_t dns_message_create (isc_mem_t * mctx, unsigned int intent, dns_message_t ** msgp)
{
    ...
    REQUIRE (msgp != NULL);
    REQUIRE (*msgp == NULL);
    REQUIRE (intent == DNS_MESSAGE_INTENTPARSE || intent == DNS_MESSAGE_INTENTRENDER);
    ...
    result = isc_mempool_create (m->mctx, sizeof (dns_rdataset_t), &m->rdspool);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
...
static isc_result_t findname (dns_name_t ** foundname, dns_name_t * target, dns_namelist_t * section)
{
    ...
    for (curr = ISC_LIST_TAIL (*section); curr != NULL; curr = ISC_LIST_PREV (curr, link))
    {
...
dns_message_find (dns_name_t * name, dns_rdataclass_t rdclass,
                  dns_rdatatype_t type, dns_rdatatype_t covers, dns_rdataset_t ** rdataset)
{
    dns_rdataset_t *curr;
...
isc_result_t
dns_message_findtype (dns_name_t * name, dns_rdatatype_t type, dns_rdatatype_t covers, dns_rdataset_t ** rdataset)
{
    dns_rdataset_t *curr;
...
static isc_result_t getname (dns_name_t * name, isc_buffer_t * source, dns_message_t * msg, dns_decompress_t * dctx)
{
    ...
    while (tries < 2)
    {
        result = dns_name_fromwire (name, source, dctx, ISC_FALSE, scratch);
...
getrdata (isc_buffer_t * source, dns_message_t * msg, dns_decompress_t * dctx,
          dns_rdataclass_t rdclass, dns_rdatatype_t rdtype, unsigned int rdatalen, dns_rdata_t * rdata)
{
    isc_buffer_t *scratch;
    ...
    for (;;)
    {
        result = dns_rdata_fromwire (rdata, rdclass, rdtype, source, dctx, 0, scratch);
...
static isc_result_t
getquestions (isc_buffer_t * source, dns_message_t * msg, dns_decompress_t * dctx, unsigned int options)
{
    isc_region_t r;
...
static isc_boolean_t update (dns_section_t section, dns_rdataclass_t rdclass)
{
    if (section == DNS_SECTION_PREREQUISITE)
        return (ISC_TF (rdclass == dns_rdataclass_any || rdclass == dns_rdataclass_none));
            ...
            dns_section_t sectionid, unsigned int options)
{
    ...
    for (count = 0; count < msg->counts[sectionid]; count++)
    {
         ...
         */
        if (msg->state == DNS_SECTION_ANY && rdtype != dns_rdatatype_opt &&    /* class is UDP SIZE */
         ...
         * section, bail.
         */
        if (msg->opcode != dns_opcode_update && rdtype != dns_rdatatype_tsig && rdtype != dns_rdatatype_opt && rdtype != dns_rdatatype_dnskey    /* in a TKEY query */
            && rdtype != dns_rdatatype_sig    /* SIG(0) */
            && rdtype != dns_rdatatype_tkey    /* Win2000 TKEY */
            && msg->rdclass != dns_rdataclass_any && msg->rdclass != rdclass)
            DO_FORMERR;
        ...
        if (rdtype == dns_rdatatype_tsig)
        {
            ...
            if (sectionid != DNS_SECTION_ADDITIONAL ||
                rdclass != dns_rdataclass_any || count != msg->counts[sectionid] - 1)
                DO_FORMERR;
        ...
        else if (rdtype == dns_rdatatype_opt)
        {
             ...
             */
            if (!dns_name_equal (dns_rootname, name) || msg->opt != NULL)
                DO_FORMERR;
            skip_name_search = ISC_TRUE;
            skip_type_search = ISC_TRUE;
        }
        else if (rdtype == dns_rdatatype_tkey)
        {
            ...
            if (sectionid != tkeysection && sectionid != DNS_SECTION_ANSWER)
                DO_FORMERR;
        }
        ...
        if (msg->opcode == dns_opcode_update && update (sectionid, rdclass))
        {
            if (rdatalen != 0)
            {
                result = DNS_R_FORMERR;
        ...
        else if (rdclass == dns_rdataclass_none && msg->opcode == dns_opcode_update && sectionid == DNS_SECTION_UPDATE)
        {
            result = getrdata (source, msg, dctx, msg->rdclass, rdtype, rdatalen, rdata);
        }
        else
            result = getrdata (source, msg, dctx, rdclass, rdtype, rdatalen, rdata);
        if (result != ISC_R_SUCCESS)
            goto cleanup;
        rdata->rdclass = rdclass;
        issigzero = ISC_FALSE;
                 ...
                 rdata->flags == 0)
        {
            ...
            if (covers == 0)
            {
                if (sectionid != DNS_SECTION_ADDITIONAL || count != msg->counts[sectionid] - 1)
                    DO_FORMERR;
                msg->sigstart = recstart;
                skip_name_search = ISC_TRUE;
                skip_type_search = ISC_TRUE;
         ...
         */
        if (preserve_order || msg->opcode == dns_opcode_update || skip_name_search)
        {
            if (rdtype != dns_rdatatype_opt && rdtype != dns_rdatatype_tsig && !issigzero)
            {
                ISC_LIST_APPEND (*section, name, link);
                free_name = ISC_FALSE;
            }
         ...
         */
        if (preserve_order || msg->opcode == dns_opcode_update || skip_type_search)
        ...
        else
        {
            ...
            rdataset = NULL;
            result = dns_message_find (name, rdclass, rdtype, covers, &rdataset);
        }
        ...
        if (rdtype == dns_rdatatype_opt && msg->opt == NULL)
        {
            ...
            rdataset = NULL;
            free_rdataset = ISC_FALSE;
            ercode = (dns_rcode_t) ((msg->opt->ttl & DNS_MESSAGE_EDNSRCODE_MASK) >> 20);
            msg->rcode |= ercode;
            isc_mempool_put (msg->namepool, name);
            free_name = ISC_FALSE;
        }
...
isc_result_t dns_message_parse (dns_message_t * msg, isc_buffer_t * source, unsigned int options)
{
    ...
    msg->opcode = ((tmpflags & DNS_MESSAGE_OPCODE_MASK) >> DNS_MESSAGE_OPCODE_SHIFT);
    ...
    if (r.length != 0)
    {
        isc_log_write (dns_lctx, ISC_LOGCATEGORY_GENERAL,
                       ...
                       "message has %u byte(s) of trailing garbage", r.length);
    }
  ...
  truncated:
    ...
    else
    {
        ...
        if (msg->saved.base == NULL)
            return (ISC_R_NOMEMORY);
        memcpy (msg->saved.base, isc_buffer_base (&origsource), msg->saved.length);
        msg->free_saved = 1;
    }
...
static inline isc_boolean_t wrong_priority (dns_rdataset_t * rds, int pass, dns_rdatatype_t preferred_glue)
{
    int pass_needed;
 ...
 */
static inline isc_boolean_t norender_rdataset (const dns_rdataset_t * rdataset, unsigned int options)
{
    switch (rdataset->type)
    {
        ...
        case dns_rdatatype_rrsig:
            if ((options & DNS_MESSAGERENDER_FILTER_AAAA) == 0 || rdataset->covers != dns_rdatatype_aaaa)
                return (ISC_FALSE);
            break;
...
isc_result_t dns_message_rendersection (dns_message_t * msg, dns_section_t sectionid, unsigned int options)
{
    ...
    if ((sectionid == DNS_SECTION_ADDITIONAL) && (options & DNS_MESSAGERENDER_ORDERED) == 0)
    {
            ...
            (rdataset->attributes & DNS_RDATASETATTR_RENDERED) == 0)
        {
            ...
            if (partial)
                result = dns_rdataset_towirepartial (rdataset,
                                                     name,
                                                     msg->cctx,
                                                     msg->buffer, msg->order, order_arg, rd_options, &count, NULL);
            else
                result = dns_rdataset_towiresorted (rdataset,
                                                    name,
                                                    msg->cctx, msg->buffer, msg->order, order_arg, rd_options, &count);
            total += count;
            ...
            if (result != ISC_R_SUCCESS)
            {
                ...
                dns_compress_rollback (msg->cctx, (isc_uint16_t) st.used);
    ...
    do
    {
        ...
        while (name != NULL)
        {
            ...
            while (rdataset != NULL)
            {
                ...
                if ((rdataset->attributes & DNS_RDATASETATTR_RENDERED) != 0)
                    goto next;

                if (((options & DNS_MESSAGERENDER_ORDERED)
                     == 0) && (sectionid == DNS_SECTION_ADDITIONAL) && wrong_priority (rdataset, pass, preferred_glue))
                    goto next;
                 ...
                 */
                if (norender_rdataset (rdataset, options) && sectionid != DNS_SECTION_QUESTION)
                {
                    if (sectionid == DNS_SECTION_ANSWER || sectionid == DNS_SECTION_AUTHORITY)
                ...
                if (partial)
                    result = dns_rdataset_towirepartial (rdataset,
                                                         ...
                                                         msg->cctx,
                                                         msg->buffer,
                                                         msg->order, msg->order_arg, rd_options, &count, NULL);
                else
                    result = dns_rdataset_towiresorted (rdataset,
                                                        name,
                                                        msg->cctx,
                                                        msg->buffer, msg->order, msg->order_arg, rd_options, &count);
                ...
                if (rdataset->trust != dns_trust_secure &&
                    (sectionid == DNS_SECTION_ANSWER || sectionid == DNS_SECTION_AUTHORITY))
                    msg->flags &= ~DNS_MESSAGEFLAG_AD;
                if (OPTOUT (rdataset))
                    msg->flags &= ~DNS_MESSAGEFLAG_AD;
...
void dns_message_renderheader (dns_message_t * msg, isc_buffer_t * target)
{
    ...
    INSIST (msg->counts[DNS_SECTION_QUESTION] < 65536 &&
            ...
            msg->counts[DNS_SECTION_AUTHORITY] < 65536 && msg->counts[DNS_SECTION_ADDITIONAL] < 65536);

    isc_buffer_putuint16 (target, tmp);
    isc_buffer_putuint16 (target, (isc_uint16_t) msg->counts[DNS_SECTION_QUESTION]);
    isc_buffer_putuint16 (target, (isc_uint16_t) msg->counts[DNS_SECTION_ANSWER]);
    isc_buffer_putuint16 (target, (isc_uint16_t) msg->counts[DNS_SECTION_AUTHORITY]);
    isc_buffer_putuint16 (target, (isc_uint16_t) msg->counts[DNS_SECTION_ADDITIONAL]);
}
...
isc_result_t dns_message_renderend (dns_message_t * msg)
{
    ...
    if (msg->opt != NULL)
    {
        ...
        msg->opt->ttl |= ((msg->rcode << 20) & DNS_MESSAGE_EDNSRCODE_MASK);
        /*
         * Render.
         */
        count = 0;
        result = dns_rdataset_towire (msg->opt, dns_rootname, msg->cctx, msg->buffer, 0, &count);
     ...
     * don't include it.
     */
    if ((msg->tsigkey != NULL || msg->sig0key != NULL) && (msg->flags & DNS_MESSAGEFLAG_TC) != 0)
    {
        isc_buffer_t *buf;
    ...
    if (msg->tsigkey != NULL)
    {
        ...
        if (result != ISC_R_SUCCESS)
            return (result);
        count = 0;
        result = dns_rdataset_towire (msg->tsig, msg->tsigname, msg->cctx, msg->buffer, 0, &count);
        msg->counts[DNS_SECTION_ADDITIONAL] += count;
        if (result != ISC_R_SUCCESS)
            return (result);
    }
    ...
    if (msg->sig0key != NULL)
    {
         ...
         */
        result = dns_rdataset_towire (msg->sig0, dns_rootname, msg->cctx, msg->buffer, 0, &count);
        msg->counts[DNS_SECTION_ADDITIONAL] += count;
        if (result != ISC_R_SUCCESS)
            return (result);
    }
...
void dns_message_renderreset (dns_message_t * msg)
{
    ...
    for (i = 0; i < DNS_SECTION_MAX; i++)
    {
        msg->cursors[i] = NULL;
        msg->counts[i] = 0;
        for (name = ISC_LIST_HEAD (msg->sections[i]); name != NULL; name = ISC_LIST_NEXT (name, link))
        {
            for (rds = ISC_LIST_HEAD (name->list); rds != NULL; rds = ISC_LIST_NEXT (rds, link))
            {
...
dns_message_findname (dns_message_t * msg, dns_section_t section,
                      ...
                      dns_rdatatype_t covers, dns_name_t ** name, dns_rdataset_t ** rdataset)
{
...
#ifdef SKAN_MSG_DEBUG
void dns_message_dumpsig (dns_message_t * msg, char *txt1)
{
    ...
    if (msg->querytsig != NULL)
    {
        ...
        result = dns_rdata_tostruct (&querytsigrdata, &querytsig, NULL);
        RUNTIME_CHECK (result == ISC_R_SUCCESS);
        hexdump (txt1, "QUERYTSIG", querytsig.signature, querytsig.siglen);
    }
}
#endif
...
isc_result_t dns_message_checksig (dns_message_t * msg, dns_view_t * view)
{
    ...
    else
    {
        ...
        result = dns_view_simplefind (view, &sig.signer, dns_rdatatype_key /* SIG(0) */ ,
        ...
        INSIST (result == ISC_R_SUCCESS);
        for (; result == ISC_R_SUCCESS; result = dns_rdataset_next (&keyset))
        {
            dst_key_t *key = NULL;
            ...
            result = dst_key_fromdns (&sig.signer, rdata.rdclass, &b, view->mctx, &key);
            if (result != ISC_R_SUCCESS)
                continue;
            if (dst_key_alg (key) != sig.algorithm ||
                dst_key_id (key) != sig.keyid ||
                !(dst_key_proto (key) == DNS_KEYPROTO_DNSSEC || dst_key_proto (key) == DNS_KEYPROTO_ANY))
            {
                dst_key_free (&key);
                continue;
            }
...
dns_message_sectiontotext (dns_message_t * msg, dns_section_t section,
                           const dns_master_style_t * style, dns_messagetextflag_t flags, isc_buffer_t * target)
{
    ...
    do
    {
        ...
        {
            if (section == DNS_SECTION_ANSWER && rdataset->type == dns_rdatatype_soa)
            {
                if ((flags & DNS_MESSAGETEXTFLAG_OMITSOA) != 0)
                    continue;
                if (seensoa && (flags & DNS_MESSAGETEXTFLAG_ONESOA) != 0)
                    continue;
                seensoa = ISC_TRUE;
            }
            ...
            {
                ADD_STRING (target, ";");
                result = dns_master_questiontotext (name, rdataset, style, target);
            }
            else
            {
                result = dns_master_rdatasettotext (name, rdataset, style, target);
            }
            if (result != ISC_R_SUCCESS)
                return (result);
        }
    ...
    if ((flags & DNS_MESSAGETEXTFLAG_NOHEADERS) == 0 && (flags & DNS_MESSAGETEXTFLAG_NOCOMMENTS) == 0)
        ADD_STRING (target, "\n");
    if (result == ISC_R_NOMORE)
        result = ISC_R_SUCCESS;
    return (result);
                                 ...
                                 const dns_master_style_t * style, dns_messagetextflag_t flags, isc_buffer_t * target)
{
    ...
    switch (section)
    {
        case DNS_PSEUDOSECTION_OPT:
            ...
            snprintf (buf, sizeof (buf), "%u", (unsigned int) ((ps->ttl & 0x00ff0000) >> 16));
                ...
                if (optlen != 0)
                {
                    ...
                    for (i = 0; i < optlen; i++)
                    {
                        ...
                        if (isprint (optdata[i]))
                            isc_buffer_putmem (target, &optdata[i], 1);
                        else
                    ...
                    dns_messagetextflag_t flags, isc_buffer_t * target)
{
    ...
    if ((flags & DNS_MESSAGETEXTFLAG_NOHEADERS) == 0)
    {
        ...
        snprintf (buf, sizeof (buf), "%1u", msg->counts[DNS_SECTION_QUESTION]);
        ADD_STRING (target, buf);
        if (msg->opcode != dns_opcode_update)
        {
        ...
        snprintf (buf, sizeof (buf), "%1u", msg->counts[DNS_SECTION_ANSWER]);
        ADD_STRING (target, buf);
        if (msg->opcode != dns_opcode_update)
        {
        ...
        snprintf (buf, sizeof (buf), "%1u", msg->counts[DNS_SECTION_AUTHORITY]);
        ADD_STRING (target, buf);
        ADD_STRING (target, ", ADDITIONAL: ");
        snprintf (buf, sizeof (buf), "%1u", msg->counts[DNS_SECTION_ADDITIONAL]);
        ADD_STRING (target, buf);
        ADD_STRING (target, "\n");
    }
    result = dns_message_pseudosectiontotext (msg, DNS_PSEUDOSECTION_OPT, style, flags, target);
    if (result != ISC_R_SUCCESS)
        return (result);

    result = dns_message_sectiontotext (msg, DNS_SECTION_QUESTION, style, flags, target);
    if (result != ISC_R_SUCCESS)
        return (result);
    result = dns_message_sectiontotext (msg, DNS_SECTION_ANSWER, style, flags, target);
    if (result != ISC_R_SUCCESS)
        return (result);
    result = dns_message_sectiontotext (msg, DNS_SECTION_AUTHORITY, style, flags, target);
    if (result != ISC_R_SUCCESS)
        return (result);
    result = dns_message_sectiontotext (msg, DNS_SECTION_ADDITIONAL, style, flags, target);
    if (result != ISC_R_SUCCESS)
        return (result);

    result = dns_message_pseudosectiontotext (msg, DNS_PSEUDOSECTION_TSIG, style, flags, target);
    if (result != ISC_R_SUCCESS)
        return (result);

    result = dns_message_pseudosectiontotext (msg, DNS_PSEUDOSECTION_SIG0, style, flags, target);
    if (result != ISC_R_SUCCESS)
        return (result);
