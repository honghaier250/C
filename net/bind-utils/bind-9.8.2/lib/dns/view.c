...
isc_result_t dns_view_create (isc_mem_t * mctx, dns_rdataclass_t rdclass, const char *name, dns_view_t ** viewp)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "dns_zt_create() failed: %s", isc_result_totext (result));
    ...
    if (result != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "dns_fwdtable_create() failed: %s", isc_result_totext (result));
        result = ISC_R_UNEXPECTED;
        goto cleanup_zt;
    }
    ...
    view->weakrefs = 0;
    view->attributes = (DNS_VIEWATTR_RESSHUTDOWN | DNS_VIEWATTR_ADBSHUTDOWN | DNS_VIEWATTR_REQSHUTDOWN);
    ...
    ISC_EVENT_INIT (&view->resevent, sizeof (view->resevent), 0, NULL,
                    DNS_EVENT_VIEWRESSHUTDOWN, resolver_shutdown, view, NULL, NULL, NULL);
    ISC_EVENT_INIT (&view->adbevent, sizeof (view->adbevent), 0, NULL,
                    DNS_EVENT_VIEWADBSHUTDOWN, adb_shutdown, view, NULL, NULL, NULL);
    ISC_EVENT_INIT (&view->reqevent, sizeof (view->reqevent), 0, NULL,
                    DNS_EVENT_VIEWREQSHUTDOWN, req_shutdown, view, NULL, NULL, NULL);
...
static inline void destroy (dns_view_t * view)
{
#ifdef BIND9
    dns_dns64_t *dns64;
#endif
    ...
    if (view->dynamickeys != NULL)
    {
        ...
        n = snprintf (keyfile, sizeof (keyfile), "%s.tsigkeys", view->name);
        if (n > 0 && (size_t) n < sizeof (keyfile))
        {
            result = isc_file_mktemplate (keyfile, template, sizeof (template));
            if (result == ISC_R_SUCCESS)
        ...
        {
            result = dns_tsigkeyring_dumpanddetach (&view->dynamickeys, fp);
            if (result == ISC_R_SUCCESS)
            {
                if (fclose (fp) == 0)
                    result = isc_file_rename (template, keyfile);
                if (result != ISC_R_SUCCESS)
    ...
    if (view->delonly != NULL)
    {
        ...
        isc_mem_put (view->mctx, view->delonly, sizeof (dns_namelist_t) * DNS_VIEW_DELONLYHASH);
        view->delonly = NULL;
    }
    if (view->rootexclude != NULL)
    {
        ...
        for (i = 0; i < DNS_VIEW_DELONLYHASH; i++)
        {
            ...
            while (name != NULL)
            {
                ISC_LIST_UNLINK (view->rootexclude[i], name, link);
                dns_name_free (name, view->mctx);
                isc_mem_put (view->mctx, name, sizeof (*name));
                name = ISC_LIST_HEAD (view->rootexclude[i]);
            }
        }
        isc_mem_put (view->mctx, view->rootexclude, sizeof (dns_namelist_t) * DNS_VIEW_DELONLYHASH);
        view->rootexclude = NULL;
    }
...
#ifdef BIND9
    for (dns64 = ISC_LIST_HEAD (view->dns64); dns64 != NULL; dns64 = ISC_LIST_HEAD (view->dns64))
    {
 ...
 */
static isc_boolean_t all_done (dns_view_t * view)
{

    if (isc_refcount_current (&view->references) == 0 &&
        view->weakrefs == 0 && RESSHUTDOWN (view) && ADBSHUTDOWN (view) && REQSHUTDOWN (view))
        return (ISC_TRUE);
...
static void view_flushanddetach (dns_view_t ** viewp, isc_boolean_t flush)
{
    dns_view_t *view;
...
#ifdef BIND9
static isc_result_t dialup (dns_zone_t * zone, void *dummy)
{
    UNUSED (dummy);
...
void dns_view_weakdetach (dns_view_t ** viewp)
{
    dns_view_t *view;
...
dns_view_createresolver (dns_view_t * view,
                         ...
                         dns_dispatchmgr_t * dispatchmgr, dns_dispatch_t * dispatchv4, dns_dispatch_t * dispatchv6)
{
    isc_result_t result;
    ...
    result = dns_resolver_create (view, taskmgr, ntasks, socketmgr,
                                  timermgr, options, dispatchmgr, dispatchv4, dispatchv6, &view->resolver);
    if (result != ISC_R_SUCCESS)
    {
        isc_task_detach (&view->task);
        return (result);
    ...
    result = dns_requestmgr_create (view->mctx, timermgr, socketmgr,
                                    ...
                                    dns_resolver_dispatchv6 (view->resolver), &view->requestmgr);
    if (result != ISC_R_SUCCESS)
    {
        dns_adb_shutdown (view->adb);
        dns_resolver_shutdown (view->resolver);
               ...
               dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    return (dns_view_find2 (view, name, type, now, options, use_hints,
                            ISC_FALSE, dbp, nodep, foundname, rdataset, sigrdataset));
}
                ...
                dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    ...
    if (zone != NULL && dns_zone_gettype (zone) == dns_zone_staticstub && !use_static_stub)
    {
        result = ISC_R_NOTFOUND;
    }
    if (result == ISC_R_SUCCESS || result == DNS_R_PARTIALMATCH)
    {
        ...
        if (result != ISC_R_SUCCESS && view->cachedb != NULL)
            ...
            goto cleanup;
        if (dns_zone_gettype (zone) == dns_zone_staticstub && dns_name_equal (name, dns_zone_getorigin (zone)))
        {
  ...
  db_find:
     ...
     */
    result = dns_db_find (db, name, NULL, type, options, now, &node, foundname, rdataset, sigrdataset);
    ...
    if (result == DNS_R_DELEGATION || result == ISC_R_NOTFOUND)
    {
        ...
        else
        {
            ...
            if (dns_rdataset_isassociated (&zrdataset))
            {
                ...
                if (sigrdataset != NULL && dns_rdataset_isassociated (&zsigrdataset))
    ...
    if (result == ISC_R_NOTFOUND && use_hints && view->hints != NULL)
    {
        ...
        result = dns_db_find (view->hints, name, NULL, type, options, now, &node, foundname, rdataset, sigrdataset);
...
dns_view_simplefind (dns_view_t * view, dns_name_t * name, dns_rdatatype_t type,
                     isc_stdtime_t now, unsigned int options,
                     isc_boolean_t use_hints, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    isc_result_t result;
    ...
    result = dns_view_find (view, name, type, now, options, use_hints,
                            NULL, NULL, dns_fixedname_name (&foundname), rdataset, sigrdataset);
    if (result == DNS_R_NXDOMAIN)
    {
        ...
        if (sigrdataset != NULL && dns_rdataset_isassociated (sigrdataset))
            dns_rdataset_disassociate (sigrdataset);
             ...
             result != DNS_R_NXRRSET && result != DNS_R_HINTNXRRSET && result != ISC_R_NOTFOUND)
    {
        if (dns_rdataset_isassociated (rdataset))
            dns_rdataset_disassociate (rdataset);
        if (sigrdataset != NULL && dns_rdataset_isassociated (sigrdataset))
...
dns_view_findzonecut (dns_view_t * view, dns_name_t * name, dns_name_t * fname,
                      ...
                      isc_boolean_t use_hints, dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
    return (dns_view_findzonecut2 (view, name, fname, now, options, use_hints, ISC_TRUE, rdataset, sigrdataset));
}
                       ...
                       dns_rdataset_t * rdataset, dns_rdataset_t * sigrdataset)
{
  ...
  db_find:
    ...
    if (!is_cache)
    {
        result = dns_db_find (db, name, NULL, dns_rdatatype_ns, options, now, NULL, fname, rdataset, sigrdataset);
    ...
    {
        result = dns_db_findzonecut (db, name, options, now, NULL, fname, rdataset, sigrdataset);
        if (result == ISC_R_SUCCESS)
        {
            if (zfname != NULL &&
                (!dns_name_issubdomain (fname, zfname) || (dns_zone_staticstub && dns_name_equal (fname, zfname))))
            {
                /*
  ...
  finish:
    if (use_zone)
    {
        ...
        if (result != ISC_R_SUCCESS)
            goto cleanup;
        dns_rdataset_clone (&zrdataset, rdataset);
        if (sigrdataset != NULL && dns_rdataset_isassociated (&zrdataset))
            dns_rdataset_clone (&zsigrdataset, sigrdataset);
    }
    else if (try_hints && use_hints && view->hints != NULL)
    {
         ...
         */
        result = dns_db_find (view->hints, dns_rootname, NULL, dns_rdatatype_ns, 0, now, NULL, fname, rdataset, NULL);
        if (result != ISC_R_SUCCESS)
        {
            /*
...
isc_result_t dns_viewlist_find (dns_viewlist_t * list, const char *name, dns_rdataclass_t rdclass, dns_view_t ** viewp)
{
    ...
    for (view = ISC_LIST_HEAD (*list); view != NULL; view = ISC_LIST_NEXT (view, link))
    {
...
dns_viewlist_findzone (dns_viewlist_t * list, dns_name_t * name,
                       isc_boolean_t allclasses, dns_rdataclass_t rdclass, dns_zone_t ** zonep)
{
    dns_view_t *view;
    ...
    for (view = ISC_LIST_HEAD (*list); view != NULL; view = ISC_LIST_NEXT (view, link))
    {
        ...
        INSIST (result == ISC_R_SUCCESS || result == ISC_R_NOTFOUND || result == DNS_R_PARTIALMATCH);
...
isc_result_t dns_view_gettsig (dns_view_t * view, dns_name_t * keyname, dns_tsigkey_t ** keyp)
{
    ...
    if (result == ISC_R_NOTFOUND)
        result = dns_tsigkey_find (keyp, keyname, NULL, view->dynamickeys);
    return (result);
}
...
isc_result_t dns_view_checksig (dns_view_t * view, isc_buffer_t * source, dns_message_t * msg)
{
    ...
    return (dns_tsig_verify (source, msg, view->statickeys, view->dynamickeys));
}
...
isc_result_t dns_view_dumpdbtostream (dns_view_t * view, FILE * fp)
{
    ...
    result = dns_master_dumptostream (view->mctx, view->cachedb, NULL, &dns_master_style_cache, fp);
    if (result != ISC_R_SUCCESS)
        return (result);
...
isc_result_t dns_view_adddelegationonly (dns_view_t * view, dns_name_t * name)
{
    ...
    if (view->delonly == NULL)
    {
        view->delonly = isc_mem_get (view->mctx, sizeof (dns_namelist_t) * DNS_VIEW_DELONLYHASH);
        if (view->delonly == NULL)
            return (ISC_R_NOMEMORY);
...
isc_result_t dns_view_excludedelegationonly (dns_view_t * view, dns_name_t * name)
{
    ...
    if (view->rootexclude == NULL)
    {
        view->rootexclude = isc_mem_get (view->mctx, sizeof (dns_namelist_t) * DNS_VIEW_DELONLYHASH);
        if (view->rootexclude == NULL)
            return (ISC_R_NOMEMORY);
        for (hash = 0; hash < DNS_VIEW_DELONLYHASH; hash++)
...
#ifdef BIND9
isc_result_t dns_view_freezezones (dns_view_t * view, isc_boolean_t value)
{
    REQUIRE (DNS_VIEW_VALID (view));
...
isc_result_t dns_view_issecuredomain (dns_view_t * view, dns_name_t * name, isc_boolean_t * secure_domain)
{
    REQUIRE (DNS_VIEW_VALID (view));

    if (view->secroots_priv == NULL)
        return (ISC_R_NOTFOUND);
    return (dns_keytable_issecuredomain (view->secroots_priv, name, secure_domain));
}
