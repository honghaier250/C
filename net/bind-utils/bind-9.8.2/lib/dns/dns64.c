                  ...
                  unsigned int flags, dns_dns64_t ** dns64)
{
    ...
    if (suffix != NULL)
        memcpy (new->bits + nbytes, suffix->type.in6.s6_addr + nbytes, 16 - nbytes);
    new->clients = NULL;
    if (clients != NULL)
                     ...
                     unsigned int flags, unsigned char *a, unsigned char *aaaa)
{
    ...
    if ((dns64->flags & DNS_DNS64_RECURSIVE_ONLY) != 0 && (flags & DNS_DNS64_RECURSIVE) == 0)
        return (DNS_R_DISALLOWED);

    if ((dns64->flags & DNS_DNS64_BREAK_DNSSEC) == 0 && (flags & DNS_DNS64_DNSSEC) != 0)
        return (DNS_R_DISALLOWED);

    if (dns64->clients != NULL)
    {
        result = dns_acl_match (reqaddr, reqsigner, dns64->clients, env, &match, NULL);
    ...
    if (dns64->mapped != NULL)
    {
        ...
        result = dns_acl_match (&netaddr, NULL, dns64->mapped, env, &match, NULL);
        if (result != ISC_R_SUCCESS)
            return (result);
        if (match <= 0)
            return (DNS_R_DISALLOWED);
...
dns_dns64_aaaaok (const dns_dns64_t * dns64, const isc_netaddr_t * reqaddr,
                  const dns_name_t * reqsigner, const dns_aclenv_t * env,
                  unsigned int flags, dns_rdataset_t * rdataset, isc_boolean_t * aaaaok, size_t aaaaoklen)
{
    struct in6_addr in6;
    ...
    for (; dns64 != NULL; dns64 = ISC_LIST_NEXT (dns64, link))
    {
        ...
        for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
        {
            ...
            if (aaaaok == NULL || !aaaaok[i])
            {
                ...
                result = dns_acl_match (&netaddr, NULL, dns64->excluded, env, &match, NULL);
