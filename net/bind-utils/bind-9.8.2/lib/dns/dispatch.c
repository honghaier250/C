...
#define MAX_INTERNAL_TASKS    64
     ...
     struct dns_dispatch
     {
         ...
         unsigned int shutting_down:1, shutdown_out:1, connected:1, tcpmsg_valid:1, recv_pending:1;    /*%< is a recv() pending? */
 ...
 * Statics.
 */
     static dns_dispentry_t *entry_search (dns_qid_t *, isc_sockaddr_t *, dns_messageid_t, in_port_t, unsigned int);
     ...
     static isc_result_t get_udpsocket (dns_dispatchmgr_t * mgr,
                                        dns_dispatch_t * disp,
                                        isc_socketmgr_t * sockmgr, isc_sockaddr_t * localaddr, isc_socket_t ** sockp);
     ...
     static isc_result_t qid_allocate (dns_dispatchmgr_t * mgr, unsigned int buckets,
                                       unsigned int increment, dns_qid_t ** qidp, isc_boolean_t needaddrtable);
...
#define LVL(x) ISC_LOG_DEBUG(x)
     ...
     static void mgr_log (dns_dispatchmgr_t * mgr, int level, const char *fmt, ...) ISC_FORMAT_PRINTF (3, 4);
     ...
     static void mgr_log (dns_dispatchmgr_t * mgr, int level, const char *fmt, ...)
{
    ...
    isc_log_write (dns_lctx,
                   DNS_LOGCATEGORY_DISPATCH, DNS_LOGMODULE_DISPATCH, level, "dispatchmgr %p: %s", mgr, msgbuf);
}

static inline void inc_stats (dns_dispatchmgr_t * mgr, isc_statscounter_t counter)
{
    if (mgr->stats != NULL)
        isc_stats_increment (mgr->stats, counter);
}

static void dispatch_log (dns_dispatch_t * disp, int level, const char *fmt, ...) ISC_FORMAT_PRINTF (3, 4);
     ...
     static void dispatch_log (dns_dispatch_t * disp, int level, const char *fmt, ...)
{
    ...
    isc_log_write (dns_lctx, DNS_LOGCATEGORY_DISPATCH, DNS_LOGMODULE_DISPATCH, level, "dispatch %p: %s", disp, msgbuf);
}

static void
request_log (dns_dispatch_t * disp, dns_dispentry_t * resp, int level, const char *fmt, ...) ISC_FORMAT_PRINTF (4, 5);

     static void request_log (dns_dispatch_t * disp, dns_dispentry_t * resp, int level, const char *fmt, ...)
{
    char msgbuf[2048];
    ...
    if (VALID_RESPONSE (resp))
    {
        ...
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DISPATCH,
                       DNS_LOGMODULE_DISPATCH, level, "dispatch %p response %p %s: %s", disp, resp, peerbuf, msgbuf);
    }
    else
    {
        isc_log_write (dns_lctx, DNS_LOGCATEGORY_DISPATCH,
                       DNS_LOGMODULE_DISPATCH, level, "dispatch %p req/resp %p: %s", disp, resp, msgbuf);
    }
}
 ...
 */
#ifdef BIND9
static void dispatch_initrandom (arc4ctx_t * actx, isc_entropy_t * entropy, isc_mutex_t * lock)
{
    int n;
...
static void dispatch_arc4addrandom (arc4ctx_t * actx, unsigned char *dat, int datlen)
{
    int n;
...
static inline isc_uint8_t dispatch_arc4get8 (arc4ctx_t * actx)
{
    isc_uint8_t si, sj;
...
static inline isc_uint16_t dispatch_arc4get16 (arc4ctx_t * actx)
{
    isc_uint16_t val;
...
static void dispatch_arc4stir (arc4ctx_t * actx)
{
    int i;
    ...
    if (actx->entropy != NULL)
    {
         ...
         */
        result = isc_entropy_getdata (actx->entropy, rnd.rnd, sizeof (rnd), NULL, 0);
...
static isc_uint16_t dispatch_random (arc4ctx_t * actx)
{
    isc_uint16_t result;
...
static isc_uint16_t dispatch_uniformrandom (arc4ctx_t * actx, isc_uint16_t upper_bound)
{
    isc_uint16_t min, r;
 ...
 */
static isc_uint32_t dns_hash (dns_qid_t * qid, isc_sockaddr_t * dest, dns_messageid_t id, in_port_t port)
{
    unsigned int ret;
 ...
 */
static dns_dispentry_t *linear_first (dns_qid_t * qid)
{
    dns_dispentry_t *ret;
 ...
 * no more entries.
 */
static dns_dispentry_t *linear_next (dns_qid_t * qid, dns_dispentry_t * resp)
{
    dns_dispentry_t *ret;
 ...
 */
static isc_boolean_t destroy_disp_ok (dns_dispatch_t * disp)
{
    if (disp->refcount != 0)
        return (ISC_FALSE);
...
static void destroy_disp (isc_task_t * task, isc_event_t * event)
{
    ...
    dispatch_log (disp, LVL (90), "shutting down; detaching from sock %p, task %p", disp->socket, disp->task[0]);    /* XXXX */
...
static dispportentry_t *port_search (dns_dispatch_t * disp, in_port_t port)
{
    ...
    portentry = ISC_LIST_HEAD (disp->port_table[port % DNS_DISPATCH_PORTTABLESIZE]);
...
static dispportentry_t *new_portentry (dns_dispatch_t * disp, in_port_t port)
{
    ...
    ISC_LIST_APPEND (disp->port_table[port % DNS_DISPATCH_PORTTABLESIZE], portentry, link);
...
static void deref_portentry (dns_dispatch_t * disp, dispportentry_t ** portentryp)
{
    ...
    if (portentry->refs == 0)
    {
        ISC_LIST_UNLINK (disp->port_table[portentry->port % DNS_DISPATCH_PORTTABLESIZE], portentry, link);
...
get_dispsocket (dns_dispatch_t * disp, isc_sockaddr_t * dest,
                isc_socketmgr_t * sockmgr, dns_qid_t * qid, dispsocket_t ** dispsockp, in_port_t * portp)
{
    int i;
    ...
    for (i = 0; i < 64; i++)
    {
        port = ports[dispatch_uniformrandom (DISP_ARC4CTX (disp), nports)];
        ...
        else if (result == ISC_R_NOPERM)
        {
            ...
            dispatch_log (disp, ISC_LOG_WARNING, "open_socket(%s) -> %s: continuing", buf, isc_result_totext (result));
        }
...
static void destroy_dispsocket (dns_dispatch_t * disp, dispsocket_t ** dispsockp)
{
    ...
    if (ISC_LINK_LINKED (dispsock, blink))
    {
        qid = DNS_QID (disp);
        LOCK (&qid->lock);
        ISC_LIST_UNLINK (qid->sock_table[dispsock->bucket], dispsock, blink);
        UNLOCK (&qid->lock);
    }
 ...
 * Return NULL if no such entry exists.
 */
static dns_dispentry_t *entry_search (dns_qid_t * qid, isc_sockaddr_t * dest, dns_messageid_t id,
                                      in_port_t port, unsigned int bucket)
{
    ...
    while (res != NULL)
    {
        if (res->id == id && isc_sockaddr_equal (dest, &res->host) && res->port == port)
        {
            return (res);
        }
...
static inline dns_dispatchevent_t *allocate_event (dns_dispatch_t * disp)
{
    ...
    if (ev == NULL)
        return (NULL);
    ISC_EVENT_INIT (ev, sizeof (*ev), 0, NULL, 0, NULL, NULL, NULL, NULL, NULL);

    return (ev);
}

static void udp_exrecv (isc_task_t * task, isc_event_t * ev)
{
...
static void udp_shrecv (isc_task_t * task, isc_event_t * ev)
{
 ...
 *    restart.
 */
static void udp_recv (isc_event_t * ev_in, dns_dispatch_t * disp, dispsocket_t * dispsock)
{
    ...
    if (dispsock != NULL && (ev->result == ISC_R_CANCELED || dispsock->resp == NULL))
    {
        /*
    ...
    else if (ev->result != ISC_R_SUCCESS)
    {
        ...
        if (ev->result != ISC_R_CANCELED)
            dispatch_log (disp, ISC_LOG_ERROR, "odd socket result in udp_recv(): %s", isc_result_totext (ev->result));
    ...
    if (disp->mgr->blackhole != NULL &&
        dns_acl_match (&netaddr, NULL, disp->mgr->blackhole, NULL, &match, NULL) == ISC_R_SUCCESS && match > 0)
    {
        if (isc_log_wouldlog (dns_lctx, LVL (10)))
        {
            ...
            dispatch_log (disp, LVL (10), "blackholed packet from %s", netaddrstr);
        }
        free_buffer (disp, ev->region.base, ev->region.length);
        goto restart;
    }
    ...
    dispatch_log (disp, LVL (92),
                  "got valid DNS message header, /QR %c, id %u", ((flags & DNS_MESSAGEFLAG_QR) ? '1' : '0'), id);
    ...
    if (resp == NULL)
    {
        ...
        LOCK (&qid->lock);
        qidlocked = ISC_TRUE;
        resp = entry_search (qid, &ev->address, id, disp->localport, bucket);
        dispatch_log (disp, LVL (90),
                      "search for response in bucket %d: %s", bucket, (resp == NULL ? "not found" : "found"));
    ...
    else if (resp->id != id || !isc_sockaddr_equal (&ev->address, &resp->host))
    {
        dispatch_log (disp, LVL (90), "response to an exclusive socket doesn't match");
        inc_stats (mgr, dns_resstatscounter_mismatch);
        free_buffer (disp, ev->region.base, ev->region.length);
        goto unlock;
    }
    ...
    if (disp != resp->disp)
    {
        ...
        if (disp->socktype != resp->disp->socktype ||
            isc_sockaddr_getport (&disp->local) != isc_sockaddr_getport (&resp->disp->local))
        {
            free_buffer (disp, ev->region.base, ev->region.length);
            goto unlock;
        }
         ...
         * via IPv6.
         */
        if (isc_sockaddr_pf (&resp->disp->local) == PF_INET6 && isc_sockaddr_pf (&disp->local) != PF_INET6)
        {
            free_buffer (disp, ev->region.base, ev->region.length);
            goto unlock;
        }
        ...
        if (!isc_sockaddr_eqaddr (&a1, &resp->disp->local) && !isc_sockaddr_eqaddr (&a2, &disp->local))
        {
            free_buffer (disp, ev->region.base, ev->region.length);
            goto unlock;
        }
  ...
  sendresponse:
    ...
    {
        ISC_EVENT_INIT (rev, sizeof (*rev), 0, NULL, DNS_EVENT_DISPATCH, resp->action, resp->arg, resp, NULL, NULL);
        request_log (disp, resp, LVL (90),
                     "[a] Sent event %p buffer %p len %d to task %p",
                     rev, rev->buffer.base, rev->buffer.length, resp->task);
 ...
 *    restart.
 */
static void tcp_recv (isc_task_t * task, isc_event_t * ev_in)
{
            ...
            default:
              ...
              logit:
                ...
                dispatch_log (disp, level, "shutting down due to TCP "
                              "receive error: %s: %s", buf, isc_result_totext (tcpmsg->result));
                do_cancel (disp);
                break;
        }
    ...
    dispatch_log (disp, LVL (90), "result %d, length == %d, addr = %p",
                  tcpmsg->result, tcpmsg->buffer.length, tcpmsg->buffer.base);
 ...
 * disp must be locked.
 */
static isc_result_t startrecv (dns_dispatch_t * disp, dispsocket_t * dispsock)
{
    isc_result_t res;
    ...
    if ((disp->attributes & DNS_DISPATCHATTR_EXCLUSIVE) != 0 && dispsock == NULL)
        return (ISC_R_SUCCESS);
    ...
    switch (disp->socktype)
    {
        ...
        case isc_sockettype_udp:
            ...
            if (dispsock != NULL)
            {
                res = isc_socket_recv (socket, &region, 1, dispsock->task, udp_exrecv, dispsock);
            ...
            {
                res = isc_socket_recv (socket, &region, 1, disp->task[0], udp_shrecv, disp);
                if (res != ISC_R_SUCCESS)
                {
                    free_buffer (disp, region.base, region.length);
        ...
        case isc_sockettype_tcp:
            res = dns_tcpmsg_readmessage (&disp->tcpmsg, disp->task[0], tcp_recv, disp);
            if (res != ISC_R_SUCCESS)
            {
                disp->shutdown_why = res;
                disp->shutting_down = 1;
...
static isc_boolean_t destroy_mgr_ok (dns_dispatchmgr_t * mgr)
{
    mgr_log (mgr, LVL (90),
             ...
             isc_mempool_getallocated (mgr->rpool), isc_mempool_getallocated (mgr->dpool));
...
static void destroy_mgr (dns_dispatchmgr_t ** mgrp)
{
    ...
    *mgrp = NULL;
    ...
    if (mgr->v4ports != NULL)
    {
        isc_mem_put (mctx, mgr->v4ports, mgr->nv4ports * sizeof (in_port_t));
    }
    if (mgr->v6ports != NULL)
    {
        isc_mem_put (mctx, mgr->v6ports, mgr->nv6ports * sizeof (in_port_t));
    }
...
static isc_result_t
open_socket (isc_socketmgr_t * mgr, isc_sockaddr_t * local, unsigned int options, isc_socket_t ** sockp)
{
    isc_socket_t *sock;
...
isc_result_t dns_dispatchmgr_create (isc_mem_t * mctx, isc_entropy_t * entropy, dns_dispatchmgr_t ** mgrp)
{
    ...
    mgr->epool = NULL;
    if (isc_mempool_create (mgr->mctx, sizeof (dns_dispatchevent_t), &mgr->epool) != ISC_R_SUCCESS)
    {
        result = ISC_R_NOMEMORY;
        goto kill_pool_lock;
    }
    ...
    mgr->rpool = NULL;
    if (isc_mempool_create (mgr->mctx, sizeof (dns_dispentry_t), &mgr->rpool) != ISC_R_SUCCESS)
    {
        result = ISC_R_NOMEMORY;
        goto kill_epool;
    }
    ...
    mgr->dpool = NULL;
    if (isc_mempool_create (mgr->mctx, sizeof (dns_dispatch_t), &mgr->dpool) != ISC_R_SUCCESS)
    {
        result = ISC_R_NOMEMORY;
        goto kill_rpool;
    }
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        if (result == ISC_R_SUCCESS)
        {
            result = dns_dispatchmgr_setavailports (mgr, v4portset, v6portset);
        }
    }
    if (v4portset != NULL)
...
isc_result_t
dns_dispatchmgr_setavailports (dns_dispatchmgr_t * mgr, isc_portset_t * v4portset, isc_portset_t * v6portset)
{
    ...
    if (nv6ports != 0)
    {
        ...
        if (v6ports == NULL)
        {
            if (v4ports != NULL)
            {
                isc_mem_put (mgr->mctx, v4ports, sizeof (in_port_t) * isc_portset_nports (v4portset));
            }
            return (ISC_R_NOMEMORY);
        }
    }
    ...
    if (mgr->v4ports != NULL)
    {
        isc_mem_put (mgr->mctx, mgr->v4ports, mgr->nv4ports * sizeof (in_port_t));
    }
    mgr->v4ports = v4ports;
    ...
    if (mgr->v6ports != NULL)
    {
        isc_mem_put (mgr->mctx, mgr->v6ports, mgr->nv6ports * sizeof (in_port_t));
    }
    mgr->v6ports = v6ports;
...
dns_dispatchmgr_setudp (dns_dispatchmgr_t * mgr,
                        ...
                        unsigned int maxrequests, unsigned int buckets, unsigned int increment)
{
    isc_result_t result;
    ...
    result = isc_mempool_create (mgr->mctx, sizeof (dispsocket_t), &mgr->spool);
    if (result != ISC_R_SUCCESS)
    {
        UNLOCK (&mgr->buffer_lock);
        goto cleanup;
...
static isc_boolean_t local_addr_match (dns_dispatch_t * disp, isc_sockaddr_t * addr)
{
    ...
    if (isc_sockaddr_getport (addr) == 0 &&
        isc_sockaddr_getport (&disp->local) == 0 && !portavailable (disp->mgr, disp->socket, NULL))
    {
        return (ISC_FALSE);
    }
...
dispatch_find (dns_dispatchmgr_t * mgr, isc_sockaddr_t * local,
               unsigned int attributes, unsigned int mask, dns_dispatch_t ** dispp)
{
    dns_dispatch_t *disp;
    ...
    while (disp != NULL)
    {
        LOCK (&disp->lock);
        if ((disp->shutting_down == 0)
            && ATTRMATCH (disp->attributes, attributes, mask) && local_addr_match (disp, local))
            break;
        UNLOCK (&disp->lock);
...
qid_allocate (dns_dispatchmgr_t * mgr, unsigned int buckets,
              unsigned int increment, dns_qid_t ** qidp, isc_boolean_t needsocktable)
{
    dns_qid_t *qid;
    ...
    qid->qid_table = isc_mem_get (mgr->mctx, buckets * sizeof (dns_displist_t));
    ...
    if (needsocktable)
    {
        qid->sock_table = isc_mem_get (mgr->mctx, buckets * sizeof (dispsocketlist_t));
        if (qid->sock_table == NULL)
        {
            isc_mem_put (mgr->mctx, qid->qid_table, buckets * sizeof (dns_displist_t));
            isc_mem_put (mgr->mctx, qid, sizeof (*qid));
            return (ISC_R_NOMEMORY);
        }
    }
    ...
    if (result != ISC_R_SUCCESS)
    {
        if (qid->sock_table != NULL)
        {
            isc_mem_put (mgr->mctx, qid->sock_table, buckets * sizeof (dispsocketlist_t));
        }
        isc_mem_put (mgr->mctx, qid->qid_table, buckets * sizeof (dns_displist_t));
        isc_mem_put (mgr->mctx, qid, sizeof (*qid));
        return (result);
...
static void qid_destroy (isc_mem_t * mctx, dns_qid_t ** qidp)
{
    ...
    *qidp = NULL;
    qid->magic = 0;
    isc_mem_put (mctx, qid->qid_table, qid->qid_nbuckets * sizeof (dns_displist_t));
    if (qid->sock_table != NULL)
    {
...
static void dispatch_free (dns_dispatch_t ** dispp)
{
    ...
    *dispp = NULL;
    ...
    if (disp->port_table != NULL)
    {
        ...
        isc_mem_put (mgr->mctx, disp->port_table, sizeof (disp->port_table[0]) * DNS_DISPATCH_PORTTABLESIZE);
    }
...
dns_dispatch_createtcp (dns_dispatchmgr_t * mgr, isc_socket_t * sock,
                        ...
                        unsigned int buckets, unsigned int increment, unsigned int attributes, dns_dispatch_t ** dispp)
{
    isc_result_t result;
    ...
    disp->ctlevent = isc_event_allocate (mgr->mctx, disp,
                                         DNS_EVENT_DISPATCHCONTROL, destroy_disp, disp, sizeof (isc_event_t));
                     ...
                     unsigned int attributes, unsigned int mask, dns_dispatch_t ** dispp)
{
    ...
    result = dns_dispatchmgr_setudp (mgr, buffersize, maxbuffers, maxrequests, buckets, increment);
    if (result != ISC_R_SUCCESS)
        return (result);
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        if ((disp->attributes & DNS_DISPATCHATTR_NOLISTEN) == 0 && (attributes & DNS_DISPATCHATTR_NOLISTEN) != 0)
        {
            disp->attributes |= DNS_DISPATCHATTR_NOLISTEN;
            if (disp->recv_pending != 0)
                isc_socket_cancel (disp->socket, disp->task[0], ISC_SOCKCANCEL_RECV);
        }
               ...
               isc_socketmgr_t * sockmgr, isc_sockaddr_t * localaddr, isc_socket_t ** sockp)
{
    ...
    if (anyport)
    {
        ...
        for (i = 0; i < 1024; i++)
        {
            ...
            prt = ports[dispatch_uniformrandom (DISP_ARC4CTX (disp), nports)];
            isc_sockaddr_setport (&localaddr_bound, prt);
            result = open_socket (sockmgr, &localaddr_bound, 0, &sock);
    ...
    else
    {
        ...
        result = open_socket (sockmgr, localaddr, ISC_SOCKET_REUSEADDRESS, &sock);
    ...
    if (j == 0xffffU)
    {
        mgr_log (mgr, ISC_LOG_ERROR,
                 ...
                 "an available port", isc_sockaddr_pf (localaddr) == AF_INET ? "4" : "6");
...
dispatch_createudp (dns_dispatchmgr_t * mgr, isc_socketmgr_t * sockmgr,
                    isc_taskmgr_t * taskmgr,
                    isc_sockaddr_t * localaddr,
                    unsigned int maxrequests, unsigned int attributes, dns_dispatch_t ** dispp)
{
    isc_result_t result;
    ...
    else
    {
        ...
        disp->port_table = isc_mem_get (mgr->mctx, sizeof (disp->port_table[0]) * DNS_DISPATCH_PORTTABLESIZE);
        if (disp->port_table == NULL)
            goto deallocate_dispatch;
        for (i = 0; i < DNS_DISPATCH_PORTTABLESIZE; i++)
            ISC_LIST_INIT (disp->port_table[i]);

        result = isc_mempool_create (mgr->mctx, sizeof (dispportentry_t), &disp->portpool);
        if (result != ISC_R_SUCCESS)
            goto deallocate_dispatch;
...
void dns_dispatch_detach (dns_dispatch_t ** dispp)
{
    ...
    *dispp = NULL;
    ...
    if (disp->refcount == 0)
    {
        ...
        {
            isc_socket_cancel (dispsock->socket, dispsock->task, ISC_SOCKCANCEL_RECV);
        }
        disp->shutting_down = 1;
    }
...
dns_dispatch_addresponse2 (dns_dispatch_t * disp, isc_sockaddr_t * dest,
                           ...
                           dns_messageid_t * idp, dns_dispentry_t ** resp, isc_socketmgr_t * sockmgr)
{
    dns_dispentry_t *res;
    ...
    if ((disp->attributes & DNS_DISPATCHATTR_EXCLUSIVE) != 0 && disp->nsockets > DNS_DISPATCH_SOCKSQUOTA)
    {
        ...
        if (oldestresp != NULL && !oldestresp->item_out)
        {
            ...
            if (rev != NULL)
            {
                ...
                ISC_EVENT_INIT (rev, sizeof (*rev), 0,
                                NULL, DNS_EVENT_DISPATCH, oldestresp->action, oldestresp->arg, oldestresp, NULL, NULL);
    ...
    if ((disp->attributes & DNS_DISPATCHATTR_EXCLUSIVE) != 0)
    {
         ...
         */
        result = get_dispsocket (disp, dest, sockmgr, qid, &dispsocket, &localport);
        if (result != ISC_R_SUCCESS)
        {
            UNLOCK (&qid->lock);
            UNLOCK (&disp->lock);
    ...
    if (((disp->attributes & DNS_DISPATCHATTR_UDP) != 0) || ((disp->attributes & DNS_DISPATCHATTR_CONNECTED) != 0))
    {
                          ...
                          dns_messageid_t * idp, dns_dispentry_t ** resp)
{
    ...
    return (dns_dispatch_addresponse2 (disp, dest, task, action, arg, idp, resp, NULL));
}
...
void dns_dispatch_removeresponse (dns_dispentry_t ** resp, dns_dispatchevent_t ** sockevent)
{
    ...
    *resp = NULL;
    ...
    if (ev == NULL && res->item_out)
    {
        ...
        n = isc_task_unsend (res->task, res, DNS_EVENT_DISPATCH, NULL, &events);
        /*
    ...
    {
        isc_socket_cancel (res->dispsocket->socket, res->dispsocket->task, ISC_SOCKCANCEL_RECV);
...
static void do_cancel (dns_dispatch_t * disp)
{
     ...
     */
    ev = disp->failsafe_ev;
    ISC_EVENT_INIT (ev, sizeof (*ev), 0, NULL, DNS_EVENT_DISPATCH, resp->action, resp->arg, resp, NULL, NULL);
    ev->result = disp->shutdown_why;
    ev->buffer.base = NULL;
    ev->buffer.length = 0;
    disp->shutdown_out = 1;
    request_log (disp, resp, LVL (10), "cancel: failsafe event %p -> task %p", ev, resp->task);
    resp->item_out = ISC_TRUE;
...
void dns_dispatch_changeattributes (dns_dispatch_t * disp, unsigned int attributes, unsigned int mask)
{
    ...
    REQUIRE ((disp->attributes & DNS_DISPATCHATTR_EXCLUSIVE) == 0 || (attributes & DNS_DISPATCHATTR_NOLISTEN) == 0);
    ...
    if ((mask & DNS_DISPATCHATTR_NOLISTEN) != 0)
    {
        ...
        else if ((disp->attributes & DNS_DISPATCHATTR_NOLISTEN) == 0 && (attributes & DNS_DISPATCHATTR_NOLISTEN) != 0)
        {
            disp->attributes |= DNS_DISPATCHATTR_NOLISTEN;
            if (disp->recv_pending != 0)
                isc_socket_cancel (disp->socket, disp->task[0], ISC_SOCKCANCEL_RECV);
...
void dns_dispatch_importrecv (dns_dispatch_t * disp, isc_event_t * event)
{
    ...
    newsevent = (isc_socketevent_t *)
        isc_event_allocate (disp->mgr->mctx, NULL,
                            DNS_EVENT_IMPORTRECVDONE, udp_shrecv, disp, sizeof (isc_socketevent_t));
    if (newsevent == NULL)
        return;
...
#if 0
void dns_dispatchmgr_dump (dns_dispatchmgr_t * mgr)
{
    dns_dispatch_t *disp;
