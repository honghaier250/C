 ...
 */
static isc_result_t
ttlfmt (unsigned int t, const char *s, isc_boolean_t verbose, isc_boolean_t space, isc_buffer_t * target)
{
    char tmp[60];
    ...
    if (verbose)
        len = snprintf (tmp, sizeof (tmp), "%s%u %s%s", space ? " " : "", t, s, t == 1 ? "" : "s");
    else
...
isc_result_t dns_ttl_totext (isc_uint32_t src, isc_boolean_t verbose, isc_buffer_t * target)
{
    ...
    if (x == 1 && !verbose)
    {
        ...
        region.base[region.length - 1] = toupper (region.base[region.length - 1]);
    }
    return (ISC_R_SUCCESS);
}
