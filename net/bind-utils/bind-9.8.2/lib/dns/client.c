...
getudpdispatch (int family, dns_dispatchmgr_t * dispatchmgr,
                isc_socketmgr_t * socketmgr, isc_taskmgr_t * taskmgr, isc_boolean_t is_shared, dns_dispatch_t ** dispp)
{
    ...
    result = dns_dispatch_getudp (dispatchmgr, socketmgr,
                                  taskmgr, &sa,
                                  buffersize, maxbuffers, maxrequests, buckets, increment, attrs, attrmask, &disp);
...
dns_client_createview (isc_mem_t * mctx, dns_rdataclass_t rdclass,
                       ...
                       dns_dispatch_t * dispatchv4, dns_dispatch_t * dispatchv6, dns_view_t ** viewp)
{
    isc_result_t result;
    ...
    result = dns_view_createresolver (view, taskmgr, ntasks, socketmgr,
                                      timermgr, 0, dispatchmgr, dispatchv4, dispatchv6);
    if (result != ISC_R_SUCCESS)
    {
        dns_view_detach (&view);
        return (result);
    ...
    if ((options & DNS_CLIENTCREATEOPT_USECACHE) != 0)
        ...
        dbtype = "ecdb";
    result = dns_db_create (mctx, dbtype, dns_rootname, dns_dbtype_cache, rdclass, 0, NULL, &view->cachedb);
    if (result != ISC_R_SUCCESS)
    {
        dns_view_detach (&view);
        return (result);
...
isc_result_t dns_client_create (dns_client_t ** clientp, unsigned int options)
{
    ...
    result = dns_client_createx (mctx, actx, taskmgr, socketmgr, timermgr, options, clientp);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
                    ...
                    unsigned int options, dns_client_t ** clientp)
{
    ...
    result = getudpdispatch (AF_INET, dispatchmgr, socketmgr, taskmgr, ISC_TRUE, &dispatchv4);
    if (result == ISC_R_SUCCESS)
        client->dispatchv4 = dispatchv4;
    result = getudpdispatch (AF_INET6, dispatchmgr, socketmgr, taskmgr, ISC_TRUE, &dispatchv6);
    if (result == ISC_R_SUCCESS)
    ...
    result = dns_client_createview (mctx, dns_rdataclass_in, options,
                                    taskmgr, 31, socketmgr, timermgr, dispatchmgr, dispatchv4, dispatchv6, &view);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
...
void dns_client_destroy (dns_client_t ** clientp)
{
    ...
    if (client->references == 0 && ISC_LIST_EMPTY (client->resctxs) &&
        ISC_LIST_EMPTY (client->reqctxs) && ISC_LIST_EMPTY (client->updatectxs))
    {
        destroyok = ISC_TRUE;
    }
    UNLOCK (&client->lock);
                       ...
                       dns_name_t * namespace, isc_sockaddrlist_t * addrs)
{
    ...
    LOCK (&client->lock);
    result = dns_viewlist_find (&client->viewlist, DNS_CLIENTVIEW_NAME, rdclass, &view);
    if (result != ISC_R_SUCCESS)
    {
        UNLOCK (&client->lock);
        return (result);
    ...
    result = dns_fwdtable_add (view->fwdtable, namespace, addrs, dns_fwdpolicy_only);
...
static inline isc_result_t start_fetch (resctx_t * rctx)
{
    isc_result_t result;
    ...
    result = dns_resolver_createfetch (rctx->view->resolver,
                                       ...
                                       rctx->type,
                                       NULL, NULL, NULL, 0,
                                       rctx->task, fetch_done, rctx, rctx->rdataset, rctx->sigrdataset, &rctx->fetch);
...
static isc_result_t view_find (resctx_t * rctx, dns_db_t ** dbp, dns_dbnode_t ** nodep, dns_name_t * foundname)
{
    ...
    result = dns_view_find (rctx->view, name, type, 0, 0, ISC_FALSE,
                            dbp, nodep, foundname, rctx->rdataset, rctx->sigrdataset);
...
static void client_resfind (resctx_t * rctx, dns_fetchevent_t * event)
{
    ...
    do
    {
        ...
        if (event == NULL && !rctx->canceled)
        {
            ...
            INSIST (rctx->sigrdataset == NULL || !dns_rdataset_isassociated (rctx->sigrdataset));
        ...
        switch (result)
        {
            ...
            case DNS_R_DNAME:
                ...
                tresult = dns_name_concatenate (prefix, &dname.dname, name, NULL);
        ...
        if (rctx->type == dns_rdatatype_any)
        {
            ...
            tresult = dns_db_allrdatasets (db, node, NULL, 0, &rdsiter);
            if (tresult != ISC_R_SUCCESS)
            {
                result = tresult;
                goto done;
            ...
            {
                dns_rdatasetiter_current (rdsiter, rctx->rdataset);
                ...
                else
                {
                     ...
                     * rdataset.
                     */
                    dns_rdataset_disassociate (rctx->rdataset);
                }
                tresult = dns_rdatasetiter_next (rdsiter);

                if (tresult == ISC_R_SUCCESS && rctx->rdataset == NULL)
                {
                    tresult = getrdataset (mctx, &rctx->rdataset);
                    if (tresult != ISC_R_SUCCESS)
                    {
                        result = tresult;
                        POST (result);
        ...
        if (ansname != NULL)
        {
            ...
            while ((rdataset = ISC_LIST_HEAD (ansname->list)) != NULL)
            {
        ...
        if (want_restart)
        {
            INSIST (rctx->rdataset == NULL && rctx->sigrdataset == NULL);
...
dns_client_resolve (dns_client_t * client, dns_name_t * name,
                    dns_rdataclass_t rdclass, dns_rdatatype_t type, unsigned int options, dns_namelist_t * namelist)
{
    isc_result_t result;
    ...
    if ((client->attributes & DNS_CLIENTATTR_OWNCTX) == 0 && (options & DNS_CLIENTRESOPT_ALLOWRUN) == 0)
    {
        /*
    ...
    result = dns_client_startresolve (client, name, rdclass, type, options,
                                      client->task, resolve_done, resarg, &resarg->trans);
    if (result != ISC_R_SUCCESS)
    {
        DESTROYLOCK (&resarg->lock);
        isc_mem_put (client->mctx, resarg, sizeof (*resarg));
...
dns_client_startresolve (dns_client_t * client, dns_name_t * name,
                         ...
                         isc_taskaction_t action, void *arg, dns_clientrestrans_t ** transp)
{
    dns_view_t *view = NULL;
    ...
    event = (dns_clientresevent_t *)
        isc_event_allocate (mctx, clone, DNS_EVENT_CLIENTRESDONE, action, arg, sizeof (*event));
    if (event == NULL)
    {
                          ...
                          dns_name_t * keyname, isc_buffer_t * keydatabuf)
{
    ...
    result = dst_key_fromdns (keyname, rdclass, keydatabuf, client->mctx, &dstkey);
    if (result != ISC_R_SUCCESS)
        goto cleanup;
...
static void request_done (isc_task_t * task, isc_event_t * event)
{
    ...
    {
        result = dns_request_getresponse (request, ctx->event->rmessage, ctx->parseoptions);
    }
...
dns_client_request (dns_client_t * client, dns_message_t * qmessage,
                    ...
                    dns_tsec_t * tsec, unsigned int timeout, unsigned int udptimeout, unsigned int udpretries)
{
    isc_appctx_t *actx;
    ...
    if ((client->attributes & DNS_CLIENTATTR_OWNCTX) == 0 && (options & DNS_CLIENTREQOPT_ALLOWRUN) == 0)
    {
        /*
         * If the client is run under application's control, we need
         * to create a new running (sub)environment for this
    ...
    result = dns_client_startrequest (client, qmessage, rmessage, server,
                                      ...
                                      udptimeout, udpretries, client->task, localrequest_done, reqarg, &reqarg->trans);
    if (result != ISC_R_SUCCESS)
    {
        DESTROYLOCK (&reqarg->lock);
        isc_mem_put (client->mctx, reqarg, sizeof (*reqarg));
...
dns_client_startrequest (dns_client_t * client, dns_message_t * qmessage,
                         ...
                         isc_task_t * task, isc_taskaction_t action, void *arg, dns_clientreqtrans_t ** transp)
{
    isc_result_t result;
    ...
    LOCK (&client->lock);
    result = dns_viewlist_find (&client->viewlist, DNS_CLIENTVIEW_NAME, qmessage->rdclass, &view);
    UNLOCK (&client->lock);
    if (result != ISC_R_SUCCESS)
        return (result);
    ...
    event = (dns_clientreqevent_t *)
        isc_event_allocate (client->mctx, clone, DNS_EVENT_CLIENTREQDONE, action, arg, sizeof (*event));
    if (event == NULL)
    {
        result = ISC_R_NOMEMORY;
        goto cleanup;
    ...
    result = dns_request_createvia3 (view->requestmgr, qmessage, NULL,
                                     ...
                                     timeout, udptimeout, udpretries, client->task, request_done, ctx, &ctx->request);
    if (result == ISC_R_SUCCESS)
    {
        dns_view_detach (&view);
...
static void update_done (isc_task_t * task, isc_event_t * event)
{
    ...
    result = dns_message_create (client->mctx, DNS_MESSAGE_INTENTPARSE, &answer);
    if (result != ISC_R_SUCCESS)
        goto out;
    uctx->state = dns_clientupdatestate_done;
    result = dns_request_getresponse (request, answer, DNS_MESSAGEPARSE_PRESERVEORDER);
  ...
  out:
    ...
    if (result != ISC_R_SUCCESS && !uctx->canceled && uctx->currentserver != NULL)
    {
        ...
        result = dns_request_createvia3 (uctx->view->requestmgr,
                                         ...
                                         client->update_udpretries, client->task, update_done, uctx, &uctx->updatereq);
        UNLOCK (&uctx->lock);
...
static isc_result_t send_update (updatectx_t * uctx)
{
    ...
    result = dns_request_createvia3 (uctx->view->requestmgr,
                                     ...
                                     client->update_udptimeout,
                                     client->update_udpretries, client->task, update_done, uctx, &uctx->updatereq);
    if (result == ISC_R_SUCCESS && uctx->state == dns_clientupdatestate_prepare)
    {
        uctx->state = dns_clientupdatestate_sent;
    }
...
static void resolveaddr_done (isc_task_t * task, isc_event_t * event)
{
    ...
    for (name = ISC_LIST_HEAD (rev->answerlist); name != NULL; name = ISC_LIST_NEXT (name, link))
    {
        for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
        {
            ...
            for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
            {
                ...
                sa = isc_mem_get (uctx->client->mctx, sizeof (*sa));
                if (sa == NULL)
                {
                    /*
                ...
                switch (family)
                {
                    case AF_INET:
                        ...
                        isc_sockaddr_fromin (sa, &rdata_a.in_addr, 53);
                        dns_rdata_freestruct (&rdata_a);
                        break;
                    case AF_INET6:
                        dns_rdataset_current (rdataset, &rdata);
                        dns_rdata_tostruct (&rdata, &rdata_aaaa, NULL);
                        isc_sockaddr_fromin6 (sa, &rdata_aaaa.in6_addr, 53);
...
static isc_result_t process_soa (updatectx_t * uctx, dns_rdataset_t * soaset, dns_name_t * soaname)
{
    ...
    else
    {
        ...
        result = dns_client_startresolve (uctx->client, &primary,
                                          uctx->rdclass,
                                          dns_rdatatype_a,
                                          0, uctx->client->task, resolveaddr_done, &uctx->bp4, &uctx->restrans);
        if (result == ISC_R_SUCCESS)
        {
            uctx->bp6 = uctx;
            result = dns_client_startresolve (uctx->client,
                                              ...
                                              uctx->rdclass,
                                              dns_rdatatype_aaaa,
                                              0, uctx->client->task, resolveaddr_done, &uctx->bp6, &uctx->restrans2);
        }
        UNLOCK (&uctx->lock);
    }
...
static void receive_soa (isc_task_t * task, isc_event_t * event)
{
    ...
    result = dns_message_create (uctx->client->mctx, DNS_MESSAGE_INTENTPARSE, &rcvmsg);
    if (result != ISC_R_SUCCESS)
        goto out;
    result = dns_request_getresponse (request, rcvmsg, DNS_MESSAGEPARSE_PRESERVEORDER);
    ...
    if (result == DNS_R_TSIGERRORSET)
    {
        ...
        result = dns_request_createvia3 (uctx->view->requestmgr,
                                         ...
                                         client->find_timeout, 3, uctx->client->task, receive_soa, uctx, &newrequest);
        if (result == ISC_R_SUCCESS)
        {
            LOCK (&uctx->lock);
    ...
    if (rcvmsg->rcode != dns_rcode_noerror && rcvmsg->rcode != dns_rcode_nxdomain)
    {
  ...
  lookforsoa:
    ...
    while (result == ISC_R_SUCCESS)
    {
        ...
        soaset = NULL;
        result = dns_message_findtype (name, dns_rdatatype_soa, 0, &soaset);
        if (result == ISC_R_SUCCESS)
            break;
        if (section == DNS_SECTION_ANSWER)
        {
            ...
            if (dns_message_findtype (name, dns_rdatatype_cname, 0,
                                      &tset) == ISC_R_SUCCESS
                || dns_message_findtype (name, dns_rdatatype_dname, 0, &tset) == ISC_R_SUCCESS)
            {
                seencname = ISC_TRUE;
                break;
            }
    ...
    if (droplabel)
    {
        ...
        else
        {
            ...
            result = dns_request_createvia3 (uctx->view->requestmgr,
                                             ...
                                             client->find_timeout *
                                             20,
                                             client->find_timeout, 3, client->task, receive_soa, uctx, &uctx->soareq);
        }
    }
...
static isc_result_t request_soa (updatectx_t * uctx)
{
    ...
    if (soaquery == NULL)
    {
        result = dns_message_create (uctx->client->mctx, DNS_MESSAGE_INTENTRENDER, &soaquery);
        if (result != ISC_R_SUCCESS)
            return (result);
    }
    ...
    result = dns_request_createvia3 (uctx->view->requestmgr,
                                     ...
                                     uctx->client->task, receive_soa, uctx, &uctx->soareq);
    if (result == ISC_R_SUCCESS)
    {
...
static void resolvesoa_done (isc_task_t * task, isc_event_t * event)
{
    ...
    for (name = ISC_LIST_HEAD (rev->answerlist); name != NULL; name = ISC_LIST_NEXT (name, link))
    {
        for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
        {
            if (dns_rdataset_isassociated (rdataset) && rdataset->type == dns_rdatatype_soa)
                break;
        }
    }
    ...
    if (rdataset == NULL)
    {
        ...
        dns_name_init (&tname, NULL);
        dns_name_getlabelsequence (&uctx->soaqname, 1, nlabels - 1, &tname);
        dns_name_clone (&tname, &uctx->soaqname);

        result = dns_client_startresolve (uctx->client, &uctx->soaqname,
                                          uctx->rdclass,
                                          dns_rdatatype_soa, 0,
                                          uctx->client->task, resolvesoa_done, uctx, &uctx->restrans);
    }
    else
...
static isc_result_t copy_name (isc_mem_t * mctx, dns_message_t * msg, dns_name_t * name, dns_name_t ** newnamep)
{
    ...
    for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
    {
        ...
        while (result == ISC_R_SUCCESS)
        {
            ...
            dns_rdata_fromregion (newrdata, rdata.rdclass, rdata.type, &r);
...
dns_client_update (dns_client_t * client, dns_rdataclass_t rdclass,
                   ...
                   dns_namelist_t * updates, isc_sockaddrlist_t * servers, dns_tsec_t * tsec, unsigned int options)
{
    isc_result_t result;
    ...
    result = dns_client_startupdate (client, rdclass, zonename,
                                     ...
                                     tsec, options, client->task, internal_update_callback, uarg, &uarg->trans);
    if (result != ISC_R_SUCCESS)
    {
        DESTROYLOCK (&uarg->lock);
        isc_mem_put (client->mctx, uarg, sizeof (*uarg));
...
dns_client_startupdate (dns_client_t * client, dns_rdataclass_t rdclass,
                        ...
                        isc_task_t * task, isc_taskaction_t action, void *arg, dns_clientupdatetrans_t ** transp)
{
    dns_view_t *view = NULL;
    ...
    uctx->event = (dns_clientupdateevent_t *)
        isc_event_allocate (client->mctx, clone, DNS_EVENT_UPDATEDONE, action, arg, sizeof (*uctx->event));
    if (uctx->event == NULL)
        goto fail;
    if (zonename != NULL)
    {
    ...
    if (servers != NULL)
    {
        for (server = ISC_LIST_HEAD (*servers); server != NULL; server = ISC_LIST_NEXT (server, link))
        {
    ...
    /* Make update message */
    result = dns_message_create (client->mctx, DNS_MESSAGE_INTENTRENDER, &uctx->updatemsg);
    if (result != ISC_R_SUCCESS)
        goto fail;
    ...
    {
        for (name = ISC_LIST_HEAD (*prerequisites); name != NULL; name = ISC_LIST_NEXT (name, link))
        {
            newname = NULL;
            result = copy_name (client->mctx, uctx->updatemsg, name, &newname);
            if (result != ISC_R_SUCCESS)
                goto fail;
            dns_message_addname (uctx->updatemsg, newname, DNS_SECTION_PREREQUISITE);
        }
    }

    for (name = ISC_LIST_HEAD (*updates); name != NULL; name = ISC_LIST_NEXT (name, link))
    {
        newname = NULL;
        result = copy_name (client->mctx, uctx->updatemsg, name, &newname);
        if (result != ISC_R_SUCCESS)
            goto fail;
        dns_message_addname (uctx->updatemsg, newname, DNS_SECTION_UPDATE);
    }
    ...
    else
    {
        ...
        result = dns_client_startresolve (uctx->client, &uctx->soaqname,
                                          uctx->rdclass,
                                          dns_rdatatype_soa, 0, client->task, resolvesoa_done, uctx, &uctx->restrans);
        if (result != ISC_R_SUCCESS)
            goto fail;
    }
...
void dns_client_destroyupdatetrans (dns_clientupdatetrans_t ** transp)
{
    ...
    REQUIRE (uctx->updatereq == NULL && uctx->updatemsg == NULL &&
             ...
             uctx->event == NULL && uctx->tsigkey == NULL && uctx->sig0key == NULL);
...
dns_client_updaterec (dns_client_updateop_t op, dns_name_t * owner,
                      ...
                      dns_rdataset_t * rdataset, dns_rdatalist_t * rdatalist, dns_rdata_t * rdata, isc_mem_t * mctx)
{
    ...
    REQUIRE ((rdataset != NULL && rdatalist != NULL && rdata != NULL) ||
             (rdataset == NULL && rdatalist == NULL && rdata == NULL && mctx != NULL));
    if (op == updateop_add)
        REQUIRE (source != NULL);
    if (source != NULL)
    {
        REQUIRE (source->type == type);
        REQUIRE (op == updateop_add || op == updateop_delete || op == updateop_exist);
    }
    ...
    if (rdataset == NULL)
    {
        ...
        isc_buffer_init (&updaterec->buffer, updaterec->data, size - offsetof (dns_client_updaterec_t, data));
...
void dns_client_freeupdate (dns_name_t ** namep)
{
    ...
    name = *namep;
    for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_HEAD (name->list))
    {
        ISC_LIST_UNLINK (name->list, rdataset, link);
        rdatalist = NULL;
        ...
        for (rdata = ISC_LIST_HEAD (rdatalist->rdata); rdata != NULL; rdata = ISC_LIST_HEAD (rdatalist->rdata))
    ...
    if ((name->attributes & DNS_NAMEATTR_HASUPDATEREC) != 0)
    {
        ...
        isc_mem_putanddetach (&updaterec->mctx, updaterec, updaterec->size);
        *namep = NULL;
    }
}
