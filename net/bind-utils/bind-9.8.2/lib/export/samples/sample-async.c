...
static void process_answer (isc_task_t * task, isc_event_t * event)
{
    ...
    if (rev->result != ISC_R_SUCCESS)
        printf ("  failed: %d(%s)\n", rev->result, dns_result_totext (rev->result));
...
{
    fprintf (stderr, "usage: sample-async [-s server_address] [-t RR type] " "input_file\n");
...
int main (int argc, char *argv[])
{
    ...
    result = dns_client_createx (mctx, query_actx, taskmgr, socketmgr, timermgr, 0, &client);
    if (result != ISC_R_SUCCESS)
    {
