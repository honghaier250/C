...
static isc_result_t make_querymessage (dns_message_t * message, const char *namestr, dns_rdatatype_t rdtype)
{
...
static void print_section (dns_message_t * message, int section, isc_buffer_t * buf)
{
    isc_result_t result;

    isc_region_t r;

    result = dns_message_sectiontotext (message, section, &dns_master_style_full, 0, buf);
    if (result != ISC_R_SUCCESS)
        goto fail;
...
int main (int argc, char *argv[])
{
    ...
    if (result == ISC_R_SUCCESS)
    {
        result = dns_message_create (mctx, DNS_MESSAGE_INTENTPARSE, &rmessage);
    }
    if (result != ISC_R_SUCCESS)
    {
    ...
    result = dns_client_request (client, qmessage, rmessage, &sa, 0, 0, NULL, 60, 0, 3);
    if (result != ISC_R_SUCCESS)
    {
