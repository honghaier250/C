...
static ISC_LIST (dns_rdatalist_t) usedrdatalists;
     ...
     static void update_addordelete (isc_mem_t * mctx, char *cmdline, isc_boolean_t isdelete, dns_name_t * name);
     ...
     static void usage (void)
{
    fprintf (stderr, "sample-update "
             "[-a auth_server] "
             "[-k keyfile] "
             "[-p prerequisite] " "[-r recursive_server] " "[-z zonename] " "(add|delete) \"name TTL RRtype RDATA\"\n");
    exit (1);
}
...
int main (int argc, char *argv[])
{
    ...
    {
        fprintf (stderr, "authoritative or recursive server " "must be specified\n");
        usage ();
    }
    ...
    if (recursive_server != NULL)
    {
        ...
        result = dns_client_setservers (client, dns_rdataclass_in, NULL, &rec_servers);
        if (result != ISC_R_SUCCESS)
        {
    ...
    result = dns_client_update (client, default_rdataclass,    /* XXX: fixed */
                                zname, prereqlistp, &updatelist, (auth_server == NULL) ? NULL : &auth_servers, tsec, 0);
    if (result != ISC_R_SUCCESS)
    {
...
static void parse_name (char **cmdlinep, dns_name_t * name)
{
    isc_result_t result;
             ...
             dns_rdatatype_t rdatatype, dns_rdata_t * rdata)
{
    ...
    while (cmdline != NULL && *cmdline != 0 && isspace ((unsigned char) *cmdline))
        cmdline++;
    ...
    if (cmdline != NULL && *cmdline != 0)
    {
        ...
        result = dns_rdata_fromtext (rdata, rdataclass, rdatatype, lex, dns_rootname, 0, mctx, buf, &callbacks);
        isc_lex_destroy (&lex);
        if (result == ISC_R_SUCCESS)
        {
...
static void
make_prereq (isc_mem_t * mctx, char *cmdline, isc_boolean_t ispositive, isc_boolean_t isrrset, dns_name_t * name)
{
    isc_result_t result;
