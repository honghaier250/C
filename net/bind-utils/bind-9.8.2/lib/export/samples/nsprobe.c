...
static isc_result_t print_name (dns_name_t * name)
{
    isc_result_t result;
...
static isc_result_t print_address (FILE * fp, isc_sockaddr_t * addr)
{
    char buf[NI_MAXHOST];

    if (getnameinfo (&addr->type.sa, addr->length, buf, sizeof (buf), NULL, 0, NI_NUMERICHOST) == 0)
    {
        fprintf (fp, "%s", buf);
    }
    else
...
ctxs_destroy (isc_mem_t ** mctxp, isc_appctx_t ** actxp,
              isc_taskmgr_t ** taskmgrp, isc_socketmgr_t ** socketmgrp, isc_timermgr_t ** timermgrp)
{
    if (*taskmgrp != NULL)
 ...
 */
static isc_result_t make_querymessage (dns_message_t * message, dns_name_t * qname0, dns_rdatatype_t rdtype)
{
 ...
 * Update statistics
 */
static inline void increment_entry (unsigned long *entryp)
{
    (*entryp)++;
    INSIST (*entryp != 0);        /* check overflow */
}

static void update_stat (struct probe_trans *trans)
{
    struct probe_ns *pns;
    ...
    for (pns = ISC_LIST_HEAD (trans->nslist); pns != NULL; pns = ISC_LIST_NEXT (pns, link))
    {
        for (server = ISC_LIST_HEAD (pns->servers); server != NULL; server = ISC_LIST_NEXT (server, link))
        {
            increment_entry (&number_of_servers);

            if (server->result_aaaa == exist || server->result_aaaa == notype)
            {
                /*
            ...
            if (verbose_level > 1 ||
                (verbose_level == 1 && strcmp (stattype, "valid") != 0 && strcmp (stattype, "unknown") != 0))
            {
                print_name (pns->name);
                putchar ('(');
                print_address (stdout, &server->address);
                printf (") for %s:%s\n", trans->domain, stattype);
            }
        }
    }
...
static isc_result_t set_nextqname (struct probe_trans *trans)
{
    isc_result_t result;
...
static void request_done (isc_task_t * task, isc_event_t * event)
{
    ...
    if (rev->result == ISC_R_SUCCESS)
    {
        ...
        {
            result = dns_message_firstname (rmessage, DNS_SECTION_ANSWER);
            while (result == ISC_R_SUCCESS)
            {
                name = NULL;
                dns_message_currentname (rmessage, DNS_SECTION_ANSWER, &name);
                for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
                {
                    (void) print_rdataset (rdataset, name);

                    if (rdataset->type == dns_rdatatype_cname || rdataset->type == dns_rdatatype_dname)
                    {
    ...
    else if (rev->result == DNS_R_RECOVERABLE || rev->result == DNS_R_BADLABELTYPE)
    {
        ...
        *resultp = brokenanswer;
        ...
        if (rmessage->counts[DNS_SECTION_ANSWER] > 0)
        {
            ...
            while (result == ISC_R_SUCCESS)
            {
                ...
                for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
                {
                    if (rdataset->type == dns_rdatatype_cname && dns_rdataset_count (rdataset) > 1)
                    {
        ...
        if (rmessage->counts[DNS_SECTION_AUTHORITY] > 0)
        {
            ...
            while (result == ISC_R_SUCCESS)
            {
                 ...
                 */
                name = NULL;
                dns_message_currentname (rmessage, DNS_SECTION_AUTHORITY, &name);
                for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
                {
                    if (rdataset->type == dns_rdatatype_soa && dns_rdataset_count (rdataset) > 1)
                    {
                        *resultp = multiplesoa;
                        goto found;
                    }
                }
                result = dns_message_nextname (rmessage, DNS_SECTION_AUTHORITY);
            }
        }
    }
    ...
    {
        fprintf (stderr, "unexpected result: %d (domain=%s, server=", rev->result, trans->domain);
  ...
  found:
    ...
    if (result == ISC_R_NOMORE)
    {
        ...
        else if (type == dns_rdatatype_a)
        {
            ...
            {
                trans->current_ns = ISC_LIST_HEAD (trans->nslist);
                for (pns = trans->current_ns; pns != NULL; pns = ISC_LIST_NEXT (pns, link))
                {
                    for (server = ISC_LIST_HEAD (pns->servers); server != NULL; server = ISC_LIST_NEXT (server, link))
                    {
                        INSIST (server->result_aaaa == none);
...
static isc_result_t probe_name (struct probe_trans *trans, dns_rdatatype_t type)
{
    ...
    for (pns = trans->current_ns; pns != NULL; pns = ISC_LIST_NEXT (pns, link))
    {
        for (server = ISC_LIST_HEAD (pns->servers); server != NULL; server = ISC_LIST_NEXT (server, link))
        {
            if ((type == dns_rdatatype_a &&
                 server->result_a == none) || (type == dns_rdatatype_aaaa && server->result_aaaa == none))
            {
  ...
  found:
    ...
    result = dns_client_startrequest (client, trans->qmessage,
                                      ...
                                      NULL, 120, 0, 4, probe_task, request_done, trans, &trans->reqid);
...
static void resolve_nsaddress (isc_task_t * task, isc_event_t * event)
{
    struct probe_trans *trans = event->ev_arg;
    ...
    for (name = ISC_LIST_HEAD (rev->answerlist); name != NULL; name = ISC_LIST_NEXT (name, link))
    {
        for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
        {
            ...
            for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
            {
                ...
                if (server == NULL)
                {
                    fprintf (stderr, "resolve_nsaddress: " "mem_get failed");
                    result = ISC_R_NOMEMORY;
                    POST (result);
                    goto cleanup;
                }
                isc_sockaddr_fromin (&server->address, &rdata_a.in_addr, 53);
...
static void resolve_ns (isc_task_t * task, isc_event_t * event)
{
    struct probe_trans *trans = event->ev_arg;
    ...
    for (name = ISC_LIST_HEAD (rev->answerlist); name != NULL; name = ISC_LIST_NEXT (name, link))
    {
        for (rdataset = ISC_LIST_HEAD (name->list); rdataset != NULL; rdataset = ISC_LIST_NEXT (rdataset, link))
        {
            ...
            for (result = dns_rdataset_first (rdataset); result == ISC_R_SUCCESS; result = dns_rdataset_next (rdataset))
            {
                ...
                if (pns == NULL)
                {
                    fprintf (stderr, "resolve_ns: mem_get failed");
                    result = ISC_R_NOMEMORY;
                    POST (result);
                    /*
                ...
                pns->name = dns_fixedname_name (&pns->fixedname);
...
{
    fprintf (stderr, "usage: nsprobe [-d] [-v [-v...]] [-c cache_address] " "[input_file]\n");
...
int main (int argc, char *argv[])
{
    ...
    result = dns_client_createx (mctx, actx, taskmgr, socketmgr, timermgr, 0, &client);
    if (result != ISC_R_SUCCESS)
    {
    ...
    {
        fprintf (stderr, "assumption failure: addrlen is too long: %ld\n", (long) res->ai_addrlen);
        exit (1);
    }
    ...
    result = dns_client_setservers (client, dns_rdataclass_in, NULL, &servers);
    if (result != ISC_R_SUCCESS)
    {
    ...
    for (i = 0; i < MAX_PROBES; i++)
    {
        ...
        result = dns_message_create (mctx, DNS_MESSAGE_INTENTRENDER, &probes[i].qmessage);
        if (result == ISC_R_SUCCESS)
        {
            result = dns_message_create (mctx, DNS_MESSAGE_INTENTPARSE, &probes[i].rmessage);
        }
        if (result != ISC_R_SUCCESS)
        {
    ...
    printf ("  valid: %lu\n"
            ...
            server_stat.multiplecname, server_stat.brokenanswer, server_stat.lame, server_stat.unknown);
