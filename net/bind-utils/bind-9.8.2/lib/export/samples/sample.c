...
static isc_result_t printdata (dns_rdataset_t * rdataset, dns_name_t * owner)
{
    isc_buffer_t target;
     ...
     static void usage (void)
{
    fprintf (stderr, "sample [-t RRtype] "
             ...
             "[-s domain:serveraddr_for_domain ] " "server_address hostname\n");

    exit (1);
}

static void set_key (dns_client_t * client, char *keynamestr, char *keystr, isc_boolean_t is_sep, isc_mem_t ** mctxp)
{
    isc_result_t result;
    ...
    result = dns_rdata_fromstruct (NULL, keystruct.common.rdclass, keystruct.common.rdtype, &keystruct, &rrdatabuf);
    if (result != ISC_R_SUCCESS)
    {
    ...
    result = dns_client_addtrustedkey (client, dns_rdataclass_in, keyname, &rrdatabuf);
    if (result != ISC_R_SUCCESS)
    {
...
static void addserver (dns_client_t * client, const char *addrstr, const char *namespace)
{
    ...
    result = dns_client_setservers (client, dns_rdataclass_in, name, &servers);
    if (result != ISC_R_SUCCESS)
    {
...
int main (int argc, char *argv[])
{
    ...
    while ((ch = getopt (argc, argv, "a:es:t:k:K:")) != -1)
    {
        switch (ch)
        {
            ...
            case 's':
                if (altserver != NULL)
                {
                    fprintf (stderr, "alternate server " "already defined: %s\n", altserver);
                    exit (1);
                }
                altserver = optarg;
                break;
    ...
    result = dns_client_resolve (client, qname, dns_rdataclass_in, type, resopt, &namelist);
    if (result != ISC_R_SUCCESS)
    {
        fprintf (stderr, "resolution failed: %s\n", dns_result_totext (result));
    }
    for (name = ISC_LIST_HEAD (namelist); name != NULL; name = ISC_LIST_NEXT (name, link))
    {
