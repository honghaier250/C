...
static struct hostent *hostfromaddr (lwres_gnbaresponse_t * addr, int af, const void *src);
...
struct hostent *lwres_getipnodebyname (const char *name, int af, int flags, int *error_num)
{
    ...
    if ((af == AF_INET6 && (flags & AI_V4MAPPED) == 0 && v4 == 1) ||
        ...
        (have_v6 == 0 && af == AF_INET6 && (((flags & AI_V4MAPPED) != 0 && have_v4) || (flags & AI_V4MAPPED) == 0)))
    {
    ...
    if (v4 == 1 || v6 == 1)
    {
        ...
        if (v4 == 1 && af == AF_INET6)
        {
            ...
            lwres_net_ntop (AF_INET, (char *) &in4,
                            mappedname + sizeof ("::ffff:") - 1, sizeof (mappedname) - sizeof ("::ffff:") + 1);
    ...
    if (have_v4 &&
        ((af == AF_INET) || (af == AF_INET6 && (flags & AI_V4MAPPED) != 0 && (he1 == NULL || (flags & AI_ALL) != 0))))
    {
...
struct hostent *lwres_getipnodebyaddr (const void *src, size_t len, int af, int *error_num)
{
    ...
    if ((af == AF_INET6 && IN6_IS_ADDR_V4COMPAT (u.in6)) ||
        (af == AF_INET6 && IN6_IS_ADDR_V4MAPPED (u.in6)) || (af == AF_INET))
    {
        ...
        if (n == LWRES_R_SUCCESS)
            (void) lwres_conf_parse (lwrctx, lwres_resolv_conf);
        if (n == LWRES_R_SUCCESS)
            n = lwres_getnamebyaddr (lwrctx, LWRES_ADDRTYPE_V4, INADDRSZ, cp, &by);
        if (n != LWRES_R_SUCCESS)
        {
            lwres_conf_clear (lwrctx);
            lwres_context_destroy (&lwrctx);
    ...
    if (n == LWRES_R_SUCCESS)
        (void) lwres_conf_parse (lwrctx, lwres_resolv_conf);
    if (n == LWRES_R_SUCCESS)
        n = lwres_getnamebyaddr (lwrctx, LWRES_ADDRTYPE_V6, IN6ADDRSZ, src, &by);
    if (n != 0)
    {
        lwres_conf_clear (lwrctx);
        lwres_context_destroy (&lwrctx);
...
static int scan_interfaces6 (int *have_v4, int *have_v6)
{
    struct LIFCONF lifc;
    ...
    *have_v4 = *have_v6 = 0;
    ...
    for (cp = buf; (*have_v4 == 0 || *have_v6 == 0) && cp < cplim; cp += cpsize)
    {
        ...
        if (lifreq.lifr_addr.sa_len > sizeof (struct sockaddr))
            cpsize += (int) lifreq.lifr_addr.sa_len - (int) (sizeof (struct sockaddr));
#else
        ...
        switch (lifreq.lifr_addr.ss_family)
        {
            case AF_INET:
                if (*have_v4 == 0)
                {
                    memcpy (&in4, &((struct sockaddr_in *) &lifreq.lifr_addr)->sin_addr, sizeof (in4));
            ...
            case AF_INET6:
                if (*have_v6 == 0)
                {
                    memcpy (&in6, &((struct sockaddr_in6 *) &lifreq.lifr_addr)->sin6_addr, sizeof (in6));
...
static int scan_interfaces (int *have_v4, int *have_v6)
{
    ...
    *have_v4 = *have_v6 = 0;
    ...
    for (cp = buf; (*have_v4 == 0 || *have_v6 == 0) && cp < cplim; cp += cpsize)
    {
        ...
        if (u.ifreq.ifr_addr.sa_len > sizeof (struct sockaddr))
            cpsize += (int) u.ifreq.ifr_addr.sa_len - (int) (sizeof (struct sockaddr));
#else
        ...
        switch (u.ifreq.ifr_addr.sa_family)
        {
            case AF_INET:
                if (*have_v4 == 0)
                {
                    memcpy (&in4, &((struct sockaddr_in *) &u.ifreq.ifr_addr)->sin_addr, sizeof (in4));
                    if (in4.s_addr == INADDR_ANY)
                        break;
            ...
            case AF_INET6:
                if (*have_v6 == 0)
                {
                    memcpy (&in6, &((struct sockaddr_in6 *) &u.ifreq.ifr_addr)->sin6_addr, sizeof (in6));
                    if (memcmp (&in6, &in6addr_any, sizeof (in6)) == 0)
                        break;
                    n = ioctl (s, SIOCGIFFLAGS, (char *) &u.ifreq);
                    if (n < 0)
...
static struct hostent *copyandmerge (struct hostent *he1, struct hostent *he2, int af, int *error_num)
{
     ...
     * Copy hostname.
     */
    he->h_name = malloc (strlen ((he1 != NULL) ? he1->h_name : he2->h_name) + 1);
    if (he->h_name == NULL)
        goto cleanup2;
