...
lwres_context_create (lwres_context_t ** contextp, void *arg,
                      lwres_malloc_t malloc_function, lwres_free_t free_function, unsigned int flags)
{
    lwres_context_t *ctx;
    ...
    ctx->use_ipv4 = 1;
    ctx->use_ipv6 = 1;
    if ((flags & (LWRES_CONTEXT_USEIPV4 | LWRES_CONTEXT_USEIPV6)) == LWRES_CONTEXT_USEIPV6)
    {
        ctx->use_ipv4 = 0;
    }
    if ((flags & (LWRES_CONTEXT_USEIPV4 | LWRES_CONTEXT_USEIPV6)) == LWRES_CONTEXT_USEIPV4)
    {
        ctx->use_ipv6 = 0;
    }
 ...
 */
void lwres_context_destroy (lwres_context_t ** contextp)
{
    lwres_context_t *ctx;
...
static void *lwres_malloc (void *arg, size_t len)
{
    void *mem;
...
static void lwres_free (void *arg, void *mem, size_t len)
{
    UNUSED (arg);
...
static lwres_result_t context_connect (lwres_context_t * ctx)
{
    int s;
    ...
    {
        memcpy (&ctx->address, &ctx->confdata.lwservers[0], sizeof (lwres_addr_t));
    ...
    {
        memcpy (&sin.sin_addr, ctx->address.address, sizeof (sin.sin_addr));
    ...
    {
        memcpy (&sin6.sin6_addr, ctx->address.address, sizeof (sin6.sin6_addr));
    ...
    if (ctx->address.family == LWRES_ADDRTYPE_V4)
    {
        if (fromlen != sizeof (sin)
            || memcmp (&sin.sin_addr, ctx->address.address,
                       sizeof (sin.sin_addr)) != 0 || sin.sin_port != htons (lwres_udp_port))
    ...
    else
    {
        if (fromlen != sizeof (sin6)
            || memcmp (&sin6.sin6_addr, ctx->address.address,
                       sizeof (sin6.sin6_addr)) != 0 || sin6.sin6_port != htons (lwres_udp_port))
            return (LWRES_R_RETRY);
    }
