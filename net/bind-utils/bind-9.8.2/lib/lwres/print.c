...
int lwres__print_sprintf (char *str, const char *format, ...)
{
    va_list ap;
...
int lwres__print_snprintf (char *str, size_t size, const char *format, ...)
{
    va_list ap;
...
int lwres__print_vsnprintf (char *str, size_t size, const char *format, va_list ap)
{
    int h;
    ...
    while (*format != '\0')
    {
        ...
        switch (*format)
        {
              ...
              doint:
                ...
                switch (*format)
                {
                    ...
                    case 'd':
                        ...
                        sprintf (buf, "%" LWRES_PRINT_QUADFORMAT "u", tmpui);
                        goto printint;
                    case 'o':
                        if (q)
                            tmpui = va_arg (ap, unsigned long long int);
                        ...
                        sprintf (buf, alt ? "%#" LWRES_PRINT_QUADFORMAT "o" : "%" LWRES_PRINT_QUADFORMAT "o", tmpui);
                        goto printint;
                    case 'u':
                        if (q)
                            tmpui = va_arg (ap, unsigned long long int);
                    ...
                    case 'x':
                        ...
                        sprintf (buf, "%" LWRES_PRINT_QUADFORMAT "x", tmpui);
                        goto printint;
                    case 'X':
                        if (q)
                            tmpui = va_arg (ap, unsigned long long int);
                      ...
                      printint:
                        if (precision != 0U || width != 0U)
                        {
                            ...
                            if (width != 0U)
                            {
                                pad = width - length - zeropad - strlen (head);
                                if (pad < 0)
                                    pad = 0;
                            }
                        }
                        count += strlen (head) + strlen (buf) + pad + zeropad;
                        if (!left)
                        {
                            while (pad > 0 && size > 1U)
                            {
            ...
            case 's':
                ...
                if (precision != 0U)
                    while (precision > 0U && *cp != '\0' && size > 1U)
                    {
                        *str++ = *cp++;
                        size--;
                        precision--;
            ...
            case 'p':
                ...
                cp = buf;
                if (zeropad > 0 && buf[0] == '0' && (buf[1] == 'x' || buf[1] == 'X'))
                {
                    if (size > 1U)
                    {
                        *str++ = *cp++;
            ...
            case 'G':
                ...
                sprintf (fmt, "%%%s%s.%lu%s%c", alt ? "#" : "",
                         plus ? "+" : space ? " " : "", precision, l ? "L" : "", *format);
                switch (*format)
                {
                    case 'e':
                    case 'E':
