...
static lwres_result_t lwres_conf_parsenameserver (lwres_context_t * ctx, FILE * fp);

static lwres_result_t lwres_conf_parselwserver (lwres_context_t * ctx, FILE * fp);

static lwres_result_t lwres_conf_parsedomain (lwres_context_t * ctx, FILE * fp);

static lwres_result_t lwres_conf_parsesearch (lwres_context_t * ctx, FILE * fp);

static lwres_result_t lwres_conf_parsesortlist (lwres_context_t * ctx, FILE * fp);

static lwres_result_t lwres_conf_parseoption (lwres_context_t * ctx, FILE * fp);

static void lwres_resetaddr (lwres_addr_t * addr);

static lwres_result_t lwres_create_addr (const char *buff, lwres_addr_t * addr, int convert_zero);
 ...
 */
static int getword (FILE * fp, char *buffer, size_t size)
{
    int ch;
...
static char *lwres_strdup (lwres_context_t * ctx, const char *str)
{
    char *p;
...
void lwres_conf_clear (lwres_context_t * ctx)
{
    ...
    {
        CTXFREE (confdata->domainname, strlen (confdata->domainname) + 1);
        confdata->domainname = NULL;
    }
    ...
    for (i = 0; i < confdata->searchnxt; i++)
    {
        ...
        {
            CTXFREE (confdata->search[i], strlen (confdata->search[i]) + 1);
            confdata->search[i] = NULL;
        }
    }
...
static lwres_result_t lwres_conf_parselwserver (lwres_context_t * ctx, FILE * fp)
{
    ...
    res = lwres_create_addr (word, &confdata->lwservers[confdata->lwnext++], 1);
    if (res != LWRES_R_SUCCESS)
        return (res);
...
static lwres_result_t lwres_conf_parsesearch (lwres_context_t * ctx, FILE * fp)
{
    ...
    for (idx = 0; idx < LWRES_CONFMAXSEARCH; idx++)
    {
        ...
        {
            CTXFREE (confdata->search[idx], strlen (confdata->search[idx]) + 1);
...
static lwres_result_t lwres_create_addr (const char *buffer, lwres_addr_t * addr, int convert_zero)
{
    struct in_addr v4;
...
static lwres_result_t lwres_conf_parsesortlist (lwres_context_t * ctx, FILE * fp)
{
    ...
    while (strlen (word) > 0U)
    {
        ...
        if (p != NULL)
        {
            res = lwres_create_addr (p, &confdata->sortlist[idx].mask, 0);
            if (res != LWRES_R_SUCCESS)
                return (res);
        }
        else
        {
             ...
             * Make up a mask.
             */
            confdata->sortlist[idx].mask = confdata->sortlist[idx].addr;

            memset (&confdata->sortlist[idx].mask.address, 0xff, confdata->sortlist[idx].addr.length);
        }
...
lwres_result_t lwres_conf_print (lwres_context_t * ctx, FILE * fp)
{
    ...
    for (i = 0; i < confdata->nsnext; i++)
    {
        ...
        p = lwres_net_ntop (af, confdata->nameservers[i].address, tmp, sizeof (tmp));
        if (p != tmp)
            return (LWRES_R_FAILURE);
    ...
    for (i = 0; i < confdata->lwnext; i++)
    {
        ...
        p = lwres_net_ntop (af, confdata->lwservers[i].address, tmp, sizeof (tmp));
        if (p != tmp)
            return (LWRES_R_FAILURE);
    ...
    if (confdata->sortlistnxt > 0)
    {
        ...
        for (i = 0; i < confdata->sortlistnxt; i++)
        {
            ...
            p = lwres_net_ntop (af, confdata->sortlist[i].addr.address, tmp, sizeof (tmp));
            if (p != tmp)
                return (LWRES_R_FAILURE);
            ...
            if (memcmp (&tmpaddr.address, confdata->sortlist[i].mask.address, confdata->sortlist[i].mask.length) != 0)
            {
                af = lwresaddr2af (confdata->sortlist[i].mask.family);
                p = lwres_net_ntop (af, confdata->sortlist[i].mask.address, tmp, sizeof (tmp));
                if (p != tmp)
                    return (LWRES_R_FAILURE);
