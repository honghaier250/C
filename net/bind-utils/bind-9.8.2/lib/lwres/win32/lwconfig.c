...
void get_win32_searchlist (lwres_context_t * ctx)
{
    HKEY hKey;
    ...
    if (RegOpenKeyEx (HKEY_LOCAL_MACHINE, TCPIP_SUBKEY, 0, KEY_READ, &hKey) != ERROR_SUCCESS)
        keyFound = FALSE;

    if (keyFound == TRUE)
    {
        /* Get the named directory */
        if (RegQueryValueEx (hKey, "SearchList", NULL, NULL, (LPBYTE) searchlist, &searchlen) != ERROR_SUCCESS)
            keyFound = FALSE;
        RegCloseKey (hKey);
    }
...
lwres_result_t lwres_conf_parse (lwres_context_t * ctx, const char *filename)
{
    ...
    if ((ret != LWRES_R_NOTFOUND && ret != LWRES_R_SUCCESS) || (ret == LWRES_R_SUCCESS && confdata->nsnext > 0))
        return (ret);
    ...
    while (pIPAddr)
    {
        ...
        res = lwres_create_addr (pIPAddr->IpAddress.String, &confdata->nameservers[confdata->nsnext++], 1);
