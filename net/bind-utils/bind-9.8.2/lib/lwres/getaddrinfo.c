...
int lwres_getaddrinfo (const char *hostname, const char *servname, const struct addrinfo *hints, struct addrinfo **res)
{
    ...
    if (hostname != NULL && (family == 0 || (flags & AI_NUMERICHOST) != 0))
    {
        ...
        if (strchr (hostname, '%') != NULL)
        {
            ...
            if (p != NULL)
                scopeid = (lwres_uint32_t) strtoul (p + 1, &ep, 10);
        ...
        if (lwres_net_pton (AF_INET, hostname, (struct in_addr *) abuf) == 1)
        {
            if (family == AF_INET6)
            {
                /*
        ...
        else if (ntmp[0] != '\0' && lwres_net_pton (AF_INET6, ntmp, abuf) == 1)
        {
...
#endif
                if (lwres_getnameinfo (ai->ai_addr, ai->ai_addrlen, nbuf, sizeof (nbuf), NULL, 0, NI_NUMERICHOST) == 0)
                {
...
static char *lwres_strsep (char **stringp, const char *delim)
{
    char *string = *stringp;
...
static int add_ipv4 (const char *hostname, int flags, struct addrinfo **aip, int socktype, int port)
{
    ...
    {
        lwres = lwres_getaddrsbyname (lwrctx, hostname, LWRES_ADDRTYPE_V4, &by);
        if (lwres != LWRES_R_SUCCESS)
        {
        ...
        while (addr != NULL)
        {
            ...
            *aip = ai;
            ai->ai_socktype = socktype;
            SIN (ai->ai_addr)->sin_port = port;
            memcpy (&SIN (ai->ai_addr)->sin_addr, addr->address, 4);
            if (flags & AI_CANONNAME)
            {
...
static int add_ipv6 (const char *hostname, int flags, struct addrinfo **aip, int socktype, int port)
{
    ...
    {
        lwres = lwres_getaddrsbyname (lwrctx, hostname, LWRES_ADDRTYPE_V6, &by);
        if (lwres != LWRES_R_SUCCESS)
        {
            if (lwres == LWRES_R_NOTFOUND)
                goto cleanup;
        ...
        while (addr != NULL)
        {
            ...
            *aip = ai;
            ai->ai_socktype = socktype;
            SIN6 (ai->ai_addr)->sin6_port = port;
            memcpy (&SIN6 (ai->ai_addr)->sin6_addr, addr->address, 16);
            if (flags & AI_CANONNAME)
            {
                ai->ai_canonname = strdup (by->realname);
                if (ai->ai_canonname == NULL)
