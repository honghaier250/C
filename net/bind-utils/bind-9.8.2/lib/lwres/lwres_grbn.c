                          ...
                          lwres_lwpacket_t * pkt, lwres_grbnresponse_t ** structp)
{
    ...
    {
        ret = lwres_data_parse (b, &grbn->rdatas[x], &grbn->rdatalen[x]);
        if (ret != LWRES_R_SUCCESS)
            goto out;
    }
  ...
  out:
    if (grbn != NULL)
    {
        ...
        if (grbn->rdatalen != NULL)
            CTXFREE (grbn->rdatalen, sizeof (lwres_uint16_t) * nrdatas);
        if (grbn->sigs != NULL)
...
void lwres_grbnresponse_free (lwres_context_t * ctx, lwres_grbnresponse_t ** structp)
{
    ...
    *structp = NULL;
    ...
    if (grbn->nrdatas > 0)
    {
        ...
        CTXFREE (grbn->rdatalen, sizeof (lwres_uint16_t) * grbn->nrdatas);
    }
    if (grbn->nsigs > 0)
    {
