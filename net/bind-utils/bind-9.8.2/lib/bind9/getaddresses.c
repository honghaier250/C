...
isc_result_t
bind9_getaddresses (const char *hostname, in_port_t port, isc_sockaddr_t * addrs, int addrsize, int *addrcount)
{
    struct in_addr in4;
    ...
    else if (strlen (hostname) <= 127U)
    {
        ...
        if (inet_pton (AF_INET6, tmpbuf, &in6) == 1)
        {
            ...
            if (d != NULL)
            {
                ...
                result = isc_netscope_pton (AF_INET6, d + 1, &in6, &zone);
            ...
            isc_sockaddr_fromnetaddr (&addrs[0], (const isc_netaddr_t *) &na, port);
  ...
  again:
    ...
    for (tmpai = ai, i = 0; tmpai != NULL && i < addrsize; tmpai = tmpai->ai_next)
    {
