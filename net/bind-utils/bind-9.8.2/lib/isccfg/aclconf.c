...
isc_result_t cfg_aclconfctx_create (isc_mem_t * mctx, cfg_aclconfctx_t ** ret)
{
    isc_result_t result;
...
void cfg_aclconfctx_detach (cfg_aclconfctx_t ** actxp)
{
    ...
    if (refs == 0)
    {
        for (dacl = ISC_LIST_HEAD (actx->named_acl_cache); dacl != NULL; dacl = next)
        {
            next = ISC_LIST_NEXT (dacl, nextincache);
            ISC_LIST_UNLINK (actx->named_acl_cache, dacl, nextincache);
            dns_acl_detach (&dacl);
        }
 ...
 */
static isc_result_t get_acl_def (const cfg_obj_t * cctx, const char *name, const cfg_obj_t ** ret)
{
    isc_result_t result;
...
convert_named_acl (const cfg_obj_t * nameobj, const cfg_obj_t * cctx,
                   ...
                   isc_mem_t * mctx, unsigned int nest_level, dns_acl_t ** target)
{
    isc_result_t result;
    ...
    for (dacl = ISC_LIST_HEAD (ctx->named_acl_cache); dacl != NULL; dacl = ISC_LIST_NEXT (dacl, nextincache))
    {
        if (strcasecmp (aclname, dacl->name) == 0)
        {
            if (ISC_MAGIC_VALID (dacl, LOOP_MAGIC))
            {
                cfg_obj_log (nameobj, lctx, ISC_LOG_ERROR, "acl loop detected: %s", aclname);
    ...
    if (result != ISC_R_SUCCESS)
    {
        cfg_obj_log (nameobj, lctx, ISC_LOG_WARNING, "undefined ACL '%s'", aclname);
        return (result);
    }
    /*
    ...
    result = cfg_acl_fromconfig (cacl, cctx, lctx, ctx, mctx, nest_level, &dacl);
...
static isc_result_t convert_keyname (const cfg_obj_t * keyobj, isc_log_t * lctx, isc_mem_t * mctx, dns_name_t * dnsname)
{
    isc_result_t result;
    ...
    result = dns_name_fromtext (dns_fixedname_name (&fixname), &buf, dns_rootname, 0, NULL);
    if (result != ISC_R_SUCCESS)
    {
        cfg_obj_log (keyobj, lctx, ISC_LOG_WARNING, "key name '%s' is not a valid domain name", txtname);
        return (result);
    }
 ...
 * parent.
 */
static int count_acl_elements (const cfg_obj_t * caml, const cfg_obj_t * cctx, isc_boolean_t * has_negative)
{
    const cfg_listelt_t *elt;
    ...
    for (elt = cfg_list_first (caml); elt != NULL; elt = cfg_list_next (elt))
    {
        ...
        else if (cfg_obj_isstring (ce))
        {
            ...
            if (strcasecmp (name, "localhost") == 0 || strcasecmp (name, "localnets") == 0)
            {
                n++;
            }
            else if (strcasecmp (name, "any") != 0 && strcasecmp (name, "none") != 0)
            {
                result = get_acl_def (cctx, name, &cacl);
                if (result == ISC_R_SUCCESS)
                    n += count_acl_elements (cacl, cctx, NULL) + 1;
            }
        }
    }
...
cfg_acl_fromconfig (const cfg_obj_t * caml,
                    ...
                    isc_log_t * lctx,
                    cfg_aclconfctx_t * ctx, isc_mem_t * mctx, unsigned int nest_level, dns_acl_t ** target)
{
    isc_result_t result;
    ...
    for (elt = cfg_list_first (caml); elt != NULL; elt = cfg_list_next (elt))
    {
        ...
        if (nest_level != 0)
        {
            result = dns_acl_create (mctx, cfg_list_length (ce, ISC_FALSE), &de->nestedacl);
            if (result != ISC_R_SUCCESS)
                goto cleanup;
        ...
        else if (cfg_obj_islist (ce))
        {
            ...
            result = cfg_acl_fromconfig (ce, cctx, lctx, ctx, mctx, new_nest_level, &inneracl);
            if (result != ISC_R_SUCCESS)
                goto cleanup;
          nested_acl:
            ...
            {
                dns_acl_merge (dacl, inneracl, ISC_TF (!neg));
        ...
        else if (cfg_obj_isstring (ce))
        {
            ...
            if (strcasecmp (name, "any") == 0)
            {
                ...
                result = dns_iptable_addprefix (iptab, NULL, 0, ISC_TF (nest_level != 0 || !neg));
                if (result != ISC_R_SUCCESS)
                    goto cleanup;
            ...
            else
            {
                if (inneracl != NULL)
                    dns_acl_detach (&inneracl);
                result = convert_named_acl (ce, cctx, lctx, ctx, mctx, new_nest_level, &inneracl);
                if (result != ISC_R_SUCCESS)
                    goto cleanup;
        ...
        {
            cfg_obj_log (ce, lctx, ISC_LOG_WARNING, "address match list contains " "unsupported element type");
         ...
         */
        if (de->nestedacl != NULL && de->type != dns_aclelementtype_nestedacl)
            dns_acl_detach (&de->nestedacl);
