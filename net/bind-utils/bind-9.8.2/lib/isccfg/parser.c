...
static isc_result_t
create_string (cfg_parser_t * pctx, const char *contents, const cfg_type_t * type, cfg_obj_t ** ret);
...
parse_symtab_elt (cfg_parser_t * pctx, const char *name,
                  cfg_type_t * elttype, isc_symtab_t * symtab, isc_boolean_t callback);
...
static void
parser_complain (cfg_parser_t * pctx, isc_boolean_t is_warning, unsigned int flags, const char *format, va_list args);
...
isc_result_t cfg_create_tuple (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    obj->value.tuple = isc_mem_get (pctx->mctx, nfields * sizeof (cfg_obj_t *));
...
const cfg_obj_t *cfg_tuple_get (const cfg_obj_t * tupleobj, const char *name)
{
    unsigned int i;
...
isc_result_t cfg_parse_special (cfg_parser_t * pctx, int special)
{
    ...
    if (pctx->token.type == isc_tokentype_special && pctx->token.value.as_char == special)
        return (ISC_R_SUCCESS);
...
static isc_result_t parse_semicolon (cfg_parser_t * pctx)
{
    ...
    CHECK (cfg_gettoken (pctx, 0));
    if (pctx->token.type == isc_tokentype_special && pctx->token.value.as_char == ';')
        return (ISC_R_SUCCESS);
...
isc_result_t cfg_parser_create (isc_mem_t * mctx, isc_log_t * lctx, cfg_parser_t ** ret)
{
    ...
    isc_lex_setcomments (pctx->lexer, (ISC_LEXCOMMENT_C | ISC_LEXCOMMENT_CPLUSPLUS | ISC_LEXCOMMENT_SHELL));
...
static isc_result_t parser_openfile (cfg_parser_t * pctx, const char *filename)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        cfg_parser_error (pctx, 0, "open: %s: %s", filename, isc_result_totext (result));
        goto cleanup;
    }
...
void cfg_print_uint64 (cfg_printer_t * pctx, const cfg_obj_t * obj)
{
    ...
    snprintf (buf, sizeof (buf), "%" ISC_PRINT_QUADFORMAT "u", obj->value.uint64);
    cfg_print_cstr (pctx, buf);
}
...
static void free_string (cfg_parser_t * pctx, cfg_obj_t * obj)
{
    isc_mem_put (pctx->mctx, obj->value.string.base, obj->value.string.length + 1);
}
...
isc_result_t cfg_parse_boolean (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    if ((strcasecmp (TOKEN_STRING (pctx), "true") == 0) ||
        (strcasecmp (TOKEN_STRING (pctx), "yes") == 0) || (strcmp (TOKEN_STRING (pctx), "1") == 0))
    {
        value = ISC_TRUE;
    }
    else if ((strcasecmp (TOKEN_STRING (pctx), "false") == 0) ||
             (strcasecmp (TOKEN_STRING (pctx), "no") == 0) || (strcmp (TOKEN_STRING (pctx), "0") == 0))
    {
        value = ISC_FALSE;
    }
    else
...
static void free_list_elt (cfg_parser_t * pctx, cfg_listelt_t * elt)
{
 ...
 */
static isc_result_t parse_list (cfg_parser_t * pctx, const cfg_type_t * listtype, cfg_obj_t ** ret)
{
...
static void print_list (cfg_printer_t * pctx, const cfg_obj_t * obj)
{
    ...
    for (elt = ISC_LIST_HEAD (*list); elt != NULL; elt = ISC_LIST_NEXT (elt, link))
    {
        print_indent (pctx);
    ...
    for (;;)
    {
      ...
      redo:
        ...
        if (strcasecmp (TOKEN_STRING (pctx), "include") == 0)
        {
            ...
            CHECK (parse_semicolon (pctx));
            CHECK (parser_openfile (pctx, includename->value.string.base));
        ...
        for (clauseset = clausesets; *clauseset != NULL; clauseset++)
        {
            ...
            {
                if (strcasecmp (TOKEN_STRING (pctx), clause->name) == 0)
                    goto done;
            }
        }
      done:
        ...
        if ((clause->flags & CFG_CLAUSEFLAG_OBSOLETE) != 0)
            cfg_parser_warning (pctx, 0, "option '%s' is obsolete", clause->name);
        if ((clause->flags & CFG_CLAUSEFLAG_NOTIMP) != 0)
            cfg_parser_warning (pctx, 0, "option '%s' is " "not implemented", clause->name);
        if ((clause->flags & CFG_CLAUSEFLAG_NYI) != 0)
            cfg_parser_warning (pctx, 0, "option '%s' is " "not implemented", clause->name);

        if ((clause->flags & CFG_CLAUSEFLAG_NOTCONFIGURED) != 0)
        {
            cfg_parser_warning (pctx, 0, "option '%s' is not " "configured", clause->name);
            result = ISC_R_FAILURE;
            goto cleanup;
        }
        ...
        result = isc_symtab_lookup (obj->value.map.symtab, clause->name, 0, &symval);
        ...
        if ((clause->flags & CFG_CLAUSEFLAG_MULTI) != 0)
        {
            ...
            {
                CHECK (cfg_create_list (pctx, &cfg_type_implicitlist, &listobj));
                symval.as_pointer = listobj;
                result = isc_symtab_define (obj->value.map.symtab, clause->name, 1, symval, isc_symexists_reject);
                if (result != ISC_R_SUCCESS)
                {
                    cfg_parser_error (pctx, CFG_LOG_NEAR, "isc_symtab_define(%s) " "failed", clause->name);
                    isc_mem_put (pctx->mctx, list, sizeof (cfg_list_t));
                    goto cleanup;
                }
            }
            else
        ...
        else
        {
            ...
            if (result == ISC_R_NOTFOUND)
            {
                isc_boolean_t callback = ISC_TF ((clause->flags & CFG_CLAUSEFLAG_CALLBACK) != 0);

                CHECK (parse_symtab_elt (pctx, clause->name, clause->type, obj->value.map.symtab, callback));
                CHECK (parse_semicolon (pctx));
            }
            else if (result == ISC_R_SUCCESS)
            {
                cfg_parser_error (pctx, CFG_LOG_NEAR, "'%s' redefined", clause->name);
                result = ISC_R_EXISTS;
                goto cleanup;
            }
            else
            {
                cfg_parser_error (pctx, CFG_LOG_NEAR, "isc_symtab_define() failed");
                goto cleanup;
            }
        }
    }
 ...
 */
static isc_result_t
parse_any_named_map (cfg_parser_t * pctx, cfg_type_t * nametype, const cfg_type_t * type, cfg_obj_t ** ret)
{
    isc_result_t result;
...
void cfg_print_mapbody (cfg_printer_t * pctx, const cfg_obj_t * obj)
{
    ...
    for (clauseset = obj->value.map.clausesets; *clauseset != NULL; clauseset++)
    {
        isc_symvalue_t symval;
        ...
        if ((flags & CFG_ADDR_V6OK) != 0 && strlen (s) <= 127U)
        {
            ...
            if (inet_pton (AF_INET6, buf, &in6a) == 1)
            {
                if (d != NULL)
                {
                    ...
                    result = isc_netscope_pton (AF_INET6, d + 1, &in6a, &zone);
                    if (result != ISC_R_SUCCESS)
                        return (result);
#else
...
isc_result_t cfg_parse_rawaddr (cfg_parser_t * pctx, unsigned int flags, isc_netaddr_t * na)
{
    ...
    if (result == ISC_R_UNEXPECTEDTOKEN)
    {
        ...
        if ((flags & CFG_ADDR_MASK) == CFG_ADDR_V4OK)
            cfg_parser_error (pctx, CFG_LOG_NEAR, "expected IPv4 address%s%s", prefix, wild);
        else if ((flags & CFG_ADDR_MASK) == CFG_ADDR_V6OK)
            cfg_parser_error (pctx, CFG_LOG_NEAR, "expected IPv6 address%s%s", prefix, wild);
        else
            cfg_parser_error (pctx, CFG_LOG_NEAR, "expected IP address%s%s", prefix, wild);
    }
  cleanup:
    return (result);
}
...
isc_result_t cfg_parse_rawport (cfg_parser_t * pctx, unsigned int flags, in_port_t * port)
{
    ...
    if ((flags & CFG_ADDR_WILDOK) != 0 &&
        pctx->token.type == isc_tokentype_string && strcmp (TOKEN_STRING (pctx), "*") == 0)
    {
        *port = 0;
        return (ISC_R_SUCCESS);
    }
    if (pctx->token.type != isc_tokentype_number)
    {
        cfg_parser_error (pctx, CFG_LOG_NEAR, "expected port number or '*'");
        return (ISC_R_UNEXPECTEDTOKEN);
    }
    if (pctx->token.value.as_ulong >= 65536U)
    {
        cfg_parser_error (pctx, CFG_LOG_NEAR, "port number out of range");
        return (ISC_R_UNEXPECTEDTOKEN);
    }
...
isc_result_t cfg_parse_netprefix (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    if (pctx->token.type == isc_tokentype_special && pctx->token.value.as_char == '/')
    {
        ...
        if (pctx->token.type != isc_tokentype_number)
        {
            cfg_parser_error (pctx, CFG_LOG_NEAR, "expected prefix length");
            return (ISC_R_UNEXPECTEDTOKEN);
        }
        prefixlen = pctx->token.value.as_ulong;
        if (prefixlen > addrlen)
        {
            cfg_parser_error (pctx, CFG_LOG_NOPREP, "invalid prefix length");
            return (ISC_R_RANGE);
        }
    }
    else
...
static isc_result_t parse_sockaddrsub (cfg_parser_t * pctx, const cfg_type_t * type, int flags, cfg_obj_t ** ret)
{
    ...
    CHECK (cfg_parse_rawaddr (pctx, flags, &netaddr));
    CHECK (cfg_peektoken (pctx, 0));
    if (pctx->token.type == isc_tokentype_string && strcasecmp (TOKEN_STRING (pctx), "port") == 0)
    {
    ...
    switch (result)
    {
        case ISC_R_SUCCESS:
            if (pctx->token.type == isc_tokentype_eof)
            {
                ...
                INSIST (result == ISC_R_NOMORE || result == ISC_R_SUCCESS);
        ...
        default:
            cfg_parser_error (pctx, CFG_LOG_NEAR, "%s", isc_result_totext (result));
            break;
    }
    return (result);
}
...
static isc_result_t cfg_getstringtoken (cfg_parser_t * pctx)
{
    ...
    if (pctx->token.type != isc_tokentype_string && pctx->token.type != isc_tokentype_qstring)
    {
...
parser_complain (cfg_parser_t * pctx, isc_boolean_t is_warning, unsigned int flags, const char *format, va_list args)
{
    ...
    snprintf (where, sizeof (where), "%s:%u: ", current_file (pctx), pctx->line);

    len = vsnprintf (message, sizeof (message), format, args);
    if (len >= sizeof (message))
        FATAL_ERROR (__FILE__, __LINE__, "error message would overflow");
    ...
    if ((flags & (CFG_LOG_NEAR | CFG_LOG_BEFORE | CFG_LOG_NOPREP)) != 0)
    {
        ...
        else
        {
            ...
            if (r.length > MAX_LOG_TOKEN)
                snprintf (tokenbuf, sizeof (tokenbuf), "'%.*s...'", MAX_LOG_TOKEN, r.base);
            else
                snprintf (tokenbuf, sizeof (tokenbuf), "'%.*s'", (int) r.length, r.base);
        }
    ...
    isc_log_write (pctx->lctx, CAT, MOD, level, "%s%s%s%s", where, message, prep, tokenbuf);
}
...
void cfg_obj_log (const cfg_obj_t * obj, isc_log_t * lctx, int level, const char *fmt, ...)
{
    ...
    isc_log_write (lctx, CAT, MOD, level,
                   "%s:%u: %s", obj->file == NULL ? "<unknown file>" : obj->file, obj->line, msgbuf);
    va_end (ap);
}
...
static isc_result_t create_map (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    isc_result_t result;
    ...
    CHECK (isc_symtab_create (pctx->mctx, 5,    /* XXX */
                              map_symtabitem_destroy, pctx, ISC_FALSE, &symtab));
...
void
cfg_print_grammar (const cfg_type_t * type, void (*f) (void *closure, const char *text, int textlen), void *closure)
{
    cfg_printer_t pctx;
