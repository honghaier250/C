...
static isc_result_t
parse_enum_or_other (cfg_parser_t * pctx, const cfg_type_t * enumtype, const cfg_type_t * othertype, cfg_obj_t ** ret);
 ...
 */
static cfg_clausedef_t dynamically_loadable_zones_clauses[] = {
...
static isc_result_t parse_matchtype (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    if (pctx->token.type == isc_tokentype_string && strcasecmp (TOKEN_STRING (pctx), "zonesub") == 0)
    {
...
static isc_result_t parse_updatepolicy (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    if (pctx->token.type == isc_tokentype_string && strcasecmp (TOKEN_STRING (pctx), "local") == 0)
    {
        cfg_obj_t *obj = NULL;

        CHECK (cfg_create_obj (pctx, &cfg_type_ustring, &obj));
        obj->value.string.length = strlen ("local");
        obj->value.string.base = isc_mem_get (pctx->mctx, obj->value.string.length + 1);
...
static isc_result_t parse_qstringornone (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    if (pctx->token.type == isc_tokentype_string && strcasecmp (TOKEN_STRING (pctx), "none") == 0)
...
static isc_result_t parse_boolorauto (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    CHECK (cfg_gettoken (pctx, CFG_LEXOPT_QSTRING));
    if (pctx->token.type == isc_tokentype_string && strcasecmp (TOKEN_STRING (pctx), "auto") == 0)
...
static isc_result_t parse_serverid (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    if (pctx->token.type == isc_tokentype_string && strcasecmp (TOKEN_STRING (pctx), "none") == 0)
        return (cfg_create_obj (pctx, &cfg_type_none, ret));
    if (pctx->token.type == isc_tokentype_string && strcasecmp (TOKEN_STRING (pctx), "hostname") == 0)
    {
...
static isc_result_t parse_portrange (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    {
        CHECK (cfg_gettoken (pctx, 0));
        if (pctx->token.type != isc_tokentype_string || strcasecmp (TOKEN_STRING (pctx), "range") != 0)
        {
            cfg_parser_error (pctx, CFG_LOG_NEAR, "expected integer or 'range'");
            return (ISC_R_UNEXPECTEDTOKEN);
        }
        ...
        if (obj->value.tuple[0]->value.uint32 > obj->value.tuple[1]->value.uint32)
        {
            cfg_parser_error (pctx, CFG_LOG_NOPREP,
                              "low port '%u' must not be larger " "than high port", obj->value.tuple[0]->value.uint32);
            result = ISC_R_RANGE;
            goto cleanup;
        }
        *ret = obj;
 ...
 */
static void doc_rpz_policies (cfg_printer_t * pctx, const cfg_type_t * type)
{
 ...
 */
static void print_rpz_cname (cfg_printer_t * pctx, const cfg_obj_t * obj)
{
    cfg_print_chars (pctx, "\"", 1);
    cfg_print_ustring (pctx, obj);
    cfg_print_chars (pctx, "\"", 1);
...
static isc_result_t parse_rpz (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    isc_result_t result;
     ...
     */
    if (cfg_obj_isvoid (obj->value.tuple[1]) || strcasecmp ("cname", cfg_obj_asstring (obj->value.tuple[1])))
    {
...
static void print_lookaside (cfg_printer_t * pctx, const cfg_obj_t * obj)
{
    ...
    if (domain->value.string.length == 4 && strncmp (domain->value.string.base, "auto", 4) == 0)
        cfg_print_cstr (pctx, "auto");
    else
...
static cfg_clausedef_t *dns64_clausesets[] = {
    dns64_clauses,
    NULL
};
...
static cfg_clausedef_t *namedconf_clausesets[] = {
    namedconf_clauses,
...
static cfg_clausedef_t *bindkeys_clausesets[] = {
    bindkeys_clauses,
    NULL
};
...
static cfg_clausedef_t *newzones_clausesets[] = {
    newzones_clauses,
    NULL
};
...
static cfg_clausedef_t *view_clausesets[] = {
    view_only_clauses,
    namedconf_or_view_clauses,
    view_clauses,
    zone_clauses,
...
static cfg_clausedef_t *zone_clausesets[] = {
    zone_only_clauses,
    zone_clauses,
    NULL
};
...
static cfg_clausedef_t *dynamically_loadable_zones_clausesets[] = {
    dynamically_loadable_zones_clauses,
    NULL
};
...
static cfg_clausedef_t *key_clausesets[] = {
    key_clauses,
    NULL
};
...
static cfg_clausedef_t *server_clausesets[] = {
    server_clauses,
    NULL
};
...
static cfg_clausedef_t *channel_clausesets[] = {
    channel_clauses,
    NULL
};
...
static cfg_clausedef_t *addzoneconf_clausesets[] = {
    addzoneconf_clauses,
    NULL
};
 ...
 */
static isc_result_t
parse_maybe_optional_keyvalue (cfg_parser_t * pctx, const cfg_type_t * type, isc_boolean_t optional, cfg_obj_t ** ret)
{
    isc_result_t result;
    ...
    CHECK (cfg_peektoken (pctx, 0));
    if (pctx->token.type == isc_tokentype_string && strcasecmp (TOKEN_STRING (pctx), kw->name) == 0)
    {
        CHECK (cfg_gettoken (pctx, 0));
    ...
    else
    {
        ...
        {
            cfg_parser_error (pctx, CFG_LOG_NEAR, "expected '%s'", kw->name);
            result = ISC_R_UNEXPECTEDTOKEN;
            goto cleanup;
        }
    }
...
parse_enum_or_other (cfg_parser_t * pctx, const cfg_type_t * enumtype, const cfg_type_t * othertype, cfg_obj_t ** ret)
{
    ...
    CHECK (cfg_peektoken (pctx, 0));
    if (pctx->token.type == isc_tokentype_string && cfg_is_enum (TOKEN_STRING (pctx), enumtype->of))
    {
...
static isc_result_t parse_v4_aaaa (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    return (parse_enum_or_other (pctx, type, &cfg_type_boolean, ret));
}
...
static cfg_clausedef_t *controls_clausesets[] = {
    controls_clauses,
    NULL
};
...
static cfg_clausedef_t *statservers_clausesets[] = {
    statservers_clauses,
    NULL
};
    ...
    for (;;)
    {
        ...
        if (pctx->token.type == isc_tokentype_string)
        {
            if (strcasecmp (TOKEN_STRING (pctx), "address") == 0)
            {
                /* read "address" */
                CHECK (cfg_gettoken (pctx, 0));
                CHECK (cfg_parse_rawaddr (pctx, *flagp, &netaddr));
                have_address++;
            }
            ...
            {
                cfg_parser_error (pctx, CFG_LOG_NEAR, "expected 'address' or 'port'");
                return (ISC_R_UNEXPECTEDTOKEN);
            }
        }
        else
...
static isc_result_t parse_addrmatchelt (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    {
        if (pctx->token.type == isc_tokentype_string && (strcasecmp (TOKEN_STRING (pctx), "key") == 0))
        {
            CHECK (cfg_parse_obj (pctx, &cfg_type_keyref, ret));
        }
        else
        {
            if (cfg_lookingat_netaddr (pctx, CFG_ADDR_V4OK | CFG_ADDR_V4PREFIXOK | CFG_ADDR_V6OK))
            {
    ...
    {
      bad:
        cfg_parser_error (pctx, CFG_LOG_NEAR, "expected IP match list element");
        return (ISC_R_UNEXPECTEDTOKEN);
    }
  cleanup:
    return (result);
 ...
 */
static unsigned int controls_sockaddr_flags = CFG_ADDR_V4OK | CFG_ADDR_V6OK | CFG_ADDR_WILDOK;
...
static isc_result_t parse_logseverity (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    CHECK (cfg_peektoken (pctx, 0));
    if (pctx->token.type == isc_tokentype_string && strcasecmp (TOKEN_STRING (pctx), "debug") == 0)
    {
...
static isc_result_t parse_logfile (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    for (;;)
    {
        ...
        {
            CHECK (cfg_gettoken (pctx, 0));
            if (strcasecmp (TOKEN_STRING (pctx), "versions") == 0 && obj->value.tuple[1] == NULL)
            {
                CHECK (cfg_parse_obj (pctx, fields[1].type, &obj->value.tuple[1]));
            }
            else if (strcasecmp (TOKEN_STRING (pctx), "size") == 0 && obj->value.tuple[2] == NULL)
            {
                CHECK (cfg_parse_obj (pctx, fields[2].type, &obj->value.tuple[2]));
            }
            else
...
static cfg_clausedef_t *lwres_clausesets[] = {
    lwres_clauses,
    NULL
};
...
static cfg_clausedef_t *rndcconf_clausesets[] = {
    rndcconf_clauses,
    NULL
};
...
static cfg_clausedef_t *rndckey_clausesets[] = {
    rndckey_clauses,
    NULL
};
...
static isc_result_t parse_sockaddrnameport (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    if (pctx->token.type == isc_tokentype_string || pctx->token.type == isc_tokentype_qstring)
    {
        ...
        {
            const cfg_tuplefielddef_t *fields = cfg_type_nameport.of;
    ...
    {
        cfg_parser_error (pctx, CFG_LOG_NEAR, "expected IP address or hostname");
        return (ISC_R_UNEXPECTEDTOKEN);
    }
  cleanup:
    CLEANUP_OBJ (obj);
...
static isc_result_t parse_masterselement (cfg_parser_t * pctx, const cfg_type_t * type, cfg_obj_t ** ret)
{
    ...
    {
        cfg_parser_error (pctx, CFG_LOG_NEAR, "expected IP address or masters name");
        return (ISC_R_UNEXPECTEDTOKEN);
    }
  cleanup:
    CLEANUP_OBJ (obj);
