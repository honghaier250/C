...
static inline isc_result_t grow_data (isc_lex_t * lex, size_t * remainingp, char **currp, char **prevp)
{
    char *new;
...
new_source (isc_lex_t * lex, isc_boolean_t is_file, isc_boolean_t need_close, void *input, const char *name)
{
    inputsource *source;
    ...
    result = isc_buffer_allocate (lex->mctx, &source->pushback, lex->max_token);
    ...
    do
    {
        ...
        if (!source->at_eof)
        {
            if (state == lexstate_start)
                ...
                source->ignored = isc_buffer_consumedlength (source->pushback);
        ...
        {
            if (!escaped && c == ';' && ((lex->comments & ISC_LEXCOMMENT_DNSMASTERFILE) != 0))
            {
                saved_state = state;
            ...
            else if (c == '/' && (lex->comments & (ISC_LEXCOMMENT_C | ISC_LEXCOMMENT_CPLUSPLUS)) != 0)
            {
                saved_state = state;
            ...
            else if (c == '#' && ((lex->comments & ISC_LEXCOMMENT_SHELL) != 0))
            {
                saved_state = state;
                state = lexstate_eatline;
                no_comments = ISC_TRUE;
      ...
      no_read:
        ...
        switch (state)
        {
            case lexstate_start:
                ...
                {
                    if (lex->last_was_eol && (options & ISC_LEXOPT_INITIALWS) != 0)
                    {
                        lex->last_was_eol = ISC_FALSE;
                ...
                else if (c == '"' && (options & ISC_LEXOPT_QSTRING) != 0)
                {
                    lex->last_was_eol = ISC_FALSE;
                    no_comments = ISC_TRUE;
                ...
                {
                    lex->last_was_eol = ISC_FALSE;
                    if ((c == '(' || c == ')') && (options & ISC_LEXOPT_DNSMULTILINE) != 0)
                    {
                        if (c == '(')
                        {
                ...
                else if (isdigit ((unsigned char) c) && (options & ISC_LEXOPT_NUMBER) != 0)
                {
                    lex->last_was_eol = ISC_FALSE;
                    if ((options & ISC_LEXOPT_OCTAL) != 0 && (c == '8' || c == '9'))
            ...
            case lexstate_number:
                if (c == EOF || !isdigit ((unsigned char) c))
                {
                    if (c == ' ' || c == '\t' || c == '\r' || c == '\n' || c == EOF || lex->specials[c])
                    {
                        ...
                        result = isc_parse_uint32 (&as_ulong, lex->data, base);
                        if (result == ISC_R_SUCCESS)
                        {
                            tokenp->type = isc_tokentype_number;
                            tokenp->value.as_ulong = as_ulong;
                        }
                        else if (result == ISC_R_BADNUMBER)
                        {
                            ...
                            v = &(tokenp->value);
                            v->as_textregion.base = lex->data;
                            v->as_textregion.length = lex->max_token - remaining;
                        }
                        else
                            goto done;
                        done = ISC_TRUE;
                        continue;
                             ...
                             ((c != 'x' && c != 'X') || (curr != &lex->data[1]) || (lex->data[0] != '0')))
                    {
                        /* Above test supports hex numbers */
                        state = lexstate_string;
                    }
                }
                else if ((options & ISC_LEXOPT_OCTAL) != 0 && (c == '8' || c == '9'))
                {
                    state = lexstate_string;
                }
                if (remaining == 0U)
                {
                    result = grow_data (lex, &remaining, &curr, &prev);
                    if (result != ISC_R_SUCCESS)
                        goto done;
                }
                INSIST (remaining > 0U);
            ...
            case lexstate_string:
                 ...
                 */
                if (c == '\r' || c == '\n' || c == EOF || (!escaped && (c == ' ' || c == '\t' || lex->specials[c])))
                {
                    pushback (source, c);
                    if (source->result != ISC_R_SUCCESS)
                    {
                    ...
                    tokenp->value.as_textregion.length = lex->max_token - remaining;
                    done = ISC_TRUE;
                    continue;
                }
                if ((options & ISC_LEXOPT_ESCAPE) != 0)
                    escaped = (!escaped && c == '\\') ? ISC_TRUE : ISC_FALSE;
                if (remaining == 0U)
                {
                    result = grow_data (lex, &remaining, &curr, &prev);
                    if (result != ISC_R_SUCCESS)
            ...
            case lexstate_maybecomment:
                ...
                else if (c == '/' && (lex->comments & ISC_LEXCOMMENT_CPLUSPLUS) != 0)
                {
                    state = lexstate_eatline;
                    continue;
                }
            ...
            case lexstate_qstring:
                ...
                {
                    if (c == '\n' && !escaped && (options & ISC_LEXOPT_QSTRINGMULTILINE) == 0)
                    {
                        pushback (source, c);
            ...
            default:
                FATAL_ERROR (__FILE__, __LINE__,
                             isc_msgcat_get (isc_msgcat, ISC_MSGSET_LEX,
                                             ISC_MSG_UNEXPECTEDSTATE, "Unexpected state %d"), state);
                /* Does not return. */
        }
...
{
    unsigned int options = ISC_LEXOPT_EOL | ISC_LEXOPT_EOF | ISC_LEXOPT_DNSMULTILINE | ISC_LEXOPT_ESCAPE;
    ...
    if (eol && ((token->type == isc_tokentype_eol) || (token->type == isc_tokentype_eof)))
        return (ISC_R_SUCCESS);
    if (token->type == isc_tokentype_string && expect == isc_tokentype_qstring)
        return (ISC_R_SUCCESS);
...
isc_result_t isc_lex_getoctaltoken (isc_lex_t * lex, isc_token_t * token, isc_boolean_t eol)
{
    unsigned int options = ISC_LEXOPT_EOL | ISC_LEXOPT_EOF |
        ISC_LEXOPT_DNSMULTILINE | ISC_LEXOPT_ESCAPE | ISC_LEXOPT_NUMBER | ISC_LEXOPT_OCTAL;
    isc_result_t result;
...
void isc_lex_ungettoken (isc_lex_t * lex, isc_token_t * tokenp)
{
    ...
    REQUIRE (source != NULL);
    REQUIRE (tokenp != NULL);
    REQUIRE (isc_buffer_consumedlength (source->pushback) != 0 || tokenp->type == isc_tokentype_eof);
