...
isc__timer_create (isc_timermgr_t * manager, isc_timertype_t type,
                   ...
                   isc_task_t * task, isc_taskaction_t action, const void *arg, isc_timer_t ** timerp);
ISC_TIMERFUNC_SCOPE isc_result_t
isc__timer_reset (isc_timer_t * timer, isc_timertype_t type,
                  isc_time_t * expires, isc_interval_t * interval, isc_boolean_t purge);
ISC_TIMERFUNC_SCOPE isc_timertype_t isc__timer_gettype (isc_timer_t * timer);

ISC_TIMERFUNC_SCOPE isc_result_t isc__timer_touch (isc_timer_t * timer);

ISC_TIMERFUNC_SCOPE void isc__timer_attach (isc_timer_t * timer0, isc_timer_t ** timerp);

ISC_TIMERFUNC_SCOPE void isc__timer_detach (isc_timer_t ** timerp);

ISC_TIMERFUNC_SCOPE isc_result_t isc__timermgr_create (isc_mem_t * mctx, isc_timermgr_t ** managerp);

ISC_TIMERFUNC_SCOPE void isc__timermgr_poke (isc_timermgr_t * manager0);

ISC_TIMERFUNC_SCOPE void isc__timermgr_destroy (isc_timermgr_t ** managerp);
...
{
    {
    isc__timer_attach, isc__timer_detach, isc__timer_reset, isc__timer_touch}
#ifndef BIND9
...
{
    {
    isc__timermgr_destroy, isc__timer_create}
#ifndef BIND9
...
static inline isc_result_t schedule (isc__timer_t * timer, isc_time_t * now, isc_boolean_t signal_ok)
{
     ...
     */
    timedwait = ISC_TF (manager->nscheduled > 0 && isc_time_seconds (&manager->due) != 0);
#endif
    ...
    if (timer->type != isc_timertype_once)
    {
        ...
        if (result != ISC_R_SUCCESS)
            return (result);
        if (timer->type == isc_timertype_limited && isc_time_compare (&timer->expires, &due) < 0)
            due = timer->expires;
    }
    else
    {
    ...
    XTRACETIMER (isc_msgcat_get (isc_msgcat, ISC_MSGSET_TIMER, ISC_MSG_SCHEDULE, "schedule"), timer, due);
    ...
    if (signal_ok && timedwait)
    {
        ...
        if (result == ISC_R_SUCCESS && isc_time_compare (&then, now) < 0)
        {
            SIGNAL (&manager->wakeup);
            signal_ok = ISC_FALSE;
            isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                           ISC_LOGMODULE_TIMER, ISC_LOG_WARNING, "*** POKED TIMER ***");
        }
    }

    if (timer->index == 1 && signal_ok)
    {
        XTRACE (isc_msgcat_get (isc_msgcat, ISC_MSGSET_TIMER, ISC_MSG_SIGNALSCHED, "signal (schedule)"));
        SIGNAL (&manager->wakeup);
    }
#else                            /* USE_TIMER_THREAD */
    if (timer->index == 1 && isc_time_compare (&timer->due, &manager->due) < 0)
...
static inline void deschedule (isc__timer_t * timer)
{
#ifdef USE_TIMER_THREAD
    ...
    if (timer->index > 0)
    {
        ...
        if (need_wakeup)
        {
            XTRACE (isc_msgcat_get (isc_msgcat, ISC_MSGSET_TIMER, ISC_MSG_SIGNALDESCHED, "signal (deschedule)"));
            SIGNAL (&manager->wakeup);
        }
#endif                            /* USE_TIMER_THREAD */
    }
...
static void destroy (isc__timer_t * timer)
{
    ...
    (void) isc_task_purgerange (timer->task, timer, ISC_TIMEREVENT_FIRSTEVENT, ISC_TIMEREVENT_LASTEVENT, NULL);
    deschedule (timer);
                   ...
                   isc_task_t * task, isc_taskaction_t action, const void *arg, isc_timer_t ** timerp)
{
    ...
    REQUIRE (type == isc_timertype_inactive || !(isc_time_isepoch (expires) && isc_interval_iszero (interval)));
    REQUIRE (timerp != NULL && *timerp == NULL);
    REQUIRE (type != isc_timertype_limited || !(isc_time_isepoch (expires) || isc_interval_iszero (interval)));
...
ISC_TIMERFUNC_SCOPE isc_timertype_t isc__timer_gettype (isc_timer_t * timer0)
{
    isc__timer_t *timer = (isc__timer_t *) timer0;
...
ISC_TIMERFUNC_SCOPE isc_result_t isc__timer_touch (isc_timer_t * timer0)
{
    isc__timer_t *timer = (isc__timer_t *) timer0;
...
static void dispatch (isc__timermgr_t * manager, isc_time_t * now)
{
    ...
    while (manager->nscheduled > 0 && !done)
    {
        ...
        if (isc_time_compare (now, &timer->due) >= 0)
        {
            ...
            else if (!isc_time_isepoch (&timer->expires) && isc_time_compare (now, &timer->expires) >= 0)
            {
                type = ISC_TIMEREVENT_LIFE;
                post_event = ISC_TRUE;
                need_schedule = ISC_FALSE;
            ...
            else
            {
                ...
                LOCK (&timer->lock);
                if (!isc_time_isepoch (&timer->idle) && isc_time_compare (now, &timer->idle) >= 0)
                {
                    idle = ISC_TRUE;
                }
                UNLOCK (&timer->lock);
                ...
                else
                {
                    ...
                    XTRACEID (isc_msgcat_get (isc_msgcat,
                                              ISC_MSGSET_TIMER, ISC_MSG_IDLERESCHED, "idle reschedule"), timer);
            ...
            if (post_event)
            {
                XTRACEID (isc_msgcat_get (isc_msgcat, ISC_MSGSET_TIMER, ISC_MSG_POSTING, "posting"), timer);
                /*
                 * XXX We could preallocate this event.
                 */
                event = (isc_timerevent_t *) isc_event_allocate (manager->mctx,
                                                                 timer,
                                                                 type, timer->action, timer->arg, sizeof (*event));
                ...
                if (event != NULL)
                {
                    ...
                    UNEXPECTED_ERROR (__FILE__, __LINE__, "%s",
                                      isc_msgcat_get (isc_msgcat,
                                                      ISC_MSGSET_TIMER,
                                                      ISC_MSG_EVENTNOTALLOC, "couldn't " "allocate event"));
            }
            ...
            if (need_schedule)
            {
                ...
                if (result != ISC_R_SUCCESS)
                    UNEXPECTED_ERROR (__FILE__, __LINE__,
                                      ...
                                      isc_msgcat_get (isc_msgcat,
                                                      ISC_MSGSET_TIMER,
                                                      ISC_MSG_SCHEDFAIL, "couldn't schedule " "timer"), result);
            }
        }
        else
        {
...
run (void *uap)
{
    ...
    while (!manager->done)
    {
        ...
        XTRACETIME (isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_RUNNING, "running"), now);
        ...
        if (manager->nscheduled > 0)
        {
            XTRACETIME2 (isc_msgcat_get (isc_msgcat,
                                         ISC_MSGSET_GENERAL, ISC_MSG_WAITUNTIL, "waituntil"), manager->due, now);
            result = WAITUNTIL (&manager->wakeup, &manager->lock, &manager->due);
            INSIST (result == ISC_R_SUCCESS || result == ISC_R_TIMEDOUT);
        }
        else
        {
            XTRACETIME (isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_WAIT, "wait"), now);
            WAIT (&manager->wakeup, &manager->lock);
        }
        XTRACE (isc_msgcat_get (isc_msgcat, ISC_MSGSET_TIMER, ISC_MSG_WAKEUP, "wakeup"));
    }
    UNLOCK (&manager->lock);
...
ISC_TIMERFUNC_SCOPE isc_result_t isc__timermgr_create (isc_mem_t * mctx, isc_timermgr_t ** managerp)
{
    ...
    if (isc_thread_create (run, manager, &manager->thread) != ISC_R_SUCCESS)
    {
        isc_mem_detach (&manager->mctx);
...
#ifdef USE_TIMER_THREAD
    XTRACE (isc_msgcat_get (isc_msgcat, ISC_MSGSET_TIMER, ISC_MSG_SIGNALDESTROY, "signal (destroy)"));
    SIGNAL (&manager->wakeup);
#endif                            /* USE_TIMER_THREAD */
...
isc_result_t isc__timer_register ()
{
