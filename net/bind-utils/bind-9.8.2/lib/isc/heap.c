...
isc_heap_create (isc_mem_t * mctx, isc_heapcompare_t compare,
                 isc_heapindex_t index, unsigned int size_increment, isc_heap_t ** heapp)
{
    isc_heap_t *heap;
...
void isc_heap_destroy (isc_heap_t ** heapp)
{
    ...
    if (heap->array != NULL)
        isc_mem_put (heap->mctx, heap->array, heap->size * sizeof (void *));
    heap->magic = 0;
...
static void float_up (isc_heap_t * heap, unsigned int i, void *elt)
{
    ...
    for (p = heap_parent (i); i > 1 && heap->compare (elt, heap->array[p]); i = p, p = heap_parent (i))
    {
...
static void sink_down (isc_heap_t * heap, unsigned int i, void *elt)
{
    ...
    while (i <= half_size)
    {
        ...
        j = heap_left (i);
        if (j < size && heap->compare (heap->array[j + 1], heap->array[j]))
            j++;
