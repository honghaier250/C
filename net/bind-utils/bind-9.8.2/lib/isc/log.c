...
isc_log_doit (isc_log_t * lctx, isc_logcategory_t * category,
              ...
              isc_msgcat_t * msgcat, int msgset, int msg, const char *format, va_list args) ISC_FORMAT_PRINTF (9, 0);
...
isc_result_t isc_logconfig_create (isc_log_t * lctx, isc_logconfig_t ** lcfgp)
{
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        result = isc_log_createchannel (lcfg, "default_syslog", ISC_LOG_TOSYSLOG, level, &destination, 0);
    }
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        result = isc_log_createchannel (lcfg, "default_stderr",
                                        ISC_LOG_TOFILEDESC, level, &destination, ISC_LOG_PRINTTIME);
    }
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        result = isc_log_createchannel (lcfg, "default_debug",
                                        ISC_LOG_TOFILEDESC, ISC_LOG_DYNAMIC, &destination, ISC_LOG_PRINTTIME);
    }

    if (result == ISC_R_SUCCESS)
        result = isc_log_createchannel (lcfg, "null", ISC_LOG_TONULL, ISC_LOG_DYNAMIC, NULL, 0);
...
void isc_log_destroy (isc_log_t ** lctxp)
{
    ...
    while ((message = ISC_LIST_HEAD (lctx->messages)) != NULL)
    {
        ...
        isc_mem_put (mctx, message, sizeof (*message) + strlen (message->text) + 1);
    }
...
void isc_logconfig_destroy (isc_logconfig_t ** lcfgp)
{
    ...
    if (lcfg->channellist_count > 0)
        isc_mem_put (mctx, lcfg->channellists, lcfg->channellist_count * sizeof (ISC_LIST (isc_logchannellist_t)));
...
isc_log_createchannel (isc_logconfig_t * lcfg, const char *name,
                       unsigned int type, int level, const isc_logdestination_t * destination, unsigned int flags)
{
    isc_logchannel_t *channel;
    ...
    REQUIRE ((flags & (unsigned int) ~(ISC_LOG_PRINTALL | ISC_LOG_DEBUGONLY)) == 0);
    ...
    switch (type)
    {
        ...
        case ISC_LOG_TOFILE:
             ...
             * writable memory.
             */
            FILE_NAME (channel) = isc_mem_strdup (mctx, destination->file.name);
...
isc_log_usechannel (isc_logconfig_t * lcfg, const char *name,
                    const isc_logcategory_t * category, const isc_logmodule_t * module)
{
    isc_log_t *lctx;
    ...
    for (channel = ISC_LIST_HEAD (lcfg->channels); channel != NULL; channel = ISC_LIST_NEXT (channel, link))
...
isc_log_vwrite (isc_log_t * lctx, isc_logcategory_t * category,
                isc_logmodule_t * module, int level, const char *format, va_list args)
{
    /*
     * Contract checking is done in isc_log_doit().
     */
                ...
                isc_logmodule_t * module, int level, const char *format, ...)
{
    ...
    va_start (args, format);
    isc_log_doit (lctx, category, module, level, ISC_TRUE, NULL, 0, 0, format, args);
    va_end (args);
}
                ...
                isc_msgcat_t * msgcat, int msgset, int msg, const char *format, ...)
{
    ...
    va_start (args, format);
    isc_log_doit (lctx, category, module, level, ISC_FALSE, msgcat, msgset, msg, format, args);
    va_end (args);
}
...
isc_log_ivwrite (isc_log_t * lctx, isc_logcategory_t * category,
                 isc_logmodule_t * module, int level,
                 isc_msgcat_t * msgcat, int msgset, int msg, const char *format, va_list args)
{
    /*
     * Contract checking is done in isc_log_doit().
     */
                 ...
                 isc_msgcat_t * msgcat, int msgset, int msg, const char *format, ...)
{
    ...
    va_start (args, format);
    isc_log_doit (lctx, category, module, level, ISC_TRUE, msgcat, msgset, msg, format, args);
    va_end (args);
}
...
void isc_log_setdebuglevel (isc_log_t * lctx, unsigned int level)
{
    ...
    if (lctx->debug_level == 0)
        for (channel = ISC_LIST_HEAD (lctx->logconfig->channels);
            ...
            if (channel->type == ISC_LOG_TOFILE &&
                (channel->flags & ISC_LOG_DEBUGONLY) != 0 && FILE_STREAM (channel) != NULL)
            {
                (void) fclose (FILE_STREAM (channel));
                FILE_STREAM (channel) = NULL;
            }
...
void isc_log_closefilelogs (isc_log_t * lctx)
{
    ...
    LOCK (&lctx->lock);
    for (channel = ISC_LIST_HEAD (lctx->logconfig->channels); channel != NULL; channel = ISC_LIST_NEXT (channel, link))
               ...
               const isc_logmodule_t * module, isc_logchannel_t * channel)
{
    ...
    ISC_LIST_INITANDPREPEND (lcfg->channellists[category_id], new_item, link);
...
static isc_result_t sync_channellist (isc_logconfig_t * lcfg)
{
    ...
    if (lcfg->channellist_count != 0)
    {
        bytes = lcfg->channellist_count * sizeof (ISC_LIST (isc_logchannellist_t));
...
static isc_result_t greatest_version (isc_logchannel_t * channel, int *greatestp)
{
    /* XXXDCL HIGHLY NT */
    ...
    if ((basename != NULL && basename2 != NULL && basename2 > basename) || (basename == NULL && basename2 != NULL))
    {
        basename = basename2;
        sep = '\\';
    }
    ...
    while (isc_dir_read (&dir) == ISC_R_SUCCESS)
    {
        if (dir.entry.length > basenamelen &&
            strncmp (dir.entry.name, basename, basenamelen) == 0 && dir.entry.name[basenamelen] == '.')
        {

            version = strtol (&dir.entry.name[basenamelen + 1], &digit_end, 10);
...
static isc_result_t roll_log (isc_logchannel_t * channel)
{
    ...
    if (FILE_VERSIONS (channel) == ISC_LOG_ROLLINFINITE || FILE_VERSIONS (channel) > greatest)
        ;                        /* Do nothing. */
    else
        /*
        ...
        {
            n = snprintf (current, sizeof (current), "%s.%d", path, greatest);
            if (n >= (int) sizeof (current) || n < 0)
                result = ISC_R_NOSPACE;
            else
                result = isc_file_remove (current);
            if (result != ISC_R_SUCCESS && result != ISC_R_FILENOTFOUND)
                syslog (LOG_ERR, "unable to remove log file '%s.%d': %s", path, greatest, isc_result_totext (result));
        }
    ...
    for (i = greatest; i > 0; i--)
    {
        ...
        if (result != ISC_R_SUCCESS && result != ISC_R_FILENOTFOUND)
            syslog (LOG_ERR,
                    ...
                    "'%s.%d': %s", path, i - 1, path, i, isc_result_totext (result));
    }
    ...
    if (FILE_VERSIONS (channel) != 0)
    {
        ...
        if (result != ISC_R_SUCCESS && result != ISC_R_FILENOTFOUND)
            syslog (LOG_ERR, "unable to rename log file '%s' to '%s.0': %s", path, path, isc_result_totext (result));
    }
    else
    {
        result = isc_file_remove (path);
        if (result != ISC_R_SUCCESS && result != ISC_R_FILENOTFOUND)
            syslog (LOG_ERR, "unable to remove log file '%s': %s", path, isc_result_totext (result));
    }
...
static isc_result_t isc_log_open (isc_logchannel_t * channel)
{
    ...
    if (stat (path, &statbuf) == 0)
    {
        ...
        if ((FILE_MAXSIZE (channel) == 0 &&
             FILE_VERSIONS (channel) != ISC_LOG_ROLLNEVER) ||
            (FILE_MAXSIZE (channel) > 0 && statbuf.st_size >= FILE_MAXSIZE (channel)))
            roll = regular_file;
    }
    ...
    if (result == ISC_R_SUCCESS && roll)
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            {
                syslog (LOG_ERR,
                        "isc_log_open: roll_log '%s' " "failed: %s", FILE_NAME (channel), isc_result_totext (result));
              ...
              isc_msgcat_t * msgcat, int msgset, int msg, const char *format, va_list args)
{
    ...
    do
    {
        ...
        if (category_channels == NULL && !matched && category_channels != ISC_LIST_HEAD (lcfg->channellists[0]))
            /*
        ...
        if (category_channels->module != NULL && category_channels->module != module)
        {
        ...
        if (((channel->flags & ISC_LOG_DEBUGONLY) != 0) && lctx->debug_level == 0)
            continue;
        ...
        if ((channel->flags & ISC_LOG_PRINTTIME) != 0 && time_string[0] == '\0')
        {
            isc_time_t isctime;

            TIME_NOW (&isctime);
            isc_time_formattimestamp (&isctime, time_string, sizeof (time_string));
        }

        if ((channel->flags & ISC_LOG_PRINTLEVEL) != 0 && level_string[0] == '\0')
        {
            if (level < ISC_LOG_CRITICAL)
                snprintf (level_string, sizeof (level_string),
                          isc_msgcat_get (isc_msgcat, ISC_MSGSET_LOG, ISC_MSG_LEVEL, "level %d: "), level);
            else if (level > ISC_LOG_DYNAMIC)
                snprintf (level_string, sizeof (level_string), "%s %d: ", log_level_strings[0], level);
            else
                snprintf (level_string, sizeof (level_string), "%s: ", log_level_strings[-level]);
        }
        ...
        if (lctx->buffer[0] == '\0')
        {
            (void) vsnprintf (lctx->buffer, sizeof (lctx->buffer), iformat, args);
            ...
            if (write_once)
            {
                ...
                isc_interval_set (&interval, lcfg->duplicate_interval, 0);
                 ...
                 */
                TIME_NOW (&oldest);
                if (isc_time_subtract (&oldest, &interval, &oldest) != ISC_R_SUCCESS)
                    /*
                ...
                while (message != NULL)
                {
                    if (isc_time_compare (&message->time, &oldest) < 0)
                    {
                        /*
                         ...
                         * existence.
                         */
                        new = ISC_LIST_NEXT (message, link);

                        ISC_LIST_UNLINK (lctx->messages, message, link);

                        isc_mem_put (lctx->mctx, message, sizeof (*message) + 1 + strlen (message->text));
                     ...
                     * filtering interval ...
                     */
                    if (strcmp (lctx->buffer, message->text) == 0)
                    {
                        /*
                 ...
                 */
                new = isc_mem_get (lctx->mctx, sizeof (isc_logmessage_t) + strlen (lctx->buffer) + 1);
                if (new != NULL)
                {
                    /*
                    ...
                    ISC_LIST_APPEND (lctx->messages, new, link);
                }
            }
        }

        printtime = ISC_TF ((channel->flags & ISC_LOG_PRINTTIME) != 0);
        printtag = ISC_TF ((channel->flags & ISC_LOG_PRINTTAG) != 0 && lcfg->tag != NULL);
        printcategory = ISC_TF ((channel->flags & ISC_LOG_PRINTCATEGORY) != 0);
        printmodule = ISC_TF ((channel->flags & ISC_LOG_PRINTMODULE) != 0);
        ...
        switch (channel->type)
        {
            case ISC_LOG_TOFILE:
                if (FILE_MAXREACHED (channel))
                {
                    ...
                    if (FILE_VERSIONS (channel) !=
                        ...
                        (stat (FILE_NAME (channel), &statbuf) != 0 &&
                         errno == ENOENT) || statbuf.st_size < FILE_MAXSIZE (channel))
                    {
                        (void) fclose (FILE_STREAM (channel));
                        FILE_STREAM (channel) = NULL;
                        FILE_MAXREACHED (channel) = ISC_FALSE;
                ...
                if (FILE_STREAM (channel) == NULL)
                {
                    ...
                    if (result != ISC_R_SUCCESS && result != ISC_R_MAXSIZE && (channel->flags & ISC_LOG_OPENERR) == 0)
                    {
                        syslog (LOG_ERR,
                                "isc_log_open '%s' failed: %s", FILE_NAME (channel), isc_result_totext (result));
                        channel->flags |= ISC_LOG_OPENERR;
                    }
                    if (result != ISC_R_SUCCESS)
                        break;
            ...
            case ISC_LOG_TOFILEDESC:
                fprintf (FILE_STREAM (channel), "%s%s%s%s%s%s%s%s%s%s\n",
                         ...
                         printmodule ? (module != NULL ? module->name
                                        : "no_module")
                         : "", printmodule ? ": " : "", printlevel ? level_string : "", lctx->buffer);
                ...
                if (FILE_MAXSIZE (channel) > 0)
                {
                    ...
                    if (fstat (fileno (FILE_STREAM (channel)),
                               &statbuf) >= 0 && statbuf.st_size > FILE_MAXSIZE (channel))
