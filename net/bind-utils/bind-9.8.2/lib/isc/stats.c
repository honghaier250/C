...
static isc_result_t create_stats (isc_mem_t * mctx, int ncounters, isc_stats_t ** statsp)
{
    ...
    stats->copiedcounters = isc_mem_get (mctx, sizeof (isc_uint64_t) * ncounters);
  ...
  clean_copiedcounters:
    isc_mem_put (mctx, stats->copiedcounters, sizeof (isc_stat_t) * ncounters);
#endif
...
void isc_stats_detach (isc_stats_t ** statsp)
{
    ...
    *statsp = NULL;
    ...
    {
        isc_mem_put (stats->mctx, stats->copiedcounters, sizeof (isc_stat_t) * stats->ncounters);
        isc_mem_put (stats->mctx, stats->counters, sizeof (isc_stat_t) * stats->ncounters);
        DESTROYLOCK (&stats->lock);
#ifdef ISC_RWLOCK_USEATOMIC
...
static inline void incrementcounter (isc_stats_t * stats, int counter)
{
    isc_int32_t prev;
...
static inline void decrementcounter (isc_stats_t * stats, int counter)
{
    isc_int32_t prev;
...
static void copy_counters (isc_stats_t * stats)
{
    ...
    {
        stats->copiedcounters[i] = (isc_uint64_t) (stats->counters[i].hi) << 32 | stats->counters[i].lo;
    }
#else
    UNUSED (i);
    memcpy (stats->copiedcounters, stats->counters, stats->ncounters * sizeof (isc_stat_t));
#endif
...
void isc_stats_dump (isc_stats_t * stats, isc_stats_dumper_t dump_fn, void *arg, unsigned int options)
{
    ...
    for (i = 0; i < stats->ncounters; i++)
    {
        if ((options & ISC_STATSDUMP_VERBOSE) == 0 && stats->copiedcounters[i] == 0)
            continue;
