...
#define ISC_TASKFUNC_SCOPE static
     ...
     ISC_TASKFUNC_SCOPE isc_result_t
         isc__task_create (isc_taskmgr_t * manager0, unsigned int quantum, isc_task_t ** taskp);
     ISC_TASKFUNC_SCOPE void isc__task_attach (isc_task_t * source0, isc_task_t ** targetp);

     ISC_TASKFUNC_SCOPE void isc__task_detach (isc_task_t ** taskp);
     ...
     ISC_TASKFUNC_SCOPE unsigned int isc__task_purge (isc_task_t * task, void *sender, isc_eventtype_t type, void *tag);

     ISC_TASKFUNC_SCOPE isc_boolean_t isc__task_purgeevent (isc_task_t * task0, isc_event_t * event);
     ...
     ISC_TASKFUNC_SCOPE unsigned int
         isc__task_unsend (isc_task_t * task, void *sender, isc_eventtype_t type, void *tag, isc_eventlist_t * events);
     ISC_TASKFUNC_SCOPE isc_result_t
         isc__task_onshutdown (isc_task_t * task0, isc_taskaction_t action, const void *arg);
     ISC_TASKFUNC_SCOPE void isc__task_shutdown (isc_task_t * task0);

     ISC_TASKFUNC_SCOPE void isc__task_destroy (isc_task_t ** taskp);

     ISC_TASKFUNC_SCOPE void isc__task_setname (isc_task_t * task0, const char *name, void *tag);

     ISC_TASKFUNC_SCOPE const char *isc__task_getname (isc_task_t * task0);

     ISC_TASKFUNC_SCOPE void *isc__task_gettag (isc_task_t * task0);

     ISC_TASKFUNC_SCOPE void isc__task_getcurrenttime (isc_task_t * task0, isc_stdtime_t * t);

     ISC_TASKFUNC_SCOPE isc_result_t
         isc__taskmgr_create (isc_mem_t * mctx, unsigned int workers,
                              unsigned int default_quantum, isc_taskmgr_t ** managerp);
     ISC_TASKFUNC_SCOPE void isc__taskmgr_destroy (isc_taskmgr_t ** managerp);

     ISC_TASKFUNC_SCOPE isc_result_t isc__task_beginexclusive (isc_task_t * task);

     ISC_TASKFUNC_SCOPE void isc__task_endexclusive (isc_task_t * task0);
     ...
     } taskmethods =
{
    ...
    isc__task_attach,
            ...
            isc__task_shutdown,
            isc__task_setname, isc__task_purge, isc__task_purgerange, isc__task_beginexclusive, isc__task_endexclusive}
#ifndef BIND9
    , (void *) isc__task_purgeevent, (void *) isc__task_unsendrange,
        (void *) isc__task_getname, (void *) isc__task_gettag, (void *) isc__task_getcurrenttime
#endif
};
...
ISC_TASKFUNC_SCOPE isc_result_t isc__task_create (isc_taskmgr_t * manager0, unsigned int quantum, isc_task_t ** taskp)
{
...
static inline isc_boolean_t task_shutdown (isc__task_t * task)
{
    ...
    {
        XTRACE (isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_SHUTTINGDOWN, "shutting down"));
        ...
        INSIST (task->state == task_state_ready || task->state == task_state_running);
        /*
         * Note that we post shutdown events LIFO.
         */
        for (event = TAIL (task->on_shutdown); event != NULL; event = prev)
        {
...
static inline void task_ready (isc__task_t * task)
{
    isc__taskmgr_t *manager = task->manager;
...
static inline isc_boolean_t task_detach (isc__task_t * task)
{
...
static inline isc_boolean_t task_send (isc__task_t * task, isc_event_t ** eventp)
{
    isc_boolean_t was_idle = ISC_FALSE;
...
dequeue_events (isc__task_t * task, void *sender, isc_eventtype_t first,
                isc_eventtype_t last, void *tag, isc_eventlist_t * events, isc_boolean_t purging)
{
    ...
    for (event = HEAD (task->events); event != NULL; event = next_event)
    {
        ...
        if (event->ev_type >= first && event->ev_type <= last &&
            ...
            (tag == NULL || event->ev_tag == tag) && (!purging || PURGE_OK (event)))
        {
...
ISC_TASKFUNC_SCOPE unsigned int
isc__task_purgerange (isc_task_t * task0, void *sender, isc_eventtype_t first, isc_eventtype_t last, void *tag)
{
    isc__task_t *task = (isc__task_t *) task0;
    ...
    count = dequeue_events (task, sender, first, last, tag, &events, ISC_TRUE);
...
ISC_TASKFUNC_SCOPE isc_boolean_t isc__task_purgeevent (isc_task_t * task0, isc_event_t * event)
{
    ...
    LOCK (&task->lock);
    for (curr_event = HEAD (task->events); curr_event != NULL; curr_event = next_event)
    {
                       ...
                       isc_eventtype_t last, void *tag, isc_eventlist_t * events)
{
    ...
    return (dequeue_events ((isc__task_t *) task, sender, first, last, tag, events, ISC_FALSE));
}
...
isc__task_unsend (isc_task_t * task, void *sender, isc_eventtype_t type, void *tag, isc_eventlist_t * events)
{
    ...
    return (dequeue_events ((isc__task_t *) task, sender, type, type, tag, events, ISC_FALSE));
}

ISC_TASKFUNC_SCOPE isc_result_t isc__task_onshutdown (isc_task_t * task0, isc_taskaction_t action, const void *arg)
{
    isc__task_t *task = (isc__task_t *) task0;
    ...
    event = isc_event_allocate (task->manager->mctx, NULL, ISC_TASKEVENT_SHUTDOWN, action, arg, sizeof (*event));
    if (event == NULL)
        return (ISC_R_NOMEMORY);
...
static void dispatch (isc__taskmgr_t * manager)
{
    ...
    while (!FINISHED (manager))
    {
        ...
        if (total_dispatch_count >= DEFAULT_TASKMGR_QUANTUM || EMPTY (manager->ready_tasks))
            break;
#endif                            /* USE_WORKER_THREADS */
        XTHREADTRACE (isc_msgcat_get (isc_msgcat, ISC_MSGSET_TASK, ISC_MSG_WORKING, "working"));
        ...
        if (task != NULL)
        {
            ...
            XTRACE (isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_RUNNING, "running"));
            ...
            do
            {
                if (!EMPTY (task->events))
                {
                     ...
                     */
                    XTRACE (isc_msgcat_get (isc_msgcat, ISC_MSGSET_TASK, ISC_MSG_EXECUTE, "execute action"));
                ...
                if (EMPTY (task->events))
                {
                     ...
                     * right now.
                     */
                    XTRACE (isc_msgcat_get (isc_msgcat, ISC_MSGSET_TASK, ISC_MSG_EMPTY, "empty"));
                    if (task->references == 0 && TASK_SHUTTINGDOWN (task))
                    {
                        /*
                         * The task is done.
                         */
                        XTRACE (isc_msgcat_get (isc_msgcat, ISC_MSGSET_TASK, ISC_MSG_DONE, "done"));
                        finished = ISC_TRUE;
                ...
                else if (dispatch_count >= task->quantum)
                {
                     ...
                     */
                    XTRACE (isc_msgcat_get (isc_msgcat, ISC_MSGSET_TASK, ISC_MSG_QUANTUM, "quantum"));
                    task->state = task_state_ready;
                    requeue = ISC_TRUE;
                    done = ISC_TRUE;
                }
...
run (void *uap)
{
    ...
    XTHREADTRACE (isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_STARTING, "starting"));

    dispatch (manager);

    XTHREADTRACE (isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_EXITING, "exiting"));
...
ISC_TASKFUNC_SCOPE isc_result_t
isc__taskmgr_create (isc_mem_t * mctx, unsigned int workers, unsigned int default_quantum, isc_taskmgr_t ** managerp)
{
    isc_result_t result;
    ...
    for (i = 0; i < workers; i++)
    {
        if (isc_thread_create (run, manager, &manager->threads[manager->workers]) == ISC_R_SUCCESS)
        {
            manager->workers++;
            started++;
        }
...
ISC_TASKFUNC_SCOPE isc_result_t isc__task_beginexclusive (isc_task_t * task0)
{
#ifdef USE_WORKER_THREADS
    isc__task_t *task = (isc__task_t *) task0;
...
isc_result_t isc__task_register ()
{
...
#if defined(HAVE_LIBXML2) && defined(BIND9)
void isc_taskmgr_renderxml (isc_taskmgr_t * mgr0, xmlTextWriterPtr writer)
{
    ...
    while (task != NULL)
    {
        ...
        if (task->name[0] != 0)
        {
            ...
            xmlTextWriterWriteFormatString (writer, "%s", task->name);
        ...
        xmlTextWriterWriteFormatString (writer, "%s", statenames[task->state]);
