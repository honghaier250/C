...
isc_result_t isc_base64_totext (isc_region_t * source, int wordlength, const char *wordbreak, isc_buffer_t * target)
{
    ...
    while (source->length > 2)
    {
        ...
        buf[1] = base64[((source->base[0] << 4) & 0x30) | ((source->base[1] >> 4) & 0x0f)];
...
static inline void base64_decode_init (base64_decode_ctx_t * ctx, int length, isc_buffer_t * target)
{
    ctx->digits = 0;
...
static inline isc_result_t base64_decode_char (base64_decode_ctx_t * ctx, int c)
{
    char *s;
...
static inline isc_result_t base64_decode_finish (base64_decode_ctx_t * ctx)
{
    if (ctx->length > 0)
