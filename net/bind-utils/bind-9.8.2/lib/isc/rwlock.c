...
static void print_lock (const char *operation, isc_rwlock_t * rwl, isc_rwlocktype_t type)
{
    fprintf (stderr,
             ...
             rwl->active, rwl->granted, rwl->readers_waiting, rwl->writers_waiting);
}
#endif
...
isc_result_t isc_rwlock_init (isc_rwlock_t * rwl, unsigned int read_quota, unsigned int write_quota)
{
    ...
    if (read_quota != 0)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__, "read quota is not supported");
    }
    if (write_quota == 0)
    ...
    if (result != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          ...
                          isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL,
                                          ISC_MSG_FAILED, "failed"), isc_result_totext (result));
...
#if defined(ISC_PLATFORM_HAVEXADD) && defined(ISC_PLATFORM_HAVECMPXCHG)
    REQUIRE (rwl->write_requests == rwl->write_completions && rwl->cnt_and_flag == 0 && rwl->readers_waiting == 0);
#else
    LOCK (&rwl->lock);
    REQUIRE (rwl->active == 0 && rwl->readers_waiting == 0 && rwl->writers_waiting == 0);
    UNLOCK (&rwl->lock);
#endif
...
isc_result_t isc_rwlock_lock (isc_rwlock_t * rwl, isc_rwlocktype_t type)
{
...
#ifdef ISC_RWLOCK_TRACE
    print_lock (isc_msgcat_get (isc_msgcat, ISC_MSGSET_RWLOCK, ISC_MSG_PRELOCK, "prelock"), rwl, type);
#endif
    ...
    else
    {
        ...
        while (1)
        {
            cntflag = isc_atomic_cmpxchg (&rwl->cnt_and_flag, 0, WRITER_ACTIVE);
            if (cntflag == 0)
                break;
...
#ifdef ISC_RWLOCK_TRACE
    print_lock (isc_msgcat_get (isc_msgcat, ISC_MSGSET_RWLOCK, ISC_MSG_POSTLOCK, "postlock"), rwl, type);
#endif
...
isc_result_t isc_rwlock_trylock (isc_rwlock_t * rwl, isc_rwlocktype_t type)
{
    ...
    if (type == isc_rwlocktype_read)
    {
        ...
        if ((cntflag & WRITER_ACTIVE) != 0)
        {
             ...
             */
            if (cntflag == READER_INCR && rwl->write_completions != rwl->write_requests)
            {
                LOCK (&rwl->lock);
...
isc_result_t isc_rwlock_tryupgrade (isc_rwlock_t * rwl)
{
    ...
    prevcnt = isc_atomic_cmpxchg (&rwl->cnt_and_flag, READER_INCR, WRITER_ACTIVE);
    /*
     * There must have been no writer, and there must have been at least
     * one reader.
     */
    INSIST ((prevcnt & WRITER_ACTIVE) == 0 && (prevcnt & ~WRITER_ACTIVE) != 0);
...
isc_result_t isc_rwlock_unlock (isc_rwlock_t * rwl, isc_rwlocktype_t type)
{
...
#ifdef ISC_RWLOCK_TRACE
    print_lock (isc_msgcat_get (isc_msgcat, ISC_MSGSET_RWLOCK, ISC_MSG_PREUNLOCK, "preunlock"), rwl, type);
#endif
    ...
    if (type == isc_rwlocktype_read)
    {
         ...
         * FIFO order.
         */
        if (prev_cnt == READER_INCR && rwl->write_completions != rwl->write_requests)
        {
            LOCK (&rwl->lock);
            BROADCAST (&rwl->writeable);
            UNLOCK (&rwl->lock);
    ...
    else
    {
        ...
        if (rwl->write_granted >= rwl->write_quota ||
            rwl->write_requests == rwl->write_completions || (rwl->cnt_and_flag & ~WRITER_ACTIVE) != 0)
        {
            /*
        ...
        if (rwl->write_requests != rwl->write_completions && wakeup_writers)
        {
            LOCK (&rwl->lock);
            BROADCAST (&rwl->writeable);
            UNLOCK (&rwl->lock);
...
#ifdef ISC_RWLOCK_TRACE
    print_lock (isc_msgcat_get (isc_msgcat, ISC_MSGSET_RWLOCK, ISC_MSG_POSTUNLOCK, "postunlock"), rwl, type);
#endif
...
static isc_result_t doit (isc_rwlock_t * rwl, isc_rwlocktype_t type, isc_boolean_t nonblock)
{
    ...
    if (type == isc_rwlocktype_read)
    {
        ...
        while (!done)
        {
            if (!skip &&
                ((rwl->active == 0 ||
                  (rwl->type == isc_rwlocktype_read && (rwl->writers_waiting == 0 || rwl->granted < rwl->read_quota)))))
            {
                rwl->type = isc_rwlocktype_read;
                rwl->active++;
                rwl->granted++;
...
isc_result_t isc_rwlock_tryupgrade (isc_rwlock_t * rwl)
{
    ...
    if (rwl->active == 1)
    {
        rwl->original = (rwl->original == isc_rwlocktype_none) ? isc_rwlocktype_read : isc_rwlocktype_none;
        rwl->type = isc_rwlocktype_write;
    }
    else
        result = ISC_R_LOCKBUSY;
...
void isc_rwlock_downgrade (isc_rwlock_t * rwl)
{
    ...
    rwl->type = isc_rwlocktype_read;
    rwl->original = (rwl->original == isc_rwlocktype_none) ? isc_rwlocktype_write : isc_rwlocktype_none;
    /*
     * Resume processing any read request that were blocked when
     * we upgraded.
     */
    if (rwl->original == isc_rwlocktype_none &&
        (rwl->writers_waiting == 0 || rwl->granted < rwl->read_quota) && rwl->readers_waiting > 0)
        BROADCAST (&rwl->readable);
...
isc_result_t isc_rwlock_unlock (isc_rwlock_t * rwl, isc_rwlocktype_t type)
{
    ...
    if (rwl->active == 0)
    {
        ...
        else
        {
            if (rwl->readers_waiting > 0)
            {
                if (rwl->writers_waiting > 0 && rwl->granted < rwl->write_quota)
                {
                    SIGNAL (&rwl->writeable);
                }
                else
