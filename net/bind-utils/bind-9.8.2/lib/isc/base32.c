...
static const char base32[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567=abcdefghijklmnopqrstuvwxyz234567";

static const char base32hex[] = "0123456789ABCDEFGHIJKLMNOPQRSTUV=0123456789abcdefghijklmnopqrstuv";

static isc_result_t
base32_totext (isc_region_t * source, int wordlength, const char *wordbreak, isc_buffer_t * target, const char base[])
{
    char buf[9];
    ...
    while (source->length > 0)
    {
        ...
        loops++;
        if (source->length != 0 && wordlength >= 0 && (int) ((loops + 1) * 8) >= wordlength)
        {
            loops = 0;
...
static inline void base32_decode_init (base32_decode_ctx_t * ctx, int length, const char base[], isc_buffer_t * target)
{
    ctx->digits = 0;
...
static inline isc_result_t base32_decode_char (base32_decode_ctx_t * ctx, int c)
{
    char *s;
...
static inline isc_result_t base32_decode_finish (base32_decode_ctx_t * ctx)
{
    if (ctx->length > 0)
...
static isc_result_t base32_tobuffer (isc_lex_t * lexer, const char base[], isc_buffer_t * target, int length)
{
    ...
    while (!ctx.seen_end && (ctx.length != 0))
    {
        ...
        if (length > 0)
            ...
            eol = ISC_TRUE;
        RETERR (isc_lex_getmastertoken (lexer, &token, isc_tokentype_string, eol));
...
static isc_result_t base32_decodestring (const char *cstr, const char base[], isc_buffer_t * target)
{
    base32_decode_ctx_t ctx;
...
static isc_result_t base32_decoderegion (isc_region_t * source, const char base[], isc_buffer_t * target)
{
    base32_decode_ctx_t ctx;
