...
isc_result_t isc_mem_create (size_t init_max_size, size_t target_size, isc_mem_t ** mctxp)
{
    ...
    result = (*mem_createfunc) (init_max_size, target_size, mctxp, ISC_MEMFLAG_DEFAULT);
...
void isc_mem_setname (isc_mem_t * mctx, const char *name, void *tag)
{
    REQUIRE (ISCAPI_MCTX_VALID (mctx));
...
const char *isc_mem_getname (isc_mem_t * mctx)
{
    REQUIRE (ISCAPI_MCTX_VALID (mctx));
...
void *isc_mem_gettag (isc_mem_t * mctx)
{
    REQUIRE (ISCAPI_MCTX_VALID (mctx));
