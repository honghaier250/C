...
static ISC_LIST (resulttable) tables;
     ...
     static isc_result_t
         register_table (unsigned int base, unsigned int nresults, const char **text, isc_msgcat_t * msgcat, int set)
{
    resulttable *table;
...
static void initialize_action (void)
{
    ...
    result = register_table (ISC_RESULTCLASS_ISC, ISC_R_NRESULTS, text, isc_msgcat, ISC_RESULT_RESULTSET);
    if (result != ISC_R_SUCCESS)
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          "register_table() %s: %u",
                          isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_FAILED, "failed"), result);
}
...
const char *isc_result_totext (isc_result_t result)
{
    ...
    text = NULL;
    for (table = ISC_LIST_HEAD (tables); table != NULL; table = ISC_LIST_NEXT (table, link))
    {
        if (result >= table->base && result <= table->last)
        {
             ...
             */
            text = isc_msgcat_get (table->msgcat, table->set, index + 1, default_text);
            break;
        }
    }
    if (text == NULL)
        text = isc_msgcat_get (isc_msgcat, ISC_RESULT_UNAVAILABLESET, 1, "(result code text not available)");
...
isc_result_t
isc_result_register (unsigned int base, unsigned int nresults, const char **text, isc_msgcat_t * msgcat, int set)
{
    initialize ();
