 ...
 * very large.
 */
static inline void add_entropy (isc_entropy_t * ent, isc_uint32_t entropy)
{
 ...
 */
static inline void subtract_entropy (isc_entropy_t * ent, isc_uint32_t entropy)
{
 ...
 * very large.
 */
static inline void add_pseudo (isc_entropy_t * ent, isc_uint32_t pseudo)
{
    /* clamp input.  Yes, this must be done. */
 ...
 */
static inline void subtract_pseudo (isc_entropy_t * ent, isc_uint32_t pseudo)
{
 ...
 */
static inline void entropypool_add_word (isc_entropypool_t * rp, isc_uint32_t val)
{
    /*
    ...
    if (rp->rotate == 0)
        ...
        rp->pool[rp->cursor++] ^= ((val << rp->rotate) | (val >> (32 - rp->rotate)));
...
static inline void reseed (isc_entropy_t * ent)
{
    isc_time_t t;
...
static inline unsigned int estimate_entropy (sample_queue_t * sq, isc_uint32_t t)
{
    isc_int32_t delta;
...
static unsigned int get_from_callback (isc_entropysource_t * source, unsigned int desired, isc_boolean_t blocking)
{
    ...
    while (desired > 0 && result == ISC_R_SUCCESS)
    {
        ...
        else if (result != ISC_R_SUCCESS && result != ISC_R_NOTBLOCKING)
            source->bad = ISC_TRUE;
 ...
 */
isc_result_t
isc_entropy_getdata (isc_entropy_t * ent, void *data, unsigned int length, unsigned int *returned, unsigned int flags)
{
    unsigned int i;
...
static inline isc_boolean_t destroy_check (isc_entropy_t * ent)
{
    isc_entropysource_t *source;
...
isc_entropy_createcallbacksource (isc_entropy_t * ent,
                                  ...
                                  isc_entropystop_t stop, void *arg, isc_entropysource_t ** sourcep)
{
    isc_result_t result;
...
static void dumpstats (isc_entropy_t * ent, FILE * out)
{
    fprintf (out,
             ...
             ent->pool.entropy, ent->pool.pseudo, ent->nsources, ent->nextsource, ent->initialized, ent->initcount);
}
                           ...
                           const char *randomfile, int use_keyboard)
{
    ...
    REQUIRE (use_keyboard == ISC_ENTROPY_KEYBOARDYES ||
             use_keyboard == ISC_ENTROPY_KEYBOARDNO || use_keyboard == ISC_ENTROPY_KEYBOARDMAYBE);
    ...
    if (randomfile != NULL && use_keyboard != ISC_ENTROPY_KEYBOARDYES)
    {
        ...
        if (result == ISC_R_SUCCESS && use_keyboard == ISC_ENTROPY_KEYBOARDMAYBE)
    ...
    {
        result = isc_entropy_createcallbacksource (ectx, kbdstart, kbdget, kbdstop, NULL, source);
        if (result == ISC_R_SUCCESS)
            (*source)->warn_keyboard = ISC_TF (use_keyboard == ISC_ENTROPY_KEYBOARDMAYBE);
