...
{
    return (isc_sockaddr_compare (a, b, ISC_SOCKADDR_CMPADDR | ISC_SOCKADDR_CMPPORT | ISC_SOCKADDR_CMPSCOPE));
}
...
isc_boolean_t isc_sockaddr_compare (const isc_sockaddr_t * a, const isc_sockaddr_t * b, unsigned int flags)
{
    ...
    switch (a->type.sa.sa_family)
    {
        case AF_INET:
            if ((flags & ISC_SOCKADDR_CMPADDR) != 0 &&
                memcmp (&a->type.sin.sin_addr, &b->type.sin.sin_addr, sizeof (a->type.sin.sin_addr)) != 0)
                return (ISC_FALSE);
            if ((flags & ISC_SOCKADDR_CMPPORT) != 0 && a->type.sin.sin_port != b->type.sin.sin_port)
                return (ISC_FALSE);
            break;
        case AF_INET6:
            if ((flags & ISC_SOCKADDR_CMPADDR) != 0 &&
                memcmp (&a->type.sin6.sin6_addr, &b->type.sin6.sin6_addr, sizeof (a->type.sin6.sin6_addr)) != 0)
                return (ISC_FALSE);
            ...
            if ((flags & ISC_SOCKADDR_CMPSCOPE) != 0 &&
                ...
                ((flags & ISC_SOCKADDR_CMPSCOPEZERO) == 0 ||
                 (a->type.sin6.sin6_scope_id != 0 && b->type.sin6.sin6_scope_id != 0)))
                return (ISC_FALSE);
#endif
            if ((flags & ISC_SOCKADDR_CMPPORT) != 0 && a->type.sin6.sin6_port != b->type.sin6.sin6_port)
                return (ISC_FALSE);
            break;
        default:
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        snprintf (array, size,
                  isc_msgcat_get (isc_msgcat, ISC_MSGSET_NETADDR,
                                  ISC_MSG_UNKNOWNADDR, "<unknown address, family %u>"), sa->type.sa.sa_family);
        array[size - 1] = '\0';
    }
}
...
unsigned int isc_sockaddr_hash (const isc_sockaddr_t * sockaddr, isc_boolean_t address_only)
{
        ...
        default:
            UNEXPECTED_ERROR (__FILE__, __LINE__,
                              isc_msgcat_get (isc_msgcat,
                                              ...
                                              "unknown address family: %d"), (int) sockaddr->type.sa.sa_family);
    ...
    if (!address_only)
    {
        g = isc_hash_calc ((const unsigned char *) &p, sizeof (p), ISC_TRUE);
...
void isc_sockaddr_anyofpf (isc_sockaddr_t * sockaddr, int pf)
{
    switch (pf)
    {
        case AF_INET:
...
isc_boolean_t isc_sockaddr_ismulticast (const isc_sockaddr_t * sockaddr)
{
    isc_netaddr_t netaddr;

    if (sockaddr->type.sa.sa_family == AF_INET || sockaddr->type.sa.sa_family == AF_INET6)
    {
        isc_netaddr_fromsockaddr (&netaddr, sockaddr);
...
isc_boolean_t isc_sockaddr_isexperimental (const isc_sockaddr_t * sockaddr)
{
    isc_netaddr_t netaddr;
...
isc_boolean_t isc_sockaddr_issitelocal (const isc_sockaddr_t * sockaddr)
{
    isc_netaddr_t netaddr;
...
isc_boolean_t isc_sockaddr_islinklocal (const isc_sockaddr_t * sockaddr)
{
    isc_netaddr_t netaddr;
...
#ifdef ISC_PLATFORM_HAVESALEN
    sockaddr->type.sunix.sun_len = (unsigned char) sizeof (sockaddr->type.sunix);
#endif
