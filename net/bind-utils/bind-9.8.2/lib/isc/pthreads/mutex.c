...
isc_result_t isc_mutex_init_profile (isc_mutex_t * mp, const char *file, int line)
{
    int i, err;
...
isc_result_t isc_mutex_lock_profile (isc_mutex_t * mp, const char *file, int line)
{
    ...
    for (i = 0; i < ISC_MUTEX_MAX_LOCKERS; i++)
    {
        ...
        else if (mp->stats->lockers[i].file == file && mp->stats->lockers[i].line == line)
        {
            locker = &mp->stats->lockers[i];
            break;
        }
...
isc_result_t isc_mutex_unlock_profile (isc_mutex_t * mp, const char *file, int line)
{
    ...
    return ((pthread_mutex_unlock ((&mp->mutex)) == 0) ? ISC_R_SUCCESS : ISC_R_UNEXPECTED);
}


void isc_mutex_statsprofile (FILE * fp)
{
    ...
    for (i = 0; i < stats_next; i++)
    {
        fprintf (fp, "%-12s %4d: %10u  %lu.%06lu %lu.%06lu\n",
                 ...
                 stats[i].locked_total.tv_usec, stats[i].wait_total.tv_sec, stats[i].wait_total.tv_usec);
        for (j = 0; j < ISC_MUTEX_MAX_LOCKERS; j++)
        {
            ...
            fprintf (fp, " %-11s %4d: %10u  %lu.%06lu %lu.%06lu\n",
                     ...
                     locker->locked_total.tv_usec, locker->wait_total.tv_sec, locker->wait_total.tv_usec);
        }
    }
}
...
isc_result_t isc_mutex_init_errcheck (isc_mutex_t * mp)
{
    ...
    err = pthread_mutex_init (mp, &attr) != 0) pthread_mutexattr_destroy (&attr);
    if (err == ENOMEM)
        return (ISC_R_NOMEMORY);
...
isc_result_t isc__mutex_init (isc_mutex_t * mp, const char *file, unsigned int line)
{
    ...
    if (err != 0)
    {
        ...
        UNEXPECTED_ERROR (file, line, "isc_mutex_init() failed: %s", strbuf);
