...
static void _deref_prefix (isc_mem_t * mctx, isc_prefix_t * prefix);
    ...
    if (radix->head != NULL)
    {
        ...
        while (Xrn != NULL)
        {
            ...
            if (Xrn->prefix != NULL)
            {
                ...
                if (func != NULL && (Xrn->data[0] != NULL || Xrn->data[1] != NULL))
                    func (Xrn->data);
            }
            else
            {
                INSIST (Xrn->data[0] == NULL && Xrn->data[1] == NULL);
            }
...
isc_result_t isc_radix_search (isc_radix_tree_t * radix, isc_radix_node_t ** target, isc_prefix_t * prefix)
{
    ...
    *target = NULL;
    ...
    while (cnt-- > 0)
    {
        ...
        if (_comp_with_mask (isc_prefix_tochar (node->prefix), isc_prefix_tochar (prefix), node->prefix->bitlen))
        {
            if (node->node_num[ISC_IS6 (prefix->family)] != -1 &&
                ((*target == NULL) ||
                 (*target)->node_num[ISC_IS6 (tfamily)] > node->node_num[ISC_IS6 (prefix->family)]))
            {
                *target = node;
                  ...
                  isc_radix_node_t * source, isc_prefix_t * prefix)
{
    ...
    if (radix->head == NULL)
    {
        ...
        if (source != NULL)
        {
            ...
            if (source->node_num[0] != -1)
                node->node_num[0] = radix->num_added_node + source->node_num[0];
            if (source->node_num[1] != -1)
                node->node_num[1] = radix->num_added_node + source->node_num[1];
        ...
        else
        {
            ...
            {
                /* "any" or "none" */
                node->node_num[0] = node->node_num[1] = ++radix->num_added_node;
            }
            else
            {
                node->node_num[ISC_IS6 (fam)] = ++radix->num_added_node;
            }
            node->data[0] = NULL;
            node->data[1] = NULL;
        }
    ...
    if (differ_bit == bitlen && node->bit == bitlen)
    {
        if (node->prefix != NULL)
        {
            ...
            {
                /* Merging node */
                if (node->node_num[0] == -1 && source->node_num[0] != -1)
                {
                    node->node_num[0] = radix->num_added_node + source->node_num[0];
                    node->data[0] = source->data[0];
                }
                if (node->node_num[1] == -1 && source->node_num[0] != -1)
                {
                    node->node_num[1] = radix->num_added_node + source->node_num[1];
                    node->data[1] = source->data[1];
                }
        ...
        INSIST (node->data[0] == NULL && node->node_num[0] == -1 && node->data[1] == NULL && node->node_num[1] == -1);
        if (source != NULL)
        {
            /* Merging node */
            if (source->node_num[0] != -1)
    ...
    if (source != NULL)
    {
        /* Merging node */
        if (source->node_num[0] != -1)
            new_node->node_num[0] = radix->num_added_node + source->node_num[0];
        if (source->node_num[1] != -1)
            new_node->node_num[1] = radix->num_added_node + source->node_num[1];
    ...
    else
    {
        ...
        {
            /* "any" or "none" */
            new_node->node_num[0] = new_node->node_num[1] = ++radix->num_added_node;
        }
        else
        {
            new_node->node_num[ISC_IS6 (fam)] = ++radix->num_added_node;
        }
    ...
    else
    {
        ...
        radix->num_active_node++;
        if (differ_bit < radix->maxbits && BIT_TEST (addr[differ_bit >> 3], 0x80 >> (differ_bit & 07)))
        {
            glue->r = new_node;
            glue->l = node;
        }
