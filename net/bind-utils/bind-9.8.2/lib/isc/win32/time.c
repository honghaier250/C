...
void isc_interval_set (isc_interval_t * i, unsigned int seconds, unsigned int nanoseconds)
{
     ...
     */
    i->interval = (LONGLONG) seconds *INTERVALS_PER_S + (nanoseconds + NS_INTERVAL - 1) / NS_INTERVAL;
}
...
void isc_time_formattimestamp (const isc_time_t * t, char *buf, unsigned int len)
{
    ...
    REQUIRE (len > 0);
    if (FileTimeToLocalFileTime (&t->absolute, &localft) && FileTimeToSystemTime (&localft, &st))
    {
        GetDateFormat (LOCALE_USER_DEFAULT, 0, &st, "dd-MMM-yyyy", DateBuf, 50);
        GetTimeFormat (LOCALE_USER_DEFAULT, TIME_NOTIMEMARKER | TIME_FORCE24HOURFORMAT, &st, NULL, TimeBuf, 50);

        snprintf (buf, len, "%s %s.%03u", DateBuf, TimeBuf, st.wMilliseconds);
...
/* strftime() format: "%a, %d %b %Y %H:%M:%S GMT" */
    ...
    {
        GetDateFormat (LOCALE_USER_DEFAULT, 0, &st, "ddd',', dd-MMM-yyyy", DateBuf, 50);
...
/* strtime() format: "%Y-%m-%dT%H:%M:%SZ" */
    ...
    if (FileTimeToSystemTime (&t->absolute, &st))
    {
        GetDateFormat (LOCALE_NEUTRAL, 0, &st, "yyyy-MM-dd", DateBuf, 50);
        GetTimeFormat (LOCALE_NEUTRAL, TIME_NOTIMEMARKER | TIME_FORCE24HOURFORMAT, &st, "hh':'mm':'ss", TimeBuf, 50);
