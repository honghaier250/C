...
static void get_addr (unsigned int family, isc_netaddr_t * dst, struct sockaddr *src)
{
    dst->family = family;
    switch (family)
    {
        case AF_INET:
            memcpy (&dst->type.in, &((struct sockaddr_in *) src)->sin_addr, sizeof (struct in_addr));
            break;
        case AF_INET6:
            memcpy (&dst->type.in6, &((struct sockaddr_in6 *) src)->sin6_addr, sizeof (struct in6_addr));
...
isc_result_t isc_interfaceiter_create (isc_mem_t * mctx, isc_interfaceiter_t ** iterp)
{
    ...
    if ((iter->socket = socket (AF_INET, SOCK_DGRAM, 0)) < 0)
    {
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__, "making interface scan socket: %s", strbuf);
    ...
    for (;;)
    {
                      ...
                      0, 0, iter->buf4, iter->buf4size, &bytesReturned, 0, 0) == SOCKET_ERROR)
        {
            ...
            if (error != WSAEFAULT && error != WSAENOBUFS)
            {
                errno = error;
                isc__strerror (error, strbuf, sizeof (strbuf));
                UNEXPECTED_ERROR (__FILE__, __LINE__, "get interface configuration: %s", strbuf);
                result = ISC_R_UNEXPECTED;
                goto ioctl_failure;
            }
            /*
        ...
        else
        {
             ...
             * case and retry.
             */
            if (bytesReturned > 0 && (bytesReturned < iter->buf4size))
                break;
        }
        if (iter->buf4size >= IFCONF_SIZE_MAX * sizeof (INTERFACE_INFO))
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__, "get interface configuration: " "maximum buffer size exceeded");
            result = ISC_R_UNEXPECTED;
            goto ioctl_failure;
        }
        isc_mem_put (mctx, iter->buf4, iter->buf4size);

        iter->buf4size += IFCONF_SIZE_INCREMENT * sizeof (INTERFACE_INFO);
    }
  ...
  inet6_only:
     ...
     * necessary.
     */
    iter->buf6size = sizeof (SOCKET_ADDRESS_LIST) + IFCONF_SIZE_INITIAL * sizeof (SOCKET_ADDRESS);
    ...
    for (;;)
    {
                      ...
                      0, 0, iter->buf6, iter->buf6size, &bytesReturned, 0, 0) == SOCKET_ERROR)
        {
            ...
            if (error != WSAEFAULT && error != WSAENOBUFS)
            {
                errno = error;
                isc__strerror (error, strbuf, sizeof (strbuf));
                UNEXPECTED_ERROR (__FILE__, __LINE__, "sio address list query: %s", strbuf);
                result = ISC_R_UNEXPECTED;
                goto ioctl6_failure;
            }
            /*
        ...
        iter->buf6size += IFCONF_SIZE_INCREMENT * sizeof (SOCKET_ADDRESS);
    }
...
static isc_result_t internal_current (isc_interfaceiter_t * iter)
{
    ...
    get_addr (AF_INET, &iter->current.address, (struct sockaddr *) &(iter->IFData.iiAddress));
    ...
    {
        get_addr (AF_INET, &iter->current.dstaddress, (struct sockaddr *) &(iter->IFData.iiBroadcastAddress));
    }
     ...
     */
    get_addr (AF_INET, &iter->current.netmask, (struct sockaddr *) &(iter->IFData.iiNetmask));
...
static isc_result_t internal_current6 (isc_interfaceiter_t * iter)
{
    ...
    get_addr (AF_INET6, &iter->current.address, iter->buf6->Address[iter->pos6].lpSockaddr);
    ...
    if (ifNamed == FALSE)
        sprintf (iter->current.name, "TCP/IPv6 Interface %d", iter->pos6 + 1);
