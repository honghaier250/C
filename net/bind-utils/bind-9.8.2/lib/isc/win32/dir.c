...
isc_result_t isc_dir_read (isc_dir_t * dir)
{
    ...
    else
    {
         ...
         */
        if (FindNextFile (dir->search_handle, &dir->entry.find_data) == FALSE)
            /*
 ...
 */
static isc_result_t start_directory (isc_dir_t * dir)
{
     ...
     * Make sure that the space for the name is long enough.
     */
    INSIST (sizeof (dir->entry.name) > strlen (dir->entry.find_data.cFileName));
...
isc_result_t isc_dir_createunique (char *templet)
{
     ...
     */
    for (x = templet + strlen (templet) - 1; *x == 'X' && x >= templet; x--, pid /= 10)
        *x = pid % 10 + '0';
