...
isc_result_t isc_condition_init (isc_condition_t * cond)
{
    HANDLE h;
 ...
 */
static isc_result_t register_thread (unsigned long thrd, isc_condition_t * gblcond, isc_condition_thread_t ** localcond)
{
    HANDLE hc;
...
static isc_result_t
find_thread_condition (unsigned long thrd, isc_condition_t * cond, isc_condition_thread_t ** threadcondp)
{
    ...
    for (threadcond = ISC_LIST_HEAD (cond->threadlist);
         threadcond != NULL; threadcond = ISC_LIST_NEXT (threadcond, link))
    {
...
isc_result_t isc_condition_signal (isc_condition_t * cond)
{
...
isc_result_t isc_condition_broadcast (isc_condition_t * cond)
{
...
isc_result_t isc_condition_destroy (isc_condition_t * cond)
{
 ...
 * the mutex.
 */
static isc_result_t wait (isc_condition_t * cond, isc_mutex_t * mutex, DWORD milliseconds)
{
    DWORD result;
    ...
    result = WaitForMultipleObjects (2, threadcond->handle, FALSE, milliseconds);
...
isc_result_t isc_condition_wait (isc_condition_t * cond, isc_mutex_t * mutex)
{
    return (wait (cond, mutex, INFINITE));
}

isc_result_t isc_condition_waituntil (isc_condition_t * cond, isc_mutex_t * mutex, isc_time_t * t)
{
    DWORD milliseconds;
    ...
    if (microseconds > 0xFFFFFFFFi 64 * 1000)
