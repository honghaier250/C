 ...
 */
void sock_dump (isc_socket_t * sock)
{
 ...
 */
     static void signal_iocompletionport_exit (isc_socketmgr_t * manager)
{
    int i;
        ...
        if (!PostQueuedCompletionStatus (manager->hIoCompletionPort, 0, 0, 0))
        {
            ...
            FATAL_ERROR (__FILE__, __LINE__,
                         isc_msgcat_get (isc_msgcat, ISC_MSGSET_SOCKET,
                                         ISC_MSG_FAILED, "Can't request service thread to exit: %s"), strbuf);
        }
    }
}
 ...
 */
void iocompletionport_createthreads (int total_threads, isc_socketmgr_t * manager)
{
    int errval;
    ...
    {
        manager->hIOCPThreads[i] = CreateThread (NULL, 0, SocketIoThread, manager, 0, &manager->dwIOCPThreadIds[i]);
        if (manager->hIOCPThreads[i] == NULL)
        {
            ...
            FATAL_ERROR (__FILE__, __LINE__,
                         isc_msgcat_get (isc_msgcat, ISC_MSGSET_SOCKET,
                                         ISC_MSG_FAILED, "Can't create IOCP thread: %s"), strbuf);
            exit (1);
        }
    }
}
 ...
 */
void iocompletionport_init (isc_socketmgr_t * manager)
{
    int errval;
    ...
    if (hHeapHandle == NULL)
    {
        ...
        FATAL_ERROR (__FILE__, __LINE__,
                     isc_msgcat_get (isc_msgcat, ISC_MSGSET_SOCKET,
                                     ISC_MSG_FAILED, "HeapCreate() failed during " "initialization: %s"), strbuf);
        exit (1);
    }
    ...
    manager->hIoCompletionPort = CreateIoCompletionPort (INVALID_HANDLE_VALUE, NULL, 0, manager->maxIOCPThreads);
    if (manager->hIoCompletionPort == NULL)
    {
        ...
        FATAL_ERROR (__FILE__, __LINE__,
                     isc_msgcat_get (isc_msgcat, ISC_MSGSET_SOCKET,
                                     ISC_MSG_FAILED,
                                     "CreateIoCompletionPort() failed " "during initialization: %s"), strbuf);
        exit (1);
    }
 ...
 */
void iocompletionport_update (isc_socket_t * sock)
{
    HANDLE hiocp;
    ...
    hiocp = CreateIoCompletionPort ((HANDLE) sock->fd, sock->manager->hIoCompletionPort, (ULONG_PTR) sock, 0);
    ...
    if (hiocp == NULL)
    {
        ...
        isc_log_iwrite (isc_lctx,
                        ...
                        "iocompletionport_update: failed to open" " io completion port: %s", strbuf);
 ...
 */
void socket_close (isc_socket_t * sock)
{
...
static void initialise (void)
{
    ...
    err = WSAIoctl (sock, SIO_GET_EXTENSION_FUNCTION_POINTER,
                    &GUIDConnectEx, sizeof (GUIDConnectEx), &ISCConnectEx, sizeof (ISCConnectEx), &dwBytes, NULL, NULL);
    INSIST (err == 0);

    err = WSAIoctl (sock, SIO_GET_EXTENSION_FUNCTION_POINTER,
                    &GUIDAcceptEx, sizeof (GUIDAcceptEx), &ISCAcceptEx, sizeof (ISCAcceptEx), &dwBytes, NULL, NULL);
    INSIST (err == 0);

    err = WSAIoctl (sock, SIO_GET_EXTENSION_FUNCTION_POINTER,
                    &GUIDGetAcceptExSockaddrs, sizeof (GUIDGetAcceptExSockaddrs),
                    &ISCGetAcceptExSockaddrs, sizeof (ISCGetAcceptExSockaddrs), &dwBytes, NULL, NULL);
    INSIST (err == 0);
...
{
    RUNTIME_CHECK (isc_once_do (&initialise_once, initialise) == ISC_R_SUCCESS);
    if (!initialised)
        exit (1);
}

int internal_sendmsg (isc_socket_t * sock, IoCompletionInfo * lpo, struct msghdr *messagehdr, int flags, int *Error)
{
    int Result;
...
static void queue_receive_request (isc_socket_t * sock)
{
  ...
  retry:
    ...
    if (lpo == NULL)
    {
        lpo = (IoCompletionInfo *) HeapAlloc (hHeapHandle, HEAP_ZERO_MEMORY, sizeof (IoCompletionInfo));
    ...
    Result = WSARecvFrom ((SOCKET) sock->fd, iov, 1,
                          ...
                          &sock->recvbuf.from_addr_len, (LPWSAOVERLAPPED) lpo, NULL);
            ...
            default:
                ...
                if (isc_result == ISC_R_UNEXPECTED)
                    UNEXPECTED_ERROR (__FILE__, __LINE__,
                                      "WSARecvFrom: Windows error code: %d, isc result %d", Error, isc_result);
    ...
    socket_log (__LINE__, sock, NULL, IOEVENT,
                isc_msgcat, ISC_MSGSET_SOCKET,
                ISC_MSG_DOIORECV, "queue_io_request: fd %d result %d error %d", sock->fd, Result, Error);
            ...
            isc_msgcat_t * msgcat, int msgset, int message, const char *fmt, ...)
{
    ...
    if (address == NULL)
    {
        isc_log_iwrite (isc_lctx, category, module, level,
                        msgcat, msgset, message, "socket %p line %d: %s", sock, lineno, msgbuf);
    }
    else
    {
        isc_sockaddr_format (address, peerbuf, sizeof (peerbuf));
        isc_log_iwrite (isc_lctx, category, module, level,
                        msgcat, msgset, message, "socket %p line %d peer %s: %s", sock, lineno, peerbuf, msgbuf);
    }
 ...
 */
static isc_result_t make_nonblock (SOCKET fd)
{
    int ret;
    ...
    if (ret == -1)
    {
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__, "ioctlsocket(%d, FIOBIO, %d): %s", fd, flags, strbuf);
 ...
 */
isc_result_t connection_reset_fix (SOCKET fd)
{
...
build_msghdr_send (isc_socket_t * sock, isc_socketevent_t * dev,
                   struct msghdr *msg, char *cmsg, WSABUF * iov, IoCompletionInfo * lpo)
{
    ...
    if (buffer == NULL)
    {
        ...
        socket_log (__LINE__, sock, NULL, TRACE,
                    ...
                    "alloc_buffer %p %d %p %d", cpbuffer, sizeof (buflist_t), cpbuffer->buf, write_count);
...
static isc_socketevent_t *allocate_socketevent (isc_socket_t * sock, isc_eventtype_t eventtype,
...
static void dump_msg (struct msghdr *msg, isc_socket_t * sock)
{
    unsigned int i;
    ...
    for (i = 0; i < (unsigned int) msg->msg_iovlen; i++)
        printf ("\t\t%d\tbase %p, len %d\n", i, msg->msg_iov[i].buf, msg->msg_iov[i].len);
}
#endif
 ...
 * map the error code
 */
int map_socket_error (isc_socket_t * sock, int windows_errno, int *isc_errno, char *errorstring, size_t bufsize)
{
...
static void fill_recv (isc_socket_t * sock, isc_socketevent_t * dev)
{
    isc_region_t r;
    ...
    if (sock->type == isc_sockettype_udp)
    {
        ...
        memcpy (&dev->address.type, &sock->recvbuf.from_addr, sock->recvbuf.from_addr_len);
 ...
 */
static void completeio_recv (isc_socket_t * sock)
{
 ...
 */
static int
completeio_send (isc_socket_t * sock, isc_socketevent_t * dev, struct msghdr *messagehdr, int cc, int send_errno)
{
    ...
    if (send_errno != 0)
    {
        ...
        return (map_socket_error (sock, send_errno, &dev->result, strbuf, sizeof (strbuf)));
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__, "completeio_send: %s: %s", addrbuf, strbuf);
...
static int startio_send (isc_socket_t * sock, isc_socketevent_t * dev, int *nbytes, int *send_errno)
{
    char *cmsg = NULL;
    ...
    if (*nbytes < 0)
    {
        ...
        if (isc_log_wouldlog (isc_lctx, IOEVENT_LEVEL))
        {
            ...
            socket_log (__LINE__, sock, NULL, IOEVENT,
                        ...
                        "bytes, err %d/%s", sock->fd, *nbytes, *send_errno, strbuf);
        }
        status = DOIO_HARD;
        goto done;
    }
...
static isc_result_t allocate_socket (isc_socketmgr_t * manager, isc_sockettype_t type, isc_socket_t ** socketp)
{
    isc_socket_t *sock;
    ...
    socket_log (__LINE__, sock, NULL, EVENT, NULL, 0, 0, "allocated");
...
static void consistent (isc_socket_t * sock)
{
    ...
    if (crash)
    {
        socket_log (__LINE__, sock, NULL, CREATION, isc_msgcat, ISC_MSGSET_SOCKET,
                    ISC_MSG_DESTROYING, "SOCKET INCONSISTENT: %s", crash_reason);
        sock_dump (sock);
 ...
 */
static void maybe_free_socket (isc_socket_t ** socketp, int lineno)
{
    ...
    *socketp = NULL;
    ...
    if (sock->pending_iocp > 0
        ...
        || !ISC_LIST_EMPTY (sock->send_list) || !ISC_LIST_EMPTY (sock->accept_list) || sock->fd != INVALID_SOCKET)
    {
        UNLOCK (&sock->lock);
        return;
    }
...
void free_socket (isc_socket_t ** sockp, int lineno)
{
    isc_socketmgr_t *manager;
 ...
 * in 'socketp'.
 */
isc_result_t isc__socket_create (isc_socketmgr_t * manager, int pf, isc_sockettype_t type, isc_socket_t ** socketp)
{
            ...
            if (sock->fd != INVALID_SOCKET)
            {
                ...
                if (result != ISC_R_SUCCESS)
                {
                    socket_log (__LINE__, sock, NULL, EVENT, NULL, 0, 0,
                                ...
                                sock->pending_recv, sock->pending_send, sock->references);
                    closesocket (sock->fd);
                    _set_state (sock, SOCK_CLOSED);
                    sock->fd = INVALID_SOCKET;
    ...
    {
        socket_log (__LINE__, sock, NULL, EVENT, NULL, 0, 0,
                    "closed %d %d %d make_nonblock_failed", sock->pending_recv, sock->pending_send, sock->references);
        closesocket (sock->fd);
        sock->fd = INVALID_SOCKET;
        free_socket (&sock, __LINE__);
        return (result);
    ...
    if (type == isc_sockettype_udp)
    {
        ...
        /* 2292bis */
        if ((pf == AF_INET6) && (setsockopt (sock->fd, IPPROTO_IPV6, IPV6_RECVPKTINFO, (char *) &on, sizeof (on)) < 0))
        {
...
#else
        /* 2292 */
        if ((pf == AF_INET6) && (setsockopt (sock->fd, IPPROTO_IPV6, IPV6_PKTINFO, (char *) &on, sizeof (on)) < 0))
        {
            isc__strerror (WSAGetLastError (), strbuf, sizeof (strbuf));
            UNEXPECTED_ERROR (__FILE__, __LINE__,
        ...
        if (pf == AF_INET6)
        {
            (void) setsockopt (sock->fd, IPPROTO_IPV6, IPV6_USE_MIN_MTU, (char *) &on, sizeof (on));
        }
#endif
...
#if defined(SO_RCVBUF)
        optlen = sizeof (size);
        if (getsockopt (sock->fd, SOL_SOCKET, SO_RCVBUF, (char *) &size, &optlen) >= 0 && size < RCVBUFSIZE)
        {
            size = RCVBUFSIZE;
            (void) setsockopt (sock->fd, SOL_SOCKET, SO_RCVBUF, (char *) &size, sizeof (size));
        }
#endif
...
void isc__socket_detach (isc_socket_t ** socketp)
{
    ...
    socket_log (__LINE__, sock, NULL, EVENT, NULL, 0, 0,
                "detach_socket %d %d %d", sock->pending_recv, sock->pending_send, sock->references);
 ...
 */
static void internal_accept (isc_socket_t * sock, IoCompletionInfo * lpo, int accept_errno)
{
    ...
    socket_log (__LINE__, sock, NULL, TRACE,
                isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_ACCEPTLOCK, "internal_accept called");
    ...
    ISCGetAcceptExSockaddrs (lpo->acceptbuffer, 0,
                             ...
                             (LPSOCKADDR *) & localaddr, &localaddr_len, (LPSOCKADDR *) & remoteaddr, &remoteaddr_len);
    ...
    socket_log (__LINE__, nsock, &nsock->address, TRACE,
                isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_ACCEPTLOCK, "internal_accept parent %p", sock);

    result = make_nonblock (adev->newsocket->fd);
    INSIST (result == ISC_R_SUCCESS);

    INSIST (setsockopt (nsock->fd, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT, (char *) &sock->fd, sizeof (sock->fd)) == 0);
 ...
 */
static void internal_connect (isc_socket_t * sock, IoCompletionInfo * lpo, int connect_errno)
{
...
#undef ERROR_MATCH
            default:
                ...
                UNEXPECTED_ERROR (__FILE__, __LINE__, "internal_connect: connect() %s", strbuf);
        }
    }
    else
    {
        ...
        socket_log (__LINE__, sock, &sock->address, IOEVENT,
                    isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_ACCEPTEDCXN, "internal_connect: success");
    }
 ...
 */
static void internal_recv (isc_socket_t * sock, int nbytes)
{
    INSIST (VALID_SOCKET (sock));
    ...
    socket_log (__LINE__, sock, NULL, IOEVENT,
                isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_INTERNALRECV, "internal_recv: %d bytes received", nbytes);
               ...
               struct msghdr *messagehdr, int nbytes, int send_errno, IoCompletionInfo * lpo)
{
    ...
    socket_log (__LINE__, sock, NULL, IOEVENT,
                isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_INTERNALSEND, "internal_send: task got socket event %p", dev);
    ...
    while (buffer != NULL)
    {
        ...
        socket_log (__LINE__, sock, NULL, TRACE,
                    isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_ACCEPTLOCK, "free_buffer %p %p", buffer, buffer->buf);
...
// by the caller.
//
static isc_result_t restart_accept (isc_socket_t * parent, IoCompletionInfo * lpo)
{
...
static isc_threadresult_t WINAPI SocketIoThread (LPVOID ThreadContext)
{
    ...
    if (!SetThreadPriority (GetCurrentThread (), THREAD_PRIORITY_ABOVE_NORMAL))
    {
        ...
        FATAL_ERROR (__FILE__, __LINE__,
                     isc_msgcat_get (isc_msgcat, ISC_MSGSET_SOCKET,
                                     ISC_MSG_FAILED, "Can't set thread priority: %s"), strbuf);
    }
    ...
    while (TRUE)
    {
      wait_again:
        bSuccess = GetQueuedCompletionStatus (manager->hIoCompletionPort,
                                              &nbytes, (LPDWORD) & sock, (LPWSAOVERLAPPED *) & lpo, INFINITE);
            ...
            switch (request)
            {
                case SOCKET_RECV:
                    ...
                    if (!sock->connected &&
                        ((errstatus == ERROR_HOST_UNREACHABLE) ||
                         (errstatus == WSAENETRESET) || (errstatus == WSAECONNRESET)))
                    {
                        /* ignore soft errors */
                        queue_receive_request (sock);
                        break;
                ...
                case SOCKET_SEND:
                    ...
                    if (senddone_is_active (sock, lpo->dev))
                    {
                        ...
                        socket_log (__LINE__, sock, NULL, EVENT, NULL, 0, 0, "canceled_send");
                ...
                case SOCKET_ACCEPT:
                    ...
                    if (acceptdone_is_active (sock, lpo->adev))
                    {
                        ...
                        else
                        {
                            ...
                            isc_result = isc__errno2resultx (errstatus, __FILE__, __LINE__);
                            socket_log (__LINE__, sock, NULL, EVENT, NULL, 0, 0,
                                        "restart_accept() failed: errstatus=%d isc_result=%d", errstatus, isc_result);
                        }
                    }
                    ...
                    if (acceptdone_is_active (sock, lpo->adev))
                    {
                        ...
                        socket_log (__LINE__, sock, NULL, EVENT, NULL, 0, 0, "canceled_accept");
                ...
                case SOCKET_CONNECT:
                    ...
                    if (connectdone_is_active (sock, lpo->cdev))
                    {
                        ...
                        socket_log (__LINE__, sock, NULL, EVENT, NULL, 0, 0, "canceled_connect");
    ...
    manager_log (manager, TRACE,
                 isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_EXITING, "SocketIoThread exiting"));
...
isc_result_t isc__socketmgr_getmaxsockets (isc_socketmgr_t * manager, unsigned int *nsockp)
{
    REQUIRE (VALID_MANAGER (manager));
    REQUIRE (nsockp != NULL);
...
void isc__socketmgr_setstats (isc_socketmgr_t * manager, isc_stats_t * stats)
{
    REQUIRE (VALID_MANAGER (manager));
...
void isc__socketmgr_destroy (isc_socketmgr_t ** managerp)
{
    ...
    for (i = 0; i < manager->maxIOCPThreads; i++)
    {
        if (isc_thread_join ((isc_thread_t) manager->hIOCPThreads[i], NULL) != ISC_R_SUCCESS)
            UNEXPECTED_ERROR (__FILE__, __LINE__,
...
static void queue_receive_event (isc_socket_t * sock, isc_task_t * task, isc_socketevent_t * dev)
{
    ...
    socket_log (__LINE__, sock, NULL, EVENT, NULL, 0, 0, "queue_receive_event: event %p -> task %p", dev, ntask);
}
 ...
 */
static isc_result_t socket_recv (isc_socket_t * sock, isc_socketevent_t * dev, isc_task_t * task, unsigned int flags)
{
    int cc = 0;
    ...
    switch (io_state)
    {
        ...
        case DOIO_SOFT:
            ...
            if ((flags & ISC_SOCKFLAG_NORETRY) == 0)
            {
                ...
                socket_log (__LINE__, sock, NULL, EVENT, NULL, 0, 0, "socket_send: event %p -> task %p", dev, ntask);
...
isc__socket_sendv (isc_socket_t * sock, isc_bufferlist_t * buflist,
                   isc_task_t * task, isc_taskaction_t action, const void *arg)
{
    return (isc_socket_sendtov (sock, buflist, task, action, arg, NULL, NULL));
}
...
isc_result_t isc__socket_bind (isc_socket_t * sock, isc_sockaddr_t * sockaddr, unsigned int options)
{
    ...
    if ((options & ISC_SOCKET_REUSEADDRESS) != 0 &&
        ...
        setsockopt (sock->fd, SOL_SOCKET, SO_REUSEADDR, (char *) &on, sizeof (on)) < 0)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__,
    ...
    socket_log (__LINE__, sock, sockaddr, TRACE, isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_BOUND, "bound");
    sock->bound = 1;
...
isc_result_t isc__socket_listen (isc_socket_t * sock, unsigned int backlog)
{
    ...
    socket_log (__LINE__, sock, NULL, TRACE, isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_BOUND, "listening");
    sock->listener = 1;
 ...
 */
isc_result_t isc__socket_accept (isc_socket_t * sock, isc_task_t * task, isc_taskaction_t action, const void *arg)
{
    isc_socket_newconnev_t *adev;
    ...
    adev = (isc_socket_newconnev_t *)
        isc_event_allocate (manager->mctx, task, ISC_SOCKEVENT_NEWCONN, action, arg, sizeof (*adev));
    if (adev == NULL)
    {
        UNLOCK (&sock->lock);
        return (ISC_R_NOMEMORY);
    ...
    lpo = (IoCompletionInfo *) HeapAlloc (hHeapHandle, HEAP_ZERO_MEMORY, sizeof (IoCompletionInfo));
    RUNTIME_CHECK (lpo != NULL);
    lpo->acceptbuffer = (void *) HeapAlloc (hHeapHandle, HEAP_ZERO_MEMORY, (sizeof (SOCKADDR_STORAGE) + 16) * 2);
    ...
    socket_log (__LINE__, sock, NULL, TRACE,
                isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_BOUND, "accepting for nsock %p fd %d", nsock, nsock->fd);
                     ...
                     isc_task_t * task, isc_taskaction_t action, const void *arg)
{
    ...
    cdev = (isc_socket_connev_t *) isc_event_allocate (manager->mctx, sock,
                                                       ISC_SOCKEVENT_CONNECT, action, arg, sizeof (*cdev));
    if (cdev == NULL)
    {
        UNLOCK (&sock->lock);
        return (ISC_R_NOMEMORY);
    ...
    if (sock->type == isc_sockettype_tcp)
    {
        ...
        sock->address = *addr;
        ISCConnectEx (sock->fd, &addr->type.sa, addr->length, NULL, 0, NULL, (LPOVERLAPPED) lpo);
...
void isc__socket_ipv6only (isc_socket_t * sock, isc_boolean_t yes)
{
    ...
    if (sock->pf == AF_INET6)
    {
        (void) setsockopt (sock->fd, IPPROTO_IPV6, IPV6_V6ONLY, (char *) &onoff, sizeof (onoff));
    }
#endif
}

void isc__socket_cleanunix (isc_sockaddr_t * addr, isc_boolean_t active)
{
    UNUSED (addr);
    UNUSED (active);
}

isc_result_t isc__socket_permunix (isc_sockaddr_t * addr, isc_uint32_t perm, isc_uint32_t owner, isc_uint32_t group)
{
    UNUSED (addr);
    UNUSED (perm);
    UNUSED (owner);
...
void isc__socket_setname (isc_socket_t * socket, const char *name, void *tag)
{
...
const char *isc__socket_getname (isc_socket_t * socket)
{
    return (socket->name);
}

void *isc__socket_gettag (isc_socket_t * socket)
{
    return (socket->tag);
}

void isc__socketmgr_setreserved (isc_socketmgr_t * manager, isc_uint32_t reserved)
{
    UNUSED (manager);
    UNUSED (reserved);
}

void isc___socketmgr_maxudp (isc_socketmgr_t * manager, int maxudp)
{
...
void isc_socketmgr_renderxml (isc_socketmgr_t * mgr, xmlTextWriterPtr writer)
{
    isc_socket_t *sock;
    ...
    while (sock != NULL)
    {
        ...
        if (sock->connected)
        {
            isc_sockaddr_format (&sock->address, peerbuf, sizeof (peerbuf));
