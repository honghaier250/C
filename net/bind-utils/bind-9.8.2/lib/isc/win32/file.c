...
static int gettemp (char *path, int *doopen)
{
    ...
    for (;;)
    {
        if (doopen)
        {
            if ((*doopen = open (path, O_CREAT | O_EXCL | O_RDWR, _S_IREAD | _S_IWRITE)) >= 0)
                return (1);
            if (errno != EEXIST)
                return (0);
        }
 ...
 */
static isc_result_t file_stats (const char *file, struct stat *stats)
{
...
int isc_file_safemovefile (const char *oldname, const char *newname)
{
    BOOL filestatus;
...
isc_result_t isc_file_getmodtime (const char *file, isc_time_t * time)
{
    ...
    if (!GetFileTime ((HANDLE) _get_osfhandle (fh), NULL, NULL, &time->absolute))
    {
        close (fh);
        errno = EINVAL;
        return (isc__errno2result (errno));
...
isc_result_t isc_file_settime (const char *file, isc_time_t * time)
{
     ...
     */
    if (!SetFileTime ((HANDLE) _get_osfhandle (fh), NULL, &time->absolute, &time->absolute))
    {
        close (fh);
        errno = EINVAL;
        return (isc__errno2result (errno));
...
isc_result_t isc_file_isplainfile (const char *filename)
{
    /*
...
isc_result_t isc_file_splitpath (isc_mem_t * mctx, char *path, char **dirname, char **basename)
{
    ...
    if ((slash != NULL && backslash != NULL && backslash > slash) || (slash == NULL && backslash != NULL))
        slash = backslash;
