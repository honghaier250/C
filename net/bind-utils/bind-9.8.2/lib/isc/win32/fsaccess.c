...
BOOL is_ntfs (const char *file)
{
    ...
    GetVolumeInformation (drive, NULL, 0, NULL, 0, NULL, FSType, sizeof (FSType));
 ...
 * cleared.
 */
isc_result_t FAT_fsaccess_set (const char *path, isc_fsaccess_t access)
{
    int mode;
...
#define SET_AND_CLEAR(user, group, other) \
    ...
    bits = ISC_FSACCESS_WRITE | ISC_FSACCESS_CREATECHILD | ISC_FSACCESS_DELETECHILD;
...
isc_result_t NTFS_Access_Control (const char *filename, const char *user, int access, isc_boolean_t isdir)
{
    ...
    if (!InitializeAcl (pacl, sizeof (aclBuffer), ACL_REVISION))
        return (ISC_R_NOPERM);
    if (!LookupAccountName (0, user, psid, &sidBufferSize, domainBuffer, &domainBufferSize, &snu))
        return (ISC_R_NOPERM);
    domainBufferSize = sizeof (domainBuffer);
    if (!LookupAccountName (0, "Administrators", padminsid, &adminSidBufferSize, domainBuffer, &domainBufferSize, &snu))
    {
        errval = GetLastError ();
        return (ISC_R_NOPERM);
    }
    ...
    if (NTFSbits == (FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE))
...
isc_result_t NTFS_fsaccess_set (const char *path, isc_fsaccess_t access, isc_boolean_t isdir)
{
...
isc_result_t isc_fsaccess_changeowner (const char *filename, const char *user)
{
    ...
    if (!SetFileSecurity (filename, OWNER_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION, &psd))
        return (ISC_R_NOPERM);
