...
static isc_result_t render_404 (const char *, const char *,
                                ...
                                const char **, isc_buffer_t *, isc_httpdfree_t **, void **);
static isc_result_t render_500 (const char *, const char *,
                                void *,
                                unsigned int *, const char **,
                                const char **, isc_buffer_t *, isc_httpdfree_t **, void **);

static void destroy_client (isc_httpd_t ** httpdp)
{
    ...
    *httpdp = NULL;
    ...
    if (httpd->headerlen > 0)
        isc_mem_put (httpdmgr->mctx, httpd->headerdata, httpd->headerlen);
...
isc_httpdmgr_create (isc_mem_t * mctx, isc_socket_t * sock, isc_task_t * task,
                     ...
                     isc_httpdondestroy_t * ondestroy, void *cb_arg, isc_timermgr_t * tmgr, isc_httpdmgr_t ** httpdp)
{
    isc_result_t result;
...
static void httpdmgr_destroy (isc_httpdmgr_t * httpdmgr)
{
    isc_mem_t *mctx;
...
static isc_result_t process_request (isc_httpd_t * httpd, int length)
{
    char *s;
...
static void isc_httpd_accept (isc_task_t * task, isc_event_t * ev)
{
    isc_result_t result;
    ...
    isc_buffer_init (&httpd->headerbuffer, httpd->headerdata, httpd->headerlen);
    ...
    result = isc_socket_recv (httpd->sock, &r, 1, task, isc_httpd_recvdone, httpd);
    /* FIXME!!! */
    POST (result);
...
render_404 (const char *url, const char *querystring,
            ...
            const char **mimetype, isc_buffer_t * b, isc_httpdfree_t ** freecb, void **freecb_args)
{
...
static void isc_httpd_recvdone (isc_task_t * task, isc_event_t * ev)
{
    isc_region_t r;
    ...
    if (result == ISC_R_NOTFOUND)
    {
        ...
        /* check return code? */
        (void) isc_socket_recv (httpd->sock, &r, 1, task, isc_httpd_recvdone, httpd);
        goto out;
    }
    ...
    if (url == NULL)
        result = httpd->mgr->render_404 (httpd->url, httpd->querystring,
                                         ...
                                         &httpd->retcode,
                                         &httpd->retmsg,
                                         &httpd->mimetype, &httpd->bodybuffer, &httpd->freecb, &httpd->freecb_arg);
    else
    ...
    isc_httpd_addheaderuint (httpd, "Content-Length", isc_buffer_usedlength (&httpd->bodybuffer));
    ...
    /* check return code? */
    (void) isc_socket_sendv (httpd->sock, &httpd->bufflist, task, isc_httpd_senddone, httpd);
...
void isc_httpdmgr_shutdown (isc_httpdmgr_t ** httpdmgrp)
{
    ...
    *httpdmgrp = NULL;
    ...
    while (httpd != NULL)
    {
        isc_socket_cancel (httpd->sock, httpdmgr->task, ISC_SOCKCANCEL_ALL);
...
isc_result_t isc_httpd_addheader (isc_httpd_t * httpd, const char *name, const char *val)
{
    ...
    if (val != NULL)
        sprintf (isc_buffer_used (&httpd->headerbuffer), "%s: %s\r\n", name, val);
    else
        sprintf (isc_buffer_used (&httpd->headerbuffer), "%s\r\n", name);
...
static void isc_httpd_senddone (isc_task_t * task, isc_event_t * ev)
{
    isc_httpd_t *httpd = ev->ev_arg;
