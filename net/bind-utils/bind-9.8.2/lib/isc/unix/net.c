...
static isc_result_t try_proto (int domain)
{
    ...
    if (domain == PF_INET6)
    {
        ...
        if (getsockname (s, (struct sockaddr *) &sin6, (void *) &len) < 0)
        {
            ...
            isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                           ISC_LOGMODULE_SOCKET, ISC_LOG_ERROR, "IPv6 is not supported.");
        ...
        else
        {
            ...
            else
            {
                isc_log_write (isc_lctx,
                               ...
                               ISC_LOG_ERROR, "IPv6 structures in kernel and " "user space do not match.");
                isc_log_write (isc_lctx,
                               ISC_LOGCATEGORY_GENERAL, ISC_LOGMODULE_SOCKET, ISC_LOG_ERROR, "IPv6 is not supported.");
                result = ISC_R_NOTFOUND;
            }
        }
    }
...
static isc_result_t getudpportrange_sysctl (int af, in_port_t * low, in_port_t * high)
{
