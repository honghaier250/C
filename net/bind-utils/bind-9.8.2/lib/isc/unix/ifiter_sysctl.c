...
isc_result_t isc_interfaceiter_create (isc_mem_t * mctx, isc_interfaceiter_t ** iterp)
{
    ...
    if (sysctl (mib, 6, NULL, &bufsize, NULL, (size_t) 0) < 0)
    {
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          isc_msgcat_get (isc_msgcat,
                                          ...
                                          ISC_MSG_GETIFLISTSIZE, "getting interface " "list size: sysctl: %s"), strbuf);
    ...
    if (sysctl (mib, 6, iter->buf, &bufused, NULL, (size_t) 0) < 0)
    {
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          isc_msgcat_get (isc_msgcat,
                                          ISC_MSGSET_IFITERSYSCTL,
                                          ISC_MSG_GETIFLIST, "getting interface list: " "sysctl: %s"), strbuf);
        result = ISC_R_UNEXPECTED;
        goto failure;
    }
...
static isc_result_t internal_current (isc_interfaceiter_t * iter)
{
    ...
    else if (ifam->ifam_type == RTM_NEWADDR)
    {
        ...
        for (i = 0; i < RTAX_MAX; i++)
        {
             ...
             */
            sa = (struct sockaddr *) ((char *) (sa) + ROUNDUP (_FAKE_SA_LEN_DST (sa)));
#else
            /* XXX untested. */
            sa = (struct sockaddr *) ((char *) (sa) + ROUNDUP (sizeof (struct sockaddr)));
#endif
#endif
        }
        ...
        get_addr (family, &iter->current.address, addr_sa, iter->current.name);

        if (mask_sa != NULL)
            get_addr (family, &iter->current.netmask, mask_sa, iter->current.name);

        if (dst_sa != NULL && (iter->current.flags & INTERFACE_F_POINTTOPOINT) != 0)
            get_addr (family, &iter->current.dstaddress, dst_sa, iter->current.name);
    ...
    else
    {
        printf (isc_msgcat_get (isc_msgcat, ISC_MSGSET_IFITERSYSCTL,
                                ISC_MSG_UNEXPECTEDTYPE, "warning: unexpected interface list " "message type\n"));
        return (ISC_R_IGNORE);
    }
}
