...
static void get_addr (unsigned int family, isc_netaddr_t * dst, struct sockaddr *src, char *ifname)
{
    ...
    switch (family)
    {
        ...
        case AF_INET6:
            ...
            memcpy (&dst->type.in6, &sa6->sin6_addr, sizeof (struct in6_addr));
            ...
            else
            {
                ...
                if (IN6_IS_ADDR_LINKLOCAL (&sa6->sin6_addr))
                {
                    ...
                    memcpy (&zone16, &sa6->sin6_addr.s6_addr[2], sizeof (zone16));
                    zone16 = ntohs (zone16);
                    if (zone16 != 0)
                    {
                        /* the zone ID is embedded */
                        isc_netaddr_setzone (dst, (isc_uint32_t) zone16);
                    ...
                    else if (ifname != NULL)
                    {
                        ...
                        if (zone != 0)
                        {
                            isc_netaddr_setzone (dst, (isc_uint32_t) zone);
                        }
#endif
                    }
                }
...
static isc_result_t linux_if_inet6_next (isc_interfaceiter_t * iter)
{
    if (iter->proc != NULL && fgets (iter->entry, sizeof (iter->entry), iter->proc) != NULL)
...
static isc_result_t linux_if_inet6_current (isc_interfaceiter_t * iter)
{
    char address[33];
    ...
    if (iter->proc == NULL)
    {
        isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                       ISC_LOGMODULE_INTERFACE, ISC_LOG_ERROR, "/proc/net/if_inet6:iter->proc == NULL");
    ...
    {
        isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                       ISC_LOGMODULE_INTERFACE, ISC_LOG_ERROR, "/proc/net/if_inet6:sscanf() -> %d (expected 6)", res);
        return (ISC_R_FAILURE);
    }
    if (strlen (address) != 32)
    {
        isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                       ISC_LOGMODULE_INTERFACE, ISC_LOG_ERROR, "/proc/net/if_inet6:strlen(%s) != 32", address);
        return (ISC_R_FAILURE);
    }
    for (i = 0; i < 16; i++)
    {
        ...
        byte = ((strchr (hex, address[i * 2]) - hex) << 4) | (strchr (hex, address[i * 2 + 1]) - hex);
