...
static isc_result_t resource2rlim (isc_resource_t resource, int *rlim_resource)
{
    ...
    switch (resource)
    {
        ...
        default:
             ...
             */
            REQUIRE (resource >= isc_resource_coresize && resource <= isc_resource_stacksize);
...
isc_result_t isc_resource_setlimit (isc_resource_t resource, isc_resourcevalue_t value)
{
    ...
    else
    {
        ...
        isc_boolean_t rlim_t_is_signed = ISC_TF (((double) (ISC_PLATFORM_RLIMITTYPE) - 1) < 0);

        if (rlim_t_is_signed)
            rlim_max = ~((ISC_PLATFORM_RLIMITTYPE) 1 << (sizeof (ISC_PLATFORM_RLIMITTYPE) * 8 - 1));
        else
