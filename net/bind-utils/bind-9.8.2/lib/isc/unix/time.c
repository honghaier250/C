...
isc_result_t isc_time_now (isc_time_t * t)
{
     ...
     */
    if (sizeof (tv.tv_sec) > sizeof (t->seconds) && ((tv.tv_sec | (unsigned int) -1) ^ (unsigned int) -1) != 0U)
        return (ISC_R_RANGE);
...
isc_result_t isc_time_nowplusinterval (isc_time_t * t, const isc_interval_t * i)
{
     ...
     */
    if ((tv.tv_sec > INT_MAX || i->seconds > INT_MAX) && ((long long) tv.tv_sec + i->seconds > UINT_MAX))
        return (ISC_R_RANGE);
...
isc_result_t isc_time_add (const isc_time_t * t, const isc_interval_t * i, isc_time_t * result)
{
     ...
     * and getting another 1 added below the result is UINT_MAX.)
     */
    if ((t->seconds > INT_MAX || i->seconds > INT_MAX) && ((long long) t->seconds + i->seconds > UINT_MAX))
        return (ISC_R_RANGE);
...
isc_result_t isc_time_subtract (const isc_time_t * t, const isc_interval_t * i, isc_time_t * result)
{
    ...
    if ((unsigned int) t->seconds < i->seconds ||
        ((unsigned int) t->seconds == i->seconds && t->nanoseconds < i->nanoseconds))
        return (ISC_R_RANGE);
...
void isc_time_formattimestamp (const isc_time_t * t, char *buf, unsigned int len)
{
    ...
    INSIST (flen < len);
    if (flen != 0)
        snprintf (buf + flen, len - flen, ".%03u", t->nanoseconds / 1000000);
    else
