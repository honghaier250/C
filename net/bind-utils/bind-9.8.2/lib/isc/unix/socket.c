...
typedef enum
{ poll_idle, poll_active, poll_checking } pollstate_t;
...
struct isc__socket
{
    ...
    unsigned int pending_recv:1, pending_send:1, pending_accept:1, listener:1,    /* listener socket */
...
isc__socket_create (isc_socketmgr_t * manager, int pf, isc_sockettype_t type, isc_socket_t ** socketp);
ISC_SOCKETFUNC_SCOPE void isc__socket_attach (isc_socket_t * sock, isc_socket_t ** socketp);

ISC_SOCKETFUNC_SCOPE void isc__socket_detach (isc_socket_t ** socketp);

ISC_SOCKETFUNC_SCOPE isc_result_t isc__socketmgr_create (isc_mem_t * mctx, isc_socketmgr_t ** managerp);

ISC_SOCKETFUNC_SCOPE isc_result_t
isc__socketmgr_create2 (isc_mem_t * mctx, isc_socketmgr_t ** managerp, unsigned int maxsocks);
ISC_SOCKETFUNC_SCOPE void isc__socketmgr_destroy (isc_socketmgr_t ** managerp);

ISC_SOCKETFUNC_SCOPE isc_result_t
isc__socket_recvv (isc_socket_t * sock, isc_bufferlist_t * buflist,
                   unsigned int minimum, isc_task_t * task, isc_taskaction_t action, const void *arg);
ISC_SOCKETFUNC_SCOPE isc_result_t
isc__socket_recv (isc_socket_t * sock, isc_region_t * region,
                  unsigned int minimum, isc_task_t * task, isc_taskaction_t action, const void *arg);
ISC_SOCKETFUNC_SCOPE isc_result_t
isc__socket_recv2 (isc_socket_t * sock, isc_region_t * region,
                   unsigned int minimum, isc_task_t * task, isc_socketevent_t * event, unsigned int flags);
ISC_SOCKETFUNC_SCOPE isc_result_t
...
ISC_SOCKETFUNC_SCOPE void isc__socket_cleanunix (isc_sockaddr_t * sockaddr, isc_boolean_t active);

ISC_SOCKETFUNC_SCOPE isc_result_t
isc__socket_permunix (isc_sockaddr_t * sockaddr, isc_uint32_t perm, isc_uint32_t owner, isc_uint32_t group);
ISC_SOCKETFUNC_SCOPE isc_result_t
isc__socket_bind (isc_socket_t * sock, isc_sockaddr_t * sockaddr, unsigned int options);
ISC_SOCKETFUNC_SCOPE isc_result_t isc__socket_filter (isc_socket_t * sock, const char *filter);

ISC_SOCKETFUNC_SCOPE isc_result_t isc__socket_listen (isc_socket_t * sock, unsigned int backlog);

ISC_SOCKETFUNC_SCOPE isc_result_t
isc__socket_accept (isc_socket_t * sock, isc_task_t * task, isc_taskaction_t action, const void *arg);
ISC_SOCKETFUNC_SCOPE isc_result_t
isc__socket_connect (isc_socket_t * sock, isc_sockaddr_t * addr,
                     isc_task_t * task, isc_taskaction_t action, const void *arg);
ISC_SOCKETFUNC_SCOPE isc_result_t isc__socket_getpeername (isc_socket_t * sock, isc_sockaddr_t * addressp);

ISC_SOCKETFUNC_SCOPE isc_result_t isc__socket_getsockname (isc_socket_t * sock, isc_sockaddr_t * addressp);

ISC_SOCKETFUNC_SCOPE void isc__socket_cancel (isc_socket_t * sock, isc_task_t * task, unsigned int how);

ISC_SOCKETFUNC_SCOPE isc_sockettype_t isc__socket_gettype (isc_socket_t * sock);

ISC_SOCKETFUNC_SCOPE isc_boolean_t isc__socket_isbound (isc_socket_t * sock);

ISC_SOCKETFUNC_SCOPE void isc__socket_ipv6only (isc_socket_t * sock, isc_boolean_t yes);

#if defined(HAVE_LIBXML2) && defined(BIND9)
ISC_SOCKETFUNC_SCOPE void isc__socketmgr_renderxml (isc_socketmgr_t * mgr0, xmlTextWriterPtr writer);
#endif

ISC_SOCKETFUNC_SCOPE isc_result_t
isc__socket_fdwatchcreate (isc_socketmgr_t * manager, int fd, int flags,
                           isc_sockfdwatch_t callback, void *cbarg, isc_task_t * task, isc_socket_t ** socketp);
ISC_SOCKETFUNC_SCOPE isc_result_t isc__socket_fdwatchpoke (isc_socket_t * sock, int flags);
...
static struct
{
     ...
     */
#ifndef BIND9
    void *recvv, *send, *sendv, *sendto2, *cleanunix, *permunix, *filter, *listen, *accept, *getpeername, *isbound;
#endif
} socketmethods =
{
    {
    isc__socket_attach,
            ...
            isc__socket_recv,
            isc__socket_cancel,
            isc__socket_getsockname, isc__socket_gettype, isc__socket_ipv6only, isc__socket_fdwatchpoke}
#ifndef BIND9
    , (void *) isc__socket_recvv, (void *) isc__socket_send,
        ...
        (void *) isc__socket_accept, (void *) isc__socket_getpeername, (void *) isc__socket_isbound
#endif
};
...
manager_log (isc__socketmgr_t * sockmgr,
     ...
     static void
         manager_log (isc__socketmgr_t * sockmgr,
                      isc_logcategory_t * category, isc_logmodule_t * module, int level, const char *fmt, ...)
{
    char msgbuf[2048];
    ...
    isc_log_write (isc_lctx, category, module, level, "sockmgr %p: %s", sockmgr, msgbuf);
}
#endif
...
socket_log (isc__socket_t * sock, isc_sockaddr_t * address,
            isc_logcategory_t * category, isc_logmodule_t * module, int level,
            isc_msgcat_t * msgcat, int msgset, int message, const char *fmt, ...) ISC_FORMAT_PRINTF (9, 10);
     static void
         socket_log (isc__socket_t * sock, isc_sockaddr_t * address,
                     isc_logcategory_t * category, isc_logmodule_t * module, int level,
                     isc_msgcat_t * msgcat, int msgset, int message, const char *fmt, ...)
{
    ...
    if (address == NULL)
    {
        isc_log_iwrite (isc_lctx, category, module, level, msgcat, msgset, message, "socket %p: %s", sock, msgbuf);
    }
    else
    {
        isc_sockaddr_format (address, peerbuf, sizeof (peerbuf));
        isc_log_iwrite (isc_lctx, category, module, level,
                        msgcat, msgset, message, "socket %p %s: %s", sock, peerbuf, msgbuf);
    }
}
 ...
 * setting IPV6_V6ONLY.
 */
static void FIX_IPV6_RECVPKTINFO (isc__socket_t * sock)
{
    ...
    if (setsockopt (sock->fd, IPPROTO_IPV6, IPV6_RECVPKTINFO, (void *) &on, sizeof (on)) < 0)
    {
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          ...
                          "%s: %s", sock->fd,
                          isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_FAILED, "failed"), strbuf);
    }
}
#else
 ...
 */
static inline void inc_stats (isc_stats_t * stats, isc_statscounter_t counterid)
{
    REQUIRE (counterid != -1);
...
static inline isc_result_t watch_fd (isc__socketmgr_t * manager, int fd, int msg)
{
    ...
    event.data.fd = fd;
    if (epoll_ctl (manager->epoll_fd, EPOLL_CTL_ADD, fd, &event) == -1 && errno != EEXIST)
    {
        result = isc__errno2result (errno);
    }
...
static inline isc_result_t unwatch_fd (isc__socketmgr_t * manager, int fd, int msg)
{
    isc_result_t result = ISC_R_SUCCESS;
    ...
    memset (&event.data, 0, sizeof (event.data));
    event.data.fd = fd;
    if (epoll_ctl (manager->epoll_fd, EPOLL_CTL_DEL, fd, &event) == -1 && errno != ENOENT)
    {
        char strbuf[ISC_STRERRORSIZE];

        isc__strerror (errno, strbuf, sizeof (strbuf));
        UNEXPECTED_ERROR (__FILE__, __LINE__, "epoll_ctl(DEL), %d: %s", fd, strbuf);
     ...
     */
    LOCK (&manager->fdlock[lockid]);
    if (msg == SELECT_POKE_READ && manager->fdpollinfo[fd].want_write == 1)
    {
        pfds[1].events = POLLOUT;
        pfds[1].fd = fd;
        writelen += sizeof (pfds[1]);
    }
    if (msg == SELECT_POKE_WRITE && manager->fdpollinfo[fd].want_read == 1)
    {
...
static void wakeup_socket (isc__socketmgr_t * manager, int fd, int msg)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        ...
        isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                       ...
                       "failed to start watching FD (%d): %s", fd, isc_result_totext (result));
    }
}
 ...
 */
static void select_poke (isc__socketmgr_t * mgr, int fd, int msg)
{
    int cc;
    ...
    if (cc < 0)
    {
        ...
        FATAL_ERROR (__FILE__, __LINE__,
                     isc_msgcat_get (isc_msgcat, ISC_MSGSET_SOCKET,
                                     ISC_MSG_WRITEFAILED, "write() failed " "during watcher poke: %s"), strbuf);
    }
 ...
 */
static void select_readmsg (isc__socketmgr_t * mgr, int *fd, int *msg)
{
    int buf[2];
    ...
    if (cc < 0)
    {
        ...
        *fd = -1;                /* Silence compiler. */
        ...
        FATAL_ERROR (__FILE__, __LINE__,
                     isc_msgcat_get (isc_msgcat, ISC_MSGSET_SOCKET,
                                     ISC_MSG_READFAILED, "read() failed " "during watcher poke: %s"), strbuf);
 ...
 * CMSG_SPACE.
 */
static inline ISC_SOCKADDR_LEN_T cmsg_len (ISC_SOCKADDR_LEN_T len)
{
#ifdef CMSG_LEN
    return (CMSG_LEN (len));
#else
...
static inline ISC_SOCKADDR_LEN_T cmsg_space (ISC_SOCKADDR_LEN_T len)
{
#ifdef CMSG_SPACE
 ...
 */
static void process_cmsg (isc__socket_t * sock, struct msghdr *msg, isc_socketevent_t * dev)
{
#ifdef USE_CMSG
    struct cmsghdr *cmsgp;
    ...
    while (cmsgp != NULL)
    {
        socket_log (sock, NULL, TRACE, isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_PROCESSCMSG, "processing cmsg %p", cmsgp);

#ifdef ISC_PLATFORM_HAVEIN6PKTINFO
        if (cmsgp->cmsg_level == IPPROTO_IPV6 && cmsgp->cmsg_type == IPV6_PKTINFO)
        {
            ...
            socket_log (sock, NULL, TRACE,
                        ...
                        ISC_MSG_IFRECEIVED, "interface received on ifindex %u", dev->pktinfo.ipi6_ifindex);
...
#ifdef SO_TIMESTAMP
        if (cmsgp->cmsg_level == SOL_SOCKET && cmsgp->cmsg_type == SCM_TIMESTAMP)
        {
                   ...
                   struct msghdr *msg, struct iovec *iov, size_t * write_countp)
{
    ...
    while (buffer != NULL)
    {
        ...
        if (used.length > 0)
        {
            iov[iovcount].iov_base = (void *) (used.base + skip_count);
  ...
  config:
    ...
    if ((sock->type == isc_sockettype_udp) && ((dev->attributes & ISC_SOCKEVENTATTR_PKTINFO) != 0))
    {
        ...
        socket_log (sock, NULL, TRACE,
                    ...
                    "sendto pktinfo data, ifindex %u", dev->pktinfo.ipi6_ifindex);
         ...
         * is used.
         */
        cmsgp = (struct cmsghdr *) (sock->sendcmsgbuf + msg->msg_controllen);
...
static isc_socketevent_t *allocate_socketevent (isc__socket_t * sock, isc_eventtype_t eventtype,
                                                isc_taskaction_t action, const void *arg)
{
    isc_socketevent_t *ev;

    ev = (isc_socketevent_t *) isc_event_allocate (sock->manager->mctx, sock, eventtype, action, arg, sizeof (*ev));
...
static void dump_msg (struct msghdr *msg)
{
    unsigned int i;

    printf ("MSGHDR %p\n", msg);
    printf ("\tname %p, namelen %ld\n", msg->msg_name, (long) msg->msg_namelen);
...
#ifdef ISC_NET_BSD44MSGHDR
    printf ("\tcontrol %p, controllen %ld\n", msg->msg_control, (long) msg->msg_controllen);
#endif
}
#endif
    ...
    if (cc < 0)
    {
        ...
        if (isc_log_wouldlog (isc_lctx, IOEVENT_LEVEL))
        {
            ...
            socket_log (sock, NULL, IOEVENT,
                        isc_msgcat, ISC_MSGSET_SOCKET,
                        ISC_MSG_DOIORECV,
                        "doio_recv: recvmsg(%d) %d bytes, err %d/%s", sock->fd, cc, recv_errno, strbuf);
        }
...
#undef ALWAYS_HARD
        ...
        inc_stats (sock->manager->stats, sock->statsindex[STATID_RECVFAIL]);
        return (DOIO_HARD);
    }
        ...
        if (isc_sockaddr_getport (&dev->address) == 0)
        {
            if (isc_log_wouldlog (isc_lctx, IOEVENT_LEVEL))
            {
                socket_log (sock, &dev->address, IOEVENT,
                            isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_ZEROPORT, "dropping source port zero packet");
            }
            return (DOIO_SOFT);
        }
        /*
    ...
    socket_log (sock, &dev->address, IOEVENT,
                isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_PKTRECV, "packet received correctly");
...
#undef ALWAYS_HARD
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__, "internal_send: %s: %s", addrbuf, strbuf);
        dev->result = isc__errno2result (send_errno);
        inc_stats (sock->manager->stats, sock->statsindex[STATID_SENDFAIL]);
        return (DOIO_HARD);
    }
    ...
    if (cc == 0)
    {
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          "doio_send: send() %s 0",
                          isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_RETURNED, "returned"));
    }
 ...
 * references exist.
 */
static void closesocket (isc__socketmgr_t * manager, isc__socket_t * sock, int fd)
{
...
static void destroy (isc__socket_t ** sockp)
{
    int fd;
    ...
    socket_log (sock, NULL, CREATION, isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_DESTROYING, "destroying");
...
static isc_result_t allocate_socket (isc__socketmgr_t * manager, isc_sockettype_t type, isc__socket_t ** socketp)
{
    isc__socket_t *sock;
    ...
    ISC_EVENT_INIT (&sock->readable_ev, sizeof (intev_t),
                    ISC_EVENTATTR_NOPURGE, NULL, ISC_SOCKEVENT_INTR, NULL, sock, sock, NULL, NULL);
    ISC_EVENT_INIT (&sock->writable_ev, sizeof (intev_t),
                    ISC_EVENTATTR_NOPURGE, NULL, ISC_SOCKEVENT_INTW, NULL, sock, sock, NULL, NULL);
  ...
  error:
    if (sock->recvcmsgbuf != NULL)
        isc_mem_put (manager->mctx, sock->recvcmsgbuf, sock->recvcmsgbuflen);
    if (sock->sendcmsgbuf != NULL)
        isc_mem_put (manager->mctx, sock->sendcmsgbuf, sock->sendcmsgbuflen);
 ...
 */
static void free_socket (isc__socket_t ** socketp)
{
    ...
    if (sock->recvcmsgbuf != NULL)
        isc_mem_put (sock->manager->mctx, sock->recvcmsgbuf, sock->recvcmsgbuflen);
    if (sock->sendcmsgbuf != NULL)
        isc_mem_put (sock->manager->mctx, sock->sendcmsgbuf, sock->sendcmsgbuflen);
...
static void clear_bsdcompat (void)
{
#ifdef __linux__
    struct utsname buf;
...
static isc_result_t opensocket (isc__socketmgr_t * manager, isc__socket_t * sock)
{
    isc_result_t result;
  ...
  again:
     ...
     */
    if (manager->reserved != 0 && sock->type == isc_sockettype_udp && sock->fd >= 0 && sock->fd < manager->reserved)
    {
        int new, tmp;
    ...
    if (sock->fd >= (int) manager->maxsocks)
    {
        ...
        isc_log_iwrite (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                        ...
                        isc_msgcat, ISC_MSGSET_SOCKET,
                        ISC_MSG_TOOMANYFDS,
                        "socket: file descriptor exceeds limit (%d/%u)", sock->fd, manager->maxsocks);
    ...
    if (sock->fd < 0)
    {
        switch (errno)
        {
            ...
            case ENFILE:
                ...
                isc_log_iwrite (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                                ISC_LOGMODULE_SOCKET, ISC_LOG_ERROR,
                                isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_TOOMANYFDS, "%s: %s", err, strbuf);
                /* fallthrough */
            case ENOBUFS:
                return (ISC_R_NORESOURCES);
...
#ifdef SO_BSDCOMPAT
    RUNTIME_CHECK (isc_once_do (&bsdcompat_once, clear_bsdcompat) == ISC_R_SUCCESS);
    if (sock->type != isc_sockettype_unix && bsdcompat &&
        setsockopt (sock->fd, SOL_SOCKET, SO_BSDCOMPAT, (void *) &on, sizeof (on)) < 0)
    {
        isc__strerror (errno, strbuf, sizeof (strbuf));
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          "setsockopt(%d, SO_BSDCOMPAT) %s: %s",
                          sock->fd, isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_FAILED, "failed"), strbuf);
        /* Press on... */
    }
#endif

#ifdef SO_NOSIGPIPE
    if (setsockopt (sock->fd, SOL_SOCKET, SO_NOSIGPIPE, (void *) &on, sizeof (on)) < 0)
    {
        isc__strerror (errno, strbuf, sizeof (strbuf));
        UNEXPECTED_ERROR (__FILE__, __LINE__,
    ...
    if (sock->type == isc_sockettype_udp)
    {
        ...
        if (setsockopt (sock->fd, SOL_SOCKET, SO_TIMESTAMP, (void *) &on, sizeof (on)) < 0 && errno != ENOPROTOOPT)
        {
            isc__strerror (errno, strbuf, sizeof (strbuf));
            UNEXPECTED_ERROR (__FILE__, __LINE__,
        ...
        if (sock->pf == AF_INET6 && sock->recvcmsgbuflen == 0U)
        {
             ...
             */
            UNEXPECTED_ERROR (__FILE__, __LINE__, "No buffer available to receive " "IPv6 destination");
        }
#ifdef ISC_PLATFORM_HAVEIN6PKTINFO
#ifdef IPV6_RECVPKTINFO
        /* RFC 3542 */
        if ((sock->pf == AF_INET6)
            && (setsockopt (sock->fd, IPPROTO_IPV6, IPV6_RECVPKTINFO, (void *) &on, sizeof (on)) < 0))
        {
            isc__strerror (errno, strbuf, sizeof (strbuf));
            UNEXPECTED_ERROR (__FILE__, __LINE__,
                              "setsockopt(%d, IPV6_RECVPKTINFO) "
        ...
        /* RFC 2292 */
        if ((sock->pf == AF_INET6)
            && (setsockopt (sock->fd, IPPROTO_IPV6, IPV6_PKTINFO, (void *) &on, sizeof (on)) < 0))
        {
            isc__strerror (errno, strbuf, sizeof (strbuf));
            UNEXPECTED_ERROR (__FILE__, __LINE__,
        ...
        /* use minimum MTU */
        if (sock->pf == AF_INET6 &&
            setsockopt (sock->fd, IPPROTO_IPV6, IPV6_USE_MIN_MTU, (void *) &on, sizeof (on)) < 0)
        {
            isc__strerror (errno, strbuf, sizeof (strbuf));
            UNEXPECTED_ERROR (__FILE__, __LINE__,
        ...
        if (sock->pf == AF_INET6)
        {
            ...
            (void) setsockopt (sock->fd, IPPROTO_IPV6, IPV6_MTU, &mtu, sizeof (mtu));
        }
#endif
        ...
        if (sock->pf == AF_INET6)
        {
            ...
            (void) setsockopt (sock->fd, IPPROTO_IPV6, IPV6_MTU_DISCOVER, &action, sizeof (action));
        }
#endif
        ...
        if (sock->pf == AF_INET)
        {
            ...
            (void) setsockopt (sock->fd, IPPROTO_IP, IP_MTU_DISCOVER, &action, sizeof (action));
        }
#endif
        ...
        if (sock->pf == AF_INET)
        {
            ...
            (void) setsockopt (sock->fd, IPPROTO_IP, IP_DONTFRAG, &off, sizeof (off));
        }
#endif

#if defined(SO_RCVBUF)
        optlen = sizeof (size);
        if (getsockopt (sock->fd, SOL_SOCKET, SO_RCVBUF, (void *) &size, &optlen) >= 0 && size < RCVBUFSIZE)
        {
            size = RCVBUFSIZE;
            if (setsockopt (sock->fd, SOL_SOCKET, SO_RCVBUF, (void *) &size, sizeof (size)) == -1)
            {
                isc__strerror (errno, strbuf, sizeof (strbuf));
                UNEXPECTED_ERROR (__FILE__, __LINE__,
 ...
 */
ISC_SOCKETFUNC_SCOPE isc_result_t
isc__socket_create (isc_socketmgr_t * manager0, int pf, isc_sockettype_t type, isc_socket_t ** socketp)
{
    ...
    {
        case isc_sockettype_udp:
            sock->statsindex = (pf == AF_INET) ? upd4statsindex : upd6statsindex;
            break;
        case isc_sockettype_tcp:
            sock->statsindex = (pf == AF_INET) ? tcp4statsindex : tcp6statsindex;
            break;
        case isc_sockettype_unix:
    ...
    *socketp = (isc_socket_t *) sock;
...
#ifdef USE_DEVPOLL
    INSIST (sock->manager->fdpollinfo[sock->fd].want_read == 0 && sock->manager->fdpollinfo[sock->fd].want_write == 0);
#endif
    UNLOCK (&manager->fdlock[lockid]);
    ...
    socket_log (sock, NULL, CREATION, isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_CREATED, "created");

    return (ISC_R_SUCCESS);
}

#ifdef BIND9
ISC_SOCKETFUNC_SCOPE isc_result_t isc__socket_open (isc_socket_t * sock0)
{
    isc_result_t result;
                           ...
                           isc_sockfdwatch_t callback, void *cbarg, isc_task_t * task, isc_socket_t ** socketp)
{
    ...
    *socketp = (isc_socket_t *) sock;
    ...
    socket_log (sock, NULL, CREATION, isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_CREATED, "fdwatch-created");
...
ISC_SOCKETFUNC_SCOPE isc_result_t isc__socket_fdwatchpoke (isc_socket_t * sock0, int flags)
{
    isc__socket_t *sock = (isc__socket_t *) sock0;
    ...
    {
        LOCK (&sock->lock);
        if (((flags & ISC_SOCKFDWATCH_READ) != 0) && !sock->pending_recv)
            select_poke (sock->manager, sock->fd, SELECT_POKE_READ);
        if (((flags & ISC_SOCKFDWATCH_WRITE) != 0) && !sock->pending_send)
            select_poke (sock->manager, sock->fd, SELECT_POKE_WRITE);
        UNLOCK (&sock->lock);
    }

    socket_log (sock, NULL, TRACE, isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_POKED, "fdwatch-poked flags: %d", flags);
 ...
 */
ISC_SOCKETFUNC_SCOPE void isc__socket_attach (isc_socket_t * sock0, isc_socket_t ** socketp)
{
    isc__socket_t *sock = (isc__socket_t *) sock0;
...
#ifdef BIND9
ISC_SOCKETFUNC_SCOPE isc_result_t isc__socket_close (isc_socket_t * sock0)
{
    isc__socket_t *sock = (isc__socket_t *) sock0;
...
static void dispatch_recv (isc__socket_t * sock)
{
    ...
    if (sock->type != isc_sockettype_fdwatch)
    {
        ...
        if (ev == NULL)
            return;
        socket_log (sock, NULL, EVENT, NULL, 0, 0, "dispatch_recv:  event %p -> task %p", ev, ev->ev_sender);
        sender = ev->ev_sender;
    }
    else
    {
...
static void dispatch_send (isc__socket_t * sock)
{
    ...
    if (sock->type != isc_sockettype_fdwatch)
    {
        ...
        if (ev == NULL)
            return;
        socket_log (sock, NULL, EVENT, NULL, 0, 0, "dispatch_send:  event %p -> task %p", ev, ev->ev_sender);
        sender = ev->ev_sender;
    }
    else
    {
 ...
 */
static void internal_accept (isc_task_t * me, isc_event_t * ev)
{
    isc__socket_t *sock;
    ...
    LOCK (&sock->lock);
    socket_log (sock, NULL, TRACE,
                isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_ACCEPTLOCK, "internal_accept called, locked socket");
    ...
    fd = accept (sock->fd, &NEWCONNSOCK (dev)->peer_address.type.sa, (void *) &addrlen);
        ...
        switch (errno)
        {
            ...
            case EMFILE:
                isc_log_iwrite (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                                ISC_LOGMODULE_SOCKET, ISC_LOG_ERROR,
                                isc_msgcat, ISC_MSGSET_SOCKET,
                                ISC_MSG_TOOMANYFDS, "%s: too many open file descriptors", err);
                goto soft_error;
    ...
    else
    {
        if (addrlen == 0U)
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__, "internal_accept(): " "accept() failed to return " "remote address");

            (void) close (fd);
            goto soft_error;
        }
        else if (NEWCONNSOCK (dev)->peer_address.type.sa.sa_family != sock->pf)
        {
            UNEXPECTED_ERROR (__FILE__, __LINE__,
                              "internal_accept(): "
                              "accept() returned peer address "
                              "family %u (expected %u)", NEWCONNSOCK (dev)->peer_address.type.sa.sa_family, sock->pf);
            (void) close (fd);
            goto soft_error;
        }
        else if (fd >= (int) manager->maxsocks)
        {
            isc_log_iwrite (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                            ...
                            isc_msgcat, ISC_MSGSET_SOCKET,
                            ISC_MSG_TOOMANYFDS,
                            "accept: " "file descriptor exceeds limit (%d/%u)", fd, manager->maxsocks);
            (void) close (fd);
            goto soft_error;
        }
    }
    ...
    if (fd != -1)
    {
        ...
        socket_log (sock, &NEWCONNSOCK (dev)->peer_address, CREATION,
                    ...
                    "accepted connection, new socket %p", dev->newsocket);
...
static void internal_recv (isc_task_t * me, isc_event_t * ev)
{
    ...
    LOCK (&sock->lock);
    socket_log (sock, NULL, IOEVENT,
                isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_INTERNALRECV, "internal_recv: task %p got event %p", me, ev);
...
static void internal_send (isc_task_t * me, isc_event_t * ev)
{
    isc_socketevent_t *dev;
    ...
    LOCK (&sock->lock);
    socket_log (sock, NULL, IOEVENT,
                isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_INTERNALSEND, "internal_send: task %p got event %p", me, ev);
...
static void internal_fdwatch_write (isc_task_t * me, isc_event_t * ev)
{
    isc__socket_t *sock;
    ...
    UNLOCK (&sock->lock);
    more_data = (sock->fdwatchcb) (me, (isc_socket_t *) sock, sock->fdwatcharg, ISC_SOCKFDWATCH_WRITE);
    LOCK (&sock->lock);
...
static void internal_fdwatch_read (isc_task_t * me, isc_event_t * ev)
{
    isc__socket_t *sock;
    ...
    UNLOCK (&sock->lock);
    more_data = (sock->fdwatchcb) (me, (isc_socket_t *) sock, sock->fdwatcharg, ISC_SOCKFDWATCH_READ);
    LOCK (&sock->lock);
...
#ifdef USE_KQUEUE
static isc_boolean_t process_fds (isc__socketmgr_t * manager, struct kevent *events, int nevents)
{
    int i;
    ...
    if (nevents == manager->nevents)
    {
        ...
        manager_log (manager, ISC_LOGCATEGORY_GENERAL,
                     ISC_LOGMODULE_SOCKET, ISC_LOG_INFO, "maximum number of FD events (%d) received", nevents);
    }
...
#elif defined(USE_EPOLL)
static isc_boolean_t process_fds (isc__socketmgr_t * manager, struct epoll_event *events, int nevents)
{
    int i;
    ...
    for (i = 0; i < nevents; i++)
    {
        ...
        process_fd (manager, events[i].data.fd, (events[i].events & EPOLLIN) != 0, (events[i].events & EPOLLOUT) != 0);
    }
...
#elif defined(USE_DEVPOLL)
static isc_boolean_t process_fds (isc__socketmgr_t * manager, struct pollfd *events, int nevents)
{
    int i;
...
#endif
        process_fd (manager, events[i].fd, (events[i].events & POLLIN) != 0, (events[i].events & POLLOUT) != 0);
    }
...
static void process_fds (isc__socketmgr_t * manager, int maxfd, fd_set * readfds, fd_set * writefds)
{
    ...
    for (i = 0; i < maxfd; i++)
    {
        ...
        if (i == manager->pipe_fds[0] || i == manager->pipe_fds[1])
            continue;
#endif                            /* USE_WATCHER_THREAD */
        process_fd (manager, i, FD_ISSET (i, readfds), FD_ISSET (i, writefds));
    }
}
#endif
...
static isc_boolean_t process_ctlfd (isc__socketmgr_t * manager)
{
    ...
    for (;;)
    {
        ...
        manager_log (manager, IOEVENT,
                     isc_msgcat_get (isc_msgcat, ISC_MSGSET_SOCKET,
                                     ISC_MSG_WATCHERMSG, "watcher got message %d " "for socket %d"), msg, fd);
 ...
 */
static isc_threadresult_t watcher (void *uap)
{
    ...
    while (!done)
    {
        ...
        {
#ifdef USE_KQUEUE
            cc = kevent (manager->kqueue_fd, NULL, 0, manager->events, manager->nevents, NULL);
#elif defined(USE_EPOLL)
            ...
            memcpy (manager->write_fds_copy, manager->write_fds, manager->fd_bufsize);
            maxfd = manager->maxfd + 1;
            UNLOCK (&manager->lock);

            cc = select (maxfd, manager->read_fds_copy, manager->write_fds_copy, NULL, NULL);
#endif                            /* USE_KQUEUE */
            ...
            else if (cc > 0)
            {
                if (pollstate == poll_checking)
                {
                    ...
                    manager_log (manager,
                                 ...
                                 ISC_LOGMODULE_SOCKET, ISC_LOG_DEBUG (1), "unexpected POLL timeout");
                }
...
#elif defined(USE_SELECT)
        process_fds (manager, maxfd, manager->read_fds_copy, manager->write_fds_copy);
    ...
    manager_log (manager, TRACE, "%s",
                 isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_EXITING, "watcher exiting"));
...
#ifdef BIND9
ISC_SOCKETFUNC_SCOPE void isc__socketmgr_setreserved (isc_socketmgr_t * manager0, isc_uint32_t reserved)
{
    isc__socketmgr_t *manager = (isc__socketmgr_t *) manager0;
...
ISC_SOCKETFUNC_SCOPE void isc___socketmgr_maxudp (isc_socketmgr_t * manager0, int maxudp)
{
    isc__socketmgr_t *manager = (isc__socketmgr_t *) manager0;
...
static isc_result_t setup_watcher (isc_mem_t * mctx, isc__socketmgr_t * manager)
{
    ...
    manager->events = isc_mem_get (mctx, sizeof (struct kevent) * manager->nevents);
    ...
    if (manager->kqueue_fd == -1)
    {
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          "kqueue %s: %s",
                          isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_FAILED, "failed"), strbuf);
        isc_mem_put (mctx, manager->events, sizeof (struct kevent) * manager->nevents);
        return (result);
    }
...
#elif defined(USE_EPOLL)
    manager->nevents = ISC_SOCKET_MAXEVENTS;
    manager->events = isc_mem_get (mctx, sizeof (struct epoll_event) * manager->nevents);
    if (manager->events == NULL)
        return (ISC_R_NOMEMORY);
    ...
    if (manager->epoll_fd == -1)
    {
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          "epoll_create %s: %s",
                          isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_FAILED, "failed"), strbuf);
        isc_mem_put (mctx, manager->events, sizeof (struct epoll_event) * manager->nevents);
        return (result);
    }
#ifdef USE_WATCHER_THREAD
    result = watch_fd (manager, manager->pipe_fds[0], SELECT_POKE_READ);
     ...
     */
    manager->nevents = ISC_SOCKET_MAXEVENTS;
    manager->events = isc_mem_get (mctx, sizeof (struct pollfd) * manager->nevents);
    if (manager->events == NULL)
        return (ISC_R_NOMEMORY);
    /*
     ...
     */
    manager->fdpollinfo = isc_mem_get (mctx, sizeof (pollinfo_t) * manager->maxsocks);
    if (manager->fdpollinfo == NULL)
    {
        isc_mem_put (mctx, manager->events, sizeof (struct pollfd) * manager->nevents);
        return (ISC_R_NOMEMORY);
    }
    ...
    if (manager->devpoll_fd == -1)
    {
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          ...
                          isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_FAILED, "failed"), strbuf);
        isc_mem_put (mctx, manager->events, sizeof (struct pollfd) * manager->nevents);
        isc_mem_put (mctx, manager->fdpollinfo, sizeof (pollinfo_t) * manager->maxsocks);
        return (result);
    }
#ifdef USE_WATCHER_THREAD
    result = watch_fd (manager, manager->pipe_fds[0], SELECT_POKE_READ);
     ...
     */
    manager->fd_bufsize = howmany (manager->maxsocks, NFDBITS) * sizeof (fd_mask);
#else
...
static void cleanup_watcher (isc_mem_t * mctx, isc__socketmgr_t * manager)
{
    ...
    if (result != ISC_R_SUCCESS)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          "epoll_ctl(DEL) %s",
                          isc_msgcat_get (isc_msgcat, ISC_MSGSET_GENERAL, ISC_MSG_FAILED, "failed"));
    }
#endif                            /* USE_WATCHER_THREAD */
...
isc__socketmgr_create2 (isc_mem_t * mctx, isc_socketmgr_t ** managerp, unsigned int maxsocks)
{
    ...
    for (i = 0; i < FDLOCK_COUNT; i++)
    {
        ...
        if (result != ISC_R_SUCCESS)
        {
            ...
            isc_mem_put (mctx, manager->fdlock, FDLOCK_COUNT * sizeof (isc_mutex_t));
            manager->fdlock = NULL;
            goto cleanup_lock;
        }
    }
  ...
  free_manager:
    ...
    if (manager->fdstate != NULL)
    {
        isc_mem_put (mctx, manager->fdstate, manager->maxsocks * sizeof (int));
    }
    if (manager->fds != NULL)
    {
        isc_mem_put (mctx, manager->fds, manager->maxsocks * sizeof (isc_socket_t *));
    }
...
#ifdef BIND9
isc_result_t isc__socketmgr_getmaxsockets (isc_socketmgr_t * manager0, unsigned int *nsockp)
{
    isc__socketmgr_t *manager = (isc__socketmgr_t *) manager0;
...
void isc__socketmgr_setstats (isc_socketmgr_t * manager0, isc_stats_t * stats)
{
    isc__socketmgr_t *manager = (isc__socketmgr_t *) manager0;
...
ISC_SOCKETFUNC_SCOPE void isc__socketmgr_destroy (isc_socketmgr_t ** managerp)
{
    ...
    while (!ISC_LIST_EMPTY (manager->socklist))
    {
        ...
        manager_log (manager, CREATION, "%s",
                     isc_msgcat_get (isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_SOCKETSREMAIN, "sockets exist"));
    ...
    isc_mem_put (manager->mctx, manager->fds, manager->maxsocks * sizeof (isc__socket_t *));
    isc_mem_put (manager->mctx, manager->fdstate, manager->maxsocks * sizeof (int));
    ...
    if (manager->fdlock != NULL)
    {
        for (i = 0; i < FDLOCK_COUNT; i++)
            DESTROYLOCK (&manager->fdlock[i]);
        isc_mem_put (manager->mctx, manager->fdlock, FDLOCK_COUNT * sizeof (isc_mutex_t));
    }
    DESTROYLOCK (&manager->lock);
...
static isc_result_t socket_recv (isc__socket_t * sock, isc_socketevent_t * dev, isc_task_t * task, unsigned int flags)
{
    ...
    switch (io_state)
    {
        case DOIO_SOFT:
            ...
            socket_log (sock, NULL, EVENT, NULL, 0, 0, "socket_recv: event %p -> task %p", dev, ntask);
...
socket_send (isc__socket_t * sock, isc_socketevent_t * dev, isc_task_t * task,
             isc_sockaddr_t * address, struct in6_pktinfo *pktinfo, unsigned int flags)
{
    int io_state;
    ...
    if (pktinfo != NULL)
    {
        ...
        if (!isc_sockaddr_issitelocal (&dev->address) && !isc_sockaddr_islinklocal (&dev->address))
        {
            socket_log (sock, NULL, TRACE, isc_msgcat,
                        ISC_MSGSET_SOCKET, ISC_MSG_PKTINFOPROVIDED,
                        "pktinfo structure provided, ifindex %u " "(set to 0)", pktinfo->ipi6_ifindex);
    ...
    switch (io_state)
    {
        case DOIO_SOFT:
            ...
            if ((flags & ISC_SOCKFLAG_NORETRY) == 0)
            {
                 ...
                 */
                if (ISC_LIST_EMPTY (sock->send_list) && !sock->pending_send)
                    select_poke (sock->manager, sock->fd, SELECT_POKE_WRITE);
                ISC_LIST_ENQUEUE (sock->send_list, dev, ev_link);

                socket_log (sock, NULL, EVENT, NULL, 0, 0, "socket_send: event %p -> task %p", dev, ntask);
                  ...
                  isc_task_t * task, isc_taskaction_t action, const void *arg)
{
     ...
     */
    return (isc__socket_sendto (sock, region, task, action, arg, NULL, NULL));
}
...
isc__socket_sendv (isc_socket_t * sock, isc_bufferlist_t * buflist,
                   isc_task_t * task, isc_taskaction_t action, const void *arg)
{
    return (isc__socket_sendtov (sock, buflist, task, action, arg, NULL, NULL));
}
...
#define S_ISSOCK(mode) 0
    ...
    if (active)
    {
        if (stat (sockaddr->type.sunix.sun_path, &sb) < 0)
        {
            ...
            isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                           ISC_LOGMODULE_SOCKET, ISC_LOG_ERROR,
                           "isc_socket_cleanunix: stat(%s): %s", sockaddr->type.sunix.sun_path, strbuf);
            return;
        }
        if (!(S_ISSOCK (sb.st_mode) || S_ISFIFO (sb.st_mode)))
        {
            isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                           ISC_LOGMODULE_SOCKET, ISC_LOG_ERROR,
                           "isc_socket_cleanunix: %s: not a socket", sockaddr->type.sunix.sun_path);
            return;
        }
        if (unlink (sockaddr->type.sunix.sun_path) < 0)
        {
            ...
            isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                           ISC_LOGMODULE_SOCKET, ISC_LOG_ERROR,
                           "isc_socket_cleanunix: unlink(%s): %s", sockaddr->type.sunix.sun_path, strbuf);
        }
        return;
    }
    ...
    if (s < 0)
    {
        ...
        isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                       ...
                       "isc_socket_cleanunix: socket(%s): %s", sockaddr->type.sunix.sun_path, strbuf);
        return;
    }
    ...
    if (connect (s, (struct sockaddr *) &sockaddr->type.sunix, sizeof (sockaddr->type.sunix)) < 0)
    {
        switch (errno)
        {
            case ECONNREFUSED:
            ...
            default:
                ...
                isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                               ISC_LOGMODULE_SOCKET, ISC_LOG_WARNING,
                               "isc_socket_cleanunix: connect(%s): %s", sockaddr->type.sunix.sun_path, strbuf);
                break;
        }
    }
  cleanup:
...
isc__socket_permunix (isc_sockaddr_t * sockaddr, isc_uint32_t perm, isc_uint32_t owner, isc_uint32_t group)
{
    ...
    if (chmod (path, perm) < 0)
    {
        ...
        isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                       ISC_LOGMODULE_SOCKET, ISC_LOG_ERROR,
                       "isc_socket_permunix: chmod(%s, %d): %s", path, perm, strbuf);
    ...
    if (chown (path, owner, group) < 0)
    {
        ...
        isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                       ISC_LOGMODULE_SOCKET, ISC_LOG_ERROR,
                       "isc_socket_permunix: chown(%s, %d, %d): %s", path, owner, group, strbuf);
        result = ISC_R_FAILURE;
    }
    return (result);
#else
...
ISC_SOCKETFUNC_SCOPE isc_result_t
isc__socket_bind (isc_socket_t * sock0, isc_sockaddr_t * sockaddr, unsigned int options)
{
    isc__socket_t *sock = (isc__socket_t *) sock0;
    ...
    if ((options & ISC_SOCKET_REUSEADDRESS) != 0 &&
        ...
        setsockopt (sock->fd, SOL_SOCKET, SO_REUSEADDR, (void *) &on, sizeof (on)) < 0)
    {
        UNEXPECTED_ERROR (__FILE__, __LINE__,
  ...
  bind_socket:
    ...
    {
        inc_stats (sock->manager->stats, sock->statsindex[STATID_BINDFAIL]);
    ...
    socket_log (sock, sockaddr, TRACE, isc_msgcat, ISC_MSGSET_SOCKET, ISC_MSG_BOUND, "bound");
    sock->bound = 1;
...
ISC_SOCKETFUNC_SCOPE isc_result_t isc__socket_filter (isc_socket_t * sock0, const char *filter)
{
    isc__socket_t *sock = (isc__socket_t *) sock0;
    ...
    if (setsockopt (sock->fd, SOL_SOCKET, SO_ACCEPTFILTER, &afa, sizeof (afa)) == -1)
    {
        isc__strerror (errno, strbuf, sizeof (strbuf));
        socket_log (sock, NULL, CREATION, isc_msgcat, ISC_MSGSET_SOCKET,
                    ISC_MSG_FILTER, "setsockopt(SO_ACCEPTFILTER): %s", strbuf);
 ...
 */
ISC_SOCKETFUNC_SCOPE isc_result_t isc__socket_listen (isc_socket_t * sock0, unsigned int backlog)
{
    isc__socket_t *sock = (isc__socket_t *) sock0;
    ...
    REQUIRE (sock->bound);
    REQUIRE (sock->type == isc_sockettype_tcp || sock->type == isc_sockettype_unix);
 ...
 */
ISC_SOCKETFUNC_SCOPE isc_result_t
isc__socket_accept (isc_socket_t * sock0, isc_task_t * task, isc_taskaction_t action, const void *arg)
{
    isc__socket_t *sock = (isc__socket_t *) sock0;
    ...
    dev = (isc_socket_newconnev_t *)
        isc_event_allocate (manager->mctx, task, ISC_SOCKEVENT_NEWCONN, action, arg, sizeof (*dev));
    if (dev == NULL)
    {
        UNLOCK (&sock->lock);
        return (ISC_R_NOMEMORY);
                     ...
                     isc_task_t * task, isc_taskaction_t action, const void *arg)
{
    ...
    dev = (isc_socket_connev_t *) isc_event_allocate (manager->mctx, sock,
                                                      ISC_SOCKEVENT_CONNECT, action, arg, sizeof (*dev));
    if (dev == NULL)
    {
        UNLOCK (&sock->lock);
        return (ISC_R_NOMEMORY);
...
#undef ERROR_MATCH
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__, "connect(%s) %d/%s", addrbuf, errno, strbuf);
  ...
  success:
    if (cc == 0)
    {
        ...
        inc_stats (sock->manager->stats, sock->statsindex[STATID_CONNECT]);
 ...
 */
static void internal_connect (isc_task_t * me, isc_event_t * ev)
{
    isc__socket_t *sock;
     ...
     */
    optlen = sizeof (cc);
    if (getsockopt (sock->fd, SOL_SOCKET, SO_ERROR, (void *) &cc, (void *) &optlen) < 0)
        cc = errno;
    else
        errno = cc;
    ...
    if (errno != 0)
    {
        ...
        {
            sock->connecting = 1;
            select_poke (sock->manager, sock->fd, SELECT_POKE_CONNECT);
            UNLOCK (&sock->lock);
...
#undef ERROR_MATCH
            default:
                ...
                isc_sockaddr_format (&sock->peer_address, peerbuf, sizeof (peerbuf));
                isc__strerror (errno, strbuf, sizeof (strbuf));
                UNEXPECTED_ERROR (__FILE__, __LINE__, "internal_connect: connect(%s) %s", peerbuf, strbuf);
        }
    }
    else
    {
 ...
 */
ISC_SOCKETFUNC_SCOPE void isc__socket_cancel (isc_socket_t * sock0, isc_task_t * task, unsigned int how)
{
    isc__socket_t *sock = (isc__socket_t *) sock0;
     ...
     */
    if (((how & ISC_SOCKCANCEL_RECV) == ISC_SOCKCANCEL_RECV) && !ISC_LIST_EMPTY (sock->recv_list))
    {
        isc_socketevent_t *dev;
    ...
    if (((how & ISC_SOCKCANCEL_SEND) == ISC_SOCKCANCEL_SEND) && !ISC_LIST_EMPTY (sock->send_list))
    {
        isc_socketevent_t *dev;
    ...
    if (((how & ISC_SOCKCANCEL_ACCEPT) == ISC_SOCKCANCEL_ACCEPT) && !ISC_LIST_EMPTY (sock->accept_list))
    {
        isc_socket_newconnev_t *dev;
        ...
        while (dev != NULL)
        {
            ...
            if ((task == NULL) || (task == current_task))
            {
                ...
                dev->result = ISC_R_CANCELED;
                dev->ev_sender = sock;
                isc_task_sendanddetach (&current_task, ISC_EVENT_PTR (&dev));
            }
     ...
     */
    if (((how & ISC_SOCKCANCEL_CONNECT) == ISC_SOCKCANCEL_CONNECT) && sock->connect_ev != NULL)
    {
        isc_socket_connev_t *dev;
...
ISC_SOCKETFUNC_SCOPE isc_sockettype_t isc__socket_gettype (isc_socket_t * sock0)
{
    isc__socket_t *sock = (isc__socket_t *) sock0;
...
ISC_SOCKETFUNC_SCOPE isc_boolean_t isc__socket_isbound (isc_socket_t * sock0)
{
    isc__socket_t *sock = (isc__socket_t *) sock0;
...
ISC_SOCKETFUNC_SCOPE void isc__socket_ipv6only (isc_socket_t * sock0, isc_boolean_t yes)
{
    isc__socket_t *sock = (isc__socket_t *) sock0;
    ...
    if (sock->pf == AF_INET6)
    {
        if (setsockopt (sock->fd, IPPROTO_IPV6, IPV6_V6ONLY, (void *) &onoff, sizeof (int)) < 0)
        {
            char strbuf[ISC_STRERRORSIZE];
...
int isc__socketmgr_waitevents (isc_socketmgr_t * manager0, struct timeval *tvp, isc_socketwait_t ** swaitp)
{
    ...
    if (tvp != NULL)
    {
        ...
        tsp = NULL;
    swait_private.nevents = kevent (manager->kqueue_fd, NULL, 0, manager->events, manager->nevents, tsp);
    ...
    if (tvp != NULL)
        ...
        timeout = -1;
    swait_private.nevents = epoll_wait (manager->epoll_fd, manager->events, manager->nevents, timeout);
    n = swait_private.nevents;
#elif defined(USE_DEVPOLL)
    dvp.dp_fds = manager->events;
    dvp.dp_nfds = manager->nevents;
    if (tvp != NULL)
    {
        dvp.dp_timeout = tvp->tv_sec * 1000 + (tvp->tv_usec + 999) / 1000;
    }
    else
        dvp.dp_timeout = -1;
    ...
    n = select (swait_private.maxfd, swait_private.readset, swait_private.writeset, NULL, tvp);
#endif
...
ISC_SOCKETFUNC_SCOPE const char *isc__socket_getname (isc_socket_t * socket0)
{
    isc__socket_t *socket = (isc__socket_t *) socket0;
...
isc_result_t isc__socket_register ()
{
...
static const char *_socktype (isc_sockettype_t type)
{
...
ISC_SOCKETFUNC_SCOPE void isc_socketmgr_renderxml (isc_socketmgr_t * mgr0, xmlTextWriterPtr writer)
{
    ...
    while (sock != NULL)
    {
        ...
        if (sock->name[0] != 0)
        {
            ...
            xmlTextWriterWriteFormatString (writer, "%s", sock->name);
        ...
        xmlTextWriterWriteElement (writer, ISC_XMLCHAR "type", ISC_XMLCHAR _socktype (sock->type));

        if (sock->connected)
        {
            isc_sockaddr_format (&sock->peer_address, peerbuf, sizeof (peerbuf));
            xmlTextWriterWriteElement (writer, ISC_XMLCHAR "peer-address", ISC_XMLCHAR peerbuf);
        }
        ...
        if (getsockname (sock->fd, &addr.type.sa, (void *) &len) == 0)
        {
            ...
            xmlTextWriterWriteElement (writer, ISC_XMLCHAR "local-address", ISC_XMLCHAR peerbuf);
        }

        xmlTextWriterStartElement (writer, ISC_XMLCHAR "states");
        if (sock->pending_recv)
            xmlTextWriterWriteElement (writer, ISC_XMLCHAR "state", ISC_XMLCHAR "pending-receive");
        if (sock->pending_send)
            xmlTextWriterWriteElement (writer, ISC_XMLCHAR "state", ISC_XMLCHAR "pending-send");
        if (sock->pending_accept)
            xmlTextWriterWriteElement (writer, ISC_XMLCHAR "state", ISC_XMLCHAR "pending_accept");
        if (sock->listener)
            xmlTextWriterWriteElement (writer, ISC_XMLCHAR "state", ISC_XMLCHAR "listener");
        if (sock->connected)
            xmlTextWriterWriteElement (writer, ISC_XMLCHAR "state", ISC_XMLCHAR "connected");
        if (sock->connecting)
            xmlTextWriterWriteElement (writer, ISC_XMLCHAR "state", ISC_XMLCHAR "connecting");
        if (sock->bound)
            xmlTextWriterWriteElement (writer, ISC_XMLCHAR "state", ISC_XMLCHAR "bound");
