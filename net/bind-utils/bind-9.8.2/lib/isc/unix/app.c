...
ISC_APPFUNC_SCOPE isc_result_t isc__app_onrun (isc_mem_t * mctx, isc_task_t * task, isc_taskaction_t action, void *arg);
...
ISC_APPFUNC_SCOPE isc_result_t isc__appctx_create (isc_mem_t * mctx, isc_appctx_t ** ctxp);

ISC_APPFUNC_SCOPE void isc__appctx_destroy (isc_appctx_t ** ctxp);

ISC_APPFUNC_SCOPE void isc__appctx_settaskmgr (isc_appctx_t * ctx, isc_taskmgr_t * taskmgr);

ISC_APPFUNC_SCOPE void isc__appctx_setsocketmgr (isc_appctx_t * ctx, isc_socketmgr_t * socketmgr);

ISC_APPFUNC_SCOPE void isc__appctx_settimermgr (isc_appctx_t * ctx, isc_timermgr_t * timermgr);
...
static struct
{
     ...
     */
#ifndef BIND9
    void *run, *shutdown, *start, *onrun, *reload, *finish, *block, *unblock;
#endif
} appmethods =
{
    {
    isc__appctx_destroy,
            ...
            isc__app_ctxfinish, isc__appctx_settaskmgr, isc__appctx_setsocketmgr, isc__appctx_settimermgr}
#ifndef BIND9
    , (void *) isc__app_run, (void *) isc__app_shutdown,
        (void *) isc__app_start, (void *) isc__app_onrun, (void *) isc__app_reload,
        (void *) isc__app_finish, (void *) isc__app_block, (void *) isc__app_unblock
#endif
};
    ...
    if (sigfillset (&sa.sa_mask) != 0 || sigaction (sig, &sa, NULL) < 0)
    {
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          isc_msgcat_get (isc_msgcat, ISC_MSGSET_APP,
                                          ISC_MSG_SIGNALSETUP, "handle_signal() %d setup: %s"), sig, strbuf);
...
ISC_APPFUNC_SCOPE isc_result_t isc__app_ctxstart (isc_appctx_t * ctx0)
{
    ...
    if (presult != 0)
    {
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_app_start() pthread_init: %s", strbuf);
        return (ISC_R_UNEXPECTED);
    }
#endif
...
ISC_APPFUNC_SCOPE isc_result_t isc__app_start (void)
{
    ...
    if (sigemptyset (&sset) != 0 ||
        sigaddset (&sset, SIGHUP) != 0 || sigaddset (&sset, SIGINT) != 0 || sigaddset (&sset, SIGTERM) != 0)
    {
        isc__strerror (errno, strbuf, sizeof (strbuf));
        UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_app_start() sigsetops: %s", strbuf);
        return (ISC_R_UNEXPECTED);
    }
    ...
    {
        isc__strerror (presult, strbuf, sizeof (strbuf));
        UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_app_start() pthread_sigmask: %s", strbuf);
        return (ISC_R_UNEXPECTED);
    }
    ...
    {
        isc__strerror (presult, strbuf, sizeof (strbuf));
        UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_app_start() sigprocmask: %s", strbuf);
        return (ISC_R_UNEXPECTED);
    }
#endif                            /* ISC_PLATFORM_USETHREADS */
...
ISC_APPFUNC_SCOPE isc_result_t isc__app_onrun (isc_mem_t * mctx, isc_task_t * task, isc_taskaction_t action, void *arg)
{
    ...
    event = isc_event_allocate (mctx, cloned_task, ISC_APPEVENT_SHUTDOWN, action, arg, sizeof (*event));
    if (event == NULL)
    {
...
ISC_APPFUNC_SCOPE isc_result_t isc__app_ctxrun (isc_appctx_t * ctx0)
{
    isc__appctx_t *ctx = (isc__appctx_t *) ctx0;
    ...
    if (!ctx->running)
    {
         ...
         */
        for (event = ISC_LIST_HEAD (ctx->on_run); event != NULL; event = next_event)
        {
    ...
    while (!ctx->want_shutdown)
    {
        ...
        {
            isc__strerror (errno, strbuf, sizeof (strbuf));
            UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_app_run() sigsetops: %s", strbuf);
            return (ISC_R_UNEXPECTED);
        }
...
ISC_APPFUNC_SCOPE isc_result_t isc__app_run ()
{
    return (isc__app_ctxrun ((isc_appctx_t *) & isc_g_appctx));
}

ISC_APPFUNC_SCOPE isc_result_t isc__app_ctxshutdown (isc_appctx_t * ctx0)
{
    isc__appctx_t *ctx = (isc__appctx_t *) ctx0;
    ...
    if (want_kill)
    {
        ...
        else
        {
            ...
            if (result != 0)
            {
                ...
                UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_app_shutdown() " "pthread_kill: %s", strbuf);
                return (ISC_R_UNEXPECTED);
            }
#else
            ...
            {
                isc__strerror (errno, strbuf, sizeof (strbuf));
                UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_app_shutdown() " "kill: %s", strbuf);
                return (ISC_R_UNEXPECTED);
            }
...
ISC_APPFUNC_SCOPE isc_result_t isc__app_shutdown ()
{
    return (isc__app_ctxshutdown ((isc_appctx_t *) & isc_g_appctx));
}

ISC_APPFUNC_SCOPE isc_result_t isc__app_ctxsuspend (isc_appctx_t * ctx0)
{
    isc__appctx_t *ctx = (isc__appctx_t *) ctx0;
    ...
    if (want_kill)
    {
        ...
        else
        {
            ...
            {
                isc__strerror (result, strbuf, sizeof (strbuf));
                UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_app_reload() " "pthread_kill: %s", strbuf);
                return (ISC_R_UNEXPECTED);
            }
#else
            ...
            {
                isc__strerror (errno, strbuf, sizeof (strbuf));
                UNEXPECTED_ERROR (__FILE__, __LINE__, "isc_app_reload() " "kill: %s", strbuf);
                return (ISC_R_UNEXPECTED);
            }
#endif
        }
...
ISC_APPFUNC_SCOPE void isc__app_ctxfinish (isc_appctx_t * ctx0)
{
    isc__appctx_t *ctx = (isc__appctx_t *) ctx0;
...
ISC_APPFUNC_SCOPE void isc__app_block (void)
{
    ...
    RUNTIME_CHECK (sigemptyset (&sset) == 0 && sigaddset (&sset, SIGINT) == 0 && sigaddset (&sset, SIGTERM) == 0);
...
ISC_APPFUNC_SCOPE void isc__appctx_settaskmgr (isc_appctx_t * ctx0, isc_taskmgr_t * taskmgr)
{
    isc__appctx_t *ctx = (isc__appctx_t *) ctx0;
...
ISC_APPFUNC_SCOPE void isc__appctx_setsocketmgr (isc_appctx_t * ctx0, isc_socketmgr_t * socketmgr)
{
    isc__appctx_t *ctx = (isc__appctx_t *) ctx0;
...
ISC_APPFUNC_SCOPE void isc__appctx_settimermgr (isc_appctx_t * ctx0, isc_timermgr_t * timermgr)
{
    isc__appctx_t *ctx = (isc__appctx_t *) ctx0;
...
isc_result_t isc__app_register ()
{
