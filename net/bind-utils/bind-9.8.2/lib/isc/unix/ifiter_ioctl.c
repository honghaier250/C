...
static isc_result_t getbuf4 (isc_interfaceiter_t * iter)
{
    ...
    for (;;)
    {
         ...
         */
        if (ioctl (iter->socket, SIOCGIFCONF, (char *) &iter->ifc) == -1)
        {
            if (errno != EINVAL)
            {
                ...
                UNEXPECTED_ERROR (__FILE__, __LINE__,
                                  isc_msgcat_get (isc_msgcat,
                                                  ...
                                                  ISC_MSG_GETIFCONFIG, "get interface " "configuration: %s"), strbuf);
                goto unexpected;
            }
            /*
        ...
        else
        {
             ...
             * retry.
             */
            if (iter->ifc.ifc_len + 2 * sizeof (struct ifreq) < iter->bufsize)
                break;
        }
...
#if defined(SIOCGLIFCONF) && defined(SIOCGLIFADDR)
static isc_result_t getbuf6 (isc_interfaceiter_t * iter)
{
    char strbuf[ISC_STRERRORSIZE];
    ...
    for (;;)
    {
         ...
         * and is really hard to shut up.
         */
        if (ioctl (iter->socket6, SIOCGLIFCONF, (char *) &iter->lifc) == -1)
        {
#ifdef __hpux
            /*
        ...
        else
        {
             ...
             * retry.
             */
            if (iter->lifc.lifc_len + 2 * sizeof (struct LIFREQ) < iter->bufsize6)
                break;
        }
    ...
    if (result == ISC_R_SUCCESS)
    {
        ...
        if ((iter->socket6 = socket (AF_INET6, SOCK_DGRAM, 0)) < 0)
        {
            ...
            UNEXPECTED_ERROR (__FILE__, __LINE__,
                              isc_msgcat_get (isc_msgcat,
                                              ISC_MSGSET_IFITERIOCTL,
                                              ISC_MSG_MAKESCANSOCKET, "making interface " "scan socket: %s"), strbuf);
            result = ISC_R_UNEXPECTED;
...
#ifdef HAVE_TRUCLUSTER
static void get_inaddr (isc_netaddr_t * dst, struct in_addr *src)
{
    dst->family = AF_INET;
    memcpy (&dst->type.in, src, sizeof (struct in_addr));
}

static isc_result_t internal_current_clusteralias (isc_interfaceiter_t * iter)
{
    struct clua_info ci;
...
static isc_result_t internal_current4 (isc_interfaceiter_t * iter)
{
    ...
    if (iter->ifc.ifc_len == 0 || iter->pos == (unsigned int) iter->ifc.ifc_len)
    {
#ifdef __linux
    ...
    get_addr (family, &iter->current.address, (struct sockaddr *) &ifrp->ifr_addr, ifreq.ifr_name);
    ...
    switch (family)
    {
        case AF_INET:
            ...
            break;
        case AF_INET6:
            if (memcmp (&iter->current.address.type.in6, &in6addr_any, sizeof (in6addr_any)) == 0)
                return (ISC_R_IGNORE);
            break;
    }
    ...
    {
        isc__strerror (errno, strbuf, sizeof (strbuf));
        UNEXPECTED_ERROR (__FILE__, __LINE__, "%s: getting interface flags: %s", ifreq.ifr_name, strbuf);
        return (ISC_R_IGNORE);
    }
    ...
    memcpy (&lifreq.lifr_addr, &iter->current.address.type.in6, sizeof (iter->current.address.type.in6));

    if (ioctl (iter->socket, SIOCGLIFADDR, &lifreq) < 0)
    {
        isc__strerror (errno, strbuf, sizeof (strbuf));
        UNEXPECTED_ERROR (__FILE__, __LINE__, "%s: getting interface address: %s", ifreq.ifr_name, strbuf);
        return (ISC_R_IGNORE);
    }
    ...
    isc_log_write (isc_lctx, ISC_LOGCATEGORY_GENERAL,
                   ...
                   isc_msgcat_get (isc_msgcat,
                                   ISC_MSGSET_IFITERIOCTL,
                                   ISC_MSG_GETIFCONFIG, "prefix length for %s is unknown " "(assume 128)"), sabuf);
    prefixlen = 128;
#endif
  ...
  inet:
        ...
        if (ioctl (iter->socket, SIOCGIFDSTADDR, (char *) &ifreq) < 0)
        {
            ...
            UNEXPECTED_ERROR (__FILE__, __LINE__,
                              isc_msgcat_get (isc_msgcat,
                                              ISC_MSGSET_IFITERIOCTL,
                                              ISC_MSG_GETDESTADDR,
                                              "%s: getting " "destination address: %s"), ifreq.ifr_name, strbuf);
            return (ISC_R_IGNORE);
        }
        get_addr (family, &iter->current.dstaddress, (struct sockaddr *) &ifreq.ifr_dstaddr, ifreq.ifr_name);
    }
#endif
    ...
    if (ioctl (iter->socket, SIOCGIFNETMASK, (char *) &ifreq) < 0)
    {
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          isc_msgcat_get (isc_msgcat,
                                          ISC_MSGSET_IFITERIOCTL,
                                          ISC_MSG_GETNETMASK, "%s: getting netmask: %s"), ifreq.ifr_name, strbuf);
        return (ISC_R_IGNORE);
    }
    get_addr (family, &iter->current.netmask, (struct sockaddr *) &ifreq.ifr_addr, ifreq.ifr_name);
    return (ISC_R_SUCCESS);
}

#if defined(SIOCGLIFCONF) && defined(SIOCGLIFADDR)
static isc_result_t internal_current6 (isc_interfaceiter_t * iter)
{
    struct LIFREQ *ifrp;
    ...
    get_addr (family, &iter->current.address, (struct sockaddr *) &lifreq.lifr_addr, lifreq.lifr_name);
    ...
    {
        isc__strerror (errno, strbuf, sizeof (strbuf));
        UNEXPECTED_ERROR (__FILE__, __LINE__, "%s: getting interface flags: %s", lifreq.lifr_name, strbuf);
        return (ISC_R_IGNORE);
    }
        ...
        if (ioctl (fd, SIOCGLIFDSTADDR, (char *) &lifreq) < 0)
        {
            ...
            UNEXPECTED_ERROR (__FILE__, __LINE__,
                              isc_msgcat_get (isc_msgcat,
                                              ISC_MSGSET_IFITERIOCTL,
                                              ISC_MSG_GETDESTADDR,
                                              "%s: getting " "destination address: %s"), lifreq.lifr_name, strbuf);
            return (ISC_R_IGNORE);
        }
        get_addr (family, &iter->current.dstaddress, (struct sockaddr *) &lifreq.lifr_dstaddr, lifreq.lifr_name);
    }
#endif
    ...
    if (family == AF_INET6)
    {
        ...
        for (i = 0; i < lifreq.lifr_addrlen; i += 8)
        {
            ...
            iter->current.netmask.type.in6.s6_addr[i / 8] = (~0 << bits) & 0xff;
        }
    ...
    if (ioctl (fd, SIOCGLIFNETMASK, (char *) &lifreq) < 0)
    {
        ...
        UNEXPECTED_ERROR (__FILE__, __LINE__,
                          isc_msgcat_get (isc_msgcat,
                                          ISC_MSGSET_IFITERIOCTL,
                                          ISC_MSG_GETNETMASK, "%s: getting netmask: %s"), lifreq.lifr_name, strbuf);
        return (ISC_R_IGNORE);
    }
    get_addr (family, &iter->current.netmask, (struct sockaddr *) &lifreq.lifr_addr, lifreq.lifr_name);
...
#if defined(SIOCGLIFCONF) && defined(SIOCGLIFADDR)
static isc_result_t internal_next6 (isc_interfaceiter_t * iter)
{
#ifdef ISC_PLATFORM_HAVESALEN
    struct LIFREQ *ifrp;
#endif
