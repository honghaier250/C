...
isc_socket_sendto (isc_socket_t * sock, isc_region_t * region, isc_task_t * task,
                   isc_taskaction_t action, const void *arg, isc_sockaddr_t * address, struct in6_pktinfo * pktinfo)
{
    REQUIRE (ISCAPI_SOCKET_VALID (sock));

    return (sock->methods->sendto (sock, region, task, action, arg, address, pktinfo));
}
                          ...
                          isc_sockfdwatch_t callback, void *cbarg, isc_task_t * task, isc_socket_t ** socketp)
{
    ...
    return (manager->methods->fdwatchcreate (manager, fd, flags, callback, cbarg, task, socketp));
}
