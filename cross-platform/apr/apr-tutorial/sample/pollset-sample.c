...
static int do_accept(apr_pollset_t * pollset, apr_socket_t * lsock, apr_pool_t * mp)
{
    ...
    if (rv == APR_SUCCESS)
    {
        ...
        apr_pollfd_t pfd = { mp, APR_POLL_SOCKET, APR_POLLIN, 0, {NULL}
        , serv_ctx };
        pfd.desc.s = ns;
...
static int recv_req_cb(serv_ctx_t * serv_ctx, apr_pollset_t * pollset, apr_socket_t * sock)
{
    while (1)
    {
        ...
        {                        /* expect the end of the request. no guarantee */
            ...
            apr_pollfd_t pfd = { serv_ctx->mp, APR_POLL_SOCKET, APR_POLLIN, 0, {NULL}
            , serv_ctx };
            pfd.desc.s = sock;
...
static int send_resp_cb(serv_ctx_t * serv_ctx, apr_pollset_t * pollset, apr_socket_t * sock)
{
...

    {
        ...
        if (wlen == len)
        {
            apr_pollfd_t pfd = { serv_ctx->mp, APR_POLL_SOCKET, APR_POLLOUT, 0, {NULL}
            , serv_ctx };
            pfd.desc.s = sock;
            apr_pollset_remove(pollset, &pfd);
