...
void INI::Level::addKey(std::string name, std::string value)
{
    ...
    std::pair < Level::ValueMap::const_iterator, bool > res = this->values.insert(std::make_pair(name, value));
...
{
    std::string buffer = ("Error '" + msg + "' on line #" + Utils::String::toString(this->lines));
...
input_file(filename.c_str()), input(&input_file), lines(0)
{
    if (!input)
...
void INI::Parser::dump(std::ostream & s, const INI::Level & l, const std::string & sname)
{
    ...
    for (INI::Level::Values::const_iterator it = l.ordered_values.begin(); it != l.ordered_values.end(); ++it)
        s << (*it)->first << '=' << (*it)->second << std::endl;

    for (INI::Level::Sections::const_iterator it = l.ordered_sections.begin(); it != l.ordered_sections.end(); ++it)
    {
