...
MenuItemNumberbox::MenuItemNumberbox(std::string label, int id, int min, int max, int initial, int jump):MenuItem(label, id), min(min), max(max), initial(initial), current(initial), jump(jump)
{
...
void MenuItemNumberbox::draw(Window * window, int x, int y, int width, bool hilite)
{
    ...
    // If hilite:
    //      label   < text >
    MenuItem::draw(window, x, y, (width - number.size() - 3), hilite);

    int rightmost = x + width;

    window->print(((hilite) ? "<" : "["), rightmost - number.size() - 2, y, ((hilite) ? EngineGlobals::Theme::hilite_text : EngineGlobals::Theme::text));

    window->print(((hilite) ? ">" : "]"), rightmost - 1, y, ((hilite) ? EngineGlobals::Theme::hilite_text : EngineGlobals::Theme::text));

    window->print(number, rightmost - number.size() - 1, y, EngineGlobals::Theme::hilite_text);
}
...
void MenuItemNumberbox::handleInput()
{
    ...
    else if (InputManager::isPressed("right") || InputManager::isPressed(KEY_RIGHT))
        this->increase();

    else if (InputManager::isPressed('r') || InputManager::isPressed('R') || InputManager::isPressed(' ') || InputManager::isPressed('\n') || InputManager::isPressed(KEY_ENTER))
        this->reset();
}
