...
MenuItemTextlist::MenuItemTextlist(std::string label, int id, std::vector < std::string > options, std::string initial):MenuItem(label, id), options(options), currentIndex(-1)
{
...
void MenuItemTextlist::draw(Window * window, int x, int y, int width, bool hilite)
{
    ...
    // If hilite:
    //      label   < text >
    MenuItem::draw(window, x, y, (width - text.size() - 5), hilite);

    int rightmost = x + width;

    window->print(((hilite) ? "<" : "["), rightmost - text.size() - 2, y, ((hilite) ? EngineGlobals::Theme::hilite_text : EngineGlobals::Theme::text));

    window->print(((hilite) ? ">" : "]"), rightmost - 1, y, ((hilite) ? EngineGlobals::Theme::hilite_text : EngineGlobals::Theme::text));

    window->print(text, rightmost - text.size() - 1, y, EngineGlobals::Theme::hilite_text);
}
...
void MenuItemTextlist::handleInput()
{
    ...
    else if (InputManager::isPressed('r') || InputManager::isPressed('R') || InputManager::isPressed(' '))
        this->reset();
}
