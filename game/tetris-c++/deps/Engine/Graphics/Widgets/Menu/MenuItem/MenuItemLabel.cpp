...
MenuItemLabel::MenuItemLabel(std::string label, int id, std::string rightLabel):MenuItem(label, id), rightLabel(rightLabel)
{
...
void MenuItemLabel::draw(Window * window, int x, int y, int width, bool hilite)
{
    ...
    window->print(this->rightLabel, x + width - rightLabelSize, y, ((hilite) ? EngineGlobals::Theme::hilite_text : EngineGlobals::Theme::text));
}
