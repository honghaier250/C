...
MenuItemCheckbox::MenuItemCheckbox(std::string label, int id, bool initial):MenuItem(label, id), checked(initial)
{
...
void MenuItemCheckbox::draw(Window * window, int x, int y, int width, bool hilite)
{
    ...
    window->print(((hilite) ? "<" : "["), posx - 8, y, ((hilite) ? EngineGlobals::Theme::hilite_text : EngineGlobals::Theme::text));

    window->print("ON", posx - 7, y, ((this->checked) ? EngineGlobals::Theme::hilite_text : EngineGlobals::Theme::text));

    window->print("|", posx - 5, y, EngineGlobals::Theme::text);

    window->print("OFF", posx - 4, y, ((this->checked) ? EngineGlobals::Theme::text : EngineGlobals::Theme::hilite_text));

    window->print(((hilite) ? ">" : "]"), posx - 1, y, ((hilite) ? EngineGlobals::Theme::hilite_text : EngineGlobals::Theme::text));
}
...
void MenuItemCheckbox::handleInput()
{
    ...
    else if (InputManager::isPressed(' ') || InputManager::isPressed('\n') || InputManager::isPressed(KEY_ENTER))
        this->toggle();
}
