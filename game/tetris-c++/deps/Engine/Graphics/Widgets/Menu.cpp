...
Menu::Menu(int x, int y, int width, int height):current(NULL), currentIndex(0), x(x), y(y), width(width), height(height), selected(false), selectedItem(NULL)
{
}
...
void Menu::draw(Window * window)
{
    ...
    for (unsigned int curitem = draw_begin, yoffset = 0; curitem < draw_end; curitem++, yoffset++)
    {
        ...
        {
            window->print("(more)", this->x + this->width / 2 - 3, this->y + yoffset, EngineGlobals::Theme::dim_text);
            continue;
        }
        // Menu is big and needs scrolling,
        ...
        {
            window->print("(more)", this->x + this->width / 2 - 3, this->y + yoffset + 1, EngineGlobals::Theme::dim_text);
            continue;
        }
...
//      MenuItemCheckbox* pCheckbox = dynamic_cast<MenuItemCheckbox*>a
        ...
        if (!this->item[curitem])
        {
            for (int j = 0; j < (this->width); j++)
                window->printChar(((EngineGlobals::Screen::fancy_borders) ? ACS_HLINE : '-'), this->x + j, this->y + yoffset, EngineGlobals::Theme::dim_text);
        }
        else
        {
            this->item[curitem]->draw(window, this->x, this->y + yoffset, this->width,
...
void Menu::handleInput()
{
    ...
    if (InputManager::isPressed("down") ||    // user-defined
        InputManager::isPressed(KEY_DOWN) || InputManager::isPressed('\t'))
        this->goNext();

    else if (InputManager::isPressed("up") || InputManager::isPressed(KEY_UP) || InputManager::isPressed(KEY_BTAB))
        this->goPrevious();

    else if (InputManager::isPressed(KEY_HOME) || InputManager::isPressed(KEY_PPAGE))
        this->goFirst();

    else if (InputManager::isPressed(KEY_END) || InputManager::isPressed(KEY_NPAGE))
        this->goLast();

    else if (InputManager::isPressed(KEY_ENTER) || InputManager::isPressed('\n'))
    {
