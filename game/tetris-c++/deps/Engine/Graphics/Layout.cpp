...
Layout::Layout(int width, int height):main(NULL)
{
    intendedWidth = width;
...
void Layout::windowsInit()
{
    ...
    if ((current_width < intendedWidth) || (current_height < intendedHeight))
    {
        Ncurses::exit();
        std::cerr << "Error! Your console screen is smaller than " << intendedWidth << "x" << intendedHeight << "\n" << "Please resize your window and try again." << std::endl;
