...
Animation(window), particle(NULL)
{
}
...
void AnimationFire::load()
{
    ...
    for (unsigned int i = 0; i < width; i++)
        for (unsigned int j = 0; j < height; j++)
            coolingMap->set(i, j, Utils::Random::between(INTENSITY_MIN, INTENSITY_PERCENT(13)));

    // Will smooth the cooling map a number of times
    for (int n = 0; n < 10; n++)
        for (unsigned int i = 1; i < width - 1; i++)
            for (unsigned int j = 1; j < height - 1; j++)
                coolingMap->set(i, j, (coolingMap->at(i - 1, j) + coolingMap->at(i + 1, j) + coolingMap->at(i, j + 1) + coolingMap->at(i, j - 1)) / 4);
...
void AnimationFire::update()
{
    ...
    if (burst)
        cooling_ratio = INTENSITY_PERCENT(1);
    if (dim)
        cooling_ratio = INTENSITY_PERCENT(30);
