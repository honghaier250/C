...
void AnimationSnakes::update()
{
    ...
    if ((addTimer.delta_ms() > delay) && (this->lilsnakes.size() < MAX_SNAKES))
    {
        this->addSnake();
...
void AnimationSnakes::draw()
{
    ...
    {
        window->printChar('@', this->lilsnakes[i].x, this->lilsnakes[i].y, Colors::pair("green", "default", true));

        for (int j = 1; j < (this->lilsnakes[i].size); j++)
        {
            window->printChar('o', this->lilsnakes[i].x, this->lilsnakes[i].y - j, Colors::pair("green", "default"));
