...
Animation(window), buffer1(NULL), buffer2(NULL)
{
}
...
void AnimationWater::load()
{
    ...
    for (unsigned int i = 0; i < width; i++)
    {
        ...
        {
            buffer1->set(i, j, Utils::Random::between(HEIGHT_MIN, HEIGHT_PERCENT(13)));
            buffer2->set(i, j, Utils::Random::between(HEIGHT_MIN, HEIGHT_PERCENT(25)));
        }
    }
...
void AnimationWater::update()
{
    ...
    if (Utils::Random::booleanWithChance(0.31))
        buffer2->set(Utils::Random::between(0, buffer2->width() - 1), Utils::Random::between(0, buffer2->height() - 1), HEIGHT_PERCENT(90));
    ...
    for (unsigned int i = 1; i < (buffer1->width() - 1); i++)
    {
        for (unsigned int j = 1; j < (buffer1->height() - 1); j++)
        {
            ...
            buffer2->set(i, j, ((buffer1->at(i - 1, j) + buffer1->at(i + 1, j) + buffer1->at(i, j + 1) + buffer1->at(i, j - 1)) >> 1) - buffer2->at(i, j));
