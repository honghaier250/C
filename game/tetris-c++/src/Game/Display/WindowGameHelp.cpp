...
void WindowGameHelp::run()
{
    ...
    while (true)
    {
        ...
        this->main->print(((activatedIndex == 0) ? "(Help)" : " Help "), 2, 0, ((activatedIndex == 0) ? EngineGlobals::Theme::hilite_hilite_text : EngineGlobals::Theme::text));

        this->main->print(((activatedIndex == 1) ? "(Credits)" : " Credits "), 12, 0, ((activatedIndex == 1) ? EngineGlobals::Theme::hilite_hilite_text : EngineGlobals::Theme::text));
        ...
        if (activatedIndex == 0)
        {
            ...
            this->windows[0]->print(Utils::String::split(InputManager::keyToString(InputManager::getBind("left")) + "\n" +
                                                         ...
                                                         InputManager::keyToString(InputManager::getBind("pause")) + "\n" + InputManager::keyToString(InputManager::getBind("quit")) + "\n", '\n'), 37, 4, EngineGlobals::Theme::text);

            this->windows[0]->print("Your settings and scores are stored at:", 1, 16, EngineGlobals::Theme::text);

            this->windows[0]->print("~/.local/share/yetris/" + Globals::Profiles::current->name, 0, 17, EngineGlobals::Theme::hilite_text);
        }
        //
        // Credits
        ...
        else if (InputManager::isPressed("quit") || InputManager::isPressed(KEY_ENTER) || InputManager::isPressed('\n'))
            return;
    }
}
