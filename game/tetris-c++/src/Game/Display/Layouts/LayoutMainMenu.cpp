...
LayoutMainMenu::LayoutMainMenu(int width, int height, GameStateMainMenu * state):Layout(width, height), state(state), logo(NULL), menu(NULL), animationContainer(NULL), animation(NULL)
{
    this->windowsInit();
}
...
void LayoutMainMenu::windowsInit()
{
    ...
    // LOGO
    this->logo = new Window(this->main, 0, 0, 0, 9);

    if (EngineGlobals::Screen::show_borders)
    {
        this->logo->borders(EngineGlobals::Screen::fancy_borders ? Window::BORDER_FANCY : Window::BORDER_REGULAR);
    }
    ...
    this->menu = new Window(this->main, (this->main->getW() / 3) - 2, this->logo->getH() + 1, (this->main->getW() / 3) + 2, this->main->getH() - this->logo->getH() - 2);
    ...
    this->animationContainer = new Window(this->main, 0, posy, 0, height);
