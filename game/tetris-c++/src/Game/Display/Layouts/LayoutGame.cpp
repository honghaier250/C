...
LayoutGame::LayoutGame(Game * game, int width, int height):Layout(width, height), game(game), leftmost(NULL), hold(NULL), score(NULL), middle_left(NULL), board(NULL), middle_right(NULL), rightmost(NULL), helpWindows(NULL), animation(NULL)
{
    this->windowsInit();
}
...
void LayoutGame::windowsInit()
{
    ...
    // Leftmost window
    this->leftmost = new Window(this->main, leftmost_x, 0, 4 * 2 + 2 + 2,    // 4 blocks + borders + 2 space
                                0);
    this->leftmost->borders(Window::BORDER_NONE);    // container Window

    if (Globals::Profiles::current->settings.game.can_hold)
    {
        this->hold = new Window(this->leftmost, 0, 0, this->leftmost->getW(), 4);
        this->hold->refresh();
    ...
    // we can't hold.
    int score_y = ((this->hold) ? this->hold->getH() : 0);

    int score_height = ((this->hold) ? this->leftmost->getH() - this->hold->getH() : this->leftmost->getH());

    this->score = new Window(this->leftmost, 0, score_y, 0, score_height);
    this->score->refresh();

    // Middle-left, container for the board
    this->middle_left = new Window(this->main, this->leftmost->getX() + this->leftmost->getW(), 0, 10 * 2 + 2,    // 10 blocks + borders
    ...
    this->middle_right = new Window(this->main, this->middle_left->getX() + this->middle_left->getW(), 0, 4 * 2 + 2,    // 4 blocks + borders
                                    middle_right_height);
    ...
    {
        this->next[i] = new Window(this->middle_right, 0, this->next[i - 1]->getY() + this->next[i - 1]->getH() + 1, 0, 2);
    ...
    if (Globals::Profiles::current->settings.screen.show_statistics)
    {
        ...
        this->rightmost = new Window(this->main, this->middle_right->getX() + this->middle_right->getW(), 0, this->main->getW() - (this->middle_right->getX() + this->middle_right->getW()) - 1, 0);
    ...
    // Le pause window.
    this->pause = new Window(this->main, this->main->getW() / 4, this->main->getH() / 2 - 1,    //center
                             this->main->getW() / 2, 6);
