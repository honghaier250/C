...
menuGameSettings(NULL), menuGameSettingsActivated(false), menuGUIOptions(NULL), menuGUIOptionsActivated(false), menuProfiles(NULL), menuProfilesActivated(false), menuControls(NULL), menuControlsActivated(false), helpWindows(NULL)
{
}
...
std::string getProfileName(Window * main)
{
    ...
    Window window(main, main->getW() / 3, main->getH() / 2 - 10 / 2, main->getW() / 3, 10);
    window.setTitle("New Profile");

    window.clear();
    window.print("Please enter a name:", 1, 1);

    window.print("(default: " + Utils::File::getUser() + ")", 1, 4, Globals::Profiles::current->settings.theme.hilite_text);
    ...
    mvgetnstr(main->getY() + window.getY() + 3, main->getX() + window.getX() + 1, name, 255);
...
void GameStateMainMenu::update()
{
    ...
    {
        if (InputManager::isPressed((int) 'd') || InputManager::isPressed((int) 'D'))
        {
        ...
        else if (InputManager::isPressed((int) 's') || InputManager::isPressed((int) 'S'))
        {
            // Switch to profile
            std::string name = this->menuProfiles->currentLabel();
        ...
        else if (InputManager::isPressed((int) 'c') || InputManager::isPressed((int) 'C'))
        {
    ...
    else if (this->menuControlsActivated)
    {
        ...
        if (this->menuControls->willQuit())
        {
            ...
            if (!key.empty())
            {
                ...
                if ((tmp != KEY_ENTER) && (tmp != '\n') && (tmp != ERR) && (!InputManager::isBound(tmp)))
                {
...
void GameStateMainMenu::createMainMenu()
{
    ...
    // settings.
    this->menu = new Menu(1, 1, this->layout->menu->getW() - 2, this->layout->menu->getH() - 2);
...
void GameStateMainMenu::createSinglePlayerMenu()
{
    ...
    this->menuSinglePlayer = new Menu(1, 1, this->layout->menu->getW() - 2, this->layout->menu->getH() - 2);
    ...
    check = new MenuItemCheckbox("Invisible", INVISIBLE, Globals::Profiles::current->scores->score.invisible);
    menuSinglePlayer->add(check);

    check = new MenuItemCheckbox("Slide Left", SLIDE_LEFT, Globals::Profiles::current->scores->score.slide_left);
    menuSinglePlayer->add(check);

    check = new MenuItemCheckbox("Slide Right", SLIDE_RIGHT, Globals::Profiles::current->scores->score.slide_right);
    menuSinglePlayer->add(check);

    check = new MenuItemCheckbox("Hold Piece", HOLD_PIECE, Globals::Profiles::current->settings.game.can_hold);
    menuSinglePlayer->add(check);

    check = new MenuItemCheckbox("Ghost Piece", GHOST_PIECE, Globals::Profiles::current->settings.game.has_ghost);
    menuSinglePlayer->add(check);
}
...
void GameStateMainMenu::createGameSettingsMenu()
{
    ...
    this->menuGameSettings = new Menu(1, 1, this->layout->menu->getW() - 2, this->layout->menu->getH() - 2);
    ...
    check = new MenuItemCheckbox("Show Statistics", SHOW_STATISTICS, Globals::Profiles::current->settings.screen.show_statistics);
    menuGameSettings->add(check);

    MenuItemNumberbox *box;

    box = new MenuItemNumberbox("Next Pieces", NEXT_PIECES, 1, 7, Globals::Profiles::current->settings.game.next_pieces);
    ...
    list = new MenuItemTextlist("Random Piece", RANDOM_ALGORITHM, options, Globals::Profiles::current->scores->score.random_algorithm);
    menuGameSettings->add(list);

    box = new MenuItemNumberbox("Line clear delay(ms)", LINE_DELAY, 0, 300, Globals::Profiles::current->settings.game.line_clear_delay);
    menuGameSettings->add(box);
}
...
void GameStateMainMenu::createGUIOptionsMenu()
{
    ...
    this->menuGUIOptions = new Menu(1, 1, this->layout->menu->getW() - 2, this->layout->menu->getH() - 2);
    ...
    check = new MenuItemCheckbox("Show Borders", SHOW_BORDERS, EngineGlobals::Screen::show_borders);
    menuGUIOptions->add(check);

    check = new MenuItemCheckbox("Fancy Borders", FANCY_BORDERS, EngineGlobals::Screen::fancy_borders);
    menuGUIOptions->add(check);

    check = new MenuItemCheckbox("Outer Border", OUTER_BORDER, EngineGlobals::Screen::outer_border);
    menuGUIOptions->add(check);

    check = new MenuItemCheckbox("Center Horizontal", CENTER_HORIZONTAL, EngineGlobals::Screen::center_horizontally);
    menuGUIOptions->add(check);

    check = new MenuItemCheckbox("Center Vertical", CENTER_VERTICAL, EngineGlobals::Screen::center_vertically);
    menuGUIOptions->add(check);
    ...
    list = new MenuItemTextlist("Menu Animation", ANIMATION_MENU, options, Globals::Profiles::current->settings.screen.animation_menu);
    menuGUIOptions->add(list);

    list = new MenuItemTextlist("Game Animation", ANIMATION_GAME, options, Globals::Profiles::current->settings.screen.animation_game);
    menuGUIOptions->add(list);
}
...
void GameStateMainMenu::createControlsMenu()
{
    ...
    this->menuControls = new Menu(1, 1, this->layout->menu->getW() - 2, this->layout->menu->getH() - 2);
...
void GameStateMainMenu::createProfilesMenu()
{
    ...
    this->menuProfiles = new Menu(1, 4, this->layout->menu->getW() - 2, this->layout->menu->getH() - 2 - 3);
