...
Board::Board(int x, int y, int width, int height):x(x), y(y)
{
...
void Board::draw(Window * win)
{
    for (int i = 0; i < width; i++)
        for (int j = 0; j < height; j++)
            if (this->block[i][j])
                this->block[i][j]->draw(win, (this->x + i) * 2,    // 2 chars
                                        this->y + j);
}
    ...
    for (int i = 0; i < 5; i++)
        for (int j = 0; j < 5; j++)
            if (global_pieces[p->getType()][p->getRotation()][j][i] != 0)
            {
                ...
                if ((block_x >= this->x + this->width) || (block_x < this->x) || (block_y >= this->y + this->height))
                    return false;
