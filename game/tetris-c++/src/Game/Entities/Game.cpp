...
layout(NULL),
gameOver(false),
pieceCurrent(NULL), pieceGhost(NULL), pieceHold(NULL), board(NULL), rotationSystem(NULL), movedPieceDown(NULL), canHold(true), willClearLines(true), isInvisible(false), isPaused(false), showPauseMenu(false), showHelp(false), pauseMenu(NULL)
{
}
...
void Game::start()
{
    ...
    this->pauseMenu = new Menu(1, 1, this->layout->pause->getW() - 2, this->layout->pause->getH() - 2);
...
void Game::handleInput()
{
    ...
    {
        (this->isPaused) ? this->pause(false) : this->pause(true);
    ...
    {
        this->rotationSystem->rotate(this->pieceCurrent, this->board, 1);
    }
    ...
    {
        this->rotationSystem->rotate(this->pieceCurrent, this->board, 2);
    }
...
void Game::update()
{
    ...
    if (Globals::Profiles::current->settings.game.has_ghost)
    {
        this->pieceGhost->update(this->pieceCurrent, this->board);
    }
...
{
    if ((!Globals::Profiles::current->settings.game.can_hold) || (!this->canHold))
        return;
