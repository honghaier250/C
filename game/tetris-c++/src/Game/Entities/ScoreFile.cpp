...
points(0), lines(0), level(1), starting_level(1), initial_noise(0), invisible(false), slide_left(false), slide_right(false), random_algorithm("regular")
{
}
...
bool ScoreEntry::isLike(ScoreEntry & other)
{
    return (this->starting_level == other.starting_level &&
            this->initial_noise == other.initial_noise && this->invisible == other.invisible && this->slide_left == other.slide_left && this->slide_right == other.slide_right && this->random_algorithm == other.random_algorithm);
}
...
void ScoreFile::load()
{
    ...
    std::string score_file = (Globals::Profiles::current->directory + "hscores" + "." + ScoreFile::extension);
    ...
    for (INI::Level::Sections::const_iterator it = ini.top().ordered_sections.begin(); it != ini.top().ordered_sections.end(); ++it)
    {
        // This is SOO ugly!
