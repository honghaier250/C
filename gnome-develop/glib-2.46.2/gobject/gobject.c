...
static void object_interface_check_properties(gpointer check_data, gpointer g_iface);
...
static GObjectNotifyQueue *g_object_notify_queue_freeze(GObject * object, gboolean conditional)
{
    ...
    if (!nqueue)
    {
        ...
        g_datalist_id_set_data_full(&object->qdata, quark_notify_queue, nqueue, g_object_notify_queue_free);
    }

    if (nqueue->freeze_count >= 65535)
        g_critical("Free queue for %s (%p) is larger than 65535," " called g_object_freeze_notify() too often." " Forgot to call g_object_thaw_notify() or infinite loop", G_OBJECT_TYPE_NAME(object), object);
    else
        nqueue->freeze_count++;
    G_UNLOCK(notify_lock);
...
static void g_object_notify_queue_add(GObject * object, GObjectNotifyQueue * nqueue, GParamSpec * pspec)
{
    G_LOCK(notify_lock);
   ...
   */
    gobject_signals[NOTIFY] =
        g_signal_new(g_intern_static_string("notify"),
                     ...
                     G_SIGNAL_RUN_FIRST | G_SIGNAL_NO_RECURSE | G_SIGNAL_DETAILED | G_SIGNAL_NO_HOOKS | G_SIGNAL_ACTION, G_STRUCT_OFFSET(GObjectClass, notify), NULL, NULL, g_cclosure_marshal_VOID__PARAM, G_TYPE_NONE, 1, G_TYPE_PARAM);
...
void g_object_class_install_properties(GObjectClass * oclass, guint n_pspecs, GParamSpec ** pspecs)
{
    ...
    if (CLASS_HAS_DERIVED_CLASS(oclass))
        g_error("Attempt to add properties to %s after it was derived", G_OBJECT_CLASS_NAME(oclass));
...
void g_object_class_override_property(GObjectClass * oclass, guint property_id, const gchar * name)
{
    ...
    if (parent_type != G_TYPE_NONE)
        overridden = g_param_spec_pool_lookup(pspec_pool, name, parent_type, TRUE);
    if (!overridden)
    {
        GType *ifaces;
        guint n_ifaces;
...
static void g_object_finalize(GObject * object)
{
    ...
    {
        g_critical("object %s %p finalized while still in-construction", G_OBJECT_TYPE_NAME(object), object);
    }
...
void g_object_notify(GObject * object, const gchar * property_name)
{
    ...
    if (!pspec)
        g_warning("%s: object class '%s' has no property named '%s'", G_STRFUNC, G_OBJECT_TYPE_NAME(object), property_name);
    else
...
static void consider_issuing_property_deprecation_warning(const GParamSpec * pspec)
{
    ...
    if (!already)
        g_warning("The property %s:%s is deprecated and shouldn't be used " "anymore. It will be removed in a future version.", g_type_name(pspec->owner_type), pspec->name);
}
...
static inline void object_get_property(GObject * object, GParamSpec * pspec, GValue * value)
{
    ...
    if (class == NULL)
    {
        g_warning("'%s::%s' is not a valid property name; '%s' is not a GObject subtype", g_type_name(pspec->owner_type), pspec->name, g_type_name(pspec->owner_type));
        return;
    }
...
static inline void object_set_property(GObject * object, GParamSpec * pspec, const GValue * value, GObjectNotifyQueue * nqueue)
{
    ...
    else if (g_param_value_validate(pspec, &tmp_value) && !(pspec->flags & G_PARAM_LAX_VALIDATION))
    {
        ...
        g_warning("value \"%s\" of type '%s' is invalid or out of range for property '%s' of type '%s'", contents, G_VALUE_TYPE_NAME(value), pspec->name, g_type_name(pspec->value_type));
        g_free(contents);
    }
    else
    {
...
static void object_interface_check_properties(gpointer check_data, gpointer g_iface)
{
    ...
    while (n--)
    {
        ...
        if (!class_pspec)
        {
            g_critical("Object class %s doesn't implement property " "'%s' from interface '%s'", g_type_name(G_OBJECT_CLASS_TYPE(class)), pspecs[n]->name, g_type_name(iface_type));
...
#define SUBSET(a,b,mask) (((a) & ~(b) & (mask)) == 0)
        ...
        {
            g_critical("Flags for property '%s' on class '%s' remove functionality compared with the " "property on interface '%s'\n", pspecs[n]->name, g_type_name(G_OBJECT_CLASS_TYPE(class)), g_type_name(iface_type));
            continue;
        }
            ...
            case G_PARAM_READABLE | G_PARAM_WRITABLE:
                ...
                if (pspecs[n]->value_type != class_pspec->value_type)
                    g_critical("Read/writable property '%s' on class '%s' has type '%s' which is not exactly equal to the "
                               ...
                               g_type_name(G_OBJECT_CLASS_TYPE(class)), g_type_name(G_PARAM_SPEC_VALUE_TYPE(class_pspec)), g_type_name(G_PARAM_SPEC_VALUE_TYPE(pspecs[n])), g_type_name(iface_type));
                break;
...
static gpointer g_object_new_with_custom_constructor(GObjectClass * class, GObjectConstructParam * params, guint n_params)
{
    ...
    if (object == NULL)
    {
        g_critical("Custom constructor for class %s returned NULL (which is invalid). " "Please use GInitable instead.", G_OBJECT_CLASS_NAME(class));
        return NULL;
    }
...
static gpointer g_object_new_internal(GObjectClass * class, GObjectConstructParam * params, guint n_params)
{
    GObjectNotifyQueue *nqueue = NULL;
    GObject *object;

    if G_UNLIKELY
        (CLASS_HAS_CUSTOM_CONSTRUCTOR(class)) return g_object_new_with_custom_constructor(class, params, n_params);
    ...
    if (n_parameters)
    {
        ...
        for (i = 0; i < n_parameters; i++)
        {
            ...
            if G_UNLIKELY
                (!pspec)
            {
                g_critical("%s: object class '%s' has no property named '%s'", G_STRFUNC, g_type_name(object_type), parameters[i].name);
                continue;
            }
            ...
            {
                g_critical("%s: property '%s' of object class '%s' is not writable", G_STRFUNC, pspec->name, g_type_name(object_type));
                continue;
            }
            ...
            if (pspec->flags & (G_PARAM_CONSTRUCT | G_PARAM_CONSTRUCT_ONLY))
            {
                ...
                if G_UNLIKELY
                    (k != j)
                {
                    g_critical("%s: construct property '%s' for type '%s' cannot be set twice", G_STRFUNC, parameters[i].name, g_type_name(object_type));
                    continue;
                }
            }
    ...
    if (first_property_name)
    {
        ...
        do
        {
            ...
            if G_UNLIKELY
                (!pspec)
            {
                g_critical("%s: object class '%s' has no property named '%s'", G_STRFUNC, g_type_name(object_type), name);
            ...
            if (pspec->flags & (G_PARAM_CONSTRUCT | G_PARAM_CONSTRUCT_ONLY))
            {
                ...
                if G_UNLIKELY
                    (i != n_params)
                {
                    g_critical("%s: property '%s' for type '%s' cannot be set twice", G_STRFUNC, name, g_type_name(object_type));
                    break;
                }
            }
 ...
 */
        void g_object_get_property(GObject * object, const gchar * property_name, GValue * value)
        {
            ...
            else
            {
                ...
                {
                    g_warning("%s: can't retrieve property '%s' of type '%s' as value of type '%s'", G_STRFUNC, pspec->name, g_type_name(pspec->value_type), G_VALUE_TYPE_NAME(value));
                    g_object_unref(object);
                    return;
                }
                else
 ...
 */
        void g_object_disconnect(gpointer _object, const gchar * signal_spec, ...)
        {
            ...
            while (signal_spec)
            {
                ...
                else if (!g_signal_handlers_disconnect_matched(object, mask | (detail ? G_SIGNAL_MATCH_DETAIL : 0), sid, detail, NULL, (gpointer) callback, data))
 ...
 */
        gboolean g_object_replace_qdata(GObject * object, GQuark quark, gpointer oldval, gpointer newval, GDestroyNotify destroy, GDestroyNotify * old_destroy)
        {
            ...
            return g_datalist_id_replace_data(&object->qdata, quark, oldval, newval, destroy, old_destroy);
        }
 ...
 */
        gpointer g_object_dup_data(GObject * object, const gchar * key, GDuplicateFunc dup_func, gpointer user_data)
        {
            ...
            return g_datalist_id_dup_data(&object->qdata, g_quark_from_string(key), dup_func, user_data);
        }
 ...
 */
        gboolean g_object_replace_data(GObject * object, const gchar * key, gpointer oldval, gpointer newval, GDestroyNotify destroy, GDestroyNotify * old_destroy)
        {
            ...
            return g_datalist_id_replace_data(&object->qdata, g_quark_from_string(key), oldval, newval, destroy, old_destroy);
        }
