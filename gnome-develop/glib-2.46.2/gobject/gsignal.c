...
static inline Handler *handler_new(guint signal_id, gpointer instance, gboolean after);
static void handler_insert(guint signal_id, gpointer instance, Handler * handler);
static Handler *handler_lookup(gpointer instance, gulong handler_id, GClosure * closure, guint * signal_id_p);
...
static HandlerMatch *handlers_find(gpointer instance, GSignalMatchType mask, guint signal_id, GQuark detail, GClosure * closure, gpointer func, gpointer data, gboolean one_and_only);
...
static inline Emission *emission_find(guint signal_id, GQuark detail, gpointer instance);
    ...
    if (mask & G_SIGNAL_MATCH_ID)
    {
        ...
        for (handler = hlist ? hlist->handlers : NULL; handler; handler = handler->next)
            if (handler->sequential_number &&
                ...
                ((mask & G_SIGNAL_MATCH_FUNC) || (handler->closure->marshal == node->c_marshaller && G_REAL_CLOSURE(handler->closure)->meta_marshal == NULL && ((GCClosure *) handler->closure)->callback == func)))
            {
...
static void node_update_single_va_closure(SignalNode * node)
{
    ...
    if (G_TYPE_IS_OBJECT(node->itype) && (node->flags & (G_SIGNAL_MUST_COLLECT)) == 0 && (node->emission_hooks == NULL || node->emission_hooks->hooks == NULL))
    {
        ...
        else if (bsa->n_nodes == 1)
        {
            ...
            if (cc->instance_type == 0)
            {
                ...
                if (run_type == G_SIGNAL_RUN_FIRST || run_type == G_SIGNAL_RUN_LAST)
                {
                    closure = cc->closure;
...
void _g_signals_destroy(GType itype)
{
    ...
    for (i = 1; i < g_n_signal_nodes; i++)
    {
        ...
        {
            if (node->destroyed)
                g_warning(G_STRLOC ": signal \"%s\" of type '%s' already destroyed", node->name, type_debug_name(node->itype));
            else
...
void g_signal_stop_emission(gpointer instance, guint signal_id, GQuark detail)
{
    ...
    if (node && g_type_is_a(G_TYPE_FROM_INSTANCE(instance), node->itype))
    {
        ...
        if (emission)
        {
            ...
            g_warning(G_STRLOC ": no emission of signal \"%s\" to stop for instance '%p'", node->name, instance);
    }
    else
...
gboolean g_signal_parse_name(const gchar * detailed_signal, GType itype, guint * signal_id_p, GQuark * detail_p, gboolean force_detail_quark)
{
    ...
    if (!node || node->destroyed || (detail && !(node->flags & G_SIGNAL_DETAILED)))
        return FALSE;
...
void g_signal_stop_emission_by_name(gpointer instance, const gchar * detailed_signal)
{
    ...
    if (signal_id)
    {
        ...
        if (detail && !(node->flags & G_SIGNAL_DETAILED))
            ...
            g_warning("%s: signal '%s' is invalid for instance '%p' of type '%s'", G_STRLOC, detailed_signal, instance, g_type_name(itype));
        else
        {
            Emission *emission = emission_find(signal_id, detail, instance);
...
guint g_signal_lookup(const gchar * name, GType itype)
{
    ...
    if (!signal_id)
    {
        ...
        if (!g_type_name(itype))
            g_warning(G_STRLOC ": unable to lookup signal \"%s\" for invalid type id '%" G_GSIZE_FORMAT "'", name, itype);
            ...
            g_warning(G_STRLOC ": unable to lookup signal \"%s\" of unloaded type '%s'", name, g_type_name(itype));
    }
...
guint *g_signal_list_ids(GType itype, guint * n_ids)
{
    ...
    *n_ids = result->len;
    ...
    if (!n_nodes)
    {
        /* give elaborate warnings */
        if (!g_type_name(itype))
            g_warning(G_STRLOC ": unable to list signals for invalid type id '%" G_GSIZE_FORMAT "'", itype);
...
guint g_signal_new(const gchar * signal_name, GType itype, GSignalFlags signal_flags, guint class_offset, GSignalAccumulator accumulator, gpointer accu_data, GSignalCMarshaller c_marshaller, GType return_type, guint n_params, ...)
{
    ...
    signal_id = g_signal_new_valist(signal_name, itype, signal_flags, class_offset ? g_signal_type_cclosure_new(itype, class_offset) : NULL, accumulator, accu_data, c_marshaller, return_type, n_params, args);
...
guint g_signal_new_class_handler(const gchar * signal_name, GType itype, GSignalFlags signal_flags, GCallback class_handler, GSignalAccumulator accumulator, gpointer accu_data, GSignalCMarshaller c_marshaller, GType return_type, guint n_params, ...)
{
    ...
    signal_id = g_signal_new_valist(signal_name, itype, signal_flags, class_handler ? g_cclosure_new(class_handler, NULL, NULL) : NULL, accumulator, accu_data, c_marshaller, return_type, n_params, args);
...
g_signal_newv(const gchar * signal_name, GType itype, GSignalFlags signal_flags, GClosure * class_closure, GSignalAccumulator accumulator, gpointer accu_data, GSignalCMarshaller c_marshaller, GType return_type, guint n_params, GType * param_types)
{
    ...
    if (node && !node->destroyed)
    {
        g_warning(G_STRLOC ": signal \"%s\" already exists in the '%s' %s", name, type_debug_name(node->itype), G_TYPE_IS_INTERFACE(node->itype) ? "interface" : "class ancestry");
        g_free(name);
        SIGNAL_UNLOCK();
        return 0;
    }
    ...
    for (i = 0; i < n_params; i++)
        ...
        {
            g_warning(G_STRLOC ": parameter %d of type '%s' for signal \"%s::%s\" is not a value type", i + 1, type_debug_name(param_types[i]), type_debug_name(itype), name);
            g_free(name);
            SIGNAL_UNLOCK();
            return 0;
        }
...
#define ADD_CHECK(__type__) \
    ...
    ADD_CHECK(BOOLEAN)
            ...
            ADD_CHECK(UCHAR)
            ADD_CHECK(INT)
            ADD_CHECK(UINT) ADD_CHECK(LONG) ADD_CHECK(ULONG) ADD_CHECK(ENUM) ADD_CHECK(FLAGS) ADD_CHECK(FLOAT) ADD_CHECK(DOUBLE) ADD_CHECK(STRING) ADD_CHECK(PARAM) ADD_CHECK(BOXED) ADD_CHECK(POINTER) ADD_CHECK(OBJECT) ADD_CHECK(VARIANT)}
...
void g_signal_chain_from_overridden_handler(gpointer instance, ...)
{
    ...
    if (closure)
    {
        ...
        {
            g_closure_invoke(closure, NULL, n_params + 1, instance_and_params, &emission->ihint);
        }
        else
        {
...
static void node_check_deprecated(const SignalNode * node)
{
    ...
    if (g_enable_diagnostic[0] == '1')
    {
        ...
        {
            g_warning("The signal %s::%s is deprecated and shouldn't be used " "anymore. It will be removed in a future version.", type_debug_name(node->itype), node->name);
        }
    }
}
...
static guint signal_handlers_foreach_matched_R(gpointer instance, GSignalMatchType mask, guint signal_id, GQuark detail, GClosure * closure, gpointer func, gpointer data, void (*callback) (gpointer instance, gulong handler_seq_no))
{
    HandlerMatch *mlist;
    guint n_handlers = 0;
...
gboolean g_signal_has_handler_pending(gpointer instance, guint signal_id, GQuark detail, gboolean may_be_blocked)
{
    ...
    mlist = handlers_find(instance, (G_SIGNAL_MATCH_ID | G_SIGNAL_MATCH_DETAIL | (may_be_blocked ? 0 : G_SIGNAL_MATCH_UNBLOCKED)), signal_id, detail, NULL, NULL, NULL, TRUE);
    if (mlist)
    {
        has_pending = TRUE;
        handler_match_free1_R(mlist, instance);
...
void g_signal_emitv(const GValue * instance_and_params, guint signal_id, GQuark detail, GValue * return_value)
{
    ...
    for (i = 0; i < node->n_params; i++)
        ...
        {
            g_critical("%s: value for '%s' parameter %u for signal \"%s\" is of type '%s'", G_STRLOC, type_debug_name(node->param_types[i]), i, node->name, G_VALUE_TYPE_NAME(param_values + i));
            SIGNAL_UNLOCK();
            return;
        }
    if (node->return_type != G_TYPE_NONE)
    {
        ...
        {
            g_critical("%s: return value '%s' for signal \"%s\" is of type '%s'", G_STRLOC, type_debug_name(node->return_type), node->name, G_VALUE_TYPE_NAME(return_value));
            SIGNAL_UNLOCK();
            return;
        }
    }
    ...
    if (node->single_va_closure != NULL && (node->single_va_closure == SINGLE_VA_CLOSURE_EMPTY_MAGIC || _g_closure_is_void(node->single_va_closure, instance)))
    {
...
void g_signal_emit_valist(gpointer instance, guint signal_id, GQuark detail, va_list var_args)
{
    ...
    if (node->single_va_closure != NULL)
    {
        ...
        if (node->single_va_closure != SINGLE_VA_CLOSURE_EMPTY_MAGIC && !_g_closure_is_void(node->single_va_closure, instance))
        {
        ...
        if (fastpath)
        {
            ...
            if (closure != NULL)
            {
                ...
                _g_closure_invoke_va(closure, return_accu, instance, var_args, node->n_params, node->param_types);
            ...
            if (rtype != G_TYPE_NONE)
            {
                ...
                G_VALUE_LCOPY(&emission_return, var_args, static_scope ? G_VALUE_NOCOPY_CONTENTS : 0, &error);
                if (!error)
...
void g_signal_emit_by_name(gpointer instance, const gchar * detailed_signal, ...)
{
    ...
    if (signal_id)
    {
        ...
        g_warning("%s: signal name '%s' is invalid for instance '%p' of type '%s'", G_STRLOC, detailed_signal, instance, g_type_name(itype));
}
...
EMIT_CLEANUP:
    ...
    if ((node->flags & G_SIGNAL_RUN_CLEANUP) && class_closure)
    {
        ...
        g_closure_invoke(class_closure, node->return_type != G_TYPE_NONE ? &accu : NULL, node->n_params + 1, instance_and_params, &emission.ihint);
        if (need_unset)
            g_value_unset(&accu);
        SIGNAL_LOCK();
        emission.chain_type = G_TYPE_NONE;
