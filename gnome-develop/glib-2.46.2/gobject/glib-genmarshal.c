...
static const GScannerConfig scanner_config_template = {
    ...
    (G_CSET_a_2_z "_0123456789" G_CSET_A_2_Z) /* cset_identifier_nth */ ,
    ...
    if (gen_cbody && gen_valist && !have_std_marshaller)
    {
                ...
                if (iarg->getter)
                {
                    ...
                    if (iarg->box != NULL)
                    {
                        ...
                        if (iarg->box_takes_type)
                            g_fprintf(fout, "  arg%i = %s (param_types[%i] & ~G_SIGNAL_TYPE_STATIC_SCOPE, arg%i);\n", i, iarg->box, i, i);
                        else
                            g_fprintf(fout, "  arg%i = %s (arg%i);\n", i, iarg->box, i);
                    }
                }
                i++;
            }
        ...
        for (node = sig->args; node; node = node->next)
        {
            ...
            if (iarg->unbox)
            {
                ...
                g_fprintf(fout, "arg%i != NULL)\n  ", i);
                if (iarg->box_takes_type)
                    g_fprintf(fout, "  %s (param_types[%i] & ~G_SIGNAL_TYPE_STATIC_SCOPE, arg%i);\n", iarg->unbox, i, i);
                else
                    g_fprintf(fout, "  %s (arg%i);\n", iarg->unbox, i);
            }
            i++;
