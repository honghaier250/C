...
#else                            /* !G_ENABLE_DEBUG */
#    define DEBUG_CODE(debug_type, code_block)
                                            /* code_block */
...
static inline IFaceEntry *lookup_iface_entry_I(volatile IFaceEntries * entries, TypeNode * iface_node)
{
    ...
    G_ATOMIC_ARRAY_DO_TRANSACTION(&iface_node->_prot.offsets, guint8, entry = NULL; offsets = transaction_data; offset_index = entries->offset_index; if (offsets != NULL && offset_index < G_ATOMIC_ARRAY_DATA_SIZE(offsets))
                                  {
                                  index = offsets[offset_index]; if (index > 0)
                                  {
                                  /* zero means unset, subtract one to get real index */
                                  index -= 1; if (index < IFACE_ENTRIES_N_ENTRIES(entries))
                                  {
                                  check = (IFaceEntry *) & entries->entry[index]; if (check->iface_type == NODE_TYPE(iface_node)) entry = check;}
                                  }
                                  }
    ) ;
...
static inline gboolean type_lookup_iface_vtable_I(TypeNode * node, TypeNode * iface_node, gpointer * vtable_ptr)
{
    ...
    G_ATOMIC_ARRAY_DO_TRANSACTION(CLASSED_NODE_IFACES_ENTRIES(node), IFaceEntries, entry = lookup_iface_entry_I(transaction_data, iface_node); res = entry != NULL; if (vtable_ptr)
                                  {
...
static gboolean check_value_table_I(const gchar * type_name, const GTypeValueTable * value_table)
{
    ...
    {
        if (value_table->value_free || value_table->value_copy || value_table->value_peek_pointer || value_table->collect_format || value_table->collect_value || value_table->lcopy_format || value_table->lcopy_value)
...
static gboolean check_type_info_I(TypeNode * pnode, GType ftype, const gchar * type_name, const GTypeInfo * info)
{
    ...
    {
        if (pnode)
            g_warning("cannot instantiate '%s', derived from non-instantiatable parent type '%s'", type_name, NODE_NAME(pnode));
        else
            g_warning("cannot instantiate '%s' as non-instantiatable fundamental", type_name);
        return FALSE;
    }
    /* check class & interface members */
    if (!((finfo->type_flags & G_TYPE_FLAG_CLASSED) || is_interface) && (info->class_init || info->class_finalize || info->class_data || info->class_size || info->base_init || info->base_finalize))
    {
        if (pnode)
            g_warning("cannot create class for '%s', derived from non-classed parent type '%s'", type_name, NODE_NAME(pnode));
        else
            g_warning("cannot create class for '%s' as non-classed fundamental", type_name);
        return FALSE;
    }
...
static gboolean check_add_interface_L(GType instance_type, GType iface_type)
{
    ...
    {
        g_warning("attempting to add an interface (%s) to class (%s) after class_init", NODE_NAME(iface), NODE_NAME(node));
        return FALSE;
    }
    ...
    if (NODE_PARENT_TYPE(tnode) && !type_lookup_iface_entry_L(node, tnode))
    {
        ...
        g_warning("cannot add sub-interface '%s' to type '%s' which does not conform to super-interface '%s'", NODE_NAME(iface), NODE_NAME(node), NODE_NAME(tnode));
        return FALSE;
    }
    ...
    if (tnode)
    {
        g_warning("cannot add interface type '%s' to type '%s', since type '%s' already conforms to interface", NODE_NAME(iface), NODE_NAME(node), NODE_NAME(tnode));
        return FALSE;
    }
    ...
    for (i = 0; i < IFACE_NODE_N_PREREQUISITES(iface); i++)
    {
        ...
        {
            g_warning("cannot add interface type '%s' to type '%s' which does not conform to prerequisite '%s'", NODE_NAME(iface), NODE_NAME(node), NODE_NAME(tnode));
            return FALSE;
        }
    }
    return TRUE;
...
static void type_data_make_W(TypeNode * node, const GTypeInfo * info, const GTypeValueTable * value_table)
{
    ...
    node->mutatable_check_cache = (node->data->common.value_table->value_init != NULL && !((G_TYPE_FLAG_VALUE_ABSTRACT | G_TYPE_FLAG_ABSTRACT) & GPOINTER_TO_UINT(type_get_qdata_L(node, static_quark_type_flags))));
...
static void type_iface_add_prerequisite_W(TypeNode * iface, TypeNode * prerequisite_node)
{
    ...
    g_assert(NODE_IS_IFACE(iface) && IFACE_NODE_N_PREREQUISITES(iface) < MAX_N_PREREQUISITES && (prerequisite_node->is_instantiatable || NODE_IS_IFACE(prerequisite_node)));
...
void g_type_interface_add_prerequisite(GType interface_type, GType prerequisite_type)
{
    ...
    {
        G_WRITE_UNLOCK(&type_rw_lock);
        g_warning("unable to add prerequisite '%s' to interface '%s' which is already in use for '%s'", type_descriptive_name_I(prerequisite_type), type_descriptive_name_I(interface_type), type_descriptive_name_I(holders->instance_type));
        return;
    }
    if (prerequisite_node->is_instantiatable)
    {
        ...
        for (i = 0; i < IFACE_NODE_N_PREREQUISITES(iface); i++)
        {
            ...
            {
                G_WRITE_UNLOCK(&type_rw_lock);
                g_warning("adding prerequisite '%s' to interface '%s' conflicts with existing prerequisite '%s'", type_descriptive_name_I(prerequisite_type), type_descriptive_name_I(interface_type), type_descriptive_name_I(NODE_TYPE(prnode)));
                return;
            }
        }
    ...
    {
        G_WRITE_UNLOCK(&type_rw_lock);
        g_warning("prerequisite '%s' for interface '%s' is neither instantiatable nor interface", type_descriptive_name_I(prerequisite_type), type_descriptive_name_I(interface_type));
    }
}
...
void g_type_remove_class_cache_func(gpointer cache_data, GTypeClassCacheFunc cache_func)
{
    ...
    if (!found_it)
        g_warning(G_STRLOC ": cannot remove unregistered class cache func %p with data %p", cache_func, cache_data);
}
...
void g_type_remove_interface_check(gpointer check_data, GTypeInterfaceCheckFunc check_func)
{
    ...
    if (!found_it)
        g_warning(G_STRLOC ": cannot remove unregistered class check func %p with data %p", check_func, check_data);
}
...
void g_type_class_unref(gpointer g_class)
{
    ...
    if (node && node->is_classed && NODE_REFCOUNT(node))
        ...
        g_warning("cannot unreference class of invalid (unclassed) type '%s'", type_descriptive_name_I(class->g_type));
}
...
gpointer g_type_class_peek(GType type)
{
    ...
    node = lookup_type_node_I(type);
    if (node && node->is_classed && NODE_REFCOUNT(node) && g_atomic_int_get(&node->data->class.init_state) == INITIALIZED)
...
void g_type_default_interface_unref(gpointer g_iface)
{
    ...
    if (node && NODE_IS_IFACE(node))
        ...
        g_warning("cannot unreference invalid interface default vtable for '%s'", type_descriptive_name_I(vtable->g_type));
}
...
static inline gboolean type_node_check_conformities_UorL(TypeNode * node, TypeNode * iface_node,
                                                         /*        support_inheritance */
                                                         gboolean support_interfaces, gboolean support_prerequisites, gboolean have_lock)
{
    gboolean match;
...
gboolean g_type_check_instance_is_fundamentally_a(GTypeInstance * type_instance, GType fundamental_type)
{
    TypeNode *node;
    if (!type_instance || !type_instance->g_class)
        return FALSE;
    ...
    if (type_instance)
    {
        if (type_instance->g_class)
        {
            ...
            if (is_instantiatable)
                ...
                g_warning("invalid uninstantiatable type '%s' in cast to '%s'", type_descriptive_name_I(type_instance->g_class->g_type), type_descriptive_name_I(iface_type));
        }
        else
            g_warning("invalid unclassed pointer in cast to '%s'", type_descriptive_name_I(iface_type));
    }
...
GTypeClass *g_type_check_class_cast(GTypeClass * type_class, GType is_a_type)
{
    if (type_class)
    {
        ...
        if (is_classed)
            g_warning("invalid class cast from '%s' to '%s'", type_descriptive_name_I(type_class->g_type), type_descriptive_name_I(is_a_type));
        else
            g_warning("invalid unclassed type '%s' in class cast to '%s'", type_descriptive_name_I(type_class->g_type), type_descriptive_name_I(is_a_type));
    }
    else
        g_warning("invalid class cast from (NULL) pointer to '%s'", type_descriptive_name_I(is_a_type));
    return type_class;
}
...
restart_table_peek:
    ...
    if (!has_refed_data)
        g_warning("can't peek value table for type '%s' which is not currently referenced", type_descriptive_name_I(type));
...
gint g_type_add_instance_private(GType class_gtype, gsize private_size)
{
    ...
    {
        g_warning("cannot add private field to invalid (non-instantiatable) type '%s'", type_descriptive_name_I(class_gtype));
        return 0;
    }

    if (node->plugin != NULL)
    {
        g_warning("cannot use g_type_add_instance_private() with dynamic type '%s'", type_descriptive_name_I(class_gtype));
        return 0;
    }
...
gint g_type_class_get_instance_private_offset(gpointer g_class)
{
    ...
    if (node->data->instance.private_size == parent_size)
        g_error("g_type_class_get_instance_private_offset() called on class %s but it has no private data", g_type_name(instance_type));
