...
retry:
        ...
        {
            if (!g_object_replace_data(object, "test", GINT_TO_POINTER(value), GINT_TO_POINTER(new_value), NULL, NULL))
                goto retry;
        }
        g_thread_yield();
    }
...
static void test_qdata_threaded(void)
{
    gint sum;
    gint i;
...
static void test_qdata_dup(void)
{
    gchar *s, *s2;
    GQuark quark;
    gboolean b;
