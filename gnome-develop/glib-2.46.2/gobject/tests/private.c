...
G_DEFINE_TYPE_WITH_CODE(TestObject, test_object, G_TYPE_OBJECT, G_ADD_PRIVATE(TestObject))
...
static void test_object_init(TestObject * self)
{
    ...
    if (g_test_verbose())
        g_printerr("Offset of %sPrivate for type '%s': %d\n", G_OBJECT_TYPE_NAME(self), G_OBJECT_TYPE_NAME(self), TestObject_private_offset);

    priv->dummy_0 = 42;
    priv->dummy_1 = 3.14159f;
}

static int test_object_get_dummy_0(TestObject * self)
{
    TestObjectPrivate *priv = test_object_get_instance_private(self);
...
static float test_object_get_dummy_1(TestObject * self)
{
    TestObjectPrivate *priv = test_object_get_instance_private(self);
...
G_DEFINE_TYPE_WITH_CODE(TestDerived, test_derived, test_object_get_type(), G_ADD_PRIVATE(TestDerived))
     static void test_derived_finalize(GObject * obj)
{
...
static void test_derived_class_init(TestDerivedClass * klass)
{
    G_OBJECT_CLASS(klass)->finalize = test_derived_finalize;
}

static void test_derived_init(TestDerived * self)
{
    TestDerivedPrivate *priv = test_derived_get_instance_private(self);

    if (g_test_verbose())
        g_printerr("Offset of %sPrivate for type '%s': %d\n", G_OBJECT_TYPE_NAME(self), G_OBJECT_TYPE_NAME(self), TestDerived_private_offset);

    priv->dummy_2 = g_strdup("Hello");
}

static const char *test_derived_get_dummy_2(TestDerived * self)
{
    TestDerivedPrivate *priv = test_derived_get_instance_private(self);
...
G_DEFINE_TYPE(TestMixed, test_mixed, test_object_get_type())
     static void test_mixed_class_init(TestMixedClass * klass)
{
    g_type_class_add_private(klass, sizeof(TestMixedPrivate));
}

static void test_mixed_init(TestMixed * self)
{
    TestMixedPrivate *priv = G_TYPE_INSTANCE_GET_PRIVATE(self, test_mixed_get_type(), TestMixedPrivate);

    if (g_test_verbose())
        g_printerr("Offset of %sPrivate for type '%s': %d\n", G_OBJECT_TYPE_NAME(self), G_OBJECT_TYPE_NAME(self), TestMixed_private_offset);

    priv->dummy_3 = 47;
}

static gint test_mixed_get_dummy_3(TestMixed * self)
{
    TestMixedPrivate *priv = G_TYPE_INSTANCE_GET_PRIVATE(self, test_mixed_get_type(), TestMixedPrivate);
...
G_DEFINE_TYPE_WITH_CODE(TestMixedDerived, test_mixed_derived, test_mixed_get_type(), G_ADD_PRIVATE(TestMixedDerived))
     static void test_mixed_derived_class_init(TestMixedDerivedClass * klass)
{
}

static void test_mixed_derived_init(TestMixedDerived * self)
{
    TestMixedDerivedPrivate *priv = test_mixed_derived_get_instance_private(self);

    if (g_test_verbose())
        g_printerr("Offset of %sPrivate for type '%s': %d\n", G_OBJECT_TYPE_NAME(self), G_OBJECT_TYPE_NAME(self), TestMixedDerived_private_offset);

    priv->dummy_4 = g_get_monotonic_time();
}

static gint64 test_mixed_derived_get_dummy_4(TestMixedDerived * self)
{
    TestMixedDerivedPrivate *priv = test_mixed_derived_get_instance_private(self);
...
static void private_instance(void)
{
...
static void private_derived_instance(void)
{
...
static void private_mixed_derived_instance(void)
{
