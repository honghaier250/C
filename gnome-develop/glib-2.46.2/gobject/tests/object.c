...
G_DEFINE_TYPE(MySingletonObject, my_singleton_object, G_TYPE_OBJECT)
     ...
     static void my_singleton_object_init(MySingletonObject * obj)
{
}

static GObject *my_singleton_object_constructor(GType type, guint n_construct_properties, GObjectConstructParam * construct_params)
{
    GObject *object;
    ...
    object = G_OBJECT_CLASS(my_singleton_object_parent_class)->constructor(type, n_construct_properties, construct_params);
    singleton = (MySingletonObject *) object;

    return object;
}

static void my_singleton_object_finalize(MySingletonObject * obj)
{
    singleton = NULL;
...
static void my_singleton_object_class_init(MySingletonObjectClass * klass)
{
...
static void test_object_constructor_singleton(void)
{
...
G_DEFINE_TYPE(MyInfanticideObject, my_infanticide_object, G_TYPE_OBJECT)
     static void my_infanticide_object_init(MyInfanticideObject * obj)
{
}

static GObject *my_infanticide_object_constructor(GType type, guint n_construct_properties, GObjectConstructParam * construct_params)
{
    GObject *object;

    object = G_OBJECT_CLASS(my_infanticide_object_parent_class)->constructor(type, n_construct_properties, construct_params);
...
static void my_infanticide_object_class_init(MyInfanticideObjectClass * klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS(klass);
...
static void test_object_constructor_infanticide(void)
{
    GObject *obj;
    int i;
    ...
    for (i = 0; i < 1000; i++)
    {
        g_test_expect_message("GLib-GObject", G_LOG_LEVEL_CRITICAL, "*finalized while still in-construction*");
        g_test_expect_message("GLib-GObject", G_LOG_LEVEL_CRITICAL, "*Custom constructor*returned NULL*");
