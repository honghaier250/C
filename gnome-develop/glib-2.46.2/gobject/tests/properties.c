...
enum
{ PROP_0, PROP_FOO, PROP_BAR, PROP_BAZ, PROP_QUUX, N_PROPERTIES };
...
static void test_object_set_quux(TestObject * obj, const gchar * quux)
{
...
static void test_object_class_init(TestObjectClass * klass)
{
    ...
    properties[PROP_QUUX] = g_param_spec_string("quux", "quux", "quux", NULL, G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY);
...
static void on_notify2(GObject * gobject, GParamSpec * pspec, Notifys * n)
{
    g_assert(n->pspec[n->pos] == pspec);
    n->pos++;
}

static void properties_notify_queue(void)
{
    TestObject *obj = g_object_new(test_object_get_type(), NULL);
    Notifys n;
...
static void properties_construct(void)
{
    ...
    obj = g_object_new(test_object_get_type(),
                       "foo", 1, "foo", 2, "foo", 3, "foo", 4, "foo", 5, "bar", FALSE, "foo", 6, "foo", 7, "foo", 8, "foo", 9, "foo", 10, "baz", "boo", "foo", 11, "foo", 12, "foo", 13, "foo", 14, "foo", 15, "foo", 16, "foo", 17, "foo", 18, NULL);
