...
static void test_source(GSource * one, GCallback quit_callback)
{
    GClosure *closure;
    GMainLoop *loop;
...
static gboolean simple_quit_callback(gpointer user_data)
{
...
static void test_closure_idle(void)
{
    GSource *source;
...
static void test_closure_timeout(void)
{
    GSource *source;
...
static gboolean iochannel_quit_callback(GIOChannel * channel, GIOCondition cond, gpointer user_data)
{
    GMainLoop *loop = user_data;
...
static void test_closure_iochannel(void)
{
    GIOChannel *chan;
    GSource *source;
    char *path;
    ...
    {
        path = g_test_build_filename(G_TEST_BUILT, g_get_prgname(), NULL);
    }
...
static void test_closure_child(void)
{
    GSource *source;
    GPid pid;
    GError *error = NULL;
    ...
    {
        argv[0] = g_test_build_filename(G_TEST_BUILT, g_get_prgname(), NULL);
    }
    argv[1] = "-l";
    argv[2] = NULL;

    g_spawn_async(NULL, argv, NULL, G_SPAWN_STDOUT_TO_DEV_NULL | G_SPAWN_STDERR_TO_DEV_NULL | G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL, &pid, &error);
    g_assert_no_error(error);
...
#ifdef G_OS_UNIX
static gboolean fd_quit_callback(gint fd, GIOCondition condition, gpointer user_data)
{
    GMainLoop *loop = user_data;
...
static void test_closure_fd(void)
{
    gint fd;
    GSource *source;
...
static gboolean send_usr1(gpointer user_data)
{
    kill(getpid(), SIGUSR1);
    return FALSE;
}

static gboolean closure_quit_callback(gpointer user_data)
{
    GMainLoop *loop = user_data;
...
static void test_closure_signal(void)
{
    GSource *source;
