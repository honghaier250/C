...
static GVariantSerialised gvs_tuple_get_child(GVariantSerialised value, gsize index_)
{
    ...
    if (member_info->i + 1)
        start = gvs_read_unaligned_le(value.data + value.size - offset_size * (member_info->i + 1), offset_size);
    else
        start = 0;
    ...
    else if (member_info->ending_type == G_VARIANT_MEMBER_ENDING_FIXED)
    {
        ...
        end = gvs_read_unaligned_le(value.data + value.size - offset_size * (member_info->i + 2), offset_size);
...
static gboolean gvs_tuple_is_normal(GVariantSerialised value)
{
    ...
    if G_UNLIKELY
        (value.data == NULL && value.size != 0) return FALSE;
...
gsize g_variant_serialised_n_children(GVariantSerialised serialised)
{
    ...
    DISPATCH_CASES(serialised.type_info, return gvs_ /**/, /**/ _n_children(serialised);) g_assert_not_reached();
}
...
GVariantSerialised g_variant_serialised_get_child(GVariantSerialised serialised, gsize index_)
{
    ...
    {
        DISPATCH_CASES(serialised.type_info, child = gvs_ /**/, /**/ _get_child(serialised, index_); g_assert(child.size || child.data == NULL); g_variant_serialised_check(child); return child;) g_assert_not_reached();
    }
 ...
 */
void g_variant_serialiser_serialise(GVariantSerialised serialised, GVariantSerialisedFiller gvs_filler, const gpointer * children, gsize n_children)
{
    g_variant_serialised_check(serialised);

    DISPATCH_CASES(serialised.type_info, gvs_ /**/, /**/ _serialise(serialised, gvs_filler, children, n_children); return;) g_assert_not_reached();
}
 ...
 */
gsize g_variant_serialiser_needed_size(GVariantTypeInfo * type_info, GVariantSerialisedFiller gvs_filler, const gpointer * children, gsize n_children)
{
    DISPATCH_CASES(type_info, return gvs_ /**/, /**/ _needed_size(type_info, gvs_filler, children, n_children);) g_assert_not_reached();
}
...
{
    DISPATCH_CASES(serialised.type_info, return gvs_ /**/, /**/ _is_normal(serialised);) if (serialised.data == NULL)
        return FALSE;
    ...
    switch (g_variant_type_info_get_type_char(serialised.type_info))
    {
        ...
        case 'o':
            return g_variant_serialiser_is_object_path(serialised.data, serialised.size);

        case 'g':
            return g_variant_serialiser_is_signature(serialised.data, serialised.size);
