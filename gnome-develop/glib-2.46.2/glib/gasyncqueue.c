...
static gpointer g_async_queue_pop_intern_unlocked(GAsyncQueue * queue, gboolean wait, gint64 end_time)
{
    gpointer retval;
...
gpointer g_async_queue_timed_pop(GAsyncQueue * queue, GTimeVal * end_time)
{
    ...
    if (end_time != NULL)
    {
        m_end_time = g_get_monotonic_time() + ((gint64) end_time->tv_sec * G_USEC_PER_SEC + end_time->tv_usec - g_get_real_time());
    }
    else
        m_end_time = -1;
