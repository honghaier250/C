...
GVariant *g_variant_lookup_value(GVariant * dictionary, const gchar * key, const GVariantType * expected_type)
{
    ...
    g_return_val_if_fail(g_variant_is_of_type(dictionary, G_VARIANT_TYPE("a{s*}")) || g_variant_is_of_type(dictionary, G_VARIANT_TYPE("a{o*}")), NULL);
        ...
        (array_element_size != element_size)
    {
        if (array_element_size)
            g_critical("g_variant_get_fixed_array: assertion "
                       "'g_variant_array_has_fixed_size (value, element_size)' " "failed: array size %" G_GSIZE_FORMAT " does not match " "given element_size %" G_GSIZE_FORMAT ".", array_element_size, element_size);
        else
...
GVariant *g_variant_new_fixed_array(const GVariantType * element_type, gconstpointer elements, gsize n_elements, gsize element_size)
{
    ...
    {
        if (array_element_size)
            g_critical("g_variant_new_fixed_array: array size %" G_GSIZE_FORMAT " does not match given element_size %" G_GSIZE_FORMAT ".", array_element_size, element_size);
        else
    ...
    value = g_variant_new_from_data(array_type, data, n_elements * element_size, FALSE, g_free, data);
...
const gchar *g_variant_get_string(GVariant * value, gsize * length)
{
    ...
    g_return_val_if_fail(value != NULL, NULL);
    g_return_val_if_fail(g_variant_is_of_type(value, G_VARIANT_TYPE_STRING) || g_variant_is_of_type(value, G_VARIANT_TYPE_OBJECT_PATH) || g_variant_is_of_type(value, G_VARIANT_TYPE_SIGNATURE), NULL);
...
GVariant *g_variant_new_objv(const gchar * const *strv, gssize length)
{
    ...
    return g_variant_new_from_children(G_VARIANT_TYPE_OBJECT_PATH_ARRAY, strings, length, TRUE);
}
...
{
    if G_UNLIKELY
        (string == NULL) string = g_string_new(NULL);

    switch (g_variant_classify(value))
    {
        case G_VARIANT_CLASS_MAYBE:
            if (type_annotate)
                g_string_append_printf(string, "@%s ", g_variant_get_type_string(value));
        ...
        case G_VARIANT_CLASS_INT32:
             ...
             */
            g_string_append_printf(string, "%" G_GINT32_FORMAT, g_variant_get_int32(value));
            break;
...
gboolean g_variant_equal(gconstpointer one, gconstpointer two)
{
    ...
    if (g_variant_get_type_info((GVariant *) one) != g_variant_get_type_info((GVariant *) two))
        return FALSE;
...
gint g_variant_compare(gconstpointer one, gconstpointer two)
{
    ...
    switch (g_variant_classify(a))
    {
        ...
        case G_VARIANT_CLASS_BYTE:
            return ((gint) g_variant_get_byte(a)) - ((gint) g_variant_get_byte(b));

        case G_VARIANT_CLASS_INT16:
            return ((gint) g_variant_get_int16(a)) - ((gint) g_variant_get_int16(b));

        case G_VARIANT_CLASS_UINT16:
            return ((gint) g_variant_get_uint16(a)) - ((gint) g_variant_get_uint16(b));
...
void g_variant_builder_add_value(GVariantBuilder * builder, GVariant * value)
{
    ...
    if (!GVSB(builder)->uniform_item_types)
    {
        ...
        if (GVSB(builder)->expected_type)
            GVSB(builder)->expected_type = g_variant_type_next(GVSB(builder)->expected_type);

        if (GVSB(builder)->prev_item_type)
            GVSB(builder)->prev_item_type = g_variant_type_next(GVSB(builder)->prev_item_type);
    }
    else
...
void g_variant_builder_open(GVariantBuilder * builder, const GVariantType * type)
{
    ...
    if (GVSB(parent)->prev_item_type)
    {
        if (!GVSB(builder)->uniform_item_types)
            ...
            GVSB(builder)->prev_item_type = g_variant_type_first(GVSB(parent)->prev_item_type);

        else if (!g_variant_type_is_variant(GVSB(builder)->type))
            /* maybes and arrays */
            GVSB(builder)->prev_item_type = g_variant_type_element(GVSB(parent)->prev_item_type);
    }
}
...
GVariant *g_variant_builder_end(GVariantBuilder * builder)
{
    ...
    if (g_variant_type_is_definite(GVSB(builder)->type))
        ...
        my_type = g_variant_make_tuple_type(GVSB(builder)->children, GVSB(builder)->offset);

    else if (g_variant_type_is_dict_entry(GVSB(builder)->type))
        my_type = g_variant_make_dict_entry_type(GVSB(builder)->children[0], GVSB(builder)->children[1]);
    else
        g_assert_not_reached();

    value = g_variant_new_from_children(my_type, g_renew(GVariant *, GVSB(builder)->children, GVSB(builder)->offset), GVSB(builder)->offset, GVSB(builder)->trusted);
            ...
            case '&':
                    ...
                    (copy_only)
                {
                    ...
                    g_critical("g_variant_check_format_string() is being called by a function with a GVariant varargs "
                               ...
                               "data inside of a GVariant instance that may no longer exist by the time the function " "returns.  Modify your code to use a format string without '&'.", original_format);
                    return FALSE;
                }
...
static gboolean valid_format_string(const gchar * format_string, gboolean single, GVariant * value)
{
    ...
    if G_UNLIKELY
        (type == NULL || (single && *endptr != '\0'))
    {
        if (single)
            g_critical("'%s' is not a valid GVariant format string", format_string);
        else
            g_critical("'%s' does not have a valid GVariant format " "string as a prefix", format_string);
    ...
    if G_UNLIKELY
        (value && !g_variant_is_of_type(value, type))
    {
        gchar *fragment;
        gchar *typestr;
    ...
    switch (*(*str)++)
    {
        case 'a':
            if (ptr != NULL)
            {
                ...
                if G_UNLIKELY
                    (!g_variant_type_is_array(type)) g_error("g_variant_new: expected array GVariantBuilder but " "the built value has type '%s'", g_variant_get_type_string(value));

                type = g_variant_type_element(type);

                if G_UNLIKELY
                    (!g_variant_type_is_subtype_of(type, (GVariantType *) * str))
                        g_error("g_variant_new: expected GVariantBuilder array element " "type '%s' but the built value has element type '%s'", g_variant_type_dup_string((GVariantType *) * str), g_variant_get_type_string(value) + 1);
                ...
                /* special case: NULL pointer for empty array */
            {
                ...
                if G_UNLIKELY
                    (!g_variant_type_is_definite(type)) g_error("g_variant_new: NULL pointer given with indefinite " "array type; unable to determine which type of empty " "array to construct.");
        ...
        case '@':
            if G_UNLIKELY
                (!g_variant_is_of_type(ptr, (GVariantType *) * str)) g_error("g_variant_new: expected GVariant of type '%s' but " "received value has type '%s'", g_variant_type_dup_string((GVariantType *) * str), g_variant_get_type_string(ptr));
        ...
        case '?':
            if G_UNLIKELY
                (!g_variant_type_is_basic(g_variant_get_type(ptr))) g_error("g_variant_new: format string '?' expects basic-typed " "GVariant, but received value has type '%s'", g_variant_get_type_string(ptr));

            return ptr;

        case 'r':
            if G_UNLIKELY
                (!g_variant_type_is_tuple(g_variant_get_type(ptr))) g_error("g_variant_new: format string 'r' expects tuple-typed " "GVariant, but received value has type '%s'", g_variant_get_type_string(ptr));
...
GVariant *g_variant_new(const gchar * format_string, ...)
{
    ...
    g_return_val_if_fail(valid_format_string(format_string, TRUE, NULL) && format_string[0] != '?' && format_string[0] != '@' && format_string[0] != '*' && format_string[0] != 'r', NULL);
