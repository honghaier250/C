    ...
    switch (stream->stream[0])
    {
        ...
        case '9':
            for (end = stream->stream; end != stream->end; end++)
                if (!g_ascii_isalnum(*end) && *end != '-' && *end != '+' && *end != '.')
                    break;
            break;
        ...
        case '%':
             ...
             */
            for (end = stream->stream + 1; end != stream->end && *end != ',' && *end != ':' && *end != '>' && *end != ']' && !g_ascii_isspace(*end); end++)
...
static gboolean token_stream_peek_string(TokenStream * stream, const gchar * token)
{
    gint length = strlen(token);

    return token_stream_prepare(stream) && stream->stream - stream->this == length && memcmp(stream->this, token, length) == 0;
}
...
static gchar *number_get_pattern(AST * ast, GError ** error)
{
    ...
    if (strchr(number->token, '.') || (!g_str_has_prefix(number->token, "0x") && strchr(number->token, 'e')) || strstr(number->token, "inf") || strstr(number->token, "nan"))
        return g_strdup("Md");
...
{
    ast_set_error(ast, error, NULL, G_VARIANT_PARSE_ERROR_NUMBER_OUT_OF_RANGE, "number out of range for type '%c'", g_variant_type_peek_string(type)[0]);
    return NULL;
}
...
static GVariant *positional_get_value(AST * ast, const GVariantType * type, GError ** error)
{
    ...
    if G_UNLIKELY
        (!g_variant_is_of_type(positional->value, type)) return ast_type_error(ast, type, error);
...
GVariant *g_variant_parse(const GVariantType * type, const gchar * text, const gchar * limit, const gchar ** endptr, GError ** error)
{
    ...
    if ((ast = parse(&stream, NULL, error)))
    {
        ...
        if (result != NULL)
        {
            ...
            if (endptr == NULL)
            {
                while (stream.stream != limit && g_ascii_isspace(*stream.stream))
                    stream.stream++;
...
static void add_lines_from_range(GString * err, const gchar * str, const gchar * start1, const gchar * end1, const gchar * start2, const gchar * end2)
{
...
gchar *g_variant_parse_error_print_context(GError * error, const gchar * source_str)
{
    ...
    else
    {
        ...
        if (comma && comma < colon)
        {
            ...
            if (!parse_num(error->message, dash, &start1) || !parse_num(dash + 1, comma, &end1) || !parse_num(comma + 1, dash2, &start2) || !parse_num(dash2 + 1, colon, &end2))
                goto out;
