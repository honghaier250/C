...
static GIConv open_converter(const gchar * to_codeset, const gchar * from_codeset, GError ** error)
{
    ...
    if (cd == (GIConv) - 1)
    {
        ...
        {
            if (errno == EINVAL)
                g_set_error(error, G_CONVERT_ERROR, G_CONVERT_ERROR_NO_CONVERSION, _("Conversion from character set '%s' to '%s' is not supported"), from_codeset, to_codeset);
            else
            ...
            switch (errno)
            {
                ...
                case EILSEQ:
                    if (save_p)
                    {
                         ...
                         */
                        g_set_error(error, G_CONVERT_ERROR, G_CONVERT_ERROR_ILLEGAL_SEQUENCE, _("Cannot convert fallback '%s' to codeset '%s'"), insert_str, to_codeset);
                        have_error = TRUE;
                        break;
                    }
                    else if (p)
...
gchar *g_locale_to_utf8(const gchar * opsysstring, gssize len, gsize * bytes_read, gsize * bytes_written, GError ** error)
{
    ...
    if (g_get_charset(&charset))
        ...
        return g_convert(opsysstring, len, "UTF-8", charset, bytes_read, bytes_written, error);
}
...
gchar *g_locale_from_utf8(const gchar * utf8string, gssize len, gsize * bytes_read, gsize * bytes_written, GError ** error)
{
    ...
    if (g_get_charset(&charset))
        ...
        return g_convert(utf8string, len, charset, "UTF-8", bytes_read, bytes_written, error);
}
...
gchar *g_filename_to_uri(const gchar * filename, const gchar * hostname, GError ** error)
{
    ...
    {
        g_set_error(error, G_CONVERT_ERROR, G_CONVERT_ERROR_NOT_ABSOLUTE_PATH, _("The pathname '%s' is not an absolute path"), filename);
        return NULL;
    }

    if (hostname && !(g_utf8_validate(hostname, -1, NULL) && hostname_validate(hostname)))
    {
