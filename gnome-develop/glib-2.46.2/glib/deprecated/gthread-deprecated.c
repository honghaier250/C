...
GLIB_AVAILABLE_IN_ALL void g_thread_init_glib(void);
...
GThread *g_thread_create_full(GThreadFunc func, gpointer data, gulong stack_size, gboolean joinable, gboolean bound, GThreadPriority priority, GError ** error)
{
    ...
    thread = g_thread_new_internal(NULL, g_deprecated_thread_proxy, func, data, stack_size, error);
    ...
    if (!private_key->index)
    {
        ...
        if (!private_key->index)
        {
            if (g_thread_free_indices)
            {
                ...
                g_thread_free_indices = g_slist_delete_link(g_thread_free_indices, g_thread_free_indices);
            }
            else
...
void g_static_private_free(GStaticPrivate * private_key)
{
     ...
     */
    G_LOCK(g_thread);
    g_thread_free_indices = g_slist_prepend(g_thread_free_indices, GUINT_TO_POINTER(idx));
    G_UNLOCK(g_thread);
}
