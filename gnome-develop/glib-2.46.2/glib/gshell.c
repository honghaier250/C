...
static GSList *tokenize_command_line(const gchar * command_line, GError ** error)
{
    ...
    if (current_quote)
    {
        if (current_quote == '\\')
            ...
            g_set_error(error, G_SHELL_ERROR, G_SHELL_ERROR_BAD_QUOTING, _("Text ended before matching quote was found for %c." " (The text was '%s')"), current_quote, command_line);
