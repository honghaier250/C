...
static void propagate_error(GMarkupParseContext * context, GError ** dest, GError * src)
{
    if (context->flags & G_MARKUP_PREFIX_ERROR_POSITION)
        g_prefix_error(&src, _("Error on line %d char %d: "), context->line_number, context->char_number);
...
static gboolean slow_name_validate(GMarkupParseContext * context, const gchar * name, GError ** error)
{
    ...
    {
        /* is_name_char */
        if (!(g_ascii_isalnum(*p) || (!IS_COMMON_NAME_END_CHAR(*p) && (*p == '.' || *p == '-' || *p == '_' || *p == ':' || g_unichar_isalpha(g_utf8_get_char(p))))))
        {
...
static gboolean name_validate(GMarkupParseContext * context, const gchar * name, GError ** error)
{
    ...
    /* name start char */
    p = name;
    if (G_UNLIKELY(IS_COMMON_NAME_END_CHAR(*p) || !(g_ascii_isalpha(*p) || *p == '_' || *p == ':')))
        goto slow_validate;
        ...
        if (*from == '&')
        {
            ...
            if (*from == '#')
            {
                ...
                if (end == from || errno != 0)
                {
                    set_unescape_error(context, error,
                                       from, G_MARKUP_ERROR_PARSE, _("Failed to parse '%-.*s', which " "should have been a digit " "inside a character reference " "(&#234; for example) - perhaps " "the digit is too large"), (int) (end - from), from);
                    return FALSE;
                }
                else if (*end != ';')
                {
                    set_unescape_error(context, error,
                                       from, G_MARKUP_ERROR_PARSE, _("Character reference did not end with a " "semicolon; " "most likely you used an ampersand " "character without intending to start " "an entity - escape ampersand as &amp;"));
                    return FALSE;
                }
                else
                {
                    ...
                    {
                        set_unescape_error(context, error, from, G_MARKUP_ERROR_PARSE, _("Character reference '%-.*s' does not " "encode a permitted character"), (int) (end - from), from);
                        return FALSE;
                    }
                }
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (end)
                        set_unescape_error(context, error, from, G_MARKUP_ERROR_PARSE, _("Entity name '%-.*s' is not known"), (int) (end - from), from);
                    else
                        set_unescape_error(context, error, from, G_MARKUP_ERROR_PARSE, _("Entity did not end with a semicolon; " "most likely you used an ampersand " "character without intending to start " "an entity - escape ampersand as &amp;"));
                }
                return FALSE;
            }
        }
...
static void add_to_partial(GMarkupParseContext * context, const gchar * text_start, const gchar * text_end)
{
    ...
    if (text_start != text_end)
        g_string_insert_len(context->partial_chunk, -1, text_start, text_end - text_start);
}
...
static void ensure_no_outstanding_subparser(GMarkupParseContext * context)
{
    if (context->awaiting_pop)
        g_critical("During the first end_element call after invoking a "
                   "subparser you must pop the subparser stack and handle " "the freeing of the subparser user_data.  This can be " "done by calling the end function of the subparser.  " "Very probably, your program just leaked memory.");
        ...
        switch (context->state)
        {
            ...
            case STATE_AFTER_OPEN_ANGLE:
                ...
                else
                {
                    ...
                    set_error(context, error, G_MARKUP_ERROR_PARSE, _("'%s' is not a valid character following " "a '<' character; it may not begin an " "element name"), utf8_str(context->iter, buf));
                }
                break;
            ...
            case STATE_AFTER_ELISION_SLASH:
                ...
                else
                {
                    ...
                    set_error(context, error, G_MARKUP_ERROR_PARSE, _("Odd character '%s', expected a '>' character " "to end the empty-element tag '%s'"), utf8_str(context->iter, buf), current_element(context));
                }
                break;
            ...
            case STATE_AFTER_ATTRIBUTE_NAME:
                ...
                if (context->iter != context->current_text_end)
                {
                    ...
                    else
                    {
                        ...
                        set_error(context, error, G_MARKUP_ERROR_PARSE, _("Odd character '%s', expected a '=' after " "attribute name '%s' of element '%s'"), utf8_str(context->iter, buf), current_attribute(context), current_element(context));
                    ...
                    else
                    {
                        ...
                        set_error(context,
                                  ...
                                  _("Odd character '%s', expected a '>' or '/' "
                                    "character to end the start tag of " "element '%s', or optionally an attribute; " "perhaps you used an invalid character in " "an attribute name"), utf8_str(context->iter, buf), current_element(context));
                    }
                    ...
                    else
                    {
                        ...
                        set_error(context,
                                  ...
                                  _("Odd character '%s', expected an open quote mark "
                                    "after the equals sign when giving value for " "attribute '%s' of element '%s'"), utf8_str(context->iter, buf), current_attribute(context), current_element(context));
                    }
                }
                break;
            ...
            case STATE_AFTER_CLOSE_TAG_SLASH:
                ...
                else
                {
                    ...
                    set_error(context, error, G_MARKUP_ERROR_PARSE, _("'%s' is not a valid character following " "the characters '</'; '%s' may not begin an " "element name"), utf8_str(context->iter, buf), utf8_str(context->iter, buf));
                }
                break;
            ...
            case STATE_AFTER_CLOSE_TAG_NAME:
                ...
                if (context->iter != context->current_text_end)
                {
                    ...
                    if (*context->iter != '>')
                    {
                        ...
                        set_error(context, error, G_MARKUP_ERROR_PARSE, _("'%s' is not a valid character following " "the close element name '%s'; the allowed " "character is '>'"), utf8_str(context->iter, buf), close_name->str);
                    }
                    ...
                    {
                        set_error(context, error, G_MARKUP_ERROR_PARSE, _("Element '%s' was closed, but the currently " "open element is '%s'"), close_name->str, current_element(context));
                    }
                    else
                    {
                        advance_char(context);
...
gboolean g_markup_parse_context_end_parse(GMarkupParseContext * context, GError ** error)
{
    ...
    switch (context->state)
    {
        ...
        case STATE_AFTER_ELISION_SLASH:
            set_error(context, error, G_MARKUP_ERROR_PARSE, _("Document ended unexpectedly, expected to see a close angle " "bracket ending the tag <%s/>"), current_element(context));
            break;
        ...
        case STATE_AFTER_ATTRIBUTE_EQUALS_SIGN:
            set_error_literal(context, error, G_MARKUP_ERROR_PARSE, _("Document ended unexpectedly after the equals sign " "following an attribute name; no attribute value"));
            break;
        ...
        case STATE_INSIDE_CLOSE_TAG_NAME:
        case STATE_AFTER_CLOSE_TAG_NAME:
            set_error(context, error, G_MARKUP_ERROR_PARSE, _("Document ended unexpectedly inside the close tag for " "element '%s'"), current_element(context));
            break;
...
gboolean g_markup_collect_attributes(const gchar * element_name, const gchar ** attribute_names, const gchar ** attribute_values, GError ** error, GMarkupCollectType first_type, const gchar * first_attr, ...)
{
        ...
        switch (type)
        {
            ...
            case G_MARKUP_COLLECT_TRISTATE:
                ...
                else
                {
                    ...
                    {
                        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, "element '%s', attribute '%s', value '%s' " "cannot be parsed as a boolean value", element_name, attr, value);
    ...
    for (i = 0; attribute_names[i]; i++)
        if ((collected & (G_GUINT64_CONSTANT(1) << i)) == 0)
        {
            ...
            if (i == j)
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ATTRIBUTE, "attribute '%s' invalid for element '%s'", attribute_names[i], element_name);
            else
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, "attribute '%s' given multiple times for element '%s'", attribute_names[i], element_name);
