...
static gboolean g_thread_pool_start_thread(GRealThreadPool * pool, GError ** error);
...
{
    if (pool->sort_func)
        g_async_queue_push_sorted_unlocked(pool->queue, data, pool->sort_func, pool->sort_user_data);
    else
...
static GRealThreadPool *g_thread_pool_wait_for_new_pool(void)
{
    ...
    do
    {
        ...
        else if (local_max_idle_time > 0)
        {
            ...
            pool = g_async_queue_timeout_pop(unused_thread_queue, local_max_idle_time * 1000);
        }
        else
        {
...
static gpointer g_thread_pool_wait_for_new_task(GRealThreadPool * pool)
{
    ...
    if (pool->running || (!pool->immediate && g_async_queue_length_unlocked(pool->queue) > 0))
    {
        ...
        else if (pool->pool.exclusive)
        {
            ...
            DEBUG_MSG(("thread %p in exclusive pool %p waits for task " "(%d running, %d unprocessed).", g_thread_self(), pool, pool->num_threads, g_async_queue_length_unlocked(pool->queue)));
        }
        else
        {
             ...
             */
            DEBUG_MSG(("thread %p in pool %p waits for up to a 1/2 second for task " "(%d running, %d unprocessed).", g_thread_self(), pool, pool->num_threads, g_async_queue_length_unlocked(pool->queue)));

            task = g_async_queue_timeout_pop_unlocked(pool->queue, G_USEC_PER_SEC / 2);
        }
    }
    else
    {
        ...
        DEBUG_MSG(("pool %p not active, thread %p will go to global pool "
                   "(running: %s, immediate: %s, len: %d).", pool, g_thread_self(), pool->running ? "true" : "false", pool->immediate ? "true" : "false", g_async_queue_length_unlocked(pool->queue)));
    }
...
void g_thread_pool_free(GThreadPool * pool, gboolean immediate, gboolean wait_)
{
    ...
    if (wait_)
    {
        while (g_async_queue_length_unlocked(real->queue) != -real->num_threads && !(immediate && real->num_threads == 0))
