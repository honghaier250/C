...
void g_variant_unref(GVariant * value)
{
    ...
    {
        if G_UNLIKELY
            (value->state & STATE_LOCKED) g_critical("attempting to free a locked GVariant instance.  " "This should never happen.");
...
GBytes *g_variant_get_data_as_bytes(GVariant * value)
{
    ...
    if (data == bytes_data && size == bytes_size)
        ...
        return g_bytes_new_from_bytes(value->contents.serialised.bytes, data - bytes_data, size);
}
...
GVariant *g_variant_get_child_value(GVariant * value, gsize index_)
{
...

    {
        ...
        child->ref_count = 1;
        child->contents.serialised.bytes = g_bytes_ref(value->contents.serialised.bytes);
