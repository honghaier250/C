...
#    endif
    ...
    while (test & G_FILE_TEST_IS_EXECUTABLE)
    {
        ...
        if (_stricmp(lastdot, ".exe") == 0 || _stricmp(lastdot, ".cmd") == 0 || _stricmp(lastdot, ".bat") == 0 || _stricmp(lastdot, ".com") == 0)
            return TRUE;
...
static char *format_error_message(const gchar * filename, const gchar * format_string, int saved_errno) G_GNUC_FORMAT(2);

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wformat-nonliteral"

     static char *format_error_message(const gchar * filename, const gchar * format_string, int saved_errno)
{
    gchar *display_name;
    gchar *msg;
 ...
 */
static void set_file_error(GError ** error, const gchar * filename, const gchar * format_string, int saved_errno)
{
    char *msg = format_error_message(filename, format_string, saved_errno);

    g_set_error_literal(error, G_FILE_ERROR, g_file_error_from_errno(saved_errno), msg);
    g_free(msg);
}

static gboolean get_contents_stdio(const gchar * filename, FILE * f, gchar ** contents, gsize * length, GError ** error)
{
    gchar buf[4096];
        ...
        while (total_bytes + bytes >= total_allocated)
        {
            ...
            if (tmp == NULL)
            {
                ...
                g_set_error(error,
                            G_FILE_ERROR,
                            G_FILE_ERROR_NOMEM,
                            g_dngettext(GETTEXT_PACKAGE, "Could not allocate %lu byte to read file \"%s\"", "Could not allocate %lu bytes to read file \"%s\"", (gulong) total_allocated), (gulong) total_allocated, display_filename);
...
static gboolean get_contents_regfile(const gchar * filename, struct stat *stat_buf, gint fd, gchar ** contents, gsize * length, GError ** error)
{
    gchar *buf;
    gsize bytes_read;
    gsize size;
    ...
    if (buf == NULL)
    {
        display_filename = g_filename_display_name(filename);
        g_set_error(error,
                    G_FILE_ERROR, G_FILE_ERROR_NOMEM, g_dngettext(GETTEXT_PACKAGE, "Could not allocate %lu byte to read file \"%s\"", "Could not allocate %lu bytes to read file \"%s\"", (gulong) alloc_size), (gulong) alloc_size, display_filename);
        g_free(display_filename);
        goto error;
    }
    ...
    while (bytes_read < size)
    {
        ...
        if (rc < 0)
        {
            if (errno != EINTR)
            {
                ...
                g_free(buf);
                display_filename = g_filename_display_name(filename);
                g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(save_errno), _("Failed to read from file '%s': %s"), display_filename, g_strerror(save_errno));
                g_free(display_filename);
                goto error;
            }
        }
...
static gboolean rename_file(const char *old_name, const char *new_name, GError ** err)
{
    ...
    if (g_rename(old_name, new_name) == -1)
    {
        ...
        g_set_error(err, G_FILE_ERROR, g_file_error_from_errno(save_errno), _("Failed to rename file '%s' to '%s': g_rename() failed: %s"), display_old_name, display_new_name, g_strerror(save_errno));
...
static gchar *write_to_temp_file(const gchar * contents, gssize length, const gchar * dest_file, GError ** err)
{
    ...
    {
        int saved_errno = errno;
        set_file_error(err, tmp_name, _("Failed to create file '%s': %s"), saved_errno);
        goto out;
    }
    ...
    while (length > 0)
    {
        ...
        if (s < 0)
        {
            ...
            set_file_error(err, tmp_name, _("Failed to write file '%s': write() failed: %s"), saved_errno);
            close(fd);
            g_unlink(tmp_name);
...
#ifdef HAVE_FSYNC
    {
        ...
        {
            int saved_errno = errno;
            set_file_error(err, tmp_name, _("Failed to write file '%s': fsync() failed: %s"), saved_errno);
            close(fd);
            g_unlink(tmp_name);
...
gboolean g_file_set_contents(const gchar * filename, const gchar * contents, gssize length, GError ** error)
{
    ...
    if (!rename_file(tmp_filename, filename, &rename_error))
    {
        ...
        {
            int saved_errno = errno;
            set_file_error(error, filename, _("Existing file '%s' could not be removed: g_unlink() failed: %s"), saved_errno);
            g_unlink(tmp_filename);
            retval = FALSE;
            goto out;
        }
 ...
 */
static gint wrap_g_mkdir(const gchar * filename, int flags G_GNUC_UNUSED, int mode)
{
...
static gint g_get_tmp_name(const gchar * tmpl, gchar ** name_used, GTmpFileCallback f, gint flags, gint mode, GError ** error)
{
    int retval;
    const char *tmpdir;
    const char *sep;
...
gint g_file_open_tmp(const gchar * tmpl, gchar ** name_used, GError ** error)
{
    ...
    result = g_get_tmp_name(tmpl, &fulltemplate, wrap_g_open, O_CREAT | O_EXCL | O_RDWR | O_BINARY, 0600, error);
    if (result != -1)
    {
        if (name_used)
            *name_used = fulltemplate;
...
static gchar *g_build_pathname_va(const gchar * first_element, va_list * args, gchar ** str_array)
{
    ...
    while (TRUE)
    {
        ...
        if (TRUE)
        {
            ...
            last_trailing = end;
            while (last_trailing >= element + 1 && (last_trailing[-1] == '\\' || last_trailing[-1] == '/'))
                last_trailing--;
...
gchar *g_path_get_dirname(const gchar * file_name)
{
    ...
    if (base == file_name + 1 && g_ascii_isalpha(file_name[0]) && file_name[1] == ':')
        base++;
    else if (G_IS_DIR_SEPARATOR(file_name[0]) && G_IS_DIR_SEPARATOR(file_name[1]) && file_name[2] && !G_IS_DIR_SEPARATOR(file_name[2]) && base >= file_name + 2)
    {
...
gchar *g_get_current_dir(void)
{
    ...
    pwd = g_getenv("PWD");
    if (pwd != NULL && g_stat(".", &dotbuf) == 0 && g_stat(pwd, &pwdbuf) == 0 && dotbuf.st_dev == pwdbuf.st_dev && dotbuf.st_ino == pwdbuf.st_ino)
        return g_strdup(pwd);
...
static gint wrap_libc_open(const gchar * filename, int flags, int mode)
{
...
gint g_mkstemp(gchar * tmpl)
{
     ...
     */
    return get_tmp_file(tmpl, wrap_libc_open, O_RDWR | O_CREAT | O_EXCL, 0600);
}
