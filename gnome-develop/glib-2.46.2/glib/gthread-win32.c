...
typedef struct
{
    ...
    void (__stdcall * DeleteConditionVariable) (gpointer cond);    /* fake */
      BOOL(__stdcall * SleepConditionVariableSRW) (gpointer cond, gpointer lock, DWORD timeout, ULONG flags);
...
static CRITICAL_SECTION *g_rec_mutex_impl_new(void)
{
    CRITICAL_SECTION *cs;
...
static void g_rec_mutex_impl_free(CRITICAL_SECTION * cs)
{
    DeleteCriticalSection(cs);
    g_slice_free(CRITICAL_SECTION, cs);
}

static CRITICAL_SECTION *g_rec_mutex_get_impl(GRecMutex * mutex)
{
...
void g_cond_wait(GCond * cond, GMutex * entered_mutex)
{
    g_thread_impl_vtable.SleepConditionVariableSRW(cond, entered_mutex, INFINITE, 0);
}

gboolean g_cond_wait_until(GCond * cond, GMutex * entered_mutex, gint64 end_time)
{
    gint64 span;
    ...
    if G_UNLIKELY
        (span > G_GINT64_CONSTANT(1000) * G_MAXINT32) span = INFINITE;
...
static DWORD g_private_get_impl(GPrivate * key)
{
        ...
        (impl == 0)
    {
        ...
        if (impl == 0)
        {
            ...
            if (key->notify != NULL)
            {
                ...
                if G_UNLIKELY
                    (destructor == NULL) g_thread_abort(errno, "malloc");
                 ...
                 */
                if (InterlockedCompareExchangePointer(&g_private_destructors, destructor, destructor->next) != destructor->next)
...
static guint __stdcall g_thread_win32_proxy(gpointer data)
{
...
GRealThread *g_system_thread_new(GThreadFunc func, gulong stack_size, GError ** error)
{
    GThreadWin32 *thread;
    guint ignore;
    ...
    if (thread->handle == NULL)
    {
        ...
        g_set_error(error, G_THREAD_ERROR, G_THREAD_ERROR_AGAIN, "Error creating thread: %s", win_error);
        g_free(win_error);
...
static GThreadXpWaiter *g_thread_xp_waiter_get(void)
{
...
static void __stdcall g_thread_xp_CallThisOnThreadExit(void)
{
    GThreadXpWaiter *waiter;
...
static void __stdcall g_thread_xp_InitializeSRWLock(gpointer mutex)
{
    *(GThreadSRWLock * volatile *) mutex = NULL;
}

static void __stdcall g_thread_xp_DeleteSRWLock(gpointer mutex)
{
...
static GThreadSRWLock *__stdcall g_thread_xp_get_srwlock(GThreadSRWLock * volatile *lock)
{
...
static void __stdcall g_thread_xp_AcquireSRWLockExclusive(gpointer mutex)
{
...
static BOOLEAN __stdcall g_thread_xp_TryAcquireSRWLockExclusive(gpointer mutex)
{
    GThreadSRWLock *lock = g_thread_xp_get_srwlock(mutex);
...
static void __stdcall g_thread_xp_ReleaseSRWLockExclusive(gpointer mutex)
{
    GThreadSRWLock *lock = *(GThreadSRWLock * volatile *) mutex;
...
static void g_thread_xp_srwlock_become_reader(GThreadSRWLock * lock)
{
    if G_UNLIKELY
        (!lock->ever_shared)
    {
...
static void __stdcall g_thread_xp_AcquireSRWLockShared(gpointer mutex)
{
    GThreadSRWLock *lock = g_thread_xp_get_srwlock(mutex);
...
static BOOLEAN __stdcall g_thread_xp_TryAcquireSRWLockShared(gpointer mutex)
{
    GThreadSRWLock *lock = g_thread_xp_get_srwlock(mutex);
...
static void __stdcall g_thread_xp_ReleaseSRWLockShared(gpointer mutex)
{
    GThreadSRWLock *lock = g_thread_xp_get_srwlock(mutex);
...
static void __stdcall g_thread_xp_InitializeConditionVariable(gpointer cond)
{
    *(GThreadXpCONDITION_VARIABLE * volatile *) cond = NULL;
}

static void __stdcall g_thread_xp_DeleteConditionVariable(gpointer cond)
{
...
static GThreadXpCONDITION_VARIABLE *__stdcall g_thread_xp_get_condition_variable(GThreadXpCONDITION_VARIABLE * volatile *cond)
{
...
static BOOL __stdcall g_thread_xp_SleepConditionVariableSRW(gpointer cond, gpointer mutex, DWORD timeout, ULONG flags)
{
...
static void __stdcall g_thread_xp_WakeConditionVariable(gpointer cond)
{
    GThreadXpCONDITION_VARIABLE *cv = g_thread_xp_get_condition_variable(cond);
...
static void __stdcall g_thread_xp_WakeAllConditionVariable(gpointer cond)
{
    GThreadXpCONDITION_VARIABLE *cv = g_thread_xp_get_condition_variable(cond);
    volatile GThreadXpWaiter *waiter;
...
/* {{{2 XP Setup */
static void g_thread_xp_init(void)
{
...
static gboolean g_thread_lookup_native_funcs(void)
{
