...
gchar *g_win32_getlocale(void)
{
     ...
     */
    if (((ev = getenv("LC_ALL")) != NULL && ev[0] != '\0') || ((ev = getenv("LC_MESSAGES")) != NULL && ev[0] != '\0') || ((ev = getenv("LANG")) != NULL && ev[0] != '\0'))
        return g_strdup(ev);
...
gchar *g_win32_get_package_installation_directory_of_module(gpointer hmodule)
{
    ...
    do
    {
        ...
        *p = '\0';
        ...
        if (g_ascii_strcasecmp(p + 1, "bin") == 0 || g_ascii_strcasecmp(p + 1, "lib") == 0)
            break;
    }
    while (p != NULL);
...
gchar *g_win32_get_package_installation_subdirectory_utf8(const gchar * package, const gchar * dll_name, const gchar * subdir)
{
    ...
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS prefix = g_win32_get_package_installation_directory_utf8(package, dll_name);
    G_GNUC_END_IGNORE_DEPRECATIONS dirname = g_build_filename(prefix, subdir, NULL);
    g_free(prefix);
...
gchar *g_win32_get_package_installation_subdirectory(const gchar * package, const gchar * dll_name, const gchar * subdir)
{
    ...
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS prefix = g_win32_get_package_installation_directory(package, dll_name);
    G_GNUC_END_IGNORE_DEPRECATIONS dirname = g_build_filename(prefix, subdir, NULL);
    g_free(prefix);
    ...
    if (is_ver_checked)
    {
        switch (os_type)
        {
            ...
            case G_WIN32_OS_SERVER:
                if (osverinfo.wProductType == VER_NT_SERVER || osverinfo.wProductType == VER_NT_DOMAIN_CONTROLLER)
                    is_type_checked = TRUE;
                break;
            default:
