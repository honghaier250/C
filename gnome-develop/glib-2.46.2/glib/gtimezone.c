...
again:
    ...
    if (ref_count == 1)
    {
        if (tz->name != NULL)
        {
            ...
            if G_UNLIKELY
                (g_atomic_int_get(&tz->ref_count) != 1)
            {
                G_UNLOCK(time_zones);
                goto again;
            }
        ...
        (!g_atomic_int_compare_and_exchange(&tz->ref_count, ref_count, ref_count - 1)) goto again;
}
...
#ifdef G_OS_UNIX
static GBytes *zone_info_unix(const gchar * identifier)
{
    gchar *filename;
    ...
    if (file != NULL)
    {
        zoneinfo = g_bytes_new_with_free_func(g_mapped_file_get_contents(file), g_mapped_file_get_length(file), (GDestroyNotify) g_mapped_file_unref, g_mapped_file_ref(file));
...
static void init_zone_from_iana_info(GTimeZone * gtz, GBytes * zoneinfo)
{
    ...
    g_return_if_fail(size >= sizeof(struct tzhead) && memcmp(header, "TZif", 4) == 0);
    ...
    if (header->tzh_version == '2')
    {
        ...
        header = (const struct tzhead *)
            (((const gchar *) (header + 1)) +
             ...
             guint32_from_be(header->tzh_ttisstdcnt) + 8 * guint32_from_be(header->tzh_leapcnt) + 5 * guint32_from_be(header->tzh_timecnt) + 6 * guint32_from_be(header->tzh_typecnt) + guint32_from_be(header->tzh_charcnt));
    ...
    gtz->transitions = g_array_sized_new(FALSE, TRUE, sizeof(Transition), time_count);
...
static void rule_from_windows_time_zone_info(TimeZoneRule * rule, TIME_ZONE_INFORMATION * tzi)
{
    /* Set offset */
...
{
    const gchar *subkey = "SYSTEM\\CurrentControlSet\\Control\\TimeZoneInformation";
    HKEY key;
    gchar *key_name = NULL;
    if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, subkey, 0, KEY_QUERY_VALUE, &key) == ERROR_SUCCESS)
    {
        DWORD size = 0;
        if (RegQueryValueExA(key, "TimeZoneKeyName", NULL, NULL, NULL, &size) == ERROR_SUCCESS)
        {
            key_name = g_malloc((gint) size);
            if (RegQueryValueExA(key, "TimeZoneKeyName", NULL, NULL, (LPBYTE) key_name, &size) != ERROR_SUCCESS)
            {
                g_free(key_name);
                key_name = NULL;
            }
...
static gint rules_from_windows_time_zone(const gchar * identifier, TimeZoneRule ** rules)
{
    HKEY key;
    gchar *subkey, *subkey_dynamic;
    gchar *key_name = NULL;
    const gchar *reg_key = "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Time Zones\\";
    ...
    *rules = NULL;
    ...
    if (RegQueryValueExA(key, "Std", NULL, NULL, (LPBYTE) & (tzi.StandardName), &size) != ERROR_SUCCESS)
        goto failed;

    size = sizeof tzi.DaylightName;

    if (RegQueryValueExA(key, "Dlt", NULL, NULL, (LPBYTE) & (tzi.DaylightName), &size) != ERROR_SUCCESS)
        goto failed;

    RegCloseKey(key);
    if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, subkey_dynamic, 0, KEY_QUERY_VALUE, &key) == ERROR_SUCCESS)
    {
        DWORD first, last;
        int year, i;
        gchar *s;
        ...
        size = sizeof first;
        if (RegQueryValueExA(key, "FirstEntry", NULL, NULL, (LPBYTE) & first, &size) != ERROR_SUCCESS)
            goto failed;

        size = sizeof last;
        if (RegQueryValueExA(key, "LastEntry", NULL, NULL, (LPBYTE) & last, &size) != ERROR_SUCCESS)
            goto failed;
        ...
        *rules = g_new0(TimeZoneRule, rules_num);
        ...
        for (year = first, i = 0; year <= last; year++)
        {
            ...
            size = sizeof regtzi;
            if (RegQueryValueExA(key, s, NULL, NULL, (LPBYTE) & regtzi, &size) != ERROR_SUCCESS)
            {
                g_free(*rules);
                *rules = NULL;
                break;
...
failed:
    ...
    else if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, subkey, 0, KEY_QUERY_VALUE, &key) == ERROR_SUCCESS)
    {
        size = sizeof regtzi;
        if (RegQueryValueExA(key, "TZI", NULL, NULL, (LPBYTE) & regtzi, &size) == ERROR_SUCCESS)
        {
            rules_num = 2;
...
static gint64 boundary_for_year(TimeZoneDate * boundary, gint year, gint32 offset)
{
    TimeZoneDate buffer;
    GDate date;
    ...
    return ((g_date_get_julian(&date) - unix_epoch_start) * seconds_per_day + buffer.hour * 3600 + buffer.min * 60 + buffer.sec - offset);
}

static void fill_transition_info_from_rule(TransitionInfo * info, TimeZoneRule * rule, gboolean is_dst)
{
    ...
    if (name)
        ...
        info->abbrev = g_strdup_printf("%+03d%02d", (int) offset / 3600, (int) abs(offset / 60) % 60);
}

static void init_zone_from_rules(GTimeZone * gtz, TimeZoneRule * rules, gint rules_num)
{
    ...
    for (ri = 0; ri < rules_num - 1; ri++)
    {
        ...
        else if (rules[ri].std_offset || rules[ri].dlt_offset)
        {
            ...
            {
                gint32 dlt_offset = (dlt_first ? last_offset : rules[ri].dlt_offset);
                gint32 std_offset = (dlt_first ? rules[ri].std_offset : last_offset);
...
static gboolean parse_julian_boundary(gchar ** pos, TimeZoneDate * boundary, gboolean ignore_leap)
{
    gint day = 0;
    GDate date;
...
static gboolean parse_tz_boundary(const gchar * identifier, TimeZoneDate * boundary)
{
    gchar *pos;
 ...
 */
static gint rules_from_identifier(const gchar * identifier, TimeZoneRule ** rules)
{
    gchar *pos;
    TimeZoneRule tzr;
        ...
        /* Windows allows us to use the US DST boundaries if they're not given */
    {
        ...
        if ((rules_num = rules_from_windows_time_zone("Pacific Standard Time", rules)))
        {
