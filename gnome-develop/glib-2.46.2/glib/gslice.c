...
static inline ThreadMemory *thread_memory_from_self(void)
{
    ...
    if (G_UNLIKELY(!tmem))
    {
        ...
        if G_UNLIKELY
            (sys_page_size == 0) g_slice_init_nomessage();
...
static void magazine_cache_push_magazine(guint ix, ChunkLink * magazine_chunks, gsize count)    /* must be >= MIN_MAGAZINE_SIZE */
{
...
void g_slice_free1(gsize mem_size, gpointer mem_block)
{
    ...
    if (G_UNLIKELY(!mem_block))
        return;
    if (G_UNLIKELY(allocator->config.debug_blocks) && !smc_notify_free(mem_block, mem_size))
        abort();
    if (G_LIKELY(acat == 1))    /* allocate through magazine layer */
    {
...
void g_slice_free_chain_with_offset(gsize mem_size, gpointer mem_chain, gsize next_offset)
{
    ...
    if (G_LIKELY(acat == 1))    /* allocate through magazine layer */
    {
        ...
        while (slice)
        {
            ...
            if (G_UNLIKELY(allocator->config.debug_blocks) && !smc_notify_free(current, mem_size))
                abort();
            if (G_UNLIKELY(thread_memory_magazine2_is_full(tmem, ix)))
            {
                thread_memory_swap_magazines(tmem, ix);
