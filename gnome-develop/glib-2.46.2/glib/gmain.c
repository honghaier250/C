...
static void g_child_source_remove_internal(GSource * child_source, GMainContext * context);
...
static void g_source_iter_init(GSourceIter * iter, GMainContext * context, gboolean may_modify);
...
static gboolean g_unix_signal_watch_dispatch(GSource * source, GSourceFunc callback, gpointer user_data);
...
/* Holds context's lock
 */
static GSourceList *find_source_list_for_priority(GMainContext * context, gint priority, gboolean create)
{
    GList *iter, *last;
    ...
    for (iter = context->source_lists; iter != NULL; last = iter, iter = iter->next)
    {
        ...
        if (source_list->priority > priority)
        {
            ...
            context->source_lists = g_list_insert_before(context->source_lists, iter, source_list);
            return source_list;
        }
    }
...
static guint g_source_attach_unlocked(GSource * source, GMainContext * context, gboolean do_wakeup)
{
    GSList *tmp_list;
    guint id;
...
static void g_child_source_remove_internal(GSource * child_source, GMainContext * context)
{
    ...
    parent_source->priv->child_sources = g_slist_remove(parent_source->priv->child_sources, child_source);
...
static void g_source_set_priority_unlocked(GSource * source, GMainContext * context, gint priority)
{
    ...
    g_return_if_fail(source->priv->parent_source == NULL || source->priv->parent_source->priority == priority);
...
gint64 g_get_monotonic_time(void)
{
    ...
    if (timebase_info.denom == 0)
    {
        ...
        else
        {
            ...
            g_error("Got weird mach timebase info of %d/%d.  Please file a bug against GLib.", timebase_info.numer, timebase_info.denom);
        }
    }
...
gint64 g_get_monotonic_time(void)
{
    ...
    if G_UNLIKELY
        (result != 0) g_error("GLib requires working CLOCK_MONOTONIC");
...
gboolean g_main_context_wait(GMainContext * context, GCond * cond, GMutex * mutex)
{
    ...
    if G_UNLIKELY
        (cond != &context->cond || mutex != &context->mutex)
    {
        static gboolean warned;

        if (!warned)
        {
            g_critical("WARNING!! g_main_context_wait() will be removed in a future release.  " "If you see this message, please file a bug immediately.");
            warned = TRUE;
        }
    }
...
static gboolean g_main_context_iterate(GMainContext * context, gboolean block, gboolean dispatch, GThread * self)
{
    ...
    if (!g_main_context_acquire(context))
    {
        ...
        got_ownership = g_main_context_wait(context, &context->cond, &context->mutex);
...
void g_main_loop_run(GMainLoop * loop)
{
    ...
    if (!g_main_context_acquire(loop->context))
    {
        ...
        while (loop->is_running && !got_ownership)
            got_ownership = g_main_context_wait(loop->context, &loop->context->cond, &loop->context->mutex);
