...
GArray *g_array_prepend_vals(GArray * farray, gconstpointer data, guint len)
{
    ...
    memmove(g_array_elt_pos(array, len), g_array_elt_pos(array, 0), g_array_elt_len(array, array->len));
...
GArray *g_array_insert_vals(GArray * farray, guint index_, gconstpointer data, guint len)
{
    ...
    memmove(g_array_elt_pos(array, len + index_), g_array_elt_pos(array, index_), g_array_elt_len(array, array->len - index_));
...
GArray *g_array_remove_index(GArray * farray, guint index_)
{
    ...
    if (index_ != array->len - 1)
        memmove(g_array_elt_pos(array, index_), g_array_elt_pos(array, index_ + 1), g_array_elt_len(array, array->len - index_ - 1));
...
GArray *g_array_remove_range(GArray * farray, guint index_, guint length)
{
    ...
    if (index_ + length != array->len)
        memmove(g_array_elt_pos(array, index_), g_array_elt_pos(array, index_ + length), (array->len - (index_ + length)) * array->elt_size);
...
void g_array_sort(GArray * farray, GCompareFunc compare_func)
{
    ...
    g_qsort_with_data(array->data, array->len, array->elt_size, (GCompareDataFunc) compare_func, NULL);
}
...
gpointer g_ptr_array_remove_index(GPtrArray * array, guint index_)
{
    ...
    if (index_ != rarray->len - 1)
        memmove(rarray->pdata + index_, rarray->pdata + index_ + 1, sizeof(gpointer) * (rarray->len - index_ - 1));
...
GPtrArray *g_ptr_array_remove_range(GPtrArray * array, guint index_, guint length)
{
    ...
    {
        memmove(&rarray->pdata[index_], &rarray->pdata[index_ + length], (rarray->len - (index_ + length)) * sizeof(gpointer));
    }
...
void g_ptr_array_insert(GPtrArray * array, gint index_, gpointer data)
{
    ...
    if (index_ < rarray->len)
        memmove(&(rarray->pdata[index_ + 1]), &(rarray->pdata[index_]), (rarray->len - index_) * sizeof(gpointer));
...
void g_ptr_array_sort(GPtrArray * array, GCompareFunc compare_func)
{
    ...
    /* Don't use qsort as we want a guaranteed stable sort */
    g_qsort_with_data(array->pdata, array->len, sizeof(gpointer), (GCompareDataFunc) compare_func, NULL);
}
