...
static void g_win32_print_access_mode(int flags)
{
    g_print("%s%s%s%s%s%s%s%s%s%s",
            ...
            (flags & _O_TEMPORARY ? "|O_TEMPORARY" : ""), (flags & _O_CREAT ? "|O_CREAT" : ""), (flags & _O_TRUNC ? "|O_TRUNC" : ""), (flags & _O_EXCL ? "|O_EXCL" : ""), (flags & _O_TEXT ? "|O_TEXT" : ""), (flags & _O_BINARY ? "|O_BINARY" : ""));
}
...
static void create_thread(GIOWin32Channel * channel, GIOCondition condition, unsigned (__stdcall * thread) (void *parameter))
{
    ...
    thread_handle = (HANDLE) _beginthreadex(NULL, 0, thread, channel, 0, &channel->thread_id);
    if (thread_handle == 0)
        ...
        case G_IO_WIN32_FILE_DESC:
            if (channel->debug)
                g_print(" FD thread=%#x buffer_condition:{%s}"
                        ...
                        channel->thread_id, condition_to_string(buffer_condition), condition_to_string(watch->pollfd.events), condition_to_string(watch->pollfd.revents), condition_to_string(channel->revents));
...
static gboolean g_io_win32_check(GSource * source)
{
    ...
    switch (channel->type)
    {
        ...
        case G_IO_WIN32_SOCKET:
            ...
            else
            {
                ...
                if (channel->debug)
                    g_print("\n  revents={%s} condition={%s}"
                            "\n  WSAEnumNetworkEvents(%d,0) sets events={%s}", condition_to_string(watch->pollfd.revents), condition_to_string(watch->condition), channel->fd, event_mask_to_string(events.lNetworkEvents));
                ...
                if (watch->pollfd.revents != 0 && events.lNetworkEvents == 0 && !(channel->event_mask & FD_WRITE))
                {
                    ...
                    if (channel->debug)
                        g_print("  ResetEvent(%p)", (HANDLE) watch->pollfd.fd);
             ...
             */
            if (!(watch->pollfd.revents & G_IO_HUP) && channel->ever_writable && !channel->write_would_have_blocked && (channel->event_mask & FD_WRITE))
                watch->pollfd.revents |= G_IO_OUT;

            if (channel->debug)
                g_print("\n  revents={%s} retval={%s}\n", condition_to_string(watch->pollfd.revents), condition_to_string((watch->pollfd.revents | buffer_condition) & watch->condition));
...
static gboolean g_io_win32_dispatch(GSource * source, GSourceFunc callback, gpointer user_data)
{
    ...
    if (channel->debug)
        g_print("g_io_win32_dispatch: pollfd.revents=%s condition=%s result=%s\n", condition_to_string(watch->pollfd.revents), condition_to_string(watch->condition), condition_to_string((watch->pollfd.revents | buffer_condition) & watch->condition));
...
static GIOStatus g_io_win32_msg_read(GIOChannel * channel, gchar * buf, gsize count, gsize * bytes_read, GError ** err)
{
    ...
    *bytes_read = 0;
    ...
    if (count < sizeof(MSG))
    {
        g_set_error_literal(err, G_IO_CHANNEL_ERROR, G_IO_CHANNEL_ERROR_INVAL, "Incorrect message size");    /* Informative enough error message? */
...
static GSource *g_io_win32_fd_create_watch(GIOChannel * channel, GIOCondition condition)
{
    ...
    if (win32_channel->debug)
        g_print("g_io_win32_fd_create_watch: channel=%p fd=%d condition={%s} event=%p\n", channel, win32_channel->fd, condition_to_string(condition), (HANDLE) watch->pollfd.fd);
...
static GIOStatus g_io_win32_sock_read(GIOChannel * channel, gchar * buf, gsize count, gsize * bytes_read, GError ** err)
{
    ...
    if (win32_channel->debug)
        g_print("g_io_win32_sock_read: channel=%p sock=%d count=%" G_GSIZE_FORMAT, channel, win32_channel->fd, count);
...
static GIOStatus g_io_win32_sock_write(GIOChannel * channel, const gchar * buf, gsize count, gsize * bytes_written, GError ** err)
{
    ...
    if (win32_channel->debug)
        g_print("g_io_win32_sock_write: channel=%p sock=%d count=%" G_GSIZE_FORMAT, channel, win32_channel->fd, count);
...
static GSource *g_io_win32_sock_create_watch(GIOChannel * channel, GIOCondition condition)
{
    ...
    if (win32_channel->debug)
        g_print("g_io_win32_sock_create_watch: channel=%p sock=%d event=%p condition={%s}\n", channel, win32_channel->fd, (HANDLE) watch->pollfd.fd, condition_to_string(watch->condition));
...
static GIOFlags g_io_win32_fd_get_flags_internal(GIOChannel * channel, struct _stati64 *st)
{
    ...
    {
        channel->is_readable = (PeekNamedPipe((HANDLE) _get_osfhandle(win32_channel->fd), &c, 0, &count, NULL, NULL) != 0) || GetLastError() == ERROR_BROKEN_PIPE;
