...
static void print_entry(GOptionGroup * group, gint max_length, const GOptionEntry * entry, GString * string, GHashTable * aliases)
{
    GString *str;
    const gchar *long_name;
    ...
    g_string_append_printf(string, "%s%*s %s\n", str->str, (int) (max_length + 4 - _g_utf8_strwidth(str->str)), "", entry->description ? TRANSLATE(group, entry->description) : "");
...
static gboolean group_list_has_visible_entries(GOptionContext * context, GList * group_list, gboolean main_entries)
{
    while (group_list)
    {
...
gchar *g_option_context_get_help(GOptionContext * context, gboolean main_help, GOptionGroup * group)
{
    ...
    while (list != NULL)
    {
        ...
        for (i = 0; i < g->n_entries; i++)
        {
            ...
            {
                g_hash_table_insert(aliases, &entry->long_name, g_strdup_printf("%s-%s", g->name, entry->long_name));
            }
            else
                g_hash_table_insert(shadow_map, (gpointer) entry->long_name, entry);

            if (seen[(guchar) entry->short_name] && !(entry->flags & G_OPTION_FLAG_NOALIAS))
                entry->short_name = 0;
            else
                seen[(guchar) entry->short_name] = TRUE;
        }
...
static gboolean parse_double(const gchar * arg_name, const gchar * arg, gdouble * result, GError ** error)
{
    gchar *end;
    gdouble tmp;
        ...
        case G_OPTION_ARG_STRING_ARRAY:
            {
                ...
                if (change->allocated.array.len == 0)
                {
                    ...
                    change->allocated.array.data = g_renew(gchar *, change->allocated.array.data, change->allocated.array.len + 2);
        ...
        case G_OPTION_ARG_CALLBACK:
            {
                ...
                if (!(entry->flags & (G_OPTION_FLAG_NO_ARG | G_OPTION_FLAG_OPTIONAL_ARG)) && !data)
                    return FALSE;
...
static gboolean parse_short_option(GOptionContext * context, GOptionGroup * group, gint idx, gint * new_idx, gchar arg, gint * argc, gchar *** argv, GError ** error, gboolean * parsed)
{
    gint j;
...
static gboolean parse_long_option(GOptionContext * context, GOptionGroup * group, gint * idx, gchar * arg, gboolean aliased, gint * argc, gchar *** argv, GError ** error, gboolean * parsed)
{
    gint j;
...
static gboolean parse_remaining_arg(GOptionContext * context, GOptionGroup * group, gint * idx, gint * argc, gchar *** argv, GError ** error, gboolean * parsed)
{
    ...
    for (j = 0; j < group->n_entries; j++)
    {
        ...
        g_return_val_if_fail(group->entries[j].arg == G_OPTION_ARG_CALLBACK || group->entries[j].arg == G_OPTION_ARG_STRING_ARRAY || group->entries[j].arg == G_OPTION_ARG_FILENAME_ARRAY, FALSE);
...
void g_option_group_add_entries(GOptionGroup * group, const GOptionEntry * entries)
{
    ...
    for (i = group->n_entries; i < group->n_entries + n_entries; i++)
    {
        ...
        {
            g_warning(G_STRLOC ": ignoring invalid short option '%c' (%d) in entry %s:%s", c, c, group->name, group->entries[i].long_name);
            group->entries[i].short_name = '\0';
        }

        if (group->entries[i].arg != G_OPTION_ARG_NONE && (group->entries[i].flags & G_OPTION_FLAG_REVERSE) != 0)
        {
            g_warning(G_STRLOC ": ignoring reverse flag on option of arg-type %d in entry %s:%s", group->entries[i].arg, group->name, group->entries[i].long_name);

            group->entries[i].flags &= ~G_OPTION_FLAG_REVERSE;
        }

        if (group->entries[i].arg != G_OPTION_ARG_CALLBACK && (group->entries[i].flags & (G_OPTION_FLAG_NO_ARG | G_OPTION_FLAG_OPTIONAL_ARG | G_OPTION_FLAG_FILENAME)) != 0)
        {
            g_warning(G_STRLOC ": ignoring no-arg, optional-arg or filename flags (%d) on option of arg-type %d in entry %s:%s", group->entries[i].flags, group->entries[i].arg, group->name, group->entries[i].long_name);
