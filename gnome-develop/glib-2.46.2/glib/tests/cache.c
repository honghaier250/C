...
static gpointer value_create(gpointer key)
{
    gint *value;
...
static gpointer key_dup(gpointer key)
{
    gint *newkey;
...
static guint key_hash(gconstpointer key)
{
    return *(guint *) key;
}
...
static gboolean key_equal(gconstpointer key1, gconstpointer key2)
{
    return *(gint *) key1 == *(gint *) key2;
}

static void key_foreach(gpointer valuep, gpointer keyp, gpointer data)
{
    gint *count = data;
    gint *key = keyp;
...
static void value_foreach(gpointer keyp, gpointer nodep, gpointer data)
{
    gint *count = data;
    gint *key = keyp;
...
static void test_cache_basic(void)
{
    ...
    c = g_cache_new(value_create, value_destroy, key_dup, key_destroy, key_hash, value_hash, key_equal);
