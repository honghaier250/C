...
static void test_g_array(void)
{
    g_autoptr(GArray) val = g_array_new(0, 0, sizeof(gpointer));
    g_assert(val != NULL);
}

static void test_g_ptr_array(void)
{
    g_autoptr(GPtrArray) val = g_ptr_array_new();
    g_assert(val != NULL);
}

static void test_g_byte_array(void)
{
    g_autoptr(GByteArray) val = g_byte_array_new();
    g_assert(val != NULL);
}

static void test_g_main_context(void)
{
    g_autoptr(GMainContext) val = g_main_context_new();
    g_assert(val != NULL);
}

static void test_g_main_loop(void)
{
    g_autoptr(GMainLoop) val = g_main_loop_new(NULL, TRUE);
    g_assert(val != NULL);
}

static void test_g_source(void)
{
    g_autoptr(GSource) val = g_timeout_source_new_seconds(2);
    g_assert(val != NULL);
}

static void test_g_mapped_file(void)
{
    g_autoptr(GMappedFile) val = g_mapped_file_new(g_test_get_filename(G_TEST_DIST, "keyfiletest.ini", NULL), FALSE, NULL);
    g_assert(val != NULL);
}

static void parser_start(GMarkupParseContext * context, const gchar * element_name, const gchar ** attribute_names, const gchar ** attribute_values, gpointer user_data, GError ** error)
{
}

static void parser_end(GMarkupParseContext * context, const gchar * element_name, gpointer user_data, GError ** error)
{
}
...
static void test_g_rand(void)
{
    g_autoptr(GRand) val = g_rand_new();
    g_assert(val != NULL);
}

static void test_g_regex(void)
{
    g_autoptr(GRegex) val = g_regex_new(".*", 0, 0, NULL);
    g_assert(val != NULL);
}

static void test_g_match_info(void)
{
...
static void test_g_scanner(void)
{
...
static void test_g_sequence(void)
{
    g_autoptr(GSequence) val = g_sequence_new(NULL);
    g_assert(val != NULL);
}

static void test_g_slist(void)
{
...
static void test_g_string(void)
{
    g_autoptr(GString) val = g_string_new("");
    g_assert(val != NULL);
}

static void test_g_string_chunk(void)
{
    g_autoptr(GStringChunk) val = g_string_chunk_new(42);
    g_assert(val != NULL);
}

static gpointer mythread(gpointer data)
{
    g_usleep(G_USEC_PER_SEC);
    return NULL;
}

static void test_g_thread(void)
{
    g_autoptr(GThread) val = g_thread_new("bob", mythread, NULL);
    g_assert(val != NULL);
}

static void test_g_mutex(void)
{
    g_auto(GMutex) val;
...
static void test_g_mutex_locker(void)
{
    GMutex mutex;
...
static void test_g_cond(void)
{
    g_auto(GCond) val;
    g_cond_init(&val);
}

static void test_g_timer(void)
{
    g_autoptr(GTimer) val = g_timer_new();
    g_assert(val != NULL);
}

static void test_g_time_zone(void)
{
    g_autoptr(GTimeZone) val = g_time_zone_new("UTC");
    g_assert(val != NULL);
}

static void test_g_tree(void)
{
    g_autoptr(GTree) val = g_tree_new((GCompareFunc) strcmp);
    g_assert(val != NULL);
}

static void test_g_variant(void)
{
    g_autoptr(GVariant) val = g_variant_new_string("hello");
    g_assert(val != NULL);
}

static void test_g_variant_builder(void)
{
...
static void test_g_variant_iter(void)
{
...
static void test_g_variant_dict(void)
{
...
static void test_g_variant_type(void)
{
