...
static void subparser_start_element(GMarkupParseContext * context, const gchar * element_name, const gchar ** attribute_names, const gchar ** attribute_values, gpointer user_data, GError ** error)
{
    g_string_append_printf(user_data, "{%s}", element_name);

    /* we don't like trouble... */
    if (strcmp(element_name, "trouble") == 0)
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, "we don't like trouble");
}
...
static char *subparser_end(GMarkupParseContext * ctx, GError ** error)
{
    ...
    {
        g_free(result);
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, "got no data");
...
static void test(gconstpointer user_data)
{
