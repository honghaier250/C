...
static gchar *invalid_mutation(const gchar * type_string)
{
    ...
    if (have_parens && have_braces && randomly(0.3))
    {
        ...
        while ((pp = strchr(pp + 1, p)))
            np++;
        while ((bp = strchr(bp + 1, b)))
            nb++;
...
static void calculate_type_info(const GVariantType * type, gsize * fixed_size, guint * alignment)
{
    ...
    else
    {
        ...
        if (g_variant_type_equal(type, G_VARIANT_TYPE_BOOLEAN) || g_variant_type_equal(type, G_VARIANT_TYPE_BYTE))
        {
            al = fs = 1;
        }

        else if (g_variant_type_equal(type, G_VARIANT_TYPE_INT16) || g_variant_type_equal(type, G_VARIANT_TYPE_UINT16))
        {
            al = fs = 2;
        }

        else if (g_variant_type_equal(type, G_VARIANT_TYPE_INT32) || g_variant_type_equal(type, G_VARIANT_TYPE_UINT32) || g_variant_type_equal(type, G_VARIANT_TYPE_HANDLE))
        {
            al = fs = 4;
        }

        else if (g_variant_type_equal(type, G_VARIANT_TYPE_INT64) || g_variant_type_equal(type, G_VARIANT_TYPE_UINT64) || g_variant_type_equal(type, G_VARIANT_TYPE_DOUBLE))
        {
            al = fs = 8;
        }
        else if (g_variant_type_equal(type, G_VARIANT_TYPE_STRING) || g_variant_type_equal(type, G_VARIANT_TYPE_OBJECT_PATH) || g_variant_type_equal(type, G_VARIANT_TYPE_SIGNATURE))
        {
            al = 1;
            fs = 0;
        }
        ...
        /* it is a leaf */
    {
        ...
        if (serialised->data)
        {
            switch (instance->data_size)
            {
                ...
                default:
                    memcpy(serialised->data, instance->data.string, instance->data_size);
                    break;
            }
        }
    }
...
static gboolean check_tree(TreeInstance * instance, GVariantSerialised serialised)
{
        ...
        /* is a container */
    {
        ...
        if (g_variant_serialised_n_children(serialised) != instance->n_children)
            return FALSE;
...
static gboolean tree_instance_check_gvariant(TreeInstance * tree, GVariant * value)
{
    ...
    switch (g_variant_type_info_get_type_char(tree->info))
    {
        ...
        case 'o':
        case 'g':
            return strcmp(g_variant_get_string(value, NULL), tree->data.string) == 0;
...
static void do_failed_test(const char *test, const gchar * pattern)
{
...
static void test_invalid_varargs(void)
{
    ...
    g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, "*GVariant format string*");
    g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, "*valid_format_string*");
    value = g_variant_new("z");
    g_test_assert_expected_messages();
    g_assert(value == NULL);

    g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, "*valid GVariant format string as a prefix*");
    g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, "*valid_format_string*");
    ...
    g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, "*type of 'q' but * has a type of 'y'*");
    g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, "*valid_format_string*");
...
static void test_varargs_empty_array(void)
{
...
{
    {
        ...
        g_variant_builder_add_value(&array, g_variant_new_dict_entry(g_variant_new_string("temperature"), g_variant_new_variant(g_variant_new_double(37.5))));
        check_and_free(g_variant_new("(ma{sv}m(a{sv})ma{sv}ii)", NULL, FALSE, NULL, &array, 7777, 8888), "(nothing, nothing, {'size': <(800, 600)>, " "'title': <'Test case'>, " "'temperature': <37.5>}, " "7777, 8888)");

        check_and_free(g_variant_new("(imimimmimmimmi)", 123, FALSE, 321, TRUE, 123, FALSE, TRUE, 321, TRUE, FALSE, 321, TRUE, TRUE, 123), "(123, nothing, 123, nothing, just nothing, 123)");

        check_and_free(g_variant_new("(ybnixd)", 'a', 1, 22, 33, (guint64) 44, 5.5), "(0x61, true, 22, 33, 44, 5.5)");

        check_and_free(g_variant_new("(@y?*rv)", g_variant_new("y", 'a'), g_variant_new("y", 'b'), g_variant_new("y", 'c'), g_variant_new("(y)", 'd'), g_variant_new("y", 'e')), "(0x61, 0x62, 0x63, (0x64,), <byte 0x65>)");
    }
...

    {
        ...
        g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, "*NULL has already been returned*");
...

    {
        ...
        value = g_variant_new("(mymbmnmqmimumxmtmhmdmv)",
                              FALSE, 'a', FALSE, TRUE, FALSE, (gint16) 123, FALSE, (guint16) 123, FALSE, (gint32) 123, FALSE, (guint32) 123, FALSE, (gint64) 123, FALSE, (guint64) 123, FALSE, (gint32) - 1, FALSE, (gdouble) 37.5, NULL);

        /* both NULL */
        g_variant_get(value, "(mymbmnmqmimumxmtmhmdmv)", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);

        /* NULL values */
        memset(justs, 1, sizeof justs);
        g_variant_get(value, "(mymbmnmqmimumxmtmhmdmv)", &justs[0], NULL, &justs[1], NULL, &justs[2], NULL, &justs[3], NULL, &justs[4], NULL, &justs[5], NULL, &justs[6], NULL, &justs[7], NULL, &justs[8], NULL, &justs[9], NULL, NULL);
        ...
        g_variant_get(value, "(mymbmnmqmimumxmtmhmdmv)",
                      &justs[0], &byteval, &justs[1], &bval, &justs[2], &i16val, &justs[3], &u16val, &justs[4], &i32val, &justs[5], &u32val, &justs[6], &i64val, &justs[7], &u64val, &justs[8], &hval, &justs[9], &dval, &vval);
        g_assert(!(justs[0] || justs[1] || justs[2] || justs[3] || justs[4] || justs[5] || justs[6] || justs[7] || justs[8] || justs[9]));
        ...
        bval = TRUE;
        dval = 88.88;
        g_variant_get(value, "(mymbmnmqmimumxmtmhmdmv)", NULL, &byteval, NULL, &bval, NULL, &i16val, NULL, &u16val, NULL, &i32val, NULL, &u32val, NULL, &i64val, NULL, &u64val, NULL, &hval, NULL, &dval, &vval);
        g_assert(byteval == '\0' && bval == FALSE);
        g_assert(i16val == 0 && u16val == 0 && i32val == 0 && u32val == 0 && i64val == 0 && u64val == 0 && hval == 0 && dval == 0.0);
        g_assert(vval == NULL);
        ...
        /* test all 'just' */
        value = g_variant_new("(mymbmnmqmimumxmtmhmdmv)",
                              TRUE, 'a', TRUE, TRUE, TRUE, (gint16) 123, TRUE, (guint16) 123, TRUE, (gint32) 123, TRUE, (guint32) 123, TRUE, (gint64) 123, TRUE, (guint64) 123, TRUE, (gint32) - 1, TRUE, (gdouble) 37.5, g_variant_new("()"));
...

    {
        ...
        do_failed_test("/gvariant/varargs/subprocess/empty-array", "*which type of empty array*");
    }
...
static void test_parse_failures(void)
{
    ...
    for (i = 0; i < G_N_ELEMENTS(test); i += 3)
    {
        ...
        if (!strstr(error->message, test[i + 2]))
            g_error("test %d: Can't find '%s' in '%s'", i / 3, test[i + 2], error->message);

        if (!g_str_has_prefix(error->message, test[i + 1]))
            g_error("test %d: Expected location '%s' in '%s'", i / 3, test[i + 1], error->message);

        g_error_free(error);
    }
}

static void test_parse_bad_format_char(void)
{
...
static void test_parse_bad_format_string(void)
{
...
static void test_parse_positional(void)
{
    ...
    if (g_test_undefined())
    {
        do_failed_test("/gvariant/parse/subprocess/bad-format-char", "*GVariant format string*");

        do_failed_test("/gvariant/parse/subprocess/bad-format-string", "*can not parse as*");

        do_failed_test("/gvariant/parse/subprocess/bad-args", "*expected GVariant of type 'i'*");
    }
}
...
static GVariant *untrusted(GVariant * a)
{
    GVariant *b;
    const GVariantType *type;
    GBytes *bytes;
...
static void test_fixed_array(void)
{
    ...
    for (i = 0; i < 5; i++)
        values[i] = i + 1;
    a = g_variant_new_fixed_array(G_VARIANT_TYPE_INT32, values, G_N_ELEMENTS(values), sizeof(values[0]));
...
static void test_check_format_string(void)
{
    GVariant *value;
...
static void verify_gvariant_checksum(const gchar * sha256, GVariant * v)
{
    gchar *checksum;
    checksum = g_compute_checksum_for_data(G_CHECKSUM_SHA256, g_variant_get_data(v), g_variant_get_size(v));
    g_assert_cmpstr(sha256, ==, checksum);
    g_free(checksum);
}

static void verify_gvariant_checksum_va(const gchar * sha256, const gchar * fmt, ...)
{
    va_list args;
    GVariant *v;
...
{
    verify_gvariant_checksum_va("e8a4b2ee7ede79a3afb332b5b6cc3d952a65fd8cffb897f5d18016577c33d7cc", "u", 42);
    verify_gvariant_checksum_va("c53e363c33b00cfce298229ee83856b8a98c2e6126cab13f65899f62473b0df5", "s", "moocow");
    verify_gvariant_checksum_va("2b4c342f5433ebe591a1da77e013d1b72475562d48578dca8b84bac6651c3cb9", "y", 9);
    verify_gvariant_checksum_va("12a3ae445661ce5dee78d0650d33362dec29c4f82af05e7e57fb595bbbacf0ca", "t", G_MAXUINT64);
    verify_gvariant_checksum_va("e25a59b24440eb6c833aa79c93b9840e6eab6966add0dacf31df7e9e7000f5b3", "d", 3.14159);
    verify_gvariant_checksum_va("4bf5122f344554c53bde2ebb8cd2b7e3d1600ad631c385a5d7cce23c7785459a", "b", TRUE);
    verify_gvariant_checksum_va("ca2fd00fa001190744c15c317643ab092e7048ce086a243e2be9437c898de1bb", "q", G_MAXUINT16);
}

static void test_checksum_nested(void)
{
    static const char *const strv[] = { "foo", "bar", "baz", NULL };

    verify_gvariant_checksum_va("31fbc92f08fddaca716188fe4b5d44ae122fc6306fd3c6925af53cfa47ea596d", "(uu)", 41, 43);
    verify_gvariant_checksum_va("01759d683cead856d1d386d59af0578841698a424a265345ad5413122f220de8", "(su)", "moocow", 79);
    verify_gvariant_checksum_va("52b3ae95f19b3e642ea1d01185aea14a09004c1d1712672644427403a8a0afe6", "(qyst)", G_MAXUINT16, 9, "moocow", G_MAXUINT64);
    verify_gvariant_checksum_va("6fc6f4524161c3ae0d316812d7088e3fcd372023edaea2d7821093be40ae1060", "(@ay)", g_variant_new_bytestring("\xFF\xFF\xFF"));
    verify_gvariant_checksum_va("572aca386e1a983dd23bb6eb6e3dfa72eef9ca7c7744581aa800e18d7d9d0b0b", "(^as)", strv);
    verify_gvariant_checksum_va("4bddf6174c791bb44fc6a4106573031690064df34b741033a0122ed8dc05bcf3", "(yvu)", 254, g_variant_new("(^as)", strv), 42);
}

static void test_gbytes(void)
{
    GVariant *a;
    GVariant *tuple;
    GBytes *bytes;
...
static void test_print_context(void)
{
    ContextTest tests[] = {
        {NULL, "(1, 2, 3, 'abc", "          ^^^^"},
        {NULL, "[1, 2, 3, 'str']", " ^        ^^^^^"},
...
{
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS g_assert(g_variant_parser_get_error_quark() == g_variant_parse_error_quark());
