...
static int test_in_chunks(const gchar * contents, gint length, gint chunk_size, GMarkupParseFlags flags)
{
...
static int test_file(const gchar * filename, GMarkupParseFlags flags)
{
    gchar *contents;
    gsize length;
    GError *error;
...
static gchar *get_expected_filename(const gchar * filename, GMarkupParseFlags flags)
{
    gchar *f, *p, *expected;
...
int main(int argc, char *argv[])
{
    ...
    while ((name = g_dir_read_name(dir)) != NULL)
    {
        ...
        g_test_add_data_func_full(path, g_test_build_filename(G_TEST_DIST, "markups", name, NULL), test_parse, g_free);
        g_free(path);
    }
    g_dir_close(dir);
