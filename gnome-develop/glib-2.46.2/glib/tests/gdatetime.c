...
static void test_GDateTime_invalid(void)
{
    GDateTime *dt;
...
static void test_GDateTime_new_from_timeval(void)
{
    ...
    if (g_test_verbose())
        g_printerr("\nDT%04d-%02d-%02dT%02d:%02d:%02d%s\n",
                   g_date_time_get_year(dt), g_date_time_get_month(dt), g_date_time_get_day_of_month(dt), g_date_time_get_hour(dt), g_date_time_get_minute(dt), g_date_time_get_second(dt), g_date_time_get_timezone_abbreviation(dt));
...
static void test_GDateTime_new_from_timeval_utc(void)
{
    GDateTime *dt;
    GTimeVal tv, tv2;
...
#elif defined G_OS_WIN32
    g_assert_cmpstr("E. South America Standard Time", ==, g_date_time_get_timezone_abbreviation(dt));
#endif
...
static void test_non_utf8_printf(void)
{
    gchar *oldlocale;
    ...
    for (year = 1; year <= 9999; year++)
    {
        ...
        for (month = 1; month <= 12; month++)
            for (day = 1; day <= days_in_month(year, month); day++)
            {
                ...
                /* sanity check */
                if G_UNLIKELY
                    (g_date_time_get_year(dt) != year ||
                     g_date_time_get_month(dt) != month ||
                     g_date_time_get_day_of_month(dt) != day) g_error("%04d-%02d-%02d comes out as %04d-%02d-%02d", year, month, day, g_date_time_get_year(dt), g_date_time_get_month(dt), g_date_time_get_day_of_month(dt));

                if G_UNLIKELY
                    (g_date_time_get_week_numbering_year(dt) != week_year ||
                     g_date_time_get_week_of_year(dt) != week_num ||
                     g_date_time_get_day_of_week(dt) != weekday)
g_error("%04d-%02d-%02d should be %04d-W%02d-%d but " "comes out as %04d-W%02d-%d", year, month, day, week_year, week_num, weekday, g_date_time_get_week_numbering_year(dt), g_date_time_get_week_of_year(dt), g_date_time_get_day_of_week(dt));

                if G_UNLIKELY
                    (g_date_time_to_unix(dt) != unix_time) g_error("%04d-%02d-%02d 00:00:00 UTC should have unix time %" G_GINT64_FORMAT " but comes out as %" G_GINT64_FORMAT, year, month, day, unix_time, g_date_time_to_unix(dt));

                if G_UNLIKELY
                    (g_date_time_get_day_of_year(dt) != day_of_year) g_error("%04d-%02d-%02d should be day of year %d" " but comes out as %d", year, month, day, day_of_year, g_date_time_get_day_of_year(dt));

                if G_UNLIKELY
                    (g_date_time_get_hour(dt) != 0 ||
                     g_date_time_get_minute(dt) != 0 ||
                     g_date_time_get_seconds(dt) != 0) g_error("%04d-%02d-%02d 00:00:00 UTC comes out " "as %02d:%02d:%02.6f", year, month, day, g_date_time_get_hour(dt), g_date_time_get_minute(dt), g_date_time_get_seconds(dt));
                /* done */
...
static void test_find_interval(void)
{
    GTimeZone *tz;
    GDateTime *dt;
    gint64 u;
...
static void test_adjust_time(void)
{
    GTimeZone *tz;
    GDateTime *dt;
    gint64 u, u2;
...
static void test_posix_parse(void)
{
    GTimeZone *tz;
    GDateTime *gdt1, *gdt2;
