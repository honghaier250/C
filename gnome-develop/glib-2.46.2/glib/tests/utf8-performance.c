...
static const char str_ascii[] = "The quick brown fox jumps over the lazy dog";

static const gchar str_latin1[] = "Zwölf Boxkämpfer jagen Viktor quer über den großen Sylter Deich";

/* Energizing GOELRO-talk in Russian, used by KDE */
static const char str_cyrillic[] = "Широкая электрификация южных губерний даст мощный толчок подъёму " "сельского хозяйства.";

/* First sentence from the Wikipedia article:
 * http://zh.wikipedia.org/w/index.php?title=%E6%B1%89%E5%AD%97&oldid=13053137 */
static const char str_han[] = "漢字，亦稱中文字、中国字，在台灣又被稱為國字，是漢字文化圈廣泛使用的一種文字，屬於表意文字的詞素音節文字";
...
static int grind_get_char(const char *str, gsize len)
{
    gunichar acc = 0;
    GRIND_LOOP_BEGIN
    {
...
static int grind_get_char_validated(const char *str, gsize len)
{
    gunichar acc = 0;
    GRIND_LOOP_BEGIN
    {
...
static int grind_utf8_to_ucs4(const char *str, gsize len)
{
    GRIND_LOOP_BEGIN
    {
        gunichar *ustr;
...
static int grind_get_char_backwards(const char *str, gsize len)
{
    gunichar acc = 0;
    GRIND_LOOP_BEGIN
    {
...
static int grind_utf8_to_ucs4_sized(const char *str, gsize len)
{
    GRIND_LOOP_BEGIN
    {
        gunichar *ustr;
...
static int grind_utf8_to_ucs4_fast(const char *str, gsize len)
{
    GRIND_LOOP_BEGIN
    {
        gunichar *ustr;
...
static int grind_utf8_to_ucs4_fast_sized(const char *str, gsize len)
{
    GRIND_LOOP_BEGIN
    {
        gunichar *ustr;
...
static int grind_utf8_validate(const char *str, gsize len)
{
    GRIND_LOOP_BEGIN g_utf8_validate(str, -1, NULL);
    GRIND_LOOP_END;
    return 0;
}

static int grind_utf8_validate_sized(const char *str, gsize len)
{
    GRIND_LOOP_BEGIN g_utf8_validate(str, len, NULL);
    GRIND_LOOP_END;
    return 0;
}
...
static void perform(gconstpointer data)
{
...
static void add_cases(const char *path, GrindFunc func)
{
