...
static gboolean check_signaled(GWakeup * wakeup)
{
    GPollFD fd;
...
static void wait_for_signaled(GWakeup * wakeup)
{
    GPollFD fd;
...
static void context_clear(struct context *ctx)
{
...
static void context_quit(struct context *ctx)
{
    ctx->quit = TRUE;
    g_wakeup_signal(ctx->wakeup);
}

static struct token *context_pop_token(struct context *ctx)
{
    struct token *token;

    g_mutex_lock(&ctx->lock);
    token = ctx->pending_tokens->data;
    ctx->pending_tokens = g_slist_delete_link(ctx->pending_tokens, ctx->pending_tokens);
    g_mutex_unlock(&ctx->lock);

    return token;
}

static void context_push_token(struct context *ctx, struct token *token)
{
...
static void dispatch_token(struct token *token)
{
    if (token->ttl > 0)
    {
        struct context *ctx;
...
static struct token *token_new(int ttl)
{
    struct token *token;
