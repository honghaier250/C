...
static void acquire(int nr, gboolean use_pointers)
{
    GThread *self;
    ...
    if (!(use_pointers ? g_pointer_bit_trylock(&ptrs[nr], bits[nr]) : g_bit_trylock(&locks[nr], bits[nr])))
    {
        if (g_test_verbose())
...
static void testcase(gconstpointer data)
{
    gboolean use_pointers = GPOINTER_TO_INT(data);
...
#    define SUFFIX ""
    ...
    for (i = 0; i < THREADS; i++)
        threads[i] = g_thread_new("foo", thread_func, GINT_TO_POINTER(use_pointers));
