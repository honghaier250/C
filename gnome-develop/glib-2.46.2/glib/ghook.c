...
GHook *g_hook_find_data(GHookList * hook_list, gboolean need_valids, gpointer data)
{
    ...
    {
        /* test only non-destroyed hooks */
        if (hook->data == data && hook->hook_id && (!need_valids || G_HOOK_ACTIVE(hook)))
            return hook;
...
GHook *g_hook_find_func(GHookList * hook_list, gboolean need_valids, gpointer func)
{
    ...
    {
        /* test only non-destroyed hooks */
        if (hook->func == func && hook->hook_id && (!need_valids || G_HOOK_ACTIVE(hook)))
            return hook;
...
GHook *g_hook_find_func_data(GHookList * hook_list, gboolean need_valids, gpointer func, gpointer data)
{
    ...
    {
        /* test only non-destroyed hooks */
        if (hook->data == data && hook->func == func && hook->hook_id && (!need_valids || G_HOOK_ACTIVE(hook)))
            return hook;
