...
static void g_futex_wait(const volatile gint * address, gint value)
{
    ...
    if G_LIKELY
        (g_atomic_int_get(address) == value)
    {
        WaitAddress *waiter;
...
#ifdef USE_ASM_GOTO
retry:
    __asm__ volatile goto ("lock bts %1, (%0)\n" "jc %l[contended]":    /* no output */
                           :"r" (address), "r"(lock_bit):"cc", "memory":contended);
    return;
...
gboolean g_bit_trylock(volatile gint * address, gint lock_bit)
{
    ...
    __asm__ volatile ("lock bts %2, (%1)\n" "setnc %%al\n" "movzx %%al, %0":"=r" (result):"r"(address), "r"(lock_bit):"cc", "memory");
...
{
#ifdef USE_ASM_GOTO
    asm volatile ("lock btr %1, (%0)":    /* no output */
                  :"r" (address), "r"(lock_bit):"cc", "memory");
#else
    guint mask = 1u << lock_bit;
...
static const volatile gint *g_futex_int_address(const volatile void *address)
{
    ...
    G_STATIC_ASSERT(G_BYTE_ORDER == G_LITTLE_ENDIAN || (G_BYTE_ORDER == G_BIG_ENDIAN && sizeof(int) == 4 && (sizeof(gpointer) == 4 || sizeof(gpointer) == 8)));
...
retry:
        asm volatile goto ("lock bts %1, (%0)\n" "jc %l[contended]":    /* no output */
                           :"r" (address), "r"((gsize) lock_bit):"cc", "memory":contended);
        return;
...
gboolean(g_pointer_bit_trylock) (volatile void *address, gint lock_bit)
{
...

    {
        ...
        asm volatile ("lock bts %2, (%1)\n" "setnc %%al\n" "movzx %%al, %0":"=r" (result):"r"(address), "r"((gsize) lock_bit):"cc", "memory");
...
#ifdef USE_ASM_GOTO
        asm volatile ("lock btr %1, (%0)":    /* no output */
                      :"r" (address), "r"((gsize) lock_bit):"cc", "memory");
#else
        volatile gsize *pointer_address = address;
        gsize mask = 1u << lock_bit;
