...
gboolean g_match_info_next(GMatchInfo * match_info, GError ** error)
{
    ...
    match_info->matches = pcre_exec(match_info->regex->pcre_re,
                                    match_info->regex->extra, match_info->string, match_info->string_len, match_info->pos, match_info->regex->match_opts | match_info->match_opts, match_info->offsets, match_info->n_offsets);
    if (IS_PCRE_ERROR(match_info->matches))
    {
        g_set_error(error, G_REGEX_ERROR, G_REGEX_ERROR_MATCH, _("Error while matching regular expression %s: %s"), match_info->regex->pattern, match_error(match_info->matches));
        return FALSE;
    }
 ...
 */
static pcre *regex_compile(const gchar * pattern, GRegexCompileFlags compile_options, GRegexCompileFlags * compile_options_out, GRegexMatchFlags * match_options, GError ** error);
...
GRegex *g_regex_new(const gchar * pattern, GRegexCompileFlags compile_options, GRegexMatchFlags match_options, GError ** error)
{
    ...
    {
        g_set_error_literal(error, G_REGEX_ERROR, G_REGEX_ERROR_COMPILE, _("PCRE library is compiled with incompatible options"));
        return NULL;
    }
    ...
    re = regex_compile(pattern, compile_options, &compile_options, &match_options, error);
    ...
    if (optimize)
    {
        ...
        if (errmsg != NULL)
        {
            GError *tmp_error = g_error_new(G_REGEX_ERROR,
                                            ...
                                            _("Error while optimizing " "regular expression %s: %s"),
                                            regex->pattern,
                                            errmsg);
            g_propagate_error(error, tmp_error);
...
static pcre *regex_compile(const gchar * pattern, GRegexCompileFlags compile_options, GRegexCompileFlags * compile_options_out, GRegexMatchFlags * match_options, GError ** error)
{
    ...
    if (!(compile_options & G_REGEX_NEWLINE_CR) && !(compile_options & G_REGEX_NEWLINE_LF))
    {
...
gint g_regex_get_max_lookbehind(const GRegex * regex)
{
    ...
    pcre_fullinfo(regex->pcre_re, regex->extra, PCRE_INFO_MAXLOOKBEHIND, &max_lookbehind);
...
gboolean g_regex_match_all_full(const GRegex * regex, const gchar * string, gssize string_len, gint start_position, GRegexMatchFlags match_options, GMatchInfo ** match_info, GError ** error)
{
    ...
    pcre_re = regex_compile(regex->pattern, regex->compile_opts | PCRE_NO_AUTO_POSSESS, NULL, NULL, error);
    ...
    while (!done)
    {
        ...
        info->matches = pcre_dfa_exec(pcre_re, extra, info->string, info->string_len, info->pos, regex->match_opts | match_options, info->offsets, info->n_offsets, info->workspace, info->n_workspace);
        ...
        {
            g_set_error(error, G_REGEX_ERROR, G_REGEX_ERROR_MATCH, _("Error while matching regular expression %s: %s"), regex->pattern, match_error(info->matches));
        }
    }
...
error:
    ...
    tmp_error = g_error_new(G_REGEX_ERROR, G_REGEX_ERROR_REPLACE, _("Error while parsing replacement " "text \"%s\" at char %lu: %s"), replacement, (gulong) (p - replacement), error_detail);
    g_propagate_error(error, tmp_error);
