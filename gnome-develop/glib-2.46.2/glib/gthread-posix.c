...
#    endif
    ...
    if G_UNLIKELY
        (mutex == NULL) g_thread_abort(errno, "malloc");
...
#    endif
    ...
    if G_UNLIKELY
        ((status = pthread_mutex_init(mutex, pattr)) != 0) g_thread_abort(status, "pthread_mutex_init");
...
void g_mutex_lock(GMutex * mutex)
{
    ...
    if G_UNLIKELY
        ((status = pthread_mutex_lock(g_mutex_get_impl(mutex))) != 0) g_thread_abort(status, "pthread_mutex_lock");
}
...
void g_mutex_unlock(GMutex * mutex)
{
    ...
    if G_UNLIKELY
        ((status = pthread_mutex_unlock(g_mutex_get_impl(mutex))) != 0) g_thread_abort(status, "pthread_mutex_unlock");
}
...
gboolean g_mutex_trylock(GMutex * mutex)
{
    ...
    if G_LIKELY
        ((status = pthread_mutex_trylock(g_mutex_get_impl(mutex))) == 0) return TRUE;

    if G_UNLIKELY
        (status != EBUSY) g_thread_abort(status, "pthread_mutex_trylock");
...
static pthread_rwlock_t *g_rw_lock_impl_new(void)
{
    ...
    if G_UNLIKELY
        (rwlock == NULL) g_thread_abort(errno, "malloc");

    if G_UNLIKELY
        ((status = pthread_rwlock_init(rwlock, NULL)) != 0) g_thread_abort(status, "pthread_rwlock_init");
...
#    elif defined (HAVE_PTHREAD_CONDATTR_SETCLOCK) && defined (CLOCK_MONOTONIC)
    if G_UNLIKELY
        ((status = pthread_condattr_setclock(&attr, CLOCK_MONOTONIC)) != 0) g_thread_abort(status, "pthread_condattr_setclock");
#    else
...
#    endif
    ...
    if G_UNLIKELY
        (cond == NULL) g_thread_abort(errno, "malloc");

    if G_UNLIKELY
        ((status = pthread_cond_init(cond, &attr)) != 0) g_thread_abort(status, "pthread_cond_init");
...
void g_cond_wait(GCond * cond, GMutex * mutex)
{
    ...
    if G_UNLIKELY
        ((status = pthread_cond_wait(g_cond_get_impl(cond), g_mutex_get_impl(mutex))) != 0) g_thread_abort(status, "pthread_cond_wait");
}
...
void g_cond_signal(GCond * cond)
{
    ...
    if G_UNLIKELY
        ((status = pthread_cond_signal(g_cond_get_impl(cond))) != 0) g_thread_abort(status, "pthread_cond_signal");
}
...
void g_cond_broadcast(GCond * cond)
{
    ...
    if G_UNLIKELY
        ((status = pthread_cond_broadcast(g_cond_get_impl(cond))) != 0) g_thread_abort(status, "pthread_cond_broadcast");
}
...
#    endif
    ...
    if G_UNLIKELY
        (status != ETIMEDOUT) g_thread_abort(status, "pthread_cond_timedwait");
...
static pthread_key_t *g_private_impl_new(GDestroyNotify notify)
{
    ...
    if G_UNLIKELY
        (key == NULL) g_thread_abort(errno, "malloc");
    status = pthread_key_create(key, notify);
    if G_UNLIKELY
        (status != 0) g_thread_abort(status, "pthread_key_create");
...
static void g_private_impl_free(pthread_key_t * key)
{
    ...
    if G_UNLIKELY
        (status != 0) g_thread_abort(status, "pthread_key_delete");
    free(key);
}
...
void g_private_set(GPrivate * key, gpointer value)
{
    ...
    if G_UNLIKELY
        ((status = pthread_setspecific(*g_private_get_impl(key), value)) != 0) g_thread_abort(status, "pthread_setspecific");
}
...
void g_private_replace(GPrivate * key, gpointer value)
{
    ...
    if G_UNLIKELY
        ((status = pthread_setspecific(*impl, value)) != 0) g_thread_abort(status, "pthread_setspecific");
}
...
GRealThread *g_system_thread_new(GThreadFunc thread_func, gulong stack_size, GError ** error)
{
    GThreadPosix *thread;
    pthread_attr_t attr;
    gint ret;
...
static void __attribute__ ((noinline)) g_mutex_unlock_slowpath(GMutex * mutex, guint prev)
{
...
void g_mutex_lock(GMutex * mutex)
{
    ...
    if G_UNLIKELY
        (g_atomic_int_add(&mutex->i[0], 1) != 0) g_mutex_lock_slowpath(mutex);
}
...
void g_mutex_unlock(GMutex * mutex)
{
    ...
    if G_UNLIKELY
        (prev != 1) g_mutex_unlock_slowpath(mutex, prev);
}
