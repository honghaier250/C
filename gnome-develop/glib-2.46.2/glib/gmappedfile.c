...
static GMappedFile *mapped_file_new_from_fd(int fd, gboolean writable, const gchar * filename, GError ** error)
{
    GMappedFile *file;
    struct stat st;
    ...
    if (fstat(fd, &st) == -1)
    {
        ...
        g_set_error(error,
                    ...
                    display_filename ? display_filename : "fd", display_filename ? "' " : "", display_filename ? display_filename : "", display_filename ? "'" : "", g_strerror(save_errno));
    ...
    if (file->contents == MAP_FAILED)
    {
        ...
        g_set_error(error,
                    G_FILE_ERROR,
                    g_file_error_from_errno(save_errno),
                    _("Failed to map %s%s%s%s: mmap() failed: %s"), display_filename ? display_filename : "fd", display_filename ? "' " : "", display_filename ? display_filename : "", display_filename ? "'" : "", g_strerror(save_errno));
        g_free(display_filename);
        goto out;
    }
...
GMappedFile *g_mapped_file_new(const gchar * filename, gboolean writable, GError ** error)
{
    ...
    if (fd == -1)
    {
        ...
        g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(save_errno), _("Failed to open file '%s': open() failed: %s"), display_filename, g_strerror(save_errno));
        g_free(display_filename);
        return NULL;
    }
...
GBytes *g_mapped_file_get_bytes(GMappedFile * file)
{
    ...
    return g_bytes_new_with_free_func(file->contents, file->length, (GDestroyNotify) g_mapped_file_unref, g_mapped_file_ref(file));
}
