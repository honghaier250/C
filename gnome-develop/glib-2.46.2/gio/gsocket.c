...
static gint g_socket_send_messages_with_blocking(GSocket * socket, GOutputMessage * messages, guint num_messages, gint flags, gboolean blocking, GCancellable * cancellable, GError ** error);
...
G_DEFINE_TYPE_WITH_CODE(GSocket, g_socket, G_TYPE_OBJECT, G_ADD_PRIVATE(GSocket) g_networking_init(); G_IMPLEMENT_INTERFACE(G_TYPE_INITABLE, g_socket_initable_iface_init));
...
static gboolean check_socket(GSocket * socket, GError ** error)
{
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_NOT_INITIALIZED, _("Invalid socket, initialization failed due to: %s"), socket->priv->construct_error->message);
        return FALSE;
    }
...
static gint g_socket_create_socket(GSocketFamily family, GSocketType type, int protocol, GError ** error)
{
    ...
    if (family <= 0)
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Unable to create socket: %s"), _("Unknown family was specified"));
        return -1;
    }
...
static void g_socket_constructed(GObject * object)
{
    ...
    if (socket->priv->fd >= 0)
        ...
        socket->priv->fd = g_socket_create_socket(socket->priv->family, socket->priv->type, socket->priv->protocol, &socket->priv->construct_error);
...
static void g_socket_class_init(GSocketClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_FAMILY,
                                    g_param_spec_enum("family", P_("Socket family"), P_("The sockets address family"), G_TYPE_SOCKET_FAMILY, G_SOCKET_FAMILY_INVALID, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_TYPE, g_param_spec_enum("type", P_("Socket type"), P_("The sockets type"), G_TYPE_SOCKET_TYPE, G_SOCKET_TYPE_STREAM, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_PROTOCOL,
                                    g_param_spec_enum("protocol",
                                                      P_("Socket protocol"),
                                                      P_("The id of the protocol to use, or -1 for unknown"), G_TYPE_SOCKET_PROTOCOL, G_SOCKET_PROTOCOL_UNKNOWN, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_FD, g_param_spec_int("fd", P_("File descriptor"), P_("The sockets file descriptor"), G_MININT, G_MAXINT, -1, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_BLOCKING, g_param_spec_boolean("blocking", P_("blocking"), P_("Whether or not I/O on this socket is blocking"), TRUE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_LISTEN_BACKLOG, g_param_spec_int("listen-backlog", P_("Listen backlog"), P_("Outstanding connections in the listen queue"), 0, SOMAXCONN, 10, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_KEEPALIVE, g_param_spec_boolean("keepalive", P_("Keep connection alive"), P_("Keep connection alive by sending periodic pings"), FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_LOCAL_ADDRESS, g_param_spec_object("local-address", P_("Local address"), P_("The local address the socket is bound to"), G_TYPE_SOCKET_ADDRESS, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_REMOTE_ADDRESS,
                                    g_param_spec_object("remote-address", P_("Remote address"), P_("The remote address the socket is connected to"), G_TYPE_SOCKET_ADDRESS, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 2.26
   */
    g_object_class_install_property(gobject_class, PROP_TIMEOUT, g_param_spec_uint("timeout", P_("Timeout"), P_("The timeout in seconds on socket I/O"), 0, G_MAXUINT, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 2.32
   */
    g_object_class_install_property(gobject_class, PROP_BROADCAST, g_param_spec_boolean("broadcast", P_("Broadcast"), P_("Whether to allow sending to broadcast addresses"), FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 2.32
   */
    g_object_class_install_property(gobject_class, PROP_TTL, g_param_spec_uint("ttl", P_("TTL"), P_("Time-to-live of outgoing unicast packets"), 0, G_MAXUINT, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_object_class_install_property(gobject_class, PROP_MULTICAST_LOOPBACK,
                                    g_param_spec_boolean("multicast-loopback", P_("Multicast loopback"), P_("Whether outgoing multicast packets loop back to the local host"), TRUE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 2.32
   */
    g_object_class_install_property(gobject_class, PROP_MULTICAST_TTL, g_param_spec_uint("multicast-ttl", P_("Multicast TTL"), P_("Time-to-live of outgoing multicast packets"), 0, G_MAXUINT, 1, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
void g_socket_set_keepalive(GSocket * socket, gboolean keepalive)
{
    ...
    if (!g_socket_set_option(socket, SOL_SOCKET, SO_KEEPALIVE, keepalive, &error))
    {
...
guint g_socket_get_ttl(GSocket * socket)
{
    ...
    {
        g_socket_get_option(socket, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &value, &error);
    }
    else
...
void g_socket_set_ttl(GSocket * socket, guint ttl)
{
    ...
    else if (socket->priv->family == G_SOCKET_FAMILY_IPV6)
    {
        ...
        g_socket_set_option(socket, IPPROTO_IPV6, IPV6_UNICAST_HOPS, ttl, &error);
    }
    else
        g_return_if_reached();
...
void g_socket_set_broadcast(GSocket * socket, gboolean broadcast)
{
    ...
    if (!g_socket_set_option(socket, SOL_SOCKET, SO_BROADCAST, broadcast, &error))
    {
...
gboolean g_socket_get_multicast_loopback(GSocket * socket)
{
    ...
    if (socket->priv->family == G_SOCKET_FAMILY_IPV4)
    {
        g_socket_get_option(socket, IPPROTO_IP, IP_MULTICAST_LOOP, &value, &error);
    }
    else if (socket->priv->family == G_SOCKET_FAMILY_IPV6)
    {
        g_socket_get_option(socket, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, &value, &error);
    }
    else
...
void g_socket_set_multicast_loopback(GSocket * socket, gboolean loopback)
{
    ...
    if (socket->priv->family == G_SOCKET_FAMILY_IPV4)
    {
        g_socket_set_option(socket, IPPROTO_IP, IP_MULTICAST_LOOP, loopback, &error);
    }
    else if (socket->priv->family == G_SOCKET_FAMILY_IPV6)
    {
        g_socket_set_option(socket, IPPROTO_IP, IP_MULTICAST_LOOP, loopback, NULL);
        g_socket_set_option(socket, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, loopback, &error);
    }
    else
        g_return_if_reached();
...
guint g_socket_get_multicast_ttl(GSocket * socket)
{
    ...
    if (socket->priv->family == G_SOCKET_FAMILY_IPV4)
    {
        g_socket_get_option(socket, IPPROTO_IP, IP_MULTICAST_TTL, &value, &error);
    }
    else if (socket->priv->family == G_SOCKET_FAMILY_IPV6)
    {
        g_socket_get_option(socket, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &value, &error);
    }
    else
        g_return_val_if_reached(FALSE);
...
void g_socket_set_multicast_ttl(GSocket * socket, guint ttl)
{
    ...
    else if (socket->priv->family == G_SOCKET_FAMILY_IPV6)
    {
        ...
        g_socket_set_option(socket, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, ttl, &error);
    }
    else
        g_return_if_reached();
...
static guint if_nametoindex(const gchar * iface)
{
...
static gboolean g_socket_multicast_group_operation(GSocket * socket, GInetAddress * group, gboolean source_specific, const gchar * iface, gboolean join_group, GError ** error)
{
...
#else
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, join_group ? _("Error joining multicast group: %s") : _("Error leaving multicast group: %s"), _("No support for source-specific multicast"));
            return FALSE;
#endif
        }
        else
            optname = join_group ? IP_ADD_MEMBERSHIP : IP_DROP_MEMBERSHIP;
        result = setsockopt(socket->priv->fd, IPPROTO_IP, optname, &mc_req, sizeof(mc_req));
    }
    else if (g_inet_address_get_family(group) == G_SOCKET_FAMILY_IPV6)
    {
        ...
        result = setsockopt(socket->priv->fd, IPPROTO_IPV6, optname, &mc_req_ipv6, sizeof(mc_req_ipv6));
    }
    else
        g_return_val_if_reached(FALSE);
    ...
    if (result < 0)
    {
        ...
        g_set_error(error, G_IO_ERROR, socket_io_error_from_errno(errsv), join_group ? _("Error joining multicast group: %s") : _("Error leaving multicast group: %s"), socket_strerror(errsv));
        return FALSE;
    }
    ...
    switch (socket->priv->family)
    {
        ...
        case G_SOCKET_FAMILY_IPV6:
#if defined (IPPROTO_IPV6) && defined (IPV6_V6ONLY)
            {
                ...
                if (!g_socket_get_option(socket, IPPROTO_IPV6, IPV6_V6ONLY, &v6_only, NULL))
                    return FALSE;
    ...
    while (1)
    {
        if (connect(socket->priv->fd, (struct sockaddr *) &buffer, g_socket_address_get_native_size(address)) < 0)
        {
...
#endif
            {
                ...
                g_set_error_literal(error, G_IO_ERROR, socket_io_error_from_errno(errsv), socket_strerror(errsv));
...
gboolean g_socket_shutdown(GSocket * socket, gboolean shutdown_read, gboolean shutdown_write, GError ** error)
{
    ...
    {
        int errsv = get_socket_errno();
        g_set_error(error, G_IO_ERROR, socket_io_error_from_errno(errsv), _("Unable to shutdown socket: %s"), socket_strerror(errsv));
        return FALSE;
    }
...
static GIOCondition update_condition(GSocket * socket)
{
    ...
    if (socket->priv->current_events & FD_CLOSE)
    {
        ...
        if (r > 0 || (r < 0 && errsv == WSAENOTCONN))
            condition |= G_IO_IN;
        else if (r == 0 || (r < 0 && (errsv == WSAESHUTDOWN || errsv == WSAECONNRESET || errsv == WSAECONNABORTED || errsv == WSAENETRESET)))
            condition |= G_IO_HUP;
        else
            condition |= G_IO_ERR;
    }
...
#ifdef G_OS_WIN32
static gboolean socket_source_prepare_win32(GSource * source, gint * timeout)
{
...
gboolean g_socket_condition_wait(GSocket * socket, GIOCondition condition, GCancellable * cancellable, GError ** error)
{
    ...
    return g_socket_condition_timed_wait(socket, condition, -1, cancellable, error);
}
...
gssize g_socket_send_message(GSocket * socket, GSocketAddress * address, GOutputVector * vectors, gint num_vectors, GSocketControlMessage ** messages, gint num_messages, gint flags, GCancellable * cancellable, GError ** error)
{
...
#ifndef G_OS_WIN32
    {
        ...
        /* iov */
        {
            ...
            if (sizeof *msg.msg_iov == sizeof *vectors &&
                ...
                G_STRUCT_OFFSET(struct iovec, iov_base) == G_STRUCT_OFFSET(GOutputVector, buffer) && sizeof msg.msg_iov->iov_len == sizeof vectors->size && G_STRUCT_OFFSET(struct iovec, iov_len) == G_STRUCT_OFFSET(GOutputVector, size))
                /* ABI is compatible */
            {
        ...
        while (1)
        {
            ...
            if (result < 0)
            {
                ...
                if (socket->priv->blocking && (errsv == EWOULDBLOCK || errsv == EAGAIN))
                {
                    if (!g_socket_condition_wait(socket, G_IO_OUT, cancellable, error))
                        return -1;
...
#else
    {
        ...
        {
            if (address)
                result = WSASendTo(socket->priv->fd, bufs, num_vectors, &bytes_sent, flags, (const struct sockaddr *) &addr, addrlen, NULL, NULL);
            else
                result = WSASend(socket->priv->fd, bufs, num_vectors, &bytes_sent, flags, NULL, NULL);
...
{
    return g_socket_send_messages_with_blocking(socket, messages, num_messages, flags, socket->priv->blocking, cancellable, error);
}
...
#    endif
        ...
        for (i = 0; i < num_messages; ++i)
        {
            ...
            /* iov */
            {
                ...
                if (sizeof(struct iovec) == sizeof(GOutputVector) &&
                    ...
                    G_STRUCT_OFFSET(struct iovec, iov_base) == G_STRUCT_OFFSET(GOutputVector, buffer) && sizeof msg_hdr->msg_iov->iov_len == sizeof msg->vectors->size && G_STRUCT_OFFSET(struct iovec, iov_len) == G_STRUCT_OFFSET(GOutputVector, size))
                    /* ABI is compatible */
                {
        ...
        while (num_sent < num_messages)
        {
            ...
            ret = sendmmsg(socket->priv->fd, msgvec + num_sent, num_messages - num_sent, flags | G_SOCKET_DEFAULT_SEND_FLAGS);
            ...
            if (ret < 0)
            {
                ...
                if (blocking && (errsv == EWOULDBLOCK || errsv == EAGAIN))
                {
                    if (!g_socket_condition_wait(socket, G_IO_OUT, cancellable, error))
                        return -1;
                ...
                if (num_sent > 0 && (errsv == EWOULDBLOCK || errsv == EAGAIN))
                {
                    max_sent = num_sent;
                    break;
                }
...
#else
    {
        ...
        for (i = 0; i < num_messages; ++i)
        {
            ...
            result = g_socket_send_message(socket, msg->address, msg->vectors, msg->num_vectors, msg->control_messages, msg->num_control_messages, flags, cancellable, &msg_error);
...
static GSocketAddress *cache_recv_address(GSocket * socket, struct sockaddr *native, int native_len)
{
...
gssize g_socket_receive_message(GSocket * socket, GSocketAddress ** address, GInputVector * vectors, gint num_vectors, GSocketControlMessage *** messages, gint * num_messages, gint * flags, GCancellable * cancellable, GError ** error)
{
...
#ifndef G_OS_WIN32
    {
        ...
        if (sizeof *msg.msg_iov == sizeof *vectors &&
            sizeof msg.msg_iov->iov_base == sizeof vectors->buffer &&
            G_STRUCT_OFFSET(struct iovec, iov_base) == G_STRUCT_OFFSET(GInputVector, buffer) && sizeof msg.msg_iov->iov_len == sizeof vectors->size && G_STRUCT_OFFSET(struct iovec, iov_len) == G_STRUCT_OFFSET(GInputVector, size))
            /* ABI is compatible */
        {
            msg.msg_iov = (struct iovec *) vectors;
            msg.msg_iovlen = num_vectors;
...
#    endif
...
#else
    {
        ...
        while (1)
        {
            addrlen = sizeof addr;
            if (address)
                result = WSARecvFrom(socket->priv->fd, bufs, num_vectors, &bytes_received, &win_flags, (struct sockaddr *) &addr, &addrlen, NULL, NULL);
            else
                result = WSARecv(socket->priv->fd, bufs, num_vectors, &bytes_received, &win_flags, NULL, NULL);
            if (result != 0)
            {
                int errsv = get_socket_errno();
...
#    ifdef SO_PEERCRED
    {
        ...
        if (getsockopt(socket->priv->fd, SOL_SOCKET, SO_PEERCRED, native_creds_buf, &optlen) == 0)
        {
            ret = g_credentials_new();
            g_credentials_set_native(ret, G_CREDENTIALS_NATIVE_TYPE, native_creds_buf);
        }
    }
...
#    elif G_CREDENTIALS_USE_SOLARIS_UCRED
    {
        ...
        {
            ret = g_credentials_new();
            g_credentials_set_native(ret, G_CREDENTIALS_TYPE_SOLARIS_UCRED, ucred);
            ucred_free(ucred);
        }
    }
#    else
...
#    endif
    ...
    if (!ret)
    {
        ...
        g_set_error(error, G_IO_ERROR, socket_io_error_from_errno(errsv), _("Unable to read socket credentials: %s"), socket_strerror(errsv));
    }
