        ...
        else if (state->strinfo->len)
        {
            if (!is_valid_choices(state->default_value, state->strinfo))
            {
                if (state->is_enum)
                    ...
                    g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, "<%s> contains string not in " "<choices>", tag);
            }
        }
    }
}
...
static void key_state_start_choices(KeyState * state, GError ** error)
{
    ...
    if (state->is_enum)
    {
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, "<choices> cannot be specified for keys " "tagged as having an enumerated type");
        return;
    }
...
static void key_state_start_aliases(KeyState * state, GError ** error)
{
    if (state->has_aliases)
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, "<aliases> already specified for this key");
    else if (!state->is_flags && !state->is_enum && !state->has_choices)
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, "<aliases> can only be specified for keys with " "enumerated or flags types or after <choices>");
}
...
static void key_state_add_alias(KeyState * state, const gchar * alias, const gchar * target, GError ** error)
{
    if (strinfo_builder_contains(state->strinfo, alias))
    {
        ...
        {
            if (state->is_enum)
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, "<alias value='%s'/> given when '%s' is already " "a member of the enumerated type", alias, alias);

            else
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, "<alias value='%s'/> given when " "<choice value='%s'/> was already given", alias, alias);
        }

        else
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, "<alias value='%s'/> already specified", alias);
    ...
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, "alias target '%s' is not in %s", target, state->is_enum ? "enumerated type" : "<choices>");
        return;
    }
    ...
    if (state->serialised == NULL)
    {
        ...
        else
        {
            ...
            if (state->minimum || state->maximum)
                g_variant_builder_add(&builder, "(y(**))", 'r', state->minimum, state->maximum);
...
static gboolean is_valid_keyname(const gchar * key, GError ** error)
{
    ...
    if (key[0] == '\0')
    {
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("empty names are not permitted"));
        return FALSE;
    }
    ...
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("invalid name '%s': names must begin " "with a lowercase letter"), key);
        return FALSE;
    }
    ...
    for (i = 1; key[i]; i++)
    {
        ...
        {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("invalid name '%s': invalid character '%c'; " "only lowercase letters, numbers and hyphen ('-') " "are permitted."), key, key[i]);
            return FALSE;
        }

        if (key[i] == '-' && key[i + 1] == '-')
        {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("invalid name '%s': two successive hyphens ('--') " "are not permitted."), key);
            return FALSE;
        }
    }
    ...
    if (key[i - 1] == '-')
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("invalid name '%s': the last character may not be a " "hyphen ('-')."), key);
        return FALSE;
    }

    if (i > 1024)
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("invalid name '%s': maximum length is 1024"), key);
        return FALSE;
    }
...
static void schema_state_add_child(SchemaState * state, const gchar * name, const gchar * schema, GError ** error)
{
    ...
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("<child name='%s'> already specified"), name);
        return;
    }
...
static KeyState *schema_state_add_key(SchemaState * state, GHashTable * enum_table, GHashTable * flags_table, const gchar * name, const gchar * type_string, const gchar * enum_type, const gchar * flags_type, GError ** error)
{
    SchemaState *node;
    GString *strinfo;
    KeyState *key;
    ...
    if (state->list_of)
    {
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("cannot add keys to a 'list-of' schema"));
        return NULL;
    }
    ...
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("<key name='%s'> already specified"), name);
        return NULL;
    }
    ...
    for (node = state; node; node = node->extends)
        if (node->extends)
        {
            ...
            {
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("<key name='%s'> shadows <key name='%s'> in " "<schema id='%s'>; use <override> to modify value"), name, name, node->extends_name);
                return NULL;
            }
        }
    ...
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_MISSING_ATTRIBUTE, _("exactly one of 'type', 'enum' or 'flags' must " "be specified as an attribute to <key>"));
        return NULL;
    }
    ...
    if (type_string == NULL)    /* flags or enums was specified */
    {
        ...
        if (enum_state == NULL)
        {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("<%s id='%s'> not (yet) defined."), flags_type ? "flags" : "enum", flags_type ? flags_type : enum_type);
            return NULL;
        }
    ...
    else
    {
        ...
        {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("invalid GVariant type string '%s'"), type_string);
            return NULL;
        }
...
static void schema_state_add_override(SchemaState * state, KeyState ** key_state, GString ** string, const gchar * key, const gchar * l10n, const gchar * context, GError ** error)
{
    SchemaState *parent;
    KeyState *original;
    ...
    {
        g_set_error_literal(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("<override> given but schema isn't " "extending anything"));
        return;
    }
    ...
    if (original == NULL)
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("no <key name='%s'> to override"), key);
        return;
    }

    if (g_hash_table_lookup(state->keys, key))
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("<override name='%s'> already specified"), key);
        return;
    }
...
static void parse_state_start_schema(ParseState * state, const gchar * id, const gchar * path, const gchar * gettext_domain, const gchar * extends_name, const gchar * list_of, GError ** error)
{
    SchemaState *extends;
    gchar *my_id;
    ...
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("<schema id='%s'> already specified"), id);
        return;
    }
    ...
    if (extends_name)
    {
        ...
        if (extends == NULL)
        {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("<schema id='%s'> extends not yet existing " "schema '%s'"), id, extends_name);
            return;
        }
    }
    else
    ...
    if (list_of)
    {
        ...
        {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("<schema id='%s'> is list of not yet existing " "schema '%s'"), id, list_of);
            return;
        }

        if (tmp->path)
        {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Can not be a list of a schema with a path"));
            return;
        }
    }
    ...
    if (extends)
    {
        if (extends->path)
        {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Can not extend a schema with a path"));
            return;
        }
        ...
        if (list_of)
        {
            ...
            {
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("<schema id='%s'> is a list, extending " "<schema id='%s'> which is not a list"), id, extends_name);
                return;
            }
            ...
            if (!is_subclass(list_of, extends->list_of, state->schema_table))
            {
                g_set_error(error, G_MARKUP_ERROR,
                            G_MARKUP_ERROR_INVALID_CONTENT, _("<schema id='%s' list-of='%s'> extends <schema " "id='%s' list-of='%s'> but '%s' does not " "extend '%s'"), id, list_of, extends_name, extends->list_of, list_of, extends->list_of);
                return;
            }
        }
        else
    ...
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("a path, if given, must begin and end with a slash"));
        return;
    }

    if (path && list_of && !g_str_has_suffix(path, ":/"))
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("the path of a list must end with ':/'"));
        return;
    }

    if (path && (g_str_has_prefix(path, "/apps/") || g_str_has_prefix(path, "/desktop/") || g_str_has_prefix(path, "/system/")))
        g_printerr("warning: Schema '%s' has path '%s'.  Paths starting with " "'/apps/', '/desktop/' or '/system/' are deprecated.\n", id, path);
...
static void parse_state_start_enum(ParseState * state, const gchar * id, gboolean is_flags, GError ** error)
{
    ...
    {
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("<%s id='%s'> already specified"), is_flags ? "flags" : "enum", id);
        return;
    }
...
#define NO_ATTRS()  COLLECT (G_MARKUP_COLLECT_INVALID, NULL)
    ...
    else if (strcmp(container, "schemalist") == 0)
    {
        if (strcmp(element_name, "schema") == 0)
        {
            ...
            if (COLLECT(STRING, "id", &id, OPTIONAL | STRING, "path", &path, OPTIONAL | STRING, "gettext-domain", &gettext_domain, OPTIONAL | STRING, "extends", &extends, OPTIONAL | STRING, "list-of", &list_of))
    ...
    else if (strcmp(container, "schema") == 0)
    {
        if (strcmp(element_name, "key") == 0)
        {
            ...
            if (COLLECT(STRING, "name", &name, OPTIONAL | STRING, "type", &type_string, OPTIONAL | STRING, "enum", &enum_type, OPTIONAL | STRING, "flags", &flags_type))
        ...
        else if (strcmp(element_name, "child") == 0)
        {
            ...
            if (COLLECT(STRING, "name", &name, STRING, "schema", &schema))
                schema_state_add_child(state->schema_state, name, schema, error);
            return;
        }
    ...
    else if (strcmp(container, "key") == 0)
    {
        ...
        else if (strcmp(element_name, "summary") == 0)
        {
            if (NO_ATTRS())
            {
                if (state->key_state->summary_seen && state->strict)
                    g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Only one <%s> element allowed inside <%s>"), element_name, container);
                else
        ...
        {
            const gchar *min, *max;
            if (COLLECT(STRING | OPTIONAL, "min", &min, STRING | OPTIONAL, "max", &max))
    ...
    if (container)
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT, _("Element <%s> not allowed inside <%s>"), element_name, container);
    else
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT, _("Element <%s> not allowed at the top level"), element_name);
}
...
static void end_element(GMarkupParseContext * context, const gchar * element_name, gpointer user_data, GError ** error)
{
    ...
    else if (strcmp(element_name, "enum") == 0 || strcmp(element_name, "flags") == 0)
...
static void text(GMarkupParseContext * context, const gchar * text, gsize text_len, gpointer user_data, GError ** error)
{
    ...
    else
    {
        ...
        for (i = 0; i < text_len; i++)
            ...
            {
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("text may not appear inside <%s>"), g_markup_parse_context_get_element(context));
                break;
            }
    }
}
...
static void output_key(gpointer key, gpointer value, gpointer user_data)
{
    ...
    if (state->child_schema && !g_hash_table_lookup(data->schema_table, state->child_schema))
...
static void output_schema(gpointer key, gpointer value, gpointer user_data)
{
    ...
    if (state->extends_name)
        gvdb_hash_table_insert_string(data.pair.table, ".extends", state->extends_name);

    if (state->list_of)
        gvdb_hash_table_insert_string(data.pair.table, ".list-of", state->list_of);

    if (data.l10n)
        gvdb_hash_table_insert_string(data.pair.table, ".gettext-domain", state->gettext_domain);
}
...
static gboolean write_to_file(GHashTable * schema_table, const gchar * filename, GError ** error)
{
    ...
    success = gvdb_table_write_contents(data.root_pair.table, filename, G_BYTE_ORDER != G_LITTLE_ENDIAN, error);
...
static GHashTable *parse_gschema_files(gchar ** files, gboolean strict)
{
    ...
    while ((filename = *files++) != NULL)
    {
        ...
        context = g_markup_parse_context_new(&parser, G_MARKUP_TREAT_CDATA_AS_TEXT | G_MARKUP_PREFIX_ERROR_POSITION | G_MARKUP_IGNORE_QUALIFIED, &state, NULL);
    ...
    while ((filename = *files++))
    {
        ...
        for (i = 0; groups[i]; i++)
        {
            ...
            for (j = 0; keys[j]; j++)
            {
                ...
                if (state == NULL)
                {
                    fprintf(stderr, _("No such key '%s' in schema '%s' as " "specified in override file '%s'"), key, group, filename);
                ...
                if (value == NULL)
                {
                    fprintf(stderr, _("error parsing key '%s' in schema '%s' " "as specified in override file '%s': " "%s."), key, group, filename, error->message);
                ...
                if (state->minimum)
                {
                    ...
                    {
                        fprintf(stderr, _("override for key '%s' in schema '%s' in " "override file '%s' is outside the range " "given in the schema"), key, group, filename);
                ...
                else if (state->strinfo->len)
                {
                    ...
                    {
                        fprintf(stderr, _("override for key '%s' in schema '%s' in " "override file '%s' is not in the list " "of valid choices"), key, group, filename);
...
int main(int argc, char **argv)
{
    ...
    if (!schema_files)
    {
        ...
        {
            if (g_str_has_suffix(file, ".gschema.xml") || g_str_has_suffix(file, ".enums.xml"))
                g_ptr_array_add(files, g_build_filename(srcdir, file, NULL));

            else if (g_str_has_suffix(file, ".gschema.override"))
                g_ptr_array_add(overrides, g_build_filename(srcdir, file, NULL));
        }
