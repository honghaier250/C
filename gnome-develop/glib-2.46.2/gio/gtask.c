...
static void g_task_return_now(GTask * task)
{
    ...
    {
        task->callback(task->source_object, G_ASYNC_RESULT(task), task->callback_data);
    }
...
static void g_task_start_task_thread(GTask * task, GTaskThreadFunc task_func)
{
    ...
    if (task->cancellable)
    {
         ...
         * completed.
         */
        g_signal_connect_data(task->cancellable, "cancelled", G_CALLBACK(task_thread_cancelled), g_object_ref(task), task_thread_cancelled_disconnect_notify, 0);
    }
...
{
    if (task->check_cancellable && g_cancellable_set_error_if_cancelled(task->cancellable, error))
        return TRUE;
    else if (task->error)
    {
...
static void g_task_class_init(GTaskClass * klass)
{
   ...
   * Since: 2.44
   */
    g_object_class_install_property(gobject_class, PROP_COMPLETED, g_param_spec_boolean("completed", P_("Task completed"), P_("Whether the task has completed yet"), FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
}
