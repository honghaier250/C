...
{
    void (*function) (GObject * target, GSettingsBackend * backend, const gchar * name, gpointer origin_tag, gchar ** names);
...
static gboolean g_settings_backend_invoke_closure(gpointer user_data)
{
    ...
    closure->function(closure->target, closure->backend, closure->name, closure->origin_tag, closure->names);
...
static void g_settings_backend_dispatch_signal(GSettingsBackend * backend, gsize function_offset, const gchar * name, gpointer origin_tag, const gchar * const *names)
{
    GSettingsBackendWatch *watch;
    ...
    while (closures)
    {
        ...
        if (closure->context)
            g_main_context_invoke(closure->context, g_settings_backend_invoke_closure, closure);
        else
...
void g_settings_backend_changed(GSettingsBackend * backend, const gchar * key, gpointer origin_tag)
{
    ...
    g_settings_backend_dispatch_signal(backend, G_STRUCT_OFFSET(GSettingsListenerVTable, changed), key, origin_tag, NULL);
}
...
void g_settings_backend_keys_changed(GSettingsBackend * backend, const gchar * path, gchar const *const *items, gpointer origin_tag)
{
    ...
    g_settings_backend_dispatch_signal(backend, G_STRUCT_OFFSET(GSettingsListenerVTable, keys_changed), path, origin_tag, items);
}
...
void g_settings_backend_path_changed(GSettingsBackend * backend, const gchar * path, gpointer origin_tag)
{
    ...
    g_settings_backend_dispatch_signal(backend, G_STRUCT_OFFSET(GSettingsListenerVTable, path_changed), path, origin_tag, NULL);
}
...
void g_settings_backend_writable_changed(GSettingsBackend * backend, const gchar * key)
{
    ...
    g_settings_backend_dispatch_signal(backend, G_STRUCT_OFFSET(GSettingsListenerVTable, writable_changed), key, NULL, NULL);
}
...
void g_settings_backend_path_writable_changed(GSettingsBackend * backend, const gchar * path)
{
    ...
    g_settings_backend_dispatch_signal(backend, G_STRUCT_OFFSET(GSettingsListenerVTable, path_writable_changed), path, NULL, NULL);
}
...
GVariant *g_settings_backend_read(GSettingsBackend * backend, const gchar * key, const GVariantType * expected_type, gboolean default_value)
{
    ...
    if G_UNLIKELY
        (value && !g_variant_is_of_type(value, expected_type))
    {
 ...
 * Returns: the value that was read, or %NULL
 */
GVariant *g_settings_backend_read_user_value(GSettingsBackend * backend, const gchar * key, const GVariantType * expected_type)
{
    GVariant *value;

    value = G_SETTINGS_BACKEND_GET_CLASS(backend)->read_user_value(backend, key, expected_type);
...
gboolean g_settings_backend_write(GSettingsBackend * backend, const gchar * key, GVariant * value, gpointer origin_tag)
{
    ...
    success = G_SETTINGS_BACKEND_GET_CLASS(backend)->write(backend, key, value, origin_tag);
    g_variant_unref(value);
...
static GVariant *g_settings_backend_real_read_user_value(GSettingsBackend * backend, const gchar * key, const GVariantType * expected_type)
{
...
{
    return g_tree_new_full((GCompareDataFunc) strcmp, NULL, g_free, g_settings_backend_variant_unref0);
}
...
static gboolean g_settings_backend_verify(gpointer impl)
{
    ...
    if (strcmp(G_OBJECT_TYPE_NAME(backend), "GMemorySettingsBackend") == 0 && g_strcmp0(g_getenv("GSETTINGS_BACKEND"), "memory") != 0)
    {
...
GSettingsBackend *g_settings_backend_get_default(void)
{
    ...
    backend = _g_io_module_get_default(G_SETTINGS_BACKEND_EXTENSION_POINT_NAME, "GSETTINGS_BACKEND", g_settings_backend_verify);
