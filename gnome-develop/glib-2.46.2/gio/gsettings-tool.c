...
static gboolean check_relocatable_schema(GSettingsSchema * schema, const gchar * schema_id)
{
    if (schema == NULL)
    {
    ...
    {
        g_printerr(_("Schema '%s' is not relocatable " "(path must not be specified)\n"), schema_id);
        return FALSE;
    }
...
static gboolean check_schema(GSettingsSchema * schema, const gchar * schema_id)
{
    ...
    if (is_relocatable_schema(schema))
    {
        g_printerr(_("Schema '%s' is relocatable " "(path must be specified)\n"), schema_id);
        return FALSE;
    }
...
{
    g_print("%s\n", g_settings_is_writable(global_settings, global_key) ? "true" : "false");
}
...
static void gsettings_set(void)
{
     ...
     */
    if (new == NULL && g_variant_type_equal(type, G_VARIANT_TYPE_STRING) && global_value[0] != '\'' && global_value[0] != '"')
    {
        g_clear_error(&error);
...
static int gsettings_help(gboolean requested, const gchar * command)
{
    ...
    else
    {
        ...
        if (strstr(synopsis, "[COMMAND]"))
            ...
            g_string_append(string, _("  SCHEMA    The name of the schema\n" "  PATH      The path, for relocatable schemas\n"));

        if (strstr(synopsis, "[KEY]"))
            g_string_append(string, _("  KEY       The (optional) key within the schema\n"));

        else if (strstr(synopsis, "KEY"))
            g_string_append(string, _("  KEY       The key within the schema\n"));
