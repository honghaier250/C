...
static void g_dbus_server_class_init(GDBusServerClass * klass)
{
   ...
   */
    g_object_class_install_property(gobject_class,
                                    ...
                                    g_param_spec_flags("flags",
                                                       ...
                                                       G_TYPE_DBUS_SERVER_FLAGS, G_DBUS_SERVER_FLAGS_NONE, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
   ...
   */
    g_object_class_install_property(gobject_class,
                                    PROP_GUID,
                                    g_param_spec_string("guid", P_("GUID"), P_("The guid of the server"), NULL, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
   ...
   */
    g_object_class_install_property(gobject_class,
                                    PROP_ADDRESS,
                                    g_param_spec_string("address", P_("Address"), P_("The address to listen on"), NULL, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
   ...
   */
    g_object_class_install_property(gobject_class,
                                    PROP_CLIENT_ADDRESS, g_param_spec_string("client-address", P_("Client Address"), P_("The address clients can use"), NULL, G_PARAM_READABLE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
   ...
   * Since: 2.26
   */
    g_object_class_install_property(gobject_class, PROP_ACTIVE, g_param_spec_boolean("active", P_("Active"), P_("Whether the server is currently active"), FALSE, G_PARAM_READABLE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
   ...
   */
    g_object_class_install_property(gobject_class,
                                    ...
                                    g_param_spec_object("authentication-observer",
                                                        ...
                                                        G_TYPE_DBUS_AUTH_OBSERVER, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
   ...
   * Since: 2.26
   */
    _signals[NEW_CONNECTION_SIGNAL] = g_signal_new(I_("new-connection"), G_TYPE_DBUS_SERVER, G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GDBusServerClass, new_connection), g_signal_accumulator_true_handled, NULL,    /* accu_data */
...
retry:
        ...
        if (g_unix_socket_address_abstract_names_supported())
            address = g_unix_socket_address_new_with_type(s->str, -1, G_UNIX_SOCKET_ADDRESS_ABSTRACT);
        else
            address = g_unix_socket_address_new(s->str);
        g_string_free(s, TRUE);

        local_error = NULL;
        if (!g_socket_listener_add_address(server->listener, address, G_SOCKET_TYPE_STREAM, G_SOCKET_PROTOCOL_DEFAULT, NULL,    /* source_object */
    ...
    else if (abstract != NULL)
    {
        if (!g_unix_socket_address_abstract_names_supported())
        {
            g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, _("Abstract name space not supported"));
            goto out;
        }
...
static gboolean try_tcp(GDBusServer * server, const gchar * address_entry, GHashTable * key_value_pairs, gboolean do_nonce, GError ** error)
{
    ...
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Cannot specify nonce file when creating a server"));
        goto out;
    }
    ...
    for (l = resolved_addresses; l != NULL; l = l->next)
    {
        ...
        if (!g_socket_listener_add_address(server->listener, socket_address, G_SOCKET_TYPE_STREAM, G_SOCKET_PROTOCOL_TCP, NULL,    /* GObject *source_object */
...
again:
        ...
        while (bytes_remaining > 0)
        {
            ...
            if (ret == -1)
            {
                if (errno == EINTR)
                    goto again;
                g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errno), _("Error writing nonce file at '%s': %s"), server->nonce_file, strerror(errno));
                goto out;
            }
            bytes_written += ret;
            bytes_remaining -= ret;
        ...
        server->client_address = g_strdup_printf("nonce-tcp:host=%s,port=%d,noncefile=%s", host, port_num, file_escaped);
        g_free(file_escaped);
    }
    else
    {
...
static gboolean on_run(GSocketService * service, GSocketConnection * socket_connection, GObject * source_object, gpointer user_data)
{
    ...
    if (server->nonce != NULL)
    {
        ...
        if (!g_input_stream_read_all(g_io_stream_get_input_stream(G_IO_STREAM(socket_connection)), buf, 16, &bytes_read, NULL,    /* GCancellable */
                                     NULL))    /* GError */
            goto out;
    ...
    connection = g_dbus_connection_new_sync(G_IO_STREAM(socket_connection), server->guid, connection_flags, server->authentication_observer, NULL,    /* GCancellable */
                                            NULL);    /* GError */
    if (connection == NULL)
        goto out;
    ...
    if (server->flags & G_DBUS_SERVER_FLAGS_RUN_IN_THREAD)
    {
        ...
        claimed = FALSE;
        g_signal_emit(server, _signals[NEW_CONNECTION_SIGNAL], 0, connection, &claimed);
        if (claimed)
...
static gboolean initable_init(GInitable * initable, GCancellable * cancellable, GError ** error)
{
    ...
    {
        g_set_error(&last_error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("The string '%s' is not a valid D-Bus GUID"), server->guid);
        goto out;
    }
    ...
    for (n = 0; addr_array != NULL && addr_array[n] != NULL; n++)
    {
        ...
        if (g_dbus_is_supported_address(address_entry, &this_error) && _g_dbus_address_parse_entry(address_entry, &transport_name, &key_value_pairs, &this_error))
        {
            ...
            if (FALSE)
            {
                ...
                g_set_error(&this_error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Cannot listen on unsupported transport '%s'"), transport_name);
