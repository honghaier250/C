...
static GFileAttributeValue *g_file_info_create_value(GFileInfo * info, guint32 attr_id)
{
    ...
    if (info->mask != NO_ATTRIBUTE_MASK && !_g_file_attribute_matcher_matches_id(info->mask, attr_id))
        return NULL;
...
retry:
    ...
    if (mi < matcher->sub_matchers->len)
        g_array_append_vals(result->sub_matchers, &g_array_index(matcher->sub_matchers, SubMatcher, mi), matcher->sub_matchers->len - mi);
...
const char *g_file_attribute_matcher_enumerate_next(GFileAttributeMatcher * matcher)
{
    ...
    while (1)
    {
        ...
        if (sub_matcher->mask == 0xffffffff && (sub_matcher->id & (NS_MASK << NS_POS)) == matcher->iterator_ns)
