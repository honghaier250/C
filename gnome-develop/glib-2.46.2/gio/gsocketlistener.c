...
static void g_socket_listener_class_init(GSocketListenerClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_LISTEN_BACKLOG,
                                    g_param_spec_int("listen-backlog", P_("Listen backlog"), P_("outstanding connections in the listen queue"), 0, 2000, 10, G_PARAM_CONSTRUCT | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 2.46
   */
    signals[EVENT] = g_signal_new(I_("event"), G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GSocketListenerClass, event), NULL, NULL, NULL, G_TYPE_NONE, 2, G_TYPE_SOCKET_LISTENER_EVENT, G_TYPE_SOCKET);
...
gboolean g_socket_listener_add_socket(GSocketListener * listener, GSocket * socket, GObject * source_object, GError ** error)
{
    ...
    if (source_object)
        g_object_set_qdata_full(G_OBJECT(socket), source_quark, g_object_ref(source_object), g_object_unref);
...
gboolean g_socket_listener_add_address(GSocketListener * listener, GSocketAddress * address, GSocketType type, GSocketProtocol protocol, GObject * source_object, GSocketAddress ** effective_address, GError ** error)
{
    ...
    g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_BINDING, socket);
    ...
    g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_LISTENING, socket);
    ...
    g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_LISTENED, socket);
...
gboolean g_socket_listener_add_inet_port(GSocketListener * listener, guint16 port, GObject * source_object, GError ** error)
{
         ...
         */
    {
        ...
        g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_BINDING, socket6);
        ...
        g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_BOUND, socket6);
        g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_LISTENING, socket6);
        ...
        if (source_object)
            g_object_set_qdata_full(G_OBJECT(socket6), source_quark, g_object_ref(source_object), g_object_unref);
         ...
         */
    {
             ...
             */
        {
            ...
            g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_BINDING, socket4);
            ...
            g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_BOUND, socket4);
            g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_LISTENING, socket4);
            ...
            g_signal_emit(listener, signals[EVENT], 0, G_SOCKET_LISTENER_LISTENED, socket4);

            if (source_object)
                g_object_set_qdata_full(G_OBJECT(socket4), source_quark, g_object_ref(source_object), g_object_unref);
        }
        else
...
GSocket *g_socket_listener_accept_socket(GSocketListener * listener, GObject ** source_object, GCancellable * cancellable, GError ** error)
{
    ...
    if (listener->priv->sockets->len == 1)
    {
        ...
        if (!g_socket_condition_wait(accept_socket, G_IO_IN, cancellable, error))
            return NULL;
    }
    else
    {
...
static gboolean accept_ready(GSocket * accept_socket, GIOCondition condition, gpointer user_data)
{
    ...
    if (socket)
    {
        source_object = g_object_get_qdata(G_OBJECT(accept_socket), source_quark);
        if (source_object)
            g_object_set_qdata_full(G_OBJECT(task), source_quark, g_object_ref(source_object), g_object_unref);
...
void g_socket_listener_accept_socket_async(GSocketListener * listener, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    sources = add_sources(listener, accept_ready, task, cancellable, g_main_context_get_thread_default());
...
guint16 g_socket_listener_add_any_inet_port(GSocketListener * listener, GObject * source_object, GError ** error)
{
    ...
    if ((socket4 != NULL || socket6 != NULL) && G_SOCKET_LISTENER_GET_CLASS(listener)->changed)
        G_SOCKET_LISTENER_GET_CLASS(listener)->changed(listener);
