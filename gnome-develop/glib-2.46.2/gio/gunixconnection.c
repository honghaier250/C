...
G_DEFINE_TYPE_WITH_CODE(GUnixConnection, g_unix_connection, G_TYPE_SOCKET_CONNECTION, g_socket_connection_factory_register_type(g_define_type_id, G_SOCKET_FAMILY_UNIX, G_SOCKET_TYPE_STREAM, G_SOCKET_PROTOCOL_DEFAULT););
...
gint g_unix_connection_receive_fd(GUnixConnection * connection, GCancellable * cancellable, GError ** error)
{
    ...
    if (nscm != 1)
    {
        ...
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, ngettext("Expecting 1 control message, got %d", "Expecting 1 control message, got %d", nscm), nscm);
    ...
    if (nfd != 1)
    {
        ...
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, ngettext("Expecting one fd, but got %d\n", "Expecting one fd, but got %d\n", nfd), nfd);
...
gboolean g_unix_connection_send_credentials(GUnixConnection * connection, GCancellable * cancellable, GError ** error)
{
    ...
    if (g_socket_send_message(socket, NULL,    /* address */
                              &vector, 1, &scm, num_messages, G_SOCKET_MSG_NONE, cancellable, error) != 1)
    {
...
static void send_credentials_async_thread(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
    GError *error = NULL;

    if (g_unix_connection_send_credentials(G_UNIX_CONNECTION(source_object), cancellable, &error))
...
GCredentials *g_unix_connection_receive_credentials(GUnixConnection * connection, GCancellable * cancellable, GError ** error)
{
...
#ifdef __linux__
    {
        ...
        opt_val = 0;
        if (!g_socket_get_option(socket, SOL_SOCKET, SO_PASSCRED, &opt_val, NULL))
        {
        ...
        if (opt_val == 0)
        {
            if (!g_socket_set_option(socket, SOL_SOCKET, SO_PASSCRED, TRUE, NULL))
            {
    ...
    num_bytes_read = g_socket_receive_message(socket, NULL,    /* GSocketAddress **address */
    ...
    else
    {
        if (nscm != 0)
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Not expecting control message, but got %d"), nscm);
            goto out;
        }
        else
        {
