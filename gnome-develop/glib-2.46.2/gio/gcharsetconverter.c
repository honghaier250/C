...
G_DEFINE_TYPE_WITH_CODE(GCharsetConverter, g_charset_converter, G_TYPE_OBJECT, G_IMPLEMENT_INTERFACE(G_TYPE_CONVERTER, g_charset_converter_iface_init); G_IMPLEMENT_INTERFACE(G_TYPE_INITABLE, g_charset_converter_initable_iface_init))
...
static void g_charset_converter_class_init(GCharsetConverterClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_TO_CHARSET, g_param_spec_string("to-charset", P_("To Charset"), P_("The character encoding to convert to"), NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_FROM_CHARSET, g_param_spec_string("from-charset", P_("From Charset"), P_("The character encoding to convert from"), NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class,
                                    PROP_USE_FALLBACK, g_param_spec_boolean("use-fallback", P_("Fallback enabled"), P_("Use fallback (of form \\<hexval>) for invalid bytes"), FALSE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS));
}
...
static GConverterResult g_charset_converter_convert(GConverter * converter, const void *inbuf, gsize inbuf_size, void *outbuf, gsize outbuf_size, GConverterFlags flags, gsize * bytes_read, gsize * bytes_written, GError ** error)
{
    GCharsetConverter *conv;
    gsize res;
    ...
    if (inbuf_size == 0)
    {
        ...
        {
            g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_PARTIAL_INPUT, _("Incomplete multibyte sequence in input"));
            return G_CONVERTER_ERROR;
        }
    }
    ...
    if (res == (gsize) - 1 && *bytes_read == 0)
    {
        ...
        switch (errsv)
        {
            ...
            case E2BIG:
                ...
                g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_NO_SPACE, _("Not enough space in destination"));
                break;
            ...
            case EILSEQ:
                ...
                if (conv->use_fallback)
                {
                    ...
                    g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA, _("Invalid byte sequence in conversion input"));
                break;

            default:
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Error during conversion: %s"), g_strerror(errsv));
                break;
        }
...
static gboolean g_charset_converter_initable_init(GInitable * initable, GCancellable * cancellable, GError ** error)
{
    ...
    {
        if (errno == EINVAL)
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, _("Conversion from character set '%s' to '%s' is not supported"), conv->from, conv->to);
        else
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Could not open converter from '%s' to '%s'"), conv->from, conv->to);
        return FALSE;
    }
