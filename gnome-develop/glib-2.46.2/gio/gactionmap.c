...
{
    return G_ACTION_MAP_GET_IFACE(action_map)->lookup_action(action_map, action_name);
}
...
void g_action_map_add_action_entries(GActionMap * action_map, const GActionEntry * entries, gint n_entries, gpointer user_data)
{
    ...
    for (i = 0; n_entries == -1 ? entries[i].name != NULL : i < n_entries; i++)
    {
        ...
        if (entry->parameter_type)
        {
            ...
            {
                g_critical("g_action_map_add_entries: the type " "string '%s' given as the parameter type for " "action '%s' is not a valid GVariant type " "string.  This action will not be added.", entry->parameter_type, entry->name);
                return;
            }
        ...
        if (entry->state)
        {
            ...
            if (state == NULL)
            {
                g_critical("g_action_map_add_entries: GVariant could " "not parse the state value given for action '%s' " "('%s'): %s.  This action will not be added.", entry->name, entry->state, error->message);
                g_error_free(error);
                continue;
            }
        ...
        if (entry->activate != NULL)
            g_signal_connect(action, "activate", G_CALLBACK(entry->activate), user_data);

        if (entry->change_state != NULL)
            g_signal_connect(action, "change-state", G_CALLBACK(entry->change_state), user_data);
