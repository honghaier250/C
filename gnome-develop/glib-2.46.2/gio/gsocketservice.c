...
G_DEFINE_TYPE_WITH_PRIVATE(GSocketService, g_socket_service, G_TYPE_SOCKET_LISTENER)
     ...
     static gboolean g_socket_service_real_incoming(GSocketService * service, GSocketConnection * connection, GObject * source_object)
{
    return FALSE;
}
...
static void g_socket_service_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
    GSocketService *service = G_SOCKET_SERVICE(object);
...
static void g_socket_service_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    GSocketService *service = G_SOCKET_SERVICE(object);
...
static void g_socket_service_class_init(GSocketServiceClass * class)
{
   ...
   */
    g_socket_service_incoming_signal =
        g_signal_new(I_("incoming"), G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GSocketServiceClass, incoming), g_signal_accumulator_true_handled, NULL, NULL, G_TYPE_BOOLEAN, 2, G_TYPE_SOCKET_CONNECTION, G_TYPE_OBJECT);
   ...
   * Since: 2.46
   */
    g_object_class_install_property(gobject_class, PROP_ACTIVE, g_param_spec_boolean("active", P_("Active"), P_("Whether the service is currently accepting connections"), TRUE, G_PARAM_CONSTRUCT | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
