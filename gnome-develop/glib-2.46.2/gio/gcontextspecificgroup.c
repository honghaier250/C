...
static gboolean g_context_specific_source_dispatch(GSource * source, GSourceFunc callback, gpointer user_data)
{
...
static GContextSpecificSource *g_context_specific_source_new(const gchar * name, gpointer instance)
{
 ...
 *    example)
 */
static void g_context_specific_group_request_state(GContextSpecificGroup * group, gboolean requested_state, GCallback requested_func)
{
    if (requested_state != group->requested_state)
    {
        ...
        else
        {
            ...
            g_main_context_invoke(GLIB_PRIVATE_CALL(g_get_worker_context) (), g_context_specific_group_change_state, group);
        }
    }
...
gpointer g_context_specific_group_get(GContextSpecificGroup * group, GType type, goffset context_offset, GCallback start_func)
{
    GContextSpecificSource *css;
...
void g_context_specific_group_remove(GContextSpecificGroup * group, GMainContext * context, gpointer instance, GCallback stop_func)
{
    GContextSpecificSource *css;

    if (!context)
    {
        g_critical("Removing %s with NULL context.  This object was probably directly constructed from a " "dynamic language.  This is not a valid use of the API.", G_OBJECT_TYPE_NAME(instance));
        return;
    }
...
void g_context_specific_group_emit(GContextSpecificGroup * group, guint signal_id)
{
    g_mutex_lock(&group->lock);
