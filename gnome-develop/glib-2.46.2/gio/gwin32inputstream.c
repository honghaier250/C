   ...
   */
    g_object_class_install_property(gobject_class,
                                    ...
                                    g_param_spec_pointer("handle",
                                                         P_("File handle"), P_("The file handle to read from"), G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
   ...
   */
    g_object_class_install_property(gobject_class,
                                    ...
                                    g_param_spec_boolean("close-handle",
                                                         ...
                                                         P_("Whether to close the file handle when the stream is closed"), TRUE, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
}
...
static gssize g_win32_input_stream_read(GInputStream * stream, void *buffer, gsize count, GCancellable * cancellable, GError ** error)
{
    ...
    else
    {
        ...
        if (errsv == ERROR_IO_PENDING && _g_win32_overlap_wait_result(win32_stream->priv->handle, &overlap, &nread, cancellable))
        {
            retval = nread;
            goto end;
        }
        ...
        else if (errsv == ERROR_HANDLE_EOF || errsv == ERROR_BROKEN_PIPE)
        {
...
GInputStream *g_win32_input_stream_new_from_fd(gint fd, gboolean close_fd)
{
    GWin32InputStream *win32_stream;
