...
gchar *g_icon_to_string(GIcon * icon)
{
    ...
    else if (G_IS_THEMED_ICON(icon))
    {
        ...
        if (names != NULL && names[0] != NULL && names[0][0] != '.' &&    /* Allowing icons starting with dot would break G_ICON_SERIALIZATION_MAGIC0 */
...
static GIcon *g_icon_new_from_tokens(char **tokens, GError ** error)
{
    ...
    if (num_tokens < 1)
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Wrong number of tokens (%d)"), num_tokens);
        goto out;
    }
    ...
    if (type == 0)
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("No type for class name %s"), tokens[0]);
        goto out;
    }

    if (!g_type_is_a(type, G_TYPE_ICON))
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Type %s does not implement the GIcon interface"), tokens[0]);
        goto out;
    }
    ...
    if (klass == NULL)
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Type %s is not classed"), tokens[0]);
        goto out;
    }
    ...
    if (version_str)
    {
        ...
        if (endp == NULL || *endp != '\0')
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Malformed version number: %s"), version_str);
            goto out;
        }
    }
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Type %s does not implement from_tokens() on the GIcon interface"), tokens[0]);
        goto out;
    }
...
GIcon *g_icon_new_for_string(const gchar * str, GError ** error)
{
    ...
    if (g_str_has_prefix(str, G_ICON_SERIALIZATION_MAGIC0))
    {
        ...
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Can't handle the supplied version of the icon encoding"));
...
GIcon *g_icon_deserialize(GVariant * value)
{
    ...
    g_return_val_if_fail(g_variant_is_of_type(value, G_VARIANT_TYPE_STRING) || g_variant_is_of_type(value, G_VARIANT_TYPE("(sv)")), NULL);
...
GVariant *g_icon_serialize(GIcon * icon)
{
    ...
    if (result)
    {
        ...
        {
            g_critical("g_icon_serialize() on icon type '%s' returned GVariant of type '%s' but it must return " "one with type '(sv)'", G_OBJECT_TYPE_NAME(icon), g_variant_get_type_string(result));
