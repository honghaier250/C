...
#define NO_ATTRS()  COLLECT (G_MARKUP_COLLECT_INVALID, NULL)
    ...
    else if (strcmp(container, "gresource") == 0)
    {
        ...
        {
            COLLECT(OPTIONAL | STRDUP, "alias", &state->alias, OPTIONAL | BOOL, "compressed", &state->compressed, OPTIONAL | STRDUP, "preprocess", &state->preproc_options);
    ...
    if (container)
        ...
        g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT, _("Element <%s> not allowed at toplevel"), element_name);
...
static void end_element(GMarkupParseContext * context, const gchar * element_name, gpointer user_data, GError ** error)
{
    ...
    else if (strcmp(element_name, "file") == 0)
    {
        ...
        {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("File %s appears multiple times in the resource"), key);
            return;
        }
        ...
        if (sourcedirs != NULL)
        {
            ...
            if (real_file == NULL)
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Failed to locate '%s' in any source directory"), file);
                return;
            }
        }
        else
        {
            ...
            if (!exists)
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Failed to locate '%s' in current directory"), file);
                return;
            }
        ...
        if (state->preproc_options)
        {
            ...
            for (i = 0; options[i]; i++)
            {
                ...
                {
                    g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Unknown processing option \"%s\""), options[i]);
                    g_strfreev(options);
                    goto cleanup;
                }
            }
            ...
            if (xml_stripblanks && xmllint != NULL)
            {
                ...
                if ((fd = g_mkstemp(tmp_file)) == -1)
                {
                    ...
                    g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), _("Failed to create temp file: %s"), g_strerror(errsv));
                    g_free(tmp_file);
                    tmp_file = NULL;
                    goto cleanup;
                }
                ...
                proc = g_subprocess_new(G_SUBPROCESS_FLAGS_STDOUT_SILENCE, error, xmllint, "--nonet", "--noblanks", "--output", tmp_file, real_file, NULL);
                g_free(real_file);
                real_file = NULL;
            ...
            if (to_pixdata)
            {
                ...
                {
                    g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, "to-pixbuf preprocessing requested but GDK_PIXBUF_PIXDATA " "not set and gdk-pixbuf-pixdata not found in path");
                    goto cleanup;
                }
                ...
                proc = g_subprocess_new(G_SUBPROCESS_FLAGS_STDOUT_SILENCE, error, gdk_pixbuf_pixdata, real_file, tmp_file2, NULL);
                g_free(real_file);
                real_file = NULL;
        ...
        {
            g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Error reading file %s: %s"), real_file, my_error->message);
        ...
        if (state->compressed)
        {
            ...
            GZlibCompressor *compressor = g_zlib_compressor_new(G_ZLIB_COMPRESSOR_FORMAT_ZLIB, 9);
            GOutputStream *out2 = g_converter_output_stream_new(out, G_CONVERTER(compressor));

            if (!g_output_stream_write_all(out2, data->content, data->size, NULL, NULL, NULL) || !g_output_stream_close(out2, NULL, NULL))
            {
                g_set_error(error, G_MARKUP_ERROR, G_MARKUP_ERROR_INVALID_CONTENT, _("Error compressing file %s"), real_file);
                goto cleanup;
            }
...
static GHashTable *parse_resource_file(const gchar * filename, gboolean collect_data)
{
    ...
    else if (collect_data)
    {
        ...
        while (g_hash_table_iter_next(&iter, (gpointer *) & key, (gpointer *) & data))
        {
            ...
            v_data = g_variant_new_from_data(G_VARIANT_TYPE("ay"), data->content, data->content_size, TRUE, g_free, data->content);
...
static gboolean write_to_file(GHashTable * table, const gchar * filename, GError ** error)
{
    gboolean success;

    success = gvdb_table_write_contents(table, filename, G_BYTE_ORDER != G_LITTLE_ENDIAN, error);
...
int main(int argc, char **argv)
{
    ...
    else if (generate_source)
    {
        ...
        if (!g_file_get_contents(binary_target, (char **) &data, &data_size, NULL))
        {
