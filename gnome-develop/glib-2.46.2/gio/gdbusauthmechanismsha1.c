...
static void _g_dbus_auth_mechanism_sha1_class_init(GDBusAuthMechanismSha1Class * klass)
{
...
static gchar *ensure_keyring_directory(GError ** error)
{
    ...
    if (g_file_test(path, G_FILE_TEST_EXISTS | G_FILE_TEST_IS_DIR))
    {
        if (g_getenv("G_DBUS_COOKIE_SHA1_KEYRING_DIR_IGNORE_PERMISSION") == NULL)
        {
            ...
            {
                g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errno), _("Error when getting information for directory '%s': %s"), path, strerror(errno));
                g_free(path);
                path = NULL;
                goto out;
            }
            ...
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Permissions on directory '%s' are malformed. Expected mode 0700, got 0%o"), path, statbuf.st_mode & 0777);
                g_free(path);
                path = NULL;
                goto out;
            }
...
static gchar *keyring_lookup_entry(const gchar * cookie_context, gint cookie_id, GError ** error)
{
    ...
    for (n = 0; lines[n] != NULL; n++)
    {
        ...
        if (*endp != '\0')
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("First token of line %d of the keyring at '%s' with content '%s' is malformed"), n + 1, path, line);
            g_strfreev(tokens);
            goto out;
        }
        ...
        if (*endp != '\0')
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Second token of line %d of the keyring at '%s' with content '%s' is malformed"), n + 1, path, line);
            g_strfreev(tokens);
            goto out;
        }
    ...
    if (lock_fd != -1)
    {
        ...
        if (!keyring_release_lock(path, lock_fd, &local_error))
        {
            if (error != NULL)
            {
                ...
                {
                    g_prefix_error(error, _("(Additionally, releasing the lock for '%s' also failed: %s) "), path, local_error->message);
                }
            }
            else
            {
