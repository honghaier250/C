...
static void got_new_info(GObject * source_object, GAsyncResult * res, gpointer user_data)
{
    ...
    if (!g_file_monitor_is_cancelled(G_FILE_MONITOR(poll_monitor)))
    {
        ...
        if (event != -1)
        {
            ...
            if (!g_file_monitor_is_cancelled(G_FILE_MONITOR(poll_monitor)) && (event == G_FILE_MONITOR_EVENT_CHANGED || event == G_FILE_MONITOR_EVENT_CREATED))
...
static gboolean poll_file_timeout(gpointer data)
{
    ...
    g_file_query_info_async(poll_monitor->file, G_FILE_ATTRIBUTE_ETAG_VALUE "," G_FILE_ATTRIBUTE_STANDARD_SIZE, 0, 0, NULL, got_new_info, g_object_ref(poll_monitor));
...
GFileMonitor *_g_poll_file_monitor_new(GFile * file)
{
    ...
    g_file_query_info_async(file, G_FILE_ATTRIBUTE_ETAG_VALUE "," G_FILE_ATTRIBUTE_STANDARD_SIZE, 0, 0, NULL, got_initial_info, g_object_ref(poll_monitor));
