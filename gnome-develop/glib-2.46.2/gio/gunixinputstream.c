...
G_DEFINE_TYPE_WITH_CODE(GUnixInputStream, g_unix_input_stream, G_TYPE_INPUT_STREAM,
                        ...
                        G_IMPLEMENT_INTERFACE(G_TYPE_POLLABLE_INPUT_STREAM, g_unix_input_stream_pollable_iface_init) G_IMPLEMENT_INTERFACE(G_TYPE_FILE_DESCRIPTOR_BASED, g_unix_input_stream_file_descriptor_based_iface_init))
     ...
     static void g_unix_input_stream_skip_async(GInputStream * stream, gsize count, int io_priority, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer data);
     ...
     static void g_unix_input_stream_class_init(GUnixInputStreamClass * klass)
{
   ...
   */
    g_object_class_install_property(gobject_class,
                                    ...
                                    g_param_spec_int("fd",
                                                     P_("File descriptor"),
                                                     P_("The file descriptor to read from"), G_MININT, G_MAXINT, -1, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
   ...
   */
    g_object_class_install_property(gobject_class,
                                    ...
                                    g_param_spec_boolean("close-fd",
                                                         ...
                                                         P_("Whether to close the file descriptor when the stream is closed"), TRUE, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
}
...
static gssize g_unix_input_stream_read(GInputStream * stream, void *buffer, gsize count, GCancellable * cancellable, GError ** error)
{
    ...
    if (unix_stream->priv->is_pipe_or_socket && g_cancellable_make_pollfd(cancellable, &poll_fds[1]))
        nfds = 2;
    else
        nfds = 1;
    ...
    while (1)
    {
        ...
        if (poll_ret == -1)
        {
            ...
            g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), _("Error reading from file descriptor: %s"), g_strerror(errsv));
            break;
        }
...
static gboolean g_unix_input_stream_close(GInputStream * stream, GCancellable * cancellable, GError ** error)
{
    ...
    if (res == -1)
    {
        ...
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), _("Error closing file descriptor: %s"), g_strerror(errsv));
    }
