...
static void async_init_thread(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
    GError *error = NULL;
...
static gboolean g_async_initable_real_init_finish(GAsyncInitable * initable, GAsyncResult * res, GError ** error)
{
     ...
     * them any more.
     */
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS if (G_IS_SIMPLE_ASYNC_RESULT(res))
    {
    ...
    G_GNUC_END_IGNORE_DEPRECATIONS g_return_val_if_fail(g_task_is_valid(res, initable), FALSE);
