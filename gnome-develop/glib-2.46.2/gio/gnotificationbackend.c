...
GNotificationBackend *g_notification_backend_new_default(GApplication * application)
{
    ...
    backend_type = _g_io_module_get_default_type(G_NOTIFICATION_BACKEND_EXTENSION_POINT_NAME, "GNOTIFICATION_BACKEND", G_STRUCT_OFFSET(GNotificationBackendClass, is_supported));
...
void g_notification_backend_send_notification(GNotificationBackend * backend, const gchar * id, GNotification * notification)
{
...
void g_notification_backend_withdraw_notification(GNotificationBackend * backend, const gchar * id)
{
    g_return_if_fail(G_IS_NOTIFICATION_BACKEND(backend));
