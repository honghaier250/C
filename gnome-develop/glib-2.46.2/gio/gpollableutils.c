...
GSource *g_pollable_source_new(GObject * pollable_stream)
{
    ...
    g_return_val_if_fail(G_IS_POLLABLE_INPUT_STREAM(pollable_stream) || G_IS_POLLABLE_OUTPUT_STREAM(pollable_stream), NULL);
...
gssize g_pollable_stream_read(GInputStream * stream, void *buffer, gsize count, gboolean blocking, GCancellable * cancellable, GError ** error)
{
    ...
    {
        return g_pollable_input_stream_read_nonblocking(G_POLLABLE_INPUT_STREAM(stream), buffer, count, cancellable, error);
    }
}
...
gssize g_pollable_stream_write(GOutputStream * stream, const void *buffer, gsize count, gboolean blocking, GCancellable * cancellable, GError ** error)
{
    ...
    {
        return g_pollable_output_stream_write_nonblocking(G_POLLABLE_OUTPUT_STREAM(stream), buffer, count, cancellable, error);
    }
}
...
gboolean g_pollable_stream_write_all(GOutputStream * stream, const void *buffer, gsize count, gboolean blocking, gsize * bytes_written, GCancellable * cancellable, GError ** error)
{
    ...
    {
        res = g_pollable_stream_write(stream, (char *) buffer + _bytes_written, count - _bytes_written, blocking, cancellable, error);
        if (res == -1)
        {
            if (bytes_written)
