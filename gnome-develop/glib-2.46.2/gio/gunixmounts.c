...
gboolean g_unix_is_mount_path_system_internal(const char *mount_path)
{
    ...
    if (g_str_has_prefix(mount_path, "/dev/") || g_str_has_prefix(mount_path, "/proc/") || g_str_has_prefix(mount_path, "/sys/"))
        return TRUE;
...
#    endif
    {
         ...
         */
        if (mntent->mnt_fsname != NULL && mntent->mnt_fsname[0] == '/' && g_hash_table_lookup(mounts_hash, mntent->mnt_fsname))
            continue;
...
#    endif
    {
        if ((strcmp(mntent->mnt_dir, "ignore") == 0) || (strcmp(mntent->mnt_dir, "swap") == 0) || (strcmp(mntent->mnt_dir, "none") == 0))
            continue;
...
#    ifdef HAVE_HASMNTOPT
            || (hasmntopt(mntent, "user") != NULL && hasmntopt(mntent, "user") != hasmntopt(mntent, "user_xattr")) || hasmntopt(mntent, "pamconsole") != NULL || hasmntopt(mntent, "users") != NULL || hasmntopt(mntent, "owner") != NULL
#    endif
            )
...
static GList *_g_get_unix_mount_points(void)
{
    ...
    while (!getmntent(file, &mntent))
    {
        if ((strcmp(mntent.mnt_mountp, "ignore") == 0) || (strcmp(mntent.mnt_mountp, "swap") == 0) || (strcmp(mntent.mnt_mountp, "none") == 0))
            continue;
...
{
    if (event_type != G_FILE_MONITOR_EVENT_CHANGED && event_type != G_FILE_MONITOR_EVENT_CREATED && event_type != G_FILE_MONITOR_EVENT_DELETED)
        return;
...
static gboolean proc_mounts_changed(GIOChannel * channel, GIOCondition cond, gpointer user_data)
{
    if (cond & G_IO_ERR)
        g_context_specific_group_emit(&mount_monitor_group, signals[MOUNTS_CHANGED]);
...
static gboolean mount_change_poller(gpointer user_data)
{
    ...
    for (new_it = current_mounts, old_it = mount_poller_mounts; new_it != NULL && old_it != NULL; new_it = g_list_next(new_it), old_it = g_list_next(old_it))
    {
...
static void mount_monitor_start(void)
{
    ...
    if (get_mtab_monitor_file() != NULL)
    {
        ...
        if (g_strcmp0(mtab_path, "/proc/mounts") == 0)
        {
            ...
            {
                g_warning("Error creating IO channel for /proc/mounts: %s (%s, %d)", error->message, g_quark_to_string(error->domain), error->code);
                g_error_free(error);
            }
            else
            {
                proc_mounts_watch_source = g_io_create_watch(proc_mounts_channel, G_IO_ERR);
                g_source_set_callback(proc_mounts_watch_source, (GSourceFunc) proc_mounts_changed, NULL, NULL);
                g_source_attach(proc_mounts_watch_source, g_main_context_get_thread_default());
    ...
    else
    {
        ...
        mount_poller_time = (guint64) g_get_monotonic_time();
        g_source_set_callback(proc_mounts_watch_source, mount_change_poller, NULL, NULL);
        g_source_attach(proc_mounts_watch_source, g_main_context_get_thread_default());
        g_source_unref(proc_mounts_watch_source);
    }
}
...
static void g_unix_mount_monitor_class_init(GUnixMountMonitorClass * klass)
{
   ...
   */
    signals[MOUNTS_CHANGED] = g_signal_new(I_("mounts-changed"), G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
   ...
   */
    signals[MOUNTPOINTS_CHANGED] = g_signal_new(I_("mountpoints-changed"), G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
...
{
    return g_context_specific_group_get(&mount_monitor_group, G_TYPE_UNIX_MOUNT_MONITOR, G_STRUCT_OFFSET(GUnixMountMonitor, context), mount_monitor_start);
}
...
static GUnixMountType guess_mount_type(const char *mount_path, const char *device_path, const char *filesystem_type)
{
    ...
    if ((strcmp(filesystem_type, "udf") == 0) || (strcmp(filesystem_type, "iso9660") == 0) || (strcmp(filesystem_type, "cd9660") == 0))
        type = G_UNIX_MOUNT_TYPE_CDROM;
    else if ((strcmp(filesystem_type, "nfs") == 0) || (strcmp(filesystem_type, "nfs4") == 0))
    ...
    else if (g_str_has_prefix(device_path, "/vol/"))
    {
        ...
        if (g_str_has_prefix(name, "cdrom"))
            type = G_UNIX_MOUNT_TYPE_CDROM;
        else if (g_str_has_prefix(name, "floppy") || g_str_has_prefix(device_path, "/vol/dev/diskette/"))
            type = G_UNIX_MOUNT_TYPE_FLOPPY;
    ...
    else
    {
        ...
        if (g_str_has_prefix(basename, "cdr") || g_str_has_prefix(basename, "cdwriter") || g_str_has_prefix(basename, "burn") || g_str_has_prefix(basename, "dvdr"))
            type = G_UNIX_MOUNT_TYPE_CDROM;
...
static const char *type_to_icon(GUnixMountType type, gboolean is_mount_point, gboolean use_symbolic)
{
...
gboolean g_unix_mount_guess_can_eject(GUnixMountEntry * mount_entry)
{
    ...
    if (guessed_type == G_UNIX_MOUNT_TYPE_IPOD || guessed_type == G_UNIX_MOUNT_TYPE_CDROM)
        return TRUE;
...
gboolean g_unix_mount_guess_should_display(GUnixMountEntry * mount_entry)
{
    ...
    if (mount_path != NULL)
    {
        ...
        if (strncmp(mount_path, "/run/media/", sizeof("/run/media/") - 1) == 0 && strncmp(mount_path + sizeof("/run/media/") - 1, user_name, user_name_len) == 0 && mount_path[sizeof("/run/media/") - 1 + user_name_len] == '/')
        ...
        if (is_in_runtime_dir || g_str_has_prefix(mount_path, "/media/"))
        {
            ...
            {
                struct stat st;
                if (g_stat(mount_entry->device_path, &st) == 0 && S_ISBLK(st.st_mode) && g_access(mount_path, R_OK | X_OK) != 0)
                    return FALSE;
            }
            return TRUE;
        }
