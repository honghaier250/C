...
static void g_inet_socket_address_class_init(GInetSocketAddressClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_ADDRESS, g_param_spec_object("address", P_("Address"), P_("The address"), G_TYPE_INET_ADDRESS, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_PORT, g_param_spec_uint("port", P_("Port"), P_("The port"), 0, 65535, 0, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 2.32
   */
    g_object_class_install_property(gobject_class, PROP_FLOWINFO, g_param_spec_uint("flowinfo", P_("Flow info"), P_("IPv6 flow info"), 0, G_MAXUINT32, 0, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 2.32
   */
    g_object_class_install_property(gobject_class, PROP_SCOPE_ID, g_param_spec_uint("scope-id", P_("Scope ID"), P_("IPv6 scope ID"), 0, G_MAXUINT32, 0, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
GSocketAddress *g_inet_socket_address_new_from_string(const char *address, guint port)
{
    ...
    if (strchr(address, ':'))
    {
        ...
        if (res->ai_family == AF_INET6 && res->ai_addrlen == sizeof(struct sockaddr_in6))
        {
