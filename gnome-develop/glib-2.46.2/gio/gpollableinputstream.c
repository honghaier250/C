...
G_DEFINE_INTERFACE(GPollableInputStream, g_pollable_input_stream, G_TYPE_INPUT_STREAM)
     ...
     static gssize g_pollable_input_stream_default_read_nonblocking(GPollableInputStream * stream, void *buffer, gsize count, GError ** error);
...
static gssize g_pollable_input_stream_default_read_nonblocking(GPollableInputStream * stream, void *buffer, gsize count, GError ** error)
{
    ...
    return G_INPUT_STREAM_GET_CLASS(stream)->read_fn(G_INPUT_STREAM(stream), buffer, count, NULL, error);
}
...
gssize g_pollable_input_stream_read_nonblocking(GPollableInputStream * stream, void *buffer, gsize count, GCancellable * cancellable, GError ** error)
{
    ...
    res = G_POLLABLE_INPUT_STREAM_GET_INTERFACE(stream)->read_nonblocking(stream, buffer, count, error);
