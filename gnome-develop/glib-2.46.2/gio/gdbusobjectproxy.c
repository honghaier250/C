...
G_DEFINE_TYPE_WITH_CODE(GDBusObjectProxy, g_dbus_object_proxy, G_TYPE_OBJECT, G_ADD_PRIVATE(GDBusObjectProxy) G_IMPLEMENT_INTERFACE(G_TYPE_DBUS_OBJECT, dbus_object_interface_init))
...
static void g_dbus_object_proxy_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
    GDBusObjectProxy *proxy = G_DBUS_OBJECT_PROXY(object);
...
static void g_dbus_object_proxy_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    GDBusObjectProxy *proxy = G_DBUS_OBJECT_PROXY(object);
...
static void g_dbus_object_proxy_class_init(GDBusObjectProxyClass * klass)
{
   ...
   * Since: 2.30
   */
    g_object_class_install_property(gobject_class, PROP_G_OBJECT_PATH, g_param_spec_string("g-object-path", "Object Path", "The object path of the proxy", NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 2.30
   */
    g_object_class_install_property(gobject_class, PROP_G_CONNECTION, g_param_spec_object("g-connection", "Connection", "The connection of the proxy", G_TYPE_DBUS_CONNECTION, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
}
...
static void g_dbus_object_proxy_init(GDBusObjectProxy * proxy)
{
    ...
    proxy->priv->map_name_to_iface = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, (GDestroyNotify) g_object_unref);
}
...
static GDBusInterface *g_dbus_object_proxy_get_interface(GDBusObject * object, const gchar * interface_name)
{
    GDBusObjectProxy *proxy = G_DBUS_OBJECT_PROXY(object);
    GDBusProxy *ret;
...
GDBusObjectProxy *g_dbus_object_proxy_new(GDBusConnection * connection, const gchar * object_path)
{
    ...
    return G_DBUS_OBJECT_PROXY(g_object_new(G_TYPE_DBUS_OBJECT_PROXY, "g-object-path", object_path, "g-connection", connection, NULL));
}

void _g_dbus_object_proxy_add_interface(GDBusObjectProxy * proxy, GDBusProxy * interface_proxy)
{
    ...
    g_hash_table_insert(proxy->priv->map_name_to_iface, g_strdup(interface_name), g_object_ref(interface_proxy));
...
void _g_dbus_object_proxy_remove_interface(GDBusObjectProxy * proxy, const gchar * interface_name)
{
