...
static gboolean g_memory_buffer_write(GMemoryBuffer * mbuf, const void *buffer, gsize count)
{
    guint8 *dest;
    gsize new_size;
...
static void g_dbus_message_class_init(GDBusMessageClass * klass)
{
   ...
   * Since: 2.26
   */
    g_object_class_install_property(gobject_class, PROP_LOCKED, g_param_spec_boolean("locked", P_("Locked"), P_("Whether the message is locked"), FALSE, G_PARAM_READABLE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
}
...
static gboolean validate_headers(GDBusMessage * message, GError ** error)
{
    ...
    switch (message->type)
    {
        case G_DBUS_MESSAGE_TYPE_INVALID:
            g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("type is INVALID"));
            goto out;
            break;

        case G_DBUS_MESSAGE_TYPE_METHOD_CALL:
            if (g_dbus_message_get_header(message, G_DBUS_MESSAGE_HEADER_FIELD_PATH) == NULL || g_dbus_message_get_header(message, G_DBUS_MESSAGE_HEADER_FIELD_MEMBER) == NULL)
            {
                g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("METHOD_CALL message: PATH or MEMBER header field is missing"));
                goto out;
            }
            break;
        ...
        case G_DBUS_MESSAGE_TYPE_METHOD_RETURN:
            ...
            {
                g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("METHOD_RETURN message: REPLY_SERIAL header field is missing"));
                goto out;
            }
            break;

        case G_DBUS_MESSAGE_TYPE_ERROR:
            if (g_dbus_message_get_header(message, G_DBUS_MESSAGE_HEADER_FIELD_ERROR_NAME) == NULL || g_dbus_message_get_header(message, G_DBUS_MESSAGE_HEADER_FIELD_REPLY_SERIAL) == NULL)
            {
                g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("ERROR message: REPLY_SERIAL or ERROR_NAME header field is missing"));
                goto out;
            }
            break;
        ...
        case G_DBUS_MESSAGE_TYPE_SIGNAL:
            if (g_dbus_message_get_header(message, G_DBUS_MESSAGE_HEADER_FIELD_PATH) == NULL ||
                g_dbus_message_get_header(message, G_DBUS_MESSAGE_HEADER_FIELD_INTERFACE) == NULL || g_dbus_message_get_header(message, G_DBUS_MESSAGE_HEADER_FIELD_MEMBER) == NULL)
            {
                g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("SIGNAL message: PATH, INTERFACE or MEMBER header field is missing"));
                goto out;
            }
            if (g_strcmp0(g_dbus_message_get_path(message), "/org/freedesktop/DBus/Local") == 0)
            {
                g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("SIGNAL message: The PATH header field is using the reserved value /org/freedesktop/DBus/Local"));
                goto out;
            }
            if (g_strcmp0(g_dbus_message_get_interface(message), "org.freedesktop.DBus.Local") == 0)
            {
                g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("SIGNAL message: The INTERFACE header field is using the reserved value org.freedesktop.DBus.Local"));
                goto out;
            }
            break;
...
static const gchar *read_string(GMemoryBuffer * mbuf, gsize len, GError ** error)
{
    ...
    if G_UNLIKELY
        (mbuf->pos + len >= mbuf->valid_len || mbuf->pos + len < mbuf->pos)
    {
        mbuf->pos = mbuf->valid_len;
        /* G_GSIZE_FORMAT doesn't work with gettext, so we use %lu */
        g_set_error(error,
                    G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, g_dngettext(GETTEXT_PACKAGE, "Wanted to read %lu byte but only got %lu", "Wanted to read %lu bytes but only got %lu", (gulong) len), (gulong) len, (gulong) (mbuf->valid_len - mbuf->pos));
        return NULL;
    }

    if G_UNLIKELY
        (mbuf->data[mbuf->pos + len] != '\0')
    {
        str = g_strndup(mbuf->data + mbuf->pos, len);
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Expected NUL byte after the string '%s' but found byte %d"), str, mbuf->data[mbuf->pos + len]);
        g_free(str);
        mbuf->pos += len + 1;
        return NULL;
    }
    ...
    if G_UNLIKELY
        (!g_utf8_validate(str, -1, &end_valid))
    {
        gint offset;
        gchar *valid_str;
        ...
        g_set_error(error,
                    G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Expected valid UTF-8 string but found invalid bytes at byte offset %d (length of string is %d). " "The valid UTF-8 string up until that point was '%s'"), offset, (gint) len, valid_str);
        g_free(valid_str);
        return NULL;
    }
...
static GVariant *parse_value_from_blob(GMemoryBuffer * buf, const GVariantType * type, gboolean just_align, guint indent, GError ** error)
{
    GVariant *ret;
    GError *local_error;
    gboolean is_leaf;
...
#ifdef DEBUG_SERIALIZER
    {
        ...
        g_print("%*s%s type %s from offset 0x%04x", indent, "", just_align ? "Aligning" : "Reading", s, (gint) g_seekable_tell(G_SEEKABLE(buf)));
        g_free(s);
    }
    ...
    switch (type_string[0])
    {
        ...
        case 'o':                /* G_VARIANT_TYPE_OBJECT_PATH */
            ...
            if (!just_align)
            {
                ...
                {
                    g_set_error(&local_error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Parsed value '%s' is not a valid D-Bus object path"), v);
                    goto fail;
                }
        ...
        case 'g':                /* G_VARIANT_TYPE_SIGNATURE */
            if (!just_align)
            {
                ...
                {
                    g_set_error(&local_error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Parsed value '%s' is not a valid D-Bus signature"), v);
                    goto fail;
                }
            ...
            if (!just_align)
            {
                ...
                if (array_len > (2 << 26))
                {
                    ...
                    g_set_error(&local_error,
                                ...
                                g_dngettext(GETTEXT_PACKAGE, "Encountered array of length %u byte. Maximum length is 2<<26 bytes (64 MiB).", "Encountered array of length %u bytes. Maximum length is 2<<26 bytes (64 MiB).", array_len), array_len);
                    goto fail;
                }
                ...
                if (fixed_size != 0)
                {
                    ...
                    if (array_len % fixed_size != 0)
                    {
                        g_set_error(&local_error,
                                    G_IO_ERROR,
                                    G_IO_ERROR_INVALID_ARGUMENT,
                                    _("Encountered array of type 'a%c', expected to have a length a multiple " "of %u bytes, but found to be %u bytes in length"), g_variant_type_peek_string(element_type)[0], fixed_size, array_len);
                        goto fail;
                    }
                ...
                else
                {
                    ...
                    {
                        GVariant *item;
                        item = parse_value_from_blob(buf, element_type, TRUE, indent + 2, NULL);
                        g_assert(item == NULL);
                    }
                    else
                    {
                        ...
                        {
                            GVariant *item;
                            item = parse_value_from_blob(buf, element_type, FALSE, indent + 2, &local_error);
                            if (item == NULL)
                            {
        ...
        default:
            if (g_variant_type_is_dict_entry(type))
            {
                ...
                if (!just_align)
                {
                    ...
                    key = parse_value_from_blob(buf, key_type, FALSE, indent + 2, &local_error);
                    if (key == NULL)
                        goto fail;
                    value_type = g_variant_type_value(type);
                    value = parse_value_from_blob(buf, value_type, FALSE, indent + 2, &local_error);
                    if (value == NULL)
                    {
                        g_variant_unref(key);
                        goto fail;
            ...
            else if (g_variant_type_is_variant(type))
            {
                ...
                if (!just_align)
                {
                    ...
                    {
                        g_set_error(&local_error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Parsed value '%s' for variant is not a valid D-Bus signature"), sig);
                        goto fail;
                    }
                    variant_type = g_variant_type_new(sig);
                    value = parse_value_from_blob(buf, variant_type, FALSE, indent + 2, &local_error);
            ...
            else
            {
                gchar *s;
                s = g_variant_type_dup_string(type);
                g_set_error(&local_error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Error deserializing GVariant with type string '%s' from the D-Bus wire format"), s);
                g_free(s);
                goto fail;
            }
            break;
...
gssize g_dbus_message_bytes_needed(guchar * blob, gsize blob_len, GError ** error)
{
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, "Unable to determine message blob length - given blob is malformed");
    }
...
GDBusMessage *g_dbus_message_new_from_blob(guchar * blob, gsize blob_len, GDBusCapabilityFlags capabilities, GError ** error)
{
    ...
    switch (endianness)
    {
        ...
        case 'B':
            ...
            break;
        default:
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Invalid endianness value. Expected 0x6c ('l') or 0x42 ('B') but found value 0x%02x"), endianness);
            goto out;
    }
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Invalid major protocol version. Expected 1 but found %d"), major_protocol_version);
        goto out;
    }
...
#endif                            /* DEBUG_SERIALIZER */
    headers = parse_value_from_blob(&mbuf, G_VARIANT_TYPE("a{yv}"), FALSE, 2, error);
    if (headers == NULL)
        goto out;
    ...
    if (signature != NULL)
    {
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Signature header with signature '%s' found but message body is empty"), signature_str);
            goto out;
        }
        else if (signature_str_len > 0)
        {
            ...
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Parsed value '%s' is not a valid D-Bus signature (for body)"), signature_str);
                goto out;
            }
...
#endif                            /* DEBUG_SERIALIZER */
            message->body = parse_value_from_blob(&mbuf, variant_type, FALSE, 2, error);
            g_variant_type_free(variant_type);
            if (message->body == NULL)
                goto out;
        }
    ...
    else
    {
        ...
        if (message_body_len != 0)
        {
            ...
            g_set_error(error,
                        G_IO_ERROR,
                        G_IO_ERROR_INVALID_ARGUMENT,
                        g_dngettext(GETTEXT_PACKAGE, "No signature header in message but the message body is %u byte", "No signature header in message but the message body is %u bytes", message_body_len), message_body_len);
            goto out;
        }
    }
...
static gboolean append_value_to_blob(GVariant * value, const GVariantType * type, GMemoryBuffer * mbuf, gsize * out_padding_added, GError ** error)
{
    gsize padding_added;
    const gchar *type_string;
        ...
        case 'a':                /* G_VARIANT_TYPE_ARRAY */
            {
                ...
                if (value != NULL)
                {
                    ...
                    if (g_variant_n_children(value) == 0)
                    {
                        ...
                        if (!append_value_to_blob(NULL, element_type, mbuf, &padding_added_for_item, error))
                            goto fail;
                    ...
                    else
                    {
                        ...
                        {
                            gsize padding_added_for_item;
                            if (!append_value_to_blob(item, g_variant_get_type(item), mbuf, &padding_added_for_item, error))
                            {
                                g_variant_unref(item);
                                goto fail;
                            }
        ...
        default:
            if (g_variant_type_is_dict_entry(type) || g_variant_type_is_tuple(type))
            {
                ...
                if (value != NULL)
                {
                    ...
                    while ((item = g_variant_iter_next_value(&iter)) != NULL)
                    {
                        if (!append_value_to_blob(item, g_variant_get_type(item), mbuf, NULL, error))
                        {
                            g_variant_unref(item);
                            goto fail;
                        }
            ...
            else if (g_variant_type_is_variant(type))
            {
                if (value != NULL)
                {
                    ...
                    g_memory_buffer_put_byte(mbuf, '\0');
                    if (!append_value_to_blob(child, g_variant_get_type(child), mbuf, NULL, error))
                    {
            ...
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Error serializing GVariant with type string '%s' to the D-Bus wire format"), g_variant_get_type_string(value));
                goto fail;
            }
            break;
    }
...
static gboolean append_body_to_blob(GVariant * value, GMemoryBuffer * mbuf, GError ** error)
{
    GVariant *item;
    GVariantIter iter;
...
guchar *g_dbus_message_to_blob(GDBusMessage * message, gsize * out_size, GDBusCapabilityFlags capabilities, GError ** error)
{
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Message has %d file descriptors but the header field indicates %d file descriptors"), num_fds_in_message, num_fds_according_to_header);
        goto out;
    }
    ...
    if (!append_value_to_blob(header_fields, g_variant_get_type(header_fields), &mbuf, NULL, error))
    {
    ...
    if (message->body != NULL)
    {
        ...
        if (signature == NULL)
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Message body has signature '%s' but there is no signature header"), signature_str);
            g_free(tupled_signature_str);
            goto out;
        }
        else if (g_strcmp0(tupled_signature_str, g_variant_get_type_string(message->body)) != 0)
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Message body has type signature '%s' but signature in the header field is '%s'"), tupled_signature_str, g_variant_get_type_string(message->body));
            g_free(tupled_signature_str);
            goto out;
        }
        g_free(tupled_signature_str);
    ...
    else
    {
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Message body is empty but signature in the header field is '(%s)'"), signature_str);
            goto out;
        }
    }
...
gboolean g_dbus_message_to_gerror(GDBusMessage * message, GError ** error)
{
    ...
    if (error_name != NULL)
    {
        ...
        else
        {
            ...
            if (body != NULL)
            {
                g_dbus_error_set_dbus_error(error, error_name, "", _("Error return with body of type '%s'"), g_variant_get_type_string(body));
            }
            else
            {
                g_dbus_error_set_dbus_error(error, error_name, "", _("Error return with empty body"));
            }
        }
    }
    else
    {
         ...
         */
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, "Error return without error-name header!");
    }
        ...
        if (num_fds > 0)
        {
            for (n = 0; n < num_fds; n++)
            {
                ...
                if (fstat(fds[n], &statbuf) == 0)
                {
                    ...
                    g_string_append_printf(fs, "%s" "uid=%u", fs->len > 0 ? "," : "", (guint) statbuf.st_uid);
                    g_string_append_printf(fs, "%s" "gid=%u", fs->len > 0 ? "," : "", (guint) statbuf.st_gid);
