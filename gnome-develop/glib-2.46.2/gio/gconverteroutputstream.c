...
static gssize g_converter_output_stream_write_nonblocking(GPollableOutputStream * stream, const void *buffer, gsize size, GError ** error);

static GSource *g_converter_output_stream_create_source(GPollableOutputStream * stream, GCancellable * cancellable);

static void g_converter_output_stream_pollable_iface_init(GPollableOutputStreamInterface * iface);

G_DEFINE_TYPE_WITH_CODE(GConverterOutputStream, g_converter_output_stream, G_TYPE_FILTER_OUTPUT_STREAM, G_ADD_PRIVATE(GConverterOutputStream) G_IMPLEMENT_INTERFACE(G_TYPE_POLLABLE_OUTPUT_STREAM, g_converter_output_stream_pollable_iface_init))
...
static gboolean flush_buffer(GConverterOutputStream * stream, gboolean blocking, GCancellable * cancellable, GError ** error)
{
    GConverterOutputStreamPrivate *priv;
    ...
    if (available > 0)
    {
        res = g_pollable_stream_write_all(base_stream, buffer_data(&priv->converted_buffer), available, blocking, &nwritten, cancellable, error);
...
static gssize write_internal(GOutputStream * stream, const void *buffer, gsize count, gboolean blocking, GCancellable * cancellable, GError ** error)
{
    GConverterOutputStream *cstream;
    GConverterOutputStreamPrivate *priv;
    gssize retval;
    ...
    while (!priv->finished && converted_bytes < to_convert_size)
    {
        ...
        res = g_converter_convert(priv->converter,
                                  ...
                                  to_convert_size - converted_bytes, buffer_data(&priv->converted_buffer) + buffer_data_size(&priv->converted_buffer), buffer_tailspace(&priv->converted_buffer), 0, &bytes_read, &bytes_written, &my_error);
...
static gboolean g_converter_output_stream_flush(GOutputStream * stream, GCancellable * cancellable, GError ** error)
{
    ...
    while (!priv->finished && !flushed)
    {
        ...
        res = g_converter_convert(priv->converter,
                                  ...
                                  buffer_tailspace(&priv->converted_buffer), is_closing ? G_CONVERTER_INPUT_AT_END : G_CONVERTER_FLUSH, &bytes_read, &bytes_written, &my_error);
...
static gboolean g_converter_output_stream_can_poll(GPollableOutputStream * stream)
{
    ...
    return (G_IS_POLLABLE_OUTPUT_STREAM(base_stream) && g_pollable_output_stream_can_poll(G_POLLABLE_OUTPUT_STREAM(base_stream)));
}
...
static gssize g_converter_output_stream_write_nonblocking(GPollableOutputStream * stream, const void *buffer, gsize count, GError ** error)
{
    return write_internal(G_OUTPUT_STREAM(stream), buffer, count, FALSE, NULL, error);
}
