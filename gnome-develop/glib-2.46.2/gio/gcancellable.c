...
static void g_cancellable_class_init(GCancellableClass * klass)
{
   ...
   */
    signals[CANCELLED] = g_signal_new(I_("cancelled"), G_TYPE_FROM_CLASS(gobject_class), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GCancellableClass, cancelled), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
...
gulong g_cancellable_connect(GCancellable * cancellable, GCallback callback, gpointer data, GDestroyNotify data_destroy_func)
{
    ...
    {
        id = g_signal_connect_data(cancellable, "cancelled", callback, data, (GClosureNotify) data_destroy_func, 0);
...
static void cancellable_source_cancelled(GCancellable * cancellable, gpointer user_data)
{
...
static void cancellable_source_finalize(GSource * source)
{
    ...
    {
        g_cancellable_disconnect(cancellable_source->cancellable, cancellable_source->cancelled_handler);
...
GSource *g_cancellable_source_new(GCancellable * cancellable)
{
    ...
    if (cancellable)
    {
         ...
         */
        cancellable_source->cancelled_handler = g_signal_connect(cancellable, "cancelled", G_CALLBACK(cancellable_source_cancelled), source);
        if (g_cancellable_is_cancelled(cancellable))
            g_source_set_ready_time(source, 0);
    }
