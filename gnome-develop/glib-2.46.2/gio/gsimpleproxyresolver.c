...
G_DEFINE_TYPE_WITH_CODE(GSimpleProxyResolver, g_simple_proxy_resolver, G_TYPE_OBJECT, G_ADD_PRIVATE(GSimpleProxyResolver) G_IMPLEMENT_INTERFACE(G_TYPE_PROXY_RESOLVER, g_simple_proxy_resolver_iface_init))
     enum
     {
         PROP_0,
         PROP_DEFAULT_PROXY,
...
static void g_simple_proxy_resolver_init(GSimpleProxyResolver * resolver)
{
    ...
    resolver->priv->uri_proxies = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_free);
}

static void g_simple_proxy_resolver_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    GSimpleProxyResolver *resolver = G_SIMPLE_PROXY_RESOLVER(object);
...
static void g_simple_proxy_resolver_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
    GSimpleProxyResolver *resolver = G_SIMPLE_PROXY_RESOLVER(object);
...
static gboolean ignore_host(GSimpleProxyResolver * resolver, const gchar * host, gushort port)
{
    GSimpleProxyResolverPrivate *priv = resolver->priv;
    ...
    if (priv->ignore_domains)
    {
        ...
        for (i = 0; priv->ignore_domains[i].length; i++)
        {
            ...
            if ((domain->port == 0 || domain->port == port) && (offset == 0 || (offset > 0 && host[offset - 1] == '.')) && (g_ascii_strcasecmp(domain->name, host + offset) == 0))
            {
                ignore = TRUE;
                break;
            }
...
static gchar **g_simple_proxy_resolver_lookup(GProxyResolver * proxy_resolver, const gchar * uri, GCancellable * cancellable, GError ** error)
{
    ...
    if (priv->ignore_ips || priv->ignore_domains)
    {
        ...
        if (_g_uri_parse_authority(uri, &host, &port, NULL) && ignore_host(resolver, host, port))
            proxy = "direct://";
...
static void g_simple_proxy_resolver_lookup_async(GProxyResolver * proxy_resolver, const gchar * uri, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    GSimpleProxyResolver *resolver = G_SIMPLE_PROXY_RESOLVER(proxy_resolver);
    GTask *task;
    GError *error = NULL;
    ...
    proxies = g_simple_proxy_resolver_lookup(proxy_resolver, uri, cancellable, &error);
    if (proxies)
...
static gchar **g_simple_proxy_resolver_lookup_finish(GProxyResolver * resolver, GAsyncResult * result, GError ** error)
{
...
static void g_simple_proxy_resolver_class_init(GSimpleProxyResolverClass * resolver_class)
{
   ...
   */
    g_object_class_install_property(object_class, PROP_DEFAULT_PROXY, g_param_spec_string("default-proxy", P_("Default proxy"), P_("The default proxy URI"), NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_object_class_install_property(object_class, PROP_IGNORE_HOSTS, g_param_spec_boxed("ignore-hosts", P_("Ignore hosts"), P_("Hosts that will not use the proxy"), G_TYPE_STRV, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
{
    return g_object_new(G_TYPE_SIMPLE_PROXY_RESOLVER, "default-proxy", default_proxy, "ignore-hosts", ignore_hosts, NULL);
}
...
void g_simple_proxy_resolver_set_uri_proxy(GSimpleProxyResolver * resolver, const gchar * uri_scheme, const gchar * proxy)
{
    ...
    g_hash_table_replace(resolver->priv->uri_proxies, g_ascii_strdown(uri_scheme, -1), g_strdup(proxy));
}
