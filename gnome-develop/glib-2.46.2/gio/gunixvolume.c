...
GUnixVolume *_g_unix_volume_new(GVolumeMonitor * volume_monitor, GUnixMountPoint * mountpoint)
{
    ...
    if (!(g_unix_mount_point_is_user_mountable(mountpoint) || g_str_has_prefix(g_unix_mount_point_get_device_path(mountpoint), "/vol/")) || g_unix_mount_point_is_loopback(mountpoint))
        return NULL;
...
static void eject_mount_done(GObject * source, GAsyncResult * result, gpointer user_data)
{
...
static void eject_mount_do(GVolume * volume, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data, const gchar * const *argv)
{
    ...
    g_subprocess_communicate_utf8_async(subprocess, NULL, g_task_get_cancellable(task), eject_mount_done, task);
}

static void g_unix_volume_mount(GVolume * volume, GMountMountFlags flags, GMountOperation * mount_operation, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    GUnixVolume *unix_volume = G_UNIX_VOLUME(volume);
...
static gchar *g_unix_volume_get_identifier(GVolume * volume, const gchar * kind)
{
    ...
    if (unix_volume->identifier_type != NULL && strcmp(kind, unix_volume->identifier_type) == 0)
