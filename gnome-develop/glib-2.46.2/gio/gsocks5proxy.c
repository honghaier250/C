...
G_DEFINE_TYPE_WITH_CODE(GSocks5Proxy, g_socks5_proxy, G_TYPE_OBJECT,
                        G_IMPLEMENT_INTERFACE(G_TYPE_PROXY, g_socks5_proxy_iface_init) _g_io_modules_ensure_extension_points_registered(); g_io_extension_point_implement(G_PROXY_EXTENSION_POINT_NAME, g_define_type_id, "socks5", 0))
...
static gboolean parse_nego_reply(const guint8 * data, gboolean has_auth, gboolean * must_auth, GError ** error)
{
    ...
    switch (data[1])
    {
        ...
        default:
            g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_PROXY_AUTH_FAILED, _("The SOCKSv5 proxy requires an authentication " "method that is not supported by GLib."));
            return FALSE;
            break;
    }
...
static gint set_auth_msg(guint8 * msg, const gchar * username, const gchar * password, GError ** error)
{
    ...
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_PROXY_FAILED, _("Username or password is too long for SOCKSv5 " "protocol."));
        return FALSE;
    }
...
static gint set_connect_msg(guint8 * msg, const gchar * hostname, guint16 port, GError ** error)
{
    ...
    else
    {
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_PROXY_FAILED, _("Hostname '%s' is too long for SOCKSv5 protocol"), hostname);
            return -1;
        }
    ...
    switch (data[1])
    {
        case SOCKS5_REP_SUCCEEDED:
            ...
            switch (data[3])
            {
                ...
                default:
                    g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_PROXY_FAILED, _("The SOCKSv5 proxy server uses unknown address type."));
                    return FALSE;
            }
            break;
...
static GIOStream *g_socks5_proxy_connect(GProxy * proxy, GIOStream * io_stream, GProxyAddress * proxy_address, GCancellable * cancellable, GError ** error)
{
    ...
    /* Receive SOCKS5 response and reply with authentication if required */
    {
        ...
        if (!g_input_stream_read_all(in, data, sizeof(data), NULL, cancellable, error))
            goto error;
    ...
    /* Read SOCKS5 response */
    {
        ...
        {
            case SOCKS5_ATYP_IPV4:
                if (!g_input_stream_read_all(in, data, 6, NULL, cancellable, error))
                    goto error;
                break;

            case SOCKS5_ATYP_IPV6:
                if (!g_input_stream_read_all(in, data, 18, NULL, cancellable, error))
                    goto error;
                break;

            case SOCKS5_ATYP_DOMAINNAME:
                if (!g_input_stream_read_all(in, data, 1, NULL, cancellable, error))
                    goto error;
                if (!g_input_stream_read_all(in, data, data[0] + 2, NULL, cancellable, error))
                    goto error;
                break;
        }
    }
...
static void do_read(GAsyncReadyCallback callback, GTask * task, ConnectAsyncData * data)
{
    GInputStream *in;
    in = g_io_stream_get_input_stream(data->io_stream);
    g_input_stream_read_async(in, data->buffer + data->offset, data->length - data->offset, g_task_get_priority(task), g_task_get_cancellable(task), callback, task);
}

static void do_write(GAsyncReadyCallback callback, GTask * task, ConnectAsyncData * data)
{
    GOutputStream *out;
    out = g_io_stream_get_output_stream(data->io_stream);
    g_output_stream_write_async(out, data->buffer + data->offset, data->length - data->offset, g_task_get_priority(task), g_task_get_cancellable(task), callback, task);
}

static void g_socks5_proxy_connect_async(GProxy * proxy, GIOStream * io_stream, GProxyAddress * proxy_address, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    GTask *task;
    ...
    g_object_get(G_OBJECT(proxy_address), "destination-hostname", &data->hostname, "destination-port", &data->port, "username", &data->username, "password", &data->password, NULL);
