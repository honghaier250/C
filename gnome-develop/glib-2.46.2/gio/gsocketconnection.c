     ...
     static GInputStream *g_socket_connection_get_input_stream(GIOStream * io_stream)
{
    ...
    if (connection->priv->input_stream == NULL)
        connection->priv->input_stream = (GInputStream *) _g_socket_input_stream_new(connection->priv->socket);
...
static GOutputStream *g_socket_connection_get_output_stream(GIOStream * io_stream)
{
    ...
    if (connection->priv->output_stream == NULL)
        connection->priv->output_stream = (GOutputStream *) _g_socket_output_stream_new(connection->priv->socket);
...
gboolean g_socket_connection_connect(GSocketConnection * connection, GSocketAddress * address, GCancellable * cancellable, GError ** error)
{
    ...
    return g_socket_connect(connection->priv->socket, address, cancellable, error);
}

static gboolean g_socket_connection_connect_callback(GSocket * socket, GIOCondition condition, gpointer user_data);
...
void g_socket_connection_connect_async(GSocketConnection * connection, GSocketAddress * address, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (g_socket_connect(connection->priv->socket, address, cancellable, &tmp_error))
    {
    ...
    else if (g_error_matches(tmp_error, G_IO_ERROR, G_IO_ERROR_PENDING))
    {
        ...
        source = g_socket_create_source(connection->priv->socket, G_IO_OUT, cancellable);
        g_task_attach_source(task, source, (GSourceFunc) g_socket_connection_connect_callback);
...
GSocketAddress *g_socket_connection_get_remote_address(GSocketConnection * connection, GError ** error)
{
    ...
    {
        return connection->priv->cached_remote_address ? g_object_ref(connection->priv->cached_remote_address) : NULL;
    }
 ...
 */
void g_socket_connection_set_cached_remote_address(GSocketConnection * connection, GSocketAddress * address)
{
...
static void g_socket_connection_class_init(GSocketConnectionClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_SOCKET, g_param_spec_object("socket", P_("Socket"), P_("The underlying GSocket"), G_TYPE_SOCKET, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
static gboolean g_socket_connection_close(GIOStream * stream, GCancellable * cancellable, GError ** error)
{
    ...
    if (connection->priv->output_stream)
        g_output_stream_close(connection->priv->output_stream, cancellable, NULL);
    if (connection->priv->input_stream)
...
void g_socket_connection_factory_register_type(GType g_type, GSocketFamily family, GSocketType type, gint protocol)
{
    ...
    if (connection_factories == NULL)
        connection_factories = g_hash_table_new_full(connection_factory_hash, connection_factory_equal, (GDestroyNotify) g_free, NULL);
