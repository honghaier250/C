...
G_DEFINE_TYPE_WITH_CODE(GResourceFile, g_resource_file, G_TYPE_OBJECT, G_IMPLEMENT_INTERFACE(G_TYPE_FILE, g_resource_file_file_iface_init))
#define g_resource_file_enumerator_get_type _g_resource_file_enumerator_get_type
    G_DEFINE_TYPE(GResourceFileEnumerator, g_resource_file_enumerator, G_TYPE_FILE_ENUMERATOR);
     ...
     static GFileEnumerator *_g_resource_file_enumerator_new(GResourceFile * file, const char *attributes, GFileQueryInfoFlags flags, GCancellable * cancellable, GError ** error);
...
static gboolean g_resource_file_has_uri_scheme(GFile * file, const char *uri_scheme)
{
...
static GFile *g_resource_file_resolve_relative_path(GFile * file, const char *relative_path)
{
    GResourceFile *resource = G_RESOURCE_FILE(file);
    char *filename;
    GFile *child;
...
static GFileEnumerator *g_resource_file_enumerate_children(GFile * file, const char *attributes, GFileQueryInfoFlags flags, GCancellable * cancellable, GError ** error)
{
    GResourceFile *resource = G_RESOURCE_FILE(file);
    return _g_resource_file_enumerator_new(resource, attributes, flags, cancellable, error);
}

static GFile *g_resource_file_get_child_for_display_name(GFile * file, const char *display_name, GError ** error)
{
    GFile *new_file;
...
static GFileInfo *g_resource_file_query_info(GFile * file, const char *attributes, GFileQueryInfoFlags flags, GCancellable * cancellable, GError ** error)
{
    GResourceFile *resource = G_RESOURCE_FILE(file);
    ...
    if (!is_dir)
    {
        ...
        if (!res)
        {
            ...
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND, _("The resource at '%s' does not exist"), resource->path);
            }
            else
                g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, my_error->message);
    ...
    else
    {
        ...
        if ((_g_file_attribute_matcher_matches_id(matcher, G_FILE_ATTRIBUTE_ID_STANDARD_CONTENT_TYPE) ||
             ((~resource_flags & G_RESOURCE_FLAGS_COMPRESSED) && _g_file_attribute_matcher_matches_id(matcher, G_FILE_ATTRIBUTE_ID_STANDARD_FAST_CONTENT_TYPE))) && (bytes = g_resources_lookup_data(resource->path, 0, NULL)))
        {
            const guchar *data;
            gsize data_size;
...
static GFileInfo *g_resource_file_query_filesystem_info(GFile * file, const char *attributes, GCancellable * cancellable, GError ** error)
{
    GFileInfo *info;
    GFileAttributeMatcher *matcher;
...
static GFileAttributeInfoList *g_resource_file_query_settable_attributes(GFile * file, GCancellable * cancellable, GError ** error)
{
    return g_file_attribute_info_list_ref(resource_writable_attributes);
}

static GFileAttributeInfoList *g_resource_file_query_writable_namespaces(GFile * file, GCancellable * cancellable, GError ** error)
{
    return g_file_attribute_info_list_ref(resource_writable_namespaces);
}

static GFileInputStream *g_resource_file_read(GFile * file, GCancellable * cancellable, GError ** error)
{
    GResourceFile *resource = G_RESOURCE_FILE(file);
    GError *my_error = NULL;
    GInputStream *stream;
...
static GFileMonitor *g_resource_file_monitor_file(GFile * file, GFileMonitorFlags flags, GCancellable * cancellable, GError ** error)
{
...
static GFileInfo *g_resource_file_enumerator_next_file(GFileEnumerator * enumerator, GCancellable * cancellable, GError ** error);
static gboolean g_resource_file_enumerator_close(GFileEnumerator * enumerator, GCancellable * cancellable, GError ** error);
...
static void g_resource_file_enumerator_class_init(GResourceFileEnumeratorClass * klass)
{
    GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
...
static GFileEnumerator *_g_resource_file_enumerator_new(GResourceFile * file, const char *attributes, GFileQueryInfoFlags flags, GCancellable * cancellable, GError ** error)
{
    ...
    if (children == NULL && strcmp("/", file->path) != 0)
    {
        ...
        if (res)
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_NOT_DIRECTORY, _("The resource at '%s' is not a directory"), file->path);
        else
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND, _("The resource at '%s' does not exist"), file->path);
        return NULL;
    }

    resource = g_object_new(G_TYPE_RESOURCE_FILE_ENUMERATOR, "container", file, NULL);
...
static GFileInfo *g_resource_file_enumerator_next_file(GFileEnumerator * enumerator, GCancellable * cancellable, GError ** error)
{
    ...
    if (resource->children == NULL || resource->children[resource->index] == NULL)
        return NULL;
    ...
    info = g_file_query_info(file, resource->attributes, resource->flags, cancellable, error);
...
static gssize g_resource_file_input_stream_read(GInputStream * stream, void *buffer, gsize count, GCancellable * cancellable, GError ** error);
static gssize g_resource_file_input_stream_skip(GInputStream * stream, gsize count, GCancellable * cancellable, GError ** error);
static gboolean g_resource_file_input_stream_close(GInputStream * stream, GCancellable * cancellable, GError ** error);
...
static gboolean g_resource_file_input_stream_seek(GFileInputStream * stream, goffset offset, GSeekType type, GCancellable * cancellable, GError ** error);
static GFileInfo *g_resource_file_input_stream_query_info(GFileInputStream * stream, const char *attributes, GCancellable * cancellable, GError ** error);
...
static void g_resource_file_input_stream_class_init(GResourceFileInputStreamClass * klass)
{
    GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
...
static GFileInputStream *_g_resource_file_input_stream_new(GInputStream * in_stream, GFile * file)
{
...
static gboolean g_resource_file_input_stream_seek(GFileInputStream * stream, goffset offset, GSeekType type, GCancellable * cancellable, GError ** error)
{
    ...
    if (!G_IS_SEEKABLE(file->stream))
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, _("Input stream doesn't implement seek"));
        return FALSE;
    }

    return g_seekable_seek(G_SEEKABLE(file->stream), offset, type, cancellable, error);
}
