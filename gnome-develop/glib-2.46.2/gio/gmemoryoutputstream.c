...
static GSource *g_memory_output_stream_create_source(GPollableOutputStream * stream, GCancellable * cancellable);
...
G_DEFINE_TYPE_WITH_CODE(GMemoryOutputStream, g_memory_output_stream, G_TYPE_OUTPUT_STREAM,
                        G_ADD_PRIVATE(GMemoryOutputStream) G_IMPLEMENT_INTERFACE(G_TYPE_SEEKABLE, g_memory_output_stream_seekable_iface_init); G_IMPLEMENT_INTERFACE(G_TYPE_POLLABLE_OUTPUT_STREAM, g_memory_output_stream_pollable_iface_init))
     static void g_memory_output_stream_class_init(GMemoryOutputStreamClass * klass)
{
   ...
   * Since: 2.24
   **/
    g_object_class_install_property(gobject_class, PROP_DATA, g_param_spec_pointer("data", P_("Data Buffer"), P_("Pointer to buffer where data will be written."), G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 2.24
   **/
    g_object_class_install_property(gobject_class, PROP_SIZE, g_param_spec_ulong("size", P_("Data Buffer Size"), P_("Current size of the data buffer."), 0, G_MAXULONG, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 2.24
   **/
    g_object_class_install_property(gobject_class, PROP_DATA_SIZE, g_param_spec_ulong("data-size", P_("Data Size"), P_("Size of data written to the buffer."), 0, G_MAXULONG, 0, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
   ...
   **/
    g_object_class_install_property(gobject_class,
                                    ...
                                    g_param_spec_pointer("realloc-function",
                                                         P_("Memory Reallocation Function"), P_("Function with realloc semantics called to enlarge the buffer."), G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
   ...
   **/
    g_object_class_install_property(gobject_class,
                                    ...
                                    g_param_spec_pointer("destroy-function",
                                                         P_("Destroy Notification Function"), P_("Function called with the buffer as argument when the stream is destroyed."), G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
}
...
GOutputStream *g_memory_output_stream_new(gpointer data, gsize size, GReallocFunc realloc_function, GDestroyNotify destroy_function)
{
    ...
    stream = g_object_new(G_TYPE_MEMORY_OUTPUT_STREAM, "data", data, "size", size, "realloc-function", realloc_function, "destroy-function", destroy_function, NULL);
...
GBytes *g_memory_output_stream_steal_as_bytes(GMemoryOutputStream * ostream)
{
    ...
    result = g_bytes_new_with_free_func(ostream->priv->data, ostream->priv->valid_len, ostream->priv->destroy, ostream->priv->data);
    ostream->priv->data = NULL;
