...
G_DEFINE_TYPE_WITH_CODE(GCocoaNotificationBackend, g_cocoa_notification_backend, G_TYPE_NOTIFICATION_BACKEND,
                        _g_io_modules_ensure_extension_points_registered(); g_io_extension_point_implement(G_NOTIFICATION_BACKEND_EXTENSION_POINT_NAME, g_define_type_id, "cocoa", 0));

static NSString *nsstring_from_cstr(const char *cstr)
{
    if (!cstr)
        return nil;
...
static NSImage *nsimage_from_gicon(GIcon * icon)
{
...
static void activate_detailed_action(const char *action)
{
    char *name;
    GVariant *target;
...
@interface GNotificationCenterDelegate: NSObject < NSUserNotificationCenterDelegate > @end @ implementation GNotificationCenterDelegate - (void) userNotificationCenter: (NSUserNotificationCenter *) center didActivateNotification:(NSUserNotification *)
    notification
{
...
@end static GNotificationCenterDelegate *cocoa_notification_delegate;

static gboolean g_cocoa_notification_backend_is_supported(void)
{
...
static void add_actions_to_notification(NSUserNotification * userNotification, GNotification * notification)
{
...
static void g_cocoa_notification_backend_send_notification(GNotificationBackend * backend, const gchar * cstr_id, GNotification * notification)
{
...
static void g_cocoa_notification_backend_withdraw_notification(GNotificationBackend * backend, const gchar * cstr_id)
{
...
static void g_cocoa_notification_backend_init(GCocoaNotificationBackend * backend)
{
}

static void g_cocoa_notification_backend_class_init(GCocoaNotificationBackendClass * klass)
{
