...
static int app_help(gboolean requested, const gchar * command)
{
    ...
    if (topic)
    {
        ...
        g_string_append_printf(string, "\n  %s %s %s\n\n", "gapplication", topic->command, topic->synopsis ? _(topic->synopsis) : "");
        ...
        if (topic->synopsis)
        {
            ...
            for (i = 0; i < G_N_ELEMENTS(substvars); i++)
                if (strstr(topic->synopsis, substvars[i].var))
                    g_string_append_printf(string, "  %-*.*s   %s\n", maxwidth, maxwidth, _(substvars[i].var), _(substvars[i].description));
    ...
    else
    {
        ...
        for (i = 0; i < G_N_ELEMENTS(topics); i++)
            g_string_append_printf(string, "  %-*.*s   %s\n", maxwidth, maxwidth, topics[i].command, _(topics[i].summary));
...
static int app_call(const gchar * app_id, const gchar * method_name, GVariant * parameters)
{
    ...
    result = g_dbus_connection_call_sync(session, app_id, object_path, "org.freedesktop.Application", method_name, parameters, G_VARIANT_TYPE_UNIT, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
