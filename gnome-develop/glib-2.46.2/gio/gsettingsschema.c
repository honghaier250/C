...
static void parse_into_text_tables(const gchar * directory, GHashTable * summaries, GHashTable * descriptions)
{
...
static GHashTable **g_settings_schema_source_get_text_tables(GSettingsSchemaSource * source)
{
...
static void ensure_schema_lists(void)
{
    if (g_once_init_enter(&schema_lists_initialised))
    {
        ...
        g_settings_schema_source_list_schemas(schema_sources, TRUE, &non_relocatable_schema_list, &relocatable_schema_list);
...
GVariantIter *g_settings_schema_get_value(GSettingsSchema * schema, const gchar * key)
{
    ...
    if G_UNLIKELY
        (value == NULL || !g_variant_is_of_type(value, G_VARIANT_TYPE_TUPLE)) g_error("Settings schema '%s' does not contain a key named '%s'", schema->id, key);
...
void g_settings_schema_key_init(GSettingsSchemaKey * key, GSettingsSchema * schema, const gchar * name)
{
    GVariantIter *iter;
    GVariant *data;
    guchar code;
...
gboolean g_settings_schema_key_type_check(GSettingsSchemaKey * key, GVariant * value)
{
...
GVariant *g_settings_schema_key_range_fixup(GSettingsSchemaKey * key, GVariant * value)
{
    const gchar *target;
    ...
    target = strinfo_string_from_alias(key->strinfo, key->strinfo_length, g_variant_get_string(value, NULL));
...
GVariant *g_settings_schema_key_get_translated_default(GSettingsSchemaKey * key)
{
    ...
    if (value == NULL)
    {
        g_warning("Failed to parse translated string '%s' for " "key '%s' in schema '%s': %s", translated, key->name, g_settings_schema_get_id(key->schema), error->message);
    ...
    {
        g_warning("Translated default '%s' for key '%s' in schema '%s' " "is outside of valid range", key->unparsed, key->name, g_settings_schema_get_id(key->schema));
        g_variant_unref(value);
        value = NULL;
    }
...
gint g_settings_schema_key_to_enum(GSettingsSchemaKey * key, GVariant * value)
{
    ...
    it_worked = strinfo_enum_from_string(key->strinfo, key->strinfo_length, g_variant_get_string(value, NULL), &result);
...
gboolean g_settings_schema_key_range_check(GSettingsSchemaKey * key, GVariant * value)
{
    ...
    if (key->minimum)
    {
        return g_variant_compare(key->minimum, value) <= 0 && g_variant_compare(value, key->maximum) <= 0;
    }

    return strinfo_is_string_valid(key->strinfo, key->strinfo_length, g_variant_get_string(value, NULL));
}
