...
void g_dbus_error_register_error_domain(const gchar * error_domain_quark_name, volatile gsize * quark_volatile, const GDBusErrorEntry * entries, guint num_entries)
{
    ...
    if (g_once_init_enter(quark_volatile))
    {
        ...
        {
            g_warn_if_fail(g_dbus_error_register_error(quark, entries[n].error_code, entries[n].dbus_error_name));
        }
...
GError *g_dbus_error_new_for_dbus_error(const gchar * dbus_error_name, const gchar * dbus_error_message)
{
    ...
    if (re != NULL)
    {
        error = g_error_new(re->pair.error_domain, re->pair.error_code, "GDBus.Error:%s: %s", dbus_error_name, dbus_error_message);
    }
    else
    {
        ...
        {
            error = g_error_new(G_IO_ERROR, G_IO_ERROR_DBUS_ERROR, "GDBus.Error:%s: %s", dbus_error_name, dbus_error_message);
        }
    }
