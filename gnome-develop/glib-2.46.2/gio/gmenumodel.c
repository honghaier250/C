...
G_DEFINE_TYPE(GMenuLinkHashIter, g_menu_link_hash_iter, G_TYPE_MENU_LINK_ITER)
     static gboolean g_menu_link_hash_iter_get_next(GMenuLinkIter * link_iter, const gchar ** out_name, GMenuModel ** value)
{
...
G_DEFINE_TYPE(GMenuAttributeHashIter, g_menu_attribute_hash_iter, G_TYPE_MENU_ATTRIBUTE_ITER)
     static gboolean g_menu_attribute_hash_iter_get_next(GMenuAttributeIter * attr_iter, const gchar ** name, GVariant ** value)
{
...
static void g_menu_attribute_hash_iter_class_init(GMenuAttributeHashIterClass * class)
{
    GObjectClass *object_class = G_OBJECT_CLASS(class);
...
G_DEFINE_ABSTRACT_TYPE(GMenuModel, g_menu_model, G_TYPE_OBJECT)
     ...
     static GMenuAttributeIter *g_menu_model_real_iterate_item_attributes(GMenuModel * model, gint item_index)
{
    ...
    {
        g_critical("GMenuModel implementation '%s' doesn't override iterate_item_attributes() " "and fails to return sane values from get_item_attributes()", G_OBJECT_TYPE_NAME(model));
        result = NULL;
    }
...
static GVariant *g_menu_model_real_get_item_attribute_value(GMenuModel * model, gint item_index, const gchar * attribute, const GVariantType * expected_type)
{
    GHashTable *table = NULL;
    GVariant *value = NULL;
...
static GMenuLinkIter *g_menu_model_real_iterate_item_links(GMenuModel * model, gint item_index)
{
    GHashTable *table = NULL;
    ...
    {
        g_critical("GMenuModel implementation '%s' doesn't override iterate_item_links() " "and fails to return sane values from get_item_links()", G_OBJECT_TYPE_NAME(model));
        result = NULL;
    }
...
static GMenuModel *g_menu_model_real_get_item_link(GMenuModel * model, gint item_index, const gchar * link)
{
    GHashTable *table = NULL;
...
static void g_menu_model_class_init(GMenuModelClass * class)
{
   ...
   **/
    g_menu_model_items_changed_signal = g_signal_new(I_("items-changed"), G_TYPE_MENU_MODEL, G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_generic, G_TYPE_NONE, 3, G_TYPE_INT, G_TYPE_INT, G_TYPE_INT);
}
...
{
    return G_MENU_MODEL_GET_CLASS(model)->iterate_item_attributes(model, item_index);
}
...
{
    return G_MENU_MODEL_GET_CLASS(model)->get_item_attribute_value(model, item_index, attribute, expected_type);
}
...
{
    return G_MENU_MODEL_GET_CLASS(model)->get_item_link(model, item_index, link);
}
...
gboolean g_menu_attribute_iter_get_next(GMenuAttributeIter * iter, const gchar ** out_name, GVariant ** value)
{
    ...
    iter->priv->valid = G_MENU_ATTRIBUTE_ITER_GET_CLASS(iter)->get_next(iter, &name, &iter->priv->value);
...
gboolean g_menu_link_iter_get_next(GMenuLinkIter * iter, const gchar ** out_link, GMenuModel ** value)
{
    ...
    iter->priv->valid = G_MENU_LINK_ITER_GET_CLASS(iter)->get_next(iter, &name, &iter->priv->value);
