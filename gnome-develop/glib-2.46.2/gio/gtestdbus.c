...
static void g_test_dbus_get_property(GObject * object, guint property_id, GValue * value, GParamSpec * pspec)
{
    GTestDBus *self = (GTestDBus *) object;
...
static void g_test_dbus_set_property(GObject * object, guint property_id, const GValue * value, GParamSpec * pspec)
{
    GTestDBus *self = (GTestDBus *) object;
   ...
   */
    g_object_class_install_property(object_class, PROP_FLAGS,
                                    g_param_spec_flags("flags",
                                                       ...
                                                       P_("Flags specifying the behaviour of the D-Bus session"), G_TYPE_TEST_DBUS_FLAGS, G_TEST_DBUS_NONE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
...
static void start_daemon(GTestDBus * self)
{
...
#endif
                             G_SPAWN_SEARCH_PATH, NULL, NULL, &self->priv->bus_pid, NULL, &self->priv->bus_stdout_fd, NULL, &error);
    g_assert_no_error(error);
    ...
    g_io_channel_read_line(channel, &self->priv->bus_address, NULL, &termpos, &error);
    g_assert_no_error(error);
    ...
    if (g_getenv("G_DBUS_MONITOR") != NULL)
    {
        ...
        command = g_strdup_printf("dbus-monitor --address %s", self->priv->bus_address);
