...
static const gchar *gvdb_table_item_get_key(GvdbTable * file, const struct gvdb_hash_item *item, gsize * size)
{
    ...
    *size = guint16_from_le(item->key_size);
    ...
    if G_UNLIKELY
        (start > end || end > file->size) return NULL;
...
static gconstpointer gvdb_table_dereference(GvdbTable * file, const struct gvdb_pointer *pointer, gint alignment, gsize * size)
{
    ...
    if G_UNLIKELY
        (start > end || end > file->size || start & (alignment - 1)) return NULL;
...
static void gvdb_table_setup_root(GvdbTable * file, const struct gvdb_pointer *pointer)
{
    ...
    if G_UNLIKELY
        (header == NULL || size < sizeof *header) return;
    ...
    if G_UNLIKELY
        (n_bloom_words * sizeof(guint32_le) > size) return;
    ...
    if G_UNLIKELY
        (n_buckets > G_MAXUINT / sizeof(guint32_le) || n_buckets * sizeof(guint32_le) > size) return;
    ...
    if G_UNLIKELY
        (size % sizeof(struct gvdb_hash_item)) return;

    file->hash_items = (gpointer) (file->hash_buckets + n_buckets);
    file->n_hash_items = size / sizeof(struct gvdb_hash_item);
}

static GvdbTable *new_from_data(const void *data, gsize data_len, gboolean trusted, gpointer user_data, GvdbRefFunc ref, GDestroyNotify unref, const char *filename, GError ** error)
{
    GvdbTable *file;
    ...
    if (sizeof(struct gvdb_header) <= file->size)
    {
        ...
        if (header->signature[0] == GVDB_SIGNATURE0 && header->signature[1] == GVDB_SIGNATURE1 && guint32_from_le(header->version) == 0)
        ...
        else
        {
            if (filename)
                ...
                g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_INVAL, "invalid gvdb header");
...
GvdbTable *gvdb_table_new(const gchar * filename, gboolean trusted, GError ** error)
{
    ...
    return new_from_data(g_mapped_file_get_contents(mapped), g_mapped_file_get_length(mapped), trusted, mapped, (GvdbRefFunc) g_mapped_file_ref, (GDestroyNotify) g_mapped_file_unref, filename, error);
}
 ...
 * Returns: a new #GvdbTable
 **/
GvdbTable *gvdb_table_new_from_data(const void *data, gsize data_len, gboolean trusted, gpointer user_data, GvdbRefFunc ref, GDestroyNotify unref, GError ** error)
{
    return new_from_data(data, data_len, trusted, user_data, ref, unref, NULL, error);
}
...
static gboolean gvdb_table_check_name(GvdbTable * file, struct gvdb_hash_item *item, const gchar * key, guint key_length)
{
    ...
    if G_UNLIKELY
        (this_key == NULL || this_size > key_length) return FALSE;

    key_length -= this_size;

    if G_UNLIKELY
        (memcmp(this_key, key + key_length, this_size) != 0) return FALSE;
    ...
    if G_LIKELY
        (parent < file->n_hash_items && this_size > 0) return gvdb_table_check_name(file, &file->hash_items[parent], key, key_length);
...
static const struct gvdb_hash_item *gvdb_table_lookup(GvdbTable * file, const gchar * key, gchar type)
{
    ...
    if G_UNLIKELY
        (file->n_buckets == 0 || file->n_hash_items == 0) return NULL;
    ...
    if (bucket == file->n_buckets - 1 || (lastno = guint32_from_le(file->hash_buckets[bucket + 1])) > file->n_hash_items)
        ...
        (itemno < lastno)
    {
        ...
        if (hash_value == guint32_from_le(item->hash_value))
            if G_LIKELY
                (gvdb_table_check_name(file, item, key, key_length)) if G_LIKELY
...
static const struct gvdb_hash_item *gvdb_table_get_item(GvdbTable * table, guint32_le item_no)
{
    ...
    if G_LIKELY
        (item_no_native < table->n_hash_items) return table->hash_items + item_no_native;
...
static GVariant *gvdb_table_value_from_item(GvdbTable * table, const struct gvdb_hash_item *item)
{
    ...
    variant = g_variant_new_from_data(G_VARIANT_TYPE_VARIANT, data, size, table->trusted, table->unref_user_data, table->ref_user_data ? table->ref_user_data(table->user_data) : table->user_data);
