...
GResource *g_resource_new_from_data(GBytes * data, GError ** error)
{
    ...
    table = gvdb_table_new_from_data(g_bytes_get_data(data, NULL), g_bytes_get_size(data), TRUE, g_bytes_ref(data), (GvdbRefFunc) g_bytes_ref, (GDestroyNotify) g_bytes_unref, error);
...
static gboolean do_lookup(GResource * resource, const gchar * path, GResourceLookupFlags lookup_flags, gsize * size, guint32 * flags, const void **data, gsize * data_size, GError ** error)
{
    char *free_path = NULL;
    gsize path_len;
    gboolean res = FALSE;
    ...
    if (value == NULL)
    {
        g_set_error(error, G_RESOURCE_ERROR, G_RESOURCE_ERROR_NOT_FOUND, _("The resource at '%s' does not exist"), path);
    }
    else
    {
 ...
 * Since: 2.32
 **/
GInputStream *g_resource_open_stream(GResource * resource, const gchar * path, GResourceLookupFlags lookup_flags, GError ** error)
{
    const void *data;
    gsize data_size;
    guint32 flags;
    ...
    g_object_set_data_full(G_OBJECT(stream), "g-resource", g_resource_ref(resource), (GDestroyNotify) g_resource_unref);
 ...
 * Since: 2.32
 **/
GBytes *g_resource_lookup_data(GResource * resource, const gchar * path, GResourceLookupFlags lookup_flags, GError ** error)
{
    const void *data;
    guint32 flags;
    gsize data_size;
    ...
    if (flags & G_RESOURCE_FLAGS_COMPRESSED)
    {
        ...
        do
        {
            res = g_converter_convert(G_CONVERTER(decompressor), s, s_size, d, d_size, G_CONVERTER_INPUT_AT_END, &bytes_read, &bytes_written, NULL);
            if (res == G_CONVERTER_ERROR)
            {
                g_free(uncompressed);
                g_object_unref(decompressor);

                g_set_error(error, G_RESOURCE_ERROR, G_RESOURCE_ERROR_INTERNAL, _("The resource at '%s' failed to decompress"), path);
                return NULL;
 ...
 * Since: 2.32
 **/
gboolean g_resource_get_info(GResource * resource, const gchar * path, GResourceLookupFlags lookup_flags, gsize * size, guint32 * flags, GError ** error)
{
 ...
 * Since: 2.32
 **/
gchar **g_resource_enumerate_children(GResource * resource, const gchar * path, GResourceLookupFlags lookup_flags, GError ** error)
{
    gchar **children;
    gsize path_len;
 ...
 * Since: 2.32
 **/
GInputStream *g_resources_open_stream(const gchar * path, GResourceLookupFlags lookup_flags, GError ** error)
{
    ...
    for (l = registered_resources; l != NULL; l = l->next)
    {
        ...
        if (stream == NULL && g_error_matches(my_error, G_RESOURCE_ERROR, G_RESOURCE_ERROR_NOT_FOUND))
        {
 ...
 * Since: 2.32
 **/
GBytes *g_resources_lookup_data(const gchar * path, GResourceLookupFlags lookup_flags, GError ** error)
{
    GBytes *res = NULL;
    GList *l;
    GBytes *data;
    ...
    for (l = registered_resources; l != NULL; l = l->next)
    {
        ...
        if (data == NULL && g_error_matches(my_error, G_RESOURCE_ERROR, G_RESOURCE_ERROR_NOT_FOUND))
        {
            g_clear_error(&my_error);
        }
        else
 ...
 * Since: 2.32
 **/
gchar **g_resources_enumerate_children(const gchar * path, GResourceLookupFlags lookup_flags, GError ** error)
{
 ...
 * Since: 2.32
 **/
gboolean g_resources_get_info(const gchar * path, GResourceLookupFlags lookup_flags, gsize * size, guint32 * flags, GError ** error)
{
    gboolean res = FALSE;
    GList *l;
    gboolean r_res;
    ...
    for (l = registered_resources; l != NULL; l = l->next)
    {
        ...
        if (!r_res && g_error_matches(my_error, G_RESOURCE_ERROR, G_RESOURCE_ERROR_NOT_FOUND))
        {
            g_clear_error(&my_error);
        }
        else
