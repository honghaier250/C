...
static gsize ik_source_read_some_events(InotifyKernelSource * iks, gchar * buffer, gsize buffer_len)
{
    gssize result;
...
static gchar *ik_source_read_all_the_events(InotifyKernelSource * iks, gchar * buffer, gsize buffer_len, gsize * length_out)
{
    gsize n_read;
...
static gboolean ik_source_dispatch(GSource * source, GSourceFunc func, gpointer user_data)
{
    ...
    if (iks->is_bored || g_source_query_unix_fd(source, iks->fd_tag))
    {
        ...
        while (offset < buffer_len)
        {
            ...
            else if (event->mask & IN_MOVED_FROM)
            {
                ...
                if G_UNLIKELY
                    (!new) g_warning("inotify: got IN_MOVED_FROM event with already-pending cookie %#x", event->cookie);
...
static InotifyKernelSource *ik_source_new(gboolean(*callback) (ik_event_t * event))
{
