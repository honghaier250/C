...
gboolean _ip_startup(gboolean(*cb) (ik_event_t * event, inotify_sub * sub, gboolean file_event))
{
...
static ip_watched_file_t *ip_watched_file_new(const gchar * dirname, const gchar * filename)
{
...
static gboolean ip_event_dispatch(GList * dir_list, GList * file_list, ik_event_t * event)
{
    ...
    for (l = dir_list; l; l = l->next)
    {
        ...
        for (subl = dir->subs; subl; subl = subl->next)
        {
             ...
             */
            if (sub->filename && event->name && strcmp(sub->filename, event->name) && (!event->pair || !event->pair->name || strcmp(sub->filename, event->pair->name)))
                continue;
