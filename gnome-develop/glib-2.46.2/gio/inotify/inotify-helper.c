...
static gboolean ih_event_callback(ik_event_t * event, inotify_sub * sub, gboolean file_event);
...
static char *_ih_fullpath_from_event(ik_event_t * event, const char *dirname, const char *filename)
{
    char *fullpath;
...
static gboolean ih_event_callback(ik_event_t * event, inotify_sub * sub, gboolean file_event)
{
    ...
    if (event->mask & IN_MOVE)
    {
        ...
        {
            /* this is a rename */
            interesting = g_file_monitor_source_handle_event(sub->user_data, G_FILE_MONITOR_EVENT_RENAMED, event->name, event->pair->name, NULL, event->timestamp);
        }
        else
        {
            GFile *other;
            ...
            interesting = g_file_monitor_source_handle_event(sub->user_data, ih_mask_to_EventFlags(event->mask), event->name, NULL, other, event->timestamp);
        ...
        interesting = g_file_monitor_source_handle_event(sub->user_data, ih_mask_to_EventFlags(event->mask), event->name, NULL, NULL, event->timestamp);
    ...
    if (event->mask & IN_CREATE)
    {
        ...
        if (s != 0 || !S_ISREG(buf.st_mode) || buf.st_nlink != 1)
            g_file_monitor_source_handle_event(sub->user_data, G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT, event->name, NULL, NULL, event->timestamp);
    }
...
static void ih_not_missing_callback(inotify_sub * sub)
{
    ...
    g_file_monitor_source_handle_event(sub->user_data, G_FILE_MONITOR_EVENT_CREATED, sub->filename, NULL, NULL, now);
    g_file_monitor_source_handle_event(sub->user_data, G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT, sub->filename, NULL, NULL, now);
}
