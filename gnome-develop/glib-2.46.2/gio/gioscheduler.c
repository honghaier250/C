...
static void io_job_thread(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
...
void g_io_scheduler_cancel_all_jobs(void)
{
    ...
    for (l = active_jobs; l != NULL; l = l->next)
    {
        ...
        if (job->cancellable)
            cancellable_list = g_list_prepend(cancellable_list, g_object_ref(job->cancellable));
    }
    G_UNLOCK(active_jobs);
