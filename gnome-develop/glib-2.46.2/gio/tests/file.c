...
static void written_cb(GObject * source, GAsyncResult * res, gpointer user_data)
{
    ...
    if (data->pos < strlen(data->data))
    {
        g_output_stream_write_async(data->ostream, data->data + data->pos, strlen(data->data) - data->pos, 0, NULL, written_cb, data);
    }
    else
    {
        g_assert(!g_output_stream_is_closed(data->ostream));
...
static void test_create_delete(gconstpointer d)
{
    ...
    data->file = g_file_new_tmp("g_file_create_delete_XXXXXX", &iostream, NULL);
...
static void test_replace_load(void)
{
    ...
    data->file = g_file_new_tmp("g_file_replace_load_XXXXXX", &iostream, NULL);
    g_assert(data->file != NULL);
    g_object_unref(iostream);
...
static void test_replace_cancel(void)
{
    ...
    g_file_replace_contents(file, replace_data, strlen(replace_data), NULL, FALSE, 0, NULL, NULL, &error);
    g_assert_no_error(error);

    ostream = g_file_replace(file, NULL, TRUE, 0, NULL, &error);
    g_assert_no_error(error);

    g_output_stream_write_all(G_OUTPUT_STREAM(ostream), replace_data, strlen(replace_data), &nwrote, NULL, &error);
    g_assert_no_error(error);
...
static void on_file_deleted(GObject * object, GAsyncResult * result, gpointer user_data)
{
...
static void test_async_delete(void)
{
    GFile *file;
    GFileIOStream *iostream;
    GError *local_error = NULL;
    ...
    file = g_file_new_tmp("g_file_delete_XXXXXX", &iostream, error);
    g_assert_no_error(local_error);
    g_object_unref(iostream);
...
#ifdef G_OS_UNIX
static void test_copy_preserve_mode(void)
{
    GFile *tmpfile;
    GFile *dest_tmpfile;
    GFileInfo *dest_info;
    ...
    tmpfile = g_file_new_tmp("tmp-copy-preserve-modeXXXXXX", &iostream, error);
    g_assert_no_error(local_error);
    g_io_stream_close((GIOStream *) iostream, NULL, error);
    g_assert_no_error(local_error);
    g_clear_object(&iostream);

    g_file_set_attribute(tmpfile, G_FILE_ATTRIBUTE_UNIX_MODE, G_FILE_ATTRIBUTE_TYPE_UINT32, &romode, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, NULL, error);
    g_assert_no_error(local_error);

    dest_tmpfile = g_file_new_tmp("tmp-copy-preserve-modeXXXXXX", &iostream, error);
    g_assert_no_error(local_error);
    g_io_stream_close((GIOStream *) iostream, NULL, error);
    g_assert_no_error(local_error);
    g_clear_object(&iostream);

    g_file_copy(tmpfile, dest_tmpfile, G_FILE_COPY_OVERWRITE | G_FILE_COPY_NOFOLLOW_SYMLINKS | G_FILE_COPY_ALL_METADATA, NULL, NULL, NULL, error);
    g_assert_no_error(local_error);

    dest_info = g_file_query_info(dest_tmpfile, G_FILE_ATTRIBUTE_UNIX_MODE, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, NULL, error);
    g_assert_no_error(local_error);
...
static guint64 get_size_from_du(const gchar * path)
{
    GSubprocess *du;
    gchar *result;
    gchar *endptr;
    ...
    du = g_subprocess_new(G_SUBPROCESS_FLAGS_STDOUT_PIPE, &error, "du", "--bytes", "-s", path, NULL);
    g_assert_no_error(error);
...
static void test_measure(void)
{
    ...
    ok = g_file_measure_disk_usage(file, G_FILE_MEASURE_APPARENT_SIZE, NULL, NULL, NULL, &num_bytes, &num_dirs, &num_files, &error);
    g_assert(ok);
    g_assert_no_error(error);
...
static void measure_progress(gboolean reporting, guint64 current_size, guint64 num_dirs, guint64 num_files, gpointer user_data)
{
...
static void measure_done(GObject * source, GAsyncResult * res, gpointer user_data)
{
    MeasureData *data = user_data;
...
static void test_measure_async(void)
{
    gchar *path;
    GFile *file;
    MeasureData *data;
    ...
    g_file_measure_disk_usage_async(file, G_FILE_MEASURE_APPARENT_SIZE, 0, NULL, measure_progress, data, measure_done, data);
}
