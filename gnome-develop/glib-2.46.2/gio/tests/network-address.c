...
static void test_parse_uri(gconstpointer d)
{
...
static void test_parse_host(gconstpointer d)
{
    const ParseTest *test = d;
    GNetworkAddress *address;
    GError *error;
...
static void test_resolve_address(gconstpointer d)
{
 ...
 */
static void test_resolve_address_gresolver(gconstpointer d)
{
    const ResolveTest *test = d;
    GResolver *resolver;
    GList *addrs;
...
static void find_ifname_and_index(void)
{
...
static void test_scope_id(GSocketConnectable * addr)
{
    GSocketAddressEnumerator *addr_enum;
...
static void test_host_scope_id(void)
{
...
static void test_uri_scope_id(void)
{
    GSocketConnectable *addr;
    char *uri;
    GError *error = NULL;
    ...
    uri = g_strdup_printf("http://[%s%%%s]:%d/foo", SCOPE_ID_TEST_ADDR, SCOPE_ID_TEST_IFNAME, SCOPE_ID_TEST_PORT);
    ...
    uri = g_strdup_printf("http://[%s%%25%s]:%d/foo", SCOPE_ID_TEST_ADDR, SCOPE_ID_TEST_IFNAME, SCOPE_ID_TEST_PORT);
    addr = g_network_address_parse_uri(uri, 0, &error);
    g_free(uri);
    g_assert_no_error(error);
...
static void assert_socket_address_matches(GSocketAddress * a, const gchar * expected_address, guint16 expected_port)
{
...
static void test_loopback_sync(void)
{
...
static void got_addr(GObject * source_object, GAsyncResult * result, gpointer user_data)
{
    ...
    else
    {
        ...
        g_socket_address_enumerator_next_async(enumerator, NULL, got_addr, user_data);
    }
}

static void test_loopback_async(void)
{
    GSocketConnectable *addr;    /* owned */
    GSocketAddressEnumerator *enumerator;    /* owned */
    AsyncData data = { 0, };
