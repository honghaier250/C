...
static gboolean append_gv_to_dbus_iter(DBusMessageIter * iter, GVariant * value, GError ** error)
{
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, "Error serializing GVariant with type-string '%s' to a D-Bus message", g_variant_get_type_string(value));
        goto fail;
    }
...
static void replace(char *blob, gsize len, const char *before, const char *after)
{
    gsize i;
...
static void message_serialize_invalid(void)
{
    ...
    for (n = 0; n < 3; n++)
    {
        ...
        switch (n)
        {
            case 0:
                /* invalid UTF-8 */
                dbus_message_append_args(dbus_message, DBUS_TYPE_STRING, &valid_utf8_str, DBUS_TYPE_INVALID);
                break;

            case 1:
                /* invalid object path */
                dbus_message_append_args(dbus_message, DBUS_TYPE_OBJECT_PATH, &valid_object_path, DBUS_TYPE_INVALID);
                break;

            case 2:
                /* invalid signature */
                dbus_message_append_args(dbus_message, DBUS_TYPE_SIGNATURE, &valid_signature, DBUS_TYPE_INVALID);
                break;
...
static void message_parse_empty_arrays_of_arrays(void)
{
    GVariant *body;
    GError *error = NULL;
     ...
     * dict_entry).
     */
    body = g_variant_parse(G_VARIANT_TYPE("(aaax)"), "([@aax [], [], []],)", NULL, NULL, &error);
    g_assert_no_error(error);
    check_serialization(body, "value 0:   array:\n" "    array:\n" "    array:\n" "    array:\n");
    g_variant_unref(body);

    body = g_variant_parse(G_VARIANT_TYPE("(aaa{uu})"), "([@aa{uu} [], [], []],)", NULL, NULL, &error);
    g_assert_no_error(error);
    check_serialization(body, "value 0:   array:\n" "    array:\n" "    array:\n" "    array:\n");
    g_variant_unref(body);
    ...
    body = g_variant_parse(G_VARIANT_TYPE("(a(aa{sv}as))"), "([ ([], [])," "   ([], [])," "   ([], [])],)", NULL, NULL, &error);
    g_assert_no_error(error);
    check_serialization(body, "value 0:   array:\n" "    struct:\n" "      array:\n" "      array:\n" "    struct:\n" "      array:\n" "      array:\n" "    struct:\n" "      array:\n" "      array:\n");
    g_variant_unref(body);
}
...
int main(int argc, char *argv[])
{
    ...
    g_test_add_func("/gdbus/message-parse-empty-arrays-of-arrays", message_parse_empty_arrays_of_arrays);
