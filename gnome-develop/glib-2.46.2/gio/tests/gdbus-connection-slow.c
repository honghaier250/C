...
static void test_connection_flush_signal_handler(GDBusConnection * connection, const gchar * sender_name, const gchar * object_path, const gchar * interface_name, const gchar * signal_name, GVariant * parameters, gpointer user_data)
{
...
static void test_connection_flush(void)
{
    ...
    signal_handler_id = g_dbus_connection_signal_subscribe(connection, NULL,    /* sender */
                                                           "org.gtk.GDBus.FlushInterface", "SomeSignal", "/org/gtk/GDBus/FlushObject", NULL, G_DBUS_SIGNAL_FLAGS_NONE, test_connection_flush_signal_handler, NULL, NULL);
    ...
    for (n = 0; n < 50; n++)
    {
        ...
        error = NULL;
        ret = g_spawn_command_line_sync(flush_helper, NULL,    /* stdout */
                                        NULL,    /* stderr */
                                        &exit_status, &error);
        g_assert_no_error(error);
...
static gboolean large_message_timeout_cb(gpointer data)
{
    (void) data;
...
static void large_message_on_name_appeared(GDBusConnection * connection, const gchar * name, const gchar * name_owner, gpointer user_data)
{
    ...
    error = NULL;
    result = g_dbus_connection_call_sync(connection, "com.example.TestService",    /* bus name */
...
static void test_connection_large_message(void)
{
    ...
    timeout_id = g_timeout_add_seconds(LARGE_MESSAGE_TIMEOUT_SECONDS, large_message_timeout_cb, NULL);

    watcher_id = g_bus_watch_name(G_BUS_TYPE_SESSION, "com.example.TestService", G_BUS_NAME_WATCHER_FLAGS_NONE, large_message_on_name_appeared, large_message_on_name_vanished, GUINT_TO_POINTER(timeout_id),    /* user_data */
                                  NULL);    /* GDestroyNotify */
    g_main_loop_run(loop);
