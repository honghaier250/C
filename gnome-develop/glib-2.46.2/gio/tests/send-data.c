...
static void socket_client_event(GSocketClient * client, GSocketClientEvent event, GSocketConnectable * connectable, GSocketConnection * connection)
{
    ...
    printf("% 12ld.%06ld GSocketClient => %s [%s]\n", tv.tv_sec, tv.tv_usec, g_enum_get_value(event_class, event)->value_nick, connection ? G_OBJECT_TYPE_NAME(connection) : "");
}
...
int main(int argc, char *argv[])
{
    ...
    {
        connection = g_socket_client_connect_to_host(client, argv[1], 7777, cancellable, &error);
    }
    if (connection == NULL)
    {
