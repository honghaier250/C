...
static void check_property(const char *output, GParamSpec * pspec, GValue * value)
{
    ...
    msg = g_strdup_printf("%s %s.%s: %s != %s\n", output, g_type_name(pspec->owner_type), pspec->name, dv, v);
...
static void test_type(gconstpointer data)
{
    GObjectClass *klass;
    GObject *instance;
    GParamSpec **pspecs;
    ...
    if (g_type_is_a(type, G_TYPE_BINDING) ||
        ...
        g_type_is_a(type, G_TYPE_PROPERTY_ACTION) || g_type_is_a(type, G_TYPE_SETTINGS) || g_type_is_a(type, G_TYPE_SOCKET_CONNECTION) || g_type_is_a(type, G_TYPE_SIMPLE_IO_STREAM) || g_type_is_a(type, G_TYPE_THEMED_ICON) || FALSE)
    {
        g_test_skip("mandatory construct params");
        return;
    }

    if (g_type_is_a(type, G_TYPE_DBUS_MENU_MODEL) || g_type_is_a(type, G_TYPE_DBUS_METHOD_INVOCATION))
    {
        g_test_skip("crash in finalize");
        return;
    }

    if (g_type_is_a(type, G_TYPE_FILE_ENUMERATOR) || g_type_is_a(type, G_TYPE_FILE_IO_STREAM))
    {
    ...
    if (G_IS_INITABLE(instance) && !g_initable_init(G_INITABLE(instance), NULL, NULL))
    {
    ...
    for (i = 0; i < n_pspecs; ++i)
    {
        ...
        if (g_type_is_a(type, G_TYPE_APPLICATION) && (strcmp(pspec->name, "is-remote") == 0))
        {
            g_test_message("skipping GApplication:is-remote");
            continue;
        }

        if (g_type_is_a(type, G_TYPE_PROXY_ADDRESS_ENUMERATOR) && (strcmp(pspec->name, "proxy-resolver") == 0))
        {
            g_test_message("skipping GProxyAddressEnumerator:proxy-resolver");
            continue;
        }

        if (g_type_is_a(type, G_TYPE_SOCKET_CLIENT) && (strcmp(pspec->name, "proxy-resolver") == 0))
        {
...
static const GType *list_all_types(void)
{
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; otypes[i]; i++)
    {
        ...
        testname = g_strdup_printf("/Default Values/%s", g_type_name(otypes[i]));
