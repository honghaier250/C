...
static void test_basic(void)
{
    ...
    if (g_test_undefined())
    {
        g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, "*assertion*g_variant_is_of_type*failed*");
    ...
    if (g_test_undefined())
    {
        g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, "*assertion*!= NULL*failed*");
        g_action_activate(G_ACTION(action), NULL);
        g_test_assert_expected_messages();
    }
...
static gboolean strv_strv_cmp(gchar ** a, gchar ** b)
{
    guint n;
...
G_GNUC_BEGIN_IGNORE_DEPRECATIONS static void test_simple_group(void)
{
...
G_GNUC_END_IGNORE_DEPRECATIONS static void test_stateful(void)
{
    GSimpleAction *action;
    GVariant *state;
...
static void test_default_activate(void)
{
    GSimpleAction *action;
    GVariant *state;
...
static void activate_foo(GSimpleAction * simple, GVariant * parameter, gpointer user_data)
{
...
static void activate_bar(GSimpleAction * simple, GVariant * parameter, gpointer user_data)
{
    g_assert(user_data == GINT_TO_POINTER(123));
...
static void change_volume_state(GSimpleAction * action, GVariant * value, gpointer user_data)
{
    gint requested;
...
G_GNUC_BEGIN_IGNORE_DEPRECATIONS static void test_entries(void)
{
    ...
    g_simple_action_group_add_entries(actions, entries, G_N_ELEMENTS(entries), GINT_TO_POINTER(123));
    ...
    g_action_group_activate_action(G_ACTION_GROUP(actions), "bar", g_variant_new_string("param"));
    g_assert(bar_activated);
    g_assert(!foo_activated);
    ...
    if (g_test_undefined())
    {
        ...
        g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, "*not a valid GVariant type string*");
        g_simple_action_group_add_entries(actions, &bad_type, 1, NULL);
        g_test_assert_expected_messages();

        g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, "*could not parse*");
    ...
    /* should change */
    g_action_group_change_action_state(G_ACTION_GROUP(actions), "volume", g_variant_new_int32(7));
    state = g_action_group_get_action_state(G_ACTION_GROUP(actions), "volume");
    g_assert_cmpint(g_variant_get_int32(state), ==, 7);
    g_variant_unref(state);

    /* should not change */
    g_action_group_change_action_state(G_ACTION_GROUP(actions), "volume", g_variant_new_int32(11));
    state = g_action_group_get_action_state(G_ACTION_GROUP(actions), "volume");
    g_assert_cmpint(g_variant_get_int32(state), ==, 7);
    g_variant_unref(state);
...
G_GNUC_END_IGNORE_DEPRECATIONS static void test_parse_detailed(void)
{
    struct
    {
    ...
    for (i = 0; i < G_N_ELEMENTS(testcases); i++)
    {
        ...
        if (success && testcases[i].expected_error)
            g_error("Unexpected success on '%s'.  Expected error containing '%s'", testcases[i].detailed, testcases[i].expected_error);
        ...
        if (!success)
        {
            if (!strstr(error->message, testcases[i].expected_error))
                g_error("Failure message '%s' for string '%s' did not contained expected substring '%s'", error->message, testcases[i].detailed, testcases[i].expected_error);
...
static void count_activation(const gchar * action)
{
    gint count;
...
static gint activation_count(const gchar * action)
{
    if (activation_counts == NULL)
        return 0;
...
static gboolean compare_action_groups(GActionGroup * a, GActionGroup * b)
{
    gchar **alist;
    gchar **blist;
    gint i;
...
static void list_cb(GObject * source, GAsyncResult * res, gpointer user_data)
{
...
static gboolean call_list(gpointer user_data)
{
    GDBusConnection *bus;

    bus = g_bus_get_sync(G_BUS_TYPE_SESSION, NULL, NULL);
    g_dbus_connection_call(bus, g_dbus_connection_get_unique_name(bus), "/", "org.gtk.Actions", "List", NULL, NULL, 0, G_MAXINT, NULL, list_cb, user_data);
    g_object_unref(bus);

    return G_SOURCE_REMOVE;
}

static void describe_cb(GObject * source, GAsyncResult * res, gpointer user_data)
{
    GDBusConnection *bus = G_DBUS_CONNECTION(source);
    GMainLoop *loop = user_data;
    GError *error = NULL;
...
static gboolean call_describe(gpointer user_data)
{
    GDBusConnection *bus;

    bus = g_bus_get_sync(G_BUS_TYPE_SESSION, NULL, NULL);
    g_dbus_connection_call(bus, g_dbus_connection_get_unique_name(bus), "/", "org.gtk.Actions", "Describe", g_variant_new("(s)", "copy"), NULL, 0, G_MAXINT, NULL, describe_cb, user_data);
    g_object_unref(bus);

    return G_SOURCE_REMOVE;
}

G_GNUC_BEGIN_IGNORE_DEPRECATIONS static void test_dbus_export(void)
{
    GDBusConnection *bus;
    GSimpleActionGroup *group;
    ...
    group = g_simple_action_group_new();
    g_simple_action_group_add_entries(group, exported_entries, G_N_ELEMENTS(exported_entries), NULL);
...
static gpointer do_export(gpointer data)
{
    ...
    for (i = 0; i < 100000; i++)
    {
        ...
        g_simple_action_set_enabled(G_SIMPLE_ACTION(action), !g_action_get_enabled(action));
...
static void test_dbus_threaded(void)
{
...
G_GNUC_END_IGNORE_DEPRECATIONS static void test_bug679509(void)
{
    GDBusConnection *bus;
    GDBusActionGroup *proxy;
    GMainLoop *loop;
...
static void state_changed(GActionGroup * group, const gchar * action_name, GVariant * value, gpointer user_data)
{
    GString *string;
...
static void verify_changed(const gchar * log_entry)
{
    g_assert_cmpstr(state_change_log, ==, log_entry);
    g_clear_pointer(&state_change_log, g_free);
}

static void ensure_state(GSimpleActionGroup * group, const gchar * action_name, const gchar * expected)
{
    GVariant *value;
    gchar *printed;
...
static void test_property_actions(void)
{
    GSimpleActionGroup *group;
    ...
    /* inverted */
    action = g_object_new(G_TYPE_PROPERTY_ACTION, "name", "disable-proxy", "object", client, "property-name", "enable-proxy", "invert-boolean", TRUE, NULL);
    g_action_map_add_action(G_ACTION_MAP(group), G_ACTION(action));
    g_object_unref(action);
