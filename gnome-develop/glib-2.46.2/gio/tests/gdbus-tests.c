...
gboolean _g_assert_property_notify_run(gpointer object, const gchar * property_name)
{
    ...
    g_free(s);
    timeout_id = g_timeout_add_seconds(30, on_property_notify_timeout, &data);
...
static gboolean _give_up(gpointer data)
{
...
void ensure_gdbus_testserver_up(void)
{
    ...
    id = g_timeout_add_seconds(60, _give_up, "waited more than ~ 60s for gdbus-testserver to take its bus name");
...
gboolean _g_assert_signal_received_run(gpointer object, const gchar * signal_name)
{
    ...
    timeout_id = g_timeout_add_seconds(30, on_signal_received_timeout, &data);
    g_main_loop_run(data.loop);
    g_signal_handler_disconnect(object, handler_id);
    g_source_remove(timeout_id);
    g_main_loop_unref(data.loop);
...
GDBusConnection *_g_bus_get_priv(GBusType bus_type, GCancellable * cancellable, GError ** error)
{
    ...
    ret = g_dbus_connection_new_for_address_sync(address, G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_CLIENT | G_DBUS_CONNECTION_FLAGS_MESSAGE_BUS_CONNECTION, NULL,    /* GDBusAuthObserver */
                                                 cancellable, error);
    g_free(address);
