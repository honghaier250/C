...
static gboolean server_on_allow_mechanism(GDBusAuthObserver * observer, const gchar * mechanism, gpointer user_data)
{
...
static GDBusServer *server_new_for_mechanism(const gchar * allowed_mechanism)
{
    gchar *addr;
    gchar *guid;
    GDBusServer *server;
    ...
    error = NULL;
    server = g_dbus_server_new_sync(addr, flags, guid, auth_observer, NULL,    /* cancellable */
                                    &error);
    g_assert_no_error(error);
    g_assert(server != NULL);

    g_signal_connect(auth_observer, "allow-mechanism", G_CALLBACK(server_on_allow_mechanism), (gpointer) allowed_mechanism);
...
static gboolean test_auth_on_new_connection(GDBusServer * server, GDBusConnection * connection, gpointer user_data)
{
...
static gboolean test_auth_on_timeout(gpointer user_data)
{
...
static gpointer test_auth_client_thread_func(gpointer user_data)
{
    ...
    g_signal_connect(auth_observer, "allow-mechanism", G_CALLBACK(server_on_allow_mechanism), (gpointer) data->allowed_client_mechanism);

    c = g_dbus_connection_new_for_address_sync(data->address, G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_CLIENT, auth_observer, NULL,    /* GCancellable */
                                               &error);
    g_assert_no_error(error);
    g_assert(c != NULL);
    g_clear_object(&c);
...
static void test_auth_mechanism(const gchar * allowed_client_mechanism, const gchar * allowed_server_mechanism)
{
    GDBusServer *server;
    GMainLoop *loop;
    ...
    g_signal_connect(server, "new-connection", G_CALLBACK(test_auth_on_new_connection), loop);
    ...
    client_thread = g_thread_new("gdbus-client-thread", test_auth_client_thread_func, &data);
...
static void auth_client_external(void)
{
    test_auth_mechanism("EXTERNAL", NULL);
}

static void auth_client_dbus_cookie_sha1(void)
{
    test_auth_mechanism("DBUS_COOKIE_SHA1", NULL);
}

static void auth_server_anonymous(void)
{
    test_auth_mechanism(NULL, "ANONYMOUS");
}

static void auth_server_external(void)
{
    test_auth_mechanism(NULL, "EXTERNAL");
}

static void auth_server_dbus_cookie_sha1(void)
{
...
static void temp_dbus_keyrings_setup(void)
{
    GError *error = NULL;
...
static void temp_dbus_keyrings_teardown(void)
{
    GDir *dir;
    GError *error = NULL;
    const gchar *name;
