...
static gboolean idle_quit_loop(gpointer user_data)
{
    g_main_loop_quit(loop);
    return FALSE;
}

static void completed_cb(GObject * gobject, GParamSpec * pspec, gpointer user_data)
{
    gboolean *notification_emitted = user_data;
    *notification_emitted = TRUE;
}

static void wait_for_completed_notification(GTask * task)
{
...
static void basic_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
...
static gboolean basic_return(gpointer user_data)
{
...
static void basic_destroy_notify(gpointer user_data)
{
...
static void error_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
    gssize *result_out = user_data;
    GError *error = NULL;
...
static gboolean error_return(gpointer user_data)
{
    GTask *task = user_data;
...
static void error_destroy_notify(gpointer user_data)
{
    gboolean *destroyed = user_data;
...
static void same_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
...
static gboolean same_start(gpointer user_data)
{
    ...
    *weak_pointer = task;
    ...
    g_signal_connect(task, "notify::completed", (GCallback) completed_cb, &same_notification_emitted);
...
static void test_return_from_same_iteration(void)
{
...
static void toplevel_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
    gboolean *result_out = user_data;
    GError *error = NULL;
...
static void test_return_from_toplevel(void)
{
    GTask *task;
    ...
    g_object_add_weak_pointer(G_OBJECT(task), (gpointer *) & task);
    g_signal_connect(task, "notify::completed", (GCallback) completed_cb, &toplevel_notification_emitted);
...
static void anon_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
    gssize *result_out = user_data;
    GError *error = NULL;
...
static gpointer anon_thread_func(gpointer user_data)
{
    GTask *task = user_data;
...
static gboolean anon_start(gpointer user_data)
{
    GTask *task = user_data;

    anon_thread = g_thread_new("test_return_from_anon_thread", anon_thread_func, task);
    return FALSE;
}

static void test_return_from_anon_thread(void)
{
    GTask *task;
    gssize result = 0;
    ...
    g_object_add_weak_pointer(G_OBJECT(task), (gpointer *) & task);
    g_signal_connect(task, "notify::completed", (GCallback) completed_cb, &anon_thread_notification_emitted);
...
static void wrong_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
    gssize *result_out = user_data;
    GError *error = NULL;
...
static gpointer wrong_thread_func(gpointer user_data)
{
    GTask *task = user_data;
...
static gboolean wrong_start(gpointer user_data)
{
    GTask *task = user_data;

    wrong_thread = g_thread_new("test_return_from_anon_thread", wrong_thread_func, task);
    return FALSE;
}

static void test_return_from_wrong_thread(void)
{
    GTask *task;
    gssize result = 0;
    ...
    g_object_add_weak_pointer(G_OBJECT(task), (gpointer *) & task);
    g_signal_connect(task, "notify::completed", (GCallback) completed_cb, &wrong_thread_notification_emitted);
...
static void report_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
    gpointer *weak_pointer = user_data;
    GError *error = NULL;
    gssize ret;
    ...
    *weak_pointer = result;
    ...
    g_signal_connect(result, "notify::completed", (GCallback) completed_cb, &error_notification_emitted);
...
static void test_report_error(void)
{
    ...
    g_task_report_new_error(NULL, report_callback, &weak_pointer, test_report_error, G_IO_ERROR, G_IO_ERROR_FAILED, "Failed");
    g_main_loop_run(loop);
...
static void priority_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
...
static void cancel_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
    ...
    {
        g_assert(!g_cancellable_is_cancelled(cancellable) || !g_task_get_check_cancellable(task));
    }
    else
    {
        g_assert(g_cancellable_is_cancelled(cancellable) && g_task_get_check_cancellable(task));
...
static void test_check_cancellable(void)
{
    GTask *task;
    GCancellable *cancellable;
    int state;
    ...
    {
        task = g_task_new(NULL, cancellable, cancel_callback, GINT_TO_POINTER(state));
...
static void return_if_cancelled_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
    GError *error = NULL;
...
static void test_return_if_cancelled(void)
{
    GTask *task;
    GCancellable *cancellable;
    gboolean cancelled;
...
static void task_weak_notify(gpointer user_data, GObject * ex_task)
{
...
static void run_in_thread_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
...
static void run_in_thread_thread(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
...
static void test_run_in_thread(void)
{
    GTask *task;
...
static void run_in_thread_sync_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
    /* g_task_run_in_thread_sync() does not invoke the task's callback */
    g_assert_not_reached();
}

static void run_in_thread_sync_thread(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
    gboolean *thread_ran = task_data;
...
static void test_run_in_thread_sync(void)
{
    GTask *task;
...
static void quit_main_loop_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
    GError *error = NULL;
    gboolean ret;
...
static void set_sequence_number_thread(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
...
static void fake_task_thread(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
...
static void fake_task_callback(GObject * source, GAsyncResult * result, gpointer user_data)
{
    if (--fake_tasks_running == 0)
        g_main_loop_quit(loop);
}

static void clog_up_thread_pool(void)
{
    GTask *task;
    int i;
...
static void unclog_thread_pool(void)
{
    g_mutex_unlock(&fake_task_mutex);
    g_main_loop_run(loop);
}

static void test_run_in_thread_priority(void)
{
    GTask *task;
    GCancellable *cancellable;
...
static void run_nested_task_thread(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
    GTask *nested;
...
static void test_run_in_thread_nested(void)
{
    GTask *task;
...
static void run_overflow_task_thread(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
...
static void test_run_in_thread_overflow(void)
{
    GCancellable *cancellable;
    GTask *task;
...
static void return_on_cancel_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
...
static void return_on_cancel_thread(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
    ...
    *state = THREAD_RUNNING;
    ...
    if (!g_task_get_return_on_cancel(task) || g_task_set_return_on_cancel(task, FALSE))
    {
...
static void test_return_on_cancel(void)
{
    GTask *task;
    GCancellable *cancellable;
...
static gpointer cancel_sync_runner_thread(gpointer task)
{
    g_task_run_in_thread_sync(task, return_on_cancel_thread);
    return NULL;
}

static void test_return_on_cancel_sync(void)
{
    GTask *task;
    GCancellable *cancellable;
    volatile ThreadState thread_state;
    ...
    g_mutex_lock(&roc_init_mutex);
    g_mutex_lock(&roc_finish_mutex);
    runner_thread = g_thread_new("return-on-cancel-sync runner thread", cancel_sync_runner_thread, task);
...
static void return_on_cancel_atomic_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
    gboolean *callback_ran = user_data;
    GError *error = NULL;
    gssize ret;
...
static void return_on_cancel_atomic_thread(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
...
static void test_return_on_cancel_atomic(void)
{
    GTask *task;
    GCancellable *cancellable;
    volatile gint state;
...
static void test_return_pointer(void)
{
    GObject *object, *ret;
    GTask *task;
    GCancellable *cancellable;
...
static void keepalive_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
    gssize *result_out = user_data;
    GError *error = NULL;
...
static void test_object_keepalive(void)
{
    GObject *object;
    GTask *task;
    gssize result;
...
static void legacy_error_callback(GObject * object, GAsyncResult * result, gpointer user_data)
{
    gssize *result_out = user_data;
    GError *error = NULL;
...
static gboolean legacy_error_return(gpointer user_data)
{
    ...
    else
    {
        ...
        G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
        g_simple_async_result_set_error(simple, G_IO_ERROR, G_IO_ERROR_FAILED, "Failed");
...
static void test_legacy_error(void)
{
    ...
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
    simple = g_simple_async_result_new(NULL, legacy_error_callback, &result, test_legacy_error);
    G_GNUC_END_IGNORE_DEPRECATIONS;
