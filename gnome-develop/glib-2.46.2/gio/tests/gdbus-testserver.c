...
static const gchar introspection_xml[] =
    ...
    "    <property type='ao' name='ao' access='readwrite' />" "    <property type='s' name='foo' access='readwrite' />" "    <property type='s' name='PropertyThatWillBeInvalidated' access='readwrite' />" "  </interface>" "</node>";

static gboolean end_sleep(gpointer data)
{
...
static void handle_method_call(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * method_name, GVariant * parameters, GDBusMethodInvocation * invocation, gpointer user_data)
{
    if (g_strcmp0(method_name, "HelloWorld") == 0)
    {
        ...
        {
            g_dbus_method_invocation_return_dbus_error(invocation, "com.example.TestException", "Yo is not a proper greeting");
        }
        else
        {
            gchar *response;
    ...
    else if (g_strcmp0(method_name, "DoubleHelloWorld") == 0)
    {
        ...
        g_dbus_method_invocation_return_value(invocation, g_variant_new("(ss)", reply1, reply2));
        g_free(reply1);
        g_free(reply2);
    }
    else if (g_strcmp0(method_name, "PairReturn") == 0)
    {
        g_dbus_method_invocation_return_value(invocation, g_variant_new("(su)", "foo", 42));
    }
    else if (g_strcmp0(method_name, "TestPrimitiveTypes") == 0)
    {
        ...
        g_variant_get(parameters, "(ybnqiuxtd&s&o&g)", &val_byte, &val_boolean, &val_int16, &val_uint16, &val_int32, &val_uint32, &val_int64, &val_uint64, &val_double, &val_string, &val_objpath, &val_signature);
        ...
        g_dbus_method_invocation_return_value(invocation,
                                              g_variant_new("(ybnqiuxtdsog)",
                                                            val_byte + 1, !val_boolean, val_int16 + 1, val_uint16 + 1, val_int32 + 1, val_uint32 + 1, val_int64 + 1, val_uint64 + 1, -val_double + 0.123, ret_string, ret_objpath, ret_signature));
    ...
    else if (g_strcmp0(method_name, "TestStructureTypes") == 0)
    {
        ...
        g_variant_get(parameters, "((ii)(&s(ii)aya{ss}))", &x, &y, &desc, &x1, &y1, &iter1, &iter2);
        ...
        g_dbus_method_invocation_return_value(invocation, g_variant_new("((ii)(&s(ii)aya{ss}))", x + 1, y + 1, desc_ret, x1 + 2, y1 + 2, &ret1, &ret2));
    ...
    else if (g_strcmp0(method_name, "FrobSetProperty") == 0)
    {
        ...
        g_dbus_connection_emit_signal(connection, NULL, "/com/example/TestObject", "org.freedesktop.DBus.Properties", "PropertiesChanged", g_variant_new_parsed("('com.example.Frob', [{%s, %v}], @as [])", name, value), NULL);
        g_dbus_method_invocation_return_value(invocation, NULL);
    }
    else if (g_strcmp0(method_name, "FrobInvalidateProperty") == 0)
    {
        ...
        g_dbus_connection_emit_signal(connection, NULL, "/com/example/TestObject", "org.freedesktop.DBus.Properties", "PropertiesChanged", g_variant_new_parsed("('com.example.Frob', @a{sv} [], ['PropertyThatWillBeInvalidated'])"), NULL);
        g_dbus_method_invocation_return_value(invocation, NULL);
    }
    else if (g_strcmp0(method_name, "EmitSignal") == 0)
    {
        ...
        g_dbus_connection_emit_signal(connection, NULL, "/com/example/TestObject", "com.example.Frob", "TestSignal", g_variant_new_parsed("(%s, %o, <'a variant'>)", str_ret, path_ret), NULL);
        g_free(str_ret);
        g_free(path_ret);
        g_dbus_method_invocation_return_value(invocation, NULL);
    }
    ...
    {
        g_dbus_connection_emit_signal(connection, NULL, "/com/example/TestObject", "com.example.Frob", "TestSignal2", g_variant_new_parsed("(42, )"), NULL);
        g_dbus_method_invocation_return_value(invocation, NULL);
    }
...
static GVariant *handle_get_property(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * property_name, GError ** error, gpointer user_data)
{
    ...
    {
        g_set_error(error, G_DBUS_ERROR, G_DBUS_ERROR_FAILED, "no such property: %s", property_name);
    }
...
{
    g_set_error(error, G_DBUS_ERROR, G_DBUS_ERROR_FAILED, "SetProperty not implemented");
    return FALSE;
}
...
static void on_bus_acquired(GDBusConnection * connection, const gchar * name, gpointer user_data)
{
    ...
    id = g_dbus_connection_register_object(connection, "/com/example/TestObject", introspection_data->interfaces[0], &interface_vtable, NULL, NULL, NULL);
    g_assert(id > 0);
}
...
int main(int argc, char *argv[])
{
    ...
    owner_id = g_bus_own_name(G_BUS_TYPE_SESSION, "com.example.TestService", G_BUS_NAME_OWNER_FLAGS_NONE, on_bus_acquired, on_name_acquired, on_name_lost, NULL, NULL);
