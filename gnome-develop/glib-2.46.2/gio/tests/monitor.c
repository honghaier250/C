...
static gboolean create_file_idle(gpointer data)
{
    MonitorData *d = data;
    GError *error = NULL;
...
static gboolean write_file_idle(gpointer data)
{
    MonitorData *d = data;
    GError *error = NULL;
...
static gboolean delete_file_idle(gpointer data)
{
    MonitorData *d = data;
    GError *error = NULL;
...
static void changed_cb(GFileMonitor * monitor, GFile * file, GFile * other_file, GFileMonitorEvent event, gpointer data)
{
    MonitorData *d = data;
    ...
    switch (d->state)
    {
        case 1:
            ...
            break;
        case 3:
            g_assert(event == G_FILE_MONITOR_EVENT_CHANGED || event == G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT);
...
static void file_changed_cb(GFileMonitor * monitor, GFile * file, GFile * other_file, GFileMonitorEvent event, gpointer data)
{
    gint *state = data;
...
static void test_directory_monitor(void)
{
    gchar *path;
    GFile *file;
    GFile *child;
