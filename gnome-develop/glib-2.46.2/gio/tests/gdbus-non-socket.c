...
static void pokee_method_call(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * method_name, GVariant * parameters, GDBusMethodInvocation * invocation, gpointer user_data)
{
    const gchar *str;
    gchar *ret;
...
static void test_non_socket(void)
{
    ...
    switch ((first_child = fork()))
    {
        ...
        case 0:
             ...
             */
            connection = g_dbus_connection_new_sync(streams[0], guid, G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_SERVER | G_DBUS_CONNECTION_FLAGS_DELAY_MESSAGE_PROCESSING, NULL,    /* GDBusAuthObserver */
                                                    NULL, &error);
            g_free(guid);
            g_assert_no_error(error);
            ...
            error = NULL;
            g_dbus_connection_register_object(connection, "/pokee", (GDBusInterfaceInfo *) & pokee_object_info, &pokee_vtable, NULL,    /* user_data */
    ...
    connection = g_dbus_connection_new_sync(streams[1], NULL,    /* guid */
                                            G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_CLIENT, NULL,    /* GDBusAuthObserver */
                                            NULL, &error);
    g_assert_no_error(error);
    g_object_unref(streams[1]);
    ...
    error = NULL;
    ret = g_dbus_connection_call_sync(connection, NULL,    /* name */
                                      "/pokee", "org.gtk.GDBus.Pokee", "Poke", g_variant_new("(s)", "I am the POKER!"), G_VARIANT_TYPE("(s)"),    /* return type */
                                      G_DBUS_CALL_FLAGS_NONE, -1, NULL,    /* cancellable */
                                      &error);
    g_assert_no_error(error);
