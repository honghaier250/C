...
static void active_notify_cb(GSocketService * service, GParamSpec * pspec, gpointer data)
{
...
static void connected_cb(GObject * client, GAsyncResult * result, gpointer user_data)
{
...
static void test_start_stop(void)
{
    ...
    g_socket_listener_add_address(G_SOCKET_LISTENER(service), saddr, G_SOCKET_TYPE_STREAM, G_SOCKET_PROTOCOL_TCP, NULL, &listening_addr, &error);
    g_assert_no_error(error);
    g_object_unref(saddr);

    client = g_socket_client_new();
    g_socket_client_connect_async(client, G_SOCKET_CONNECTABLE(listening_addr), NULL, connected_cb, service);
...
static void test_threaded_socket_service_init(TestThreadedSocketService * service)
{
}

static void test_threaded_socket_service_finalize(GObject * object)
{
...
static void test_threaded_socket_service_class_init(TestThreadedSocketServiceClass * klass)
{
...
static gboolean connection_cb(GThreadedSocketService * service, GSocketConnection * connection, GObject * source_object, gpointer user_data)
{
...
static void client_connected_cb(GObject * client, GAsyncResult * result, gpointer user_data)
{
...
static void test_threaded_712570(void)
{
    GSocketService *service;
    ...
    g_socket_listener_add_address(G_SOCKET_LISTENER(service), addr, G_SOCKET_TYPE_STREAM, G_SOCKET_PROTOCOL_TCP, NULL, &listening_addr, &error);
    g_assert_no_error(error);
    g_object_unref(addr);
    ...
    client = g_socket_client_new();
    g_socket_client_connect_async(client, G_SOCKET_CONNECTABLE(listening_addr), NULL, client_connected_cb, loop);
    g_object_unref(client);
    g_object_unref(listening_addr);
