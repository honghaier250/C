...
static void print_result(const gchar * filename, guint64 disk_usage, guint64 num_dirs, guint64 num_files, GError * error, gchar nl)
{
    if (!error)
    {
        if (option_format_size)
        {
            ...
            g_print("%s: %s (%" G_GUINT64_FORMAT " dirs, %" G_GUINT64_FORMAT " files)%c", filename, str, num_dirs, num_files, nl);
            g_free(str);
        }
        else
            g_print("%s: %" G_GUINT64_FORMAT " (%" G_GUINT64_FORMAT " dirs, %" G_GUINT64_FORMAT " files)%c", filename, disk_usage, num_dirs, num_files, nl);
    }
    else
    {
...
static void async_ready_func(GObject * source, GAsyncResult * result, gpointer user_data)
{
...
static void report_progress(gboolean reporting, guint64 disk_usage, guint64 num_dirs, guint64 num_files, gpointer user_data)
{
...
int main(int argc, char **argv)
{
    ...
    while (argv[i])
    {
        ...
        if (option_use_async)
        {
            g_file_measure_disk_usage_async(file, flags, G_PRIORITY_DEFAULT, NULL, progress, argv[i], async_ready_func, argv[i]);
            outstanding_asyncs++;
        }
        else
        {
            ...
            g_file_measure_disk_usage(file, flags, NULL, progress, argv[i], &disk_usage, &num_dirs, &num_files, &error);
