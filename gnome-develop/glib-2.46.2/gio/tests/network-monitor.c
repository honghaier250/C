...
static void notify_handler(GObject * object, GParamSpec * pspec, gpointer user_data)
{
...
static void network_changed_handler(GNetworkMonitor * monitor, gboolean available, gpointer user_data)
{
    gboolean *emitted = user_data;
...
static void assert_signals(GNetworkMonitor * monitor, gboolean should_emit_notify, gboolean should_emit_network_changed, gboolean expected_network_available)
{
    gboolean emitted_notify = FALSE, emitted_network_changed = FALSE;
    guint h1, h2;

    h1 = g_signal_connect(monitor, "notify::network-available", G_CALLBACK(notify_handler), &emitted_notify);
    h2 = g_signal_connect(monitor, "network-changed", G_CALLBACK(network_changed_handler), &emitted_network_changed);
...
static void reach_cb(GObject * source, GAsyncResult * res, gpointer user_data)
{
    GError *error = NULL;
    gboolean reachable;
...
static gboolean test_reach_async(gpointer user_data)
{
    CanReachData *data = user_data;

    g_network_monitor_can_reach_async(data->monitor, G_SOCKET_CONNECTABLE(data->sockaddr), NULL, reach_cb, data);
    return G_SOURCE_REMOVE;
}

static void run_tests(GNetworkMonitor * monitor, TestAddress * addresses, gboolean should_be_reachable)
{
    GError *error = NULL;
    int i;
    gboolean reachable;
    ...
    for (i = 0; addresses[i].address; i++)
    {
        ...
        reachable = g_network_monitor_can_reach(monitor, G_SOCKET_CONNECTABLE(sockaddr), NULL, &error);
...
static void test_remove_default(void)
{
    ...
    g_network_monitor_base_remove_network(G_NETWORK_MONITOR_BASE(monitor), ip4_default);
    assert_signals(monitor, FALSE, TRUE, TRUE);
    g_network_monitor_base_remove_network(G_NETWORK_MONITOR_BASE(monitor), ip6_default);
...
static void test_add_networks(void)
{
    GNetworkMonitor *monitor;
    GError *error = NULL;
    ...
    g_network_monitor_base_add_network(G_NETWORK_MONITOR_BASE(monitor), net127.mask);
    ...
    g_network_monitor_base_add_network(G_NETWORK_MONITOR_BASE(monitor), net10.mask);
    assert_signals(monitor, FALSE, TRUE, FALSE);
    run_tests(monitor, net127.addresses, TRUE);
    run_tests(monitor, net10.addresses, TRUE);
    run_tests(monitor, net192.addresses, FALSE);
    ...
    g_network_monitor_base_add_network(G_NETWORK_MONITOR_BASE(monitor), net192.mask);
    assert_signals(monitor, FALSE, TRUE, FALSE);
    run_tests(monitor, net127.addresses, TRUE);
    run_tests(monitor, net10.addresses, TRUE);
    run_tests(monitor, net192.addresses, TRUE);
    ...
    g_network_monitor_base_add_network(G_NETWORK_MONITOR_BASE(monitor), netlocal6.mask);
    assert_signals(monitor, FALSE, TRUE, FALSE);
    run_tests(monitor, net127.addresses, TRUE);
    run_tests(monitor, net10.addresses, TRUE);
    run_tests(monitor, net192.addresses, TRUE);
    ...
    g_network_monitor_base_add_network(G_NETWORK_MONITOR_BASE(monitor), netfe80.mask);
    assert_signals(monitor, FALSE, TRUE, FALSE);
    run_tests(monitor, net127.addresses, TRUE);
    run_tests(monitor, net10.addresses, TRUE);
    run_tests(monitor, net192.addresses, TRUE);
...
static void test_remove_networks(void)
{
    GNetworkMonitor *monitor;
    GError *error = NULL;
    ...
    g_network_monitor_base_remove_network(G_NETWORK_MONITOR_BASE(monitor), net127.mask);
    assert_signals(monitor, FALSE, TRUE, FALSE);
    run_tests(monitor, net127.addresses, FALSE);
    run_tests(monitor, net10.addresses, TRUE);
    run_tests(monitor, net192.addresses, TRUE);
    ...
    g_network_monitor_base_remove_network(G_NETWORK_MONITOR_BASE(monitor), net10.mask);
    assert_signals(monitor, FALSE, TRUE, FALSE);
    run_tests(monitor, net127.addresses, FALSE);
    run_tests(monitor, net10.addresses, FALSE);
    run_tests(monitor, net192.addresses, TRUE);
    ...
    g_network_monitor_base_remove_network(G_NETWORK_MONITOR_BASE(monitor), net192.mask);
    assert_signals(monitor, FALSE, TRUE, FALSE);
    run_tests(monitor, net127.addresses, FALSE);
    run_tests(monitor, net10.addresses, FALSE);
    run_tests(monitor, net192.addresses, FALSE);
    ...
    g_network_monitor_base_remove_network(G_NETWORK_MONITOR_BASE(monitor), netlocal6.mask);
    assert_signals(monitor, FALSE, TRUE, FALSE);
    run_tests(monitor, net127.addresses, FALSE);
    run_tests(monitor, net10.addresses, FALSE);
    run_tests(monitor, net192.addresses, FALSE);
    ...
    g_network_monitor_base_remove_network(G_NETWORK_MONITOR_BASE(monitor), netfe80.mask);
    assert_signals(monitor, FALSE, TRUE, FALSE);
    run_tests(monitor, net127.addresses, FALSE);
    run_tests(monitor, net10.addresses, FALSE);
    run_tests(monitor, net192.addresses, FALSE);
...
static void init_test(TestMask * test)
{
    GError *error = NULL;
    int i;
...
static void cleanup_test(TestMask * test)
{
    int i;
...
static void watch_network_changed(GNetworkMonitor * monitor, gboolean available, gpointer user_data)
{
    g_print("Network is %s\n", available ? "up" : "down");
}

static void watch_connectivity_changed(GNetworkMonitor * monitor, GParamSpec * pspec, gpointer user_data)
{
    g_print("Connectivity is %d\n", g_network_monitor_get_connectivity(monitor));
}

static void watch_metered_changed(GNetworkMonitor * monitor, GParamSpec * pspec, gpointer user_data)
{
    g_print("Metered is %d\n", g_network_monitor_get_network_metered(monitor));
}

static void do_watch_network(void)
{
    ...
    g_signal_connect(monitor, "network-changed", G_CALLBACK(watch_network_changed), NULL);
    g_signal_connect(monitor, "notify::connectivity", G_CALLBACK(watch_connectivity_changed), NULL);
    g_signal_connect(monitor, "notify::network-metered", G_CALLBACK(watch_metered_changed), NULL);
