...
static const gchar introspection_xml[] =
    ...
    "    <method name='Hello'>"
    "      <arg type='s' name='greeting' direction='in'/>"
    "      <arg type='s' name='response' direction='out'/>" "    </method>" "    <property type='i' name='PartitionNumber' access='read'/>" "    <property type='s' name='Notes' access='readwrite'/>" "  </interface>" "</node>";
...
static void manager_method_call(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * method_name, GVariant * parameters, GDBusMethodInvocation * invocation, gpointer user_data)
{
    ...
    response = g_strdup_printf("Method %s.%s with user_data '%s' on object path %s called with arg '%s'", interface_name, method_name, (const gchar *) user_data, object_path, greeting);
...
static void block_method_call(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * method_name, GVariant * parameters, GDBusMethodInvocation * invocation, gpointer user_data)
{
...
static GVariant *block_get_property(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * property_name, GError ** error, gpointer user_data)
{
    GVariant *ret;
    const gchar *node;
    gint major;
...
static gboolean block_set_property(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * property_name, GVariant * value, GError ** error, gpointer user_data)
{
    /* TODO */
    g_assert_not_reached();
}
...
static void partition_method_call(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * method_name, GVariant * parameters, GDBusMethodInvocation * invocation, gpointer user_data)
{
    const gchar *greeting;
    gchar *response;
...
static GDBusInterfaceInfo **subtree_introspect(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * node, gpointer user_data)
{
    ...
    else
    {
        ...
        if (strlen(node) == 4)
            g_ptr_array_add(p, g_dbus_interface_info_ref(partition_interface_info));
    }
...
static void on_bus_acquired(GDBusConnection * connection, const gchar * name, gpointer user_data)
{
    ...
    registration_id = g_dbus_connection_register_subtree(connection, "/org/gtk/GDBus/TestSubtree/Devices", &subtree_vtable, G_DBUS_SUBTREE_FLAGS_NONE, NULL,    /* user_data */
...
int main(int argc, char *argv[])
{
    ...
    owner_id = g_bus_own_name(G_BUS_TYPE_SESSION, "org.gtk.GDBus.TestSubtree", G_BUS_NAME_OWNER_FLAGS_NONE, on_bus_acquired, on_name_acquired, on_name_lost, NULL, NULL);
