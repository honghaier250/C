...
G_DEFINE_TYPE(TestAppLaunchContext, test_app_launch_context, G_TYPE_APP_LAUNCH_CONTEXT)
     static gchar *test_app_launch_context_get_startup_notify_id(GAppLaunchContext * context, GAppInfo * info, GList * uris)
{
    requested_startup_id = TRUE;
    return g_strdup("expected startup id");
}

static void test_app_launch_context_init(TestAppLaunchContext * ctx)
{
}

static void test_app_launch_context_class_init(GAppLaunchContextClass * class)
{
...
G_DEFINE_TYPE(TestApplication, test_application, G_TYPE_APPLICATION)
     static void saw_action(const gchar * action)
{
    ...
    {
        case 0:
            g_assert_cmpstr(action, ==, "activate");
...
static void test_application_frob(GSimpleAction * action, GVariant * parameter, gpointer user_data)
{
    g_assert(parameter == NULL);
    saw_action("frob");
}

static void test_application_tweak(GSimpleAction * action, GVariant * parameter, gpointer user_data)
{
    g_assert(parameter == NULL);
    saw_action("tweak");
}

static void test_application_twiddle(GSimpleAction * action, GVariant * parameter, gpointer user_data)
{
    g_assert(parameter == NULL);
    saw_action("twiddle");
}

static void test_application_quit(GSimpleAction * action, GVariant * parameter, gpointer user_data)
{
...
static void test_application_activate(GApplication * application)
{
...
static void test_application_open(GApplication * application, GFile ** files, gint n_files, const gchar * hint)
{
    GFile *f;
...
static void test_application_startup(GApplication * application)
{
    G_APPLICATION_CLASS(test_application_parent_class)->startup(application);

    g_action_map_add_action_entries(G_ACTION_MAP(application), app_actions, G_N_ELEMENTS(app_actions), application);
}

static void test_application_before_emit(GApplication * application, GVariant * platform_data)
{
...
static void test_application_class_init(GApplicationClass * class)
{
...
static void test_dbus_appinfo(void)
{
    const gchar *argv[] = { "myapp", NULL };
    TestApplication *app;
    int status;

    appinfo = g_desktop_app_info_new_from_filename(g_test_build_filename(G_TEST_DIST, "org.gtk.test.dbusappinfo.desktop", NULL));
    g_assert(appinfo != NULL);

    app = g_object_new(test_application_get_type(), "application-id", "org.gtk.test.dbusappinfo", "flags", G_APPLICATION_HANDLES_OPEN, NULL);
