...
static GDBusInterfaceInfo *org_gtk_Notifications_get_interface(void)
{
    ...
    if (iface_info == NULL)
    {
        ...
        info = g_dbus_node_info_new_for_xml("<node>"
                                            ...
                                            "    </method>" "    <method name='RemoveNotification'>" "      <arg type='s' direction='in' />" "      <arg type='s' direction='in' />" "    </method>" "  </interface>" "</node>", &error);
...
static void g_notification_server_notification_added(GNotificationServer * server, const gchar * app_id, const gchar * notification_id, GVariant * notification)
{
    ...
    {
        notifications = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, (GDestroyNotify) g_variant_unref);
...
static void g_notification_server_notification_removed(GNotificationServer * server, const gchar * app_id, const gchar * notification_id)
{
    GHashTable *notifications;
...
static void
org_gtk_Notifications_method_call(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * method_name, GVariant * parameters, GDBusMethodInvocation * invocation, gpointer user_data)
{
...
static void g_notification_server_dispose(GObject * object)
{
...
static void g_notification_server_get_property(GObject * object, guint property_id, GValue * value, GParamSpec * pspec)
{
    GNotificationServer *server = G_NOTIFICATION_SERVER(object);
...
static void g_notification_server_class_init(GNotificationServerClass * class)
{
    ...
    g_object_class_install_property(object_class, PROP_IS_RUNNING, g_param_spec_boolean("is-running", "", "", FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

    g_signal_new("notification-received", G_TYPE_NOTIFICATION_SERVER, G_SIGNAL_RUN_FIRST, 0, NULL, NULL, g_cclosure_marshal_generic, G_TYPE_NONE, 3, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_VARIANT);

    g_signal_new("notification-removed", G_TYPE_NOTIFICATION_SERVER, G_SIGNAL_RUN_FIRST, 0, NULL, NULL, g_cclosure_marshal_generic, G_TYPE_NONE, 2, G_TYPE_STRING, G_TYPE_STRING);
}

static void g_notification_server_bus_acquired(GDBusConnection * connection, const gchar * name, gpointer user_data)
{
    ...
    server->object_id = g_dbus_connection_register_object(connection, "/org/gtk/Notifications", org_gtk_Notifications_get_interface(), &vtable, server, NULL, NULL);
...
static void g_notification_server_name_acquired(GDBusConnection * connection, const gchar * name, gpointer user_data)
{
    GNotificationServer *server = user_data;
...
static void g_notification_server_name_lost(GDBusConnection * connection, const gchar * name, gpointer user_data)
{
    GNotificationServer *server = user_data;
...
static void g_notification_server_init(GNotificationServer * server)
{
    server->applications = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, (GDestroyNotify) g_hash_table_unref);

    server->name_owner_id = g_bus_own_name(G_BUS_TYPE_SESSION, "org.gtk.Notifications", G_BUS_NAME_OWNER_FLAGS_NONE, g_notification_server_bus_acquired, g_notification_server_name_acquired, g_notification_server_name_lost, server, NULL);
}
...
gchar **g_notification_server_list_notifications(GNotificationServer * server, const gchar * app_id)
{
    GHashTable *notifications;
