...
static void set_up_mock_xdg_runtime_dir(void)
{
    GError *error = NULL;
    GSocketAddress *addr;

    mock_bus = g_socket_new(G_SOCKET_FAMILY_UNIX, G_SOCKET_TYPE_STREAM, 0, &error);
    g_assert_no_error(error);
...
static void tear_down_mock_xdg_runtime_dir(void)
{
    GError *error = NULL;
...
static void set_up_mock_dbus_launch(void)
{
    path = g_strconcat(g_test_get_dir(G_TEST_BUILT), ":", g_getenv("PATH"), NULL);
    g_debug("PATH=%s", path);
...
static void tear_down_mock_dbus_launch(void)
{
    g_clear_pointer(&path, g_free);
}

static void test_x11_autolaunch(void)
{
...
static void test_xdg_runtime(void)
{
    if (g_test_subprocess())
    {
        g_unsetenv("DISPLAY");
