...
static GPtrArray *get_test_subprocess_args(const char *mode, ...) G_GNUC_NULL_TERMINATED;

     static GPtrArray *get_test_subprocess_args(const char *mode, ...)
{
    GPtrArray *ret;
    char *path;
    const char *binname;
...
static void check_ready(GObject * source, GAsyncResult * res, gpointer user_data)
{
    gboolean ret;
    GError *error = NULL;

    ret = g_subprocess_wait_check_finish(G_SUBPROCESS(source), res, &error);
    g_assert(ret);
...
static void test_noop_all_to_null(void)
{
    GError *local_error = NULL;
    GError **error = &local_error;
    GPtrArray *args;
    ...
    args = get_test_subprocess_args("noop", NULL);
    proc = g_subprocess_newv((const gchar * const *) args->pdata, G_SUBPROCESS_FLAGS_STDOUT_SILENCE | G_SUBPROCESS_FLAGS_STDERR_SILENCE, error);
    g_ptr_array_free(args, TRUE);
    g_assert_no_error(local_error);
...
static void test_noop_no_wait(void)
{
    GError *local_error = NULL;
    GError **error = &local_error;
    GPtrArray *args;
...
static void test_noop_stdin_inherit(void)
{
    GError *local_error = NULL;
    GError **error = &local_error;
    GPtrArray *args;
...
static gchar *splice_to_string(GInputStream * stream, GError ** error)
{
...
#ifdef G_OS_UNIX
static void test_echo_merged(void)
{
    GError *local_error = NULL;
    GError **error = &local_error;
    GSubprocess *proc;
    ...
    proc = g_subprocess_newv((const gchar * const *) args->pdata, G_SUBPROCESS_FLAGS_STDOUT_PIPE | G_SUBPROCESS_FLAGS_STDERR_MERGE, error);
    g_ptr_array_free(args, TRUE);
    g_assert_no_error(local_error);
...
static void test_cat_on_input_splice_complete(GObject * object, GAsyncResult * result, gpointer user_data)
{
...
static void test_cat_utf8(void)
{
    GError *local_error = NULL;
    GError **error = &local_error;
    GSubprocess *proc;
    ...
    proc = g_subprocess_newv((const gchar * const *) args->pdata, G_SUBPROCESS_FLAGS_STDIN_PIPE | G_SUBPROCESS_FLAGS_STDOUT_PIPE, error);
    g_ptr_array_free(args, TRUE);
    g_assert_no_error(local_error);
    ...
    g_output_stream_splice_async(stdin_stream, input_buf_stream, G_OUTPUT_STREAM_SPLICE_CLOSE_SOURCE | G_OUTPUT_STREAM_SPLICE_CLOSE_TARGET, G_PRIORITY_DEFAULT, NULL, test_cat_on_input_splice_complete, &data);
    data.events_pending++;
    g_output_stream_splice_async(output_buf_stream, stdout_stream, G_OUTPUT_STREAM_SPLICE_CLOSE_SOURCE | G_OUTPUT_STREAM_SPLICE_CLOSE_TARGET, G_PRIORITY_DEFAULT, NULL, test_cat_on_input_splice_complete, &data);
    data.events_pending++;
    ...
    g_assert_cmpmem(g_bytes_get_data(output_buf, NULL), g_bytes_get_size(output_buf), "hello, world!", 13);
...
static gpointer cancel_soon(gpointer user_data)
{
...
static void on_one_multi_splice_done(GObject * obj, GAsyncResult * res, gpointer user_data)
{
...
static gboolean on_idle_multisplice(gpointer user_data)
{
    TestMultiSpliceData *data = user_data;
...
static void on_subprocess_exited(GObject * object, GAsyncResult * result, gpointer user_data)
{
...
static void test_multi_1(void)
{
    ...
    data.events_pending++;
    g_output_stream_splice_async(second_stdin, first_stdout, splice_flags, G_PRIORITY_DEFAULT, NULL, on_one_multi_splice_done, &data);
    data.events_pending++;
    g_output_stream_splice_async(third_stdin, second_stdout, splice_flags, G_PRIORITY_DEFAULT, NULL, on_one_multi_splice_done, &data);
    data.events_pending++;
    g_output_stream_splice_async(membuf, third_stdout, splice_flags, G_PRIORITY_DEFAULT, NULL, on_one_multi_splice_done, &data);
...
static void on_communicate_complete(GObject * proc, GAsyncResult * result, gpointer user_data)
{
    ...
    data->running = FALSE;
    if (data->is_utf8)
        (void) g_subprocess_communicate_utf8_finish((GSubprocess *) proc, result, &stdout_str, NULL, &data->error);
    else
        (void) g_subprocess_communicate_finish((GSubprocess *) proc, result, &stdout, NULL, &data->error);
    if (data->error)
        return;
...
static void test_communicate_async(void)
{
    GError *error = NULL;
    GPtrArray *args;
    ...
    g_subprocess_communicate_async(proc, input, cancellable, on_communicate_complete, &data);
...
static void test_communicate(void)
{
    GError *error = NULL;
    GPtrArray *args;
    GSubprocess *proc;
    ...
    args = get_test_subprocess_args("cat", NULL);
    proc = g_subprocess_newv((const gchar * const *) args->pdata, G_SUBPROCESS_FLAGS_STDIN_PIPE | G_SUBPROCESS_FLAGS_STDOUT_PIPE | G_SUBPROCESS_FLAGS_STDERR_MERGE, &error);
    g_assert_no_error(error);
    g_ptr_array_free(args, TRUE);
...
static void test_communicate_utf8_async(void)
{
    GError *error = NULL;
    GPtrArray *args;
    TestAsyncCommunicateData data = { 0, };
    ...
    data.is_utf8 = TRUE;
    g_subprocess_communicate_utf8_async(proc, "hello world", cancellable, on_communicate_complete, &data);
...
static void test_communicate_utf8(void)
{
    GError *error = NULL;
    GPtrArray *args;
    GSubprocess *proc;
...
static void test_communicate_nothing(void)
{
    GError *error = NULL;
    GPtrArray *args;
    GSubprocess *proc;
...
static void test_communicate_utf8_invalid(void)
{
    GError *error = NULL;
    GPtrArray *args;
    TestAsyncCommunicateData data = { 0, };
    ...
    data.is_utf8 = TRUE;
    g_subprocess_communicate_utf8_async(proc, "\xFF\xFF", cancellable, on_communicate_complete, &data);
...
static gboolean send_terminate(gpointer user_data)
{
...
static void on_request_quit_exited(GObject * object, GAsyncResult * result, gpointer user_data)
{
    GSubprocess *subprocess = G_SUBPROCESS(object);
    GError *error = NULL;
...
#ifdef G_OS_UNIX
static gboolean send_signal(gpointer user_data)
{
    GSubprocess *proc = user_data;
...
#ifdef G_OS_UNIX
static void test_stdout_file(void)
{
    GError *local_error = NULL;
    GError **error = &local_error;
    GSubprocessLauncher *launcher;
...
static void test_stdout_fd(void)
{
    GError *local_error = NULL;
    GError **error = &local_error;
    GSubprocessLauncher *launcher;
...
static void test_child_setup(void)
{
    GError *local_error = NULL;
    GError **error = &local_error;
    GSubprocessLauncher *launcher;
...
static void test_launcher_environment(void)
{
    GSubprocessLauncher *launcher;
    GError *error = NULL;
    GSubprocess *proc;
