...
static void free_recorded_event(RecordedEvent * event)
{
    g_free(event->file);
...
#if 0
static void output_event(RecordedEvent * event)
{
    if (event->step >= 0)
        g_print(">>>> step %d\n", event->step);
    else
    {
        ...
        g_print("%s file=%s other_file=%s\n", g_enum_get_value(G_ENUM_CLASS(class), event->event_type)->value_nick, event->file, event->other_file);
        g_type_class_unref(class);
    }
}

static void output_events(GList * list)
{
    GList *l;
...
static void check_expected_event(gint i, RecordedEvent * e1, RecordedEvent * e2)
{
...
static void check_expected_events(RecordedEvent * expected, gsize n_expected, GList * recorded)
{
    gint i;
    GList *l;
...
static void record_event(TestData * data, gint event_type, const gchar * file, const gchar * other_file, gint step)
{
    RecordedEvent *event;
...
static gboolean atomic_replace_step(gpointer user_data)
{
    TestData *data = user_data;
    GError *error = NULL;
...
static gboolean change_step(gpointer user_data)
{
    TestData *data = user_data;
    ...
    switch (data->step)
    {
        ...
        case 2:
            record_event(data, -1, NULL, NULL, 2);
            g_file_set_attribute(data->file, G_FILE_ATTRIBUTE_UNIX_MODE, G_FILE_ATTRIBUTE_TYPE_UINT32, &mode, G_FILE_QUERY_INFO_NONE, NULL, &error);
            g_assert_no_error(error);
            break;
        case 3:
            record_event(data, -1, NULL, NULL, 3);
...
static void test_file_changes(void)
{
    GError *error = NULL;
    TestData data;
...
static gboolean dir_step(gpointer user_data)
{
    TestData *data = user_data;
...
static void test_dir_monitor(void)
{
    GError *error = NULL;
    TestData data;
...
static gboolean nodir_step(gpointer user_data)
{
    TestData *data = user_data;
    GFile *parent;
    GError *error = NULL;
...
static void test_dir_non_existent(void)
{
    TestData data;
    GError *error = NULL;
...
static gboolean cross_dir_step(gpointer user_data)
{
    TestData *data = user_data;
    GFile *file, *file2;
    GError *error = NULL;
...
static void test_cross_dir_moves(void)
{
    GError *error = NULL;
    TestData data[2];
