...
static gboolean quit_later_cb(gpointer data G_GNUC_UNUSED)
{
...
static void closed_cb(GDBusConnection * c G_GNUC_UNUSED, gboolean remote_peer_vanished, GError * error, gpointer test_data)
{
    ...
    if (error == NULL)
        ...
        g_debug("closed (%d, %s %d \"%s\")", remote_peer_vanished, g_quark_to_string(error->domain), error->code, error->message);
...
static void close_async_cb(GObject * source G_GNUC_UNUSED, GAsyncResult * res G_GNUC_UNUSED, gpointer nil G_GNUC_UNUSED)
{
    GError *error = NULL;

    if (g_dbus_connection_close_finish(G_DBUS_CONNECTION(source), res, &error))
    {
        g_debug("closed connection");
    }
    else
    {
        g_warning("failed to close connection: %s (%s #%d)", error->message, g_quark_to_string(error->domain), error->code);
    }
}

static void test_exit_on_close_subprocess(gconstpointer test_data)
{
    const TestData *td = test_data;
    GDBusConnection *c;
    ...
    g_assert_cmpint(g_dbus_connection_get_exit_on_close(c), ==, (td->exit_on_close != EXPLICITLY_FALSE));
    ...
    if (td->who_closes == LOCAL)
    {
        ...
        v = g_dbus_connection_call_sync(c, "org.freedesktop.DBus", "/org/freedesktop/DBus", "org.freedesktop.DBus", "ListNames", NULL, G_VARIANT_TYPE("(as)"), G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
...
static void test_exit_on_close(gconstpointer test_data)
{
    const TestData *td = test_data;
    ...
    if (td->exit_on_close == EXPLICITLY_FALSE || td->who_closes == LOCAL)
