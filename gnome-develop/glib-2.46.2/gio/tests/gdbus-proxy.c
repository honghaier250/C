...
static void test_methods(GDBusProxy * proxy)
{
    ...
    error = NULL;
    result = g_dbus_proxy_call_sync(proxy, "Sleep", g_variant_new("(i)", 500 /* msec */ ),
                                    G_DBUS_CALL_FLAGS_NONE, 100 /* msec */ ,
                                    NULL, &error);
    ...
    result = g_dbus_proxy_call_sync(proxy, "Sleep", g_variant_new("(i)", 500 /* msec */ ),
                                    G_DBUS_CALL_FLAGS_NONE, -1,    /* use proxy default (e.g. -1 -> e.g. 25000 msec) */
                                    NULL, &error);
    g_assert_no_error(error);
    g_assert(result != NULL);
    ...
    result = g_dbus_proxy_call_sync(proxy, "Sleep", g_variant_new("(i)", 500 /* msec */ ),
                                    G_DBUS_CALL_FLAGS_NONE, -1,    /* use proxy default (e.g. 250 msec) */
                                    NULL, &error);
    g_assert_error(error, G_IO_ERROR, G_IO_ERROR_TIMED_OUT);
    g_assert(!g_dbus_error_is_remote_error(error));
    g_assert(result == NULL);
...
static void test_properties(GDBusProxy * proxy)
{
    ...
    g_assert(strv_equal(names, "PropertyThatWillBeInvalidated", "ab", "ad", "ai", "an", "ao", "aq", "as", "at", "au", "ax", "ay", "b", "d", "foo", "i", "n", "o", "q", "s", "t", "u", "x", "y", NULL));
     ...
     */
    result = g_dbus_proxy_call_sync(proxy, "FrobInvalidateProperty", g_variant_new("(s)", "OMGInvalidated"), G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    g_assert_no_error(error);
    g_assert(result != NULL);
    g_assert_cmpstr(g_variant_get_type_string(result), ==, "()");
    g_variant_unref(result);
    ...
    error = NULL;
    proxy2 = g_dbus_proxy_new_sync(g_dbus_proxy_get_connection(proxy), G_DBUS_PROXY_FLAGS_GET_INVALIDATED_PROPERTIES, NULL,    /* GDBusInterfaceInfo */
    ...
    result = g_dbus_proxy_call_sync(proxy2, "FrobInvalidateProperty", g_variant_new("(s)", "OMGInvalidated2"), G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    g_assert_no_error(error);
    g_assert(result != NULL);
    g_assert_cmpstr(g_variant_get_type_string(result), ==, "()");
    g_variant_unref(result);
...
static void test_signals(GDBusProxy * proxy)
{
    ...
    result = g_dbus_proxy_call_sync(proxy, "EmitSignal", g_variant_new("(so)", "Accept the next proposition you hear", "/some/path"), G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    g_assert_no_error(error);
    g_assert(result != NULL);
    g_assert_cmpstr(g_variant_get_type_string(result), ==, "()");
    g_variant_unref(result);
...
static void test_bogus_signal(GDBusProxy * proxy)
{
    GError *error = NULL;
    GVariant *result;
    GDBusInterfaceInfo *old_iface_info;

    result = g_dbus_proxy_call_sync(proxy, "EmitSignal2", NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    g_assert_no_error(error);
    g_assert(result != NULL);
    g_assert_cmpstr(g_variant_get_type_string(result), ==, "()");
    g_variant_unref(result);
...
static void test_bogus_property(GDBusProxy * proxy)
{
    GError *error = NULL;
    GVariant *result;
    GDBusInterfaceInfo *old_iface_info;
     ...
     */
    result = g_dbus_proxy_call_sync(proxy, "FrobSetProperty", g_variant_new("(sv)", "i", g_variant_new_int32(42)), G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    g_assert_no_error(error);
    g_assert(result != NULL);
    g_assert_cmpstr(g_variant_get_type_string(result), ==, "()");
    g_variant_unref(result);
...
static const gchar *frob_dbus_interface_xml = "<node>" "  <interface name='com.example.Frob'>"
    ...
    "      <arg type='s' name='somestring' direction='in'/>" "      <arg type='s' name='somestring' direction='out'/>" "    </method>" "    <method name='Sleep'>" "      <arg type='i' name='timeout' direction='in'/>" "    </method>"
    ...
    "    <signal name='TestSignal2'>" "      <arg type='u' name='somenumber'/>" "    </signal>" "  </interface>" "</node>";
...
static void test_expected_interface(GDBusProxy * proxy)
{
    ...
    if (g_test_undefined())
    {
        ...
        g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_WARNING, "*Trying to set property y of type s but according to the expected interface the type is y*");
        g_dbus_proxy_set_cached_property(proxy, "y", g_variant_new_string("error_me_out!"));
    ...
    if (g_test_undefined())
    {
         ...
         */
        g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_WARNING, "*Trying to get property i with type i but according to the expected interface the type is u*");
    ...
    error = NULL;
    value = g_dbus_proxy_call_sync(proxy, "FrobSetProperty", g_variant_new("(sv)", "d", g_variant_new_double(85.0)), G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    g_assert_no_error(error);
    g_assert(value != NULL);
...
static void test_basic(GDBusProxy * proxy)
{
    ...
    g_object_get(proxy, "g-connection", &conn, "g-interface-info", &info, "g-flags", &flags, "g-name", &name, "g-object-path", &path, "g-interface-name", &interface, "g-default-timeout", &timeout, NULL);
...
static void kill_test_service(GDBusConnection * connection)
{
#ifdef G_OS_UNIX
    guint pid;
    GVariant *ret;
    ...
    ret = g_dbus_connection_call_sync(connection, "org.freedesktop.DBus", "/org/freedesktop/DBus", "org.freedesktop.DBus", "GetConnectionUnixProcessID", g_variant_new("(s)", name), NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
...
static void test_proxy(void)
{
    ...
    g_assert_no_error(error);
    error = NULL;
    proxy = g_dbus_proxy_new_sync(connection, G_DBUS_PROXY_FLAGS_NONE, NULL,    /* GDBusInterfaceInfo */
                                  "com.example.TestService",    /* name */
                                  "/com/example/TestObject",    /* object path */
                                  "com.example.Frob",    /* interface */
                                  NULL,    /* GCancellable */
...
static gboolean fail_test(gpointer user_data)
{
...
{
    g_dbus_proxy_new_for_bus(G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL,    /* GDBusInterfaceInfo */
                             "com.example.TestService",    /* name */
                             "/com/example/TestObject",    /* object path */
                             "com.example.Frob",    /* interface */
                             NULL,    /* GCancellable */
...
static void test_no_properties(void)
{
    ...
    error = NULL;
    proxy = g_dbus_proxy_new_for_bus_sync(G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES, NULL,    /* GDBusInterfaceInfo */
                                          "com.example.TestService",    /* name */
                                          "/com/example/TestObject",    /* object path */
                                          "com.example.Frob",    /* interface */
                                          NULL,    /* GCancellable */
...
static void check_error(GObject * source, GAsyncResult * result, gpointer user_data)
{
    GError *error = NULL;
    GVariant *reply;
...
static void test_wellknown_noauto(void)
{
    GError *error = NULL;
    GDBusProxy *proxy;

    proxy = g_dbus_proxy_new_for_bus_sync(G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START, NULL, "some.name.that.does.not.exist", "/", "some.interface", NULL, &error);
    g_assert_no_error(error);
    g_assert(proxy != NULL);
