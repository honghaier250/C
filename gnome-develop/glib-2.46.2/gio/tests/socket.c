...
static gpointer echo_server_dgram_thread(gpointer user_data)
{
...
static IPTestData *create_server_full(GSocketFamily family, GSocketType socket_type, GThreadFunc server_thread, gboolean v4mapped)
{
    IPTestData *data;
    GSocket *server;
    GError *error = NULL;
    ...
    data->server = server = g_socket_new(family, socket_type, G_SOCKET_PROTOCOL_DEFAULT, &error);
    g_assert_no_error(error);
...
static IPTestData *create_server(GSocketFamily family, GThreadFunc server_thread, gboolean v4mapped)
{
...
static gboolean test_ip_async_read_ready(GSocket * client, GIOCondition cond, gpointer user_data)
{
    IPTestData *data = user_data;
    GError *error = NULL;
    gssize len;
...
static gboolean test_ip_async_write_ready(GSocket * client, GIOCondition cond, gpointer user_data)
{
    IPTestData *data = user_data;
    GError *error = NULL;
    GSource *source;
    ...
    g_source_set_callback(source, (GSourceFunc) test_ip_async_read_ready, data, NULL);
...
static gboolean test_ip_async_timed_out(GSocket * client, GIOCondition cond, gpointer user_data)
{
    IPTestData *data = user_data;
    GError *error = NULL;
    GSource *source;
    ...
    g_source_set_callback(source, (GSourceFunc) test_ip_async_write_ready, data, NULL);
    g_source_attach(source, NULL);
    g_source_unref(source);
...
static gboolean test_ip_async_connected(GSocket * client, GIOCondition cond, gpointer user_data)
{
    IPTestData *data = user_data;
    GError *error = NULL;
    GSource *source;
    ...
    if (data->family == G_SOCKET_FAMILY_IPV4)
    {
        ...
        source = g_socket_create_source(client, G_IO_IN, NULL);
        g_source_set_callback(source, (GSourceFunc) test_ip_async_timed_out, data, NULL);
        g_source_attach(source, NULL);
        g_source_unref(source);
    }
    else
...
static gboolean idle_test_ip_async_connected(gpointer user_data)
{
    IPTestData *data = user_data;
...
static void test_ip_async(GSocketFamily family)
{
    IPTestData *data;
    GError *error = NULL;
    GSocket *client;
    ...
    client = g_socket_new(family, G_SOCKET_TYPE_STREAM, G_SOCKET_PROTOCOL_DEFAULT, &error);
    g_assert_no_error(error);
    data->client = client;
    ...
    else
    {
        ...
        g_clear_error(&error);
        source = g_socket_create_source(client, G_IO_OUT, NULL);
        g_source_set_callback(source, (GSourceFunc) test_ip_async_connected, data, NULL);
        g_source_attach(source, NULL);
        g_source_unref(source);
    }
    g_object_unref(addr);
    ...
    if (family == G_SOCKET_FAMILY_IPV4)
    {
        ...
        len = g_socket_receive_with_blocking(client, buf, sizeof(buf), TRUE, NULL, &error);
        g_assert_no_error(error);
    ...
    else
    {
        ...
        len = g_socket_send_with_blocking(client, testbuf, strlen(testbuf) + 1, TRUE, NULL, &error);
...
static void test_ipv4_async(void)
{
    test_ip_async(G_SOCKET_FAMILY_IPV4);
}

static void test_ipv6_async(void)
{
    if (!ipv6_supported)
    {
...
static void test_ip_sync(GSocketFamily family)
{
    IPTestData *data;
    GError *error = NULL;
    GSocket *client;
...
static void test_ipv4_sync(void)
{
    test_ip_sync(G_SOCKET_FAMILY_IPV4);
}

static void test_ipv6_sync(void)
{
    if (!ipv6_supported)
    {
        g_test_skip("No support for IPv6");
...
static void test_ip_sync_dgram(GSocketFamily family)
{
    IPTestData *data;
    GError *error = NULL;
    GSocket *client;
    ...
    data = create_server_full(family, G_SOCKET_TYPE_DATAGRAM, echo_server_dgram_thread, FALSE);

    dest_addr = g_socket_get_local_address(data->server, &error);

    client = g_socket_new(family, G_SOCKET_TYPE_DATAGRAM, G_SOCKET_PROTOCOL_DEFAULT, &error);
    g_assert_no_error(error);
...
static void test_ipv4_sync_dgram(void)
{
    test_ip_sync_dgram(G_SOCKET_FAMILY_IPV4);
}

static void test_ipv6_sync_dgram(void)
{
    if (!ipv6_supported)
    {
        g_test_skip("No support for IPv6");
...
static gpointer graceful_server_thread(gpointer user_data)
{
    IPTestData *data = user_data;
    GSocket *sock;
    GError *error = NULL;
...
static void test_close_graceful(void)
{
...
#if defined (IPPROTO_IPV6) && defined (IPV6_V6ONLY)
static gpointer v4mapped_server_thread(gpointer user_data)
{
    IPTestData *data = user_data;
    GSocket *sock;
    GError *error = NULL;
...
static void test_ipv6_v4mapped(void)
{
    IPTestData *data;
    GError *error = NULL;
    GSocket *client;
    ...
    client = g_socket_new(G_SOCKET_FAMILY_IPV4, G_SOCKET_TYPE_STREAM, G_SOCKET_PROTOCOL_DEFAULT, &error);
    g_assert_no_error(error);
...
static int duplicate_fd(int fd)
{
    ...
    if (!DuplicateHandle(GetCurrentProcess(), (HANDLE) fd, GetCurrentProcess(), &newfd, 0, FALSE, DUPLICATE_SAME_ACCESS))
    {
        return -1;
    }
...
static void test_reuse_tcp(void)
{
    GSocket *sock1, *sock2;
    GError *error = NULL;
    GInetAddress *iaddr;
    ...
    sock1 = g_socket_new(G_SOCKET_FAMILY_IPV4, G_SOCKET_TYPE_STREAM, G_SOCKET_PROTOCOL_DEFAULT, &error);
    g_assert_no_error(error);
    ...
    sock2 = g_socket_new(G_SOCKET_FAMILY_IPV4, G_SOCKET_TYPE_STREAM, G_SOCKET_PROTOCOL_DEFAULT, &error);
    g_assert_no_error(error);
...
static void test_reuse_udp(void)
{
    GSocket *sock1, *sock2;
    GError *error = NULL;
    GInetAddress *iaddr;
    ...
    sock1 = g_socket_new(G_SOCKET_FAMILY_IPV4, G_SOCKET_TYPE_DATAGRAM, G_SOCKET_PROTOCOL_DEFAULT, &error);
    g_assert_no_error(error);
    ...
    sock2 = g_socket_new(G_SOCKET_FAMILY_IPV4, G_SOCKET_TYPE_DATAGRAM, G_SOCKET_PROTOCOL_DEFAULT, &error);
    g_assert_no_error(error);
...
static void test_get_available(gconstpointer user_data)
{
    ...
    listener = g_socket_new(G_SOCKET_FAMILY_IPV4, socket_type, G_SOCKET_PROTOCOL_DEFAULT, &err);
    g_assert_no_error(err);
    g_assert(G_IS_SOCKET(listener));

    client = g_socket_new(G_SOCKET_FAMILY_IPV4, socket_type, G_SOCKET_PROTOCOL_DEFAULT, &err);
    g_assert_no_error(err);
...
int main(int argc, char *argv[])
{
    ...
    sock = g_socket_new(G_SOCKET_FAMILY_IPV6, G_SOCKET_TYPE_STREAM, G_SOCKET_PROTOCOL_DEFAULT, &error);
    if (sock != NULL)
    {
        ipv6_supported = TRUE;
        g_object_unref(sock);
    ...
    g_test_add_data_func("/socket/get_available/datagram", GUINT_TO_POINTER(G_SOCKET_TYPE_DATAGRAM), test_get_available);
    g_test_add_data_func("/socket/get_available/stream", GUINT_TO_POINTER(G_SOCKET_TYPE_STREAM), test_get_available);
