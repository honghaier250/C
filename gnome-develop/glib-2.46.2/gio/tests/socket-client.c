...
static GTlsCertificate *lookup_client_certificate(GTlsClientConnection * conn, GError ** error)
{
    GList *l, *accepted;
    ...
    for (l = accepted; l != NULL; l = g_list_next(l))
    {
        ...
        certificates = g_tls_database_lookup_certificates_issued_by(database, l->data, g_tls_connection_get_interaction(base), G_TLS_DATABASE_LOOKUP_KEYPAIR, NULL, error);
        if (error && *error)
            break;
    ...
    if (certificate == NULL && error && !*error)
        g_set_error_literal(error, G_TLS_ERROR, G_TLS_ERROR_CERTIFICATE_REQUIRED, "Server requested a certificate, but could not find relevant certificate in database.");
    return certificate;
}

static gboolean
make_connection(const char *argument, GTlsCertificate * certificate, GCancellable * cancellable, GSocket ** socket, GSocketAddress ** address, GIOStream ** connection, GInputStream ** istream, GOutputStream ** ostream, GError ** error)
{
    ...
    *socket = g_socket_new(socket_family, socket_type, 0, error);
    ...
    if (unix_socket)
    {
        ...
        if (addr == NULL)
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, "Could not parse '%s' as unix socket name", argument);
            return FALSE;
        }
        ...
        *address = g_socket_address_enumerator_next(enumerator, cancellable, error);
        if (*address == NULL)
        {
            if (error != NULL && *error == NULL)
                g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, "No more addresses to try");
            return FALSE;
        }
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < 2; i++)
    {
        if (make_connection(argv[1], certificate, cancellable, &socket, &address, &connection, &istream, &ostream, &error))
            break;
