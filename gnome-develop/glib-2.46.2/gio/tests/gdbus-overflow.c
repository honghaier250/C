...
static void test_overflow(void)
{
    ...
    producer = g_dbus_connection_new_sync(G_IO_STREAM(socket_connection), NULL,    /* guid */
    ...
    {
        error = NULL;
        g_dbus_connection_emit_signal(producer, NULL,    /* destination */
                                      "/org/foo/Object", "org.foo.Interface", "Member", g_variant_new("(s)", "a string"), &error);
        g_assert_no_error(error);
    }
    ...
    g_assert(socket_connection != NULL);
    g_object_unref(socket);
    consumer = g_dbus_connection_new_sync(G_IO_STREAM(socket_connection), NULL,    /* guid */
                                          G_DBUS_CONNECTION_FLAGS_DELAY_MESSAGE_PROCESSING, NULL,    /* GDBusAuthObserver */
                                          NULL,    /* GCancellable */
                                          &error);
    g_assert_no_error(error);
    g_object_unref(socket_connection);
