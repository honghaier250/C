...
static void signal_handler(GDBusConnection * connection, const gchar * sender_name, const gchar * object_path, const gchar * interface_name, const gchar * signal_name, GVariant * parameters, gpointer user_data)
{
    DeliveryData *data = user_data;
...
static gpointer test_delivery_in_thread_func(gpointer _data)
{
    ...
    g_dbus_connection_call(c, "org.freedesktop.DBus",    /* bus_name */
                           ...
                           "GetId",    /* method name */
                           NULL, NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, (GAsyncReadyCallback) msg_cb_expect_success, &data);
    ...
    g_dbus_connection_call(c, "org.freedesktop.DBus",    /* bus_name */
                           ...
                           "org.freedesktop.DBus",    /* interface name */
                           "GetId",    /* method name */
                           NULL, NULL, G_DBUS_CALL_FLAGS_NONE, -1, ca, (GAsyncReadyCallback) msg_cb_expect_error_cancelled, &data);
    g_main_loop_run(thread_loop);
    g_object_unref(ca);
     ...
     * the message bus.
     */
    subscription_id = g_dbus_connection_signal_subscribe(c, "org.freedesktop.DBus",    /* sender */
                                                         "org.freedesktop.DBus",    /* interface */
                                                         "NameOwnerChanged",    /* member */
                                                         "/org/freedesktop/DBus",    /* path */
                                                         NULL, G_DBUS_SIGNAL_FLAGS_NONE, signal_handler, &data, NULL);
...
static void test_delivery_in_thread(void)
{
    ...
    thread = g_thread_new("deliver", test_delivery_in_thread_func, NULL);
...
static void test_method_calls_on_proxy(GDBusProxy * proxy)
{
    ...
    for (n = 0; n < 2; n++)
    {
        ...
        thread1 = g_thread_new("sleep", test_sleep_in_thread_func, &data1);

        data2.proxy = proxy;
        data2.msec = 20;
        data2.num = 200;
        data2.async = do_async;
        thread2 = g_thread_new("sleep2", test_sleep_in_thread_func, &data2);

        data3.proxy = proxy;
        data3.msec = 100;
        data3.num = 40;
        data3.async = do_async;
        thread3 = g_thread_new("sleep3", test_sleep_in_thread_func, &data3);
...
static void ensure_connection_works(GDBusConnection * conn)
{
    GVariant *v;
    GError *error = NULL;

    v = g_dbus_connection_call_sync(conn, "org.freedesktop.DBus", "/org/freedesktop/DBus", "org.freedesktop.DBus", "GetId", NULL, NULL, 0, -1, NULL, &error);
    g_assert_no_error(error);
    g_assert(v != NULL);
 ...
 */
static gpointer get_sync_in_thread(gpointer data)
{
...
static void test_threaded_singleton(void)
{
    guint i, n;
    guint unref_wins = 0;
    guint get_wins = 0;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        thread = g_thread_new("get_sync_in_thread", get_sync_in_thread, GUINT_TO_POINTER(get_delay));
