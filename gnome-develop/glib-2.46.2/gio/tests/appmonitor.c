...
static gboolean create_app(gpointer data)
{
    const gchar *path = data;
    gchar *file;
    GError *error = NULL;
    const gchar *contents = "[Desktop Entry]\n" "Name=Application\n" "Version=1.0\n" "Type=Application\n" "Exec=true\n";
...
static void delete_app(gpointer data)
{
    const gchar *path = data;
    gchar *file;
...
static void changed_cb(GAppInfoMonitor * monitor, GMainLoop * loop)
{
    changed_fired = TRUE;
...
static void test_app_monitor(void)
{
    gchar *path;
