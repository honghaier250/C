...
static void check_data(gint fd, const gchar * expected)
{
    gssize len, actual;
    gchar *buffer;
    ...
    if (actual != len || memcmp(buffer, expected, len) != 0)
    {
        buffer[MIN(len + 100, actual)] = '\0';

        g_error("\nExpected\n-----\n%s-----\nGot (%s)\n-----\n%s-----\n", expected, (actual > len) ? "truncated" : "full", buffer);
    }
}
...
static void spawn(const gchar * expected_stdout, const gchar * expected_stderr, const gchar * first_arg, ...)
{
    GError *error = NULL;
    const gchar *arg;
    GPtrArray *array;
    ...
    g_spawn_async_with_pipes(NULL, args, env, G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL, &pid, NULL, &data->stdout_pipe, expected_stderr ? &data->stderr_pipe : NULL, &error);
...
static void test_remote_command_line(void)
{
    GDBusConnection *c;
    GFile *file;
    gchar *replies;
    ...
    spawn("exit status: 0\n", NULL, "./cmd", "echo", "--abc", "-d", NULL);

    spawn("exit status: 0\n", NULL, "./cmd", "env", NULL);

    spawn("exit status: 0\n", NULL, "./cmd", "getenv", NULL);

    spawn("print test\n" "exit status: 0\n", NULL, "./cmd", "print", "test", NULL);

    spawn("exit status: 0\n", "printerr test\n", "./cmd", "printerr", "test", NULL);

    spawn("exit status: 0\n", NULL, "./cmd", "file", "foo", NULL);

    spawn("exit status: 0\n", NULL, "./cmd", "properties", NULL);
...
static void test_remote_actions(void)
{
    GDBusConnection *c;
    ...
    spawn("actions quit new action1 action2\n" "exit status: 0\n", NULL, "./actions", "list", NULL);

    spawn("exit status: 0\n", NULL, "./actions", "activate", NULL);

    spawn("exit status: 0\n", NULL, "./actions", "set-state", NULL);
...
static void nonunique_activate(GApplication * application)
{
...
static GApplication *make_app(gboolean non_unique)
{
    GApplication *app;
    gboolean ok;

    app = g_application_new("org.gtk.Test-Application", non_unique ? G_APPLICATION_NON_UNIQUE : 0);
...
static void test_nonunique(void)
{
...
static void properties(void)
{
    ...
    g_object_get(app, "is-registered", &registered, "is-remote", &remote, NULL);

    g_assert(registered);
    g_assert(!remote);

    g_object_set(app, "inactivity-timeout", 1000, NULL);
...
static gboolean release_app(gpointer user_data)
{
    g_application_release(user_data);
    return G_SOURCE_REMOVE;
}

static void nodbus_activate(GApplication * app)
{
...
static void noappid_activate(GApplication * app)
{
...
static gboolean quit_app(gpointer user_data)
{
    quitted = TRUE;
...
static void quit_activate(GApplication * app)
{
    activated = TRUE;
    g_application_hold(app);
...
G_DEFINE_TYPE(TestLocCmdApp, test_loc_cmd_app, G_TYPE_APPLICATION)
     static void test_loc_cmd_app_init(TestLocCmdApp * app)
{
}

static void test_loc_cmd_app_startup(GApplication * app)
{
    g_assert_not_reached();
}

static void test_loc_cmd_app_shutdown(GApplication * app)
{
    g_assert_not_reached();
}

static gboolean test_loc_cmd_app_local_command_line(GApplication * application, gchar *** arguments, gint * exit_status)
{
    return TRUE;
}

static void test_loc_cmd_app_class_init(TestLocCmdAppClass * klass)
{
...
static void test_local_command_line(void)
{
    char *binpath = g_test_build_filename(G_TEST_BUILT, "unimportant", NULL);
    gchar *argv[] = { binpath, "-invalid", NULL };
    GApplication *app;

    app = g_object_new(test_loc_cmd_app_get_type(), "application-id", "org.gtk.Unimportant", "flags", G_APPLICATION_FLAGS_NONE, NULL);
    g_application_run(app, 1, argv);
    g_object_unref(app);
    g_free(binpath);
}

static void test_resource_path(void)
{
    GApplication *app;
...
static gint test_help_command_line(GApplication * app, GApplicationCommandLine * command_line, gpointer user_data)
{
...
static gint test_local_options(GApplication * app, GVariantDict * options, gpointer data)
{
...
static gint second_handler(GApplication * app, GVariantDict * options, gpointer data)
{
    gboolean *called = data;
...
static void test_handle_local_options_success(void)
{
    if (g_test_subprocess())
    {
        char *binpath = g_test_build_filename(G_TEST_BUILT, "unimportant", NULL);
...
static void test_handle_local_options_failure(void)
{
    if (g_test_subprocess())
    {
        char *binpath = g_test_build_filename(G_TEST_BUILT, "unimportant", NULL);
...
static void test_handle_local_options_passthrough(void)
{
    if (g_test_subprocess())
    {
        char *binpath = g_test_build_filename(G_TEST_BUILT, "unimportant", NULL);
