...
static void per_thread_thing_finalize(GObject * object)
{
...
static void per_thread_thing_init(PerThreadThing * thing)
{
}

static void per_thread_thing_class_init(PerThreadThingClass * class)
{
    class->finalize = per_thread_thing_finalize;

    g_signal_new("changed", per_thread_thing_get_type(), G_SIGNAL_RUN_FIRST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static gpointer per_thread_thing_get(void)
{
    return g_context_specific_group_get(&group, per_thread_thing_get_type(), G_STRUCT_OFFSET(PerThreadThing, context), start_func);
}

static gpointer test_identity_thread(gpointer user_data)
{
...
static void test_context_specific_identity(void)
{
...
static void changed_emitted(PerThreadThing * thing, gpointer user_data)
{
...
static gpointer test_emit_thread(gpointer user_data)
{
    gint *observed_value = user_data;
    GMainContext *my_context;
    gpointer instance;
...
static void test_context_specific_emit(void)
{
    GThread *threads[N_THREADS];
    gboolean exited = FALSE;
    guint i, n;
