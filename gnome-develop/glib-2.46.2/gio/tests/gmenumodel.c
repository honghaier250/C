...
static GString *g_menu_markup_print_string(GString * string, GMenuModel * model, gint indent, gint tabstop)
{
...
static TestItem *test_item_new(GHashTable * attributes, GHashTable * links)
{
    TestItem *item;
...
static void test_item_free(gpointer data)
{
    TestItem *item = data;
...
static gboolean random_menu_is_mutable(GMenuModel * model)
{
    return TRUE;
}

static gint random_menu_get_n_items(GMenuModel * model)
{
...
static void random_menu_get_item_attributes(GMenuModel * model, gint position, GHashTable ** table)
{
    RandomMenu *menu = (RandomMenu *) model;
    TestItem *item;
...
static void random_menu_get_item_links(GMenuModel * model, gint position, GHashTable ** table)
{
    RandomMenu *menu = (RandomMenu *) model;
    TestItem *item;
...
static void random_menu_finalize(GObject * object)
{
    RandomMenu *menu = (RandomMenu *) object;

    g_sequence_free(menu->items);

    G_OBJECT_CLASS(random_menu_parent_class)->finalize(object);
}

static void random_menu_init(RandomMenu * menu)
{
}

static void random_menu_class_init(GMenuModelClass * class)
{
...
static void random_menu_change(RandomMenu * menu, GRand * rand)
{
...
static gboolean mirror_menu_is_mutable(GMenuModel * model)
{
...
static gint mirror_menu_get_n_items(GMenuModel * model)
{
    MirrorMenu *menu = (MirrorMenu *) model;
...
static void mirror_menu_get_item_attributes(GMenuModel * model, gint position, GHashTable ** table)
{
    MirrorMenu *menu = (MirrorMenu *) model;
    TestItem *item;
...
static void mirror_menu_get_item_links(GMenuModel * model, gint position, GHashTable ** table)
{
    MirrorMenu *menu = (MirrorMenu *) model;
    TestItem *item;
...
static void mirror_menu_finalize(GObject * object)
{
    ...
    G_OBJECT_CLASS(mirror_menu_parent_class)->finalize(object);
}

static void mirror_menu_init(MirrorMenu * menu)
{
}

static void mirror_menu_class_init(GMenuModelClass * class)
{
    GObjectClass *object_class = G_OBJECT_CLASS(class);
...
static void mirror_menu_changed(GMenuModel * model, gint position, gint removed, gint added, gpointer user_data)
{
...
static gboolean check_menus_equal(GMenuModel * a, GMenuModel * b)
{
    gboolean equal = TRUE;
    gint a_n, b_n;
    gint i;
...
static void assert_menus_equal(GMenuModel * a, GMenuModel * b)
{
...
static void assert_menuitem_equal(GMenuItem * item, GMenuModel * model, gint index)
{
    GMenuAttributeIter *attr_iter;
    GMenuLinkIter *link_iter;
    const gchar *name;
...
static gboolean roundtrip_step(gpointer data)
{
    struct roundtrip_state *state = data;

    if (check_menus_equal(G_MENU_MODEL(state->random), G_MENU_MODEL(state->proxy)) && check_menus_equal(G_MENU_MODEL(state->random), G_MENU_MODEL(state->proxy_mirror)))
    {
        state->success++;
        state->count = 0;
...
static void test_dbus_roundtrip(void)
{
...
static void items_changed(GMenuModel * model, gint position, gint removed, gint added, gpointer data)
{
    items_changed_count++;
}
...
static void test_dbus_subscriptions(void)
{
    GDBusConnection *bus;
    GMenu *menu;
    GDBusMenuModel *proxy;
    ...
    g_signal_connect(proxy, "items-changed", G_CALLBACK(items_changed), NULL);
...
static gpointer do_modify(gpointer data)
{
...
static void test_attribute_iter(void)
{
    GMenu *menu;
    GMenuItem *item;
    const gchar *name;
