...
G_DEFINE_TYPE(TestApplication, test_application, G_TYPE_APPLICATION)
     static gboolean test_application_dbus_register(GApplication * application, GDBusConnection * connection, const gchar * object_path, GError ** error)
{
    /* We must chain up to the parent class */
    if (!G_APPLICATION_CLASS(test_application_parent_class)->dbus_register(application, connection, object_path, error))
        return FALSE;
...
static void test_application_dbus_unregister(GApplication * application, GDBusConnection * connection, const gchar * object_path)
{
    ...
    G_APPLICATION_CLASS(test_application_parent_class)->dbus_unregister(application, connection, object_path);
}
...
static GApplication *test_application_new(const gchar * application_id, GApplicationFlags flags)
{
