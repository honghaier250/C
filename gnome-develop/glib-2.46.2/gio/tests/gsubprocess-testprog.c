...
static void write_all(int fd, const guint8 * buf, gsize len)
{
    while (len > 0)
    {
        gssize bytes_written = write(fd, buf, len);
        if (bytes_written < 0)
            g_error("Failed to write to fd %d: %s", fd, strerror(errno));
        buf += bytes_written;
        len -= bytes_written;
    }
}

static int echo_mode(int argc, char **argv)
{
    int i;
...
static int echo_stdout_and_stderr_mode(int argc, char **argv)
{
    int i;
...
static int cat_mode(int argc, char **argv)
{
    ...
    {
        do
            status = g_io_channel_read_chars(chan_stdin, buf, sizeof(buf), &bytes_read, error);
        ...
        do
            status = g_io_channel_write_chars(chan_stdout, buf, bytes_read, &bytes_written, error);
        while (status == G_IO_STATUS_AGAIN);
...
static gint sleep_forever_mode(int argc, char **argv)
{
    GMainLoop *loop;
...
static int write_to_fds(int argc, char **argv)
{
    int i;
...
static int env_mode(int argc, char **argv)
{
    char **env;
    int i;
...
static int cwd_mode(int argc, char **argv)
{
    char *cwd;
...
static int printenv_mode(int argc, char **argv)
{
    gint i;
