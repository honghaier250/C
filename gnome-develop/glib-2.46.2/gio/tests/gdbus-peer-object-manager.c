...
static void mock_interface_init(MockInterface * self)
{

}

static GDBusInterfaceInfo *mock_interface_get_info(GDBusInterfaceSkeleton * skeleton)
{
...
static GVariant *mock_interface_get_property(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * property_name, GError ** error, gpointer user_data)
{
...
static GDBusInterfaceVTable *mock_interface_get_vtable(GDBusInterfaceSkeleton * interface)
{
...
static GVariant *mock_interface_get_properties(GDBusInterfaceSkeleton * interface)
{
    ...
    for (n = 0; info->properties[n] != NULL; n++)
    {
        if (info->properties[n]->flags & G_DBUS_PROPERTY_INFO_FLAGS_READABLE)
        {
            ...
            value = (vtable->get_property) (g_dbus_interface_skeleton_get_connection(interface), NULL, g_dbus_interface_skeleton_get_object_path(interface), info->name, info->properties[n]->name, NULL, interface);
            if (value != NULL)
            {
...
static void mock_interface_flush(GDBusInterfaceSkeleton * skeleton)
{

}

static void mock_interface_class_init(MockInterfaceClass * klass)
{
...
static void on_server_connection(GObject * source, GAsyncResult * result, gpointer user_data)
{
    Test *test = user_data;
    GError *error = NULL;
...
static void on_client_connection(GObject * source, GAsyncResult * result, gpointer user_data)
{
    Test *test = user_data;
    GError *error = NULL;
...
static void setup(Test * test, gconstpointer unused)
{
    GError *error = NULL;
    GSocket *socket;
    ...
    g_dbus_connection_new(G_IO_STREAM(stream), guid, G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_SERVER | G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_ALLOW_ANONYMOUS, NULL, NULL, on_server_connection, test);
    ...
    g_dbus_connection_new(G_IO_STREAM(stream), NULL, G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_CLIENT | G_DBUS_CONNECTION_FLAGS_AUTHENTICATION_ALLOW_ANONYMOUS, NULL, NULL, on_client_connection, test);
...
static void teardown(Test * test, gconstpointer unused)
{
...
static void on_result(GObject * source, GAsyncResult * result, gpointer user_data)
{
    Test *test = user_data;
...
static void test_object_manager(Test * test, gconstpointer unused)
{
    ...
    g_dbus_object_manager_client_new(test->client, G_DBUS_OBJECT_MANAGER_CLIENT_FLAGS_DO_NOT_AUTO_START, dbus_name, "/objects", NULL, NULL, NULL, NULL, on_result, test);
