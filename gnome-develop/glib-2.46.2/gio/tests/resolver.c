...
static void print_resolved_service(const char *service, GList * targets, GError * error)
{
    ...
    else
    {
        ...
        {
            printf("%s:%u (pri %u, weight %u)\n", g_srv_target_get_hostname(t->data), (guint) g_srv_target_get_port(t->data), (guint) g_srv_target_get_priority(t->data), (guint) g_srv_target_get_weight(t->data));
...
static void print_resolved_soa(const char *rrname, GList * records, GError * error)
{
    ...
    else
    {
        for (t = records; t; t = t->next)
        {
            g_variant_get(t->data, "(&s&suuuuu)", &primary_ns, &administrator, &serial, &refresh, &retry, &expire, &ttl);
            printf("%s %s (serial %u, refresh %u, retry %u, expire %u, ttl %u)\n", primary_ns, administrator, (guint) serial, (guint) refresh, (guint) retry, (guint) expire, (guint) ttl);
            g_variant_unref(t->data);
        }
        g_list_free(records);
    }
...
static gboolean record_type_arg(const gchar * option_name, const gchar * value, gpointer data, GError ** error)
{
    ...
    {
        g_set_error(error, G_OPTION_ERROR, G_OPTION_ERROR_BAD_VALUE, "Specify MX, TXT, NS or SOA for the special record lookup types");
        return FALSE;
    }
