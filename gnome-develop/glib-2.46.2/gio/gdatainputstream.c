     ...
     static void g_data_input_stream_class_init(GDataInputStreamClass * klass)
{
   ...
   */
    g_object_class_install_property(object_class,
                                    PROP_BYTE_ORDER,
                                    g_param_spec_enum("byte-order", P_("Byte order"), P_("The byte order"), G_TYPE_DATA_STREAM_BYTE_ORDER, G_DATA_STREAM_BYTE_ORDER_BIG_ENDIAN, G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB));
   ...
   */
    g_object_class_install_property(object_class,
                                    ...
                                    g_param_spec_enum("newline-type",
                                                      P_("Newline type"), P_("The accepted types of line ending"), G_TYPE_DATA_STREAM_NEWLINE_TYPE, G_DATA_STREAM_NEWLINE_TYPE_LF, G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB));
}
...
static gboolean read_data(GDataInputStream * stream, void *buffer, gsize size, GCancellable * cancellable, GError ** error)
{
    ...
    while ((available = g_buffered_input_stream_get_available(G_BUFFERED_INPUT_STREAM(stream))) < size)
    {
        ...
        if (res == 0)
        {
            g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Unexpected early end-of-stream"));
            return FALSE;
        }
    }
...
static void g_data_input_stream_read_complete(GTask * task, gsize read_length, gsize skip_length)
{
...
static void g_data_input_stream_read_line_ready(GObject * object, GAsyncResult * result, gpointer user_data)
{
    ...
    if (data->stop_chars)
    {
        found_pos = scan_for_chars(G_DATA_INPUT_STREAM(buffer), &data->checked, data->stop_chars, data->stop_chars_len);
        newline_len = 0;
    }
    else
        found_pos = scan_for_newline(G_DATA_INPUT_STREAM(buffer), &data->checked, &data->last_saw_cr, &newline_len);
        ...
        /* didn't find a full line; need to buffer some more bytes */
    {
        ...
        /* try again */
        g_buffered_input_stream_fill_async(buffer, -1, g_task_get_priority(task), g_task_get_cancellable(task), g_data_input_stream_read_line_ready, user_data);
    }
    else
    {
...
static void g_data_input_stream_read_async(GDataInputStream * stream, const gchar * stop_chars, gssize stop_chars_len, gint io_priority, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
...
void g_data_input_stream_read_line_async(GDataInputStream * stream, gint io_priority, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    g_data_input_stream_read_async(stream, NULL, 0, io_priority, cancellable, callback, user_data);
}
...
void g_data_input_stream_read_until_async(GDataInputStream * stream, const gchar * stop_chars, gint io_priority, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    g_data_input_stream_read_async(stream, stop_chars, -1, io_priority, cancellable, callback, user_data);
}
 ...
 * Since: 2.30
 */
gchar *g_data_input_stream_read_line_finish_utf8(GDataInputStream * stream, GAsyncResult * result, gsize * length, GError ** error)
{
    gchar *res;
...
void g_data_input_stream_read_upto_async(GDataInputStream * stream, const gchar * stop_chars, gssize stop_chars_len, gint io_priority, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    g_data_input_stream_read_async(stream, stop_chars, stop_chars_len, io_priority, cancellable, callback, user_data);
}
