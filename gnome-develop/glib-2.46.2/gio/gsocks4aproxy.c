...
G_DEFINE_TYPE_WITH_CODE(GSocks4aProxy, g_socks4a_proxy, G_TYPE_OBJECT,
                        G_IMPLEMENT_INTERFACE(G_TYPE_PROXY, g_socks4a_proxy_iface_init) _g_io_modules_ensure_extension_points_registered(); g_io_extension_point_implement(G_PROXY_EXTENSION_POINT_NAME, g_define_type_id, "socks4a", 0))
...
static gint set_connect_msg(guint8 * msg, const gchar * hostname, guint16 port, const char *username, GError ** error)
{
    ...
    if (username)
    {
        ...
        {
            g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_PROXY_FAILED, _("Username is too long for SOCKSv4 protocol"));
            return -1;
        }
    ...
    if (!is_ip)
    {
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_PROXY_FAILED, _("Hostname '%s' is too long for SOCKSv4 protocol"), hostname);
            return -1;
        }
...
static GIOStream *g_socks4a_proxy_connect(GProxy * proxy, GIOStream * io_stream, GProxyAddress * proxy_address, GCancellable * cancellable, GError ** error)
{
    ...
    /* Read SOCKS4 response */
    {
        ...
        if (!g_input_stream_read_all(in, data, SOCKS4_CONN_REP_LEN, NULL, cancellable, error))
            goto error;
...
static void g_socks4a_proxy_connect_async(GProxy * proxy, GIOStream * io_stream, GProxyAddress * proxy_address, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    GError *error = NULL;
    GTask *task;
