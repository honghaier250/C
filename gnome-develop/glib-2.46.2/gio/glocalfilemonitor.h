...
struct _GLocalFileMonitorClass
{
    ...
    void (*start) (GLocalFileMonitor * local_monitor, const gchar * dirname, const gchar * basename, const gchar * filename, GFileMonitorSource * source);
...
/* for various users in glib */
     typedef void (*GFileMonitorCallback) (GFileMonitor * monitor, GFile * child, GFile * other, GFileMonitorEvent event, gpointer user_data);
...
GLIB_AVAILABLE_IN_2_44 gboolean g_file_monitor_source_handle_event(GFileMonitorSource * fms, GFileMonitorEvent event_type, const gchar * child, const gchar * rename_to, GFile * other, gint64 event_time);
