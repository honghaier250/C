...
G_DEFINE_TYPE_WITH_CODE(GDBusObjectSkeleton, g_dbus_object_skeleton, G_TYPE_OBJECT, G_ADD_PRIVATE(GDBusObjectSkeleton) G_IMPLEMENT_INTERFACE(G_TYPE_DBUS_OBJECT, dbus_object_interface_init))
...
static void g_dbus_object_skeleton_get_property(GObject * _object, guint prop_id, GValue * value, GParamSpec * pspec)
{
    GDBusObjectSkeleton *object = G_DBUS_OBJECT_SKELETON(_object);
...
static void g_dbus_object_skeleton_set_property(GObject * _object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    GDBusObjectSkeleton *object = G_DBUS_OBJECT_SKELETON(_object);
...
static gboolean g_dbus_object_skeleton_authorize_method_default(GDBusObjectSkeleton * object, GDBusInterfaceSkeleton * interface, GDBusMethodInvocation * invocation)
{
    return TRUE;
}
...
static void g_dbus_object_skeleton_class_init(GDBusObjectSkeletonClass * klass)
{
   ...
   */
    g_object_class_install_property(gobject_class,
                                    PROP_G_OBJECT_PATH, g_param_spec_string("g-object-path", "Object Path", "The object path where the object is exported", NULL, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS));
   ...
   */
    signals[AUTHORIZE_METHOD_SIGNAL] =
        g_signal_new(I_("authorize-method"),
                     ...
                     G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GDBusObjectSkeletonClass, authorize_method), _g_signal_accumulator_false_handled, NULL, NULL, G_TYPE_BOOLEAN, 2, G_TYPE_DBUS_INTERFACE_SKELETON, G_TYPE_DBUS_METHOD_INVOCATION);
}
...
static void g_dbus_object_skeleton_init(GDBusObjectSkeleton * object)
{
    ...
    object->priv->map_name_to_iface = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, (GDestroyNotify) g_object_unref);
}
...
GDBusObjectSkeleton *g_dbus_object_skeleton_new(const gchar * object_path)
{
    ...
    return G_DBUS_OBJECT_SKELETON(g_object_new(G_TYPE_DBUS_OBJECT_SKELETON, "g-object-path", object_path, NULL));
}
...
void g_dbus_object_skeleton_add_interface(GDBusObjectSkeleton * object, GDBusInterfaceSkeleton * interface_)
{
    ...
    g_hash_table_insert(object->priv->map_name_to_iface, g_strdup(info->name), g_object_ref(interface_));
...
void g_dbus_object_skeleton_remove_interface(GDBusObjectSkeleton * object, GDBusInterfaceSkeleton * interface_)
{
    ...
    {
        g_mutex_unlock(&object->priv->lock);
        g_warning("Tried to remove interface with name %s from object " "at path %s but no such interface exists", info->name, object->priv->object_path);
    }
    else if (other_interface != interface_)
    {
        g_mutex_unlock(&object->priv->lock);
        g_warning("Tried to remove interface %p with name %s from object " "at path %s but the object has the interface %p", interface_, info->name, object->priv->object_path, other_interface);
    }
    else
    {
        g_object_ref(interface_);
...
static GDBusInterface *g_dbus_object_skeleton_get_interface(GDBusObject * _object, const gchar * interface_name)
{
    GDBusObjectSkeleton *object = G_DBUS_OBJECT_SKELETON(_object);
    GDBusInterface *ret;
...
gboolean _g_dbus_object_skeleton_has_authorize_method_handlers(GDBusObjectSkeleton * object)
{
    ...
    has_handlers = g_signal_has_handler_pending(object, signals[AUTHORIZE_METHOD_SIGNAL], 0, TRUE);
    has_default_class_handler = (G_DBUS_OBJECT_SKELETON_GET_CLASS(object)->authorize_method == g_dbus_object_skeleton_authorize_method_default);
