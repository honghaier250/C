...
G_DEFINE_TYPE_WITH_CODE(GTcpConnection, g_tcp_connection,
                        ...
                        g_socket_connection_factory_register_type(g_define_type_id,
                                                                  G_SOCKET_FAMILY_IPV4,
                                                                  G_SOCKET_TYPE_STREAM, G_SOCKET_PROTOCOL_TCP); g_socket_connection_factory_register_type(g_define_type_id, G_SOCKET_FAMILY_IPV6, G_SOCKET_TYPE_STREAM, G_SOCKET_PROTOCOL_TCP););
...
static void g_tcp_connection_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    ...
    {
        case PROP_GRACEFUL_DISCONNECT:
            g_tcp_connection_set_graceful_disconnect(connection, g_value_get_boolean(value));
            break;
...
static void g_tcp_connection_class_init(GTcpConnectionClass * class)
{
    ...
    g_object_class_install_property(gobject_class, PROP_GRACEFUL_DISCONNECT,
                                    g_param_spec_boolean("graceful-disconnect", P_("Graceful Disconnect"), P_("Whether or not close does a graceful disconnect"), FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
static gboolean g_tcp_connection_close(GIOStream * stream, GCancellable * cancellable, GError ** error)
{
    ...
    if (connection->priv->graceful_disconnect && !g_cancellable_is_cancelled(cancellable) /* Cancelled -> close fast */ )
    {
        ...
        else
        {
            while (TRUE)
            {
                ret = g_socket_receive_with_blocking(socket, buffer, sizeof(buffer), TRUE, cancellable, error);
                if (ret < 0)
                {
                    had_error = TRUE;
                    error = NULL;
...
static gboolean close_read_ready(GSocket * socket, GIOCondition condition, GTask * task)
{
    GError *error = NULL;
    char buffer[1024];
    gssize ret;

    ret = g_socket_receive_with_blocking(socket, buffer, sizeof(buffer), FALSE, g_task_get_cancellable(task), &error);
    if (ret < 0)
    {
