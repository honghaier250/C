...
static void list_resource(GResource * resource, const gchar * path, const gchar * section, const gchar * prefix, gboolean details)
{
    gchar **children;
    gsize size;
    guint32 flags;
...
typedef gboolean(*SectionCallback) (GElf_Shdr * shdr, const gchar * name, gpointer data);

static void elf_foreach_resource_section(Elf * elf, SectionCallback callback, gpointer data)
{
    ...
    for (scnidx = 1; scnidx < shnum; scnidx++)
    {
        ...
        if (section_name == NULL || !g_str_has_prefix(section_name, ".gresource."))
            continue;
...
static gboolean list_resources_cb(GElf_Shdr * shdr, const gchar * section, gpointer data)
{
    CallbackData *d = data;
    GResource *resource;
    ...
    list_resource(resource, "/", d->section ? "" : section, d->path, d->details);
...
static void elf_list_resources(Elf * elf, int fd, const gchar * section, const gchar * path, gboolean details)
{
    CallbackData data;
...
static gboolean extract_resource_cb(GElf_Shdr * shdr, const gchar * section, gpointer data)
{
    CallbackData *d = data;
    GResource *resource;
...
static void elf_extract_resource(Elf * elf, int fd, const gchar * section, const gchar * path)
{
    CallbackData data;
...
static gboolean print_section_name(GElf_Shdr * shdr, const gchar * name, gpointer data)
{
    g_print("%s\n", name);
    return TRUE;
}
...
static void cmd_sections(const gchar * file, const gchar * section, const gchar * path, gboolean details)
{
    GResource *resource;
...
static void cmd_list(const gchar * file, const gchar * section, const gchar * path, gboolean details)
{
    GResource *resource;
...
static void cmd_extract(const gchar * file, const gchar * section, const gchar * path, gboolean details)
{
    GResource *resource;
...
static gint cmd_help(gboolean requested, const gchar * command)
{
    ...
    else
    {
        ...
        if (option)
            g_string_append(string, _("  SECTION   An (optional) elf section name\n"));

        if (strstr(synopsis, _("[COMMAND]")))
            g_string_append(string, _("  COMMAND   The (optional) command to explain\n"));

        if (strstr(synopsis, _("FILE")))
        {
            if (strcmp(command, "sections") == 0)
                g_string_append(string, _("  FILE      An elf file (a binary or a shared library)\n"));
            else
                g_string_append(string, _("  FILE      An elf file (a binary or a shared library)\n" "            or a compiled resource file\n"));
        }

        if (strstr(synopsis, _("[PATH]")))
            g_string_append(string, _("  PATH      An (optional) resource path (may be partial)\n"));
