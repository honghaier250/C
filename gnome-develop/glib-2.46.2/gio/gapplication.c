...
G_DEFINE_TYPE_WITH_CODE(GApplication, g_application, G_TYPE_OBJECT,
                        G_ADD_PRIVATE(GApplication) G_IMPLEMENT_INTERFACE(G_TYPE_ACTION_GROUP, g_application_action_group_iface_init) G_IMPLEMENT_INTERFACE(G_TYPE_ACTION_MAP, g_application_action_map_iface_init))
...
G_DEFINE_TYPE_WITH_CODE(GApplicationExportedActions, g_application_exported_actions, G_TYPE_SIMPLE_ACTION_GROUP, G_IMPLEMENT_INTERFACE(G_TYPE_REMOTE_ACTION_GROUP, g_application_exported_actions_iface_init))
     static void g_application_exported_actions_activate_action_full(GRemoteActionGroup * remote, const gchar * action_name, GVariant * parameter, GVariant * platform_data)
{
    GApplicationExportedActions *exported = (GApplicationExportedActions *) remote;

    G_APPLICATION_GET_CLASS(exported->application)->before_emit(exported->application, platform_data);

    g_action_group_activate_action(G_ACTION_GROUP(exported), action_name, parameter);

    G_APPLICATION_GET_CLASS(exported->application)->after_emit(exported->application, platform_data);
}

static void g_application_exported_actions_change_action_state_full(GRemoteActionGroup * remote, const gchar * action_name, GVariant * value, GVariant * platform_data)
{
    GApplicationExportedActions *exported = (GApplicationExportedActions *) remote;
...
static void g_application_exported_actions_init(GApplicationExportedActions * actions)
{
}
...
static void g_application_exported_actions_class_init(GApplicationExportedActionsClass * class)
{
}

static GActionGroup *g_application_exported_actions_new(GApplication * application)
{
...
static void g_application_pack_option_entries(GApplication * application, GVariantDict * dict)
{
    GHashTableIter iter;
    gpointer item;
...
static GVariantDict *g_application_parse_command_line(GApplication * application, gchar *** arguments, GError ** error)
{
    ...
    while (application->priv->option_groups)
    {
        ...
        application->priv->option_groups = g_slist_delete_link(application->priv->option_groups, application->priv->option_groups);
    }
    ...
    if ((application->priv->flags & (G_APPLICATION_IS_SERVICE | G_APPLICATION_IS_LAUNCHER)) == 0)
    {
        ...
        option_group = g_option_group_new("gapplication", _("GApplication options"), _("Show GApplication options"), NULL, NULL);
...
static void add_packed_option(GApplication * application, GOptionEntry * entry)
{
    ...
    g_hash_table_insert(application->priv->packed_options, g_strdup(entry->long_name), g_slice_dup(GOptionEntry, entry));
}
...
void g_application_add_main_option_entries(GApplication * application, const GOptionEntry * entries)
{
    ...
    {
        GOptionEntry my_entries[2] = { {NULL}
        , {NULL}
        };
...
{
    if (!g_signal_has_handler_pending(application, g_application_signals[SIGNAL_ACTIVATE], 0, TRUE) && G_APPLICATION_GET_CLASS(application)->activate == g_application_real_activate)
    {
        ...
        g_warning("Your application does not implement " "g_application_activate() and has no handlers connected " "to the 'activate' signal.  It should do one of these.");
        warned = TRUE;
    }
}
...
{
    if (!g_signal_has_handler_pending(application, g_application_signals[SIGNAL_OPEN], 0, TRUE) && G_APPLICATION_GET_CLASS(application)->open == g_application_real_open)
    {
        static gboolean warned;
...
{
    if (!g_signal_has_handler_pending(application, g_application_signals[SIGNAL_COMMAND_LINE], 0, TRUE) && G_APPLICATION_GET_CLASS(application)->command_line == g_application_real_command_line)
    {
        static gboolean warned;
        ...
        g_warning("Your application claims to support custom command line " "handling but does not implement g_application_command_line() " "and has no handlers connected to the 'command-line' signal.");
...
static gint g_application_real_handle_local_options(GApplication * application, GVariantDict * options)
{
    return -1;
}
...
static void g_application_call_command_line(GApplication * application, const gchar * const *arguments, GVariant * options, gint * exit_status)
{
    ...
    else
    {
        ...
        cmdline = g_object_new(G_TYPE_APPLICATION_COMMAND_LINE, "arguments", v, "options", options, NULL);
...
static gboolean g_application_real_local_command_line(GApplication * application, gchar *** arguments, int *exit_status)
{
    ...
    {
        g_application_call_command_line(application, (const gchar **) *arguments, g_variant_dict_end(options), exit_status);
    }
    else
    {
        if (n_args <= 1)
...
static gboolean g_application_real_dbus_register(GApplication * application, GDBusConnection * connection, const gchar * object_path, GError ** error)
{
    return TRUE;
}

static void g_application_real_dbus_unregister(GApplication * application, GDBusConnection * connection, const gchar * object_path)
{
}
...
static void g_application_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    ...
    switch (prop_id)
    {
        case PROP_APPLICATION_ID:
            g_application_set_application_id(application, g_value_get_string(value));
            break;
...
static void g_application_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
    ...
    {
        case PROP_APPLICATION_ID:
            g_value_set_string(value, g_application_get_application_id(application));
            break;
        ...
        case PROP_IS_REGISTERED:
            g_value_set_boolean(value, g_application_get_is_registered(application));
            break;

        case PROP_IS_REMOTE:
            g_value_set_boolean(value, g_application_get_is_remote(application));
            break;

        case PROP_INACTIVITY_TIMEOUT:
            g_value_set_uint(value, g_application_get_inactivity_timeout(application));
            break;
...
static void g_application_init(GApplication * application)
{
     ...
     */
    g_signal_connect_swapped(application->priv->actions, "action-added", G_CALLBACK(g_action_group_action_added), application);
    g_signal_connect_swapped(application->priv->actions, "action-enabled-changed", G_CALLBACK(g_action_group_action_enabled_changed), application);
    g_signal_connect_swapped(application->priv->actions, "action-state-changed", G_CALLBACK(g_action_group_action_state_changed), application);
    g_signal_connect_swapped(application->priv->actions, "action-removed", G_CALLBACK(g_action_group_action_removed), application);
}

static gboolean g_application_handle_local_options_accumulator(GSignalInvocationHint * ihint, GValue * return_accu, const GValue * handler_return, gpointer dummy)
{
    gint value;
...
static void g_application_class_init(GApplicationClass * class)
{
    ...
    g_object_class_install_property(object_class, PROP_APPLICATION_ID,
                                    g_param_spec_string("application-id", P_("Application identifier"), P_("The unique identifier for the application"), NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(object_class, PROP_FLAGS,
                                    g_param_spec_flags("flags", P_("Application flags"), P_("Flags specifying the behaviour of the application"), G_TYPE_APPLICATION_FLAGS, G_APPLICATION_FLAGS_NONE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(object_class, PROP_RESOURCE_BASE_PATH, g_param_spec_string("resource-base-path", P_("Resource base path"), P_("The base resource path for the application"), NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(object_class, PROP_IS_REGISTERED, g_param_spec_boolean("is-registered", P_("Is registered"), P_("If g_application_register() has been called"), FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(object_class, PROP_IS_REMOTE, g_param_spec_boolean("is-remote", P_("Is remote"), P_("If this application instance is remote"), FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(object_class, PROP_INACTIVITY_TIMEOUT,
                                    g_param_spec_uint("inactivity-timeout", P_("Inactivity timeout"), P_("Time (ms) to stay alive after becoming idle"), 0, G_MAXUINT, 0, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(object_class, PROP_ACTION_GROUP,
                                    g_param_spec_object("action-group", P_("Action group"), P_("The group of actions that the application exports"), G_TYPE_ACTION_GROUP, G_PARAM_DEPRECATED | G_PARAM_WRITABLE | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 2.44
   */
    g_object_class_install_property(object_class, PROP_IS_BUSY, g_param_spec_boolean("is-busy", P_("Is busy"), P_("If this application is currently marked busy"), FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_application_signals[SIGNAL_STARTUP] = g_signal_new(I_("startup"), G_TYPE_APPLICATION, G_SIGNAL_RUN_FIRST, G_STRUCT_OFFSET(GApplicationClass, startup), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
   ...
   */
    g_application_signals[SIGNAL_SHUTDOWN] = g_signal_new(I_("shutdown"), G_TYPE_APPLICATION, G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GApplicationClass, shutdown), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
   ...
   */
    g_application_signals[SIGNAL_ACTIVATE] = g_signal_new(I_("activate"), G_TYPE_APPLICATION, G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GApplicationClass, activate), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
   ...
   */
    g_application_signals[SIGNAL_OPEN] = g_signal_new(I_("open"), G_TYPE_APPLICATION, G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GApplicationClass, open), NULL, NULL, NULL, G_TYPE_NONE, 3, G_TYPE_POINTER, G_TYPE_INT, G_TYPE_STRING);
   ...
   */
    g_application_signals[SIGNAL_COMMAND_LINE] =
        g_signal_new(I_("command-line"), G_TYPE_APPLICATION, G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GApplicationClass, command_line), g_signal_accumulator_first_wins, NULL, NULL, G_TYPE_INT, 1, G_TYPE_APPLICATION_COMMAND_LINE);
   ...
   **/
    g_application_signals[SIGNAL_HANDLE_LOCAL_OPTIONS] =
        g_signal_new(I_("handle-local-options"), G_TYPE_APPLICATION, G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GApplicationClass, handle_local_options), g_application_handle_local_options_accumulator, NULL, NULL, G_TYPE_INT, 1, G_TYPE_VARIANT_DICT);
...
gboolean g_application_id_is_valid(const gchar * application_id)
{
    ...
    {
        if (g_ascii_isalnum(*application_id) || (*application_id == '-') || (*application_id == '_'))
        {
            allow_dot = TRUE;
        }
...
gboolean g_application_register(GApplication * application, GCancellable * cancellable, GError ** error)
{
    ...
    if (!application->priv->is_registered)
    {
        ...
        application->priv->impl = g_application_impl_register(application, application->priv->id, application->priv->flags, application->priv->actions, &application->priv->remote_actions, cancellable, error);
        ...
        if (!application->priv->is_remote)
        {
            ...
            if (!application->priv->did_startup)
                g_critical("GApplication subclass '%s' failed to chain up on" " ::startup (from start of override function)", G_OBJECT_TYPE_NAME(application));
        }
    }
...
void g_application_activate(GApplication * application)
{
    ...
    if (application->priv->is_remote)
        g_application_impl_activate(application->priv->impl, get_platform_data(application, NULL));
...
void g_application_open(GApplication * application, GFile ** files, gint n_files, const gchar * hint)
{
    ...
    if (application->priv->is_remote)
        g_application_impl_open(application->priv->impl, files, n_files, hint, get_platform_data(application, NULL));
...
int g_application_run(GApplication * application, int argc, char **argv)
{
    ...
    if (application->priv->flags & G_APPLICATION_IS_SERVICE && application->priv->is_registered && !application->priv->use_count && !application->priv->inactivity_timeout_id)
    {
    ...
    if (application->priv->is_registered && !application->priv->is_remote)
    {
        ...
        if (!application->priv->did_shutdown)
            g_critical("GApplication subclass '%s' failed to chain up on" " ::shutdown (from end of override function)", G_OBJECT_TYPE_NAME(application));
    }
...
static gboolean g_application_query_action(GActionGroup * group, const gchar * action_name, gboolean * enabled, const GVariantType ** parameter_type, const GVariantType ** state_type, GVariant ** state_hint, GVariant ** state)
{
    ...
    if (application->priv->remote_actions != NULL)
        return g_action_group_query_action(G_ACTION_GROUP(application->priv->remote_actions), action_name, enabled, parameter_type, state_type, state_hint, state);

    if (application->priv->actions != NULL)
        return g_action_group_query_action(application->priv->actions, action_name, enabled, parameter_type, state_type, state_hint, state);
...
static void g_application_change_action_state(GActionGroup * action_group, const gchar * action_name, GVariant * value)
{
    ...
    g_return_if_fail(application->priv->is_remote || application->priv->actions != NULL);
    g_return_if_fail(application->priv->is_registered);

    if (application->priv->remote_actions)
        g_remote_action_group_change_action_state_full(application->priv->remote_actions, action_name, value, get_platform_data(application, NULL));
...
static void g_application_activate_action(GActionGroup * action_group, const gchar * action_name, GVariant * parameter)
{
    ...
    if (application->priv->remote_actions)
        g_remote_action_group_activate_action_full(application->priv->remote_actions, action_name, parameter, get_platform_data(application, NULL));

    else
        g_action_group_activate_action(application->priv->actions, action_name, parameter);
}

static GAction *g_application_lookup_action(GActionMap * action_map, const gchar * action_name)
{
...
static void g_application_remove_action(GActionMap * action_map, const gchar * action_name)
{
    GApplication *application = G_APPLICATION(action_map);
...
static void g_application_notify_busy_binding(GObject * object, GParamSpec * pspec, gpointer user_data)
{
...
void g_application_bind_busy_property(GApplication * application, gpointer object, const gchar * property)
{
    ...
    if (g_signal_handler_find(object, G_SIGNAL_MATCH_ID | G_SIGNAL_MATCH_DETAIL | G_SIGNAL_MATCH_FUNC, notify_id, property_quark, NULL, g_application_notify_busy_binding, NULL) > 0)
    {
    ...
    closure = g_cclosure_new(G_CALLBACK(g_application_notify_busy_binding), binding, g_application_busy_binding_destroy);
...
void g_application_unbind_busy_property(GApplication * application, gpointer object, const gchar * property)
{
    ...
    handler_id = g_signal_handler_find(object, G_SIGNAL_MATCH_ID | G_SIGNAL_MATCH_DETAIL | G_SIGNAL_MATCH_FUNC, notify_id, property_quark, NULL, g_application_notify_busy_binding, NULL);
    if (handler_id == 0)
    {
