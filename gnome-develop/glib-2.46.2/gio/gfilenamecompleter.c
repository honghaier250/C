...
static void g_filename_completer_class_init(GFilenameCompleterClass * klass)
{
   ...
   **/
    signals[GOT_COMPLETION_DATA] = g_signal_new(I_("got-completion-data"), G_TYPE_FILENAME_COMPLETER, G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GFilenameCompleterClass, got_completion_data), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
...
static void got_more_files(GObject * source_object, GAsyncResult * res, gpointer user_data)
{
    ...
    for (l = infos; l != NULL; l = l->next)
    {
        ...
        if (data->should_escape)
            basename = g_uri_escape_string(name, G_URI_RESERVED_CHARS_ALLOWED_IN_PATH, TRUE);
        else
...
char **g_filename_completer_get_completions(GFilenameCompleter * completer, const char *initial_text)
{
    ...
    for (l = possible_matches; l != NULL; l = l->next)
    {
        ...
        if (g_str_has_prefix(possible_match, prefix))
            g_ptr_array_add(res, g_strconcat(initial_text, possible_match + strlen(prefix), NULL));
    }
