...
G_DEFINE_TYPE_WITH_PRIVATE(GDelayedSettingsBackend, g_delayed_settings_backend, G_TYPE_SETTINGS_BACKEND)
...
static GVariant *g_delayed_settings_backend_read(GSettingsBackend * backend, const gchar * key, const GVariantType * expected_type, gboolean default_value)
{
    ...
    if (result == NULL)
        result = g_settings_backend_read(delayed->priv->backend, key, expected_type, default_value);

    return result;
}

static GVariant *g_delayed_settings_backend_read_user_value(GSettingsBackend * backend, const gchar * key, const GVariantType * expected_type)
{
    GDelayedSettingsBackend *delayed = G_DELAYED_SETTINGS_BACKEND(backend);
...
static void delayed_backend_changed(GObject * target, GSettingsBackend * backend, const gchar * key, gpointer origin_tag)
{
    ...
    if (origin_tag != delayed->priv)
        g_settings_backend_changed(G_SETTINGS_BACKEND(delayed), key, origin_tag);
}

static void delayed_backend_keys_changed(GObject * target, GSettingsBackend * backend, const gchar * path, gpointer origin_tag, const gchar * const *items)
{
    GDelayedSettingsBackend *delayed = G_DELAYED_SETTINGS_BACKEND(target);

    if (origin_tag != delayed->priv)
        g_settings_backend_keys_changed(G_SETTINGS_BACKEND(delayed), path, items, origin_tag);
}
...
static void delayed_backend_path_changed(GObject * target, GSettingsBackend * backend, const gchar * path, gpointer origin_tag)
{
    ...
    if (origin_tag != delayed->priv)
        g_settings_backend_path_changed(G_SETTINGS_BACKEND(delayed), path, origin_tag);
}
...
static void delayed_backend_writable_changed(GObject * target, GSettingsBackend * backend, const gchar * key)
{
    ...
    if (g_tree_lookup(delayed->priv->delayed, key) != NULL && !g_settings_backend_get_writable(delayed->priv->backend, key))
    {
...
static void delayed_backend_path_writable_changed(GObject * target, GSettingsBackend * backend, const gchar * path)
{
    ...
    if (n_keys > 0)
    {
        ...
        for (i = 0; i < state.index; i++)
            if (g_tree_lookup(delayed->priv->delayed, state.keys[i]) != NULL && !g_settings_backend_get_writable(delayed->priv->backend, state.keys[i]))
