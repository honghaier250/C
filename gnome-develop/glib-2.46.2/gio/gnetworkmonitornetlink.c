...
static gboolean read_netlink_messages(GSocket * socket, GIOCondition condition, gpointer user_data);
...
G_DEFINE_TYPE_WITH_CODE(GNetworkMonitorNetlink, g_network_monitor_netlink, G_TYPE_NETWORK_MONITOR_BASE,
                        ...
                        G_IMPLEMENT_INTERFACE(G_TYPE_INITABLE,
                                              g_network_monitor_netlink_initable_iface_init) _g_io_modules_ensure_extension_points_registered(); g_io_extension_point_implement(G_NETWORK_MONITOR_EXTENSION_POINT_NAME, g_define_type_id, "netlink", 20))
...
static gboolean g_network_monitor_netlink_initable_init(GInitable * initable, GCancellable * cancellable, GError ** error)
{
    ...
    {
        int errsv = errno;
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), _("Could not create network monitor: %s"), g_strerror(errno));
        return FALSE;
    }
    ...
    if (!g_socket_set_option(nl->priv->sock, SOL_SOCKET, SO_PASSCRED, TRUE, NULL))
    {
        int errsv = errno;
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), _("Could not create network monitor: %s"), g_strerror(errno));
        return FALSE;
    ...
    g_source_set_callback(nl->priv->source, (GSourceFunc) read_netlink_messages, nl, NULL);
    g_source_attach(nl->priv->source, g_main_context_get_thread_default());
...
static void queue_request_dump(GNetworkMonitorNetlink * nl)
{
    ...
    g_source_set_callback(nl->priv->dump_source, (GSourceFunc) timeout_request_dump, nl, NULL);
    g_source_attach(nl->priv->dump_source, g_main_context_get_thread_default());
}

static void add_network(GNetworkMonitorNetlink * nl, GSocketFamily family, gint dest_len, guint8 * dest)
{
...
static void remove_network(GNetworkMonitorNetlink * nl, GSocketFamily family, gint dest_len, guint8 * dest)
{
    GInetAddress *dest_addr;
    GInetAddressMask *network;
...
{
    g_network_monitor_base_set_networks(G_NETWORK_MONITOR_BASE(nl), (GInetAddressMask **) nl->priv->dump_networks->pdata, nl->priv->dump_networks->len);
...
static gboolean read_netlink_messages(GSocket * socket, GIOCondition condition, gpointer user_data)
{
    ...
    len = g_socket_receive_message(nl->priv->sock, NULL, &iv, 1, NULL, NULL, &flags, NULL, &error);
    if (len < 0)
    {
    ...
    iv.size = len;
    len = g_socket_receive_message(nl->priv->sock, &addr, &iv, 1, NULL, NULL, NULL, NULL, &error);
    if (len < 0)
    {
        g_warning("Error on netlink socket: %s", error->message);
        g_error_free(error);
...
#define UNALIGNED_IN6_IS_ADDR_MC_LINKLOCAL(a)           \
              ((a[0] == 0xff) && ((a[1] & 0xf) == 0x2))
                    ...
                    if (!nl->priv->dump_networks && rtmsg->rtm_family == AF_INET6 && rtmsg->rtm_dst_len != 0 && UNALIGNED_IN6_IS_ADDR_MC_LINKLOCAL(dest))
                        continue;
...
static void g_network_monitor_netlink_class_init(GNetworkMonitorNetlinkClass * nl_class)
{
...
static void g_network_monitor_netlink_iface_init(GNetworkMonitorInterface * monitor_iface)
{
}
