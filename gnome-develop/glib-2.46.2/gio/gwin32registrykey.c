...
typedef NTSTATUS(NTAPI * NtQueryKeyFunc) (HANDLE key_handle, KEY_INFORMATION_CLASS key_info_class, PVOID key_info_buffer, ULONG key_info_buffer_size, PULONG result_size);

typedef NTSTATUS
    (*NtNotifyChangeMultipleKeysFunc) (HANDLE key_handle,
                                       ULONG subkey_count,
                                       POBJECT_ATTRIBUTES subkeys, HANDLE event, PIO_APC_ROUTINE apc_routine, PVOID apc_closure, PIO_STATUS_BLOCK status_block, ULONG filter, BOOLEAN watch_tree, PVOID buffer, ULONG buffer_size, BOOLEAN async);
...
G_DEFINE_BOXED_TYPE(GWin32RegistrySubkeyIter, g_win32_registry_subkey_iter, g_win32_registry_subkey_iter_copy, g_win32_registry_subkey_iter_free)
/**
     ...
     GWin32RegistryValueIter *g_win32_registry_value_iter_copy(const GWin32RegistryValueIter * iter)
{
    ...
    if (iter->value_data_expanded != NULL)
        new_iter->value_data_expanded = g_wcsdup((gunichar2 *) iter->value_data_expanded, iter->value_data_expanded_charsize * sizeof(gunichar2));

    new_iter->value_data_expanded_charsize = iter->value_data_expanded_charsize;

    if (iter->value_data_expanded_u8 != NULL)
        new_iter->value_data_expanded_u8 = g_memdup(iter->value_data_expanded_u8, iter->value_data_expanded_charsize);
 ...
 **/
     ...
     static gboolean g_win32_registry_key_initable_init(GInitable * initable, GCancellable * cancellable, GError ** error);

G_DEFINE_TYPE_WITH_CODE(GWin32RegistryKey, g_win32_registry_key, G_TYPE_OBJECT, G_ADD_PRIVATE(GWin32RegistryKey) G_IMPLEMENT_INTERFACE(G_TYPE_INITABLE, g_win32_registry_key_initable_iface_init));

     static void g_win32_registry_key_dispose(GObject * object)
{
...
GWin32RegistryKey *g_win32_registry_key_new(const gchar * path, GError ** error)
{
    ...
    return g_initable_new(G_TYPE_WIN32_REGISTRY_KEY, NULL, error, "path", path, NULL);
}
...
GWin32RegistryKey *g_win32_registry_key_new_w(const gunichar2 * path, GError ** error)
{
    ...
    result = g_initable_new(G_TYPE_WIN32_REGISTRY_KEY, NULL, error, "path-utf16", g_wcsdup(path, -1), NULL);
...
static gboolean g_win32_registry_key_initable_init(GInitable * initable, GCancellable * cancellable, GError ** error)
{
    ...
    {
        priv->absolute_path_w = g_utf8_to_utf16(priv->absolute_path, -1, NULL, NULL, error);
    ...
    {
        g_set_error(error, G_IO_ERROR, g_io_error_from_win32_error(opened), "Failed to open registry key '%S'", path);
        return FALSE;
    }
...
GWin32RegistryKey *g_win32_registry_key_get_child_w(GWin32RegistryKey * key, const gunichar2 * subkey, GError ** error)
{
    ...
    if (opened != ERROR_SUCCESS)
    {
        g_set_error(error, G_IO_ERROR, g_io_error_from_win32_error(opened), "Failed to open registry subkey '%S' of key '%S'", subkey, key_path);
        return NULL;
    }
    ...
    result->priv->absolute_path_w = g_malloc((wcslen(key_path) + 2 + subkey_len) * sizeof(gunichar2));
...
gboolean g_win32_registry_subkey_iter_init(GWin32RegistrySubkeyIter * iter, GWin32RegistryKey * key, GError ** error)
{
    ...
    status = RegQueryInfoKeyW(key->priv->handle, NULL, NULL, NULL, &subkey_count, &max_subkey_len, NULL, NULL, NULL, NULL, NULL, NULL);

    if (status != ERROR_SUCCESS)
    {
        g_set_error(error, G_IO_ERROR, g_io_error_from_win32_error(status), "Failed to query info for registry key '%S'", g_win32_registry_key_get_path_w(key));
        return FALSE;
    }
...
gboolean g_win32_registry_subkey_iter_next(GWin32RegistrySubkeyIter * iter, gboolean skip_errors, GError ** error)
{
    ...
    if G_UNLIKELY
        (iter->counter >= iter->subkey_count)
    {
        g_critical("g_win32_registry_subkey_iter_get_next_w: must not be called again " "after FALSE has already been returned.");
        return FALSE;
    }
    ...
    while (TRUE)
    {
        ...
        status = RegEnumKeyExW(iter->key->priv->handle, iter->counter, iter->subkey_name, &subkey_len, NULL, NULL, NULL, NULL);
        ...
        if (!skip_errors)
        {
            g_set_error(error, G_IO_ERROR, g_io_error_from_win32_error(status), "Failed to enumerate subkey #%d for key '%S'", iter->counter, g_win32_registry_key_get_path_w(iter->key));
            iter->subkey_count = 0;
...
gboolean g_win32_registry_subkey_iter_get_name_w(GWin32RegistrySubkeyIter * iter, gunichar2 ** subkey_name, gsize * subkey_name_len, GError ** error)
{
    ...
    if G_UNLIKELY
        (iter->counter >= iter->subkey_count)
    {
        g_critical("g_win32_registry_subkey_iter_get_name_w: must not be called " "after FALSE has already been returned by " "g_win32_registry_subkey_iter_next.");
        return FALSE;
    }
...
gboolean g_win32_registry_subkey_iter_get_name(GWin32RegistrySubkeyIter * iter, gchar ** subkey_name, gsize * subkey_name_len, GError ** error)
{
    ...
    iter->subkey_name_u8 = g_utf16_to_utf8(iter->subkey_name, iter->subkey_name_len, NULL, &subkey_name_len_glong, error);
...
gboolean g_win32_registry_value_iter_init(GWin32RegistryValueIter * iter, GWin32RegistryKey * key, GError ** error)
{
    ...
    status = RegQueryInfoKeyW(key->priv->handle, NULL, NULL, NULL, NULL, NULL, NULL, &value_count, &max_value_len, &max_data_len, NULL, NULL);
...
static GWin32RegistryValueType _g_win32_registry_type_w_to_g(DWORD value_type)
{
    switch (value_type)
    {
        case REG_BINARY:
...
static gsize ensure_nul_termination(GWin32RegistryValueType value_type, guint8 * value_data, gsize value_data_size)
{
    gsize new_size = value_data_size;

    if (value_type == G_WIN32_REGISTRY_VALUE_EXPAND_STR || value_type == G_WIN32_REGISTRY_VALUE_LINK || value_type == G_WIN32_REGISTRY_VALUE_STR)
    {
        if ((value_data_size < 2) || (value_data[value_data_size - 1] != 0) || (value_data[value_data_size - 2] != 0))
        {
    ...
    {
        if ((value_data_size < 4) || (value_data[value_data_size - 1] != 0) || (value_data[value_data_size - 2] != 0) || (value_data[value_data_size - 3] != 0) || (value_data[value_data_size - 4] != 0))
        {
            value_data[value_data_size] = 0;
            value_data[value_data_size + 1] = 0;
...
gboolean g_win32_registry_value_iter_next(GWin32RegistryValueIter * iter, gboolean skip_errors, GError ** error)
{
    ...
    if G_UNLIKELY
        (iter->counter >= iter->value_count)
    {
        g_critical("g_win32_registry_value_iter_next: must not be called " "again after FALSE has already been returned.");
        return FALSE;
    }
    ...
    while (TRUE)
    {
        ...
        status = RegEnumValueW(iter->key->priv->handle, iter->counter, iter->value_name, &value_name_len_w, NULL, &value_type_w, (LPBYTE) iter->value_data, &value_data_size_w);

        if (status != ERROR_SUCCESS && !skip_errors)
        {
            g_set_error(error, G_IO_ERROR, g_io_error_from_win32_error(status), "Failed to enumerate value #%d for key '%S'", iter->counter, g_win32_registry_key_get_path_w(iter->key));
            iter->value_count = 0;
        ...
        value_data_size_w = ensure_nul_termination(value_type_g, iter->value_data, value_data_size_w);
...
gboolean g_win32_registry_value_iter_get_value_type(GWin32RegistryValueIter * iter, GWin32RegistryValueType * value_type, GError ** error)
{
    ...
    if G_UNLIKELY
        (iter->counter >= iter->value_count)
    {
        g_critical("g_win32_registry_value_iter_get_type: must not be called " "again after NULL has already been returned.");
        return FALSE;
    }
...
gboolean g_win32_registry_value_iter_get_name_w(GWin32RegistryValueIter * iter, gunichar2 ** value_name, gsize * value_name_len, GError ** error)
{
    ...
    if G_UNLIKELY
        (iter->counter >= iter->value_count)
    {
        g_critical("g_win32_registry_value_iter_get_name_w: must not be called " "again after NULL has already been returned.");
        return FALSE;
    }
...
gboolean g_win32_registry_value_iter_get_name(GWin32RegistryValueIter * iter, gchar ** value_name, gsize * value_name_len, GError ** error)
{
    ...
    if G_UNLIKELY
        (iter->counter >= iter->value_count)
    {
        g_critical("g_win32_registry_value_iter_get_name: must not be called " "again after NULL has already been returned.");
        return FALSE;
    }

    if (iter->value_name_u8 == NULL)
    {
        iter->value_name_u8 = g_utf16_to_utf8(iter->value_name, iter->value_name_len, NULL, &value_name_len_glong, error);
...
static gboolean expand_value(gunichar2 * value, const gunichar2 * value_name, gpointer * expanded_value, gsize * expanded_charsize, GError ** error)
{
    DWORD value_data_expanded_charsize_w;

    value_data_expanded_charsize_w = ExpandEnvironmentStringsW(value, (gunichar2 *) * expanded_value, *expanded_charsize);

    if (value_data_expanded_charsize_w > *expanded_charsize)
    {
        *expanded_value = g_realloc(*expanded_value, value_data_expanded_charsize_w * sizeof(gunichar2));
    ...
    {
        g_set_error(error, G_IO_ERROR, g_io_error_from_win32_error(GetLastError()), "Failed to expand data '%S' of value %S", value, value_name);
        return FALSE;
    }
...
gboolean g_win32_registry_value_iter_get_data_w(GWin32RegistryValueIter * iter, gboolean auto_expand, gpointer * value_data, gsize * value_data_size, GError ** error)
{
    ...
    if G_UNLIKELY
        (iter->counter >= iter->value_count)
    {
        g_critical("g_win32_registry_value_iter_get_data_w: must not be called " "again after FALSE has already been returned.");
        return FALSE;
    }
    ...
    {
        if (!expand_value((gunichar2 *) iter->value_data, iter->value_name, (gpointer *) & iter->value_data_expanded, &iter->value_data_expanded_charsize, error))
            return FALSE;
...
gboolean g_win32_registry_value_iter_get_data(GWin32RegistryValueIter * iter, gboolean auto_expand, gpointer * value_data, gsize * value_data_size, GError ** error)
{
    ...
    if G_UNLIKELY
        (iter->counter >= iter->value_count)
    {
        g_critical("g_win32_registry_value_iter_get_data: must not be called " "again after FALSE has already been returned.");
        return FALSE;
    }

    if (iter->value_type != G_WIN32_REGISTRY_VALUE_EXPAND_STR && iter->value_type != G_WIN32_REGISTRY_VALUE_LINK && iter->value_type != G_WIN32_REGISTRY_VALUE_STR && iter->value_type != G_WIN32_REGISTRY_VALUE_MULTI_STR)
    {
        *value_data = iter->value_data;
    ...
    if (!auto_expand || (iter->value_type != G_WIN32_REGISTRY_VALUE_EXPAND_STR))
    {
        if (iter->value_data_u8 == NULL)
        {
            iter->value_data_u8 = g_convert((const gchar *) iter->value_data, iter->value_actual_data_size - sizeof(gunichar2) /* excl. 0 */ ,
                                            "UTF8", "UTF16", NULL, &value_data_len_gsize, error);
    ...
    if (iter->value_data_expanded_u8 == NULL)
    {
        if (!g_win32_registry_value_iter_get_data_w(iter, TRUE, &tmp, &tmp_size, error))
            return FALSE;

        iter->value_data_expanded_u8 = g_convert((const gchar *) iter->value_data_expanded, iter->value_data_expanded_charsize * sizeof(gunichar2) - sizeof(gunichar2) /* excl. 0 */ ,
                                                 "UTF8", "UTF16", NULL, &value_data_len_gsize, error);
...
static void _g_win32_registry_key_reread_kernel(GWin32RegistryKey * key, GWin32RegistryKeyPrivate * buf)
{
    NTSTATUS status;
    ...
    status = nt_query_key(key->priv->handle, KeyBasicInformation, basic_info, basic_info_size, &datasize);
    ...
    buf->absolute_path_w = g_wcsdup(&basic_info->Name[0], basic_info->NameLength + sizeof(gunichar2));
    g_free(basic_info);
}

static void _g_win32_registry_key_reread_user(GWin32RegistryKey * key, GWin32RegistryKeyPrivate * buf)
{
...
static void _g_win32_registry_key_reread(GWin32RegistryKey * key, GWin32RegistryKeyPrivate * buf)
{
...
static gboolean _g_win32_registry_key_update_path(GWin32RegistryKey * key)
{
...
const gchar *g_win32_registry_key_get_path(GWin32RegistryKey * key)
{
    ...
    if (change_indicator == G_WIN32_KEY_CHANGED && !(key->priv->update_flags & G_WIN32_REGISTRY_UPDATED_PATH))
    {
    ...
    if (key->priv->absolute_path == NULL)
    {
        ...
        key->priv->absolute_path = g_utf16_to_utf8(key->priv->absolute_path_w, -1, NULL, NULL, NULL);
    }
...
gboolean g_win32_registry_key_get_value(GWin32RegistryKey * key, gboolean auto_expand, const gchar * value_name, GWin32RegistryValueType * value_type, gpointer * value_data, gsize * value_data_size, GError ** error)
{
    ...
    g_return_val_if_fail(value_type != NULL || value_data != NULL || value_data_size != NULL, FALSE);
    ...
    result = g_win32_registry_key_get_value_w(key, auto_expand, value_name_w, &value_type_g, &value_data_w, &value_data_w_size, error);
    ...
    if (value_type_g == G_WIN32_REGISTRY_VALUE_EXPAND_STR || value_type_g == G_WIN32_REGISTRY_VALUE_LINK || value_type_g == G_WIN32_REGISTRY_VALUE_STR || value_type_g == G_WIN32_REGISTRY_VALUE_MULTI_STR)
    {
        value_data_u8 = g_convert((const gchar *) value_data_w, value_data_w_size - sizeof(gunichar2) /* excl. 0 */ ,
                                  "UTF8", "UTF16", NULL, &value_data_u8_len, error);
        g_free(value_data_w);
...
gboolean g_win32_registry_key_get_value_w(GWin32RegistryKey * key, gboolean auto_expand, const gunichar2 * value_name, GWin32RegistryValueType * value_type, gpointer * value_data, gsize * value_data_size, GError ** error)
{
    ...
    status = RegQueryValueExW(key->priv->handle, value_name, NULL, &value_type_w, NULL, &req_value_data_size);

    if (status != ERROR_MORE_DATA && status != ERROR_SUCCESS)
    {
        g_set_error(error, G_IO_ERROR, g_io_error_from_win32_error(status), "Failed to query value '%S' for key '%S'", value_name, g_win32_registry_key_get_path_w(key));
    ...
    if (value_data == NULL && (!auto_expand || value_type_g != G_WIN32_REGISTRY_VALUE_EXPAND_STR))
    {
        if (value_type)
            *value_type = value_type_g;
    ...
    status = RegQueryValueExW(key->priv->handle, value_name, NULL, &value_type_w2, (gpointer) req_value_data, &req_value_data_size2);

    if (status != ERROR_SUCCESS)
    {
        g_set_error(error, G_IO_ERROR, g_io_error_from_win32_error(status), "Failed to query value '%S' of size %lu for key '%S'", value_name, req_value_data_size, g_win32_registry_key_get_path_w(key));
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, "Type of value '%S' of key '%S' changed from %u to %u" " between calls", value_name, g_win32_registry_key_get_path_w(key), value_type_g, value_type_g2);
        g_free(req_value_data);
        return FALSE;
    }

    req_value_data_size = ensure_nul_termination(value_type_g, (guint8 *) req_value_data, req_value_data_size2);
    ...
    if (value_type_g == G_WIN32_REGISTRY_VALUE_EXPAND_STR && auto_expand)
    {
        ...
        if (!expand_value((gunichar2 *) req_value_data, value_name, (gpointer *) & value_data_expanded, &value_data_expanded_charsize_w, error))
            return FALSE;
...
static VOID NTAPI key_changed(PVOID closure, PIO_STATUS_BLOCK status_block, ULONG reserved)
{
...
gboolean g_win32_registry_key_watch(GWin32RegistryKey * key, gboolean watch_children, GWin32RegistryKeyWatcherFlags watch_flags, GWin32RegistryKeyWatchCallbackFunc callback, gpointer user_data, GError ** error)
{
    ...
    filter = ((watch_flags & G_WIN32_REGISTRY_WATCH_NAME) ? REG_NOTIFY_CHANGE_NAME : 0) |
        ...
        ((watch_flags & G_WIN32_REGISTRY_WATCH_VALUES) ? REG_NOTIFY_CHANGE_LAST_SET : 0) | ((watch_flags & G_WIN32_REGISTRY_WATCH_SECURITY) ? REG_NOTIFY_CHANGE_SECURITY : 0);
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, "Couldn't get NtNotifyChangeMultipleKeys() from ntdll");
        return FALSE;
    }

    started_to_watch = g_atomic_int_compare_and_exchange(&key->priv->watch_indicator, G_WIN32_KEY_UNWATCHED, G_WIN32_KEY_WATCHED);
    ...
    status = nt_notify_change_multiple_keys(key->priv->handle, 0, NULL, NULL, key_changed, (PVOID) key, status_block, filter, watch_children, NULL, 0, TRUE);
...
static void g_win32_registry_key_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
...
static void g_win32_registry_key_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    GWin32RegistryKey *key = G_WIN32_REGISTRY_KEY(object);
...
static void g_win32_registry_key_class_init(GWin32RegistryKeyClass * klass)
{
   ...
   * Since: 2.46
   */
    g_object_class_install_property(gobject_class, PROP_PATH, g_param_spec_string("path", "Path", "Path to the key in the registry", NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 2.46
   */
    g_object_class_install_property(gobject_class, PROP_PATH_UTF16, g_param_spec_pointer("path-utf16", "Path (UTF-16)", "Path to the key in the registry, in UTF-16", G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
}

static void g_win32_registry_key_init(GWin32RegistryKey * key)
{
