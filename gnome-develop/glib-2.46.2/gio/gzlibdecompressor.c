...
static void g_zlib_decompressor_class_init(GZlibDecompressorClass * klass)
{
   ...
   * Since: 2.26
   */
    g_object_class_install_property(gobject_class, PROP_FILE_INFO, g_param_spec_object("file-info", P_("file info"), P_("File info"), G_TYPE_FILE_INFO, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
}
...
static GConverterResult g_zlib_decompressor_convert(GConverter * converter, const void *inbuf, gsize inbuf_size, void *outbuf, gsize outbuf_size, GConverterFlags flags, gsize * bytes_read, gsize * bytes_written, GError ** error)
{
    GZlibDecompressor *decompressor;
    int res;
    ...
    *bytes_written = outbuf_size - decompressor->zstream.avail_out;
    ...
    if (decompressor->header_data != NULL && decompressor->header_data->gzheader.done == 1)
    {
        ...
        if (data->filename[0] != '\0')
            g_file_info_set_attribute_byte_string(data->file_info, G_FILE_ATTRIBUTE_STANDARD_NAME, data->filename);
