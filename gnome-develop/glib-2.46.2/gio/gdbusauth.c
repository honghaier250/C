...
static void _g_dbus_auth_class_init(GDBusAuthClass * klass)
{
    ...
    g_object_class_install_property(gobject_class,
                                    ...
                                    g_param_spec_object("stream",
                                                        P_("IO Stream"),
                                                        P_("The underlying GIOStream used for I/O"), G_TYPE_IO_STREAM, G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME | G_PARAM_STATIC_BLURB | G_PARAM_STATIC_NICK));
}
...
static void add_mechanism(GDBusAuth * auth, GDBusAuthObserver * observer, GType mechanism_type)
{
    const gchar *name;
...
static gchar *_my_g_data_input_stream_read_line(GDataInputStream * dis, gsize * out_line_length, GCancellable * cancellable, GError ** error)
{
    ...
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Unexpected lack of content trying to read a line"));
    }
...
static gchar *_my_g_input_stream_read_line_safe(GInputStream * i, gsize * out_line_length, GCancellable * cancellable, GError ** error)
{
    ...
    while (TRUE)
    {
        ...
        if (num_read == 0)
        {
            ...
            {
                g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Unexpected lack of content trying to (safely) read a line"));
            }
            goto fail;
        }
...
static GDBusAuthMechanism *client_choose_mech_and_send_initial_response(GDBusAuth * auth,
                                                                        GCredentials * credentials_that_were_sent,
                                                                        const gchar * const *supported_auth_mechs, GPtrArray * attempted_auth_mechs, GDataOutputStream * dos, GCancellable * cancellable, GError ** error)
{
...
again:
    ...
    if (auth_mech_to_use_gtype == (GType) 0)
    {
        ...
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Exhausted all available authentication mechanisms (tried: %s) (available: %s)"), tried_str->str, available);
...
#if 0
    g_printerr("using auth mechanism with name '%s' of type '%s' with initial response '%s'\n", _g_dbus_auth_mechanism_get_name(auth_mech_to_use_gtype), g_type_name(G_TYPE_FROM_INSTANCE(mech)), initial_response);
#endif
...
gchar *_g_dbus_auth_run_client(GDBusAuth * auth, GDBusAuthObserver * observer, GDBusCapabilityFlags offered_capabilities, GDBusCapabilityFlags * out_negotiated_capabilities, GCancellable * cancellable, GError ** error)
{
    gchar *s;
    ...
    if (G_IS_UNIX_CONNECTION(auth->priv->stream))
    {
        ...
        if (!g_unix_connection_send_credentials(G_UNIX_CONNECTION(auth->priv->stream), cancellable, error))
            goto out;
    }
    else
    {
...
choose_mechanism:
                ...
                g_free(line);
                mech = client_choose_mech_and_send_initial_response(auth, credentials, (const gchar * const *) supported_auth_mechs, attempted_auth_mechs, dos, cancellable, error);
                if (mech == NULL)
                    goto out;
...
_g_dbus_auth_run_server(GDBusAuth * auth,
                        ...
                        const gchar * guid,
                        gboolean allow_anonymous, GDBusCapabilityFlags offered_capabilities, GDBusCapabilityFlags * out_negotiated_capabilities, GCredentials ** out_received_credentials, GCancellable * cancellable, GError ** error)
{
    gboolean ret;
    ServerState state;
    GDataInputStream *dis;
...
change_state:
                        switch (_g_dbus_auth_mechanism_server_get_state(mech))
                        {
                            case G_DBUS_AUTH_MECHANISM_STATE_ACCEPTED:
                                ...
                                {
                                    /* disconnect */
                                    g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Cancelled via GDBusAuthObserver::authorize-authenticated-peer"));
                                    goto out;
                                }
                                else
                                {
