...
static void g_dbus_menu_group_changed(GDBusMenuGroup * group, guint menu_id, gint position, gint removed, GVariant * added);
static void g_dbus_menu_model_changed(GDBusMenuModel * proxy, GSequence * items, gint position, gint removed, gint added);
static GDBusMenuGroup *g_dbus_menu_group_get_from_path(GDBusMenuPath * path, guint group_id);
static GDBusMenuModel *g_dbus_menu_model_get_from_group(GDBusMenuGroup * group, guint menu_id);
...
static gboolean path_identifier_equal(gconstpointer a, gconstpointer b)
{
    ...
    return id_a->connection == id_b->connection && g_str_equal(id_a->bus_name, id_b->bus_name) && g_str_equal(id_a->object_path, id_b->object_path);
}
...
static void g_dbus_menu_path_signal(GDBusConnection * connection, const gchar * sender_name, const gchar * object_path, const gchar * interface_name, const gchar * signal_name, GVariant * parameters, gpointer user_data)
{
...
{
    if (path->active++ == 0)
        path->watch_id = g_dbus_connection_signal_subscribe(path->id->connection, path->id->bus_name, "org.gtk.Menus", "Changed", path->id->object_path, NULL, G_DBUS_SIGNAL_FLAGS_NONE, g_dbus_menu_path_signal, path, NULL);
}
...
static GDBusMenuPath *g_dbus_menu_path_get(GMainContext * context, GDBusConnection * connection, const gchar * bus_name, const gchar * object_path)
{
...
static void g_dbus_menu_group_go_offline(GDBusMenuGroup * group)
{
    ...
    g_dbus_connection_call(group->path->id->connection, group->path->id->bus_name, group->path->id->object_path, "org.gtk.Menus", "End", g_variant_new_parsed("([ %u ],)", group->id), NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL, NULL);
    group->state = GROUP_OFFLINE;
}

static void g_dbus_menu_group_start_ready(GObject * source_object, GAsyncResult * result, gpointer user_data)
{
    ...
    if (group->active++ == 0)
    {
        ...
        if (group->state == GROUP_OFFLINE)
        {
            ...
            g_dbus_connection_call(group->path->id->connection,
                                   ...
                                   "org.gtk.Menus", "Start", g_variant_new_parsed("([ %u ],)", group->id), G_VARIANT_TYPE("(a(uuaa{sv}))"), G_DBUS_CALL_FLAGS_NONE, -1, NULL, g_dbus_menu_group_start_ready, g_dbus_menu_group_ref(group));
...
static GDBusMenuGroup *g_dbus_menu_group_get(GMainContext * context, GDBusConnection * connection, const gchar * bus_name, const gchar * object_path, guint group_id)
{
    GDBusMenuGroup *group;
    GDBusMenuPath *path;
...
static void g_dbus_menu_model_get_item_attributes(GMenuModel * model, gint item_index, GHashTable ** table)
{
    GDBusMenuModel *proxy = G_DBUS_MENU_MODEL(model);
    GDBusMenuModelItem *item;
    GSequenceIter *iter;
...
static void g_dbus_menu_model_get_item_links(GMenuModel * model, gint item_index, GHashTable ** table)
{
    GDBusMenuModel *proxy = G_DBUS_MENU_MODEL(model);
    GDBusMenuModelItem *item;
    GSequenceIter *iter;
...
#if 0
static void dump_proxy(gpointer key, gpointer value, gpointer data)
{
...
static void dump_group(gpointer key, gpointer value, gpointer data)
{
...
static void dump_path(gpointer key, gpointer value, gpointer data)
{
...
void g_dbus_menu_model_dump(void)
{
