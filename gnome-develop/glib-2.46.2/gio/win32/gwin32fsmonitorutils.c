...
static gboolean g_win32_fs_monitor_handle_event(GWin32FSMonitorPrivate * monitor, gchar * filename, PFILE_NOTIFY_INFORMATION pfni)
{
    ...
    switch (pfni->Action)
    {
        ...
        case FILE_ACTION_MODIFIED:
            {
                ...
                if (monitor->file_attribs != INVALID_FILE_ATTRIBUTES && success_attribs && attrib_data.dwFileAttributes != monitor->file_attribs)
        ...
        case FILE_ACTION_RENAMED_NEW_NAME:
            if (monitor->pfni_prev != NULL && monitor->pfni_prev->Action == FILE_ACTION_RENAMED_OLD_NAME)
            {
    ...
    if (fme != -1)
        return g_file_monitor_source_handle_event(monitor->fms, fme, filename, renamed_file, NULL, g_get_monotonic_time());
    else
        return FALSE;
}

static void CALLBACK g_win32_fs_monitor_callback(DWORD error, DWORD nBytes, LPOVERLAPPED lpOverlapped)
{
    gulong offset;
    ...
    DWORD notify_filter = monitor->isfile ?
        (FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_ATTRIBUTES | FILE_NOTIFY_CHANGE_SIZE) : (FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_DIR_NAME | FILE_NOTIFY_CHANGE_ATTRIBUTES | FILE_NOTIFY_CHANGE_SIZE);

    /* If monitor->self is NULL the GWin32FileMonitor object has been destroyed. */
    if (monitor->self == NULL || g_file_monitor_is_cancelled(monitor->self) || monitor->file_notify_buffer == NULL)
    {
    ...
    do
    {
        ...
        if (pfile_notify_walker->Action > 0)
        {
            ...
            if (monitor->isfile)
            {
                ...
                if (_wcsnicmp(pfile_notify_walker->FileName, monitor->wfilename_long, long_filename_length) == 0)
                {
                    if (_wcsnicmp(pfile_notify_walker->FileName, monitor->wfilename_short, short_filename_length) == 0)
                    {
                ...
                else if (_wcsnicmp(pfile_notify_walker->FileName, monitor->wfilename_short, short_filename_length) == 0)
                {
...
void g_win32_fs_monitor_init(GWin32FSMonitorPrivate * monitor, gchar * dirname, gchar * filename, gboolean isfile)
{
    wchar_t *wdirname_with_long_prefix = NULL;
    const gchar LONGPFX[] = "\\\\?\\";
    gchar *fullpath_with_long_prefix, *dirname_with_long_prefix;
    DWORD notify_filter = isfile ? (FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_ATTRIBUTES | FILE_NOTIFY_CHANGE_SIZE) : (FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_DIR_NAME | FILE_NOTIFY_CHANGE_ATTRIBUTES | FILE_NOTIFY_CHANGE_SIZE);
    ...
    if (dirname != NULL)
    {
        ...
        if (isfile)
        {
            ...
            monitor->wfullpath_with_long_prefix = g_utf8_to_utf16(fullpath_with_long_prefix, -1, NULL, NULL, NULL);
            ...
            if (GetLongPathNameW(monitor->wfullpath_with_long_prefix, wlongname, MAX_PATH_LONG) == 0)
            {
                ...
                monitor->wfilename_long = wbasename_long != NULL ? wcsdup(wbasename_long + 1) : wcsdup(wfullpath);
            }
            else
            {
                wbasename_long = wcsrchr(wlongname, L'\\');
                monitor->wfilename_long = wbasename_long != NULL ? wcsdup(wbasename_long + 1) : wcsdup(wlongname);
            ...
            if (GetShortPathNameW(monitor->wfullpath_with_long_prefix, wshortname, MAX_PATH_LONG) == 0)
            {
                ...
                monitor->wfilename_short = wbasename_short != NULL ? wcsdup(wbasename_short + 1) : wcsdup(wfullpath);
            }
            else
            {
                wbasename_short = wcsrchr(wshortname, L'\\');
                monitor->wfilename_short = wbasename_short != NULL ? wcsdup(wbasename_short + 1) : wcsdup(wshortname);
            }
    ...
    success_attribs = GetFileAttributesExW(monitor->wfullpath_with_long_prefix, GetFileExInfoStandard, &attrib_data);
    if (success_attribs)
    ...
    monitor->hDirectory = CreateFileW(wdirname_with_long_prefix != NULL ? wdirname_with_long_prefix : monitor->wfullpath_with_long_prefix,
                                      FILE_GENERIC_READ | FILE_GENERIC_WRITE, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OVERLAPPED, NULL);
