...
static void do_lookup_by_name(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
    ...
    if (retval == 0)
    {
        ...
        else
        {
            ...
            g_task_return_new_error(task, G_RESOLVER_ERROR, G_RESOLVER_ERROR_NOT_FOUND, _("Error resolving '%s': %s"), hostname, _("No valid addresses were found"));
        }
    }
    else
    {
        g_task_return_new_error(task, G_RESOLVER_ERROR, g_resolver_error_from_addrinfo_error(retval), _("Error resolving '%s': %s"), hostname, gai_strerror(retval));
    }
...
static void do_lookup_by_address(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
    ...
    g_socket_address_to_native(gsockaddr, (struct sockaddr *) &sockaddr, sizeof(sockaddr), NULL);
    ...
    else
    {
        ...
        g_task_return_new_error(task, G_RESOLVER_ERROR, g_resolver_error_from_addrinfo_error(retval), _("Error reverse-resolving '%s': %s"), phys ? phys : "(unknown)", gai_strerror(retval));
        g_free(phys);
    }
}
...
static GVariant *parse_res_soa(guchar * answer, guchar * end, guchar ** p)
{
    ...
    *p += dn_expand(answer, end, *p, rnamebuf, sizeof(rnamebuf));
    ...
    return g_variant_new("(ssuuuuu)", mnamebuf, rnamebuf, serial, refresh, retry, expire, ttl);
}
...
static GList *g_resolver_records_from_res_query(const gchar * rrname, gint rrtype, guchar * answer, gint len, gint herr, GError ** error)
{
    gint count;
    gchar namebuf[1024];
    guchar *end, *p;
    ...
    if (len <= 0)
    {
        ...
        {
            g_set_error(error, G_RESOLVER_ERROR, G_RESOLVER_ERROR_NOT_FOUND, _("No DNS record of the requested type for '%s'"), rrname);
        }
        else if (herr == TRY_AGAIN)
        {
            g_set_error(error, G_RESOLVER_ERROR, G_RESOLVER_ERROR_TEMPORARY_FAILURE, _("Temporarily unable to resolve '%s'"), rrname);
        }
        else
        {
            g_set_error(error, G_RESOLVER_ERROR, G_RESOLVER_ERROR_INTERNAL, _("Error resolving '%s'"), rrname);
        }
...
{
    return g_variant_new("(qqqs)", (guint16) rec->Data.SRV.wPriority, (guint16) rec->Data.SRV.wWeight, (guint16) rec->Data.SRV.wPort, rec->Data.SRV.pNameTarget);
}
...
static GVariant *parse_dns_soa(DNS_RECORD * rec)
{
    return g_variant_new("(ssuuuuu)",
                         ...
                         rec->Data.SOA.pNameAdministrator, (guint32) rec->Data.SOA.dwSerialNo, (guint32) rec->Data.SOA.dwRefresh, (guint32) rec->Data.SOA.dwRetry, (guint32) rec->Data.SOA.dwExpire, (guint32) rec->Data.SOA.dwDefaultTtl);
}
...
{
    return g_variant_new("(qs)", (guint16) rec->Data.MX.wPreference, rec->Data.MX.pNameExchange);
}
...
static GList *g_resolver_records_from_DnsQuery(const gchar * rrname, WORD dnstype, DNS_STATUS status, DNS_RECORD * results, GError ** error)
{
    DNS_RECORD *rec;
    gpointer record;
    GList *records;
...
static void do_lookup_records(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
...
static GList *lookup_records(GResolver * resolver, const gchar * rrname, GResolverRecordType record_type, GCancellable * cancellable, GError ** error)
{
    GTask *task;
    GList *records;
...
static void lookup_records_async(GResolver * resolver, const char *rrname, GResolverRecordType record_type, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    GTask *task;
    LookupRecordsData *lrd;
...
static GList *lookup_records_finish(GResolver * resolver, GAsyncResult * result, GError ** error)
{
    g_return_val_if_fail(g_task_is_valid(result, resolver), NULL);
