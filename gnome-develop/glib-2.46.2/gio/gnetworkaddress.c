...
G_DEFINE_TYPE_WITH_CODE(GNetworkAddress, g_network_address, G_TYPE_OBJECT, G_ADD_PRIVATE(GNetworkAddress) G_IMPLEMENT_INTERFACE(G_TYPE_SOCKET_CONNECTABLE, g_network_address_connectable_iface_init))
...
static void g_network_address_class_init(GNetworkAddressClass * klass)
{
    ...
    g_object_class_install_property(gobject_class, PROP_HOSTNAME, g_param_spec_string("hostname", P_("Hostname"), P_("Hostname to resolve"), NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_PORT, g_param_spec_uint("port", P_("Port"), P_("Network port"), 0, 65535, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(gobject_class, PROP_SCHEME, g_param_spec_string("scheme", P_("Scheme"), P_("URI Scheme"), NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
}
...
static void g_network_address_set_addresses(GNetworkAddress * addr, GList * addresses, guint64 resolver_serial)
{
    GList *a;
...
static gboolean g_network_address_parse_sockaddr(GNetworkAddress * addr)
{
    ...
    sockaddr = g_inet_socket_address_new_from_string(addr->priv->hostname, addr->priv->port);
    if (sockaddr)
    {
        addr->priv->sockaddrs = g_list_prepend(addr->priv->sockaddrs, sockaddr);
        return TRUE;
...
GSocketConnectable *g_network_address_new_loopback(guint16 port)
{
    ...
    addr = g_object_new(G_TYPE_NETWORK_ADDRESS, "hostname", "localhost", "port", port, NULL);
...
GSocketConnectable *g_network_address_parse(const gchar * host_and_port, guint16 default_port, GError ** error)
{
        ...
        /* escaped host part (to allow, eg. "[2001:db8::1]:888") */
    {
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, "The ']' character (in hostname '%s') must come at the" " end or be immediately followed by ':' and a port", host_and_port);
            return NULL;
        }
    ...
    if (port != NULL)
    {
        if (port[0] == '\0')
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, "If a ':' character is given, it must be followed by a " "port (in hostname '%s').", host_and_port);
            g_free(name);
            return NULL;
        }
...
gboolean _g_uri_parse_authority(const char *uri, char **host, guint16 * port, char **userinfo)
{
    ...
    if (at != NULL)
    {
        ...
        while (1)
        {
            ...
            if (!(g_ascii_isalnum(c) || strchr(G_URI_OTHER_UNRESERVED, c) || strchr(G_URI_RESERVED_CHARS_SUBCOMPONENT_DELIMITERS, c) || c == ':'))
                return FALSE;
        }
    ...
    else
    {
        while (1)
        {
            ...
            /* unreserved /  sub-delims */
            if (!(g_ascii_isalnum(c) || strchr(G_URI_OTHER_UNRESERVED, c) || strchr(G_URI_RESERVED_CHARS_SUBCOMPONENT_DELIMITERS, c)))
                goto error;
        }
...
static GSocketAddress *g_network_address_address_enumerator_next(GSocketAddressEnumerator * enumerator, GCancellable * cancellable, GError ** error)
{
    ...
    if (addr_enum->addresses == NULL)
    {
        ...
        if (addr->priv->resolver_serial != 0 && addr->priv->resolver_serial != serial)
        {
        ...
        if (!addr->priv->sockaddrs)
        {
            ...
            addresses = g_resolver_lookup_by_name(resolver, addr->priv->hostname, cancellable, error);
            if (!addresses)
            {
...
static void have_addresses(GNetworkAddressAddressEnumerator * addr_enum, GTask * task, GError * error)
{
    GSocketAddress *sockaddr;
...
static void got_addresses(GObject * source_object, GAsyncResult * result, gpointer user_data)
{
    ...
    if (!addr_enum->addr->priv->sockaddrs)
    {
        ...
        if (!error)
        {
            g_network_address_set_addresses(addr_enum->addr, addresses, g_resolver_get_serial(resolver));
        }
    }
    have_addresses(addr_enum, task, error);
}

static void g_network_address_address_enumerator_next_async(GSocketAddressEnumerator * enumerator, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    GNetworkAddressAddressEnumerator *addr_enum = G_NETWORK_ADDRESS_ADDRESS_ENUMERATOR(enumerator);
    GSocketAddress *sockaddr;
    GTask *task;
    ...
    if (addr_enum->addresses == NULL)
    {
        ...
        if (!addr->priv->sockaddrs)
        {
            ...
            {
                g_resolver_lookup_by_name_async(resolver, addr->priv->hostname, cancellable, got_addresses, task);
            }
            g_object_unref(resolver);
            return;
        }
