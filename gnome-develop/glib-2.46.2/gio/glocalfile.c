...
static void g_local_file_class_init(GLocalFileClass * klass)
{
    ...
    g_file_attribute_info_list_add(list, G_FILE_ATTRIBUTE_UNIX_MODE, G_FILE_ATTRIBUTE_TYPE_UINT32, G_FILE_ATTRIBUTE_INFO_COPY_WITH_FILE | G_FILE_ATTRIBUTE_INFO_COPY_WHEN_MOVED);
...
#ifdef HAVE_UTIMES
    g_file_attribute_info_list_add(list, G_FILE_ATTRIBUTE_TIME_MODIFIED, G_FILE_ATTRIBUTE_TYPE_UINT64, G_FILE_ATTRIBUTE_INFO_COPY_WITH_FILE | G_FILE_ATTRIBUTE_INFO_COPY_WHEN_MOVED);
    g_file_attribute_info_list_add(list, G_FILE_ATTRIBUTE_TIME_MODIFIED_USEC, G_FILE_ATTRIBUTE_TYPE_UINT32, G_FILE_ATTRIBUTE_INFO_COPY_WITH_FILE | G_FILE_ATTRIBUTE_INFO_COPY_WHEN_MOVED);
 ...
 * Returns: a #GFile
 */
GFile *g_local_file_new_from_dirname_and_basename(const gchar * dirname, const gchar * basename)
{
    GLocalFile *local;
...
static void get_mount_info(GFileInfo * fs_info, const char *path, GFileAttributeMatcher * matcher)
{
    ...
    if (mount_info_hash == NULL)
        mount_info_hash = g_hash_table_new_full(device_hash, device_equal, g_free, NULL);
...
#    endif                        /* STATFS_ARGS == 2 */
     ...
     */
#    ifndef G_OS_WIN32
    if (statfs_result == 0 && statfs_buffer.f_bavail == 0 && statfs_buffer.f_bfree == 0)
        no_size = TRUE;
#    endif                        /* G_OS_WIN32 */
...
#    endif
    ...
    if (!no_size && g_file_attribute_matcher_matches(attribute_matcher, G_FILE_ATTRIBUTE_FILESYSTEM_USED))
    {
#ifdef G_OS_WIN32
        gchar *localdir = g_path_get_dirname(local->filename);
        wchar_t *wdirname = g_utf8_to_utf16(localdir, -1, NULL, NULL, NULL);
...
static GFile *g_local_file_set_display_name(GFile * file, const char *display_name, GCancellable * cancellable, GError ** error)
{
    ...
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_EXISTS, _("Can't rename file, filename already exists"));
        return NULL;
    }
    ...
    if (g_rename(local->filename, new_local->filename) == -1)
    {
        ...
        if (errsv == EINVAL)
            ...
            g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_FILENAME, _("Invalid filename"));
        else
            g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), _("Error renaming file: %s"), g_strerror(errsv));
...
static gboolean g_local_file_set_attribute(GFile * file, const char *attribute, GFileAttributeType type, gpointer value_p, GFileQueryInfoFlags flags, GCancellable * cancellable, GError ** error)
{
    GLocalFile *local = G_LOCAL_FILE(file);
...
{
    return _g_local_file_output_stream_create(G_LOCAL_FILE(file)->filename, FALSE, flags, NULL, cancellable, error);
}

static GFileOutputStream *g_local_file_replace(GFile * file, const char *etag, gboolean make_backup, GFileCreateFlags flags, GCancellable * cancellable, GError ** error)
{
    return _g_local_file_output_stream_replace(G_LOCAL_FILE(file)->filename, FALSE, etag, make_backup, flags, NULL, cancellable, error);
}
...
static GFileIOStream *g_local_file_create_readwrite(GFile * file, GFileCreateFlags flags, GCancellable * cancellable, GError ** error)
{
    ...
    output = _g_local_file_output_stream_create(G_LOCAL_FILE(file)->filename, TRUE, flags, NULL, cancellable, error);
    if (output == NULL)
        return NULL;
...
static GFileIOStream *g_local_file_replace_readwrite(GFile * file, const char *etag, gboolean make_backup, GFileCreateFlags flags, GCancellable * cancellable, GError ** error)
{
    GFileOutputStream *output;
    GFileIOStream *res;

    output = _g_local_file_output_stream_replace(G_LOCAL_FILE(file)->filename, TRUE, etag, make_backup, flags, NULL, cancellable, error);
    if (output == NULL)
        return NULL;
...
static gboolean path_has_prefix(const char *path, const char *prefix)
{
    ...
    if (strncmp(path, prefix, prefix_len) == 0 && (prefix_len == 0 ||    /* empty prefix always matches */
...
gboolean _g_local_file_is_lost_found_dir(const char *path, dev_t path_dev)
{
    ...
    if (!(S_ISDIR(statbuf.st_mode) && statbuf.st_uid == 0 && statbuf.st_gid == 0))
        goto out;
...
retry:
    ...
    if (g_rename(local->filename, trashfile) == -1)
    {
        ...
        if (errsv == EXDEV)
            ...
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, _("Unable to trash file: %s"), g_strerror(errsv));
        else
            g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), _("Unable to trash file: %s"), g_strerror(errsv));
        return FALSE;
    }
...
static gboolean g_local_file_trash(GFile * file, GCancellable * cancellable, GError ** error)
{
    ...
    if (success && op.fAnyOperationsAborted)
    {
        ...
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Unable to trash file: %s"), _("internal error"));
...
static gboolean g_local_file_make_directory(GFile * file, GCancellable * cancellable, GError ** error)
{
    ...
    if (g_mkdir(local->filename, 0777) == -1)
    {
        ...
        if (errsv == EINVAL)
            ...
            g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), _("Error creating directory: %s"), g_strerror(errsv));
        return FALSE;
    }
...
static gboolean g_local_file_make_symbolic_link(GFile * file, const char *symlink_value, GCancellable * cancellable, GError ** error)
{
    ...
    if (symlink(symlink_value, local->filename) == -1)
    {
        ...
        if (errsv == EINVAL)
            ...
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, _("Filesystem does not support symbolic links"));
        else
            g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), _("Error making symbolic link: %s"), g_strerror(errsv));
        return FALSE;
    }
    return TRUE;
#else
...
static gboolean g_local_file_copy(GFile * source, GFile * destination, GFileCopyFlags flags, GCancellable * cancellable, GFileProgressCallback progress_callback, gpointer progress_callback_data, GError ** error)
{
...
static gboolean g_local_file_move(GFile * source, GFile * destination, GFileCopyFlags flags, GCancellable * cancellable, GFileProgressCallback progress_callback, gpointer progress_callback_data, GError ** error)
{
    ...
    if (res == 0)
    {
        ...
        if (flags & G_FILE_COPY_OVERWRITE)
        {
            ...
            {
                if (source_is_dir)
                    g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_WOULD_MERGE, _("Can't move directory over directory"));
                else
    ...
    if (g_rename(local_source->filename, local_destination->filename) == -1)
    {
        ...
        if (errsv == EXDEV)
            ...
            g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, _("Move between mounts not supported"));
...
static gboolean g_local_file_measure_size_error(GFileMeasureFlags flags, gint saved_errno, GSList * name, GError ** error)
{
    ...
    if ((name->next == NULL) || (flags & G_FILE_MEASURE_REPORT_ANY_ERROR))
    {
        ...
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(saved_errno), _("Could not determine the disk usage of %s: %s"), filename->str, g_strerror(saved_errno));
...
static gboolean g_local_file_measure_size_of_contents(gint fd, GSList * dir_name, MeasureState * state, GError ** error);

static gboolean g_local_file_measure_size_of_file(gint parent_fd, GSList * name, MeasureState * state, GError ** error)
{
    GLocalFileStat buf;
    ...
    if (state->progress_callback)
    {
        ...
        if (state->last_progress_report)
        {
            ...
            {
                (*state->progress_callback) (TRUE, state->disk_usage, state->num_dirs, state->num_files, state->progress_data);
...
static gboolean
g_local_file_measure_disk_usage(GFile * file, GFileMeasureFlags flags, GCancellable * cancellable, GFileMeasureProgressCallback progress_callback, gpointer progress_data, guint64 * disk_usage, guint64 * num_dirs, guint64 * num_files, GError ** error)
{
    GLocalFile *local_file = G_LOCAL_FILE(file);
