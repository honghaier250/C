    ...
    while (priv->addr_enum == NULL && *priv->next_proxy)
    {
        ...
        if (strcmp("direct", priv->proxy_type) == 0)
        {
            if (priv->connectable)
                ...
                connectable = g_network_address_new(priv->dest_hostname, priv->dest_port);
        }
        else
        {
            GError *error = NULL;
            ...
            if (error)
            {
                g_warning("Invalid proxy URI '%s': %s", priv->proxy_uri, error->message);
                g_error_free(error);
            }
...
static GSocketAddress *g_proxy_address_enumerator_next(GSocketAddressEnumerator * enumerator, GCancellable * cancellable, GError ** error)
{
    ...
    {
        priv->proxies = g_proxy_resolver_lookup(priv->proxy_resolver, priv->dest_uri, cancellable, error);
    ...
    while (result == NULL && (*priv->next_proxy || priv->addr_enum))
    {
        ...
        {
            priv->proxy_address = g_socket_address_enumerator_next(priv->addr_enum, cancellable, first_error ? NULL : &first_error);
        }
        ...
        result = g_object_new(G_TYPE_PROXY_ADDRESS,
                              ...
                              "destination-protocol", dest_protocol, "destination-hostname", dest_hostname, "destination-port", priv->dest_port, "username", priv->proxy_username, "password", priv->proxy_password, "uri", priv->proxy_uri, NULL);
...
static void next_proxy(GTask * task)
{
    ...
    if (*priv->next_proxy)
    {
        ...
        if (priv->addr_enum)
        {
            g_socket_address_enumerator_next_async(priv->addr_enum, g_task_get_cancellable(task), address_enumerate_cb, task);
            return;
        }
    }
...
static void dest_hostname_lookup_cb(GObject * object, GAsyncResult * result, gpointer user_data)
{
    ...
    priv->dest_ips = g_resolver_lookup_by_name_finish(G_RESOLVER(object), result, &priv->last_error);
    if (priv->dest_ips)
        return_result(task);
    else
    {
...
static void address_enumerate_cb(GObject * object, GAsyncResult * result, gpointer user_data)
{
    ...
    g_clear_error(&priv->last_error);
    priv->proxy_address = g_socket_address_enumerator_next_finish(priv->addr_enum, result, &priv->last_error);
    if (priv->proxy_address)
    {
        if (!priv->supports_hostname && !priv->dest_ips)
        {
            GResolver *resolver;
            resolver = g_resolver_get_default();
            g_resolver_lookup_by_name_async(resolver, priv->dest_hostname, g_task_get_cancellable(task), dest_hostname_lookup_cb, task);
            g_object_unref(resolver);
            return;
        }
...
static void proxy_lookup_cb(GObject * object, GAsyncResult * result, gpointer user_data)
{
    ...
    g_clear_error(&priv->last_error);
    priv->proxies = g_proxy_resolver_lookup_finish(G_PROXY_RESOLVER(object), result, &priv->last_error);
    priv->next_proxy = priv->proxies;
...
static void g_proxy_address_enumerator_next_async(GSocketAddressEnumerator * enumerator, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (priv->proxies == NULL)
    {
        g_proxy_resolver_lookup_async(priv->proxy_resolver, priv->dest_uri, cancellable, proxy_lookup_cb, task);
        return;
    }
    ...
    if (priv->addr_enum)
    {
        ...
        {
            g_socket_address_enumerator_next_async(priv->addr_enum, cancellable, address_enumerate_cb, task);
            return;
        }
    }
...
static void g_proxy_address_enumerator_class_init(GProxyAddressEnumeratorClass * proxy_enumerator_class)
{
    ...
    g_object_class_install_property(object_class, PROP_URI, g_param_spec_string("uri", P_("URI"), P_("The destination URI, use none:// for generic socket"), NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_object_class_install_property(object_class,
                                    PROP_DEFAULT_PORT,
                                    g_param_spec_uint("default-port", P_("Default port"), P_("The default port to use if uri does not specify one"), 0, 65535, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));

    g_object_class_install_property(object_class,
                                    PROP_CONNECTABLE, g_param_spec_object("connectable", P_("Connectable"), P_("The connectable being enumerated."), G_TYPE_SOCKET_CONNECTABLE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
   ...
   */
    g_object_class_install_property(object_class,
                                    PROP_PROXY_RESOLVER, g_param_spec_object("proxy-resolver", P_("Proxy resolver"), P_("The proxy resolver to use."), G_TYPE_PROXY_RESOLVER, G_PARAM_READWRITE | G_PARAM_CONSTRUCT | G_PARAM_STATIC_STRINGS));
}
