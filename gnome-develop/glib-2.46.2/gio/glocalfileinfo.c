...
static void escape_xattr(GFileInfo * info, const char *gio_attr,    /* gio attribute name */
...
void _g_local_file_info_get_parent_info(const char *dir, GFileAttributeMatcher * attribute_matcher, GLocalParentFileInfo * parent_info)
{
    ...
    if (_g_file_attribute_matcher_matches_id(attribute_matcher, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_RENAME) ||
        ...
        _g_file_attribute_matcher_matches_id(attribute_matcher, G_FILE_ATTRIBUTE_ID_ACCESS_CAN_TRASH) || _g_file_attribute_matcher_matches_id(attribute_matcher, G_FILE_ATTRIBUTE_ID_UNIX_IS_MOUNTPOINT))
    {
...
static void get_access_rights(GFileAttributeMatcher * attribute_matcher, GFileInfo * info, const gchar * path, GLocalFileStat * statbuf, GLocalParentFileInfo * parent_info)
{
    ...
    if (parent_info)
    {
        ...
        if (parent_info->writable)
        {
            if (parent_info->is_sticky)
            {
                ...
                if (uid == statbuf->st_uid || uid == parent_info->owner || uid == 0)
#endif
                    writable = TRUE;
            }
            else
...
#ifndef G_OS_WIN32
    else if (S_ISCHR(statbuf->st_mode) || S_ISBLK(statbuf->st_mode) || S_ISFIFO(statbuf->st_mode)
#    ifdef S_ISSOCK
...
static char *get_content_type(const char *basename, const char *path, GLocalFileStat * statbuf, gboolean is_symlink, gboolean symlink_broken, GFileQueryInfoFlags flags, gboolean fast)
{
    if (is_symlink && (symlink_broken || (flags & G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS)))
...
static void get_thumbnail_attributes(const char *path, GFileInfo * info, const GLocalFileStat * stat_buf)
{
    GChecksum *checksum;
    char *uri;
    char *filename;
    ...
    filename = g_build_filename(g_get_user_cache_dir(), "thumbnails", "large", basename, NULL);

    if (g_file_test(filename, G_FILE_TEST_IS_REGULAR))
    {
        _g_file_info_set_attribute_byte_string_by_id(info, G_FILE_ATTRIBUTE_ID_THUMBNAIL_PATH, filename);
        _g_file_info_set_attribute_boolean_by_id(info, G_FILE_ATTRIBUTE_ID_THUMBNAIL_IS_VALID, thumbnail_verify(filename, uri, stat_buf));
    }
    else
    {
        g_free(filename);
        filename = g_build_filename(g_get_user_cache_dir(), "thumbnails", "normal", basename, NULL);
        ...
        {
            g_free(filename);
            filename = g_build_filename(g_get_user_cache_dir(), "thumbnails", "fail", "gnome-thumbnail-factory", basename, NULL);
...
static void win32_get_file_user_info(const gchar * filename, gchar ** group_name, gchar ** user_name, gchar ** real_name)
{
    ...
    if ((GetFileSecurityW(wfilename,
                          ...
                          NULL,
                          sd_size, &sd_size) || (ERROR_INSUFFICIENT_BUFFER == GetLastError())) && (psd = g_try_malloc(sd_size)) != NULL && GetFileSecurityW(wfilename, GROUP_SECURITY_INFORMATION | OWNER_SECURITY_INFORMATION, psd, sd_size, &sd_size))
    {
        PSID psid = 0;
        BOOL defaulted;
        ...
        do
        {
            ...
            if (!LookupAccountSidW(NULL,    /* local machine */
                                   psid, name, &name_len, domain, &domain_len,    /* no domain info yet */
            ...
            if (name && domain && LookupAccountSidW(NULL,    /* local machine */
                                                    psid, name, &name_len, domain, &domain_len,    /* no domain info yet */
                                                    &name_use))
            {
...
static gboolean file_is_hidden(const gchar * path, const gchar * basename)
{
    ...
    if G_UNLIKELY
        (hidden_cache == NULL) hidden_cache = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, maybe_unref_hash_table);
    ...
    if (!g_hash_table_lookup_extended(hidden_cache, dirname, NULL, &table))
    {
        ...
        g_hash_table_insert(hidden_cache, mydirname = g_strdup(dirname), table = read_hidden_file(dirname));

        remove_from_cache_source = g_timeout_source_new_seconds(5);
        g_source_set_priority(remove_from_cache_source, G_PRIORITY_DEFAULT);
        g_source_set_callback(remove_from_cache_source, remove_from_hidden_cache, mydirname, NULL);
        g_source_attach(remove_from_cache_source, GLIB_PRIVATE_CALL(g_get_worker_context) ());
...
void _g_local_file_info_get_nostat(GFileInfo * info, const char *basename, const char *path, GFileAttributeMatcher * attribute_matcher)
{
...
static const char *get_icon_name(const char *path, const char *content_type, gboolean use_symbolic, gboolean * with_fallbacks_out)
{
...
static GIcon *get_icon(const char *path, const char *content_type, gboolean use_symbolic)
{
    GIcon *icon = NULL;
...
GFileInfo *_g_local_file_info_get(const char *basename, const char *path, GFileAttributeMatcher * attribute_matcher, GFileQueryInfoFlags flags, GLocalParentFileInfo * parent_info, GError ** error)
{
    GFileInfo *info;
...
#else
    {
        ...
        while (len > 0 && G_IS_DIR_SEPARATOR(wpath[len - 1]))
            len--;
        if (len > 0 && (!g_path_is_absolute(path) || len > g_path_skip_root(path) - path))
            wpath[len] = '\0';
    ...
    if (res == -1)
    {
        ...
        if (errsv != EACCES)
        {
            ...
            g_object_unref(info);
            g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), _("Error when getting information for file '%s': %s"), display_name, g_strerror(errsv));
            g_free(display_name);
            return NULL;
        }
    }
...
#ifndef G_OS_WIN32
    if (_g_file_attribute_matcher_matches_id(attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_IS_HIDDEN))
    {
        if (basename != NULL && (basename[0] == '.' || file_is_hidden(path, basename)))
            g_file_info_set_is_hidden(info, TRUE);
    }
    ...
    if (_g_file_attribute_matcher_matches_id(attribute_matcher,
        ...
        _g_file_attribute_matcher_matches_id(attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_ICON) || _g_file_attribute_matcher_matches_id(attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_SYMBOLIC_ICON))
    {
        ...
        if (content_type)
        {
            ...
            if (_g_file_attribute_matcher_matches_id(attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_ICON) || _g_file_attribute_matcher_matches_id(attribute_matcher, G_FILE_ATTRIBUTE_ID_STANDARD_SYMBOLIC_ICON))
            {
                GIcon *icon;
    ...
    if (stat_ok && parent_info && parent_info->device != 0 && _g_file_attribute_matcher_matches_id(attribute_matcher, G_FILE_ATTRIBUTE_ID_UNIX_IS_MOUNTPOINT) && statbuf.st_dev != parent_info->device)
    ...
    {
        class->local_file_add_info(vfs, path, device, attribute_matcher, info, NULL, &parent_info->extra_data, &parent_info->free_extra_data);
    }
...
#    define FSTAT fstat
    ...
    if (FSTAT(fd, &stat_buf) == -1)
    {
        ...
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), _("Error when getting information for file descriptor: %s"), g_strerror(errsv));
        return NULL;
    }
...
#ifdef G_OS_UNIX
static gboolean set_unix_uid_gid(char *filename, const GFileAttributeValue * uid_value, const GFileAttributeValue * gid_value, GFileQueryInfoFlags flags, GError ** error)
{
    int res;
    guint32 val = 0;
    uid_t uid;
...
static gboolean set_mtime_atime(char *filename, const GFileAttributeValue * mtime_value, const GFileAttributeValue * mtime_usec_value, const GFileAttributeValue * atime_value, const GFileAttributeValue * atime_usec_value, GError ** error)
{
    int res;
    guint64 val = 0;
    guint32 val_usec = 0;
...
gboolean _g_local_file_info_set_attribute(char *filename, const char *attribute, GFileAttributeType type, gpointer value_p, GFileQueryInfoFlags flags, GCancellable * cancellable, GError ** error)
{
