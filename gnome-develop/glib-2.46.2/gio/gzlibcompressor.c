...
static void g_zlib_compressor_set_gzheader(GZlibCompressor * compressor)
{
    ...
    if (compressor->format != G_ZLIB_COMPRESSOR_FORMAT_GZIP || compressor->file_info == NULL)
        return;
...
static void g_zlib_compressor_class_init(GZlibCompressorClass * klass)
{
    ...
    g_object_class_install_property(gobject_class,
                                    ...
                                    g_param_spec_enum("format",
                                                      ...
                                                      P_("The format of the compressed data"), G_TYPE_ZLIB_COMPRESSOR_FORMAT, G_ZLIB_COMPRESSOR_FORMAT_ZLIB, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class,
                                    PROP_LEVEL,
                                    g_param_spec_int("level",
                                                     P_("compression level"),
                                                     P_("The level of compression from 0 (no compression) to 9 (most compression), -1 for the default level"), -1, 9, -1, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
   ...
   * Since: 2.26
   */
    g_object_class_install_property(gobject_class, PROP_FILE_INFO, g_param_spec_object("file-info", P_("file info"), P_("File info"), G_TYPE_FILE_INFO, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
static GConverterResult g_zlib_compressor_convert(GConverter * converter, const void *inbuf, gsize inbuf_size, void *outbuf, gsize outbuf_size, GConverterFlags flags, gsize * bytes_read, gsize * bytes_written, GError ** error)
{
    GZlibCompressor *compressor;
    int res;
    int flush;
