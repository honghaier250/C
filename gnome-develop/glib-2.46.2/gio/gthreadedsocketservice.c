...
G_DEFINE_TYPE_WITH_PRIVATE(GThreadedSocketService, g_threaded_socket_service, G_TYPE_SOCKET_SERVICE)
     enum
     {
         PROP_0,
         PROP_MAX_THREADS
     ...
     static void g_threaded_socket_service_func(gpointer _data, gpointer user_data)
{
    ...
    g_signal_emit(threaded, g_threaded_socket_service_run_signal, 0, data->connection, data->source_object, &result);
...
static void g_threaded_socket_service_class_init(GThreadedSocketServiceClass * class)
{
   ...
   */
    g_threaded_socket_service_run_signal =
        g_signal_new(I_("run"), G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GThreadedSocketServiceClass, run), g_signal_accumulator_true_handled, NULL, NULL, G_TYPE_BOOLEAN, 2, G_TYPE_SOCKET_CONNECTION, G_TYPE_OBJECT);

    g_object_class_install_property(gobject_class, PROP_MAX_THREADS,
                                    g_param_spec_int("max-threads", P_("Max threads"), P_("The max number of threads handling clients for this service"), -1, G_MAXINT, 10, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
