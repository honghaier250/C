   ...
   */
    g_object_class_install_property(gobject_class,
                                    ...
                                    g_param_spec_pointer("handle",
                                                         P_("File handle"), P_("The file handle to write to"), G_PARAM_READABLE | G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB));
...
static gssize g_win32_output_stream_write(GOutputStream * stream, const void *buffer, gsize count, GCancellable * cancellable, GError ** error)
{
    ...
    else
    {
        ...
        if (errsv == ERROR_IO_PENDING && _g_win32_overlap_wait_result(win32_stream->priv->handle, &overlap, &nwritten, cancellable))
        {
            retval = nwritten;
            goto end;
        }
        ...
        errsv = GetLastError();
        if (errsv == ERROR_HANDLE_EOF || errsv == ERROR_BROKEN_PIPE)
        {
            retval = 0;
        }
        else
...
GOutputStream *g_win32_output_stream_new_from_fd(gint fd, gboolean close_fd)
{
    GWin32OutputStream *win32_stream;
