...
static GDBusInterfaceInfo *org_gtk_Menus_get_interface(void)
{
    ...
    if (interface_info == NULL)
    {
        ...
        info = g_dbus_node_info_new_for_xml("<node>"
                                            ...
                                            "    </method>"
                                            "    <method name='End'>"
                                            "      <arg type='au' name='groups' direction='in'/>" "    </method>" "    <signal name='Changed'>" "      arg type='a(uuuuaa{sv})' name='changes'/>" "    </signal>" "  </interface>" "</node>", &error);
        if (info == NULL)
            g_error("%s\n", error->message);
...
static GMenuExporterMenu *g_menu_exporter_group_add_menu(GMenuExporterGroup * group, GMenuModel * model);
static void g_menu_exporter_group_remove_menu(GMenuExporterGroup * group, guint id);

static GMenuExporterGroup *g_menu_exporter_create_group(GMenuExporter * exporter);
static GMenuExporterGroup *g_menu_exporter_lookup_group(GMenuExporter * exporter, guint group_id);
...
static GMenuExporterLink *g_menu_exporter_menu_create_links(GMenuExporterMenu * menu, gint position)
{
...
static GVariant *g_menu_exporter_menu_describe_item(GMenuExporterMenu * menu, gint position)
{
    ...
    for (link = g_sequence_get(iter); link; link = link->next)
        g_variant_builder_add(&builder, "{sv}", link->name, g_variant_new("(uu)", g_menu_exporter_group_get_id(link->menu->group), link->menu->id));
...
static void g_menu_exporter_menu_items_changed(GMenuModel * model, gint position, gint removed, gint added, gpointer user_data)
{
...
static void g_menu_exporter_menu_prepare(GMenuExporterMenu * menu)
{
    ...
    if (g_menu_model_is_mutable(menu->model))
        menu->handler_id = g_signal_connect(menu->model, "items-changed", G_CALLBACK(g_menu_exporter_menu_items_changed), menu);
...
static GMenuExporterMenu *g_menu_exporter_menu_new(GMenuExporterGroup * group, guint id, GMenuModel * model)
{
    GMenuExporterMenu *menu;
...
static void g_menu_exporter_group_subscribe(GMenuExporterGroup * group, GVariantBuilder * builder)
{
    GHashTableIter iter;
    gpointer key, val;
...
static void g_menu_exporter_group_unsubscribe(GMenuExporterGroup * group, gint count)
{
...
static GMenuExporterGroup *g_menu_exporter_group_new(GMenuExporter * exporter, guint id)
{
    GMenuExporterGroup *group;
...
static void g_menu_exporter_remote_subscribe(GMenuExporterRemote * remote, guint group_id, GVariantBuilder * builder)
{
    GMenuExporterGroup *group;
    guint count;
...
static void g_menu_exporter_remote_unsubscribe(GMenuExporterRemote * remote, guint group_id)
{
    GMenuExporterGroup *group;
    guint count;
...
static gboolean g_menu_exporter_remote_has_subscriptions(GMenuExporterRemote * remote)
{
...
static GMenuExporterRemote *g_menu_exporter_remote_new(GMenuExporter * exporter, guint watch_id)
{
...
static void g_menu_exporter_name_vanished(GDBusConnection * connection, const gchar * name, gpointer user_data)
{
...
static GVariant *g_menu_exporter_subscribe(GMenuExporter * exporter, const gchar * sender, GVariant * group_ids)
{
    GMenuExporterRemote *remote;
    GVariantBuilder builder;
    GVariantIter iter;
    ...
    if (remote == NULL)
    {
        ...
        watch_id = g_bus_watch_name_on_connection(exporter->connection, sender, G_BUS_NAME_WATCHER_FLAGS_NONE, NULL, g_menu_exporter_name_vanished, exporter, NULL);
...
static void g_menu_exporter_unsubscribe(GMenuExporter * exporter, const gchar * sender, GVariant * group_ids)
{
    GMenuExporterRemote *remote;
    GVariantIter iter;
    guint32 id;
...
static void g_menu_exporter_report(GMenuExporter * exporter, GVariant * report)
{
    ...
    g_dbus_connection_emit_signal(exporter->connection, NULL, exporter->object_path, "org.gtk.Menus", "Changed", g_variant_builder_end(&builder), NULL);
}
...
static void
g_menu_exporter_method_call(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * method_name, GVariant * parameters, GDBusMethodInvocation * invocation, gpointer user_data)
{
    GMenuExporter *exporter = user_data;
    GVariant *group_ids;
...
guint g_dbus_connection_export_menu_model(GDBusConnection * connection, const gchar * object_path, GMenuModel * menu, GError ** error)
{
    ...
    id = g_dbus_connection_register_object(connection, object_path, org_gtk_Menus_get_interface(), &vtable, exporter, g_menu_exporter_free, error);
