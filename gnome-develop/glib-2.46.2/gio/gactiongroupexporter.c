...
static GVariant *g_action_group_describe_action(GActionGroup * action_group, const gchar * name)
{
...
const char org_gtk_Actions_xml[] =
    ...
    "    </method>"
    "    <signal name='Changed'>"
    "      <arg type='as' name='removals'/>" "      <arg type='a{sb}' name='enable_changes'/>" "      <arg type='a{sv}' name='state_changes'/>" "      <arg type='a{s(bgav)}' name='additions'/>" "    </signal>" "  </interface>" "</node>";
...
static gboolean g_action_group_exporter_dispatch_events(gpointer user_data)
{
    ...
    while (g_hash_table_iter_next(&iter, &key, &value))
    {
         ...
         */
        g_assert(((events & (ACTION_ENABLED_CHANGED_EVENT | ACTION_STATE_CHANGED_EVENT)) == 0) != ((events & (ACTION_REMOVED_EVENT | ACTION_ADDED_EVENT)) == 0));
    ...
    g_dbus_connection_emit_signal(exporter->connection, NULL, exporter->object_path, "org.gtk.Actions", "Changed", g_variant_new("(asa{sb}a{sv}a{s(bgav)})", &removes, &enabled_changes, &state_changes, &adds), NULL);
...
static guint g_action_group_exporter_get_events(GActionGroupExporter * exporter, const gchar * name)
{
    return (gsize) g_hash_table_lookup(exporter->pending_changes, name);
}

static void g_action_group_exporter_set_events(GActionGroupExporter * exporter, const gchar * name, guint events)
{
...
static void g_action_group_exporter_action_added(GActionGroup * action_group, const gchar * action_name, gpointer user_data)
{
    GActionGroupExporter *exporter = user_data;
    guint event_mask;
...
static void g_action_group_exporter_action_removed(GActionGroup * action_group, const gchar * action_name, gpointer user_data)
{
    GActionGroupExporter *exporter = user_data;
    guint event_mask;
...
static void g_action_group_exporter_action_state_changed(GActionGroup * action_group, const gchar * action_name, GVariant * value, gpointer user_data)
{
    GActionGroupExporter *exporter = user_data;
    guint event_mask;
     ...
     */
    g_assert(~event_mask & ACTION_REMOVED_EVENT || event_mask & ACTION_ADDED_EVENT);
...
static void g_action_group_exporter_action_enabled_changed(GActionGroup * action_group, const gchar * action_name, gboolean enabled, gpointer user_data)
{
    GActionGroupExporter *exporter = user_data;
    guint event_mask;
...
static void
org_gtk_Actions_method_call(GDBusConnection * connection, const gchar * sender, const gchar * object_path, const gchar * interface_name, const gchar * method_name, GVariant * parameters, GDBusMethodInvocation * invocation, gpointer user_data)
{
    GActionGroupExporter *exporter = user_data;
    ...
    else if (g_str_equal(method_name, "Describe"))
    {
        ...
        {
            g_dbus_method_invocation_return_error(invocation, G_DBUS_ERROR, G_DBUS_ERROR_INVALID_ARGS, "The named action ('%s') does not exist.", name);
            return;
        }
    ...
    else if (g_str_equal(method_name, "Activate"))
    {
        ...
        if (G_IS_REMOTE_ACTION_GROUP(exporter->action_group))
            g_remote_action_group_activate_action_full(G_REMOTE_ACTION_GROUP(exporter->action_group), name, parameter, platform_data);
        else
    ...
    else if (g_str_equal(method_name, "SetState"))
    {
        ...
        if (G_IS_REMOTE_ACTION_GROUP(exporter->action_group))
            g_remote_action_group_change_action_state_full(G_REMOTE_ACTION_GROUP(exporter->action_group), name, state, platform_data);
        else
...
static void g_action_group_exporter_free(gpointer user_data)
{
    ...
    g_signal_handlers_disconnect_by_func(exporter->action_group, g_action_group_exporter_action_added, exporter);
    g_signal_handlers_disconnect_by_func(exporter->action_group, g_action_group_exporter_action_enabled_changed, exporter);
    g_signal_handlers_disconnect_by_func(exporter->action_group, g_action_group_exporter_action_state_changed, exporter);
    g_signal_handlers_disconnect_by_func(exporter->action_group, g_action_group_exporter_action_removed, exporter);
...
guint g_dbus_connection_export_action_group(GDBusConnection * connection, const gchar * object_path, GActionGroup * action_group, GError ** error)
{
    ...
    id = g_dbus_connection_register_object(connection, object_path, org_gtk_Actions, &vtable, exporter, g_action_group_exporter_free, error);
    ...
    g_signal_connect(action_group, "action-added", G_CALLBACK(g_action_group_exporter_action_added), exporter);
    g_signal_connect(action_group, "action-removed", G_CALLBACK(g_action_group_exporter_action_removed), exporter);
    g_signal_connect(action_group, "action-state-changed", G_CALLBACK(g_action_group_exporter_action_state_changed), exporter);
    g_signal_connect(action_group, "action-enabled-changed", G_CALLBACK(g_action_group_exporter_action_enabled_changed), exporter);
