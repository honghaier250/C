...
static void desktop_file_dir_changed(GFileMonitor * monitor, GFile * file, GFile * other_file, GFileMonitorEvent event_type, gpointer user_data)
{
 ...
 */
static gboolean desktop_file_dir_app_name_is_masked(DesktopFileDir * dir, const gchar * app_name)
{
 ...
 * reference.
 */
static void add_to_table_if_appropriate(GHashTable * apps, const gchar * app_name, GDesktopAppInfo * info)
{
    if (!info)
        return;
...
{
    if G_UNLIKELY
        (static_token_results_size == static_token_results_allocated)
    {
...
static void merge_token_results(gboolean first)
{
    ...
    if (first)
    {
            ...
            (static_search_results_allocated < static_token_results_size)
        {
            ...
            static_search_results = g_renew(struct search_result, static_search_results, static_search_results_allocated);
        }
    ...
    else
    {
        ...
        for (i = 0; i < static_token_results_size; i++)
        {
             ...
             * going to be matches.
             */
            while (k < static_search_results_size && static_search_results[k].app_name < static_token_results[i].app_name)
                k++;

            if (k < static_search_results_size && static_search_results[k].app_name == static_token_results[i].app_name)
            {
                /* We have a match.
                 *
                ...
                static_search_results[j].category = MAX(static_search_results[k].category, static_token_results[i].category);
                j++;
            }
        }
...
{
    if G_UNLIKELY
        (static_total_results_size + static_search_results_size > static_total_results_allocated)
    {
    ...
    memcpy(static_total_results + static_total_results_size, static_search_results, static_search_results_size * sizeof(struct search_result));
...
static UnindexedMimeTweaks *desktop_file_dir_unindexed_get_tweaks(DesktopFileDir * dir, const gchar * mime_type)
{
...
/* consumes 'to_add' */
static void expand_strv(gchar *** strv_ptr, gchar ** to_add, gchar * const *blacklist)
{
...
static void desktop_file_dir_unindexed_read_mimeapps_list(DesktopFileDir * dir, const gchar * filename, const gchar * added_group, gboolean tweaks_permitted)
{
    UnindexedMimeTweaks *tweaks;
    ...
    if G_UNLIKELY
        (mime_types != NULL && !tweaks_permitted)
    {
        g_warning("%s contains a [%s] group, but it is not permitted here.  Only the non-desktop-specific " "mimeapps.list file may add or remove associations.", filename, added_group);
    ...
    if G_UNLIKELY
        (mime_types != NULL && !tweaks_permitted)
    {
        g_warning("%s contains a [%s] group, but it is not permitted here.  Only the non-desktop-specific " "mimeapps.list file may add or remove associations.", filename, REMOVED_ASSOCIATIONS_GROUP);
        g_strfreev(mime_types);
        mime_types = NULL;
    }
...
static GDesktopAppInfo *desktop_file_dir_unindexed_get_app(DesktopFileDir * dir, const gchar * desktop_id)
{
...
static void desktop_file_dir_unindexed_get_all(DesktopFileDir * dir, GHashTable * apps)
{
    GHashTableIter iter;
    gpointer app_name;
    gpointer filename;
...
static void memory_index_add_token(MemoryIndex * mi, const gchar * token, gint match_category, const gchar * app_name)
{
...
static void memory_index_add_string(MemoryIndex * mi, const gchar * string, gint match_category, const gchar * app_name)
{
...
static void desktop_file_dir_unindexed_setup_search(DesktopFileDir * dir)
{
    ...
    while (g_hash_table_iter_next(&iter, &app, &path))
    {
        ...
        if (g_key_file_load_from_file(key_file, path, G_KEY_FILE_NONE, NULL) && !g_key_file_get_boolean(key_file, "Desktop Entry", "Hidden", NULL))
        {
...
static void desktop_file_dir_unindexed_search(DesktopFileDir * dir, const gchar * search_token)
{
    GHashTableIter iter;
    gpointer key, value;
...
static void desktop_file_dir_unindexed_mime_lookup(DesktopFileDir * dir, const gchar * mime_type, GPtrArray * hits, GPtrArray * blacklist)
{
    UnindexedMimeTweaks *tweaks;
    gint i;
    ...
    if (tweaks->additions)
    {
        for (i = 0; tweaks->additions[i]; i++)
        {
            ...
            if (!desktop_file_dir_app_name_is_masked(dir, app_name) && !array_contains(blacklist, app_name) && !array_contains(hits, app_name))
...
static void desktop_file_dir_unindexed_default_lookup(DesktopFileDir * dir, const gchar * mime_type, GPtrArray * results)
{
    UnindexedMimeTweaks *tweaks;
    gint i;
...
static void desktop_file_dir_unindexed_get_implementations(DesktopFileDir * dir, GList ** results, const gchar * interface)
{
 ...
 * here.
 */
static void desktop_file_dir_create_for_config(GArray * array, const gchar * config_dir)
{
    DesktopFileDir dir = { 0, };
...
static void desktop_file_dir_init(DesktopFileDir * dir)
{
     ...
     */
    dir->monitor = g_local_file_monitor_new_in_worker(watch_dir, TRUE, G_FILE_MONITOR_NONE, desktop_file_dir_changed, dir, NULL);
 ...
 */
static GDesktopAppInfo *desktop_file_dir_get_app(DesktopFileDir * dir, const gchar * desktop_id)
{
    if (!dir->app_names)
        return NULL;
 ...
 */
static void desktop_file_dir_mime_lookup(DesktopFileDir * dir, const gchar * mime_type, GPtrArray * hits, GPtrArray * blacklist)
{
 ...
 */
static void desktop_file_dir_default_lookup(DesktopFileDir * dir, const gchar * mime_type, GPtrArray * results)
{
 ...
 */
static void desktop_file_dir_search(DesktopFileDir * dir, const gchar * search_token)
{
    desktop_file_dir_unindexed_search(dir, search_token);
}

static void desktop_file_dir_get_implementations(DesktopFileDir * dir, GList ** results, const gchar * interface)
{
...
static void g_desktop_app_info_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
...
static void g_desktop_app_info_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
    GDesktopAppInfo *self = G_DESKTOP_APP_INFO(object);
...
static void g_desktop_app_info_class_init(GDesktopAppInfoClass * klass)
{
   ...
   */
    g_object_class_install_property(gobject_class, PROP_FILENAME, g_param_spec_string("filename", "Filename", "", NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}
...
static gboolean g_desktop_app_info_load_from_keyfile(GDesktopAppInfo * info, GKeyFile * key_file)
{
    char *start_group;
    char *type;
    char *try_exec;
    ...
    if (info->icon_name)
    {
        ...
        else
        {
            ...
            if ((p = strrchr(info->icon_name, '.')) != NULL && (strcmp(p, ".png") == 0 || strcmp(p, ".xpm") == 0 || strcmp(p, ".svg") == 0))
                *p = 0;
...
static gboolean expand_application_parameters(GDesktopAppInfo * info, const gchar * exec_line, GList ** uris, int *argc, char ***argv, GError ** error)
{
    GList *uri_list = *uris;
    ...
    if (exec_line == NULL)
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Desktop file didn't specify Exec field"));
        return FALSE;
    }
...
static void notify_desktop_launch(GDBusConnection * session_bus, GDesktopAppInfo * info, long pid, const char *display, const char *sn_id, GList * uris)
{
    ...
    g_dbus_message_set_body(msg, g_variant_new("(@aysxasa{sv})", g_variant_new_bytestring(desktop_file_id), display ? display : "", (gint64) pid, &uri_variant, &extras_variant));
...
g_desktop_app_info_launch_uris_with_spawn(GDesktopAppInfo * info,
                                          ...
                                          GList * uris,
                                          GAppLaunchContext * launch_context, GSpawnFlags spawn_flags, GSpawnChildSetupFunc user_setup, gpointer user_setup_data, GDesktopAppLaunchCallback pid_callback, gpointer pid_callback_data, GError ** error)
{
    ...
    do
    {
        ...
        {
            g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Unable to find terminal required for application"));
            goto out;
        }
        ...
        if (info->filename)
        {
            envp = g_environ_setenv(envp, "GIO_LAUNCHED_DESKTOP_FILE", info->filename, TRUE);
            envp = g_environ_setenv(envp, "GIO_LAUNCHED_DESKTOP_FILE_PID", "XXXXXXXXXXXXXXXXXXXX",    /* filled in child_setup */
                                    TRUE);
        ...
        if (!g_spawn_async(info->path, argv, envp, spawn_flags, child_setup, &data, &pid, error))
        {
            if (sn_id)
        ...
        notify_desktop_launch(session_bus, info, pid, NULL, sn_id, launched_uris);
...
static gchar *object_path_from_appid(const gchar * app_id)
{
    gchar *path;
    gint i, n;
...
static GVariant *g_desktop_app_info_make_platform_data(GDesktopAppInfo * info, GList * uris, GAppLaunchContext * launch_context)
{
    GVariantBuilder builder;
...
static gboolean g_desktop_app_info_launch_uris_with_dbus(GDesktopAppInfo * info, GDBusConnection * session_bus, GList * uris, GAppLaunchContext * launch_context)
{
    GVariantBuilder builder;
    gchar *object_path;
    ...
    g_dbus_connection_call(session_bus, info->app_id, object_path, "org.freedesktop.Application", uris ? "Open" : "Activate", g_variant_builder_end(&builder), NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL, NULL);
    g_free(object_path);
                                        ...
                                        GAppLaunchContext * launch_context, GSpawnFlags spawn_flags, GSpawnChildSetupFunc user_setup, gpointer user_setup_data, GDesktopAppLaunchCallback pid_callback, gpointer pid_callback_data, GError ** error)
{
    ...
    if (session_bus && info->app_id)
        ...
        success = g_desktop_app_info_launch_uris_with_spawn(info, session_bus, info->exec, uris, launch_context, spawn_flags, user_setup, user_setup_data, pid_callback, pid_callback_data, error);
...
{
    return g_desktop_app_info_launch_uris_internal(appinfo, uris, launch_context, _SPAWN_FLAGS_DEFAULT, NULL, NULL, NULL, NULL, error);
}
...
g_desktop_app_info_launch_uris_as_manager(GDesktopAppInfo * appinfo,
                                          ...
                                          GAppLaunchContext * launch_context, GSpawnFlags spawn_flags, GSpawnChildSetupFunc user_setup, gpointer user_setup_data, GDesktopAppLaunchCallback pid_callback, gpointer pid_callback_data, GError ** error)
{
    return g_desktop_app_info_launch_uris_internal((GAppInfo *) appinfo, uris, launch_context, spawn_flags, user_setup, user_setup_data, pid_callback, pid_callback_data, error);
}
...
static char *ensure_dir(DirType type, GError ** error)
{
    ...
    if (type == APP_DIR)
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), _("Can't create user application configuration folder %s: %s"), display_name, g_strerror(errsv));
    else
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(errsv), _("Can't create user MIME configuration folder %s: %s"), display_name, g_strerror(errsv));
...
static gboolean update_mimeapps_list(const char *desktop_id, const char *content_type, UpdateMimeFlags flags, GError ** error)
{
    ...
    if (!load_succeeded || (!g_key_file_has_group(key_file, ADDED_ASSOCIATIONS_GROUP) && !g_key_file_has_group(key_file, REMOVED_ASSOCIATIONS_GROUP) && !g_key_file_has_group(key_file, DEFAULT_APPLICATIONS_GROUP)))
    {
        g_key_file_free(key_file);
        key_file = g_key_file_new();
    }
    ...
    for (k = 0; content_types && content_types[k]; k++)
    {
        ...
        if (string == NULL || desktop_id == NULL)
            g_key_file_remove_key(key_file, DEFAULT_APPLICATIONS_GROUP, content_types[k], NULL);
        else
    ...
    for (k = 0; content_types && content_types[k]; k++)
    {
        ...
        if (list[0] == NULL || desktop_id == NULL)
            g_key_file_remove_key(key_file, ADDED_ASSOCIATIONS_GROUP, content_types[k], NULL);
        else
    ...
    for (k = 0; content_types && content_types[k]; k++)
    {
        ...
        if (list[0] == NULL || desktop_id == NULL)
            g_key_file_remove_key(key_file, REMOVED_ASSOCIATIONS_GROUP, content_types[k], NULL);
        else
            g_key_file_set_string_list(key_file, REMOVED_ASSOCIATIONS_GROUP, content_types[k], (const char *const *) list, i);
...
static gboolean g_desktop_app_info_set_as_last_used_for_type(GAppInfo * appinfo, const char *content_type, GError ** error)
{
    ...
    if (!info->desktop_id)
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Application information lacks an identifier"));
        return FALSE;
    }
...
static void run_update_command(char *command, char *subdir)
{
    ...
    if (g_spawn_async("/", argv, NULL,    /* envp */
                      G_SPAWN_SEARCH_PATH | G_SPAWN_STDOUT_TO_DEV_NULL | G_SPAWN_STDERR_TO_DEV_NULL | G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL,    /* No setup function */
                      &pid, &error))
...
static gboolean g_desktop_app_info_ensure_saved(GDesktopAppInfo * info, GError ** error)
{
    ...
    if (info->terminal)
        g_key_file_set_boolean(key_file, G_KEY_FILE_DESKTOP_GROUP, G_KEY_FILE_DESKTOP_KEY_TERMINAL, TRUE);
    if (info->nodisplay)
    ...
    if (info->generic_name != NULL)
        g_key_file_set_string(key_file, G_KEY_FILE_DESKTOP_GROUP, GENERIC_NAME_KEY, info->generic_name);

    if (info->fullname != NULL)
        g_key_file_set_string(key_file, G_KEY_FILE_DESKTOP_GROUP, FULL_NAME_KEY, info->fullname);
    ...
    if (fd == -1)
    {
        ...
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Can't create user desktop file %s"), display_name);
        g_free(display_name);
        g_free(filename);
        g_free(data);
        return FALSE;
 ...
 */
static gchar **get_list_of_mimetypes(const gchar * content_type, gboolean include_fallback)
{
    gchar *unaliased;
    GPtrArray *array;
...
static gchar **g_desktop_app_info_get_desktop_ids_for_content_type(const gchar * content_type, gboolean include_fallback)
{
...
gchar ***g_desktop_app_info_search(const gchar * search_string)
{
    ...
    for (i = 0; i < n_categories; i++)
    {
        ...
        while (start_of_category + n_items_in_category < static_total_results_size && static_total_results[start_of_category + n_items_in_category].category == this_category)
            n_items_in_category++;
...
G_GNUC_BEGIN_IGNORE_DEPRECATIONS typedef GDesktopAppInfoLookupIface GDesktopAppInfoLookupInterface;
...
char *g_desktop_app_info_get_string(GDesktopAppInfo * info, const char *key)
{
    ...
    return g_key_file_get_string(info->keyfile, G_KEY_FILE_DESKTOP_GROUP, key, NULL);
}
...
gboolean g_desktop_app_info_get_boolean(GDesktopAppInfo * info, const char *key)
{
    ...
    return g_key_file_get_boolean(info->keyfile, G_KEY_FILE_DESKTOP_GROUP, key, NULL);
}
...
gboolean g_desktop_app_info_has_key(GDesktopAppInfo * info, const char *key)
{
    ...
    return g_key_file_has_key(info->keyfile, G_KEY_FILE_DESKTOP_GROUP, key, NULL);
}
...
static gboolean app_info_has_action(GDesktopAppInfo * info, const gchar * action_name)
{
    gint i;
...
void g_desktop_app_info_launch_action(GDesktopAppInfo * info, const gchar * action_name, GAppLaunchContext * launch_context)
{
    ...
    if (session_bus && info->app_id)
    {
        ...
        g_dbus_connection_call(session_bus, info->app_id, object_path,
                               "org.freedesktop.Application", "ActivateAction", g_variant_new("(sav@a{sv})", action_name, NULL, g_desktop_app_info_make_platform_data(info, NULL, launch_context)), NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL, NULL);
        g_free(object_path);
    }
    else
    {
        ...
        if (exec_line)
            g_desktop_app_info_launch_uris_with_spawn(info, session_bus, exec_line, NULL, launch_context, _SPAWN_FLAGS_DEFAULT, NULL, NULL, NULL, NULL, NULL);
    }
