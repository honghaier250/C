...
static gboolean is_valid_unix(const gchar * address_entry, GHashTable * key_value_pairs, GError ** error)
{
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Address '%s' is invalid (need exactly one of path, tmpdir or abstract keys)"), address_entry);
        goto out;
    }
...
gboolean g_dbus_is_supported_address(const gchar * string, GError ** error)
{
    ...
    for (n = 0; a[n] != NULL; n++)
    {
        ...
        if (!_g_dbus_address_parse_entry(a[n], &transport_name, &key_value_pairs, error))
            goto out;
...
gboolean _g_dbus_address_parse_entry(const gchar * address_entry, gchar ** out_transport_name, GHashTable ** out_key_value_pairs, GError ** error)
{
    ...
    for (n = 0; kv_pairs != NULL && kv_pairs[n] != NULL; n++)
    {
        ...
        if (s == NULL)
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Key/Value pair %d, '%s', in address element '%s' does not contain an equal sign"), n, kv_pair, address_entry);
            goto out;
        }
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Error unescaping key or value in Key/Value pair %d, '%s', in address element '%s'"), n, kv_pair, address_entry);
            g_free(key);
            g_free(value);
            goto out;
        }
 ...
 */
static GIOStream *g_dbus_address_connect(const gchar * address_entry, const gchar * transport_name, GHashTable * key_value_pairs, GCancellable * cancellable, GError ** error)
{
    GIOStream *ret;
    ...
    else if (g_strcmp0(transport_name, "unix") == 0)
    {
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Error in address '%s' - the unix transport requires exactly one of the " "keys 'path' or 'abstract' to be set"), address_entry);
        }
        else if (path != NULL)
        {
        ...
        {
            connectable = G_SOCKET_CONNECTABLE(g_unix_socket_address_new_with_type(abstract, -1, G_UNIX_SOCKET_ADDRESS_ABSTRACT));
        }
        else
        {
    ...
    else if (g_strcmp0(transport_name, "tcp") == 0 || g_strcmp0(transport_name, "nonce-tcp") == 0)
    {
        ...
        if (host == NULL)
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Error in address '%s' - the host attribute is missing or malformed"), address_entry);
            goto out;
        }
        ...
        {
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Error in address '%s' - the port attribute is missing or malformed"), address_entry);
            goto out;
        }
        ...
        if (is_nonce)
        {
            ...
            if (nonce_file == NULL)
            {
                g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Error in address '%s' - the noncefile attribute is missing or malformed"), address_entry);
                goto out;
            }
        }
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Unknown or unsupported transport '%s' for address '%s'"), transport_name, address_entry);
    }
    ...
    if (connectable != NULL)
    {
        ...
        if (nonce_file != NULL)
        {
            ...
            if (num_bytes_read != 16)
            {
                ...
                {
                    g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, _("Error reading from nonce file '%s', expected 16 bytes, got %d"), nonce_file, (gint) num_bytes_read);
                }
                g_object_unref(ret);
                ret = NULL;
                fclose(f);
...
static GIOStream *g_dbus_address_try_connect_one(const gchar * address_entry, gchar ** out_guid, GCancellable * cancellable, GError ** error)
{
    ...
    if (!_g_dbus_address_parse_entry(address_entry, &transport_name, &key_value_pairs, error))
        goto out;
...
static void get_stream_thread_func(GTask * task, gpointer source_object, gpointer task_data, GCancellable * cancellable)
{
    GetStreamData *data = task_data;
    GIOStream *stream;
    GError *error = NULL;

    stream = g_dbus_address_get_stream_sync(data->address, &data->guid, cancellable, &error);
    if (stream)
...
static gchar *get_session_address_dbus_launch(GError ** error)
{
    ...
    {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Cannot spawn a message bus when setuid"));
        goto out;
    }
...
static HANDLE set_shm(const char *shm_name, const char *value)
{
    ...
    shared_mem = CreateFileMappingA(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, strlen(value) + 1, shm_name);
    if (shared_mem == 0)
        return 0;
...
{
    if (((HANDLE) _get_osfhandle(fileno(stdout)) == INVALID_HANDLE_VALUE || (HANDLE) _get_osfhandle(fileno(stderr)) == INVALID_HANDLE_VALUE) && AllocConsole())
    {
...
__declspec(dllexport)
     void CALLBACK g_win32_run_session_bus(HWND hwnd, HINSTANCE hinst, char *cmdline, int nCmdShow);
...
#    endif
            ...
            res = CreateProcessW(rundll_path, args, 0, 0, FALSE, NORMAL_PRIORITY_CLASS | CREATE_NO_WINDOW | DETACHED_PROCESS, 0, NULL /* TODO: Should be root */ ,
                                 &si, &pi);
            if (res)
                address = read_shm(DBUS_DAEMON_ADDRESS_INFO);
        }
    ...
    if (address == NULL)
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Session dbus not running, and autolaunch failed"));
    ...
    switch (bus_type)
    {
        ...
        case G_BUS_TYPE_STARTER:
            ...
            else
            {
                if (starter_bus != NULL)
                {
                    g_set_error(&local_error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Cannot determine bus address from DBUS_STARTER_BUS_TYPE environment variable" " - unknown value '%s'"), starter_bus);
                }
                else
                {
                    g_set_error_literal(&local_error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Cannot determine bus address because the DBUS_STARTER_BUS_TYPE environment " "variable is not set"));
                }
            }
            break;

        default:
            g_set_error(&local_error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Unknown bus type %d"), bus_type);
            break;
    }
...
out:
    if (G_UNLIKELY(_g_dbus_debug_address()))
    {
        ...
        {
            g_print("GDBus-debug:Address: Cannot look-up address bus type '%s': %s\n", _g_dbus_enum_to_string(G_TYPE_BUS_TYPE, bus_type), local_error ? local_error->message : "");
        }
        _g_dbus_debug_print_unlock();
    }
