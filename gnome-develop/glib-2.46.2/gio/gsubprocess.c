...
G_DEFINE_TYPE_WITH_CODE(GSubprocess, g_subprocess, G_TYPE_OBJECT, G_IMPLEMENT_INTERFACE(G_TYPE_INITABLE, initable_iface_init));
...
static gint unix_open_file(const char *filename, gint mode, GError ** error)
{
    ...
    if (my_fd < 0)
    {
        ...
        g_set_error(error, G_IO_ERROR, g_io_error_from_errno(saved_errno), _("Error opening file '%s': %s"), display_name, g_strerror(saved_errno));
        g_free(display_name);
        /* fall through... */
    }
...
static void g_subprocess_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
...
static gboolean initable_init(GInitable * initable, GCancellable * cancellable, GError ** error)
{
    ...
    success = g_spawn_async_with_pipes(self->launcher ? self->launcher->cwd : NULL, self->argv, self->launcher ? self->launcher->envp : NULL, spawn_flags,
#ifdef G_OS_UNIX
...
static void g_subprocess_class_init(GSubprocessClass * class)
{
    ...
    g_object_class_install_property(gobject_class, PROP_FLAGS, g_param_spec_flags("flags", P_("Flags"), P_("Subprocess flags"), G_TYPE_SUBPROCESS_FLAGS, 0, G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
    g_object_class_install_property(gobject_class, PROP_ARGV, g_param_spec_boxed("argv", P_("Arguments"), P_("Argument vector"), G_TYPE_STRV, G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
}
...
GSubprocess *g_subprocess_newv(const gchar * const *argv, GSubprocessFlags flags, GError ** error)
{
    ...
    return g_initable_new(G_TYPE_SUBPROCESS, NULL, error, "argv", argv, "flags", flags, NULL);
}
...
static void g_subprocess_wait_cancelled(GCancellable * cancellable, gpointer user_data)
{
...
static void g_subprocess_sync_done(GObject * source_object, GAsyncResult * result, gpointer user_data)
{
...
{
    return g_subprocess_wait(subprocess, cancellable, error) && g_spawn_check_exit_status(subprocess->status, error);
}
...
{
    return g_subprocess_wait_finish(subprocess, result, error) && g_spawn_check_exit_status(subprocess->status, error);
}
...
static void g_subprocess_dispatch_signal(GSubprocess * subprocess, gint signalnum)
{
     ...
     */
    g_main_context_invoke_full(GLIB_PRIVATE_CALL(g_get_worker_context) (), G_PRIORITY_HIGH_IDLE, g_subprocess_actually_send_signal, g_slice_dup(SignalRecord, &signal_record), NULL);
}
...
#else
    g_critical("g_subprocess_get_term_sig() called on Windows, where " "g_subprocess_get_if_signaled() always returns FALSE...");
    return 0;
#endif
}

/*< private >*/
void g_subprocess_set_launcher(GSubprocess * subprocess, GSubprocessLauncher * launcher)
{
...
static void g_subprocess_communicate_made_progress(GObject * source_object, GAsyncResult * result, gpointer user_data)
{
    ...
    if (source == subprocess->stdin_pipe || source == state->stdout_buf || source == state->stderr_buf)
    {
...
static gboolean g_subprocess_communicate_cancelled(gpointer user_data)
{
...
static CommunicateState *g_subprocess_communicate_internal(GSubprocess * subprocess, gboolean add_nul, GBytes * stdin_buf, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    CommunicateState *state;
    GTask *task;
    ...
    if (subprocess->stdin_pipe)
    {
        ...
        g_output_stream_splice_async(subprocess->stdin_pipe, (GInputStream *) state->stdin_buf,
                                     G_OUTPUT_STREAM_SPLICE_CLOSE_SOURCE | G_OUTPUT_STREAM_SPLICE_CLOSE_TARGET, G_PRIORITY_DEFAULT, state->cancellable, g_subprocess_communicate_made_progress, g_object_ref(task));
        state->outstanding_ops++;
    }
    ...
    if (subprocess->stdout_pipe)
    {
        ...
        g_output_stream_splice_async((GOutputStream *) state->stdout_buf, subprocess->stdout_pipe, G_OUTPUT_STREAM_SPLICE_CLOSE_SOURCE, G_PRIORITY_DEFAULT, state->cancellable, g_subprocess_communicate_made_progress, g_object_ref(task));
        state->outstanding_ops++;
    }
    ...
    if (subprocess->stderr_pipe)
    {
        ...
        g_output_stream_splice_async((GOutputStream *) state->stderr_buf, subprocess->stderr_pipe, G_OUTPUT_STREAM_SPLICE_CLOSE_SOURCE, G_PRIORITY_DEFAULT, state->cancellable, g_subprocess_communicate_made_progress, g_object_ref(task));
        state->outstanding_ops++;
    }

    g_subprocess_wait_async(subprocess, state->cancellable, g_subprocess_communicate_made_progress, g_object_ref(task));
    state->outstanding_ops++;
...
gboolean g_subprocess_communicate(GSubprocess * subprocess, GBytes * stdin_buf, GCancellable * cancellable, GBytes ** stdout_buf, GBytes ** stderr_buf, GError ** error)
{
    ...
    g_subprocess_sync_setup();
    g_subprocess_communicate_internal(subprocess, FALSE, stdin_buf, cancellable, g_subprocess_sync_done, &result);
    g_subprocess_sync_complete(&result);
...
gboolean g_subprocess_communicate_utf8(GSubprocess * subprocess, const char *stdin_buf, GCancellable * cancellable, char **stdout_buf, char **stderr_buf, GError ** error)
{
    ...
    g_subprocess_sync_setup();
    g_subprocess_communicate_internal(subprocess, TRUE, stdin_bytes, cancellable, g_subprocess_sync_done, &result);
    g_subprocess_sync_complete(&result);
...
static gboolean communicate_result_validate_utf8(const char *stream_name, char **return_location, GMemoryOutputStream * buffer, GError ** error)
{
    ...
    if (buffer)
    {
        ...
        *return_location = g_memory_output_stream_steal_data(buffer);
        if (!g_utf8_validate(*return_location, -1, &end))
        {
            ...
            g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, "Invalid UTF-8 in child %s at offset %lu", stream_name, (unsigned long) (end - *return_location));
            return FALSE;
        }
    }
    else
