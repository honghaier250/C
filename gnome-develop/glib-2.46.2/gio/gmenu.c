...
G_DEFINE_TYPE(GMenu, g_menu, G_TYPE_MENU_MODEL) G_DEFINE_TYPE(GMenuItem, g_menu_item, G_TYPE_OBJECT)
     struct item
     {
         GHashTable *attributes;
         GHashTable *links;
...
static void g_menu_get_item_attributes(GMenuModel * model, gint position, GHashTable ** table)
{
    GMenu *menu = G_MENU(model);
...
static void g_menu_get_item_links(GMenuModel * model, gint position, GHashTable ** table)
{
    GMenu *menu = G_MENU(model);
...
static gboolean valid_attribute_name(const gchar * name)
{
    ...
    for (i = 1; name[i]; i++)
    {
        if (name[i] != '-' && !g_ascii_islower(name[i]) && !g_ascii_isdigit(name[i]))
            return FALSE;
