...
G_DEFINE_TYPE(GWin32AppInfoFileExtension, g_win32_appinfo_file_extension, G_TYPE_OBJECT) G_DEFINE_TYPE(GWin32AppInfoHandler, g_win32_appinfo_handler, G_TYPE_OBJECT) G_DEFINE_TYPE(GWin32AppInfoApplication, g_win32_appinfo_application, G_TYPE_OBJECT)
     static void g_win32_appinfo_url_schema_dispose(GObject * object)
{
...
static void g_win32_appinfo_handler_dispose(GObject * object)
{
...
static void g_win32_appinfo_file_extension_dispose(GObject * object)
{
...
static void g_win32_appinfo_application_dispose(GObject * object)
{
...
static void g_win32_appinfo_url_schema_class_init(GWin32AppInfoURLSchemaClass * klass)
{
...
static void g_win32_appinfo_file_extension_class_init(GWin32AppInfoFileExtensionClass * klass)
{
    GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
...
static void g_win32_appinfo_handler_class_init(GWin32AppInfoHandlerClass * klass)
{
    GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
...
static void g_win32_appinfo_application_class_init(GWin32AppInfoApplicationClass * klass)
{
    GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
...
static void g_win32_appinfo_url_schema_init(GWin32AppInfoURLSchema * self)
{
    self->handlers = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);
}

static void g_win32_appinfo_file_extension_init(GWin32AppInfoFileExtension * self)
{
    self->handlers = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);
    self->other_apps = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);
}

static void g_win32_appinfo_handler_init(GWin32AppInfoHandler * self)
{
}

static void g_win32_appinfo_application_init(GWin32AppInfoApplication * self)
{
    self->supported_urls = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);
    self->supported_exts = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);
}
...
static gunichar2 *g_wcsdup(const gunichar2 * str, gssize str_size)
{
    if (str_size == -1)
    {
...
static gunichar2 *read_resource_string(gunichar2 * res)
{
    gunichar2 *id_str;
    ...
    resource_module = LoadLibraryExW(filename_str, 0, LOAD_LIBRARY_AS_DATAFILE | LOAD_LIBRARY_AS_IMAGE_RESOURCE);
...
static void read_handler_icon(GWin32RegistryKey * proxy_key, GWin32RegistryKey * program_key, GIcon ** icon_out)
{
    gint counter;
    GWin32RegistryKey *key;
    ...
    *icon_out = NULL;
    ...
    for (counter = 0; counter < 2; counter++)
    {
        ...
        if (icon_key != NULL)
        {
            ...
            if (g_win32_registry_key_get_value(icon_key, TRUE, "", &default_type, (gpointer *) & default_value, NULL, NULL))
            {
                if (default_type == G_WIN32_REGISTRY_VALUE_STR || default_value[0] != '\0')
...
/* output_size is in *bytes*, not gunichar2s! */
     static gboolean build_registry_path(gunichar2 * output, gsize output_size, ...)
{
    va_list ap;
    gboolean result;
...
static gboolean build_registry_pathv(gunichar2 * output, gsize output_size, va_list components)
{
    ...
    for (length = 0, component = va_arg(lentest, gunichar2 *); component != NULL; component = va_arg(lentest, gunichar2 *))
    {
    ...
    if ((length >= REG_PATH_MAX_SIZE) || (length * sizeof(gunichar2) >= output_size))
        return FALSE;

    output[0] = L'\0';

    for (p = output, component = va_arg(components, gunichar2 *); component != NULL; component = va_arg(components, gunichar2 *))
    {
        length = wcslen(component);
        wcscat(p, component);
        p += length;
...
static GWin32RegistryKey *_g_win32_registry_key_build_and_new_w(GError ** error, ...)
{
    va_list ap;
...
static gboolean utf8_and_fold(const gunichar2 * str, gchar ** str_u8, gchar ** str_u8_folded)
{
    gchar *u8;
    gchar *folded;
...
follow_class_chain_to_handler(const gunichar2 * program_id,
                              ...
                              gunichar2 ** program_command, GWin32RegistryKey ** program_key, gunichar2 ** proxy_id, gunichar2 ** proxy_command, GWin32RegistryKey ** proxy_key, gchar ** program_id_u8, gchar ** program_id_folded)
{
    GWin32RegistryKey *key;
    ...
    *proxy_command = NULL;
    ...
    key = _g_win32_registry_key_build_and_new_w(NULL, HKCR, program_id, SHELL_OPEN_COMMAND, NULL);

    if (key != NULL)
    {
        got_value = g_win32_registry_key_get_value_w(key, TRUE, L"", &val_type, (void **) program_command, NULL, NULL);
        if (got_value && val_type == G_WIN32_REGISTRY_VALUE_STR)
        {
            if ((program_id_u8 != NULL || program_id_folded != NULL) && !utf8_and_fold(program_id, program_id_u8, program_id_folded))
            {
                g_object_unref(key);
    ...
    got_value = g_win32_registry_key_get_value_w(key, TRUE, L"", &val_type, (void **) proxy_id, &proxy_id_size, NULL);
    if (!got_value || (val_type != G_WIN32_REGISTRY_VALUE_STR))
    {
        g_object_unref(key);
    ...
    key = _g_win32_registry_key_build_and_new_w(NULL, HKCR, *proxy_id, SHELL_OPEN_COMMAND, NULL);
    ...
    got_value = g_win32_registry_key_get_value_w(key, TRUE, L"", &val_type, (void **) proxy_command, NULL, NULL);
    g_object_unref(key);

    if (!got_value || val_type != G_WIN32_REGISTRY_VALUE_STR || ((program_id_u8 != NULL || program_id_folded != NULL) && !utf8_and_fold(program_id, program_id_u8, program_id_folded)))
    {
        g_clear_pointer(proxy_id, g_free);
...
static void extract_executable(gunichar2 * commandline, gchar ** ex_out, gchar ** ex_basename_out, gchar ** ex_folded_out, gchar ** ex_folded_basename_out)
{
    gchar *ex;
    gchar *ex_folded;
    gunichar2 *p;
        ...
        *ex_out = ex;
        ...
        if (ex_basename_out)
        {
            *ex_basename_out = &ex[strlen(ex) - 1];
            ...
            {
                if ((*ex_basename_out)[0] == '/' || (*ex_basename_out)[0] == '\\')
                {
                    *ex_basename_out += 1;
                    break;
                }
        ...
        *ex_folded_out = ex_folded;
        ...
        if (ex_folded_basename_out)
        {
            *ex_folded_basename_out = &ex_folded[strlen(ex_folded) - 1];
            ...
            {
                if ((*ex_folded_basename_out)[0] == '/' || (*ex_folded_basename_out)[0] == '\\')
                {
...
static void get_url_association(const gunichar2 * schema)
{
    ...
    user_choice = _g_win32_registry_key_build_and_new_w(NULL, URL_ASSOCIATIONS, schema, USER_CHOICE, NULL);
    ...
    if (!g_win32_registry_key_get_value_w(user_choice, TRUE, L"Progid", &val_type, (void **) &program_id, &program_id_size, NULL))
    {
        g_free(schema_u8);
    ...
    if (!follow_class_chain_to_handler(program_id, program_id_size, &program_command, &program_key, &proxy_id, &proxy_command, &proxy_key, &program_id_u8, &program_id_folded))
    {
        g_free(schema_u8);
        g_free(schema_folded);
        g_free(program_id);
    ...
    if (handler_rec == NULL)
    {
        ...
        handler_rec->handler_id_folded = g_strdup(program_id_folded);
        handler_rec->handler_command = program_command ? g_wcsdup(program_command, -1) : NULL;
        handler_rec->proxy_id = proxy_id ? g_wcsdup(proxy_id, -1) : NULL;
        handler_rec->proxy_command = proxy_command ? g_wcsdup(proxy_command, -1) : NULL;
        extract_executable(proxy_command ? proxy_command : program_command, &handler_rec->executable, &handler_rec->executable_basename, &handler_rec->executable_folded, NULL);
        read_handler_icon(proxy_key, program_key, &handler_rec->icon);
        g_hash_table_insert(handlers, g_strdup(program_id_folded), handler_rec);
    }
    else
    {
    ...
    handler_rec_in_url = g_hash_table_lookup(schema_rec->handlers, program_id_folded);

    if (handler_rec_in_url == NULL && schema_rec->chosen_handler != handler_rec)
        g_hash_table_insert(schema_rec->handlers, g_strdup(program_id_folded), g_object_ref(handler_rec));
...
static void get_file_ext(const gunichar2 * ext)
{
    ...
    open_with_progids = _g_win32_registry_key_build_and_new_w(NULL, FILE_EXTS, ext, OPEN_WITH_PROGIDS, NULL);

    user_choice = _g_win32_registry_key_build_and_new_w(NULL, FILE_EXTS, ext, USER_CHOICE, NULL);
    ...
    file_extn = NULL;
    file_ext_known = g_hash_table_lookup_extended(extensions, ext_folded, NULL, (void **) &file_extn);
    ...
    if (user_choice != NULL)
    {
        if (g_win32_registry_key_get_value_w(user_choice, TRUE, L"Progid", &val_type, (void **) &program_id, &program_id_size, NULL))
        {
            ...
            if (val_type == G_WIN32_REGISTRY_VALUE_STR && follow_class_chain_to_handler(program_id, program_id_size, &program_command, &program_key, &proxy_id, &proxy_command, &proxy_key, &program_id_u8, &program_id_folded))
            {
                handler_rec = g_hash_table_lookup(handlers, program_id_folded);
                ...
                handler_rec_in_ext = g_hash_table_lookup(file_extn->handlers, program_id_folded);

                if (file_extn->chosen_handler == NULL)
                {
                    g_hash_table_insert(file_extn->handlers, g_strdup(program_id_folded), g_object_ref(handler_rec));
                }
                else if (handler_rec_in_ext == NULL)
                {
                    if (file_extn->chosen_handler->handler_id_folded && strcmp(file_extn->chosen_handler->handler_id_folded, program_id_folded) != 0)
                        g_hash_table_insert(file_extn->handlers, g_strdup(program_id_folded), g_object_ref(handler_rec));
                }
            ...
            while (g_win32_registry_value_iter_next(&iter, TRUE, NULL))
            {
                ...
                if ((!g_win32_registry_value_iter_get_value_type(&iter,
                    ...
                    (!g_win32_registry_value_iter_get_name_w(&iter, &value_name,
                                                             &value_name_len,
                                                             NULL)) ||
                    (value_name_len <= 0) || (!g_win32_registry_value_iter_get_data_w(&iter, TRUE, (void **) &value_data, &value_data_size, NULL)) || (value_data_size < sizeof(gunichar2)) || (value_data[0] == L'\0'))
                    continue;

                value_name_size = sizeof(gunichar2) * (value_name_len + 1);

                if (!follow_class_chain_to_handler(value_name, value_name_size, &program_command, &program_key, &proxy_id, &proxy_command, &proxy_key, &program_id_u8, &program_id_folded))
                    continue;
                ...
                if (handler_rec == NULL)
                {
                    ...
                    handler_rec->proxy_key = proxy_key;
                    handler_rec->key = program_key;
                    handler_rec->handler_id = g_wcsdup(value_name, value_name_size);
                    handler_rec->handler_id_folded = g_strdup(program_id_folded);
                    handler_rec->handler_command = program_command ? g_wcsdup(program_command, -1) : NULL;
                    handler_rec->proxy_id = proxy_id ? g_wcsdup(proxy_id, -1) : NULL;
                    handler_rec->proxy_command = proxy_command ? g_wcsdup(proxy_command, -1) : NULL;
                ...
                if (handler_rec_in_ext == NULL)
                {
                    if (file_extn->chosen_handler == NULL)
                        g_hash_table_insert(file_extn->handlers, g_strdup(program_id_folded), g_object_ref(handler_rec));
                    else if (file_extn->chosen_handler->handler_id_folded && strcmp(file_extn->chosen_handler->handler_id_folded, program_id_folded) != 0)
                        g_hash_table_insert(file_extn->handlers, g_strdup(program_id_folded), g_object_ref(handler_rec));
                }
...
static void collect_capable_apps_from_clients(GPtrArray * capable_apps, GPtrArray * priority_capable_apps, gboolean user_registry)
{
    ...
    if (user_registry)
        clients = g_win32_registry_key_new_w(L"HKEY_CURRENT_USER\\Software\\Clients", NULL);
    else
        clients = g_win32_registry_key_new_w(L"HKEY_LOCAL_MACHINE\\Software\\Clients", NULL);
    ...
    while (g_win32_registry_subkey_iter_next(&clients_iter, TRUE, NULL))
    {
        ...
        if (!g_win32_registry_subkey_iter_get_name_w(&clients_iter, &client_type_name, &client_type_name_len, NULL))
            continue;

        system_client_type = g_win32_registry_key_get_child_w(clients, client_type_name, NULL);

        if (system_client_type == NULL)
            continue;

        if (g_win32_registry_key_get_value_w(system_client_type, TRUE, L"", &default_type, (gpointer *) & default_value, NULL, NULL))
        {
            if (default_type != G_WIN32_REGISTRY_VALUE_STR || default_value[0] == L'\0')
                g_clear_pointer(&default_value, g_free);
        }

        if (!g_win32_registry_subkey_iter_init(&subkey_iter, system_client_type, NULL))
        {
            g_clear_pointer(&default_value, g_free);
        ...
        while (g_win32_registry_subkey_iter_next(&subkey_iter, TRUE, NULL))
        {
            ...
            if (!g_win32_registry_subkey_iter_get_name_w(&subkey_iter, &client_name, &client_name_len, NULL))
                continue;

            system_client = g_win32_registry_key_get_child_w(system_client_type, client_name, NULL);
            ...
            system_client_assoc = g_win32_registry_key_get_child_w(system_client, L"Capabilities\\FileAssociations", NULL);
            ...
            {
                system_client_assoc = g_win32_registry_key_get_child_w(system_client, L"Capabilities\\UrlAssociations", NULL);
...
static void collect_capable_apps_from_registered_apps(GPtrArray * capable_apps, gboolean user_registry)
{
    GWin32RegistryValueIter iter;
    ...
    if (user_registry)
        registered_apps = g_win32_registry_key_new_w(L"HKEY_CURRENT_USER\\Software\\RegisteredApplications", NULL);
    else
        registered_apps = g_win32_registry_key_new_w(L"HKEY_LOCAL_MACHINE\\Software\\RegisteredApplications", NULL);
    ...
    while (g_win32_registry_value_iter_next(&iter, TRUE, NULL))
    {
        ...
        if ((!g_win32_registry_value_iter_get_value_type(&iter,
                                                         &value_type,
                                                         NULL)) ||
            (value_type != G_WIN32_REGISTRY_VALUE_STR) || (!g_win32_registry_value_iter_get_data_w(&iter, TRUE, (void **) &value_data, &value_data_size, NULL)) || (value_data_size < sizeof(gunichar2)) || (value_data[0] == L'\0'))
            continue;

        if (build_registry_path(possible_location, sizeof(possible_location), HKCU, value_data, NULL))
        ...
        if (!build_registry_path(possible_location, sizeof(possible_location), user_registry ? HKCU : HKLM, value_data, NULL))
            continue;
...
static void read_capable_app(gunichar2 * input_app_key_path, gboolean user_specific, gboolean default_app)
{
    GWin32AppInfoApplication *app;
    ...
    capabilities = g_win32_registry_key_get_child_w(appkey, L"Capabilities", NULL);
    ...
    shell_open_command_key = g_win32_registry_key_get_child_w(appkey, L"shell\\open\\command", NULL);
    ...
    success = g_win32_registry_key_get_value_w(shell_open_command_key, TRUE, L"", &vtype, (gpointer *) & shell_open_command, NULL, NULL);
    ...
    extract_executable(shell_open_command, &app_executable, &app_executable_basename, &app_executable_folded, &app_executable_folded_basename);
    ...
    if (app == NULL)
    {
        ...
        app->executable_basename = &app->executable[app_executable_basename - app_executable];
        app->executable_folded = g_strdup(app_executable_folded);
        ...
        g_hash_table_insert(apps_by_id, g_strdup(canonical_name_folded), app);
    }

    fallback_friendly_name = NULL;
    success = g_win32_registry_key_get_value_w(appkey, TRUE, L"", &vtype, (void **) &fallback_friendly_name, NULL, NULL);
    ...
    if (fallback_friendly_name && app->pretty_name == NULL)
    {
        ...
        g_clear_pointer(&app->pretty_name_u8, g_free);
        app->pretty_name_u8 = g_utf16_to_utf8(fallback_friendly_name, -1, NULL, NULL, NULL);
    }

    friendly_name = NULL;
    success = g_win32_registry_key_get_value_w(capabilities, TRUE, L"LocalizedString", &vtype, (void **) &friendly_name, NULL, NULL);
    ...
    if (friendly_name && app->localized_pretty_name == NULL)
    {
        ...
        g_clear_pointer(&app->localized_pretty_name_u8, g_free);
        app->localized_pretty_name_u8 = g_utf16_to_utf8(friendly_name, -1, NULL, NULL, NULL);
    }

    description = NULL;
    success = g_win32_registry_key_get_value_w(capabilities, TRUE, L"ApplicationDescription", &vtype, (void **) &description, NULL, NULL);
    ...
    default_icon_key = g_win32_registry_key_get_child_w(appkey, L"DefaultIcon", NULL);

    icon_source = NULL;

    if (default_icon_key != NULL)
    {
        success = g_win32_registry_key_get_value_w(default_icon_key, TRUE, L"", &vtype, (void **) &icon_source, NULL, NULL);
    ...
    if (icon_source == NULL)
    {
        success = g_win32_registry_key_get_value_w(capabilities, TRUE, L"ApplicationIcon", &vtype, (void **) &icon_source, NULL, NULL);
    ...
    success = g_win32_registry_key_get_value_w(capabilities, TRUE, L"ApplicationName", &vtype, (void **) &narrow_application_name, NULL, NULL);
    ...
    associations = g_win32_registry_key_get_child_w(capabilities, L"FileAssociations", NULL);
        ...
        if (g_win32_registry_value_iter_init(&iter, associations, NULL))
        {
            ...
            while (g_win32_registry_value_iter_next(&iter, TRUE, NULL))
            {
                ...
                if ((!g_win32_registry_value_iter_get_value_type(&iter,
                    ...
                    (file_extension[0] != L'.') || (!g_win32_registry_value_iter_get_data_w(&iter, TRUE, (void **) &extension_handler, &extension_handler_size, NULL)) || (extension_handler_size < sizeof(gunichar2)) || (extension_handler[0] == L'\0'))
                    continue;

                if (!follow_class_chain_to_handler(extension_handler, extension_handler_size, &program_command, &program_key, &proxy_id, &proxy_command, &proxy_key, &program_id_u8, &program_id_folded))
                    continue;
                ...
                if (handler_rec == NULL)
                {
                    ...
                    handler_rec->proxy_key = proxy_key;
                    handler_rec->key = program_key;
                    handler_rec->handler_id = g_wcsdup(extension_handler, extension_handler_size);
                    handler_rec->handler_id_folded = g_strdup(program_id_folded);
                    handler_rec->handler_command = program_command ? g_wcsdup(program_command, -1) : NULL;
                    handler_rec->proxy_id = proxy_id ? g_wcsdup(proxy_id, -1) : NULL;
                    handler_rec->proxy_command = proxy_command ? g_wcsdup(proxy_command, -1) : NULL;
                ...
                if (utf8_and_fold(file_extension, &file_extension_u8, &file_extension_folded))
                {
                    ext = g_hash_table_lookup(extensions, file_extension_folded);
                    ...
                    handler_rec_in_ext = g_hash_table_lookup(ext->handlers, program_id_folded);

                    if (handler_rec_in_ext == NULL)
                    {
                        if (ext->chosen_handler == NULL)
                            g_hash_table_insert(ext->handlers, g_strdup(program_id_folded), g_object_ref(handler_rec));
                        else if (ext->chosen_handler->handler_id_folded && strcmp(ext->chosen_handler->handler_id_folded, program_id_folded) != 0)
                            g_hash_table_insert(ext->handlers, g_strdup(program_id_folded), g_object_ref(handler_rec));
                    }

                    handler_rec_in_ext = g_hash_table_lookup(app->supported_exts, file_extension_folded);

                    if (handler_rec_in_ext == NULL)
                        g_hash_table_insert(app->supported_exts, g_strdup(file_extension_folded), g_object_ref(handler_rec));
            ...
            while (g_win32_registry_value_iter_next(&iter, TRUE, NULL))
            {
                ...
                if ((!g_win32_registry_value_iter_get_value_type(&iter,
                    ...
                    (!g_win32_registry_value_iter_get_name_w(&iter,
                                                             ...
                                                             NULL)) ||
                    (url_schema_len <= 0) ||
                    (url_schema[0] == L'\0') || (!g_win32_registry_value_iter_get_data_w(&iter, TRUE, (void **) &schema_handler, &schema_handler_size, NULL)) || (schema_handler_size < sizeof(gunichar2)) || (schema_handler[0] == L'\0'))
                    continue;

                if (!follow_class_chain_to_handler(schema_handler, schema_handler_size, &program_command, &program_key, &proxy_id, &proxy_command, &proxy_key, &program_id_u8, &program_id_folded))
                    continue;
                ...
                if (handler_rec == NULL)
                {
                    ...
                    handler_rec->proxy_key = proxy_key;
                    handler_rec->key = program_key;
                    handler_rec->handler_id = g_wcsdup(schema_handler, schema_handler_size);
                    handler_rec->handler_id_folded = g_strdup(program_id_folded);
                    handler_rec->handler_command = program_command ? g_wcsdup(program_command, -1) : NULL;
                    handler_rec->proxy_id = proxy_id ? g_wcsdup(proxy_id, -1) : NULL;
                    handler_rec->proxy_command = proxy_command ? g_wcsdup(proxy_command, -1) : NULL;
                ...
                if (utf8_and_fold(url_schema, &schema_u8, &schema_folded))
                {
                    ...
                    if (schema == NULL)
                    {
                        ...
                        schema->schema_folded = g_strdup(schema_folded);
                        g_hash_table_insert(urls, g_strdup(schema_folded), schema);
                    }

                    handler_rec_in_url = g_hash_table_lookup(schema->handlers, program_id_folded);

                    if (handler_rec_in_url == NULL)
                        g_hash_table_insert(schema->handlers, g_strdup(program_id_folded), g_object_ref(handler_rec));

                    handler_rec_in_url = g_hash_table_lookup(app->supported_urls, schema_folded);

                    if (handler_rec_in_url == NULL)
                        g_hash_table_insert(app->supported_urls, g_strdup(schema_folded), g_object_ref(handler_rec));
...
static void read_urls(GWin32RegistryKey * url_associations)
{
    ...
    {
        if (!g_win32_registry_subkey_iter_get_name_w(&url_iter, &url_schema, &url_schema_len, NULL))
            continue;
...
static void read_exeapps(void)
{
    ...
    applications_key = g_win32_registry_key_new_w(L"HKEY_CLASSES_ROOT\\Applications", NULL);
    ...
    while (g_win32_registry_subkey_iter_next(&app_iter, TRUE, NULL))
    {
        ...
        if (!g_win32_registry_subkey_iter_get_name_w(&app_iter, &app_exe_basename, &app_exe_basename_len, NULL))
            continue;

        incapable_app = g_win32_registry_key_get_child_w(applications_key, app_exe_basename, NULL);

        if (incapable_app == NULL)
            continue;

        extract_executable(app_exe_basename, &appexe, &appexe_basename, &appexe_folded, &appexe_folded_basename);

        shell_open_command_key = g_win32_registry_key_get_child_w(incapable_app, L"shell\\open\\command", NULL);
        ...
        success = g_win32_registry_key_get_value_w(incapable_app, TRUE, L"FriendlyAppName", &vtype, (void **) &friendly_app_name, NULL, NULL);
        ...
        no_open_with = FALSE;
        success = g_win32_registry_key_get_value_w(incapable_app, TRUE, L"NoOpenWith", &vtype, NULL, NULL, NULL);

        if (success)
            no_open_with = TRUE;

        default_icon_key = g_win32_registry_key_get_child_w(incapable_app, L"DefaultIcon", NULL);
        ...
        if (app == NULL)
        {
            ...
            app->command = shell_open_command ? g_wcsdup(shell_open_command, -1) : NULL;
            ...
            if (friendly_app_name)
            {
                ...
                g_clear_pointer(&app->localized_pretty_name_u8, g_free);
                app->localized_pretty_name_u8 = g_utf16_to_utf8(friendly_app_name, -1, NULL, NULL, NULL);
            }
            ...
            g_hash_table_insert(apps_by_exe, g_strdup(appexe_folded_basename), app);
        }

        supported_key = g_win32_registry_key_get_child_w(incapable_app, L"SupportedTypes", NULL);
        ...
        if (supported_key)
        {
            ...
            if (g_win32_registry_value_iter_init(&sup_iter, supported_key, NULL))
            {
                ...
                while (g_win32_registry_value_iter_next(&sup_iter, TRUE, NULL))
                {
                    ...
                    if ((!g_win32_registry_value_iter_get_name_w(&sup_iter, &ext_name, &ext_name_len, NULL)) || (ext_name_len <= 0) || (ext_name[0] != L'.') || (!utf8_and_fold(ext_name, &ext_u8, &ext_folded)))
                        continue;
                    ...
                    g_hash_table_insert(file_extn->other_apps, g_strdup(appexe_folded), g_object_ref(app));
...
static void read_exts(GWin32RegistryKey * file_exts)
{
    ...
    {
        if (!g_win32_registry_subkey_iter_get_name_w(&ext_iter, &file_extension, &file_extension_len, NULL))
            continue;
...
static void read_class_extension(GWin32RegistryKey * classes_root, gunichar2 * class_name, gsize class_name_len)
{
    gchar *ext_u8;
    gchar *ext_folded;
    GWin32AppInfoFileExtension *file_extn;
    ...
    if (!follow_class_chain_to_handler(program_id, program_id_size, &program_command, &program_key, &proxy_id, &proxy_command, &proxy_key, &ext_u8, &ext_folded))
    {
    ...
    if (handler_rec == NULL)
    {
        ...
        extract_executable(proxy_command ? proxy_command : program_command, &handler_rec->executable, &handler_rec->executable_basename, &handler_rec->executable_folded, NULL);
        read_handler_icon(proxy_key, program_key, &handler_rec->icon);
        g_hash_table_insert(handlers, g_strdup(ext_folded), handler_rec);
    }
    else
    {
        g_clear_object(&program_key);
    ...
    handler_rec_in_ext = g_hash_table_lookup(file_extn->handlers, ext_folded);

    if (file_extn->chosen_handler == NULL)
        g_hash_table_insert(file_extn->handlers, g_strdup(ext_folded), g_object_ref(handler_rec));
    else if (handler_rec_in_ext == NULL)
    {
        if (file_extn->chosen_handler->handler_id_folded && strcmp(file_extn->chosen_handler->handler_id_folded, ext_folded) != 0)
            g_hash_table_insert(file_extn->handlers, g_strdup(ext_folded), g_object_ref(handler_rec));
    }
...
static void read_class_url(GWin32RegistryKey * classes_root, gunichar2 * class_name, gsize class_name_len)
{
    GWin32RegistryKey *class_key;
    gboolean success;
    GWin32RegistryValueType vtype;
    ...
    success = g_win32_registry_key_get_value_w(class_key, TRUE, L"URL Protocol", &vtype, NULL, NULL, NULL);
    ...
    if (schema_rec == NULL)
    {
        ...
        schema_rec->chosen_handler = g_object_ref(handler_rec);
        g_hash_table_insert(urls, g_strdup(program_id_folded), schema_rec);
    }
...
static void read_classes(GWin32RegistryKey * classes_root)
{
    ...
    {
        if ((!g_win32_registry_subkey_iter_get_name_w(&class_iter, &class_name, &class_name_len, NULL)) || (class_name_len <= 1))
            continue;
...
static void link_chosen_handlers(void)
{
    GHashTableIter iter;
    ...
    while (g_hash_table_iter_next(&iter, (gpointer *) & schema_folded, (gpointer *) & schema))
    {
        ...
        while (g_hash_table_iter_next(&handler_iter, (gpointer *) & handler_id_folded, (gpointer *) & handler))
        {
            ...
            if (handler->proxy_command && handler->proxy_id && utf8_and_fold(handler->proxy_id, NULL, &proxy_id_folded))
            {
    ...
    while (g_hash_table_iter_next(&iter, (gpointer *) & ext_folded, (gpointer *) & ext))
    {
        if (ext->chosen_handler != NULL)
            continue;
            ...
            if (handler->proxy_command && handler->proxy_id && utf8_and_fold(handler->proxy_id, NULL, &proxy_id_folded))
            {
                ...
                if (proxy)
                {
                    ...
                    g_debug("Linking ext %s to proxy handler %c ? \"%S\" : %S\n",
                            ...
                            ext->chosen_handler->proxy_id ? ext->chosen_handler->proxy_id : ext->chosen_handler->handler_id, ext->chosen_handler->proxy_command ? ext->chosen_handler->proxy_command : ext->chosen_handler->handler_command);
                }
...
static void link_handlers_to_registered_apps(void)
{
    GHashTableIter iter;
    ...
    while (g_hash_table_iter_next(&sup_iter, (gpointer *) & schema_folded, (gpointer *) & schema))
    {
        if (schema->chosen_handler == NULL)
    ...
    while (g_hash_table_iter_next(&sup_iter, (gpointer *) & ext_folded, (gpointer *) & ext))
    {
        if (ext->chosen_handler == NULL)
        {
    ...
    while (g_hash_table_iter_next(&iter, (gpointer *) & app_id_folded, (gpointer *) & app))
    {
        if (app->supported_urls)
        {
            ...
            while (g_hash_table_iter_next(&sup_iter, (gpointer *) & schema_folded, (gpointer *) & handler))
            {
                schema = g_hash_table_lookup(urls, schema_folded);

                g_assert(schema != NULL);

                if (schema->chosen_handler != NULL && schema->chosen_handler->app == NULL)
                {
            ...
            while (g_hash_table_iter_next(&sup_iter, (gpointer *) & schema_folded, (gpointer *) & handler))
            {
                if (handler->app == NULL)
                {
                    ...
                    g_debug("\n directly to schema handler to %c ? \"%S\" : %S\n",
                            handler->proxy_id ? 'P' : 'T', handler->proxy_id ? handler->proxy_id : handler->handler_id, handler->proxy_command ? handler->proxy_command : handler->handler_command);
                }
            }
        }
        ...
        if (app->supported_exts)
        {
            ...
            while (g_hash_table_iter_next(&sup_iter, (gpointer *) & ext_folded, (gpointer *) & handler))
            {
                ext = g_hash_table_lookup(extensions, ext_folded);

                g_assert(ext != NULL);

                if (ext->chosen_handler != NULL && ext->chosen_handler->app == NULL)
                {
...
static void link_handlers_to_unregistered_apps(void)
{
    GHashTableIter iter;
    ...
    while (g_hash_table_iter_next(&iter, (gpointer *) & handler_id_fc, (gpointer *) & handler))
    {
        gchar *hndexe_fc_basename;

        if ((handler->app != NULL) || (handler->executable_folded == NULL))
            continue;
        ...
        while (g_hash_table_iter_next(&app_iter, (gpointer *) & canonical_name_fc, (gpointer *) & app))
        {
            if (app->executable_folded == NULL)
                continue;

            if (strcmp(app->executable_folded, handler->executable_folded) != 0)
                continue;
        ...
        while ((hndexe_fc_basename != NULL) && (g_hash_table_iter_next(&app_iter, (gpointer *) & appexe_fc_basename, (gpointer *) & app)))
        {
...
static void update_registry_data(void)
{
    guint i;
    ...
    url_associations = g_win32_registry_key_new_w(L"HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\Shell\\Associations\\UrlAssociations", NULL);
    file_exts = g_win32_registry_key_new_w(L"HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileExts", NULL);
    ...
    collect_capable_apps_from_clients(capable_apps_keys, priority_capable_apps_keys, FALSE);
    collect_capable_apps_from_clients(user_capable_apps_keys, priority_capable_apps_keys, TRUE);
    collect_capable_apps_from_registered_apps(user_capable_apps_keys, TRUE);
    collect_capable_apps_from_registered_apps(capable_apps_keys, FALSE);
    collect_end = GetTickCount();

    apps_by_id = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);
    apps_by_exe = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);
    urls = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);
    extensions = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);
    handlers = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_object_unref);
    alloc_end = GetTickCount();

    for (i = 0; i < priority_capable_apps_keys->len; i++)
        read_capable_app(g_ptr_array_index(priority_capable_apps_keys, i), TRUE, TRUE);
    for (i = 0; i < user_capable_apps_keys->len; i++)
        read_capable_app(g_ptr_array_index(user_capable_apps_keys, i), TRUE, FALSE);
    for (i = 0; i < capable_apps_keys->len; i++)
        read_capable_app(g_ptr_array_index(capable_apps_keys, i), FALSE, FALSE);
    ...
    g_debug("Collecting capable appnames: %lums\n"
            ...
            "Postprocessing:..............%lums\n"
            "TOTAL:                       %lums\n",
            collect_end - collect_start, alloc_end - collect_end, capable_end - alloc_end, url_end - capable_end, ext_end - url_end, exeapp_end - ext_end, classes_end - exeapp_end, postproc_end - classes_end, postproc_end - collect_start);
...
static void watch_keys(void)
{
    if (url_associations_key)
        g_win32_registry_key_watch(url_associations_key, TRUE, G_WIN32_REGISTRY_WATCH_NAME | G_WIN32_REGISTRY_WATCH_ATTRIBUTES | G_WIN32_REGISTRY_WATCH_VALUES, NULL, NULL, NULL);

    if (file_exts_key)
        g_win32_registry_key_watch(file_exts_key, TRUE, G_WIN32_REGISTRY_WATCH_NAME | G_WIN32_REGISTRY_WATCH_ATTRIBUTES | G_WIN32_REGISTRY_WATCH_VALUES, NULL, NULL, NULL);

    if (user_clients_key)
        g_win32_registry_key_watch(user_clients_key, TRUE, G_WIN32_REGISTRY_WATCH_NAME | G_WIN32_REGISTRY_WATCH_ATTRIBUTES | G_WIN32_REGISTRY_WATCH_VALUES, NULL, NULL, NULL);

    if (system_clients_key)
        g_win32_registry_key_watch(system_clients_key, TRUE, G_WIN32_REGISTRY_WATCH_NAME | G_WIN32_REGISTRY_WATCH_ATTRIBUTES | G_WIN32_REGISTRY_WATCH_VALUES, NULL, NULL, NULL);

    if (applications_key)
        g_win32_registry_key_watch(applications_key, TRUE, G_WIN32_REGISTRY_WATCH_NAME | G_WIN32_REGISTRY_WATCH_ATTRIBUTES | G_WIN32_REGISTRY_WATCH_VALUES, NULL, NULL, NULL);

    if (user_registered_apps_key)
        g_win32_registry_key_watch(user_registered_apps_key, TRUE, G_WIN32_REGISTRY_WATCH_NAME | G_WIN32_REGISTRY_WATCH_ATTRIBUTES | G_WIN32_REGISTRY_WATCH_VALUES, NULL, NULL, NULL);

    if (system_registered_apps_key)
        g_win32_registry_key_watch(system_registered_apps_key, TRUE, G_WIN32_REGISTRY_WATCH_NAME | G_WIN32_REGISTRY_WATCH_ATTRIBUTES | G_WIN32_REGISTRY_WATCH_VALUES, NULL, NULL, NULL);

    if (classes_root_key)
        g_win32_registry_key_watch(classes_root_key, FALSE, G_WIN32_REGISTRY_WATCH_NAME | G_WIN32_REGISTRY_WATCH_ATTRIBUTES | G_WIN32_REGISTRY_WATCH_VALUES, NULL, NULL, NULL);
}

static void g_win32_appinfo_init(void)
{
    static gsize initialized;

    if (g_once_init_enter(&initialized))
    {
        url_associations_key = g_win32_registry_key_new_w(L"HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\Shell\\Associations\\UrlAssociations", NULL);
        file_exts_key = g_win32_registry_key_new_w(L"HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\FileExts", NULL);
        user_clients_key = g_win32_registry_key_new_w(L"HKEY_CURRENT_USER\\Software\\Clients", NULL);
        system_clients_key = g_win32_registry_key_new_w(L"HKEY_LOCAL_MACHINE\\Software\\Clients", NULL);
        applications_key = g_win32_registry_key_new_w(L"HKEY_CLASSES_ROOT\\Applications", NULL);
        user_registered_apps_key = g_win32_registry_key_new_w(L"HKEY_CURRENT_USER\\Software\\RegisteredApplications", NULL);
        system_registered_apps_key = g_win32_registry_key_new_w(L"HKEY_LOCAL_MACHINE\\Software\\RegisteredApplications", NULL);
        classes_root_key = g_win32_registry_key_new_w(L"HKEY_CLASSES_ROOT", NULL);
    ...
    if ((url_associations_key && g_win32_registry_key_has_changed(url_associations_key)) ||
        ...
        (system_registered_apps_key && g_win32_registry_key_has_changed(system_registered_apps_key)) || (classes_root_key && g_win32_registry_key_has_changed(classes_root_key)))
    {
...
static GAppInfo *g_win32_app_info_new_from_app(GWin32AppInfoApplication * app, GWin32AppInfoHandler * handler)
{
...
static gboolean g_win32_app_info_equal(GAppInfo * appinfo1, GAppInfo * appinfo2)
{
    ...
    if (info1->app == NULL || info2->app == NULL)
        return info1 == info2;

    if (info1->app->canonical_name_folded != NULL && info2->app->canonical_name_folded != NULL)
        return (strcmp(info1->app->canonical_name_folded, info2->app->canonical_name_folded)) == 0;

    if (info1->app->executable_folded != NULL && info2->app->executable_folded != NULL)
        return (strcmp(info1->app->executable_folded, info2->app->executable_folded)) == 0;
...
static const char *g_win32_app_info_get_display_name(GAppInfo * appinfo)
{
    GWin32AppInfo *info = G_WIN32_APP_INFO(appinfo);
...
static const char *g_win32_app_info_get_commandline(GAppInfo * appinfo)
{
    GWin32AppInfo *info = G_WIN32_APP_INFO(appinfo);
...
static char *expand_macro_single(char macro, file_or_uri * obj)
{
    char *result = NULL;
...
static gboolean expand_macro(char macro, GString * exec, GWin32AppInfo * info, GList ** stat_obj_list, GList ** obj_list)
{
    GList *objs = *obj_list;
    char *expanded;
    ...
    switch (macro)
    {
        ...
        case '~':
            if (*stat_obj_list)
            {
                ...
                for (o = *stat_obj_list, i = 0; macro == '~' && o && i < 2; o = o->next, i++) ;
...
static gboolean expand_application_parameters(GWin32AppInfo * info, const gchar * exec_line, GList ** objs, int *argc, char ***argv, GError ** error)
{
    GList *obj_list = *objs;
    ...
    if (exec_line == NULL)
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, P_("Application registry did not specify" " a shell\\open\\command"));
        return FALSE;
    }
    ...
    while (*p)
    {
        if (p[0] == '%' && p[1] != '\0')
        {
            if (expand_macro(p[1], expanded_exec, info, stat_obj_list, objs))
                res = TRUE;
     ...
     */
    for (a_char = expanded_exec->str; a_char <= &expanded_exec->str[expanded_exec->len]; a_char++)
    {
        if (*a_char == '\\')
            *a_char = '/';
    }
...
static gchar *get_appath_for_exe(gunichar2 * exe_basename)
{
    ...
    apppath_key = _g_win32_registry_key_build_and_new_w(NULL, L"HKEY_LOCAL_MACHINE\\" L"\\SOFTWARE" L"\\Microsoft" L"\\Windows" L"\\CurrentVersion" L"\\App Paths\\", exe_basename, NULL);

    if (apppath_key == NULL)
        return NULL;

    got_value = g_win32_registry_key_get_value_w(apppath_key, TRUE, L"Path", &val_type, (void **) &appath, NULL, NULL);
...
static gboolean g_win32_app_info_launch_internal(GWin32AppInfo * info, GList * objs, GAppLaunchContext * launch_context, GSpawnFlags spawn_flags, GError ** error)
{
    ...
    if (info->handler)
    {
        ...
        {
            command = g_utf16_to_utf8(info->handler->handler_command, -1, NULL, NULL, NULL);
            exe_basename = g_utf8_to_utf16(info->handler->executable_basename, -1, NULL, NULL, NULL);
        }
        else if (info->handler->proxy_command)
        {
            command = g_utf16_to_utf8(info->handler->proxy_command, -1, NULL, NULL, NULL);
            exe_basename = g_utf8_to_utf16(info->handler->executable_basename, -1, NULL, NULL, NULL);
        }
    }
    ...
    if (command == NULL)
    {
        ...
        exe_basename = g_utf8_to_utf16(info->app->executable_basename, -1, NULL, NULL, NULL);
    }
    ...
    if (apppath)
    {
        ...
        for (p = envp, p_index = 0; p[0]; p++, p_index++)
            if ((p[0][0] == 'p' || p[0][0] == 'P') && (p[0][1] == 'a' || p[0][1] == 'A') && (p[0][2] == 't' || p[0][2] == 'T') && (p[0][3] == 'h' || p[0][3] == 'H') && (p[0][4] == '='))
                break;
        ...
        else
        {
            ...
            if (p_path[0] != '\0')
                envp[p_index] = g_strdup_printf("PATH=%s%c%s", apppath, G_SEARCHPATH_SEPARATOR, p_path);
            else
    ...
    do
    {
        ...
        if (!expand_application_parameters(info, command, &objs, &argc, &argv, error))
            goto out;

        if (!g_spawn_async(NULL, argv, envp, spawn_flags, NULL, NULL, &pid, error))
            goto out;
...
static void free_file_or_uri(gpointer ptr)
{
    file_or_uri *obj = ptr;
    g_free(obj->file);
    g_free(obj->uri);
...
static gboolean g_win32_app_supports_uris(GWin32AppInfoApplication * app)
{
...
static gboolean g_win32_app_info_launch_uris(GAppInfo * appinfo, GList * uris, GAppLaunchContext * launch_context, GError ** error)
{
    ...
    res = g_win32_app_info_launch_internal(G_WIN32_APP_INFO(appinfo), objs, launch_context, G_SPAWN_SEARCH_PATH, error);
...
static const char **g_win32_app_info_get_supported_types(GAppInfo * appinfo)
{
    GWin32AppInfo *info = G_WIN32_APP_INFO(appinfo);
...
GAppInfo *g_app_info_create_from_commandline(const char *commandline, const char *application_name, GAppInfoCreateFlags flags, GError ** error)
{
    ...
    if (application_name)
    {
        app->canonical_name = g_utf8_to_utf16(application_name, -1, NULL, NULL, NULL);
    ...
    extract_executable(app->command, &app->executable, &app->executable_basename, &app->executable_folded, NULL);
...
GAppInfo *g_app_info_get_default_for_uri_scheme(const char *uri_scheme)
{
    ...
    if (scheme != NULL && scheme->chosen_handler != NULL && scheme->chosen_handler->app != NULL)
        result = g_win32_app_info_new_from_app(scheme->chosen_handler->app, scheme->chosen_handler);
...
GAppInfo *g_app_info_get_default_for_type(const char *content_type, gboolean must_support_uris)
{
    ...
    if (ext != NULL)
    {
        if (ext->chosen_handler != NULL && ext->chosen_handler->app != NULL && (!must_support_uris || g_win32_app_supports_uris(ext->chosen_handler->app)))
            result = g_win32_app_info_new_from_app(ext->chosen_handler->app, ext->chosen_handler);
        else
        {
            ...
            {
                if (handler->app && (!must_support_uris || g_win32_app_supports_uris(ext->chosen_handler->app)))
                {
            ...
            if (result == NULL)
            {
                ...
                {
                    if (!must_support_uris || g_win32_app_supports_uris(ext->chosen_handler->app))
                    {
...
GList *g_app_info_get_all(void)
{
    ...
    for (apps_i = apps; apps_i; apps_i = apps_i->next)
        infos = g_list_prepend(infos, g_win32_app_info_new_from_app(apps_i->data, NULL));
...
GList *g_app_info_get_all_for_type(const char *content_type)
{
    ...
    if (ext->chosen_handler != NULL && ext->chosen_handler->app != NULL)
        result = g_list_prepend(result, g_win32_app_info_new_from_app(ext->chosen_handler->app, ext->chosen_handler));

    g_hash_table_iter_init(&iter, ext->handlers);

    while (g_hash_table_iter_next(&iter, NULL, (gpointer *) & handler))
    {
        if (handler->app && (ext->chosen_handler == NULL || ext->chosen_handler->app != app))
            result = g_list_prepend(result, g_win32_app_info_new_from_app(handler->app, handler));
    }
