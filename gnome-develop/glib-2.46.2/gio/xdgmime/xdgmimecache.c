...
static int cache_magic_matchlet_compare_to_data(XdgMimeCache * cache, xdg_uint32_t offset, const void *data, size_t len)
{
    ...
    for (i = range_start; i < range_start + range_length; i++)
    {
        ...
        if (mask_offset)
        {
            ...
            {
                if ((((unsigned char *) cache->buffer)[data_offset + j] & ((unsigned char *) cache->buffer)[mask_offset + j]) != ((((unsigned char *) data)[j + i]) & ((unsigned char *) cache->buffer)[mask_offset + j]))
                {
    ...
    *prio = 0;
    ...
    for (j = 0; j < n_entries; j++)
    {
        ...
        else
        {
            ...
            {
                if (mime_types[n] && _xdg_mime_mime_type_equal(mime_types[n], non_match))
                    mime_types[n] = NULL;
            }
        }
    }
...
static int cache_glob_node_lookup_suffix(XdgMimeCache * cache, xdg_uint32_t n_entries, xdg_uint32_t offset, const char *file_name, int len, int case_sensitive_check, MimeWeight mime_types[], int n_mime_types)
{
    ...
    while (max >= min)
    {
        ...
        else
        {
            ...
            if (len > 0)
            {
                n = cache_glob_node_lookup_suffix(cache, n_children, child_offset, file_name, len, case_sensitive_check, mime_types, n_mime_types);
            }
            if (n == 0)
            {
                i = 0;
