...
static void g_file_input_stream_real_query_info_async(GFileInputStream * stream, const char *attributes, int io_priority, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data);
...
G_DEFINE_TYPE_WITH_CODE(GFileInputStream, g_file_input_stream, G_TYPE_INPUT_STREAM, G_ADD_PRIVATE(GFileInputStream) G_IMPLEMENT_INTERFACE(G_TYPE_SEEKABLE, g_file_input_stream_seekable_iface_init))
...
void g_file_input_stream_query_info_async(GFileInputStream * stream, const char *attributes, int io_priority, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    if (!g_input_stream_set_pending(input_stream, &error))
    {
        g_task_report_error(stream, callback, user_data, g_file_input_stream_query_info_async, error);
        return;
    }
