...
G_DEFINE_TYPE_WITH_CODE(GListStore, g_list_store, G_TYPE_OBJECT, G_IMPLEMENT_INTERFACE(G_TYPE_LIST_MODEL, g_list_store_iface_init));

static void g_list_store_items_changed(GListStore * store, guint position, guint removed, guint added)
{
...
static void g_list_store_get_property(GObject * object, guint property_id, GValue * value, GParamSpec * pspec)
{
    GListStore *store = G_LIST_STORE(object);
...
static void g_list_store_set_property(GObject * object, guint property_id, const GValue * value, GParamSpec * pspec)
{
    GListStore *store = G_LIST_STORE(object);
    ...
    switch (property_id)
    {
        case PROP_ITEM_TYPE:    /* construct-only */
            ...
            if (!g_type_is_a(store->item_type, G_TYPE_OBJECT))
                g_critical("GListStore cannot store items of type '%s'. Items must be GObjects.", g_type_name(store->item_type));
            break;
...
static void g_list_store_class_init(GListStoreClass * klass)
{
   ...
   * Since: 2.44
   **/
    g_object_class_install_property(object_class, PROP_ITEM_TYPE, g_param_spec_gtype("item-type", "", "", G_TYPE_OBJECT, G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}
...
void g_list_store_remove_all(GListStore * store)
{
    ...
    n_items = g_sequence_get_length(store->items);
    g_sequence_remove_range(g_sequence_get_begin_iter(store->items), g_sequence_get_end_iter(store->items));
...
void g_list_store_splice(GListStore * store, guint position, guint n_removals, gpointer * additions, guint n_additions)
{
    ...
    if (n_additions)
    {
        ...
        for (i = 0; i < n_additions; i++)
        {
            ...
            {
                g_critical("%s: item %d is a %s instead of a %s.  GListStore is now in an undefined state.", G_STRFUNC, i, G_OBJECT_TYPE_NAME(additions[i]), g_type_name(store->item_type));
                return;
            }
