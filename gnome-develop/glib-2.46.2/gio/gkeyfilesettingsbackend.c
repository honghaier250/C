...
static gboolean convert_path(GKeyfileSettingsBackend * kfsb, const gchar * key, gchar ** group, gchar ** basename)
{
    ...
    if (key_len < kfsb->prefix_len || memcmp(key, kfsb->prefix, kfsb->prefix_len) != 0)
        return FALSE;
...
static void keyfile_to_tree(GKeyfileSettingsBackend * kfsb, GTree * tree, GKeyFile * keyfile, gboolean dup_check)
{
    ...
    for (i = 0; groups[i]; i++)
    {
        ...
        if (!is_root_group && (g_str_has_prefix(groups[i], "/") || g_str_has_suffix(groups[i], "/") || strstr(groups[i], "//")))
            continue;
...
static void g_keyfile_settings_backend_keyfile_reload(GKeyfileSettingsBackend * kfsb)
{
    ...
    if (memcmp(kfsb->digest, digest, sizeof digest) != 0)
    {
        ...
        if (length > 0)
            g_key_file_load_from_data(keyfiles[1], contents, length, G_KEY_FILE_KEEP_COMMENTS | G_KEY_FILE_KEEP_TRANSLATIONS, NULL);
...
static void g_keyfile_settings_backend_keyfile_writable(GKeyfileSettingsBackend * kfsb)
{
    ...
    if (fileinfo)
    {
        writable = g_file_info_get_attribute_boolean(fileinfo, G_FILE_ATTRIBUTE_ACCESS_CAN_WRITE) && g_file_info_get_attribute_boolean(fileinfo, G_FILE_ATTRIBUTE_ACCESS_CAN_EXECUTE);
