...
G_DEFINE_TYPE_WITH_CODE(GNetworkMonitorBase, g_network_monitor_base, G_TYPE_OBJECT,
                        ...
                        G_IMPLEMENT_INTERFACE(G_TYPE_NETWORK_MONITOR,
                                              g_network_monitor_base_iface_init) _g_io_modules_ensure_extension_points_registered(); g_io_extension_point_implement(G_NETWORK_MONITOR_EXTENSION_POINT_NAME, g_define_type_id, "base", 0))
...
static void g_network_monitor_base_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
    GNetworkMonitorBase *monitor = G_NETWORK_MONITOR_BASE(object);
    ...
    switch (prop_id)
    {
        ...
        case PROP_CONNECTIVITY:
            g_value_set_enum(value, monitor->priv->is_available ? G_NETWORK_CONNECTIVITY_FULL : G_NETWORK_CONNECTIVITY_LOCAL);
            break;
...
static void g_network_monitor_base_class_init(GNetworkMonitorBaseClass * monitor_class)
{
...
static gboolean g_network_monitor_base_can_reach_sockaddr(GNetworkMonitorBase * base, GSocketAddress * sockaddr)
{
    GInetAddress *iaddr;
    int i;
...
static gboolean g_network_monitor_base_can_reach(GNetworkMonitor * monitor, GSocketConnectable * connectable, GCancellable * cancellable, GError ** error)
{
    ...
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_NETWORK_UNREACHABLE, _("Network unreachable"));
        return FALSE;
    }
    ...
    if (base->priv->have_ipv4_default_route && base->priv->have_ipv6_default_route)
    {
        g_object_unref(enumerator);
        g_object_unref(addr);
        return TRUE;
    ...
    if (error && !*error)
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_HOST_UNREACHABLE, _("Host unreachable"));
    }
    return FALSE;
}

static void can_reach_async_got_address(GObject * object, GAsyncResult * result, gpointer user_data)
{
    ...
    if (!addr)
    {
        ...
        else
        {
            ...
            g_task_return_new_error(task, G_IO_ERROR, G_IO_ERROR_HOST_UNREACHABLE, _("Host unreachable"));
            g_object_unref(task);
            return;
        }
    }
    ...
    g_socket_address_enumerator_next_async(enumerator, g_task_get_cancellable(task), can_reach_async_got_address, task);
}

static void g_network_monitor_base_can_reach_async(GNetworkMonitor * monitor, GSocketConnectable * connectable, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    GTask *task;
    GSocketAddressEnumerator *enumerator;
    ...
    {
        g_task_return_new_error(task, G_IO_ERROR, G_IO_ERROR_NETWORK_UNREACHABLE, _("Network unreachable"));
        g_object_unref(task);
        return;
    }

    enumerator = g_socket_connectable_proxy_enumerate(connectable);
    g_socket_address_enumerator_next_async(enumerator, cancellable, can_reach_async_got_address, task);
    g_object_unref(enumerator);
}

static gboolean g_network_monitor_base_can_reach_finish(GNetworkMonitor * monitor, GAsyncResult * result, GError ** error)
{
...
static void g_network_monitor_base_iface_init(GNetworkMonitorInterface * monitor_iface)
{
...
static gboolean g_network_monitor_base_initable_init(GInitable * initable, GCancellable * cancellable, GError ** error)
{
    return TRUE;
}
...
static gboolean emit_network_changed(gpointer user_data)
{
    ...
    {
        is_available = (monitor->priv->have_ipv4_default_route || monitor->priv->have_ipv6_default_route);
...
static void queue_network_changed(GNetworkMonitorBase * monitor)
{
    ...
    if (monitor->priv->initializing)
    {
        monitor->priv->is_available = (monitor->priv->have_ipv4_default_route || monitor->priv->have_ipv6_default_route);
    }
}
 ...
 * Since: 2.32
 */
void g_network_monitor_base_add_network(GNetworkMonitorBase * monitor, GInetAddressMask * network)
{
    int i;
 ...
 * Since: 2.32
 */
void g_network_monitor_base_remove_network(GNetworkMonitorBase * monitor, GInetAddressMask * network)
{
    int i;
 ...
 * it with @networks.
 */
void g_network_monitor_base_set_networks(GNetworkMonitorBase * monitor, GInetAddressMask ** networks, gint length)
{
    int i;
