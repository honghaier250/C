...
static gint64 pending_change_get_ready_time(const PendingChange * change, GFileMonitorSource * fms)
{
    if (change->dirty)
...
static int pending_change_compare_ready_time(gconstpointer a_p, gconstpointer b_p, gpointer user_data)
{
...
static GSequenceIter *g_file_monitor_source_find_pending_change(GFileMonitorSource * fms, const gchar * child)
{
    return g_hash_table_lookup(fms->pending_changes_table, child);
}

static void g_file_monitor_source_add_pending_change(GFileMonitorSource * fms, const gchar * child, gint64 now)
{
...
static gboolean g_file_monitor_source_set_pending_change_dirty(GFileMonitorSource * fms, GSequenceIter * iter)
{
    PendingChange *change;
...
static gboolean g_file_monitor_source_get_pending_change_dirty(GFileMonitorSource * fms, GSequenceIter * iter)
{
    PendingChange *change;
...
static void g_file_monitor_source_remove_pending_change(GFileMonitorSource * fms, GSequenceIter * iter, const gchar * child)
{
...
static void g_file_monitor_source_queue_event(GFileMonitorSource * fms, GFileMonitorEvent event_type, const gchar * child, GFile * other)
{
    QueuedEvent *event;
...
static gboolean g_file_monitor_source_file_changed(GFileMonitorSource * fms, const gchar * child, gint64 now)
{
...
static void g_file_monitor_source_file_changes_done(GFileMonitorSource * fms, const gchar * child)
{
    GSequenceIter *pending;
...
static void g_file_monitor_source_file_created(GFileMonitorSource * fms, const gchar * child, gint64 event_time)
{
...
static void g_file_monitor_source_send_event(GFileMonitorSource * fms, GFileMonitorEvent event_type, const gchar * child, GFile * other)
{
...
static void g_file_monitor_source_send_synthetic_created(GFileMonitorSource * fms, const gchar * child)
{
    g_file_monitor_source_file_changes_done(fms, child);
    g_file_monitor_source_queue_event(fms, G_FILE_MONITOR_EVENT_CREATED, child, NULL);
    g_file_monitor_source_queue_event(fms, G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT, child, NULL);
...
gboolean g_file_monitor_source_handle_event(GFileMonitorSource * fms, GFileMonitorEvent event_type, const gchar * child, const gchar * rename_to, GFile * other, gint64 event_time)
{
    ...
    if (fms->basename && (!child || !g_str_equal(child, fms->basename)) && (!rename_to || !g_str_equal(rename_to, fms->basename)))
        return TRUE;
...
static gboolean g_file_monitor_source_set_rate_limit(GFileMonitorSource * fms, gint64 rate_limit)
{
    gboolean changed;
...
static gboolean g_file_monitor_source_dispatch(GSource * source, GSourceFunc callback, gpointer user_data)
{
...
static GFileMonitorSource *g_file_monitor_source_new(gpointer instance, const gchar * filename, gboolean is_directory, GFileMonitorFlags flags)
{
...
G_DEFINE_ABSTRACT_TYPE(GLocalFileMonitor, g_local_file_monitor, G_TYPE_FILE_MONITOR)
     ...
     static void g_local_file_monitor_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec)
{
...
static void g_local_file_monitor_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec)
{
    GLocalFileMonitor *monitor = G_LOCAL_FILE_MONITOR(object);
    gint64 rate_limit;
...
#ifndef G_OS_WIN32
static void g_local_file_monitor_mounts_changed(GUnixMountMonitor * mount_monitor, gpointer user_data)
{
...
static void g_local_file_monitor_start(GLocalFileMonitor * local_monitor, const gchar * filename, gboolean is_directory, GFileMonitorFlags flags, GMainContext * context)
{
    ...
    if (is_directory && !class->mount_notify && (flags & G_FILE_MONITOR_WATCH_MOUNTS))
    {
        ...
        g_signal_connect_object(local_monitor->mount_monitor, "mounts-changed", G_CALLBACK(g_local_file_monitor_mounts_changed), local_monitor, 0);
#endif
    }
    ...
    G_LOCAL_FILE_MONITOR_GET_CLASS(local_monitor)->start(local_monitor, source->dirname, source->basename, source->filename, source);
...
static GLocalFileMonitor *g_local_file_monitor_new(gboolean is_remote_fs, GError ** error)
{
    GType type = G_TYPE_INVALID;

    if (is_remote_fs)
        type = _g_io_module_get_default_type(G_NFS_FILE_MONITOR_EXTENSION_POINT_NAME, "GIO_USE_FILE_MONITOR", G_STRUCT_OFFSET(GLocalFileMonitorClass, is_supported));

    if (type == G_TYPE_INVALID)
        type = _g_io_module_get_default_type(G_LOCAL_FILE_MONITOR_EXTENSION_POINT_NAME, "GIO_USE_FILE_MONITOR", G_STRUCT_OFFSET(GLocalFileMonitorClass, is_supported));

    if (type == G_TYPE_INVALID)
    {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Unable to find default local file monitor type"));
        return NULL;
    }
...
GFileMonitor *g_local_file_monitor_new_for_path(const gchar * pathname, gboolean is_directory, GFileMonitorFlags flags, GError ** error)
{
...
GFileMonitor *g_local_file_monitor_new_in_worker(const gchar * pathname, gboolean is_directory, GFileMonitorFlags flags, GFileMonitorCallback callback, gpointer user_data, GError ** error)
{
    GLocalFileMonitor *monitor;
    gboolean is_remote_fs;
