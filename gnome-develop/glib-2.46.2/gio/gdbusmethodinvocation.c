...
GDBusMethodInvocation *_g_dbus_method_invocation_new(const gchar * sender,
                                                     ...
                                                     const GDBusMethodInfo * method_info, const GDBusPropertyInfo * property_info, GDBusConnection * connection, GDBusMessage * message, GVariant * parameters, gpointer user_data)
{
...
static void g_dbus_method_invocation_return_value_internal(GDBusMethodInvocation * invocation, GVariant * parameters, GUnixFDList * fd_list)
{
    GDBusMessage *reply;
    GError *error;
    ...
    if (invocation->method_info != NULL)
    {
        ...
        if (!g_variant_is_of_type(parameters, type))
        {
            ...
            g_warning("Type of return value is incorrect: expected '%s', got '%s''", type_string, g_variant_get_type_string(parameters));
    ...
    if (invocation->property_info != NULL)
    {
        if (g_str_equal(invocation->method_name, "Get"))
        {
            ...
            {
                g_warning("Type of return value for property 'Get' call should be '(v)' but got '%s'", g_variant_get_type_string(parameters));
                goto out;
            }
            ...
            {
                g_warning("Value returned from property 'Get' call for '%s' should be '%s' but is '%s'", invocation->property_info->name, invocation->property_info->signature, g_variant_get_type_string(nested));
        ...
        else if (g_str_equal(invocation->method_name, "GetAll"))
        {
            ...
            {
                g_warning("Type of return value for property 'GetAll' call should be '(a{sv})' but got '%s'", g_variant_get_type_string(parameters));
                goto out;
            }
        ...
        else if (g_str_equal(invocation->method_name, "Set"))
        {
            ...
            {
                g_warning("Type of return value for property 'Set' call should be '()' but got '%s'", g_variant_get_type_string(parameters));
                goto out;
            }
        }
...
void g_dbus_method_invocation_return_dbus_error(GDBusMethodInvocation * invocation, const gchar * error_name, const gchar * error_message)
{
    ...
    if (G_UNLIKELY(_g_dbus_debug_return()))
    {
        ...
        g_print("========================================================================\n"
                ...
                "      in response to %s.%s()\n"
                "      on object %s\n"
                "      to name %s\n" "      reply-serial %d\n", error_name, error_message, invocation->interface_name, invocation->method_name, invocation->object_path, invocation->sender, g_dbus_message_get_serial(invocation->message));
        _g_dbus_debug_print_unlock();
    }
