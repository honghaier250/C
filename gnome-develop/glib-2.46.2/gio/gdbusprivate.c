...
static gboolean _g_socket_read_with_control_messages_ready(GSocket * socket, GIOCondition condition, gpointer user_data)
{
    ...
    result = g_socket_receive_message(socket, NULL,    /* address */
                                      &vector, 1, data->messages, data->num_messages, NULL, g_task_get_cancellable(task), &error);
...
static void _g_socket_read_with_control_messages(GSocket * socket, void *buffer, gsize count, GSocketControlMessage *** messages, gint * num_messages, gint io_priority, GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    GTask *task;
    ...
    source = g_socket_create_source(socket, G_IO_IN | G_IO_HUP | G_IO_ERR, cancellable);
...
static void _g_dbus_worker_do_read_cb(GInputStream * input_stream, GAsyncResult * res, gpointer user_data)
{
    ...
    if (worker->socket == NULL)
        bytes_read = g_input_stream_read_finish(g_io_stream_get_input_stream(worker->stream), res, &error);
    else
        bytes_read = _g_socket_read_with_control_messages_finish(worker->socket, res, &error);
    if (worker->read_num_ancillary_messages > 0)
    {
        ...
        for (n = 0; n < worker->read_num_ancillary_messages; n++)
        {
            ...
            else
            {
                if (error == NULL)
                {
                    g_set_error(&error, G_IO_ERROR, G_IO_ERROR_FAILED, "Unexpected ancillary message of type %s received from peer", g_type_name(G_TYPE_FROM_INSTANCE(control_message)));
    ...
    if (bytes_read == -1)
    {
        if (G_UNLIKELY(_g_dbus_debug_transport()))
        {
            ...
            g_print("========================================================================\n"
                    ...
                    "  ---- READ ERROR on stream of type %s:\n" "  ---- %s %d: %s\n", g_type_name(G_TYPE_FROM_INSTANCE(g_io_stream_get_input_stream(worker->stream))), g_quark_to_string(error->domain), error->code, error->message);
         ...
         */
        if (worker->close_expected || g_error_matches(error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
    ...
    g_debug("read %d bytes (is_closed=%d blocking=%d condition=0x%02x) stream %p, %p",
            ...
            g_socket_condition_check(g_socket_connection_get_socket(G_SOCKET_CONNECTION(worker->stream)), G_IO_IN | G_IO_OUT | G_IO_HUP), worker->stream, worker);
#endif
...
static void _g_dbus_worker_do_read_unlocked(GDBusWorker * worker)
{
    ...
    if (worker->socket == NULL)
        g_input_stream_read_async(g_io_stream_get_input_stream(worker->stream),
                                  ...
                                  worker->read_buffer_bytes_wanted - worker->read_buffer_cur_size, G_PRIORITY_DEFAULT, worker->cancellable, (GAsyncReadyCallback) _g_dbus_worker_do_read_cb, _g_dbus_worker_ref(worker));
    else
    {
        ...
        _g_socket_read_with_control_messages(worker->socket,
                                             ...
                                             &worker->read_ancillary_messages, &worker->read_num_ancillary_messages, G_PRIORITY_DEFAULT, worker->cancellable, (GAsyncReadyCallback) _g_dbus_worker_do_read_cb, _g_dbus_worker_ref(worker));
    }
}
...
static void write_message_continue_writing(MessageToWriteData * data)
{
    ...
    else if (G_IS_SOCKET_OUTPUT_STREAM(ostream) && data->total_written == 0)
    {
        ...
        if (fd_list != NULL && g_unix_fd_list_get_length(fd_list) > 0)
        {
            ...
            {
                g_task_return_new_error(task, G_IO_ERROR, G_IO_ERROR_FAILED, "Tried sending a file descriptor but remote peer does not support this capability");
                g_object_unref(task);
                goto out;
            }
        ...
        error = NULL;
        bytes_written = g_socket_send_message(data->worker->socket, NULL,    /* address */
                                              &vector, 1, control_message != NULL ? &control_message : NULL, control_message != NULL ? 1 : 0, G_SOCKET_MSG_NONE, data->worker->cancellable, &error);
        ...
        if (bytes_written == -1)
        {
            ...
            if (g_error_matches(error, G_IO_ERROR, G_IO_ERROR_WOULD_BLOCK))
            {
                ...
                g_source_set_callback(source, (GSourceFunc) on_socket_ready, data, NULL);    /* GDestroyNotify */
    ...
    else
    {
        ...
        if (fd_list != NULL)
        {
            g_task_return_new_error(task, G_IO_ERROR, G_IO_ERROR_FAILED, "Tried sending a file descriptor on unsupported stream of type %s", g_type_name(G_TYPE_FROM_INSTANCE(ostream)));
            g_object_unref(task);
            goto out;
        }
#endif

        g_output_stream_write_async(ostream, (const gchar *) data->blob + data->total_written, data->blob_size - data->total_written, G_PRIORITY_DEFAULT, data->worker->cancellable, write_message_async_cb, data);
    }
#ifdef G_OS_UNIX
out:
#endif
...
{
    g_output_stream_flush_async(g_io_stream_get_output_stream(data->worker->stream), G_PRIORITY_DEFAULT, data->worker->cancellable, ostream_flush_cb, data);
}
 ...
 */
static void message_written_unlocked(GDBusWorker * worker, MessageToWriteData * message_data)
{
    if (G_UNLIKELY(_g_dbus_debug_message()))
    {
        gchar *s;
 ...
 */
static void iostream_close_cb(GObject * source_object, GAsyncResult * res, gpointer user_data)
{
    GDBusWorker *worker = user_data;
    GError *error = NULL;
...
write_next:
    ...
    if (worker->pending_close_attempts != NULL)
    {
        ...
        if (!g_input_stream_has_pending(input))
        {
            ...
            g_io_stream_close_async(worker->stream, G_PRIORITY_DEFAULT, NULL, iostream_close_cb, _g_dbus_worker_ref(worker));
        }
    }
    else
    {
    ...
    else if (data != NULL)
    {
        ...
        else
        {
            ...
            if (new_blob == NULL)
            {
                 ...
                 */
                g_warning("Error encoding GDBusMessage with serial %d altered by filter function: %s", g_dbus_message_get_serial(data->message), error->message);
                g_error_free(error);
            }
            else
            {
 ...
 * output_pending may be anything
 */
static void schedule_writing_unlocked(GDBusWorker * worker, MessageToWriteData * write_data, FlushData * flush_data, CloseData * close_data)
{
    if (write_data != NULL)
    ...
    if (close_data != NULL)
        worker->pending_close_attempts = g_list_prepend(worker->pending_close_attempts, close_data);
    ...
    if (worker->output_pending == PENDING_NONE)
    {
        ...
        idle_source = g_idle_source_new();
        g_source_set_priority(idle_source, G_PRIORITY_DEFAULT);
        g_source_set_callback(idle_source, continue_writing_in_idle_cb, _g_dbus_worker_ref(worker), (GDestroyNotify) _g_dbus_worker_unref);
...
GDBusWorker *_g_dbus_worker_new(GIOStream * stream,
                                ...
                                GDBusWorkerMessageReceivedCallback message_received_callback, GDBusWorkerMessageAboutToBeSentCallback message_about_to_be_sent_callback, GDBusWorkerDisconnectedCallback disconnected_callback, gpointer user_data)
{
    GDBusWorker *worker;
    GSource *idle_source;
    ...
    idle_source = g_idle_source_new();
    g_source_set_priority(idle_source, G_PRIORITY_DEFAULT);
    g_source_set_callback(idle_source, _g_dbus_worker_do_initial_read, _g_dbus_worker_ref(worker), (GDestroyNotify) _g_dbus_worker_unref);
...
gboolean _g_dbus_worker_flush_sync(GDBusWorker * worker, GCancellable * cancellable, GError ** error)
{
    ...
    if (pending_writes > 0 || worker->write_num_messages_written != worker->write_num_messages_flushed)
    {
...
gchar *_g_dbus_get_machine_id(GError ** error)
{
    ...
    if (!GetCurrentHwProfileA(&info))
    {
        ...
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_FAILED, _("Unable to get Hardware profile: %s"), message);
        g_free(message);
        return NULL;
    }
    ...
    *dest = 0;
    ...
    ret = NULL;
    first_error = NULL;
    if (!g_file_get_contents("/var/lib/dbus/machine-id", &ret, NULL, &first_error) && !g_file_get_contents("/etc/machine-id", &ret, NULL, NULL))
    {
        g_propagate_prefixed_error(error, first_error, _("Unable to load /var/lib/dbus/machine-id or /etc/machine-id: "));
    }
    else
    {
...
static void write_message_print_transport_debug(gssize bytes_written, MessageToWriteData * data)
{
    ...
    g_print("========================================================================\n"
            ...
            bytes_written, g_dbus_message_get_serial(data->message), data->blob_size, data->total_written, g_type_name(G_TYPE_FROM_INSTANCE(g_io_stream_get_output_stream(data->worker->stream))));
    _g_dbus_debug_print_unlock();
out:
    ;
}
...
static void read_message_print_transport_debug(gssize bytes_read, GDBusWorker * worker)
{
    ...
    g_print("========================================================================\n"
            ...
            "       size %d to offset %" G_GSIZE_FORMAT " from a %s\n", bytes_read, serial, message_length, worker->read_buffer_cur_size, g_type_name(G_TYPE_FROM_INSTANCE(g_io_stream_get_input_stream(worker->stream))));
    _g_dbus_debug_print_unlock();
out:
    ;
}
