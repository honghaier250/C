...
GModule *g_module_open(const gchar * file_name, GModuleFlags flags)
{
    ...
    if (name)
    {
        ...
        if (name)
            handle = _g_module_open(name, (flags & G_MODULE_BIND_LAZY) != 0, (flags & G_MODULE_BIND_LOCAL) != 0);
    }
    else
    {
    ...
    if (handle)
    {
        ...
        if (check_failed)
        {
            ...
            error = g_strconcat("GModule (", file_name, ") ", "initialization check failed: ", check_failed, NULL);
    ...
    if (module != NULL && (module_debug_flags & G_MODULE_DEBUG_RESIDENT_MODULES))
