...
static void test_small_writes(void)
{
    GIOChannel *io;
    ...
    if (error)
    {
        g_warning("Unable to open file %s: %s", "iochannel-test-outfile", error->message);
        g_clear_error(&error);
...
gint main(gint argc, gchar * argv[])
{
    ...
    while (TRUE)
    {
        ...
        do
            status = g_io_channel_write_chars(gio_w, buffer->str, buffer->len, &length_out, &gerr);
        while (status == G_IO_STATUS_AGAIN);
        if (status != G_IO_STATUS_NORMAL)
            break;
