...
static void test_thread_functions(void)
{
    ...
    DEBUG_MSG(("[funcs] Setting max unused threads to %d", max_unused_threads));
    g_thread_pool_set_max_unused_threads(max_unused_threads);

    DEBUG_MSG(("[funcs] Getting max unused threads = %d", g_thread_pool_get_max_unused_threads()));
    g_assert(g_thread_pool_get_max_unused_threads() == max_unused_threads);

    DEBUG_MSG(("[funcs] Getting num unused threads = %d", g_thread_pool_get_num_unused_threads()));
    ...
    DEBUG_MSG(("[funcs] Setting max idle time to %d", max_idle_time));
    g_thread_pool_set_max_idle_time(max_idle_time);

    DEBUG_MSG(("[funcs] Getting max idle time = %d", g_thread_pool_get_max_idle_time()));
...
static void test_thread_stop_unused(void)
{
    GThreadPool *pool;
    guint i;
    guint limit = 100;
    ...
    DEBUG_MSG(("[unused] ===> pushed %d threads onto the idle pool", limit));
    ...
    DEBUG_MSG(("[unused] stopped idle threads, %d remain", g_thread_pool_get_num_unused_threads()));
...
static void test_thread_pools_entry_func(gpointer data, gpointer user_data)
{
#ifdef DEBUG
    guint id = 0;
    ...
    DEBUG_MSG(("[pool] ---> [%3.3d] exiting thread (abs count:%ld, " "running count:%ld, left over:%ld)", id, abs_thread_counter, running_thread_counter, leftover_task_counter));
    G_UNLOCK(thread_counter_pools);
}

static void test_thread_pools(void)
{
...
static gint test_thread_sort_compare_func(gconstpointer a, gconstpointer b, gpointer user_data)
{
    guint32 id1, id2;
...
static void test_thread_sort_entry_func(gpointer data, gpointer user_data)
{
    guint thread_id;
    gboolean is_sorted;
    ...
    DEBUG_MSG(("%s ---> entered thread:%2.2d, last thread:%2.2d", is_sorted ? "[  sorted]" : "[unsorted]", thread_id, last_thread_id));
...
static void test_thread_sort(gboolean sort)
{
    GThreadPool *pool;
    guint limit;
    guint max_threads;
    ...
    pool = g_thread_pool_new(test_thread_sort_entry_func, GINT_TO_POINTER(sort), max_threads, FALSE, NULL);

    g_thread_pool_set_max_unused_threads(MAX_UNUSED_THREADS);

    if (sort)
    {
        g_thread_pool_set_sort_function(pool, test_thread_sort_compare_func, GUINT_TO_POINTER(69));
    }
    ...
    for (i = 0; i < limit; i++)
    {
        ...
        DEBUG_MSG(("%s ===> pushed new thread with id:%d, number " "of threads:%d, unprocessed:%d", sort ? "[  sorted]" : "[unsorted]", id, g_thread_pool_get_num_threads(pool), g_thread_pool_unprocessed(pool)));
    }
...
static void test_thread_idle_time_entry_func(gpointer data, gpointer user_data)
{
#ifdef DEBUG
    guint thread_id;
...
static gboolean test_thread_idle_timeout(gpointer data)
{
    gint i;
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        DEBUG_MSG(("[idle] ===> pushed new thread with id:%d, number " "of threads:%d, unprocessed:%d", 100 + i, g_thread_pool_get_num_threads(idle_pool), g_thread_pool_unprocessed(idle_pool)));
    }

    return FALSE;
}

static void test_thread_idle_time(void)
{
    guint limit = 50;
    guint interval = 10000;
    gint i;

    idle_pool = g_thread_pool_new(test_thread_idle_time_entry_func, NULL, 0, FALSE, NULL);
    ...
    for (i = 0; i < limit; i++)
    {
        ...
        DEBUG_MSG(("[idle] ===> pushed new thread with id:%d, " "number of threads:%d, unprocessed:%d", i, g_thread_pool_get_num_threads(idle_pool), g_thread_pool_unprocessed(idle_pool)));
    }

    g_assert_cmpint(g_thread_pool_unprocessed(idle_pool), <=, limit);

    g_timeout_add((interval - 1000), test_thread_idle_timeout, GUINT_TO_POINTER(interval));
}

static gboolean test_check_start_and_stop(gpointer user_data)
{
    ...
    if (test_number == 3)
    {
        ...
        DEBUG_MSG(("***** POOL RUNNING THREAD COUNT:%ld", running_thread_counter));
        G_UNLOCK(thread_counter_pools);
    }
    ...
    if (test_number == 4 || test_number == 5)
    {
        ...
        DEBUG_MSG(("***** POOL SORT THREAD COUNT:%ld", sort_thread_counter));
    ...
    if (test_number == 7)
    {
        ...
        quit &= idle < 1;
        DEBUG_MSG(("***** POOL IDLE THREAD COUNT:%d, UNPROCESSED JOBS:%d", idle, g_thread_pool_unprocessed(idle_pool)));
    }
