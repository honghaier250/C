...
static void fill_memory(guint ** mem, guint n, guint val)
{
    guint j, o = 0;
    for (j = 0; j < n; j++)
        mem[j][o] = val;
...
static guint64 access_memory3(guint ** mema, guint ** memb, guint ** memd, guint n, guint64 repeats)
{
    guint64 accu = 0, i, j;
    const guint o = 0;
    for (i = 0; i < repeats; i++)
...
static void touch_mem(guint64 block_size, guint64 n_blocks, guint64 repeats)
{
