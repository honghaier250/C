...
static gboolean read_all(GIOChannel * channel, char *buf, gsize len)
{
    gsize bytes_read = 0;
    gsize count;
    GIOError err;
...
static gboolean write_all(GIOChannel * channel, char *buf, gsize len)
{
...
static gboolean adder_callback(GIOChannel * source, GIOCondition condition, gpointer data)
{
    char buf1[32];
    char buf2[32];
    ...
    if (!read_all(source, buf1, 32) || !read_all(source, buf2, 32))
    {
...
static gpointer adder_thread(gpointer data)
{
...
static void do_add(GIOChannel * in, gint a, gint b)
{
    char buf1[32] = { 0, };
    char buf2[32] = { 0, };
...
static gboolean adder_response(GIOChannel * source, GIOCondition condition, gpointer data)
{
    char result[32];
...
static void create_adder_thread(void)
{
    GError *err = NULL;
    TestData *test_data;
...
static void remove_crawler(void)
{
...
static gint crawler_callback(gpointer data)
{
    GSource *source = data;
...
static gboolean recurser_idle(gpointer data)
{
...
static gboolean recurser_start(gpointer data)
{
    GMainContext *context;
    GSource *source;
