...
static gint sort_compare(gconstpointer p1, gconstpointer p2, gpointer user_data)
{
    gint32 id1;
    gint32 id2;
    ...
    DEBUG_MSG(("comparing #1:%d and #2:%d, returning %d", id1, id2, (id1 > id2 ? +1 : id1 == id2 ? 0 : -1)));
...
static void enter_thread(gpointer data, gpointer user_data)
{
    ...
    DEBUG_MSG(("thread id:%d added to async queue (size:%d)", id, len));
}
...
int main(int argc, char *argv[])
{
    ...
    PRINT_MSG(("creating thread pool with max threads:%d, max unused threads:%d...", max_threads, max_unused_threads));
    ...
    PRINT_MSG(("%s %d %s %d ms", msg, sort_multiplier, sort_multiplier == 1 ? "time in" : "times, once every", sort_interval));
