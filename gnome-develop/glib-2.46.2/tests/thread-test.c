...
static gpointer test_g_mutex_thread(gpointer data)
{
...
static void test_g_mutex(void)
{
    ...
    thread = g_thread_create(test_g_mutex_thread, GINT_TO_POINTER(42), TRUE, NULL);
...
static gpointer test_g_static_rec_mutex_thread(gpointer data)
{
    g_assert(GPOINTER_TO_INT(data) == 42);
    g_assert(g_static_rec_mutex_trylock(&test_g_static_rec_mutex_mutex) == FALSE);
...
static void test_g_static_rec_mutex(void)
{
    GThread *thread;

    g_assert(g_static_rec_mutex_trylock(&test_g_static_rec_mutex_mutex));
    test_g_static_rec_mutex_thread_ready = FALSE;
    thread = g_thread_create(test_g_static_rec_mutex_thread, GINT_TO_POINTER(42), TRUE, NULL);
    /* This busy wait is only for testing purposes and not an example of
     * good code!*/
...
static gpointer test_g_static_private_constructor(void)
{
...
static void test_g_static_private_destructor(gpointer data)
{
    g_mutex_lock(&test_g_static_private_mutex);
    test_g_static_private_counter--;
    g_mutex_unlock(&test_g_static_private_mutex);
...
static gpointer test_g_static_private_thread(gpointer data)
{
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        if (!private1 || number % 7 > 3)
        {
            ...
            g_static_private_set(&test_g_static_private_private1, private1, test_g_static_private_destructor);
        }
        *private1 = number;
        ...
        if (!private2 || number % 13 > 5)
        {
            ...
            g_static_private_set(&test_g_static_private_private2, private2, test_g_static_private_destructor);
        }
        *private2 = number * 2;
        g_usleep(G_USEC_PER_SEC / 5);
...
static void test_g_static_private(void)
{
    ...
    for (i = 0; i < THREADS; i++)
    {
        threads[i] = g_thread_create(test_g_static_private_thread, GINT_TO_POINTER(i), TRUE, NULL);
    }
...
static gpointer test_g_static_rw_lock_thread(gpointer data)
{
    while (test_g_static_rw_lock_run)
    {
        if (g_random_double() > .2)    /* I'm a reader */
        {
            ...
            else /* I'll only try */ if (!g_static_rw_lock_reader_trylock(&test_g_static_rw_lock_lock))
                continue;
        ...
        else                    /* I'm a writer */
        {
            ...
            else /* I'll only try */ if (!g_static_rw_lock_writer_trylock(&test_g_static_rw_lock_lock))
                continue;
            G_LOCK(test_g_static_rw_lock_state);
            g_assert(test_g_static_rw_lock_state == 0);
...
static void test_g_static_rw_lock(void)
{
    GThread *threads[THREADS];
    guint i;
    for (i = 0; i < THREADS; i++)
    {
        threads[i] = g_thread_create(test_g_static_rw_lock_thread, NULL, TRUE, NULL);
    }
    g_usleep(G_USEC_PER_SEC * 5);
...
static gpointer test_g_once_init_func(gpointer arg)
{
    guint *count = arg;
    g_usleep(g_random_int_range(20, 1000));
    (*count)++;
...
static gpointer test_g_once_thread(gpointer ignore)
{
    guint i;
    G_LOCK(test_g_once);
...
static void test_g_thread_once(void)
{
    ...
    {
        threads[i] = g_thread_create(test_g_once_thread, GUINT_TO_POINTER(i % 2), TRUE, NULL);
    }
    G_UNLOCK(test_g_once);
    for (i = 0; i < G_ONCE_THREADS; i++)
    {
