...
static void shuffle(GSource ** sources, int num)
{
    int i, a, b;
    GSource *tmp;
...
static void thread_pool_attach_func(gpointer data, gpointer user_data)
{
...
static void thread_pool_destroy_func(gpointer data, gpointer user_data)
{
    GSource *source = data;
...
int main(int argc, char **argv)
{
    ...
    pool = g_thread_pool_new(thread_pool_attach_func, context, 20, TRUE, NULL);
    start = g_get_monotonic_time();
    for (i = 0; i < NSOURCES; i++)
    {
        sources[i] = g_idle_source_new();
    ...
    pool = g_thread_pool_new(thread_pool_destroy_func, context, 20, TRUE, NULL);
    start = g_get_monotonic_time();
    for (i = 0; i < NSOURCES; i++)
    {
        g_thread_pool_push(pool, sources[i], &error);
