...
/* --- functions --- */
static gint my_list_compare_one(gconstpointer a, gconstpointer b)
{
...
static gint my_list_compare_two(gconstpointer a, gconstpointer b)
{
    gint one = *((const gint *) a);
    gint two = *((const gint *) b);
    return two - one;
...
static gboolean find_first_that(gpointer key, gpointer value, gpointer user_data)
{
    gint *v = value;
    gint *test = user_data;
    return (*v == *test);
...
static void test_g_parse_debug_string(void)
{
    GDebugKey keys[] = {
        {"foo", 1},
        {"bar", 2},
...
static void log_warning_error_tests(void)
{
    g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_MESSAGE, "*is a g_message test*");
    g_message("this is a g_message test.");
    g_test_assert_expected_messages();

    g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_MESSAGE, "*non-printable UTF-8*");
    g_message("non-printable UTF-8: \"\xc3\xa4\xda\x85\"");
    g_test_assert_expected_messages();

    g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_MESSAGE, "*unsafe chars*");
    g_message("unsafe chars: \"\x10\x11\x12\n\t\x7f\x81\x82\x83\"");
    g_test_assert_expected_messages();

    g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_WARNING, "*harmless warning*");
    g_warning("harmless warning with parameters: %d %s %#x", 42, "Boo", 12345);
    g_test_assert_expected_messages();

    g_test_expect_message(G_LOG_DOMAIN, G_LOG_LEVEL_CRITICAL, "*g_print*assertion*failed*");
    g_print(NULL);
    g_test_assert_expected_messages();
}
...
static void type_sizes(void)
{
    guint16 gu16t1 = 0x44afU, gu16t2 = 0xaf44U;
    guint32 gu32t1 = 0x02a7f109U, gu32t2 = 0x09f1a702U;
    guint64 gu64t1 = G_GINT64_CONSTANT(0x1d636b02300a7aa7U), gu64t2 = G_GINT64_CONSTANT(0xa77a0a30026b631dU);
    /* type sizes */
...
static void test_info(void)
{
    ...
    if (g_test_verbose())
        g_printerr("TestGLib v%u.%u.%u (i:%u b:%u)\n", glib_major_version, glib_minor_version, glib_micro_version, glib_interface_age, glib_binary_age);
...
static void test_paths(void)
{
    ...
    for (i = 0; i < n_skip_root_checks; i++)
    {
        ...
        if ((skipped && !skip_root_checks[i].without_root) || (!skipped && skip_root_checks[i].without_root) || ((skipped && skip_root_checks[i].without_root) && strcmp(skipped, skip_root_checks[i].without_root)))
        {
            g_error("\nfailed for \"%s\"==\"%s\" (returned: \"%s\")\n", skip_root_checks[i].filename, (skip_root_checks[i].without_root ? skip_root_checks[i].without_root : "<NULL>"), (skipped ? skipped : "<NULL>"));
        }
    }
    if (g_test_verbose())
        g_printerr("ok\n");
...
static void test_file_functions(void)
{
...
static void hash_table_tests(void)
{
...
static void relation_test(void)
{
...
#    if 0
    ...
    {
        printf("%d %d\n", *(gint *) g_tuples_index(tuples, i, 0), *(gint *) g_tuples_index(tuples, i, 1));
    }
#    endif
...
static void gstring_tests(void)
{
    ...
    g_string_printf(string2, "%s|%0100d|%s|%s|%0*d|%*.*f|%10000.10000f", "this pete guy sure is a wuss, like he's the number ", 1, " wuss.  everyone agrees.\n", string1->str, 10, 666, 15, 15, 666.666666666, 666.666666666);
#else
    g_string_printf(string2, "%s|%0100d|%s|%s|%0*d|%*.*f|%100.100f", "this pete guy sure is a wuss, like he's the number ", 1, " wuss.  everyone agrees.\n", string1->str, 10, 666, 15, 15, 666.666666666, 666.666666666);
#endif
...
static void various_string_tests(void)
{
...
#define REF_STR_DATE_ONLY "1980-02-22"
    ...
    if (g_test_verbose())
        g_printerr("\t=> UTC stamp = %ld.%06ld (should be: %ld.%06ld) (%ld.%06ld off)\n", date.tv_sec, date.tv_usec, ref_date.tv_sec, ref_date.tv_usec, date.tv_sec - ref_date.tv_sec, date.tv_usec - ref_date.tv_usec);
    ...
    g_assert(strcmp(string, GLIB_TEST_STRING) == 0);
    g_free(string);
    string = g_strconcat(GLIB_TEST_STRING, GLIB_TEST_STRING, GLIB_TEST_STRING, NULL);
    g_assert(string != NULL);
    g_assert(strcmp(string, GLIB_TEST_STRING GLIB_TEST_STRING GLIB_TEST_STRING) == 0);
    g_free(string);
...
#ifndef G_DISABLE_DEPRECATED
static void test_mem_chunks(void)
{
