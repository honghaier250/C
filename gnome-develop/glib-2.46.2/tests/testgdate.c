...
static void g_date_debug_print(GDate * d)
{
    if (!d)
        g_print("NULL!\n");
    else
        g_print("julian: %u (%s) DMY: %u %u %u (%s)\n", d->julian_days, d->julian ? "valid" : "invalid", d->day, d->month, d->year, d->dmy ? "valid" : "invalid");
...
int main(int argc, char **argv)
{
    ...
    TEST("Successfully parsed a %x-formatted string", g_date_valid(d) && g_date_get_month(d) == 1 && g_date_get_day(d) == 10 && g_date_get_year(d) == 2000);
    if (failed)
    ...
    while (i < n_check_years)
    {
        ...
        TEST("Number of Sunday weeks in year is 52 or 53", sunday_weeks_in_year == 52 || sunday_weeks_in_year == 53);

        TEST("Number of Monday weeks in year is 52 or 53", monday_weeks_in_year == 52 || monday_weeks_in_year == 53);
        ...
        while (m < 13)
        {
            ...
            while (day <= dim)
            {
                ...
                TEST("Day of year is consistent with Julian dates", ((g_date_get_julian(d) + 1 - first_day_of_year) == (g_date_get_day_of_year(d))));

                if (failed)
                {
                    g_print("first day: %u this day: %u day of year: %u\n", first_day_of_year, g_date_get_julian(d), g_date_get_day_of_year(d));
                }

                if (m == G_DATE_DECEMBER && day == 31)
                {
                    TEST("Last day of year equals number of days in year", g_date_get_day_of_year(d) == days_in_year);
                    if (failed)
                    {
                ...
                TEST("Day of year is not more than number of days in the year", g_date_get_day_of_year(d) <= days_in_year);

                TEST("Monday week of year is not more than number of weeks in the year", g_date_get_monday_week_of_year(d) <= monday_weeks_in_year);
                if (failed)
                {
                    g_print("Weeks in year: %u\n", monday_weeks_in_year);
                    g_date_debug_print(d);
                }
                TEST("Monday week of year is >= than last week of year", g_date_get_monday_week_of_year(d) >= monday_week_of_year);

                if (g_date_get_weekday(d) == G_DATE_MONDAY)
                {

                    TEST("Monday week of year on Monday 1 more than previous day's week of year", (g_date_get_monday_week_of_year(d) - monday_week_of_year) == 1);
                    if ((m == G_DATE_JANUARY && day <= 4) || (m == G_DATE_DECEMBER && day >= 29))
                    {
                        TEST("ISO 8601 week of year on Monday Dec 29 - Jan 4 is 1", (g_date_get_iso8601_week_of_year(d) == 1));
                    }
                    else
                    {
                        TEST("ISO 8601 week of year on Monday 1 more than previous day's week of year", (g_date_get_iso8601_week_of_year(d) - iso8601_week_of_year) == 1);
                    }
                }
                else
                {
                    TEST("Monday week of year on non-Monday 0 more than previous day's week of year", (g_date_get_monday_week_of_year(d) - monday_week_of_year) == 0);
                    if (!(day == 1 && m == G_DATE_JANUARY))
                    {
                        TEST("ISO 8601 week of year on non-Monday 0 more than previous day's week of year (", (g_date_get_iso8601_week_of_year(d) - iso8601_week_of_year) == 0);
                    }
                }
                ...
                TEST("Sunday week of year is not more than number of weeks in the year", g_date_get_sunday_week_of_year(d) <= sunday_weeks_in_year);
                if (failed)
                {
                    g_date_debug_print(d);
                }
                TEST("Sunday week of year is >= than last week of year", g_date_get_sunday_week_of_year(d) >= sunday_week_of_year);

                if (g_date_get_weekday(d) == G_DATE_SUNDAY)
                {
                    TEST("Sunday week of year on Sunday 1 more than previous day's week of year", (g_date_get_sunday_week_of_year(d) - sunday_week_of_year) == 1);
                }
                else
                {
                    TEST("Sunday week of year on non-Sunday 0 more than previous day's week of year", (g_date_get_sunday_week_of_year(d) - sunday_week_of_year) == 0);
                }

                sunday_week_of_year = g_date_get_sunday_week_of_year(d);

                TEST("Date is equal to itself", g_date_compare(d, d) == 0);
          ...
          /*************** Increments ***********/
                ...
                while (k < 402)    /* Need to get 400 year increments in */
                {
                  ...
                  /***** Days ******/
                    ...
                    TEST("Adding days gives a value greater than previous", g_date_compare(d, &tmp) > 0);

                    g_date_subtract_days(d, k);
                    TEST("Forward days then backward days returns us to current day", g_date_get_day(d) == day);
                    ...
                    TEST("Forward days then backward days returns us to current month", g_date_get_month(d) == m);
                    ...
                    TEST("Forward days then backward days returns us to current year", g_date_get_year(d) == y);
                  ...
                  /******* Months ********/
                    ...
                    TEST("Adding months gives a larger value", g_date_compare(d, &tmp) > 0);
                    g_date_subtract_months(d, k);

                    TEST("Forward months then backward months returns us to current month", g_date_get_month(d) == m);
                    ...
                    TEST("Forward months then backward months returns us to current year", g_date_get_year(d) == y);
                    ...
                    if (day < 29)
                    {
                        ...
                        TEST("Forward months then backward months returns us to current day", g_date_get_day(d) == day);
                  ...
                  /******* Years ********/
                    ...
                    TEST("Adding years gives a larger value", g_date_compare(d, &tmp) > 0);

                    g_date_subtract_years(d, k);

                    TEST("Forward years then backward years returns us to current month", g_date_get_month(d) == m);
                    ...
                    TEST("Forward years then backward years returns us to current year", g_date_get_year(d) == y);
                    ...
                    if (m != 2 && day != 29)
                    {
                        TEST("Forward years then backward years returns us to current day", g_date_get_day(d) == day);
          ...
          /*****  increment test relative to our local Julian count */
                ...
                if (!discontinuity)
                {
                    ...
                    TEST("Julians are sequential with increment 1", j + 1 == g_date_get_julian(d));
                    if (failed)
                    {
                    ...
                    g_date_add_days(d, 1);
                    TEST("Next day has julian 1 higher", g_date_get_julian(d) == j + 2);
                    ...
                    if (j != G_DATE_BAD_JULIAN)
                    {
                        ...
                        TEST("Previous day has julian 1 lower", g_date_get_julian(d) == j);
        ...
        prev_y = y;
        y = check_years[i];
        if (prev_y == G_DATE_BAD_YEAR || (prev_y + 1) != y)
            discontinuity = TRUE;
    }
