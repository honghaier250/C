...
static void my_test_set_property(GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void my_test_get_property(GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
...
static void my_test_class_init(GTestClass * klass)
{
    ...
    my_test_signals[TEST_SIGNAL1] = g_signal_new("test-signal1", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GTestClass, test_signal1), NULL, NULL, g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
    my_test_signals[TEST_SIGNAL2] = g_signal_new("test-signal2", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GTestClass, test_signal2), NULL, NULL, g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);
    my_test_signals[TEST_SIGNAL3] = g_signal_new("test-signal3", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(GTestClass, test_signal3), NULL, NULL, g_cclosure_marshal_generic, G_TYPE_STRING, 1, G_TYPE_INT);

    g_object_class_install_property(G_OBJECT_CLASS(klass), ARG_TEST_PROP, g_param_spec_int("test-prop", "Test Prop", "Test property", 0, 1, 0, G_PARAM_READWRITE));
...
static void my_test_do_signal1(GTest * test)
{
...
static void my_test_do_signal2(GTest * test)
{
...
static void my_test_do_signal3(GTest * test)
{
    gchar *res;
...
static void my_test_do_prop(GTest * test)
{
    test->value = g_rand_int(rand);
    g_object_notify(G_OBJECT(test), "test-prop");
}

static gpointer run_thread(GTest * test)
{
    gint i = 1;
...
static void notify(GObject * object, GParamSpec * spec, gpointer user_data)
{
    gint value;
