...
static gboolean check_stop(gpointer data)
{
    GMainLoop *loop = data;
...
static void write_or_die(const gchar * filename, const gchar * contents, gssize length)
{
    GError *error = NULL;
    gchar *displayname;
...
static GMappedFile *map_or_die(const gchar * filename, gboolean writable)
{
    GError *error = NULL;
    GMappedFile *map;
    gchar *displayname;
...
static gboolean signal_parent(gpointer data)
{
#ifndef G_OS_WIN32
...
static int child_main(int argc, char *argv[])
{
    ...
    write_or_die(childname, g_mapped_file_get_contents(map), g_mapped_file_get_length(map));
...
static void test_child_private(gchar * argv0)
{
    GError *error = NULL;
    GMappedFile *map;
    gchar *buffer;
    ...
    child_argv[2] = pid;
    child_argv[3] = NULL;
    if (!g_spawn_async(dir, child_argv, NULL, 0, NULL, NULL, &child_pid, &error))
    {
