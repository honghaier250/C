...
static gboolean ucs4_equal(gunichar * a, gunichar * b)
{
    while (*a && *b && (*a == *b))
    {
        a++;
...
static gboolean utf16_equal(gunichar2 * a, gunichar2 * b)
{
    while (*a && *b && (*a == *b))
    {
        a++;
...
static gint utf16_count(gunichar2 * a)
{
    gint result = 0;
...
static void print_ucs4(const gchar * prefix, gunichar * ucs4, gint ucs4_len)
{
    gint i;
    g_print("%s ", prefix);
...
static void process(gint line, gchar * utf8, Status status, gunichar * ucs4, gint ucs4_len)
{
    const gchar *end;
    ...
    if (status == VALID || status == NOTUNICODE)
    {
        ...
        if (!ucs4_equal(ucs4_result, ucs4) || items_read != strlen(utf8) || items_written != ucs4_len)
        {
    ...
    if (status == VALID)
    {
        ...
        if (!ucs4_equal(ucs4_result, ucs4) || items_written != ucs4_len)
        {
        ...
        if (strcmp(utf8_result, utf8) != 0 || items_read != ucs4_len || items_written != strlen(utf8))
        {
...
#    define TARGET "UTF-16"
        ...
        if (!(utf16_expected_tmp = (gunichar2 *) g_convert(utf8, -1, TARGET, "UTF-8", NULL, &bytes_written, NULL)))
        {
        ...
        if (items_read != strlen(utf8) || utf16_count(utf16_from_utf8) != items_written)
        {
        ...
        if (items_read != ucs4_len || utf16_count(utf16_from_ucs4) != items_written)
        {
            fail("line %d: length error in conversion to ucs16\n", line);
            return;
        }

        if (!utf16_equal(utf16_from_utf8, utf16_expected) || !utf16_equal(utf16_from_ucs4, utf16_expected))
        {
        ...
        if (items_read != utf16_count(utf16_from_utf8) || items_written != strlen(utf8))
        {
        ...
        if (items_read != utf16_count(utf16_from_utf8) || items_written != ucs4_len)
        {
            fail("line %d: length error in conversion from ucs16 to ucs4\n", line);
            return;
        }

        if (strcmp(utf8, utf8_result) != 0 || !ucs4_equal(ucs4, ucs4_result))
        {
