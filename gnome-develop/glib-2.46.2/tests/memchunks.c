...
GMemChunk *old_mem_chunk_new(const gchar * name, gint atom_size, gulong area_size, gint type);
...
void old_mem_chunk_free(GMemChunk * mem_chunk, gpointer mem);
...
static gulong old_mem_chunk_compute_size(gulong size, gulong min_size) G_GNUC_CONST;
     static gint old_mem_chunk_area_compare(GMemArea * a, GMemArea * b);
     static gint old_mem_chunk_area_search(GMemArea * a, gchar * addr);
...
gpointer old_mem_chunk_alloc(GMemChunk * mem_chunk)
{
    ...
    while (mem_chunk->free_atoms)
    {
        ...
        temp_area = g_tree_search(mem_chunk->mem_tree, (GCompareFunc) old_mem_chunk_area_search, mem);
    ...
    if ((!mem_chunk->mem_area) || ((mem_chunk->mem_area->index + mem_chunk->atom_size) > mem_chunk->area_size))
    {
        ...
        else
        {
            ...
            mem_chunk->mem_area = (GMemArea *) g_malloc0(sizeof(GMemArea) - MEM_AREA_SIZE + mem_chunk->area_size);
#else                            /* !ENABLE_GC_FRIENDLY */
            mem_chunk->mem_area = (GMemArea *) g_malloc(sizeof(GMemArea) - MEM_AREA_SIZE + mem_chunk->area_size);
...
void old_mem_chunk_clean(GMemChunk * mem_chunk)
{
    ...
    if (mem_chunk->type == G_ALLOC_AND_FREE)
    {
        ...
        while (temp_free_atom)
        {
            ...
            mem_area = g_tree_search(mem_chunk->mem_tree, (GCompareFunc) old_mem_chunk_area_search, mem);
...
void old_mem_chunk_print(GMemChunk * mem_chunk)
{
    ...
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_INFO, "%s: %ld bytes using %d mem areas", mem_chunk->name, mem, mem_chunk->num_mem_areas);
}
...
static gulong old_mem_chunk_compute_size(gulong size, gulong min_size)
{
    gulong power_of_2;
    gulong lower, upper;
