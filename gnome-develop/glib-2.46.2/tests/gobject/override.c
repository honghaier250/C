...
static void test_i_foo(TestI * self)
{
    record("TestI::foo");
}

static void test_i_default_init(gpointer g_class)
{
    foo_signal_id = g_signal_newv("foo", TEST_TYPE_I, G_SIGNAL_RUN_LAST, g_cclosure_new(G_CALLBACK(test_i_foo), NULL, NULL), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, NULL);
}
...
#define TEST_TYPE_A (test_a_get_type())
     ...
     static void test_a_foo(TestI * self)
{
...
static void test_a_bar(TestA * self)
{
    record("TestA::bar");
}

static gchar *test_a_baz(TestA * self, GObject * object, gpointer pointer)
{
    record("TestA::baz");
...
static void test_a_class_init(TestAClass * class)
{
    class->bar = test_a_bar;

    bar_signal_id = g_signal_new("bar", TEST_TYPE_A, G_SIGNAL_RUN_LAST, G_STRUCT_OFFSET(TestAClass, bar), NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0, NULL);

    baz_signal_id = g_signal_new_class_handler("baz", TEST_TYPE_A, G_SIGNAL_RUN_LAST, G_CALLBACK(test_a_baz), NULL, NULL, g_cclosure_marshal_STRING__OBJECT_POINTER, G_TYPE_STRING, 2, G_TYPE_OBJECT, G_TYPE_POINTER);
}

static void test_a_interface_init(TestIClass * iface)
{
    g_signal_override_class_closure(foo_signal_id, TEST_TYPE_A, g_cclosure_new(G_CALLBACK(test_a_foo), NULL, NULL));
}

static DEFINE_TYPE_FULL(TestA, test_a, test_a_class_init, NULL, NULL, G_TYPE_OBJECT, INTERFACE(test_a_interface_init, TEST_TYPE_I))
#define TEST_TYPE_B (test_b_get_type())
     ...
     static void test_b_foo(TestI * self)
{
    GValue args[1] = { G_VALUE_INIT };
...
static void test_b_bar(TestA * self)
{
    GValue args[1] = { G_VALUE_INIT };
...
static gchar *test_b_baz(TestA * self, GObject * object, gpointer pointer)
{
    gchar *retval = NULL;
...
static void test_b_class_init(TestBClass * class)
{
    g_signal_override_class_closure(foo_signal_id, TEST_TYPE_B, g_cclosure_new(G_CALLBACK(test_b_foo), NULL, NULL));
    g_signal_override_class_closure(bar_signal_id, TEST_TYPE_B, g_cclosure_new(G_CALLBACK(test_b_bar), NULL, NULL));
    g_signal_override_class_handler("baz", TEST_TYPE_B, G_CALLBACK(test_b_baz));
}

static DEFINE_TYPE(TestB, test_b, test_b_class_init, NULL, NULL, TEST_TYPE_A)
#define TEST_TYPE_C (test_c_get_type())
     ...
     static void test_c_foo(TestI * self)
{
    GValue args[1] = { G_VALUE_INIT };
...
static void test_c_bar(TestA * self)
{
    GValue args[1] = { G_VALUE_INIT };
...
static gchar *test_c_baz(TestA * self, GObject * object, gpointer pointer)
{
    gchar *retval = NULL;
...
static void test_c_class_init(TestBClass * class)
{
    g_signal_override_class_closure(foo_signal_id, TEST_TYPE_C, g_cclosure_new(G_CALLBACK(test_c_foo), NULL, NULL));
    g_signal_override_class_closure(bar_signal_id, TEST_TYPE_C, g_cclosure_new(G_CALLBACK(test_c_bar), NULL, NULL));
    g_signal_override_class_handler("baz", TEST_TYPE_C, G_CALLBACK(test_c_baz));
}

static DEFINE_TYPE(TestC, test_c, test_c_class_init, NULL, NULL, TEST_TYPE_B)
...
static void test(GType type, const gchar * signal, const gchar * expected, const gchar * expected_retval)
{
...
#endif
    {
        g_printerr("*** emitting %s on a %s instance\n" "    Expecting: %s\n" "    Got: %s\n", signal, g_type_name(type), expected, test_string->str);
