...
/* --- methods --- */
static GObject *my_singleton_constructor(GType type, guint n_construct_properties, GObjectConstructParam * construct_properties)
{
    if (the_one_and_only)
...
static void my_singleton_init(MySingleton * self)
{
    g_assert(the_one_and_only == NULL);
    the_one_and_only = self;
}

static void my_singleton_class_init(MySingletonClass * klass)
{
