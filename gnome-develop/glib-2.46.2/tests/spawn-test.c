...
static void run_tests(void)
{
    ...
    err = NULL;
    if (!g_spawn_command_line_sync("nonexistent_application foo 'bar baz' blah blah", NULL, NULL, NULL, &err))
    {
        g_error_free(err);
    }
    else
    ...
    err = NULL;
    if (!g_spawn_command_line_async("nonexistent_application foo bar baz \"blah blah\"", &err))
    {
        g_error_free(err);
    }
    else
    ...
    err = NULL;
#ifdef G_OS_UNIX
    if (!g_spawn_command_line_sync("/bin/sh -c 'echo hello'", &output, NULL, NULL, &err))
    {
...
#    ifdef G_OS_WIN32
    ...
    if (!g_spawn_command_line_sync("netstat -n", &output, &erroutput, NULL, &err))
    {
        fprintf(stderr, "Error: %s\n", err->message);
        g_error_free(err);
        exit(1);
    ...
    if (!g_spawn_command_line_sync("'.\\spawn-test-win32-gui.exe' 2", &output, &erroutput, NULL, &err))
    {
        fprintf(stderr, "Error: %s\n", err->message);
        g_error_free(err);
        exit(1);
    ...
    if (!g_spawn_async(NULL, argv, NULL, G_SPAWN_FILE_AND_ARGV_ZERO, NULL, NULL, NULL, &err))
    {
        fprintf(stderr, "Error: %s\n", err->message);
        g_error_free(err);
        exit(1);
    ...
    if (pipe(pipedown) < 0 || pipe(pipeup) < 0)
    {
        fprintf(stderr, "Could not create pipes\n");
        exit(1);
    }

    if (!g_shell_parse_argv(g_strdup_printf("'.\\spawn-test-win32-gui.exe' pipes %d %d", pipedown[0], pipeup[1]), NULL, &argv, &err))
    {
        fprintf(stderr, "Error parsing command line? %s\n", err->message);
        g_error_free(err);
        exit(1);
    ...
    if (!g_spawn_async(NULL, argv, NULL, G_SPAWN_LEAVE_DESCRIPTORS_OPEN | G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL, NULL, &err))
    {
        fprintf(stderr, "Error: %s\n", err->message);
        g_error_free(err);
        exit(1);
    ...
    else
    {
        ...
        n = strlen("Bye then");
        if (write(pipedown[1], &n, sizeof(n)) == -1 || write(pipedown[1], "Bye then", n) == -1)
        {
