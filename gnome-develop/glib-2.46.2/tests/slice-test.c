...
/* --- functions --- */
static inline int corruption(void)
{
...
static inline gpointer memchunk_alloc(GMemChunk ** memchunkp, guint size)
{
    size = MAX(size, 1);
...
static inline void memchunk_free(GMemChunk * memchunk, gpointer chunk)
{
...
static gpointer test_memchunk_thread(gpointer data)
{
    GMemChunk **memchunks;
    guint i, j;
    guint8 **ps;
...
static gpointer test_sliced_mem_thread(gpointer data)
{
    guint32 rand_accu = 2147483563;
    guint i, j;
    guint8 **ps;
