...
extern int libconfig_yyparse (void *scanner, struct parse_context *ctx, struct scan_context *scan_ctx);
extern int libconfig_yylex_init_extra (struct scan_context *scan_ctx, yyscan_t * scanner);
...
static void __config_write_value (const config_value_t * value, int type, int format, int depth, unsigned short tab_width, FILE * stream)
{
    char fbuf[64];
    ...
    switch (type)
	{
	    ...
	    case CONFIG_TYPE_LIST:
		{
		    ...
		    if (list)
			{
				...
				{
				    __config_write_value (&((*s)->value), (*s)->type, config_setting_get_format (*s), depth + 1, tab_width, stream);
...
static void __config_list_add (config_list_t * list, config_setting_t * setting)
{
	...
	{
	    list->elements = (config_setting_t **) realloc (list->elements, (list->length + CHUNK_SIZE) * sizeof (config_setting_t *));
	}
...
static config_setting_t *__config_list_search (config_list_t * list, const char *name, unsigned int *idx)
{
...
static config_setting_t *__config_list_remove (config_list_t * list, int idx)
{
    ...
    memmove (base, base + sizeof (config_setting_t *), len * sizeof (config_setting_t *));
...
static int __config_vector_checktype (const config_setting_t * vector, int type)
{
    ...
    return ((vector->value.list->elements[0]->type == type) ? CONFIG_TRUE : CONFIG_FALSE);
}
...
static int __config_read (config_t * config, FILE * stream, const char *filename, const char *str)
{
    yyscan_t scanner;
...
static config_setting_t *config_setting_create (config_setting_t * parent, const char *name, int type)
{
...
static int __config_setting_get_int (const config_setting_t * setting, int *value)
{
    switch (setting->type)
	{
	    case CONFIG_TYPE_INT:
...
static int __config_setting_get_int64 (const config_setting_t * setting, long long *value)
{
    switch (setting->type)
	{
	    case CONFIG_TYPE_INT64:
...
static int __config_setting_get_float (const config_setting_t * setting, double *value)
{
    switch (setting->type)
	{
	    case CONFIG_TYPE_FLOAT:
...
{
    return (setting->format != 0 ? setting->format : setting->config->default_format);
}
...
config_setting_t *config_setting_set_int_elem (config_setting_t * vector, int idx, int value)
{
...
config_setting_t *config_setting_set_int64_elem (config_setting_t * vector, int idx, long long value)
{
    config_setting_t *element = NULL;
...
config_setting_t *config_setting_set_float_elem (config_setting_t * vector, int idx, double value)
{
    config_setting_t *element = NULL;
...
config_setting_t *config_setting_set_bool_elem (config_setting_t * vector, int idx, int value)
{
    config_setting_t *element = NULL;
...
config_setting_t *config_setting_set_string_elem (config_setting_t * vector, int idx, const char *value)
{
    config_setting_t *element = NULL;
...
config_setting_t *config_setting_get_elem (const config_setting_t * vector, unsigned int idx)
{
