...
struct bst_table *bst_create (bst_comparison_func * compare, void *param, struct libavl_allocator *allocator)
{
...
void *bst_t_copy (struct bst_traverser *trav, const struct bst_traverser *src)
{
    ...
    if (trav != src)
	{
	    ...
	    if (trav->bst_generation == trav->bst_table->bst_generation)
		{
		    ...
		    memcpy (trav->bst_stack, (const void *) src->bst_stack, sizeof *trav->bst_stack * trav->bst_height);
		}
	}
...
static void copy_error_recovery (struct bst_node **stack, int height, struct bst_table *new, bst_item_func * destroy)
{
...
struct bst_table *bst_copy (const struct bst_table *org, bst_copy_func * copy, bst_item_func * destroy, struct libavl_allocator *allocator)
{
    ...
    assert (org != NULL);
    new = bst_create (org->bst_compare, org->bst_param, allocator != NULL ? allocator : org->bst_alloc);
    if (new == NULL)
	return NULL;
    ...
    for (;;)
	{
	    while (x->bst_link[0] != NULL)
		{
		    ...
		    y->bst_link[0] = new->bst_alloc->libavl_malloc (new->bst_alloc, sizeof *y->bst_link[0]);
	    ...
	    for (;;)
		{
			...
			{
			    y->bst_link[1] = new->bst_alloc->libavl_malloc (new->bst_alloc, sizeof *y->bst_link[1]);
...
static void vine_to_tree (struct bst_table *tree)
{
	...
	{
	    fprintf (stderr, "libavl: Tree too big (%lu nodes) to handle.", (unsigned long) tree->bst_count);
	    exit (EXIT_FAILURE);
	}
}
