...
static int yaml_parser_load_alias (yaml_parser_t * parser, yaml_event_t * first_event)
{
    ...
    yaml_free (anchor);
    return yaml_parser_set_composer_error (parser, "found undefined alias", first_event->start_mark);
}
...
static int yaml_parser_load_scalar (yaml_parser_t * parser, yaml_event_t * first_event)
{
    ...
    SCALAR_NODE_INIT (node, tag, first_event->data.scalar.value, first_event->data.scalar.length, first_event->data.scalar.style, first_event->start_mark, first_event->end_mark);
    ...
    if (!yaml_parser_register_anchor (parser, index, first_event->data.scalar.anchor))
    return 0;
...
static int yaml_parser_load_sequence (yaml_parser_t * parser, yaml_event_t * first_event)
{
    ...
    SEQUENCE_NODE_INIT (node, tag, items.start, items.end, first_event->data.sequence_start.style, first_event->start_mark, first_event->end_mark);
    ...
    if (!yaml_parser_register_anchor (parser, index, first_event->data.sequence_start.anchor))
    return 0;
    ...
    while (event.type != YAML_SEQUENCE_END_EVENT)
    {
        ...
        if (!item_index)
        return 0;
        if (!PUSH (parser, parser->document->nodes.start[index - 1].data.sequence.items, item_index))
        return 0;
        if (!yaml_parser_parse (parser, &event))
        return 0;
    }
...
static int yaml_parser_load_mapping (yaml_parser_t * parser, yaml_event_t * first_event)
{
    ...
    MAPPING_NODE_INIT (node, tag, pairs.start, pairs.end, first_event->data.mapping_start.style, first_event->start_mark, first_event->end_mark);
    ...
    if (!yaml_parser_register_anchor (parser, index, first_event->data.mapping_start.anchor))
    return 0;
    ...
    {
        if (!STACK_LIMIT (parser, parser->document->nodes.start[index - 1].data.mapping.pairs, INT_MAX - 1))
        return 0;
        ...
        if (!pair.value)
        return 0;
        if (!PUSH (parser, parser->document->nodes.start[index - 1].data.mapping.pairs, pair))
        return 0;
        if (!yaml_parser_parse (parser, &event))
        return 0;
    }
